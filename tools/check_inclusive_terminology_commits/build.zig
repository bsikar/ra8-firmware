...
//!
//! Build graph for the `check_inclusive_terminology_commits` host gate (#858). One
//! executable, plus the test step `scripts/checks/check_zig.py --test` runs:
//! the detector algebra, and the exit-status contract
//! `scripts/builders/check_inclusive_terminology_commits.sh` passes through.
...
pub fn build(b: *std.Build) void {
    ...
    const executable = b.addExecutable(.{
        .name = "check_inclusive_terminology_commits",
    ...
    const run_step = b.step("run", "Scan commit-message text for banned legacy terminology");
    ...
    const test_step = b.step("test", "Run check_inclusive_terminology_commits tests");
