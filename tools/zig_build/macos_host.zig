...
/// would otherwise answer for it, and only inside the first document, because a
/// re-exported library's document must not answer for the library we link.
pub fn tbdDeclaresTarget(tbd_text: []const u8, wanted: []const u8) bool {
    var lines = std.mem.splitScalar(u8, firstDocument(tbd_text), '\n');
    while (lines.next()) |raw_line| {
        const trimmed = std.mem.trim(u8, raw_line, " \t\r");
        // A block-item marker is the only dash a `targets:` field can carry.
        const line = if (std.mem.startsWith(u8, trimmed, "- ")) trimmed[2..] else trimmed;
...
/// The first YAML document of a `.tbd`, which is the library the linker resolves
...
fn firstDocument(tbd_text: []const u8) []const u8 {
    ...
    while (offset < tbd_text.len) {
        const newline = std.mem.indexOfScalarPos(u8, tbd_text, offset, '\n');
        const line_end = newline orelse tbd_text.len;
        const line = std.mem.trim(u8, tbd_text[offset..line_end], " \t\r");
        if (isDocumentBoundary(line)) {
            if (seen_header) return tbd_text[0..offset];
            seen_header = true;
        }
        offset = if (newline) |index| index + 1 else tbd_text.len;
    }
    return tbd_text;
}

/// A YAML document start (`--- !tapi-tbd`) or explicit end (`...`).
fn isDocumentBoundary(line: []const u8) bool {
    return std.mem.startsWith(u8, line, "---") or std.mem.eql(u8, line, "...");
}
