...
//!
//! Build graph for the `list_tests` host tool (#858). One executable, plus the
//! test step `scripts/checks/check_zig.py --test` runs: the listing
//! `just/tests.just` shows and the exit-status contract
//! `scripts/builders/list_tests.sh` passes through.
...
pub fn build(b: *std.Build) void {
    ...
    const run_step = b.step("run", "List one category's test targets");
    ...
    const test_step = b.step("test", "Run list_tests tests");
