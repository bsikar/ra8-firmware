...
static void internal_atlas(uint8_t atlas[k_test_atlas_size])
{
  ...
  (void)memcpy(&atlas[k_jof_ofs_magic], "JOF1", k_jof_magic_len);
  internal_u16(&atlas[k_jof_ofs_width], 2U);
  internal_u16(&atlas[k_jof_ofs_height], 2U);
  internal_u16(&atlas[k_jof_ofs_tile_w], 2U);
  internal_u16(&atlas[k_jof_ofs_tile_h], 1U);
  atlas[k_jof_ofs_bpp]   = 1U;
  atlas[k_jof_ofs_codec] = (uint8_t)k_jof_codec_raw;
  internal_u32(&atlas[k_jof_ofs_tile_count], k_test_tile_count);
  ...
  internal_u32(&atlas[k_test_index + k_jof_idx_ofs_length], k_test_tile_bytes);
  internal_u32(&atlas[k_test_index + k_jof_index_entry], k_test_payload_b);
  internal_u32(&atlas[k_test_index + k_jof_index_entry + k_jof_idx_ofs_length], k_test_tile_bytes);
  internal_u32(&atlas[k_test_footer + k_jof_ftr_index_off], k_test_index);
  internal_u32(&atlas[k_test_footer + k_jof_ftr_tile_count], k_test_tile_count);
  internal_u32(&atlas[k_test_footer + k_jof_ftr_total_size], k_test_atlas_size);
  (void)memcpy(&atlas[k_test_footer + k_jof_ftr_magic], "JOFE", k_jof_magic_len);
}
