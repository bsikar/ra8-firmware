...
typedef struct {
  alignas(max_align_t) uint8_t bytes[k_ra8_compress_scratch_bytes]; /**< tdefl state bytes. */
} test_compressor_t;
...
static ra8_err_t internal_flat_read(void*     opaque,
                                    ...
                                    uint32_t* out_read)
{
  const test_object_t* object = (test_object_t*)opaque;
  *out_read                   = 0U;
...
static uint32_t internal_build(bool corrupt_inner)
{
  ...
  TEST_ASSERT(s_fixture.out == blob);
  if (corrupt_inner) {
...
static void internal_test_valid(void)
{
  ...
  char           expected[512U];
  const uint32_t flat_len = book_header(s_fixture.out)->total_size;
