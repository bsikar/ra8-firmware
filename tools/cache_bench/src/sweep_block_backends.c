/**
 * @file src/sweep_block_backends.c
 ...
 * @param[out] output Header buffer of ::k_book_container_header_len bytes.
 ...
 * @note Encoding matches the in-tree ::book_chunked reader contract.
 * @since 0.1.0
 */
RA8_INTERNAL
static void
                              ...
                              uint64_t*           output_length)
{
  ...
  const uint64_t table_bytes    = ((uint64_t)count + 1U) * k_book_container_entry_len;
  const uint64_t payload_offset = (uint64_t)k_book_container_header_len + table_bytes;
  ...
  for (uint32_t chunk = 0U; chunk < count; ++chunk) {
    ...
    const uint64_t table_offset =
      (uint64_t)k_book_container_header_len + (((uint64_t)chunk + 1U) * k_book_container_entry_len);
...
{
  const uint64_t table_offset =
    (uint64_t)k_book_container_header_len + ((uint64_t)chunk * k_book_container_entry_len);
  if ((rb->scratch->read(rb->scratch->ctx, table_offset, begin, sizeof(*begin)) != k_cb_io_ok) ||
      (rb->scratch->read(rb->scratch->ctx,
                         table_offset + k_book_container_entry_len,
                         end,
                               ...
                               cb_sweep_config_t*  config)
{
  ...
  *rb      = (cbs_rbkc_t){.scratch        = config->scratch,
                          ...
                          .payload_offset = (uint64_t)k_book_container_header_len +
                                            (((uint64_t)count + 1U) * k_book_container_entry_len)};
