 ...
 * inc/sweep_block.h) and `main` live in src/cache_bench_report.c, which reaches the
 * engine only through the public inc/cache_bench.h surface.
 *
 *
 ...
 *          origin; the typed enum documents that origin.
 * @see https://github.com/aappleby/smhasher
 * @since 0.1.0
 */
typedef enum : uint64_t {
  k_hash_mix_mul = 0xFF51AFD7ED558CCDULL, /**< Murmur3 64-bit finalizer multiplier. */
} cb_hash_mul_t;
 ...
 *          properties; the typed enum documents this constraint.
 * @since 0.1.0
 */
typedef enum : uint8_t {
  k_hash_shift = 33U, /**< Murmur3 64-bit finalizer shift. */
} cb_hash_shift_t;
