...
//!
//! Build graph for the `check_assert_casts` host gate (#858). One executable,
//! plus the test step `scripts/checks/check_zig.py --test` runs: the detector
//! algebra and the exit-status contract
//! `scripts/builders/check_assert_casts.sh` passes through.
...
pub fn build(b: *std.Build) void {
    ...
    const test_step = b.step("test", "Run check_assert_casts tests");
