...
//!
//! Build graph for the `check_no_wave_references` host gate (#858). One
//! executable, plus the test step `scripts/checks/check_zig.py --test` runs:
//! the scope, detector and report algebra, and the exit-status contract
//! `scripts/builders/check_no_wave_references.sh` passes through.
...
pub fn build(b: *std.Build) void {
    ...
    const run_step = b.step("run", "Scan first-party files for session-bookkeeping references");
    ...
    const test_step = b.step("test", "Run check_no_wave_references tests");
