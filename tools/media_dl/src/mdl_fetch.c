...
typedef enum : uint8_t {
  k_fetch_max_attempts =
      4U, /**< 1 initial + up to 3 governed retries on a retryable class. */
} mdl_fetch_retry_t;
...
RA8_PRIV bool mdl_fetch_is_retryable(ra8_err_t rc) {
  return (rc == k_ra8_err_busy) || (rc == k_ra8_err_timeout) ||
         (rc == k_ra8_fail);
}
...
RA8_INTERNAL static const char *fail_reason(long status) {
  return (status >= (long)k_http_server_err) ? "server error"
                                             : "transport error";
}

RA8_PRIV void mdl_fetch_reason(ra8_err_t err, long status, char *buf,
                               size_t cap) {
  if (buf == nullptr) {
    return;
  }
 ...
 */
RA8_INTERNAL static void record_fail(const mdl_fetch_ctx_t *ctx,
                                     const char *url, long status,
                                     ra8_err_t err) {
  ...
  (void)snprintf(item->url, sizeof(item->url), "%s",
                 (url != nullptr) ? url : "");
  item->status = status;
  item->err = err;
  log->count += 1U;
...
RA8_INTERNAL static bool copy_file(const char *src, const char *dst) {
  ...
  for (uint32_t chunk = 0U; chunk < (uint32_t)k_fetch_copy_max_chunks;
       ++chunk) {
 ...
 */
RA8_INTERNAL static bool page_leaf(size_t page_no, const char *url, char *out,
                                   size_t cap) {
  char ext[8];
 ...
 */
RA8_INTERNAL static bool try_reuse(mdl_fetch_ctx_t *ctx, uint64_t url_hash,
  ...
  const int sn = snprintf(src_abs, sizeof(src_abs), "%s/%s",
                          ctx->series_abs_dir, held->rel_path);
  if ((sn < 0) || ((size_t)sn >= sizeof(src_abs))) {
    return false;
  }
  uint64_t have = 0U;
  if ((mdl_hash_file(src_abs, &have) != k_ra8_ok) ||
      (have != held->content_hash)) {
  ...
  if ((held_dot != nullptr) && (tab_dot != nullptr) && (tre_dot != nullptr)) {
    ...
    if (strcmp(tab_dot + 1, held_ext) != 0) {
      (void)snprintf(act_abs, sizeof(act_abs), "%.*s.%s",
                     (int)(tab_dot - target_abs), target_abs, held_ext);
      (void)snprintf(act_rel, sizeof(act_rel), "%.*s.%s",
                     (int)(tre_dot - target_rel), target_rel, held_ext);
    }
  }
  ...
  return mdl_state_add_page(ctx->state, url_hash, held->content_hash, act_rel,
                            held->etag, held->last_modified);
}

/** @brief Governor host key for `url`, or NULL when it cannot be parsed. */
RA8_INTERNAL static const char *page_host(const char *url, char *buf,
                                          size_t cap) {
...
governed_get_file(mdl_fetch_ctx_t *ctx, const char *host, const char *url,
                  const mdl_net_req_t *req, const char *target_abs,
                  uint32_t jmin, uint32_t jmax, mdl_net_resp_t *out_resp,
                  size_t *out_bytes) {
  if (mdl_governor_acquire(ctx->gov, host, jmin, jmax) ==
  ...
  mdl_net_resp_t resp = {};
  const ra8_err_t rc =
      mdl_net_get_file(ctx->session->net, url, req, target_abs, &got, &resp);
 ...
 */
RA8_INTERNAL static ra8_err_t
fetch_with_retry(mdl_fetch_ctx_t *ctx, const char *host, const char *url,
                 const mdl_net_req_t *req, const char *target_abs,
                 uint32_t jmin, uint32_t jmax, mdl_net_resp_t *out_resp,
                 size_t *out_bytes) {
  ra8_err_t rc = k_ra8_fail;
  for (uint8_t attempt = 0U; attempt < (uint8_t)k_fetch_max_attempts;
       ++attempt) {
...
ra8_err_t mdl_fetch_asset(mdl_fetch_ctx_t *ctx, const char *url,
                          const char *target_abs, const char *referer,
                          mdl_net_resp_t *out_resp, size_t *out_bytes) {
  if (out_resp != nullptr) {
  ...
  if ((ctx == nullptr) || (ctx->session == nullptr) ||
      (ctx->session->net == nullptr) || (ctx->site == nullptr) ||
      (url == nullptr) || (url[0] == '\0') || (target_abs == nullptr) ||
      (target_abs[0] != '/')) {
  ...
  mdl_net_resp_t resp = {};
  const ra8_err_t rc =
      fetch_with_retry(ctx, host, url, &req, staged, jmin, jmax, &resp, &got);
  if (out_resp != nullptr) {
    *out_resp = resp;
  }
...
page_paths_for_type(const char *target_abs, const char *target_rel,
                    const char *ext, char *actual_abs, size_t actual_abs_cap,
                    char *actual_rel, size_t actual_rel_cap) {
  if ((target_abs == nullptr) || (target_rel == nullptr) || (ext == nullptr) ||
      (actual_abs == nullptr) || (actual_abs_cap == 0U) ||
      (actual_rel == nullptr) || (actual_rel_cap == 0U)) {
    return false;
  }
  actual_abs[0] = '\0';
  actual_rel[0] = '\0';
  ...
  const int abs_len = snprintf(actual_abs, actual_abs_cap, "%.*s.%s",
                               (int)(dot_abs - target_abs), target_abs, ext);
  const int rel_len = snprintf(actual_rel, actual_rel_cap, "%.*s.%s",
                               (int)(dot_rel - target_rel), target_rel, ext);
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static bool discard_stale_page(const char *guessed_abs,
 ...
 */
RA8_INTERNAL static ra8_err_t
fetch_page_staged(mdl_fetch_ctx_t *ctx, const char *host, const char *url,
                  const mdl_net_req_t *req, const char *target_abs,
                  uint32_t jmin, uint32_t jmax, char *staged, size_t staged_cap,
                  mdl_net_resp_t *resp, size_t *out_bytes) {
  if (!mdl_atomic_tmp_path(target_abs, staged, staged_cap)) {
    return k_ra8_fail;
  }
  const ra8_err_t rc = fetch_with_retry(ctx, host, url, req, staged, jmin, jmax,
                                        resp, out_bytes);
  if (rc != k_ra8_ok) {
    mdl_atomic_abort(staged);
    staged[0] = '\0';
...
RA8_INTERNAL static ra8_err_t
do_fetch_page(mdl_fetch_ctx_t *ctx, const char *chapter_url, const char *url,
              const char *target_abs, const char *target_rel, size_t *out_bytes,
  ...
  const mdl_page_rec_t *recorded =
      mdl_state_find_page(ctx->state, mdl_hash_str(url));
  ...
  mdl_net_req_t req = {
      ...
      .referer = chapter_url,
      .if_none_match =
          (held != nullptr && held->etag[0] != '\0') ? held->etag : nullptr,
      .if_modified_since = (held != nullptr && held->last_modified[0] != '\0')
                               ? held->last_modified
                               : nullptr,
      .timeout_ms = ctx->timeout_ms,
  };
  ...
  size_t got = 0U;
  mdl_net_resp_t resp = {};
  ra8_err_t rc = fetch_page_staged(ctx, host, url, &req, target_abs, jmin, jmax,
                                   staged, sizeof(staged), &resp, &got);
  if (rc != k_ra8_ok) {
    if (out_resp != nullptr) {
      *out_resp = resp;
    }
  ...
  if (resp.status == (long)k_http_not_modified) {
    ...
    staged[0] = '\0';
    if ((held != nullptr) &&
        try_reuse(ctx, mdl_hash_str(url), target_abs, target_rel)) {
      if (out_resp != nullptr) {
        *out_resp = resp;
      }
      return k_ra8_ok;
    ...
    resp = (mdl_net_resp_t){};
    got = 0U;
    rc = fetch_page_staged(ctx, host, url, &req, target_abs, jmin, jmax, staged,
                           sizeof(staged), &resp, &got);
    if ((rc != k_ra8_ok) || (resp.status == (long)k_http_not_modified)) {
      ...
      const ra8_err_t failure =
          (rc != k_ra8_ok) ? rc : k_ra8_err_validation_failed;
  ...
  char true_ext[8];
  if (!mdl_urlname_sniff_file(staged, nullptr, true_ext, sizeof(true_ext),
                              nullptr, 0U)) {
    mdl_atomic_abort(staged);
  ...
  char act_abs[PATH_MAX];
  char act_rel[k_mdl_relpath_max];
  if (!page_paths_for_type(target_abs, target_rel, true_ext, act_abs,
                           sizeof(act_abs), act_rel, sizeof(act_rel))) {
    mdl_atomic_abort(staged);
    record_fail(ctx, url, resp.status, k_ra8_err_invalid_size);
    return k_ra8_err_invalid_size;
  }
  ...
  const char *etag_to_save = (resp.etag[0] != '\0')
                                 ? resp.etag
                                 : ((held != nullptr) ? held->etag : "");
  const char *lastmod_to_save =
      (resp.last_modified[0] != '\0')
          ? resp.last_modified
          : ((held != nullptr) ? held->last_modified : "");
  if ((ctx->state->page_rec_count >= (uint32_t)k_mdl_max_page_recs) &&
      (recorded == nullptr)) {
    mdl_atomic_abort(staged);
  ...
  if (!discard_stale_page(target_abs, act_abs) ||
      !mdl_state_add_page(ctx->state, mdl_hash_str(url), content, act_rel,
                          etag_to_save, lastmod_to_save)) {
    record_fail(ctx, url, resp.status, k_ra8_fail);
    return k_ra8_fail;
  }
  if (out_bytes != nullptr) {
 ...
 * checkpoint. */
RA8_INTERNAL static ra8_err_t
fetch_one_page(mdl_fetch_ctx_t *ctx, const char *chapter_url,
               const char *dest_abs, const char *dest_rel, size_t page_no,
               size_t idx, mdl_chapter_rec_t *rec, mdl_fetch_stats_t *stats,
  ...
  const int an =
      snprintf(target_abs, sizeof(target_abs), "%s/%s", dest_abs, leaf);
  const int rn =
      snprintf(target_rel, sizeof(target_rel), "%s/%s", dest_rel, leaf);
  ...
  const bool has_validator =
      (held != nullptr) &&
      ((held->etag[0] != '\0') || (held->last_modified[0] != '\0'));
  if (!ctx->refetch && !has_validator &&
      try_reuse(ctx, url_hash, target_abs, target_rel)) {
    ...
    *out = (mdl_page_outcome_t){.bytes = 0U, .elapsed_ms = 0U, .reused = true};
    ...
    size_t got = 0U;
    mdl_net_resp_t resp = {};
    const ra8_err_t rc = do_fetch_page(ctx, chapter_url, url, target_abs,
                                       target_rel, &got, &resp);
    if (rc != k_ra8_ok) {
 ...
 * page. */
RA8_INTERNAL static ra8_err_t
fetch_chapter_pages(mdl_fetch_ctx_t *ctx, const char *chapter_url,
                    const char *dest_abs, const char *dest_rel, size_t base,
                    mdl_chapter_rec_t *rec, mdl_fetch_stats_t *stats,
                    const mdl_run_pos_t *pos) {
  for (size_t i = 0U; i < ctx->images->count; ++i) {
    const size_t page_no = base + i + 1U;
    mdl_page_outcome_t out = {};
    const ra8_err_t rc = fetch_one_page(ctx, chapter_url, dest_abs, dest_rel,
                                        page_no, i, rec, stats, &out);
    if (rc != k_ra8_ok) {
      return rc;
    }
...
RA8_INTERNAL static ra8_err_t
governed_get_buf(mdl_fetch_ctx_t *ctx, const char *host, const char *url,
                 const mdl_net_req_t *req, uint32_t jmin, uint32_t jmax,
  ...
  mdl_net_resp_t resp = {};
  const ra8_err_t rc =
      mdl_net_get_buf(ctx->session->net, url, req, ctx->page_buf, ctx->page_cap,
                      out_len, &resp);
  mdl_governor_observe(ctx->gov, host, resp.status, resp.retry_after);
  mdl_governor_release(ctx->gov, host);
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t fetch_chapter_html(mdl_fetch_ctx_t *ctx,
  ...
  const ra8_err_t erc = mdl_extract_images(
      ctx->page_buf, len, chapter_url, ctx->site->page_img_attr,
      ctx->site->page_img_url_contains, ctx->images);
  if (erc != k_ra8_ok) {
 ...
 */
RA8_INTERNAL static bool
resolve_dest(mdl_fetch_ctx_t *ctx, mdl_fetch_layout_t layout, const char *id,
             const char *combined_abs, const char *combined_rel,
             size_t global_no, char *chap_abs, size_t chap_cap,
             const char **dest_abs, const char **dest_rel, size_t *base) {
...
RA8_INTERNAL static mdl_chap_status_t process_chapter(
    mdl_fetch_ctx_t *ctx, const char *chapter_url, mdl_fetch_layout_t layout,
    const char *combined_abs, const char *combined_rel, size_t *global_no,
    mdl_fetch_stats_t *stats, size_t chapter_index, size_t chapter_total) {
  ...
  mdl_chapter_rec_t *rec = mdl_state_add_chapter_numbered(
      ctx->state, id, chapter_url, chapter_number, number_known);
  if (rec == nullptr) {
  ...
  if (ctx->site->chapter_title_selector[0] != '\0') {
    const ra8_err_t title_rc = mdl_extract_selector(
        ctx->page_buf, chapter_html_len, ctx->site->chapter_title_selector,
        extracted_title, sizeof(extracted_title));
    if (title_rc == k_ra8_ok) {
      ...
      (void)fprintf(
          stderr, "media_dl: chapter title selector '%s' failed for '%s'; %s\n",
          ctx->site->chapter_title_selector, chapter_url,
  ...
  if (ctx->site->chapter_number_selector[0] != '\0') {
    ...
    const ra8_err_t number_rc = mdl_extract_selector(
        ctx->page_buf, chapter_html_len, ctx->site->chapter_number_selector,
        extracted_number, sizeof(extracted_number));
    if (number_rc == k_ra8_ok) {
      if (!mdl_urlname_chapter_text_parse(extracted_number, &chapter_number)) {
        (void)fprintf(stderr,
                      ...
                      "value '%s' for "
                      "'%s'\n",
                      ctx->site->chapter_number_selector, extracted_number,
                      chapter_url);
      ...
      (void)fprintf(stderr,
                    ...
                    "preserving %s "
                    "number\n",
                    ctx->site->chapter_number_selector, chapter_url,
                    number_known ? "recorded" : "unknown");
    }
  }
  if (!mdl_state_set_chapter_metadata(rec, chapter_title, chapter_number,
                                      number_known)) {
    return k_ch_failed;
  }
  ...
  size_t base = 0U;
  if (!resolve_dest(ctx, layout, id, combined_abs, combined_rel, *global_no,
                    chap_abs, sizeof(chap_abs), &dest_abs, &dest_rel, &base)) {
    return k_ch_failed;
  }
  const mdl_run_pos_t pos = {.chapter_index = chapter_index,
                             .chapter_total = chapter_total,
                             .chapter_id = id};
  if (fetch_chapter_pages(ctx, chapter_url, dest_abs, dest_rel, base, rec,
                          stats, &pos) != k_ra8_ok) {
    (void)checkpoint(ctx);
    return k_ch_failed;
  }
...
RA8_INTERNAL static bool ctx_ready(const mdl_fetch_ctx_t *ctx) {
  return (ctx->session != nullptr) && (ctx->state != nullptr) &&
         (ctx->series_abs_dir != nullptr) && (ctx->series_url != nullptr) &&
         (ctx->site != nullptr) && (ctx->page_buf != nullptr) &&
         (ctx->images != nullptr);
}

ra8_err_t mdl_fetch_run(mdl_fetch_ctx_t *ctx, const mdl_url_list_t *chapters,
                        mdl_fetch_layout_t layout, const char *combined_dir_rel,
  ...
  if (layout == k_mdl_layout_combined) {
    if ((combined_dir_rel == nullptr) ||
        !mdl_join_dir_under(ctx->series_abs_dir, combined_dir_rel, combined_abs,
                            sizeof(combined_abs))) {
      return k_ra8_err_invalid_arg;
    }
  } else {
  ...
  for (size_t i = 0U; i < chapters->count; ++i) {
    const mdl_chap_status_t s = process_chapter(
        ctx, chapters->urls[i], layout, combined_abs, combined_dir_rel,
        &global_no, stats, i + 1U, chapters->count);
    if (tally(s, stats)) {
      any_fail = true;
      if (layout == k_mdl_layout_combined) {
