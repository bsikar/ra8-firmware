 ...
 */
static const uint8_t s_png_magic[] = {0x89U, 'P', 'N', 'G', 0x0DU, 0x0AU, 0x1AU, 0x0AU};
                               ...
                               mdl_net_resp_t*      resp)
{
  ...
  if (resp != nullptr) {
    ...
    if (f->resp_content_type != nullptr) {
      (void)snprintf(resp->content_type, sizeof(resp->content_type), "%s", f->resp_content_type);
    }
  }
  ...
  if ((f->response_prefix != nullptr) && (f->response_prefix_len > 0U)) {
    if (fwrite(f->response_prefix, 1U, f->response_prefix_len, fp) != f->response_prefix_len) {
      (void)fclose(fp);
      return k_ra8_fail;
    }
  }
...
static mock_net_t g_mock;
/** @brief Session over the fake with an embedded 64-KiB buffer. */
static mdl_session_t g_sess;
/** @brief Selectors and disabled politeness bounds. */
static mdl_site_t g_site;
/** @brief Persistent state under test. */
static mdl_state_t g_state;
/** @brief Live chapter list for one scenario. */
static mdl_url_list_t g_chapters;
/** @brief Extracted-image scratch list. */
static mdl_url_list_t g_images;
/** @brief Chapter-HTML scratch buffer. */
static char g_page[k_page_bytes];
/** @brief Governor wired into run_fetch, or NULL. */
static mdl_governor_t* g_fetch_gov;
/** @brief Failure log populated by run_fetch. */
...
static void test_chapter_number_selector_strict_and_persisted(void)
{
  ...
  (void)snprintf(g_site.chapter_number_selector,
                 sizeof(g_site.chapter_number_selector),
                 "%s",
                 "class:chapter-number");
  g_mock.map   = s_map_number_selector;
  g_mock.map_n = sizeof(s_map_number_selector) / sizeof(s_map_number_selector[0]);
  ...
  TEST_ASSERT_EQ(
    (int64_t)k_ra8_ok,
    run_fetch(abs_dir, state_path, k_mdl_layout_separate, nullptr, false, 0U, &valid_stats));
  const mdl_chapter_rec_t* chapter = mdl_state_find_chapter(&g_state, "chapter-special");
  ...
  TEST_ASSERT_EQ(
    (int64_t)k_ra8_fail,
    run_fetch(abs_dir, state_path, k_mdl_layout_separate, nullptr, false, 0U, &invalid_stats));
  TEST_ASSERT_EQ((int64_t)0, (int64_t)g_mock.get_file_calls);
  TEST_ASSERT_EQ((int64_t)1, (int64_t)g_faillog.count);
  TEST_ASSERT_EQ((int64_t)k_ra8_err_validation_failed, (int64_t)g_faillog.items[0].err);
...
static void test_first_run_then_update_only_new(void)
{
  ...
  (void)snprintf(g_site.chapter_title_selector,
                 sizeof(g_site.chapter_title_selector),
                 "%s",
                 "class:chapter-title");
  g_mock.map   = s_map3;
  ...
  TEST_ASSERT(strcmp(mdl_state_find_chapter(&g_state, "chapter-1")->title, "Chapter One") == 0);
...
static void test_chapter_title_overflow_fails_without_page_fetch(void)
{
  ...
  TEST_ASSERT_EQ((int64_t)k_ra8_fail,
                 run_fetch(abs_dir, state_path, k_mdl_layout_separate, nullptr, false, 0U, &stats));
  TEST_ASSERT_EQ((int64_t)1, (int64_t)stats.chapters_failed);
  TEST_ASSERT_EQ((int64_t)0, (int64_t)g_mock.get_file_calls);
  TEST_ASSERT_EQ((int64_t)1, (int64_t)g_faillog.count);
  TEST_ASSERT_EQ((int64_t)k_ra8_err_invalid_size, (int64_t)g_faillog.items[0].err);
  const mdl_chapter_rec_t* chapter = mdl_state_find_chapter(&g_state, "chapter-9");
  TEST_ASSERT_NOT_NULL(chapter);
...
static void test_refetch_bypasses_valid_cache(void)
{
  ...
  TEST_ASSERT(strcmp(mdl_state_find_chapter(&g_state, "chapter-2")->title, "chapter-2") == 0);
...
static void test_page_magic_required_and_old_destination_preserved(void)
{
  ...
  TEST_ASSERT_EQ(
    (int64_t)k_ra8_fail,
    run_fetch(abs_dir, state_path, k_mdl_layout_separate, nullptr, false, 0U, &rejected));
  TEST_ASSERT_EQ((int64_t)1, (int64_t)rejected.pages_failed);
  TEST_ASSERT_EQ(old_hash, page_hash(abs_dir, "chapter-1/page_0001.jpg"));
  const mdl_page_rec_t* retained = mdl_state_find_page(&g_state, mdl_hash_str("http://cdn/a.jpg"));
...
static void test_page_magic_changes_extension_and_rekeys_state(void)
{
  ...
  const mdl_page_rec_t* record = mdl_state_find_page(&g_state, mdl_hash_str("http://cdn/a.jpg"));
...
static void test_304_without_cached_entity_refetches_unconditionally(void)
{
  ...
  TEST_ASSERT_EQ(
    (int64_t)k_ra8_ok,
    run_fetch(abs_dir, state_path, k_mdl_layout_separate, nullptr, false, 0U, &recovered));
...
static void test_fetch_asset_policy_atomic_and_nonempty(void)
{
  ...
  TEST_ASSERT_NOT_NULL(fp);
  TEST_ASSERT_EQ((int64_t)strlen("keep"), (int64_t)fwrite("keep", 1U, strlen("keep"), fp));
  ...
  TEST_ASSERT_NOT_NULL(fp);
  char retained[8] = {};
  TEST_ASSERT_EQ((int64_t)strlen("keep"), (int64_t)fread(retained, 1U, sizeof(retained), fp));
  TEST_ASSERT_EQ((int64_t)0, (int64_t)fclose(fp));
