...
if [ "$BUILD" -eq 1 ]; then
  ...
  cmake -B "$MDL_DIR/build" -S "$MDL_DIR" >/dev/null || { say "media_dl configure FAILED"; exit 99; }
  cmake --build "$MDL_DIR/build" -j >/dev/null || { say "media_dl build FAILED"; exit 99; }
  cmake -B "$VIEWER_DIR/build" -S "$VIEWER_DIR" -DCMAKE_BUILD_TYPE=Release >/dev/null \
    || { say "viewer configure FAILED"; exit 99; }
  cmake --build "$VIEWER_DIR/build" -j >/dev/null || { say "viewer build FAILED"; exit 99; }
fi
[ -x "$MEDIA_DL" ] || { say "missing $MEDIA_DL (build first)"; exit 99; }
[ -x "$VIEWER" ]   || { say "missing $VIEWER (build first)"; exit 99; }

# --- synthetic, non-copyright pages -----------------------------------------
rm -rf "$WORK"
mkdir -p "$PAGES" "$PPMDIR"
python3 "$HERE/gen_pages.py" "$PAGES" 6 800 1200 || { say "gen_pages FAILED"; exit 99; }
...
for fmt in $FORMATS; do
  ...
  case "$fmt" in
    cbr)    command -v rar >/dev/null 2>&1 || { say "SKIP  $fmt  (needs 'rar' on PATH)"; skip=$((skip+1)); continue; } ;;
    cbt.xz) command -v xz  >/dev/null 2>&1 || { say "SKIP  $fmt  (needs 'xz' on PATH)";  skip=$((skip+1)); continue; } ;;
    rabook) python3 -c 'import PIL' >/dev/null 2>&1 || { say "SKIP  $fmt  (needs python3 + Pillow)"; skip=$((skip+1)); continue; } ;;
  esac
  ...
  fdir="$WORK/in_${fmt//./_}"
  rm -rf "$fdir"; cp -R "$PAGES" "$fdir"

  # 1) package
  if ! "$MEDIA_DL" --pack "$fdir" --format "$fmt" >"$WORK/pack_${fmt//./_}.log" 2>&1; then
    say "FAIL  $fmt  (pack)"; sed 's/^/        /' "$WORK/pack_${fmt//./_}.log"
    fail=$((fail+1)); failed_list="$failed_list $fmt(pack)"; continue
  fi
  art="$(artifact_for "$fmt" "$fdir")"
  if [ ! -s "$art" ]; then
    say "FAIL  $fmt  (no artifact at $art)"; fail=$((fail+1)); failed_list="$failed_list $fmt(artifact)"; continue
  fi
  ...
  if ! "$VIEWER" "$art" --headless --dump-ppm "$ppm" >"$WORK/view_${fmt//./_}.log" 2>&1; then
    say "FAIL  $fmt  (viewer open/render)"; sed 's/^/        /' "$WORK/view_${fmt//./_}.log"
    fail=$((fail+1)); failed_list="$failed_list $fmt(view)"; continue
  fi

  # 3) assert the render is non-blank
  if ! python3 "$HERE/check_ppm.py" "$ppm" >"$WORK/ppm_${fmt//./_}.log" 2>&1; then
    say "FAIL  $fmt  (blank render)"; sed 's/^/        /' "$WORK/ppm_${fmt//./_}.log"
    fail=$((fail+1)); failed_list="$failed_list $fmt(blank)"; continue
  fi
