...
static void test_numeric_contracts(void)
{
  ...
  TEST_ASSERT(!mdl_cli_parse_nums(&tailed_from, &nums));
 ...
 * @since 0.1.0
 */
static void matrix_push(char* argv[], size_t* argc, const char* token)
{
 ...
 * @since 0.1.0
 */
static bool matrix_base_contains(cli_matrix_mode_t mode, cli_matrix_option_t opt)
{
 ...
 * @since 0.1.0
 */
static bool matrix_needs_pick(cli_matrix_option_t opt)
{
  return (opt == k_opt_out) || (opt == k_opt_chapters) || (opt == k_opt_from) ||
         (opt == k_opt_format) || (opt == k_opt_separate) || (opt == k_opt_update) ||
         (opt == k_opt_incomplete) || (opt == k_opt_progress) || (opt == k_opt_refetch);
}
 ...
 */
static void
matrix_build_base(cli_matrix_mode_t mode, cli_matrix_option_t opt, char* argv[], size_t* argc)
{
  *argc                       = 0U;
  const cli_mode_case_t* base = &s_matrix_modes[mode];
  for (size_t i = 0U; i < base->argc; ++i) {
    matrix_push(argv, argc, base->argv[i]);
  }
  ...
  if ((mode == k_matrix_verify) &&
    ...
    matrix_push(argv, argc, "verify-dir");
  }
}
 ...
 * @since 0.1.0
 */
static void matrix_add_prerequisites(cli_matrix_mode_t   mode,
                                     cli_matrix_option_t opt,
                                     char*               argv[],
                                     size_t*             argc)
{
  if (((mode == k_matrix_search) || (mode == k_matrix_browse)) && matrix_needs_pick(opt)) {
    matrix_push(argv, argc, "--pick");
    matrix_push(argv, argc, "1");
  }
  if ((opt == k_opt_proxy) || (opt == k_opt_socks5)) {
    matrix_push(argv, argc, "--allow-private");
  }
}
 ...
 * @since 0.1.0
 */
static void matrix_add_option(cli_matrix_option_t opt, char* argv[], size_t* argc)
{
  const cli_option_case_t* option = &s_matrix_options[opt];
  for (size_t i = 0U; i < option->count; ++i) {
    matrix_push(argv, argc, option->token[i]);
  }
}
 ...
 * @note Not thread-safe: validation may write diagnostics to stderr.
 * @since 0.1.0
 */
static bool matrix_validate(char* argv[], size_t argc, mdl_cli_mode_t* mode)
{
  mdl_args_t args = {};
  mdl_cli_parse((int)argc, argv, &args);
  return mdl_cli_validate(&args, mode);
}
 ...
 * @since 0.1.0
 */
static bool matrix_has_spelling(const char* spelling)
{
...
/**
 * @brief Capture the public usage block from stderr into fixed storage.
 * @details Temporarily redirects the stderr descriptor to a host temp stream,
 *          restores it before assertions, and reads the bounded usage bytes.
 * @param[out] out Captured NUL-terminated usage text.
 * @param[in]  cap Destination capacity including NUL.
 * @return Whether redirection, restoration, and bounded capture succeeded.
 * @retval true  @p out contains the complete usage block.
 * @retval false A host descriptor/stream operation failed or text overflowed.
 * @pre @p out is non-NULL.
 * @pre @p cap is greater than one and stderr is open.
 * @post stderr is restored before return whenever it was saved.
 * @post @p out is NUL-terminated.
 * @note Host-only and not thread-safe with concurrent stderr writers.
 * @since 0.1.0
 */
static bool capture_usage(char* out, size_t cap)
{
  out[0]   = '\0';
  FILE* fp = tmpfile();
  if (fp == nullptr) {
  ...
  bool ok = (fflush(stderr) == 0) && (dup2(fileno(fp), STDERR_FILENO) >= 0);
  if (ok) {
    mdl_cli_usage("media_dl");
    ok = fflush(stderr) == 0;
  }
  const bool restored = dup2(saved, STDERR_FILENO) >= 0;
  (void)close(saved);
  if (ok && restored) {
    rewind(fp);
    const size_t n = fread(out, 1U, cap - 1U, fp);
    out[n]         = '\0';
    ok             = (ferror(fp) == 0) && (feof(fp) != 0);
  } else {
    ok = false;
  }
  ok = (fclose(fp) == 0) && ok;
 ...
 * @since 0.1.0
 */
static void test_exhaustive_mode_option_matrix(void)
{
  ...
  for (size_t mode_index = 0U; mode_index < (size_t)k_matrix_mode_count; ++mode_index) {
    ...
    for (size_t opt_index = 0U; opt_index < (size_t)k_matrix_option_count; ++opt_index) {
      ...
      size_t                    argc = 0U;
      matrix_build_base(mode, opt, argv, &argc);
      const bool expected = s_matrix_options[opt].allowed[mode];
      if (expected) {
        matrix_add_prerequisites(mode, opt, argv, &argc);
      }
      if (!matrix_base_contains(mode, opt)) {
        matrix_add_option(opt, argv, &argc);
      }
      mdl_cli_mode_t actual_mode = k_mdl_cli_mode_invalid;
      const bool     accepted    = matrix_validate(argv, argc, &actual_mode);
      if ((accepted != expected) || (accepted && (actual_mode != s_matrix_modes[mode].expected))) {
        (void)fprintf(stderr,
                      "matrix mismatch: mode=%s option=%s expected=%s actual=%s\n",
                      s_matrix_modes[mode].name,
                      s_matrix_options[opt].name,
                      expected ? "allowed" : "rejected",
                      accepted ? mdl_cli_mode_name(actual_mode) : "rejected");
      }
 ...
 * @since 0.1.0
 */
static void test_every_public_option_repeated(void)
{
  ...
  for (size_t opt_index = 0U; opt_index < (size_t)k_matrix_option_count; ++opt_index) {
    ...
    matrix_add_option(opt, argv, &argc);
    mdl_cli_mode_t actual_mode = k_mdl_cli_mode_invalid;
    if (matrix_validate(argv, argc, &actual_mode)) {
      (void)fprintf(stderr, "repetition accepted unexpectedly: %s\n", s_matrix_options[opt].name);
    }
 ...
 * @pre ::matrix_needs_pick enumerates every conditional download row.
 * @post Every unpicked conditional row is rejected in both discovery modes.
 * @post Every rejection leaves the mode invalid.
 * @note Host-only parser test with expected diagnostics on stderr.
 * @since 0.1.0
 */
static void test_discovery_download_options_require_pick(void)
{
  ...
  for (size_t mode_index = 0U; mode_index < (sizeof(modes) / sizeof(modes[0])); ++mode_index) {
    for (size_t opt_index = 0U; opt_index < (size_t)k_matrix_option_count; ++opt_index) {
      const cli_matrix_option_t opt = (cli_matrix_option_t)opt_index;
      if (!matrix_needs_pick(opt)) {
        continue;
      }
      char*  argv[k_matrix_argv_cap];
      size_t argc = 0U;
      matrix_build_base(modes[mode_index], opt, argv, &argc);
      matrix_add_option(opt, argv, &argc);
      mdl_cli_mode_t actual_mode = k_mdl_cli_mode_invalid;
      TEST_ASSERT(!matrix_validate(argv, argc, &actual_mode));
      TEST_ASSERT_EQ((uint8_t)k_mdl_cli_mode_invalid, (uint8_t)actual_mode);
    }
  }
 ...
 * @pre Host descriptor redirection and temporary streams are available.
 ...
 * @post stderr is restored before assertions complete.
 * @note Host-only coverage guard; performs no network activity.
 * @since 0.1.0
 */
static void test_help_long_options_have_matrix_rows(void)
{
  TEST_BEGIN("CLI help options covered by matrix");
  char help[k_matrix_help_cap];
  TEST_ASSERT(capture_usage(help, sizeof(help)));
  const char* at = help;
  while ((at = strstr(at, "--")) != nullptr) {
    ...
    memcpy(token, at, len);
    token[len] = '\0';
    if (!matrix_has_spelling(token)) {
      (void)fprintf(stderr, "help option missing from applicability matrix: %s\n", token);
    }
    TEST_ASSERT(matrix_has_spelling(token));
    at = end;
  }
...
int32_t main(void)
{
  test_exhaustive_mode_option_matrix();
  test_every_public_option_repeated();
  test_discovery_download_options_require_pick();
  test_help_long_options_have_matrix_rows();
  test_all_modes();
  test_allowlists();
  test_usage_errors();
