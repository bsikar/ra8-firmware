...
//!
//! Build graph for the `check_nsc_veneer_defs` host gate (#858). One
//! executable, plus the test step `scripts/checks/check_zig.py --test` runs:
//! the declaration and definition algebra, and the exit-status contract
//! `scripts/builders/check_nsc_veneer_defs.sh` passes through.
...
pub fn build(b: *std.Build) void {
    ...
    const run_step = b.step("run", "Check every declared RA8_NSC_VENEER has a definition");
    ...
    const test_step = b.step("test", "Run check_nsc_veneer_defs tests");
