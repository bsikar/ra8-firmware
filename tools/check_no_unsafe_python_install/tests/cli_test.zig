...
test "a source past the old read ceiling is still scanned" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try write(tmp.dir, "self.zig", "placeholder\n");
    try write(tmp.dir, "docs/small.md", unsafe_line);
    // A sparse hole keeps the fixture off the disk while the file still
    // reports 65 MiB, past any constant ceiling a reader might impose. The
    // Python this gate replaces called read() with no ceiling, so size never
    ...
    try std.testing.expect(std.mem.indexOf(u8, outcome.err, "docs/small.md:1") != null);
}
