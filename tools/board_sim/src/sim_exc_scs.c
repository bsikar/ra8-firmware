/**
 * @file sim_exc_scs.c
 * @brief SCS write watchers + DWT time base (see sim_exc.h)
 *
 * @details
...
/** @brief Implementation of `sim_exc_install_scb_nvic()` -- SCB ctrl + NVIC hooks. */
void sim_exc_install_scb_nvic(uc_engine* uc)
{
...
/** @brief Implementation of `sim_exc_reboot_requested()` -- plain flag read. */
bool sim_exc_reboot_requested(void)
{
  return s_reboot_request;
}

/** @brief Implementation of `sim_exc_clear_reboot_request()` -- plain clear. */
void sim_exc_clear_reboot_request(void)
{
