/**
 * @file sim_view.c
 * @brief Board-view presentation implementation (see sim_view.h)
 *
 * @details
 ...
 * + core-control state -- moved verbatim out of the board_sim main
 * translation unit.
 *
...
/** @brief Implementation of `sim_view_publish()` -- run-loop telemetry. */
void sim_view_publish(uint32_t pc, uint32_t chunks)
{
  s_view_pc     = pc;
  s_view_chunks = chunks;
}

/** @brief Implementation of `sim_view_mark_stopped()` -- final held frame. */
void sim_view_mark_stopped(uint32_t pc)
{
  s_view_running = false;
  s_view_pc      = pc;
}

/** @brief Implementation of `sim_view_wheel()` -- scrollback paging. */
void sim_view_wheel(int32_t notches)
{
  if (notches > 0) {
...
/** @brief Implementation of `sim_view_select_console_tab()` -- tab switch. */
void sim_view_select_console_tab(uint32_t tab_idx)
{
  s_view_console_ch = (board_console_ch_t)tab_idx;
  s_view_scroll     = 0U;
  s_view_autoscroll = true;
...
/** @brief Implementation of `sim_view_reset_console()` -- warm-reboot view reset. */
void sim_view_reset_console(void)
{
...
/** @brief Implementation of `sim_primary_core()` -- plain state read. */
board_primary_core_t sim_primary_core(void)
{
  return s_primary_core;
}

/** @brief Implementation of `sim_set_primary_core()` -- plain state write. */
void sim_set_primary_core(board_primary_core_t core)
{
  s_primary_core = core;
}

/** @brief Implementation of `sim_low_power()` -- plain state read. */
bool sim_low_power(void)
{
  return s_low_power;
}

/** @brief Implementation of `sim_set_low_power()` -- plain state write. */
void sim_set_low_power(bool on)
{
  s_low_power = on;
}
