/**
 * @file sim_prof.c
 * @brief Firmware profiler implementation (see sim_prof.h)
 *
 * @details
 * The BOARD_SIM_PROFILE profiler: FUNC-symbol collection, the wall-time
 ...
 * verbatim out of the board_sim main translation unit.
 *
...
/** @brief Implementation of `sim_prof_install()` -- arms the insn hook in insn mode. */
...
/** @brief Implementation of `sim_prof_mode()` -- plain state read. */
prof_mode_t sim_prof_mode(void)
{
  return s_prof_mode;
}

/** @brief Implementation of `sim_prof_total_insns()` -- plain counter read. */
uint64_t sim_prof_total_insns(void)
{
  return s_prof_total_i;
}

/** @brief Implementation of `sim_prof_set_stop_pc()` -- plain state write. */
void sim_prof_set_stop_pc(uint32_t pc)
{
  s_prof_stop_pc = pc;
}

/** @brief Implementation of `sim_prof_stop_hit()` -- plain flag read. */
bool sim_prof_stop_hit(void)
{
