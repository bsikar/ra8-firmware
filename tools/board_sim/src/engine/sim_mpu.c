/**
 * @file sim_mpu.c
 * @brief Armv8-M MPU enforcement implementation (see sim_mpu.h)
 *
 * @details
...
/** @brief Implementation of `sim_mpu_install()` -- RLAR capture + CTRL edges. */
void sim_mpu_install(uc_engine* uc)
{
...
/** @brief Implementation of `sim_mpu_fault_pending()` -- plain flag read. */
bool sim_mpu_fault_pending(void)
{
  return s_mpu_fault;
}

/** @brief Implementation of `sim_mpu_clear_fault()` -- plain flag clear. */
void sim_mpu_clear_fault(void)
{
  s_mpu_fault = false;
}
