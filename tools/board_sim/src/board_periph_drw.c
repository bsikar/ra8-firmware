 ...
 * @c 0x40444000 as it actually behaves on real silicon: INERT. On the bench the
 * D/AVE 2D engine never rasterizes (issue #247) -- a CONTROL/ORIGIN trigger
 * synthesises no pixels, PERFCOUNT never advances, the framebuffer is left
 * exactly as the CPU last wrote it, and HWREVISION reads 0 on both the Secure
 * and Non-Secure aliases. board_sim's governing invariant is SIM == HIL: an app
 * must produce the IDENTICAL result in the emulator and on hardware, hardware
 * defects included. This block therefore reproduces the inert engine rather than
 * an idealized one.
 *
 ...
 *  - @c HWREVISION (read alias of CONTROL2 @ 0x004) reads 0 exactly like silicon
 *    (issue #247). The driver only stashes it in a diagnostic global and never
 *    gates on a non-zero "present" stamp, so a zero revision keeps SIM == HIL;
 *    faking a non-zero code here would be a fresh SIM != HIL divergence.
 *
