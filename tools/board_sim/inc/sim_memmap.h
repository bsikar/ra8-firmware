 ...
 * views stay coherent). sim_memmap_init() maps every region on an engine,
 ...
 * through the sparse model in sim_mmio. The cpu1 engine re-maps the same
 ...
 * @see sim_memmap_regions()  Returns the table.
 * @since 0.1.0
 */
typedef struct {
 ...
 * @see sim_memmap_regions()  The table this maps.
 * @since 0.1.0
 */
RA8_PRIV bool sim_memmap_init(uc_engine* uc);
 ...
 * @since 0.1.0
 */
RA8_PRIV const mem_region_t* sim_memmap_regions(uint32_t* count);

/**
 * @brief Host buffer backing the on-chip SRAM (shared with cpu1).
 *
 * @return The SRAM host backing, or NULL before sim_memmap_init().
 * @retval NULL sim_memmap_init() has not run (or its allocation failed).
 * @pre sim_memmap_init() succeeded.
 * @pre None otherwise.
 ...
 * @since 0.1.0
 */
RA8_PRIV uint8_t* sim_memmap_sram_buf(void);
 ...
 * @since 0.1.0
 */
RA8_PRIV uint64_t sim_memmap_mram_base(void);
