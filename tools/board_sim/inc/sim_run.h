 ...
 * environment knobs (BOARD_SIM_WALL_S / MAX_CHUNKS / IDLE_STOP / USB_STOP /
 ...
 *
 * @invariant The referenced buffers/strings outlive sim_run_and_report()
 *            (argv strings and the run-long ELF image).
 * @see sim_run_and_report()  The consumer.
 * @since 0.1.0
 */
typedef struct {
 ...
 * @since 0.1.0
 */
RA8_PRIV int sim_run_and_report(const sim_run_cfg_t* cfg);
