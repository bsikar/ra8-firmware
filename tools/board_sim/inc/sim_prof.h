/**
 * @file sim_prof.h
 * @brief Firmware profiler (BOARD_SIM_PROFILE): sampling, hooks, reports
 *
 * @details
 * Two profiling modes, bucketed by ELF FUNC symbol:
 *
 *  - `BOARD_SIM_PROFILE=1` -- wall-time sample: charge each emulation chunk's
 ...
 *  - `BOARD_SIM_PROFILE=full` (or `insn`) -- per-instruction: a code hook
 ...
 *
 * The run loop reads the mode / instruction totals / BOARD_SIM_STOP_PC state
 * through the accessors here; everything else stays private to sim_prof.c.
 *
...
/**
 * @enum prof_mode_t
 * @brief Profiler mode parsed from BOARD_SIM_PROFILE.
 *
 ...
 *
 * @details Reads BOARD_SIM_PROFILE to pick the mode; when enabled, walks the
 ...
 * @see sim_prof_install()  Arms the per-instruction hook afterwards.
 * @since 0.1.0
 */
 ...
 * @since 0.1.0
 */
RA8_PRIV void sim_prof_install(uc_engine* uc);

/**
 * @brief The latched profiler mode.
 *
 * @return The mode BOARD_SIM_PROFILE selected.
 ...
 * @since 0.1.0
 */
RA8_PRIV prof_mode_t sim_prof_mode(void);
 ...
 * @since 0.1.0
 */
RA8_PRIV uint64_t sim_prof_total_insns(void);

/**
 * @brief Set the BOARD_SIM_STOP_PC early-stop address (0 disables).
 *
 ...
 * @since 0.1.0
 */
RA8_PRIV void sim_prof_set_stop_pc(uint32_t pc);

/**
 * @brief Whether the BOARD_SIM_STOP_PC address was reached.
 *
 ...
 * @since 0.1.0
 */
RA8_PRIV bool sim_prof_stop_hit(void);
