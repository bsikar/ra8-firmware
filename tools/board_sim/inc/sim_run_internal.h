/**
 * @file sim_run_internal.h
 * @brief Shared run-loop state + cross-TU run helpers (board_sim internal)
 *
 * @details
 * The chunked run phase is split across three translation units to keep each
 * under the file-size bar: sim_run.c (setup + the per-chunk phase helpers +
 * the driver), sim_run_inner.c (the inner exception-resolve loop) and
 * sim_run_report.c (the run-end report + finalize). This internal header
 ...
 * NOT part of the board_sim public surface (unlike sim_run.h).
 *
 ...
 * sim_run_inner.c; called by the run loop in sim_run.c.
 *
 ...
 *
 * @details Defined in sim_run_report.c; called by sim_run_and_report after the
 * run loop returns.
 *
 ...
 *
 * @details Defined in sim_run_report.c; called by sim_run_and_report.
 *
 ...
 *
 * @details Defined in sim_run_report.c; called by sim_run_and_report. Inert in
 * headless mode.
 *
 ...
 *
 * @details Defined in sim_run_report.c; called by sim_run_and_report as the
 * final teardown step.
 *
 * @param[in] st The run state (buffers, engine, stop flags).
 * @return The process exit status (::board_sim_exit_t).
