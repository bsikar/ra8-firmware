/**
 * @file sim_args.h
 * @brief board_sim command-line parsing (usage text + option decoding)
 *
 * @details
 * Decodes the board_sim CLI into one ::sim_args_t bundle: the firmware image
...
/**
 * @struct sim_args_t
 * @brief Every value the board_sim CLI parse produces for the setup phase.
 *
 ...
 * @see sim_args_parse()  The producer.
 * @since 0.1.0
 */
typedef struct {
  ...
  board_device_t sim_device;                       /**< --device selected part (RA8D2 default). */
  bool           usbhs_loop;                       /**< --usbhs-loop chip-internal self-loop.   */
} sim_args_t;

/**
 * @brief Parse the board_sim command line into @p out (or print usage).
 *
 * @details
 ...
 * @see sim_run_and_report()  Consumes the values via ::sim_run_cfg_t.
 * @since 0.1.0
 */
RA8_PRIV bool sim_args_parse(int argc, char** argv, sim_args_t* out);
