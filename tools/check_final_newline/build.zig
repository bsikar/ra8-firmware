...
//!
//! Build graph for the `check_final_newline` host gate (#858). One
//! executable, plus the test step `scripts/checks/check_zig.py --test` runs:
//! the scope and detector algebra, and the exit-status contract
//! `scripts/builders/check_final_newline.sh` passes through.
...
pub fn build(b: *std.Build) void {
    ...
    const run_step = b.step("run", "Scan first-party sources for a missing trailing newline");
    ...
    const test_step = b.step("test", "Run check_final_newline tests");
