...
/// Walk `dir_path` recursively and append every entry whose NAME matches a
/// header glob, DIRECTORIES INCLUDED. `Path.rglob("*.h")` matches a directory
/// named `x.h`, and the predecessor did not filter those out, so one under a
/// `src/` tree is scanned and can be reported. The test is
/// `matchesHeaderGlob`, not `isHeader`: the walk inherits glob semantics, so a
/// file called exactly `.h` is a hit here even though pathlib gives it no
/// suffix and the explicit-path branch below drops it.
fn collectRecursive(
    allocator: std.mem.Allocator,
    dir: std.fs.Dir,
    dir_path: []const u8,
    ...
    while (try iterator.next()) |entry| {
        ...
        if (implementation.matchesHeaderGlob(child)) try out.append(child);
