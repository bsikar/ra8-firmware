...
test "the discovery glob matches a name the pathlib suffix rule rejects" {
    // rglob("*.h") matches ".h" because pathlib's globber matches a leading
    // dot, while Path(".h").suffix is empty. A sweep reported such a file and
    // an explicit path list dropped it; both halves are preserved.
    try testing.expect(implementation.matchesHeaderGlob("x/src/.h"));
    try testing.expect(!implementation.isHeader("x/src/.h"));
}

test "the discovery glob agrees with the suffix rule on ordinary names" {
    try testing.expect(implementation.matchesHeaderGlob("libs/m/src/a.h"));
    try testing.expect(implementation.matchesHeaderGlob("libs/m/src/a.hpp"));
    try testing.expect(implementation.matchesHeaderGlob("libs/m/src/a.hh"));
    try testing.expect(implementation.matchesHeaderGlob("libs/m/src/a.hxx"));
    try testing.expect(implementation.matchesHeaderGlob("libs/m/src/a..h"));
    try testing.expect(!implementation.matchesHeaderGlob("libs/m/src/a.c"));
    try testing.expect(!implementation.matchesHeaderGlob("libs/m/src/a.h.txt"));
    try testing.expect(!implementation.matchesHeaderGlob("libs/m/src/a.H"));
}

test "the discovery glob reads the name, not the directories above it" {
    try testing.expect(!implementation.matchesHeaderGlob("libs/a.h/src/widget.c"));
}
