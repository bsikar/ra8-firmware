...
//!
//! Build graph for the `check_tz_boundary_discard` host gate (#858, #1250).
//! One executable plus the test step `scripts/checks/check_zig.py --test`
//! runs: the two discard rules and the exit-status contract CI depends on.
...
pub fn build(b: *std.Build) void {
    ...
    const executable = b.addExecutable(.{
        .name = "check_tz_boundary_discard",
    ...
    const run_step = b.step("run", "Check TrustZone boot-boundary error discards");
    ...
    const test_step = b.step("test", "Run check_tz_boundary_discard tests");
