...
from epub_package import _parse_xml
...
def selftest() -> int:
    ...
    entity_xml = '<!DOCTYPE x [<!ENTITY payload "boom">]><x>&payload;</x>'
    for encoding in ("utf-8", "utf-16-le", "utf-16-be", "utf-32-le", "utf-32-be"):
        try:
            _parse_xml(entity_xml.encode(encoding))
        except ET.ParseError:
            pass
        else:
            _fl_fail(f"selftest FAILED: XML entity declaration accepted as {encoding}")
