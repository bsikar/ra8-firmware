...
(``reflow``'s existing image handling) is shared with EPUB books instead of
...
* ``--rtl`` stamps the right-to-left reading-order flag
  (``k_book_flag_rtl`` in apps/shared_libs/book/inc/book.h) into the header;
...
# (kept in lockstep with apps/shared_libs/book/inc/book.h and BlobBuilder.serialize)
HEADER_FMT = "<8s23I"
HEADER_BYTES = 100
CHAPTER_FMT = "<3I"
CHAPTER_BYTES = 12
...
def page_dom(name: str) -> dict[str, object]:
    ...
    Mirrors the fixed-layout EPUB shape so ``reflow``'s existing image path
