...
`book_open()` is called, which is what lets several full books (covers and
...
def _header_preamble() -> list[str]:
    ...
    return [
        ...
        " * @details Each entry is the chunked RBKC container (book_open inflates it",
        " *          on demand) plus a gray8 cover thumbnail the shelf blits without any",
        " *          boot-time inflation. Regenerate with tools/epub_compile/src/",
        " *          bake_library.py (the",
...
def _emit_library_table(count: int, names: list[tuple]) -> list[str]:
    ...
    parts = [
        ...
        f"  k_library_count = {count}U, /**< Number of entries in k_library. */",
        "} library_count_t;",
        "",
