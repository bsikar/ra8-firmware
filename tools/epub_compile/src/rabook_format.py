...
"""The .rabook on-disk constants, kept in lockstep with apps/shared_libs/book/inc/book.h.
...
# --- on-disk constants, kept in lockstep with apps/shared_libs/book/inc/book.h ------
MAGIC = b"RABOOK1\x00"
FORMAT_VERSION = 1
NIL = 0xFFFFFFFF
NODE_ELEMENT = 0
...
IMG_GRAY4 = 0
IMG_SVG = 1
# book_image_pixfmt_t: the raster depth stored in the image descriptor's
# former padding byte (book_image_t.pixel_format). 0 is the default that
...
# gray8 (lossless). Kept in lockstep with book.h.
PIXFMT_GRAY4 = 0
PIXFMT_GRAY8 = 1
# Header feature-flag bits (book_flag_t). The firmware validator rejects any
# bit outside its known mask, so only emit bits defined there.
FLAG_RTL = 0x00000001
# .rabook chunked container ("RBKC"; keep in sync with book_container_t in
