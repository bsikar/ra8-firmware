...
"""Generate apps/shared_libs/book/inc/book_library.h from compiled .rabook blobs.
...
(`g_book_library[]`) so firmware can list the bundled library and size the
...
def unwrap_container(data: bytes) -> bytes:
    ...
    Keep in sync with book_container_t in apps/shared_libs/book/inc/book.h:
...
def _generated_constants(entries: list[dict[str, object]], biggest: int) -> list[str]:
    ...
    return [
        ...
        f"  k_book_library_count = {len(entries)}U, /**< Bundled book count. */",
        ...
        f"  k_book_library_max_inflated = {biggest}U, /**< Worst-case inflated size. */",
...
def main() -> int:
    ...
    `g_book_library[]`. Anything that persists a book index -- a saved
    ...
    """
    ap = argparse.ArgumentParser(description="Generate the book library manifest header.")
