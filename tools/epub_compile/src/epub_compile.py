...
The on-device reader (apps/shared_libs/book) never unzips or parses XHTML at runtime.
...
def _run_selftest() -> int:
    """Load the test-only fixed-layout contract and return its result."""
    tests_dir = Path(__file__).resolve().parents[1] / "tests"
    sys.path.insert(0, str(tests_dir))
    from epub_selftest import selftest  # noqa: PLC0415 -- selftest path added above
