...
//!
//! Build graph for the `doxygen_md_filter` host tool (#858). One executable,
//! plus the test step `scripts/checks/check_zig.py --test` runs: the two
//! Markdown transforms and the exit-status contract the docs build depends on.
...
pub fn build(b: *std.Build) void {
    ...
    const run_step = b.step("run", "Filter one Markdown page to stdout");
    ...
    const test_step = b.step("test", "Run doxygen_md_filter tests");
