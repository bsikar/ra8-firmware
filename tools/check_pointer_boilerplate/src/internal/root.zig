...
///
/// ASCII upper case folds to lower. Python's `re.IGNORECASE` also matches a
/// handful of non-ASCII code points against an ASCII letter, because they
/// share an upper case with it; enumerated against the interpreter, the ones
/// that fold onto a letter this pattern contains are the long s and both
/// Turkish dotted forms of `i`, which the `internal ` word carries. The
/// Kelvin sign folds onto `k`, which this pattern does not contain; it is
/// kept so the rule is the same rule rather than a convenient subset.
