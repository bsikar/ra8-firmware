...
//!
//! Generated character classes for `check_mcdc_floor` (#858, #1205). CPython's
//! `int(text)` does not parse its argument directly: it first runs
...
//! `int("\u{3000}12")` is 12 and `int("\u{663}2")` is 32, while the ASCII
//! control characters U+001C..U+001F, which `str.isspace` also accepts, are
//! left alone by that transform and make the parse fail.
//!
//! The report this gate reads is machine-written and unvalidated upstream, so
//! a `covered_decisions` arriving as a string went through exactly that
//! transform before the predecessor coerced it.
//!
