...
//!
//! Build graph for the `check_mcdc_floor` host gate (#858). One executable,
//! plus the test step `scripts/checks/check_zig.py --test` runs: the pure
//! scope, arithmetic and rendering rules, and the exit-status contract that
//! `scripts/builders/check_mcdc_floor.sh` passes through.
...
pub fn build(b: *std.Build) void {
    ...
    const run_step = b.step("run", "Gate the per-file reachable-MC/DC floor");
