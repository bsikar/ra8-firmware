...
// checkAuthorityCanSign refuses a client certificate authority that cannot
...
func checkAuthorityCanSign(authority *x509.Certificate, where string) error {
	if authority == nil {
		return fmt.Errorf("%w: client certificate authority bundle holds no certificate", ErrIdentity)
	}
	if authority.KeyUsage != 0 && authority.KeyUsage&x509.KeyUsageCertSign == 0 {
		return fmt.Errorf("%w: %s in the client CA bundle may not sign certificates", ErrIdentity, where)
	}
	return nil
}
