...
func (s *Store) BeginBoardSegment(ctx context.Context, actor BoardActor, expectedVersion uint64, token board.Token, key string, bound, recoveryMargin time.Duration) (BoardSegment, error) {
	if s == nil || s.pool == nil || actor.id == "" || actor.kind == "system" ||
		!validBoardID(actor.boardID) || expectedVersion > math.MaxInt64 ||
		token.BoardID != actor.boardID || !validSegmentKey(key) ||
	...
	segment := BoardSegment{ID: id, BoardID: actor.boardID, LeaseID: token.LeaseID,
		Generation: token.Generation, Key: key, StartedAt: now, DeadlineAt: now.Add(bound),
		RecoveryMarginMS: uint64(recoveryMargin.Milliseconds())}
	_, err = tx.Exec(ctx, `INSERT INTO board_segments
		(id,board_id,lease_id,generation,actor_id,segment_key,started_at,deadline_at,recovery_margin_ms)
		VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`, segment.ID, segment.BoardID, segment.LeaseID,
		int64(segment.Generation), actor.id, segment.Key, segment.StartedAt, segment.DeadlineAt,
...
// lease holder. A timed-out/open segment remains durable until explicitly
// reconciled; it is never silently cleared by a later lease.
func (s *Store) FinishBoardSegment(ctx context.Context, actor BoardActor, segmentID string, token board.Token, outcome string) error {
	if s == nil || s.pool == nil || actor.id == "" || actor.kind == "system" ||
		!validBoardID(actor.boardID) || !validSegmentID(segmentID) || token.BoardID != actor.boardID ||
	...
	var leaseID, actorID string
	var generation int64
	var ended bool
	err = tx.QueryRow(ctx, `SELECT lease_id::text, generation, actor_id, ended_at IS NOT NULL
	...
	if err == pgx.ErrNoRows || leaseID != token.LeaseID || uint64(generation) != token.Generation || actorID != actor.id || ended {
	...
	if err := appendAudit(ctx, tx, actor.id, "board.segment.finished", "board", actor.boardID,
		"ok", "", "", "", map[string]any{"segment_id": segmentID, "lease_id": token.LeaseID, "generation": token.Generation, "outcome": outcome}); err != nil {
