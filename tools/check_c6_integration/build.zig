...
//!
//! Build graph for the `check_c6_integration` host gate (#858). One
//! executable, plus the test step `scripts/checks/check_zig.py --test` runs:
//! the five hand-written matchers and the contract checks, and the
//! exit-status contract `scripts/builders/check_c6_integration.sh` passes
//! through.
...
pub fn build(b: *std.Build) void {
    ...
    const run_step = b.step("run", "Gate the offline ESP32-C6 staging and link contract");
    ...
    const test_step = b.step("test", "Run check_c6_integration tests");
