/**
 * @file sim_tz.h
 * @brief TrustZone Secure/Non-Secure seams for board_sim
 *
 * @details
 ...
 * closes the gap in board_sim's single flat domain: it seeds SAU_TYPE with
 ...
 * fallback main() unless SAU_TYPE.SREGION is implemented; board_sim maps the
 ...
 * @since 0.1.0
 */
RA8_PRIV void sim_tz_install(uc_engine* uc, const uint8_t* elf, long elf_len);
 ...
 * stalling bring-up. board_sim collapses the S/NS split into one flat,
 ...
 * @since 0.1.0
 */
RA8_PRIV void sim_tz_patch_cmse(uc_engine* uc, const uint8_t* elf, long elf_len);
 ...
 * @since 0.1.0
 */
RA8_PRIV void sim_tz_set_ns_vector_base(uint32_t base);
 ...
 * @since 0.1.0
 */
RA8_PRIV uint32_t sim_tz_ns_vector_base(void);
