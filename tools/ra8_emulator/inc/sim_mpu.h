/**
 * @file sim_mpu.h
 * @brief Armv8-M MPU enforcement model for board_sim
 *
 * @details
 * The firmware programs the MPU via ra8_mpu (RNR -> RBAR -> RLAR per region,
 * then CTRL). board_sim watches those writes (no banked per-region storage in
 ...
 * @since 0.1.0
 */
RA8_PRIV void sim_mpu_install(uc_engine* uc);
 ...
 * @since 0.1.0
 */
RA8_PRIV bool sim_mpu_fault_pending(void);
 ...
 * @since 0.1.0
 */
RA8_PRIV void sim_mpu_clear_fault(void);
