 ...
 * installed by sim_memmap on the Secure window and its bit[28] Non-secure
 ...
 * @see sim_memmap_init()  Installs the callbacks over this window.
 * @since 0.1.0
 */
 ...
 * @pre The window was installed by sim_memmap_init().
 ...
 * the spin-settle toggle that mmio_read() uses. board_sim's own introspection
 ...
 * @since 0.1.0
 */
RA8_PRIV uint32_t sim_mmio_reads(void);
 ...
 * @since 0.1.0
 */
RA8_PRIV uint32_t sim_mmio_writes(void);
 ...
 * @see sim_mmio_print_bgc_and_table()  The rest of the MMIO report.
 * @since 0.1.0
 */
RA8_PRIV void sim_mmio_print_counts(void);
 ...
 * @note Not thread-safe; call once at run end.
 * @see sim_mmio_print_counts()  The counters line printed just before.
 * @since 0.1.0
 */
RA8_PRIV void sim_mmio_print_bgc_and_table(void);
