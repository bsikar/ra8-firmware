...
/** @brief Context adapting the legacy contiguous composite to a fill surface. */
typedef struct {
  uint16_t* pixels; /**< Caller-owned row-major RGB565 pixels. */
  uint16_t  stride; /**< Complete composite row width.         */
} overlay_memory_context_t;

/**
 * @brief Fill one already-clipped rectangle in a contiguous RGB565 surface.
 * @details Fill one already-clipped rectangle in a contiguous rgb565 surface; this step is contained within the board overlay model and uses bounded caller or module-owned storage.
 * @param[in,out] context Callback context registered for this operation.
 ...
 * @return The memory fill result produced by the board overlay model.
 * @retval true The memory fill condition holds or completed successfully; false otherwise.
 * @pre Arguments satisfy the ranges documented for memory fill. @pre The call executes on the emulator's single owning thread.
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static bool internal_memory_fill(void*    context,
                                              uint16_t x,
                                              uint16_t y,
                                              uint16_t width,
                                              uint16_t height,
                                              uint16_t color)
{
  overlay_memory_context_t* const memory = (overlay_memory_context_t*)context;
  if ((memory == nullptr) || (memory->pixels == nullptr)) {
    return false;
  }
  for (uint16_t row = 0U; row < height; row++) {
    uint16_t* const begin =
      &memory->pixels[((size_t)(y + row) * (size_t)memory->stride) + (size_t)x];
 ...
 * @details Draw one led indicator dot (filled when on) plus its caption; this step is contained within the board overlay model and uses bounded caller or module-owned storage.
 * @param[in,out] surface Descriptor-backed presentation surface to access.
 * @param[in] x Horizontal coordinate in pixels.
 * @param[in] y Vertical coordinate in pixels.
 * @param[in] led Led input used by the operation.
 * @pre Arguments satisfy the ranges documented for draw led. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the board overlay model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_draw_led(board_overlay_surface_t*  surface,
                                           int32_t                   x,
                                           int32_t                   y,
                                           const board_led_status_t* led)
{
  ...
  priv_fill_rect(surface, x - 1, y - 1, k_dot + 2, k_dot + 2, (uint16_t)k_ovl_led_ring);
  const uint16_t fill = led->on ? led->color : (uint16_t)k_ovl_led_off;
  priv_fill_rect(surface, x, y, k_dot, k_dot, fill);
  priv_draw_text(surface,
                 x + k_dot + 8,
                 y + k_led_label_dy,
 ...
 * @details Paint the three led indicators in a row; returns the next free y; this step is contained within the board overlay model and uses bounded caller or module-owned storage.
 * @param[in,out] surface Descriptor-backed presentation surface to access.
 * @param[in] x Horizontal coordinate in pixels.
 * @param[in] y Vertical coordinate in pixels.
 * @param[in] st Board-status snapshot used to render or report state.
 * @return The draw leds result produced by the board overlay model.
 * @retval value The operation-specific draw leds value.
 * @pre Arguments satisfy the ranges documented for draw leds. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the board overlay model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static int32_t
internal_draw_leds(board_overlay_surface_t* surface, int32_t x, int32_t y, const board_status_t* st)
{
  const int32_t row_y = priv_section_head(surface, x, y, "LEDS");
  for (uint32_t i = 0U; i < (uint32_t)k_overlay_led_count; i++) {
    internal_draw_led(surface, x + ((int32_t)i * k_led_col_dx), row_y, &st->leds[i]);
  }
 ...
 * @details Paint the run-stats block (pc / chunks / mmio / state); next free y; this step is contained within the board overlay model and uses bounded caller or module-owned storage.
 * @param[in,out] surface Descriptor-backed presentation surface to access.
 * @param[in] x Horizontal coordinate in pixels.
 * @param[in] y Vertical coordinate in pixels.
 * @param[in] st Board-status snapshot used to render or report state.
 * @return The draw run stats result produced by the board overlay model.
 * @retval value The operation-specific draw run stats value.
 * @pre Arguments satisfy the ranges documented for draw run stats. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the board overlay model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static int32_t internal_draw_run_stats(board_overlay_surface_t* surface,
                                                    int32_t                  x,
                                                    int32_t                  y,
                                                    const board_status_t*    st)
{
  ...
  int32_t cy = priv_section_head(surface, x, y, "RUN");
  (void)snprintf(buf, sizeof(buf), "0x%08X", st->pc);
  cy = priv_kv_row(surface, x, cy, "pc", buf, (uint16_t)k_ovl_heading);
  (void)snprintf(buf, sizeof(buf), "%s   chunk %u", st->running ? "RUNNING" : "parked", st->chunks);
  cy = priv_kv_row(surface,
                   x,
                   cy,
                   "state",
                   buf,
                   st->running ? (uint16_t)k_ovl_ok : (uint16_t)k_ovl_dim);
  (void)snprintf(buf, sizeof(buf), "%u rd / %u wr", st->mmio_reads, st->mmio_writes);
  cy = priv_kv_row(surface, x, cy, "mmio", buf, (uint16_t)k_ovl_text);
  return cy;
}

/**
 * @brief Paint the I/O block (USB / IRQ / touch / SD); returns next free y.
 * @details Paint the i/o block (usb / irq / touch / sd); returns next free y; this step is contained within the board overlay model and uses bounded caller or module-owned storage.
 * @param[in,out] surface Descriptor-backed presentation surface to access.
 * @param[in] x Horizontal coordinate in pixels.
 * @param[in] y Vertical coordinate in pixels.
 * @param[in] st Board-status snapshot used to render or report state.
 * @return The draw io block result produced by the board overlay model.
 * @retval value The operation-specific draw io block value.
 * @pre Arguments satisfy the ranges documented for draw io block. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the board overlay model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static int32_t internal_draw_io_block(board_overlay_surface_t* surface,
                                                   int32_t                  x,
                                                   int32_t                  y,
                                                   const board_status_t*    st)
{
  ...
  int32_t cy = priv_section_head(surface, x, y, "I/O");
  cy         = priv_kv_row(surface,
                           x,
                           cy,
                           "usb",
  ...
  cy = priv_kv_row(surface, x, cy, "irq", buf, (uint16_t)k_ovl_text);
  if (st->has_touch) {
    (void)snprintf(buf, sizeof(buf), "%u, %u", st->touch_x, st->touch_y);
  } else {
    (void)snprintf(buf, sizeof(buf), "-");
  }
  cy                        = priv_kv_row(surface, x, cy, "touch", buf, (uint16_t)k_ovl_text);
  ...
  cy = priv_kv_row(surface, x, cy, "sd", buf, (uint16_t)k_ovl_text);
  return cy;
}
 ...
 * than exist. Both ::internal_console_tab_rect and ::internal_console_tab_row_count build on this.
 *
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static uint32_t internal_console_tabs_per_row(int32_t panel_w, uint32_t count)
{
  if (count == 0U) {
    return 1U;
  }
 ...
 *
 * @details Ceiling-divides @p count by ::internal_console_tabs_per_row, so the tab bar is
 * exactly tall enough to show every lane. ::internal_draw_console and the hit-test use it
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static uint32_t internal_console_tab_row_count(int32_t panel_w, uint32_t count)
{
  if (count == 0U) {
    return 0U;
  }
  const uint32_t per_row = internal_console_tabs_per_row(panel_w, count);
 ...
 *
 * @details The bar wraps into ::internal_console_tabs_per_row columns; cell @p idx lands at
 ...
 * ::internal_draw_console_tabs and ::board_overlay_hit_console_tab derive their geometry
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_console_tab_rect(int32_t  panel_x,
                                                   int32_t  panel_w,
                                                   uint32_t idx,
                                                   uint32_t count,
                                                   int32_t* r)
 ...
 * from ::internal_console_tab_rect so the drawn cells line up with the click hit-test.
 *
 * @param[in,out] surface Clipped composite rectangle sink.
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_draw_console_tabs(board_overlay_surface_t* surface,
                                                    int32_t                  panel_x,
                                                    int32_t                  panel_w,
                                                    const board_status_t*    st)
{
  ...
  for (uint32_t i = 0U; i < count; i++) {
    ...
    internal_console_tab_rect(panel_x, panel_w, i, count, rect);
    ...
    priv_fill_rect(surface, rect[0], rect[1], rect[2], rect[3], bg);
    enum : uint8_t {
 ...
 *
 * @pre @p surface is valid and @p st is non-NULL.
 * @pre The sidebar geometry constants fit inside @p surface.
...
internal_draw_console_heading(board_overlay_surface_t* surface, int32_t x, const board_status_t* st)
{
  enum : uint8_t {
  ...
  priv_draw_text(surface, x, (int32_t)k_con_head_y, buf, head_col, 1);
 ...
 * @note Not thread-safe; the overlay is drawn from one thread.
  * @since 0.1.0
 */
RA8_INTERNAL static void internal_draw_console_body(board_overlay_surface_t* surface,
                                                    int32_t                  panel_x,
                                                    int32_t                  body_y,
                                                    int32_t                  body_h,
                                                    const board_status_t*    st)
{
  ...
  for (int32_t i = 0; i < rows; i++) {
    ...
    priv_draw_text(surface, panel_x + (int32_t)k_con_pad, ly, st->console[i], col, 1);
  }
}

/**
 * @brief Perform draw console for the board overlay model.
 * @details Perform draw console for the board overlay model; this step is contained within the board overlay model and uses bounded caller or module-owned storage.
 * @param[in,out] surface Descriptor-backed presentation surface to access.
 * @param[in] x Horizontal coordinate in pixels.
 * @param[in] st Board-status snapshot used to render or report state.
 * @pre Arguments satisfy the ranges documented for draw console. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the board overlay model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void
internal_draw_console(board_overlay_surface_t* surface, int32_t x, const board_status_t* st)
{
  internal_draw_console_heading(surface, x, st);
  const int32_t panel_x = x;
  const int32_t panel_w = (int32_t)k_ovl_sidebar_w - (2 * (int32_t)k_pad_x);
  const int32_t panel_h = (int32_t)surface->height - (int32_t)k_con_y - (int32_t)k_con_bottom;
  if (panel_h < (int32_t)k_con_line_h) {
    return;
  }
  priv_fill_rect(surface, panel_x, (int32_t)k_con_y, panel_w, panel_h, (uint16_t)k_ovl_console_bg);
  /* The tab bar wraps into as many rows as the channel count needs; the scrolling
   * body fills whatever is left below it. */
  internal_draw_console_tabs(surface, panel_x, panel_w, st);
  ...
  const int32_t tab_rows = (int32_t)internal_console_tab_row_count(panel_w, tab_count);
  ...
  internal_draw_console_body(surface, panel_x, body_y, body_h, st);
}

/**
 * @brief Draw one labelled push-button face at @p x (green when @p pressed).
 * @details Draw one labelled push-button face at @p x (green when @p pressed); this step is contained within the board overlay model and uses bounded caller or module-owned storage.
 * @param[in,out] surface Descriptor-backed presentation surface to access.
 * @param[in] x Horizontal coordinate in pixels.
 * @param[in] label NUL-terminated label used by the operation.
 * @param[in] pressed Pressed input used by the operation.
 * @pre Arguments satisfy the ranges documented for draw button. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the board overlay model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void
internal_draw_button(board_overlay_surface_t* surface, int32_t x, const char* label, bool pressed)
{
  ...
  priv_fill_rect(surface, x, (int32_t)k_btn_y, (int32_t)k_btn_w, (int32_t)k_btn_h, face);
  priv_draw_text(surface,
 ...
 * @details Paint the "buttons" heading and the clickable sw1 / sw2 buttons; this step is contained within the board overlay model and uses bounded caller or module-owned storage.
 * @param[in,out] surface Descriptor-backed presentation surface to access.
 * @param[in] x Horizontal coordinate in pixels.
 * @param[in] st Board-status snapshot used to render or report state.
 * @pre Arguments satisfy the ranges documented for draw buttons. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the board overlay model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void
internal_draw_buttons(board_overlay_surface_t* surface, int32_t x, const board_status_t* st)
{
  (void)priv_section_head(surface, x, (int32_t)k_btn_head_y, "BUTTONS  (click)");
  internal_draw_button(surface, x, "SW1", st->sw1_pressed);
  internal_draw_button(surface, x + (int32_t)k_btn_w + (int32_t)k_btn_gap, "SW2", st->sw2_pressed);
}

/**
 * @brief Clamp a SOC value to 0..100 for display.
 * @details Clamp a soc value to 0..100 for display; this step is contained within the board overlay model and uses bounded caller or module-owned storage.
 * @param[in] soc Soc input used by the operation.
 * @return The battery clamp result produced by the board overlay model.
 * @retval value The operation-specific battery clamp value.
 * @pre Arguments satisfy the ranges documented for battery clamp. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the board overlay model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static uint8_t internal_battery_clamp(uint8_t soc)
{
 ...
 * @details Pick the battery gauge fill colour: green charging, else red/amber/green by level; this step is contained within the board overlay model and uses bounded caller or module-owned storage.
 * @param[in] soc Soc input used by the operation.
 * @param[in] charging Charging input used by the operation.
 * @return The battery fill color result produced by the board overlay model.
 * @retval value The operation-specific battery fill color value.
 * @pre Arguments satisfy the ranges documented for battery fill color. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the board overlay model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static uint16_t internal_battery_fill_color(uint8_t soc, bool charging)
{
  if (charging) {
 ...
 * @param[in]     st      Live status snapshot (battery_soc / battery_charging).
  * @pre Arguments satisfy the ranges documented for draw power. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the board overlay model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void
internal_draw_power(board_overlay_surface_t* surface, int32_t x, const board_status_t* st)
{
  (void)priv_section_head(surface, x, (int32_t)k_pwr_head_y, "POWER  (drag)");
  const uint8_t  soc    = internal_battery_clamp(st->battery_soc);
  const int32_t  fill_w = ((int32_t)k_pwr_track_w * (int32_t)soc) / (int32_t)k_pwr_soc_full;
  const uint16_t fill   = internal_battery_fill_color(soc, st->battery_charging);
  ...
  priv_fill_rect(surface, x, (int32_t)k_pwr_y, fill_w, (int32_t)k_pwr_h, fill);
  char buf[16];
  ...
  priv_fill_rect(surface,
                 ...
                 (int32_t)k_pwr_h + 2,
                 (uint16_t)k_ovl_btn_border);
  priv_fill_rect(surface, cx, (int32_t)k_pwr_y, (int32_t)k_pwr_chg_w, (int32_t)k_pwr_h, face);
  priv_draw_text(surface,
 ...
 * @param[in] x Horizontal coordinate in pixels.
 * @param[in] st Board-status snapshot used to render or report state.
 * @pre Arguments satisfy the ranges documented for draw core. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the board overlay model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void
internal_draw_core(board_overlay_surface_t* surface, int32_t x, const board_status_t* st)
{
  ...
  priv_fill_rect(surface,
                 ...
                 (int32_t)k_btn_h + 2,
                 (uint16_t)k_ovl_btn_border);
  priv_fill_rect(surface, lx, (int32_t)k_btn_y, (int32_t)k_core_lp_w, (int32_t)k_btn_h, face);
  char cbuf[16];
  (void)snprintf(cbuf,
                 sizeof(cbuf),
                 "%s %s",
...
bool board_overlay_draw_sidebar(board_overlay_surface_t* surface,
                                ...
                                const board_status_t*    st)
{
  if ((surface == nullptr) || !surface->ok || (surface->fill == nullptr) || (panel_w == 0U) ||
      (surface->width != board_overlay_total_width(panel_w)) ||
      (surface->height < board_overlay_total_height(1U))) {
    return false;
  }
  priv_fill_rect(surface,
                 (int32_t)panel_w,
  ...
  priv_draw_text(surface, x, y, "EK-RA8D2  EMULATOR", (uint16_t)k_ovl_accent, 2);
  ...
  y = internal_draw_run_stats(surface, x, y, st);
  y += (int32_t)k_section_gap;
  (void)internal_draw_leds(surface, x, y, st);
  (void)internal_draw_io_block(surface, x, (int32_t)k_io_head_y, st);
  internal_draw_power(surface, x, st);
  internal_draw_core(surface, x, st);
  internal_draw_buttons(surface, x, st);
  internal_draw_console(surface, x, st);
  return surface->ok;
}
                           ...
                           const board_status_t* st)
{
  ...
  priv_blit_panel(out, w, h, panel, panel_w, panel_h);
  overlay_memory_context_t memory  = {.pixels = out, .stride = w};
  board_overlay_surface_t  surface = {.context = &memory,
                                      .fill    = internal_memory_fill,
                                      .width   = w,
                                      .height  = h,
                                      .ok      = true};
  (void)board_overlay_draw_sidebar(&surface, panel_w, st);
}
                                   ...
                                   uint32_t* out_idx)
{
  ...
  for (uint32_t i = 0U; i < count; i++) {
    int32_t rect[4] = {0, 0, 0, 0};
    internal_console_tab_rect(origin_x, pan_w, i, count, rect);
    if (cx < rect[0]) {
      continue;
    }
    if (cx >= (rect[0] + rect[2])) {
