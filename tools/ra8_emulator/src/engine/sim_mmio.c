/**
 * @file sim_mmio.c
 * @brief Sparse MMIO model implementation (see sim_mmio.h)
 *
 * @details
 * The slot table, settle toggle, MRMS readback quirk, BG_BGC witness and the
 * run-end report -- moved verbatim out of the board_sim main translation
...
void mmio_write(uc_engine* uc, uint64_t offset, unsigned size, uint64_t value, void* user)
{
   ...
   * captures both so the run loop can boot the second engine (see sim_cpu1). */
  sim_cpu1_notify_mmio_write(mmio_abs, value);
...
/** @brief Implementation of `sim_mmio_reads()` -- plain counter read. */
uint32_t sim_mmio_reads(void)
{
  return s_mmio_reads;
}

/** @brief Implementation of `sim_mmio_writes()` -- plain counter read. */
uint32_t sim_mmio_writes(void)
{
  return s_mmio_writes;
}

/** @brief Implementation of `sim_mmio_print_counts()` -- run-end report line. */
void sim_mmio_print_counts(void)
{
  (void)fprintf(stderr,
...
/** @brief Implementation of `sim_mmio_print_bgc_and_table()` -- run-end report. */
void sim_mmio_print_bgc_and_table(void)
{
