...
RA8_INTERNAL static void internal_run_classify_click(const emu_run_cfg_t* cfg, run_loop_t* st)
{
  ...
  if (board_overlay_hit_console_tab((uint16_t)cfg->click_x,
                                    ...
                                    &click_tab_idx)) {
    emu_view_select_console_tab(click_tab_idx);
 ...
 *
 * @details Charges the previous chunk's wall time to its start PC (RA8_EMU_
...
RA8_INTERNAL static bool internal_run_stop_prof_idle(run_loop_t* st)
{
  ...
  const uint64_t d     = emu_prof_total_insns() - st->prof_idle_prev_i;
  st->prof_idle_prev_i = emu_prof_total_insns();
...
/**
 * @brief Steady-state idle early-stop (RA8_EMU_IDLE_STOP).
 *
...
RA8_INTERNAL static bool internal_run_stop_idle(run_loop_t* st)
{
  ...
  const uint64_t idle_sig = (uint64_t)emu_mmio_reads() + (uint64_t)emu_mmio_writes() +
                            (uint64_t)emu_exc_pendsv_takes() + (uint64_t)emu_exc_svc_takes() +
...
RA8_INTERNAL static bool internal_run_stop_usb(run_loop_t* st)
{
  ...
  if ((st->guards.usbh_stop_settle > 0U) && !recording && emu_usbh_done()) {
    st->usbh_stop_run++;
...
/**
 * @brief Generic console-banner early-stop (RA8_EMU_STOP_ON).
 *
