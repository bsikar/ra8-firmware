 ...
 * run loop, which clears it via ::sim_mve_nocp_take and relaunches instead of
 * ending the run -- the same shape as the MPU and divide-by-zero latches.
 *
 * @note Read and cleared only by ::sim_mve_nocp_take.
 ...
 * ::sim_mve_nocp_spurious for why that report happens.
 *
 ...
 * ::sim_mve_nocp_spurious call whether or not it matched, so at most one
 * report is ever absorbed per serviced fault.
 *
 * @note Read and cleared only by ::sim_mve_nocp_spurious.
 ...
 * NoCP hook through ::sim_mve_nocp_emulate, and the invalid-instruction
...
bool sim_mve_nocp_emulate(uc_engine* uc, uint32_t pc)
{
...
bool sim_mve_nocp_spurious(uint32_t pc)
{
...
/** @brief Implementation of `sim_mve_nocp_take()` -- test-and-clear the latch. */
bool sim_mve_nocp_take(void)
{
