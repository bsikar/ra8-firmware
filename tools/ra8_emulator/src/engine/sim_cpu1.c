/**
 * @file sim_cpu1.c
 * @brief Second-core (cpu1) engine implementation (see sim_cpu1.h)
 *
 * @details
...
/** @brief Implementation of `sim_cpu1_notify_mmio_write()` -- release capture. */
void sim_cpu1_notify_mmio_write(uint64_t mmio_abs, uint64_t value)
{
...
/** @brief Implementation of `sim_cpu1_init()` -- detect + build the engine. */
void sim_cpu1_init(const uint8_t* elf, long elf_len)
{
  s_cpu1_uc = cpu1_engine_init(elf, elf_len);
}

/** @brief Implementation of `sim_cpu1_step()` -- release boot + one interleave. */
void sim_cpu1_step(void)
{
