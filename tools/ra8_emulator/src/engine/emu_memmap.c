 ...
 * so ra8_emulator backs them with a single host buffer (mirrors the SRAM/NS_SRAM2
...
/** @brief Implementation of `emu_memmap_init()` -- regions, TSN seed, MMIO windows. */
bool emu_memmap_init(uc_engine* uc)
{
...
/** @brief Implementation of `emu_memmap_sram_buf()` -- shared backing access. */
uint8_t* emu_memmap_sram_buf(void)
{
  return s_sram_buf;
}
