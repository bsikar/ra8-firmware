 ...
 * first-party `ra8_usb_host_*` primitives, and with no peer the control
 * transfer wedges (SUREQ never acked -> k_ra8_err_busy). Rather than model a
 * second controller register-by-register, seam those primitives the same way
 * ra8_eth_* is seamed to a virtual net peer (the register model "cannot
 * satisfy" that sequence either): present a virtual boot keyboard that answers
 * chapter-9 enumeration and streams interrupt-IN reports. This lets a host
 * example (usb_host_keyboard) enumerate + read reports end to end with no
 * hardware -- validating the host stack's control/data logic, not silicon
 * timing.
 * ==========================================================================*/
...
/** @brief Standard boot-keyboard HID REPORT descriptor (63 bytes, USB HID 1.11
 * E.6). */
...
static const uint8_t s_k_vkbd_keycodes[k_vkbd_num_keys] = {k_vkbd_key_r,
                                                           k_vkbd_key_a,
                                                           k_vkbd_key_8,
                                                           k_vkbd_key_d,
                                                           k_vkbd_key_2};
 ...
 * @details Read the 5th (stack-passed) argument of an aapcs call: mem32[sp]; this step is contained within the emu USB host seam model and uses bounded caller or module-owned storage.
 * @param[in,out] uc Unicorn engine whose emulated state is read or updated.
 * @return The USB host arg5 result produced by the emu USB host seam model.
 * @retval value The operation-specific USB host arg5 value.
 * @pre Arguments satisfy the ranges documented for USB host arg5. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static uint32_t internal_usbh_arg5(uc_engine* uc)
{
  uint32_t sp = 0U;
  uint32_t p  = 0U;
  (void)uc_reg_read(uc, UC_ARM_REG_SP, &sp);
  (void)emu_mem_read(uc, (uint64_t)sp, &p, sizeof(p));
  return p;
}
 ...
 * @pre Arguments satisfy the ranges documented for on USB host ok. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void
internal_on_usbh_ok(uc_engine* uc, uint64_t address, uint32_t size, void* user)
{
  (void)address;
  (void)size;
  (void)user;
 ...
 * @param[in] size Size of the requested region or access in bytes.
 * @param[in,out] user Hook context supplied when the callback was registered.
 * @pre Arguments satisfy the ranges documented for on USB host line state. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void
internal_on_usbh_line_state(uc_engine* uc, uint64_t address, uint32_t size, void* user)
{
  (void)address;
  (void)size;
  (void)user;
 ...
 * @param[in] size Size of the requested region or access in bytes.
 * @param[in,out] user Hook context supplied when the callback was registered.
 * @pre Arguments satisfy the ranges documented for on USB host control xfer. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void
internal_on_usbh_control_xfer(uc_engine* uc, uint64_t address, uint32_t size, void* user)
{
  (void)address;
  (void)size;
  (void)user;
  ...
  const uint32_t out_ptr = internal_usbh_arg5(uc);

  uint8_t s[8] = {};
  (void)emu_mem_read(uc, (uint64_t)setup_ptr, s, sizeof(s));
   ...
   */
  if (out_ptr != 0U) {
    (void)emu_mem_write(uc, (uint64_t)out_ptr, &n, sizeof(n));
  }
 ...
 * @param[in] size Size of the requested region or access in bytes.
 * @param[in,out] user Hook context supplied when the callback was registered.
 * @pre Arguments satisfy the ranges documented for on USB host bulk in. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void
internal_on_usbh_bulk_in(uc_engine* uc, uint64_t address, uint32_t size, void* user)
{
  (void)address;
  (void)size;
  (void)user;
  ...
  if (buf != 0U) {
    (void)emu_mem_write(uc, (uint64_t)buf, rep, n);
  }
  if (out_ptr != 0U) {
    (void)emu_mem_write(uc, (uint64_t)out_ptr, &n, sizeof(n));
  }
 ...
 * MRAM.BIN is the 1 MiB MRAM code window. Seams the first-party
 * `ra8_usb_hmsc_*` class API (one level above the BOT/SCSI bulk transport) so a
 * host MSC app (usb_host_msc_browse) enumerates, READ_CAPACITYs, mounts the
 * FAT16, browses the root directory, and content-verifies MRAM.BIN -- all with
 * no peer device. The boot/FAT/root sectors are a byte-identical replica of the
 * device's selftest_fat_fill_sector; the data region is read live from emulated
 * MRAM, so it matches the host's own MRAM compare byte-for-byte.
 * --------------------------------------------------------------------------*/
 ...
 *  last host step before usb_host_msc_browse's PASS (read by the USBH_STOP
 * guard).*/
static bool s_vmsc_write_seen = false;

/** @brief True when the virtual disk is writable (usb_host_file_ops links
 *  `fileops_backend_write`); else the disk is read-only and WRITE(10) is
 * rejected.*/
 ...
 * @details A writable host (usb_host_file_ops) creates a file: ra8_fs rewrites
 * a few FAT / root / data sectors. We keep those writes in a small overlay so
 * the read-back reads them back; everything else is still synthesized on the
 * fly.
 */
typedef struct {
...
/** @brief Write overlay for the writable disk (file_ops touches only a
 * handful). */
static vmsc_overlay_t s_vmsc_overlay[k_vmsc_overlay_slots];

/**
 * @brief Return an overwritten sector if @p lba is in the overlay.
 * @details Return an overwritten sector if @p lba is in the overlay; this step is contained within the emu USB host seam model and uses bounded caller or module-owned storage.
 * @param[in] lba Logical block address selected by the storage operation.
 * @param[out] out Destination storage receiving the computed result.
 * @return The vmsc overlay get result produced by the emu USB host seam model.
 * @retval true The vmsc overlay get condition holds or completed successfully; false otherwise.
 * @pre Arguments satisfy the ranges documented for vmsc overlay get. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static bool internal_vmsc_overlay_get(uint32_t lba, uint8_t* out)
{
...
/** @brief Record an overwritten sector (update existing slot or take a free
 * one).  * @details Record an overwritten sector (update existing slot or take a free; this step is contained within the emu USB host seam model and uses bounded caller or module-owned storage.
 * @param[in] lba Logical block address selected by the storage operation.
 * @param[in] in In input used by the operation.
 * @pre Arguments satisfy the ranges documented for vmsc overlay put. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_vmsc_overlay_put(uint32_t lba, const uint8_t* in)
{
 ...
 * @details Little-endian 16-bit store into a sector buffer; this step is contained within the emu USB host seam model and uses bounded caller or module-owned storage.
 ...
 * @pre Arguments satisfy the ranges documented for vmsc put16. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_vmsc_put16(uint8_t* p, uint16_t v)
{
 ...
 * @details Little-endian 32-bit store into a sector buffer; this step is contained within the emu USB host seam model and uses bounded caller or module-owned storage.
 * @param[in,out] p Module-owned state object processed by the operation.
 * @param[in] v Register or payload value processed by the operation.
 * @pre Arguments satisfy the ranges documented for vmsc put32. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_vmsc_put32(uint8_t* p, uint32_t v)
{
 ...
 * @details Synthesize the fat16 boot sector (bpb), mirroring the device side; this step is contained within the emu USB host seam model and uses bounded caller or module-owned storage.
 * @param[out] out Destination storage receiving the computed result.
 * @pre Arguments satisfy the ranges documented for vmsc fill boot. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_vmsc_fill_boot(uint8_t* out)
{
  out[0] = (uint8_t)k_bpb_jmp0;
  out[1] = (uint8_t)k_bpb_jmp1;
  out[2] = (uint8_t)k_bpb_jmp2; /* jmp + nop */
  (void)memcpy(&out[k_bpb_off_oem], s_k_vmsc_oem, sizeof(s_k_vmsc_oem));
  internal_vmsc_put16(&out[k_bpb_off_bytspersec], (uint16_t)k_vmsc_block_size);
  out[k_bpb_off_secperclus] = (uint8_t)k_bpb_secperclus_1; /* sectors/cluster */
  internal_vmsc_put16(&out[k_bpb_off_rsvdseccnt],
                      (uint16_t)k_bpb_rsvdseccnt_1); /* reserved sectors */
  out[k_bpb_off_numfats] = (uint8_t)k_bpb_numfats_1; /* number of FATs   */
  internal_vmsc_put16(&out[k_bpb_off_rootentcnt],
                      (uint16_t)k_bpb_rootentcnt_512); /* root entries */
  internal_vmsc_put16(&out[k_bpb_off_totsec16], (uint16_t)k_vmsc_total_sectors);
  out[k_bpb_off_media] = (uint8_t)k_bpb_media_f8;                           /* media descriptor */
  internal_vmsc_put16(&out[k_bpb_off_fatsz16], (uint16_t)k_bpb_fatsz16_17); /* sectors per FAT  */
  internal_vmsc_put16(&out[k_bpb_off_secpertrk],
                      (uint16_t)k_bpb_secpertrk_32); /* sectors per track */
  internal_vmsc_put16(&out[k_bpb_off_numheads], (uint16_t)k_bpb_numheads_16); /* heads          */
  out[k_bpb_off_drvnum]  = (uint8_t)k_bpb_drvnum_80;                          /* drive number   */
  out[k_bpb_off_bootsig] = (uint8_t)k_bpb_bootsig_29;                         /* boot signature */
  internal_vmsc_put32(&out[k_bpb_off_volid], (uint32_t)k_vmsc_volid);
  (void)memcpy(&out[k_bpb_off_vollab], s_k_vmsc_label, sizeof(s_k_vmsc_label));
  (void)memcpy(&out[k_bpb_off_filsystype], s_k_vmsc_fstype, sizeof(s_k_vmsc_fstype));
 ...
 * @details Synthesize one fat sector: mram.bin chains clusters 2..2049; this step is contained within the emu USB host seam model and uses bounded caller or module-owned storage.
 * @param[in] fat_sector Fat sector input used by the operation.
 * @param[out] out Destination storage receiving the computed result.
 * @pre Arguments satisfy the ranges documented for vmsc fill fat. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_vmsc_fill_fat(uint32_t fat_sector, uint8_t* out)
{
  ...
  for (uint32_t j = 0U; j < (uint32_t)k_vmsc_entries_per_fs; j++) {
    ...
    internal_vmsc_put16(&out[(size_t)j * 2U], value);
  }
}

/**
 * @brief Synthesize root-directory sector 0: volume label + MRAM.BIN entry.
 * @details Synthesize root-directory sector 0: volume label + mram.bin entry; this step is contained within the emu USB host seam model and uses bounded caller or module-owned storage.
 * @param[in] root_sector Root sector input used by the operation.
 * @param[out] out Destination storage receiving the computed result.
 * @pre Arguments satisfy the ranges documented for vmsc fill root. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_vmsc_fill_root(uint32_t root_sector, uint8_t* out)
{
  if (root_sector != 0U) {
    return;
  }
  (void)memcpy(&out[0], s_k_vmsc_label, sizeof(s_k_vmsc_label));
  out[k_dir_off_attr] = (uint8_t)k_dir_attr_vollabel; /* volume-label attribute */
  uint8_t* e          = &out[k_dir_off_entry];
  (void)memcpy(e, s_k_vmsc_fname, sizeof(s_k_vmsc_fname));
  e[k_dir_off_attr] = (uint8_t)k_dir_attr_readonly; /* read-only attribute */
  internal_vmsc_put16(&e[k_dir_off_fstcluslo], (uint16_t)k_vmsc_first_cluster);
  internal_vmsc_put32(&e[k_dir_off_filesize], (uint32_t)k_vmsc_file_bytes);
}

/**
 * @brief Fill one 512-byte volume sector (boot / FAT / root / live MRAM data).
 * @details Fill one 512-byte volume sector (boot / fat / root / live mram data); this step is contained within the emu USB host seam model and uses bounded caller or module-owned storage.
 * @param[in,out] uc Unicorn engine whose emulated state is read or updated.
 * @param[in] lba Logical block address selected by the storage operation.
 * @param[out] out Destination storage receiving the computed result.
 * @pre Arguments satisfy the ranges documented for vmsc fill sector. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_vmsc_fill_sector(uc_engine* uc, uint32_t lba, uint8_t* out)
{
  (void)memset(out, 0, (size_t)k_vmsc_block_size);
  if (lba == 0U) {
    internal_vmsc_fill_boot(out);
  } else if (lba < (uint32_t)k_vmsc_root_lba) {
    internal_vmsc_fill_fat(lba - 1U, out);
  } else if (lba < (uint32_t)k_vmsc_data_lba) {
    internal_vmsc_fill_root(lba - (uint32_t)k_vmsc_root_lba, out);
  } else {
    ...
    if (cluster <= (uint32_t)k_vmsc_last_mram_clus) {
      ...
      (void)emu_mem_read(uc,
                         (uint64_t)k_vmsc_mram_base + (uint64_t)off,
                         out,
                         (size_t)k_vmsc_block_size);
    }
  }
}
 ...
 * @param[in] size Size of the requested region or access in bytes.
 * @param[in,out] user Hook context supplied when the callback was registered.
 * @pre Arguments satisfy the ranges documented for on hmsc ok. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void
internal_on_hmsc_ok(uc_engine* uc, uint64_t address, uint32_t size, void* user)
{
  (void)address;
  (void)size;
  (void)user;
...
/** @brief ra8_usb_hmsc_device_t field offsets + reported bulk EP
 * packet/VID/PID. */
typedef enum : uint32_t {
 ...
 * @param[in] size Size of the requested region or access in bytes.
 * @param[in,out] user Hook context supplied when the callback was registered.
 * @pre Arguments satisfy the ranges documented for on hmsc enumerate. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void
internal_on_hmsc_enumerate(uc_engine* uc, uint64_t address, uint32_t size, void* user)
{
  (void)address;
  (void)size;
  (void)user;
  ...
  if (dev_ptr != 0U) {
    ...
    d[1]                        = 1U;                                     /* bulk_in_ep          */
    d[2]                        = 2U;                                     /* bulk_out_ep         */
    internal_vmsc_put16(&d[6], (uint16_t)k_hmsc_bulk_mps);                /* bulk_in_max_packet  */
    internal_vmsc_put16(&d[8], (uint16_t)k_hmsc_bulk_mps);                /* bulk_out_max_packet */
    internal_vmsc_put16(&d[k_hmsc_off_vid], (uint16_t)k_hmsc_vendor_id);  /* vendor_id           */
    internal_vmsc_put16(&d[k_hmsc_off_pid], (uint16_t)k_hmsc_product_id); /* product_id          */
    (void)emu_mem_write(uc, (uint64_t)dev_ptr, d, sizeof(d));
  }
  eth_hook_return(uc, 0U); /* k_ra8_ok */
}
 ...
 * @param[in] size Size of the requested region or access in bytes.
 * @param[in,out] user Hook context supplied when the callback was registered.
 * @pre Arguments satisfy the ranges documented for on hmsc read capacity. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void
internal_on_hmsc_read_capacity(uc_engine* uc, uint64_t address, uint32_t size, void* user)
{
  (void)address;
  (void)size;
  (void)user;
  ...
  if (bc_ptr != 0U) {
    (void)emu_mem_write(uc, (uint64_t)bc_ptr, &block_count, sizeof(block_count));
  }
  if (bs_ptr != 0U) {
    (void)emu_mem_write(uc, (uint64_t)bs_ptr, &block_size, sizeof(block_size));
  }
  eth_hook_return(uc, 0U); /* k_ra8_ok */
}
 ...
 * @param[in] size Size of the requested region or access in bytes.
 * @param[in,out] user Hook context supplied when the callback was registered.
 * @pre Arguments satisfy the ranges documented for on hmsc read10. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void
internal_on_hmsc_read10(uc_engine* uc, uint64_t address, uint32_t size, void* user)
{
  (void)address;
  (void)size;
  (void)user;
  ...
  for (uint32_t i = 0U; (i < count) && (buf != 0U); i++) {
    ...
    if (!internal_vmsc_overlay_get(lba + i, sec)) { /* a host WRITE(10) wins over the synthesis. */
      internal_vmsc_fill_sector(uc, lba + i, sec);
    }
 ...
 * @param[in] size Size of the requested region or access in bytes.
 * @param[in,out] user Hook context supplied when the callback was registered.
 * @pre Arguments satisfy the ranges documented for on hmsc write10. @pre The call executes on the emulator's single owning thread.
 * @post State changes remain confined to the emu USB host seam model and documented output objects. @post Ownership of caller-supplied storage is unchanged.
 * @note The operation is synchronous and does not transfer heap ownership.
 * @since 0.1.0
 */
RA8_INTERNAL static void
internal_on_hmsc_write10(uc_engine* uc, uint64_t address, uint32_t size, void* user)
{
  (void)address;
  (void)size;
  (void)user;
  if (!s_vmsc_writable) {
    /* Read-only disk (usb_host_msc_browse): reject. This is that app's last
     * step before PASS, so flag it for the host early-stop. */
   ...
   * so the host's read-back sees them, and accept. The write is mid-ladder
   * here, so do NOT trip the write-seen early-stop -- that app stops on its
   * banner. */
  uint32_t lba   = 0U;
  uint32_t count = 0U;
  uint32_t buf   = 0U;
  ...
  for (uint32_t i = 0U; (i < count) && (buf != 0U); i++) {
    ...
    (void)emu_mem_read(uc,
                       ...
                       sec,
                       sizeof(sec));
    internal_vmsc_overlay_put(lba + i, sec);
  }
  eth_hook_return(uc, 0U); /* k_ra8_ok -- write accepted */
}

/**
 * @brief Install the virtual USB host-mode device seam if the host stack is
 * linked.
 *
 * @details Picks the virtual device class from the firmware's linked host
 * stack: an MSC host (links `ra8_usb_hmsc_read10`) gets a read-only FAT16 disk
 * seamed at the `ra8_usb_hmsc_*` class API; otherwise a USB-host-capable
 * firmware (links `ra8_usb_host_control_xfer`) gets a HID boot keyboard seamed
 * at the `ra8_usb_host_*` primitives. Device-mode apps link neither call path,
 * so the hooks are inert there and board_usb.c's device-mode virtual host is
 * untouched.
 *
...
{
  const uint32_t msc = elf_sym_addr(elf, "ra8_usb_hmsc_read10", nullptr);
  if (msc != 0U) {
    ...
    s_vmsc_writable = (elf_sym_addr(elf, "fileops_backend_write", nullptr) != 0U);
    eth_seam_hook(uc, elf, "ra8_usb_hmsc_init", (void*)internal_on_hmsc_ok);
    eth_seam_hook(uc, elf, "ra8_usb_hmsc_enumerate", (void*)internal_on_hmsc_enumerate);
    eth_seam_hook(uc, elf, "ra8_usb_hmsc_read_capacity", (void*)internal_on_hmsc_read_capacity);
    eth_seam_hook(uc, elf, "ra8_usb_hmsc_read10", (void*)internal_on_hmsc_read10);
    eth_seam_hook(uc, elf, "ra8_usb_hmsc_write10", (void*)internal_on_hmsc_write10);
    eth_seam_hook(uc, elf, "ra8_usb_hmsc_close", (void*)internal_on_hmsc_ok);
    (void)priv_emu_io_errf("  usb-host seam : hmsc=0x%08X (virtual MSC FAT16 "
                           "disk, file MRAM.BIN, %s)\n",
                           msc,
                           s_vmsc_writable ? "read-write" : "read-only");
    return true;
  }
  const uint32_t cx = elf_sym_addr(elf, "ra8_usb_host_control_xfer", nullptr);
  if (cx == 0U) {
    return false; /* not a USB-host-capable firmware -- nothing to seam. */
  }
  eth_seam_hook(uc, elf, "ra8_usb_host_line_state", (void*)internal_on_usbh_line_state);
  eth_seam_hook(uc, elf, "ra8_usb_host_control_xfer", (void*)internal_on_usbh_control_xfer);
  eth_seam_hook(uc, elf, "ra8_usb_host_bulk_in", (void*)internal_on_usbh_bulk_in);
  eth_seam_hook(uc, elf, "ra8_usb_host_bus_reset", (void*)internal_on_usbh_ok);
  eth_seam_hook(uc, elf, "ra8_usb_host_set_uact", (void*)internal_on_usbh_ok);
  eth_seam_hook(uc, elf, "ra8_usb_host_set_target", (void*)internal_on_usbh_ok);
  eth_seam_hook(uc, elf, "ra8_usb_host_pipe_setup", (void*)internal_on_usbh_ok);
  (void)priv_emu_io_errf("  usb-host seam : control=0x%08X (virtual HID boot "
                         "keyboard \"RA8D2\")\n",
                         cx);
  return true;
}
