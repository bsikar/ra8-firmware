 ...
 * exception unit, so ra8_emulator takes SysTick / PendSV / SVCall by hand: it
 ...
 * (0xFFFFFF80..0xFFFFFFFF). This covers both the Armv7-M values ra8_emulator
 ...
 *
 * @details
 * ra8_emulator is a single flat (no Secure/Non-Secure split) address space, so
...
RA8_INTERNAL static void internal_on_intr(uc_engine* uc, uint32_t int_no, void* user_data)
{
  ...
  /* MVE (Helium) contiguous load/store raises a NoCP UsageFault on Unicorn's
   * M33; ::emu_mve_nocp_emulate documents the decode and why it precedes SVCall. */
  if (emu_mve_nocp_emulate(uc, pc)) {
    (void)uc_emu_stop(uc);
    return;
  }
   ...
   * Secure state. ra8_emulator has a single flat address space (no S/NS split), so
 ...
 * next instruction. ra8_emulator runs the CPU in long chunks, so without help it
...
/** @brief Implementation of `emu_exc_arm_systick()` -- pend the periodic tick. */
void emu_exc_arm_systick(void)
{
  s_systick_pending = true;
}

/** @brief Implementation of `emu_exc_take_exc_return()` -- read + clear latch. */
...
/** @brief Implementation of `emu_exc_clear_pendsv_stop()` -- per-launch clear. */
void emu_exc_clear_pendsv_stop(void)
{
  s_pendsv_stop = false;
}

/** @brief Implementation of `emu_exc_pendsv_stop()` -- plain flag read. */
bool emu_exc_pendsv_stop(void)
{
  return s_pendsv_stop;
}

/** @brief Implementation of `emu_exc_bkpt_hit()` -- plain flag read. */
bool emu_exc_bkpt_hit(void)
{
  return s_bkpt_hit;
}

/** @brief Implementation of `emu_exc_bkpt_pc()` -- plain state read. */
uint32_t emu_exc_bkpt_pc(void)
{
  return s_bkpt_pc;
}

/** @brief Implementation of `emu_exc_systick_fires()` -- plain counter read. */
uint32_t emu_exc_systick_fires(void)
{
  return s_systick_fires;
}

/** @brief Implementation of `emu_exc_pendsv_takes()` -- plain counter read. */
uint32_t emu_exc_pendsv_takes(void)
{
  return s_pendsv_takes;
}

/** @brief Implementation of `emu_exc_svc_takes()` -- plain counter read. */
uint32_t emu_exc_svc_takes(void)
{
  return s_svc_takes;
}

/** @brief Implementation of `emu_exc_reset()` -- warm-reboot exception state. */
