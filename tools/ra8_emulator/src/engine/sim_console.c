/**
 * @file sim_console.c
 * @brief Emulator console surfaces implementation (see sim_console.h)
 *
 * @details
 ...
 * any ra8_log user) printed nothing in board_sim. We seed the ready bits into
...
/** @brief Implementation of `sim_console_install()` -- ITM seed + STIM0 echo hook. */
...
/** @brief Implementation of `sim_console_reset()` -- drop the pending ITM line. */
void sim_console_reset(void)
{
  s_itm_len = 0U;
}
