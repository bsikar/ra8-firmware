...
static void args_defaults(const char* elf_path, emu_args_t* out)
{
  *out             = (emu_args_t){};
  ...
  out->emu_device  = k_board_device_ra8d2;
}
...
static bool args_try_mode(int argc, char** argv, int* i, emu_args_t* out)
{
  ...
  if (strncmp(argv[idx], "--view", sizeof("--view")) == 0) {
    ...
    (void)board_modem_attach(); /* answer the ra8_modem_at AT path (SCI7 modem model) */
             ...
             ((idx + 1) < argc)) {
    emu_set_primary_core((strncmp(argv[idx + 1], "m33", sizeof("m33")) == 0) ? k_core_m33
                                                                             : k_core_m85);
    (*i)++;
  } else if ((strncmp(argv[idx], "--device", sizeof("--device")) == 0) && ((idx + 1) < argc)) {
    /* Select the emulated part; anything but "ra8p1" (incl. "ra8d2") = RA8D2. */
    out->emu_device = (strncmp(argv[idx + 1], "ra8p1", sizeof("ra8p1")) == 0)
...
static bool args_try_sym(int argc, char** argv, int* i, emu_args_t* out)
{
  ...
  if ((strncmp(argv[idx], "--dump-sym", sizeof("--dump-sym")) == 0) && ((idx + 1) < argc)) {
     ...
     * counter analog of RA8_EMU_STOP_ON, for the jlink_memprobe EIL mode: a
