 ...
 * outputs, holding the live window, saving the SD image, closing the raw
 * presentation surface + engine, and mapping the #67 exit code). Split out of
 * emu_run.c so each TU
 * stays under the file-size bar. Output text and order are exactly the
 * pre-split report's. The priv_run_report / priv_run_write_outputs / priv_run_hold_view /
 * priv_run_cleanup contracts live on their declarations in emu_run_internal.h.
 *
 ...
 * @see internal_run_print_stop_summary()  First consumer.
 * @see internal_run_print_verdict()  Second consumer.
 * @since 0.1.0
 */
typedef struct {
 ...
 * @post Zero or one summary line has been written to injected error sink.
 * @post The SD model is unchanged (read-only query).
 * @note Not thread-safe; part of the single-threaded report.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_run_print_sd_summary(void)
{
  bool        sd_att = false;
  uint64_t    sd_b   = 0U;
  uint8_t     sd_f   = 0U;
  ...
  if (sd_att && (sd_f != 0U)) {
    (void)priv_emu_io_errf("  SD card       : %lu %s FAT%u '%s' (created by --sd-new)\n",
                           sd_sz,
                           sd_u,
                           (unsigned)sd_f,
                           (sd_l != nullptr) ? sd_l : "");
  } else if (sd_att) {
    (void)priv_emu_io_errf("  SD card       : %lu %s image attached\n", sd_sz, sd_u);
  }
}
 ...
 * @pre injected error sink is the report stream.
 * @post The summary section has been written to injected error sink.
 * @post The engine state is unchanged (read-only queries).
 * @note Not thread-safe; part of the single-threaded report.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_run_print_stop_summary(uc_engine* uc, const run_stop_t* st)
{
  ...
  (void)priv_emu_io_errf("  final PC      : 0x%08X\n", st->run_pc);
  if (emu_exc_bkpt_hit()) {
    (void)priv_emu_io_errf("  => firmware executed a BKPT @ 0x%08X (deliberate trap: "
                           "Default_Handler / failed assert / fault give-up)\n",
                           emu_exc_bkpt_pc());
  }
  (void)priv_emu_io_errf("  chunks run    : %u   SysTick ticks: %u\n",
                         st->chunks,
                         emu_exc_systick_fires());
  if (emu_mve_emulated_count() > 0U) {
    (void)priv_emu_io_errf("  MVE (Helium)  : %llu instruction(s) emulated "
                           "(M85 vector ops the M33 "
                           "core lacks)\n",
                           (unsigned long long)emu_mve_emulated_count());
  }
  if (emu_lob_emulated_count() > 0U) {
    (void)priv_emu_io_errf("  LOB (loop)    : %llu DLS/LE instruction(s) "
                           "emulated (M85 hardware-loop "
                           "ops)\n",
                           (unsigned long long)emu_lob_emulated_count());
  }
  prof_report();
  (void)priv_emu_io_errf("  exceptions    : %u PendSV  %u SVCall (real Cortex-M entry/return)\n",
                         emu_exc_pendsv_takes(),
                         emu_exc_svc_takes());
  (void)priv_emu_io_errf("  touch clicks  : %u drained via ra8_touch -> I3C -> GT911\n",
 ...
 * @pre injected error sink is the report stream.
 * @post Zero or one verdict paragraph has been written to injected error sink.
 * @post No process state changes (pure reporting).
 * @note Not thread-safe; part of the single-threaded report.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_run_print_verdict(const run_stop_t* st)
{
  if (st->timed_out && !st->idle_stopped && !st->usb_stopped && !st->stop_sym_hit &&
     ...
     * full-budget run prints. Conflating the two let a load-correlated
     * truncation masquerade as success, dropping a deterministic banner without
     * failing the gate (#168). A caller that wants the run bounded by a
     * deterministic event (not by CPU-time) should pass RA8_EMU_STOP_ON /
     * RA8_EMU_MAX_CHUNKS. */
    (void)priv_emu_io_errf("  => ra8_emulator TRUNCATED by the wall-clock guard at chunk %u of %u "
             ...
             !emu_exc_bkpt_hit()) {
    if (st->idle_stopped) {
      (void)priv_emu_io_errf("  => firmware EXECUTED to the run budget (idle steady-state: no "
                             "observable change for %u chunks, stopped at chunk %u).\n",
                             st->idle_stop_chunks,
                             st->chunks);
    } else if (st->usb_stopped) {
      (void)priv_emu_io_errf("  => firmware EXECUTED to the run budget (USB enumerated: device "
                             "CONFIGURED, stopped at chunk %u).\n",
                             st->chunks);
    } else {
      (void)priv_emu_io_errf("  => firmware EXECUTED to the run budget (no "
 ...
 * @post One line per probe has been written to injected error sink.
 * @post The engine memory is unchanged (read-only probes).
 * @note Not thread-safe; part of the single-threaded report.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_run_print_dump_syms(uc_engine* uc, const emu_run_cfg_t* cfg)
{
  ...
  for (uint32_t d = 0U; d < cfg->dump_sym_n; d++) {
    if (cfg->dump_sym_addrs[d] == 0U) {
      (void)priv_emu_io_errf("  dump-sym      : %s = <unresolved>\n", cfg->dump_sym_names[d]);
      continue;
    }
    uint32_t v = 0U;
    if (emu_mem_read(uc, (uint64_t)cfg->dump_sym_addrs[d], &v, sizeof(v)) == UC_ERR_OK) {
      (void)priv_emu_io_errf("  dump-sym      : %s @0x%08X = %u (0x%08X)\n",
                             cfg->dump_sym_names[d],
                             cfg->dump_sym_addrs[d],
                             v,
                             v);
    } else {
      (void)priv_emu_io_errf("  dump-sym      : %s @0x%08X = <unreadable>\n",
                             cfg->dump_sym_names[d],
                             cfg->dump_sym_addrs[d]);
    }
  }
...
void priv_run_report(const run_loop_t* st)
{
  ...
  internal_run_print_stop_summary(st->cfg->uc, &stop);
  internal_run_print_verdict(&stop);
  internal_run_print_dump_syms(st->cfg->uc, st->cfg);
}
...
void priv_run_write_outputs(const run_loop_t* st)
{
  ...
  if ((cfg->ppm_path != nullptr) && (st->presentation->fd >= 0)) {
    ...
    if (built && (write_ppm(cfg->ppm_path, st->presentation) == 0)) {
      (void)priv_emu_io_errf("  wrote %s (%ux%u)\n",
                             cfg->ppm_path,
                             (unsigned)st->comp_w,
                             (unsigned)st->comp_h);
    } else {
      (void)priv_emu_io_errf("  could not write %s\n", cfg->ppm_path);
    }
  }
  if (cfg->record_dir != nullptr) {
    (void)priv_emu_io_errf("  recorded %u frame(s) to %s (%ux%u, ~%u fps)\n",
                           st->rec_frames,
                           cfg->record_dir,
                           (unsigned)st->comp_w,
                           (unsigned)st->comp_h,
...
void priv_run_hold_view(const run_loop_t* st)
{
  ...
  if (!st->closed) { /* run ended on its own -- keep the last frame up until
    ...
    (void)priv_emu_io_errf("ra8_emulator: run ended; close the window to exit\n");
...
int priv_run_cleanup(const run_loop_t* st)
{
  ...
  (void)priv_emu_elf_source_close(cfg->elf); /* warm reboot source lifetime ends here. */
  emu_cpu1_close(cfg->memory);
  (void)emu_memmap_detach(cfg->memory, cfg->uc);
