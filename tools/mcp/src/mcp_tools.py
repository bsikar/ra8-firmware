...
# MCP clients commonly start from a GUI with a reduced PATH. Enter Just through
# the repository resolver, and use the platform shell's absolute path so the
# resolver remains reachable even when PATH contains no command directories.
_JUST_LAUNCHER = ("/bin/bash", "-p", "scripts/dev/run_just.sh")


def _just_argv(*args: str) -> list[str]:
    """Return a Just command routed through the repository-owned resolver."""
    return [*_JUST_LAUNCHER, *args]
...
def tool_list_apps(args: dict[str, Any]) -> str:
    ...
    for app in apps:
        ...
        hay = f"{app_identifier} {app['group']} {app['description']}".lower()
        if needle and needle not in hay:
            continue
        rows.append(f"  {app['group']:<28} {app_identifier:<60} {app['description']}")
    header = f"{len(rows)} firmware app(s)" + (f" matching '{needle}'" if needle else "")
    body = "\n".join(rows) if rows else "  (none matched)"
    return (
        f"{header}\n"
        "  build: just apps::build <app> | flash: just apps::hardware::flash <app> | "
        "emulate: just apps::emulator::run <app>\n\n"
        f"{'  TIER/GROUP':<30} {'APP IDENTIFIER':<60} DESCRIPTION\n{body}\n"
    )
...
def tool_app_info(args: dict[str, Any]) -> str:
    ...
    out = [
        ...
        "",
        "build:    just apps::build " + app["id"],
        "flash:    just apps::hardware::flash " + app["id"] + "   (local J-Link)",
        "run on the emulator: just apps::emulator::run " + app["id"] + "     (tools/ra8_emulator)",
    ]
...
def tool_repo_overview(_args: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"APPS: {len(apps)} firmware apps under examples/ and apps/board/. "
        "Use list_apps / app_info.\n\n"
        "COMMON WORKFLOWS (also exposed as tools):\n"
        "  just apps::build <app>              cross-compile one app\n"
...
def tool_build_app(args: dict[str, Any]) -> str:
    """Cross-compile one firmware app via Just and return the log tail."""
    app = require_app(str(args.get("app", "")).strip())
    return run_command(_just_argv("apps::build", app["id"]), timeout=900)


def tool_run_tests(_args: dict[str, Any]) -> str:
    """Compile and run unit tests in the host's supported CI environment."""
    return run_command(_just_argv("quality::gate::run", "unit-tests"), timeout=900)
...
def tool_quality_gate(args: dict[str, Any]) -> str:
    ...
    registered_gate = _GATES.get(gate)
    if registered_gate is None:
        msg = f"unknown gate '{gate}'. Choose one of: {', '.join(sorted(_GATES))}"
        raise ValueError(msg)
    timeout = 900 if gate in ("tidy", "cppcheck", "mcdc") else 300
    return run_command(_just_argv("quality::gate::run", registered_gate), timeout=timeout)


def tool_coverage(args: dict[str, Any]) -> str:
    """Run the DO-178C Level B MC/DC coverage build and return its tail.
    ...
    return run_command(_just_argv("quality::gate::run", "mcdc"), timeout=1200)
...
def tool_emu_app(args: dict[str, Any]) -> str:
    ...
    """
    app = require_app(str(args.get("app", "")).strip())
    return run_command(["bash", "scripts/emu/smoke.sh", app["id"]], timeout=900)
...
def tool_hil(args: dict[str, Any]) -> str:
    ...
    ``recover`` / ``flash-retry`` / ``reflash`` require an ``app``; ``probe`` /
    ``erase`` / ``dlm-reset`` do not.
    """
    action = str(args.get("action", "")).strip()
    recipe = _HIL_ACTIONS.get(action)
    if recipe is None:
        msg = f"unknown action '{action}'. Choose: {', '.join(sorted(_HIL_ACTIONS))}"
        raise ValueError(msg)
    recipe_args = [recipe]
    if action in ("flash", "recover", "flash-retry", "reflash"):
        app = require_app(str(args.get("app", "")).strip())
        recipe_args.append(app["id"])
    if not bool(args.get("confirm", False)):
        return (
            "[dry run] HIL hardware action withheld.\n"
            f"Command: just {' '.join(recipe_args)}\n"
            "Re-call hil with confirm=true to execute."
        )
    return run_command(_just_argv(*recipe_args), timeout=600)
...
TOOLS: list[dict[str, Any]] = [
    ...
    {
        "name": "search_code",
        "description": "Search first-party source (apps, libs, examples, tests, "
        "port, scripts, tools, docs) for a regex pattern. Skips third_party and build.",
        "inputSchema": _schema(
            {
    ...
    {
        "name": "build_app",
        "description": "Cross-compile one firmware app (just apps::build <app>) and return the "
    ...
    },
    {
        "name": "run_tests",
        "description": (
            "Host-compile and run the unit-test suite (just quality::gate::run unit-tests)."
        ),
        "inputSchema": _schema({}),
    ...
    },
    {
        ...
        "flash-retry, reflash, erase, probe, dlm-reset. HARDWARE -- previews "
        "unless confirm=true. flash/recover/flash-retry/reflash need an app.",
        "inputSchema": _schema(
            {
    ...
    {
        "name": "coverage",
        "description": "Run the DO-178C Level B MC/DC coverage gate and return "
