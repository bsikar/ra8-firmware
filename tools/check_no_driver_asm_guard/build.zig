...
//!
//! Build graph for the `check_no_driver_asm_guard` host gate (#858). One
//! executable, plus the test step `scripts/checks/check_zig.py --test` runs:
//! the comment stripper, the conditional walk and the asm detector, and the
//! exit-status contract `scripts/builders/check_no_driver_asm_guard.sh`
//! passes through.
...
pub fn build(b: *std.Build) void {
    ...
    const executable = b.addExecutable(.{
        .name = "check_no_driver_asm_guard",
    ...
    const test_step = b.step("test", "Run check_no_driver_asm_guard tests");
