...
static void test_init_happy(void)
{
  ...
  TEST_ASSERT_EQ(0, (*ra_mram_reg16((uint16_t)k_ra_mram_off_mrcpc0) & 0x01U));
  TEST_ASSERT_EQ(0, (*ra_mram_reg16((uint16_t)k_ra_mram_off_mrcpc1) & 0x01U));
  /* MRPSC.MHSPEN clear. */
  TEST_ASSERT_EQ(0, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mrpsc)));
  /* prefetch_en=true means MRCPFB == 1. */
  TEST_ASSERT_EQ(1, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mrcpfb)));
  /* ECC enables programmed. */
  const uint16_t mrceecc = *ra_mram_reg16((uint16_t)k_ra_mram_off_mrceecc);
  TEST_ASSERT_EQ(1, (mrceecc & (uint16_t)k_ra_mrceecc_mask_eccen));
...
static void test_init_null_cfg(void)
{
  ...
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_init(nullptr));
...
static void test_init_bad_mrcfreq(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_init(&cfg));
...
static void test_init_all_optional_features_disabled(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_init(&cfg));
  TEST_ASSERT_EQ(0, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mrcpfb)));
  TEST_ASSERT_EQ(
    0,
    (*ra_mram_reg16((uint16_t)k_ra_mram_off_mrceecc) & (uint16_t)k_ra_mrceecc_mask_eccen));
  TEST_ASSERT_EQ(
    0,
    (*ra_mram_reg16((uint16_t)k_ra_mram_off_mrcdecc) & (uint16_t)k_ra_mrcdecc_mask_dececen));
...
static void test_deinit_locks_everything(void)
{
  ...
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mrcps) = (uint8_t)k_ra_mrcps_mask_errors;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_deinit());
...
static void test_get_status_paths(void)
{
  ...
  /* NULL out -> rejected. */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_get_status(nullptr));
  ...
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mrcps) = expected;
  ...
  uint8_t got = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_get_status(&got));
...
static void test_get_extended_status(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_get_extended_status(nullptr));
  ...
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr) = (uint32_t)k_ra_mstatr_mask_mrdy;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_get_extended_status(&s));
  TEST_ASSERT_EQ(0x21, s.mrcps);
  TEST_ASSERT_EQ(k_ra_mastat_mask_cmdlk, s.mastat);
  TEST_ASSERT_EQ(k_ra_mrezs_mask_whukzf, s.mrezs);
  TEST_ASSERT_EQ(k_ra_mcmdr_mask_cmd_progress, s.mcmdr);
  TEST_ASSERT_EQ(k_ra_mstatr_mask_mrdy, s.mstatr);
...
static void test_clear_status_paths(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_clear_status((uint8_t)k_ra_mrcps_mask_prgbsyc));

  /* Valid mask: PRGERRC + ECCERRC. */
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mrcps) = 0xFFU;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_clear_status((uint8_t)k_ra_mrcps_mask_errors));
  /* The driver writes the W1C mask; the sim mmap stores whatever we wrote. */
  TEST_ASSERT_EQ(k_ra_mrcps_mask_errors, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mrcps)));
...
static void test_set_rww_disable(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_rww_disable(true));
  TEST_ASSERT_EQ(0, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mrcpfb)));

  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_rww_disable(false));
  TEST_ASSERT_EQ(1, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mrcpfb)));
...
static void test_write_block_validation(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_flash_write_block((uint32_t)k_test_addr_in_mram, nullptr, 1U, k_ra_flash_world_ns));

  /* len = 0 */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_flash_write_block((uint32_t)k_test_addr_in_mram, buf, 0U, k_ra_flash_world_ns));

  /* len > 32 */
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_flash_write_block((uint32_t)k_test_addr_in_mram, buf, 33U, k_ra_flash_world_ns));

  /* address below window */
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_flash_write_block((uint32_t)k_test_addr_below_mram, buf, 4U, k_ra_flash_world_ns));

  /* address at-or-above end */
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_flash_write_block((uint32_t)k_test_addr_above_mram, buf, 4U, k_ra_flash_world_ns));

  /* spans page boundary: start at offset 30 of a page, write 4 bytes */
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_flash_write_block((uint32_t)k_ra_flash_code_start + 30U, buf, 4U, k_ra_flash_world_ns));
...
static void test_erase_block_alignment(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_flash_erase_block((uint32_t)k_test_addr_misaligned, k_ra_flash_world_ns));
...
static void test_write_block_simulator_reachable_paths(void)
{
  ...
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mrcps) = (uint8_t)k_ra_mrcps_mask_abufemp;
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_flash_write_block((uint32_t)k_test_addr_in_mram, buf, sizeof(buf), k_ra_flash_world_s));
  TEST_ASSERT_EQ(0x11, (*(volatile uint8_t*)(uintptr_t)k_test_addr_in_mram));
  TEST_ASSERT_EQ(k_ra_mrcpc1_key_disable, (*ra_mram_reg16((uint16_t)k_ra_mram_off_mrcpc1)));
  TEST_ASSERT_EQ(0, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mrpsc)));
  TEST_ASSERT_EQ(1, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mrcpfb)));
  ...
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mrcps) =
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_error,
                 ra_flash_write_block((uint32_t)k_test_addr_in_mram + 4U,
                                      buf,
                                      sizeof(buf),
                                      k_ra_flash_world_ns));
  TEST_ASSERT_EQ(k_ra_mrcpc0_key_disable, (*ra_mram_reg16((uint16_t)k_ra_mram_off_mrcpc0)));
...
static void test_block_protect_set(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_flash_block_protect_set(k_ra_flash_world_ns, false, true));

  /* NS lock writes the keyed value. */
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_block_protect_set(k_ra_flash_world_ns, true, false));
  TEST_ASSERT_EQ(k_ra_mrcbprot0_key_lock, (*ra_mram_reg16((uint16_t)k_ra_mram_off_mrcbprot0)));

  /* S unlock writes the unlock-keyed value. */
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_block_protect_set(k_ra_flash_world_s, false, false));
  TEST_ASSERT_EQ(k_ra_mrcbprot1_key_unlock, (*ra_mram_reg16((uint16_t)k_ra_mram_off_mrcbprot1)));

  /* Permanent + lock is accepted (warning logged). */
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_block_protect_set(k_ra_flash_world_s, true, true));
...
static void test_force_stop_happy(void)
{
  ...
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr) = (uint32_t)k_ra_mstatr_mask_mrdy;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_force_stop());
...
static void test_force_stop_cmdlk(void)
{
  ...
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr) = (uint32_t)k_ra_mstatr_mask_mrdy;
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mastat)  = (uint8_t)k_ra_mastat_mask_cmdlk;
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_flash_force_stop());
...
static void test_force_stop_timeout(void)
{
  ...
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr) = 0U;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_flash_force_stop());
...
static void test_reset_happy(void)
{
  ...
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr)  = (uint32_t)k_ra_mstatr_mask_mrdy;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_reset());
...
static void test_reset_not_initialized_and_stop_error(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_flash_reset());

  const ra_flash_cfg_t cfg = make_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_init(&cfg));
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr) = (uint32_t)k_ra_mstatr_mask_mrdy;
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mastat)  = (uint8_t)k_ra_mastat_mask_cmdlk;
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_flash_reset());
...
static void test_set_startup_area_temporary(void)
{
  ...
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr)  = (uint32_t)k_ra_mstatr_mask_mrdy;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_startup_area(k_ra_flash_startup_alternate, true));
  /* MSUACR holds key + alternate bit. */
  const uint16_t want =
    (uint16_t)((uint16_t)k_ra_msuacr_key | (uint16_t)k_ra_msuacr_swap_alternate);
  TEST_ASSERT_EQ(want, (*ra_mram_reg16((uint16_t)k_ra_mram_off_msuacr)));

  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_set_startup_area((ra_flash_startup_t)99U, true));
...
static void test_set_startup_area_default_and_permanent(void)
{
  ...
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr) = (uint32_t)k_ra_mstatr_mask_mrdy;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_startup_area(k_ra_flash_startup_default, true));
  TEST_ASSERT_EQ((k_ra_msuacr_key | k_ra_msuacr_swap_default),
                 (*ra_mram_reg16((uint16_t)k_ra_mram_off_msuacr)));

  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr) = (uint32_t)k_ra_mstatr_mask_mrdy;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_startup_area(k_ra_flash_startup_default, false));
  TEST_ASSERT_EQ(k_ra_msaddr_config_set_startup, (*ra_mram_reg32((uint16_t)k_ra_mram_off_msaddr)));
...
static void test_get_startup_area(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_get_startup_area(nullptr, nullptr));

  *ra_mram_reg32((uint16_t)k_ra_mram_off_msuasmon) =
    (uint32_t)((uint32_t)k_ra_msuasmon_mask_btflg | (uint32_t)k_ra_msuasmon_mask_fspr);
  uint8_t btflg = 0U;
  uint8_t fspr  = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_get_startup_area(&btflg, &fspr));
  TEST_ASSERT_EQ(1, btflg);
  TEST_ASSERT_EQ(1, fspr);
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_get_startup_area(&btflg, nullptr));
...
static void test_config_set_write_validation(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_config_set_write(0x02C9F040UL, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_config_set_write(0x00000000UL, buf));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_config_set_write(0xFFFFFFFFUL, buf));

  /* Happy path: pre-stage MSTATR so the wait succeeds and no errors. */
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr) = (uint32_t)k_ra_mstatr_mask_mrdy;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_config_set_write((uint32_t)k_test_addr_extra_in, buf));
...
static void test_config_set_write_ofs_window(void)
{
  ...
  uint16_t buf[k_ra_mram_config_set_word_count]  = {};
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr) = (uint32_t)k_ra_mstatr_mask_mrdy;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_config_set_write((uint32_t)k_ra_flash_ofs_start, buf));
  TEST_ASSERT_EQ(k_ra_flash_ofs_start, (*ra_mram_reg32((uint16_t)k_ra_mram_off_msaddr)));
...
static void test_mcdc_config_set_write_extra_window(void)
{
  ...
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr) = (uint32_t)k_ra_mstatr_mask_mrdy;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_config_set_write((uint32_t)k_ra_flash_extra_start, buf));
...
static void test_extra_mram_write_validation(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_flash_extra_mram_write((uint32_t)k_test_addr_extra_in, nullptr, 4U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_flash_extra_mram_write((uint32_t)k_test_addr_extra_in, buf, 0U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_flash_extra_mram_write((uint32_t)k_test_addr_extra_in, buf, 33U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_extra_mram_write(0x00000000UL, buf, 4U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_flash_extra_mram_write((uint32_t)k_test_addr_extra_bad, buf, 4U));
  /* page-spanning */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_flash_extra_mram_write((uint32_t)k_ra_flash_extra_start + 30U, buf, 4U));
...
static void test_extra_mram_erase_validation(void)
{
  ...
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_extra_mram_erase(0x02C9F005UL));
...
static void test_arc_argument_validation(void)
{
  ...
  uint32_t out = 0U;
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_arc_read(k_ra_flash_arc_sec, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_arc_read((ra_flash_arc_id_t)99U, &out));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_arc_increment((ra_flash_arc_id_t)99U));

  /* Without init these should report not_initialized. */
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_flash_arc_read(k_ra_flash_arc_sec, &out));
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_flash_arc_increment(k_ra_flash_arc_sec));
...
static void test_arc_oembl_read_increment_paths(void)
{
  ...
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mcntdtr1) = 0x80000000UL;
  uint32_t count                                   = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_arc_read(k_ra_flash_arc_oembl, &count));
  TEST_ASSERT_EQ(3, count);
  TEST_ASSERT_EQ(k_ra_mcntselr_oembl,
                 (*ra_mram_reg8((uint16_t)k_ra_mram_off_mcntselr) & k_ra_mcntselr_mask));

  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr)   = (uint32_t)k_ra_mstatr_mask_mrdy;
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mcntdtr0) = 0x00000001UL;
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mcntdtr1) = 0x00000000UL;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_arc_increment(k_ra_flash_arc_oembl));

  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr)   = (uint32_t)k_ra_mstatr_mask_mrdy;
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mcntdtr0) = 0xFFFFFFFFUL;
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mcntdtr1) = 0xFFFFFFFFUL;
  TEST_ASSERT_EQ(k_ra_err_out_of_range, ra_flash_arc_increment(k_ra_flash_arc_oembl));

  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr) = 0U;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_flash_arc_read(k_ra_flash_arc_oembl, &count));
...
static void test_zeroize_huk_paths(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_flash_zeroize_huk());

  const ra_flash_cfg_t cfg = make_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_init(&cfg));

  /* Sim resets MREZS to 0, so the WHUKEXE check passes immediately. */
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_zeroize_huk());
  TEST_ASSERT_EQ(k_ra_mrezc_full_zero, (*ra_mram_reg16((uint16_t)k_ra_mram_off_mrezc)));
...
static void test_zeroize_huk_timeout(void)
{
  ...
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mrezs) = (uint8_t)k_ra_mrezs_mask_whukexe;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_flash_zeroize_huk());
...
static void test_set_security_attribution(void)
{
  ...
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_security_attribution(0x55AAU));
  TEST_ASSERT_EQ(0x55AAU, (*ra_mram_reg16((uint16_t)k_ra_mram_off_msar)));
...
static void test_set_ecc_encoder_decoder_enable(void)
{
  ...
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_ecc_encoder_enable(true));
  TEST_ASSERT_EQ(
    1,
    (*ra_mram_reg16((uint16_t)k_ra_mram_off_mrceecc) & (uint16_t)k_ra_mrceecc_mask_eccen));

  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_ecc_decoder_enable(false));
  TEST_ASSERT_EQ(
    0,
    (*ra_mram_reg16((uint16_t)k_ra_mram_off_mrcdecc) & (uint16_t)k_ra_mrcdecc_mask_dececen));
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_ecc_encoder_enable(false));
  TEST_ASSERT_EQ(
    0,
    (*ra_mram_reg16((uint16_t)k_ra_mram_off_mrceecc) & (uint16_t)k_ra_mrceecc_mask_eccen));
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_ecc_decoder_enable(true));
  TEST_ASSERT_EQ(
    k_ra_mrcdecc_mask_dececen,
    (*ra_mram_reg16((uint16_t)k_ra_mram_off_mrcdecc) & (uint16_t)k_ra_mrcdecc_mask_dececen));
...
static void test_get_ecc_error_addr(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_flash_get_ecc_error_addr(nullptr, nullptr, nullptr, nullptr));
  ...
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mrerdea) = 0x44444444UL;
  ...
  uint32_t c                                      = 0;
  uint32_t d                                      = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_get_ecc_error_addr(&a, &b, &c, &d));
  TEST_ASSERT_EQ(0x11111111, a);
  TEST_ASSERT_EQ(0x22222222, b);
  TEST_ASSERT_EQ(0x33333333, c);
  TEST_ASSERT_EQ(0x44444444, d);
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_get_ecc_error_addr(&a, nullptr, &c, &d));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_get_ecc_error_addr(&a, &b, nullptr, &d));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_get_ecc_error_addr(&a, &b, &c, nullptr));
...
static void test_get_program_error_addr(void)
{
  ...
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_get_program_error_addr(nullptr));
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mrcpea) = 0xCAFEBABEUL;
  uint32_t addr                                  = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_get_program_error_addr(&addr));
...
static void test_update_clock_freq(void)
{
  ...
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_update_clock_freq(0xFFFFU, 100U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_update_clock_freq(100U, 0xFFU));
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_update_clock_freq(150U, 75U));
  /* Verify the keyed value landed. */
  const uint32_t want_mrcfreq = ((uint32_t)0x1EU << 24) | 150U;
  TEST_ASSERT_EQ(want_mrcfreq, (*ra_mram_reg32((uint16_t)k_ra_mram_off_mrcfreq)));
...
static void test_set_update_transfer(void)
{
  ...
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_set_update_transfer(99U));
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_update_transfer(0x05U));
  TEST_ASSERT_EQ(0x05, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mctrlsr)));
...
static void test_get_update_status(void)
{
  ...
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_get_update_status(nullptr, nullptr, nullptr));
  ...
  *ra_mram_reg16((uint16_t)k_ra_mram_off_mctrstatr) =
  ...
  uint8_t d = 0U;
  uint8_t e = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_get_update_status(&b, &d, &e));
  TEST_ASSERT_EQ(1, b);
  TEST_ASSERT_EQ(1, d);
  TEST_ASSERT_EQ(1, e);
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_get_update_status(&b, nullptr, &e));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_get_update_status(&b, &d, nullptr));
...
static void test_set_irq_enable(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_set_irq_enable((ra_flash_irq_src_t)99U, true));

  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_irq_enable(k_ra_flash_irq_code_ecc_ted, true));
  TEST_ASSERT_EQ(k_ra_mrcraeint_mask_intenbtc, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mrcraeint)));
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_irq_enable(k_ra_flash_irq_code_ecc_dec, true));
  TEST_ASSERT_EQ((k_ra_mrcraeint_mask_intenbtc | k_ra_mrcraeint_mask_intenbdc),
                 (*ra_mram_reg8((uint16_t)k_ra_mram_off_mrcraeint)));
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_irq_enable(k_ra_flash_irq_code_ecc_ted, false));
  TEST_ASSERT_EQ(k_ra_mrcraeint_mask_intenbdc, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mrcraeint)));

  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_irq_enable(k_ra_flash_irq_extra_ecc_dec, true));
  TEST_ASSERT_EQ(k_ra_mrcraeint_mask_intenbdc, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mreraint)));
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_irq_enable(k_ra_flash_irq_extra_ecc_ted, true));
  TEST_ASSERT_EQ((k_ra_mrcraeint_mask_intenbdc | k_ra_mrcraeint_mask_intenbtc),
                 (*ra_mram_reg8((uint16_t)k_ra_mram_off_mreraint)));

  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_irq_enable(k_ra_flash_irq_program_err, true));
  TEST_ASSERT_EQ(k_ra_mrcpaeint_mask_mrcaeie, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mrcpaeint)));

  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_irq_enable(k_ra_flash_irq_extra_err, true));
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_irq_enable(k_ra_flash_irq_extra_cmdlk, true));
  TEST_ASSERT_EQ((k_ra_mpaeint_mask_mreaeie | k_ra_mpaeint_mask_cmdlkie),
                 (*ra_mram_reg8((uint16_t)k_ra_mram_off_mpaeint)));
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_irq_enable(k_ra_flash_irq_extra_err, false));
  TEST_ASSERT_EQ(k_ra_mpaeint_mask_cmdlkie, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mpaeint)));

  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_irq_enable(k_ra_flash_irq_extra_ready, true));
  TEST_ASSERT_EQ(k_ra_mrdyie_mask_mrdyie, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mrdyie)));

  /* Disable round trip. */
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_irq_enable(k_ra_flash_irq_program_err, false));
  TEST_ASSERT_EQ(0, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mrcpaeint)));
...
static void test_callback_set_and_dispatch(void)
{
  ...
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mrcraes) = (uint8_t)k_ra_mrcraes_mask_any;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_callback_set(nullptr, nullptr));
  (void)ra_flash_dispatch_isr();
  TEST_ASSERT_EQ(0, s_cb_invocations);
  /* W1C cleared the status. */
  TEST_ASSERT_EQ(0, (*ra_mram_reg8((uint16_t)k_ra_mram_off_mrcraes)));

  /* Register a callback and stage three events. */
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_callback_set(test_callback, (void*)0xDEADBEEFUL));
  ...
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr)  = (uint32_t)k_ra_mstatr_mask_mrdy;
  ...
  TEST_ASSERT_EQ((k_ra_flash_irq_extra_ready), s_cb_last_src);
...
static void test_callback_set_idempotent(void)
{
  ...
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_callback_set(test_callback, nullptr));
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_callback_set(nullptr, nullptr));
  /* No events staged; dispatch returns 0. */
  TEST_ASSERT_EQ(0, ra_flash_dispatch_isr());
...
static void test_open_close_aliases(void)
{
  ...
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_open(nullptr));
  const ra_flash_cfg_t cfg = make_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_open(&cfg));
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_close());
...
static void test_set_window_paths(void)
{
  ...
  ra_sim_mmap_reset();
  /* 0/0 disables. */
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_window(0U, 0U));
  /* low >= high is rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_set_window(0x100U, 0x100U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_set_window(0x200U, 0x100U));
  /* Valid window accepted. */
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_flash_set_window((uintptr_t)k_ra_flash_code_start,
...
static void test_erase_validation(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_flash_erase((uintptr_t)k_test_addr_in_mram, 1U));
  const ra_flash_cfg_t cfg = make_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_init(&cfg));
  /* Zero blocks rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_erase((uintptr_t)k_test_addr_in_mram, 0U));
  /* Misaligned address rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_erase((uintptr_t)k_test_addr_misaligned, 1U));
  /* Below window rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_erase((uintptr_t)k_test_addr_below_mram, 1U));
  /* Range exceeding window rejected. */
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_flash_erase((uintptr_t)k_ra_flash_code_start + (uintptr_t)k_ra_flash_code_size, 1U));
  /* Window-blocked rejected. */
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_flash_set_window((uintptr_t)k_ra_flash_code_start,
                                     (uintptr_t)k_ra_flash_code_start + 0x40U));
  TEST_ASSERT_EQ(k_ra_err_out_of_range,
                 ra_flash_erase((uintptr_t)k_ra_flash_code_start + 0x40U, 1U));
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_window(0U, 0U));
...
static void test_write_validation_and_chunking(void)
{
  ...
  /* NULL src */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_write((uintptr_t)k_test_addr_in_mram, nullptr, 32U));
  /* len = 0 */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_write((uintptr_t)k_test_addr_in_mram, buf, 0U));
  /* len not multiple of 32 */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_write((uintptr_t)k_test_addr_in_mram, buf, 33U));
  /* misaligned address */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_write((uintptr_t)k_test_addr_misaligned, buf, 32U));
  /* out of window */
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_flash_write((uintptr_t)k_ra_flash_code_start + (uintptr_t)k_ra_flash_code_size, buf, 32U));
  /* Soft window block */
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_flash_set_window((uintptr_t)k_ra_flash_code_start,
                                     (uintptr_t)k_ra_flash_code_start + 0x20U));
  TEST_ASSERT_EQ(k_ra_err_out_of_range,
                 ra_flash_write((uintptr_t)k_ra_flash_code_start + 0x40U, buf, 32U));
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_window(0U, 0U));
...
static void test_blank_check_paths(void)
{
  ...
  /* NULL out rejected. */
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_flash_blank_check((uintptr_t)k_ra_flash_code_start, 1U, nullptr));
  /* len = 0 rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_flash_blank_check((uintptr_t)k_ra_flash_code_start, 0U, &blank));
  /* Outside both windows rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_blank_check((uintptr_t)0x10000000UL, 4U, &blank));
  ...
  blank = false;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_blank_check((uintptr_t)k_test_addr_extra_in, 16U, &blank));
...
static void test_status_decoder(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_flash_status(nullptr));
  ...
  *ra_mram_reg16((uint16_t)k_ra_mram_off_mrcbprot1) = (uint16_t)k_ra_mrcbprot1_key_unlock;
  ...
  ra_flash_status_t s = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_status(&s));
...
static void test_suspend_resume_round_trip(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_suspend());
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_resume());
  const uint16_t after_resume = *ra_mram_reg16((uint16_t)k_ra_mram_off_mentryr);
  TEST_ASSERT_EQ(0, (after_resume & (uint16_t)k_ra_mentryr_mask_pcka));
...
static void test_lock_set_ns_world(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_lock_set(ns_addr, (uint16_t)k_ra_mrcbprot0_key_lock));
  TEST_ASSERT_EQ(k_ra_mrcbprot0_key_lock, *ra_mram_reg16((uint16_t)k_ra_mram_off_mrcbprot0));
...
static void test_lock_set_s_world(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_lock_set(s_addr, (uint16_t)k_ra_mrcbprot1_key_lock));
  TEST_ASSERT_EQ(k_ra_mrcbprot1_key_lock, *ra_mram_reg16((uint16_t)k_ra_mram_off_mrcbprot1));
...
static void test_lock_set_validation(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_flash_lock_set(0x01FFFFFCU, (uint16_t)k_ra_mrcbprot0_key_lock));
  /* Address above code-MRAM rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_flash_lock_set(0x02100000U, (uint16_t)k_ra_mrcbprot0_key_lock));
  /* Bogus key bytes rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_flash_lock_set((uintptr_t)k_ra_flash_code_start, 0x1234U));
...
static void test_block_protect_set_mcdc(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_block_protect_set(k_ra_flash_world_ns, false, false));
  /* Vector 2: permanent=T, lock=T. C1=T, C2=F -> Decision F -> ok. */
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_block_protect_set(k_ra_flash_world_ns, true, true));
...
static void test_set_window_mcdc(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_window(1U, 2U));
  /* Vector 2: low=0, high=1. C1=T, C2=F -> Decision F; then 0<1 -> ok. */
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_set_window(0U, 1U));
...
static void test_write_validation_mcdc(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_write((uintptr_t)k_ra_flash_code_start, buf, 0U));
  /* Vector 3: len=33 (not page-aligned). C1=F, C2=T. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_write((uintptr_t)k_ra_flash_code_start, buf, 33U));
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_write((uintptr_t)0xDEADBEEFU, buf, 32U));
...
static void test_mcdc_blank_check_region_or3(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_blank_check((uintptr_t)k_ra_flash_code_start, 4U, &blank));
  /* V4: addr in none -> region check fails (dec T -> invalid_arg). */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_blank_check((uintptr_t)0x05000000UL, 4U, &blank));
