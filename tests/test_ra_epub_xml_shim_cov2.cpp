/**
 * @file test_ra_epub_xml_shim_cov2.cpp
 * @brief Complementary line-coverage tests for libs/ra_epub/src/ra_epub_xml_shim.cpp.
 *
 * @par Tag
 ...
 * C-linkage entry points. It is a companion to `test_ra_epub_xml_shim_cov.cpp`;
 ...
 * genuine production symbols out of `ra_core_hal` (the same object the firmware
 ...
 * `ra_epub_open.c` (the shim entry points are not exposed in a public header).
 *
 ...
 *          `ra_epub_open.c`; used only to receive the parsed OPF path.
 */
typedef struct {
  char opf_path[k_ra_epub_max_path_len]; /**< Parsed rootfile full-path. */
 ...
 * found.
 *  - V1: full_path="OEBPS/c.opf" -> C1=F,C2=F. Decision F (copy + k_ra_ok;
...
void test_cov2_container_error_paths(void)
{
  ...
  assert(ra_epub_xml_parse_container(nullptr, 1U, &res) == k_ra_err_null_ptr);
  assert(ra_epub_xml_parse_container(bytes_of("x"), 1U, nullptr) == k_ra_err_null_ptr);

  /* Zero length -> invalid_size. */
  assert(ra_epub_xml_parse_container(bytes_of("x"), 0U, &res) == k_ra_err_invalid_size);

  /* Comment-only document -> parses, but exposes no root element. */
  constexpr const char* k_comment = "<!-- only a comment -->";
  assert(ra_epub_xml_parse_container(bytes_of(k_comment), std::strlen(k_comment), &res) ==
  ...
  assert(ra_epub_xml_parse_container(bytes_of(k_no_rootfile), std::strlen(k_no_rootfile), &res) ==
         k_ra_err_validation_failed);
  ...
  assert(ra_epub_xml_parse_container(bytes_of(k_no_attr), std::strlen(k_no_attr), &res) ==
         k_ra_err_validation_failed);
  ...
  assert(ra_epub_xml_parse_container(bytes_of(k_empty_attr), std::strlen(k_empty_attr), &res) ==
         k_ra_err_validation_failed);
...
void test_cov2_container_success(void)
{
  ...
  assert(ra_epub_xml_parse_container(bytes_of(k_nested), std::strlen(k_nested), &res) == k_ra_ok);
  ...
  assert(ra_epub_xml_parse_container(bytes_of(k_direct), std::strlen(k_direct), &res) == k_ra_ok);
 ...
 *  - chapter cap `if (count >= k_ra_epub_max_chapters) return k_ra_err_no_mem;`
...
void test_cov2_opf_edges(void)
{
  ...
  /* Zero length -> invalid_size. */
  assert(ra_epub_xml_parse_opf(bytes_of("x"), 0U, &book) == k_ra_err_invalid_size);

  /* Comment-only document -> no package root element. */
  constexpr const char* k_comment = "<!-- only a comment -->";
  assert(ra_epub_xml_parse_opf(bytes_of(k_comment), std::strlen(k_comment), &book) ==
         k_ra_err_validation_failed);

  /* Legacy cover meta with an unresolved id + spine skip legs. */
  std::memset(&book, 0, sizeof(book));
  assert(ra_epub_xml_parse_opf(bytes_of(k_opf_cover_meta_spine_edges),
  ...
  /* Chapter cap: a spine of (cap + 1) resolving itemrefs -> no_mem. */
  {
    ...
    xml += "</spine></package>";
    std::memset(&book, 0, sizeof(book));
    assert(ra_epub_xml_parse_opf(bytes_of(xml.c_str()), xml.size(), &book) == k_ra_err_no_mem);
  }
 ...
 *
 * @par MC/DC:
 * Sequential single-condition legs of `ra_epub_xml_parse_ncx`:
...
void test_cov2_ncx_edges(void)
{
  ...
  assert(ra_epub_xml_parse_ncx(bytes_of(k_bad), std::strlen(k_bad), &book) ==
         k_ra_err_validation_failed);

  constexpr const char* k_comment = "<!-- only a comment -->";
  assert(ra_epub_xml_parse_ncx(bytes_of(k_comment), std::strlen(k_comment), &book) ==
         k_ra_err_validation_failed);
 ...
 *
 * @par MC/DC:
 * Sequential single-condition legs of `ra_epub_xml_parse_nav`:
 *  - `if (xml_len == 0U)` -> invalid_size.
...
void test_cov2_nav_edges(void)
{
  ...
  /* Zero length -> invalid_size. */
  assert(ra_epub_xml_parse_nav(bytes_of("x"), 0U, &book) == k_ra_err_invalid_size);

  /* Malformed XML -> parse failure. */
  constexpr const char* k_bad = "<a><b></c>";
  assert(ra_epub_xml_parse_nav(bytes_of(k_bad), std::strlen(k_bad), &book) ==
         k_ra_err_validation_failed);

  /* Comment-only -> no root element. */
  constexpr const char* k_comment = "<!-- only a comment -->";
  assert(ra_epub_xml_parse_nav(bytes_of(k_comment), std::strlen(k_comment), &book) ==
         k_ra_err_validation_failed);

  /* toc nav present but with no <ol> -> validation_failed. */
  std::memset(&book, 0, sizeof(book));
  assert(ra_epub_xml_parse_nav(bytes_of(k_nav_no_ol), std::strlen(k_nav_no_ol), &book) ==
         k_ra_err_validation_failed);

  /* Stray non-<li> child in the <ol> is skipped; one real entry remains. */
  std::memset(&book, 0, sizeof(book));
  assert(ra_epub_xml_parse_nav(bytes_of(k_nav_stray_then_li),
...
int main(void)
{
  ...
  test_cov2_ncx_edges();
  test_cov2_nav_edges();
  std::fprintf(stderr, "[OK ] test_ra_epub_xml_shim_cov2.cpp\n");
  return 0;
}
