/**
 * @file test_ra_dfu_boot.c
 * @brief Host unit tests for the pure boot logic of the ra_dfu core.
 *
 * @details
...
/**
 * @brief `ra_dfu_hdr_valid` magic/length/CRC gate, with MC/DC vectors.
 *
 * @par MC/DC:
...
{
  TEST_BEGIN("ra_dfu: hdr_valid magic/len/crc (MC/DC)");

  /* NULL guard. */
  TEST_ASSERT(!ra_dfu_hdr_valid(nullptr, (uint32_t)k_test_crc_good));

  /* Outer AND: magic_ok && len_ok && crc_ok. */
  const ra_dfu_img_hdr_t v1 =
    mk_hdr((uint32_t)k_ra_dfu_hdr_magic, 1U, (uint32_t)k_test_len_ok, (uint32_t)k_test_crc_good);
  TEST_ASSERT(ra_dfu_hdr_valid(&v1, (uint32_t)k_test_crc_good)); /* T T T */

  const ra_dfu_img_hdr_t v2 = mk_hdr((uint32_t)k_ra_dfu_hdr_magic + 1U,
                                     1U,
                                     (uint32_t)k_test_len_ok,
                                     (uint32_t)k_test_crc_good);
  TEST_ASSERT(!ra_dfu_hdr_valid(&v2, (uint32_t)k_test_crc_good)); /* F T T */

  /* T F T -> false: len unaligned (also covers inner L4). */
  const ra_dfu_img_hdr_t v3 = mk_hdr((uint32_t)k_ra_dfu_hdr_magic,
                                     1U,
                                     (uint32_t)k_test_len_unalign,
                                     (uint32_t)k_test_crc_good);
  TEST_ASSERT(!ra_dfu_hdr_valid(&v3, (uint32_t)k_test_crc_good)); /* T F T */

  /* T T F -> false: computed CRC differs from stored. */
  TEST_ASSERT(!ra_dfu_hdr_valid(&v1, (uint32_t)k_test_crc_bad)); /* T T F */

  /* Inner len_ok vectors (magic_ok=T, crc_ok=T held). */
  const ra_dfu_img_hdr_t l2 =
    mk_hdr((uint32_t)k_ra_dfu_hdr_magic, 1U, 0U, (uint32_t)k_test_crc_good);
  TEST_ASSERT(!ra_dfu_hdr_valid(&l2, (uint32_t)k_test_crc_good)); /* len == 0 */

  const ra_dfu_img_hdr_t l3 = mk_hdr((uint32_t)k_ra_dfu_hdr_magic,
                                     1U,
                                     (uint32_t)k_test_len_toobig,
                                     (uint32_t)k_test_crc_good);
  TEST_ASSERT(!ra_dfu_hdr_valid(&l3, (uint32_t)k_test_crc_good)); /* len > max */

  /* Boundary: exactly img_max is valid (page-aligned). */
  const ra_dfu_img_hdr_t lmax =
    mk_hdr((uint32_t)k_ra_dfu_hdr_magic, 1U, (uint32_t)k_ra_dfu_img_max, (uint32_t)k_test_crc_good);
  TEST_ASSERT(ra_dfu_hdr_valid(&lmax, (uint32_t)k_test_crc_good));

  TEST_END("ra_dfu: hdr_valid magic/len/crc (MC/DC)");
}
...
{
  TEST_BEGIN("ra_dfu: select_slot A/B/none (MC/DC)");

  /* none-decision MC/DC. */
  TEST_ASSERT_EQ(k_ra_dfu_slot_none, ra_dfu_select_slot(false, 0U, false, 0U));
  TEST_ASSERT_EQ(k_ra_dfu_slot_a, ra_dfu_select_slot(true, 5U, false, 0U));
...
/**
 * @brief `ra_dfu_boot_decide` trigger + slot mapping, with MC/DC vectors.
 *
 * @par MC/DC:
...
{
  TEST_BEGIN("ra_dfu: boot_decide trigger + mapping (MC/DC)");

  /* Trigger wins even with a valid slot. */
  TEST_ASSERT_EQ(k_ra_dfu_action_dfu, ra_dfu_boot_decide(true, true, 5U, false, 0U));

  /* No trigger: map select_slot. */
  TEST_ASSERT_EQ(k_ra_dfu_action_jump_a, ra_dfu_boot_decide(false, true, 5U, false, 0U));
  TEST_ASSERT_EQ(k_ra_dfu_action_jump_b, ra_dfu_boot_decide(false, false, 0U, true, 5U));
  TEST_ASSERT_EQ(k_ra_dfu_action_jump_b, ra_dfu_boot_decide(false, true, 3U, true, 9U));
  TEST_ASSERT_EQ(k_ra_dfu_action_jump_a, ra_dfu_boot_decide(false, true, 9U, true, 3U));

  /* No trigger, no valid slot -> DFU. */
  TEST_ASSERT_EQ(k_ra_dfu_action_dfu, ra_dfu_boot_decide(false, false, 0U, false, 0U));

  TEST_END("ra_dfu: boot_decide trigger + mapping (MC/DC)");
}
