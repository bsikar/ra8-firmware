/**
 * @file test_ra_fs_fat_lfn_cov.c
 * @brief Coverage booster for libs/ra_fs/src/ra_fs_fat_lfn.c.
 *
 * @details
 * Dedicated companion test executable that drives the branches in
 * ra_fs_fat_lfn.c not yet exercised by test_ra_fs_lfn.c:
 *
 ...
 *
 * Line 129 of ra_fs_fat_lfn.c (defensive buffer-overflow guard in
 ...
 *          freed), then guard=5 > 4 triggers k_ra_err_protocol_error.
 */
typedef enum : uint32_t {
...
static void build_fat16_vol(void)
{
  ...
  put16(bpb, 11U, (uint16_t)k_lcov_blk_sz);
  bpb[13] = 1U;
  put16(bpb, 14U, (uint16_t)k_lcov_fat1_lba);
  bpb[16] = (uint8_t)k_lcov_num_fats;
  put16(bpb, 17U, 16U);
  put16(bpb, 19U, (uint16_t)k_lcov_blocks);
  put16(bpb, 22U, (uint16_t)k_lcov_fat_sz);
  bpb[510] = 0x55U;
  bpb[511] = 0xAAU;
}
...
static void stamp_lfn_entry(uint8_t* slot, uint8_t csum)
{
  ...
  slot[11] = (uint8_t)k_lfn_attr_magic;
  slot[12] = 0x00U;
  slot[13] = csum;
  put16(slot, 26U, 0U);
 ...
 * @since 0.1.0
 */
static void create_files_in(ra_fs_mount_t* h, const char* dir_path, uint32_t count)
{
 ...
 *          is mounted. ra_fs_open("mybook.epub") calls priv_dir_find_long,
 ...
 * test_ra_fs_lfn.c provides the "false" vector (correct checksum -> line 149).
 */
static void test_lfn_cov_checksum_mismatch(void)
{
  TEST_BEGIN("ra_fs LFN cov: checksum mismatch -> line 147");
   ...
   * the 8.3 entry is not an LFN match -> k_ra_err_not_found. */
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_fs_open(h, "mybook.epub", k_ra_fs_mode_read, &f));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_vol();
  TEST_END("ra_fs LFN cov: checksum mismatch -> line 147");
}
 ...
 *          and the search succeeds, returning k_ra_ok.
 *
 * @par MC/DC:
 * Decision at line 274: `needle[0] == '/'` (single condition).
 * - This test: needle[0]='/' -> true -> line 276 (needle++) executed.
 * - Normal path via ra_fs_open: priv_resolve_parent strips the path separator
...
{
  TEST_BEGIN("ra_fs LFN cov: leading-slash strip -> lines 275-276");
  build_vol_lfn_good();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  uint8_t   ent[k_ra_fs_dir_entry_bytes] = {};
  /* want="/mybook.epub": needle[0]=='/' -> needle++ (lines 275-276),
   * then the scan finds the LFN+8.3 entry -> k_ra_ok. */
  TEST_ASSERT_EQ(k_ra_ok, priv_dir_find_long(h, &root_loc, "/mybook.epub", &lba, &off, ent));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_vol();
  TEST_END("ra_fs LFN cov: leading-slash strip -> lines 275-276");
}
 ...
 *          entries is mounted. ra_fs_open("mybook.epub") -> priv_dir_find_long:
...
{
  TEST_BEGIN("ra_fs LFN cov: scan_continue + eod -> lines 262 + 301");
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_vol();
  TEST_END("ra_fs LFN cov: scan_continue + eod -> lines 262 + 301");
}
 ...
 *          inject backend before calling ra_fs_open("mybook.epub"). The path
...
{
  TEST_BEGIN("ra_fs LFN cov: priv_read_sector fail in find_long -> line 286");
  build_fat16_vol();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_open(h, "mybook.epub", k_ra_fs_mode_read, &f));
  h->backend = saved;

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_vol();
  TEST_END("ra_fs LFN cov: priv_read_sector fail in find_long -> line 286");
}
 ...
 *          ra_fs_open("/SUB/mybook.epub") is called.
...
{
  TEST_BEGIN("ra_fs LFN cov: walk_next fail in find_long -> lines 262+296-298");
  build_fat16_vol();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mkdir(h, "/SUB"));
  /* 14 files + dot + dotdot = 16 entries; fills the first (only) cluster. */
  create_files_in(h, "/SUB", 14U);

  ra_fs_backend_t saved = h->backend;
  /* Read 1 = root sector (find SUB), Read 2 = subdir sector, Read 3 = FAT. */
  swap_to_inject(h, 2U, 0U);
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_open(h, "/SUB/mybook.epub", k_ra_fs_mode_read, &f));
  h->backend = saved;

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_vol();
  TEST_END("ra_fs LFN cov: walk_next fail in find_long -> lines 262+296-298");
}
...
{
  TEST_BEGIN("ra_fs LFN cov: priv_read_sector fail in find_free -> line 317");
  build_fat16_vol();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_error, priv_dir_find_free(h, &root_loc, &lba, &off));
  h->backend = saved;

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_vol();
  TEST_END("ra_fs LFN cov: priv_read_sector fail in find_free -> line 317");
}
...
{
  TEST_BEGIN("ra_fs LFN cov: walk_next fail in find_free -> lines 328-330");
  build_fat16_vol();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mkdir(h, "/SUB"));
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_error, priv_dir_find_free(h, &subdir_loc, &lba, &off));
  h->backend = saved;

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_vol();
  TEST_END("ra_fs LFN cov: walk_next fail in find_free -> lines 328-330");
}

/**
 * @test test_lfn_cov_no_mem_free
 * @brief priv_dir_find_free returns k_ra_err_no_mem when the fixed root dir
...
{
  TEST_BEGIN("ra_fs LFN cov: root dir full -> line 333 (k_ra_err_no_mem)");
  build_fat16_vol();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  dir_loc_t root_loc = {.is_root = 1U, .cluster = 0U};
  uint32_t  lba = 0U, off = 0U;
  TEST_ASSERT_EQ(k_ra_err_no_mem, priv_dir_find_free(h, &root_loc, &lba, &off));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_vol();
  TEST_END("ra_fs LFN cov: root dir full -> line 333 (k_ra_err_no_mem)");
}
...
{
  TEST_BEGIN("ra_fs LFN cov: priv_fat_get fail in free_chain -> line 346");
  build_fat16_vol();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_error, priv_free_chain(h, 2U));
  h->backend = saved;

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_vol();
  TEST_END("ra_fs LFN cov: priv_fat_get fail in free_chain -> line 346");
}
...
{
  TEST_BEGIN("ra_fs LFN cov: priv_fat_set fail in free_chain -> line 350");
  build_fat16_vol();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_vol();
  TEST_END("ra_fs LFN cov: priv_fat_set fail in free_chain -> line 350");
}

/**
 * @test test_lfn_cov_protocol_error
 * @brief priv_free_chain fires the cyclic-chain guard and returns
 *        k_ra_err_protocol_error (line 359).
 *
...
{
  TEST_BEGIN("ra_fs LFN cov: cyclic FAT guard -> line 359 (protocol_error)");
  build_fat16_vol();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  TEST_ASSERT_EQ(k_ra_err_protocol_error, priv_free_chain(h, (uint32_t)k_lcov_cy_a));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_vol();
  TEST_END("ra_fs LFN cov: cyclic FAT guard -> line 359 (protocol_error)");
}
