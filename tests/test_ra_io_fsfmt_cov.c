/**
 * @file test_ra_io_fsfmt_cov.c
 * @brief Coverage supplement: uncovered branches in ra_io_fsfmt.c.
 *
 * @details
 * Drives the branches that test_ra_io_fsfmt.c does not reach:
 ...
 * the source: ra_io_fsfmt_probe validates backend non-null before dispatching to
...
static uint8_t s_cov_disk[(size_t)k_cov_small_blocks * (size_t)k_ra_io_block_size_bytes];

/* ==========================================================================
 * First-fail mock backend
 *
 * Used in test_fat_probe_rejects_exfat_marker to reach ra_io_fsfmt.c
...
static uint8_t s_cov_exfat_blk[(size_t)k_ra_io_block_size_bytes];

/**
 * @brief Mock read_block: returns an error on call 1, the exFAT block on call 2+.
 *
 * @details
 * Increments s_cov_call on every invocation. Call 1 returns k_ra_err_hw_error to
 ...
 *
 * @return ra_err_t
 * @retval k_ra_err_hw_error First call (s_cov_call == 1).
 * @retval k_ra_ok           Second call and beyond; buf filled from s_cov_exfat_blk.
 *
 ...
 * @since 0.1.0
 */
static ra_err_t mock_read_first_fail(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf)
{
  (void)ctx;
  (void)lba;
  (void)count;
 ...
 *
 * @details
 * Constructs a ra_fs_backend_t that is zero-initialised so its read_block field
 * is nullptr. ra_io_fsfmt_probe validates that the backend pointer itself is not
 ...
 *
 * @pre ra_io_fsfmt_init() succeeds and registers the built-in formats.
 * @pre A zero-initialised ra_fs_backend_t has read_block == nullptr.
 * @post ra_io_fsfmt_probe returns k_ra_err_not_found.
 ...
 *
 * @details
 * After ra_io_fsfmt_init the registry order is [exfat, fat]. The mock backend
 ...
 * k_ra_err_not_found because both built-in probes returned false.
 *
 * @par MC/DC:
 * No compound decisions under test; each branch is a single condition. No
 * additional MC/DC vectors are required.
 *
 * @pre ra_io_fsfmt_init() succeeds.
 ...
 * fat_probe returns false at line 148. The overall probe returns k_ra_err_not_found.
 *
 * @par MC/DC:
 * No compound decisions under test; each branch is a single condition. No
 * additional MC/DC vectors are required.
...
static void test_fat_probe_missing_sig1(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_io_blockdev_ram_init(&bd, &state, s_cov_disk, k_cov_small_blocks, false));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_write(&bd, 0U, 1U, blk));
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_io_fsfmt_cov.c\n");
  return 0;
}
