 ...
 * *opened and parsed* on-device, because `ra_epub_open_streamed()` reads only the
 ...
 *     is byte-identical to the resident `ra_epub_open()` path, and that the total
 *     bytes fetched is far less than the archive (the filler is never read).
 *  2. **The `ra_vmem` page cache** (`ra_vmem_stream`, item 3) fronting the archive
...
static ra_vmem_frame_t s_meta[(size_t)k_frames];
...
/**
 * @brief ra_vsource read callback over ::s_arc (the page-cache path backing).
 * @param[in]  ctx    Unused.
 ...
 * @return k_ra_ok always (loader clamps to the object end before calling).
 * @pre `offset + len <= s_arc_size` (guaranteed by ra_vsource_loader).
 * @pre `buf` is writable for `len` bytes.
 * @post ::g_storage_bytes advanced by `len`.
 * @note Not thread-safe.
 * @since 0.1.0
 */
static ra_err_t storage_read(void* ctx, uint64_t offset, uint8_t* buf, uint32_t len)
{
  (void)ctx;
...
/**
 * @brief Direct ra_epub stream read over a resident buffer (no cache).
...
static void assert_chapters(ra_epub_book_t* book)
{
  static const char* const markers[] = {"ALPHA", "BRAVO", "CHARLIE", "DELTA"};
  for (uint16_t i = 0U; i < 4U; ++i) {
    uint8_t chbuf[k_chapter_max] = {};
    size_t  got                  = 0U;
    TEST_ASSERT_EQ(k_ra_ok, ra_epub_load_chapter(book, i, chbuf, sizeof(chbuf) - 1U, &got));
    TEST_ASSERT(got > 0U);
    chbuf[got] = (uint8_t)'\0';
 ...
 * @brief `ra_epub_open_streamed()` over a seek+read callback parses byte-identically
 *        to `ra_epub_open()` on the same archive, while fetching far fewer bytes
...
static void test_stream_direct_parse_equivalence(void)
{
  ...
  ra_epub_book_t            resident = {};
  const ra_epub_mem_media_t mem      = {.data = s_arc, .size = s_arc_size};
  TEST_ASSERT_EQ(k_ra_ok, ra_epub_open(&mem, "big.epub", &resident));

  /* Streamed parse over the same bytes through a seek+read callback. */
  buf_src_t              src   = {.data = s_arc, .size = s_arc_size};
  ra_epub_stream_media_t media = {.read = direct_read, .ctx = &src, .size = (uint64_t)s_arc_size};
  ra_epub_book_t         book  = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_epub_open_streamed(&media, "big.epub", &book));
  ...
  uint16_t cs = 0U;
  uint16_t cr = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_epub_get_chapter_count(&book, &cs));
  TEST_ASSERT_EQ(k_ra_ok, ra_epub_get_chapter_count(&resident, &cr));
  TEST_ASSERT_EQ(4U, cs);
  TEST_ASSERT_EQ(cr, cs);

  ra_epub_metadata_t ms = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_epub_get_metadata(&book, &ms));
  ...
  for (uint16_t i = 0U; i < cs; ++i) {
    ...
    size_t  ga               = 0U;
    size_t  gb               = 0U;
    TEST_ASSERT_EQ(k_ra_ok, ra_epub_load_chapter(&book, i, a, sizeof(a), &ga));
    TEST_ASSERT_EQ(k_ra_ok, ra_epub_load_chapter(&resident, i, b, sizeof(b), &gb));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_epub_close(&resident));
  TEST_END("epub stream: direct callback == resident parse, no whole-file read");
}

/* ---------------------------------------------------------------------------
 * Test: streaming through the ra_vmem page cache -- bounded residency + churn.
 * ---------------------------------------------------------------------------
 */

/**
 * @test test_stream_via_pagecache_bounded
 * @brief Open + parse the large EPUB through a fixed, tiny `ra_vmem` frame pool:
...
{
  TEST_BEGIN("epub stream: bounded residency through ra_vmem page cache");
  build_big_epub();
  g_storage_bytes = 0U;
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_vsource_add_paged(&vs, storage_read, nullptr, 0U, (uint64_t)s_arc_size, &obj));
  ra_vmem_t     vm  = {};
  ra_vmem_cfg_t cfg = {.frame_mem    = s_frames,
  ...
  ra_vmem_stream_t st = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_vmem_stream_init(&st, &vm, obj, (uint64_t)s_arc_size));

  /* Open + parse the whole book through the tiny cache. */
  ra_epub_stream_media_t media = {.read = ra_vmem_stream_read,
                                  .ctx  = &st,
  ...
  size_t  cover_got          = 0U;
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_epub_get_resource(&book, "cover.png", cover, sizeof(cover), &cover_got));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_epub_close(&book));
  TEST_END("epub stream: bounded residency through ra_vmem page cache");
}
 ...
 * @brief `ra_epub_open_streamed()` rejects NULL/empty media and a non-ZIP stream.
 *
 * @par MC/DC:
...
static void test_stream_open_arg_guards(void)
{
  ...
  ra_epub_book_t         book = {};
  buf_src_t              src  = {.data = s_arc, .size = s_arc_size};
  ra_epub_stream_media_t good = {.read = direct_read, .ctx = &src, .size = (uint64_t)s_arc_size};

  /* NULL-OR guard, each operand flipped. */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_epub_open_streamed(nullptr, "x", &book));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_epub_open_streamed(&good, "x", nullptr));

  /* invalid-arg OR guard, each operand flipped. */
  ra_epub_stream_media_t no_read = {.read = nullptr, .ctx = &src, .size = (uint64_t)s_arc_size};
  ra_epub_stream_media_t no_size = {.read = direct_read, .ctx = &src, .size = 0U};
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_epub_open_streamed(&no_read, "x", &book));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_epub_open_streamed(&no_size, "x", &book));
  ...
  ra_epub_stream_media_t junk     = {.read = direct_read,
                                     .ctx  = &jsrc,
                                     .size = (uint64_t)sizeof(k_junk)};
  TEST_ASSERT_EQ(k_ra_err_validation_failed, ra_epub_open_streamed(&junk, "x", &book));
...
/* ---------------------------------------------------------------------------
 * Test: ra_vmem_stream init + read guards.
 * ---------------------------------------------------------------------------
 */

/**
 * @test test_vmem_stream_guards
 * @brief `ra_vmem_stream_init` rejects NULL/zero args; `ra_vmem_stream_read`
 ...
 * condition; init's NULL guards are RA_CHECK_NULL_PTR one-condition checks and
 * read's guards are separate single-condition early returns.)
 */
static void test_vmem_stream_guards(void)
{
  TEST_BEGIN("ra_vmem_stream: init + read guards");
  /* init guards. */
  ra_vmem_stream_t st = {};
  ra_vmem_t        vm = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_vmem_stream_init(nullptr, &vm, 0U, 1U));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_vmem_stream_init(&st, nullptr, 0U, 1U));
  /* vm.cfg.frame_bytes is 0 on a zeroed cache; size=0 trips the size guard first. */
  TEST_ASSERT_EQ(k_ra_err_invalid_size, ra_vmem_stream_init(&st, &vm, 0U, 0U));

  /* read guards -- all return 0 bytes. */
  uint8_t buf[4] = {};
  TEST_ASSERT_EQ(0U, ra_vmem_stream_read(nullptr, 0U, buf, sizeof(buf)));
  ra_vmem_stream_t bound = {.vm          = &vm,
                            .object_id   = 0U,
                            .frame_bytes = (uint32_t)k_frame_bytes,
                            .size        = 100U};
  TEST_ASSERT_EQ(0U, ra_vmem_stream_read(&bound, 0U, nullptr, sizeof(buf))); /* NULL buf */
  TEST_ASSERT_EQ(0U, ra_vmem_stream_read(&bound, 0U, buf, 0U));              /* len 0    */
  TEST_ASSERT_EQ(0U, ra_vmem_stream_read(&bound, 100U, buf, sizeof(buf)));   /* at EOF   */
  ra_vmem_stream_t unbound = {.vm = &vm, .object_id = 0U, .frame_bytes = 0U, .size = 100U};
  TEST_ASSERT_EQ(0U, ra_vmem_stream_read(&unbound, 0U, buf, sizeof(buf))); /* frame_bytes 0 */

  TEST_END("ra_vmem_stream: init + read guards");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_epub_streaming.c\n");
  return 0;
}
