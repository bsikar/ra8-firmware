 ...
 * the hardware enumeration ladder (`ra_usb_hmsc_enumerate`), which the
 ...
 * `s_usb_hmsc_state` (shared via `ra_hal_internal.h`, the same object the
 ...
 * cannot signal success under the host simulator: `ra_usb_host_bulk_out`
 * W0C-clears the pipe's `BEMPSTS` bit before waiting, and the plain-RAM
 * `ra_sim_mmap` backing has no active USB SIE model to re-assert it, so
 * the CBW push deterministically returns `k_ra_err_hw_timeout`. That is
...
typedef enum : uint8_t {
  ...
  k_test_lun_bad     = 5U,  /**< Out-of-range LUN (> k_ra_hmsc_max_lun). */
 ...
 *
 * @details Mirrors `test_ra_usb_hmsc.c::prep`: wipes the simulated MMIO
 ...
 *
 * @details On real hardware `ra_usb_hmsc_enumerate` sets this flag after a
 ...
 * `ra_usb_hmsc_init`; only `attached` needs injecting.
 */
 ...
 * returns `k_ra_err_hw_timeout` because the simulator cannot re-assert the
 * pipe's BEMPSTS bit)
 */
static void test_inquiry_attached_drives_bot_push(void)
{
  ...
  /* Reaches internal_check_ready (LUN in range -> k_ra_ok),
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_usb_hmsc_inquiry(k_test_lun_ok, &resp));
...
static void test_read_capacity_attached_drives_bot_push(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_timeout,
                 ra_usb_hmsc_read_capacity(k_test_lun_ok, &block_count, &block_size));
...
static void test_read10_attached_drives_bot_push(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_timeout,
                 ra_usb_hmsc_read10(k_test_lun_ok, 0U, k_test_block_count, buf));
...
static void test_write10_attached_drives_bot_push(void)
{
  ...
  uint8_t buf[k_test_scratch_len] = {};
  TEST_ASSERT_EQ(k_ra_err_hw_timeout,
                 ra_usb_hmsc_write10(k_test_lun_ok, 0U, k_test_block_count, buf));
 ...
 * !attached legs; this case covers the `target_lun > k_ra_hmsc_max_lun`
...
static void test_scsi_lun_out_of_range_rejected(void)
{
   ...
   * state guards; the LUN guard (target_lun > k_ra_hmsc_max_lun) then
   * rejects -- the leg the base suite cannot reach. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_hmsc_inquiry(k_test_lun_bad, &resp));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_usb_hmsc_read_capacity(k_test_lun_bad, &block_count, &block_size));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_usb_hmsc_read10(k_test_lun_bad, 0U, k_test_block_count, buf));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_usb_hmsc_write10(k_test_lun_bad, 0U, k_test_block_count, buf));
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_usb_hmsc_cov.c\n");
  return 0;
}
