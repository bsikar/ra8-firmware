/**
 * @file tests/test_ra_canfd_frame_cov.c
 * @brief Coverage-gap tests for libs/ra_hal/src/ra_canfd_frame.c
 *
 * @details
 * The original test_ra_canfd.c suite left the #ifdef RA_SIMULATOR_MODE
 * function ra_canfd_test_inject_frame() completely untested.  That
 * function is compiled into every host test build (RA_SIMULATOR_MODE is
 ...
 *
 *  - Bad channel -> k_ra_err_null_ptr (null-check branch).
 ...
 *
 * Additionally each inject call is followed by ra_canfd_receive() to
...
typedef enum : uint32_t {
  ...
  k_inj_len_short  = 8U,          /**< data_len <= k_ra_canfd_data_bytes_max.       */
  k_inj_len_over   = 200U,        /**< data_len > k_ra_canfd_data_bytes_max: clamp. */
...
/**
 * @brief Inject with an out-of-range channel; expect k_ra_err_null_ptr.
 *
 * @details
 * Drives the `if (reg == nullptr) { return k_ra_err_null_ptr; }` branch
 * (ra_canfd_frame.c lines 335-336).  ra_canfd(2) returns nullptr because
...
static void test_inject_bad_channel(void)
{
  ...
  const ra_err_t err = ra_canfd_test_inject_frame((uint8_t)k_inj_ch_bad,
...
/**
 * @brief Inject a frame with a valid channel and non-null data; expect k_ra_ok.
 *
 * @details
 * Drives the full happy path of ra_canfd_test_inject_frame():
 *
 ...
 *  - k_ra_ok returned (line 349).
 *
 * The test then calls ra_canfd_receive() to confirm the register state
 * matches what the production path expects (RFEMP=0, ID correct).
 *
 * @par MC/DC:
 * Decision 1: `data_len > k_ra_canfd_data_bytes_max` (single condition).
...
static void test_inject_happy_path(void)
{
  ...
  uint8_t data[k_ra_canfd_data_bytes_max];
  fill_pattern(data, (uint32_t)k_inj_len_short);

  /* Arm RFEMP so we can verify it gets cleared. */
  volatile r_canfd_t* reg = ra_canfd((uint8_t)k_inj_ch0);
  reg->CFDRFSTS[0]        = (uint32_t)k_ra_rfsts_bit_empty;

  const ra_err_t err = ra_canfd_test_inject_frame((uint8_t)k_inj_ch0,
                                                  (uint32_t)k_inj_id_ext,
                                                  (uint32_t)k_inj_ptr_dlc15,
                                                  (uint32_t)k_inj_fdsts_fd,
                                                  data,
  ...
  const ra_err_t   rx  = ra_canfd_receive((uint8_t)k_inj_ch0, &out);
  TEST_ASSERT_EQ(k_ra_ok, rx);
 ...
 *   `(data_len > k_ra_canfd_data_bytes_max) ? k_ra_canfd_data_bytes_max : data_len`
 * (ra_canfd_frame.c lines 341-343).  data_len=200 forces copy_len to be
 ...
 *
 * @par MC/DC:
 * Decision: `data_len > k_ra_canfd_data_bytes_max` (single condition).
 ...
 * @post ra_canfd_test_inject_frame() returns k_ra_ok.
 * @note Not thread-safe.
 * @since 0.1.0
 */
static void test_inject_clamp_data_len(void)
{
  ...
  uint8_t data[k_ra_canfd_data_bytes_max];
  fill_pattern(data, (uint32_t)k_ra_canfd_data_bytes_max);
  ...
  TEST_ASSERT_EQ(data[0], reg->CFDRF[0].DF[0]);
  TEST_ASSERT_EQ(data[k_ra_canfd_data_bytes_max - 1U],
                 reg->CFDRF[0].DF[k_ra_canfd_data_bytes_max - 1U]);
 ...
 * inside the DF[] copy loop (ra_canfd_frame.c line 345).  When the
...
static void test_inject_null_data_zeroes_df(void)
{
  ...
  volatile r_canfd_t* reg = ra_canfd((uint8_t)k_inj_ch1);
  for (uint32_t i = 0U; i < (uint32_t)k_inj_len_null; i++) {
    reg->CFDRF[0].DF[i] = (uint8_t)k_inj_byte_seed;
  }

  const ra_err_t err = ra_canfd_test_inject_frame((uint8_t)k_inj_ch1,
                                                  (uint32_t)k_inj_id_ext,
                                                  (uint32_t)k_inj_ptr_dlc15,
                                                  (uint32_t)k_inj_fdsts_fd,
                                                  nullptr,
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_canfd_frame_cov.c\n");
  return 0;
}
