/**
 * @file test_ra_iic_b_peripheral.c
...
/**
 * @enum ra_iic_b_peripheral_test_const_t
 * @brief Numeric constants used by the tests.
 */
typedef enum : uint32_t {
  k_ra_iic_b_peripheral_test_ch0         = 0U,
  k_ra_iic_b_peripheral_test_ch_oor      = 9U,
  k_ra_iic_b_peripheral_test_addr_7b     = 0x42U,
  k_ra_iic_b_peripheral_test_byte_a      = 0xA5U,
  k_ra_iic_b_peripheral_test_byte_b      = 0x5AU,
  k_ra_iic_b_peripheral_test_ntst_tdbef0 = 0x01U,
  k_ra_iic_b_peripheral_test_ntst_rdbff0 = 0x02U,
} ra_iic_b_peripheral_test_const_t;

static const ra_iic_b_peripheral_cfg_t k_cfg = {
  .peripheral_addr_7b = (uint8_t)k_ra_iic_b_peripheral_test_addr_7b,
  .general_call       = 1U,
};

static void prearm(volatile r_iic_b_regs_t* reg)
{
  reg->NTST = (uint32_t)((uint32_t)k_ra_iic_b_peripheral_test_ntst_tdbef0 |
 ...
 * ``ra_iic_b_peripheral_open``. The precondition is a single-condition
 ...
 *
 * @par MC/DC:
 * Exercises the ``ch >= k_ra_iic_b_max_channels`` range-check decision in
 * ``ra_iic_b_peripheral_open``. Single-condition decision: vector
 ...
 *
 * @par MC/DC:
 * Drives the happy-path branches of ``ra_iic_b_peripheral_open`` and
 ...
 * ``ra_iic_b_peripheral_send`` (vector: len=1, data=valid -> dec F).
 * Companion vectors live in ``test_send_null`` (len=1, data=NULL ->
 * dec T) and the dedicated ``test_mcdc_ra_iic_b_peripheral`` block
 * (len=0, data=*). N+1 = 3 satisfied jointly.
 */
static void test_send_ok(void)
{
  TEST_BEGIN("ra_iic_b_peripheral_send ok");
  ra_sim_mmap_reset();
  volatile r_iic_b_regs_t* reg = ra_iic_b((uint8_t)k_ra_iic_b_peripheral_test_ch0);
  prearm(reg);
 ...
 * ``ra_iic_b_peripheral_send`` (vector: len=1, data=NULL -> dec T,
 * returns k_ra_err_null_ptr). Pairs with ``test_send_ok`` to flip the
 * data condition and with ``test_mcdc_ra_iic_b_peripheral`` to flip
 * len. N+1 = 3 jointly.
 */
 ...
 * ``ra_iic_b_peripheral_receive`` (vector: len=1, buf=valid -> dec F).
 * Companion vectors live in ``test_mcdc_ra_iic_b_peripheral``
 * (len=0,*) and (len=1, buf=NULL). N+1 = 3 jointly.
 */
static void test_receive_ok(void)
{
  TEST_BEGIN("ra_iic_b_peripheral_receive ok");
  ra_sim_mmap_reset();
  volatile r_iic_b_regs_t* reg = ra_iic_b((uint8_t)k_ra_iic_b_peripheral_test_ch0);
  prearm(reg);
 ...
 *
 * @par MC/DC:
 * Two single-condition guards in ``ra_iic_b_peripheral_status``:
 *   (a) ``ch >= k_ra_iic_b_max_channels`` (range)
...
{
  TEST_BEGIN("ra_iic_b_peripheral_status reflects flags");
  ra_sim_mmap_reset();
  volatile r_iic_b_regs_t* reg = ra_iic_b((uint8_t)k_ra_iic_b_peripheral_test_ch0);
  reg->NTST                    = (uint32_t)((uint32_t)k_ra_iic_b_peripheral_test_ntst_tdbef0 |
                                            (uint32_t)k_ra_iic_b_peripheral_test_ntst_rdbff0);
...
/**
 * @test test_mcdc_ra_iic_b_peripheral
 *
 * @par MC/DC:
 * Decision A (libs/ra_hal/src/ra_iic_b_peripheral.c:179): // CITES-OK: MC/DC tracker requires file:line
 ...
 * Decision B (libs/ra_hal/src/ra_iic_b_peripheral.c:216): // CITES-OK: MC/DC tracker requires file:line
 * ``(len > 0U) && (buf == nullptr)``. Mirror vectors. DO-178C 6.4.4.3.
 */
static void test_mcdc_ra_iic_b_peripheral(void)
{
