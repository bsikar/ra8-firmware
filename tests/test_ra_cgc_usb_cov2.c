/**
 * @file test_ra_cgc_usb_cov2.c
 * @brief Complementary white-box line-coverage tests for the PLL2 + USB clock
 *        driver (`ra_cgc_usb.c`) -- the legs `test_ra_cgc_usb_cov.c` cannot reach.
 *
 * @par Tag
 * [Ring 1 / HAL] {World: S}
 *
 * @details
 * This TU compiles a second instrumented copy of `ra_cgc_usb.c` (exported
 * symbols renamed `*_cov2`) so gcovr unions its `.gcda` with the sibling
 * `test_ra_cgc_usb_cov.c` copy. It exists to close the residual gaps that the
 ...
 *  2. The USB-FS "PLL2 enable failed" abort inside `ra_cgc_usbfs_clock_enable`
 ...
 *     `ra_sys_usbckcr` / `ra_sys_usb60ckcr` accessors are redirected to a
 ...
 * deterministic per-bit mocks, and `RA_SIMULATOR_MODE` is undefined for the
 ...
 *
 * The driver caches `ra_sys_usbck*()` once per SRDY wait, so the SRDY level a
...
static const uint8_t k_cov2_hococr_hcstp = (uint8_t)(1U << k_ra_hococr_hcstp);
...
{
  const uint8_t sreq = (uint8_t)(1U << k_ra_usbckcr_bit_sreq);
  const uint8_t srdy = (uint8_t)(1U << k_ra_usbckcr_bit_srdy);
...
/** @brief Stateful mock for `ra_sys_usbckcr()` -- drives the USB-FS handshake. */
static volatile uint8_t* mock_ra_sys_usbckcr(void)
{
  cov2_apply_srdy(&s_usbfs_ckcr);
  return &s_usbfs_ckcr;
}

/** @brief Stateful mock for `ra_sys_usb60ckcr()` -- drives the USB-HS handshake. */
static volatile uint8_t* mock_ra_sys_usb60ckcr(void)
{
 ...
 * production driver in ra_core_hal; redirect the OSCSF poll helpers and the two
 * USB clock-register accessors to the scriptable mocks above. */
ra_err_t ra_cgc_pll2_enable_cov2(uint8_t mul_int, uint8_t mul_quarters, ra_plodiv_t p_div_code);
ra_err_t ra_cgc_usbfs_clock_enable_cov2(void);
ra_err_t ra_cgc_usbhs_pll_enable_cov2(void);
ra_err_t ra_cgc_ensure_hoco_running_for_usb_ck_cov2(void);

#define ra_cgc_pll2_enable                    ra_cgc_pll2_enable_cov2
#define ra_cgc_usbfs_clock_enable             ra_cgc_usbfs_clock_enable_cov2
#define ra_cgc_usbhs_pll_enable               ra_cgc_usbhs_pll_enable_cov2
#define ra_cgc_ensure_hoco_running_for_usb_ck ra_cgc_ensure_hoco_running_for_usb_ck_cov2
#define ra_cgc_wait_oscsf_set                 mock_wait_oscsf_set
#define ra_cgc_wait_oscsf_clear               mock_wait_oscsf_clear
#define ra_sys_usbckcr                        mock_ra_sys_usbckcr
#define ra_sys_usb60ckcr                      mock_ra_sys_usb60ckcr

/* Undefine RA_SIMULATOR_MODE for the driver body ONLY so the static SRDY poll
 ...
 * `mul_quarters > 3` both return ::k_ra_err_invalid_arg, and pre-asserting
 * OSCSF.PLL2SF makes the idempotency guard short-circuit to ::k_ra_ok without
...
static void test_pll2_arg_and_idempotency(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_cgc_pll2_enable_cov2((uint8_t)k_cov2_pll2_zero_mul,
                                         (uint8_t)k_cov2_pll2_ok_quarters,
                                         k_ra_plodiv_div4));

  /* mul_quarters > 3 -> invalid_arg. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_cgc_pll2_enable_cov2((uint8_t)k_cov2_pll2_mul,
 ...
 * scripted to time out. `ra_cgc_usbfs_clock_enable` then takes its "pll2 enable
...
static void test_usbfs_pll2_enable_failed(void)
{
  ...
  s_set_fail_bit = (int)k_ra_oscsf_bit_pll2sf; /* PLL2SF lock poll times out. */
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_cgc_usbfs_clock_enable_cov2());
...
static void test_usbfs_hoco_stabilize_timeout(void)
{
  ...
  s_set_fail_bit = (int)k_ra_oscsf_bit_hocosf; /* PLL2SF passes; HOCOSF fails. */
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_cgc_usbfs_clock_enable_cov2());
 ...
 * `if (got == expected)`, and each `if (err != k_ra_ok)` are single-condition.)
 */
static void test_usbfs_hcstp_and_second_srdy_timeout(void)
{
  ...
  *ra_sys_hococr() = k_cov2_hococr_hcstp; /* Exercise the HOCO-restart branch. */
 ...
 * `ra_cgc_usbfs_clock_enable` takes its terminal ::k_ra_ok return -- a line the
...
static void test_usbfs_full_success(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_cgc_usbfs_clock_enable_cov2());
 ...
 * clears the bit; with the HOCOSF set poll passing the helper returns ::k_ra_ok.
 *
 * @par MC/DC:
...
static void test_ensure_hoco_hcstp_restart(void)
{
  ...
  *ra_sys_hococr() = k_cov2_hococr_hcstp;
  s_set_fail_bit   = -1;
  TEST_ASSERT_EQ(k_ra_ok, ra_cgc_ensure_hoco_running_for_usb_ck_cov2());
...
static void test_usbhs_pll2_enable_failed(void)
{
  ...
  s_set_fail_bit = (int)k_ra_oscsf_bit_pll2sf; /* MOSCSF passes; PLL2SF fails. */
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_cgc_usbhs_pll_enable_cov2());
...
static void test_usbhs_hoco_stabilize_timeout(void)
{
  ...
  s_set_fail_bit = (int)k_ra_oscsf_bit_hocosf; /* MOSCSF + PLL2SF pass; HOCOSF fails. */
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_cgc_usbhs_pll_enable_cov2());
...
static void test_usbhs_hcstp_and_second_srdy_timeout(void)
{
  ...
  *ra_sys_hococr() = k_cov2_hococr_hcstp; /* Exercise the ensure-HOCO restart. */
  s_set_fail_bit   = -1;                  /* All OSCSF polls pass.             */
  s_srdy_mode      = k_cov2_srdy_stuck_high;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_cgc_usbhs_pll_enable_cov2());
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_cgc_usb_cov2.c\n");
  return 0;
}
