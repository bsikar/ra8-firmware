/**
 * @file test_ra_fs_fat_file_cov.c
 * @brief Coverage booster for libs/ra_fs/src/ra_fs_fat_file.c.
 *
 * @details
 * Dedicated companion test executable that drives the branches in
 * ra_fs_fat_file.c that no sibling test yet exercises:
 *
 ...
 *   - ra_fs_open guards (lines 453, 462, 484).
 *
 * The in-memory block device harness mirrors the pattern from
 * tests/test_ra_fs_fat.c exactly.  An additional "inject" backend wraps
 ...
 *          reaches zero the next read returns ::k_ra_err_hw_error.  When
 ...
 *          geometry used by test_ra_fs_fat.c so shared assumptions hold.
 *
 ...
 *
 * @details Allocates 65536 sectors (32 MiB) and calls ra_fs_format
...
{
  TEST_BEGIN("ra_fs_fat_file cov: reopen in write mode calls truncate");
  build_fat16_volume();
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "DATA.TXT", k_ra_fs_mode_write, &f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_write(f, &one, 1U));
  ...
  free_volume();
  TEST_END("ra_fs_fat_file cov: reopen in write mode calls truncate");
}
 ...
 * offset is advanced to size (line 148).  A subsequent ra_fs_tell confirms
...
{
  TEST_BEGIN("ra_fs_fat_file cov: open existing in append mode");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "APND.TXT", k_ra_fs_mode_write, &f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_write(f, payload, sizeof(payload)));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));

  /* Append mode: covers line 148 (f->offset = f->size_bytes). */
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "APND.TXT", k_ra_fs_mode_append, &f));
  uint32_t pos = 0U;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "APND.TXT", k_ra_fs_mode_read, &f));
  pos = 99U;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_tell(f, &pos));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fat_file cov: open existing in append mode");
}

/**
 * @test test_open_existing_no_mem
 * @brief File-table exhaustion returns k_ra_err_no_mem from priv_open_existing.
 *
 * @details
 * Creates five files, opens four to fill all four file slots (k_ra_fs_max_files
 * == 4), then tries to open the fifth existing file.  priv_alloc_file_slot
 * returns nullptr and priv_open_existing returns k_ra_err_no_mem (line 128).
 *
 * Line targeted: 128.
 *
 * @pre Volume is formatted and accessible.
...
{
  TEST_BEGIN("ra_fs_fat_file cov: open existing file with full file table");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));

  /* Create five files on disk so a fifth one exists to open. */
  ra_fs_file_t* tmp = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "A.TXT", k_ra_fs_mode_write, &tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "B.TXT", k_ra_fs_mode_write, &tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "C.TXT", k_ra_fs_mode_write, &tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "D.TXT", k_ra_fs_mode_write, &tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "E.TXT", k_ra_fs_mode_write, &tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(tmp));

  /* Fill all four file slots. */
  ra_fs_file_t* fa = nullptr;
  ra_fs_file_t* fb = nullptr;
  ra_fs_file_t* fc = nullptr;
  ra_fs_file_t* fd = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "A.TXT", k_ra_fs_mode_read, &fa));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "B.TXT", k_ra_fs_mode_read, &fb));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "C.TXT", k_ra_fs_mode_read, &fc));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "D.TXT", k_ra_fs_mode_read, &fd));

  /* Fifth open: existing file but no slot available -- line 128. */
  ra_fs_file_t* fe = nullptr;
  TEST_ASSERT_EQ(k_ra_err_no_mem, ra_fs_open(h, "E.TXT", k_ra_fs_mode_read, &fe));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(fa));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(fb));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(fc));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(fd));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fat_file cov: open existing file with full file table");
}
 ...
 * @brief A path component longer than k_ra_fs_short_name_len is rejected.
 *
 * @details
 * ra_fs_open with a path whose first component has 13 characters triggers the
 * `len > k_ra_fs_short_name_len` check in priv_enter_subdir (line 264).
 *
 * Line targeted: 264.
 *
 * @pre Volume is formatted and accessible.
 * @post Open returns k_ra_err_invalid_arg.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
...
{
  TEST_BEGIN("ra_fs_fat_file cov: subdir component name too long");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));

  /* Component "TOOLONGNAME13" is 13 chars > k_ra_fs_short_name_len (12). */
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_fs_open(h, "/TOOLONGNAME13/FILE.TXT", k_ra_fs_mode_read, &f));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fat_file cov: subdir component name too long");
}
...
{
  TEST_BEGIN("ra_fs_fat_file cov: subdir component fails 8.3 pack");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_fs_open(h, "/.HIDDEN/FILE.TXT", k_ra_fs_mode_read, &f));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fat_file cov: subdir component fails 8.3 pack");
}
 ...
 * ra_fs_open tries to descend into "/CORR/FILE.TXT", priv_enter_subdir finds
 * first_cluster < k_cluster_first_data and returns k_ra_err_protocol_error
 * (line 287).
 *
 * Line targeted: 287.
 *
 ...
 * V1: cl=0 (zeroed entry) -> TRUE -> k_ra_err_protocol_error.
 ...
 *
 * @pre Volume is formatted and accessible.
 * @post Open returns k_ra_err_protocol_error.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
...
{
  TEST_BEGIN("ra_fs_fat_file cov: subdir entry has cluster 0");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));

  /* Create the directory -- its dir entry goes at root LBA 65, offset 0. */
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mkdir(h, "/CORR"));
  ...
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_err_protocol_error, ra_fs_open(h, "/CORR/FILE.TXT", k_ra_fs_mode_read, &f));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fat_file cov: subdir entry has cluster 0");
}
 ...
 * resolves successfully.  A final ra_fs_open with the 33-component path causes
 * the depth loop in priv_resolve_parent to exhaust its bound (k_path_max_depth
 * == 32) and return k_ra_err_invalid_arg (line 329).
 *
 * Line targeted: 329.
 *
 * @pre Volume is formatted and accessible.
...
{
  TEST_BEGIN("ra_fs_fat_file cov: path depth exceeds k_path_max_depth");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  for (uint32_t i = 0U; i < 32U; i++) {
    ...
    TEST_ASSERT_EQ(k_ra_ok, ra_fs_mkdir(h, mkdir_path));
  }
  ...
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_fs_open(h, mkdir_path, k_ra_fs_mode_read, &f));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fat_file cov: path depth exceeds k_path_max_depth");
}
 ...
 *
 * @details
 * ra_fs_listdir with a multi-component path whose first component does not
 * exist causes priv_resolve_parent -> priv_enter_subdir to return
 * k_ra_err_not_found, which priv_resolve_dir then propagates at line 348.
 *
 * Line targeted: 348.
 *
 * @pre Volume is formatted and accessible.
 * @post listdir returns k_ra_err_not_found.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
...
{
  TEST_BEGIN("ra_fs_fat_file cov: resolve_dir propagates not-found error");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_fs_listdir(h, "/NOMATCH/SUB", count_cb, &count));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fat_file cov: resolve_dir propagates not-found error");
}
 ...
 *
 * @details
 * Creates subdirectory "SUB", then calls ra_fs_listdir with "/SUB/".  In
 ...
 * line 354 is taken, setting *out = parent (line 355) and returning k_ra_ok
 * (line 356).
 *
...
{
  TEST_BEGIN("ra_fs_fat_file cov: trailing slash resolves to parent dir");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_listdir(h, "/SUB/", count_cb, &count));
  TEST_ASSERT_EQ(0U, count);

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fat_file cov: trailing slash resolves to parent dir");
}
 ...
 * k_ra_err_no_mem, which priv_create_new propagates at line 401.
 *
 * Line targeted: 401.
 *
 * @pre Volume is formatted and accessible.
 * @post The seventeenth ra_fs_open returns k_ra_err_no_mem.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
...
{
  TEST_BEGIN("ra_fs_fat_file cov: root directory full returns no_mem");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  for (uint32_t i = 0U; i < 16U; i++) {
    TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, names[i], k_ra_fs_mode_write, &tmp));
    TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(tmp));
  }

  /* Seventeenth file: root directory is full, priv_dir_find_free fails. */
  TEST_ASSERT_EQ(k_ra_err_no_mem, ra_fs_open(h, "F16.TXT", k_ra_fs_mode_write, &tmp));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fat_file cov: root directory full returns no_mem");
}
 ...
 * returns nullptr and priv_create_new returns k_ra_err_no_mem (line 405).
 *
 * Line targeted: 405.
 *
 * @pre Volume is formatted and accessible.
 * @post The fifth open returns k_ra_err_no_mem.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
...
{
  TEST_BEGIN("ra_fs_fat_file cov: create new file with full slot table");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  ra_fs_file_t* tmp = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "PA.TXT", k_ra_fs_mode_write, &tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "PB.TXT", k_ra_fs_mode_write, &tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "PC.TXT", k_ra_fs_mode_write, &tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "PD.TXT", k_ra_fs_mode_write, &tmp));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(tmp));
  ...
  ra_fs_file_t* fc = nullptr;
  ra_fs_file_t* fd = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "PA.TXT", k_ra_fs_mode_read, &fa));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "PB.TXT", k_ra_fs_mode_read, &fb));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "PC.TXT", k_ra_fs_mode_read, &fc));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "PD.TXT", k_ra_fs_mode_read, &fd));

  /* NEW.TXT does not exist; create path -> dir slot free but no file slot. */
  ra_fs_file_t* fn = nullptr;
  TEST_ASSERT_EQ(k_ra_err_no_mem, ra_fs_open(h, "NEW.TXT", k_ra_fs_mode_write, &fn));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fat_file cov: create new file with full slot table");
}

/* ===========================================================================
 * Tests targeting ra_fs_open guards (lines 453, 462)
 * ===========================================================================
 */

/**
 * @test test_open_not_in_use
 * @brief ra_fs_open on an unmounted handle returns k_ra_err_invalid_state.
 *
 * @details
 * Mounts, stores the handle, unmounts (setting in_use=0), then calls
 * ra_fs_open on the stale handle.  Line 453 returns k_ra_err_invalid_state.
 *
 * Line targeted: 453.
 *
 * @par MC/DC:
 * Decision: `if (handle->in_use == 0U)` (line 452, 1 condition).
 * V1: in_use=0 (unmounted) -> TRUE -> k_ra_err_invalid_state (line 453).
 * V2: in_use=1 (mounted)   -> FALSE -> continue (all other tests).
 * N=1 condition, 1 independent vector per DO-178C 6.4.4.3.
 *
 * @pre Volume is formatted and accessible.
 * @post ra_fs_open returns k_ra_err_invalid_state.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
...
{
  TEST_BEGIN("ra_fs_fat_file cov: ra_fs_open on unmounted handle");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  ...
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_fs_open(h, "DEAD.TXT", k_ra_fs_mode_read, &f));

  free_volume();
  TEST_END("ra_fs_fat_file cov: ra_fs_open on unmounted handle");
}

/**
 * @test test_open_exfat_dispatch
 * @brief ra_fs_open on an exFAT volume dispatches to priv_exfat_open.
 *
 * @details
 * Formats and mounts an exFAT volume, then calls ra_fs_open in read mode for
 * a non-existent file.  Line 462 (`return priv_exfat_open(...)`) is executed;
 * priv_exfat_open returns k_ra_err_not_found since the file does not exist.
 *
 * Line targeted: 462.
 *
 * @pre s_disk has space for 65536 sectors (32 MiB).
 * @post ra_fs_open returns k_ra_err_not_found.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
...
{
  TEST_BEGIN("ra_fs_fat_file cov: ra_fs_open dispatches to exFAT");
  build_exfat_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));

  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_fs_open(h, "EFILE.TXT", k_ra_fs_mode_read, &f));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fat_file cov: ra_fs_open dispatches to exFAT");
}
 ...
 * attempts to write the freed FAT entry, fails, and returns k_ra_err_hw_error.
 ...
 *
 * @par MC/DC:
 * Decision: `if (err != k_ra_ok)` at line 72 (1 condition, truncate free chain).
 ...
 *
 * Decision: `if (err != k_ra_ok)` at line 143 (1 condition, truncate result in open_existing).
 * V1: truncate fails  -> TRUE  -> lines 144-145.
 * V2: truncate OK     -> FALSE -> continue (test_reopen_write_truncates).
 *
 * @pre Volume is formatted and accessible.
 * @post ra_fs_open returns k_ra_err_hw_error.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
...
{
  TEST_BEGIN("ra_fs_fat_file cov: free_chain write error propagates through truncate");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "WFAIL.TXT", k_ra_fs_mode_write, &f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_write(f, &one, 1U));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));
  ...
  ra_fs_file_t* f2 = nullptr;
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_open(h, "WFAIL.TXT", k_ra_fs_mode_write, &f2));

  h->backend = saved;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fat_file cov: free_chain write error propagates through truncate");
}
 ...
 *
 * @par MC/DC:
 * Decision: `if (err != k_ra_ok)` at line 84 (1 condition, read-sector in truncate).
...
{
  TEST_BEGIN("ra_fs_fat_file cov: directory read error inside priv_truncate_existing");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "RFAIL.TXT", k_ra_fs_mode_write, &f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_write(f, &one, 1U));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));
   ...
   */
  ra_fs_file_t* f2 = nullptr;
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_open(h, "RFAIL.TXT", k_ra_fs_mode_write, &f2));

  h->backend = saved;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fat_file cov: directory read error inside priv_truncate_existing");
}
 ...
 * The ra_fs_open for a new file (write mode) consumes:
 ...
 *
 * @par MC/DC:
 * Decision: `if (err != k_ra_ok)` at line 166 (1 condition, read in write_new_dir_entry).
 * V1: read fails  -> TRUE  -> line 167 (return err).
 * V2: read OK     -> FALSE -> continue (all file-creation tests).
 *
 * Decision: `if (err != k_ra_ok)` at line 408 (1 condition, write_new_dir_entry result).
...
{
  TEST_BEGIN("ra_fs_fat_file cov: read error in priv_write_new_dir_entry");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
   ...
   */
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_open(h, "NDIR.TXT", k_ra_fs_mode_write, &f));

  h->backend = saved;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fat_file cov: read error in priv_write_new_dir_entry");
}

/**
 * @test test_dir_find_io_error
 * @brief A backend read failure during priv_dir_find surfaces at ra_fs_open line 484.
 *
 * @details
 * Swaps to the inject backend with reads_left=0 (fail immediately).  The
 * first read inside priv_dir_find fails, returning k_ra_err_hw_error (which is
 * neither k_ra_ok nor k_ra_err_not_found).  ra_fs_open propagates it at
 * line 484 (`return err`).
 *
 * Line targeted: 484.
 *
 * @par MC/DC:
 * Decision: `if (err != k_ra_err_not_found)` at line 483 (1 condition).
 * V1: err=k_ra_err_hw_error -> TRUE  -> line 484.
 * V2: err=k_ra_err_not_found -> FALSE -> continue to creation path (all create tests).
 *
 * @pre Volume is formatted and accessible.
 * @post ra_fs_open returns k_ra_err_hw_error.
 *
...
{
  TEST_BEGIN("ra_fs_fat_file cov: dir_find I/O error propagated at ra_fs_open line 484");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_open(h, "FIND.TXT", k_ra_fs_mode_read, &f));

  h->backend = saved;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fat_file cov: dir_find I/O error propagated at ra_fs_open line 484");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_fs_fat_file_cov.c\n");
  return 0;
}
