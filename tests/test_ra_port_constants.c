...
static void test_pin_encoding_matches_led_constants(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_port_6, RA_PIN_PORT(k_ra_pin_led1));
  TEST_ASSERT_EQ(0, RA_PIN_PIN(k_ra_pin_led1));

  TEST_ASSERT_EQ(k_ra_port_3, RA_PIN_PORT(k_ra_pin_led2));
  TEST_ASSERT_EQ(3, RA_PIN_PIN(k_ra_pin_led2));

  TEST_ASSERT_EQ(k_ra_port_10, RA_PIN_PORT(k_ra_pin_led3));
  TEST_ASSERT_EQ(7, RA_PIN_PIN(k_ra_pin_led3));
...
static void test_ra_pin_macro_roundtrip(void)
{
  ...
  TEST_ASSERT_EQ(2, RA_PIN_PORT(p));
  TEST_ASSERT_EQ(5, RA_PIN_PIN(p));
...
static void test_port_count_invariants(void)
{
  ...
  TEST_ASSERT_EQ(15, k_ra_port_count);
  TEST_ASSERT_EQ(14, k_ra_port_max);
  TEST_ASSERT_EQ(16, k_ra_pin_count);
  TEST_ASSERT_EQ(15, k_ra_pin_max);
