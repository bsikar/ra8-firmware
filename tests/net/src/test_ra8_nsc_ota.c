 ...
 * (2 conditions in ``ra8_nsc_ota_commit()``)
 *
 ...
 * in ``libs/ra8_nsc/src/ra8_nsc_ota.c``. It contains zero compound decisions,
 * so MC/DC is trivially satisfied by any single call. This presence sentinel
 * documents that fact and exercises the call so the decision-free statement
 * coverage row is non-zero.
 *
