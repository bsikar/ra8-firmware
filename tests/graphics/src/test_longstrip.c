 ...
 *
 * The atlas is built with the raw codec (codec 0), so `jof_read_tile`
...
static void t_wt_write_header(uint32_t width, uint32_t height, uint32_t band_h, uint32_t bands)
{
  ...
  t_wt_put_u16(&hdr[k_atlas_off_tile_h], (uint16_t)band_h);
  hdr[k_atlas_off_bpp]   = (uint8_t)k_t_wt_bpp;
  hdr[k_atlas_off_codec] = (uint8_t)k_jof_codec_raw;
 ...
 * @post The returned length is what `jof_parse()` should be given.
 *
 * @note Not thread-safe; the fixture is file-scope state.
 *
 ...
 *          result is validated by `jof_parse()` in the tests, so a
...
static longstrip_decode_ctx_t s_dctx;
...
static void t_wt_open(longstrip_t* wt)
{
  const uint32_t total = t_wt_build_strip();
  s_store              = (jof_memstore_t){.buf = s_atlas, .cap = k_t_atlas_cap, .len = total};

  s_dctx.pread       = jof_memstore_pread;
  ...
  TEST_ASSERT_EQ(k_ra8_ok, jof_parse(jof_memstore_pread, &s_store, total, &s_dctx.info));
  ...
  const longstrip_cfg_t wcfg = {.pread      = jof_memstore_pread,
                                .pread_ctx  = &s_store,
                                .atlas_size = total,
                                .cache      = &s_cache,
                                .image_id   = 1U,
                                ...
                                .blit       = t_wt_blit,
                                .blit_ctx   = &s_blit};
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_open(wt, &wcfg));
}
 ...
 * condition (not compound): `jof_parse` derives tile_cols as
...
static void t_open_validates(void)
{
  ...
  longstrip_cfg_t cfg   = {.pread      = jof_memstore_pread,
                           .pread_ctx  = &s_store,
                           .atlas_size = total,
                           .cache      = &s_cache,
                           .image_id   = 1U,
  ...
  cfg.viewport_w = 0U;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, longstrip_open(&wt, &cfg));
  ...
  /* Null-seam guards. */
  longstrip_cfg_t nn = cfg;
  nn.pread           = nullptr;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, longstrip_open(&wt, &nn));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, longstrip_open(nullptr, &cfg));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, longstrip_open(&wt, nullptr));
 ...
 * ::longstrip_open refuses: a long strip is defined as a single full-width
...
static uint32_t t_wt_build_two_column_atlas(void)
{
  ...
  t_wt_put_u16(&s_atlas[k_atlas_off_tile_h], th);
  s_atlas[k_atlas_off_bpp]   = 1U; /* gray8 (bpp 1) keeps the atlas tiny */
  s_atlas[k_atlas_off_codec] = (uint8_t)k_jof_codec_raw;
 ...
 * `jof_parse` derives tile_cols as ceil(width / tile_w), so
...
static void t_open_rejects_non_band(void)
{
  ...
  s_store                   = (jof_memstore_t){.buf = s_atlas, .cap = k_t_atlas_cap, .len = total};
  const longstrip_cfg_t cfg = {.pread      = jof_memstore_pread,
                               .pread_ctx  = &s_store,
                               .atlas_size = total,
                               .cache      = &s_cache,
                               .image_id   = 2U,
                               ...
                               .blit_ctx   = &s_blit};
  longstrip_t           wt  = {};
  TEST_ASSERT_EQ(k_ra8_err_not_supported, longstrip_open(&wt, &cfg));
...
static void t_geometry(void)
{
  ...
  uint16_t band = k_band_index_poison_wide;
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_band_at_y(&wt, 0U, &band));
  TEST_ASSERT_EQ(0, band);
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_band_at_y(&wt, 63U, &band));
  TEST_ASSERT_EQ(0, band);
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_band_at_y(&wt, 64U, &band));
  TEST_ASSERT_EQ(1, band);
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_band_at_y(&wt, 799U, &band));
  TEST_ASSERT_EQ(12, band); /* last (edge) band */
  TEST_ASSERT_EQ(k_ra8_err_out_of_range, longstrip_band_at_y(&wt, 800U, &band));

  /* clamp: below/above/inside. */
  TEST_ASSERT_EQ(0, longstrip_clamp_scroll(&wt, -100));
  TEST_ASSERT_EQ(k_t_wt_max_scroll, longstrip_clamp_scroll(&wt, 100000));
  TEST_ASSERT_EQ(300, longstrip_clamp_scroll(&wt, 300));

  /* visible range at the top: rows [0,256) -> bands 0..3. */
  uint16_t first = k_band_index_poison;
  uint16_t last  = k_band_index_poison;
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_visible_bands(&wt, 0, &first, &last));
  TEST_ASSERT_EQ(0, first);
  TEST_ASSERT_EQ(3, last);
  /* at the bottom: rows [544,800) -> bands 8..12. */
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_visible_bands(&wt, (int32_t)k_t_wt_max_scroll, &first, &last));
...
static void t_zero_skip(void)
{
  ...
  for (int32_t target = 0; !done; target += step) {
    ...
    (void)longstrip_scroll_by(&wt, t - wt.scroll_y);
    TEST_ASSERT_EQ(t, wt.scroll_y);
    s_blit.scroll_y   = wt.scroll_y;
    s_blit.px_checked = 0U;

    longstrip_render_stats_t st = {};
    TEST_ASSERT_EQ(k_ra8_ok, longstrip_render(&wt, &st));
...
static void t_scroll_by_end_pin(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_fling(&wt, 7));
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_scroll_by(&wt, 300));
  TEST_ASSERT_EQ(300, wt.scroll_y);
  TEST_ASSERT_EQ(7, wt.velocity);

  /* V3: drive to the bottom -> pinned at max_scroll -> velocity zeroed. */
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_fling(&wt, 9));
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_scroll_by(&wt, 100000));
...
static void t_fling(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_fling(&wt, 200));
  int32_t ticks          = 0;
  bool    moved_from_top = false;
  while (longstrip_tick(&wt)) {
    if (wt.scroll_y > 0) {
      moved_from_top = true;
    }
    ticks++;
  ...
  (void)longstrip_scroll_by(&wt, k_scroll_beyond_ends);
  TEST_ASSERT_EQ(k_t_wt_max_scroll, wt.scroll_y);
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_fling(&wt, -200));
  ticks = 0;
  while (longstrip_tick(&wt)) {
...
static void t_prefetch(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_prefetch(&wt, 0U));
  uint32_t miss1 = 0;
  ...
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_fling(&wt, 5)); /* velocity>=0 -> downward */
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_prefetch(&wt, 2U));
  uint32_t miss2 = 0;
  ...
  longstrip_t small = wt;
  small.band_count  = 1U; /* pretend one band that fits: skip branch taken */
  small.scroll_y    = 0;
  small.max_scroll  = 0;
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_prefetch(&small, 2U));
...
static void t_bounded_memory(void)
{
  ...
  for (int32_t target = 0; target <= (int32_t)k_t_wt_max_scroll; target += 32) {
    (void)longstrip_scroll_by(&wt, target - wt.scroll_y);
    s_blit.scroll_y             = wt.scroll_y;
    longstrip_render_stats_t st = {};
    TEST_ASSERT_EQ(k_ra8_ok, longstrip_render(&wt, &st));
    TEST_ASSERT_EQ(0, st.skipped);
  ...
  (void)longstrip_scroll_by(&wt, -(int32_t)k_scroll_beyond_ends);
  s_blit.scroll_y             = wt.scroll_y;
  longstrip_render_stats_t st = {};
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_render(&wt, &st));
...
static void t_set_viewport_validates(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, longstrip_set_viewport(&wt, k_t_wt_view_w, (uint16_t)k_t_wt_view_h));
  TEST_ASSERT_EQ(k_t_wt_max_scroll, wt.max_scroll);
  /* Vector 2: zero width only. Vector 3: zero height only. */
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, longstrip_set_viewport(&wt, 0U, k_t_wt_view_h));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, longstrip_set_viewport(&wt, k_t_wt_view_w, 0U));
  /* A rejected call leaves the previous viewport and clamp untouched. */
  TEST_ASSERT_EQ(k_t_wt_max_scroll, wt.max_scroll);
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, longstrip_set_viewport(nullptr, 1U, 1U));
...
static void t_paginated_last_page_is_not_clamped(void)
{
  ...
  for (uint32_t p = 0U; p < pages; p++) {
    ...
    TEST_ASSERT_EQ(k_ra8_ok, longstrip_set_viewport(&wt, k_t_wt_view_w, (uint16_t)content));
    TEST_ASSERT_EQ(k_ra8_ok, longstrip_scroll_by(&wt, (int32_t)top - wt.scroll_y));
