 ...
 * fixtures live in support/inc/vin_test_util.h.
 *
...
/**
 * @brief A stuck AXI OUTSTAND flag surfaces as a bounded timeout.
 * @par MC/DC:
 * No compound decision is exercised in
 * ``libs/ra8_hal/src/ra8_vin.c@ra8_vin_capture_disarm``: the initialized
 * fixture takes its single-condition active-state guard, then the injected
 * wait failure drives the bounded AXI-drain timeout.
 * @since 0.1.0
 */
static void test_capture_stop_timeout(void)
{
  TEST_BEGIN("vin capture_disarm stuck AXI drain times out");
  prep();
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(mtc_reg));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_vin_capture_stop());
  ra8_fake_mmio_reset();
  TEST_END("vin capture_disarm stuck AXI drain times out");
}
