...
/**
 * @enum ra_i3c_i2c_test_addr_t
 * @brief Test addresses and payload constants.
 */
typedef enum : uint8_t {
  k_ra_i3c_i2c_test_target  = 0x50U,
  k_ra_i3c_i2c_test_byte_a  = 0xA5U,
  k_ra_i3c_i2c_test_byte_b  = 0x5AU,
  k_ra_i3c_i2c_test_byte_c  = 0x33U,
  k_ra_i3c_i2c_test_rx_byte = 0xC3U,
} ra_i3c_i2c_test_addr_t;

/**
 * @enum ra_i3c_i2c_test_ch_t
 * @brief Channel constants used by tests.
 */
typedef enum : uint8_t {
  k_ra_i3c_i2c_test_ch_zero = 0U,
  k_ra_i3c_i2c_test_ch_oor  = 1U, /**< IIC_B has only ch 0 on RA8D2. */
  k_ra_i3c_i2c_test_ch_huge = 200U,
} ra_i3c_i2c_test_ch_t;
...
/**
 * @enum ra_i3c_i2c_test_addr_byte_t
 * @brief Pre-shifted address bytes for write / read transactions.
 */
typedef enum : uint8_t {
  k_ra_i3c_i2c_test_addr_w = (uint8_t)(k_ra_i3c_i2c_test_target << 1U),       /**< 0xA0. */
  k_ra_i3c_i2c_test_addr_r = (uint8_t)((k_ra_i3c_i2c_test_target << 1U) | 1U) /**< 0xA1. */
} ra_i3c_i2c_test_addr_byte_t;

static const uint8_t s_payload[2] = {
  (uint8_t)k_ra_i3c_i2c_test_byte_a,
  (uint8_t)k_ra_i3c_i2c_test_byte_b,
};

static uint8_t s_long_buffer[1000000];

static const ra_i3c_i2c_cfg_t k_iic_b_cfg = {
...
static void test_init_configured(void)
{
  ...
  TEST_ASSERT((reg->BCTL & (uint32_t)k_ra_i3c_i2c_msk_bctl_buse) != 0U);
...
static void test_init_bad_inputs(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_null_ptr, internal_i3c_i2c_init(0U, nullptr));
  ra_i3c_i2c_cfg_t bad = k_iic_b_cfg;
  bad.bus_hz           = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, internal_i3c_i2c_init(0U, &bad));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 internal_i3c_i2c_init((uint8_t)k_ra_i3c_i2c_test_ch_oor, &k_iic_b_cfg));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 internal_i3c_i2c_init((uint8_t)k_ra_i3c_i2c_test_ch_huge, &k_iic_b_cfg));
...
static void test_deinit_releases(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_ok, internal_i3c_i2c_init(0U, &k_iic_b_cfg));
  TEST_ASSERT_EQ(k_ra_ok, internal_i3c_i2c_deinit(0U));
  bool stopped = false;
  TEST_ASSERT_EQ(k_ra_ok, ra_mstp_is_stopped(k_ra_mstp_i3c, &stopped));
  TEST_ASSERT(stopped);
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, internal_i3c_i2c_deinit((uint8_t)k_ra_i3c_i2c_test_ch_oor));
...
static void test_set_clock_updates(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 internal_i3c_i2c_set_clock(0U, (uint32_t)k_ra_i3c_i2c_speed_standard, 60000000U));
  TEST_ASSERT(i3c_i2c_regs(0U)->STDBR != before);
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, internal_i3c_i2c_set_clock(0U, 0U, 60000000U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, internal_i3c_i2c_set_clock(0U, 100000U, 0U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 internal_i3c_i2c_set_clock((uint8_t)k_ra_i3c_i2c_test_ch_oor, 100000U, 60000000U));
...
static void test_write_happy(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    internal_i3c_i2c_write(0U, (uint8_t)k_ra_i3c_i2c_test_target, s_payload, 2U, false));
   ...
   * transaction). */
  volatile const r_i3c_i2c_regs_t* reg = i3c_i2c_regs(0U);
  TEST_ASSERT_EQ(k_ra_i3c_i2c_test_byte_b, (reg->NTDTBP0 & 0xFFU));
...
static void test_write_zero_length(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    internal_i3c_i2c_write(0U, (uint8_t)k_ra_i3c_i2c_test_target, s_payload, 0U, false));
...
static void test_write_null_data(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 internal_i3c_i2c_write(0U, (uint8_t)k_ra_i3c_i2c_test_target, nullptr, 1U, false));
...
static void test_write_bad_channel(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 internal_i3c_i2c_write((uint8_t)k_ra_i3c_i2c_test_ch_oor,
...
static void test_write_timeout_on_start(void)
{
  ...
  i3c_i2c_regs(0U)->BCST = (uint32_t)k_ra_i3c_i2c_msk_bcst_bfref;
  TEST_ASSERT_EQ(
    k_ra_err_hw_timeout,
    internal_i3c_i2c_write(0U, (uint8_t)k_ra_i3c_i2c_test_target, s_payload, 1U, false));
...
{
  TEST_BEGIN("internal_i3c_i2c_write: BCST.BFREF=0 => k_ra_err_busy");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, internal_i3c_i2c_init(0U, &k_iic_b_cfg));
  /* BFREF clear == bus busy. NTST primed so the call would otherwise
   * proceed -- the busy gate must fire first. */
  i3c_i2c_regs(0U)->NTST =
    (uint32_t)k_ra_i3c_i2c_msk_ntst_tdbef0 | (uint32_t)k_ra_i3c_i2c_msk_ntst_rdbff0;
...
{
  TEST_BEGIN("internal_i3c_i2c_write: NACKDF latched mid-transfer => k_ra_err_nack + STOP");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, internal_i3c_i2c_init(0U, &k_iic_b_cfg));
  prime_ntst(0U);
  ...
  TEST_END("internal_i3c_i2c_write: NACKDF latched mid-transfer => k_ra_err_nack + STOP");
}
...
static void test_write_restart_holds_bus(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    internal_i3c_i2c_write(0U, (uint8_t)k_ra_i3c_i2c_test_target, s_payload, 1U, true));
  /* CNDCTL last write should be START (no subsequent STOP). */
  TEST_ASSERT_EQ(k_ra_i3c_i2c_msk_cndctl_stcnd, i3c_i2c_regs(0U)->CNDCTL);
...
static void test_read_happy(void)
{
  ...
  uint8_t buf[2] = {0U, 0U};
  TEST_ASSERT_EQ(k_ra_ok,
                 internal_i3c_i2c_read(0U, (uint8_t)k_ra_i3c_i2c_test_target, buf, 2U, false));
  ...
  TEST_ASSERT_EQ(k_ra_i3c_i2c_msk_ackctl_acktwp, (i3c_i2c_regs(0U)->ACKCTL));
  TEST_ASSERT_EQ(k_ra_i3c_i2c_msk_cndctl_spcnd, i3c_i2c_regs(0U)->CNDCTL);
...
static void test_read_zero_length_rejected(void)
{
  ...
  uint8_t buf = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 internal_i3c_i2c_read(0U, (uint8_t)k_ra_i3c_i2c_test_target, &buf, 0U, false));
...
static void test_read_null_out(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, internal_i3c_i2c_init(0U, &k_iic_b_cfg));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 internal_i3c_i2c_read(0U, (uint8_t)k_ra_i3c_i2c_test_target, nullptr, 1U, false));
...
static void test_read_bad_channel(void)
{
  ...
  uint8_t buf = 0U;
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 internal_i3c_i2c_read((uint8_t)k_ra_i3c_i2c_test_ch_oor,
                                       (uint8_t)k_ra_i3c_i2c_test_target,
                                       &buf,
                                       1U,
                                       false));
...
static void test_read_timeout_on_start(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_timeout,
                 internal_i3c_i2c_read(0U, (uint8_t)k_ra_i3c_i2c_test_target, &buf, 1U, false));
...
{
  TEST_BEGIN("internal_i3c_i2c_read: BCST.BFREF=0 => k_ra_err_busy");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, internal_i3c_i2c_init(0U, &k_iic_b_cfg));
  i3c_i2c_regs(0U)->NTST =
    (uint32_t)k_ra_i3c_i2c_msk_ntst_tdbef0 | (uint32_t)k_ra_i3c_i2c_msk_ntst_rdbff0;
  ...
  TEST_ASSERT_EQ(k_ra_err_busy,
                 internal_i3c_i2c_read(0U, (uint8_t)k_ra_i3c_i2c_test_target, &buf, 1U, false));
  TEST_END("internal_i3c_i2c_read: BCST.BFREF=0 => k_ra_err_busy");
}
...
static void test_transfer_happy(void)
{
  ...
  prime_ntst(0U);
  uint8_t       rx     = 0U;
  const uint8_t tx[1U] = {(uint8_t)k_ra_i3c_i2c_test_byte_c};
  TEST_ASSERT_EQ(k_ra_ok,
                 internal_i3c_i2c_transfer(0U, (uint8_t)k_ra_i3c_i2c_test_target, tx, 1U, &rx, 1U));
...
static void test_transfer_null_args_rejected(void)
{
  ...
  TEST_ASSERT_EQ(
    ...
    internal_i3c_i2c_transfer(0U, (uint8_t)k_ra_i3c_i2c_test_target, nullptr, 0U, nullptr, 0U));
  /* tx_len > 0 with NULL tx -> null_ptr. */
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    internal_i3c_i2c_transfer(0U, (uint8_t)k_ra_i3c_i2c_test_target, nullptr, 1U, &rx, 1U));
  /* rx_len > 0 with NULL rx -> null_ptr. */
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    internal_i3c_i2c_transfer(0U, (uint8_t)k_ra_i3c_i2c_test_target, s_payload, 1U, nullptr, 1U));
  /* Out-of-range channel -> null_ptr (matches the rest of the API). */
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 internal_i3c_i2c_transfer((uint8_t)k_ra_i3c_i2c_test_ch_oor,
                                           (uint8_t)k_ra_i3c_i2c_test_target,
                                           s_payload,
                                           1U,
                                           &rx,
...
{
  TEST_BEGIN("internal_i3c_i2c_transfer: bus-busy => k_ra_err_busy");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, internal_i3c_i2c_init(0U, &k_iic_b_cfg));
  i3c_i2c_regs(0U)->NTST =
    (uint32_t)k_ra_i3c_i2c_msk_ntst_tdbef0 | (uint32_t)k_ra_i3c_i2c_msk_ntst_rdbff0;
  ...
  TEST_ASSERT_EQ(
    k_ra_err_busy,
    internal_i3c_i2c_transfer(0U, (uint8_t)k_ra_i3c_i2c_test_target, s_payload, 1U, &rx, 1U));
  TEST_END("internal_i3c_i2c_transfer: bus-busy => k_ra_err_busy");
}
...
static void test_abort_resets_channel(void)
{
   ...
   * latched flags. */
  i3c_i2c_regs(0U)->BIE  = (uint32_t)k_ra_i3c_i2c_msk_bie_nackdie;
  i3c_i2c_regs(0U)->NTIE = (uint32_t)k_ra_i3c_i2c_msk_ntie_tdbeie0;
  i3c_i2c_regs(0U)->BST =
    (uint32_t)k_ra_i3c_i2c_msk_bst_nackdf | (uint32_t)k_ra_i3c_i2c_msk_bst_alf;

  TEST_ASSERT_EQ(k_ra_ok, internal_i3c_i2c_abort(0U));
  TEST_ASSERT_EQ(0, i3c_i2c_regs(0U)->BIE);
  TEST_ASSERT_EQ(0, i3c_i2c_regs(0U)->NTIE);
  TEST_ASSERT_EQ(k_ra_i3c_i2c_msk_cndctl_spcnd, i3c_i2c_regs(0U)->CNDCTL);
  /* NACK / AL flags should be W0C-cleared. */
  TEST_ASSERT((i3c_i2c_regs(0U)->BST & (uint32_t)k_ra_i3c_i2c_msk_bst_nackdf) == 0U);
  TEST_ASSERT((i3c_i2c_regs(0U)->BST & (uint32_t)k_ra_i3c_i2c_msk_bst_alf) == 0U);

  TEST_ASSERT_EQ(k_ra_err_invalid_arg, internal_i3c_i2c_abort((uint8_t)k_ra_i3c_i2c_test_ch_oor));
...
static void test_scan_no_response(void)
{
  ...
  bool acked = true;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout,
                 internal_i3c_i2c_scan(0U, (uint8_t)k_ra_i3c_i2c_test_target, &acked));
  TEST_ASSERT(!acked);
...
static void test_scan_bad_args(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, internal_i3c_i2c_init(0U, &k_iic_b_cfg));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 internal_i3c_i2c_scan(0U, (uint8_t)k_ra_i3c_i2c_test_target, nullptr));
  bool acked = false;
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 internal_i3c_i2c_scan((uint8_t)k_ra_i3c_i2c_test_ch_oor,
                                       (uint8_t)k_ra_i3c_i2c_test_target,
                                       &acked));
...
static void test_errors_mask_and_clear(void)
{
  ...
  reg->BST = (uint32_t)k_ra_i3c_i2c_msk_bst_alf | (uint32_t)k_ra_i3c_i2c_msk_bst_nackdf |
             (uint32_t)k_ra_i3c_i2c_msk_bst_todf;

  uint8_t mask = 0U;
  TEST_ASSERT_EQ(k_ra_ok, internal_i3c_i2c_get_errors(0U, &mask));
  TEST_ASSERT_EQ(((uint8_t)k_ra_i3c_i2c_err_arb_lost | (uint8_t)k_ra_i3c_i2c_err_nack |
                  (uint8_t)k_ra_i3c_i2c_err_timeout),
                 mask);

  TEST_ASSERT_EQ(k_ra_ok, internal_i3c_i2c_clear_errors(0U));
  TEST_ASSERT_EQ(k_ra_ok, internal_i3c_i2c_get_errors(0U, &mask));
  TEST_ASSERT_EQ(k_ra_i3c_i2c_err_none, mask);

  TEST_ASSERT_EQ(k_ra_err_null_ptr, internal_i3c_i2c_get_errors(0U, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 internal_i3c_i2c_get_errors((uint8_t)k_ra_i3c_i2c_test_ch_oor, &mask));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 internal_i3c_i2c_clear_errors((uint8_t)k_ra_i3c_i2c_test_ch_oor));
...
static void test_attach_handler_toggles_iers(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, internal_i3c_i2c_attach_handler(0U, stub_iic_b_cb, nullptr));
  TEST_ASSERT(i3c_i2c_regs(0U)->BIE != 0U);
  TEST_ASSERT(i3c_i2c_regs(0U)->NTIE != 0U);

  TEST_ASSERT_EQ(k_ra_ok, internal_i3c_i2c_attach_handler(0U, nullptr, nullptr));
  TEST_ASSERT_EQ(0, i3c_i2c_regs(0U)->BIE);
  TEST_ASSERT_EQ(0, i3c_i2c_regs(0U)->NTIE);

  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    internal_i3c_i2c_attach_handler((uint8_t)k_ra_i3c_i2c_test_ch_oor, stub_iic_b_cb, nullptr));
...
static void test_dispatch_eri_fires_callback(void)
{
  ...
  s_iic_b_cb_count      = 0;
  s_iic_b_cb_err        = 0;
  i3c_i2c_regs(0U)->BST = (uint32_t)k_ra_i3c_i2c_msk_bst_nackdf;
  internal_i3c_i2c_dispatch_eri(0U);
  TEST_ASSERT_EQ(1, s_iic_b_cb_count);
  TEST_ASSERT_EQ(k_ra_i3c_i2c_err_nack, s_iic_b_cb_err);
  ...
  internal_i3c_i2c_dispatch_eri((uint8_t)k_ra_i3c_i2c_test_ch_oor);
...
static void test_mcdc_iic_b(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    internal_i3c_i2c_transfer(0U, (uint8_t)k_ra_i3c_i2c_test_target, tx_buf, 0U, rx_buf, 0U));

  /* Decision B V3: tx_len!=0 with NULL tx -> null_ptr. */
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    internal_i3c_i2c_transfer(0U, (uint8_t)k_ra_i3c_i2c_test_target, nullptr, 1U, rx_buf, 0U));

  /* Decision C V3: rx_len!=0 with NULL rx -> null_ptr. tx_len=0 to
   * keep decision A=F (because rx_len!=0) and decision B=F (tx_len=0). */
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    internal_i3c_i2c_transfer(0U, (uint8_t)k_ra_i3c_i2c_test_target, tx_buf, 0U, nullptr, 1U));
 ...
 *
 * @par MC/DC:
 * Decision at libs/ra_hal/src/ra_i3c_i2c.c (call site) -> helper at
 * libs/ra_hal/src/ra_i3c_i2c.c:
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_i3c_i2c.c\n");
  return 0;
}
