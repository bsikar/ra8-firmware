/**
 * @file test_ra_bkup.c
 * @brief Unit tests for ra_bkup.c (Battery Backup Function driver)
 *
...
/**
 * @enum ra_bkup_test_const_t
 * @brief Local numeric constants for the tests.
 */
typedef enum : uint32_t {
  k_ra_bkup_test_word_first   = 0U,      /**< First VBTBKR word slot.       */
  k_ra_bkup_test_word_mid     = 7U,      /**< Mid-array word slot.          */
  k_ra_bkup_test_word_last    = 31U,     /**< Last legal VBTBKR word slot.  */
  k_ra_bkup_test_word_bad     = 32U,     /**< First invalid word slot.      */
  k_ra_bkup_test_word_huge    = 200U,    /**< Way out of range.             */
  k_ra_bkup_test_bad_level    = 6U,      /**< 110b -- "setting prohibited". */
  k_ra_bkup_test_byte_first   = 0U,      /**< First VBTBKR byte slot.       */
  k_ra_bkup_test_byte_mid     = 64U,     /**< Mid-array byte slot.          */
  k_ra_bkup_test_byte_last    = 127U,    /**< Last legal byte slot.         */
  k_ra_bkup_test_byte_bad     = 128U,    /**< First invalid byte slot.      */
  k_ra_bkup_test_byte_huge    = 999U,    /**< Way out of range.             */
  k_ra_bkup_test_chan_bad     = 3U,      /**< First invalid channel index.  */
  k_ra_bkup_test_nc_width_bad = 8U,      /**< First invalid VINCW encoding. */
  k_ra_bkup_test_timeout      = 32U,     /**< Reasonable poll budget.       */
  k_ra_bkup_test_saba_aligned = 0x40U,   /**< 32-byte aligned (low bits 0). */
  k_ra_bkup_test_saba_unalign = 0x41U,   /**< Unaligned -- bit 0 set.       */
  k_ra_bkup_test_saba_max_ok  = 0xFFE0U, /**< saba_max boundary.            */
} ra_bkup_test_const_t;

/**
 * @enum ra_bkup_test_words_t
...
/**
 * @enum ra_bkup_test_byte_t
 * @brief 8-bit byte test patterns.
 */
typedef enum : uint8_t {
  k_ra_bkup_test_byte_pat_a = 0x5AU, /**< Byte marker A. */
  k_ra_bkup_test_byte_pat_b = 0xA5U, /**< Byte marker B. */
} ra_bkup_test_byte_t;
...
static ra_bkup_config_t make_cfg(void)
{
  const ra_bkup_config_t cfg = {
    .vdet_level    = k_ra_bkup_vdet_2p10v,
    .enable_switch = true,
    .enable_backup = true,
  };
  return cfg;
...
make_chan_cfg(bool ien, bool ie, bool ce, bool ze, ra_bkup_edge_t edge)
{
  const ra_bkup_tamper_chan_cfg_t cfg = {
    ...
    .clear_backup       = ce,
    .zeroize_huk        = ze,
    .capture_src        = k_ra_bkup_capture_src_vbtadf,
  };
  return cfg;
}

static ra_bkup_tamper_config_t make_tamper_cfg(void)
{
  const ra_bkup_tamper_config_t cfg = {
...
static void prep(void)
{
  ...
  (void)ra_bkup_attach_handler(nullptr, nullptr);
}
...
static void test_cold_start_init_happy(void)
{
  ...
  *ra_bkup_vbtbpsr() =
  ...
  TEST_ASSERT((*ra_bkup_vbtbpsr() & (uint8_t)k_ra_bkup_vbtbpsr_mask_vbporm) != 0U);
...
static void test_get_status_vcc(void)
{
  ...
  *ra_bkup_vbtbpsr() =
    (uint8_t)((uint8_t)k_ra_bkup_vbtbpsr_mask_swm | (uint8_t)k_ra_bkup_vbtbpsr_mask_vbporm);
  *ra_bkup_vbtadsr() = 0U;
...
static void test_get_status_vbatt_with_por(void)
{
  ...
  *ra_bkup_vbtbpsr() = (uint8_t)k_ra_bkup_vbtbpsr_mask_vbporf;
...
static void test_clear_status(void)
{
  ...
  *ra_bkup_vbtadsr() = (uint8_t)k_ra_bkup_vbtadsr_mask_all;
  ...
  TEST_ASSERT((bpsr_after & (uint8_t)k_ra_bkup_vbtbpsr_mask_vbporf) == 0U);
  TEST_ASSERT((bpsr_after & (uint8_t)k_ra_bkup_vbtbpsr_mask_swm) != 0U);
  /* Only VBTADF0 cleared. */
  const uint8_t adsr_after = *ra_bkup_vbtadsr();
  TEST_ASSERT((adsr_after & (uint8_t)k_ra_bkup_vbtadsr_mask_vbtadf0) == 0U);
  TEST_ASSERT((adsr_after & (uint8_t)k_ra_bkup_vbtadsr_mask_vbtadf1) != 0U);
  TEST_ASSERT((adsr_after & (uint8_t)k_ra_bkup_vbtadsr_mask_vbtadf2) != 0U);
...
static void test_tamper_init_happy(void)
{
  ...
  *ra_bkup_vbtadsr() = (uint8_t)k_ra_bkup_vbtadsr_mask_all;
  ...
  const uint8_t expected_adcr1 = (uint8_t)((uint8_t)k_ra_bkup_vbtadcr1_mask_vbtadie0 |
...
static void test_tamper_disable(void)
{
  ...
  *ra_bkup_vbtictlr()  = (uint8_t)k_ra_bkup_vbtictlr_mask_all;
  *ra_bkup_vbtictlr2() = 0xFFU;
  *ra_bkup_vbtadsr()   = (uint8_t)k_ra_bkup_vbtadsr_mask_all;
  *ra_bkup_vbtadcr1()  = 0xFFU;
...
static void test_security_apply_get(void)
{
  ...
  const ra_bkup_security_config_t in = {
    .bbfsar = (uint32_t)k_ra_bkup_test_bbfsar_value,
    .saba   = (uint16_t)k_ra_bkup_test_saba_aligned,
    .pabas  = 0U,
...
static void test_security_bad_args(void)
{
  ...
  ra_bkup_security_config_t cfg = {
    .bbfsar = (uint32_t)k_ra_bkup_test_bbfsar_bad,
    .saba   = (uint16_t)k_ra_bkup_test_saba_aligned,
    .pabas  = 0U,
    .pabans = 0U,
  };
...
static void test_isr_handle_armed_and_fired(void)
{
  ...
  *ra_bkup_vbtadcr1() = (uint8_t)((uint8_t)k_ra_bkup_vbtadcr1_mask_vbtadie0 |
                                  (uint8_t)k_ra_bkup_vbtadcr1_mask_vbtadie2);
  *ra_bkup_vbtadsr() =
...
static void test_all_vdet_levels(void)
{
  ...
  prep();
  const ra_bkup_vdet_level_t levels[] = {
    k_ra_bkup_vdet_2p80v,
    k_ra_bkup_vdet_2p53v,
    k_ra_bkup_vdet_2p10v,
    k_ra_bkup_vdet_1p95v,
    k_ra_bkup_vdet_1p85v,
    k_ra_bkup_vdet_1p75v,
  };
