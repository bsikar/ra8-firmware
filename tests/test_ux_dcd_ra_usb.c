/**
 * @file test_ux_dcd_ra_usb.c
 * @brief MC/DC vector tests for port/usbx/ux_dcd_ra_usb.c
 *
 * @details
 * The USBX device-controller-driver bridge ``port/usbx/ux_dcd_ra_usb.c``
 ...
 * ``port/usbx/ux_dcd_ra_usb.c`` as a ``static inline`` helper using
...
{
  enum : uint8_t {
    k_test_ux_max_pipes = 10U, /**< Mirrors k_ux_dcd_ra_usb_max_pipes. */
  };
...
{
  TEST_BEGIN("ux_dcd_ra_usb MC/DC: tr || ep null-guard (line 156)");
  ...
  TEST_END("ux_dcd_ra_usb MC/DC: tr || ep null-guard (line 156)");
}
...
{
  TEST_BEGIN("ux_dcd_ra_usb MC/DC: in_len && data ptr guard (line 173)");
  uint8_t     payload[8] = {};
  ...
  TEST_END("ux_dcd_ra_usb MC/DC: in_len && data ptr guard (line 173)");
}
...
{
  TEST_BEGIN("ux_dcd_ra_usb MC/DC: ep_create pipe range (line 215)");
  ...
  TEST_END("ux_dcd_ra_usb MC/DC: ep_create pipe range (line 215)");
}
