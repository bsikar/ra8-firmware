/**
 * @file test_ra_stbtt_alloc.c
 * @brief Unit tests for libs/ra_reflow/src/ra_stbtt_alloc.c
 *
 * @details
...
#include "ra_stbtt_alloc.h"
#include "unity_minimal.h"

enum {
  k_align       = 16,        /**< Mirror of k_ra_stbtt_align.       */
  k_arena_bytes = 96 * 1024, /**< Mirror of k_ra_stbtt_arena_bytes. */
};
 ...
 *
 * @par MC/DC:
 * Decision: ``if (s_offset > s_high_water)`` in ra_stbtt_malloc
 * (1 condition, libs/ra_reflow/src/ra_stbtt_alloc.c@ra_stbtt_malloc).
...
{
  TEST_BEGIN("ra_stbtt_alloc: alignment + high-water");
  uint8_t* a = (uint8_t*)ra_stbtt_malloc(1U);
  uint8_t* b = (uint8_t*)ra_stbtt_malloc(1U);
  ...
  ra_stbtt_free(a);
  ra_stbtt_free(b);
  TEST_ASSERT_EQ((2 * k_align), ra_stbtt_alloc_high_water());
  TEST_END("ra_stbtt_alloc: alignment + high-water");
}
 ...
 * for callers (ra_reflow, ra_epub) that never call an explicit reset.
 *
 * @par MC/DC:
 * Decision: ``if (s_live == 0U)`` in ra_stbtt_free
 * (1 condition, libs/ra_reflow/src/ra_stbtt_alloc.c@ra_stbtt_free).
...
{
  TEST_BEGIN("ra_stbtt_alloc: full-drain auto-reset");
  uintptr_t base = (uintptr_t)ra_stbtt_malloc(64U);
  uint8_t*  p2   = (uint8_t*)ra_stbtt_malloc(64U);
  TEST_ASSERT_NOT_NULL((void*)base);
  TEST_ASSERT_NOT_NULL(p2);
  ra_stbtt_free((void*)base);
  ra_stbtt_free(p2);
  uintptr_t again = (uintptr_t)ra_stbtt_malloc(64U);
  TEST_ASSERT_EQ(base, again);
  ra_stbtt_free((void*)again);
  TEST_END("ra_stbtt_alloc: full-drain auto-reset");
}
...
{
  TEST_BEGIN("ra_stbtt_alloc: partial free does not rewind");
  uint8_t* a = (uint8_t*)ra_stbtt_malloc(64U);
  uint8_t* b = (uint8_t*)ra_stbtt_malloc(64U);
  TEST_ASSERT_NOT_NULL(a);
  TEST_ASSERT_NOT_NULL(b);
  ra_stbtt_free(a); /* b still live */
  uint8_t* c = (uint8_t*)ra_stbtt_malloc(64U);
  TEST_ASSERT_NOT_NULL(c);
  TEST_ASSERT((uintptr_t)c != (uintptr_t)a);
  TEST_ASSERT((uintptr_t)c != (uintptr_t)b);
  ra_stbtt_free(b);
  ra_stbtt_free(c); /* drains -> reset */
  TEST_END("ra_stbtt_alloc: partial free does not rewind");
}
 ...
 *
 * @par MC/DC:
 * Decisions in ra_stbtt_malloc
 * (libs/ra_reflow/src/ra_stbtt_alloc.c@ra_stbtt_malloc), each 1 condition:
 *  - ``if (n > k_ra_stbtt_arena_bytes)``: V-T request exceeds capacity
 *    outright -> nullptr; V-F normal requests elsewhere proceed.
 *  - ``if (aligned > k_ra_stbtt_arena_bytes - s_offset)``: V-T the
...
{
  TEST_BEGIN("ra_stbtt_alloc: exhaustion -> nullptr");
  ...
  ra_stbtt_free(small);
  TEST_END("ra_stbtt_alloc: exhaustion -> nullptr");
}
 ...
 *
 * @par MC/DC:
 * Decision: ``if (p == nullptr)`` in ra_stbtt_free
 * (1 condition, libs/ra_reflow/src/ra_stbtt_alloc.c@ra_stbtt_free).
...
{
  TEST_BEGIN("ra_stbtt_alloc: free(nullptr) is a no-op");
  ra_stbtt_free(nullptr);
  ra_stbtt_free(nullptr);
  uintptr_t base = (uintptr_t)ra_stbtt_malloc(32U);
  ra_stbtt_free((void*)base);
  uintptr_t again = (uintptr_t)ra_stbtt_malloc(32U);
  TEST_ASSERT_EQ(base, again);
  ra_stbtt_free((void*)again);
  TEST_END("ra_stbtt_alloc: free(nullptr) is a no-op");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_stbtt_alloc.c\n");
  return 0;
}
