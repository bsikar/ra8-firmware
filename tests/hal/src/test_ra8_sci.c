...
typedef enum : uint8_t {
  ...
  k_t_ref_poison  = 0xFFU, /**< Poison written into a module-stop refcount
                                out-parameter before the call, so a query that
...
/**
 * @par MC/DC:
 * Decision: `if (mst_err != k_ra8_ok)` in `ra8_sci_init()` (1 condition)
 * - Vector 1: MSTPCRB read-back settles -> `ra8_mstp_enable()` returns
 *   k_ra8_ok -> the decision is false and bring-up continues (control;
 *   covered by test_init_sets_ccr0_enables above).
 * - Vector 2: MSTPCRB armed to never satisfy its bounded read-back ->
 *   `ra8_mstp_enable()` returns k_ra8_err_hw_timeout -> the decision is true
 *   and init reports k_ra8_err_hw_init_failed (this case).
 * N = 1 condition, N+1 = 2 vectors. Vectors 1+2 differ only in the module-stop
 * read-back outcome and produce opposite outcomes, so `mst_err` independently
 * affects the decision.
 */
static void test_sci_init_mstp_enable_timeout(void)
{
  TEST_BEGIN("ra8_sci_init: MSTP enable read-back timeout");
  prep();

  /* SCI5 is MSTPB26, so ra8_mstp_enable() polls MSTPCRB. Arm the seam AFTER
   * prep(): ra8_mstp_init() polls the same register and would fail first. */
  /* HUM Ch 11.2.7 "MSTPCRB : Module Stop Control Register B", p 444 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait((const volatile void*)&ra8_mstp()->MSTPCRB));
  TEST_ASSERT_EQ(k_ra8_err_hw_init_failed, ra8_sci_init(k_t_channel, &k_cfg));

  /* The rejected enable rolled its module-stop reference back, so a retry
     starts from a clean count rather than believing SCI5 is already on. */
  uint8_t ref = k_t_ref_poison;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_get_refcount(k_ra8_mstp_sci5, &ref));
  TEST_ASSERT_EQ(0U, ref);

  /* Disarming the seam lets the very same call succeed, proving the rejection
   * came from the read-back and not from the channel or the config. */
  ra8_fake_mmio_reset();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_sci_init(k_t_channel, &k_cfg));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_get_refcount(k_ra8_mstp_sci5, &ref));
  TEST_ASSERT_EQ(1U, ref);
  TEST_END("ra8_sci_init: MSTP enable read-back timeout");
}
...
int main(void)
{
  ...
  test_sci_init_mstp_enable_timeout();
  return 0;
}
