 ...
 * @since 0.1.0
 */
static void test_wdt_clear_status_blocking_timeout(void)
{
  TEST_BEGIN("ra8_wdt_clear_status_blocking stuck status times out");
  ra8_fake_mmap_reset();
  ra8_fake_mmio_reset();
  volatile r_wdt_regs_t* reg = ra8_wdt();
  /* HUM Ch 27.2.3 "WDTSR : WDT Status Register" p 1260 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&reg->WDTSR));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout,
                 ra8_wdt_clear_status_blocking((uint16_t)k_ra8_wdt_status_underflow));
  ra8_fake_mmio_reset();
  TEST_END("ra8_wdt_clear_status_blocking stuck status times out");
}
...
static void (*const s_test_roster[])(void) = {
  ...
  test_wdt_clear_status_blocking_timeout,
