...
/** @brief A stuck RSTCTL reset bit makes initialization time out. @since 0.1.0 */
RA8_INTERNAL static void internal_test_init_reset_timeout(void)
{
  TEST_BEGIN("ra8_i3c_i2c_init: reset timeout");
  internal_prep();
  volatile r_i3c_i2c_regs_t* reg = i3c_i2c_regs(0U);
  /* HUM Ch 40.2.4 "RSTCTL : Reset Control Register" p 2451 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&reg->RSTCTL));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_i3c_i2c_init(0U, &s_iic_b_cfg));
  ra8_fake_mmio_reset();
  TEST_END("ra8_i3c_i2c_init: reset timeout");
}

/**
 * @test internal_test_init_mstp_timeouts
 * @brief Prove IIC_B initialization propagates both MSTP timeouts.
 * @details Both I3C and IIC0 live in MSTPCRB. The existing nth-wait seam lets
 * either read-back time out without changing production behavior.
 * @par MC/DC:
 * The block bring-up has one ``RA8_RETURN_ON_ERROR`` decision per ungate.
 * - Vector 1: first I3C wait times out -> first decision true.
 * - Vector 2: first wait settles, second IIC0 wait times out -> first false,
 *   second true.
 * - Vector 3: both waits settle -> both decisions false.
 * Each single-condition decision takes both outcomes.
 * @pre The channel-zero register window is mapped.
 * @pre The MSTP reference table is reset before each vector.
 * @post Both armed calls return ::k_ra8_err_hw_timeout unchanged.
 * @post The disarmed control initializes and deinitializes successfully.
 * @note File-local, single-threaded host fixture.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_test_init_mstp_timeouts(void)
{
  TEST_BEGIN("ra8_i3c_i2c_init: propagates both MSTP timeouts");
  internal_prep();
  /* HUM Ch 11.2.7 "MSTPCRB : Module Stop Control Register B", p 444 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_nth_wait(&ra8_mstp()->MSTPCRB, 0U));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_i3c_i2c_init(0U, &s_iic_b_cfg));

  internal_prep();
  /* HUM Ch 11.2.7 "MSTPCRB : Module Stop Control Register B" p 444 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_nth_wait(&ra8_mstp()->MSTPCRB, 1U));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_i3c_i2c_init(0U, &s_iic_b_cfg));

  internal_prep();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_i3c_i2c_init(0U, &s_iic_b_cfg));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_i3c_i2c_deinit(0U));
  TEST_END("ra8_i3c_i2c_init: propagates both MSTP timeouts");
}
...
static void (*const s_test_roster[])(void) = {
  ...
  internal_test_init_mstp_timeouts,
  internal_test_init_reset_timeout,
