...
/**
 * @par MC/DC:
 * (no compound decisions in this test -- the GRAMINIT poll has one
 * condition; this case holds it true through the bounded loop while the
 * ordinary init cases exercise its false/exit direction)
 */
static void test_init_channel0_ram_init_poll_budget(void)
{
  TEST_BEGIN("canfd init exhausts the staged RAM-init poll");
  ra8_fake_mmap_reset();
  ra8_fake_mmio_reset();

  volatile r_canfd_t* reg = ra8_canfd((uint8_t)k_ra8_canfd_test_channel_0);
  TEST_ASSERT_NOT_NULL((void*)reg);
  /* HUM Ch 41.2.13 "CFDGSTS : Global Status Register" p 2730 */
  reg->CFDGSTS = (uint32_t)(1UL << k_ra8_gsts_bit_graminit);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_satisfy_after(ra8_sys_canfdckcr(), 0U));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_satisfy_after(&reg->CFDGSTS, 0U));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_satisfy_after(&reg->CFDC[0].STS, 0U));

  TEST_ASSERT_EQ(k_ra8_ok, ra8_canfd_init((uint8_t)k_ra8_canfd_test_channel_0));
  TEST_END("canfd init exhausts the staged RAM-init poll");
}
...
static void test_set_bitrate_250k_with_fd(void)
{
  ...
  volatile r_canfd_t* reg = ra8_canfd((uint8_t)k_ra8_canfd_test_channel_0);
  /* HUM Ch 41.2.4 "CFDC0STS : Status Register" p 2711 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_satisfy_after(&reg->CFDC[0].STS, 0U));
...
static void (*const s_test_roster[])(void) = {
  ...
  test_init_channel0_ram_init_poll_budget,
  test_init_channel1,
