...
/**
 * @test test_init_propagates_mstp_timeout
 * @brief Prove MIPI CSI initialization propagates its module-stop timeout.
 * @details Arms MSTPCRC through the existing fake-MMIO wait seam, then repeats
 * the same public call with a reset seam as the successful control.
 * @par MC/DC:
 * Decision: ``RA8_RETURN_ON_ERROR(mst_err, ...)`` in ``ra8_mipi_csi_init()``.
 * - Vector 1: unarmed MSTPCRC settles -> false, initialization succeeds.
 * - Vector 2: armed MSTPCRC never settles -> true, timeout is returned.
 * N+1 = 2 vectors for the one-condition decision.
 * @pre The MIPI CSI fake register window is mapped.
 ...
 * @since 0.1.0
 */
static void test_init_propagates_mstp_timeout(void)
{
  TEST_BEGIN("mipi_csi init propagates MSTP timeout");
  prep();
  ra8_fake_mmio_reset();
  const ra8_mipi_csi_config_t cfg = make_cfg();
  /* HUM Ch 11.2.8 "MSTPCRC : Module Stop Control Register C", p 446 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&ra8_mstp()->MSTPCRC));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_mipi_csi_init(&cfg));

  prep();
  ra8_fake_mmio_reset();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mipi_csi_init(&cfg));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mipi_csi_deinit());
  TEST_END("mipi_csi init propagates MSTP timeout");
}

/**
 * @test test_init_propagates_reset_status_timeout
 * @brief Prove initialization propagates a stuck reset-status timeout.
 * @details Holds RTST.VSRSTS high through the bounded public initialization
 * call. ``test_init_happy`` supplies the matching clear-status control.
 * @par MC/DC:
 * Decision: ``RA8_RETURN_ON_ERROR(rst_err, ...)`` in
 * ``ra8_mipi_csi_init()``.
 * - Vector 1: RTST.VSRSTS is clear -> false, initialization succeeds.
 * - Vector 2: RTST.VSRSTS remains set -> true, timeout is returned.
 * N+1 = 2 vectors for the one-condition decision.
 * @pre The MIPI CSI fake register window is mapped.
 * @pre RTST can remain busy for the synchronous bounded wait.
 * @post The busy-status call returns ::k_ra8_err_hw_timeout unchanged.
 * @post Receiver programming has not changed MCT0.
 * @note Single-threaded host fixture.
 * @since 0.1.0
 */
static void test_init_propagates_reset_status_timeout(void)
{
  TEST_BEGIN("mipi_csi init propagates reset-status timeout");
  prep();
  const ra8_mipi_csi_config_t cfg              = make_cfg();
  *ra8_mipi_csi_reg32(k_ra8_mipi_csi_off_rtst) = k_ra8_mipi_csi_rtst_vsrsts_mask;

  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_mipi_csi_init(&cfg));
  TEST_ASSERT_EQ(0U, *ra8_mipi_csi_reg32(k_ra8_mipi_csi_off_mct0));
  TEST_END("mipi_csi init propagates reset-status timeout");
}
...
static void test_init_epct_emct(void)
{
  ...
  cfg.epd_long_spacer       = k_t_spacer_long;
  cfg.epd_short_spacer      = k_t_spacer_short;
...
static void test_init_bad_args(void)
{
  ...
  cfg                 = make_cfg();
  cfg.epd_long_spacer = k_t_spacer_over; /* 16-bit, overflows 15-bit field. */
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_mipi_csi_init(&cfg));

  cfg                  = make_cfg();
  cfg.epd_short_spacer = k_t_spacer_over;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_mipi_csi_init(&cfg));
...
/**
 * @par MC/DC:
 * (no compound decisions in this test -- RTST.VSRSTS remains set through the
 * bounded reset-status poll; test_reset covers the clear/success direction)
 */
static void test_reset_stuck_status_times_out(void)
{
  TEST_BEGIN("mipi_csi reset times out while RTST stays busy");
  prep();
  const ra8_mipi_csi_config_t cfg = make_cfg();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mipi_csi_init(&cfg));
  *ra8_mipi_csi_reg32(k_ra8_mipi_csi_off_rtst) = k_ra8_mipi_csi_rtst_vsrsts_mask;

  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_mipi_csi_reset());
  TEST_END("mipi_csi reset times out while RTST stays busy");
}
...
static void test_set_epd(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mipi_csi_start_receive());
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, ra8_mipi_csi_set_epd(false, false, 0U, 0U));
  TEST_ASSERT_EQ(epct, *ra8_mipi_csi_reg32(k_ra8_mipi_csi_off_epct));
...
static void test_set_lrte(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mipi_csi_start_receive());
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, ra8_mipi_csi_set_lrte(k_ra8_mipi_csi_vlsien_x1, false));
...
static void test_deinit(void)
{
  ...
  *ra8_mipi_csi_reg32(k_ra8_mipi_csi_off_rxie)  = k_t_all_ones;
  *ra8_mipi_csi_reg32(k_ra8_mipi_csi_off_dlie0) = k_t_all_ones;
  *ra8_mipi_csi_reg32(k_ra8_mipi_csi_off_pmie)  = k_t_all_ones;
  *ra8_mipi_csi_reg32(k_ra8_mipi_csi_off_gsie)  = k_t_all_ones;
  for (uint8_t vc = 0U; vc < 16U; ++vc) {
    const ra8_mipi_csi_off_t off = ra8_mipi_csi_vc_off(k_ra8_mipi_csi_off_vcie0, vc);
    *ra8_mipi_csi_reg32(off)     = k_t_all_ones;
  }
...
static void (*const s_test_roster[])(void) = {
  test_init_happy,
  test_init_propagates_mstp_timeout,
  test_init_propagates_reset_status_timeout,
  ...
  test_reset,
  test_reset_stuck_status_times_out,
