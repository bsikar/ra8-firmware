...
/**
 * @enum gpt_pwm_dma_fixture_t
 * @brief Poison values written into out-parameters before a call, so one that fails without assigning is detectable.
 */
typedef enum : uint8_t {
  /** @brief Poison channel ID retained when allocation fails. */
  k_dma_ch_unset              = 0xFFU,
  k_gpt_fail_second_mstp_wait = 1U, /**< Fail the V-channel MSTP read-back. */
  k_gpt_fail_third_mstp_wait  = 2U, /**< Fail the W-channel MSTP read-back. */
} gpt_pwm_dma_fixture_t;

/**
 * @enum gpt_pwm_dma_fixture2_t
...
/**
 * @test test_gpt_three_phase_open_rolls_back_mstp_failures
 * @brief V- and W-channel MSTP failures roll back every earlier channel.
 * @par MC/DC:
 * The per-channel error guard is a single-condition decision. Fail-nth vectors
 * 1 and 2 drive it true after one and two successful acquisitions;
 * ::test_gpt_three_phase_open_starts_synchronously drives it false throughout.
 * @pre Fake MMIO, GPT state, and MSTP reference counts are reset per vector.
 * @post Every acquired GPT MSTP reference is released after either failure.
 * @post A disarmed retry opens and closes all three channels successfully.
 * @note Single-threaded host fixture.
 * @since 0.1.0
 */
static void test_gpt_three_phase_open_rolls_back_mstp_failures(void)
{
  TEST_BEGIN("gpt three_phase_open rolls back V/W MSTP failures");
  ra8_gpt_three_phase_cfg_t cfg = make_three_phase_cfg();
  uint8_t                   ref = k_dma_ch_unset;

  prep_w35();
  TEST_ASSERT_EQ(
    k_ra8_ok,
    /* HUM Ch 11.2.10 "MSTPCRE : Module Stop Control Register E" p 449 */
    ra8_fake_mmio_fail_nth_wait(&ra8_mstp()->MSTPCRE, (uint32_t)k_gpt_fail_second_mstp_wait));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_gpt_three_phase_open(&cfg));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_get_refcount(k_ra8_mstp_gpt0, &ref));
  TEST_ASSERT_EQ(0U, ref);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_get_refcount(k_ra8_mstp_gpt1, &ref));
  TEST_ASSERT_EQ(0U, ref);
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    /* HUM Ch 11.2.10 "MSTPCRE : Module Stop Control Register E" p 449 */
    ra8_fake_mmio_fail_nth_wait(&ra8_mstp()->MSTPCRE, (uint32_t)k_gpt_fail_third_mstp_wait));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_gpt_three_phase_open(&cfg));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_get_refcount(k_ra8_mstp_gpt0, &ref));
  TEST_ASSERT_EQ(0U, ref);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_get_refcount(k_ra8_mstp_gpt1, &ref));
  TEST_ASSERT_EQ(0U, ref);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_get_refcount(k_ra8_mstp_gpt2, &ref));
  TEST_ASSERT_EQ(0U, ref);

  prep_w35();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_gpt_three_phase_open(&cfg));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_gpt_three_phase_close());
  TEST_END("gpt three_phase_open rolls back V/W MSTP failures");
}
...
int main(void)
{
  ...
  test_gpt_three_phase_open_rolls_back_mstp_failures();
