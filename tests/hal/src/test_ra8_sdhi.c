...
/** @brief One-shot RSPEND service state for the CMD12-timeout fixture. */
static uint8_t s_initial_rsp_pending;
...
/**
 * @brief Service only the initial command and FIFO, withholding CMD12 RSPEND.
 *
 * @details The first poll belongs to the multi-block command. Subsequent polls
 * service the write FIFO but deliberately leave SD_INFO1.RSPEND clear, so the
 * driver's final CMD12 wait reaches its bounded timeout.
 */
static void sdhi_stop_timeout_hook(void)
{
  volatile r_sdhi_regs_t* reg = ra8_sdhi(s_srv_inst);
  if (reg != nullptr) {
    /* HUM Ch 47.2.11 "SD_INFO2 : SD Card Interrupt Flag Register 2" p 3132 */
    reg->SD_INFO2 = reg->SD_INFO2 | (uint32_t)k_sdhi_srv_brebwe;
    if (s_initial_rsp_pending != 0U) {
      /* HUM Ch 47.2.10 "SD_INFO1 : SD Card Interrupt Flag Register 1" p 3129 */
      reg->SD_INFO1         = reg->SD_INFO1 | (uint32_t)k_sdhi_srv_rspend;
      s_initial_rsp_pending = 0U;
    }
  }
}
...
/**
 * @brief Install the one-shot initial-command/CMD12-timeout poll hook.
 *
 * @param[in] inst SDHI instance index to service.
 */
static void sdhi_stop_timeout_hook_arm(uint8_t inst)
{
  s_srv_inst            = inst;
  s_initial_rsp_pending = 1U;
  ra8_fake_mmio_set_poll_hook(sdhi_stop_timeout_hook);
}
...
/**
 * @par MC/DC:
 * (no compound decisions in this test -- the fake fails only the second
 * SD_INFO1 wait, proving ACMD6 timeout propagation after CMD55 succeeds)
 */
static void test_set_bus_width_acmd6_timeout(void)
{
  TEST_BEGIN("sdhi set_bus_width_4bit propagates ACMD6 timeout");
  prep();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_sdhi_init((uint8_t)k_ra8_sdhi_test_inst_0));
  volatile r_sdhi_regs_t* reg = ra8_sdhi((uint8_t)k_ra8_sdhi_test_inst_0);
  /* HUM Ch 47.2.10 "SD_INFO1 : SD Card Interrupt Flag Register 1" p 3129 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_nth_wait(&reg->SD_INFO1, 1U));

  TEST_ASSERT_EQ(k_ra8_err_hw_timeout,
                 ra8_sdhi_set_bus_width_4bit((uint8_t)k_ra8_sdhi_test_inst_0, 0U));
  TEST_END("sdhi set_bus_width_4bit propagates ACMD6 timeout");
}

/**
 * @par MC/DC:
 * (no compound decisions in this test -- the staged SD_INFO1 failure drives
 * the internal command wait loop through continuation and exhaustion)
 */
static void test_read_block_command_timeout(void)
{
  TEST_BEGIN("sdhi read_block propagates a staged command timeout");
  prep();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_sdhi_init((uint8_t)k_ra8_sdhi_test_inst_0));
  volatile r_sdhi_regs_t* reg = ra8_sdhi((uint8_t)k_ra8_sdhi_test_inst_0);
  /* HUM Ch 47.2.10 "SD_INFO1 : SD Card Interrupt Flag Register 1" p 3129 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&reg->SD_INFO1));
  uint8_t buf[k_t_block_len] = {};

  TEST_ASSERT_EQ(k_ra8_err_hw_timeout,
                 ra8_sdhi_read_block((uint8_t)k_ra8_sdhi_test_inst_0, 0U, buf, 1U));
  TEST_END("sdhi read_block propagates a staged command timeout");
}

/**
 * @par MC/DC:
 * (no compound decisions in this test -- the staged SD_INFO1 failure drives
 * the write command wait loop through continuation and exhaustion)
 */
static void test_write_block_command_timeout(void)
{
  TEST_BEGIN("sdhi write_block propagates a staged command timeout");
  prep();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_sdhi_init((uint8_t)k_ra8_sdhi_test_inst_0));
  volatile r_sdhi_regs_t* reg = ra8_sdhi((uint8_t)k_ra8_sdhi_test_inst_0);
  /* HUM Ch 47.2.10 "SD_INFO1 : SD Card Interrupt Flag Register 1" p 3129 */
  ...
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout,
                 ra8_sdhi_write_block((uint8_t)k_ra8_sdhi_test_inst_0, 0U, buf, 1U));
  TEST_END("sdhi write_block propagates a staged command timeout");
}

/**
 * @par MC/DC:
 * (no compound decisions in this test -- the one-shot hook services the
 * initial multi-block command and FIFO, then withholds RSPEND from CMD12)
 */
static void test_write_block_stop_command_timeout(void)
{
  TEST_BEGIN("sdhi write_block propagates the final CMD12 timeout");
  prep();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_sdhi_init((uint8_t)k_ra8_sdhi_test_inst_0));
  uint8_t buf[k_t_block_len * 2U] = {};
  sdhi_stop_timeout_hook_arm((uint8_t)k_ra8_sdhi_test_inst_0);

  const ra8_err_t err = ra8_sdhi_write_block((uint8_t)k_ra8_sdhi_test_inst_0, 0U, buf, 2U);
  sdhi_flags_hook_disarm();

  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, err);
  TEST_ASSERT_EQ(k_ra8_sdhi_cmd_stop_transmission,
                 /* HUM Ch 47.2.1 "SD_CMD : Command Type Register" p 3123 */
                 ra8_sdhi((uint8_t)k_ra8_sdhi_test_inst_0)->SD_CMD);
  TEST_END("sdhi write_block propagates the final CMD12 timeout");
}

/**
 * @par MC/DC:
 * (no compound decisions in this test -- the command wait succeeds and the
 * staged SD_INFO2 failure drives the BRE loop continuation/exhaustion path)
 */
static void test_read_block_fifo_timeout(void)
{
  TEST_BEGIN("sdhi read_block propagates a staged BRE timeout");
  prep();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_sdhi_init((uint8_t)k_ra8_sdhi_test_inst_0));
  volatile r_sdhi_regs_t* reg = ra8_sdhi((uint8_t)k_ra8_sdhi_test_inst_0);
  /* HUM Ch 47.2.10 "SD_INFO1 : SD Card Interrupt Flag Register 1" p 3129 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_satisfy_after(&reg->SD_INFO1, 0U));
  /* HUM Ch 47.2.11 "SD_INFO2 : SD Card Interrupt Flag Register 2" p 3132 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&reg->SD_INFO2));
  uint8_t buf[k_t_block_len] = {};

  TEST_ASSERT_EQ(k_ra8_err_hw_timeout,
                 ra8_sdhi_read_block((uint8_t)k_ra8_sdhi_test_inst_0, 0U, buf, 1U));
  TEST_END("sdhi read_block propagates a staged BRE timeout");
}

/**
 * @par MC/DC:
 * (no compound decisions in this test -- the command wait succeeds and the
 * staged SD_INFO2 failure drives the BWE loop continuation/exhaustion path)
 */
static void test_write_block_fifo_timeout(void)
{
  TEST_BEGIN("sdhi write_block propagates a staged BWE timeout");
  prep();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_sdhi_init((uint8_t)k_ra8_sdhi_test_inst_0));
  volatile r_sdhi_regs_t* reg = ra8_sdhi((uint8_t)k_ra8_sdhi_test_inst_0);
  /* HUM Ch 47.2.10 "SD_INFO1 : SD Card Interrupt Flag Register 1" p 3129 */
  ...
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout,
                 ra8_sdhi_write_block((uint8_t)k_ra8_sdhi_test_inst_0, 0U, buf, 1U));
  TEST_END("sdhi write_block propagates a staged BWE timeout");
}
...
int main(void)
{
  ...
  test_set_bus_width_acmd6_timeout();
  test_read_block_command_timeout();
  test_write_block_command_timeout();
  test_write_block_stop_command_timeout();
  test_read_block_fifo_timeout();
  test_write_block_fifo_timeout();
  return 0;
}
