...
/**
 * @test test_write_dma_arg_guards
 * @details The former channel/length OR is now two sequential single-condition
 * guards. The three vectors cover success, invalid channel, and zero length;
 * no compound decision remains.
 *
 * @par MC/DC:
 * (no compound decisions in this test -- both guards are sequential)
 */
static void test_write_dma_arg_guards(void)
{
  TEST_BEGIN("spi_b ra8_spi_write_dma sequential channel and length guards");
  prep();
  ...
  TEST_END("spi_b ra8_spi_write_dma sequential channel and length guards");
}

/**
 * @test test_read_dma_arg_guards
 * @details The former channel/length OR is now two sequential single-condition
 * guards. The three vectors cover success, invalid channel, and zero length;
 * no compound decision remains.
 *
 ...
 * (no compound decisions in this test -- both guards are sequential)
 */
static void test_read_dma_arg_guards(void)
{
  TEST_BEGIN("spi_b ra8_spi_read_dma sequential channel and length guards");
  prep();
  uint8_t   out_dma_ch = k_t_dma_ch_unset;
  uint8_t   buf[1]     = {};
  ...
  TEST_END("spi_b ra8_spi_read_dma sequential channel and length guards");
}
...
int main(void)
{
  ...
  test_write_dma_arg_guards();
  test_read_dma_arg_guards();
