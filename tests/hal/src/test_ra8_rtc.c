...
{
  TEST_BEGIN("ra8_rtc_init wait seam delayed and exhausted");
  ra8_fake_mmap_reset();
  ra8_fake_mmio_reset();
  volatile r_rtc_regs_t* rtc = ra8_rtc();
  /* HUM Ch 26.2.21 "RCR2 : RTC Control Register 2 (in Calendar Count Mode)" p 1232 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_satisfy_after(&rtc->RCR2, 2U));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_rtc_init());

  ra8_fake_mmap_reset();
  ra8_fake_mmio_reset();
  /* HUM Ch 26.2.21 "RCR2 : RTC Control Register 2 (in Calendar Count Mode)" p 1232 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&rtc->RCR2));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_rtc_init());
  ra8_fake_mmio_reset();
  TEST_END("ra8_rtc_init wait seam delayed and exhausted");
}
 ...
 * @since 0.1.0
 */
static void test_clock_init_oscillator_timeouts(void)
{
  TEST_BEGIN("ra8_rtc_clock_init oscillator timeouts");
  ra8_fake_mmap_reset();
  ra8_fake_mmio_reset();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(ra8_sys_lococr()));
  TEST_ASSERT_EQ(k_ra8_err_hw_init_failed, ra8_rtc_clock_init(k_ra8_rtc_clk_loco));
  ra8_fake_mmap_reset();
  ra8_fake_mmio_reset();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(ra8_sys_sosccr()));
  TEST_ASSERT_EQ(k_ra8_err_hw_init_failed, ra8_rtc_clock_init(k_ra8_rtc_clk_subclock));
  ra8_fake_mmio_reset();
  TEST_END("ra8_rtc_clock_init oscillator timeouts");
}
...
int main(void)
{
  ...
  test_clock_init_oscillator_timeouts();
