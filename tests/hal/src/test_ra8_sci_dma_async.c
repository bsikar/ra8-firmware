...
typedef enum : uint32_t {
  ...
  k_mcdc_sci_baud_large   = 134217728U, /**< 2^27; 32 * baud is one past UINT32_MAX.        */
  k_mcdc_sci_brr_sentinel = 0xA55AU,    /**< Proves rejection does not write BRR.           */
  k_mcdc_sci_div_sentinel = 0xA5U,      /**< Proves rejection does not write CKS.           */
} mcdc_sci_const_t;
...
/**
 * @test test_write_dma_reg_len_guards
 *
 * @details The former `reg == nullptr || len == 0U` compound is now two
 * sequential single-condition guards in `ra8_sci_write_dma`. These vectors
 * cover the successful path, the invalid-channel guard, and the zero-length
 * guard independently; no compound decision remains.
 *
 * @par MC/DC:
 * (no compound decisions in this test -- both guards are sequential)
 * @see ra8_sci_write_dma
 */
static void test_write_dma_reg_len_guards(void)
{
  TEST_BEGIN("write_dma sequential reg and len guards");
  prep();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_dma_init());
  TEST_ASSERT_EQ(k_ra8_ok, ra8_sci_init((uint8_t)k_mcdc_sci_ch_ok, &k_cfg));
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_sci_write_dma((uint8_t)k_mcdc_sci_ch_ok,
                                   ...
                                   nullptr,
                                   &dma_ch));
  TEST_END("write_dma sequential reg and len guards");
}

/**
 * @test test_read_dma_reg_len_guards
 *
 * @details The former `reg == nullptr || len == 0U` compound is now two
 * sequential single-condition guards in `ra8_sci_read_dma`. These vectors
 * cover the successful path and both guards independently; no compound
 * decision remains.
 *
 * @par MC/DC:
 * (no compound decisions in this test -- both guards are sequential)
 ...
 */
static void test_read_dma_reg_len_guards(void)
{
  TEST_BEGIN("read_dma sequential reg and len guards");
  prep();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_dma_init());
  TEST_ASSERT_EQ(k_ra8_ok, ra8_sci_init((uint8_t)k_mcdc_sci_ch_ok, &k_cfg));
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_sci_read_dma((uint8_t)k_mcdc_sci_ch_ok,
                                  ...
                                  nullptr,
                                  &dma_ch));
  TEST_END("read_dma sequential reg and len guards");
}

/**
 * @test test_baud_calculate_large_baud_does_not_wrap
 *
 * @par MC/DC:
 * The repaired denominator uses 64-bit arithmetic and has no overflow guard.
 * This single-path boundary test complements the normal-baud loop vectors:
 * 32 * 2^27 is 2^32, larger than every uint32_t PCLK, so every quotient is
 * mathematically zero and the request is unreachable. A uint32_t product
 * instead wraps that exact valid input to zero.
 */
static void test_baud_calculate_large_baud_does_not_wrap(void)
{
  TEST_BEGIN("ra8_sci_baud_calculate: 64-bit denominator rejects unreachable large baud");
  prep();

  uint16_t brr     = (uint16_t)k_mcdc_sci_brr_sentinel;
  uint8_t  clk_div = (uint8_t)k_mcdc_sci_div_sentinel;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_sci_baud_calculate((uint32_t)k_mcdc_sci_baud_large,
                                        (uint32_t)k_mcdc_sci_pclk_ok,
                                        &brr,
                                        &clk_div));
  TEST_ASSERT_EQ(k_mcdc_sci_brr_sentinel, brr);
  TEST_ASSERT_EQ(k_mcdc_sci_div_sentinel, clk_div);
  TEST_END("ra8_sci_baud_calculate: 64-bit denominator rejects unreachable large baud");
}
...
int main(void)
{
  ...
  test_baud_calculate_large_baud_does_not_wrap();
  test_mcdc_async_write_data_len();
  test_mcdc_async_read_buf_len();
  test_mcdc_abort_direction();
  test_write_dma_reg_len_guards();
  test_read_dma_reg_len_guards();
  return 0;
}
