...
typedef enum : uint16_t {
  ...
  k_isr_event_base =
    0x50U, /**< First ELC event of the contiguous block the exhaustion case registers. */
  k_isr_test_priority = 5U, /**< Mid-range NVIC priority every registration in this file uses. */
} isr_fixture_t;
...
/**
 * @par MC/DC:
 * Decision: `if (slot == k_ra8_isr_slot_none)` in ra8_isr_register (1 condition)
 * - Vector 1: registrations 1..96, a free slot exists -> false (slot allocated)
 * - Vector 2: registration 97, every slot in use      -> true  (no_mem returned)
 * N+1 = 2 vectors for N=1: minimal MC/DC. The same pair is the only thing that
 * drives `internal_find_free`'s bounded scan past its last slot, so it also
 * covers that loop's exhaustion exit.
 */
static void test_register_exhausts_every_slot(void)
{
  TEST_BEGIN("ra8_isr_register: registration past the last slot reports no_mem");
  ra8_fake_mmap_reset();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_isr_init());

  for (uint16_t i = 0U; i < (uint16_t)k_ra8_isr_slot_count; ++i) {
    const ra8_elc_event_t event = (ra8_elc_event_t)((uint16_t)k_isr_event_base + i);
    uint16_t              slot  = (uint16_t)k_isr_slot_poison;
    TEST_ASSERT_EQ(
      k_ra8_ok,
      ra8_isr_register(event, stub_handler_b, nullptr, (uint8_t)k_isr_test_priority, &slot));
    TEST_ASSERT_EQ(i, slot);
  }

  /* Vector 2: every slot is in use, so internal_find_free scans all 96 and
   * falls out of its loop, and register must refuse rather than overrun. */
  const uint16_t        past = (uint16_t)k_isr_event_base + (uint16_t)k_ra8_isr_slot_count;
  const ra8_elc_event_t overflow_event = (ra8_elc_event_t)past;
  uint16_t              overflow_slot  = (uint16_t)k_isr_slot_poison;
  TEST_ASSERT_EQ(k_ra8_err_no_mem,
                 ra8_isr_register(overflow_event,
                                  stub_handler_b,
                                  nullptr,
                                  (uint8_t)k_isr_test_priority,
                                  &overflow_slot));
  TEST_ASSERT_EQ(k_isr_slot_poison, overflow_slot);

  /* Leave the allocator empty for any later case. */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_isr_init());
  TEST_END("ra8_isr_register: registration past the last slot reports no_mem");
}
...
int main(void)
{
  ...
  test_register_exhausts_every_slot();
  return 0;
}
