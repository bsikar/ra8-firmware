...
static void test_xfer8_timeout_sptef(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    ra8_fake_mmio_fail_wait((const volatile void*)&ra8_spi(k_ra8_spi_test_ch_zero)->SPSR));
 ...
 * @pre The SPI fake mmap/mmio windows are resettable.
 ...
 *
 * Guards C and D: the former channel/length OR decisions in
 * ``ra8_spi_write_dma`` and ``ra8_spi_read_dma`` are now sequential
 * single-condition guards. The same three vectors cover success, invalid
 * channel, and zero length independently; no compound decision remains.
 *
 *
