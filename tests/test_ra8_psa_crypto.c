/**
 * @file test_ra8_psa_crypto.c
 * @brief Unit tests for the ra8_psa_crypto facade
 *
 * @details
 * Exercises every public entry point in ``ra8_psa_crypto.h`` against
 * the simulator-mode stand-ins defined in ``ra8_psa_crypto.c``:
 *
...
static void test_mcdc_aead_decrypt_aad_pair(void)
{
  ...
  uint8_t         out[k_psa_mcdc_plain_len];
  size_t          ol = 0U;
  const ra8_err_t v1 = ra8_psa_aead_decrypt(k,
                                            k_ra8_psa_alg_aes_gcm,
                                            k_test_nonce,
                                            sizeof(k_test_nonce),
                                            nullptr,
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra8_psa_crypto.c\n");
  return 0;
}
