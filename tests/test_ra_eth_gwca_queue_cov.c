/**
 * @file test_ra_eth_gwca_queue_cov.c
 * @brief Coverage-focused unit tests for ra_eth_gwca_queue.c
 *
 * @details
 * Companion to test_ra_eth_gwca.c. That base suite deliberately skips
 * the ::ra_eth_gwca_tx_frame happy path on the host: it assumed a
 ...
 * ra_sim_mmap's constructor). A pointer into that window is below 2^32,
 * so ::ra_eth_gwca_set_descriptor_buffer / ::ra_eth_gwca_decode_ptr
 ...
 *
 * @details ra_sim_mmap maps 0x22000000..0x221FFFFF (2 MiB SRAM) as
 ...
 * (reached through ::ra_eth_gwca_configure_queue):
...
/**
 * @test ra_eth_gwca_decode_ptr rejects a null descriptor.
 *
 * @par MC/DC:
 * Single condition `if (desc == nullptr)` in ::ra_eth_gwca_decode_ptr:
...
static void test_decode_ptr_null(void)
{
  ...
  prep();
  TEST_ASSERT_NULL(ra_eth_gwca_decode_ptr(nullptr));
  TEST_END("decode_ptr null guard");
}

/**
 * @test ra_eth_gwca_tx_frame full enqueue happy path (host-round-trippable).
 *
 * @par MC/DC:
 ...
 * run: `if (err != k_ra_ok)` (find_slot succeeded) and
...
static void test_tx_frame_happy(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_eth_gwca_tx_frame(chain, 4U, &tail, frame, 64U, 128U));
  ...
  TEST_ASSERT_EQ(k_ra_gwdcc_dt_fsingle, chain[0].dt);
  TEST_ASSERT_EQ(1U, tail);
  TEST_END("tx_frame happy path");
}

/**
 * @test ra_eth_gwca_tx_frame propagates find_slot's queue-full return.
 *
 * @par MC/DC:
 * Single condition `if (err != k_ra_ok)` on the tx_frame enqueue path:
 *   - V_T: every data slot pre-seeded FSINGLE so ::ra_eth_gwca_find_slot
 *     finds no FEMPTY slot and returns k_ra_err_no_data, which tx_frame
 *     forwards unchanged.
...
static void test_tx_frame_queue_full(void)
{
  ...
  chain[0].dt = (uint8_t)k_ra_gwdcc_dt_fsingle;
  chain[1].dt = (uint8_t)k_ra_gwdcc_dt_fsingle;
  chain[2].dt = (uint8_t)k_ra_gwdcc_dt_fsingle;

  TEST_ASSERT_EQ(k_ra_err_no_data, ra_eth_gwca_tx_frame(chain, 4U, &tail, frame, 64U, 128U));
  TEST_END("tx_frame queue full");
}

/**
 * @test ra_eth_gwca_tx_frame rejects a FEMPTY slot with no backing buffer.
 *
 * @par MC/DC:
 ...
 *     is FEMPTY with ptr_h/ptr_l still zero; ::ra_eth_gwca_decode_ptr
 *     returns nullptr and tx_frame returns k_ra_err_invalid_arg before
 *     any memcpy runs.
...
static void test_tx_frame_unbacked_slot(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_eth_gwca_tx_frame(chain, 4U, &tail, frame, 64U, 128U));
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_eth_gwca_queue_cov.c\n");
  return 0;
}
