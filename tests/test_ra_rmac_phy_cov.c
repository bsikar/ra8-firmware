/**
 * @file test_ra_rmac_phy_cov.c
 * @brief Line-coverage top-up for ra_rmac_phy.c
 *
 * @details
 * Sibling to test_ra_rmac_phy.c. That test covers the happy paths and
 ...
 * the injected ra_rmac_phy_io_t read/write function pointers, so each
 ...
 *
 * Uncovered lines targeted (libs/ra_hal/src/ra_rmac_phy.c):
 ...
 * @retval k_ra_ok        Read serviced.
 * @retval k_ra_err_hw_error Injected read failure for ``reg``.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok        Write serviced.
 * @retval k_ra_err_hw_error Injected write failure for ``reg``.
 *
 * @pre ``ctx`` is non-NULL.
 * @pre ``reg`` < ::k_test_reg_count.
...
/**
 * @brief Reset the sim + driver + mock between cases.
 *
 * @details
 ...
 * @post ``ra_sim_mmap`` window is reset.
 *
 ...
 *
 * @return A ::ra_rmac_phy_cfg_t advertising 10/100 + 1000BASE-T.
 *
 * @pre None.
 * @pre None.
 ...
 * ``if (err != k_ra_ok)`` after the BMCR.RESET write in
...
static void test_reset_write_fail(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_rmac_phy_open(&cfg));
 ...
 * @par MC/DC:
 * No compound decision in the touched leg. Drives the single condition
 * ``if (err != k_ra_ok)`` inside the reset-poll loop to its true arm
 ...
 * @par MC/DC:
 * No compound decision in the touched leg. Drives the single condition
 * ``if (err != k_ra_ok)`` after the ANAR write in
 ...
 * ``if (err != k_ra_ok)`` after the 1000T-CTRL write is true (line 196)
 ...
 * @par MC/DC:
 * No compound decision in the touched leg. Drives the single condition
 * ``if (err != k_ra_ok)`` after the BMSR read to true (line 449). The
...
static void test_link_bmsr_read_fail(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_rmac_phy_link_status_get(&lk));
 ...
 * full MC/DC for that decision lives in test_ra_rmac_phy.c. The
...
static void test_link_1000half(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_rmac_phy_speed_1000h, lk.speed);
...
static void test_link_10full(void)
{
  ...
  ra_rmac_phy_link_t lk       = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_rmac_phy_link_status_get(&lk));
  TEST_ASSERT_EQ(k_ra_rmac_phy_speed_10f, lk.speed);
...
static void test_link_10half(void)
{
  ...
  ra_rmac_phy_link_t lk       = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_rmac_phy_link_status_get(&lk));
  TEST_ASSERT_EQ(k_ra_rmac_phy_speed_10h, lk.speed);
...
int32_t main(void)
{
  ...
  test_link_10full();
  test_link_10half();
  (void)fprintf(stderr, "[OK ] test_ra_rmac_phy_cov.c\n");
  return 0;
}
