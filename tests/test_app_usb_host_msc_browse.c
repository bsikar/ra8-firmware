 ...
 * surface (PFS routing + ra_usb_hmsc init/attach/inquiry/read_capacity
...
static ra_usb_hmsc_device_t s_test_hmsc_attach_device;
...
static void reset_world(void)
{
  ...
  s_test_hmsc_attach_device = (ra_usb_hmsc_device_t){};
}
 ...
 *
 * @par MC/DC: Decision under test: ``ra_usb_hmsc_init() != k_ra_ok``.
...
static void test_hmsc_inquiry_and_read_capacity_shape(void)
{
  ...
  uint32_t blocks = 0U;
  uint32_t bsize  = 0U;
  ra_err_t e2     = ra_usb_hmsc_read_capacity((uint8_t)k_test_hmsc_target_lun, &blocks, &bsize);
...
static void test_hmsc_pfs_route_invalid_pin_rejected(void)
{
  ...
  TEST_ASSERT(ra_pfs_route_peripheral(bad_pin, (ra_psel_t)k_test_hmsc_psel_usbhs, "test.bad") !=
              k_ra_ok);
