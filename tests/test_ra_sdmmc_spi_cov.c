/**
 * @file test_ra_sdmmc_spi_cov.c
 * @brief Coverage-completion unit tests for the SPI-mode SD driver core TU.
 *
 * @details
 * The companion ``test_ra_sdmmc_spi.c`` exercises the block-I/O translation
 * unit (``ra_sdmmc_spi_io.c``) and the happy identification path. This file
 * targets the error / recovery legs of the protocol core
 * (``ra_sdmmc_spi.c``) that the block-I/O tests never reach: chip-select
 ...
 * ``ra_sdmmc_spi_run_init_sequence()``. Every fault is injected
 ...
 * @return ::k_ra_ok, or ::k_ra_err_hw_timeout on the armed call.
 * @retval k_ra_ok           CS toggled.
 * @retval k_ra_err_hw_timeout The armed cs call fired.
 * @pre None.
 ...
 * @return ::k_ra_ok, or ::k_ra_err_hw_timeout once the fault index is reached.
 * @retval k_ra_ok            Bytes exchanged from the queue.
 * @retval k_ra_err_hw_timeout Armed fault fired (this and later calls).
 ...
 * @param[in]  len Byte count.
 * @return ::k_ra_err_not_supported on NULL tx, else ::mock_xfer's result.
 * @retval k_ra_err_not_supported NULL tx (drives read_r3_or_r7_tail fallback).
 * @retval k_ra_ok               Non-NULL tx exchanged.
 * @pre The mock is bound into the driver transport.
 * @post On the non-NULL path, ::mock_xfer's postconditions hold.
 * @note Test-only helper.
 * @since 0.1.0
 */
static ra_err_t mock_xfer_no_nulltx(void* ctx, const uint8_t* tx, uint8_t* rx, uint32_t len)
{
  if (tx == nullptr) {
 ...
 * @since 0.1.0
 */
static ra_err_t smart_xfer_acmd41(void* ctx, const uint8_t* tx, uint8_t* rx, uint32_t len)
{
  (void)ctx;
...
static const ra_sdmmc_spi_transport_t s_tr = {
...
static const ra_sdmmc_spi_transport_t s_tr_nonulltx = {
  .set_clock = mock_set_clock,
  .cs        = mock_cs,
...
static const ra_sdmmc_spi_transport_t s_tr_smart = {
  .set_clock = mock_set_clock,
  .cs        = mock_cs,
 ...
 * @since 0.1.0
 */
static void cov_bind(const ra_sdmmc_spi_transport_t* tr)
{
...
/**
 * @par MC/DC:
 * Single-condition decision ``if (err != k_ra_ok)`` on the ``cs`` return in
 * ``ra_sdmmc_spi_cs_assert``. The armed cs failure flips it true; the
...
static void test_cs_assert_cs_failure(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_sdmmc_spi_cs_assert());
 ...
 * @par MC/DC:
 * Single-condition decision ``if (err != k_ra_ok)`` on the ``cs`` return in
 * ``ra_sdmmc_spi_cs_release``; the armed cs failure flips it true.
 */
static void test_cs_release_cs_failure(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_sdmmc_spi_cs_release());
  TEST_END("cs_release propagates a cs failure");
}

/**
 * @par MC/DC:
 * Single-condition decision ``if (err != k_ra_ok)`` inside
 * ``internal_read_r1`` (reached through ``ra_sdmmc_spi_send_command``). Call 1
...
static void test_send_command_r1_read_fault(void)
{
  ...
  uint8_t r1          = 0U;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_sdmmc_spi_send_command(k_sd_cmd_go_idle_state, 0U, &r1));
  TEST_END("send_command R1-read xfer fault");
}

/**
 * @par MC/DC:
 * Single-condition decision ``if (err != k_ra_ok)`` after the CMD55 prefix in
 * ``ra_sdmmc_spi_send_acmd``; the armed fault on the first frame flips it true
...
static void test_send_acmd_cmd55_fault(void)
{
  ...
  uint8_t r1          = 0U;
  TEST_ASSERT(ra_sdmmc_spi_send_acmd(k_sd_acmd_sd_send_op_cond, 0U, &r1) != k_ra_ok);
  TEST_END("send_acmd CMD55-prefix xfer fault");
}

/**
 * @par MC/DC:
 * Single-condition decision ``if (err != k_ra_ok)`` in
 * ``ra_sdmmc_spi_wait_data_token``; the armed fault on the first poll flips it
 * true.
 */
static void test_wait_data_token_fault(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_sdmmc_spi_wait_data_token());
  TEST_END("wait_data_token xfer fault");
}

/**
 * @par MC/DC:
 * Two single-condition decisions in ``ra_sdmmc_spi_wait_not_busy_bounded``:
 * the ``if (err != k_ra_ok)`` xfer-fault leg (first sub-case) and the
...
static void test_wait_not_busy_bounded_legs(void)
{
  ...
  cov_bind(&s_tr);
  s_mock.xfer_fail_at = 1U;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_sdmmc_spi_wait_not_busy_bounded(10U));
  ...
  mock_queue_byte(0x00U);
  mock_queue_byte(0x00U);
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_sdmmc_spi_wait_not_busy_bounded(3U));
...
static void test_init_wake_cs_failure(void)
{
  ...
  TEST_ASSERT(ra_sdmmc_spi_run_init_sequence() != k_ra_ok);
  TEST_END("init wake-card cs failure");
}

/**
 * @par MC/DC:
 * Single-condition ``if (err != k_ra_ok)`` after ``cs_assert`` in
...
/**
 * @par MC/DC:
 * Single-condition ``if (cs(true) != k_ra_ok)`` in
...
/**
 * @par MC/DC:
 * Single-condition ``if (err != k_ra_ok)`` after ``send_command`` in
...
static void test_init_cmd8_echo_mismatch(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_protocol_error, ra_sdmmc_spi_run_init_sequence());
  TEST_END("init CMD8 echo mismatch");
}

/**
 * @par MC/DC:
 * Single-condition ``if (err != k_ra_ok)`` (false control) inside
...
static void test_init_tail_fallback_success(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_sdmmc_spi_run_init_sequence());
  TEST_ASSERT_EQ(k_ra_sdmmc_spi_type_sdhc, s_sdmmc_spi_state.card_type);
  TEST_END("init R3/R7 tail per-byte fallback success");
}

/**
 * @par MC/DC:
 * Single-condition ``if (err != k_ra_ok)`` (true leg) inside the
...
/**
 * @par MC/DC:
 * Single-condition ``if (err != k_ra_ok)`` after ``send_acmd`` in
...
static void test_init_acmd41_exhausts_attempts(void)
{
  ...
  cov_bind(&s_tr_smart);
  TEST_ASSERT_EQ(k_ra_err_hw_init_failed, ra_sdmmc_spi_run_init_sequence());
...
/**
 * @par MC/DC:
 * Single-condition ``if (err != k_ra_ok)`` after ``read_r3_or_r7_tail`` in
...
static void test_init_ocr_no_ccs_classifies_sdv2(void)
{
  ...
  q_cmd_r1((uint8_t)k_cov_r1_ready); /* CMD16. */
  TEST_ASSERT_EQ(k_ra_ok, ra_sdmmc_spi_run_init_sequence());
  TEST_ASSERT_EQ(k_ra_sdmmc_spi_type_sdv2, s_sdmmc_spi_state.card_type);
...
/**
 * @par MC/DC:
 * Single-condition ``if (err != k_ra_ok)`` after ``wait_data_token`` in
...
/**
 * @par MC/DC:
 * Single-condition ``if (err != k_ra_ok)`` inside the CSD-body read loop of
...
int main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] all ra_sdmmc_spi_cov tests passed\n");
  return 0;
}
