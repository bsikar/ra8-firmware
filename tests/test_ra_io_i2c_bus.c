/**
 * @file test_ra_io_i2c_bus.c
 * @brief Unit tests for the ra_io I2C-bus facade
 *        (libs/ra_io/src/ra_io_i2c_bus*.c).
 *
 * @details
 * Binds both backends of the facade -- classic RIIC
 * (`ra_io_i2c_bus_bind_riic`) and the I3C block's I2C-compatibility mode
 * (`ra_io_i2c_bus_bind_i3c_compat`) -- against the ``ra_sim_mmap``
 ...
 *    `ra_i2c_*` / `ra_i3c_*` drivers, observed through the same register
 ...
 *  - the `ra_io_i2c_bus_as_ops` bridge into the Ring-3 seam, including
...
/**
 * @enum ra_io_i2c_bus_test_const_t
...
static void bring_up_riic(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_i2c_init((uint8_t)k_test_riic_ch, &cfg));
  volatile r_i2c_regs_t* reg = ra_i2c_regs((uint8_t)k_test_riic_ch);
 ...
 * priming the ra_i3c_i2c and ra_touch tests use.
 *
 * @pre ``prep`` ran (clean window + MSTP up).
 * @pre Channel 0 registers are mapped.
...
static void bring_up_i3c_compat(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_init((uint8_t)k_test_i3c_ch, &cfg));
...
{
  TEST_BEGIN("ra_io_i2c_bus: NULL handle + never-bound handle rejected");
  prep();

  uint8_t          rx  = 0U;
  ra_i2c_bus_ops_t ops = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_io_i2c_bus_write(nullptr, (uint8_t)k_test_periph_7b, s_payload, 1U, true));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_io_i2c_bus_read(nullptr, (uint8_t)k_test_periph_7b, &rx, 1U));
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_io_i2c_bus_transfer(nullptr, (uint8_t)k_test_periph_7b, s_payload, 1U, &rx, 1U));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_i2c_bus_as_ops(nullptr, &ops));

  const ra_io_i2c_bus_t unbound = {};
  TEST_ASSERT_EQ(k_ra_err_not_initialized,
                 ra_io_i2c_bus_write(&unbound, (uint8_t)k_test_periph_7b, s_payload, 1U, true));
  TEST_ASSERT_EQ(k_ra_err_not_initialized,
                 ra_io_i2c_bus_read(&unbound, (uint8_t)k_test_periph_7b, &rx, 1U));
  TEST_ASSERT_EQ(
    k_ra_err_not_initialized,
    ra_io_i2c_bus_transfer(&unbound, (uint8_t)k_test_periph_7b, s_payload, 1U, &rx, 1U));
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_io_i2c_bus_as_ops(&unbound, &ops));
  TEST_END("ra_io_i2c_bus: NULL handle + never-bound handle rejected");
}
...
{
  TEST_BEGIN("ra_io_i2c_bus: binder NULL / channel-range rejection");
  prep();

  ra_io_i2c_bus_t bus = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_i2c_bus_bind_riic(nullptr, (uint8_t)k_test_riic_ch));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_i2c_bus_bind_i3c_compat(nullptr, (uint8_t)k_test_i3c_ch));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_io_i2c_bus_bind_riic(&bus, (uint8_t)k_test_riic_ch_oor));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_io_i2c_bus_bind_i3c_compat(&bus, (uint8_t)k_test_i3c_ch_oor));
  /* A failed bind leaves the handle unbound. */
  uint8_t rx = 0U;
  TEST_ASSERT_EQ(k_ra_err_not_initialized,
                 ra_io_i2c_bus_read(&bus, (uint8_t)k_test_periph_7b, &rx, 1U));
  /* Good binds are accepted for both backends. */
  TEST_ASSERT_EQ(k_ra_ok, ra_io_i2c_bus_bind_riic(&bus, (uint8_t)k_test_riic_ch));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_i2c_bus_bind_i3c_compat(&bus, (uint8_t)k_test_i3c_ch));
  TEST_END("ra_io_i2c_bus: binder NULL / channel-range rejection");
}
 ...
 * decisions carry their MC/DC vectors in test_ra_i2c.c)
 */
static void test_riic_forwarding(void)
{
  TEST_BEGIN("ra_io_i2c_bus: RIIC write / read / transfer forward");
  prep();
  bring_up_riic();
   ...
   * via ICCR2.SP, the exact register effect test_ra_i2c.c pins. */
  TEST_ASSERT_EQ(
    k_ra_ok,
...
{
  TEST_BEGIN("ra_io_i2c_bus: I3C-compat write / read / transfer forward");
  prep();
  bring_up_i3c_compat();
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_io_i2c_bus_write(&bus, (uint8_t)k_test_periph_7b, s_payload, sizeof(s_payload), false));
  TEST_ASSERT_EQ(0U, (reg->CNDCTL & (uint32_t)k_ra_i3c_i2c_msk_cndctl_spcnd));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_deinit((uint8_t)k_test_i3c_ch));
  TEST_END("ra_io_i2c_bus: I3C-compat write / read / transfer forward");
}
...
{
  TEST_BEGIN("ra_io_i2c_bus_as_ops: seam trampolines forward + guard");
  prep();
  bring_up_riic();
  ...
  ra_i2c_bus_ops_t ops = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_i2c_bus_as_ops(&bus, nullptr));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_i2c_bus_as_ops(&bus, &ops));
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ops.read(nullptr, (uint8_t)k_test_periph_7b, &rx, 1U));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ops.transfer(nullptr, (uint8_t)k_test_periph_7b, &cmd, 1U, &rd, 1U));

  TEST_ASSERT_EQ(k_ra_ok, ra_i2c_deinit((uint8_t)k_test_riic_ch));
  TEST_END("ra_io_i2c_bus_as_ops: seam trampolines forward + guard");
}
...
int32_t main(void)
{
  ...
  test_as_ops_bridge();
  (void)fprintf(stderr, "[OK  ] test_ra_io_i2c_bus.c\n");
  return 0;
}
