/**
 * @file test_ra_io_blockdev_sdhi_cov.c
 * @brief Coverage-boost tests for ra_io_blockdev_sdhi.c.
 *
 * @details
 * The native-SDHI block-device backend is a thin shim: every vtable op
 * (read / write / get_caps) drops the singleton context and forwards to
 * the ``ra_sdcard`` HAL driver. The sibling test_ra_io_blockdev_backends.c
 * only binds the backend and checks the NULL guard, so the three
 * forwarding bodies stay uncovered because their data path needs a live
 * card.
 *
 * This file drives all three bodies:
 *
 *   - sdhi_read / sdhi_write: called through the public
 *     ra_io_blockdev_read / ra_io_blockdev_write dispatcher with the card
 *     torn down, so ``ra_sdcard_read_blocks`` / ``ra_sdcard_write_blocks``
 *     return ::k_ra_err_invalid_state. The forwarding statement executes
 *     regardless of the card driver's verdict, so the invalid-state leg
 *     covers every line of both bodies without any hardware bring-up.
 *   - sdhi_get_caps error leg: called with the card torn down so
 *     ``ra_sdcard_get_capacity`` returns ::k_ra_err_invalid_state and the
 *     early ``return cap`` runs.
 *   - sdhi_get_caps success snapshot: the ``ra_sdcard`` singleton is
 *     brought fully up so ``ra_sdcard_get_capacity`` returns ::k_ra_ok and
 *     the whole capabilities-fill block runs; every field is checked
 *     against the backend's fixed SD contract.
 *
 * Bringing the card up on the host is the interesting part. ``ra_sdcard``
 * runs the SD Physical-Layer init as a blocking sequence of
 * ``ra_sdhi_send_command`` calls, each spinning on SD_INFO1.RSPEND. In
 * RA_SIMULATOR_MODE the SDHI register window is plain host RAM (see
 * tests/mocks/ra_sim_mmap.c), so nothing sets RSPEND on its own and the
 * driver clears the bit after every command. The register handshake must
 * therefore be serviced concurrently while init blocks.
 *
 * A background servicer thread does exactly that -- WITHOUT any SIGALRM /
 * setitimer timer signal. The response registers SD_RSP10..76 are
 * pre-loaded once with a single universal value set that satisfies every
 * command the init sequence issues (CMD8 echo, ACMD41 OCR busy+CCS, and a
 * CSD v2 layout), because the driver only ever reads those registers --
 * it never writes them. The servicer then simply keeps SD_INFO1.RSPEND
 * asserted; each ``ra_sdhi_send_command`` sees it on its next poll,
 * copies the pre-loaded response, and returns. The two-million-iteration
 * per-command spin budget dwarfs the thread's scheduling latency, so the
 * bring-up is deterministic in result on a preemptive multi-core host.
 *
 * Builds as a standalone executable auto-discovered by the
 * tests/CMakeLists.txt ``test_*.c`` GLOB; its .gcda stream is merged by
 * gcovr with the sibling tests so the newly hit lines count toward the
 * global ra_io_blockdev_sdhi.c coverage total without touching the
 * sibling test file.
 *
 * @copyright Copyright (c) 2026 Brighton Sikarskie
 * SPDX-License-Identifier: MIT
 */

#include <pthread.h>
#include <stdatomic.h>
#include <stdint.h>
#include <string.h>

#include "ra8d2_sdhi_regs.h"
#include "ra_err.h"
#include "ra_io_blockdev.h"
#include "ra_io_blockdev_sdhi.h"
#include "ra_mstp.h"
#include "ra_sdcard.h"
#include "ra_sim_mmap.h"
#include "unity_minimal.h"

/* ===========================================================================
 * Test constants
 * ===========================================================================
 */

/**
 * @enum cov_sdhi_inst_t
 * @brief SDHI instance index used by every test.
 *
 * @since 0.1.0
 */
typedef enum : uint8_t {
  k_cov_sdhi_inst = 0U, /**< Single-instance driver; all tests share SDHI 0. */
} cov_sdhi_inst_t;

/**
 * @enum cov_sdhi_rsp_t
 * @brief Universal SD response-register values that satisfy every init command.
 *
 * @details
 * The driver copies SD_RSP10..76 verbatim on each command and only reads
 * them, so one static value set can service the whole init sequence:
 *
 * - SD_RSP10 (rsp[0]) = 0xC00001AA
 *     - low 12 bits 0x1AA -> CMD8 SEND_IF_COND echo matches the host pattern.
 *     - bit 31 set        -> ACMD41 OCR "busy done" so the op-cond loop exits
 *                            on its first iteration.
 *     - bit 30 set (CCS)  -> high-capacity card -> SDHC/SDXC classification.
 *     - upper 16 bits     -> CMD3 relative-card-address (value is unused).
 * - SD_RSP76 (rsp[3]) = 0x40000000 -> CSD_STRUCTURE = 1 -> CSD v2 decode.
 * - SD_RSP32 (rsp[1]) = 0xF0000000 -> CSD v2 C_SIZE middle 16 bits = 0xF000.
 * - SD_RSP54 (rsp[2]) = 0x00000000 -> CSD v2 C_SIZE top 6 bits = 0.
 *
 * CSD v2 capacity = (C_SIZE + 1) * 1024 = (0xF000 + 1) * 1024 = 62,915,584
 * 512-byte blocks, below the SDXC boundary, so the card classifies as SDHC.
 *
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_cov_sdhi_rsp10          = 0xC00001AAUL, /**< CMD8 echo | OCR busy | CCS.         */
  k_cov_sdhi_rsp32          = 0xF0000000UL, /**< CSD v2 C_SIZE middle bits.          */
  k_cov_sdhi_rsp54          = 0x00000000UL, /**< CSD v2 C_SIZE top bits (zero).      */
  k_cov_sdhi_rsp76          = 0x40000000UL, /**< CSD_STRUCTURE = 1 (v2 layout).      */
  k_cov_sdhi_rspend_bit     = 0x00000001UL, /**< SD_INFO1.RSPEND (bit 0).            */
  k_cov_sdhi_expect_blocks  = 62915584UL,   /**< (0xF000 + 1) * 1024 blocks.         */
  k_cov_sdhi_init_retry_max = 8U,           /**< Bring-up attempts before giving up. */
} cov_sdhi_rsp_t;

/* ===========================================================================
 * RSPEND servicer thread (no SIGALRM)
 * ===========================================================================
 */

/** @brief Servicer run flag; cleared to stop the thread. */
static atomic_int s_srv_run;

/** @brief Servicer thread handle. */
static pthread_t s_srv_thread;

/** @brief SDHI instance the servicer keeps RSPEND asserted on. */
static uint8_t s_srv_inst;

/**
 * @brief Continuously re-assert SD_INFO1.RSPEND for the serviced instance.
 *
 * @details
 * The driver clears RSPEND after each command; this loop keeps setting it
 * again so the next ``ra_sdhi_send_command`` poll always observes it. The
 * response registers were pre-loaded before the thread started and are
 * never written by the driver, so no other register work is needed. No
 * timer signal is used -- the thread is a plain busy re-assert.
 *
 * @param[in] arg Unused pthread cookie.
 *
 * @return Always nullptr.
 *
 * @pre s_srv_inst is a valid SDHI instance index.
 * @pre The SDHI register window is mapped (RA_SIMULATOR_MODE).
 * @post SD_INFO1.RSPEND is held high until s_srv_run is cleared.
 *
 * @note Runs on a dedicated thread; shares only the RAM-backed register
 *       window and the atomic run flag with the main thread.
 *
 * @since 0.1.0
 */
static void* sdhi_servicer_main(void* arg)
{
  (void)arg;
  volatile r_sdhi_regs_t* reg = ra_sdhi(s_srv_inst);
  if (reg == nullptr) {
    return nullptr;
  }
  while (atomic_load(&s_srv_run) != 0) {
    reg->SD_INFO1 = reg->SD_INFO1 | (uint32_t)k_cov_sdhi_rspend_bit;
  }
  return nullptr;
}

/**
 * @brief Pre-load the universal responses and start the RSPEND servicer.
 *
 * @param[in] inst SDHI instance index to service.
 *
 * @pre No servicer thread is currently running.
 * @pre ra_sim_mmap_reset() has already scrubbed the register window.
 * @post SD_RSP10..76 hold the universal init responses and the servicer
 *       thread is running with RSPEND asserted.
 *
 * @note Not reentrant; tests are single-threaded apart from this servicer.
 *
 * @since 0.1.0
 */
static void sdhi_servicer_start(uint8_t inst)
{
  s_srv_inst                  = inst;
  volatile r_sdhi_regs_t* reg = ra_sdhi(inst);
  TEST_ASSERT_NOT_NULL((const void*)reg);
  reg->SD_RSP10 = (uint32_t)k_cov_sdhi_rsp10;
  reg->SD_RSP32 = (uint32_t)k_cov_sdhi_rsp32;
  reg->SD_RSP54 = (uint32_t)k_cov_sdhi_rsp54;
  reg->SD_RSP76 = (uint32_t)k_cov_sdhi_rsp76;
  reg->SD_INFO1 = reg->SD_INFO1 | (uint32_t)k_cov_sdhi_rspend_bit;

  atomic_store(&s_srv_run, 1);
  const int rc = pthread_create(&s_srv_thread, nullptr, sdhi_servicer_main, nullptr);
  TEST_ASSERT_EQ(0, rc);
}

/**
 * @brief Stop the RSPEND servicer and join its thread.
 *
 * @pre sdhi_servicer_start() was called previously.
 * @post The servicer thread has terminated and been joined.
 *
 * @since 0.1.0
 */
static void sdhi_servicer_stop(void)
{
  atomic_store(&s_srv_run, 0);
  (void)pthread_join(s_srv_thread, nullptr);
}

/* ===========================================================================
 * Fixtures
 * ===========================================================================
 */

/**
 * @brief Scrub every register window and force the card driver un-init'd.
 *
 * @pre Called from a test body before touching any HAL function.
 * @post MMIO windows read zero and ra_sdcard is not initialized.
 *
 * @since 0.1.0
 */
static void cov_prep(void)
{
  ra_sim_mmap_reset();
  (void)ra_mstp_init();
  (void)ra_sdcard_deinit();
}

/**
 * @brief Bring the ``ra_sdcard`` singleton fully up against the servicer.
 *
 * @details
 * Starts the RSPEND servicer, then runs ``ra_sdcard_init`` (retrying a
 * few times purely as insurance against a pathological single-attempt
 * scheduling miss), and stops the servicer. Asserts the card came up.
 *
 * @pre cov_prep() has just run.
 * @post ra_sdcard_get_capacity() returns ::k_ra_ok with a non-zero count.
 *
 * @since 0.1.0
 */
static void cov_card_up(void)
{
  sdhi_servicer_start((uint8_t)k_cov_sdhi_inst);
  const ra_sdcard_cfg_t cfg = {.instance = (uint8_t)k_cov_sdhi_inst};
  ra_err_t              err = k_ra_err_hw_timeout;
  for (uint32_t attempt = 0U; attempt < (uint32_t)k_cov_sdhi_init_retry_max; ++attempt) {
    err = ra_sdcard_init(&cfg);
    if (err == k_ra_ok) {
      break;
    }
    (void)ra_sdcard_deinit();
  }
  sdhi_servicer_stop();
  TEST_ASSERT_EQ(k_ra_ok, err);
}

/* ===========================================================================
 * Test functions
 * ===========================================================================
 */

/**
 * @brief sdhi_read forwards through the dispatcher to the card driver.
 *
 * @details
 * Binds the backend and reads one block with the card torn down. The
 * public ``ra_io_blockdev_read`` null-checks ``buf`` then forwards to
 * ``sdhi_read``, whose ``return ra_sdcard_read_blocks(...)`` executes and
 * surfaces ::k_ra_err_invalid_state (card never initialized). Line
 * coverage of the forwarding body does not depend on the card being up.
 *
 * @par MC/DC:
 * sdhi_read has one single-condition decision, the ``RA_CHECK_NULL_PTR(buf)``
 * guard (no ``&&`` / ``||``). Its FALSE arm is covered here (buf non-NULL);
 * the TRUE arm is unreachable through ra_io_blockdev_read, which null-checks
 * buf before forwarding, so no compound-decision vectors are required.
 *
 * @since 0.1.0
 */
static void test_sdhi_read_forwards(void)
{
  TEST_BEGIN("sdhi_read forwards to card driver");
  cov_prep();
  ra_io_blockdev_t bd = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_sdhi_init(&bd));

  uint8_t buf[(size_t)k_ra_io_block_size_bytes];
  memset(buf, 0xA5, sizeof(buf));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_io_blockdev_read(&bd, 0U, 1U, buf));
  TEST_END("sdhi_read forwards to card driver");
}

/**
 * @brief sdhi_write forwards through the dispatcher to the card driver.
 *
 * @details
 * Mirror of test_sdhi_read_forwards for the write path: a single-block
 * write with the card torn down forwards to ``sdhi_write``, whose
 * ``return ra_sdcard_write_blocks(...)`` executes and surfaces
 * ::k_ra_err_invalid_state.
 *
 * @par MC/DC:
 * sdhi_write has one single-condition decision, the ``RA_CHECK_NULL_PTR(buf)``
 * guard. FALSE arm covered here (buf non-NULL); the TRUE arm is unreachable
 * through ra_io_blockdev_write, which null-checks buf first. No compound
 * decision exists in this body.
 *
 * @since 0.1.0
 */
static void test_sdhi_write_forwards(void)
{
  TEST_BEGIN("sdhi_write forwards to card driver");
  cov_prep();
  ra_io_blockdev_t bd = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_sdhi_init(&bd));

  uint8_t buf[(size_t)k_ra_io_block_size_bytes];
  memset(buf, 0x5A, sizeof(buf));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_io_blockdev_write(&bd, 0U, 1U, buf));
  TEST_END("sdhi_write forwards to card driver");
}

/**
 * @brief sdhi_get_caps early-returns the card driver's capacity error.
 *
 * @details
 * With the card torn down, ``ra_sdcard_get_capacity`` returns
 * ::k_ra_err_invalid_state, so ``sdhi_get_caps`` takes the ``if (cap !=
 * k_ra_ok) return cap;`` early-out and the capabilities-fill block is
 * skipped.
 *
 * @par MC/DC:
 * sdhi_get_caps up to the early-out has two single-condition decisions:
 * - ``RA_CHECK_NULL_PTR(out)`` -- FALSE arm covered here (out non-NULL); the
 *   TRUE arm is unreachable through ra_io_blockdev_get_caps, which null-checks
 *   out before forwarding.
 * - ``if (cap != k_ra_ok)`` -- TRUE arm covered here (invalid-state card); its
 *   FALSE arm is covered by test_sdhi_get_caps_success.
 * Neither is a compound (``&&`` / ``||``) decision.
 *
 * @since 0.1.0
 */
static void test_sdhi_get_caps_error(void)
{
  TEST_BEGIN("sdhi_get_caps error leg");
  cov_prep();
  ra_io_blockdev_t bd = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_sdhi_init(&bd));

  ra_io_blockdev_caps_t caps = {};
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_io_blockdev_get_caps(&bd, &caps));
  TEST_END("sdhi_get_caps error leg");
}

/**
 * @brief sdhi_get_caps fills the full SD capabilities snapshot on success.
 *
 * @details
 * Brings the ``ra_sdcard`` singleton fully up (see cov_card_up), binds the
 * backend, then queries capabilities. ``ra_sdcard_get_capacity`` returns
 * ::k_ra_ok, so the whole fill block runs; every field is checked against
 * the backend's fixed SD contract (one-block erase unit, 512-byte program
 * size and logical block, zero erase value, no erase-before-write, and
 * writable), plus the decoded block count.
 *
 * @par MC/DC:
 * The only decision reached in the success path is the single-condition
 * ``if (cap != k_ra_ok)`` whose FALSE arm is covered here; its TRUE arm is
 * covered by test_sdhi_get_caps_error. No compound decision exists.
 *
 * @since 0.1.0
 */
static void test_sdhi_get_caps_success(void)
{
  TEST_BEGIN("sdhi_get_caps success snapshot");
  cov_prep();
  cov_card_up();

  /* Sanity: the driver reports the capacity our CSD v2 responses encode. */
  uint32_t blocks = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_sdcard_get_capacity(&blocks));
  TEST_ASSERT_EQ((uint32_t)k_cov_sdhi_expect_blocks, blocks);

  ra_io_blockdev_t bd = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_sdhi_init(&bd));

  ra_io_blockdev_caps_t caps = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_get_caps(&bd, &caps));
  TEST_ASSERT_EQ((uint32_t)k_cov_sdhi_expect_blocks, caps.block_count);
  TEST_ASSERT_EQ(1U, caps.erase_unit_blocks);
  TEST_ASSERT_EQ((uint32_t)k_ra_io_block_size_bytes, caps.program_size_bytes);
  TEST_ASSERT_EQ((uint32_t)k_ra_io_block_size_bytes, (uint32_t)caps.logical_block_bytes);
  TEST_ASSERT_EQ((uint8_t)k_ra_io_erase_value_zero, caps.erase_value);
  TEST_ASSERT(!caps.must_erase_before_write);
  TEST_ASSERT(!caps.read_only);

  TEST_ASSERT_EQ(k_ra_ok, ra_sdcard_deinit());
  TEST_END("sdhi_get_caps success snapshot");
}

/**
 * @brief ra_io_blockdev_sdhi_init NULL guard and successful bind.
 *
 * @details
 * Confirms the bind helper rejects a NULL handle and, given a valid
 * handle, wires the SDHI vtable so a later dispatch reaches the backend.
 *
 * @par MC/DC:
 * The bind helper's only decision is the single-condition
 * ``RA_CHECK_NULL_PTR(bd)`` guard: TRUE arm (bd == NULL) and FALSE arm
 * (valid bd) are both exercised here. No compound decision exists.
 *
 * @since 0.1.0
 */
static void test_sdhi_bind(void)
{
  TEST_BEGIN("sdhi_init NULL guard + bind");
  cov_prep();
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_blockdev_sdhi_init(nullptr));

  ra_io_blockdev_t bd = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_sdhi_init(&bd));
  TEST_ASSERT_NOT_NULL((const void*)bd.iface);
  TEST_END("sdhi_init NULL guard + bind");
}

/* ===========================================================================
 * Entry point
 * ===========================================================================
 */

/**
 * @brief Run all native-SDHI block-device coverage tests.
 *
 * @return 0 on success; the harness calls exit(1) on the first failure.
 *
 * @pre The ra_core_hal OBJECT library was built with coverage instrumentation.
 * @post Every forwarding body in ra_io_blockdev_sdhi.c has executed at least
 *       once, including the get_caps success and error legs.
 *
 * @since 0.1.0
 */
int32_t main(void)
{
  test_sdhi_bind();
  test_sdhi_read_forwards();
  test_sdhi_write_forwards();
  test_sdhi_get_caps_error();
  test_sdhi_get_caps_success();
  (void)fprintf(stderr, "[OK  ] test_ra_io_blockdev_sdhi_cov.c\n");
  return 0;
}
