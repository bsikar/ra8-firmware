/**
 * @file test_ra_flash_config_cov.c
 * @brief Coverage-completion tests for ra_flash_config.c (ARC counters).
 *
 * @details
 * ra_flash_config.c is exercised broadly by test_ra_flash.c, but the
 ...
 * ``0x02F27E00`` -- OUTSIDE every window the shared ``ra_sim_mmap``
 ...
 * mmap in a module constructor (exactly the mechanism ``ra_sim_mmap``
 * uses for the windows it does own), then drives the *real*
 * ra_flash_config.c code with seeded register / counter contents and
 ...
 *    ``counter < k_ra_flash_arc_count`` so no out-of-range id arrives.
 *  - the ``words_per`` clamp -- ``words_per`` is only ever 16 or 2, so
 *    ``words_per > k_ra_mram_arc_max_words`` (16) can never fire.
 ...
 *          ``ra_sim_mmap``. A failed map would segfault every ARC test,
...
{
  *(volatile uint16_t*)(uintptr_t)k_ra_flash_ofs_arccs_addr = v;
}

/** @brief Write one 32-bit word into the ARC_SEC counter image. */
static void arc_seed_sec_word(uint32_t idx, uint32_t v)
{
  ((volatile uint32_t*)(uintptr_t)k_ra_flash_ofs_arc_sec_addr)[idx] = v;
}

/** @brief Write one 32-bit word into the ARC_NSEC counter image. */
static void arc_seed_nsec_word(uint32_t idx, uint32_t v)
{
  ((volatile uint32_t*)(uintptr_t)k_ra_flash_ofs_arc_nsec_addr)[idx] = v;
}

/** @brief Build a valid ra_flash configuration for init. */
 ...
 *
 * @par MC/DC:
 * No compound (&&/||) decision on this path. ``ra_flash_arc_read`` takes
 * the non-OEMBL return, ``internal_arc_read_locked`` selects the
 * ``id == k_ra_flash_arc_sec`` else-if arm (a single condition), and the
 ...
 *
 * @par MC/DC:
 * No compound decision. Exercises all four ``k_ra_flash_arc_nsec_*``
...
static void test_arc_read_nsec_all_slots_single(void)
{
  ...
  arc_seed_arccs((uint16_t)k_ra_arc_arcns_single);
  ...
  uint32_t c2 = 0U;
  uint32_t c3 = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_arc_read(k_ra_flash_arc_nsec_0, &c0));
 ...
 * ``id == k_ra_flash_arc_nsec_1`` slot offset. Seeds slot 1 (words 2..3)
 ...
 * ``id == k_ra_flash_arc_sec`` single-condition guard, ``0 + 1 <= 64``
...
static void test_arc_increment_sec_ok(void)
{
  ...
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mastat)  = 0U;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_arc_increment(k_ra_flash_arc_sec));
 ...
 * command lock after MRDY and returns ``k_ra_err_hw_error``, which the
...
static void test_arc_increment_sec_cmdlk(void)
{
  ...
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mastat)  = (uint8_t)k_ra_mastat_mask_cmdlk;
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_flash_arc_increment(k_ra_flash_arc_sec));
 ...
 * (ARCNS=single) returns ``k_ra_arc_nsec_single`` (256). Count 0 + 1
...
static void test_arc_increment_nsec_single_max(void)
{
  ...
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mastat)  = 0U;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_arc_increment(k_ra_flash_arc_nsec_0));
 ...
 * return the multiple-counter maximum ``k_ra_arc_nsec_multiple`` (64) via
...
static void test_arc_increment_nsec_multiple_max(void)
{
  ...
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mastat)  = 0U;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_arc_increment(k_ra_flash_arc_nsec_1));
 ...
 * ``!in_ofs && !in_extra`` inside ``ra_flash_config_set_write``; the
 * startup config-set target ``k_ra_msaddr_config_set_startup`` sits in
 * the OFS window, so ``in_ofs`` is T,T and the rejection is skipped
 * (F,F) -- already an MC/DC vector covered by test_ra_flash.c. This test
 ...
 * returns ``k_ra_err_hw_error`` and ``ra_flash_set_startup_area``
...
static void test_set_startup_area_permanent_hw_error(void)
{
  ...
  *ra_mram_reg32((uint16_t)k_ra_mram_off_mstatr) =
    (uint32_t)((uint32_t)k_ra_mstatr_mask_mrdy | (uint32_t)k_ra_mstatr_mask_any_err);

  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_flash_set_startup_area(k_ra_flash_startup_default, false));
 ...
 * above ``k_ra_flash_extra_start`` (so the earlier guard passes), but
 * ``end_excl = mram_addr + len`` overruns
 * ``k_ra_flash_extra_start + k_ra_flash_extra_size``, driving the
 * end-of-region rejection to ``k_ra_err_invalid_arg``. Complements the
 * pre-existing below-start and past-start rejections in test_ra_flash.c.
 */
static void test_extra_mram_write_end_past_extra(void)
{
  ...
  const uint32_t near_end =
    (uint32_t)k_ra_flash_extra_start + (uint32_t)k_ra_flash_extra_size - 16U;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_flash_extra_mram_write(near_end, buf, (uint32_t)k_ra_mram_write_size_bytes));
  TEST_END("flash extra_mram_write end past extra");
}

/**
 * @brief Run every ra_flash_config coverage-completion test.
 *
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_flash_config_cov.c\n");
  return 0;
}
