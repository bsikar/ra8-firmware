/**
 * @file test_ra_nsc_ota.c
 * @brief Unit tests + MC/DC vectors for libs/ra_nsc/src/ra_nsc_ota.c
 *
 ...
 * (2 conditions, libs/ra_nsc/src/ra_nsc_ota.c line 57)
 *
 * Vectors (Chilenski masking-MC/DC, N+1 = 3 for N=2):
 *  - V1: target=k_ra_ota_bank_a (0)  -> C1=F shorts. Decision F (forward).
 *  - V2: target=k_ra_ota_bank_b (1)  -> C1=T, C2=F. Decision F (forward).
...
{
  TEST_BEGIN("ra_nsc_ota_commit MC/DC: target_bank != A && target_bank != B");
 ...
 *
 * @par MC/DC:
 * ``ra_nsc_flash_bank_config`` is a non-branching forwarder
 * (libs/ra_nsc/src/ra_nsc_ota.c line 89). It contains zero compound
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_nsc_ota.c\n");
  return 0;
}
