/**
 * @file test_ra_usb_pmsc_scsi_cov.c
 * @brief Branch-coverage tests for the device-MSC SCSI command
 *        handlers in `libs/ra_hal/src/ra_usb_pmsc_scsi.c`
 *
 * @details
 * The sibling `test_ra_usb_pmsc.c` drives the happy paths through the
 ...
 * declared non-static in `ra_usb_pmsc_internal.h` precisely for this
 ...
 *
 * @details Mirrors the private offsets in `ra_usb_pmsc_scsi.c` so the
 ...
 *
 * @details Chosen distinct from `k_ra_err_invalid_size` so a test can
 * tell a backend-propagated failure apart from a size-guard rejection.
 * @note Read-only after definition.
 * @since 0.1.0
 */
static const ra_err_t s_test_error = k_ra_err_hw_error;

/* ---- Mock storage callbacks ---- */

static ra_err_t cb_read_ok(void* ctx, uint32_t lba, uint32_t block_count, uint8_t* buf)
{
  (void)ctx;
  (void)lba;
  (void)block_count;
...
static ra_err_t cb_read_err(void* ctx, uint32_t lba, uint32_t block_count, uint8_t* buf)
{
  (void)ctx;
  (void)lba;
  (void)block_count;
...
static ra_err_t cb_write_ok(void* ctx, uint32_t lba, uint32_t block_count, const uint8_t* buf)
{
  (void)ctx;
  (void)lba;
  (void)block_count;
...
static ra_err_t cb_write_err(void* ctx, uint32_t lba, uint32_t block_count, const uint8_t* buf)
{
  (void)ctx;
  (void)lba;
  (void)block_count;
...
static ra_err_t cb_cap_normal(void* ctx, uint32_t* block_count, uint32_t* block_size)
{
  (void)ctx;
...
static ra_err_t cb_cap_err(void* ctx, uint32_t* block_count, uint32_t* block_size)
{
  (void)ctx;
  *block_count = 0U;
  *block_size  = 0U;
...
static ra_err_t cb_cap_bs_zero(void* ctx, uint32_t* block_count, uint32_t* block_size)
{
  (void)ctx;
  *block_count = (uint32_t)k_test_block_count;
  *block_size  = 0U; /* Forces the k_ra_pmsc_block_size_default fallback. */
  return k_ra_ok;
}

static ra_err_t cb_cap_count_zero(void* ctx, uint32_t* block_count, uint32_t* block_size)
{
  (void)ctx;
...
static ra_err_t cb_inq_ok(void* ctx, uint8_t* vendor8, uint8_t* product16, uint8_t* revision4)
{
  (void)ctx;
  (void)vendor8;
  (void)product16;
...
static ra_err_t cb_inq_err(void* ctx, uint8_t* vendor8, uint8_t* product16, uint8_t* revision4)
{
  (void)ctx;
  (void)vendor8;
  (void)product16;
...
static const ra_usb_pmsc_storage_t s_ok_storage = {
 ...
 *
 * @details Mirrors `test_ra_usb_pmsc.c`'s `prep()` sequence so the
 ...
 */
static void setup_with(const ra_usb_pmsc_storage_t* storage)
{
  ra_sim_mmap_reset();
  (void)ra_mstp_init();
  (void)ra_usb_pmsc_close();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_pmsc_init(k_ra_usb_speed_fs));
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_pmsc_attach_storage(storage));
}
...
{
  for (uint8_t i = 0U; i < k_ra_pmsc_cdb_max_len; ++i) {
...
/**
 * @par MC/DC:
 * (no compound decisions -- the guard `err != k_ra_ok` is a single
...
static void test_inquiry_backend_error_propagates(void)
{
  ...
  ra_usb_pmsc_storage_t st = s_ok_storage;
...
/**
 * @par MC/DC:
 * (no compound decisions -- single `err != k_ra_ok` guard driven true.)
 */
...
/**
 * @par MC/DC:
 * (no compound decisions -- single `cap_err != k_ra_ok` guard driven
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_usb_pmsc_scsi_cov.c\n");
  return 0;
}
