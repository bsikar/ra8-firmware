/**
 * @file test_ra_gfx_text_cov.c
 * @brief Line-coverage tests for libs/ra_gfx/src/ra_gfx_text.c
 *
 * @details
 * Companion to test_ra_gfx_text.c (which carries the MC/DC vectors). This TU
 * targets source lines the existing suite never executes, driving them through
 * the real public ra_gfx API on a host-side framebuffer + the bundled 8x16
 ...
 *
 * Because the ra_gfx module keeps its bind state in a single file-static, the
 * "not yet initialised" early-return guards can only be reached before any
 * successful ra_gfx_init() in this process. test_uninitialized_guards() runs
 * first from main() for exactly that reason.
 *
 * Targeted source lines (libs/ra_gfx/src/ra_gfx_text.c):
 *   277-281  internal_put_pixel RGB565 case (via ra_gfx_pixel)
 *   322-327  internal_get_pixel RGB565 case (via ra_gfx_blit, src rgb565)
 *   329-333  internal_get_pixel RGB888 case (via ra_gfx_blit, src rgb888)
 *   343      internal_get_pixel defensive `return 0U` -- UNREACHABLE via API
 *   565      ra_gfx_clear not-initialised guard
 *   592,603,606,613,616  ra_gfx_set_clip guard + corner clamps
 *   628      ra_gfx_reset_clip not-initialised guard
 *   640      ra_gfx_pixel not-initialised guard
 *   650,652  internal_gray_to_color (via ra_gfx_blit_gray8)
 ...
 *   698,700-701,703-704,708-710,713-718,720  ra_gfx_blit_gray8 body
 *   726      ra_gfx_line not-initialised guard
 *   759      ra_gfx_rect not-initialised guard
 *   833      ra_gfx_circle not-initialised guard
 *   1002     ra_gfx_text_out not-initialised guard
 *   1049     ra_gfx_blit not-initialised guard
 *
...
/** @brief Source gray buffer for ra_gfx_blit_gray8 (one byte per sample). */
 ...
 *
 * @par Coverage:
 * Lands every "ra_gfx_init() was not called" early return without first binding
 * the module, so it MUST run before any successful init in this TU. Targets
 * lines 565 (ra_gfx_clear), 592 (ra_gfx_set_clip), 628 (ra_gfx_reset_clip),
 * 640 (ra_gfx_pixel), 700-701 (ra_gfx_blit_gray8), 726 (ra_gfx_line),
 * 759 (ra_gfx_rect), 833 (ra_gfx_circle), 1002 (ra_gfx_text_out, reached past
 * the non-null str/font check), and 1049 (ra_gfx_blit, reached past the
...
static void test_uninitialized_guards(void)
{
  ...
  TEST_ASSERT_EQ(
    ...
    ra_gfx_text_out(0, 0, "x", &ra_gfx_font_8x16, k_cov_col_fg, k_cov_col_bg)); /* 1002 */
  TEST_ASSERT_EQ(k_ra_err_not_initialized,
                 ra_gfx_blit(s_blit_src,
 ...
 *
 * @par Coverage:
 * ra_gfx_pixel in RGB565 routes through internal_plot -> internal_put_pixel and
 * takes the k_ra_gfx_format_rgb565 switch arm, landing lines 277-281. The
...
static void test_put_pixel_565_path(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_gfx_init(s_fb, k_cov_dim, k_cov_dim, k_ra_gfx_format_rgb565));
  TEST_ASSERT_EQ(k_ra_ok, ra_gfx_clear(k_cov_col_bg));
  const size_t  off  = cov_off(k_cov_pos, k_cov_pos, (size_t)k_cov_bpp_565);
  const uint8_t base = s_fb[off];
  TEST_ASSERT_EQ(k_ra_ok, ra_gfx_pixel(k_cov_pos, k_cov_pos, k_cov_col_fg)); /* 277-281 */
 ...
 *
 * @par Coverage:
 * ra_gfx_blit reads each source pixel with internal_get_pixel. A source bound as
 * k_ra_gfx_format_rgb565 lands the RGB565 read arm (322-327); a source bound as
 * k_ra_gfx_format_rgb888 lands the RGB888 read arm (329-333). The framebuffer
 ...
 *
 * @par Coverage:
 * ra_gfx_set_clip with a fully off-screen positive origin and negative spans
 ...
 *
 * @par Coverage:
 * ra_gfx_blit_gray8 in RGB565 runs its body (698,703-704) and the clipped fast
 ...
 *
 * @par Coverage:
 * ra_gfx_blit_gray8 in a non-RGB565 format takes the format-mismatch arm at 708
...
static void test_blit_gray8_slow_path(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_gfx_init(s_fb, k_cov_dim, k_cov_dim, k_ra_gfx_format_argb8888));
  TEST_ASSERT_EQ(k_ra_ok, ra_gfx_clear(k_cov_col_bg));
  cov_fill_gray();
 ...
 *
 * @par Coverage:
 * ra_gfx_blit_gray8's argument guard (703-704): a NULL source returns
 * k_ra_err_invalid_arg, confirming the validation line executes after the
...
static void test_blit_gray8_arg_guard(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_gfx_blit_gray8(nullptr, k_cov_gray_dim, k_cov_gray_dim, 0, 0)); /* 703-704 */
 ...
 *
 * @par Coverage:
 * Exercises ra_gfx_text_size as a supporting measurement check: a 3-character
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_gfx_text_cov.c\n");
  return 0;
}
