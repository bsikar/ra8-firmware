/**
 * @file test_ra8_ble_mesh.c
 * @brief Unit tests for libs/ra8_ble_host mesh wrapper.
 *
...
extern void    ra8_ble_mesh_test_emit_event(const ra8_ble_mesh_event_t* evt);
extern uint8_t ra8_ble_mesh_test_prov_active(void);

static uint32_t s_evt_count;

static void evt_cb(void* ctx, const ra8_ble_mesh_event_t* evt)
{
  (void)ctx;
  (void)evt;
  s_evt_count++;
...
static void test_init_null(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_ble_mesh_init(nullptr));
...
static void test_init_invalid_count(void)
{
  ...
  ra8_ble_mesh_config_t cfg = {};
  cfg.element_count         = 0U;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_ble_mesh_init(&cfg));
  cfg.element_count = (uint8_t)(k_ra8_ble_mesh_max_elements + 1U);
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_ble_mesh_init(&cfg));
...
static void test_lifecycle(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ble_mesh_init(&cfg));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ble_mesh_attach_event_handler(evt_cb, nullptr));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ble_mesh_prov_enable());
  TEST_ASSERT_EQ(1U, ra8_ble_mesh_test_prov_active());
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ble_mesh_prov_disable());
  TEST_ASSERT_EQ(0U, ra8_ble_mesh_test_prov_active());
  s_evt_count              = 0U;
  ra8_ble_mesh_event_t evt = {.kind = k_ra8_ble_mesh_evt_provisioned};
  ra8_ble_mesh_test_emit_event(&evt);
  TEST_ASSERT_EQ(1U, s_evt_count);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ble_mesh_factory_reset());
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ble_mesh_close());
  TEST_ASSERT_EQ(k_ra8_err_not_initialized, ra8_ble_mesh_close());
  TEST_END("test_lifecycle");
}

/* --------------------------------------------------------------------- */
/* MC/DC vector tests for libs/ra8_ble_host/src/ra8_ble_mesh.c */
/* --------------------------------------------------------------------- */
...
static void test_mcdc_mesh_validate_element_count(void)
{
  ...
  cfg.element_count = 0U;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_ble_mesh_init(&cfg));
  cfg.element_count = (uint8_t)((uint8_t)k_ra8_ble_mesh_max_elements + 1U);
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_ble_mesh_init(&cfg));
...
static void test_mcdc_mesh_emit_event_guard(void)
{
  ...
  ra8_ble_mesh_test_emit_event(&evt);
  TEST_ASSERT_EQ(1U, s_evt_count);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ble_mesh_attach_event_handler(nullptr, nullptr));
  ra8_ble_mesh_test_emit_event(&evt);
  TEST_ASSERT_EQ(1U, s_evt_count);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ble_mesh_attach_event_handler(evt_cb, nullptr));
  ra8_ble_mesh_test_emit_event(nullptr);
  TEST_ASSERT_EQ(1U, s_evt_count);
