 ...
 * ``ra_usb_hhid_init(HS)``, registers an attach callback, then
 * configures the keyboard for boot-protocol via
 * ``ra_usb_hhid_set_protocol`` + ``ra_usb_hhid_set_idle`` and polls
 * ``ra_usb_hhid_get_input_report`` at 10 ms cadence.
 *
 * This test exercises the same module surface (PFS routing, SCI8
 * init, ra_usb_hhid init/attach/configure/get_input_report) under
 * mocked MMIO.
 *
...
static const ra_port_pin_t k_test_hkbd_pin_txd =
  (ra_port_pin_t)(((uint16_t)k_ra_port_13 << 8) | (uint16_t)k_ra_pin_2);
static const ra_port_pin_t k_test_hkbd_pin_rxd =
  (ra_port_pin_t)(((uint16_t)k_ra_port_13 << 8) | (uint16_t)k_ra_pin_3);

/** @brief Captured attach state. */
static uintptr_t            s_test_hkbd_attach_ctx;
static ra_usb_hhid_device_t s_test_hkbd_attach_device;
...
static void reset_world(void)
{
  ...
  s_test_hkbd_attach_device = (ra_usb_hhid_device_t){};
}
 ...
 *
 * @par MC/DC: Decision under test (in app): the ``ra_pfs_route_peripheral
 ...
 *
 * @par MC/DC: Decision under test: ``ra_sci_init() != k_ra_ok``. This
...
static void test_hkbd_set_protocol_and_idle_call_shape(void)
{
  ...
  TEST_ASSERT(e2 != k_ra_err_invalid_arg);
