/**
 * @file test_ra_nsc_eth.c
 * @brief MC/DC unit test for libs/ra_nsc/src/ra_nsc_eth.c
 *
 * @details
 ...
 * @endcode
 *
 * In the host build ``RA_NSC_CHECK_NS_RANGE_R`` is a no-op; the
 * tail call lands in ``ra_net_pal_send_frame``.  We avoid pulling
 * the PAL into a known state by NOT initialising it -- the tail
 * call then returns ``k_ra_err_invalid_state`` (decision F path)
 ...
 */
static void test_mcdc_ra_nsc_eth(void)
{
  ...
  const ra_err_t v_max = ra_nsc_eth_send(s_frame, (uint16_t)k_test_eth_len_max);
  TEST_ASSERT(v_max != k_ra_err_invalid_arg);

  TEST_END("ra_nsc_eth_send MC/DC: length validator OR");
}

int32_t main(void)
{
  test_mcdc_ra_nsc_eth();
  (void)fprintf(stderr, "[OK ] test_ra_nsc_eth.c\n");
  return 0;
}
