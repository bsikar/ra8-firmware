/**
 * @file test_ra_dfu_antirollback.c
...
/*
 * Anti-rollback is opt-in behind RA_ENABLE_ROOT_OF_TRUST (default OFF, see
 * ra_dfu_antirollback.h). The globbed ``ra_dfu_antirollback.c`` therefore
 ...
 * implementation in directly (the test include path covers libs/ra_dfu/src),
 * keeping the gate exercisable -- mirroring tests/test_ra_root_of_trust.c.
 */
#define RA_ENABLE_ROOT_OF_TRUST

#include <stdint.h>

#include "ra_dfu_antirollback.c" // NOLINT(bugprone-suspicious-include) -- pull in flag-gated impl
#include "ra_err.h"
...
static ra_err_t s_mock_read_result = k_ra_ok;
/** @brief Mock backing: status the mock commit returns (injects commit fails). */
static ra_err_t s_mock_commit_result = k_ra_ok;
 ...
 * @return ::s_mock_read_result (``k_ra_ok`` unless a failure was injected).
 * @retval k_ra_ok           Stored value reported.
 ...
 * @since 0.1.0
 */
static ra_err_t mock_read(uint32_t* out_min_version)
{
 ...
 * @return ::s_mock_commit_result (``k_ra_ok`` unless a failure was injected).
 * @retval k_ra_ok Commit recorded.
 * @pre ``mock_reset`` ran to seed the backing state.
 ...
 * @since 0.1.0
 */
static ra_err_t mock_commit(uint32_t new_version)
{
 ...
 * @post Read and commit results are ``k_ra_ok``; commit-called is false.
...
static void mock_reset(uint32_t stored)
{
  ...
  s_mock_read_result   = k_ra_ok;
  s_mock_commit_result = k_ra_ok;
 ...
 * - Vector 1: image=7, stored=5 -> false -> ACCEPT (k_ra_ok).
 * - Vector 2: image=3, stored=5 -> true  -> REJECT (k_ra_err_validation_failed).
...
static void test_check_policy_mcdc(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_rot_antirollback_check((uint32_t)k_test_arb_newer, (uint32_t)k_test_arb_stored));
  /* Vector 2: older -> condition true -> reject. */
  TEST_ASSERT_EQ(
    k_ra_err_validation_failed,
    ra_rot_antirollback_check((uint32_t)k_test_arb_older, (uint32_t)k_test_arb_stored));
  /* Vector 3: equal boundary -> condition false -> accept. */
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_rot_antirollback_check((uint32_t)k_test_arb_stored, (uint32_t)k_test_arb_stored));
 ...
 *      ``ra_rot_antirollback_verify`` is a single condition; this establishes
 ...
 *
 * @par MC/DC:
 * Decision: policy result ``policy_err != k_ra_ok`` inside verify (1 condition).
...
static void test_verify_older_rejected(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_validation_failed,
                 ra_rot_antirollback_verify(&s_mock_store, (uint32_t)k_test_arb_older));
 ...
 *
 * @par MC/DC:
 * Three independent NULL guards (``RA_CHECK_NULL_PTR``), each a single
 * condition:
...
static void test_verify_null_store_denied(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rot_antirollback_verify(nullptr, (uint32_t)k_test_arb_newer));

  const ra_rot_antirollback_store_t no_read = {.read = nullptr, .commit = mock_commit};
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rot_antirollback_verify(&no_read, (uint32_t)k_test_arb_newer));

  const ra_rot_antirollback_store_t no_commit = {.read = mock_read, .commit = nullptr};
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rot_antirollback_verify(&no_commit, (uint32_t)k_test_arb_newer));
 ...
 *
 * @par MC/DC:
 * Decision: ``read_err != k_ra_ok`` inside verify (1 condition).
 * - Vector 1: read returns k_ra_ok -> false -> proceed (accept test).
 * - Vector 2: read returns k_ra_err_hw_error -> true -> DENY (this test).
...
static void test_verify_read_failure_denied(void)
{
  ...
  mock_reset((uint32_t)k_test_arb_stored);
  s_mock_read_result = k_ra_err_hw_error; /* inject a backing-store read fault */
  TEST_ASSERT_EQ(k_ra_err_hw_error,
                 ra_rot_antirollback_verify(&s_mock_store, (uint32_t)k_test_arb_newer));
