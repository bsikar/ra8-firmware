/**
 * @file test_ra_i3c_i2c_peripheral.c
...
/**
 * @enum ra_i3c_i2c_peripheral_test_const_t
 * @brief Numeric constants used by the tests.
 */
typedef enum : uint32_t {
  k_ra_i3c_i2c_peripheral_test_ch0         = 0U,
  k_ra_i3c_i2c_peripheral_test_ch_oor      = 9U,
  k_ra_i3c_i2c_peripheral_test_addr_7b     = 0x42U,
  k_ra_i3c_i2c_peripheral_test_byte_a      = 0xA5U,
  k_ra_i3c_i2c_peripheral_test_byte_b      = 0x5AU,
  k_ra_i3c_i2c_peripheral_test_ntst_tdbef0 = 0x01U,
  k_ra_i3c_i2c_peripheral_test_ntst_rdbff0 = 0x02U,
} ra_i3c_i2c_peripheral_test_const_t;

static const ra_i3c_i2c_peripheral_cfg_t k_cfg = {
  .peripheral_addr_7b = (uint8_t)k_ra_i3c_i2c_peripheral_test_addr_7b,
  .general_call       = 1U,
};

static void prearm(volatile r_i3c_i2c_regs_t* reg)
{
  reg->NTST = (uint32_t)((uint32_t)k_ra_i3c_i2c_peripheral_test_ntst_tdbef0 |
                         (uint32_t)k_ra_i3c_i2c_peripheral_test_ntst_rdbff0);
}
 ...
 * Vector: cfg=NULL -> dec T (k_ra_err_null_ptr). Companion test
...
static void test_open_null(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    internal_i3c_i2c_peripheral_open((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0, nullptr));
 ...
 *
 * @par MC/DC:
 * Exercises the ``ch >= k_ra_i3c_i2c_max_channels`` range-check decision in
 * ``internal_i3c_i2c_peripheral_open``. Single-condition decision: vector
 * ch=9 -> dec T (k_ra_err_invalid_arg); the in-range vector is provided
...
static void test_open_oor(void)
{
  ...
  TEST_ASSERT_EQ(
    ...
    internal_i3c_i2c_peripheral_open((uint8_t)k_ra_i3c_i2c_peripheral_test_ch_oor, &k_cfg));
...
static void test_open_close(void)
{
  ...
  ra_sim_mmap_reset();
  volatile r_i3c_i2c_regs_t* reg = i3c_i2c_regs((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0);
  TEST_ASSERT_EQ(
    k_ra_ok,
    internal_i3c_i2c_peripheral_open((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0, &k_cfg));
  TEST_ASSERT_EQ(((uint32_t)k_ra_i3c_i2c_peripheral_test_addr_7b << 1U), reg->MSDVAD);
  TEST_ASSERT(reg->BCTL & (uint32_t)k_ra_i3c_i2c_msk_bctl_buse);
  TEST_ASSERT_EQ(k_ra_ok,
                 internal_i3c_i2c_peripheral_close((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0));
 ...
 * dec T) and the dedicated ``test_mcdc_ra_i3c_i2c_peripheral`` block
...
static void test_send_ok(void)
{
  ...
  volatile r_i3c_i2c_regs_t* reg = i3c_i2c_regs((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0);
  prearm(reg);
  const uint8_t data[1] = {(uint8_t)k_ra_i3c_i2c_peripheral_test_byte_a};
  TEST_ASSERT_EQ(
    k_ra_ok,
    internal_i3c_i2c_peripheral_send((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0, data, 1U));
  TEST_ASSERT_EQ(k_ra_i3c_i2c_peripheral_test_byte_a, (reg->NTDTBP0 & 0xFFU));
 ...
 * returns k_ra_err_null_ptr). Pairs with ``test_send_ok`` to flip the
 * data condition and with ``test_mcdc_ra_i3c_i2c_peripheral`` to flip
 * len. N+1 = 3 jointly.
 */
static void test_send_null(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    internal_i3c_i2c_peripheral_send((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0, nullptr, 1U));
 ...
 * Companion vectors live in ``test_mcdc_ra_i3c_i2c_peripheral``
...
static void test_receive_ok(void)
{
  ...
  volatile r_i3c_i2c_regs_t* reg = i3c_i2c_regs((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0);
  prearm(reg);
  reg->NTDTBP0   = (uint32_t)k_ra_i3c_i2c_peripheral_test_byte_b;
  uint8_t buf[1] = {0U};
  TEST_ASSERT_EQ(
    k_ra_ok,
    internal_i3c_i2c_peripheral_receive((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0, buf, 1U));
  TEST_ASSERT_EQ(k_ra_i3c_i2c_peripheral_test_byte_b, buf[0]);
 ...
 *   (a) ``ch >= k_ra_i3c_i2c_max_channels`` (range)
 *   (b) ``mask == nullptr`` (precondition)
 * No compound boolean in the unit under test. Vector:
 * ch=0, mask=valid -> dec F (returns k_ra_ok with flag bits set);
 * second call passes mask=NULL -> dec T (k_ra_err_null_ptr). N+1 = 2
...
static void test_status(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    internal_i3c_i2c_peripheral_status((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0, &mask));
  TEST_ASSERT(mask & (uint8_t)k_ra_i3c_i2c_peripheral_status_tx_empty);
  TEST_ASSERT(mask & (uint8_t)k_ra_i3c_i2c_peripheral_status_rx_full);
  TEST_ASSERT(mask & (uint8_t)k_ra_i3c_i2c_peripheral_status_nack);

  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    internal_i3c_i2c_peripheral_status((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0, nullptr));
...
static void test_mcdc_ra_i3c_i2c_peripheral(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    internal_i3c_i2c_peripheral_send((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0, nullptr, 0U));
  prearm(reg);
  TEST_ASSERT_EQ(
    k_ra_ok,
    internal_i3c_i2c_peripheral_send((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0, data, 1U));
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    internal_i3c_i2c_peripheral_receive((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0, nullptr, 0U));
  prearm(reg);
  reg->NTDTBP0 = (uint32_t)k_ra_i3c_i2c_peripheral_test_byte_b;
  TEST_ASSERT_EQ(
    k_ra_ok,
  ...
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    internal_i3c_i2c_peripheral_receive((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0, nullptr, 1U));
  (void)internal_i3c_i2c_peripheral_close((uint8_t)k_ra_i3c_i2c_peripheral_test_ch0);
...
int main(void)
{
  ...
  test_receive_ok();
  test_status();
  test_mcdc_ra_i3c_i2c_peripheral();
  return 0;
}
