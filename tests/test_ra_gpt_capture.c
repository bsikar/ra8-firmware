/**
 * @file test_ra_gpt_capture.c
 * @brief Unit tests for the GPT input-capture and external-event-count API.
 *
 * @details
 * Exercises ``ra_gpt_capture_configure`` / ``ra_gpt_capture_read`` (input
 * capture via GTICASR / GTICBSR -> GTCCRA / GTCCRB) and
 * ``ra_gpt_event_count_configure`` (external edge counting via GTUPSR /
 * GTDNSR). Each register write is observed through the simulated MMIO
 * window provided by ``ra_sim_mmap``; the two compound argument guards
...
/**
 * @enum ra_gpt_cap_test_channel_t
 * @brief Channel ids used by the capture / event-count tests.
 */
typedef enum : uint8_t {
  k_ra_gpt_cap_test_channel_valid = 0U,   /**< First valid channel. */
  k_ra_gpt_cap_test_channel_last  = 13U,  /**< Last valid channel.  */
  k_ra_gpt_cap_test_channel_bad   = 14U,  /**< One past the last.   */
  k_ra_gpt_cap_test_channel_huge  = 200U, /**< Far out of range.    */
} ra_gpt_cap_test_channel_t;

/**
 * @enum ra_gpt_cap_test_const_t
 * @brief Capture / count masks and latched values used by the tests.
 */
typedef enum : uint32_t {
  k_ra_gpt_cap_test_latched_a  = 0x0001ABCDUL, /**< GTCCRA capture value.        */
  k_ra_gpt_cap_test_latched_b  = 0x00027654UL, /**< GTCCRB capture value.        */
  k_ra_gpt_cap_test_reserved   = 0x80000000UL, /**< Bit 31: reserved/illegal.    */
  k_ra_gpt_cap_test_gtwp_lock  = 0x0000A501UL, /**< GTWP key with WP re-locked.  */
  k_ra_gpt_cap_test_bad_select = 9U,           /**< Out-of-range ra_gpt_ccr_sel. */
} ra_gpt_cap_test_const_t;

/**
 * @brief Reset the simulated MMIO window and the MSTP model before a test.
 *
 * @details Mirrors the ``prep_w35`` helper in ``test_ra_gpt.c``: a clean
...
static void test_capture_configure_a_writes_gticasr(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_capture_configure((uint8_t)k_ra_gpt_cap_test_channel_valid,
                                          k_ra_gpt_ccr_a,
...
static void test_capture_configure_b_writes_gticbsr(void)
{
  ...
  volatile r_gpt_channel_regs_t* reg = ra_gpt((uint8_t)k_ra_gpt_cap_test_channel_last);
...
static void test_capture_configure_none_disables(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_capture_configure((uint8_t)k_ra_gpt_cap_test_channel_valid,
                                          k_ra_gpt_ccr_a,
                                          (uint32_t)k_ra_gpt_cap_src_none));
  volatile r_gpt_channel_regs_t* reg = ra_gpt((uint8_t)k_ra_gpt_cap_test_channel_valid);
  TEST_ASSERT_EQ(0U, reg->GTICASR);
...
static void test_capture_configure_bad_channel(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_gpt_capture_configure((uint8_t)k_ra_gpt_cap_test_channel_bad,
                                          k_ra_gpt_ccr_a,
                                          (uint32_t)k_ra_gpt_cap_src_ioca_rising));
...
static void test_capture_read_a_and_b(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_gpt_capture_read((uint8_t)k_ra_gpt_cap_test_channel_valid, k_ra_gpt_ccr_a, &value_a));
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_gpt_capture_read((uint8_t)k_ra_gpt_cap_test_channel_valid, k_ra_gpt_ccr_b, &value_b));
  TEST_ASSERT_EQ(k_ra_gpt_cap_test_latched_a, value_a);
  TEST_ASSERT_EQ(k_ra_gpt_cap_test_latched_b, value_b);
...
static void test_capture_read_arg_guards(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_gpt_capture_read((uint8_t)k_ra_gpt_cap_test_channel_valid, k_ra_gpt_ccr_a, nullptr));
  /* Out-of-range channel. */
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_gpt_capture_read((uint8_t)k_ra_gpt_cap_test_channel_huge, k_ra_gpt_ccr_a, &value));
  /* Out-of-range selector. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_gpt_capture_read((uint8_t)k_ra_gpt_cap_test_channel_valid,
                                     (ra_gpt_ccr_sel_t)k_ra_gpt_cap_test_bad_select,
                                     &value));
...
static void test_event_count_configure_writes_up_down(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_event_count_configure((uint8_t)k_ra_gpt_cap_test_channel_valid,
...
static void test_event_count_configure_none_disables(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_event_count_configure((uint8_t)k_ra_gpt_cap_test_channel_valid,
                                              (uint32_t)k_ra_gpt_cnt_src_gtetrga_rising,
                                              (uint32_t)k_ra_gpt_cnt_src_gtetrgb_falling));
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_event_count_configure((uint8_t)k_ra_gpt_cap_test_channel_valid,
...
static void test_event_count_configure_bad_channel(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_gpt_event_count_configure((uint8_t)k_ra_gpt_cap_test_channel_bad,
                                              (uint32_t)k_ra_gpt_cnt_src_gtetrga_rising,
                                              (uint32_t)k_ra_gpt_cnt_src_none));
 ...
 *
 * @par MC/DC:
 * Decision: `if (((uint8_t)which > (uint8_t)k_ra_gpt_ccr_b) ||
 *               (((source_mask & ~k_ra_gpt_cap_src_valid_mask)) != 0U))`
 * (2 conditions, in ra_gpt_capture_configure)
...
static void test_mcdc_capture_configure_arg_guard(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_gpt_capture_configure((uint8_t)k_ra_gpt_cap_test_channel_valid,
                                          k_ra_gpt_ccr_a,
                                          (uint32_t)k_ra_gpt_cap_test_reserved));
 ...
 *
 * @par MC/DC:
 * Decision: `if ((((up_source & ~k_ra_gpt_cnt_src_valid_mask)) != 0U) ||
 *               (((down_source & ~k_ra_gpt_cnt_src_valid_mask)) != 0U))`
 * (2 conditions, in ra_gpt_event_count_configure)
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_gpt_capture.c\n");
  return 0;
}
