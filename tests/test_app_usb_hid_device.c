 ...
 *   1. ra_usb_pal_init(speed=full)
 *   2. ra_usb_pal_set_event_handler(...)
 *   3. ra_usb_pal_attach(true)         -- D+ pull-up on
 *   4. (host issues bus reset; mock fires k_ra_usb_pal_event_reset)
 *   5. ra_usb_pal_ep_open(EP1 IN intr) -- HID report endpoint
 *   6. ra_usb_pal_ep_send(report)      -- first INT-IN report
 *
...
static ra_usb_speed_t s_last_event_speed;

/**
 * @brief Mock USB event handler -- mirrors USBX DCD bridge.
 */
static void test_usb_event_cb(void* ctx, ra_usb_speed_t speed, uint16_t event_mask)
{
  (void)ctx;
...
static void reset_world(void)
{
  ...
  s_last_event_speed = k_ra_usb_speed_fs;
}
 ...
 *
 * @par MC/DC:
 * Decision under test: ``ra_usb_pal_ep_open() != ok`` -- 4 atomic
 ...
 *
 * @par MC/DC:
 * Decision under test: ``ra_usb_pal_ep_send() != ok``. Two atomic
 ...
 *
 * @par MC/DC:
 * Decision under test: ``ra_usb_pal_set_event_handler() != ok``.
...
static void test_usb_ep_open_before_init_rejected(void)
{
  ...
  /* Deliberately skip ra_usb_pal_init. */
  ra_err_t err = ra_usb_pal_ep_open((uint8_t)k_test_usb_ep_hid_in,
                                    k_ra_usb_pal_ep_dir_in,
                                    k_ra_usb_pal_ep_type_intr,
                                    (uint16_t)k_test_usb_hid_max_packet);
  TEST_ASSERT(err != k_ra_ok);
