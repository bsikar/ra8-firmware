/**
 * @file test_ra_io_stream.c
 * @brief Unit tests for the ra_io targetable byte-stream layer (issue #157).
 *
 * @details
 ...
 * validation, and the `ra_log` -> stream redirect captured into a RAM buffer.
 *
...
static void test_ram_sink(void)
{
  ...
  ra_io_stream_t           s                        = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_ram_init(&s, &st, buf, k_t_ram_cap));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_puts(&s, "n="));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_put_u32(&s, 42U));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_putc(&s, '\n'));
  uint32_t used = 0;
...
static void test_ram_sink_full(void)
{
  ...
  ra_io_stream_ram_state_t st     = {};
  ra_io_stream_t           s      = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_ram_init(&s, &st, buf, 2));
  TEST_ASSERT_EQ(k_ra_err_no_mem, ra_io_stream_puts(&s, "abc"));
  uint32_t used = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_ram_used(&st, &used));
...
static void test_formatting(void)
{
  ...
  ra_io_stream_t           s                        = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_ram_init(&s, &st, buf, k_t_ram_cap));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_put_u32(&s, 0U));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_putc(&s, ' '));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_put_u32(&s, 4294967295U));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_putc(&s, ' '));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_put_hex(&s, 0xDEADBEEFU, 8U));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_putc(&s, ' '));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_put_hex(&s, 0xABU, 4U));
  uint32_t used = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_ram_used(&st, &used));
  TEST_ASSERT(memcmp(buf, "0 4294967295 deadbeef 00ab", 26) == 0);
  /* min_digits range */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_io_stream_put_hex(&s, 1U, 0U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_io_stream_put_hex(&s, 1U, 9U));
  TEST_END("formatting");
}
...
static void test_blockdev_sink(void)
{
  ...
  static uint8_t             disk[(size_t)k_t_bd_blocks * (size_t)k_ra_io_block_size_bytes];
  ra_io_blockdev_ram_state_t bstate = {};
  ra_io_blockdev_t           bd     = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_ram_init(&bd, &bstate, disk, k_t_bd_blocks, false));

  ra_io_stream_blockdev_state_t sstate = {};
  ra_io_stream_t                s      = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_blockdev_init(&s, &sstate, &bd, 0));

  uint8_t payload[(size_t)k_t_stream_bytes];
  for (uint32_t i = 0; i < (uint32_t)k_t_stream_bytes; ++i) {
    payload[i] = (uint8_t)((i * 3u + 1u) & 0xFFu);
  }
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_write(&s, payload, k_t_stream_bytes, nullptr));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_flush(&s));

  uint8_t blk0[(size_t)k_ra_io_block_size_bytes] = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_read(&bd, 0, 1, blk0));
  TEST_ASSERT(memcmp(blk0, payload, (size_t)k_t_first_chunk) == 0);

  uint8_t blk1[(size_t)k_ra_io_block_size_bytes] = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_read(&bd, 1, 1, blk1));
  TEST_ASSERT(memcmp(blk1, &payload[(size_t)k_t_first_chunk], (size_t)k_t_tail_chunk) == 0);
  /* tail of the second sector is zero-padded */
  for (uint32_t i = (uint32_t)k_t_tail_chunk; i < (uint32_t)k_ra_io_block_size_bytes; ++i) {
...
static void test_same_call_two_targets(void)
{
  ...
  ra_io_stream_ram_state_t rst                       = {};
  ra_io_stream_t           a                         = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_ram_init(&a, &rst, rbuf, k_t_ram_cap));

  static uint8_t             disk[(size_t)k_ra_io_block_size_bytes];
  ra_io_blockdev_ram_state_t bstate = {};
  ra_io_blockdev_t           bd     = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_ram_init(&bd, &bstate, disk, 1, false));
  ra_io_stream_blockdev_state_t sstate = {};
  ra_io_stream_t                b      = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_blockdev_init(&b, &sstate, &bd, 0));

  const char* msg = "hello";
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_puts(&a, msg));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_puts(&b, msg));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_flush(&b));
  TEST_ASSERT(memcmp(rbuf, msg, 5) == 0);
  uint8_t blk[(size_t)k_ra_io_block_size_bytes] = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_read(&bd, 0, 1, blk));
  TEST_ASSERT(memcmp(blk, msg, 5) == 0);
  TEST_END("one call, two targets");
}

/* =============================================================================
 * uart / usb-cdc bind, validation, ra_log redirect
 * =============================================================================
 */
...
static void test_uart_usbcdc_bind(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_stream_uart_init(nullptr, &ust, 8));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_uart_init(&s, &ust, 8));
  ra_io_stream_t              s2  = {};
  ra_io_stream_usbcdc_state_t cst = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_stream_usbcdc_init(nullptr, &cst, 0x81));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_usbcdc_init(&s2, &cst, 0x81));
...
static void test_stream_validation(void)
{
  ...
  ra_io_stream_t unbound = {}; /* iface == nullptr */
  uint8_t        b       = 'x';
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_io_stream_write(&unbound, &b, 1, nullptr));
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_io_stream_flush(&unbound));
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_io_stream_putc(&unbound, 'y'));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_stream_write(nullptr, &b, 1, nullptr));
  uint8_t                  buf[(size_t)k_t_ram_cap] = {};
  ra_io_stream_ram_state_t st                       = {};
  ra_io_stream_t           s                        = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_ram_init(&s, &st, buf, k_t_ram_cap));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_stream_write(&s, nullptr, 1, nullptr));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_stream_puts(&s, nullptr));
  TEST_END("stream validation");
}

/**
 * @par MC/DC:
 * (no compound decisions under test -- attach a stream to ra_log, emit a line,
 * confirm the capture buffer holds the tag and message, then detach)
 */
static void test_ra_log_redirect(void)
{
  TEST_BEGIN("ra_log redirect");
  ...
  ra_log_init();
  ra_log_error("TST", "boom");
  ra_io_log_detach();
  uint32_t used = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_ram_used(&st, &used));
  TEST_ASSERT(used > 0u);
  buf[used] = '\0';
  TEST_ASSERT(strstr(buf, "TST") != nullptr);
  TEST_ASSERT(strstr(buf, "boom") != nullptr);
  TEST_END("ra_log redirect");
}
...
int32_t main(void)
{
  ...
  test_ra_log_redirect();
  (void)fprintf(stderr, "[OK  ] test_ra_io_stream.c\n");
  return 0;
}
