/**
 * @file test_ra_wdt_supervisor_cov.c
 * @brief Coverage-lift tests for libs/ra_wdt_supervisor/src/ra_wdt_supervisor.c
 *
 * @par Tag
 * [Test / Host] {World: N/A}
 *
 ...
 *    initialising without calling ``ra_wdt_supervisor_set_now_hook`` and
 *    then calling ``ra_wdt_supervisor_register_thread``, which routes
 *    through ``internal_fill_slot`` -> ``s_state.now()``.  A subsequent
 *    ``ra_wdt_supervisor_tick`` also calls ``s_state.now()`` via the same
 ...
 *    initialising without calling ``ra_wdt_supervisor_set_refresh_hook``,
 *    registering one alive worker, and calling ``ra_wdt_supervisor_tick``.
 *    ``ra_wdt_refresh_deferred`` writes to the WDT peripheral register at
 ...
 * non-zero status code.  The host shim (``ra_wdt_sup_tx_shim.h``) provides
 ...
 * @since 0.1.0
 */
static uint8_t s_cov_stack[1024];
 ...
 * its deadline when ``ra_wdt_supervisor_tick`` evaluates the registry.
 *
 ...
 * invoke the real ``ra_wdt_refresh_deferred`` and its register writes.
 ...
 *
 * @details
 * After ``ra_wdt_supervisor_init`` the module holds
 * ``s_state.now = internal_default_now``.  A call to
 * ``ra_wdt_supervisor_register_thread`` routes through
 ...
 *
 * A subsequent ``ra_wdt_supervisor_tick`` also dispatches through
 ...
 *       calls ``ra_wdt_supervisor_deinit`` at the top of its own body.
...
static void test_default_now_hook_called_by_register(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_wdt_supervisor_set_refresh_hook(hook_cov_noop_refresh));

  uint8_t h = (uint8_t)k_ra_wdt_sup_handle_invalid;
  /* register_thread -> internal_fill_slot -> s_state.now() = internal_default_now(). */
  TEST_ASSERT_EQ(k_ra_ok, ra_wdt_supervisor_register_thread("cov_t1", 200U, &h));
  TEST_ASSERT_EQ(0U, h);
  TEST_ASSERT_EQ(1U, ra_wdt_supervisor_thread_count());
 ...
 * ``ra_wdt_supervisor_tick`` evaluates the registry and finds all threads
 ...
 * calls ``ra_wdt_refresh_deferred``.  This covers source lines 117, 119,
 * and 120.
 *
 ...
 *
 * ``ra_wdt_refresh_deferred`` writes to the WDT peripheral WDTRR register
 * at 0x40202600.  The ``ra_sim_mmap`` constructor maps the full peripheral
...
static void test_default_refresh_hook_called_by_tick(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_wdt_supervisor_init(&cfg));
  TEST_ASSERT_EQ(k_ra_ok, ra_wdt_supervisor_set_now_hook(hook_cov_now));
  /* Intentionally skip set_refresh_hook: s_state.refresh == internal_default_refresh. */

  uint8_t h = (uint8_t)k_ra_wdt_sup_handle_invalid;
  TEST_ASSERT_EQ(k_ra_ok, ra_wdt_supervisor_register_thread("cov_t2", 200U, &h));
  TEST_ASSERT_EQ(k_ra_ok, ra_wdt_supervisor_checkin(h));

  /* tick: any_present=T, all_alive=T (gap=0 <= 200) -> will_refresh=T.
   * s_state.refresh() == internal_default_refresh() -> ra_wdt_refresh_deferred(). */
  bool did_refresh = false;
  TEST_ASSERT_EQ(k_ra_ok, ra_wdt_supervisor_tick(&did_refresh));
  TEST_ASSERT_EQ(1, did_refresh);
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_wdt_supervisor_cov.c\n");
  return 0;
}
