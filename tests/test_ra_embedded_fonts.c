/**
 * @file test_ra_embedded_fonts.c
 * @brief #109 foundation: enumerate + extract an EPUB-embedded font (ra_epub)
 *        and bind it as the reflow engine's active face (ra_reflow).
 *
 * @details
 ...
 *   1. enumerate (`ra_epub_get_embedded_font_count`) + extract
 *      (`ra_epub_get_embedded_font`) the font and CRC-check the bytes;
 *   2. bind the extracted bytes into a reflow engine (`ra_reflow_bind_font`),
...
/**
 * @test ra_epub enumerates the manifest font item and extracts its bytes.
 */
static void test_enumerate_and_extract(void)
{
  ...
  ra_epub_book_t            book  = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_epub_open(&media, "fonts.epub", &book));
  ...
  TEST_ASSERT_EQ(
    ...
    ra_epub_get_embedded_font(&book, 1U, s_extracted_font, sizeof(s_extracted_font), &over));
  (void)ra_epub_close(&book);
...
static int32_t priv_first_advance(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_reflow_layout_chapter(&s_engine, (const uint8_t*)k_chapter, strlen(k_chapter), &pages));
  TEST_ASSERT(s_engine.glyph_count >= (uint32_t)k_tef_min_m_glyphs);
  return s_engine.glyphs[1].x - s_engine.glyphs[0].x;
}

/**
 * @test ra_reflow_bind_font binds the embedded face; a laid-out glyph is
 ...
 *   (2 conditions, libs/ra_reflow/src/ra_reflow_layout.c@ra_reflow_bind_font)
...
static void test_bind_measure_and_mcdc(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_reflow_init((uint16_t)k_tef_viewport_w,
  ...
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_reflow_bind_font(&s_engine, garbage, sizeof(garbage)));
  TEST_ASSERT(s_engine.font_data == s_extracted_font); /* unchanged */
  TEST_ASSERT_EQ(k_ra_err_not_supported,
                 ra_reflow_bind_font(&s_engine, tagged_corrupt, sizeof(tagged_corrupt)));
  TEST_ASSERT(s_engine.font_data == s_extracted_font); /* unchanged */

  /* Single-condition guards. */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_reflow_bind_font(nullptr, s_extracted_font, got));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_reflow_bind_font(&s_engine, nullptr, got));
  TEST_ASSERT_EQ(
    k_ra_err_invalid_size,
    ra_reflow_bind_font(&s_engine, s_extracted_font, (size_t)k_ra_reflow_min_font_bytes - 1U));
