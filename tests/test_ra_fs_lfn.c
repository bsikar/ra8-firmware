/**
 * @file test_ra_fs_lfn.c
 * @brief VFAT long-filename (LFN) read support for ra_fs (#101).
 *
 * @details
 ...
 * 8.3 alias `MYBOOK~1.EPU`. Verifies that ra_fs_open resolves the file by its
 * long name *and* by its 8.3 alias, that a wrong name still misses, and that
 * ra_fs_listdir reports the long name. The 8.3-only behaviour is unchanged
 * (covered by tests/test_ra_fs_fat.c).
 *
 ...
 *      decisions are covered in tests/test_ra_fs_fat.c.
 */
static void test_lfn_open_by_long_and_short(void)
{
  TEST_BEGIN("ra_fs LFN: open by long name + 8.3 alias");
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(mount, "mybook.epub", k_ra_fs_mode_read, &f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));
  /* Case-insensitive long-name match. */
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(mount, "MyBook.EPUB", k_ra_fs_mode_read, &f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));
  /* The 8.3 alias still opens the same entry. */
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(mount, "MYBOOK~1.EPU", k_ra_fs_mode_read, &f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));
  /* A different long name misses. */
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_fs_open(mount, "other.epub", k_ra_fs_mode_read, &f));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(mount));
  free_volume();
  TEST_END("ra_fs LFN: open by long name + 8.3 alias");
}

/**
 * @test test_lfn_listdir_reports_long_name
 * @brief ra_fs_listdir reports the reassembled long name, not the 8.3 alias.
 *
 * @par MC/DC: not applicable -- enumeration acceptance path.
 */
static void test_lfn_listdir_reports_long_name(void)
{
  TEST_BEGIN("ra_fs LFN: listdir reports the long name");
  build_volume_with_lfn();
  ra_fs_mount_t* mount = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &mount));

  listdir_capture_t cap = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_listdir(mount, "/", capture_cb, &cap));
  TEST_ASSERT_EQ(1U, cap.count);
  TEST_ASSERT_EQ(0, strcmp(cap.last, "mybook.epub"));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(mount));
  free_volume();
  TEST_END("ra_fs LFN: listdir reports the long name");
}
