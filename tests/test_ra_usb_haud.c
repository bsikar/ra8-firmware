...
static void walk_to_attach(void)
{
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_usb_haud_attach_callback(stub_on_attach, (void*)k_test_haud_ctx_token));
  for (uint8_t i = 0U; i < k_test_haud_max_steps; ++i) {
    ...
    ra_usb_fs()->DCPCTR = 0U;
    TEST_ASSERT_EQ(k_ra_ok, ra_usb_haud_step());
  }
...
static void test_init_fs_returns_ok(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_haud_init(k_ra_usb_speed_fs));
...
static void test_init_bad_speed(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_haud_init((ra_usb_speed_t)9U));
...
static void test_close_without_init(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_haud_close());
...
static void test_pre_init_guards(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_haud_attach_callback(stub_on_attach, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_haud_step());

  uint8_t  buf[8] = {};
  uint16_t got    = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_haud_set_format(2U, 16U, 48000U));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_haud_set_volume(0U, 0));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_haud_set_mute(0U, true));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_haud_send_samples(buf, (uint16_t)sizeof(buf)));
  TEST_ASSERT_EQ(k_ra_err_invalid_state,
                 ra_usb_haud_recv_samples(buf, (uint16_t)sizeof(buf), &got));
...
static void test_pre_attach_guards(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_haud_set_format(2U, 16U, 48000U));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_haud_set_volume(0U, 0));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_haud_set_mute(0U, false));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_haud_send_samples(buf, (uint16_t)sizeof(buf)));
  TEST_ASSERT_EQ(k_ra_err_invalid_state,
                 ra_usb_haud_recv_samples(buf, (uint16_t)sizeof(buf), &got));
...
static void test_null_arg_rejection(void)
{
  ...
  uint8_t  buf[8] = {};
  uint16_t got    = 0U;
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_haud_send_samples(nullptr, (uint16_t)sizeof(buf)));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_haud_recv_samples(nullptr, (uint16_t)sizeof(buf), &got));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_haud_recv_samples(buf, (uint16_t)sizeof(buf), nullptr));
...
static void test_set_format_setup_envelope(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_haud_set_format(2U, 24U, 96000U));
  /* USBREQ low byte = bmRequestType, high byte = bRequest.
   * For SET_CUR endpoint: 0x01 << 8 | 0x22 = 0x0122. */
  TEST_ASSERT_EQ(0x0122U, ra_usb_fs()->USBREQ);
  /* USBVAL = SAMPLING_FREQ_CONTROL << 8 = 0x0100. */
  TEST_ASSERT_EQ(0x0100U, ra_usb_fs()->USBVAL);
  /* USBLENG = 3 (24-bit LE sample rate payload). */
  TEST_ASSERT_EQ(3U, ra_usb_fs()->USBLENG);
...
static void test_set_volume_setup_envelope(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_haud_set_volume(0U, (int16_t)-3072));
  /* USBREQ = (0x01 << 8) | 0x21 = 0x0121. */
  TEST_ASSERT_EQ(0x0121U, ra_usb_fs()->USBREQ);
  /* USBVAL = (VOLUME_CONTROL << 8) | channel = 0x0200. */
  TEST_ASSERT_EQ(0x0200U, ra_usb_fs()->USBVAL);
  /* USBINDX = (FU id 2 << 8) | AC iface 0 = 0x0200. */
  TEST_ASSERT_EQ(0x0200U, ra_usb_fs()->USBINDX);
  /* USBLENG = 2 (signed 16-bit dB payload). */
  TEST_ASSERT_EQ(2U, ra_usb_fs()->USBLENG);
...
static void test_set_mute_setup_envelope(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_haud_set_mute(0U, true));
  /* USBREQ = (0x01 << 8) | 0x21 = 0x0121. */
  TEST_ASSERT_EQ(0x0121U, ra_usb_fs()->USBREQ);
...
static void test_iso_invalid_args(void)
{
  ...
  uint8_t  buf[8] = {};
  uint16_t got    = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_haud_send_samples(buf, 0U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_haud_recv_samples(buf, 0U, &got));
...
static void test_mcdc_haud(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_haud_init(k_ra_usb_speed_fs));
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_haud_init(k_ra_usb_speed_hs));
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_haud_init((ra_usb_speed_t)9U));
  ...
  uint8_t buf[16] = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_haud_send_samples(nullptr, 4U));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_haud_send_samples(nullptr, 0U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_haud_send_samples(buf, 0U));

  const ra_err_t c_in = ra_usb_haud_set_format(2U, 16U, 48000U);
  TEST_ASSERT(c_in != k_ra_err_invalid_arg);
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_haud_set_format(0U, 16U, 48000U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_haud_set_format(9U, 16U, 48000U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_haud_set_format(2U, 4U, 48000U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_haud_set_format(2U, 64U, 48000U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_haud_set_format(2U, 16U, 4000U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_haud_set_format(2U, 16U, 384000U));
