/**
 * @file test_ra_jpeg_sw_cov.c
 * @brief Coverage supplement for ra_jpeg_sw.c -- entropy-coder
 *        primitives and ra_jpeg_sw_get_dimensions() error paths.
 *
 * @details
 * The main test_ra_jpeg_sw.c exercises ra_jpeg_sw.c via the full
 ...
 * directly:
 *
 *   br_fill / ra_jpeg_sw_br_get_bits:
 ...
 * vector across the test suite (either here or in test_ra_jpeg_sw.c).
 *
...
/* ------------------------------------------------------------------ */
/* Bit-reader helpers: br_fill / ra_jpeg_sw_br_get_bits */
/* ------------------------------------------------------------------ */
 ...
 *
 * @details
 * Calls ra_jpeg_sw_br_get_bits() with a zero-length buffer and
 ...
 * nbits stays 0, so ra_jpeg_sw_br_get_bits returns -1.
 ...
 *        (exercised by the full decode round-trip in test_ra_jpeg_sw.c).
 * N+1 = 2.
 * @since 0.1.0
 */
 ...
 * ra_jpeg_sw_br_get_bits(br, 4) finds nbits(0) < n(4), so the
...
static void test_br_get_bits_refill_and_succeed(void)
{
  ...
  br.buf                     = buf;
  br.len                     = (uint32_t)sizeof buf;
  int32_t r                  = ra_jpeg_sw_br_get_bits(&br, 4U);
  TEST_ASSERT_EQ(0, r);
  TEST_END("jpeg_sw_cov br_get_bits: refill branch succeeds (line 161)");
}

/* ------------------------------------------------------------------ */
/* ra_jpeg_sw_htab_decode error paths */
/* ------------------------------------------------------------------ */
...
static void test_htab_decode_empty_stream(void)
{
  ...
  ra_jpeg_htab_t      h  = {};
  ra_jpeg_bitreader_t br = {};
  int32_t             r  = ra_jpeg_sw_htab_decode(&br, &h);
  TEST_ASSERT_EQ(-1, r);
...
static void test_htab_decode_underflow_in_loop(void)
{
   ...
   * ra_jpeg_sw_htab_build leaves every maxcode entry at -1 when
...
/* ------------------------------------------------------------------ */
/* ra_jpeg_sw_get_dimensions guard returns */
/* ------------------------------------------------------------------ */
 ...
 * true and the function returns k_ra_err_invalid_size (line 495).
 *
 * @par MC/DC:
...
static void test_get_dim_too_short(void)
{
  ...
  uint16_t             w     = 0U;
  uint16_t             h     = 0U;
  ra_err_t             e     = ra_jpeg_sw_get_dimensions(buf, 3U, &w, &h);
...
static void test_get_dim_non_marker_byte(void)
{
  ...
  uint16_t             w     = 0U;
  uint16_t             h     = 0U;
  ra_err_t             e     = ra_jpeg_sw_get_dimensions(buf, (uint32_t)sizeof buf, &w, &h);
 ...
 * true and the function returns k_ra_err_protocol_error (line 511).
 *
 * @par MC/DC:
 ...
 * true, so the function returns k_ra_err_protocol_error (line 520).
 *
 * @par MC/DC:
 ...
 * k_ra_err_protocol_error (line 528).
 *
 * @par MC/DC:
 ...
 * k_ra_err_not_supported (line 532).
 *
 * @par MC/DC:
 ...
 *
 * @details
 * Runs all coverage-supplement tests for ra_jpeg_sw.c.  The
 * ra_sim_mmap_reset() call clears the RA8D2 MMIO backing store;
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_jpeg_sw_cov.c\n");
  return 0;
}
