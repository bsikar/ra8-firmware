 ...
 *       [internal_test_dims_scale_landscape]
 *
 ...
 *   - internal_test_dims_clamp_w_to_1: src 1x10000, max_edge 2 -> width rounds to 0 -> 1.
 *   - internal_test_dims_clamp_h_to_1: src 10000x1, max_edge 2 -> height rounds to 0 -> 1.
 *
 * Decision: `if (dst_w == 0 || dst_h == 0)` in ra8_rabook_gray4_downscale --
 *   2-condition short-circuit OR. N+1 = 3 vectors:
 *   - Vector 1 (control):    dst_w=2, dst_h=2 -> false (proceeds)   [internal_test_downscale_identity]
 *   - Vector 2 (vary dst_w): dst_w=0, dst_h=1 -> true -> invalid_arg [internal_test_downscale_zero_dst]
 *   - Vector 3 (vary dst_h): dst_w=1, dst_h=0 -> true -> invalid_arg [internal_test_downscale_zero_dst_h]
 ...
 *   N+1 = 3 vectors:
 *   - Vector 1 (control):    src_w=2, src_h=2 -> false (samples)            [internal_test_downscale_identity]
 *   - Vector 2 (vary src_w): src_w=0, src_h=2 -> true -> ok + zeroed dst    [internal_test_downscale_zero_src]
 *   - Vector 3 (vary src_h): src_w=2, src_h=0 -> true -> ok + zeroed dst    [internal_test_downscale_zero_src_h]
...
/**
 * @test internal_test_dims_no_scale
 * @par MC/DC:
 ...
 * varied-condition vectors V2/V3/V4 are in sibling tests internal_test_dims_zero_src,
 * internal_test_dims_zero_src_h, and internal_test_dims_zero_max_edge of this file). @brief Verify dims no scale behavior. @details Executes the dims no scale scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_dims_no_scale(void)
{
  uint16_t ow = 0U;
  uint16_t oh = 0U;
  ra8_rabook_gray4_output_dims(k_t_src_small_w, k_t_src_small_h, k_t_cap_generous, &ow, &oh);
  internal_check(ow == k_t_src_small_w && oh == k_t_src_small_h,
                 "dims: no scale when within max_edge");
}

/**
 * @test internal_test_dims_exact_edge
 * @par MC/DC:
 ...
 * `&&` or `||` in the code under test that this case touches) @brief Verify dims exact edge behavior. @details Executes the dims exact edge scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_dims_exact_edge(void)
{
  uint16_t ow = 0U;
  uint16_t oh = 0U;
  ra8_rabook_gray4_output_dims(k_t_cap_edge, k_t_fit_h, k_t_cap_edge, &ow, &oh);
  internal_check(ow == k_t_cap_edge && oh == k_t_fit_h, "dims: exact max_edge passes unchanged");
}

/**
 * @test internal_test_dims_scale_landscape
 * @par MC/DC:
 * (no compound decisions in this test -- exercises the single-condition
 ...
 * code under test that this case touches) @brief Verify dims scale landscape behavior. @details Executes the dims scale landscape scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_dims_scale_landscape(void)
{
  uint16_t ow = 0U;
  uint16_t oh = 0U;
  ra8_rabook_gray4_output_dims(k_t_land_w, k_t_land_h, k_t_cap_edge, &ow, &oh);
  internal_check(ow == k_t_cap_edge && oh == k_t_fit_h, "dims: 2x landscape scale down");
}

/**
 * @test internal_test_dims_scale_portrait
 * @par MC/DC:
 * (no compound decisions in this test -- exercises the single-condition
 ...
 * this case touches) @brief Verify dims scale portrait behavior. @details Executes the dims scale portrait scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_dims_scale_portrait(void)
{
  uint16_t ow = 0U;
  uint16_t oh = 0U;
  ra8_rabook_gray4_output_dims(k_t_port_w, k_t_port_h, k_t_cap_edge, &ow, &oh);
  internal_check(oh <= k_t_cap_edge, "dims: portrait longer edge clamped");
  internal_check(ow >= 1U, "dims: portrait width at least 1");
}

/**
 * @test internal_test_dims_null_out
 * @par MC/DC:
 ...
 * pointers). @brief Verify dims null out behavior. @details Executes the dims null out scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_dims_null_out(void)
{
  ...
  internal_check(ow == k_t_dim_poison && oh == k_t_dim_poison,
                 "dims: null out ptr leaves values unchanged");
}

/**
 * @test internal_test_dims_zero_src
 * @par MC/DC:
 * Decision: `src_w == 0 || src_h == 0 || max_edge == 0`
 * (3 conditions, function `ra8_rabook_gray4_output_dims`).
 ...
 * V1 and the other varied vectors V3/V4 are in sibling tests internal_test_dims_no_scale,
 * internal_test_dims_zero_src_h, and internal_test_dims_zero_max_edge of this file). @brief Verify dims zero src behavior. @details Executes the dims zero src scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_dims_zero_src(void)
{
  ...
  internal_check(ow == 0U && oh == 0U, "dims: zero src_w yields (0,0)");
}

/**
 * @test internal_test_dims_zero_src_h
 * @par MC/DC:
 * Decision: `src_w == 0 || src_h == 0 || max_edge == 0`
 * (3 conditions, function `ra8_rabook_gray4_output_dims`).
 * - V1 src_w=100, src_h=50,  max_edge=200 -> C1=F,C2=F,C3=F -> F (proceeds).
 ...
 * V1 and the other varied vectors V2/V4 are in sibling tests internal_test_dims_no_scale,
 * internal_test_dims_zero_src, and internal_test_dims_zero_max_edge of this file). @brief Verify dims zero src h behavior. @details Executes the dims zero src h scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_dims_zero_src_h(void)
{
  /*
  ...
  internal_check(ow == 0U && oh == 0U, "dims: zero src_h yields (0,0)");
}

/**
 * @test internal_test_dims_zero_max_edge
 * @par MC/DC:
 * Decision: `src_w == 0 || src_h == 0 || max_edge == 0`
 * (3 conditions, function `ra8_rabook_gray4_output_dims`).
 * - V1 src_w=100, src_h=50,  max_edge=200 -> C1=F,C2=F,C3=F -> F (proceeds).
 ...
 * internal_test_dims_no_scale, internal_test_dims_zero_src, and internal_test_dims_zero_src_h of this file). @brief Verify dims zero max edge behavior. @details Executes the dims zero max edge scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_dims_zero_max_edge(void)
{
  /*
  ...
  internal_check(ow == 0U && oh == 0U, "dims: zero max_edge yields (0,0)");
}

/**
 * @test internal_test_dims_clamp_w_to_1
 * @par MC/DC:
 * (no compound decisions in this test -- exercises the single-condition
 ...
 * `&&` or `||` in the code under test that this case touches) @brief Verify dims clamp w to 1 behavior. @details Executes the dims clamp w to 1 scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_dims_clamp_w_to_1(void)
{
  /*
  ...
  internal_check(ow == 1U, "dims: degenerate width clamped up to 1");
  internal_check(oh == 2U, "dims: degenerate-width height is 2");
}

/**
 * @test internal_test_dims_clamp_h_to_1
 * @par MC/DC:
 * (no compound decisions in this test -- exercises the single-condition
 ...
 * no `&&` or `||` in the code under test that this case touches) @brief Verify dims clamp h to 1 behavior. @details Executes the dims clamp h to 1 scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_dims_clamp_h_to_1(void)
{
  /*
  ...
  internal_check(ow == 2U, "dims: degenerate-height width is 2");
  internal_check(oh == 1U, "dims: degenerate height clamped up to 1");
}
...
/**
 * @test internal_test_encode_4px_exact_palette
 * @par MC/DC:
 ...
 * pixels; no `&&` or `||` in the code under test that this case touches) @brief Verify encode 4px exact palette behavior. @details Executes the encode 4px exact palette scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_encode_4px_exact_palette(void)
{
  /*
  ...
  internal_check(err == k_ra8_ok, "encode: exact palette 4px -- ok");
  internal_check(out_size == 2U, "encode: exact palette 4px -- size");
  internal_check(out[0] == 0x01U, "encode: exact palette 4px -- byte0");
  internal_check(out[1] == k_t_pack_2_3, "encode: exact palette 4px -- byte1");
}

/**
 * @test internal_test_encode_3px_odd
 * @par MC/DC:
 ...
 * zero-padded; no `&&` or `||` in the code under test that this case touches) @brief Verify encode 3px odd behavior. @details Executes the encode 3px odd scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_encode_3px_odd(void)
{
  /*
   * 3 pixels: v = 0, 85, 255
   * nib[0] = (0   + 8) / 17 = 0
  ...
  internal_check(err == k_ra8_ok, "encode: odd pixel count -- ok");
  internal_check(out_size == 2U, "encode: odd pixel count -- size");
  internal_check(out[0] == k_t_pack_0_1, "encode: odd pixel count -- byte0");
  internal_check(out[1] == k_t_pack_15_pad, "encode: odd pixel count -- byte1 (low nibble zero)");
}

/**
 * @test internal_test_encode_1px
 * @par MC/DC:
 ...
 * that this case touches) @brief Verify encode 1px behavior. @details Executes the encode 1px scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_encode_1px(void)
{
  ...
  internal_check(err == k_ra8_ok, "encode: single pixel -- ok");
  internal_check(out_size == 1U, "encode: single pixel -- size");
  internal_check(out[0] == k_t_pack_8_pad, "encode: single pixel -- byte (nib=8)");
}

/**
 * @test internal_test_encode_0px
 * @par MC/DC:
 * (no compound decisions in this test -- exercises the single-condition
 * early-out `if (n_pixels == 0U)` that returns 0 bytes for an empty image; no
 * `&&` or `||` in the code under test that this case touches) @brief Verify encode 0px behavior. @details Executes the encode 0px scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_encode_0px(void)
{
  uint8_t  dummy[1] = {};
  uint32_t out_size = k_t_size_poison;

  ra8_err_t err = ra8_rabook_gray4_encode(dummy, 0U, 0U, dummy, sizeof(dummy), &out_size);
  internal_check(err == k_ra8_ok && out_size == 0U, "encode: 0 pixels returns 0 bytes");
}

/**
 * @test internal_test_encode_buffer_too_small
 * @par MC/DC:
 * (no compound decisions in this test -- exercises the single-condition
 * capacity internal_check `if (out_cap < n_bytes)` returning k_ra8_err_no_mem; no `&&`
 * or `||` in the code under test that this case touches) @brief Verify encode buffer too small behavior. @details Executes the encode buffer too small scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_encode_buffer_too_small(void)
{
  ...
  internal_check(err == k_ra8_err_no_mem, "encode: buffer too small returns no_mem");
}

/**
 * @test internal_test_encode_null_src
 * @par MC/DC:
 ...
 * touches) @brief Verify encode null src behavior. @details Executes the encode null src scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_encode_null_src(void)
{
  uint8_t   out[4]   = {};
  uint32_t  out_size = 0U;
  ra8_err_t err      = ra8_rabook_gray4_encode(nullptr, 4U, 1U, out, sizeof(out), &out_size);
  internal_check(err == k_ra8_err_null_ptr, "encode: null src returns null_ptr");
}

/**
 * @test internal_test_encode_max_nib
 * @par MC/DC:
 ...
 * case touches) @brief Verify encode max nib behavior. @details Executes the encode max nib scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_encode_max_nib(void)
{
  ...
  internal_check(out[0] == k_t_pack_15_15 && out[1] == k_t_pack_15_pad,
...
/**
 * @test internal_test_gray8_encode_copies
 * @par MC/DC:
 ...
 * `||` in the code under test that this case touches) @brief Verify gray8 encode copies behavior. @details Executes the gray8 encode copies scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_gray8_encode_copies(void)
{
  ...
  internal_check(err == k_ra8_ok, "gray8 encode: ok");
  internal_check(out_size == 4U, "gray8 encode: size == w*h");
  internal_check(memcmp(out, pixels, sizeof(pixels)) == 0, "gray8 encode: bytes copied verbatim");
}

/**
 * @test internal_test_gray8_encode_0px
 * @par MC/DC:
 * (no compound decisions in this test -- exercises the single-condition
 ...
 * in the code under test that this case touches) @brief Verify gray8 encode 0px behavior. @details Executes the gray8 encode 0px scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_gray8_encode_0px(void)
{
  uint8_t  dummy[1] = {};
  uint32_t out_size = k_t_size_poison;

  ra8_err_t err = ra8_rabook_gray8_encode(dummy, 0U, 0U, dummy, sizeof(dummy), &out_size);
  internal_check(err == k_ra8_ok && out_size == 0U, "gray8 encode: 0 pixels returns 0 bytes");
}

/**
 * @test internal_test_gray8_encode_buffer_too_small
 * @par MC/DC:
 * (no compound decisions in this test -- exercises the single-condition
 * capacity internal_check `if (out_cap < n_bytes)` returning k_ra8_err_no_mem; no `&&`
 * or `||` in the code under test that this case touches) @brief Verify gray8 encode buffer too small behavior. @details Executes the gray8 encode buffer too small scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_gray8_encode_buffer_too_small(void)
{
  static const uint8_t pixels[] = {0U, 255U, 128U, 64U};
  uint8_t              out[1]   = {};
  uint32_t             out_size = 0U;
  ...
  internal_check(err == k_ra8_err_no_mem, "gray8 encode: buffer too small returns no_mem");
}

/**
 * @test internal_test_gray8_encode_null_src
 * @par MC/DC:
 * (no compound decisions in this test -- a precondition null-guard test that
 ...
 * this case touches) @brief Verify gray8 encode null src behavior. @details Executes the gray8 encode null src scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_gray8_encode_null_src(void)
{
  uint8_t   out[4]   = {};
  uint32_t  out_size = 0U;
  ra8_err_t err      = ra8_rabook_gray8_encode(nullptr, 4U, 1U, out, sizeof(out), &out_size);
  internal_check(err == k_ra8_err_null_ptr, "gray8 encode: null src returns null_ptr");
}
...
/**
 * @test internal_test_downscale_identity
 * @par MC/DC:
 ...
 * sibling tests: internal_test_downscale_zero_dst / internal_test_downscale_zero_dst_h complete
 * decision A, and internal_test_downscale_zero_src / internal_test_downscale_zero_src_h complete
 * decision B. @brief Verify downscale identity behavior. @details Executes the downscale identity scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_downscale_identity(void)
{
  static const uint8_t src[]  = {10U, 20U, 30U, 40U};
  uint8_t              dst[4] = {};

  ra8_err_t err = ra8_rabook_gray4_downscale(src, 2U, 2U, dst, 2U, 2U);
  internal_check(err == k_ra8_ok, "downscale: identity 2x2 -- ok");
  internal_check(dst[0] == k_t_px_tl && dst[1] == k_t_px_tr && dst[2] == k_t_px_bl &&
                   dst[3] == k_t_px_br,
                 "downscale: identity 2x2 -- pixels unchanged");
}

/**
 * @test internal_test_downscale_2to1_horizontal
 * @par MC/DC:
 ...
 * no `&&` or `||` in the code under test that this case drives) @brief Verify downscale 2to1 horizontal behavior. @details Executes the downscale 2to1 horizontal scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_downscale_2to1_horizontal(void)
{
  /*
  ...
  internal_check(err == k_ra8_ok, "downscale: 4x1->2x1 -- ok");
  internal_check(dst[0] == 0U && dst[1] == k_t_px_sample2,
                 "downscale: 4x1->2x1 -- sampled at x=0,2");
}

/**
 * @test internal_test_downscale_null_ptr
 * @par MC/DC:
 ...
 * in the code under test that this case touches) @brief Verify downscale null ptr behavior. @details Executes the downscale null ptr scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_downscale_null_ptr(void)
{
  uint8_t buf[4] = {};
  internal_check(ra8_rabook_gray4_downscale(nullptr, 2U, 2U, buf, 2U, 2U) == k_ra8_err_null_ptr,
                 "downscale: null src");
  internal_check(ra8_rabook_gray4_downscale(buf, 2U, 2U, nullptr, 2U, 2U) == k_ra8_err_null_ptr,
                 "downscale: null dst");
}

/**
 * @test internal_test_downscale_zero_dst
 * @par MC/DC:
 ...
 * V1 is in internal_test_downscale_identity and V3 is in internal_test_downscale_zero_dst_h of
 * this file). @brief Verify downscale zero dst behavior. @details Executes the downscale zero dst scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_downscale_zero_dst(void)
{
  static const uint8_t src[]  = {1U, 2U, 3U, 4U};
  uint8_t              dst[1] = {};
  ra8_err_t            err    = ra8_rabook_gray4_downscale(src, 2U, 2U, dst, 0U, 1U);
  internal_check(err == k_ra8_err_invalid_arg, "downscale: dst_w=0 returns invalid_arg");
}

/**
 * @test internal_test_downscale_zero_src
 * @par MC/DC:
 ...
 * V1 is in internal_test_downscale_identity and V3 is in internal_test_downscale_zero_src_h of
 * this file). @brief Verify downscale zero src behavior. @details Executes the downscale zero src scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_downscale_zero_src(void)
{
  ...
  internal_check(err == k_ra8_ok, "downscale: zero src_w returns ok");
  internal_check(dst[0] == 0U && dst[1] == 0U, "downscale: zero src_w zeroes dst");
}

/**
 * @test internal_test_downscale_zero_dst_h
 * @par MC/DC:
 * Decision: `dst_w == 0 || dst_h == 0`
 * (2 conditions, function `ra8_rabook_gray4_downscale`).
 * - V1 dst_w=2, dst_h=2 -> C1=F,C2=F -> F (proceeds).
 ...
 * V1 is in internal_test_downscale_identity and V2 is in internal_test_downscale_zero_dst of this
 * file). @brief Verify downscale zero dst h behavior. @details Executes the downscale zero dst h scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_downscale_zero_dst_h(void)
{
  /*
  ...
  internal_check(err == k_ra8_err_invalid_arg, "downscale: dst_h=0 returns invalid_arg");
}

/**
 * @test internal_test_downscale_zero_src_h
 * @par MC/DC:
 * Decision: `src_w == 0 || src_h == 0`
 * (2 conditions, function `ra8_rabook_gray4_downscale`; the source guard,
 * reached only with a valid non-zero dst).
 ...
 * V1 is in internal_test_downscale_identity and V2 is in internal_test_downscale_zero_src of this
 * file). @brief Verify downscale zero src h behavior. @details Executes the downscale zero src h scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_downscale_zero_src_h(void)
{
  /*
  ...
  internal_check(err == k_ra8_ok, "downscale: zero src_h returns ok");
  internal_check(dst[0] == 0U && dst[1] == 0U, "downscale: zero src_h zeroes dst");
}
...
int main(void)
{
  ...
  internal_test_dims_no_scale();
  internal_test_dims_exact_edge();
  internal_test_dims_scale_landscape();
  internal_test_dims_scale_portrait();
  internal_test_dims_null_out();
  internal_test_dims_zero_src();
  internal_test_dims_zero_src_h();
  internal_test_dims_zero_max_edge();
  internal_test_dims_clamp_w_to_1();
  internal_test_dims_clamp_h_to_1();

  internal_test_encode_4px_exact_palette();
  internal_test_encode_3px_odd();
  internal_test_encode_1px();
  internal_test_encode_0px();
  internal_test_encode_buffer_too_small();
  internal_test_encode_null_src();
  internal_test_encode_max_nib();

  internal_test_gray8_encode_copies();
  internal_test_gray8_encode_0px();
  internal_test_gray8_encode_buffer_too_small();
  internal_test_gray8_encode_null_src();

  internal_test_downscale_identity();
  internal_test_downscale_2to1_horizontal();
  internal_test_downscale_null_ptr();
  internal_test_downscale_zero_dst();
  internal_test_downscale_zero_dst_h();
  internal_test_downscale_zero_src();
  internal_test_downscale_zero_src_h();
