/**
 * @file test_ra_glcdc_gamma.c
 * @brief Unit tests for the GLCDC gamma correction API.
 *
 * @details
 * Exercises `ra_glcdc_set_gamma` (per-channel LUT + AREA register writes)
 * and `ra_glcdc_gamma_enable` (GAMSW.GAMON bit).  Every register write is
 * observed through the simulated MMIO window provided by `ra_sim_mmap`.
 *
 ...
 */
typedef enum : uint8_t {
  k_test_count_valid = 16U, /**< Valid LUT depth (k_ra_glcdc_gamma_lut_depth). */
 ...
 *
 * @details
 * k_ra_glcdc_gamma_lut_depth = 16 values (two 11-bit gains per register,
 ...
 *
 * @details
 * k_ra_glcdc_gamma_lut_depth = 16 values (two 10-bit thresholds per register,
...
static void test_set_gamma_r_writes_lut_and_area(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_glcdc_set_gamma(k_ra_glcdc_ch_r, k_gains, k_thresholds, (uint8_t)k_test_count_valid));

  volatile ra_glcdc_gam_t* gam = ra_glcdc_gam_regs((uint8_t)k_test_ch_r);
...
static void test_set_gamma_g_writes_lut_and_area(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_glcdc_set_gamma(k_ra_glcdc_ch_g, k_gains, k_thresholds, (uint8_t)k_test_count_valid));

  volatile ra_glcdc_gam_t* gam = ra_glcdc_gam_regs((uint8_t)k_test_ch_g);
  TEST_ASSERT_NOT_NULL((void*)gam);
...
static void test_set_gamma_b_writes_lut_and_area(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_glcdc_set_gamma(k_ra_glcdc_ch_b, k_gains, k_thresholds, (uint8_t)k_test_count_valid));

  volatile ra_glcdc_gam_t* gam = ra_glcdc_gam_regs((uint8_t)k_test_ch_b);
  TEST_ASSERT_NOT_NULL((void*)gam);
...
static void test_set_gamma_channels_do_not_alias(void)
{
  ...
  volatile ra_glcdc_gam_t* g_gam = ra_glcdc_gam_regs((uint8_t)k_test_ch_g);
  volatile ra_glcdc_gam_t* b_gam = ra_glcdc_gam_regs((uint8_t)k_test_ch_b);
...
static void test_gamma_enable_sets_gamon(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_glcdc_gamma_enable(true));
...
{
  TEST_BEGIN("glcdc set_gamma: null gain -> k_ra_err_null_ptr");
  prep();

  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_glcdc_set_gamma(k_ra_glcdc_ch_r, nullptr, k_thresholds, (uint8_t)k_test_count_valid));

  TEST_END("glcdc set_gamma: null gain -> k_ra_err_null_ptr");
}
...
{
  TEST_BEGIN("glcdc set_gamma: null threshold -> k_ra_err_null_ptr");
  prep();

  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_glcdc_set_gamma(k_ra_glcdc_ch_r, k_gains, nullptr, (uint8_t)k_test_count_valid));

  TEST_END("glcdc set_gamma: null threshold -> k_ra_err_null_ptr");
}
...
{
  TEST_BEGIN("glcdc set_gamma: channel >= count -> k_ra_err_invalid_arg");
  prep();

  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_glcdc_set_gamma((ra_glcdc_color_channel_t)k_test_ch_bad,
                                    k_gains,
                                    k_thresholds,
                                    (uint8_t)k_test_count_valid));

  TEST_END("glcdc set_gamma: channel >= count -> k_ra_err_invalid_arg");
}
 ...
 *
 * @par MC/DC:
 * Decision: `if (count != (uint8_t)k_ra_glcdc_gamma_lut_depth)` (1 condition)
 * - Vector 1: count = 16 (valid)  -> false -> ok.
 * - Vector 2: count = 0  (zero)   -> true  -> k_ra_err_invalid_arg.
 * - Vector 3: count = 15 (short)  -> true  -> k_ra_err_invalid_arg.
 * - Vector 4: count = 17 (over)   -> true  -> k_ra_err_invalid_arg.
...
static void test_mcdc_set_gamma_count_guard(void)
{
  ...
  TEST_ASSERT_EQ(
    ...
    ra_glcdc_set_gamma(k_ra_glcdc_ch_r, k_gains, k_thresholds, (uint8_t)k_test_count_zero));

  /* Vector 3: count == 15 -> invalid_arg. */
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_glcdc_set_gamma(k_ra_glcdc_ch_r, k_gains, k_thresholds, (uint8_t)k_test_count_short));

  /* Vector 4: count == 17 -> invalid_arg. */
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_glcdc_set_gamma(k_ra_glcdc_ch_r, k_gains, k_thresholds, (uint8_t)k_test_count_over));
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_glcdc_gamma.c\n");
  return 0;
}
