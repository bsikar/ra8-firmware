/**
 * @file test_ra_fs_fat_mcdc.c
 * @brief Residual MC/DC vectors for the still-uncovered decisions in ra_fs_fat.c.
 *
 * @details
 * Third sibling of ``tests/test_ra_fs_fat.c`` and
 * ``tests/test_ra_fs_fat_variants.c``. Those files (plus
 * ``tests/test_ra_fs_format.c``) cover the bulk of the FAT/exFAT adapter; this
 ...
 *
 * Coverage is driven by the real public ``ra_fs_*`` functions executing the
 * branch; each case asserts the returned ``ra_err_t`` or an observable state.
 * The in-memory block-device harness and the FAT16 BPB builder mirror
 * ``tests/test_ra_fs_fat.c`` exactly. LFN directory images are planted by
 * writing raw 32-byte entries into the fixed root region, then resolved via
 * ``ra_fs_open`` with a non-8.3 long name (a 4+ char extension forces the
 * long-name fallback).
 *
...
/** @brief Memory-backed block device handed to ra_fs. */
typedef struct {
 ...
 * @par MC/DC:
 * Two decisions in `priv_name_ieq` (libs/ra_fs/src/ra_fs_fat.c lines 1583, 1590),
 * reached when `ra_fs_open` falls back to the long-name lookup
...
{
  TEST_BEGIN("ra_fs MC/DC: LFN name compare length arms");
  build_fat16_volume();
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "AB.EPUB", k_ra_fs_mode_read, &f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));
  /* Needle longer than lname: loop exits on C2=F; return sees *a != '\0'. */
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_fs_open(h, "AB.EPUBX", k_ra_fs_mode_read, &f));
  /* lname longer than needle (needle is a strict prefix): loop exits on C1=F;
   * return sees *b != '\0'. */
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_fs_open(h, "AB.EPU", k_ra_fs_mode_read, &f));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs MC/DC: LFN name compare length arms");
}
 ...
 * (libs/ra_fs/src/ra_fs_fat.c line 1555, 2 conditions), reached while
 * reassembling a planted LFN chain through the `ra_fs_open` long-name fallback.
...
{
  TEST_BEGIN("ra_fs MC/DC: LFN terminator (val==0 || val==pad)");
  const char    short11[11] = {'A', 'B', '1', '2', '3', '4', '5', '6', 'E', 'P', 'U'};
  const uint8_t csum        = sfn_checksum((const uint8_t*)short11);
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs MC/DC: LFN terminator (val==0 || val==pad)");
}
 ...
 * (libs/ra_fs/src/ra_fs_fat.c line 1542, 2 conditions). `k_lfn_max_entries` is
 * 19. Reached by planting an LFN sub-entry with an out-of-range order byte and
 * resolving the directory through the `ra_fs_open` long-name fallback.
...
{
  TEST_BEGIN("ra_fs MC/DC: LFN order range (order<1 || order>19)");
  const char     short11[11] = {'A', 'B', '1', '2', '3', '4', '5', '6', 'E', 'P', 'U'};
  const uint8_t  csum        = sfn_checksum((const uint8_t*)short11);
  ...
  f = nullptr;
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_fs_open(h, "AB.EPUB", k_ra_fs_mode_read, &f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs MC/DC: LFN order range (order<1 || order>19)");
}

/**
 * @enum ra_fs_mcdc_read_t
 ...
 * walk_cache_idx)` in `priv_read_one_chunk` (libs/ra_fs/src/ra_fs_fat.c line
...
{
  TEST_BEGIN("ra_fs MC/DC: read walk-cache resume (cache set, backward seek)");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "BACK.BIN", k_ra_fs_mode_write, &f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_write(f, wr, (uint32_t)k_read_payload));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "BACK.BIN", k_ra_fs_mode_read, &f));
  /* Forward sequential read across clusters: cache set, target ahead (C1,C2 T). */
  uint32_t total = 0U;
  while (total < (uint32_t)k_read_payload) {
    uint32_t got = 0U;
    TEST_ASSERT_EQ(k_ra_ok, ra_fs_read(f, &rd[total], (uint32_t)k_read_chunk, &got));
    if (got == 0U) {
      break;
    }
    total += got;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs MC/DC: read walk-cache resume (cache set, backward seek)");
}
 ...
 * (libs/ra_fs/src/ra_fs_fat.c line 4059, 2 conditions; the FAT12 boundary is
...
{
  TEST_BEGIN("ra_fs format MC/DC: FAT16 band upper bound (count<65525)");
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs format MC/DC: FAT16 band upper bound (count<65525)");
}
 ...
 * (libs/ra_fs/src/ra_fs_fat.c line 4062, 2 conditions). The FAT16 boundary is
 ...
 *   sibling test_ra_fs_format `test_mcdc_format_count_in_band_capacity` (a tiny
...
{
  TEST_BEGIN("ra_fs format MC/DC: FAT32 band lower bound (count>=65525)");
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs format MC/DC: FAT32 band lower bound (count>=65525)");
}
 ...
 * `ra_fs_format` (libs/ra_fs/src/ra_fs_fat.c line 5082, 4 conditions). The guard
...
{
  TEST_BEGIN("ra_fs format MC/DC: unsupported type (all four !=) reject");
  alloc_card((uint32_t)k_disk_blocks_fat16);
  ra_fs_format_opts_t opts = {};
  opts.type                = k_ra_fs_type_unknown; /* none of 12/16/32/exFAT */
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_fs_format(&s_backend, &opts));
  free_volume();
  TEST_END("ra_fs format MC/DC: unsupported type (all four !=) reject");
}

/**
 * @brief Capacity stub reporting a non-512 block size to drive the format guard.
 */
static ra_err_t cap_bad_block_size(void* ctx, uint32_t* block_count, uint32_t* block_size)
{
  (void)ctx;
 ...
 * @par MC/DC:
 * Decision: `if (block_size != 512 || block_count == 0U)` in `ra_fs_format`
 * (libs/ra_fs/src/ra_fs_fat.c line 5095, 2 conditions), after `get_capacity`.
...
{
  TEST_BEGIN("ra_fs format MC/DC: block-size guard (block_size!=512)");
  alloc_card((uint32_t)k_disk_blocks_fat16);
  ra_fs_format_opts_t opts = {};
  opts.type                = k_ra_fs_type_fat16;
  ...
  ra_fs_backend_t bad = s_backend;
  bad.get_capacity    = cap_bad_block_size;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_fs_format(&bad, &opts));
  free_volume();
  TEST_END("ra_fs format MC/DC: block-size guard (block_size!=512)");
}

/**
 * @enum ra_fs_mcdc_exfat_t
 ...
 * `priv_exfat_label_utf16` (libs/ra_fs/src/ra_fs_fat.c line 4975, 2 conditions),
 * reached when `ra_fs_format` lays the exFAT root volume-label entry.
...
{
  TEST_BEGIN("ra_fs format MC/DC: exFAT label pack (n<max && label[n])");
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs format MC/DC: exFAT label pack (n<max && label[n])");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_fs_fat_mcdc.c\n");
  return 0;
}
