 ...
 *
 * The guard above it in the same function,
 * `if ((cur == nullptr) || (cur->out == nullptr))`, is exercised only in its
 * false arm: every vector here drives it through the emitters, which build
 * the cursor themselves and hand it on by address, so no call site can pass
 * a null cursor or a cursor over a null buffer. Neither true arm is
 * reachable without breaking that construction, which no test may do. The
 * guard stays because ::internal_put is the one place every character in the
 * module passes through, and a future caller that builds a cursor from
 * caller-supplied storage would reach it.
 *
 * @pre None.
 * @post No process state is mutated beyond the local buffer.
 * @note Not thread-safe; single-threaded test context.
