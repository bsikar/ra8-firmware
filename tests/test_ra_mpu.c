...
/* =============================================================================
 * MC/DC vector tests for the compound boolean decisions in ra_mpu.c
 * =============================================================================
 */
 ...
 */
static ra_mpu_region_t mcdc_region(ra_mpu_perm_t priv, ra_mpu_perm_t unpriv, uint32_t size)
{
  const ra_mpu_region_t r = {
    .base       = (uintptr_t)k_test_mpu_region_base,
    .size       = size,
 ...
 * (2 conditions, libs/ra_mpu/src/ra_mpu.c line 48 -- ra_mpu_is_pow2),
 * exercised through `ra_mpu_validate_region` via `ra_mpu_set_region`.
...
{
  TEST_BEGIN("ra_mpu_is_pow2 MC/DC via ra_mpu_set_region(size)");
  mpu_test_setup();
 ...
 *
 * @par MC/DC:
 * Decision: `if (!ra_mpu_is_pow2(r->size) || r->size < min_region_size)`
 * (2 conditions, libs/ra_mpu/src/ra_mpu.c line 98).
...
{
  TEST_BEGIN("ra_mpu validate_region MC/DC: !is_pow2 || size < min");
  mpu_test_setup();
 ...
 *
 * @par MC/DC:
 * Decision: `if (priv == k_ra_mpu_perm_rw && unpriv == k_ra_mpu_perm_rw)`
 * (2 conditions, libs/ra_mpu/src/ra_mpu.c line 65), exercised via
 * `ra_mpu_set_region` -> `ra_mpu_validate_region` -> `ra_mpu_encode_ap`.
...
{
  TEST_BEGIN("ra_mpu_encode_ap L65 MC/DC: priv==rw && unpriv==rw");
  mpu_test_setup();
 ...
 *
 * @par MC/DC:
 * Decision: `if (priv == k_ra_mpu_perm_ro && unpriv == k_ra_mpu_perm_none)`
 * (2 conditions, libs/ra_mpu/src/ra_mpu.c line 68). Reached only when
...
{
  TEST_BEGIN("ra_mpu_encode_ap L68 MC/DC: priv==ro && unpriv==none");
  mpu_test_setup();
...
{
  TEST_BEGIN("ra_mpu_validate_cfg MC/DC: region_count>0 && regions==NULL");
  mpu_test_setup();

  /* Vector 1: zero regions, regions=NULL is irrelevant (short-circuit). */
  const ra_mpu_cfg_t cfg_v1 = {
    .regions      = nullptr,
    .region_count = 0U,
    .mair0        = 0U,
    .mair1        = 0U,
