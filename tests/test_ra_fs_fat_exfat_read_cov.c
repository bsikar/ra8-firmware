/**
 * @file tests/test_ra_fs_fat_exfat_read_cov.c
 * @brief Coverage-boost tests for `ra_fs_fat_exfat_read.c`.
 *
 * @details
...
typedef enum : uint32_t {
  ...
  k_rc_max_files     = 4U,      /**< File table capacity (k_ra_fs_max_files).   */
} rc_const_t;
 ...
 *          returns k_ra_err_out_of_range without touching the disk, simulating
 ...
 *
 * @return Error code.
 * @retval k_ra_ok              Sectors copied into buf.
 * @retval k_ra_err_out_of_range Countdown expired or request out of range.
 *
 * @pre ctx and buf are non-NULL.
 * @post buf holds the requested sectors on k_ra_ok.
 *
 * @since 0.1.0
 */
static ra_err_t rc_read(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf)
{
 ...
 *
 * @return Error code.
 * @retval k_ra_ok              Sectors written to the RAM disk.
 * @retval k_ra_err_out_of_range Request out of range.
 *
 * @pre ctx and buf are non-NULL.
 * @post Disk bytes match buf on k_ra_ok.
 *
 * @since 0.1.0
 */
static ra_err_t rc_write(void* ctx, uint32_t lba, uint32_t count, const uint8_t* buf)
{
 ...
 * @since 0.1.0
 */
static ra_err_t rc_capacity(void* ctx, uint32_t* block_count, uint32_t* block_size)
{
  const rc_disk_t* d = (const rc_disk_t*)ctx;
 ...
 *
 * @pre None.
 * @post ra_fs_mount returns an error after BPS shift is patched.
 *
 * @since 0.1.0
 */
static void test_parse_bad_bps_shift(void)
{
  ...
  ra_err_t       e                         = ra_fs_mount(&s_ctrl_backend, &h);
 ...
 *
 * Decision B: `if (e != k_ra_ok)` after priv_fat_get -- 1 cond.
 ...
 * @post priv_exfat_next_entry returns != k_ra_ok.
 *
 * @since 0.1.0
 */
static void test_next_entry_fat_read_fail(void)
{
  ...
  ra_err_t e     = priv_exfat_next_entry(h, &cur, buf);
  TEST_ASSERT(e != k_ra_ok);
 ...
 *          per_cluster boundary reads EOC and returns k_ra_err_not_found.
 *
 * @par MC/DC:
 ...
 *
 * @pre Volume is formatted and mounted.
 * @post priv_exfat_next_entry returns k_ra_err_not_found.
 *
 * @since 0.1.0
 */
static void test_next_entry_eoc(void)
{
  ...
  s_rd_remaining = (int32_t)k_rc_rd_never;
  ra_err_t e     = priv_exfat_next_entry(h, &cur, buf);
  TEST_ASSERT_EQ(k_ra_err_not_found, e);
 ...
 *
 * @pre Volume is formatted and mounted.
 * @post priv_exfat_next_entry returns k_ra_ok; cur.cluster == root_cluster+1.
 *
 * @since 0.1.0
 */
 ...
 *
 * @par MC/DC:
 * Decision: `if (e != k_ra_ok)` after priv_read_sector -- 1 cond.
 ...
 *          e != k_ra_err_not_found and returns it (line 266).
 *
 * @par MC/DC:
 * Decision A (priv_exfat_match_set line 207): `if (e != k_ra_ok)` -- 1 cond.
 * V1: stream read fails -> true -> line 208 (this test).
 * V2: stream read ok -> false -> check type (other tests).
 *
 * Decision B (priv_exfat_find line 265): `if (e != k_ra_err_not_found)` -- 1 cond.
 * V3: e = io-error -> true -> line 266 (this test).
 * V4: e = not_found -> false -> continue loop (wrong-type tests).
 *
 * @pre None.
 * @post ra_fs_open returns != k_ra_ok.
 *
 * @since 0.1.0
 */
 ...
 *          returns k_ra_err_not_found at line 211.
 *
 * @par MC/DC:
 ...
 *
 * @par MC/DC:
 * Decision: `if (e != k_ra_ok)` after name priv_exfat_next_entry -- 1 cond.
 ...
 *          proceeds, then sees nm[0] != 0xC1 and returns k_ra_err_not_found
 *          at line 224.
 *
 * @par MC/DC:
 ...
 *
 * @par MC/DC:
 * Decision (priv_exfat_find line 252): `if (e != k_ra_ok)` -- 1 cond.
...
static void test_find_first_read_fail(void)
{
  ...
  s_rd_remaining  = (int32_t)k_rc_rd_at_0;
  ra_fs_file_t* f = nullptr;
  ra_err_t      e = ra_fs_open(h, "X.TXT", k_ra_fs_mode_read, &f);
  TEST_ASSERT(e != k_ra_ok);
 ...
 *
 * @details ra_fs_open with k_ra_fs_mode_write on an exFAT mount dispatches to
 *          priv_exfat_open, which returns k_ra_err_not_supported at line 277.
 *
 * @par MC/DC:
 * Decision: `if (mode != k_ra_fs_mode_read)` -- 1 cond.
 * V1: mode = write -> true -> line 277, not_supported (this test).
 * V2: mode = read -> false -> priv_exfat_find (success-path tests).
 *
 * @pre Volume is formatted and mounted.
 * @post ra_fs_open returns k_ra_err_not_supported.
 *
 * @since 0.1.0
 */
static void test_open_write_mode_rejected(void)
{
  ...
  s_rd_remaining  = (int32_t)k_rc_rd_never;
  ra_fs_file_t* f = nullptr;
  ra_err_t      e = ra_fs_open(h, "X.TXT", k_ra_fs_mode_write, &f);
 ...
 * @post The 5th ra_fs_open returns k_ra_err_no_mem.
 *
 * @since 0.1.0
 */
static void test_open_file_table_full(void)
{
  ...
  const uint8_t data = (uint8_t)'X';
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_write_file(h, "F.TXT", &data, 1U));

  s_rd_remaining = (int32_t)k_rc_rd_never;

  /* Fill all four file slots. */
  ra_fs_file_t* slots[(uint32_t)k_rc_max_files] = {};
  for (uint32_t i = 0U; i < (uint32_t)k_rc_max_files; i++) {
    TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "F.TXT", k_ra_fs_mode_read, &slots[i]));
  }

  /* Fifth open must fail with no_mem. */
  ra_fs_file_t* extra = nullptr;
  TEST_ASSERT_EQ(k_ra_err_no_mem, ra_fs_open(h, "F.TXT", k_ra_fs_mode_read, &extra));

  /* Release all open slots before unmount. */
  for (uint32_t i = 0U; i < (uint32_t)k_rc_max_files; i++) {
    TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(slots[i]));
  }
...
int main(void)
{
  ...
  printf("[OK  ] test_ra_fs_fat_exfat_read_cov.c\n");
  return 0;
}
