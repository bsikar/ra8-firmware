/**
 * @file test_ra_error_handler.c
 * @brief Unit tests for ra_error_handler.c fatal sink
 *
 * @details
 * The only public function in `ra_error_handler.c` is
 * `internal_ra_fatal_error`, which halts the CPU on target and calls
...
static void test_fatal_error_path(void)
{
  ...
  if (sigsetjmp(s_trap_jmp, 1) == 0) {
    internal_ra_fatal_error("TEST", "fatal message", (uint32_t)k_ra_err_hw_error);
    TEST_FAIL_FMT("%s", "internal_ra_fatal_error returned");
  }
...
static void test_fatal_error_zero_code(void)
{
  ...
  if (sigsetjmp(s_trap_jmp, 1) == 0) {
    internal_ra_fatal_error("ASSERT", "zero err", 0U);
    TEST_FAIL_FMT("%s", "internal_ra_fatal_error returned");
  }
  TEST_ASSERT_EQ(1, s_trap_hit);
...
static void test_fatal_error_large_code(void)
{
  ...
  if (sigsetjmp(s_trap_jmp, 1) == 0) {
    internal_ra_fatal_error("FAULT", "huge err", 0xDEADBEEFUL);
    TEST_FAIL_FMT("%s", "internal_ra_fatal_error returned");
  }
  TEST_ASSERT_EQ(1, s_trap_hit);

  TEST_END("internal_ra_fatal_error with large err");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_error_handler.c\n");
  return 0;
}
