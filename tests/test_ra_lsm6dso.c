/**
 * @file test_ra_lsm6dso.c
 * @brief Host-side unit tests for the LSM6DSO driver
 *
 * @details
 * Exercises every public function in ``libs/ra_lsm6dso`` against a
 * canned-response mock I2C transport. The mock implements both the
 * ``read_regs`` and ``write_regs`` callbacks of ``ra_lsm6dso_bus_t``
 ...
 *
 * Test coverage:
 *   - ``ra_lsm6dso_init`` -- NULL pointer rejection (3 atomic conditions).
 *   - ``ra_lsm6dso_who_am_i`` -- happy path (returns 0x6C) and wrong-ID
 *     path (mock returns 0xFF).
 *   - ``ra_lsm6dso_set_accel_range`` -- writes CTRL1_XL with the FS_XL
 *     bits programmed, leaves the ODR nibble intact.
 *   - ``ra_lsm6dso_set_gyro_range``  -- writes CTRL2_G with the FS_G +
 *     FS_125 bits programmed.
 *   - ``ra_lsm6dso_set_odr`` -- writes CTRL1_XL and CTRL2_G with the
 *     ODR nibble in [7:4].
 *   - ``ra_lsm6dso_read_accel`` / ``_read_gyro`` -- bursts 6 bytes
 *     and combines them little-endian; checks against canned ints.
 *   - ``ra_lsm6dso_read_temp`` -- converts (raw / 256 + 25) C into
 *     centi-degC.
 *   - I2C NAK propagation: when the mock returns ``k_ra_err_nack``,
...
static void mock_reset(void)
{
  ...
  s_mock.forced_err = k_ra_ok;
}

/** @brief Mock ``read_regs`` callback. */
static ra_err_t mock_read(void* ctx, uint8_t reg, uint8_t* buf, uint32_t len)
{
  (void)ctx;
  if (s_mock.forced_err != k_ra_ok) {
...
static ra_err_t mock_write(void* ctx, uint8_t reg, const uint8_t* buf, uint32_t len)
{
  (void)ctx;
  if (s_mock.forced_err != k_ra_ok) {
    return s_mock.forced_err;
...
static ra_lsm6dso_bus_t make_bus(void)
{
...
/**
 * @test ra_lsm6dso_init_validates_inputs
 *
 * @par MC/DC:
 * Decision in ``ra_lsm6dso_init``: four ``RA_CHECK_NULL_PTR`` guards.
...
static void test_init_validates_inputs(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_lsm6dso_init(nullptr, &bus));
  /* Vector 2: bus == NULL. */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_lsm6dso_init(&dev, nullptr));
  /* Vector 3: bus.read_regs == NULL. */
  ra_lsm6dso_bus_t bad_read = bus;
  bad_read.read_regs        = nullptr;
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_lsm6dso_init(&dev, &bad_read));
  /* Vector 4: bus.write_regs == NULL. */
  ra_lsm6dso_bus_t bad_write = bus;
  bad_write.write_regs       = nullptr;
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_lsm6dso_init(&dev, &bad_write));
  /* Vector 5: happy path. */
...
/**
 * @test ra_lsm6dso_who_am_i_happy
 *
 * @par MC/DC:
 * Decision: ``r != k_ra_ok`` after the transport read. Two vectors
...
/**
 * @test ra_lsm6dso_who_am_i_wrong_id
 *
 * @par MC/DC:
...
/**
 * @test ra_lsm6dso_who_am_i_null_rejected
 *
 * @par MC/DC:
 * Three ``RA_CHECK_NULL_PTR`` guards in ``who_am_i``: dev, out_id,
...
static void test_who_am_i_null_rejected(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lsm6dso_init(&dev, &bus));
  uint8_t id = 0U;
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_lsm6dso_who_am_i(nullptr, &id));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_lsm6dso_who_am_i(&dev, nullptr));
  /* Uninitialized device -> not_initialized. */
  ra_lsm6dso_t fresh = {};
  TEST_ASSERT(ra_lsm6dso_who_am_i(&fresh, &id) != k_ra_ok);
...
/**
 * @test ra_lsm6dso_nack_propagation
 *
 * @par MC/DC:
 * Confirms the driver does not eat transport errors. The mock's
 * ``forced_err`` knob makes every read / write return ``k_ra_err_nack``
 * unconditionally.
 */
static void test_nack_propagation(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_nack, ra_lsm6dso_set_gyro_range(&dev, k_lsm6dso_g_fs_500dps));
  TEST_ASSERT_EQ(k_ra_err_nack, ra_lsm6dso_set_odr(&dev, k_lsm6dso_odr_104hz));
  ra_lsm6dso_xyz_t xyz   = {};
  int32_t          tcent = 0;
  TEST_ASSERT_EQ(k_ra_err_nack, ra_lsm6dso_read_accel(&dev, &xyz));
  TEST_ASSERT_EQ(k_ra_err_nack, ra_lsm6dso_read_gyro(&dev, &xyz));
  TEST_ASSERT_EQ(k_ra_err_nack, ra_lsm6dso_read_temp(&dev, &tcent));
...
/**
 * @test ra_lsm6dso_set_accel_range_writes_fs_xl
 *
 * @par MC/DC:
...
static void test_set_accel_range_writes_fs_xl(void)
{
  ...
  s_mock.regs[(uint8_t)k_lsm6dso_reg_ctrl1_xl] = 0x40U;
...
/**
 * @test ra_lsm6dso_set_accel_range_invalid
 *
 * @par MC/DC:
...
/**
 * @test ra_lsm6dso_set_gyro_range_writes_fs_g
 *
 * @par MC/DC:
...
static void test_set_gyro_range_writes_fs_g(void)
{
  ...
  s_mock.regs[(uint8_t)k_lsm6dso_reg_ctrl2_g] = 0x40U; /* preserved ODR */
  TEST_ASSERT_EQ(k_ra_ok, ra_lsm6dso_set_gyro_range(&dev, k_lsm6dso_g_fs_125dps));
  ...
  s_mock.regs[(uint8_t)k_lsm6dso_reg_ctrl2_g] = 0x40U;
  TEST_ASSERT_EQ(k_ra_ok, ra_lsm6dso_set_gyro_range(&dev, k_lsm6dso_g_fs_1000dps));
...
/**
 * @test ra_lsm6dso_set_odr_writes_both_blocks
 *
 * @par MC/DC:
...
static void test_set_odr_writes_both_blocks(void)
{
  ...
  s_mock.regs[(uint8_t)k_lsm6dso_reg_ctrl1_xl] = 0x0CU;
...
/**
 * @test ra_lsm6dso_set_odr_invalid
 *
 * @par MC/DC:
...
static void test_set_odr_invalid(void)
{
  ...
  const ra_lsm6dso_bus_t bus = make_bus();
  TEST_ASSERT_EQ(k_ra_ok, ra_lsm6dso_init(&dev, &bus));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_lsm6dso_set_odr(&dev, (ra_lsm6dso_odr_t)0x7FU));
...
/**
 * @test ra_lsm6dso_read_accel_combines_le_bytes
 *
 * @par MC/DC:
...
static void test_read_gyro_combines_le_bytes(void)
{
  ...
  s_mock.regs[base + 4] = 0xFFU;
  s_mock.regs[base + 5] = 0x7FU; /* Z = 32767 */

  ra_lsm6dso_xyz_t out = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_lsm6dso_read_gyro(&dev, &out));
...
/**
 * @test ra_lsm6dso_read_temp_converts_to_centi_c
 *
 * @par MC/DC:
 * Two vectors:
...
static void test_read_temp_converts(void)
{
  ...
  int32_t t             = -1;
  TEST_ASSERT_EQ(k_ra_ok, ra_lsm6dso_read_temp(&dev, &t));
...
/**
 * @test ra_lsm6dso_fifo_drains_words
 *
 * @par MC/DC:
...
static void test_fifo_drains_words(void)
{
  ...
  s_mock.regs[(uint8_t)k_lsm6dso_reg_fifo_data_out + 7] = 0xBBU;

  uint8_t  buf[16] = {};
  uint32_t words   = 0xFFFFU;
...
/**
 * @test ra_lsm6dso_fifo_validates_inputs
 *
 * @par MC/DC:
