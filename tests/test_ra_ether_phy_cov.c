/**
 * @file test_ra_ether_phy_cov.c
 * @brief Coverage-extension unit tests for ra_ether_phy.c
 *
 * @details
 * Exercises the branches that remain uncovered after test_ra_ether_phy.c.
 ...
 *
 *  - 86-87   ra_ether_phy_open(): BMCR write fails; opened flag rolled back.
 *  - 93-94   ra_ether_phy_open(): BMCR-poll read fails inside the loop.
 *  - 101-102 ra_ether_phy_open(): reset-poll timeout exhausted.
 *  - 157     ra_ether_phy_link_status_get(): BMSR read fails.
 *  - 172     ra_ether_phy_link_status_get(): LPA speed resolved to 100half.
 *  - 173-174 ra_ether_phy_link_status_get(): LPA speed resolved to 10full.
 *  - 175-176 ra_ether_phy_link_status_get(): LPA speed resolved to 10half.
 *  - 177     ra_ether_phy_link_status_get(): closing brace of speed chain.
 *
 ...
 *
 * @details
 * Mirrors the state used in test_ra_ether_phy.c so both files can be
 ...
 *
 * @details
 * Returns k_ra_err_hw_error on the next call when fail_next_read is set,
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              Read succeeded.
 * @retval k_ra_err_hw_error    fail_next_read was non-zero.
 * @retval k_ra_err_invalid_arg reg out of range.
 *
 ...
 * @since 0.1.0
 */
static ra_err_t cov_bus_read(void* ctx, uint8_t phy, uint8_t reg, uint16_t* out)
{
  (void)phy;
 ...
 *
 * @details
 * Returns k_ra_err_hw_error on the next call when fail_next_write is set,
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              Write succeeded.
 * @retval k_ra_err_hw_error    fail_next_write was non-zero.
 * @retval k_ra_err_invalid_arg reg out of range.
 *
 * @pre ctx is a valid test_cov_io_t pointer.
 ...
 * @since 0.1.0
 */
static ra_err_t cov_bus_write(void* ctx, uint8_t phy, uint8_t reg, uint16_t data)
{
  (void)phy;
  test_cov_io_t* st = (test_cov_io_t*)ctx;
 ...
 * normal ra_ether_phy_open() succeeds on the second poll (reset auto-clears).
 *
 * @pre None.
 ...
 *
 * @return ra_ether_phy_cfg_t with cov_bus_read / cov_bus_write and s_cov_io.
 *
 ...
 * @since 0.1.0
 */
static ra_ether_phy_cfg_t cov_make_cfg(void)
{
 ...
 * ra_ether_phy_open() must clear s_state.opened and propagate the error.
 ...
 *
 * @par MC/DC:
 * Decision: ``if (err != k_ra_ok)`` at line 85 (single condition).
 * - Vector T: write returns hw_error    -> error propagated (this test).
 * - Vector F: write returns ok          -> covered by test_ra_ether_phy.c.
...
static void test_open_write_fails(void)
{
  ...
  cov_prep();
  ra_ether_phy_cfg_t cfg = cov_make_cfg();

  /* Inject write fault so the initial BMCR reset write fails. */
  s_cov_io.fail_next_write = 1U;
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_ether_phy_open(&cfg));
 ...
 * ra_ether_phy_open() must clear s_state.opened and return the error.
 * The bus write succeeds (no fail_next_write), then the first poll read
 * fails (fail_next_read = 1), triggering the error path.
 *
 * @par MC/DC:
 * Decision: ``if (err != k_ra_ok)`` at line 92 (single condition;
 ...
 * - Vector F: poll read returns ok       -> covered by test_ra_ether_phy.c.
...
/**
 * @brief open() returns k_ra_err_hw_timeout when reset never self-clears.
 *
 * @details
 * Covers source lines 101-102: reset_reads_remaining is set to 33, which
 * exceeds the 32-iteration poll limit (k_ra_ether_phy_reset_poll_max).
 ...
 *
 * @par MC/DC:
 * Decision: loop exit condition ``i < k_ra_ether_phy_reset_poll_max``
 * (single condition; branch coverage excluded by GCOVR_EXCL_BR_LINE on
 * the for line). The inner decision
 * ``if ((reg & k_ra_ether_phy_bmcr_reset) == 0U)`` is a single condition.
 * - Vector T (reset cleared): loop exits early -> covered by test_ra_ether_phy.c.
...
{
  TEST_BEGIN("open: reset-poll timeout returns k_ra_err_hw_timeout");
  cov_prep();
  ra_ether_phy_cfg_t cfg = cov_make_cfg();
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_ether_phy_open(&cfg));

  /* opened was rolled back; verify a subsequent open can succeed. */
  s_cov_io.reset_reads_remaining = 1U;
  TEST_ASSERT_EQ(k_ra_ok, ra_ether_phy_open(&cfg));
  TEST_ASSERT_EQ(k_ra_ok, ra_ether_phy_close());
  TEST_END("open: reset-poll timeout returns k_ra_err_hw_timeout");
}
 ...
 * fails; ra_ether_phy_link_status_get() must return the error immediately.
 * This is triggered by injecting fail_next_read after a successful open.
 *
 * @par MC/DC:
 * Decision: ``if (err != k_ra_ok)`` at line 156 (single condition).
 * - Vector T: BMSR read returns hw_error -> line 157 hit (this test).
 * - Vector F: BMSR read returns ok       -> covered by test_ra_ether_phy.c.
 * Single-condition decision; both vectors satisfy MC/DC.
 *
 * @since 0.1.0
 */
static void test_link_status_bmsr_read_fails(void)
{
  ...
  cov_prep();
  const ra_ether_phy_cfg_t cfg = cov_make_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_ether_phy_open(&cfg));

  /* Inject a read fault; the next read in link_status_get is the BMSR read. */
  s_cov_io.fail_next_read  = 1U;
  ra_ether_phy_link_t link = {};
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_ether_phy_link_status_get(&link));
 ...
 * the speed field is resolved to k_ra_ether_phy_speed_100h.
 *
 * @par MC/DC:
 * Decision: ``if ((lpa & k_ra_ether_phy_anar_100half) != 0U)`` at line 171
 ...
 *             by test_ra_ether_phy.c (test_link_status).
 *
 * @since 0.1.0
 */
static void test_link_status_speed_100half(void)
{
  TEST_BEGIN("link_status_get: LPA 100half resolves speed to k_ra_ether_phy_speed_100h");
  cov_prep();
  const ra_ether_phy_cfg_t cfg = cov_make_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_ether_phy_open(&cfg));
  ...
  TEST_ASSERT_EQ(k_ra_ether_phy_speed_100h, link.speed);

  TEST_ASSERT_EQ(k_ra_ok, ra_ether_phy_close());
  TEST_END("link_status_get: LPA 100half resolves speed to k_ra_ether_phy_speed_100h");
}
 ...
 * (LPA bit 6 set), speed is resolved to k_ra_ether_phy_speed_10f.
 *
 * @par MC/DC:
 * Decision: ``if ((lpa & k_ra_ether_phy_anar_10full) != 0U)`` at line 173
...
{
  TEST_BEGIN("link_status_get: LPA 10full resolves speed to k_ra_ether_phy_speed_10f");
  cov_prep();
  const ra_ether_phy_cfg_t cfg = cov_make_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_ether_phy_open(&cfg));
  ...
  TEST_ASSERT_EQ(1, link.link_up);
  TEST_ASSERT_EQ(1, link.auto_neg_done);
  TEST_ASSERT_EQ(k_ra_ether_phy_speed_10f, link.speed);

  TEST_ASSERT_EQ(k_ra_ok, ra_ether_phy_close());
  TEST_END("link_status_get: LPA 10full resolves speed to k_ra_ether_phy_speed_10f");
}
 ...
 * when true (LPA bit 5 set), speed is resolved to k_ra_ether_phy_speed_10h.
 * The closing brace at line 177 is also reached via this execution path.
 *
 * @par MC/DC:
 * Decision: ``if ((lpa & k_ra_ether_phy_anar_10half) != 0U)`` at line 175
 ...
 *             implicitly by the no-link paths in test_ra_ether_phy.c.
 *
 * @since 0.1.0
 */
static void test_link_status_speed_10half(void)
{
  TEST_BEGIN("link_status_get: LPA 10half resolves speed to k_ra_ether_phy_speed_10h");
  cov_prep();
  const ra_ether_phy_cfg_t cfg = cov_make_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_ether_phy_open(&cfg));
  ...
  TEST_ASSERT_EQ(1, link.link_up);
  TEST_ASSERT_EQ(1, link.auto_neg_done);
  TEST_ASSERT_EQ(k_ra_ether_phy_speed_10h, link.speed);

  TEST_ASSERT_EQ(k_ra_ok, ra_ether_phy_close());
  TEST_END("link_status_get: LPA 10half resolves speed to k_ra_ether_phy_speed_10h");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_ether_phy_cov.c\n");
  return 0;
}
