/**
 * @file test_ra_eth_gwca_default_cov.c
 * @brief Coverage-focused unit tests for ra_eth_gwca_default.c
 *
 * @details
 * Companion to test_ra_eth_gwca.c. This TU targets the one-call
 ...
 * bottom): the two ::ra_eth_gwca_set_operation_mode call sites and the
 * three ::ra_eth_gwca_reload_queue / ::ra_eth_gwca_kick_tx failure
 * returns can only fail on real silicon (the RA_SIMULATOR_MODE build
 * short-circuits the poll to k_ra_ok, and every index that would make
 ...
 * space and re-gate every module so ::ra_eth_gwca_init can re-enable
 ...
 * mutate a single field afterward to steer ::ra_eth_gwca_default_open
 ...
 */
static void make_valid_state(ra_eth_gwca_default_state_t* st,
                             ra_gwca_basic_descriptor_t*  linkfix,
                             ra_gwca_basic_descriptor_t*  rx_chain,
                             uint8_t*                     rx_pool,
                             ra_gwca_ext_descriptor_t*    tx_chain,
                             uint8_t*                     tx_pool)
{
  *st                = (ra_eth_gwca_default_state_t){};
 ...
 *
 * @par MC/DC:
 * Two single-condition guards in ::ra_eth_gwca_default_send:
 *   D1: `if (len == 0U)`
 *     - V_F: len = 64          -> false (falls through)
 *     - V_T: len = 0           -> true  (returns k_ra_err_invalid_arg)
 *   D2: `if (len > state->tx_slot_bytes)`
 *     - V_F: len = 64, cap 64  -> false (falls through)
 *     - V_T: len = 65, cap 64  -> true  (returns k_ra_err_invalid_arg)
...
static void test_default_send_guards_and_rearm(void)
{
  ...
  prep();
  ra_eth_gwca_default_state_t                          st = {};
  [[gnu::aligned(16)]] static ra_gwca_ext_descriptor_t tx_chain[2];
  static uint8_t                                       tx_pool[128];
  static uint8_t                                       frame[64];

  tx_chain[0]       = (ra_gwca_ext_descriptor_t){};
  tx_chain[1]       = (ra_gwca_ext_descriptor_t){};
  st.tx_chain       = tx_chain;
  st.tx_depth       = 2U;
  st.tx_pool        = tx_pool;
  ...
  /* Null-pointer guards (RA_CHECK_NULL_PTR). */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_eth_gwca_default_send(nullptr, frame, 64U));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_eth_gwca_default_send(&st, nullptr, 64U));

  /* D1 V_T: len == 0. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_eth_gwca_default_send(&st, frame, 0U));
  /* D2 V_T: len > tx_slot_bytes. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_eth_gwca_default_send(&st, frame, 65U));
   ...
   * RA_SIMULATOR_MODE (the write-back spin is host-compiled-out). */
  tx_chain[1].dt = (uint8_t)k_ra_gwdcc_dt_lempty;
  TEST_ASSERT_EQ(k_ra_ok, ra_eth_gwca_default_send(&st, frame, 64U));
  TEST_ASSERT_EQ(k_ra_gwdcc_dt_link, tx_chain[1].dt);
  TEST_ASSERT_EQ(k_ra_gwdcc_dt_fsingle, tx_chain[0].dt);
 ...
 *
 * @par MC/DC:
 * Single condition `if (reload_err != k_ra_ok)` in
 * ::ra_eth_gwca_default_send:
 *   - V_T: tx_queue_index = 40 (>= 32, no GWDCC register) ->
 *     ::ra_eth_gwca_reload_queue returns k_ra_err_invalid_arg ->
...
static void test_default_send_reload_failure(void)
{
  ...
  tx_chain[0]       = (ra_gwca_ext_descriptor_t){};
  tx_chain[1]       = (ra_gwca_ext_descriptor_t){};
  tx_chain[1].dt    = (uint8_t)k_ra_gwdcc_dt_link; /* not LEMPTY -> rearm no-op */
  st.tx_chain       = tx_chain;
  st.tx_depth       = 2U;
  st.tx_pool        = tx_pool;
  st.tx_slot_bytes  = 64U;
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_eth_gwca_default_send(&st, frame, 64U));
 ...
 *   In ::ra_eth_gwca_default_recv, `if (err == k_ra_err_no_data)`:
...
static void test_default_recv_rearm_disabled(void)
{
  ...
  prep();
  ra_eth_gwca_default_state_t                            st = {};
  [[gnu::aligned(16)]] static ra_gwca_basic_descriptor_t rx_chain[4];
  static uint8_t                                         rx_pool[3U * 128U];
  ...
  st.rx_queue_index = 0U;
  st.rx_head        = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_eth_gwca_init_ring(rx_chain, 4U, 128U));

  /* Force the terminator into the GWCA-disabled state. */
  rx_chain[3].dt = (uint8_t)k_ra_gwdcc_dt_lempty;
  TEST_ASSERT_EQ(k_ra_err_no_data, ra_eth_gwca_default_recv(&st, out, sizeof(out), &got_len));
  /* Terminator re-armed to LINK, software cursor snapped to 0. */
  TEST_ASSERT_EQ(k_ra_gwdcc_dt_link, rx_chain[3].dt);
 ...
 * k_ra_err_invalid_arg return that ::ra_eth_gwca_rx_frame propagates.
...
static void test_rx_drain_reject(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_eth_gwca_init_ring(rx_chain, 4U, 128U));
  rx_chain[0].dt    = (uint8_t)k_ra_gwdcc_dt_fsingle;
  rx_chain[0].ptr_h = 0U;
  rx_chain[0].ptr_l = 0U;
  rx_chain[0].ds_l  = 128U;
  rx_chain[0].ds_h  = 0U;
  st.rx_head        = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_eth_gwca_default_recv(&st, out, sizeof(out), &got_len));
  ...
  rx_chain[0].dt    = (uint8_t)k_ra_gwdcc_dt_fsingle;
  rx_chain[0].ptr_h = 0U;
  rx_chain[0].ptr_l = 0x100U;
  rx_chain[0].ds_l  = 0xFFU;
  rx_chain[0].ds_h  = 0xFU;
  st.rx_head        = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_eth_gwca_default_recv(&st, out, 1U, &got_len));
 ...
 *   - V_T: tx_depth = 1 -> returns k_ra_err_invalid_arg, which the
...
static void test_default_open_bad_tx_depth(void)
{
  ...
  prep();
  [[gnu::aligned(16)]] static ra_gwca_basic_descriptor_t linkfix[8];
  [[gnu::aligned(16)]] static ra_gwca_basic_descriptor_t rx_chain[4];
  static uint8_t                                         rx_pool[3U * 128U];
  [[gnu::aligned(16)]] static ra_gwca_ext_descriptor_t   tx_chain[2];
  static uint8_t                                         tx_pool[128];
  ...
  make_valid_state(&st, linkfix, rx_chain, rx_pool, tx_chain, tx_pool);
  st.tx_depth = 1U; /* < 2 -> internal_tx_ext_init rejects */

  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_eth_gwca_default_open(&st));
  TEST_ASSERT_EQ(k_ra_eth_gwca_step_fail_2, g_ra_eth_gwca_pre_step);
  TEST_ASSERT_EQ(k_ra_eth_gwca_step_fail_1, g_ra_eth_gwca_open_step);
 ...
 *   - V_T: tx_slot_bytes = 4096 (> 2048) -> k_ra_err_invalid_arg.
...
static void test_default_open_bad_tx_slot_bytes(void)
{
  ...
  [[gnu::aligned(16)]] static ra_gwca_basic_descriptor_t rx_chain[4];
  static uint8_t                                         rx_pool[3U * 128U];
  [[gnu::aligned(16)]] static ra_gwca_ext_descriptor_t   tx_chain[4];
  static uint8_t                                         tx_pool[3U * 128U];
  ra_eth_gwca_default_state_t                            st = {};
 ...
 *
 * @par MC/DC:
 * Single condition `if (err != k_ra_ok)` guarding the bring_up call
 * in ::internal_default_open_pre:
 *   - V_T: linkfix_count = 33 (> 32) -> ::ra_eth_gwca_bring_up rejects
...
static void test_default_open_bad_linkfix(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_eth_gwca_default_open(&st));
  TEST_ASSERT_EQ(k_ra_eth_gwca_step_fail_3, g_ra_eth_gwca_pre_step);
  TEST_ASSERT_EQ(k_ra_eth_gwca_step_fail_1, g_ra_eth_gwca_open_step);
 ...
 *
 * @par MC/DC:
 * Single condition `if (err != k_ra_ok)` guarding the queues helper
 * in ::ra_eth_gwca_default_open:
 *   - V_T: rx_queue_index = 40 (>= 32, no GWDCC register) ->
 *     ::ra_eth_gwca_configure_queue rejects -> the queues helper
...
static void test_default_open_bad_rx_queue(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_eth_gwca_default_open(&st));
  TEST_ASSERT_EQ(k_ra_eth_gwca_step_fail_2, g_ra_eth_gwca_open_step);
...
static void test_default_open_happy(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_eth_gwca_default_open(&st));
  TEST_ASSERT_EQ(k_ra_eth_gwca_step_ok_3, g_ra_eth_gwca_open_step);
  TEST_ASSERT_EQ(0U, st.rx_head);
...
/**
 * @test default_open propagates a ra_eth_gwca_init failure.
 *
 * @par MC/DC:
 * Single condition `if (err != k_ra_ok)` guarding the init call in
 * ::internal_default_open_pre:
 *   - V_T: the ESWM MSTP ref count is pre-saturated to UINT8_MAX by
 *     255 successful ::ra_eth_gwca_init calls, so the 256th enable
 *     inside default_open returns k_ra_err_invalid_state -> pre stamps
...
static void test_default_open_init_saturation(void)
{
  ...
  /* Deliberately does NOT call prep() after saturating: ra_mstp_init
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_eth_gwca_default_open(&st));
  TEST_ASSERT_EQ(k_ra_eth_gwca_step_fail_1, g_ra_eth_gwca_pre_step);
  TEST_ASSERT_EQ(k_ra_eth_gwca_step_fail_1, g_ra_eth_gwca_open_step);
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_eth_gwca_default_cov.c\n");
  return 0;
}
