/**
 * @file test_ra_img_arena_cov.c
 * @brief Coverage supplement for libs/ra_reflow/src/ra_img_arena.c
 *
 * @details
 * Drives the ten source lines not reached by the existing image-decode
 * integration tests:
 *
 *  - ra_img_arena_malloc() partial-capacity OOM (source line 65): the
 *    aligned request does not fit the remaining bytes even though the
 *    raw size is within the total capacity.
 *
 *  - ra_img_arena_realloc_sized() non-nullptr p path (lines 93-102):
 ...
 * test_ra_reflow_image.c or by earlier tests within this file.
 *
 ...
 * @brief ra_img_arena_malloc returns nullptr when the aligned request exceeds
 ...
 *    Covered by test_ra_reflow_image.c.
 *
 * @pre None.
...
{
  TEST_BEGIN("ra_img_arena_malloc: partial-capacity OOM (line 65)");
  static uint8_t s_buf[32U];
  ra_img_arena_t arena = {.base = s_buf, .cap = sizeof s_buf, .offset = 0U, .live = 0U};
 ...
 * @brief ra_img_arena_realloc_sized with non-nullptr p and sufficient arena
 ...
 *
 * @par MC/DC:
 * Decisions in ra_img_arena_realloc_sized (ra_img_arena.c):
 *
...
{
  TEST_BEGIN("ra_img_arena_realloc_sized: non-nullptr p, success + copy > 0");
  static uint8_t s_buf[128U];
  ra_img_arena_t arena = {.base = s_buf, .cap = sizeof s_buf, .offset = 0U, .live = 0U};
  ra_img_arena_bind(&arena);

  uint8_t* const old_blk = (uint8_t*)ra_img_arena_malloc(16U);
  TEST_ASSERT_NOT_NULL(old_blk);
  memset(old_blk, 0xAB, 16U);

  /* Grow to 32 bytes: oldsz(16) < newsz(32) -> copy = 16 -> memcpy runs. */
  uint8_t* const new_blk = (uint8_t*)ra_img_arena_realloc_sized(old_blk, 16U, 32U);
 ...
 * @brief ra_img_arena_realloc_sized returns nullptr when the fresh allocation
 ...
 * for the requested new size.  ra_img_arena_malloc returns nullptr for the
...
{
  TEST_BEGIN("ra_img_arena_realloc_sized: non-nullptr p, fresh-alloc fails");
  static uint8_t s_buf[32U];
  ra_img_arena_t arena = {.base = s_buf, .cap = sizeof s_buf, .offset = 0U, .live = 0U};
  ra_img_arena_bind(&arena);
 ...
 * @brief ra_img_arena_realloc_sized with oldsz == 0 skips the memcpy
...
{
  TEST_BEGIN("ra_img_arena_realloc_sized: oldsz=0 skips memcpy (line 98 false)");
 ...
 * @brief ra_img_arena_realloc_sized with oldsz > newsz selects copy = newsz
...
{
  TEST_BEGIN("ra_img_arena_realloc_sized: shrink (oldsz > newsz -> copy = newsz)");
  static uint8_t s_buf[128U];
  ra_img_arena_t arena = {.base = s_buf, .cap = sizeof s_buf, .offset = 0U, .live = 0U};
  ra_img_arena_bind(&arena);

  uint8_t* const old_blk = (uint8_t*)ra_img_arena_malloc(32U);
  TEST_ASSERT_NOT_NULL(old_blk);
  ...
  uint8_t* const new_blk = (uint8_t*)ra_img_arena_realloc_sized(old_blk, 32U, 16U);
  TEST_ASSERT_NOT_NULL(new_blk);
...
int32_t main(void)
{
  ...
  test_realloc_shrink();
  (void)fprintf(stderr, "[OK ] test_ra_img_arena_cov.c\n");
  return 0;
}
