...
static void test_init_happy_ch1(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_channel_init(k_ra_lvd_ch1, &cfg));
  ...
  TEST_ASSERT_EQ(k_ra_lvd_pvdlvl_2_85v, (cmpcr & (uint8_t)k_ra_lvd_cmpcr_mask_pvdlvl));
  ...
  TEST_ASSERT_EQ(k_ra_lvd_edge_fall, (cr1 & (uint8_t)k_ra_lvd_cr1_mask_idtsel));
...
static void test_init_n_channel_no_cr1(void)
{
  ...
  *ra_lvd_reg8(k_ra_lvd_pvd2_cr1_off) = 0xA5U;
  ...
  cfg.irq_enable   = true;
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_channel_init(k_ra_lvd_ch4, &cfg));

  const uint8_t cmpcr = *ra_lvd_reg8(k_ra_lvd_pvd4_cmpcr_off);
  TEST_ASSERT((cmpcr & (uint8_t)k_ra_lvd_cmpcr_mask_pvde) != 0U);
  TEST_ASSERT_EQ(0xA5U, *ra_lvd_reg8(k_ra_lvd_pvd2_cr1_off));
...
static void test_init_n_channel_rejects_irq_response(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_lvd_channel_init(k_ra_lvd_ch4, &cfg));

  cfg.response = k_ra_lvd_response_nmi;
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_lvd_channel_init(k_ra_lvd_ch5, &cfg));
...
static void test_init_null_cfg(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_lvd_channel_init(k_ra_lvd_ch1, nullptr));
...
static void test_init_bad_channel(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_lvd_channel_init((ra_lvd_channel_t)k_ra_lvd_test_bogus_ch, &cfg));
...
static void test_init_bad_threshold(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_lvd_channel_init(k_ra_lvd_ch1, &cfg));
...
static void test_deinit_clears_regs(void)
{
  ...
  const ra_lvd_cfg_t cfg = make_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_channel_init(k_ra_lvd_ch2, &cfg));
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_channel_deinit(k_ra_lvd_ch2));

  TEST_ASSERT_EQ(0U, *ra_lvd_reg8(k_ra_lvd_pvd2_cmpcr_off));
  /* CR0 carries the bit3 always-write-1 marker even at "deinit". */
  const uint8_t cr0 = *ra_lvd_reg8(k_ra_lvd_pvd2_cr0_off);
  TEST_ASSERT_EQ(k_ra_lvd_cr0_mask_bit3, (cr0 & (uint8_t)k_ra_lvd_cr0_mask_bit3));
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_cmpe) == 0U);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_rie) == 0U);
  TEST_ASSERT_EQ(0U, *ra_lvd_reg8(k_ra_lvd_pvd2_cr1_off));
  TEST_ASSERT_EQ(0U, *ra_lvd_reg8(k_ra_lvd_pvd2_sr_off));
  TEST_ASSERT_EQ(0U, *ra_lvd_reg8(k_ra_lvd_pvd2_fcr_off));

  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_lvd_channel_deinit((ra_lvd_channel_t)k_ra_lvd_test_bogus_ch));
...
static void test_set_threshold_preserves_pvde(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_set_threshold(k_ra_lvd_ch1, k_ra_lvd_pvdlvl_1_71v));

  const uint8_t cmpcr = *ra_lvd_reg8(k_ra_lvd_pvd1_cmpcr_off);
  TEST_ASSERT((cmpcr & (uint8_t)k_ra_lvd_cmpcr_mask_pvde) != 0U);
  TEST_ASSERT_EQ(k_ra_lvd_pvdlvl_1_71v, (cmpcr & (uint8_t)k_ra_lvd_cmpcr_mask_pvdlvl));

  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_lvd_set_threshold(k_ra_lvd_ch1, (ra_lvd_pvdlvl_t)0x20U));
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_lvd_set_threshold((ra_lvd_channel_t)k_ra_lvd_test_bogus_ch, k_ra_lvd_pvdlvl_2_85v));
...
static void test_set_irq_edge(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_set_irq_edge(k_ra_lvd_ch1, k_ra_lvd_edge_both));
  const uint8_t cr1 = *ra_lvd_reg8(k_ra_lvd_pvd1_cr1_off);
  TEST_ASSERT_EQ(k_ra_lvd_edge_both, (cr1 & (uint8_t)k_ra_lvd_cr1_mask_idtsel));
  /* IRQSEL still set from init (maskable). */
  TEST_ASSERT((cr1 & (uint8_t)k_ra_lvd_cr1_mask_irqsel) != 0U);

  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_lvd_set_irq_edge(k_ra_lvd_ch1, (ra_lvd_edge_t)0x3U));
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_lvd_set_irq_edge(k_ra_lvd_ch4, k_ra_lvd_edge_rise));
...
static void test_set_irq_kind(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_set_irq_kind(k_ra_lvd_ch1, k_ra_lvd_irq_nmi));
  const uint8_t cr1 = *ra_lvd_reg8(k_ra_lvd_pvd1_cr1_off);
  TEST_ASSERT((cr1 & (uint8_t)k_ra_lvd_cr1_mask_irqsel) == 0U);
  TEST_ASSERT_EQ(k_ra_lvd_edge_rise, (cr1 & (uint8_t)k_ra_lvd_cr1_mask_idtsel));

  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_lvd_set_irq_kind(k_ra_lvd_ch5, k_ra_lvd_irq_maskable));
...
static void test_enable_disable_irq(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_enable_irq(k_ra_lvd_ch1));
  cr0 = *ra_lvd_reg8(k_ra_lvd_pvd1_cr0_off);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_rie) != 0U);

  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_disable_irq(k_ra_lvd_ch1));
  cr0 = *ra_lvd_reg8(k_ra_lvd_pvd1_cr0_off);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_rie) == 0U);

  /* n-channels reject IRQ enable. */
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_lvd_enable_irq(k_ra_lvd_ch4));
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_lvd_disable_irq(k_ra_lvd_ch5));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_lvd_enable_irq((ra_lvd_channel_t)k_ra_lvd_test_bogus_ch));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_lvd_disable_irq((ra_lvd_channel_t)k_ra_lvd_test_bogus_ch));
...
static void test_enable_disable_reset_m(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_enable_reset(k_ra_lvd_ch1));
  uint8_t cr0 = *ra_lvd_reg8(k_ra_lvd_pvd1_cr0_off);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_ri) != 0U);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_rie) != 0U);

  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_disable_reset(k_ra_lvd_ch1));
  cr0 = *ra_lvd_reg8(k_ra_lvd_pvd1_cr0_off);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_rie) == 0U);
...
static void test_enable_disable_reset_n(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_unlock_n_channels());
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_enable_reset(k_ra_lvd_ch4));
  uint8_t cr0 = *ra_lvd_reg8(k_ra_lvd_pvd4_cr0_off);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_re) != 0U);

  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_disable_reset(k_ra_lvd_ch4));
...
static void test_enable_disable_cmpe(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_disable_cmpe(k_ra_lvd_ch1));
  uint8_t cr0 = *ra_lvd_reg8(k_ra_lvd_pvd1_cr0_off);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_cmpe) == 0U);

  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_enable_cmpe(k_ra_lvd_ch1));
  cr0 = *ra_lvd_reg8(k_ra_lvd_pvd1_cr0_off);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_cmpe) != 0U);

  /* Bad channel still rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_lvd_enable_cmpe((ra_lvd_channel_t)k_ra_lvd_test_bogus_ch));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_lvd_disable_cmpe((ra_lvd_channel_t)k_ra_lvd_test_bogus_ch));
...
static void test_set_filter_full_range(void)
{
  ...
  for (uint8_t i = 0U; i < (uint8_t)(sizeof(divs) / sizeof(divs[0])); ++i) {
    TEST_ASSERT_EQ(k_ra_ok, ra_lvd_set_filter(k_ra_lvd_ch1, divs[i], true));
    const uint8_t cr0 = *ra_lvd_reg8(k_ra_lvd_pvd1_cr0_off);
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_set_filter(k_ra_lvd_ch1, k_ra_lvd_loco_div_2, false));
  const uint8_t cr0 = *ra_lvd_reg8(k_ra_lvd_pvd1_cr0_off);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_dfdis) != 0U);

  /* Bad div / bad channel rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_lvd_set_filter(k_ra_lvd_ch1, (ra_lvd_loco_div_t)0x4U, true));
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_lvd_set_filter((ra_lvd_channel_t)k_ra_lvd_test_bogus_ch, k_ra_lvd_loco_div_2, true));
...
static void test_set_hysteresis_lvd_then_hvd(void)
{
  ...
  /* LVD -> HVD */
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_set_hysteresis_mode(k_ra_lvd_ch1, k_ra_lvd_hysteresis_hvd));
  TEST_ASSERT_EQ(k_ra_lvd_fcr_mask_rhsel,
                 (*ra_lvd_reg8(k_ra_lvd_pvd1_fcr_off) & (uint8_t)k_ra_lvd_fcr_mask_rhsel));

  /* HVD -> LVD */
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_set_hysteresis_mode(k_ra_lvd_ch1, k_ra_lvd_hysteresis_lvd));
  TEST_ASSERT_EQ(0U, (*ra_lvd_reg8(k_ra_lvd_pvd1_fcr_off) & (uint8_t)k_ra_lvd_fcr_mask_rhsel));

  /* Bad value rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_lvd_set_hysteresis_mode(k_ra_lvd_ch1, (ra_lvd_hysteresis_t)0x2U));
...
static void test_set_hysteresis_hvd_requires_ri(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_state,
                 ra_lvd_set_hysteresis_mode(k_ra_lvd_ch1, k_ra_lvd_hysteresis_hvd));

  /* n channels are not subject to the RI check. */
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_unlock_n_channels());
  ra_lvd_cfg_t ncfg = make_cfg();
  ncfg.response     = k_ra_lvd_response_reset;
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_channel_init(k_ra_lvd_ch4, &ncfg));
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_set_hysteresis_mode(k_ra_lvd_ch4, k_ra_lvd_hysteresis_hvd));
...
static void test_set_negate_mode(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_set_negate_mode(k_ra_lvd_ch1, k_ra_lvd_negate_after_assert));
  uint8_t cr0 = *ra_lvd_reg8(k_ra_lvd_pvd1_cr0_off);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_rn) != 0U);

  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_set_negate_mode(k_ra_lvd_ch1, k_ra_lvd_negate_after_voltage));
  cr0 = *ra_lvd_reg8(k_ra_lvd_pvd1_cr0_off);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_rn) == 0U);

  /* Bad arg + n-channel rejection + RHSEL=HVD conflict. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_lvd_set_negate_mode(k_ra_lvd_ch1, (ra_lvd_negate_t)0x2U));
  TEST_ASSERT_EQ(k_ra_err_not_supported,
                 ra_lvd_set_negate_mode(k_ra_lvd_ch4, k_ra_lvd_negate_after_voltage));

  /* Force RI=1 so HVD becomes legal. Then HVD + RN=1 conflicts. */
  ra_lvd_cfg_t cfg2 = make_cfg();
  cfg2.response     = k_ra_lvd_response_reset;
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_channel_init(k_ra_lvd_ch2, &cfg2));
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_set_hysteresis_mode(k_ra_lvd_ch2, k_ra_lvd_hysteresis_hvd));
  TEST_ASSERT_EQ(k_ra_err_invalid_state,
                 ra_lvd_set_negate_mode(k_ra_lvd_ch2, k_ra_lvd_negate_after_assert));
...
static void test_status_read_and_clear(void)
{
  ...
  *ra_lvd_reg8(k_ra_lvd_pvd1_sr_off) =
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_get_status(k_ra_lvd_ch1, &st));
  TEST_ASSERT(st.crossed);
  TEST_ASSERT(st.above);

  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_clear_status(k_ra_lvd_ch1));

  /* DET should be cleared, MON preserved. */
  const uint8_t sr = *ra_lvd_reg8(k_ra_lvd_pvd1_sr_off);
  TEST_ASSERT_EQ(0U, (sr & (uint8_t)k_ra_lvd_sr_mask_det));
  TEST_ASSERT_EQ(k_ra_lvd_sr_mask_mon, (sr & (uint8_t)k_ra_lvd_sr_mask_mon));

  /* n-channels: get_status / clear_status must report unsupported. */
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_lvd_get_status(k_ra_lvd_ch4, &st));
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_lvd_clear_status(k_ra_lvd_ch5));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_lvd_get_status(k_ra_lvd_ch1, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_lvd_get_status((ra_lvd_channel_t)k_ra_lvd_test_bogus_ch, &st));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_lvd_clear_status((ra_lvd_channel_t)k_ra_lvd_test_bogus_ch));
...
static void test_set_security(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_set_security((uint32_t)k_ra_lvd_pvdsar_mask_nonsec0));
  TEST_ASSERT_EQ(k_ra_lvd_pvdsar_mask_nonsec0, *ra_lvd_reg32(k_ra_lvd_pvdsar_off));

  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_set_security((uint32_t)k_ra_lvd_pvdsar_mask_all));
  TEST_ASSERT_EQ(k_ra_lvd_pvdsar_mask_all, *ra_lvd_reg32(k_ra_lvd_pvdsar_off));

  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_set_security(0U));
  TEST_ASSERT_EQ(0U, *ra_lvd_reg32(k_ra_lvd_pvdsar_off));

  /* Reserved bit -- driver must reject. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_lvd_set_security((uint32_t)k_ra_lvd_test_pvdsar_bad));
...
static void test_unlock_relock_n_channels(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_unlock_n_channels());
  TEST_ASSERT_EQ(0U, *ra_lvd_reg8(k_ra_lvd_pvdlr_off));

  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_relock_n_channels());
  TEST_ASSERT_EQ(k_ra_lvd_pvdlr_mask_lock, *ra_lvd_reg8(k_ra_lvd_pvdlr_off));
...
static void test_elc_event_enable_disable(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_disable_cmpe(k_ra_lvd_ch1));
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_enable_elc_event(k_ra_lvd_ch1));
  uint8_t cr0 = *ra_lvd_reg8(k_ra_lvd_pvd1_cr0_off);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_cmpe) != 0U);

  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_disable_elc_event(k_ra_lvd_ch1));
  cr0 = *ra_lvd_reg8(k_ra_lvd_pvd1_cr0_off);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_cmpe) == 0U);

  /* n channels reject ELC ops. */
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_lvd_enable_elc_event(k_ra_lvd_ch4));
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_lvd_disable_elc_event(k_ra_lvd_ch4));
...
static void test_configure_for_standby_m(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_configure_for_standby(k_ra_lvd_ch1));
  const uint8_t cr0 = *ra_lvd_reg8(k_ra_lvd_pvd1_cr0_off);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_dfdis) != 0U);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_ri) == 0U);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_rn) == 0U);
...
static void test_configure_for_standby_n(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_configure_for_standby(k_ra_lvd_ch4));
  const uint8_t cr0 = *ra_lvd_reg8(k_ra_lvd_pvd4_cr0_off);
  TEST_ASSERT((cr0 & (uint8_t)k_ra_lvd_cr0_mask_dfdis) != 0U);
...
static void test_cancel_deep_standby_path(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_cancel_deep_standby_path());
  TEST_ASSERT((*ra_lvd_reg8(k_ra_lvd_pvd1_cr0_off) & (uint8_t)k_ra_lvd_cr0_mask_ri) == 0U);
  TEST_ASSERT((*ra_lvd_reg8(k_ra_lvd_pvd2_cr0_off) & (uint8_t)k_ra_lvd_cr0_mask_ri) == 0U);
...
static void test_attach_and_dispatch_shared(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_attach_handler(stub_cb, (void*)(uintptr_t)k_ra_lvd_test_ctx_val));
  ...
  *ra_lvd_reg8(k_ra_lvd_pvd2_sr_off) = (uint8_t)k_ra_lvd_sr_mask_det;
  ...
  TEST_ASSERT_EQ(k_ra_lvd_ch2, s_cb_last_ch);
  TEST_ASSERT_EQ(k_ra_lvd_test_ctx_val, (uintptr_t)s_cb_last_ctx);
  /* DET should now be cleared. */
  TEST_ASSERT_EQ(0U, (*ra_lvd_reg8(k_ra_lvd_pvd2_sr_off) & (uint8_t)k_ra_lvd_sr_mask_det));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_attach_handler(nullptr, nullptr));
...
static void test_attach_per_channel_handler(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_lvd_attach_channel_handler(k_ra_lvd_ch1,
                                               stub_chan_cb,
  ...
  *ra_lvd_reg8(k_ra_lvd_pvd1_sr_off) = (uint8_t)k_ra_lvd_sr_mask_det;
  ...
  TEST_ASSERT_EQ(k_ra_lvd_test_ctx_chan, (uintptr_t)s_cb_last_ctx);
  ...
  *ra_lvd_reg8(k_ra_lvd_pvd2_sr_off) = (uint8_t)k_ra_lvd_sr_mask_det;
  ...
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_lvd_attach_channel_handler((ra_lvd_channel_t)k_ra_lvd_test_bogus_ch, stub_chan_cb, nullptr));
  TEST_ASSERT_EQ(k_ra_err_not_supported,
                 ra_lvd_attach_channel_handler(k_ra_lvd_ch4, stub_chan_cb, nullptr));
...
static void test_every_threshold_value(void)
{
  ...
  for (uint8_t v = (uint8_t)k_ra_lvd_pvdlvl_min; v <= (uint8_t)k_ra_lvd_pvdlvl_max; ++v) {
    ...
    TEST_ASSERT_EQ(k_ra_ok, ra_lvd_channel_init(k_ra_lvd_ch1, &cfg));
    const uint8_t cmpcr = *ra_lvd_reg8(k_ra_lvd_pvd1_cmpcr_off);
    TEST_ASSERT_EQ(v, (cmpcr & (uint8_t)k_ra_lvd_cmpcr_mask_pvdlvl));
    TEST_ASSERT_EQ(k_ra_ok, ra_lvd_channel_deinit(k_ra_lvd_ch1));
  }
...
static void test_mcdc_lvd(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_channel_init(k_ra_lvd_ch1, &cfg_v1));
  TEST_ASSERT_EQ(k_ra_err_invalid_state,
                 ra_lvd_set_hysteresis_mode(k_ra_lvd_ch1, k_ra_lvd_hysteresis_hvd));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_channel_init(k_ra_lvd_ch4, &cfg_v3));
  TEST_ASSERT_EQ(k_ra_ok, ra_lvd_set_hysteresis_mode(k_ra_lvd_ch4, k_ra_lvd_hysteresis_hvd));
