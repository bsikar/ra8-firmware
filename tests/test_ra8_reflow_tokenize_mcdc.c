/**
 * @file test_ra8_reflow_tokenize_mcdc.c
 * @brief Supplemental MC/DC unit tests for libs/ra8_reflow/src/ra8_reflow_tokenize.c
 *
 * @details
 * Companion to `tests/test_ra8_reflow_tokenize.c`. That file establishes the
 ...
 * (`ra8_reflow_tok_decode_entity`) or the full single-pass walk
 ...
 * the public `ra8_reflow_init` / `ra8_reflow_register_face` /
 * `ra8_reflow_layout_chapter` API with the baked Ahem face.
 *
 ...
 * single-pass walk, exactly as `tests/test_ra8_reflow_tokenize.c` does, so the
...
static uint32_t count_kind(ra8_reflow_token_kind_t kind)
{
  uint32_t n = 0U;
  for (uint32_t i = 0U; i < s_engine.token_count; ++i) {
 ...
 * hex vs decimal in priv_decode_numeric (ra8_reflow_tokenize.c).
...
static void test_numeric_base_select(void)
{
  ...
  uint32_t cp   = 0U;
  size_t   used = 0U;
  TEST_ASSERT(ra8_reflow_tok_decode_entity("&#65;", 5U, &cp, &used)); /* V1 decimal */
  TEST_ASSERT_EQ(k_cp_uppercase_a, cp);
  TEST_ASSERT_EQ(k_used_dec_a, used);
  TEST_ASSERT(ra8_reflow_tok_decode_entity("&#x4f;", 6U, &cp, &used)); /* V2 'x' hex */
  TEST_ASSERT_EQ(k_cp_hex_4f, cp);
  TEST_ASSERT(ra8_reflow_tok_decode_entity("&#X4F;", 6U, &cp, &used)); /* V3 'X' hex */
  TEST_ASSERT_EQ(k_cp_hex_4f_upper, cp);
  TEST_ASSERT(!ra8_reflow_tok_decode_entity("&#x", k_avail_three, &cp, &used)); /* V4 short */
...
static void test_numeric_digit_classes(void)
{
  ...
  uint32_t cp   = 0U;
  size_t   used = 0U;
  TEST_ASSERT(ra8_reflow_tok_decode_entity("&#x2a;", 6U, &cp, &used)); /* V1 dec + lc-hex */
  TEST_ASSERT_EQ(0x2AU, cp);
  TEST_ASSERT(ra8_reflow_tok_decode_entity("&#x1B;", 6U, &cp, &used)); /* V2 dec + uc-hex */
  TEST_ASSERT_EQ(0x1BU, cp);
  TEST_ASSERT(!ra8_reflow_tok_decode_entity("&#x1g;", 6U, &cp, &used)); /* V3 bad hex digit   */
  TEST_ASSERT(!ra8_reflow_tok_decode_entity("&#1a;", 5U, &cp, &used));  /* V4 'a' not decimal */
 ...
 * post-loop terminator check in priv_decode_numeric (ra8_reflow_tokenize.c).
...
static void test_numeric_terminator(void)
{
  ...
  uint32_t cp   = 0U;
  size_t   used = 0U;
  TEST_ASSERT(ra8_reflow_tok_decode_entity("&#65;", 5U, &cp, &used)); /* V1 accept */
  TEST_ASSERT_EQ(k_cp_uppercase_a, cp);
  TEST_ASSERT(!ra8_reflow_tok_decode_entity("&#;", 3U, &cp, &used));  /* V2 no digits     */
  TEST_ASSERT(!ra8_reflow_tok_decode_entity("&#65", 4U, &cp, &used)); /* V3 no terminator */
 ...
 * reject in ra8_reflow_tok_decode_entity (ra8_reflow_tokenize.c). `window` is
...
static void test_entity_short_guard(void)
{
  ...
  TEST_ASSERT(ra8_reflow_tok_decode_entity("&amp;", 5U, &cp, &used)); /* V1 proceed */
  TEST_ASSERT_EQ('&', cp);
  TEST_ASSERT(!ra8_reflow_tok_decode_entity("&lt", 3U, &cp, &used));    /* V2 window<min */
  TEST_ASSERT(!ra8_reflow_tok_decode_entity(nul_at_1, 5U, &cp, &used)); /* V3 src[1]==0  */
 ...
 * the named-entity match in ra8_reflow_tok_decode_entity (ra8_reflow_tokenize.c).
...
static void test_named_entity_match(void)
{
  ...
  uint32_t cp   = 0U;
  size_t   used = 0U;
  TEST_ASSERT(ra8_reflow_tok_decode_entity("&quot;", 6U, &cp, &used)); /* V1 match */
  TEST_ASSERT_EQ('"', cp);
  TEST_ASSERT(!ra8_reflow_tok_decode_entity("&ampX", 5U, &cp, &used)); /* V2 no ';'        */
  TEST_ASSERT(!ra8_reflow_tok_decode_entity("&zzz;", 5U, &cp, &used)); /* V3 name mismatch */
 ...
 *
 * @par MC/DC:
 * Decisions in priv_handle_cdata (ra8_reflow_tokenize.c):
 ...
 *
 * @par MC/DC:
 * Decisions in priv_handle_end (ra8_reflow_tokenize.c):
...
/**
 * @brief ra8_reflow_css_loader_fn stub that hands back ::k_ext_css for any href.
 *
...
/**
 * @brief ra8_reflow_css_loader_fn stub that always fails (no bytes).
 *
 ...
 * @note Test helper; not thread-safe.
 */
static ra8_err_t css_fail_stub(void*           ctx,
                               const char*     href,
                               uint32_t        href_len,
 ...
 *
 * @return The first text token's 0xRRGGBB colour (or k_ra8_reflow_color_inherit),
 ...
 * priv_decode_numeric (ra8_reflow_tokenize.c). The existing vectors V1-V4 in
...
static void test_numeric_prefix_avail_zero(void)
{
  ...
  TEST_ASSERT(!ra8_reflow_tok_decode_entity("&#", 2U, &cp, &used));
...
static void test_numeric_digit_subrange(void)
{
  ...
  TEST_ASSERT(!ra8_reflow_tok_decode_entity("&#/;", 4U, &cp, &used));
  /* V7: hex mode, c='g' -> c>='a' true, c<='f' false -> reject (L262). */
  TEST_ASSERT(!ra8_reflow_tok_decode_entity("&#xg;", 5U, &cp, &used));
 ...
 *
 * @par MC/DC:
 * Decision: `if ((tlen > 0U) && !ra8_reflow_tok_emit(...))` -- the CDATA text emit
 * in priv_handle_cdata (libs/ra8_reflow/src/ra8_reflow_tokenize.c, 2 conditions, AND).
 ...
 *
 * @par MC/DC:
 * Decision: `if (ra8_reflow_tok_is_block(tag) && !priv_suppressed(ctx) &&
 * !ra8_reflow_tok_emit(...))` -- the block-end emit in priv_handle_end
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra8_reflow_tokenize_mcdc.c\n");
  return 0;
}
