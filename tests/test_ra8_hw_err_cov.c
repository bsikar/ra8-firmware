 ...
 *        (`ra8_hw_err.h`) and the host MMIO fault seam (`ra8_sim_mmio.c`).
 *
 * @par Tag
 * [Ring 3 / HAL] {World: S}
 *
 ...
 * build (`RA8_SIMULATOR_MODE` + `UNIT_TEST`) each waiter consults the programmable
 ...
 * Arming `ra8_sim_mmio_fail_wait` forces a deterministic timeout regardless of
 * RAM; arming `ra8_sim_mmio_satisfy_after(n)` makes the flag "assert" on the
 ...
 * drop-in replacement for the deleted ``#ifdef RA8_SIMULATOR_MODE return k_ra8_ok``
...
{
  TEST_BEGIN("ra8_sim_mmio_satisfy_after asserts the flag mid-wait");
  ra8_sim_mmio_reset();

  volatile uint32_t r32 = 0U; /* RAM never sets it; the seam does, on poll n. */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_sim_mmio_satisfy_after(&r32, k_test_satisfy_n));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_hw_wait_flag_set32(&r32, (uint32_t)k_test_mask32, k_test_budget));

  volatile uint8_t r8 = 0U; /* n == 0 -> satisfied on the first poll. */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_sim_mmio_satisfy_after(&r8, 0U));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_hw_wait_flag_set8(&r8, (uint8_t)k_test_mask8, k_test_budget));
  TEST_END("ra8_sim_mmio_satisfy_after asserts the flag mid-wait");
}
...
{
  TEST_BEGIN("ra8_sim_mmio arm functions validate inputs + table capacity");
  ra8_sim_mmio_reset();

  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_sim_mmio_fail_wait(nullptr));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_sim_mmio_satisfy_after(nullptr, k_test_satisfy_n));
  ...
  for (uint8_t i = 0U; i < (uint8_t)k_ra8_sim_mmio_max_faults; ++i) {
    TEST_ASSERT_EQ(k_ra8_ok, ra8_sim_mmio_fail_wait(&regs[i]));
  }
  TEST_ASSERT_EQ(k_ra8_err_no_mem,
                 ra8_sim_mmio_fail_wait(&regs[(uint8_t)k_ra8_sim_mmio_max_faults]));
  ra8_sim_mmio_reset();
  TEST_END("ra8_sim_mmio arm functions validate inputs + table capacity");
}
...
{
  TEST_BEGIN("ra8_sim_mmio reset restores transparency; re-arm updates in place");
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_sim_mmio_satisfy_after(&r32, 0U));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_sim_mmio_fail_wait(&r32));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout,
                 ra8_hw_wait_flag_set32(&r32, (uint32_t)k_test_mask32, k_test_budget));
  ra8_sim_mmio_reset();
  TEST_END("ra8_sim_mmio reset restores transparency; re-arm updates in place");
}

/**
 * @test test_sim_mmio_wait_eval_direct
 * @details Directly exercises ::ra8_sim_mmio_wait_eval: an un-armed register (NULL
...
{
  TEST_BEGIN("ra8_sim_mmio_wait_eval succeeds for an un-armed register");
  ra8_sim_mmio_reset();
  TEST_ASSERT(ra8_sim_mmio_wait_eval(nullptr, 0U, true));
  TEST_ASSERT(ra8_sim_mmio_wait_eval(nullptr, 0U, false)); /* un-armed -> succeed. */

  volatile uint32_t r32 = 0U;
  TEST_ASSERT(ra8_sim_mmio_wait_eval(&r32, 0U, true));
  TEST_ASSERT(ra8_sim_mmio_wait_eval(&r32, 0U, false)); /* un-armed -> succeed. */
  TEST_END("ra8_sim_mmio_wait_eval succeeds for an un-armed register");
}
