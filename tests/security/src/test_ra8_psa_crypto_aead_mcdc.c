 ...
 * tests/hal/src/test_ra8_xspi.c::test_set_xip_mode_mcdc_addr_bytes for the
 * reference template).
 *
...
static ra8_err_t enc_ct(ra8_psa_key_t  key,
                        ...
                        size_t*        ct_len)
{
  const uint8_t plain[k_psa_mcdc_plain_len] = {};
  return ra8_psa_aead_encrypt(key,
                              k_ra8_psa_alg_aes_gcm,
                              k_test_nonce,
                              sizeof(k_test_nonce),
...
static void test_mcdc_aead_encrypt_check_plain_aad_quad(void)
{
  ...
  TEST_ASSERT_NOT_NULL(k);
  const uint8_t plain[k_psa_mcdc_plain_len] = {};
  const uint8_t aad[k_psa_mcdc_short_aad]   = {};
...
static void test_mcdc_aead_decrypt_size_pair(void)
{
  ...
  uint8_t tiny[k_psa_mcdc_tiny_len] = {};
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
