 ...
 * canonical N+1 pattern (see tests/hal/src/test_ra8_xspi.c::
...
static void test_mcdc_handle_valid_range_check(void)
{
  ...
  struct ra8_psa_key_handle* const below =
    (ra8_psa_key_t)(in_pool - (uintptr_t)k_psa_pool_far_span);
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_psa_key_destroy(below));
  /* V3: at/above the pool end -> C1 false, C2 (handle >= end) true. */
  struct ra8_psa_key_handle* const above =
    (ra8_psa_key_t)(in_pool + (uintptr_t)k_psa_pool_far_span);
