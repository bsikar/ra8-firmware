 ...
 * @par Example:
 * @code
 * internal_ra8_esp_hosted_tx_shim_set_ticks(k_t_rtos_ticks_pre_wrap);
 * @endcode
 * @see internal_ra8_esp_hosted_tx_shim_set_ticks
 * @since 0.1.0
 */
 ...
 * @brief Verify timer cb.
 * @details Implements the fixture-only timer cb operation with bounded static state.
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_timer_cb(void* arg)
{
  (void)arg;
  s_timer_hits++;
}
 ...
 * @brief Verify thread body.
 * @details Implements the fixture-only thread body operation with bounded static state.
 * @param[in,out] arg Callback-owned fixture state passed through the host model.
 * @pre Host mock storage is initialized. @pre Pointer arguments follow their directions.
 * @post The mock transition is observable. @post No physical hardware is accessed.
 * @note Host-only deterministic fixture code; it does not access physical hardware.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_thread_body(void const* arg)
{
...
/**
 * @test internal_test_ms_to_ticks_maps_every_class
 ...
 */
RA8_INTERNAL static void internal_test_ms_to_ticks_maps_every_class(void)
{
  TEST_BEGIN("ms_to_ticks maps 0 / positive / negative / HOSTED_BLOCK_MAX");
  TEST_ASSERT_EQ(0U, priv_ra8_esp_hosted_rtos_ms_to_ticks(0));
  TEST_ASSERT_EQ(250U, priv_ra8_esp_hosted_rtos_ms_to_ticks(250));
  TEST_ASSERT_EQ(0xFFFFFFFFU, priv_ra8_esp_hosted_rtos_ms_to_ticks(-1));
  TEST_ASSERT_EQ(0xFFFFFFFFU, priv_ra8_esp_hosted_rtos_ms_to_ticks((int)HOSTED_BLOCK_MAX));
  TEST_ASSERT_EQ(0xFFFFFFFFU, priv_ra8_esp_hosted_rtos_ms_to_ticks(-100000));
...
/**
 * @test internal_test_queue_words_rounds_up_and_rejects_oversize
 ...
 * `port/esp-hosted/src/ra8_esp_hosted_rtos_pool.c@priv_ra8_esp_hosted_rtos_queue_words`
 * (2 conditions).
 ...
 * @post The focused scenario leaves no unverified result.
 */
RA8_INTERNAL static void internal_test_queue_words_rounds_up_and_rejects_oversize(void)
{
  TEST_BEGIN("queue_words rounds up and rejects over sixteen words");
  TEST_ASSERT_EQ(7U, priv_ra8_esp_hosted_rtos_queue_words(28U));
  TEST_ASSERT_EQ(8U, priv_ra8_esp_hosted_rtos_queue_words(29U));
  TEST_ASSERT_EQ(1U, priv_ra8_esp_hosted_rtos_queue_words(1U));
  TEST_ASSERT_EQ(16U, priv_ra8_esp_hosted_rtos_queue_words(64U));
  TEST_ASSERT_EQ(0U, priv_ra8_esp_hosted_rtos_queue_words(0U));
  TEST_ASSERT_EQ(0U, priv_ra8_esp_hosted_rtos_queue_words(65U));
  TEST_ASSERT_EQ(0U, priv_ra8_esp_hosted_rtos_queue_words(68U));
...
/**
 * @test internal_test_us_spin_iters_guards_and_clamps
 ...
 * `port/esp-hosted/src/ra8_esp_hosted_rtos.c@priv_ra8_esp_hosted_rtos_us_spin_iters`
 * (2 conditions).
 ...
 * @post The focused scenario leaves no unverified result.
 */
RA8_INTERNAL static void internal_test_us_spin_iters_guards_and_clamps(void)
{
  TEST_BEGIN("us_spin_iters guards zero inputs and clamps long spins");
  TEST_ASSERT_EQ(250U, priv_ra8_esp_hosted_rtos_us_spin_iters(1000000000U, 1U));
  TEST_ASSERT_EQ(0U, priv_ra8_esp_hosted_rtos_us_spin_iters(0U, 1U));
  TEST_ASSERT_EQ(0U, priv_ra8_esp_hosted_rtos_us_spin_iters(1000000000U, 0U));
  ...
  TEST_ASSERT_EQ(1U, priv_ra8_esp_hosted_rtos_us_spin_iters(100000U, 1U));
  /* 999 us at 1 GHz is 249750 iterations, just inside the 250000 clamp. */
  TEST_ASSERT_EQ(249750U, priv_ra8_esp_hosted_rtos_us_spin_iters(1000000000U, 999U));
  TEST_ASSERT_EQ(250000U, priv_ra8_esp_hosted_rtos_us_spin_iters(1000000000U, 5000U));
...
/**
 * @test internal_test_slot_take_claims_then_reports_full
 ...
 * @post The focused scenario leaves no unverified result.
 */
RA8_INTERNAL static void internal_test_slot_take_claims_then_reports_full(void)
{
  TEST_BEGIN("slot_take claims free rows then reports the table full");
  bool used[3] = {};
  TEST_ASSERT_EQ(3U, priv_ra8_esp_hosted_rtos_slot_take(NULL, 3U));
  TEST_ASSERT_EQ(0U, priv_ra8_esp_hosted_rtos_slot_take(used, 3U));
  TEST_ASSERT_EQ(1U, priv_ra8_esp_hosted_rtos_slot_take(used, 3U));
  TEST_ASSERT_EQ(2U, priv_ra8_esp_hosted_rtos_slot_take(used, 3U));
  TEST_ASSERT_EQ(3U, priv_ra8_esp_hosted_rtos_slot_take(used, 3U));
  TEST_END("slot_take claims free rows then reports the table full");
}

/**
 * @test internal_test_slot_index_rejects_null_foreign_and_freed
 ...
 * `port/esp-hosted/src/ra8_esp_hosted_rtos.c@priv_ra8_esp_hosted_rtos_slot_index`.
 ...
 * @post The focused scenario leaves no unverified result.
 */
RA8_INTERNAL static void internal_test_slot_index_rejects_null_foreign_and_freed(void)
{
  ...
  TEST_ASSERT_EQ(1U,
                 priv_ra8_esp_hosted_rtos_slot_index(&rows[1], rows, sizeof(rows[0]), 2U, used));
  TEST_ASSERT_EQ(2U, priv_ra8_esp_hosted_rtos_slot_index(NULL, rows, sizeof(rows[0]), 2U, used));
  TEST_ASSERT_EQ(2U,
                 priv_ra8_esp_hosted_rtos_slot_index(&rows[1], NULL, sizeof(rows[0]), 2U, used));
  TEST_ASSERT_EQ(2U,
                 priv_ra8_esp_hosted_rtos_slot_index(&rows[1], rows, sizeof(rows[0]), 2U, NULL));
  TEST_ASSERT_EQ(2U,
                 priv_ra8_esp_hosted_rtos_slot_index(&elsewhere, rows, sizeof(rows[0]), 2U, used));
  used[1] = false;
  TEST_ASSERT_EQ(2U,
                 priv_ra8_esp_hosted_rtos_slot_index(&rows[1], rows, sizeof(rows[0]), 2U, used));
...
/**
 * @test internal_test_init_rejects_second_call_and_deinit_needs_init
 ...
 * @post The focused scenario leaves no unverified result.
 */
RA8_INTERNAL static void internal_test_init_rejects_second_call_and_deinit_needs_init(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_not_initialized, priv_ra8_esp_hosted_rtos_deinit());
  TEST_ASSERT_EQ(k_ra8_ok, priv_ra8_esp_hosted_rtos_init());
  TEST_ASSERT_EQ(true, priv_ra8_esp_hosted_rtos_is_ready());
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, priv_ra8_esp_hosted_rtos_init());
...
/**
 * @test internal_test_init_reports_a_refused_byte_pool
 ...
 * @post The focused scenario leaves no unverified result.
 */
RA8_INTERNAL static void internal_test_init_reports_a_refused_byte_pool(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_rtos_error, priv_ra8_esp_hosted_rtos_init());
  TEST_ASSERT_EQ(false, priv_ra8_esp_hosted_rtos_is_ready());
  TEST_END("init reports a refused byte pool");
}

/**
 * @test internal_test_bind_fills_its_rows_and_rejects_null
 ...
 * @post The focused scenario leaves no unverified result.
 */
RA8_INTERNAL static void internal_test_bind_fills_its_rows_and_rejects_null(void)
{
  TEST_BEGIN("each binder fills its own rows and rejects a null table");
  hosted_osi_funcs_t table = {};
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, priv_ra8_esp_hosted_rtos_bind(NULL));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, priv_ra8_esp_hosted_rtos_bind_pool(NULL));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, priv_ra8_esp_hosted_rtos_bind_sync(NULL));

  TEST_ASSERT_EQ(k_ra8_ok, priv_ra8_esp_hosted_rtos_bind(&table));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, priv_ra8_esp_hosted_rtos_bind_pool(&table));
  TEST_ASSERT_NOT_NULL(table._h_malloc);
  TEST_ASSERT_NOT_NULL(table._h_create_queue);
  TEST_ASSERT_NULL(table._h_create_mutex);

  TEST_ASSERT_EQ(k_ra8_ok, priv_ra8_esp_hosted_rtos_bind_sync(&table));
...
/**
 * @test internal_test_thread_create_bounds_cancel_and_table
 ...
 * Both name-copy call sites provide the fixed slot buffer and its positive
 * size, so ::internal_copy_name treats those as preconditions; its nullable
 * source remains a separate decision driven below. The entry shim checks the
 * occupancy flag once. A live row's callback is non-null by construction,
 * and cancellation clears the callback before publishing the row as free.
 ...
 * @post The focused scenario leaves no unverified result.
 */
RA8_INTERNAL static void internal_test_thread_create_bounds_cancel_and_table(void)
{
  ...
  TEST_ASSERT_NULL(
    s_funcs._h_thread_create("t", DFLT_TASK_PRIO, 64U, internal_thread_body, &marker));
  TEST_ASSERT_NULL(
    s_funcs._h_thread_create("t", DFLT_TASK_PRIO, 8192U, internal_thread_body, &marker));

  void* const t = s_funcs._h_thread_create("spi_trans",
                                           DFLT_TASK_PRIO,
                                           DFLT_TASK_STACK_SIZE,
                                           internal_thread_body,
                                           &marker);
  ...
  /* Run the trampoline the way ThreadX would; it must reach internal_thread_body. */
  cb->entry(cb->input);
  ...
  void* const t3 = s_funcs._h_thread_create(NULL,
                                            DFLT_TASK_PRIO,
                                            DFLT_TASK_STACK_SIZE,
                                            internal_thread_body,
                                            NULL);
  ...
  void* const t4 =
    s_funcs._h_thread_create("d", DFLT_TASK_PRIO, DFLT_TASK_STACK_SIZE, internal_thread_body, NULL);
  TEST_ASSERT_NOT_NULL(t4);
  TEST_ASSERT_NULL(s_funcs._h_thread_create("e",
                                            DFLT_TASK_PRIO,
                                            DFLT_TASK_STACK_SIZE,
                                            internal_thread_body,
                                            NULL));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, priv_ra8_esp_hosted_rtos_deinit());
  TEST_ASSERT_NULL(s_funcs._h_thread_create("t",
                                            DFLT_TASK_PRIO,
                                            DFLT_TASK_STACK_SIZE,
                                            internal_thread_body,
                                            NULL));
...
/**
 * @test internal_test_timer_oneshot_and_periodic
 ...
 * (true,true).
 * Firing a timer also drives the expiry shim's occupancy decision. A live
 * row's callback is non-null by construction; stop clears the callback before
 * publishing the row as free, so no second invariant check is needed.
 * @pre The port is initialised.
 ...
 * @post The focused scenario leaves no unverified result.
 */
RA8_INTERNAL static void internal_test_timer_oneshot_and_periodic(void)
{
  ...
  TEST_ASSERT_NULL(s_funcs._h_timer_start("t", 0, H_TIMER_TYPE_ONESHOT, internal_timer_cb, NULL));
  TEST_ASSERT_NULL(s_funcs._h_timer_start("t", 50, 99, internal_timer_cb, NULL));

  void* const one =
    s_funcs._h_timer_start("once", 50, H_TIMER_TYPE_ONESHOT, internal_timer_cb, NULL);
  TEST_ASSERT_NOT_NULL(one);
  TEST_ASSERT_EQ(0U, ((const TX_TIMER*)one)->reschedule);
  TEST_ASSERT_EQ(true, internal_ra8_esp_hosted_tx_shim_fire_timer((TX_TIMER*)one));
  TEST_ASSERT_EQ(1U, s_timer_hits);
  TEST_ASSERT_EQ(false, ((const TX_TIMER*)one)->active);
  TEST_ASSERT_EQ(false, internal_ra8_esp_hosted_tx_shim_fire_timer((TX_TIMER*)one));

  void* const rep =
    s_funcs._h_timer_start("rep", 20, H_TIMER_TYPE_PERIODIC, internal_timer_cb, NULL);
  TEST_ASSERT_NOT_NULL(rep);
  TEST_ASSERT_EQ(20U, ((const TX_TIMER*)rep)->reschedule);
  TEST_ASSERT_EQ(true, internal_ra8_esp_hosted_tx_shim_fire_timer((TX_TIMER*)rep));
  TEST_ASSERT_EQ(true, internal_ra8_esp_hosted_tx_shim_fire_timer((TX_TIMER*)rep));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, priv_ra8_esp_hosted_rtos_deinit());
  TEST_ASSERT_NULL(
    s_funcs._h_timer_start("down", 50, H_TIMER_TYPE_ONESHOT, internal_timer_cb, NULL));
  TEST_END("one-shot timers stop after one expiry; periodic ones repeat");
}

/**
 * @test internal_test_timer_table_exhaustion
 ...
 * @post The focused scenario leaves no unverified result.
 */
RA8_INTERNAL static void internal_test_timer_table_exhaustion(void)
{
...
/**
 * @test internal_test_get_time_ms_extends_across_a_32_bit_rollover
 ...
 * @post The focused scenario leaves no unverified result.
 */
RA8_INTERNAL static void internal_test_get_time_ms_extends_across_a_32_bit_rollover(void)
{
  ...
  internal_ra8_esp_hosted_tx_shim_set_ticks(k_t_rtos_ticks_late_pre_wrap);
  ...
  internal_ra8_esp_hosted_tx_shim_set_ticks(0x00000010U);
  ...
  internal_ra8_esp_hosted_tx_shim_set_ticks(0x00000020U);
...
/**
 * @test internal_test_sleeps_yield_and_delays
 ...
 * count and a count above the bound. Both delay loops use the compile-time
 * maximum as their sole loop condition and break once `i >= iters`; this keeps
 * the NASA Rule 2 bound local while avoiding an impossible compound condition
 * between a value and the same constant that already clamps it. The zero,
 * within-bound, and above-bound vectors below exercise immediate break,
 * requested break, and maximum-bound termination respectively.
 * @pre The port is initialised.
 ...
 * @post The focused scenario leaves no unverified result.
 */
RA8_INTERNAL static void internal_test_sleeps_yield_and_delays(void)
{
  TEST_BEGIN("sleeps advance the kernel clock; usleep splits; delays spin");
  internal_reset_port();
  internal_ra8_esp_hosted_tx_shim_set_ticks(0U);
...
/**
 * @test internal_test_deinit_releases_every_outstanding_object
 ...
 * `port/esp-hosted/src/ra8_esp_hosted_rtos.c@priv_ra8_esp_hosted_rtos_deinit`, and
 * the semaphore and mutex walks in
 * `port/esp-hosted/src/ra8_esp_hosted_rtos_sync.c@priv_ra8_esp_hosted_rtos_sync_deinit`
 * (2 conditions each).
 * - Vector 1: used=T, the release refuses -> true; the worst-error is raised.
 *   Driven by ::internal_test_deinit_reports_a_refused_delete, which arms the model to
 ...
 * @post The focused scenario leaves no unverified result.
 */
RA8_INTERNAL static void internal_test_deinit_releases_every_outstanding_object(void)
{
  ...
  TEST_ASSERT_NOT_NULL(
    s_funcs._h_timer_start("t", 10, H_TIMER_TYPE_PERIODIC, internal_timer_cb, NULL));
  TEST_ASSERT_NOT_NULL(s_funcs._h_thread_create("t",
                                                DFLT_TASK_PRIO,
                                                DFLT_TASK_STACK_SIZE,
                                                internal_thread_body,
                                                NULL));
...
/**
 * @test internal_test_deinit_reports_a_refused_delete
 ...
 * success in ::internal_test_deinit_releases_every_outstanding_object.
 ...
 * ::internal_test_deinit_releases_every_outstanding_object.
 * @pre The port is initialised.
 * @post The port is left down.
 * @note Not thread-safe; single-threaded test context.
 * @since 0.1.0
 ...
 * @post The focused scenario leaves no unverified result.
 */
RA8_INTERNAL static void internal_test_deinit_reports_a_refused_delete(void)
{
  TEST_BEGIN("deinit reports a refused delete");
  internal_reset_port();
  TEST_ASSERT_NOT_NULL(s_funcs._h_create_queue(4U, 28U));
  internal_ra8_esp_hosted_tx_shim_arm(k_ra8_esp_hosted_tx_shim_family_queue, TX_QUEUE_ERROR);
  TEST_ASSERT_EQ(k_ra8_err_rtos_error, priv_ra8_esp_hosted_rtos_deinit());
  TEST_ASSERT_EQ(false, priv_ra8_esp_hosted_rtos_is_ready());
  ...
  TEST_ASSERT_NOT_NULL(
    s_funcs._h_timer_start("t", 10, H_TIMER_TYPE_PERIODIC, internal_timer_cb, NULL));
  internal_ra8_esp_hosted_tx_shim_arm(k_ra8_esp_hosted_tx_shim_family_timer, TX_TIMER_ERROR);
  TEST_ASSERT_EQ(k_ra8_err_rtos_error, priv_ra8_esp_hosted_rtos_deinit());
  ...
  TEST_ASSERT_NOT_NULL(s_funcs._h_thread_create("t",
                                                ...
                                                internal_thread_body,
                                                NULL));
  internal_ra8_esp_hosted_tx_shim_arm(k_ra8_esp_hosted_tx_shim_family_thread, TX_THREAD_ERROR);
  TEST_ASSERT_EQ(k_ra8_err_rtos_error, priv_ra8_esp_hosted_rtos_deinit());

  /* A live semaphore whose delete is refused: the sync walk must record it. */
  internal_reset_port();
  TEST_ASSERT_NOT_NULL(s_funcs._h_create_semaphore(1));
  internal_ra8_esp_hosted_tx_shim_arm(k_ra8_esp_hosted_tx_shim_family_semaphore,
                                      TX_SEMAPHORE_ERROR);
  TEST_ASSERT_EQ(k_ra8_err_rtos_error, priv_ra8_esp_hosted_rtos_deinit());
  ...
  internal_reset_port();
  TEST_ASSERT_NOT_NULL(s_funcs._h_create_mutex());
  internal_ra8_esp_hosted_tx_shim_arm(k_ra8_esp_hosted_tx_shim_family_mutex, TX_MUTEX_ERROR);
  TEST_ASSERT_EQ(k_ra8_err_rtos_error, priv_ra8_esp_hosted_rtos_deinit());
  TEST_ASSERT_EQ(false, priv_ra8_esp_hosted_rtos_is_ready());
...
int main(void)
{
  ...
  internal_test_us_spin_iters_guards_and_clamps();
  internal_test_slot_take_claims_then_reports_full();
  internal_test_slot_index_rejects_null_foreign_and_freed();
  internal_test_init_rejects_second_call_and_deinit_needs_init();
  internal_test_init_reports_a_refused_byte_pool();
  internal_test_bind_fills_its_rows_and_rejects_null();
  internal_test_thread_create_bounds_cancel_and_table();
  internal_test_timer_oneshot_and_periodic();
  internal_test_timer_table_exhaustion();
  internal_test_get_time_ms_extends_across_a_32_bit_rollover();
  internal_test_sleeps_yield_and_delays();
  internal_test_deinit_releases_every_outstanding_object();
  internal_test_deinit_reports_a_refused_delete();
  if (priv_ra8_esp_hosted_rtos_is_ready()) {
    (void)priv_ra8_esp_hosted_rtos_deinit();
  }
  return 0;
