...
static void test_run_self_test(void)
{
  ...
  /* The unarmed fake wait models the completion decision that dumb RAM cannot
   * provide; production still reads the exact target register. */
  volatile ra8_dotf_regs_t* reg = ra8_dotf_regs((uint8_t)k_dotf_test_ch0);
  TEST_ASSERT_EQ(0, reg->REG00);

  /* HUM Ch 45.3 "Register Descriptions" p 3049 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&reg->REG00));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_dotf_run_self_test((uint8_t)k_dotf_test_ch0, &snap));
  /* HUM Ch 45.3 "Register Descriptions" p 3049 */
  TEST_ASSERT_EQ(0, reg->REG00);
  ra8_fake_mmio_reset();
...
/**
 * @test test_open_propagates_public_failures
 * @brief Malformed public configuration and MSTP faults propagate from open.
 * @par MC/DC:
 * Each error guard is a single-condition decision paired with
 * ::test_open_arms_channel: invalid key, invalid region, invalid SCA, and
 * failed MSTP read-back drive the true legs; the valid fixture drives false.
 * @pre Fake MMIO and DOTF state are reset before each vector.
 * @post Each malformed input is rejected with ::k_ra8_err_invalid_arg.
 * @post MSTP failure leaves the channel unopened and a disarmed retry succeeds.
 * @note Single-threaded host fixture.
 * @since 0.1.0
 */
static void test_open_propagates_public_failures(void)
{
  TEST_BEGIN("dotf open propagates malformed input and MSTP failures");
  ...
  prep();
  cfg                   = make_open_cfg((uint8_t)k_dotf_test_ch0, false);
  cfg.region.start_addr = (uint32_t)k_dotf_test_start_bad;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_dotf_open(&cfg));

  prep();
  cfg           = make_open_cfg((uint8_t)k_dotf_test_ch0, false);
  cfg.sca_level = (ra8_dotf_sca_level_t)k_dotf_test_bad_sca;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_dotf_open(&cfg));
  ...
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_dotf_open(&cfg));
  ra8_fake_mmio_reset();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_dotf_open(&cfg));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_dotf_close());

  TEST_END("dotf open propagates malformed input and MSTP failures");
}
...
int main(void)
{
  ...
  test_open_propagates_public_failures();
