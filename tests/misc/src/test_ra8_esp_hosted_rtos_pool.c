 ...
 * (2 conditions) is covered by ::k_t_pool_msg_bytes (control), zero, and
 * ::k_t_pool_msg_over_bytes.
...
RA8_INTERNAL static void internal_test_queue_create_bounds_and_table_exhaustion(void)
{
  ...
  internal_reset_port();
  TEST_ASSERT_NULL(s_funcs._h_create_queue(4U, 0U));
 ...
 *
 * @par MC/DC:
 * Decision `!s_sync.ready || (max_count <= 0)` in
...
internal_test_semaphore_starts_at_one_so_the_vendored_drain_empties_it(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, priv_ra8_esp_hosted_rtos_deinit());
  TEST_ASSERT_NULL(s_funcs._h_create_semaphore(3));
