 ...
 * @brief Prove SSIE initialization propagates its module-stop timeout.
 * @details Arms MSTPCRC through the existing fake-MMIO wait seam, then repeats
 * the same public call with the seam disarmed as the successful control.
 * @par MC/DC:
 * Decision: ``RA8_RETURN_ON_ERROR(mst_err, ...)`` in the SSIE power-up helper.
 ...
 * @pre The channel-zero SSIE register window is mapped.
...
{
  TEST_BEGIN("ssie init propagates MSTP timeout");
  prep();
  ra8_fake_mmio_reset();
  const ra8_ssie_cfg_t cfg = make_controller_i2s_cfg();
  /* HUM Ch 11.2.8 "MSTPCRC : Module Stop Control Register C", p 446 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&ra8_mstp()->MSTPCRC));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_ssie_init((uint8_t)k_ra8_ssie_test_ch0, &cfg));

  prep();
  ra8_fake_mmio_reset();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ssie_init((uint8_t)k_ra8_ssie_test_ch0, &cfg));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ssie_deinit((uint8_t)k_ra8_ssie_test_ch0));
  TEST_END("ssie init propagates MSTP timeout");
}
...
/**
 * @brief SSIE reset waits surface init, FIFO, and recovery timeouts.
 * @par MC/DC:
 * The compound direction validator in
 * ``libs/ra8_hal/src/ra8_ssie.c@ra8_ssie_start`` is held on its valid-TX
 * vector and is covered by the direction-validation tests. These three
 * vectors independently drive the single-condition reset-error guards in
 * init, start, and recovery.
 * @since 0.1.0
 */
static void test_reset_wait_timeouts(void)
{
  TEST_BEGIN("ssie reset wait timeouts");
  prep();
  ra8_fake_mmio_reset();
  const ra8_ssie_cfg_t    cfg = make_controller_i2s_cfg();
  volatile r_ssie_regs_t* reg = ra8_ssie((uint8_t)k_ra8_ssie_test_ch0);
  /* HUM Ch 46.2.3 "SSIFCR : FIFO Control Register" p 3077 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&reg->SSIFCR));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_ssie_init((uint8_t)k_ra8_ssie_test_ch0, &cfg));
  ...
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout,
                 ra8_ssie_start((uint8_t)k_ra8_ssie_test_ch0, k_ra8_ssie_dir_tx));

  ra8_fake_mmio_reset();
  /* HUM Ch 46.2.3 "SSIFCR : FIFO Control Register" p 3077 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&reg->SSIFCR));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_ssie_start_recovery((uint8_t)k_ra8_ssie_test_ch0));
  ra8_fake_mmio_reset();
  TEST_END("ssie reset wait timeouts");
}
