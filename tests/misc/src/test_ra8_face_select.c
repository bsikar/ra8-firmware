 ...
 * multi-face book is never serialized/loaded) and `reflow_register_face`'s
 * validation + cap.
 *
...
static void tfs_init(void)
{
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_init((uint16_t)k_tfs_viewport_w,
...
static void tfs_layout(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    reflow_layout_chapter(&s_engine, (const uint8_t*)k_chapter, strlen(k_chapter), &pages));
}
...
static int32_t tfs_face_of(int32_t cp)
{
  for (uint32_t i = 0U; i < s_engine.glyph_count; ++i) {
    if (s_engine.glyphs[i].cp == cp) {
      return (int32_t)(((uint32_t)s_engine.glyphs[i].style >> (uint32_t)k_reflow_face_shift) &
...
static void tfs_register_all_faces(void)
{
  for (uint16_t ci = 0U; ci < s_engine.css.face_count; ++ci) {
    TEST_ASSERT_EQ(
      k_ra8_ok,
      reflow_register_face(&s_engine, (uint8_t)ci, k_fixture_ahem, (size_t)k_fixture_ahem_len));
  }
}
 ...
 *
 * @par MC/DC:
 * Decision: the `priv_resolve_face_slot` guard (apps/shared_libs/reflow/src/
 * reflow_tokenize.c) `if (face_count==0 || (set & family)==0 ||
...
static void test_cache_bypass_with_faces(void)
{
  ...
  size_t n = 0U;
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_cache_serialize(&s_engine,
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_register_face(&s_engine, 0U, k_fixture_ahem, (size_t)k_fixture_ahem_len));
  size_t n2 = 0U;
  ...
  TEST_ASSERT_EQ(
    ...
    reflow_cache_load(&s_engine, (const uint8_t*)k_chapter, strlen(k_chapter), s_blob, n));
...
/**
 * @test `reflow_register_face` validates its inputs, rejects an invalid blob
 *       without disturbing the registry, and enforces the face cap.
 *
 * @par MC/DC:
 * Decision: `if (engine == nullptr || blob == nullptr)` (2 conditions, OR,
 * `reflow_register_face` in apps/shared_libs/reflow/src/reflow_layout_driver.c).
 ...
 * decision (DO-178C 6.4.4.3) in `reflow_bind_font` (test_bind_measure_and_mcdc).
 */
static void test_register_validation(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 reflow_register_face(nullptr, 0U, k_fixture_ahem, (size_t)k_fixture_ahem_len));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 reflow_register_face(&s_engine, 0U, nullptr, (size_t)k_fixture_ahem_len));
  TEST_ASSERT_EQ(
    k_ra8_err_invalid_size,
    reflow_register_face(&s_engine, 0U, k_fixture_ahem, (size_t)k_reflow_min_font_bytes - 1U));
  ...
  TEST_ASSERT_EQ(k_ra8_err_not_supported,
                 reflow_register_face(&s_engine, 0U, s_garbage, sizeof(s_garbage)));
  TEST_ASSERT_EQ(0, s_engine.face_count);

  /* Fill to the cap; the next register is rejected (no_mem). */
  for (uint8_t k = 0U; k < (uint8_t)k_reflow_max_faces; ++k) {
    TEST_ASSERT_EQ(k_ra8_ok,
                   reflow_register_face(&s_engine, k, k_fixture_ahem, (size_t)k_fixture_ahem_len));
  }
  TEST_ASSERT_EQ(k_reflow_max_faces, s_engine.face_count);
