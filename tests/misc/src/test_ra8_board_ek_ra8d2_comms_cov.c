...
/**
 * @test test_mipi_dsi_mstp_timeout_propagates
 * @brief A failed DSI module-stop release is returned by the board wrapper.
 * @par MC/DC:
 * Decision: `if (err != k_ra8_ok)` after `ra8_mipi_dsi_init()` (1 condition).
 * - Vector 1: MSTPCRC settles -> false (the existing PHY-success case).
 * - Vector 2: MSTPCRC never settles -> true (this case).
 * N+1 = 2 vectors; only the module-stop read-back outcome differs.
 * @pre Fake MMIO and module-stop state are reset.
 * @pre The MIPI PHY ready bit is planted so control reaches DSI init.
 * @post The DSI timeout is propagated unchanged.
 * @post No HS-clock start is attempted.
 * @note Single-threaded host fixture.
 * @since 0.1.0
 */
static void test_mipi_dsi_mstp_timeout_propagates(void)
{
  TEST_BEGIN("mipi_dsi_init: MSTP timeout propagates");
  reset_state();
  ...
  *ra8_mipi_phy_reg32(k_ra8_mipi_phy_off_sfr) = (uint32_t)k_ra8_mipi_phy_sfr_ready_mask;
  ...
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_board_mipi_dsi_init());
  ra8_fake_mmio_reset();
  TEST_END("mipi_dsi_init: MSTP timeout propagates");
}
...
/**
 * @test test_uart_console_init_mstp_timeout
 * @brief The board console wrapper propagates SCI8 module-stop failure.
 * @par MC/DC:
 * Decision: `if (err != k_ra8_ok)` after `ra8_sci_init()` (1 condition).
 * - Vector 1: MSTPCRB settles -> false (the successful console-init case).
 * - Vector 2: MSTPCRB never settles -> true (this case).
 * N+1 = 2 vectors; only the module-stop read-back differs.
 * @pre PCLKA is initialized and the console pins are unclaimed.
 * @pre SCI8 has no module-stop reference.
 * @post The SCI error is returned and the console remains uninitialized.
 * @post The failed release leaves the SCI8 reference count at zero.
 * @note Must run before the successful console-init/read cases.
 * @since 0.1.0
 */
static void test_uart_console_init_mstp_timeout(void)
{
  TEST_BEGIN("uart_console_init: MSTP timeout propagates");
  reset_state();
  cgc_init_with_oscsf_preseed();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_init());
  ...
  TEST_ASSERT_EQ(k_ra8_err_hw_init_failed, ra8_board_uart_console_init(115200U));
  uint8_t ref = k_board_poison_len;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_get_refcount(k_ra8_mstp_sci8, &ref));
  TEST_ASSERT_EQ(0U, ref);
  ra8_fake_mmio_reset();
  TEST_END("uart_console_init: MSTP timeout propagates");
}
...
int main(void)
{
  ...
  test_mipi_dsi_mstp_timeout_propagates();
  ...
  test_uart_console_init_mstp_timeout();
