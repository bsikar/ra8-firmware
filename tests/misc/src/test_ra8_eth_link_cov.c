...
/**
 * @test internal_test_link_status_mdio_errors
 *
 * @details Injects failures on each of the two Clause-22 reads performed by
 * internal_phy_read_link and verifies the public status call preserves the
 * exact transport error. This replaces broad coverage exclusions on the BMSR
 * and BMCR error edges with executable fault evidence.
 */
RA8_INTERNAL static void internal_test_link_status_mdio_errors(void)
{
  TEST_BEGIN("link_status: BMSR and BMCR MDIO failures propagate");
  internal_prep();
  s_eth_state.opened = 1U;
  ...
  s_mdio_fail_err    = k_ra8_err_hw_timeout;
  ra8_eth_link_t out = {};
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_eth_link_status_cov(&out));

  s_mdio_fail_reg = (uint8_t)k_cov_reg_bmcr;
  s_mdio_fail_err = k_ra8_err_hw_error;
  TEST_ASSERT_EQ(k_ra8_err_hw_error, ra8_eth_link_status_cov(&out));
  TEST_END("link_status: BMSR and BMCR MDIO failures propagate");
}
...
static void (*const s_test_roster[])(void) = {
  ...
  internal_test_link_status_mdio_errors,
