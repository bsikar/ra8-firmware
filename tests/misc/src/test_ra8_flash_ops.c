 ...
 * Shared fixture constants live in support/inc/flash_test_util.h.
 *
...
/**
 * @test test_mcdc_blank_check_in_code_window_and2
 *
 * @par MC/DC:
 ...
 *   in_code = false; the address is below EVERY flash window, so the call
 *   returns k_ra8_err_invalid_arg. Pair (V1,V2) varies C1 alone. Note the
 *   address has to be BELOW k_ra8_flash_code_start (0x02000000): the
 *   out-of-region address the sibling region-OR case uses, 0x05000000, is
 *   ABOVE it and therefore leaves C1 true.
 * - V3: address = code_start, len = k_ra8_flash_code_size + 1 -> C1=T,
 *   C2=F -> in_code = false; the span starts inside the window and runs one
 *   byte past its end, so the call returns k_ra8_err_invalid_arg without ever
 *   dereferencing the address. Pair (V1,V3) varies C2 alone.
 * N = 2 conditions, N+1 = 3 vectors: minimal MC/DC. V1+V2 prove the lower
 * bound independently affects the outcome; V1+V3 prove the same for the upper
 * bound.
 *
 * @note V3 is the vector the old deactivation rationale said could not exist.
 *       `ra8_flash_blank_check` caps only `len == 0`, so a window-spanning
 *       length reaches the decision intact.
 */
static void test_mcdc_blank_check_in_code_window_and2(void)
{
  TEST_BEGIN("flash blank_check MC/DC: in_code window AND (both bounds)");
  ra8_fake_mmap_reset();
  bool blank = false;

  /* V1: fully inside the code window -> both conditions true. */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_flash_blank_check((uintptr_t)k_ra8_flash_code_start, 4U, &blank));

  /* V2: below EVERY window -> lower bound false, upper bound masked. */
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_flash_blank_check((uintptr_t)0x01000000UL, 4U, &blank));

  /* V3: starts inside the window but spans one byte past its end -> lower
     bound true, upper bound false. Rejected before any read. */
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_flash_blank_check((uintptr_t)k_ra8_flash_code_start,
                                       (uint32_t)k_ra8_flash_code_size + 1U,
                                       &blank));

  /* The exact-fit length is still accepted, so V3 is an off-by-one rejection
     and not a blanket refusal of large spans. */
  TEST_ASSERT_EQ(k_ra8_ok,
                 ra8_flash_blank_check((uintptr_t)k_ra8_flash_code_start,
                                       (uint32_t)k_ra8_flash_code_size,
                                       &blank));
  TEST_END("flash blank_check MC/DC: in_code window AND (both bounds)");
}
...
int main(void)
{
  ...
  test_mcdc_blank_check_in_code_window_and2();
