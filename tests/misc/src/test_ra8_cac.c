...
/**
 * @brief Exercise delayed and exhausted CACR0 settle waits.
 * @par MC/DC:
 * No compound decision is exercised in
 * ``libs/ra8_hal/src/ra8_cac.c@ra8_cac_init``. The delayed vector makes the
 * single-condition settle check false then true; the exhausted vector keeps
 * it false through the bounded loop.
 * @since 0.1.0
 */
static void test_init_wait_seam(void)
{
  TEST_BEGIN("cac init wait seam delayed and exhausted");
  ra8_fake_mmap_reset();
  ra8_fake_mmio_reset();
  volatile r_cac_regs_t* reg = ra8_cac();
  /* HUM Ch 10.2.1 "CACR0 : CAC Control Register 0" p 421 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_satisfy_after(&reg->CACR0, 2U));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_cac_init(1U, 0U));

  ra8_fake_mmap_reset();
  ra8_fake_mmio_reset();
  /* HUM Ch 10.2.1 "CACR0 : CAC Control Register 0" p 421 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&reg->CACR0));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_cac_init(1U, 0U));
  ra8_fake_mmio_reset();
  TEST_END("cac init wait seam delayed and exhausted");
}

/**
 * @test test_init_propagates_mstp_timeout
 * @brief Prove CAC initialization propagates a module-stop read-back timeout.
 * @details Arms the existing fake-MMIO seam on MSTPCRC, calls the public CAC
 * initialization API, and then disarms the seam for the success control.
 * @par MC/DC:
 * Decision: ``RA8_RETURN_ON_ERROR(mst_err, ...)`` in ``ra8_cac_init()``.
 ...
 * @pre The fake MMIO windows and MSTP reference counts can be reset.
 * @pre No CAC module-stop reference is held on entry.
