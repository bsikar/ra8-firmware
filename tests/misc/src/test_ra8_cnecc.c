...
/**
 * @test test_init_mstp_timeout
 * @brief CNECC initialization propagates its first module-stop failure.
 * @par MC/DC:
 * Decision: `RA8_RETURN_ON_ERROR(err, ...)` in `ra8_cnecc_init()` (1 condition).
 * - Vector 1: MSTPCRC settles -> false (the normal init case).
 * - Vector 2: MSTPCRC never settles -> true (this case).
 * N+1 = 2 vectors; only the module-stop read-back differs.
 * @pre Fake CNECC and MSTP state are reset.
 * @pre No CNECC instance holds an MSTP reference.
 * @post The timeout is returned unchanged.
 * @post A disarmed retry initializes both instances.
 * @note Single-threaded host fixture.
 * @since 0.1.0
 */
static void test_init_mstp_timeout(void)
{
  TEST_BEGIN("cnecc init propagates MSTP timeout");
  prep();
  ...
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_cnecc_init(&cfg));
  prep();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_cnecc_init(&cfg));
  TEST_END("cnecc init propagates MSTP timeout");
}
