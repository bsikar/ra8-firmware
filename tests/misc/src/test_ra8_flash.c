 ...
 * in support/inc/flash_test_util.h.
 *
...
/**
 * @test Every MENTRYR transition reports a bounded timeout when status never converges.
 *
 * @details Arms the host MMIO seam on MENTRYR before each transition. The
 * production write still occurs, but every subsequent poll reports the status
 * as unsatisfied until the fixed iteration budget expires.
 *
 * @par MC/DC:
 * Each transition has one status-bit decision. Its existing round-trip test is
 * the false-control for timeout, while the corresponding vector here forces
 * that same decision to remain unsatisfied and reaches the timeout return.
 *
 * @pre The fake MRAM register window and MMIO fault registry are reset.
 * @pre No other test has an armed MENTRYR fault.
 * @post Enter, exit, suspend, and resume each returned k_ra8_err_hw_timeout.
 * @post The MMIO fault registry was reset between transitions.
 * @note Test-only; not thread-safe because the fake register window is shared.
 * @since 0.1.0
 */
static void test_pe_mode_transition_timeouts(void)
{
  TEST_BEGIN("flash P/E transitions time out when MENTRYR status never converges");
  volatile const void* const entry_reg =
    (volatile const void*)ra8_mram_reg16((uint16_t)k_ra8_mram_off_mentryr);

  ra8_fake_mmap_reset();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(entry_reg));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_flash_enter_pe_mode());

  ra8_fake_mmio_reset();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(entry_reg));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_flash_exit_pe_mode());

  ra8_fake_mmio_reset();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(entry_reg));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_flash_suspend());

  ra8_fake_mmio_reset();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(entry_reg));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_flash_resume());

  ra8_fake_mmio_reset();
  TEST_END("flash P/E transitions time out when MENTRYR status never converges");
}
...
static void (*const s_test_roster[])(void) = {
  ...
  test_pe_mode_transition_timeouts,
