...
/**
 * @par MC/DC:
 * The poll-exit OR is false for every bounded iteration: no address match and
 * no STOP. The existing write/stop cases cover each true direction.
 */
RA8_INTERNAL static void internal_test_poll_budget_without_event(void)
{
  TEST_BEGIN("target_poll exhausts its budget without an event");
  internal_prep();
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_i2c_peripheral_poll((uint8_t)k_t_ch, &ev));
  TEST_ASSERT_EQ(k_ra8_i2c_peripheral_event_none, ev);
  TEST_END("target_poll exhausts its budget without an event");
}
...
static void (*const s_test_roster[])(void) = {
  ...
  internal_test_poll_budget_without_event,
