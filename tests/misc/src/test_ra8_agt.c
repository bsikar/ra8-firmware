...
/**
 * @test test_agt_cascade_mstp_failures
 * @brief Both sequential cascade module-stop failures propagate and roll back.
 * @par MC/DC:
 * Decisions: the two `RA8_RETURN_ON_ERROR` sites in
 * `internal_agt_cascade_mstp_enable_both()` (one condition each).
 * - Control: both MSTPCRD read-backs settle -> cascade starts.
 * - Vector 1: fail wait 0 -> AGT0 acquire fails at the first decision.
 * - Vector 2: fail wait 1 -> AGT0 succeeds and AGT1 fails at the second.
 * Each failure vector differs from the control only in one wait outcome.
 * @pre Runs before any successful AGT acquire latches channel ownership.
 * @pre Fake MMIO can count sequential waits on one register.
 * @post Failed acquisitions do not leave an unbalanced MSTP reference.
 * @post The disarmed control starts and is then deinitialized cleanly.
 * @note This case must remain first in the roster.
 * @since 0.1.0
 */
static void test_agt_cascade_mstp_failures(void)
{
  const ra8_agt_cascade_cfg_t cfg = {
    .reload32     = 0U,
    .clock        = k_ra8_agt_cascade_clk_pclkb,
  ...
  TEST_BEGIN("agt cascade propagates AGT0 and AGT1 MSTP failures");

  ra8_fake_mmap_reset();
  ra8_fake_mmio_reset();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_init());
  /* HUM Ch 11.2.9 "MSTPCRD : Module Stop Control Register D", p 448 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_nth_wait(&ra8_mstp()->MSTPCRD, 0U));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_agt_start_cascade(&cfg));

  ra8_fake_mmap_reset();
  ra8_fake_mmio_reset();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_init());
  /* HUM Ch 11.2.9 "MSTPCRD : Module Stop Control Register D" p 448 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_nth_wait(&ra8_mstp()->MSTPCRD, 1U));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_agt_start_cascade(&cfg));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_agt_deinit((uint8_t)k_ra8_agt_cascade_lo_channel));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_agt_start_cascade(&cfg));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_agt_deinit((uint8_t)k_ra8_agt_cascade_lo_channel));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_agt_deinit((uint8_t)k_ra8_agt_cascade_hi_channel));
  TEST_END("agt cascade propagates AGT0 and AGT1 MSTP failures");
}
 ...
 *   other start arm in this file).
 * - Vector 2: MSTPCRD armed to never satisfy its bounded read-back ->
 *   `ra8_mstp_enable()` returns k_ra8_err_hw_timeout -> true, and the acquire
 *   re-raises it (this case).
 * N = 1 condition, N+1 = 2 vectors. Both run the same call with only the
 * read-back outcome differing, so `err` independently affects the decision.
 *
 * @note This case must run FIRST in the roster. `s_agt_mstp_held[0]` latches
 *       on the first successful acquire, and once latched every later acquire
 *       short-circuits before ra8_mstp_enable() is ever called.
 */
static void test_agt_mstp_acquire_readback_timeout(void)
{
  TEST_BEGIN("agt mstp acquire read-back timeout");
  ra8_fake_mmap_reset();
  ra8_fake_mmio_reset();

  /* AGT0 is MSTPD5, so the acquire polls MSTPCRD. */
  /* HUM Ch 11.2.9 "MSTPCRD : Module Stop Control Register D" p 448 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait((const volatile void*)&ra8_mstp()->MSTPCRD));
  TEST_ASSERT_EQ(
    k_ra8_err_hw_timeout,
    ra8_agt_start_free_run((uint8_t)k_ra8_agt_test_channel_valid, (uint16_t)k_ra8_agt_test_reload));

  /* The refused acquire happened before any timer register was touched. */
  volatile r_agt_regs_t* reg = ra8_agt((uint8_t)k_ra8_agt_test_channel_valid);
  TEST_ASSERT_NOT_NULL((void*)reg);
  /* HUM Ch 24.2.4 "AGTCR : AGT Control Register" p 1167 */
  TEST_ASSERT_EQ(0, reg->AGTCR);
  /* HUM Ch 24.2.1 "AGT : AGT Counter" p 1165 */
  TEST_ASSERT_EQ(0, reg->AGT);

  /* Control: disarmed, the identical call programs the timer. */
  ra8_fake_mmio_reset();
  TEST_ASSERT_EQ(
    k_ra8_ok,
    ra8_agt_start_free_run((uint8_t)k_ra8_agt_test_channel_valid, (uint16_t)k_ra8_agt_test_reload));
  ...
  /* HUM Ch 24.2.4 "AGTCR : AGT Control Register" p 1167 */
  TEST_ASSERT_EQ(k_ra8_agt_test_tstart_bit, reg->AGTCR);
  TEST_END("agt mstp acquire read-back timeout");
}
...
static void (*const s_test_roster[])(void) = {
  test_agt_cascade_mstp_failures,
  test_agt_mstp_acquire_readback_timeout,
