...
/**
 * @test test_eth_eswm_mstp_failures
 * @brief Each sequential ESWM module-stop error is propagated and retryable.
 * @par MC/DC:
 * The two single-condition decisions after ETHPHYCLK and ESWM releases use
 * a success control plus fail-nth vectors 0 and 1 on MSTPCRC.
 * @pre Module-stop state has no references.
 * @pre Fake MMIO can count sequential waits on MSTPCRC.
 * @post Each failure is returned unchanged.
 * @post A disarmed retry completes and publishes a nonzero clock.
 * @note Single-threaded host fixture.
 * @since 0.1.0
 */
static void test_eth_eswm_mstp_failures(void)
{
  TEST_BEGIN("ethernet ESWM bring-up propagates both MSTP failures");
  uint32_t hz = 0U;
  reset_state();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_init());
  /* HUM Ch 11.2.8 "MSTPCRC : Module Stop Control Register C", p 447 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_nth_wait(&ra8_mstp()->MSTPCRC, 0U));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, priv_ra8_board_eth_eswm_bring_up(&hz));

  reset_state();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_init());
  /* HUM Ch 11.2.8 "MSTPCRC : Module Stop Control Register C" p 446 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_nth_wait(&ra8_mstp()->MSTPCRC, 1U));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, priv_ra8_board_eth_eswm_bring_up(&hz));

  reset_state();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_init());
  TEST_ASSERT_EQ(k_ra8_ok, priv_ra8_board_eth_eswm_bring_up(&hz));
  TEST_ASSERT(hz > 0U);
  TEST_END("ethernet ESWM bring-up propagates both MSTP failures");
}

/**
 * @test test_eth_etha_init_and_mode_failures
 * @brief ETHA initialization and the first mode wait propagate independently.
 * @par MC/DC:
 * One vector fails the MSTPCRC release in `ra8_etha_init`; the next lets init
 * succeed and fails the first EAMS wait. A disarmed call is the control.
 * @pre Module-stop and fake-MMIO state are reset before every vector.
 * @pre ETHA1 is backed by fake MMIO.
 * @post Each error is returned from the exact board boundary.
 * @post The control reaches CONFIG.
 * @note Single-threaded host fixture.
 * @since 0.1.0
 */
static void test_eth_etha_init_and_mode_failures(void)
{
  TEST_BEGIN("ethernet ETHA init and mode failures propagate");
  reset_state();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_init());
  /* HUM Ch 11.2.8 "MSTPCRC : Module Stop Control Register C", p 447 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&ra8_mstp()->MSTPCRC));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, priv_ra8_board_eth_etha_to_config());

  reset_state();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_init());
  /* HUM Ch 32.3.1.2 "EAMS : Mode Status Register" p 1631 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&ra8_etha(1)->EAMS));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, priv_ra8_board_eth_etha_to_config());

  reset_state();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_init());
  TEST_ASSERT_EQ(k_ra8_ok, priv_ra8_board_eth_etha_to_config());
  TEST_END("ethernet ETHA init and mode failures propagate");
}
...
int main(void)
{
  ...
  test_eth_eswm_mstp_failures();
  test_eth_etha_init_and_mode_failures();
  return 0;
}
