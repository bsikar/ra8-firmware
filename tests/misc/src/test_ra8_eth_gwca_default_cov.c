...
/**
 * @test test_default_open_rx_ring_input_failures
 * @brief Public RX depth and pool fields drive both ring-propagation guards.
 * @par MC/DC:
 * The init-ring and attach-buffer guards each have one condition. A depth of
 * one drives the first true leg; a null pool with a valid ring drives the
 * second. ::test_default_open_happy supplies both false legs.
 * @pre Caller-owned ring storage is aligned and otherwise valid.
 * @post Each malformed state is rejected before hardware bring-up.
 * @note Single-threaded host fixture.
 * @since 0.1.0
 */
static void test_default_open_rx_ring_input_failures(void)
{
  TEST_BEGIN("default_open propagates RX ring input failures");
  ...
  make_valid_state(&st, s_linkfix, s_rx_chain, s_rx_pool, s_tx_chain, s_tx_pool);
  st.rx_pool = nullptr;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_eth_gwca_default_open(&st));
  TEST_ASSERT_EQ(k_ra8_eth_gwca_step_fail_2, g_ra8_eth_gwca_pre_step);
  TEST_ASSERT_EQ(k_ra8_eth_gwca_step_fail_1, g_ra8_eth_gwca_open_step);
  TEST_END("default_open propagates RX ring input failures");
}
...
int main(void)
{
  ...
  test_default_open_rx_ring_input_failures();
