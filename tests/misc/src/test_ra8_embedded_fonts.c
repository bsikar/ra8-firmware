 ...
 *   1. enumerate (`epub_get_embedded_font_count`) + extract
 *      (`epub_get_embedded_font`) the font and CRC-check the bytes;
 *   2. bind the extracted bytes into a reflow engine (`reflow_bind_font`),
...
/**
 * @test epub enumerates the manifest font item and extracts its bytes.
 *
 * @par MC/DC:
 ...
 * stbtt_InitFont(...) == 0` is exercised in reflow_bind_font by
...
static void test_enumerate_and_extract(void)
{
  ...
  epub_book_t            book  = {};
  TEST_ASSERT_EQ(k_ra8_ok, epub_open(&media, "fonts.epub", &book));
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    epub_get_embedded_font(&book, 0U, s_extracted_font, sizeof(s_extracted_font), &got));
  ...
  TEST_ASSERT_EQ(
    ...
    epub_get_embedded_font(&book, 1U, s_extracted_font, sizeof(s_extracted_font), &over));
...
/**
 * @test reflow_bind_font binds the embedded face; a laid-out glyph is
 ...
 *   (2 conditions, apps/shared_libs/reflow/src/reflow_layout_driver.c@reflow_bind_font)
...
static void test_bind_measure_and_mcdc(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_init((uint16_t)k_tef_viewport_w,
  ...
  TEST_ASSERT_EQ(k_ra8_ok, reflow_bind_font(&s_engine, s_extracted_font, got));
  ...
  TEST_ASSERT_EQ(k_ra8_err_not_supported,
                 reflow_bind_font(&s_engine, s_garbage, sizeof(s_garbage)));
  TEST_ASSERT(s_engine.font_data == s_extracted_font); /* unchanged */
  TEST_ASSERT_EQ(k_ra8_err_not_supported,
                 reflow_bind_font(&s_engine, s_tagged_corrupt, sizeof(s_tagged_corrupt)));
  TEST_ASSERT(s_engine.font_data == s_extracted_font); /* unchanged */

  /* Single-condition guards. */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, reflow_bind_font(nullptr, s_extracted_font, got));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, reflow_bind_font(&s_engine, nullptr, got));
  TEST_ASSERT_EQ(
    k_ra8_err_invalid_size,
    reflow_bind_font(&s_engine, s_extracted_font, (size_t)k_reflow_min_font_bytes - 1U));
