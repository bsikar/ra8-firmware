...
/**
 * @test test_verify_propagates_compute_rejections
 * @brief Verify preserves every public input error from the compute path.
 * @par MC/DC:
 * The `err != k_ra8_ok` return guard has one condition. The three invalid
 * vectors take it after the address-null, address-alignment, and length-range
 * guards respectively; `test_verify_match_then_mismatch` supplies its false
 * control with otherwise identical verification flow.
 * @pre Fake mapped SRAM is available, although rejected vectors never read it.
 * @post Each invalid input returns its exact compute error before comparison.
 * @note Single-threaded host fixture.
 * @since 0.1.0
 */
static void test_verify_propagates_compute_rejections(void)
{
  TEST_BEGIN("cnecc verify propagates null, alignment, and length errors");
  prep();
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_cnecc_verify(0U, (uint32_t)k_ra8_cnecc_cov_ram_len, 0U));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_cnecc_verify((uint32_t)k_ra8_cnecc_cov_ram_base + 1U,
                                  (uint32_t)k_ra8_cnecc_cov_ram_len,
                                  0U));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_cnecc_verify((uint32_t)k_ra8_cnecc_cov_ram_base, 2U, 0U));
  TEST_END("cnecc verify propagates null, alignment, and length errors");
}
...
int main(void)
{
  ...
  test_verify_propagates_compute_rejections();
