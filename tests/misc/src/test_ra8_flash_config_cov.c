 ...
 * These are input invariants rather than missing hardware models, and the
 * module clears the coverage threshold without excluding either source line.
 * The PE-mode enter/exit/suspend/resume transitions and the MSUINITR-kick
 * timeout are driven separately in ``test_ra8_flash.c`` through the
 * ``ra8_fake_mmio`` fault seam.
 *
