...
/**
 * @test test_init_and_deinit_propagate_mstp_timeout
 * @brief Verify NPU lifecycle calls preserve module-stop failures.
 * @details Arms the host fake-MMIO seam on MSTPCRA, which owns the NPU
 * module-stop bit, so both acquisition and release exhaust the real bounded
 * read-back loop. The release failure must leave the NPU initialized and its
 * module reference owned until an unarmed retry succeeds.
 * @pre Host MMIO is reset and the module-stop controller is initialized.
 * @post A failed init leaves the NPU uninitialized.
 * @post A failed deinit leaves the NPU initialized; a later retry releases it.
 * @note File-local host test; no NPU or module-stop hardware is accessed.
 * @since Version 0.1.0
 * @par MC/DC:
 * (no compound decisions in this test -- each injected read-back timeout
 * exercises a bounded single-condition poll in the module-stop controller)
 */
static void test_init_and_deinit_propagate_mstp_timeout(void)
{
  TEST_BEGIN("npu init/deinit propagate MSTP read-back timeout");
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&ra8_mstp()->MSTPCRA));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_npu_init());
  uint32_t id = k_t_id_unset;
  TEST_ASSERT_EQ(k_ra8_err_not_initialized, ra8_npu_read_id(&id));
  ...
  /* HUM Ch 11.2.6 "MSTPCRA : Module Stop Control Register A" p 443 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&ra8_mstp()->MSTPCRA));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_npu_deinit());
  TEST_ASSERT_EQ(k_ra8_ok, ra8_npu_read_id(&id));

  ra8_fake_mmio_reset();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_npu_deinit());
  TEST_END("npu init/deinit propagate MSTP read-back timeout");
}
...
/**
 * @par MC/DC:
 * (no compound decisions in this test -- STATUS.reset remains set for the
 * bounded reset poll; the ordinary reset case covers its clear direction)
 */
static void test_reset_stuck_status_propagates_timeout(void)
{
  TEST_BEGIN("npu reset propagates a stuck STATUS.reset timeout");
  npu_prep();
  *ra8_npu_reg(k_ra8_npu_off_status) = (uint32_t)(1U << k_ra8_npu_status_reset_bit);

  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_npu_reset());
  TEST_END("npu reset propagates a stuck STATUS.reset timeout");
}
...
int main(void)
{
  ...
  test_init_and_deinit_propagate_mstp_timeout();
  ...
  test_reset_stuck_status_propagates_timeout();
  test_irq_arm_gates();
