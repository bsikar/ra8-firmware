...
/**
 * @brief Verify ra8_flash_write_block reports a controller-busy timeout.
 *
 * @details
 * Holds MRCPS.PRGBSYC high so the initial buffer-ready wait cannot complete.
 * This exercises the public write-block error return without altering the
 * production polling path.
 *
 * @pre ra8_flash_init() succeeded.
 * @pre MRCPS.PRGBSYC = 1.
 * @post ra8_flash_write_block returns k_ra8_err_hw_timeout.
 *
 * @par MC/DC:
 * No compound decision is exercised in
 * ``libs/ra8_hal/src/ra8_flash.c@ra8_flash_write_block``. Valid arguments
 * pass validation and the injected initial wait failure drives the first
 * single-error propagation guard.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
static void test_write_block_busy_wait_timeout(void)
{
  TEST_BEGIN("flash write_block: busy wait timeout propagates");
  ra8_fake_mmap_reset();
  (void)ra8_flash_deinit();
  const ra8_flash_cfg_t cfg = cfg_make();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_flash_init(&cfg));
  ...
  *ra8_mram_reg8((uint16_t)k_ra8_mram_off_mrcps) = (uint8_t)k_ra8_mrcps_mask_prgbsyc;
  ...
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout,
                 ra8_flash_write_block(k_ra8_flash_code_start,
                                       buf,
                                       k_ra8_mram_write_size_bytes,
                                       k_ra8_flash_world_s));
  TEST_END("flash write_block: busy wait timeout propagates");
}

/**
 * @brief Verify ra8_flash_write_block reports a commit timeout.
 *
 * @details
 * Leaves MRCPS clear. The initial buffer-ready wait succeeds because the
 * controller is idle, while the commit wait times out because ABUFEMP never
 * asserts. This drives the error return after the program window has restored
 * its gates.
 *
 * @pre ra8_flash_init() succeeded.
 * @pre MRCPS = 0.
 ...
 * @par MC/DC:
 * No compound decision is exercised in
 * ``libs/ra8_hal/src/ra8_flash.c@ra8_flash_write_block``. The ready vector
 * passes the initial single-error guard, then the missing ABUFEMP indication
 * drives the commit error guard.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
static void test_write_block_commit_wait_timeout(void)
{
  TEST_BEGIN("flash write_block: commit wait timeout propagates");
  ra8_fake_mmap_reset();
  (void)ra8_flash_deinit();
  const ra8_flash_cfg_t cfg = cfg_make();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_flash_init(&cfg));
  ...
  *ra8_mram_reg8((uint16_t)k_ra8_mram_off_mrcps) = 0U;
  ...
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout,
                 ra8_flash_write_block(k_ra8_flash_code_start,
                                       ...
                                       k_ra8_mram_write_size_bytes,
                                       k_ra8_flash_world_s));
  TEST_END("flash write_block: commit wait timeout propagates");
}
...
int main(void)
{
  ...
  test_write_block_busy_wait_timeout();
  test_write_block_commit_wait_timeout();
