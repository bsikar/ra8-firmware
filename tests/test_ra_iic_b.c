...
static void test_init_configured(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_b_init(0U, &k_iic_b_cfg));
...
static void test_init_bad_inputs(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_iic_b_init(0U, nullptr));
  ra_iic_b_cfg_t bad = k_iic_b_cfg;
  bad.bus_hz         = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_iic_b_init(0U, &bad));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_iic_b_init((uint8_t)k_ra_iic_b_test_ch_oor, &k_iic_b_cfg));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_iic_b_init((uint8_t)k_ra_iic_b_test_ch_huge, &k_iic_b_cfg));
...
static void test_deinit_releases(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_b_init(0U, &k_iic_b_cfg));
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_b_deinit(0U));
  bool stopped = false;
  TEST_ASSERT_EQ(k_ra_ok, ra_mstp_is_stopped(k_ra_mstp_i3c, &stopped));
  TEST_ASSERT(stopped);
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_iic_b_deinit((uint8_t)k_ra_iic_b_test_ch_oor));
...
static void test_set_clock_updates(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_iic_b_set_clock((uint8_t)k_ra_iic_b_test_ch_oor, 100000U, 60000000U));
...
static void test_write_happy(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_iic_b_write(0U, (uint8_t)k_ra_iic_b_test_target, s_payload, 2U, false));
   ...
   * transaction). */
  volatile const r_iic_b_regs_t* reg = ra_iic_b(0U);
  TEST_ASSERT_EQ(k_ra_iic_b_test_byte_b, (reg->NTDTBP0 & 0xFFU));
  TEST_ASSERT_EQ(k_ra_iic_b_msk_cndctl_spcnd, reg->CNDCTL);
...
static void test_write_zero_length(void)
{
  ...
  prime_ntst(0U);
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_iic_b_write(0U, (uint8_t)k_ra_iic_b_test_target, s_payload, 0U, false));
...
static void test_write_null_data(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_iic_b_write(0U, (uint8_t)k_ra_iic_b_test_target, nullptr, 1U, false));
...
static void test_write_bad_channel(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_iic_b_write((uint8_t)k_ra_iic_b_test_ch_oor,
...
static void test_write_timeout_on_start(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_timeout,
                 ra_iic_b_write(0U, (uint8_t)k_ra_iic_b_test_target, s_payload, 1U, false));
...
static void test_write_restart_holds_bus(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_b_init(0U, &k_iic_b_cfg));
  prime_ntst(0U);
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_b_write(0U, (uint8_t)k_ra_iic_b_test_target, s_payload, 1U, true));
  /* CNDCTL last write should be START (no subsequent STOP). */
  TEST_ASSERT_EQ(k_ra_iic_b_msk_cndctl_stcnd, ra_iic_b(0U)->CNDCTL);
...
static void test_read_happy(void)
{
  ...
  prime_ntst(0U);
  uint8_t buf[2] = {0U, 0U};
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_b_read(0U, (uint8_t)k_ra_iic_b_test_target, buf, 2U, false));
...
static void test_read_zero_length_rejected(void)
{
  ...
  uint8_t buf = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_iic_b_read(0U, (uint8_t)k_ra_iic_b_test_target, &buf, 0U, false));
...
static void test_read_null_out(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_b_init(0U, &k_iic_b_cfg));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_iic_b_read(0U, (uint8_t)k_ra_iic_b_test_target, nullptr, 1U, false));
...
static void test_read_bad_channel(void)
{
  ...
  uint8_t buf = 0U;
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_iic_b_read((uint8_t)k_ra_iic_b_test_ch_oor,
                               (uint8_t)k_ra_iic_b_test_target,
                               &buf,
                               1U,
                               false));
...
static void test_read_timeout_on_start(void)
{
  ...
  uint8_t buf        = 0U;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout,
                 ra_iic_b_read(0U, (uint8_t)k_ra_iic_b_test_target, &buf, 1U, false));
...
static void test_transfer_happy(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_b_transfer(0U, (uint8_t)k_ra_iic_b_test_target, tx, 1U, &rx, 1U));
...
static void test_transfer_null_args_rejected(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_iic_b_transfer(0U, (uint8_t)k_ra_iic_b_test_target, nullptr, 0U, nullptr, 0U));
  /* tx_len > 0 with NULL tx -> null_ptr. */
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_iic_b_transfer(0U, (uint8_t)k_ra_iic_b_test_target, nullptr, 1U, &rx, 1U));
  /* rx_len > 0 with NULL rx -> null_ptr. */
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_iic_b_transfer(0U, (uint8_t)k_ra_iic_b_test_target, s_payload, 1U, nullptr, 1U));
  /* Out-of-range channel -> null_ptr (matches the rest of the API). */
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_iic_b_transfer((uint8_t)k_ra_iic_b_test_ch_oor,
                                   (uint8_t)k_ra_iic_b_test_target,
                                   s_payload,
                                   1U,
                                   &rx,
...
static void test_transfer_busy_rejection(void)
{
  ...
  uint8_t rx         = 0U;
  TEST_ASSERT_EQ(k_ra_err_busy,
                 ra_iic_b_transfer(0U, (uint8_t)k_ra_iic_b_test_target, s_payload, 1U, &rx, 1U));
...
static void test_abort_resets_channel(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_b_abort(0U));
  TEST_ASSERT_EQ(0, ra_iic_b(0U)->BIE);
  TEST_ASSERT_EQ(0, ra_iic_b(0U)->NTIE);
  TEST_ASSERT_EQ(k_ra_iic_b_msk_cndctl_spcnd, ra_iic_b(0U)->CNDCTL);
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_iic_b_abort((uint8_t)k_ra_iic_b_test_ch_oor));
...
static void test_scan_no_response(void)
{
   ...
   * empty bus. */
  bool acked = true;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_iic_b_scan(0U, (uint8_t)k_ra_iic_b_test_target, &acked));
  TEST_ASSERT(!acked);
...
static void test_scan_bad_args(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_b_init(0U, &k_iic_b_cfg));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_iic_b_scan(0U, (uint8_t)k_ra_iic_b_test_target, nullptr));
  bool acked = false;
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_iic_b_scan((uint8_t)k_ra_iic_b_test_ch_oor, (uint8_t)k_ra_iic_b_test_target, &acked));
...
static void test_errors_mask_and_clear(void)
{
  ...
  uint8_t mask = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_b_get_errors(0U, &mask));
  TEST_ASSERT_EQ(((uint8_t)k_ra_iic_b_err_arb_lost | (uint8_t)k_ra_iic_b_err_nack |
                  (uint8_t)k_ra_iic_b_err_timeout),
                 mask);

  TEST_ASSERT_EQ(k_ra_ok, ra_iic_b_clear_errors(0U));
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_b_get_errors(0U, &mask));
  TEST_ASSERT_EQ(k_ra_iic_b_err_none, mask);

  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_iic_b_get_errors(0U, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_iic_b_get_errors((uint8_t)k_ra_iic_b_test_ch_oor, &mask));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_iic_b_clear_errors((uint8_t)k_ra_iic_b_test_ch_oor));
...
static void test_attach_handler_toggles_iers(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_b_attach_handler(0U, stub_iic_b_cb, nullptr));
  TEST_ASSERT(ra_iic_b(0U)->BIE != 0U);
  TEST_ASSERT(ra_iic_b(0U)->NTIE != 0U);

  TEST_ASSERT_EQ(k_ra_ok, ra_iic_b_attach_handler(0U, nullptr, nullptr));
  TEST_ASSERT_EQ(0, ra_iic_b(0U)->BIE);
  TEST_ASSERT_EQ(0, ra_iic_b(0U)->NTIE);

  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_iic_b_attach_handler((uint8_t)k_ra_iic_b_test_ch_oor, stub_iic_b_cb, nullptr));
...
static void test_dispatch_eri_fires_callback(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_iic_b_err_nack, s_iic_b_cb_err);
...
static void test_legacy_pass_throughs(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_init(0U, &k_iic_b_cfg));
  prime_ntst(0U);
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_write(0U, (uint8_t)k_ra_iic_b_test_target, s_payload, 2U));
  prime_ntst(0U);
  uint8_t buf = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_iic_read(0U, (uint8_t)k_ra_iic_b_test_target, &buf, 1U));
...
static void test_mcdc_iic_b(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_iic_b_transfer(0U, (uint8_t)k_ra_iic_b_test_target, tx_buf, 0U, rx_buf, 0U));

  /* Decision B V3: tx_len!=0 with NULL tx -> null_ptr. */
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_iic_b_transfer(0U, (uint8_t)k_ra_iic_b_test_target, nullptr, 1U, rx_buf, 0U));

  /* Decision C V3: rx_len!=0 with NULL rx -> null_ptr. tx_len=0 to
   * keep decision A=F (because rx_len!=0) and decision B=F (tx_len=0). */
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_iic_b_transfer(0U, (uint8_t)k_ra_iic_b_test_target, tx_buf, 0U, nullptr, 1U));
...
static void test_mcdc_iic_b_internal_len_buf_invalid(void)
{
  ...
  uint8_t scratch = 0U;
  TEST_ASSERT(!ra_iic_b_internal_len_buf_invalid(0U, nullptr));
  TEST_ASSERT(ra_iic_b_internal_len_buf_invalid(4U, nullptr));
...
static void test_mcdc_iic_b_internal_should_dispatch(void)
{
  ...
  TEST_ASSERT(!ra_iic_b_internal_should_dispatch(0x10U, nullptr));
