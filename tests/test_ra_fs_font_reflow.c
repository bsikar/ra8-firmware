/**
 * @file test_ra_fs_font_reflow.c
 * @brief End-to-end: font stored on a FAT volume -> read via ra_fs ->
 *        rendered through the heap-free ra_reflow pipeline.
 *
 * @details
 ...
 * (`ra_stbtt_alloc`), so the whole read-and-render path is heap-free
...
/* ---------------------------------------------------------------------------
 * In-memory FAT16 block device (mirrors tests/test_ra_fs_fat.c).
 * ---------------------------------------------------------------------------
 */
...
static bool load_src_font(void)
{
  ...
  /* strip "test_ra_fs_font_reflow.c" and "tests/" -> repo root. */
 ...
 * already-MC/DC-covered ra_fs / ra_reflow APIs through one round trip:
...
{
  TEST_BEGIN("font on FAT volume -> ra_fs -> ra_reflow render");
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &mnt));

  /* 2. Write the font onto the card through the real FAT write path. */
  ra_fs_file_t* wf = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(mnt, "FONT.OTF", k_ra_fs_mode_write, &wf));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_write(wf, s_src_font, s_font_len));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(wf));

  /* 3. Read it back off the card into the font buffer ra_reflow will use. */
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_fs_font_reflow.c\n");
  return 0;
}
