...
static void test_gpt_init_forwards(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_nsc_gpt_init(0U, nullptr));
  ra_gpt_cfg_t cfg = {};
  ...
  cfg.duty_b       = 0U;
  cfg.auto_start   = true;
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_gpt_init(0U, &cfg));
  uint32_t cnt = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_gpt_read(0U, &cnt));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_nsc_gpt_read(0U, nullptr));
...
static void test_adc_dac_acmphs_init_forwards(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_adc_init());
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_dac_b_init());
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_acmphs_init());
  uint16_t raw = 0U;
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_nsc_adc_read_channel(0U, nullptr));
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_dac_b_write(0U, 0x800U));
  ra_level_t lvl = k_ra_level_low;
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_acmphs_read_output(0U, &lvl));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_nsc_acmphs_read_output(0U, nullptr));
...
static void test_crc_init_compute(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_crc_init(k_ra_crc_poly_16_ccitt));
  uint32_t      out  = 0U;
  const uint8_t data = 0xAAU;
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_crc_compute(&data, 1U, &out));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_nsc_crc_compute(nullptr, 1U, &out));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_nsc_crc_compute(&data, 1U, nullptr));
...
static void test_glcdc_pdm_eth_init(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_glcdc_init(&glcfg));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_nsc_glcdc_init(nullptr));
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_pdm_init());
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_eth_init());
