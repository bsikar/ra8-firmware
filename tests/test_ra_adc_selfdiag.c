...
static ra_tsn_config_t make_tsn_cfg(void)
{
...
/* ---------------------------------------------------------------------------
 * ra_adc_self_diagnose: validation + happy / fail paths
 * --------------------------------------------------------------------------- */
...
static void test_self_diagnose_rejects(void)
{
  ...
  uint16_t code = 0U;
  bool     pass = true;
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_adc_self_diagnose(k_ra_adc_selfdiag_mode_1, nullptr, &pass));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_adc_self_diagnose(k_ra_adc_selfdiag_mode_1, &code, nullptr));
  /* Mode 0 (and any value outside 1..3) must be rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_adc_self_diagnose((ra_adc_selfdiag_mode_t)0U, &code, &pass));
...
static void test_self_diagnose_mode1_pass(void)
{
  ...
  *ra_adc_b_adexdr(k_test_adexdr_selfdiag0) = k_test_exd_mode1_ideal;
  ...
  const uint32_t diagval =
    *ra_adc_b_adsgdcr(k_ra_adc_b_scan_groups - 1U) & k_ra_adsgdcr_mask_diagval;
...
static void test_read_internal_null(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_adc_read_internal_channel(k_ra_adc_chan_temperature, nullptr));
...
/* ---------------------------------------------------------------------------
 * ra_tsn_read_die_temp_milli_c: close-the-loop
 * --------------------------------------------------------------------------- */
...
static void test_die_temp_closed_loop(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_adc_init());
  const ra_tsn_config_t cfg = make_tsn_cfg();
 ...
 * before ra_tsn_init and rejects a null output)
 */
static void test_die_temp_guards(void)
{
  ...
  ra_sim_mmap_reset();
  (void)ra_mstp_init();
  (void)ra_tsn_deinit(); /* Ensure the init flag is clear. */

  int32_t milli = 0;
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_tsn_read_die_temp_milli_c(nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_tsn_read_die_temp_milli_c(&milli));
 ...
 * libs/ra_hal/src/adc.c@internal_selfdiag_in_band,
 * libs/ra_hal/src/adc.c@ra_adc_self_diagnose,
 * libs/ra_hal/src/adc.c@internal_is_supported_ext_chan.
 *
 * Decision A: ``internal_selfdiag_in_band`` (adc.c):
 * ``return (diff <= +tol) && (diff >= -tol);`` driven through
 * ``ra_adc_self_diagnose`` mode 1 (expected 0) by the injected ADEXDR0
 ...
 * ``ra_adc_read_internal_channel``:
 * - V1: chan=temperature  -> C1=T          -> supported -> k_ra_ok
 * - V2: chan=int_ref_volt -> C1=F, C2=T    -> supported -> k_ra_ok
...
static void test_mcdc_adc_selfdiag(void)
{
  ...
  *ra_adc_b_adexdr(k_test_adexdr_vref) = (uint32_t)k_test_vref_raw;
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_adc_read_internal_channel((ra_adc_internal_chan_t)0x66U, &raw));
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_adc_selfdiag.c\n");
  return 0;
}
