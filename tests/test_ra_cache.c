/**
 * @file test_ra_cache.c
 * @brief Unit tests for ra_cache.c (Cortex-M85 L1 D-cache maintenance).
 *
 * @details
...
} ra_cache_test_const_t;

/** @brief Typed access to a seeded/observed SCB register in the sim map. */
static volatile uint32_t* reg(ra_cache_test_reg_t addr)
{
  return (volatile uint32_t*)addr;
}

/** @brief Seed CTR so ra_cache reports a @p dmin-derived line size. */
...
{
  TEST_BEGIN("ra_cache: clean of one aligned line hits that line");
  ra_sim_mmap_reset();
  seed_line((uint32_t)k_t_dmin_for_32);
...
{
  TEST_BEGIN("ra_cache: clean spanning a line boundary walks both lines");
  ra_sim_mmap_reset();
  seed_line((uint32_t)k_t_dmin_for_32);
...
{
  TEST_BEGIN("ra_cache: invalidate of an unaligned range covers the head line");
  ra_sim_mmap_reset();
  seed_line((uint32_t)k_t_dmin_for_32);
...
{
  TEST_BEGIN("ra_cache: clean+invalidate writes DCCIMVAC");
  ra_sim_mmap_reset();
  seed_line((uint32_t)k_t_dmin_for_32);
...
/**
 * @par MC/DC: not applicable -- the NULL guard is a single RA_CHECK_NULL_PTR
 *      condition; this case proves the rejection branch.
 */
static void test_null_addr_rejected(void)
{
  TEST_BEGIN("ra_cache: NULL address is rejected");
  ra_sim_mmap_reset();
  seed_line((uint32_t)k_t_dmin_for_32);
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_cache_dcache_clean_by_addr(nullptr, (uint32_t)k_t_line_32));
  TEST_END("ra_cache: NULL address is rejected");
}
...
{
  TEST_BEGIN("ra_cache: zero size is a no-op success");
  ra_sim_mmap_reset();
  seed_line((uint32_t)k_t_dmin_for_32);
  *reg(k_t_dccmvac) = (uint32_t)k_t_sentinel;
...
{
  TEST_BEGIN("ra_cache: invalidate_all guards a degenerate CCSIDR (MC/DC)");
