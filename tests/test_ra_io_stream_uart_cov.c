/**
 * @file test_ra_io_stream_uart_cov.c
 * @brief Coverage-boost tests for ra_io_stream_uart.c.
 *
 * @details
 * Targets the four source lines left uncovered after test_ra_io_stream.c:
 *
 ...
 *   67  -- uart_write success path: `return k_ra_ok` statement.
 *
 * The four lines are reached only when `ra_sci_write_polling` returns
 * `k_ra_ok`.  Using `len = 0` achieves this deterministically in
 * RA_SIMULATOR_MODE: the byte-loop is skipped and `internal_wait_tx_end`
 ...
 * empty poll resolves immediately, as in test_ra_uart.c.
 *
 * @par MC/DC:
 ...
 *
 * @details
 * Mirrors the technique used in test_ra_uart.c: the simulator peripheral
 * window is zeroed first, then the TDRE bit in CSR is set so that the
 * ra_sci_putc_polling spin resolves on the first iteration without
 * busy-waiting.
 *
 * @pre RA_SIMULATOR_MODE is defined and ra_sim_mmap_install has run.
 * @post CSR[k_ra_sci_csr_bit_tdre] is 1 for SCI channel 0.
...
{
  ra_sim_mmap_reset();
  volatile r_sci_regs_t* reg = ra_sci((uint8_t)k_uart_cov_channel);
  reg->CSR                   = (uint32_t)(1U << (uint8_t)k_ra_sci_csr_bit_tdre);
}
 ...
 * ra_io_stream_write dispatcher.  len = 0 causes ra_sci_write_polling to
 * return k_ra_ok immediately (the byte loop and TEND wait are both skipped),
 ...
 *
 * @pre ra_sim_mmap is installed (constructor, runs before main).
...
static void test_write_success_with_out_written(void)
{
  ...
  ra_io_stream_t            s     = {};
  ra_io_stream_uart_state_t state = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_uart_init(&s, &state, (uint8_t)k_uart_cov_channel));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_write(&s, buf, 0U, &written));
...
static void test_write_success_null_out_written(void)
{
  ...
  const uint8_t buf[1] = {(uint8_t)k_uart_cov_test_byte};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_write(&s, buf, 0U, nullptr));
 ...
 * byte loop executes: CSR.TDRE is pre-seeded so ra_sci_putc_polling
 * resolves on the first poll, and ra_sci_write_polling returns k_ra_ok.
...
static void test_write_one_byte_tdre_seeded(void)
{
  ...
  const uint8_t buf[1]  = {(uint8_t)k_uart_cov_test_byte};
  uint32_t      written = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_write(&s, buf, 1U, &written));
  /* ra_sci_write_polling consumed one byte; out_written == len == 1. */
 ...
 *
 * @details
 * Drives uart_flush through the ra_io_stream_flush dispatcher.  In
 * RA_SIMULATOR_MODE internal_wait_tx_end returns k_ra_ok immediately, so
 ...
 *
 * @pre ra_sim_mmap is installed.
 * @pre SCI channel 0 is valid.
 * @post Flush returns k_ra_ok.
 ...
 *
 * @pre All MMIO windows are mapped (ra_sim_mmap_install constructor has run).
 * @pre The test binary is compiled with RA_SIMULATOR_MODE defined.
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_io_stream_uart_cov.c\n");
  return 0;
}
