/**
 * @file test_ra_ethosu_shim.c
 * @brief Unit tests for ra_ethosu_shim.c (Arm ethos-u-core-driver -> ra_npu adapter)
 *
 * @details
 * Compiled with `-DRA_DEVICE_RA8P1` (see tests/CMakeLists.txt) so the
 * device-gated `ra_ethosu_shim.c` and `ra_npu.c` bodies are live and
 * `ra_npu_regs.h` is includable. The NPU register window (`0x40140000`) sits
 ...
 *
 * ethosu_invoke_v3() blocks in ra_npu_wait() until STATUS.cmd_end latches;
...
static struct ethosu_driver* shim_prep(void)
{
  ...
  /* No real NPU reaches cmd_end here: latch it so the blocking ra_npu_wait()
...
static void shim_assert_reg64(uint32_t lo_off, uint64_t expect)
{
  ...
  TEST_ASSERT_EQ((expect & (uint64_t)k_test_shim_addr_lo_mask), *ra_npu_reg(lo_off));
  TEST_ASSERT_EQ((expect >> k_test_shim_addr_hi_shift), *ra_npu_reg(hi_off));
}
 ...
 */
static void shim_assert_region(ra_npu_region_idx_t idx, uint64_t base)
{
 ...
 * single condition, and the ra_npu submit/run happy paths plus poll's
...
static void test_reserve_and_invoke_programs_registers(void)
{
  ...
  base_addrs[k_ra_npu_region_0] = (uint64_t)(uintptr_t)s_shim_weights;
  base_addrs[k_ra_npu_region_1] = (uint64_t)(uintptr_t)s_shim_input;
  base_addrs[k_ra_npu_region_2] = (uint64_t)(uintptr_t)s_shim_output;
  sizes[k_ra_npu_region_0]      = sizeof(s_shim_weights);
  sizes[k_ra_npu_region_1]      = sizeof(s_shim_input);
  sizes[k_ra_npu_region_2]      = sizeof(s_shim_output);
  ...
  shim_assert_region(k_ra_npu_region_2, base_addrs[k_ra_npu_region_2]);

  /* ra_npu_run() kicked the job: CMD.transition_to_running_state == bit 0. */
 ...
 * overflow guard is the single condition `num_base_addr > k_ra_npu_region_count`)
 */
static void test_invoke_rejects_bad_args(void)
{
  ...
  TEST_ASSERT(ethosu_invoke_v3(drv,
                               ...
                               nullptr) != 0);
  /* Region count past ra_npu's region_base[] capacity -- overflow rejected. */
  TEST_ASSERT(ethosu_invoke_v3(drv,
                               s_shim_cmd,
                               (int)sizeof(s_shim_cmd),
                               base_addrs,
