...
static void test_mcdc_l2cap_evt_trampoline_lemeta_3cond(void)
{
  ...
  ra8_ble_host_test_inject_event((uint8_t)k_test_evt_unrecognised, s_params_full, 19U);
  ...
  before                         = ra8_ble_host_test_event_count();
  ra8_ble_host_test_inject_event((uint8_t)k_test_evt_le_meta, s_other_sub, 19U);
  TEST_ASSERT_EQ(before, ra8_ble_host_test_event_count());

  /* V4: all three conditions true -> a connected event is dispatched. */
  before = ra8_ble_host_test_event_count();
  ra8_ble_host_test_inject_event((uint8_t)k_test_evt_le_meta, s_params_full, 19U);
