/**
 * @file test_ra_wdt_sup_tx_shim_cov.c
 * @brief Coverage-lift tests for libs/ra_wdt_supervisor/src/ra_wdt_sup_tx_shim.h
 *
 * @par Tag
 * [Test / Host] {World: N/A}
 *
 * @details
 * Drives the two inline stubs in ra_wdt_sup_tx_shim.h that the existing
 ...
 *    (tests drive the supervisor synchronously via ra_wdt_supervisor_tick
 ...
 *    internal_default_now helper in ra_wdt_supervisor.c. That helper is
 *    exercised only when no custom now hook is installed; the existing
 *    test_ra_wdt_supervisor_cov.c does exercise it, but the .gcda for
 ...
 *
 * No hardware registers are touched; no ra_sim_mmap window is required.
 *
 ...
 *
 * This call covers source lines 288, 290, and 291 of ra_wdt_sup_tx_shim.h.
 *
 * @par MC/DC:
 ...
 * 1 kHz tick constant (k_ra_wdt_sup_default_tick_ms = 1), so all
 * deadline arithmetic sees t = 0 ms -- meaning registered threads are
 * never considered overdue under the default hook.
 *
 * This call covers source lines 307 and 309 of ra_wdt_sup_tx_shim.h.
 *
 * @par MC/DC:
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_wdt_sup_tx_shim_cov.c\n");
  return 0;
}
