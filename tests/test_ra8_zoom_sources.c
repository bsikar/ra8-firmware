 ...
 *
 * @details
 * The zoom engine sees pixels only through ::ra8_zoom_read_fn, and this file
 * owns the two implementations of that seam plus the ::ra8_tile_rect_t producer
 * they share:
 *
 *   - **tiled** (`ra8_zoom_tiles`): a gray8 image paged through a real
 ...
 *     image size.
 *   - **book** (`ra8_zoom_book`): a `.rabook` image-pool figure, at both
 *     retained depths, checked against ::ra8_book_src_image_rect so the adapter
...
static uint8_t s_cells[(size_t)k_t_cells * (size_t)k_t_tile * (size_t)k_t_tile];
 ...
 */
static void t_tiles_open(ra8_tile_cache_t* cache, ra8_zoom_tile_src_t* ts)
{
 ...
 */
static void t_tile_src_bind_checks(ra8_tile_cache_t* cache, ra8_zoom_tile_src_t* ts)
{
  t_tiles_open(cache, ts);
  ra8_zoom_source_t src = {};
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_zoom_tile_src_bind(nullptr, &src));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_zoom_tile_src_bind(ts, nullptr));
  ra8_zoom_tile_src_t unbound = {};
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_zoom_tile_src_bind(&unbound, &src));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_zoom_tile_src_bind(ts, &src));
 ...
 *
 * @par MC/DC:
 * Decision libs/ra8_zoom/src/ra8_zoom_tiles.c@ra8_zoom_tile_src_init
...
static void t_tile_src_init_validates(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_zoom_tile_src_init(nullptr, &cache, 1U, 8U, 8U, 4U, 4U));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_zoom_tile_src_init(&ts, nullptr, 1U, 8U, 8U, 4U, 4U));
  /* V2 .. V5 */
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_zoom_tile_src_init(&ts,
                                        &cache,
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    ra8_zoom_tile_src_init(&ts, &cache, 1U, (uint32_t)k_t_img_w, (uint32_t)k_t_img_h, 1U, 1U));
  TEST_ASSERT_EQ(
    k_ra8_err_invalid_size,
    ra8_zoom_tile_src_init(&ts, &cache, 1U, (uint32_t)k_t_grid_over, (uint32_t)k_t_img_h, 1U, 1U));
  TEST_ASSERT_EQ(
    k_ra8_err_invalid_size,
    ra8_zoom_tile_src_init(&ts, &cache, 1U, (uint32_t)k_t_img_w, (uint32_t)k_t_grid_over, 1U, 1U));
 ...
 */
static void t_tile_read_edges_and_failclosed(ra8_tile_cache_t* cache, ra8_zoom_tile_src_t* ts)
{
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    ra8_zoom_tile_read(ts, 0U, 0U, (uint32_t)k_t_img_w, 8U, s_out, (uint32_t)k_t_img_w));
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 ra8_zoom_tile_read(ts, 0U, 0U, 8U, 8U, s_out, (uint32_t)k_t_out_w));
  s_bad_width = false;
  t_tiles_open(cache, ts);
  s_bad_height = true;
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
...
static void t_tile_read_assembles_and_bounds_residency(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_zoom_tile_read(nullptr, 0U, 0U, 4U, 4U, s_out, (uint32_t)k_t_out_w));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_zoom_tile_read(&ts, 0U, 0U, 4U, 4U, nullptr, (uint32_t)k_t_out_w));
  ra8_zoom_tile_src_t unbound = {};
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_zoom_tile_read(&unbound, 0U, 0U, 4U, 4U, s_out, (uint32_t)k_t_out_w));
  /* V2 / V3 / V4 */
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_zoom_tile_read(&ts, 0U, 0U, 0U, 20U, s_out, (uint32_t)k_t_out_w));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_zoom_tile_read(&ts, 0U, 0U, 40U, 0U, s_out, (uint32_t)k_t_out_w));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_zoom_tile_read(&ts, 0U, 0U, 40U, 20U, s_out, 8U));
  /* V6 / V7 */
  TEST_ASSERT_EQ(k_ra8_err_out_of_range,
                 ra8_zoom_tile_read(&ts, 90U, 0U, 40U, 20U, s_out, (uint32_t)k_t_out_w));
  TEST_ASSERT_EQ(k_ra8_err_out_of_range,
                 ra8_zoom_tile_read(&ts, 0U, 60U, 40U, 20U, s_out, (uint32_t)k_t_out_w));
...
static void t_tiles_prefetch_warms_lead_edge(void)
{
  ...
  uint16_t warmed = 0U;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_zoom_tiles_prefetch(nullptr, &v, k_ra8_zoom_pan_right, 4U, &warmed));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_zoom_tiles_prefetch(&ts, nullptr, k_ra8_zoom_pan_right, 4U, &warmed));
  ra8_zoom_tile_src_t unbound = {};
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_zoom_tiles_prefetch(&unbound, &v, k_ra8_zoom_pan_right, 4U, &warmed));

  /* No travel warms nothing; a rightward pan warms the column beyond the view. */
  s_decodes = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_zoom_tiles_prefetch(&ts, &v, k_ra8_zoom_pan_none, 4U, &warmed));
  TEST_ASSERT_EQ(0U, warmed);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_zoom_tiles_prefetch(&ts, &v, k_ra8_zoom_pan_right, 4U, &warmed));
...
static void t_book_setup(void)
{
  ...
  s_book.hdr.format_version    = k_ra8_book_format_version;
  ...
  s_book.hdr.cover_image_index = k_ra8_book_nil;

  s_book.chapters[0].root_node = 0U;
  s_book.nodes[0]              = (ra8_book_node_t){.kind         = k_ra8_book_node_text,
                                                   .text_off     = 0U,
                                                   .first_child  = k_ra8_book_nil,
                                                   .next_sibling = k_ra8_book_nil};

  s_book.images[0] = (ra8_book_image_t){.width        = (uint16_t)k_b_g4_w,
  ...
  s_book.images[1] = (ra8_book_image_t){.width        = (uint16_t)k_b_g8_w,
  ...
  s_book.images[2] = (ra8_book_image_t){.width     = 4U,
                                        .height    = 4U,
  ...
  const uint8_t* body = (const uint8_t*)&s_book + sizeof(ra8_book_header_t);
  s_book.hdr.crc32    = t_book_crc32(body, (size_t)(sizeof(s_book) - sizeof(ra8_book_header_t)));
}
 ...
 */
static void t_book_gray8_and_failclosed(const ra8_book_src_t* src)
{
  static uint8_t      s_via_adapter[k_b_out_cap];
  ra8_zoom_book_src_t bs   = {};
  ra8_zoom_source_t   zsrc = {};
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_zoom_book_src_init(&bs, src, 1U));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_zoom_book_src_bind(&bs, &zsrc));
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_zoom_book_src_init(&bs, src, 0U));
 ...
 */
static void
t_book_guards(const ra8_book_src_t* src, ra8_zoom_book_src_t* bs, ra8_zoom_source_t* zsrc)
{
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_zoom_book_src_init(nullptr, src, 0U));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_zoom_book_src_init(bs, nullptr, 0U));
  /* A vector entry is not magnifiable by a raster viewer, and says so. */
  TEST_ASSERT_EQ(k_ra8_err_not_supported, ra8_zoom_book_src_init(bs, src, (uint32_t)k_b_svg_idx));

  /* V1: the gray4 figure binds and reads as gray8. */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_zoom_book_src_init(bs, src, 0U));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_zoom_book_src_bind(nullptr, zsrc));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_zoom_book_src_bind(bs, nullptr));
  ra8_zoom_book_src_t unbound = {};
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_zoom_book_src_bind(&unbound, zsrc));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_zoom_book_src_bind(bs, zsrc));
  TEST_ASSERT_EQ(k_b_g4_w, zsrc->width);

  static uint8_t s_guard_buf[k_b_out_cap];
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_zoom_book_read(nullptr, 0U, 0U, 1U, 1U, s_guard_buf, 1U));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_zoom_book_read(bs, 0U, 0U, 1U, 1U, nullptr, 1U));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_zoom_book_read(&unbound, 0U, 0U, 1U, 1U, s_guard_buf, 1U));
}
 ...
 *
 * @par MC/DC:
 * Decision libs/ra8_zoom/src/ra8_zoom_book.c@ra8_zoom_book_src_init
 ...
 *          zoom engine is byte-identical to ::ra8_book_src_image_rect's own
...
static void t_book_src_binds_both_depths(void)
{
  ...
  t_book_setup();
  ra8_book_src_t src = {};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_book_src_resident(&src, &s_book, (uint32_t)sizeof(s_book)));
