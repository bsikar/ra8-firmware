/**
 * @file test_ra_epub_open_cov.c
 * @brief Coverage-boost tests for libs/ra_epub/src/ra_epub_open.c.
 *
 * @details
 * Drives the previously-uncovered branches in ra_epub_open.c by
 * constructing minimal in-memory ZIP / EPUB archives via the miniz
 * writer API.  Each test targets a specific uncovered line group:
 *
 *   Group A -- priv_extract error returns (line 181, 188):
 *     - Container entry absent from archive   -> line 181 (not_found).
 *     - Container entry uncomp_size > 4096    -> line 188 (no_mem).
 *
 *   Group B -- priv_parse_archive error returns (lines 318, 324, 330, 336):
 *     - Missing container.xml                 -> line 318 (not_found propagated).
 *     - Syntactically invalid container.xml   -> line 324 (validation_failed).
 *     - container.xml points to missing OPF   -> line 330 (not_found for OPF).
 *     - OPF present but unparseable XML       -> line 336 (validation_failed).
 *
 *   Group C -- ra_epub_open cleanup path (lines 390, 391):
 *     Any of the Group-B failures drives lines 390-391 (priv_zip_destroy
 *     and return on failure).
 *
 *   Group D -- priv_dirname no-slash arm (line 144):
 *     OPF stored at archive root ("content.opf", no directory prefix).
 *
 *   Group E -- priv_load_toc branches (lines 261, 271, 272, 274, 280):
 *     - Nav item with empty href              -> line 261 (empty toc_path).
 *     - Nav stored at bare path, full fails   -> lines 271, 272 (fallback).
 *     - Both extraction paths fail            -> line 274 (toc bail-out).
 *     - EPUB2 NCX kind                        -> line 280 (NCX branch).
 *
 * Lines excluded from coverage measurement (GCOVR_EXCL_LINE in source):
 *   136 -- priv_dirname dst=NULL guard: dst is always &out_book->opf_dir
 *           (a struct member), never NULL.
 *   140 -- priv_dirname path=NULL guard: path is always cres.opf_path
 *           (a local char array), never NULL.
 *   148 -- priv_dirname len>=cap truncation body: opf_path is bounded by
 *           k_ra_epub_max_path_len chars so len < cap always holds.
 *   149 -- closing brace of the unreachable truncation arm.
 *   185 -- mz_zip_reader_file_stat failure after a successful locate call
 *           requires a corrupt central-directory entry; no well-formed
 *           in-memory ZIP can produce this.
 *   191 -- mz_zip_reader_extract_to_mem failure after stat succeeds requires
 *           a corrupt compressed stream; no well-formed in-memory ZIP can
 *           produce this.
...
typedef enum : uint32_t {
  ...
  k_cov_path_max = 192U,   /**< Mirror of k_ra_epub_max_path_len.             */
} cov_open_sizes_t;
 ...
 *
 * @details Drives line 280 (the ra_epub_xml_parse_ncx call in priv_load_toc).
 * @since 0.1.0
 */
...
/* ---------------------------------------------------------------------------
 * Group A/C -- priv_extract returns k_ra_err_not_found (lines 181, 318, 390, 391).
 * ---------------------------------------------------------------------------
 */

/**
 * @test test_open_no_container_xml
 * @brief ZIP missing META-INF/container.xml -> k_ra_err_not_found.
 *
 * @details The archive contains only a "mimetype" entry.  priv_extract()
 *          fails to locate container.xml (line 181), priv_parse_archive()
 *          propagates the error (line 318), and ra_epub_open() runs
 *          priv_zip_destroy + return (lines 390-391).
 *
 * @par MC/DC:
 * Decision: `if (err != k_ra_ok)` at line 317 (1 condition).
 * - V1 (this test): err=not_found -> true  -> line 318.
 * - V2 (other tests with valid EPUBs): err=k_ra_ok -> false -> continues.
...
/* ---------------------------------------------------------------------------
 * Group A/C -- priv_extract returns k_ra_err_no_mem (lines 188, 318, 390, 391).
 * ---------------------------------------------------------------------------
 */

/**
 * @test test_open_container_too_large
 * @brief container.xml uncomp_size > 4096 -> k_ra_err_no_mem.
 *
 * @details The container.xml entry is padded with an XML comment to push
 *          its uncompressed size beyond k_ra_epub_container_xml_buf (4096).
 *          priv_extract() returns no_mem (line 188), priv_parse_archive()
 *          propagates it (line 318), and ra_epub_open() cleans up (390-391).
 *
 * @par MC/DC:
...
static void test_open_container_too_large(void)
{
  ...
  static char big_container[k_cov_pad_len + 512U];
  size_t      pos = 0U;
  ...
  memcpy(big_container + pos, k_comment_open, co_len);
  pos += co_len;

  /* Fill padding. */
  for (size_t i = 0U; i < (size_t)k_cov_pad_len; ++i) {
    big_container[pos + i] = 'x';
  }
  pos += (size_t)k_cov_pad_len;

  memcpy(big_container + pos, k_comment_close, cc_len);
  pos += cc_len;

  /* Append the real container body. */
  const size_t body_len = strlen(k_container_good);
  memcpy(big_container + pos, k_container_good, body_len);
  pos += body_len;
  big_container[pos] = '\0';
 ...
 * @brief container.xml is not valid XML -> k_ra_err_validation_failed.
 *
 * @details ra_epub_xml_parse_container() returns validation_failed;
 *          priv_parse_archive propagates it at line 324; ra_epub_open
 *          cleans up at lines 390-391.
 *
 * @par MC/DC:
 * Decision: `if (err != k_ra_ok)` at line 323 (1 condition).
 * - V1 (this test): err=validation_failed -> true  -> line 324.
 * - V2 (valid-container tests): err=k_ra_ok -> false -> continues.
 * Pair (V1,V2) gives complete MC/DC for this 1-condition decision.
 *
 ...
 *          priv_parse_archive propagates it at line 330; ra_epub_open
 *          cleans up at lines 390-391.
 *
 * @par MC/DC:
 * Decision: `if (err != k_ra_ok)` at line 329 (1 condition).
 * - V1 (this test): err=not_found -> true  -> line 330.
 * - V2 (valid-OPF tests): err=k_ra_ok -> false -> continues.
 * Pair (V1,V2) gives complete MC/DC for this 1-condition decision.
 *
 * @pre s_zip_buf writable.
 * @pre None.
 ...
 * @brief OPF entry present but not valid XML -> k_ra_err_validation_failed.
 *
 * @details ra_epub_xml_parse_opf() returns validation_failed; priv_parse_archive
 *          propagates it at line 336; ra_epub_open cleans up at lines 390-391.
 *
 * @par MC/DC:
 * Decision: `if (err != k_ra_ok)` at line 335 (1 condition).
 ...
 *          line 144 is hit; opf_dir is left as "".  ra_epub_open succeeds.
 *
 * @par MC/DC:
 ...
 * @post ra_epub_close returns k_ra_ok.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
 ...
 *
 * @details tinyxml2 returns "" (not NULL) for an empty attribute value.
 *          ra_epub_xml_parse_opf sees nav_href != NULL and calls
 ...
 *          2. err == k_ra_err_not_found -> line 271 hit.
 *          3. extract("does_not_exist.xhtml")       -> not_found.
 *          4. err != k_ra_ok -> line 274 hit (return).
 *          ra_epub_open still returns k_ra_ok (TOC is best-effort).
 *
 * @par MC/DC:
 * Decision: `if (err == k_ra_err_not_found)` at line 269 (1 condition).
 * - V1 (this test): err=not_found -> true  -> line 271 (fallback).
 * - V2 (test_toc_fallback_bare_path): err=k_ra_ok (fallback succeeds) -> line 271 not taken on second path.
 *
 * Decision: `if (err != k_ra_ok)` at line 273 (1 condition).
 * - V1 (this test): err still not_found after fallback -> true  -> line 274.
 * - V2 (test_toc_fallback_bare_path): fallback succeeds -> err=k_ra_ok -> false.
 ...
 *          3. err == k_ra_ok -> line 274 NOT taken.
 *          ra_epub_xml_parse_nav runs, toc_count > 0.
 *
 * @par MC/DC:
 * Decision: `if (err == k_ra_err_not_found)` at line 269 (1 condition).
 * - V1 (test_toc_both_paths_fail): both fail -- provides the true arm.
 * - V2 (this test): first extract not_found -> true -> line 271; but
 *   second extract succeeds -> err=k_ra_ok -> line 274 NOT taken.
 ...
 *          causes ra_epub_xml_parse_opf to set toc_kind=k_ra_epub_toc_ncx and
 *          toc_path="toc.ncx".  priv_load_toc then dispatches to
 *          ra_epub_xml_parse_ncx (line 280).
 *
 * @par MC/DC:
 * Decision: `if (book->toc_kind == (uint8_t)k_ra_epub_toc_nav)` at line 277
 * (1 condition).
 * - V1 (other nav tests): toc_kind == k_ra_epub_toc_nav -> true  -> line 278.
 * - V2 (this test): toc_kind == k_ra_epub_toc_ncx     -> false -> line 280.
 * Pair (V1,V2) gives complete MC/DC for this 1-condition decision.
 *
 ...
 *          minimal in-memory ZIP, calls ra_epub_open / ra_epub_close, and
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_epub_open_cov.c\n");
  return 0;
}
