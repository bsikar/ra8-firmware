 ...
 * FAT16 volume through `ra_fs` (over a RAM block backend -- the RAM-disk pattern
 * from `tests/test_ra_epub_fs.c`). A lightweight compile *spy* is injected
 * through the DIP seam (@ref ra_rabook_import_compile_fn) so the test can count
 ...
 * `ra_fs` read path is exercised end to end.
 *
 * Coverage:
 ...
 *          `ra_log_*` calls inside the manager route here instead of poking the
...
/* --- RAM block device + a real FAT16 volume via ra_fs_format --------------- */
...
static ra_err_t
spy_compile(void* ctx, ra_fs_mount_t* mount, const char* epub_path, const char* out_path)
{
  (void)ctx;
  (void)epub_path;
  s_compile_calls++;
  ...
  return ra_fs_write_file(mount, out_path, payload, (uint32_t)sizeof(payload));
}

/** @brief Seam binding that always fails without writing (fallback test). */
static ra_err_t
spy_compile_fail(void* ctx, ra_fs_mount_t* mount, const char* epub_path, const char* out_path)
{
  (void)ctx;
  (void)mount;
  (void)epub_path;
...
static void put_source(ra_fs_mount_t* mount, const char* path)
{
  (void)ra_fs_unlink(mount, path);
  ra_fs_file_t* file = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(mount, path, k_ra_fs_mode_write, &file));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_write(file, s_epub, (uint32_t)s_epub_len));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(file));
}

/** @brief Assert the cache file at @p path holds exactly @p want[0..len). */
static void
assert_cache_bytes(ra_fs_mount_t* mount, const char* path, const uint8_t* want, uint32_t len)
{
  ra_fs_file_t* file = nullptr;
  ...
  uint32_t got = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_read(file, got_buf, (uint32_t)sizeof(got_buf), &got));
  TEST_ASSERT_EQ(len, got);
  TEST_ASSERT_EQ(0, memcmp(got_buf, want, (size_t)len));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(file));
}

static ra_rabook_import_cfg_t
make_cfg(ra_fs_mount_t* mount, uint32_t importer_version, ra_rabook_import_compile_fn compile)
{
...
{
  TEST_BEGIN("ra_rabook_import: miss -> compile -> cache -> hit");
  build_epub("<html><body><p>Hello there.</p></body></html>");
  ra_fs_mount_t* mount = fresh_volume();
  put_source(mount, "BOOK.EPB");

  ra_rabook_import_cfg_t     cfg = make_cfg(mount, (uint32_t)k_importer_version_a, spy_compile);
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_rabook_import_open(&cfg, "BOOK.EPB", path1, (uint32_t)sizeof(path1), &out));
  TEST_ASSERT_EQ(k_ra_rabook_import_compiled, out);
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_rabook_import_open(&cfg, "BOOK.EPB", path2, (uint32_t)sizeof(path2), &out));
  TEST_ASSERT_EQ(k_ra_rabook_import_hit, out);
  TEST_ASSERT_EQ(1, s_compile_calls);
  TEST_ASSERT_EQ(0, strcmp(path1, path2));
  assert_cache_bytes(mount, path2, v1, (uint32_t)k_spy_len);

  teardown(mount);
  TEST_END("ra_rabook_import: miss -> compile -> cache -> hit");
}
...
{
  TEST_BEGIN("ra_rabook_import: bumped version re-derives");
  ...
  ra_rabook_import_cfg_t cfg_a = make_cfg(mount, (uint32_t)k_importer_version_a, spy_compile);
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_rabook_import_open(&cfg_a, "BOOK.EPB", path, (uint32_t)sizeof(path), &out));
  TEST_ASSERT_EQ(k_ra_rabook_import_compiled, out);
  TEST_ASSERT_EQ(1, s_compile_calls);

  ra_rabook_import_cfg_t cfg_b = make_cfg(mount, (uint32_t)k_importer_version_b, spy_compile);
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_rabook_import_open(&cfg_b, "BOOK.EPB", path, (uint32_t)sizeof(path), &out));
  TEST_ASSERT_EQ(k_ra_rabook_import_compiled, out);
  ...
  teardown(mount);
  TEST_END("ra_rabook_import: bumped version re-derives");
}
...
{
  TEST_BEGIN("ra_rabook_import: changed content -> new cache key");
  ra_fs_mount_t* mount = fresh_volume();
  s_compile_calls      = 0;
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_rabook_import_open(&cfg, "BOOK.EPB", path_a, (uint32_t)sizeof(path_a), &out));
  TEST_ASSERT_EQ(k_ra_rabook_import_compiled, out);
  TEST_ASSERT_EQ(1, s_compile_calls);
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_rabook_import_open(&cfg, "BOOK.EPB", path_b, (uint32_t)sizeof(path_b), &out));
  TEST_ASSERT_EQ(k_ra_rabook_import_compiled, out);
  TEST_ASSERT_EQ(2, s_compile_calls);
  TEST_ASSERT(strcmp(path_a, path_b) != 0);

  teardown(mount);
  TEST_END("ra_rabook_import: changed content -> new cache key");
}
...
{
  TEST_BEGIN("ra_rabook_import: compile error leaves no cache");
  ...
  ra_rabook_import_cfg_t cfg_ok = make_cfg(mount, (uint32_t)k_importer_version_a, spy_compile);
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_rabook_import_open(&cfg_ok, "BOOK.EPB", path, (uint32_t)sizeof(path), &out));
  TEST_ASSERT_EQ(k_ra_rabook_import_compiled, out);
  TEST_ASSERT_EQ(2, s_compile_calls);

  teardown(mount);
  TEST_END("ra_rabook_import: compile error leaves no cache");
}
...
static void test_guards(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rabook_import_open(nullptr, "BOOK.EPB", path, (uint32_t)sizeof(path), &out));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rabook_import_open(&cfg, nullptr, path, (uint32_t)sizeof(path), &out));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rabook_import_open(&cfg, "BOOK.EPB", nullptr, (uint32_t)sizeof(path), &out));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rabook_import_open(&cfg, "BOOK.EPB", path, (uint32_t)sizeof(path), nullptr));

  /* Missing injected dependencies. */
  ra_rabook_import_cfg_t bad = cfg;
  bad.mount                  = nullptr;
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rabook_import_open(&bad, "BOOK.EPB", path, (uint32_t)sizeof(path), &out));
  bad         = cfg;
  bad.compile = nullptr;
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rabook_import_open(&bad, "BOOK.EPB", path, (uint32_t)sizeof(path), &out));
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_size, ra_rabook_import_open(&cfg, "BOOK.EPB", path, 1U, &out));

  /* Missing source -> propagated open error, never a compile. */
  TEST_ASSERT(ra_rabook_import_open(&cfg, "NOPE.EPB", path, (uint32_t)sizeof(path), &out) !=
              k_ra_ok);
  TEST_ASSERT_EQ(0, s_compile_calls);

  teardown(mount);
  TEST_END("ra_rabook_import: NULL + capacity guards");
}

int32_t main(void)
{
  ra_log_set_byte_sink(test_log_sink, nullptr);
  test_miss_then_hit();
