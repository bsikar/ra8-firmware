 ...
 *
 * - Lines 716, 717:
 *   ra_ceu_byte_swap_set with swap_16_bit = true (COWS bit).
 *
 ...
 * from test_ra_ceu so gcovr sees combined coverage.
 *
 ...
 *
 * @details
 * Addresses lie in the SDRAM range (0x68xxxxxx) that ra_sim_mmap
 ...
 * the `bundle_size_bytes & ~k_ra_ceu_bundle_align_mask` operation in
 ...
 *
 * @details
 * Zeroes the ra_sim_mmap window so no leftover register state from a
 ...
 * @post ra_sim_mmap is zeroed; MSTP model is in its default state; any
...
/* ---------------------------------------------------------------------------
 * Helper: build a minimal valid ra_ceu_config_t
 * --------------------------------------------------------------------------- */
 ...
 *
 * @details
 * Fills every required field with sensible defaults so that ra_ceu_init
 * succeeds without further setup.  Tests that need a specific override
 * mutate the returned struct before passing it to ra_ceu_init.
 *
 * @return ra_ceu_config_t Populated config struct.
 *
 * @pre None.
 ...
 *
 * ra_ceu_plane_b_program is used as the entry point because it delegates
 * directly to internal_validate_buffers without adding an extra non-null
 * guard on y_top (unlike ra_ceu_capture_start_ex).
 * --------------------------------------------------------------------------- */
 ...
 *
 * @details
 * ra_ceu_capture_start_ex with all eight buffer fields set to distinct
 ...
 *
 * @pre ra_ceu_init has been called so the MSTP model is live.
...
static void test_cov_program_addresses_bundle2(void)
{
  ...
  TEST_ASSERT_EQ(k_test_cov_buf_e, *ra_ceu_reg32(k_ra_ceu_off_cdayr2));
  TEST_ASSERT_EQ(k_test_cov_buf_f, *ra_ceu_reg32(k_ra_ceu_off_cdacr2));
  TEST_ASSERT_EQ(k_test_cov_buf_g, *ra_ceu_reg32(k_ra_ceu_off_cdbyr2));
  TEST_ASSERT_EQ(k_test_cov_buf_h, *ra_ceu_reg32(k_ra_ceu_off_cdbcr2));
 ...
 *
 * @details
 * ra_ceu_plane_b_program with every buffer field non-null and
...
static void test_cov_plane_b_all_overrides(void)
{
  ...
  TEST_ASSERT_EQ(k_test_cov_buf_a, *ra_ceu_reg32_plane(k_ra_ceu_off_cdayr, k_ra_ceu_plane_b_off));
  TEST_ASSERT_EQ(k_test_cov_buf_b, *ra_ceu_reg32_plane(k_ra_ceu_off_cdacr, k_ra_ceu_plane_b_off));
  TEST_ASSERT_EQ(k_test_cov_buf_c, *ra_ceu_reg32_plane(k_ra_ceu_off_cdbyr, k_ra_ceu_plane_b_off));
  TEST_ASSERT_EQ(k_test_cov_buf_d, *ra_ceu_reg32_plane(k_ra_ceu_off_cdbcr, k_ra_ceu_plane_b_off));
  TEST_ASSERT_EQ(k_test_cov_buf_e, *ra_ceu_reg32_plane(k_ra_ceu_off_cdayr2, k_ra_ceu_plane_b_off));
  TEST_ASSERT_EQ(k_test_cov_buf_f, *ra_ceu_reg32_plane(k_ra_ceu_off_cdacr2, k_ra_ceu_plane_b_off));
  TEST_ASSERT_EQ(k_test_cov_buf_g, *ra_ceu_reg32_plane(k_ra_ceu_off_cdbyr2, k_ra_ceu_plane_b_off));
  TEST_ASSERT_EQ(k_test_cov_buf_h, *ra_ceu_reg32_plane(k_ra_ceu_off_cdbcr2, k_ra_ceu_plane_b_off));
...
/* ---------------------------------------------------------------------------
 * Test: ra_ceu_byte_swap_set -- swap_16_bit true path (lines 716-717)
 * --------------------------------------------------------------------------- */

/**
 * @brief Cover lines 716-717: ra_ceu_byte_swap_set with swap_16_bit true.
 *
 * @details
 * The sibling test_ra_ceu.c exercises ra_ceu_byte_swap_set with
 ...
 * the false branch is covered by test_ra_ceu.c, the true branch here)
 *
 * @pre None.
...
static void test_cov_byte_swap_16_bit(void)
{
  ...
  prep();
  *ra_ceu_reg32(k_ra_ceu_off_cdocr) = 0U;
  const ra_ceu_byte_swap_t sw       = {
    .swap_8_bit  = false,
    .swap_16_bit = true,
    .swap_32_bit = false,
  };
 ...
 *
 * @pre Linked against ra_core_hal OBJECT library with RA_SIMULATOR_MODE
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_ceu_cov.c\n");
  return 0;
}
