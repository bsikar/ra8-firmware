/**
 * @file test_ra_sdmmc_card_reflow.c
 * @brief Full SD-over-SPI path: a modelled SD card serves a FAT volume to
 *        the real ra_sdmmc_spi driver, ra_fs reads a font off it, and
 *        ra_reflow renders -- all heap-free, all on host.
 *
 * @details
 * This is the hardware-faithful version of test_ra_fs_font_reflow: instead
 ...
 * CMD17 single-block read) exactly as the @ref ra_sdmmc_spi driver expects.
 * The driver's transport seam routes every byte exchange into the card
 * model, so the genuine ra_sdmmc_spi command/response/data-token code runs.
 * `ra_sdmmc_spi_bind_fs_backend` then plugs the card straight into ra_fs.
 *
 ...
 *
 * Pipeline: image (FAT) -> SD card model -> ra_sdmmc_spi -> ra_fs ->
 * ra_reflow -> ra_gfx framebuffer.
 *
...
static void sd_process_cmd(sd_card_t* c)
{
  ...
  switch (idx) {
    ...
    case (uint8_t)k_sd_idx_cmd9: { /* SEND_CSD -> R1 + token + 16-byte CSD + CRC */
      ...
      const uint16_t crc9                       = ra_sdmmc_spi_crc16(&c->resp[2], k_sd_csd_len);
    ...
    case (uint8_t)k_sd_idx_cmd17: { /* READ_SINGLE_BLOCK (SDHC: arg = block) */
      ...
      const uint16_t crc17               = ra_sdmmc_spi_crc16(&c->resp[2], (uint32_t)k_sd_block);
...
/* ---- ra_sdmmc_spi transport bound to the card model ---------------------- */
static ra_err_t t_set_clock(void* ctx, uint32_t hz)
{
  (void)ctx;
  (void)hz;
  return k_ra_ok;
}
static ra_err_t t_cs(void* ctx, bool asserted)
{
  (void)ctx;
  (void)asserted;
  return k_ra_ok;
}
static ra_err_t t_xfer(void* ctx, const uint8_t* tx, uint8_t* rx, uint32_t len)
{
...
static const ra_fs_backend_t s_mem_backend = {.read_block   = mem_read,
 ...
 * already-covered ra_sdmmc_spi / ra_fs / ra_reflow APIs end to end through
...
{
  TEST_BEGIN("SD card (SPI) serves FAT font -> ra_fs -> ra_reflow");
  ...
  build_fat16();
  ra_fs_mount_t* setup = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_mem_backend, &setup));
  ra_fs_file_t* wf = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(setup, "FONT.OTF", k_ra_fs_mode_write, &wf));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_write(wf, s_src_font, s_font_len));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(wf));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(setup));

  /* Bring up the SD card through the REAL ra_sdmmc_spi driver. */
  s_card.image                         = s_disk.bytes;
  s_card.image_len                     = s_disk.block_count * (uint32_t)k_disk_block_size;
  const ra_sdmmc_spi_transport_t trans = {.set_clock = t_set_clock,
                                          .cs        = t_cs,
                                          .xfer      = t_xfer,
                                          .ctx       = &s_card};
  TEST_ASSERT_EQ(k_ra_ok, ra_sdmmc_spi_init(&trans));

  /* Mount the card as a FAT filesystem and read the font back off it. */
  ra_fs_backend_t sd_backend = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_sdmmc_spi_bind_fs_backend(&sd_backend));
  ra_fs_mount_t* mnt = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&sd_backend, &mnt));
  ra_fs_file_t* rf = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(mnt, "FONT.OTF", k_ra_fs_mode_read, &rf));
  uint32_t got = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_read(rf, s_card_font, k_font_cap, &got));
  TEST_ASSERT_EQ(s_font_len, got);
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(rf));
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gfx_init(s_fb, (uint16_t)k_fb_w, (uint16_t)k_fb_h, k_ra_gfx_format_argb8888));
  TEST_ASSERT_EQ(k_ra_ok, ra_gfx_clear((uint32_t)k_bg_argb));
  ra_reflow_t engine = {};
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_reflow_init((uint16_t)k_fb_w,
                                (uint16_t)k_fb_h,
                                s_card_font,
                                s_font_len,
                                (uint16_t)k_font_px,
  ...
  uint32_t    pages = 0U;
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_reflow_layout_chapter(&engine, (const uint8_t*)xhtml, strlen(xhtml), &pages));
  TEST_ASSERT(pages >= 1U);
  TEST_ASSERT_EQ(k_ra_ok, ra_reflow_render_page(&engine, 0U, nullptr));
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_sdmmc_card_reflow.c\n");
  return 0;
}
