 ...
 *
 * @details
 * SHA-256 is on the root-of-trust verify path (ra_rot_verify_image re-hashes the
 ...
 * ra_psa_hash_compute to the published FIPS 180-4 / NIST SHA-256 test vectors, an
 ...
 *
 * @details Both test cases run in one process; the second ra_psa_crypto_init
 * returns k_ra_err_exists rather than k_ra_ok. Split, so there is no compound
 * decision to cover.
 */
static void ensure_psa_init(void)
{
  const ra_err_t err = ra_psa_crypto_init();
  if (err != k_ra_ok) {
    TEST_ASSERT_EQ(k_ra_err_exists, err);
  }
}
...
{
  uint8_t expect[k_ra_psa_sha256_len];
  hex_to_bytes(expect_hex, expect, (size_t)k_ra_psa_sha256_len);
 ...
 * decision logic is exercised by test_ra_psa_crypto.c.)
 */
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_psa_sha256_kat.c\n");
  return 0;
}
