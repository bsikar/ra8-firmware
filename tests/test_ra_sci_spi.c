/**
 * @file test_ra_sci_spi.c
 * @brief Unit tests for the SCI Simple-SPI controller driver
 *        (libs/ra_hal/src/ra_sci_spi.c).
 *
 * @details
 * Exercises all five public entry points of the SCI_B Simple-SPI driver
 * against the ``ra_sim_mmap``-backed register window:
 *
 *  - ``ra_sci_spi_init``      -- MSTP enable + Simple-SPI CCR programming;
 *  - ``ra_sci_spi_deinit``    -- TE/RE clear + MSTP release;
 *  - ``ra_sci_spi_set_clock`` -- runtime CCR2 (CKS/BRR) reprogramme;
 *  - ``ra_sci_spi_xfer8``     -- single-frame full-duplex exchange;
 *  - ``ra_sci_spi_xfer``      -- multi-byte full-duplex exchange.
 *
 * Determinism note: the xfer paths poll CSR.TDRE then CSR.RDRF via the
 * bounded ``ra_hw_wait_flag_set32`` helper. The simulator backs MMIO with
 ...
 * budget and return ``k_ra_err_hw_timeout``. No SIGALRM / setitimer
...
} ra_sci_spi_test_const_t;

/**
 * @enum ra_sci_spi_test_byte_t
...
} ra_sci_spi_test_byte_t;

/**
 * @brief Standard mode-0 / 1 MHz Simple-SPI descriptor.
 */
static const ra_sci_spi_cfg_t k_spi_cfg = {
...
{
  TEST_BEGIN("ra_sci_spi_init: mode 0 enables TE+RE in Simple-SPI mode");
  prep();

  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_init((uint8_t)k_spi_test_ch, &k_spi_cfg));

  volatile const r_sci_regs_t* reg = ra_sci((uint8_t)k_spi_test_ch);
  /* CCR0 must enable both transmit and receive. */
  TEST_ASSERT((reg->CCR0 & (1U << (uint8_t)k_ra_sci_ccr0_bit_te)) != 0U);
  ...
  TEST_ASSERT_EQ(0, (reg->CCR3 & (1U << (uint8_t)k_ra_sci_ccr3_bit_lsbf)));
  TEST_END("ra_sci_spi_init: mode 0 enables TE+RE in Simple-SPI mode");
}
...
{
  TEST_BEGIN("ra_sci_spi_init: mode 3 + LSB-first sets CPOL, CPHA, LSBF");
  prep();

  const ra_sci_spi_cfg_t cfg = {
    .baud_hz   = (uint32_t)k_spi_test_baud,
    .pclk_hz   = (uint32_t)k_spi_test_pclk,
    .mode      = k_ra_spi_mode_3,
    .lsb_first = true,
  };
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_init((uint8_t)k_spi_test_ch, &cfg));

  volatile const r_sci_regs_t* reg = ra_sci((uint8_t)k_spi_test_ch);
  TEST_ASSERT((reg->CCR3 & (1U << (uint8_t)k_ra_sci_ccr3_bit_cpol)) != 0U);
  TEST_ASSERT((reg->CCR3 & (1U << (uint8_t)k_ra_sci_ccr3_bit_cpha)) != 0U);
  TEST_ASSERT((reg->CCR3 & (1U << (uint8_t)k_ra_sci_ccr3_bit_lsbf)) != 0U);
  TEST_END("ra_sci_spi_init: mode 3 + LSB-first sets CPOL, CPHA, LSBF");
}
...
{
  TEST_BEGIN("ra_sci_spi_init: null cfg / bad channel / zero baud rejected");
  prep();

  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_sci_spi_init((uint8_t)k_spi_test_ch, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_sci_spi_init((uint8_t)k_spi_test_ch_bad, &k_spi_cfg));

  const ra_sci_spi_cfg_t cfg_zero = {
    .baud_hz   = (uint32_t)k_spi_test_baud_zero,
    .pclk_hz   = (uint32_t)k_spi_test_pclk,
    .mode      = k_ra_spi_mode_0,
    .lsb_first = false,
  };
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_sci_spi_init((uint8_t)k_spi_test_ch, &cfg_zero));
  TEST_END("ra_sci_spi_init: null cfg / bad channel / zero baud rejected");
}

/* =========================================================================
 * ra_sci_spi_set_clock (drives internal_resolve_rate across all branches)
 * =========================================================================
 */
...
{
  TEST_BEGIN("ra_sci_spi_set_clock: normal / clamp / fast rate resolution");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_init((uint8_t)k_spi_test_ch, &k_spi_cfg));

  volatile r_sci_regs_t* reg = ra_sci((uint8_t)k_spi_test_ch);

  /* Clear CCR2 so set_clock's write is observable: ra_sci_spi_init already */
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_sci_spi_set_clock((uint8_t)k_spi_test_ch,
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_sci_spi_set_clock((uint8_t)k_spi_test_ch,
                                      ...
                                      (uint32_t)k_spi_test_pclk));
  TEST_END("ra_sci_spi_set_clock: normal / clamp / fast rate resolution");
}
...
{
  TEST_BEGIN("ra_sci_spi_set_clock: bad channel + zero baud rejected");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_init((uint8_t)k_spi_test_ch, &k_spi_cfg));

  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_sci_spi_set_clock((uint8_t)k_spi_test_ch_bad,
                                      (uint32_t)k_spi_test_baud,
                                      (uint32_t)k_spi_test_pclk));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_sci_spi_set_clock((uint8_t)k_spi_test_ch,
                                      ...
                                      (uint32_t)k_spi_test_pclk));
  TEST_END("ra_sci_spi_set_clock: bad channel + zero baud rejected");
}
...
{
  TEST_BEGIN("ra_sci_spi_deinit: clears CCR0 and releases MSTP");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_init((uint8_t)k_spi_test_ch_alt, &k_spi_cfg));

  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_deinit((uint8_t)k_spi_test_ch_alt));
  volatile const r_sci_regs_t* reg = ra_sci((uint8_t)k_spi_test_ch_alt);
  TEST_ASSERT_EQ(0U, reg->CCR0);

  bool stopped = false;
  TEST_ASSERT_EQ(k_ra_ok, ra_mstp_is_stopped(k_ra_mstp_sci3, &stopped));
  TEST_ASSERT(stopped);

  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_sci_spi_deinit((uint8_t)k_spi_test_ch_bad));
  TEST_END("ra_sci_spi_deinit: clears CCR0 and releases MSTP");
}
...
{
  TEST_BEGIN("ra_sci_spi_xfer8: full-duplex frame round trip");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_init((uint8_t)k_spi_test_ch, &k_spi_cfg));
  ...
  seed_ready((uint8_t)k_spi_test_ch);
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_sci_spi_xfer8((uint8_t)k_spi_test_ch, (uint8_t)k_spi_test_tx_byte, nullptr));
  TEST_END("ra_sci_spi_xfer8: full-duplex frame round trip");
}

/**
 * @par MC/DC:
 * (no compound decisions in this test -- exercises the channel-out-of-range
 * guard via the NULL register pointer returned by ra_sci(99))
 */
static void test_xfer8_bad_channel(void)
{
  TEST_BEGIN("ra_sci_spi_xfer8: bad channel rejected");
  prep();
  uint8_t rx = 0U;
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_sci_spi_xfer8((uint8_t)k_spi_test_ch_bad, (uint8_t)k_spi_test_tx_byte, &rx));
  TEST_END("ra_sci_spi_xfer8: bad channel rejected");
}
...
{
  TEST_BEGIN("ra_sci_spi_xfer8: TDRE poll times out");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_init((uint8_t)k_spi_test_ch, &k_spi_cfg));
...
{
  TEST_BEGIN("ra_sci_spi_xfer: multi-byte full-duplex loop");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_init((uint8_t)k_spi_test_ch, &k_spi_cfg));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_xfer((uint8_t)k_spi_test_ch, tx, rx, 4U));
  ...
  seed_ready((uint8_t)k_spi_test_ch);
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_xfer((uint8_t)k_spi_test_ch, tx, nullptr, 2U));
  TEST_END("ra_sci_spi_xfer: multi-byte full-duplex loop");
}
...
{
  TEST_BEGIN("ra_sci_spi_xfer: len 0 no-op + bad channel rejected");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_init((uint8_t)k_spi_test_ch, &k_spi_cfg));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_xfer((uint8_t)k_spi_test_ch, tx, rx, 0U));
  /* Bad channel is rejected up-front. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_sci_spi_xfer((uint8_t)k_spi_test_ch_bad, tx, rx, 1U));
  TEST_END("ra_sci_spi_xfer: len 0 no-op + bad channel rejected");
}

/**
 * @par MC/DC:
 * (no compound decisions in this test -- the per-frame xfer8 times out and
 * ra_sci_spi_xfer forwards that error, exercising its error-propagation
 * branch)
 */
static void test_xfer_propagates_timeout(void)
{
  TEST_BEGIN("ra_sci_spi_xfer: forwards per-frame timeout");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_init((uint8_t)k_spi_test_ch, &k_spi_cfg));
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_sci_spi.c\n");
  return 0;
}
