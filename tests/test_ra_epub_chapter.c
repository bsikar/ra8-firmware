 ...
 * @brief Exercise the success path of `ra_epub_get_chapter_count` (in_use==1).
 *
 * @par Coverage:
 * The existing MC/DC test only drives the NULL guard and the not-initialized
 * arm; this adds the `*out_count = book->chapter_count; return k_ra_ok;` body
 ...
 * @brief Exercise the success path of `ra_epub_get_metadata` (in_use==1).
 *
 * @par Coverage:
 * Runs the four `out_meta->* = book->*` assignments + `return k_ra_ok`, which
 ...
 * @brief Exercise the success path of `ra_epub_get_embedded_font_count`.
 *
 * @par Coverage:
 * Adds the `*out_count = book->embedded_font_count; return k_ra_ok;` body to the
 ...
 * ra_epub_get_toc_kind). Per DO-178C 6.4.4.3 N+1 = 3 vectors:
 ...
 * runs the `*out_kind = book->toc_kind; return k_ra_ok;` success body.
 */
static void test_epub_get_toc_kind(void)
{
  ...
  ra_epub_book_t book = {};
  uint8_t        kind = 0U;
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_epub_get_toc_kind(&book, &kind)); /* V1 */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_epub_get_toc_kind(nullptr, &kind));      /* V2 */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_epub_get_toc_kind(&book, nullptr));      /* V3 */
  book.in_use   = 1U;
 ...
 * ra_epub_get_toc_count). Per DO-178C 6.4.4.3 N+1 = 3 vectors:
...
static void test_epub_get_toc_count(void)
{
  ...
  ra_epub_book_t book = {};
  uint16_t       cnt  = 0U;
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_epub_get_toc_count(&book, &cnt)); /* V1 */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_epub_get_toc_count(nullptr, &cnt));      /* V2 */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_epub_get_toc_count(&book, nullptr));     /* V3 */
  book.in_use    = 1U;
 ...
 * ra_epub_get_toc_entry). N+1 = 3 vectors (V1 both-false, V2 book=NULL,
...
static void test_epub_get_toc_entry(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_epub_get_toc_entry(&book, 0U, &entry)); /* V1 */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_epub_get_toc_entry(nullptr, 0U, &entry));      /* V2 */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_epub_get_toc_entry(&book, 0U, nullptr));       /* V3 */
  book.in_use    = 1U;
  book.toc_count = 1U;
  (void)strcpy(book.toc[0].href, "ch1.xhtml");
  book.toc[0].depth = (uint8_t)k_test_epub_depth_val;
  /* idx >= toc_count -> out_of_range. */
  TEST_ASSERT_EQ(k_ra_err_out_of_range, ra_epub_get_toc_entry(&book, book.toc_count, &entry));
 ...
 * - the match `return k_ra_ok` (entries 0 and 2);
 * - the loop fall-through `return k_ra_err_not_found` (entry 1 matches nothing);
 ...
 * (3 conditions, OR; ra_epub_load_chapter). Per DO-178C 6.4.4.3 N+1 = 4 vectors:
 ...
 * @par MC/DC:
 * Decision: ``if (book == NULL || out_buf == NULL || got_len == NULL)``
 * (3 conditions, OR; ra_epub_get_cover_image). N+1 = 4 vectors (V1 all non-NULL
 ...
 * @brief Drive `ra_epub_get_resource`'s ready-book pre-extract guards.
 *
 * @par Coverage:
 ...
 * @brief Drive `ra_epub_get_embedded_font`'s ready-book pre-extract guards.
 *
 * @par Coverage:
...
static void test_epub_get_embedded_font_guards(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_out_of_range,
                 ra_epub_get_embedded_font(&book, 0U, buf, sizeof(buf), &got));
 ...
 * out_h == NULL)`` (4 conditions, OR; ra_epub_render_glyph). N+1 = 5 vectors:
...
static void test_epub_render_glyph_guards(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_epub_set_font(&book, s_bogus_font, (size_t)k_test_epub_font_min_bytes));
