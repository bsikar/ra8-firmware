/**
 * @file test_ra_reflow_render_cov.c
 * @brief Line-coverage (gcovr) tests for libs/ra_reflow/src/ra_reflow_render.c.
 *
 * @details
 * The existing MC/DC render suite (tests/test_ra_reflow_render.c) drives the
...
/** @brief A shapes SVG with a filled rect (drawn via ra_svg_render, no image). */
 ...
 * @return k_ra_ok always.
 */
static ra_err_t svg_shapes_loader(void*           ctx,
                                  const char*     href,
                                  uint32_t        href_len,
                                  const uint8_t** out,
                                  size_t*         out_len)
...
static int load_font(void)
{
  ...
  while (base > 0U && here[base - 1U] != '/') {
    base--; /* drop "test_ra_reflow_render_cov.c" */
  }
  while (base > 0U && here[base - 1U] == '/') {
    base--;
  }
...
{
  return ra_reflow_init((uint16_t)k_rc_vp_w,
                        (uint16_t)k_rc_vp_h,
                        s_font,
                        s_font_len,
...
/** @brief Bind the RGB888 framebuffer (cleared to zero) to ra_gfx. */
static void bind_clear_fb(void)
{
  memset(s_fb, 0, sizeof s_fb);
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_gfx_init(s_fb, (uint16_t)k_rc_vp_w, (uint16_t)k_rc_vp_h, k_ra_gfx_format_rgb888));
}
 ...
 *
 * @details Targets ra_reflow_render.c lines 89-93 (priv_init_font's offset<0 /
...
static void test_cov_render_bad_default_font(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_validation_failed, ra_reflow_render_page(&s_engine, 0U, nullptr));
  TEST_END("render cov: junk default font -> validation_failed");
}
 ...
 *
 * @details Targets ra_reflow_render.c line 219-220 (priv_blit_glyph's underline
 * gate) and 156-170 (priv_draw_underline). The tokenizer stamps every `<a>`
 * glyph with k_ra_reflow_style_underline, so rendering the page draws the
 ...
 *
 * @details Targets ra_reflow_render.c lines 305-316 (priv_render_images loop),
 * 284-302 (priv_render_one_image: href fetch + raster branch) and the
 * ra_img_decode_blit call. A loader returning the 2x2 PNG yields a recorded image
 ...
 *
 * @details Targets ra_reflow_render.c lines 257-281 (priv_render_svg: the
 * image-href unwrap path *and* the ra_svg_render shapes path) and 297-299
 ...
 * drives the ra_svg_render branch.
 */
static void test_cov_render_svg_image(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, init_engine());
  ra_img_arena_t arena = {.base = s_arena_buf, .cap = sizeof s_arena_buf, .offset = 0U, .live = 0U};
  TEST_ASSERT_EQ(k_ra_ok, ra_reflow_set_image_loader(&s_engine, svg_loader, nullptr, &arena));
  uint32_t    pages = 0U;
  const char* doc_a = "<p>Cover:</p><img src=\"cover.svg\"/><p>End.</p>";
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_reflow_layout_chapter(&s_engine, (const uint8_t*)doc_a, strlen(doc_a), &pages));
  TEST_ASSERT(s_engine.image_box_count >= 1U);
  bind_clear_fb();
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_reflow_render_page(&s_engine, s_engine.image_boxes[0].page_index, nullptr));
  ...
  /* Route B: shapes SVG -> ra_svg_render branch (no embedded <image> href). */
  TEST_ASSERT_EQ(k_ra_ok, init_engine());
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_reflow_set_image_loader(&s_engine, svg_shapes_loader, nullptr, &arena));
  const char* doc_b = "<p>Shapes:</p><img src=\"art.svg\"/><p>Done.</p>";
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_reflow_layout_chapter(&s_engine, (const uint8_t*)doc_b, strlen(doc_b), &pages));
  TEST_ASSERT(s_engine.image_box_count >= 1U);
  bind_clear_fb();
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_reflow_render_page(&s_engine, s_engine.image_boxes[0].page_index, nullptr));
 ...
 *
 * @details Targets ra_reflow_render.c lines 402-403 (the `fi >= nfaces ->
 ...
 * the page still rasterises. Driven via ra_reflow_render_page_at() to also
...
static void test_cov_render_face_index_clamp(void)
{
  ...
  bind_clear_fb();
  TEST_ASSERT_EQ(k_ra_ok, ra_reflow_render_page_at(&s_engine, 0, 0, 0));
  TEST_ASSERT(fb_has_drawn_pixel());
  TEST_END("render cov: out-of-range face index clamp");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_reflow_render_cov.c\n");
  return 0;
}
