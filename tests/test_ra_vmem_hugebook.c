 ...
 * (::ra_vsource + ::ra_vmem SLRU) over a multi-gigabyte object through a
 ...
 *     ::ra_vsource_loader read directly, bypassing the cache), including the
...
static ra_vmem_frame_t s_meta[(size_t)k_h_frames];
...
/**
 * @brief ra_vsource read callback: serve generated bytes (no backing buffer).
 *
 ...
 * @retval k_ra_ok Buffer filled with the generated content.
 *
 ...
 * @since 0.1.0
 */
static ra_err_t t_read(void* ctx, uint64_t offset, uint8_t* buf, uint32_t len)
{
  (void)ctx;
 ...
 *          object's end -- mirroring ::ra_vsource_loader without using the cache.
 *
 ...
 * @since 0.1.0
 */
static void t_touch(ra_vmem_t* vm, uint32_t obj, uint32_t page)
{
  const uint64_t off = (uint64_t)page * (uint64_t)k_h_frame_bytes;
  void*          p   = nullptr;
  const ra_err_t e   = ra_vmem_get(vm, obj, off, &p);
...
/** @brief Wire ra_vsource + ra_vmem over the static fixtures for the huge object. */
static uint32_t t_setup(ra_vsource_t* vs, ra_vsource_obj_t* objs, ra_vmem_t* vm)
{
  TEST_ASSERT_EQ(k_ra_ok, ra_vsource_init(vs, objs, 1U));
  uint32_t obj = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_vsource_add_paged(vs, t_read, nullptr, 0U, t_obj_size(), &obj));
 ...
 *          through the un-cached ::ra_vsource_loader into a scratch buffer, then
 ...
 * @since 0.1.0
 */
static void t_crosscheck(ra_vmem_t* vm, ra_vsource_t* vs, uint32_t obj, uint32_t page)
{
  const uint64_t off = (uint64_t)page * (uint64_t)k_h_frame_bytes;
  uint8_t        uncached[(size_t)k_h_frame_bytes];
  TEST_ASSERT_EQ(k_ra_ok, ra_vsource_loader(vs, obj, off, uncached, (uint32_t)k_h_frame_bytes));
  uint8_t ref[(size_t)k_h_frame_bytes];
  t_ref_fill(off, ref);
  ...
  const ra_err_t e = ra_vmem_get(vm, obj, off, &p);
  TEST_ASSERT_EQ(k_ra_ok, e);
  TEST_ASSERT_EQ(0, memcmp(p, uncached, (size_t)k_h_frame_bytes));
  TEST_ASSERT_EQ(k_ra_ok, ra_vmem_put(vm, p));
}
 ...
 */
static void t_warm_hot(ra_vmem_t* vm, uint32_t obj)
{
 ...
 */
static void t_scan_flood(ra_vmem_t* vm, uint32_t obj)
{
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_vmem_hugebook.c\n");
  return 0;
}
