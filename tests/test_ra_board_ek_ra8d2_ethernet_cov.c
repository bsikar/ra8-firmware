/**
 * @file test_ra_board_ek_ra8d2_ethernet_cov.c
 * @brief Coverage-boosting unit tests for the EK-RA8D2 Ethernet BSP layer
 *
 * @details
 * Companion to test_ra_board_ek_ra8d2.c.  Exercises the two error-return
 * branches in ra_board_ek_ra8d2_ethernet.c that are reachable from a
 ...
 *
 *   - Source line 134 + 770:
 *       Pre-claim the PHY RSTN pin (P7.8) so ra_gpio_output_init inside
 *       internal_eth_phy_hw_reset returns k_ra_err_gpio_conflict.
 *       ra_board_ethernet_init propagates the error at line 770.
 *
 *   - Source line 199 + 775:
 *       Pre-claim the MDC pin (P4.15) so ra_pfs_route_peripheral inside
 *       internal_eth_route_alt_pins returns k_ra_err_gpio_conflict on the
 *       second non-RSTN pin.  ra_board_ethernet_init propagates the error
 *       at line 775.
 *
 * All other uncovered branches in the module are error-return paths where
 * the called HAL function cannot fail under RA_SIMULATOR_MODE given the
...
/**
 * @brief Verify that ra_board_ethernet_init propagates a gpio conflict from
 *        internal_eth_phy_hw_reset.
 *
 * @details
 * Pre-claims the PHY RSTN pin (P7.8 = k_ra_board_eth_pin_rstn) via
 * ra_pin_validator_claim before calling ra_board_ethernet_init.  The call
 * sequence inside ra_board_ethernet_init is:
 *
 *   1. internal_eth_phy_hw_reset():
 *      ra_gpio_output_init(P7.8, LOW) -> ra_pin_validator_claim(P7.8, ...)
 *      fails with k_ra_err_gpio_conflict (P7.8 already claimed).
 *      -> return err at source line 134.
 *   2. ra_board_ethernet_init:
 *      err != k_ra_ok -> return err at source line 770.
 *
 * The test verifies that the returned error is non-zero.  The exact
 * value (k_ra_err_gpio_conflict) is asserted to confirm the conflict
 * path, not a different failure mode.
 *
 * @par MC/DC:
 * Decision: `if (err != k_ra_ok)` in internal_eth_phy_hw_reset (line 133)
 *   1 condition.
 * - Vector A: err=k_ra_ok (happy path, covered by test_board_ethernet_init
 *   in test_ra_board_ek_ra8d2.c)
 ...
 *
 * Decision: `if (err != k_ra_ok)` in ra_board_ethernet_init (line 769)
 *   1 condition.
 * - Vector A: err=k_ra_ok (happy path, existing test)
 * - Vector B: err=k_ra_err_gpio_conflict (this test)
 ...
 *       returned before ra_gpio_output_init had a chance to claim it.
 *
...
static void test_eth_phy_hw_reset_gpio_conflict(void)
{
  ...
  /* Pre-claim the RSTN pin so ra_gpio_output_init sees a conflict. */
  /* NOLINTBEGIN(clang-analyzer-optin.core.EnumCastOutOfRange) */
  const ra_port_pin_t rstn_pin = (ra_port_pin_t)k_ra_board_eth_pin_rstn;
  /* NOLINTEND(clang-analyzer-optin.core.EnumCastOutOfRange) */
  TEST_ASSERT_EQ(k_ra_ok, ra_pin_validator_claim(rstn_pin, "test"));

  const ra_err_t err = ra_board_ethernet_init();
...
/**
 * @brief Verify that ra_board_ethernet_init propagates a pin-conflict error
 *        from internal_eth_route_alt_pins.
 *
 * @details
 * Pre-claims the MDC pin (P4.15 = k_ra_board_eth_pin_mdc) and leaves the
 * RSTN pin (P7.8) free.  The call sequence inside ra_board_ethernet_init
 * is then:
 *
 *   1. internal_eth_phy_hw_reset(): RSTN (P7.8) is free, gpio_output_init
 *      succeeds, gpio_write succeeds -> returns k_ra_ok.
 *
 ...
 *
 *   3. ra_board_ethernet_init:
 *      err != k_ra_ok -> return err at source line 775.
 *
 * @par MC/DC:
 * Decision: `if (err != k_ra_ok)` in internal_eth_route_alt_pins (line 198)
 *   1 condition.
 * - Vector A: err=k_ra_ok (MDINT at index 0 routed successfully; also the
 *   full happy-path in test_ra_board_ek_ra8d2.c)
 * - Vector B: err=k_ra_err_gpio_conflict (MDC at index 1, this test)
 * Vectors A+B: minimal MC/DC for a single-condition decision.
 *
 * Decision: `if (err != k_ra_ok)` in ra_board_ethernet_init (line 774)
 *   1 condition.
 * - Vector A: err=k_ra_ok (happy path, existing test)
 * - Vector B: err=k_ra_err_gpio_conflict (this test)
...
static void test_eth_route_alt_pins_pfs_conflict(void)
{
  ...
  /* NOLINTBEGIN(clang-analyzer-optin.core.EnumCastOutOfRange) */
  const ra_port_pin_t mdc_pin = (ra_port_pin_t)k_ra_board_eth_pin_mdc;
  /* NOLINTEND(clang-analyzer-optin.core.EnumCastOutOfRange) */
  TEST_ASSERT_EQ(k_ra_ok, ra_pin_validator_claim(mdc_pin, "test"));
 ...
 * @return 0 on success.
 *
 * @pre ra_sim_mmap register window allocated by the test framework constructor.
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_board_ek_ra8d2_ethernet_cov.c\n");
  return 0;
}
