...
static uint16_t mcu_tx_count(void) { return s_io.mcu_to_modem.tail; }
...
static void test_send_cmd_before_init_fails(void) {
  ...
  TEST_ASSERT_EQ((int)k_ra_err_not_initialized,
                 (int)ra_modem_at_send_cmd("AT", NULL, 50U));
...
static void test_at_error_returned(void) {
  ...
  TEST_ASSERT_EQ((int)k_ra_err_hw_error,
                 (int)ra_modem_at_send_cmd("AT+BAD", NULL, 1000U));
...
static void test_at_cme_error(void) {
  ...
  TEST_ASSERT_EQ((int)k_ra_err_hw_error,
                 (int)ra_modem_at_send_cmd("AT+CPIN?", NULL, 1000U));
...
static void test_timeout(void) {
  ...
  TEST_ASSERT_EQ((int)k_ra_err_hw_timeout,
                 (int)ra_modem_at_send_cmd("AT", NULL, 100U));
...
static void test_send_cmd_capture(void) {
  ...
  char out[64];
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_modem_at_send_cmd_capture(
                                   "AT+CSQ", out, sizeof out, 1000U));
...
static void test_urc_dispatch(void) {
  ...
  s_urc_hits = 0;
  s_last_urc[0] = '\0';
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_modem_at_register_unsolicited_handler(
                                   "+CMTI:", urc_handler, NULL));
...
static void test_urc_table_full(void) {
  ...
  const char *prefixes[] = {
      "+A:", "+B:", "+C:", "+D:", "+E:", "+F:", "+G:", "+H:"};
  for (uint8_t i = 0U; i < (uint8_t)(sizeof prefixes / sizeof prefixes[0]);
       ++i) {
    TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_modem_at_register_unsolicited_handler(
                                     prefixes[i], urc_handler, NULL));
  }
  TEST_ASSERT_EQ(
      (int)k_ra_err_no_mem,
...
static void test_expected_response(void) {
  ...
  TEST_ASSERT_EQ((int)k_ra_ok,
                 (int)ra_modem_at_send_cmd("AT+CPIN?", "+CPIN:", 1000U));
...
static void test_send_cmd_null_arg(void) {
  ...
  TEST_ASSERT_EQ((int)k_ra_err_null_ptr,
                 (int)ra_modem_at_send_cmd(NULL, NULL, 100U));
