 ...
 * file-size cap (see ``tests/storage/src/test_ra8_fs_format.c`` and
 * ``tests/storage/src/test_ra8_fs_format_exfat.c``). Both halves format and then mount a
...
/* LBA-TARGETED read fault, for the paths whose ordinal position in the read
 * stream is not stable enough to aim at:
 *
 *   - `s_fault_read_lba`  the ABSOLUTE sector the backend will be asked for
 *                         (partition-relative LBA plus `partition_base_lba`),
 *   - `s_fault_lba_skip`  how many matching reads to let through first, so a
 *                         driver that touches one sector several times can
 *                         have any single visit failed,
 *   - `s_fault_lba_armed` the enable, cleared the moment the fault fires.
 *
 * One-shot on purpose, exactly like the ordinal `s_fault_read_at` is not: a
 * caller's ERROR HANDLING often needs a working device (a rollback, a
 * fallback read), and a latching fault would make every recovery path in the
 * tree look broken. Note the FAT sector cache in ra8_fs_fat_alloc.c serves a
 * repeated read of the SAME FAT sector without touching the backend, so a
 * skip count only advances on reads that actually reach here. */
[[maybe_unused]] static uint64_t s_fault_read_lba  = 0U;
[[maybe_unused]] static uint32_t s_fault_lba_skip  = 0U;
[[maybe_unused]] static bool     s_fault_lba_armed = false;
...
[[maybe_unused]] RA8_INTERNAL static void internal_fault_reset(void)
{
  ...
  s_fault_read_lba  = 0U;
  s_fault_lba_skip  = 0U;
  s_fault_lba_armed = false;
}

/** @brief Arm a one-shot read fault on @p lba, after @p skip earlier visits. */
[[maybe_unused]] RA8_INTERNAL static void internal_fault_read_lba(uint64_t lba, uint32_t skip)
{
  s_fault_read_lba  = lba;
  s_fault_lba_skip  = skip;
  s_fault_lba_armed = true;
}

[[maybe_unused]] RA8_INTERNAL static ra8_err_t
internal_fault_read(void* ctx, uint64_t lba, uint32_t count, uint8_t* buf)
{
  if (s_fault_lba_armed && (lba == s_fault_read_lba)) {
    if (s_fault_lba_skip == 0U) {
      s_fault_lba_armed = false;
