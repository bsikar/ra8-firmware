 ...
 * @par MC/DC:
 * Decision: `if (in11[0] == k_dir_marker_kanji_e5)` in
 * `libs/ra8_fs/src/ra8_fs_fat_name.c@priv_83_to_str`, which restores a
 * 0x05-escaped leading byte back to the raw 0xE5 kanji byte.
 *
 * The former `j > 0 &&` term was redundant: 0x05 is not a space, so a true
 * marker comparison guarantees the base-copy loop incremented `j`. Removing
 * the co-dependent condition preserves behavior and leaves one testable
 * condition instead of cataloguing an avoidable MC/DC deactivation.
 * - V-true:  in11[0]=0x05 -> restored to 0xE5.
 * - V-false: in11[0]='F'  -> ordinary name remains unchanged. @brief Exercise the mcdc 83 to str kanji escape filesystem operation. @details Runs the mcdc 83 to str kanji escape vector through production filesystem seams and checks observable state. @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state. @since 0.1.0
 */
RA8_INTERNAL static void internal_test_mcdc_83_to_str_kanji_escape(void)
{
  TEST_BEGIN("ra8_fs name: priv_83_to_str kanji escape");
  ...
  TEST_END("ra8_fs name: priv_83_to_str kanji escape");
}
