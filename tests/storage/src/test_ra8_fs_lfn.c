 ...
 * (covered by tests/storage/src/test_ra8_fs_fat.c).
 *
...
static const uint16_t s_lfn_name_pad = 0xFFFFU;
 ...
 *
 * @note Not thread-safe; the fixture disk is file-scope state. @details Implements the bounded lfn write bpb fixture step using caller-owned state. @since 0.1.0
 */
RA8_INTERNAL static void internal_lfn_write_bpb(void)
{
  ...
  internal_put16(bpb, k_bpb_off_rsvd_sec_cnt, (uint16_t)k_reserved); /* reserved sectors */
  bpb[16] = (uint8_t)k_num_fats;
  internal_put16(bpb, k_bpb_off_root_ent_cnt, 16U);             /* root dir entries */
  internal_put16(bpb, k_bpb_off_tot_sec16, (uint16_t)k_blocks); /* total sectors    */
  internal_put16(bpb, k_bpb_off_fat_sz16, (uint16_t)k_sectors_fat);
 ...
 *
 * @note Not thread-safe; the fixture disk is file-scope state. @since 0.1.0
 */
RA8_INTERNAL static void internal_lfn_write_name_slot(uint8_t* root)
{
  ...
  lfn[k_lfn_off_chksum] = internal_sfn_checksum(s_alias83);
  internal_put16(lfn, k_dir_off_fst_clus_lo, 0U);
  ...
  for (uint32_t i = 0U; i < k_lfn_chars_per_entry; i++) {
    if (i < strlen(lname)) {
      internal_put16(lfn, (uint32_t)k_off[i], (uint16_t)(uint8_t)lname[i]);
    } else if (i == strlen(lname)) {
      internal_put16(lfn, (uint32_t)k_off[i], 0x0000U); /* name terminator */
    } else {
      internal_put16(lfn, (uint32_t)k_off[i], s_lfn_name_pad); /* padding */
    }
  }
}

/** @brief Build a FAT16 volume with the LFN chain + 8.3 entry for mybook.epub. @details Implements the bounded build volume with lfn fixture step using caller-owned state. @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state. @since 0.1.0 */
RA8_INTERNAL static void internal_build_volume_with_lfn(void)
{
  free(s_disk.bytes);
  ...
  internal_lfn_write_name_slot(root);
  ...
  internal_put16(sfn, k_dir_off_fst_clus_lo, 0U); /* first cluster low = 0 (empty file) */
  internal_put16(sfn, k_dir_off_fst_clus_hi, 0U); /* first cluster high                 */
 ...
 *      decisions are covered in tests/storage/src/test_ra8_fs_fat.c. @details Runs the lfn open by long and short vector through production filesystem seams and checks observable state. @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state. @since 0.1.0
 */
RA8_INTERNAL static void internal_test_lfn_open_by_long_and_short(void)
{
  TEST_BEGIN("ra8_fs LFN: open by long name + 8.3 alias");
  internal_build_volume_with_lfn();
 ...
 *
 * @par MC/DC: not applicable -- enumeration acceptance path. @details Runs the lfn listdir reports long name vector through production filesystem seams and checks observable state. @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state. @since 0.1.0
 */
RA8_INTERNAL static void internal_test_lfn_listdir_reports_long_name(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_listdir(mount, "/", internal_capture_cb, &cap));
