 ...
 *
 * `internal_stream_dump_image()` runs at the end of each scenario, so
...
#include "fs_exfat_stream_test_util.h"
 ...
 * @since 0.1.0 @details Implements the bounded fat entry fixture step using caller-owned state.
 */
RA8_INTERNAL static uint32_t internal_fat_entry(const ra8_fs_mount_t* h, uint32_t clus)
{
  return internal_disk_get_u32le(internal_fat_byte(h, clus));
}
 ...
 *       the test.
 * @since 0.1.0 @details Implements the bounded chain walk fixture step using caller-owned state.
 */
RA8_INTERNAL static uint32_t
internal_chain_walk(const ra8_fs_mount_t* h, uint32_t first, uint32_t* out, uint32_t cap)
{
  uint32_t clus = first;
  uint32_t n    = 0U;
  while (n < cap) {
    out[n] = clus;
    n++;
    const uint32_t next = internal_fat_entry(h, clus);
...
internal_expect_chain_over(const ra8_fs_mount_t* h, uint32_t first, uint32_t total, uint32_t taken)
{
 ...
 */
RA8_INTERNAL static void internal_test_stream_fragmentation_forces_chain(void)
{
  ...
  const uint32_t first = internal_disk_get_u32le(strm + (uint32_t)k_xs_off_strm_clus);
  ...
  static const uint8_t blocker[k_xs_sub_sector] = {};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_write_file(h, "BLOCK.BIN", blocker, (uint32_t)k_xs_sub_sector));
  const uint32_t blk_strm = internal_root_byte(h, (uint32_t)k_mut_root_strm0_idx + 3U);
  TEST_ASSERT_EQ(first + 1U, internal_disk_get_u32le(blk_strm + (uint32_t)k_xs_off_strm_clus));
  ...
  TEST_ASSERT_EQ(first, internal_disk_get_u32le(strm + (uint32_t)k_xs_off_strm_clus));

  internal_expect_chain_over(h, first, total, first + 1U);

  internal_stream_expect_contents(h, "FIRST.BIN", total, (uint8_t)k_xs_seed_a);
  ...
  internal_stream_dump_image("stream_fragmentation_forces_chain", h);
 ...
 * @since 0.1.0 @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state.
 */
RA8_INTERNAL static void internal_test_stream_chained_file_reads_and_frees(void)
{
  TEST_BEGIN("exfat stream: a chained file frees every cluster");
  internal_build_exfat_volume();
  ra8_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_mount(&s_backend, &h));
  const uint32_t empty_used = internal_alloc_bitmap_used(h);
  ...
  static const uint8_t blocker[k_xs_sub_sector] = {};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_write_file(h, "PIN.BIN", blocker, (uint32_t)k_xs_sub_sector));
  ...
  const uint32_t total = (uint32_t)k_xs_sub_sector + (uint32_t)k_xs_multi_cluster;
  internal_stream_expect_contents(h, "FRAG.BIN", total, (uint8_t)k_xs_seed_b);
  const uint32_t strm = internal_stream_strm0_off(h);
  TEST_ASSERT_EQ(k_xsc_flag_chained, s_disk.bytes[strm + (uint32_t)k_xs_off_strm_flags]);

  internal_stream_dump_image("stream_chained_before_unlink", h);

  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unlink(h, "FRAG.BIN"));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unlink(h, "PIN.BIN"));
  TEST_ASSERT_EQ(empty_used, internal_alloc_bitmap_used(h));

  internal_stream_dump_image("stream_chained_after_unlink", h);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  internal_free_volume();
 ...
 * `libs/ra8_fs/src/ra8_fs_fat_exfat_stream.c@internal_exfat_materialize_run`
 ...
 * @since 0.1.0 @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state.
 */
RA8_INTERNAL static void internal_test_stream_multi_cluster_run_materializes(void)
{
  ...
  TEST_ASSERT_EQ(k_xsc_flag_chained, s_disk.bytes[strm + (uint32_t)k_xs_off_strm_flags]);
  internal_expect_chain_over(h, first, total, first + owned);
  internal_stream_expect_contents(h, "RUN.BIN", total, (uint8_t)k_xs_seed_a);

  internal_stream_dump_image("stream_multi_cluster_run_materializes", h);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  internal_free_volume();
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_expect_patched_head(ra8_fs_mount_t* h, uint32_t total)
{
 ...
 * `libs/ra8_fs/src/ra8_fs_fat_exfat_stream.c@internal_exfat_cluster_at`
 * (1 condition). This case supplies the FALSE vector; every sequential write
 * supplies the TRUE one.
 *
 * @since 0.1.0 @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state.
 */
RA8_INTERNAL static void internal_test_stream_chained_backward_seek(void)
{
  ...
  internal_expect_patched_head(h, total);

  internal_stream_dump_image("stream_chained_backward_seek", h);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  internal_free_volume();
 ...
 * `libs/ra8_fs/src/ra8_fs_fat_exfat_write.c@internal_exfat_bitmap_window`
 ...
 * @since 0.1.0 @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state.
 */
RA8_INTERNAL static void internal_test_stream_crosses_bitmap_sector(void)
{
  ...
  TEST_ASSERT_EQ(empty_used + want, internal_alloc_bitmap_used(h));

  const uint32_t strm = internal_stream_strm0_off(h);
  TEST_ASSERT_EQ(k_xsc_bitmap_span, internal_disk_get_u32le(strm + (uint32_t)k_xs_off_strm_dlen));
  TEST_ASSERT_EQ(k_xsc_flag_contig, s_disk.bytes[strm + (uint32_t)k_xs_off_strm_flags]);
  ...
  static const uint32_t k_probe[] = {0U,
                                     (uint32_t)k_xsc_probe_before,
                                     (uint32_t)k_xsc_probe_after,
                                     (uint32_t)k_xsc_probe_last};
  for (uint32_t i = 0U; i < (uint32_t)(sizeof k_probe / sizeof k_probe[0]); i++) {
    uint8_t  one = 0U;
    uint32_t got = 0U;
    TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_seek(r, k_probe[i]));
    TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_read(r, &one, 1U, &got));
    TEST_ASSERT_EQ(1U, got);
    TEST_ASSERT_EQ(internal_stream_byte_at(k_probe[i], (uint8_t)k_xs_seed_a), one);
  }
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_close(r));

  internal_stream_dump_image("stream_crosses_bitmap_sector", h);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  internal_free_volume();
 ...
 * @since 0.1.0 @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state.
 */
RA8_INTERNAL static void internal_test_stream_volume_full(void)
{
  ...
  internal_stream_dump_image("stream_volume_full", h);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  internal_free_volume();
...
int main(void)
{
  ...
  internal_test_stream_multi_cluster_run_materializes();
  internal_test_stream_chained_backward_seek();
  internal_test_stream_crosses_bitmap_sector();
  internal_test_stream_volume_full();
  return 0;
}
