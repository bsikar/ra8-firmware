 ...
 * `internal_unicode_dump_image()` from the shared fixture, so `RA8_EXFAT_DUMP_DIR=<dir>`
 ...
 *
 * @note Not thread-safe.
 * @since 0.1.0 @details Implements the bounded disk rd32 fixture step using caller-owned state.
 */
RA8_INTERNAL static uint32_t internal_disk_rd32(uint32_t off)
{
  return internal_disk_rd16(off) | (internal_disk_rd16(off + 2U) << (uint32_t)k_mut_shift_byte16);
}
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_expand_volume_upcase(uint32_t base, uint32_t words)
{
  uint32_t idx = 0U;
  uint32_t w   = 0U;
  while ((w < words) && (idx < (uint32_t)k_ux_bmp_units)) {
    const uint32_t v = internal_disk_rd16(base + (w * 2U));
    w++;
    ...
    idx += internal_disk_rd16(base + (w * 2U));
    w++;
  }
}
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_load_volume_upcase(const ra8_fs_mount_t* h, uint32_t* out_csum)
{
  ...
  for (uint32_t i = 0U; i < (uint32_t)k_ux_scan_slots; i++) {
    const uint32_t off = internal_root_byte(h, i);
  ...
  *out_csum              = internal_disk_rd32(entry_off + (uint32_t)k_ux_upc_off_csum);
  const uint32_t cluster = internal_disk_rd32(entry_off + (uint32_t)k_mut_strm_off_clus);
  const uint32_t bytes   = internal_disk_rd32(entry_off + (uint32_t)k_mut_strm_off_dlen);
  internal_expand_volume_upcase(internal_cluster_byte(h, cluster), bytes / 2U);
}
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static uint32_t internal_expected_name_hash(const uint16_t* units, uint32_t n)
{
  uint32_t h = 0U;
 ...
 * @since 0.1.0 @details Implements the bounded seen cb fixture step using caller-owned state.
 */
RA8_INTERNAL static void internal_seen_cb(const char* name, uint8_t attr, uint64_t size, void* ctx)
{
  (void)attr;
  (void)size;
 ...
 *
 * @note Not thread-safe.
 * @since 0.1.0 @details Implements the bounded listing holds fixture step using caller-owned state.
 */
RA8_INTERNAL static uint8_t internal_listing_holds(ra8_fs_mount_t* h, const char* want)
{
  const char* probe = want;
  if (ra8_fs_listdir(h, "/", internal_seen_cb, (void*)&probe) != k_ra8_ok) {
    return 0U;
  }
 ...
 * `tests/storage/src/test_ra8_fs_utf.c` isolates, wired to the exFAT paths.
 *
 ...
 */
RA8_INTERNAL static void internal_test_exfat_non_ascii_round_trip(void)
{
  ...
  for (uint32_t i = 0U; i < (uint32_t)(sizeof(names) / sizeof(names[0])); i++) {
    ...
    internal_unicode_dump_image(tags[i], h->partition_base_lba);
    ...
    TEST_ASSERT_EQ(1U, internal_listing_holds(h, renamed));
    TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unlink(h, renamed));
    TEST_ASSERT_EQ(0U, internal_listing_holds(h, renamed));
  }
 ...
 * `libs/ra8_fs/src/ra8_fs_fat_exfat_write.c@internal_exfat_build_set` (1 cond).
 ...
 * @since 0.1.0 @pre Pointer arguments address their documented readable or writable extents. @post No access exceeds a caller-advertised capacity. @note Test-only helpers retain no hidden ownership beyond documented fixture state.
 */
RA8_INTERNAL static void internal_test_exfat_name_entries_hold_real_units(void)
{
  ...
  const uint32_t name = internal_root_byte(h, (uint32_t)k_mut_root_name0_idx);
  ...
  for (uint32_t i = 0U; i < want; i++) {
    TEST_ASSERT_EQ(s_acc_units[i],
                   internal_disk_rd16(name + (uint32_t)k_ux_name_off + (i * 2U))); /* V1 */
  }
  /* The slots past the name stay zero, which is what a reader uses to stop. */
  TEST_ASSERT_EQ(0U, internal_disk_rd16(name + (uint32_t)k_ux_name_off + (want * 2U))); /* V2 */

  internal_unicode_dump_image("unicode_name_units", h->partition_base_lba);
 ...
 * @since 0.1.0 @pre Pointer arguments address their documented readable or writable extents. @post No access exceeds a caller-advertised capacity. @note Test-only helpers retain no hidden ownership beyond documented fixture state.
 */
RA8_INTERNAL static void internal_test_exfat_name_hash_matches_an_independent_computation(void)
{
  ...
  internal_load_volume_upcase(h, &stored_csum);
  ...
  for (uint32_t i = 0U; i < (uint32_t)(sizeof(names) / sizeof(names[0])); i++) {
    ...
    TEST_ASSERT_EQ(internal_expected_name_hash(units[i], counts[i]),
                   internal_disk_rd16(strm + (uint32_t)k_ux_strm_off_hsh));
    TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  }
  ...
  static const uint16_t plain_units[] = {'P', 'L', 'A', 'I', 'N', '.', 'T', 'X', 'T'};
  const uint32_t        strm          = internal_root_byte(h, (uint32_t)k_mut_root_strm0_idx);
  TEST_ASSERT_EQ(internal_expected_name_hash(plain_units, 9U),
                 internal_disk_rd16(strm + (uint32_t)k_ux_strm_off_hsh));
 ...
 * @since 0.1.0 @pre Pointer arguments address their documented readable or writable extents. @post No access exceeds a caller-advertised capacity. @note Test-only helpers retain no hidden ownership beyond documented fixture state.
 */
RA8_INTERNAL static void internal_test_upcase_table_is_the_canonical_one(void)
{
...
RA8_INTERNAL static void internal_test_exfat_mixed_case_lookup(void)
{
  ...
  TEST_ASSERT_EQ(1U, internal_listing_holds(h, s_acc_u8));

  internal_unicode_dump_image("unicode_mixed_case", h->partition_base_lba);
 ...
 * @since 0.1.0 @pre Pointer arguments address their documented readable or writable extents. @post No access exceeds a caller-advertised capacity. @note Test-only helpers retain no hidden ownership beyond documented fixture state.
 */
RA8_INTERNAL static void internal_test_foreign_upcase_table_refuses_non_ascii(void)
{
  ...
  ra8_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_mount(&s_backend, &h));
  const uint32_t upc = internal_root_byte(h, (uint32_t)k_mut_root_upcase_idx);
  ...
  TEST_ASSERT_EQ(1U, internal_listing_holds(h, "PLAIN.TXT"));
...
int main(void)
{
  internal_test_exfat_non_ascii_round_trip();
  internal_test_exfat_name_entries_hold_real_units();
  internal_test_exfat_name_hash_matches_an_independent_computation();
  internal_test_upcase_table_is_the_canonical_one();
  internal_test_exfat_mixed_case_lookup();
  internal_test_foreign_upcase_table_refuses_non_ascii();
  return 0;
}
