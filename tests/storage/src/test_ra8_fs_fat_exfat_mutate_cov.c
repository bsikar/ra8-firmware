/**
 * @file test_ra8_fs_fat_exfat_mutate_cov.c
...
/**
 * @test internal_test_exfat_fat_chain_cycle_is_corrupt
 * @brief A FAT chain with no EOC reports corruption when its bound expires.
 * @par MC/DC:
 * `priv_is_eoc(m, next) != 0U` is false for the self-link in this test and
 * true for the two-cluster control above. The bounded loop then exhausts
 * exactly once here and returns ::k_ra8_err_protocol_error.
 * @pre A mounted exFAT volume owns one allocated file cluster.
 * @post The corrupt chain is not reported as successfully released.
 * @note The fixture temporarily narrows the declared cluster count to reach
 *       the same bound a full-volume cycle reaches without thousands of writes.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_test_exfat_fat_chain_cycle_is_corrupt(void)
{
  TEST_BEGIN("exfat mutate cov: cyclic FAT chain reports protocol error");
  internal_build_exfat_volume();
  ra8_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_mount(&s_backend, &h));
  ...
  const uint32_t first_clus = internal_disk_get_u32le(strm_off + (uint32_t)k_mut_strm_off_clus);
  s_disk.bytes[strm_off + (uint32_t)k_mut_strm_off_flags] &= (uint8_t)~(uint8_t)k_mut_no_fat_bit;
  internal_disk_set_u32le(internal_fat_byte(h, first_clus), first_clus);

  const uint32_t saved_count = h->count_of_clusters;
  h->count_of_clusters       = 1U;
  TEST_ASSERT_EQ(k_ra8_err_protocol_error, ra8_fs_unlink(h, "A.TXT"));
  h->count_of_clusters = saved_count;

  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  internal_free_volume();
  TEST_END("exfat mutate cov: cyclic FAT chain reports protocol error");
}
...
int main(void)
{
  ...
  internal_test_exfat_fat_chain_cycle_is_corrupt();
