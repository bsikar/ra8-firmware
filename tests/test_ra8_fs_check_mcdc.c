 ...
 * - corrupted entry  -> C1 true,  C2 true        (first out of range). @brief Exercise the mcdc fat entry filesystem operation. @details Runs the mcdc fat entry vector through production filesystem seams and checks observable state. @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state. @since 0.1.0
 */
RA8_INTERNAL static void internal_test_mcdc_fat_entry(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 internal_chk_run(h, &r)); /* file: A C1=true C2=false, B C1=true C2=false */
  TEST_ASSERT_EQ(0U, r.faults_total);
  internal_chk_teardown(h);
  h                 = internal_chk_setup((uint32_t)k_fmt_blocks_fat16,
                                         k_ra8_fs_type_fat16,
                                         "A.BIN",
                                         (uint32_t)k_chk_payload_mc);
  const uint32_t eo = internal_fat_root_entry(h, "A       BIN");
  internal_disk_wr16(eo + (uint32_t)k_chk_off_fst_lo,
                     (uint16_t)k_chk_huge_lo); /* B C1=true C2=true */
 ...
 * - subdirectory     -> C1 false                 (never the fixed-root path). @brief Exercise the mcdc fat tree filesystem operation. @details Runs the mcdc fat tree vector through production filesystem seams and checks observable state. @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state. @since 0.1.0
 */
RA8_INTERNAL static void internal_test_mcdc_fat_tree(void)
{
  ...
  ra8_fs_check_report_t r = {};
  TEST_ASSERT_EQ(k_ra8_ok, internal_chk_run(h, &r)); /* FAT16 root: C1 true C2 true */
  TEST_ASSERT_EQ(0U, r.faults_total);
  internal_chk_teardown(h);
  h = internal_chk_setup((uint32_t)k_fmt_blocks_fat32,
                         k_ra8_fs_type_fat32,
                         "B.BIN",
                         (uint32_t)k_chk_payload_mc);
  r = (ra8_fs_check_report_t){};
  TEST_ASSERT_EQ(k_ra8_ok, internal_chk_run(h, &r)); /* FAT32 root: C1 true C2 false */
  TEST_ASSERT_EQ(0U, r.faults_total);
  internal_chk_teardown(h);
 ...
 * - used cluster     -> C1 false, C2 false       (a live cluster). @brief Exercise the mcdc fat diff filesystem operation. @details Runs the mcdc fat diff vector through production filesystem seams and checks observable state. @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state. @since 0.1.0
 */
RA8_INTERNAL static void internal_test_mcdc_fat_diff(void)
{
  ...
  ra8_fs_mount_t* h = internal_chk_setup((uint32_t)k_fmt_blocks_fat16,
                                         ...
                                         "A.BIN",
                                         (uint32_t)k_chk_payload_mc);
  internal_disk_wr16(internal_fat16_off(h, h->count_of_clusters),
                     (uint16_t)k_chk_defective); /* C1 false C2 true */
  ra8_fs_check_report_t r = {};
  TEST_ASSERT_EQ(k_ra8_ok, internal_chk_run(h, &r)); /* free: C1 true; used: C1 false C2 false */
 ...
 * - unknown sentinel -> C1 false                 (0xFFFFFFFF, not compared). @brief Exercise the mcdc fat fsinfo filesystem operation. @details Runs the mcdc fat fsinfo vector through production filesystem seams and checks observable state. @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state. @since 0.1.0
 */
RA8_INTERNAL static void internal_test_mcdc_fat_fsinfo(void)
{
  TEST_BEGIN("MC/DC priv_fat_fsinfo: FSInfo trust and free-count compare");
  const uint32_t  v = (uint32_t)k_fmt_blocks_fat32;
  ra8_fs_mount_t* h =
    internal_chk_setup(v, k_ra8_fs_type_fat32, "B.BIN", (uint32_t)k_chk_payload_mc);
  ra8_fs_check_report_t r = {};
  TEST_ASSERT_EQ(k_ra8_ok, internal_chk_run(h, &r)); /* A C1=false C2=false; B C1=true C2=false */
  TEST_ASSERT_EQ(0U, r.faults_total);
  internal_chk_teardown(h);
  h = internal_chk_setup(v, k_ra8_fs_type_fat32, "B.BIN", (uint32_t)k_chk_payload_mc);
  ...
  internal_disk_wr32(foff + (uint32_t)k_chk_fsi_off_free,
                     internal_disk_rd32(foff + (uint32_t)k_chk_fsi_off_free) +
                       1U); /* B C1=true C2=true */
  r = (ra8_fs_check_report_t){};
  TEST_ASSERT_EQ(k_ra8_ok, internal_chk_run(h, &r));
  TEST_ASSERT_EQ(k_ra8_fs_check_fault_free_count_bad, r.first_fault.kind);
  internal_chk_teardown(h);
  h = internal_chk_setup(v, k_ra8_fs_type_fat32, "B.BIN", (uint32_t)k_chk_payload_mc);
  internal_disk_wr16(h->partition_base_lba * (uint32_t)k_fmt_block_size +
                       (uint32_t)k_chk_fsi_off_num,
                     0U); /* A C1=true */
  r = (ra8_fs_check_report_t){};
  TEST_ASSERT_EQ(k_ra8_ok, internal_chk_run(h, &r));
  TEST_ASSERT_EQ(0U, r.faults_total);
  ...
  const uint32_t fsi2  = (uint32_t)internal_disk_rd16(boot2 + (uint32_t)k_chk_fsi_off_num);
  const uint32_t foff2 = (h->partition_base_lba + fsi2) * (uint32_t)k_fmt_block_size;
  internal_disk_wr32(foff2 + (uint32_t)k_chk_fsi_off_free,
                     (uint32_t)k_chk_fsi_unknown); /* B C1=false */
  r = (ra8_fs_check_report_t){};
  TEST_ASSERT_EQ(k_ra8_ok, internal_chk_run(h, &r));
  TEST_ASSERT_EQ(0U, r.faults_total);
  internal_chk_teardown(h);
 ...
 * Also covers the not-in-use guard (an unmounted handle -> invalid_state). @brief Exercise the mcdc check locked filesystem operation. @details Runs the mcdc check locked vector through production filesystem seams and checks observable state. @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state. @since 0.1.0
 */
RA8_INTERNAL
 ...
 * - a valid set        -> C1 false, C2 false   (a normal File entry set). @brief Exercise the mcdc exchk set filesystem operation. @details Runs the mcdc exchk set vector through production filesystem seams and checks observable state. @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state. @since 0.1.0
 */
RA8_INTERNAL static void internal_test_mcdc_exchk_set(void)
{
  TEST_BEGIN("MC/DC priv_exchk_set: SecondaryCount bounds");
  const uint32_t  v = (uint32_t)k_fmt_blocks_exfat;
  ra8_fs_mount_t* h =
    internal_chk_setup(v, k_ra8_fs_type_exfat, "T.BIN", (uint32_t)k_chk_payload_ex);
  ra8_fs_check_report_t r = {};
  TEST_ASSERT_EQ(k_ra8_ok, internal_chk_run(h, &r)); /* valid set: C1 false C2 false */
  TEST_ASSERT_EQ(0U, r.faults_total);
  internal_chk_teardown(h);
  h           = internal_chk_setup(v, k_ra8_fs_type_exfat, "T.BIN", (uint32_t)k_chk_payload_ex);
  uint32_t fe = internal_exfat_first_file(h);
  ...
  internal_chk_teardown(h);
  h  = internal_chk_setup(v, k_ra8_fs_type_exfat, "T.BIN", (uint32_t)k_chk_payload_ex);
  fe = internal_exfat_first_file(h);
 ...
 * (0x85) -> C1 false C2 false. One clean walk exercises every vector. @brief Exercise the mcdc exchk scan dir filesystem operation. @details Runs the mcdc exchk scan dir vector through production filesystem seams and checks observable state. @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state. @since 0.1.0
 */
RA8_INTERNAL
 ...
 * - live cluster      -> C1 false                 (allocated). @brief Exercise the mcdc exchk diff byte filesystem operation. @details Runs the mcdc exchk diff byte vector through production filesystem seams and checks observable state. @pre Pointer arguments address their documented readable or writable extents. @pre Required fixture and backend state is initialized before the call. @post No access exceeds a caller-advertised capacity. @post The return value or assertions describe the observed filesystem state. @note Test-only helpers retain no hidden ownership beyond documented fixture state. @since 0.1.0
 */
RA8_INTERNAL static void internal_test_mcdc_exchk_diff_byte(void)
{
  ...
  ra8_fs_mount_t* h =
    internal_chk_setup(v, k_ra8_fs_type_exfat, "T.BIN", (uint32_t)k_chk_payload_ex);
  ra8_fs_check_report_t r = {};
  TEST_ASSERT_EQ(k_ra8_ok, internal_chk_run(h, &r)); /* A T,F and F; B T,F and F */
  TEST_ASSERT_EQ(0U, r.faults_total);
  internal_chk_teardown(h);
  h = internal_chk_setup(v, k_ra8_fs_type_exfat, "T.BIN", (uint32_t)k_chk_payload_ex);
  internal_exfat_set_spare_bit(h); /* A C1=true C2=true: allocated, referenced by nothing */
  r = (ra8_fs_check_report_t){};
  TEST_ASSERT_EQ(k_ra8_ok, internal_chk_run(h, &r));
  ...
  internal_exfat_clear_ref_bit(h, first); /* B C1=true C2=true: referenced, bitmap bit clear */
  r = (ra8_fs_check_report_t){};
  TEST_ASSERT_EQ(k_ra8_ok, internal_chk_run(h, &r));
...
int32_t main(void)
{
  ...
  internal_test_mcdc_fat_diff();
  internal_test_mcdc_fat_fsinfo();
  internal_test_mcdc_check_locked();
  internal_test_mcdc_exchk_set();
  internal_test_mcdc_exchk_scan_dir();
  internal_test_mcdc_exchk_diff_byte();
  return 0;
}
