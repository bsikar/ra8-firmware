/**
 * @file test_ra_io_spi_bus.c
 * @brief Unit tests for the ra_io SPI-bus facade
 *        (libs/ra_io/src/ra_io_spi_bus*.c).
 *
 * @details
 * Binds both backends of the facade -- SPI_B (`ra_io_spi_bus_bind_spi_b`)
 * and SCI Simple-SPI (`ra_io_spi_bus_bind_sci_spi`) -- against the
 * ``ra_sim_mmap`` register substrate and drives the vtable end to end:
 *
 ...
 *    `ra_spi_*` / `ra_sci_spi_*` drivers, observed through the same
 ...
 *  - the `ra_io_spi_bus_as_ops` bridge into the Ring-3 seam, including
...
/**
 * @enum ra_io_spi_bus_test_const_t
...
static void bring_up_spi_b(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_spi_init((uint8_t)k_test_spi_b_ch, &cfg));
  ra_spi((uint8_t)k_test_spi_b_ch)->SPSR =
    (uint32_t)k_ra_spsr_mask_sptef | (uint32_t)k_ra_spsr_mask_sprf;
}
...
static void bring_up_sci_spi(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_init((uint8_t)k_test_sci_ch, &cfg));
...
{
  TEST_BEGIN("ra_io_spi_bus: NULL handle + never-bound handle rejected");
  prep();

  uint8_t          rx  = 0U;
  ra_spi_bus_ops_t ops = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_spi_bus_xfer8(nullptr, (uint8_t)k_test_tx_byte, &rx));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_io_spi_bus_write_read(nullptr, &rx, &rx, 1U, k_ra_spi_width_8));
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_io_spi_bus_set_clock(nullptr, (uint32_t)k_test_baud_hz, (uint32_t)k_test_pclk_hz));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_spi_bus_as_ops(nullptr, &ops));

  const ra_io_spi_bus_t unbound = {};
  TEST_ASSERT_EQ(k_ra_err_not_initialized,
                 ra_io_spi_bus_xfer8(&unbound, (uint8_t)k_test_tx_byte, &rx));
  TEST_ASSERT_EQ(k_ra_err_not_initialized,
                 ra_io_spi_bus_write_read(&unbound, &rx, &rx, 1U, k_ra_spi_width_8));
  TEST_ASSERT_EQ(
    k_ra_err_not_initialized,
    ra_io_spi_bus_set_clock(&unbound, (uint32_t)k_test_baud_hz, (uint32_t)k_test_pclk_hz));
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_io_spi_bus_as_ops(&unbound, &ops));
  TEST_END("ra_io_spi_bus: NULL handle + never-bound handle rejected");
}
...
{
  TEST_BEGIN("ra_io_spi_bus: binder NULL / channel-range rejection");
  prep();

  ra_io_spi_bus_t bus = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_spi_bus_bind_spi_b(nullptr, (uint8_t)k_test_spi_b_ch));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_spi_bus_bind_sci_spi(nullptr, (uint8_t)k_test_sci_ch));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_io_spi_bus_bind_spi_b(&bus, (uint8_t)k_test_spi_b_ch_oor));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_io_spi_bus_bind_sci_spi(&bus, (uint8_t)k_test_sci_ch_oor));
  /* A failed bind leaves the handle unbound. */
  uint8_t rx = 0U;
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_io_spi_bus_xfer8(&bus, (uint8_t)k_test_tx_byte, &rx));
  /* Good binds are accepted for both backends. */
  TEST_ASSERT_EQ(k_ra_ok, ra_io_spi_bus_bind_spi_b(&bus, (uint8_t)k_test_spi_b_ch));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_spi_bus_bind_sci_spi(&bus, (uint8_t)k_test_sci_ch));
  TEST_END("ra_io_spi_bus: binder NULL / channel-range rejection");
}
 ...
 * decisions carry their MC/DC vectors in test_ra_spi_b.c)
 */
static void test_spi_b_forwarding(void)
{
  TEST_BEGIN("ra_io_spi_bus: SPI_B xfer8 / write_read / set_clock forward");
  prep();
  bring_up_spi_b();
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_io_spi_bus_write_read(&bus, nullptr, nullptr, 1U, k_ra_spi_width_8));

  /* set_clock: clear SPCR3 so the reprogramme is observable. */
  ra_spi((uint8_t)k_test_spi_b_ch)->SPCR3 = 0U;
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_io_spi_bus_set_clock(&bus, (uint32_t)k_test_baud_hz, (uint32_t)k_test_pclk_hz));
  TEST_ASSERT(((ra_spi((uint8_t)k_test_spi_b_ch)->SPCR3) & k_ra_spcr3_mask_spbr) != 0U);

  TEST_ASSERT_EQ(k_ra_ok, ra_spi_deinit((uint8_t)k_test_spi_b_ch));
  TEST_END("ra_io_spi_bus: SPI_B xfer8 / write_read / set_clock forward");
}
...
{
  TEST_BEGIN("ra_io_spi_bus: SCI xfer8 / write_read / set_clock forward");
  prep();
  bring_up_sci_spi();
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_spi_deinit((uint8_t)k_test_sci_ch));
  TEST_END("ra_io_spi_bus: SCI xfer8 / write_read / set_clock forward");
}
 ...
 *
 * @par MC/DC:
 * Decision: ``if (width != k_ra_spi_width_8)`` in the SCI backend's
 * ``write_read`` row (1 condition). N+1 = 2 vectors:
 * - V1: width=16 -> true  -> k_ra_err_not_supported (no transfer starts)
 * - V2: width=8  -> false -> forwards to ra_sci_spi_xfer and succeeds
 */
static void test_sci_spi_width_guard(void)
{
  TEST_BEGIN("ra_io_spi_bus: SCI backend rejects non-8-bit widths");
  prep();
  bring_up_sci_spi();
  ...
  TEST_ASSERT_EQ(k_ra_err_not_supported,
                 ra_io_spi_bus_write_read(&bus, buf, buf, 1U, k_ra_spi_width_16));
  TEST_ASSERT_EQ(k_ra_err_not_supported,
                 ra_io_spi_bus_write_read(&bus, buf, buf, 1U, k_ra_spi_width_32));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_spi_bus_write_read(&bus, buf, buf, 1U, k_ra_spi_width_8));
  TEST_END("ra_io_spi_bus: SCI backend rejects non-8-bit widths");
}
...
{
  TEST_BEGIN("ra_io_spi_bus_as_ops: seam trampoline forwards + guards");
  prep();
  bring_up_spi_b();
  ...
  ra_spi_bus_ops_t ops = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_spi_bus_as_ops(&bus, nullptr));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_spi_bus_as_ops(&bus, &ops));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_spi_deinit((uint8_t)k_test_spi_b_ch));
  TEST_END("ra_io_spi_bus_as_ops: seam trampoline forwards + guards");
}
...
int32_t main(void)
{
  ...
  test_as_ops_bridge();
  (void)fprintf(stderr, "[OK  ] test_ra_io_spi_bus.c\n");
  return 0;
}
