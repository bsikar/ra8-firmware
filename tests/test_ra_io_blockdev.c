/**
 * @file test_ra_io_blockdev.c
 * @brief Unit tests for the ra_io block-device fabric (Phase 1, issue #156).
 *
 * @details
 ...
 * `ra_fs_backend_t` bridge by formatting + mounting a real FAT volume on a
...
static uint8_t s_disk[(size_t)k_test_disk_blocks * (size_t)k_ra_io_block_size_bytes];

/** @brief One-block scratch for read/write round-trips. */
static uint8_t s_small[(size_t)k_test_small_blocks * (size_t)k_ra_io_block_size_bytes];
...
static void test_ram_init_validation(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_io_blockdev_ram_init(nullptr, &state, s_small, k_test_small_blocks, false));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_io_blockdev_ram_init(&bd, nullptr, s_small, k_test_small_blocks, false));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_io_blockdev_ram_init(&bd, &state, nullptr, k_test_small_blocks, false));
  TEST_ASSERT_EQ(k_ra_err_invalid_size, ra_io_blockdev_ram_init(&bd, &state, s_small, 0, false));
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_io_blockdev_ram_init(&bd, &state, s_small, k_test_small_blocks, false));
...
static void test_ram_read_write_roundtrip(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_write(&bd, 2, 1, out));

  uint8_t in[(size_t)k_ra_io_block_size_bytes];
  (void)memset(in, 0, sizeof(in));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_read(&bd, 2, 1, in));
...
static void test_ram_bounds(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_io_blockdev_ram_init(&bd, &state, s_small, k_test_small_blocks, false));
  uint8_t buf[(size_t)k_ra_io_block_size_bytes] = {};
  /* count exceeds capacity */
  TEST_ASSERT_EQ(k_ra_err_out_of_range, ra_io_blockdev_read(&bd, 0, k_test_small_blocks + 1u, buf));
  /* lba+count walks off the end */
  TEST_ASSERT_EQ(k_ra_err_out_of_range, ra_io_blockdev_write(&bd, k_test_small_blocks, 1, buf));
  TEST_END("ram bounds");
}
...
static void test_ram_erase(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_io_blockdev_ram_init(&bd, &state, s_small, k_test_small_blocks, false));
  uint8_t ones[(size_t)k_ra_io_block_size_bytes];
  (void)memset(ones, 0xFF, sizeof(ones));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_write(&bd, 1, 1, ones));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_erase(&bd, 1, 1));

  uint8_t in[(size_t)k_ra_io_block_size_bytes];
  (void)memset(in, 0xAA, sizeof(in));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_read(&bd, 1, 1, in));
  uint32_t nonzero = 0;
  for (uint32_t i = 0; i < (uint32_t)k_ra_io_block_size_bytes; ++i) {
    if (in[i] != 0u) {
      nonzero++;
...
static void test_ram_read_only(void)
{
  ...
  ra_io_blockdev_t           bd    = {};
  ra_io_blockdev_ram_state_t state = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_ram_init(&bd, &state, s_small, k_test_small_blocks, true));
  uint8_t buf[(size_t)k_ra_io_block_size_bytes] = {};
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_io_blockdev_write(&bd, 0, 1, buf));
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_io_blockdev_erase(&bd, 0, 1));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_read(&bd, 0, 1, buf));
  ra_io_blockdev_caps_t caps = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_get_caps(&bd, &caps));
  TEST_ASSERT(caps.read_only);
...
static void test_unbound_handle(void)
{
  ...
  ra_io_blockdev_t      bd                                    = {}; /* iface == nullptr */
  uint8_t               buf[(size_t)k_ra_io_block_size_bytes] = {};
  ra_io_blockdev_caps_t caps                                  = {};
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_io_blockdev_read(&bd, 0, 1, buf));
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_io_blockdev_write(&bd, 0, 1, buf));
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_io_blockdev_get_caps(&bd, &caps));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_blockdev_read(nullptr, 0, 1, buf));
...
static ra_err_t fk_read(void* c, uint32_t l, uint32_t n, uint8_t* b)
{
  (void)c;
  (void)l;
  (void)n;
...
static ra_err_t fk_write(void* c, uint32_t l, uint32_t n, const uint8_t* b)
{
  (void)c;
  (void)l;
  (void)n;
...
static ra_err_t fk_erase(void* c, uint32_t l, uint32_t n)
{
  (void)c;
  (void)l;
  (void)n;
...
static ra_err_t fk_caps_ones(const void* c, ra_io_blockdev_caps_t* out)
{
  (void)c;
  *out                     = (ra_io_blockdev_caps_t){};
...
static const ra_io_blockdev_iface_t k_fk_iface_no_erase = {
  .read     = fk_read,
  .write    = fk_write,
  .erase    = nullptr,
...
static const ra_io_blockdev_iface_t k_fk_iface_ones = {
  .read     = fk_read,
  .write    = fk_write,
  .erase    = fk_erase,
  .get_caps = fk_caps_ones,
...
static void test_dispatch_optional_callbacks(void)
{
  ...
  ra_io_blockdev_t bd = {.iface = &k_fk_iface_no_erase, .ctx = nullptr};
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_io_blockdev_erase(&bd, 0, 1));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_sync(&bd));
...
static void test_fs_bridge_erase_value_gate(void)
{
  ...
  ra_io_blockdev_t bd = {.iface = &k_fk_iface_ones, .ctx = nullptr};
  ra_fs_backend_t  be = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_as_fs_backend(&bd, &be));
  TEST_ASSERT_NOT_NULL(be.erase_blocks);
  /* all-ones medium: erase trampoline declines so ra_fs zero-writes */
  TEST_ASSERT_EQ(k_ra_err_not_supported, be.erase_blocks(be.ctx, 0, 1));
...
static void test_fs_bridge_fat_roundtrip(void)
{
  ...
  ra_io_blockdev_t           bd    = {};
  ra_io_blockdev_ram_state_t state = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_ram_init(&bd, &state, s_disk, k_test_disk_blocks, false));
  ra_fs_backend_t be = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_as_fs_backend(&bd, &be));
  ...
  opts.label               = "RAIO";
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_format(&be, &opts));

  ra_fs_mount_t* mnt = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&be, &mnt));
  TEST_ASSERT_EQ(k_ra_fs_type_fat16, mnt->type);

  uint8_t payload[(size_t)k_test_payload_len];
  for (uint32_t i = 0; i < (uint32_t)k_test_payload_len; ++i) {
    payload[i] = (uint8_t)((i * 7u + 3u) & 0xFFu);
  }
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_write_file(mnt, "HELLO.BIN", payload, k_test_payload_len));

  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(mnt, "HELLO.BIN", k_ra_fs_mode_read, &f));
  uint8_t  got[(size_t)k_test_payload_len] = {};
  uint32_t got_len                         = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_read(f, got, k_test_payload_len, &got_len));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(mnt));
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_io_blockdev.c\n");
  return 0;
}
