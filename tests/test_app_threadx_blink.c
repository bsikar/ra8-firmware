 ...
 *
 * @par MC/DC:
 * Decision under test: ``ra_board_led_init() != k_ra_ok`` evaluated for
 ...
 * inside its while(1): ra_board_led_toggle. We loop k_test_blink_iters
 * to confirm the BSP is re-entrant call-after-call (post-init).
 *
 * @par MC/DC:
 * Decision under test: ``ra_board_led_toggle(led) != k_ra_ok`` inside
 ...
 *
 * @par MC/DC:
 * Decision under test: ``ra_board_led_toggle(LED2) != k_ra_ok``.
 ...
 * The app's ra_board_led_init returns k_ra_err_invalid_arg for this,
 * matching the panic_halt branch in main().
 *
 * @par MC/DC:
 * Decision under test: ``ra_board_led_init() != k_ra_ok`` -- second
 ...
 * ra_board_led_toggle would break out (mirrors the app's `break` on
 * non-ok return).
 *
 * @par MC/DC:
 * Decision under test: ``ra_board_led_toggle() != k_ra_ok`` -- failure
