/**
 * @file test_ra_io_stream_blockdev_cov.c
 * @brief Coverage-boost tests for ra_io_stream_blockdev.c.
 *
 * @details
 * Targets the eight source lines left uncovered after test_ra_io_stream.c:
 *
 *   73  -- bdsink_commit_sector: error return when ra_io_blockdev_write fails.
 ...
 * internal vtable (ra_io_blockdev_internal.h). The mock write callback returns
 * k_ra_err_hw_error once the write-call index reaches the configured threshold,
 ...
 *
 * @details
 * The mock write callback counts calls and returns k_ra_err_hw_error for
 ...
 * @retval k_ra_ok Read completed successfully.
 *
 ...
 * @since 0.1.0
 */
static ra_err_t mock_fault_read(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf)
{
  (void)ctx;
  (void)lba;
  (void)memset(buf, 0, (size_t)count * (size_t)k_ra_io_block_size_bytes);
  return k_ra_ok;
}
 ...
 * writes_until_fail` the call returns k_ra_err_hw_error without modifying
 ...
 * @retval k_ra_ok           Call index is within the success budget.
 * @retval k_ra_err_hw_error Call index reached the configured fail threshold.
 *
 ...
 * @post On failure write_count is unchanged and k_ra_err_hw_error is returned.
 *
 * @note Not thread-safe (single-threaded test context).
 * @since 0.1.0
 */
static ra_err_t mock_fault_write(void* ctx, uint32_t lba, uint32_t count, const uint8_t* buf)
{
  (void)lba;
  (void)count;
  (void)buf;
 ...
 *
 * @return ra_err_t Always k_ra_ok.
 * @retval k_ra_ok Capabilities populated successfully.
 *
 * @pre out is writable.
 * @pre ctx is not inspected.
 * @post out->logical_block_bytes equals k_ra_io_block_size_bytes.
 * @post out->read_only is false.
 *
 * @note Not thread-safe (single-threaded test context).
 * @since 0.1.0
 */
static ra_err_t mock_fault_get_caps(const void* ctx, ra_io_blockdev_caps_t* out)
{
  (void)ctx;
  out->block_count             = 16U;
  ...
  out->erase_value             = (uint8_t)k_ra_io_erase_value_zero;
...
static const ra_io_blockdev_iface_t k_mock_fault_iface = {
 ...
 * includes ra_io_blockdev_internal.h for MC/DC vector access to error paths.
 *
 ...
 */
static void
fixture_fault_init(ra_io_blockdev_t* bd, mock_fault_state_t* st, uint32_t writes_until_fail)
{
...
/**
 * @brief bdsink_flush returns k_ra_ok immediately when fill == 0 (line 227).
 *
 * @details
 * Binds a block-device sink to a small RAM disk and calls ra_io_stream_flush
 ...
 * - Vector 1 (this test): fill = 0 -> true -> k_ra_ok, line 227 executed.
 * - Vector 2 (test_blockdev_sink in test_ra_io_stream.c): fill = 88 -> false
...
static void test_bdsink_flush_no_pending_bytes(void)
{
  ...
  static uint8_t             disk[(size_t)k_cov_bd_ram_blocks * (size_t)k_ra_io_block_size_bytes];
  ra_io_blockdev_ram_state_t bstate = {};
  ra_io_blockdev_t           bd     = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_ram_init(&bd, &bstate, disk, k_cov_bd_ram_blocks, false));
  ra_io_stream_blockdev_state_t sstate = {};
  ra_io_stream_t                s      = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_blockdev_init(&s, &sstate, &bd, k_cov_bd_start_lba));
  /* flush with nothing written: fill == 0 hits line 227 */
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_flush(&s));
 ...
 * `*out_written = len` (line 194) before returning k_ra_ok. The test asserts
 ...
 * - Vector 2 (test_blockdev_sink in test_ra_io_stream.c): nullptr -> false ->
 *   assignment skipped.
 * Minimal MC/DC: N+1 = 2 vectors for N = 1 condition.
 *
 * @since 0.1.0
 ...
 * bdsink_commit_sector, which calls ra_io_blockdev_write. The mock returns
 * k_ra_err_hw_error; bdsink_commit_sector propagates it at line 73. Back in
 * bdsink_write, e != k_ra_ok (line 186 true), out_written != nullptr (line 187
 * true), *out_written = done = 512 (line 188), return e (line 190).
 *
 * @par MC/DC:
 * Decision: `e != k_ra_ok` in bdsink_write loop body (line 186, single condition)
 * - Vector 1 (this test): e = k_ra_err_hw_error -> true -> error return path.
 * - Vector 2 (test_blockdev_sink in test_ra_io_stream.c): e = k_ra_ok -> false
...
static void test_bdsink_write_error_propagates_no_out_written(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_io_stream_blockdev_init(&s, &sstate, &bd, k_cov_bd_start_lba));
  uint8_t        payload[(size_t)k_cov_bd_one_sector] = {};
  const ra_err_t e = ra_io_stream_write(&s, payload, k_cov_bd_one_sector, nullptr);
  TEST_ASSERT(e != k_ra_ok);
 ...
 *
 * @pre RA_SIMULATOR_MODE is active (host build, no hardware access).
 * @pre The ra_core_hal OBJECT library was built with --coverage instrumentation.
 * @post All eight uncovered lines in ra_io_stream_blockdev.c have been executed.
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_io_stream_blockdev_cov.c\n");
  return 0;
}
