/**
 * @file test_ra_rsip_key_injection.c
 * @brief Unit tests for ra_rsip_key_injection.c (RSIP wrapped-key wrap)
 *
 * @details
 * Mirrors the ``test_ra_sce.c`` injection-side tests for the RSIP
 * stub layer:
 *
 ...
 *
 * Each test resets ``ra_sim_mmap`` first so cases stay independent.
 *
...
/**
 * @brief Reset ra_sim_mmap before every test so cases stay
...
int32_t main(void)
{
  ...
  test_null_args();
  (void)fprintf(stderr, "[OK ] test_ra_rsip_key_injection.c\n");
  return 0;
}
