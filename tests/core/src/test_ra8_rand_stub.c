...
static void test_srand_zero_does_not_lock(void)
{
  ...
  // NOLINTNEXTLINE(bugprone-random-generator-seed,cert-msc32-c,cert-msc51-cpp) -- deterministic rand stub is the behavior under test.
  srand(0U);
  /* If s_state were left zero xorshift returns 0 forever. */
  // NOLINTNEXTLINE(misc-predictable-rand,cert-msc30-c,cert-msc50-cpp,clang-analyzer-security.insecureAPI.rand) -- deterministic rand stub is the behavior under test.
  const int r = rand();
  TEST_ASSERT(r != 0);
