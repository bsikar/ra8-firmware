 ...
 *
 * @details
 * Mirrors examples/ek_ra8d2/blink_hal/main.c which calls ra_time_init,
 * iterates ra_board_led_init across all three EK-RA8D2 LEDs, then runs
 * ra_board_led_toggle on each in a steady-state loop. Replays the
 * exact init order against the host-mocked HAL.
 *
 * Exercised modules:
 *   - ra_board_ek_ra8d2 (LED bring-up + toggle)
 *   - ra_time           (SysTick init at the reset-default MOCO clock)
 *
 ...
 * ra_time_init call with no compound boolean decisions in the path.
 */
 ...
 *
 * @par MC/DC:
 * Compound decision under test (in app): ``ra_board_led_init(i) != ok``
 ...
 *
 * @par MC/DC:
 * Compound decision under test: ``ra_board_led_toggle(i) != ok`` inside
 ...
 *
 * @par MC/DC:
 * Decision vector under test: ``led_id >= k_ra_board_led_count`` guard
 * inside ra_board_led_init. Failure vector pairing with the all-ok init
 ...
 * @par MC/DC:
 * Decision vector under test: ``led_id >= k_ra_board_led_count`` guard
 * inside ra_board_led_toggle. Failure vector pairing with the all-ok
 * toggle loop.
 */
