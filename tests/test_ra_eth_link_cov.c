/**
 * @file test_ra_eth_link_cov.c
 * @brief White-box coverage tests for ra_eth_link.c (PHY link poll + MAC resync)
 *
 * @details
 * ra_eth_link.c realigns the on-chip RMAC's MPIC.LSC / MPIC.PIPP with
 * whatever the off-chip PHY auto-negotiated. Every negotiated-speed
 * decision hangs off the *value* returned by ra_rmac_mdio_c22_read.
 * The plain host register backing cannot present a non-zero MDIO read
 * result: ra_rmac.c issues the MPSM transaction with the PRD data
 ...
 * ra_eth_link_status entry point.
 *
 * To drive those paths honestly this file follows the repository's
 * established white-box pattern (see test_ra_usb_hmsc_enum_cov.c): it
 * renames the one exported symbol so the instrumented copy does not
 * clash with the production ra_eth_link_status linked from
 * ra_core_hal, redirects the four hardware-transport calls
 * (ra_rmac_mdio_c22_read, ra_rmac_set_link, ra_etha_set_mode,
 * ra_delay_ms) to deterministic mocks that let the test inject real
 ...
 * are rewritten to the mocks below. ra_etha.h transitively provides
 * ra_rmac.h (ra_rmac_mdio_c22_read / ra_rmac_set_link), ra_etha_ops.h
 * (ra_etha_set_mode) and every enum type the mocks need. */
#include "ra_etha.h"
...
/**
 * @enum ra_eth_link_cov_reg_t
 * @brief Clause-22 PHY register indices mirrored by the mock bus.
 *
 * @details Duplicated here rather than pulled from ra_eth_internal.h
...
} ra_eth_link_cov_reg_t;

/**
 * @enum ra_eth_link_cov_bits_t
...
} ra_eth_link_cov_bits_t;

/**
 * @enum ra_eth_link_cov_call_t
 * @brief 1-based ETHA-mode call indices inside internal_program_mpic.
 *
 * @details internal_program_mpic issues four ra_etha_set_mode calls in
...
static ra_err_t s_mdio_fail_err;

/** @brief Running count of ra_etha_set_mode calls in the current test. */
...
static ra_err_t s_etha_fail_err;

/** @brief Error code the mock ra_rmac_set_link returns (default k_ra_ok). */
static ra_err_t s_setlink_err;

/**
 * @brief Deterministic stand-in for ra_rmac_mdio_c22_read.
 *
 ...
 * @return k_ra_ok on a served read, else the injected failure code.
 */
static ra_err_t
mock_mdio_read(ra_rmac_port_t port, uint8_t phy_addr, uint8_t reg_addr, uint16_t* out)
{
  (void)port;
  (void)phy_addr;
  if (out == nullptr) {
...
/**
 * @brief Deterministic stand-in for ra_etha_set_mode.
 *
 ...
 *                 failing leg.
 * @return k_ra_ok, or ::s_etha_fail_err on the armed call index.
 */
static ra_err_t mock_etha_set_mode(ra_etha_port_t port, ra_etha_opc_t mode)
{
  (void)port;
  (void)mode;
  ++s_etha_call_count;
...
/**
 * @brief Deterministic stand-in for ra_rmac_set_link.
 *
 * @param[in] port   Ignored.
 * @param[in] iface  Ignored.
 * @param[in] speed  Ignored.
 * @param[in] duplex Ignored.
 * @return ::s_setlink_err (k_ra_ok unless a test forces rejection).
 */
static ra_err_t mock_rmac_set_link(ra_rmac_port_t   port,
                                   ra_rmac_pis_t    iface,
                                   ra_rmac_lsc_t    speed,
...
/**
 * @brief No-op stand-in for ra_delay_ms.
 *
 ...
 * in the production TU. ra_eth_link_status becomes ra_eth_link_status_cov
 * so it never clashes with the copy already inside ra_core_hal; the
 ...
 * ra_eth_channel_to_port stay un-renamed and resolve to their single
 * ra_eth.c definitions in ra_core_hal. */
#define ra_eth_link_status           ra_eth_link_status_cov
#define g_ra_eth_phy_bmsr_after_wait g_ra_eth_phy_bmsr_after_wait_cov
#define g_ra_eth_anlpar              g_ra_eth_anlpar_cov
#define g_ra_eth_gbsr                g_ra_eth_gbsr_cov
#define g_ra_eth_resync_speed_lsc    g_ra_eth_resync_speed_lsc_cov
#define g_ra_eth_resync_duplex       g_ra_eth_resync_duplex_cov
#define ra_rmac_mdio_c22_read        mock_mdio_read
#define ra_rmac_set_link             mock_rmac_set_link
#define ra_etha_set_mode             mock_etha_set_mode
#define ra_delay_ms                  mock_delay_ms

#include "ra_eth_link.c" // NOLINT(bugprone-suspicious-include) -- white-box copy

/**
 * @brief Reset every mock knob + shared driver latch before each test.
 */
static void prep(void)
{
  ...
  s_mdio_fail_reg    = 0U;
  s_mdio_fail_err    = k_ra_err_hw_error;

  s_etha_call_count   = 0U;
  s_etha_fail_on_call = (uint32_t)k_cov_etha_call_none;
  s_etha_fail_err     = k_ra_err_hw_timeout;

  s_setlink_err = k_ra_ok;
...
/**
 * @test test_pick_negotiated_speed_ladder
 *
 * @par MC/DC:
 ...
 * The all-bits vector proves the documented priority (1000F wins).
 */
static void test_pick_negotiated_speed_ladder(void)
{
  TEST_BEGIN("pick_negotiated_speed: ANLPAR/GBSR priority ladder");
  prep();
  ra_rmac_lsc_t    speed  = k_ra_rmac_lsc_1000mbit;
  ra_rmac_duplex_t duplex = k_ra_rmac_duplex_full;

  /* All abilities clear -> the default lowest tier survives. */
  internal_pick_negotiated_speed(0U, 0U, &speed, &duplex);
  TEST_ASSERT_EQ(k_ra_rmac_lsc_10mbit, speed);
  TEST_ASSERT_EQ(k_ra_rmac_duplex_half, duplex);
  ...
  TEST_ASSERT_EQ(k_ra_rmac_lsc_1000mbit, speed);
  TEST_ASSERT_EQ(k_ra_rmac_duplex_half, duplex);
...
/**
 * @test test_wait_for_autoneg_an_done
 *
 * @par MC/DC:
 ...
 * AN_COMPLETE-set exit.)
 */
static void test_wait_for_autoneg_an_done(void)
{
...
/**
 * @test test_wait_for_autoneg_mdio_fail
 *
 * @par MC/DC:
 * (no compound decision -- this vector drives the MDIO-read-failed
 * early return leg of the poll loop.)
 */
static void test_wait_for_autoneg_mdio_fail(void)
{
  ...
  TEST_ASSERT_EQ(0U, g_ra_eth_phy_bmsr_after_wait_cov);
  TEST_END("wait_for_autoneg: MDIO read failure aborts the poll");
}

/**
 * @test test_wait_for_autoneg_exhaust
 *
 * @par MC/DC:
 ...
 */
static void test_wait_for_autoneg_exhaust(void)
{
...
/**
 * @test test_query_negotiated_speed_ok
 *
 * @par MC/DC:
 * (no compound decision -- both MDIO reads succeed; the two error
 * guards are single-condition and take their false leg here.)
 */
static void test_query_negotiated_speed_ok(void)
{
...
/**
 * @test test_query_negotiated_speed_anlpar_fail
 *
 * @par MC/DC:
 * (no compound decision -- the ANLPAR read fails, taking the first
 * single-condition error guard's true leg.)
 */
static void test_query_negotiated_speed_anlpar_fail(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_error,
                 internal_query_negotiated_speed(k_ra_rmac_port_0, &speed, &duplex));
  TEST_END("query_negotiated_speed: ANLPAR read failure propagates");
}

/**
 * @test test_query_negotiated_speed_gbsr_fail
 *
 * @par MC/DC:
 * (no compound decision -- ANLPAR read succeeds while the GBSR read is
 * failed, taking the second single-condition error guard's true leg.)
 */
static void test_query_negotiated_speed_gbsr_fail(void)
{
  ...
  s_mdio_fail_err              = k_ra_err_hw_timeout;
  ra_rmac_lsc_t    speed       = k_ra_rmac_lsc_10mbit;
  ra_rmac_duplex_t duplex      = k_ra_rmac_duplex_half;
...
/**
 * @test test_program_mpic_gmii_ok
 *
 * @par MC/DC:
 ...
 * with every bracketed transition succeeding.)
 */
static void test_program_mpic_gmii_ok(void)
{
...
/**
 * @test test_program_mpic_mii_ok
 *
 * @par MC/DC:
 * (no compound decision -- takes port 1 and the sub-gigabit MII leg,
 * i.e. the speed==1000 gate's false direction.)
 */
static void test_program_mpic_mii_ok(void)
{
  TEST_BEGIN("program_mpic: 100 Mbps -> MII, port 1 path");
  prep();
  TEST_ASSERT_EQ(
    k_ra_ok,
    internal_program_mpic(k_ra_rmac_port_1, k_ra_rmac_lsc_100mbit, k_ra_rmac_duplex_half));
  TEST_END("program_mpic: 100 Mbps -> MII, port 1 path");
}

/**
 * @test test_program_mpic_first_disable_fail
 *
 * @par MC/DC:
 * (no compound decision -- the first DISABLE transition fails, taking
 * the first single-condition error guard's true leg.)
 */
static void test_program_mpic_first_disable_fail(void)
{
...
/**
 * @test test_program_mpic_config_fail
 *
 * @par MC/DC:
 * (no compound decision -- the CONFIG transition fails, taking the
 * second single-condition error guard's true leg.)
 */
static void test_program_mpic_config_fail(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_err_hw_timeout,
    internal_program_mpic(k_ra_rmac_port_0, k_ra_rmac_lsc_100mbit, k_ra_rmac_duplex_full));
  TEST_END("program_mpic: CONFIG transition timeout");
}

/**
 * @test test_program_mpic_setlink_fail
 *
 * @par MC/DC:
 * (no compound decision -- ra_rmac_set_link rejects the args so the
 * set_err single-condition guard takes its true leg while ETHA is
 * still restored to OPERATION.)
 */
static void test_program_mpic_setlink_fail(void)
{
  TEST_BEGIN("program_mpic: set_link rejects -> invalid_arg returned");
  prep();
  s_setlink_err = k_ra_err_invalid_arg;
  TEST_ASSERT_EQ(
...
/**
 * @test test_program_mpic_second_disable_fail
 *
 * @par MC/DC:
 * (no compound decision -- set_link succeeds but the second DISABLE
 * fails, taking the dis_err single-condition guard's true leg.)
 */
static void test_program_mpic_second_disable_fail(void)
{
...
/**
 * @test test_program_mpic_operation_fail
 *
 * @par MC/DC:
 ...
 * false leg and the op_err is returned unfiltered.)
 */
static void test_program_mpic_operation_fail(void)
{
 ...
 * take their false leg and the resync latch is set.)
 */
static void test_resync_mac_speed_ok(void)
{
  TEST_BEGIN("resync_mac_speed: happy path sets the resync latch");
  prep();
  s_phy_regs[k_cov_reg_bmsr]   = (uint16_t)k_cov_bmsr_an_done;
  s_phy_regs[k_cov_reg_anlpar] = (uint16_t)k_cov_anlpar_100f;
  TEST_ASSERT_EQ(k_ra_ok, internal_resync_mac_speed(k_ra_rmac_port_0, 0U));
  TEST_ASSERT_EQ(1, s_eth_mac_speed_resynced ? 1 : 0);
  TEST_END("resync_mac_speed: happy path sets the resync latch");
}

/**
 * @test test_resync_mac_speed_query_fail
 *
 * @par MC/DC:
 * (no compound decision -- the ANLPAR read fails so the query-error
 * single-condition guard takes its true leg and the latch stays low.)
 */
static void test_resync_mac_speed_query_fail(void)
{
  ...
  s_mdio_fail_reg            = (uint8_t)k_cov_reg_anlpar;
  s_mdio_fail_err            = k_ra_err_hw_error;
  TEST_ASSERT_EQ(k_ra_err_hw_error, internal_resync_mac_speed(k_ra_rmac_port_0, 0U));
  TEST_ASSERT_EQ(0, s_eth_mac_speed_resynced ? 1 : 0);
  TEST_END("resync_mac_speed: query failure propagates, latch unset");
}

/**
 * @test test_resync_mac_speed_program_fail
 *
 * @par MC/DC:
 ...
 * true leg and the latch stays low.)
 */
static void test_resync_mac_speed_program_fail(void)
{
  ...
  s_etha_fail_on_call          = (uint32_t)k_cov_etha_call_disable1;
  s_etha_fail_err              = k_ra_err_hw_timeout;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, internal_resync_mac_speed(k_ra_rmac_port_0, 0U));
  TEST_ASSERT_EQ(0, s_eth_mac_speed_resynced ? 1 : 0);
  TEST_END("resync_mac_speed: MPIC program failure propagates");
}

/* =============================================================================
 * ra_eth_link_status -- public poller (via the renamed _cov copy).
 * =============================================================================
 */

/**
 * @test test_link_status_speed100_decode
 *
 * @par MC/DC:
 ...
 * returns before the resync gate.)
 */
static void test_link_status_speed100_decode(void)
{
...
/**
 * @test test_link_status_resynced_skip
 *
 * @par MC/DC:
 ...
 * set, so the second gate short-circuits to k_ra_ok without a MPIC
 * write.)
 */
static void test_link_status_resynced_skip(void)
{
...
/**
 * @test test_link_status_triggers_resync
 *
 * @par MC/DC:
 ...
 * to internal_resync_mac_speed, which then sets the latch.)
 */
static void test_link_status_triggers_resync(void)
{
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_eth_link_cov.c\n");
  return 0;
}
