 ...
 * @brief Tests for the CBZ (ZIP-of-images) comic reader (ra8_comic +
 * ra8_comic_cbz).
 *
 * @details
 ...
 * through `ra8_comic_open` over a seek+read callback -- the #151 streaming path
 * -- and proves:
 *
 ...
 * @return Result reported by ::ra8_comic_open.
...
internal_tc_open_std(ra8_comic_t* c, ra8_comic_page_t* pages, char* names, uint64_t size);
 ...
 * @brief `ra8_comic_open` rejects NULL args, zero size, and a non-archive
 * stream.
 *
 * @par MC/DC:
...
RA8_INTERNAL static void internal_test_comic_open_arg_guards(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_comic_open(nullptr, internal_tc_read, nullptr, 1U, pages, 1U, names, 1U));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_comic_open(&c, nullptr, nullptr, 1U, pages, 1U, names, 1U));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_comic_open(&c, internal_tc_read, nullptr, 1U, nullptr, 1U, names, 1U));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_comic_open(&c, internal_tc_read, nullptr, 1U, pages, 1U, nullptr, 1U));
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 ra8_comic_open(&c, internal_tc_read, nullptr, 0U, pages, 1U, names, 1U));
 ...
 * @brief `ra8_comic_page_read` / `_info` reject out-of-range pages and a
...
RA8_INTERNAL static void internal_test_comic_facade_edges(void)
{
  ...
  memcpy(s_arc, s_tc_sig_zip_lfh, sizeof(s_tc_sig_zip_lfh));
  s_arc_size = 8U;
  TEST_ASSERT_EQ(
    k_ra8_err_invalid_size,
    ra8_comic_open(&c, internal_tc_read, nullptr, 8U, pages, 0U, names, (uint32_t)sizeof(names)));
  TEST_ASSERT_EQ(
    k_ra8_err_invalid_size,
    ra8_comic_open(&c, internal_tc_read, nullptr, 8U, pages, (uint32_t)k_tc_page_cap, names, 0U));

  /* Magic read shorter than the four bytes the ZIP test needs. */
  memcpy(s_arc, s_tc_sig_short, sizeof(s_tc_sig_short));
  s_arc_size = 2U;
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, internal_tc_open_std(&c, pages, names, 2U));

  /* 'P' but not "PK" -> neither ZIP nor RAR. */
  memcpy(s_arc, s_tc_sig_not_pk, sizeof(s_tc_sig_not_pk));
  s_arc_size = 8U;
  ...
  memcpy(s_arc, s_tc_sig_pk_other, sizeof(s_tc_sig_pk_other));
  s_arc_size = 8U;
  TEST_ASSERT_EQ(k_ra8_err_not_supported, internal_tc_open_std(&c, pages, names, 8U));
  ...
  memcpy(s_arc, s_tc_sig_eocd, sizeof(s_tc_sig_eocd));
