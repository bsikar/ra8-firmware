...
static void test_init_clears_wupen_and_resets_mstp(void)
{
  ...
  *wupen1_ptr() = 0xCAFEBABEU;
  ...
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_pwr_init());
  TEST_ASSERT_EQ((long long)0, (long long)*wupen0_ptr());
  TEST_ASSERT_EQ((long long)0, (long long)*wupen1_ptr());
...
static void test_module_request_release_round_trip(void)
{
  ...
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_pwr_module_request(k_ra_mstp_sci0));

  uint8_t ref = 0U;
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_mstp_get_refcount(k_ra_mstp_sci0, &ref));
  TEST_ASSERT_EQ((long long)1, (long long)ref);

  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_pwr_module_release(k_ra_mstp_sci0));
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_mstp_get_refcount(k_ra_mstp_sci0, &ref));
...
static void test_set_clear_wake_source(void)
{
  ...
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_pwr_set_wake_source(k_ra_pwr_wake_irq3));
  /* IRQ3 is bit 3 in WUPEN0. */
  TEST_ASSERT(((*wupen0_ptr()) & (1U << 3)) != 0U);

  bool enabled = false;
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_pwr_wake_source_is_enabled(k_ra_pwr_wake_irq3, &enabled));
  TEST_ASSERT(enabled);

  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_pwr_clear_wake_source(k_ra_pwr_wake_irq3));
  TEST_ASSERT_EQ((long long)0, (long long)((*wupen0_ptr()) & (1U << 3)));
...
static void test_wake_source_wupen1(void)
{
  ...
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_pwr_set_wake_source(k_ra_pwr_wake_agt0));
  /* AGT0 is bit 0 in WUPEN1. */
  TEST_ASSERT_EQ((long long)0x1U, (long long)*wupen1_ptr());
...
static void test_wake_source_invalid_id(void)
{
  ...
  TEST_ASSERT_EQ((int)k_ra_err_invalid_arg, (int)ra_pwr_set_wake_source(bogus));
  TEST_ASSERT_EQ((int)k_ra_err_invalid_arg, (int)ra_pwr_clear_wake_source(bogus));

  bool enabled = false;
  TEST_ASSERT_EQ((int)k_ra_err_invalid_arg, (int)ra_pwr_wake_source_is_enabled(bogus, &enabled));
  TEST_ASSERT_EQ((int)k_ra_err_null_ptr,
                 (int)ra_pwr_wake_source_is_enabled(k_ra_pwr_wake_irq0, nullptr));
...
static void test_software_standby_requires_wake_source(void)
{
  ...
  TEST_ASSERT_EQ((int)k_ra_err_invalid_state, (int)ra_pwr_enter_software_standby());

  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_pwr_set_wake_source(k_ra_pwr_wake_rtc_alarm));
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_pwr_enter_software_standby());
...
static void test_get_clock_hz_forwards_to_cgc(void)
{
  ...
  uint32_t hz = 0U;
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_pwr_get_clock_hz(k_ra_clock_id_cpuclk0, &hz));
  TEST_ASSERT(hz != 0U);

  TEST_ASSERT_EQ((int)k_ra_err_null_ptr, (int)ra_pwr_get_clock_hz(k_ra_clock_id_cpuclk0, nullptr));
