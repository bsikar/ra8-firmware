/**
 * @file test_ra_i3c_cov.c
 * @brief Coverage supplement for ra_i3c.c -- drives branches not yet
 *        exercised by test_ra_i3c.c.
 *
 * @details
 ...
 *
 * Harness: identical to test_ra_i3c.c -- RA_SIMULATOR_MODE mmap backing
 * store, ra_mstp_init() before each case, unity_minimal.h assertions.
 *
...
static const ra_i3c_cfg_t k_native_cfg = {
  .mode     = k_ra_i3c_mode_native,
  .bus_hz   = 0U,
  .pclka_hz = 0U,
};

/** @brief I2C-compat bring-up configuration (valid clocks). */
static const ra_i3c_cfg_t k_i2c_cfg = {
 ...
 *
 * @details Mirrors the prep() helper in test_ra_i3c.c.  Does NOT reset
 * the module-level s_i3c_chan[] state; callers must call ra_i3c_deinit()
 * or ra_i3c_init() themselves to reach the desired channel state.
 */
...
{
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_init(0U, &k_i2c_cfg));
}
...
{
  prep();
  (void)ra_i3c_deinit(0U); /* clears .initialized */
}

/* ---------------------------------------------------------------------------
 * Test: priv_ra_i3c_fifo_write partial-word path (lines 250-258) and
 *       ra_i3c_send_ccc regular-transfer path (lines 575-577).
 *
 * @par MC/DC:
 * Decision A (ra_i3c_send_ccc, line 561): ``len <= k_ra_i3c_immediate_max_bytes``
 * - V1: len=3  -> C1=T -> immediate path (covered by test_ra_i3c.c)
 ...
 *
 * The regular-FIFO path calls priv_ra_i3c_fifo_write(reg, payload, 5);
...
static void test_init_i2c_mode_error(void)
{
  ...
  prep();
  /* bus_hz=0 causes internal_i3c_i2c_init to return k_ra_err_invalid_arg;
   * ra_i3c_init's guard at line 318 is true and returns on line 319. */
  const ra_i3c_cfg_t i2c_fail_cfg = {
    .mode     = k_ra_i3c_mode_i2c,
    .bus_hz   = 0U,
    .pclka_hz = 8000000U,
  };
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_init(0U, &i2c_fail_cfg));
  TEST_END("i3c init i2c mode propagates internal error (line 319)");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_deinit out-of-range channel (line 345).
 *
 * @par MC/DC:
...
static void test_deinit_oob(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_deinit(255U));
  TEST_END("i3c deinit rejects out-of-range channel (line 345)");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_attach_handler out-of-range channel (line 412).
 *
 * @par MC/DC:
 * (no new compound decisions; single channel-range guard)
 * ---------------------------------------------------------------------------
...
static void test_attach_handler_oob(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_attach_handler(255U, nullptr, nullptr));
  TEST_END("i3c attach_handler rejects out-of-range channel (line 412)");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_dispatch out-of-range channel (line 422) and i2c-mode
 *       dispatch (lines 428-434).
 *
 * @par MC/DC:
 * Decision A (ra_i3c_dispatch line 431): ``fn != nullptr``
...
/* ---------------------------------------------------------------------------
 * Test: ra_i3c_recv_ccc returns invalid_arg via internal_recv_ccc_invalid
 *       (line 593) when target_addr > addr_mask with max_len > 0.
 *
 * @par MC/DC:
 * Decision at ra_i3c_recv_ccc line 592 (call to
 * ra_i3c_internal_recv_ccc_invalid): see test_ra_i3c.c for the full
...
static void test_recv_ccc_internal_invalid(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_recv_ccc(0x8DU, 0xFFU, buf, 4U, &got_len));
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_recv_ccc(0x8DU, 0x10U, buf, 0U, &got_len));
  TEST_END("i3c recv_ccc: internal predicate triggers invalid_arg (line 593)");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_write error paths (lines 615, 618, 632).
 *
 * @par MC/DC:
 * Decision (ra_i3c_write line 628): ``(len > 0U) && (data == nullptr)``
 * is already fully covered by test_ra_i3c.c.  The three new guards here
...
static void test_write_error_paths(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_write(255U, 0x10U, &dummy, 1U, false));

  /* Line 618: channel 0 not yet initialized. */
  prep_uninit();
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_i3c_write(0U, 0x10U, &dummy, 1U, false));

  /* Line 632: len > k_ra_i3c_cmd_xfer_length_max (0xFFFF). */
  prep_native();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_write(0U, 0x10U, &dummy, 0x10000U, false));
  TEST_END("i3c write: oob channel / uninit / len overflow errors");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_read error paths (lines 660, 663).
 *
 * @par MC/DC:
...
static void test_read_error_paths(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_read(255U, 0x10U, buf, 4U, false));

  /* Line 663: channel 0 not initialized. */
  prep_uninit();
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_i3c_read(0U, 0x10U, buf, 4U, false));
  TEST_END("i3c read: oob channel / uninit errors");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_transfer validation (lines 701, 704).
 *
 * @par MC/DC:
 * (no new compound decisions; single guards)
 * ---------------------------------------------------------------------------
...
static void test_transfer_error_paths(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_transfer(255U, 0x10U, nullptr, 0U, buf, 2U));

  /* Line 704: channel 0 in native mode is not I2C. */
  prep_native();
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_i3c_transfer(0U, 0x10U, nullptr, 0U, buf, 2U));
  TEST_END("i3c transfer: oob channel / native-mode error paths");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_set_clock -- entire function uncovered (lines 709-718).
 *
 * @par MC/DC:
...
static void test_set_clock_coverage(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_set_clock(255U, 100000U, 8000000U));

  /* Lines 714, 715: channel 0 in native mode is not I2C. */
  prep_native();
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_i3c_set_clock(0U, 100000U, 8000000U));

  /* Lines 717, 718: channel 0 in I2C mode -- happy path. */
  prep_i2c();
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_set_clock(0U, 100000U, 8000000U));
  TEST_END("i3c set_clock: full function coverage (lines 709-718)");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_scan error returns (lines 723, 726).
 * The happy path (line 728) is already covered by test_ra_i3c_i2c.c.
 *
 * @par MC/DC:
 * (no new compound decisions; single guards)
 * ---------------------------------------------------------------------------
...
static void test_scan_error_paths(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_scan(255U, 0x10U, &acked));

  /* Line 726: channel 0 in native mode is not I2C. */
  prep_native();
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_i3c_scan(0U, 0x10U, &acked));
  TEST_END("i3c scan: oob channel / native-mode error returns");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_get_errors -- entire function uncovered (lines 731-740).
 *
 * @par MC/DC:
 * (no new compound decisions; single guards on each return path)
 * ---------------------------------------------------------------------------
...
static void test_get_errors_coverage(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_get_errors(255U, &mask));

  /* Lines 736, 737: channel 0 in native mode. */
  prep_native();
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_i3c_get_errors(0U, &mask));

  /* Lines 739, 740: channel 0 in I2C mode -- happy path. */
  prep_i2c();
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_get_errors(0U, &mask));
  TEST_END("i3c get_errors: full function coverage (lines 731-740)");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_clear_errors -- entire function uncovered (lines 742-751).
 *
 * @par MC/DC:
 * (no new compound decisions; single guards on each return path)
 * ---------------------------------------------------------------------------
...
static void test_clear_errors_coverage(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_clear_errors(255U));

  /* Lines 747, 748: native mode channel. */
  prep_native();
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_i3c_clear_errors(0U));

  /* Lines 750, 751: I2C mode -- happy path. */
  prep_i2c();
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_clear_errors(0U));
  TEST_END("i3c clear_errors: full function coverage (lines 742-751)");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_abort -- entire function uncovered (lines 753-762).
 *
 * @par MC/DC:
 * (no new compound decisions; single guards on each return path)
 * ---------------------------------------------------------------------------
...
static void test_abort_coverage(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_abort(255U));

  /* Lines 758, 759: native mode channel. */
  prep_native();
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_i3c_abort(0U));

  /* Lines 761, 762: I2C mode -- happy path.  internal_i3c_i2c_abort
   * performs only register writes (no polling), safe in the sim. */
  prep_i2c();
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_abort(0U));
  TEST_END("i3c abort: full function coverage (lines 753-762)");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_peripheral_close -- entire function uncovered (lines 787-796).
 *
 * @par MC/DC:
 * (no new compound decisions; single guards on each return path)
 * ---------------------------------------------------------------------------
...
static void test_peripheral_close_coverage(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_peripheral_close(255U));

  /* Lines 792, 793: native mode channel. */
  prep_native();
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_i3c_peripheral_close(0U));

  /* Lines 795, 796: I2C mode channel -- happy path.
   * internal_i3c_i2c_peripheral_close performs register writes only. */
  prep_i2c();
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_peripheral_close(0U));
  TEST_END("i3c peripheral_close: full function coverage (lines 787-796)");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_peripheral_send -- entire function uncovered (lines 798-807).
 *
 * @par MC/DC:
 * (no new compound decisions; single guards on each return path)
 * ---------------------------------------------------------------------------
...
static void test_peripheral_send_coverage(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_peripheral_send(255U, data, 1U));

  /* Lines 803, 804: native mode channel. */
  prep_native();
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_i3c_peripheral_send(0U, data, 1U));

  /* Lines 806, 807: I2C mode -- len=0 so the for-loop body is never
   * entered, avoiding any TDBEF0 spin.  Returns k_ra_ok. */
  prep_i2c();
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_peripheral_send(0U, nullptr, 0U));
  TEST_END("i3c peripheral_send: full function coverage (lines 798-807)");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_peripheral_receive -- entire function uncovered (lines 809-818).
 *
 * @par MC/DC:
 * (no new compound decisions; single guards on each return path)
 * ---------------------------------------------------------------------------
...
static void test_peripheral_receive_coverage(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_peripheral_receive(255U, buf, 1U));

  /* Lines 814, 815: native mode channel. */
  prep_native();
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_i3c_peripheral_receive(0U, buf, 1U));

  /* Lines 817, 818: I2C mode -- len=0 so the for-loop body is skipped,
   * no RDBFF0 spin.  Returns k_ra_ok. */
  prep_i2c();
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_peripheral_receive(0U, nullptr, 0U));
  TEST_END("i3c peripheral_receive: full function coverage (lines 809-818)");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_peripheral_status error returns (lines 823, 826).
 * The happy path is already covered by test_ra_i3c_i2c_peripheral.c.
 *
 * @par MC/DC:
 * (no new compound decisions; single guards)
 * ---------------------------------------------------------------------------
...
static void test_peripheral_status_error_paths(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_peripheral_status(255U, &mask));

  /* Line 826: channel 0 in native mode. */
  prep_native();
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_i3c_peripheral_status(0U, &mask));
  TEST_END("i3c peripheral_status: oob / native-mode error returns");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_ibi_read -- IBI type hot-join branch (line 939).
 *
 * @par MC/DC:
 * Decision (ra_i3c_ibi_read line 938 else-if):
 *   ``ibi_id == k_ibi_id_hot_join`` (single condition, already in else of
 *   the IBI_ST==0 guard).
 * - V1: ibi_id=0x84 (not hot-join), IBI_ST=0 -> interrupt type (line 937)
 *       [covered by test_ra_i3c.c test_ibi_read_one]
...
static void test_ibi_hot_join(void)
{
  ...
  ra_i3c()->NIBIQP = 0x80000200U;

  ra_i3c_ibi_t ibi = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_ibi_read(&ibi));
  TEST_ASSERT_EQ(k_ra_i3c_ibi_type_hot_join, ibi.type);
  TEST_ASSERT_EQ(0U, ibi.payload_len);
  TEST_END("i3c ibi_read: hot-join type (line 939)");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_ibi_read -- IBI type main-request branch (line 941).
 *
 * @par MC/DC:
...
static void test_ibi_main_request(void)
{
  ...
  ra_i3c()->NIBIQP = 0x80000400U;

  ra_i3c_ibi_t ibi = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_ibi_read(&ibi));
  TEST_ASSERT_EQ(k_ra_i3c_ibi_type_main_request, ibi.type);
  TEST_ASSERT_EQ(0U, ibi.payload_len);
  TEST_END("i3c ibi_read: main-request type (line 941)");
}

/* ---------------------------------------------------------------------------
 * Test: ra_i3c_ibi_read -- payload length clamping (lines 947-948).
 *
 * @par MC/DC:
 * Decision (ra_i3c_ibi_read line 946): ``to_copy > k_max_ibi_payload``
 * where k_max_ibi_payload = sizeof(out_ibi->payload) = 8.
 * - V1: len=2  -> 2 <= 8 -> no clamp (covered by test_ra_i3c.c)
...
static void test_ibi_payload_clamping(void)
{
  ...
  ra_i3c()->NIBIQP  = 0x00008409U;
  ra_i3c()->NTDTBP0 = 0xDEADBEEFU; /* payload data (two reads of 4 bytes) */

  ra_i3c_ibi_t ibi = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_ibi_read(&ibi));
  TEST_ASSERT_EQ(k_ra_i3c_ibi_type_interrupt, ibi.type);
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_i3c_cov.c\n");
  return 0;
}
