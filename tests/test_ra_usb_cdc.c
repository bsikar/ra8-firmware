...
static void test_init_bad_speed(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_cdc_init((ra_usb_speed_t)9U));
...
static void test_init_hs_picks_512_bulk(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_cdc_init(k_ra_usb_speed_hs));
  ...
  TEST_ASSERT_EQ(k_ra_cdc_intr_max_packet, reg->PIPEMAXP);
...
static void test_pre_init_calls(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_cdc_attach(true));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_cdc_send(buf, 1U));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_cdc_recv(buf, &len));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_cdc_deinit());

  ra_usb_cdc_line_coding_t coding = {};
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_cdc_get_line_coding(&coding));
  bool dtr = false;
  bool rts = false;
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_cdc_get_line_state(&dtr, &rts));

  ra_usb_setup_t setup = {};
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_cdc_handle_setup(&setup));
...
static void test_handle_setup_set_control_line_state(void)
{
  ...
  bool dtr = false;
  bool rts = false;
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_cdc_get_line_state(&dtr, &rts));
  TEST_ASSERT(dtr);
  TEST_ASSERT(rts);
...
static void test_handle_setup_rejects_standard(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_cdc_handle_setup(nullptr));
...
static void test_send_recv_arg_validation(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_cdc_send(nullptr, 4U));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_cdc_recv(nullptr, &len));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_cdc_recv(buf, nullptr));
  uint16_t zero = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_cdc_recv(buf, &zero));

  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_cdc_get_line_coding(nullptr));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_cdc_get_line_state(nullptr, nullptr));
  bool dtr = false;
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_cdc_get_line_state(&dtr, nullptr));
...
static void test_mcdc_cdc(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_cdc_init(k_ra_usb_speed_hs));
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_cdc_init((ra_usb_speed_t)k_test_cdc_speed_bad));
