/**
 * @file test_ra_ota_parse_cov.c
 * @brief Coverage-gap tests for libs/ra_ota/src/ra_ota_parse.c.
 *
 * @details
 * Companion to test_ra_ota.c and test_ra_ota_cov.c. Targets 17 lines in
 * ra_ota_parse.c left uncovered by the primary suites:
 *
 *   Line 202 -- priv_validate_cfg_flash: bank_size > k_ra_ota_max_image_bytes.
 *   Line 216 -- ra_ota_internal_validate_cfg: crypto sub-validator fails.
 ...
 *   Line 313 -- ra_ota_internal_json_u32: key not found in JSON.
 *   Line 334 -- ra_ota_internal_json_u32: no digits after skip loop.
 ...
 *   Line 504 -- ra_ota_internal_manifest_decode: url key missing.
 *   Line 508 -- ra_ota_internal_manifest_decode: size key missing.
 *   Line 511 -- ra_ota_internal_manifest_decode: image_size_bytes == 0.
 *   Line 514 -- ra_ota_internal_manifest_decode: image_size_bytes > max.
 *
 * All paths are reached through the promoted internal helpers exported by
 * ra_ota_internal.h: ra_ota_internal_validate_cfg,
 * ra_ota_internal_manifest_decode, and ra_ota_internal_json_u32. The private
 * helpers (priv_json_str, priv_hex_nibble, priv_hex_decode,
 * priv_manifest_decode_crypto) are exercised transitively.
 *
 * ra_ota_internal_validate_cfg and ra_ota_internal_manifest_decode are pure
 * with respect to module state (documented in ra_ota_parse.c). No
 * ra_ota_init call is required.
 *
 ...
 * checks inside ra_ota_internal_validate_cfg.
 * ============================================================================= */

static ra_err_t dummy_net_open(void* ctx, const char* url, uint32_t* out_len)
{
  (void)ctx;
  (void)url;
  (void)out_len;
...
static ra_err_t dummy_net_read(void* ctx, uint8_t* dst, uint32_t cap, uint32_t* out_len)
{
  (void)ctx;
  (void)dst;
  (void)cap;
...
static ra_err_t dummy_net_close(void* ctx)
{
  (void)ctx;
  return k_ra_ok;
}

static ra_err_t dummy_sha_init(void* ctx)
{
  (void)ctx;
  return k_ra_ok;
}

static ra_err_t dummy_sha_update(void* ctx, const uint8_t* data, uint32_t len)
{
  (void)ctx;
  (void)data;
  (void)len;
...
static ra_err_t dummy_sha_final(void* ctx, uint8_t out[k_ra_ota_sha256_bytes])
{
  (void)ctx;
  (void)out;
  return k_ra_ok;
...
static ra_err_t dummy_ecdsa_verify(void*          ctx,
                                   uint32_t       key,
...
static ra_err_t dummy_flash_erase(void* ctx, uint32_t addr, uint32_t len)
{
  (void)ctx;
  (void)addr;
  (void)len;
...
static ra_err_t dummy_flash_program(void* ctx, uint32_t addr, const uint8_t* src, uint32_t len)
{
  (void)ctx;
  (void)addr;
  (void)src;
...
static ra_err_t dummy_flash_set_startup(void* ctx, uint8_t which, bool persistent)
{
  (void)ctx;
  (void)which;
  (void)persistent;
...
static ra_err_t dummy_flash_readback(void* ctx, uint32_t addr, uint8_t* dst, uint32_t len)
{
  (void)ctx;
  (void)addr;
  (void)dst;
...
/**
 * @brief Build a minimal fully-populated ra_ota_cfg_t using dummy stubs.
 *
 * @details All function pointers reference the local dummy stubs above.
 *          bank_size_bytes is 4096 (well within k_ra_ota_max_image_bytes).
 *          manifest_url is non-empty.
 *
 * @return ra_ota_cfg_t A valid, fully populated configuration.
 *
 * @pre  None.
 * @post Returned struct passes ra_ota_internal_validate_cfg.
...
{
  ra_ota_cfg_t c = {};
  (void)snprintf(c.manifest_url, k_ra_ota_url_max_bytes, "https://example.test/manifest.json");
...
/**
 * @brief Verify priv_validate_cfg_flash rejects bank_size > k_ra_ota_max_image_bytes.
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 202.
 ...
 * @post ra_ota_internal_validate_cfg returns k_ra_err_invalid_arg.
 * @post No module state mutated.
 *
 * @par MC/DC:
 * Decision: `cfg->flash.bank_size_bytes > k_ra_ota_max_image_bytes` (1 condition).
 * - This test: bank_size = k_ra_ota_max_image_bytes + 1 -> true -> line 202.
 * - Existing test_ra_ota.c test_init_validation (bank_size = 4096) covers false.
 *
 * @since 0.1.0
 */
static void test_validate_cfg_flash_bank_too_large(void)
{
  ...
  ra_ota_cfg_t c          = priv_make_cfg();
  c.flash.bank_size_bytes = (uint32_t)k_ra_ota_max_image_bytes + 1U;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_ota_internal_validate_cfg(&c));
  TEST_END("parse_cov: validate_cfg flash bank_size > max -> line 202");
}

/**
 * @brief Verify ra_ota_internal_validate_cfg propagates a crypto sub-validator error.
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 216.
 *          Net pointers are all valid so priv_validate_cfg_net returns k_ra_ok.
 *          crypto.sha256_init is NULL so priv_validate_cfg_crypto returns
 *          k_ra_err_null_ptr which is propagated at line 216.
 *
 * @pre  None (pure validator; no module init required).
 * @pre  priv_make_cfg() returns a valid config.
 * @post ra_ota_internal_validate_cfg returns k_ra_err_null_ptr.
 * @post No module state mutated.
 *
 * @par MC/DC:
 * Decision: `e != k_ra_ok` after priv_validate_cfg_crypto (1 condition).
 * - This test: crypto.sha256_init NULL -> e = k_ra_err_null_ptr -> true -> line 216.
 * - Existing test_ra_ota.c (full validation success) covers the false arm.
 *
 * @since 0.1.0
 */
static void test_validate_cfg_crypto_null(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_ota_internal_validate_cfg(&c));
 ...
 *        the key (line 268).
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 268.
 ...
 *          Exercised transitively: ra_ota_internal_manifest_decode calls
 *          priv_json_str for the version field first.
 *
 * @pre  None (pure parser; no module init required).
 * @pre  json is NUL-terminated.
 * @post ra_ota_internal_manifest_decode returns k_ra_err_invalid_arg.
 * @post No module state mutated.
 *
 * @par MC/DC:
...
static void test_json_str_no_open_quote(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_ota_internal_manifest_decode("{\"version\":nostring}", &m));
 ...
 *        quote (line 273).
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 273.
...
static void test_json_str_no_close_quote(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_ota_internal_manifest_decode("{\"version\":\"unclosed}", &m));
 ...
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 277.
 *          k_ra_ota_version_str_bytes = 32 bytes. A version value of 32
 *          characters gives n = 32, n + 1 = 33 > cap (32) -> line 277.
 *
 * @pre  None (pure parser; no module init required).
 * @pre  json is NUL-terminated.
 * @post ra_ota_internal_manifest_decode returns k_ra_err_invalid_size.
 * @post No module state mutated.
 *
 * @par MC/DC:
...
static void test_json_str_value_too_long(void)
{
  ...
  TEST_ASSERT_EQ(
    ...
    ra_ota_internal_manifest_decode("{\"version\":\"12345678901234567890123456789012\","
                                    "\"url\":\"x\"}",
                                    &m));
...
/**
 * @brief Verify ra_ota_internal_json_u32 returns invalid_arg when the key is
 *        absent from the JSON document (line 313).
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 313.
 *          Called directly (exported through ra_ota_internal.h).
 *
 * @pre  None (pure parser; no module init required).
 * @pre  json and key are NUL-terminated; out_v is non-NULL.
 * @post ra_ota_internal_json_u32 returns k_ra_err_invalid_arg.
 ...
 * - test_ra_ota.c covers the false arm (key present).
 *
 * @since 0.1.0
 */
static void test_json_u32_key_not_found(void)
{
  TEST_BEGIN("parse_cov: ra_ota_internal_json_u32 key not found -> line 313");
  uint32_t v = 0xDEADBEEFUL;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_ota_internal_json_u32("{\"foo\":42}", "\"size\"", &v));
  /* out_v must be unchanged on failure. */
  TEST_ASSERT_EQ(0xDEADBEEFUL, v);
  TEST_END("parse_cov: ra_ota_internal_json_u32 key not found -> line 313");
}

/**
 * @brief Verify ra_ota_internal_json_u32 returns invalid_arg when there are no
 *        digit characters after the skip loop (line 334).
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 334.
 ...
 * - test_ra_ota.c covers the false arm (at least one digit consumed).
 *
 * @since 0.1.0
 */
static void test_json_u32_no_digits(void)
{
  TEST_BEGIN("parse_cov: ra_ota_internal_json_u32 no digits -> line 334");
  uint32_t v = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_ota_internal_json_u32("{\"size\":\"notanumber\"}", "\"size\"", &v));
  TEST_END("parse_cov: ra_ota_internal_json_u32 no digits -> line 334");
}

/**
 * @brief Verify priv_hex_nibble covers the uppercase A-F return path (line 370).
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 370.
 *          Reached transitively through ra_ota_internal_manifest_decode when the
 ...
 * @post ra_ota_internal_manifest_decode returns k_ra_ok.
 * @post image_sha256[0] == 0xAA, image_sha256[1] == 0xBB, signature_len == 2.
 *
 * @par MC/DC:
 * Decision: `ra_ota_internal_char_in_range(c, 'A', 'F')` (1 condition).
 * - This test: c in 'A'..'F' -> true -> uppercase return at line 370.
 * - test_mcdc_hex_nibble_pair_completion in test_ra_ota.c (c = ':') covers false.
 *
 * @since 0.1.0
 */
static void test_hex_nibble_uppercase(void)
{
  ...
  const ra_err_t e =
    ra_ota_internal_manifest_decode("{ \"version\": \"1.0.0\","
                                    " \"url\": \"https://x\","
                                    " \"size\": 256,"
 ...
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 402.
 *          Reached transitively through ra_ota_internal_manifest_decode: the
...
static void test_hex_decode_odd_length(void)
{
  ...
  const ra_err_t e = ra_ota_internal_manifest_decode("{ \"version\": \"1.0.0\","
                                                     " \"url\": \"https://x\","
                                                     " \"size\": 256,"
                                                     " \"sha256\": \"abc\","
                                                     " \"signature\": \"aabb\" }",
 ...
 *        (line 406).
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 406.
 *          k_ra_ota_sha256_bytes = 32 is the out_cap for the sha256 field.
...
static void test_hex_decode_exceeds_cap(void)
{
  ...
  ra_ota_manifest_t m = {};
  /* sha256 hex: 66 even-length chars -> 33 bytes > k_ra_ota_sha256_bytes (32). */
  const ra_err_t e =
    ra_ota_internal_manifest_decode("{ \"version\": \"1.0.0\","
                                    " \"url\": \"https://x\","
                                    " \"size\": 256,"
 ...
 *        absent (line 452).
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 452.
 ...
 *          priv_json_str returns k_ra_err_invalid_arg -> line 452.
 *
 * @pre  None (pure parser; no module init required).
 * @pre  json is NUL-terminated; out is non-NULL.
 * @post ra_ota_internal_manifest_decode returns k_ra_err_invalid_arg.
 ...
 *
 * @par MC/DC:
 * Decision: `e != k_ra_ok` after priv_json_str for sha256 (1 condition).
 ...
 *
 * @par MC/DC:
 * Decision: `e != k_ra_ok` after priv_json_str for signature (1 condition).
 ...
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 465.
...
/**
 * @brief Verify ra_ota_internal_manifest_decode propagates error when the url
 *        key is absent (line 504).
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 504.
 *          The version field parses successfully; the url key is absent ->
 *          priv_json_str returns k_ra_err_invalid_arg -> line 504.
 *
 * @pre  None (pure parser; no module init required).
 * @pre  json is NUL-terminated; out is non-NULL.
 * @post ra_ota_internal_manifest_decode returns k_ra_err_invalid_arg.
 ...
 *
 * @par MC/DC:
 * Decision: `e != k_ra_ok` after priv_json_str for url (1 condition).
...
static void test_manifest_decode_no_url(void)
{
  ...
  const ra_err_t e = ra_ota_internal_manifest_decode("{ \"version\": \"1.0.0\" }", &m);
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, e);
  TEST_END("parse_cov: manifest_decode url missing -> line 504");
}

/**
 * @brief Verify ra_ota_internal_manifest_decode propagates error when the size
 *        key is absent (line 508).
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 508.
 *          version and url parse successfully; the size key is absent ->
 *          ra_ota_internal_json_u32 returns k_ra_err_invalid_arg -> line 508.
 *
 * @pre  None (pure parser; no module init required).
 * @pre  json is NUL-terminated; out is non-NULL.
 * @post ra_ota_internal_manifest_decode returns k_ra_err_invalid_arg.
 ...
 *
 * @par MC/DC:
 * Decision: `e != k_ra_ok` after ra_ota_internal_json_u32 for size (1 condition).
...
static void test_manifest_decode_no_size(void)
{
  ...
  const ra_err_t e =
    ra_ota_internal_manifest_decode("{ \"version\": \"1.0.0\", \"url\": \"https://x\" }", &m);
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, e);
  TEST_END("parse_cov: manifest_decode size missing -> line 508");
}

/**
 * @brief Verify ra_ota_internal_manifest_decode returns invalid_arg when
 *        image_size_bytes is zero (line 511).
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 511.
...
/**
 * @brief Verify ra_ota_internal_manifest_decode returns invalid_size when
 *        image_size_bytes exceeds k_ra_ota_max_image_bytes (line 514).
 *
 * @details Covers libs/ra_ota/src/ra_ota_parse.c line 514.
 *          k_ra_ota_max_image_bytes = 0x80000 = 524288. size = 524289 exceeds
 *          the cap -> line 514.
 *
 * @pre  None (pure parser; no module init required).
 * @pre  json is NUL-terminated; out is non-NULL.
 ...
 *
 * @par MC/DC:
 * Decision: `out->image_size_bytes > k_ra_ota_max_image_bytes` (1 condition).
...
static void test_manifest_decode_size_exceeds_max(void)
{
  ...
  ra_ota_manifest_t m = {};
  /* 524289 = 0x80001 > k_ra_ota_max_image_bytes (0x80000 = 524288). */
  const ra_err_t e = ra_ota_internal_manifest_decode("{ \"version\": \"1.0.0\","
                                                     " \"url\": \"https://x\","
                                                     " \"size\": 524289 }",
                                                     &m);
...
int main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_ota_parse_cov.c\n");
  return 0;
}
