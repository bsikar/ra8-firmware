/**
 * @file test_ra_xspi_cov.c
 * @brief Supplemental host coverage for the xSPI XIP enter/exit surface.
 *
 * @details
 * The primary suite (``test_ra_xspi.c``) covers init, direct-command,
 ...
 * ``ra_xspi_xip_enter()`` / ``ra_xspi_xip_exit()`` -- leaving those two
 ...
 *       the only branch is the single-condition ``RA_CHECK_NULL_PTR`` guard,
...
typedef enum : uint8_t {
  ...
  k_test_xspi_cov_bad   = 99U, /**< Out-of-range instance (ra_xspi -> nullptr). */
 ...
 *
 * @details
 * Mirrors the ``prep_w51`` pattern in ``test_ra_xspi.c``: reset the fake
 * peripheral RAM, (re)initialise the module-stop shadow, then run
 * ``ra_xspi_init`` so the register window is gated open and the XIP entry
 ...
 *
 * @return None.
 * @pre The host sim mmap window is available (linked ra_sim_mmap mock).
 ...
 * @post ``ra_xspi(k_test_xspi_cov_inst0)`` returns a non-null pointer.
...
static void prep_xip(void)
{
  ra_sim_mmap_reset();
  (void)ra_mstp_init();
  TEST_ASSERT_EQ(k_ra_ok, ra_xspi_init((uint8_t)k_test_xspi_cov_inst0, k_ra_xspi_lio_1s8s8s));
}
 ...
 *
 * @par MC/DC:
 * (no compound decisions in the code under test -- ``ra_xspi_xip_enter``
 * packs the enter/exit codes and pokes BMCTL0 + CMCTLCH[0/1]; its only
 * branch is the single-condition ``RA_CHECK_NULL_PTR`` guard, driven
 * false here by a valid instance and true by ::test_xip_enter_bad_instance.)
 */
static void test_xip_enter_happy(void)
{
  TEST_BEGIN("ra_xspi_xip_enter arms XIPEN + read-only map");
  prep_xip();

  const uint32_t code_word =
    ((uint32_t)k_test_xspi_cov_enter_code << k_ra_xspi_cmctlch_xipencode_pos) |
    ((uint32_t)k_test_xspi_cov_exit_code << k_ra_xspi_cmctlch_xipexcode_pos);
  const uint32_t expect_ch = code_word | (uint32_t)k_ra_xspi_cmctlch_xipen_mask;

  TEST_ASSERT_EQ(k_ra_ok,
                 ra_xspi_xip_enter((uint8_t)k_test_xspi_cov_inst0,
                                   (uint8_t)k_test_xspi_cov_enter_code,
                                   (uint8_t)k_test_xspi_cov_exit_code));

  volatile r_xspi_regs_t* reg = ra_xspi((uint8_t)k_test_xspi_cov_inst0);
  ...
  TEST_END("ra_xspi_xip_enter arms XIPEN + read-only map");
}
 ...
 *
 * @par MC/DC:
 * (single-condition ``RA_CHECK_NULL_PTR(reg, ...)`` guard -- this vector
...
{
  TEST_BEGIN("ra_xspi_xip_enter rejects out-of-range instance");
  prep_xip();
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_xspi_xip_enter((uint8_t)k_test_xspi_cov_bad,
                                   (uint8_t)k_test_xspi_cov_enter_code,
                                   (uint8_t)k_test_xspi_cov_exit_code));
  TEST_END("ra_xspi_xip_enter rejects out-of-range instance");
}
 ...
 *
 * @par MC/DC:
 * (no compound decisions -- ``ra_xspi_xip_exit`` clears CMCTLCH[0/1] and
 * restores BMCTL0 to read/write; its only branch is the single-condition
 * ``RA_CHECK_NULL_PTR`` guard, false here for a valid instance and true in
 * ::test_xip_exit_bad_instance.)
 */
static void test_xip_exit_happy(void)
{
  TEST_BEGIN("ra_xspi_xip_exit disarms XIPEN + restores read/write");
  prep_xip();
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_xspi_xip_exit((uint8_t)k_test_xspi_cov_inst0));
  ...
  TEST_END("ra_xspi_xip_exit disarms XIPEN + restores read/write");
}
...
{
  TEST_BEGIN("ra_xspi_xip_exit rejects out-of-range instance");
  prep_xip();
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_xspi_xip_exit((uint8_t)k_test_xspi_cov_bad));
  TEST_END("ra_xspi_xip_exit rejects out-of-range instance");
}
...
int32_t main(void)
{
  ...
  test_xip_exit_happy();
  test_xip_exit_bad_instance();
  (void)fprintf(stderr, "[OK ] test_ra_xspi_cov.c\n");
  return 0;
}
