...
static void test_init_happy(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_init(&cfg));

  /* MCT0: VDLN = 2, GRMD set. */
  const uint32_t mct0 = *ra_mipi_csi_reg32(k_ra_mipi_csi_off_mct0);
  TEST_ASSERT_EQ(k_ra_mipi_csi_lanes_2, (mct0 & (uint32_t)k_ra_mipi_csi_mct0_vdln_mask));
  ...
  TEST_ASSERT_EQ(0, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_mct3));
...
static void test_init_dt_and_short_packet(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_mipi_csi_test_dt_low_value, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dtel));
  TEST_ASSERT_EQ(k_ra_mipi_csi_test_dt_high_value, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dteh));

  const uint32_t gsct = *ra_mipi_csi_reg32(k_ra_mipi_csi_off_gsct);
  TEST_ASSERT_EQ(k_ra_mipi_csi_test_threshold, (gsct & (uint32_t)k_ra_mipi_csi_gsct_shth_mask));
...
static void test_init_epct_emct(void)
{
  ...
  TEST_ASSERT_EQ(0x100, (epct & (uint32_t)k_ra_mipi_csi_epct_slp_mask));
  TEST_ASSERT_EQ(0x02000000ULL, (epct & (uint32_t)k_ra_mipi_csi_epct_ssp_mask));

  const uint32_t emct = *ra_mipi_csi_reg32(k_ra_mipi_csi_off_emct);
  TEST_ASSERT_EQ(((uint32_t)k_ra_mipi_csi_vlsien_x2 << (uint32_t)k_ra_mipi_csi_emct_vlsien_shift),
                 (emct & (uint32_t)k_ra_mipi_csi_emct_vlsien_mask));
...
static void test_init_irq_masks(void)
{
  ...
  TEST_ASSERT_EQ(cfg.rx_irq_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_rxie));
  TEST_ASSERT_EQ(cfg.dl_irq_mask[0], *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dlie0));
  TEST_ASSERT_EQ(cfg.dl_irq_mask[1], *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dlie1));
  for (uint8_t vc = 0U; vc < 16U; ++vc) {
    const ra_mipi_csi_off_t off = ra_mipi_csi_vc_off(k_ra_mipi_csi_off_vcie0, vc);
    TEST_ASSERT_EQ(((uint32_t)1U << vc), *ra_mipi_csi_reg32(off));
  }
  TEST_ASSERT_EQ(cfg.pm_irq_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_pmie));
  TEST_ASSERT_EQ(cfg.short_irq_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_gsie));
...
static void test_init_null_cfg(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_mipi_csi_init(nullptr));
...
static void test_init_bad_lanes(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_mipi_csi_init(&cfg));
...
static void test_start_then_stop(void)
{
  ...
  const ra_mipi_csi_config_t cfg = make_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_init(&cfg));
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_start_receive());

  TEST_ASSERT_EQ(
    k_ra_mipi_csi_mct3_rxen_mask,
    (*ra_mipi_csi_reg32(k_ra_mipi_csi_off_mct3) & (uint32_t)k_ra_mipi_csi_mct3_rxen_mask));

  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_mipi_csi_start_receive());

  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_stop_receive());
  TEST_ASSERT_EQ(0, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_mct3));
  TEST_ASSERT_EQ(
    k_ra_mipi_csi_rtct_vsrst_mask,
...
static void test_reset(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_reset());
  TEST_ASSERT_EQ(
    k_ra_mipi_csi_rtct_vsrst_mask,
    (*ra_mipi_csi_reg32(k_ra_mipi_csi_off_rtct) & (uint32_t)k_ra_mipi_csi_rtct_vsrst_mask));
...
static void test_status_get_clear(void)
{
  ...
  *ra_mipi_csi_reg32(k_ra_mipi_csi_off_rxst) = (uint32_t)k_ra_mipi_csi_test_ractdet_marker;
  ...
  uint32_t mask = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_get_status(&mask));
  TEST_ASSERT_EQ(k_ra_mipi_csi_test_ractdet_marker, mask);

  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_clear_status((uint32_t)k_ra_mipi_csi_rxsc_ractdetc_mask));
  TEST_ASSERT_EQ(k_ra_mipi_csi_rxsc_ractdetc_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_rxsc));

  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_mipi_csi_get_status(nullptr));
...
static void test_set_rx_irq_enable(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_mipi_csi_set_rx_irq_enable((uint32_t)k_ra_mipi_csi_rxie_ractdete_mask));
  TEST_ASSERT_EQ(k_ra_mipi_csi_rxie_ractdete_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_rxie));
...
static void test_get_module_irq_status(void)
{
  ...
  *ra_mipi_csi_reg32(k_ra_mipi_csi_off_mist) = (uint32_t)k_ra_mipi_csi_mist_rxs_mask;
  uint32_t out                               = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_get_module_irq_status(&out));
  TEST_ASSERT_EQ(k_ra_mipi_csi_mist_rxs_mask, out);
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_mipi_csi_get_module_irq_status(nullptr));
...
static void test_get_module_info(void)
{
  ...
  *ra_mipi_csi_reg32(k_ra_mipi_csi_off_mcg) = (uint32_t)k_ra_mipi_csi_test_mcg_value;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_get_module_info(&info));
  TEST_ASSERT_EQ(2, info.version);
  TEST_ASSERT_EQ(0xA, info.lanes_max);
  TEST_ASSERT_EQ(0x10, info.fifo_stages);
  TEST_ASSERT_EQ(k_ra_mipi_csi_test_mcg_value, info.raw);

  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_mipi_csi_get_module_info(nullptr));
...
static void test_set_data_type_filter(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_mipi_csi_set_data_type_filter((uint32_t)k_ra_mipi_csi_test_dt_low_value,
...
static void test_set_ecc_mode(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_set_ecc_mode(true, true));
  uint32_t mct0 = *ra_mipi_csi_reg32(k_ra_mipi_csi_off_mct0);
  TEST_ASSERT((mct0 & (uint32_t)k_ra_mipi_csi_mct0_eccv13_mask) != 0UL);
  TEST_ASSERT((mct0 & (uint32_t)k_ra_mipi_csi_mct0_lfsren_mask) != 0UL);

  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_set_ecc_mode(false, false));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_start_receive());
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_mipi_csi_set_ecc_mode(true, true));
...
static void test_set_frame_error_mode(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_set_frame_error_mode(true, true, true));
  const uint32_t mct0 = *ra_mipi_csi_reg32(k_ra_mipi_csi_off_mct0);
  TEST_ASSERT((mct0 & (uint32_t)k_ra_mipi_csi_mct0_zlmd_mask) != 0UL);
  TEST_ASSERT((mct0 & (uint32_t)k_ra_mipi_csi_mct0_edmd_mask) != 0UL);
  TEST_ASSERT((mct0 & (uint32_t)k_ra_mipi_csi_mct0_rvmd_mask) != 0UL);
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_start_receive());
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_mipi_csi_set_frame_error_mode(false, false, false));
...
static void test_set_epd(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_set_epd(true, true, 0x123U, 0x456U));
  const uint32_t epct = *ra_mipi_csi_reg32(k_ra_mipi_csi_off_epct);
  TEST_ASSERT((epct & (uint32_t)k_ra_mipi_csi_epct_epden_mask) != 0UL);
  TEST_ASSERT((epct & (uint32_t)k_ra_mipi_csi_epct_epdop_mask) != 0UL);
  TEST_ASSERT_EQ(0x123, (epct & (uint32_t)k_ra_mipi_csi_epct_slp_mask));

  /* Reject bad spacers. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_mipi_csi_set_epd(true, true, 0x8000U, 0U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_mipi_csi_set_epd(true, true, 0U, 0x8000U));
...
static void test_set_lrte(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_set_lrte(k_ra_mipi_csi_vlsien_x4, true));
  const uint32_t emct = *ra_mipi_csi_reg32(k_ra_mipi_csi_off_emct);
  TEST_ASSERT((emct & (uint32_t)k_ra_mipi_csi_emct_eotpen_mask) != 0UL);
  TEST_ASSERT_EQ(((uint32_t)k_ra_mipi_csi_vlsien_x4 << (uint32_t)k_ra_mipi_csi_emct_vlsien_shift),
                 (emct & (uint32_t)k_ra_mipi_csi_emct_vlsien_mask));

  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_mipi_csi_set_lrte((ra_mipi_csi_vlsien_t)4U, false));
...
static void test_dl_status_paths(void)
{
  ...
  *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dlst1) = (uint32_t)k_ra_mipi_csi_dlst_ess_mask;
  ...
  uint32_t v = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_dl_get_status(0U, &v));
  TEST_ASSERT_EQ(k_ra_mipi_csi_dlst_esh_mask, v);
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_dl_get_status(1U, &v));
  TEST_ASSERT_EQ(k_ra_mipi_csi_dlst_ess_mask, v);

  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_dl_clear_status(0U, (uint32_t)k_ra_mipi_csi_dlsc_eshc_mask));
  TEST_ASSERT_EQ(k_ra_mipi_csi_dlsc_eshc_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dlsc0));

  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_dl_set_irq_enable(0U, (uint32_t)k_ra_mipi_csi_dlie_all_mask));
  TEST_ASSERT_EQ(k_ra_mipi_csi_dlie_all_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dlie0));

  /* Bad lane index. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_mipi_csi_dl_get_status(2U, &v));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_mipi_csi_dl_clear_status(2U, 0U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_mipi_csi_dl_set_irq_enable(2U, 0U));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_mipi_csi_dl_get_status(0U, nullptr));
...
static void test_vc_status_paths(void)
{
  ...
  for (uint8_t vc = 0U; vc < 16U; ++vc) {
    ...
    *ra_mipi_csi_reg32(st_off)     = ((uint32_t)1U << vc);
    ...
    uint32_t v = 0U;
    TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_vc_get_status(vc, &v));
    TEST_ASSERT_EQ(((uint32_t)1U << vc), v);

    TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_vc_clear_status(vc, ((uint32_t)1U << vc)));
    const ra_mipi_csi_off_t clr_off = ra_mipi_csi_vc_off(k_ra_mipi_csi_off_vcsc0, vc);
    TEST_ASSERT_EQ(((uint32_t)1U << vc), *ra_mipi_csi_reg32(clr_off));

    TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_vc_set_irq_enable(vc, ((uint32_t)1U << vc)));
    const ra_mipi_csi_off_t ie_off = ra_mipi_csi_vc_off(k_ra_mipi_csi_off_vcie0, vc);
    TEST_ASSERT_EQ(((uint32_t)1U << vc), *ra_mipi_csi_reg32(ie_off));
  }

  uint32_t v = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_mipi_csi_vc_get_status(16U, &v));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_mipi_csi_vc_clear_status(16U, 0U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_mipi_csi_vc_set_irq_enable(16U, 0U));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_mipi_csi_vc_get_status(0U, nullptr));
...
static void test_pm_status_paths(void)
{
  ...
  *ra_mipi_csi_reg32(k_ra_mipi_csi_off_pmst) = (uint32_t)k_ra_mipi_csi_test_pmst_marker;
  ...
  uint32_t v = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_pm_get_status(&v));
  TEST_ASSERT_EQ(k_ra_mipi_csi_test_pmst_marker, v);

  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_pm_clear_status((uint32_t)k_ra_mipi_csi_pmsc_all_mask));
  TEST_ASSERT_EQ(k_ra_mipi_csi_pmsc_all_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_pmsc));

  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_pm_set_irq_enable((uint32_t)k_ra_mipi_csi_pmie_all_mask));
  TEST_ASSERT_EQ(k_ra_mipi_csi_pmie_all_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_pmie));

  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_mipi_csi_pm_get_status(nullptr));
...
static void test_short_packet_configure(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_mipi_csi_short_packet_configure((uint8_t)k_ra_mipi_csi_test_threshold, true));
  const uint32_t gsct = *ra_mipi_csi_reg32(k_ra_mipi_csi_off_gsct);
  TEST_ASSERT_EQ(k_ra_mipi_csi_test_threshold, (gsct & (uint32_t)k_ra_mipi_csi_gsct_shth_mask));
  TEST_ASSERT((gsct & (uint32_t)k_ra_mipi_csi_gsct_gfif_mask) != 0UL);

  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_mipi_csi_short_packet_configure(0x10U, false));

  TEST_ASSERT_EQ(k_ra_ok,
                 ra_mipi_csi_short_packet_set_irq_enable((uint32_t)k_ra_mipi_csi_gsie_all_mask));
  TEST_ASSERT_EQ(k_ra_mipi_csi_gsie_all_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_gsie));
...
static void test_short_packet_status(void)
{
  ...
  *ra_mipi_csi_reg32(k_ra_mipi_csi_off_gsst) =
  ...
  uint32_t v = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_short_packet_get_status(&v));
  TEST_ASSERT((v & (uint32_t)k_ra_mipi_csi_gsst_gov_mask) != 0UL);

  TEST_ASSERT_EQ(k_ra_ok,
                 ra_mipi_csi_short_packet_clear_status((uint32_t)k_ra_mipi_csi_gssc_govc_mask));
  TEST_ASSERT_EQ(k_ra_mipi_csi_gssc_govc_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_gssc));

  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_mipi_csi_short_packet_get_status(nullptr));
...
static void test_read_short_packet(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_empty, ra_mipi_csi_read_short_packet(&pkt));
...
static void test_short_packet_clear_fifo(void)
{
  ...
  *ra_mipi_csi_reg32(k_ra_mipi_csi_off_gsst) = (uint32_t)k_ra_mipi_csi_gsst_gcd_mask;
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_short_packet_clear_fifo());
  /* GSIU.GFCLR must have been released back to 0. */
  TEST_ASSERT_EQ(0, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_gsiu));

  /* Now without GCD set -> hw_timeout. */
  *ra_mipi_csi_reg32(k_ra_mipi_csi_off_gsst) = 0U;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_mipi_csi_short_packet_clear_fifo());
  TEST_ASSERT_EQ(0, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_gsiu));
...
static void test_short_packet_re_enable(void)
{
  ...
  const ra_mipi_csi_config_t cfg = make_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_init(&cfg));
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_short_packet_re_enable_store());
  TEST_ASSERT(
...
static void test_attach_and_dispatch(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_mipi_csi_attach_handler(stub_event_cb, (void*)(uintptr_t)k_ra_mipi_csi_test_ctx_marker));

  *ra_mipi_csi_reg32(k_ra_mipi_csi_off_rxst) = (uint32_t)k_ra_mipi_csi_test_ractdet_marker;
  ra_mipi_csi_dispatch();

  TEST_ASSERT_EQ(1, s_cb_count);
  TEST_ASSERT_EQ(k_ra_mipi_csi_test_ractdet_marker, s_cb_last_rxst);
  TEST_ASSERT_EQ((uintptr_t)k_ra_mipi_csi_test_ctx_marker, (uintptr_t)s_cb_last_ctx);

  TEST_ASSERT_EQ(k_ra_mipi_csi_rxsc_ractdetc_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_rxsc));

  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_attach_handler(nullptr, nullptr));
  ra_mipi_csi_dispatch();
  TEST_ASSERT_EQ(1, s_cb_count);
...
static void test_dispatch_dl(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_mipi_csi_attach_dl_handler(stub_dl_cb, (void*)(uintptr_t)k_ra_mipi_csi_test_dl_marker));
  ...
  *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dlst1) = (uint32_t)k_ra_mipi_csi_dlst_ess_mask;
  ...
  TEST_ASSERT_EQ(k_ra_mipi_csi_dlst_esh_mask, s_dl_last_mask[0]);
  TEST_ASSERT_EQ(k_ra_mipi_csi_dlst_ess_mask, s_dl_last_mask[1]);
  /* DLSC should have been written with the W1C-able subset. */
  TEST_ASSERT_EQ(((uint32_t)k_ra_mipi_csi_dlst_esh_mask & (uint32_t)k_ra_mipi_csi_dlsc_all_mask),
                 *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dlsc0));
...
static void test_dispatch_vc(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_mipi_csi_attach_vc_handler(stub_vc_cb, (void*)(uintptr_t)k_ra_mipi_csi_test_vc_marker));
  ...
  *ra_mipi_csi_reg32(st5) =
  ...
  TEST_ASSERT_EQ(k_ra_mipi_csi_vcst_crc_mask, s_vc_last_mask[0]);
  TEST_ASSERT_EQ(1, s_vc_calls[5]);
  TEST_ASSERT_EQ(k_ra_mipi_csi_vcst_lsr_mask, s_vc_last_mask[5]);
  /* Generic summary callback (vc=0xFF -> idx 16) carries MLF only. */
  TEST_ASSERT_EQ(1, s_vc_calls[16]);
  TEST_ASSERT_EQ(k_ra_mipi_csi_vcst_mlf_mask, s_vc_last_mask[16]);
  ...
  TEST_ASSERT_EQ(k_ra_mipi_csi_vcst_crc_mask, *ra_mipi_csi_reg32(clr0));
...
static void test_dispatch_pm(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_mipi_csi_attach_pm_handler(stub_pm_cb, (void*)(uintptr_t)k_ra_mipi_csi_test_pm_marker));

  *ra_mipi_csi_reg32(k_ra_mipi_csi_off_pmst) = (uint32_t)k_ra_mipi_csi_test_pmst_marker;
  ra_mipi_csi_dispatch_pm();

  TEST_ASSERT_EQ(1, s_pm_calls);
  TEST_ASSERT_EQ(k_ra_mipi_csi_test_pmst_marker, s_pm_last_mask);
  /* PMSC must have been written with only the lower 8 bits. */
  TEST_ASSERT_EQ(((uint32_t)k_ra_mipi_csi_test_pmst_marker & (uint32_t)k_ra_mipi_csi_pmst_w1c_mask),
                 *ra_mipi_csi_reg32(k_ra_mipi_csi_off_pmsc));
...
static void test_dispatch_short_packet(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_mipi_csi_attach_short_packet_handler(stub_gst_cb,
...
static void test_power_transition(void)
{
  ...
  const ra_mipi_csi_config_t cfg = make_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_init(&cfg));
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_enter_stop());
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_exit_stop());
...
static void test_deinit(void)
{
  ...
  *ra_mipi_csi_reg32(k_ra_mipi_csi_off_gsie)  = 0xFFFFFFFFUL;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_deinit());

  TEST_ASSERT_EQ(0, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_mct3));
  TEST_ASSERT_EQ(0, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_rxie));
  TEST_ASSERT_EQ(0, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dlie0));
  TEST_ASSERT_EQ(0, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dlie1));
  TEST_ASSERT_EQ(0, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_pmie));
  TEST_ASSERT_EQ(0, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_gsie));
  for (uint8_t vc = 0U; vc < 16U; ++vc) {
    const ra_mipi_csi_off_t off = ra_mipi_csi_vc_off(k_ra_mipi_csi_off_vcie0, vc);
    TEST_ASSERT_EQ(0, *ra_mipi_csi_reg32(off));
  }
...
static void test_set_virtual_channels(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_mipi_csi_set_virtual_channels((uint16_t)k_ra_mipi_csi_test_vc_keep_mask));

  /* VC0 + VC2 retain (saved was zero, so default mask was restored). */
  TEST_ASSERT_EQ(k_ra_mipi_csi_test_vc_seed,
                 *ra_mipi_csi_reg32(ra_mipi_csi_vc_off(k_ra_mipi_csi_off_vcie0, 0U)));
  /* VC1, VC3..VC15 zeroed. */
  TEST_ASSERT_EQ(0, *ra_mipi_csi_reg32(ra_mipi_csi_vc_off(k_ra_mipi_csi_off_vcie0, 1U)));
  TEST_ASSERT_EQ(0, *ra_mipi_csi_reg32(ra_mipi_csi_vc_off(k_ra_mipi_csi_off_vcie0, 5U)));

  /* Re-enabling VC1 restores the saved value. */
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_set_virtual_channels((uint16_t)0xFFFFU));
  TEST_ASSERT_EQ(k_ra_mipi_csi_test_vc_seed,
                 *ra_mipi_csi_reg32(ra_mipi_csi_vc_off(k_ra_mipi_csi_off_vcie0, 1U)));
...
static void test_set_virtual_channels_empty_mask(void)
{
  ...
  const ra_mipi_csi_config_t cfg = make_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_init(&cfg));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_mipi_csi_set_virtual_channels(0U));
...
static void test_set_data_format_per_vc(void)
{
  ...
  /* VC0 -> RGB888 (DTEH bit). */
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_set_data_format(0U, k_ra_mipi_csi_format_rgb888));
  TEST_ASSERT_EQ(k_ra_mipi_csi_dteh_rgb888_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dteh));
  TEST_ASSERT_EQ(0, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dtel));

  /* VC1 -> YUV422_8 (DTEL bit). DTEH stays set. */
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_set_data_format(1U, k_ra_mipi_csi_format_yuv422_8));
  TEST_ASSERT_EQ(k_ra_mipi_csi_dtel_yuv422_8_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dtel));
  TEST_ASSERT_EQ(k_ra_mipi_csi_dteh_rgb888_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dteh));

  /* Disable VC0 -- DTEH clears. */
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_set_data_format(0U, k_ra_mipi_csi_format_off));
  TEST_ASSERT_EQ(0, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dteh));
  TEST_ASSERT_EQ(k_ra_mipi_csi_dtel_yuv422_8_mask, *ra_mipi_csi_reg32(k_ra_mipi_csi_off_dtel));
...
static void test_set_data_format_bad_args(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_mipi_csi_set_data_format(16U, k_ra_mipi_csi_format_rgb888));

  /* Format unsupported on RA8D2 DTEL/DTEH (RAW10). */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_mipi_csi_set_data_format(0U, k_ra_mipi_csi_format_raw10));
...
static void test_attach_error_handler(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_attach_error_handler(stub_err_cb, ctx_token));
  ...
  *ra_mipi_csi_reg32(st_off) =
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_attach_error_handler(nullptr, nullptr));
  *ra_mipi_csi_reg32(k_ra_mipi_csi_off_mist) =
    (uint32_t)((uint32_t)1U << (uint32_t)k_ra_mipi_csi_mist_vc_shift);
...
static void test_error_handler_no_errors_silent(void)
{
  ...
  s_err_calls = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_csi_attach_error_handler(stub_err_cb, nullptr));
