...
ra_err_t ra_epub_xml_parse_ncx(const uint8_t* xml_bytes, std::size_t xml_len, ra_epub_book_t* book);

ra_err_t ra_epub_xml_parse_nav(const uint8_t* xml_bytes, std::size_t xml_len, ra_epub_book_t* book);
...
void assert_shared_toc(const ra_epub_book_t& book)
{
...
void test_parse_ncx_and_nav(void)
{
  ...
  assert(ra_epub_xml_parse_ncx(ncx, std::strlen(k_ncx_doc), &book) == k_ra_ok);
  assert_shared_toc(book);

  std::memset(&book, 0, sizeof(book));
  const auto* nav = reinterpret_cast<const uint8_t*>(k_nav_doc);
  assert(ra_epub_xml_parse_nav(nav, std::strlen(k_nav_doc), &book) == k_ra_ok);
  assert_shared_toc(book);
 ...
 *
 * `ra_epub_xml_parse_opf` must record which navigation document to load:
...
void test_parse_opf_toc_source(void)
{
  ...
  assert(ra_epub_xml_parse_opf(nav, std::strlen(k_opf_with_nav), &book) == k_ra_ok);
  assert(book.toc_kind == static_cast<uint8_t>(k_ra_epub_toc_nav));
  assert(std::strcmp(book.toc_path, "nav.xhtml") == 0);

  std::memset(&book, 0, sizeof(book));
  const auto* ncx = reinterpret_cast<const uint8_t*>(k_opf_with_ncx);
  assert(ra_epub_xml_parse_opf(ncx, std::strlen(k_opf_with_ncx), &book) == k_ra_ok);
  assert(book.toc_kind == static_cast<uint8_t>(k_ra_epub_toc_ncx));
  ...
  assert(ra_epub_xml_parse_opf(plain, std::strlen(k_valid_opf), &book) == k_ra_ok);
  assert(book.toc_kind == static_cast<uint8_t>(k_ra_epub_toc_none));
...
void test_parse_toc_guards(void)
{
  ...
  assert(ra_epub_xml_parse_ncx(nullptr, 16U, &book) == k_ra_err_null_ptr);
  assert(ra_epub_xml_parse_ncx(ncx, 0U, &book) == k_ra_err_invalid_size);
  assert(ra_epub_xml_parse_nav(nullptr, 16U, &book) == k_ra_err_null_ptr);
  ...
  assert(ra_epub_xml_parse_ncx(bad_ncx, std::strlen(k_not_ncx), &book) ==
         k_ra_err_validation_failed);
  ...
  assert(ra_epub_xml_parse_nav(bad_nav, std::strlen(k_not_nav), &book) ==
         k_ra_err_validation_failed);
...
/**
 * @brief Truncation length copy_bounded() yields for a `k_ra_epub_meta_len`
 *        destination: `cap - 1` payload bytes plus the NUL terminator.
 */
constexpr std::size_t k_meta_truncated_len = static_cast<std::size_t>(k_ra_epub_meta_len) - 1U;

/* NCX whose first navPoint <text> is longer than `k_ra_epub_meta_len`, so
...
void test_mcdc_copy_bounded_truncation(void)
{
  ...
  assert(ra_epub_xml_parse_ncx(ncx, std::strlen(k_ncx_long_title), &book) == k_ra_ok);
 ...
 * Driven through ``ra_epub_xml_parse_opf`` spine resolution.
...
void test_mcdc_manifest_item_missing_id(void)
{
  ...
  assert(ra_epub_xml_parse_opf(xml, std::strlen(k_opf_manifest_item_no_id), &book) == k_ra_ok);
...
void test_mcdc_cover_by_meta(void)
{
  ...
  assert(ra_epub_xml_parse_opf(xml_ok, std::strlen(k_opf_meta_cover_ok), &book) == k_ra_ok);
  ...
  assert(ra_epub_xml_parse_opf(xml_no_name, std::strlen(k_opf_meta_no_name), &book) == k_ra_ok);
  ...
  assert(ra_epub_xml_parse_opf(xml_no_content, std::strlen(k_opf_meta_no_content), &book) ==
         k_ra_ok);
  assert(book.cover_path[0] == '\0');
  ...
  assert(ra_epub_xml_parse_opf(xml_other, std::strlen(k_opf_meta_other_name), &book) == k_ra_ok);
  assert(book.cover_path[0] == '\0');
 ...
 * Driven through ``ra_epub_xml_parse_opf`` TOC-source selection.
...
void test_mcdc_nav_manifest_props_not_nav(void)
{
  ...
  assert(ra_epub_xml_parse_opf(xml, std::strlen(k_opf_props_not_nav), &book) == k_ra_ok);
  assert(book.toc_kind == static_cast<uint8_t>(k_ra_epub_toc_none));
  assert(book.toc_path[0] == '\0');
 ...
 * 2 conditions). Driven through ``ra_epub_xml_parse_opf``.
...
void test_mcdc_collect_fonts_href_and_type(void)
{
  ...
  assert(ra_epub_xml_parse_opf(xml, std::strlen(k_opf_font_and_no_href), &book) == k_ra_ok);
...
void test_mcdc_nav_by_type_untyped_first(void)
{
  ...
  assert(ra_epub_xml_parse_nav(nav, std::strlen(k_nav_untyped_first), &book) == k_ra_ok);
  assert(book.toc_count == 1U);
  assert(std::strcmp(book.toc[0].title, "Chapter One") == 0);
  assert(std::strcmp(book.toc[0].href, "c1.xhtml") == 0);
 ...
 * ``ra_epub_xml_parse_ncx`` (xml_shim.cpp line 610; 2 conditions).
 ...
 * ``ra_epub_xml_parse_nav`` (xml_shim.cpp line 638; 2 conditions).
...
void test_mcdc_ncx_nav_book_null(void)
{
  ...
  const auto* ncx = reinterpret_cast<const uint8_t*>(k_ncx_doc);
  assert(ra_epub_xml_parse_ncx(ncx, std::strlen(k_ncx_doc), nullptr) == k_ra_err_null_ptr);

  const auto* nav = reinterpret_cast<const uint8_t*>(k_nav_doc);
  assert(ra_epub_xml_parse_nav(nav, std::strlen(k_nav_doc), nullptr) == k_ra_err_null_ptr);
