/**
 * @file test_ra_sdcard_cov.c
 * @brief Coverage-extension tests for ra_sdcard.c -- targets 26 uncovered lines
 *
 * @details
 * Dedicated test binary that exercises the error and alternate-path branches
 * left uncovered by test_ra_sdcard.c.  Covers:
 *
 ...
 *
 * @invariant inst < k_ra_sdhi_instance_count when handler is armed.
 *
 * @since 0.1.0
 */
typedef struct {
 ...
 * k_ra_err_hw_init_failed at source line 293.
 *
 * @par MC/DC:
 ...
 *   -> k_ra_err_hw_init_failed returned.
 *
 * @since 0.1.0
 */
 ...
 * ra_sdhi_send_command returns k_ra_err_hw_timeout.  internal_run_acmd41
 * propagates it at line 164.  ra_sdcard_init then calls ra_sdhi_deinit
 * (line 414) and returns at line 415.
 *
 * @par MC/DC:
 * Decision: ``e55 != k_ra_ok`` -- single condition.
 * - Vector 1 (existing success tests): e55=k_ra_ok -> false.
 * - Vector 2 (this test): e55=k_ra_err_hw_timeout -> true -> return at line 164.
 *
 * @since 0.1.0
 */
 ...
 *
 * @par MC/DC:
 * Decision: ``e41 != k_ra_ok`` -- single condition.
 * - Vector 1 (existing success tests): e41=k_ra_ok -> false.
 * - Vector 2 (this test): e41=k_ra_err_hw_timeout -> true -> return at line 173.
 *
 * @since 0.1.0
 */
 ...
 * returns k_ra_err_hw_init_failed.
 *
 ...
 * - ``e55 != k_ra_ok``: false every iteration (CMD55 succeeds).
 * - ``e41 != k_ra_ok``: false every iteration (ACMD41 succeeds).
 ...
 *
 * After a successful init, ra_sdcard_read_blocks is called.  The SDSC
 ...
 * - ``s_sdcard.type == k_ra_sdcard_type_sdsc``: true -> line 467.
 *
 * @since 0.1.0
 */
static void test_cov_csd_v1_sdsc_card(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_sdcard_type_sdsc, t);
 ...
 * SDHC/SDXC boundary (67,108,864) and yields k_ra_sdcard_type_sdxc.
 *
 * @par MC/DC:
 * Decision in internal_classify:
...
static void test_cov_csd_v2_sdxc_card(void)
{
  ...
  ra_sdcard_card_type_t t = k_ra_sdcard_type_unknown;
  TEST_ASSERT_EQ(k_ra_ok, ra_sdcard_get_type(&t));
  TEST_ASSERT_EQ(k_ra_sdcard_type_sdxc, t);
 ...
 * internal_decode_csd returns k_ra_err_hw_init_failed at line 247.
 * internal_sdcard_publish_and_select returns it at line 346.
 * ra_sdcard_init calls ra_sdhi_deinit at line 423 and returns at line 424.
 *
 * @par MC/DC:
 * Decisions in internal_decode_csd:
