...
static void test_output_init_happy_low(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_pfs_mask_pdr, *pfs);
...
static void test_output_init_invalid_port(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_port, err);
...
static void test_output_init_invalid_pin(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_pin, err);
...
static void test_output_init_conflict(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpio_output_init((ra_port_pin_t)k_ra_gpio_test_pin_alt, k_ra_level_low));
  /* Second claim of the same pin must fail with gpio_conflict. */
  TEST_ASSERT_EQ(k_ra_err_gpio_conflict,
                 ra_gpio_output_init((ra_port_pin_t)k_ra_gpio_test_pin_alt, k_ra_level_high));
...
static void test_input_init_pull_up(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_pfs_mask_pcr, *pfs);
...
static void test_input_init_invalid_port(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_port,
                 ra_gpio_input_init((ra_port_pin_t)k_ra_gpio_test_pin_bad_port, k_ra_pull_none));
...
static void test_input_init_invalid_pin(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_pin,
                 ra_gpio_input_init((ra_port_pin_t)k_ra_gpio_test_pin_bad_pin, k_ra_pull_none));
...
static void test_write_high_sets_posr(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_gpio_write(pin, k_ra_level_high));
...
static void test_write_low_sets_porr(void)
{
  ...
  const ra_port_pin_t pin = (ra_port_pin_t)k_ra_gpio_test_pin_alt;
  TEST_ASSERT_EQ(k_ra_ok, ra_gpio_write(pin, k_ra_level_low));
...
static void test_write_invalid_port(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_port,
                 ra_gpio_write((ra_port_pin_t)k_ra_gpio_test_pin_bad_port, k_ra_level_low));
...
static void test_write_invalid_pin(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_pin,
                 ra_gpio_write((ra_port_pin_t)k_ra_gpio_test_pin_bad_pin, k_ra_level_low));
...
static void test_toggle_when_low(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_gpio_toggle(pin));
  const uint32_t expected = (uint32_t)(1UL << (uint32_t)k_ra_pin_2);
...
static void test_toggle_invalid_port(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_port,
                 ra_gpio_toggle((ra_port_pin_t)k_ra_gpio_test_pin_bad_port));
...
static void test_toggle_invalid_pin(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_pin,
                 ra_gpio_toggle((ra_port_pin_t)k_ra_gpio_test_pin_bad_pin));
...
static void test_read_high_and_low(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_gpio_read(pin, &got_high));
  TEST_ASSERT_EQ(k_ra_level_high, got_high);

  /* And cleared. */
  reg->PCNTR2        = 0U;
  ra_level_t got_low = k_ra_level_high;
  TEST_ASSERT_EQ(k_ra_ok, ra_gpio_read(pin, &got_low));
  TEST_ASSERT_EQ(k_ra_level_low, got_low);
...
static void test_read_null_out(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_gpio_read((ra_port_pin_t)k_ra_gpio_test_pin_valid_low, nullptr));
...
static void test_read_invalid_port(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_port,
                 ra_gpio_read((ra_port_pin_t)k_ra_gpio_test_pin_bad_port, &got));
...
static void test_read_invalid_pin(void)
{
  ...
  ra_level_t got = k_ra_level_low;
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_pin,
                 ra_gpio_read((ra_port_pin_t)k_ra_gpio_test_pin_bad_pin, &got));
...
static void test_release_pin(void)
{
  ...
  const ra_port_pin_t pin = (ra_port_pin_t)k_ra_gpio_test_pin_alt;
  TEST_ASSERT_EQ(k_ra_ok, ra_gpio_output_init(pin, k_ra_level_low));
  TEST_ASSERT(ra_pin_validator_is_claimed(pin));

  TEST_ASSERT_EQ(k_ra_ok, ra_gpio_release(pin));
  TEST_ASSERT(!ra_pin_validator_is_claimed(pin));
...
static void test_route_peripheral_happy(void)
{
  ...
  const ra_port_pin_t pin = (ra_port_pin_t)k_ra_gpio_test_pin_alt;
  TEST_ASSERT_EQ(k_ra_ok, ra_pfs_route_peripheral(pin, k_ra_psel_sci_async, "TEST"));
...
static void test_route_peripheral_null_owner(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_pfs_route_peripheral((ra_port_pin_t)k_ra_gpio_test_pin_alt, k_ra_psel_sci_async, nullptr));
...
static void test_route_peripheral_invalid_port(void)
{
  ...
  extern ra_err_t ra_pfs_route_peripheral(ra_port_pin_t pin, ra_psel_t psel, const char* owner);
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_port,
                 ra_pfs_route_peripheral((ra_port_pin_t)k_ra_gpio_test_pin_bad_port,
                                         k_ra_psel_sci_async,
                                         "TEST"));
...
static void test_route_peripheral_invalid_pin(void)
{
  ...
  extern ra_err_t ra_pfs_route_peripheral(ra_port_pin_t pin, ra_psel_t psel, const char* owner);
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_pin,
                 ra_pfs_route_peripheral((ra_port_pin_t)k_ra_gpio_test_pin_bad_pin,
                                         k_ra_psel_sci_async,
                                         "TEST"));
...
static void test_route_peripheral_conflict(void)
{
  ...
  extern ra_err_t     ra_pfs_route_peripheral(ra_port_pin_t pin, ra_psel_t psel, const char* owner);
  const ra_port_pin_t pin = (ra_port_pin_t)k_ra_gpio_test_pin_alt;
  TEST_ASSERT_EQ(k_ra_ok, ra_pfs_route_peripheral(pin, k_ra_psel_sci_async, "FIRST"));
  TEST_ASSERT_EQ(k_ra_err_gpio_conflict,
                 ra_pfs_route_peripheral(pin, k_ra_psel_sci_async, "SECOND"));
...
static void test_vtable_output_init_and_write(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    g_ra_gpio_pin_interface.output_init(g_ra_gpio_pin_interface.ctx, pin, k_ra_level_low));
  TEST_ASSERT_EQ(k_ra_ok,
                 g_ra_gpio_pin_interface.write(g_ra_gpio_pin_interface.ctx, pin, k_ra_level_high));
...
static void test_vtable_read_and_toggle(void)
{
  ...
  const ra_port_pin_t pin = (ra_port_pin_t)k_ra_gpio_test_pin_alt;
  ra_level_t          got = k_ra_level_low;
  TEST_ASSERT_EQ(k_ra_ok, g_ra_gpio_pin_interface.read(g_ra_gpio_pin_interface.ctx, pin, &got));
  TEST_ASSERT_EQ(k_ra_level_low, got);
  TEST_ASSERT_EQ(k_ra_ok, g_ra_gpio_pin_interface.toggle(g_ra_gpio_pin_interface.ctx, pin));
...
static void test_gpio_attach_irq_happy(void)
{
  ...
  uint8_t irqcr = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_icu_read_irqcr((uint8_t)k_ra_gpio_test_irq_num, &irqcr));
...
static void test_gpio_attach_irq_null_cfg(void)
{
  ...
  const ra_err_t err = ra_gpio_attach_irq((ra_port_pin_t)k_ra_gpio_test_pin_alt,
                                          ...
                                          stub_irq_handler,
                                          nullptr);
  TEST_ASSERT_EQ(k_ra_err_null_ptr, err);
...
static void test_gpio_attach_irq_bad_num(void)
{
  ...
  const ra_err_t          err = ra_gpio_attach_irq((ra_port_pin_t)k_ra_gpio_test_pin_alt,
                                                   ...
                                                   stub_irq_handler,
                                                   nullptr);
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, err);
...
static void test_gpio_detach_irq_happy(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_gpio_detach_irq(pin, (uint8_t)k_ra_gpio_test_irq_num));
  TEST_ASSERT(!ra_pin_validator_is_claimed(pin));
...
static void test_gpio_detach_irq_not_attached(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_not_found, err);
