...
static void test_start_happy(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_gpt_start_free_run((uint8_t)k_ra_gpt_test_channel_valid, (uint32_t)k_ra_gpt_test_period));
  volatile r_gpt_channel_regs_t* reg = ra_gpt((uint8_t)k_ra_gpt_test_channel_valid);
  TEST_ASSERT_NOT_NULL((void*)reg);
  TEST_ASSERT_EQ(k_ra_gpt_test_period, reg->GTPR);
  TEST_ASSERT_EQ(0, reg->GTCNT);
  TEST_ASSERT_EQ(k_ra_gpt_test_gtcr_saw, reg->GTCR);
  TEST_ASSERT_EQ(k_ra_gpt_test_gtstr1, reg->GTSTR);
...
static void test_start_last_channel(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_start_free_run((uint8_t)k_ra_gpt_test_channel_last, 0U));
...
static void test_start_bad_channel(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_start_free_run((uint8_t)k_ra_gpt_test_channel_bad, 0U));
...
static void test_start_huge_channel(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_start_free_run((uint8_t)k_ra_gpt_test_channel_huge, 0U));
...
static void test_stop_happy(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_gpt_start_free_run((uint8_t)k_ra_gpt_test_channel_valid, (uint32_t)k_ra_gpt_test_period));
  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_stop((uint8_t)k_ra_gpt_test_channel_valid));

  volatile r_gpt_channel_regs_t* reg = ra_gpt((uint8_t)k_ra_gpt_test_channel_valid);
  TEST_ASSERT_EQ(k_ra_gpt_test_gtstp1, reg->GTSTP);
...
static void test_stop_bad_channel(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_stop((uint8_t)k_ra_gpt_test_channel_bad));
...
static void test_read_happy(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_gpt_test_count, out);
...
static void test_read_null_out(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_read((uint8_t)k_ra_gpt_test_channel_valid, nullptr));
...
static void test_read_bad_channel(void)
{
  ...
  uint32_t out = 0U;
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_read((uint8_t)k_ra_gpt_test_channel_bad, &out));
...
static void test_gpt_init_configured(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_init((uint8_t)k_ra_gpt_test_channel_valid, &cfg));

  volatile r_gpt_channel_regs_t* reg = ra_gpt((uint8_t)k_ra_gpt_test_channel_valid);
  TEST_ASSERT_NOT_NULL((void*)reg);
  TEST_ASSERT_EQ(k_ra_gpt_test_period, reg->GTPR);
  TEST_ASSERT_EQ(k_ra_gpt_test_period, reg->GTPBR);
  TEST_ASSERT_EQ(k_ra_gpt_test_duty_a, reg->GTCCR[0]);
  TEST_ASSERT_EQ(k_ra_gpt_test_duty_b, reg->GTCCR[1]);
...
static void test_gpt_init_null_cfg(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_init((uint8_t)k_ra_gpt_test_channel_valid, nullptr));
...
static void test_gpt_init_bad_channel(void)
{
  ...
  const ra_gpt_cfg_t cfg = make_gpt_cfg();
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_init((uint8_t)k_ra_gpt_test_channel_bad, &cfg));
...
static void test_gpt_deinit(void)
{
  ...
  const ra_gpt_cfg_t cfg = make_gpt_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_init((uint8_t)k_ra_gpt_test_channel_valid, &cfg));
  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_deinit((uint8_t)k_ra_gpt_test_channel_valid));
  TEST_END("gpt deinit");
}
...
static void test_gpt_deinit_bad_channel(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_deinit((uint8_t)k_ra_gpt_test_channel_bad));
...
static void test_gpt_set_period(void)
{
  ...
  const ra_gpt_cfg_t cfg = make_gpt_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_init((uint8_t)k_ra_gpt_test_channel_valid, &cfg));
  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_set_period((uint8_t)k_ra_gpt_test_channel_valid, 0xAABBCCDDUL));
...
static void test_gpt_set_duty_a_and_b(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_set_duty((uint8_t)k_ra_gpt_test_channel_valid, k_ra_gpt_ccr_a, 0x1234U));
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_set_duty((uint8_t)k_ra_gpt_test_channel_valid, k_ra_gpt_ccr_b, 0x5678U));
...
static void test_gpt_set_duty_bad_sel(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_gpt_set_duty((uint8_t)k_ra_gpt_test_channel_valid, (ra_gpt_ccr_sel_t)9U, 0U));
...
static void test_gpt_status_read_and_clear(void)
{
  ...
  uint32_t mask = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_get_status((uint8_t)k_ra_gpt_test_channel_valid, &mask));
  TEST_ASSERT_EQ(((uint32_t)k_ra_gpt_status_overflow | (uint32_t)k_ra_gpt_status_ccra), mask);

  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_gpt_clear_status((uint8_t)k_ra_gpt_test_channel_valid, (uint32_t)k_ra_gpt_status_overflow));

  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_get_status((uint8_t)k_ra_gpt_test_channel_valid, &mask));
  TEST_ASSERT_EQ(k_ra_gpt_status_ccra, mask);
...
static void test_gpt_attach_and_dispatch_ovf(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_attach_handler((uint8_t)k_ra_gpt_test_channel_valid,
                                       stub_gpt_cb,
  ...
  TEST_ASSERT_EQ(k_ra_gpt_status_overflow, s_gpt_cb_last_mask);
...
static void test_gpt_dispatch_und_ccra_ccrb(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_attach_handler((uint8_t)k_ra_gpt_test_channel_valid, stub_gpt_cb, nullptr));
...
static void test_gpt_dispatch_no_handler(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_attach_handler((uint8_t)k_ra_gpt_test_channel_valid, nullptr, nullptr));
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_attach_handler((uint8_t)k_ra_gpt_test_channel_middle, nullptr, nullptr));
...
static void test_gpt_attach_bad_channel(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_gpt_attach_handler((uint8_t)k_ra_gpt_test_channel_bad, stub_gpt_cb, nullptr));
...
static void test_gpt_power_transition(void)
{
  ...
  const ra_gpt_cfg_t cfg = make_gpt_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_init((uint8_t)k_ra_gpt_test_channel_valid, &cfg));
  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_enter_stop((uint8_t)k_ra_gpt_test_channel_valid));
  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_exit_stop((uint8_t)k_ra_gpt_test_channel_valid));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_gpt_enter_stop((uint8_t)k_ra_gpt_test_channel_bad));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_gpt_exit_stop((uint8_t)k_ra_gpt_test_channel_bad));
...
static void test_gpt_write_dma_streams_periods_to_gtpr(void)
{
  ...
  s_gpt_dma_done           = 0;
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_write_dma((uint8_t)k_ra_gpt_test_channel_valid,
                                  periods,
...
static void test_gpt_read_dma_captures_gtcnt(void)
{
  ...
  s_gpt_dma_done  = 0;
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_read_dma((uint8_t)k_ra_gpt_test_channel_valid,
                                 out,
...
static void test_gpt_dma_arg_validation(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_write_dma(0U, nullptr, 1U, nullptr, nullptr, &dch));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_write_dma(0U, periods, 1U, nullptr, nullptr, nullptr));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_read_dma(0U, nullptr, 1U, nullptr, nullptr, &dch));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_read_dma(0U, counts, 1U, nullptr, nullptr, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_gpt_write_dma(99U, periods, 1U, nullptr, nullptr, &dch));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_gpt_read_dma(99U, counts, 1U, nullptr, nullptr, &dch));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_gpt_write_dma(0U, periods, 0U, nullptr, nullptr, &dch));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_gpt_read_dma(0U, counts, 0U, nullptr, nullptr, &dch));
...
static void test_gpt_period_set_buffers_gtpbr(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_gpt_period_set((uint8_t)k_ra_gpt_test_channel_valid, (uint32_t)k_ra_gpt_t2_period_2));
  volatile r_gpt_channel_regs_t* reg = ra_gpt((uint8_t)k_ra_gpt_test_channel_valid);
  TEST_ASSERT_EQ(k_ra_gpt_t2_period_2, reg->GTPBR);

  /* Bad channel rejection. */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_period_set((uint8_t)k_ra_gpt_t2_ch_bad, 0U));
...
static void test_gpt_duty_cycle_set_routes_to_c_e(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_duty_cycle_set((uint8_t)k_ra_gpt_test_channel_valid,
                                       k_ra_gpt_pin_a,
  ...
  volatile r_gpt_channel_regs_t* reg = ra_gpt((uint8_t)k_ra_gpt_test_channel_valid);
  TEST_ASSERT_EQ(k_ra_gpt_t2_duty_u, reg->GTCCR[2]);
  TEST_ASSERT_EQ(k_ra_gpt_t2_duty_v, reg->GTCCR[3]);
  ...
  TEST_ASSERT_EQ(
    ...
    ra_gpt_duty_cycle_set((uint8_t)k_ra_gpt_test_channel_valid, (ra_gpt_pwm_pin_t)9U, 0U));
  /* Bad channel. */
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_gpt_duty_cycle_set((uint8_t)k_ra_gpt_t2_ch_bad, k_ra_gpt_pin_a, 0U));
...
static void test_gpt_counter_set_writes_gtcnt(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_counter_set((uint8_t)k_ra_gpt_test_channel_valid, 0xCAFE0000UL));
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_state,
                 ra_gpt_counter_set((uint8_t)k_ra_gpt_test_channel_valid, 0xDEADBEEFUL));
  /* Bad channel. */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_counter_set((uint8_t)k_ra_gpt_t2_ch_bad, 0U));
...
static void test_gpt_pwm_pin_configure_active_high(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_gpt_pwm_pin_configure((uint8_t)k_ra_gpt_test_channel_valid, k_ra_gpt_pin_a, &pin_cfg));

  volatile r_gpt_channel_regs_t* reg = ra_gpt((uint8_t)k_ra_gpt_test_channel_valid);
  /* OAE bit 8 set, GTIOA[4:0] == 0x9 (active-high), OADFLT bit 6 set. */
  TEST_ASSERT_EQ(k_ra_gpt_t2_active_high_a, (reg->GTIOR & 0x0000001FUL));
...
static void test_gpt_pwm_pin_configure_active_low_b(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_gpt_pwm_pin_configure((uint8_t)k_ra_gpt_test_channel_valid, k_ra_gpt_pin_b, &pin_cfg));

  volatile r_gpt_channel_regs_t* reg = ra_gpt((uint8_t)k_ra_gpt_test_channel_valid);
  /* GTIOB[4:0] sits at bits 16..20; expect 0x6 << 16. */
  TEST_ASSERT_EQ(((uint32_t)k_ra_gpt_t2_active_low_a << 16U), (reg->GTIOR & 0x001F0000UL));
  TEST_ASSERT((reg->GTIOR & (uint32_t)k_ra_gpt_t2_obe_mask) == 0U);

  /* NULL cfg + bad pin + bad channel. */
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_gpt_pwm_pin_configure((uint8_t)k_ra_gpt_test_channel_valid, k_ra_gpt_pin_a, nullptr));
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_gpt_pwm_pin_configure((uint8_t)k_ra_gpt_test_channel_valid, (ra_gpt_pwm_pin_t)5U, &pin_cfg));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_gpt_pwm_pin_configure((uint8_t)k_ra_gpt_t2_ch_bad, k_ra_gpt_pin_a, &pin_cfg));
...
static void test_gpt_dead_time_set(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_dead_time_set((uint8_t)k_ra_gpt_test_channel_valid,
                                      (uint32_t)k_ra_gpt_t2_dt_rise,
                                      (uint32_t)k_ra_gpt_t2_dt_fall));
  volatile r_gpt_channel_regs_t* reg = ra_gpt((uint8_t)k_ra_gpt_test_channel_valid);
  TEST_ASSERT_EQ(k_ra_gpt_t2_dt_rise, reg->GTDVU);
  TEST_ASSERT_EQ(k_ra_gpt_t2_dt_fall, reg->GTDVD);
  TEST_ASSERT_EQ(k_ra_gpt_t2_gtdtcr_tde, reg->GTDTCR);

  /* Both zero -> TDE cleared. */
  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_dead_time_set((uint8_t)k_ra_gpt_test_channel_valid, 0U, 0U));
  TEST_ASSERT_EQ(0, reg->GTDTCR);

  /* Bad channel. */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_dead_time_set((uint8_t)k_ra_gpt_t2_ch_bad, 0U, 0U));
...
static void test_gpt_three_phase_open_starts_synchronously(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_three_phase_open(&cfg));
  ...
  for (uint8_t i = 0U; i < (uint8_t)k_ra_gpt_three_phase_count; ++i) {
    ...
    TEST_ASSERT_EQ(k_ra_gpt_t2_period, reg->GTPR);
  }

  /* Re-open while open -> rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_gpt_three_phase_open(&cfg));
  TEST_ASSERT_EQ(k_ra_ok, ra_gpt_three_phase_close());
...
static void test_gpt_three_phase_set_duty_atomic(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gpt_three_phase_set_duty((uint32_t)k_ra_gpt_t2_duty_u + 1U,
  ...
  TEST_ASSERT_EQ(((uint32_t)k_ra_gpt_t2_duty_u + 1U), u_reg->GTCCR[2]);
  TEST_ASSERT_EQ(((uint32_t)k_ra_gpt_t2_duty_u + 1U), u_reg->GTCCR[3]);
  TEST_ASSERT_EQ(((uint32_t)k_ra_gpt_t2_duty_v + 1U), v_reg->GTCCR[2]);
  TEST_ASSERT_EQ(((uint32_t)k_ra_gpt_t2_duty_w + 1U), w_reg->GTCCR[2]);

  /* Out-of-range duty -> rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_gpt_three_phase_set_duty((uint32_t)k_ra_gpt_t2_duty_huge, 0U, 0U));
...
static void test_gpt_three_phase_close_stops_all(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_gpt_three_phase_close());
  /* set_duty after close -> rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_gpt_three_phase_set_duty(0U, 0U, 0U));
...
static void test_gpt_three_phase_arg_validation(void)
{
  ...
  /* NULL cfg. */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_gpt_three_phase_open(nullptr));

  /* Bad channel id. */
  ra_gpt_three_phase_cfg_t cfg = make_three_phase_cfg();
  cfg.channels[1]              = (uint8_t)k_ra_gpt_t2_ch_bad;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_gpt_three_phase_open(&cfg));
...
static void test_mcdc_dead_time_set_tde_predicate(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_gpt_dead_time_set((uint8_t)k_ra_gpt_test_channel_valid, (uint32_t)k_ra_gpt_t2_dt_rise, 0U));
  TEST_ASSERT_EQ(k_ra_gpt_t2_gtdtcr_tde, reg->GTDTCR);

  /* Vector 3: falling != 0 only -> TDE asserted. */
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_gpt_dead_time_set((uint8_t)k_ra_gpt_test_channel_valid, 0U, (uint32_t)k_ra_gpt_t2_dt_fall));
  TEST_ASSERT_EQ(k_ra_gpt_t2_gtdtcr_tde, reg->GTDTCR);
