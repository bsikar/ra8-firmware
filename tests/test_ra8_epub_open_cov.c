 ...
 *
 * @details
 * Drives the previously-uncovered branches in ra8_epub_open.c by
 ...
 *
 *   Group C -- ra8_epub_open cleanup path (lines 390, 391):
 ...
 *           k_ra8_epub_max_path_len chars so len < cap always holds.
...
typedef enum : uint32_t {
  ...
  k_cov_path_max = 192U,   /**< Mirror of k_ra8_epub_max_path_len.            */
} cov_open_sizes_t;
 ...
 *          propagates the error (line 318), and ra8_epub_open() runs
...
static void test_open_no_container_xml(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_not_found, ra8_epub_open(&media, nullptr, &book));
 ...
 *          its uncompressed size beyond k_ra8_epub_container_xml_buf (4096).
 *          priv_extract() returns no_mem (line 188), priv_parse_archive()
 *          propagates it (line 318), and ra8_epub_open() cleans up (390-391).
 *
 * @par MC/DC:
...
static void test_open_container_too_large(void)
{
  ...
  ra8_epub_book_t            book  = {};
  const ra8_epub_mem_media_t media = {.data = s_zip_buf, .size = s_zip_size};
  TEST_ASSERT_EQ(k_ra8_err_no_mem, ra8_epub_open(&media, nullptr, &book));
  TEST_ASSERT_EQ(0, book.in_use);
 ...
 *          priv_parse_archive propagates it at line 324; ra8_epub_open
 ...
 *          priv_parse_archive propagates it at line 330; ra8_epub_open
 *          cleans up at lines 390-391.
 *
 * @par MC/DC:
 ...
 *          propagates it at line 336; ra8_epub_open cleans up at lines 390-391.
 *
 * @par MC/DC:
 ...
 *          line 144 is hit; opf_dir is left as "".  ra8_epub_open succeeds.
 *
 * @par MC/DC:
 ...
 * @post ra8_epub_close returns k_ra8_ok.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
 ...
 *          ra8_epub_open still returns k_ra8_ok (TOC is best-effort).
 *
 * @par MC/DC:
 ...
 *          ra8_epub_xml_parse_nav runs, toc_count > 0.
 *
 * @par MC/DC:
 * Decision: `if (err == k_ra8_err_not_found)` at line 269 (1 condition).
 ...
 *          causes ra8_epub_xml_parse_opf to set toc_kind=k_ra8_epub_toc_ncx and
 *          toc_path="toc.ncx".  priv_load_toc then dispatches to
 *          ra8_epub_xml_parse_ncx (line 280).
 *
 * @par MC/DC:
 * Decision: `if (book->toc_kind == (uint8_t)k_ra8_epub_toc_nav)` at line 277
 * (1 condition).
 * - V1 (other nav tests): toc_kind == k_ra8_epub_toc_nav -> true  -> line 278.
 * - V2 (this test): toc_kind == k_ra8_epub_toc_ncx     -> false -> line 280.
 * Pair (V1,V2) gives complete MC/DC for this 1-condition decision.
 *
...
static void test_toc_ncx_branch(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_epub_open(&media, nullptr, &book));
  TEST_ASSERT_EQ(1, book.in_use);
  TEST_ASSERT_EQ(k_ra8_epub_toc_ncx, book.toc_kind);
 ...
 *          minimal in-memory ZIP, calls ra8_epub_open / ra8_epub_close, and
