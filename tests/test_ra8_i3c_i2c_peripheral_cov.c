 ...
 *
 *  - address-range guard in ra8_i3c_i2c_peripheral_open (line 99)
...
/**
 * @test internal_test_cov_open_addr_oor
 ...
 * guard inside ra8_i3c_i2c_peripheral_open. Passing 0x80 must yield
 ...
 *
 * @since 0.1.0 @pre Fixed-capacity fixture storage required by this operation is available. @post Documented outputs contain the exercised result when the operation succeeds. */
RA8_INTERNAL static void internal_test_cov_open_addr_oor(void)
{
  TEST_BEGIN("ra8_i3c_i2c_peripheral_open addr > 0x7F");
  ra8_i3c_i2c_peripheral_cfg_t cfg_oor = {
    .peripheral_addr_7b = (uint8_t)k_cov_addr_oor,
    .general_call       = 0U,
  };
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_i3c_i2c_peripheral_open((uint8_t)k_cov_ch0, &cfg_oor));
  TEST_END("ra8_i3c_i2c_peripheral_open addr > 0x7F");
}

/**
 * @test internal_test_cov_close_oor_channel
 * @brief Verify that an out-of-range channel is rejected by close.
 *
 * @details
 * Exercises the `reg == nullptr` guard in ra8_i3c_i2c_peripheral_close.
 ...
 *
 * @since 0.1.0 @pre Fixed-capacity fixture storage required by this operation is available. @post Documented outputs contain the exercised result when the operation succeeds. */
RA8_INTERNAL static void internal_test_cov_close_oor_channel(void)
{
  TEST_BEGIN("ra8_i3c_i2c_peripheral_close oor channel");
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_i3c_i2c_peripheral_close((uint8_t)k_cov_ch_oor));
  TEST_END("ra8_i3c_i2c_peripheral_close oor channel");
}

/**
 * @test internal_test_cov_send_oor_channel
 * @brief Verify that an out-of-range channel is rejected by send.
 *
 * @details
 * Exercises the `reg == nullptr` guard in ra8_i3c_i2c_peripheral_send.
 ...
 *
 * @since 0.1.0 @pre Fixed-capacity fixture storage required by this operation is available. @post Documented outputs contain the exercised result when the operation succeeds. */
RA8_INTERNAL static void internal_test_cov_send_oor_channel(void)
{
  TEST_BEGIN("ra8_i3c_i2c_peripheral_send oor channel");
  const uint8_t data[1] = {(uint8_t)k_cov_byte_a};
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_i3c_i2c_peripheral_send((uint8_t)k_cov_ch_oor, data, 1U));
  TEST_END("ra8_i3c_i2c_peripheral_send oor channel");
}

/**
 * @test internal_test_cov_receive_oor_channel
 * @brief Verify that an out-of-range channel is rejected by receive.
 *
 * @details
 * Exercises the `reg == nullptr` guard in ra8_i3c_i2c_peripheral_receive.
 ...
 *
 * @since 0.1.0 @pre Fixed-capacity fixture storage required by this operation is available. @post Documented outputs contain the exercised result when the operation succeeds. */
RA8_INTERNAL static void internal_test_cov_receive_oor_channel(void)
{
  TEST_BEGIN("ra8_i3c_i2c_peripheral_receive oor channel");
  uint8_t buf[1] = {0U};
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_i3c_i2c_peripheral_receive((uint8_t)k_cov_ch_oor, buf, 1U));
  TEST_END("ra8_i3c_i2c_peripheral_receive oor channel");
}

/**
 * @test internal_test_cov_status_oor_channel
 * @brief Verify that an out-of-range channel is rejected by status.
 *
 * @details
 * Exercises the `reg == nullptr` guard in ra8_i3c_i2c_peripheral_status.
 ...
 *
 * @since 0.1.0 @pre Fixed-capacity fixture storage required by this operation is available. @post Documented outputs contain the exercised result when the operation succeeds. */
RA8_INTERNAL static void internal_test_cov_status_oor_channel(void)
{
  TEST_BEGIN("ra8_i3c_i2c_peripheral_status oor channel");
  uint8_t mask = 0U;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_i3c_i2c_peripheral_status((uint8_t)k_cov_ch_oor, &mask));
  TEST_END("ra8_i3c_i2c_peripheral_status oor channel");
}

/**
 * @test internal_test_cov_status_stop_bit
 ...
 * in ra8_i3c_i2c_peripheral_status. Pre-seeding BST with
...
RA8_INTERNAL static void internal_test_cov_status_stop_bit(void)
{
  TEST_BEGIN("ra8_i3c_i2c_peripheral_status BST.SPCNDDF -> stop");
  ra8_fake_mmap_reset();
  ...
  uint8_t mask = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_i3c_i2c_peripheral_status((uint8_t)k_cov_ch0, &mask));
  TEST_ASSERT(mask & (uint8_t)k_ra8_i3c_i2c_peripheral_status_stop);
  TEST_END("ra8_i3c_i2c_peripheral_status BST.SPCNDDF -> stop");
}

/**
 * @test internal_test_cov_send_timeout
 ...
 * inside ra8_i3c_i2c_peripheral_send propagates that code back
 * to the caller.
 *
 ...
 *
 * @since 0.1.0 @post Documented outputs contain the exercised result when the operation succeeds. */
RA8_INTERNAL static void internal_test_cov_send_timeout(void)
{
  TEST_BEGIN("ra8_i3c_i2c_peripheral_send NTST timeout");
  ra8_fake_mmap_reset();
  /* NTST.TDBEF0 is zero after reset; withhold pre-arming so the spin exhausts. */
  const uint8_t data[1] = {(uint8_t)k_cov_byte_a};
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_i3c_i2c_peripheral_send((uint8_t)k_cov_ch0, data, 1U));
  TEST_END("ra8_i3c_i2c_peripheral_send NTST timeout");
}
...
int main(void)
{
  internal_test_cov_open_addr_oor();
  internal_test_cov_close_oor_channel();
  internal_test_cov_send_oor_channel();
  internal_test_cov_receive_oor_channel();
  internal_test_cov_status_oor_channel();
  internal_test_cov_status_stop_bit();
  internal_test_cov_send_timeout();
  return 0;
}
