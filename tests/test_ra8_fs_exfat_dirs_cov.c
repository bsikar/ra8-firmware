 ...
 * @since 0.1.0
 */
static void write_small(ra8_fs_mount_t* h, const char* path)
{
 ...
 * @since 0.1.0
 */
static void test_scanner_catches_orphaned_entry_set(void)
{
  ...
  write_small(h, "/D/F.BIN");

  char msg[k_chk_msg_cap] = {};
  TEST_ASSERT_EQ(k_dcov_one, exfat_scan(h, msg, (uint32_t)k_chk_msg_cap));
  ...
  TEST_ASSERT_EQ(k_dcov_none, exfat_scan(h, msg, (uint32_t)k_chk_msg_cap));
 ...
 * (libs/ra8_fs/src/ra8_fs_fat_exfat_dir.c@priv_exfat_mkdir_check), plus the
 * not-found arm of the component lookup in
 * libs/ra8_fs/src/ra8_fs_fat_exfat_dir.c@priv_exfat_enter.
 *
 * @pre A freshly formatted 64 MiB exFAT volume.
 ...
 * @since 0.1.0
 */
static void test_mkdir_argument_refusals(void)
{
  ...
  exfat_verify(h, "mkdir_argument_refusals");
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  free_volume();
 ...
 * (libs/ra8_fs/src/ra8_fs_fat_exfat_dir.c@priv_exfat_enter) -> true, the
...
static void test_resolution_refusals(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_fs_listdir(h, "/PLAIN.BIN", count_cb, &(mut_list_ctx_t){}));
  ...
  exfat_verify(h, "resolution_refusals");
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  free_volume();
 ...
 * both in libs/ra8_fs/src/ra8_fs_fat_exfat_dir.c@priv_exfat_rmdir_locate.
 *
 * @pre A freshly formatted 64 MiB exFAT volume.
 * @post Each call reports its own error code.
 ...
 * @since 0.1.0
 */
static void test_rmdir_refusals(void)
{
 ...
 * (libs/ra8_fs/src/ra8_fs_fat_exfat_mutate.c@priv_exfat_rename_prepare), each
...
static void test_rename_refusals(void)
{
  ...
  exfat_verify(h, "rename_refusals");
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  free_volume();
 ...
 * @since 0.1.0
 */
static void test_mkdir_rolls_back_its_cluster(void)
{
  ...
  exfat_verify(h, "mkdir_rollback_early");
  ...
  exfat_verify(h, "mkdir_rollback_late");
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  free_volume();
 ...
 * (libs/ra8_fs/src/ra8_fs_fat_exfat_dir.c@priv_exfat_mkdir_check) -> true: the
 ...
 * @since 0.1.0
 */
static void test_mkdir_read_failure_propagates(void)
{
  ...
  TEST_ASSERT_EQ(before, alloc_bitmap_used(h));
  exfat_verify(h, "mkdir_read_failure");
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  free_volume();
 ...
 * @since 0.1.0
 */
static void build_two_cluster_dir(ra8_fs_mount_t* h, uint8_t tail_type)
{
 ...
 * @since 0.1.0
 */
static void test_contiguous_run_walks_into_its_second_cluster(void)
{
  ...
  build_two_cluster_dir(h, (uint8_t)k_chk_type_eod);
  TEST_ASSERT_EQ(baseline + (uint32_t)k_dcov_two, alloc_bitmap_used(h));

  mut_list_ctx_t counted = {};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_listdir(h, "/BIG", count_cb, &counted));
  ...
  exfat_verify(h, "contiguous_run_second_cluster");
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  free_volume();
 ...
 * @since 0.1.0
 */
static void test_rmdir_walks_a_run_to_its_end(void)
{
  ...
  build_two_cluster_dir(h, (uint8_t)k_dcov_remnant);
  ...
  exfat_verify(h, "rmdir_run_to_its_end");
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  free_volume();
 ...
 * @since 0.1.0
 */
static void test_trailing_slash_names_the_directory(void)
{
  ...
  name_ctx_t plain = {};
  name_ctx_t slash = {};
  TEST_ASSERT_EQ(k_dcov_one, list_names(h, "/D", &plain));
  TEST_ASSERT_EQ(k_dcov_one, list_names(h, "/D/", &slash));
  ...
  exfat_verify(h, "trailing_slash_names_the_directory");
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  free_volume();
 ...
 * (libs/ra8_fs/src/ra8_fs_fat_exfat_dir.c@priv_exfat_enter) -> true, which the
 ...
 * @since 0.1.0
 */
static void test_lookup_and_resolve_edges(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_not_found,
                 ra8_fs_listdir(h, "/NOPE/SUB", count_cb, &(mut_list_ctx_t){}));
  ...
  exfat_verify(h, "lookup_and_resolve_edges");
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  free_volume();
 ...
 * @since 0.1.0
 */
static void test_rmdir_read_faults_cost_nothing(void)
{
  ...
  for (uint32_t n = 0U; n < (uint32_t)k_dcov_rd_sweep; n++) {
    ...
    TEST_ASSERT_EQ(before, alloc_bitmap_used(h));
    exfat_check(h, "rmdir_read_fault_sweep");
 ...
 * @since 0.1.0
 */
static void test_rmdir_write_fault_keeps_the_directory(void)
{
  ...
  exfat_verify(h, "rmdir_write_fault");
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  free_volume();
 ...
 * set), both in
 * libs/ra8_fs/src/ra8_fs_fat_exfat_dir.c@priv_exfat_dir_alloc.
 *
 * @pre A freshly formatted 64 MiB exFAT volume.
 ...
 * @since 0.1.0
 */
static void test_mkdir_allocation_failures(void)
{
  ...
  exfat_verify(h, "mkdir_bitmap_mark_failure");
...
int32_t main(void)
{
  test_scanner_catches_orphaned_entry_set();
  test_mkdir_argument_refusals();
  test_resolution_refusals();
  test_rmdir_refusals();
  test_rename_refusals();
  test_mkdir_rolls_back_its_cluster();
  test_mkdir_read_failure_propagates();
  test_mkdir_fills_a_directory();
  test_contiguous_run_walks_into_its_second_cluster();
  test_rmdir_walks_a_run_to_its_end();
  test_trailing_slash_names_the_directory();
  test_lookup_and_resolve_edges();
  test_rmdir_read_faults_cost_nothing();
  test_rmdir_write_fault_keeps_the_directory();
  test_mkdir_allocation_failures();
  (void)fprintf(stderr, "[OK  ] test_ra8_fs_exfat_dirs_cov.c\n");
  return 0;
}
