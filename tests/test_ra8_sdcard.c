...
static void sdcard_srv_respond(volatile r_sdhi_regs_t* reg, uint32_t cmd)
{
  switch (cmd) {
    case 8U: /* CMD8 R7: echoes the low 12 bits of the argument. */
      ...
      reg->SD_RSP76 = 0U;
      break;
    case k_t_acmd41_op_cond: /* ACMD41 R3: OCR with busy-done + CCS bits set. */
    ...
    case 3U: /* CMD3 R6: RCA in upper 16 bits. */
      ...
      break;
    case k_t_cmd9_send_csd: /* CMD9 R2: 128-bit CSD; rsp[3][31:30] is CSD_STRUCTURE. */
    ...
    case 6U: /* ACMD6 SET_BUS_WIDTH R1: clean (no error bits). */
      reg->SD_RSP10 = 0U;
      break;
    case k_t_cmd55_app: /* CMD55 APP_CMD R1: echo APP_CMD unless a test forces a decline. */
