/**
 * @file test_ra_keycache.c
 * @brief Unit tests for the reusable ra_mem keyed-LRU cache (Layer 3, #147).
 *
 * @details
 ...
 * regression guard shared by ::ra_glyph_atlas and ::ra_tile_cache, which are both
...
static ra_err_t t_render(void* ctx, const void* key, uint8_t* cell, uint32_t cell_bytes, void* user)
{
  (void)ctx;
  s_render_calls++;
...
static ra_keycache_cfg_t t_cfg(void)
{
  ra_keycache_cfg_t cfg = {};
...
static void test_render_hit(void)
{
  ...
  s_render_calls        = 0;
  ra_keycache_t     kc  = {};
  ra_keycache_cfg_t cfg = t_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_init(&kc, &cfg));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_put(&kc, v.data));

  ra_keycache_view_t v2 = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_get(&kc, &k, &v2)); /* hit -> no render */
  TEST_ASSERT(v2.data == v.data);
  TEST_ASSERT_EQ(1, s_render_calls); /* unchanged */
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_put(&kc, v2.data));

  uint32_t hits = 0;
  uint32_t miss = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_stats(&kc, &hits, &miss, nullptr));
...
static void test_eviction(void)
{
  ...
  for (uint32_t i = 0; i < 8U; ++i) {
    ...
    ra_keycache_view_t v = {};
    TEST_ASSERT_EQ(k_ra_ok, ra_keycache_get(&kc, &k, &v));
    TEST_ASSERT_EQ(k_ra_ok, ra_keycache_put(&kc, v.data));
  }
  uint32_t ev = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_stats(&kc, nullptr, nullptr, &ev));
  ...
  ra_keycache_view_t v0     = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_get(&kc, &k0, &v0)); /* evicted -> re-render */
  TEST_ASSERT_EQ(before + 1U, s_render_calls);
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_put(&kc, v0.data));

  uint32_t           before2 = s_render_calls;
  t_key_t            k7      = t_key(7U, 0U);
  ra_keycache_view_t v7      = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_get(&kc, &k7, &v7)); /* recent -> hit */
  TEST_ASSERT_EQ(before2, s_render_calls);
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_put(&kc, v7.data));
...
static void test_pin_protection(void)
{
  ...
  for (uint32_t i = 0; i < (uint32_t)k_t_cells; ++i) {
    ...
    ra_keycache_view_t v = {};
    TEST_ASSERT_EQ(k_ra_ok, ra_keycache_get(&kc, &k, &v)); /* pinned (no put) */
    pins[i] = v.data;
  }
  t_key_t            kn = t_key(99U, 7U);
  ra_keycache_view_t vn = {};
  TEST_ASSERT_EQ(k_ra_err_no_mem, ra_keycache_get(&kc, &kn, &vn)); /* all pinned */
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_put(&kc, pins[0]));
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_get(&kc, &kn, &vn)); /* now evicts + renders */
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_put(&kc, vn.data));
  for (uint32_t i = 1; i < (uint32_t)k_t_cells; ++i) {
    TEST_ASSERT_EQ(k_ra_ok, ra_keycache_put(&kc, pins[i]));
  }
...
static void test_render_failure(void)
{
  ...
  ra_keycache_view_t v = {};
  TEST_ASSERT_EQ(k_ra_err_timeout, ra_keycache_get(&kc, &k, &v)); /* render fails */

  /* Nothing should be resident: a subsequent get is still a miss. */
  uint32_t miss = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_stats(&kc, nullptr, &miss, nullptr));
  TEST_ASSERT_EQ(1, miss);
...
static void test_validation(void)
{
  ...
  ra_keycache_view_t v   = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_keycache_init(nullptr, &cfg));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_keycache_init(&kc, nullptr));
  ra_keycache_cfg_t bad = t_cfg();
  bad.cell_count        = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_size, ra_keycache_init(&kc, &bad));
  bad           = t_cfg();
  bad.key_bytes = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_size, ra_keycache_init(&kc, &bad));
  bad            = t_cfg();
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_keycache_init(&kc, &bad));

  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_init(&kc, &cfg));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_keycache_get(nullptr, &k, &v));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_keycache_get(&kc, nullptr, &v));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_keycache_get(&kc, &k, nullptr));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_keycache_put(&kc, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_keycache_put(&kc, &s_cells[1])); /* mid-cell */
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_get(&kc, &k, &v));
  TEST_ASSERT_EQ(k_ra_ok, ra_keycache_put(&kc, v.data));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_keycache_put(&kc, v.data)); /* already unpinned */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_keycache_stats(nullptr, nullptr, nullptr, nullptr));
...
int32_t main(void)
{
  ...
  test_validation();
  (void)fprintf(stderr, "[OK  ] test_ra_keycache.c\n");
  return 0;
}
