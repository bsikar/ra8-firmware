...
static void test_status_read_and_clear(void)
{
  ...
  *ra_glcdc_reg32(k_ra_glcdc_off_sys_stat) = 0xDEADBEEFU;
  ...
  uint32_t mask = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_glcdc_get_status(&mask));
  TEST_ASSERT_EQ(0xDEADBEEFU, mask);
  TEST_ASSERT_EQ(k_ra_ok, ra_glcdc_clear_status(0xF0F0F0F0U));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_glcdc_get_status(nullptr));
...
static void test_attach_and_dispatch(void)
{
  ...
  prep_w61();
  TEST_ASSERT_EQ(k_ra_ok, ra_glcdc_attach_handler(stub_glcdc_cb, (void*)(uintptr_t)0x77U));
...
static void test_set_layer2_happy(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_glcdc_set_layer2(&cfg));

  /* Format -> FLM6. */
  TEST_ASSERT_EQ(k_ra_glcdc_fmt_rgb565, *ra_glcdc_reg32(k_ra_glcdc_off_gr2_fmt));
  /* Base address -> FLM2. */
  TEST_ASSERT_EQ(k_test_glcdc_fb2_addr, *ra_glcdc_reg32(k_ra_glcdc_off_gr2_saddr));
  /* Line stride -> FLM3 high half. */
  TEST_ASSERT_EQ(((uint32_t)k_test_layer2_strd << 16U), *ra_glcdc_reg32(k_ra_glcdc_off_gr2_flm3));
  /* FLM5 = (h<<16) | w. */
  ...
  /* AB7 = alpha << 16. */
  TEST_ASSERT_EQ(((uint32_t)k_test_alpha_half << 16U), *ra_glcdc_reg32(k_ra_glcdc_off_gr2_ab7));
  /* AB1 = DISPSEL=01 (lower-layer blend). */
  TEST_ASSERT_EQ(1, *ra_glcdc_reg32(k_ra_glcdc_off_gr2_ab1));
  /* FLMRD enabled. */
...
static void test_set_blend_alpha(void)
{
  ...
  /* AB7 = alpha << 16. */
  TEST_ASSERT_EQ(((uint32_t)k_test_alpha_half << 16U), *ra_glcdc_reg32(k_ra_glcdc_off_gr1_ab7));
...
static void test_clut_swap_now_false(void)
{
  ...
  TEST_ASSERT_EQ(0, *ra_glcdc_reg32(k_ra_glcdc_off_gr1_clutint));
...
static void test_clut_swap_now_true(void)
{
  ...
  TEST_ASSERT_EQ(0x10000U, *ra_glcdc_reg32(k_ra_glcdc_off_gr1_clutint));
...
static void test_clut_layer2(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_glcdc_set_clut_double_buffered((uint8_t)k_test_layer2,
                                                   src,
                                                   (uint32_t)k_test_clut_small,
                                                   false));
...
static void test_clut_invalid_args(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_glcdc_set_clut_double_buffered(2U, src, (uint32_t)k_test_clut_small, false));
  /* 0 entries is invalid. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_glcdc_set_clut_double_buffered((uint8_t)k_test_layer1, src, 0U, false));
  /* > 256 entries is invalid. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_glcdc_set_clut_double_buffered((uint8_t)k_test_layer1, src, 257U, false));
...
static void test_set_dithering_modes(void)
{
  ...
  TEST_ASSERT_EQ((2U << 8U), *ra_glcdc_reg32(k_ra_glcdc_off_panel_dtha));
  TEST_ASSERT_EQ(k_ra_ok, ra_glcdc_set_dithering(k_ra_dither_2x2));
  TEST_ASSERT_EQ((3U << 8U), *ra_glcdc_reg32(k_ra_glcdc_off_panel_dtha));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_glcdc_set_dithering((ra_glcdc_dither_mode_t)0xFFU));
