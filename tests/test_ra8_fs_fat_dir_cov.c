/**
 * @file test_ra8_fs_fat_dir_cov.c
 * @brief Coverage booster for libs/ra8_fs/src/ra8_fs_fat_dir.c.
 *
 * @details
 * Dedicated companion test executable that drives the branches in
 * ra8_fs_fat_dir.c that no sibling test yet exercises:
 *
 *   - ra8_fs_listdir guards (lines 123, 128, 131, 149, 156, 159)
 *   - priv_dir_cluster_init error + SPC>1 loop (lines 233, 237-239, 241)
 *   - priv_fat_mkdir error paths (lines 283, 295, 300, 305-306, 315-316)
 *   - ra8_fs_mkdir guards (lines 355, 358)
 *   - ra8_fs_unlink guards (lines 395, 404, 408, 421, 427)
 *   - priv_rename_prepare error paths (lines 474, 480, 487, 491, 494)
 *   - priv_fat_rename error paths (lines 545, 552, 557)
 *   - ra8_fs_rename guards (lines 582, 585, 588, 591)
 *
...
static ra8_err_t wco_read(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf)
{
...
static ra8_err_t wco_write(void* ctx, uint32_t lba, uint32_t count, const uint8_t* buf)
{
  wcount_disk_t* d = (wcount_disk_t*)ctx;
  if (d->writes_left == 0U) {
    return k_ra8_err_hw_error;
...
static ra8_err_t wco_capacity(void* ctx, uint32_t* block_count, uint32_t* block_size)
{
  wcount_disk_t* d = (wcount_disk_t*)ctx;
  *block_count     = d->block_count;
  *block_size      = (uint32_t)k_geo_blk_sz;
 ...
 * @since 0.1.0
 */
static void swap_to_wcount(ra8_fs_mount_t* h, uint32_t writes_left)
{
 ...
 * @since 0.1.0
 */
static void create_empty_files(ra8_fs_mount_t* h, const char* dir_path, uint32_t count)
{
 ...
 * @post All targeted lines in ra8_fs_fat_dir.c are exercised.
 *
