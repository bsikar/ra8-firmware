/**
 * @file test_ra_book.c
 * @brief Unit tests for the flat, XIP e-book container format (libs/ra_book).
 *
...
static void setup_mock_book(mock_book_t* b)
{
  ...
  b->nodes[0].kind         = k_ra_book_node_element;
  ...
  b->nodes[1].kind         = k_ra_book_node_text;
  ...
  b->stylesheets[0].scope_chapter = k_ra_book_nil;

  b->images[0].id_off    = img_href_off;
  b->images[0].width     = 16;
  b->images[0].height    = 16;
  b->images[0].format    = k_ra_book_image_gray4;
  ...
  b->images[1].width     = 0;
  b->images[1].height    = 0;
  b->images[1].format    = k_ra_book_image_svg;
  ...
  uint32_t       body_len = sizeof(*b) - sizeof(ra_book_header_t);
...
static void test_ra_book_invalid(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_book_validate(NULL, 100));

  uint8_t buffer[64] = {0};
  TEST_ASSERT_EQ(k_ra_err_invalid_size, ra_book_validate(buffer, sizeof(buffer)));
  ...
  body                 = (const uint8_t*)&b + sizeof(ra_book_header_t);
  b.hdr.crc32          = compute_crc32(body, body_len);
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_book_validate(&b, sizeof(b)));

  setup_mock_book(&b);
  b.hdr.crc32 ^= 0xFFFFFFFF;
  TEST_ASSERT_EQ(k_ra_err_range_check_failed, ra_book_validate(&b, sizeof(b)));
...
static void test_ra_book_valid_walk(void)
{
  ...
  const ra_book_header_t* hdr = ra_book_header(&b);
  TEST_ASSERT_NOT_NULL(hdr);

  TEST_ASSERT_EQ(0, strcmp(ra_book_string(&b, hdr->title_off), "Title"));
  TEST_ASSERT_EQ(0, strcmp(ra_book_string(&b, hdr->author_off), "Author"));
  TEST_ASSERT_EQ(0, strcmp(ra_book_string(&b, hdr->language_off), "en"));
  TEST_ASSERT_EQ(0, strcmp(ra_book_string(&b, hdr->identifier_off), "ID123"));
  TEST_ASSERT_EQ(0, hdr->cover_image_index);

  TEST_ASSERT_EQ(1, hdr->chapter_count);
  const ra_book_chapter_t* chaps = ra_book_chapters(&b);
  TEST_ASSERT_NOT_NULL(chaps);
  TEST_ASSERT_EQ(0, strcmp(ra_book_string(&b, chaps[0].title_off), "Chapter 1"));
  TEST_ASSERT_EQ(0, strcmp(ra_book_string(&b, chaps[0].href_off), "ch1.xhtml"));
  TEST_ASSERT_EQ(0, chaps[0].root_node);

  TEST_ASSERT_EQ(2, hdr->node_count);
  const ra_book_node_t* nodes = ra_book_nodes(&b);
  TEST_ASSERT_NOT_NULL(nodes);

  const ra_book_node_t* n0 = &nodes[0];
  TEST_ASSERT_EQ(k_ra_book_node_element, n0->kind);
  TEST_ASSERT_EQ(0, strcmp(ra_book_node_name(&b, n0), "div"));
  TEST_ASSERT_EQ(1, n0->attr_count);
  TEST_ASSERT_EQ(1, n0->first_child);
  TEST_ASSERT_EQ(k_ra_book_nil, n0->next_sibling);

  const ra_book_attr_t* attrs = ra_book_attrs(&b);
  TEST_ASSERT_NOT_NULL(attrs);
  const ra_book_attr_t* a0 = &attrs[n0->first_attr];
  TEST_ASSERT_EQ(0, strcmp(ra_book_string(&b, a0->name_off), "class"));
  TEST_ASSERT_EQ(0, strcmp(ra_book_string(&b, a0->value_off), "main"));

  const ra_book_node_t* n1 = &nodes[n0->first_child];
  TEST_ASSERT_EQ(k_ra_book_node_text, n1->kind);
  TEST_ASSERT_EQ(0, strcmp(ra_book_node_text(&b, n1), "Hello World"));
  TEST_ASSERT_EQ(0, n1->attr_count);
  TEST_ASSERT_EQ(k_ra_book_nil, n1->first_child);
  TEST_ASSERT_EQ(k_ra_book_nil, n1->next_sibling);

  TEST_ASSERT_EQ(1, hdr->stylesheet_count);
  const ra_book_stylesheet_t* styles = ra_book_stylesheets(&b);
  TEST_ASSERT_NOT_NULL(styles);
  TEST_ASSERT_EQ(0, strcmp(ra_book_string(&b, styles[0].source_off), "body { margin: 0; }"));
  TEST_ASSERT_EQ(k_ra_book_nil, styles[0].scope_chapter);

  TEST_ASSERT_EQ(2, hdr->image_count);
  const ra_book_image_t* imgs = ra_book_images(&b);
  TEST_ASSERT_NOT_NULL(imgs);

  TEST_ASSERT_EQ(0, strcmp(ra_book_string(&b, imgs[0].id_off), "img1.png"));
  ...
  TEST_ASSERT_NOT_NULL(ra_book_image_data(&b, &imgs[0]));

  TEST_ASSERT_EQ(0, strcmp(ra_book_string(&b, imgs[1].id_off), "svg1.svg"));
  TEST_ASSERT_EQ(0, imgs[1].width);
  TEST_ASSERT_EQ(0, imgs[1].height);
  TEST_ASSERT_EQ(k_ra_book_image_svg, imgs[1].format);
  TEST_ASSERT_EQ(20, imgs[1].data_size);
  TEST_ASSERT_EQ(50, imgs[1].raw_size);
  TEST_ASSERT_NOT_NULL(ra_book_image_data(&b, &imgs[1]));
