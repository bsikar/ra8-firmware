 ...
 *
 * @details
 * The black-box tests in `test_ra_cgc_extended.c` link against the production
 * `ra_cgc_usb.c` and drive it through the sim register mirror. Two families of
 * lines stay unreachable that way:
 *
 *  1. The PLL2 / HOCO / main-XTAL "stabilization poll failed" error legs.
 *     Those legs fire only when ::ra_cgc_wait_oscsf_set / ::ra_cgc_wait_oscsf_clear
 ...
 *     `RA_SIMULATOR_MODE` the static `internal_wait_usbck*srdy` helpers fake the
 ...
 *
 * This TU compiles a second, instrumented copy of `ra_cgc_usb.c` to reach both
 * families the same way `test_ra_usb_hmsc_enum_cov.c` reaches its ladder:
 *
 *  - The four exported symbols are renamed with a `_cov` suffix so they do not
 *    collide with the production copy linked from `ra_core_hal`.
 *  - The two external OSCSF poll helpers (`ra_cgc_wait_oscsf_set` /
 *    `ra_cgc_wait_oscsf_clear`) are redirected to deterministic, per-bit
 ...
 *  - `RA_SIMULATOR_MODE` is undefined ONLY for the included `.c` body so the
 ...
 *    pulls in do not reference `RA_SIMULATOR_MODE`, so only the driver body is
 *    affected; the `ra_sim_mmap` backing (a separate TU) is unchanged.
 *
...
static ra_err_t s_clear_result = k_ra_ok;
/**
...
/** @brief Mock for `ra_cgc_wait_oscsf_clear()` -- returns the scripted result. */
static ra_err_t mock_wait_oscsf_clear(uint8_t bit)
{
  (void)bit;
  return s_clear_result;
}

/** @brief Mock for `ra_cgc_wait_oscsf_set()` -- fails only for ::s_set_fail_bit. */
static ra_err_t mock_wait_oscsf_set(uint8_t bit)
{
...
static const uint8_t k_cov_hococr_hcstp = (uint8_t)(1U << k_ra_hococr_hcstp);

/** @brief PLL2 USB-path multiplier (matches the driver's k_ra_pll2_usbfs_mul). */
...
{
  ra_sim_mmap_reset();
  s_clear_result = k_ra_ok;
  s_set_fail_bit = -1;
}

/* Rename the exported symbols so the instrumented copy does not clash with the
 * production driver in ra_core_hal, and redirect the OSCSF poll helpers to the
 * scriptable mocks above. */
ra_err_t ra_cgc_pll2_enable_cov(uint8_t mul_int, uint8_t mul_quarters, ra_plodiv_t p_div_code);
ra_err_t ra_cgc_usbfs_clock_enable_cov(void);
ra_err_t ra_cgc_usbhs_pll_enable_cov(void);
ra_err_t ra_cgc_ensure_hoco_running_for_usb_ck_cov(void);

#define ra_cgc_pll2_enable                    ra_cgc_pll2_enable_cov
#define ra_cgc_usbfs_clock_enable             ra_cgc_usbfs_clock_enable_cov
#define ra_cgc_usbhs_pll_enable               ra_cgc_usbhs_pll_enable_cov
#define ra_cgc_ensure_hoco_running_for_usb_ck ra_cgc_ensure_hoco_running_for_usb_ck_cov
#define ra_cgc_wait_oscsf_set                 mock_wait_oscsf_set
#define ra_cgc_wait_oscsf_clear               mock_wait_oscsf_clear

/* Undefine RA_SIMULATOR_MODE for the driver body ONLY (all of its headers are
...
#undef RA_SIMULATOR_MODE
#include "ra_cgc_usb.c" // NOLINT(bugprone-suspicious-include) -- white-box copy
...
static void test_pll2_program_success(void)
{
  ...
  s_clear_result = k_ra_ok;
  s_set_fail_bit = -1;
  TEST_ASSERT_EQ(k_ra_ok, ra_cgc_pll2_enable_cov(k_cov_pll2_mul, 0U, k_ra_plodiv_div4));
 ...
 *
 * @details The stop-wait leg (`ra_cgc_wait_oscsf_clear(PLL2SF)` fails) is the
 ...
 *
 * @par MC/DC:
 * (no compound decisions -- single-condition `if (err != k_ra_ok)` after the
 * stop poll.)
 */
static void test_pll2_stop_wait_timeout(void)
{
  ...
  s_clear_result = k_ra_err_hw_timeout; /* PLL2SF stop poll never clears. */
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_cgc_pll2_enable_cov(k_cov_pll2_mul, 0U, k_ra_plodiv_div4));
...
static void test_ensure_hoco_hcstp_and_wait(void)
{
  ...
  cov_reset();
  *ra_sys_hococr() = k_cov_hococr_hcstp;
  s_set_fail_bit   = -1;
  TEST_ASSERT_EQ(k_ra_ok, ra_cgc_ensure_hoco_running_for_usb_ck_cov());

  /* Poll times out -> failure returned. */
  cov_reset();
  *ra_sys_hococr() = k_cov_hococr_hcstp;
  s_set_fail_bit   = (int)k_ra_oscsf_bit_hocosf;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_cgc_ensure_hoco_running_for_usb_ck_cov());
 ...
 * HOCOSF set poll is scripted to fail, so `ra_cgc_usbfs_clock_enable` takes the
 * "HOCO stabilization timeout" return before the USBCKCR handshake.
 *
 * @par MC/DC:
 * (no compound decisions -- single-condition `if (hoco_err != k_ra_ok)`.)
 */
static void test_usbfs_hoco_stabilize_timeout(void)
{
  ...
  s_set_fail_bit = (int)k_ra_oscsf_bit_hocosf;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_cgc_usbfs_clock_enable_cov());
 ...
 * built without `RA_SIMULATOR_MODE`, the static USBCKSRDY poll reads the plain
 ...
 * `if (err != k_ra_ok)` error checks are single-condition.)
 */
static void test_usbfs_handshake_srdy_timeout(void)
{
  ...
  *ra_sys_hococr() = k_cov_hococr_hcstp; /* Exercise the HOCO-restart branch. */
 ...
 *
 * @details The first step of `ra_cgc_usbhs_pll_enable` waits OSCSF.MOSCSF; when
 * that set poll is scripted to fail the routine returns immediately with the
 * propagated timeout, before PLL2 is touched.
 *
 * @par MC/DC:
 * (no compound decisions -- single-condition `if (osc_err != k_ra_ok)`.)
 */
static void test_usbhs_moscsf_timeout(void)
{
  ...
  s_set_fail_bit = (int)k_ra_oscsf_bit_moscsf;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_cgc_usbhs_pll_enable_cov());
 ...
 * fail, so `ra_cgc_pll2_enable` returns a timeout and the USB-HS routine takes
 * its "pll2 enable failed" return -- a leg the black-box USB-HS timeout test
 * never reaches (it fails MOSCSF first).
 *
 * @par MC/DC:
 * (no compound decisions -- single-condition `if (pll2_err != k_ra_ok)`.)
 */
static void test_usbhs_pll2_enable_failed(void)
{
  ...
  s_set_fail_bit = (int)k_ra_oscsf_bit_pll2sf;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_cgc_usbhs_pll_enable_cov());
 ...
 * `RA_SIMULATOR_MODE`, the static USB60CKSRDY poll reads the plain register
