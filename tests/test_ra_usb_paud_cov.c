/**
 * @file test_ra_usb_paud_cov.c
 * @brief Line-coverage top-up for the native USB device-side Audio (UAC1)
 *        class layer (`libs/ra_hal/src/ra_usb_paud.c`).
 *
 * @details
 * The sibling functional/MC-DC suite `test_ra_usb_paud.c` exercises the
 * public contract but only ever drives the descriptor / recv-frame /
 * setup-callback-error entry points BEFORE `ra_usb_paud_init` (the
 * `k_ra_err_invalid_state` rejection legs). This file drives the same
...
static ra_err_t s_cb_return = k_ra_ok;
 ...
 */
static ra_err_t test_paud_cov_cb(void* ctx, const ra_usb_setup_t* setup)
{
  (void)ctx;
  (void)setup;
  s_cb_calls++;
 ...
 *
 * @par MC/DC:
 * (single-condition decisions only -- `RA_CHECK_NULL_PTR(desc)` and
 * `if (desc_len == 0U)`; no `&&`/`||` in the covered statements, so
 * per-condition independence is trivially the branch pair below)
 *
 * @details Drives `ra_usb_paud_set_descriptors` after a successful init:
 *  - NULL desc  -> `k_ra_err_null_ptr`   (null guard, false leg of the ptr check)
 *  - len == 0   -> `k_ra_err_invalid_arg` (zero-length guard true leg)
 *  - valid pair -> `k_ra_ok`              (pointer + length stored)
 */
static void test_set_descriptors_post_init(void)
{
  TEST_BEGIN("ra_usb_paud_set_descriptors stores a valid blob after init");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_paud_init(k_ra_usb_speed_fs));

  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_usb_paud_set_descriptors(nullptr, (uint16_t)k_test_paud_desc_len));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_usb_paud_set_descriptors(s_desc_blob, (uint16_t)k_test_paud_desc_len_zero));
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_paud_set_descriptors(s_desc_blob, (uint16_t)k_test_paud_desc_len));

  TEST_END("ra_usb_paud_set_descriptors stores a valid blob after init");
}
 ...
 * `if (err == k_ra_ok)`; both branch legs are taken across the three
 * calls below, giving full branch coverage of each 1-condition decision)
 *
 * @details Drives `ra_usb_paud_recv_frame` after init:
 *  - max_len == 0                -> `k_ra_err_invalid_arg` (arg guard true leg)
 *  - BRDYSTS clear (empty pipe)  -> `k_ra_err_no_data`, `*got_len == 0`
 *    (queue_out no-data fast path -> `err != k_ra_ok` else leg, line 333)
 *  - BRDYSTS + CFIFOCTR seeded   -> `k_ra_ok`, one byte drained
 *    (queue_out success -> `err == k_ra_ok` then leg, line 331)
 */
static void test_recv_frame_post_init(void)
{
  TEST_BEGIN("ra_usb_paud_recv_frame drains a seeded iso-OUT FIFO");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_paud_init(k_ra_usb_speed_fs));
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_paud_recv_frame(buf, 0U, &got));
  ...
  TEST_ASSERT_EQ(k_ra_err_no_data,
                 ra_usb_paud_recv_frame(buf, (uint16_t)k_test_paud_recv_cap, &got));
  ...
  reg->CFIFOCTR = (uint16_t)(k_ra_fifoctr_frdy | (uint16_t)k_test_paud_drain_len);
 ...
 *
 * @par MC/DC:
 * (single-condition decision `if (cb_err != k_ra_ok)`; the sibling suite
 * covers the `== k_ra_ok` leg, this case covers the `!= k_ra_ok` leg,
 * completing branch coverage of the 1-condition decision on line 426)
 *
 * @details A registered class handler that returns a non-`k_ra_ok` code
 * makes `ra_usb_paud_handle_setup` STALL EP0 via
 * `ra_usb_control_response(speed, false)`. Under simulation that helper
 * returns `k_ra_ok` (it only drives the DCPCTR PID=STALL register), so
 * the dispatcher itself reports `k_ra_ok` while the callback fired once.
 */
static void test_handle_setup_callback_stall(void)
{
  TEST_BEGIN("ra_usb_paud_handle_setup stalls EP0 when the callback rejects");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_paud_init(k_ra_usb_speed_fs));
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_paud_attach_setup_handler(test_paud_cov_cb, nullptr));

  s_cb_return          = k_ra_err_not_supported;
  ...
  TEST_END("ra_usb_paud_handle_setup stalls EP0 when the callback rejects");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_usb_paud_cov.c\n");
  return 0;
}
