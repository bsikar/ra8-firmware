/**
 * @file test_ra_board_ek_ra8d2_comms_cov.c
 ...
 * branches and loop bodies in ra_board_ek_ra8d2_comms.c that the primary
 * test cannot reach without hardware-state manipulation:
 *
 *   Lines 154, 155, 167:
 *     Pre-seed the MIPI PHY SFR (k_ra_mipi_phy_sfr_ready_mask) so
 *     ra_mipi_phy_init spin-loops find the ready bit on the first
 *     iteration.  ra_mipi_dsi_init then runs (no spin-loop), and
 *     ra_mipi_dsi_hs_clock_start times out because PLSR stays zero in
 *     the simulator.  Covers the dsi_init call (line 154), the
 *     not-taken branch check (line 155), and the hs_clock_start return
 *     (line 167).
 *
 *   Line 225 (was 224):
 *     Call ra_board_uart_console_init with a non-zero baud before
 *     ra_cgc_init so PCLKA is still the MOCO default (~8 MHz), which is
 *     below k_ra_board_uart_console_min_pclka_hz.  The function returns
 *     k_ra_err_not_initialized at this guard.
 *
 *   Line 236 (was 235):
 *     After a successful ra_cgc_init, pre-claim the TXD pin so
 *     ra_pfs_route_peripheral returns k_ra_err_gpio_conflict and the
 *     function propagates it.
 *
 *   Line 243 (was 242):
 *     After a successful ra_cgc_init, pre-claim only the RXD pin so
 ...
 *     After a successful console init, call ra_board_uart_console_read
 *     with RDRF unset (getc times out, early-return path) and then
 *     with RDRF pre-seeded (getc succeeds, byte-copy path).
 *
 * Source lines marked GCOVR_EXCL_LINE in ra_board_ek_ra8d2_comms.c
 * (original numbering before edits):
 *   156: return after ra_mipi_dsi_init -- dsi_init with valid static cfg
 *        always returns k_ra_ok in RA_SIMULATOR_MODE.
 *   221: return after ra_cgc_get_clock_hz -- call with valid clock-id and
 *        non-null pointer always returns k_ra_ok.
 *   254: return after ra_sci_init -- sci_init(8, valid_cfg) always returns
 *        k_ra_ok in RA_SIMULATOR_MODE (MSTP timeout excluded on host).
 *
...
/**
 * @brief Pre-seed the OSCSF register and run ra_cgc_init.
 *
 * @details
 * Under RA_SIMULATOR_MODE the CGC oscillator spin-loops check the OSCSF
 ...
 * returns, s_clock_hz[PCLKA] is set to k_ra_pclka_hz (125 MHz), which
 * exceeds the console uart minimum.
 *
 * @pre ra_sim_mmap_reset() has been called (OSCSF is writable memory).
 * @post PCLKA is published to k_ra_pclka_hz in the CGC state table.
 *
 * @note Not thread-safe; single-threaded test context only.
 * @since 0.1.0
 */
...
/* -------------------------------------------------------------------------
 * 1. ra_board_mipi_dsi_init -- dsi_init path + hs_clock_start (lines 154,155,167)
 * -------------------------------------------------------------------------
 */

/**
 * @brief Verify that ra_board_mipi_dsi_init reaches ra_mipi_dsi_init and
 *        ra_mipi_dsi_hs_clock_start when ra_mipi_phy_init succeeds.
 *
 * @details
 * The existing test_stubs_return_not_supported exercises only the
 * ra_mipi_phy_init failure path (DPHY SFR never set in sim -> timeout ->
 * early return at source line 145).  To cover source lines 154, 155,
 * and 167, ra_mipi_phy_init must succeed.
 *
 ...
 *
 * @par MC/DC:
 * Decision: `if (err != k_ra_ok)` at source line 155 (1 condition).
 * - Vector A: err=k_ra_ok (this test -- dsi_init succeeds, branch not taken)
 * - Vector B: err!=k_ra_ok (would be taken if dsi_init failed; excluded
...
static void test_mipi_dsi_phy_succeeds_hs_clock_fails(void)
{
  ...
  reset_state();
  /* Pre-seed both SFR ready bits so ra_mipi_phy_init spin-loops resolve
   * on the first iteration in RA_SIMULATOR_MODE. */
  *ra_mipi_phy_reg32(k_ra_mipi_phy_off_sfr) = (uint32_t)k_ra_mipi_phy_sfr_ready_mask;
  /* ra_mipi_dsi_hs_clock_start waits for PLSR which stays zero -> timeout. */
  const ra_err_t err = ra_board_mipi_dsi_init();
  TEST_ASSERT(err != k_ra_ok);
  TEST_END("mipi_dsi_init: phy ok -> dsi ok -> hs_clock times out (lines 154,155,167)");
}

/* -------------------------------------------------------------------------
 * 2. ra_board_uart_console_write -- not-initialized guard (line 270)
 * -------------------------------------------------------------------------
 */

/**
 * @brief Verify that ra_board_uart_console_write rejects calls when the
 *        console has not been initialized.
 *
 * @details
 * The existing test (test_uart_console_write_validates in
 * test_ra_board_ek_ra8d2.c) exercises the len==0 early-success path
 * and the null-data/non-zero-len invalid-arg path, but not the
 * s_uart_console_initialized==false guard at source line 270.
 *
 * This test calls ra_board_uart_console_write with a valid non-null
 ...
 * ra_board_uart_console_init call in this sequence).
 *
 * @par MC/DC:
 ...
 * @post No side effects; guard returns before forwarding to ra_sci.
 *
 * @note Not thread-safe; single-threaded test context.
 * @since 0.1.0
 */
static void test_uart_console_write_not_initialized(void)
{
  ...
  const ra_err_t       err         = ra_board_uart_console_write(s_payload, sizeof(s_payload) - 1U);
  TEST_ASSERT_EQ(k_ra_err_not_initialized, err);
  TEST_END("uart_console_write: not-initialized returns error (line 270)");
}

/* -------------------------------------------------------------------------
 * 3. ra_board_uart_console_read -- not-initialized guard (lines 287, 288)
 * -------------------------------------------------------------------------
 */

/**
 * @brief Verify that ra_board_uart_console_read rejects calls when the
 *        console has not been initialized.
 *
 * @details
 * The existing test (test_uart_console_read_validates in
 * test_ra_board_ek_ra8d2.c) exercises null-out_len, cap==0, and
...
static void test_uart_console_read_not_initialized(void)
{
  ...
  uint8_t        buf[4]  = {};
  size_t         out_len = 0U;
  const ra_err_t err     = ra_board_uart_console_read(buf, sizeof(buf), &out_len);
  TEST_ASSERT_EQ(k_ra_err_not_initialized, err);
  TEST_END("uart_console_read: not-initialized returns error (lines 287,288)");
}

/* -------------------------------------------------------------------------
 * 4. ra_board_uart_console_init -- PCLKA below minimum (line 225)
 * -------------------------------------------------------------------------
 */

/**
 * @brief Verify that ra_board_uart_console_init rejects initialization
 *        when PCLKA is below the minimum required frequency.
 *
 * @details
 * At binary startup, s_clock_hz[PCLKA] is k_ra_moco_hz (~8 MHz), which
 * is below k_ra_board_uart_console_min_pclka_hz (16 MHz).  Calling
 * ra_board_uart_console_init with a non-zero baud rate (so it passes the
 ...
 *
 * @par MC/DC:
 * Decision: `if (pclka_hz < k_ra_board_uart_console_min_pclka_hz)` at
 ...
 * N+1=2 vectors; minimal MC/DC.
 *
 * @pre s_clock_hz[PCLKA] == k_ra_moco_hz (no cgc_init called yet).
...
static void test_uart_console_init_pclka_below_minimum(void)
{
  ...
  reset_state();
  /* PCLKA defaults to k_ra_moco_hz (~8 MHz) at binary start without cgc_init.
   * That is below k_ra_board_uart_console_min_pclka_hz (16 MHz). */
  const ra_err_t err = ra_board_uart_console_init(115200U);
  TEST_ASSERT_EQ(k_ra_err_not_initialized, err);
  TEST_END("uart_console_init: pclka below min returns not_initialized (line 225)");
}

/* -------------------------------------------------------------------------
 * 5. ra_board_uart_console_init -- TXD pin conflict (line 236)
 * -------------------------------------------------------------------------
 */

/**
 * @brief Verify that ra_board_uart_console_init propagates a pin-conflict
 *        error when the TXD pin is already claimed.
 *
 * @details
 * Source line 235 is `return err;` inside the `if (err != k_ra_ok)` check
 * after the ra_pfs_route_peripheral call for TXD (PD02 = port 13 pin 2).
 * Pre-claiming that pin via ra_pin_validator_claim causes
 * ra_pfs_route_peripheral to return k_ra_err_gpio_conflict, which is then
 * propagated.  cgc_init is run first so that the PCLKA guard does not
 * intercept the call.
 *
 * @par MC/DC:
 * Decision: `if (err != k_ra_ok)` after the TXD route call (1 condition).
 * - Vector A: err=k_ra_ok (happy path -- tests 7 and 8 in this file)
 * - Vector B: err=k_ra_err_gpio_conflict (this test)
 * N+1=2 vectors; minimal MC/DC.
 *
...
static void test_uart_console_init_txd_pin_conflict(void)
{
  ...
  const ra_port_pin_t txd_pin = (ra_port_pin_t)k_ra_board_uart_console_pin_txd;
  /* NOLINTEND(clang-analyzer-optin.core.EnumCastOutOfRange) */
  TEST_ASSERT_EQ(k_ra_ok, ra_pin_validator_claim(txd_pin, "test.txd.conflict"));
  const ra_err_t err = ra_board_uart_console_init(115200U);
...
/* -------------------------------------------------------------------------
 * 6. ra_board_uart_console_init -- RXD pin conflict (line 243)
 * -------------------------------------------------------------------------
 */
 ...
 *
 * @details
 * Source line 242 is `return err;` inside the `if (err != k_ra_ok)` check
 * after the ra_pfs_route_peripheral call for RXD (PD03 = port 13 pin 3).
 * Only the RXD pin is pre-claimed; the TXD pin is left free so the TXD
 * route succeeds and execution reaches the RXD route.
 *
 * @par MC/DC:
 * Decision: `if (err != k_ra_ok)` after the RXD route call (1 condition).
 * - Vector A: err=k_ra_ok (happy path -- tests 7 and 8 in this file)
 * - Vector B: err=k_ra_err_gpio_conflict (this test)
 * N+1=2 vectors; minimal MC/DC.
 *
...
static void test_uart_console_init_rxd_pin_conflict(void)
{
  ...
  /* NOLINTBEGIN(clang-analyzer-optin.core.EnumCastOutOfRange) */
  const ra_port_pin_t rxd_pin = (ra_port_pin_t)k_ra_board_uart_console_pin_rxd;
  /* NOLINTEND(clang-analyzer-optin.core.EnumCastOutOfRange) */
  TEST_ASSERT_EQ(k_ra_ok, ra_pin_validator_claim(rxd_pin, "test.rxd.conflict"));
  const ra_err_t err = ra_board_uart_console_init(115200U);
  TEST_ASSERT_EQ(k_ra_err_gpio_conflict, err);
...
/* -------------------------------------------------------------------------
 * 7. ra_board_uart_console_read -- loop with no byte available (lines 294-299)
 * -------------------------------------------------------------------------
 */

/**
 * @brief Verify that ra_board_uart_console_read returns k_ra_ok immediately
 *        (non-blocking stop) when ra_sci_getc_polling finds no byte.
 *
 * @details
 * A successful ra_board_uart_console_init is required to set
 ...
 * ra_sci_getc_polling spins ra_hw_wait_flag_set32 for k_ra_hw_budget_medium
 * iterations, finds RDRF clear throughout, and returns k_ra_err_hw_timeout.
 * ra_board_uart_console_read treats this as a non-blocking "no data yet"
 * and returns k_ra_ok at source line 299.
 *
 * @par MC/DC:
 * Decision: `if (err != k_ra_ok)` inside the read loop (1 condition).
 * - Vector A: err=k_ra_err_hw_timeout (this test, RDRF not set) -> taken
 * - Vector B: err=k_ra_ok             (test 8 below, RDRF pre-seeded) -> not taken
 * N+1=2 vectors; minimal MC/DC.
 *
...
/* -------------------------------------------------------------------------
 * 8. ra_board_uart_console_read -- loop with byte available (lines 301-304)
 * -------------------------------------------------------------------------
 */

/**
 * @brief Verify that ra_board_uart_console_read copies a received byte and
 *        increments out_len when ra_sci_getc_polling succeeds.
 *
 * @details
 * Continuing from test 7 (s_uart_console_initialized is still true),
 * the SCI8 CSR register is pre-seeded with RDRF (bit 31) set.
 * ra_sci_getc_polling finds RDRF set on the first spin-loop check and
 * returns k_ra_ok; RDR is zero (never written), so the received byte is 0.
 ...
 * termination check (source line 303), and returns k_ra_ok (source line 304).
 *
 * @par MC/DC:
 * (See test 7 for the full MC/DC table covering the `if (err != k_ra_ok)`
...
static void test_uart_console_read_byte_available(void)
{
  ...
  sci_reg->CSR                   = (uint32_t)(1U << (uint8_t)k_ra_sci_csr_bit_rdrf);
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_board_ek_ra8d2_comms_cov.c\n");
  return 0;
}
