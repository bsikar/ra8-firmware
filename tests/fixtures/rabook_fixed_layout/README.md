...
content model end to end through `tools/epub_compile/src/epub_compile.py` and is
...
1. **`<img src>` does not match the manifest image id.** In the emitted
   `.rabook`, the DOM image source has one parent segment followed by
   `images/page1.png`; this is the verbatim author string relative to the
   XHTML, while the image table is keyed by the manifest href
   `images/page1.png`. For any real publisher layout (`text/` and
...
2. **The `ereader_shelf` reader is text-only.** It extracts plain chapter text,
   ...
   **cover** followed by **blank** pages. `reflow` *can* lay an `<img>` out
