...
Drop real `.epub` files into `real/` and probe them against the `epub`
...
`epub_probe.c` opens an `.epub` through `epub_open()` (host /
...
bounded by fixed arenas -- the miniz pool in `epub_miniz_alloc.h` and the
shared OPF/NCX scratch in `epub_open.c`. A book that exceeds either fails
