/**
 * @file test_ra_io_fsfmt.c
 * @brief Unit tests for the ra_io filesystem-format registry (issue #159).
 *
 * @details
...
static uint8_t s_small[(size_t)k_t_small_blocks * (size_t)k_ra_io_block_size_bytes];

/** @brief Stub-format probe: block 0 begins with the stub magic byte. */
static bool stub_probe(const ra_fs_backend_t* be)
{
  if (be == nullptr) {
    return false;
  }
  ...
  if (be->read_block(be->ctx, 0, 1, blk) != k_ra_ok) {
    return false;
  }
...
static const ra_io_fsfmt_t k_test_stub = {
  .name = "stub",
  .caps =
    {
...
static ra_err_t make_volume(ra_fs_type_t type, ra_fs_backend_t* out_be)
{
  static ra_io_blockdev_ram_state_t state;
  static ra_io_blockdev_t           bd;
  RA_RETURN_ON_ERROR(ra_io_blockdev_ram_init(&bd, &state, s_disk, k_t_fat_blocks, false),
                     "t",
                     "bd");
  RA_RETURN_ON_ERROR(ra_io_blockdev_as_fs_backend(&bd, out_be), "t", "bridge");
  ra_fs_format_opts_t opts = {};
  opts.type                = type;
  opts.label               = "FMT";
  return ra_fs_format(out_be, &opts);
}
...
static void test_probe_builtins(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_io_fsfmt_init());

  ra_fs_backend_t be = {};
  TEST_ASSERT_EQ(k_ra_ok, make_volume(k_ra_fs_type_fat16, &be));
  const ra_io_fsfmt_t* fmt = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_io_fsfmt_probe(&be, &fmt));
  ...
  /* exFAT: craft a boot sector with the EXFAT name. ra_fs_format(exFAT) needs a
   * larger geometry than this fixture, and the probe only inspects block 0. */
  ra_io_blockdev_ram_state_t xstate = {};
  ra_io_blockdev_t           xbd    = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_ram_init(&xbd, &xstate, s_small, k_t_small_blocks, false));
  uint8_t xblk[(size_t)k_ra_io_block_size_bytes] = {};
  (void)memcpy(&xblk[3], "EXFAT   ", 8);
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_write(&xbd, 0, 1, xblk));
  ra_fs_backend_t xbe = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_as_fs_backend(&xbd, &xbe));
  const ra_io_fsfmt_t* xfmt = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_io_fsfmt_probe(&xbe, &xfmt));
...
static void test_foreign_format(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_io_fsfmt_init());
  TEST_ASSERT_EQ(k_ra_ok, ra_io_fsfmt_register(&k_test_stub));

  ra_io_blockdev_ram_state_t state = {};
  ra_io_blockdev_t           bd    = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_ram_init(&bd, &state, s_small, k_t_small_blocks, false));
  uint8_t blk[(size_t)k_ra_io_block_size_bytes] = {};
  blk[0]                                        = (uint8_t)k_t_stub_magic;
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_write(&bd, 0, 1, blk));
  ra_fs_backend_t be = {};
...
static void test_register_and_notfound(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_io_fsfmt_init());
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_fsfmt_register(nullptr));
  const ra_io_fsfmt_t bad_name = {.name = nullptr, .probe = stub_probe};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_fsfmt_register(&bad_name));
  const ra_io_fsfmt_t bad_probe = {.name = "x", .probe = nullptr};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_fsfmt_register(&bad_probe));
  /* fill the registry (2 built-ins already in) then overflow */
  for (uint32_t i = 0; i < (uint32_t)k_ra_io_fsfmt_max; ++i) {
    const ra_err_t e = ra_io_fsfmt_register(&k_test_stub);
    if (e != k_ra_ok) {
      TEST_ASSERT_EQ(k_ra_err_no_mem, e);
      break;
    }
  }
  ...
  ra_io_blockdev_t           bd    = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_ram_init(&bd, &state, s_small, k_t_small_blocks, false));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_erase(&bd, 0, k_t_small_blocks));
  ra_fs_backend_t be = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_as_fs_backend(&bd, &be));
  const ra_io_fsfmt_t* fmt = nullptr;
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_io_fsfmt_probe(&be, &fmt));
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_io_fsfmt.c\n");
  return 0;
}
