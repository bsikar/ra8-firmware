/**
 * @file test_ra_vsource.c
 * @brief Unit tests for the ra_mem object-source registry (Layer 1, #147).
 *
 * @details
 ...
 * page cache wired to ra_vsource_loader pages a file-like object in from a
...
static void test_loader_paged(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_vsource_loader(&vs, oid, 0U, frame, k_t_frame_bytes));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_vsource_loader(&vs, oid, 64U, frame, k_t_frame_bytes));
  ...
  TEST_ASSERT_EQ(k_ra_err_out_of_range, ra_vsource_loader(&vs, oid, 100U, frame, k_t_frame_bytes));
...
static void test_xip(void)
{
  ...
  uint32_t paged_id = 0;
  uint32_t xip_id   = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_vsource_add_paged(&vs, t_read, nullptr, 0U, 100U, &paged_id));
  TEST_ASSERT_EQ(k_ra_ok, ra_vsource_add_xip(&vs, s_xip, k_t_xip_bytes, &xip_id));

  const uint8_t* p = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_vsource_xip_ptr(&vs, xip_id, 32U, 16U, &p));
  TEST_ASSERT(p == &s_xip[32]);
  TEST_ASSERT_EQ(0, memcmp(p, &s_xip[32], 16u));
  /* out-of-range length, and the paged object refuses XIP */
  TEST_ASSERT_EQ(k_ra_err_out_of_range, ra_vsource_xip_ptr(&vs, xip_id, 250U, 16U, &p));
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_vsource_xip_ptr(&vs, paged_id, 0U, 4U, &p));
  TEST_END("vsource XIP pointer");
}

/**
 * @par MC/DC:
 * (no compound decisions under test -- a page cache wired to ra_vsource_loader
...
static void test_vmem_integration(void)
{
  ...
  static ra_vmem_frame_t meta[(size_t)k_t_frames];
  static int32_t         buckets[(size_t)k_t_buckets];

  ra_vsource_t vs = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_vsource_init(&vs, s_objs, k_t_objs));
  uint32_t oid = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_vsource_add_paged(&vs, t_read, nullptr, 0U, k_t_store_bytes, &oid));
  ...
  for (uint32_t k = 0; k < 3U; ++k) {
    void* page = nullptr;
    TEST_ASSERT_EQ(k_ra_ok, ra_vmem_get(&vm, oid, (uint64_t)offs[k], &page));
    TEST_ASSERT_EQ(0, memcmp(page, &s_store[offs[k]], (size_t)k_t_frame_bytes));
    TEST_ASSERT_EQ(k_ra_ok, ra_vmem_put(&vm, page));
  }
...
static void test_validation(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_size, ra_vsource_init(&vs, s_objs, 0U));

  TEST_ASSERT_EQ(k_ra_ok, ra_vsource_init(&vs, s_objs, 1U)); /* capacity 1 */
...
int32_t main(void)
{
  ...
  test_validation();
  (void)fprintf(stderr, "[OK  ] test_ra_vsource.c\n");
  return 0;
}
