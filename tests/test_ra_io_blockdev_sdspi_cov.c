/**
 * @file test_ra_io_blockdev_sdspi_cov.c
 * @brief Coverage-boost tests for ra_io_blockdev_sdspi.c.
 *
 * @details
 * The SD-over-SPI block-device backend is a thin shim: every vtable op
 * (read / write / erase / get_caps) forwards to the ``ra_sdmmc_spi``
 * singleton driver. The sibling test_ra_io_blockdev_backends.c only binds
 ...
 *
 * This file supplies that card by bringing the ``ra_sdmmc_spi`` driver up
 ...
 *
 * The backend's own ``RA_CHECK_NULL_PTR`` true-arms are provably
 * unreachable through the public ra_io_blockdev_* dispatcher (it null-
 ...
 * global ra_io_blockdev_sdspi.c coverage total without touching the
 * sibling test file.
 *
 ...
 *
 * @return ra_err_t Always ::k_ra_ok.
 * @retval k_ra_ok CS state recorded.
 *
 * @pre None.
 ...
 *
 * @return ra_err_t Always ::k_ra_ok.
 * @retval k_ra_ok len bytes exchanged.
 *
 * @pre len > 0.
...
/* ===========================================================================
 * SD response builder helpers (mirrors of test_ra_sdmmc_spi.c framing)
 * ===========================================================================
 */
 ...
 *
 * @pre out points at k_ra_sdmmc_spi_csd_response_len writable bytes.
 ...
 *
 * @pre csd points at k_ra_sdmmc_spi_csd_response_len bytes.
 ...
 * @post Enough RX bytes are queued to satisfy a full ra_sdmmc_spi_init.
 * @since 0.1.0
 */
...
/**
 * @brief Queue a single ra_sdmmc_spi_read_block session delivering @p block.
 *
 * @param[in] block 512-byte payload the card returns, with a valid CRC16.
 *
 * @pre block points at k_ra_sdmmc_spi_block_size bytes.
 ...
 * @since 0.1.0
 */
static void fixture_card_up(ra_io_blockdev_t* bd)
{
  per_test_setup();
...
static void test_sdspi_read_bulk_and_error(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_read(&bd, 0U, 2U, buf));
  TEST_ASSERT_EQ(0, memcmp(&buf[0], block0, (size_t)k_ra_sdmmc_spi_block_size));
  TEST_ASSERT_EQ(
    0,
    memcmp(&buf[(size_t)k_ra_sdmmc_spi_block_size], block1, (size_t)k_ra_sdmmc_spi_block_size));
 ...
 *
 * @details
 * A count=1 write routes through ra_sdmmc_spi_write_blocks' CMD24 fallback.
 * The mock returns a "data accepted" token and a one-byte busy window, so
 * the forward returns k_ra_ok.
 *
 * @par MC/DC:
 * sdspi_write has one single-condition decision, the `RA_CHECK_NULL_PTR(buf)`
 * guard. Its FALSE arm is covered here (buf non-NULL); the TRUE arm is
 * unreachable through ra_io_blockdev_write, which null-checks buf before
...
static void test_sdspi_write_forwards(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_write(&bd, 1U, 1U, buf));
 ...
 * all-zero, then erases the remaining range, so the forward returns k_ra_ok.
 *
 * @par MC/DC:
 * sdspi_erase is straight-line -- it drops ctx and tail-calls
 * ra_sdmmc_spi_erase_blocks. No boolean decision (compound or otherwise)
...
static void test_sdspi_erase_forwards(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_erase(&bd, 0U, 64U));
 ...
 * writable). Then tears the driver down so ra_sdmmc_spi_get_capacity returns
 * k_ra_err_invalid_state and the RA_RETURN_ON_ERROR early-return leg runs.
 *
 * @par MC/DC:
 * sdspi_get_caps has two single-condition decisions (no &&/||):
 * - `RA_CHECK_NULL_PTR(out)` -- FALSE arm covered here (out non-NULL); the TRUE
 *   arm is unreachable through ra_io_blockdev_get_caps, which null-checks out
 *   before forwarding.
 * - `RA_RETURN_ON_ERROR(ra_sdmmc_spi_get_capacity(...))` inner
 *   `ra_err_is_error()` -- FALSE arm covered by the happy snapshot; TRUE arm
...
static void test_sdspi_get_caps_and_error(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_sdmmc_spi_deinit());
  ra_io_blockdev_caps_t caps2 = {};
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_io_blockdev_get_caps(&bd, &caps2));
  TEST_END("sdspi_get_caps snapshot + driver error");
}

/**
 * @brief ra_io_blockdev_sdspi_init: NULL guard and successful bind.
 *
 * @details
 ...
 * `RA_CHECK_NULL_PTR(bd)` guard: the TRUE arm (bd == NULL) and FALSE arm
 ...
 * @post Every forwarding body in ra_io_blockdev_sdspi.c has executed at least
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_io_blockdev_sdspi_cov.c\n");
  return 0;
}
