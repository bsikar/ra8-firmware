 ...
 * @note Not thread-safe.
 */
static void open_s_epub(ra_epub_book_t* book)
{
  const ra_epub_mem_media_t media = {.data = s_epub, .size = s_epub_len};
  TEST_ASSERT_EQ(k_ra_ok, ra_epub_open(&media, "pipe.epub", book));
}
...
{
  TEST_BEGIN("ra_rabook_pipeline: BMP images transcoded to gray4");
  build_epub_raster();
  ra_fs_mount_t* mount = fresh_volume();
...
{
  TEST_BEGIN("ra_rabook_pipeline: gray scratch too small skips downscale image");
  build_epub_raster();
  ra_fs_mount_t* mount = fresh_volume();
  ...
  teardown(mount);
  TEST_END("ra_rabook_pipeline: gray scratch too small skips downscale image");
}
...
{
  TEST_BEGIN("ra_rabook_pipeline: TOC titles attach to chapters");
  build_epub_toc();
  ra_fs_mount_t* mount = fresh_volume();
  ...
  TEST_ASSERT_EQ(0, strcmp(ra_book_string(s_readback, chaps[0].title_off), "Chapter One"));
  TEST_ASSERT_EQ(0, strcmp(ra_book_string(s_readback, chaps[1].title_off), "Chapter Two"));

  teardown(mount);
  TEST_END("ra_rabook_pipeline: TOC titles attach to chapters");
}
 ...
 *
 * @par MC/DC:
 * Drives the `if (err == k_ra_err_not_found)` true arm in s_compile_stylesheets
 * (single condition): the manifest declares style.css but the ZIP omits it, so
 * ra_epub_get_resource returns not_found and the stage `continue`s rather than
...
{
  TEST_BEGIN("ra_rabook_pipeline: absent CSS item skipped");
  ...
  teardown(mount);
  TEST_END("ra_rabook_pipeline: absent CSS item skipped");
}
 ...
 *
 * @par MC/DC:
 * Drives the `if (err != k_ra_ok)` true arm of the stylesheet resource load
 * (single condition): the stylesheet is present but the load buffer is sized to
 * three bytes, so ra_epub_get_resource returns k_ra_err_no_mem -- neither ok nor
 ...
 * compile-failure guard in ra_rabook_compile_from_epub (no file is written).
 */
static void test_pipeline_css_load_error_propagates(void)
{
  TEST_BEGIN("ra_rabook_pipeline: CSS load error propagates");
  build_epub_css(true);
  ra_fs_mount_t* mount = fresh_volume();
  ...
  teardown(mount);
  TEST_END("ra_rabook_pipeline: CSS load error propagates");
}
 ...
 *
 * @par MC/DC:
 * Drives the `if (err != k_ra_ok)` true arm of the resource load in
 * s_add_manifest_image (single condition): the manifest declares missing.png but
 * the archive omits it, so ra_epub_get_resource fails and the item returns nil
...
{
  TEST_BEGIN("ra_rabook_pipeline: absent image item skipped");
 ...
 *
 * @par MC/DC:
 * Drives the `if (err != k_ra_ok)` true arm of the chapter-load guard in
 * s_compile_chapters (single condition): the XHTML scratch is sized to four
 * bytes, so ra_epub_load_chapter returns k_ra_err_no_mem and the stage returns
...
{
  TEST_BEGIN("ra_rabook_pipeline: chapter overflow propagates");
  build_epub(false);
  ra_fs_mount_t* mount = fresh_volume();
  ...
  teardown(mount);
  TEST_END("ra_rabook_pipeline: chapter overflow propagates");
}
 ...
 *
 * @par MC/DC:
 * Drives the `if (err != k_ra_ok)` true arm of the parse guard in
 * s_compile_chapters (single condition): the chapter body is plain text with no
 * element, so ra_rabook_xml_parse_chapter finds no `<body>` root and returns
 * k_ra_err_no_mem, which the stage and s_compile_to_blob propagate. The false arm
 * is the well-formed chapter every passing compile parses.
 */
static void test_pipeline_malformed_chapter_propagates(void)
{
  TEST_BEGIN("ra_rabook_pipeline: malformed chapter propagates");
  ...
  teardown(mount);
  TEST_END("ra_rabook_pipeline: malformed chapter propagates");
}
 ...
 *
 * @par MC/DC:
 * Covers every guard of ra_rabook_compile_from_epub_to_buffer. The shared-arg
...
{
  TEST_BEGIN("ra_rabook_pipeline: compile-to-buffer + null guards");
  build_epub(false);
  ...
  uint32_t    len  = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_rabook_compile_from_epub_to_buffer(&book, &bufs, &scr, &blob, &len));
  TEST_ASSERT(blob != nullptr && len > 0U);
  TEST_ASSERT_EQ(k_ra_ok, ra_book_validate((const uint8_t*)blob, (size_t)len));

  /* Null guards: shared-arg check, then each output pointer. */
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rabook_compile_from_epub_to_buffer(nullptr, &bufs, &scr, &blob, &len));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rabook_compile_from_epub_to_buffer(&book, &bufs, &scr, nullptr, &len));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rabook_compile_from_epub_to_buffer(&book, &bufs, &scr, &blob, nullptr));

  TEST_ASSERT_EQ(k_ra_ok, ra_epub_close(&book));
  TEST_END("ra_rabook_pipeline: compile-to-buffer + null guards");
}
...
{
  TEST_BEGIN("ra_rabook_pipeline: filesystem entry null guards");
  build_epub(false);
  ra_fs_mount_t* mount = fresh_volume();
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rabook_compile_from_epub(nullptr, &bufs, &scr, mount, "OUT.RAB"));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rabook_compile_from_epub(&book, &bufs, &scr, nullptr, "OUT.RAB"));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rabook_compile_from_epub(&book, &bufs, &scr, mount, nullptr));

  TEST_ASSERT_EQ(k_ra_ok, ra_epub_close(&book));
  teardown(mount);
  TEST_END("ra_rabook_pipeline: filesystem entry null guards");
}

/**
 * @test test_pipeline_compile_init_rejects_bad_buffers
 * @brief A NULL builder-arena member fails ra_rabook_compile_init and propagates
 *        out of s_compile_to_blob.
 *
 * @par MC/DC:
 * Drives the `if (err != k_ra_ok)` true arm of the builder-init guard in
 ...
 * ra_rabook_compile_init rejects the arenas and s_compile_to_blob returns its
 * error. The false arm is every passing compile.
 */
static void test_pipeline_compile_init_rejects_bad_buffers(void)
{
  TEST_BEGIN("ra_rabook_pipeline: compile_init rejects bad arenas");
  build_epub(false);
  ...
  uint32_t    len  = 0U;
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_rabook_compile_from_epub_to_buffer(&book, &bufs, &scr, &blob, &len));

  TEST_ASSERT_EQ(k_ra_ok, ra_epub_close(&book));
  TEST_END("ra_rabook_pipeline: compile_init rejects bad arenas");
}
 ...
 *
 * @par MC/DC:
 * Drives the `if (err != k_ra_ok)` true arm of the chapter-count guard in
 ...
 * ra_epub_get_chapter_count then returns k_ra_err_not_initialized, which the
...
{
  TEST_BEGIN("ra_rabook_pipeline: closed book fails at chapter count");

  ra_epub_book_t book = {}; /* never opened: in_use == 0 */
  ...
  TEST_END("ra_rabook_pipeline: closed book fails at chapter count");
}
