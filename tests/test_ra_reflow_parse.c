/**
 * @file test_ra_reflow_parse.c
 * @brief MC/DC vectors for libs/ra_reflow/src/ra_reflow_parse.c
 *
 ...
 * (2 conditions, libs/ra_reflow/src/ra_reflow_parse.c line 62)
 *
 ...
 *        -> C1=T shorts. Decision T -> k_ra_err_null_ptr.
 *  - V3: engine=&e, xhtml_buf=NULL
 *        -> C1=F, C2=T. Decision T -> k_ra_err_null_ptr.
 *
 * Independence:
 *  - V1 vs V2 vary C1 with C2 held F: outcome flips.
 *  - V1 vs V3 vary C2 with C1 held F: outcome flips.
 *
 * Note: V1 returns ``k_ra_err_not_initialized`` (the next guard), not
 * ``k_ra_ok``, because s_engine.in_use == 0. The MC/DC obligation
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_reflow_parse.c\n");
  return 0;
}
