/**
 * @file test_ra_ble_security_cov.c
 * @brief Coverage-completion tests for libs/ra_ble_host/src/ra_ble_security.c.
 *
 * @details
 * The baseline suite in test_ra_ble_security.c exercises init / close /
 * bond-count / event dispatch, but never drives the three action APIs
 * ra_ble_security_pair(), ra_ble_security_passkey_reply() (happy path)
 * and ra_ble_security_clear_bonds(). Under the host build the vendor
 * NimBLE tree is not linked (RA_TARGET_BUILD is undefined), so those
 * functions reduce to their pure-software #else legs: an
 * initialized-guard plus an unconditional k_ra_ok return. Every one of
 ...
 */
static void init_with(ra_ble_security_io_cap_t io_cap)
{
 ...
 * ra_ble_security_pair, ra_ble_security.c initialized-guard).
 * - V1 initialized==0 -> C1=T. Decision T (return k_ra_err_not_initialized).
...
static void test_pair_not_initialized(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_ble_security_pair(0x0010U));
 ...
 *   discards conn_handle and returns k_ra_ok).
...
static void test_pair_initialized(void)
{
  ...
  reset_state();
  init_with(k_ra_ble_io_cap_no_input_no_out);
  /* Host #else leg has no controller to reject the handle -> k_ra_ok. */
  TEST_ASSERT_EQ(k_ra_ok, ra_ble_security_pair(0x0010U));
 ...
 * @par MC/DC:
 * Decision: `if (s_state.initialized == 0U)` (1 condition,
 * ra_ble_security_passkey_reply, ra_ble_security.c initialized-guard).
 * - V1 initialized==0 -> C1=T. Decision T (return k_ra_err_not_initialized),
...
static void test_passkey_reply_not_initialized(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_ble_security_passkey_reply(0x0020U, 424242U, 1U));
 ...
 *
 * @par MC/DC:
 * Two single-condition decisions in ra_ble_security_passkey_reply:
 ...
 * discards conn_handle / accept and returns k_ra_ok. Pairs with
...
static void test_passkey_reply_happy(void)
{
  ...
  reset_state();
  init_with(k_ra_ble_io_cap_keyboard_only);
  /* In-range passkey (boundary min) + accept flag -> host leg returns k_ra_ok. */
  TEST_ASSERT_EQ(k_ra_ok, ra_ble_security_passkey_reply(0x0020U, 0U, 0U));
  /* Boundary max 999999 also accepted. */
  TEST_ASSERT_EQ(k_ra_ok, ra_ble_security_passkey_reply(0x0020U, 999999U, 1U));
 ...
 * @par MC/DC:
 * Decision: `if (s_state.initialized == 0U)` (1 condition,
 * ra_ble_security_clear_bonds, ra_ble_security.c initialized-guard).
 * - V1 initialized==0 -> C1=T. Decision T (return k_ra_err_not_initialized).
...
static void test_clear_bonds_not_initialized(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_ble_security_clear_bonds());
 ...
 *   returns k_ra_ok. Completes the N+1 pair; also asserts the postcondition
...
static void test_clear_bonds_initialized(void)
{
  ...
  ra_ble_security_test_set_bond_count(3U);
  uint8_t count = 0xFFU;
  TEST_ASSERT_EQ(k_ra_ok, ra_ble_security_bond_count(&count));
  TEST_ASSERT_EQ(3U, count);
  /* Clear -> host leg zeroes the mirror. */
  TEST_ASSERT_EQ(k_ra_ok, ra_ble_security_clear_bonds());
  count = 0xFFU;
  TEST_ASSERT_EQ(k_ra_ok, ra_ble_security_bond_count(&count));
