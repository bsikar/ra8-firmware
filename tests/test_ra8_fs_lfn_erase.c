 ...
 * ::test_unlink_leaves_no_orphans() proves it stays quiet after a real
 * `ra8_fs_unlink()`. That alone would prove nothing -- a scanner that can never
 * fire is also always quiet -- so ::test_orphan_scan_negative_control() first
...
/**
 * @test test_unlink_leaves_no_orphans
 ...
 *   test_orphan_scan_negative_control below plants it deliberately).
 * - V4: a long-name slot with a DIFFERENT checksum -> C1=T, C2=T, C3=F -> the
 *   run resets, so an unrelated chain is not swept up
 *   (test_rename_moves_the_chain leaves one in front of the new entry).
 ...
 * @since 0.1.0
 */
static void test_unlink_leaves_no_orphans(void)
{
  ...
  write_and_verify(h, "/Quarterly Report 2026.pdf");
...
/**
 * @test test_orphan_scan_negative_control
 * @brief The orphan scanner fires on the deletion this fix replaced.
 *
 * @details Without this case, ::test_unlink_leaves_no_orphans() proves nothing:
 ...
 * @since 0.1.0
 */
static void test_orphan_scan_negative_control(void)
{
  ...
  const uint8_t* ent83 = root_slot(h, 2U);
  TEST_ASSERT(ent83[k_lw_off_attr] != (uint8_t)k_lw_attr_lfn);
  root_slot(h, 2U)[k_lw_off_name] = (uint8_t)k_lw_free_used;
...
/**
 * @test test_rename_moves_the_chain
 ...
 * @since 0.1.0
 */
static void test_rename_moves_the_chain(void)
{
  ...
  write_and_verify(h, "/Draft Chapter One.txt");
  write_and_verify(h, "/Occupied Name Here.txt");
  ...
  TEST_ASSERT_EQ(1U, listdir_is_exactly(h, "/", "Final Chapter One Revised.txt"));
  TEST_ASSERT_EQ(0U, count_orphan_slots(h));
  ...
  uint32_t got                = 0U;
  fill_payload(want, (uint32_t)k_lw_payload);
  ...
  TEST_ASSERT_EQ(1U, listdir_is_exactly(h, "/", "CH1.TXT"));
...
/**
 * @test test_long_named_directory
 ...
 * @since 0.1.0
 */
static void test_long_named_directory(void)
{
  ...
  TEST_ASSERT_EQ(1U, listdir_is_exactly(h, "/", "Reading List"));

  write_and_verify(h, "/Reading List/Chapter One.txt");
  TEST_ASSERT_EQ(1U, listdir_is_exactly(h, "/Reading List", "Chapter One.txt"));
...
/**
 * @test test_formatted_volume_images
 * @brief Build the two images the out-of-band `fsck.fat` evidence is taken from.
 *
 * @details Same story as ::test_unlink_leaves_no_orphans() and its negative
 ...
 * ::test_unlink_leaves_no_orphans(); this case exists for the artefacts.
 *
 ...
 * @since 0.1.0
 */
static void test_formatted_volume_images(void)
{
  ...
  write_and_verify(h, "/Quarterly Report 2026.pdf");
  write_and_verify(h, "/Meeting Notes 2026-08-04.md");
  maybe_dump_image("populated");
  ...
  root_slot(h, chain_len)[k_lw_off_name] = (uint8_t)k_lw_free_used;
  scan_root_of(h, &r);
  TEST_ASSERT_EQ(chain_len, r.orphans);
  maybe_dump_image("orphan_control");
...
int32_t main(void)
{
  test_unlink_leaves_no_orphans();
  test_orphan_scan_negative_control();
  test_rename_moves_the_chain();
  test_long_named_directory();
  test_formatted_volume_images();
  (void)fprintf(stderr, "[OK  ] test_ra8_fs_lfn_erase.c\n");
  return 0;
}
