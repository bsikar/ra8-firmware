/**
 * @file test_ra_glyph_atlas.c
 * @brief Unit tests for the ra_mem glyph atlas (Layer 3, #147).
 *
 * @details
...
static ra_glyph_atlas_cfg_t t_cfg(void)
{
  ra_glyph_atlas_cfg_t cfg = {};
...
/** @brief A glyph key. */
static ra_glyph_key_t t_key(uint32_t glyph, uint16_t face, uint16_t size, uint16_t mode)
{
  ra_glyph_key_t k = {};
  k.glyph_id       = glyph;
  k.face_id        = face;
...
static void test_render_hit(void)
{
  ...
  s_render_calls           = 0;
  ra_glyph_atlas_t     a   = {};
  ra_glyph_atlas_cfg_t cfg = t_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_init(&a, &cfg));

  ra_glyph_key_t k = t_key(0x41U, 1U, 16U, 0U);
  ra_glyph_t     g = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_get(&a, &k, &g)); /* miss -> render */
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_put(&a, g.bitmap));

  ra_glyph_t g2 = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_get(&a, &k, &g2)); /* hit -> no render */
  TEST_ASSERT(g2.bitmap == g.bitmap);
  TEST_ASSERT_EQ(1, s_render_calls); /* unchanged */
  TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_put(&a, g2.bitmap));

  uint32_t hits = 0;
  uint32_t miss = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_stats(&a, &hits, &miss, nullptr));
...
static void test_eviction(void)
{
  ...
  for (uint32_t i = 0; i < 8U; ++i) {
    ra_glyph_key_t k = t_key(i, 1U, 16U, 0U);
    ra_glyph_t     g = {};
    TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_get(&a, &k, &g));
    TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_put(&a, g.bitmap));
  }
  uint32_t ev = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_stats(&a, nullptr, nullptr, &ev));
  ...
  ra_glyph_key_t k0     = t_key(0U, 1U, 16U, 0U);
  ra_glyph_t     g0     = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_get(&a, &k0, &g0)); /* evicted -> re-render */
  TEST_ASSERT_EQ(before + 1U, s_render_calls);
  TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_put(&a, g0.bitmap));
...
static void test_pin_protection(void)
{
  ...
  for (uint32_t i = 0; i < (uint32_t)k_t_cells; ++i) {
    ra_glyph_key_t k = t_key(i, 2U, 12U, 0U);
    ra_glyph_t     g = {};
    TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_get(&a, &k, &g)); /* pinned (no put) */
    pins[i] = g.bitmap;
  }
  ra_glyph_key_t kn = t_key(99U, 2U, 12U, 0U);
  ra_glyph_t     gn = {};
  TEST_ASSERT_EQ(k_ra_err_no_mem, ra_glyph_atlas_get(&a, &kn, &gn)); /* all pinned */
  TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_put(&a, pins[0]));
  TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_get(&a, &kn, &gn)); /* now evicts + renders */
  TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_put(&a, gn.bitmap));
  for (uint32_t i = 1; i < (uint32_t)k_t_cells; ++i) {
    TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_put(&a, pins[i]));
  }
...
static void test_validation(void)
{
  ...
  ra_glyph_atlas_t     a   = {};
  ra_glyph_atlas_cfg_t cfg = t_cfg();
  ra_glyph_key_t       k   = t_key(1U, 1U, 16U, 0U);
  ra_glyph_t           g   = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_glyph_atlas_init(nullptr, &cfg));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_glyph_atlas_init(&a, nullptr));
  ra_glyph_atlas_cfg_t bad = t_cfg();
  bad.cell_count           = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_size, ra_glyph_atlas_init(&a, &bad));

  TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_init(&a, &cfg));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_glyph_atlas_get(nullptr, &k, &g));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_glyph_atlas_get(&a, nullptr, &g));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_glyph_atlas_get(&a, &k, nullptr));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_glyph_atlas_put(&a, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_glyph_atlas_put(&a, &s_cells[1])); /* mid-cell */
  TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_get(&a, &k, &g));
  TEST_ASSERT_EQ(k_ra_ok, ra_glyph_atlas_put(&a, g.bitmap));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_glyph_atlas_put(&a, g.bitmap)); /* already unpinned */
...
int32_t main(void)
{
  ...
  test_pin_protection();
  test_validation();
  (void)fprintf(stderr, "[OK  ] test_ra_glyph_atlas.c\n");
  return 0;
}
