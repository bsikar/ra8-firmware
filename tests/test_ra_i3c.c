...
static void test_init(void)
{
  ...
  TEST_ASSERT_EQ(0, ra_i3c()->RSTCTL);
  TEST_END("i3c init");
}
...
static void test_status_read_and_clear(void)
{
  ...
  uint32_t mask = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_get_status(&mask));
  TEST_ASSERT_EQ(0xDEADBEEFU, mask);
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_clear_status(0xF0F0F0F0U));
  /* Bits in mask 0xF0F0F0F0 must have been cleared. */
  TEST_ASSERT_EQ((0xDEADBEEFU & ~0xF0F0F0F0U), ra_i3c()->INST);
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_i3c_get_status(nullptr));
...
static void test_set_address(void)
{
  ...
  TEST_ASSERT_EQ(expect, ra_i3c()->MSDVAD);
  /* Out-of-range 7-bit dynamic address must be rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_set_address(0x80U));
...
static void test_bus_enable(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_bus_enable(false));
  TEST_ASSERT_EQ(0, ((ra_i3c()->BCTL >> 31) & 0x1U));
...
static void test_dynamic_address_assign(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_dynamic_address_assign(targets, 2U));
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_i3c_dynamic_address_assign(nullptr, 1U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_dynamic_address_assign(targets, 0U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_dynamic_address_assign(targets, 99U));
...
static void test_set_dynamic_address(void)
{
  ...
  TEST_ASSERT_EQ(0x34U, ra_i3c()->NCMDQP);
  /* Bounds check on both addresses. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_set_dynamic_address(0x80U, 0x10U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_set_dynamic_address(0x10U, 0x80U));
...
static void test_reset_dynamic_addresses(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_reset_dynamic_addresses());
  /* RSTDAA cmd2 is zero -> last write. */
  TEST_ASSERT_EQ(0, ra_i3c()->NCMDQP);
...
static void test_send_ccc_broadcast(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_send_ccc(0x00U, 0U, nullptr, 0U));
...
static void test_send_ccc_directed_with_payload(void)
{
  ...
  TEST_ASSERT_EQ(expect_data, ra_i3c()->NCMDQP);
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_i3c_send_ccc(0x80U, 0x42U, nullptr, 3U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_send_ccc(0x80U, 0x80U, payload, 3U));
...
static void test_recv_ccc_directed(void)
{
  ...
  uint8_t buf[4]  = {};
  uint8_t got_len = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_recv_ccc(0x8DU, 0x42U, buf, 4U, &got_len));
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_recv_ccc(0x07U, 0x42U, buf, 4U, &got_len));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_i3c_recv_ccc(0x8DU, 0x42U, nullptr, 4U, &got_len));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_i3c_recv_ccc(0x8DU, 0x42U, buf, 4U, nullptr));
...
static void test_write_regular(void)
{
  ...
  TEST_ASSERT_EQ(0x08070605U, ra_i3c()->NTDTBP0);
...
static void test_ibi_read_empty(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_i3c_ibi_read(nullptr));
...
static void test_set_hdr_mode_ts_and_validation(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_set_hdr_mode(0x80U, k_ra_i3c_hdr_mode_ddr));
  /* Bogus mode rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_set_hdr_mode(0x10U, (ra_i3c_hdr_mode_t)9U));
...
static void test_ibi_enable_writes_ntibivctl(void)
{
  ...
  TEST_ASSERT_EQ(1, (ra_i3c()->NTIBIVCTL & 0xFFU));

  /* Out-of-range address rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_ibi_enable(0x80U));
...
static void test_ibi_drain_aliases_read(void)
{
  ...
  /* Empty queue path. */
  ra_i3c_ibi_t ibi = {};
  TEST_ASSERT_EQ(k_ra_err_no_data, ra_i3c_ibi_drain(&ibi));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_i3c_ibi_drain(nullptr));

  /* Stage one IBI and verify drain returns it. */
  ra_i3c()->NTST    = (uint32_t)k_ra_i3c_ntst_ibiqeff_mask;
  ra_i3c()->NIBIQP  = ((uint32_t)0x84U << 8); /* len=0, IBI ID=0x84. */
  ra_i3c()->NTDTBP0 = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_ibi_drain(&ibi));
  TEST_ASSERT_EQ(0x42U, ibi.address);
...
static void test_mcdc_i3c(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_dynamic_address_assign(&one, 0U));
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_dynamic_address_assign(&one, 1U));
  /* Use a count strictly greater than k_ra_i3c_max_targets. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_dynamic_address_assign(many, 0xFFU));

  /* Decision B: set_dynamic_address */
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_set_dynamic_address(0x33U, 0x44U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_set_dynamic_address(0x80U, 0x44U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_i3c_set_dynamic_address(0x33U, 0x80U));

  /* Decision C: send_ccc */
  uint8_t payload[1] = {0xA5U};
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_send_ccc(0x00U, 0x33U, nullptr, 0U));
  TEST_ASSERT_EQ(k_ra_ok, ra_i3c_send_ccc(0x00U, 0x33U, payload, 1U));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_i3c_send_ccc(0x00U, 0x33U, nullptr, 1U));
