...
RA8_TEST_HELPER void ra8_test_rabook_verify(const ra8_test_rabook_roundtrip_t* roundtrip)
{
  ...
  const ra8_book_chapter_t* chapters = ra8_book_chapters(blob);
  TEST_ASSERT_EQ(0, strcmp(ra8_book_string(blob, chapters[0].title_off), "Chapter One"));
  TEST_ASSERT_EQ(0, strcmp(ra8_book_string(blob, chapters[0].href_off), "ch1.xhtml"));
  TEST_ASSERT_EQ(roundtrip->body_idx, chapters[0].root_node);

  const ra8_book_node_t* nodes = ra8_book_nodes(blob);
  const ra8_book_node_t* body  = &nodes[roundtrip->body_idx];
  ...
  const ra8_book_node_t* para = &nodes[roundtrip->p_idx];
  TEST_ASSERT_EQ(roundtrip->text_idx, para->first_child);
  const ra8_book_node_t* text = &nodes[roundtrip->text_idx];
