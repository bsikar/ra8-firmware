 ...
 * header/member builders used by `test_ra8_unarch_tar.c`. Keeping fixture
...
static const uint8_t s_tt_ustar_magic[] = {'u', 's', 't', 'a', 'r'};
/** @brief ustar version field, the two ASCII digits "00" at offset 263. */
static const uint8_t s_tt_ustar_version[] = {'0', '0'};
                               ...
                               uint64_t    dsize)
{
  ...
  memcpy(&block[k_tt_off_magic], s_tt_ustar_magic, sizeof(s_tt_ustar_magic));
  block[k_tt_off_magic_nul] = 0U;
  memcpy(&block[k_tt_off_version], s_tt_ustar_version, sizeof(s_tt_ustar_version));
  if (prefix != nullptr) {
...
static size_t internal_tb_add(size_t      off,
                              ...
                              size_t      dsize)
{
  internal_tb_header(&s_arc[off], name, prefix, type, (uint64_t)dsize);
  off += k_tt_tar_block;
  if (dsize > 0U) {
...
RA8_INTERNAL
static ra8_err_t
internal_tt_open(ra8_unarch_tar_t* t, ra8_unarch_mem_t* mem, const ra8_decomp_limits_t* lim)
{
  mem->base = s_arc;
  mem->len  = s_arc_len;
