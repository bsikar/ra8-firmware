/**
 * @file test_ra_spi_b_target_cov.c
 * @brief Coverage tests for the SPI_B target-mode SPCMD encoding paths.
 *
 * @details
 * Companion to ``test_ra_spi_b_target.c``.  The existing suite exercises
 * ``ra_spi_b_target_init`` and ``ra_spi_b_target_xfer`` with SPI mode 0
 ...
 *
 * Coverage targets (lines in ``ra_spi_b_target.c``):
 *
 *  - 217-218 : ``case k_ra_spi_mode_1:`` -- sets CPHA (bit 0).
 *  - 220-221 : ``case k_ra_spi_mode_2:`` -- sets CPOL (bit 1).
 *  - 223-225 : ``case k_ra_spi_mode_3:`` -- sets CPHA | CPOL.
 *  - 231-232 : ``if (cfg->lsb_first)`` true branch -- sets LSBF (bit 12).
 *
 * Each test calls ``ra_spi_b_target_init`` and reads the SPCMD[0] word
...
/**
 * @enum ra_spi_b_target_cov_test_t
 * @brief Integer constants used by coverage tests (magic-number gate).
 *
 * @details
 * The SPB field in SPCMD[0] is programmed with ``k_ra_spcmd_spb_8bit``
 * (value 7) shifted left to position ``k_ra_spcmd_bit_spb_lo`` (bit 16),
 * giving ``0x00070000``.  Each mode adds its CPOL/CPHA bits on top of
 * that fixed SPB encoding.  The LSB-first bit (LSBF, bit 12,
 * ``k_ra_spcmd_mask_lsbf = 0x00001000``) is orthogonal and tested with
...
typedef enum : uint32_t {
   ...
   * k_ra_spcmd_mask_lsbf=0x1000 + k_cov_spb_8bit_shifted.
   */
  k_cov_spcmd_mode0_lsbf = 0x00071000U,
} ra_spi_b_target_cov_test_t;
 ...
 *
 * @details Mirrors the ``prep`` helper in ``test_ra_spi_b_target.c`` so
 ...
 *
 * @details
 * Calls ``ra_spi_b_target_init`` with ``k_ra_spi_mode_1`` and reads the
 ...
 *
 * Covers ``ra_spi_b_target.c`` lines 217-218 (``case k_ra_spi_mode_1``
 * body and break).
 *
 * @par MC/DC: switch arm k_ra_spi_mode_1
 * One condition per arm; a single vector per arm satisfies MC/DC.
 * Vector: cfg.mode = k_ra_spi_mode_1 -> CPHA set, CPOL clear.
 */
 ...
 *
 * @details
 * Calls ``ra_spi_b_target_init`` with ``k_ra_spi_mode_2`` and asserts
 * that SPCMD[0] equals ``k_cov_spcmd_mode2``: only CPOL (bit 1) and
 * the 8-bit SPB encoding are set; CPHA (bit 0) is clear.
 *
 * Covers ``ra_spi_b_target.c`` lines 220-221 (``case k_ra_spi_mode_2``
 * body and break).
 *
 * @par MC/DC: switch arm k_ra_spi_mode_2
 * Vector: cfg.mode = k_ra_spi_mode_2 -> CPOL set, CPHA clear.
 */
 ...
 *
 * @details
 * Calls ``ra_spi_b_target_init`` with ``k_ra_spi_mode_3`` and asserts
 * that SPCMD[0] equals ``k_cov_spcmd_mode3``: both CPHA (bit 0) and
 * CPOL (bit 1) are set, along with the 8-bit SPB encoding.
 *
 * Covers ``ra_spi_b_target.c`` lines 223-225 (``case k_ra_spi_mode_3``
 * body lines and break).
 *
 * @par MC/DC: switch arm k_ra_spi_mode_3
 * Vector: cfg.mode = k_ra_spi_mode_3 -> CPHA set, CPOL set.
 */
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_spi_b_target_cov.c\n");
  return 0;
}
