 ...
 *
 * Every scenario ends with `exfat_verify()`, the structural scan that recomputes
 ...
 * @since 0.1.0
 */
static uint32_t cluster_bytes(const ra8_fs_mount_t* h)
{
 ...
 * @retval >0 `entries_per_cluster / 3`.
 *
 * @pre @p h is non-NULL and mounted.
 ...
 * @since 0.1.0
 */
static uint32_t files_per_cluster(const ra8_fs_mount_t* h)
{
  return entries_per_cluster(h) / (uint32_t)k_growth_entries_per_file;
}
 ...
 * @since 0.1.0
 */
static void make_empty_file(ra8_fs_mount_t* h, const char* path)
{
 ...
 * @since 0.1.0
 */
static void fill_dir(ra8_fs_mount_t* h, const char* dir, uint32_t first, uint32_t count)
{
  for (uint32_t i = 0U; i < count; i++) {
    char path[k_growth_path_cap] = {};
    (void)snprintf(path, sizeof(path), "%s/F%05u", dir, first + i);
    make_empty_file(h, path);
  }
}
 ...
 * @since 0.1.0
 */
static void drain_dir(ra8_fs_mount_t* h, const char* dir, uint32_t first, uint32_t count)
{
  for (uint32_t i = 0U; i < count; i++) {
    char path[k_growth_path_cap] = {};
    (void)snprintf(path, sizeof(path), "%s/F%05u", dir, first + i);
 ...
 * @since 0.1.0
 */
static void consume_clusters(ra8_fs_mount_t* h, const char* path, uint32_t clusters)
{
  const uint32_t cbytes = cluster_bytes(h);
 ...
 * @since 0.1.0
 */
static uint8_t first_dir_flags(const ra8_fs_mount_t* h)
{
 ...
 * @since 0.1.0
 */
static uint32_t first_dir_datalen(const ra8_fs_mount_t* h)
{
  const uint32_t strm_off = root_byte(h, (uint32_t)k_mut_root_strm0_idx);
  return disk_get_u32le(strm_off + (uint32_t)k_mut_strm_off_dlen);
}
 ...
 * @since 0.1.0
 */
static void test_fill_past_one_cluster(void)
{
  ...
  const uint32_t  fpc      = files_per_cluster(h);

  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_mkdir(h, "/BIG"));
  TEST_ASSERT_EQ(baseline + 1U, alloc_bitmap_used(h)); /* one cluster, as ever */

  /* Fill to exactly the pre-#677 ceiling: the directory must still own its one
   * cluster, because these sets all fit inside it. */
  fill_dir(h, "/BIG", 0U, fpc);
  TEST_ASSERT_EQ(baseline + 1U, alloc_bitmap_used(h));
  exfat_verify(h, "grow_ceiling_reached");
  ...
  make_empty_file(h, "/BIG/F00042ceil");
  TEST_ASSERT_EQ(baseline + 2U, alloc_bitmap_used(h));
  ...
  fill_dir(h, "/BIG", fpc + 1U, total - (fpc + 1U));

  name_ctx_t ctx = {};
  TEST_ASSERT_EQ(total, list_names(h, "/BIG", &ctx));
  const uint32_t expect_clusters = (total + fpc - 1U) / fpc;
  TEST_ASSERT_EQ(baseline + expect_clusters, alloc_bitmap_used(h));
  ...
  exfat_verify(h, "grow_past_one_cluster");
 ...
 * @since 0.1.0
 */
static void test_growth_across_bitmap_sector_boundary(void)
{
  ...
  consume_clusters(h, "/FILL.BIN", to_fill);
  TEST_ASSERT_EQ(target, alloc_bitmap_used(h));
  ...
  fill_dir(h, "/D", 0U, nfiles);
  name_ctx_t ctx = {};
  TEST_ASSERT_EQ(nfiles, list_names(h, "/D", &ctx));
  exfat_verify(h, "grow_bitmap_boundary_built");

  /* Free the whole run back across the boundary and confirm nothing leaks. */
  drain_dir(h, "/D", 0U, nfiles);
  ...
  exfat_verify(h, "grow_bitmap_boundary_freed");
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  free_volume();
 ...
 * @since 0.1.0
 */
static void test_convert_forces_fat_chain(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_mkdir(h, "/D"));
  TEST_ASSERT_EQ(cbytes, first_dir_datalen(h)); /* one cluster */
  TEST_ASSERT_EQ(1U, ((first_dir_flags(h) & (uint8_t)k_mut_no_fat_bit) != 0U) ? 1U : 0U);
  ...
  fill_pattern(payload, (uint32_t)k_growth_blk_payload, (uint8_t)k_growth_seed_a);
  ...
  TEST_ASSERT_EQ(0U, ((first_dir_flags(h) & (uint8_t)k_mut_no_fat_bit) != 0U) ? 1U : 0U);
  TEST_ASSERT_EQ(cbytes * 3U, first_dir_datalen(h));
  ...
  exfat_verify(h, "grow_converted_to_chain");
  ...
  exfat_verify(h, "grow_chain_freed");
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  free_volume();
 ...
 * @since 0.1.0
 */
static void test_rmdir_multicluster_frees_all(void)
{
  ...
  fill_dir(h, "/M", 0U, nfiles);
  const uint32_t clusters = (nfiles + fpc - 1U) / fpc;
  TEST_ASSERT_EQ(baseline + clusters, alloc_bitmap_used(h));
  exfat_verify(h, "grow_multicluster_before_rmdir");

  drain_dir(h, "/M", 0U, nfiles);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_rmdir(h, "/M"));
  TEST_ASSERT_EQ(baseline, alloc_bitmap_used(h)); /* every cluster of the run came back */
  TEST_ASSERT_EQ(k_ra8_err_not_found, ra8_fs_stat(h, "/M", &(ra8_fs_stat_t){}));

  exfat_verify(h, "grow_multicluster_after_rmdir");
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(h));
  free_volume();
 ...
 * (libs/ra8_fs/src/ra8_fs_fat_exfat_openw.c@priv_exfat_open_created), the
 ...
 * @since 0.1.0
 */
static void test_volume_full_reports_no_mem(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_mkdir(h, "/D"));
  fill_dir(h, "/D", 0U, fpc); /* fills the one cluster exactly, no growth yet */
...
int32_t main(void)
{
  test_fill_past_one_cluster();
  test_growth_across_bitmap_sector_boundary();
  test_convert_forces_fat_chain();
  test_rmdir_multicluster_frees_all();
  test_volume_full_reports_no_mem();
  (void)fprintf(stderr, "[OK  ] test_ra8_fs_exfat_dir_growth.c\n");
  return 0;
}
