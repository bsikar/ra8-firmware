 ...
 *
 * @details
 * The regular host test build compiles ``ra_psa_crypto.c`` under
 * ``RA_SIMULATOR_MODE``, which substitutes a deterministic stand-in for AES-GCM
 ...
 * RA_SIMULATOR_MODE`` path calls through ``psa_*``) directly, with NO
 * RA_SIMULATOR_MODE, and pins it to published vectors:
 *
 ...
 *
 * A stub backend cannot satisfy these; the sim ``ra_psa_crypto_sim.c`` would
 ...
 * only) and does not link the sim'd ``ra_core_hal``.
 *
