...
static void test_init_bad_speed(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_hmsc_init((ra_usb_speed_t)9U));
...
static void test_pre_init_guards(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_hmsc_read_capacity(0U, &block_count, &block_size));
...
static void test_build_cbw_signature_layout(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_hmsc_build_cbw(0U, 512U, true, cdb, 10U, cbw));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_hmsc_build_cbw(1U, 256U, false, cdb, 10U, cbw));
...
static void test_build_cbw_arg_rejection(void)
{
  ...
  uint8_t cdb[10]             = {};
  uint8_t cbw[k_test_cbw_len] = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_hmsc_build_cbw(0U, 0U, true, nullptr, 10U, cbw));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_hmsc_build_cbw(0U, 0U, true, cdb, 10U, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_hmsc_build_cbw(99U, 0U, true, cdb, 10U, cbw));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_hmsc_build_cbw(0U, 0U, true, cdb, 0U, cbw));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_hmsc_build_cbw(0U, 0U, true, cdb, 99U, cbw));
...
static void test_decode_csw_status(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_hmsc_decode_csw(csw, 0xCAFEBABEU, &status_out));
  TEST_ASSERT_EQ(k_ra_hmsc_csw_status_passed, status_out);

  /* status = 0x01 (failed). */
  csw[k_test_csw_off_status] = 0x01U;
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_hmsc_decode_csw(csw, 0xCAFEBABEU, &status_out));
  TEST_ASSERT_EQ(k_ra_hmsc_csw_status_failed, status_out);

  /* status = 0x02 (phase error). */
  csw[k_test_csw_off_status] = 0x02U;
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_hmsc_decode_csw(csw, 0xCAFEBABEU, &status_out));
  TEST_ASSERT_EQ(k_ra_hmsc_csw_status_phase_error, status_out);

  /* status = 0x99 -> rejected as bogus. */
  csw[k_test_csw_off_status] = 0x99U;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_hmsc_decode_csw(csw, 0xCAFEBABEU, &status_out));

  /* Tag mismatch -> invalid_arg. */
  csw[k_test_csw_off_status] = 0x00U;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_hmsc_decode_csw(csw, 0xDEADBEEFU, &status_out));
  ...
  /* NULL pointers. */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_hmsc_decode_csw(nullptr, 0U, &status_out));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_hmsc_decode_csw(csw, 0U, nullptr));
...
static void test_mcdc_hmsc(void)
{
  ...
  /* B-V2: cdb_len=6 -> ok. */
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_hmsc_build_cbw(0U, 36U, true, cdb6, 6U, cbw));
  /* B-V1: cdb_len=0 -> invalid_arg. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_hmsc_build_cbw(0U, 0U, false, cdb6, 0U, cbw));
  /* B-V3: cdb_len=20 -> invalid_arg. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_hmsc_build_cbw(0U, 0U, false, cdb6, 20U, cbw));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_hmsc_decode_csw(csw, tag, &out_status));
...
static void test_mcdc_hmsc_decode_csw_status_and_chain(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_hmsc_decode_csw(csw, 0xCAFEBABEU, &out));
  TEST_ASSERT_EQ(k_ra_hmsc_csw_status_passed, out);

  csw[(size_t)k_test_csw_off_status] = (uint8_t)k_ra_hmsc_csw_status_failed;
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_hmsc_decode_csw(csw, 0xCAFEBABEU, &out));
  TEST_ASSERT_EQ(k_ra_hmsc_csw_status_failed, out);

  csw[(size_t)k_test_csw_off_status] = (uint8_t)k_ra_hmsc_csw_status_phase_error;
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_hmsc_decode_csw(csw, 0xCAFEBABEU, &out));
  TEST_ASSERT_EQ(k_ra_hmsc_csw_status_phase_error, out);
