/**
 * @file test_ra_flash_irq_cov.c
 * @brief Coverage booster for ra_flash_irq.c.
 *
 * @details
 * Drives the branches in ra_flash_irq.c left uncovered by the primary test
 * suite (test_ra_flash.c + test_ra_flash_edge_cases.c):
 *
 *   - MASTAT.MREAE delivery in ra_flash_dispatch_isr (lines 301-303).
 *   - MASTAT.CMDLK delivery in ra_flash_dispatch_isr (lines 305-307).
 ...
 *   - ra_flash_erase: happy-path loop and success return (lines 438,
 *     442-449, 450).
 *   - ra_flash_erase: loop early-return on hw error (line 446).
 *   - ra_flash_write: happy-path loop and success return (lines 462,
 *     466-472, 475-476).
 *   - ra_flash_write: loop early-return on hw error (line 473).
 *   - ra_flash_blank_check: in_extra RHS evaluation (line 504).
 *
 * Lines 169 and 171 (the default: arm of the set_irq_enable switch) carry
 * GCOVR_EXCL_LINE markers in the source because the src >= k_ra_flash_irq_count
...
/**
 * @enum ra_flash_irq_cov_addr_t
 * @brief Per-test address constants for ra_flash_irq_cov tests.
 *
 * @details
 ...
 *
 * @code
 * ra_flash_erase((uintptr_t)k_test_below_code_aligned, 1U);
 * @endcode
 *
 * @since 0.1.0
 */
 ...
 * @details Incremented once per ra_flash_dispatch_isr callback delivery.
 ...
 * @note Not thread-safe; tests are single-threaded.
 * @since 0.1.0 */
static ra_flash_irq_src_t s_cov_cb_last = (ra_flash_irq_src_t)0U;
 ...
 *
 * @param[in] ev Event descriptor from ra_flash_dispatch_isr.
 *
 * @pre Called from ra_flash_dispatch_isr with a valid event pointer.
 * @post s_cov_cb_count incremented; s_cov_cb_last updated.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
static void cov_callback(const ra_flash_isr_event_t* ev)
{
  s_cov_cb_count++;
...
/**
 * @brief Return a valid ra_flash_cfg_t for use in tests requiring init.
 *
 * @return Populated configuration with sensible clock and ECC settings.
 *
 * @pre None.
 * @post Returned struct is valid for ra_flash_init().
 *
 * @note Thread-safe (returns by value).
 * @since 0.1.0
 */
static ra_flash_cfg_t cfg_make(void)
{
...
/**
 * @brief Verify ra_flash_dispatch_isr delivers the extra-MRAM access-error event.
 *
 * @details
 * Sets MASTAT.MREAE to 1 and registers a callback, then calls
 * ra_flash_dispatch_isr.  Asserts that exactly one callback was delivered
 * with src == k_ra_flash_irq_extra_err, covering lines 301-303 in
 * ra_flash_irq.c.
 *
 * @par MC/DC:
 * (no compound decisions in this test -- the if at line 300 is a single
 * condition: `(mastat & k_ra_mastat_mask_mreae) != 0U`)
 *
 * @pre ra_sim_mmap_reset() called.
 * @post s_cov_cb_count == 1, s_cov_cb_last == k_ra_flash_irq_extra_err.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
static void test_dispatch_mastat_mreae(void)
{
  ...
  ra_sim_mmap_reset();
  s_cov_cb_count = 0U;
  s_cov_cb_last  = (ra_flash_irq_src_t)0U;

  TEST_ASSERT_EQ(k_ra_ok, ra_flash_callback_set(cov_callback, nullptr));
  /* HUM Ch 59 "MASTAT : Extra MRAM Access Status Register" p 3577 */
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mastat) = (uint8_t)k_ra_mastat_mask_mreae;
  const uint32_t n                              = ra_flash_dispatch_isr();
  TEST_ASSERT_EQ(1, n);
  TEST_ASSERT_EQ(1, s_cov_cb_count);
  TEST_ASSERT_EQ(k_ra_flash_irq_extra_err, s_cov_cb_last);
...
/**
 * @brief Verify ra_flash_dispatch_isr delivers the command-lock event.
 *
 * @details
 * Sets MASTAT.CMDLK to 1 and registers a callback, then calls
 * ra_flash_dispatch_isr.  Asserts that exactly one callback was delivered
 * with src == k_ra_flash_irq_extra_cmdlk, covering lines 305-307.
 *
 * @par MC/DC:
 * (no compound decisions in this test -- the if at line 304 is a single
 * condition: `(mastat & k_ra_mastat_mask_cmdlk) != 0U`)
 *
 * @pre ra_sim_mmap_reset() called.
 * @post s_cov_cb_count == 1, s_cov_cb_last == k_ra_flash_irq_extra_cmdlk.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
static void test_dispatch_mastat_cmdlk(void)
{
  ...
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mastat) = (uint8_t)k_ra_mastat_mask_cmdlk;
  ...
  TEST_ASSERT_EQ(1, n);
  TEST_ASSERT_EQ(1, s_cov_cb_count);
  TEST_ASSERT_EQ(k_ra_flash_irq_extra_cmdlk, s_cov_cb_last);
...
/**
 * @brief Verify ra_flash_erase rejects an aligned address below code MRAM.
 *
 * @details
 * Uses an address that satisfies the 32-byte alignment check (so that
 * check does not mask line 418) but falls below k_ra_flash_code_start,
 ...
 * condition: `address < k_ra_flash_code_start`)
 *
 * @pre ra_flash_init() succeeded.
 * @post ra_flash_erase returns k_ra_err_invalid_arg.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
static void test_validate_range_below_code_start_aligned(void)
{
  ...
  const ra_flash_cfg_t cfg = cfg_make();
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_init(&cfg));

  /* 0x01FFE000 is 32-byte aligned (0x01FFE000 & 0x1F == 0) and below
   * k_ra_flash_code_start (0x02000000), so the alignment check passes and
   * the address-below-code-start check (line 417-418) fires. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_flash_erase((uintptr_t)k_test_below_code_aligned, 1U));
  TEST_END("flash erase: aligned addr below code-MRAM rejected (line 418)");
}

/* ---------------------------------------------------------------------------
 * Test: ra_flash_erase happy path (lines 377,383-384,426,438,442-450)
 * ------------------------------------------------------------------------ */

/**
 * @brief Verify the ra_flash_erase happy path completes without error.
 *
 * @details
 ...
 * k_ra_ok on the first spin), then calls ra_flash_erase with one block at
 * k_ra_flash_code_start.  The call must pass internal_validate_range
 ...
 * before returning k_ra_ok (line 450).
 *
 * @par MC/DC:
 ...
 * @post ra_flash_erase returns k_ra_ok.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
static void test_erase_happy_path(void)
{
  ...
  /* Stage MRCPS so that both spin-loops inside ra_flash_write_block return
  ...
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mrcps) = (uint8_t)k_ra_mrcps_mask_abufemp;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_erase((uintptr_t)k_ra_flash_code_start, 1U));
  TEST_END("flash erase: happy path covers loop and success return");
}

/* ---------------------------------------------------------------------------
 * Test: ra_flash_erase loop error return (line 446)
 * ------------------------------------------------------------------------ */

/**
 * @brief Verify ra_flash_erase propagates an erase_block hw error immediately.
 *
 * @details
 * Pre-stages MRCPS with ABUFEMP (so the spin-loops complete) and PRGERRC
 * (so the post-write status check inside ra_flash_write_block returns
 * k_ra_err_hw_error).  The error propagates through ra_flash_erase_block
 * back to the ra_flash_erase loop, which returns it at line 446.
 *
 * @par MC/DC:
 * (no compound decisions in this test -- the if at line 445 is a single
 * condition: `err != k_ra_ok`)
 *
 * @pre ra_flash_init() succeeded.
 * @pre MRCPS = ABUFEMP | PRGERRC.
 * @post ra_flash_erase returns k_ra_err_hw_error.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
static void test_erase_loop_hw_error(void)
{
   ...
   * error check in ra_flash_write_block -> k_ra_err_hw_error propagates up. */
  /* HUM Ch 59 "MRCPS : Code MRAM Program Status Register" p 3601 */
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mrcps) =
    (uint8_t)(k_ra_mrcps_mask_abufemp | k_ra_mrcps_mask_prgerrc);

  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_flash_erase((uintptr_t)k_ra_flash_code_start, 1U));
  TEST_END("flash erase: loop propagates erase_block hw error (line 446)");
}

/* ---------------------------------------------------------------------------
 * Test: ra_flash_write happy path (lines 462,466-472,475-476)
 * ------------------------------------------------------------------------ */

/**
 * @brief Verify the ra_flash_write happy path completes without error.
 *
 * @details
 * Calls ra_flash_write with a 32-byte buffer aligned to k_ra_flash_code_start.
 * MRCPS is pre-staged with ABUFEMP so both spin-loops inside
 * ra_flash_write_block resolve immediately and no error bits are detected.
 ...
 * @pre ra_flash_init() succeeded.
 * @pre MRCPS.ABUFEMP = 1.
 * @post ra_flash_write returns k_ra_ok.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
static void test_write_happy_path(void)
{
  ...
  *ra_mram_reg8((uint16_t)k_ra_mram_off_mrcps) = (uint8_t)k_ra_mrcps_mask_abufemp;
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_flash_write((uintptr_t)k_ra_flash_code_start, buf, k_ra_mram_write_size_bytes));
  TEST_END("flash write: happy path covers loop and success return");
}

/* ---------------------------------------------------------------------------
 * Test: ra_flash_write loop error return (line 473)
 * ------------------------------------------------------------------------ */

/**
 * @brief Verify ra_flash_write propagates a write_block hw error immediately.
 *
 * @details
 * Pre-stages MRCPS with ABUFEMP | PRGERRC.  The spin-loops complete
 * normally but the post-write error check in ra_flash_write_block returns
 * k_ra_err_hw_error, which the ra_flash_write loop propagates at line 473.
 *
 * @par MC/DC:
 ...
 * @pre ra_flash_init() succeeded.
 * @pre MRCPS = ABUFEMP | PRGERRC.
 * @post ra_flash_write returns k_ra_err_hw_error.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
...
/* ---------------------------------------------------------------------------
 * Test: ra_flash_blank_check in_extra RHS (line 504)
 * ------------------------------------------------------------------------ */

/**
 * @brief Verify ra_flash_blank_check evaluates the in_extra upper-bound
 *        condition when the address is inside the extra-MRAM window.
 *
 * @details
 * Calls ra_flash_blank_check with an address inside the extra-MRAM region
 * (k_ra_flash_extra_start = 0x27000000).  The logical-AND at lines 503-504
 * short-circuits on the first condition when the address is below
 * k_ra_flash_extra_start; to reach line 504 the first condition must be
 * true, which happens here because the address equals k_ra_flash_extra_start.
 ...
 *
 * @pre Extra-MRAM sim region at k_ra_flash_extra_start is mmap-backed.
 * @post ra_flash_blank_check returns k_ra_ok with out_blank = true.
 *
 * @note No ra_flash_init required; blank_check has no init guard.
 * @since 0.1.0
 */
static void test_blank_check_extra_mram_path(void)
{
  ...
  volatile uint8_t* p = (volatile uint8_t*)(uintptr_t)k_ra_flash_extra_start;
  for (uint32_t i = 0U; i < 16U; ++i) {
    p[i] = 0xFFU;
  }

  bool blank = false;
  /* address >= k_ra_flash_extra_start forces evaluation of line 503 (true)
   * and therefore also line 504 (the upper-bound check). */
  TEST_ASSERT_EQ(k_ra_ok, ra_flash_blank_check((uintptr_t)k_ra_flash_extra_start, 16U, &blank));
...
/**
 * @brief Run all ra_flash_irq_cov coverage tests.
 *
 * @return 0 on success (individual test failures exit via TEST_FAIL_FMT).
 *
 * @pre Host test binary linked against ra_core_hal with RA_SIMULATOR_MODE.
 * @post All uncovered branches in ra_flash_irq.c are exercised.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
int main(void)
{
  ...
  (void)fprintf(stderr, "[DONE] test_ra_flash_irq_cov: all tests passed\n");
  return 0;
}
