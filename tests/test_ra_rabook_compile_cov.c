/**
 * @file test_ra_rabook_compile_cov.c
 * @brief Line-coverage top-up for the RABOOK1 builder error / guard legs (#149).
 *
 * @details
 * The sibling @ref test_ra_rabook_compile.c drives the happy path plus a few
 * single-arena overflow arms. This file completes line coverage of
 * `libs/ra_rabook_compile/src/ra_rabook_compile.c` by exercising every remaining
 ...
 * inputs alone:
 *
 *  - NULL-context guards on every builder entry point (return @ref k_ra_book_nil).
 *  - The sticky `failed` short-circuit on @ref ra_rabook_intern,
 *    @ref ra_rabook_add_element, @ref ra_rabook_add_text,
 *    @ref ra_rabook_add_chapter, @ref ra_rabook_add_image,
 *    @ref ra_rabook_add_stylesheet and @ref ra_rabook_set_metadata.
 *  - The NULL-payload latch: @ref ra_rabook_intern with a NULL string,
 *    @ref ra_rabook_add_element with NULL attrs / non-zero count, and
 *    @ref ra_rabook_add_image with NULL data / non-zero size.
 ...
 *  - The out-of-range index rejections in @ref ra_rabook_link_child and
 *    @ref ra_rabook_link_sibling (parent/child and node/sibling arms).
 *  - The two @ref ra_rabook_finalize size-error arms: a computed layout that
...
static ra_book_chapter_t    s_chapters[k_c_chapter_cap];
static ra_book_node_t       s_nodes[k_c_node_cap];
static ra_book_attr_t       s_attrs[k_c_attr_cap];
static ra_book_stylesheet_t s_styles[k_c_style_cap];
static ra_book_image_t      s_images[k_c_image_cap];
...
{
  TEST_BEGIN("ra_rabook_compile: intern NULL-str latch + failed short-circuit");
...
{
  TEST_BEGIN("ra_rabook_compile: add_element NULL-attrs latch + failed short-circuit");
  ra_rabook_buffers_t buf = full_buffers();
  ra_rabook_ctx_t     ctx = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_rabook_compile_init(&ctx, &buf));
...
{
  TEST_BEGIN("ra_rabook_compile: add_text overflow latch + failed short-circuit");
  ra_rabook_buffers_t buf = full_buffers();
  buf.node_cap            = 1U;
  ra_rabook_ctx_t ctx     = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_rabook_compile_init(&ctx, &buf));
 ...
 * child guard; both return @ref k_ra_err_invalid_arg.
 */
static void test_rabook_cov_link_child_range(void)
{
  TEST_BEGIN("ra_rabook_compile: link_child out-of-range parent + child");
  ra_rabook_buffers_t buf = full_buffers();
  ra_rabook_ctx_t     ctx = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_rabook_compile_init(&ctx, &buf));

  const uint32_t only = ra_rabook_add_text(&ctx, ra_rabook_intern(&ctx, "n"));
  TEST_ASSERT_EQ(0U, only);

  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_rabook_link_child(&ctx, (uint32_t)k_c_out_of_range_index, only));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_rabook_link_child(&ctx, only, (uint32_t)k_c_out_of_range_index));
  TEST_END("ra_rabook_compile: link_child out-of-range parent + child");
}
 ...
 * sibling guard; both return @ref k_ra_err_invalid_arg.
 */
static void test_rabook_cov_link_sibling_range(void)
{
  TEST_BEGIN("ra_rabook_compile: link_sibling out-of-range node + sibling");
  ra_rabook_buffers_t buf = full_buffers();
  ra_rabook_ctx_t     ctx = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_rabook_compile_init(&ctx, &buf));
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_rabook_link_sibling(&ctx, (uint32_t)k_c_out_of_range_index, only));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_rabook_link_sibling(&ctx, only, (uint32_t)k_c_out_of_range_index));
  TEST_END("ra_rabook_compile: link_sibling out-of-range node + sibling");
}
...
{
  TEST_BEGIN("ra_rabook_compile: add_chapter overflow latch + failed short-circuit");
  ra_rabook_buffers_t buf = full_buffers();
  buf.chapter_cap         = 1U;
  ra_rabook_ctx_t ctx     = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_rabook_compile_init(&ctx, &buf));
...
{
  TEST_BEGIN("ra_rabook_compile: add_image NULL-data latch");
  ra_rabook_buffers_t buf = full_buffers();
  ra_rabook_ctx_t     ctx = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_rabook_compile_init(&ctx, &buf));
  ...
  TEST_ASSERT(ctx.failed);
  TEST_END("ra_rabook_compile: add_image NULL-data latch");
}
...
{
  TEST_BEGIN("ra_rabook_compile: add_image pool overflow latch + failed short-circuit");
  ra_rabook_buffers_t buf = full_buffers();
  ...
  TEST_ASSERT_EQ(k_ra_book_nil,
                 ra_rabook_add_image(&ctx,
                                     ...
                                     data,
                                     (uint32_t)k_c_pool_overflow_bytes));
  TEST_END("ra_rabook_compile: add_image pool overflow latch + failed short-circuit");
}
...
{
  TEST_BEGIN("ra_rabook_compile: add_stylesheet overflow latch + failed short-circuit");
  ra_rabook_buffers_t buf = full_buffers();
 ...
 * @ref k_ra_err_no_mem without recording any offset.
 */
static void test_rabook_cov_set_metadata_failed(void)
{
  TEST_BEGIN("ra_rabook_compile: set_metadata after overflow -> no_mem");
  ra_rabook_buffers_t buf = full_buffers();
  ra_rabook_ctx_t     ctx = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_rabook_compile_init(&ctx, &buf));
  ...
  TEST_END("ra_rabook_compile: set_metadata after overflow -> no_mem");
}
 ...
 * arm, s_compute_layout returns @ref k_ra_err_invalid_size, and finalize
...
{
  TEST_BEGIN("ra_rabook_compile: finalize layout > 32-bit -> invalid_size");
  ra_rabook_ctx_t ctx   = {};
  ...
  uint32_t    len  = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_size, ra_rabook_finalize(&ctx, &blob, &len));
  TEST_ASSERT_NULL(blob);
  TEST_END("ra_rabook_compile: finalize layout > 32-bit -> invalid_size");
}
 ...
 * takes its true arm and finalize returns @ref k_ra_err_invalid_size. The
...
{
  TEST_BEGIN("ra_rabook_compile: finalize blob > out_cap -> invalid_size");
  ra_rabook_buffers_t buf = full_buffers();
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_size, ra_rabook_finalize(&ctx, &blob, &len));
  TEST_ASSERT_NULL(blob);
  TEST_END("ra_rabook_compile: finalize blob > out_cap -> invalid_size");
}
 ...
 *          `ra_log_error`; registering a sink makes `internal_itm_ready()`
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_rabook_compile_cov.c\n");
  return 0;
}
