/**
 * @file test_ra_box.c
 * @brief Unit tests for the ra_box layout engine (stack / grid / sizing).
 *
 * @details
 * Pure host tests -- ra_box is allocation-free geometry. Covers the
 ...
 * shared child-walk guard), ``ra_box_add`` and ``ra_box_layout``.
 *
...
static ra_box_t leaf(int16_t fixed, uint16_t flex)
{
  ra_box_t n  = {};
...
static ra_box_t container(ra_box_kind_t kind, int16_t pad, int16_t gap, uint8_t cols)
{
  ra_box_t n  = {};
  n.kind      = (uint8_t)kind;
  n.pad       = pad;
 ...
 *
 * @par MC/DC:
 * (functional test; ra_box_add and ra_box_layout compound decisions are
 * covered by their dedicated MC/DC tests)
 */
static void test_box_build(void)
{
  TEST_BEGIN("ra_box build + links");
  ra_box_t      store[k_box_cap];
  ra_box_tree_t t;
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_box_tree_init(nullptr, store, k_box_cap));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_box_tree_init(&t, store, 0U));
  TEST_ASSERT_EQ(k_ra_ok, ra_box_tree_init(&t, store, k_box_cap));

  ra_box_t      root = container(k_ra_box_stack_v, 0, 0, 1U);
  const int16_t ri   = ra_box_add(&t, (int16_t)k_ra_box_none, &root);
  TEST_ASSERT_EQ(0, ri);
  ra_box_t      a  = leaf(10, 0U);
  ra_box_t      b  = leaf(20, 0U);
  const int16_t ai = ra_box_add(&t, ri, &a);
  const int16_t bi = ra_box_add(&t, ri, &b);
  TEST_ASSERT_EQ(1, ai);
  TEST_ASSERT_EQ(2, bi);
  /* Sibling chain: root.first_child == a, a.next == b. */
  TEST_ASSERT_EQ(ai, store[ri].first_child);
  TEST_ASSERT_EQ(bi, store[ai].next);
  TEST_END("ra_box build + links");
}
...
{
  TEST_BEGIN("ra_box stack_v fixed+flex");
  ra_box_t      store[k_box_cap];
  ra_box_tree_t t;
  (void)ra_box_tree_init(&t, store, k_box_cap);
  ra_box_t           rootn = container(k_ra_box_stack_v, 10, 10, 1U);
  const int16_t      r     = ra_box_add(&t, (int16_t)k_ra_box_none, &rootn);
  ra_box_t           an = leaf(30, 0U), bn = leaf(0, 1U), cn = leaf(20, 0U);
  const int16_t      a     = ra_box_add(&t, r, &an);
  const int16_t      b     = ra_box_add(&t, r, &bn);
  const int16_t      c     = ra_box_add(&t, r, &cn);
  const ra_ui_rect_t frame = {0, 0, 100, 200};
  ...
  TEST_END("ra_box stack_v fixed+flex");
}
...
{
  TEST_BEGIN("ra_box stack_h flex split");
  ra_box_t      store[k_box_cap];
  ra_box_tree_t t;
  (void)ra_box_tree_init(&t, store, k_box_cap);
  ra_box_t           rootn = container(k_ra_box_stack_h, 0, 20, 1U);
  const int16_t      r     = ra_box_add(&t, (int16_t)k_ra_box_none, &rootn);
  ra_box_t           an = leaf(0, 1U), bn = leaf(0, 1U);
  const int16_t      a     = ra_box_add(&t, r, &an);
  const int16_t      b     = ra_box_add(&t, r, &bn);
  const ra_ui_rect_t frame = {0, 0, 220, 50};
  TEST_ASSERT_EQ(k_ra_ok, ra_box_layout(&t, r, &frame));
  ...
  TEST_END("ra_box stack_h flex split");
}
...
{
  TEST_BEGIN("ra_box grid row-major");
  ra_box_t      store[k_box_cap];
  ra_box_tree_t t;
  (void)ra_box_tree_init(&t, store, k_box_cap);
  ra_box_t           rootn = container(k_ra_box_grid, 0, 10, 2U);
  const int16_t      r     = ra_box_add(&t, (int16_t)k_ra_box_none, &rootn);
  ra_box_t           c0 = leaf(0, 0U), c1 = leaf(0, 0U), c2 = leaf(0, 0U);
  const int16_t      i0    = ra_box_add(&t, r, &c0);
  const int16_t      i1    = ra_box_add(&t, r, &c1);
  const int16_t      i2    = ra_box_add(&t, r, &c2);
  const ra_ui_rect_t frame = {0, 0, 100, 100};
  TEST_ASSERT_EQ(k_ra_ok, ra_box_layout(&t, r, &frame));
  ...
  TEST_END("ra_box grid row-major");
}
 ...
 *
 * @par MC/DC:
 * Decision: `(link != k_ra_box_none) && (guard < count)`
 * (2 conditions, libs/ra_box/src/ra_box.c@internal_iter_live)
 * Standard: DO-178C Table A-7 obj 5. Exercised indirectly through the
 * child walks in ra_box_layout:
 ...
 * live -- is only reachable on a cyclic link, which ra_box_add never
...
{
  TEST_BEGIN("ra_box internal_iter_live MC/DC");
  ra_box_t      store[k_box_cap];
  ra_box_tree_t t;
  (void)ra_box_tree_init(&t, store, k_box_cap);
  ...
  TEST_END("ra_box internal_iter_live MC/DC");
}
 ...
 *
 * @par MC/DC:
 * Decisions in libs/ra_box/src/ra_box.c@ra_box_add:
...
{
  TEST_BEGIN("ra_box ra_box_add MC/DC");
  ra_box_t      store[k_box_cap];
  ra_box_tree_t t;
  (void)ra_box_tree_init(&t, store, k_box_cap);
  ra_box_t n = leaf(0, 1U);

  TEST_ASSERT_EQ(k_ra_box_none, ra_box_add(nullptr, (int16_t)k_ra_box_none, &n));
  TEST_ASSERT_EQ(k_ra_box_none, ra_box_add(&t, (int16_t)k_ra_box_none, nullptr));

  const int16_t root = ra_box_add(&t, (int16_t)k_ra_box_none, &n); /* A=F -> root */
  TEST_ASSERT_EQ(0, root);
  const int16_t child = ra_box_add(&t, root, &n); /* A=T,B=F,C=F -> child */
  TEST_ASSERT_EQ(1, child);
  TEST_ASSERT_EQ(k_ra_box_none, ra_box_add(&t, (int16_t)-2, &n)); /* B flips */
  TEST_ASSERT_EQ(k_ra_box_none, ra_box_add(&t, (int16_t)50, &n)); /* C flips */
  TEST_END("ra_box ra_box_add MC/DC");
}
 ...
 *
 * @par MC/DC:
 * Decision in libs/ra_box/src/ra_box.c@ra_box_layout:
...
{
  TEST_BEGIN("ra_box ra_box_layout MC/DC");
  ra_box_t           store[k_box_cap];
  ra_box_tree_t      t;
  const ra_ui_rect_t frame = {0, 0, 10, 10};
  (void)ra_box_tree_init(&t, store, k_box_cap);
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_box_layout(&t, 0, &frame)); /* count=0 */
  ra_box_t      n = leaf(0, 1U);
  ...
  TEST_END("ra_box ra_box_layout MC/DC");
}
