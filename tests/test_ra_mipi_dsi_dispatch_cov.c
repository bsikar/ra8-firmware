/**
 * @file test_ra_mipi_dsi_dispatch_cov.c
 * @brief Coverage-focused host unit tests for ra_mipi_dsi_dispatch.c
 *
 * @details
 * Companion to `test_ra_mipi_dsi.c`. That file exercises the public
 * contract; this file drives the residual uncovered lines of
 * `ra_mipi_dsi_dispatch.c` deterministically by pre-seeding the
 * simulated MIPI DSI-2 register block through `ra_sim_mmap`:
 *
 *  - `ra_mipi_dsi_video_start` with the HBP/HFP "no-LP" flags set
 *    (the sibling test only sets HSA);
 *  - `ra_mipi_dsi_clear_status` for the sequence-channel-1, receive,
 *    fatal-error, and PHY-lane clear branches;
 *  - `ra_mipi_dsi_rx_result_get` for receive-result slots 1, 2, and 3;
 *  - `ra_mipi_dsi_dispatch_video` buffer over/underflow soft-reset leg;
 *  - the top-level `ra_mipi_dsi_dispatch` fan-out for every interrupt
...
typedef enum : uint32_t {
  ...
  k_cov_all_isr         = (uint32_t)k_ra_mipi_dsi_isr_sq0 | (uint32_t)k_ra_mipi_dsi_isr_sq1 |
 ...
 */
static ra_mipi_dsi_event_t s_cov_cb_last_event;
 ...
 */
static void stub_cov_cb(void* ctx, ra_mipi_dsi_event_t event, uint32_t status_mask)
{
  (void)ctx;
  ++s_cov_cb_count;
...
static void prep(void)
{
  ...
  (void)ra_mstp_init();
  s_cov_cb_count      = 0U;
  s_cov_cb_last_event = k_ra_mipi_dsi_event_phy;
 ...
 *
 * @return A fully-populated ::ra_mipi_dsi_video_cfg_t.
 */
static ra_mipi_dsi_video_cfg_t make_video_cfg_all_no_lp(void)
{
 ...
 * @par MC/DC:
 * (no compound decision -- `ra_mipi_dsi_video_start` uses three
...
static void test_cov_video_start_hbp_hfp_no_lp(void)
{
  ...
  const ra_mipi_dsi_video_cfg_t v = make_video_cfg_all_no_lp();
  ...
  TEST_ASSERT(((reg->VMSET0R & (uint32_t)k_ra_mipi_dsi_vmset0_hbpnolp) != 0U));
  TEST_ASSERT(((reg->VMSET0R & (uint32_t)k_ra_mipi_dsi_vmset0_hfpnolp) != 0U));
 ...
 * @par MC/DC:
 * `ra_mipi_dsi_clear_status` is a chain of six independent
...
static void test_cov_rx_result_slots_1_2_3(void)
{
  ...
  /* Slot 1 -> RXRSS1R. */
  reg->RXRSSR  = (uint32_t)k_ra_mipi_dsi_rxrssr_slt1vld;
  ...
  reg->RXRSSR  = (uint32_t)k_ra_mipi_dsi_rxrssr_slt2vld;
  ...
  memset((void*)&r, 0, sizeof(r));
  reg->RXRSSR  = (uint32_t)k_ra_mipi_dsi_rxrssr_slt3vld;
...
static void test_cov_dispatch_video_buffer_reset(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_mipi_dsi_attach_handler(stub_cov_cb, nullptr));

  volatile r_mipi_dsi_regs_t* reg = ra_mipi_dsi();

  /* V1: overflow -> soft-reset leg executes, RSTCR ends at 0. */
  reg->RSTCR = (uint32_t)k_cov_rstcr_seed;
  reg->VMSR  = (uint32_t)k_ra_mipi_dsi_vmsr_vbufovf;
  ra_mipi_dsi_dispatch_video();
  TEST_ASSERT_EQ(0U, reg->RSTCR);
  TEST_ASSERT_EQ(k_ra_mipi_dsi_event_video, s_cov_cb_last_event);

  /* V3: underflow -> the other mask bit also drives the reset leg. */
  reg->RSTCR = (uint32_t)k_cov_rstcr_seed;
  reg->VMSR  = (uint32_t)k_ra_mipi_dsi_vmsr_vbufudf;
  ra_mipi_dsi_dispatch_video();
  TEST_ASSERT_EQ(0U, reg->RSTCR);
 ...
 * @par MC/DC:
 * `ra_mipi_dsi_dispatch` is a chain of seven independent
...
static void test_cov_dispatch_fanout_all_and_none(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_mipi_dsi_event_phy, s_cov_cb_last_event);
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_mipi_dsi_dispatch_cov.c\n");
  return 0;
}
