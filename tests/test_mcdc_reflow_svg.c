...
/** @brief The number every `internal_numf()` vector must parse out of its input. */
static const float s_t_expect_five = 5.0F;
 ...
 */
static const float s_t_below_neg_3p4 = 3.4F;

/** @brief Lower bound proving a transform argument was parsed, not defaulted. */
static const float s_t_parsed_nonzero = 0.5F;
...
/** @brief Host framebuffer (ARGB8888) bound by every internal_render/draw case. */
 ...
 * libs/ra8_reflow/src/ra8_reflow_svg_xform.c@priv_ra8_svgp_numf).
...
RA8_INTERNAL static void internal_test_svgp_numf_scanner_mcdc(void)
{
  ...
  TEST_ASSERT(internal_numf(" 5") == s_t_expect_five);     /* Dec A V2: whitespace skip   */
  TEST_ASSERT(internal_numf(",5") == s_t_expect_five);     /* Dec A V3: comma skip (C3)   */
  TEST_ASSERT(internal_numf("5") == s_t_expect_five);      /* Dec A V4 / Dec B C2,C3 true */
  TEST_ASSERT(internal_numf("5)") == s_t_expect_five);     /* Dec B C2 false (')' < '0')  */
  TEST_ASSERT(internal_numf("5a") == s_t_expect_five);     /* Dec B C3 false ('a' > '9')  */
  TEST_ASSERT(internal_numf("-3.5") < -s_t_below_neg_3p4); /* sign + fraction arms        */
 ...
 * libs/ra8_reflow/src/ra8_reflow_svg_xform.c@internal_xform_read) -- the per-argument
...
RA8_INTERNAL static void internal_test_svgp_xform_list_mcdc(void)
{
  ...
  TEST_ASSERT(t.ue > s_t_parsed_nonzero); /* translate x parsed (space-separated args) */
  TEST_ASSERT(t.ua > s_t_parsed_nonzero); /* scale x parsed (comma-separated args)     */
 ...
 * libs/ra8_reflow/src/ra8_reflow_svg_shape.c@priv_ra8_svgp_draw_line) -- the point-list
 ...
 * libs/ra8_reflow/src/ra8_reflow_svg.c@internal_attr_at) -- the "attribute name must
 ...
 * libs/ra8_reflow/src/ra8_reflow_svg_doc.c@internal_elem_at) -- element-name
