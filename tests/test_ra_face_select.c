/**
 * @file test_ra_face_select.c
 * @brief #109 (items 2-3): per-run embedded `@font-face` selection in ra_reflow.
 *
 * @details
 ...
 * multi-face book is never serialized/loaded) and `ra_reflow_register_face`'s
 * validation + cap.
 *
...
static void tfs_init(void)
{
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_reflow_init((uint16_t)k_tfs_viewport_w,
...
static int32_t tfs_face_of(int32_t cp)
{
  for (uint32_t i = 0U; i < s_engine.glyph_count; ++i) {
    if (s_engine.glyphs[i].cp == cp) {
      return (int32_t)(((uint32_t)s_engine.glyphs[i].style >> (uint32_t)k_ra_reflow_face_shift) &
...
static void tfs_register_all_faces(void)
{
  for (uint16_t ci = 0U; ci < s_engine.css.face_count; ++ci) {
    TEST_ASSERT_EQ(
      k_ra_ok,
      ra_reflow_register_face(&s_engine, (uint8_t)ci, k_fixture_ahem, (size_t)k_fixture_ahem_len));
  }
}
 ...
 *
 * @par MC/DC:
 * Decision: the `priv_resolve_face_slot` guard (libs/ra_reflow/src/
 * ra_reflow_tokenize.c) `if (face_count==0 || (set & family)==0 ||
...
static void test_cache_bypass_with_faces(void)
{
  ...
  TEST_ASSERT_EQ(
    ...
    ra_reflow_cache_load(&s_engine, (const uint8_t*)k_chapter, strlen(k_chapter), s_blob, n));
...
/**
 * @test `ra_reflow_register_face` validates its inputs, rejects an invalid blob
...
static void test_register_validation(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_reflow_register_face(nullptr, 0U, k_fixture_ahem, (size_t)k_fixture_ahem_len));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_reflow_register_face(&s_engine, 0U, nullptr, (size_t)k_fixture_ahem_len));
  TEST_ASSERT_EQ(k_ra_err_invalid_size,
                 ra_reflow_register_face(&s_engine,
                                         0U,
                                         k_fixture_ahem,
                                         (size_t)k_ra_reflow_min_font_bytes - 1U));
