...
static int s_complete_last_ctx = 0;

static void reset_state(void)
{
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_mstp_init());
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_dma_init());
  s_complete_count    = 0;
...
static void test_init_marks_channels_free(void)
{
  ...
  for (uint8_t ch = 0U; ch < (uint8_t)k_ra_dma_channel_count; ++ch) {
    bool busy = true;
    TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_dma_channel_is_busy(ch, &busy));
    TEST_ASSERT(!busy);
  }
...
static void test_request_allocates_channel_zero(void)
{
  ...
  uint8_t ch = 0xFFU;
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_dma_request(&req, &ch));
  TEST_ASSERT_EQ((int)0, (int)ch);
...
static void test_request_rejects_bad_engine(void)
{
  ...
  uint8_t ch = 0U;
  TEST_ASSERT_EQ((int)k_ra_err_not_supported, (int)ra_dma_request(&req, &ch));
...
static void test_request_rejects_zero_count(void)
{
  ...
  uint8_t ch = 0U;
  TEST_ASSERT_EQ((int)k_ra_err_invalid_arg, (int)ra_dma_request(&req, &ch));
...
static void test_request_null_ptrs(void)
{
  ...
  uint8_t ch = 0U;
  TEST_ASSERT_EQ((int)k_ra_err_null_ptr, (int)ra_dma_request(nullptr, &ch));
  ...
  TEST_ASSERT_EQ((int)k_ra_err_null_ptr, (int)ra_dma_request(&req, nullptr));
...
static void test_release_round_trip(void)
{
  ...
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_dma_release(ch));
  ...
  TEST_ASSERT_EQ((int)k_ra_err_invalid_state, (int)ra_dma_release(ch));
...
static void test_release_bad_channel(void)
{
  ...
  reset_state();
  TEST_ASSERT_EQ((int)k_ra_err_invalid_arg, (int)ra_dma_release(99U));
...
static void test_channel_exhaustion(void)
{
  ...
  uint8_t ch = 0U;
  TEST_ASSERT_EQ((int)k_ra_err_no_mem, (int)ra_dma_request(&req, &ch));
...
static void test_sim_dma_memcpy_byte(void)
{
  ...
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_sim_dma_memcpy(ch));

  TEST_ASSERT_EQ((int)0x10, (int)dst[0]);
  TEST_ASSERT_EQ((int)0x60, (int)dst[5]);
...
static void test_sim_dma_memcpy_word(void)
{
  ...
  TEST_ASSERT_EQ((long long)0xDEADBEEFU, (long long)dst[0]);
  TEST_ASSERT_EQ((long long)0xCAFEBABEU, (long long)dst[1]);
  TEST_ASSERT_EQ((long long)0x12345678U, (long long)dst[2]);
...
static void test_sim_dma_complete_fires_callback(void)
{
  ...
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_sim_dma_complete(ch));
  TEST_ASSERT_EQ((int)1, (int)s_complete_count);
  TEST_ASSERT_EQ((int)0xABCD, (int)s_complete_last_ctx);
...
static void test_dma_request_without_init_fails(void)
{
  ...
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_dma_init());
  TEST_ASSERT_EQ((int)k_ra_ok, (int)ra_dma_deinit());
  ...
  uint8_t ch = 0U;
  TEST_ASSERT_EQ((int)k_ra_err_not_initialized, (int)ra_dma_request(&req, &ch));
...
static void test_channel_is_busy_bad_inputs(void)
{
  ...
  TEST_ASSERT_EQ((int)k_ra_err_null_ptr, (int)ra_dma_channel_is_busy(0U, nullptr));
  bool busy = false;
  TEST_ASSERT_EQ((int)k_ra_err_invalid_arg, (int)ra_dma_channel_is_busy(99U, &busy));
