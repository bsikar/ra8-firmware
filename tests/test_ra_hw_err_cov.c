/**
 * @file test_ra_hw_err_cov.c
 * @brief Coverage + behaviour tests for the bounded-wait primitives
 *        (`ra_hw_err.h`) and the host MMIO fault seam (`ra_sim_mmio.c`).
 *
 * @par Tag
 * [Ring 3 / HAL] {World: S}
 *
 * @details
 * `ra_hw_wait_flag_{set,clear}{8,32}` are the shared "spin until this status bit
 * changes" primitives every HAL poll loop delegates to. Under the host test
 * build (`RA_SIMULATOR_MODE` + `UNIT_TEST`) each waiter consults the programmable
 ...
 * Arming `ra_sim_mmio_fail_wait` forces a deterministic timeout regardless of
 * RAM; arming `ra_sim_mmio_satisfy_after(n)` makes the flag "assert" on the
 ...
 * `k_ra_err_hw_timeout` legs of all four waiters, plus every branch of the seam
...
typedef enum : uint8_t {
  k_test_overflow_regs = (uint8_t)k_ra_sim_mmio_max_faults + 1U,
} test_overflow_t;

/**
 * @test test_hw_err_null_ptr
 * @details All four waiters reject a NULL register with k_ra_err_null_ptr.
 */
static void test_hw_err_null_ptr(void)
{
  TEST_BEGIN("ra_hw_wait_flag_* reject NULL register");
  ra_sim_mmio_reset();
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_hw_wait_flag_set8(nullptr, (uint8_t)k_test_mask8, k_test_budget));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_hw_wait_flag_clear8(nullptr, (uint8_t)k_test_mask8, k_test_budget));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_hw_wait_flag_set32(nullptr, (uint32_t)k_test_mask32, k_test_budget));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_hw_wait_flag_clear32(nullptr, (uint32_t)k_test_mask32, k_test_budget));
  TEST_END("ra_hw_wait_flag_* reject NULL register");
}
 ...
 * @details `ra_sim_mmio_fail_wait` forces a timeout even when the flag is staged
 * as already satisfied -- proving the seam overrides the RAM poll.
 */
static void test_hw_err_fail_wait(void)
{
  TEST_BEGIN("ra_sim_mmio_fail_wait forces timeout regardless of RAM");
  ra_sim_mmio_reset();

  volatile uint32_t r32 = (uint32_t)k_test_mask32; /* staged satisfied... */
  TEST_ASSERT_EQ(k_ra_ok, ra_sim_mmio_fail_wait(&r32));
  TEST_ASSERT_EQ(k_ra_err_hw_timeout,
                 ra_hw_wait_flag_set32(&r32, (uint32_t)k_test_mask32, k_test_budget));

  volatile uint8_t c8 = 0U; /* clear-wait staged satisfied... */
  TEST_ASSERT_EQ(k_ra_ok, ra_sim_mmio_fail_wait(&c8));
  TEST_ASSERT_EQ(k_ra_err_hw_timeout,
                 ra_hw_wait_flag_clear8(&c8, (uint8_t)k_test_mask8, k_test_budget));
  TEST_END("ra_sim_mmio_fail_wait forces timeout regardless of RAM");
}

/**
 * @test test_hw_err_satisfy_after
 * @details `ra_sim_mmio_satisfy_after(n)` makes the flag assert on the n-th
...
{
  TEST_BEGIN("ra_sim_mmio_satisfy_after asserts the flag mid-wait");
  ra_sim_mmio_reset();

  volatile uint32_t r32 = 0U; /* RAM never sets it; the seam does, on poll n. */
  TEST_ASSERT_EQ(k_ra_ok, ra_sim_mmio_satisfy_after(&r32, k_test_satisfy_n));
  TEST_ASSERT_EQ(k_ra_ok, ra_hw_wait_flag_set32(&r32, (uint32_t)k_test_mask32, k_test_budget));

  volatile uint8_t r8 = 0U; /* n == 0 -> satisfied on the first poll. */
  TEST_ASSERT_EQ(k_ra_ok, ra_sim_mmio_satisfy_after(&r8, 0U));
  TEST_ASSERT_EQ(k_ra_ok, ra_hw_wait_flag_set8(&r8, (uint8_t)k_test_mask8, k_test_budget));
  TEST_END("ra_sim_mmio_satisfy_after asserts the flag mid-wait");
}

/**
 * @test test_sim_mmio_arm_errors
 * @details The arm entry points validate their inputs (NULL register) and the
 * bounded table (no_mem once ::k_ra_sim_mmio_max_faults distinct registers are
 * armed).
 */
static void test_sim_mmio_arm_errors(void)
{
  TEST_BEGIN("ra_sim_mmio arm functions validate inputs + table capacity");
  ra_sim_mmio_reset();

  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_sim_mmio_fail_wait(nullptr));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_sim_mmio_satisfy_after(nullptr, k_test_satisfy_n));
  ...
  for (uint8_t i = 0U; i < (uint8_t)k_ra_sim_mmio_max_faults; ++i) {
    TEST_ASSERT_EQ(k_ra_ok, ra_sim_mmio_fail_wait(&regs[i]));
  }
  TEST_ASSERT_EQ(k_ra_err_no_mem, ra_sim_mmio_fail_wait(&regs[(uint8_t)k_ra_sim_mmio_max_faults]));
  ra_sim_mmio_reset();
  TEST_END("ra_sim_mmio arm functions validate inputs + table capacity");
}

/**
 * @test test_sim_mmio_reset_and_update
 * @details `ra_sim_mmio_reset` disarms every register (transparent again), and
 * re-arming the same register updates it in place (fail_wait over a prior
 * satisfy_after wins).
 */
static void test_sim_mmio_reset_and_update(void)
{
  TEST_BEGIN("ra_sim_mmio reset restores transparency; re-arm updates in place");
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_sim_mmio_satisfy_after(&r32, 0U));
  TEST_ASSERT_EQ(k_ra_ok, ra_sim_mmio_fail_wait(&r32));
  TEST_ASSERT_EQ(k_ra_err_hw_timeout,
                 ra_hw_wait_flag_set32(&r32, (uint32_t)k_test_mask32, k_test_budget));
  ra_sim_mmio_reset();
  TEST_END("ra_sim_mmio reset restores transparency; re-arm updates in place");
}
...
int32_t main(void)
{
  ...
  test_sim_mmio_reset_and_update();
  test_sim_mmio_wait_eval_direct();
  (void)fprintf(stderr, "[OK ] test_ra_hw_err_cov.c\n");
  return 0;
}
