/**
 * @file test_ra_nsc_wdt.c
 * @brief Unit tests for the secure-side watchdog NSC veneers (ra_nsc_wdt.c).
 *
 * @details
 * ``ra_nsc_wdt_start`` and ``ra_nsc_wdt_refresh`` are the two argument-free
 ...
 * ``RA_NSC_VENEER`` compiles away, so both veneers reduce to a direct forward
 * into the ``ra_wdt`` driver over the simulated register block: the arm veneer
 * calls ``ra_wdt_init`` with the fixed Secure-side config, and the refresh
 * veneer calls ``ra_wdt_refresh_deferred`` (the WDTRR 0x00/0xFF unlock).
 *
 * The suite drives both from the host with the WDT register block mapped by
 * ``ra_sim_mmap_reset``: it proves the arm veneer configures and arms cleanly
 ...
 * @brief ``ra_nsc_wdt_start`` forwards the fixed Secure config into ``ra_wdt_init``
 *        and arms the counter (k_ra_ok).
 *
 * @par MC/DC:
 * (no compound decisions under test -- the veneer is an argument-free forward
 * into ra_wdt_init; the single outcome asserted is its k_ra_ok return.)
 */
static void test_nsc_wdt_start_arms(void)
{
  TEST_BEGIN("ra_nsc_wdt_start arms the secure WDT");
  ra_sim_mmap_reset();
   ...
   * k_ra_ok. */
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_wdt_start());
  TEST_END("ra_nsc_wdt_start arms the secure WDT");
}

/**
 * @test test_nsc_wdt_refresh_kicks
 * @brief ``ra_nsc_wdt_refresh`` issues the WDTRR heartbeat (final byte 0xFF).
 *
 * @par MC/DC:
 * (no compound decisions under test -- the veneer is an argument-free forward
 * into ra_wdt_refresh_deferred; the assertion is over the WDTRR unlock byte.)
 */
static void test_nsc_wdt_refresh_kicks(void)
{
  TEST_BEGIN("ra_nsc_wdt_refresh writes the WDTRR unlock byte");
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_wdt_start());
  ...
  TEST_END("ra_nsc_wdt_refresh writes the WDTRR unlock byte");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_nsc_wdt.c\n");
  return 0;
}
