/**
 * @file test_ra_jpeg_sw_decode_cov.c
 * @brief Black-box line-coverage supplement for ra_jpeg_sw_decode.c.
 *
 * @par Tag
 * [Ring 3 / HAL] {World: NS}
 *
 * @details
 * The round-trip tests in test_ra_jpeg_sw.c drive the decoder with
 ...
 * ra_jpeg_sw_decode.c uncovered.  This file drives those paths
 * directly through the public ra_jpeg_sw_decode() entry point using
 * hand-built JPEG byte streams.  Because it calls the real
 * (un-renamed) production symbol linked from ra_core_hal, gcovr
 * merges the covered lines into the aggregate .gcda for
 * ra_jpeg_sw_decode.c (a renamed white-box #include copy would not
 * union into that object).
 *
 * The entropy fixtures rely on the canonical-Huffman builder in
 * ra_jpeg_sw.c: a BITS list with a single length-1 code assigns
 ...
 * total == 0, which makes ra_jpeg_sw_htab_decode() return -1 the
...
/**
 * @brief Drive the three top-level guard returns of ra_jpeg_sw_decode().
 *
 * @details
...
static void test_decode_top_level_guards(void)
{
  ...
  uint16_t             w           = 0U;
  uint16_t             h           = 0U;
  ra_err_t             e           = ra_jpeg_sw_decode(too_short,
...
static void test_decode_dqt_guards(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_err_protocol_error,
    ra_jpeg_sw_decode(dqt_hdr, (uint32_t)sizeof dqt_hdr, s_out, (uint32_t)k_cov_out_big, &w, &h));

  static const uint8_t dqt_body[] = {0xFFU, 0xD8U, 0xFFU, 0xDBU, 0x00U, 0x03U, 0x00U};
  TEST_ASSERT_EQ(
    k_ra_err_protocol_error,
    ra_jpeg_sw_decode(dqt_body, (uint32_t)sizeof dqt_body, s_out, (uint32_t)k_cov_out_big, &w, &h));
...
static void test_decode_dht_guards(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_err_protocol_error,
    ra_jpeg_sw_decode(dht_hdr, (uint32_t)sizeof dht_hdr, s_out, (uint32_t)k_cov_out_big, &w, &h));

  static const uint8_t dht_bits[] = {0xFFU, 0xD8U, 0xFFU, 0xC4U, 0x00U, 0x03U, 0x00U};
  TEST_ASSERT_EQ(
    k_ra_err_protocol_error,
    ra_jpeg_sw_decode(dht_bits, (uint32_t)sizeof dht_bits, s_out, (uint32_t)k_cov_out_big, &w, &h));
  ...
  TEST_ASSERT_EQ(k_ra_err_protocol_error,
                 ra_jpeg_sw_decode(dht_total,
  ...
  TEST_ASSERT_EQ(
    k_ra_err_protocol_error,
    ra_jpeg_sw_decode(dht_vals, (uint32_t)sizeof dht_vals, s_out, (uint32_t)k_cov_out_big, &w, &h));
 ...
 * dedicated MC/DC coverage in test_ra_jpeg_sw.c.
 * @since 0.1.0
 */
static void test_decode_sof0_guards(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_err_protocol_error,
    ra_jpeg_sw_decode(sof0_hdr, (uint32_t)sizeof sof0_hdr, s_out, (uint32_t)k_cov_out_big, &w, &h));

  static const uint8_t sof0_len[] = {0xFFU, 0xD8U, 0xFFU, 0xC0U, 0x00U, 0x05U};
  TEST_ASSERT_EQ(
    k_ra_err_protocol_error,
    ra_jpeg_sw_decode(sof0_len, (uint32_t)sizeof sof0_len, s_out, (uint32_t)k_cov_out_big, &w, &h));
 ...
 * decision keeps its independence pairs in test_ra_jpeg_sw.c; this
...
static void test_decode_sos_guards(void)
{
  ...
  uint32_t n = 0U;
  cov_append(buf, &n, k_soi, (uint32_t)sizeof k_soi);
  cov_append(buf, &n, k_sof0_8x8_gray, (uint32_t)sizeof k_sof0_8x8_gray);
  static const uint8_t sos_marker[] = {0xFFU, 0xDAU};
  cov_append(buf, &n, sos_marker, (uint32_t)sizeof sos_marker);
  TEST_ASSERT_EQ(k_ra_err_protocol_error,
                 ra_jpeg_sw_decode(buf, n, s_out, (uint32_t)k_cov_out_big, &w, &h));
  ...
  cov_append(buf, &n, sos_short, (uint32_t)sizeof sos_short);
  TEST_ASSERT_EQ(k_ra_err_protocol_error,
                 ra_jpeg_sw_decode(buf, n, s_out, (uint32_t)k_cov_out_big, &w, &h));
  ...
  cov_append(buf, &n, sos_ns2, (uint32_t)sizeof sos_ns2);
  TEST_ASSERT_EQ(k_ra_err_not_supported,
                 ra_jpeg_sw_decode(buf, n, s_out, (uint32_t)k_cov_out_big, &w, &h));
...
static void test_decode_scan_buffer_too_small(void)
{
  ...
  static uint8_t buf[64];
  static uint8_t tiny[(uint32_t)k_cov_out_small];
  uint16_t       w = 0U;
  uint16_t       h = 0U;
  ...
  cov_append(buf, &n, k_soi, (uint32_t)sizeof k_soi);
  cov_append(buf, &n, k_sof0_8x8_gray, (uint32_t)sizeof k_sof0_8x8_gray);
  cov_append(buf, &n, k_sos_gray, (uint32_t)sizeof k_sos_gray);
  static const uint8_t entropy[] = {0x00U, 0x00U};
  cov_append(buf, &n, entropy, (uint32_t)sizeof entropy);
  cov_append(buf, &n, k_eoi, (uint32_t)sizeof k_eoi);

  TEST_ASSERT_EQ(k_ra_err_invalid_size,
                 ra_jpeg_sw_decode(buf, n, tiny, (uint32_t)k_cov_out_small, &w, &h));
 ...
 * ra_jpeg_sw_htab_decode() returns -1, tripping `if (t < 0)` in
 ...
 * The `if (e != k_ra_ok)` propagation checks in the y-block and scan
 ...
 * The following ra_jpeg_sw_br_get_bits(br, 8) then underflows (only 4
...
static void test_decode_dc_magnitude_underflow(void)
{
  ...
  cov_append(buf, &n, k_soi, (uint32_t)sizeof k_soi);
  cov_append(buf, &n, k_sof0_8x8_gray, (uint32_t)sizeof k_sof0_8x8_gray);
  cov_append(buf, &n, k_dht_dc_t8, (uint32_t)sizeof k_dht_dc_t8);
  cov_append(buf, &n, k_sos_gray, (uint32_t)sizeof k_sos_gray);
 ...
 * AC table, whose ra_jpeg_sw_htab_decode() returns -1, tripping
...
static void test_decode_ac_huffman_failure(void)
{
  ...
  cov_append(buf, &n, k_soi, (uint32_t)sizeof k_soi);
  cov_append(buf, &n, k_sof0_8x8_gray, (uint32_t)sizeof k_sof0_8x8_gray);
  cov_append(buf, &n, k_dht_dc, (uint32_t)sizeof k_dht_dc);
  cov_append(buf, &n, k_sos_gray, (uint32_t)sizeof k_sos_gray);
  static const uint8_t entropy[] = {0x00U, 0x00U};
  cov_append(buf, &n, entropy, (uint32_t)sizeof entropy);
  cov_append(buf, &n, k_eoi, (uint32_t)sizeof k_eoi);
 ...
 * ra_jpeg_sw_br_get_bits(br, 15) underflows, tripping `if (v < 0)`.
 *
 * @par MC/DC:
...
static void test_decode_ac_magnitude_underflow(void)
{
  ...
  cov_append(buf, &n, k_sof0_8x8_gray, (uint32_t)sizeof k_sof0_8x8_gray);
  cov_append(buf, &n, k_dht_dc, (uint32_t)sizeof k_dht_dc);
  cov_append(buf, &n, k_dht_ac_f0, (uint32_t)sizeof k_dht_ac_f0);
  cov_append(buf, &n, k_sos_gray, (uint32_t)sizeof k_sos_gray);
  static const uint8_t entropy[] = {0x00U};
  cov_append(buf, &n, entropy, (uint32_t)sizeof entropy);
  cov_append(buf, &n, k_eoi, (uint32_t)sizeof k_eoi);
...
static void test_decode_ac_zrl_index_overrun(void)
{
  ...
  cov_append(buf, &n, k_sof0_8x8_gray, (uint32_t)sizeof k_sof0_8x8_gray);
  cov_append(buf, &n, k_dht_dc, (uint32_t)sizeof k_dht_dc);
  cov_append(buf, &n, k_dht_ac_zrl, (uint32_t)sizeof k_dht_ac_zrl);
  cov_append(buf, &n, k_sos_gray, (uint32_t)sizeof k_sos_gray);
 ...
 * block fails immediately (`if (e != k_ra_ok)` after dec_block(ci=1)),
 ...
 * second `if (e != k_ra_ok)` after dec_block(ci=2) fires.
 *
 * @par MC/DC:
 * Both propagation decisions are single-condition:
 * - Cb `if (e != k_ra_ok)`: V_T this test's first fixture; V_F the
 *   round-trip's successful Cb block.
 * - Cr `if (e != k_ra_ok)`: V_T this test's second fixture; V_F the
 *   round-trip's successful Cr block.
 * - dec_decode_scan chroma `if (e != k_ra_ok)`: V_T (either fixture);
...
static void test_decode_chroma_block_failures(void)
{
  ...
  uint32_t n = 0U;
  cov_append(buf, &n, k_soi, (uint32_t)sizeof k_soi);
  cov_append(buf, &n, k_sof0_444, (uint32_t)sizeof k_sof0_444);
  cov_append(buf, &n, k_dht_dc, (uint32_t)sizeof k_dht_dc);
  cov_append(buf, &n, k_dht_ac, (uint32_t)sizeof k_dht_ac);
  cov_append(buf, &n, k_sos_444_cb_fail, (uint32_t)sizeof k_sos_444_cb_fail);
  cov_append(buf, &n, entropy, (uint32_t)sizeof entropy);
  cov_append(buf, &n, k_eoi, (uint32_t)sizeof k_eoi);
  TEST_ASSERT_EQ(k_ra_err_protocol_error,
                 ra_jpeg_sw_decode(buf, n, s_out, (uint32_t)k_cov_out_big, &w, &h));
  ...
  cov_append(buf, &n, k_dht_dc, (uint32_t)sizeof k_dht_dc);
  cov_append(buf, &n, k_dht_ac, (uint32_t)sizeof k_dht_ac);
  cov_append(buf, &n, k_sos_444_cr_fail, (uint32_t)sizeof k_sos_444_cr_fail);
  cov_append(buf, &n, entropy, (uint32_t)sizeof entropy);
  cov_append(buf, &n, k_eoi, (uint32_t)sizeof k_eoi);
  TEST_ASSERT_EQ(k_ra_err_protocol_error,
                 ra_jpeg_sw_decode(buf, n, s_out, (uint32_t)k_cov_out_big, &w, &h));
...
static void test_decode_grayscale_edge_success(void)
{
  ...
  uint32_t n = 0U;
  cov_append(buf, &n, k_soi, (uint32_t)sizeof k_soi);
  cov_append(buf, &n, k_sof0_5x5_gray, (uint32_t)sizeof k_sof0_5x5_gray);
  cov_append(buf, &n, k_dht_dc, (uint32_t)sizeof k_dht_dc);
  cov_append(buf, &n, k_dht_ac, (uint32_t)sizeof k_dht_ac);
  cov_append(buf, &n, k_sos_gray, (uint32_t)sizeof k_sos_gray);
  static const uint8_t entropy[] = {0x00U, 0x00U};
 ...
 *
 * @details
 * Runs every ra_jpeg_sw_decode.c coverage fixture.  ra_sim_mmap_reset()
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_jpeg_sw_decode_cov.c\n");
  return 0;
}
