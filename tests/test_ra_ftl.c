 ...
 */
static ra_err_t fake_read(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf)
{
 ...
 */
static ra_err_t fake_write(void* ctx, uint32_t lba, uint32_t count, const uint8_t* buf)
{
  test_fake_state_t* st = (test_fake_state_t*)ctx;
  if (lba + count > (uint32_t)k_test_ftl_phys) {
    return k_ra_err_out_of_range;
 ...
 */
static ra_err_t fake_erase(void* ctx, uint32_t lba, uint32_t count)
{
  test_fake_state_t* st = (test_fake_state_t*)ctx;
  if (lba + count > (uint32_t)k_test_ftl_phys) {
    return k_ra_err_out_of_range;
 ...
 */
static ra_err_t fake_get_caps(const void* ctx, ra_io_blockdev_caps_t* out)
{
  (void)ctx;
...
static const ra_io_blockdev_iface_t k_fake_iface = {
  .read     = fake_read,
  .write    = fake_write,
  .erase    = fake_erase,
 ...
 */
static void fake_bind(ra_io_blockdev_t* bd)
{
...
static void test_fake_rejects_nonblank(void)
{
  ...
  ra_io_blockdev_t fake = {};
  fake_bind(&fake);
  uint8_t blk[(size_t)k_test_ftl_block];
  (void)memset(blk, 0xAB, sizeof(blk));
  /* First write to a blank sector succeeds. */
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_write(&fake, 0U, 1U, blk));
  /* Second write to the now non-blank sector is rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_io_blockdev_write(&fake, 0U, 1U, blk));
  /* After an erase the sector is writable again. */
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_erase(&fake, 0U, 1U));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_write(&fake, 0U, 1U, blk));
...
static void test_ftl_init_validation(void)
{
  ...
  ra_ftl_pblock_t pb[(size_t)k_test_ftl_phys] = {};
  uint8_t         scratch[(size_t)k_test_ftl_block];

  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_ftl_init(nullptr, &fake, map, k_test_ftl_logical, pb, k_test_ftl_phys, scratch));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_ftl_init(&ftl, nullptr, map, k_test_ftl_logical, pb, k_test_ftl_phys, scratch));
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_ftl_init(&ftl, &fake, nullptr, k_test_ftl_logical, pb, k_test_ftl_phys, scratch));
  TEST_ASSERT_EQ(k_ra_err_invalid_size,
                 ra_ftl_init(&ftl, &fake, map, 0U, pb, k_test_ftl_phys, scratch));
  /* No spare block (logical == physical) is rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_ftl_init(&ftl, &fake, map, k_test_ftl_phys, pb, k_test_ftl_phys, scratch));
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_ftl_init(&ftl, &fake, map, k_test_ftl_logical, pb, k_test_ftl_phys, scratch));
...
static void test_ftl_unwritten_reads_erase_value(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_read(&bd, 3U, 1U, got));
  bool all_ff = true;
...
static void test_ftl_soak_integrity_wear(void)
{
  ...
  for (uint32_t op = 0; op < (uint32_t)k_test_ftl_ops; ++op) {
    ...
    if (do_write) {
      ...
      TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_write(&bd, lbn, 1U, blk));
      shadow_tag[lbn] = tag;
      ++writes;
      continue;
    }
    ...
    uint8_t got[(size_t)k_test_ftl_block];
    TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_read(&bd, lbn, 1U, got));
  ...
  uint32_t hi = 0;
  uint32_t lo = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_ftl_wear_stats(&ftl, &hi, &lo));
  TEST_ASSERT(hi >= lo);
  /*
...
static void test_ftl_presented_erase_unmaps(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_write(&bd, 5U, 1U, blk));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_erase(&bd, 5U, 1U));

  uint8_t got[(size_t)k_test_ftl_block];
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_read(&bd, 5U, 1U, got));
  bool all_ff = true;
  for (size_t i = 0; i < sizeof(got); ++i) {
    if (got[i] != (uint8_t)k_test_ftl_erase_byte) {
      all_ff = false;
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_ftl.c\n");
  return 0;
}
