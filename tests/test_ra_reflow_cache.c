/**
 * @file test_ra_reflow_cache.c
 * @brief Host unit tests for the import-time pagination cache
 *        (libs/ra_reflow/src/ra_reflow_cache.c, #79).
 *
 * @details
 * Proves the serialise -> load round-trip restores an identical layout
 * (so `ra_reflow_layout_chapter()` can be skipped), and that every
...
static ra_reflow_glyph_t s_glyph_save[k_t_glyphs];
static ra_reflow_page_t  s_page_save[k_t_pages];

static const size_t s_content_len = sizeof(s_content);

/** @brief Initialise the shared engine at a given body font size. */
static ra_err_t init_engine(ra_reflow_t* eng, uint16_t font_px)
{
  return ra_reflow_init((uint16_t)k_t_vp_w,
                        (uint16_t)k_t_vp_h,
                        s_font,
                        (size_t)k_t_font_len,
                        font_px,
...
static void build_layout(ra_reflow_t* eng)
{
...
static size_t serialize_ok(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_reflow_cache_serialize(&s_engine,
                                           s_content,
                                           s_content_len,
                                           s_blob,
                                           (size_t)k_t_blob_cap,
 ...
 * @brief `ra_reflow_cache_size()` equals the serialised byte count and
...
static void test_size_matches_serialize(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, init_engine(&s_engine, (uint16_t)k_t_font_px));
  build_layout(&s_engine);

  size_t want = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_reflow_cache_size(&s_engine, &want));
  const size_t formula = (size_t)k_ra_reflow_cache_header_bytes +
                         ((size_t)k_t_glyphs * (size_t)k_ra_reflow_cache_glyph_bytes) +
                         ((size_t)k_t_pages * (size_t)k_ra_reflow_cache_page_bytes);
...
static void test_roundtrip_restores_layout(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_reflow_cache_load(&s_engine, s_content, s_content_len, s_blob, n));
...
static void test_empty_layout_roundtrip(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, init_engine(&s_engine, (uint16_t)k_t_font_px));
  const size_t n = serialize_ok();
  TEST_ASSERT_EQ(k_ra_reflow_cache_header_bytes, n);
  TEST_ASSERT_EQ(k_ra_ok, ra_reflow_cache_load(&s_engine, s_content, s_content_len, s_blob, n));
...
static void test_stale_on_font_change(void)
{
  ...
  build_layout(&s_engine);
  const size_t n = serialize_ok();
  TEST_ASSERT_EQ(k_ra_ok, init_engine(&s_engine, (uint16_t)k_t_font_px2));
  TEST_ASSERT_EQ(k_ra_err_invalid_state,
                 ra_reflow_cache_load(&s_engine, s_content, s_content_len, s_blob, n));
...
static void test_stale_on_content_change(void)
{
  ...
  const size_t n = serialize_ok();
  TEST_ASSERT_EQ(k_ra_err_invalid_state,
                 ra_reflow_cache_load(&s_engine, other, sizeof(other), s_blob, n));
...
static void test_corrupt_body_checksum(void)
{
  ...
  build_layout(&s_engine);
  const size_t n = serialize_ok();
  s_blob[(size_t)k_ra_reflow_cache_header_bytes + (size_t)k_t_glyphs] ^= (uint8_t)k_t_flip;
...
static void test_truncated_blob(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_size,
                 ra_reflow_cache_load(&s_engine,
                                      s_content,
                                      s_content_len,
                                      s_blob,
                                      (size_t)k_ra_reflow_cache_header_bytes - 1U));
  TEST_ASSERT_EQ(k_ra_err_invalid_size,
                 ra_reflow_cache_load(&s_engine, s_content, s_content_len, s_blob, n - 1U));
...
static void test_cap_too_small(void)
{
  ...
  build_layout(&s_engine);
  size_t need = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_reflow_cache_size(&s_engine, &need));
  size_t n = 0U;
  TEST_ASSERT_EQ(
    k_ra_err_invalid_size,
    ra_reflow_cache_serialize(&s_engine, s_content, s_content_len, s_blob, need - 1U, &n));
 ...
 *
 * @par MC/DC:
 * Decision in libs/ra_reflow/src/ra_reflow_cache.c@ra_reflow_cache_size:
...
{
  TEST_BEGIN("reflow_cache: ra_reflow_cache_size MC/DC");
  TEST_ASSERT_EQ(k_ra_ok, init_engine(&s_engine, (uint16_t)k_t_font_px));
  build_layout(&s_engine);
  size_t sz = 0U;
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_reflow_cache_size(nullptr, &sz));       /* C1=T      */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_reflow_cache_size(&s_engine, nullptr)); /* C2=T      */
  TEST_ASSERT_EQ(k_ra_ok, ra_reflow_cache_size(&s_engine, &sz));               /* C1=F,C2=F */
  /* in_use==0 simple guard (not compound). */
  ra_reflow_t cold = {};
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_reflow_cache_size(&cold, &sz));
  TEST_END("reflow_cache: ra_reflow_cache_size MC/DC");
}
 ...
 *
 * @par MC/DC:
 * Decisions in libs/ra_reflow/src/ra_reflow_cache.c@ra_reflow_cache_serialize:
...
{
  TEST_BEGIN("reflow_cache: ra_reflow_cache_serialize MC/DC");
  TEST_ASSERT_EQ(k_ra_ok, init_engine(&s_engine, (uint16_t)k_t_font_px));
  build_layout(&s_engine);
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_reflow_cache_serialize(nullptr, s_content, s_content_len, s_blob, cap, &n));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_reflow_cache_serialize(&s_engine, s_content, s_content_len, nullptr, cap, &n));
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_reflow_cache_serialize(&s_engine, s_content, s_content_len, s_blob, cap, nullptr));
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_reflow_cache_serialize(&s_engine, s_content, s_content_len, s_blob, cap, &n));
  /* D2 (A=F is the D1 baseline above). */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_reflow_cache_serialize(&s_engine, nullptr, s_content_len, s_blob, cap, &n));
  TEST_ASSERT_EQ(k_ra_ok, ra_reflow_cache_serialize(&s_engine, nullptr, 0U, s_blob, cap, &n));
  TEST_END("reflow_cache: ra_reflow_cache_serialize MC/DC");
}
 ...
 *
 * @par MC/DC:
 * Decisions in libs/ra_reflow/src/ra_reflow_cache.c@ra_reflow_cache_load:
...
{
  TEST_BEGIN("reflow_cache: ra_reflow_cache_load MC/DC");
  TEST_ASSERT_EQ(k_ra_ok, init_engine(&s_engine, (uint16_t)k_t_font_px));
  build_layout(&s_engine);
  const size_t n = serialize_ok();
  /* D1 */
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_reflow_cache_load(nullptr, s_content, s_content_len, s_blob, n));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_reflow_cache_load(&s_engine, s_content, s_content_len, nullptr, n));
  TEST_ASSERT_EQ(k_ra_ok, ra_reflow_cache_load(&s_engine, s_content, s_content_len, s_blob, n));
  /* D2 (A=F is the D1 baseline above). */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_reflow_cache_load(&s_engine, nullptr, s_content_len, s_blob, n));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_reflow_cache_load(&s_engine, nullptr, 0U, s_blob, n));
  /* in_use==0 simple guard (not compound). */
  ra_reflow_t cold = {};
  TEST_ASSERT_EQ(k_ra_err_not_initialized,
                 ra_reflow_cache_load(&cold, s_content, s_content_len, s_blob, n));
  TEST_END("reflow_cache: ra_reflow_cache_load MC/DC");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_reflow_cache.c\n");
  return 0;
}
