/**
 * @file test_ra_usb_host_bulk_cov.c
 * @brief Coverage unit tests for the USB host-mode bulk engine
 *        (libs/ra_hal/src/ra_usb_host_bulk.c).
 *
 * @details
 * Drives the polled host-mode bulk-transfer engine through its public
 * entry points (``ra_usb_host_set_target``, ``ra_usb_host_pipe_setup``,
 * ``ra_usb_host_bulk_out`` / ``ra_usb_host_bulk_in``, and
 * ``ra_usb_host_line_state``) against the RAM-backed peripheral window
 * installed by ``ra_sim_mmap``. Because the host simulator models the
...
typedef enum : uint16_t {
  ...
  k_thb_pipe_hi     = 99U,   /**< Rejected: pipe > k_ra_usb_max_pipe_num. */
  k_thb_dev_addr    = 5U,    /**< Valid device address slot.              */
  k_thb_dev_addr_hi = 200U,  /**< Rejected: addr > k_ra_usb_dev_addr_max. */
  k_thb_ep          = 1U,    /**< Valid endpoint number (1..15).          */
  k_thb_ep_zero     = 0U,    /**< Rejected: endpoint 0.                   */
  k_thb_ep_hi       = 99U,   /**< Rejected: ep > k_ra_pipecfg_epnum_mask. */
  k_thb_mps         = 64U,   /**< Common FS bulk max packet size.         */
  k_thb_mps_zero    = 0U,    /**< Rejected: max packet 0.                 */
  k_thb_mps_hi      = 9999U, /**< Rejected: mps > k_ra_usb_pipemaxp_mxps. */
  ...
  k_thb_len_too_big = 2000U, /**< Exceeds k_ra_usb_pipe_max_packet.       */
 ...
 *
 * @par MC/DC:
 * (no compound decisions in this test -- ``ra_usb_host_set_target``
...
{
  TEST_BEGIN("ra_usb_host_set_target rejects bad speed / addr, applies DEVSEL");
  prep();

  /* Bogus speed -> internal_pick returns nullptr. */
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_usb_host_set_target((ra_usb_speed_t)k_thb_speed_bogus, (uint8_t)k_thb_dev_addr));
  /* Device address past the DEVADDn range. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_usb_host_set_target(k_ra_usb_speed_fs, (uint8_t)k_thb_dev_addr_hi));
...
{
  TEST_BEGIN("ra_usb_host_pipe_setup validates every argument field");
  prep();

  /* Bogus speed -> internal_pick returns nullptr. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_usb_host_pipe_setup((ra_usb_speed_t)k_thb_speed_bogus,
                                        (uint8_t)k_thb_pipe,
  ...
  /* pipe_num == 0. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_usb_host_pipe_setup(k_ra_usb_speed_fs,
  ...
  TEST_END("ra_usb_host_pipe_setup validates every argument field");
}
...
{
  TEST_BEGIN("ra_usb_host_pipe_setup configures a bulk pipe and clears status");
  prep();
 ...
 *
 * @par MC/DC:
 * (no compound decisions in this test -- ``ra_usb_host_bulk_out`` guards
...
{
  TEST_BEGIN("ra_usb_host_bulk_out rejects bad pointer / speed / pipe / length");
  prep();
  ...
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_usb_host_bulk_out(k_ra_usb_speed_fs, (uint8_t)k_thb_pipe, nullptr, (uint16_t)k_thb_len_ok));
  /* Bogus speed. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_usb_host_bulk_out((ra_usb_speed_t)k_thb_speed_bogus,
                                      (uint8_t)k_thb_pipe,
                                      data,
                                      (uint16_t)k_thb_len_ok));
  /* pipe_num == 0. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_usb_host_bulk_out(k_ra_usb_speed_fs,
                                      (uint8_t)k_thb_pipe_zero,
                                      data,
                                      (uint16_t)k_thb_len_ok));
  /* pipe_num > max. */
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_usb_host_bulk_out(k_ra_usb_speed_fs, (uint8_t)k_thb_pipe_hi, data, (uint16_t)k_thb_len_ok));
  /* Valid guards but ra_usb_queue_in rejects the oversize length: this
   * reaches the ``qerr != k_ra_ok`` early return after BEMP is armed. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_usb_host_bulk_out(k_ra_usb_speed_fs,
                                      (uint8_t)k_thb_pipe,
                                      data,
  ...
  TEST_END("ra_usb_host_bulk_out rejects bad pointer / speed / pipe / length");
}
...
{
  TEST_BEGIN("ra_usb_host_bulk_out arms BEMP then times out with no BEMP event");
  prep();
  ...
  TEST_ASSERT_EQ(
    k_ra_err_hw_timeout,
    ra_usb_host_bulk_out(k_ra_usb_speed_fs, (uint8_t)k_thb_pipe, data, (uint16_t)k_thb_len_ok));
  ...
  TEST_END("ra_usb_host_bulk_out arms BEMP then times out with no BEMP event");
}
 ...
 *
 * @par MC/DC:
 * (no compound decisions in this test -- every ``ra_usb_host_bulk_in``
...
{
  TEST_BEGIN("ra_usb_host_bulk_in rejects bad pointers / speed / pipe / unconfigured pipe");
  prep();
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_usb_host_bulk_in(k_ra_usb_speed_fs,
                                     (uint8_t)k_thb_pipe,
                                     nullptr,
                                     (uint16_t)k_thb_mps,
                                     &got));
  ...
  TEST_ASSERT_EQ(
    k_ra_err_null_ptr,
    ra_usb_host_bulk_in(k_ra_usb_speed_fs, (uint8_t)k_thb_pipe, buf, (uint16_t)k_thb_mps, nullptr));
  /* Bogus speed. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_usb_host_bulk_in((ra_usb_speed_t)k_thb_speed_bogus,
                                     (uint8_t)k_thb_pipe,
                                     buf,
                                     (uint16_t)k_thb_mps,
                                     &got));
  ...
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_usb_host_bulk_in(k_ra_usb_speed_fs, (uint8_t)k_thb_pipe_hi, buf, (uint16_t)k_thb_mps, &got));
  ...
  TEST_END("ra_usb_host_bulk_in rejects bad pointers / speed / pipe / unconfigured pipe");
}
...
{
  TEST_BEGIN("ra_usb_host_bulk_in consumes one short packet and ends the transfer");
  prep();
...
{
  TEST_BEGIN("ra_usb_host_bulk_in clamps a packet larger than the destination");
  prep();
...
{
  TEST_BEGIN("ra_usb_host_bulk_in releases a zero-length packet and ends");
  prep();
  ...
  TEST_END("ra_usb_host_bulk_in releases a zero-length packet and ends");
}
...
{
  TEST_BEGIN("ra_usb_host_bulk_in times out when no packet ever arrives");
  prep();
  ...
  TEST_END("ra_usb_host_bulk_in times out when no packet ever arrives");
}
...
{
  TEST_BEGIN("ra_usb_host_bulk_in ends when the destination buffer fills");
  prep();
 ...
 *
 * @par MC/DC:
 * (no compound decisions in this test -- ``ra_usb_host_line_state`` has a
 * single nullptr guard and a pure read; no ``&&`` or ``||`` in the code
 * under test that this case touches)
 */
static void test_line_state(void)
{
  TEST_BEGIN("ra_usb_host_line_state returns 0 for bad speed, LNST otherwise");
  prep();

  /* Bogus speed -> 0. */
  TEST_ASSERT_EQ(0U, ra_usb_host_line_state((ra_usb_speed_t)k_thb_speed_bogus));
