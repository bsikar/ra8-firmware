 ...
 * (2 conditions, libs/ra_reflow/src/ra_reflow_layout.c line 535). N+1=3.
...
static void test_mcdc_set_font_size_range_or(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_reflow_set_font_size(&s_engine, (uint16_t)k_mcdc_font_px_ok));
  /* V2 too low. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_reflow_set_font_size(&s_engine, (uint16_t)k_mcdc_font_px_too_low));
  /* V3 too high. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_reflow_set_font_size(&s_engine, (uint16_t)k_mcdc_font_px_too_high));
  TEST_ASSERT_EQ(k_ra_ok, ra_reflow_close(&s_engine));
 ...
 *   the public API: ra_reflow_layout_chapter sets buf and len atomically
 *   from the caller, and ra_reflow_init zeroes both. Argued by code
...
static void test_mcdc_set_font_size_state_or(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_state,
                 ra_reflow_set_font_size(&s_engine, (uint16_t)k_mcdc_font_px_ok));
 ...
 * (2 conditions, libs/ra_reflow/src/ra_reflow_layout.c line 510).
 * Reachable only via ra_reflow_run_layout (called from
 * ra_reflow_layout_chapter). The decision drives whether to synthesise
...
static void test_mcdc_run_layout_empty_guard(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_reflow_layout_chapter(&s_engine, (const uint8_t*)k_empty_p, strlen(k_empty_p), &pages));
...
int main(void)
{
  if (!priv_load_font()) {
    ...
    (void)fprintf(stderr,
                  "[SKIP] test_ra_reflow: Literata-Regular.ttf not loadable -- "
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_reflow: all MC/DC + 8 tests passed\n");
  return 0;
}
