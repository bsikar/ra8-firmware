/**
 * @file test_ra_epub_chapter_cov.c
 * @brief Coverage-boost tests for libs/ra_epub/src/ra_epub_chapter.c.
 *
 * @details
 * Exercises four groups of previously-uncovered code paths:
 *
 *   1. ra_epub_manifest_count() -- null-book early return (line 482).
 *   2. ra_epub_manifest_item()  -- null / not-initialised guard (line 493)
 ...
 *      line 267) and the ra_epub_render_glyph() call site at line 578.
...
typedef enum : uint32_t {
  k_cov_font_min_bytes = 16U,    /**< Minimum bytes ra_epub_set_font() accepts.       */
...
/**
 * @var s_render_px
 * @brief Font pixel height passed to ra_epub_render_glyph() in tests.
 ...
 *          (.../tests/test_ra_epub_chapter_cov.c).  Two successive calls to
...
static void priv_resolve_fw_root(char* out, size_t cap)
{
  ...
  priv_dirname_inplace(out); /* remove test_ra_epub_chapter_cov.c */
...
/* ---------------------------------------------------------------------------
 * Tests: ra_epub_manifest_count null-book guard (line 482).
 * ---------------------------------------------------------------------------
 */

/**
 * @test test_manifest_count_null_book
 * @brief ra_epub_manifest_count(nullptr) returns 0 -- line 482.
 *
 * @details The existing suite calls ra_epub_manifest_count() only on
...
/* ---------------------------------------------------------------------------
 * Tests: ra_epub_manifest_item null / not-ready / range guards (lines 493,496).
 * ---------------------------------------------------------------------------
 */

/**
 * @test test_manifest_item_null_and_not_ready
 * @brief ra_epub_manifest_item() null + not-ready guard -- line 493.
 *
...
static void test_manifest_item_null_and_not_ready(void)
{
  ...
  ra_epub_book_t book = {};
  /* V1: null pointer. */
  TEST_ASSERT_NULL(ra_epub_manifest_item(nullptr, 0U));
  /* V2: non-null book with in_use == 0 (not initialised). */
  TEST_ASSERT_NULL(ra_epub_manifest_item(&book, 0U));
  TEST_END("epub_manifest_item: null + not-ready -> nullptr");
}

/**
 * @test test_manifest_item_out_of_range
 * @brief ra_epub_manifest_item() out-of-range index returns nullptr -- line 496.
 *
...
static void test_render_glyph_stbtt_init_fail(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_epub_set_font(&book, s_fake_ttf_sig, sizeof(s_fake_ttf_sig)));
 ...
 *          k_ra_err_no_mem at line 271.  Entering priv_render_into at line
...
static void test_render_glyph_no_mem(void)
{
  ...
  uint32_t w          = 0U;
  uint32_t h          = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_epub_set_font(&book, s_font_buf, s_font_len));
  /* max_pixels == 0: any non-space glyph satisfies total > max_pixels. */
  TEST_ASSERT_EQ(
    k_ra_err_no_mem,
    ra_epub_render_glyph(&book, (int32_t)k_cov_cp_letter_a, s_render_px, s_glyph_buf, 0U, &w, &h));
  TEST_END("epub_render_glyph: priv_render_into total > max_pixels -> no_mem");
}
 ...
 *          out_w and out_h are set (lines 278-279), and k_ra_ok is returned
 *          (line 280), covering the full priv_render_into success path and
 *          the ra_epub_render_glyph() call site at line 578.  Line 219
 *          (return k_ra_ok from priv_font_init) is also covered because
...
int32_t main(void)
{
  ...
  /* Group 1: ra_epub_manifest_count / ra_epub_manifest_item guards. */
  ...
  (void)fprintf(stderr, "[OK ] test_ra_epub_chapter_cov.c\n");
  return 0;
}
