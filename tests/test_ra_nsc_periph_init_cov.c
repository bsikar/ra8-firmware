/**
 * @file test_ra_nsc_periph_init_cov.c
 * @brief Coverage-focused unit tests for libs/ra_nsc/src/ra_nsc_periph_init.c
 *
 * @details
 * Supplements test_ra_nsc.c with a dedicated test binary so that every
 * reachable line in ra_nsc_periph_init() accumulates gcov hits from an
 * independent executable.
 *
 * The function under test has two reachable branches:
 *
 * 1. First call (s_initialized == false): runs
 *    ra_mstp_init -> ra_pwr_init -> ra_isr_init -> ra_dma_init, sets
 *    s_initialized = true, and returns k_ra_ok.
 * 2. Subsequent calls (s_initialized == true): returns k_ra_ok immediately
 ...
 * functions always return k_ra_ok in RA_SIMULATOR_MODE -- the host simulator
 ...
 *
 * @par MC/DC:
 * Decision: ``if (s_initialized)`` in ra_nsc_periph_init() (single condition)
 ...
 * state. Does NOT reset the s_initialized flag inside ra_nsc_periph_init.c --
 ...
 *
 * @pre Binary is running under RA_SIMULATOR_MODE.
...
/**
 * @brief ra_nsc_periph_init: first call executes the full substrate init path.
 *
 * @details
 * When s_initialized is false (process start, never called before in this
 * binary), ra_nsc_periph_init() must run the four-step substrate sequence:
 * ra_mstp_init, ra_pwr_init, ra_isr_init, ra_dma_init.  On the simulator all
 * four always succeed, so the call must return k_ra_ok and set the internal
 * s_initialized flag to true.
 *
 * This test exercises lines 116..117, 124..125, 132..133, 140..141, 158..160
 * of ra_nsc_periph_init.c (non-excluded happy-path body).
 *
 * @par MC/DC:
 ...
 * @pre ra_nsc_periph_init() has never been called in this process.
 * @post ra_nsc_periph_init() returned k_ra_ok.
...
{
  TEST_BEGIN("ra_nsc_periph_init: first call -- full substrate init path");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_periph_init());
  TEST_END("ra_nsc_periph_init: first call -- full substrate init path");
}

/**
 * @brief ra_nsc_periph_init: subsequent calls return k_ra_ok via fast path.
 *
 * @details
 * After the first successful call has set s_initialized to true, every
 * following call must return k_ra_ok immediately without re-running the
 * four substrate init steps.  This exercises line 112 (if check with
 * s_initialized == true) and line 113 (early return k_ra_ok).
 *
 ...
 *
 * @pre test_periph_init_first_call() has already run and returned k_ra_ok.
 * @post ra_nsc_periph_init() returns k_ra_ok on every call.
...
{
  TEST_BEGIN("ra_nsc_periph_init: idempotent -- three fast-path calls");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_periph_init());
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_periph_init());
  TEST_ASSERT_EQ(k_ra_ok, ra_nsc_periph_init());
  TEST_END("ra_nsc_periph_init: idempotent -- three fast-path calls");
}

/**
 * @brief ra_nsc_periph_init: return value is always k_ra_ok on the simulator.
 *
 * @details
 * Verifies that repeated ra_nsc_periph_init() calls in different orderings
 * (with intervening ra_sim_mmap_reset() calls) still return k_ra_ok.
 * ra_sim_mmap_reset() zeroes register windows but does NOT reset the static
 * s_initialized flag, so these calls all take the idempotent fast path.
 *
 * @pre test_periph_init_first_call() has already run.
 * @post All calls return k_ra_ok.
 *
 * @note Not thread-safe; single-threaded test context.
 * @since 0.1.0
 */
...
{
  TEST_BEGIN("ra_nsc_periph_init: always k_ra_ok after first successful init");
  prep();
  ra_err_t err = ra_nsc_periph_init();
  TEST_ASSERT_EQ(k_ra_ok, err);
  prep();
  err = ra_nsc_periph_init();
  TEST_ASSERT_EQ(k_ra_ok, err);
  TEST_END("ra_nsc_periph_init: always k_ra_ok after first successful init");
}
 ...
 * test_periph_init_first_call() is the very first ra_nsc_periph_init()
 ...
 *
 * @pre RA_SIMULATOR_MODE is defined (test build).
 * @post All reachable lines in ra_nsc_periph_init.c have been exercised.
 *
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_nsc_periph_init_cov.c\n");
  return 0;
}
