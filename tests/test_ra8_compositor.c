/**
 * @file test_ra8_compositor.c
 * @brief Host unit tests + demo for the ra8_compositor framebuffer compositor.
 *
 * @details
 ...
 * ra8_gfx framebuffer, and assert:
 *
 ...
 *
 * ra8_compositor has no compound boolean decisions, so there is no `@par MC/DC`
...
static void t_build_demo(ra8_compositor_t*        s,
                         ra8_compositor_widget_t* pool,
                         uint16_t                 cap,
                         uint32_t                 footer_color,
                         const char*              footer_text)
{
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_init(s, pool, cap));
  uint16_t idx = 0U;

  ra8_compositor_widget_t root = {.kind    = k_ra8_compositor_kind_rect,
                                  .color   = (uint32_t)k_t_bg,
                                  .visible = true,
                                  .axis    = k_ra8_compositor_axis_col,
                                  .pad     = (int16_t)k_t_pad,
                                  .gap     = (int16_t)k_t_gap};
  TEST_ASSERT_EQ(k_ra8_ok,
                 ra8_compositor_add(s, &root, (uint16_t)k_ra8_compositor_no_parent, &idx));

  ra8_compositor_widget_t status = {.kind    = k_ra8_compositor_kind_label,
                                    .color   = (uint32_t)k_t_status,
                                    .fg      = (uint32_t)k_t_fg,
                                    .text    = "RA8D2",
                                    .visible = true,
                                    .fixed   = (int16_t)k_t_status_h};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_add(s, &status, k_t_root, &idx));

  ra8_compositor_widget_t body = {.kind    = k_ra8_compositor_kind_rect,
                                  .color   = (uint32_t)k_t_body,
                                  .visible = true,
                                  .flex    = 1U};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_add(s, &body, k_t_root, &idx));

  ra8_compositor_widget_t footer = {.kind    = k_ra8_compositor_kind_label,
                                    .color   = footer_color,
                                    .fg      = (uint32_t)k_t_fg,
                                    .text    = footer_text,
                                    .visible = true,
                                    .fixed   = (int16_t)k_t_footer_h};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_add(s, &footer, k_t_root, &idx));
}
...
{
  const ra8_compositor_rect_t frame              = {0, 0, k_t_fb_w, k_t_fb_h};
  ra8_compositor_widget_t     pool[k_t_pool_cap] = {};
  ra8_compositor_t            s                  = {};
  t_build_demo(&s, pool, (uint16_t)k_t_pool_cap, footer_color, footer_text);
  TEST_ASSERT_EQ(
    k_ra8_ok,
    ra8_gfx_init(buf, (uint16_t)k_t_fb_w, (uint16_t)k_t_fb_h, k_ra8_gfx_format_argb8888));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_compose_full(&s, &frame));
...
/** @brief ra8_compositor_init guards + success. */
static void test_init_guards(void)
{
  TEST_BEGIN("ra8_compositor init guards");
  ra8_compositor_widget_t pool[2] = {};
  ra8_compositor_t        s       = {};
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_init(nullptr, pool, 2U));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_init(&s, nullptr, 2U));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_compositor_init(&s, pool, 0U));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_init(&s, pool, 2U));
  TEST_ASSERT_EQ(0, s.count);
  TEST_ASSERT_EQ(2, s.cap);
  TEST_END("ra8_compositor init guards");
}

/** @brief ra8_compositor_add guards: null, no-mem, bad parent, success. */
static void test_add_guards(void)
{
  TEST_BEGIN("ra8_compositor add guards");
  ra8_compositor_widget_t pool[2] = {};
  ra8_compositor_t        s       = {};
  ra8_compositor_widget_t w       = {.kind = k_ra8_compositor_kind_rect, .visible = true};
  uint16_t                idx     = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_init(&s, pool, 2U));

  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_compositor_add(nullptr, &w, (uint16_t)k_ra8_compositor_no_parent, &idx));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_compositor_add(&s, nullptr, (uint16_t)k_ra8_compositor_no_parent, &idx));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_compositor_add(&s, &w, (uint16_t)k_ra8_compositor_no_parent, nullptr));

  /* First node must be the root (no-parent sentinel). */
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_compositor_add(&s, &w, 0U, &idx));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_add(&s, &w, (uint16_t)k_ra8_compositor_no_parent, &idx));
  TEST_ASSERT_EQ(0, idx);

  /* Second node must name an existing parent. */
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_compositor_add(&s, &w, 5U, &idx));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_add(&s, &w, 0U, &idx));
  TEST_ASSERT_EQ(1, idx);

  /* Pool now full (cap 2): the next add is rejected. */
  TEST_ASSERT_EQ(k_ra8_err_no_mem, ra8_compositor_add(&s, &w, 0U, &idx));
  TEST_END("ra8_compositor add guards");
}
...
{
  TEST_BEGIN("ra8_compositor layout column math");
  ra8_compositor_widget_t pool[k_t_pool_cap] = {};
  ra8_compositor_t        s                  = {};
  t_build_demo(&s, pool, (uint16_t)k_t_pool_cap, (uint32_t)k_t_footer, "p.1");

  const ra8_compositor_rect_t frame = {0, 0, k_t_fb_w, k_t_fb_h};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_layout(&s, &frame));
  ...
  TEST_END("ra8_compositor layout column math");
}

/** @brief Row tiling with flex weights 1:3 and an all-fixed row (no flex). */
static void test_layout_row_and_fixed(void)
{
  TEST_BEGIN("ra8_compositor layout row + flex split");
  ra8_compositor_widget_t pool[4] = {};
  ra8_compositor_t        s       = {};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_init(&s, pool, 4U));
  uint16_t                idx = 0U;
  ra8_compositor_widget_t row = {.kind    = k_ra8_compositor_kind_container,
                                 .visible = true,
                                 .axis    = k_ra8_compositor_axis_row};
  TEST_ASSERT_EQ(k_ra8_ok,
                 ra8_compositor_add(&s, &row, (uint16_t)k_ra8_compositor_no_parent, &idx));
  ra8_compositor_widget_t a = {.kind = k_ra8_compositor_kind_rect, .visible = true, .flex = 1U};
  ra8_compositor_widget_t b = {.kind = k_ra8_compositor_kind_rect, .visible = true, .flex = 3U};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_add(&s, &a, 0U, &idx));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_add(&s, &b, 0U, &idx));

  const ra8_compositor_rect_t frame = {0, 0, 80, 40};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_layout(&s, &frame));
  ...
  TEST_END("ra8_compositor layout row + flex split");
}

/** @brief Layout guards, invisible-child skip, and overflow clamping. */
static void test_layout_edges(void)
{
  TEST_BEGIN("ra8_compositor layout guards + clamps");
  ra8_compositor_widget_t     pool[4] = {};
  ra8_compositor_t            s       = {};
  const ra8_compositor_rect_t frame   = {0, 0, 40, 40};

  /* Guards: null scene / frame, empty scene. */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_layout(nullptr, &frame));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_init(&s, pool, 4U));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_layout(&s, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_compositor_layout(&s, &frame));

  /* Root with a huge pad (content clamps to 0) plus an invisible child. */
  uint16_t                idx  = 0U;
  ra8_compositor_widget_t root = {.kind    = k_ra8_compositor_kind_container,
                                  .visible = true,
                                  .axis    = k_ra8_compositor_axis_col,
                                  .pad     = 100};
  TEST_ASSERT_EQ(k_ra8_ok,
                 ra8_compositor_add(&s, &root, (uint16_t)k_ra8_compositor_no_parent, &idx));
  ra8_compositor_widget_t big = {.kind = k_ra8_compositor_kind_rect, .visible = true, .fixed = 999};
  ra8_compositor_widget_t flx = {.kind = k_ra8_compositor_kind_rect, .visible = true, .flex = 1U};
  ra8_compositor_widget_t hid = {.kind = k_ra8_compositor_kind_rect, .visible = false, .flex = 1U};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_add(&s, &big, 0U, &idx));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_add(&s, &flx, 0U, &idx));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_add(&s, &hid, 0U, &idx));
  ...
  TEST_END("ra8_compositor layout guards + clamps");
}
...
{
  TEST_BEGIN("ra8_compositor damage + invalidate");
  ra8_compositor_widget_t pool[k_t_pool_cap] = {};
  ra8_compositor_t        s                  = {};
  t_build_demo(&s, pool, (uint16_t)k_t_pool_cap, (uint32_t)k_t_footer, "p.1");
  const ra8_compositor_rect_t frame = {0, 0, k_t_fb_w, k_t_fb_h};
  ...
  ra8_compositor_rect_t rect = {1, 2, 3, 4};
  uint16_t              n    = 9U;

  /* Guards. */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_damage(nullptr, &rect, &n));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_damage(&s, nullptr, &n));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_damage(&s, &rect, nullptr));

  /* Nothing dirty -> empty rect, count 0. */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_damage(&s, &rect, &n));
  TEST_ASSERT_EQ(0, n);
  TEST_ASSERT_EQ(0, rect.w);
  TEST_ASSERT_EQ(0, rect.h);

  /* invalidate guards. */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_invalidate(nullptr, 0U));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_compositor_invalidate(&s, 99U));

  /* One dirty node -> damage equals its bounds. */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_invalidate(&s, k_t_status_i));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_damage(&s, &rect, &n));
  TEST_ASSERT_EQ(1, n);
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_invalidate(&s, k_t_footer_i));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_damage(&s, &rect, &n));
  TEST_ASSERT_EQ(2, n);
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_invalidate(&s, k_t_body_i));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_damage(&s, &rect, &n));
  TEST_ASSERT_EQ(2, n); /* body skipped despite being dirty */

  /* clear_dirty guards + effect. */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_clear_dirty(nullptr));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_clear_dirty(&s));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_damage(&s, &rect, &n));
  TEST_ASSERT_EQ(0, n);
  TEST_END("ra8_compositor damage + invalidate");
}
...
{
  TEST_BEGIN("ra8_compositor render/compose guards");
  ra8_compositor_widget_t     pool[2] = {};
  ra8_compositor_t            s       = {};
  const ra8_compositor_rect_t frame   = {0, 0, 8, 8};
  ra8_compositor_rect_t       dmg     = {};
  uint16_t                    n       = 0U;
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_render(nullptr, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_compose_full(nullptr, &frame));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_compose_full(&s, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_compose_dirty(nullptr, &frame, &dmg, &n));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_compose_dirty(&s, nullptr, &dmg, &n));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_compose_dirty(&s, &frame, nullptr, &n));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_compositor_compose_dirty(&s, &frame, &dmg, nullptr));

  /* Empty scene -> render / compose forward invalid_arg from the count check. */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_init(&s, pool, 2U));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_compositor_render(&s, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_compositor_compose_full(&s, &frame));
  TEST_END("ra8_compositor render/compose guards");
}
...
{
  TEST_BEGIN("ra8_compositor dirty==full recomposite + CRC golden");
  const ra8_compositor_rect_t frame = {0, 0, k_t_fb_w, k_t_fb_h};
  ...
  ra8_compositor_rect_t dmg = {};
  uint16_t              dn  = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_compose_dirty(&s, &frame, &dmg, &dn));
  TEST_ASSERT_EQ(1, dn);
  ...
  TEST_END("ra8_compositor dirty==full recomposite + CRC golden");
}
