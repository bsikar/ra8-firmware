...
static void test_import_invalid_args(void)
{
  ...
  TEST_ASSERT_EQ((int32_t)k_ra_err_invalid_size, (int32_t)ra_psa_key_import(&k, &attr, big, sizeof(big)));
  /* Empty usage rejected. */
  const ra_psa_key_attr_t no_use = {.type = k_ra_psa_key_type_aes, .alg = k_ra_psa_alg_aes_gcm,
...
static void test_import_destroy_round_trip(void)
{
  ...
  ra_psa_key_t k = NULL;
  TEST_ASSERT_EQ((int32_t)k_ra_ok, (int32_t)ra_psa_key_import(&k, &attr, k_test_aes_key, sizeof(k_test_aes_key)));
...
static void test_hash_known_vector(void)
{
  ...
  size_t        out_len = 0U;
  TEST_ASSERT_EQ((int32_t)k_ra_ok,
                 (int32_t)ra_psa_hash_compute(k_ra_psa_alg_sha_256, input, sizeof(input), digest, sizeof(digest),
                                              &out_len));
...
static void test_hash_invalid_args(void)
{
  ...
  TEST_ASSERT_EQ((int32_t)k_ra_err_invalid_arg,
                 (int32_t)ra_psa_hash_compute(k_ra_psa_alg_aes_gcm, (const uint8_t*)"x", 1U, out, sizeof(out), &ol));
...
static void test_sign_verify_round_trip(void)
{
  ...
  ra_psa_key_t k = NULL;
  TEST_ASSERT_EQ((int32_t)k_ra_ok, (int32_t)ra_psa_key_import(&k, &attr, k_test_ecdsa_key, sizeof(k_test_ecdsa_key)));
  ...
  TEST_ASSERT_EQ((int32_t)k_ra_ok,
                 (int32_t)ra_psa_hash_compute(k_ra_psa_alg_sha_256, msg, sizeof(msg), digest, sizeof(digest),
                                              &digest_len));

  uint8_t sig[k_ra_psa_max_sig_bytes];
  size_t  sig_len = 0U;
  TEST_ASSERT_EQ((int32_t)k_ra_ok, (int32_t)ra_psa_sign_hash(k, k_ra_psa_alg_ecdsa_sha_256, digest, digest_len, sig,
                                                              sizeof(sig), &sig_len));
  TEST_ASSERT(sig_len > 0U);

  TEST_ASSERT_EQ((int32_t)k_ra_ok, (int32_t)ra_psa_verify_hash(k, k_ra_psa_alg_ecdsa_sha_256, digest, digest_len, sig,
                                                                sig_len));

  /* Tamper a byte and ensure verification fails. */
  sig[0] ^= 0xFFU;
  TEST_ASSERT_EQ((int32_t)k_ra_err_crc_mismatch, (int32_t)ra_psa_verify_hash(k, k_ra_psa_alg_ecdsa_sha_256, digest,
...
static void test_sign_wrong_alg(void)
{
  ...
  ra_psa_key_t k = NULL;
  TEST_ASSERT_EQ((int32_t)k_ra_ok, (int32_t)ra_psa_key_import(&k, &attr, k_test_ecdsa_key, sizeof(k_test_ecdsa_key)));
  uint8_t hash[k_ra_psa_sha256_len] = {0};
  uint8_t sig[k_ra_psa_max_sig_bytes];
  size_t  sl = 0U;
...
static void test_aead_round_trip(void)
{
  ...
  size_t        ct_len = 0U;
  TEST_ASSERT_EQ((int32_t)k_ra_ok, (int32_t)ra_psa_aead_encrypt(k, k_ra_psa_alg_aes_gcm, k_test_nonce,
                                                                  sizeof(k_test_nonce), aad, sizeof(aad), plain,
                                                                  sizeof(plain), ct, sizeof(ct), &ct_len));
  TEST_ASSERT_EQ((int64_t)(sizeof(plain) + (size_t)k_ra_psa_gcm_tag_len), (int64_t)ct_len);

  uint8_t recovered[k_psa_test_plain_len];
  size_t  rec_len = 0U;
  TEST_ASSERT_EQ((int32_t)k_ra_ok, (int32_t)ra_psa_aead_decrypt(k, k_ra_psa_alg_aes_gcm, k_test_nonce,
                                                                  sizeof(k_test_nonce), aad, sizeof(aad), ct, ct_len,
  ...
  TEST_ASSERT_EQ((int32_t)k_ra_err_crc_mismatch,
                 (int32_t)ra_psa_aead_decrypt(k, k_ra_psa_alg_aes_gcm, k_test_nonce, sizeof(k_test_nonce), aad,
                                              sizeof(aad), ct, ct_len, recovered, sizeof(recovered), &rec_len));
...
static void test_aead_invalid_args(void)
{
  ...
  /* Wrong nonce length. */
  TEST_ASSERT_EQ((int32_t)k_ra_err_invalid_size,
                 (int32_t)ra_psa_aead_encrypt(k, k_ra_psa_alg_aes_gcm, k_test_nonce, 8U, NULL, 0U,
                                              (const uint8_t*)"hi", 2U, out, sizeof(out), &ol));
  /* Output too small. */
  TEST_ASSERT_EQ((int32_t)k_ra_err_invalid_size,
                 (int32_t)ra_psa_aead_encrypt(k, k_ra_psa_alg_aes_gcm, k_test_nonce, sizeof(k_test_nonce), NULL, 0U,
                                              (const uint8_t*)"hi", 2U, out, 2U, &ol));
  /* Decrypt with cipher_len < tag length. */
  uint8_t tiny[8] = {0};
  TEST_ASSERT_EQ((int32_t)k_ra_err_invalid_size,
                 (int32_t)ra_psa_aead_decrypt(k, k_ra_psa_alg_aes_gcm, k_test_nonce, sizeof(k_test_nonce), NULL, 0U,
                                              tiny, sizeof(tiny), out, sizeof(out), &ol));
