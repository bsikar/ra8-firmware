/**
 * @file test_ra_io_blockdev_vsource.c
 * @brief Unit tests for the block-device -> ra_vsource read adapter (#147/#155).
 *
 * @details
 * Proves the Ring-4 adapter (`ra_io_blockdev_vsource.h`) reads back bytes
 * identical to ::ra_io_blockdev_read over a RAM block device. Blocks are written
 ...
 * validation guards and the end-to-end wiring into ::ra_vsource_add_paged so the
...
static ra_io_blockdev_vsource_ctx_t s_ctx;
static ra_vsource_obj_t             s_objs[(size_t)k_t_objs];

/** @brief Bind a RAM block device and fill it with a deterministic pattern. */
static void t_setup_disk(void)
{
  s_bd    = (ra_io_blockdev_t){};
  s_state = (ra_io_blockdev_ram_state_t){};
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_ram_init(&s_bd, &s_state, s_disk, k_t_blocks, false));
  for (uint32_t i = 0; i < (uint32_t)k_t_total; ++i) {
    s_golden[i] = (uint8_t)((i * 13U + 7U) & 0xFFu);
  }
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_write(&s_bd, 0U, k_t_blocks, s_golden));
}
...
static void test_init_validation(void)
{
  ...
  ra_io_blockdev_vsource_ctx_t ctx = {};
  ra_io_blockdev_t             bd  = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_blockdev_vsource_init(nullptr, &bd));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_blockdev_vsource_init(&ctx, nullptr));
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_vsource_init(&ctx, &bd));
...
static void test_read_validation(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_io_blockdev_vsource_read(nullptr, 0U, buf, k_t_block_len));
  t_setup_disk();
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_vsource_init(&s_ctx, &s_bd));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_io_blockdev_vsource_read(&s_ctx, 0U, nullptr, k_t_block_len));
...
static void test_aligned_block(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_io_blockdev_vsource_read(&s_ctx, (uint64_t)k_t_block_len * 2u, got, k_t_block_len));
...
static void test_unaligned_head_only(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_vsource_read(&s_ctx, k_t_head_off, got, k_t_head_len));
...
static void test_unaligned_span(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_vsource_read(&s_ctx, k_t_span_off, got, k_t_span_len));
...
static void test_out_of_range(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_out_of_range,
                 ra_io_blockdev_vsource_read(&s_ctx,
 ...
 * and paging through ::ra_vsource_loader yields the golden bytes)
 */
static void test_vsource_wiring(void)
{
  TEST_BEGIN("bdvs wired into ra_vsource_add_paged");
  t_setup_disk();
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_vsource_init(&s_ctx, &s_bd));
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_vsource_add_paged(&vs, ra_io_blockdev_vsource_read, &s_ctx, 0U, k_t_total, &oid));

  /* Page a 512-byte frame in at an aligned object offset via the loader. */
  uint8_t frame[(size_t)k_t_block_len] = {};
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_vsource_loader(&vs, oid, (uint64_t)k_t_block_len * 3u, frame, k_t_block_len));
  TEST_ASSERT_EQ(0, memcmp(frame, &s_golden[(size_t)k_t_block_len * 3u], (size_t)k_t_block_len));
  TEST_END("bdvs wired into ra_vsource_add_paged");
}
...
int32_t main(void)
{
  ...
  test_out_of_range();
  test_vsource_wiring();
  (void)fprintf(stderr, "[OK  ] test_ra_io_blockdev_vsource.c\n");
  return 0;
}
