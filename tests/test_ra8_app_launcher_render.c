 ...
 *
 * @details
 * Ties the app framework (`ra8_app`, #146) to the framebuffer compositor
 * (`ra8_compositor`, #145): the "chrome" enumerates the app **registry** and
 * renders a **launcher** -- a header bar plus one tile per registered app --
 * into an in-memory `ra8_gfx` framebuffer, and the result is pinned with a
 ...
 * (`ra8_app_count` + `ra8_app_at` + `app->name`), so this is a faithful host proxy
 * for the on-panel chrome. There are no compound boolean decisions under test
 * here (the logic lives in `ra8_app` / `ra8_compositor`, each tested in its own
 ...
 * `ra8_app_at` -- adding a label tile carrying each app's `name` under the root
 * column. The NULL-slot guard mirrors the on-target chrome (`app_shell_log_menu`).
 */
static void lr_build_launcher(ra8_compositor_t*         s,
                              ra8_compositor_widget_t*  pool,
                              uint16_t                  cap,
                              const ra8_app_registry_t* reg)
{
  ...
  ra8_compositor_widget_t header = {.kind    = k_ra8_compositor_kind_label,
                                    .color   = (uint32_t)k_lr_header,
                                    .fg      = (uint32_t)k_lr_fg,
                                    .text    = "Apps",
                                    .visible = true,
                                    .fixed   = (int16_t)k_lr_header_h};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_add(s, &header, 0U, &idx));
  ...
  for (uint16_t i = 0U; i < count; ++i) {
    ...
    if (ra8_app_at(reg, i, &app) == k_ra8_ok) {
      if (app != nullptr) {
        ra8_compositor_widget_t tile = {.kind    = k_ra8_compositor_kind_label,
        ...
        uint16_t                ti   = 0U;
        TEST_ASSERT_EQ(k_ra8_ok, ra8_compositor_add(s, &tile, 0U, &ti));
      }
    }
  }
}
...
{
  ra8_compositor_widget_t     pool[k_lr_pool_cap] = {};
  ra8_compositor_t            s                   = {};
  const ra8_compositor_rect_t frame               = {0, 0, k_lr_fb_w, k_lr_fb_h};
...
{
  TEST_BEGIN("ra8_app: compositor launcher CRC golden + uninstall reflow");
  ...
  TEST_END("ra8_app: compositor launcher CRC golden + uninstall reflow");
}
