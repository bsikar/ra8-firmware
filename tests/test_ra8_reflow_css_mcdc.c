/**
 * @file test_ra8_reflow_css_mcdc.c
 * @brief Supplemental MC/DC host tests for the content-CSS cascade (#111/#140).
 *
 * @details
 * Drives the REAL `ra8_reflow_css` parser / matcher / cascade through the
 * compound boolean decisions that `test_ra8_css.c` leaves uncovered, so each
 * condition is exercised both true and false. Every test reaches the target
 * decision through a public entry point (::ra8_css_parse, ::ra8_css_parse_inline,
 * ::ra8_css_cascade, ::ra8_css_cascade_ctx, ::ra8_css_rule_matches,
 * ::ra8_css_match_face, ::ra8_css_face_src) with crafted CSS / element input;
 ...
 *  - ra8_css_parse        : the `/` not-followed-by-`*` (cond3-false) comment scan.
 ...
 *  - ra8_css_match_face   : the italic-only (cond2) / second-regular (cond3) arms.
 *  - ra8_css_face_src     : the null-out_len / valid-index 4-OR guard.
 *
...
static ra8_css_style_t inl(const char* decls)
{
  ra8_css_style_t d = {};
...
static void test_ancestor_constraint_arms(void)
{
  ...
  const ra8_css_element_t a_bq[1] = {elem(k_ra8_reflow_tag_blockquote, nullptr, nullptr)};
  const ra8_css_element_t a_h1[1] = {elem(k_ra8_reflow_tag_h1, nullptr, nullptr)};
  ...
  const ra8_css_element_t a_box[1]   = {elem(k_ra8_reflow_tag_blockquote, nullptr, "box")};
  const ra8_css_element_t a_other[1] = {elem(k_ra8_reflow_tag_blockquote, nullptr, "other")};
  ...
  const ra8_css_element_t a_main[1] = {elem(k_ra8_reflow_tag_blockquote, "main", nullptr)};
  const ra8_css_element_t a_side[1] = {elem(k_ra8_reflow_tag_blockquote, "side", nullptr)};
...
static void test_rule_family_empty(void)
{
  ...
  ra8_css_element_t pe = elem(k_ra8_reflow_tag_p, nullptr, nullptr);
  ra8_css_element_t he = elem(k_ra8_reflow_tag_h1, nullptr, nullptr);
...
static void test_class_list_multi_token(void)
{
  ...
  const ra8_css_element_t a_has[1] = {elem(k_ra8_reflow_tag_blockquote, nullptr, " lead box ")};
  const ra8_css_element_t a_no[1]  = {elem(k_ra8_reflow_tag_blockquote, nullptr, "lead only")};
...
static void test_resolve_specificity_override(void)
{
  ...
  ra8_css_element_t e = elem(k_ra8_reflow_tag_p, nullptr, "note");
...
static void test_class_list_trailing_ws_and_samelen_mcdc(void)
{
  ...
  const ra8_css_element_t a_trail[1] = {elem(k_ra8_reflow_tag_blockquote, nullptr, "lead only ")};
  /* Non-matching token the SAME length as "box" (3) -> tlen==nlen, memcmp!=0. */
  const ra8_css_element_t a_samelen[1] = {elem(k_ra8_reflow_tag_blockquote, nullptr, "cat")};
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra8_reflow_css_mcdc.c\n");
  return 0;
}
