/**
 * @file test_ra_usb_pvnd_cov.c
 * @brief Coverage top-up for the native USB device-Vendor class layer
 *
 * @details
 * Targets the residual uncovered lines in
 * ``libs/ra_hal/src/ra_usb_pvnd.c`` that the primary suite
 * (``test_ra_usb_pvnd.c``) does not reach:
 *   - ``ra_usb_pvnd_set_descriptors``: the NULL-pointer reject, the
 *     zero-length reject, and the store-and-succeed happy path (lines
 *     209-215) -- the primary suite only calls it pre-init.
 *   - ``ra_usb_pvnd_recv``: both legs of the queue drain -- the
 *     empty-pipe leg (``ra_usb_queue_out`` returns ``k_ra_err_no_data``
 *     and ``*got_len`` is zeroed) and the drained-bank leg (queue_out
 *     returns ``k_ra_ok`` and ``*got_len`` reflects the byte count),
 *     lines 247-255.
 *   - ``ra_usb_pvnd_handle_setup``: the application-callback-returned-error
 *     leg that stalls EP0 (line 293).
 *   - ``ra_usb_pvnd_init``: the ``ra_usb_device_init`` failure leg
 *     (lines 170-171) driven by saturating the USBFS module-stop
 *     ref-count so ``ra_mstp_enable`` inside ``ra_usb_device_init``
 *     returns an error.
 *
 ...
 * ``internal_configure_pipes`` failure return in ``ra_usb_pvnd_init``
 ...
 * @retval k_ra_ok Default accept path.
 */
 ...
 *
 * @details ``ra_mstp_init`` clears every module-stop ref-count to zero,
 * which is essential for the ref-count-saturation case: each test starts
 * from a clean count. ``ra_usb_pvnd_close`` drops any prior init.
 */
...
/**
 * @par MC/DC:
 * (no compound decision exercised -- ``RA_CHECK_NULL_PTR`` and
...
{
  TEST_BEGIN("ra_usb_pvnd_set_descriptors: NULL / zero-len / store");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_pvnd_init(k_ra_usb_speed_fs));

  /* NULL blob -> RA_CHECK_NULL_PTR reject (line 209). */
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_usb_pvnd_set_descriptors(nullptr, (uint16_t)k_test_desc_len));
  /* Zero length -> invalid_arg (lines 210-211). */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_pvnd_set_descriptors(s_sample_desc, 0U));
  /* Valid blob -> stored, k_ra_ok (lines 213-215). */
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_usb_pvnd_set_descriptors(s_sample_desc, (uint16_t)sizeof(s_sample_desc)));

  TEST_END("ra_usb_pvnd_set_descriptors: NULL / zero-len / store");
}

/**
 * @par MC/DC:
 * (no compound decision exercised -- ``if (err == k_ra_ok)`` at the
 * drain site is a single condition; this case pins it FALSE via an
 * empty pipe and the sibling case pins it TRUE)
 *
 * @details With BRDYSTS clear (the reset default), ``ra_usb_queue_out``
 * takes its no-data fast path and returns ``k_ra_err_no_data``. The
...
{
  TEST_BEGIN("ra_usb_pvnd_recv reports no_data and zeroes got_len on empty pipe");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_pvnd_init(k_ra_usb_speed_fs));

  uint8_t  buf[k_test_recv_capacity] = {};
  uint16_t got                       = 0xBEEFU;
  TEST_ASSERT_EQ(k_ra_err_no_data, ra_usb_pvnd_recv(buf, (uint16_t)k_test_recv_capacity, &got));
  /* Error leg must clear the out-count (line 253). */
  TEST_ASSERT_EQ(0U, got);

  TEST_END("ra_usb_pvnd_recv reports no_data and zeroes got_len on empty pipe");
}
 ...
 * bank. ``ra_usb_queue_out`` then returns ``k_ra_ok`` and the class
 * layer copies the drained byte count into ``*got_len`` (line 251).
 */
static void test_recv_drains_bank(void)
{
  TEST_BEGIN("ra_usb_pvnd_recv drains a staged bank and reports the byte count");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_pvnd_init(k_ra_usb_speed_fs));
  ...
  reg->CFIFOCTR = (uint16_t)((uint16_t)k_ra_fifoctr_frdy | (uint16_t)k_test_recv_avail);
...
/**
 * @par MC/DC:
 * (no compound decision exercised -- ``if (cb_err != k_ra_ok)`` is a
 ...
 * callback-error branch and stalls EP0 via ``ra_usb_control_response``
 * (line 293). Under simulation that stall request itself succeeds, so
 * ``ra_usb_pvnd_handle_setup`` returns ``k_ra_ok`` even though the
...
{
  TEST_BEGIN("ra_usb_pvnd_handle_setup stalls EP0 when the vendor callback fails");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_pvnd_init(k_ra_usb_speed_fs));
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_pvnd_attach_setup_handler(test_setup_cb, nullptr));

  s_setup_cb_return_code = k_ra_err_timeout; /* Force the error branch. */
  ...
  /* The stall response itself returns k_ra_ok under simulation. */
  ...
  TEST_END("ra_usb_pvnd_handle_setup stalls EP0 when the vendor callback fails");
}

/**
 * @par MC/DC:
 * (no compound decision exercised -- ``if (usb_err != k_ra_ok)`` is a
 * single condition; this case pins it TRUE while every other init case
 * pins it FALSE)
 *
 * @details Drives the ``ra_usb_device_init`` failure leg of
 * ``ra_usb_pvnd_init`` (lines 170-171). ``ra_usb_device_init`` calls
 * ``ra_mstp_enable`` for the USBFS module; that helper returns
 * ``k_ra_err_invalid_state`` once the module's ref-count is saturated at
 * ``UINT8_MAX``. Saturating it here (255 enables, no matching disables)
 * makes the device-init call fail, so ``ra_usb_pvnd_init`` logs the
 * failure and returns ``k_ra_err_hw_init_failed`` without ever setting
 * ``s_state.initialized``. ``ra_mstp_init`` in ``prep`` restores a clean
 * ref-count for every other case.
 */
static void test_init_device_init_failure(void)
{
  TEST_BEGIN("ra_usb_pvnd_init maps a device-init failure to hw_init_failed");
  prep();

  /* Pin the USBFS module-stop ref-count at its ceiling so the enable
   * inside ra_usb_device_init trips the saturation guard. */
  for (uint16_t i = 0U; i < (uint16_t)k_test_mstp_saturate; ++i) {
    (void)ra_mstp_enable(k_ra_mstp_usbfs);
  }

  TEST_ASSERT_EQ(k_ra_err_hw_init_failed, ra_usb_pvnd_init(k_ra_usb_speed_fs));

  TEST_END("ra_usb_pvnd_init maps a device-init failure to hw_init_failed");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_usb_pvnd_cov.c\n");
  return 0;
}
