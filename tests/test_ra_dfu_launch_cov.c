 ...
 *
 * @details
 * `ra_dfu_launch` is the shared copy-to-run hand-off behind the DFU
 ...
 * `#ifndef RA_SIMULATOR_MODE`. Under the host test build `RA_SIMULATOR_MODE`
 ...
 *  - the `ra_dfu_run_target_valid` cross-check early return; and
 *  - (only when `RA_ENABLE_ROOT_OF_TRUST` is defined) the root-of-trust
 ...
 * `ra_dfu_launch` linked from `ra_core_hal` across all three of those legs.
 *
 ...
 * with `RA_ENABLE_ROOT_OF_TRUST` defined: the one exported symbol
 * (`ra_dfu_launch`) is renamed to `ra_dfu_launch_cov` so it does not collide
 * with the production copy, and the three root-of-trust dependencies
 * (`ra_rot_trailer_after`, `ra_rot_verify_image`, `ra_rot_antirollback_verify`
 ...
 *
 * @par MC/DC:
 * `ra_dfu_launch` carries no compound decisions -- both guards are
 ...
 * - `!ra_dfu_run_target_valid(entry, img_len)`: V1 valid target -> false
 *   (continue); V2 bad entry -> true (return).
 * The third call (src!=0, valid target) falls through both guards; the
 * hardware copy/branch is `#ifndef RA_SIMULATOR_MODE` and compiled out on the
 * host, so control simply returns.
 */
static void test_launch_production_default_paths(void)
{
  TEST_BEGIN("ra_dfu_launch: src==0, invalid target, and valid fall-through");

  uint8_t img[64] = {};

  /* src == 0 -> nothing to copy -> early return. */
  ra_dfu_launch(0U, (uint32_t)k_tc_img_len, (uint32_t)k_ra_dfu_run_base);

  /* Bad entry -> ra_dfu_run_target_valid false -> caller-fallback return. */
  ra_dfu_launch((uintptr_t)img, (uint32_t)k_tc_img_len, (uint32_t)k_tc_entry_bad);

  /* Valid: entry == run base, len is a 32-byte page. Both guards fall
   * through; the copy/branch is compiled out under RA_SIMULATOR_MODE, so the
   * call returns without touching MSP/VTOR. Reaching here proves it. */
  ra_dfu_launch((uintptr_t)img, (uint32_t)k_tc_img_len, (uint32_t)k_ra_dfu_run_base);

  TEST_ASSERT(true);
  TEST_END("ra_dfu_launch: src==0, invalid target, and valid fall-through");
}

/* =============================================================================
 * Root-of-trust white-box copy: a second, instrumented compile of the module
 * with RA_ENABLE_ROOT_OF_TRUST defined and its verify dependencies mocked.
 * =============================================================================
 */

/** @brief Mock trailer `mock_rot_trailer_after` hands back (never NULL). */
static ra_rot_trailer_t s_mock_trailer = {};
/** @brief Result `mock_rot_verify_image` returns. */
static ra_err_t s_verify_err = k_ra_ok;
/** @brief Result `mock_rot_antirollback_verify` returns. */
static ra_err_t s_antirollback_err = k_ra_ok;
/** @brief Dummy store the default-store mock hands back (never NULL). */
static const ra_rot_antirollback_store_t s_mock_store = {};

static const ra_rot_trailer_t* mock_rot_trailer_after(const void* image_base, uint32_t body_len)
{
  (void)image_base;
  (void)body_len;
...
static ra_err_t
mock_rot_verify_image(const uint8_t* body, uint32_t body_len, const ra_rot_trailer_t* trailer)
{
  (void)body;
  (void)body_len;
  (void)trailer;
...
static const ra_rot_antirollback_store_t* mock_rot_antirollback_default_store(void)
{
  return &s_mock_store;
}

static ra_err_t mock_rot_antirollback_verify(const ra_rot_antirollback_store_t* store,
 ...
 * production `ra_dfu_launch` linked from ra_core_hal, enable the flag-gated
 * root-of-trust arms, and redirect their dependencies to the mocks above. */
void ra_dfu_launch_cov(uintptr_t src, uint32_t img_len, uint32_t entry);

#define RA_ENABLE_ROOT_OF_TRUST
#define ra_dfu_launch                     ra_dfu_launch_cov
#define ra_rot_trailer_after              mock_rot_trailer_after
#define ra_rot_verify_image               mock_rot_verify_image
#define ra_rot_antirollback_verify        mock_rot_antirollback_verify
#define ra_rot_antirollback_default_store mock_rot_antirollback_default_store

#include "ra_dfu_launch.c" // NOLINT(bugprone-suspicious-include) -- white-box copy

/* =============================================================================
 * White-box tests: each drives the instrumented `ra_dfu_launch_cov` on scripted
 ...
 * - `!ra_dfu_run_target_valid(entry, img_len)`: V (true, bad entry) -> return.
...
{
  TEST_BEGIN("ra_dfu_launch_cov: src==0 and bad-entry early returns");

  uint8_t img[64] = {};

  /* src == 0 -> early return before the root-of-trust gate. */
  ra_dfu_launch_cov(0U, (uint32_t)k_tc_img_len, (uint32_t)k_ra_dfu_run_base);

  /* Bad entry -> run-target cross-check false -> return before verify. */
  ra_dfu_launch_cov((uintptr_t)img, (uint32_t)k_tc_img_len, (uint32_t)k_tc_entry_bad);

  TEST_ASSERT(true);
  TEST_END("ra_dfu_launch_cov: src==0 and bad-entry early returns");
}
 ...
 *
 * @par MC/DC:
 * Decision `ra_rot_verify_image(...) != k_ra_ok` (1 condition).
 * - V (true): a forged / mis-signed image (mock returns crc_mismatch) ->
 *   DEFAULT-DENY, the launch returns without reaching anti-rollback.
 * The false control (verify == k_ra_ok) is `test_launch_cov_rot_accept`.
 * N+1 = 2 vectors for N=1 condition: minimal MC/DC.
 */
static void test_launch_cov_rot_verify_deny(void)
{
  TEST_BEGIN("ra_dfu_launch_cov(ROT): verify failure default-denies");

  uint8_t img[64]    = {};
  s_verify_err       = k_ra_err_crc_mismatch; /* Signature did not verify. */
  s_antirollback_err = k_ra_ok;
  ra_dfu_launch_cov((uintptr_t)img, (uint32_t)k_tc_img_len, (uint32_t)k_ra_dfu_run_base);

  TEST_ASSERT(true);
  TEST_END("ra_dfu_launch_cov(ROT): verify failure default-denies");
}
 ...
 *
 * @par MC/DC:
 * Decision `ra_rot_antirollback_verify(...) != k_ra_ok` (1 condition),
 * reached only when verify passed:
 * - verify guard false (verify == k_ra_ok), so control enters the
 *   anti-rollback guard;
...
{
  TEST_BEGIN("ra_dfu_launch_cov(ROT): anti-rollback failure default-denies");

  uint8_t img[64]    = {};
  s_verify_err       = k_ra_ok;
  s_antirollback_err = k_ra_err_validation_failed; /* Downgrade / unprovisioned. */
  ra_dfu_launch_cov((uintptr_t)img, (uint32_t)k_tc_img_len, (uint32_t)k_ra_dfu_run_base);

  TEST_ASSERT(true);
  TEST_END("ra_dfu_launch_cov(ROT): anti-rollback failure default-denies");
}
 ...
 * - `ra_rot_verify_image(...) != k_ra_ok` -> false (verify == k_ra_ok);
 * - `ra_rot_antirollback_verify(...) != k_ra_ok` -> false (== k_ra_ok).
 * Both guards fall through to the hardware copy/branch, which is
 * `#ifndef RA_SIMULATOR_MODE` and compiled out on the host, so the accepted
...
{
  TEST_BEGIN("ra_dfu_launch_cov(ROT): authentic + fresh image is accepted");
  ...
  TEST_ASSERT(true);
  TEST_END("ra_dfu_launch_cov(ROT): authentic + fresh image is accepted");
}
...
int main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_dfu_launch_cov.c\n");
  return 0;
}
