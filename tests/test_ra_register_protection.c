/**
 * @file test_ra_register_protection.c
 * @brief Unit tests for the scoped PRCR unlock helper macro
 *
 * @details
 * `libs/ra_core/inc/ra_register_protection.h` exposes the
 * `RA_PROTECTED_WRITE(unlock_val)` block macro. It expands to a
 ...
 * base address, so `*ra_sys_prcr() = ...` lands in observable host
...
static void test_protected_write_unlocks_then_relocks(void)
{
  ...
  {
    observed_during_body = *ra_sys_prcr();
  }
...
static void test_protected_write_relocks_on_break(void)
{
   ...
   * `RA_PROTECTED_WRITE` (or `ra_sys_prcr_lock_all()`) to relock.
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_register_protection.c\n");
  return 0;
}
