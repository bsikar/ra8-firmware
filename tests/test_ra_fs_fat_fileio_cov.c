/**
 * @file test_ra_fs_fat_fileio_cov.c
 * @brief Coverage booster for libs/ra_fs/src/ra_fs_fat_fileio.c.
 *
 * @details
 * Dedicated black-box companion executable that drives the error and edge
 * legs in ra_fs_fat_fileio.c that no sibling test yet exercises. It calls
 * only the public ra_fs API (ra_fs_read / ra_fs_write / ra_fs_write_file /
 * ra_fs_seek / ra_fs_tell / ra_fs_size), so its coverage merges into the one
 * production ra_fs_fat_fileio.gcda that gcovr aggregates.
 *
 ...
 *   - ra_fs_read               -> 188 (file not in use)
 ...
 *   - ra_fs_write              -> 419 (zero-length), 429 (dir-entry read error)
 *   - ra_fs_write_file         -> 439/442/445 (NULL args), 448 (mount not in
 *                                 use), 456 (open error), 461 (write error)
 *   - ra_fs_seek               -> 495 (file not in use)
 *   - ra_fs_tell               -> 533 (file not in use)
 *   - ra_fs_size               -> 567 (file not in use)
 *
 * The in-memory block-device harness mirrors tests/test_ra_fs_fat.c exactly.
 ...
 *          used by test_ra_fs_fat.c so shared assumptions hold: first FAT at
...
static void build_fat16_volume(void)
{
  ...
  put16(bpb, 11U, (uint16_t)k_fio_block_size);
  bpb[13] = 1U;
  put16(bpb, 14U, 1U);
  bpb[16] = 2U;
  put16(bpb, 17U, 16U);
  put16(bpb, 19U, (uint16_t)k_fio_blocks_fat16);
  put16(bpb, 22U, 32U);
  bpb[510] = 0x55U;
  bpb[511] = 0xAAU;
}
 ...
 * @since 0.1.0
 */
static void use_inject(ra_fs_mount_t* h,
                       uint32_t       reads_left,
 ...
 * @since 0.1.0
 */
static void seed_file(ra_fs_mount_t* h, const char* name, uint32_t len)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_write(f, pat, len));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));
}

/* ===========================================================================
 * ra_fs_read / seek / tell / size: file-not-in-use legs
 * ===========================================================================
 */
 ...
 *          k_ra_err_invalid_state. No compound decision under test (each guard
 *          is a single condition on the copy path).
 */
static void test_read_seek_tell_size_not_in_use(void)
{
  TEST_BEGIN("ra_fs_fileio: read/seek/tell/size not-in-use");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "NIU.TXT", k_ra_fs_mode_write, &f));
  ...
  uint8_t  buf[8]     = {};
  uint32_t got        = 99U;
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_fs_read(&closed, buf, sizeof(buf), &got)); /* 188 */
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_fs_seek(&closed, 0U));                     /* 495 */
  uint32_t pos = 99U;
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_fs_tell(&closed, &pos)); /* 533 */
  uint32_t sz = 99U;
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_fs_size(&closed, &sz)); /* 567 */

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fileio: read/seek/tell/size not-in-use");
}

/* ===========================================================================
 * ra_fs_write: zero-length early return (line 419)
 * ===========================================================================
 */
...
{
  TEST_BEGIN("ra_fs_fileio: write zero length");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "ZERO.TXT", k_ra_fs_mode_write, &f));
  uint8_t byte = (uint8_t)'0';
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_write(f, &byte, 0U)); /* 419 */
  uint32_t sz = 99U;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fileio: write zero length");
}

/* ===========================================================================
 * ra_fs_write_file: argument, state, open, and write legs
 * ===========================================================================
 */

/**
 * @test test_write_file_guards
 * @brief Drive ra_fs_write_file NULL-argument guards (439/442/445), the
...
{
  TEST_BEGIN("ra_fs_fileio: write_file guards");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_fs_write_file(nullptr, "F.TXT", data, sizeof(data)));                 /* 439 */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_fs_write_file(h, nullptr, data, sizeof(data)));    /* 442 */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_fs_write_file(h, "F.TXT", nullptr, sizeof(data))); /* 445 */

  ra_fs_mount_t closed_mount = *h;
  closed_mount.in_use        = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_state,
                 ra_fs_write_file(&closed_mount, "F.TXT", data, sizeof(data))); /* 448 */

  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_fs_write_file(h, ".TXT", data, sizeof(data))); /* 456 */

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fileio: write_file guards");
}

/**
 * @test test_write_file_write_error
 * @brief ra_fs_write_file where the internal write fails but open and close
 *        succeed (line 461).
 *
 ...
 *          the data-sector write inside ra_fs_write. ra_fs_close performs no
 *          I/O, so the write error is returned at line 461.
 */
static void test_write_file_write_error(void)
{
  TEST_BEGIN("ra_fs_fileio: write_file write error");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  const ra_fs_backend_t saved                   = h->backend;
  const uint8_t         data[k_fio_small_write] = {5U, 6U, 7U, 8U};
  use_inject(h, k_fio_reads_inf, k_fio_lba_none, h->first_data_lba, 0U);
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_write_file(h, "WERR.BIN", data, sizeof(data))); /* 461 */
  h->backend = saved;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fileio: write_file write error");
}

/* ===========================================================================
 * ra_fs_read: cluster-walk error and end-of-chain legs
 * ===========================================================================
 */
...
{
  TEST_BEGIN("ra_fs_fileio: read walk FAT-get error");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  seed_file(h, "RWALK.BIN", k_fio_two_clusters);
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "RWALK.BIN", k_ra_fs_mode_read, &f));

  const ra_fs_backend_t saved = h->backend;
  use_inject(h, k_fio_reads_inf, h->first_fat_lba, k_fio_lba_none, 0U);
  uint8_t  buf[k_fio_two_clusters] = {};
  uint32_t got                     = 99U;
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_read(f, buf, sizeof(buf), &got)); /* 59, 131 */
  h->backend = saved;

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fileio: read walk FAT-get error");
}
...
{
  TEST_BEGIN("ra_fs_fileio: read walk hits EOC");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  seed_file(h, "REOC.BIN", k_fio_two_clusters);
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "REOC.BIN", k_ra_fs_mode_read, &f));
  ...
  s_disk.bytes[fat_byte]      = 0xFFU;
  s_disk.bytes[fat_byte + 1U] = 0xFFU;

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_seek(f, (uint32_t)k_fio_block_size));
  uint8_t  buf[k_fio_block_size] = {};
  uint32_t got                   = 99U;
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_fs_read(f, buf, sizeof(buf), &got)); /* 62,63,131 */

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fileio: read walk hits EOC");
}

/* ===========================================================================
 * ra_fs_write: streaming error legs
 * ===========================================================================
 */
 ...
 *          the FAT sector. The first thing ra_fs_write does is allocate the
 *          head cluster, which scans the FAT; that read fails.
 */
static void test_write_stream_alloc_error(void)
{
  TEST_BEGIN("ra_fs_fileio: write stream alloc error");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "WALLOC.BIN", k_ra_fs_mode_write, &f));

  const ra_fs_backend_t saved                   = h->backend;
  const uint8_t         data[k_fio_small_write] = {9U, 10U, 11U, 12U};
  use_inject(h, k_fio_reads_inf, h->first_fat_lba, k_fio_lba_none, 0U);
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_write(f, data, sizeof(data))); /* 352 */
  h->backend = saved;

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fileio: write stream alloc error");
}
...
{
  TEST_BEGIN("ra_fs_fileio: write_into_sector read error");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "WRMW.BIN", k_ra_fs_mode_write, &f));

  const ra_fs_backend_t saved                   = h->backend;
  const uint8_t         data[k_fio_small_write] = {13U, 14U, 15U, 16U};
  use_inject(h, k_fio_reads_inf, h->first_data_lba, k_fio_lba_none, 0U);
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_write(f, data, sizeof(data))); /* 310 */
  h->backend = saved;

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fileio: write_into_sector read error");
}
...
{
  TEST_BEGIN("ra_fs_fileio: write_into_sector write error");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "WSEC.BIN", k_ra_fs_mode_write, &f));

  const ra_fs_backend_t saved                   = h->backend;
  const uint8_t         data[k_fio_small_write] = {17U, 18U, 19U, 20U};
  use_inject(h, k_fio_reads_inf, k_fio_lba_none, h->first_data_lba, 0U);
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_write(f, data, sizeof(data))); /* 374 */
  h->backend = saved;

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fileio: write_into_sector write error");
}

/**
 * @test test_write_dir_entry_read_error
 * @brief The directory-entry read after a successful stream write fails
 *        (ra_fs_write line 429).
 *
...
{
  TEST_BEGIN("ra_fs_fileio: write dir-entry read error");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "WDIR.BIN", k_ra_fs_mode_write, &f));

  const ra_fs_backend_t saved                   = h->backend;
  const uint8_t         data[k_fio_small_write] = {21U, 22U, 23U, 24U};
  use_inject(h, k_fio_reads_inf, f->dir_entry_lba, k_fio_lba_none, 0U);
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_write(f, data, sizeof(data))); /* 429 */
  h->backend = saved;

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fileio: write dir-entry read error");
}
...
{
  TEST_BEGIN("ra_fs_fileio: walk_grow FAT-get error");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "WGGET.BIN", k_ra_fs_mode_write, &f));

  static uint8_t        data[k_fio_two_clusters] = {};
  const ra_fs_backend_t saved                    = h->backend;
  use_inject(h, k_fio_reads_walk_get, k_fio_lba_none, k_fio_lba_none, 0U);
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_write(f, data, sizeof(data))); /* 255 */
  h->backend = saved;

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_close(f));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("ra_fs_fileio: walk_grow FAT-get error");
}
...
{
  TEST_BEGIN("ra_fs_fileio: walk_grow FAT-set error");
  build_fat16_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_backend, &h));
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_open(h, "WGSET.BIN", k_ra_fs_mode_write, &f));

  static uint8_t        data[k_fio_two_clusters] = {};
  const ra_fs_backend_t saved                    = h->backend;
  use_inject(h, k_fio_reads_walk_set, k_fio_lba_none, k_fio_lba_none, 0U);
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_write(f, data, sizeof(data))); /* 265 */
  h->backend = saved;
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_fs_fat_fileio_cov.c\n");
  return 0;
}
