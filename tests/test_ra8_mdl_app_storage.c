/**
 * @file test_ra8_mdl_app_storage.c
...
typedef union {
  ...
  uint8_t     bytes[k_app_storage_work_bytes]; /**< Opaque backend state.    */
...
static uint8_t s_app_storage_disk[(size_t)k_app_storage_disk_blocks * k_ra8_io_block_size_bytes];
static ra8_io_blockdev_ram_state_t s_app_storage_ram_state;
static ra8_io_blockdev_t           s_app_storage_blockdev;
static ra8_fs_backend_t            s_app_storage_backend;
static ra8_fs_mount_t*             s_app_storage_mount;
 ...
 * @post `s_app_storage_mount` owns the mount released by the parity test.
...
{
  TEST_ASSERT_EQ(k_ra8_ok,
                 ra8_io_blockdev_ram_init(&s_app_storage_blockdev,
                                          &s_app_storage_ram_state,
                                          s_app_storage_disk,
                                          k_app_storage_disk_blocks,
                                          false));
  TEST_ASSERT_EQ(k_ra8_ok,
                 ra8_io_blockdev_as_fs_backend(&s_app_storage_blockdev, &s_app_storage_backend));
  const ra8_fs_format_opts_t format = {.type = k_ra8_fs_type_fat12, .label = "MDLAPP"};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_format(&s_app_storage_backend, &format));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_mount(&s_app_storage_backend, &s_app_storage_mount));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_io_vfs_init());
  TEST_ASSERT_EQ(k_ra8_ok, ra8_io_vfs_mount("app", s_app_storage_mount));
  const fw_fs_ra8_vfs_cfg_t config = {.mount_name      = "app",
                                      .mount           = s_app_storage_mount,
...
RA8_INTERNAL static void internal_test_app_storage_parity(void)
{
  ...
  char posix_root[] = "/tmp/mdl_app_storage_XXXXXX";
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(s_app_storage_mount));
  s_app_storage_mount = nullptr;
...
RA8_INTERNAL static void internal_test_app_storage_faults(void)
{
  ...
  char posix_root[] = "/tmp/mdl_app_fault_XXXXXX";
  TEST_ASSERT(mkdtemp(posix_root) == posix_root);
  fw_fs_t                 inner  = {};
  ...
  TEST_ASSERT(snprintf(real_host, sizeof(real_host), "%s/real", posix_root) > 0);
  TEST_ASSERT(snprintf(link_host, sizeof(link_host), "%s/link", posix_root) > 0);
