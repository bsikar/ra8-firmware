...
/**
 * @var s_fb2
 * @brief Second render buffer for the determinism cross-check.
 * @note Single-threaded test use only.
 */
static uint16_t s_fb2[(size_t)k_fb_h * (size_t)k_fb_w];
...
static void test_shelf_render_pixels(void)
{
  ...
  const size_t c0 = row0 + (size_t)(card[0].x + (card[0].w / 2));
