/**
 * @file test_ra_doc_window.c
 * @brief Unit tests for the DOC window-comparison API.
 *
 * @details
 * Exercises ra_doc_set_window() and ra_doc_window_compare() using the
 * simulated MMIO window provided by ra_sim_mmap. Covers:
...
} ra_doc_win_test_val_t;

/**
 * @enum ra_doc_win_test_misc_t
 * @brief Miscellaneous test constants.
 */
typedef enum : uint8_t {
  k_doc_wt_bad_polarity = 2U, /**< One past k_ra_doc_window_outside: invalid. */
} ra_doc_win_test_misc_t;

/**
 * @brief Reset sim MMIO and call ra_doc_init() before each test.
 *
 * @details
 ...
 * @pre ra_mstp module is safe to call without initialisation.
...
static void test_set_window_inside_programs_regs(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_doc_set_window((uint16_t)k_doc_wt_lower, (uint16_t)k_doc_wt_upper, k_ra_doc_window_inside));
  ...
  TEST_ASSERT_EQ(0U, (reg->DOSR & (uint8_t)k_ra_doc_mask_dopcf));
...
static void test_set_window_outside_programs_regs(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_doc_set_window((uint16_t)k_doc_wt_lower, (uint16_t)k_doc_wt_upper, k_ra_doc_window_outside));
...
static void test_window_inside_mid_sets_flag(void)
{
  ...
  TEST_ASSERT_EQ(
    ...
    ra_doc_set_window((uint16_t)k_doc_wt_lower, (uint16_t)k_doc_wt_upper, k_ra_doc_window_inside));
  bool flag = false;
  TEST_ASSERT_EQ(k_ra_ok, ra_doc_window_compare((uint16_t)k_doc_wt_mid, &flag));
...
static void test_window_inside_below_no_flag(void)
{
  ...
  TEST_ASSERT_EQ(
    ...
    ra_doc_set_window((uint16_t)k_doc_wt_lower, (uint16_t)k_doc_wt_upper, k_ra_doc_window_inside));
  bool flag = true;
  TEST_ASSERT_EQ(k_ra_ok, ra_doc_window_compare((uint16_t)k_doc_wt_below, &flag));
...
static void test_window_inside_above_no_flag(void)
{
  ...
  TEST_ASSERT_EQ(
    ...
    ra_doc_set_window((uint16_t)k_doc_wt_lower, (uint16_t)k_doc_wt_upper, k_ra_doc_window_inside));
  bool flag = true;
  TEST_ASSERT_EQ(k_ra_ok, ra_doc_window_compare((uint16_t)k_doc_wt_above, &flag));
  TEST_ASSERT_EQ(false, flag);
...
static void test_window_boundary_lower_inside_no_flag(void)
{
  ...
  TEST_ASSERT_EQ(
    ...
    ra_doc_set_window((uint16_t)k_doc_wt_lower, (uint16_t)k_doc_wt_upper, k_ra_doc_window_inside));
  bool flag = true;
  TEST_ASSERT_EQ(k_ra_ok, ra_doc_window_compare((uint16_t)k_doc_wt_lower, &flag));
  TEST_ASSERT_EQ(false, flag);
...
static void test_window_boundary_upper_inside_no_flag(void)
{
  ...
  TEST_ASSERT_EQ(
    ...
    ra_doc_set_window((uint16_t)k_doc_wt_lower, (uint16_t)k_doc_wt_upper, k_ra_doc_window_inside));
  bool flag = true;
  TEST_ASSERT_EQ(k_ra_ok, ra_doc_window_compare((uint16_t)k_doc_wt_upper, &flag));
  TEST_ASSERT_EQ(false, flag);
 ...
 * rejection path in ra_doc_set_window())
 */
static void test_set_window_lower_gt_upper_rejected(void)
{
  TEST_BEGIN("doc set_window lower > upper -> k_ra_err_invalid_arg");
  prep_window();

  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_doc_set_window((uint16_t)k_doc_wt_upper, (uint16_t)k_doc_wt_lower, k_ra_doc_window_inside));
  TEST_END("doc set_window lower > upper -> k_ra_err_invalid_arg");
}
...
static void test_set_window_lower_eq_upper_rejected(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_doc_set_window((uint16_t)k_doc_wt_lower, (uint16_t)k_doc_wt_lower, k_ra_doc_window_inside));
  TEST_END("doc set_window lower == upper -> k_ra_err_invalid_arg");
}
 ...
 * out-of-range rejection path in ra_doc_set_window())
 */
static void test_set_window_bad_polarity_rejected(void)
{
  TEST_BEGIN("doc set_window bad polarity -> k_ra_err_invalid_arg");
  prep_window();

  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_doc_set_window((uint16_t)k_doc_wt_lower,
                                   (uint16_t)k_doc_wt_upper,
                                   (ra_doc_window_polarity_t)k_doc_wt_bad_polarity));
  TEST_END("doc set_window bad polarity -> k_ra_err_invalid_arg");
}

/**
 * @par MC/DC:
 * (no compound decisions in this test -- exercises the null-pointer
 * rejection path in ra_doc_window_compare())
 */
static void test_window_compare_null_flag_rejected(void)
{
  TEST_BEGIN("doc window_compare null out_flag -> k_ra_err_null_ptr");
  prep_window();

  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_doc_set_window((uint16_t)k_doc_wt_lower, (uint16_t)k_doc_wt_upper, k_ra_doc_window_inside));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_doc_window_compare((uint16_t)k_doc_wt_mid, nullptr));
  TEST_END("doc window_compare null out_flag -> k_ra_err_null_ptr");
}
...
{
  TEST_BEGIN("doc window_compare OMS!=0 -> k_ra_err_invalid_state");
  prep_window();

  /* Force OMS to 01 (add mode) to simulate a stale add16/sub16 call. */
  volatile r_doc_regs_t* reg = ra_doc();
  reg->DOCR                  = (uint8_t)k_ra_doc_mode_add;

  bool flag = false;
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_doc_window_compare((uint16_t)k_doc_wt_mid, &flag));
  TEST_END("doc window_compare OMS!=0 -> k_ra_err_invalid_state");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_doc_window.c\n");
  return 0;
}
