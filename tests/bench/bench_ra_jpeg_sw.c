...
int main(void)
{
  ra_bench_print_header("bench_ra_jpeg_sw");
  fill_gradient();

  /* Encode once (untimed) -- decode is the hot path. */
  uint32_t produced = 0U;
  ra_err_t e        = ra_jpeg_sw_encode(s_rgb_in,
  ...
  RA_BENCH_TIME("jpeg_decode_64x64_q75", produced, {
    (void)
      ra_jpeg_sw_decode(s_jpeg, produced, s_rgb_out, (uint32_t)k_bench_jpeg_rgb_bytes, &dw, &dh);
  });
  return 0;
}
