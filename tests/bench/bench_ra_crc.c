 ...
 *
 * @details
 * Drives `ra_crc_compute()` via the host-side simulator MMIO mock
...
{
  uint32_t crc = 0U;
  RA_BENCH_TIME(name, len, {
    ra_crc_reset();
    (void)ra_crc_compute(s_buf, len, &crc);
  });
...
int main(void)
{
  ra_bench_print_header("bench_ra_crc");
  fill_buf();
  if (ra_crc_init(k_ra_crc_poly_32_ieee802_3) != k_ra_ok) {
    (void)fprintf(stderr, "ra_crc_init failed\n");
    return 1;
  }
