 ...
 * drives one or more `RA_BENCH_TIME(...)` blocks. The harness
 * adaptively chooses an iteration count so each measurement runs
 * for at least `k_ra_bench_min_seconds` of wall time, then prints a
...
typedef enum : uint32_t {
  ...
  k_ra_bench_bytes_per_mb = 1000000U,
} ra_bench_limits_t;
 ...
 */
static inline uint64_t ra_bench_now_ns(void)
{
  struct timespec ts = {0, 0};
  (void)clock_gettime(CLOCK_MONOTONIC, &ts);
  return ((uint64_t)ts.tv_sec * (uint64_t)k_ra_bench_ns_per_s) + (uint64_t)ts.tv_nsec;
}

/**
 * @brief One-time CSV header. Call from main() before any benches.
 */
static inline void ra_bench_print_header(const char* exe_name)
{
  (void)fprintf(stdout, "# ra_bench: %s\n", exe_name);
 ...
 */
static inline void
ra_bench_report(const char* name, uint64_t iters, uint64_t elapsed_ns, uint64_t bytes_per_iter)
{
  ...
  if ((bytes_per_iter > 0U) && (elapsed_ns > 0U)) {
    ...
    double seconds     = (double)elapsed_ns / (double)k_ra_bench_ns_per_s;
    mb_per_s           = (total_bytes / seconds) / (double)k_ra_bench_bytes_per_mb;
  }
  (void)
 ...
 * `k_ra_bench_min_us`, then reports the final measurement.
 *
 ...
 */
#define RA_BENCH_TIME(name_str, bytes_iter, body)                                                  \
  do {                                                                                             \
    uint64_t _iters = (uint64_t)k_ra_bench_min_iters;                                              \
    uint64_t _ns    = 0U;                                                                          \
    while (_iters <= (uint64_t)k_ra_bench_max_iters) {                                             \
      uint64_t _t0 = ra_bench_now_ns();                                                            \
      for (uint64_t _i = 0U; _i < _iters; _i++) {                                                  \
        body                                                                                       \
      }                                                                                            \
      uint64_t _t1 = ra_bench_now_ns();                                                            \
      _ns          = _t1 - _t0;                                                                    \
      uint64_t _us = _ns / (uint64_t)k_ra_bench_ns_per_us;                                         \
      if (_us >= (uint64_t)k_ra_bench_min_us) {                                                    \
        break;                                                                                     \
      }                                                                                            \
      _iters *= 2U;                                                                                \
    }                                                                                              \
    ra_bench_report((name_str), _iters, _ns, (uint64_t)(bytes_iter));                              \
  } while (0)
