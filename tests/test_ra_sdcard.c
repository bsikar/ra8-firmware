 ...
 * SD_INFO1 is asserted by the ra_sim_mmio poll-hook (no timer, no thread).
 *
 ...
 * (installed via ra_sim_mmio_set_poll_hook) inspects the driver's
 ...
 * command index), so it does nothing until ra_sdhi_send_command writes the next
 ...
 * ra_sim_mmio_set_poll_hook). It holds SD_INFO2.BRE/BWE asserted for the block
...
{
  s_srv_inst = inst;
  ra_sim_mmio_set_poll_hook(sdcard_hook);
}
...
static void test_init_full_sequence(void)
{
  ...
  sdcard_hook_arm((uint8_t)k_ra_sdcard_test_inst);
