/**
 * @file test_ra_dual_core_cov.c
 * @brief Coverage-lift tests for libs/ra_hal/src/ra_dual_core.c
 *
 * @par Tag
 * [Test / Host] {World: N/A}
 *
 * @details
 * Exercises the one branch in ra_cpu1_is_running() not reached by the sibling
 ...
 *
 * Root cause of the gap: test_app_cpu1_pingpong calls ra_cpu1_is_running()
 * only after at least one successful ra_cpu1_release(), which sets ACT=1 in
 ...
 * context.
 *
 * MC/DC coverage summary for ra_cpu1_is_running() (2 sequential conditions):
 *
 ...
 *
 * @details
 * Calls ra_sim_mmap_reset() to zero the peripheral-window RAM.  The
 * module-internal s_sim struct is file-static to ra_dual_core.c and is
 ...
 * ra_cpu1_release() can set the ACT bit.
 *
 * @pre None.
 ...
 *
 * @brief ra_cpu1_is_running() returns false when CPU1 has never been activated.
 *
 * @details
 ...
 * before any ra_cpu1_release() call; subsequent tests in this file may alter
 * s_sim.actcsr.
 *
 * @par MC/DC:
 ...
 *
 * @pre s_sim.actcsr == 0 (fresh process, no prior ra_cpu1_release call).
 * @pre ra_sim_mmap_reset() has not altered s_sim (it does not -- s_sim is
 *      private to ra_dual_core.c).
 * @post ra_cpu1_is_running() returned false.
 * @post s_sim is unchanged.
 * @note Must execute before any ra_cpu1_release() in this process.
 * @since 0.1.0
 */
static void test_dc_cov_is_running_before_release(void)
{
  reset_mmio();
  TEST_BEGIN("ra_dual_core_cov: is_running returns false before any release (ACT=0)");
  /* s_sim.actcsr starts at 0: ACT bit is clear -> active=false -> early return false. */
  TEST_ASSERT_EQ(0, ra_cpu1_is_running());
  TEST_END("ra_dual_core_cov: is_running returns false before any release (ACT=0)");
}

/**
 * @test test_dc_cov_release_sets_cpu1_active
 *
 * @brief ra_cpu1_release() with valid args succeeds and CPU1 becomes running.
 *
 * @details
 ...
 * ACTREQ write, and the poll returns k_ra_ok on its first iteration.
 * After release, ra_cpu1_is_running() must return true (ACT=1, CPUWAIT=0).
 *
 * @par MC/DC:
 * ra_cpu1_release() contains no compound boolean decisions (all conditions
 * are on separate ifs).  Each branch is covered by a single input vector.
 *
 * @pre ra_cpu1_is_running() was already exercised with ACT=0 (test above).
 * @pre ra_sim_mmap_reset() called in reset_mmio().
 * @post s_sim.actcsr has the ACT bit set.
 * @post ra_cpu1_is_running() returns true.
...
{
  reset_mmio();
  TEST_BEGIN("ra_dual_core_cov: release with valid args -> is_running true");
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_cpu1_release((void*)(uintptr_t)k_dc_cov_dummy_entry, (void*)(uintptr_t)k_dc_cov_dummy_sp));
  TEST_ASSERT_EQ(1, ra_cpu1_is_running());
  TEST_END("ra_dual_core_cov: release with valid args -> is_running true");
}

/**
 * @test test_dc_cov_halt_stops_cpu1
 *
 * @brief ra_cpu1_halt() stalls CPU1 and ra_cpu1_is_running() returns false.
 *
 * @details
 * After a successful release (previous test), CPUWAIT=0.  Halt writes
 * CPUWAIT=1.  ra_cpu1_is_running() then sees ACT=1 (still set) and
 * CPUWAIT=1 (stalled) -> returns !stalled = false.
 *
 * @par MC/DC:
 * ra_cpu1_halt() has no compound boolean decisions.  The single call
...
{
  reset_mmio();
  TEST_BEGIN("ra_dual_core_cov: halt -> is_running false (active but stalled)");
  TEST_ASSERT_EQ(k_ra_ok, ra_cpu1_halt());
  TEST_ASSERT_EQ(0, ra_cpu1_is_running());
  TEST_END("ra_dual_core_cov: halt -> is_running false (active but stalled)");
}

/**
 * @test test_dc_cov_release_null_entry
 *
 * @brief ra_cpu1_release() rejects a NULL entry pointer.
 *
 * @details
 * Validates the first null-pointer guard: entry==nullptr, sp valid ->
 * returns k_ra_err_null_ptr without touching any CPU_CTRL registers.
 *
 * @par MC/DC:
...
{
  reset_mmio();
  TEST_BEGIN("ra_dual_core_cov: release rejects NULL entry");
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_cpu1_release(nullptr, (void*)(uintptr_t)k_dc_cov_dummy_sp));
  TEST_END("ra_dual_core_cov: release rejects NULL entry");
}

/**
 * @test test_dc_cov_release_misaligned_entry
 *
 * @brief ra_cpu1_release() rejects an entry address that is not 128-byte aligned.
 *
 * @details
...
{
  reset_mmio();
  TEST_BEGIN("ra_dual_core_cov: release rejects mis-aligned entry");
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_cpu1_release((void*)((uintptr_t)k_dc_cov_dummy_entry + 1U),
                                 (void*)(uintptr_t)k_dc_cov_dummy_sp));
  TEST_END("ra_dual_core_cov: release rejects mis-aligned entry");
}

/**
 * @test test_dc_cov_release_misaligned_sp
 *
 * @brief ra_cpu1_release() rejects an sp address that is not 8-byte aligned.
 *
 * @details
...
{
  reset_mmio();
  TEST_BEGIN("ra_dual_core_cov: release rejects mis-aligned sp");
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_cpu1_release((void*)(uintptr_t)k_dc_cov_dummy_entry,
                                 (void*)((uintptr_t)k_dc_cov_dummy_sp + 1U)));
  TEST_END("ra_dual_core_cov: release rejects mis-aligned sp");
}
 ...
 * Subsequent tests may call ra_cpu1_release() which sets the ACT bit.
 *
