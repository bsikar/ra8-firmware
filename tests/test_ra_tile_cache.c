/**
 * @file test_ra_tile_cache.c
 * @brief Unit tests for the ra_mem image-tile cache (Layer 3b, #147).
 *
 * @details
 * Mirrors the glyph-atlas tests for the second ::ra_keycache facade: decode-on-
...
static ra_tile_key_t      s_keys[(size_t)k_t_cells];
static ra_tile_dims_t     s_dims[(size_t)k_t_cells];
...
static ra_err_t t_decode(void*                ctx,
                         const ra_tile_key_t* key,
                         uint8_t*             cell,
                         uint32_t             cell_bytes,
                         uint16_t*            out_w,
                         uint16_t*            out_h)
...
static ra_err_t t_decode_fail(void*                ctx,
                              const ra_tile_key_t* key,
                              uint8_t*             cell,
                              uint32_t             cell_bytes,
                              uint16_t*            out_w,
...
static ra_tile_cache_cfg_t t_cfg(void)
{
  ra_tile_cache_cfg_t cfg = {};
...
/** @brief A tile key. */
static ra_tile_key_t t_key(uint32_t image, uint16_t tx, uint16_t ty, uint16_t zoom)
{
  ra_tile_key_t k = {};
  k.image_id      = image;
  k.tile_x        = tx;
...
static void test_decode_hit(void)
{
  ...
  s_decode_calls          = 0;
  ra_tile_cache_t     tc  = {};
  ra_tile_cache_cfg_t cfg = t_cfg();
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_init(&tc, &cfg));

  ra_tile_key_t k = t_key(0x42U, 1U, 2U, 0U);
  ra_tile_t     t = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_get(&tc, &k, &t)); /* miss -> decode */
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_put(&tc, t.pixels));

  ra_tile_t t2 = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_get(&tc, &k, &t2)); /* hit -> no decode */
  TEST_ASSERT(t2.pixels == t.pixels);
  TEST_ASSERT_EQ(1, s_decode_calls); /* unchanged */
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_put(&tc, t2.pixels));

  /* An edge tile reports its smaller decoded dimensions. */
  ra_tile_key_t ke = t_key(0x42U, (uint16_t)k_t_edge_x, 0U, 0U);
  ra_tile_t     te = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_get(&tc, &ke, &te));
  TEST_ASSERT_EQ(k_t_edge_dim, te.width);
  TEST_ASSERT_EQ(k_t_edge_dim, te.height);
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_put(&tc, te.pixels));

  uint32_t hits = 0;
  uint32_t miss = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_stats(&tc, &hits, &miss, nullptr));
...
static void test_eviction(void)
{
  ...
  for (uint32_t i = 0; i < 6U; ++i) {
    ra_tile_key_t k = t_key(1U, (uint16_t)i, 0U, 0U);
    ra_tile_t     t = {};
    TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_get(&tc, &k, &t));
    TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_put(&tc, t.pixels));
  }
  uint32_t ev = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_stats(&tc, nullptr, nullptr, &ev));
  ...
  ra_tile_key_t k0     = t_key(1U, 0U, 0U, 0U);
  ra_tile_t     t0     = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_get(&tc, &k0, &t0)); /* evicted -> re-decode */
  TEST_ASSERT_EQ(before + 1U, s_decode_calls);
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_put(&tc, t0.pixels));
...
static void test_pin_protection(void)
{
  ...
  for (uint32_t i = 0; i < (uint32_t)k_t_cells; ++i) {
    ra_tile_key_t k = t_key(2U, (uint16_t)i, 1U, 0U);
    ra_tile_t     t = {};
    TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_get(&tc, &k, &t)); /* pinned (no put) */
    pins[i] = t.pixels;
  }
  ra_tile_key_t kn = t_key(2U, 99U, 1U, 0U);
  ra_tile_t     tn = {};
  TEST_ASSERT_EQ(k_ra_err_no_mem, ra_tile_cache_get(&tc, &kn, &tn)); /* all pinned */
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_put(&tc, pins[0]));
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_get(&tc, &kn, &tn)); /* now evicts + decodes */
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_put(&tc, tn.pixels));
  for (uint32_t i = 1; i < (uint32_t)k_t_cells; ++i) {
    TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_put(&tc, pins[i]));
  }
...
static void test_decode_failure(void)
{
  ...
  ra_tile_key_t k = t_key(1U, 1U, 1U, 1U);
  ra_tile_t     t = {};
  TEST_ASSERT_EQ(k_ra_err_timeout, ra_tile_cache_get(&tc, &k, &t)); /* decode fails */

  uint32_t miss = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_stats(&tc, nullptr, &miss, nullptr));
...
static void test_validation(void)
{
  ...
  ra_tile_cache_t     tc  = {};
  ra_tile_cache_cfg_t cfg = t_cfg();
  ra_tile_key_t       k   = t_key(1U, 1U, 1U, 0U);
  ra_tile_t           t   = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_tile_cache_init(nullptr, &cfg));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_tile_cache_init(&tc, nullptr));
  ra_tile_cache_cfg_t bad = t_cfg();
  bad.decode              = nullptr;
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_tile_cache_init(&tc, &bad));
  bad            = t_cfg();
  bad.cell_count = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_size, ra_tile_cache_init(&tc, &bad));

  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_init(&tc, &cfg));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_tile_cache_get(nullptr, &k, &t));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_tile_cache_get(&tc, nullptr, &t));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_tile_cache_get(&tc, &k, nullptr));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_tile_cache_put(&tc, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_tile_cache_put(&tc, &s_cells[1])); /* mid-cell */
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_get(&tc, &k, &t));
  TEST_ASSERT_EQ(k_ra_ok, ra_tile_cache_put(&tc, t.pixels));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_tile_cache_put(&tc, t.pixels)); /* already unpinned */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_tile_cache_stats(nullptr, nullptr, nullptr, nullptr));
...
int32_t main(void)
{
  ...
  test_decode_failure();
  test_validation();
  (void)fprintf(stderr, "[OK  ] test_ra_tile_cache.c\n");
  return 0;
}
