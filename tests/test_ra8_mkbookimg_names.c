 ...
 * (`mkbookimg_dest_name`, shared through `tools/mkbookimg/mkbookimg_names.h`)
...
#include "../tools/mkbookimg/mkbookimg_names.h"
#include "ra8_err.h"
#include "ra8_fs.h"
#include "ra8_log.h"
...
static ra8_err_t mem_write(void* ctx, uint64_t lba, uint32_t count, const uint8_t* buf)
{
  ...
  memcpy(&disk->bytes[(size_t)lba * (size_t)k_disk_block_size],
         buf,
         (size_t)count * (size_t)k_disk_block_size);
  return k_ra8_ok;
}
...
static void assert_name_roundtrips(ra8_fs_mount_t* mount, const char* name, const char* body)
{
  ...
  TEST_ASSERT_EQ(0, memcmp(got_buf, body, (size_t)len));
