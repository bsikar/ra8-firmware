/**
 * @file test_ra_usb_hmsc_enum_cov.c
 * @brief White-box line-coverage tests for the polled host-MSC enumeration
 *        ladder (`ra_usb_hmsc_enum.c`).
 *
 * @details
 * The enumeration ladder in `ra_usb_hmsc_enum.c` walks a device from attach
 ...
 * `ra_usb_host_control_xfer`, whose success depends on real SIE handshakes
 ...
 * transfer driven through the public `ra_usb_hmsc_enumerate` can only ever
 * reach the timeout legs (already exercised by `test_ra_usb_hmsc.c`).
 *
 ...
 * exported symbol (`ra_usb_hmsc_enumerate`) is renamed to
 * `ra_usb_hmsc_enumerate_cov` so it does not collide with the production
 * copy linked from `ra_core_hal`. The mocks feed realistic USB descriptor
 ...
 * production definition in `ra_usb_hmsc.c` (linked from `ra_core_hal`), so
...
static ra_err_t mock_ctrl_xfer(ra_usb_speed_t        speed,
                               ...
                               uint16_t*             out_received)
{
  ...
  if (req == 0x05U) { /* SET_ADDRESS */
    return s_setaddr_err;
  }
  if (req == 0x09U) { /* SET_CONFIGURATION */
    return s_setcfg_err;
  }
  if (req == 0xFEU) { /* GET_MAX_LUN */
...
static uint16_t mock_line_state(ra_usb_speed_t speed)
{
  (void)speed;
  return s_line_state;
}

static ra_err_t mock_bus_reset(ra_usb_speed_t speed, bool assert_reset)
{
  (void)speed;
  (void)assert_reset;
  return k_ra_ok;
...
static ra_err_t mock_set_uact(ra_usb_speed_t speed, bool enable)
{
  (void)speed;
  (void)enable;
  return k_ra_ok;
...
static ra_err_t mock_set_target(ra_usb_speed_t speed, uint8_t dev_addr)
{
  (void)speed;
  (void)dev_addr;
  return k_ra_ok;
...
static ra_err_t mock_pipe_setup(ra_usb_speed_t speed,
                                uint8_t        pipe_num,
                                uint8_t        dev_addr,
                                uint8_t        ep_num,
                                bool           device_to_host,
...
static void cov_on_attach(void* ctx, const ra_usb_hmsc_device_t* device)
{
  (void)device;
  ++s_attach_count;
...
static uint16_t build_msc_config(uint8_t* buf)
{
  ...
  buf[o + 5U] = (uint8_t)k_tc_cfg_value;
  buf[o + 6U] = 0U;
  buf[o + 7U] = 0U;
  buf[o + 8U] = 0U;
  ...
  buf[o + 5U] = (uint8_t)k_tc_class_msc;
  buf[o + 6U] = (uint8_t)k_tc_subclass_scsi;
  buf[o + 7U] = (uint8_t)k_tc_protocol_bbb;
  buf[o + 8U] = 0U;
  ...
  buf[o + 6U]         = 0U;
  o                   = (uint16_t)(o + k_tc_ep_desc_len);
  buf[k_tc_off_total] = (uint8_t)(o & 0xFFU);
  return o;
}
...
static void reset_state(void)
{
  ...
  s_usb_hmsc_state.speed       = k_ra_usb_speed_fs;
  ...
  s_dev_desc[k_tc_off_vid]      = (uint8_t)((uint16_t)k_tc_vid & 0xFFU);
  s_dev_desc[k_tc_off_vid + 1U] = (uint8_t)(((uint16_t)k_tc_vid >> k_tc_byte_bits) & 0xFFU);
  s_dev_desc[k_tc_off_pid]      = (uint8_t)((uint16_t)k_tc_pid & 0xFFU);
  s_dev_desc[k_tc_off_pid + 1U] = (uint8_t)(((uint16_t)k_tc_pid >> k_tc_byte_bits) & 0xFFU);
  ...
  s_dev_err     = k_ra_ok;
  s_cfg_err     = k_ra_ok;
  s_setaddr_err = k_ra_ok;
  s_setcfg_err  = k_ra_ok;
  s_lun_err     = k_ra_ok;
  s_lun_rx      = 1U;
  s_lun_val     = 0U;
  s_line_state  = 1U;
 ...
 * the production `ra_usb_hmsc_enumerate` linked from ra_core_hal, and redirect
 * the hardware-transport calls to the deterministic mocks above. */
ra_err_t ra_usb_hmsc_enumerate_cov(ra_usb_hmsc_device_t* out_device);

#define ra_usb_hmsc_enumerate    ra_usb_hmsc_enumerate_cov
#define ra_usb_host_control_xfer mock_ctrl_xfer
#define ra_usb_host_line_state   mock_line_state
#define ra_usb_host_bus_reset    mock_bus_reset
#define ra_usb_host_set_uact     mock_set_uact
#define ra_usb_host_set_target   mock_set_target
#define ra_usb_host_pipe_setup   mock_pipe_setup
#define ra_delay_ms              mock_delay_ms
#define ra_time_ms               mock_time_ms

#include "ra_usb_hmsc_enum.c" // NOLINT(bugprone-suspicious-include) -- white-box copy
...
static void test_read_dev_desc_short_and_full(void)
{
  ...
  /* Full read: 18 bytes -> k_ra_ok. */
  s_dev_rx = (uint16_t)k_tc_dev_desc_len;
  TEST_ASSERT_EQ(k_ra_ok, internal_enum_read_dev_desc(desc));

  /* Short read: fewer than 18 bytes -> hw_error. */
  s_dev_rx = (uint16_t)(k_tc_dev_desc_len - 1U);
  TEST_ASSERT_EQ(k_ra_err_hw_error, internal_enum_read_dev_desc(desc));
...
static void test_hunt_success_and_attach_timeout(void)
{
  ...
  uint8_t addr                    = 0xFFU;
  s_line_state                    = 1U;
  TEST_ASSERT_EQ(k_ra_ok, internal_enum_hunt(desc, &addr));
  ...
  s_time_step   = 1500U;               /* 0 -> 1500 (<=2000) -> 3000 (>2000, break). */
  s_dev_err     = k_ra_err_hw_timeout; /* No descriptor ever answers.                */
  uint8_t addr2 = 0xFFU;
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, internal_enum_hunt(desc, &addr2));
...
static void test_assign_addr_both_legs(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, internal_enum_assign_addr(&addr_nonzero));
  TEST_ASSERT_EQ(2U, addr_nonzero);

  /* Default address (0) -> SET_ADDRESS + retarget -> operating address 1. */
  uint8_t addr_zero = 0U;
  TEST_ASSERT_EQ(k_ra_ok, internal_enum_assign_addr(&addr_zero));
...
static void test_iface_is_msc_all_fields(void)
{
  ...
  iface[5] = (uint8_t)k_tc_class_hid; /* wrong class -> false. */
  TEST_ASSERT(!internal_enum_iface_is_msc(iface));

  iface[5] = (uint8_t)k_tc_class_msc;
  iface[6] = (uint8_t)k_tc_sub_rbc; /* wrong subclass -> false. */
  TEST_ASSERT(!internal_enum_iface_is_msc(iface));

  iface[6] = (uint8_t)k_tc_subclass_scsi;
  iface[7] = (uint8_t)k_tc_proto_cbi; /* wrong protocol -> false. */
  TEST_ASSERT(!internal_enum_iface_is_msc(iface));
...
static void test_walk_cfg_success_and_errors(void)
{
  ...
  blob[o + 5U] = (uint8_t)k_tc_class_hid;
  o            = (uint16_t)(o + k_tc_iface_desc_len);
  ...
  blob[o + 0U] = (uint8_t)k_tc_iface_desc_len;
  blob[o + 1U] = (uint8_t)k_tc_dtype_iface;
  blob[o + 5U] = (uint8_t)k_tc_class_msc;
  blob[o + 6U] = (uint8_t)k_tc_subclass_scsi;
  blob[o + 7U] = (uint8_t)k_tc_protocol_bbb;
  o            = (uint16_t)(o + k_tc_iface_desc_len);
  blob[o + 0U] = (uint8_t)k_tc_ep_desc_len;
  blob[o + 1U] = (uint8_t)k_tc_dtype_endpoint;
  blob[o + 2U] = (uint8_t)k_tc_ep_in_addr;
  ...
  blob[o + 4U] = (uint8_t)k_tc_mps_lo;
  o            = (uint16_t)(o + k_tc_ep_desc_len);
  TEST_ASSERT_EQ(k_ra_ok, internal_enum_walk_cfg(blob, o));
  TEST_ASSERT_EQ(1U, s_usb_hmsc_state.device.bulk_in_ep);
  TEST_ASSERT_EQ(2U, s_usb_hmsc_state.device.bulk_out_ep);
  ...
  in_only[p + 5U]         = (uint8_t)k_tc_class_msc;
  in_only[p + 6U]         = (uint8_t)k_tc_subclass_scsi;
  in_only[p + 7U]         = (uint8_t)k_tc_protocol_bbb;
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_error, internal_enum_walk_cfg(in_only, p));
  ...
  out_only[q + 5U]        = (uint8_t)k_tc_class_msc;
  out_only[q + 6U]        = (uint8_t)k_tc_subclass_scsi;
  out_only[q + 7U]        = (uint8_t)k_tc_protocol_bbb;
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_error, internal_enum_walk_cfg(out_only, q));

  /* A zero-length descriptor breaks the walk before any endpoint is found. */
  s_usb_hmsc_state.device = (ra_usb_hmsc_device_t){};
  uint8_t zero_len[16]    = {};
  zero_len[0]             = 0U; /* bLength == 0 -> break. */
  TEST_ASSERT_EQ(k_ra_err_hw_error, internal_enum_walk_cfg(zero_len, (uint16_t)sizeof zero_len));
 ...
 *
 * @note End-to-end run of the renamed `ra_usb_hmsc_enumerate_cov`: the mocks
...
static void test_enumerate_full_success(void)
{
  ...
  ra_usb_hmsc_device_t dev = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_hmsc_enumerate_cov(&dev));
  TEST_ASSERT(s_usb_hmsc_state.attached);
  TEST_ASSERT_EQ(1U, s_attach_count);
 ...
 * timeout out of `ra_usb_hmsc_enumerate_cov` without firing the callback.
 */
static void test_enumerate_ladder_failure_and_guard(void)
{
  ...
  ra_usb_hmsc_device_t dev     = {};
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_hmsc_enumerate_cov(&dev));

  /* Ladder failure: no descriptor ever answers -> hunt timeout out the top. */
  reset_state();
  s_usb_hmsc_state.attach_cb = cov_on_attach;
  s_line_state               = 1U;
  s_dev_err                  = k_ra_err_hw_timeout;
  TEST_ASSERT(ra_usb_hmsc_enumerate_cov(&dev) != k_ra_ok);
  TEST_ASSERT_EQ(0U, s_attach_count);
  TEST_ASSERT(!s_usb_hmsc_state.attached);
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_usb_hmsc_enum_cov.c\n");
  return 0;
}
