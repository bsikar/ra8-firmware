/**
 * @file test_ra_display_pal_policy.c
 * @brief Unit tests for the e-ink page-turn refresh-cadence policy.
 *
 * @details
 * Pure-logic coverage of ra_display_pal_policy: init validation + clamping, the
...
static void test_policy_init_validates(void)
{
  ...
  display_policy_t p = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, display_policy_init(nullptr, k_display_policy_fast_clean, 8U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, display_policy_init(&p, (display_policy_kind_t)99U, 8U));
...
static void test_policy_decide_validates(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, display_policy_init(&p, k_display_policy_fast_clean, 8U));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, display_policy_decide(nullptr, k_display_event_turn, &d));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, display_policy_decide(&p, k_display_event_turn, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, display_policy_decide(&p, (display_turn_event_t)9U, &d));
...
static void test_policy_open_is_init(void)
{
  ...
  for (uint32_t i = 0U; i < 3U; i++) {
    display_policy_t          p = {};
    display_policy_decision_t d = {};
    TEST_ASSERT_EQ(k_ra_ok, display_policy_init(&p, kinds[i], k_test_clean_every));
    p.turns_since_clean = 3U; /* dirty -> open must reset it. */
    TEST_ASSERT_EQ(k_ra_ok, display_policy_decide(&p, k_display_event_open, &d));
...
static void test_policy_fast_only(void)
{
  ...
  display_policy_t          p = {};
  display_policy_decision_t d = {};
  TEST_ASSERT_EQ(k_ra_ok, display_policy_init(&p, k_display_policy_fast_only, k_test_clean_every));
  for (uint32_t i = 0U; i < 10U; i++) {
    TEST_ASSERT_EQ(k_ra_ok, display_policy_decide(&p, k_display_event_turn, &d));
    TEST_ASSERT_EQ(k_display_refresh_fast, d.hint);
    TEST_ASSERT(!d.full_update);
  }
  TEST_ASSERT_EQ(k_ra_ok, display_policy_decide(&p, k_display_event_chapter, &d));
...
static void test_policy_quality(void)
{
  ...
  display_policy_t          p = {};
  display_policy_decision_t d = {};
  TEST_ASSERT_EQ(k_ra_ok, display_policy_init(&p, k_display_policy_quality, k_test_clean_every));
...
static void test_policy_fast_clean_cadence(void)
{
  ...
  display_policy_t          p = {};
  display_policy_decision_t d = {};
  TEST_ASSERT_EQ(k_ra_ok, display_policy_init(&p, k_display_policy_fast_clean, k_test_clean_every));
 ...
 * (2 conditions), in priv_decide_fast_clean (libs/ra_display_pal/src/ra_display_pal_policy.c).
 * With clean_every = 4:
...
static void test_policy_full_rect(void)
{
  ...
  display_rect_t r = {};
  TEST_ASSERT_EQ(k_ra_err_null_ptr, display_policy_full_rect(100U, 50U, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, display_policy_full_rect(0U, 50U, &r));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, display_policy_full_rect(100U, 0U, &r));
  TEST_ASSERT_EQ(k_ra_ok, display_policy_full_rect(100U, 50U, &r));
