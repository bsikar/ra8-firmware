/**
 * @file test_app_ra_sdhi_card_demo.c
 * @brief Integration test: ra_sdhi_card_demo raw-block round-trip logic (pure)
 *
 * @details
 * Mirrors the app-level pure logic in
 * `examples/ek_ra8d2/hw_pending/ra_sdhi_card_demo/main.c`. The SDHI command
 * sequencing is owned + covered by `ra_sdcard` / `ra_sdhi` unit tests; this
 ...
 * they run in-process without any ra_sim_mmap.
 *
 ...
 * i=511).
 */
static void test_ra_sdhi_card_fill_pattern(void)
{
  TEST_BEGIN("ra_sdhi_card_demo: LCG payload pattern");
  ...
  TEST_END("ra_sdhi_card_demo: LCG payload pattern");
}
 ...
 */
static void test_ra_sdhi_card_roundtrip_verdict(void)
{
  TEST_BEGIN("ra_sdhi_card_demo: round-trip verdict MC/DC");
  ...
  TEST_END("ra_sdhi_card_demo: round-trip verdict MC/DC");
}

int main(void)
{
  test_ra_sdhi_card_fill_pattern();
  test_ra_sdhi_card_roundtrip_verdict();
  return 0;
}
