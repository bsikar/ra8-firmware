 ...
 */
static ra8_err_t
mock_mdio_read(ra8_rmac_port_t port, uint8_t phy_addr, uint8_t reg_addr, uint16_t* out)
{
  (void)port;
  (void)phy_addr;
  if (out == nullptr) {
 ...
 *                 failing leg.
 * @return k_ra8_ok, or ::s_etha_fail_err on the armed call index.
 */
static ra8_err_t mock_etha_set_mode(ra8_etha_port_t port, ra8_etha_opc_t mode)
{
  (void)port;
  (void)mode;
  ++s_etha_call_count;
 ...
 * @return ::s_setlink_err (k_ra8_ok unless a test forces rejection).
 */
static ra8_err_t mock_rmac_set_link(ra8_rmac_port_t   port,
