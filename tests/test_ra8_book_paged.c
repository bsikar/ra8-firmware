 ...
 * @pre All pointers are non-NULL; @p len > 0 and >= sizeof(ra8_book_header_t).
 ...
 */
static void pbook_bind_paged(ra8_book_src_t*    psrc,
                             ra8_vsource_t*     vs,
...
static void test_ra8_book_paged_matches_resident(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 ra8_vsource_add_paged(&vs, pbook_read, nullptr, 0U, (uint64_t)sizeof(book), &oid));
   ...
   *  V1 (both false): the successful bind above -> k_ra8_ok.
   *  V2 (size == 0, frame_bytes ok): C1 true  -> k_ra8_err_invalid_size.
   *  V3 (size ok, frame_bytes < 2):  C2 true  -> k_ra8_err_invalid_size. */
  ra8_book_src_t bad = {};
  TEST_ASSERT_EQ(
    k_ra8_err_null_ptr,
    ra8_book_src_paged(nullptr, &vm, oid, (uint32_t)k_pb_frame_bytes, (uint32_t)sizeof(book)));
  TEST_ASSERT_EQ(
    k_ra8_err_null_ptr,
    ra8_book_src_paged(&bad, nullptr, oid, (uint32_t)k_pb_frame_bytes, (uint32_t)sizeof(book)));
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 ra8_book_src_paged(&bad, &vm, oid, (uint32_t)k_pb_frame_bytes, 0U)); /* V2 */
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 ra8_book_src_paged(&bad, &vm, oid, 1U, (uint32_t)sizeof(book))); /* V3 */
...
static void test_ra8_book_paged_long_run(void)
{
  ...
  uint32_t oid = 0U;
  TEST_ASSERT_EQ(k_ra8_ok,
                 ra8_vsource_add_paged(&vs, pbook_read, nullptr, 0U, (uint64_t)sizeof(b), &oid));
  ra8_vmem_cfg_t cfg = {
    .frame_mem    = s_pb_frames,
    .frame_bytes  = (uint32_t)k_pb_frame_bytes,
    .frame_count  = (uint32_t)k_pb_frame_count,
...
static uint64_t pbc_pack(uint8_t* out, const uint8_t* blob, uint32_t blob_len, uint32_t chunk_bytes)
{
  ...
  memcpy(&out[k_ra8_book_container_header_len + ((size_t)count * k_ra8_book_container_entry_len)],
         &off,
         sizeof(off));
 ...
 *        ra8_vsource -> ra8_vmem -> ra8_book_src_paged -- matches resident text.
 *
 * @par Coverage:
 ...
 */
static void test_ra8_book_paged_chunked_backing(void)
{
  ...
  TEST_END("ra8_book paged over chunked container == resident");
}
...
typedef struct {
  ...
  ra8_book_chapter_t chapters[1]; /**< One chapter rooted at node 0.  */
  ra8_book_node_t    nodes[1];    /**< One self-cyclic <div> element. */
...
{
  ra8_book_header_t* h    = (ra8_book_header_t*)blob;
  const uint8_t*     body = (const uint8_t*)blob + sizeof(ra8_book_header_t);
  h->crc32                = pbook_crc32(body, total - (uint32_t)sizeof(ra8_book_header_t));
}

/** @brief Fill the common header fields shared by the edge fixtures. */
static void pbook_edge_header(ra8_book_header_t* h,
                              uint32_t           total,
                              ...
                              uint32_t           string_size)
{
  ...
  h->format_version    = k_ra8_book_format_version;
  h->total_size        = total;
  h->chapter_count     = 1U;
  ...
  h->cover_image_index = k_ra8_book_nil;
}

/**
 * @test test_ra8_book_paged_cyclic_walk_guard
 ...
 * (valid header + trailer CRC); ra8_book_validate does not check DOM acyclicity,
 ...
 * acyclic walk in test_ra8_book_paged_matches_resident; the ok-false legs by the
 * overflow tests. Together they complete the N+1 vector sets.
 */
static void test_ra8_book_paged_cyclic_walk_guard(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 ra8_book_chapter_text_src(&psrc, 0U, out, sizeof out, &len));

  TEST_END("ra8_book paged cyclic blob drained by the walk guard");
}

/** @brief span > (text, block-p) blob used to overflow emit_break on the block. */
typedef struct {
  ra8_book_header_t  hdr;         /**< Container header.                      */
  ra8_book_chapter_t chapters[1]; /**< One chapter rooted at the inline span. */
  ra8_book_node_t    nodes[3];    /**< span, text, block <p>.                 */
  char               strings[32]; /**< "", "span", "p", "AB".                 */
} pbook_ovf_t;

/**
 * @test test_ra8_book_paged_block_break_overflow
 ...
 */
static void test_ra8_book_paged_block_break_overflow(void)
{
  ...
  TEST_END("ra8_book paged block break overflow reports failure");
}
...
typedef struct {
  ra8_book_header_t  hdr;                    /**< Container header.              */
  ra8_book_chapter_t chapters[1];            /**< One chapter rooted at node 0.  */
  ra8_book_node_t    nodes[2];               /**< long-tag element + text child. */
  char               strings[k_pbe_str_cap]; /**< "" then the 70-char tag name.  */
} pbook_longtag_t;

/**
 * @test test_ra8_book_paged_long_tag_name
 ...
 * test_ra8_book_paged_matches_resident. N+1 vectors complete.
 */
static void test_ra8_book_paged_long_tag_name(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_book_chapter_text_src(&psrc, 0U, out, sizeof out, &len));
  TEST_ASSERT(len >= 2U);

  TEST_END("ra8_book paged over-long tag name truncates");
}
 ...
 *          backing is armed -- reaching ra8_book_src_prefetch_chapter's record-read
 *          error return without needing to walk any DOM.
 */
typedef struct {
  ra8_book_header_t  hdr;                   /**< Container header (frames 0..1).      */
  uint8_t            gap[k_pb_frame_bytes]; /**< Pad chapters past the header frames. */
  ra8_book_chapter_t chapters[1];           /**< One chapter rooted at node 0.        */
  ra8_book_node_t    nodes[2];              /**< div element + text child.            */
  char               strings[16];           /**< "" then "div".                       */
} pbook_farchap_t;
 ...
 *        ra8_book_src_prefetch_chapter() verbatim.
 *
 * @par Coverage:
 * Reaches ra8_book_src_prefetch_chapter's `if (ce != k_ra8_ok) return ce;` arm: the
 ...
 * chapter-record read; ra8_book_src_prefetch_chapter has no compound decisions)
 */
static void test_ra8_book_paged_prefetch_record_fault(void)
{
  ...
  s_pbook_fault_armed = true;
  const ra8_err_t e   = ra8_book_src_prefetch_chapter(&psrc, 0U);
 ...
 * ra8_book_src_prefetch_chapter(&psrc, 1) is a HIT with no new miss -- the frame was
 ...
 *
 * @par MC/DC:
 * (no compound decisions under test -- ra8_book_src_prefetch_chapter's guards are
 ...
 */
static void test_ra8_book_paged_prefetch_warms(void)
{
...
int32_t main(void)
{
  ...
  test_ra8_book_paged_block_break_overflow();
