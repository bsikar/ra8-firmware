...
static ra8_err_t fkn_read(void* ctx, uint64_t lba, uint32_t count, uint8_t* buf)
{
...
static ra8_err_t fkn_write(void* ctx, uint64_t lba, uint32_t count, const uint8_t* buf)
{
  fkn_disk_t* d = (fkn_disk_t*)ctx;
  if ((lba >= d->block_count) || (count > (d->block_count - lba))) {
    return k_ra8_err_out_of_range;
...
static ra8_err_t fkn_capacity(void* ctx, uint64_t* block_count, uint32_t* block_size)
{
...
static const ra8_fs_backend_t s_backend = {
  .read_block   = fkn_read,
  .write_block  = fkn_write,
  .get_capacity = fkn_capacity,
  .erase_blocks = nullptr,
  .ctx          = &s_disk,
};

/** @brief Allocate a zeroed device of @p blocks sectors of @p bps bytes. */
static void fkn_alloc(uint64_t blocks, uint32_t bps)
{
 ...
 * @since 0.1.0
 */
static void fkn_dump_image(const char* tag)
{
  ...
  (void)fwrite(s_disk.bytes, 1U, total, fp);
  (void)fclose(fp);
}

/** @brief Fill @p buf with a position-dependent pattern from @p seed. */
static void fkn_pattern(uint8_t* buf, uint32_t len, uint8_t seed)
{
...
static void fkn_file_roundtrip(ra8_fs_mount_t* m)
{
  static uint8_t s_out[k_fkn_payload];
  static uint8_t s_in[k_fkn_payload];
  fkn_pattern(s_out, (uint32_t)k_fkn_payload, (uint8_t)k_fkn_seed);
 ...
 */
static void test_4kn_fat16_roundtrip(void)
{
  TEST_BEGIN("4Kn: FAT16 format + mount + file round-trip at 4096-byte sectors");
  fkn_alloc((uint64_t)k_fkn_blocks_fat16, (uint32_t)k_fkn_bps);
 ...
 */
static void test_4kn_fat32_roundtrip(void)
{
  TEST_BEGIN("4Kn: FAT32 format + mount + file round-trip at 4096-byte sectors");
  fkn_alloc((uint64_t)k_fkn_blocks_fat32, (uint32_t)k_fkn_bps);
  ra8_fs_format_opts_t opts = {};
 ...
 */
static void test_4kn_exfat_roundtrip(void)
{
  TEST_BEGIN("4Kn: exFAT format + mount + dirs + check at 4096-byte sectors");
  fkn_alloc((uint64_t)k_fkn_blocks_exfat, (uint32_t)k_fkn_bps);
  ra8_fs_format_opts_t opts = {};
  ...
  fkn_pattern(s_nested, (uint32_t)k_fkn_trunc_down, (uint8_t)k_fkn_seed);
 ...
 */
static void test_4kn_mismatch_refused(void)
{
  TEST_BEGIN("4Kn: sector-size mismatch between image and device is refused");
  /* A 512-byte FAT16 image... */
  fkn_alloc((uint64_t)k_fkn_blocks_512, (uint32_t)k_fkn_bps_512);
  ra8_fs_format_opts_t opts = {};
  opts.type                 = k_ra8_fs_type_fat16;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_format(&s_backend, &opts));
 ...
 */
static void test_mcdc_bps_valid(void)
{
...
int32_t main(void)
{
  test_4kn_fat16_roundtrip();
  test_4kn_fat32_roundtrip();
  test_4kn_exfat_roundtrip();
  test_4kn_mismatch_refused();
  test_mcdc_bps_valid();
  (void)fprintf(stderr, "[OK  ] test_ra8_fs_4kn.c\n");
  return 0;
}
