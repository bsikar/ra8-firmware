/**
 * @file test_ra_io_compress.c
 * @brief Unit tests for the ra_io DEFLATE compress/decompress (issue #161).
 *
 * @details
...
static void test_round_trip(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra_ok,
    ra_io_decompress(s_packed, packed_len, s_restored, k_t_payload_bytes, &restored_len));
...
static void test_decompress_validation(void)
{
  ...
  uint32_t out_len = 0;
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_io_decompress(nullptr, packed_len, s_restored, k_t_payload_bytes, &out_len));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_io_decompress(s_packed, packed_len, nullptr, k_t_payload_bytes, &out_len));
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_io_compress.c\n");
  return 0;
}
