/**
 * @file test_ra_img_arena_mcdc.c
 * @brief MC/DC vectors for the compound guard in libs/ra_reflow/src/ra_img_arena.c.
 *
 * @details
 * The image bump-arena free path guards on `(p == nullptr) || (a == nullptr)`
 * where `a` is the currently-bound arena (`s_arena`). The existing
 * test_ra_img_arena_cov.c only ever frees a real block while an arena is
 ...
 * @brief Independent-influence vectors for the ra_img_arena_free early-return
 *        guard.
 *
 * @par MC/DC:
 * Decision: `if ((p == nullptr) || (a == nullptr))` (2 conditions, OR;
 * libs/ra_reflow/src/ra_img_arena.c@ra_img_arena_free). `a` is the bound arena
 * `s_arena`, so `a == nullptr` is reached by freeing after ra_img_arena_unbind().
 *
...
{
  TEST_BEGIN("ra_img_arena_free MC/DC: (p==null) || (arena==null)");
  ...
  void* const block = ra_img_arena_malloc(16U);
  TEST_ASSERT_NOT_NULL(block);
  TEST_ASSERT_EQ(1, arena.live);
  ra_img_arena_free(block);
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_img_arena_mcdc.c\n");
  return 0;
}
