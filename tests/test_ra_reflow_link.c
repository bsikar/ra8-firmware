/**
 * @file test_ra_reflow_link.c
 * @brief Host unit tests + MC/DC for the #110 link/anchor query API.
 *
 * @details
 * Covers libs/ra_reflow/src/ra_reflow_link.c:
 *  - ra_reflow_href_split(): classification of fragment / chapter /
 ...
 *  - ra_reflow_hit_test_link() and ra_reflow_find_anchor() against a
...
static ra_reflow_href_kind_t split_kind(const char* href)
{
  ra_reflow_href_kind_t kind = k_ra_reflow_href_empty;
  uint32_t              pl = 0U, fo = 0U, fl = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_reflow_href_split(href, (uint32_t)strlen(href), &kind, &pl, &fo, &fl));
  return kind;
}
 ...
 *
 * @par MC/DC:
 * Decisions in ra_reflow_href_split()/priv_href_classify():
...
{
  TEST_BEGIN("ra_reflow_href_split classify MC/DC");
  TEST_ASSERT_EQ(k_ra_reflow_href_fragment, split_kind("#foot"));
  TEST_ASSERT_EQ(k_ra_reflow_href_chapter, split_kind("ch2.xhtml"));
  TEST_ASSERT_EQ(k_ra_reflow_href_chapter_fragment, split_kind("ch2.xhtml#f"));
  TEST_ASSERT_EQ(k_ra_reflow_href_external, split_kind("http://x/y"));
  TEST_ASSERT_EQ(k_ra_reflow_href_external, split_kind("mailto:a@b"));
  TEST_ASSERT_EQ(k_ra_reflow_href_chapter, split_kind("sub/ch.xhtml"));
  TEST_ASSERT_EQ(k_ra_reflow_href_empty, split_kind(""));
  TEST_ASSERT_EQ(k_ra_reflow_href_fragment, split_kind("#"));
  TEST_END("ra_reflow_href_split classify MC/DC");
}
...
{
  TEST_BEGIN("ra_reflow_href_split spans + null guard");
  ra_reflow_href_kind_t kind = k_ra_reflow_href_empty;
  uint32_t              pl = 0U, fo = 0U, fl = 0U;
...
{
  TEST_BEGIN("ra_reflow_hit_test_link");
  ...
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_reflow_hit_test_link(&s_eng, 0U, 40, 110, &off, &len));
  /* Null guard. */
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_reflow_hit_test_link(&s_eng, 1U, 40, 110, NULL, &len));
  TEST_END("ra_reflow_hit_test_link");
}
...
{
  TEST_BEGIN("ra_reflow_find_anchor");
  memset(&s_eng, 0, sizeof s_eng);
  s_eng.in_use = 1U;
  ...
  /* Unknown id. */
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_reflow_find_anchor(&s_eng, "barz", 4U, &page));
  /* Zero length -> invalid arg. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_reflow_find_anchor(&s_eng, "foot", 0U, &page));
  TEST_END("ra_reflow_find_anchor");
}
