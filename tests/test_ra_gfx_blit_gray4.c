/**
 * @file test_ra_gfx_blit_gray4.c
 * @brief Unit tests for ra_gfx_blit_gray4_zoom() -- the reader-loupe sub-rect blit.
 *
 * @details
 * Binds ra_gfx to a small ARGB8888 framebuffer (whose in-memory R byte equals
...
/** @brief Re-bind ra_gfx to a freshly-cleared ARGB8888 scratch framebuffer. */
static void rebind_argb(void)
{
  (void)memset(s_g4_fb, 0, sizeof(s_g4_fb));
  const ra_err_t err =
    ra_gfx_init(s_g4_fb, k_test_g4_fb_w, k_test_g4_fb_h, k_ra_gfx_format_argb8888);
...
static void test_blit_gray4_1to1(void)
{
  ...
  rebind_argb();
  TEST_ASSERT_EQ(k_ra_ok, ra_gfx_blit_gray4_zoom(k_g4_ramp4x4, 4, 4, 0, 0, 4, 4, 1, 0, 0));
...
static void test_blit_gray4_zoom2(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_gfx_blit_gray4_zoom(k_g4_ramp4x4, 4, 4, 0, 0, 2, 2, 2, 0, 0));
...
static void test_blit_gray4_nibble_parity(void)
{
  ...
  rebind_argb();
  TEST_ASSERT_EQ(k_ra_ok, ra_gfx_blit_gray4_zoom(k_g4_odd3x2, 3, 2, 0, 0, 3, 2, 1, 0, 0));
...
static void test_blit_gray4_clips_at_edges(void)
{
  ...
  rebind_argb();
  fill_sentinel(0xAA);
  TEST_ASSERT_EQ(k_ra_ok, ra_gfx_blit_gray4_zoom(k_g4_ramp4x4, 4, 4, 2, 0, 4, 1, 1, 0, 0));
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_gfx_blit_gray4_zoom(k_g4_ramp4x4, 4, 4, 0, 0, 4, 1, 1, k_test_g4_fb_w - 2, 0));
 ...
 * libs/ra_gfx/src/ra_gfx_blit_gray4.c@ra_gfx_blit_gray4_zoom
 * - Vector 1: src_w=4, src_h=4 -> false (both false; control) -> k_ra_ok
...
static void test_mcdc_blit_gray4_dims(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_not_initialized,
                 ra_gfx_blit_gray4_zoom(k_g4_ramp4x4, 4, 4, 0, 0, 4, 4, 1, 0, 0));
  rebind_argb();
  /* Scalar guards. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_gfx_blit_gray4_zoom(nullptr, 4, 4, 0, 0, 4, 4, 1, 0, 0));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_gfx_blit_gray4_zoom(k_g4_ramp4x4, 4, 4, 0, 0, 4, 4, 0, 0, 0));
  /* MC/DC vectors for (src_w <= 0) || (src_h <= 0). */
  TEST_ASSERT_EQ(k_ra_ok, ra_gfx_blit_gray4_zoom(k_g4_ramp4x4, 4, 4, 0, 0, 4, 4, 1, 0, 0));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_gfx_blit_gray4_zoom(k_g4_ramp4x4, 0, 4, 0, 0, 4, 4, 1, 0, 0));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_gfx_blit_gray4_zoom(k_g4_ramp4x4, 4, 0, 0, 0, 4, 4, 1, 0, 0));
  TEST_END("gray4 blit MC/DC dimension + scalar guards");
}

int main(void)
{
  /* Must run first: asserts the not-initialized path before any ra_gfx_init(). */
