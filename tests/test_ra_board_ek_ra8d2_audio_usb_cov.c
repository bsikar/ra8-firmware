/**
 * @file test_ra_board_ek_ra8d2_audio_usb_cov.c
 * @brief Coverage-boosting unit tests for the EK-RA8D2 audio + USB BSP layer
 *
 * @details
 * Companion to test_ra_board_ek_ra8d2.c.  The primary board test only
 * exercises the argument-rejection guards of
 * ``ra_board_ek_ra8d2_audio_usb.c`` (bad sample rate / depth / channels,
 ...
 *   / ra_board_audio_init / ra_board_audio_play_sample_block):
 *     - Call ra_board_audio_init with every supported bit depth
 ...
 *       pins route cleanly and ra_ssie_init succeeds in RA_SIMULATOR_MODE,
 *       so the success tail (s_audio_initialized = true) is covered.
 *     - Pre-claim the BCLK pin so internal_audio_route_pins returns a
 *       gpio-conflict from its first ra_pfs_route_peripheral, covering the
 *       loop error-return leg and ra_board_audio_init's route-error tail.
 ...
 *       is written and ra_board_audio_play_sample_block returns k_ra_ok.
 *     - Pre-seed SSIFSR.TDC to the full FIFO depth so ra_ssie_write_buffer
 ...
 *   internal_usbhs_clock_and_mstp / ra_board_usbhs_device_init /
 *   ra_board_usbhs_host_init):
 *     - Pre-claim PD07 so the role-select GPIO drive fails and
 *       ra_board_usbhs_device_init returns at its role-select guard.
 *     - Pre-seed OSCSF so ra_cgc_usbhs_pll_enable's oscillator-stable and
 *       PLL2 waits resolve; internal_usbhs_clock_and_mstp then reaches its
 *       ra_mstp_enable tail and the device / host init reach the underlying
 *       ra_usb_*_init calls.
 *
 * No source line in ra_board_ek_ra8d2_audio_usb.c is marked GCOVR_EXCL by
 ...
 * fail in RA_SIMULATOR_MODE with the fixed board constants); they are
 ...
 *
 * @details
 * Zeroes every hardware register window (ra_sim_mmap_reset) and frees all
 * claimed pins (ra_pin_validator_reset).  Called at the start of each test
...
/**
 * @brief Verify ra_board_audio_init succeeds for every supported bit depth.
 *
 * @details
 ...
 * arm of internal_audio_bits_to_word executes and returns k_ra_ok, after
 * which the six DA7212 pins route cleanly (clean pin validator) and
 * ra_ssie_init succeeds in RA_SIMULATOR_MODE (SSIRST/FIFO-reset polls read
 ...
 * ``channels != mono && channels != stereo`` (ra_board_audio_init).  Every
 ...
 * in test_ra_board_ek_ra8d2.c and the source carries an mcdc-deactivated
...
static void test_audio_init_all_bit_depths(void)
{
  ...
  for (uint32_t i = 0U; i < sizeof(s_depths) / sizeof(s_depths[0]); ++i) {
    ...
    const ra_err_t err      = ra_board_audio_init(k_test_audio_rate_hz, s_depths[i], channels);
...
/**
 * @brief Verify ra_board_audio_init propagates a routing pin conflict.
 *
 * @details
 * Pre-claiming the BCLK pin (P403) makes the first ra_pfs_route_peripheral
 * inside internal_audio_route_pins return k_ra_err_gpio_conflict, exercising
 * the loop's error-return leg and ra_board_audio_init's route-error tail.
 * A valid sample rate, depth and channel count are supplied so control
 * reaches the routing step rather than an earlier guard.
 *
 * @par MC/DC:
 * The route loop's decision ``if (err != k_ra_ok)`` is a single condition.
...
static void test_audio_route_pin_conflict(void)
{
  ...
  reset_state();
  const ra_port_pin_t bclk = (ra_port_pin_t)k_ra_board_audio_pin_bclk;
  TEST_ASSERT_EQ(k_ra_ok, ra_pin_validator_claim(bclk, "test.audio.bclk"));
  const ra_err_t err = ra_board_audio_init(k_test_audio_rate_hz, 16U, 2U);
  TEST_ASSERT_EQ(k_ra_err_gpio_conflict, err);
  TEST_END("audio_init propagates a route pin conflict");
}

/* -------------------------------------------------------------------------
 * 3. ra_board_audio_play_sample_block -- full-write success path
 * -------------------------------------------------------------------------
 */

/**
 * @brief Verify a play block fully written returns k_ra_ok.
 *
 * @details
 * A successful ra_board_audio_init sets s_audio_initialized.  The SSIE
 * transmit-FIFO count (SSIFSR.TDC) is zero after reset, so ra_ssie_write_buffer
 * writes every 32-bit word and reports words_written == words; the short-write
 * guard is not taken and the function returns k_ra_ok.  The PCM buffer is a
 ...
 * test_audio_play_sample_block_validates in test_ra_board_ek_ra8d2.c.  The
...
static void test_audio_play_full_write(void)
{
  ...
  reset_state();
  TEST_ASSERT_EQ(k_ra_ok, ra_board_audio_init(k_test_audio_rate_hz, 16U, 2U));

  static uint32_t s_pcm_words[k_test_pcm_word_cnt] = {};
  const int16_t*  buf                              = (const int16_t*)s_pcm_words;
  const ra_err_t  err = ra_board_audio_play_sample_block(buf, (uint32_t)k_test_pcm_len);
  TEST_ASSERT_EQ(k_ra_ok, err);
  TEST_END("audio_play_sample_block writes a full block and returns ok");
}

/* -------------------------------------------------------------------------
 * 4. ra_board_audio_play_sample_block -- short-write timeout leg
 * -------------------------------------------------------------------------
 */
 ...
 * depth (32).  ra_ssie_write_buffer reads TDC >= depth on the first sample
 * and breaks immediately, so words_written stays zero and is less than the
 * requested word count; ra_board_audio_play_sample_block then takes the
 * short-write guard and returns k_ra_err_hw_timeout.
 *
 * @par MC/DC:
...
static void test_audio_play_short_write(void)
{
  ...
  /* Fill the transmit-FIFO count so ra_ssie_write_buffer writes zero words. */
  volatile r_ssie_regs_t* ssie = ra_ssie((uint8_t)k_ra_board_audio_ssie_channel);
  ssie->SSIFSR = (uint32_t)((uint32_t)k_ra_ssie_fifo_depth << (uint32_t)k_ra_ssie_shift_tdc);
 ...
 * ra_gpio_output_init(scl) call, so it returns the conflict without releasing
 * anything, and internal_io_expander_apply propagates it out of
 * ra_board_io_expander_set_usbhs_device_mode.
 *
 * @par MC/DC:
 * Both decisions on the path (bus-recover's ``if (err != k_ra_ok)`` and
 * apply's ``if (err != k_ra_ok)``) are single-condition; this test supplies
...
static void test_io_expander_bus_recover_scl_conflict(void)
{
  ...
  reset_state();
  const ra_port_pin_t scl = RA_PIN(k_ra_port_5, k_ra_pin_12);
  TEST_ASSERT_EQ(k_ra_ok, ra_pin_validator_claim(scl, "test.ioexp.scl"));
  const ra_err_t err = ra_board_io_expander_set_usbhs_device_mode();
  TEST_ASSERT_EQ(k_ra_err_gpio_conflict, err);
 ...
 *
 * @details
 * Pre-claiming only SDA1 (P511) lets bus-recover's ra_gpio_output_init(scl)
 * succeed (SCL1 free) but its ra_gpio_input_init(sda) fails; the recover
 * routine then releases the SCL claim it just took and returns the conflict,
 * which apply propagates.
 *
 * @par MC/DC:
 * The two ``if (err != k_ra_ok)`` decisions crossed here are single-condition;
...
static void test_io_expander_bus_recover_sda_conflict(void)
{
  ...
  reset_state();
  const ra_port_pin_t sda = RA_PIN(k_ra_port_5, k_ra_pin_11);
  TEST_ASSERT_EQ(k_ra_ok, ra_pin_validator_claim(sda, "test.ioexp.sda"));
  const ra_err_t err = ra_board_io_expander_set_usbhs_device_mode();
  TEST_ASSERT_EQ(k_ra_err_gpio_conflict, err);
 ...
 * first ra_gpio_read(sda) in the bus-recovery pulse loop observe SDA released,
 ...
 *
 * @par MC/DC:
 * The loop guards ``if (rd == k_ra_ok)`` and ``if (sda_lvl == high)`` are
...
static void test_io_expander_bus_recover_sda_high(void)
{
  ...
  volatile r_port_regs_t* p5 = ra_port(k_ra_port_5);
  p5->PCNTR2                 = (uint32_t)(1UL << (uint32_t)k_ra_pin_11);
  const ra_err_t err         = ra_board_io_expander_set_usbhs_device_mode();
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, err);
 ...
 * ra_gpio_output_init(pullup_a); apply propagates the conflict.
 *
 * @par MC/DC:
 * enable_pullups' ``if (err != k_ra_ok)`` and apply's guard are
...
static void test_io_expander_pullup_conflict(void)
{
  ...
  reset_state();
  const ra_port_pin_t pullup_a = RA_PIN(k_ra_port_1, k_ra_pin_9);
  TEST_ASSERT_EQ(k_ra_ok, ra_pin_validator_claim(pullup_a, "test.ioexp.pullup"));
  const ra_err_t err = ra_board_io_expander_set_usbhs_device_mode();
  TEST_ASSERT_EQ(k_ra_err_gpio_conflict, err);
 ...
 * / drain / finish without a timeout, since ra_i2c_init never touches ICSR2
 ...
 * ra_board_io_expander_set_usbhs_device_mode returns k_ra_ok.
 *
 * @par MC/DC:
 * The program-u15 and apply decisions crossed here are single-condition
 * ``if (err != k_ra_ok)`` checks; this test supplies the all-not-taken
...
static void test_io_expander_apply_success(void)
{
  ...
  volatile r_i2c_regs_t* riic1 = ra_i2c_regs((uint8_t)1U);
  riic1->ICSR2 = (uint8_t)((uint8_t)k_ra_i2c_msk_icsr2_tdre | (uint8_t)k_ra_i2c_msk_icsr2_tend);
  const ra_err_t err = ra_board_io_expander_set_usbhs_device_mode();
  TEST_ASSERT_EQ(k_ra_ok, err);
 ...
 *
 * @details
 * Each of ra_board_io_expander_set_usbhs_host_mode,
 * ra_board_io_expander_apply_project_sw4_defaults and
 * ra_board_io_expander_set_octospi_active is a one-line delegation to
...
static void test_io_expander_entry_points(void)
{
  ...
  reset_state();
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_board_io_expander_set_usbhs_host_mode());
  reset_state();
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_board_io_expander_apply_project_sw4_defaults());
  reset_state();
  TEST_ASSERT_EQ(k_ra_err_hw_timeout, ra_board_io_expander_set_octospi_active());
 ...
 * ra_gpio_output_init fail, so ra_board_usbhs_device_init returns the
 * conflict at its role-select guard, before the U15 override or the CGC/MSTP
 * bring-up.
 *
 * @par MC/DC:
 * The role-select guard ``if (pd07_err != k_ra_ok)`` is single-condition;
...
static void test_usbhs_device_role_pin_conflict(void)
{
  ...
  reset_state();
  const ra_port_pin_t pd07 = RA_PIN(k_ra_port_13, k_ra_pin_7);
  TEST_ASSERT_EQ(k_ra_ok, ra_pin_validator_claim(pd07, "test.usbhs.pd07"));
 ...
 * ra_cgc_usbhs_pll_enable's main-oscillator, PLL2 and HOCO waits resolve, so
 * internal_usbhs_clock_and_mstp advances past ra_cgc_usbhs_pll_enable to its
 * ra_mstp_enable tail and ra_board_usbhs_device_init reaches
 * ra_usb_device_init.  The assertion checks the module's s_usbhs_probe reached
 * its post-device-init value, which proves the pre / post probe writes around
 * ra_usb_device_init executed regardless of that call's final code (so the
 * check does not depend on host-vs-silicon USB behaviour).
 *
 * @par MC/DC:
 * The bring-up guards (``if (err != k_ra_ok)`` after pll-enable and after
...
{
  TEST_BEGIN("usbhs_device_init reaches ra_usb_device_init after clocking");
  reset_state();
...
/**
 * @brief Verify host init reaches ra_usb_host_init after clocking.
 *
 * @details
 * With OSCSF pre-seeded, internal_usbhs_clock_and_mstp succeeds and
 * ra_board_usbhs_host_init reaches ra_usb_host_init, whose HS bring-up skips
 * the real PHY PLL-lock poll in RA_SIMULATOR_MODE and returns k_ra_ok, so the
 * board host-init returns k_ra_ok.
 *
 * @par MC/DC:
 * The bring-up guard ``if (err != k_ra_ok)`` after clock-and-mstp is
 * single-condition; this test supplies the not-taken (success) vector.
 *
 * @pre Clean simulated state, then OSCSF pre-seeded.
 * @post ra_usb_host_init reached; board host-init returned k_ra_ok.
 *
 * @note Not thread-safe; single-threaded test context.
 * @since 0.1.0
 */
...
{
  TEST_BEGIN("usbhs_host_init reaches ra_usb_host_init after clocking");
  reset_state();
  *ra_sys_oscsf()    = (uint8_t)0xFFU;
  const ra_err_t err = ra_board_usbhs_host_init();
  TEST_ASSERT_EQ(k_ra_ok, err);
  TEST_END("usbhs_host_init reaches ra_usb_host_init after clocking");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_board_ek_ra8d2_audio_usb_cov.c\n");
  return 0;
}
