 ...
 *    bounded data-stage pull `internal_pull_data_stage`.
 ...
 * buffer, calls `internal_pull_data_stage`, observes that the host register
 * fixture reports `k_ra8_err_no_data`
...
static void test_handle_setup_set_line_coding(void)
{
  ...
  /* The empty host DCP returns k_ra8_err_no_data, so the apply
