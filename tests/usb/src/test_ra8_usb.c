...
/** @brief A missing USBFS SCKE acknowledgement surfaces as a timeout. @since 0.1.0 */
RA8_INTERNAL static void internal_test_init_fs_scke_timeout(void)
{
  TEST_BEGIN("ra8_usb_device_init FS SCKE timeout");
  internal_prep();

  volatile r_usb_regs_t* reg = ra8_usb_fs();
  /* HUM Ch 36.2.1 "SYSCFG : System Configuration Control Register" p 1967 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(&reg->SYSCFG));
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_usb_device_init(k_ra8_usb_speed_fs));
  ra8_fake_mmio_reset();
  TEST_END("ra8_usb_device_init FS SCKE timeout");
}
...
/**
 * @par MC/DC:
 * Decision: `if (reg == nullptr)` in `ra8_usb_dispatch()`
 * (libs/ra8_hal/src/ra8_usb_irq.c) -- 1 condition.
 * - Vector 1: speed = k_ra8_usb_speed_fs -> `priv_pick()` returns the USBFS
 *   register block -> the decision is false and the dispatcher acks INTSTS0
 *   and relays to the attached handler (control; the second half of this
 *   case, and internal_test_attach_and_dispatch above).
 * - Vector 2: speed = 9 (no such controller) -> `priv_pick()` returns nullptr
 *   -> the decision is true and the dispatcher returns without touching a
 *   register or invoking a handler (this case).
 * N = 1 condition, N+1 = 2 vectors. Vectors 1+2 differ only in whether
 * `priv_pick()` resolved a controller and produce opposite outcomes, so that
 * condition independently affects the decision. @brief Verify dispatch rejects a bogus speed. @details Drives ra8_usb_dispatch with an out-of-range speed and asserts nothing is acked and no handler runs. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_dispatch_rejects_bogus_speed(void)
{
  TEST_BEGIN("usb dispatch rejects bogus speed");
  internal_prep_cb();
  ra8_usb_attach_handler(k_ra8_usb_speed_fs, internal_stub_usb_cb, (void*)(uintptr_t)0xAAU);
  /* HUM Ch 36.2.14 "INTSTS0 : Interrupt Status Register 0", p 1986 */
  ra8_usb_fs()->INTSTS0 = (uint16_t)k_t_intsts_fs_b;

  /* 9 is not a controller instance, so priv_pick() cannot resolve a register
     block and the dispatcher must bail out before the W0C ack. */
  ra8_usb_dispatch((ra8_usb_speed_t)9U);
  TEST_ASSERT_EQ(0, s_usb_cb_count);
  /* No ack was written, so the staged status word survives untouched. */
  /* HUM Ch 36.2.14 "INTSTS0 : Interrupt Status Register 0", p 1986 */
  TEST_ASSERT_EQ(k_t_intsts_fs_b, ra8_usb_fs()->INTSTS0);

  /* Control: the very same staged state dispatched at a real speed does relay. */
  ra8_usb_dispatch(k_ra8_usb_speed_fs);
  TEST_ASSERT_EQ(1, s_usb_cb_count);
  TEST_ASSERT_EQ(k_t_intsts_fs_b, s_usb_cb_last_mask);
  TEST_END("usb dispatch rejects bogus speed");
}
...
static void (*const s_test_roster[])(void) = {
  ...
  internal_test_init_fs_scke_timeout,
  ...
  internal_test_dispatch_rejects_bogus_speed,
};
