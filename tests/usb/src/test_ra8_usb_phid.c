...
/**
 * @test internal_test_send_report_id_queue_timeout
 * @brief Verify report-ID queue failures stop before the payload write.
 * @details Arms the USBFS CFIFO wait seam before sending a report with a
 *          non-zero ID, making the first queue operation time out.
 * @pre The device is initialized at full speed.
 * @pre The fake MMIO wait table is empty before the test arms CFIFOCTR.
 * @post `ra8_usb_phid_send_report` returns `k_ra8_err_hw_timeout`.
 * @post The function does not attempt to queue the payload after the ID fails.
 * @note Host-only and single-threaded.
 * @par MC/DC:
 * The timeout result drives `rid_err != k_ra8_ok` true; the preceding success
 * case drives it false.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_test_send_report_id_queue_timeout(void)
{
  TEST_BEGIN("ra8_usb_phid_send_report propagates report-ID queue timeout");
  internal_prep();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_usb_phid_init(k_ra8_usb_speed_fs));
  /* HUM Ch 36.2.8 "CFIFOCTR : CFIFO Port Control Register" p 1977 */
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait((const volatile void*)&ra8_usb_fs()->CFIFOCTR));

  const uint8_t payload[] = {k_t_report_b0};
  TEST_ASSERT_EQ(k_ra8_err_hw_timeout, ra8_usb_phid_send_report(1U, payload, 1U));
  TEST_END("ra8_usb_phid_send_report propagates report-ID queue timeout");
}
...
int main(void)
{
  ...
  internal_test_send_report_id_queue_timeout();
