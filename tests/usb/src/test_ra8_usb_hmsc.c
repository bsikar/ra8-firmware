 ...
 * `examples/ek_ra8d2/hw_validated/hil/usb_host_msc_browse` and
...
/**
 * @test test_init_propagates_host_mstp_failure
 *
 * @brief Verify host-controller bring-up failure is mapped to the HMSC
 *        lifecycle error.
 *
 * @details Saturates the existing USBFS MSTP reference count. The next
 * `ra8_usb_host_init()` call then fails through its real MSTP dependency, so
 * this exercises the HMSC error branch without a test-only production seam.
 *
 * @par MC/DC:
 * Decision under test: the single-condition `usb_err != k_ra8_ok` guard in
 * `libs/ra8_hal/src/ra8_usb_hmsc.c@ra8_usb_hmsc_init`. This fault vector
 * drives the condition true after USBFS MSTP saturation;
 * ::test_init_fs_returns_ok drives it false (N+1 = 2). The separate
 * ::test_mcdc_hmsc supplies the N+1 = 3 vectors for this function's compound
 * speed gate.
 *
 * @pre Fake MMIO and MSTP bookkeeping are reset.
 * @post HMSC initialization reports `k_ra8_err_hw_init_failed`.
 *
 * @note Single-threaded host test.
 */
static void test_init_propagates_host_mstp_failure(void)
{
  TEST_BEGIN("ra8_usb_hmsc_init propagates USBFS MSTP saturation");
  prep();

  for (uint16_t i = 0U; i < (uint16_t)UINT8_MAX; ++i) {
    TEST_ASSERT_EQ(k_ra8_ok, ra8_mstp_enable(k_ra8_mstp_usbfs));
  }

  TEST_ASSERT_EQ(k_ra8_err_hw_init_failed, ra8_usb_hmsc_init(k_ra8_usb_speed_fs));
  TEST_END("ra8_usb_hmsc_init propagates USBFS MSTP saturation");
}
...
int main(void)
{
  ...
  test_init_bad_speed();
  test_init_propagates_host_mstp_failure();
