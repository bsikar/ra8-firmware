...
static void test_mark_ns_covers_query(void)
{
  ...
  ra_sim_world_reset();
  TEST_ASSERT_EQ(k_ra_ok, ra_sim_world_mark_ns(s_ns_buf, (uint32_t)sizeof(s_ns_buf)));
...
static void test_secure_overlap_blocks(void)
{
  ...
  ra_sim_world_reset();
  TEST_ASSERT_EQ(k_ra_ok, ra_sim_world_mark_ns(s_ns_buf, (uint32_t)sizeof(s_ns_buf)));
  TEST_ASSERT_EQ(k_ra_ok, ra_sim_world_mark_s(s_ns_buf + 64, 16U));
...
static void test_unrelated_secure_region_does_not_block(void)
{
  ...
  ra_sim_world_reset();
  TEST_ASSERT_EQ(k_ra_ok, ra_sim_world_mark_ns(s_ns_buf, (uint32_t)sizeof(s_ns_buf)));
  TEST_ASSERT_EQ(k_ra_ok, ra_sim_world_mark_s(s_s_buf, (uint32_t)sizeof(s_s_buf)));
  TEST_ASSERT(ra_sim_world_check_ns_range(s_ns_buf, 64U));
...
static void test_region_table_full(void)
{
  ...
  for (uint8_t i = 0U; i < (uint8_t)k_ra_sim_world_max_regions; ++i) {
    TEST_ASSERT_EQ(k_ra_ok, ra_sim_world_mark_ns(s_ns_buf + i, 1U));
  }
  TEST_ASSERT_EQ(k_ra_err_no_mem, ra_sim_world_mark_ns(s_ns_buf + 32, 1U));
