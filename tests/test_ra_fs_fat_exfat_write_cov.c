/**
 * @file tests/test_ra_fs_fat_exfat_write_cov.c
 * @brief Coverage-boost tests for `ra_fs_fat_exfat_write.c`.
 *
 * @details
...
/**
 * @struct wc_disk_t
 * @brief Memory-backed disk presented to `ra_fs` as a block device.
 *
...
/**
 * @brief Countdown-faulting `ra_fs` read backend.
 *
 * @details Decrements `s_rd_remaining` on each call. When the counter
 *          reaches zero it returns `k_ra_err_out_of_range` without touching
 ...
 * @retval k_ra_ok             Sectors read successfully.
 * @retval k_ra_err_out_of_range Request out of range, or countdown expired.
 *
 * @pre ctx and buf are non-NULL.
 * @post buf holds the requested sectors when k_ra_ok is returned.
 *
 * @since 0.1.0
 */
static ra_err_t wc_read(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf)
{
  if (s_rd_remaining == 0) {
...
/**
 * @brief Countdown-faulting `ra_fs` write backend.
 *
 * @param[in] ctx   Pointer to `wc_disk_t`.
 * @param[in] lba   Start sector.
 * @param[in] count Sector count.
 ...
 *
 * @return k_ra_ok or k_ra_err_out_of_range.
 * @retval k_ra_ok             Sectors written successfully.
 * @retval k_ra_err_out_of_range Request out of range, or countdown expired.
 *
 * @pre ctx and buf are non-NULL.
 * @post Disk bytes match @p buf on k_ra_ok.
 *
 * @since 0.1.0
 */
static ra_err_t wc_write(void* ctx, uint32_t lba, uint32_t count, const uint8_t* buf)
{
  if (s_wr_remaining == 0) {
    return k_ra_err_out_of_range;
  }
...
/**
 * @brief Report disk geometry to `ra_fs`.
 *
 * @param[in]  ctx         Pointer to `wc_disk_t`.
 ...
 * @since 0.1.0
 */
static ra_err_t wc_capacity(void* ctx, uint32_t* block_count, uint32_t* block_size)
{
  const wc_disk_t* d = (const wc_disk_t*)ctx;
 ...
 * @since 0.1.0
 */
static const ra_fs_backend_t s_ctrl_backend = {
  .read_block   = wc_read,
...
static void build_exfat_volume(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_format(&s_ctrl_backend, &opts));
}
 ...
 * @since 0.1.0
 */
static uint32_t root_entry_off(const ra_fs_mount_t* h, uint32_t idx)
{
 ...
 * @since 0.1.0
 */
static uint32_t fat_entry_off(const ra_fs_mount_t* h, uint32_t clus)
{
 ...
 * @since 0.1.0
 */
static void patch_root_full(const ra_fs_mount_t* h, uint32_t start)
{
 ...
 * `ra_fs_fat_exfat_write.c`.  Its full declaration is in the umbrella header
 * `ra_fs_fat_internal.h` (included above), which is on the include path for
 ...
 *
 * @details Calling `ra_fs_write_file` with "/" as the path causes
 *          `priv_exfat_create` to strip the leading slash, leaving an
 *          empty name of length 0, and return `k_ra_err_invalid_arg`.
 *
 * Lines targeted: 548.
 *
 * @par MC/DC:
 ...
 * @post ra_fs_write_file returns k_ra_err_invalid_arg.
 *
 * @since 0.1.0
 */
static void test_create_empty_path(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_ctrl_backend, &h));

  const uint8_t dummy = (uint8_t)'X';
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_fs_write_file(h, "/", &dummy, 1U));
 ...
 *          `priv_exfat_create` returns `k_ra_err_invalid_arg` at line 551.
 *
 * Lines targeted: 551.
 *
 * @par MC/DC:
...
static void test_create_path_too_long(void)
{
  ...
  const uint8_t dummy = (uint8_t)'X';
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_fs_write_file(h, k_long_name, &dummy, 1U));
 ...
 *
 * @details `ra_fs_write_file` passes `len=0` to `priv_exfat_create`, which
 *          returns `k_ra_err_invalid_arg` at line 554.
 *
 * Lines targeted: 554.
 *
 * @par MC/DC:
...
static void test_create_zero_len(void)
{
  ...
  const uint8_t dummy = (uint8_t)'X';
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_fs_write_file(h, "X.TXT", &dummy, 0U));
 ...
 * @brief `priv_exfat_find_bitmap` returns `k_ra_err_not_found` at the
 ...
 *          sees EOD at entry 0 and returns `k_ra_err_not_found` at line 82.
 ...
 *
 * @pre Volume is formatted and accessible.
 * @post ra_fs_write_file returns k_ra_err_not_found.
 *
 * @since 0.1.0
 */
static void test_find_bitmap_eod(void)
{
  ...
  const uint8_t dummy = (uint8_t)'X';
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_fs_write_file(h, "X.TXT", &dummy, 1U));
 ...
 *
 * @par MC/DC:
 * Decision: `if (r != k_ra_ok)` in find_bitmap -- 1 condition.
 * V1: r=k_ra_err_out_of_range -> T -> error returned (this test).
 * V2: r=k_ra_ok -> F -> entry examined (normal path).
 *
 * @pre Volume is formatted and accessible.
 * @post ra_fs_write_file returns non-ok.
 *
 * @since 0.1.0
 */
static void test_find_bitmap_read_fail(void)
{
  ...
  const uint8_t  dummy = (uint8_t)'X';
  const ra_err_t r     = ra_fs_write_file(h, "X.TXT", &dummy, 1U);
  TEST_ASSERT(r != k_ra_ok);
 ...
 *
 * @par MC/DC:
 * Decision: `if (e != k_ra_ok)` in bitmap_scan -- 1 condition.
 * V1: e=error -> T -> early return (this test).
 * V2: e=k_ra_ok -> F -> process bitmap byte (normal scan path).
 *
 * @pre Volume is formatted and accessible.
 * @post ra_fs_write_file returns non-ok.
 *
 ...
 * @brief `priv_exfat_bitmap_scan` returns `k_ra_err_no_mem` when every cluster
 ...
 *          and returns `k_ra_err_no_mem` at line 143.
 *
 ...
 * V2: free bit found -> k_ra_ok (normal write_file path).
 *
 * @pre Volume is formatted and accessible.
 * @post ra_fs_write_file returns k_ra_err_no_mem.
 *
 * @since 0.1.0
 */
static void test_bitmap_scan_full_volume(void)
{
  ...
  const uint8_t dummy = (uint8_t)'X';
  TEST_ASSERT_EQ(k_ra_err_no_mem, ra_fs_write_file(h, "X.TXT", &dummy, 1U));
 ...
 *
 * @par MC/DC:
 * Decision: `if (we != k_ra_ok)` in bmp_switch -- 1 condition.
 * V1: we=error -> T -> return write error (this test).
 * V2: we=k_ra_ok -> F -> proceed to read (test_bmp_switch_sector_change).
 *
 * @pre Volume is formatted and accessible.
...
static void test_bmp_switch_write_fails(void)
{
  ...
  const ra_err_t r = priv_exfat_bmp_switch(h, h->first_fat_lba + 1U, &loaded, sec);
  TEST_ASSERT(r != k_ra_ok);
 ...
 *          the write of the old sector succeeds (line 153, we=k_ra_ok) and the
 *          read of the new sector succeeds (line 157), returning k_ra_ok.
 *
 ...
 *
 * @pre Volume is formatted and accessible.
 * @post priv_exfat_bmp_switch returns k_ra_ok.
 *
 * @since 0.1.0
 */
static void test_bmp_switch_sector_change(void)
{
  TEST_BEGIN("exfat write cov: bmp_switch flush+load -> k_ra_ok (lines 153,154,157)");
  build_exfat_volume();
  ra_fs_mount_t* h = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_ctrl_backend, &h));
  ...
  uint32_t loaded               = h->first_fat_lba;
  uint8_t  sec[k_wc_block_size] = {};
  TEST_ASSERT_EQ(k_ra_ok, priv_exfat_bmp_switch(h, h->first_fat_lba + 1U, &loaded, sec));

  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  free_volume();
  TEST_END("exfat write cov: bmp_switch flush+load -> k_ra_ok (lines 153,154,157)");
}
 ...
 *
 * @par MC/DC:
 * Decision: `if (e != k_ra_ok)` in bmp_switch read path -- 1 condition.
 * V1: e=error -> T -> return error (this test).
 * V2: e=k_ra_ok -> F -> loaded updated, k_ra_ok returned (normal path).
 *
 * @pre Volume is formatted and accessible.
 * @post ra_fs_write_file returns non-ok.
 *
 ...
 *
 * @par MC/DC:
 * Decision: `if (e != k_ra_ok)` in write_data -- 1 condition.
 * V1: e=error -> T -> early return (this test).
 * V2: e=k_ra_ok -> F -> continue sector loop (normal write path).
 *
 * @pre Volume is formatted and accessible.
 * @post ra_fs_write_file returns non-ok.
 *
 ...
 *
 * @par MC/DC:
 * Decision: `if (r != k_ra_ok)` in find_dir_space -- 1 condition.
 * V1: r=error -> T -> return r (this test).
 * V2: r=k_ra_ok -> F -> examine slot (normal scan path).
 *
 * @pre Volume is formatted and accessible.
 * @post ra_fs_write_file returns non-ok.
 *
 ...
 *        `k_ra_err_no_mem` when FAT[root] is EOC (lines 356-358, 361, 362, 531).
 *
 ...
 *          function returns `k_ra_err_no_mem` at line 362.  `priv_exfat_link`
 ...
 *          k_ra_ok` true and exercises line 359.
 *
 ...
 *
 * @par MC/DC:
 * Decision: `if (fe != k_ra_ok)` in find_dir_space FAT chain -- 1 condition.
 * V1: fe=error -> T -> return fe (this test).
 * V2: fe=k_ra_ok -> F -> is_eoc check (test_dir_space_full_root_eoc).
 *
 * @pre Volume is formatted and accessible.
 * @post ra_fs_write_file returns non-ok.
 *
 ...
 * 3 entries free, returning k_ra_ok.  The file is created successfully.
 *
 ...
 *
 * @pre Volume is formatted and accessible.
 * @post ra_fs_write_file returns k_ra_ok.
 *
 * @since 0.1.0
 */
static void test_dir_space_chain(void)
{
  ...
  const uint8_t dummy = (uint8_t)'X';
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_write_file(h, "X.TXT", &dummy, 1U));
 ...
 *
 * @par MC/DC:
 * Decision: `if (e != k_ra_ok)` in write_dir_set read -- 1 condition.
 * V1: e=error -> T -> return e (this test).
 * V2: e=k_ra_ok -> F -> patch and write (normal write_dir_set path).
 *
 * @pre Volume is formatted and accessible.
 * @post ra_fs_write_file returns non-ok.
 *
 ...
 *
 * @par MC/DC:
 * Decision: `if (e != k_ra_ok)` in write_dir_set write -- 1 condition.
 * V1: e=error -> T -> return e (this test).
 * V2: e=k_ra_ok -> F -> next entry (normal write_dir_set path).
 *
 * @pre Volume is formatted and accessible.
 * @post ra_fs_write_file returns non-ok.
 *
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra_fs_fat_exfat_write_cov.c\n");
  return 0;
}
