...
static void test_init_fs_returns_ok(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_hcdc_init(k_ra_usb_speed_fs));
...
static void test_init_bad_speed(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_hcdc_init((ra_usb_speed_t)9U));
...
static void test_attach_callback_fires_once(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_usb_hcdc_attach_callback(stub_on_attach, (void*)k_test_hcdc_ctx_token));
  ...
  for (uint8_t i = 0U; i < k_test_hcdc_max_steps; ++i) {
    ...
    ra_usb_fs()->DCPCTR = 0U;
    TEST_ASSERT_EQ(k_ra_ok, ra_usb_hcdc_step());
  }
...
static void test_send_null_arg_rejection(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_hcdc_send(nullptr, 0U));

  /* Walk through enumeration so we are post-attach. */
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_hcdc_attach_callback(stub_on_attach, nullptr));
  for (uint8_t i = 0U; i < k_test_hcdc_max_steps; ++i) {
    if (s_attach_count != 0U) {
      break;
    }
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_hcdc_send(nullptr, 8U));
...
static void test_recv_null_arg_rejection(void)
{
  ...
  uint8_t  buf[8] = {};
  uint16_t got    = 0U;
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_hcdc_recv(nullptr, sizeof(buf), &got));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_hcdc_recv(buf, sizeof(buf), nullptr));

  /* Pre-attach with valid pointers should still fail with invalid_state. */
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_hcdc_recv(buf, sizeof(buf), &got));
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_hcdc_recv(buf, 0U, &got));
...
static void test_set_line_coding_arg_rejection(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_state,
                 ra_usb_hcdc_set_line_coding(9600U, k_ra_hcdc_parity_none, k_ra_hcdc_stop_1));
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_usb_hcdc_set_line_coding(0U, k_ra_hcdc_parity_none, k_ra_hcdc_stop_1));
  /* Out-of-range parity. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_usb_hcdc_set_line_coding(9600U, (ra_usb_hcdc_parity_t)99U, k_ra_hcdc_stop_1));
  /* Out-of-range stop bits. */
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_usb_hcdc_set_line_coding(9600U, k_ra_hcdc_parity_none, (ra_usb_hcdc_stop_bits_t)99U));

  /* Valid call succeeds (DCPCTR has SUREQ cleared). */
  ra_usb_fs()->DCPCTR = 0U;
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_usb_hcdc_set_line_coding(115200U, k_ra_hcdc_parity_even, k_ra_hcdc_stop_2));
...
static void test_pre_init_guards(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_hcdc_attach_callback(stub_on_attach, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_hcdc_step());

  uint8_t  buf[4] = {};
  uint16_t got    = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_hcdc_send(buf, sizeof(buf)));
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_usb_hcdc_recv(buf, sizeof(buf), &got));
...
static void test_host_set_uact_and_bus_reset(void)
{
  ...
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_host_init(k_ra_usb_speed_fs));

  TEST_ASSERT_EQ(k_ra_ok, ra_usb_host_set_uact(k_ra_usb_speed_fs, true));
  volatile r_usb_regs_t* reg = ra_usb_fs();
  TEST_ASSERT((reg->DVSTCTR0 & (uint16_t)0x10U) != 0U); /* UACT bit 4. */

  TEST_ASSERT_EQ(k_ra_ok, ra_usb_host_bus_reset(k_ra_usb_speed_fs, true));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_host_bus_reset(k_ra_usb_speed_fs, false));
  TEST_ASSERT_EQ(0, (reg->DVSTCTR0 & (uint16_t)0x40U));

  /* Bogus speed rejected. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_host_set_uact((ra_usb_speed_t)9U, true));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_host_bus_reset((ra_usb_speed_t)9U, true));

  TEST_ASSERT_EQ(k_ra_ok, ra_usb_host_deinit(k_ra_usb_speed_fs));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_host_init((ra_usb_speed_t)9U));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_host_deinit((ra_usb_speed_t)9U));
...
static void test_host_setup_request_validates(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_usb_host_setup_request(k_ra_usb_speed_fs, nullptr));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_host_setup_request((ra_usb_speed_t)9U, &setup));

  /* Clean SUREQ first; success path. */
  ra_usb_fs()->DCPCTR = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_host_setup_request(k_ra_usb_speed_fs, &setup));
  /* The setup mirror registers should now hold the request. */
  TEST_ASSERT_EQ(0x0680U, ra_usb_fs()->USBREQ);
  TEST_ASSERT_EQ(0x0100U, ra_usb_fs()->USBVAL);
  TEST_ASSERT_EQ(18U, ra_usb_fs()->USBLENG);

  /* SUREQ still asserted -> next call gets busy. */
  TEST_ASSERT_EQ(k_ra_err_busy, ra_usb_host_setup_request(k_ra_usb_speed_fs, &setup));
...
static void test_mcdc_hcdc(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_usb_hcdc_send(nullptr, 4U));
