/**
 * @file test_ra_rsip_devsec_failclosed.c
 ...
 *
 * @details
 * ``ra_rsip_devsec.c`` (device lifecycle / debug authorisation / tamper /
 ...
 * ``#if defined(RA_INSECURE_STUB_CRYPTO) || defined(RA_SIMULATOR_MODE)`` and the
 * production ``#else`` returns ``k_ra_err_not_supported`` for every entry point,
 * writing no fabricated state.
 *
 * The rest of the host build compiles ``ra_rsip_devsec.c`` under
 * ``RA_SIMULATOR_MODE`` (the guarded ``#if`` branch -- exercised by ``test_life``
 * / ``test_debug_level`` / ``test_tamper`` in ``test_ra_rsip.c``). This target
 * is registered by hand in ``tests/CMakeLists.txt`` and rebuilds JUST that TU
 * with the two guard flags UNDEFINED (``-URA_SIMULATOR_MODE
 * -URA_INSECURE_STUB_CRYPTO``), so the fail-closed production ``#else`` is the
 ...
 * return ``k_ra_err_not_supported`` and every output sentinel must be left
 * untouched -- if the sim ``#if`` branch were compiled instead, the getters
 * would return ``k_ra_ok`` and overwrite the sentinels, and this test would
...
static void test_tamper_dpa_fail_closed(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_rsip_dpa_arm(true));
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_rsip_dpa_arm(false));
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr,
                "[OK ] test_ra_rsip_devsec_failclosed.c -- RSIP device-security fails closed\n");
  return 0;
}
