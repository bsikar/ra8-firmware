 ...
 * s_fake.actcsr (file-static to the TU).  The halt used in reset_world() sets
 ...
 * "not active" branch.  Because s_fake is a file-static struct initialized to
 ...
 * @post s_fake.actcsr is unchanged (not accessible from here).
 ...
 *
 * @details
 * At process start s_fake.actcsr == 0 (zero-initialized static).  The ACT
...
static void test_dc_cov_is_running_before_release(void)
{
  ...
  /* s_fake.actcsr starts at 0: ACT bit is clear -> active=false -> early return false. */
 ...
 * pass all validation checks, the fake shim sets ACT synchronously on the
 ...
 * @pre ra8_fake_mmap_reset() called in reset_mmio().
 * @post s_fake.actcsr has the ACT bit set.
 ...
 *
 * @pre s_fake.actcsr has ACT=1 from the preceding release test.
 * @post s_fake.waitcr has CPUWAIT=1.
 ...
 *
 * @details
 * Arms the ra8_fake_mmio fault seam on the CPU1ACTCSR key so the ACT poll
 ...
 * success the deleted RA8_OFF_TARGET short-circuit used to return. Runs
 ...
 *
 * @pre The ra8_fake_mmio seam is reset then armed to fail on the ACTCSR key.
...
static void test_dc_cov_release_act_timeout(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fake_mmio_fail_wait(ra8_dual_core_test_actcsr_key()));
  TEST_ASSERT_EQ(
    k_ra8_err_timeout,
    ra8_cpu1_release((void*)(uintptr_t)k_dc_cov_dummy_entry, (void*)(uintptr_t)k_dc_cov_dummy_sp));
  ra8_fake_mmio_reset();
 ...
 * first test so that s_fake.actcsr is still at its zero-initialized value.
...
int main(void)
{
  ...
  /* Validation-path tests: no release succeeds here, s_fake.actcsr stays 0. */
