/**
 * @file test_ra_epub_xml_shim_cov.cpp
 * @brief White-box line-coverage tests for libs/ra_epub/src/ra_epub_xml_shim.cpp.
 *
 * @par Tag
 * [Ring 4 / EPUB] {World: NS}
 *
 * @details
 * The MC/DC vector suite in `test_ra_epub_xml_shim.cpp` drives the four
 ...
 * renamed (`ra_epub_xml_parse_*` -> `ra_epub_xml_parse_*_cov`) so they do
 * not collide with the production copy linked from `ra_core_hal`, then
 ...
 * clash with the production `ra_epub_xml_parse_*` linked from ra_core_hal.
 * Both the declarations (top of the TU) and the definitions are renamed
 * consistently by these macros. */
#define ra_epub_xml_parse_container ra_epub_xml_parse_container_cov
#define ra_epub_xml_parse_opf       ra_epub_xml_parse_opf_cov
#define ra_epub_xml_parse_ncx       ra_epub_xml_parse_ncx_cov
#define ra_epub_xml_parse_nav       ra_epub_xml_parse_nav_cov
 ...
 * resolve from ra_core_hal (only the header is included transitively). */
#include "ra_epub_xml_shim.cpp" // NOLINT(bugprone-suspicious-include) -- white-box copy
 ...
 *    stored count clamps to the cap (::k_ra_epub_max_fonts /
 *    ::k_ra_epub_max_manifest).
 *  - `collect_manifest_items` also skips non-`<item>` children
 *    (`if (!elem_local_is(item, "item")) continue;`).
 * `toc_emit`'s cap guard (`toc_count >= k_ra_epub_max_toc`) is driven by
...
void test_cov_collect_guards_and_caps(void)
{
  ...
  /* Font cap: feed more font items than the cap; count clamps. */
  {
    ...
    for (int i = 0; i < static_cast<int>(k_ra_epub_max_fonts) + 3; ++i) {
      xml += "<item id=\"f";
      xml += std::to_string(i);
      xml += "\" href=\"f";
      xml += std::to_string(i);
    ...
    assert(book.embedded_font_count == static_cast<uint16_t>(k_ra_epub_max_fonts));
  }
   ...
   * over-cap run of items; the stray is skipped, the count clamps. */
  {
    ...
    for (int i = 0; i < static_cast<int>(k_ra_epub_max_manifest) + 2; ++i) {
      xml += "<item id=\"i";
      xml += std::to_string(i);
      xml += "\" href=\"h";
      xml += std::to_string(i);
    ...
    assert(book.manifest_count == static_cast<uint16_t>(k_ra_epub_max_manifest));
  }

  /* toc_emit cap: pre-seed the count to the cap, the emit is dropped. */
  std::memset(&book, 0, sizeof(book));
  book.toc_count = static_cast<uint16_t>(k_ra_epub_max_toc);
  toc_emit(&book, "dropped", "dropped.xhtml", 0U);
  assert(book.toc_count == static_cast<uint16_t>(k_ra_epub_max_toc));
 ...
 * reaches the identifier copy and returns k_ra_ok.
 * -----------------------------------------------------------------------
 */
...
void test_cov_find_identifier_variants(void)
{
  ...
  assert(ra_epub_xml_parse_opf_cov(match, std::strlen(k_opf_id_match), &book) == k_ra_ok);
  assert(std::strcmp(book.identifier, "ID-MATCH") == 0);

  std::memset(&book, 0, sizeof(book));
  const auto* noattr = reinterpret_cast<const uint8_t*>(k_opf_id_no_attr);
  assert(ra_epub_xml_parse_opf_cov(noattr, std::strlen(k_opf_id_no_attr), &book) == k_ra_ok);
  assert(std::strcmp(book.identifier, "ID-NOATTR") == 0);

  std::memset(&book, 0, sizeof(book));
  const auto* nouid = reinterpret_cast<const uint8_t*>(k_opf_no_uid);
  assert(ra_epub_xml_parse_opf_cov(nouid, std::strlen(k_opf_no_uid), &book) == k_ra_ok);
 ...
 *  - zero length -> k_ra_err_invalid_size.
 *  - malformed XML (mismatched tags) -> parse fails -> k_ra_err_validation_failed.
 *  - a comment-only document parses but exposes no root element ->
 *    k_ra_err_validation_failed. (Should tinyxml2 instead treat it as a
...
void test_cov_entry_error_paths(void)
{
  ...
  assert(ra_epub_xml_parse_container_cov(nonnull, 0U, &res) == k_ra_err_invalid_size);
  assert(ra_epub_xml_parse_opf_cov(nonnull, 0U, &book) == k_ra_err_invalid_size);
  assert(ra_epub_xml_parse_ncx_cov(nonnull, 0U, &book) == k_ra_err_invalid_size);
  assert(ra_epub_xml_parse_nav_cov(nonnull, 0U, &book) == k_ra_err_invalid_size);

  /* Malformed XML -> parse failure. */
  constexpr const char* k_bad = "<a><b></c>";
  const auto*           bad   = reinterpret_cast<const uint8_t*>(k_bad);
  assert(ra_epub_xml_parse_container_cov(bad, std::strlen(k_bad), &res) ==
         k_ra_err_validation_failed);
  assert(ra_epub_xml_parse_opf_cov(bad, std::strlen(k_bad), &book) == k_ra_err_validation_failed);
  assert(ra_epub_xml_parse_ncx_cov(bad, std::strlen(k_bad), &book) == k_ra_err_validation_failed);
  assert(ra_epub_xml_parse_nav_cov(bad, std::strlen(k_bad), &book) == k_ra_err_validation_failed);
  ...
  assert(ra_epub_xml_parse_container_cov(noroot, std::strlen(k_noroot), &res) ==
         k_ra_err_validation_failed);
  assert(ra_epub_xml_parse_opf_cov(noroot, std::strlen(k_noroot), &book) ==
         k_ra_err_validation_failed);
  assert(ra_epub_xml_parse_ncx_cov(noroot, std::strlen(k_noroot), &book) ==
         k_ra_err_validation_failed);
  assert(ra_epub_xml_parse_nav_cov(noroot, std::strlen(k_noroot), &book) ==
         k_ra_err_validation_failed);
 ...
 *  - parse_opf chapter cap: a spine longer than ::k_ra_epub_max_chapters
...
void test_cov_spine_and_nav_edges(void)
{
  ...
  assert(ra_epub_xml_parse_opf_cov(spine, std::strlen(k_opf_spine_edges), &book) == k_ra_ok);
   ...
   * trips the k_ra_epub_max_chapters guard. */
  {
    std::string xml =
      "<?xml version=\"1.0\"?>"
      "<package xmlns=\"http://www.idpf.org/2007/opf\" version=\"3.0\"><metadata/>"
      ...
      "<spine>";
    for (int i = 0; i < static_cast<int>(k_ra_epub_max_chapters) + 1; ++i) {
      xml += "<itemref idref=\"c\"/>";
    }
    xml += "</spine></package>";
    std::memset(&book, 0, sizeof(book));
    const auto* over = reinterpret_cast<const uint8_t*>(xml.c_str());
    assert(ra_epub_xml_parse_opf_cov(over, xml.size(), &book) == k_ra_err_no_mem);
  }

  /* NCX bare navPoint -> empty first entry, real second entry. */
  std::memset(&book, 0, sizeof(book));
  const auto* ncx = reinterpret_cast<const uint8_t*>(k_ncx_bare_navpoint);
  assert(ra_epub_xml_parse_ncx_cov(ncx, std::strlen(k_ncx_bare_navpoint), &book) == k_ra_ok);
  ...
  assert(ra_epub_xml_parse_nav_cov(nav, std::strlen(k_nav_span_li), &book) == k_ra_ok);
  assert(book.toc_count == 2U);
