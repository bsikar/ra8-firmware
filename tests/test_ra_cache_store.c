/**
 * @file test_ra_cache_store.c
 * @brief Unit tests for ra_cache_store over LevelX standalone + a RAM NOR sim (#201).
 *
 * @details
 * Drives the persistent key->blob store end to end on the host: put/get random
 * reads (through the `ra_vsource` streaming shape), duplicate rejection,
...
static ra_cache_store_entry_t s_index[k_t_index_cap];
 ...
 * @since 0.1.0
 */
static ra_cache_store_cfg_t t_cfg(LX_NOR_FLASH* f, bool format)
{
  return (ra_cache_store_cfg_t){.nor_flash         = f,
                                ...
                                .overprovision_pct = (uint8_t)k_ra_cache_store_overprov_pct,
                                .format            = format};
}
 ...
 * out-of-range reads through ra_cache_store_read)
 */
static void test_put_get_roundtrip(void)
{
  ...
  lx_nor_sim_ram_wipe();
  ra_cache_store_t     st  = {};
  ra_cache_store_cfg_t cfg = t_cfg(t_next_flash(), true);
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_init(&st, &cfg));

  uint8_t data[1500];
  t_fill(data, sizeof(data), 7U);
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_put(&st, k_t_key_a, data, sizeof(data)));

  ra_cache_store_reader_t rd = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_get(&st, k_t_key_a, &rd));
  TEST_ASSERT_EQ(1500, rd.byte_len);

  uint8_t out[1500];
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_read(&rd, 0, out, sizeof(out)));
  TEST_ASSERT(memcmp(out, data, sizeof(data)) == 0);

  uint8_t slice[300];
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_read(&rd, 400, slice, sizeof(slice)));
  TEST_ASSERT(memcmp(slice, &data[400], sizeof(slice)) == 0);

  TEST_ASSERT_EQ(k_ra_err_out_of_range, ra_cache_store_read(&rd, 1400, out, 200));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_close(&st));
  TEST_END("put/get roundtrip");
}

/**
 * @test vsource_integration
 * @brief A cached entry pages through ra_vsource_add_paged / ra_vsource_loader.
 * @par MC/DC:
 * (no compound decisions under test -- binds ra_cache_store_read into a vsource
...
static void test_vsource_integration(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_vsource_add_paged(&vs, ra_cache_store_read, &rd, 0U, rd.byte_len, &id));

  uint8_t frame[512];
  TEST_ASSERT_EQ(k_ra_ok, ra_vsource_loader(&vs, id, 0U, frame, sizeof(frame)));
  TEST_ASSERT(memcmp(frame, data, sizeof(frame)) == 0);

  /* Bytes 1024..1499 = 476 real bytes then zero padding. */
  TEST_ASSERT_EQ(k_ra_ok, ra_vsource_loader(&vs, id, 1024U, frame, sizeof(frame)));
  TEST_ASSERT(memcmp(frame, &data[1024], 476U) == 0);
  for (uint32_t i = 476U; i < sizeof(frame); i++) {
...
static void test_put_edge_cases(void)
{
  ...
  ra_cache_store_reader_t rd = {};
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_cache_store_get(&st, k_t_key_a, &rd));

  uint8_t data[600];
  t_fill(data, sizeof(data), 1U);
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_put(&st, k_t_key_a, data, sizeof(data)));
  TEST_ASSERT_EQ(k_ra_err_exists, ra_cache_store_put(&st, k_t_key_a, data, sizeof(data)));

  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_cache_store_put(&st, k_t_key_b, NULL, 8U));
  TEST_ASSERT_EQ(k_ra_err_invalid_size, ra_cache_store_put(&st, k_t_key_b, data, 0U));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_cache_store_put(NULL, k_t_key_b, data, 8U));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_close(&st));
...
static void test_evict_reuse(void)
{
  ...
  uint8_t data[1000];
  t_fill(data, sizeof(data), 9U);
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_put(&st, k_t_key_a, data, sizeof(data)));
  uint32_t live_after_put = st.live_sectors;
  TEST_ASSERT(live_after_put > 0U);

  TEST_ASSERT_EQ(k_ra_err_not_found, ra_cache_store_evict(&st, k_t_key_b));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_evict(&st, k_t_key_a));
...
static void test_pin_blocks_evict(void)
{
  ...
  uint8_t data[300];
  t_fill(data, sizeof(data), 5U);
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_put(&st, k_t_key_a, data, sizeof(data)));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_pin(&st, k_t_key_a, true));
  TEST_ASSERT_EQ(k_ra_err_busy, ra_cache_store_evict(&st, k_t_key_a));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_pin(&st, k_t_key_a, false));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_evict(&st, k_t_key_a));
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_cache_store_pin(&st, k_t_key_b, true));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_close(&st));
...
static void test_recovery_clean(void)
{
  ...
  uint8_t a[700];
  uint8_t b[1300];
  t_fill(a, sizeof(a), 11U);
  t_fill(b, sizeof(b), 22U);
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_put(&st, k_t_key_a, a, sizeof(a)));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_put(&st, k_t_key_b, b, sizeof(b)));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_pin(&st, k_t_key_a, true));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_close(&st));

  /* Reboot: fresh control block over the same (persisted) backing. */
  ra_cache_store_t     st2  = {};
  ra_cache_store_cfg_t cfg2 = t_cfg(t_next_flash(), false);
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_init(&st2, &cfg2));

  ra_cache_store_reader_t rd = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_get(&st2, k_t_key_a, &rd));
  uint8_t out[700];
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_read(&rd, 0, out, sizeof(out)));
  TEST_ASSERT(memcmp(out, a, sizeof(a)) == 0);
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_get(&st2, k_t_key_b, &rd));

  /* Pin survived the checkpoint. */
  TEST_ASSERT_EQ(k_ra_err_busy, ra_cache_store_evict(&st2, k_t_key_a));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_evict(&st2, k_t_key_b));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_close(&st2));
...
static void test_recovery_crash_replay(void)
{
  ...
  uint8_t a[700];
  uint8_t b[900];
  t_fill(a, sizeof(a), 44U);
  t_fill(b, sizeof(b), 55U);
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_put(&st, k_t_key_a, a, sizeof(a)));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_put(&st, k_t_key_b, b, sizeof(b)));
  ...
  int32_t sb = ra_cs_index_find(&st, k_t_key_b);
  TEST_ASSERT(sb >= 0);
  uint32_t torn_header = st.index[sb].start_sector + st.index[sb].sector_count;
  uint8_t  garbage[512];
  t_fill(garbage, sizeof(garbage), 0x5AU);
  TEST_ASSERT_EQ(k_ra_ok, ra_cs_sector_write(&st, torn_header + 1U, garbage));
  ...
  uint8_t c[400];
  t_fill(c, sizeof(c), 66U);
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_put(&st2, k_t_key_c, c, sizeof(c)));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_get(&st2, k_t_key_c, &rd));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_close(&st2));
...
static void test_helper_guards(void)
{
  ...
  TEST_ASSERT_EQ(0, ra_cs_crc32(NULL, 8U));
  TEST_ASSERT_EQ(0, ra_cs_crc32(buf, 0U));
  TEST_ASSERT(ra_cs_crc32(buf, 8U) != 0U);

  ra_cache_store_t empty = {};
  TEST_ASSERT_EQ(-1, ra_cs_index_find(NULL, 0U));
  TEST_ASSERT_EQ(-1, ra_cs_index_find(&empty, 0U)); /* index == NULL */
  TEST_ASSERT_EQ(-1, ra_cs_index_add(NULL, 0U, 0U, 1U, 0U, false));
  TEST_ASSERT_EQ(-1, ra_cs_index_add(&empty, 0U, 0U, 1U, 0U, false)); /* index == NULL */

  uint8_t sect[512];
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_cs_sector_read(NULL, 0U, sect));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_cs_sector_read(&empty, 0U, NULL));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_cs_sector_write(NULL, 0U, sect));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_cs_sector_release(NULL, 0U));
...
static void test_init_validation(void)
{
  ...
  ra_cache_store_t     st   = {};
  ra_cache_store_cfg_t base = t_cfg(t_next_flash(), true);

  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_cache_store_init(NULL, &base));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_cache_store_init(&st, NULL));

  ra_cache_store_cfg_t c = base;
  c.nor_flash            = NULL;
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_cache_store_init(&st, &c));
  c                 = base;
  ...
  c           = base;
  c.index_cap = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_size, ra_cache_store_init(&st, &c));
  c               = base;
  c.staging_bytes = 8U;
  TEST_ASSERT_EQ(k_ra_err_invalid_size, ra_cache_store_init(&st, &c));
  c                   = base;
  c.overprovision_pct = 99U;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_cache_store_init(&st, &c));
  c                 = base;
  ...
  ra_cache_store_reader_t rd   = {};
  uint8_t                 d[4] = {};
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_cache_store_put(&st, 1U, d, 4U));
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_cache_store_get(&st, 1U, &rd));
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_cache_store_evict(&st, 1U));
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_cache_store_pin(&st, 1U, true));
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_cache_store_sync(&st));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_cache_store_read(NULL, 0U, d, 4U));
...
static void test_limits_and_sync(void)
{
  ...
  for (uint32_t i = 0U; i < (uint32_t)k_t_index_cap; i++) {
    TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_put(&st, 1000U + i, tiny, sizeof(tiny)));
  }
  /* Index full -> no_mem. */
  TEST_ASSERT_EQ(k_ra_err_no_mem, ra_cache_store_put(&st, 9999U, tiny, sizeof(tiny)));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_sync(&st));

  /* Free the index, then exhaust the sector budget with big entries. */
  for (uint32_t i = 0U; i < (uint32_t)k_t_index_cap; i++) {
    TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_evict(&st, 1000U + i));
  }
  ...
  for (uint32_t i = 0U; i < 20U; i++) {
    rc = ra_cache_store_put(&st, 2000U + i, big, sizeof(big));
    if (rc != k_ra_ok) {
      break;
    }
    n++;
  }
  TEST_ASSERT_EQ(k_ra_err_no_mem, rc); /* budget (overprovisioned) exhausted */
  TEST_ASSERT(n > 0U);
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_close(&st));
...
static void test_evict_then_crash(void)
{
  ...
  uint8_t a[700];
  uint8_t b[500];
  t_fill(a, sizeof(a), 12U);
  t_fill(b, sizeof(b), 34U);
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_put(&st, k_t_key_a, a, sizeof(a)));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_put(&st, k_t_key_b, b, sizeof(b)));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_evict(&st, k_t_key_a));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_init(&st2, &cfg2));
  ra_cache_store_reader_t rd = {};
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_cache_store_get(&st2, k_t_key_a, &rd));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_get(&st2, k_t_key_b, &rd));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_close(&st2));
 ...
 * @since 0.1.0
 */
static void t_plant_super(ra_cache_store_t* st, uint32_t magic, bool good_crc)
{
  ra_cs_super_t sb = {.magic           = magic,
                      .version         = (uint32_t)k_ra_cs_format_version,
                      .seq             = st->next_seq,
                      .clean           = (uint32_t)k_ra_cs_clean,
                      .entry_count     = 0U,
                      .live_sectors    = 0U,
                      ...
                      .crc             = 0U};
  sb.crc = good_crc ? ra_cs_crc32((const uint8_t*)&sb, (uint32_t)(sizeof(sb) - sizeof(sb.crc)))
                    : 0xDEADBEEFU;
  uint8_t buf[512];
  (void)memset(buf, 0, sizeof(buf));
  (void)memcpy(buf, &sb, sizeof(sb));
  TEST_ASSERT_EQ(k_ra_ok, ra_cs_sector_write(st, 0U, buf));
}
...
static void test_corrupt_super_replays(void)
{
  ...
  for (uint32_t variant = 0U; variant < 2U; variant++) {
    ...
    TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_init(&st, &cfg));
    uint8_t a[600];
    t_fill(a, sizeof(a), (uint8_t)(70U + variant));
    TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_put(&st, k_t_key_a, a, sizeof(a)));

    if (variant == 0U) {
      t_plant_super(&st, (uint32_t)k_ra_cs_super_magic, false); /* good magic, bad CRC */
    } else {
      t_plant_super(&st, 0x0BADC0DEU, true); /* bad magic */
    }
 ...
 * @since 0.1.0
 */
static void t_plant_hdr(ra_cache_store_t* st,
                        uint32_t          at_sector,
                        uint32_t          self_field,
                        uint16_t          count,
                        bool              good_crc)
{
  ra_cs_entry_hdr_t h = {.magic        = (uint32_t)k_ra_cs_entry_magic,
                         .seq          = 1U,
                         .key          = 0xDEAD0000U + at_sector,
                         .byte_len     = 16U,
                         .start_sector = self_field,
                         .sector_count = count,
                         .flags        = 0U,
                         .hdr_crc      = 0U};
  h.hdr_crc = good_crc ? ra_cs_crc32((const uint8_t*)&h, (uint32_t)(sizeof(h) - sizeof(h.hdr_crc)))
                       : 0x1234ABCDU;
  uint8_t buf[512];
  (void)memset(buf, 0, sizeof(buf));
  (void)memcpy(buf, &h, sizeof(h));
  TEST_ASSERT_EQ(k_ra_ok, ra_cs_sector_write(st, at_sector, buf));
}
...
static void test_scan_rejects_bad_headers(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_init(&st, &cfg));
  uint8_t a[600];
  t_fill(a, sizeof(a), 21U);
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_put(&st, k_t_key_a, a, sizeof(a)));

  t_plant_hdr(&st, 20U, 999U, 2U, true);    /* start_sector mismatch   */
  t_plant_hdr(&st, 22U, 22U, 0U, true);     /* sector_count == 0       */
  t_plant_hdr(&st, 24U, 24U, 60000U, true); /* run runs past the media */
  t_plant_hdr(&st, 26U, 26U, 2U, false);    /* bad CRC                 */
  ...
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_cache_store_get(&st2, 0xDEAD0014U, &rd));
  TEST_ASSERT_EQ(k_ra_ok, ra_cache_store_close(&st2));
...
static void test_write_fault_propagates(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_hw_init_failed, ra_cache_store_put(&st, k_t_key_b, a, sizeof(a)));
  lx_nor_sim_ram_fail_writes(0U);
  TEST_END("write fault propagates");
}

/**
 * @brief Run every ra_cache_store test.
...
int main(void)
{
  ...
  (void)fprintf(stderr, "[DONE] ra_cache_store\n");
  return 0;
}
