/**
 * @file test_ra_reflow_table.c
 * @brief Host unit tests + MC/DC for minimal `<table>` layout (#107).
 *
 * @details
 * Lays out tables through `ra_reflow` (with the fixed-metric Ahem face) and
 ...
 * libs/ra_reflow/src/ra_reflow_layout.c@priv_layout_cell -- the column-fit
 * wrap). N+1 = 3 vectors:
 ...
 * AND; libs/ra_reflow/src/ra_reflow_layout.c@priv_layout_row -- the row
...
int32_t main(void)
{
  ...
  test_cell_wrap_mcdc();
  test_row_break_mcdc();
  (void)fprintf(stderr, "[OK ] test_ra_reflow_table.c\n");
  return 0;
}
