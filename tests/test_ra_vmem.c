/**
 * @file test_ra_vmem.c
 * @brief Unit tests for the ra_mem unified page cache (Layer 2, #147).
 *
 * @details
...
static ra_vmem_frame_t s_meta[(size_t)k_t_frames];
...
static ra_vmem_cfg_t t_cfg(void)
{
  ra_vmem_cfg_t cfg = {};
...
static void* t_get(ra_vmem_t* vm, uint32_t obj, uint32_t page)
{
  void*          p = nullptr;
  const ra_err_t e = ra_vmem_get(vm, obj, (uint64_t)page * (uint64_t)k_t_frame_bytes, &p);
...
static void test_scan_resistance(void)
{
  ...
  for (uint32_t pass = 0; pass < 2U; ++pass) {
    TEST_ASSERT_EQ(k_ra_ok, ra_vmem_put(&vm, t_get(&vm, 1U, 0U)));
    TEST_ASSERT_EQ(k_ra_ok, ra_vmem_put(&vm, t_get(&vm, 1U, 1U)));
  }
  /* Linear page-turn flood: 30 distinct probationary pages through 8 frames. */
  for (uint32_t pg = 100U; pg < 130U; ++pg) {
    TEST_ASSERT_EQ(k_ra_ok, ra_vmem_put(&vm, t_get(&vm, 1U, pg)));
  }
  /* The hot set must have SURVIVED the flood (protected segment). */
  uint32_t miss_before = 0;
  TEST_ASSERT_EQ(k_ra_ok, ra_vmem_stats(&vm, nullptr, &miss_before, nullptr));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_vmem_stats(&vm, nullptr, &miss_after, nullptr));
  TEST_ASSERT_EQ(miss_before, miss_after); /* both were hits -> no new misses */
  TEST_ASSERT_EQ(k_ra_ok, ra_vmem_put(&vm, h0));
  TEST_ASSERT_EQ(k_ra_ok, ra_vmem_put(&vm, h1));
...
static void test_pin_protection(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_no_mem,
                 ra_vmem_get(&vm, 2U, (uint64_t)k_t_frames * (uint64_t)k_t_frame_bytes, &extra));
  /* Release one pin; now the miss succeeds. */
  TEST_ASSERT_EQ(k_ra_ok, ra_vmem_put(&vm, pins[0]));
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_vmem_get(&vm, 2U, (uint64_t)k_t_frames * (uint64_t)k_t_frame_bytes, &extra));
  TEST_ASSERT_EQ(k_ra_ok, ra_vmem_put(&vm, extra));
  for (uint32_t i = 1; i < (uint32_t)k_t_frames; ++i) {
    TEST_ASSERT_EQ(k_ra_ok, ra_vmem_put(&vm, pins[i]));
  }
...
static void test_validation(void)
{
  ...
  ra_vmem_cfg_t cfg = t_cfg();
  void*         p   = nullptr;
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_vmem_init(nullptr, &cfg));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_vmem_init(&vm, nullptr));
  ra_vmem_cfg_t bad = t_cfg();
  bad.frame_count   = 0U;
  TEST_ASSERT_EQ(k_ra_err_invalid_size, ra_vmem_init(&vm, &bad));

  TEST_ASSERT_EQ(k_ra_ok, ra_vmem_init(&vm, &cfg));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_vmem_get(nullptr, 0U, 0U, &p));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_vmem_get(&vm, 0U, 0U, nullptr));
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_vmem_put(&vm, nullptr));
  /* put of a non-frame pointer and of an unpinned frame */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_vmem_put(&vm, &s_frames[1])); /* mid-frame */
  void* fr = t_get(&vm, 9U, 9U);
  TEST_ASSERT_EQ(k_ra_ok, ra_vmem_put(&vm, fr));
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_vmem_put(&vm, fr)); /* already unpinned */
