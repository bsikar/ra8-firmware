...
/* ===========================================================================
 * MC/DC vector coverage for compound boolean decisions in ra_ssie.c.
 *
 * Per docs/MCDC_GAPS.csv there are 10 compound decisions in ra_ssie.c.
 ...
 * (2 conditions, libs/ra_hal/src/ra_ssie.c line 410 -- internal_validate_init_cfg)
 ...
 *
 * @par MC/DC:
 * Decision: `if (dir != k_ra_ssie_dir_rx && dir != k_ra_ssie_dir_tx &&
 *               dir != k_ra_ssie_dir_tx_rx)`
 * (3 conditions, libs/ra_hal/src/ra_ssie.c line 518 -- ra_ssie_start)
 *
...
static void test_mcdc_start_dir(void)
{
  ...
  TEST_ASSERT(r1 != k_ra_err_invalid_arg || r1 == k_ra_ok);
  ra_err_t r2 = ra_ssie_start((uint8_t)k_ra_ssie_test_ch0, k_ra_ssie_dir_tx);
  TEST_ASSERT(r2 != k_ra_err_invalid_arg || r2 == k_ra_ok);
 ...
 * (2 conditions, libs/ra_hal/src/ra_ssie.c line 623 -- ra_ssie_set_thresholds)
 * - V1: tx=0x10, rx=0x10 -> F,F -> ok
 * - V2: tx=0x40, rx=0x10 -> T short-circ -> invalid_arg
 * - V3: tx=0x10, rx=0x40 -> F,T -> invalid_arg
 */
 ...
 * (2 conditions, libs/ra_hal/src/ra_ssie.c line 735 -- internal_validate_dma_cfg)
 * Reached via ra_ssie_attach_dma(). want_tx = (tx_dma_channel < 8);
...
static void test_mcdc_attach_dma_neither(void)
{
  ...
  ra_err_t v1 = ra_ssie_attach_dma((uint8_t)k_ra_ssie_test_ch0, &dma1);
  /* The "neither" gate must not have rejected the call. Other ra_err_t
   * outcomes are acceptable (DMA layer may or may not be ready in sim). */
  (void)v1;
  /* V2 */
  ra_ssie_dma_cfg_t dma2 = {
  ...
  ra_err_t v2 = ra_ssie_attach_dma((uint8_t)k_ra_ssie_test_ch0, &dma2);
  (void)v2;
  /* V3 -- both invalid: must be rejected with invalid_arg by the "neither" gate. */
  ra_ssie_dma_cfg_t dma3 = {
    .tx_dma_channel = (uint8_t)k_ssie_mcdc_dma_bad,
 ...
 * (3 conditions, libs/ra_hal/src/ra_ssie.c line 738 -- internal_validate_dma_cfg)
 *
...
static void test_mcdc_attach_dma_tx_buffer(void)
{
  ...
  (void)ra_ssie_attach_dma((uint8_t)k_ra_ssie_test_ch0, &v1cfg);
  /* V2: want_tx=F (tx ch out of range), rx valid. */
  ra_ssie_dma_cfg_t v2cfg = {
    .tx_dma_channel = (uint8_t)k_ssie_mcdc_dma_bad,
    .rx_dma_channel = (uint8_t)k_ssie_mcdc_dma_valid,
    .tx_buffer      = nullptr,
    .rx_buffer      = rxbuf,
  ...
  (void)ra_ssie_attach_dma((uint8_t)k_ra_ssie_test_ch0, &v2cfg);
  /* V3: want_tx=T, tx_buffer=NULL -> invalid_arg. */
  ra_ssie_dma_cfg_t v3cfg = {
    .tx_dma_channel = (uint8_t)k_ssie_mcdc_dma_valid,
    .rx_dma_channel = (uint8_t)k_ssie_mcdc_dma_valid,
    .tx_buffer      = nullptr,
    .rx_buffer      = rxbuf,
 ...
 * (3 conditions, libs/ra_hal/src/ra_ssie.c line 741 -- internal_validate_dma_cfg)
 *
 ...
 * (2 conditions, libs/ra_hal/src/ra_ssie.c line 963 -- ra_ssie_attach_dma_pair)
