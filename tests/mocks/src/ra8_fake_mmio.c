...
/**
 * @brief Optional synchronous bounded-wait hook.
 * @details Runs before each ::ra8_fake_mmio_wait_eval decision so a test can
 * publish peripheral response data on the driver's thread.
 * @note Test-only. Not thread-safe (tests are single-threaded).
 */
static void (*s_wait_hook)(const volatile void* reg, uint32_t iter);
...
{
  if (s_wait_hook != nullptr) {
    s_wait_hook(reg, iter);
  }
