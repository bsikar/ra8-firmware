 ...
 *
 * @details
 * Mirrors examples/ek_ra8d2/hil_needs_revalidation/lpm_deep_standby_3_demo/src/main.c bring-up:
 * ra8_lpm_init -> ra8_lpm_arm_dpsier(2, DRTCAIE) ->
 * ra8_lpm_arm_wupen0_bits(RTCALM) ->
 * ra8_lpm_enter_sleep(k_ra8_sleep_mode_deep_standby_3). The host fake
 ...
 */
static void test_lpm_dpsby3_init_ok(void)
{
  reset_world();
  TEST_BEGIN("lpm_deep_standby_3_demo: init ok");
  const ra8_lpm_config_t cfg = make_demo_cfg();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_lpm_init(&cfg));
  TEST_END("lpm_deep_standby_3_demo: init ok");
}
 ...
 */
static void test_lpm_dpsby3_init_null(void)
{
  reset_world();
  TEST_BEGIN("lpm_deep_standby_3_demo: NULL cfg rejected");
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_lpm_init(nullptr));
  TEST_END("lpm_deep_standby_3_demo: NULL cfg rejected");
}
 ...
 */
static void test_lpm_dpsby3_arm_dpsier2_drtca(void)
{
  reset_world();
  TEST_BEGIN("lpm_deep_standby_3_demo: arm DPSIER2.DRTCAIE");
  const ra8_lpm_config_t cfg = make_demo_cfg();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_lpm_init(&cfg));
  TEST_ASSERT_EQ(
    k_ra8_ok,
  ...
  TEST_END("lpm_deep_standby_3_demo: arm DPSIER2.DRTCAIE");
}

/**
 * @brief Deep Standby 3 entry writes LPMD=0xA then HAL clears LPSCR.
 *
 * @par MC/DC:
 * Decision: ``ra8_lpm_enter_sleep != ok``. One atomic condition x 2
 * vectors -- valid mode (this) + invalid mode covered elsewhere.
 */
static void test_lpm_dpsby3_enter_ok(void)
{
  reset_world();
  TEST_BEGIN("lpm_deep_standby_3_demo: enter Deep Standby 3 ok");
  const ra8_lpm_config_t cfg = make_demo_cfg();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_lpm_init(&cfg));
  TEST_ASSERT_EQ(
    k_ra8_ok,
  ...
  TEST_END("lpm_deep_standby_3_demo: enter Deep Standby 3 ok");
}

int main(void)
{
  test_lpm_dpsby3_init_ok();
  test_lpm_dpsby3_init_null();
  test_lpm_dpsby3_arm_dpsier2_drtca();
  test_lpm_dpsby3_enter_ok();
  return 0;
}
