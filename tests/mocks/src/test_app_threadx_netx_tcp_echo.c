 ...
 *
 * @details
 * The production app at examples/ek_ra8d2/hw_validated/hil/threadx_netx_tcp_echo/src/main.c
 ...
 *
 * @par MC/DC:
 * No compound decision is present; this case verifies callback-slot
 * replacement through observable dispatch behavior.
 */
static void test_netx_attach_event_handler(void)
{
  ...
  ra8_eth_attach_handler(test_netx_eth_event_cb, (void*)k_eth_handler_ctx_token);
...
/**
 * @brief NULL handler detaches the callback slot.
 *
 * @par MC/DC:
 * No compound decision is present; this case verifies the documented NULL
 * detach contract through the same callback slot.
 */
static void test_netx_attach_null_handler_detaches(void)
{
  reset_world();
  TEST_ASSERT_EQ(k_ra8_ok, ra8_eth_init());
  TEST_BEGIN("netx_tcp_echo: attach with NULL fn detaches");
  /* The contract treats attach(NULL) as a detach. The NetX wrapper relies on
...
int main(void)
{
  ...
  test_netx_attach_null_handler_detaches();
  return 0;
}
