 ...
 *
 * @details
 * Mirrors examples/ek_ra8d2/hw_validated/hil/clock_check/src/main.c, which sequences
...
/**
 * @enum app_clock_check_fixture_t
 * @brief All-bits-set register values, so a write that clears the wrong field leaves evidence.
 */
typedef enum : uint8_t {
  k_sys_oscsf_all_ready =
    0xFFU, /**< All oscillator-stabilisation flag set, so clock bring-up sees all sources ready. */
} app_clock_check_fixture_t;
