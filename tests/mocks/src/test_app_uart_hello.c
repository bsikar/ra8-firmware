 ...
 * "hello" demo at examples/ek_ra8d2/hw_validated/hil/uart_hello/src/main.c. Exercises the full
 * bring-up sequence (PFS routing -> ra8_sci_init -> LED init) and
 * the steady-state loop (write_polling + led_toggle), with mocked
 * MMIO via the existing tests/mocks/src/ra8_fake_mmap.c.
 *
 * Exercised modules:
...
/** @brief Pin map mirrors examples/ek_ra8d2/hw_validated/hil/uart_hello/src/main.c PD_02 / PD_03. */
