 ...
 *
 * Tests use ``ra_sim_world_mark_ns(ptr, len)`` to declare a
 * region as "Non-Secure" and ``ra_sim_world_mark_s(ptr, len)``
 * to declare a region as "Secure". A subsequent
 * ``ra_sim_world_check_ns_range(ptr, len)`` returns whether the
...
/**
 * @enum ra_sim_world_limits_t
 */
typedef enum : uint8_t {
  k_ra_sim_world_max_regions = 16U,
} ra_sim_world_limits_t;

/**
 * @enum ra_sim_world_attr_t
 * @brief Attribute tag for a marked region.
 */
typedef enum : uint8_t {
  k_ra_sim_world_attr_secure = 0U,
  k_ra_sim_world_attr_ns     = 1U,
} ra_sim_world_attr_t;
