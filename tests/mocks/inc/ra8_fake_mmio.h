 ...
 * @post The optional synchronous poll and wait hooks are cleared.
 *
...
/**
 * @brief Register a synchronous hook invoked before each bounded-wait evaluation.
 *
 * @details
 * Unlike ::ra8_fake_mmio_set_poll_hook, this hook covers the shared
 * ::ra8_fake_mmio_wait_eval seam used by drivers whose waits were migrated from
 * off-target short-circuits. It runs on the driver's thread before the fake
 * evaluates the wait condition, so a test can publish response data and clear a
 * command bit at the exact point a peripheral would complete a transaction.
 * This keeps scripted bus read-back deterministic without a helper thread or
 * wall-clock timing. Pass ``nullptr`` to remove the hook;
 * ::ra8_fake_mmio_reset also clears it.
 *
 * @param[in] hook Function invoked with the polled register and zero-based wait
 *                 iteration, or ``nullptr`` to disable.
 *
 * @pre Called from a host (``RA8_OFF_TARGET`` + ``UNIT_TEST``) test binary.
 * @post Subsequent ::ra8_fake_mmio_wait_eval calls invoke @p hook first.
 *
 * @note Test-only. Not thread-safe. The hook runs inline with the driver.
 * @since 0.1.0
 */
void ra8_fake_mmio_set_wait_hook(void (*hook)(const volatile void* reg, uint32_t iter));
