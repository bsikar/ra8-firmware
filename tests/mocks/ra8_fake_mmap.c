 ...
 * fixed addresses, which ::ra8_fake_mmap_install checks against the
 ...
 * that do not define it. When set, ::ra8_fake_region_mappable skips the one
 ...
 *
 * @param[in] asan_shadow_gap ::ra8_fake_region_t::asan_shadow_gap of the region.
 *
 ...
 * @since 0.1.0
 */
static bool ra8_fake_region_mappable(bool asan_shadow_gap)
{
 ...
 * @since 0.1.0
 */
static void ra8_fake_mmap_check_overlap(uintptr_t image_lo, uintptr_t image_hi)
{
  for (uint8_t i = 0U; i < (uint8_t)k_ra8_fake_region_count; ++i) {
    const ra8_fake_region_t region = s_ra8_fake_regions[i];
    if (!ra8_fake_region_mappable(region.asan_shadow_gap)) {
      continue;
    }
    if ((region.base < image_hi) && (image_lo < (region.base + region.size))) {
      (void)fprintf(stderr,
                    "ra8_fake_mmap: window 0x%llx..0x%llx overlaps the loaded "
 ...
 * @since 0.1.0
 */
static void ra8_fake_mmap_map_regions(void)
{
  for (uint8_t i = 0U; i < (uint8_t)k_ra8_fake_region_count; ++i) {
    const ra8_fake_region_t region = s_ra8_fake_regions[i];
    if (!ra8_fake_region_mappable(region.asan_shadow_gap)) {
    ...
    if (p == MAP_FAILED || (uintptr_t)p != region.base) {
      (void)fprintf(stderr,
                    "ra8_fake_mmap: failed to map 0x%llx..0x%llx\n",
                    (unsigned long long)region.base,
                    (unsigned long long)region.base + (unsigned long long)region.size);
      abort();
    }
 ...
 * shadow-gap region is skipped up front (see ::ra8_fake_region_mappable).
 *
 ...
 */
[[gnu::constructor]] static void ra8_fake_mmap_install(int argc, char** argv, char** envp)
{
  (void)argc;
  (void)envp;
  ...
  ra8_fake_mmap_check_overlap(image_lo, image_hi);
  ra8_fake_mmap_map_regions();
