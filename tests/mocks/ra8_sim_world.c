...
void ra8_sim_world_reset(void)
{
  for (uint8_t i = 0U; i < (uint8_t)k_ra8_sim_world_max_regions; ++i) {
    s_regions[i].start  = 0U;
    s_regions[i].end    = 0U;
    s_regions[i].attr   = k_ra8_sim_world_attr_secure;
    s_regions[i].in_use = false;
  }
}

static ra8_err_t internal_mark(const void* ptr, uint32_t len, ra8_sim_world_attr_t attr)
{
  if (len == 0U) {
...
ra8_err_t ra8_sim_world_mark_ns(const void* ptr, uint32_t len)
{
  return internal_mark(ptr, len, k_ra8_sim_world_attr_ns);
}

ra8_err_t ra8_sim_world_mark_s(const void* ptr, uint32_t len)
{
  return internal_mark(ptr, len, k_ra8_sim_world_attr_secure);
}

bool ra8_sim_world_check_ns_range(const void* ptr, uint32_t len)
{
  if (len == 0U) {
    return false;
  }
  ...
  for (uint8_t i = 0U; i < (uint8_t)k_ra8_sim_world_max_regions; ++i) {
    ...
    if (overlaps && (s_regions[i].attr == k_ra8_sim_world_attr_secure)) {
      return false; /* Secure overlap kills the query. */
    }
    if ((s_regions[i].attr == k_ra8_sim_world_attr_ns) && (s_regions[i].start <= want_start) &&
