 ...
 *
 * @details
 * Calls ``ra_isr_lookup_slot(event, &slot)`` to find the slot
 * allocated to ``event`` by ``ra_isr_register``, then calls
 * ``ra_isr_dispatch(slot)``. If the event was not registered,
 * returns ``k_ra_err_not_found`` without invoking anything.
 *
