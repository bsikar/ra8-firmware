 ...
 *
 * @details
 * The host unit-test build forces ``RA8_SIMULATOR_MODE`` on every HAL TU, so a
 * driver's "spin until this status bit changes" loop reads its register out of
 * ordinary host RAM (see ``ra8_sim_mmap.c``). RAM alone can model two states --
 ...
 * ``#ifdef RA8_SIMULATOR_MODE return k_ra8_ok`` short-circuits (T1-01) -- with
...
typedef enum : uint8_t {
  k_ra8_sim_mmio_max_faults = 8U, /**< Max distinct registers armed at once. */
} ra8_sim_mmio_limits_t;

/**
 * @brief Disarm every register: the seam becomes fully transparent.
 *
 * @details
 * Call from a test's ``setUp()`` (alongside ``ra8_sim_mmap_reset()``) so armed
 * faults never leak between cases. After this, ::ra8_sim_mmio_wait_eval returns
 ...
 *
 * @details
 * Forces every ::ra8_sim_mmio_wait_eval for ``reg`` to report "not satisfied",
 ...
 *
 * @pre ``reg`` is non-NULL.
 * @pre Fewer than ::k_ra8_sim_mmio_max_faults distinct registers are armed
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_sim_mmio_fail_wait(const volatile void* reg);
 ...
 * ::ra8_sim_mmio_wait_eval for ``reg`` reports "not satisfied" for iterations
 ...
 * with ``n`` at or above the budget it behaves like ::ra8_sim_mmio_fail_wait.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_sim_mmio_satisfy_after(const volatile void* reg, uint32_t n);
 ...
 * ::ra8_sim_mmio_fail_wait fails the FIRST such loop and hides the later legs;
 ...
 * path of any single stage. ::ra8_sim_mmio_wait_eval counts a new wait-loop on
 * each ``iter == 0`` poll; ``n == 0`` is therefore equivalent to
 * ::ra8_sim_mmio_fail_wait. Re-arming an already-armed register updates it and
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_sim_mmio_fail_nth_wait(const volatile void* reg, uint32_t n);

/**
 * @brief Register a synchronous per-poll hook invoked from ::ra8_sim_mmio_poll.
 *
 * @details
 * The i2c / i3c / sdhi status polls route each bounded-wait iteration through
 * ::ra8_sim_mmio_poll. When a hook is installed it runs once at the top of every
 ...
 * to race. Pass ``nullptr`` to remove the hook; ::ra8_sim_mmio_reset also clears
 * it, so a hook never leaks between test cases.
 *
 * @param[in] hook Function invoked once per ::ra8_sim_mmio_poll call, or
 *                 ``nullptr`` to disable. Reads/writes only the RAM-backed sim
 *                 register windows.
 *
 * @pre Called from a host (``RA8_SIMULATOR_MODE`` + ``UNIT_TEST``) test binary.
 * @post Subsequent ::ra8_sim_mmio_poll calls invoke @p hook (or none if nullptr).
 *
...
/*
 * The per-poll hook ``ra8_sim_mmio_wait_eval(reg, iter, real_cond)`` is defined in
 * ra8_sim_mmio.c but DECLARED in ``ra8_hw_err.h`` (guarded by
 * ``RA8_SIMULATOR_MODE && UNIT_TEST``). It is placed there, not here, because the
