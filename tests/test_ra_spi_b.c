 ...
 *  - line 766  ra_spi_write    : (tx==NULL) && (len > 0)
 *  - line 790  ra_spi_read     : (rx==NULL) && (len > 0)
 *  - line 1056 ra_spi_write_dma: (channel >= count) || (len == 0)
 *  - line 1106 ra_spi_read_dma : (channel >= count) || (len == 0)
 *  - line 1190 ra_spi_dispatch_spei : (mask != 0) && (cb != NULL)
 *
 * The internal helpers are exercised via the public entry points
 * (ra_spi_init reaches both internal_spbr and internal_spcmd; the
 * xfer wrappers reach internal_xfer_common; ra_spi_attach_transfer
 * + ra_spi_dispatch_spei reach the SPEI compound).
 *
 ...
 *
 * Reached through ra_spi_init -> internal_spbr. SPCR3 SPBR field is
 ...
 * @par MC/DC:
 * Decision (line 218): `(mode == k_ra_spi_mode_1) || (mode == k_ra_spi_mode_3)`
 ...
 * @par MC/DC:
 * Decision (line 221): `(mode == k_ra_spi_mode_2) || (mode == k_ra_spi_mode_3)`
 * (2 cond, N+1=3).
 * - V1: mode=0 -> F||F -> F
 * - V2: mode=2 -> T||F -> T
 * - V3: mode=3 -> F||T -> T
...
{
  TEST_BEGIN("spi_b MC/DC ra_spi_write: tx==NULL && len>0");
  prep();
  ra_spi_cfg_t cfg = {
    .baud_hz   = (uint32_t)k_test_baud_hz,
    .pclka_hz  = (uint32_t)k_test_pclka_hz,
...
{
  TEST_BEGIN("spi_b MC/DC ra_spi_read: rx==NULL && len>0");
  prep();
  ra_spi_cfg_t cfg = {
    .baud_hz   = (uint32_t)k_test_baud_hz,
    .pclka_hz  = (uint32_t)k_test_pclka_hz,
...
{
  TEST_BEGIN("spi_b MC/DC ra_spi_write_dma: channel || len arg guard");
  prep();
...
{
  TEST_BEGIN("spi_b MC/DC ra_spi_read_dma: channel || len arg guard");
  prep();
  uint8_t  out_dma_ch = 0xFFU;
  uint8_t  buf[1]     = {0U};
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_spi_b.c\n");
  return 0;
}
