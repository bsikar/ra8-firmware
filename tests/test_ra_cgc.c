...
static void test_switch_pll1_target_updates_cpuclk0(void)
{
  ...
  *ra_sys_oscsf() = (uint8_t)k_ra_cgc_test_all_oscsf;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_cgc_switch_pll1_target(500000000U));
  uint32_t hz = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_cgc_get_clock_hz(k_ra_clock_id_cpuclk0, &hz));
...
static void test_stop_detection_arm_and_fire(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_cgc_enable_stop_detection(nullptr, &ctx_val));
  TEST_ASSERT_EQ(k_ra_ok, ra_cgc_enable_stop_detection(stub_ostd_handler, &ctx_val));
