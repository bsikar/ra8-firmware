...
static void internal_test_media_download_roundtrip(void)
{
  ...
  TEST_ASSERT_EQ(k_mdl_format_rabook, session.format);
  TEST_ASSERT_EQ(k_mdl_format_rabook, ra8_c6_model()->mdl_format);
...
RA8_INTERNAL static void internal_test_media_start_argument_mcdc(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra8_err_null_ptr,
    ra8_c6link_mdl_start(nullptr, "https://example.test/book", k_mdl_format_rabook, &session));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_c6link_mdl_start(link, nullptr, k_mdl_format_rabook, &session));
  TEST_ASSERT_EQ(
    k_ra8_err_null_ptr,
    ra8_c6link_mdl_start(link, "https://example.test/book", k_mdl_format_rabook, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_c6link_mdl_start(link, "", k_mdl_format_rabook, &session));
  TEST_ASSERT_EQ(
    k_ra8_err_invalid_arg,
    ra8_c6link_mdl_start(link, "http://example.test/book", k_mdl_format_rabook, &session));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_c6link_mdl_start(link, "https://", k_mdl_format_rabook, &session));
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_c6link_mdl_start(link, overlong, k_mdl_format_rabook, &session));
  TEST_ASSERT_EQ(
    k_ra8_err_invalid_arg,
    ra8_c6link_mdl_start(link, "https://example.test/book", k_mdl_format_invalid, &session));
  TEST_ASSERT_EQ(
    k_ra8_ok,
    ra8_c6link_mdl_start(link, "https://example.test/book", k_mdl_format_rabook, &session));
...
int main(void)
{
  ...
  priv_test_c6link_mdl_decode_run();
  return 0;
}
