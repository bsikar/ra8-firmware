 ...
 *
 * Coverage (six tests):
 *   - ``internal_test_init_validation``       -- null cfg, missing pointers,
 *                                       empty URL, bank-size 0.
 *   - ``internal_test_check_bad_manifest``    -- backend returns garbage.
 *   - ``internal_test_partial_download_recovery`` -- network EOFs early, then
 *                                       a "reboot" (deinit/init)
 *                                       lets the second attempt run
 *                                       cleanly.
 *   - ``internal_test_signature_mismatch``    -- SHA matches, ECDSA rejects.
 *   - ``internal_test_sha256_mismatch``       -- fresh re-hash differs from
 *                                       manifest digest.
 *   - ``internal_test_happy_path``            -- end-to-end success including
...
internal_compute_xor_hash(const uint8_t* data, uint32_t len, uint8_t out[32])
{
...
RA8_INTERNAL static ra8_err_t internal_mock_net_open(void* ctx, const char* url, uint32_t* out_len)
{
  (void)ctx;
  ...
  if (strstr(url, "manifest") != nullptr) {
    s_net_payload   = (const uint8_t*)s_mock_manifest;
    s_net_remaining = (uint32_t)strlen(s_mock_manifest);
  } else {
    s_net_payload   = s_mock_image;
    s_net_remaining = k_test_image_size;
  }
  s_net_offset = 0U;
  *out_len     = s_net_remaining;
  return k_ra8_ok;
}

RA8_INTERNAL static ra8_err_t
internal_mock_net_read(void* ctx, uint8_t* dst, uint32_t cap, uint32_t* out)
{
  (void)ctx;
  if ((dst == nullptr) || (out == nullptr)) {
    return k_ra8_err_null_ptr;
  }
  const uint32_t remain = s_net_remaining - s_net_offset;
  ...
  if (s_inject_short_eof && (s_net_offset >= k_test_chunk_short)) {
    return k_ra8_err_hw_error;
  }
  uint32_t n = (cap < remain) ? cap : remain;
  if (s_inject_short_eof && (n > k_test_chunk_short)) {
    n = k_test_chunk_short;
  }
  if (n > 0U) {
    (void)memcpy(dst, s_net_payload + s_net_offset, n);
    s_net_offset += n;
  }
  *out = n;
  return k_ra8_ok;
...
RA8_INTERNAL static ra8_err_t internal_mock_net_close(void* ctx)
{
  (void)ctx;
  return k_ra8_ok;
}
...
RA8_INTERNAL static ra8_err_t internal_mock_sha_init(void* ctx)
{
  (void)ctx;
  (void)memset(s_running_hash, 0, sizeof s_running_hash);
  return k_ra8_ok;
}

RA8_INTERNAL static ra8_err_t internal_mock_sha_update(void* ctx, const uint8_t* data, uint32_t len)
{
  (void)ctx;
  for (uint32_t i = 0U; i < len; ++i) {
    s_running_hash[i % k_ra8_ota_sha256_bytes] ^= data[i];
  }
  return k_ra8_ok;
}

RA8_INTERNAL static ra8_err_t internal_mock_sha_final(void*   ctx,
                                                      uint8_t out[k_ra8_ota_sha256_bytes])
{
  (void)ctx;
  (void)memcpy(out, s_running_hash, k_ra8_ota_sha256_bytes);
  return k_ra8_ok;
}

RA8_INTERNAL static ra8_err_t internal_mock_ecdsa_verify(void*          ctx,
                                                         uint32_t       key,
...
{
  (void)ctx;
  if (s_ecdsa_should_fail) {
    return k_ra8_err_hw_error;
  }
  if (key != k_test_pubkey) {
    return k_ra8_err_invalid_arg;
  }
  if ((sig_len != sizeof s_expected_sig) || (memcmp(sig, s_expected_sig, sig_len) != 0)) {
    return k_ra8_err_hw_error;
  }
  ...
  if (s_bind_check_enabled) {
    if (memcmp(digest, s_bound_expected, k_ra8_ota_sha256_bytes) != 0) {
      return k_ra8_err_hw_error;
    }
  }
  return k_ra8_ok;
...
RA8_INTERNAL static ra8_err_t internal_mock_flash_erase(void* ctx, uint32_t addr, uint32_t len)
{
  (void)ctx;
  if ((addr < k_test_bank_addr) || ((addr + len) > (k_test_bank_addr + k_test_bank_size))) {
    return k_ra8_err_invalid_arg;
  }
  (void)memset(s_bank_storage + (addr - k_test_bank_addr), k_ota_erased_byte, len);
  return k_ra8_ok;
}

RA8_INTERNAL static ra8_err_t
internal_mock_flash_program(void* ctx, uint32_t addr, const uint8_t* src, uint32_t len)
{
  (void)ctx;
  if ((addr < k_test_bank_addr) || ((addr + len) > (k_test_bank_addr + k_test_bank_size))) {
    return k_ra8_err_invalid_arg;
  }
  (void)memcpy(s_bank_storage + (addr - k_test_bank_addr), src, len);
  return k_ra8_ok;
}

RA8_INTERNAL static ra8_err_t
internal_mock_flash_set_startup(void* ctx, uint8_t which, bool persistent)
{
  (void)ctx;
  (void)which;
  (void)persistent;
...
RA8_INTERNAL static ra8_err_t
internal_mock_flash_readback(void* ctx, uint32_t addr, uint8_t* dst, uint32_t len)
{
  (void)ctx;
  if ((addr < k_test_bank_addr) || ((addr + len) > (k_test_bank_addr + k_test_bank_size))) {
    return k_ra8_err_invalid_arg;
  }
  (void)memcpy(dst, s_bank_storage + (addr - k_test_bank_addr), len);
...
RA8_INTERNAL static void internal_make_image(void)
{
  for (uint32_t i = 0U; i < k_test_image_size; ++i) {
    s_mock_image[i] = (uint8_t)(i ^ k_t_image_mask);
  }
}

RA8_INTERNAL static void internal_make_manifest(void)
{
  /* Build a manifest whose sha256 == internal_compute_xor_hash(s_mock_image). */
  internal_compute_xor_hash(s_mock_image, k_test_image_size, s_expected_hash);
  for (uint32_t i = 0U; i < sizeof s_expected_sig; ++i) {
    s_expected_sig[i] = (uint8_t)(k_t_sig_base + i);
  }
  static const char nibble[] = "0123456789abcdef";

  char hex[(2U * k_ra8_ota_sha256_bytes) + 1U] = {};
  for (uint32_t i = 0U; i < k_ra8_ota_sha256_bytes; ++i) {
    hex[(size_t)2U * i] = nibble[s_expected_hash[i] >> 4U];
    hex[(2U * i) + 1U]  = nibble[s_expected_hash[i] & k_t_nibble_mask];
  }

  char sig_hex[(2U * sizeof s_expected_sig) + 1U] = {};
  for (uint32_t i = 0U; i < sizeof s_expected_sig; ++i) {
    sig_hex[(size_t)2U * i] = nibble[s_expected_sig[i] >> 4U];
    sig_hex[(2U * i) + 1U]  = nibble[s_expected_sig[i] & k_t_nibble_mask];
  }
  (void)snprintf(s_mock_manifest,
                 sizeof s_mock_manifest,
...
RA8_INTERNAL static void internal_reset_globals(void)
{
  s_inject_short_eof   = false;
  s_ecdsa_should_fail  = false;
  s_corrupt_readback   = false;
  s_bind_check_enabled = false;
  s_reset_count        = 0U;
  (void)memset(s_bank_storage, 0, sizeof s_bank_storage);
}
 ...
 *
 * @details
 * Mirrors ``internal_bind_manifest_material`` in libs/ra8_ota/src/ra8_ota.c: streams
 ...
 */
RA8_INTERNAL static void
internal_compute_expected_bound(const ra8_ota_manifest_t* m,
                                ...
                                uint8_t                   out[k_ra8_ota_sha256_bytes])
{
  ...
  (void)internal_mock_sha_init(nullptr);
  (void)internal_mock_sha_update(nullptr, (const uint8_t*)m->version, k_ra8_ota_version_str_bytes);
  (void)internal_mock_sha_update(nullptr, (const uint8_t*)m->image_url, k_ra8_ota_url_max_bytes);
  (void)internal_mock_sha_update(nullptr, size_le, sizeof size_le);
  (void)internal_mock_sha_update(nullptr, image_digest, k_ra8_ota_sha256_bytes);
  (void)internal_mock_sha_final(nullptr, out);
}

RA8_INTERNAL static ra8_ota_cfg_t internal_make_cfg(void)
{
  ra8_ota_cfg_t cfg = {};
  ...
  cfg.net.open                  = internal_mock_net_open;
  cfg.net.read                  = internal_mock_net_read;
  cfg.net.close                 = internal_mock_net_close;
  cfg.crypto.sha256_init        = internal_mock_sha_init;
  cfg.crypto.sha256_update      = internal_mock_sha_update;
  cfg.crypto.sha256_final       = internal_mock_sha_final;
  cfg.crypto.ecdsa_verify       = internal_mock_ecdsa_verify;
  cfg.flash.erase               = internal_mock_flash_erase;
  cfg.flash.program             = internal_mock_flash_program;
  cfg.flash.set_startup         = internal_mock_flash_set_startup;
  cfg.flash.readback            = internal_mock_flash_readback;
...
{
  TEST_BEGIN("internal_test_init_validation");
  (void)ra8_ota_deinit();
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_ota_init(nullptr));

  ra8_ota_cfg_t cfg = internal_make_cfg();
  ...
  TEST_END("internal_test_init_validation");
}
 ...
 * code under test that this case touches)
 */
RA8_INTERNAL static void internal_test_check_bad_manifest(void)
{
  TEST_BEGIN("internal_test_check_bad_manifest");
  internal_reset_globals();
  internal_make_image();
  /* Garbage manifest -- missing required fields. */
  (void)snprintf(s_mock_manifest, sizeof s_mock_manifest, "{ \"hello\": \"world\" }");
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ota_deinit());
  TEST_END("internal_test_check_bad_manifest");
}
 ...
 * code under test that this case touches)
 */
RA8_INTERNAL static void internal_test_partial_download_recovery(void)
{
  TEST_BEGIN("internal_test_partial_download_recovery");
  internal_reset_globals();
  internal_make_image();
  internal_make_manifest();
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ota_deinit());
  TEST_END("internal_test_partial_download_recovery");
}
 ...
 * code under test that this case touches)
 */
RA8_INTERNAL static void internal_test_signature_mismatch(void)
{
  TEST_BEGIN("internal_test_signature_mismatch");
  internal_reset_globals();
  internal_make_image();
  internal_make_manifest();
  ...
  s_ecdsa_should_fail = true;
  TEST_ASSERT_EQ(k_ra8_err_hw_error, ra8_ota_verify_signature(&m));
  TEST_ASSERT_EQ(k_ra8_ota_state_error, ra8_ota_get_state());
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ota_deinit());
  TEST_END("internal_test_signature_mismatch");
}
 ...
 * code under test that this case touches)
 */
RA8_INTERNAL static void internal_test_sha256_mismatch(void)
{
  TEST_BEGIN("internal_test_sha256_mismatch");
  internal_reset_globals();
  internal_make_image();
  internal_make_manifest();
  ...
  TEST_ASSERT_EQ(k_ra8_ota_state_error, ra8_ota_get_state());
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ota_deinit());
  TEST_END("internal_test_sha256_mismatch");
}
 ...
 * code under test that this case touches)
 */
RA8_INTERNAL static void internal_test_happy_path(void)
{
  TEST_BEGIN("internal_test_happy_path");
  internal_reset_globals();
  internal_make_image();
  internal_make_manifest();
  ...
  TEST_ASSERT_EQ(1U, s_reset_count);

  /* Programmed bank should match the source image. */
  TEST_ASSERT_EQ(0, memcmp(s_bank_storage, s_mock_image, k_test_image_size));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ota_deinit());
  TEST_END("internal_test_happy_path");
}

/**
 * @test internal_test_metadata_tamper_rejected
 *
 * @par MC/DC:
 ...
 */
RA8_INTERNAL static void internal_test_metadata_tamper_rejected(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ota_check_for_update(&m));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ota_download_to_inactive_bank(&m));
  internal_compute_expected_bound(&m, s_expected_hash, s_bound_expected);
  s_bind_check_enabled = true;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ota_verify_signature(&m));
  TEST_ASSERT_EQ(k_ra8_ota_state_committing, ra8_ota_get_state());
  TEST_ASSERT_EQ(k_ra8_ok, ra8_ota_deinit());
...
/**
 * @test internal_test_mcdc_download_state_guard
 *
 * @par MC/DC:
 ...
 */
RA8_INTERNAL static void internal_test_mcdc_download_state_guard(void)
{
...
/**
 * @test internal_test_mcdc_run_full_update_terminal
 *
 * @par MC/DC:
 ...
 * N+1 = 3 vectors for N=2.
 */
RA8_INTERNAL static void internal_test_mcdc_run_full_update_terminal(void)
{
...
/**
 * @test internal_test_mcdc_hex_decode_invalid_nibble
 *
 * @par MC/DC:
 * Decision: `if ((hi == k_ra8_ota_hex_invalid_nibble) || (lo == k_ra8_ota_hex_invalid_nibble))`
 * (libs/ra8_ota/src/ra8_ota.c, internal_hex_decode).
 * Reached through ra8_ota_check_for_update -> internal_manifest_decode_crypto
 ...
 * N+1 = 3 vectors for N=2.
 */
RA8_INTERNAL static void internal_test_mcdc_hex_decode_invalid_nibble(void)
{
  ...
  internal_make_image();
  internal_make_manifest();
  char* sha_field = strstr(s_mock_manifest, "\"sha256\": \"");
  ...
  internal_make_image();
  internal_make_manifest();
  char* sha_field2 = strstr(s_mock_manifest, "\"sha256\": \"");
...
/**
 * @test internal_test_mcdc_hex_nibble_pair_completion
 *
 * @par MC/DC:
 * Three sibling decisions in libs/ra8_ota/src/ra8_ota.c internal_hex_nibble:
 * - line 448: ``(c >= '0') && (c <= '9')``
 * - line 451: ``(c >= 'a') && (c <= 'f')``
 * - line 454: ``(c >= 'A') && (c <= 'F')``
 *
 * For each 2-cond AND, N+1 = 3 vectors. The pre-existing
 * internal_test_mcdc_hex_decode_invalid_nibble corrupts with 'Z' which gives:
 *   448: T,F (Z >= '0' but Z > '9')
 *   451: F,- (Z < 'a')
 *   454: T,F (Z >= 'A' but Z > 'F')
 *
 ...
 * field of the manifest, so internal_hex_nibble is exercised on real input.
 */
RA8_INTERNAL static void internal_test_mcdc_hex_nibble_pair_completion(void)
{
...
/**
 * @test internal_test_mcdc_json_u32_skip_chars
 *
 * @par MC/DC:
 * Decision (libs/ra8_ota/src/ra8_ota.c, priv_ota_json_u32, line 403):
 *   ``(*p == ':') || (*p == ' ') || (*p == '"')``
 ...
 */
RA8_INTERNAL static void internal_test_mcdc_json_u32_skip_chars(void)
{
  TEST_BEGIN("ra8_ota MC/DC: priv_ota_json_u32 skip-char OR");
  uint32_t v = 0U;
  /* V1: colon between key and value. */
  TEST_ASSERT_EQ(k_ra8_ok, priv_ota_json_u32("\"size\":42}", "\"size\"", &v));
  TEST_ASSERT_EQ(42, v);
  /* V2: space (after key, before digits). */
  v = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, priv_ota_json_u32("\"size\" 7}", "\"size\"", &v));
  TEST_ASSERT_EQ(7, v);
  /* V3: quote (e.g. "size""123"). */
  v = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, priv_ota_json_u32("\"size\"\"3", "\"size\"", &v));
  TEST_ASSERT_EQ(3, v);
  /* V4: digit immediately after key -- skip loop exits at first iter. */
  v = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, priv_ota_json_u32("\"size\"9}", "\"size\"", &v));
  TEST_ASSERT_EQ(9, v);
  TEST_END("ra8_ota MC/DC: priv_ota_json_u32 skip-char OR");
}

/**
 * @test internal_test_mcdc_ota_char_in_range
 *
 * @par MC/DC:
 ...
 * N+1 = 3.
 */
RA8_INTERNAL static void internal_test_mcdc_ota_char_in_range(void)
{
  TEST_BEGIN("ra8_ota MC/DC: char_in_range AND");
  TEST_ASSERT(!priv_ota_char_in_range('@', 'A', 'F'));
  TEST_ASSERT(priv_ota_char_in_range('C', 'A', 'F'));
  TEST_ASSERT(!priv_ota_char_in_range('Z', 'A', 'F'));
  TEST_END("ra8_ota MC/DC: char_in_range AND");
}

/**
 * @test internal_test_mcdc_ota_download_state_invalid
 *
 * @par MC/DC:
 * Decision at libs/ra8_ota/src/ra8_ota.c (call site) -> helper at
 * libs/ra8_ota/src/ra8_ota.c:
 ...
 * N+1 = 3.
 */
RA8_INTERNAL static void internal_test_mcdc_ota_download_state_invalid(void)
{
  TEST_BEGIN("ra8_ota MC/DC: download_state_invalid AND");
  TEST_ASSERT(!priv_ota_download_state_invalid((uint32_t)k_ra8_ota_state_idle,
...
int main(void)
{
  ...
  internal_test_check_bad_manifest();
  internal_test_partial_download_recovery();
  internal_test_signature_mismatch();
  internal_test_sha256_mismatch();
  internal_test_happy_path();
  internal_test_metadata_tamper_rejected();
  internal_test_mcdc_download_state_guard();
  internal_test_mcdc_run_full_update_terminal();
  internal_test_mcdc_hex_decode_invalid_nibble();
  internal_test_mcdc_json_u32_skip_chars();
  internal_test_mcdc_hex_nibble_pair_completion();
  internal_test_mcdc_ota_char_in_range();
  internal_test_mcdc_ota_download_state_invalid();
  return 0;
}
