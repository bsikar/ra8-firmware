 ...
 *
 * @details
 * The existing MC/DC suite (tests/test_ra8_reflow_layout.c) drives the layout
 * engine's compound decisions through `ra8_reflow_init()` with a *dummy* font, so
 ...
 *  - the bad-font guard in ra8_reflow_run_layout (priv_init_font error returns);
...
static ra8_err_t init_eng(uint16_t w, uint16_t h)
{
...
{
  uint32_t pages = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, init_eng(w, h));
  TEST_ASSERT_EQ(
    k_ra8_ok,
 ...
 * @brief A junk font blob makes ra8_reflow_run_layout fail validation.
 *
 * @details Targets ra8_reflow_layout.c lines 231-235 (priv_init_font's offset<0 /
 * InitFont==0 returns) and 1601-1602 (ra8_reflow_run_layout propagating that
...
static void test_cov_bad_font_run_layout(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 ra8_reflow_init((uint16_t)k_cov_vp_w,
                                 (uint16_t)k_cov_vp_h,
                                 s_junk_font,
  ...
  /* Also drive ra8_reflow_run_layout directly (it re-inits the same junk font). */
  TEST_ASSERT_EQ(k_ra8_err_validation_failed, ra8_reflow_run_layout(&s_eng));
 ...
 *
 * @details Targets ra8_reflow_layout.c lines 401-415 (priv_finish_page body
 ...
 *
 * @details Targets ra8_reflow_layout.c lines 445-461 (priv_justify_glyphs: the
 ...
 *
 * @details Targets ra8_reflow_layout.c line 500-501 (priv_finish_line's
 ...
 *
 * @details Targets ra8_reflow_layout.c lines 698-700 (priv_open_block's
 ...
 *
 * @details Targets ra8_reflow_layout.c lines 822-829 (priv_apply_image_placeholder
 ...
 *
 * @details Targets ra8_reflow_layout.c lines 916-917 (priv_image_resolve_size
...
static void test_cov_image_loader_failure_fallback(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, init_eng((uint16_t)k_cov_vp_w, (uint16_t)k_cov_vp_h_tall));
  ra8_img_arena_t arena = {.base   = s_arena_buf,
                           .cap    = sizeof s_arena_buf,
                           .offset = 0U,
                           .live   = 0U};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_reflow_set_image_loader(&s_eng, fail_loader, nullptr, &arena));
...
static void test_cov_image_real_block(void)
{
  ...
  ra8_img_arena_t arena = {.base   = s_arena_buf,
                           ...
                           .offset = 0U,
                           .live   = 0U};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_reflow_set_image_loader(&s_eng, png_loader, nullptr, &arena));
 ...
 *
 * @details Targets ra8_reflow_layout.c's table family: priv_table_columns
 ...
 *
 * @details Targets ra8_reflow_layout.c lines 1482-1484 (priv_layout_table's
 ...
 *
 * @details Targets ra8_reflow_layout.c line 1213 (priv_match_block_end's
 ...
 *
 * @details Targets ra8_reflow_layout.c lines 1585-1589 (ra8_reflow_run_layout),
...
static void test_cov_lifecycle_guards(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_reflow_run_layout(nullptr));
  TEST_ASSERT_EQ(k_ra8_err_not_initialized, ra8_reflow_run_layout(&closed));

  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_reflow_set_image_loader(nullptr, png_loader, nullptr, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_not_initialized,
                 ra8_reflow_set_image_loader(&closed, png_loader, nullptr, nullptr));

  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_reflow_set_css_loader(nullptr, nullptr, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_not_initialized, ra8_reflow_set_css_loader(&closed, nullptr, nullptr));
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra8_reflow_layout_cov.c\n");
  return 0;
}
