/**
 * @file test_ra_ftl_cov.c
 * @brief Branch coverage supplement for the Flash Translation Layer (ra_ftl.c).
 *
 * @details
 * Drives error-injection paths, bounds-check branches, capability-validation
 * arms, and the sync forward-path that are not reached by test_ra_ftl.c.
 ...
 *   718 -- ra_ftl_init: get_caps failure
 *   740 -- ra_ftl_as_blockdev: raw not set
 *   753 -- ra_ftl_wear_stats: pblocks not set
 *
 * Line 194 (defensive ftl_alloc_blank guard) is marked GCOVR_EXCL_LINE in
 * the source: ftl_pick_free with non-null arguments returns only k_ra_ok or
 * k_ra_err_no_data, so the guard is provably unreachable.
 *
 ...
 * individual tests can drive the capability-validation paths of ra_ftl_init
...
typedef struct {
  ...
  bool     fail_erase;       /**< Return k_ra_err_invalid_state from erase.    */
  bool     fail_write;       /**< Return k_ra_err_invalid_state from write.    */
  bool     fail_read;        /**< Return k_ra_err_invalid_state from read.     */
  bool     fail_get_caps;    /**< Return k_ra_err_not_supported from get_caps. */
 ...
 */
static ra_err_t cov_read(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf)
{
 ...
 */
static ra_err_t cov_write(void* ctx, uint32_t lba, uint32_t count, const uint8_t* buf)
{
  cov_fake_t* st = (cov_fake_t*)ctx;
  if (st->fail_write) {
    return k_ra_err_invalid_state;
  ...
  for (size_t i = 0; i < n; ++i) {
    if (st->store[base + i] != (uint8_t)k_cov_erase_byte) {
      return k_ra_err_invalid_state; /* program to non-blank sector rejected */
    }
  }
 ...
 * (two independent single-condition early returns; no compound decision)
 */
static ra_err_t cov_erase(void* ctx, uint32_t lba, uint32_t count)
{
  cov_fake_t* st = (cov_fake_t*)ctx;
  if (st->fail_erase) {
    return k_ra_err_invalid_state;
 ...
 */
static ra_err_t cov_get_caps(const void* ctx, ra_io_blockdev_caps_t* out)
{
...
/**
 * @par MC/DC:
 * (no decision -- forwards the call and returns k_ra_ok)
 */
static ra_err_t cov_sync(void* ctx)
{
  (void)ctx;
  return k_ra_ok;
}
...
static const ra_io_blockdev_iface_t k_cov_iface = {
  .read     = cov_read,
  .write    = cov_write,
  .erase    = cov_erase,
 ...
 */
static void cov_fake_bind(ra_io_blockdev_t* bd)
{
...
static void test_cov_cap_errors(void)
{
  ...
  ra_io_blockdev_t raw = {};
  ra_ftl_t         ftl = {};
  uint16_t         map[(size_t)k_cov_logical];
  ra_ftl_pblock_t  pb[(size_t)k_cov_phys] = {};
  ...
  TEST_ASSERT_EQ(
    ...
    ra_ftl_init(&ftl, &raw, map, (uint32_t)k_cov_logical, pb, (uint32_t)k_cov_phys, scratch));
  ...
  /* physical_blocks > k_ra_ftl_max_pblocks: ftl_validate_init_args line 697.
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_size,
                 ra_ftl_init(&ftl, &raw, map, (uint32_t)k_cov_logical, pb, 0xFFFFU, scratch));

  /* get_caps failure: ra_ftl_init line 718. */
  cov_fake_bind(&raw);
...
static void test_cov_not_init(void)
{
  ...
  /* ra_ftl_as_blockdev: raw == nullptr => line 740. */
  ra_io_blockdev_t bd = {};
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_ftl_as_blockdev(&ftl_zero, &bd));

  /* ra_ftl_wear_stats: pblocks == nullptr => line 753. */
  uint32_t hi = 0;
  uint32_t lo = 0;
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_ftl_wear_stats(&ftl_zero, &hi, &lo));
...
/**
 * @par MC/DC:
 * (each ra_io_blockdev_* call exercises one independent branch in ftl_bounds;
...
static void test_cov_bounds(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_out_of_range,
                 ra_io_blockdev_read(&bd, 0U, (uint32_t)k_cov_logical + 1U, buf));

  /* lba at capacity -> line 334; propagated through read -> line 373. */
  TEST_ASSERT_EQ(k_ra_err_out_of_range, ra_io_blockdev_read(&bd, (uint32_t)k_cov_logical, 1U, buf));

  /* count > logical_blocks -> line 331; propagated through write -> line 420. */
  TEST_ASSERT_EQ(k_ra_err_out_of_range,
                 ra_io_blockdev_write(&bd, 0U, (uint32_t)k_cov_logical + 1U, buf));

  /* count > logical_blocks -> line 331; propagated through erase -> line 465. */
  TEST_ASSERT_EQ(k_ra_err_out_of_range,
                 ra_io_blockdev_erase(&bd, 0U, (uint32_t)k_cov_logical + 1U));
...
static void test_cov_read_error(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_io_blockdev_write(&bd, 0U, 1U, wbuf));

  /* Inject read failure; ftl_read_one forwards the error -> line 379. */
  s_cov.fail_read = true;
  uint8_t rbuf[(size_t)k_cov_block];
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_io_blockdev_read(&bd, 0U, 1U, rbuf));
  s_cov.fail_read = false;
...
static void test_cov_reclaim_erase_fail(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_state, ra_io_blockdev_write(&bd, 1U, 1U, buf));
  s_cov.fail_erase = false;
...
static void test_cov_no_mem(void)
{
   ...
   * causing ftl_alloc_blank to return k_ra_err_no_mem (line 191).
   * No I/O is issued to the underlying fake device. */
  for (uint32_t i = 0U; i < (uint32_t)k_cov_phys; ++i) {
    pb[i].state = (uint8_t)k_ra_ftl_pstate_live;
  }
  uint8_t buf[(size_t)k_cov_block];
  (void)memset(buf, 0xEE, sizeof(buf));
  TEST_ASSERT_EQ(k_ra_err_no_mem, ra_io_blockdev_write(&bd, 0U, 1U, buf));
 ...
 * RA_CHECK_NULL_PTR guards evaluate with non-null pointers so the
...
static void test_cov_sync(void)
{
   ...
   * ftl_dev_sync (lines 539-545), which forwards through ra_io_blockdev_sync
...
int32_t main(void)
{
  ...
  test_cov_no_mem();
  test_cov_sync();
  (void)fprintf(stderr, "[OK  ] test_ra_ftl_cov.c\n");
  return 0;
}
