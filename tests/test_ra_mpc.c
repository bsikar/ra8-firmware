...
static void test_route_peripheral_sets_pmr_and_psel(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mpc_route_peripheral(k_ra_port_1, 5U, k_ra_mpc_psel_sci0));
  ...
  TEST_ASSERT_EQ(expected_psel, (*pfs & (uint32_t)k_ra_pfs_mask_psel));
...
static void test_set_gpio_output_clears_pmr(void)
{
  ...
  *pfs = 0xFFFFFFFFU;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mpc_set_gpio(k_ra_port_3, 7U, k_ra_mpc_dir_output));

  TEST_ASSERT_EQ(k_ra_pfs_mask_pdr, *ra_pfs_pmn(k_ra_port_3, 7U));
...
static void test_set_gpio_input(void)
{
  ...
  *pfs = 0xDEADBEEFU;
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mpc_set_gpio(k_ra_port_2, 0U, k_ra_mpc_dir_input));
  TEST_ASSERT_EQ(0, *ra_pfs_pmn(k_ra_port_2, 0U));
...
static void test_set_analog(void)
{
  ...
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ(k_ra_ok, ra_mpc_set_analog(k_ra_port_0, 1U));
  TEST_ASSERT_EQ(k_ra_pfs_mask_asel, *ra_pfs_pmn(k_ra_port_0, 1U));
...
static void test_set_irq(void)
{
  ...
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ(k_ra_ok, ra_mpc_set_irq(k_ra_port_4, 2U));
  TEST_ASSERT_EQ(k_ra_pfs_mask_isel, *ra_pfs_pmn(k_ra_port_4, 2U));
...
static void test_set_pull_up_then_off(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mpc_set_pull(k_ra_port_5, 3U, k_ra_mpc_pull_up));
  volatile const uint32_t* pfs = ra_pfs_pmn(k_ra_port_5, 3U);
  TEST_ASSERT((*pfs & (uint32_t)k_ra_pfs_mask_pcr) != 0U);

  TEST_ASSERT_EQ(k_ra_ok, ra_mpc_set_pull(k_ra_port_5, 3U, k_ra_mpc_pull_off));
  TEST_ASSERT_EQ(0, (*pfs & (uint32_t)k_ra_pfs_mask_pcr));
...
static void test_set_open_drain(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_mpc_set_open_drain(k_ra_port_6, 4U, true));
  volatile const uint32_t* pfs = ra_pfs_pmn(k_ra_port_6, 4U);
  TEST_ASSERT((*pfs & (uint32_t)k_ra_pfs_mask_ncodr) != 0U);

  TEST_ASSERT_EQ(k_ra_ok, ra_mpc_set_open_drain(k_ra_port_6, 4U, false));
  TEST_ASSERT_EQ(0, (*pfs & (uint32_t)k_ra_pfs_mask_ncodr));
...
static void test_read_pfs_null_out(void)
{
  ...
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_mpc_read_pfs(k_ra_port_0, 0U, nullptr));
...
static void test_read_pfs_roundtrip(void)
{
  ...
  ra_sim_mmap_reset();
  TEST_ASSERT_EQ(k_ra_ok, ra_mpc_set_analog(k_ra_port_0, 2U));
  uint32_t v = 0U;
  TEST_ASSERT_EQ(k_ra_ok, ra_mpc_read_pfs(k_ra_port_0, 2U, &v));
  TEST_ASSERT_EQ(k_ra_pfs_mask_asel, v);
...
static void test_bounds_checks(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_port, ra_mpc_set_gpio(bad_port, 0U, k_ra_mpc_dir_input));
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_pin,
                 ra_mpc_set_gpio(k_ra_port_0, bad_pin, k_ra_mpc_dir_input));

  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_port, ra_mpc_set_analog(bad_port, 0U));
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_pin, ra_mpc_set_irq(k_ra_port_0, bad_pin));
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_port, ra_mpc_set_pull(bad_port, 0U, k_ra_mpc_pull_off));
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_pin, ra_mpc_set_open_drain(k_ra_port_0, bad_pin, false));
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_port,
                 ra_mpc_route_peripheral(bad_port, 0U, k_ra_mpc_psel_gpio));
  uint32_t v = 0U;
  TEST_ASSERT_EQ(k_ra_err_gpio_invalid_pin, ra_mpc_read_pfs(k_ra_port_0, bad_pin, &v));
