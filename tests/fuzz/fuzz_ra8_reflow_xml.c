 ...
 * reader in `libs/ra8_epub/src/ra8_epub_xml_shim.c`. Every one of those XML
 * documents comes straight out of the untrusted book file. The existing
 * fuzz_ra8_epub harness only reaches this XML layer after miniz has
...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
  ...
  (void)priv_ra8_epub_xml_parse_opf(data, size, &s_book);
  memset(&s_book, 0, sizeof s_book);
  (void)priv_ra8_epub_xml_parse_ncx(data, size, &s_book);
  memset(&s_book, 0, sizeof s_book);
  (void)priv_ra8_epub_xml_parse_nav(data, size, &s_book);
  return 0;
}
