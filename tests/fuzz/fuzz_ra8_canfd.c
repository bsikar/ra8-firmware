 ...
 * register block via ::ra8_canfd_test_inject_frame, then calls
 * ::ra8_canfd_receive to exercise the ID / DLC / FD-status decoder.
...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
  ...
  if (ra8_canfd_test_inject_frame((uint8_t)k_fuzz_canfd_channel,
                                  id_word,
                                  ptr_word,
                                  fdsts_word,
                                  payload,
                                  payload_len) != k_ra8_ok) {
    return 0;
  }
