 ...
 * libs/ra_epub/src/ra_epub_xml_shim.cpp, driven from ra_epub_open.c). Every one
 ...
 * existing fuzz_ra_epub harness only reaches this XML layer after miniz has
 ...
 * (ra_epub_cpp_alloc.cpp), so no host heap is involved.
 *
 ...
 * ra_epub_xml_shim.cpp (definition) and ra_epub_open.c (consumer); there is no
 ...
 * ra_epub_book_t in place and returns an ra_err_t.
 */
...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
  ...
  (void)ra_epub_xml_parse_opf(data, size, &s_book);
  memset(&s_book, 0, sizeof s_book);
  (void)ra_epub_xml_parse_ncx(data, size, &s_book);
  memset(&s_book, 0, sizeof s_book);
  (void)ra_epub_xml_parse_nav(data, size, &s_book);
  return 0;
}
