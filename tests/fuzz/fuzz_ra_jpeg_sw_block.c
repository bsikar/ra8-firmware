/**
 * @file fuzz_ra_jpeg_sw_block.c
 * @brief libFuzzer harness focused on the JPEG block-level Huffman decoder
 *
 * @details
 * Sister harness to ``fuzz_ra_jpeg_sw.c`` -- where that one mutates the
 ...
 * declared static in ``libs/ra_hal/src/ra_jpeg_sw.c``).
 *
...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
  ...
  uint16_t out_w = 0U;
  uint16_t out_h = 0U;
  (void)ra_jpeg_sw_decode(s_buf, (uint32_t)pos, s_out, sizeof s_out, &out_w, &out_h);
  return 0;
}
