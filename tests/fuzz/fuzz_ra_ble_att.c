 ...
 *
 * The harness mirrors tests/test_ra_ble_att_dispatch.c: bring up the
...
/* UNIT_TEST hooks declared in ra_ble_host.h. */
...
static int fuzz_ble_setup(void)
{
  ...
  if (ra_ble_host_init(&cfg) != k_ra_ok) {
    return -1;
  }
  uint8_t svc_uuid[16];
  uint8_t chr_uuid[16];
  ...
  if (ra_ble_host_gatt_register_service(svc_uuid, &svc) != k_ra_ok) {
    return -1;
  }
  if (ra_ble_host_gatt_register_char(svc,
                                     chr_uuid,
                                     props,
                                     s_value_buf,
  ...
  ra_ble_host_test_inject_connect((uint16_t)k_fuzz_conn_handle);
  return 0;
}
...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
  ...
  ra_ble_host_test_inject_acl((uint16_t)k_fuzz_conn_handle,
                              s_frame,
