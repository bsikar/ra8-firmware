 ...
 * @brief libFuzzer harness for the ra8_etha RX frame parser
 *
 * @details
 * Pumps arbitrary bytes through ::ra8_etha_test_inject_rx, the test
...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
  ...
  uint8_t  src[6] = {};
  uint16_t etype  = 0U;
  (void)ra8_etha_test_inject_rx(data, (uint32_t)size, dst, src, &etype);
  return 0;
}
