/**
 * @file fuzz_ra_tls.c
 * @brief libFuzzer harness for the ra_tls Mbed TLS facade.
 *
 * @details
 * The host test build defines ``RA_SIMULATOR_MODE`` for every TU, so
 * ``ra_tls.c`` substitutes a single-byte loopback drain for each
 * ``mbedtls_ssl_*`` call. That keeps the harness self-contained
 * (no real Mbed TLS link dependency) while still exercising:
 *
 *   - ra_tls_session_open / close pool bookkeeping (NASA Rule 3 cap).
...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
  ...
  (void)ra_tls_handshake(session);

  size_t sent = 0U;
  (void)ra_tls_send(session, data, size, &sent);

  uint8_t recv_buf[k_fuzz_recv_chunk];
  size_t  received = 0U;
  (void)ra_tls_recv(session, recv_buf, sizeof recv_buf, &received);

  (void)ra_tls_session_close(session);
  return 0;
}
