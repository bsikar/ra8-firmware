 ...
 * @font-face resources parsed by epub and rasterised through stb_truetype
 * (see priv_font_init / priv_render_into in apps/shared_libs/epub/src/epub_chapter.c
 * and the reflow layout driver). The font bytes are fully attacker-
...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
   ...
   * in epub_chapter.c priv_font_init). */
