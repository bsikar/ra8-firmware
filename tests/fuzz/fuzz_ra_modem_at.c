/**
 * @file fuzz_ra_modem_at.c
 * @brief libFuzzer harness for the ra_modem_at AT response parser.
 *
 * @details
 ...
 * iteration calls ra_modem_at_send_cmd() which internally pumps the
...
static ra_err_t fuzz_tx(void* ctx, uint8_t byte)
{
  (void)ctx;
  (void)byte;
  return k_ra_ok;
...
static ra_err_t fuzz_rx(void* ctx, uint8_t* out)
{
  (void)ctx;
...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
  ...
  (void)ra_modem_at_send_cmd("AT", "OK", 50U);
  return 0;
}
