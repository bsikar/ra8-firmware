/**
 * @file test_ra_canfd_afl.c
 * @brief Unit tests for the CANFD Acceptance-Filter-List config API.
 *
 * @details
 * Exercises ``ra_canfd_set_accept_filter`` (libs/ra_hal/src/ra_canfd_afl.c):
...
/**
 * @enum ra_canfd_afl_test_channel_t
 * @brief Channel ids used by the AFL tests.
 */
typedef enum : uint8_t {
  k_ra_canfd_afl_test_channel_0   = 0U, /**< First valid channel.  */
  k_ra_canfd_afl_test_channel_1   = 1U, /**< Second valid channel. */
  k_ra_canfd_afl_test_channel_bad = 2U, /**< One past the last.    */
} ra_canfd_afl_test_channel_t;

/**
 * @enum ra_canfd_afl_test_vals_t
...
static void test_set_accept_filter_programs_two_rules(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_canfd_set_accept_filter((uint8_t)k_ra_canfd_afl_test_channel_0, rules, 2U));

  volatile r_canfd_t* reg = ra_canfd((uint8_t)k_ra_canfd_afl_test_channel_0);
...
static void test_set_accept_filter_standard_rule(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_canfd_set_accept_filter((uint8_t)k_ra_canfd_afl_test_channel_0, &rule, 1U));
...
static void test_set_accept_filter_extended_rtr_rule(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_canfd_set_accept_filter((uint8_t)k_ra_canfd_afl_test_channel_1, &rule, 1U));

  volatile r_canfd_t* reg = ra_canfd((uint8_t)k_ra_canfd_afl_test_channel_1);
  TEST_ASSERT((reg->CFDGAFL[0].ID & k_afl_ide_bit) != 0U);
  TEST_ASSERT((reg->CFDGAFL[0].ID & k_afl_rtr_bit) != 0U);
...
static void test_set_accept_filter_null_rules(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_canfd_set_accept_filter((uint8_t)k_ra_canfd_afl_test_channel_0, nullptr, 1U));
...
static void test_set_accept_filter_bad_channel(void)
{
  ...
  rule.mask                = k_afl_std_mask;
  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_canfd_set_accept_filter((uint8_t)k_ra_canfd_afl_test_channel_bad, &rule, 1U));
...
static void test_set_accept_filter_zero_count(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_canfd_set_accept_filter((uint8_t)k_ra_canfd_afl_test_channel_0, &rule, 0U));
...
static void test_set_accept_filter_over_capacity(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_canfd_set_accept_filter((uint8_t)k_ra_canfd_afl_test_channel_0,
                                            &rule,
                                            (uint8_t)(k_ra_canfd_afl_rule_capacity + 1U)));
...
static void test_set_accept_filter_standard_overflow(void)
{
  ...
  ra_canfd_afl_rule_t id_rule = {};
  id_rule.id                  = k_afl_std_overflow;
  id_rule.extended            = false;
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_canfd_set_accept_filter((uint8_t)k_ra_canfd_afl_test_channel_0, &id_rule, 1U));

  /* Standard mask with bit 11 set. */
  ra_canfd_afl_rule_t mask_rule = {};
  mask_rule.mask                = k_afl_std_overflow;
  mask_rule.extended            = false;
  TEST_ASSERT_EQ(
    k_ra_err_invalid_arg,
    ra_canfd_set_accept_filter((uint8_t)k_ra_canfd_afl_test_channel_0, &mask_rule, 1U));
...
static void test_set_accept_filter_bad_target_rx(void)
{
  ...
  rule.id                  = k_afl_std_id;
  rule.mask                = k_afl_std_mask;
  rule.target_rx           = (uint8_t)k_ra_canfd_rx_fifo_count; /* one past the last FIFO */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_canfd_set_accept_filter((uint8_t)k_ra_canfd_afl_test_channel_0, &rule, 1U));
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_canfd_afl.c\n");
  return 0;
}
