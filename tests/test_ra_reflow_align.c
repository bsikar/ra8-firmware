/**
 * @file test_ra_reflow_align.c
 * @brief Host unit tests + MC/DC for text alignment / justification (#108).
 *
 * @details
 * Lays out paragraphs with `style="text-align:..."` through `ra_reflow` (with
...
{
  if ((align == (uint8_t)k_ra_reflow_align_justify) && (allow != 0U)) {
    return 1U;
  }
  return 0U;
}
 ...
 * libs/ra_reflow/src/ra_reflow_layout.c@priv_finish_line -- the
...
static void test_justify_gate_mcdc(void)
{
  ...
  TEST_ASSERT_EQ(1, mirror_justify_applies((uint8_t)k_ra_reflow_align_justify, 1U));
  TEST_ASSERT_EQ(0, mirror_justify_applies((uint8_t)k_ra_reflow_align_justify, 0U));
  TEST_ASSERT_EQ(0, mirror_justify_applies((uint8_t)k_ra_reflow_align_center, 1U));
...
{
  return (align == (uint8_t)k_ra_reflow_align_center) ? (slack / 2) : slack;
}
...
static void test_align_offset_mcdc(void)
{
  ...
  TEST_ASSERT_EQ(50, mirror_align_offset((uint8_t)k_ra_reflow_align_center, 100));
  TEST_ASSERT_EQ(100, mirror_align_offset((uint8_t)k_ra_reflow_align_right, 100));
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_reflow_align.c\n");
  return 0;
}
