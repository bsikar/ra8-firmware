/**
 * @file test_ra8_vin.c
 * @brief Unit tests for ra8_vin.c (Video Input Module driver)
 *
 * @details
 * Tests run on the host build and use the `ra8_sim_mmap` shim so the
 * driver's MMIO accesses land in plain RAM. Each test resets the
 * mmap and (where needed) ra8_mstp before exercising the API.
 *
 ...
 */
static ra8_vin_config_t make_cfg(void)
{
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK  ] test_ra8_vin.c\n");
  return 0;
}
