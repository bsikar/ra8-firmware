/**
 * @file test_ra_reflow_layout_cov.c
 * @brief Line-coverage (gcovr) tests for libs/ra_reflow/src/ra_reflow_layout.c.
 *
 * @details
 * The existing MC/DC suite (tests/test_ra_reflow_layout.c) drives the layout
 * engine's compound decisions through `ra_reflow_init()` with a *dummy* font, so
 * it never reaches the real line-break / page-break / table body. These tests
 * lay out real content with the fixed-metric Ahem face (the same fixture the
 * alignment tests use, so they never SKIP) to execute the per-element source
 * lines that the decision-only suite leaves cold:
 *
 *  - heading font scales `<h4>..<h6>` (priv_block_font_px h4/h5/h6 arms);
 *  - the bad-font guard in ra_reflow_run_layout (priv_init_font error returns);
 *  - the page-finish body on multi-page content (priv_finish_page);
 *  - the justify remainder + no-space early-out (priv_justify_glyphs);
 *  - the centre/right slack<=0 early-out (priv_finish_line);
 *  - block-open / rule / image-placeholder / table opening with line content
 *    (priv_open_block, priv_apply_rule, priv_apply_image_placeholder,
 *     priv_layout_table newline);
 *  - the image loader-failure fallback (priv_image_resolve_size);
 *  - full table layout: column scan, row-cell scan, nested-table depth, empty
 *    table, non-row / non-cell token skips (priv_table_columns, priv_row_cells,
 *    priv_match_block_end, priv_layout_table);
...
/** @brief A blob that passes the >= 16-byte init check but is not a real font. */
 ...
 * @details Same fixture as tests/test_ra_reflow_image.c -- the smallest raster
 * that probes to a valid intrinsic size so layout records an image box.
 */
 ...
 * @param[in]  href     Unused image href.
 * @param[in]  href_len Unused href length.
 * @param[out] out      Unused (left untouched).
 * @param[out] out_len  Unused (left untouched).
 * @return k_ra_err_not_found always.
 */
static ra_err_t
...
static ra_err_t init_eng(uint16_t w, uint16_t h)
{
...
static uint32_t lay(const char* doc, uint16_t w, uint16_t h)
{
  uint32_t pages = 0U;
  TEST_ASSERT_EQ(k_ra_ok, init_eng(w, h));
  TEST_ASSERT_EQ(
    k_ra_ok,
...
/**
 * @test test_cov_heading_font_scales
 * @brief Lay out `<h4>..<h6>` to execute the h4/h5/h6 arms of priv_block_font_px.
 *
 * @details Targets ra_reflow_layout.c lines 293-301 (the h4/h5/h6 switch cases)
...
static void test_cov_heading_font_scales(void)
{
  ...
  const uint32_t pages = lay(doc, (uint16_t)k_cov_vp_w, (uint16_t)k_cov_vp_h);
...
/**
 * @test test_cov_bad_font_run_layout
 * @brief A junk font blob makes ra_reflow_run_layout fail validation.
 *
 * @details Targets ra_reflow_layout.c lines 231-235 (priv_init_font's offset<0 /
 * InitFont==0 returns) and 1601-1602 (ra_reflow_run_layout propagating that
...
static void test_cov_bad_font_run_layout(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_reflow_init((uint16_t)k_cov_vp_w,
                                (uint16_t)k_cov_vp_h,
                                s_junk_font,
  ...
  /* Also drive ra_reflow_run_layout directly (it re-inits the same junk font). */
  TEST_ASSERT_EQ(k_ra_err_validation_failed, ra_reflow_run_layout(&s_eng));
  TEST_END("layout cov: junk font -> run_layout validation_failed");
}

/**
 * @test test_cov_multipage_finish_page
 * @brief Long content over a short viewport forces page breaks.
 *
 * @details Targets ra_reflow_layout.c lines 401-415 (priv_finish_page body
...
/**
 * @test test_cov_justify_remainder_and_no_space
 * @brief Justify a wrapped multi-space line (remainder spread) and a no-space line.
 *
 * @details Targets ra_reflow_layout.c lines 445-461 (priv_justify_glyphs: the
...
static void test_cov_justify_remainder_and_no_space(void)
{
  ...
  uint32_t pages = lay(doc, (uint16_t)k_cov_vp_w, (uint16_t)k_cov_vp_h_tall);
  TEST_ASSERT(pages >= 1U);
  TEST_ASSERT(s_eng.glyph_count > 0U);

  /* A justified line whose only content is one long unbroken word: when it
     wraps, the wrapped line has no inter-word space, so priv_justify_glyphs
     takes the spaces==0 early return. */
  const char* doc2 =
    "<p style=\"text-align:justify\">aaaaaaaaaaaaaaaaaaaaaaaa bbbbbbbbbbbbbbbbbbbbbbbb</p>";
  pages = lay(doc2, (uint16_t)k_cov_vp_w_narrow, (uint16_t)k_cov_vp_h_tall);
  TEST_ASSERT(pages >= 1U);
  TEST_ASSERT(s_eng.glyph_count > 0U);
...
/**
 * @test test_cov_right_align_slack_nonpositive
 * @brief A right-aligned line wider than the column takes the slack<=0 early-out.
 *
 * @details Targets ra_reflow_layout.c line 500-501 (priv_finish_line's
...
static void test_cov_right_align_slack_nonpositive(void)
{
  ...
  const uint32_t pages = lay(doc, (uint16_t)k_cov_vp_w_narrow, (uint16_t)k_cov_vp_h_tall);
  TEST_ASSERT(pages >= 1U);
  TEST_ASSERT(s_eng.glyph_count > 0U);
...
/**
 * @test test_cov_open_block_and_rule_with_content
 * @brief Open a block and a rule while the current line already has glyphs.
 *
 * @details Targets ra_reflow_layout.c lines 698-700 (priv_open_block's
 * line-flush newline when line_has_content) and 789-791 (priv_apply_rule's
...
static void test_cov_open_block_and_rule_with_content(void)
{
  ...
  const uint32_t pages = lay(doc, (uint16_t)k_cov_vp_w, (uint16_t)k_cov_vp_h_tall);
  TEST_ASSERT(pages >= 1U);
  TEST_ASSERT(s_eng.glyph_count > 0U);
...
/**
 * @test test_cov_image_placeholder_wrap
 * @brief An `<img>` with no loader bound wraps when it overflows the line.
 *
 * @details Targets ra_reflow_layout.c lines 822-829 (priv_apply_image_placeholder
...
/**
 * @test test_cov_image_loader_failure_fallback
 * @brief A failing image loader makes priv_image_resolve_size return false.
 *
 * @details Targets ra_reflow_layout.c lines 916-917 (priv_image_resolve_size
...
static void test_cov_image_loader_failure_fallback(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, init_eng((uint16_t)k_cov_vp_w, (uint16_t)k_cov_vp_h_tall));
  ra_img_arena_t arena = {.base = s_arena_buf, .cap = sizeof s_arena_buf, .offset = 0U, .live = 0U};
  TEST_ASSERT_EQ(k_ra_ok, ra_reflow_set_image_loader(&s_eng, fail_loader, nullptr, &arena));
...
/**
 * @test test_cov_image_real_block
 * @brief A bound loader resolves an `<img>` to a real, recorded image box.
 *
 * @details Targets ra_reflow_layout.c priv_place_image / priv_image_resolve_size
...
static void test_cov_image_real_block(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, init_eng((uint16_t)k_cov_vp_w, (uint16_t)k_cov_vp_h_tall));
  ra_img_arena_t arena = {.base = s_arena_buf, .cap = sizeof s_arena_buf, .offset = 0U, .live = 0U};
  TEST_ASSERT_EQ(k_ra_ok, ra_reflow_set_image_loader(&s_eng, png_loader, nullptr, &arena));
...
/**
 * @test test_cov_table_layout
 * @brief A multi-row table exercises the column / row / cell scan loops.
 *
 * @details Targets ra_reflow_layout.c's table family: priv_table_columns
 * (lines 1254-1255 non-row skip), priv_row_cells (lines 1404-1405 non-cell
...
/**
 * @test test_cov_table_with_dirty_line_and_empty
 * @brief A table opened mid-line, and an empty table, hit the table edge cases.
 *
 * @details Targets ra_reflow_layout.c lines 1482-1484 (priv_layout_table's
...
static void test_cov_table_with_dirty_line_and_empty(void)
{
  ...
  /* An empty table (no cells) -> priv_table_columns returns 0 -> cols==0 path. */
  const char* doc2 = "<p>x</p><table></table><p>y</p>";
  pages            = lay(doc2, (uint16_t)k_cov_vp_w, (uint16_t)k_cov_vp_h_tall);
  TEST_ASSERT(pages >= 1U);
  TEST_ASSERT(s_eng.glyph_count > 0U);
...
/**
 * @test test_cov_table_nested_depth
 * @brief A nested `<table>` drives priv_match_block_end's depth-increment.
 *
 * @details Targets ra_reflow_layout.c line 1213 (priv_match_block_end's
...
/**
 * @test test_cov_lifecycle_guards
 * @brief Null / not-initialised guards on run_layout + the loader setters.
 *
 * @details Targets ra_reflow_layout.c lines 1585-1589 (ra_reflow_run_layout),
 * 1686-1690 (ra_reflow_set_image_loader) and 1698-1703 (ra_reflow_set_css_loader)
...
static void test_cov_lifecycle_guards(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_reflow_run_layout(nullptr));
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_reflow_run_layout(&closed));

  TEST_ASSERT_EQ(k_ra_err_null_ptr,
                 ra_reflow_set_image_loader(nullptr, png_loader, nullptr, nullptr));
  TEST_ASSERT_EQ(k_ra_err_not_initialized,
                 ra_reflow_set_image_loader(&closed, png_loader, nullptr, nullptr));

  TEST_ASSERT_EQ(k_ra_err_null_ptr, ra_reflow_set_css_loader(nullptr, nullptr, nullptr));
  TEST_ASSERT_EQ(k_ra_err_not_initialized, ra_reflow_set_css_loader(&closed, nullptr, nullptr));
...
int32_t main(void)
{
  test_cov_heading_font_scales();
  test_cov_bad_font_run_layout();
  test_cov_multipage_finish_page();
  test_cov_justify_remainder_and_no_space();
  test_cov_right_align_slack_nonpositive();
  test_cov_open_block_and_rule_with_content();
  test_cov_image_placeholder_wrap();
  test_cov_image_loader_failure_fallback();
  test_cov_image_real_block();
  test_cov_table_layout();
  test_cov_table_with_dirty_line_and_empty();
  test_cov_table_nested_depth();
  test_cov_lifecycle_guards();
  (void)fprintf(stderr, "[OK ] test_ra_reflow_layout_cov.c\n");
  return 0;
}
