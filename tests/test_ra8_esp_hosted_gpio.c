...
static mock_level_t* mock_row(ra8_port_pin_t pin)
{
...
{
  return mock_output_init(ctx, pin, level);
}
...
static ra8_err_t mock_read(void* ctx, ra8_port_pin_t pin, ra8_level_t* out_level)
{
  ...
  const mock_level_t* row = mock_row(pin);
  if (row == nullptr) {
    return k_ra8_err_no_mem;
  }
...
static void set_level(ra8_port_pin_t pin, uint8_t level)
{
  mock_level_t* row = mock_row(pin);
...
static uint8_t get_level(ra8_port_pin_t pin)
{
  const mock_level_t* row = mock_row(pin);
  TEST_ASSERT_NOT_NULL((void*)row);
  return row->level;
...
static hosted_osi_funcs_t bound_vtable(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_esp_hosted_gpio_bind(&funcs));
  return funcs;
}
...
static void reset_state(void)
{
  ...
  ra8_esp_hosted_gpio_set_pin_interface(&s_mock_pin_if);
}
 ...
 * pin_max)` in
 * `port/esp-hosted/src/ra8_esp_hosted_gpio.c@ra8_esp_hosted_gpio_decode_pin`
 * (3 conditions).
 ...
 */
static void test_decode_pin(void)
{
  ...
  TEST_ASSERT(ra8_esp_hosted_gpio_decode_pin((void*)(uintptr_t)k_gpio_test_port_zero,
  ...
  TEST_ASSERT(ra8_esp_hosted_gpio_decode_pin(RA8_ESP_HOSTED_GPIO_PORT(src),
  ...
  TEST_ASSERT(!ra8_esp_hosted_gpio_decode_pin((void*)(uintptr_t)k_gpio_test_bad_port,
                                              (uint32_t)k_gpio_test_pin_three,
                                              &pin));
...
static void test_bind(void)
{
  ...
  reset_state();
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_esp_hosted_gpio_bind(nullptr));
 ...
 * decode rejection)
 */
static void test_config_gpio(void)
{
  ...
  TEST_ASSERT_EQ(H_RESET_VAL_INACTIVE, get_level(pin));
...
static void test_read_write(void)
{
  ...
  set_level(pin, (uint8_t)H_DR_VAL_ACTIVE);
  ...
  TEST_ASSERT_EQ(H_RESET_VAL_ACTIVE, get_level(pin));
 ...
 */
static void test_hold_and_wakeup_reason(void)
{
...
/**
 * @par MC/DC:
 * `ra8_esp_hosted_gpio_edge_seen` is a chain of five single-condition
 ...
 */
static void test_edge_seen_matrix(void)
{
  TEST_BEGIN("esp_hosted gpio: edge detection across every sense and pair");
  reset_state();

  /* Falling: only a 1 -> 0 transition counts. */
  TEST_ASSERT(!ra8_esp_hosted_gpio_edge_seen(0U, 0U, (uint8_t)k_ra8_icu_irqmd_falling));
  TEST_ASSERT(!ra8_esp_hosted_gpio_edge_seen(0U, 1U, (uint8_t)k_ra8_icu_irqmd_falling));
  TEST_ASSERT(ra8_esp_hosted_gpio_edge_seen(1U, 0U, (uint8_t)k_ra8_icu_irqmd_falling));
  TEST_ASSERT(!ra8_esp_hosted_gpio_edge_seen(1U, 1U, (uint8_t)k_ra8_icu_irqmd_falling));

  /* Rising: only a 0 -> 1 transition counts. */
  TEST_ASSERT(!ra8_esp_hosted_gpio_edge_seen(0U, 0U, (uint8_t)k_ra8_icu_irqmd_rising));
  TEST_ASSERT(ra8_esp_hosted_gpio_edge_seen(0U, 1U, (uint8_t)k_ra8_icu_irqmd_rising));
  TEST_ASSERT(!ra8_esp_hosted_gpio_edge_seen(1U, 0U, (uint8_t)k_ra8_icu_irqmd_rising));
  TEST_ASSERT(!ra8_esp_hosted_gpio_edge_seen(1U, 1U, (uint8_t)k_ra8_icu_irqmd_rising));

  /* Both: any transition counts, no transition does not. */
  TEST_ASSERT(!ra8_esp_hosted_gpio_edge_seen(0U, 0U, (uint8_t)k_ra8_icu_irqmd_both));
  TEST_ASSERT(ra8_esp_hosted_gpio_edge_seen(0U, 1U, (uint8_t)k_ra8_icu_irqmd_both));
  TEST_ASSERT(ra8_esp_hosted_gpio_edge_seen(1U, 0U, (uint8_t)k_ra8_icu_irqmd_both));
  TEST_ASSERT(!ra8_esp_hosted_gpio_edge_seen(1U, 1U, (uint8_t)k_ra8_icu_irqmd_both));

  /* Low level: the current sample alone decides, exactly as the ICU does. */
  TEST_ASSERT(ra8_esp_hosted_gpio_edge_seen(0U, 0U, (uint8_t)k_ra8_icu_irqmd_low));
  TEST_ASSERT(!ra8_esp_hosted_gpio_edge_seen(0U, 1U, (uint8_t)k_ra8_icu_irqmd_low));
  TEST_ASSERT(ra8_esp_hosted_gpio_edge_seen(1U, 0U, (uint8_t)k_ra8_icu_irqmd_low));
  TEST_ASSERT(!ra8_esp_hosted_gpio_edge_seen(1U, 1U, (uint8_t)k_ra8_icu_irqmd_low));

  /* An unknown sense reports no event rather than guessing one. */
  TEST_ASSERT(!ra8_esp_hosted_gpio_edge_seen(0U, 1U, (uint8_t)k_gpio_test_bad_sense));
 ...
 *
 * The trampoline's guard `if ((row == nullptr) || (row->handler == nullptr))`
 * is installed by the registration below but cannot be driven from a host
 * test: it runs only from an ICU vector, and there is no host seam that
 * raises one.
 * Neither arm is reachable either -- the trampoline is only ever installed
 * with the address of a row that has just been filled in, so a null context
 * or a null handler would mean the registration itself had gone wrong. The
 * guard exists because the ICU hands the context back as a bare `void*`.
 */
static void test_hardware_interrupt_path(void)
{
  ...
  TEST_ASSERT_EQ(
    RET_OK,
    f._h_config_gpio_as_interrupt(prt, num, (uint32_t)H_DR_INTR_EDGE, count_handler, &s_levels[0]));
  ...
  TEST_ASSERT_EQ(
    RET_FAIL,
    f._h_config_gpio_as_interrupt(prt, num, (uint32_t)H_DR_INTR_EDGE, count_handler, nullptr));
 ...
 * out-of-range sense)
 */
static void test_interrupt_rejections(void)
{
 ...
 * `port/esp-hosted/src/ra8_esp_hosted_gpio_edge.c@ra8_esp_hosted_gpio_edge_poll_once`
 * (2 conditions).
 ...
 */
static void test_polled_path(void)
{
  ...
  set_level(pin, (uint8_t)H_HS_VAL_ACTIVE);
  ra8_esp_hosted_gpio_edge_poll_once();
 ...
 * same for the pin comparison. N+1 = 3 vectors for N=2: minimal MC/DC.
 */
static void test_polled_table_bounds(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra8_err_null_ptr,
    ra8_esp_hosted_gpio_edge_register(RA8_PIN(k_ra8_port_4, k_ra8_pin_0), sense, nullptr, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 ra8_esp_hosted_gpio_edge_register(RA8_PIN(k_ra8_port_4, k_ra8_pin_0),
                                                   (uint8_t)k_gpio_test_bad_sense,
                                                   count_handler,
                                                   nullptr));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, ra8_esp_hosted_gpio_set_edge_poll_ms(0U));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_esp_hosted_gpio_set_edge_poll_ms((uint16_t)k_gpio_test_poll_ms));
  ...
  for (uint8_t i = 0U; i < (uint8_t)k_ra8_esp_hosted_gpio_row_max; i++) {
    ...
    TEST_ASSERT_EQ(k_ra8_ok, ra8_esp_hosted_gpio_edge_register(pin, sense, count_handler, nullptr));
    TEST_ASSERT_EQ(k_ra8_err_exists,
                   ra8_esp_hosted_gpio_edge_register(pin, sense, count_handler, nullptr));
  }
  TEST_ASSERT_EQ(k_ra8_esp_hosted_gpio_row_max, ra8_esp_hosted_gpio_edge_count());

  const ra8_port_pin_t overflow = RA8_PIN(k_ra8_port_4, k_ra8_esp_hosted_gpio_row_max);
  TEST_ASSERT_EQ(k_ra8_err_no_mem,
                 ra8_esp_hosted_gpio_edge_register(overflow, sense, count_handler, nullptr));

  TEST_ASSERT_EQ(k_ra8_err_not_found, ra8_esp_hosted_gpio_edge_unregister(overflow));
  for (uint8_t i = 0U; i < (uint8_t)k_ra8_esp_hosted_gpio_row_max; i++) {
    TEST_ASSERT_EQ(k_ra8_ok, ra8_esp_hosted_gpio_edge_unregister(RA8_PIN(k_ra8_port_4, i)));
  }
  TEST_ASSERT_EQ(0U, ra8_esp_hosted_gpio_edge_count());
...
static void test_pin_interface_seam(void)
{
  ...
  reset_state();
  TEST_ASSERT(ra8_esp_hosted_gpio_pin_interface() == &s_mock_pin_if);
  ra8_esp_hosted_gpio_set_pin_interface(nullptr);
  TEST_ASSERT_NOT_NULL((void*)ra8_esp_hosted_gpio_pin_interface());
  TEST_ASSERT(ra8_esp_hosted_gpio_pin_interface() != &s_mock_pin_if);
  TEST_ASSERT_NOT_NULL((void*)ra8_esp_hosted_gpio_pin_interface()->read);
  ra8_esp_hosted_gpio_set_pin_interface(&s_mock_pin_if);
...
static void (*const s_test_roster[])(void) = {
  ...
  test_read_write,
  test_pull,
  test_hold_and_wakeup_reason,
  test_edge_seen_matrix,
  test_hardware_interrupt_path,
  test_interrupt_rejections,
  test_polled_path,
  test_polled_table_bounds,
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra8_esp_hosted_gpio.c\n");
  return 0;
}
