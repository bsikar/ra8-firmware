/**
 * @file test_app_threadx_fs_levelx_demo.c
 * @brief Integration test: ra8_fs-on-LevelX FAT format + read/write flow
 *
 * @details
 * The production app at
 * examples/ek_ra8d2/hw_validated/hil/threadx_fs_levelx_demo/main.c stands up
 * ThreadX + LevelX + ra8_fs, lays a FAT superblock on top of LevelX-managed
 * wear-levelled NOR sectors (via port/levelx/src/lx_fs_backend.c), then
 * writes and reads a known message back. Neither ThreadX nor LevelX are
 * linked into the host test build (RA8_OFF_TARGET), so this test exercises
 * the same ra8_fs call surface the app drives:
 *   - ra8_cgc / ra8_time pre-kernel boot.
 *   - The SCI panic-flush helper used by demo_panic_halt().
 *   - ra8_fs precondition guards on a fresh / partially-initialized backend.
 * (The full LevelX-backed roundtrip -- format, mount, write, read, unlink over
 * a real LevelX instance on a RAM NOR fake -- lives in
 * tests/test_lx_fs_backend.c.)
 *
...
/**
 * @enum app_threadx_fs_levelx_demo_fixture_t
...
} app_threadx_fs_levelx_demo_fixture_t;

/** @brief Per-test enums. */
typedef enum : uint32_t {
  k_test_fslx_block_count = 256U, /**< Mock backend size in sectors.        */
  k_test_fslx_block_size  = 512U, /**< FAT bytes-per-sector.                */
  k_test_fslx_sci_chan    = 8U,   /**< SCI channel routed to J-Link OB CDC. */
} test_fslx_const_t;

/* -------------------------------------------------------------------------
 * Mock block-device backend (returns zeroes -> mount fails BPB validation,
 * which is exactly what the production demo path would trigger before its
 * own ra8_fs_format).
 * ------------------------------------------------------------------------- */
...
static ra8_err_t mock_read_block(void* ctx, uint64_t lba, uint32_t count, uint8_t* buf)
{
  ...
  (void)memset(buf, 0, (size_t)count * (size_t)k_test_fslx_block_size);
  return k_ra8_ok;
}
...
static ra8_err_t mock_get_capacity(void* ctx, uint64_t* block_count, uint32_t* block_size)
{
  ...
  *block_count = (uint32_t)k_test_fslx_block_count;
  *block_size  = (uint32_t)k_test_fslx_block_size;
  return k_ra8_ok;
}
 ...
 */
static void test_fslx_pre_kernel_bringup(void)
{
  reset_world();
  TEST_BEGIN("fslx_demo: pre-kernel CGC + clock readback");
  ...
  TEST_END("fslx_demo: pre-kernel CGC + clock readback");
}

/**
 * @brief Mount call rejects NULL backend (backend bind precondition equivalent).
 *
 * @par MC/DC:
 ...
 * covers backend==NULL.
 */
static void test_fslx_mount_rejects_null_backend(void)
{
  reset_world();
  TEST_BEGIN("fslx_demo: mount(NULL backend) rejected");
  ra8_fs_mount_t* handle = nullptr;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_fs_mount(nullptr, &handle));
  TEST_END("fslx_demo: mount(NULL backend) rejected");
}
 ...
 */
static void test_fslx_mount_rejects_null_handle(void)
{
  reset_world();
  TEST_BEGIN("fslx_demo: mount(NULL out_handle) rejected");
  const ra8_fs_backend_t backend = make_mock_backend();
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_fs_mount(&backend, nullptr));
  TEST_END("fslx_demo: mount(NULL out_handle) rejected");
}
 ...
 * walks: a fresh LevelX partition has no BPB, so ra8_fs_mount returns
 * an error -- the demo therefore formats before mounting. We model
 * the mount-on-empty-device step here.
 *
 ...
 */
static void test_fslx_mount_rejects_unformatted(void)
{
  reset_world();
  TEST_BEGIN("fslx_demo: mount of unformatted device fails");
  const ra8_fs_backend_t backend = make_mock_backend();
  ra8_fs_mount_t*        handle  = nullptr;
  TEST_ASSERT(ra8_fs_mount(&backend, &handle) != k_ra8_ok);
  TEST_END("fslx_demo: mount of unformatted device fails");
}
 ...
 */
static void test_fslx_mount_rejects_partial_backend(void)
{
  reset_world();
  TEST_BEGIN("fslx_demo: mount rejects backend with NULL get_capacity");
  ra8_fs_backend_t backend = make_mock_backend();
  backend.get_capacity     = nullptr;
  ra8_fs_mount_t* handle   = nullptr;
  TEST_ASSERT(ra8_fs_mount(&backend, &handle) != k_ra8_ok);
  TEST_END("fslx_demo: mount rejects backend with NULL get_capacity");
}
 ...
 */
static void test_fslx_panic_flush_before_init(void)
{
  reset_world();
  TEST_BEGIN("fslx_demo: panic flush before SCI init returns cleanly");
  ...
  (void)ra8_sci_flush((uint8_t)k_test_fslx_sci_chan);
  TEST_END("fslx_demo: panic flush before SCI init returns cleanly");
}

int main(void)
{
  test_fslx_pre_kernel_bringup();
  test_fslx_mount_rejects_null_backend();
  test_fslx_mount_rejects_null_handle();
  test_fslx_mount_rejects_unformatted();
  test_fslx_mount_rejects_partial_backend();
  test_fslx_panic_flush_before_init();
  return 0;
}
