/**
 * @file test_ra_i3c_i2c_peripheral_cov.c
 * @brief Coverage-extension tests for the IIC_B peripheral driver.
 *
 * @details
 * Targets the branches in ra_i3c_i2c_peripheral.c that are not reached
 * by test_ra_i3c_i2c_peripheral.c:
 *
 ...
 * test_ra_i3c_i2c_peripheral, so coverage from both binaries accumulates.
 *
...
/**
 * @enum ra_i3c_i2c_peripheral_cov_const_t
...
} ra_i3c_i2c_peripheral_cov_const_t;
 ...
 *
 * @details
 * Exercises the `cfg->peripheral_addr_7b > k_ra_i3c_i2c_peripheral_max_addr_7b`
 * guard inside internal_i3c_i2c_peripheral_open. Passing 0x80 must yield
 * k_ra_err_invalid_arg without touching the hardware registers.
 *
 * @pre ra_i3c_i2c_peripheral module compiled with RA_SIMULATOR_MODE.
 * @post No register side-effects (guard fires before any register write).
 *
 * @note Not thread-safe; tests are single-threaded.
 *
 * @par MC/DC:
 * Decision: `cfg->peripheral_addr_7b > k_ra_i3c_i2c_peripheral_max_addr_7b`
 * (single condition, no compound boolean).
 * - Vector 1: addr=0x42 -> dec F (k_ra_ok)              [test_open_close]
 * - Vector 2: addr=0x80 -> dec T (k_ra_err_invalid_arg) [this test]
...
static void test_cov_open_addr_oor(void)
{
  ...
  ra_i3c_i2c_peripheral_cfg_t cfg_oor = {
 ...
 * k_ra_err_invalid_arg immediately.
 *
 * @pre ra_i3c_i2c_peripheral module compiled with RA_SIMULATOR_MODE.
 ...
 * - Vector 1: ch=0  -> dec F (k_ra_ok)              [test_open_close]
 * - Vector 2: ch=9  -> dec T (k_ra_err_invalid_arg) [this test]
...
static void test_cov_close_oor_channel(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, internal_i3c_i2c_peripheral_close((uint8_t)k_cov_ch_oor));
 ...
 * @par MC/DC:
 * Decision: `reg == nullptr` (single condition).
 * - Vector 1: ch=0  -> dec F (k_ra_ok)              [test_send_ok]
 * - Vector 2: ch=9  -> dec T (k_ra_err_invalid_arg) [this test]
 * N+1 = 2 satisfied jointly.
 *
 * @since 0.1.0
 ...
 * @par MC/DC:
 * Decision: `reg == nullptr` (single condition).
 * - Vector 1: ch=0  -> dec F (k_ra_ok)              [test_receive_ok]
 * - Vector 2: ch=9  -> dec T (k_ra_err_invalid_arg) [this test]
 * N+1 = 2 satisfied jointly.
 *
 * @since 0.1.0
 ...
 * A valid (non-null) out_mask pointer is supplied so RA_CHECK_NULL_PTR
 ...
 * @par MC/DC:
 * Decision: `reg == nullptr` (single condition).
 * - Vector 1: ch=0  -> dec F (k_ra_ok)              [test_status]
 * - Vector 2: ch=9  -> dec T (k_ra_err_invalid_arg) [this test]
 * N+1 = 2 satisfied jointly.
 *
 * @since 0.1.0
 ...
 *
 * @details
 * Exercises the `(bst & k_ra_i3c_i2c_msk_bst_spcnddf) != 0U` branch
 * in internal_i3c_i2c_peripheral_status. Pre-seeding BST with
 * k_ra_i3c_i2c_msk_bst_spcnddf (bit 1, representing a Stop Condition
 * Detected Flag) causes the status function to OR in
 * k_ra_i3c_i2c_peripheral_status_stop. The existing test_status covers
 * only the NACKDF bit and leaves this if-arm uncovered.
 *
 * @pre ra_i3c_i2c_peripheral module compiled with RA_SIMULATOR_MODE.
 * @pre ra_sim_mmap_reset() clears all backing registers to zero.
 * @post mask includes k_ra_i3c_i2c_peripheral_status_stop.
 *
 * @note Not thread-safe; tests are single-threaded.
 *
 * @par MC/DC:
 * Decision: `(bst & k_ra_i3c_i2c_msk_bst_spcnddf) != 0U` (single condition).
...
static void test_cov_status_stop_bit(void)
{
  ...
  TEST_ASSERT(mask & (uint8_t)k_ra_i3c_i2c_peripheral_status_stop);
 ...
 * k_ra_i3c_i2c_peripheral_spin_budget (50000) iterations and
 * internal_wait_ntst returns k_ra_err_hw_timeout. RA_RETURN_ON_ERROR
 ...
 * ra_sim_mmap_reset(); no register pre-seeding is required to reach
 * the timeout path.
 *
 * @pre ra_i3c_i2c_peripheral module compiled with RA_SIMULATOR_MODE.
 * @pre ra_sim_mmap_reset() clears NTST to zero.
 * @post Function returns k_ra_err_hw_timeout.
 *
 * @note Not thread-safe; tests are single-threaded.
 * @note The spin is deterministic: it always takes exactly
 *       k_ra_i3c_i2c_peripheral_spin_budget iterations before returning.
 *
 * @par MC/DC:
 * Decision (loop exhaustion): `i < k_ra_i3c_i2c_peripheral_spin_budget`
 ...
 * - Vector 1: NTST.TDBEF0 set before budget exhausts -> k_ra_ok [test_send_ok]
 * - Vector 2: NTST.TDBEF0 never set, budget exhausts -> k_ra_err_hw_timeout
 *             [this test]
 * N+1 = 2 satisfied jointly across the suite.
 *
 * @since 0.1.0
