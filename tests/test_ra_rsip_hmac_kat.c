 ...
 *
 * @details
 * On the host build ``RA_RSIP_SOFTWARE_BACKEND`` computes HMAC-SHA-256 (RFC 2104)
 ...
 * -- a tautology. This test instead pins ra_rsip_hmac_sha256_{init,update,final}
...
{
  uint8_t expect[k_ra_rsip_sha256_digest_bytes];
  hex_to_bytes(expect_hex, expect, (size_t)k_ra_rsip_sha256_digest_bytes);

  ra_rsip_hmac_sha256_ctx_t ctx = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_rsip_hmac_sha256_init(&ctx, key, key_len));
  TEST_ASSERT_EQ(k_ra_ok, ra_rsip_hmac_sha256_update(&ctx, data, data_len));
  uint8_t mac[k_ra_rsip_sha256_digest_bytes] = {};
  TEST_ASSERT_EQ(k_ra_ok, ra_rsip_hmac_sha256_final(&ctx, mac));
  TEST_ASSERT_EQ(0, memcmp(mac, expect, (size_t)k_ra_rsip_sha256_digest_bytes));
}
 ...
 * decision logic is exercised by the ra_rsip driver's own tests.)
 */
static void test_hmac_sha256_rfc4231(void)
{
  ...
  const ra_err_t         init_err = ra_rsip_init(&cfg);
  if (init_err != k_ra_ok) {
    TEST_ASSERT_EQ(k_ra_err_exists, init_err);
  }
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_rsip_hmac_kat.c\n");
  return 0;
}
