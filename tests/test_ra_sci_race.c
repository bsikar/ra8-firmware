/**
 * @file test_ra_sci_race.c
 * @brief Concurrency-race regression tests for the SCI_B driver.
 *
 * @details
 * Covers recon finding T1-02: the runtime-reconfigure CCR2
 * read-modify-write (``ra_sci_set_baud``) and the channel teardown
 * (``ra_sci_deinit``) are wrapped in the existing ``ra_register_guard``
 ...
 * under ``RA_SIMULATOR_MODE``, so these tests assert the *result* of the
 ...
 * through the simulated MMIO window provided by ``ra_sim_mmap``.
 *
...
} ra_sci_race_channel_t;

/**
 * @enum ra_sci_race_len_t
 ...
 */
static const ra_sci_cfg_t k_race_cfg = {
  .baud      = 115200U,
 ...
 *
 * @details Mirrors the ``prep`` helper in ``test_ra_sci.c``: a clean
...
{
  TEST_BEGIN("ra_sci_set_baud: guarded RMW updates BRR, preserves CCR2/CCR0");
  prep_race();
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_init((uint8_t)k_sci_race_ch_baud, &k_race_cfg));

  volatile r_sci_regs_t* reg = ra_sci((uint8_t)k_sci_race_ch_baud);
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_sci_baud_calculate((uint32_t)k_sci_race_baud_target,
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_sci_set_baud((uint8_t)k_sci_race_ch_baud,
                                 (uint32_t)k_sci_race_baud_target,
                                 (uint32_t)k_sci_race_pclk_hz));

  const uint32_t brr_mask   = (uint32_t)k_ra_sci_ccr2_mask_brr_field;
  const uint32_t ccr2_after = reg->CCR2;
  /* BRR field holds the recomputed value. */
  TEST_ASSERT_EQ(((uint32_t)expect_brr << (uint8_t)k_ra_sci_ccr2_shift_brr),
                 (ccr2_after & brr_mask));
  ...
  TEST_END("ra_sci_set_baud: guarded RMW updates BRR, preserves CCR2/CCR0");
}

/**
 * @par MC/DC:
 * (no compound decisions in this test -- exercises the two single-condition
 * validation guards of ra_sci_set_baud individually)
 */
static void test_set_baud_validation(void)
{
  TEST_BEGIN("ra_sci_set_baud: bad channel + zero baud rejected");
  prep_race();
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_init((uint8_t)k_sci_race_ch_baud, &k_race_cfg));

  /* Out-of-range channel -> internal_reg() returns NULL. */
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_sci_set_baud((uint8_t)k_sci_race_ch_bad,
                                 (uint32_t)k_sci_race_baud_target,
                                 (uint32_t)k_sci_race_pclk_hz));
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_sci_set_baud((uint8_t)k_sci_race_ch_baud,
                                 ...
                                 (uint32_t)k_sci_race_pclk_hz));
  TEST_END("ra_sci_set_baud: bad channel + zero baud rejected");
}
...
{
  TEST_BEGIN("ra_sci_deinit: guarded teardown clears CCR0 + async state");
  prep_race();
  TEST_ASSERT_EQ(k_ra_ok, ra_sci_init((uint8_t)k_sci_race_ch_deinit, &k_race_cfg));
  ...
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_sci_read((uint8_t)k_sci_race_ch_deinit, rx_buf, (uint32_t)k_sci_race_len));
  TEST_ASSERT_EQ(k_ra_ok,
                 ra_sci_write((uint8_t)k_sci_race_ch_deinit, tx_buf, (uint32_t)k_sci_race_len));

  volatile r_sci_regs_t* reg = ra_sci((uint8_t)k_sci_race_ch_deinit);
  TEST_ASSERT((reg->CCR0 & (1U << (uint8_t)k_ra_sci_ccr0_bit_rie)) != 0U);
  TEST_ASSERT((reg->CCR0 & (1U << (uint8_t)k_ra_sci_ccr0_bit_tie)) != 0U);

  TEST_ASSERT_EQ(k_ra_ok, ra_sci_deinit((uint8_t)k_sci_race_ch_deinit));
  ...
  reg->RDR = 0xA5U;
  ra_sci_dispatch_rxi((uint8_t)k_sci_race_ch_deinit);
  ra_sci_dispatch_txi((uint8_t)k_sci_race_ch_deinit);
  TEST_ASSERT_EQ(0U, reg->CCR0);
  ...
  TEST_ASSERT_EQ(k_ra_err_invalid_arg,
                 ra_sci_write((uint8_t)k_sci_race_ch_deinit, tx_buf, (uint32_t)k_sci_race_len));
  TEST_END("ra_sci_deinit: guarded teardown clears CCR0 + async state");
}

/**
 * @par MC/DC:
 * (no compound decisions in this test -- exercises the single-condition
 * bad-channel guard of ra_sci_deinit)
 */
static void test_deinit_validation(void)
{
  TEST_BEGIN("ra_sci_deinit: bad channel rejected");
  prep_race();
  TEST_ASSERT_EQ(k_ra_err_invalid_arg, ra_sci_deinit((uint8_t)k_sci_race_ch_bad));
  TEST_END("ra_sci_deinit: bad channel rejected");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_sci_race.c\n");
  return 0;
}
