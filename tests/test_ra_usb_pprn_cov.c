/**
 * @file test_ra_usb_pprn_cov.c
 * @brief Coverage top-up for the native USB device-side Printer class
 *        layer (`libs/ra_hal/src/ra_usb_pprn.c`)
 *
 * @details
 * Companion to `test_ra_usb_pprn.c`. That suite covers the public-API
 * happy path and the argument-rejection contract; this file drives the
 * residual legs the original suite never reaches:
 *
 *  - `ra_usb_pprn_init` hardware-init-failed leg: the underlying
 *    `ra_usb_device_init` is forced to fail by saturating the USBFS
 *    module-stop reference count (255 pending users) so the controller's
 *    `ra_mstp_enable` returns `k_ra_err_invalid_state`. The class layer
 *    logs the failure and remaps it to `k_ra_err_hw_init_failed`.
 *  - `ra_usb_pprn_recv` forwarding into `ra_usb_queue_out`: the success
 ...
 *    (BRDY status bit cleared so the drain reports `k_ra_err_no_data` and
 *    the caller length is zeroed). The original suite only ever exercised
 *    the pre-forward argument guards.
 *  - `ra_usb_pprn_handle_setup` callback-error leg: an attached class
 *    handler returns a non-ok status, so the dispatcher STALLs the
 *    control endpoint via `ra_usb_control_response(speed, false)`.
 *
 * All hardware is the host register-window model brought up by
 * `ra_sim_mmap_reset` + `ra_mstp_init`; no live controller, no timers,
 ...
 *
 * @details Mirrors the file-static pipe assignments of `ra_usb_pprn.c`
 ...
 *
 * @details Returns `k_ra_err_not_supported` so the dispatcher in
 * `ra_usb_pprn_handle_setup` takes the STALL leg. Records that it ran.
 *
 * @param[in] ctx Opaque context (unused).
 * @param[in] setup SETUP packet (unused).
 * @return Always `k_ra_err_not_supported`.
 * @retval k_ra_err_not_supported Forces the caller onto the STALL path.
 ...
 * @since 0.1.0
 */
static ra_err_t test_fail_cb(void* ctx, const ra_usb_setup_t* setup)
{
  (void)ctx;
  (void)setup;
  s_cb_calls++;
 ...
 * enables) so the next `ra_mstp_enable` inside `ra_usb_device_init`
 * returns `k_ra_err_invalid_state` without ungating the module. The
 * class layer sees the non-ok status, logs it via `ra_log_error_val`,
 * and returns `k_ra_err_hw_init_failed` (source lines 203-204). This is
 ...
 *
 * @par MC/DC:
 * (no compound decision on this leg -- `if (usb_err != k_ra_ok)` is a
 * single condition. The `ra_usb_pprn_init` speed gate
 * `(speed != FS) && (speed != HS)` is not the subject here: a valid FS
 * speed makes the first condition false and short-circuits; its full
 * N+1 vector set lives in `test_ra_usb_pprn.c::test_mcdc_pprn`.)
 */
static void test_init_hw_init_failed(void)
{
  TEST_BEGIN("ra_usb_pprn_init maps a controller-init failure to hw_init_failed");
  prep();

  /* Drive the refcount to UINT8_MAX so the next enable request (issued
   * from inside ra_usb_device_init for the USBFS module) is rejected. */
  for (uint16_t i = 0U; i < (uint16_t)k_test_pprn_mstp_saturate; ++i) {
    (void)ra_mstp_enable(k_ra_mstp_usbfs);
  }

  TEST_ASSERT_EQ(k_ra_err_hw_init_failed, ra_usb_pprn_init(k_ra_usb_speed_fs));
  TEST_END("ra_usb_pprn_init maps a controller-init failure to hw_init_failed");
}
 ...
 * its BRDY status bit is set so `ra_usb_queue_out` bypasses the no-data
 * fast path, and CFIFOCTR is seeded with FRDY asserted and a non-zero
 * DTLN so the drain delivers `k_test_pprn_dtln` bytes. `ra_usb_pprn_recv`
 * therefore reaches its `ra_usb_queue_out` forward (source line 279-280),
 * observes `k_ra_ok`, and stamps the caller length from the transfer
 * result (source line 282).
 *
 * @par MC/DC:
 * (no compound decision under test -- the `if (err == k_ra_ok)` result
 * split in `ra_usb_pprn_recv` is a single condition; its false leg is
 * exercised by `test_recv_no_data_drain`.)
 */
static void test_recv_success_drain(void)
{
  TEST_BEGIN("ra_usb_pprn_recv delivers a drained OUT packet and stamps got_len");
  prep();
  ...
  reg->CFIFOCTR = (uint16_t)((uint16_t)k_ra_fifoctr_frdy | (uint16_t)k_test_pprn_dtln);
 ...
 * bit so `ra_usb_queue_out` takes its fast no-data leg and returns
 * `k_ra_err_no_data`. `ra_usb_pprn_recv` propagates the error and zeroes
 ...
 * (`err != k_ra_ok`) vector for the single-condition
 * `if (err == k_ra_ok)` result split in `ra_usb_pprn_recv`.)
 */
static void test_recv_no_data_drain(void)
{
  TEST_BEGIN("ra_usb_pprn_recv reports no_data and zeroes got_len on an empty pipe");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_pprn_init(k_ra_usb_speed_fs));
  ...
  TEST_ASSERT_EQ(k_ra_err_no_data, ra_usb_pprn_recv(buf, (uint16_t)k_test_pprn_recv_cap, &got));
  /* The error leg zeroes the reported length. */
  TEST_ASSERT_EQ(0U, got);
  TEST_END("ra_usb_pprn_recv reports no_data and zeroes got_len on an empty pipe");
}
 ...
 * `ra_usb_control_response(speed, false)` (source line 366). Under the
 * host model `ra_usb_control_response` sets the DCP PID to STALL and
 * returns `k_ra_ok`, so `ra_usb_pprn_handle_setup` returns `k_ra_ok`.
 *
 * @par MC/DC:
 * (no compound decision under test -- `if (s_state.setup_cb != nullptr)`
 * and `if (cb_err != k_ra_ok)` are independent single-condition guards.
...
{
  TEST_BEGIN("ra_usb_pprn_handle_setup STALLs the control endpoint on callback error");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_pprn_init(k_ra_usb_speed_fs));
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_pprn_attach_setup_handler(test_fail_cb, nullptr));
  ...
  TEST_END("ra_usb_pprn_handle_setup STALLs the control endpoint on callback error");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_usb_pprn_cov.c\n");
  return 0;
}
