 ...
 * ra_board_led_init(LED1) + ra_board_sw_init(SW1), then exercise
 * ra_board_sw_read + ra_board_led_on / ra_board_led_off as the
 * mainline does.
 *
 ...
 *
 * @par MC/DC:
 * Decision in app: ``*out_state == k_ra_board_sw_pressed``. One
...
/**
 * @brief Bad LED id rejected by ra_board_led_on.
 *
 * @par MC/DC:
 * Decision: ``led < k_ra_board_led_count``. One atomic condition x
...
static void test_gpio_app_bad_led(void)
{
  ...
  TEST_ASSERT(ra_board_led_on((ra_board_led_id_t)k_ra_board_led_count) != k_ra_ok);
