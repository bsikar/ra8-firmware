/**
 * @file test_ra_usb_cdc_cov.c
 * @brief Coverage-focused unit tests for the native USB CDC ACM class
 *        layer (`libs/ra_hal/src/ra_usb_cdc.c`)
 *
 * @details
 * Companion to `test_ra_usb_cdc.c`. That file covers the public-API
 ...
 *    dispatch switch.
 *  - `ra_usb_cdc_recv` forwarding to `ra_usb_queue_out` on an empty
 ...
 * `ra_usb_cdc.c` that are not exported through the public header so the
...
{
  TEST_BEGIN("ra_usb_cdc_handle_setup drives SET_LINE_CODING dispatch");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_cdc_init(k_ra_usb_speed_fs));

  ra_usb_setup_t setup = {
    .bm_request_type = (uint8_t)k_test_cdc_iface_class,
    .b_request       = (uint8_t)k_ra_cdc_req_set_line_coding,
    .w_value         = 0U,
    .w_index         = 0U,
    .w_length        = (uint16_t)k_test_cdc_line_coding_wl,
  };
  /* The data-stage stub returns k_ra_err_not_supported, so the apply
   * branch is skipped and the handler ACKs the status stage: k_ra_ok. */
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_cdc_handle_setup(&setup));
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_cdc_get_line_coding(&coding));
  TEST_ASSERT_EQ(9600U, coding.dte_rate);
  TEST_ASSERT_EQ(8U, coding.data_bits);
  TEST_END("ra_usb_cdc_handle_setup drives SET_LINE_CODING dispatch");
}
 ...
 * `default` leg and returns `k_ra_err_not_supported`. This is the only
 ...
 * bm == 0x80 (both true) vector in `test_ra_usb_cdc.c`, the second
 * condition is shown to independently affect the outcome.
 */
static void test_handle_setup_unknown_class_request(void)
{
  TEST_BEGIN("ra_usb_cdc_handle_setup default-rejects unknown class req");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_cdc_init(k_ra_usb_speed_fs));
  ...
  TEST_ASSERT_EQ(k_ra_err_not_supported, ra_usb_cdc_handle_setup(&setup));
  TEST_END("ra_usb_cdc_handle_setup default-rejects unknown class req");
}

/**
 * @test test_recv_forwards_to_queue_out
 *
 * @details After init, calls `ra_usb_cdc_recv` with a non-NULL buffer
 * and a non-zero capacity so both `RA_CHECK_NULL_PTR` guards and the
 * `*inout_len == 0` guard pass and control reaches the
 * `ra_usb_queue_out` forward. The bulk-OUT pipe's BRDYSTS status bit is
 * pre-cleared so `ra_usb_queue_out` takes its fast no-data leg
 * (BRDYSTS bit for the pipe clear -> `k_ra_err_no_data`) without
 ...
 * macros; `ra_usb_queue_out`'s own compound argument check is covered
 * in the `ra_usb_xfer` test suite)
 */
static void test_recv_forwards_to_queue_out(void)
{
  TEST_BEGIN("ra_usb_cdc_recv forwards to ra_usb_queue_out (no-data leg)");
  prep();
  TEST_ASSERT_EQ(k_ra_ok, ra_usb_cdc_init(k_ra_usb_speed_fs));

  /* Present an empty bulk-OUT pipe: clear BRDYSTS so the pipe-2 status
   * bit reads 0 and ra_usb_queue_out reports no data. */
  volatile r_usb_regs_t* reg = ra_usb_fs();
  reg->BRDYSTS               = 0U;

  uint8_t  buf[k_test_cdc_recv_cap] = {};
  uint16_t len                      = (uint16_t)k_test_cdc_recv_cap;
  TEST_ASSERT_EQ(k_ra_err_no_data, ra_usb_cdc_recv(buf, &len));
  /* The no-data leg zeroes the reported length. */
  TEST_ASSERT_EQ(0U, len);
  TEST_END("ra_usb_cdc_recv forwards to ra_usb_queue_out (no-data leg)");
}
...
int32_t main(void)
{
  ...
  (void)fprintf(stderr, "[OK ] test_ra_usb_cdc_cov.c\n");
  return 0;
}
