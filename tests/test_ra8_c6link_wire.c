 ...
 * @note Written only through ::ra8_c6link_priv_arena_alloc.
 ...
 */
static ra8_err_t t_no_transfer(void* ctx, const uint8_t* tx, uint8_t* rx, uint16_t len)
{
  (void)ctx;
  (void)tx;
  (void)len;
 ...
 * @return Always false.
 */
static bool t_no_handshake(void* ctx)
{
  (void)ctx;
  return false;
}
 ...
 * @return Nothing.
 */
static void t_no_delay(void* ctx, uint16_t ms)
{
  (void)ctx;
  (void)ms;
}

/** @brief Open the fixture link with an inert transport and a live arena. */
static void t_open(void)
{
  s_link                         = (ra8_c6link_t){};
  ra8_c6link_cfg_t cfg           = {};
  cfg.transport.transfer         = t_no_transfer;
  cfg.transport.handshake_active = t_no_handshake;
  cfg.transport.delay_ms         = t_no_delay;
  cfg.arena                      = s_arena;
 ...
 * Decisions: libs/ra8_c6link/src/ra8_c6link_arena.c@ra8_c6link_priv_arena_alloc
 */
static void test_arena_guards(void)
{
  TEST_BEGIN("c6link arena guards");
  t_open();
  TEST_ASSERT_NOT_NULL(ra8_c6link_priv_arena_alloc(&s_link, (size_t)k_t_alloc_small));
  TEST_ASSERT_NULL(ra8_c6link_priv_arena_alloc(nullptr, (size_t)k_t_alloc_small));

  ra8_c6link_t bare = {};
  TEST_ASSERT_NULL(ra8_c6link_priv_arena_alloc(&bare, (size_t)k_t_alloc_small));
 ...
 * Decisions: libs/ra8_c6link/src/ra8_c6link_arena.c@ra8_c6link_priv_arena_bind
 */
static void test_arena_bump(void)
{
  TEST_BEGIN("c6link arena bump and exhaustion");
  t_open();
  uint8_t* first = (uint8_t*)ra8_c6link_priv_arena_alloc(&s_link, (size_t)k_t_alloc_small);
  TEST_ASSERT_NOT_NULL(first);
  uint8_t* second = (uint8_t*)ra8_c6link_priv_arena_alloc(&s_link, (size_t)k_t_alloc_small);
  TEST_ASSERT_NOT_NULL(second);
  /* Five bytes must round up to eight, so the blocks are eight apart. */
  TEST_ASSERT_EQ(k_t_alloc_pad, (second - first));

  TEST_ASSERT_NULL(ra8_c6link_priv_arena_alloc(&s_link, (size_t)k_t_arena_bytes));
  ra8_c6link_priv_arena_reset(&s_link);
  TEST_ASSERT_NOT_NULL(ra8_c6link_priv_arena_alloc(&s_link, (size_t)k_t_arena_bytes));
  ...
  TEST_ASSERT_NULL(ra8_c6link_priv_arena_alloc(&odd, sizeof s_odd_arena));
  TEST_ASSERT_NOT_NULL(ra8_c6link_priv_arena_alloc(&odd, sizeof s_odd_arena - k_t_odd_slack));

  /* Every helper tolerates a null link rather than faulting on it. */
  ra8_c6link_priv_arena_reset(nullptr);
  ProtobufCAllocator alloc = {};
  ra8_c6link_priv_arena_bind(nullptr, &s_link);
  ra8_c6link_priv_arena_bind(&alloc, nullptr);
  TEST_ASSERT_NULL(alloc.alloc);
  ra8_c6link_priv_arena_bind(&alloc, &s_link);
 ...
 * Decisions: libs/ra8_c6link/src/ra8_c6link_arena.c@ra8_c6link_priv_arena_free
 */
static void test_arena_free_rollback(void)
{
  ...
  /* Freeing the newest block returns its space, so the next allocation reuses
     the same address. */
  ra8_c6link_priv_arena_free(&s_link, second);
  uint8_t* again = (uint8_t*)ra8_c6link_priv_arena_alloc(&s_link, (size_t)k_t_alloc_small);
  TEST_ASSERT_EQ((intptr_t)second, (intptr_t)again);

  /* Every other spelling is a no-op that must not corrupt the offset. */
  ra8_c6link_priv_arena_free(nullptr, again);
  ra8_c6link_priv_arena_free(&s_link, nullptr);
  ra8_c6link_priv_arena_free(&s_link, again);
  ra8_c6link_priv_arena_free(&s_link, again);
  uint8_t* third = (uint8_t*)ra8_c6link_priv_arena_alloc(&s_link, (size_t)k_t_alloc_small);
  TEST_ASSERT_EQ((intptr_t)again, (intptr_t)third);

  /* Freeing a block that is not the newest one is retained until a reset. */
  ra8_c6link_priv_arena_free(&s_link, first);
  uint8_t* fourth = (uint8_t*)ra8_c6link_priv_arena_alloc(&s_link, (size_t)k_t_alloc_small);
 ...
 * and is compared field by field)
 */
static void test_frame_filler(void)
{
  TEST_BEGIN("c6link filler frame");
  (void)memset(s_frame, (int32_t)k_t_poison, sizeof s_frame);
  ra8_c6link_priv_frame_filler(s_frame);
  ...
  TEST_ASSERT_EQ(k_ra8_c6link_frame_idle, ra8_c6link_priv_frame_classify(s_frame, &view));
 ...
 * N+1 = 3 vectors for N=2: minimal MC/DC.
 * Decisions: libs/ra8_c6link/src/ra8_c6link_frame.c@ra8_c6link_priv_frame_classify
 * Decisions: libs/ra8_c6link/src/ra8_c6link_frame.c@ra8_c6link_priv_frame_seal
 */
static void test_frame_classify(void)
{
  ...
  ra8_c6link_priv_frame_seal(s_frame, (uint8_t)ESP_SERIAL_IF, 0U, (uint16_t)k_t_payload_len);
  TEST_ASSERT_EQ(k_ra8_c6link_frame_data, ra8_c6link_priv_frame_classify(s_frame, &view));
  ...
  TEST_ASSERT_EQ(k_ra8_c6link_frame_malformed, ra8_c6link_priv_frame_classify(s_frame, &view));
  ...
  TEST_ASSERT_EQ(k_ra8_c6link_frame_bad_checksum, ra8_c6link_priv_frame_classify(s_frame, &view));
  ...
  (void)ra8_c6link_priv_frame_classify(s_frame, &view);
  TEST_ASSERT_EQ(
    stated,
    ((uint16_t)s_frame[k_t_hdr_csum_lo] | ((uint16_t)s_frame[k_t_hdr_csum_lo + 1U] << 8U)));

  TEST_ASSERT_EQ(k_ra8_c6link_frame_malformed, ra8_c6link_priv_frame_classify(nullptr, &view));
 ...
 * Decisions: libs/ra8_c6link/src/ra8_c6link_tlv.c@ra8_c6link_priv_tlv_open
 */
static void test_tlv_roundtrip(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_c6link_priv_tlv_open(nullptr, 0U, (uint16_t)k_t_proto_len, &body_at));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 ra8_c6link_priv_tlv_open(&buf, 0U, (uint16_t)k_t_proto_len, nullptr));

  uint8_t env[(size_t)k_ra8_c6link_max_payload] = {};
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 ra8_c6link_priv_tlv_open(env, 1U, (uint16_t)k_t_proto_len, &body_at));
  TEST_ASSERT_EQ(0, body_at);

  TEST_ASSERT_EQ(
    k_ra8_ok,
    ra8_c6link_priv_tlv_open(env, (uint16_t)sizeof env, (uint16_t)k_t_proto_len, &body_at));
  ...
  uint16_t       got = 0U;
  const uint8_t* body =
    ra8_c6link_priv_tlv_body(env, (uint16_t)(body_at + (uint16_t)k_t_proto_len), &got);
 ...
 * Vector 1 paired with each of 2, 3 and 4 proves the corresponding condition
 * independently decides. N+1 = 4 vectors for N=3: minimal MC/DC.
 * Decisions: libs/ra8_c6link/src/ra8_c6link_tlv.c@ra8_c6link_priv_tlv_body
 * Decisions: libs/ra8_c6link/src/ra8_c6link_tlv.c@ra8_c6link_tlv_named
 */
static void test_tlv_rejects(void)
{
  ...
  TEST_ASSERT_NULL(ra8_c6link_priv_tlv_body(nullptr, full, &got));
  TEST_ASSERT_NULL(ra8_c6link_priv_tlv_body(env, full, nullptr));
  TEST_ASSERT_NULL(
    ra8_c6link_priv_tlv_body(env, (uint16_t)((uint16_t)k_ra8_c6link_tlv_overhead - 1U), &got));

  env[k_ra8_c6link_tlv_type]++;
  TEST_ASSERT_NULL(ra8_c6link_priv_tlv_body(env, full, &got));
  env[k_ra8_c6link_tlv_type]--;
  ...
  /* A body that claims more octets than the frame carries, and one that
     claims none at all. */
  TEST_ASSERT_NULL(ra8_c6link_priv_tlv_body(env, (uint16_t)(full - 1U), &got));
  ...
  TEST_ASSERT_NOT_NULL(ra8_c6link_priv_tlv_body(env, full, &got));
 ...
 * Vector 1 paired with each of 2, 3 and 4 proves the corresponding condition
 * independently decides. N+1 = 4 vectors for N=3: minimal MC/DC.
 * Decisions: libs/ra8_c6link/src/ra8_c6link.c@ra8_c6link_priv_copy_mac
 * Decisions: libs/ra8_c6link/src/ra8_c6link.c@ra8_c6link_priv_copy_str
 */
static void test_copy_helpers(void)
{
  ...
  TEST_ASSERT(ra8_c6link_priv_copy_mac(&mac, &field));
  TEST_ASSERT_EQ(0, memcmp(mac.octet, raw, sizeof raw));
  TEST_ASSERT(!ra8_c6link_priv_copy_mac(&mac, nullptr));
  TEST_ASSERT(!ra8_c6link_priv_copy_mac(nullptr, &field));
  ...
  TEST_ASSERT_EQ(sizeof text - 1, ra8_c6link_priv_copy_str(text, (uint8_t)sizeof text, &wide));
  TEST_ASSERT_EQ(0, text[sizeof text - 1U]);
  TEST_ASSERT_EQ(0, ra8_c6link_priv_copy_str(text, (uint8_t)sizeof text, nullptr));
  TEST_ASSERT_EQ(0, ra8_c6link_priv_copy_str(nullptr, (uint8_t)sizeof text, &wide));
  TEST_ASSERT_EQ(0, ra8_c6link_priv_copy_str(text, 0U, &wide));
 ...
 * produced them)
 */
static void test_wire_literals(void)
{
  ...
  ra8_c6link_priv_frame_seal(s_frame, (uint8_t)ESP_SERIAL_IF, 1U, (uint16_t)k_t_payload_len);
 ...
 * ::ra8_c6link_priv_frame_classify)
 ...
 * Decisions: libs/ra8_c6link/src/ra8_c6link.c@ra8_c6link_priv_copy_str
 */
static void test_mcdc_wire_guards(void)
{
  ...
  ra8_c6link_priv_frame_seal(nullptr, (uint8_t)ESP_SERIAL_IF, 0U, (uint16_t)k_t_payload_len);
  (void)memset(s_frame, 0, sizeof s_frame);
  ra8_c6link_priv_frame_seal(s_frame,
                             (uint8_t)ESP_SERIAL_IF,
                             0U,
                             (uint16_t)((uint16_t)k_ra8_c6link_max_payload + 1U));
  TEST_ASSERT_EQ(0, s_frame[0]);
  ra8_c6link_priv_frame_filler(nullptr);

  /* frame_classify: neither guard, then each on its own. */
  ra8_c6link_priv_frame_seal(s_frame, (uint8_t)ESP_SERIAL_IF, 0U, (uint16_t)k_t_payload_len);
  TEST_ASSERT_EQ(k_ra8_c6link_frame_data, ra8_c6link_priv_frame_classify(s_frame, &view));
  TEST_ASSERT_EQ(k_ra8_c6link_frame_malformed, ra8_c6link_priv_frame_classify(nullptr, &view));
  TEST_ASSERT_EQ(k_ra8_c6link_frame_malformed, ra8_c6link_priv_frame_classify(s_frame, nullptr));
  ...
  char                text[8] = {};
  TEST_ASSERT_EQ(0, ra8_c6link_priv_copy_str(text, (uint8_t)sizeof text, &hollow));
 ...
 */
static void test_mcdc_caps_guard(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_c6link_caps_bytes,
                 ra8_c6link_priv_caps(s_frame, (uint8_t)k_ra8_c6link_caps_bytes));
  TEST_ASSERT_EQ(ESP_PRIV_EVENT_INIT, s_frame[0]);
  TEST_ASSERT_EQ(0, ra8_c6link_priv_caps(nullptr, (uint8_t)k_ra8_c6link_caps_bytes));
  TEST_ASSERT_EQ(0,
                 ra8_c6link_priv_caps(s_frame, (uint8_t)((uint8_t)k_ra8_c6link_caps_bytes - 1U)));
...
int32_t main(void)
{
  ...
  test_wire_literals();
  test_copy_helpers();
  test_mcdc_wire_guards();
  test_mcdc_caps_guard();
  (void)fprintf(stderr, "[OK  ] test_ra8_c6link_wire.c\n");
  return 0;
}
