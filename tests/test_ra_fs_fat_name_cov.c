/**
 * @file test_ra_fs_fat_name_cov.c
 * @brief Coverage booster for libs/ra_fs/src/ra_fs_fat_name.c.
 *
 * @details
 * Dedicated companion test executable that drives the branches in
 * ra_fs_fat_name.c that no sibling test yet exercises:
 *
 ...
 *
 * The internal header ra_fs_fat_internal.h is included to access dir_walk_t
 ...
 * @retval k_ra_err_out_of_range LBA + count exceeds block_count.
 ...
 * @since 0.1.0
 */
static ra_err_t ncov_mem_read(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf)
{
 ...
 * @since 0.1.0
 */
static ra_err_t ncov_mem_write(void* ctx, uint32_t lba, uint32_t count, const uint8_t* buf)
{
  ncov_mem_disk_t* d = (ncov_mem_disk_t*)ctx;
  if (lba + count > d->block_count) {
    return k_ra_err_out_of_range;
 ...
 * @retval k_ra_ok Capacity fields populated.
 ...
 * @since 0.1.0
 */
static ra_err_t ncov_mem_capacity(void* ctx, uint32_t* block_count, uint32_t* block_size)
{
  ncov_mem_disk_t* d = (ncov_mem_disk_t*)ctx;
...
static const ra_fs_backend_t s_ncov_backend = {
 ...
 *          reaches zero the next read returns k_ra_err_hw_error. Writes
 ...
 * @param[out] buf   Destination buffer.
 * @return Error code.
 * @retval k_ra_ok           Sector read succeeded.
 * @retval k_ra_err_hw_error reads_left reached zero.
 ...
 * @since 0.1.0
 */
static ra_err_t ncov_inj_read(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf)
{
 ...
 * @since 0.1.0
 */
static ra_err_t ncov_inj_write(void* ctx, uint32_t lba, uint32_t count, const uint8_t* buf)
{
  ncov_inject_disk_t* d = (ncov_inject_disk_t*)ctx;
  if (lba + count > d->block_count) {
    return k_ra_err_out_of_range;
 ...
 * @since 0.1.0
 */
static ra_err_t ncov_inj_capacity(void* ctx, uint32_t* block_count, uint32_t* block_size)
{
  ncov_inject_disk_t* d = (ncov_inject_disk_t*)ctx;
  *block_count          = d->block_count;
  *block_size           = (uint32_t)k_ncov_blk_sz;
...
static void ncov_build_fat16_vol(void)
{
  ...
  ncov_put16(bpb, 11U, (uint16_t)k_ncov_blk_sz);
  bpb[13] = 1U;
  ncov_put16(bpb, 14U, 1U);
  bpb[16] = 2U;
  ncov_put16(bpb, 17U, 16U);
  ncov_put16(bpb, 19U, (uint16_t)k_ncov_fat16_spc1);
  ncov_put16(bpb, 22U, 32U);
  bpb[510] = 0x55U;
  bpb[511] = 0xAAU;
}
 ...
 * @since 0.1.0
 */
static void ncov_swap_to_inject(ra_fs_mount_t* h, uint32_t reads_left)
{
 ...
 * @since 0.1.0
 */
static void ncov_create_files_in_sub(ra_fs_mount_t* h, uint32_t count)
{
 ...
 *          The condition at line 244 is true, setting eod=1 and returning k_ra_ok.
 *
 * @par MC/DC:
 ...
 *
 * @pre None.
 * @post eod == 1 and return value is k_ra_ok.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
static void test_ncov_walk_fixed_root_eod(void)
{
  ...
  w.cur_lba         = 100;
  eod               = 0;
  TEST_ASSERT_EQ(k_ra_ok, priv_dir_walk_next_sector(&m, &w, &eod));
 ...
 *          SUB's cluster is EOC (set by ra_fs_mkdir), so eod is set to 1
 *          (lines 262-263) and priv_dir_find returns k_ra_err_not_found (line 321).
 *
 * @par MC/DC:
 ...
 * V1: err=k_ra_ok -> false -> continue while (this test).
 * V2: err!=k_ra_ok -> true -> return (FAT-failure test).
 *
...
static void test_ncov_walk_cluster_chain_eoc(void)
{
  ...
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_ncov_backend, &h));
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mkdir(h, "/SUB"));
  /* 14 files + . + .. = 16 entries, filling the single-sector cluster */
  ncov_create_files_in_sub(h, 14U);
  ra_fs_file_t* f = nullptr;
  /* 8.3 and LFN searches both exhaust SUB, hitting lines 262-263 then 321 */
  TEST_ASSERT_EQ(k_ra_err_not_found, ra_fs_open(h, "/SUB/NOTHERE.TXT", k_ra_fs_mode_read, &f));
 ...
 *          priv_fat_get returns k_ra_err_hw_error (line 259); priv_dir_find
 *          propagates it at line 318.
 *
 * @par MC/DC:
 * Decision: `if (err != k_ra_ok)` at line 258 (1 condition).
 * V1: err=hw_error != k_ra_ok -> true -> line 259 returned (this test).
 * V2: err=k_ra_ok             -> false -> follow chain (cycle-guard test).
 *
 * Decision: `if (err != k_ra_ok)` at line 317 (1 condition).
 * V1: err!=k_ra_ok -> true -> line 318 (this test).
 * V2: err=k_ra_ok  -> false (EOC test above).
 *
 * @pre None (volume built internally).
...
static void test_ncov_walk_fat_read_fail(void)
{
  ...
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_open(h, "/SUB/NOTHERE.TXT", k_ra_fs_mode_read, &f));
  h->backend = saved;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  ncov_free_vol();
 ...
 * @brief priv_dir_walk_next_sector returns k_ra_err_protocol_error when
 ...
 *          and finds 1 > 0 (count_of_clusters) -> returns k_ra_err_protocol_error
 *          at line 269.
 *
 * @par MC/DC:
 ...
 *
 * @pre None (volume built internally).
 * @post priv_dir_walk_next_sector returns k_ra_err_protocol_error.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
static void test_ncov_walk_cycle_guard(void)
{
  ...
  s_ncov_disk.bytes[512U + 4U] = 0x03U;
  s_ncov_disk.bytes[512U + 5U] = 0x00U;
  ra_fs_mount_t* h             = nullptr;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_mount(&s_ncov_backend, &h));
  ...
  w.cluster_hops       = 0;
  uint8_t eod          = 0;
  TEST_ASSERT_EQ(k_ra_err_protocol_error, priv_dir_walk_next_sector(h, &w, &eod));
 ...
 *          inject backend configured for 0 reads. The first ra_fs_open triggers
 ...
 * V1: err=hw_error != k_ra_ok -> true -> line 296 returned (this test).
 * V2: err=k_ra_ok             -> false -> scan entries (normal path).
 *
 * @pre None (volume built internally).
 * @post ra_fs_open returns k_ra_err_hw_error.
 *
...
static void test_ncov_dir_find_initial_read_fail(void)
{
  ...
  ra_fs_file_t* f = nullptr;
  TEST_ASSERT_EQ(k_ra_err_hw_error, ra_fs_open(h, "/ANYFILE.TXT", k_ra_fs_mode_read, &f));
  h->backend = saved;
  TEST_ASSERT_EQ(k_ra_ok, ra_fs_unmount(h));
  ncov_free_vol();
 ...
 *
 * @details Runs all coverage-booster tests for ra_fs_fat_name.c in sequence.
 ...
 *
 * @pre None.
 * @post All targeted lines in ra_fs_fat_name.c have been executed.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
