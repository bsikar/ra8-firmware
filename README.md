...
| Per-app boot files (vector table + SystemInit + linker script) | `examples/<app>/{vector_table,system_init,secure_exception,trustzone_init}.c` + `examples/<app>/linker_script.ld` | each app |
| Secure-app code | `src/secure_app/` | 2 files |
| Example apps (each standalone) | `examples/<app>/main.c` + per-app boot + linker | `examples/blink/`, `examples/blink_hal/` |
...
```sh
make blink             # cross-compile examples/blink/main.c
...
Each app is standalone; `cd examples/blink && make` produces the same
artifacts as `make blink` from the repo root.
...
make -C examples/<app> ozone    open SEGGER Ozone GUI debugger on that app's elf
make -C examples/<app> debug    attach gdb via JLinkGDBServer on that app's elf
...
After `make blink`, artifacts land in `examples/blink/build/`:

```
examples/blink/build/blink.elf    ELF with DWARF symbols (debugger target)
examples/blink/build/blink.hex    Intel HEX (for flashers)
examples/blink/build/blink.bin    Raw binary (zero-padded; rarely needed)
examples/blink/build/blink.map    Linker map
```

`blink_hal` produces the equivalent files under `examples/blink_hal/build/`.
...
```sh
make -C examples/blink flash             # uses scripts/flash.sh examples/blink/build/blink.hex
make -C examples/blink_hal flash         # ditto for blink_hal
```
...
```sh
make -C examples/blink ozone     # SEGGER Ozone GUI -- best for HardFault triage
make -C examples/blink debug     # gdb attached via JLinkGDBServer
```
...
ELF path it loads is `examples/blink/build/blink.elf`; the per-app `make
