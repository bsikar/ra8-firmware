...
| 6 RESET | MCU_RESET_N; retain R1 pull-up and U7.3 hierarchical load | C6_EN/U3.3; retain R11 pull-up to +3V3_RADIO |
...
and R1's protected-output loading reviews. As an initial common screen,
12 uA adverse output-node leakage with a 9801 ohm minimum 10k pull-up gives
<=0.358190439 mA at maximum rail, below TI's 0.4 mA test current. Its
VOL<=0.25 V at VDD>=1.5 V and 250 nA high-impedance leakage test-point
limit do not qualify all partial-power states or the total 12 uA allocation.
MCU/U7/fixture loading and radio EN thresholds/capacitance still need their
complete receiver-specific checks; the module's 3.3 V/25 C table is not a
new full-temperature guarantee. Do not infer a guaranteed low-POR reset
level from the 15 uA POR test with an arbitrarily stronger pull-up.
...
reset_sink = rail_max/rmin+12e-6
...
assert mr_sink < .0004 and reset_sink < .0004
