...
zero-allocation, bounded, MC/DC-testable text-flow engine (`libs/ra_reflow`) fed
by a zero-heap EPUB container parser (`libs/ra_epub`).

---

## 1. Container / package surface (accepted)

Parsed by `libs/ra_epub` (miniz for ZIP, tinyxml2 for XML), zero-heap:
...
The `ra_reflow` v1 tokenizer recognizes the tags below
(`libs/ra_reflow/src/ra_reflow_tokenize.c`). This list is the **ratified element
contract**:
...
1. The intrinsic block/inline/heading layout `ra_reflow` applies by tag.
...
`RA_REFLOW_USE_LITEHTML` (default **OFF**). litehtml brings the full CSS box
...
| Embedded fonts (`@font-face`) | **Ignore.** Text renders in the device font (`libs/ra_sdfont` / stb_truetype). |

---

## 5. Image formats

- **Cover image:** the raw bytes are extracted by `ra_epub_get_cover_image()`
...
|-----------------|-----------------|
| Container / package | `libs/ra_epub/` |
| Element subset + flow-through | `libs/ra_reflow/src/ra_reflow_tokenize.c` (tag table) |
| Block/heading/inline layout | `libs/ra_reflow/src/ra_reflow_layout.c` |
| CSS off by default | `RA_REFLOW_USE_LITEHTML` (CMake option, default OFF) |
