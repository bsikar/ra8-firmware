...
| REQ-DRV-025      | ra8_eth_gptp      | Generic PTP timer (HUM Ch 35): increment, 78-bit offset, time and AVTP readout.                                              | `src/ra8_eth_gptp.c`              | `test_ra8_eth_gptp.c`                     |
...
| REQ-HAL-014      | An EPUB content reader SHALL parse OPF + spine and return chapter text.                                                                       | `libs/ra8_epub/src/ra8_epub_open.c`, `ra8_epub_chapter.c`, `ra8_epub_xml_shim.c` | `tests/test_ra8_epub.c`, `test_ra8_epub_open.c`, `test_ra8_epub_chapter.c` |
...
| REQ-PORT-010     | The secure key vault SHALL hold all 256-bit symmetric keys in a static array unreachable from NS after the SAU partition is enabled.            | `libs/ra8_secure_app/src/key_vault.c`                    | `tests/test_ra8_key_vault.c`                   |
| REQ-PORT-011     | A key-import secure-app SHALL accept wrapped key blobs and install them into the key vault under a documented enum-typed key-class.              | `libs/ra8_secure_app/src/key_import.c`                   | `tests/test_secure_app_key_import.c`          |
| REQ-PORT-012     | An OTA-commit secure-app SHALL verify the staged image hash and atomically swap the active MRAM bank.                                              | `libs/ra8_secure_app/src/ota_commit.c`                   | `tests/test_secure_app_ota_commit.c`          |
| REQ-PORT-013     | A secure TRNG path SHALL provide entropy bytes to PSA-Crypto on the secure side.                                                                    | `libs/ra8_secure_app/src/secure_trng.c`                  | `tests/test_secure_app_secure_trng.c`         |
...
| REQ-SAFE-017     | First-party branch + statement coverage SHALL reach 90/90 (IEC 61508 Annex C minimum).                | `bash scripts/ci.sh --gate coverage-tree`                                                                               | CI job `coverage.yml::coverage-tree`                  |
...
| REQ-PERF-007     | OTA commit SHALL complete in <= 2 s for a 256 KiB image excluding network transfer.                    | `libs/ra8_ota/src/ra8_ota.c`, `libs/ra8_secure_app/src/ota_commit.c`                                                          | `tests/test_ra8_ota.c`, `tests/test_secure_app_ota_commit.c` |
