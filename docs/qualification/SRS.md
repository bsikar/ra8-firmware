...
| REQ-CHIP-004     | Each app SHALL bring up the SAU per `trustzone_init.c` before transitioning to NS code (where applicable).| `examples/ek_ra8d2/blink/trustzone_init.c`                                                      | `tests/test_ra8_sim_world.c`                          |
...
| REQ-CORE-005     | A monotonic millisecond time source SHALL exist via `ra8_time_now_ms()` driven by SysTick.                                              | `libs/ra8_core/inc/ra8_time.h`, `libs/ra8_core/src/ra8_time.c`      | `tests/test_ra8_time.c`, `tests/test_ra8_sim_time.c` |
...
| REQ-DRV-010      | ra8_ceu           | Camera Encoding Unit init + frame capture path setup.                                                                       | `src/ra8_ceu.c`                   | `test_ra8_ceu.c`                          |
...
| REQ-DRV-046      | ra8_mipi_csi      | MIPI CSI-2 receiver (camera path) init.                                                                                          | `src/ra8_mipi_csi.c`              | `test_ra8_mipi_csi.c`                     |
| REQ-DRV-047      | ra8_mipi_dsi      | MIPI DSI display-side init + commands.                                                                                          | `src/ra8_mipi_dsi.c`              | `test_ra8_mipi_dsi.c`                     |
| REQ-DRV-048      | ra8_mipi_phy      | Shared MIPI D-PHY analog setup.                                                                                                  | `src/ra8_mipi_phy.c`              | `test_ra8_mipi_phy.c`                     |
...
| REQ-DRV-060      | ra8_rsip          | Renesas Secure IP API surface (BLOCKED-VENDOR for production-grade key wrap).                                                      | `src/ra8_rsip.c`                  | `test_ra8_rsip.c`, `test_ra8_rsip_edge_cases.c` (software emulator only) |
...
| REQ-DRV-089      | ra8_vin           | Video Input (parallel-camera) controller init.                                                                                                    | `src/ra8_vin.c`                   | `test_ra8_vin.c`                          |
...
| REQ-HAL-010      | A PSA-Crypto integration SHALL expose the canonical PSA APIs through the project's logging/error pipeline.                                | `libs/ra8_psa_crypto/src/ra8_psa_crypto.c`        | `tests/test_ra8_psa_crypto.c`                  |
...
| REQ-HAL-012      | A BLE host stack SHALL provide ATT, GATT (server + client), L2CAP, security and mesh surfaces.                                              | `libs/ra8_ble_host/src/ra8_ble_att.c`, `ra8_ble_gatt.c`, `ra8_ble_gatt_client.c`, `ra8_ble_l2cap.c`, `ra8_ble_security.c`, `ra8_ble_mesh.c` | `tests/test_ra8_ble_*.c` (HW-blocked for end-to-end: ESP32-C6 companion) |
...
| REQ-EXT-009      | OV5640 5 MP camera                                                    | `libs/ra8_hal/src/ra8_ceu.c`, `ra8_mipi_csi.c`, `ra8_vin.c`                   | `tests/test_ra8_ceu.c`, `tests/test_ra8_mipi_csi.c`, `tests/test_ra8_vin.c` |
