...
- The first-party C/C++ code under [`../../libs/`](../../libs/),
  [`../../apps/`](../../apps/), and [`../../port/`](../../port/).
- The board application binaries under
...
- The Cortex-M33 secondary core, exercised by M33/CPU1 and dual-core example
  images.
...
Ring 0 is silicon register definitions plus the board boot files (or an
explicit app-local override): same-named files under the selected app's
`src/` directory and its root `linker_script.ld`.
...
(`libs/ra8_hal/inc/ra8_*_regs.h`) and Ring 3 is the driver
...
Application requirements are derived from `scripts/dev/ra8_apps.py` rather
than a hand-maintained per-app table. The retained 118/118 RA8D2 build and
selected-app HIL result are historical; the current matrix build and target
execution are pending. App-local tests and `tests/mocks/` contribute to the
host suite, whose shrink-only registration floor is enforced by
`tests/run_tests.sh`, but bidirectional
one-app/one-test trace remains pending.
...
requirements below when D-006..D-012 were registered, so the
...
| REQ-SAFE-017     | First-party coverage SHALL be judged one translation unit at a time against `.github/tree-coverage-baseline.txt`: a unit with no row SHALL enter at >= 90% line / 80% branch; a unit carrying a frozen row is shrink-only, so its uncovered line and branch debt SHALL NOT grow and its ratio SHALL NOT fall, and it burns down toward that entry floor; a unit no host build executes SHALL carry an explicit `UNMEASURED` row naming the reason class re-derived from the tree, which is a declared absence of measurement and never a pass. The IEC 61508 Annex C 90% statement and 90% branch target is the destination this ratchet walks toward, not a met claim: the open residue is every `UNMEASURED` row plus the compiled-but-unexecuted units in #727. | `scripts/checks/check_tree_coverage.py` (the executable form of this row: it re-reads these floors and fails when they and its own differ, in either direction), `.github/tree-coverage-baseline.txt` (one row per unit), `just quality::gate::run coverage-tree` | CI job `coverage.yml::coverage-tree`; the gate selftest ties this row to the enforced floors on every run |
...
flagged `TBD-MEASURE`. The closure path is the guarded HIL workflow in
...
The former hand-counted matrix described the retired flat test tree and is no
longer qualification evidence. The shrink-only registration floor is derived
and enforced by `tests/run_tests.sh`. The retained 2026-08-22 snapshot counted
693 test sources and 689 registrations on each clean standalone host and passed
689/689 in the Linux/devcontainer gate in 8.66 s; macOS execution was not
claimed because low-address tests require
Linux/container execution. A refreshed bidirectional requirement/test count is
pending and must be generated rather than transcribed into this document.
...
- Application-level trace is derived from the live
  `scripts/dev/ra8_apps.py` inventory; a refreshed one-app/one-test matrix is
  pending.
- **REQ-SAFE-001/002/004/006/008** -- enforced by lint, not by host
  test.
- **REQ-PERF-001/002/004/005** -- require HW-in-the-loop measurement;
  closure path is the guarded Raspberry Pi 5 rig workflow in
  `docs/HIL_DEVELOPER_WORKFLOW.md`.
...
(verification objective tables). The 118/118 RA8D2 build, selected-app HIL,
and remote-GDB lifecycle results are retained historical evidence;
current-candidate hardware execution is pending.
