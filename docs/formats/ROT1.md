...
**Producer:** `scripts/secrets/rot_sign.py` &nbsp;|&nbsp;
...
### 4.1 Signing (`scripts/secrets/rot_sign.py`)
...
```
$ python3 scripts/secrets/rot_sign.py keygen --key /tmp/demo.pem
$ python3 -c "open('/tmp/body.bin','wb').write(bytes((i*7+3)&0xFF for i in range(1024)))"
$ python3 scripts/secrets/rot_sign.py sign --key /tmp/demo.pem --image /tmp/body.bin \
...
```
$ python3 scripts/secrets/rot_sign.py selftest
...
- `scripts/secrets/rot_sign.py` -- the signing tool and its `selftest`
