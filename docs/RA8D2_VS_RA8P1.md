...
> [`libs/ra_core/inc/ra_device.h`](../libs/ra_core/inc/ra_device.h); this doc
...
|--------|--------|---------|
| `RA_DEVICE_RA8D2` | default (no flag) | RA8D2. Host unit-test + every current build. |
| `RA_DEVICE_RA8P1` | `cmake/toolchain-ra8p1.cmake` | RA8P1 (adds the NPU + DP-FPU). |

`ra_device.h` enforces "exactly one" (`#error` on both, defaults to RA8D2 on
neither) and derives:

- **Feature-presence macros** for `#if` guards: `RA_HAS_NPU`, `RA_HAS_NPUCLK`
  (RA8P1 only), `RA_HAS_OFS3` (RA8D2 only).
- **A typed-enum runtime mirror** `k_ra_feat_*` (0/1) and `k_ra_device_t`
  (`k_ra_device_ra8d2 = 0x8D2`, `k_ra_device_ra8p1 = 0x8F1`) for code that needs
  the selection at run time rather than in the preprocessor.

Register headers that carry a base-address delta guard it with
`#if defined(RA_DEVICE_RA8P1)`; because the bases are byte-identical today, only
the NPU window is actually device-specific. Prefer the named `RA_HAS_*` flag
over a raw `RA_DEVICE_RA8P1` check in feature code so the intent ("this chip has
...
|------|-------|-------|------|---------------|
| Ethos-U55 NPU | absent | present @ `0x40140000` | `RA_HAS_NPU` | ra_npu driver + `ra_npu_regs.h`; sim-modelled (#222). **Done.** |
| NPUCLK domain | absent | present | `RA_HAS_NPUCLK` | CGC NPUCLK; on-silicon wiring #229. |
| OFS3 / WDT1 option reg | present | **absent** | `RA_HAS_OFS3` | `ra_ofs.{h,c}` gates OFS3 out of the option map (#223). **Done.** |
| M85 FPU width | single (`fpv5-sp-d16`) | **double** (`fpv5-d16`) | toolchain | `cmake/toolchain-ra8p1.cmake` appends `-mfpu=fpv5-d16`; witnessed by `ra_fpu_probe` (#225). **Done.** |
...
driver ([`ra_npu.h`](../libs/ra_hal/inc/ra_npu.h)) is `submit -> run -> poll ->
read-output`; it is gated behind `RA_HAS_NPU` so the TU is empty on the RA8D2.
The inference path is layered:

1. **Driver** `ra_npu` -- programs QBASE/QSIZE (command stream) + BASEPn (tensor
   arena bases), kicks the job, waits for `STATUS.cmd_end`. **Done.**
2. **Adapter** `ra_ethosu_shim` -- provides the Arm ethos-u-core-driver C API
   (`ethosu_reserve_driver` / `ethosu_invoke_v3` / `ethosu_release_driver`) on
   top of `ra_npu`, so TFLite-micro's Ethos-U op can drive the NPU (#228).
   **Done** (host-tested).
3. **Runtime** TFLite-micro (vendored SOUP, dormant behind `RA_USE_TFLITE_MICRO`)
...
`ra_npu_sim_cmd.h` SE55 command convention, so the whole driver + adapter path is
...
- **Host (device-agnostic):** `ra_ofs`, `ra_fpu_probe`, `ra_npu`,
  `ra_ethosu_shim` unit tests build with `-DRA_DEVICE_RA8P1` and pass on the
  Linux test host.
...
ADC (pending) - `#226` `ra_board_ra8p1` board layer + bring-up - `#227` Vela
