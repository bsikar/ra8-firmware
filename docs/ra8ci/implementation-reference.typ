...
#table(
  ...
  [`ra8ci github shadow-evidence`], [`CURRENT`], [Grades several pull requests' comparisons and reports, per task, how many commits were observed and graded and which of them agreed, diverged, or conflicted. #1481 holds the gate move until conclusions have been compared over representative pull requests, which `shadow-compare` cannot answer because it grades one commit. A task short of the threshold is reported with the commits nobody judged named, not counted, and with a shortfall saying how many graded commits remain, because those named pull requests are where the missing evidence is. A conflicting task is named with no shortfall on purpose: a remaining count beside a conflict would read as a number of pull requests that clears it, and no number does. `ungraded_commits` names the accumulated commits that graded nothing, every pairing on them indeterminate: they stay in the commit record because they were looked at, and are named apart from it because a caller counting the commits reads the breadth of the evidence as wider than it is.],
  [`ra8ci github evidence-page`], [`CURRENT`], [Writes the same accumulated answer as `shadow-evidence`, rendered for the person deciding whether a required check may move rather than for a pipe: the decision first, then the tasks that disagreed with Actions, then the tasks still short of the threshold with the commits nobody judged named under them, then the tasks that may move. The sections are in that order because that is the order the decision is made in; a page ordered by task name reads as a list of facts. Both commands read, grade and accumulate through one function and return one verdict, so the page and the document cannot answer differently. Directly under the counts the page says how many of the accumulated commits graded nothing and names them, because the first line's commit count is read as the breadth the threshold was met across and a commit nobody judged any pairing on is inside it; the line is absent entirely when every commit graded something, so that the place the warning appears is not one a reader learns to skip. The catalog digest and the mode are deliberately absent from the page: they say which configuration produced an answer, which is something to check a document against rather than something to read while deciding whether a task may gate.],
  ...
  [`ra8ci github reconcile`], [`CURRENT`], [Reports what one commit already carries for a document of task outcomes: per task, the publish decision and every published run with its identifier, which of five standings that identifier holds (`ours`, `absent` for a run carrying none, `foreign` for a value in a shape this plane never writes, `superseded` for our shape at a version this build does not derive, and `other subject` for our current shape whose digest is for another name or another commit, which is what a renamed run and a re-used branch leave behind), whether this plane posted it, and what it said, meaning its title and an excerpt of its output summary with `summary_truncated` when the excerpt is not the whole body. The summary is reported and never matched on: the decision beside it has no intended summary to compare against, so a run whose title agrees while its summary describes other work is settled here and visible only in this report. It builds only the `checks:read` reader, so the read that decides a write cannot perform one. A conflict is reported rather than refused, and is a non-zero exit. It also reports what the document does not plan: every decision is made by asking the listing about one name at a time, so a run left under a task name since retired or renamed is accounted for by nothing, and those runs are listed in `unplanned_runs` with a count, each carrying the same standing. The standing is the part the bare yes or no cannot carry: a leftover of ours under a retired name is a name to clean up, a run of ours derived for another commit was posted against the wrong commit, and a stranger's run under one of our names is an argument about who owns the name, and all three report the same `ours: false`. They move neither `settled` nor the exit status, because a leftover run is a fact about the commit rather than a verdict on the runs in hand.],
...
`evidence-page` is deliberately not in that pipe block. It writes a page for a person to read, not the next command's input, and listing it there would imply a pipe that does not exist. It is a subcommand of its own rather than a flag on `shadow-evidence`, because a command whose output shape depends on a flag is one a pipeline can be pointed at wrongly without failing.
