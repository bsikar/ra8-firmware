...
`pull-request-evidence` is the whole of the first two columns of that work done from pull request numbers: `pull-request` and `evidence-run` answer the same questions one pull request at a time, for an operator deciding which pull requests are representative before any of them is gathered.
