...
|---:|---|---|---|
| **0** | BSP | `src/boot/` | Vector table, SystemInit, linker script. CPU-state setup before C runtime is live. |
...
| **6** | Application | `examples/`, `src/main.c` (legacy), test mocks | The firmware "user code" -- whatever drives the HAL to do something useful. The blink demo, board-bringup smoke tests, and unit-test harnesses all live at Ring 6. |
...
|---|---|---|
| `{World: S}` | Runs in the Secure world. Has full access to all peripherals and memory. Cannot be called directly from NS code -- only via NSC veneers. | `libs/ra_hal/`, `libs/ra_*_pal/` (when serving the secure side), `src/secure_app/`, `src/boot/`, secure-side examples. |
| `{World: NS}` | Runs in the Non-Secure world. Reaches into Secure code only through `__cmse_nonsecure_entry` veneers in `libs/ra_nsc/`. | `libs/ra_hal/` driver TUs that the SAU partition keeps NS, NS-side examples. |
...
`libs/ra_nsc/`, `src/secure_app/`, or `examples/<name>/`:
