...
| `GPT-3`, `GPT-4`, `GPT-4o`, `GPT-3.5`, ... | OpenAI product family. The bare token `GPT` and Renesas channel names `GPT0`..`GPT13`, `GPT320`..`GPT323`, `ra_gpt`, `GPT_BASE` etc. are ALLOWED -- the RA8D2 chip has a GPT (general PWM timer) peripheral, so the dash is what discriminates the AI product from the hardware block. |
...
RA_REVIEWED_BY("Claude")               // AI-OK: forbidden-pattern example
...
ra_gpt_init(k_ra_gpt_ch0, &cfg);
volatile uint32_t* base = (uint32_t*)k_ra_gpt0_base;
