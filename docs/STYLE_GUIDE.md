...
| `@since` | Yes on public declarations | `ra8ci since` | Semantic version the file first appeared at. **Presence** is gated only for `ra8_*` declarations in `libs/ra8_*/inc/`; the **value** of every `@since` anywhere in the tree must equal the top-level `VERSION`. |
