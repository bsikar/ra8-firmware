...
| `@since` | Yes on public declarations | `check-since-version.py` | Semantic version the file first appeared at. **Presence** is gated only for `ra8_*` declarations in `libs/ra8_*/inc/`: that is the versioned contract, and a Zig implementation behind an unchanged C header makes no separate promise. The **value** of every `@since` in any tracked first-party file must equal the top-level `VERSION`, in every language (#900): C, Zig, Rust, Markdown, shell, Python. This row claimed "anywhere in the tree" while the gate read only `.c/.h/.cpp/.hpp`. A deliberately illustrative tag carries an inline `SINCE-EXAMPLE-OK: <reason>` marker on the same line, and the reason is mandatory. |
