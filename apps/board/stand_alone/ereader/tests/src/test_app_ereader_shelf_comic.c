 ...
 *     EXACT baked fixture header -- `comic_open` over a resident-buffer read
 *     seam, `comic_page_read` of page 0, then `ra8_img_decode_blit` into a
...
/** @brief ::comic_read_fn over a resident buffer (the app's blob seam twin). */
...
{
  /** App-twin buffers retained off-stack for this probe. */
  static comic_page_t s_page_index[k_tsc_page_cap];
  static char         s_names[k_tsc_name_cap];
  static uint8_t      s_pagebuf[k_tsc_pagebuf];
  static uint8_t      s_decode_arena[k_tsc_arena];
  comic_t             c   = {};
  ...
  *pages     = comic_page_count(&c);
  size_t got = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, comic_page_read(&c, 0U, s_pagebuf, sizeof s_pagebuf, &got));
  TEST_ASSERT_EQ(
    k_ra8_ok,
  ...
  ra8_img_arena_t arena = {.base   = s_decode_arena,
                           .cap    = sizeof s_decode_arena,
                           .offset = 0U,
                           .live   = 0U};
  TEST_ASSERT_EQ(k_ra8_ok,
 ...
 *          comic + ra8_img_decode_blit path renders both containers alike.
 *
 * @par MC/DC:
...
static int32_t m_edge_dir(int32_t x, int32_t fb_w, bool rtl)
{
  ...
  if (x < third) {
    ...
    /* The centre third is intentionally neutral. */
  }
