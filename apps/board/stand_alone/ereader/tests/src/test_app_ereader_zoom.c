...
static void t_page_sampler_is_pure_and_bounded(void)
{
  ...
  key.tile_x = (uint16_t)k_t_ez_tile_x;
  key.tile_y = (uint16_t)k_t_ez_off_tile;
  TEST_ASSERT_EQ(k_ra8_err_out_of_range,
                 ez_tile_decode(nullptr, &key, s_cell, sizeof(s_cell), &tw, &th));
...
static void t_present_lens_only(ra8_ui_rect_t content)
{
  ...
  TEST_ASSERT(
    ez_scene_tick(&s_scene, (uint32_t)k_t_ez_burst_ms + (uint32_t)k_zoom_settle_ms_default));
  TEST_ASSERT_EQ(k_ra8_ok, ez_scene_present(&s_scene, &plan));
  TEST_ASSERT(plan.present);
  TEST_ASSERT(plan.quality);
}
 ...
 *
 * @par Tick outcomes:
 * - V4: the page's own settle is due            -> true.
 * - V5: loupe OPEN, only the loupe settle is due -> true.
 * - V6: loupe CLOSED, only its settle is due     -> false; hidden lens pixels
 *       must not repaint the page.
 * - A quiet tick with neither timer armed        -> false.
 * The production tick expresses this priority as single-condition branches;
 * these vectors retain the behavioral proof without claiming a stale compound
 * decision.
 *
 * @details The pan test is the app's remaining compound decision and is
 *          enforced by the repository-wide per-file MC/DC floor. The tick
 *          vectors independently prove its page/lens priority behavior.
 */
static void t_scene_decisions_are_covered(void)
{
  ...
  TEST_ASSERT(ez_scene_tick(&s_scene, 3U + (uint32_t)k_zoom_settle_ms_default));
  TEST_ASSERT_EQ(k_ra8_ok, ez_scene_present(&s_scene, &plan));
  TEST_ASSERT(!ez_scene_tick(&s_scene, 3U + (uint32_t)k_zoom_settle_ms_default));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ez_scene_present(&s_scene, &plan));
  TEST_ASSERT(ez_scene_tick(&s_scene, 20U + (uint32_t)k_zoom_settle_ms_default));
  TEST_ASSERT_EQ(k_ra8_ok, ez_scene_present(&s_scene, &plan));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ez_scene_present(&s_scene, &plan));
  s_scene.lens_on = false;
  TEST_ASSERT(!ez_scene_tick(&s_scene, 30U + (uint32_t)k_zoom_settle_ms_default));
  TEST_ASSERT_EQ(k_ra8_ok, ez_scene_present(&s_scene, &plan));
  TEST_ASSERT(!plan.present);
...
static void t_prefetch_is_bounded(void)
{
  ...
  t_ez_open();
  uint16_t warmed = 0U;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ez_scene_prefetch(nullptr, k_zoom_pan_right, &warmed));
  TEST_ASSERT_EQ(k_ra8_ok, ez_scene_prefetch(&s_scene, k_zoom_pan_none, &warmed));
  TEST_ASSERT_EQ(0U, warmed);
  TEST_ASSERT_EQ(k_ra8_ok, ez_scene_prefetch(&s_scene, k_zoom_pan_right, &warmed));
