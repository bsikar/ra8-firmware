 ...
 * transcode the baked 1536x2048 grayscale PNG through `jof_produce`
...
static void setup_reader(void)
{
  ...
  const jof_produce_cfg_t pcfg = {.pull       = png_pull,
                                  .pull_ctx   = &cur,
                                  ...
                                  .webp_work  = NULL};
  TEST_ASSERT_EQ(k_ra8_ok, jof_produce(&pcfg, &s_info));
  TEST_ASSERT_EQ(k_ra8_ok, jof_parse(jof_memstore_pread, &s_store, s_store.len, &s_info));
...
static void t_put_u16(jof_memstore_t* st, uint16_t v)
{
  const uint8_t b[2] = {(uint8_t)(v & 0xFFU), (uint8_t)(v >> 8U)};
  TEST_ASSERT_EQ(k_ra8_ok, jof_memstore_sink(st, b, sizeof(b)));
}

/** @brief Append a little-endian u32 to a memstore (hand atlas builder). */
static void t_put_u32(jof_memstore_t* st, uint32_t v)
{
...
static void t_put_bytes(jof_memstore_t* st, const uint8_t* p, size_t n)
{
  TEST_ASSERT_EQ(k_ra8_ok, jof_memstore_sink(st, p, n));
}
 ...
 * @since 0.1.0
 */
static void build_bpp3_atlas(jof_memstore_t* st)
{
  *st = (jof_memstore_t){.buf = s_store_buf, .cap = sizeof(s_store_buf), .len = 0U};
  ...
  const uint8_t bpp_codec[2] = {(uint8_t)k_t_colour_bpp, (uint8_t)k_jof_codec_raw};
 ...
 *
 * @details
 * `jof_produce()` carves its band, tile stage and compressed-tile
...
static void test_manga_work_arena_covers_advertised_cap(void)
{
  ...
  const uint32_t need = jof_work_bytes((uint16_t)k_t_cap_edge,
  ...
  TEST_ASSERT_EQ(0U, jof_work_bytes((uint16_t)k_t_cap_edge, (uint16_t)k_t_cap_edge, 0U, 0U));
 ...
 * tile cache carry their vectors in test_jof*.c / test_ra8_tile_cache*;
 ...
 * test_jof.c. Here the oracle is the known solid-fill / frame pixels.)
 */
static void test_manga_fixture_pattern(void)
{
  /** Decoded-tile probe buffer retained off-stack for this test. */
...
static void test_manga_status_text(void)
{
  ...
  char tiny[1] = {'x'};
  TEST_ASSERT_EQ(k_ra8_ok, mg_reader_status(&s_reader, tiny, (uint32_t)sizeof(tiny)));
  TEST_ASSERT_EQ('\0', tiny[0]);
  TEST_END("ereader_manga: mg_reader_status formatting");
}

/**
 * @test test_manga_render_clips_partial_edge_tiles
 * @brief Render a deliberately non-tile-aligned viewport at both axes.
 * @details Moves the otherwise valid reader one pixel into the page so the
 ...
 * @post The reader remains at the requested one-pixel viewport anchor.
 * @note Not thread-safe because it uses the file-local reader fixture.
 * @since 0.1.0
 * @par MC/DC:
 * `sy < low || sy >= high` and its `sx` counterpart each receive a false/false
 * interior control, a low-edge-only true vector, and a high-edge-only true
 * vector from the two partially covered boundary tiles.
 */
static void test_manga_render_clips_partial_edge_tiles(void)
{
  TEST_BEGIN("ereader_manga: partial edge tiles are clipped");
  setup_reader();
  s_reader.view_x = 1;
  s_reader.view_y = 1;
  TEST_ASSERT_EQ(k_ra8_ok, mg_reader_render(&s_reader));
  TEST_ASSERT_EQ(1, s_reader.view_x);
  TEST_ASSERT_EQ(1, s_reader.view_y);
  TEST_END("ereader_manga: partial edge tiles are clipped");
}

/**
 * @test test_manga_init_rejects_each_geometry_bound
 * @brief Vary every framebuffer and atlas geometry guard independently.
 * @details Starts with the same valid production fixture as the golden render,
 ...
 * @post Restoring the valid geometry returns ::k_ra8_ok.
 * @note Not thread-safe because it borrows the file-local cache fixture.
 * @since 0.1.0
 * @par MC/DC:
 * The valid configuration is the all-false control. Each remaining vector
 * makes exactly one operand of the two- or four-condition geometry guard true.
 */
static void test_manga_init_rejects_each_geometry_bound(void)
{
  TEST_BEGIN("ereader_manga: each geometry bound is enforced");
  setup_reader();
  mg_reader_t     reader = {};
  jof_info_t      info   = s_info;
  mg_reader_cfg_t cfg    = {.fb       = s_fb,
                            .fb_w     = (int32_t)k_t_fb_w,
                            .fb_h     = (int32_t)k_t_fb_h,
                            .cache    = &s_cache,
                            .info     = &info,
  ...
  TEST_ASSERT_EQ(k_ra8_ok, mg_reader_init(&reader, &cfg));
  cfg.fb_w = 0;
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, mg_reader_init(&reader, &cfg));
  cfg.fb_w = (int32_t)k_t_fb_w;
  cfg.fb_h = (int32_t)k_mg_statusbar_h;
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, mg_reader_init(&reader, &cfg));
  cfg.fb_h = (int32_t)k_t_fb_h;
  ...
  info = s_info;
  TEST_ASSERT_EQ(k_ra8_ok, mg_reader_init(&reader, &cfg));
  TEST_END("ereader_manga: each geometry bound is enforced");
}
...
static void test_manga_bpp_guard(void)
{
  ...
  jof_memstore_t store3 = {};
  build_bpp3_atlas(&store3);
  jof_info_t info3 = {};
  TEST_ASSERT_EQ(k_ra8_ok, jof_parse(jof_memstore_pread, &store3, store3.len, &info3));
...
{
  /** Eviction-stress storage retained off-stack for this test. */
...
int main(void)
{
  ...
  test_manga_render_clips_partial_edge_tiles();
  test_manga_init_rejects_each_geometry_bound();
