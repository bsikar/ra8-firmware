...
static_assert((uint32_t)k_mg_index_off ==
                (uint32_t)k_jof_hdr_bytes + ((uint32_t)k_mg_bands * (uint32_t)k_mg_payload),
              "index_off must trail the header + all band streams");
static_assert((uint32_t)k_mg_index_len == (uint32_t)k_mg_bands * (uint32_t)k_jof_index_entry,
              "index_len must be bands * index-entry size");
static_assert((uint32_t)k_mg_atlas_size ==
                (uint32_t)k_mg_index_off + (uint32_t)k_mg_index_len + (uint32_t)k_jof_footer_bytes,
              "atlas size must be index_off + index + footer");
static_assert((uint32_t)k_mg_img_h <= (uint32_t)k_jof_max_dim,
              "strip height must be within the JOF dimension cap");
static_assert((uint32_t)k_mg_bands <= (uint32_t)k_jof_max_tiles,
...
/** @brief Fixed storage shared by the cache setup and residency probes. */
typedef struct {
  /** Page data. */
  uint8_t pc_frames[(size_t)k_mg_frames * (size_t)k_mg_frame_bytes];
  /** Page metadata. */
  ra8_vmem_frame_t pc_meta[(size_t)k_mg_frames];
  /** Page keys. */
  ra8_vmem_key_t pc_keys[(size_t)k_mg_frames];
  /** Page buckets. */
  int32_t pc_buckets[(size_t)k_mg_pc_buckets];
  /** Tile data. */
  uint8_t tc_cells[(size_t)k_mg_cells * (size_t)k_mg_cell_bytes];
  /** Tile keys. */
  ra8_tile_key_t tc_keys[(size_t)k_mg_cells];
  /** Tile dimensions. */
  ra8_tile_dims_t tc_dims[(size_t)k_mg_cells];
  /** Tile metadata. */
  ra8_keycache_cell_t tc_meta[(size_t)k_mg_cells];
  /** Tile buckets. */
  int32_t tc_buckets[(size_t)k_mg_tc_buckets];
} t_mg_cache_storage_t;

/** @brief Cache storage reached by both setup and residency probes. */
static t_mg_cache_storage_t s_cache_storage;
 ...
 *          parsed ::jof_info_t and derives each atlas's base offset
...
typedef struct {
  ...
  const jof_info_t*  info; /**< Shared parsed atlas geometry. */
} t_mg_decode_ctx_t;
...
{
  static const uint8_t magic[k_jof_magic_len] = {'J', 'O', 'F', '1'};
  if (off < (uint32_t)k_jof_magic_len) {
    return magic[off];
  }
  if (off < (uint32_t)k_jof_ofs_height) {
    return t_mg_le_byte((uint32_t)k_mg_img_w, off - (uint32_t)k_jof_ofs_width);
  }
  if (off < (uint32_t)k_jof_ofs_tile_w) {
    return t_mg_le_byte((uint32_t)k_mg_img_h, off - (uint32_t)k_jof_ofs_height);
  }
  if (off < (uint32_t)k_jof_ofs_tile_h) {
    return t_mg_le_byte((uint32_t)k_mg_tile_w, off - (uint32_t)k_jof_ofs_tile_w);
  }
  if (off < (uint32_t)k_jof_ofs_bpp) {
    return t_mg_le_byte((uint32_t)k_mg_tile_h, off - (uint32_t)k_jof_ofs_tile_h);
  }
  if (off == (uint32_t)k_jof_ofs_bpp) {
    return (uint8_t)k_mg_bpp;
  }
  if (off == (uint32_t)k_jof_ofs_codec) {
    return (uint8_t)k_jof_codec_raw;
  }
  if ((off >= (uint32_t)k_jof_ofs_tile_count) &&
      (off < ((uint32_t)k_jof_ofs_tile_count + (uint32_t)sizeof(uint32_t)))) {
    return t_mg_le_byte((uint32_t)k_mg_bands, off - (uint32_t)k_jof_ofs_tile_count);
  }
 ...
 *
 * @pre `n < k_mg_bands` and `j < k_jof_index_entry`.
...
{
  if (j < (uint32_t)k_jof_idx_ofs_length) {
    const uint32_t toff = (uint32_t)k_jof_hdr_bytes + (n * (uint32_t)k_mg_payload);
    return t_mg_le_byte(toff, j - (uint32_t)k_jof_idx_ofs_offset);
  }
  return t_mg_le_byte((uint32_t)k_mg_payload, j - (uint32_t)k_jof_idx_ofs_length);
}
...
{
  static const uint8_t magic[k_jof_magic_len] = {'J', 'O', 'F', 'E'};
  if (off < (uint32_t)k_jof_ftr_tile_count) {
    return t_mg_le_byte((uint32_t)k_mg_index_off, off - (uint32_t)k_jof_ftr_index_off);
  }
  if (off < (uint32_t)k_jof_ftr_total_size) {
    return t_mg_le_byte((uint32_t)k_mg_bands, off - (uint32_t)k_jof_ftr_tile_count);
  }
  if (off < (uint32_t)k_jof_ftr_magic) {
    return t_mg_le_byte((uint32_t)k_mg_atlas_size, off - (uint32_t)k_jof_ftr_total_size);
  }
  return magic[off - (uint32_t)k_jof_ftr_magic];
}
...
static uint8_t t_mg_vol_byte(uint64_t abs)
{
  ...
  if (local < (uint32_t)k_jof_hdr_bytes) {
    return t_mg_hdr_byte(local);
  }
  if (local < (uint32_t)k_mg_index_off) {
    const uint32_t rel  = local - (uint32_t)k_jof_hdr_bytes;
  ...
  if (local < ((uint32_t)k_mg_index_off + (uint32_t)k_mg_index_len)) {
    ...
    const uint32_t n   = rel / (uint32_t)k_jof_index_entry;
    const uint32_t j   = rel - (n * (uint32_t)k_jof_index_entry);
 ...
 *          and reads the band (tile row) with ::jof_read_tile over the
                                  ...
                                  uint16_t*             out_h)
{
  ...
  return jof_read_tile(t_mg_atlas_pread,
                       &ap,
                       dc->info,
                       key->tile_x,
                       key->tile_y,
...
static uint32_t t_mg_pc_valid(void)
{
  ...
  for (uint32_t i = 0U; i < (uint32_t)k_mg_frames; ++i) {
    if (s_cache_storage.pc_meta[i].valid != 0U) {
      ++live;
    }
  }
  return live;
...
static uint32_t t_mg_tc_valid(void)
{
  ...
  for (uint32_t i = 0U; i < (uint32_t)k_mg_cells; ++i) {
    if (s_cache_storage.tc_meta[i].valid != 0U) {
      ++live;
    }
  }
  return live;
 ...
 * @since 0.1.0
 */
static void t_mg_open_page(ra8_vmem_stream_t* st, uint32_t page, jof_info_t* info)
{
  t_mg_atlas_pread_t ap = {.st = st, .base = (uint64_t)page * (uint64_t)k_mg_atlas_size};
  TEST_ASSERT_EQ(k_ra8_ok, jof_parse(t_mg_atlas_pread, &ap, (uint64_t)k_mg_atlas_size, info));
                           ...
                           jof_info_t*        info)
{
  ...
  ra8_vmem_cfg_t vcfg = {.frame_mem     = s_cache_storage.pc_frames,
                         .frame_bytes   = (uint32_t)k_mg_frame_bytes,
                         .frame_count   = (uint32_t)k_mg_frames,
                         .meta          = s_cache_storage.pc_meta,
                         .keys          = s_cache_storage.pc_keys,
                         .buckets       = s_cache_storage.pc_buckets,
  ...
  *dc                             = (t_mg_decode_ctx_t){.st = st, .info = info};
  const ra8_tile_cache_cfg_t tcfg = {.cell_mem     = s_cache_storage.tc_cells,
                                     .cell_bytes   = (uint32_t)k_mg_cell_bytes,
                                     .cell_count   = (uint32_t)k_mg_cells,
                                     .meta         = s_cache_storage.tc_meta,
                                     .keys         = s_cache_storage.tc_keys,
                                     .dims         = s_cache_storage.tc_dims,
                                     .buckets      = s_cache_storage.tc_buckets,
