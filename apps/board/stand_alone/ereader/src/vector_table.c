/**
 * @file apps/board/stand_alone/ereader/src/vector_table.c
 ...
 * in usb_cdc_echo/linker_script.ld defines them with this exact spelling.
 */
...
/**
 * @brief Service the SysTick exception selected by the linked application.
 * @details Resolves to the weak core timebase handler unless a strong RTOS or
 *          application handler overrides it. The vector table references the
 *          selected implementation without owning its subsystem policy.
 * @pre Entered for a SysTick exception with a valid exception frame.
 * @pre The selected handler's timebase or scheduler state is initialised.
 * @post The selected implementation has processed one SysTick event.
 * @post The vector table itself remains unchanged.
 * @note Runs in handler mode; thread-safety follows the selected implementation.
 * @since 0.1.0
 */
 ...
 * after ra8_isr_register armed the NVIC line halted the chip -- this NSC
 * demo routes USBFS_INT to NVIC slot 0 (ra8_usb_device_attach), so it hit
 * exactly that. The working tz_secure_only_usb_fs / threadx_usbx_cdc_demo
 * apps use this dispatching form; this app now matches.
 */
...
/**
 * @brief Enter the Secure e-reader image after processor reset.
 * @details Copies the initialised data image, clears BSS, performs system and
 *          TrustZone initialisation, and transfers control to the firmware
 *          entry point. If control ever returns, the handler parks the core.
 ...
 * @post C runtime storage is initialised before application code consumes it.
 * @post Control reaches the Non-Secure image or the terminal fallback loop.
 * @note Single-core boot entry; not thread-safe and not expected to return.
 * @since 0.1.0
 */
void Reset_Handler(void)
{
  /* Step 1: initialise the C runtime FIRST. SystemInit() below performs the
   * full Secure bring-up (clock tree + SAU + `ra8_trustzone_init`), and
   * ra8_trustzone_init() BLXNS-es into the Non-Secure world and never returns on
   * hardware. So the .data copy MUST run before SystemInit() -- if it ran after
   * (the classic CMSIS order) it would be dead code here, and every Secure
   * driver would execute against an uninitialised .data image (e.g. log-tag
   * pointers read as NULL, which the pin validator rejects as a null owner). */

  /* Copy initialized data from flash/MRAM to SRAM. */
  // NOLINTBEGIN(clang-analyzer-security.ArrayBound) -- Linker symbols define ranges, not one-element arrays.
  ...
  /* Step 2: core-level init (VTOR, FPU, priority grouping, interrupts masked)
   * followed by the Secure bring-up and the BLXNS into the NS image. Safe to run
   * after the .data copy: the core steps touch no .data globals, and the Secure
   * drivers now see a fully initialised .data image. Does not return on hardware. */
  SystemInit();
...
/**
 * @brief Capture a HardFault and transfer to the common exception reporter.
 * @details Selects the faulting MSP or PSP from EXC_RETURN and supplies
 *          architectural exception number 3 to `ra8_exception_report()`.
 *          Off-target builds call the same reporter with a null frame.
 * @pre A HardFault was raised, or the off-target test invokes the handler.
 * @pre The common exception reporter is linked into the image.
 * @post The reporter receives exception number 3 and the available frame.
 * @post Control does not return to the faulting context.
...
/**
 * @brief Capture a MemManage fault and transfer to the exception reporter.
 * @details Selects the faulting MSP or PSP from EXC_RETURN and supplies
 *          architectural exception number 4 to `ra8_exception_report()`.
 *          Off-target builds call the same reporter with a null frame.
 * @pre A MemManage fault was raised, or the off-target test invokes the handler.
 * @pre The common exception reporter is linked into the image.
 * @post The reporter receives exception number 4 and the available frame.
 * @post Control does not return to the faulting context.
 * @note Runs in fault context and is not thread-callable.
 * @since 0.1.0
 */
...
/**
 * @brief Capture a BusFault and transfer to the common exception reporter.
 * @details Selects the faulting MSP or PSP from EXC_RETURN and supplies
 *          architectural exception number 5 to `ra8_exception_report()`.
 *          Off-target builds call the same reporter with a null frame.
 * @pre A BusFault was raised, or the off-target test invokes the handler.
 * @pre The common exception reporter is linked into the image.
 * @post The reporter receives exception number 5 and the available frame.
 * @post Control does not return to the faulting context.
 * @note Runs in fault context and is not thread-callable.
 * @since 0.1.0
 */
...
/**
 * @brief Capture a UsageFault and transfer to the common exception reporter.
 * @details Selects the faulting MSP or PSP from EXC_RETURN and supplies
 *          architectural exception number 6 to `ra8_exception_report()`.
 *          Off-target builds call the same reporter with a null frame.
 * @pre A UsageFault was raised, or the off-target test invokes the handler.
 * @pre The common exception reporter is linked into the image.
 * @post The reporter receives exception number 6 and the available frame.
 * @post Control does not return to the faulting context.
 * @note Runs in fault context and is not thread-callable.
 * @since 0.1.0
 */
