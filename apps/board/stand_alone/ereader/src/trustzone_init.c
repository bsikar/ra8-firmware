...
/**
 * @brief Write a 32-bit word through a Secure-side absolute address.
 * @details The one place this unit turns an address into a store: the SAU and
 *          CPSCU register writers and the NS image copy all drive it, so no
 *          caller repeats the conversion. Each caller documents its own target.
 * @param[in] addr  Word-aligned Secure-side address (register page or the NS
 *                  image window carved by ::tz_sram_ns_boundary).
 * @param[in] value Value to store.
 * @pre Caller is in Secure state.
 * @pre ``addr`` is word-aligned and Secure-side writable.
 * @post The 32-bit store has landed (verifiable via SWD read-back).
 * @post No other location is affected.
...
/**
 * @brief Read a 32-bit word through a Secure-side absolute address.
 * @details Volatile load, the mirror of ::tz_write32; the @c volatile cast
 *          prevents the compiler from caching or reordering the access relative
 *          to other register touches in the SAU bring-up.
 * @param[in] addr Word-aligned Secure-side address (register page or the NS
 *                 image window in Secure MRAM).
 * @return The contents of the addressed word.
 * @retval 0 Possible for an unimplemented register field or a blank window.
 * @pre Caller is in Secure state.
 * @pre ``addr`` is word-aligned and Secure-side readable.
 * @post No state change.
 * @post The returned value reflects the live location.
 * @note Not thread-safe; secure-boot only.
 * @since 0.1.0
 */
 ...
 * @details Selects the region via SAU_RNR, then writes its base into SAU_RBAR
 *          and its limit (32-byte-aligned, ENABLE set, plus the optional NSC bit)
 *          into SAU_RLAR, defining one Secure / Non-secure / NSC address window.
...
/* The NS image copy exists only for the SRAM-run configuration. Under
 * RA8_EREADER_NS_XIP the NS .text/.rodata execute in place from the OSPI NS
 * alias, ra8_trustzone_init() takes the #ifdef arm that does no copy, and this
 * routine has no call site at all -- so it is compiled out rather than defined
 * and excused. An unreferenced static is exactly what -Wunused-function is for,
 * and this file is built with -Werror. */
#ifndef RA8_EREADER_NS_XIP
/**
...
{
  /* Driven through tz_read32 / tz_write32 rather than open-coded casts: those
   * two are the unit's only integer-to-pointer conversions, and both are
   * `static inline`, so the emitted loop is the same word copy. */
  const uintptr_t load_base = (uintptr_t)k_tz_ns_load_base;
  const uintptr_t run_base  = (uintptr_t)k_tz_ns_run_base;
  for (uintptr_t off = 0U; off < (uintptr_t)k_tz_ns_copy_size; off += sizeof(uint32_t)) {
    tz_write32(run_base + off, tz_read32(load_base + off));
  }
}
#endif /* !RA8_EREADER_NS_XIP */
...
/**
 * @brief Prepare the USB stack for a Secure-to-Non-secure handoff.
 *
 * @details Performs the Secure-side USB groundwork before the NS image takes
 *          ownership of the controller: routes the USB FS-device and HS-host
 ...
 * @post The I/O-expander host-mode write has been attempted (success or busy).
 * @note Not thread-safe; secure-boot only.
 * @since 0.1.0
 */
