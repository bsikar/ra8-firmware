/**
 * @file apps/board/stand_alone/ereader/src/system_init.c
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_enable_icache(void)
{
 ...
 * @pre Execution is privileged during the single-threaded reset path.
 * @pre The boot-ROM invalid-line guarantee remains valid for this image.
 * @post CCR.DC is set and the data cache is enabled.
 * @post The control-register write is globally observed.
 * @note This application-local implementation is unsuitable without the
 *   documented boot-ROM invalid-line guarantee.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_enable_dcache(void)
{
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_enable_branch_predictor(void)
{
 ...
 * @details Selects one region and writes its base attributes and enabled
 *   limit exactly as prepared by internal_mpu_init().
 * @param[in] region MPU region number to select.
 * @param[in] base_attr Aligned base address combined with RBAR attributes.
 * @param[in] limit_enable Aligned limit combined with RLAR attributes.
...
/**
 * @brief Programme and enable the Cortex-M85 core MPU.
 * @details Installs code, private SRAM, SDRAM, and peripheral regions before
 *   enabling the MPU with the privileged default memory map.
 * @pre Execution is privileged during the single-threaded reset path.
 * @pre No application or interrupt accesses memory under an earlier MPU map.
 * @post The four boot regions and their MAIR attributes are installed.
 * @post The MPU is enabled with PRIVDEFENA and barriers have retired the map.
 * @note Call once during reset before caches, interrupts, or application code.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_mpu_init(void)
{
  enum : uint32_t {
