...
#
# mdl end-to-end integration harness. For EVERY export format it runs the
...
#
# Usage: apps/host/mdl/tests/integration.sh [--no-build] [FORMAT ...]
set -uo pipefail
...
for fmt in $FORMATS; do
  ...
  # 1) package
  if ! "$MDL" --pack "$fdir" --format "$fmt" >"$WORK/pack_${fmt//./_}.log" 2>&1; then
    say "FAIL  $fmt  (pack)"
...
rule
say "mdl integration: $pass passed, $fail failed, $skip skipped"
