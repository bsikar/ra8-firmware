...
# The hermetic loopback fixture disables per-host rate pacing. Governor behavior
# has dedicated unit coverage; sleeping between local requests only makes this
# functional test runtime grow with the number of fixture requests.
printf '%s\n' \
  'name = LocalFixture' \
  'host = 127.0.0.1' \
  'kind = manga' \
...
(cd "${WORK}" && "${MDL}" --help >"${WORK}/help.out" 2>"${WORK}/help.err")
grep -q '^usage:$' "${WORK}/help.err"
grep -q -- '--ca-file <FILE>' "${WORK}/help.err"
(cd "${WORK}" && "${MDL}" --version >"${WORK}/version.out" 2>"${WORK}/version.err")
grep -qx 'mdl 0.1.0' "${WORK}/version.out"
[[ ! -e "${WORK}/downloads" ]]
"${MDL}" --list --out "${WORK}/empty-library" >"${WORK}/list-empty.log"
grep -q 'no tracked series under' "${WORK}/list-empty.log"
[[ ! -e "${WORK}/empty-library" ]]
"${MDL}" --update-all --config "${WORK}/site.conf" --out "${WORK}/empty-library" \
  >"${WORK}/update-all-empty.log"
grep -q 'no tracked series to update under' "${WORK}/update-all-empty.log"
[[ ! -e "${WORK}/empty-library" ]]
mkdir -p "${WORK}/verify-empty"
set +e
"${MDL}" --verify "${WORK}/verify-empty" >"${WORK}/verify-empty.log" 2>&1
VERIFY_EMPTY_RC=$?
"${MDL}" --verify "${WORK}/verify-missing" >"${WORK}/verify-missing.log" 2>&1
VERIFY_MISSING_RC=$?
set -e
...
"${MDL}" --config "${WORK}/site.conf" --browse --allow-private \
...
"${MDL}" --config "${WORK}/site.conf" --search 'local series' --allow-private \
...
"${MDL}" --config "${WORK}/broken.conf" --browse --allow-private \
...
"${MDL}" --config "${WORK}/site.conf" --search local --pick 1 \
...
"${MDL}" --config "${WORK}/site.conf" --browse --pick 1 --chapters 1 \
...
set +e
"${MDL}" --config "${WORK}/no-chapters.conf" --series "${BASE}/series/" \
...
set +e
"${MDL}" --config "${WORK}/site.conf" --series "${BASE}/series/" \
...
set +e
NO_PROXY=127.0.0.1 "${MDL}" "${TLS_BASE}/direct.cbz" \
...
CURL_CA_BUNDLE="" SSL_CERT_FILE="" NO_PROXY=127.0.0.1 \
  "${MDL}" "${TLS_BASE}/direct.cbz" --out "${WORK}/direct" --ca-file "${TLS_CERT}" \
...
set +e
CURL_CA_BUNDLE="" SSL_CERT_FILE="" NO_PROXY=127.0.0.1 \
  "${MDL}" "${TLS_BASE}/cookie-required.cbz" --out "${WORK}/direct" \
...
ca_input_failure() {
  ...
  local ca_file="$2"
  set +e
  NO_PROXY=127.0.0.1 "${MDL}" "${TLS_BASE}/cookie-required.cbz" \
...
CURL_CA_BUNDLE="" SSL_CERT_FILE="" NO_PROXY=127.0.0.1 \
  "${MDL}" "${TLS_BASE}/direct.INCOMPLETE.cbz" --out "${WORK}/direct" \
  --ca-file "${TLS_CERT}" --allow-private \
  --contact https://github.com/bsikar/ra8-firmware
...
"${MDL}" --pack "${PAGE_DIR}" --format epub
EPUB="${PAGE_DIR}.epub"
cp "${EPUB}" "${WEB}/direct.epub"
CURL_CA_BUNDLE="" SSL_CERT_FILE="" NO_PROXY=127.0.0.1 \
  "${MDL}" "${TLS_BASE}/direct.epub" --out "${WORK}/direct" --ca-file "${TLS_CERT}" \
  --allow-private --contact https://github.com/bsikar/ra8-firmware
...
set +e
CURL_CA_BUNDLE="" SSL_CERT_FILE="" NO_PROXY=127.0.0.1 \
  "${MDL}" "${TLS_BASE}/bad.cbz" --out "${WORK}/direct" --ca-file "${TLS_CERT}" \
...
set +e
"${MDL}" --verify "${WORK}/verify-symlink" >"${WORK}/verify-symlink.log" 2>&1
VERIFY_SYMLINK_RC=$?
set -e
...
"${MDL}" --list --out "${WORK}/out" >"${WORK}/list.log"
grep -q "${BASE}/series/" "${WORK}/list.log"
"${MDL}" --config "${WORK}/site.conf" --series "${BASE}/series/" --update \
  --out "${WORK}/out" --format loose --allow-private \
  --contact https://github.com/bsikar/ra8-firmware
"${MDL}" --update-all --config "${WORK}/site.conf" --out "${WORK}/out" \
  --format loose --allow-private --contact https://github.com/bsikar/ra8-firmware
"${MDL}" --verify "${WORK}/out"
cp "${PAGE}" "${WORK}/page.save"
printf 'corrupt' >"${PAGE}"
set +e
"${MDL}" --verify "${WORK}/out" >"${WORK}/verify-corrupt.log" 2>&1
VERIFY_RC=$?
set -e
[[ ${VERIFY_RC} -eq 1 ]]
...
"${MDL}" "${BASE}/page.html" --attr src --max 1 --out "${WORK}/page-mode" \
  --allow-private --contact https://github.com/bsikar/ra8-firmware
[[ -s "${WORK}/page-mode/page_0001.png" ]]
[[ ! -e "${WORK}/page-mode/page_0001.jpg" ]]
set +e
"${MDL}" "${BASE}/page-invalid.html" --attr src --max 1 --out "${WORK}/page-invalid" \
...
set +e
"${MDL}" --pack "${WORK}/pack-empty" --format cbz >"${WORK}/pack-empty.log" 2>&1
PACK_EMPTY_RC=$?
set -e
[[ ${PACK_EMPTY_RC} -eq 1 ]]
...
mkdir -p "${WORK}/init"
(cd "${WORK}/init" && "${MDL}" --init-site https://reader.example/books/)
...
set +e
"${MDL}" --pack "${WORK}/no-such-pack-dir" --format cbz \
  >"${WORK}/pack-missing.log" 2>&1
PACK_MISSING_RC=$?
(cd "${WORK}/init" && "${MDL}" --init-site 'not-a-url' >"${WORK}/init-nohost.log" 2>&1)
INIT_NOHOST_RC=$?
set -e
...
set +e
"${MDL}" --list --out "${WORK}/out" >"${WORK}/list-corrupt.log" 2>&1
LIST_CORRUPT_RC=$?
"${MDL}" --update-all --config "${WORK}/site.conf" --out "${WORK}/out" \
  --format loose --allow-private --contact https://github.com/bsikar/ra8-firmware \
...
set +e
"${MDL}" --remove untracked --out "${WORK}/out" >"${WORK}/remove-untracked.log" 2>&1
REMOVE_UNTRACKED_RC=$?
"${MDL}" --remove junk --out "${WORK}/out" >"${WORK}/remove-junk.log" 2>&1
REMOVE_JUNK_RC=$?
"${MDL}" --remove not-series --out "${WORK}/out" >"${WORK}/remove-mismatch.log" 2>&1
REMOVE_MISMATCH_RC=$?
"${MDL}" --remove symlink-remove --out "${WORK}/out" >"${WORK}/remove-symlink.log" 2>&1
REMOVE_SYMLINK_RC=$?
set -e
...
"${MDL}" --remove series --out "${WORK}/browse-picked"
[[ ! -e "${WORK}/browse-picked/series" ]]
"${MDL}" --remove "${BASE}/series/" --out "${WORK}/out"
[[ ! -e "${WORK}/out/series/.mdl_state" ]]
expect_usage --remove series --out "${WORK}/out" --timeout 1

# After every original state-dependent vector, use a separate library to prove
# that a current cover whose cached response changes is revalidated and
# republished. Isolation keeps this added coverage from changing prior vectors.
COVER_OUT="${WORK}/cover-refetch"
"${MDL}" \
  --config "${WORK}/site.conf" \
  --series "${BASE}/series/" \
  --chapters 1 \
  ...
  --allow-private \
  --contact https://github.com/bsikar/ra8-firmware
COVER="${COVER_OUT}/series/cover.png"
COVER_BEFORE="$("${CMAKE}" -E sha256sum "${COVER}")"
COVER_BEFORE="${COVER_BEFORE%% *}"
printf '\211PNG\r\n\032\nfixture-cover-two' >"${WEB}/series/cover.bin"
"${MDL}" \
  --config "${WORK}/site.conf" \
  --series "${BASE}/series/" \
  --chapters 1 \
  ...
  --refetch \
  --contact https://github.com/bsikar/ra8-firmware
COVER_AFTER="$("${CMAKE}" -E sha256sum "${COVER}")"
COVER_AFTER="${COVER_AFTER%% *}"
[[ "${COVER_BEFORE}" != "${COVER_AFTER}" ]]
"${CMAKE}" -E compare_files "${COVER}" "${WEB}/series/cover.bin"

printf 'mdl local HTTP integration: PASS\n'
