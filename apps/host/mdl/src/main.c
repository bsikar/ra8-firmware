/**
 * @file main.c
 * @brief mdl -- host CLI for the e-reader media downloader.
 *
 * @details
...
RA8_INTERNAL static ra8_err_t internal_credential_capacity_error(ra8_io_stream_t* diagnostic,
                                                                 ...
                                                                 size_t           supplied)
{
  ra8_err_t error = priv_mdl_stream_text(k_ra8_ok, diagnostic, "mdl: ");
                                                           ...
                                                           ra8_io_stream_t*  diagnostic)
{
  ...
  if (args->cookie_file != nullptr) {
    ...
    if (error == k_ra8_err_invalid_size) {
      ...
      (void)ra8_io_stream_puts(diagnostic, "mdl: could not load safe --cookie-file input\n");
    }
    if (error != k_ra8_ok) {
      return error;
    }
  ...
  if (error == k_ra8_err_invalid_size) {
    ...
    (void)ra8_io_stream_puts(diagnostic, "mdl: could not load safe nonempty --ca-file input\n");
  }
...
internal_prepare_args(mdl_args_t* args, mdl_cli_mode_t mode, ra8_io_stream_t* diagnostic)
{
  ...
  if (internal_resolve_config_path(args) != k_ra8_ok) {
    (void)ra8_io_stream_puts(diagnostic, "mdl: could not resolve config path\n");
    return 1;
  }
  ...
  if (path_error != k_ra8_ok) {
    (void)ra8_io_stream_puts(diagnostic, "mdl: could not resolve portable command path\n");
    return 1;
  }
  if (internal_uses_cache_path(mode) && (internal_resolve_cache_path(args) != k_ra8_ok)) {
    (void)ra8_io_stream_puts(diagnostic, "mdl: could not resolve persistent cache path\n");
    return 1;
  }
  if (args->ignore_robots) {
    (void)ra8_io_stream_puts(diagnostic,
                             "mdl: WARNING: --ignore-robots set; "
...
internal_prepare_run_policy(const mdl_args_t* args, mdl_run_opts_t* opts, mdl_format_t* format)
{
  *format = mdl_format_from_str(args->format);
  if (*format == k_mdl_format_invalid) {
    const char* const parts[] = {"mdl: bad --format '",
                                 args->format,
                                          ...
                                          const mdl_nums_t*     nums)
{
  ...
  if (internal_storage_init() != k_ra8_ok) {
    (void)internal_diagnostic("mdl: could not initialize portable filesystem binding\n");
    return 1;
  }
  const int result = priv_mdl_compose_dispatch(a, mode, format, opts, nums, &run);
  if (fw_fs_posix_deinit(&s_fs_posix) != k_ra8_ok) {
    (void)internal_diagnostic("mdl: filesystem shutdown failed\n");
    return 1;
  }
  return result;
}
...
int main(int argc, char** argv)
{
  ...
  if (mode == k_mdl_cli_mode_version) {
    return internal_exit_from_error(ra8_io_stream_puts(&s_output, "mdl 0.1.0\n"), false);
  }
  ...
  mdl_format_t format        = k_mdl_format_invalid;
