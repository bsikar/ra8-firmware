 ...
 *
 * @details
 * ``epub_xml_shim.c`` implements four strict streamed entry points that
 * ``epub_open.c`` calls. Those four functions and the container-result
 * struct they share used to be restated by hand in the shim, in
 * ``epub_open.c`` and in three test translation units -- five copies
 ...
 * ``apps/shared_libs/epub`` go through ``epub_open()``.
 *
 ...
 * @brief Out-parameter struct returned by `priv_epub_xml_parse_container()`.
 *
 * @details
 ...
 * second pass, `priv_epub_xml_parse_opf()`.
 *
 * @invariant `opf_path` is NUL-terminated whenever the parse returned
 *            `k_ra8_ok`.
 * @invariant `opf_path` is never longer than `k_epub_max_path_len - 1`.
 *
 * @par Example:
 * @code
 ...
 * @endcode
 *
 * @see priv_epub_xml_parse_container()
 */
typedef struct {
  char opf_path[k_epub_max_path_len]; /**< Rootfile path, NUL-terminated. */
} epub_container_result_t;
 ...
 *
 * @see priv_epub_xml_parse_opf()
 *
 * @since 0.1.0
 */
RA8_PRIV ra8_err_t priv_epub_xml_parse_container(const uint8_t*           xml_bytes,
                                                 size_t                   xml_len,
 ...
 *
 * @see priv_epub_xml_parse_ncx()
 *
 * @since 0.1.0
 */
RA8_PRIV ra8_err_t priv_epub_xml_parse_opf(const uint8_t* xml_bytes,
                                           size_t         xml_len,
                                           epub_book_t*   book);
 ...
 *
 * @pre @p xml_bytes is readable for @p xml_len bytes.
 * @pre `priv_epub_xml_parse_opf()` has already run for @p book.
 ...
 * @note Thread-safe for distinct books.
 *
 * @see priv_epub_xml_parse_nav()
 *
 * @since 0.1.0
 */
RA8_PRIV ra8_err_t priv_epub_xml_parse_ncx(const uint8_t* xml_bytes,
                                           size_t         xml_len,
                                           epub_book_t*   book);
 ...
 * @since 0.1.0
 */
RA8_PRIV ra8_err_t priv_epub_xml_parse_nav(const uint8_t* xml_bytes,
                                           size_t         xml_len,
                                           epub_book_t*   book);
