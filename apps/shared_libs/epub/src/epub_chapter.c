 ...
 * `epub_book_t`:
 *
 *   - `epub_get_chapter_count()` -- size of the spine.
 *   - `epub_load_chapter()`      -- extract one XHTML body via miniz.
 *   - `epub_get_metadata()`      -- title / author / language strings.
 *   - `epub_get_cover_image()`   -- raw bytes of the cover, decoded by
 *                                      the caller via stb_image.
 *   - `epub_set_font()`          -- attach a TTF blob.
 *   - `epub_render_glyph()`      -- rasterise a code point via
 ...
 * in apps/shared_libs/third_party/stb/stb_truetype_impl.c so that clang-tidy's
...
/**
 * @enum epub_chapter_internal_t
 * @brief Implementation-only sizing constants.
 */
typedef enum : uint16_t {
  k_epub_min_font_bytes = 16, /**< Smallest TTF blob we'll accept. */
...
/**
 * @brief Pure glyph-dim-invalid predicate.
...
/**
 * @brief Pure book-not-ready predicate.
                                         ...
                                         size_t*         got_len)
{
  ...
  /* Name lookup does not fully parse extended-size metadata, so malformed
   * central-directory fields may still fail at this validation boundary. */
 ...
 *
 * Splits the second half of `epub_render_glyph` out so that
                                      ...
                                      uint32_t*             out_h)
{
  ...
  const int w = x1 - x0;
  const int h = y1 - y0;
  /* The raw glyf bounding box remains untrusted after the font directory has
   * initialized; reject reversed coordinates before the signed dimensions are
   * converted to size_t. */
...
epub_toc_entry_to_chapter(const epub_book_t* book, uint16_t toc_idx, uint16_t* out_chapter_idx)
{
  ...
  const char* const href     = book->toc[toc_idx].href;
  size_t            base_len = 0U;
  while ((href[base_len] != '\0') && (href[base_len] != '#')) {
    ++base_len;
  }
...
ra8_err_t epub_set_font(epub_book_t* book, const uint8_t* font_data, size_t font_size)
{
  ...
  if (font_size < (size_t)k_epub_min_font_bytes) {
    return k_ra8_err_invalid_size;
  }
