 ...
 * and ::epub_reflow_img_load (resolves an `<img src>` href to encoded
 * bytes in a caller-owned bounded scratch). Atlas sources are either stored
 * archive entries (windowed with `epub_entry_pread`) or external pread
 * seams (SDRAM/SD stores filled by the import path in
 * `epub_img_import.c`). Tile decode itself is `jof_read_tile()`
 * -- one bounded read plus one bounded inflate per miss.
 *
 * Guarded on `__has_include`: the pure `epub` core (and epub-only apps
 * that never pull in `ra8_mem`/`jof`) still link with this TU empty.
 *
 ...
 * are on the include path (reflow / ra8_mem consumers and the host test
...
static const char* const s_tag = "epub_img_tiles";
 ...
 * @pre @p binder->source_count <= ::k_epub_tile_max_sources.
 ...
 */
RA8_INTERNAL
static int32_t internal_find(const epub_tile_binder_t* binder, uint32_t image_id)
{
...
/**
 * @brief ::jof_pread_fn adapter over a stored archive entry.
 * @details @p ctx is the owning ::epub_tile_source_t; the read windows
 *          the entry's uncompressed bytes via `epub_entry_pread`.
...
{
  const epub_tile_source_t* src = (const epub_tile_source_t*)ctx;
  return epub_entry_pread(src->book, src->path, offset, buf, len, got);
}
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_measure_entry(epub_book_t* book, const char* path, uint64_t* out_size)
{
  epub_entry_reader_t reader = {};
  ra8_err_t           err    = epub_entry_open(book, path, &reader, out_size);
  if (err != k_ra8_ok) {
    return err;
  }
  err = epub_entry_close(&reader);
  return err;
}
 ...
 *          `jof_read_tile()` (entry pread for book-backed sources,
 ...
 * @param[in]  ctx        The owning ::epub_tile_binder_t.
...
static ra8_err_t internal_tile_decode(void*                 ctx,
                                      ...
                                      uint16_t*             out_h)
{
  epub_tile_binder_t* binder = (epub_tile_binder_t*)ctx;
  ...
  epub_tile_source_t* src   = &binder->sources[si];
  const jof_pread_fn  pread = (src->book != nullptr) ? internal_entry_pread : src->pread;
  void*               pctx  = (src->book != nullptr) ? (void*)src : src->pread_ctx;
  const ra8_err_t     err   = jof_read_tile(pread,
                                            pctx,
                                            &src->info,
                                            key->tile_x,
                                            key->tile_y,
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_check_slot(const epub_tile_binder_t* binder, uint32_t image_id)
{
  if (binder->source_count >= (uint8_t)k_epub_tile_max_sources) {
 ...
 * @pre `plen < k_epub_max_path_len` (caller-validated).
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_fill_book_source(epub_tile_source_t* src,
                                           epub_book_t*        book,
                                           const char*         path,
                                           size_t              plen,
                                           uint32_t            image_id)
{
  ...
  if (err == k_ra8_ok) {
    err = jof_parse(internal_entry_pread, src, src->total_size, &src->info);
  }
  if (err != k_ra8_ok) {
                               ...
                               uint32_t            image_id)
{
  ...
  if ((plen == 0U) || (plen >= (size_t)k_epub_max_path_len)) {
    return k_ra8_err_invalid_arg;
  }
  ...
  epub_tile_source_t* src = &binder->sources[binder->source_count];
  (void)memset(src, 0, sizeof(*src));
                                   ...
                                   uint32_t            image_id)
{
  ...
  src->image_id   = image_id;
  err             = jof_parse(pread, pread_ctx, total_size, &src->info);
  if (err != k_ra8_ok) {
    (void)memset(src, 0, sizeof(*src));
    return err;
  }
                               ...
                               ra8_tile_t*         out_tile)
{
  ...
  const jof_info_t* info = &binder->sources[si].info;
 ...
 * @details Split out so `epub_reflow_img_load` stays under the
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_loader_args_ok(const epub_img_loader_t* ld, uint32_t href_len)
{
  if (ld->book == nullptr) {
    return k_ra8_err_invalid_arg;
  }
  ...
  if (href_len >= (uint32_t)k_epub_max_path_len) {
    return k_ra8_err_invalid_arg;
  }
  return k_ra8_ok;
}
...
/**
 * @brief Reject any NULL `epub_reflow_img_load` pointer argument.
 * @details Split out so `epub_reflow_img_load` stays under the
 *          NASA-Rule-4 statement budget.
                               ...
                               size_t*         out_len)
{
  ...
  *out_len              = 0U;
  epub_img_loader_t* ld = (epub_img_loader_t*)ctx;
  ...
  size_t          got = 0U;
  const ra8_err_t err = epub_get_resource(ld->book, path, ld->scratch, ld->scratch_cap, &got);
  if (err != k_ra8_ok) {
    return err;
  }
