 ...
 *
 * @details
 * `epub` is a small, dependency-light domain layer that opens an EPUB
 ...
 *      statically-allocated chapter list (`k_epub_max_chapters`).
 ...
 *     `epub_mem_media_t` describing an in-memory `.epub` blob.
 ...
 *
 * @details
 * The reader holds at most `k_epub_max_chapters` chapter entries; an
 * EPUB with a longer spine is truncated and `epub_open()` returns
 * `k_ra8_err_no_mem`. `k_epub_max_path_len` covers the longest
 * `<item href="...">` string we can store; `k_epub_meta_len` is the
 * cap for any single Dublin Core metadata field (title, creator,
 * language).
 */
typedef enum : uint16_t {
  k_epub_max_chapters = 64,  /**< Max spine length we accept.                         */
  k_epub_max_toc      = 64,  /**< Max table-of-contents entries we keep.              */
  k_epub_max_path_len = 192, /**< Max `href` length (incl. NUL).                      */
  k_epub_meta_len     = 128, /**< Max bytes per metadata field.                       */
  k_epub_max_fonts    = 8,   /**< Max embedded font manifest items kept.              */
  k_epub_max_manifest = 96,  /**< Max `<manifest>` `<item>` entries kept (OPF order). */
  k_epub_id_len       = 64,  /**< Max manifest item id length (incl. NUL).            */
  k_epub_media_len    = 48,  /**< Max `media-type` length (incl. NUL).                */
 ...
 */
typedef struct {
  xml_workspace_t reader; /**< 4096-byte open-element stack. */
  /** Spine idref spans retained between OPF passes. */
  xml_span_t references[k_epub_max_chapters];
  xml_span_t legacy_cover_id; /**< EPUB 2 cover manifest id.       */
  uint16_t   reference_count; /**< Valid entries in @p references. */
} epub_xml_workspace_t;

/**
 * @enum epub_toc_kind_t
 ...
 * reader prefers the EPUB 3 nav document. `k_epub_toc_none` means no
 * usable TOC was found (the book is still readable via the spine).
 */
typedef enum : uint8_t {
  k_epub_toc_none = 0, /**< No table of contents parsed.        */
  k_epub_toc_ncx  = 1, /**< Parsed from an EPUB 2 NCX document. */
  k_epub_toc_nav  = 2, /**< Parsed from an EPUB 3 nav.xhtml.    */
} epub_toc_kind_t;
 ...
 * `epub_book_t::chapter_paths`), and may carry a `#fragment` that
 ...
 */
typedef struct {
  char    title[k_epub_meta_len];    /**< Navigation label text ("" if none). */
  char    href[k_epub_max_path_len]; /**< Target href (rel. to OPF dir).      */
  uint8_t depth;                     /**< Nesting level; 0 == top level.      */
} epub_toc_entry_t;

/**
 * @enum epub_render_t
 * @brief Pixel-format selectors for `epub_render_glyph()`.
 *
 * @details
 ...
 */
typedef enum : uint8_t {
  k_epub_render_alpha8 = 0, /**< 8 bits per pixel, alpha mask. */
} epub_render_t;
 ...
 * of this struct as the `media` argument to `epub_open()`.
 *
 ...
 * `epub_open_streamed()`. Unlike ::epub_mem_media_t (which requires the
 ...
 * the resident blob must out-live a book opened via `epub_open()`.
 *
 ...
 */
typedef struct {
  epub_stream_read_fn read; /**< Seek+read callback (non-NULL).                         */
 ...
 */
typedef struct {
  char id[k_epub_id_len];            /**< Manifest item `id` (or "" if absent).  */
  char href[k_epub_max_path_len];    /**< Item `href` (relative to the OPF dir). */
  char media_type[k_epub_media_len]; /**< Item `media-type` (or "" if absent).   */
 ...
 *
 * @details
 * Populated by `epub_open()`. The struct is exposed (not behind a
 * pointer) so callers can statically allocate it. Treat all fields as
 * read-only; modify only via the `epub_*` API.
 *
...
typedef struct {
   ...
   * epub_open.c; the book record owns the archive in-place rather
  ...
  alignas(max_align_t) uint8_t
    zip_archive_storage[k_epub_zip_archive_bytes]; /**< Zip archive storage.              */
  ...
  epub_stream_media_t stream_media; /**< Streamed media descriptor; {} for the resident path. */

  /* --- Chapter table --------------------------------------------------- */
  uint16_t chapter_count; /**< Spine length actually stored. */
  char     chapter_paths[k_epub_max_chapters]
                        [k_epub_max_path_len]; /**< Manifest hrefs (relative to OPF dir). */

  /* --- Metadata -------------------------------------------------------- */
  char title[k_epub_meta_len];      /**< Dublin Core `<dc:title>`.                       */
  char author[k_epub_meta_len];     /**< Dublin Core `<dc:creator>`.                     */
  char language[k_epub_meta_len];   /**< Dublin Core `<dc:language>`.                    */
  char identifier[k_epub_meta_len]; /**< Dublin Core `<dc:identifier>` (unique book id). */
  ...
  /* --- OPF base directory --------------------------------------------- */
  char opf_dir[k_epub_max_path_len]; /**< Directory portion of the OPF. */

  /* --- Table of contents ---------------------------------------------- */
  char     toc_path[k_epub_max_path_len]; /**< Nav/NCX href (rel. to OPF dir); "" if none. */
  uint8_t  toc_kind;                      /**< `epub_toc_kind_t`: source of the TOC.       */
  uint16_t toc_count;                     /**< Number of entries stored in `toc`.          */
  epub_toc_entry_t toc[k_epub_max_toc];   /**< Flattened TOC, document order.              */
 ...
 * @brief Dublin Core metadata bundle returned by `epub_get_metadata()`.
 */
typedef struct {
 ...
 *
 * @details
 * Treats `media` as a pointer to `epub_mem_media_t`, opens the ZIP
 ...
 * blob (::epub_mem_media_t), it takes a ::epub_stream_media_t -- a seek+read
 ...
 * parsed at all.
 *
 * The parsed book is identical to one opened via `epub_open()`: every
 * accessor (`epub_load_chapter()`, `epub_get_cover_image()`,
 * `epub_get_resource()`, ...) works unchanged, streaming each entry from the
 * backing on demand. `epub_close()` tears the reader down for both paths.
 *
 ...
 * @retval k_ra8_err_no_mem            Spine longer than `k_epub_max_chapters`.
 ...
 * @note The whole-file `epub_open()` stays the right choice for small, already
 ...
 *
 * @param[in,out] book Book opened by `epub_open()`.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t epub_close(epub_book_t* book);
 ...
 * @post `*out_count <= k_epub_max_chapters`.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t epub_get_chapter_count(const epub_book_t* book, uint16_t* out_count);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t epub_load_chapter(epub_book_t* book,
                                          uint16_t     idx,
                                          uint8_t*     out_xhtml,
                                          size_t       max_len,
                                          size_t*      got_len);
 ...
 *
 * @param[in]  book     Open book.
 * @param[out] out_kind Receives a `epub_toc_kind_t` value.
 *
 * @return ra8_err_t
 * @retval k_ra8_ok                  Reported.
 * @retval k_ra8_err_null_ptr        Any pointer NULL.
 ...
 * @pre `book->in_use == 1`.
 * @post `*out_kind` is one of the `epub_toc_kind_t` enumerators.
 * @post `*out_kind == k_epub_toc_none` iff `toc_count == 0`.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t epub_get_toc_kind(const epub_book_t* book, uint8_t* out_kind);
 ...
 * `epub_get_toc_entry()` to read each entry's title, href, and depth.
 *
 * @param[in]  book      Open book.
 ...
 * @pre `book` non-NULL, `out_count` non-NULL.
 * @pre `book->in_use == 1`.
 * @post `*out_count <= k_epub_max_toc`.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t epub_get_toc_count(const epub_book_t* book, uint16_t* out_count);
 ...
 */
[[nodiscard]] ra8_err_t
epub_get_toc_entry(const epub_book_t* book, uint16_t idx, epub_toc_entry_t* out_entry);
 ...
 * `epub_load_chapter()` is indexed by spine position. This strips any
 ...
 */
[[nodiscard]] ra8_err_t
epub_toc_entry_to_chapter(const epub_book_t* book, uint16_t toc_idx, uint16_t* out_chapter_idx);
 ...
 * valid until `epub_close()` is called. Missing fields point at the
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t epub_get_metadata(const epub_book_t* book, epub_metadata_t* out_meta);
 ...
 * caller decodes it with `stb_image` (see
 * `apps/shared_libs/third_party/stb`) or
 * any other decoder.
 *
 * @param[in]  book    Open book.
 ...
 */
[[nodiscard]] ra8_err_t
epub_get_cover_image(epub_book_t* book, uint8_t* out_buf, size_t max_len, size_t* got_len);
 ...
 * @note Not thread-safe; single-threaded reader context.
 * @see epub_get_cover_image(), epub_get_embedded_font()
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t epub_get_resource(epub_book_t* book,
                                          const char*  path,
                                          uint8_t*     out_buf,
                                          size_t       max_len,
                                          size_t*      got_len);
 ...
 * document order, up to ::k_epub_max_manifest. The on-device compiler walks
 ...
 * @return Item count (0 .. ::k_epub_max_manifest), or 0 if @p book is NULL.
 * @retval 0 No items, a closed book, or a NULL handle.
 * @pre @p book was populated by `epub_open()`.
 ...
 * lives in @p book and stays valid until `epub_close()`. The caller must not
 * write through the pointer.
 *
 * @param[in] book  Open book (`in_use == 1`).
 * @param[in] index Zero-based item index, `< epub_manifest_count(book)`.
 ...
 * @pre @p book was populated by `epub_open()`.
 * @pre @p index is less than ::epub_manifest_count for @p book.
 * @post No state is mutated; the returned storage is owned by @p book.
 * @note Not thread-safe; single-threaded reader context.
 * @see epub_manifest_count()
 * @since 0.1.0
 */
[[nodiscard]] const epub_manifest_item_t* epub_manifest_item(const epub_book_t* book,
                                                             uint16_t           index);
 ...
 * ::k_epub_max_fonts). This returns how many were found so the caller can
 * iterate `epub_get_embedded_font()`. Zero is normal -- many books rely on
 * the reading-system font.
 *
 * @param[in]  book      Open book.
 * @param[out] out_count Receives the embedded font count (0 .. ::k_epub_max_fonts).
 *
 ...
 * from the open ZIP (same path as `epub_get_cover_image()`). The returned
 * bytes can be validated + bound into the reflow engine via
 * `reflow_bind_font()`, or attached with `epub_set_font()`.
 *
 * @param[in,out] book    Open book.
 * @param[in]     idx     Font index, `0 .. epub_get_embedded_font_count()-1`.
 ...
 *
 * @pre `book->in_use == 1` and `idx < epub_get_embedded_font_count()`.
 * @pre @p out_buf and @p got_len are non-NULL, `max_len > 0`.
 * @post On success `*got_len` holds the font length; on failure `*got_len == 0`.
 *
 * @note Not thread-safe; single-threaded init-context use.
 * @see epub_get_embedded_font_count(), reflow_bind_font()
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t epub_get_embedded_font(epub_book_t* book,
                                               uint16_t     idx,
                                               uint8_t*     out_buf,
                                               size_t       max_len,
                                               size_t*      got_len);
...
/**
 * @brief Attach a TTF font blob to be used by `epub_render_glyph()`.
 *
 * @details
 * `epub` does not embed a default font; the caller (or the host
 ...
 *     via `epub_load_chapter()` style code into a caller buffer.
 *
 * @param[in,out] book      Open book.
 ...
 */
[[nodiscard]] ra8_err_t
epub_set_font(epub_book_t* book, const uint8_t* font_data, size_t font_size);
 ...
 * `epub_set_font()`. The resulting bitmap is 8 bits per pixel,
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t epub_render_glyph(const epub_book_t* book,
                                          int32_t            codepoint,
                                          float              font_size,
                                          uint8_t*           out_bitmap,
                                          size_t             max_pixels,
