 ...
 * NASA Power of 10 Rule 3. But `epub` opens `.epub` archives through the
 ...
 * reusable across an arbitrary number of `epub_load_chapter` calls (each
...
/**
 * @enum epub_miniz_alloc_limits_t
...
typedef enum : uint32_t {
  k_epub_miniz_pool_bytes = 163840U, /**< Arena size, bytes (160 KiB). */
 ...
 *
 * @details Embed this type in the object that owns a miniz archive. The byte
 * member has ``max_align_t`` alignment without requiring an implementation
 * global. The bytes are private allocator state while the arena is live.
 */
typedef struct {
  /** @brief Allocator backing bytes. */
  alignas(max_align_t) uint8_t bytes[k_epub_miniz_pool_bytes];
} epub_miniz_workspace_t;

/**
 * @struct epub_miniz_arena_t
 * @brief Descriptor for one caller-owned miniz allocation arena.
 *
 * @details Initialise with ::epub_miniz_arena_init, then pass this
 ...
 * @note Reinitialise with ::epub_miniz_arena_init before reuse.
 * @since 0.1.0
 */
 ...
 *
 * @param[in] opaque Pointer to an initialised ::epub_miniz_arena_t.
 ...
 *
 * @param[in] opaque  Pointer to the owning ::epub_miniz_arena_t.
 * @param[in] address Block previously returned by ::epub_miniz_alloc /
 *                    ::epub_miniz_realloc, or NULL.
 *
