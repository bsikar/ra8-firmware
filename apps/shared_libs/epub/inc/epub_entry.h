 ...
 *
 * @details
 * Split out of `epub.h` so that header stays inside the 1000-line
 * maintainability cap. This is the forward-streaming cursor over a single ZIP
 * entry: `epub_open()` / `epub_open_streamed()` give you a book, and
 * `epub_entry_open()` / `_read()` / `_close()` inflate one archive entry a
 * bounded block at a time instead of materialising it whole (as
 * `epub_get_resource()` does). `epub_entry_pread()` is the positioned
 ...
 * directly (`epub.h` alone is not sufficient for the entry API).
 *
 *
 * [Ring 4 / EPUB]
 * {World: NS}
...
/**
 * @enum epub_entry_reader_layout_t
 * @brief Fixed-layout constants for @ref epub_entry_reader_t.
 *
 * @details
 ...
 * numeric literal.
 *
 * @invariant `k_epub_entry_reader_reserved_bytes` keeps `done` plus the
 *            padding at 8 bytes, matching the alignment of the leading 64-bit
 *            members.
 * @see epub_entry_reader_t
 * @since 0.1.0
 */
 ...
 * resident book (`epub_open()`) and a streamed book
 * (`epub_open_streamed()`); in the streamed case each compressed block is
 ...
 * @see epub_entry_open()
 * @see epub_entry_read()
 * @see epub_entry_close()
 * @since 0.1.0
 */
typedef struct {
  void*        iter;     /**< Opaque `mz_zip_reader_extract_iter_state*`; NULL when closed. */
  epub_book_t* book;     /**< Owning book (archive + allocator); borrowed.                  */
 ...
 * inflated yet; the caller pulls them with `epub_entry_read()`. The entry's
 ...
 *
 * @return ra8_err_t
 * @retval k_ra8_ok                    Cursor open; read with `epub_entry_read()`.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t epub_entry_open(epub_book_t*         book,
                                        const char*          path,
 ...
 * entry inflates to.
 *
 * @param[in]  reader Cursor from `epub_entry_open()`.
 ...
 *
 * @pre @p reader came from a successful `epub_entry_open()`.
 ...
 *
 * @param[in,out] reader Cursor from `epub_entry_open()`.
 *
 * @return ra8_err_t
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t epub_entry_close(epub_entry_reader_t* reader);
 ...
 * random-access primitive the tile source (`epub_img_tiles`) uses to page a
 ...
 * (`epub_entry_read()`) already provides.
 *
 * @param[in]  book   Open book (`in_use == 1`, archive active).
 * @param[in]  path   Entry path, OPF-dir-relative or archive-rooted, NUL-terminated.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t epub_entry_pread(epub_book_t* book,
                                         const char*  path,
                                         uint64_t     offset,
                                         uint8_t*     buf,
                                         size_t       len,
