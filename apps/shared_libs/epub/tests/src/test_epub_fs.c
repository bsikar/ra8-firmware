...
internal_mem_read(void* ctx, uint64_t lba, uint32_t count, uint8_t* buf)
{
  ...
  if ((lba > d->block_count) || ((uint64_t)count > ((uint64_t)d->block_count - lba))) {
...
internal_mem_write(void* ctx, uint64_t lba, uint32_t count, const uint8_t* buf)
{
  ...
  (void)memcpy(&d->bytes[(size_t)lba * (uint32_t)k_disk_block_size],
               buf,
               (size_t)count * (uint32_t)k_disk_block_size);
  return k_ra8_ok;
}
...
RA8_INTERNAL static void internal_build_fat16_volume(void)
{
  ...
  s_disk.bytes       = &s_disk_bytes[0];
  s_disk.block_count = (uint32_t)k_disk_blocks;
  (void)memset(s_disk.bytes, 0, s_disk.byte_count);
...
/** @brief EPUB source and generated bytes shared by the filesystem test phases. */
static struct {
  uint8_t     epub[k_epub_cap]; /**< Generated EPUB bytes.        */
  size_t      epub_len;         /**< Finalized EPUB length.       */
  uint8_t     read[k_read_cap]; /**< Filesystem readback storage. */
  const char* mimetype;         /**< Required EPUB media type.    */
  ...
  .chapter_one = "<?xml version=\"1.0\"?><html><body><h1>Letter 1</h1>"
                 "<p>You will rejoice to hear that no disaster has accompanied the "
                 "commencement of an enterprise which you have regarded with such evil "
                 "forebodings.</p></body></html>",
  .chapter_two = "<?xml version=\"1.0\"?><html><body><h1>Chapter 1</h1>"
...
RA8_INTERNAL static void internal_build_epub(void)
{
  ...
  struct {
    ...
    {"mimetype", s_fixture.mimetype, MZ_NO_COMPRESSION},
    {"META-INF/container.xml", s_fixture.container, MZ_DEFAULT_COMPRESSION},
    {"OEBPS/content.opf", s_fixture.opf, MZ_DEFAULT_COMPRESSION},
    {"OEBPS/c1.xhtml", s_fixture.chapter_one, MZ_DEFAULT_COMPRESSION},
    {"OEBPS/c2.xhtml", s_fixture.chapter_two, MZ_DEFAULT_COMPRESSION},
  };
  ...
  TEST_ASSERT((heap != nullptr) && (hsz > 0U) && (hsz <= sizeof(s_fixture.epub)));
  (void)memcpy(s_fixture.epub, heap, hsz);
  s_fixture.epub_len = hsz;
  mz_free(heap);
...
RA8_INTERNAL static void internal_write_epub(ra8_fs_mount_t* mount, const char* path)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_write(f, s_fixture.epub, (uint32_t)s_fixture.epub_len));
 ...
 *        epub_open_streamed_fs fails cleanly and closes the file handle.
 *
 * @details
...
{
  TEST_BEGIN("epub_fs: corrupt FAT chain -> streamed open fails, file closed");
  internal_build_fat16_volume();
  internal_build_epub();
  TEST_ASSERT(s_fixture.epub_len > (size_t)k_disk_block_size); /* multi-cluster chain */
  ...
  epub_book_t          book = {};
  TEST_ASSERT_EQ(k_ra8_err_validation_failed, epub_open_streamed_fs(mount, "BOOK.EPB", &io, &book));
  TEST_ASSERT(io.file == nullptr); /* the failed open released the handle */
  TEST_ASSERT_EQ(0, book.in_use);

  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(mount));
  TEST_END("epub_fs: corrupt FAT chain -> streamed open fails, file closed");
}
 ...
 * @pre `s_fixture.epub` / `s_fixture.epub_len` describe the bytes that were written.
...
RA8_INTERNAL static void internal_check_fs_byte_roundtrip(ra8_fs_mount_t* mount)
{
  ...
  TEST_ASSERT_EQ(s_fixture.epub_len, sz);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_read(rf, s_fixture.read, sz, &gt));
  TEST_ASSERT_EQ(sz, gt);
  TEST_ASSERT_EQ(0, memcmp(s_fixture.read, s_fixture.epub, (size_t)sz));
  (void)ra8_fs_close(rf);
}

/**
 * @test internal_test_epub_fs_stream_read_mcdc
 * @brief Independently vary every operand of the filesystem callback guard.
 *
 * @par MC/DC:
 * Decision owner:
 * apps/shared_libs/epub/src/epub_fs.c@priv_epub_fs_stream_read
 * For `(io == nullptr) || (io->file == nullptr) || (buf == nullptr)`, the
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_test_epub_fs_stream_read_mcdc(void)
{
  TEST_BEGIN("epub_fs MC/DC: stream callback context/file/buffer OR");
  internal_build_fat16_volume();
  internal_build_epub();
  ra8_fs_mount_t* mount = nullptr;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_mount(&s_backend, &mount));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_open(mount, "BOOK.EPB", k_ra8_fs_mode_read, &file));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(mount));
  TEST_END("epub_fs MC/DC: stream callback context/file/buffer OR");
}
...
{
  TEST_BEGIN("epub_fs: streamed open roundtrip (no whole-file residency)");
  internal_build_fat16_volume();
  internal_build_epub();
  ...
  epub_stream_fs_ctx_t io   = {};
  epub_book_t          book = {};
  TEST_ASSERT_EQ(k_ra8_ok, epub_open_streamed_fs(mount, "BOOK.EPB", &io, &book));
  ...
  size_t  got                             = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, epub_load_chapter(&book, 0U, chbuf, sizeof(chbuf) - 1U, &got));
  TEST_ASSERT(got > 0U);
  chbuf[got] = (uint8_t)'\0';
  TEST_ASSERT(strstr((const char*)chbuf, "rejoice") != nullptr);

  size_t got2 = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, epub_load_chapter(&book, 1U, chbuf, sizeof(chbuf) - 1U, &got2));
  TEST_ASSERT(got2 > 0U);
  chbuf[got2] = (uint8_t)'\0';
  TEST_ASSERT(strstr((const char*)chbuf, "Genevese") != nullptr);

  TEST_ASSERT_EQ(k_ra8_ok, epub_close_streamed_fs(&io, &book));
  TEST_ASSERT(io.file == nullptr); /* file released on close */
  TEST_ASSERT_EQ(0, book.in_use);

  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(mount));
  TEST_END("epub_fs: streamed open roundtrip (no whole-file residency)");
}

/**
 * @test internal_test_epub_fs_streamed_guards
 * @brief epub_open_streamed_fs rejects NULL args and a missing file, closes the
 *        source file on a parse failure, and epub_close_streamed_fs is a safe
...
{
  TEST_BEGIN("epub_fs: streamed open guards");
  internal_build_fat16_volume();
  internal_build_epub();
  ra8_fs_mount_t* mount = nullptr;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_mount(&s_backend, &mount));
  ...
  /* A small non-ZIP file: it opens + sizes fine but epub_open_streamed rejects
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_open_streamed_fs(nullptr, "BOOK.EPB", &io, &book));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_open_streamed_fs(mount, nullptr, &io, &book));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_open_streamed_fs(mount, "BOOK.EPB", nullptr, &book));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_open_streamed_fs(mount, "BOOK.EPB", &io, nullptr));

  /* Missing file -> propagated open error; no file left open. */
  TEST_ASSERT(epub_open_streamed_fs(mount, "NOPE.EPB", &io, &book) != k_ra8_ok);
  TEST_ASSERT(io.file == nullptr);

  /* Non-ZIP file -> parse failure; the adapter closes the file it opened. */
  TEST_ASSERT(epub_open_streamed_fs(mount, "BAD.EPB", &io, &book) != k_ra8_ok);
  TEST_ASSERT(io.file == nullptr);
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_close_streamed_fs(nullptr, &book));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_close_streamed_fs(&io, nullptr));
  TEST_ASSERT(epub_close_streamed_fs(&io, &book) != k_ra8_ok); /* book not in_use */
  TEST_ASSERT(io.file == nullptr);

  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(mount));
  TEST_END("epub_fs: streamed open guards");
}

int main(void)
{
  internal_test_epub_fs_stream_read_mcdc();
