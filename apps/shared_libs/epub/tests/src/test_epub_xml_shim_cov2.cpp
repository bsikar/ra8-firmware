/**
 * @file test_epub_xml_shim_cov2.cpp
 * @brief Complementary line-coverage tests for apps/shared_libs/epub/src/epub_xml_shim.c.
 *
 * @par Tag
 * [Ring 4 / EPUB] {World: NS}
 *
 * @details
 * Complements ``test_epub_xml_shim.cpp`` with black-box malformed-input,
 ...
 * ``epub_xml_shim_internal.h``. Every fixture is immutable and every parse
 * uses caller-owned ``epub_xml_workspace_t`` state.
 *
...
static epub_xml_workspace_t s_xml_workspace = {};
...
RA8_INTERNAL static void internal_test_cov2_container_error_paths(void)
{
  ...
  assert(priv_epub_xml_parse_container(nullptr, 1U, &res, &s_xml_workspace) == k_ra8_err_null_ptr);
  assert(priv_epub_xml_parse_container(internal_bytes_of("x"), 1U, nullptr, &s_xml_workspace) ==
         k_ra8_err_null_ptr);

  /* Zero length -> invalid_size. */
  assert(priv_epub_xml_parse_container(internal_bytes_of("x"), 0U, &res, &s_xml_workspace) ==
         k_ra8_err_invalid_size);

  /* Comment-only document -> parses, but exposes no root element. */
  constexpr const char* k_comment = "<!-- only a comment -->";
  assert(priv_epub_xml_parse_container(internal_bytes_of(k_comment),
  ...
  assert(priv_epub_xml_parse_container(internal_bytes_of(k_no_rootfile),
  ...
  constexpr const char* k_nested_rootfile =
    ...
    "</rootfiles></container>";
  assert(priv_epub_xml_parse_container(internal_bytes_of(k_nested_rootfile),
  ...
  assert(priv_epub_xml_parse_container(internal_bytes_of(k_no_attr),
  ...
  assert(priv_epub_xml_parse_container(internal_bytes_of(k_empty_attr),
...
RA8_INTERNAL static void internal_test_cov2_container_success(void)
{
  ...
  assert(priv_epub_xml_parse_container(internal_bytes_of(k_nested),
                                       std::strlen(k_nested),
                                       &res,
  ...
  assert(priv_epub_xml_parse_container(internal_bytes_of(k_direct),
                                       std::strlen(k_direct),
                                       &res,
                                       &s_xml_workspace) == k_ra8_ok);
 ...
 *  - chapter cap `if (count >= k_epub_max_chapters) return k_ra8_err_no_mem;`
...
RA8_INTERNAL static void internal_test_cov2_opf_edges(void)
{
  ...
  /* Zero length -> invalid_size. */
  assert(priv_epub_xml_parse_opf(internal_bytes_of("x"), 0U, &book) == k_ra8_err_invalid_size);

  /* Comment-only document -> no package root element. */
  constexpr const char* k_comment = "<!-- only a comment -->";
  assert(priv_epub_xml_parse_opf(internal_bytes_of(k_comment), std::strlen(k_comment), &book) ==
         k_ra8_err_validation_failed);

  /* Legacy cover meta with an unresolved id + spine skip legs. */
  std::memset(&book, 0, sizeof(book));
  assert(priv_epub_xml_parse_opf(internal_bytes_of(s_opf_cover_meta_spine_edges),
  ...
  /* Chapter cap: a spine of (cap + 1) resolving itemrefs -> no_mem. */
  {
    std::string xml =
      ...
      "<spine>";
    for (int i = 0; i < static_cast<int>(k_epub_max_chapters) + 1; ++i) {
      xml += "<itemref idref=\"c\"/>";
    }
    xml += "</spine></package>";
    std::memset(&book, 0, sizeof(book));
    assert(priv_epub_xml_parse_opf(internal_bytes_of(xml.c_str()), xml.size(), &book) ==
           k_ra8_err_no_mem);
  }
}
 ...
 *
 * @par MC/DC:
 * Sequential single-condition legs of `priv_epub_xml_parse_ncx`:
...
RA8_INTERNAL static void internal_test_cov2_ncx_edges(void)
{
  ...
  assert(priv_epub_xml_parse_ncx(internal_bytes_of(k_bad), std::strlen(k_bad), &book) ==
         k_ra8_err_validation_failed);

  constexpr const char* k_comment = "<!-- only a comment -->";
  assert(priv_epub_xml_parse_ncx(internal_bytes_of(k_comment), std::strlen(k_comment), &book) ==
         k_ra8_err_validation_failed);
}
 ...
 *
 * @par MC/DC:
 * Sequential single-condition legs of `priv_epub_xml_parse_nav`:
 *  - `if (xml_len == 0U)` -> invalid_size.
...
RA8_INTERNAL static void internal_test_cov2_nav_edges(void)
{
  ...
  /* Zero length -> invalid_size. */
  assert(priv_epub_xml_parse_nav(internal_bytes_of("x"), 0U, &book) == k_ra8_err_invalid_size);

  /* Malformed XML -> parse failure. */
  constexpr const char* k_bad = "<a><b></c>";
  assert(priv_epub_xml_parse_nav(internal_bytes_of(k_bad), std::strlen(k_bad), &book) ==
         k_ra8_err_validation_failed);

  /* Comment-only -> no root element. */
  constexpr const char* k_comment = "<!-- only a comment -->";
  assert(priv_epub_xml_parse_nav(internal_bytes_of(k_comment), std::strlen(k_comment), &book) ==
         k_ra8_err_validation_failed);

  /* toc nav present but with no <ol> -> validation_failed. */
  std::memset(&book, 0, sizeof(book));
  assert(priv_epub_xml_parse_nav(internal_bytes_of(s_nav_no_ol), std::strlen(s_nav_no_ol), &book) ==
         k_ra8_err_validation_failed);

  /* Stray non-<li> child in the <ol> is skipped; one real entry remains. */
  std::memset(&book, 0, sizeof(book));
  assert(priv_epub_xml_parse_nav(internal_bytes_of(s_nav_stray_then_li),
...
{
  epub_container_result_t res = {};
  assert(priv_epub_xml_parse_container(internal_bytes_of(s_container_sibling_first),
  ...
  std::memset(&res, 0, sizeof(res));
  assert(priv_epub_xml_parse_container(internal_bytes_of(s_container_global_first),
  ...
  assert(priv_epub_xml_parse_opf(internal_bytes_of(s_opf_operand_matrix),
  ...
  std::memset(&s_book, 0, sizeof(s_book));
  assert(priv_epub_xml_parse_ncx(internal_bytes_of(s_ncx_unowned_label),
  ...
  std::memset(&s_book, 0, sizeof(s_book));
  assert(priv_epub_xml_parse_nav(internal_bytes_of(s_nav_indirect_ol),
                                 std::strlen(s_nav_indirect_ol),
                                 &s_book) == k_ra8_err_validation_failed);

  std::memset(&s_book, 0, sizeof(s_book));
  assert(priv_epub_xml_parse_nav(internal_bytes_of(s_nav_operand_matrix),
