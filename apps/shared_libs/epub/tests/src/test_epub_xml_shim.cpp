/**
 * @file test_epub_xml_shim.cpp
 * @brief MC/DC vector tests for apps/shared_libs/epub/src/epub_xml_shim.c.
 *
 * @par Tag
 ...
 * ``test_epub_xml_shim_cov2`` executable supplies malformed-input,
 ...
 * internal reader-error arm after successful ``xml_validate`` is
...
epub_xml_workspace_t s_xml_workspace = {};
 ...
 * @par MC/DC:
 * Decisions:
 * - apps/shared_libs/epub/src/epub_xml_shim.c@priv_epub_xml_attr
 * - apps/shared_libs/epub/src/epub_xml_shim.c@priv_epub_xml_find
 * - apps/shared_libs/epub/src/epub_xml_shim.c@priv_epub_xml_parse_container
 *
...
RA8_INTERNAL static void internal_test_mcdc_parse_container_null_or_zero(void)
{
  ...
  epub_container_result_t res = {};

  /* Decision 1, V1: both non-NULL -> success. */
  const auto* xml_v1 = reinterpret_cast<const uint8_t*>(s_valid_container);
  assert(
    priv_epub_xml_parse_container(xml_v1, std::strlen(s_valid_container), &res, &s_xml_workspace) ==
    k_ra8_ok);
  assert(std::strcmp(res.opf_path, "OEBPS/content.opf") == 0);

  /* Decision 1, V2: xml_bytes NULL -> null_ptr. */
  assert(priv_epub_xml_parse_container(nullptr, 16U, &res, &s_xml_workspace) == k_ra8_err_null_ptr);

  /* Decision 1, V3: out NULL -> null_ptr. */
  assert(priv_epub_xml_parse_container(xml_v1,
                                       std::strlen(s_valid_container),
                                       nullptr,
                                       &s_xml_workspace) == k_ra8_err_null_ptr);

  /* Decision 1, V4: workspace NULL -> null_ptr. */
  assert(priv_epub_xml_parse_container(xml_v1, std::strlen(s_valid_container), &res, nullptr) ==
         k_ra8_err_null_ptr);
  ...
  assert(priv_epub_xml_parse_container(xml_v5,
  ...
  assert(priv_epub_xml_parse_container(xml_v6,
 ...
 * @par MC/DC:
 * Decisions:
 * - apps/shared_libs/epub/src/epub_xml_shim.c@internal_opf_status
 * - apps/shared_libs/epub/src/epub_xml_shim.c@internal_opf_shape
 * - apps/shared_libs/epub/src/epub_xml_shim.c@priv_epub_xml_parse_opf
 *
...
RA8_INTERNAL static void internal_test_mcdc_parse_opf_null_and_manifest_spine(void)
{
  ...
  assert(priv_epub_xml_parse_opf(xml_v1, std::strlen(s_valid_opf), &book) == k_ra8_ok);

  /* Decision 1, V2: xml_bytes NULL -> null_ptr. */
  assert(priv_epub_xml_parse_opf(nullptr, 16U, &book) == k_ra8_err_null_ptr);

  /* Decision 1, V3: book NULL -> null_ptr. */
  assert(priv_epub_xml_parse_opf(xml_v1, std::strlen(s_valid_opf), nullptr) == k_ra8_err_null_ptr);
  ...
  assert(priv_epub_xml_parse_opf(xml_v5, std::strlen(s_opf_no_manifest), &book) ==
  ...
  assert(priv_epub_xml_parse_opf(xml_v6, std::strlen(s_opf_no_spine), &book) ==
         k_ra8_err_validation_failed);
}
...
RA8_INTERNAL static void internal_assert_shared_toc(const epub_book_t& book)
{
 ...
 * - apps/shared_libs/epub/src/epub_xml_toc.c@internal_ncx_event
 * - apps/shared_libs/epub/src/epub_xml_toc.c@priv_epub_xml_parse_ncx
 * - apps/shared_libs/epub/src/epub_xml_toc.c@internal_nav_event
 * - apps/shared_libs/epub/src/epub_xml_toc.c@internal_nav_event_start
 * - apps/shared_libs/epub/src/epub_xml_toc.c@priv_epub_xml_parse_nav
...
RA8_INTERNAL static void internal_test_parse_ncx_and_nav(void)
{
  ...
  assert(priv_epub_xml_parse_ncx(ncx, std::strlen(s_ncx_doc), &book) == k_ra8_ok);
  internal_assert_shared_toc(book);

  std::memset(&book, 0, sizeof(book));
  const auto* nav = reinterpret_cast<const uint8_t*>(s_nav_doc);
  assert(priv_epub_xml_parse_nav(nav, std::strlen(s_nav_doc), &book) == k_ra8_ok);
  internal_assert_shared_toc(book);

  std::memset(&book, 0, sizeof(book));
  const auto* ncx_missing = reinterpret_cast<const uint8_t*>(s_ncx_missing_src);
  assert(priv_epub_xml_parse_ncx(ncx_missing, std::strlen(s_ncx_missing_src), &book) == k_ra8_ok);
  ...
  assert(priv_epub_xml_parse_nav(fallback, std::strlen(s_nav_fallback), &book) == k_ra8_ok);
  assert(std::strcmp(book.toc[0].title, "Fallback") == 0);

  std::memset(&book, 0, sizeof(book));
  const auto* span = reinterpret_cast<const uint8_t*>(s_nav_span);
  assert(priv_epub_xml_parse_nav(span, std::strlen(s_nav_span), &book) == k_ra8_ok);
  assert(std::strcmp(book.toc[0].title, "Span title") == 0);

  std::memset(&book, 0, sizeof(book));
  const auto* self_closing = reinterpret_cast<const uint8_t*>(s_nav_self_closing);
  assert(priv_epub_xml_parse_nav(self_closing, std::strlen(s_nav_self_closing), &book) == k_ra8_ok);
 ...
 *
 * `priv_epub_xml_parse_opf` must record which navigation document to load:
 ...
 * @par MC/DC:
 * Decisions:
 * - apps/shared_libs/epub/src/epub_xml_shim.c@internal_metadata_text
 * - apps/shared_libs/epub/src/epub_xml_shim.c@internal_mark_metadata
 * - apps/shared_libs/epub/src/epub_xml_shim.c@internal_opf_first
 * - apps/shared_libs/epub/src/epub_xml_shim.c@internal_opf_first_event
 * - apps/shared_libs/epub/src/epub_xml_shim.c@internal_opf_metadata_child
 * - apps/shared_libs/epub/src/epub_xml_shim.c@internal_manifest_item
 * - apps/shared_libs/epub/src/epub_xml_shim.c@internal_manifest_lookup
 * - apps/shared_libs/epub/src/epub_xml_shim.c@internal_opf_resolve_refs
 *
 ...
 * extracted from ``priv_epub_xml_parse_opf``; the vectors covering it are
...
RA8_INTERNAL static void internal_test_parse_opf_toc_source(void)
{
  ...
  assert(priv_epub_xml_parse_opf(nav, std::strlen(s_opf_with_nav), &book) == k_ra8_ok);
  assert(book.toc_kind == static_cast<uint8_t>(k_epub_toc_nav));
  assert(std::strcmp(book.toc_path, "nav.xhtml") == 0);

  std::memset(&book, 0, sizeof(book));
  const auto* ncx = reinterpret_cast<const uint8_t*>(s_opf_with_ncx);
  assert(priv_epub_xml_parse_opf(ncx, std::strlen(s_opf_with_ncx), &book) == k_ra8_ok);
  assert(book.toc_kind == static_cast<uint8_t>(k_epub_toc_ncx));
  ...
  assert(priv_epub_xml_parse_opf(plain, std::strlen(s_valid_opf), &book) == k_ra8_ok);
  assert(book.toc_kind == static_cast<uint8_t>(k_epub_toc_none));
  assert(book.toc_path[0] == '\0');

  std::memset(&book, 0, sizeof(book));
  const auto* identifiers = reinterpret_cast<const uint8_t*>(s_opf_identifiers);
  assert(priv_epub_xml_parse_opf(identifiers, std::strlen(s_opf_identifiers), &book) == k_ra8_ok);
 ...
 * - apps/shared_libs/epub/src/epub_xml_toc.c@internal_select_nav
 * - apps/shared_libs/epub/src/epub_xml_toc.c@internal_nav_has_list
 * - apps/shared_libs/epub/src/epub_xml_toc.c@priv_epub_xml_parse_ncx
 * - apps/shared_libs/epub/src/epub_xml_toc.c@priv_epub_xml_parse_nav
...
RA8_INTERNAL static void internal_test_parse_toc_guards(void)
{
  ...
  assert(priv_epub_xml_parse_ncx(nullptr, 16U, &book) == k_ra8_err_null_ptr);
  assert(priv_epub_xml_parse_ncx(ncx, 0U, &book) == k_ra8_err_invalid_size);
  assert(priv_epub_xml_parse_nav(nullptr, 16U, &book) == k_ra8_err_null_ptr);
  ...
  assert(priv_epub_xml_parse_ncx(bad_ncx, std::strlen(k_not_ncx), &book) ==
         k_ra8_err_validation_failed);
  ...
  assert(priv_epub_xml_parse_nav(bad_nav, std::strlen(k_not_nav), &book) ==
         k_ra8_err_validation_failed);
}
...
/** @test TOC capacity is exact and exhaustion is failure-atomic. @par MC/DC: NCX and nav at-cap/over-cap vectors independently vary list-required, enabled, entry, close, and exhaustion predicates in apps/shared_libs/epub/src/epub_xml_toc.c@internal_toc_capacity. @brief Verify toc capacity atomicity behavior. @details Executes the toc capacity atomicity scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_toc_capacity_atomicity(void)
{
  const std::string ncx_at = internal_build_toc_document(false, k_epub_max_toc);
  const std::string nav_at = internal_build_toc_document(true, k_epub_max_toc);
  epub_book_t       book   = {};
  assert(priv_epub_xml_parse_ncx(reinterpret_cast<const uint8_t*>(ncx_at.data()),
                                 ncx_at.size(),
                                 &book) == k_ra8_ok);
  assert(book.toc_count == k_epub_max_toc);
  std::memset(&book, 0, sizeof(book));
  assert(priv_epub_xml_parse_nav(reinterpret_cast<const uint8_t*>(nav_at.data()),
                                 nav_at.size(),
                                 &book) == k_ra8_ok);
  assert(book.toc_count == k_epub_max_toc);

  const std::string ncx_over = internal_build_toc_document(false, k_epub_max_toc + 1U);
  const std::string nav_over = internal_build_toc_document(true, k_epub_max_toc + 1U);
  book.toc_count             = 1U;
  std::strcpy(book.toc[0].title, "sentinel");
  const epub_toc_entry_t sentinel = book.toc[0];
  assert(priv_epub_xml_parse_ncx(reinterpret_cast<const uint8_t*>(ncx_over.data()),
                                 ncx_over.size(),
                                 &book) == k_ra8_err_no_mem);
  assert((book.toc_count == 1U) && (std::memcmp(&book.toc[0], &sentinel, sizeof(sentinel)) == 0));
  assert(priv_epub_xml_parse_nav(reinterpret_cast<const uint8_t*>(nav_over.data()),
                                 nav_over.size(),
                                 &book) == k_ra8_err_no_mem);
  assert((book.toc_count == 1U) && (std::memcmp(&book.toc[0], &sentinel, sizeof(sentinel)) == 0));
}
 ...
 */
constexpr std::size_t s_meta_truncated_len = static_cast<std::size_t>(k_epub_meta_len) - 1U;

/* NCX whose first navPoint text is longer than `k_epub_meta_len`, so the
 ...
 * ``xml_decode_prefix`` copy with ``cap == k_epub_meta_len``.
...
RA8_INTERNAL static void internal_test_mcdc_copy_bounded_truncation(void)
{
  ...
  assert(priv_epub_xml_parse_ncx(ncx, std::strlen(s_ncx_long_title), &book) == k_ra8_ok);
 ...
 * - apps/shared_libs/epub/src/epub_xml_shim.c@internal_collect_spine
 * - apps/shared_libs/epub/src/epub_xml_shim.c@internal_manifest_lookup
 * - apps/shared_libs/epub/src/epub_xml_shim.c@internal_opf_resolve_refs
...
RA8_INTERNAL static void internal_test_mcdc_manifest_item_missing_id(void)
{
  ...
  assert(priv_epub_xml_parse_opf(xml, std::strlen(s_opf_manifest_item_no_id), &book) == k_ra8_ok);
...
RA8_INTERNAL static void internal_test_mcdc_cover_by_meta(void)
{
  ...
  assert(priv_epub_xml_parse_opf(xml_ok, std::strlen(s_opf_meta_cover_ok), &book) == k_ra8_ok);
  ...
  assert(priv_epub_xml_parse_opf(xml_no_name, std::strlen(s_opf_meta_no_name), &book) == k_ra8_ok);
  ...
  assert(priv_epub_xml_parse_opf(xml_no_content, std::strlen(s_opf_meta_no_content), &book) ==
         k_ra8_ok);
  assert(book.cover_path[0] == '\0');
  ...
  assert(priv_epub_xml_parse_opf(xml_other, std::strlen(s_opf_meta_other_name), &book) == k_ra8_ok);
  assert(book.cover_path[0] == '\0');
}
...
RA8_INTERNAL static void internal_test_mcdc_nav_manifest_props_not_nav(void)
{
  ...
  assert(priv_epub_xml_parse_opf(xml, std::strlen(s_opf_props_not_nav), &book) == k_ra8_ok);
  assert(book.toc_kind == static_cast<uint8_t>(k_epub_toc_none));
  assert(book.toc_path[0] == '\0');
 ...
 * - apps/shared_libs/epub/src/epub_xml_shim.c@internal_font_type
 * - apps/shared_libs/epub/src/epub_xml_shim.c@internal_manifest_item
...
RA8_INTERNAL static void internal_test_mcdc_collect_fonts_href_and_type(void)
{
  ...
  assert(priv_epub_xml_parse_opf(xml, std::strlen(s_opf_font_and_no_href), &book) == k_ra8_ok);
  ...
  for (std::size_t i = 0U; i <= static_cast<std::size_t>(k_epub_max_fonts); ++i) {
  ...
  std::memset(&book, 0, sizeof(book));
  assert(priv_epub_xml_parse_opf(reinterpret_cast<const uint8_t*>(capped.data()),
                                 capped.size(),
                                 &book) == k_ra8_ok);
  assert(book.embedded_font_count == k_epub_max_fonts);
}
 ...
 * in apps/shared_libs/epub/src/epub_xml_toc.c@internal_select_nav. The typed toc
...
RA8_INTERNAL static void internal_test_mcdc_nav_by_type_untyped_first(void)
{
  ...
  assert(priv_epub_xml_parse_nav(nav, std::strlen(s_nav_untyped_first), &book) == k_ra8_ok);
  assert(book.toc_count == 1U);
  assert(std::strcmp(book.toc[0].title, "Chapter One") == 0);
  assert(std::strcmp(book.toc[0].href, "c1.xhtml") == 0);
}
...
RA8_INTERNAL static void internal_test_mcdc_ncx_nav_book_null(void)
{
  ...
  const auto* ncx = reinterpret_cast<const uint8_t*>(s_ncx_doc);
  assert(priv_epub_xml_parse_ncx(ncx, std::strlen(s_ncx_doc), nullptr) == k_ra8_err_null_ptr);

  const auto* nav = reinterpret_cast<const uint8_t*>(s_nav_doc);
  assert(priv_epub_xml_parse_nav(nav, std::strlen(s_nav_doc), nullptr) == k_ra8_err_null_ptr);
}
