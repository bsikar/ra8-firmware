/**
 * @file test_epub_guards.c
 * @brief MC/DC null-guard / state-guard tests for apps/shared_libs/epub
 *
 * @details
 * Split out of test_epub.c to keep each test translation unit under the
 * repository file-size cap. This sibling owns the MC/DC vector tests for
 * the NULL-argument and open-state compound decisions across the epub
 * API; the open / chapter / TOC / metadata / cover / glyph / resource
 * contract tests stay in test_epub.c. The synthetic in-memory EPUB
...
static const float s_epub_guards_epub_render_glyph_16p0 = 16.0F;
...
/** @brief Synthetic EPUB source assets shared by the document and asset builders. */
static const struct {
...
/* --------------------------------------------------------------------- */
/* MC/DC vector tests for apps/shared_libs/epub/src/epub_chapter.c */
/* --------------------------------------------------------------------- */
 ...
 * (2 conditions, apps/shared_libs/epub/src/epub_chapter.c line 192)
 ...
 * (3 conditions, apps/shared_libs/epub/src/epub_chapter.c line 208). N+1=4.
 *
 ...
 * (2 conditions, apps/shared_libs/epub/src/epub_chapter.c line 212). N+1=3.
 *
 * @par DO-178C 6.4.4.3 omission rationale:
 ...
 *   through the public API alone because epub_open atomically sets
 *   both flags and epub_close clears both. The masking pair V1+V2
 ...
 * (2 conditions, apps/shared_libs/epub/src/epub_chapter.c line 232). N+1=3.
 ...
 * (2 conditions, apps/shared_libs/epub/src/epub_chapter.c@epub_get_toc_kind)
 * - V1 book=ok, out=ok   -> C1=F, C2=F. Decision F (proceeds).
 * - V2 book=NULL         -> C1=T short-circuits. T -> null_ptr.
 * - V3 book=ok, out=NULL -> C1=F, C2=T. T -> null_ptr.
...
RA8_INTERNAL static void internal_test_mcdc_toc_kind_null_or(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_get_toc_kind(nullptr, &kind));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_get_toc_kind(&book, nullptr));
  TEST_ASSERT_EQ(k_ra8_ok, epub_close(&book));
 ...
 * @par MC/DC:
 * Decision: `if (book == NULL || out_count == NULL)`
 * (2 conditions, apps/shared_libs/epub/src/epub_chapter.c@epub_get_toc_count)
 * - V1 book=ok, out=ok   -> C1=F, C2=F. Decision F (proceeds).
 * - V2 book=NULL         -> C1=T short-circuits. T -> null_ptr.
 * - V3 book=ok, out=NULL -> C1=F, C2=T. T -> null_ptr.
...
RA8_INTERNAL static void internal_test_mcdc_toc_count_null_or(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_get_toc_count(nullptr, &n));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_get_toc_count(&book, nullptr));
  TEST_ASSERT_EQ(k_ra8_ok, epub_close(&book));
 ...
 * (2 conditions, apps/shared_libs/epub/src/epub_chapter.c@epub_get_toc_entry)
 * - V1 book=ok, out=ok   -> C1=F, C2=F. Decision F (proceeds).
 * - V2 book=NULL         -> C1=T short-circuits. T -> null_ptr.
 * - V3 book=ok, out=NULL -> C1=F, C2=T. T -> null_ptr.
...
RA8_INTERNAL static void internal_test_mcdc_toc_entry_null_or(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_get_toc_entry(nullptr, 0U, &entry));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_get_toc_entry(&book, 0U, nullptr));
  TEST_ASSERT_EQ(k_ra8_ok, epub_close(&book));
 ...
 * (2 conditions, apps/shared_libs/epub/src/epub_chapter.c@epub_toc_entry_to_chapter)
 * - V1 book=ok, out=ok   -> C1=F, C2=F. Decision F (proceeds).
 * - V2 book=NULL         -> C1=T short-circuits. T -> null_ptr.
 * - V3 book=ok, out=NULL -> C1=F, C2=T. T -> null_ptr.
...
RA8_INTERNAL static void internal_test_mcdc_toc_to_chapter_null_or(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_toc_entry_to_chapter(nullptr, 0U, &chapter));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_toc_entry_to_chapter(&book, 0U, nullptr));
  TEST_ASSERT_EQ(k_ra8_ok, epub_close(&book));
...
RA8_INTERNAL static void internal_test_mcdc_cover_image_null_or3(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_get_cover_image(&book, nullptr, sizeof(buf), &got));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_get_cover_image(&book, buf, sizeof(buf), nullptr));
  TEST_ASSERT_EQ(k_ra8_ok, epub_close(&book));
...
RA8_INTERNAL static void internal_test_mcdc_set_font_null_or(void)
{
  ...
  /* V1 */
  TEST_ASSERT_EQ(k_ra8_ok, epub_set_font(&book, k_dummy_font, sizeof(k_dummy_font)));
  /* V2 */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_set_font(nullptr, k_dummy_font, sizeof(k_dummy_font)));
  /* V3 */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_set_font(&book, nullptr, sizeof(k_dummy_font)));
  TEST_ASSERT_EQ(k_ra8_ok, epub_close(&book));
...
RA8_INTERNAL static void internal_test_mcdc_render_glyph_null_or4(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra8_err_null_ptr,
    epub_render_glyph(nullptr, 0, s_epub_guards_epub_render_glyph_16p0, bm, sizeof(bm), &w, &h));
  /* V3 */
  TEST_ASSERT_EQ(
    k_ra8_err_null_ptr,
    epub_render_glyph(&book, 0, s_epub_guards_epub_render_glyph_16p0, nullptr, sizeof(bm), &w, &h));
  /* V4 */
  TEST_ASSERT_EQ(
    k_ra8_err_null_ptr,
    epub_render_glyph(&book, 0, s_epub_guards_epub_render_glyph_16p0, bm, sizeof(bm), nullptr, &h));
  /* V5 */
  TEST_ASSERT_EQ(
    k_ra8_err_null_ptr,
    epub_render_glyph(&book, 0, s_epub_guards_epub_render_glyph_16p0, bm, sizeof(bm), &w, nullptr));
  TEST_ASSERT_EQ(k_ra8_ok, epub_close(&book));
...
RA8_INTERNAL static void internal_test_mcdc_render_glyph_state_or(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_not_initialized,
                 ...
                 epub_render_glyph(&book,
                                   (int32_t)k_test_codepoint_a,
                                   s_epub_guards_epub_render_glyph_16p0,
                                   bm,
                                   sizeof(bm),
                                   &w,
                                   &h));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, epub_set_font(&book, k_dummy_font, sizeof(k_dummy_font)));
  const int32_t rc1 =
    (int32_t)epub_render_glyph(&book, (int32_t)k_test_codepoint_a, 16.0F, bm, sizeof(bm), &w, &h);
 ...
 * (2 conditions, apps/shared_libs/epub/src/epub_chapter.c line 64).
 * priv_join_path is static; it is invoked from epub_load_chapter
 * (line 223) and epub_get_cover_image (line 262), each of which
 ...
 * constants (full_path[k_epub_max_path_len], sizeof(full_path)).
