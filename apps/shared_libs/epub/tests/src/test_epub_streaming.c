 ...
 *     is byte-identical to the resident `epub_open()` path, and that the total
...
/** @brief Archive, cache, and document state shared across streaming phases. */
static struct {
  /** Generated streaming EPUB. */
  uint8_t archive[k_arc_cap];
  /** Finalized archive length. */
  size_t archive_size;
  /** Large unreferenced member. */
  uint8_t filler[k_filler_bytes];
  /** Page-cache bytes. */
  uint8_t frames[(size_t)k_frames * (size_t)k_frame_bytes];
  /** Frame residency metadata. */
  ra8_vmem_frame_t meta[(size_t)k_frames];
  /** Cached page keys. */
  ra8_vmem_key_t keys[(size_t)k_frames];
  /** Cache hash-chain heads. */
  int32_t buckets[(size_t)k_buckets];
  /** Bytes fetched from archive storage. */
  uint64_t storage_bytes;
  /** Rootfile locator document. */
  const char* container;
  /** Package manifest and spine. */
  const char* opf;
  /** XHTML spine documents. */
  const char* chapters[4];
  /** Cover-image payload. */
  const char* cover;
} s_fixture = {
...
{
  return (ra8_vmem_cfg_t){.frame_mem    = s_fixture.frames,
...
/** @brief Add all chapter documents to the synthetic archive. */
RA8_INTERNAL static void internal_add_chapters(mz_zip_archive* zip)
{
  ...
  for (size_t index = 0U; index < 4U; ++index) {
    TEST_ASSERT(mz_zip_writer_add_mem(zip,
                                      s_paths[index],
                                      s_fixture.chapters[index],
                                      strlen(s_fixture.chapters[index]),
                                      MZ_DEFAULT_COMPRESSION) == MZ_TRUE);
  }
}

/**
 * @brief Build the large EPUB (real skeleton + big unreferenced filler) into ::s_fixture.archive.
 ...
 * @pre ::s_fixture.filler has ::k_filler_bytes bytes.
 * @post ::s_fixture.archive_size holds the finalized archive length (> ::k_filler_bytes).
...
RA8_INTERNAL static void internal_build_big_epub(void)
{
  for (size_t i = 0U; i < (size_t)k_filler_bytes; ++i) {
    s_fixture.filler[i] = (uint8_t)((i * k_epub_filler_stride) +
  ...
  TEST_ASSERT(mz_zip_writer_add_mem(&zip,
                                    "OEBPS/cover.png",
                                    s_fixture.cover,
                                    strlen(s_fixture.cover),
                                    MZ_DEFAULT_COMPRESSION) == MZ_TRUE);
  internal_add_chapters(&zip);
...
/**
 * @brief ra8_vsource read callback over ::s_fixture.archive (the page-cache path backing).
 * @param[in]  ctx    Unused.
 ...
 * @pre `offset + len <= s_fixture.archive_size` (guaranteed by ra8_vsource_loader).
 * @pre `buf` is writable for `len` bytes.
 * @post ::s_fixture.storage_bytes advanced by `len`.
 * @note Not thread-safe.
...
{
  (void)ctx;
  (void)memcpy(buf, &s_fixture.archive[offset], (size_t)len);
  s_fixture.storage_bytes += (uint64_t)len;
  return k_ra8_ok;
}
...
/**
 * @brief Direct epub stream read over a resident buffer (no cache).
...
RA8_INTERNAL static void internal_assert_chapters(epub_book_t* book)
{
  static const char* const markers[] = {"ALPHA", "BRAVO", "CHARLIE", "DELTA"};
  for (uint16_t i = 0U; i < 4U; ++i) {
    uint8_t chbuf[k_chapter_max] = {};
    size_t  got                  = 0U;
    TEST_ASSERT_EQ(k_ra8_ok, epub_load_chapter(book, i, chbuf, sizeof(chbuf) - 1U, &got));
    TEST_ASSERT(got > 0U);
    chbuf[got] = (uint8_t)'\0';
...
internal_stream_check_same_spine(epub_book_t* book, epub_book_t* resident, uint16_t* count)
{
  uint16_t cs = 0U;
  uint16_t cr = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, epub_get_chapter_count(book, &cs));
  TEST_ASSERT_EQ(k_ra8_ok, epub_get_chapter_count(resident, &cr));
  TEST_ASSERT_EQ(4U, cs);
  TEST_ASSERT_EQ(cr, cs);

  epub_metadata_t ms = {};
  TEST_ASSERT_EQ(k_ra8_ok, epub_get_metadata(book, &ms));
...
RA8_INTERNAL static void
internal_stream_check_chapters_identical(epub_book_t* book, epub_book_t* resident, uint16_t count)
{
  for (uint16_t i = 0U; i < count; ++i) {
    ...
    size_t  ga               = 0U;
    size_t  gb               = 0U;
    TEST_ASSERT_EQ(k_ra8_ok, epub_load_chapter(book, i, a, sizeof(a), &ga));
    TEST_ASSERT_EQ(k_ra8_ok, epub_load_chapter(resident, i, b, sizeof(b), &gb));
...
RA8_INTERNAL static void internal_stream_check_direct_io_bounded(void)
{
  ...
  TEST_ASSERT((size_t)(k_frame_bytes * 16U) < s_fixture.archive_size);
}

/**
 * @test internal_test_stream_direct_parse_equivalence
 * @brief `epub_open_streamed()` over a seek+read callback parses byte-identically
 *        to `epub_open()` on the same archive, while fetching far fewer bytes
...
RA8_INTERNAL static void internal_test_stream_direct_parse_equivalence(void)
{
  ...
  epub_book_t         book  = {};
  TEST_ASSERT_EQ(k_ra8_ok, epub_open_streamed(&media, "big.epub", &book));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, epub_close(&resident));
...
RA8_INTERNAL static void internal_stream_check_bounded_invariant(ra8_vmem_t* vm)
{
  ...
  TEST_ASSERT(((size_t)budget * (size_t)k_budget_ratio) <= s_fixture.archive_size);
  /* 3. The whole archive was never read: fewer bytes fetched than the filler holds. */
  TEST_ASSERT(s_fixture.storage_bytes < (uint64_t)k_filler_bytes);
...
RA8_INTERNAL static void internal_test_stream_via_pagecache_bounded(void)
{
  ...
  epub_stream_media_t media = {.read = ra8_vmem_stream_read,
                               .ctx  = &st,
                               .size = (uint64_t)s_fixture.archive_size};
  epub_book_t         book  = {};
  TEST_ASSERT_EQ(k_ra8_ok, epub_open_streamed(&media, "big.epub", &book));
  ...
  size_t  cover_got          = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, epub_get_resource(&book, "cover.png", cover, sizeof(cover), &cover_got));
  TEST_ASSERT_EQ(strlen(s_fixture.cover), cover_got);
  TEST_ASSERT(internal_bytes_equal(cover, s_fixture.cover, cover_got));
 ...
 * @brief `epub_open_streamed()` rejects NULL/empty media and a non-ZIP stream.
 *
 * @par MC/DC:
...
RA8_INTERNAL static void internal_test_stream_open_arg_guards(void)
{
  ...
  epub_book_t         book = {};
  buf_src_t           src  = {.data = s_fixture.archive, .size = s_fixture.archive_size};
  epub_stream_media_t good = {.read = internal_direct_read,
                              .ctx  = &src,
                              .size = (uint64_t)s_fixture.archive_size};

  /* NULL-OR guard, each operand flipped. */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_open_streamed(nullptr, "x", &book));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_open_streamed(&good, "x", nullptr));

  /* invalid-arg OR guard, each operand flipped. */
  epub_stream_media_t no_read = {.read = nullptr,
                                 .ctx  = &src,
                                 .size = (uint64_t)s_fixture.archive_size};
  epub_stream_media_t no_size = {.read = internal_direct_read, .ctx = &src, .size = 0U};
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, epub_open_streamed(&no_read, "x", &book));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, epub_open_streamed(&no_size, "x", &book));
  ...
  epub_stream_media_t  junk     = {.read = internal_direct_read,
                                   .ctx  = &jsrc,
                                   .size = (uint64_t)sizeof(k_junk)};
  TEST_ASSERT_EQ(k_ra8_err_validation_failed, epub_open_streamed(&junk, "x", &book));
