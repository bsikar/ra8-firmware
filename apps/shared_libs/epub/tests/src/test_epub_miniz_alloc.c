...
typedef struct {
  epub_miniz_arena_t     arena;     /**< Allocator descriptor.  */
  epub_miniz_workspace_t workspace; /**< Aligned backing bytes. */
} priv_fixture_t;

/** @brief Copy a pointer representation into an integer address. */
RA8_INTERNAL static uintptr_t internal_pointer_address(const void* pointer)
{
...
/** @brief Copy an integer address representation into an object pointer. */
RA8_INTERNAL static void* internal_address_pointer(uintptr_t address)
{
  void* pointer = nullptr;
  static_assert(sizeof(address) == sizeof(pointer), "uintptr_t must represent object pointers");
  (void)memcpy((void*)&pointer, (const void*)&address, sizeof(pointer));
  return pointer;
}

/**
 * @brief Report whether an arena descriptor still holds its saved fields.
 * @details Member-wise rather than byte-wise: ::epub_miniz_arena_t ends in
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static bool internal_arena_unchanged(const epub_miniz_arena_t* arena,
                                                  const epub_miniz_arena_t* saved)
{
...
static void internal_fixture_init(priv_fixture_t* fixture)
{
  *fixture = (priv_fixture_t){};
  TEST_ASSERT_EQ(k_ra8_ok,
                 epub_miniz_arena_init(&fixture->arena,
...
RA8_INTERNAL static void internal_test_alloc_align_and_distinct(void)
{
  ...
  void* a = epub_miniz_alloc(&fixture.arena, 1U, k_small);
  void* b = epub_miniz_alloc(&fixture.arena, 1U, k_small);
  TEST_ASSERT((a != nullptr) && (b != nullptr));
  TEST_ASSERT(a != b);
  TEST_ASSERT((internal_pointer_address(a) % alignof(max_align_t)) == 0U);
  TEST_ASSERT((internal_pointer_address(b) % alignof(max_align_t)) == 0U);
  ...
  epub_miniz_free(&fixture.arena, a);
  epub_miniz_free(&fixture.arena, b);
...
RA8_INTERNAL static void internal_test_free_coalesce_reclaim(void)
{
  ...
  for (uint32_t i = 0U; i < 8U; i++) {
    p[i] = epub_miniz_alloc(&fixture.arena, 1U, k_medium);
    TEST_ASSERT(p[i] != nullptr);
  }
  for (uint32_t i = 0U; i < 8U; i++) {
    epub_miniz_free(&fixture.arena, p[i]);
  }
  /* If coalescing works, a single big alloc near the pool size succeeds. */
  void* big = epub_miniz_alloc(&fixture.arena, 1U, (size_t)k_epub_miniz_pool_bytes / 2U);
  TEST_ASSERT(big != nullptr);
  epub_miniz_free(&fixture.arena, big);
...
RA8_INTERNAL static void internal_test_realloc_grow_preserves(void)
{
  ...
  priv_fixture_t fixture;
  internal_fixture_init(&fixture);
  uint8_t* a = (uint8_t*)epub_miniz_alloc(&fixture.arena, 1U, k_small);
  ...
  void* pin = epub_miniz_alloc(&fixture.arena, 1U, k_small);
  TEST_ASSERT(pin != nullptr);
  uint8_t* g = (uint8_t*)epub_miniz_realloc(&fixture.arena, a, 1U, k_big);
  TEST_ASSERT(g != nullptr);
  for (uint32_t i = 0U; i < (uint32_t)k_small; i++) {
    TEST_ASSERT_EQ((i & 0xFFU), g[i]);
  }
  epub_miniz_free(&fixture.arena, g);
  epub_miniz_free(&fixture.arena, pin);
 ...
 * @par MC/DC:
 * (no compound decision is authored in this test's assertions -- they are single
 * equality / null checks. It drives epub_miniz_realloc's single-condition
...
RA8_INTERNAL static void internal_test_realloc_inplace_and_null_zero(void)
{
  ...
  priv_fixture_t fixture;
  internal_fixture_init(&fixture);
  void* a = epub_miniz_alloc(&fixture.arena, 1U, k_medium);
  TEST_ASSERT(a != nullptr);
  /* Shrink request fits in place -> same pointer. */
  void* same = epub_miniz_realloc(&fixture.arena, a, 1U, k_small);
  TEST_ASSERT_EQ(a, same);
  /* realloc(NULL, n) == alloc(n). */
  void* fresh = epub_miniz_realloc(&fixture.arena, nullptr, 1U, k_small);
  TEST_ASSERT(fresh != nullptr);
  /* realloc(p, 0) frees and returns NULL. */
  void* none = epub_miniz_realloc(&fixture.arena, fresh, 0U, 0U);
  TEST_ASSERT(none == nullptr);
  epub_miniz_free(&fixture.arena, same);
 ...
 * epub_miniz_alloc.c first-fit). N+1 = 3 vectors:
 ...
 * epub_miniz_alloc.c internal_coalesce). N+1 = 3 vectors:
 ...
 * epub_miniz_alloc overflow guard). N+1 = 3 vectors:
...
RA8_INTERNAL static void internal_test_overflow_mcdc(void)
{
  ...
  TEST_ASSERT(epub_miniz_alloc(&fixture.arena, (SIZE_MAX / 2U) + 2U, 2U) == nullptr);
 ...
 *
 * @par MC/DC:
 * Drives the *production* decisions in epub_miniz_alloc() (not a mirror).
 *
...
RA8_INTERNAL static void internal_test_alloc_real_overflow_and_firstfit_mcdc(void)
{
  ...
  void* z = epub_miniz_alloc(&fixture.arena, k_arena_alloc_items, 0U);
  TEST_ASSERT(z != nullptr);
  epub_miniz_free(&fixture.arena, z);
  ...
  void* b = epub_miniz_alloc(&fixture.arena, 1U, k_small);
  TEST_ASSERT((a != nullptr) && (b != nullptr));
  epub_miniz_free(&fixture.arena, a); /* front block now free but only k_small big */
  /* k_medium > k_small */
  void* c = epub_miniz_alloc(&fixture.arena, 1U, k_medium);
  TEST_ASSERT(c != nullptr);
  TEST_ASSERT(c != b); /* did not reuse the too-small freed block */
  epub_miniz_free(&fixture.arena, b);
  epub_miniz_free(&fixture.arena, c);
 ...
 *
 * @par MC/DC:
 * Drives the *production* guard in epub_miniz_free()
...
RA8_INTERNAL static void internal_test_free_in_pool_mcdc(void)
{
  ...
  priv_fixture_t fixture;
  internal_fixture_init(&fixture);
  uint8_t* p = (uint8_t*)epub_miniz_alloc(&fixture.arena, 1U, k_small);
  ...
  void* const below = internal_address_pointer(internal_pointer_address(p) -
                                               (2U * (uintptr_t)k_epub_miniz_pool_bytes));
  epub_miniz_free(&fixture.arena, below);
  ...
  void* const above = internal_address_pointer(internal_pointer_address(p) +
                                               (2U * (uintptr_t)k_epub_miniz_pool_bytes));
  epub_miniz_free(&fixture.arena, above);
  /* V2: NULL address short-circuits the OR -> no-op. */
  epub_miniz_free(&fixture.arena, nullptr);
  ...
  epub_miniz_free(&fixture.arena, p); /* V1: in-pool, non-NULL -> actually freed */
 ...
 *
 * @par MC/DC:
 * Drives the *production* overflow guard in epub_miniz_realloc()
...
RA8_INTERNAL static void internal_test_realloc_real_overflow_mcdc(void)
{
  ...
  void* nomem = epub_miniz_realloc(&fixture.arena, p, (SIZE_MAX / 2U) + 2U, 2U);
  ...
  uint8_t* g = (uint8_t*)epub_miniz_realloc(&fixture.arena, p, 1U, k_medium);
  TEST_ASSERT(g != nullptr);
  ...
  void* none = epub_miniz_realloc(&fixture.arena, g, 4U, 0U);
  TEST_ASSERT(none == nullptr);
 ...
 *
 * @par MC/DC:
 * Decision: `if ((items * size) > k_epub_miniz_pool_bytes)` (1 condition;
 * the oversize guard in both epub_miniz_alloc and _realloc). It rejects any
...
RA8_INTERNAL static void internal_test_alloc_oversize_rejected_mcdc(void)
{
  ...
  TEST_ASSERT(epub_miniz_alloc(&fixture.arena, 1U, SIZE_MAX - 10U) == nullptr);
  /* V2: an ordinary just-over-pool size. */
  TEST_ASSERT(epub_miniz_alloc(&fixture.arena, 1U, (size_t)k_epub_miniz_pool_bytes + 1U) ==
              nullptr);
  /* V3: realloc growing past the pool is rejected; the old block is untouched. */
  uint8_t* a = (uint8_t*)epub_miniz_alloc(&fixture.arena, 1U, k_small);
  TEST_ASSERT(a != nullptr);
  a[0] = k_arena_first_byte;
  TEST_ASSERT(epub_miniz_realloc(&fixture.arena, a, 1U, SIZE_MAX - 10U) == nullptr);
...
RA8_INTERNAL static void internal_test_arena_capacity_and_failure_preservation(void)
{
  ...
  epub_miniz_workspace_t workspace;
  (void)memset(&workspace, k_miniz_fill_reuse, sizeof(workspace));
  epub_miniz_arena_t arena = {
    .base        = &workspace.bytes[7],
    .capacity    = 31U,
    .initialized = k_arena_alloc_items,
  };
  const epub_miniz_arena_t before = arena;
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 epub_miniz_arena_init(&arena, &workspace.bytes[0], sizeof(workspace.bytes) - 1U));
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 epub_miniz_arena_init(&arena, &workspace.bytes[0], sizeof(workspace.bytes)));
  TEST_ASSERT_EQ(k_epub_miniz_pool_bytes, arena.capacity);
  TEST_ASSERT_EQ(1U, arena.initialized);
  void* const block = epub_miniz_alloc(&arena, 1U, k_small);
  TEST_ASSERT(block != nullptr);
  epub_miniz_free(&arena, block);
  epub_miniz_arena_deinit(&arena);
  TEST_ASSERT(arena.base == nullptr);
  TEST_ASSERT_EQ(0U, arena.capacity);
  TEST_ASSERT_EQ(0U, arena.initialized);
  TEST_ASSERT(epub_miniz_alloc(&arena, 1U, k_small) == nullptr);
 ...
 * @par MC/DC:
 * Decision identity:
 * `apps/shared_libs/epub/src/epub_miniz_alloc.c@epub_miniz_arena_init`.
...
RA8_INTERNAL static void internal_test_arena_alignment_and_null_guards(void)
{
  ...
  alignas(max_align_t) uint8_t raw[k_epub_miniz_pool_bytes + alignof(max_align_t)];
  epub_miniz_arena_t           arena  = {.base = &raw[0], .capacity = 17U, .initialized = 7U};
  const epub_miniz_arena_t     before = arena;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_miniz_arena_init(nullptr, &raw[0], sizeof(raw)));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, epub_miniz_arena_init(&arena, nullptr, sizeof(raw)));
  TEST_ASSERT(internal_arena_unchanged(&arena, &before));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 epub_miniz_arena_init(&arena, &raw[1], (size_t)k_epub_miniz_pool_bytes));
  TEST_ASSERT(internal_arena_unchanged(&arena, &before));
  TEST_ASSERT(epub_miniz_alloc(nullptr, 1U, k_small) == nullptr);
  epub_miniz_free(nullptr, &raw[0]);
  epub_miniz_arena_deinit(nullptr);
...
RA8_INTERNAL static void internal_test_independent_arenas_teardown_reuse(void)
{
  ...
  uint8_t* const a = (uint8_t*)epub_miniz_alloc(&first.arena, 1U, k_medium);
  uint8_t* const b = (uint8_t*)epub_miniz_alloc(&second.arena, 1U, k_medium);
  TEST_ASSERT((a != nullptr) && (b != nullptr));
  TEST_ASSERT(a != b);
  (void)memset(a, k_miniz_fill_first, k_medium);
  (void)memset(b, k_miniz_fill_second, k_medium);

  epub_miniz_arena_deinit(&first.arena);
  TEST_ASSERT_EQ(
    k_ra8_ok,
    epub_miniz_arena_init(&first.arena, &first.workspace.bytes[0], sizeof(first.workspace.bytes)));
  TEST_ASSERT_EQ(k_miniz_fill_second, b[0]);
  TEST_ASSERT_EQ(k_miniz_fill_second, b[k_medium - 1U]);
  uint8_t* const reused = (uint8_t*)epub_miniz_alloc(&first.arena, 1U, k_medium);
  TEST_ASSERT(reused != nullptr);
  (void)memset(reused, k_miniz_fill_reuse, k_medium);
  TEST_ASSERT_EQ(k_miniz_fill_second, b[k_medium / 2U]);
  epub_miniz_free(&first.arena, reused);
  epub_miniz_free(&second.arena, b);
