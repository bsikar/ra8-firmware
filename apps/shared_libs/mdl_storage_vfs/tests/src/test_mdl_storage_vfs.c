...
{
  static uint8_t s_disk_bytes[(size_t)k_disk_blocks * k_ra8_io_block_size_bytes];
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    ra8_io_blockdev_ram_init(&s_blockdev, &s_ram_state, s_disk_bytes, k_disk_blocks, false));
...
internal_compare_file(const char* path, const uint8_t* expected, uint32_t expected_len)
{
  ...
  uint32_t offset                   = 0U;
  while ((offset < expected_len) && (err == k_ra8_ok)) {
    ...
    if (err == k_ra8_ok) {
      ...
      } else if (memcmp(buf, &expected[offset], want) != 0) {
 ...
 */
RA8_INTERNAL static void internal_bind_adapter(mdl_storage_vfs_t*       storage,
                                               ra8_mdl_storage_iface_t* iface,
                                               validator_t*             validator)
{
  const mdl_storage_vfs_validate_fn validate =
    (validator == nullptr) ? nullptr : internal_validate_artifact;
  const mdl_storage_vfs_config_t config = {
    .stage_leaf   = "MDLSTAGE.TMP",
    .validate     = validate,
    .validate_ctx = validator,
  };
  TEST_ASSERT_EQ(k_ra8_ok, mdl_storage_vfs_init(storage, &config, iface));
}
...
RA8_INTERNAL static void internal_test_init_guards(void)
{
  ...
  ra8_mdl_storage_iface_t  iface   = {};
  mdl_storage_vfs_config_t config  = {.stage_leaf = "MDLSTAGE.TMP"};
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, mdl_storage_vfs_init(nullptr, &config, &iface));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, mdl_storage_vfs_init(&storage, nullptr, &iface));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, mdl_storage_vfs_init(&storage, &config, nullptr));
  config.stage_leaf = nullptr;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, mdl_storage_vfs_init(&storage, &config, &iface));
  config.stage_leaf = ".";
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, mdl_storage_vfs_init(&storage, &config, &iface));
  ...
  config.stage_leaf = "BAD:LEAF.TMP";
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, mdl_storage_vfs_init(&storage, &config, &iface));
  char control_leaf[sizeof("STXGE.TMP")] = {};
  (void)memcpy(control_leaf, "STXGE.TMP", sizeof(control_leaf));
  control_leaf[k_leaf_control_index] = (char)k_control_byte;
  config.stage_leaf                  = control_leaf;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, mdl_storage_vfs_init(&storage, &config, &iface));
  char unterminated[k_mdl_storage_vfs_stage_leaf_capacity];
  (void)memset(unterminated, 'A', sizeof(unterminated));
  config.stage_leaf = unterminated;
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, mdl_storage_vfs_init(&storage, &config, &iface));
...
RA8_INTERNAL static void internal_test_path_and_existing_final_guards(void)
{
  ...
  char too_long[k_mdl_storage_vfs_path_capacity + 1U];
  ...
  too_long[3]                               = ':';
  too_long[4]                               = '/';
  too_long[k_mdl_storage_vfs_path_capacity] = '\0';
...
RA8_INTERNAL static void internal_test_path_grammar_guards(void)
{
  ...
  char control_mount[sizeof("raXm:/X.RBK")] = {};
  (void)memcpy(control_mount, "raXm:/X.RBK", sizeof(control_mount));
  ...
  char control_component[sizeof("ram:/AXB.RBK")] = {};
  (void)memcpy(control_component, "ram:/AXB.RBK", sizeof(control_component));
  ...
  char deep[k_mdl_storage_vfs_path_capacity];
...
RA8_INTERNAL static void internal_test_state_machine_guards(void)
{
  ...
  TEST_ASSERT_EQ(0U, written);
  storage.state = k_mdl_storage_vfs_writing;
  TEST_ASSERT_EQ(k_ra8_err_invalid_state,
                 iface.write(iface.ctx, payload, sizeof(payload), &written));
  storage.state = k_mdl_storage_vfs_idle;
...
RA8_INTERNAL static void internal_test_unrepresentable_stage_leaf(void)
{
  ...
  internal_setup_volume();
  mdl_storage_vfs_t       control_storage = {};
  ...
  char leaf[sizeof("STXGE.TMP")] = {};
  (void)memcpy(leaf, "STXGE.TMP", sizeof(leaf));
  leaf[k_leaf_control_index]             = (char)k_high_byte;
  mdl_storage_vfs_config_t configuration = {.stage_leaf = leaf};
  mdl_storage_vfs_t        storage       = {};
  ra8_mdl_storage_iface_t  iface         = {};
  TEST_ASSERT_EQ(k_ra8_ok, mdl_storage_vfs_init(&storage, &configuration, &iface));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, iface.begin(iface.ctx, "ram:/LEAF.RBK"));
  TEST_ASSERT_EQ(k_mdl_storage_vfs_idle, storage.state);
