...
{
  uint16_t  len = 0U;
  ra8_err_t err = internal_bounded_length(path, k_mdl_storage_vfs_path_capacity, &len);
  if (err != k_ra8_ok) {
    return err;
  }
  uint16_t colon = 0U;
...
{
  ra8_err_t err = internal_bounded_length(leaf, k_mdl_storage_vfs_stage_leaf_capacity, out_len);
  if (err != k_ra8_ok) {
    return err;
  }
  if (*out_len == 0U) {
...
{
  for (uint16_t i = 0U; i < k_mdl_storage_vfs_stage_leaf_capacity; ++i) {
 ...
 */
RA8_INTERNAL static ra8_err_t
internal_build_stage(mdl_storage_vfs_t* ctx, const path_facts_t* facts, uint16_t stage_len)
{
  const uint32_t prefix = (uint32_t)facts->last_slash + 1U;
  const uint32_t needed = prefix + (uint32_t)stage_len + 1U;
  if (needed > (uint32_t)k_mdl_storage_vfs_path_capacity) {
    return k_ra8_err_invalid_size;
  }
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_parent_check(mdl_storage_vfs_t* ctx, uint16_t last_slash)
{
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_close_writer(mdl_storage_vfs_t* ctx)
{
  if (ctx->file == nullptr) {
    return k_ra8_ok;
  }
  ...
  ctx->file                     = nullptr;
  ctx->state                    = k_mdl_storage_vfs_staged;
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_begin_prepare_stage(mdl_storage_vfs_t*  ctx,
 ...
 * @param[in,out] opaque Bound ::mdl_storage_vfs_t context.
...
RA8_INTERNAL static ra8_err_t internal_begin(void* opaque, const char* destination)
{
  ...
  mdl_storage_vfs_t* const ctx = opaque;
  if (ctx->state != k_mdl_storage_vfs_idle) {
    if (ctx->state != k_mdl_storage_vfs_committed) {
      return k_ra8_err_invalid_state;
    }
  }
  ctx->state         = k_mdl_storage_vfs_idle;
  path_facts_t facts = {};
  ...
  if (err == k_ra8_ok) {
    ...
    ctx->state         = k_mdl_storage_vfs_writing;
  }
  return err;
}
...
internal_write(void* opaque, const uint8_t* data, uint16_t len, uint16_t* written)
{
  ...
  *written = 0U;
  ...
  mdl_storage_vfs_t* const ctx = opaque;
  if (ctx->state != k_mdl_storage_vfs_writing) {
    return k_ra8_err_invalid_state;
  }
  if (ctx->file == nullptr) {
    return k_ra8_err_invalid_state;
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_staged_file_check(const mdl_storage_vfs_t* ctx,
                                                         uint64_t                 expected)
{
  ra8_io_vfs_stat_t stat = {};
...
internal_validate(void* opaque, uint64_t total_bytes, const uint8_t sha256[k_ra8_mdl_sha256_bytes])
{
  ...
  if (err == k_ra8_ok) {
    ctx->state = k_mdl_storage_vfs_ready;
  }
  return err;
}
...
RA8_INTERNAL static ra8_err_t internal_commit(void* opaque)
{
  ...
  if (ctx->state != k_mdl_storage_vfs_ready) {
    return k_ra8_err_invalid_state;
  }
  ...
  if (err == k_ra8_ok) {
    ctx->state = k_mdl_storage_vfs_committed;
  }
  return err;
}
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_abort_stage(mdl_storage_vfs_t* ctx, ra8_err_t first_err)
{
  ra8_io_vfs_stat_t stat     = {};
