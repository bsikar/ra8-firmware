 ...
 * the only format that reaches into the firmware's `jof` producer --
 ...
 * `longstrip` engine scrolls a viewport at a time.
 *
 ...
 *          full-width horizontal band -- the shape the longstrip engine
                                                      ...
                                                      mdl_export_workspace_t* ws)
{
  ...
  *out_cap  = 0U;
  ...
  const uint32_t need = jof_webp_work_bytes(w, h, (uint32_t)slen);
 ...
 * @param[in]     work_cap Work-arena size, from ::jof_work_bytes.
                                                        ...
                                                        mdl_export_workspace_t* ws)
{
  ...
  jof_produce_cfg_t cfg  = {.pull          = internal_jof_pull,
                            .pull_ctx      = &pull,
  ...
  jof_info_t        info = {};
  const ra8_err_t   rc   = jof_produce(&cfg, &info);
  return rc;
}
                                               ...
                                               mdl_export_workspace_t* ws)
{
  ...
  if (jof_probe_dims(src, slen, &w, &h) != k_ra8_ok) {
  ...
  mdl_export_output_t output = {};
  rc = priv_mdl_export_output_begin(&output, storage, out_path, k_mdl_format_jof);
  if (rc == k_ra8_ok) {
