...
{
  combined_rel[0] = '\0';
  if (r->update || !r->combine || (r->format == k_mdl_format_loose)) {
...
RA8_INTERNAL static size_t internal_export_after(const mdl_series_run_t*  r,
                                                 ...
                                                 int64_t                  run_start)
{
  if (r->format == k_mdl_format_loose) {
    return 0U;
  }
...
RA8_INTERNAL static void internal_report_cover_failure(ra8_err_t error)
{
  ra8_err_t output_error = priv_mdl_stream_text(k_ra8_ok,
                                                priv_mdl_app_context()->diagnostic,
                                                "mdl: series cover acquisition failed "
                                                "(err 0x");
  output_error =
  ...
  if (!r->update) {
    ...
    *out_sel = &s_selected;
    if ((*out_sel)->count == 0U) {
      (void)priv_mdl_stream_text(k_ra8_ok,
                                 priv_mdl_app_context()->diagnostic,
                                 "mdl: no chapters match the requested range\n");
                                              ...
                                              mdl_cache_t*            cache)
{
  ...
  if (internal_save_series_state(state_path, "mdl: failed to checkpoint series metadata '") !=
      k_ra8_ok) {
    return 1;
  }
  ...
  const ra8_err_t src =
    internal_save_series_state(state_path, "mdl: failed to save library state '");
                                                         ...
                                                         const series_prior_metadata_t* prior)
{
  ...
  if (!app->series_metadata.title_selected && (prior->title[0] != '\0')) {
    ...
    if (priv_mdl_stream_text(k_ra8_ok,
                             priv_mdl_app_context()->diagnostic,
                             "mdl: WARNING: preserving recorded series title after "
                             "selector failure\n") != k_ra8_ok) {
      return false;
    }
  }
...
internal_run_series_paths(const mdl_series_run_t* run, const mdl_site_t* site, mdl_cache_t* cache)
{
  ...
  if (load_rc != k_ra8_ok) {
    ra8_err_t output_error = internal_series_text3(priv_mdl_app_context()->diagnostic,
                                                   "mdl: refusing to overwrite unreadable "
                                                   "state '",
                                                   state_path,
                                                   "' (err 0x");
    output_error =
  ...
  if (!internal_reconcile_series_state(run, site, &prior)) {
    (void)priv_mdl_stream_text(k_ra8_ok,
                               priv_mdl_app_context()->diagnostic,
                               "mdl: scraped series metadata exceeded state bounds\n");
    return 1;
  }
...
RA8_INTERNAL static int internal_run_series_network(const mdl_series_run_t* run, mdl_site_t* site)
{
  ...
  if (priv_mdl_app_prepare_chapters(site, run->series_url, run->timeout, &cache) != k_ra8_ok) {
    (void)priv_mdl_stream_text(k_ra8_ok,
                               priv_mdl_app_context()->diagnostic,
                               "mdl: series fetch failed\n");
  } else if (priv_mdl_app_context()->chapters.count == 0U) {
    (void)priv_mdl_stream_text(k_ra8_ok,
                               priv_mdl_app_context()->diagnostic,
                               "mdl: no chapters (check chapter_url_contains)\n");
  } else {
