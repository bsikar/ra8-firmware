...
int mdl_app_run_init_site(const char* url, const char* descriptor_dir)
{
  ...
  if (priv_mdl_app_storage_ensure_directory(&priv_mdl_app_context()->storage, descriptor_dir) !=
      k_ra8_ok) {
    internal_admin_text3(priv_mdl_app_context()->diagnostic,
                         "mdl: descriptor directory is unsafe '",
                         descriptor_dir,
                         "'\n");
    return 1;
  }
  ...
  if (published == k_ra8_err_exists) {
    internal_admin_text3(priv_mdl_app_context()->diagnostic,
                         "mdl: refusing to overwrite existing site descriptor '",
                         out_path,
                         "'\n");
    return 1;
  }
  if (published != k_ra8_ok) {
    internal_admin_text3(priv_mdl_app_context()->diagnostic,
                         "mdl: failed to publish site descriptor '",
                         out_path,
                         "'\n");
    return 1;
  }
...
int mdl_app_run_verify(const char* target_dir)
{
  ...
  if (!any_target) {
    internal_admin_text3(priv_mdl_app_context()->diagnostic,
                         "mdl: no tracked series or recognized artifacts under '",
                         dir,
                         "'\n");
    if (priv_mdl_app_context()->io_error != k_ra8_ok) {
      return 1;
