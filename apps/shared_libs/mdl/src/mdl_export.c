...
mdl_format_t mdl_format_from_str(const char* s)
{
  if ((s == nullptr) || (strcmp(s, "loose") == 0)) {
    return k_mdl_format_loose;
  }
  if (strcmp(s, "cbz") == 0) {
    return k_mdl_format_cbz;
  }
  if (strcmp(s, "cbt") == 0) {
    return k_mdl_format_cbt;
  }
  if (strcmp(s, "cbt.gz") == 0) {
    return k_mdl_format_cbt_gz;
  }
  if (strcmp(s, "epub") == 0) {
    return k_mdl_format_epub;
  }
  if (strcmp(s, "jof") == 0) {
    return k_mdl_format_jof;
  }
  if (strcmp(s, "rabook") == 0) {
    return k_mdl_format_rabook;
  }
  return k_mdl_format_invalid;
}
                                     ...
                                     mdl_export_workspace_t*  ws)
{
  if ((storage == nullptr) || (chapter_dir == nullptr) || (out_path == nullptr) ||
      (fmt == k_mdl_format_loose) || (fmt == k_mdl_format_invalid) || (ws == nullptr) ||
      (ws->data == nullptr)) {
    return k_ra8_err_invalid_arg;
  }
  ws->used = 0U;
  if ((fmt == k_mdl_format_cbr) || (fmt == k_mdl_format_cbt_xz)) {
    return k_ra8_err_not_supported;
  }
