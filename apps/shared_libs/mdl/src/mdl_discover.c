...
internal_discover_url(const mdl_discover_req_t* req, char* out, size_t cap, int* rc)
{
  *rc = 2;
  if (req->mode == k_mdl_discover_browse) {
    if (req->site->browse_url[0] == '\0') {
      internal_discover_text3(req,
                              req->diagnostic,
                              "mdl: this site descriptor has no browse_url; browsing is ",
                              "",
                              "unavailable\n");
      return false;
    }
    ...
    if ((n < 0) || ((size_t)n >= cap)) {
      internal_discover_text3(req, req->diagnostic, "mdl: browse_url is too long\n", "", "");
      return false;
    }
    return true;
  }
  if (req->site->search_url[0] == '\0') {
    internal_discover_text3(req,
                            req->diagnostic,
                            "mdl: this site descriptor has no search_url; searching is ",
                            "",
                            "unavailable\n");
    return false;
  }
  if ((req->term == nullptr) || (req->term[0] == '\0')) {
    internal_discover_text3(req, req->diagnostic, "mdl: --search needs a non-empty TERM\n", "", "");
    return false;
  }
  ...
  if (!mdl_query_encode(req->term, encoded, sizeof(encoded))) {
    internal_discover_text3(req,
                            req->diagnostic,
                            "mdl: search term is too long to encode\n",
                            "",
                            "");
    return false;
  }
...
RA8_INTERNAL static void internal_print_zero(const mdl_discover_req_t* req)
{
  const char* pat = (req->site->search_result_contains[0] != '\0')
                      ...
                      : "(none configured)";
  if (req->mode == k_mdl_discover_search) {
    ra8_err_t error = priv_mdl_stream_text(k_ra8_ok, req->diagnostic, "mdl: no results for '");
    ...
    ra8_err_t error = priv_mdl_stream_text(k_ra8_ok, req->diagnostic, "mdl: nothing to browse on ");
...
RA8_INTERNAL static void internal_print_markup_changed(const mdl_discover_req_t* req)
{
  internal_discover_text3(
    req,
    req->diagnostic,
    "mdl: could not read a results page from ",
    req->site->host,
                                             ...
                                             size_t                    out_cap)
{
  ...
  if ((n < 0) || ((size_t)n >= out_cap)) {
    internal_discover_text3(req,
                            req->diagnostic,
                            "mdl: selected URL is too long to hand off\n",
                            "",
                            "");
    if (out_cap > 0U) {
      out_url[0] = '\0';
                                                  ...
                                                  size_t                    out_cap)
{
  ...
  if ((out_url == nullptr) || (out_cap == 0U)) {
    internal_discover_text3(req,
                            req->diagnostic,
                            "mdl: --pick needs an output buffer to hand off the selection\n",
                            "",
                            "");
    return (*req->io_error == k_ra8_ok) ? 2 : 1;
  }
  if (outcome != k_mdl_search_have_results) {
    ra8_err_t error = priv_mdl_stream_text(k_ra8_ok, req->diagnostic, "mdl: --pick ");
  ...
  if ((req == nullptr) || (req->session == nullptr) || (req->gov == nullptr) ||
    ...
    if ((req != nullptr) && (req->diagnostic != nullptr)) {
      (void)priv_mdl_stream_text(k_ra8_ok,
                                 req->diagnostic,
                                 "mdl: discovery called with missing dependencies\n");
    }
    return 2;
  }
