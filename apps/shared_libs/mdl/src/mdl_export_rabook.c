...
typedef struct {
  epub_book_t*                     book;        /**< Streamed intermediate EPUB.  */
...
{
  profile->book =
    mdl_export_workspace_take(workspace, sizeof(*profile->book), _Alignof(epub_book_t));
  ...
  profile->pipeline    = (ra8_rabook_pipeline_scratch_t){.xhtml          = xhtml,
                                                         ...
                                                         .pixel_format   = k_book_pixfmt_gray4,
                                                         .xml_workspace  = xml};
...
internal_close_compile_sources(ra8_err_t error, epub_book_t* book, mdl_rabook_epub_source_t* source)
{
  ra8_err_t result = error;
  if ((book != nullptr) && (book->in_use != 0U)) {
    const ra8_err_t closed = epub_close(book);
 ...
 * @pre @p blob has already passed ::book_validate.
...
RA8_INTERNAL static ra8_err_t internal_check_compiled_page_count(const void* blob,
                                                                 size_t      page_count)
{
  const book_header_t* header         = book_header(blob);
                                                    ...
                                                    mdl_rabook_flat_source_t* flat)
{
  ...
  epub_stream_media_t media = {.read = priv_mdl_rabook_epub_read,
                               .ctx  = &source,
                               .size = source.size_bytes};
  if (error == k_ra8_ok) {
    error = epub_open_streamed(&media, temp_path, profile->book);
  }
  const void* blob   = nullptr;
  uint32_t    length = 0U;
  if (error == k_ra8_ok) {
    error = rabook_compile_from_epub_to_buffer(profile->book,
                                               &profile->builder,
                                               &profile->pipeline,
                                               &blob,
                                               &length);
  ...
  if (error == k_ra8_ok) {
    error = book_validate(blob, length);
  }
  if (error == k_ra8_ok) {
                                                      ...
                                                      mdl_export_workspace_t*   workspace)
{
  mdl_export_output_t output = {};
  ra8_err_t           error =
    priv_mdl_export_output_begin(&output, storage, destination, k_mdl_format_rabook);
