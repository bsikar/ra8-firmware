...
ra8_err_t mdl_format_from_path(const char* path, mdl_format_t* out_format)
{
  ...
  } formats[] = {{".cbt.gz", k_mdl_format_cbt_gz},
                 {".rabook", k_mdl_format_rabook},
                 {".epub", k_mdl_format_epub},
                 {".cbz", k_mdl_format_cbz},
                 {".cbt", k_mdl_format_cbt},
                 {".jof", k_mdl_format_jof}};
  ...
  *out_format = k_mdl_format_invalid;
  return k_ra8_err_not_supported;
}

bool mdl_format_is_verifiable(mdl_format_t format)
{
  return (format == k_mdl_format_cbz) || (format == k_mdl_format_cbt) ||
         (format == k_mdl_format_cbt_gz) || (format == k_mdl_format_epub) ||
         (format == k_mdl_format_jof) || (format == k_mdl_format_rabook);
}
...
internal_zip_semantics(mdl_format_t format, const mdl_zip_scan_t* scan, mdl_verify_report_t* report)
{
  ...
  if ((format == k_mdl_format_cbz) && !scan->comicinfo) {
    return k_ra8_err_validation_failed;
  }
  if ((format == k_mdl_format_epub) &&
      !(scan->mimetype && scan->container && scan->opf && scan->nav)) {
    return k_ra8_err_validation_failed;
  }
  report->page_count       = scan->pages;
  report->member_count     = scan->members;
  report->metadata_present = (format == k_mdl_format_cbz) ? scan->comicinfo : scan->opf;
  return k_ra8_ok;
}
...
RA8_INTERNAL static ra8_err_t internal_verify_jof(mdl_verify_io_t* io, mdl_verify_report_t* report)
{
  ...
  if (io->size_bytes == 0U) {
    ...
    jof_info_t info;
    error = jof_parse(internal_jof_pread, io, io->size_bytes, &info);
    if (error == k_ra8_ok) {
