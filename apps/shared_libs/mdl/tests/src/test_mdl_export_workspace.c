/**
 * @file test_mdl_export_workspace.c
...
#include "test_mdl_export_workspace_internal.h"
...
/**
 * @brief Isolate every public reservation argument and capacity operand.
 * @par MC/DC:
 * `apps/shared_libs/mdl/src/mdl_export_workspace.c@mdl_export_workspace_take`
 * receives one all-valid control plus one vector for each of `ws`, `data`,
 * `bytes`, zero alignment, and non-power-of-two alignment in its five-condition
 * argument OR. Fresh workspaces then vary `start > cap` and
 * `bytes > cap - start` independently against the successful control.
 * @pre Stack storage is writable for the duration of this vector.
 * @post Every rejected reservation returns NULL without escaping storage.
 * @note Test-target-private and synchronous.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_workspace_assert_take_mcdc(void)
{
  alignas(max_align_t) uint8_t raw[64];
  mdl_export_workspace_t       workspace;
  mdl_export_workspace_init(&workspace, raw, sizeof(raw));
  TEST_ASSERT(mdl_export_workspace_take(&workspace, 1U, 1U) != nullptr);
  TEST_ASSERT(mdl_export_workspace_take(nullptr, 1U, 1U) == nullptr);
  mdl_export_workspace_init(&workspace, nullptr, sizeof(raw));
  TEST_ASSERT(mdl_export_workspace_take(&workspace, 1U, 1U) == nullptr);
  mdl_export_workspace_init(&workspace, raw, sizeof(raw));
  TEST_ASSERT(mdl_export_workspace_take(&workspace, 0U, 1U) == nullptr);
  TEST_ASSERT(mdl_export_workspace_take(&workspace, 1U, 0U) == nullptr);
  TEST_ASSERT(mdl_export_workspace_take(&workspace, 1U, 3U) == nullptr);

  mdl_export_workspace_init(&workspace, raw, 0U);
  workspace.used = 1U;
  TEST_ASSERT(mdl_export_workspace_take(&workspace, 1U, 1U) == nullptr);
  mdl_export_workspace_init(&workspace, raw, 0U);
  TEST_ASSERT(mdl_export_workspace_take(&workspace, 1U, 1U) == nullptr);
...
/**
 * @brief Prove reservations align the absolute pointer, not only the offset.
 * @details Starts from a deliberately misaligned byte and verifies a
 *          maximum-aligned reservation plus its exact cursor accounting.
 * @pre Stack storage is writable for the duration of this vector.
 * @post The returned pointer satisfies the requested absolute alignment.
 * @post Workspace use includes leading alignment padding and payload bytes.
 * @note Test-target-private and synchronous.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_workspace_assert_absolute_alignment(void)
{
  alignas(max_align_t) uint8_t raw[64];
  const size_t                 alignment = alignof(max_align_t);
  uint8_t*                     base      = &raw[1];
  const size_t                 capacity  = sizeof(raw) - 1U;
  const size_t                 padding   = alignment - 1U;
  mdl_export_workspace_t       workspace;
  mdl_export_workspace_init(&workspace, base, capacity);
  void* reservation = mdl_export_workspace_take(&workspace, 1U, alignment);
  TEST_ASSERT(reservation != nullptr);
  TEST_ASSERT(reservation == &raw[alignment]);
  TEST_ASSERT_EQ(padding + 1U, workspace.used);
  TEST_ASSERT_EQ(workspace.used, workspace.high_water);
}

/**
 * @brief Prove pointer-alignment arithmetic rejects both overflow boundaries.
 * @details Reuses one live arena pointer while placing the logical cursor at
 *          each unsigned boundary, so neither vector dereferences an invalid
 *          pointer or depends on fabricated object representations.
 * @pre Stack storage is writable for the duration of this vector.
 * @post Both overflow forms return NULL without changing the high-water mark.
 * @note Test-target-private and synchronous.
 * @par MC/DC:
 * `apps/shared_libs/mdl/src/mdl_export_workspace.c@mdl_export_workspace_take`
 * receives `used > UINTPTR_MAX - base` true in the first vector. The second
 * holds that operand false at equality while making the masked aligned-cursor
 * overflow true; the ordinary successful reservation supplies the F,F control.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_workspace_assert_alignment_overflow(void)
{
  alignas(max_align_t) uint8_t raw[64];
  mdl_export_workspace_t       workspace;
  mdl_export_workspace_init(&workspace, raw, SIZE_MAX);

  uintptr_t base = 0U;
  (void)memcpy((void*)&base, (const void*)&workspace.data, sizeof(workspace.data));
  workspace.used = (size_t)(UINTPTR_MAX - base) + 1U;
  TEST_ASSERT(mdl_export_workspace_take(&workspace, 1U, alignof(max_align_t)) == nullptr);
  TEST_ASSERT_EQ(0U, workspace.high_water);

  workspace.used = (size_t)(UINTPTR_MAX - base);
  TEST_ASSERT(mdl_export_workspace_take(&workspace, 1U, alignof(max_align_t)) == nullptr);
  TEST_ASSERT_EQ(0U, workspace.high_water);
}
                                                         ...
                                                         mdl_export_workspace_t* workspace)
{
  ...
  TEST_ASSERT(
    mdl_export_chapter_ws(mdl_test_storage_get(), k_mdl_format_cbz, directory, output, workspace) ==
                                                              ...
                                                              mdl_export_workspace_t* workspace)
{
  ...
  for (size_t i = 0U; i < (size_t)k_workspace_arena_refusal; ++i) {
    ...
    TEST_ASSERT_EQ(
      k_ra8_ok,
      priv_mdl_export_output_begin(&stage, mdl_test_storage_get(), output, k_mdl_format_epub));
...
RA8_PRIV void priv_test_mdl_export_workspace_run(void)
{
  ...
  internal_workspace_assert_take_mcdc();
  internal_workspace_assert_absolute_alignment();
  internal_workspace_assert_alignment_overflow();
