...
#include "test_mdl_state_metadata_internal.h"
...
RA8_INTERNAL static void internal_test_state_v2_numeric_migration(void)
{
  ...
  for (size_t i = 0U; i < (sizeof(cases) / sizeof(cases[0])); ++i) {
    ...
    TEST_ASSERT(internal_same_binary64(chapter->number, cases[i].number));
...
int main(void)
{
  ...
  (void)internal_test_output_fd_text(STDERR_FILENO, "[OK  ] test_mdl_state.c\n");
  return 0;
}
