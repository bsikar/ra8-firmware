 ...
 */
#include "test_mdl_fetch_fixture.h"
...
RA8_INTERNAL static void internal_test_chapter_number_selector_strict_and_persisted(void)
{
  ...
  (void)__builtin_snprintf(site->chapter_number_selector,
                           sizeof(site->chapter_number_selector),
                           "%s",
...
RA8_INTERNAL static void internal_test_first_run_then_update_only_new(void)
{
  ...
  internal_mdl_fetch_test_setup_site();
  mdl_site_t* const site = internal_mdl_fetch_test_site();
  (void)__builtin_snprintf(site->chapter_title_selector,
                           sizeof(site->chapter_title_selector),
                           "%s",
                           "class:chapter-title");
  s_mock.map   = s_map3;
...
RA8_INTERNAL static void internal_test_governor_retries_throttle(void)
{
  ...
  mdl_governor_init_clock(&gov,
                          ...
                          &clk);
  *internal_mdl_fetch_test_governor_slot() = &gov;
  ...
  const ra8_err_t   rc                     = internal_mdl_fetch_test_run(abs_dir,
                                                                         ...
                                                                         0U,
                                                                         &st);
  *internal_mdl_fetch_test_governor_slot() = nullptr; /* restore the governor-less path */
...
int main(void)
{
  ...
  (void)internal_test_output_fd_text(STDERR_FILENO, "[OK  ] test_mdl_fetch.c\n");
  return 0;
}
