...
typedef struct {
  alignas(max_align_t) uint8_t bytes[k_library_work_bytes]; /**< Backend workspace bytes. */
...
/** @brief Complete caller-owned library test fixture. */
typedef struct {
  uint8_t
    disk[(size_t)k_library_disk_blocks * (size_t)k_ra8_io_block_size_bytes]; /**< Block storage. */
  ...
  mdl_library_test_workspace_t file_work;              /**< File state workspace.      */
  mdl_library_test_workspace_t transaction_work;       /**< Transaction workspace.     */
  uint8_t                      io[k_library_io_bytes]; /**< Stream scratch.            */
  mdl_state_t                  state;                  /**< State to publish.          */
  mdl_state_t                  loaded;                 /**< Loaded state.              */
  mdl_library_workspace_t      library_workspace;      /**< Traversal workspace.       */
  mdl_library_test_workspace_t directory_work;         /**< Directory state workspace. */
} mdl_library_fixture_t;

static mdl_library_fixture_t s_fixture;
...
internal_library_bind(const fw_fs_t* inner, mdl_state_fault_fs_t* fault, mdl_storage_t* storage)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 mdl_storage_init(storage,
                                  &fault->fs,
                                  s_fixture.file_work.bytes,
                                  sizeof(s_fixture.file_work.bytes),
                                  s_fixture.transaction_work.bytes,
                                  sizeof(s_fixture.transaction_work.bytes),
                                  s_fixture.io,
                                  sizeof(s_fixture.io)));
  TEST_ASSERT_EQ(k_ra8_ok,
                 mdl_library_workspace_init(&s_fixture.library_workspace,
                                            s_fixture.directory_work.bytes,
                                            (uint32_t)sizeof(s_fixture.directory_work.bytes)));
}
...
{
  mdl_state_init(&s_fixture.state);
  mdl_state_set_series(&s_fixture.state,
  ...
  TEST_ASSERT_EQ(k_ra8_ok, mdl_state_save(storage, state_path, &s_fixture.state, &published));
...
RA8_INTERNAL static void internal_library_enumeration_absent_and_mixed(mdl_storage_t* storage)
{
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    mdl_state_load_authenticated(storage, "/mixed/alpha/.mdl_state", &s_fixture.loaded));
  TEST_ASSERT(strcmp(s_fixture.loaded.series_title, "alpha") == 0);
                                                             ...
                                                             mdl_state_fault_fs_t* fault)
{
  ...
  TEST_ASSERT_EQ(3U, s_fixture.library_workspace.required_entries);
...
internal_library_make_tree(mdl_storage_t* storage, const char* root, uint16_t depth, bool leaf_file)
{
  ...
  TEST_ASSERT(root_length < sizeof(path));
  (void)memcpy(path, root, root_length + 1U);
  for (uint16_t i = 0U; i < depth; ++i) {
    const size_t used = strlen(path);
    TEST_ASSERT((used + 3U) < sizeof(path));
    path[used]      = '/';
    path[used + 1U] = 'd';
    path[used + 2U] = (char)('0' + i);
    path[used + 3U] = '\0';
                                                         ...
                                                         const mdl_library_policy_t* policy)
{
  ...
  TEST_ASSERT_EQ(
    k_ra8_err_hw_error,
    mdl_library_remove_tree(storage, "/fault-remove", policy, &s_fixture.library_workspace));
  fault->flags = 0U;
...
/** @brief Exercise and clean up the exact directory-entry limit boundary. */
RA8_INTERNAL static void internal_library_removal_entry_limit(mdl_storage_t*        storage,
                                                              mdl_library_policy_t* policy)
{
  ...
  TEST_ASSERT_EQ(
    ...
    mdl_library_remove_tree(storage, "/entry-over", policy, &s_fixture.library_workspace));
  TEST_ASSERT_EQ(1U, s_fixture.library_workspace.entries);
  TEST_ASSERT_EQ(2U, s_fixture.library_workspace.required_entries);
                                                          ...
                                                          mdl_state_fault_fs_t* fault)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_access_denied,
                 mdl_library_remove_tree(storage, "/", &policy, &s_fixture.library_workspace));
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 mdl_library_remove_tree(storage, "/tree", &policy, &s_fixture.library_workspace));
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    mdl_library_remove_tree(storage, "/depth-ok", &policy, &s_fixture.library_workspace));
  internal_library_make_tree(storage, "/depth-over", 3U, false);
  TEST_ASSERT_EQ(
    k_ra8_err_invalid_size,
    mdl_library_remove_tree(storage, "/depth-over", &policy, &s_fixture.library_workspace));
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    mdl_library_remove_tree(storage, "/ops-ok", &policy, &s_fixture.library_workspace));
  internal_library_make_tree(storage, "/ops-over", 0U, false);
  policy.max_operations = 4U;
  TEST_ASSERT_EQ(
    k_ra8_err_invalid_size,
    mdl_library_remove_tree(storage, "/ops-over", &policy, &s_fixture.library_workspace));
  ...
  internal_library_removal_entry_limit(storage, &policy);
...
RA8_INTERNAL static void internal_library_cleanup(mdl_storage_t* storage)
{
  ...
  for (size_t i = 0U; i < (sizeof(roots) / sizeof(roots[0])); ++i) {
    TEST_ASSERT_EQ(
      k_ra8_ok,
      mdl_library_remove_tree(storage, roots[i], &policy, &s_fixture.library_workspace));
  }
                                                         ...
                                                         const char*    host_root)
{
  ...
  char         link_path[512];
  const size_t host_root_length = strlen(host_root);
  TEST_ASSERT((host_root_length + sizeof("/symlink-tree/link")) <= sizeof(link_path));
  TEST_ASSERT(__builtin_snprintf(link_path, sizeof(link_path), "%s/symlink-tree/link", host_root) >
              0);
  TEST_ASSERT_EQ(0, symlink("../outside", link_path));
  const mdl_library_policy_t policy = mdl_library_policy_default();
  TEST_ASSERT_EQ(
    k_ra8_err_access_denied,
    mdl_library_remove_tree(storage, "/symlink-tree", &policy, &s_fixture.library_workspace));
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    mdl_library_remove_tree(storage, "/outside", &policy, &s_fixture.library_workspace));
}
...
{
  char root[] = {'/', 't', 'm', 'p', '/', 'm', 'd', 'l', '_', 'l', 'i', 'b', 'r', 'a', 'r',
                 'y', '_', 'p', 'o', 'r', 't', '_', 'X', 'X', 'X', 'X', 'X', 'X', '\0'};
...
{
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_mount(&s_fixture.backend, &s_fixture.mount));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_io_vfs_init());
  TEST_ASSERT_EQ(k_ra8_ok, ra8_io_vfs_mount("ram", s_fixture.mount));
  const fw_fs_ra8_vfs_cfg_t cfg = {.mount_name      = "ram",
                                   .mount           = s_fixture.mount,
...
{
  TEST_ASSERT_EQ(k_ra8_ok,
                 ra8_io_blockdev_ram_init(&s_fixture.blockdev,
                                          &s_fixture.ram_state,
                                          s_fixture.disk,
                                          k_library_disk_blocks,
                                          false));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_io_blockdev_as_fs_backend(&s_fixture.blockdev, &s_fixture.backend));
  const ra8_fs_format_opts_t format = {.type = k_ra8_fs_type_fat12, .label = "MDLLIB"};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_format(&s_fixture.backend, &format));
  fw_fs_t               fs  = {};
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(s_fixture.mount));
  s_fixture.mount = nullptr;
}
