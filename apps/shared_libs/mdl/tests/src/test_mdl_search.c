 ...
 * `unity_minimal.h` harness, matching the repository's category-local test sources.
 ...
 * (3 conditions). Cited as apps/shared_libs/mdl/src/mdl_extract_hits.c@mdl_extract_hits.
 ...
 * Cited as apps/shared_libs/mdl/src/mdl_search.c@internal_is_unreserved.
 ...
 * apps/shared_libs/mdl/src/mdl_search.c@mdl_search_build_url.
...
int main(void)
{
  ...
  (void)internal_test_output_fd_text(STDERR_FILENO, "[OK  ] test_mdl_search.c\n");
  return 0;
}
