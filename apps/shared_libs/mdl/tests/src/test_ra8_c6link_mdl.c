...
RA8_INTERNAL static uint32_t internal_test_service_multichunk_setup(void)
{
  ...
  TEST_ASSERT_EQ(k_mdl_format_rabook, s_backend.format);
  TEST_ASSERT_EQ(k_mdl_format_rabook, s_service.active_format);
  return job;
}
...
RA8_INTERNAL static void internal_test_service_init_mcdc(void)
{
  ...
  ra8_mdl_service_backend_t backend = {.begin  = priv_test_mdl_backend_begin,
                                       .read   = priv_test_mdl_backend_read,
                                       .cancel = priv_test_mdl_backend_cancel,
                                       .ctx    = &s_backend};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_mdl_service_init(&s_service, &backend));
  ...
  backend.begin = priv_test_mdl_backend_begin;
  backend.read  = nullptr;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, ra8_mdl_service_init(&s_service, &backend));
  backend.read   = priv_test_mdl_backend_read;
