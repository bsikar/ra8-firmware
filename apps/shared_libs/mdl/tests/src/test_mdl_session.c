 ...
 * Uses the repo's `unity_minimal.h` harness, matching the repository's category-local test sources.
...
RA8_INTERNAL static void internal_test_session_disallow_blocks(void)
{
  ...
  mdl_net_iface_t net = internal_robo_iface(&f);
  internal_sess_reset(&net, "mdl/0.1.0", true);
  uint32_t crawl = 0U;
...
RA8_INTERNAL static void internal_test_session_gating_off_and_null(void)
{
  ...
  f.calls = 0U;
  internal_sess_reset(&net, "mdl/0.1.0", false);
  TEST_ASSERT(mdl_session_url_allowed(&s_sess, "https://h/a", &crawl));
...
RA8_INTERNAL static void internal_test_session_crawl_delay_respected(void)
{
  ...
  internal_sess_reset(&net2, "mdl/0.1.0", true);
...
RA8_INTERNAL static void internal_test_session_robots_5xx_denies(void)
{
  ...
  internal_sess_reset(&n5, "mdl/0.1.0", true);
  ...
  internal_sess_reset(&n4, "mdl/0.1.0", true);
  ...
  internal_sess_reset(&n6, "mdl/0.1.0", true);
  ...
  internal_sess_reset(&nrate, "mdl/0.1.0", true);
  ...
  internal_sess_reset(&ndown, "mdl/0.1.0", true);
...
int main(void)
{
  ...
  (void)internal_test_output_fd_text(STDERR_FILENO, "[OK  ] test_mdl_session.c\n");
  return 0;
}
