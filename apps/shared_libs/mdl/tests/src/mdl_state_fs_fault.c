...
typedef struct {
  alignas(max_align_t) uint8_t bytes[k_fault_workspace_bytes]; /**< Real backend state bytes. */
                                                      ...
                                                      uint32_t* out_read)
{
  ...
  const uint32_t bounded_capacity =
    internal_mdl_state_fault(fault, k_mdl_state_fault_short_read) && (capacity > 1U) ? 1U
                                                                                     : capacity;
  mdl_state_fault_workspace_t* nested = (mdl_state_fault_workspace_t*)state;
...
{
  mdl_state_fault_fs_t* fault = (mdl_state_fault_fs_t*)ctx;
  const uint32_t        bounded_length =
    internal_mdl_state_fault(fault, k_mdl_state_fault_short_write) && (length > 1U) ? 1U : length;
  mdl_state_fault_workspace_t* nested = (mdl_state_fault_workspace_t*)state;
...
{
  static const fw_fs_namespace_iface_t   names        = {.stat      = internal_mdl_state_stat,
  ...
  static const fw_fs_stream_iface_t      streams      = {.open  = internal_mdl_state_open,
  ...
  static const fw_fs_transaction_iface_t transactions = {
  ...
  if ((inner->caps.file_workspace_bytes > (uint32_t)k_fault_workspace_bytes) ||
      ...
      (inner->caps.file_workspace_align > alignof(max_align_t)) ||
      (inner->caps.transaction_workspace_align > alignof(max_align_t))) {
    return k_ra8_err_no_mem;
  }
  ...
  caps.file_workspace_align        = alignof(mdl_state_fault_workspace_t);
  caps.directory_workspace_align   = alignof(mdl_state_fault_directory_t);
  caps.transaction_workspace_align = alignof(mdl_state_fault_workspace_t);
  *fault                           = (mdl_state_fault_fs_t){.inner = inner};
  return fw_fs_bind(&fault->fs, &names, &streams, &transactions, fault, &caps);
}
