...
RA8_INTERNAL static void internal_test_short_io_and_two_bindings(void)
{
  ...
  } formats[] = {{k_mdl_format_cbz, "/tmp/mdl_export_short/a.cbz"},
                 {k_mdl_format_cbt, "/tmp/mdl_export_short/a.cbt"},
                 {k_mdl_format_cbt_gz, "/tmp/mdl_export_short/a.cbt.gz"},
                 {k_mdl_format_epub, "/tmp/mdl_export_short/a.epub"}};
...
RA8_INTERNAL static void internal_test_transaction_fault_preservation(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    mdl_verify_file(mdl_test_storage_get(), k_mdl_format_cbz, output, &workspace, &report));
  internal_assert_no_stage(directory);
...
RA8_INTERNAL static void internal_test_backfill_seek_fault(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 priv_mdl_export_output_begin(&stage, &s_fault_storage, output, k_mdl_format_cbz));
