...
RA8_INTERNAL static void internal_test_format_mapping(void)
{
  ...
  TEST_ASSERT(mdl_format_from_str("cbz") == k_mdl_format_cbz);
  TEST_ASSERT(mdl_format_from_str("cbt.xz") == k_mdl_format_invalid);
  TEST_ASSERT(mdl_format_from_str(nullptr) == k_mdl_format_loose);
  TEST_ASSERT(mdl_format_from_str("bogus") == k_mdl_format_invalid);
  mdl_format_t inferred = k_mdl_format_invalid;
  TEST_ASSERT(mdl_format_from_path("/tmp/book.CBT.GZ", &inferred) == k_ra8_ok);
  TEST_ASSERT(inferred == k_mdl_format_cbt_gz);
  ...
  TEST_ASSERT(inferred == k_mdl_format_epub);
  TEST_ASSERT(mdl_format_from_path("/tmp/book.RABOOK", &inferred) == k_ra8_ok);
  TEST_ASSERT(inferred == k_mdl_format_rabook);
  ...
  TEST_ASSERT(inferred == k_mdl_format_invalid);
  TEST_ASSERT(strcmp(mdl_format_ext(k_mdl_format_cbz), "cbz") == 0);
  TEST_ASSERT(strcmp(mdl_format_ext(k_mdl_format_cbt_gz), "cbt.gz") == 0);
  TEST_ASSERT(mdl_format_from_str("epub") == k_mdl_format_epub);
  TEST_ASSERT(mdl_format_from_str("jof") == k_mdl_format_jof);
  TEST_ASSERT(mdl_format_from_str("rabook") == k_mdl_format_rabook);
  TEST_ASSERT(strcmp(mdl_format_ext(k_mdl_format_epub), "epub") == 0);
  TEST_ASSERT(strcmp(mdl_format_ext(k_mdl_format_jof), "jof") == 0);
  TEST_ASSERT(strcmp(mdl_format_ext(k_mdl_format_rabook), "rabook") == 0);
  TEST_ASSERT(mdl_format_is_verifiable(k_mdl_format_cbz));
  TEST_ASSERT(mdl_format_is_verifiable(k_mdl_format_cbt));
  TEST_ASSERT(mdl_format_is_verifiable(k_mdl_format_epub));
  TEST_ASSERT(mdl_format_is_verifiable(k_mdl_format_jof));
  TEST_ASSERT(mdl_format_is_verifiable(k_mdl_format_cbt_gz));
  TEST_ASSERT(mdl_format_is_verifiable(k_mdl_format_rabook));
...
RA8_INTERNAL static void internal_test_robots_group_select(void)
{
  ...
  static const char txt[] = "User-agent: *\nDisallow: /all\n\n"
                            "User-agent: mdl\nDisallow: /mine\nCrawl-delay: 5\n";
  mdl_robots_t      r;
  mdl_robots_parse(txt, sizeof(txt) - 1U, "mdl", &r);
...
int main(void)
{
  ...
  (void)internal_test_output_fd_text(STDERR_FILENO, "[OK  ] test_mdl.c\n");
  return 0;
}
