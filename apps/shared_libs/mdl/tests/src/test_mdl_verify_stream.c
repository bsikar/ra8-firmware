...
RA8_INTERNAL static void internal_test_verify_arguments(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_not_supported,
                 mdl_verify_file(store, k_mdl_format_cbr, s_stream_path, &ws, &report));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 mdl_verify_file(store, k_mdl_format_loose, s_stream_path, &ws, &report));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 mdl_verify_file(store, k_mdl_format_cbt, nullptr, &ws, &report));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 mdl_verify_open_file(store, k_mdl_format_cbt, &io.owned_file, 0U, &ws, nullptr));
