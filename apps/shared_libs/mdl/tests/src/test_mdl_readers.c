...
#include "mdl_reader_archive_fixtures.h"
...
typedef struct {
  alignas(max_align_t) uint8_t bytes[k_reader_work_bytes]; /**< Backend workspace bytes. */
} mdl_reader_test_workspace_t;

/** @brief Maximally aligned verifier arena. */
typedef struct {
  alignas(max_align_t) uint8_t bytes[k_reader_verify_work]; /**< Miniz workspace bytes. */
...
/** @brief Complete caller-owned RAM/FAT/VFS reader fixture. */
typedef struct {
  uint8_t
    disk[(size_t)k_reader_disk_blocks * (size_t)k_ra8_io_block_size_bytes]; /**< Block storage. */
  ra8_io_blockdev_ram_state_t ram_state; /**< RAM driver state.    */
  ra8_io_blockdev_t           blockdev;  /**< Block-device facade. */
  ra8_fs_backend_t            backend;   /**< Filesystem backend.  */
  ra8_fs_mount_t*             mount;     /**< Active mount.        */
} mdl_reader_vfs_fixture_t;

static mdl_reader_vfs_fixture_t      s_vfs;
...
static const mdl_reader_verify_case_t s_verify_cases[] = {
  {"/book.cbz", k_mdl_format_cbz, 2U},
  {"/book.epub", k_mdl_format_epub, 5U},
  {"/book.cbt", k_mdl_format_cbt, 2U},
  {"/book.cbt.gz", k_mdl_format_cbt_gz, 2U},
  {"/book.jof", k_mdl_format_jof, 1U},
};
                                                        ...
                                                        mdl_state_fault_fs_t* fault)
{
  ...
  (void)memcpy(overlong, prefix, sizeof(prefix) - 1U);
  (void)memset(&overlong[sizeof(prefix) - 1U], 'a', sizeof(overlong) - (sizeof(prefix) - 1U));
                                                       ...
                                                       mdl_state_fault_fs_t* fault)
{
  ...
  char ext[8]   = {'k', 'e', 'e', 'p', '\0'};
  char mime[32] = {'k', 'e', 'e', 'p', '\0'};
  TEST_ASSERT_EQ(
  ...
  fault->flags             = (uint32_t)k_mdl_state_fault_read;
  static const char keep[] = {'k', 'e', 'e', 'p', '\0'};
  (void)memcpy(ext, keep, sizeof(keep));
  TEST_ASSERT_EQ(
    k_ra8_err_hw_error,
    mdl_urlname_sniff_file(storage, "/image.bin", nullptr, ext, sizeof(ext), mime, sizeof(mime)));
  TEST_ASSERT(strcmp(ext, "keep") == 0);
...
/**
 * @brief Encode one fixed-width zero-padded octal field.
 * @details Writes exactly @p width octal digits followed by one NUL byte.
 * @param[out] bytes Destination with at least `width + 1` writable bytes.
 * @param[in] width Number of octal digits to emit.
 * @param[in] value Value to encode.
 * @pre @p bytes is non-null and @p width is nonzero.
 * @pre The value fits in the requested digit width.
 * @post The destination is NUL terminated after exactly @p width digits.
 * @note Used only for deterministic TAR fixture construction.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_reader_put_octal(uint8_t* bytes, size_t width, uint32_t value)
{
  uint32_t remaining = value;
  for (size_t index = 0U; index < width; ++index) {
    const size_t position = (width - 1U) - index;
    bytes[position]       = (uint8_t)((uint8_t)'0' | (uint8_t)(remaining & 7U));
    remaining >>= 3U;
  }
  bytes[width] = 0U;
}
                                                      ...
                                                      uint32_t       length)
{
  ...
  uint8_t* header = &tar[cursor];
  (void)memset(header, 0, 512U);
  (void)memcpy(header, name, strlen(name));
  internal_reader_put_octal(&header[124], 11U, length);
  (void)memset(&header[148], ' ', 8U);
  header[156]       = '0';
  unsigned checksum = 0U;
  for (size_t index = 0U; index < 512U; ++index) {
    checksum += header[index];
  }
  internal_reader_put_octal(&header[148], 6U, checksum);
  header[154] = '\0';
  header[155] = ' ';
  cursor += 512U;
  (void)memcpy(&tar[cursor], data, length);
  cursor += length;
  const size_t padded = (cursor + 511U) & ~(size_t)511U;
  (void)memset(&tar[cursor], 0, padded - cursor);
  return padded;
}
/**
...
RA8_INTERNAL static uint32_t internal_reader_make_tar(uint8_t* tar)
{
  ...
  (void)memset(tar, 0, k_reader_archive_max);
  ...
  (void)memset(&tar[offset], 0, 1024U);
...
internal_reader_make_gzip(const uint8_t* tar, uint32_t tar_length, uint8_t* gzip)
{
  ...
  (void)memcpy(gzip, header, sizeof(header));
...
{
  static const uint8_t magic[] = {'J', 'O', 'F', '1'}; /* binary, not a C string */
  static const uint8_t end[]   = {'J', 'O', 'F', 'E'};
  (void)memset(jof, 0, 57U);
  (void)memcpy(jof, magic, sizeof(magic));
  jof[4]  = 1U;
  jof[6]  = 1U;
  jof[8]  = 1U;
  jof[10] = 1U;
  jof[12] = 1U;
  internal_reader_put_u32le(&jof[16], 1U);
  jof[32] = 0x5AU;
  internal_reader_put_u32le(&jof[33], 32U);
  internal_reader_put_u32le(&jof[37], 1U);
  internal_reader_put_u32le(&jof[41], 33U);
  internal_reader_put_u32le(&jof[45], 1U);
  internal_reader_put_u32le(&jof[49], 57U);
  (void)memcpy(&jof[53], end, sizeof(end));
  return 57U;
}
/**
...
RA8_INTERNAL static void internal_reader_expect_error(mdl_storage_t* storage,
                                                      ...
                                                      ra8_err_t      expected)
{
  mdl_verify_report_t report = {.format           = k_mdl_format_cbr,
                                .page_count       = 17U,
                                .member_count     = 19U,
  ...
  TEST_ASSERT_EQ(k_mdl_format_cbr, report.format);
...
internal_reader_replace(uint8_t* bytes, uint32_t length, const char* before, const char* after)
{
  ...
  for (uint32_t offset = 0U; (offset + width) <= length; ++offset) {
    bool matches = true;
    for (size_t index = 0U; index < width; ++index) {
      if (matches && (bytes[offset + index] != (uint8_t)before[index])) {
        matches = false;
      }
    }
    if (matches) {
      for (size_t index = 0U; index < width; ++index) {
        bytes[offset + index] = (uint8_t)after[index];
      }
      ++replaced;
...
RA8_INTERNAL static void internal_reader_verify_capacity(mdl_storage_t* storage)
{
  for (size_t index = 0U; index < 2U; ++index) {
    ...
    const mdl_format_t  format     = (index == 0U) ? k_mdl_format_cbz : k_mdl_format_cbt_gz;
    mdl_verify_report_t report     = {};
    ...
    report = (mdl_verify_report_t){.format = k_mdl_format_cbr, .page_count = 17U};
...
{
  (void)memcpy(s_archive, s_cbz, sizeof(s_cbz));
  s_archive[38] ^= 1U;
  internal_reader_write(storage, "/book.cbz", s_archive, sizeof(s_cbz));
  internal_reader_expect_error(storage, k_mdl_format_cbz, "/book.cbz", k_ra8_err_validation_failed);
  (void)memcpy(s_archive, s_cbz, sizeof(s_cbz));
  ...
  internal_reader_expect_error(storage, k_mdl_format_cbz, "/book.cbz", k_ra8_err_validation_failed);
  (void)memcpy(s_archive, s_epub, sizeof(s_epub));
  ...
  s_archive[148] ^= 1U;
  internal_reader_write(storage, "/book.cbt", s_archive, tar_length);
  internal_reader_expect_error(storage, k_mdl_format_cbt, "/book.cbt", k_ra8_err_validation_failed);
  tar_length                            = internal_reader_make_tar(s_archive);
  static const uint8_t traversal_name[] = {'.', '.', '/', 'x', '.', 'j', 'p', 'g', 0U};
  (void)memcpy(s_archive,
               traversal_name,
               sizeof(traversal_name)); /* fixed-width NUL-padded TAR name */
  internal_reader_write(storage, "/book.cbt", s_archive, tar_length);
  internal_reader_expect_error(storage, k_mdl_format_cbt, "/book.cbt", k_ra8_err_validation_failed);
}
/**
...
RA8_INTERNAL static void internal_reader_verify_stream_corrupt(mdl_storage_t* storage)
{
  ...
  s_archive[0]              = 'X';
  internal_reader_write(storage, "/book.jof", s_archive, jof_length);
  internal_reader_expect_error(storage, k_mdl_format_jof, "/book.jof", k_ra8_err_validation_failed);
                                                               ...
                                                               mdl_state_fault_fs_t* fault)
{
  ...
  mdl_verify_report_t    report = {.format = k_mdl_format_cbr, .page_count = 17U};
  fault->flags                  = (uint32_t)k_mdl_state_fault_stream_seek;
  TEST_ASSERT_EQ(
    k_ra8_err_hw_error,
    mdl_verify_open_file(storage, k_mdl_format_cbz, &borrowed, borrowed_size, &work, &report));
  TEST_ASSERT_EQ(k_mdl_format_cbr, report.format);
  TEST_ASSERT_EQ(17U, report.page_count);
  fault->flags = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, fw_fs_file_size(&borrowed, &borrowed_size));
                                                       ...
                                                       mdl_state_fault_fs_t* fault)
{
  ...
  fault->flags = (uint32_t)k_mdl_state_fault_stream_seek;
  internal_reader_expect_error(storage, k_mdl_format_cbz, "/book.cbz", k_ra8_err_hw_error);
  internal_reader_expect_error(storage, k_mdl_format_jof, "/book.jof", k_ra8_err_hw_error);
  fault->flags = 0U;
  internal_reader_verify_borrowed_fault(storage, fault);
  fault->flags = (uint32_t)k_mdl_state_fault_media;
  internal_reader_expect_error(storage, k_mdl_format_cbt, "/book.cbt", k_ra8_err_hw_not_ready);
  fault->flags = 0U;
  internal_reader_expect_error(storage, k_mdl_format_cbt, "book.cbt", k_ra8_err_invalid_arg);
  internal_reader_expect_error(storage, k_mdl_format_cbr, "/book.cbz", k_ra8_err_not_supported);
}
/**
...
{
  char root[] = {'/', 't', 'm', 'p', '/', 'm', 'd', 'l', '_', 'r', 'e', 'a', 'd', 'e',
                 'r', '_', 'p', 'o', 'r', 't', '_', 'X', 'X', 'X', 'X', 'X', 'X', '\0'};
 ...
 * @post The named VFS mount and filesystem mount are both released. @post The fixture mount is reset to NULL after successful teardown.
...
{
  TEST_ASSERT_EQ(k_ra8_ok,
                 ra8_io_blockdev_ram_init(&s_vfs.blockdev,
                                          &s_vfs.ram_state,
                                          s_vfs.disk,
                                          k_reader_disk_blocks,
                                          false));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_io_blockdev_as_fs_backend(&s_vfs.blockdev, &s_vfs.backend));
  const ra8_fs_format_opts_t format = {.type = k_ra8_fs_type_fat12, .label = "MDLREAD"};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_format(&s_vfs.backend, &format));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_mount(&s_vfs.backend, &s_vfs.mount));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_io_vfs_init());
  TEST_ASSERT_EQ(k_ra8_ok, ra8_io_vfs_mount("reader", s_vfs.mount));
  fw_fs_t                   fs    = {};
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(s_vfs.mount));
  s_vfs.mount = nullptr;
}
