...
RA8_INTERNAL static void internal_assert_cbz_comicinfo(const mdl_export_meta_t* meta)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, internal_export_chapter_meta(k_mdl_format_cbz, dir, out, meta));
  ...
  TEST_ASSERT(internal_export_chapter_meta(k_mdl_format_cbz, dir, rejected, &overlong) ==
...
RA8_INTERNAL static void internal_assert_epub_opf(const mdl_export_meta_t* meta)
{
  ...
  TEST_ASSERT(internal_export_chapter_meta(k_mdl_format_epub, dir, out, meta) == k_ra8_ok);
  mdl_test_zip_reader_t reader;
  TEST_ASSERT(
    internal_test_zip_open(&reader, out, s_test_export_arena, sizeof(s_test_export_arena)));
  char opf[4096];
...
RA8_INTERNAL static void internal_assert_epub_distinct_identifiers(const mdl_export_meta_t* meta)
{
  ...
  TEST_ASSERT(internal_export_chapter_meta(k_mdl_format_epub, dir, second, &other) == k_ra8_ok);
...
RA8_INTERNAL static void internal_assert_epub_rejection(const mdl_export_meta_t* meta)
{
  ...
  (void)unlink(rejected);
  TEST_ASSERT(internal_export_chapter_meta(k_mdl_format_epub,
                                                      ...
                                                      size_t                   expected_bytes)
{
  ...
  TEST_ASSERT(
    internal_export_chapter_meta(k_mdl_format_cbz, "/tmp/mdl_external_cover_chap", out, meta) ==
    k_ra8_ok);
  mdl_test_zip_reader_t reader;
  TEST_ASSERT(
    internal_test_zip_open(&reader, out, s_test_export_arena, sizeof(s_test_export_arena)));
                                                       ...
                                                       size_t                   expected_bytes)
{
  ...
  TEST_ASSERT(
    internal_export_chapter_meta(k_mdl_format_epub, "/tmp/mdl_external_cover_chap", out, meta) ==
    k_ra8_ok);
  mdl_test_zip_reader_t reader;
  TEST_ASSERT(
    internal_test_zip_open(&reader, out, s_test_export_arena, sizeof(s_test_export_arena)));
...
RA8_INTERNAL static void internal_assert_external_cover_rejected(mdl_export_meta_t* meta)
{
  ...
  (void)unlink(bad_out);
  TEST_ASSERT(
    internal_export_chapter_meta(k_mdl_format_cbz, "/tmp/mdl_external_cover_chap", bad_out, meta) ==
...
int main(void)
{
  ...
  (void)internal_test_output_fd_text(STDERR_FILENO, "[OK  ] test_mdl_metadata.c\n");
  return 0;
}
