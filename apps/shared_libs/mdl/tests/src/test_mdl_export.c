...
RA8_INTERNAL static void internal_test_export_cbz_roundtrip(void)
{
  ...
  const ra8_err_t rc = internal_export_chapter(k_mdl_format_cbz, dir, out);
  TEST_ASSERT_EQ(k_ra8_ok, rc);
  mdl_verify_report_t verified = {};
  TEST_ASSERT(internal_verify_file(k_mdl_format_cbz, out, &verified) == k_ra8_ok);
...
RA8_INTERNAL static void internal_test_export_cbt_structure(void)
{
  ...
  TEST_ASSERT(internal_export_chapter_meta(k_mdl_format_cbt, dir, out, &meta) == k_ra8_ok);
  mdl_verify_report_t verified = {};
  TEST_ASSERT(internal_verify_file(k_mdl_format_cbt, out, &verified) == k_ra8_ok);
  TEST_ASSERT(verified.page_count == k_expect_pages);
  TEST_ASSERT(internal_export_chapter_meta(k_mdl_format_cbt_gz, dir, gz, &meta) == k_ra8_ok);
  verified = (mdl_verify_report_t){};
  TEST_ASSERT(internal_verify_file(k_mdl_format_cbt_gz, gz, &verified) == k_ra8_ok);
  TEST_ASSERT(verified.page_count == k_expect_pages);
...
RA8_INTERNAL static void internal_test_export_skips_non_images(void)
{
  ...
  TEST_ASSERT(internal_export_chapter(k_mdl_format_cbz, dir, out) == k_ra8_ok);
  mdl_test_zip_reader_t reader;
  TEST_ASSERT(internal_test_zip_open(&reader, out));
  TEST_ASSERT_EQ(k_expect_pages + 1U, mz_zip_reader_get_num_files(&reader.zip));
  char name[k_name_probe];
...
RA8_INTERNAL static void internal_test_export_epub_roundtrip(void)
{
  ...
  TEST_ASSERT(internal_export_chapter(k_mdl_format_epub, dir, out) == k_ra8_ok);
  mdl_verify_report_t verified = {};
  TEST_ASSERT(internal_verify_file(k_mdl_format_epub, out, &verified) == k_ra8_ok);
  TEST_ASSERT(verified.page_count == k_expect_pages);
...
RA8_INTERNAL static void internal_test_export_jof_roundtrip(void)
{
  ...
  TEST_ASSERT(mdl_format_is_dir_output(k_mdl_format_jof));
  TEST_ASSERT(!mdl_format_is_dir_output(k_mdl_format_cbz));
  TEST_ASSERT(!mdl_format_is_dir_output(k_mdl_format_epub));
  TEST_ASSERT(internal_export_chapter(k_mdl_format_jof, dir, dir) == k_ra8_ok);
  mdl_verify_report_t verified = {};
  TEST_ASSERT(internal_verify_file(k_mdl_format_jof, jof, &verified) == k_ra8_ok);
  ...
  TEST_ASSERT(sz > (size_t)k_jof_hdr_bytes);

  const size_t mlen = (size_t)k_jof_magic_len;
  TEST_ASSERT(memcmp(s_test_export_arena + k_jof_ofs_magic, "JOF1", mlen) == 0);
  TEST_ASSERT(memcmp(s_test_export_arena + (sz - mlen), "JOFE", mlen) == 0);
  /* webtoon-native: a single full-width tile column (tile_w == width). */
  TEST_ASSERT_EQ(internal_read_u16(s_test_export_arena + k_jof_ofs_width),
                 internal_read_u16(s_test_export_arena + k_jof_ofs_tile_w));
...
RA8_INTERNAL static void internal_test_tar_rejects_long_name(void)
{
  ...
  TEST_ASSERT(internal_export_chapter(k_mdl_format_cbt, dir, out) == k_ra8_err_invalid_size);
  (void)unlink(path);
  (void)unlink(out);
  (void)rmdir(dir);
  ...
  TEST_ASSERT(internal_export_chapter(k_mdl_format_cbt, okdir, okout) == k_ra8_ok);
...
RA8_INTERNAL static void internal_test_export_page_cap(void)
{
  ...
  TEST_ASSERT(internal_export_chapter(k_mdl_format_cbz, dir, out) == k_ra8_err_invalid_size);
...
RA8_INTERNAL static void internal_test_verify_rejects_truncation(void)
{
  ...
  } cases[] = {{"/tmp/mdl_bad.cbz", k_mdl_format_cbz},
               {"/tmp/mdl_bad.cbt", k_mdl_format_cbt},
               {"/tmp/mdl_bad.cbt.gz", k_mdl_format_cbt_gz},
               {"/tmp/mdl_bad.epub", k_mdl_format_epub},
               {"/tmp/mdl_bad.jof", k_mdl_format_jof},
               {"/tmp/mdl_bad.rabook", k_mdl_format_rabook}};
...
int main(void)
{
  ...
  (void)internal_test_output_fd_text(STDERR_FILENO, "[OK  ] test_mdl_export.c\n");
  return 0;
}
