 ...
 * k_ra8_fail)` cites apps/shared_libs/mdl/src/mdl_fetch.c@priv_mdl_fetch_is_retryable.
 ...
 * apps/shared_libs/mdl/src/mdl_fetch.c@priv_mdl_fetch_run_incomplete.
...
RA8_INTERNAL static void internal_test_fetch_asset_policy_atomic_and_nonempty(void)
{
  ...
  mdl_session_init(&s_sess, &iface, "mdl/test", nullptr, false);
...
int main(void)
{
  ...
  (void)internal_test_output_fd_text(STDERR_FILENO, "[OK  ] test_mdl_fetch_cache.c\n");
  return 0;
}
