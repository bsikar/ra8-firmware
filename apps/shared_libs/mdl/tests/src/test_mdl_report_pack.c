/**
 * @file test_mdl_report_pack.c
 * @brief Host qualification of the mdl progress presenter and packager.
...
internal_pack_chapter(mdl_format_t format, const char* series, const char* chapter)
{
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static size_t internal_pack_folder(mdl_format_t             format,
                                                bool                     allow,
                                                const char*              series,
                                                const char*              folder,
 ...
 * apps/shared_libs/mdl/src/mdl_pack.c@mdl_pack_one_meta.
...
RA8_INTERNAL static void internal_test_pack_one_path_guards(void)
{
  ...
  size_t rc = internal_pack_chapter(k_mdl_format_cbz, s_series_dir, "a/b");
  ...
  rc = internal_pack_chapter(k_mdl_format_cbz, s_series_dir, long_id);
  TEST_ASSERT_EQ((int64_t)k_pack_failed, (int64_t)rc);
  internal_expect_text(&s_out, "");
  ...
  rc                     = internal_pack_chapter(k_mdl_format_cbz, deep, "abcdefghijkl");
  TEST_ASSERT_EQ((int64_t)k_pack_failed, (int64_t)rc);
  internal_expect_text(&s_out, "");
...
RA8_INTERNAL static void internal_test_pack_one_container(void)
{
  ...
  TEST_ASSERT((composed > 0) && ((size_t)composed < sizeof(expect)));
  const size_t rc = internal_pack_chapter(k_mdl_format_cbz, s_series_dir, "chapempty");
  TEST_ASSERT_EQ((int64_t)k_pack_failed, (int64_t)rc);
  internal_expect_text(&s_out, "");
  internal_expect_text(&s_diag, expect);
...
RA8_INTERNAL static void internal_test_pack_one_dir_output(void)
{
  ...
  size_t rc = internal_pack_chapter(k_mdl_format_jof, s_series_dir, "chapjof");
  ...
  const int composed = snprintf(expect,
                                ...
                                (unsigned)k_ra8_err_empty);
  TEST_ASSERT((composed > 0) && ((size_t)composed < sizeof(expect)));
  rc = internal_pack_chapter(k_mdl_format_jof, s_series_dir, "chapjofempty");
  TEST_ASSERT_EQ((int64_t)k_pack_failed, (int64_t)rc);
  internal_expect_text(&s_out, "");
  internal_expect_text(&s_diag, expect);
...
RA8_INTERNAL static void internal_test_pack_combined_policy(void)
{
  ...
  size_t rc = internal_pack_folder(k_mdl_format_cbz, false, s_series_dir, "combined", &stats);
  TEST_ASSERT_EQ((int64_t)k_pack_ok, (int64_t)rc);
  internal_expect_text(&s_out, "");
  internal_expect_text(&s_diag, "");
  ...
  rc = internal_pack_folder(k_mdl_format_cbz, false, s_series_dir, "combined", &stats);
  TEST_ASSERT_EQ((int64_t)k_pack_ok, (int64_t)rc);
  internal_expect_text(&s_out, "");
  internal_expect_text(&s_diag,
  ...
  rc = internal_pack_folder(k_mdl_format_cbz, true, s_series_dir, "combined", &stats);
  TEST_ASSERT_EQ((int64_t)k_pack_ok, (int64_t)rc);
  internal_expect_text(&s_out,
                       "  combined (INCOMPLETE) -> "
...
RA8_INTERNAL static void internal_test_pack_combined_dir_output(void)
{
  ...
  mdl_fetch_stats_t stats  = {};
  stats.chapters_completed = (size_t)k_stats_one_chapter;
  size_t rc = internal_pack_folder(k_mdl_format_jof, false, s_series_dir, "combjof", &stats);
  TEST_ASSERT_EQ((int64_t)k_pack_ok, (int64_t)rc);
  ...
  const int composed = snprintf(expect,
                                ...
                                (unsigned)k_ra8_err_empty);
  TEST_ASSERT((composed > 0) && ((size_t)composed < sizeof(expect)));
  rc = internal_pack_folder(k_mdl_format_jof, false, s_series_dir, "combjofempty", &stats);
  TEST_ASSERT_EQ((int64_t)k_pack_failed, (int64_t)rc);
  internal_expect_text(&s_out, "");
  internal_expect_text(&s_diag, expect);
  rc = internal_pack_folder(k_mdl_format_cbz, false, s_series_dir, "combjofempty", &stats);
  TEST_ASSERT_EQ((int64_t)k_pack_failed, (int64_t)rc);
  internal_expect_text(&s_out, "");
  internal_expect_text(&s_diag, expect);
 ...
 * apps/shared_libs/mdl/src/mdl_pack.c@internal_pack_combined_dir: the control comes
...
RA8_INTERNAL static void internal_test_pack_combined_path_guards(void)
{
  ...
  mdl_fetch_stats_t stats  = {};
  stats.chapters_completed = (size_t)k_stats_one_chapter;
  size_t rc = internal_pack_folder(k_mdl_format_cbz, false, s_series_dir, "x/y", &stats);
  TEST_ASSERT_EQ((int64_t)k_pack_failed, (int64_t)rc);
  internal_expect_text(&s_out, "");
  ...
  rc = internal_pack_folder(k_mdl_format_cbz, false, s_series_dir, long_rel, &stats);
  TEST_ASSERT_EQ((int64_t)k_pack_failed, (int64_t)rc);
  internal_expect_text(&s_out, "");
  internal_expect_text(&s_diag, "  combine export path rejected under /tmp/mdl_rp_series\n");
  char deep[k_deep_dir_chars + 1U];
  ...
  TEST_ASSERT((composed > 0) && ((size_t)composed < sizeof(expect)));
  rc = internal_pack_folder(k_mdl_format_cbz, false, deep, "abcdefghijkl", &stats);
  TEST_ASSERT_EQ((int64_t)k_pack_failed, (int64_t)rc);
  internal_expect_text(&s_out, "");
  internal_expect_text(&s_diag, expect);
...
int main(void)
{
  ...
  (void)internal_test_output_fd_text(STDERR_FILENO, "[OK  ] test_mdl_report_pack.c\n");
  return 0;
}
