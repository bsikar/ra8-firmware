...
typedef struct {
  alignas(max_align_t) uint8_t bytes[k_export_work_bytes]; /**< Backend-private state. */
...
/** @brief Complete caller-owned RAM/FAT/VFS fixture. */
typedef struct {
  /** Block storage. */
  uint8_t disk[(size_t)k_export_disk_blocks * k_ra8_io_block_size_bytes];
  /** RAM driver state. */
...
} internal_export_vfs_t;

static uint8_t               s_export_arena[k_export_arena_bytes];
static internal_export_vfs_t s_export_vfs;
...
RA8_INTERNAL static void internal_run_formats(mdl_storage_t* storage, uint64_t hashes[5])
{
  ...
  } cases[] = {{k_mdl_format_cbz, "/book.cbz", "/book.cbz"},
               {k_mdl_format_cbt, "/book.cbt", "/book.cbt"},
               {k_mdl_format_cbt_gz, "/book.cbt.gz", "/book.cbt.gz"},
               {k_mdl_format_epub, "/book.epub", "/book.epub"},
               {k_mdl_format_jof, "/chapter", "/chapter/001.jof"}};
...
{
  TEST_ASSERT_EQ(k_ra8_ok,
                 ra8_io_blockdev_ram_init(&s_export_vfs.blockdev,
                                          &s_export_vfs.ram_state,
                                          s_export_vfs.disk,
                                          k_export_disk_blocks,
                                          false));
  TEST_ASSERT_EQ(k_ra8_ok,
                 ra8_io_blockdev_as_fs_backend(&s_export_vfs.blockdev, &s_export_vfs.backend));
  const ra8_fs_format_opts_t format = {.type = k_ra8_fs_type_fat12, .label = "MDLEXPORT"};
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_format(&s_export_vfs.backend, &format));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_mount(&s_export_vfs.backend, &s_export_vfs.mount));
  TEST_ASSERT_EQ(k_ra8_ok, ra8_io_vfs_init());
  TEST_ASSERT_EQ(k_ra8_ok, ra8_io_vfs_mount("export", s_export_vfs.mount));
  const fw_fs_ra8_vfs_cfg_t cfg = {.mount_name      = "export",
                                   .mount           = s_export_vfs.mount,
...
RA8_INTERNAL static void internal_test_export_portability(void)
{
  ...
  char posix_root[] = {'/', 't', 'm', 'p', '/', 'm', 'd', 'l', '_', 'e', 'x', 'p', 'o', 'r',
                       't', '_', 'p', 'o', 'r', 't', '_', 'X', 'X', 'X', 'X', 'X', 'X', '\0'};
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_unmount(s_export_vfs.mount));
  s_export_vfs.mount = nullptr;
