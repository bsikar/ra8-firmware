...
typedef struct {
  alignas(max_align_t) uint8_t bytes[k_state_work_bytes]; /**< Backend workspace storage. */
} mdl_state_test_workspace_t;

/** @brief Complete caller-owned RAM/FAT/VFS state fixture. */
typedef struct {
  /** Block storage. */
  uint8_t disk[(size_t)k_state_disk_blocks * (size_t)k_ra8_io_block_size_bytes];
  /** RAM driver state. */
...
} mdl_state_vfs_fixture_t;

static mdl_state_vfs_fixture_t s_vfs;
...
/**
 * @brief Report whether two ordinary binary64 values and their signs match.
 * @details Numeric equality covers every finite fixture value while the
 *          explicit sign comparison keeps -0.0 distinct from +0.0.
 * @param[in] left First value.
 * @param[in] right Second value.
 * @return Whether both ordinary values and their signs match.
 * @retval true The values and signs match.
 * @retval false The values or signs differ.
...
{
  uint32_t running = state;
  for (uint32_t i = 0U; i < length; ++i) {
    running ^= bytes[i];
    for (uint8_t bit = 0U; bit < 8U; ++bit) {
      const uint32_t mask = (uint32_t)(0U - (running & 1U));
      running             = (running >> 1U) ^ (0xEDB88320UL & mask);
    }
  }
  return running;
}
...
RA8_INTERNAL static void internal_cleanup_vectors(mdl_storage_t* storage)
{
  ...
  for (size_t i = 0U; i < (sizeof(directories) / sizeof(directories[0])); ++i) {
    char         base[64];
    char         alternate[72];
    const size_t directory_length = strlen(directories[i]);
    TEST_ASSERT((directory_length + sizeof("/.mdl_state")) <= sizeof(base));
    TEST_ASSERT(__builtin_snprintf(base, sizeof(base), "%s/.mdl_state", directories[i]) > 0);
    const size_t base_length = strlen(base);
    TEST_ASSERT((base_length + sizeof(".alt")) <= sizeof(alternate));
    TEST_ASSERT(__builtin_snprintf(alternate, sizeof(alternate), "%s.alt", base) > 0);
...
{
  char root[] = {'/', 't', 'm', 'p', '/', 'm', 'd', 'l', '_', 's', 't', 'a', 't', 'e',
                 '_', 'p', 'o', 'r', 't', '_', 'X', 'X', 'X', 'X', 'X', 'X', '\0'};
...
RA8_INTERNAL static void internal_mount_vfs(fw_fs_t* fs, fw_fs_ra8_vfs_state_t* state)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_io_vfs_mount("ram", s_vfs.mount));
