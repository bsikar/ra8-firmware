...
/**
 * @brief Return this test executable's site-descriptor storage.
 * @details Function-local static storage satisfies Rule 8.9 while the typed
 *          pointer preserves access across every fixture helper in this test
 *          translation unit. Each including test executable owns one object.
 * @return Writable site descriptor shared by the fixture helpers and cases.
 * @pre The caller serializes access to the file-local fixture.
 * @pre The caller resets the descriptor before starting a new scenario.
 * @post The returned address is stable for the executable lifetime.
 * @post Repeated calls return the same descriptor.
 * @note The function-local static keeps the object at the narrowest scope
 *       that still serves the header helpers and their including test TU.
 * @since 0.1.0
 */
RA8_INTERNAL static mdl_site_t* internal_mdl_fetch_test_site(void)
{
  static mdl_site_t s_site;
  return &s_site;
}

/**
 * @brief Return the optional governor injection slot for this test executable.
 * @details Function-local static storage satisfies Rule 8.9 while the returned
 *          pointer preserves TU-wide injection and observation by the fixture
 *          helper and its focused governor test. Each executable owns one slot.
 * @return Address of the borrowed governor pointer consumed by the fetch helper.
 * @pre The caller stores either nullptr or a governor that out-lives one run.
 * @pre The caller restores nullptr after an injected-governor scenario.
 * @post The returned address is stable for the executable lifetime.
 * @post The slot is initially nullptr through static storage initialization.
 * @note The slot remains TU-local while its pointer permits TU-wide access.
 * @since 0.1.0
 */
RA8_INTERNAL static mdl_governor_t** internal_mdl_fetch_test_governor_slot(void)
{
  static mdl_governor_t* s_governor;
  return &s_governor;
}
...
{
  mdl_site_t* const site = internal_mdl_fetch_test_site();
  memset(&s_mock, 0, sizeof(s_mock));
  memset(site, 0, sizeof(*site));
  s_refetch                  = false;
  s_mock.response_prefix     = s_jpeg_magic;
  s_mock.response_prefix_len = sizeof(s_jpeg_magic);
  (void)__builtin_snprintf(site->page_img_attr, sizeof(site->page_img_attr), "%s", "data-src");
  site->page_img_url_contains[0] = '\0';
}
                            ...
                            mdl_fetch_stats_t* out)
{
  ...
  mdl_session_init(&s_sess, &iface, "mdl/test", &diagnostic, false);
  mdl_fetch_ctx_t ctx = {.session        = &s_sess,
                         ...
                         .site           = internal_mdl_fetch_test_site(),
                         .gov            = *internal_mdl_fetch_test_governor_slot(),
