 ...
 */
typedef struct {
  /** Modelled response body. */
  const uint8_t* bytes;
  /** Complete body length. */
  size_t len;
  /** Next body byte offset. */
  size_t at;
  /** Successful begin count. */
  uint32_t begins;
  /** Successful cancel count. */
  uint32_t cancels;
  /** Requested format. */
  mdl_format_t format;
  /** Requested timeout. */
  ...
  /** Copied ETag condition. */
  char if_none_match[k_ra8_mdl_etag_max];
  /** Copied date condition. */
  char if_modified_since[k_ra8_mdl_http_date_max];
  /** Bad terminal total. */
  bool terminal_total_zero;
  /** Publish `response`. */
  bool response_override;
  /** Terminal metadata. */
  ra8_mdl_http_response_t response;
  /** Selected metadata fault. */
  t_mdl_read_fault_t read_fault;
