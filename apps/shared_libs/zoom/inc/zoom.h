 ...
 *
 *  - **full-screen zoom**, where @ref zoom_view_t::dst is the whole panel and a
 ...
 * pointer, @ref zoom_read_fn -- "give me this source rectangle as gray8" -- so a
 * `.rabook` image pool (@ref zoom_book.h), a JOF atlas paged through an
 * `ra8_tile_cache` (@ref zoom_tiles.h), and a unit-test pattern generator are
 ...
 * image plane*: the source scaled by @ref zoom_view_t::scale, so the plane is
 ...
 * zoom it is centred, and the letterbox is filled with @ref k_zoom_bg_gray.
 *
 ...
 * of at most @ref k_zoom_strip_rows_max destination rows, so the resident cost is
 ...
 * supplied by the caller in @ref zoom_scratch_t; the module allocates nothing and
 ...
 * *waveform*: under @ref k_zoom_policy_responsive an interactive burst flushes
 * with @ref k_zoom_refresh_fast (the panel's A2 waveform, ~10x faster than GC16
 * but bi-level), and @ref zoom_view_tick promotes the view to
 * @ref k_zoom_refresh_quality once the gesture has been still for
 * @ref zoom_view_t::settle_ms, repainting the same rectangle in all 16 levels.
 * @ref k_zoom_policy_quality opts out and pays full GC16 per step. The engine
 * emits the *decision* (@ref zoom_present_t) rather than calling the display PAL,
 ...
 *       ::zoom_view_render additionally writes through the single ra8_gfx
 *       framebuffer binding.
 *
 * @see zoom_book.h  Bind a `.rabook` image pool as a source.
 * @see zoom_tiles.h Bind an `ra8_tile_cache` tiled atlas as a source.
 ...
 *
 * @details
 * @ref k_zoom_scale_max is a *hard* ceiling on the engine, not a policy: a
 * caller narrows it further through @ref zoom_view_cfg_t::scale_max. The
 ...
 *
 * @ref k_zoom_bg_gray is 255 (white) rather than a mid grey because 255 is
 ...
 *
 * @invariant k_zoom_scale_min <= k_zoom_scale_max.
 * @invariant k_zoom_bg_gray is an exact multiple of the 16-level palette step.
 *
 * @par Example:
 * @code
...
typedef enum : uint8_t {
  k_zoom_scale_min      = 1U,   /**< 1:1 -- one source pixel per panel pixel. */
  k_zoom_scale_max      = 8U,   /**< Engine ceiling on integer magnification. */
  k_zoom_strip_rows_max = 32U,  /**< Destination rows composited per strip.   */
  k_zoom_bg_gray        = 255U, /**< Letterbox fill level (on-palette white). */
} zoom_limits_t;
 ...
 *
 * @details Caps `width * k_zoom_scale_max` (the widest magnified plane) far
 ...
 *
 * @invariant k_zoom_dim_max * k_zoom_scale_max < INT32_MAX.
 * @see zoom_source_init
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_zoom_dim_max = 65535U, /**< Largest accepted source width or height, pixels. */
} zoom_dim_t;
 ...
 *          @ref zoom_view_tick promotes a fast (bi-level) view to a full
 ...
 *
 * @invariant k_zoom_settle_ms_default > 0.
 * @see zoom_view_tick
 * @since 0.1.0
 */
typedef enum : uint16_t {
  k_zoom_settle_ms_default = 350U, /**< Default input-silence settle window, ms. */
} zoom_timing_t;
 ...
 *
 * @invariant Exactly one of the two values is present in @ref zoom_present_t.
 * @see zoom_view_present
 * @since 0.1.0
 */
typedef enum : uint8_t {
  k_zoom_refresh_fast    = 0U, /**< Interactive burst: panel A2 (bi-level, fast). */
  k_zoom_refresh_quality = 1U, /**< Settled: panel GC16 (all 16 levels, slow).    */
} zoom_refresh_t;
 ...
 * rather than one being chosen silently. @ref k_zoom_policy_responsive
 * trades tone for latency during a gesture and pays the quality repaint once,
 * on settle; @ref k_zoom_policy_quality never shows a bi-level frame and
 ...
 *
 * @invariant Under @ref k_zoom_policy_quality the engine never emits
 *            @ref k_zoom_refresh_fast, and @ref zoom_view_tick never
 ...
 */
typedef enum : uint8_t {
  k_zoom_policy_responsive = 0U, /**< Fast during a burst, quality once settled.  */
  k_zoom_policy_quality    = 1U, /**< Always quality; no bi-level frame is shown. */
} zoom_policy_t;
 ...
 *
 * @invariant k_zoom_pan_none names "no movement" and is the zero value.
 * @see zoom_view_pan_dir
 * @since 0.1.0
 */
typedef enum : uint8_t {
  k_zoom_pan_none  = 0U, /**< No movement.                */
  k_zoom_pan_left  = 1U, /**< Viewport travels toward -x. */
  k_zoom_pan_right = 2U, /**< Viewport travels toward +x. */
  k_zoom_pan_up    = 3U, /**< Viewport travels toward -y. */
  k_zoom_pan_down  = 4U, /**< Viewport travels toward +y. */
} zoom_pan_t;
 ...
 * the declared @ref zoom_source_t::width x @ref zoom_source_t::height, so
 ...
 *         verbatim to the caller of ::zoom_view_render.
 ...
 *
 * @note Called from ::zoom_view_render only; inherits its (non-)thread-safety.
 * @see zoom_source_init
 * @since 0.1.0
 */
typedef ra8_err_t (*zoom_read_fn)(void*    ctx,
                                  uint32_t x,
                                  uint32_t y,
                                  uint32_t w,
                                  uint32_t h,
 ...
 *          engine may address. Populate it with ::zoom_source_init or with
 *          one of the adapter binders, never by hand -- the initialiser is where
 *          the extent bound (@ref k_zoom_dim_max) is enforced.
 *
 * @invariant `width` and `height` are both in `[1, k_zoom_dim_max]`.
 ...
 */
typedef struct {
  zoom_read_fn read;   /**< gray8 sub-rectangle reader (never NULL once bound). */
 ...
 *
 * ::zoom_view_open derives @c strip_rows from @c strip_cap and rejects a
 ...
 *
 * @details Produced by ::zoom_view_present and consumed by the reader's
 ...
 *   (void)display_flush(d, r, (plan.refresh == k_zoom_refresh_fast)
...
typedef struct {
  ...
  zoom_refresh_t refresh; /**< Waveform class for that flush.               */
  bool           present; /**< False: nothing changed, do not flush at all. */
} zoom_present_t;

/**
 * @struct zoom_view_cfg_t
 * @brief Everything ::zoom_view_open needs; nothing it does not.
 *
 ...
 * @invariant `scale` lies in `[k_zoom_scale_min, effective scale_max]`.
 *
 * @par Example:
 * @code
 ...
 */
typedef struct {
  zoom_source_t  src;       /**< Bound source (see ::zoom_source_init).    */
  zoom_scratch_t scratch;   /**< Caller-owned composite buffers.           */
  ra8_ui_rect_t  dst;       /**< Viewport in framebuffer pixels.           */
  uint8_t        scale;     /**< Opening magnification (0 => scale_min).   */
  uint8_t        scale_max; /**< Ladder ceiling (0 => ::k_zoom_scale_max). */
  zoom_policy_t  policy;    /**< Waveform scheduling policy.               */
 ...
 *
 * @invariant `scale` lies in `[k_zoom_scale_min, scale_max]` whenever `active`.
 ...
 * @invariant `strip_rows >= 1` and `strip_rows <= k_zoom_strip_rows_max`.
 * @invariant `settle_armed` is false under ::k_zoom_policy_quality.
 *
 ...
 */
typedef struct {
  zoom_source_t  src;           /**< Bound source.                                  */
  zoom_scratch_t scratch;       /**< Caller-owned composite buffers.                */
  ra8_ui_rect_t  dst;           /**< Viewport in framebuffer pixels.                */
  ...
  zoom_policy_t  policy;        /**< Waveform scheduling policy.                    */
  zoom_refresh_t pending_kind;  /**< Waveform the owed flush should use.            */
 ...
 *          @ref k_zoom_dim_max at this seam.
 *
 ...
 * @param[in]  width  Full-resolution source width, `[1, k_zoom_dim_max]`.
 * @param[in]  height Full-resolution source height, `[1, k_zoom_dim_max]`.
 *
 ...
 *
 * @pre  @p out addresses writable storage for one ::zoom_source_t.
 ...
 */
[[nodiscard]] ra8_err_t
zoom_source_init(zoom_source_t* out, zoom_read_fn read, void* ctx, uint32_t width, uint32_t height);
 ...
 * view opens owing a @ref k_zoom_refresh_quality flush regardless of policy --
 * the first frame a reader sees must be the good one.
 *
 * @param[out] v   View to open (overwritten wholesale).
 * @param[in]  cfg Configuration; see ::zoom_view_cfg_t.
 *
 * @return ra8_err_t Error code.
 ...
 *
 * @see zoom_view_close
 * @see zoom_view_render
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t zoom_view_open(zoom_view_t* v, const zoom_view_cfg_t* cfg);
 ...
 * strictly less capable and is recoverable by calling ::zoom_view_close
 ...
 *
 * @pre  @p v was opened by ::zoom_view_open.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t zoom_view_rebind(zoom_view_t* v, const zoom_source_t* src, uint32_t now_ms);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t zoom_view_invalidate(zoom_view_t* v, uint32_t now_ms);
 ...
 * @retval k_ra8_err_null_ptr @p v is NULL.
 *
 * @pre  @p v addresses writable storage for one ::zoom_view_t.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t zoom_view_close(zoom_view_t* v);
 ...
 * @pre  @p v, if non-NULL, addresses a ::zoom_view_t.
 ...
 * @since 0.1.0
 */
[[nodiscard]] bool zoom_view_active(const zoom_view_t* v);
 ...
 *
 * @param[in,out] v       Open view.
 * @param[in]     scale   Requested magnification, `[k_zoom_scale_min, v->scale_max]`.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t zoom_view_set_scale(zoom_view_t* v,
                                            uint8_t      scale,
                                            int32_t      focus_x,
                                            int32_t      focus_y,
                                            uint32_t     now_ms);
 ...
 * @param[in] min   Ladder floor (0 is read as ::k_zoom_scale_min).
 ...
 * @see zoom_view_set_scale
 * @since 0.1.0
 */
[[nodiscard]] uint8_t zoom_scale_cycle(uint8_t scale, uint8_t min, uint8_t max);
 ...
 * @post The anchor satisfies the clamp invariant of ::zoom_view_t.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t zoom_view_pan(zoom_view_t* v, int32_t dx, int32_t dy, uint32_t now_ms);

/**
 * @brief Pan by a discrete step in one direction (the tap-band interaction).
 *
 * @details One step is @ref zoom_view_t::dst minus a one-eighth overlap on
 ...
 *
 * @param[in,out] v      Open view.
 * @param[in]     dir    Direction; ::k_zoom_pan_none is a no-op.
 * @param[in]     now_ms Current millisecond timestamp for the settle timer.
 *
 * @return ra8_err_t Error code.
 ...
 *
 * @pre  @p v was opened by ::zoom_view_open.
 * @pre  @p dir is a ::zoom_pan_t enumerator.
 * @post The anchor satisfies the clamp invariant of ::zoom_view_t.
 * @post ::k_zoom_pan_none leaves the view byte-identical.
 *
 * @note Not thread-safe.
 * @see zoom_view_pan
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t zoom_view_pan_dir(zoom_view_t* v, zoom_pan_t dir, uint32_t now_ms);
 ...
 *          next ::zoom_view_render will read, so a tiled adapter can turn it
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t zoom_view_window(const zoom_view_t* v, ra8_ui_rect_t* out);

/**
 * @brief Advance the settle timer; promote a bi-level view to full quality.
 *
 * @details Call once per main-loop iteration. Under
 *          ::k_zoom_policy_responsive an interactive change leaves the view
 *          armed; once @ref zoom_view_t::settle_ms of input silence have
 *          passed this arms a @ref k_zoom_refresh_quality repaint of the same
 ...
 *          Call it after ::zoom_view_render, not before.
 *
 * @param[in,out] v   Open view.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t zoom_view_present(zoom_view_t* v, zoom_present_t* out);
 ...
 * destination pixel of @ref zoom_view_t::dst is written -- image where the
 * source covers it, @ref k_zoom_bg_gray in the letterbox -- so the caller
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t zoom_view_render(zoom_view_t* v);
