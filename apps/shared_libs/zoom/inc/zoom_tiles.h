 ...
 * @see zoom_book.h  The `.rabook` figure source, for EPUB.
 *
 ...
 * @post On k_ra8_ok the grid invariants of ::zoom_tile_src_t hold.
 * @post On any error @p ts is not left partially bound.
 *
 * @note Not thread-safe.
 * @see zoom_tile_src_bind
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t zoom_tile_src_init(zoom_tile_src_t*  ts,
...
/**
 * @brief Present a bound tiled image to the zoom engine as a ::zoom_source_t.
 *
 * @param[in]  ts  Binding populated by ::zoom_tile_src_init.
 ...
 *
 * @pre  @p ts was populated by ::zoom_tile_src_init.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t zoom_tile_src_bind(zoom_tile_src_t* ts, zoom_source_t* out);

/**
 * @brief ::zoom_read_fn over a tiled atlas paged by an ra8_tile_cache.
 *
 * @details Bound by ::zoom_tile_src_bind; not normally called directly.
 ...
 *          rectangle.
 *
 * @param[in]  ctx        The ::zoom_tile_src_t binding.
 ...
 *
 * @pre  @p ctx is a bound ::zoom_tile_src_t.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t zoom_tile_read(void*    ctx,
                                       uint32_t x,
                                       uint32_t y,
                                       uint32_t w,
                                       uint32_t h,
 ...
 *          view reports the source pixels it will read (::zoom_view_window),
 ...
 * @param[in]     dir        Direction of travel; ::k_zoom_pan_none warms nothing.
 ...
 * @retval k_ra8_err_*         Propagated from ::zoom_view_window or the cache.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t zoom_tiles_prefetch(zoom_tile_src_t*   ts,
                                            const zoom_view_t* v,
                                            zoom_pan_t         dir,
                                            uint16_t           max_tiles,
                                            uint16_t*          out_warmed);
