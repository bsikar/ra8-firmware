 ...
 *          same pattern `comic_tiles.c` uses.
 *
 *
...
static const char* const s_tag = "zoom_tiles";

/* zoom_tiles_prefetch forwards a zoom_pan_t straight into the cache's
...
static_assert((uint8_t)k_zoom_pan_none == (uint8_t)k_ra8_tile_pan_none,
              "zoom_pan_t and ra8_tile_pan_dir_t must agree: none");
static_assert((uint8_t)k_zoom_pan_left == (uint8_t)k_ra8_tile_pan_left,
              "zoom_pan_t and ra8_tile_pan_dir_t must agree: left");
static_assert((uint8_t)k_zoom_pan_right == (uint8_t)k_ra8_tile_pan_right,
              "zoom_pan_t and ra8_tile_pan_dir_t must agree: right");
static_assert((uint8_t)k_zoom_pan_up == (uint8_t)k_ra8_tile_pan_up,
              "zoom_pan_t and ra8_tile_pan_dir_t must agree: up");
static_assert((uint8_t)k_zoom_pan_down == (uint8_t)k_ra8_tile_pan_down,
              "zoom_pan_t and ra8_tile_pan_dir_t must agree: down");
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_tile_fetch(zoom_tile_src_t*        ts,
                                     uint16_t                tx,
                                     uint16_t                ty,
                                     const zoom_tile_span_t* span,
                                     uint8_t*                out,
...
ra8_err_t zoom_tile_src_bind(zoom_tile_src_t* ts, zoom_source_t* out)
{
  ...
  return zoom_source_init(out, zoom_tile_read, ts, ts->width, ts->height);
}
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_walk_tile_row(zoom_tile_src_t*        ts,
                                        uint16_t                ty,
                                        uint16_t                tx0,
                                        uint16_t                tx1,
                                        const zoom_tile_span_t* span,
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_walk_tiles(zoom_tile_src_t*        ts,
 ...
 *
 * @details Two fail-closed rules, kept out of ::zoom_tile_read so the read
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_read_rect_ok(const zoom_tile_src_t* ts,
                                       uint32_t               x,
                                       uint32_t               y,
                                       uint32_t               w,
                                       uint32_t               h,
                         ...
                         uint32_t out_stride)
{
  ...
  zoom_tile_src_t* ts = (zoom_tile_src_t*)ctx;
  RA8_CHECK_NULL_PTR(ts->cache, s_tag, "binding was never initialised");
                              ...
                              uint16_t*          out_warmed)
{
  ...
  ra8_ui_rect_t win = {};
  RA8_RETURN_ON_ERROR(zoom_view_window(v, &win), s_tag, "visible window");
