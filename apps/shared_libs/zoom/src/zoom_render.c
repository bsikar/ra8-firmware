 ...
 * ::zoom_view_t::strip_rows destination rows and the resident cost is
 * `O(dst.w * strip_rows)`.
 *
 ...
 *     ::k_zoom_bg_gray, so the caller never pre-clears the viewport.
 ...
 *     re-roll the grain (see the zoom.h docblock).
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_fill_row(zoom_view_t*       v,
 ...
 *          seam's `w > 0` contract if it ever reached ::zoom_read_fn.
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_fill_strip(zoom_view_t*       v,
                                     const zoom_axis_t* ax,
                                     int32_t            top,
                                     int32_t            rows,
                                     int32_t*           cached_sy)
{
  ...
  (void)memset(v->scratch.strip, (int)k_zoom_bg_gray, (size_t)rows * stride);
  if (ax->count == 0U) {
 ...
 *          so ::zoom_view_render is only the strip loop. The dither phase is
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_emit_strip(zoom_view_t*       v,
                                     const zoom_axis_t* ax,
                                     int32_t            top,
                                     int32_t            rows,
                                     int32_t*           cached_sy)
 ...
 *
 * @details Split out of ::zoom_view_render so the render body is the strip
