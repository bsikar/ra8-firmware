/**
 * @file zoom_internal.h
 * @brief Module-private seam shared by the zoom state machine and its render.
 *
 * @details
 * zoom.c owns the viewport state and zoom_render.c owns the strip
 ...
 * from `inc/`, and production code outside `apps/shared_libs/zoom` must never call it.
 *
 ...
 * source row segment to read) and ::zoom_view_window (which source pixels are
