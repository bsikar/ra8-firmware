 ...
 *          composite lives in zoom_render.c.
 *
 *
...
RA8_PRIV void
priv_zoom_axis(int32_t anchor, int32_t extent, int32_t scale, int32_t src_dim, zoom_axis_t* out)
{
 ...
 * @return True when `dim` is in `[1, k_zoom_dim_max]`.
 ...
 * @post A true result guarantees `dim * k_zoom_scale_max` fits in int32.
...
{
  return (dim != 0U) && (dim <= (uint32_t)k_zoom_dim_max);
}
 ...
 * @post The result satisfies the anchor invariant of ::zoom_view_t.
 * @post No state is mutated.
 * @note Pure; thread-safe.
 * @since 0.1.0
 */
 ...
 * @post `v->settle_armed` is true only under ::k_zoom_policy_responsive.
 * @note Not thread-safe.
 * @since 0.1.0
 */
RA8_INTERNAL
...
{
  const bool responsive = (v->policy == k_zoom_policy_responsive);
  v->last_input_ms      = now_ms;
  v->pending            = true;
  v->pending_kind       = responsive ? k_zoom_refresh_fast : k_zoom_refresh_quality;
  v->settle_armed       = responsive;
}

/**
 * @brief Validate every pointer a configuration must carry.
 * @details Split out of ::zoom_view_open so each validation stage stays
...
static ra8_err_t internal_cfg_extent_ok(const zoom_view_cfg_t* cfg)
{
  ...
  if (!internal_dim_ok(cfg->src.width) || !internal_dim_ok(cfg->src.height)) {
    ra8_log_error(s_tag, "source extent is zero or exceeds k_zoom_dim_max");
    return k_ra8_err_invalid_arg;
  }
  return k_ra8_ok;
}
 ...
 * @post On k_ra8_ok `*out_strip_rows` is in `[1, k_zoom_strip_rows_max]`.
...
static ra8_err_t internal_cfg_scratch_ok(const zoom_view_cfg_t* cfg, uint16_t* out_strip_rows)
{
  ...
  if (rows > (uint32_t)k_zoom_strip_rows_max) {
    rows = (uint32_t)k_zoom_strip_rows_max;
  }
 ...
 * @post On k_ra8_ok `k_zoom_scale_min <= *out_scale <= *out_ceiling`.
 * @post On k_ra8_ok `*out_ceiling <= k_zoom_scale_max`.
 * @note Not thread-safe (logs).
 * @since 0.1.0
 */
RA8_INTERNAL
static ra8_err_t
internal_cfg_ladder_ok(const zoom_view_cfg_t* cfg, uint8_t* out_scale, uint8_t* out_ceiling)
{
  /* The "0 means default" rewrite happens first and lifts both values to at
   * least k_zoom_scale_min, so a `< min` half in either check below could
  ...
  const uint8_t ceiling = (cfg->scale_max == 0U) ? (uint8_t)k_zoom_scale_max : cfg->scale_max;
  const uint8_t opening = (cfg->scale == 0U) ? (uint8_t)k_zoom_scale_min : cfg->scale;
  if (ceiling > (uint8_t)k_zoom_scale_max) {
 ...
 *          ::zoom_view_open so that function is "check, then adopt".
 *
 * @param[in]  cfg            Candidate configuration (already known non-NULL).
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_cfg_ok(const zoom_view_cfg_t* cfg,
 ...
 *
 * @details The last step of ::zoom_view_open, split out so the open path
 ...
 */
RA8_INTERNAL
static void internal_view_adopt(zoom_view_t*           v,
                                ...
                                uint16_t               strip_rows)
{
  ...
  v->scale_max     = ceiling;
  v->policy        = cfg->policy;
  v->settle_ms     = (cfg->settle_ms == 0U) ? (uint16_t)k_zoom_settle_ms_default : cfg->settle_ms;
...
{
  const uint8_t floor_v   = (min == 0U) ? (uint8_t)k_zoom_scale_min : min;
                              ...
                              uint32_t     now_ms)
{
  ...
  if ((scale < (uint8_t)k_zoom_scale_min) || (scale > v->scale_max)) {
...
ra8_err_t zoom_view_pan_dir(zoom_view_t* v, zoom_pan_t dir, uint32_t now_ms)
{
  ...
  int32_t       dy     = 0;
  switch (dir) {
    case k_zoom_pan_left:
      dx = -step_x;
      break;
    case k_zoom_pan_right:
      dx = step_x;
      break;
    case k_zoom_pan_up:
      dy = -step_y;
      break;
    case k_zoom_pan_down:
      dy = step_y;
      break;
    case k_zoom_pan_none:
    default:
      break;
  }
  return zoom_view_pan(v, dx, dy, now_ms);
}
