...
typedef enum : uint32_t {
  ...
  k_t_huge_dim  = 1000000U,    /**< A source extent past k_zoom_dim_max.      */
...
/**
 * @brief ::zoom_read_fn over the synthetic source, with an injectable fault.
 * @param[in]  ctx        Unused.
 ...
 */
static void t_make_src(zoom_source_t* src, uint32_t w, uint32_t h)
{
  TEST_ASSERT_EQ(k_ra8_ok, zoom_source_init(src, t_read, nullptr, w, h));
}
 ...
 */
static void t_make_cfg(zoom_view_cfg_t* cfg, const zoom_source_t* src)
{
  ...
  const zoom_view_cfg_t base = {
    ...
    .scale_max = (uint8_t)k_zoom_scale_max,
 ...
 *
 * @par MC/DC:
 * Decision apps/shared_libs/zoom/src/zoom.c@zoom_source_init
 ...
 *
 * @par MC/DC:
 * Decision apps/shared_libs/zoom/src/zoom.c@internal_dim_ok
 * `return (dim != 0U) && (dim <= k_zoom_dim_max)` (2 conditions):
...
static void t_source_init_validates(void)
{
  ...
  zoom_source_t src = {};
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_source_init(nullptr, t_read, nullptr, 1U, 1U));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_source_init(&src, nullptr, nullptr, 1U, 1U));
  /* V1 */
  TEST_ASSERT_EQ(k_ra8_ok, zoom_source_init(&src, t_read, nullptr, k_t_src_w, k_t_src_h));
  TEST_ASSERT_EQ(k_t_src_w, src.width);
  /* V2 */
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, zoom_source_init(&src, t_read, nullptr, 0U, k_t_src_h));
  /* V3 */
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 zoom_source_init(&src, t_read, nullptr, k_t_src_w, k_t_huge_dim));
 ...
 */
static void t_view_open_scratch_and_ladder(const zoom_source_t* src)
{
  zoom_view_cfg_t cfg = {};
  zoom_view_t     v   = {};
  /* V8 */
  ...
  TEST_ASSERT_EQ(k_ra8_err_no_mem, zoom_view_open(&v, &cfg));
  /* V9 */
  t_make_cfg(&cfg, src);
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, zoom_view_open(&v, &cfg));
  /* V13 */
  t_make_cfg(&cfg, src);
  ...
  TEST_ASSERT_EQ(k_zoom_scale_min, v.scale);
  TEST_ASSERT_EQ(k_zoom_scale_max, v.scale_max);
  TEST_ASSERT_EQ(k_zoom_settle_ms_default, v.settle_ms);
}
 ...
 *
 * @par MC/DC:
 * Decision apps/shared_libs/zoom/src/zoom.c@internal_cfg_extent_ok
 ...
 *
 * @par MC/DC:
 * Decision apps/shared_libs/zoom/src/zoom.c@internal_cfg_scratch_ok
...
static void t_view_open_validates(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_view_open(nullptr, &cfg));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_view_open(&v, nullptr));
  ...
  cfg.scale_max = 4U;
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_open(&v, &cfg));
  TEST_ASSERT(zoom_view_active(&v));

  /* A missing scratch pointer is rejected before any sizing. */
  t_make_cfg(&cfg, &src);
  cfg.scratch.strip = nullptr;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_view_open(&v, &cfg));
  TEST_ASSERT(!zoom_view_active(&v));
 ...
 *
 * @par MC/DC:
 * Decision apps/shared_libs/zoom/src/zoom.c@zoom_view_active
...
static void t_view_active_and_close(void)
{
  ...
  t_make_cfg(&cfg, &src);
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_open(&v, &cfg));
  TEST_ASSERT(zoom_view_active(&v));       /* V1 */
  TEST_ASSERT(!zoom_view_active(nullptr)); /* V2 */
   ...
   * zoom_view_invalidate's docblock. */
  zoom_present_t drained = {};
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &drained));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan(&v, 0, 0, 10U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &drained));
  TEST_ASSERT(!drained.present);
  const int32_t held_x = v.anchor_x;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_view_invalidate(nullptr, 10U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_invalidate(&v, 10U));
  TEST_ASSERT_EQ(held_x, v.anchor_x);
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &drained));
  TEST_ASSERT(drained.present);

  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_view_close(nullptr));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_close(&v));
  TEST_ASSERT(!zoom_view_active(&v)); /* V3 */
  /* Closing twice is not an error, and a closed view refuses every operation. */
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_close(&v));
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, zoom_view_pan(&v, 1, 0, 0U));
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, zoom_view_pan_dir(&v, k_zoom_pan_left, 0U));
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, zoom_view_set_scale(&v, 2U, 0, 0, 0U));
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, zoom_view_render(&v));
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, zoom_view_invalidate(&v, 0U));
  ra8_ui_rect_t  win  = {};
  zoom_present_t plan = {};
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, zoom_view_window(&v, &win));
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, zoom_view_present(&v, &plan));
  TEST_ASSERT(!zoom_view_tick(&v, (uint32_t)k_t_settle * 4U));
 ...
 *
 * @par MC/DC:
 * Decision apps/shared_libs/zoom/src/zoom.c@zoom_view_rebind
 ...
 * through zoom_source_init(), which would reject V2/V3 first -- the guard
...
static void t_rebind_keeps_zoom(void)
{
  ...
  t_make_cfg(&cfg, &src);
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_open(&v, &cfg));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_set_scale(&v, 4U, 0, 0, 0U));

  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_view_rebind(&v, nullptr, 0U));
  zoom_source_t bad = src;
  bad.read          = nullptr;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_view_rebind(&v, &bad, 0U));

  /* V1: a page turn keeps the magnification -- the deliberate product choice. */
  zoom_source_t next = {};
  t_make_src(&next, k_t_src_w, k_t_src_h);
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_rebind(&v, &next, 1U));
  TEST_ASSERT_EQ(4U, v.scale);

  /* V2 */
  zoom_source_t zero_w = next;
  zero_w.width         = 0U;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, zoom_view_rebind(&v, &zero_w, 1U));
  /* V3 */
  zoom_source_t huge_h = next;
  huge_h.height        = (uint32_t)k_t_huge_dim;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, zoom_view_rebind(&v, &huge_h, 1U));

  /* A shorter page re-clamps the anchor instead of showing past its bottom. */
  zoom_source_t shorter = {};
  t_make_src(&shorter, k_t_src_w, (uint32_t)k_t_view_h);
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_rebind(&v, &shorter, 2U));
  ra8_ui_rect_t win = {};
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_window(&v, &win));
  TEST_ASSERT((win.y + win.h) <= (int32_t)k_t_view_h);

  /* A closed view cannot be pointed at a new page: the reader must reopen. */
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_close(&v));
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, zoom_view_rebind(&v, &next, 3U));
 ...
 *
 * @par MC/DC:
 * Decision apps/shared_libs/zoom/src/zoom.c@zoom_view_set_scale
 * `if ((scale < k_zoom_scale_min) || (scale > v->scale_max))` (2 conditions):
...
static void t_set_scale_keeps_focus(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_view_set_scale(nullptr, 2U, 0, 0, 0U));
  /* V2 */
  TEST_ASSERT_EQ(k_ra8_err_out_of_range, zoom_view_set_scale(&v, 0U, 0, 0, 0U));
  /* V3 */
  TEST_ASSERT_EQ(k_ra8_err_out_of_range, zoom_view_set_scale(&v, 9U, 0, 0, 0U));

  /* Park the viewport mid-source so the focus maths is not against a clamp. */
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan(&v, 8, 8, 0U));
  ra8_ui_rect_t before = {};
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_window(&v, &before));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_set_scale(&v, 2U, focus_x, focus_y, 1U));
  TEST_ASSERT_EQ(2U, v.scale);
  ra8_ui_rect_t after = {};
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_window(&v, &after));
  ...
  zoom_present_t drain = {};
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &drain));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_set_scale(&v, 2U, focus_x, focus_y, 2U));
  zoom_present_t none = {};
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &none));
...
static void t_scale_cycle_ladder(void)
{
  ...
  TEST_ASSERT_EQ(2U, zoom_scale_cycle(1U, 1U, 4U));
  TEST_ASSERT_EQ(4U, zoom_scale_cycle(2U, 1U, 4U));
  TEST_ASSERT_EQ(1U, zoom_scale_cycle(4U, 1U, 4U));
  /* A ceiling that is not a power of two still ends exactly on it. */
  TEST_ASSERT_EQ(2U, zoom_scale_cycle(1U, 1U, 3U));
  TEST_ASSERT_EQ(3U, zoom_scale_cycle(2U, 1U, 3U));
  TEST_ASSERT_EQ(1U, zoom_scale_cycle(3U, 1U, 3U));
  /* Degenerate inputs land inside the ladder rather than wrapping. */
  TEST_ASSERT_EQ(k_zoom_scale_min, zoom_scale_cycle(0U, 0U, 0U));
  TEST_ASSERT_EQ(4U, zoom_scale_cycle(0U, 4U, 8U));
  TEST_ASSERT_EQ(5U, zoom_scale_cycle(9U, 5U, 2U));
 ...
 *
 * @par MC/DC:
 * Decision apps/shared_libs/zoom/src/zoom.c@zoom_view_pan
...
static void t_pan_clamps_and_marks_dirty(void)
{
  ...
  t_make_cfg(&cfg, &src);
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_open(&v, &cfg));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_view_pan(nullptr, 0, 0, 0U));

  /* Park mid-source so a zero pan really is a no-move rather than a clamp. */
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan(&v, 8, 8, 0U));
  zoom_present_t plan = {};
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &plan));

  /* V1 */
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan(&v, 0, 0, 1U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &plan));
  TEST_ASSERT(!plan.present);
  /* V2 */
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan(&v, 4, 0, 2U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &plan));
  TEST_ASSERT(plan.present);
  /* V3 */
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan(&v, 0, 4, 3U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &plan));
  TEST_ASSERT(plan.present);
  ...
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan(&v, k_t_big_delta, k_t_big_delta, 4U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &plan));
  ra8_ui_rect_t win = {};
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_window(&v, &win));
  TEST_ASSERT_EQ(k_t_src_w, win.x + win.w);
  TEST_ASSERT_EQ(k_t_src_h, win.y + win.h);
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan(&v, k_t_big_delta, k_t_big_delta, 5U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &plan));
  TEST_ASSERT(!plan.present);
  /* ...and the same at the other end, which is the negative saturation arm. */
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan(&v, -k_t_big_delta, -k_t_big_delta, 6U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_window(&v, &win));
...
static void t_pan_dir_steps_with_overlap(void)
{
  ...
  t_make_cfg(&cfg, &src);
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_open(&v, &cfg));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_view_pan_dir(nullptr, k_zoom_pan_left, 0U));

  const int32_t before = v.anchor_x;
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan_dir(&v, k_zoom_pan_none, 0U));
  TEST_ASSERT_EQ(before, v.anchor_x);

  /* One step advances by the viewport less a one-eighth overlap. */
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan_dir(&v, k_zoom_pan_right, 1U));
  TEST_ASSERT_EQ(k_t_view_w - ((int32_t)k_t_view_w / 8), v.anchor_x);
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan_dir(&v, k_zoom_pan_left, 2U));
  TEST_ASSERT_EQ(0, v.anchor_x);
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan_dir(&v, k_zoom_pan_down, 3U));
  TEST_ASSERT_EQ(k_t_view_h - ((int32_t)k_t_view_h / 8), v.anchor_y);
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan_dir(&v, k_zoom_pan_up, 4U));
 ...
 *
 * @par MC/DC:
 * Decision apps/shared_libs/zoom/src/zoom.c@zoom_view_tick
...
static void t_tick_settles_to_quality(void)
{
  ...
  t_make_cfg(&cfg, &src);
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_open(&v, &cfg));
  TEST_ASSERT(!zoom_view_tick(nullptr, 0U));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &plan));
  TEST_ASSERT(plan.present);
  TEST_ASSERT_EQ(k_zoom_refresh_quality, plan.refresh);
  TEST_ASSERT_EQ(k_t_view_w, plan.rect.w);

  /* V3: nothing armed yet, so no settle can be due however long we wait. */
  TEST_ASSERT(!zoom_view_tick(&v, (uint32_t)k_t_settle * 10U));

  /* An interactive pan under the responsive policy owes a FAST flush... */
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan(&v, 4, 0, 1000U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &plan));
  TEST_ASSERT_EQ(k_zoom_refresh_fast, plan.refresh);
  /* ...and nothing more until the settle window has elapsed. */
  TEST_ASSERT(!zoom_view_tick(&v, 1000U + (uint32_t)k_t_settle - 1U));
  /* V1 */
  TEST_ASSERT(zoom_view_tick(&v, 1000U + (uint32_t)k_t_settle));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &plan));
  TEST_ASSERT(plan.present);
  TEST_ASSERT_EQ(k_zoom_refresh_quality, plan.refresh);
  /* The settle disarms itself, so a reader repaints once, not per tick. */
  TEST_ASSERT(!zoom_view_tick(&v, 1000U + ((uint32_t)k_t_settle * 4U)));

  /* V2 */
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan(&v, 4, 0, 2000U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_close(&v));
  TEST_ASSERT(!zoom_view_tick(&v, 2000U + ((uint32_t)k_t_settle * 4U)));

  /* Under the quality policy no bi-level frame is ever shown and nothing arms. */
  t_make_cfg(&cfg, &src);
  cfg.policy = k_zoom_policy_quality;
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_open(&v, &cfg));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &plan));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan(&v, 4, 0, 3000U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_present(&v, &plan));
  TEST_ASSERT_EQ(k_zoom_refresh_quality, plan.refresh);
  TEST_ASSERT(!zoom_view_tick(&v, 3000U + ((uint32_t)k_t_settle * 4U)));

  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_view_present(&v, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_view_present(nullptr, &plan));
...
static void t_window_reports_visible_source(void)
{
  ...
  t_make_cfg(&cfg, &src);
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_open(&v, &cfg));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_view_window(&v, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_view_window(nullptr, &win));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_set_scale(&v, 1U, 0, 0, 0U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_rebind(&v, &tiny, 0U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_window(&v, &win));
  TEST_ASSERT_EQ(0, win.x);
 ...
 */
static void t_render_magnified_and_letterboxed(zoom_view_t* v)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_set_scale(v, 2U, (int32_t)k_t_dst_x, (int32_t)k_t_dst_y, 0U));
  s_reads = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_render(v));
  TEST_ASSERT_EQ(k_t_view_h / 2U, s_reads);
  for (int32_t r = 0; r < (int32_t)k_t_view_h; ++r) {
    for (int32_t c = 0; c < (int32_t)k_t_view_w; ++c) {
      const int32_t source_x = (v->anchor_x + c) / 2;
      const int32_t source_y = (v->anchor_y + r) / 2;
      const uint8_t sample   = t_sample((uint32_t)source_x, (uint32_t)source_y);
  ...
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_set_scale(v, 1U, 0, 0, 0U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_rebind(v, &small, 0U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_render(v));
  const uint8_t bg_level = ra8_gfx_dither_gray4_level((uint8_t)k_zoom_bg_gray, 0, 0);
  ...
  zoom_view_t broken = *v;
  broken.strip_rows  = 0U;
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, zoom_view_render(&broken));
   ...
   * back as background. */
  zoom_view_t adrift = *v;
  adrift.anchor_x    = k_t_axis_off;
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_render(&adrift));
  const uint8_t off_level = ra8_gfx_dither_gray4_level((uint8_t)k_zoom_bg_gray, 0, 0);
 ...
 *
 * @par MC/DC:
 * Decision apps/shared_libs/zoom/src/zoom_render.c@internal_fill_row
...
static void t_render_magnifies_and_dithers(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_open(&v, &cfg));
  t_bind_fb();
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, zoom_view_render(nullptr));
...
static void t_render_propagates_source_failure(void)
{
  ...
  s_reads   = 0U;
  s_fail_on = 3U;
  TEST_ASSERT_EQ(k_ra8_err_hw_error, zoom_view_render(&v));
  s_fail_on = 0U;
...
static void t_dither_phase_is_pan_stable(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_pan(&v, 7, 3, 0U));
  TEST_ASSERT_EQ(k_ra8_ok, zoom_view_render(&v));
