...
internal_source_read(void* ctx, uint64_t offset, uint8_t* buf, uint32_t len)
{
  ...
  if (got != len) {
    return k_ra8_err_hw_error; /* GCOVR_EXCL_LINE -- ra8_vsource_loader clamps len to o->size */
  }
  return k_ra8_ok;
}
 ...
 */
RA8_INTERNAL static ra8_err_t
internal_cache_bind(const rabook_import_compiler_ctx_t* ctx, import_stream_t* ss, uint32_t size)
{
 ...
 *          with `epub_open_streamed` reading through `ra8_vmem_stream_read`.
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_stream_open(const rabook_import_compiler_ctx_t* ctx,
                                                   ...
                                                   import_stream_t*                    ss)
{
  ...
  if (err == k_ra8_ok) {
    ...
    err = epub_open_streamed(&media, epub_path, ctx->epub);
  }
  if (err != k_ra8_ok) {
                                        ...
                                        const char*     out_path)
{
   ...
   * the bufs/scr by `rabook_compile_from_epub`, so a NULL cookie field still
   * yields k_ra8_err_null_ptr without restating those guards here. */
  const rabook_import_compiler_ctx_t* ctx = (const rabook_import_compiler_ctx_t*)compile_ctx;
 ...
 *          @p ctx->blob_buf; the blob is then run through @ref book_validate
 ...
 * @post On success @p out_path holds a `book_validate`-clean blob.
 * @post On any error @p out_path is not written.
 * @note Not thread-safe.
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t
internal_dispatch_and_cache(const rabook_import_compiler_m33_ctx_t* ctx,
                            uint32_t                                epub_len,
                            ra8_fs_mount_t*                         mount,
                            const char*                             out_path)
{
  ...
  err = book_validate(ctx->blob_buf, (size_t)blob_len);
  if (err != k_ra8_ok) {
    return err;
  }
 ...
 * @post On `k_ra8_ok`, @p out_path holds a `book_validate`-clean blob.
 * @post On any error, @p out_path is not written.
 * @note Not thread-safe.
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t
internal_fallback_or_propagate(const rabook_import_compiler_m33_ctx_t* ctx,
                               ra8_err_t                               err,
                               ra8_fs_mount_t*                         mount,
                               const char*                             epub_path,
                               const char*                             out_path)
{
  ...
  return rabook_import_compile_adapter(ctx->fallback, mount, epub_path, out_path);
}
 ...
 */
RA8_INTERNAL static ra8_err_t
internal_offload_or_fallback(const rabook_import_compiler_m33_ctx_t* ctx,
                             uint32_t                                epub_len,
                             ra8_fs_mount_t*                         mount,
                             const char*                             epub_path,
                             const char*                             out_path)
                                            ...
                                            const char*     out_path)
{
  ...
  rabook_import_compiler_m33_ctx_t* ctx = (rabook_import_compiler_m33_ctx_t*)compile_ctx;
