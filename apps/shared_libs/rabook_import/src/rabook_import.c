...
RA8_INTERNAL static uint32_t internal_crc32_block(uint32_t crc, const uint8_t* data, uint32_t len)
{
  ...
  for (uint32_t i = 0U; i < len; i++) {
    ...
    for (uint8_t b = 0U; b < (uint8_t)k_crc32_bits; b++) {
      const bool lsb = (running_crc & 1U) != 0U;
      running_crc >>= 1U;
      if (lsb) {
        running_crc ^= (uint32_t)k_crc32_poly;
      }
    }
  }
  return running_crc;
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_crc_stream_read_chunk(rabook_import_read_fn read_fn,
                                                             void*                 read_ctx,
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_crc_stream_validate_args(rabook_import_read_fn read_fn,
                                                                const uint8_t*        buf,
...
{
  if (read_fn == nullptr) {
    ra8_log_error(s_tag, "read_fn is NULL");
...
RA8_PRIV ra8_err_t priv_rabook_import_crc_stream(rabook_import_read_fn read_fn,
                                                 void*                 read_ctx,
                                                 uint64_t              expected_size,
                                                 uint8_t*              buf,
                                                 uint32_t              cap,
...
RA8_TEST_HELPER ra8_err_t rabook_import_crc_stream_test(rabook_import_read_fn read_fn,
                                                        void*                 read_ctx,
                                                        uint64_t              expected_size,
                                                        uint8_t*              buf,
                                                        uint32_t              cap,
                                                        ...
                                                        uint32_t*             out_crc)
{
  return priv_rabook_import_crc_stream(read_fn,
                                       read_ctx,
                                       expected_size,
                                       buf,
                                       cap,
                                                  ...
                                                  uint32_t*      out_crc)
{
  ...
  return priv_rabook_import_crc_stream(internal_import_fs_read,
                                       file,
                                       expected_size,
                                       buf,
                                       cap,
 ...
 * @param[in]  cap       Capacity of @p out (@ref k_rabook_import_name_cap).
 * @return Error code.
 ...
 * @param[in]  stem       Source-name stem (NUL-terminated).
 * @param[out] cache_name Receives `<stem>.rabook` (cap @ref k_rabook_import_name_cap).
...
internal_derive_names(const char* stem, char* cache_name, char* tmp_name, char* mark_name)
{
  ...
  const uint32_t           cap       = (uint32_t)k_rabook_import_name_cap;
 ...
 */
RA8_INTERNAL static ra8_err_t
internal_read_stamp(ra8_fs_mount_t* mount, const char* path, rabook_import_stamp_t* out)
{
 ...
 * @details Compares every serialized field explicitly so padding bytes cannot
 *          affect cache freshness. Focused vectors vary each field while the
 *          preceding comparisons remain true.
 * @param[in] mount     Mounted volume.
                                                 ...
                                                 const rabook_import_stamp_t* want)
{
  ...
  rabook_import_stamp_t got = {};
  ra8_err_t             err = internal_read_stamp(mount, mark_path, &got);
  if (err != k_ra8_ok) {
    return false;
  }
  return (got.magic == want->magic) && (got.format_version == want->format_version) &&
         (got.importer_version == want->importer_version) &&
         (got.source_size == want->source_size) && (got.source_crc32 == want->source_crc32);
}
 ...
 */
RA8_INTERNAL static ra8_err_t
internal_write_stamp(ra8_fs_mount_t* mount, const char* path, const rabook_import_stamp_t* stamp)
{
  RA8_CHECK_NULL_PTR(mount, s_tag, "mount");
  RA8_CHECK_NULL_PTR(path, s_tag, "path");
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_compile_and_cache(const rabook_import_cfg_t*   cfg,
                                                         const char*                  epub_path,
 ...
 * @pre Called first from @ref rabook_import_open.
 ...
 */
RA8_INTERNAL static ra8_err_t
internal_validate_open_args(const rabook_import_cfg_t*     cfg,
                            const char*                    epub_path,
                             ...
                             rabook_import_outcome_t*   out_outcome)
{
  ...
  char stem[k_rabook_import_name_cap];
  err = internal_derive_stem(epub_path, stem, (uint32_t)k_rabook_import_name_cap);
  if (err != k_ra8_ok) {
    return err;
  }
  char cache_name[k_rabook_import_name_cap];
  char tmp_name[k_rabook_import_name_cap];
  char mark_name[k_rabook_import_name_cap];
  ...
  const rabook_import_stamp_t want = {
    .magic            = (uint32_t)k_rabook_import_stamp_magic,
  ...
  if (internal_cache_is_fresh(cfg->mount, mark_name, &want)) {
    if (internal_file_exists(cfg->mount, cache_name)) {
      *out_outcome = k_rabook_import_hit;
  ...
  *out_outcome = k_rabook_import_compiled;
  return internal_copy_name(out_cache_path, cache_path_cap, cache_name);
}
