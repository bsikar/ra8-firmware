 ...
 *
 * @details
 * Exercises @ref rabook_import_compile_adapter_m33 -- the import seam binding
 ...
 *    byte-identical and passes @ref book_validate.
 *  - Reject path: a mock that returns a corrupted blob -> @ref book_validate
 ...
 *
 * @par MC/DC:
 * Cited decision: apps/shared_libs/rabook_import/src/rabook_import_compiler.c@internal_is_dispatch_failure
...
RA8_INTERNAL static ra8_fs_mount_t* internal_fresh_volume_with_epub(const char* src_path)
{
  ...
  static const uint8_t k_dummy_epub[] = {'P', 'K', 3U, 4U};
                                                            ...
                                                            uint32_t*      out_len)
{
  ...
  (void)memcpy(out_buf, s_parity_golden, (size_t)k_parity_golden_len);
...
{
  static struct {
    book_chapter_t             chapters[k_ic_chapter_cap];  /**< Chapter records.                */
    book_node_t                nodes[k_ic_node_cap];        /**< Parsed document nodes.          */
    book_attr_t                attrs[k_ic_attr_cap];        /**< Parsed node attributes.         */
    book_stylesheet_t          styles[k_ic_style_cap];      /**< Stylesheet records.             */
    book_image_t               images[k_ic_image_cap];      /**< Imported image records.         */
    char                       strpool[k_ic_string_cap];    /**< Builder string arena.           */
    uint8_t                    imgpool[k_ic_imgpool_cap];   /**< Builder image arena.            */
    uint8_t                    out[k_ic_out_cap];           /**< Final RABOOK output bytes.      */
    uint8_t                    xhtml[k_ic_xhtml_cap];       /**< XHTML source scratch.           */
    ra8_rabook_xml_workspace_t xml_workspace;               /**< XML parser workspace.           */
    uint8_t                    image_raw[k_ic_imgraw_cap];  /**< Encoded image scratch.          */
    uint8_t                    img_scratch[k_ic_arena_cap]; /**< Decoder arena bytes.            */
    uint8_t                    gray[k_ic_gray_cap];         /**< Normalized grayscale pixels.    */
    char                       css[k_ic_css_cap];           /**< Stylesheet source scratch.      */
    ra8_img_arena_t            arena;                       /**< Image-decoder arena descriptor. */
  } s_storage;
  s_ic_bufs = (ra8_rabook_buffers_t){
    .chapters       = s_storage.chapters,
    .chapter_cap    = (uint32_t)k_ic_chapter_cap,
    .nodes          = s_storage.nodes,
    .node_cap       = (uint32_t)k_ic_node_cap,
    .attrs          = s_storage.attrs,
    .attr_cap       = (uint32_t)k_ic_attr_cap,
    .stylesheets    = s_storage.styles,
    .stylesheet_cap = (uint32_t)k_ic_style_cap,
    .images         = s_storage.images,
    .image_cap      = (uint32_t)k_ic_image_cap,
    .string_pool    = s_storage.strpool,
    .string_cap     = (uint32_t)k_ic_string_cap,
    .image_pool     = s_storage.imgpool,
  ...
  s_ic_scr        = (ra8_rabook_pipeline_scratch_t){
    ...
    .image_raw     = s_storage.image_raw,
    .image_cap     = sizeof(s_storage.image_raw),
    .img_arena     = &s_storage.arena,
    .gray          = s_storage.gray,
    .gray_cap      = (uint32_t)k_ic_gray_cap,
    .css           = s_storage.css,
    .css_cap       = sizeof(s_storage.css),
    .xml_workspace = &s_storage.xml_workspace,
  };
}
...
RA8_INTERNAL static void internal_make_incore_ctx(rabook_import_compiler_ctx_t* ctx)
{
  static struct {
    /** Cache bytes. */
    uint8_t cache_frames[k_ic_cache_frames * k_ic_cache_frame_bytes];
    /** Frame residency metadata. */
    ra8_vmem_frame_t cache_meta[k_ic_cache_frames];
    /** Cached page keys. */
    ra8_vmem_key_t cache_keys[k_ic_cache_frames];
    /** Cache hash-chain heads. */
    int32_t cache_buckets[k_ic_cache_buckets];
    /** Source page-cache descriptor. */
    ra8_vmem_t cache;
    /** Parsed EPUB state. */
    epub_book_t book;
  } s_storage;
  internal_make_incore_views();
  s_storage.book = (epub_book_t){};
  *ctx           = (rabook_import_compiler_ctx_t){
    .epub               = &s_storage.book,
    .cache              = &s_storage.cache,
    .cache_frames       = s_storage.cache_frames,
    .cache_frame_bytes  = (uint32_t)k_ic_cache_frame_bytes,
    .cache_frame_count  = (uint32_t)k_ic_cache_frames,
    .cache_meta         = s_storage.cache_meta,
    .cache_keys         = s_storage.cache_keys,
    .cache_buckets      = s_storage.cache_buckets,
...
RA8_INTERNAL static void internal_assert_output_is_golden(ra8_fs_mount_t* mount, const char* path)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, book_validate(s_readback, (size_t)got));
}
...
/**
 * @brief Bind a cookie over function-local static buffers with @p dispatch.
 * @param[out] ctx      Cookie to populate (non-NULL).
 * @param[in]  dispatch Dispatch seam to bind.
 * @pre @p ctx is writable.
 * @pre The function-local adapter buffers are available.
 * @post @p ctx references the persistent EPUB/blob buffers and @p dispatch.
 * @post No global state beyond @p ctx is mutated.
 * @note Not thread-safe (returns a view over shared static buffers). @details Implements the cookie fixture operation used only by this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_make_cookie(rabook_import_compiler_m33_ctx_t* ctx,
                                              ra8_dual_core_compile_dispatch_fn dispatch)
{
  static uint8_t s_epub_load[k_epub_load_cap];
  static uint8_t s_blob[k_blob_cap];
  *ctx = (rabook_import_compiler_m33_ctx_t){
...
{
  TEST_BEGIN("rabook_import_m33: golden dispatch -> validated .rabook written");
  ra8_fs_mount_t* mount = internal_fresh_volume_with_epub("SRC.EPB");

  rabook_import_compiler_m33_ctx_t ctx = {};
  internal_make_cookie(&ctx, internal_mock_dispatch_golden);
  TEST_ASSERT_EQ(k_ra8_ok, rabook_import_compile_adapter_m33(&ctx, mount, "SRC.EPB", "OUT.RAB"));
  ...
  TEST_END("rabook_import_m33: golden dispatch -> validated .rabook written");
}
 ...
 * blob fails `book_validate`, so `s_fallback_or_propagate` reaches the
...
{
  TEST_BEGIN("rabook_import_m33: corrupt blob -> rejected, no output");
  ra8_fs_mount_t* mount = internal_fresh_volume_with_epub("SRC.EPB");

  rabook_import_compiler_m33_ctx_t ctx = {};
  internal_make_cookie(&ctx, internal_mock_dispatch_corrupt);
  TEST_ASSERT(rabook_import_compile_adapter_m33(&ctx, mount, "SRC.EPB", "OUT.RAB") != k_ra8_ok);
  ...
  internal_teardown(mount);
  TEST_END("rabook_import_m33: corrupt blob -> rejected, no output");
}
 ...
 * rabook_import_compiler.c). The mock returns k_ra8_err_hw_error with no
...
{
  TEST_BEGIN("rabook_import_m33: dispatch error -> propagated, no output");
  ra8_fs_mount_t* mount = internal_fresh_volume_with_epub("SRC.EPB");

  rabook_import_compiler_m33_ctx_t ctx = {};
  internal_make_cookie(&ctx, internal_mock_dispatch_error);
  TEST_ASSERT_EQ(k_ra8_err_hw_error,
                 rabook_import_compile_adapter_m33(&ctx, mount, "SRC.EPB", "OUT.RAB"));

  ra8_fs_file_t* file = nullptr;
  TEST_ASSERT(ra8_fs_open(mount, "OUT.RAB", k_ra8_fs_mode_read, &file) != k_ra8_ok);

  internal_teardown(mount);
  TEST_END("rabook_import_m33: dispatch error -> propagated, no output");
}
 ...
 * RA8_CHECK_NULL_PTR guards of rabook_import_compile_adapter_m33: a NULL
...
{
  TEST_BEGIN("rabook_import_m33: null guards");
  ra8_fs_mount_t* mount = internal_fresh_volume_with_epub("SRC.EPB");

  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 rabook_import_compile_adapter_m33(nullptr, mount, "SRC.EPB", "OUT.RAB"));
  ...
  internal_teardown(mount);
  TEST_END("rabook_import_m33: null guards");
}
...
{
  TEST_BEGIN("rabook_import_m33: missing source -> read error, no output");
  ra8_fs_mount_t* mount = internal_fresh_volume_with_epub("SRC.EPB");

  rabook_import_compiler_m33_ctx_t ctx = {};
  internal_make_cookie(&ctx, internal_mock_dispatch_golden);
  /* NOPE.EPB does not exist -> ra8_fs_open fails inside s_read_whole_file. */
  TEST_ASSERT(rabook_import_compile_adapter_m33(&ctx, mount, "NOPE.EPB", "OUT.RAB") != k_ra8_ok);

  ra8_fs_file_t* file = nullptr;
  TEST_ASSERT(ra8_fs_open(mount, "OUT.RAB", k_ra8_fs_mode_read, &file) != k_ra8_ok);

  internal_teardown(mount);
  TEST_END("rabook_import_m33: missing source -> read error, no output");
}
...
{
  TEST_BEGIN("rabook_import_m33: hw_error offload -> in-core fallback -> golden");
  ra8_fs_mount_t* mount =
    internal_fresh_volume_seeded("SRC.EPB", s_parity_epub, (uint32_t)k_parity_epub_len);

  rabook_import_compiler_ctx_t incore = {};
  ...
  internal_teardown(mount);
  TEST_END("rabook_import_m33: hw_error offload -> in-core fallback -> golden");
}
...
{
  TEST_BEGIN("rabook_import_m33: no_mem offload -> in-core fallback -> golden");
  ra8_fs_mount_t* mount =
    internal_fresh_volume_seeded("SRC.EPB", s_parity_epub, (uint32_t)k_parity_epub_len);
  ...
  internal_teardown(mount);
  TEST_END("rabook_import_m33: no_mem offload -> in-core fallback -> golden");
}
...
{
  TEST_BEGIN("rabook_import_m33: clean offload -> M33 result used, no fallback");
  ra8_fs_mount_t* mount = internal_fresh_volume_with_epub("SRC.EPB");

  /* A fallback that would FAIL if it were ever invoked. */
  rabook_import_compiler_ctx_t poison = {};
  ...
  internal_teardown(mount);
  TEST_END("rabook_import_m33: clean offload -> M33 result used, no fallback");
}
...
{
  TEST_BEGIN("rabook_import_m33: oversize source -> overflow -> streamed fallback");
  ra8_fs_mount_t* mount =
    internal_fresh_volume_seeded("SRC.EPB", s_parity_epub, (uint32_t)k_parity_epub_len);
  ...
  internal_teardown(mount);
  TEST_END("rabook_import_m33: oversize source -> overflow -> streamed fallback");
}
...
{
  TEST_BEGIN("rabook_import_m33: non-offload error -> propagated, no fallback");
  ra8_fs_mount_t* mount = internal_fresh_volume_with_epub("SRC.EPB");
  ...
  internal_teardown(mount);
  TEST_END("rabook_import_m33: non-offload error -> propagated, no fallback");
}
