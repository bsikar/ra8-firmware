 ...
 * in rabook_import_compiler.c's stream path (`s_source_read`,
...
internal_mem_read(void* ctx, uint64_t lba, uint32_t count, uint8_t* buf)
{
  ...
  (void)memcpy(buf, &s_disk.bytes[off], len);
  return k_ra8_ok;
}
...
internal_mem_write(void* ctx, uint64_t lba, uint32_t count, const uint8_t* buf)
{
  ...
  size_t off = (size_t)lba * (size_t)k_st_disk_block_size;
  size_t len = (size_t)count * (size_t)k_st_disk_block_size;
  (void)memcpy(&s_disk.bytes[off], buf, len);
  return k_ra8_ok;
}
 ...
 * @post @p s_disk.bytes references a freshly zeroed static backing store.
 * @note Not thread-safe.
 */
RA8_INTERNAL static ra8_fs_mount_t*
internal_fresh_volume_seeded(const char* src_path, const uint8_t* bytes, uint32_t len)
{
  static uint8_t s_disk_bytes[k_st_disk_blocks * k_st_disk_block_size];
...
/**
 * @brief Unmount @p mount and detach the RAM backing store.
 ...
 * @post @p mount is unmounted; the static backing storage remains allocated.
...
/** @brief Add the complete streamed EPUB member set to @p zip. */
RA8_INTERNAL static void internal_add_big_epub_members(mz_zip_archive*   zip,
                                                       const char*       container,
                                                       const char*       opf,
                                                       const char* const chapters[4],
                                                       const uint8_t*    filler)
{
  ...
  TEST_ASSERT(
    ...
    MZ_TRUE);
  static const char* const chapter_names[] = {"OEBPS/ch1.xhtml",
                                              "OEBPS/ch2.xhtml",
                                              "OEBPS/ch3.xhtml",
                                              "OEBPS/ch4.xhtml"};
  for (size_t i = 0U; i < (sizeof(chapter_names) / sizeof(chapter_names[0])); ++i) {
    TEST_ASSERT(mz_zip_writer_add_mem(zip,
                                      chapter_names[i],
                                      chapters[i],
                                      strlen(chapters[i]),
 ...
 * @pre The function-local filler has ::k_st_filler_bytes bytes.
...
RA8_INTERNAL static void internal_build_big_epub(void)
{
  ...
  static const char* const container = "<?xml version=\"1.0\"?>"
  ...
  static const char* const chapters[] = {
  ...
  internal_add_big_epub_members(&zip, container, opf, chapters, s_filler);
...
/** @brief Caller-owned arenas backing one streamed compiler context. */
typedef struct {
  /** Chapter records. */
  book_chapter_t chapters[k_st_chapter_cap];
  /** Parsed document nodes. */
  book_node_t nodes[k_st_node_cap];
  /** Parsed node attributes. */
  book_attr_t attrs[k_st_attr_cap];
  /** Stylesheet records. */
  book_stylesheet_t styles[k_st_style_cap];
  /** Imported image records. */
  book_image_t images[k_st_image_cap];
  /** Builder string arena. */
  char strpool[k_st_string_cap];
  /** Builder image arena. */
  uint8_t imgpool[k_st_imgpool_cap];
  /** Final RABOOK output bytes. */
  uint8_t out[k_st_out_cap];
  /** XHTML source scratch. */
  uint8_t xhtml[k_st_xhtml_cap];
  /** XML parser workspace. */
  ra8_rabook_xml_workspace_t xml_workspace;
  /** Encoded image scratch. */
  uint8_t image_raw[k_st_imgraw_cap];
  /** Decoder arena bytes. */
  uint8_t img_scratch[k_st_arena_cap];
  /** Normalized grayscale pixels. */
  uint8_t gray[k_st_gray_cap];
  /** Stylesheet source scratch. */
  char css[k_st_css_cap];
  /** Page-cache bytes. */
  uint8_t cache_frames[k_st_cache_frames * k_st_cache_frame_bytes];
  /** Cached page keys. */
  ra8_vmem_key_t cache_keys[k_st_cache_frames];
  /** Cache hash-chain heads. */
  int32_t cache_buckets[k_st_cache_buckets];
  /** Parsed EPUB state. */
  epub_book_t book;
  /** Image-decoder arena descriptor. */
  ra8_img_arena_t arena;
  /** Compiler scratch view. */
  ra8_rabook_pipeline_scratch_t scratch;
} streamed_compiler_storage_t;

/** @brief Bind the builder and scratch views over @p storage. */
RA8_INTERNAL static void internal_bind_compiler_storage(streamed_compiler_storage_t* storage)
{
  ...
  storage->scratch = (ra8_rabook_pipeline_scratch_t){
    ...
    .image_raw     = storage->image_raw,
    .image_cap     = sizeof(storage->image_raw),
    .img_arena     = &storage->arena,
    .gray          = storage->gray,
    .gray_cap      = (uint32_t)k_st_gray_cap,
    .css           = storage->css,
    .css_cap       = sizeof(storage->css),
    .xml_workspace = &storage->xml_workspace,
  };
  storage->book = (epub_book_t){};
}
...
RA8_INTERNAL static void internal_make_ctx(rabook_import_compiler_ctx_t* ctx)
{
  static streamed_compiler_storage_t s_storage;
  internal_bind_compiler_storage(&s_storage);
...
RA8_INTERNAL static void internal_test_streamed_compile_matches_desktop_golden(void)
{
  ...
  rabook_import_compiler_ctx_t ctx = {};
  internal_make_ctx(&ctx);
  TEST_ASSERT_EQ(k_ra8_ok, rabook_import_compile_adapter(&ctx, mount, "SRC.EPB", "OUT.RAB"));
...
RA8_INTERNAL static void internal_test_streamed_compile_bounded_high_water(void)
{
  ...
  rabook_import_compiler_ctx_t ctx = {};
  internal_make_ctx(&ctx);
  (void)memset(s_cache_meta, 0, sizeof(s_cache_meta));
  TEST_ASSERT_EQ(k_ra8_ok, rabook_import_compile_adapter(&ctx, mount, "BIG.EPB", "BIG.RAB"));

  /* The compile produced a valid book with the fixture's whole spine. */
  uint32_t got = 0U;
  internal_read_output(mount, "BIG.RAB", &got);
  TEST_ASSERT_EQ(k_ra8_ok, book_validate(s_readback, (size_t)got));
  const book_header_t* hdr = book_header((const void*)s_readback);
...
RA8_INTERNAL static void internal_streamed_guards_null(rabook_import_compiler_ctx_t* ctx,
                                                       ra8_fs_mount_t*               mount)
{
  /* Argument null guards -- each flipped independently. */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 rabook_import_compile_adapter(nullptr, mount, "SRC.EPB", "OUT.RAB"));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 rabook_import_compile_adapter(ctx, nullptr, "SRC.EPB", "OUT.RAB"));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, rabook_import_compile_adapter(ctx, mount, nullptr, "OUT.RAB"));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, rabook_import_compile_adapter(ctx, mount, "SRC.EPB", nullptr));

  /* Cookie null guards: open-book storage, then the cache. */
  rabook_import_compiler_ctx_t bad = {};
  internal_make_ctx(&bad);
  bad.epub = nullptr;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 rabook_import_compile_adapter(&bad, mount, "SRC.EPB", "OUT.RAB"));
  internal_make_ctx(&bad);
  bad.cache = nullptr;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 rabook_import_compile_adapter(&bad, mount, "SRC.EPB", "OUT.RAB"));
...
RA8_INTERNAL static void internal_test_streamed_adapter_guards(void)
{
  ...
  TEST_ASSERT(rabook_import_compile_adapter(&ctx, mount, "NOPE.EPB", "OUT.RAB") != k_ra8_ok);
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 rabook_import_compile_adapter(&ctx, mount, "EMPTY.EPB", "OUT.RAB"));
  ...
  TEST_ASSERT_EQ(k_ra8_err_validation_failed,
                 rabook_import_compile_adapter(&ctx, mount, "JUNK.EPB", "OUT.RAB"));
  ...
  TEST_ASSERT(rabook_import_compile_adapter(&ctx, mount, "SRC.EPB", "OUT.RAB") != k_ra8_ok);
