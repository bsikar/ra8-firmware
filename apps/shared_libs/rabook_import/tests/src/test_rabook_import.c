 ...
 * through the DIP seam (@ref rabook_import_compile_fn) so the test can count
...
internal_mem_read(void* ctx, uint64_t lba, uint32_t count, uint8_t* buf)
{
  ...
  (void)memcpy(buf, &disk->bytes[off], len);
  return k_ra8_ok;
}
...
internal_mem_write(void* ctx, uint64_t lba, uint32_t count, const uint8_t* buf)
{
  ...
  size_t off = (size_t)lba * (size_t)k_disk_block_size;
  size_t len = (size_t)count * (size_t)k_disk_block_size;
  (void)memcpy(&disk->bytes[off], buf, len);
  return k_ra8_ok;
}
...
internal_spy_compile(void* ctx, ra8_fs_mount_t* mount, const char* epub_path, const char* out_path)
{
  ...
  (void)memcpy(payload, "RBKSPY", (size_t)k_spy_hdr_len);
  payload[k_spy_idx_seq] = (uint8_t)s_compile_calls;
  (void)memcpy(s_last_payload, payload, sizeof(payload));
...
{
  static const char* const mimetype = "application/epub+zip";
  static const char* const container =
    "<?xml version=\"1.0\"?><container version=\"1.0\" "
    "xmlns=\"urn:oasis:names:tc:opendocument:xmlns:container\"><rootfiles>"
    "<rootfile full-path=\"OEBPS/content.opf\" "
    "media-type=\"application/oebps-package+xml\"/></rootfiles></container>";
  static const char* const opf =
  ...
  TEST_ASSERT(
    mz_zip_writer_add_mem(&zip, "mimetype", mimetype, strlen(mimetype), MZ_NO_COMPRESSION) ==
    MZ_TRUE);
...
{
  static uint8_t                s_disk_bytes[k_disk_blocks * k_disk_block_size];
...
/**
 * @brief Read the complete freshness marker into a typed fixture record.
 * @param[in,out] mount Mounted test volume.
 * @param[out] stamp Destination stamp.
 * @pre `BOOK.rabook.mrk` is present and exactly one stamp long.
 * @post @p stamp contains the persisted marker.
 * @note Test-only synchronous helper.
 * @since Version 0.1.0
 */
RA8_INTERNAL static void internal_read_stamp_fixture(ra8_fs_mount_t*        mount,
                                                     rabook_import_stamp_t* stamp)
{
  ra8_fs_file_t* file = nullptr;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_open(mount, "BOOK.rabook.mrk", k_ra8_fs_mode_read, &file));
  uint32_t got = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_read(file, (uint8_t*)stamp, (uint32_t)sizeof(*stamp), &got));
  TEST_ASSERT_EQ(sizeof(*stamp), got);
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_close(file));
}

/**
 * @brief Persist one stale marker and prove the manager recompiles it.
 * @param[in,out] mount Mounted test volume.
 * @param[in] cfg Active import configuration.
 * @param[in] stamp Deliberately stale marker.
 * @param[in] expected_calls Compile-spy count after the open.
 * @pre The source and cache already exist on @p mount.
 * @post The stale marker was rejected and replaced after one compile.
 * @note Test-only synchronous helper.
 * @since Version 0.1.0
 */
RA8_INTERNAL static void internal_assert_stale_stamp_recompiled(ra8_fs_mount_t*              mount,
                                                                const rabook_import_cfg_t*   cfg,
                                                                const rabook_import_stamp_t* stamp,
                                                                int expected_calls)
{
  TEST_ASSERT_EQ(
    k_ra8_ok,
    ra8_fs_write_file(mount, "BOOK.rabook.mrk", (const uint8_t*)stamp, (uint32_t)sizeof(*stamp)));
  rabook_import_outcome_t out              = k_rabook_import_hit;
  char                    path[k_path_cap] = {};
  TEST_ASSERT_EQ(k_ra8_ok, rabook_import_open(cfg, "BOOK.EPB", path, (uint32_t)sizeof(path), &out));
  TEST_ASSERT_EQ(k_rabook_import_compiled, out);
  TEST_ASSERT_EQ(expected_calls, s_compile_calls);
}
...
RA8_INTERNAL static rabook_import_cfg_t internal_make_cfg(ra8_fs_mount_t*          mount,
                                                          uint32_t                 importer_version,
                                                          rabook_import_compile_fn compile)
{
  rabook_import_cfg_t cfg = {};
 ...
 *
 * @par MC/DC:
 * The all-equal fieldwise freshness control is exercised by the hit transition;
 * the companion stale-stamp test independently varies every field. @details Executes the miss then hit scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_miss_then_hit(void)
{
  TEST_BEGIN("rabook_import: miss -> compile -> cache -> hit");
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 rabook_import_open(&cfg, "BOOK.EPB", path1, (uint32_t)sizeof(path1), &out));
  TEST_ASSERT_EQ(k_rabook_import_compiled, out);
 ...
 *
 * @par MC/DC:
 * This proves the importer-version field participates in stamp identity; the
 * companion fieldwise test pins the remaining comparisons. @details Executes the stale version recompiles scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_stale_version_recompiles(void)
{
  TEST_BEGIN("rabook_import: bumped version re-derives");
  ...
  rabook_import_cfg_t cfg_a =
    internal_make_cfg(mount, (uint32_t)k_importer_version_a, internal_spy_compile);
  TEST_ASSERT_EQ(k_ra8_ok,
                 rabook_import_open(&cfg_a, "BOOK.EPB", path, (uint32_t)sizeof(path), &out));
  TEST_ASSERT_EQ(k_rabook_import_compiled, out);
  TEST_ASSERT_EQ(1, s_compile_calls);

  rabook_import_cfg_t cfg_b =
    internal_make_cfg(mount, (uint32_t)k_importer_version_b, internal_spy_compile);
  TEST_ASSERT_EQ(k_ra8_ok,
                 rabook_import_open(&cfg_b, "BOOK.EPB", path, (uint32_t)sizeof(path), &out));
  TEST_ASSERT_EQ(k_rabook_import_compiled, out);
  TEST_ASSERT_EQ(2, s_compile_calls);
  uint8_t v2[k_spy_len];
  (void)memcpy(v2, s_last_payload, sizeof(v2));
  internal_assert_cache_bytes(mount, path, v2, (uint32_t)k_spy_len);

  internal_teardown(mount);
  TEST_END("rabook_import: bumped version re-derives");
}
 ...
 *
 * @par MC/DC:
 * Proves the CRC-32 content key still gates freshness now that it no longer
 * names the file (the name follows the source). @details Executes the content change rederives scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_content_change_rederives(void)
{
  TEST_BEGIN("rabook_import: changed content -> same name, re-derived");
  ra8_fs_mount_t* mount = internal_fresh_volume();
  s_compile_calls       = 0;
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 rabook_import_open(&cfg, "BOOK.EPB", path_a, (uint32_t)sizeof(path_a), &out));
  TEST_ASSERT_EQ(k_rabook_import_compiled, out);
  TEST_ASSERT_EQ(1, s_compile_calls);
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 rabook_import_open(&cfg, "BOOK.EPB", path_b, (uint32_t)sizeof(path_b), &out));
  TEST_ASSERT_EQ(k_rabook_import_compiled, out);
  TEST_ASSERT_EQ(2, s_compile_calls);
  ...
  internal_teardown(mount);
  TEST_END("rabook_import: changed content -> same name, re-derived");
}

/**
 * @test internal_test_each_stale_stamp_field_recompiles
 * @brief Every freshness-stamp field independently invalidates the cache.
 * @par MC/DC:
 * `apps/shared_libs/rabook_import/src/rabook_import.c@internal_cache_is_fresh`
 * receives the all-five-fields-equal hit control from
 * internal_test_miss_then_hit, then the five vectors below vary magic, format
 * version, importer version, source size, and source CRC individually. Each
 * equality therefore independently changes the five-condition AND result.
 * @details Starts from one canonical marker, varies magic, format version,
 *          importer version, source size, and source CRC one at a time, and
 *          proves each mismatch forces exactly one fresh compilation.
 * @pre The RAM FAT fixture can hold the source, cache, and marker.
 * @post All five false comparisons and the all-equal control have executed.
 * @note Test-only; each successful compile restores the canonical marker.
 * @since Version 0.1.0
 */
RA8_INTERNAL static void internal_test_each_stale_stamp_field_recompiles(void)
{
  TEST_BEGIN("rabook_import: every stale stamp field re-derives");
  internal_build_epub("<html><body><p>Stable stamp source.</p></body></html>");
  ra8_fs_mount_t* mount = internal_fresh_volume();
  internal_put_source(mount, "BOOK.EPB");
  rabook_import_cfg_t cfg =
    internal_make_cfg(mount, (uint32_t)k_importer_version_a, internal_spy_compile);
  ...
  s_compile_calls                          = 0;
  TEST_ASSERT_EQ(k_ra8_ok,
                 rabook_import_open(&cfg, "BOOK.EPB", path, (uint32_t)sizeof(path), &out));
  TEST_ASSERT_EQ(k_rabook_import_compiled, out);
  TEST_ASSERT_EQ(1, s_compile_calls);

  rabook_import_stamp_t canonical = {};
  internal_read_stamp_fixture(mount, &canonical);
  rabook_import_stamp_t stale = canonical;
  stale.magic ^= 1U;
  internal_assert_stale_stamp_recompiled(mount, &cfg, &stale, 2);

  stale = canonical;
  stale.format_version ^= 1U;
  internal_assert_stale_stamp_recompiled(mount, &cfg, &stale, 3);

  stale = canonical;
  stale.importer_version ^= 1U;
  internal_assert_stale_stamp_recompiled(mount, &cfg, &stale, 4);

  stale = canonical;
  stale.source_size ^= 1U;
  internal_assert_stale_stamp_recompiled(mount, &cfg, &stale, 5);

  stale = canonical;
  stale.source_crc32 ^= 1U;
  internal_assert_stale_stamp_recompiled(mount, &cfg, &stale, 6);

  internal_teardown(mount);
  TEST_END("rabook_import: every stale stamp field re-derives");
}
...
{
  TEST_BEGIN("rabook_import: compile error leaves no cache");
  ...
  rabook_import_cfg_t cfg_ok =
    internal_make_cfg(mount, (uint32_t)k_importer_version_a, internal_spy_compile);
  TEST_ASSERT_EQ(k_ra8_ok,
                 rabook_import_open(&cfg_ok, "BOOK.EPB", path, (uint32_t)sizeof(path), &out));
  TEST_ASSERT_EQ(k_rabook_import_compiled, out);
  TEST_ASSERT_EQ(2, s_compile_calls);

  internal_teardown(mount);
  TEST_END("rabook_import: compile error leaves no cache");
}
...
{
  TEST_BEGIN("rabook_import: NULL + capacity guards");
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 rabook_import_open(nullptr, "BOOK.EPB", path, (uint32_t)sizeof(path), &out));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 rabook_import_open(&cfg, nullptr, path, (uint32_t)sizeof(path), &out));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 rabook_import_open(&cfg, "BOOK.EPB", nullptr, (uint32_t)sizeof(path), &out));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 rabook_import_open(&cfg, "BOOK.EPB", path, (uint32_t)sizeof(path), nullptr));

  /* Missing injected dependencies. */
  rabook_import_cfg_t bad = cfg;
  bad.mount               = nullptr;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 rabook_import_open(&bad, "BOOK.EPB", path, (uint32_t)sizeof(path), &out));
  bad         = cfg;
  bad.compile = nullptr;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 rabook_import_open(&bad, "BOOK.EPB", path, (uint32_t)sizeof(path), &out));
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, rabook_import_open(&cfg, "BOOK.EPB", path, 1U, &out));

  /* Missing source -> propagated open error, never a compile. */
  TEST_ASSERT(rabook_import_open(&cfg, "NOPE.EPB", path, (uint32_t)sizeof(path), &out) != k_ra8_ok);
  TEST_ASSERT_EQ(0, s_compile_calls);

  internal_teardown(mount);
  TEST_END("rabook_import: NULL + capacity guards");
}
...
{
  TEST_BEGIN("rabook_import: name derivation edges");
  ...
  TEST_ASSERT(rabook_import_open(&cfg, "sub/BOOK.EPB", path, (uint32_t)sizeof(path), &out) !=
              k_ra8_ok);
  TEST_ASSERT_EQ(0, s_compile_calls);

  /* A path that ends in '/' has no usable stem. */
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 rabook_import_open(&cfg, "sub/", path, (uint32_t)sizeof(path), &out));
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 rabook_import_open(&cfg, k_long, path, (uint32_t)sizeof(path), &out));
  TEST_ASSERT_EQ(0, s_compile_calls);

  internal_teardown(mount);
  TEST_END("rabook_import: name derivation edges");
}
...
int main(void)
{
  ...
  internal_test_each_stale_stamp_field_recompiles();
