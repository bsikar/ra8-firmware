 ...
 *
 * @details
 * `rabook_import` makes the e-reader a self-contained appliance: drop a raw
 ...
 * @ref rabook_import_compile_fn function pointer, and the filesystem only
 ...
 * real `rabook_compile_from_epub` behind the seam -- see
 * @ref rabook_import_compiler.h for the supplied adapter.
 *
 * @par Crash safety
 ...
 * cache; the `.rabook` body CRC-32 (checked by `book_open`) is a second line
 ...
 * @see rabook_import_compiler.h  Production adapter onto the real compiler.
...
typedef enum : uint16_t {
  k_rabook_import_name_cap    = 128U, /**< Cache/temp/marker name buffer bytes incl. NUL. */
  k_rabook_import_stamp_bytes = 20U,  /**< Pinned size of the freshness stamp.            */
} rabook_import_size_t;

/**
 * @enum rabook_import_magic_t
...
typedef enum : uint32_t {
  k_rabook_import_stamp_magic = 0x52424B31U, /**< Importer marker tag "RBK1". */
} rabook_import_magic_t;

/**
 * @enum rabook_import_outcome_t
 * @brief How @ref rabook_import_open satisfied the request.
 * @since Version 0.1.0
 */
typedef enum : uint8_t {
  k_rabook_import_hit      = 0U, /**< Fresh cache reused; the compiler was NOT invoked. */
  k_rabook_import_compiled = 1U, /**< Miss/stale; the book was compiled and cached.     */
 ...
 *
 * @invariant `magic == k_rabook_import_stamp_magic` for a valid marker.
 * @since Version 0.1.0
 */
typedef struct {
  uint32_t magic;            /**< @ref k_rabook_import_stamp_magic.              */
...
} rabook_import_stamp_t;

static_assert(sizeof(rabook_import_stamp_t) == (uint32_t)k_rabook_import_stamp_bytes,
              "import stamp layout pinned");

/**
 * @typedef rabook_import_compile_fn
 ...
 *          `rabook_compile_from_epub` (through the supplied adapter); host
 ...
 * @since Version 0.1.0
 */
typedef ra8_err_t (*rabook_import_compile_fn)(void*           compile_ctx,
 ...
 * @brief Injected dependencies + versioning for @ref rabook_import_open.
 *
 ...
 *          stamps that gate reuse -- pass `k_book_format_version` and the
...
typedef struct {
  ...
  rabook_import_compile_fn compile;          /**< Compile-on-miss seam (non-NULL).     */
 ...
 *                            the caller then opens with `book_open`.
 * @param[in]  cache_path_cap Capacity of @p out_cache_path in bytes; must hold
 *                            the derived `<name>.rabook` name and its terminator.
 * @param[out] out_outcome    Receives @ref k_rabook_import_hit or
 *                            @ref k_rabook_import_compiled on success.
 *
 * @return Error code.
 ...
 * @see rabook_import_compiler.h
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t rabook_import_open(const rabook_import_cfg_t* cfg,
                                           const char*                epub_path,
                                           char*                      out_cache_path,
