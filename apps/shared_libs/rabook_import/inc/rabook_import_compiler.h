 ...
 *
 * @details
 * @ref rabook_import.h keeps the cache manager decoupled from the EPUB parser
 ...
 * @ref rabook_import_compiler_ctx_t cookie) straight into
 * @ref rabook_import_cfg_t.
 *
 ...
 * @see rabook_import.h          The cache manager that calls this seam.
 * @see rabook_compile_from_epub The compiler this wraps.
 ...
 */
typedef struct {
  epub_book_t*                epub;               /**< Caller-owned open-book storage.        */
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t rabook_import_compile_adapter(void*           compile_ctx,
 ...
 * @brief Cookie for @ref rabook_import_compile_adapter_m33 (the M33-offload binding).
 *
 ...
 * @see rabook_import_compiler_ctx_t
 * @since Version 0.1.0
 */
typedef struct {
 ...
 *          returned blob with `book_validate`, then writes it to @p out_path
 ...
 *          @ref rabook_import_compiler_ctx_t.fallback, the compile is retried
 ...
 *
 * @param[in]     compile_ctx Pointer to a @ref rabook_import_compiler_m33_ctx_t.
 * @param[in,out] mount       Mounted volume holding the source and the output.
 * @param[in]     epub_path   Root-level path of the source `.epub`.
 ...
 * @pre @p mount, @p epub_path, and @p out_path are non-NULL.
 * @post On `k_ra8_ok`, @p out_path holds a `book_validate`-clean RABOOK1 blob.
 * @post On any error, @p out_path is not written.
 *
 * @note Not thread-safe.
 * @see ra8_dual_core_compile_dispatch_fn
 * @see rabook_import_compile_adapter
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t rabook_import_compile_adapter_m33(void*           compile_ctx,
                                                          ra8_fs_mount_t* mount,
                                                          const char*     epub_path,
                                                          const char*     out_path);
