 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_ra8_vfs_compress_write_validate(const char*     path,
                                                                       const uint8_t*  src,
                                                                       const void*     scratch,
...
internal_ra8_vfs_compress_store_blob(const char* path, const uint8_t* blob, uint32_t blob_len)
{
...
{
  RA8_RETURN_ON_ERROR(
    internal_ra8_vfs_compress_write_validate(path, src, scratch, blob_buf, out_blob_len),
    s_tag,
    "validate args");

  uint32_t blob_len = 0;
  RA8_RETURN_ON_ERROR(
    ra8_compress(src, src_len, blob_buf, blob_cap, scratch, scratch_len, &blob_len),
    s_tag,
    "compress");

  RA8_RETURN_ON_ERROR(internal_ra8_vfs_compress_store_blob(path, blob_buf, blob_len),
                      s_tag,
                      "store");
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_ra8_vfs_compress_read_validate(const char*     path,
                                                                      const uint8_t*  blob_buf,
                                                                      const uint8_t*  out,
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_ra8_vfs_compress_load_blob(const char* path,
                                                                  uint8_t*    blob_buf,
                                                                  uint32_t    blob_cap,
                                                                  uint32_t*   out_blob_len)
{
...
ra8_err_t ra8_vfs_compress_read(const char* path,
                                ...
                                uint32_t*   out_len)
{
  RA8_RETURN_ON_ERROR(internal_ra8_vfs_compress_read_validate(path, blob_buf, out, out_len),
                      s_tag,
                      "validate args");

  uint32_t blob_len = 0;
  RA8_RETURN_ON_ERROR(internal_ra8_vfs_compress_load_blob(path, blob_buf, blob_cap, &blob_len),
                      s_tag,
                      "load");

  RA8_RETURN_ON_ERROR(ra8_decompress(blob_buf, blob_len, out, out_cap, out_len), s_tag, "inflate");
  return k_ra8_ok;
}
