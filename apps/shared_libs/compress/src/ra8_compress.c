 ...
 * @since 0.1.0
 */
RA8_INTERNAL static mz_bool internal_compress_put(const void* buf, int len, void* user)
{
...
/**
 * @brief Validate ::ra8_compress arguments before driving the compressor.
 *
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_ra8_compress_validate(const uint8_t*  src,
                                                             const uint8_t*  out,
                                                             const void*     scratch,
                                                             ...
                                                             const uint32_t* out_len)
{
  ...
  if (scratch_len < (uint32_t)k_ra8_compress_scratch_bytes) {
 ...
 *
 * @pre `d` points to at least ::k_ra8_compress_scratch_bytes of scratch.
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_ra8_compress_drive(tdefl_compressor* d,
                                                          compress_ctx_t*   ctx,
                                                          const uint8_t*    src,
                                                          uint32_t          src_len,
                                                          int               flags,
                                                          uint32_t*         out_len)
{
  if (tdefl_init(d, internal_compress_put, ctx, flags) != TDEFL_STATUS_OKAY) {
    return k_ra8_fail;
  }
...
ra8_err_t ra8_compress(const uint8_t* src,
                       ...
                       uint32_t*      out_len)
{
  const ra8_err_t vr = internal_ra8_compress_validate(src, out, scratch, scratch_len, out_len);
  if (vr != k_ra8_ok) {
    return vr;
  }
  tdefl_compressor* d   = (tdefl_compressor*)scratch;
  compress_ctx_t    ctx = {.out = out, .cap = out_cap, .len = 0, .overflow = 0};
  return internal_ra8_compress_drive(d, &ctx, src, src_len, (int)TDEFL_DEFAULT_MAX_PROBES, out_len);
}
                            ...
                            uint32_t*      out_len)
{
  ...
  return internal_ra8_compress_drive(d, &ctx, src, src_len, flags, out_len);
}
