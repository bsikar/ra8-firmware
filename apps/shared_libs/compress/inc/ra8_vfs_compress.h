 ...
 * codec, ::ra8_compress, or `ra8_fs_write_file` directly. Reading reverses the
 ...
 *
 * This header is opt-in: it pulls in ::ra8_compress (and through it miniz), so
 * it is NOT part of the `ra8_io.h` umbrella -- exactly like `ra8_compress.h`.
 ...
 * the caller-provided `blob_buf` via ::ra8_compress (using the caller-provided
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_vfs_compress_write(const char*    path,
                                               const uint8_t* src,
                                               uint32_t       src_len,
                                               void*          scratch,
 ...
 * caller-provided `blob_buf`, then inflates it with ::ra8_decompress into
 * `out`. Heap-free: the staging blob buffer and the output buffer are both
 * caller-provided. Reverses ::ra8_vfs_compress_write exactly.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_vfs_compress_read(const char* path,
                                              uint8_t*    blob_buf,
                                              uint32_t    blob_cap,
                                              uint8_t*    out,
                                              uint32_t    out_cap,
