 ...
 *
 * The WebP fixtures are the committed files under tests/webp/ (see
 * that dir's README for provenance), embedded inline here as in
 * tests/graphics/src/test_ra8_webp.c so the test needs no runtime file I/O. The PNG oracle
...
/* ------------------------------------------------------------------------- */
/* Committed WebP fixtures under tests/webp/, embedded inline. */
/* ------------------------------------------------------------------------- */
...
RA8_INTERNAL static ra8_err_t internal_t_mem_pull(void* ctx, uint8_t* buf, size_t cap, size_t* got)
{
  ...
  (void)memcpy(buf, &p->d[p->pos], take);
  p->pos += take;
  *got = take;
  return k_ra8_ok;
}
                                                    ...
                                                    jof_info_t*     info)
{
  ...
  *store                      = (jof_memstore_t){.buf = store->buf, .cap = store->cap, .len = 0U};
...
internal_png_append_chunk(const uint8_t* type, const uint8_t* data, uint32_t len)
{
  ...
  if (len > 0U) {
    (void)memcpy(&p[8], data, (size_t)len);
  }
...
RA8_INTERNAL static void internal_assert_golden_tile(const jof_info_t* info,
                                                     uint32_t          x0,
                                                     uint32_t          y0,
                                                     uint16_t          w,
                                                     uint16_t          h)
...
RA8_INTERNAL static void internal_check_golden_tiles(jof_memstore_t* store, const jof_info_t* info)
{
 ...
 *
 * @details
 * jof_info_t carries padding between its uint8_t and uint32_t
...
RA8_INTERNAL static bool internal_ta_info_equal(const jof_info_t* lhs, const jof_info_t* rhs)
{
 ...
 *        `jof_webp_work_bytes()` proving the sizing guarantee.
 *
 * @par MC/DC:
...
RA8_INTERNAL static void internal_test_webp_lossless_golden(void)
{
  ...
  const uint32_t need =
    jof_webp_work_bytes((uint16_t)k_t_dim, (uint16_t)k_t_dim, sizeof(s_webp_lossless));
  TEST_ASSERT(need > 0U);
  TEST_ASSERT(need <= sizeof(s_webp_work));

  jof_memstore_t store = {.buf = s_store_a, .cap = sizeof(s_store_a), .len = 0U};
  jof_info_t     info  = {};
  TEST_ASSERT_EQ(k_ra8_ok,
  ...
  jof_info_t reparsed = {};
  TEST_ASSERT_EQ(k_ra8_ok, jof_parse(jof_memstore_pread, &store, (uint64_t)store.len, &reparsed));
...
RA8_INTERNAL static void internal_test_webp_png_byte_identical(void)
{
  ...
  jof_memstore_t sa = {.buf = s_store_a, .cap = sizeof(s_store_a), .len = 0U};
  jof_info_t     ia = {};
  TEST_ASSERT_EQ(k_ra8_ok,
                 internal_produce_webp(s_webp_lossless,
                                       sizeof(s_webp_lossless),
  ...
  jof_memstore_t          sb  = {.buf = s_store_b, .cap = sizeof(s_store_b), .len = 0U};
  const jof_produce_cfg_t cfg = {
    .pull       = internal_t_mem_pull,
    .pull_ctx   = &s_pull,
    .sink       = jof_memstore_sink,
  ...
  jof_info_t ib = {};
  TEST_ASSERT_EQ(k_ra8_ok, jof_produce(&cfg, &ib));
...
/** @brief Return the over-wide WebP rejection fixture. */
RA8_INTERNAL static const uint8_t* internal_webp_wide(size_t* out_size)
{
...
RA8_INTERNAL static void internal_test_webp_hostile(void)
{
  ...
  size_t               wide_size = 0U;
  const uint8_t* const wide      = internal_webp_wide(&wide_size);
  TEST_ASSERT_EQ(
    k_ra8_err_not_supported,
    internal_produce_webp(wide, wide_size, s_webp_work, sizeof(s_webp_work), &store, &info));
...
RA8_INTERNAL static void internal_test_webp_work_bytes(void)
{
  ...
  TEST_ASSERT(jof_webp_work_bytes(512U, 512U, 4096U) > 0U);
  TEST_ASSERT_EQ(0U, jof_webp_work_bytes(0U, 512U, 4096U));
  TEST_ASSERT_EQ(0U, jof_webp_work_bytes(9000U, 512U, 4096U));
  TEST_ASSERT_EQ(0U, jof_webp_work_bytes(512U, 0U, 4096U));
  TEST_ASSERT_EQ(0U, jof_webp_work_bytes(512U, 9000U, 4096U));
  TEST_ASSERT_EQ(0U, jof_webp_work_bytes(512U, 512U, 0U));
  /* A wider frame costs strictly more arena (frame term dominates). */
  TEST_ASSERT(jof_webp_work_bytes(1024U, 512U, 4096U) > jof_webp_work_bytes(512U, 512U, 4096U));
