...
/** @brief Maximally aligned bytes for explicit result/record alias testing. */
typedef struct test_alias_storage_t {
  alignas(max_align_t)
    uint8_t bytes[(sizeof(jof_audit_record_t) * k_test_tile_count) > sizeof(jof_audit_result_t)
                    ? (sizeof(jof_audit_record_t) * k_test_tile_count)
                    : sizeof(jof_audit_result_t)]; /**< Result/record alias storage. */
} test_alias_storage_t;

/** @brief Maximally aligned bytes for explicit result/workspace alias testing. */
typedef struct test_ws_alias_storage_t {
  alignas(max_align_t)
    uint8_t bytes[sizeof(jof_audit_workspace_t) > sizeof(jof_audit_result_t)
                    ? sizeof(jof_audit_workspace_t)
                    : sizeof(jof_audit_result_t)]; /**< Result/workspace alias storage. */
} test_ws_alias_storage_t;

/** @brief Maximally aligned bytes for explicit result/tile alias testing. */
typedef struct test_tile_alias_storage_t {
  alignas(max_align_t) uint8_t bytes[sizeof(jof_audit_result_t)]; /**< Result/tile alias storage. */
} test_tile_alias_storage_t;
...
typedef struct test_deflate_bufs_t {
  jof_audit_record_t records[k_test_tile_count];          /**< Record array.   */
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_report_failure(int line)
{
...
#define CHECK(expr)                                                                                \
  do {                                                                                             \
    if (!(expr)) {                                                                                 \
      internal_report_failure(__LINE__);                                                           \
    }                                                                                              \
  } while (false)
                                             ...
                                             const uint8_t bottom[k_test_tile_bytes])
{
  ...
  (void)memcpy(&atlas[k_jof_ofs_magic], "JOF1", k_test_magic_bytes);
  internal_write_u16(&atlas[k_jof_ofs_width], 2U);
  internal_write_u16(&atlas[k_jof_ofs_height], 2U);
  internal_write_u16(&atlas[k_jof_ofs_tile_w], 2U);
  internal_write_u16(&atlas[k_jof_ofs_tile_h], 1U);
  atlas[k_jof_ofs_bpp]   = 1U;
  atlas[k_jof_ofs_codec] = (uint8_t)k_jof_codec_raw;
  internal_write_u32(&atlas[k_jof_ofs_tile_count], k_test_tile_count);
  ...
  internal_write_u32(&atlas[k_test_index_off + k_jof_idx_ofs_length], k_test_tile_bytes);
  internal_write_u32(&atlas[k_test_index_off + k_jof_index_entry], k_test_second_payload_off);
  internal_write_u32(&atlas[k_test_index_off + k_jof_index_entry + k_jof_idx_ofs_length],
                     k_test_tile_bytes);
  internal_write_u32(&atlas[k_test_footer_off + k_jof_ftr_index_off], k_test_index_off);
  internal_write_u32(&atlas[k_test_footer_off + k_jof_ftr_tile_count], k_test_tile_count);
  internal_write_u32(&atlas[k_test_footer_off + k_jof_ftr_total_size], k_test_atlas_size);
  (void)memcpy(&atlas[k_test_footer_off + k_jof_ftr_magic], "JOFE", k_test_magic_bytes);
}
                                                     ...
                                                     const uint8_t bottom[k_test_tile_bytes])
{
  ...
  internal_write_u16(&atlas[k_jof_ofs_tile_h], 1U);
  atlas[k_jof_ofs_bpp]   = 1U;
  atlas[k_jof_ofs_codec] = (uint8_t)k_jof_codec_deflate;
  internal_write_u32(&atlas[k_jof_ofs_tile_count], k_test_tile_count);
  ...
  internal_write_u32(&atlas[k_test_deflate_index_off + k_jof_idx_ofs_length],
                     k_test_deflate_stream);
  internal_write_u32(&atlas[k_test_deflate_index_off + k_jof_index_entry],
                     k_test_deflate_second_off);
  internal_write_u32(&atlas[k_test_deflate_index_off + k_jof_index_entry + k_jof_idx_ofs_length],
                     k_test_deflate_stream);
  internal_write_u32(&atlas[k_test_deflate_footer_off + k_jof_ftr_index_off],
                     k_test_deflate_index_off);
  internal_write_u32(&atlas[k_test_deflate_footer_off + k_jof_ftr_tile_count], k_test_tile_count);
  internal_write_u32(&atlas[k_test_deflate_footer_off + k_jof_ftr_total_size],
                     k_test_deflate_atlas_size);
  (void)memcpy(&atlas[k_test_deflate_footer_off + k_jof_ftr_magic], "JOFE", k_test_magic_bytes);
}
...
RA8_INTERNAL static ra8_err_t internal_run_audit(const uint8_t       atlas[k_test_atlas_size],
                                                 jof_audit_record_t  records[k_test_tile_count],
                                                 uint32_t            record_cap,
                                                 ...
                                                 jof_audit_result_t* result)
{
  test_store_t          store = {.bytes          = atlas,
                                 ...
                                 .fail_offset    = 0U};
  jof_audit_workspace_t ws    = {.records     = records,
                                 .record_cap  = record_cap,
                                 .tile        = tile,
                                 .tile_cap    = tile_cap,
                                 .scratch     = nullptr,
                                 .scratch_cap = 0U};
  return jof_audit(internal_test_pread, &store, store.len, &ws, result);
}
...
RA8_INTERNAL static void internal_expect_audit_status(test_store_t*          store,
                                                      jof_audit_workspace_t* ws,
                                                      jof_audit_result_t*    result,
                                                      ra8_err_t              expected)
{
  CHECK(jof_audit(internal_test_pread, store, store->len, ws, result) == expected);
...
RA8_INTERNAL static void internal_expect_raw_requirements(test_store_t* store)
{
  ...
  CHECK(jof_audit_requirements(internal_test_pread, store, store->len, &need) == k_ra8_ok);
...
static void internal_test_deflate_audit(void)
{
  ...
  jof_audit_result_t    result                          = {};
  jof_audit_workspace_t workspace                       = {.records     = records,
                                                           .record_cap  = k_test_tile_count,
                                                           .tile        = tile,
                                                           .tile_cap    = sizeof(tile),
                                                           .scratch     = scratch,
                                                           .scratch_cap = sizeof(scratch)};
  CHECK(jof_audit(internal_test_pread, &store, store.len, &workspace, &result) == k_ra8_ok);
...
static void internal_test_workspace_guards(void)
{
  ...
  jof_audit_workspace_t alias = {.records    = records,
                                 .record_cap = k_test_tile_count,
                                 .tile       = (uint8_t*)records,
                                 .tile_cap   = k_test_tile_bytes};
  result                      = (jof_audit_result_t){.decoded_tiles = k_test_record_poison};
  internal_expect_audit_status(&store, &alias, &result, k_ra8_err_invalid_arg);

  test_alias_storage_t result_alias  = {};
  void*                alias_bytes   = result_alias.bytes;
  jof_audit_record_t*  alias_records = alias_bytes;
  jof_audit_result_t*  alias_result  = alias_bytes;
  alias                              = (jof_audit_workspace_t){.records    = alias_records,
                                                               .record_cap = k_test_tile_count,
                                                               .tile       = tile,
                                                               .tile_cap   = sizeof(tile)};
  CHECK(jof_audit(internal_test_pread, &store, store.len, &alias, alias_result) ==
...
static void internal_test_duplicate_and_coverage(void)
{
  ...
  internal_write_u32(&atlas[k_test_index_off + k_jof_index_entry], k_test_first_payload_off);
...
static void internal_test_read_failures(void)
{
  ...
  jof_audit_record_t    records[k_test_tile_count] = {};
  uint8_t               tile[k_test_tile_bytes]    = {};
  jof_audit_result_t    result                     = {.decoded_tiles = k_test_record_poison};
  jof_audit_workspace_t workspace                  = {.records    = records,
                                                      .record_cap = k_test_tile_count,
                                                      .tile       = tile,
                                                      .tile_cap   = sizeof(tile)};
  ...
  CHECK(jof_audit(internal_test_pread, &store, store.len, &workspace, &result) == k_ra8_fail);
  CHECK(result.decoded_tiles == k_test_record_poison);

  store.fail                    = true;
  jof_audit_requirements_t need = {.record_count  = k_test_record_poison,
                                   .tile_bytes    = k_test_record_poison,
                                   .scratch_bytes = k_test_record_poison};
  CHECK(jof_audit_requirements(internal_test_pread, &store, store.len, &need) == k_ra8_fail);
  CHECK((need.record_count == k_test_record_poison) && (need.tile_bytes == k_test_record_poison) &&
        (need.scratch_bytes == k_test_record_poison));
  CHECK(jof_audit_requirements(nullptr, &store, store.len, &need) == k_ra8_err_null_ptr);
}
                                                      ...
                                                      uint8_t  codec)
{
  ...
  (void)memcpy(&atlas[k_jof_ofs_magic], "JOF1", k_test_magic_bytes);
  internal_write_u16(&atlas[k_jof_ofs_width], (uint16_t)k_test_geo_dim);
  internal_write_u16(&atlas[k_jof_ofs_height], (uint16_t)k_test_geo_dim);
  internal_write_u16(&atlas[k_jof_ofs_tile_w], tile_w);
  internal_write_u16(&atlas[k_jof_ofs_tile_h], tile_h);
  atlas[k_jof_ofs_bpp]   = (uint8_t)k_test_bpp_rgba;
  atlas[k_jof_ofs_codec] = codec;
  internal_write_u32(&atlas[k_jof_ofs_tile_count], k_test_geo_tiles);
  internal_write_u32(&atlas[k_test_geo_footer_off + k_jof_ftr_index_off], k_test_geo_index_off);
  internal_write_u32(&atlas[k_test_geo_footer_off + k_jof_ftr_tile_count], k_test_geo_tiles);
  internal_write_u32(&atlas[k_test_geo_footer_off + k_jof_ftr_total_size], k_test_geo_size);
  (void)memcpy(&atlas[k_test_geo_footer_off + k_jof_ftr_magic], "JOFE", k_test_magic_bytes);
}
...
RA8_INTERNAL static void
internal_expect_overlap(test_store_t* store, jof_audit_workspace_t* ws, jof_audit_result_t* out)
{
  *out = (jof_audit_result_t){.decoded_tiles = k_test_record_poison};
  CHECK(jof_audit(internal_test_pread, store, store->len, ws, out) == k_ra8_err_invalid_arg);
...
RA8_INTERNAL static void internal_expect_top_level_null_guards(test_store_t*          store,
                                                               ...
                                                               jof_audit_result_t*    result)
{
  CHECK(jof_audit_requirements(internal_test_pread, store, store->len, nullptr) ==
        k_ra8_err_null_ptr);
  CHECK(jof_audit(nullptr, store, store->len, workspace, result) == k_ra8_err_null_ptr);
  CHECK(jof_audit(internal_test_pread, store, store->len, workspace, nullptr) ==
        k_ra8_err_null_ptr);
  CHECK(jof_audit(internal_test_pread, store, store->len, nullptr, result) == k_ra8_err_null_ptr);
}
...
RA8_INTERNAL static void internal_test_public_argument_guards(void)
{
  ...
  CHECK(jof_audit(internal_test_pread, &store, store.len, &workspace, &result) ==
        k_ra8_err_null_ptr);
...
RA8_INTERNAL static void internal_test_descriptor_overlap_guards(void)
{
  ...
  void* workspace_bytes = &workspace;
  workspace.records     = workspace_bytes;
  internal_expect_overlap(&store, &workspace, &result);
  workspace.records = records;

  workspace.tile = workspace_bytes;
  internal_expect_overlap(&store, &workspace, &result);
...
RA8_INTERNAL static void internal_test_result_alias_guards(void)
{
  ...
  test_tile_alias_storage_t tile_alias  = {};
  void*                     tile_bytes  = tile_alias.bytes;
  jof_audit_result_t*       tile_result = tile_bytes;
  tile_result->decoded_tiles            = k_test_record_poison;
  jof_audit_workspace_t workspace       = {.records    = records,
                                           .record_cap = k_test_tile_count,
                                           .tile       = tile_bytes,
                                           .tile_cap   = k_test_tile_bytes};
  CHECK(jof_audit(internal_test_pread, &store, store.len, &workspace, tile_result) ==
        k_ra8_err_invalid_arg);
  CHECK(tile_result->decoded_tiles == k_test_record_poison);

  test_ws_alias_storage_t ws_alias        = {};
  void*                   ws_bytes        = ws_alias.bytes;
  jof_audit_workspace_t*  alias_workspace = ws_bytes;
  jof_audit_result_t*     ws_result       = ws_bytes;
  *alias_workspace                        = (jof_audit_workspace_t){.records    = records,
                                                                    .record_cap = k_test_tile_count,
                                                                    .tile       = tile,
                                                                    .tile_cap   = sizeof(tile)};
  const jof_audit_record_t* before        = alias_workspace->records;
  CHECK(jof_audit(internal_test_pread, &store, store.len, alias_workspace, ws_result) ==
        k_ra8_err_invalid_arg);
  CHECK(alias_workspace->records == before);
}
...
RA8_INTERNAL static void internal_test_scratch_overlap_guards(void)
{
  ...
  jof_audit_result_t    result = {};
  jof_audit_workspace_t workspace =
    (jof_audit_workspace_t){.records     = s_deflate_bufs.records,
                            .record_cap  = k_test_tile_count,
...
RA8_INTERNAL static void internal_test_geometry_limits(void)
{
  ...
  internal_make_geometry_atlas(atlas,
                               ...
                               (uint16_t)k_test_geo_huge_tile,
                               (uint8_t)k_jof_codec_raw);
  test_store_t             store = {.bytes = atlas, .len = sizeof(atlas)};
  jof_audit_requirements_t need  = {.record_count  = k_test_record_poison,
                                    .tile_bytes    = k_test_record_poison,
                                    .scratch_bytes = k_test_record_poison};
  CHECK(jof_audit_requirements(internal_test_pread, &store, store.len, &need) ==
        k_ra8_err_invalid_size);
