 ...
 * `jof_read_tile()` and compared byte-for-byte against a direct
 ...
 * with those arms in `test_jof_produce_reject.c`.
 */
...
/**
 * @brief Append the indexed-colour palette and optional transparency chunk.
 ...
 * @post A tRNS chunk is appended exactly when @p with_trns is true.
 * @note Test-only and not thread-safe because it mutates shared scratch storage.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_png_append_palette(bool with_trns)
{
...
internal_png_build(uint32_t w, uint32_t h, uint8_t color_type, bool with_trns, bool use_filters)
{
  ...
  s_src_len                      = 0U;
  (void)memcpy(s_src, sig, sizeof(sig));
  ...
  if (color_type == 3U) {
    internal_png_append_palette(with_trns);
  }
...
internal_produce(uint16_t tile_w, uint16_t tile_h, uint8_t codec, size_t chunk, jof_info_t* info)
{
...
RA8_INTERNAL static void internal_check_tile_pixels(const jof_info_t* info,
                                                    uint8_t           ctx_ct,
                                                    uint32_t          x0,
                                                    uint32_t          y0,
                                                    uint32_t          w,
...
RA8_INTERNAL static void internal_check_tiles(const jof_info_t* info, uint8_t ctx_ct)
{
...
RA8_INTERNAL static void internal_test_produce_png_colortypes(void)
{
  ...
  for (uint32_t i = 0U; i < 6U; i++) {
    ...
    int      sh = 0;
    int      sc = 0;
    stbi_uc* px = stbi_load_from_memory(s_src, (int)s_src_len, &sw, &sh, &sc, 0);
    ...
    jof_info_t reparsed = {};
    TEST_ASSERT_EQ(k_ra8_ok,
                   jof_parse(jof_memstore_pread, &s_store, (uint64_t)s_store.len, &reparsed));
 ...
 *
 * @return The `jof_work_bytes` working-set size in bytes.
...
RA8_INTERNAL static uint32_t internal_produce_bounded_budget(void)
{
  ...
  const uint32_t need    = jof_work_bytes((uint16_t)k_t_big_w,
                                          (uint16_t)k_t_big_h,
...
RA8_INTERNAL static void internal_produce_bounded_check_corners(const jof_info_t* info)
{
...
RA8_INTERNAL static void internal_test_produce_work_bytes(void)
{
  ...
  TEST_ASSERT_EQ(0U, jof_work_bytes(0U, 1024U, 128U, 128U));
  TEST_ASSERT_EQ(0U, jof_work_bytes(40000U, 1024U, 128U, 128U));
  TEST_ASSERT_EQ(0U, jof_work_bytes(1024U, 0U, 128U, 128U));
  TEST_ASSERT_EQ(0U, jof_work_bytes(1024U, 40000U, 128U, 128U));
  TEST_ASSERT_EQ(0U, jof_work_bytes(1024U, 1024U, 0U, 128U));
  TEST_ASSERT_EQ(0U, jof_work_bytes(1024U, 1024U, 128U, 0U));
  /* Tile-count cap: 32768x32768 at 1x1 tiles is way past 65536 tiles. */
  TEST_ASSERT_EQ(0U, jof_work_bytes(32768U, 32768U, 1U, 1U));
  /* Monotone-ish sanity: a taller band costs more arena. */
  TEST_ASSERT(jof_work_bytes(1024U, 1024U, 128U, 256U) > jof_work_bytes(1024U, 1024U, 128U, 64U));
