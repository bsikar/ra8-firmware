 ...
 * @return Result code from `jof_produce()`.
...
RA8_INTERNAL static ra8_err_t internal_t_produce(jof_info_t* info)
{
...
internal_t_read(const jof_info_t* info, uint16_t tx, uint16_t ty, uint16_t* out_w, uint16_t* out_h)
{
 ...
 *
 * @details Property 1: `jof_tile_dims()` and the decoder must agree
...
RA8_INTERNAL static void internal_t_edge_dims(void)
{
  ...
  for (uint16_t ty = 0U; ty < info.tile_rows; ty++) {
    for (uint16_t tx = 0U; tx < info.tile_cols; tx++) {
      uint16_t tw = 0U;
      uint16_t th = 0U;
      TEST_ASSERT_EQ(k_ra8_ok, jof_tile_dims(&info, tx, ty, &tw, &th));
...
RA8_INTERNAL static void internal_t_edge_distinct(void)
{
  ...
  for (uint32_t n = 0U; n < info.tile_count; n++) {
    ...
    (void)memcpy(s_tile_px[n], s_cell, (size_t)s_tile_len[n]);
