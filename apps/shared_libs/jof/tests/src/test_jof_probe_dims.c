 ...
 * decodes it, so both host tools (`tools/rabook_imagepack`, `apps/host/mdl`) delegate
 ...
 * RA8_CHECK_NULL_PTR guards in jof_probe_dims (data, out_w, out_h), each
...
RA8_INTERNAL static void internal_test_probe_null_guards(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, jof_probe_dims(nullptr, (size_t)k_p_png_ihdr, &w, &h));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, jof_probe_dims(hdr, (size_t)k_p_png_ihdr, nullptr, &h));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, jof_probe_dims(hdr, (size_t)k_p_png_ihdr, &w, nullptr));
 ...
 * `len < k_jof_sniff_bytes` guard (len 0 and len sniff-1), which returns before
...
RA8_INTERNAL static void internal_test_probe_too_short(void)
{
  ...
  uint16_t h                = 0U;
  internal_make_png(hdr, k_p_w, k_p_h);
  TEST_ASSERT_EQ(k_ra8_err_not_supported, jof_probe_dims(hdr, 0U, &w, &h));
  TEST_ASSERT_EQ(k_ra8_err_not_supported,
                 jof_probe_dims(hdr, (size_t)(k_p_sniff_min - 1U), &w, &h));
...
RA8_INTERNAL static void internal_test_probe_png(void)
{
  ...
  uint16_t h                = 0U;
  internal_make_png(hdr, k_p_w, k_p_h);
  TEST_ASSERT_EQ(k_ra8_ok, jof_probe_dims(hdr, (size_t)k_p_png_ihdr, &w, &h));
 ...
 *
 * @par MC/DC:
 * Decision: `(w == 0) || (h == 0) || (w > k_jof_max_dim) || (h > k_jof_max_dim)`
...
{
  static const uint8_t s_riff_tag[4] = {'R', 'I', 'F', 'F'};
  static const uint8_t s_webp_tag[4] = {'W', 'E', 'B', 'P'};
  static const uint8_t s_avi_tag[4]  = {'A', 'V', 'I', ' '};
  TEST_BEGIN("probe_dims: unknown magic and RIFF-but-not-WEBP");
  uint8_t  hdr[k_p_hdr_cap] = {};
  uint16_t w                = 0U;
  uint16_t h                = 0U;
  (void)memset(hdr, (int)k_p_filler, sizeof(hdr));
  TEST_ASSERT_EQ(k_ra8_err_not_supported, jof_probe_dims(hdr, (size_t)k_p_hdr_cap, &w, &h));
  ...
  TEST_ASSERT(jof_probe_dims(hdr, (size_t)k_p_hdr_cap, &w, &h) != k_ra8_ok);
...
{
  static const uint8_t s_jpeg_soi[2] = {0xFFU, 0xD8U};
  ...
  const ra8_err_t rc = jof_probe_dims(hdr, (size_t)k_p_hdr_cap, &w, &h);
