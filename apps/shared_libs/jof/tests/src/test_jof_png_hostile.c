...
#include "jof_png_hostile_fixture.h"
 ...
 * cap decision already vectored in test_jof_produce.c.) @details Executes the png hostile geometry scenario with bounded fixture state and asserts the contract-specific result. @pre Fixed-capacity fixture storage required by this operation is available. @pre Arguments follow the interface contract exercised by this helper. @post Documented outputs contain the exercised result when the operation succeeds. @post Mutations remain confined to documented outputs and file-local fixture state. @note File-local helper; no ownership escapes this focused test executable. @since 0.1.0 */
RA8_INTERNAL static void internal_test_png_hostile_geometry(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 internal_craft_produce_with(1U, 1U, (uint8_t)k_jof_codec_raw, sizeof(s_work)));
...
RA8_INTERNAL static uint32_t internal_make_zlib_stored(uint32_t w, uint32_t h, uint8_t* out)
{
  ...
  while (left > 0U) {
    ...
    (void)memcpy(&out[o], &s_raw[pos], blk);
    o += blk;
    pos += blk;
    left -= blk;
  }
...
RA8_INTERNAL static void internal_test_png_hostile_residue(void)
{
  ...
  (void)memset(&s_zbuf[zl], k_png_hostile_fill_trailing, (size_t)k_png_hostile_trailing_len);
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_assert_png_signature_failures(jof_bump_t* bump, t_pull_t* pull)
{
  s_src_len                 = 0U;
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_assert_png_null_guards(jof_bump_t* bump, t_pull_t* pull)
{
...
RA8_INTERNAL static void internal_test_png_hostile_direct_seam(void)
{
  ...
  jof_bump_t      bump = {.base = s_arena, .cap = sizeof(s_arena), .off = 0U};
  static t_pull_t s_pull;
