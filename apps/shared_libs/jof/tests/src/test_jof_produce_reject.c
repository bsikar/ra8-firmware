 ...
 * bounded-RAM high-water proof), `test_jof_produce_guards.c` (the
 * producer's *internal* calculator and carve guards) and
 * `test_jof_png_hostile.c` (hostile PNG *streams* through the decoder):
 * this suite drives `jof_produce()` itself with untrusted input and
...
RA8_INTERNAL static void internal_produce_reject_sniff(void)
{
  ...
  s_src[1] = 0x00U;
  (void)memset(&s_src[2], k_tap_fill_jpeg_body, (size_t)k_tap_jpeg_body_len);
...
RA8_INTERNAL static void internal_produce_reject_pull_and_cfg(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_hw_error, jof_produce(&cfg, &info));
  /* Config guards. */
  jof_produce_cfg_t bad = cfg;
  bad.tile_w            = 0U;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, jof_produce(&bad, &info));
  bad       = cfg;
  bad.codec = (uint8_t)k_t_codec_invalid;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, jof_produce(&bad, &info));
  bad      = cfg;
  bad.pull = NULL;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, jof_produce(&bad, &info));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, jof_produce(&cfg, NULL));
}
...
RA8_INTERNAL static void internal_produce_reject_png_ihdr(void)
{
  ...
  (void)memcpy(&s_src[8U + 8U + (size_t)k_t_png_ihdr_len + 4U + 4U],
               "yLTE",
               4U); /* PLTE -> ancillary */
...
RA8_INTERNAL static void internal_produce_reject_budget(void)
{
  /* Work arena too small (fail-closed budget). */
  {
    ...
    TEST_ASSERT_EQ(k_ra8_err_invalid_size, jof_produce(&cfg, &info));
  }
  /* Sink runs out of room (store cap tiny) -> no_mem propagates. */
  {
    ...
    static t_pull_t s_pull;
    s_pull  = (t_pull_t){.d = s_src, .n = s_src_len, .pos = 0U, .chunk = 0U};
    s_store = (jof_memstore_t){.buf = s_store_buf, .cap = k_t_starved_store_cap, .len = 0U};
    jof_info_t              info = {};
    const jof_produce_cfg_t cfg  = {
      .pull     = internal_t_pull,
      .pull_ctx = &s_pull,
    ...
    TEST_ASSERT_EQ(k_ra8_err_no_mem, jof_produce(&cfg, &info));
  }
}
