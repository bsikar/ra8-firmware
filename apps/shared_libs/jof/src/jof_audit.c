...
internal_read_exact(jof_pread_fn pread, void* ctx, uint64_t offset, uint8_t* buf, size_t len)
{
  size_t          got = 0U;
...
RA8_INTERNAL static ra8_err_t internal_make_span(const void* ptr, size_t len, internal_span_t* out)
{
  ...
  uintptr_t begin = 0U;
  static_assert(sizeof(begin) == sizeof(ptr), "uintptr_t must preserve a pointer representation");
  (void)memcpy((void*)&begin, (const void*)&ptr, sizeof(begin));
                                 ...
                                 jof_audit_requirements_t* out)
{
  ...
  jof_info_t      info = {};
  const ra8_err_t rc   = jof_parse(pread, pread_ctx, total_size, &info);
  if (rc != k_ra8_ok) {
    return rc;
  }
  ...
  const jof_audit_requirements_t candidate = {
    .record_count  = info.tile_count,
    .tile_bytes    = tile,
    .scratch_bytes = (info.codec == (uint8_t)k_jof_codec_deflate) ? jof_stored_bound(tile) : 0U,
  };
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_check_workspace_spans(const jof_audit_workspace_t*    ws,
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_check_workspace(const jof_audit_workspace_t*    ws,
                                                       const jof_audit_requirements_t* need,
                                                       const jof_audit_result_t*       out)
{
  if (ws == nullptr) {
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static uint32_t internal_duplicate_count(const jof_audit_record_t* records,
                                                      uint32_t                  count,
...
typedef struct internal_audit_context_t {
  jof_pread_fn           pread;           /**< Injected positioned reader.     */
  void*                  pread_ctx;       /**< Reader-specific context.        */
  jof_audit_workspace_t* workspace;       /**< Caller-owned reusable storage.  */
  jof_audit_result_t*    candidate;       /**< Transactional result candidate. */
...
{
  uint8_t        index[k_jof_index_entry] = {};
  const uint64_t index_at                 = (uint64_t)context->candidate->info.index_off +
                                            ((uint64_t)index_number * (uint64_t)k_jof_index_entry);
  ra8_err_t      rc =
    internal_read_exact(context->pread, context->pread_ctx, index_at, index, sizeof(index));
  if (rc != k_ra8_ok) {
    return rc;
  }
  jof_audit_record_t* const record = &context->workspace->records[index_number];
  *record                          = (jof_audit_record_t){
    .offset = internal_rd_u32(&index[k_jof_idx_ofs_offset]),
    .length = internal_rd_u32(&index[k_jof_idx_ofs_length]),
  };
  ...
  rc                = jof_read_tile(context->pread,
                                    context->pread_ctx,
  ...
  uint16_t want_w = 0U;
  uint16_t want_h = 0U;
  rc              = jof_tile_dims(&context->candidate->info, tx, ty, &want_w, &want_h);
  if (rc != k_ra8_ok) {
    return rc;
  }
                    ...
                    jof_audit_result_t*    out)
{
  ...
  ra8_err_t                rc   = jof_audit_requirements(pread, pread_ctx, total_size, &need);
  if (rc != k_ra8_ok) {
    return rc;
  }
  ...
  rc                           = jof_parse(pread, pread_ctx, total_size, &candidate.info);
  if (rc != k_ra8_ok) {
    return rc;
  }
  ...
  internal_audit_context_t context = {.pread           = pread,
                                      ...
                                      .workspace       = workspace,
                                      .candidate       = &candidate,
                                      .expected_offset = (uint32_t)k_jof_hdr_bytes};
