 ...
 */
RA8_INTERNAL
RA8_INTERNAL static void internal_wr_u16(uint8_t* buf, uint16_t v)
{
  buf[0] = (uint8_t)(v & (uint16_t)k_jof_byte_mask);
  buf[1] = (uint8_t)((uint32_t)v >> k_jof_le_sh8);
}
 ...
 */
RA8_INTERNAL
RA8_INTERNAL static void internal_wr_u32(uint8_t* buf, uint32_t v)
{
  buf[0] = (uint8_t)(v & (uint32_t)k_jof_byte_mask);
  buf[1] = (uint8_t)((v >> k_jof_le_sh8) & (uint32_t)k_jof_byte_mask);
  buf[2] = (uint8_t)((v >> k_jof_le_sh16) & (uint32_t)k_jof_byte_mask);
  buf[3] = (uint8_t)((v >> k_jof_le_sh24) & (uint32_t)k_jof_byte_mask);
}
 ...
 */
RA8_INTERNAL
RA8_INTERNAL static ra8_err_t internal_sink(jof_prod_state_t* st, const uint8_t* buf, size_t len)
{
...
RA8_PRIV ra8_err_t priv_jof_prefix_pull(void* ctx, uint8_t* buf, size_t cap, size_t* got)
{
  jof_prefix_pull_t* pfx = (jof_prefix_pull_t*)ctx;
 ...
 * @details Serializes every geometry field per the JOF layout table in jof.h.
 ...
 */
RA8_INTERNAL
RA8_INTERNAL static ra8_err_t internal_emit_header(jof_prod_state_t* st)
{
  ...
  hdr[2]                       = 'F';
  hdr[3]                       = '1';
  internal_wr_u16(&hdr[k_jof_ofs_width], st->w);
  internal_wr_u16(&hdr[k_jof_ofs_height], st->h);
  internal_wr_u16(&hdr[k_jof_ofs_tile_w], st->cfg->tile_w);
  internal_wr_u16(&hdr[k_jof_ofs_tile_h], st->cfg->tile_h);
  hdr[k_jof_ofs_bpp]   = st->bpp;
  hdr[k_jof_ofs_codec] = st->cfg->codec;
  internal_wr_u32(&hdr[k_jof_ofs_tile_count], st->tile_count);
  return internal_sink(st, hdr, sizeof(hdr));
}
 ...
 */
RA8_INTERNAL
RA8_INTERNAL static ra8_err_t internal_carve_pixel_path(jof_prod_state_t* st)
{
  ...
  // mcdc-deactivated: stage_bytes = tw_eff*th_eff*bpp with tw_eff <= w and th_eff <= tile_h, so stage_bytes <= w*tile_h*bpp = band_bytes always; the stage-overflow arm cannot flip independently of the band arm (which apps/shared_libs/jof/tests/src/test_jof_produce_guards.c drives true via a 32768-wide RGBA source and a 65535-tall tile).
  if ((band_bytes > (uint64_t)UINT32_MAX) || (stage_bytes > (uint64_t)UINT32_MAX)) {
    return k_ra8_err_invalid_size;
  }
  st->band  = priv_jof_bump_take(st->bump, (size_t)band_bytes);
  st->stage = priv_jof_bump_take(st->bump, (size_t)stage_bytes);
  st->idx   = priv_jof_bump_take(st->bump, (size_t)st->tile_count * (size_t)k_jof_index_entry);
  if ((st->band == nullptr) || (st->stage == nullptr) || (st->idx == nullptr)) {
    return k_ra8_err_invalid_size;
  }
  if (st->cfg->codec == (uint8_t)k_jof_codec_deflate) {
    st->cmp_cap = jof_stored_bound((uint32_t)stage_bytes);
    st->cmp     = priv_jof_bump_take(st->bump, (size_t)st->cmp_cap);
    if (st->cmp == nullptr) {
      return k_ra8_err_invalid_size;
    }
  }
 ...
 * @since 0.1.0
 */
RA8_PRIV ra8_err_t priv_jof_on_geom(void* ctx, uint16_t width, uint16_t height, uint8_t channels)
{
  jof_prod_state_t* st = (jof_prod_state_t*)ctx;
  if (st->geom_done != 0U) {
  ...
  if ((cols * rows) > (uint32_t)k_jof_max_tiles) {
    return k_ra8_err_invalid_size;
  }
  st->w          = width;
  st->h          = height;
  ...
  ra8_err_t err  = internal_carve_pixel_path(st);
  if (err != k_ra8_ok) {
    return err;
  }
  err = internal_emit_header(st);
  if (err != k_ra8_ok) {
    return err;
  }
  st->geom_done = 1U;
 ...
 *          through the zero-heap `ra8_compress()` into the bounded
 ...
 */
RA8_INTERNAL
RA8_INTERNAL static ra8_err_t internal_encode_tile(jof_prod_state_t* st, uint32_t payload)
{
  const uint8_t* out_bytes = st->stage;
  uint32_t       out_len   = payload;
  if (st->cfg->codec == (uint8_t)k_jof_codec_deflate) {
    uint32_t clen = 0U;
    if (ra8_compress(st->stage, payload, st->cmp, st->cmp_cap, st->dfl, st->dfl_len, &clen) !=
        k_ra8_ok) {
  ...
  uint8_t* entry = &st->idx[(size_t)st->tiles_done * (size_t)k_jof_index_entry];
  internal_wr_u32(&entry[k_jof_idx_ofs_offset], st->written);
  internal_wr_u32(&entry[k_jof_idx_ofs_length], out_len);
  const ra8_err_t err = internal_sink(st, out_bytes, (size_t)out_len);
  if (err != k_ra8_ok) {
    return err;
  }
  st->tiles_done++;
 ...
 */
RA8_INTERNAL
RA8_INTERNAL static ra8_err_t internal_flush_band(jof_prod_state_t* st, uint32_t th)
{
  ...
  for (uint32_t tx = 0U; tx < (uint32_t)st->tile_cols; tx++) {
    ...
    const ra8_err_t err = internal_encode_tile(st, row_bytes * th);
    if (err != k_ra8_ok) {
      return err;
    }
  }
...
RA8_PRIV ra8_err_t priv_jof_on_rows(void*          ctx,
                                    const uint8_t* px,
                                    uint16_t       width,
                                    uint16_t       y0,
                                    uint16_t       nrows,
                                    uint8_t        channels)
{
  ...
  while (left > 0U) {
    ...
    st->band_fill += take;
    st->rows_seen += take;
    src = &src[(size_t)take * (size_t)stride];
    left -= take;
    if (st->band_fill == (uint32_t)st->cfg->tile_h) {
      const ra8_err_t err = internal_flush_band(st, st->band_fill);
      if (err != k_ra8_ok) {
        return err;
      }
      st->band_fill = 0U;
 ...
 * @details Wraps ::priv_jof_on_geom for `ra8_jpeg_sw_decode_stripes()`, which
 ...
 * @retval other                  Propagated from ::priv_jof_on_geom.
 ...
 */
RA8_INTERNAL
RA8_INTERNAL static ra8_err_t internal_jpeg_geom(void*     ctx,
                                                 uint16_t  width,
                                                 uint16_t  height,
                                                 uint8_t   channels,
...
{
  jof_prod_state_t* st  = (jof_prod_state_t*)ctx;
  const ra8_err_t   err = priv_jof_on_geom(ctx, width, height, channels);
  if (err != k_ra8_ok) {
    return err;
  }
  const uint32_t cap = (uint32_t)width * (uint32_t)stripe_rows * (uint32_t)channels;
  uint8_t*       buf = priv_jof_bump_take(st->bump, (size_t)cap);
  if (buf == nullptr) {
    return k_ra8_err_invalid_size;
  }
  *out_stripe     = buf;
 ...
 */
RA8_INTERNAL
RA8_INTERNAL static ra8_err_t internal_finish(jof_prod_state_t* st, jof_info_t* out_info)
{
  if (st->tiles_done != st->tile_count) {
    return k_ra8_err_validation_failed;
  }
  const uint32_t index_off = st->written;
  ra8_err_t err = internal_sink(st, st->idx, (size_t)st->tile_count * (size_t)k_jof_index_entry);
  if (err != k_ra8_ok) {
    return err;
  }
  uint8_t ftr[k_jof_footer_bytes] = {};
  internal_wr_u32(&ftr[k_jof_ftr_index_off], index_off);
  internal_wr_u32(&ftr[k_jof_ftr_tile_count], st->tile_count);
  internal_wr_u32(&ftr[k_jof_ftr_total_size], st->written + (uint32_t)k_jof_footer_bytes);
  ftr[k_jof_ftr_magic]      = 'J';
  ftr[k_jof_ftr_magic + 1U] = 'O';
  ftr[k_jof_ftr_magic + 2U] = 'F';
  ftr[k_jof_ftr_magic + 3U] = 'E';
  err                       = internal_sink(st, ftr, sizeof(ftr));
  if (err != k_ra8_ok) {
    return err;
  }
  out_info->width      = st->w;
 ...
 */
RA8_INTERNAL
RA8_INTERNAL static ra8_err_t internal_check_cfg(const jof_produce_cfg_t* cfg)
{
  if ((cfg->tile_w == 0U) || (cfg->tile_h == 0U)) {
    return k_ra8_err_invalid_arg;
  }
  if (cfg->codec > (uint8_t)k_jof_codec_deflate) {
    return k_ra8_err_invalid_arg;
  }
  return k_ra8_ok;
}
...
{
  if ((max_width == 0U) || ((uint32_t)max_width > (uint32_t)k_jof_max_dim) || (max_height == 0U) ||
      ((uint32_t)max_height > (uint32_t)k_jof_max_dim)) {
    return 0U;
  }
  ...
  if ((cols * rows) > (uint64_t)k_jof_max_tiles) {
    return 0U;
  }
  const uint64_t bpp = (uint64_t)k_jof_bpp_max;
  ...
  const uint64_t png_set = (uint64_t)sizeof(tinfl_decompressor) + (uint64_t)k_jof_png_ring +
                           (uint64_t)k_jof_png_inbuf + (3U * ((uint64_t)max_width * bpp)) + 2U;
  const uint64_t dec_set = (jpeg_set > png_set) ? jpeg_set : png_set;
  const uint64_t band    = (uint64_t)max_width * (uint64_t)tile_h * bpp;
  const uint64_t tw_eff =
    ((uint64_t)tile_w < (uint64_t)max_width) ? (uint64_t)tile_w : (uint64_t)max_width;
  const uint64_t th_eff =
    ((uint64_t)tile_h < (uint64_t)max_height) ? (uint64_t)tile_h : (uint64_t)max_height;
  const uint64_t stage       = tw_eff * th_eff * bpp;
  const uint64_t cmp         = (uint64_t)jof_stored_bound((uint32_t)stage);
  const uint64_t index_bytes = cols * rows * (uint64_t)k_jof_index_entry;
  const uint64_t dfl         = (uint64_t)k_ra8_compress_scratch_bytes;
  const uint64_t total =
    dec_set + band + stage + cmp + index_bytes + dfl + (uint64_t)k_jof_carve_slack;
 ...
 */
RA8_INTERNAL
RA8_INTERNAL static ra8_err_t internal_init_state(jof_prod_state_t*        st,
                                                  const jof_produce_cfg_t* cfg)
{
  (void)memset(st, 0, sizeof(*st));
  st->cfg        = cfg;
  st->bump_store = (jof_bump_t){.base = cfg->work, .cap = cfg->work_cap, .off = 0U};
  st->bump       = &st->bump_store;
  st->cap_w      = ((cfg->max_width != 0U) && ((uint32_t)cfg->max_width < (uint32_t)k_jof_max_dim))
                     ? cfg->max_width
                     : (uint16_t)k_jof_max_dim;
  st->cap_h = ((cfg->max_height != 0U) && ((uint32_t)cfg->max_height < (uint32_t)k_jof_max_dim))
                ? cfg->max_height
                : (uint16_t)k_jof_max_dim;
  if (cfg->codec == (uint8_t)k_jof_codec_deflate) {
    st->dfl_len = (uint32_t)k_ra8_compress_scratch_bytes;
    st->dfl     = priv_jof_bump_take(st->bump, (size_t)st->dfl_len);
    if (st->dfl == nullptr) {
      return k_ra8_err_invalid_size;
    }
  }
 ...
 * @param[out] head Receives ::k_jof_sniff_bytes source bytes.
 * @return Result code.
 ...
 * @pre @p head holds ::k_jof_sniff_bytes writable bytes.
 ...
 */
RA8_INTERNAL
RA8_INTERNAL static ra8_err_t internal_sniff_head(const jof_produce_cfg_t* cfg, uint8_t* head)
{
  size_t head_len = 0U;
  while (head_len < (size_t)k_jof_sniff_bytes) {
    size_t          got = 0U;
    const ra8_err_t err =
      cfg->pull(cfg->pull_ctx, &head[head_len], (size_t)k_jof_sniff_bytes - head_len, &got);
    if (err != k_ra8_ok) {
      return err;
    }
    if (got == 0U) {
 ...
 *          `test_jof_produce.c` (JPEG head), the PNG head vector, and
 *          `test_jof_produce_webp.c` (WebP RIFF+WEBP head, plus the
 ...
 * @pre `internal_init_state()` succeeded.
...
internal_dispatch(jof_prod_state_t* st, const uint8_t* head, jof_prefix_pull_t* pfx)
{
  if ((head[0] == (uint8_t)k_jof_jpeg_soi_first) && (head[1] == (uint8_t)k_jof_jpeg_soi_second)) {
    uint8_t* window = priv_jof_bump_take(st->bump, (size_t)k_ra8_jpeg_sw_stream_min_window);
    if (window == nullptr) {
      return k_ra8_err_invalid_size;
    }
    return ra8_jpeg_sw_decode_stripes(priv_jof_prefix_pull,
                                      pfx,
                                      window,
 ...
 *          dispatch order mirrors ::internal_dispatch, which is what keeps "the
...
RA8_INTERNAL
RA8_INTERNAL static ra8_err_t
internal_probe_sniff(const uint8_t* data, size_t len, uint16_t* out_w, uint16_t* out_h)
{
  if (len < (size_t)k_jof_sniff_bytes) {
  ...
  if ((w == 0U) || (h == 0U) || (w > (uint32_t)k_jof_max_dim) || (h > (uint32_t)k_jof_max_dim)) {
    return k_ra8_err_invalid_size;
  }
  *out_w = (uint16_t)w;
  *out_h = (uint16_t)h;
...
ra8_err_t jof_probe_dims(const uint8_t* data, size_t len, uint16_t* out_w, uint16_t* out_h)
{
  ...
  return internal_probe_sniff(data, len, out_w, out_h);
}
 ...
 * @pre `internal_dispatch()` returned success.
 ...
 */
RA8_INTERNAL
RA8_INTERNAL static ra8_err_t internal_epilogue(jof_prod_state_t* st, jof_info_t* out_info)
{
  ...
  return internal_finish(st, out_info);
}

/**
 * @brief Reject any NULL `jof_produce` argument or seam.
 ...
 */
RA8_INTERNAL
RA8_INTERNAL static ra8_err_t internal_produce_args_ok(const jof_produce_cfg_t* cfg,
...
{
  static jof_prod_state_t s_prod;
  ra8_err_t               err = internal_produce_args_ok(cfg, out_info);
  if (err != k_ra8_ok) {
    return err;
  }
  err = internal_check_cfg(cfg);
  if (err != k_ra8_ok) {
    return err;
  }
  jof_prod_state_t* st = &s_prod;
  err                  = internal_init_state(st, cfg);
  if (err != k_ra8_ok) {
    return err;
  }
  uint8_t head[k_jof_sniff_bytes] = {};
  err                             = internal_sniff_head(cfg, head);
  if (err != k_ra8_ok) {
    return err;
  }
  ...
  err = internal_dispatch(st, head, &pfx);
  if (err != k_ra8_ok) {
    return err;
  }
  return internal_epilogue(st, out_info);
}
