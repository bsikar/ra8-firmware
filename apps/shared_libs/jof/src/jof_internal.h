...
/**
 * @enum jof_bump_const_t
 * @brief Bump-allocator alignment constants.
 */
typedef enum : uint8_t {
  k_jof_bump_align = 8U, /**< Every carve is 8-byte aligned. */
} jof_bump_const_t;
 ...
 * @since 0.1.0
 */
typedef ra8_err_t (*jof_rows_fn)(void*          ctx,
                                 const uint8_t* px,
                                 uint16_t       width,
                                 uint16_t       y0,
                                 uint16_t       nrows,
 ...
 * @pre @p bump has capacity per `jof_work_bytes()`.
 ...
 * @since 0.1.0
 */
RA8_PRIV ra8_err_t priv_jof_png_rows(jof_pull_fn pull,
                                     void*       pull_ctx,
                                     jof_bump_t* bump,
                                     uint16_t    max_w,
                                     uint16_t    max_h,
...
/**
 * @enum jof_carve_const_t
 * @brief Carve-sizing constant shared by both work-arena calculators.
 */
typedef enum : uint32_t {
  k_jof_carve_slack = 128U, /**< Alignment slack folded into every arena sizing. */
} jof_carve_const_t;
 ...
 */
typedef struct {
  const jof_produce_cfg_t* cfg;        /**< Caller configuration.    */
  jof_bump_t               bump_store; /**< Arena allocator state.   */
  jof_bump_t*              bump;       /**< Arena allocator (owned). */
 ...
 *          inner source. Matches ::jof_pull_fn so it can be passed
 *          as the pull seam to any decoder.
 *
 * @param[in]  ctx A ::jof_prefix_pull_t.
 ...
 *          JOF header has been sunk. Matches ::jof_geom_fn.
 *
 * @param[in] ctx      The producer state (::jof_prod_state_t).
 ...
 *          loop splits it. Matches ::jof_rows_fn.
 *
 * @param[in] ctx      The producer state (::jof_prod_state_t).
 ...
 *          the frame out through ::priv_jof_on_rows. Every carve is
 ...
 *
 * @pre `priv_jof_prefix_pull` replays the sniffed head before the source.
