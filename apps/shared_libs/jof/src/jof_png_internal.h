 ...
 * cap:
 *
 *   - `jof_png_chunk.c` -- byte source helpers plus the chunk
 *     layer: signature/IHDR prologue, PLTE/tRNS parsing, ancillary skipping
 *     and the post-IDAT walk to IEND.
 *   - `jof_png.c`       -- the pixel layer: geometry binding, the
...
typedef enum : uint8_t {
  k_ra8_png_sig_high = 0x89U, /**< High-bit signature byte. */
  k_ra8_png_sig_cr   = 0x0DU, /**< Carriage return byte.    */
  k_ra8_png_sig_lf   = 0x0AU, /**< Line feed byte.          */
  k_ra8_png_sig_sub  = 0x1AU, /**< DOS EOF byte.            */
} ra8_png_signature_t;
 ...
 */
typedef struct {
  jof_pull_fn pull;     /**< Sequential byte source. */
  void*       pull_ctx; /**< Context for `pull`.     */
  jof_geom_fn on_geom;  /**< Producer geometry hook. */
  jof_rows_fn on_rows;  /**< Producer row sink.      */
...
/* ---------------------------------------------------------------------------
 * Chunk-layer primitives (defined in jof_png_chunk.c, driven by
 * the pixel layer in jof_png.c).
 * ---------------------------------------------------------------------------
 */
