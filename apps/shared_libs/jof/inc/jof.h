 ...
 * source image is transcoded once (`jof_produce()`) into a grid of
 ...
 *     (RFC 1951, no zlib/gzip wrapper -- `ra8_compress()` output) that
 ...
 * already in-tree (miniz, reused via `ra8_compress`/`ra8_decompress`),
 ...
 * ## Validation model
 *
 * Atlases arrive from untrusted EPUB content. `jof_parse()` and
 * `jof_read_tile()` are fail-closed: every geometry field, both
 ...
 * @see jof_produce.h  Import-time transcode producer.
 * @see epub_img_tiles.h     EPUB tile-cache binder over this format.
...
typedef enum : uint8_t {
  k_jof_hdr_bytes      = 32U, /**< Header length.                   */
  k_jof_footer_bytes   = 16U, /**< Footer length.                   */
  k_jof_index_entry    = 8U,  /**< Bytes per tile-index entry.      */
  k_jof_magic_len      = 4U,  /**< Magic string length (both ends). */
  k_jof_ofs_magic      = 0U,  /**< Header: magic "JOF1".            */
  k_jof_ofs_width      = 4U,  /**< Header: u16 image width.         */
  k_jof_ofs_height     = 6U,  /**< Header: u16 image height.        */
  k_jof_ofs_tile_w     = 8U,  /**< Header: u16 tile width.          */
  k_jof_ofs_tile_h     = 10U, /**< Header: u16 tile height.         */
  k_jof_ofs_bpp        = 12U, /**< Header: u8 bytes per pixel.      */
  k_jof_ofs_codec      = 13U, /**< Header: u8 tile codec.           */
  k_jof_ofs_reserved   = 14U, /**< Header: first reserved byte.     */
  k_jof_ofs_tile_count = 16U, /**< Header: u32 tile count.          */
  k_jof_ofs_reserved2  = 20U, /**< Header: second reserved run.     */
  k_jof_ftr_index_off  = 0U,  /**< Footer: u32 index offset.        */
  k_jof_ftr_tile_count = 4U,  /**< Footer: u32 tile count.          */
  k_jof_ftr_total_size = 8U,  /**< Footer: u32 total atlas size.    */
  k_jof_ftr_magic      = 12U, /**< Footer: magic "JOFE".            */
  k_jof_idx_ofs_offset = 0U,  /**< Index entry: u32 tile offset.    */
  k_jof_idx_ofs_length = 4U,  /**< Index entry: u32 tile length.    */
} jof_layout_t;
 ...
 *          `k_jof_bpp_max` is the largest legal bytes-per-pixel.
 *
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_jof_max_dim   = 32768U, /**< Max image width/height, pixels. */
  k_jof_max_tiles = 65536U, /**< Max tiles per atlas.            */
  k_jof_bpp_max   = 4U,     /**< Max bytes per pixel.            */
} jof_limits_t;
 ...
 */
typedef enum : uint8_t {
  k_jof_codec_raw     = 0U, /**< Tile stream is the packed pixels.   */
  k_jof_codec_deflate = 1U, /**< Tile stream is one raw-DEFLATE run. */
} jof_codec_t;
 ...
 *
 * @details Filled by `jof_parse()`; callers treat it as read-only
 ...
 *          entry (`epub_entry_pread`), an SD file, or a RAM region all
 ...
 */
typedef ra8_err_t (
  *jof_pread_fn)(void* ctx, uint64_t offset, uint8_t* buf, size_t len, size_t* got);
 ...
 * @see jof_memstore_sink()
 * @see jof_memstore_pread()
 * @since 0.1.0
 */
typedef struct {
...
/**
 * @brief Append `len` bytes to a ::jof_memstore_t (producer sink).
 *
 * @details Matching `jof_sink_fn` shape (see the producer header):
 *          bind with `ctx = &store`. Fails closed when the store is full so a
 *          hostile source can never write past `cap`.
 *
 * @param[in] ctx Store to append to (a ::jof_memstore_t*).
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t jof_memstore_sink(void* ctx, const uint8_t* buf, size_t len);

/**
 * @brief Positioned read from a ::jof_memstore_t (reader seam).
 *
 * @details Matching ::jof_pread_fn: bind with `ctx = &store`.
 *          Reads clamp at `store->len`; a read at/after the end reports
 *          `*got == 0` without error, mirroring `epub_entry_pread`.
 *
 * @param[in]  ctx    Store to read from (a ::jof_memstore_t*).
 ...
 */
[[nodiscard]] ra8_err_t
jof_memstore_pread(void* ctx, uint64_t offset, uint8_t* buf, size_t len, size_t* got);
 ...
 * non-zero geometry within ::jof_limits_t, legal bpp/codec, zeroed
 ...
 * `jof_read_tile()` (the index itself may be larger than any
 ...
 */
[[nodiscard]] ra8_err_t
jof_parse(jof_pread_fn pread, void* pread_ctx, uint64_t total_size, jof_info_t* out_info);
 ...
 *
 * @pre @p info came from a successful `jof_parse()`.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t jof_tile_dims(const jof_info_t* info,
                                      uint16_t          tile_x,
                                      uint16_t          tile_y,
                                      uint16_t*         out_w,
                                      uint16_t*         out_h);
 ...
 *          worst case). Size `jof_read_tile()`'s scratch with this
 ...
 * @pre The atlas codec is one of ::jof_codec_t.
 * @post No state mutated.
 * @post Return >= @p raw_bytes + 256.
 * @note Thread-safe (pure).
 * @since 0.1.0
 */
[[nodiscard]] uint32_t jof_stored_bound(uint32_t raw_bytes);
 ...
 *                         `jof_stored_bound()` of the tile payload
 *                         for deflate atlases.
 ...
 * @retval other                       Propagated from @p pread.
 *
 * @pre @p info came from a successful `jof_parse()` over the same
 *      backing.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t jof_read_tile(jof_pread_fn      pread,
                                      void*             pread_ctx,
