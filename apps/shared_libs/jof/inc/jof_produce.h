 ...
 *
 * @details
 * `jof_produce()` turns an arbitrary encoded source image into the
 * JOF atlas (`jof.h`) -- the single on-device normalized format --
 ...
 * (sized by `jof_webp_work_bytes()`) that holds the compressed
 ...
 * `jof_work_bytes()` computes the exact requirement):
 ...
 *
 * @see jof.h  The output format + reader.
 ...
 *          code. An EPUB entry cursor (`epub_entry_read`) matches this
 *          shape directly.
 *
 ...
 *          footer); the sink never seeks. `jof_memstore_sink()`
 ...
 * @see jof_work_bytes()
 * @see jof_webp_work_bytes()
 * @since 0.1.0
 */
typedef struct {
  jof_pull_fn pull;          /**< Encoded-source byte stream.                   */
  void*       pull_ctx;      /**< Context for `pull`.                           */
  jof_sink_fn sink;          /**< Atlas byte sink (append-only).                */
  ...
  size_t      work_cap;      /**< Arena size; see jof_work_bytes().             */
 ...
 * guarantees `jof_produce()` never fails on arena exhaustion for
 * an in-budget source.
 *
 ...
 * @retval 0   An argument was zero or exceeded ::k_jof_max_dim,
 *             or the tile grid would exceed ::k_jof_max_tiles.
 ...
 * the produce config guarantees `jof_produce()` never fails on WebP
 * arena exhaustion for an in-cap WebP source. This is *separate* from
 * `jof_work_bytes()`: that arena stays small (the streaming tile
 ...
 */
[[nodiscard]] uint32_t
jof_webp_work_bytes(uint16_t max_width, uint16_t max_height, uint32_t max_src_bytes);

/**
 * @brief Read a source image's pixel dimensions without decoding its body.
 *
 * @details
 * Sniffs the same three magics `jof_produce()` dispatches on (JPEG
 ...
 * ::jof_work_bytes and ::jof_webp_work_bytes both take it
 * as input.
 *
 ...
 * @post On k_ra8_ok both `*out_w` and `*out_h` are in `[1, k_jof_max_dim]`.
 ...
 * describes the finished atlas exactly as `jof_parse()` would report
 ...
 *       (it will fail `jof_parse()` -- no torn atlas is readable).
 ...
 * @endcode
 *
 * @see jof_parse()      Validate / reopen the produced atlas.
 * @see jof_read_tile()  Page tiles back in bounded RAM.
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t jof_produce(const jof_produce_cfg_t* cfg, jof_info_t* out_info);
