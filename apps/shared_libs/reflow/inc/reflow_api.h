 ...
 *
 * @details
 * This sub-header holds the `reflow` engine's callable surface: the
 * lifecycle entry points (`reflow_init` / `_close`), the loader-binding
 ...
 * out of the umbrella `reflow.h` so that header stays small; consumers
 * still include `reflow.h` and never reference this file directly. The
 * data model these functions operate on lives in `reflow_types.h`.
 *
 *
 * [Ring 4 / Reflow]
 * {World: NS}
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t reflow_init(uint16_t       viewport_w,
                                    uint16_t       viewport_h,
 ...
 *
 * @param[in,out] engine Engine returned by `reflow_init()`.
 *
 * @return ra8_err_t
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t reflow_close(reflow_t* engine);
 ...
 * @post Binding takes effect on the next reflow_layout_chapter() / re-flow.
 *
 * @note Not thread-safe; bind before laying out.
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t reflow_set_image_loader(reflow_t*              engine,
 ...
 *
 * @details Mirrors reflow_set_image_loader(). When bound, reflow_layout_chapter()
 ...
 * @post Binding takes effect on the next reflow_layout_chapter().
 *
 * @note Not thread-safe; bind before laying out.
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t
reflow_set_css_loader(reflow_t* engine, reflow_css_loader_fn loader, void* ctx);
 ...
 *       reflow_render_page() / reflow_render_page_at().
 * @see reflow_glyph_atlas_storage_t
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t reflow_set_glyph_atlas(reflow_t*                           engine,
 ...
 * same space reflow_render_page() uses); subtract the render origin first if
 ...
 * bytes via the engine, or pass it straight to reflow_href_split().
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t reflow_hit_test_link(const reflow_t* engine,
                                             uint32_t        page_idx,
                                             int32_t         x,
                                             int32_t         y,
 ...
 * same space reflow_render_page() uses -- so subtract the render origin
 ...
 *       overlap, matching ::reflow_hit_test_link and ::ra8_ui_hit_test.
 *
 * @par Example:
 * @code
 * uint32_t idx = 0U;
 ...
 * @endcode
 *
 * @see reflow_hit_test_link
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t reflow_hit_test_image(const reflow_t* engine,
                                              uint32_t        page_idx,
                                              int32_t         x,
                                              int32_t         y,
                                              uint32_t*       out_index);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t reflow_href_split(const char*         href,
                                          uint32_t            len,
 ...
 * buffer so `reflow_set_font_size()` can re-flow without the caller
 * re-supplying it.
 *
 * Algorithm summary:
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t reflow_layout_chapter(reflow_t*      engine,
 ...
 */
[[nodiscard]] ra8_err_t
reflow_render_page(const reflow_t* engine, uint32_t page_idx, void* framebuffer);
 ...
 *
 * @details
 * Identical to `reflow_render_page()` except every glyph (and link
 ...
 * the body's top-left. `reflow_render_page()` is exactly this with a
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t reflow_render_page_at(const reflow_t* engine,
                                              uint32_t        page_idx,
                                              int32_t         origin_x,
                                              int32_t         origin_y);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t reflow_get_page_count(const reflow_t* engine, uint32_t* out_count);
 ...
 * chapter handed to `reflow_layout_chapter()`. Glyph and page state
 ...
 * the engine, exactly like the `reflow_init()` font. If a chapter is already
 ...
 * @retval k_ra8_err_not_initialized `engine->in_use == 0`.
 * @retval k_ra8_err_invalid_size    `font_len < k_reflow_min_font_bytes`.
 ...
 * @see reflow_init(), reflow_set_font_size()
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t
 ...
 * this face instead of the default bound at ::reflow_init(); unmatched runs
 * fall back to the default. The app drives this after ::epub_open by walking
 ...
 * (`reflow_cache_serialize` / `_load` return ::k_ra8_err_invalid_state), so a
 ...
 * @retval k_ra8_err_not_initialized `engine->in_use == 0`.
 * @retval k_ra8_err_invalid_size    `len < k_reflow_min_font_bytes`.
 * @retval k_ra8_err_no_mem          The registry is full (::k_reflow_max_faces).
 ...
 *
 * @note Not thread-safe; single-threaded init/layout context.
 * @see reflow_init(), reflow_bind_font(), ra8_css_match_face()
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t
 ...
 *
 * @details
 * Internal helper used by `reflow_layout_chapter()`. Implemented by the
 * no-heap streaming tokenizer in `reflow_tokenize.c`. Declared here so
 * `reflow_layout.c` can call it without a forward declaration.
 *
 ...
 */
[[nodiscard]] ra8_err_t
reflow_parse_xhtml(reflow_t* engine, const uint8_t* xhtml_buf, size_t xhtml_len);
 ...
 * `reflow_parse_xhtml()` and writes positioned glyphs into
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t reflow_run_layout(reflow_t* engine);
