 ...
 *
 * @details
 * Laying out a chapter (`reflow_layout_chapter()`) is the expensive
 ...
 * @endcode
 *
 * `reflow_cache_load()` validates the blob's key against the engine's
...
/**
 * @enum reflow_cache_id_t
 * @brief On-wire format identity for a serialised pagination cache.
 *
 * @details
 * `k_reflow_cache_magic` is the first 4 bytes of every blob (stored
 * little-endian). `k_reflow_cache_version` is bumped whenever the
 * serialised layout changes; a load of a different version is treated as
 * stale (re-layout) rather than mis-parsed.
 */
typedef enum : uint32_t {
  k_reflow_cache_magic   = 0x52464331U, /**< Blob magic ('R''F''C''1'). */
  k_reflow_cache_version = 2U,          /**< Serialised format version. */
} reflow_cache_id_t;

/**
 * @enum reflow_cache_layout_t
 ...
 */
typedef enum : size_t {
  k_reflow_cache_header_bytes = 52U, /**< Fixed header size, bytes.   */
  k_reflow_cache_glyph_bytes  = 20U, /**< Serialised bytes per glyph. */
  k_reflow_cache_page_bytes   = 8U,  /**< Serialised bytes per page.  */
 ...
 * calling `reflow_cache_serialize()`. The value is
 * `k_reflow_cache_header_bytes +
 *  engine->glyph_count * k_reflow_cache_glyph_bytes +
 *  engine->page_count  * k_reflow_cache_page_bytes`.
 *
 ...
 * @post `*out_bytes >= k_reflow_cache_header_bytes`.
 *
 * @since 0.1.0
 */
 ...
 * later `reflow_cache_load()` can detect any change that would
 ...
 *       blob accepted by `reflow_cache_load()`.
 *
 * @see reflow_cache_size()
 * @see reflow_cache_load()
 *
 * @since 0.1.0
 */
 ...
 * later `reflow_set_font_size()` can still re-flow). On any mismatch
 ...
 *       proceed without `reflow_layout_chapter()`.
 * @post On any non-`k_ra8_ok` return the engine is unchanged.
 *
 * @see reflow_cache_serialize()
 *
 * @since 0.1.0
 */
