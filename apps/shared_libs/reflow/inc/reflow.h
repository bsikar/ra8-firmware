 ...
 *
 * @details
 * `reflow` is a small, hand-written HTML reflow + pagination engine
 * sitting between `apps/shared_libs/epub` (which produces per-chapter XHTML
 ...
 *     a `reflow_limits_t` enum and lives inside the engine handle.
 ...
 *     exceed `viewport_w - 2 * k_reflow_margin_px`, the engine
 *     breaks and starts a new line.
 *   - **Page-break-on-overflow.** When the accumulated line height
 *     would exceed `viewport_h - 2 * k_reflow_margin_px`, the
 ...
 * ## Lifecycle
 *
 *   1. `reflow_init()` -- bind viewport, font, colours.
 *   2. `reflow_layout_chapter()` -- parse + lay out one XHTML
 *      chapter, return total page count.
 *   3. `reflow_render_page()` -- rasterise page N into the active
 *      ra8_gfx framebuffer.
 *   4. (optional) `reflow_set_font_size()` -- triggers a re-flow on
 *      the cached chapter.
 *   5. `reflow_close()` -- mark engine unused.
 *
 * @note This is a thin umbrella header. The data model (enums, structs,
 *       the engine handle) lives in `reflow_types.h`, and the callable
 *       function prototypes live in `reflow_api.h`. Consumers should
 *       continue to include `reflow.h` directly -- it pulls in both
...
#include "reflow_api.h"   /* lifecycle / layout / render / internal prototypes */
#include "reflow_types.h" /* enums, structs, typedefs, engine handle           */
