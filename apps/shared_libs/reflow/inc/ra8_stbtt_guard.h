 ...
 *
 * @details
 * The vendored stb_truetype
 * (`apps/shared_libs/third_party/stb/stb_truetype.h`) parses the
 ...
 * `@font-face` fonts embedded in an EPUB (via `reflow_register_face()` /
 * `reflow_bind_font()`) and the book font attached with
 * `epub_set_font()`. An attacker-supplied EPUB can thus reach the unguarded
 * parse on-device.
 *
