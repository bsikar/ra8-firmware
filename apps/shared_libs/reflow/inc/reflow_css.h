 ...
 * style itself. It sits inside `apps/shared_libs/reflow` as a peer of the HTML
 ...
 *   - **Properties** (the ones that fold onto reflow's existing per-run
 ...
 * ::reflow_html_tag_t, ::reflow_align_t, ::reflow_font_style_t) so this
 * header stays free of a circular include back into `reflow.h`.
 *
 *
 * [Ring 4 / Reflow]
 * {World: NS}
 ...
 * ::reflow_font_style_t bits (a set bit = "on"); `align` holds an
 * ::reflow_align_t. A field is only meaningful when its ::ra8_css_set_t bit is
 * present in `set`.
 */
typedef struct {
  uint8_t  set;        /**< ::ra8_css_set_t bitmask: which properties are present.  */
  uint8_t  style;      /**< ::reflow_font_style_t bit VALUES (bold/italic/ul).      */
  uint8_t  align;      /**< ::reflow_align_t (valid iff `set & align`).             */
 ...
 * element itself. `tag == k_reflow_tag_unknown` / `class_len == 0` /
 * `id_len == 0` mean that part of the constraint is absent.
 */
typedef struct {
  uint8_t  tag;       /**< Type tag, or k_reflow_tag_unknown = no type.  */
 ...
 * non-default: `sel_tag != k_reflow_tag_unknown`, `class_len > 0`,
 ...
 */
typedef struct {
  uint8_t         sel_tag;                /**< Type tag, or k_reflow_tag_unknown = no type.   */
 ...
 */
typedef struct {
  uint8_t     tag;       /**< ::reflow_html_tag_t of the element.    */
