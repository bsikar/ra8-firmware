 ...
 * API for the v1 CSS subset documented in `reflow_css.h`. The scanner walks
 * `selector { ... }` blocks and hands each to ::priv_reflow_css_parse_one_block in
...
#include "reflow.h" /* k_reflow_tag_unknown, style/align enums */
...
ra8_err_t ra8_css_parse(ra8_css_sheet_t* sheet, const char* css, uint32_t len)
{
  ...
  while (i < (size_t)len) {
    ...
    if (priv_reflow_css_is_ws(css[i])) {
      ++i;
      continue;
    }
    size_t brace = 0U;
    ...
    priv_reflow_css_parse_one_block(sheet,
                                    &css[i],
                                    brace - i,
                                    &css[brace + 1U],
                                    close - (brace + 1U));
 ...
 * (not case-folded) comparison of each token against @p name[0..nlen), byte by byte.
...
internal_class_list_has(const char* list, size_t list_len, const char* name, size_t nlen)
{
  ...
  while (i < list_len) {
    while ((i < list_len) && priv_reflow_css_is_ws(list[i])) {
      ++i;
    }
    size_t start = i;
    while ((i < list_len) && !priv_reflow_css_is_ws(list[i])) {
    ...
    bool         equal = tlen == nlen;
    for (size_t j = 0U; equal && (j < nlen); ++j) {
      if (list[start + j] != name[j]) {
        equal = false;
      }
    }
    if (equal) {
                          ...
                          const ra8_css_sheet_t*   sheet)
{
  ...
  if ((rule->sel_tag != (uint8_t)k_reflow_tag_unknown) && (el->tag != rule->sel_tag)) {
    return false;
  }
  ...
  if (rule->id_len != 0U) {
    ...
    bool        equal = (el->id != nullptr) && (el->id_len == rule->id_len);
    for (uint16_t i = 0U; equal && (i < rule->id_len); ++i) {
      if (el->id[i] != nm[i]) {
        equal = false;
      }
    }
 ...
 *   - id_len:    @p el->id must equal the interned id byte for byte.
...
static bool internal_anc_matches(const ra8_css_anc_t*     anc,
                                 ...
                                 const ra8_css_sheet_t*   sheet)
{
  if ((anc->tag != (uint8_t)k_reflow_tag_unknown) && (el->tag != anc->tag)) {
    return false;
  }
  if (anc->class_len != 0U) {
  ...
  if (anc->id_len != 0U) {
    ...
    bool        equal = (el->id != nullptr) && (el->id_len == anc->id_len);
    for (uint16_t i = 0U; equal && (i < anc->id_len); ++i) {
      if (el->id[i] != nm[i]) {
        equal = false;
      }
    }
...
{
  uint16_t spec = 0U;
  if (rule->sel_tag != (uint8_t)k_reflow_tag_unknown) {
  ...
  for (uint8_t a = 0U; a < rule->anc_count; ++a) {
    if (rule->anc[a].tag != (uint8_t)k_reflow_tag_unknown) {
      spec = (uint16_t)(spec + (uint16_t)k_priv_spec_type);
    }
                                      ...
                                      const ra8_css_style_t* inl)
{
  static const struct {
    ...
    {(uint8_t)k_ra8_css_set_bold, (uint8_t)k_reflow_style_bold},
    {(uint8_t)k_ra8_css_set_italic, (uint8_t)k_reflow_style_italic},
    {(uint8_t)k_ra8_css_set_underline, (uint8_t)k_reflow_style_underline},
  };
 ...
 * ASCII case-folding via priv_reflow_css_lower(). Returns false immediately when either
...
static bool internal_ci_eq_span(const char* a, size_t alen, const char* b, size_t blen)
{
  ...
  for (size_t k = 0U; k < alen; ++k) {
    if (priv_reflow_css_lower(a[k]) != priv_reflow_css_lower(b[k])) {
      return false;
    }
  }
  return true;
                           ...
                           bool                   want_italic)
{
  ...
  for (uint16_t i = 0U; i < sheet->face_count; ++i) {
    ...
    const bool face_bold   = f->weight_bold != 0U;
    const bool face_italic = f->style_italic != 0U;
    if ((face_bold == want_bold) && (face_italic == want_italic)) {
      return (int16_t)i;
    }
