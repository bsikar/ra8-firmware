 ...
 * the shared arc flatten in reflow_svg_shape.c. The resulting vertices are
...
{
  static const float k_cubic = 3.0F;
  const float        mt      = 1.0F - tt;
  return ((mt * mt * mt) * c0) + (k_cubic * (mt * mt) * tt * c1) + (k_cubic * mt * (tt * tt) * c2) +
         ((tt * tt * tt) * c3);
}
...
{
  static const float k_quadratic = 2.0F;
  const float        mt          = 1.0F - tt;
  return ((mt * mt) * c0) + (k_quadratic * mt * tt * c1) + ((tt * tt) * c2);
}
                                   ...
                                   int32_t            n)
{
  ...
  if ((u == 'q') || (u == 't')) {
    ...
    const int32_t  e  = sm ? INT32_C(0) : INT32_C(2);
  ...
  if ((u == 'c') || (u == 's')) {
    ...
    const svg_pt_t p1 = sm ? internal_smooth_ctrl(p0, st, 'c')
                           : internal_arg_pt(rel, st->cx, st->cy, args[0], args[1]);
    const int32_t  b  = sm ? INT32_C(0) : INT32_C(2);
...
{
  while ((*i < dlen) && (priv_ra8_svgp_ws((char)d[*i]) || (d[*i] == (uint8_t)','))) {
    ++(*i);
  }
  if (*i >= dlen) {
    return 0;
