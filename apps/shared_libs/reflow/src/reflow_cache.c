/**
 * @file reflow_cache.c
 * @brief Import-time pagination cache serialiser / loader for `reflow`.
 *
 * @details
 * Implements `reflow_cache.h`: turn a laid-out engine's `glyphs[]` +
 * `pages[]` into a keyed little-endian byte blob and back, so the
 * expensive `reflow_layout_chapter()` pass can be skipped when the
 ...
 * against the engine's static pools (`k_reflow_max_glyphs` /
 * `k_reflow_max_pages`) and the blob length, so a malformed blob can
 ...
 * @pre  `buf` has at least `k_reflow_cache_header_bytes` capacity.
 * @post Header bytes `[0, k_reflow_cache_header_bytes)` are written.
...
RA8_INTERNAL
static void
internal_put_header(uint8_t* buf, const reflow_t* engine, size_t content_len, uint32_t content_hash)
{
  size_t off = 0U;
  internal_put_u32(buf, &off, (uint32_t)k_reflow_cache_magic);
  internal_put_u16(buf, &off, (uint16_t)k_reflow_cache_version);
 ...
 * @pre  `buf` has at least `k_reflow_cache_header_bytes` readable.
 ...
 * @retval value `>= k_reflow_cache_header_bytes`.
 *
...
{
  return (size_t)k_reflow_cache_header_bytes +
         ((size_t)glyph_count * (size_t)k_reflow_cache_glyph_bytes) +
         ((size_t)page_count * (size_t)k_reflow_cache_page_bytes);
}
 ...
 */
RA8_INTERNAL
static bool internal_key_matches(const reflow_t*         engine,
 ...
 * engine's static pools (`k_reflow_max_glyphs` /
 * `k_reflow_max_pages`), then computes the expected total blob size
 ...
 *
 * @note Internal helper for `reflow_cache_load()`; depends only on its
...
{
  if (len < (size_t)k_reflow_cache_header_bytes) {
    return k_ra8_err_invalid_size;
  }
  internal_get_header(buf, key);
  if (key->magic != (uint32_t)k_reflow_cache_magic) {
    return k_ra8_err_not_found;
  }
  if (key->version != (uint32_t)k_reflow_cache_version) {
    return k_ra8_err_invalid_state;
  }
  if (key->glyph_count > (uint32_t)k_reflow_max_glyphs) {
    return k_ra8_err_invalid_size;
  }
  if (key->page_count > (uint32_t)k_reflow_max_pages) {
    return k_ra8_err_invalid_size;
  }
...
[[nodiscard]] ra8_err_t reflow_cache_size(const reflow_t* engine, size_t* out_bytes)
{
...
RA8_INTERNAL
static ra8_err_t
internal_cache_precheck(const reflow_t* engine, const uint8_t* content, size_t content_len)
{
  if (engine->in_use == 0U) {
    return k_ra8_err_not_initialized;
  }
...
[[nodiscard]] ra8_err_t reflow_cache_serialize(const reflow_t* engine,
                                               const uint8_t*  content,
                                               size_t          content_len,
                                               uint8_t*        out_buf,
                                               size_t          out_cap,
                                               size_t*         out_len)
{
  ...
  size_t off = (size_t)k_reflow_cache_header_bytes;
  ...
  const uint32_t checksum = internal_fnv1a(&out_buf[(size_t)k_reflow_cache_header_bytes],
                                           need - (size_t)k_reflow_cache_header_bytes);
...
[[nodiscard]] ra8_err_t reflow_cache_load(reflow_t*      engine,
                                          const uint8_t* content,
                                          size_t         content_len,
                                          const uint8_t* buf,
                                          size_t         len)
{
  ...
  const uint32_t checksum = internal_fnv1a(&buf[(size_t)k_reflow_cache_header_bytes],
                                           need - (size_t)k_reflow_cache_header_bytes);
