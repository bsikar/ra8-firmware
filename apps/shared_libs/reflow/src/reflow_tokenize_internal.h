 ...
 * @brief Test-access surface for reflow_tokenize.c internal helpers.
 * @ingroup grp_ereader
 *
 * @details
 ...
 * (reflow_tokenize.c, reflow_tokenize_lex.c): the lexical primitives use
 ...
 * @see priv_reflow_tok_utf8_encode
 * @since 0.1.0
 */
 ...
 * @since 0.1.0
 */
RA8_PRIV bool priv_reflow_tok_is_xml_whitespace(char c);
 ...
 * @return The matching reflow_html_tag_t, or k_reflow_tag_unknown.
 * @retval k_reflow_tag_unknown Name is null or unrecognised.
 * @pre None.
 * @pre None.
 * @post No state is modified (pure).
 * @post No state is modified (pure).
 ...
 * @since 0.1.0
 */
RA8_PRIV reflow_html_tag_t priv_reflow_tok_classify(const char* name, size_t len);
 ...
 * (reflow_tokenize_lex.c / reflow_tokenize_attr.c). Promoted from
 ...
 * introduce a block box. Defined in reflow_tokenize_lex.c.
 *
 ...
 * @since 0.1.0
 */
RA8_PRIV bool priv_reflow_tok_is_block(reflow_html_tag_t tag);
 ...
 * tag contributes nothing. Defined in reflow_tokenize_lex.c.
 *
 * @param[in] tag Classified tag.
 ...
 * @since 0.1.0
 */
RA8_PRIV uint8_t priv_reflow_tok_style_for(reflow_html_tag_t tag);

/**
 * @brief Append one token to the engine's token pool.
 *
 * @details Fills the next reflow_token_t and advances token_count, or fails
 * if the fixed pool is full. Defined in reflow_tokenize_lex.c.
 *
 ...
 * @since 0.1.0
 */
RA8_PRIV bool priv_reflow_tok_emit(reflow_t*           engine,
 ...
 * Defined in reflow_tokenize_lex.c.
 *
 ...
 * @since 0.1.0
 */
RA8_PRIV bool priv_reflow_tok_feed(reflow_t* engine, char ch, bool* last_ws);
 ...
 * @since 0.1.0
 */
RA8_PRIV bool priv_reflow_tok_stash_run(reflow_t*      engine,
                                        const uint8_t* buf,
                                        size_t         start,
                                        size_t         end,
                                        uint32_t*      out_off,
 ...
 */
RA8_PRIV bool
priv_reflow_tok_starts_with(const uint8_t* buf, size_t i, size_t len, const char* lit);
 ...
 * @since 0.1.0
 */
RA8_PRIV size_t priv_reflow_tok_skip_past(const uint8_t* buf,
                                          size_t         i,
                                          size_t         len,
                                          const char*    lit);
 ...
 *
 * @details Like ::priv_reflow_tok_skip_past but returns the literal's START
 * offset. Defined in reflow_tokenize_lex.c.
 *
 * @param[in] buf Source buffer.
 * @param[in] i   Offset to search from.
 * @param[in] len Total buffer length.
 ...
 * @since 0.1.0
 */
RA8_PRIV size_t priv_reflow_tok_find_lit(const uint8_t* buf, size_t i, size_t len, const char* lit);
 ...
 * @since 0.1.0
 */
RA8_PRIV bool priv_reflow_tok_find_attr(const uint8_t* tag,
                                        size_t         tag_len,
                                        const char*    name,
                                        size_t         name_len,
                                        size_t*        out_voff,
 ...
 * @since 0.1.0
 */
RA8_PRIV void priv_reflow_tok_capture_attr(reflow_t*      engine,
                                           const uint8_t* tag,
                                           size_t         tag_len,
                                           const char*    name,
                                           size_t         name_len,
 ...
 * pointers alias @p tag. Defined in reflow_tokenize_attr.c.
 *
 ...
 * @pre `kind` is a valid `reflow_html_tag_t` value.
 * @post No state mutated; returned pointers alias @p tag.
 * @post `el.tag == (uint8_t)kind`.
 * @note Pure read of @p tag.
 * @since 0.1.0
 */
RA8_PRIV ra8_css_element_t priv_reflow_tok_css_element(reflow_html_tag_t kind,
                                                       const uint8_t*    tag,
                                                       size_t            span);
 ...
 * declarations override the cascade. Defined in reflow_tokenize_attr.c.
 *
 * @param[in] tag  Raw tag span starting at '<'.
 * @param[in] span Length of @p tag, bytes.
 ...
 * @since 0.1.0
 */
RA8_PRIV ra8_css_style_t priv_reflow_tok_css_inline(const uint8_t* tag, size_t span);
 ...
 * empty href or a full table returns 0. Defined in reflow_tokenize_attr.c.
 *
 ...
 * @retval 1..k_reflow_max_links The newly assigned 1-based link id.
 * @pre `engine` is non-null.
 * @pre `engine->link_target_count <= k_reflow_max_links`.
 * @post On a non-zero return the table grew by one entry.
 * @post `engine->link_target_count` is unchanged on a zero return.
 * @note Not thread-safe.
 * @since 0.1.0
 */
RA8_PRIV uint8_t priv_reflow_tok_intern_link(reflow_t* engine,
                                             uint32_t  href_off,
                                             uint32_t  href_len);
 ...
 * @since 0.1.0
 */
RA8_PRIV bool priv_reflow_tok_rel_is_stylesheet(const uint8_t* rel, size_t len);
 ...
 * @pre `engine->face_count <= k_reflow_max_faces`.
 * @post No state is modified.
 * @post Return value is in [0, engine->face_count].
 * @note Pure; reads engine state only.
 * @since 0.1.0
 */
RA8_PRIV uint8_t priv_reflow_tok_resolve_face_slot(const reflow_t*        engine,
 ...
 * no heap and no DOM. Defined in `reflow_tokenize.c` and consumed by the
 * layout driver in `reflow_parse.c`; the per-decision helpers above are
 ...
 * @since 0.1.0
 */
RA8_PRIV ra8_err_t priv_reflow_xml_walk(reflow_t*      engine,
