 ...
 * stages live in the sibling reflow_svg_*.c files. No DOM, no heap. See
 * reflow_svg.h for the public scope and reflow_svg_internal.h for the
...
{
  if ((c >= '0') && (c <= '9')) {
    const uint32_t code = (uint32_t)(uint8_t)c;
    return (uint8_t)(code - (uint32_t)(uint8_t)'0');
  }
  const char l = priv_ra8_svgp_lc(c);
  if ((l >= 'a') && (l <= 'f')) {
    const uint32_t code = (uint32_t)(uint8_t)l;
    return (uint8_t)((code - (uint32_t)(uint8_t)'a') + (uint32_t)k_svg_hex_a10);
  }
...
int32_t priv_ra8_svgp_num(const uint8_t* s, size_t len, size_t* i)
{
  ...
  if ((*i < len) && ((s[*i] == (uint8_t)'-') || (s[*i] == (uint8_t)'+'))) {
    sign = (s[*i] == (uint8_t)'-') ? INT32_C(-1) : INT32_C(1);
    ++(*i);
  }
  int32_t v = 0;
  while ((*i < len) && (s[*i] >= (uint8_t)'0') && (s[*i] <= (uint8_t)'9')) {
    v = (v * (int32_t)k_svg_dec) + ((int32_t)s[*i] - (int32_t)'0');
    ++(*i);
  }
...
static bool internal_attr_at(const uint8_t* s, size_t len, size_t at, const char* name)
{
  ...
  return (s[after] == (uint8_t)'=') || priv_ra8_svgp_ws((char)s[after]);
}
...
bool priv_ra8_svgp_attr(const uint8_t* s, size_t len, const char* name, size_t* voff, size_t* vlen)
{
  ...
  for (size_t i = 0U; i < len; ++i) {
    ...
    if ((j >= len) || ((s[j] != (uint8_t)'"') && (s[j] != (uint8_t)'\''))) {
      return false;
    }
...
int32_t priv_match_grad(const svg_grads_t* grads, const uint8_t* val, size_t vlen)
{
  ...
  size_t       idend = idoff;
  while ((idend < vlen) && (val[idend] != (uint8_t)')')) {
    ++idend;
  }
  ...
  for (int32_t i = 0; i < grads->n; ++i) {
    ...
    bool        equal = strlen(gid) == idlen;
    for (size_t k = 0U; (k < idlen) && equal; ++k) {
      equal = (uint8_t)gid[k] == val[idoff + k];
    }
    if (equal) {
      return i;
    }
