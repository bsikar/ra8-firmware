 ...
 *
 * The driver reaches this module via @ref priv_reflow_layout_apply_image; the
 ...
 */
RA8_INTERNAL
static bool internal_image_resolve_size(reflow_t*             engine,
                                        ...
                                        int32_t*              out_h)
{
  ...
  const int32_t col_w   = (int32_t)engine->viewport_w - (2 * (int32_t)k_reflow_margin_px);
  const int32_t avail_h = (int32_t)engine->viewport_h - (2 * (int32_t)k_reflow_margin_px);
 ...
 */
RA8_INTERNAL
static void internal_image_record(reflow_t*             engine,
                                  priv_cursor_t*        cur,
                                  const reflow_token_t* tok,
                                  int32_t               bw,
                                  int32_t               bh)
{
  reflow_image_box_t* box = &engine->image_boxes[engine->image_box_count];
  box->x                  = (int32_t)k_reflow_margin_px;
  box->y                  = cur->y;
  box->w                  = bw;
  box->h                  = bh;
  ...
  cur->y += bh + (int32_t)k_reflow_paragraph_gap_px;
  cur->line_top         = cur->y;
...
{
  if (engine->image_box_count >= (uint32_t)k_reflow_max_images) {
    return false;
  }
  int32_t bw = 0;
  int32_t bh = 0;
