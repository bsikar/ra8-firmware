 ...
 * overflow).
 *
 * The driver enters this module through @ref priv_reflow_layout_table; the
 * core inline-flow helpers it reuses (line height, glyph advance/push, line
 * wrap, page flush) are shared via `reflow_layout_internal.h`.
 *
 *
 ...
 * @pre `engine->tokens[start].kind == k_reflow_tok_block_start`.
...
static uint32_t internal_match_block_end(const reflow_t* engine, uint32_t start, uint32_t limit)
{
  ...
  for (uint32_t i = start + 1U; i < limit; ++i) {
    ...
    if (tok->kind == (uint8_t)k_reflow_tok_block_start) {
      depth++;
    } else if (tok->kind == (uint8_t)k_reflow_tok_block_end) {
      depth--;
      if (depth == 0) {
        return i;
      }
    } else {
      /* Other token kinds do not change the matching-block depth. */
    }
  }
  return limit;
}
 ...
 * (must be `k_reflow_tag_td` or `k_reflow_tag_th`). Used by
...
{
  const reflow_token_t* tok = &engine->tokens[i];
  return (tok->kind == (uint8_t)k_reflow_tok_block_start) &&
         ((tok->tag == (uint8_t)k_reflow_tag_td) || (tok->tag == (uint8_t)k_reflow_tag_th));
}
 ...
 *
 * @details Checks both the token kind (must be `block_start`) and the tag
 * (must be `k_reflow_tag_tr`). Used by `internal_table_columns` and
 * `priv_reflow_layout_table` to locate row boundaries while scanning the flat
...
{
  const reflow_token_t* tok = &engine->tokens[i];
  return (tok->kind == (uint8_t)k_reflow_tok_block_start) && (tok->tag == (uint8_t)k_reflow_tag_tr);
}
 ...
 * `(*lines)++`. Glyphs are pushed via @ref priv_reflow_layout_push_glyph with
 ...
 */
RA8_INTERNAL
static void internal_cell_text(reflow_t*             engine,
                               ...
                               uint32_t*             lines)
{
  const int32_t  line_h = (int32_t)priv_reflow_layout_line_height(font_px);
  const uint8_t* base   = &engine->text_pool[tok->text_off];
  const uint32_t len    = tok->text_len;
  uint32_t       i      = 0U;
  while (i < len) {
    if (base[i] == (uint8_t)' ') {
      const int32_t adv = priv_reflow_layout_glyph_advance(font, font_px, (int32_t)' ');
      /* mcdc-deactivated: priv_reflow_tok_stash_run collapses leading whitespace of every text run (last_ws starts true), so a cell text token never begins with a space, and the pen always advances past cell_x after a word is emitted; a space is therefore only ever seen with *cx > cell_x, making the (*cx > cell_x) false arm unreachable. */
      if ((*cx > cell_x) && ((*cx + adv) <= cell_right)) {
        (void)priv_reflow_layout_push_glyph(engine, *cx, *cy, (int32_t)' ', font_px, 0U, color, 0U);
        *cx += adv;
      }
      ++i;
      continue;
    ...
    while ((word_end < len) && (base[word_end] != (uint8_t)' ')) {
      word_w += priv_reflow_layout_glyph_advance(font, font_px, (int32_t)base[word_end]);
      ++word_end;
    }
    ...
    while (i < word_end) {
      const int32_t adv = priv_reflow_layout_glyph_advance(font, font_px, (int32_t)base[i]);
      (void)
        priv_reflow_layout_push_glyph(engine, *cx, *cy, (int32_t)base[i], font_px, 0U, color, 0U);
      *cx += adv;
      ++i;
    }
  }
 ...
 * `k_reflow_tok_text` token, delegates to @ref internal_cell_text which
 ...
 */
RA8_INTERNAL
static uint32_t internal_layout_cell(reflow_t*             engine,
                                     const stbtt_fontinfo* font,
                                     int32_t               cell_x,
                                     int32_t               cell_w,
                                     int32_t               top_y,
                                     ...
                                     uint32_t              tend)
{
  ...
  for (uint32_t t = tstart; t < tend; ++t) {
    const reflow_token_t* tok = &engine->tokens[t];
    if (tok->kind == (uint8_t)k_reflow_tok_text) {
 ...
 */
RA8_INTERNAL
static uint32_t internal_row_cells(reflow_t*             engine,
                                   const stbtt_fontinfo* font,
                                   uint32_t              tr_start,
                                   uint32_t              tr_end,
                                   int32_t               col_w,
                                   int32_t               row_y)
{
  ...
  while (i < tr_end) {
    ...
    const int32_t  cell_x   = (int32_t)k_reflow_margin_px + ((int32_t)col * col_w) + pad;
 ...
 * flush is issued via @ref priv_reflow_layout_finish_page, and the row is re-laid
 ...
 */
RA8_INTERNAL
static uint32_t internal_layout_row(reflow_t*             engine,
                                    priv_cursor_t*        cur,
                                    const stbtt_fontinfo* font,
                                    uint32_t              tr_start,
                                    uint32_t              table_end,
                                    int32_t               col_w)
{
  ...
  const int32_t  line_h       = (int32_t)priv_reflow_layout_line_height(engine->font_px);
  ...
  if (((cur->y + row_h) > bottom_limit) && (cur->y > (int32_t)k_reflow_margin_px)) {
    ...
    (void)priv_reflow_layout_finish_page(engine,
                                   ...
                                   uint32_t*             out_next)
{
  ...
  const int32_t content_w = (int32_t)engine->viewport_w - (2 * (int32_t)k_reflow_margin_px);
  ...
  cur->align            = (uint8_t)k_reflow_align_left;
