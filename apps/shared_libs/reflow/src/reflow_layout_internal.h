 ...
 * @brief Cross-TU shared declarations for the reflow layout engine.
 * @ingroup grp_ereader
 *
 * @details
...
typedef struct {
  ...
  uint8_t  align;            /**< Active block alignment (reflow_align_t).  */
  uint8_t  reserved8[2];     /**< Padding.                                  */
} priv_cursor_t;

/* ===========================================================================
 * Cross-TU layout helpers (defined in reflow_layout.c unless noted).
 * ===========================================================================
 */
 ...
 * @since 0.1.0
 */
RA8_PRIV void priv_reflow_layout_byte_zero(uint8_t* dst, size_t n);
 ...
 * @since 0.1.0
 */
RA8_PRIV ra8_err_t priv_reflow_layout_init_font(const reflow_t* engine, stbtt_fontinfo* out_font);
 ...
 * @since 0.1.0
 */
RA8_PRIV uint16_t priv_reflow_layout_line_height(uint16_t font_px);
 ...
 * @since 0.1.0
 */
RA8_PRIV int32_t priv_reflow_layout_glyph_advance(const stbtt_fontinfo* font,
                                                  uint16_t              font_px,
                                                  int32_t               cp);
 ...
 *
 * @details Appends a new @ref reflow_glyph_t entry at
 ...
 * capacity (`engine->glyph_count >= k_reflow_max_glyphs`).
 *
 ...
 *
 * @pre `engine != nullptr`.
 * @pre `engine->glyph_count <= k_reflow_max_glyphs`.
 ...
 * @since 0.1.0
 */
RA8_PRIV bool priv_reflow_layout_push_glyph(reflow_t* engine,
                                            int32_t   x,
                                            int32_t   y,
                                            int32_t   cp,
                                            uint16_t  font_px,
 ...
 * @since 0.1.0
 */
RA8_PRIV bool priv_reflow_layout_finish_page(reflow_t* engine, priv_cursor_t* cur);
 ...
 * would exceed the bottom margin it calls @ref priv_reflow_layout_finish_page to
 ...
 * @post On false, `engine->page_count == k_reflow_max_pages`.
 *
 * @note Not thread-safe; caller must serialize access to @p engine.
 * @since 0.1.0
 */
RA8_PRIV bool priv_reflow_layout_newline(reflow_t* engine, priv_cursor_t* cur, bool allow_justify);
 ...
 * @since 0.1.0
 */
RA8_PRIV ra8_err_t priv_reflow_layout_apply_token(reflow_t*             engine,
                                                  priv_cursor_t*        cur,
 ...
 *      layout pass.
 * @pre `engine->link_rect_count == 0` on entry (reset by `reflow_run_layout`).
 ...
 * @since 0.1.0
 */
RA8_PRIV void priv_reflow_layout_build_link_rects(reflow_t* engine, const stbtt_fontinfo* font);

/**
 * @brief Apply an `<img>` token: real image when a loader is bound, else a
 *        placeholder.
 *
 * @details Defined in `reflow_layout_image.c`. When the engine has a bound
 ...
 */
RA8_PRIV bool
priv_reflow_layout_apply_image(reflow_t* engine, priv_cursor_t* cur, const reflow_token_t* tok);

/**
 * @brief Lay out a `<table>` token range as an equal-column grid.
 *
 * @details Flushes the pending line (if any) via @ref priv_reflow_layout_newline,
 ...
 * @post On `k_ra8_ok`, `cur->align == k_reflow_align_left` and
 *       `cur->line_has_content == 0`.
 *
 * @note Not thread-safe; caller must serialize access to @p engine and @p cur.
 * @since 0.1.0
 */
RA8_PRIV ra8_err_t priv_reflow_layout_table(reflow_t*             engine,
                                            priv_cursor_t*        cur,
                                            const stbtt_fontinfo* font,
                                            uint32_t              start,
                                            uint32_t*             out_next);
