 ...
 * for the v1 CSS subset documented in `reflow_css.h`. This translation unit
...
#include "reflow.h" /* style/align enums, k_ra8_css_font_* units */
 ...
 * string @p lit using ASCII case folding via priv_reflow_css_lower(). Returns true only
...
bool priv_reflow_css_ci_eq(const char* s, size_t len, const char* lit)
{
  ...
  size_t k = 0U;
  for (; (k < len) && (lit[k] != '\0'); ++k) {
    if (priv_reflow_css_lower(s[k]) != priv_reflow_css_lower(lit[k])) {
      return false;
    }
  }
 ...
 * checks for a case-insensitive match at each position via priv_reflow_css_lower(). The
...
static bool internal_ci_contains(const char* s, size_t len, const char* sub)
{
  ...
  /* mcdc-deactivated: the sole caller passes internal_ci_contains(val, vlen, "underline") -- val is non-NULL (from priv_reflow_css_trim) and sub is a compile-time literal so sl == 9 (never 0); (s == nullptr) and (sl == 0U) are unreachable, only (sl > len) varies. */
  ...
  for (size_t i = 0U; (i + sl) <= len; ++i) {
    ...
    for (size_t j = 0U; j < sl; ++j) {
      if (priv_reflow_css_lower(s[i + j]) != priv_reflow_css_lower(sub[j])) {
        hit = false;
        break;
      }
    }
...
const char* priv_reflow_css_trim(const char* s, size_t* len)
{
  size_t a = 0U;
  size_t b = *len;
  while ((a < b) && priv_reflow_css_is_ws(s[a])) {
    ++a;
  }
  while ((b > a) && priv_reflow_css_is_ws(s[b - 1U])) {
    --b;
  }
  *len = b - a;
  return &s[a];
 ...
 * its numeric value (0-15). The digit is first lower-cased via priv_reflow_css_lower() so
...
{
  if ((c >= '0') && (c <= '9')) {
    const int value = (int)c - (int)'0';
    return (uint8_t)value;
  }
  const char l = priv_reflow_css_lower(c);
  if ((l >= 'a') && (l <= 'f')) {
    const int value = ((int)l - (int)'a') + (int)k_priv_hex_a10;
    return (uint8_t)value;
  }
...
{
  /* mcdc-deactivated: the sole caller internal_apply_decl passes val (non-NULL, from priv_reflow_css_trim) and vlen > 0 (gated by (plen > 0U) && (vlen > 0U) in priv_reflow_css_parse_decls); (s == nullptr) and (len == 0U) are unreachable on any public path. */
  if ((s == nullptr) || (len == 0U)) {
    return (uint32_t)k_priv_col_invalid;
  }
  if (s[0] == '#') {
  ...
  if (priv_reflow_css_ci_eq(s, len, "black")) {
    return (uint32_t)k_priv_col_black;
  }
  if (priv_reflow_css_ci_eq(s, len, "white")) {
    return (uint32_t)k_priv_col_white;
  }
  if (priv_reflow_css_ci_eq(s, len, "red")) {
    return (uint32_t)k_priv_col_red;
  }
  if (priv_reflow_css_ci_eq(s, len, "green")) {
    return (uint32_t)k_priv_col_green;
  }
  if (priv_reflow_css_ci_eq(s, len, "blue")) {
    return (uint32_t)k_priv_col_blue;
  }
  if (priv_reflow_css_ci_eq(s, len, "gray") || priv_reflow_css_ci_eq(s, len, "grey")) {
    return (uint32_t)k_priv_col_gray;
  }
  if (priv_reflow_css_ci_eq(s, len, "silver")) {
    return (uint32_t)k_priv_col_silver;
  }
  if (priv_reflow_css_ci_eq(s, len, "maroon")) {
    return (uint32_t)k_priv_col_maroon;
  }
  if (priv_reflow_css_ci_eq(s, len, "navy")) {
...
static uint32_t internal_scan_hundredths(const char* s, size_t len, size_t* i, bool* any)
{
  ...
  while ((*i < len) && (s[*i] >= '0') && (s[*i] <= '9')) {
    const int digit = (int)s[*i] - (int)'0';
    hund            = (hund * (uint32_t)k_priv_fs_dec) + (uint32_t)digit;
    *any            = true;
    ++(*i);
  }
...
static bool internal_parse_fontsize(const char* s, size_t len, uint16_t* out_val, uint8_t* out_unit)
{
  ...
  if ((rem == (size_t)k_priv_fs_ulen) && priv_reflow_css_ci_eq(&s[i], rem, "px")) {
  ...
  if ((rem == (size_t)k_priv_fs_ulen) && priv_reflow_css_ci_eq(&s[i], rem, "em")) {
...
static bool internal_apply_emphasis(const char*      prop,
                                    ...
                                    ra8_css_style_t* out)
{
  if (priv_reflow_css_ci_eq(prop, plen, "font-weight")) {
    const bool on =
      priv_reflow_css_ci_eq(val, vlen, "bold") || priv_reflow_css_ci_eq(val, vlen, "bolder") ||
      priv_reflow_css_ci_eq(val, vlen, "600") || priv_reflow_css_ci_eq(val, vlen, "700") ||
      priv_reflow_css_ci_eq(val, vlen, "800") || priv_reflow_css_ci_eq(val, vlen, "900");
    internal_set_emphasis(out, (uint8_t)k_ra8_css_set_bold, (uint8_t)k_reflow_style_bold, on);
    return true;
  }
  if (priv_reflow_css_ci_eq(prop, plen, "font-style")) {
    const bool on =
      priv_reflow_css_ci_eq(val, vlen, "italic") || priv_reflow_css_ci_eq(val, vlen, "oblique");
    internal_set_emphasis(out, (uint8_t)k_ra8_css_set_italic, (uint8_t)k_reflow_style_italic, on);
    return true;
  }
  if (priv_reflow_css_ci_eq(prop, plen, "text-decoration") ||
      priv_reflow_css_ci_eq(prop, plen, "text-decoration-line")) {
    const bool on = internal_ci_contains(val, vlen, "underline");
    internal_set_emphasis(out,
                          (uint8_t)k_ra8_css_set_underline,
                          (uint8_t)k_reflow_style_underline,
                          on);
    return true;
  }
  return false;
 ...
 * @p out->align to the matching reflow_align constant:
 *   - "right"   -> k_reflow_align_right
 *   - "center"  -> k_reflow_align_center
 *   - "justify" -> k_reflow_align_justify
 *   - any other -> k_reflow_align_left (CSS default)
 * Comparison is case-insensitive via priv_reflow_css_ci_eq().
 *
...
static void internal_apply_align(const char* val, size_t vlen, ra8_css_style_t* out)
{
  ...
  if (priv_reflow_css_ci_eq(val, vlen, "right")) {
    out->align = (uint8_t)k_reflow_align_right;
  } else if (priv_reflow_css_ci_eq(val, vlen, "center")) {
    out->align = (uint8_t)k_reflow_align_center;
  } else if (priv_reflow_css_ci_eq(val, vlen, "justify")) {
    out->align = (uint8_t)k_reflow_align_justify;
  } else {
    out->align = (uint8_t)k_reflow_align_left;
  }
}
                                ...
                                ra8_css_style_t* out)
{
  ...
  if (priv_reflow_css_ci_eq(prop, plen, "text-align")) {
    internal_apply_align(val, vlen, out);
  } else if (priv_reflow_css_ci_eq(prop, plen, "color")) {
  ...
  } else if (priv_reflow_css_ci_eq(prop, plen, "font-size")) {
    uint16_t fv = 0U;
    uint8_t  fu = 0U;
  ...
  } else if (priv_reflow_css_ci_eq(prop, plen, "display")) {
    out->set     = (uint8_t)(out->set | (uint8_t)k_ra8_css_set_display);
    out->display = priv_reflow_css_ci_eq(val, vlen, "none") ? 1U : 0U;
  } else {
...
void priv_reflow_css_parse_decls(const char* s, size_t len, ra8_css_style_t* out)
{
  ...
  while (i < len) {
    ...
    if (colon < semi) {
      size_t      plen = colon - i;
      const char* prop = priv_reflow_css_trim(&s[i], &plen);
      size_t      vlen = semi - (colon + 1U);
      const char* val  = priv_reflow_css_trim(&s[colon + 1U], &vlen);
      if ((plen > 0U) && (vlen > 0U)) {
...
ra8_err_t ra8_css_parse_inline(const char* decls, uint32_t len, ra8_css_style_t* out)
{
  ...
  *out = (ra8_css_style_t){};
  priv_reflow_css_parse_decls(decls, (size_t)len, out);
  return k_ra8_ok;
}
