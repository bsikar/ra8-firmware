 ...
 *
 * @details
 * Companion translation unit to reflow_tokenize.c. It groups the routines
 ...
 * reflow_tokenize.c call the cross-TU helpers promoted into
 * reflow_tokenize_internal.h. Pure forward-pass scanning -- no recursion
...
static bool internal_attr_name_at(const uint8_t* tag, size_t i, const char* name, size_t name_len)
{
  ...
  const uint8_t prev = (i == 0U) ? (uint8_t)'<' : tag[i - 1U];
  return !(((prev >= (uint8_t)'a') && (prev <= (uint8_t)'z')) ||
           ((prev >= (uint8_t)'A') && (prev <= (uint8_t)'Z')));
}
...
{
  size_t j = pos;
  while ((j < tag_len) && priv_reflow_tok_is_xml_whitespace((char)tag[j])) {
    ++j;
  }
  if ((j >= tag_len) || (tag[j] != (uint8_t)'=')) {
    return false;
  }
  ++j;
  while ((j < tag_len) && priv_reflow_tok_is_xml_whitespace((char)tag[j])) {
  ...
  if ((j >= tag_len) || ((tag[j] != (uint8_t)'"') && (tag[j] != (uint8_t)'\''))) {
    return false;
  }
 ...
 *
 * @details The non-copying core of priv_reflow_tok_capture_attr(): returns the value's
                                  ...
                                  uint32_t*      out_len)
{
  ...
  *out_len      = 0U;
  size_t vstart = 0U;
  size_t vlen   = 0U;
  if (!priv_reflow_tok_find_attr(tag, tag_len, name, name_len, &vstart, &vlen)) {
    return;
  }
  if ((vlen == 0U) ||
      (((size_t)engine->text_pool_used + vlen) > (size_t)k_reflow_text_pool_bytes)) {
    return;
  }
  const uint32_t off = engine->text_pool_used;
  (void)memcpy(&engine->text_pool[off], &tag[vstart], vlen);
 ...
 */
ra8_css_element_t
priv_reflow_tok_css_element(reflow_html_tag_t kind, const uint8_t* tag, size_t span)
{
  ra8_css_element_t el = {};
  el.tag               = (uint8_t)kind;
  size_t off           = 0U;
  size_t len           = 0U;
  if (priv_reflow_tok_find_attr(tag, span, "id", sizeof("id") - 1U, &off, &len)) {
    el.id     = (const char*)&tag[off];
    el.id_len = (uint16_t)len;
  }
  if (priv_reflow_tok_find_attr(tag, span, "class", sizeof("class") - 1U, &off, &len)) {
...
ra8_css_style_t priv_reflow_tok_css_inline(const uint8_t* tag, size_t span)
{
  ...
  size_t          off = 0U;
  size_t          len = 0U;
  if (priv_reflow_tok_find_attr(tag, span, "style", sizeof("style") - 1U, &off, &len)) {
...
{
  if ((href_len == 0U) || (engine->link_target_count >= (uint32_t)k_reflow_max_links)) {
    return 0U;
  }
...
uint8_t priv_reflow_tok_resolve_face_slot(const reflow_t* engine, const ra8_css_style_t* comp)
{
  ...
  const bool    bold   = (comp->style & (uint8_t)k_reflow_style_bold) != 0U;
  const bool    italic = (comp->style & (uint8_t)k_reflow_style_italic) != 0U;
