/**
 * @file reflow_render.c
 * @brief Page-rasteriser for reflow.
 *
 * @details
 ...
 *          the function-local glyph mask and @c s_faces).
 *
...
/**
 * @brief File-static render context shared with the bound glyph atlas.
 *
 ...
 */
RA8_INTERNAL
static void internal_blit_alpha_mask(const reflow_glyph_t* g,
...
/**
 * @brief Rasterise one glyph into a function-local static mask and blit it.
 *
 ...
 *          tightly-packed mask (stride == w, matching
 ...
 * @note Not thread-safe; uses function-local static mask storage.
 * @since 0.1.0
 */
RA8_INTERNAL
static void internal_glyph_render_direct(const stbtt_fontinfo* font,
                                         ...
                                         int32_t               oy)
{
  /* Fixed rasterisation storage: rendering is single-threaded and heap-free. */
 ...
 */
RA8_INTERNAL
static void internal_render_svg(const reflow_t* engine,
                                const uint8_t*  svg,
                                size_t          len,
                                int32_t         x,
                                int32_t         y,
 ...
 */
RA8_INTERNAL
static void internal_render_one_image(const reflow_t*           engine,
...
RA8_INTERNAL
static void
internal_render_images(const reflow_t* engine, uint32_t page_idx, int32_t ox, int32_t oy)
{
 ...
 *                     `1 + k_reflow_max_faces` `stbtt_fontinfo` entries.
 ...
 *      least `1 + k_reflow_max_faces` entries.
 ...
 * shared body called by both `reflow_render_page` (ox=oy=0) and
 * `reflow_render_page_at` (caller-supplied origin).
 *
 ...
 * @note Not thread-safe; uses the function-local static glyph mask and face
 *       storage that must not be accessed concurrently.
 * @since 0.1.0
 */
RA8_INTERNAL
static ra8_err_t
internal_render_page(const reflow_t* engine, uint32_t page_idx, int32_t ox, int32_t oy)
{
  if (engine == nullptr) {
  ...
  /* One fontinfo per face (static because render is single-threaded; keeps the
   * ~1.4 KB off the stack). Default at 0, embedded at 1..N. */
  static stbtt_fontinfo s_faces[1U + (uint32_t)k_reflow_max_faces];
  uint8_t               nfaces = 0U;
  ...
  const reflow_page_t* page  = &engine->pages[page_idx];
  ra8_glyph_atlas_t*   atlas = engine->glyph_atlas;
  for (uint32_t i = 0U; i < page->glyph_count; ++i) {
    const reflow_glyph_t* g  = &engine->glyphs[page->glyph_first + i];
    uint8_t               fi = (uint8_t)(((uint32_t)g->style >> (uint32_t)k_reflow_face_shift) &
                                         (uint32_t)k_reflow_face_mask);
    if (fi >= nfaces) {
...
ra8_err_t
reflow_render_page_at(const reflow_t* engine, uint32_t page_idx, int32_t origin_x, int32_t origin_y)
{
