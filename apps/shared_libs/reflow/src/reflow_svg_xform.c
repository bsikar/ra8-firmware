...
{
  while ((*i < len) && (priv_ra8_svgp_ws((char)s[*i]) || (s[*i] == (uint8_t)','))) {
    ++(*i);
  }
  float sgn = 1.0F;
  if ((*i < len) && ((s[*i] == (uint8_t)'-') || (s[*i] == (uint8_t)'+'))) {
    sgn = (s[*i] == (uint8_t)'-') ? -1.0F : 1.0F;
    ++(*i);
  }
  float v = 0.0F;
  while ((*i < len) && (s[*i] >= (uint8_t)'0') && (s[*i] <= (uint8_t)'9')) {
    v = (v * (float)k_svg_dec) + ((float)s[*i] - (float)'0');
    ++(*i);
  }
  if ((*i < len) && (s[*i] == (uint8_t)'.')) {
    ++(*i);
    float frac = 1.0F;
    while ((*i < len) && (s[*i] >= (uint8_t)'0') && (s[*i] <= (uint8_t)'9')) {
      frac /= (float)k_svg_dec;
      v += frac * ((float)s[*i] - (float)'0');
      ++(*i);
    }
  }
  return v * sgn;
 ...
 *
 * @details Multiplies @p deg by the function-local constant ratio pi / 180. Used by the
 ...
 * @retval n  @p deg * (pi / 180).
 *
 * @pre  @p deg is a finite float value (NaN or infinity produce undefined float arithmetic).
 * @pre  Single-precision floating-point arithmetic is available.
 *
 * @post The return value equals @p deg * (pi / 180.0F).
...
static int32_t internal_xform_read(const uint8_t* v, size_t vlen, size_t* j, float* args)
{
  ...
  while (na < (int32_t)k_svg_argc_cube) {
    while ((*j < vlen) && (priv_ra8_svgp_ws((char)v[*j]) || (v[*j] == (uint8_t)','))) {
      ++(*j);
    }
...
{
  svg_utf_t result = internal_utf_identity();
  switch (kind) {
    case k_svg_xf_translate:
      result = (svg_utf_t){.a = 1.0F,
                           .b = 0.0F,
                           .c = 0.0F,
                           .d = 1.0F,
                           .e = args[0],
    ...
    case k_svg_xf_rotate:
      result = internal_xform_rotate(args, na);
      break;
    case k_svg_xf_skewx:
      result = (svg_utf_t){.a = 1.0F,
                           .b = 0.0F,
    ...
    case k_svg_xf_matrix:
      result = (svg_utf_t){.a = args[k_svg_xf_arg_a],
...
static svg_utf_t internal_parse_xform(const uint8_t* v, size_t vlen)
{
  ...
  while (i < vlen) {
    while ((i < vlen) && (priv_ra8_svgp_ws((char)v[i]) || (v[i] == (uint8_t)','))) {
      ++i;
    }
    const svg_xf_kind_t kind = internal_xform_kind(v, vlen, i);
    size_t              op   = i;
    while ((op < vlen) && (v[op] != (uint8_t)'(')) {
      ++op;
    }
    if (op >= vlen) {
      break;
