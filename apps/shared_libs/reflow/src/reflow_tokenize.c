/**
 * @file reflow_tokenize.c
 * @brief No-heap streaming XHTML tokenizer for reflow (replaces the
 *        former heap-backed DOM shim).
 *
 * @details
 ...
 * parse path: the firmware traps `_sbrk`, and the former DOM pool grew
 * via `new`, so the DOM walk could not run on the device.
 *
 * Output is byte-for-byte equivalent to the former DOM walk for the
 ...
 */
typedef struct {
  reflow_t* engine;      /**< Target engine pools.                */
...
static ra8_err_t internal_handle_cdata(tok_ctx_t* ctx, const uint8_t* buf, size_t* pi, size_t len)
{
  ...
  size_t       close = inner;
  while (((close + 3U) <= len) &&
         !((buf[close] == (uint8_t)']') && (buf[close + 1U] == (uint8_t)']') &&
           (buf[close + 2U] == (uint8_t)'>'))) {
    ++close;
  }
  if ((ctx->sp > 0U) && !internal_suppressed(ctx)) {
    ...
    for (size_t k = inner; k < close; ++k) {
      if (!priv_reflow_tok_feed(ctx->engine, (char)buf[k], &last_ws)) {
        return k_ra8_err_no_mem;
      }
    }
    const uint32_t tlen = ctx->engine->text_pool_used - off;
    if ((tlen > 0U) && !priv_reflow_tok_emit(ctx->engine,
                                             k_reflow_tok_text,
...
static ra8_err_t internal_handle_end(tok_ctx_t* ctx, const uint8_t* buf, size_t* pi, size_t len)
{
  ...
  *pi = (i < len) ? (i + 1U) : len;
  ...
  ctx->sp--;
  const reflow_html_tag_t tag = (reflow_html_tag_t)ctx->stack_el[ctx->sp].tag;
  ...
  if (priv_reflow_tok_is_block(tag) && !internal_suppressed(ctx) &&
      !priv_reflow_tok_emit(ctx->engine, k_reflow_tok_block_end, tag, ctx->style, 0U, 0U)) {
    return k_ra8_err_no_mem;
  }
 ...
 * @retval k_reflow_tag_unknown Unrecognised element name.
 ...
 */
RA8_INTERNAL
static reflow_html_tag_t
internal_parse_start(const uint8_t* buf, size_t* pi, size_t len, bool* selfclose)
{
  size_t       i      = *pi + 1U; /* past '<' */
  const size_t nstart = i;
  while ((i < len) && (buf[i] != (uint8_t)'>') && (buf[i] != (uint8_t)'/') &&
         !priv_reflow_tok_is_xml_whitespace((char)buf[i])) {
    ++i;
  }
  const reflow_html_tag_t tag = priv_reflow_tok_classify((const char*)&buf[nstart], i - nstart);
  *selfclose                  = false;
  while ((i < len) && (buf[i] != (uint8_t)'>')) {
    ...
    } else if ((c == '/') && ((i + 1U) < len) && (buf[i + 1U] == (uint8_t)'>')) {
      *selfclose = true;
    } else {
      /* Continue scanning the current start tag. */
    }
    ++i;
  }
  *pi = (i < len) ? (i + 1U) : len;
                                     ...
                                     uint16_t               css_font_px)
{
  ...
  if (block) {
    uint32_t id_off = 0U;
    uint32_t id_len = 0U;
    priv_reflow_tok_capture_attr(ctx->engine, tag, span, "id", sizeof("id") - 1U, &id_off, &id_len);
    if (!priv_reflow_tok_emit(ctx->engine,
    ...
    const uint8_t   align = ((comp->set & (uint8_t)k_ra8_css_set_align) != 0U)
                              ? comp->align
                              : (uint8_t)k_reflow_align_left;
    reflow_token_t* bst   = &ctx->engine->tokens[ctx->engine->token_count - 1U];
    bst->reserved         = align;
    bst->css_font_px      = css_font_px;
    return k_ra8_ok;
  }
  if (kind == k_reflow_tag_a) {
    uint32_t href_off = 0U;
    uint32_t href_len = 0U;
    priv_reflow_tok_capture_attr(ctx->engine,
                                 tag,
                                 span,
                                 "href",
                                 sizeof("href") - 1U,
                                 &href_off,
                                 &href_len);
    ctx->active_link = priv_reflow_tok_intern_link(ctx->engine, href_off, href_len);
  }
  return k_ra8_ok;
}
...
static void internal_handle_link(tok_ctx_t* ctx, const uint8_t* buf, size_t tag_lt, size_t end)
{
  ...
  size_t         href_off = 0U;
  size_t         href_len = 0U;
  if (!priv_reflow_tok_find_attr(tagbuf, span, "rel", sizeof("rel") - 1U, &rel_off, &rel_len) ||
      !priv_reflow_tok_find_attr(tagbuf, span, "href", sizeof("href") - 1U, &href_off, &href_len) ||
      !priv_reflow_tok_rel_is_stylesheet(&tagbuf[rel_off], rel_len)) {
    return;
  }
...
static bool internal_handle_void(tok_ctx_t*        ctx,
                                 ...
                                 ra8_err_t*        out_err)
{
  if (tag == k_reflow_tag_link) {
    internal_handle_link(ctx, buf, tag_lt, end); /* external stylesheet (no token) */
    *out_err = k_ra8_ok;
    return true;
  }
  const bool is_void =
    (tag == k_reflow_tag_br) || (tag == k_reflow_tag_hr) || (tag == k_reflow_tag_img);
  if (is_void && internal_suppressed(ctx)) {
    *out_err = k_ra8_ok; /* void tag inside a display:none subtree -> drop */
    return true;
  }
  if (tag == k_reflow_tag_br) {
    *out_err = priv_reflow_tok_emit(ctx->engine, k_reflow_tok_break, tag, ctx->style, 0U, 0U)
                 ? k_ra8_ok
                 : k_ra8_err_no_mem;
    return true;
  }
  if (tag == k_reflow_tag_hr) {
    *out_err = priv_reflow_tok_emit(ctx->engine, k_reflow_tok_rule, tag, ctx->style, 0U, 0U)
                 ? k_ra8_ok
                 : k_ra8_err_no_mem;
    return true;
  }
  if (tag == k_reflow_tag_img) {
    uint32_t     src_off = 0U;
    uint32_t     src_len = 0U;
    const size_t span    = (end > tag_lt) ? (end - tag_lt) : 0U;
    priv_reflow_tok_capture_attr(ctx->engine,
                                 ...
                                 &src_len);
    *out_err =
      priv_reflow_tok_emit(ctx->engine, k_reflow_tok_image, tag, ctx->style, src_off, src_len)
        ? k_ra8_ok
        : k_ra8_err_no_mem;
    return true;
  }
 ...
 * one). The result clamps to [k_reflow_min_font_px, k_reflow_max_font_px].
 *
 ...
 * @post Return value is clamped to [k_reflow_min_font_px, k_reflow_max_font_px] when non-zero.
...
static uint16_t internal_css_font_px(const tok_ctx_t* ctx, const ra8_css_style_t* comp)
{
  ...
  uint32_t px = ((ra8_css_font_unit_t)comp->font_unit == k_ra8_css_font_pct)
                  ? (((uint32_t)parent * (uint32_t)comp->font_val) / (uint32_t)k_reflow_pct_full)
                  : (uint32_t)comp->font_val;
  if (px < (uint32_t)k_reflow_min_font_px) {
    px = (uint32_t)k_reflow_min_font_px;
  }
  if (px > (uint32_t)k_reflow_max_font_px) {
    px = (uint32_t)k_reflow_max_font_px;
  }
  return (uint16_t)px;
}
...
static ra8_err_t internal_open_styled(tok_ctx_t*        ctx,
                                      ...
                                      bool              block)
{
  const uint8_t           base = (uint8_t)(ctx->style | priv_reflow_tok_style_for(tag));
  ...
  if (ctx->color != (uint32_t)k_reflow_color_inherit) {
  ...
  const ra8_css_style_t inl = priv_reflow_tok_css_inline(tagbuf, span);
  ...
  ctx->color = ((comp.set & (uint8_t)k_ra8_css_set_color) != 0U) ? comp.color
  ...
  ctx->face_slot = priv_reflow_tok_resolve_face_slot(ctx->engine, &comp);
  return k_ra8_ok;
}
...
static ra8_err_t internal_handle_start(tok_ctx_t* ctx, const uint8_t* buf, size_t* pi, size_t len)
{
  ...
  const reflow_html_tag_t tag       = internal_parse_start(buf, pi, len, &selfclose);
  ...
  const bool block = priv_reflow_tok_is_block(tag);
  if (selfclose) {
    if (block && !internal_suppressed(ctx)) {
      if (!priv_reflow_tok_emit(ctx->engine, k_reflow_tok_block_start, tag, ctx->style, 0U, 0U)) {
        return k_ra8_err_no_mem;
      }
      if (!priv_reflow_tok_emit(ctx->engine, k_reflow_tok_block_end, tag, ctx->style, 0U, 0U)) {
        return k_ra8_err_no_mem;
      }
    }
    return k_ra8_ok;
  ...
  ctx->stack_el[ctx->sp]      = priv_reflow_tok_css_element(tag, &buf[tag_lt], span);
...
{
  if (!priv_reflow_tok_starts_with(buf, i, len, name)) {
    return false;
  }
  ...
  return (c == '>') || (c == '/') || priv_reflow_tok_is_xml_whitespace(c);
}
...
{
  ctx->saw_element      = true;
  const size_t open_end = priv_reflow_tok_skip_past(buf, *pi, len, ">");
  const size_t close_at = priv_reflow_tok_find_lit(buf, open_end, len, close_lit);
...
{
  if (priv_reflow_tok_starts_with(buf, *pi, len, "<!--")) {
    *pi = priv_reflow_tok_skip_past(buf, *pi + 4U, len, "-->");
    return k_ra8_ok;
  }
  if (priv_reflow_tok_starts_with(buf, *pi, len, "<![CDATA[")) {
    return internal_handle_cdata(ctx, buf, pi, len);
  }
  if (priv_reflow_tok_starts_with(buf, *pi, len, "<?")) {
    *pi = priv_reflow_tok_skip_past(buf, *pi + 2U, len, "?>");
    return k_ra8_ok;
  }
  if (priv_reflow_tok_starts_with(buf, *pi, len, "<!")) {
    *pi = priv_reflow_tok_skip_past(buf, *pi + 2U, len, ">");
  ...
  if (((*pi + 1U) < len) && (buf[*pi + 1U] == (uint8_t)'/')) {
...
static bool internal_emit_text_run(tok_ctx_t* ctx, const uint8_t* buf, size_t run, size_t end)
{
  ...
  uint32_t off = 0U;
  uint32_t tln = 0U;
  if (!priv_reflow_tok_stash_run(ctx->engine, buf, run, end, &off, &tln)) {
    return false;
  }
  if (tln == 0U) {
    return true;
...
/* ===========================================================================
 * Entry point (called by reflow_parse.c)
 * ===========================================================================
 */
...
ra8_err_t priv_reflow_xml_walk(reflow_t* engine, const uint8_t* xhtml_buf, size_t xhtml_len)
{
  ...
  ctx->engine    = engine;
  ctx->style     = (uint8_t)k_reflow_style_normal;
  ctx->color     = (uint32_t)k_reflow_color_inherit;

  size_t i = 0U;
  while (i < xhtml_len) {
    if (xhtml_buf[i] == (uint8_t)'<') {
    ...
    const size_t run = i;
    while ((i < xhtml_len) && (xhtml_buf[i] != (uint8_t)'<')) {
      ++i;
    }
