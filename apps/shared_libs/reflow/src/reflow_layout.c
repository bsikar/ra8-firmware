/**
 * @file reflow_layout.c
 * @brief Greedy line-break + page-break engine for reflow.
 *
 * @details
 * Consumes the token stream produced by `reflow_parse_xhtml()` and
 ...
 *          @ref priv_reflow_internal_is_indent_tag.
 *
 * @param[in] tag Token tag value (raw @c uint8_t storage of
 *                @ref reflow_html_tag_t).
 *
 * @return Boolean indent-tag predicate.
 * @retval true  Tag is @c k_reflow_tag_li or @c k_reflow_tag_blockquote.
...
{
  return (tag == (uint8_t)k_reflow_tag_li) || (tag == (uint8_t)k_reflow_tag_blockquote);
}
 ...
 * @retval true Caller must call priv_reflow_layout_newline before emitting.
 ...
 * @details Promoted from line 953 in @c reflow_set_font_size.
 ...
 * @details Promoted from line 750 in @c reflow_run_layout.
...
/** @brief Implementation of `priv_reflow_layout_byte_zero()` -- bounded byte-walk. */
 ...
 * units and live in `reflow_layout_internal.h`.
 * ===========================================================================
 */

/** @brief Implementation of `priv_reflow_layout_init_font()` -- parse the TTF blob. */
...
/** @brief Implementation of `priv_reflow_layout_line_height()` -- scaled integer ratio. */
uint16_t priv_reflow_layout_line_height(uint16_t font_px)
{
  const uint32_t lh =
    ((uint32_t)font_px * (uint32_t)k_reflow_line_spacing_num) / (uint32_t)k_reflow_line_spacing_den;
  return (uint16_t)lh;
}
...
{
  uint32_t pct = k_reflow_pct_full;
  switch (tag) {
    case k_reflow_tag_h1:
      pct = k_reflow_h1_scale_pct;
      break;
    case k_reflow_tag_h2:
      pct = k_reflow_h2_scale_pct;
      break;
    case k_reflow_tag_h3:
      pct = k_reflow_h3_scale_pct;
      break;
    case k_reflow_tag_h4:
      pct = k_reflow_h4_scale_pct;
      break;
    case k_reflow_tag_h5:
      pct = k_reflow_h5_scale_pct;
      break;
    case k_reflow_tag_h6:
      pct = k_reflow_h6_scale_pct;
      break;
    default:
      pct = k_reflow_pct_full;
      break;
  }
  return (uint16_t)(((uint32_t)body_px * pct) / k_reflow_pct_full);
}

/** @brief Implementation of `priv_reflow_layout_glyph_advance()` -- scaled hmetrics. */
...
/** @brief Implementation of `priv_reflow_layout_push_glyph()` -- append + bounds check. */
bool priv_reflow_layout_push_glyph(reflow_t* engine,
                                   ...
                                   uint8_t   link_id)
{
  if (engine->glyph_count >= k_reflow_max_glyphs) {
    return false;
  }
  reflow_glyph_t* g = &engine->glyphs[engine->glyph_count];
  g->x              = x;
  g->y              = y;
  g->cp             = cp;
  g->color          = color;
...
/** @brief Implementation of `priv_reflow_layout_finish_page()` -- flush + reset cursor. */
bool priv_reflow_layout_finish_page(reflow_t* engine, priv_cursor_t* cur)
{
  ...
  reflow_page_t* page  = &engine->pages[engine->page_count];
  ...
  cur->y                = (int32_t)k_reflow_margin_px;
  cur->line_top         = cur->y;
  cur->x                = (int32_t)k_reflow_margin_px + (int32_t)cur->indent_px;
...
{
  if (cur->align == (uint8_t)k_reflow_align_left) {
    return;
  }
  ...
  if (cur->align == (uint8_t)k_reflow_align_justify) {
    if (allow_justify) {
      internal_justify_glyphs(engine, lo, hi, slack);
    }
    return;
  }
  const int32_t offset = (cur->align == (uint8_t)k_reflow_align_center) ? (slack / 2) : slack;
  for (uint32_t i = lo; i < hi; ++i) {
    engine->glyphs[i].x += offset;
  }
}

/** @brief Implementation of `priv_reflow_layout_newline()` -- align, advance, page-break. */
bool priv_reflow_layout_newline(reflow_t* engine, priv_cursor_t* cur, bool allow_justify)
{
  ...
  const int32_t bottom_limit = (int32_t)engine->viewport_h - (int32_t)k_reflow_margin_px;
 ...
 *
 * @details Measures the advance width of @p cp via @ref priv_reflow_layout_glyph_advance,
 * clamps it to at least `k_priv_min_word_w_px` to prevent zero-width stalls,
 * and checks the right-overflow predicate
 * (@ref priv_reflow_internal_right_overflow_break). If a line break is needed
 * and the line already has content, @ref priv_reflow_layout_newline is called before the
 * glyph is pushed. The glyph is then appended via @ref priv_reflow_layout_push_glyph and
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_emit_char(reflow_t*             engine,
                                    priv_cursor_t*        cur,
                                    ...
                                    uint8_t               link_id)
{
  const int32_t advance = priv_reflow_layout_glyph_advance(font, cur->active_font_px, cp);
  ...
  if (priv_reflow_internal_right_overflow_break(cur->x,
    ...
    if (!priv_reflow_layout_newline(engine, cur, true)) { /* wrap -> previous line may justify */
      return k_ra8_err_no_mem;
    }
  }
  if (!priv_reflow_layout_push_glyph(engine,
                                     cur->x,
                                     cur->y,
                                     cp,
                                     cur->active_font_px,
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_layout_text(reflow_t*             engine,
                                      priv_cursor_t*        cur,
                                      const stbtt_fontinfo* font,
...
static bool internal_open_block(reflow_t* engine, priv_cursor_t* cur, const reflow_token_t* tok)
{
  if (cur->line_has_content != 0U) {
    if (!priv_reflow_layout_newline(engine, cur, false)) {
      return false;
    }
  }
  ...
  if ((tok->text_len > 0U) && (engine->anchor_count < (uint32_t)k_reflow_max_anchors)) {
    reflow_anchor_t* anchor = &engine->anchors[engine->anchor_count];
  ...
  cur->active_font_px = (tok->css_font_px != 0U)
                          ? tok->css_font_px
                          : internal_block_font_px(engine->font_px, (reflow_html_tag_t)tok->tag);
  cur->line_height_px = priv_reflow_layout_line_height(cur->active_font_px);
  if (priv_reflow_internal_is_indent_tag((uint8_t)tok->tag)) {
    cur->indent_px = k_reflow_indent_px;
    cur->x         = (int32_t)k_reflow_margin_px + (int32_t)cur->indent_px;
  }
  return true;
}
...
static bool internal_close_block(reflow_t* engine, priv_cursor_t* cur, const reflow_token_t* tok)
{
  ...
  /* Paragraph gap. */
  cur->y += (int32_t)k_reflow_paragraph_gap_px;
  cur->line_top = cur->y;
  cur->x        = (int32_t)k_reflow_margin_px;
  if (priv_reflow_internal_is_indent_tag((uint8_t)tok->tag)) {
    cur->indent_px = 0U;
  }
 ...
 * (measured by `priv_reflow_layout_glyph_advance`), with a generous vertical band
 ...
 */
RA8_INTERNAL
static void internal_emit_link_rect(reflow_t*             engine,
                                    const stbtt_fontinfo* font,
                                    uint32_t              lo,
                                    uint32_t              hi,
                                    uint8_t               link,
                                    uint32_t              page)
{
  if (engine->link_rect_count >= (uint32_t)k_reflow_max_link_rects) {
    return;
  }
  const reflow_glyph_t* gfirst = &engine->glyphs[lo];
  const reflow_glyph_t* glast  = &engine->glyphs[hi - 1U];
  const int32_t         fpx    = (int32_t)gfirst->font_px;
  const int32_t x1 = glast->x + priv_reflow_layout_glyph_advance(font, glast->font_px, glast->cp);
  reflow_link_rect_t* rect = &engine->link_rects[engine->link_rect_count];
  rect->x                  = gfirst->x;
  rect->y                  = gfirst->y - fpx;
...
/** @brief Implementation of `priv_reflow_layout_build_link_rects()` -- per-page link runs. */
void priv_reflow_layout_build_link_rects(reflow_t* engine, const stbtt_fontinfo* font)
{
  for (uint32_t p = 0U; p < engine->page_count; ++p) {
    const reflow_page_t* page = &engine->pages[p];
...
/** @brief Implementation of `priv_reflow_layout_apply_token()` -- per-token switch. */
                                         ...
                                         const reflow_token_t* tok)
{
  ...
  cur->active_style =
    (uint8_t)(tok->style | (uint8_t)(((uint32_t)tok->reserved16 & (uint32_t)k_reflow_face_mask)
                                     << (uint32_t)k_reflow_face_shift));
  switch (tok->kind) {
    case k_reflow_tok_block_start:
      return internal_open_block(engine, cur, tok) ? k_ra8_ok : k_ra8_err_no_mem;
    case k_reflow_tok_block_end:
      return internal_close_block(engine, cur, tok) ? k_ra8_ok : k_ra8_err_no_mem;
    case k_reflow_tok_text:
      return internal_layout_text(engine, cur, font, tok);
    case k_reflow_tok_break:
      return priv_reflow_layout_newline(engine, cur, false) ? k_ra8_ok : k_ra8_err_no_mem;
    case k_reflow_tok_rule:
      return internal_apply_rule(engine, cur) ? k_ra8_ok : k_ra8_err_no_mem;
    case k_reflow_tok_image:
      return priv_reflow_layout_apply_image(engine, cur, tok) ? k_ra8_ok : k_ra8_err_no_mem;
    default:
      return k_ra8_ok;
  }
}
