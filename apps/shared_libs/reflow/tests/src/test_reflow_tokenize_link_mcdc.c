/**
 * @file test_reflow_tokenize_link_mcdc.c
 * @brief MC/DC tests for the tokenizer's link / stylesheet / font-face arms.
 *
 * @details
 * Split sibling of test_reflow_tokenize_scan_mcdc.c and
 * test_reflow_tokenize_tag_mcdc.c covering the link and font decision
 * families of apps/shared_libs/reflow/src/reflow_tokenize.c and
 * reflow_tokenize_attr.c: link-target interning (empty href, table
...
#include "ra8_err.h"
#include "reflow_tokenize_test_util.h"
#include "unity_minimal.h"

/** @brief Constants for the link-table-full flood document. */
typedef enum : uint32_t {
  k_max_links_count = 255U,   /**< k_reflow_max_links (link table capacity).     */
...
/**
 * @brief Write an unsigned integer as decimal ASCII without a terminator.
 * @param[out] dst Receives the decimal digits.
 * @param[in] value Unsigned value to encode.
 * @return Number of bytes written to @p dst.
 * @pre @p dst addresses space for the ten digits required by a uint32_t.
 * @post Exactly the returned number of decimal digits occupy @p dst.
 * @since 0.1.0
 */
RA8_INTERNAL static size_t internal_u32_text(char* dst, uint32_t value)
{
 ...
 * `<a href="hN">t</a>` spans, one final overflow anchor, and the closing tags.
 * Every href is unique so each link consumes a distinct slot.
 * @param[out] dst Destination buffer; receives the NUL-terminated document.
 * @param[in] cap Capacity of @p dst in bytes; at least k_flood_doc_cap.
 * @pre @p dst is non-null and addresses at least @p cap bytes.
 * @post @p dst holds a NUL-terminated document with k_max_links_count + 1 anchors.
...
RA8_INTERNAL static void internal_build_link_flood_doc(char* dst, size_t cap)
{
  ...
  (void)memcpy(&dst[pos], k_head, k_hl);
  pos += (uint32_t)k_hl;
  for (uint32_t n = 0U; n < k_max_links_count; ++n) {
    static const char k_prefix[]          = "<a href=\"h";
    static const char k_suffix[]          = "\">t</a>";
    char              tmp[k_link_tmp_cap] = {};
    size_t            tl                  = (sizeof k_prefix) - 1U;
    (void)memcpy(tmp, k_prefix, tl);
    tl += internal_u32_text(&tmp[tl], n);
    (void)memcpy(&tmp[tl], k_suffix, sizeof k_suffix);
    tl += (sizeof k_suffix) - 1U;
    if (pos + tl + (uint32_t)k_link_slack < cap) {
      (void)memcpy(&dst[pos], tmp, tl);
      pos += (uint32_t)tl;
    }
  }
  const char* const k_extra = "<a href=\"extra\">overflow</a>";
  const size_t      k_el    = strlen(k_extra);
  (void)memcpy(&dst[pos], k_extra, k_el);
  pos += (uint32_t)k_el;
  const char* const k_tail = "</body></html>";
  const size_t      k_tl   = strlen(k_tail);
  (void)memcpy(&dst[pos], k_tail, k_tl);
  pos += (uint32_t)k_tl;
  dst[pos] = '\0';
}
 ...
 * guard in priv_intern_link (reflow_tokenize.c).
 ...
 * @par MC/DC:
 * Decision owner:
 * apps/shared_libs/reflow/src/reflow_tokenize_lex.c@internal_bytes_equal
 * Decision: the shared byte-match loop `(k < len) && equal`, reached by
 * priv_rel_is_stylesheet (reflow_tokenize.c). The outer guard in
 ...
 *  - V2 rel="Xtylesheet altname" -> the compare arm goes false at the first
 *    byte while the length arm stays true -> no full match -> no load.
 * V1 supplies `(T,T)` while bytes match and `(F,-)` at the exact-length exit;
 * V2 supplies `(T,F)` after its first mismatch. These are the N+1 executions
 * proving both conditions independently control the loop. A baseline with no
 * loader gives the default colour.
...
RA8_INTERNAL static void internal_test_rel_is_stylesheet_scan(void)
{
  ...
  /* V2 rel is an early near-miss -> equal false while the bound remains true. */
  TEST_ASSERT_EQ(k_ra8_ok,
                 walk("<html><head><link rel=\"Xtylesheet\" href=\"s\"/></head>"
                      "<body><p class=\"lead\">x</p></body></html>"));
 ...
 * (reflow_tokenize.c) before parsing the fetched CSS.
 ...
 * -- the early return in priv_resolve_face_slot (reflow_tokenize.c). This is
...
RA8_INTERNAL static void internal_test_resolve_face_slot_conditions(void)
{
  ...
  uint32_t pages = 0U;
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_init((uint16_t)k_viewport_w,
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    reflow_layout_chapter(&s_engine, (const uint8_t*)doc, (uint32_t)strlen(doc), &pages));
  TEST_ASSERT(pages >= 1U);

  TEST_ASSERT_EQ(k_ra8_ok, reflow_close(&s_engine));
 ...
 * `link_target_count >= k_reflow_max_links (255)`.
...
RA8_INTERNAL static void internal_test_intern_link_table_full(void)
{
  ...
  if (err == k_ra8_ok) {
    ...
    for (uint32_t i = 0U; i < s_engine.token_count; ++i) {
      ...
      if ((tl >= 8U) && (strncmp(t, "overflow", 8U) == 0)) {
 ...
 *
 * @note MC/DC deactivation: the `comp->family_len == 0U` arm at L1412 is not
 * independently reachable because reflow_css.c's internal_family_cb only sets
...
RA8_INTERNAL static void internal_test_resolve_face_slot_family_len_zero(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    reflow_layout_chapter(&s_engine, (const uint8_t*)k_doc, (uint32_t)strlen(k_doc), &pages));
  TEST_ASSERT(pages >= 1U);
 ...
 * (comp->family_len == 0U))` in priv_reflow_tok_resolve_face_slot
 * (apps/shared_libs/reflow/src/reflow_tokenize_attr.c, 3 conditions, OR). Existing
...
{
  TEST_BEGIN("reflow_tok_resolve_face_slot MC/DC: family_len==0 direct arm");
  uint32_t pages = 0U;
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_init((uint16_t)k_viewport_w,
                             (uint16_t)k_viewport_h,
  ...
  comp.family_len      = 0U;
  TEST_ASSERT_EQ(0, priv_reflow_tok_resolve_face_slot(&s_engine, &comp));
  (void)pages;
  TEST_ASSERT_EQ(k_ra8_ok, reflow_close(&s_engine));
  TEST_END("reflow_tok_resolve_face_slot MC/DC: family_len==0 direct arm");
}
