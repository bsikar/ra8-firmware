/**
 * @file test_reflow.c
 * @brief Unit tests for apps/shared_libs/reflow (HTML+CSS reflow + paginate).
 *
 * @details
...
/**
 * @brief NULL / bad-arg guards on reflow_init() and reflow_close().
 *
 * @par MC/DC:
...
RA8_INTERNAL static void internal_test_null_and_preinit_guards_init(void)
{
  ...
  /* close() guards. */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, reflow_close(nullptr));

  /* Engine zeroed -> not initialized. */
  reflow_t closed;
  (void)memset(&closed, 0, sizeof(closed));
  TEST_ASSERT_EQ(k_ra8_err_not_initialized, reflow_close(&closed));
...
RA8_INTERNAL static void internal_test_null_and_preinit_guards_ops(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra8_err_null_ptr,
    reflow_layout_chapter(nullptr, (const uint8_t*)s_xhtml_simple, strlen(s_xhtml_simple), &pages));
  TEST_ASSERT_EQ(
    k_ra8_err_not_initialized,
    reflow_layout_chapter(&closed, (const uint8_t*)s_xhtml_simple, strlen(s_xhtml_simple), &pages));

  /* render_page() / get_page_count() / set_font_size() guards. */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, reflow_render_page(nullptr, 0U, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_not_initialized, reflow_render_page(&closed, 0U, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, reflow_get_page_count(nullptr, &pages));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, reflow_set_font_size(nullptr, k_test_font_px));
  TEST_ASSERT_EQ(k_ra8_err_not_initialized, reflow_set_font_size(&closed, k_test_font_px));
...
RA8_INTERNAL static void internal_test_simple_layout(void)
{
  ...
  uint32_t  pages = 0U;
  ra8_err_t err   = reflow_layout_chapter(&s_engine,
  ...
  TEST_ASSERT_EQ(k_ra8_ok, reflow_get_page_count(&s_engine, &page_count));
  TEST_ASSERT_EQ(pages, page_count);

  /* render_page() with an out-of-range index -> out_of_range. */
  TEST_ASSERT_EQ(k_ra8_err_out_of_range, reflow_render_page(&s_engine, page_count, nullptr));
...
RA8_INTERNAL static void internal_test_render_pixels_around_title(void)
{
  ...
  uint32_t pages = 0U;
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_layout_chapter(&s_engine,
                                       (const uint8_t*)s_xhtml_simple,
                                       strlen(s_xhtml_simple),
                                       &pages));
  TEST_ASSERT(s_engine.glyph_count > 0U);
...
/**
 * @brief `reflow_render_page_at` shifts the whole page by the origin;
 *        `reflow_render_page` equals it at origin (0,0).
 *
 * @par MC/DC:
 * (no compound decisions in this test -- exercises the public-API
 * happy path / error-rejection contract; no `&&` or `||` in the
...
RA8_INTERNAL static void internal_test_render_at_origin(void)
{
  ...
  internal_priv_bind_gfx();
  TEST_ASSERT_EQ(k_ra8_ok, reflow_render_page_at(&s_engine, 0U, 0, 0));
   ...
   * across offsets). */
  internal_priv_bind_gfx();
  TEST_ASSERT_EQ(k_ra8_ok, reflow_render_page_at(&s_engine, 0U, dx, dy));
...
RA8_INTERNAL static void internal_test_multi_paragraph(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    reflow_layout_chapter(&s_engine, (const uint8_t*)s_xhtml_multi, strlen(s_xhtml_multi), &pages));
  TEST_ASSERT(pages >= 1U);
  TEST_ASSERT(s_engine.glyph_count > 0U);
  ...
  for (uint32_t i = 0U; i < s_engine.token_count; ++i) {
    if ((s_engine.tokens[i].kind == (uint8_t)k_reflow_tok_block_start) &&
        (s_engine.tokens[i].tag == (uint8_t)k_reflow_tag_p)) {
      ++block_starts;
    }
  }
...
RA8_INTERNAL static void internal_test_bold_italic_toggling(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_layout_chapter(&s_engine,
                                       (const uint8_t*)test_xhtml_styled(),
                                       strlen(test_xhtml_styled()),
                                       &pages));
  TEST_ASSERT(s_engine.glyph_count > 0U);
  ...
  for (uint32_t i = 0U; i < s_engine.glyph_count; ++i) {
    ...
    if ((style & (uint8_t)k_reflow_style_bold) != 0U) {
      saw_bold = true;
    }
    if ((style & (uint8_t)k_reflow_style_italic) != 0U) {
      saw_italic = true;
    }
    if (style == (uint8_t)k_reflow_style_normal) {
      saw_plain = true;
    }
  }
...
RA8_INTERNAL static void internal_test_set_font_size_reflows(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, reflow_set_font_size(&s_engine, (uint16_t)k_test_font_px_large));
  uint32_t pages_large = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, reflow_get_page_count(&s_engine, &pages_large));
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, reflow_set_font_size(&s_engine, 1U));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 reflow_set_font_size(&s_engine, (uint16_t)k_reflow_max_font_px + 1U));
...
RA8_INTERNAL static void internal_test_set_font_size_without_layout(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_init(k_test_viewport_w,
                             ...
                             k_test_color_link,
                             &s_engine));
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, reflow_set_font_size(&s_engine, k_test_font_px_large));
  TEST_ASSERT_EQ(k_ra8_ok, reflow_close(&s_engine));
...
RA8_INTERNAL static void internal_test_layout_rejects_empty_input(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 reflow_layout_chapter(&s_engine, (const uint8_t*)s_xhtml_simple, 0U, &pages));
  /* Malformed XHTML rejected as validation_failed. */
  static const char* const k_bad = "<<<not xml";
  TEST_ASSERT_EQ(k_ra8_err_validation_failed,
                 reflow_layout_chapter(&s_engine, (const uint8_t*)k_bad, strlen(k_bad), &pages));
  TEST_ASSERT_EQ(k_ra8_ok, reflow_close(&s_engine));
...
int main(void)
{
  if (!internal_priv_load_font()) {
    ...
    (void)internal_test_output_fd_text(STDERR_FILENO,
                                       "[SKIP] test_reflow: Literata-Regular.ttf not loadable -- "
  ...
  (void)internal_test_output_fd_text(STDERR_FILENO,
                                     "[OK  ] test_reflow: layout/render/guard tests passed\n");
  return 0;
}
