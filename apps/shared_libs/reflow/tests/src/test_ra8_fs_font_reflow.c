 ...
 *        rendered through the heap-free reflow pipeline.
 *
 * @details
 ...
 * to reflow, which lays out and rasterises a paragraph into an
...
/* ---------------------------------------------------------------------------
 * In-memory FAT16 block device (mirrors tests/storage/src/test_ra8_fs_fat.c).
 * ---------------------------------------------------------------------------
 */
...
static uint8_t    s_disk_bytes[(size_t)k_disk_blocks_fat16 * (size_t)k_disk_block_size];
static mem_disk_t s_disk = {.bytes       = s_disk_bytes,
                            .block_count = (uint32_t)k_disk_blocks_fat16,
                            .byte_count  = sizeof s_disk_bytes};
...
internal_mem_read(void* ctx, uint64_t lba, uint32_t count, uint8_t* buf)
{
  ...
  if ((lba + (uint64_t)count) > (uint64_t)d->block_count) {
...
RA8_INTERNAL static void internal_put16(uint8_t* p, uint32_t off, uint16_t v)
{
  ...
  p[off + 1U] = (uint8_t)(((uint32_t)v >> 8U) & (uint32_t)k_byte_mask);
}
...
/* ---------------------------------------------------------------------------
 * Font loading (bundled Literata, from the configured repository root).
 * ---------------------------------------------------------------------------
 */
enum : uint32_t {
...
static uint8_t  s_src_font[k_font_cap];  /**< Font as read from disk.        */
static uint8_t  s_card_font[k_font_cap]; /**< Font as read back from the FS. */
...
RA8_INTERNAL static void internal_fat_reflow_write_read_font(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_mount(&backend, &mnt));
  ...
  /* 3. Read it back off the card into the font buffer reflow will use. */
...
{
  static uint32_t s_fb[(size_t)k_fb_w * (size_t)k_fb_h];
  ...
  uint32_t    pages = 0U;
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_layout_chapter(&engine, (const uint8_t*)xhtml, strlen(xhtml), &pages));
  TEST_ASSERT(pages >= 1U);
  TEST_ASSERT_EQ(k_ra8_ok, reflow_render_page(&engine, 0U, nullptr));
 ...
 * already-MC/DC-covered ra8_fs / reflow APIs through one round trip:
...
{
  TEST_BEGIN("font on FAT volume -> ra8_fs -> reflow render");
  ...
  if (!internal_load_src_font()) {
    ...
    TEST_END("font on FAT volume -> ra8_fs -> reflow render");
    return;
  }
