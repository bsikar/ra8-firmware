/**
 * @file test_reflow_layout_flow_mcdc.c
 * @brief MC/DC tests for the layout engine's line / anchor / page decisions.
 *
 * @details
 * Split sibling of test_reflow_layout_content_mcdc.c covering the text-flow
 * decision families of apps/shared_libs/reflow/src/reflow_layout.c: the
 ...
 * apps/shared_libs/reflow/src/reflow_layout.c@internal_finish_line). The early-out fires
...
RA8_INTERNAL static void internal_test_finish_line_no_slack_mcdc(void)
{
  ...
  TEST_ASSERT(s_eng.glyphs[0].x > (int32_t)k_reflow_margin_px);
 ...
 * (2 conditions, AND; apps/shared_libs/reflow/src/reflow_layout.c@internal_open_block --
 ...
 *        (one anchor recorded; resolvable via reflow_find_anchor).
 *  - V2: plain `<p>` (no id) -> C1 F shorts -> F (no anchor recorded).
 *  - V3: a block with an id but the anchor pool already at capacity -> C1 T,
 *        C2 F -> F. The pool cap is k_reflow_max_anchors; we approximate the
...
RA8_INTERNAL static void internal_test_open_block_anchor_mcdc(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, reflow_find_anchor(&s_eng, "top", 3U, &page));
   ...
   * exceeds k_reflow_max_anchors (the C2-false arm holds the count). */
  init_engine(k_vp_w, k_vp_h);
  (void)lay("<html><body>"
            "<p id=\"a\">x</p><p id=\"b\">y</p><p id=\"c\">z</p>"
            "</body></html>");
  TEST_ASSERT(s_eng.anchor_count >= 3U);
  TEST_ASSERT(s_eng.anchor_count <= (uint32_t)k_reflow_max_anchors);
 ...
 * apps/shared_libs/reflow/src/reflow_layout.c@priv_reflow_layout_build_link_rects:
 ...
 * apps/shared_libs/reflow/src/reflow_layout_driver.c@internal_layout_tokens:
...
RA8_INTERNAL static void internal_test_layout_tokens_final_flush_mcdc(void)
{
  ...
  const uint32_t last_page = s_eng.page_count - 1U;
  TEST_ASSERT_EQ(last_page, s_eng.image_boxes[last_box].page_index);
 ...
 * (2 conditions, OR; apps/shared_libs/reflow/src/reflow_layout_driver.c@reflow_register_face --
...
{
  TEST_BEGIN("reflow_register_face MC/DC: offset<0 || InitFont==0");
  /* V1: valid Ahem face registers. */
  init_engine(k_vp_w, k_vp_h);
  TEST_ASSERT_EQ(0, s_eng.face_count);
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_register_face(&s_eng, 0U, k_fixture_ahem, (size_t)k_fixture_ahem_len));
  ...
  TEST_ASSERT_EQ(k_ra8_err_not_supported,
                 reflow_register_face(&s_eng, 1U, k_junk_face_a, sizeof k_junk_face_a));
  TEST_ASSERT_EQ(k_ra8_err_not_supported,
                 reflow_register_face(&s_eng, 1U, k_junk_face_b, sizeof k_junk_face_b));
  TEST_ASSERT_EQ(1, s_eng.face_count); /* unchanged by the two failures */
  TEST_END("reflow_register_face MC/DC: offset<0 || InitFont==0");
}
 ...
 *
 * @par MC/DC:
 * Decision at reflow_layout.c L500:
 * `if ((hi <= lo) || (slack <= 0))`  (2 conditions, OR;
 * apps/shared_libs/reflow/src/reflow_layout.c@internal_finish_line).
 *
...
RA8_INTERNAL static void internal_test_finish_line_center_slack_zero_mcdc(void)
{
   ...
   * Glyphs shift right of the left margin (x > k_reflow_margin_px). */
  init_engine(k_vp_w, k_vp_h);
...
RA8_INTERNAL static size_t internal_build_anchor_html(char* buf, size_t buf_cap, uint32_t n)
{
  ...
  const size_t header_len  = (sizeof k_hdr) - 1U;
  const size_t footer_len  = (sizeof k_ftr) - 1U;
  const size_t entries_len = (size_t)n * (size_t)k_entry_max_len;
  const size_t needed      = ((header_len + entries_len) + footer_len) + 1U;
  if (needed > buf_cap) {
    return 0U;
  }
  size_t pos = 0U;
...
/** @brief Scratch buffer large enough for k_reflow_max_anchors+1 entries. */
/* 1 extra entry beyond k_reflow_max_anchors to drive the C2-false arm.
 * Worst case entry is 21 chars; header/footer add ~30. */
enum : uint32_t {
  k_anchor_buf_entries = 260U, /**< Entries to build (> k_reflow_max_anchors). */
};
enum : uint32_t {
 ...
 *
 * @par MC/DC:
 * Decision at reflow_layout.c L708:
 ...
 * (anchor_count == k_reflow_max_anchors).
 *
 * Vectors (N+1 = 3 for N=2, only V3 added here; V1/V2 in existing test):
 *  - V3: lay out k_reflow_max_anchors+1 elements each with a unique `id`
 *        attribute.  After the first k_reflow_max_anchors are captured
 ...
 *        anchor_count stays at exactly k_reflow_max_anchors.
...
{
  /* Static scratch buffer for the anchor-pool-full document. */
   ...
   * k_reflow_max_anchors fill the pool (C1=T, C2=T path); each
  ...
  TEST_ASSERT_EQ(k_reflow_max_anchors, s_eng.anchor_count);
 ...
 *
 * @par MC/DC:
 * Decision at reflow_layout.c L849:
 * `return (engine->glyph_count > cur->page_first_glyph) ||
 *         (engine->image_box_count > cur->page_first_image)`
 * (2 conditions, OR; apps/shared_libs/reflow/src/reflow_layout_image.c@internal_page_has_content,
