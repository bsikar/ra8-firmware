 ...
 * CMake-provided repository root; each test SKIPs if the font is absent) to
 * execute the missing lines:
 *
 ...
 * `cover.png` href) return the raster -- driving both SVG sub-routes. The
 * function-local wrapper is `<svg><image href="cover.png"/></svg>`.
 *
 * @param[in]  ctx      Unused.
 ...
 * @details The function-local SVG contains a filled rectangle rendered through
 * ra8_svg_render without a nested image.
 * @param[in]  ctx      Unused.
 * @param[in]  href     Unused.
 * @param[in]  href_len Unused.
...
{
  return reflow_init((uint16_t)k_rc_vp_w,
                     (uint16_t)k_rc_vp_h,
                     s_font,
                     s_font_len,
 ...
 *
 * @details Targets reflow_render.c lines 89-93 (internal_init_font's offset<0 /
...
{
  /* Invalid font bytes that still satisfy the 16-byte initialization guard. */
  static uint8_t s_junk_font[64];
  s_junk_font[0] = 1U;
  s_junk_font[1] = 2U;
  s_junk_font[2] = 3U;
  s_junk_font[3] = 4U;
  ...
  TEST_ASSERT_EQ(k_ra8_err_validation_failed, reflow_render_page(&s_engine, 0U, nullptr));
  TEST_END("render cov: junk default font -> validation_failed");
}
 ...
 *
 * @details Targets reflow_render.c line 219-220 (internal_blit_glyph's underline
 * gate) and 156-170 (internal_draw_underline). The tokenizer stamps every `<a>`
 * glyph with k_reflow_style_underline, so rendering the page draws the
 * underline beneath the link glyphs.
 *
 * @par MC/DC:
 * The underline gate `if ((g->style & k_reflow_style_underline) != 0U)` and
 ...
 *
 * @details Targets reflow_render.c lines 305-316 (internal_render_images loop),
 ...
 * in internal_render_images (apps/shared_libs/reflow/src/reflow_render.c), evaluated here
...
RA8_INTERNAL static void internal_test_cov_render_raster_image(void)
{
  ...
  internal_bind_clear_fb();
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_render_page(&s_engine, s_engine.image_boxes[0].page_index, nullptr));
  TEST_ASSERT(internal_fb_has_drawn_pixel());
  TEST_END("render cov: raster image decode + blit");
}
 ...
 *
 * @details Targets reflow_render.c lines 257-281 (internal_render_svg: the
...
RA8_INTERNAL static void internal_test_cov_render_svg_image(void)
{
  ...
  ra8_img_arena_t arena = {.base   = s_arena_buf,
                           ...
                           .live   = 0U};
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_set_image_loader(&s_engine, internal_svg_loader, nullptr, &arena));
  uint32_t    pages = 0U;
  const char* doc_a = "<p>Cover:</p><img src=\"cover.svg\"/><p>End.</p>";
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_layout_chapter(&s_engine, (const uint8_t*)doc_a, strlen(doc_a), &pages));
  TEST_ASSERT(s_engine.image_box_count >= 1U);
  internal_bind_clear_fb();
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_render_page(&s_engine, s_engine.image_boxes[0].page_index, nullptr));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, internal_init_engine());
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_set_image_loader(&s_engine, internal_svg_shapes_loader, nullptr, &arena));
  const char* doc_b = "<p>Shapes:</p><img src=\"art.svg\"/><p>Done.</p>";
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_layout_chapter(&s_engine, (const uint8_t*)doc_b, strlen(doc_b), &pages));
  TEST_ASSERT(s_engine.image_box_count >= 1U);
  internal_bind_clear_fb();
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_render_page(&s_engine, s_engine.image_boxes[0].page_index, nullptr));
 ...
 *
 * @details Targets reflow_render.c lines 402-403 (the `fi >= nfaces ->
 ...
 * the page still rasterises. Driven via reflow_render_page_at() to also
...
RA8_INTERNAL static void internal_test_cov_render_face_index_clamp(void)
{
  ...
  internal_bind_clear_fb();
  TEST_ASSERT_EQ(k_ra8_ok, reflow_render_page_at(&s_engine, 0, 0, 0));
  TEST_ASSERT(internal_fb_has_drawn_pixel());
  TEST_END("render cov: out-of-range face index clamp");
}
...
int main(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_test_output_ok,
                 internal_test_output_fd_text(STDERR_FILENO, "[OK ] test_reflow_render_cov.c\n"));
  return 0;
}
