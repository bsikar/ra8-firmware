 ...
 * (so `reflow_layout_chapter()` can be skipped), and that every
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_init_engine(reflow_t* eng, uint16_t font_px)
{
  return reflow_init((uint16_t)k_t_vp_w,
                     (uint16_t)k_t_vp_h,
                     s_font,
                     (size_t)k_t_font_len,
                     font_px,
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_build_layout(reflow_t* eng)
{
  eng->glyph_count = (uint32_t)k_t_glyphs;
  for (uint32_t i = 0U; i < (uint32_t)k_t_glyphs; ++i) {
    const uint32_t x        = i * (uint32_t)k_t_x_step;
    const uint32_t y        = i * (uint32_t)k_t_y_step;
    eng->glyphs[i].x        = (int32_t)x - (int32_t)k_t_x_bias;
    eng->glyphs[i].y        = (int32_t)y;
...
RA8_INTERNAL static size_t internal_serialize_ok(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    reflow_cache_serialize(&s_engine, s_content, s_content_len, s_blob, (size_t)k_t_blob_cap, &n));
  return n;
}

/**
 * @test internal_test_size_matches_serialize
 * @brief `reflow_cache_size()` equals the serialised byte count and
...
RA8_INTERNAL static void internal_test_size_matches_serialize(void)
{
  ...
  size_t want = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, reflow_cache_size(&s_engine, &want));
  const size_t formula = (size_t)k_reflow_cache_header_bytes +
                         ((size_t)k_t_glyphs * (size_t)k_reflow_cache_glyph_bytes) +
                         ((size_t)k_t_pages * (size_t)k_reflow_cache_page_bytes);
...
{
  static reflow_glyph_t s_glyph_save[k_t_glyphs];
  static reflow_page_t  s_page_save[k_t_pages];
  TEST_BEGIN("reflow_cache: round-trip restores layout");
  TEST_ASSERT_EQ(k_ra8_ok, internal_init_engine(&s_engine, (uint16_t)k_t_font_px));
  internal_build_layout(&s_engine);
  (void)memcpy(s_glyph_save, s_engine.glyphs, sizeof(s_glyph_save));
  (void)memcpy(s_page_save, s_engine.pages, sizeof(s_page_save));
  const size_t n = internal_serialize_ok();

  /* Wipe the laid-out state so a successful load must rebuild it. */
  (void)memset(s_engine.glyphs, 0, sizeof(s_glyph_save));
  (void)memset(s_engine.pages, 0, sizeof(s_page_save));
  s_engine.glyph_count = 0U;
  s_engine.page_count  = 0U;

  TEST_ASSERT_EQ(k_ra8_ok, reflow_cache_load(&s_engine, s_content, s_content_len, s_blob, n));
...
RA8_INTERNAL static void internal_test_empty_layout_roundtrip(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, internal_init_engine(&s_engine, (uint16_t)k_t_font_px));
  const size_t n = internal_serialize_ok();
  TEST_ASSERT_EQ(k_reflow_cache_header_bytes, n);
  TEST_ASSERT_EQ(k_ra8_ok, reflow_cache_load(&s_engine, s_content, s_content_len, s_blob, n));
...
RA8_INTERNAL static void internal_test_stale_on_font_change(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_state,
                 reflow_cache_load(&s_engine, s_content, s_content_len, s_blob, n));
...
RA8_INTERNAL static void internal_test_stale_on_content_change(void)
{
  ...
  const size_t n = internal_serialize_ok();
  TEST_ASSERT_EQ(k_ra8_err_invalid_state,
                 reflow_cache_load(&s_engine, other, sizeof(other), s_blob, n));
...
RA8_INTERNAL static void internal_test_corrupt_body_checksum(void)
{
  ...
  internal_build_layout(&s_engine);
  const size_t n = internal_serialize_ok();
  s_blob[(size_t)k_reflow_cache_header_bytes + (size_t)k_t_glyphs] ^= (uint8_t)k_t_flip;
...
RA8_INTERNAL static void internal_test_truncated_blob(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 reflow_cache_load(&s_engine,
                                   s_content,
                                   s_content_len,
                                   s_blob,
                                   (size_t)k_reflow_cache_header_bytes - 1U));
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 reflow_cache_load(&s_engine, s_content, s_content_len, s_blob, n - 1U));
...
RA8_INTERNAL static void internal_test_cap_too_small(void)
{
  ...
  internal_build_layout(&s_engine);
  size_t need = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, reflow_cache_size(&s_engine, &need));
  size_t n = 0U;
  TEST_ASSERT_EQ(
    k_ra8_err_invalid_size,
    reflow_cache_serialize(&s_engine, s_content, s_content_len, s_blob, need - 1U, &n));
 ...
 *
 * @par MC/DC:
 * Decision in apps/shared_libs/reflow/src/reflow_cache.c@reflow_cache_size:
...
{
  TEST_BEGIN("reflow_cache: reflow_cache_size MC/DC");
  TEST_ASSERT_EQ(k_ra8_ok, internal_init_engine(&s_engine, (uint16_t)k_t_font_px));
  internal_build_layout(&s_engine);
  size_t sz = 0U;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, reflow_cache_size(nullptr, &sz));       /* C1=T      */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, reflow_cache_size(&s_engine, nullptr)); /* C2=T      */
  TEST_ASSERT_EQ(k_ra8_ok, reflow_cache_size(&s_engine, &sz));               /* C1=F,C2=F */
  /* in_use==0 simple guard (not compound). */
  reflow_t cold = {};
  TEST_ASSERT_EQ(k_ra8_err_not_initialized, reflow_cache_size(&cold, &sz));
  TEST_END("reflow_cache: reflow_cache_size MC/DC");
}
 ...
 *
 * @par MC/DC:
 * Decisions in apps/shared_libs/reflow/src/reflow_cache.c@reflow_cache_serialize:
...
{
  TEST_BEGIN("reflow_cache: reflow_cache_serialize MC/DC");
  TEST_ASSERT_EQ(k_ra8_ok, internal_init_engine(&s_engine, (uint16_t)k_t_font_px));
  internal_build_layout(&s_engine);
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 reflow_cache_serialize(nullptr, s_content, s_content_len, s_blob, cap, &n));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 reflow_cache_serialize(&s_engine, s_content, s_content_len, nullptr, cap, &n));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 reflow_cache_serialize(&s_engine, s_content, s_content_len, s_blob, cap, nullptr));
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_cache_serialize(&s_engine, s_content, s_content_len, s_blob, cap, &n));
  /* D2 (A=F is the D1 baseline above). */
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 reflow_cache_serialize(&s_engine, nullptr, s_content_len, s_blob, cap, &n));
  TEST_ASSERT_EQ(k_ra8_ok, reflow_cache_serialize(&s_engine, nullptr, 0U, s_blob, cap, &n));
  TEST_END("reflow_cache: reflow_cache_serialize MC/DC");
}
 ...
 *
 * @par MC/DC:
 * Decisions in apps/shared_libs/reflow/src/reflow_cache.c@reflow_cache_load:
...
{
  TEST_BEGIN("reflow_cache: reflow_cache_load MC/DC");
  TEST_ASSERT_EQ(k_ra8_ok, internal_init_engine(&s_engine, (uint16_t)k_t_font_px));
  internal_build_layout(&s_engine);
  const size_t n = internal_serialize_ok();
  /* D1 */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 reflow_cache_load(nullptr, s_content, s_content_len, s_blob, n));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 reflow_cache_load(&s_engine, s_content, s_content_len, nullptr, n));
  TEST_ASSERT_EQ(k_ra8_ok, reflow_cache_load(&s_engine, s_content, s_content_len, s_blob, n));
  /* D2 (A=F is the D1 baseline above). */
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 reflow_cache_load(&s_engine, nullptr, s_content_len, s_blob, n));
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, reflow_cache_load(&s_engine, nullptr, 0U, s_blob, n));
  /* in_use==0 simple guard (not compound). */
  reflow_t cold = {};
  TEST_ASSERT_EQ(k_ra8_err_not_initialized,
                 reflow_cache_load(&cold, s_content, s_content_len, s_blob, n));
  TEST_END("reflow_cache: reflow_cache_load MC/DC");
}
