/**
 * @file test_reflow_tokenize_tag_mcdc.c
 * @brief MC/DC tests for the XHTML tokenizer's tag / attribute decisions.
 *
 * @details
 * Split sibling of test_reflow_tokenize_scan_mcdc.c and
 * test_reflow_tokenize_link_mcdc.c covering the tag-dispatch halves of
 * apps/shared_libs/reflow/src/reflow_tokenize.c and the attribute capture in
 * reflow_tokenize_attr.c: start-tag name scanning, self-close handling
 ...
 * -- the start-tag NAME scan in internal_parse_start (reflow_tokenize.c).
...
RA8_INTERNAL static void internal_test_start_tag_name_scan(void)
{
  ...
  TEST_ASSERT(count_kind(k_reflow_tok_block_start) >= 1U);

  TEST_ASSERT_EQ(k_ra8_ok, walk("<html><body><p>a<br/>b</p></body></html>")); /* '/' */
  TEST_ASSERT_EQ(k_count_one, count_kind(k_reflow_tok_break));
 ...
 *
 * @par MC/DC:
 * Decisions in the attribute-skip loop of internal_parse_start (reflow_tokenize.c):
...
RA8_INTERNAL static void internal_test_start_tag_attr_and_selfclose(void)
{
  ...
  TEST_ASSERT_EQ(k_count_one, count_kind(k_reflow_tok_image));
 ...
 * (reflow_tokenize.c). The function returns the NEGATION, so a letter-prev
 ...
 *
 * @par MC/DC:
 * Decisions in internal_attr_quoted_value (reflow_tokenize.c):
 ...
 * store guard in priv_capture_attr (reflow_tokenize.c).
 ...
 *
 * @par MC/DC:
 * Decisions in internal_open_styled (reflow_tokenize.c):
 ...
 * internal_handle_start (reflow_tokenize.c).
...
RA8_INTERNAL static void internal_test_selfclose_block_pair(void)
{
  ...
  TEST_ASSERT_EQ(k_count_one, count_kind(k_reflow_tok_block_start));
 ...
 * start-tag fork in internal_handle_lt (reflow_tokenize.c).
 ...
 * of priv_reflow_xml_walk (reflow_tokenize.c).
 ...
 * Decisions in internal_attr_quoted_value (reflow_tokenize.c):
 *
 * L906 `while ((j < tag_len) && priv_reflow_tok_is_xml_whitespace(tag[j]))`:
 ...
 *
 * L913 `while ((j < tag_len) && priv_reflow_tok_is_xml_whitespace(tag[j]))`:
 ...
 *
 * @par MC/DC:
 * Decision (L1556): `(c == '>') || (c == '/') || priv_reflow_tok_is_xml_whitespace(c)`
...
RA8_INTERNAL static void internal_test_tag_is_slash_and_whitespace(void)
{
  ...
  TEST_ASSERT(c_ws != (uint32_t)k_reflow_color_inherit);
 ...
 * (apps/shared_libs/reflow/src/reflow_tokenize_attr.c, 4 conditions). Existing vectors
...
RA8_INTERNAL static void internal_test_attr_name_prev_punct_mcdc(void)
{
  ...
  TEST_ASSERT(priv_reflow_tok_find_attr(tag_pipe, sizeof(tag_pipe) - 1U, "id", 2U, &voff, &vlen));
  /* '_' (0x5F): (prev <= 'Z') false while (prev >= 'A') true. */
  const uint8_t tag_under[] = "<p _id=\"yz\">";
  TEST_ASSERT(priv_reflow_tok_find_attr(tag_under, sizeof(tag_under) - 1U, "id", 2U, &voff, &vlen));
 ...
 *
 * @par MC/DC:
 * Decision: `if ((vlen == 0U) || (text_pool_used + vlen > k_reflow_text_pool_bytes))`
 * in priv_reflow_tok_capture_attr (apps/shared_libs/reflow/src/reflow_tokenize_attr.c, 2
...
{
  TEST_BEGIN("reflow_tok_capture_attr MC/DC: text-pool overflow (C2 true)");
  s_engine.text_pool_used = (uint32_t)k_reflow_text_pool_bytes - 1U;
  uint32_t      off       = 0U;
  uint32_t      len       = 0U;
  const uint8_t tag[]     = "<img alt=\"ab\">";
  priv_reflow_tok_capture_attr(&s_engine, tag, sizeof(tag) - 1U, "alt", 3U, &off, &len);
  TEST_ASSERT_EQ(0, len); /* "ab" (2 bytes) does not fit in 1 remaining byte */
  s_engine.text_pool_used = 0U;
  TEST_END("reflow_tok_capture_attr MC/DC: text-pool overflow (C2 true)");
}
