/**
 * @file test_reflow_tokenize.c
 * @brief Unit tests for apps/shared_libs/reflow/src/reflow_tokenize.c
 *
 * @details
 * Drives the no-heap XHTML tokenizer's exposed helpers
 * (`priv_reflow_tok_is_xml_whitespace`, `priv_reflow_tok_classify`,
 * `priv_reflow_tok_decode_entity`, `priv_reflow_tok_utf8_encode`) and the
...
static reflow_t s_engine; /* large; keep off the stack */
 ...
 * apps/shared_libs/reflow/src/reflow_tokenize_lex.c@priv_reflow_tok_is_xml_whitespace.
...
{
  TEST_BEGIN("reflow_tok_is_xml_whitespace");
  const char ws[] = {' ', '\t', '\n', '\r', '\f', '\v'};
  for (size_t i = 0U; i < sizeof(ws); ++i) {
    TEST_ASSERT(priv_reflow_tok_is_xml_whitespace(ws[i]));
  }
  TEST_ASSERT(!priv_reflow_tok_is_xml_whitespace('a'));
  TEST_ASSERT(!priv_reflow_tok_is_xml_whitespace('\0'));
  TEST_END("reflow_tok_is_xml_whitespace");
}
 ...
 * apps/shared_libs/reflow/src/reflow_tokenize_lex.c@internal_local_lower.
...
{
  TEST_BEGIN("reflow_tok_classify");
  TEST_ASSERT_EQ(k_reflow_tag_p, priv_reflow_tok_classify("p", 1U));
  TEST_ASSERT_EQ(k_reflow_tag_p, priv_reflow_tok_classify("P", 1U));
  TEST_ASSERT_EQ(k_reflow_tag_strong, priv_reflow_tok_classify("STRONG", 6U));
  TEST_ASSERT_EQ(k_reflow_tag_p, priv_reflow_tok_classify("html:p", 6U));
  TEST_ASSERT_EQ(k_reflow_tag_br, priv_reflow_tok_classify("br", 2U));
  TEST_ASSERT_EQ(k_reflow_tag_unknown, priv_reflow_tok_classify("div", 3U));
  TEST_ASSERT_EQ(k_reflow_tag_unknown,
                 priv_reflow_tok_classify("averylongtagnamethatistruncated", 31U));
  TEST_ASSERT_EQ(k_reflow_tag_unknown, priv_reflow_tok_classify(nullptr, 0U));
  TEST_END("reflow_tok_classify");
}
 ...
 * @par MC/DC:
 * Decisions in
 * apps/shared_libs/reflow/src/reflow_tokenize_lex.c@priv_reflow_tok_decode_entity and
 * apps/shared_libs/reflow/src/reflow_tokenize_lex.c@internal_decode_numeric.
...
{
  TEST_BEGIN("reflow_tok_decode_entity");
  uint32_t cp   = 0U;
  size_t   used = 0U;
  TEST_ASSERT(priv_reflow_tok_decode_entity("&amp;", 5U, &cp, &used));
  TEST_ASSERT_EQ('&', cp);
  TEST_ASSERT_EQ(5, used);
  TEST_ASSERT(priv_reflow_tok_decode_entity("&lt;x", 5U, &cp, &used));
  TEST_ASSERT_EQ('<', cp);
  ...
  TEST_ASSERT(!priv_reflow_tok_decode_entity("&zz;", 4U, &cp, &used));  /* unknown name      */
  TEST_ASSERT(!priv_reflow_tok_decode_entity("&amp", 4U, &cp, &used));  /* no terminator     */
  TEST_ASSERT(!priv_reflow_tok_decode_entity("&#;", 3U, &cp, &used));   /* no digits / short */
  TEST_ASSERT(!priv_reflow_tok_decode_entity("&#9z;", 5U, &cp, &used)); /* bad digit         */
  TEST_END("reflow_tok_decode_entity");
}
...
{
  TEST_BEGIN("reflow_tok_utf8_encode");
  uint8_t b[4];
  TEST_ASSERT_EQ(1, priv_reflow_tok_utf8_encode(0x41U, b));
  TEST_ASSERT_EQ(0x41, b[0]);
  TEST_ASSERT_EQ(2, priv_reflow_tok_utf8_encode(0xE9U, b)); /* e-acute */
  TEST_ASSERT_EQ(0xC3, b[0]);
  TEST_ASSERT_EQ(0xA9, b[1]);
  TEST_ASSERT_EQ(3, priv_reflow_tok_utf8_encode(0x20ACU, b));   /* euro    */
  TEST_ASSERT_EQ(4, priv_reflow_tok_utf8_encode(0x1F600U, b));  /* emoji   */
  TEST_ASSERT_EQ(4, priv_reflow_tok_utf8_encode(0xFFFFFFU, b)); /* clamped */
  TEST_END("reflow_tok_utf8_encode");
}
...
RA8_INTERNAL static void internal_walk_check_void_selfclose(void)
{
  ...
  for (uint32_t i = 0U; i < s_engine.token_count; ++i) {
    saw_break = saw_break || (s_engine.tokens[i].kind == k_reflow_tok_break);
    saw_image = saw_image || (s_engine.tokens[i].kind == k_reflow_tok_image);
    saw_rule  = saw_rule || (s_engine.tokens[i].kind == k_reflow_tok_rule);
  }
...
RA8_INTERNAL static void internal_walk_check_cdata(void)
{
  ...
  for (uint32_t i = 0U; i < s_engine.token_count; ++i) {
    if (s_engine.tokens[i].kind == k_reflow_tok_text) {
 ...
 * apps/shared_libs/reflow/src/reflow_tokenize.c@internal_handle_lt,
 * apps/shared_libs/reflow/src/reflow_tokenize.c@internal_parse_start,
 * apps/shared_libs/reflow/src/reflow_tokenize.c@internal_handle_start,
 * apps/shared_libs/reflow/src/reflow_tokenize.c@internal_handle_end,
 * apps/shared_libs/reflow/src/reflow_tokenize.c@internal_handle_cdata,
 * apps/shared_libs/reflow/src/reflow_tokenize.c@priv_reflow_xml_walk.
...
RA8_INTERNAL static void internal_test_walk_end_to_end(void)
{
  ...
  TEST_ASSERT_EQ(k_reflow_tok_block_start, s_engine.tokens[0].kind);
  TEST_ASSERT_EQ(k_reflow_tag_p, s_engine.tokens[0].tag);
  bool saw_bold_text = false;
  bool saw_block_end = false;
  for (uint32_t i = 0U; i < s_engine.token_count; ++i) {
    if ((s_engine.tokens[i].kind == k_reflow_tok_text) &&
        ((s_engine.tokens[i].style & (uint8_t)k_reflow_style_bold) != 0U)) {
      saw_bold_text = true;
    }
    if (s_engine.tokens[i].kind == k_reflow_tok_block_end) {
      saw_block_end = true;
    }
  }
 ...
 * `saw_img || (tokens[i].kind == k_reflow_tok_image)` (2 conditions, OR).
...
RA8_INTERNAL static void internal_test_display_none_suppressed(void)
{
  ...
  bool saw_img = false;
  for (uint32_t i = 0U; i < s_engine.token_count; ++i) {
    saw_img = saw_img || (s_engine.tokens[i].kind == (uint8_t)k_reflow_tok_image);
  }
...
/** @brief reflow_css_loader_fn stub: hands back ::k_ext_css for any href.
 ...
 * 3-condition guard in apps/shared_libs/reflow/src/reflow_tokenize.c@internal_handle_link.
