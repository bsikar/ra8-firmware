/**
 * @file test_reflow_tokenize_scan_mcdc.c
 * @brief MC/DC tests for the XHTML tokenizer's scanning decision families.
 *
 * @details
 * Split sibling of test_reflow_tokenize_tag_mcdc.c and
 * test_reflow_tokenize_link_mcdc.c covering the byte-scanning halves of
 * apps/shared_libs/reflow/src/reflow_tokenize.c: the numeric / named entity
 ...
 * hex vs decimal in internal_decode_numeric (reflow_tokenize.c).
...
RA8_INTERNAL static void internal_test_numeric_base_select(void)
{
  ...
  uint32_t cp   = 0U;
  size_t   used = 0U;
  TEST_ASSERT(priv_reflow_tok_decode_entity("&#65;", 5U, &cp, &used)); /* V1 decimal */
  TEST_ASSERT_EQ(k_cp_uppercase_a, cp);
  TEST_ASSERT_EQ(k_used_dec_a, used);
  TEST_ASSERT(priv_reflow_tok_decode_entity("&#x4f;", 6U, &cp, &used)); /* V2 'x' hex */
  TEST_ASSERT_EQ(k_cp_hex_4f, cp);
  TEST_ASSERT(priv_reflow_tok_decode_entity("&#X4F;", 6U, &cp, &used)); /* V3 'X' hex */
  TEST_ASSERT_EQ(k_cp_hex_4f_upper, cp);
  TEST_ASSERT(!priv_reflow_tok_decode_entity("&#x", k_avail_three, &cp, &used)); /* V4 short */
 ...
 * (reflow_tokenize.c): the decimal class `(c>='0')&&(c<='9')`, the
 ...
 * post-loop terminator check in internal_decode_numeric (reflow_tokenize.c).
...
RA8_INTERNAL static void internal_test_numeric_terminator(void)
{
  ...
  uint32_t cp   = 0U;
  size_t   used = 0U;
  TEST_ASSERT(priv_reflow_tok_decode_entity("&#65;", 5U, &cp, &used)); /* V1 accept */
  TEST_ASSERT_EQ(k_cp_uppercase_a, cp);
  TEST_ASSERT(!priv_reflow_tok_decode_entity("&#;", 3U, &cp, &used));  /* V2 no digits     */
  TEST_ASSERT(!priv_reflow_tok_decode_entity("&#65", 4U, &cp, &used)); /* V3 no terminator */
 ...
 * reject in priv_reflow_tok_decode_entity (reflow_tokenize.c). `window` is
...
RA8_INTERNAL static void internal_test_entity_short_guard(void)
{
  ...
  TEST_ASSERT(priv_reflow_tok_decode_entity("&amp;", 5U, &cp, &used)); /* V1 proceed */
  TEST_ASSERT_EQ('&', cp);
  TEST_ASSERT(!priv_reflow_tok_decode_entity("&lt", 3U, &cp, &used));    /* V2 window<min */
  TEST_ASSERT(!priv_reflow_tok_decode_entity(nul_at_1, 5U, &cp, &used)); /* V3 src[1]==0  */
 ...
 * the named-entity match in priv_reflow_tok_decode_entity (reflow_tokenize.c).
...
RA8_INTERNAL static void internal_test_named_entity_match(void)
{
  ...
  uint32_t cp   = 0U;
  size_t   used = 0U;
  TEST_ASSERT(priv_reflow_tok_decode_entity("&quot;", 6U, &cp, &used)); /* V1 match */
  TEST_ASSERT_EQ('"', cp);
  TEST_ASSERT(!priv_reflow_tok_decode_entity("&ampX", 5U, &cp, &used)); /* V2 no ';'        */
  TEST_ASSERT(!priv_reflow_tok_decode_entity("&zzz;", 5U, &cp, &used)); /* V3 name mismatch */
 ...
 *
 * @par MC/DC:
 * Decisions in internal_handle_cdata (reflow_tokenize.c):
 *  - close scan checks the three `]]>` bytes independently. Ordinary text,
 *    `]x`, `]]x`, and the real terminator make each byte comparison determine
 *    the result while the others are held at the needed control values. An
...
RA8_INTERNAL static void internal_test_cdata_close_and_emit(void)
{
  ...
  /* Independently reject partial terminators: first ']' only, then two ']'. */
  TEST_ASSERT_EQ(k_ra8_ok, walk("<html><body><p><![CDATA[]x]]xZ]]></p></body></html>"));
  TEST_ASSERT(text_has("]x]]xZ"));
 ...
 *
 * @par MC/DC:
 * Decisions in internal_handle_end (reflow_tokenize.c):
...
RA8_INTERNAL static void internal_test_end_tag_scan_and_block_end(void)
{
  ...
  TEST_ASSERT(count_kind(k_reflow_tok_block_end) >= 1U);
  ...
  TEST_ASSERT_EQ(k_count_one, count_kind(k_reflow_tok_block_end));
 ...
 * internal_decode_numeric (reflow_tokenize.c). The existing vectors V1-V4 in
...
RA8_INTERNAL static void internal_test_numeric_prefix_avail_zero(void)
{
  ...
  TEST_ASSERT(!priv_reflow_tok_decode_entity("&#", 2U, &cp, &used));
...
RA8_INTERNAL static void internal_test_numeric_digit_subrange(void)
{
  ...
  TEST_ASSERT(!priv_reflow_tok_decode_entity("&#xg;", 5U, &cp, &used));
 ...
 * cover it.
 *
 * @note MC/DC deactivation: the `src[i] != ';'` arm of the L271 OR
 ...
 *
 * @par MC/DC:
 * Decision: `if ((tlen > 0U) && !priv_reflow_tok_emit(...))` -- the CDATA text emit
 * in internal_handle_cdata (apps/shared_libs/reflow/src/reflow_tokenize.c, 2 conditions, AND).
 ...
 * k_reflow_max_tokens - 1 makes the opening `<p>` fill the pool, so the CDATA
...
RA8_INTERNAL static void internal_test_cdata_emit_pool_full_mcdc(void)
{
  ...
  s_engine.token_count    = (uint32_t)k_reflow_max_tokens - 1U;
  ...
  TEST_ASSERT_EQ(k_reflow_max_tokens, s_engine.token_count);
 ...
 *
 * @par MC/DC:
 * Decision: `if (priv_reflow_tok_is_block(tag) && !internal_suppressed(ctx) &&
 * !priv_reflow_tok_emit(...))` -- the block-end emit in internal_handle_end
 * (apps/shared_libs/reflow/src/reflow_tokenize.c, 3 conditions, AND). The final
 * emit-failure condition needs the token pool full at a block close. Pre-setting
 * token_count to k_reflow_max_tokens - 1 lets the opening `<p>` fill the pool
 ...
 * (apps/shared_libs/reflow/src/reflow_tokenize_lex.c, 3 conditions, AND). Existing
