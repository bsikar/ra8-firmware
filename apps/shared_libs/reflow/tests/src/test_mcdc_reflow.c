 ...
 * @brief MC/DC floor vectors for reachable compound decisions in reflow.
 *
 * @details
 * Supplements the existing reflow MC/DC suites with the independence vectors
 * the per-file MC/DC floor still reports as gaps:
 *  - reflow_css.c@priv_reflow_css_ci_eq -- the `(s == nullptr) || (lit == nullptr)`
 *    contract guard, driven directly through the TU-external comparator.
 *  - the SVG path/element scanners in reflow_svg_path.c / reflow_svg_doc.c,
 ...
 * @details Binds a function-local ARGB8888 host framebuffer, exercises the
 * reflow render path, and preserves each documented result and bound.
 ...
 * @brief Independence vectors for the priv_reflow_css_ci_eq NULL contract guard.
 *
 * @par MC/DC:
 * Decision: `if ((s == nullptr) || (lit == nullptr))` (2 conditions, OR;
 * apps/shared_libs/reflow/src/reflow_css.c@priv_reflow_css_ci_eq).
...
{
  TEST_BEGIN("reflow_css_ci_eq MC/DC: (s==null) || (lit==null)");
  const char* const red = "red";
  /* V1: neither NULL -> guard false -> real comparison, equal strings match. */
  TEST_ASSERT(priv_reflow_css_ci_eq(red, 3U, "red"));
  TEST_ASSERT(!priv_reflow_css_ci_eq(red, 3U, "blue"));
  /* V2: s NULL (C1 true) -> guard true -> false. */
  TEST_ASSERT(!priv_reflow_css_ci_eq(nullptr, 3U, "red"));
  /* V3: lit NULL (C2 true) -> guard true -> false. */
  TEST_ASSERT(!priv_reflow_css_ci_eq(red, 3U, nullptr));
  TEST_END("reflow_css_ci_eq MC/DC: (s==null) || (lit==null)");
}
 ...
 *    (3 conditions; apps/shared_libs/reflow/src/reflow_svg_path.c@internal_parse_path).
 ...
 *    (2 conditions; apps/shared_libs/reflow/src/reflow_svg_doc.c@internal_group_open).
