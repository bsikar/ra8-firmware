/**
 * @file test_reflow_layout_content_mcdc.c
 * @brief MC/DC tests for the layout engine's image / table decisions.
 *
 * @details
 * Split sibling of test_reflow_layout_flow_mcdc.c covering the embedded
 * content decision families of apps/shared_libs/reflow/src/reflow_layout.c: the
 ...
 * (3 conditions, AND; apps/shared_libs/reflow/src/reflow_layout_image.c@priv_reflow_layout_apply_image --
...
RA8_INTERNAL static void internal_test_apply_image_loader_gate_mcdc(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, reflow_set_image_loader(&s_eng, test_image_loader, nullptr, &arena));
  ...
  s_loader_select = k_loader_2x2;
  init_engine(k_vp_w, k_vp_h);
  TEST_ASSERT_EQ(k_ra8_ok, reflow_set_image_loader(&s_eng, test_image_loader, nullptr, nullptr));
  (void)lay("<html><body><p>before<img src=\"f.png\">after</p></body></html>");
  TEST_ASSERT_EQ(0, s_eng.image_box_count);
 ...
 * @par MC/DC:
 * Two decisions in
 * apps/shared_libs/reflow/src/reflow_layout_image.c@internal_image_resolve_size:
 *  (A) `if ((ra8_img_probe_size(...) != ok) || (iw <= 0) || (ih <= 0))`
 *      -- 3 conditions.
 *  (B) `if ((col_w < 1) || (avail_h < 1))` -- 2 conditions.
 *
 * Vectors for (A), N = 3 so N+1 = 4:
 *  - V1: real 2x2 PNG, normal viewport -> C1=F, C2=F, C3=F -> decision F, the
 *        box is resolved and an image box is recorded. (Control.)
 *  - V2: junk bytes -> C1=T, short-circuits -> decision T -> placeholder
 *        fallback, no box. Pair (V1,V2) varies C1 alone.
 *  - V5: a 54-byte BMP header declaring biWidth = 0, biHeight = 1 ->
 *        ra8_img_probe_size SUCCEEDS (stb_image's header parser has no
 *        zero-dimension rejection) so C1=F, and C2 (iw <= 0) is T ->
 *        decision T -> no box. Pair (V1,V5) varies C2 alone.
 *  - V6: the same BMP with biWidth = 1, biHeight = 0 -> C1=F, C2=F,
 *        C3 (ih <= 0) is T -> decision T -> no box. Pair (V1,V6) varies C3
 *        alone.
 * V5 and V6 are the vectors the old deactivation note said no public path
 * could produce; they need a probe that succeeds with a zero edge, which the
 * BMP header path supplies.
 *
 * Vectors for (B):
 *  - V1 above is the all-false control.
 *  - V3: real PNG, 32px-wide viewport (col_w == 0) -> C1=T -> decision T ->
 *        placeholder fallback, no box. Isolates the col_w<1 condition.
...
RA8_INTERNAL static void internal_test_image_resolve_size_mcdc(void)
{
  ...
  /* V5: the probe SUCCEEDS but reports width 0 -> the iw guard alone flips. */
  s_loader_select = k_loader_bmp_zero_w;
  init_engine(k_vp_w, k_vp_h);
  TEST_ASSERT_EQ(k_ra8_ok, reflow_set_image_loader(&s_eng, test_image_loader, nullptr, &arena));
  (void)lay("<html><body><p><img src=\"f.png\"></p></body></html>");
  TEST_ASSERT_EQ(0, s_eng.image_box_count);

  /* V6: the probe SUCCEEDS but reports height 0 -> the ih guard alone flips. */
  s_loader_select = k_loader_bmp_zero_h;
  init_engine(k_vp_w, k_vp_h);
  TEST_ASSERT_EQ(k_ra8_ok, reflow_set_image_loader(&s_eng, test_image_loader, nullptr, &arena));
  (void)lay("<html><body><p><img src=\"f.png\"></p></body></html>");
  TEST_ASSERT_EQ(0, s_eng.image_box_count);

  /* Leave the shared selector on a decodable fixture for later cases. */
  s_loader_select = k_loader_2x2;
 ...
 * (2 conditions, AND; apps/shared_libs/reflow/src/reflow_layout_image.c@internal_place_image --
 ...
 * (2 conditions, AND; apps/shared_libs/reflow/src/reflow_layout_table.c@internal_layout_row --
 ...
 *
 * @par MC/DC:
 * Decision at reflow_layout.c L926:
 * `if ((col_w < 1) || (avail_h < 1))`
 * (2 conditions, OR; apps/shared_libs/reflow/src/reflow_layout_image.c@internal_image_resolve_size).
 *
 ...
 *
 * @par MC/DC:
 * Decision at reflow_layout.c L1015:
 ...
 *
 * @par MC/DC:
 * Decision at reflow_layout.c L1229:
 * `(tok->tag == k_reflow_tag_td) || (tok->tag == k_reflow_tag_th)`
 * (2 conditions, OR; apps/shared_libs/reflow/src/reflow_layout_table.c@internal_is_cell_start).
 * Decision at reflow_layout.c L1236:
 * `(tok->kind == k_reflow_tok_block_start) &&
 *  (tok->tag == k_reflow_tag_tr)`
 * (2 conditions, AND; apps/shared_libs/reflow/src/reflow_layout_table.c@internal_is_row_start).
 * Decision at reflow_layout.c L1325:
 * `if ((*cx > cell_x) && ((*cx + adv) <= cell_right))`
 * (2 conditions, AND; apps/shared_libs/reflow/src/reflow_layout_table.c@internal_cell_text --
 ...
 *
 * @par MC/DC:
 * Decision at reflow_layout.c L1452:
 * `if (((cur->y + row_h) > bottom_limit) && (cur->y > (int32_t)k_reflow_margin_px))`
 * (2 conditions, AND; apps/shared_libs/reflow/src/reflow_layout_table.c@internal_layout_row).
 *
 ...
 * placed exactly at the top margin (cur->y == k_reflow_margin_px) that
 ...
 *    The table layout starts with cur->y == k_reflow_margin_px (16 px).
 *    A tall row (multi-line cell text) makes row_h > bottom_limit - 16, so
 *    C1 = T (overflow).  But cur->y == k_reflow_margin_px, so
 ...
 *   `(tok->kind == k_reflow_tok_block_start) && (tok->tag == k_reflow_tag_tr)`
 * (2 conditions, AND; apps/shared_libs/reflow/src/reflow_layout_table.c). internal_table_columns
