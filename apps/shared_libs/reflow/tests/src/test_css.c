...
static bool internal_bytes_equal(const uint8_t* bytes, const char* text, size_t len)
{
  bool equal = true;
...
static ra8_css_element_t elem(reflow_html_tag_t tag, const char* id, const char* cls)
{
  ra8_css_element_t e = {};
  e.tag               = (uint8_t)tag;
  e.id                = id;
...
static void test_parse_props(void)
{
  ...
  TEST_ASSERT((d.style & (uint8_t)k_reflow_style_bold) != 0U);
  TEST_ASSERT((d.style & (uint8_t)k_reflow_style_italic) != 0U);
  TEST_ASSERT((d.style & (uint8_t)k_reflow_style_underline) != 0U);
  TEST_ASSERT_EQ(k_reflow_align_justify, d.align);
  TEST_ASSERT_EQ(k_reflow_tag_p, s_sheet.rules[0].sel_tag); /* type only */
...
static void test_parse_normal_resets(void)
{
  ...
  TEST_ASSERT((d.style & (uint8_t)k_reflow_style_bold) == 0U); /* but off  */
  TEST_ASSERT((d.set & (uint8_t)k_ra8_css_set_italic) != 0U);
  TEST_ASSERT((d.style & (uint8_t)k_reflow_style_italic) == 0U);
  TEST_ASSERT((d.style & (uint8_t)k_reflow_style_underline) == 0U);
...
static void test_parse_selectors(void)
{
  ...
  TEST_ASSERT_EQ(k_reflow_tag_unknown, s_sheet.rules[0].sel_tag);
  TEST_ASSERT_EQ(0, s_sheet.rules[0].class_len);
  TEST_ASSERT_EQ(0, s_sheet.rules[0].id_len);
...
static void test_parse_unsupported_skipped(void)
{
  ...
  TEST_ASSERT_EQ(1, s_sheet.rule_count);
  TEST_ASSERT_EQ(k_reflow_tag_h1, s_sheet.rules[0].sel_tag);
...
static void test_inline(void)
{
  ...
  TEST_ASSERT((d.set & (uint8_t)k_ra8_css_set_bold) != 0U);
  TEST_ASSERT((d.style & (uint8_t)k_reflow_style_bold) != 0U);
  TEST_ASSERT_EQ(k_reflow_align_center, d.align);
 ...
 * @par MC/DC:
 * Decision owner:
 * apps/shared_libs/reflow/src/reflow_css_cascade.c@ra8_css_rule_matches
 * Decision (id match): `(el->id != NULL) && (el->id_len == name_len) &&
 * (all id bytes match)`. The first two conditions initialize `equal`; the
 * bounded byte loop evaluates `(equal && (i < id_len))`.
 ...
 * Vectors 1+2 isolate the non-NULL condition; 1+3 the length condition;
 * 1+4 the byte-equality condition. For the two-condition byte-loop guard,
 * V1 supplies `(T,T)` while bytes remain and `(T,F)` at the exact-length exit;
 * V4 supplies `(F,T)` after the first unequal byte. These are its N+1 vectors.
 * A fifth robustness vector supplies a NULL id with a forged matching length;
 * matching must fail closed without reading through the pointer.
 */
static void test_match_mcdc(void)
{
  ...
  ra8_css_element_t     v1 = elem(k_reflow_tag_p, "lead", nullptr);
  ra8_css_element_t     v2 = elem(k_reflow_tag_p, nullptr, nullptr);
  ra8_css_element_t     v3 = elem(k_reflow_tag_p, "leadX", nullptr);
  ra8_css_element_t     v4 = elem(k_reflow_tag_p, "xxxx", nullptr);
  ra8_css_element_t     v5 = v2;
  v5.id_len                = r->id_len;
  ...
  TEST_ASSERT(!ra8_css_rule_matches(r, &v5, &s_sheet)); /* NULL + forged length */
...
static void test_match_class_universal_type(void)
{
  ...
  ra8_css_element_t     a  = elem(k_reflow_tag_p, nullptr, "intro note last");
  ra8_css_element_t     b  = elem(k_reflow_tag_p, nullptr, "introduction");
  ra8_css_element_t     h2 = elem(k_reflow_tag_h2, nullptr, nullptr);
...
static void test_cascade_specificity(void)
{
  ...
  ra8_css_element_t el = elem(k_reflow_tag_p, "i", "c");
  ra8_css_style_t   r  = ra8_css_cascade(&s_sheet, &el, (ra8_css_style_t){}, no_inline());
  TEST_ASSERT_EQ(k_reflow_align_justify, r.align); /* #i wins */

  ra8_css_element_t el2 = elem(k_reflow_tag_p, nullptr, "c");
  ra8_css_style_t   r2  = ra8_css_cascade(&s_sheet, &el2, (ra8_css_style_t){}, no_inline());
  TEST_ASSERT_EQ(k_reflow_align_center, r2.align); /* .c beats p, * */

  ra8_css_style_t inl = {};
  inl.set             = (uint8_t)k_ra8_css_set_align;
  inl.align           = (uint8_t)k_reflow_align_left;
  ra8_css_style_t r3  = ra8_css_cascade(&s_sheet, &el, (ra8_css_style_t){}, inl);
  TEST_ASSERT_EQ(k_reflow_align_left, r3.align); /* inline beats #i */
...
static void test_cascade_source_order(void)
{
  ...
  ra8_css_style_t   r  = ra8_css_cascade(&s_sheet, &el, (ra8_css_style_t){}, no_inline());
  TEST_ASSERT_EQ(k_reflow_align_center, r.align); /* later wins */
...
static void test_cascade_inheritance(void)
{
  ...
  parent.style           = (uint8_t)((uint8_t)k_reflow_style_bold | (uint8_t)k_reflow_style_italic);

  ra8_css_element_t child = elem(k_reflow_tag_em, nullptr, nullptr);
  ...
  TEST_ASSERT((r.style & (uint8_t)k_reflow_style_bold) == 0U);
  TEST_ASSERT((r.style & (uint8_t)k_reflow_style_italic) != 0U);
...
static void test_cascade_color(void)
{
  ...
  ra8_css_element_t el = elem(k_reflow_tag_p, nullptr, "hi");
  ra8_css_style_t   r  = ra8_css_cascade(&s_sheet, &el, (ra8_css_style_t){}, no_inline());
  ...
  ra8_css_element_t kid  = elem(k_reflow_tag_strong, nullptr, nullptr);
...
static ra8_css_style_t fs(const char* value)
{
  ...
  const char* const prefix              = "font-size:";
  size_t            pos                 = 0U;
  while (((pos + 1U) < sizeof buf) && (prefix[pos] != '\0')) {
    buf[pos] = prefix[pos];
    ++pos;
  }
  size_t i = 0U;
  while (((pos + i + 1U) < sizeof buf) && (value[i] != '\0')) {
    buf[pos + i] = value[i];
    ++i;
  }
  buf[pos + i]      = '\0';
...
static void test_decoration_line_alias(void)
{
  ...
  (void)ra8_css_parse_inline("text-decoration:underline",
                             ...
                             &d1);
  TEST_ASSERT((d1.style & (uint8_t)k_reflow_style_underline) != 0U); /* cond1 */
  ra8_css_style_t d2 = {};
  (void)ra8_css_parse_inline("text-decoration-line:underline",
                             (uint32_t)strlen("text-decoration-line:underline"),
                             &d2);
  TEST_ASSERT((d2.style & (uint8_t)k_reflow_style_underline) != 0U); /* cond2 */
  ra8_css_style_t d3 = {};
...
static void test_cascade_fontsize(void)
{
  ...
  ra8_css_element_t el = elem(k_reflow_tag_p, nullptr, "big");
  ra8_css_style_t   r  = ra8_css_cascade(&s_sheet, &el, (ra8_css_style_t){}, no_inline());
  ...
  ra8_css_element_t el2 = elem(k_reflow_tag_p, nullptr, nullptr);
  ra8_css_style_t   r2  = ra8_css_cascade(&s_sheet, &el2, (ra8_css_style_t){}, no_inline());
...
static void test_display(void)
{
  ...
  ra8_css_element_t h1  = elem(k_reflow_tag_h1, nullptr, nullptr);
...
static void test_compound_selectors(void)
{
  ...
  TEST_ASSERT_EQ(k_reflow_tag_p, rnote->sel_tag);
  TEST_ASSERT(rnote->class_len > 0U);

  /* p.note matches only when BOTH the tag and the class match. */
  ra8_css_element_t p_note  = elem(k_reflow_tag_p, nullptr, "intro note");
  ra8_css_element_t p_plain = elem(k_reflow_tag_p, nullptr, nullptr);
  ra8_css_element_t h1_note = elem(k_reflow_tag_h1, nullptr, "note");
...
static void test_resolve_skip_mcdc(void)
{
  ...
  TEST_ASSERT_EQ(k_reflow_align_center, r1.align);
...
static void test_resolve_winner_mcdc(void)
{
  ...
  ra8_css_element_t pid = elem(k_reflow_tag_p, "i", nullptr);
  ra8_css_element_t p   = elem(k_reflow_tag_p, nullptr, nullptr);
  ...
  TEST_ASSERT_EQ(k_reflow_align_center, v3.align);
  ...
  TEST_ASSERT_EQ(k_reflow_align_center, v4.align);
...
static void test_fontface_parse(void)
{
  ...
  TEST_ASSERT((slen == (uint16_t)strlen("fonts/Body.ttf")) &&
              (strncmp(src, "fonts/Body.ttf", slen) == 0));
  ...
  TEST_ASSERT((slen == (uint16_t)strlen("fonts/Body-Bold.ttf")) &&
              (strncmp(src, "fonts/Body-Bold.ttf", slen) == 0));
...
static void test_fontface_cascade(void)
{
  ...
  TEST_ASSERT((ps.family_len == 4U) &&
              internal_bytes_equal(&s_sheet.names[ps.family_off], "Body", 4U));
  ra8_css_element_t em = elem(k_reflow_tag_em, nullptr, nullptr);
  ...
  TEST_ASSERT((es.family_len == 4U) &&
              internal_bytes_equal(&s_sheet.names[es.family_off], "Body", 4U));
...
static void test_descendant(void)
{
  ...
  TEST_ASSERT_EQ(k_reflow_tag_p, s_sheet.rules[0].sel_tag); /* subject p         */
  ...
  const ra8_css_element_t h1      = elem(k_reflow_tag_h1, nullptr, nullptr);
  const ra8_css_element_t chap[1] = {elem(k_reflow_tag_h1, nullptr, "chapter")};
  ...
  TEST_ASSERT((c1.style & (uint8_t)k_reflow_style_bold) != 0U);
  TEST_ASSERT((c1.style & (uint8_t)k_reflow_style_italic) != 0U);
  ...
  TEST_ASSERT((c3.style & (uint8_t)k_reflow_style_bold) == 0U);
  TEST_ASSERT((c3.style & (uint8_t)k_reflow_style_italic) != 0U);
  ...
  TEST_ASSERT((c2.style & (uint8_t)k_reflow_style_bold) == 0U);
  ...
  const ra8_css_element_t em    = elem(k_reflow_tag_em, nullptr, nullptr);
  const ra8_css_element_t pa[1] = {elem(k_reflow_tag_p, nullptr, nullptr)};
  const ra8_css_style_t   ce    = ra8_css_cascade_ctx(&s_sheet, &em, inh, inl, pa, 1U);
  TEST_ASSERT((ce.style & (uint8_t)k_reflow_style_underline) != 0U);
  const ra8_css_style_t cen = ra8_css_cascade_ctx(&s_sheet, &em, inh, inl, nullptr, 0U);
  TEST_ASSERT((cen.style & (uint8_t)k_reflow_style_underline) == 0U);
 ...
 * @details Walks a function-local, fixed-order roster of every test case so
