 ...
 *        reflow renders -- all heap-free, all on host.
 *
 * @details
 ...
 * (so `just apps::emulator::run ereader_ui` can serve a font from an emulated card).
 *
 * Pipeline: image (FAT) -> SD card model -> ra8_sdmmc_spi -> ra8_fs ->
 * reflow -> ra8_gfx framebuffer.
 *
...
typedef struct {
  uint8_t  bytes[(size_t)k_disk_blocks_fat16 * (size_t)k_disk_block_size]; /**< Bytes.       */
  uint32_t block_count;                                                    /**< Block count. */
} mem_disk_t;

static mem_disk_t s_disk = {.bytes = {}, .block_count = (uint32_t)k_disk_blocks_fat16};
...
RA8_INTERNAL static void internal_put16(uint8_t* p, uint32_t off, uint16_t v)
{
  ...
  p[off + 1U] = (uint8_t)(((uint32_t)v >> 8U) & (uint32_t)k_t_byte_mask);
}
...
{
  (void)memset(s_disk.bytes, 0, sizeof s_disk.bytes);
...
static uint8_t  s_src_font[k_font_cap];
static uint8_t  s_card_font[k_font_cap];
...
{
  const ra8_fs_backend_t mem_backend = {.read_block   = internal_mem_read,
                                        .write_block  = internal_mem_write,
                                        .get_capacity = internal_mem_cap,
                                        .ctx          = &s_disk};
  internal_build_fat16();
  ra8_fs_mount_t* setup = nullptr;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_mount(&mem_backend, &setup));
 ...
 * already-covered ra8_sdmmc_spi / ra8_fs / reflow APIs end to end through
...
{
  TEST_BEGIN("SD card (SPI) serves FAT font -> ra8_fs -> reflow");
  ...
  if (!internal_load_src_font()) {
    ...
    TEST_END("SD card (SPI) serves FAT font -> ra8_fs -> reflow");
    return;
  }
