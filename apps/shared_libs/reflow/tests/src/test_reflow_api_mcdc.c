/**
 * @file test_reflow_api_mcdc.c
 * @brief MC/DC vector tests for the public reflow API guards (v1 engine).
 *
 * @details
 * Split sibling of test_reflow.c carrying the MC/DC vector tests for the
 * compound guard decisions of apps/shared_libs/reflow/src/reflow_layout.c: the
 * reflow_init() null / viewport / font-size ORs, the
 ...
 * (2 conditions, apps/shared_libs/reflow/src/reflow_layout.c line 535). N+1=3.
...
RA8_INTERNAL static void internal_test_mcdc_layout_chapter_null_or3(void)
{
  ...
  /* V3 */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 reflow_layout_chapter(&s_engine, nullptr, strlen(s_xhtml_simple), &pages));
  /* V4 */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 reflow_layout_chapter(&s_engine,
                                       (const uint8_t*)s_xhtml_simple,
...
RA8_INTERNAL static void internal_test_mcdc_get_page_count_null_or(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_init(k_test_viewport_w,
                             ...
                             &s_engine));
  uint32_t pages = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, reflow_get_page_count(&s_engine, &pages));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, reflow_get_page_count(nullptr, &pages));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, reflow_get_page_count(&s_engine, nullptr));
  TEST_ASSERT_EQ(k_ra8_ok, reflow_close(&s_engine));
...
RA8_INTERNAL static void internal_test_mcdc_set_font_size_range_or(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, reflow_set_font_size(&s_engine, (uint16_t)k_mcdc_font_px_ok));
  /* V2 too low. */
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 reflow_set_font_size(&s_engine, (uint16_t)k_mcdc_font_px_too_low));
  /* V3 too high. */
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 reflow_set_font_size(&s_engine, (uint16_t)k_mcdc_font_px_too_high));
  TEST_ASSERT_EQ(k_ra8_ok, reflow_close(&s_engine));
 ...
 *   the public API: reflow_layout_chapter sets buf and len atomically
 *   from the caller, and reflow_init zeroes both. Argued by code
...
RA8_INTERNAL static void internal_test_mcdc_set_font_size_state_or(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_state,
                 reflow_set_font_size(&s_engine, (uint16_t)k_mcdc_font_px_ok));
 ...
 * (2 conditions, apps/shared_libs/reflow/src/reflow_layout.c line 510).
 * Reachable only via reflow_run_layout (called from
 * reflow_layout_chapter). The decision drives whether to synthesise
...
RA8_INTERNAL static void internal_test_mcdc_run_layout_empty_guard(void)
{
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    reflow_layout_chapter(&s_engine, (const uint8_t*)k_empty_p, strlen(k_empty_p), &pages));
 ...
 *          reflow_init to pass its blob validation), so when the vendored
 *          Literata face is unreachable in the sandbox the suite skips
 *          cleanly, mirroring test_reflow.c.
 *
...
int main(void)
{
  if (!internal_priv_load_font()) {
    (void)internal_test_output_fd_text(STDERR_FILENO,
                                       "[SKIP] test_reflow_api_mcdc: Literata-Regular.ttf not "
  ...
  (void)internal_test_output_fd_text(STDERR_FILENO,
                                     "[OK  ] test_reflow_api_mcdc: all MC/DC vectors passed\n");
  return 0;
}
