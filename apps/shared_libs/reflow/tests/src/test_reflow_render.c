/**
 * @file test_reflow_render.c
 * @brief MC/DC vectors for apps/shared_libs/reflow/src/reflow_render.c
 *
 * @details
 ...
 *    ``tests/usb/src/test_ux_dcd_ra8_usb.c``).
 *
 *  - A set of end-to-end tests that drive the **real** render through the public
 *    ``reflow_render_page()`` API against an ``ra8_gfx`` framebuffer, using the
 *    bundled Literata face (located from the CMake-provided repository root;
 *    the test SKIPs if the font is absent). These exercise production source
 *    the mirror cannot reach:
 ...
 * @par MC/DC:
 * Decision: ``if (w > 0 && h > 0)``
 * (2 conditions, apps/shared_libs/reflow/src/reflow_render.c@internal_blit_glyph)
 *
...
{
  TEST_BEGIN("reflow_render priv_blit_glyph size MC/DC: w>0 && h>0");
  ...
  TEST_END("reflow_render priv_blit_glyph size MC/DC: w>0 && h>0");
}
 ...
 * 2x2 RGB PNG that makes layout record an image box and the render path decode
 * and blit it. Performs one bounded, deterministic operation for this test.
 * @retval 0 Zero or false result; nonzero values describe the alternate result.
 * @pre Pointer arguments, when present, address their documented test storage.
 * @pre Scalar arguments satisfy the bounds asserted by this test helper.
 * @post The helper completes only the bounded test operation described above.
...
{
  return reflow_init((uint16_t)k_r_vp_w,
                     (uint16_t)k_r_vp_h,
                     s_font,
                     s_font_len,
 ...
 * apps/shared_libs/reflow/src/reflow_render.c@internal_blit_glyph), driven through the
 * real reflow_render_page() API. A single laid-out chapter contains both
...
{
  TEST_BEGIN("reflow_render: real glyph blit (w>0 && h>0 both arms)");
  if (internal_load_font() == 0) {
    ...
    TEST_END("reflow_render: real glyph blit (w>0 && h>0 both arms)");
    return;
  }
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_layout_chapter(&s_engine, (const uint8_t*)xhtml, strlen(xhtml), &pages));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, reflow_render_page(&s_engine, 0U, nullptr));
  TEST_ASSERT(internal_fb_has_drawn_pixel());

  /* Out-of-range page is rejected without touching the framebuffer. */
  TEST_ASSERT_EQ(k_ra8_err_out_of_range, reflow_render_page(&s_engine, pages, nullptr));
  TEST_END("reflow_render: real glyph blit (w>0 && h>0 both arms)");
}
 ...
 * (2 conditions, OR; apps/shared_libs/reflow/src/reflow_render.c@internal_init_faces),
 * driven through the real reflow_render_page() API. N+1 = 3 vectors for the
 ...
 *
 * The public reflow_register_face() validates and rejects both bad blobs
...
{
  /* Invalid font magic, despite satisfying the minimum length. */
  static const uint8_t s_junk_face[64] = {1U, 2U, 3U, 4U, 5U, 6U, 7U, 8U};
  /* Valid sfnt magic with a broken directory isolates InitFont failure. */
  static const uint8_t s_badsfnt_face[64] = {0U, 1U};
  TEST_BEGIN("reflow_render: priv_init_faces bad/good face MC/DC");
  if (internal_load_font() == 0) {
    TEST_ASSERT_EQ(
      k_ra8_test_output_ok,
      internal_test_output_fd_text(STDERR_FILENO,
                                   "[SKIP] literata_latin1.ttf not found; skipping render\n"));
    TEST_END("reflow_render: priv_init_faces bad/good face MC/DC");
    return;
  }
  TEST_ASSERT_EQ(k_ra8_ok, internal_init_engine());
  ...
  TEST_ASSERT_EQ(k_ra8_ok, reflow_register_face(&s_engine, 0U, s_font, s_font_len));
  TEST_ASSERT_EQ(k_ra8_err_not_supported,
                 reflow_register_face(&s_engine, 1U, s_junk_face, sizeof s_junk_face));
  TEST_ASSERT_EQ(k_ra8_err_not_supported,
                 reflow_register_face(&s_engine, 2U, s_badsfnt_face, sizeof s_badsfnt_face));
...
RA8_INTERNAL static void internal_reflow_render_drawn(uint32_t page)
{
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    ra8_gfx_init(s_fb, (uint16_t)k_r_vp_w, (uint16_t)k_r_vp_h, k_ra8_gfx_format_rgb888));
  TEST_ASSERT_EQ(k_ra8_ok, reflow_render_page(&s_engine, page, nullptr));
  TEST_ASSERT(internal_fb_has_drawn_pixel());
}
...
RA8_INTERNAL static void internal_bind_glyph_atlas(uint32_t cell_bytes)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, reflow_set_glyph_atlas(&s_engine, &s_atlas, &storage));
}
 ...
 * (2 conditions, OR; apps/shared_libs/reflow/src/reflow_render.c@internal_render_images),
 * driven through the real reflow_render_page() API. N+1 = 3 vectors:
...
RA8_INTERNAL static void internal_test_render_images_loader_both_arms(void)
{
  ...
  TEST_BEGIN("reflow_render: priv_render_images loader/arena MC/DC");
  if (internal_load_font() == 0) {
    TEST_ASSERT_EQ(
      k_ra8_test_output_ok,
      internal_test_output_fd_text(STDERR_FILENO,
                                   "[SKIP] literata_latin1.ttf not found; skipping render\n"));
    TEST_END("reflow_render: priv_render_images loader/arena MC/DC");
    return;
  }
  ...
  ra8_img_arena_t arena = {.base   = s_arena_buf,
                           ...
                           .live   = 0U};
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_set_image_loader(&s_engine, internal_png_loader, nullptr, &arena));
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 reflow_layout_chapter(&s_engine, (const uint8_t*)text, strlen(text), &pages));
  TEST_ASSERT(pages >= 1U);
 ...
 * plus reflow_set_glyph_atlas bind + detach (atlas == NULL) paths -- source
 * the no-cache MC/DC tests above cannot reach.
 *
 * @par MC/DC:
 * Decision: `(storage->cell_bytes == 0U) || (storage->cell_count == 0U) ||
 * (storage->bucket_count == 0U)` in reflow_set_glyph_atlas
 * (apps/shared_libs/reflow/src/reflow_render.c), evaluated here only at its all-false
...
{
  TEST_BEGIN("reflow_render: glyph atlas byte-identity + zero re-raster");
  if (internal_load_font() == 0) {
    TEST_ASSERT_EQ(k_ra8_test_output_ok,
                   internal_test_output_fd_text(
                     STDERR_FILENO,
                     "[SKIP] literata_latin1.ttf not found; skipping atlas render\n"));
    TEST_END("reflow_render: glyph atlas byte-identity + zero re-raster");
    return;
  }
  TEST_ASSERT_EQ(k_ra8_ok, internal_init_engine());
  uint32_t    pages = 0U;
  ...
  (void)memset(s_fb_ref, 0, sizeof s_fb_ref);
  TEST_ASSERT_EQ(
    k_ra8_ok,
  ...
  TEST_ASSERT_EQ(k_ra8_ok, reflow_set_glyph_atlas(&s_engine, nullptr, nullptr));
  internal_atlas_render_and_check();
  TEST_END("reflow_render: glyph atlas byte-identity + zero re-raster");
}
 ...
 * apps/shared_libs/reflow/src/reflow_render.c@reflow_set_glyph_atlas). N+1 = 4
 * vectors:
...
{
  TEST_BEGIN("reflow_set_glyph_atlas storage-zero MC/DC");
  if (internal_load_font() == 0) {
    TEST_ASSERT_EQ(k_ra8_test_output_ok,
                   internal_test_output_fd_text(
                     STDERR_FILENO,
                     "[SKIP] literata_latin1.ttf not found; skipping atlas storage MC/DC\n"));
    TEST_END("reflow_set_glyph_atlas storage-zero MC/DC");
    return;
  }
  TEST_ASSERT_EQ(k_ra8_ok, internal_init_engine());

  reflow_glyph_atlas_storage_t st = {
    .cell_mem     = s_atlas_cells,
  ...
  st.cell_bytes = 0U;
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, reflow_set_glyph_atlas(&s_engine, &s_atlas, &st));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, reflow_set_glyph_atlas(&s_engine, &s_atlas, &st));

  /* Null storage with a non-null atlas is rejected (the storage null guard). */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, reflow_set_glyph_atlas(&s_engine, &s_atlas, nullptr));
...
int main(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_test_output_ok,
                 internal_test_output_fd_text(STDERR_FILENO, "[OK ] test_reflow_render.c\n"));
  return 0;
}
