/**
 * @file reflow_v2.cpp
 * @brief LiteHTML-backed reflow engine -- reflow public API adapter.
 *
 * @par Tag
 * [Ring 4 / Reflow] {World: NS}
 *
 * @details
 * Drop-in replacement for ``apps/shared_libs/reflow/src/reflow_*.c``. Built
 * only when the consumer selects ``-DREFLOW_USE_LITEHTML=ON``;
...
public:
                                 ...
                                 litehtml::font_metrics* fm) override
  {
    if (fm != nullptr) {
      ...
      fm->ascent    = (static_cast<litehtml::pixel_t>(font_px_) *
                       static_cast<litehtml::pixel_t>(k_v2_ascent_num)) /
                      static_cast<litehtml::pixel_t>(k_v2_ascent_den);
      fm->descent =
        static_cast<litehtml::pixel_t>(font_px_) / static_cast<litehtml::pixel_t>(k_v2_descent_den);
      fm->x_height =
        static_cast<litehtml::pixel_t>(font_px_) / static_cast<litehtml::pixel_t>(k_v2_xheight_den);
...
struct v2_cache_slot {
  const reflow_t* engine = nullptr; /**< Stable engine identity, or nullptr for an unused slot. */
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static v2_state* internal_v2_state_for(const reflow_t* engine, bool create)
{
 ...
 * @post No state outside the documented outputs is modified by this helper.
 */
RA8_INTERNAL static void internal_v2_state_drop(const reflow_t* engine)
{
  for (uint8_t i = 0; i < (uint8_t)k_v2_max_engines; ++i) {
    if (s_v2_cache[i].engine == engine) {
      internal_v2_state_clear(s_v2_cache[i].state);
 ...
 * @post No state outside the documented outputs is modified by this helper.
 */
RA8_INTERNAL static ra8_err_t internal_check_engine(const reflow_t* engine)
{
  if (engine == nullptr) {
 ...
 * @post No state outside the documented outputs is modified by this helper.
 */
RA8_INTERNAL static ra8_err_t internal_v2_build_document(v2_state* state, const reflow_t* engine)
{
 ...
 * @post No state outside the documented outputs is modified by this helper.
 */
RA8_INTERNAL static void internal_v2_paginate(reflow_t* engine, int content_height)
{
  ...
  if (pages > static_cast<int>(k_reflow_max_pages)) {
    pages = static_cast<int>(k_reflow_max_pages);
  }
 ...
 * @post No state outside the documented outputs is modified by this helper.
 */
RA8_INTERNAL static ra8_err_t internal_v2_run_layout(reflow_t* engine, uint32_t* out_total_pages)
{
                                    ...
                                    reflow_t*      out_engine)
{
  ...
  if ((viewport_w == 0U) || (viewport_h == 0U) || (font_px < (uint16_t)k_reflow_min_font_px) ||
      (font_px > (uint16_t)k_reflow_max_font_px)) {
...
[[nodiscard]] ra8_err_t reflow_set_font_size(reflow_t* engine, uint16_t new_font_px)
{
  ...
  if ((new_font_px < (uint16_t)k_reflow_min_font_px) ||
      (new_font_px > (uint16_t)k_reflow_max_font_px)) {
    return k_ra8_err_invalid_arg;
  }
