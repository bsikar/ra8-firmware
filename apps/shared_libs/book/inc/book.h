 ...
 *
 * @details
 * `book` is the on-device representation of a book that has already been
 ...
 * inflated alone, without touching the rest of the file. `book_open()`
 * inflates every chunk once into a caller-provided SDRAM scratch buffer (the
 * resident fast path); `book_chunked.h` instead inflates single chunks on
 ...
 * `const book_header_t*` at inflated bytes and walks them with the inline
 ...
 *       blob with book_validate() (or open it with book_open()) before
 ...
 * @see reflow.h         Renderer that consumes the walked DOM.
 *
...
typedef enum : uint32_t {
  k_book_format_version = 1U, /**< Current `.rabook` layout revision. */
} book_version_t;

/**
 * @enum book_container_t
 ...
 *          flat blob described by @ref book_header_t, so any chunk can be
 *          inflated independently -- the property the ra8_vmem paged read path
 *          (book_chunked.h) relies on. Producers size `chunk_bytes` equal
 *          to the reader's ra8_vmem frame size so one chunk fills exactly one
 *          cache frame. @ref book_open() inflates all chunks (resident).
 * @since Version 0.1.0
 */
typedef enum : uint8_t {
  k_book_container_magic_len  = 4U,  /**< Length of the "RBKC" magic.                  */
  k_book_container_header_len = 24U, /**< Fixed header bytes ahead of the chunk table. */
  k_book_container_entry_len  = 8U,  /**< One chunk-table entry (uint64 LE offset).    */
} book_container_t;

/**
 * @enum book_sentinel_t
 ...
 */
typedef enum : uint32_t {
  k_book_nil = 0xFFFFFFFFU, /**< Absent index / "applies to all chapters". */
} book_sentinel_t;

/**
 * @enum book_flag_t
 * @brief Feature-flag bits carried in `book_header_t.flags`.
 ...
 *          pool lives. book_validate() rejects a blob whose `flags` sets any
 *          bit outside ::k_book_flag_mask_known, so an old firmware never
 *          silently mis-renders a book that depends on a semantic it does not
 *          implement. Kept in lockstep with `FLAG_RTL` in
 *          `tools/epub_compile/src/epub_compile.py` (emitted by the CBZ arm,
 *          `tools/epub_compile/src/cbz_compile.py`, under `--rtl`).
 * @invariant ::k_book_flag_mask_known is the OR of every other enumerator.
 * @see book_is_rtl()
 * @since Version 0.1.0
 */
typedef enum : uint32_t {
  k_book_flag_rtl        = 0x00000001U, /**< Right-to-left reading order (manga):
                                            *   spine pages advance leaf-first, and the
                                            *   reader mirrors its page-turn zones.     */
  k_book_flag_mask_known = 0x00000001U, /**< Every bit this firmware understands; a
                                            *   set bit outside this mask fails
                                            *   book_validate().                     */
} book_flag_t;

/**
 * @enum book_node_kind_t
 * @brief Discriminates the two kinds of DOM node.
 * @since Version 0.1.0
 */
typedef enum : uint8_t {
  k_book_node_element = 0U, /**< An element: has a tag name and attributes. */
  k_book_node_text    = 1U, /**< A text run: carries a string, no children. */
} book_node_kind_t;

/**
 * @enum book_image_format_t
 ...
 */
typedef enum : uint8_t {
  k_book_image_gray4 = 0U, /**< 4bpp gray, 2px/byte; pixel (x,y) is at flat index y*width + x. */
  k_book_image_svg   = 1U, /**< Verbatim UTF-8 SVG source (vector; on-device rasterized).      */
} book_image_format_t;

/**
 * @enum book_image_pixfmt_t
 * @brief Pixel depth of a @ref k_book_image_gray4 raster in the image pool.
 * @details A second axis, orthogonal to @ref book_image_format_t: that field
 ...
 *          (@ref book_image_t::pixel_format), so it costs no format growth and
 *          needs no @ref book_version_t bump: every `.rabook` written before
 *          this field existed zero-filled that byte, and 0 is
 *          @ref k_book_pixfmt_gray4 -- exactly the 4bpp packing those blobs
 ...
 *          book_validate() rather than silently mis-rendered.
 * @invariant An @ref k_book_image_svg entry stores 0 here (the field is
 ...
 *       book_image_pixfmt(img) == k_book_pixfmt_gray8) {
 *     ra8_gfx_blit_gray8(book_image_data(base, img), img->width, img->height, x, y);
 *   }
 * @endcode
 * @see book_image_pixfmt()
 * @see book_image_t
 * @since Version 0.1.0
 */
typedef enum : uint8_t {
  k_book_pixfmt_gray4 = 0U, /**< 4bpp packed grayscale, 2px/byte (default; every pre-field blob). */
  /** 8bpp grayscale, 1px/byte (lossless against any grey panel). */
  k_book_pixfmt_gray8 = 1U,
} book_image_pixfmt_t;

/**
 * @enum book_struct_size_t
...
typedef enum : uint16_t {
  k_book_sizeof_header     = 100U, /**< Bytes in @ref book_header_t.     */
  k_book_sizeof_chapter    = 12U,  /**< Bytes in @ref book_chapter_t.    */
  k_book_sizeof_node       = 24U,  /**< Bytes in @ref book_node_t.       */
  k_book_sizeof_attr       = 8U,   /**< Bytes in @ref book_attr_t.       */
  k_book_sizeof_stylesheet = 8U,   /**< Bytes in @ref book_stylesheet_t. */
  k_book_sizeof_image      = 24U,  /**< Bytes in @ref book_image_t.      */
} book_struct_size_t;
...
typedef struct {
  ...
  uint32_t format_version;    /**< @ref book_version_t of this blob.                          */
  uint32_t total_size;        /**< Total blob length in bytes.                                */
  uint32_t flags;             /**< @ref book_flag_t bits; unknown bits are rejected.          */
  ...
  uint32_t crc32_val;         /**< CRC-32/ISO-HDLC of the body (all bytes after this header). */
} book_header_t;

static_assert(sizeof(book_header_t) == k_book_sizeof_header, "book_header_t size pinned");
...
static_assert(sizeof(book_chapter_t) == k_book_sizeof_chapter, "book_chapter_t size pinned");
 ...
 *          each node's `next_sibling` until @ref k_book_nil. Element nodes
 *          set `name_off` (tag name) and may have attributes; text nodes set
 *          `text_off` and have neither attributes nor children.
 * @invariant `kind == k_book_node_text` implies `attr_count == 0`,
 *            `first_attr == nil` and `first_child == nil`.
 * @since Version 0.1.0
 */
typedef struct {
  uint8_t  kind;         /**< @ref book_node_kind_t.                             */
...
static_assert(sizeof(book_node_t) == k_book_sizeof_node, "book_node_t size pinned");
...
static_assert(sizeof(book_attr_t) == k_book_sizeof_attr, "book_attr_t size pinned");
 ...
 *          index, or @ref k_book_nil for a book-wide stylesheet.
 * @since Version 0.1.0
 */
typedef struct {
...
static_assert(sizeof(book_stylesheet_t) == k_book_sizeof_stylesheet,
              "book_stylesheet_t size pinned");
 ...
 *          @ref book_container_t) and inflated on open, so per-image
...
typedef struct {
  ...
  uint8_t  format;       /**< @ref book_image_format_t.                            */
  uint8_t  pixel_format; /**< @ref book_image_pixfmt_t (0 == gray4; SVG: 0).       */
...
static_assert(sizeof(book_image_t) == k_book_sizeof_image, "book_image_t size pinned");

/* -------------------------------------------------------------------------- */
/* Inline accessors. All take a validated blob base; see book_validate(). */
/* -------------------------------------------------------------------------- */
 ...
 * @pre `base` points at a blob already accepted by book_validate().
 ...
 * @since Version 0.1.0
 */
static inline const book_header_t* book_header(const void* base)
{
  return (const book_header_t*)base;
}
 ...
 *
 * @details
 * Reads ::k_book_flag_rtl out of the header's `flags` word. An RTL book's
 ...
 * @since Version 0.1.0
 */
static inline bool book_is_rtl(const void* base)
{
  return (book_header(base)->flags & (uint32_t)k_book_flag_rtl) != 0U;
}
 ...
 * @since Version 0.1.0
 */
static inline const void* book_at(const void* base, uint32_t off)
{
 ...
 * @since Version 0.1.0
 */
static inline const char* book_string(const void* base, uint32_t off)
{
  return (const char*)book_at(base, book_header(base)->string_off + off);
}

/**
 * @brief Base of the chapter table.
 * @param[in] base Blob base (non-NULL, validated).
 * @return Pointer to chapter[0]; read `book_header(base)->chapter_count` entries.
 * @pre `base` validated.
 * @pre `book_header(base)->chapter_count > 0` for the result to be dereferenceable.
 * @post Result indexes a contiguous array of `chapter_count` entries.
 * @note Thread-safe: read-only.
 * @since Version 0.1.0
 */
static inline const book_chapter_t* book_chapters(const void* base)
{
  return (const book_chapter_t*)book_at(base, book_header(base)->chapter_off);
}
 ...
 * @pre `base` validated.
 * @pre `book_header(base)->node_count > 0`.
 * @post Result indexes a contiguous array of `node_count` entries.
 * @note Thread-safe: read-only.
 * @since Version 0.1.0
 */
static inline const book_node_t* book_nodes(const void* base)
{
  return (const book_node_t*)book_at(base, book_header(base)->node_off);
}
 ...
 * @pre `base` validated.
 * @pre `book_header(base)->attr_count > 0`.
 * @post Result indexes a contiguous array of `attr_count` entries.
 * @note Thread-safe: read-only.
 * @since Version 0.1.0
 */
static inline const book_attr_t* book_attrs(const void* base)
{
  return (const book_attr_t*)book_at(base, book_header(base)->attr_off);
}
 ...
 * @pre `base` validated.
 * @pre `book_header(base)->stylesheet_count > 0`.
 * @post Result indexes a contiguous array of `stylesheet_count` entries.
 * @note Thread-safe: read-only.
 * @since Version 0.1.0
 */
static inline const book_stylesheet_t* book_stylesheets(const void* base)
{
  return (const book_stylesheet_t*)book_at(base, book_header(base)->stylesheet_off);
}
 ...
 * @pre `base` validated.
 * @pre `book_header(base)->image_count > 0`.
 * @post Result indexes a contiguous array of `image_count` entries.
 * @note Thread-safe: read-only.
 * @since Version 0.1.0
 */
static inline const book_image_t* book_images(const void* base)
{
  return (const book_image_t*)book_at(base, book_header(base)->image_off);
}

/**
 * @brief Tag name of an element node.
 * @param[in] base Blob base (non-NULL, validated).
 * @param[in] node Element node (non-NULL, `kind == k_book_node_element`).
 ...
 * @since Version 0.1.0
 */
static inline const char* book_node_name(const void* base, const book_node_t* node)
{
  return book_string(base, node->name_off);
}

/**
 * @brief Text of a text node.
 * @param[in] base Blob base (non-NULL, validated).
 * @param[in] node Text node (non-NULL, `kind == k_book_node_text`).
 ...
 * @since Version 0.1.0
 */
static inline const char* book_node_text(const void* base, const book_node_t* node)
{
  return book_string(base, node->text_off);
}
 ...
 * @since Version 0.1.0
 */
static inline const uint8_t* book_image_data(const void* base, const book_image_t* img)
{
  return &((const uint8_t*)book_at(base, book_header(base)->image_pool_off))[img->data_off];
}
 ...
 *
 * @details
 * Decodes @ref book_image_t::pixel_format into its @ref book_image_pixfmt_t.
 * Meaningful only for a @ref k_book_image_gray4 raster (4bpp vs 8bpp packing);
 * an SVG entry stores 0 and reports @ref k_book_pixfmt_gray4, so a renderer
 * must branch on @ref book_image_t::format first and only consult this for a
 * raster. Every blob written before the field existed zero-filled the byte, so
 * an old gray4 blob reports @ref k_book_pixfmt_gray4 unchanged (backward-read).
 *
 * @param[in] img Image descriptor (non-NULL) obtained from book_images().
 *
 * @return The declared pixel depth.
 * @retval k_book_pixfmt_gray4 4bpp packed grayscale (2px/byte); also every
 *                                 pre-field blob and every SVG entry.
 * @retval k_book_pixfmt_gray8 8bpp grayscale (1px/byte).
 *
 * @pre `img` belongs to a blob accepted by book_validate().
 * @pre `img` is non-NULL.
 * @post The blob is not modified (pure read).
 * @post The result is a depth book_validate() already accepted as known.
 *
 * @note Thread-safe: read-only over immutable data.
 * @see book_image_pixfmt_t
 * @see book_image_data()
 * @since Version 0.1.0
 */
static inline book_image_pixfmt_t book_image_pixfmt(const book_image_t* img)
{
  return (book_image_pixfmt_t)img->pixel_format;
}
 ...
 * outside ::k_book_flag_mask_known (a blob relying on a presentation
 ...
 * @ref book_image_pixfmt_t this build can unpack (an unknown depth is refused
...
/**
 * @typedef book_inflate_fn
 * @brief Caller-supplied DEFLATE inflater used by @ref book_open().
 * @details Keeps `book` decoupled from any one decompressor: the firmware
 ...
 */
typedef ra8_err_t (
  *book_inflate_fn)(const void* src, size_t src_len, void* dst, size_t dst_cap, size_t* out_len);
 ...
 * @ref book_container_t for the layout), inflates every chunk's zlib stream
 * in order into the caller-owned `scratch` buffer (expected to live in SDRAM),
 * then runs book_validate() over the reassembled flat blob. On success
 ...
 * chunk-by-chunk through book_chunked.h + book_src_paged().
 *
 * @param[in]  file        Pointer to the `.rabook` file bytes (non-NULL).
 * @param[in]  file_len    Length of `file` in bytes.
 * @param[in]  inflate     Decompressor callback (see @ref book_inflate_fn).
 ...
 * @retval k_ra8_err_range_check_failed Blob CRC mismatch (from book_validate()).
 *
 ...
 *
 * @details
 * A bridge for feeding a compiled book into `reflow_layout_chapter()`, which
 ...
 * NUL-terminated.
 *
 * @param[in]  base        Validated `book` blob base (non-NULL).
 ...
 *
 * @pre @p base was accepted by book_validate() / book_open().
 ...
 *
 * @pre @p base was accepted by book_validate() / book_open().
 * @pre @p chapter_idx is less than `book_header(base)->chapter_count`.
