 ...
 *
 * ::book_chunked_read has exactly the ::ra8_vsource_read_fn signature, so a
 ...
 *
 * @see book.h        Container layout + the resident open.
 * @see book_paged.h  Binds the paged object as a book source.
 * @see ra8_vsource.h     The registry ::book_chunked_read plugs into.
 *
 ...
 *
 * @details Populated by ::book_chunked_open; treat every field as read-only
 *          afterwards. `file_read` addresses the *container file bytes* (the
 *          compressed on-disk form); ::book_chunked_read then serves the
 ...
 * @p staging_cap. On success @p rd is bound and ::book_chunked_read may
 ...
 * @post On k_ra8_ok, @p rd is bound and ::book_chunked_read serves
 *       `[0, rd->inflated_total)` of the flat blob.
 * @post On any error @p rd is left unbound (its `table` stays NULL).
 *
 * @note Not thread-safe.
 * @see book_chunked_read()
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t book_chunked_open(book_chunked_t*     rd,
 ...
 *
 * @param[in]  ctx    The bound ::book_chunked_t (as a void cookie).
 ...
 *
 * @pre @p ctx was populated by ::book_chunked_open.
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t book_chunked_read(void* ctx, uint64_t offset, uint8_t* buf, uint32_t len);

/**
 * @brief Strictly validate the complete flat blob behind an open RBKC reader.
 *
 * @details Adapts the chunk-aligned @ref book_chunked_read interface to the
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t book_chunked_validate_strict(book_chunked_t* rd,
                                                     uint8_t*        chunk,
                                                     uint32_t        chunk_cap,
                                                     uint8_t*        scratch,
