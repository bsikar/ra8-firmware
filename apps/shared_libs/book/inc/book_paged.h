 ...
 *
 * An @ref book_src_t is the seam: it wraps *either* a resident blob base
 * (zero-copy, identical to book.h) *or* an @ref ra8_vmem cache fronting an
 * @ref ra8_vsource object (paged). Reads flow through ::book_src_read in both
 * modes, and the DOM walkers (::book_chapter_text_src) dispatch on the mode,
 * so the reader uses one API and selects paged only when a book exceeds budget.
 * The resident path is byte-for-byte identical to book_chapter_text(), so the
 * ereader golden render stays unchanged for RAM-fitting books.
 *
 * @note Including this header pulls in ra8_vmem; the resident book.h API has
 *       no such dependency. Only consumers that page books need this file.
 *
 * @see book.h        Resident (XIP) accessors + the on-disk format.
 ...
 *          to the book.h accessors. Paged mode sets @ref vm (an initialised
 ...
 *          Populate via ::book_src_resident / ::book_src_paged; treat the
 *          fields as read-only afterwards.
 *
 * @invariant Exactly one of (`base != NULL`) and (`vm != NULL`) holds.
 * @invariant `hdr` is a copy of the blob's ::book_header_t.
 *
 * @see book_src_resident()
 * @see book_src_paged()
 * @since Version 0.1.0
 */
typedef struct {
  ...
  book_header_t  hdr;         /**< Resident copy of the blob header.             */
} book_src_t;

/**
 * @brief Bind a resident (already-inflated, fully resident) book as a source.
 *
 * @details The zero-copy fast path: @ref book_src_t::base is set to @p base
 *          and reads become `base + off`, byte-for-byte identical to the
 *          book.h accessors. The header is copied into @ref book_src_t::hdr.
 *
 * @param[out] out  Source to populate (caller-owned).
 * @param[in]  base Validated `.rabook` blob base (non-NULL), as from book_open().
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t book_src_resident(book_src_t* out, const void* base, uint32_t size);
 ...
 *          @ref book_src_t::hdr so header fields never fault thereafter.
 *
 * @param[out] out         Source to populate (caller-owned).
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t book_src_paged(book_src_t* out,
                                       ra8_vmem_t* vm,
                                       uint32_t    object_id,
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t book_src_read(const book_src_t* src, uint32_t off, void* dst, uint32_t len);

/**
 * @brief Read one image descriptor out of a book source (resident or paged).
 *
 * @details The source-aware counterpart of the resident-only book_images()
 *          accessor: resolves image @p idx to its blob offset
 *          (`hdr.image_off + idx * sizeof(book_image_t)`) and copies the
 *          24-byte descriptor out through ::book_src_read, so a paged book
 ...
 *          ::book_src_image_rect to read pixels. This is the library-owned
 ...
 *
 * @pre  @p src was populated by book_src_resident() / book_src_paged().
 * @pre  @p out_img addresses writable storage for one ::book_image_t.
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t book_src_image(const book_src_t* src, uint32_t idx, book_image_t* out_img);
 ...
 *          depths of a @ref k_book_image_gray4 entry are served, selected by the
 *          descriptor's @ref book_image_pixfmt_t, and both yield one gray8 byte
 *          per output pixel so the renderer sees a single depth:
 *          - @ref k_book_pixfmt_gray4: 2 pixels per byte, pixel `(px,py)` at flat
 ...
 *          width (::book_src_read itself faults a paged source frame-by-frame),
 ...
 *
 * @param[in]  src        Bound book source.
 * @param[in]  img        Image descriptor from ::book_src_image (@c format must
 *                        be @ref k_book_image_gray4; @c pixel_format selects the
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t book_src_image_rect(const book_src_t*   src,
 ...
 *
 * @details The source-aware counterpart of book_chapter_text(): same output
 ...
 *          delegates to book_chapter_text() and is byte-for-byte identical;
 ...
 *
 * @note Not thread-safe.
 * @see book_chapter_text()
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t book_chapter_text_src(const book_src_t* src,
 ...
 *          range ::book_chapter_text_src reads for that chapter -- and hands it
 ...
 *
 * @pre  @p src was populated by book_src_paged() (paged mode, `src->vm != NULL`).
 ...
 * @see book_chapter_text_src()  The demand read this warms ahead of.
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t book_src_prefetch_chapter(const book_src_t* src, uint32_t chapter_idx);
