...
typedef struct {
  book_chunked_t* rd;         /**< Open compressed reader.          */
...
{
  uintptr_t a_begin = 0U;
  uintptr_t b_begin = 0U;
  static_assert(sizeof(a_begin) == sizeof(a), "uintptr_t must preserve pointer representation");
  static_assert(sizeof(b_begin) == sizeof(b), "uintptr_t must preserve pointer representation");
  (void)memcpy((void*)&a_begin, (const void*)&a, sizeof(a_begin));
  (void)memcpy((void*)&b_begin, (const void*)&b, sizeof(b_begin));
  *invalid = true;
 ...
 */
RA8_INTERNAL
static bool internal_output_is_aliased(const book_chunked_t* rd,
                                       const uint8_t*        chunk,
                                       uint32_t              chunk_cap,
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_validate_workspaces(const book_chunked_t* rd,
                                              const uint8_t*        chunk,
                                              uint32_t              chunk_len,
                                              const uint8_t*        scratch,
                                              uint32_t              scratch_cap)
...
static ra8_err_t internal_load_chunk(chunk_validate_t* ctx, uint32_t idx)
{
  ...
  const ra8_err_t err = book_chunked_read(ctx->rd, offset, ctx->chunk, span);
  if (err == k_ra8_ok) {
    ctx->loaded     = true;
    ctx->loaded_idx = idx;
    ctx->loaded_len = span;
...
static ra8_err_t internal_chunk_flat_read(void* opaque, uint64_t offset, uint8_t* dst, uint32_t len)
{
  ...
  if ((cursor > ctx->rd->inflated_total) || ((uint64_t)len > (ctx->rd->inflated_total - cursor))) {
    return k_ra8_err_out_of_range;
  }
  uint32_t copied = 0U;
  while (copied < len) {
    const uint32_t idx = (uint32_t)(cursor / (uint64_t)ctx->rd->chunk_bytes);
    ra8_err_t      err = internal_load_chunk(ctx, idx);
    if (err != k_ra8_ok) {
      return err;
    }
    const uint32_t in_chunk = (uint32_t)(cursor % (uint64_t)ctx->rd->chunk_bytes);
                                       ...
                                       book_header_t*  out_header)
{
  ...
  *out_header = (book_header_t){};
  ...
  err                     = book_validate_stream_strict(internal_chunk_flat_read,
                                                        &ctx,
                                                        rd->inflated_total,
                                                        scratch,
                                                        scratch_cap,
