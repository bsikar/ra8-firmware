 ...
 * and copy the requested slice out). ::book_src_read is the single primitive
...
typedef enum : uint32_t {
  k_book_paged_span_pad  = 2U, /**< Extra frame iterations over the exact span.  */
  k_book_paged_min_frame = 2U, /**< Smallest accepted frame size (Rule-2 bound). */
} book_paged_bound_t;
...
ra8_err_t book_src_resident(book_src_t* out, const void* base, uint32_t size)
{
  ...
  out->frame_bytes = 0U;
  out->size        = size;
  (void)memcpy(&out->hdr, base, sizeof(book_header_t));
  return k_ra8_ok;
}
                         ...
                         uint32_t    size)
{
  ...
  if ((size == 0U) || (frame_bytes < (uint32_t)k_book_paged_min_frame)) {
    return k_ra8_err_invalid_size;
  }
  out->base        = nullptr;
  out->vm          = vm;
  ...
  return book_src_read(out, 0U, &out->hdr, (uint32_t)sizeof(book_header_t));
}

/**
 * @brief Copy a byte range out of a paged source, frame by frame.
 *
 * @details The demand-fetch worker behind ::book_src_read: walks @p len bytes
...
RA8_INTERNAL
static ra8_err_t
internal_book_src_read_paged(const book_src_t* src, uint32_t off, void* dst, uint32_t len)
{
  uint8_t*       d         = (uint8_t*)dst;
  uint32_t       cur       = off;
  uint32_t       remaining = len;
  const uint32_t max_iter  = (len / src->frame_bytes) + (uint32_t)k_book_paged_span_pad;
 ...
 * book_src_image_rect() for "read me this rectangle" as unpacked gray8.
 * =========================================================================== */

/**
 * @enum book_image_rect_const_t
 * @brief 4bpp nibble packing + the per-read pixel budget (no magic numbers).
 * @details A @ref k_book_image_gray4 pool byte holds two pixels; the high
 ...
 *          (`(nib << 4) | nib`). ::book_src_image_rect unpacks each row in
 *          spans of at most @ref k_book_rect_chunk_px pixels so the staging
 *          buffer -- and every ::book_src_read -- stays small and bounded.
 * @since Version 0.1.0
 */
typedef enum : uint32_t {
  k_book_gray4_nib_lo   = 0x0FU, /**< Low-nibble mask (odd flat index).                        */
  k_book_gray4_nib_sh   = 4U,    /**< Nibble shift and 4->8 bit replicate amount.              */
  k_book_gray4_ppb      = 2U,    /**< Pixels packed per pool byte.                             */
  k_book_rect_chunk_px  = 512U,  /**< Max pixels unpacked per bounded span read.               */
  k_book_rect_chunk_pad = 2U,    /**< Loop-bound headroom over ceil(w / k_book_rect_chunk_px). */
} book_image_rect_const_t;

/**
 * @brief Unpack one horizontal gray4 run `[x, x+w)` of source row @p ry to gray8.
 *
 * @details The per-row worker of ::book_src_image_rect. Walks the run in
 *          spans of at most ::k_book_rect_chunk_px pixels: each span's flat
 *          nibble index maps to an absolute pool offset
 *          (`image_pool_off + data_off + (flat >> 1)`), the packed span is copied
 *          out via ::book_src_read (bounds-checked, paged-fault-aware), and
 ...
 * @retval k_ra8_err_*            A ::book_src_read fault (returned verbatim).
 *
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_book_image_row(const book_src_t*   src,
                                                      ...
                                                      uint8_t*            out)
{
  ...
  uint8_t        packed[(k_book_rect_chunk_px / k_book_gray4_ppb) + 1U];
  uint32_t       done     = 0U;
  const uint32_t max_iter = (w / (uint32_t)k_book_rect_chunk_px) + (uint32_t)k_book_rect_chunk_pad;
  for (uint32_t it = 0U; it < max_iter; ++it) {
    ...
    const uint32_t cpx =
      (remain < (uint32_t)k_book_rect_chunk_px) ? remain : (uint32_t)k_book_rect_chunk_px;
    ...
    const ra8_err_t re = book_src_read(src, (uint32_t)off, packed, n);
    if (re != k_ra8_ok) {
      return re;
    }
    for (uint32_t i = 0U; i < cpx; ++i) {
      ...
      const uint8_t  nib  = ((flat & 1U) != 0U) ? (uint8_t)(byte & (uint8_t)k_book_gray4_nib_lo)
                                                : (uint8_t)(byte >> (uint8_t)k_book_gray4_nib_sh);
      out[done + i]       = (uint8_t)((nib << (uint8_t)k_book_gray4_nib_sh) | nib);
    }
    done += cpx;
  }
 ...
 *          straight into @p out via ::book_src_read -- which is itself bounded
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_book_image_row_gray8(const book_src_t*   src,
                                                            const book_image_t* img,
                                                            uint32_t            x,
                                                            uint32_t            ry,
                                                            uint32_t            w,
                                                            uint8_t*            out)
{
  ...
  if ((off + (uint64_t)w) <= (uint64_t)src->size) {
    re = book_src_read(src, (uint32_t)off, out, w);
  }
  return re;
}
...
ra8_err_t book_src_image(const book_src_t* src, uint32_t idx, book_image_t* out_img)
{
  ...
  const uint32_t off = src->hdr.image_off + (idx * (uint32_t)sizeof(book_image_t));
  return book_src_read(src, off, out_img, (uint32_t)sizeof(book_image_t));
}
                              ...
                              uint32_t            out_stride)
{
  ...
  const bool is_gray8 = (pf == k_book_pixfmt_gray8);
...
/**
 * @enum book_paged_walk_bound_t
 * @brief Staging sizes and loop bounds for the paged text walk (no magic numbers).
 * @since Version 0.1.0
 */
typedef enum : uint32_t {
  k_book_paged_strbuf = 256U, /**< Staging chunk for a paged text run.       */
  k_book_paged_tagbuf = 64U,  /**< Staging for one (short) element tag name. */
  k_book_paged_pad    = 2U,   /**< Loop-bound headroom over the exact span.  */
 ...
 *          (`node_off + idx * sizeof(book_node_t)`) and copies the 24-byte
 *          record out via ::book_src_read, which bounds-checks the range and
...
internal_paged_node(const book_src_t* src, uint32_t idx, book_node_t* out)
{
  const uint32_t off = src->hdr.node_off + (idx * (uint32_t)sizeof(book_node_t));
  return book_src_read(src, off, out, (uint32_t)sizeof(book_node_t));
}
...
RA8_INTERNAL
static ra8_err_t
internal_paged_str_short(const book_src_t* src, uint32_t abs_off, char* buf, uint32_t cap)
{
  if (abs_off >= src->size) {
    return k_ra8_err_out_of_range;
  }
  ...
  const ra8_err_t re     = book_src_read(src, abs_off, buf, chunk);
  if (re != k_ra8_ok) {
    return re;
  }
  uint32_t nlen = 0U;
 ...
 *          in `k_book_paged_strbuf`-sized chunks, calling the shared
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static bool internal_paged_emit_run(const book_src_t* src,
                                                 uint32_t          abs_off,
                                                 char*             out,
                                                 size_t            cap,
                                                 size_t*           pos,
                                                 ...
                                                 ra8_err_t*        io_err)
{
  char           buf[k_book_paged_strbuf];
  uint32_t       off = abs_off;
  const uint32_t max_iter =
    (src->size / (uint32_t)(k_book_paged_strbuf - 1U)) + (uint32_t)k_book_paged_pad;
  for (uint32_t it = 0U; it < max_iter; ++it) {
    if (off >= src->size) {
      return true; /* Ran to blob end without a NUL (degenerate); treat as done. */
    }
    const uint32_t  remain = src->size - off;
    const uint32_t  want   = (uint32_t)(k_book_paged_strbuf - 1U);
    const uint32_t  chunk  = (remain < want) ? remain : want;
    const ra8_err_t re     = book_src_read(src, off, buf, chunk);
    if (re != k_ra8_ok) {
      *io_err = re;
      return false;
    }
 ...
 * @param[in,out] stack    Caller walk stack of length @c k_book_xhtml_stack.
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static bool internal_paged_visit_node(const book_src_t* src,
                                                   uint32_t          n,
                                                   char*             out,
                                                   size_t            cap,
                                                   size_t*           pos,
 ...
 *
 * @pre  @p src is a paged source bound by book_src_paged().
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static bool internal_walk_text_paged(const book_src_t* src,
                                                  uint32_t          root,
                                                  uint32_t          node_count,
                                                  char*             out,
                                                  size_t            cap,
                                                  ...
                                                  ra8_err_t*        io_err)
{
  ...
  static uint32_t s_paged_stack[k_book_xhtml_stack];
                                ...
                                size_t*           out_len)
{
  ...
  if (src->base != nullptr) {
    return book_chapter_text(src->base, chapter_idx, out, cap, out_len);
  }

  /* Paged source: read the chapter record, then walk the DOM frame-by-frame. */
  book_chapter_t  chap = {};
  const uint32_t  coff = src->hdr.chapter_off + (chapter_idx * (uint32_t)sizeof(book_chapter_t));
  const ra8_err_t ce   = book_src_read(src, coff, &chap, (uint32_t)sizeof(book_chapter_t));
  if (ce != k_ra8_ok) {
    return ce;
  }
  size_t    pos    = 0U;
...
ra8_err_t book_src_prefetch_chapter(const book_src_t* src, uint32_t chapter_idx)
{
   ...
   * book_chapter_text_src() reads for this chapter. Reading the record here
  ...
  const uint64_t noff =
    (uint64_t)src->hdr.node_off + ((uint64_t)chap.root_node * sizeof(book_node_t));
