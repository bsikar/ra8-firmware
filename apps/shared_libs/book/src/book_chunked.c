 ...
 * (book_internal.h), so the container format has exactly one in-firmware
 ...
 * ::book_chunked_read stages one compressed stream and inflates it into
 * the caller's frame.
 *
...
RA8_INTERNAL
static ra8_err_t
internal_table_check(const book_chunked_t* rd, const uint64_t* table_buf, uint64_t payload_len)
{
  if (table_buf[0] != 0U) {
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_load_table(book_chunked_t* rd,
                                                  uint64_t        file_len,
                                                  uint64_t*       table_buf,
                                                  uint32_t        table_cap_entries)
{
  ...
  if (file_len < ((uint64_t)k_book_container_header_len + table_bytes)) {
    return k_ra8_err_invalid_size;
  }
  const ra8_err_t err = rd->file_read(rd->file_ctx,
                                      (uint64_t)k_book_container_header_len,
                                      (uint8_t*)table_buf,
                                      (uint32_t)table_bytes);
  if (err != k_ra8_ok) {
    return err;
  }
  const uint64_t  payload_off = (uint64_t)k_book_container_header_len + table_bytes;
...
/**
 * @brief The argument-checked body of ::book_chunked_open.
 *
 * @details
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_chunked_open_body(book_chunked_t* rd,
                                                         uint64_t        file_len,
                                                         uint64_t*       table_buf,
                                                         uint32_t        table_cap_entries)
{
  if (file_len < (uint64_t)k_book_container_header_len) {
    return k_ra8_err_invalid_size;
  }
  uint8_t   hdr[k_book_container_header_len] = {};
                            ...
                            uint32_t            staging_cap)
{
  ...
  RA8_CHECK_NULL_PTR(file_read, s_tag_chunked, "open: null file_read");
  RA8_CHECK_NULL_PTR(inflate, s_tag_chunked, "open: null inflate");
 ...
 *
 * @details
 * The I/O tail of ::book_chunked_read after the geometry checks: reads
...
RA8_INTERNAL
static ra8_err_t
internal_stage_and_inflate(const book_chunked_t* rd, uint32_t idx, uint8_t* buf, uint32_t len)
{
  ...
  size_t produced = 0U;
  err             = rd->inflate_cb(rd->staging, (size_t)clen, buf, (size_t)len, &produced);
  if (err != k_ra8_ok) {
    return err;
  }
...
ra8_err_t book_chunked_read(void* ctx, uint64_t offset, uint8_t* buf, uint32_t len)
{
  ...
  const book_chunked_t* rd = (const book_chunked_t*)ctx;
  if (rd->table == nullptr) {
