...
typedef struct {
  ...
  book_node_t    nodes[k_bc_node_cap];  /**< DOM node table.                */
...
static_assert(offsetof(bc_container_t, offsets) == k_book_container_header_len,
              "bc_container_t chunk table must sit at the wire offset");
static_assert(offsetof(bc_container_t, blob) ==
                (k_book_container_header_len + (2U * k_book_container_entry_len)),
 ...
 *          tables fit inside `total_size`, so book_validate() accepts it.
 */
static void bc_build_blob(bc_book_t* b)
{
  ...
  b->nodes[0].kind         = (uint8_t)k_book_node_element;
  b->nodes[0].first_attr   = k_book_nil;
  b->nodes[0].first_child  = k_book_nil;
  b->nodes[0].next_sibling = k_book_nil;
  b->nodes[1].kind         = (uint8_t)k_book_node_text;
  ...
  uint32_t       body_len = sizeof(*b) - (uint32_t)sizeof(book_header_t);
  b->hdr.crc32_val        = bc_crc32(body, body_len);
}

/** @brief Wrap @p b in a valid single-chunk "RBKC" container. */
static void bc_build_container(bc_container_t* c, const bc_book_t* b)
{
  (void)memcpy(c->magic, "RBKC", k_bc_magic_len);
...
/**
 * @test test_book_validate_total_size_range
 * @brief The `total_size` bounds guard rejects too-small and too-large headers.
 *
 * @par Targeted code:
 * `book_validate`'s `(total < sizeof(header)) || (total > size)` invalid-size
 ...
 * apps/shared_libs/book/src/book.c@book_validate:
 ...
 */
static void test_book_validate_total_size_range(void)
{
  TEST_BEGIN("book_validate total_size out-of-range guard");
  bc_book_t b;

  bc_build_blob(&b);
  b.hdr.total_size = (uint32_t)sizeof(book_header_t) - 1U;
  ...
  TEST_END("book_validate total_size out-of-range guard");
}
...
/**
 * @test test_book_validate_table_extent
 ...
 * so the conjunction is false and book_validate returns invalid_size.
 *
 * @par MC/DC:
 * Decision: `(off <= total) && (end <= (uint64_t)total)` (2 conditions) in
 * apps/shared_libs/book/src/book.c@internal_table_fits:
 ...
 *   supplied by test_book.c@test_book_invalid; isolates C1 vs V1).
 ...
 */
static void test_book_validate_table_extent(void)
{
  TEST_BEGIN("book_validate table extent overflow guard");
  bc_book_t b;
  ...
  TEST_END("book_validate table extent overflow guard");
}

/**
 * @test test_book_open_null_guards
 * @brief Every required pointer argument is rejected by the entry-point guards.
 *
 * @par Targeted code:
 * The `book_open` entry and its five `RA8_CHECK_NULL_PTR` guards (file,
 ...
 */
static void test_book_open_null_guards(void)
{
  TEST_BEGIN("book_open null-argument guards");
  bc_book_t      b;
  bc_container_t c;
  bc_build_blob(&b);
  ...
  TEST_ASSERT_EQ(
    k_ra8_err_null_ptr,
    book_open(nullptr, sizeof(c), bc_inflate, scratch, sizeof(scratch), &out_base, &out_size));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 book_open(&c, sizeof(c), nullptr, scratch, sizeof(scratch), &out_base, &out_size));
  TEST_ASSERT_EQ(
    k_ra8_err_null_ptr,
    book_open(&c, sizeof(c), bc_inflate, nullptr, sizeof(scratch), &out_base, &out_size));
  TEST_ASSERT_EQ(
    k_ra8_err_null_ptr,
    book_open(&c, sizeof(c), bc_inflate, scratch, sizeof(scratch), nullptr, &out_size));
  TEST_ASSERT_EQ(
    k_ra8_err_null_ptr,
    book_open(&c, sizeof(c), bc_inflate, scratch, sizeof(scratch), &out_base, nullptr));

  TEST_END("book_open null-argument guards");
}

/**
 * @brief Open a container image and assert the expected header-guard result.
 * @param[in]  want    Expected `book_open` result code.
 ...
 * @post `book_open` returned @p want.
...
static void bc_expect_open(ra8_err_t             want,
                           ...
                           size_t*               os)
{
  TEST_ASSERT_EQ(want, book_open(cont, len, bc_inflate, scratch, scap, ob, os));
}
 ...
 * `priv_book_container_header_fields` rejection reached through `book_open`:
 ...
 * (3 conditions) in apps/shared_libs/book/src/book.c@priv_book_container_header_fields:
...
{
  TEST_BEGIN("book_open container header guards");
  bc_book_t      b;
  bc_container_t c;
  bc_build_blob(&b);
  bc_build_container(&c, &b);
  ...
  (void)memcpy(bad_magic.magic, "XXXX", k_bc_magic_len);
  ...
  TEST_END("book_open container header guards");
}

/**
 * @test test_book_open_container_geometry
 ...
 */
static void test_book_open_container_geometry(void)
{
  TEST_BEGIN("book_open container geometry guards");
  bc_book_t      b;
  bc_container_t c;
  bc_build_blob(&b);
  bc_build_container(&c, &b);
  ...
  TEST_ASSERT_EQ(
    ...
    book_open(&c, k_bc_table_cut, bc_inflate, scratch, sizeof(scratch), &out_base, &out_size));

  /* Inflated total larger than the scratch capacity. */
  TEST_ASSERT_EQ(
    k_ra8_err_invalid_size,
    book_open(&c, sizeof(c), bc_inflate, scratch, k_bc_tiny_cap, &out_base, &out_size));
  ...
  TEST_END("book_open container geometry guards");
}

/**
 * @test test_book_open_inflate_stage
 ...
 * `produced != expected` size guard, plus the post-inflate `book_validate`
 * failure pass-through in `book_open`.
 *
 * @par MC/DC:
 * (no compound decisions under test -- the inflate-stage guards are
 * independent single-condition checks)
 */
static void test_book_open_inflate_stage(void)
{
  TEST_BEGIN("book_open per-chunk inflate stage");
  bc_book_t      b;
  bc_container_t c;
  bc_build_blob(&b);
  bc_build_container(&c, &b);
  ...
  TEST_ASSERT_EQ(
    ...
    book_open(&c, sizeof(c), bc_inflate, scratch, sizeof(scratch), &out_base, &out_size));
  ...
  corrupt.blob.hdr.crc32_val ^= k_crc32_init;
  ...
  TEST_END("book_open per-chunk inflate stage");
}

/**
 * @test test_book_open_success
 ...
 * success, and the `book_open` tail that publishes `*out_base`/`*out_size`.
 ...
 */
static void test_book_open_success(void)
{
  TEST_BEGIN("book_open full success path");
  bc_book_t      b;
  bc_container_t c;
  bc_build_blob(&b);
  bc_build_container(&c, &b);
  ...
  TEST_END("book_open full success path");
}
...
bc_pack_multi(uint8_t* out, const uint8_t* blob, uint32_t blob_len, uint32_t chunk_bytes)
{
  ...
  for (uint32_t i = 0U; i <= count; ++i) {
    ...
    internal_copy_object(&out[pos], &ent, sizeof(ent));
    pos += sizeof(ent);
  }
  (void)memcpy(&out[pos], blob, blob_len);
  return pos + blob_len;
}

/**
 * @test test_book_open_multi_chunk
 ...
 */
static void test_book_open_multi_chunk(void)
{
  TEST_BEGIN("book_open multi-chunk reassembly");
  bc_book_t b;
  bc_build_blob(&b);
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    book_open(s_file, file_len, bc_inflate, scratch, sizeof(scratch), &out_base, &out_size));
  TEST_ASSERT_EQ(sizeof(b), out_size);
  TEST_ASSERT(internal_objects_equal(scratch, &b, sizeof(b)));

  TEST_END("book_open multi-chunk reassembly");
}

int main(void)
{
  test_book_validate_total_size_range();
  test_book_validate_table_extent();
  test_book_open_null_guards();
  test_mcdc_container_header_fields();
  test_book_open_container_geometry();
  test_book_open_inflate_stage();
  test_book_open_success();
  test_book_open_multi_chunk();
  return 0;
}
