/**
 * @file test_book_xhtml.c
 * @brief MC/DC unit tests for the compiled-book XHTML/text serializer
 *        (apps/shared_libs/book/src/book_xhtml.c).
 *
 * @details
 * Drives `book_chapter_to_xhtml()` and `book_chapter_text()` over a
 ...
 */
typedef struct {
  book_header_t  hdr;                   /**< Container header with offsets. */
  book_chapter_t chapters[1];           /**< Single spine chapter.          */
  book_node_t    nodes[k_bx_node_cap];  /**< DOM node table.                */
  book_attr_t    attrs[k_bx_attr_cap];  /**< Attribute table.               */
...
static uint32_t bx_intern(bx_book_t* b, uint32_t* off, const char* s)
{
  ...
  (void)memcpy(&b->strings[at], s, len + 1U);
  *off += (uint32_t)len + 1U;
  return at;
}
...
static void bx_build_nodes(bx_book_t* b, const bx_offsets_t* o)
{
  ...
  b->nodes[k_bx_node_div]    = (book_node_t){.kind         = (uint8_t)k_book_node_element,
                                             .attr_count   = 1U,
                                             .name_off     = o->div,
                                             .first_attr   = k_bx_attr_class,
                                             .first_child  = k_bx_node_p,
                                             .next_sibling = k_book_nil};
  b->nodes[k_bx_node_p]      = (book_node_t){.kind         = (uint8_t)k_book_node_element,
                                             .name_off     = o->p,
                                             .first_attr   = k_book_nil,
                                             .first_child  = k_bx_node_ptext,
                                             .next_sibling = k_bx_node_img};
  b->nodes[k_bx_node_ptext]  = (book_node_t){.kind         = (uint8_t)k_book_node_text,
                                             .text_off     = o->ptext,
                                             .first_attr   = k_book_nil,
                                             .first_child  = k_book_nil,
                                             .next_sibling = k_book_nil};
  b->nodes[k_bx_node_img]    = (book_node_t){.kind         = (uint8_t)k_book_node_element,
                                             .attr_count   = 1U,
                                             .name_off     = o->img,
                                             .first_attr   = k_bx_attr_src,
                                             .first_child  = k_book_nil,
                                             .next_sibling = k_bx_node_br};
  b->nodes[k_bx_node_br]     = (book_node_t){.kind         = (uint8_t)k_book_node_element,
                                             .name_off     = o->br,
                                             .first_attr   = k_book_nil,
                                             .first_child  = k_book_nil,
                                             .next_sibling = k_bx_node_span};
  b->nodes[k_bx_node_span]   = (book_node_t){.kind         = (uint8_t)k_book_node_element,
                                             .name_off     = o->span,
                                             .first_attr   = k_book_nil,
                                             .first_child  = k_bx_node_sptext,
                                             .next_sibling = k_book_nil};
  b->nodes[k_bx_node_sptext] = (book_node_t){.kind         = (uint8_t)k_book_node_text,
                                             .text_off     = o->sptext,
                                             .first_attr   = k_book_nil,
                                             .first_child  = k_book_nil,
                                             .next_sibling = k_book_nil};

  b->attrs[k_bx_attr_class] = (book_attr_t){.name_off = o->cls, .value_off = o->classv};
  b->attrs[k_bx_attr_src]   = (book_attr_t){.name_off = o->src, .value_off = o->srcv};
}
...
static void bx_build_cyclic(bx_book_t* b)
{
  ...
  b->nodes[0]              = (book_node_t){.kind         = (uint8_t)k_book_node_text,
                                           .text_off     = s_x,
                                           .first_attr   = k_book_nil,
                                           .first_child  = k_book_nil,
 ...
 */
static void test_book_xhtml_serializes_and_escapes(void)
{
  TEST_BEGIN("book_xhtml serialize + entity escaping + void elements");
  bx_book_t b;
  bx_build(&b);
  char   out[k_bx_out_cap] = {};
  size_t len               = 0U;

  TEST_ASSERT_EQ(k_ra8_ok, book_chapter_to_xhtml(&b, 0U, out, sizeof(out), &len));
  out[len] = '\0';
  ...
  TEST_END("book_xhtml serialize + entity escaping + void elements");
}
 ...
 */
static void test_book_xhtml_to_xhtml_overflow_graded(void)
{
  TEST_BEGIN("book_xhtml to_xhtml graded overflow");
  bx_book_t b;
  bx_build(&b);
  char   out[k_bx_out_cap] = {};
  size_t len               = 0U;
  ...
  for (size_t i = 0U; i < (sizeof(caps) / sizeof(caps[0])); ++i) {
    ...
    TEST_ASSERT_EQ(k_ra8_err_invalid_size, book_chapter_to_xhtml(&b, 0U, out, caps[i], &len));
  }
  TEST_END("book_xhtml to_xhtml graded overflow");
}
 ...
 */
static void test_book_xhtml_invalid_chapter_and_null_guards(void)
{
  TEST_BEGIN("book_xhtml invalid chapter + null guards");
  bx_book_t b;
  bx_build(&b);
  char   out[k_bx_out_cap] = {};
  size_t len               = 0U;

  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, book_chapter_to_xhtml(&b, 1U, out, sizeof(out), &len));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, book_chapter_text(&b, 1U, out, sizeof(out), &len));
  TEST_ASSERT(book_chapter_to_xhtml(nullptr, 0U, out, sizeof(out), &len) != k_ra8_ok);
  TEST_ASSERT(book_chapter_to_xhtml(&b, 0U, nullptr, sizeof(out), &len) != k_ra8_ok);
  TEST_ASSERT(book_chapter_text(&b, 0U, out, sizeof(out), nullptr) != k_ra8_ok);
  TEST_END("book_xhtml invalid chapter + null guards");
}
 ...
 */
static void test_book_xhtml_text_collapses_whitespace(void)
{
  TEST_BEGIN("book_xhtml chapter_text whitespace collapse");
  bx_book_t b;
  bx_build(&b);
  char   out[k_bx_out_cap] = {};
  size_t len               = 0U;

  TEST_ASSERT_EQ(k_ra8_ok, book_chapter_text(&b, 0U, out, sizeof(out), &len));
  out[len] = '\0';
  TEST_ASSERT_NOT_NULL(strstr(out, "hi there you zz"));
  TEST_ASSERT_NULL(strchr(out, '\t'));
  TEST_ASSERT_NULL(strchr(out, '\r'));
  TEST_END("book_xhtml chapter_text whitespace collapse");
}
 ...
 */
static void test_book_xhtml_text_block_breaks(void)
{
  TEST_BEGIN("book_xhtml chapter_text block breaks");
  bx_book_t b;
  bx_build(&b);
  char   out[k_bx_out_cap] = {};
  size_t len               = 0U;
  ...
  TEST_END("book_xhtml chapter_text block breaks");
}
 ...
 */
static void test_book_xhtml_text_overflow_graded(void)
{
  TEST_BEGIN("book_xhtml chapter_text graded overflow");
  bx_book_t b;
  bx_build(&b);
  char         out[k_bx_out_cap] = {};
  size_t       len               = 0U;
  ...
  for (size_t i = 0U; i < (sizeof(caps) / sizeof(caps[0])); ++i) {
    len = k_xhtml_poison_len;
    TEST_ASSERT_EQ(k_ra8_err_invalid_size, book_chapter_text(&b, 0U, out, caps[i], &len));
  }
  TEST_END("book_xhtml chapter_text graded overflow");
}
 ...
 */
static void test_book_xhtml_walk_guard_exhaustion(void)
{
  TEST_BEGIN("book_xhtml walk iteration-guard exhaustion");
  bx_book_t b;
  bx_build_cyclic(&b);
  char   out[k_bx_big_cap] = {};
  size_t len               = 0U;

  TEST_ASSERT_EQ(k_ra8_err_invalid_size, book_chapter_to_xhtml(&b, 0U, out, sizeof(out), &len));
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, book_chapter_text(&b, 0U, out, sizeof(out), &len));
  TEST_END("book_xhtml walk iteration-guard exhaustion");
}
...
static void bx_build_small(bx_book_t* b)
{
  ...
  b->chapters[0].root_node = 0U;
  b->nodes[0]              = (book_node_t){.kind         = (uint8_t)k_book_node_element,
                                           .name_off     = s_p,
                                           .first_attr   = k_book_nil,
                                           .first_child  = 1U,
                                           .next_sibling = k_book_nil};
  b->nodes[1]              = (book_node_t){.kind         = (uint8_t)k_book_node_text,
                                           .text_off     = s_x,
                                           .first_attr   = k_book_nil,
                                           .first_child  = k_book_nil,
                                           .next_sibling = k_book_nil};
...
static void bx_build_trail(bx_book_t* b)
{
  ...
  b->nodes[1]              = (book_node_t){.kind         = (uint8_t)k_book_node_text,
                                           ...
                                           .first_child  = k_book_nil,
                                           .next_sibling = 2U};
  b->nodes[2]              = (book_node_t){.kind         = (uint8_t)k_book_node_element,
                                           .name_off     = s_p,
                                           .first_attr   = k_book_nil,
                                           .first_child  = 3U,
                                           .next_sibling = k_book_nil};
  b->nodes[3]              = (book_node_t){.kind         = (uint8_t)k_book_node_text,
                                           .text_off     = s_x,
                                           .first_attr   = k_book_nil,
                                           .first_child  = k_book_nil,
                                           .next_sibling = k_book_nil};
 ...
 */
static void test_book_xhtml_close_chain_overflow(void)
{
  TEST_BEGIN("book_xhtml close-tag chain overflow");
  bx_book_t b;
  bx_build_small(&b);
  char         out[k_bx_out_cap] = {};
  size_t       len               = 0U;
  ...
  TEST_END("book_xhtml close-tag chain overflow");
}
 ...
 */
static void test_book_xhtml_break_trims_trailing_space(void)
{
  TEST_BEGIN("book_xhtml break trims trailing space");
  bx_book_t b;
  bx_build_trail(&b);
  char   out[k_bx_out_cap] = {};
  size_t len               = 0U;
  ...
  TEST_END("book_xhtml break trims trailing space");
}
...
typedef struct {
  ...
  book_chapter_t chapters[1];        /**< Single spine chapter.            */
  book_node_t    nodes[k_bx_deep_n]; /**< Linear chain of nested elements. */
...
static void bx_build_deep(bx_deep_t* b)
{
  ...
  for (uint32_t i = 0U; i < (k_bx_deep_n - 1U); ++i) {
    b->nodes[i] = (book_node_t){.kind         = (uint8_t)k_book_node_element,
                                .name_off     = 1U,
                                .first_attr   = k_book_nil,
                                .first_child  = i + 1U,
                                .next_sibling = k_book_nil};
  }
  b->nodes[k_bx_deep_n - 1U] = (book_node_t){.kind         = (uint8_t)k_book_node_text,
                                             .text_off     = 3U,
                                             .first_attr   = k_book_nil,
                                             .first_child  = k_book_nil,
                                             .next_sibling = k_book_nil};
 ...
 */
static void test_book_xhtml_deep_nest_bounds(void)
{
  TEST_BEGIN("book_xhtml deep-nest walk bounds");
  static bx_deep_t s_b;
  bx_build_deep(&s_b);
  char   out[k_bx_big_cap] = {};
  size_t len               = k_xhtml_poison_len;

  TEST_ASSERT_EQ(k_ra8_err_invalid_size, book_chapter_to_xhtml(&s_b, 0U, out, sizeof(out), &len));
  len = k_xhtml_poison_len;
  TEST_ASSERT_EQ(k_ra8_ok, book_chapter_text(&s_b, 0U, out, sizeof(out), &len));
  TEST_ASSERT_EQ(0, len); /* descent capped before reaching the leaf */
  TEST_END("book_xhtml deep-nest walk bounds");
}

int main(void)
{
  test_book_xhtml_serializes_and_escapes();
  test_book_xhtml_to_xhtml_overflow_graded();
  test_book_xhtml_invalid_chapter_and_null_guards();
  test_book_xhtml_text_collapses_whitespace();
  test_book_xhtml_text_block_breaks();
  test_book_xhtml_text_overflow_graded();
  test_book_xhtml_walk_guard_exhaustion();
  test_book_xhtml_close_chain_overflow();
  test_book_xhtml_break_trims_trailing_space();
  test_book_xhtml_deep_nest_bounds();
  return 0;
}
