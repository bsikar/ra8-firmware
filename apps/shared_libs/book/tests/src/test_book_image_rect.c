 ...
 * a 600x2 image (wider than one unpack span, so ::book_src_image_rect must
 * stitch several bounded ::book_src_read spans per row); each pool nibble is
...
typedef struct {
  ...
  book_image_t   images[3];              /**< Images (2 gray4 + 1 gray8). */
...
/** @brief Gray8 value book_src_image_rect must produce for pixel `(px, py)`. */
...
static book_image_t ibook_img(uint16_t w, uint16_t h, uint32_t data_off, uint32_t data_size)
{
  return (book_image_t){.id_off       = 0U,
                        .width        = w,
                        .height       = h,
...
static book_image_t ibook_img_gray8(uint16_t w, uint16_t h, uint32_t data_off, uint32_t data_size)
{
  return (book_image_t){.id_off       = 0U,
                        .width        = w,
                        .height       = h,
...
static void ibook_setup(ibook_t* b)
{
  ...
  b->nodes[0]              = (book_node_t){.kind         = k_book_node_text,
                                           .text_off     = 0U,
  ...
  b->hdr.crc32_val        = ibook_crc32(body, body_len);
}
...
static ra8_err_t ibook_read(void* ctx, uint64_t offset, uint8_t* buf, uint32_t len)
{
  ...
  (void)memcpy(buf, &s_ibook_bytes[(size_t)offset], len);
  return k_ra8_ok;
}
...
static void
ibook_bind_paged(book_src_t* psrc, ra8_vsource_t* vs, ra8_vsource_obj_t* objs, ra8_vmem_t* vm)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, book_src_paged(psrc, vm, oid, (uint32_t)k_ib_frame_bytes, s_ibook_len));
}
...
/**
 * @test test_book_src_image_descriptor
 * @brief book_src_image reads a descriptor identically resident and paged.
 *
 * @par MC/DC:
 ...
 */
static void test_book_src_image_descriptor(void)
{
  TEST_BEGIN("book_src_image descriptor read");
  ...
  book_image_t img = {};
  /* Null + range guards. */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, book_src_image(nullptr, 0U, &img));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, book_src_image(&rsrc, 0U, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, book_src_image(&rsrc, 3U, &img)); /* == image_count */

  /* Every descriptor read matches the resident inline accessor byte-for-byte. */
  const book_image_t* inl = book_images(&s_book);
  for (uint32_t i = 0U; i < s_book.hdr.image_count; ++i) {
    book_image_t got = {};
    TEST_ASSERT_EQ(k_ra8_ok, book_src_image(&rsrc, i, &got));
    TEST_ASSERT(internal_objects_equal(&got, &inl[i], sizeof(got)));
  }
  ...
  for (uint32_t i = 0U; i < s_book.hdr.image_count; ++i) {
    book_image_t got = {};
    TEST_ASSERT_EQ(k_ra8_ok, book_src_image(&psrc, i, &got));
    TEST_ASSERT(internal_objects_equal(&got, &inl[i], sizeof(got)));
  }

  TEST_END("book_src_image descriptor read");
}
...
static void ibook_rect_both(const book_src_t*   rsrc,
                            const book_src_t*   psrc,
                            ...
                            uint32_t            h)
{
  ...
  (void)memset(s_out_r, k_fill_poison_a, sizeof s_out_r);
  (void)memset(s_out_p, k_fill_poison_b, sizeof s_out_p);
  TEST_ASSERT_EQ(k_ra8_ok, book_src_image_rect(rsrc, img, x, y, w, h, s_out_r, w));
  TEST_ASSERT_EQ(k_ra8_ok, book_src_image_rect(psrc, img, x, y, w, h, s_out_p, w));
  ibook_verify_rect(iw, x, y, w, h, s_out_r, w);
  TEST_ASSERT_EQ(0, memcmp(s_out_r, s_out_p, (size_t)w * (size_t)h));
}

/**
 * @test test_book_src_image_rect_matches
 ...
 * test_mcdc_book_src_image_rect_guards)
 */
static void test_book_src_image_rect_matches(void)
{
  TEST_BEGIN("book_src_image_rect unpack matches");
  ...
  book_image_t img0 = {};
  book_image_t img1 = {};
  TEST_ASSERT_EQ(k_ra8_ok, book_src_image(&rsrc, 0U, &img0));
  TEST_ASSERT_EQ(k_ra8_ok, book_src_image(&rsrc, 1U, &img1));
  ...
  (void)memset(s_out, k_fill_poison_a, sizeof s_out);
  TEST_ASSERT_EQ(k_ra8_ok,
                 book_src_image_rect(&rsrc, &img0, 1U, 1U, 3U, 2U, s_out, (uint32_t)k_img0_w));
  ...
  TEST_END("book_src_image_rect unpack matches");
}

/** @brief Drive the three null-pointer guards of book_src_image_rect and the
...
static void ibook_guard_singleconds(const book_src_t* rsrc, const book_image_t* img, uint8_t* s_out)
{
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 book_src_image_rect(nullptr, img, 0U, 0U, k_img0_w, k_img0_h, s_out, k_img0_w));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 book_src_image_rect(rsrc, nullptr, 0U, 0U, k_img0_w, k_img0_h, s_out, k_img0_w));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 book_src_image_rect(rsrc, img, 0U, 0U, k_img0_w, k_img0_h, nullptr, k_img0_w));

  /* Vector 1: an SVG (non-raster) descriptor -> A true -> rejected. */
  book_image_t svg = *img;
  svg.format       = (uint8_t)k_book_image_svg;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 book_src_image_rect(rsrc, &svg, 0U, 0U, k_img0_w, k_img0_h, s_out, k_img0_w));

  /* Vector 3: a gray8 raster -> A false, B true, C false -> proceeds. */
  book_image_t g8 = *img;
  g8.pixel_format = (uint8_t)k_book_pixfmt_gray8;
  TEST_ASSERT_EQ(k_ra8_ok,
                 book_src_image_rect(rsrc, &g8, 0U, 0U, k_img0_w, k_img0_h, s_out, k_img0_w));

  /* Vector 4: a raster with an unknown depth -> A false, B true, C true -> rejected. */
  book_image_t unk = *img;
  unk.pixel_format = (uint8_t)(k_book_pixfmt_gray8 + 1U);
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 book_src_image_rect(rsrc, &unk, 0U, 0U, k_img0_w, k_img0_h, s_out, k_img0_w));
}

/**
 * @test test_mcdc_book_src_image_rect_guards
 * @brief The two compound bound guards reject malformed rectangles.
 *
 * @par MC/DC:
 * Decision: `(w == 0U) || (h == 0U) || (out_stride < w)` (3 conditions) in
 * apps/shared_libs/book/src/book_paged.c@book_src_image_rect:
 ...
 * in apps/shared_libs/book/src/book_paged.c@book_src_image_rect:
 ...
 *
 * @par MC/DC:
 * Decision: `(img->format != k_book_image_gray4) ||
 ...
 */
static void test_mcdc_book_src_image_rect_guards(void)
{
  TEST_BEGIN("book_src_image_rect bound guards (MC/DC)");
  ...
  TEST_ASSERT_EQ(k_ra8_ok, book_src_resident(&rsrc, &s_book, (uint32_t)sizeof(s_book)));
  book_image_t img = {};
  TEST_ASSERT_EQ(k_ra8_ok, book_src_image(&rsrc, 0U, &img));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, /* V1: all false */
                 book_src_image_rect(&rsrc, &img, 0U, 0U, k_img0_w, k_img0_h, s_out, k_img0_w));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, /* V2: w==0 */
                 book_src_image_rect(&rsrc, &img, 0U, 0U, 0U, k_img0_h, s_out, k_img0_w));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, /* V3: h==0 */
                 book_src_image_rect(&rsrc, &img, 0U, 0U, k_img0_w, 0U, s_out, k_img0_w));
  TEST_ASSERT_EQ(
    k_ra8_err_invalid_arg, /* V4: out_stride < w */
    book_src_image_rect(&rsrc, &img, 0U, 0U, k_img0_w, k_img0_h, s_out, (uint32_t)k_img0_w - 1U));
  ...
  TEST_ASSERT_EQ(k_ra8_err_out_of_range, /* V2: x+w > width */
                 book_src_image_rect(&rsrc, &img, 1U, 0U, k_img0_w, k_img0_h, s_out, k_img0_w));
  TEST_ASSERT_EQ(k_ra8_err_out_of_range, /* V3: y+h > height */
                 book_src_image_rect(&rsrc, &img, 0U, 1U, k_img0_w, k_img0_h, s_out, k_img0_w));

  TEST_END("book_src_image_rect bound guards (MC/DC)");
}
 ...
 * test_mcdc_book_src_image_rect_guards)
 */
static void test_book_src_image_rect_gray8(void)
{
  TEST_BEGIN("book_src_image_rect gray8 full-resolution read");
  ...
  book_image_t img2 = {};
  TEST_ASSERT_EQ(k_ra8_ok, book_src_image(&rsrc, 2U, &img2));
  TEST_ASSERT_EQ(k_book_image_gray4, img2.format); /* raster tag */
  TEST_ASSERT_EQ(k_book_pixfmt_gray8, book_image_pixfmt(&img2));
  ...
  (void)memset(s_out_p, k_fill_poison_b, sizeof s_out_p);
  TEST_ASSERT_EQ(k_ra8_ok,
                 book_src_image_rect(&rsrc, &img2, 0U, 0U, k_img2_w, k_img2_h, s_out_r, k_img2_w));
  TEST_ASSERT_EQ(k_ra8_ok,
                 book_src_image_rect(&psrc, &img2, 0U, 0U, k_img2_w, k_img2_h, s_out_p, k_img2_w));
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    book_src_image_rect(&rsrc, &img2, 3U, 1U, k_img2_sub_w, 2U, s_out, (uint32_t)k_img2_w));
  ibook_verify_rect_g8(k_img2_w, 3U, 1U, k_img2_sub_w, 2U, s_out, (uint32_t)k_img2_w);

  TEST_END("book_src_image_rect gray8 full-resolution read");
}

/**
 * @test test_book_src_image_rect_faults
 ...
 */
static void test_book_src_image_rect_faults(void)
{
  TEST_BEGIN("book_src_image_rect fault propagation");
  ...
  book_image_t past = {};
  TEST_ASSERT_EQ(k_ra8_ok, book_src_image(&rsrc, 0U, &past));
  past.data_off = (uint32_t)sizeof(s_book.pool); /* pool_off + data_off == total_size */
  TEST_ASSERT_EQ(k_ra8_err_out_of_range,
                 book_src_image_rect(&rsrc, &past, 0U, 0U, k_img0_w, 1U, s_out, k_img0_w));
  ...
  book_image_t img0 = {};
  TEST_ASSERT_EQ(k_ra8_ok, book_src_image(&psrc, 0U, &img0));

  s_ibook_fault_off   = (uint32_t)k_ib_frame_bytes * 2U; /* frames 0,1 hold the header */
  s_ibook_fault_armed = true;
  const ra8_err_t e =
    book_src_image_rect(&psrc, &img0, 0U, 0U, k_img0_w, k_img0_h, s_out, k_img0_w);
  TEST_ASSERT(e != k_ra8_ok); /* the loader fault propagated through the unpack */
  s_ibook_fault_armed = false;

  TEST_END("book_src_image_rect fault propagation");
}

int main(void)
{
  test_book_src_image_descriptor();
  test_book_src_image_rect_matches();
  test_mcdc_book_src_image_rect_guards();
  test_book_src_image_rect_gray8();
  test_book_src_image_rect_faults();
  return 0;
}
