/**
 * @file test_book.c
 * @brief Unit tests for the flat, XIP e-book container format (apps/shared_libs/book).
...
typedef enum : uint8_t {
  k_book_pixfmt_unknown =
    2, /**< One past k_book_pixfmt_gray8: unpackable depth, so validate must reject it. */
...
typedef enum : uint32_t {
  k_book_crc_flip_mask =
    0xFFFFFFFFU, /**< Flips every bit of a valid CRC so the check cannot pass by chance. */
...
static uint32_t compute_crc32(const uint8_t* data, size_t len)
{
  ...
  for (size_t i = 0U; i < len; ++i) {
    ...
    for (uint8_t bit = 0U; bit < 8U; ++bit) {
      uint32_t mask = ((crc & 1U) != 0U) ? UINT32_MAX : 0U;
...
static uint32_t mock_intern(mock_book_t* b, int* offset, const char* s)
{
  ...
  (void)memcpy(&b->strings[*offset], s, len + 1U);
  *offset += (int)len + 1;
  return at;
}
...
{
  b->nodes[0].kind         = k_book_node_element;
  ...
  b->nodes[0].next_sibling = k_book_nil;

  b->nodes[1].kind         = k_book_node_text;
  b->nodes[1].reserved     = 0;
  b->nodes[1].attr_count   = 0;
  b->nodes[1].name_off     = 0;
  b->nodes[1].text_off     = off->text_content;
  b->nodes[1].first_attr   = k_book_nil;
  b->nodes[1].first_child  = k_book_nil;
  b->nodes[1].next_sibling = k_book_nil;
}
...
static void setup_mock_attrs_and_css(mock_book_t* b, const mock_book_offsets_t* off)
{
  ...
  b->stylesheets[0].scope_chapter = k_book_nil;
}
...
static void setup_mock_images(mock_book_t* b, const mock_book_offsets_t* off)
{
  ...
  b->images[0].format       = k_book_image_gray4;
  b->images[0].pixel_format = k_book_pixfmt_gray4;
  ...
  b->images[1].width        = 0;
  b->images[1].height       = 0;
  b->images[1].format       = k_book_image_svg;
  b->images[1].pixel_format = k_book_pixfmt_gray4;
...
static void setup_mock_book(mock_book_t* b)
{
  ...
  uint32_t       body_len = sizeof(*b) - sizeof(book_header_t);
  b->hdr.crc32_val        = compute_crc32(body, body_len);
}

/**
 * @test test_book_invalid
 * @brief book_validate() rejects null, short, mis-magicked, mis-versioned,
 ...
 * tests/src/test_book_cov.c)
 */
static void test_book_invalid(void)
{
  TEST_BEGIN("book validation invalid parameters");

  TEST_ASSERT_EQ(k_ra8_err_null_ptr, book_validate(NULL, 100));

  uint8_t buffer[k_book_image_pool_cap] = {};
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, book_validate(buffer, sizeof(buffer)));

  mock_book_t b;
  setup_mock_book(&b);
  (void)memcpy(b.hdr.magic, "WRONGMAG", 8);
  const uint8_t* body     = &((const uint8_t*)&b)[sizeof(book_header_t)];
  uint32_t       body_len = sizeof(b) - sizeof(book_header_t);
  b.hdr.crc32_val         = compute_crc32(body, body_len);
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, book_validate(&b, sizeof(b)));

  setup_mock_book(&b);
  b.hdr.format_version = k_book_version_unsupported;
  body                 = &((const uint8_t*)&b)[sizeof(book_header_t)];
  b.hdr.crc32_val      = compute_crc32(body, body_len);
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, book_validate(&b, sizeof(b)));

  setup_mock_book(&b);
  b.hdr.crc32_val ^= k_book_crc_flip_mask;
  TEST_ASSERT_EQ(k_ra8_err_range_check_failed, book_validate(&b, sizeof(b)));

  setup_mock_book(&b);
  b.hdr.chapter_off = sizeof(b) + k_book_img0_bytes;
  body              = &((const uint8_t*)&b)[sizeof(book_header_t)];
  b.hdr.crc32_val   = compute_crc32(body, body_len);
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, book_validate(&b, sizeof(b)));

  TEST_END("book validation invalid parameters");
}

/**
 * @test test_book_pixfmt
 ...
 *
 * @par MC/DC:
 * Decision: `if (imgs[i].pixel_format > k_book_pixfmt_gray8)` in
 * book_image_pixfmts_known (single condition; N+1 = 2 vectors).
 ...
 */
static void test_book_pixfmt(void)
{
  TEST_BEGIN("book validation image pixel format");
  mock_book_t b;

  /* gray8 is a known depth: accepted, and the accessor decodes it. */
  setup_mock_book(&b);
  b.images[0].pixel_format = (uint8_t)k_book_pixfmt_gray8;
  const uint8_t* body      = &((const uint8_t*)&b)[sizeof(book_header_t)];
  uint32_t       body_len  = sizeof(b) - sizeof(book_header_t);
  b.hdr.crc32_val          = compute_crc32(body, body_len);
  TEST_ASSERT_EQ(k_ra8_ok, book_validate(&b, sizeof(b)));
  TEST_ASSERT_EQ(k_book_pixfmt_gray8, book_image_pixfmt(&book_images(&b)[0]));
  /* Backward-read: image 1 kept its zero byte (== gray4) and still validates. */
  TEST_ASSERT_EQ(k_book_pixfmt_gray4, book_image_pixfmt(&book_images(&b)[1]));
  ...
  TEST_END("book validation image pixel format");
}
 ...
 *
 * @pre @p b passed book_validate().
 * @post The element node, its attribute, and the text child all match.
 * @note Not thread-safe; single-threaded host-test helper.
 * @since 0.1.0
 */
static void book_walk_check_nodes(const mock_book_t* b, const book_header_t* hdr)
{
  TEST_ASSERT_EQ(2, hdr->node_count);
  const book_node_t* nodes = book_nodes(b);
  TEST_ASSERT_NOT_NULL(nodes);

  const book_node_t* n0 = &nodes[0];
  TEST_ASSERT_EQ(k_book_node_element, n0->kind);
  TEST_ASSERT_EQ(0, strcmp(book_node_name(b, n0), "div"));
  TEST_ASSERT_EQ(1, n0->attr_count);
  TEST_ASSERT_EQ(1, n0->first_child);
  TEST_ASSERT_EQ(k_book_nil, n0->next_sibling);

  const book_attr_t* attrs = book_attrs(b);
  TEST_ASSERT_NOT_NULL(attrs);
  const book_attr_t* a0 = &attrs[n0->first_attr];
  TEST_ASSERT_EQ(0, strcmp(book_string(b, a0->name_off), "class"));
  TEST_ASSERT_EQ(0, strcmp(book_string(b, a0->value_off), "main"));

  const book_node_t* n1 = &nodes[n0->first_child];
  TEST_ASSERT_EQ(k_book_node_text, n1->kind);
  TEST_ASSERT_EQ(0, strcmp(book_node_text(b, n1), "Hello World"));
  TEST_ASSERT_EQ(0, n1->attr_count);
  TEST_ASSERT_EQ(k_book_nil, n1->first_child);
  TEST_ASSERT_EQ(k_book_nil, n1->next_sibling);
}
 ...
 * @since 0.1.0
 */
static void book_walk_check_images(const mock_book_t* b, const book_header_t* hdr)
{
  TEST_ASSERT_EQ(2, hdr->image_count);
  const book_image_t* imgs = book_images(b);
  TEST_ASSERT_NOT_NULL(imgs);

  TEST_ASSERT_EQ(0, strcmp(book_string(b, imgs[0].id_off), "img1.png"));
  TEST_ASSERT_EQ(16, imgs[0].width);
  TEST_ASSERT_EQ(16, imgs[0].height);
  TEST_ASSERT_EQ(k_book_image_gray4, imgs[0].format);
  TEST_ASSERT_EQ(k_book_pixfmt_gray4, book_image_pixfmt(&imgs[0]));
  TEST_ASSERT_EQ(10, imgs[0].data_size);
  TEST_ASSERT_EQ(128, imgs[0].raw_size);
  TEST_ASSERT_NOT_NULL(book_image_data(b, &imgs[0]));

  TEST_ASSERT_EQ(0, strcmp(book_string(b, imgs[1].id_off), "svg1.svg"));
  TEST_ASSERT_EQ(0, imgs[1].width);
  TEST_ASSERT_EQ(0, imgs[1].height);
  TEST_ASSERT_EQ(k_book_image_svg, imgs[1].format);
  TEST_ASSERT_EQ(k_book_pixfmt_gray4, book_image_pixfmt(&imgs[1]));
  TEST_ASSERT_EQ(20, imgs[1].data_size);
  TEST_ASSERT_EQ(50, imgs[1].raw_size);
  TEST_ASSERT_NOT_NULL(book_image_data(b, &imgs[1]));
}

/**
 * @test test_book_valid_walk
 ...
 */
static void test_book_valid_walk(void)
{
  TEST_BEGIN("book validation and walking valid book");
  ...
  const book_header_t* hdr = book_header(&b);
  TEST_ASSERT_NOT_NULL(hdr);

  TEST_ASSERT_EQ(0, strcmp(book_string(&b, hdr->title_off), "Title"));
  TEST_ASSERT_EQ(0, strcmp(book_string(&b, hdr->author_off), "Author"));
  TEST_ASSERT_EQ(0, strcmp(book_string(&b, hdr->language_off), "en"));
  TEST_ASSERT_EQ(0, strcmp(book_string(&b, hdr->identifier_off), "ID123"));
  TEST_ASSERT_EQ(0, hdr->cover_image_index);

  TEST_ASSERT_EQ(1, hdr->chapter_count);
  const book_chapter_t* chaps = book_chapters(&b);
  TEST_ASSERT_NOT_NULL(chaps);
  TEST_ASSERT_EQ(0, strcmp(book_string(&b, chaps[0].title_off), "Chapter 1"));
  TEST_ASSERT_EQ(0, strcmp(book_string(&b, chaps[0].href_off), "ch1.xhtml"));
  TEST_ASSERT_EQ(0, chaps[0].root_node);

  book_walk_check_nodes(&b, hdr);

  TEST_ASSERT_EQ(1, hdr->stylesheet_count);
  const book_stylesheet_t* styles = book_stylesheets(&b);
  TEST_ASSERT_NOT_NULL(styles);
  TEST_ASSERT_EQ(0, strcmp(book_string(&b, styles[0].source_off), "body { margin: 0; }"));
  TEST_ASSERT_EQ(k_book_nil, styles[0].scope_chapter);

  book_walk_check_images(&b, hdr);

  TEST_END("book validation and walking valid book");
}

/**
 * @enum book_test_flag_t
 * @brief Header `flags` vectors used by the flags-mask validation tests.
 * @details Values chosen around ::k_book_flag_mask_known: the first bit
...
/**
 * @test test_book_flags_known_mask
 * @brief book_validate() accepts every defined header flag bit and rejects
 *        any blob carrying a bit outside ::k_book_flag_mask_known.
 *
 * @par MC/DC:
 * (no compound decisions under test -- the flags guard in
 * book_validate() is the single condition
 * `(hdr->flags & ~k_book_flag_mask_known) != 0U`; the vectors below are
 * boundary coverage of that one condition, both polarities)
 * - flags = 0                       -> accepted (legacy v1 blob).
 * - flags = k_book_flag_rtl      -> accepted (all defined bits set).
 ...
 */
static void test_book_flags_known_mask(void)
{
  TEST_BEGIN("book flags known-mask validation");
  ...
  TEST_END("book flags known-mask validation");
}

/**
 * @test test_book_rtl_accessor
 * @brief book_is_rtl() decodes exactly the RTL bit of the header flags.
 *
 * @par MC/DC:
 * (no compound decisions in this test -- book_is_rtl() is a single
 ...
 */
static void test_book_rtl_accessor(void)
{
  TEST_BEGIN("book RTL flag accessor");

  mock_book_t b;
  setup_mock_book(&b);
  TEST_ASSERT_EQ(false, book_is_rtl(&b));

  b.hdr.flags = k_book_flag_rtl;
  TEST_ASSERT_EQ(k_ra8_ok, book_validate(&b, sizeof(b)));
  TEST_ASSERT_EQ(true, book_is_rtl(&b));

  b.hdr.flags = 0;
  TEST_ASSERT_EQ(false, book_is_rtl(&b));

  TEST_END("book RTL flag accessor");
}
...
int main(void)
{
  test_book_invalid();
  test_book_pixfmt();
  test_book_valid_walk();
  test_book_flags_known_mask();
  test_book_rtl_accessor();
  return 0;
}
