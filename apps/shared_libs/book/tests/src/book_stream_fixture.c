...
typedef struct {
  alignas(max_align_t) uint8_t bytes[k_ra8_compress_scratch_bytes]; /**< Compressor bytes. */
} stream_compressor_t;
...
{
  return (uint32_t)offsetof(stream_book_t, pool) + (uint32_t)sizeof(g_book.pool);
}
...
{
  const uint8_t* body = &((const uint8_t*)&g_book)[k_book_sizeof_header];
  g_book.hdr.crc32_val =
    internal_fixture_crc32(body, priv_book_fixture_flat_len() - (uint32_t)k_book_sizeof_header);
}
...
static void internal_setup_book_header(void)
{
  ...
  g_book.hdr.format_version   = (uint32_t)k_book_format_version;
...
RA8_PRIV void priv_book_fixture_setup(void)
{
  ...
  g_book.chapters[0] = (book_chapter_t){.title_off = title, .href_off = href, .root_node = 0U};
  g_book.nodes[0]    = (book_node_t){.kind         = (uint8_t)k_book_node_element,
                                     .attr_count   = 1U,
                                     .name_off     = body,
                                     .text_off     = 0U,
                                     .first_attr   = 0U,
                                     .first_child  = 1U,
                                     .next_sibling = (uint32_t)k_book_nil};
  g_book.nodes[1]    = (book_node_t){.kind         = (uint8_t)k_book_node_text,
                                     .attr_count   = 0U,
                                     .name_off     = 0U,
                                     .text_off     = text,
                                     .first_attr   = (uint32_t)k_book_nil,
                                     .first_child  = (uint32_t)k_book_nil,
                                     .next_sibling = (uint32_t)k_book_nil};
  g_book.attrs[0]    = (book_attr_t){.name_off = klass, .value_off = page};
  g_book.styles[0] = (book_stylesheet_t){.source_off = css, .scope_chapter = (uint32_t)k_book_nil};
  g_book.images[0] = (book_image_t){.id_off       = raster,
                                    .width        = 2U,
                                    .height       = 2U,
                                    ...
                                    .data_size    = 2U,
                                    .raw_size     = 2U};
  g_book.images[1] = (book_image_t){.id_off       = svg,
                                    .width        = 0U,
                                    .height       = 0U,
...
RA8_PRIV ra8_err_t priv_book_fixture_validate(void)
{
  ...
  book_header_t hdr = {};
  return book_validate_stream_strict(priv_book_fixture_read,
                                     &mem,
                                     mem.len,
                                     g_validate_work,
...
RA8_PRIV void priv_book_fixture_open_packed(stream_guard_fixture_t* fixture)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 book_chunked_open(&fixture->reader,
