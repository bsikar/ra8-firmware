 ...
 * The size guard
 * `(source_size < k_book_sizeof_header) || (scratch_cap == 0U)` executes
 ...
 * Decisions: apps/shared_libs/book/src/book_xhtml.c@book_chapter_to_xhtml
 */
RA8_INTERNAL static void internal_test_flat_happy_and_args(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 book_chapter_to_xhtml(&g_book, 0U, xhtml, sizeof(xhtml) - 1U, &xhtml_len));
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 book_validate_stream_strict(NULL,
                                             &mem,
                                             mem.len,
                                             g_validate_work,
  ...
  TEST_ASSERT_EQ(
    ...
    book_validate_stream_strict(priv_book_fixture_read, &mem, mem.len, g_validate_work, 0U, &hdr));
 ...
 * Decisions: apps/shared_libs/book/src/book_stream_wire.c@internal_validate_header_fields
 * Decisions: apps/shared_libs/book/src/book_stream_wire.c@internal_validate_header_layout
 * Decisions: apps/shared_libs/book/src/book_stream_wire.c@priv_book_stream_string_ref
 * Decisions: apps/shared_libs/book/src/book_stream.c@internal_validate_chapters
 * Decisions: apps/shared_libs/book/src/book_stream.c@internal_validate_crc
 */
RA8_INTERNAL static void internal_test_header_table_string_crc_corruption(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, priv_book_fixture_validate());
  priv_book_fixture_setup();
  g_book.hdr.crc32_val ^= UINT32_MAX;
 ...
 * Decisions: apps/shared_libs/book/src/book_stream.c@internal_forward_link
 * Decisions: apps/shared_libs/book/src/book_stream_wire.c@priv_book_stream_nonempty_string_ref
 * Decisions: apps/shared_libs/book/src/book_stream.c@internal_mark_forward_link
 * Decisions: apps/shared_libs/book/src/book_stream.c@internal_validate_nodes
 * Decisions: apps/shared_libs/book/src/book_stream.c@internal_validate_one_node
 */
RA8_INTERNAL static void internal_test_dom_ownership_and_renderer_safety(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, priv_book_fixture_validate());
  priv_book_fixture_setup();
  g_book.nodes[1].kind         = (uint8_t)k_book_node_element;
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, priv_book_fixture_validate());
  priv_book_fixture_setup();
  g_book.nodes[0].first_child = (uint32_t)k_book_nil;
  priv_book_fixture_refresh_crc();
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, priv_book_fixture_validate());
  priv_book_fixture_setup();
 ...
 * Decisions: apps/shared_libs/book/src/book_stream.c@internal_validate_raster
 * Decisions: apps/shared_libs/book/src/book_stream.c@internal_validate_svg
 * Decisions: apps/shared_libs/book/src/book_stream.c@internal_validate_images
 */
 ...
 * Decisions: apps/shared_libs/book/src/book_chunked_validate.c@internal_load_chunk
 * Decisions: apps/shared_libs/book/src/book_chunked_validate.c@internal_chunk_flat_read
 * Decisions: apps/shared_libs/book/src/book_chunked_validate.c@book_chunked_validate_strict
 */
RA8_INTERNAL static void internal_test_production_rbkc_round_trip_and_chunk_corruption(void)
{
  ...
  TEST_ASSERT(book_chunked_validate_strict(&fixture.reader,
                                           g_reader_chunk,
                                           sizeof(g_reader_chunk),
                                           g_validate_work,
                                           sizeof(g_validate_work),
 ...
 * @details Member-wise rather than byte-wise: ::book_chunked_t interleaves
 ...
 */
RA8_INTERNAL
static bool internal_reader_unchanged(const book_chunked_t* reader, const book_chunked_t* saved)
{
  if ((reader->file_read != saved->file_read) || (reader->file_ctx != saved->file_ctx)) {
    return false;
  }
  if ((reader->inflate_cb != saved->inflate_cb) || (reader->table != saved->table)) {
    return false;
  }
...
{
  alignas(max_align_t) uint8_t reader_header_alias[sizeof(book_chunked_t) > sizeof(book_header_t)
                                                     ? sizeof(book_chunked_t)
                                                     : sizeof(book_header_t)] = {};
  void*                        header_alias_bytes                             = reader_header_alias;
  book_chunked_t*              header_alias_reader                            = header_alias_bytes;
  book_header_t*               alias_header                                   = header_alias_bytes;
  *header_alias_reader                                                        = fixture->reader;
  const book_chunked_t saved = *header_alias_reader;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 book_chunked_validate_strict(header_alias_reader,
                                              g_reader_chunk,
                                              sizeof(g_reader_chunk),
                                              g_validate_work,
                                              sizeof(g_validate_work),
                                              alias_header));
  TEST_ASSERT(internal_reader_unchanged(header_alias_reader, &saved));

  book_header_t hdr = {.total_size = UINT32_MAX};
  alignas(max_align_t)
    uint8_t reader_chunk_alias[sizeof(book_chunked_t) > k_stream_chunk ? sizeof(book_chunked_t)
                                                                       : k_stream_chunk] = {};
  void*     chunk_alias_bytes        = reader_chunk_alias;
  book_chunked_t* chunk_alias_reader = chunk_alias_bytes;
  uint8_t*        alias_chunk        = chunk_alias_bytes;
  *chunk_alias_reader                = fixture->reader;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 book_chunked_validate_strict(chunk_alias_reader,
                                              alias_chunk,
                                              k_stream_chunk,
                                              g_validate_work,
                                              sizeof(g_validate_work),
...
{
  book_header_t  hdr    = {.total_size = UINT32_MAX};
  book_chunked_t broken = fixture->reader;
  broken.chunk_count++;
