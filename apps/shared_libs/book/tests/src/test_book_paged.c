 ...
 *   - resident, via ::book_src_resident (zero-copy `base + off`), and
 *   - paged, via ::book_src_paged over an ::ra8_vmem cache fronting an
 ...
 * edge tests live in test_book_paged_edge.c.
 *
...
static void pbook_bind(book_src_t*        psrc,
                       ra8_vsource_t*     vs,
                       ...
                       uint32_t*          out_oid)
{
  ...
  book_src_t bound = {};
  TEST_ASSERT_EQ(k_ra8_ok,
                 book_src_paged(&bound, vm, oid, (uint32_t)k_pb_frame_bytes, s_pbook_len));
  *psrc    = bound;
  *out_oid = oid;
}

/** @brief MC/DC + null-guard vectors for the book_src_paged bind guard. */
static void pbook_bind_guard_vectors(ra8_vmem_t* vm, uint32_t oid)
{
  /* MC/DC for book_src_paged's `(size == 0U) || (frame_bytes < 2)` guard
  ...
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 book_src_paged(nullptr, vm, oid, (uint32_t)k_pb_frame_bytes, s_pbook_len));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 book_src_paged(&bad, nullptr, oid, (uint32_t)k_pb_frame_bytes, s_pbook_len));
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 book_src_paged(&bad, vm, oid, (uint32_t)k_pb_frame_bytes, 0U));          /* V2 */
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, book_src_paged(&bad, vm, oid, 1U, s_pbook_len)); /* V3 */
}
                                  ...
                                  uint32_t          ch)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, book_chapter_text(book, ch, legacy, sizeof legacy, &l_len));
...
/**
 * @test test_book_paged_matches_resident
 * @brief Paged chapter text is byte-identical to the resident / legacy walk.
 *
 * @par MC/DC:
 * Decision: `(size == 0U) || (frame_bytes < k_book_paged_min_frame)`
 * (2 conditions) in apps/shared_libs/book/src/book_paged.c@book_src_paged:
 ...
 */
static void test_book_paged_matches_resident(void)
{
  TEST_BEGIN("book paged chapter text == resident");
  ...
  size_t tlen    = 0U;
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 book_chapter_text_src(&psrc, 0U, tiny, sizeof tiny, &tlen));

  TEST_END("book paged chapter text == resident");
}

/**
 * @test test_book_paged_guards
 ...
 * is covered by test_book_paged_matches_resident)
 */
static void test_book_paged_guards(void)
{
  TEST_BEGIN("book paged source guards");

  static pbook_t s_book;
  pbook_setup(&s_book);

  book_src_t rsrc = {};
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, book_src_resident(nullptr, &s_book, (uint32_t)sizeof(s_book)));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, book_src_resident(&rsrc, nullptr, (uint32_t)sizeof(s_book)));
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, book_src_resident(&rsrc, &s_book, 0U));
  TEST_ASSERT_EQ(k_ra8_ok, book_src_resident(&rsrc, &s_book, (uint32_t)sizeof(s_book)));

  /* Read primitive: range + null guards. */
  uint8_t dst[16] = {};
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, book_src_read(nullptr, 0U, dst, 1U));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, book_src_read(&rsrc, 0U, nullptr, 1U));
  TEST_ASSERT_EQ(k_ra8_err_out_of_range, book_src_read(&rsrc, (uint32_t)sizeof(s_book), dst, 1U));
  TEST_ASSERT_EQ(k_ra8_ok, book_src_read(&rsrc, 0U, dst, 8U)); /* header magic */
  TEST_ASSERT(internal_objects_equal(dst, "RABOOK1", 7U));
  TEST_ASSERT_EQ(k_ra8_ok, book_src_read(&rsrc, 0U, dst, 0U)); /* len==0 short-circuit */

  /* chapter_text_src: arg + range guards. */
  char   out[k_pbook_name_cap] = {};
  size_t len                   = 0U;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, book_chapter_text_src(nullptr, 0U, out, sizeof out, &len));
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg,
                 book_chapter_text_src(&rsrc, s_book.hdr.chapter_count, out, sizeof out, &len));

  TEST_END("book paged source guards");
}

/**
 * @brief One-chapter fixture whose body is a single long text run.
 * @details The run is longer than one paged staging chunk
 *          (k_book_paged_strbuf - 1 == 255) so the paged emitter chunks it,
...
static void plbook_setup(plbook_t* b)
{
  ...
  (void)memcpy(&b->strings[cur], "div", 4U);
  cur += 4U;
  ...
  (void)memcpy(&b->strings[cur], "END", 4U);
  cur += 4U;
...
/**
 * @test test_book_paged_long_run
 * @brief A text run spanning multiple paged chunks stays byte-identical.
 *
 * @par Coverage:
 * Drives book_paged_emit_run's multi-chunk path (the "no NUL in chunk" arm:
 ...
 */
static void test_book_paged_long_run(void)
{
  TEST_BEGIN("book paged long text run == resident");

  static plbook_t s_b;
  plbook_setup(&s_b);
  TEST_ASSERT_EQ(k_ra8_ok, book_validate(&s_b, sizeof(s_b)));

  s_pbook_bytes = (const uint8_t*)&s_b;
  s_pbook_len   = (uint32_t)sizeof(s_b);

  book_src_t rsrc = {};
  TEST_ASSERT_EQ(k_ra8_ok, book_src_resident(&rsrc, &s_b, (uint32_t)sizeof(s_b)));
  ...
  size_t r_len                   = 0U;
  size_t p_len                   = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, book_chapter_text_src(&rsrc, 0U, res, sizeof res, &r_len));
  TEST_ASSERT_EQ(k_ra8_ok, book_chapter_text_src(&psrc, 0U, pag, sizeof pag, &p_len));
  ...
  TEST_END("book paged long text run == resident");
}

/**
 * @test test_book_paged_read_faults
 * @brief Unbound-source guard and paged backing-fault propagation.
 *
 * @par Coverage:
 * Two otherwise-unreached error paths in book_paged.c:
 *  - ::book_src_read 's `(src->base == NULL) && (src->vm == NULL)` arm, which
 ...
 *    ::book_chapter_text_src propagates a non-ok code instead of `k_ra8_ok`.
 *
 * @par MC/DC:
 ...
 */
static void test_book_paged_read_faults(void)
{
  TEST_BEGIN("book paged read faults propagate");
  ...
  uint8_t dst[8]    = {};
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, book_src_read(&orphan, 0U, dst, (uint32_t)sizeof(dst)));
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    book_src_paged(&psrc, &vm, oid, (uint32_t)k_pb_frame_bytes, (uint32_t)sizeof(s_book)));
  ...
  size_t          olen                    = 0U;
  const ra8_err_t e = book_chapter_text_src(&psrc, 0U, out, sizeof(out), &olen);
  ...
  TEST_END("book paged read faults propagate");
}

int main(void)
{
  test_book_paged_matches_resident();
  test_book_paged_guards();
  test_book_paged_long_run();
  test_book_paged_read_faults();
  return 0;
}
