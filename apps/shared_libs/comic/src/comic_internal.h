 ...
 *
 * @details
 * `comic.c` owns the public API, the shared page-index storage, the entry-name
 * image filter, and the name-sort. Each container backend -- `comic_cbz.c`
 * (miniz ZIP) and `comic_cbr.c` (RAR via `ra8_rar.h`) -- enumerates its image
 ...
 *          ::comic_page_t slot with the backend bookkeeping. Rejects an index
 ...
 *          `epub_miniz_alloc` static pool (no heap).
 ...
 * @details Iterates the tar member chain (`unarch_tar_next`) from offset 0,
 ...
 * @param[in,out] c Comic with `c->tar` bound by `unarch_tar_open`,
 *                  `kind == cbt`.
 * @return ra8_err_t status.
 * @retval k_ra8_ok                Archive walked; index populated.
 * @retval k_ra8_err_invalid_size  The page index / name arena was too small.
 ...
 * @pre `c->tar` was bound by `unarch_tar_open`; buffers are bound.
 * @pre `c->size` is the archive length.
 * @post On k_ra8_ok, the index holds every image member in archive order.
 * @post On any error the index is left partially filled (caller discards @p c).
 * @note Not thread-safe.
 ...
 * @details Streams the member's literal bytes through ::unarch_tar_read,
