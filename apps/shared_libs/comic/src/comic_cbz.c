 ...
 * path `epub_open_streamed` uses -- so only the central directory and one
...
static_assert(k_comic_zip_bytes >= sizeof(mz_zip_archive),
              "k_comic_zip_bytes too small for mz_zip_archive");
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static mz_zip_archive* internal_zip(comic_t* c)
{
  /* Reinterpret the aligned byte arena as the archive via a `void*` seam (as
   * `epub` does): a direct `uint8_t* -> mz_zip_archive*` cast would trip
 ...
 * @param[in]  opaque   The comic's ::comic_stream_t (via `m_pIO_opaque`).
...
{
  const comic_stream_t* sm = (const comic_stream_t*)opaque;
  if (sm == nullptr) {
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_set_alloc(comic_t* c, mz_zip_archive* zip)
{
  const ra8_err_t err = epub_miniz_arena_init(&c->miniz_arena,
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_add_entry(comic_t* c, mz_zip_archive* zip, mz_uint i)
{
...
RA8_PRIV ra8_err_t priv_comic_cbz_open(comic_t* c)
{
  ...
  if (mz_zip_reader_init(zip, (mz_uint64)c->size, 0U) == MZ_FALSE) {
    epub_miniz_arena_deinit(&c->miniz_arena);
...
RA8_PRIV ra8_err_t
priv_comic_cbz_extract(comic_t* c, const comic_page_t* p, uint8_t* buf, size_t cap, size_t* got)
{
...
RA8_PRIV ra8_err_t priv_comic_cbz_close(comic_t* c)
{
