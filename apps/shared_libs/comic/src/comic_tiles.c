 ...
 *
 * @details
 * Implements ::comic_tile_reader_t. On import the page's encoded bytes are
 * streamed through `jof_produce()` into a caller memstore, the produced
 * atlas is validated with `jof_parse()`, and bound as the reader's single
 * source under a fresh epoch. ::comic_tiles_tile then pages the page's
 * full-resolution tiles through the owned ::ra8_tile_cache, decode-on-miss via
 * `jof_read_tile()` -- one bounded read plus one bounded inflate per tile.
 * The whole page is never resident; the resident cost is the cache cell budget.
 *
 * The size-threshold front end (::comic_tiles_footprint /
 * ::comic_tiles_over_budget) keeps small pages on the caller's existing
 ...
 * `jof` still links this TU empty (a comic-only reader keeps the
 ...
 * @brief ::jof_pull_fn adapter over an in-memory encoded page.
 * @details The comic page's encoded bytes are already extracted into a caller
 *          buffer by `comic_page_read`; this feeds them to the producer's
 ...
 * @param[in]  ctx A ::comic_tiles_pull_t.
...
{
  comic_tiles_pull_t* p      = (comic_tiles_pull_t*)ctx;
...
RA8_INTERNAL static ra8_err_t internal_tile_decode(void*                 ctx,
                                                   ...
                                                   uint16_t*             out_h)
{
  comic_tile_reader_t* r   = (comic_tile_reader_t*)ctx;
  const ra8_err_t      err = jof_read_tile(jof_memstore_pread,
                                           &r->store,
                                           &r->info,
                                           key->tile_x,
                                           key->tile_y,
                                ...
                                uint64_t*      out_decoded_bytes)
{
  ...
  *out_w             = w;
  *out_h             = h;
  *out_decoded_bytes = (uint64_t)w * (uint64_t)h * (uint64_t)k_comic_tiles_decoded_bpp;
  return k_ra8_ok;
}
...
/**
 * @brief Reject any NULL ::comic_tiles_import argument (guards, split out).
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_import_args_ok(const comic_tile_reader_t*      r,
                                                      const uint8_t*                  enc,
 ...
 *          re-validates it with `jof_parse()` into `r->info`.
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_transcode(comic_tile_reader_t*            r,
                                                 const uint8_t*                  enc,
                                                 size_t                          enc_len,
                                                 const comic_tiles_import_cfg_t* cfg)
{
  r->store = (jof_memstore_t){.buf = cfg->atlas, .cap = cfg->atlas_cap, .len = 0U};
  comic_tiles_pull_t      pull = {.d = enc, .n = enc_len, .off = 0U};
  ...
  jof_info_t      produced = {};
  const ra8_err_t perr     = jof_produce(&pcfg, &produced);
  if (perr != k_ra8_ok) {
    return perr;
  }
  return jof_parse(jof_memstore_pread, &r->store, (uint64_t)r->store.len, &r->info);
}
