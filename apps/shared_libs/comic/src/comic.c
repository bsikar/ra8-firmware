 ...
 *
 * @details
 * The container-agnostic half of `comic.h`. ::comic_open reads the leading
 ...
 * the sorted names. ::comic_page_read / ::comic_page_info / ::comic_close
...
RA8_PRIV ra8_err_t priv_comic_page_add(comic_t*    c,
                                       const char* name,
                                       uint16_t    name_len,
                                       uint64_t    raw_size,
                                       uint8_t     extractable,
                                       ...
                                       uint64_t    pack_size)
{
  ...
  comic_page_t* p = &c->pages[c->page_count];
  *p              = (comic_page_t){};
...
internal_name_cmp(const char* a, uint16_t alen, const char* b, uint16_t blen)
{
  ...
  int            c = 0;
  for (uint16_t i = 0U; (i < m) && (c == 0); ++i) {
    c = (int)(uint8_t)a[i] - (int)(uint8_t)b[i];
  }
  if (c != 0) {
    return (c < 0) ? -1 : 1;
  }
...
/**
 * @brief Reject any NULL required argument to ::comic_open.
 ...
 * @pre The caller forwards ::comic_open's pointer arguments unchanged.
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_open_reject_null(const comic_t*      c,
                                                        comic_read_fn       read,
                                                        const comic_page_t* pages,
                                                        const char*         names)
{
  RA8_CHECK_NULL_PTR(c, s_tag_comic, "open: null c");
  RA8_CHECK_NULL_PTR(read, s_tag_comic, "open: null read");
 ...
 *          page index populated (unsorted). Split from ::comic_open so the
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_open_detect(comic_t* c)
{
  ...
  const ra8_err_t terr = unarch_tar_open(&c->tar, c->read, c->ctx, c->size, nullptr);
  if (terr == k_ra8_ok) {
    c->kind = k_comic_kind_cbt;
                          ...
                          uint8_t*       out_extractable)
{
  ...
  const comic_page_t* p = &c->pages[page];
  if (name_buf != nullptr) {
    ...
    *out_name_len = p->name_len;
    ...
    /* The caller did not request a page name or its length. */
  }
...
ra8_err_t comic_page_read(comic_t* c, uint32_t page, uint8_t* buf, size_t cap, size_t* got)
{
  ...
  *got = 0U;
  ...
  const comic_page_t* p = &c->pages[page];
  if (c->kind == k_comic_kind_cbz) {
    return priv_comic_cbz_extract(c, p, buf, cap, got);
  }
  if (c->kind == k_comic_kind_cbt) {
