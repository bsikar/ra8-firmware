 ...
 * @details Centralizes the parser's permitted bytewise object copy.
 ...
 * @details Performs an exact bytewise equality check over two bounded spans.
 * @param[in] lhs First span of @p len readable bytes.
 * @param[in] rhs Second span of @p len readable bytes.
 * @param[in] len Number of bytes to compare.
 * @return Whether every corresponding byte is equal.
 * @retval true The byte spans are identical.
 * @retval false At least one byte differs.
 * @pre @p lhs is readable for @p len bytes.
 * @pre @p rhs is readable for @p len bytes.
 * @post Neither input span is modified.
 * @post The result depends only on the supplied bytes and length.
 * @note Pure and thread-safe for immutable input spans.
 * @since Version 0.1.0
 */
RA8_INTERNAL static bool internal_bytes_equal(const void* lhs, const void* rhs, size_t len)
{
...
static bool internal_vint(const uint8_t* buf, size_t len, size_t* pos, uint64_t* val)
{
  ...
  for (uint16_t i = 0U; i < (uint16_t)k_ra8_rar_vint_max; ++i) {               /* <=10 bytes */
    ...
    *pos += 1U;
    const uint32_t d = (uint32_t)b & (uint32_t)k_rar_vint_data;
    if (shift < hi_bit) {
    ...
    if ((b & (uint8_t)k_rar_vint_cont) == 0U) {
      internal_copy_object(val, words, sizeof(*val)); /* LE host + target: word[0] low */
      return true;
    }
...
static void internal_rar_match_signature(const uint8_t* hdr, size_t got, ra8_rar_t* rar)
{
  ...
  if (got >= (size_t)k_ra8_rar_sig5_len) {
    if (internal_bytes_equal(hdr, k_sig5, (size_t)k_ra8_rar_sig5_len)) {
      rar->version   = k_ra8_rar_ver_5;
      rar->first_off = (uint64_t)k_ra8_rar_sig5_len;
      return;
    }
  }
  if (internal_bytes_equal(hdr, k_sig4, (size_t)k_ra8_rar_sig4_len)) {
...
ra8_err_t ra8_rar_open(ra8_rar_t* rar, ra8_rar_read_fn read, void* ctx, uint64_t size)
{
  ...
  RA8_CHECK_NULL_PTR(read, s_tag_rar, "open: null read");
  *rar = (ra8_rar_t){};
