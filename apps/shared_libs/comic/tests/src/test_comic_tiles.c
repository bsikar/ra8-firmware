 ...
 *   - the tile path (`comic_tiles_*`) opens the same large page, and every
 ...
 * size-threshold decision (`comic_tiles_over_budget`) carries an MC/DC
...
RA8_INTERNAL static uint8_t internal_pix_rgb(uint32_t x, uint32_t y, uint32_t ch)
{
  if (ch == (uint32_t)k_chan_r) {
    return (uint8_t)(((x * (uint32_t)k_pat_r_x) + (y * (uint32_t)k_pat_r_y)) &
                     (uint32_t)k_byte_mask);
  }
  if (ch == (uint32_t)k_chan_g) {
    return (uint8_t)(((x * (uint32_t)k_pat_g_x) + (y * (uint32_t)k_pat_g_y)) &
                     (uint32_t)k_byte_mask);
  }
 ...
 * @details Implements comic_read_fn directly over immutable s_arc bytes.
 ...
 * @retval comic_tiles_import_cfg_t Configuration bound to file-local storage.
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static comic_tiles_import_cfg_t internal_import_cfg(uint8_t codec)
{
  static uint8_t s_work[k_work_cap];
  static uint8_t s_atlas[k_atlas_cap];
  return (comic_tiles_import_cfg_t){
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static size_t internal_open_big_page_bytes(comic_t* comic)
{
  const ra8_err_t oerr = comic_open(comic,
                                    internal_arc_read,
                                    nullptr,
                                    (uint64_t)s_arc_size,
                                    s_pages,
  ...
  TEST_ASSERT_EQ(2U, comic_page_count(comic));
  size_t          got  = 0U;
  const ra8_err_t rerr = comic_page_read(comic, 0U, s_pagebuf, sizeof s_pagebuf, &got);
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static void internal_walk_and_verify_tiles(comic_tile_reader_t* r,
                                                        const jof_info_t*    info)
{
  for (uint16_t ty = 0U; ty < info->tile_rows; ty++) {
    for (uint16_t tx = 0U; tx < info->tile_cols; tx++) {
      ra8_tile_t t = {};
      TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_tile(r, tx, ty, &t));
      internal_verify_tile(&t, tx, ty);
      TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_release(r, t.pixels));
    }
  }
  ra8_tile_t a = {};
  ra8_tile_t b = {};
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_tile(r, 0U, 0U, &a));
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_release(r, a.pixels));
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_tile(r, 0U, 0U, &b));
  TEST_ASSERT(a.pixels == b.pixels);
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_release(r, b.pixels));
}
 ...
 * @pre @p r completed comic_tiles_init.
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static void internal_assert_import_cfg_guards(comic_tile_reader_t* r)
{
  comic_tiles_import_cfg_t no_work = internal_import_cfg((uint8_t)k_jof_codec_deflate);
  no_work.work                     = nullptr;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_import(r, s_png, 4U, &no_work));
  comic_tiles_import_cfg_t no_atlas = internal_import_cfg((uint8_t)k_jof_codec_deflate);
  no_atlas.atlas                    = nullptr;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_import(r, s_png, 4U, &no_atlas));
}
...
RA8_INTERNAL static void internal_test_footprint_and_budget(void)
{
  ...
  uint16_t h  = 0U;
  uint64_t db = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_footprint(s_png, s_png_len, &w, &h, &db));
  TEST_ASSERT_EQ(k_big_w, w);
  TEST_ASSERT_EQ(k_big_h, h);
  TEST_ASSERT_EQ(k_big_w * (uint64_t)k_big_h * (uint64_t)k_comic_tiles_decoded_bpp, db);

  /* MC/DC vectors for the threshold decision. */
  TEST_ASSERT(comic_tiles_over_budget((uint64_t)1024U * 1024U, (uint64_t)k_budget_bytes));
  TEST_ASSERT(!comic_tiles_over_budget((uint64_t)1024U * 1024U, 0U));
  TEST_ASSERT(!comic_tiles_over_budget((uint64_t)512U * 1024U, (uint64_t)1024U * 1024U));
  /* The big page is over budget; the small page is not. */
  TEST_ASSERT(comic_tiles_over_budget(db, (uint64_t)k_budget_bytes));
  internal_png_build_rgb((uint32_t)k_small_w, (uint32_t)k_small_h);
  uint64_t small_db = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_footprint(s_png, s_png_len, &w, &h, &small_db));
  TEST_ASSERT(!comic_tiles_over_budget(small_db, (uint64_t)k_budget_bytes));

  /* Guards -- every required pointer, and the zero-length / non-image paths. */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_footprint(nullptr, 4U, &w, &h, &db));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_footprint(s_png, 4U, nullptr, &h, &db));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_footprint(s_png, 4U, &w, nullptr, &db));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_footprint(s_png, 4U, &w, &h, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, comic_tiles_footprint(s_png, 0U, &w, &h, &db));
  static const uint8_t junk[8] = {'n', 'o', 't', 'i', 'm', 'a', 'g', 'e'};
  TEST_ASSERT_EQ(k_ra8_err_not_supported, comic_tiles_footprint(junk, sizeof junk, &w, &h, &db));
 ...
 *
 * @par MC/DC:
 * The `comic_tiles_over_budget` `&&` decision is exercised here only as a
...
RA8_INTERNAL static void internal_test_large_page_opens(void)
{
  ...
  uint16_t h  = 0U;
  uint64_t db = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_footprint(s_pagebuf, got, &w, &h, &db));
  TEST_ASSERT(comic_tiles_over_budget(db, (uint64_t)k_budget_bytes));
  ...
  comic_tile_reader_t  r       = {};
  ra8_tile_cache_cfg_t storage = internal_cache_cfg();
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_init(&r, &storage, s_scratch, (uint32_t)sizeof s_scratch));
  const comic_tiles_import_cfg_t cfg = internal_import_cfg((uint8_t)k_jof_codec_deflate);
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_import(&r, s_pagebuf, got, &cfg));

  jof_info_t info = {};
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_info(&r, &info));
...
RA8_INTERNAL static void internal_test_zoom_subrect(void)
{
  ...
  ra8_tile_cache_cfg_t storage = internal_cache_cfg();
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_init(&r, &storage, s_scratch, (uint32_t)sizeof s_scratch));
  const comic_tiles_import_cfg_t cfg = internal_import_cfg((uint8_t)k_jof_codec_raw);
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_import(&r, s_pagebuf, got, &cfg));

  /* Center-ish interior tile (2,1) fetched on its own -> native pixels. */
  ra8_tile_t t = {};
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_tile(&r, 2U, 1U, &t));
...
RA8_INTERNAL static void internal_test_tile_over_cell_budget(void)
{
  ...
  ra8_tile_t t = {};
  TEST_ASSERT_EQ(k_ra8_err_no_mem, comic_tiles_tile(&r, 0U, 0U, &t));
...
RA8_INTERNAL static void internal_test_small_page_flat(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 comic_open(&comic,
                            ...
                            (uint32_t)sizeof s_names));
  size_t got = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, comic_page_read(&comic, 1U, s_pagebuf, sizeof s_pagebuf, &got));
...
RA8_INTERNAL static void internal_test_page_turn_epoch(void)
{
  ...
  internal_png_build_rgb((uint32_t)k_big_w, (uint32_t)k_big_h);
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_import(&r, s_png, s_png_len, &cfg));
  const uint32_t epoch_a = r.epoch;
  ra8_tile_t     ta      = {};
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_tile(&r, 0U, 0U, &ta));
  const uint8_t a00 = ta.pixels[0];
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_release(&r, ta.pixels));
  ...
  ra8_tile_t tb = {};
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_tile(&r, 0U, 0U, &tb));
  ...
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_release(&r, tb.pixels));
...
RA8_INTERNAL static void internal_test_guards(void)
{
  ...
  /* init guards. */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_init(nullptr, &storage, s_scratch, 4U));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_init(&r, nullptr, s_scratch, 4U));
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_init(&r, &storage, s_scratch, (uint32_t)sizeof s_scratch));

  /* Fetching before an import is invalid_state. */
  ra8_tile_t t    = {};
  jof_info_t info = {};
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, comic_tiles_tile(&r, 0U, 0U, &t));
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, comic_tiles_info(&r, &info));

  /* import guards. */
  const comic_tiles_import_cfg_t cfg = internal_import_cfg((uint8_t)k_jof_codec_deflate);
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_import(nullptr, s_png, 4U, &cfg));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_import(&r, nullptr, 4U, &cfg));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_import(&r, s_png, 4U, nullptr));
  internal_assert_import_cfg_guards(&r);
  internal_png_build_rgb((uint32_t)k_small_w, (uint32_t)k_small_h);
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, comic_tiles_import(&r, s_png, 0U, &cfg));
  ...
  static const uint8_t junk[16] =
    {'n', 'o', 't', ' ', 'a', 'n', ' ', 'i', 'm', 'a', 'g', 'e', '!', '!', '!', '!'};
  TEST_ASSERT(comic_tiles_import(&r, junk, sizeof junk, &cfg) != k_ra8_ok);
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, comic_tiles_tile(&r, 0U, 0U, &t));

  /* A valid import, then out-of-range tile coordinates. */
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_import(&r, s_png, s_png_len, &cfg));
  TEST_ASSERT_EQ(k_ra8_ok, comic_tiles_info(&r, &info));
  TEST_ASSERT_EQ(k_ra8_err_out_of_range, comic_tiles_tile(&r, (uint16_t)info.tile_cols, 0U, &t));
  TEST_ASSERT_EQ(k_ra8_err_out_of_range, comic_tiles_tile(&r, 0U, (uint16_t)info.tile_rows, &t));

  /* Reader null guards. */
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_tile(nullptr, 0U, 0U, &t));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_tile(&r, 0U, 0U, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_info(&r, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_release(&r, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, comic_tiles_release(nullptr, s_scratch));

  /* A too-small atlas store makes the producer fail; reader stays unbound. */
  comic_tiles_import_cfg_t tiny = internal_import_cfg((uint8_t)k_jof_codec_deflate);
  tiny.atlas_cap                = 16U;
  internal_png_build_rgb((uint32_t)k_big_w, (uint32_t)k_big_h);
  TEST_ASSERT(comic_tiles_import(&r, s_png, s_png_len, &tiny) != k_ra8_ok);
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, comic_tiles_tile(&r, 0U, 0U, &t));
