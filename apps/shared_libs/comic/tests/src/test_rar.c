 ...
 *   2. through the `comic` facade opened by magic, proving a `.cbr` pages,
...
static size_t tr_vint(uint8_t* out, uint64_t v)
{
  ...
  uint64_t value = v;
  for (;;) {
    uint8_t b = (uint8_t)(value & k_t_vint_payload_mask);
    value >>= k_t_vint_shift;
    if (value != 0U) {
                       ...
                       bool           is_dir)
{
  ...
  /** Data-area-present header flag. */
  const uint64_t hflags = has_data ? UINT64_C(2) : UINT64_C(0);
  /** Directory file flag. */
  const uint64_t fflags = is_dir ? UINT64_C(1) : UINT64_C(0);
  ...
  // NOLINTNEXTLINE(bugprone-not-null-terminated-result) -- counted archive field deliberately carries no terminator.
  (void)memcpy(&body[b], name, nlen);
  b += nlen;

  size_t p = 4U; /* leave the header CRC32 as zero (reader does not verify it) */
  p += tr_vint(&out[p], (uint64_t)b);
  (void)memcpy(&out[p], body, b);
  p += b;
  if (has_data) {
    (void)memcpy(&out[p], data, dlen);
    p += dlen;
  }
  return p;
}
...
static size_t tr4_file(uint8_t* out, const char* name, const uint8_t* data, size_t dlen)
{
  ...
  (void)memcpy(&out[32], name, nlen);
  (void)memcpy(&out[head], data, dlen);
  return head + dlen;
}
...
static size_t tr4_main(uint8_t* out)
{
  ...
  (void)memset(&out[k_tr_off_high_pos_av], 0, k_tr_len_high_pos_av); /* HighPosAV + PosAV */
...
static void tr_build_rar5(void)
{
  ...
  size_t               p        = 0U;
  (void)memcpy(&s_arc[p], k_sig, sizeof(k_sig));
  p += sizeof(k_sig);
...
static void test_rar_open_version_detect(void)
{
  ...
  /* Not a RAR archive. */
  static const uint8_t k_junk[] = {'P', 'K', 0x03U, 0x04U, ' ', 't', 'h', 'i', 's', ' ',
                                   'i', 's', ' ',   'a',   ' ', 'z', 'i', 'p', ',', ' ',
                                   'n', 'o', 't',   ' ',   'a', ' ', 'r', 'a', 'r', '\0'};
...
static void tr_check_page(comic_t* c, uint32_t i, const char* name)
{
 ...
 *        decodes, the directory is skipped, and trailing junk after the end block
 *        does not discard the valid page index.
 *
 * @par MC/DC:
...
static void test_comic_cbr5_pages(void)
{
  ...
  tr_build_rar5();
  /* A tolerant CBR walk keeps valid pages when bytes after the end block are
   * too short to form another RAR header. */
  s_arc[s_arc_size] = 0xA5U;
  s_arc_size++;
  comic_t      c                    = {};
  comic_page_t pages[k_tr_page_cap] = {};
  ...
  TEST_ASSERT_EQ(k_comic_kind_cbr, comic_kind(&c));
  /* Four image members (dir + filler excluded). */
  TEST_ASSERT_EQ(k_tr_img_count, comic_page_count(&c));
...
{
  (void)memcpy(s_arc, k_tr_sig5, sizeof(k_tr_sig5));
  (void)memcpy(&s_arc[sizeof(k_tr_sig5)], block, blen);
...
{
  (void)memcpy(s_arc, k_tr_sig4, sizeof(k_tr_sig4));
  (void)memcpy(&s_arc[sizeof(k_tr_sig4)], block, blen);
...
static void test_rar_open_next_extract_edges(void)
{
  ...
  (void)memcpy(s_arc, k_tr_sig5, (size_t)k_tr_len_signature);
 ...
 * @since 0.1.0
 */
static ra8_err_t tr_open(comic_t* c, comic_page_t* pages, char* names)
{
  return comic_open(c,
                    tr_read,
                    nullptr,
...
/**
 * @test test_mcdc_comic_cbr_sort_and_empty
 ...
 *
 * @par MC/DC:
 * `apps/shared_libs/comic/src/comic.c@internal_name_cmp` receives a true/true
 * byte-compare control, an equal/prefix comparison that terminates with
 * `i < m` false while `c == 0`, and an `a`/`b` mismatch that makes `c == 0`
 * false while `i < m` remains true. These three vectors independently vary
 * both operands of the comparison loop. The same fixture also drives the
 * shorter, longer, and equal length-tiebreak arms.
 */
static void test_mcdc_comic_cbr_sort_and_empty(void)
{
  TEST_BEGIN("comic cbr: name-sort length tiebreak + no-image not_found");
  static const uint8_t k_one[1] = {};

  /* Prefix, duplicate, and first-byte-different members exercise both loop
   * operands as well as the comparator's length tiebreaks. */
  size_t p = 0U;
  (void)memcpy(&s_arc[p], k_tr_sig5, sizeof(k_tr_sig5));
  p += sizeof(k_tr_sig5);
  ...
  p += tr5_file(&s_arc[p], "a.png", k_one, sizeof(k_one), 0U, false);
  p += tr5_file(&s_arc[p], "b.png", k_one, sizeof(k_one), 0U, false);
  ...
  TEST_ASSERT_EQ(4U, comic_page_count(&c));
  char     nb[k_tr_name_buf] = {};
  uint16_t nl                = 0U;
  TEST_ASSERT_EQ(k_ra8_ok,
                 comic_page_info(&c, 0U, nb, (uint16_t)sizeof(nb), &nl, nullptr, nullptr));
  nb[nl] = '\0';
  ...
  comic_t      c2                    = {};
  comic_page_t pages2[k_tr_page_cap] = {};
...
int main(void)
{
  ...
  test_mcdc_comic_cbr_sort_and_empty();
  return 0;
}
