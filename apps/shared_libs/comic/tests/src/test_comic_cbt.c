 ...
 *      a tdefl DEFLATE body) opens through `comic_open_wrapped` into a
 *      caller arena,
...
/** @brief Compare byte representations through compatible pointer types. */
...
static comic_page_t s_pages[k_tcb_page_cap];
                                                ...
                                                size_t      n)
{
  ...
  size_t               cursor                = off;
  uint8_t*             b                     = &dst[cursor];
  (void)memset(b, 0, (size_t)k_tcb_tar_block);
  (void)strncpy((char*)&b[k_tcb_off_name], name, (size_t)k_tcb_len_name);
  ...
  (void)memcpy(&b[k_tcb_off_magic], s_tcb_ustar_magic, sizeof(s_tcb_ustar_magic));
  (void)memcpy(&b[k_tcb_off_version], s_tcb_ustar_version, sizeof(s_tcb_ustar_version));
  (void)memset(&b[k_tcb_off_chksum], (int)' ', k_tcb_len_chksum);
  uint32_t sum = 0U;
  ...
  cursor += k_tcb_tar_block;
  if (n > 0U) {
    (void)memcpy(&dst[cursor], data, n);
    const size_t padded = ((n + 511U) / 512U) * 512U;
    (void)memset(&dst[cursor + n], 0, padded - n);
    cursor += padded;
  }
  return cursor;
}
...
RA8_INTERNAL static size_t internal_tcb_build_tar(uint8_t* dst)
{
  ...
  (void)memset(&dst[off], 0, (size_t)k_tcb_end_marker_bytes);
 ...
 * @return Result from ::comic_open_wrapped.
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_tcb_open_wrapped(comic_t* c)
{
  return comic_open_wrapped(c,
                            internal_tcb_read,
                            nullptr,
                            (uint64_t)s_arc_len,
                            s_pages,
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_tcb_assert_two_pages(comic_t* c, comic_kind_t kind)
{
  TEST_ASSERT_EQ(kind, comic_kind(c));
  TEST_ASSERT_EQ(2U, comic_page_count(c));
  char     nb[k_tcb_name_cap] = {};
  uint16_t nl                 = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, comic_page_info(c, 0U, nb, sizeof(nb), &nl, nullptr, nullptr));
  TEST_ASSERT(internal_bytes_equal(nb, "page1.png", 9U)); /* sorted: page1 first */
  size_t got = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, comic_page_read(c, 0U, s_out, sizeof(s_out), &got));
  TEST_ASSERT_EQ(8U, got);
  TEST_ASSERT(internal_bytes_equal(s_out, "PAGE-ONE", 8U));
  TEST_ASSERT_EQ(k_ra8_ok, comic_page_read(c, 1U, s_out, sizeof(s_out), &got));
  TEST_ASSERT_EQ(8U, got);
  TEST_ASSERT(internal_bytes_equal(s_out, "PAGE-TWO", 8U));
...
RA8_INTERNAL static void internal_test_cbt_bare_tar_open(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 comic_open(&c,
                            ...
                            s_names,
                            (uint32_t)sizeof(s_names)));
  internal_tcb_assert_two_pages(&c, k_comic_kind_cbt);

  /* Non-archive bytes are still cleanly refused by every backend. */
  (void)memset(s_arc, k_tcb_fill_non_archive, (size_t)k_tcb_non_archive_bytes);
 ...
 *
 * @par MC/DC:
 * Decision: `!is_gzip && !is_xz` in `comic_open_wrapped` (2 conditions)
...
RA8_INTERNAL static void internal_test_cbt_wrapped_gzip_and_xz(void)
{
  ...
  (void)memcpy(s_arc, k_fx_xz_cbt, sizeof(k_fx_xz_cbt));
 ...
 *
 * @return The `comic_open_wrapped` result code.
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_tcb_open_full(comic_t*      c,
                                                     comic_read_fn read,
                                                     uint64_t      file_len,
                                                     uint8_t*      arena,
                                                     size_t        arena_len)
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_tcb_arg_guards(comic_t* c)
{
  const size_t tar_len = internal_tcb_build_tar(s_inner);
  internal_tcb_gzip_wrap(s_inner, tar_len);
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
  ...
  TEST_ASSERT_EQ(
    k_ra8_ok,
    internal_tcb_open_full(c, internal_tcb_read, (uint64_t)s_arc_len, s_arena, sizeof(s_arena)));
  TEST_ASSERT_EQ(k_ra8_ok, comic_close(c));
}
 ...
 *
 * @par MC/DC:
 * The former `inner_gzip || inner_xz` decision is now two sequential
 * single-condition guards in `internal_open_unwrapped`, so it no longer needs
 * MC/DC. Gzip-of-gzip and gzip-of-xz independently cover each depth guard;
 * gzip-of-tar covers the honest path into the inner container.
 * The remaining compound decision,
 * `apps/shared_libs/comic/src/comic_wrapped.c@internal_arena_valid` receives
 * aligned/full-capacity (T,T -> true), misaligned/full-capacity (F,- -> false),
 * and aligned/header-only (T,F -> false) arena vectors from
 * internal_tcb_arg_guards. The three calls are the minimal two-condition set.
...
RA8_INTERNAL static void internal_test_cbt_dead_walker_guards(void)
{
  ...
  comic_t c = {}; /* zero-init leaves tar.live == 0 */
