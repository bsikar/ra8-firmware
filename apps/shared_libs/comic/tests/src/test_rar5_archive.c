...
static comic_page_t s_fpages[16];
...
static void test_cbr_compressed_parity(void)
{
  ...
  size_t               p        = 0U;
  (void)memcpy(&s_farc[p], k_sig, sizeof(k_sig));
  p += sizeof(k_sig);
  ...
  TEST_ASSERT_EQ(1U, comic_page_count(&c));
  uint16_t nl  = 0U;
  uint64_t raw = 0U;
  uint8_t  ex  = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, comic_page_info(&c, 0U, nullptr, 0U, &nl, &raw, &ex));
  TEST_ASSERT_EQ(1U, ex); /* compressed RAR5 page is extractable */
  TEST_ASSERT_EQ(plen, raw);

  static uint8_t s_obuf[k_cf_png_max];
  (void)memset(s_obuf, 0, sizeof(s_obuf));
  size_t got = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, comic_page_read(&c, 0U, s_obuf, sizeof(s_obuf), &got));
...
static size_t a4_main(uint8_t* out)
{
  ...
  (void)memset(&out[k_av_off_high_pos_av], 0, k_av_len_high_pos_av);
 ...
 *
 * @par MC/DC:
 * Completes the apps/shared_libs/comic/src/comic_cbr.c@internal_add_member `is_store || is_rar5`
...
static void test_cbr_rar4_compressed_unsupported(void)
{
  ...
  size_t         got = 0U;
  TEST_ASSERT_EQ(k_ra8_err_not_supported, comic_page_read(&c, 0U, s_obuf, sizeof(s_obuf), &got));
  TEST_ASSERT_EQ(k_ra8_ok, comic_close(&c));
 ...
 *
 * @par MC/DC:
 * Decision apps/shared_libs/comic/src/ra8_rar.c@ra8_rar_extract:
