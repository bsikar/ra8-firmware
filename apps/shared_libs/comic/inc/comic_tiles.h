 ...
 *
 * @details
 * `comic` streams a page's *encoded* bytes out of a CBZ/CBR archive and the
 ...
 * through `jof_produce()` into a caller-supplied atlas store (an SDRAM
 ...
 * inflate per tile miss (`jof_read_tile()`), resident decoded pixels bounded
 ...
 * bytes*, not pixels -- ::comic_tiles_over_budget) and only routes a page
 ...
 *   // ... blit tile.pixels ...
 *   comic_tiles_release(&r, tile.pixels);
 * } else {
 ...
 *       reports `k_ra8_err_not_supported` from ::comic_tiles_footprint /
 *       ::comic_tiles_import and the caller keeps the whole-decode path.
 *
 * @see jof.h                The JOF atlas format + tile reader.
 * @see jof_produce.h        The import-time transcode producer.
 * @see ra8_tile_cache.h         The decode-on-demand tile cache.
 * @see epub_img_tiles.h     The EPUB tile binder this mirrors.
 * @see comic.h              The CBZ/CBR page reader that supplies the bytes.
 *
...
typedef enum : uint8_t {
  k_comic_tiles_decoded_bpp = 4U, /**< Bytes/pixel used for the budget estimate. */
} comic_tiles_const_t;
 ...
 *
 * @details Mirrors the producer configuration (`jof_produce.h`) minus the
 *          source (the import path streams the caller's encoded page bytes).
 *          Size @p work with `jof_work_bytes()` over the same caps and tile
 *          geometry. @p webp_work is the optional whole-frame arena for WebP
 *          pages (`jof_webp_work_bytes()`); leave it NULL to fail-closed
 ...
 * @see comic_tiles_init()
 * @since Version 0.1.0
 */
typedef struct {
  ra8_tile_cache_t cache;       /**< Owned tile cache (decode wired to this). */
  jof_memstore_t   store;       /**< Atlas backing for the bound page.        */
  jof_info_t       info;        /**< Parsed geometry of the bound page.       */
 ...
 * @see comic_tiles_over_budget()
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t comic_tiles_footprint(const uint8_t* enc,
                                              size_t         len,
                                              uint16_t*      out_w,
                                              uint16_t*      out_h,
 ...
 *
 * @pre @p decoded_bytes came from ::comic_tiles_footprint (or is 0).
 ...
 * @see comic_tiles_footprint()
 * @since Version 0.1.0
 */
[[nodiscard]] bool comic_tiles_over_budget(uint64_t decoded_bytes, uint64_t budget_bytes);
 ...
 *          with `jof_stored_bound()` over the cell size; a reader paging
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t comic_tiles_init(comic_tile_reader_t*        r,
 ...
 *
 * @details Streams @p enc through `jof_produce()` into @p cfg->atlas (a
 ...
 *
 * @param[in,out] r       Reader from ::comic_tiles_init.
 ...
 *
 * @pre @p r came from ::comic_tiles_init.
 * @pre @p cfg->work is sized per `jof_work_bytes()` for the same caps.
 ...
 *
 * @note Not thread-safe.
 * @see comic_tiles_tile()
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t comic_tiles_import(comic_tile_reader_t*            r,
                                           const uint8_t*                  enc,
                                           size_t                          enc_len,
 ...
 * @pre A page was bound via ::comic_tiles_import.
 ...
 *          (`jof_read_tile()`); on a hit the cell is reused. The returned
 *          pixels are tightly packed (`out_tile->width * bpp` bytes per row) and
 *          stay valid until ::comic_tiles_release. Edge tiles report their
 ...
 *
 * @pre @p r has a page bound via ::comic_tiles_import.
 * @pre The caller will ::comic_tiles_release the returned tile.
 * @post On success the cell's pin count grew by one.
 * @post On any error no new pin is held.
 *
 * @note Not thread-safe.
 * @see comic_tiles_release()
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t
 ...
 *
 * @pre @p pixels came from ::comic_tiles_tile on this reader.
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t comic_tiles_release(comic_tile_reader_t* r, const uint8_t* pixels);
