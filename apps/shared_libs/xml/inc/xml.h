...
typedef enum : uint16_t {
  k_xml_max_element_depth = 499U, /**< Accepted element levels, including root. */
  k_xml_workspace_frames  = 512U, /**< Physical frame capacity with headroom.   */
} xml_limits_t;
...
typedef struct {
  xml_frame_t frames[k_xml_workspace_frames]; /**< Open-element stack. */
} xml_workspace_t;

/** @brief Pull-event kind. */
typedef enum : uint8_t {
  k_xml_event_none  = 0U, /**< No event / end of source. */
  k_xml_event_start = 1U, /**< Element start.            */
  k_xml_event_end   = 2U, /**< Element end.              */
  k_xml_event_text  = 3U, /**< Character-data run.       */
  k_xml_event_cdata = 4U, /**< CDATA payload.            */
} xml_event_kind_t;

/** @brief One XML pull event. */
typedef struct {
  xml_span_t markup;          /**< Complete markup span, or text payload. */
  xml_span_t name;            /**< Element name for start/end.            */
  uint16_t   depth;           /**< Root is depth zero.                    */
  uint16_t   attribute_count; /**< Source-order attributes on start.      */
  uint8_t    kind;            /**< ::xml_event_kind_t.                    */
  uint8_t    self_closing;    /**< One for an empty-element start.        */
} xml_event_t;

/** @brief Source-order attribute view. */
typedef struct {
  xml_span_t name;  /**< Attribute name.                    */
  xml_span_t value; /**< Quoted value excluding delimiters. */
} xml_attribute_t;
...
typedef struct {
  ...
  xml_workspace_t* workspace;        /**< Caller-owned stack.            */
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t xml_reader_init(xml_reader_t*    reader,
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t xml_reader_next(xml_reader_t* reader, xml_event_t* out_event);
 ...
 */
[[nodiscard]] ra8_err_t
xml_validate(const uint8_t* source, size_t source_len, xml_workspace_t* workspace);
 ...
 * @pre @p event aliases the same live source later passed to ::xml_attr_next.
 ...
 * @param[in,out] cursor Cursor initialised by ::xml_attr_begin.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t xml_attr_next(const uint8_t*     source,
                                      size_t             source_len,
 ...
 */
[[nodiscard]] bool
xml_span_equal(const uint8_t* source, size_t source_len, xml_span_t span, const char* literal);
 ...
 * @since 0.1.0
 */
[[nodiscard]] bool xml_span_local_equal(const uint8_t* source,
                                        size_t         source_len,
                                        xml_span_t     span,
                                        const char*    literal);
 ...
 */
[[nodiscard]] bool
xml_decoded_equal(const uint8_t* source, size_t source_len, xml_span_t left, xml_span_t right);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t xml_decode(const uint8_t* source,
                                   size_t         source_len,
                                   xml_span_t     span,
                                   char*          destination,
                                   size_t         capacity,
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t xml_decode_prefix(const uint8_t* source,
                                          size_t         source_len,
                                          xml_span_t     span,
                                          char*          destination,
                                          size_t         capacity,
