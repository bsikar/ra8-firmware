 ...
 * @since 0.1.0
 */
RA8_PRIV bool priv_xml_span_valid(size_t source_len, xml_span_t span);

/**
 * @brief Compare bounded source bytes with an ASCII character sequence.
 * @details Compares byte by byte so plain-character literals are never passed
 *          to `memcmp()` alongside essentially-unsigned source bytes.
 * @param[in] source Immutable source bytes.
 * @param[in] offset First source byte to compare.
 * @param[in] literal Character sequence whose first @p length bytes are compared.
 * @param[in] length Number of bytes to compare.
 * @return True only when every byte matches.
 * @retval true The complete byte sequence matched.
 * @retval false At least one byte differed.
 * @pre @p source spans at least `offset + length` readable bytes.
 * @pre @p literal spans at least @p length readable characters.
 * @post No memory is modified.
 * @post The result depends only on the supplied byte sequences.
 * @note Pure and thread-safe.
 * @since 0.1.0
 */
RA8_PRIV bool
 ...
 * @since 0.1.0
 */
RA8_PRIV ra8_err_t priv_xml_qname(const uint8_t* source, size_t end, size_t start, size_t* out_end);
 ...
 * @since 0.1.0
 */
RA8_PRIV ra8_err_t priv_xml_raw(const uint8_t* source, size_t start, size_t end);
 ...
 * @since 0.1.0
 */
RA8_PRIV ra8_err_t priv_xml_doctype(xml_reader_t* reader);
