...
{
  static const uint8_t xml_open[]  = {'<', 'x', '>'};
  static const uint8_t xml_close[] = {'<', '/', 'x', '>'};
  size_t               used        = 0U;
  for (uint16_t i = 0U; i < levels; ++i) {
    TEST_ASSERT((used + 3U) <= capacity);
    (void)memcpy(&destination[used], xml_open, sizeof(xml_open));
    used += 3U;
  }
  for (uint16_t i = 0U; i < levels; ++i) {
    TEST_ASSERT((used + 4U) <= capacity);
    (void)memcpy(&destination[used], xml_close, sizeof(xml_close));
    used += 4U;
  }
  return used;
}
 ...
 * - apps/shared_libs/xml/src/xml.c@internal_start
 * - apps/shared_libs/xml/src/xml.c@internal_end
 * - apps/shared_libs/xml/src/xml.c@xml_reader_next
 * - apps/shared_libs/xml/src/xml.c@xml_validate
 */
RA8_INTERNAL static void internal_depth_bound(void)
{
  static uint8_t  s_source[4096];
  xml_workspace_t workspace = {};
  size_t length = internal_deep_document(s_source, sizeof(s_source), k_xml_max_element_depth);
  TEST_ASSERT(xml_validate(s_source, length, &workspace) == k_ra8_ok);
  length =
    internal_deep_document(s_source, sizeof(s_source), (uint16_t)(k_xml_max_element_depth + 1U));
  TEST_ASSERT(xml_validate(s_source, length, &workspace) == k_ra8_err_validation_failed);
}
 ...
 * Decisions:
 * - apps/shared_libs/xml/src/xml_decode.c@xml_span_equal
 * - apps/shared_libs/xml/src/xml.c@internal_attributes
 */
RA8_INTERNAL static void internal_independent_readers(void)
{
  ...
  xml_workspace_t      left_workspace  = {};
  xml_workspace_t      right_workspace = {};
  xml_reader_t         left_reader     = {};
  xml_reader_t         right_reader    = {};
  xml_event_t          event           = {};
  TEST_ASSERT(xml_reader_init(&left_reader, left, sizeof(left) - 1U, &left_workspace) == k_ra8_ok);
  TEST_ASSERT(xml_reader_init(&right_reader, right, sizeof(right) - 1U, &right_workspace) ==
              k_ra8_ok);
  TEST_ASSERT(xml_reader_next(&left_reader, &event) == k_ra8_ok);
  TEST_ASSERT(xml_span_equal(left, sizeof(left) - 1U, event.name, "a"));
  TEST_ASSERT(xml_reader_next(&right_reader, &event) == k_ra8_ok);
  TEST_ASSERT(xml_span_equal(right, sizeof(right) - 1U, event.name, "r"));
  TEST_ASSERT(xml_reader_next(&left_reader, &event) == k_ra8_ok);
  TEST_ASSERT(xml_span_equal(left, sizeof(left) - 1U, event.name, "b"));
  TEST_ASSERT(xml_reader_next(&right_reader, &event) == k_ra8_ok);
  TEST_ASSERT(xml_span_equal(right, sizeof(right) - 1U, event.name, "s"));
 ...
 * - apps/shared_libs/xml/src/xml.c@xml_reader_init
 * - apps/shared_libs/xml/src/xml.c@xml_reader_next
 * - apps/shared_libs/xml/src/xml.c@xml_attr_begin
 * - apps/shared_libs/xml/src/xml.c@xml_attr_next
 * - apps/shared_libs/xml/src/xml_decode.c@priv_xml_span_valid
 */
RA8_INTERNAL static void internal_public_reader_guards(void)
{
  ...
  xml_reader_t         reader    = {};
  xml_event_t          event     = {};
  TEST_ASSERT(xml_reader_init(nullptr, source, sizeof(source) - 1U, &workspace) ==
              k_ra8_err_null_ptr);
  TEST_ASSERT(xml_reader_init(&reader, nullptr, sizeof(source) - 1U, &workspace) ==
              k_ra8_err_null_ptr);
  TEST_ASSERT(xml_reader_init(&reader, source, sizeof(source) - 1U, nullptr) == k_ra8_err_null_ptr);
  TEST_ASSERT(xml_reader_init(&reader, source, 0U, &workspace) == k_ra8_err_invalid_size);
  TEST_ASSERT(xml_reader_init(&reader, source, sizeof(source) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_reader_next(nullptr, &event) == k_ra8_err_null_ptr);
  TEST_ASSERT(xml_reader_next(&reader, nullptr) == k_ra8_err_null_ptr);
  TEST_ASSERT(xml_reader_next(&reader, &event) == k_ra8_ok);

  xml_attr_cursor_t cursor = {.position = 7U, .emitted = 8U};
  xml_attr_begin(nullptr, &cursor);
  TEST_ASSERT((cursor.position == 7U) && (cursor.emitted == 8U));
  xml_attr_begin(&event, nullptr);
  xml_attr_begin(&event, &cursor);
  xml_attribute_t attribute = {};
  bool            present   = false;
  TEST_ASSERT(xml_attr_next(nullptr, sizeof(source) - 1U, &event, &cursor, &attribute, &present) ==
              k_ra8_err_null_ptr);
  TEST_ASSERT(xml_attr_next(source, sizeof(source) - 1U, nullptr, &cursor, &attribute, &present) ==
              k_ra8_err_null_ptr);
  TEST_ASSERT(xml_attr_next(source, sizeof(source) - 1U, &event, nullptr, &attribute, &present) ==
              k_ra8_err_null_ptr);
  TEST_ASSERT(xml_attr_next(source, sizeof(source) - 1U, &event, &cursor, nullptr, &present) ==
              k_ra8_err_null_ptr);
  TEST_ASSERT(xml_attr_next(source, sizeof(source) - 1U, &event, &cursor, &attribute, nullptr) ==
              k_ra8_err_null_ptr);
  TEST_ASSERT((xml_attr_next(source, sizeof(source) - 1U, &event, &cursor, &attribute, &present) ==
               k_ra8_ok) &&
              present);
  xml_event_t forged   = event;
  forged.markup.offset = UINT32_MAX;
  xml_attr_begin(&forged, &cursor);
  TEST_ASSERT(xml_attr_next(source, sizeof(source) - 1U, &forged, &cursor, &attribute, &present) ==
 ...
 * - apps/shared_libs/xml/src/xml_decode.c@internal_digit
 * - apps/shared_libs/xml/src/xml_decode.c@internal_entity
 * - apps/shared_libs/xml/src/xml_decode.c@internal_decode
 * - apps/shared_libs/xml/src/xml_decode.c@internal_decode_one
 * - apps/shared_libs/xml/src/xml_decode.c@xml_decode
 * - apps/shared_libs/xml/src/xml.c@internal_attr_parse
 * - apps/shared_libs/xml/src/xml.c@internal_attr_value
 * - apps/shared_libs/xml/src/xml.c@internal_attr_duplicate
 * - apps/shared_libs/xml/src/xml.c@internal_attributes
 * - apps/shared_libs/xml/src/xml.c@internal_text
 */
RA8_INTERNAL static void internal_entities_and_faults(void)
{
  ...
  xml_workspace_t workspace = {};
  TEST_ASSERT(xml_validate(source, sizeof(source) - 1U, &workspace) == k_ra8_ok);
  ...
  TEST_ASSERT(xml_attr_next(source, sizeof(source) - 1U, &event, &cursor, &attribute, &present) ==
              k_ra8_ok);
  TEST_ASSERT(present && xml_span_equal(source, sizeof(source) - 1U, attribute.name, "a"));
  char   decoded[8]  = {};
  size_t decoded_len = 0U;
  TEST_ASSERT(xml_decode(source,
                         sizeof(source) - 1U,
                         attribute.value,
                         decoded,
                         sizeof(decoded),
  ...
  TEST_ASSERT(xml_validate(mismatch, sizeof(mismatch) - 1U, &workspace) ==
              k_ra8_err_validation_failed);
  TEST_ASSERT(xml_validate(bad_entity, sizeof(bad_entity) - 1U, &workspace) ==
              k_ra8_err_validation_failed);
  TEST_ASSERT(xml_validate(duplicate, sizeof(duplicate) - 1U, &workspace) ==
              k_ra8_err_validation_failed);
}
 ...
 * operands. Decisions: apps/shared_libs/xml/src/xml_decode.c@xml_decoded_equal
 */
RA8_INTERNAL static void internal_entity_comparison(void)
{
  static const uint8_t source[]  = {'<',  'r', ' ', 'a',  '=',  '\'', 0xC3U, 0xA9U,
                                    '\'', ' ', 'b', '=',  '\'', '&',  '#',   'x',
                                    'E',  '9', ';', '\'', '/',  '>',  0U};
  xml_workspace_t      workspace = {};
  xml_reader_t         reader    = {};
  ...
  TEST_ASSERT(xml_reader_next(&reader, &event) == k_ra8_ok);
  xml_attr_cursor_t cursor  = {};
  xml_attribute_t   left    = {};
  xml_attribute_t   right   = {};
  bool              present = false;
  xml_attr_begin(&event, &cursor);
  TEST_ASSERT(
    (xml_attr_next(source, sizeof(source) - 1U, &event, &cursor, &left, &present) == k_ra8_ok) &&
    present);
  TEST_ASSERT(
    (xml_attr_next(source, sizeof(source) - 1U, &event, &cursor, &right, &present) == k_ra8_ok) &&
    present);
  TEST_ASSERT(xml_decoded_equal(source, sizeof(source) - 1U, left.value, right.value));
}
...
{
  xml_workspace_t workspace = {};
  TEST_ASSERT(xml_validate(source, length, &workspace) == k_ra8_err_validation_failed);
}
 ...
 * - apps/shared_libs/xml/src/xml.c@internal_space
 * - apps/shared_libs/xml/src/xml.c@internal_ascii_letter
 * - apps/shared_libs/xml/src/xml.c@internal_name_start
 * - apps/shared_libs/xml/src/xml.c@internal_name_continue
 * - apps/shared_libs/xml/src/xml.c@priv_xml_qname
 * - apps/shared_libs/xml/src/xml_decode.c@internal_xml_char
 * - apps/shared_libs/xml/src/xml_decode.c@internal_utf8_next
 * - apps/shared_libs/xml/src/xml_decode.c@internal_utf8_lead
 * - apps/shared_libs/xml/src/xml.c@internal_end
 * - apps/shared_libs/xml/src/xml.c@internal_comment
 * - apps/shared_libs/xml/src/xml.c@internal_xml_target
 * - apps/shared_libs/xml/src/xml.c@internal_special
 * - apps/shared_libs/xml/src/xml.c@internal_text
 */
RA8_INTERNAL static void internal_hostile_syntax(void)
{
  ...
  static const uint8_t invalid_attr[] =
    {'<', 'a', ' ', 'x', '=', '\'', 0xC0U, 0x80U, '\'', '/', '>'};
  xml_workspace_t workspace = {};
  TEST_ASSERT(xml_validate(close_space, sizeof(close_space) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_validate(good_colon, sizeof(good_colon) - 1U, &workspace) == k_ra8_ok);
...
RA8_INTERNAL static void internal_character_classes(void)
{
  ...
  xml_workspace_t      workspace       = {};
  TEST_ASSERT(xml_validate(names, sizeof(names) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_validate(entities, sizeof(entities) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_validate(utf8, sizeof(utf8) - 1U, &workspace) == k_ra8_ok);
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static void internal_declaration_accepts_bom(void)
{
  static const uint8_t bom_tail[] = "<?xml version='1.0' encoding='UTF-8'?><!DOCTYPE a><a/>";
  uint8_t              bom[3U + sizeof(bom_tail)] = {};
  bom[0]                                          = (uint8_t)k_priv_utf8_bom_first;
  bom[1]                                          = (uint8_t)k_priv_utf8_bom_second;
  bom[2]                                          = (uint8_t)k_priv_utf8_bom_third;
  (void)memcpy(&bom[3], bom_tail, sizeof(bom_tail));
  xml_workspace_t workspace = {};
  TEST_ASSERT(xml_validate(bom, sizeof(bom) - 1U, &workspace) == k_ra8_ok);
}
 ...
 * - apps/shared_libs/xml/src/xml.c@internal_markup_end
 * - apps/shared_libs/xml/src/xml.c@internal_xml_target
 * - apps/shared_libs/xml/src/xml.c@internal_encoding
 * - apps/shared_libs/xml/src/xml.c@internal_declaration_attr
 * - apps/shared_libs/xml/src/xml.c@internal_declaration
 * - apps/shared_libs/xml/src/xml.c@internal_pi
 * - apps/shared_libs/xml/src/xml.c@internal_special
 * - apps/shared_libs/xml/src/xml.c@xml_reader_init
 * - apps/shared_libs/xml/src/xml_doctype.c@internal_space
 * - apps/shared_libs/xml/src/xml_doctype.c@internal_skip_space
 * - apps/shared_libs/xml/src/xml_doctype.c@internal_pubid_byte
 * - apps/shared_libs/xml/src/xml_doctype.c@internal_literal
 * - apps/shared_libs/xml/src/xml_doctype.c@internal_keyword
 * - apps/shared_libs/xml/src/xml_doctype.c@internal_external_id
 * - apps/shared_libs/xml/src/xml_doctype.c@priv_xml_doctype
 */
RA8_INTERNAL static void internal_declarations(void)
{
  ...
  static const uint8_t duplicate_bom[] =
    {0xEFU, 0xBBU, 0xBFU, 0xEFU, 0xBBU, 0xBFU, '<', 'a', '/', '>', 0U};
  static const uint8_t misplaced_bom[] = {' ', 0xEFU, 0xBBU, 0xBFU, '<', 'a', '/', '>', 0U};
  static const uint8_t feff_text[] = {'<', 'a', '>', 0xEFU, 0xBBU, 0xBFU, '<', '/', 'a', '>', 0U};
  xml_workspace_t      workspace   = {};
  TEST_ASSERT(xml_validate(declaration, sizeof(declaration) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_validate(lower_encoding, sizeof(lower_encoding) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_validate(doctype, sizeof(doctype) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_validate(system_id, sizeof(system_id) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_validate(public_id, sizeof(public_id) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_validate(valid_pi, sizeof(valid_pi) - 1U, &workspace) == k_ra8_ok);
  internal_declaration_accepts_bom();
  TEST_ASSERT(xml_validate(feff_text, sizeof(feff_text) - 1U, &workspace) == k_ra8_ok);
 ...
 * - apps/shared_libs/xml/src/xml_decode.c@internal_decode
 * - apps/shared_libs/xml/src/xml_decode.c@internal_decode_one
 * - apps/shared_libs/xml/src/xml_decode.c@xml_decode_prefix
 * - apps/shared_libs/xml/src/xml_decode.c@xml_decoded_size
 * - apps/shared_libs/xml/src/xml_decode.c@xml_span_equal
 * - apps/shared_libs/xml/src/xml_decode.c@xml_span_local_equal
 * - apps/shared_libs/xml/src/xml_decode.c@xml_decoded_equal
 */
RA8_INTERNAL static void internal_span_bounds_and_prefix(void)
{
  ...
  char   decoded[5] = {};
  size_t length     = 0U;
  TEST_ASSERT(xml_decode_prefix(source,
                                sizeof(source) - 1U,
                                attribute.value,
                                decoded,
                                sizeof(decoded),
  ...
  const xml_span_t forged = {UINT32_MAX, 8U};
  TEST_ASSERT(!xml_span_equal(source, sizeof(source) - 1U, forged, "x"));
  TEST_ASSERT(xml_decoded_size(source, sizeof(source) - 1U, forged, &length) ==
              k_ra8_err_validation_failed);
}
...
RA8_INTERNAL static void internal_public_span_guards(void)
{
  ...
  const xml_span_t     name       = {0U, 4U};
  const xml_span_t     left       = {0U, 6U};
  const xml_span_t     right      = {6U, 6U};
  const xml_span_t     different  = {12U, 1U};
  const xml_span_t     forged     = {UINT32_MAX, 8U};
  char                 decoded[3] = {};
  size_t               length     = 0U;
  TEST_ASSERT((xml_decode(values, sizeof(values) - 1U, left, decoded, sizeof(decoded), &length) ==
               k_ra8_ok) &&
              (length == 2U) && (strcmp(decoded, "A&") == 0));
  TEST_ASSERT(xml_decode(values, sizeof(values) - 1U, left, decoded, 2U, &length) ==
              k_ra8_err_no_mem);
  TEST_ASSERT(xml_decode(nullptr, sizeof(values) - 1U, left, decoded, sizeof(decoded), &length) ==
              k_ra8_err_null_ptr);
  TEST_ASSERT(xml_decode(values, sizeof(values) - 1U, left, nullptr, sizeof(decoded), &length) ==
              k_ra8_err_null_ptr);
  TEST_ASSERT(xml_decode(values, sizeof(values) - 1U, left, decoded, sizeof(decoded), nullptr) ==
              k_ra8_err_null_ptr);
  TEST_ASSERT(
    xml_decode_prefix(nullptr, sizeof(values) - 1U, left, decoded, sizeof(decoded), &length) ==
    k_ra8_err_null_ptr);
  TEST_ASSERT(
    xml_decode_prefix(values, sizeof(values) - 1U, left, nullptr, sizeof(decoded), &length) ==
    k_ra8_err_null_ptr);
  TEST_ASSERT(
    xml_decode_prefix(values, sizeof(values) - 1U, left, decoded, sizeof(decoded), nullptr) ==
    k_ra8_err_null_ptr);
  TEST_ASSERT(xml_decoded_size(nullptr, sizeof(values) - 1U, left, &length) == k_ra8_err_null_ptr);
  TEST_ASSERT(xml_decoded_size(values, sizeof(values) - 1U, left, nullptr) == k_ra8_err_null_ptr);
  TEST_ASSERT(xml_span_equal(names, sizeof(names) - 1U, name, "ns:a"));
  TEST_ASSERT(!xml_span_equal(nullptr, sizeof(names) - 1U, name, "ns:a"));
  TEST_ASSERT(!xml_span_equal(names, sizeof(names) - 1U, name, nullptr));
  TEST_ASSERT(!xml_span_equal(names, sizeof(names) - 1U, forged, "a"));
  TEST_ASSERT(xml_span_local_equal(names, sizeof(names) - 1U, name, "a"));
  TEST_ASSERT(!xml_span_local_equal(nullptr, sizeof(names) - 1U, name, "a"));
  TEST_ASSERT(!xml_span_local_equal(names, sizeof(names) - 1U, forged, "a"));
  TEST_ASSERT(xml_decoded_equal(values, sizeof(values) - 1U, left, right));
  TEST_ASSERT(!xml_decoded_equal(values, sizeof(values) - 1U, left, different));
  TEST_ASSERT(!xml_decoded_equal(values, sizeof(values) - 1U, left, forged));
  TEST_ASSERT(!xml_decoded_equal(nullptr, sizeof(values) - 1U, left, right));
}
...
RA8_INTERNAL static void internal_doctype_lexer_vectors(void)
{
  ...
  xml_workspace_t      workspace       = {};
  TEST_ASSERT(xml_validate(tab_spacing, sizeof(tab_spacing) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_validate(double_quoted, sizeof(double_quoted) - 1U, &workspace) == k_ra8_ok);
 ...
 * @par MC/DC:
 * `xml_reader_next()` only dispatches to the lexer once it has already
 ...
 * Decisions: apps/shared_libs/xml/src/xml_doctype.c@priv_xml_doctype
 */
RA8_INTERNAL static void internal_doctype_direct_seam(void)
{
  ...
  xml_workspace_t      workspace   = {};
  xml_reader_t         reader      = {};
  TEST_ASSERT(xml_reader_init(&reader, truncated, sizeof(truncated) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(priv_xml_doctype(&reader) == k_ra8_err_validation_failed);
  TEST_ASSERT(xml_reader_init(&reader, mistyped, sizeof(mistyped) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(priv_xml_doctype(&reader) == k_ra8_err_validation_failed);
}
...
RA8_INTERNAL static void internal_xml_lexer_vectors(void)
{
  ...
  xml_workspace_t      workspace     = {};
  TEST_ASSERT(xml_validate(hex_letter, sizeof(hex_letter) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_validate(hex_upper_x, sizeof(hex_upper_x) - 1U, &workspace) == k_ra8_ok);
...
RA8_INTERNAL static void internal_xml_attribute_vectors(void)
{
  ...
  xml_workspace_t      workspace       = {};
  TEST_ASSERT(xml_validate(slash_space, sizeof(slash_space) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_validate(space_tag, sizeof(space_tag) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_validate(spaced_equals, sizeof(spaced_equals) - 1U, &workspace) == k_ra8_ok);
...
RA8_INTERNAL static void internal_xml_attr_guard_vectors(void)
{
  ...
  xml_attr_cursor_t cursor      = {};
  xml_event_t       forged_name = event;
  forged_name.name.offset       = UINT32_MAX;
  xml_attr_begin(&forged_name, &cursor);
  TEST_ASSERT(
    xml_attr_next(source, sizeof(source) - 1U, &forged_name, &cursor, &attribute, &present) ==
    k_ra8_err_validation_failed);

  xml_event_t empty_markup   = event;
  empty_markup.markup.length = 0U;
  xml_attr_begin(&empty_markup, &cursor);
  TEST_ASSERT(
    xml_attr_next(source, sizeof(source) - 1U, &empty_markup, &cursor, &attribute, &present) ==
    k_ra8_err_validation_failed);

  xml_attr_cursor_t parked = {.position = event.markup.offset + event.markup.length - 1U,
                              .emitted  = 0U};
  TEST_ASSERT(xml_attr_next(source, sizeof(source) - 1U, &event, &parked, &attribute, &present) ==
              k_ra8_err_validation_failed);
}
...
RA8_INTERNAL static void internal_xml_structure_vectors(void)
{
  ...
  xml_workspace_t      workspace       = {};
  TEST_ASSERT(xml_validate(empty_comment, sizeof(empty_comment) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_validate(bracket_text, sizeof(bracket_text) - 1U, &workspace) == k_ra8_ok);
...
RA8_INTERNAL static void internal_xml_pi_vectors(void)
{
  ...
  xml_workspace_t      workspace     = {};
  TEST_ASSERT(xml_validate(alt_targets, sizeof(alt_targets) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_validate(standalone_no, sizeof(standalone_no) - 1U, &workspace) == k_ra8_ok);
  TEST_ASSERT(xml_validate(trailing_space, sizeof(trailing_space) - 1U, &workspace) == k_ra8_ok);
...
RA8_INTERNAL static void internal_expect_no_bom(const uint8_t* source, size_t length)
{
  xml_workspace_t workspace = {};
  xml_reader_t    reader    = {};
  TEST_ASSERT(xml_reader_init(&reader, source, length, &workspace) == k_ra8_ok);
...
RA8_INTERNAL static void internal_xml_public_boundary_vectors(void)
{
  ...
  const xml_span_t     left          = {0U, 6U};
  const xml_span_t     right         = {6U, 6U};
  const xml_span_t     overrun       = {0U, (uint32_t)sizeof(values)};
  const xml_span_t     forged        = {UINT32_MAX, 8U};
  char                 decoded[3]    = {};
  size_t               length        = 0U;
  xml_workspace_t      workspace     = {};
  xml_reader_t         reader        = {};
  TEST_ASSERT(!xml_span_equal(values, sizeof(values) - 1U, overrun, "A&"));
  TEST_ASSERT(xml_decode(values, sizeof(values) - 1U, left, decoded, 0U, &length) ==
              k_ra8_err_no_mem);
  TEST_ASSERT(!xml_decoded_equal(values, sizeof(values) - 1U, forged, right));
  TEST_ASSERT(xml_reader_init(&reader, values, (size_t)UINT32_MAX + 1U, &workspace) ==
