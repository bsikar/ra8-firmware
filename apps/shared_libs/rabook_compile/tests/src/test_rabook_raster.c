...
typedef enum : uint32_t {
  k_test_codec_arena_bytes = 1024U * 1024U, /**< One MiB per codec arena. */
...
/** @brief Persistent decode and geometry buffers shared by the raster vectors. */
static struct {
  uint8_t rgba[k_test_rgba_bytes];               /**< Decoded RGBA pixels.         */
  uint8_t gray[k_test_gray_bytes];               /**< Downscaled grayscale pixels. */
  uint8_t geometry_bmp[k_test_bmp_header_bytes]; /**< Mutable BMP header fixture.  */
} s_raster_buffers;
...
{
  /** Aligned persistent codec arenas returned through the workspace. */
  alignas(16) static struct {
    uint8_t stb[k_test_codec_arena_bytes];  /**< stb decoder arena.  */
    uint8_t webp[k_test_codec_arena_bytes]; /**< WebP decoder arena. */
  } s_codec_arenas;
  *stb  = (ra8_img_arena_t){.base   = s_codec_arenas.stb,
                            .cap    = sizeof s_codec_arenas.stb,
                            .offset = 0U,
                            .live   = 0U};
  *webp = (ra8_webp_arena_t){.base   = s_codec_arenas.webp,
                             .cap    = sizeof s_codec_arenas.webp,
                             .offset = 0U,
                             .live   = 0U};
  return (ra8_rabook_raster_workspace_t){.stb_arena  = stb,
                                         .webp_arena = webp,
                                         .rgba       = s_raster_buffers.rgba,
                                         .rgba_cap   = sizeof s_raster_buffers.rgba,
                                         .gray       = s_raster_buffers.gray,
                                         .gray_cap   = sizeof s_raster_buffers.gray};
}
...
{
  (void)memset(s_raster_buffers.geometry_bmp, 0, sizeof s_raster_buffers.geometry_bmp);
  s_raster_buffers.geometry_bmp[k_bmp_off_magic_first]  = (uint8_t)'B';
  s_raster_buffers.geometry_bmp[k_bmp_off_magic_second] = (uint8_t)'M';
  internal_put_le(&s_raster_buffers.geometry_bmp[k_bmp_off_pixel_data],
                  (uint32_t)k_test_bmp_header_bytes,
                  (uint8_t)k_bmp_u32_bytes);
  internal_put_le(&s_raster_buffers.geometry_bmp[k_bmp_off_info_size],
                  (uint32_t)k_test_bmp_info_bytes,
                  (uint8_t)k_bmp_u32_bytes);
  internal_put_le(&s_raster_buffers.geometry_bmp[k_bmp_off_width], width, (uint8_t)k_bmp_u32_bytes);
  internal_put_le(&s_raster_buffers.geometry_bmp[k_bmp_off_height],
                  height,
                  (uint8_t)k_bmp_u32_bytes);
  internal_put_le(&s_raster_buffers.geometry_bmp[k_bmp_off_planes],
                  (uint32_t)k_test_bmp_planes,
                  (uint8_t)k_bmp_u16_bytes);
  internal_put_le(&s_raster_buffers.geometry_bmp[k_bmp_off_bpp],
...
RA8_INTERNAL static void internal_expect_geometry_rejected(uint32_t width, uint32_t height)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 ra8_rabook_raster_encode(s_raster_buffers.geometry_bmp,
                                          sizeof s_raster_buffers.geometry_bmp,
                                          0U,
...
/** @brief Require one raster input/workspace combination to return @p expected. */
RA8_INTERNAL static void internal_expect_raster_error(const uint8_t*                 encoded,
                                                      size_t                         encoded_size,
                                                      ...
                                                      ra8_err_t                      expected)
{
  TEST_ASSERT_EQ(expected,
                 ra8_rabook_raster_encode(encoded,
                                          encoded_size,
                                          0U,
                                          (uint8_t)k_book_pixfmt_gray8,
                                          workspace,
...
{
  /** RIFF/WEBP signature with no image chunk behind it. */
  static const uint8_t webp_header_only[] = {
    0x52U,
    0x49U,
    0x46U,
    0x46U,
  ...
  ra8_rabook_raster_workspace_t ws   = internal_fresh_workspace(&stb, &webp);
  ra8_rabook_raster_result_t    got  = {.encoded_size = UINT32_MAX};
  internal_expect_raster_error(webp_header_only,
                               sizeof(webp_header_only),
                               &ws,
                               &got,
  ...
  internal_expect_raster_error(s_webp, sizeof(s_webp), &ws, &got, k_ra8_err_no_mem);
  ...
  ws       = internal_fresh_workspace(&stb, &webp);
  webp.cap = 0U;
  internal_expect_raster_error(s_webp, sizeof(s_webp), &ws, &got, k_ra8_err_validation_failed);
  TEST_ASSERT_EQ(k_test_no_bytes, got.encoded_size);
