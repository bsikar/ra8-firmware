...
/** @brief Persistent storage shared by the container test vectors. */
typedef struct {
  uint8_t        flat[k_t_flat_bytes];             /**< Flat RABOOK source bytes.      */
  uint8_t        packed[k_t_packed_bytes];         /**< Packed RBKC destination.       */
  uint8_t        input[k_t_chunk_bytes];           /**< Compressor input chunk.        */
  uint8_t        compressed[k_t_compressed_bytes]; /**< Compressed chunk bytes.        */
  uint8_t        restored[k_t_flat_bytes];         /**< Decompressed round-trip bytes. */
  uint64_t       offsets[k_t_offset_count];        /**< Container chunk offsets.       */
  t_compressor_t compressor;                       /**< Deflate compressor workspace.  */
} t_storage_t;

static t_storage_t s_storage;
...
RA8_INTERNAL static uint32_t internal_rd_u32(const uint8_t* p)
{
  ...
  for (uint8_t i = 0U; i < 4U; i++) {
    const uint32_t shift = (uint32_t)i * (uint32_t)k_t_bits_per_byte;
    value |= (uint32_t)p[i] << shift;
  }
  return value;
}
 ...
 * @pre The flat fixture storage spans ::k_t_flat_bytes writable bytes.
...
{
  for (uint32_t i = 0U; i < (uint32_t)sizeof(s_storage.flat); i++) {
    s_storage.flat[i] = (uint8_t)((i * (uint32_t)k_t_pattern_mul) + (uint32_t)k_t_pattern_add);
  }
}
 ...
 * @pre The compressor fixture storage satisfies max_align_t alignment.
...
{
  return (ra8_rabook_container_workspace_t){.input          = s_storage.input,
                                            .compressed     = s_storage.compressed,
                                            .compressor     = s_storage.compressor.bytes,
                                            .offsets        = s_storage.offsets,
                                            .input_cap      = sizeof(s_storage.input),
                                            .compressed_cap = sizeof(s_storage.compressed),
...
{
  return (t_io_t){.src              = s_storage.flat,
                  .dst              = s_storage.packed,
                  .src_len          = sizeof(s_storage.flat),
                  .dst_cap          = sizeof(s_storage.packed),
...
RA8_INTERNAL static void internal_test_multi_chunk_round_trip(void)
{
  ...
  internal_fill_flat();
  (void)memset(s_storage.packed, 0, sizeof(s_storage.packed));
  (void)memset(s_storage.restored, 0, sizeof(s_storage.restored));
  ...
  TEST_ASSERT_EQ(k_ra8_ok,
                 ra8_rabook_container_write(internal_mem_read,
                                            &io,
                                            sizeof(s_storage.flat),
                                            k_t_chunk_bytes,
                                            internal_mem_write,
                                            &io,
                                            &ws,
                                            &out_len));
  TEST_ASSERT((s_storage.packed[0] == (uint8_t)'R') && (s_storage.packed[1] == (uint8_t)'B') &&
              (s_storage.packed[2] == (uint8_t)'K') && (s_storage.packed[3] == (uint8_t)'C'));
  TEST_ASSERT_EQ(k_t_chunk_bytes, internal_rd_u32(&s_storage.packed[4]));
  TEST_ASSERT_EQ(k_t_flat_bytes, internal_rd_u64(&s_storage.packed[8]));
  TEST_ASSERT_EQ(k_t_chunk_count, internal_rd_u32(&s_storage.packed[16]));
  TEST_ASSERT_EQ(0, internal_rd_u32(&s_storage.packed[20]));

  const uint64_t payload_off = (uint64_t)k_book_container_header_len +
                               ((uint64_t)k_t_offset_count * (uint64_t)k_book_container_entry_len);
  TEST_ASSERT_EQ(0, internal_rd_u64(&s_storage.packed[k_book_container_header_len]));
  for (uint32_t i = 0U; i < k_t_chunk_count; i++) {
    const uint64_t begin = internal_rd_u64(
      &s_storage.packed[k_book_container_header_len + (i * k_book_container_entry_len)]);
    const uint64_t end = internal_rd_u64(
      &s_storage.packed[k_book_container_header_len + ((i + 1U) * k_book_container_entry_len)]);
    ...
    const size_t   got       = tinfl_decompress_mem_to_mem(&s_storage.restored[flat_off],
                                                           expected,
                                                           &s_storage.packed[payload_off + begin],
                                                           (size_t)(end - begin),
                                                           TINFL_FLAG_PARSE_ZLIB_HEADER);
    TEST_ASSERT_EQ(expected, got);
  }
  TEST_ASSERT_EQ(0, memcmp(s_storage.flat, s_storage.restored, sizeof(s_storage.flat)));
  TEST_ASSERT_EQ(
    out_len,
    (payload_off +
     internal_rd_u64(&s_storage.packed[k_book_container_header_len +
                                       (k_t_chunk_count * k_book_container_entry_len)])));
...
RA8_INTERNAL static void internal_test_validation(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 ra8_rabook_container_write(internal_mem_read,
                                            ...
                                            &out_len));
  ws                = internal_full_workspace();
  ws.compressor_cap = k_ra8_compress_scratch_bytes - 1U;
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 ra8_rabook_container_write(internal_mem_read,
                                            &io,
                                            k_t_flat_bytes,
