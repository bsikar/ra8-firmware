 ...
 * `apps/shared_libs/rabook_compile/src/rabook_compile.c` by exercising every remaining
 ...
 * inputs alone:
 *
 *  - NULL-context guards on every builder entry point (return @ref k_book_nil).
...
{
  static struct {
    book_chapter_t    chapters[k_c_chapter_cap]; /**< Chapter records.           */
    book_node_t       nodes[k_c_node_cap];       /**< Parsed document nodes.     */
    book_attr_t       attrs[k_c_attr_cap];       /**< Parsed node attributes.    */
    book_stylesheet_t styles[k_c_style_cap];     /**< Stylesheet records.        */
    book_image_t      images[k_c_image_cap];     /**< Imported image records.    */
    char              strpool[k_c_string_cap];   /**< Builder string arena.      */
    uint8_t           imgpool[k_c_imgpool_cap];  /**< Builder image arena.       */
    uint8_t           out[k_c_out_cap];          /**< Final RABOOK output bytes. */
  } s_storage;
...
{
  TEST_BEGIN("rabook_compile: NULL-ctx guards return nil");
  static const uint8_t data[1] = {};

  TEST_ASSERT_EQ(k_book_nil, ra8_rabook_intern(nullptr, "x"));
  TEST_ASSERT_EQ(k_book_nil, ra8_rabook_add_element(nullptr, 0U, nullptr, 0U));
  TEST_ASSERT_EQ(k_book_nil, ra8_rabook_add_text(nullptr, 0U));
  TEST_ASSERT_EQ(k_book_nil, ra8_rabook_add_chapter(nullptr, 0U, 0U, 0U));
  TEST_ASSERT_EQ(k_book_nil,
                 ra8_rabook_add_image(nullptr,
                                      ...
                                      data,
                                      1U));
  TEST_ASSERT_EQ(k_book_nil, ra8_rabook_add_stylesheet(nullptr, 0U, 0U));
  TEST_END("rabook_compile: NULL-ctx guards return nil");
}
...
{
  TEST_BEGIN("rabook_compile: intern NULL-str latch + failed short-circuit");
  ...
  TEST_ASSERT_EQ(k_book_nil, ra8_rabook_intern(&ctx, nullptr));
  TEST_ASSERT(ctx.failed);

  /* A valid string once failed is latched -> short-circuit to nil. */
  TEST_ASSERT_EQ(k_book_nil, ra8_rabook_intern(&ctx, "later"));
  TEST_END("rabook_compile: intern NULL-str latch + failed short-circuit");
}
...
{
  TEST_BEGIN("rabook_compile: add_element NULL-attrs latch + failed short-circuit");
  ra8_rabook_buffers_t buf = internal_full_buffers();
  ra8_rabook_ctx_t     ctx = {};
  TEST_ASSERT_EQ(k_ra8_ok, rabook_compile_init(&ctx, &buf));
  ...
  TEST_ASSERT_EQ(k_book_nil, ra8_rabook_add_element(&ctx, name, nullptr, 2U));
  TEST_ASSERT(ctx.failed);

  /* Any further add on the failed ctx short-circuits to nil. */
  TEST_ASSERT_EQ(k_book_nil, ra8_rabook_add_element(&ctx, name, nullptr, 0U));
  TEST_END("rabook_compile: add_element NULL-attrs latch + failed short-circuit");
}
...
{
  TEST_BEGIN("rabook_compile: add_text overflow latch + failed short-circuit");
  ra8_rabook_buffers_t buf = internal_full_buffers();
  buf.node_cap             = 1U;
  ra8_rabook_ctx_t ctx     = {};
  TEST_ASSERT_EQ(k_ra8_ok, rabook_compile_init(&ctx, &buf));
  ...
  TEST_ASSERT_EQ(k_book_nil, ra8_rabook_add_text(&ctx, word)); /* overflow     */
  TEST_ASSERT(ctx.failed);
  TEST_ASSERT_EQ(k_book_nil, ra8_rabook_add_text(&ctx, word)); /* failed */
  TEST_END("rabook_compile: add_text overflow latch + failed short-circuit");
}
...
{
  TEST_BEGIN("rabook_compile: link_child out-of-range parent + child");
  ra8_rabook_buffers_t buf = internal_full_buffers();
  ra8_rabook_ctx_t     ctx = {};
  TEST_ASSERT_EQ(k_ra8_ok, rabook_compile_init(&ctx, &buf));
  ...
  TEST_END("rabook_compile: link_child out-of-range parent + child");
}
...
{
  TEST_BEGIN("rabook_compile: link_sibling out-of-range node + sibling");
  ra8_rabook_buffers_t buf = internal_full_buffers();
  ra8_rabook_ctx_t     ctx = {};
  TEST_ASSERT_EQ(k_ra8_ok, rabook_compile_init(&ctx, &buf));
  ...
  TEST_END("rabook_compile: link_sibling out-of-range node + sibling");
}
...
{
  TEST_BEGIN("rabook_compile: add_chapter overflow latch + failed short-circuit");
  ra8_rabook_buffers_t buf = internal_full_buffers();
  buf.chapter_cap          = 1U;
  ra8_rabook_ctx_t ctx     = {};
  TEST_ASSERT_EQ(k_ra8_ok, rabook_compile_init(&ctx, &buf));
  ...
  TEST_ASSERT_EQ(k_book_nil, ra8_rabook_add_chapter(&ctx, 0U, 0U, 0U)); /* overflow        */
  TEST_ASSERT(ctx.failed);
  TEST_ASSERT_EQ(k_book_nil, ra8_rabook_add_chapter(&ctx, 0U, 0U, 0U)); /* failed */
  TEST_END("rabook_compile: add_chapter overflow latch + failed short-circuit");
}
...
{
  TEST_BEGIN("rabook_compile: add_image NULL-data latch");
  ra8_rabook_buffers_t buf = internal_full_buffers();
  ra8_rabook_ctx_t     ctx = {};
  TEST_ASSERT_EQ(k_ra8_ok, rabook_compile_init(&ctx, &buf));
  ...
  TEST_ASSERT(ctx.failed);
  TEST_END("rabook_compile: add_image NULL-data latch");
}
...
{
  TEST_BEGIN("rabook_compile: add_image pool overflow latch + failed short-circuit");
  ra8_rabook_buffers_t buf = internal_full_buffers();
  buf.image_pool_cap       = (uint32_t)k_c_tiny_pool_cap; /* only 4 pool bytes */
  ra8_rabook_ctx_t ctx     = {};
  TEST_ASSERT_EQ(k_ra8_ok, rabook_compile_init(&ctx, &buf));

  static const uint8_t data[k_c_pool_overflow_bytes] = {};
  ...
  TEST_ASSERT_EQ(k_book_nil,
                 ra8_rabook_add_image(&ctx,
                                      ...
                                      data,
                                      (uint32_t)k_c_pool_overflow_bytes));
  TEST_END("rabook_compile: add_image pool overflow latch + failed short-circuit");
}
...
{
  TEST_BEGIN("rabook_compile: add_stylesheet overflow latch + failed short-circuit");
  ra8_rabook_buffers_t buf = internal_full_buffers();
  buf.stylesheet_cap       = 1U;
  ra8_rabook_ctx_t ctx     = {};
  TEST_ASSERT_EQ(k_ra8_ok, rabook_compile_init(&ctx, &buf));

  TEST_ASSERT_EQ(0U, ra8_rabook_add_stylesheet(&ctx, 0U, (uint32_t)k_book_nil)); /* fills 0 */
  TEST_ASSERT_EQ(k_book_nil,
                 ra8_rabook_add_stylesheet(&ctx, 0U, (uint32_t)k_book_nil)); /* overflow */
  TEST_ASSERT(ctx.failed);
  TEST_ASSERT_EQ(k_book_nil,
                 ra8_rabook_add_stylesheet(&ctx, 0U, (uint32_t)k_book_nil)); /* failed */
  TEST_END("rabook_compile: add_stylesheet overflow latch + failed short-circuit");
}
...
{
  TEST_BEGIN("rabook_compile: set_metadata after overflow -> no_mem");
  ra8_rabook_buffers_t buf = internal_full_buffers();
  ra8_rabook_ctx_t     ctx = {};
  TEST_ASSERT_EQ(k_ra8_ok, rabook_compile_init(&ctx, &buf));
  ...
  TEST_END("rabook_compile: set_metadata after overflow -> no_mem");
}
...
{
  TEST_BEGIN("rabook_compile: finalize layout > 32-bit -> invalid_size");
  ra8_rabook_ctx_t ctx  = {};
  ...
  TEST_END("rabook_compile: finalize layout > 32-bit -> invalid_size");
}
...
{
  TEST_BEGIN("rabook_compile: finalize blob > out_cap -> invalid_size");
  ra8_rabook_buffers_t buf = internal_full_buffers();
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, ra8_rabook_finalize(&ctx, &blob, &len));
  TEST_ASSERT_NULL(blob);
  TEST_END("rabook_compile: finalize blob > out_cap -> invalid_size");
}
