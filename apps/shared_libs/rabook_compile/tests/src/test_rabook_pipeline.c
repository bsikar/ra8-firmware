 ...
 *    from apps/shared_libs/epub/tests/src/test_epub.c) and opened with @ref epub_open.
 *  - A real FAT16 volume is formatted over a RAM block backend (the RAM-disk
 *    pattern from apps/shared_libs/epub/tests/src/test_epub_fs.c) so the pipeline's
 *    @ref ra8_fs_write_file lands on a genuine filesystem.
 *  - Happy path (no cover): the compile succeeds, the written `.rabook` passes
 *    @ref book_validate, chapter_count / title / author read back as built,
 ...
 * @ref rabook_compile_from_epub and the @ref k_ra8_err_not_found arm of
...
RA8_INTERNAL static void internal_test_pipeline_text_only_no_cover(void)
{
  ...
  TEST_ASSERT((size > 0U) && ((size_t)size <= sizeof(s_pipe.readback)));
  uint32_t got = 0U;
  ...
  TEST_ASSERT_EQ(0, strcmp(book_string(s_pipe.readback, hdr->title_off), "Pipeline Title"));
  TEST_ASSERT_EQ(0, strcmp(book_string(s_pipe.readback, hdr->author_off), "Pipeline Author"));
  TEST_ASSERT_EQ(0, strcmp(book_string(s_pipe.readback, hdr->language_off), "en"));

  /* String-pool offset 0 is the reserved empty string. */
  TEST_ASSERT_EQ(0, strcmp(book_string(s_pipe.readback, 0U), ""));

  /* The single spine chapter has a 'body' root element. */
  const book_chapter_t* chaps = book_chapters(s_pipe.readback);
  const book_node_t*    nodes = book_nodes(s_pipe.readback);
  const book_node_t*    root  = &nodes[chaps[0].root_node];
  TEST_ASSERT_EQ(k_book_node_element, root->kind);
  TEST_ASSERT_EQ(0, strcmp(book_node_name(s_pipe.readback, root), "body"));
 ...
 * both arrays with `just tools::rabook_golden` after any format/emitter change.
 *
 * @par MC/DC:
...
RA8_INTERNAL static void internal_test_pipeline_parity_byte_identical(const uint8_t* epub,
                                                                      size_t         epub_len,
                                                                      ...
                                                                      uint32_t       golden_len)
{
  ...
  epub_book_t            book  = {};
  const epub_mem_media_t media = {.data = epub, .size = epub_len};
  TEST_ASSERT_EQ(k_ra8_ok, epub_open(&media, "rabook_parity.epub", &book));
  ...
  TEST_ASSERT_EQ(golden_len, got);
  for (uint32_t i = 0U; i < got; i++) {
    if (s_pipe.readback[i] != golden[i]) {
      internal_report_diff("  parity diff at offset ", i, s_pipe.readback[i], golden[i]);
      break;
    }
  }
  TEST_ASSERT_EQ(0, memcmp(s_pipe.readback, golden, (size_t)got));
 ...
 * @post The RAM volume is unmounted and its backing store freed.
 * @note Not thread-safe (writes file-scope fixture buffers). @since Version 0.1.0 */
RA8_INTERNAL static void internal_test_pipeline_parity_noimg_byte_identical(const uint8_t* epub,
                                                                            size_t         epub_len,
                                                                            const uint8_t* golden,
                                                                            uint32_t golden_len)
{
  ...
  for (uint32_t i = 0U; i < got; i++) {
    if (s_pipe.readback[i] != golden[i]) {
      internal_report_diff("  noimg parity diff at offset ", i, s_pipe.readback[i], golden[i]);
      break;
    }
  }
  TEST_ASSERT_EQ(0, memcmp(s_pipe.readback, golden, (size_t)got));
 ...
 * blob byte-for-byte identical to tools/epub_compile/src/epub_compile.py run with
 ...
 * @note Not thread-safe (writes file-scope fixture buffers). @since Version 0.1.0 */
RA8_INTERNAL static void
internal_test_pipeline_parity_realbook_byte_identical(const uint8_t* epub,
                                                      size_t         epub_len,
                                                      const uint8_t* golden,
                                                      uint32_t       golden_len)
{
  ...
  epub_book_t            book  = {};
  const epub_mem_media_t media = {.data = epub, .size = epub_len};
  TEST_ASSERT_EQ(k_ra8_ok, epub_open(&media, "walden.epub", &book));
  ...
  for (uint32_t i = 0U; i < got; i++) {
    if (s_pipe.readback[i] != golden[i]) {
      internal_report_diff("  realbook parity diff at offset ", i, s_pipe.readback[i], golden[i]);
      break;
    }
  }
  TEST_ASSERT_EQ(0, memcmp(s_pipe.readback, golden, (size_t)got));
...
/** @brief Validate generated fixture metadata before exercising the pipeline. */
RA8_INTERNAL static void internal_validate_golden_fixtures(void)
{
  TEST_ASSERT_EQ(k_parity_epub_len, sizeof(s_parity_epub));
  TEST_ASSERT_EQ(k_parity_golden_len, sizeof(s_parity_golden));
  TEST_ASSERT_EQ(k_parity_golden_noimg_len, sizeof(s_parity_golden_noimg));
  TEST_ASSERT_EQ(k_realbook_epub_len, sizeof(s_realbook_epub));
  TEST_ASSERT_EQ(k_realbook_golden_noimg_len, sizeof(s_realbook_golden_noimg));
  TEST_ASSERT((s_parity_epub[0] == (uint8_t)'P') && (s_parity_epub[1] == (uint8_t)'K'));
  TEST_ASSERT((s_realbook_epub[0] == (uint8_t)'P') && (s_realbook_epub[1] == (uint8_t)'K'));
  TEST_ASSERT((s_parity_golden[0] == (uint8_t)'R') && (s_parity_golden_noimg[0] == (uint8_t)'R') &&
              (s_realbook_golden_noimg[0] == (uint8_t)'R'));
}

int main(void)
{
  ra8_log_set_byte_sink(internal_log_sink, nullptr);
  internal_validate_golden_fixtures();
  internal_test_pipeline_text_only_no_cover();
  internal_test_pipeline_undecodable_cover_skipped();
  internal_test_pipeline_parity_byte_identical(s_parity_epub,
                                               (size_t)k_parity_epub_len,
                                               s_parity_golden,
                                               (uint32_t)k_parity_golden_len);
  internal_test_pipeline_parity_noimg_byte_identical(s_parity_epub,
                                                     (size_t)k_parity_epub_len,
                                                     s_parity_golden_noimg,
                                                     (uint32_t)k_parity_golden_noimg_len);
  internal_test_pipeline_parity_realbook_byte_identical(s_realbook_epub,
                                                        (size_t)k_realbook_epub_len,
                                                        s_realbook_golden_noimg,
                                                        (uint32_t)k_realbook_golden_noimg_len);
  return 0;
}
