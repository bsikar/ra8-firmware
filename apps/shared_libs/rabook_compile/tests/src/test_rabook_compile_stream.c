/**
 * @file test_rabook_compile_stream.c
...
{
  TEST_BEGIN("rabook_compile: external stream == memory blob");
  ...
  TEST_ASSERT_EQ(mem_len, stream_len);
  TEST_ASSERT_EQ(0, memcmp(s_fixture.expected, s_fixture.stream_out, (size_t)stream_len));
  TEST_ASSERT_EQ(k_ra8_ok, book_validate(s_fixture.stream_out, (size_t)stream_len));
  ...
  TEST_END("rabook_compile: external stream == memory blob");
}
...
{
  TEST_BEGIN("rabook_compile: short stream read/write rejected");
  ra8_rabook_ctx_t            ctx = {};
  ra8_test_rabook_roundtrip_t rt  = {};
  ra8_test_rabook_init(&s_fixture, &ctx);
  ra8_test_rabook_populate(&s_fixture, &ctx, &rt, true);
  uint8_t  scratch[2] = {};
  uint32_t out_len    = UINT32_MAX;
  ...
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 ra8_rabook_finalize_stream(&ctx,
                                            ...
                                            (uint32_t)sizeof(scratch),
                                            &out_len));
  TEST_ASSERT_EQ(UINT32_MAX, out_len);
  ...
  TEST_END("rabook_compile: short stream read/write rejected");
}

/**
 * @test internal_test_rabook_stream_null_writer
 * @brief A missing destination callback fails before layout or publication.
 * @details Supplies an otherwise initialized internal-pool builder and pins
 *          the public finalizer's explicit writer validation.
 * @pre Shared fixture arenas are writable.
 * @post Finalization reports null pointer and preserves the output sentinel.
 * @note Test-only; no callback can run in this vector.
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_test_rabook_stream_null_writer(void)
{
  TEST_BEGIN("rabook_compile: null stream writer rejected");
  ra8_rabook_ctx_t ctx = {};
  ra8_test_rabook_init(&s_fixture, &ctx);
  uint32_t out_len = UINT32_MAX;
  TEST_ASSERT_EQ(
    k_ra8_err_null_ptr,
    ra8_rabook_finalize_stream(&ctx, nullptr, nullptr, nullptr, nullptr, nullptr, 0U, &out_len));
  TEST_ASSERT_EQ(UINT32_MAX, out_len);
  TEST_END("rabook_compile: null stream writer rejected");
}
...
{
  TEST_BEGIN("rabook_compile: stream destination exhaustion");
  ra8_rabook_ctx_t            ctx = {};
  ra8_test_rabook_roundtrip_t rt  = {};
  ra8_test_rabook_init(&s_fixture, &ctx);
  ra8_test_rabook_populate(&s_fixture, &ctx, &rt, true);
  ...
  TEST_ASSERT_EQ(UINT32_MAX, out_len);
  TEST_END("rabook_compile: stream destination exhaustion");
}
...
{
  TEST_BEGIN("rabook_compile: external pool offsets and mode");
  ...
  TEST_END("rabook_compile: external pool offsets and mode");
}
...
{
  TEST_BEGIN("rabook_compile: external pool uint32 overflow");
  ra8_rabook_buffers_t buf = ra8_test_rabook_buffers(&s_fixture);
  ra8_rabook_ctx_t     ctx = {};
  TEST_ASSERT_EQ(k_ra8_ok, rabook_compile_init(&ctx, &buf));
  const uint32_t href = ra8_rabook_intern(&ctx, "spooled.bin");
  ...
  TEST_END("rabook_compile: external pool uint32 overflow");
}
...
{
  TEST_BEGIN("rabook_compile: external pool layout overflow");
  ra8_rabook_buffers_t buf = ra8_test_rabook_buffers(&s_fixture);
  ra8_rabook_ctx_t     ctx = {};
  TEST_ASSERT_EQ(k_ra8_ok, rabook_compile_init(&ctx, &buf));
  const uint32_t href = ra8_rabook_intern(&ctx, "spooled.bin");
  ...
  TEST_ASSERT_EQ(UINT32_MAX, out_len);
  TEST_END("rabook_compile: external pool layout overflow");
}
...
int main(void)
{
  ...
  internal_test_rabook_stream_null_writer();
