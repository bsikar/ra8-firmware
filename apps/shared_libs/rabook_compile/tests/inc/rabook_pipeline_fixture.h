...
/** @brief Persistent storage shared by the pipeline fixture builders and tests. */
typedef struct {
  /** Chapter records. */
  book_chapter_t chapters[k_chapter_cap];
  /** Parsed document nodes. */
  book_node_t nodes[k_node_cap];
  /** Parsed node attributes. */
  book_attr_t attrs[k_attr_cap];
  /** Stylesheet records. */
  book_stylesheet_t styles[k_style_cap];
  /** Imported image records. */
  book_image_t images[k_image_cap];
  /** Builder string arena. */
  char strpool[k_string_cap];
  /** Builder image arena. */
  uint8_t imgpool[k_imgpool_cap];
  /** Final RABOOK output bytes. */
  uint8_t out[k_out_cap];
  /** XHTML source scratch. */
  uint8_t xhtml[k_xhtml_cap];
  /** XML parser workspace. */
  ra8_rabook_xml_workspace_t xml_workspace;
  /** Encoded image scratch. */
  uint8_t image_raw[k_imgraw_cap];
  /** Decoder arena bytes. */
  uint8_t img_scratch[k_arena_cap];
  /** Normalized grayscale pixels. */
  uint8_t gray[k_graypix_cap];
  /** Stylesheet source scratch. */
  char css[k_css_cap];
  /** Generated EPUB fixture. */
  uint8_t epub[k_epub_cap];
  /** Finalized EPUB length. */
  size_t epub_len;
  /** Filesystem readback bytes. */
  uint8_t readback[k_read_cap];
  /** RAM-disk bytes. */
  uint8_t disk_bytes[(size_t)k_disk_blocks * (size_t)k_disk_block_size];
  /** RAM-disk descriptor. */
  mem_disk_t disk;
  /** Filesystem backend facade. */
  ra8_fs_backend_t backend;
} pipe_storage_t;

static pipe_storage_t s_pipe;
...
/** @brief Add the mandatory text-only members to @p zip. */
RA8_INTERNAL static inline void internal_add_epub_base(mz_zip_archive* zip, const char* opf)
{
  TEST_ASSERT(
...
/**
 * @brief Build a tiny text-only `.epub` into @p s_pipe.epub.
 ...
 * @pre @p s_pipe.epub is large enough for the finalized archive.
 * @post @p s_pipe.epub holds the finalized ZIP and @p s_pipe.epub_len its length.
...
{
  static const char opf_with_cover[] =
    "<?xml version=\"1.0\"?><package xmlns=\"http://www.idpf.org/2007/opf\" version=\"3.0\" "
    "unique-identifier=\"id\"><metadata xmlns:dc=\"http://purl.org/dc/elements/1.1/\">"
    "<dc:title>Pipeline Title</dc:title><dc:creator>Pipeline Author</dc:creator>"
    "<dc:language>en</dc:language><dc:identifier id=\"id\">urn:test:pipe</dc:identifier></metadata>"
  ...
  static const uint8_t garbage_cover[16] = {
    0xDEU,
    0xADU,
    0xBEU,
    0xEFU,
  ...
  const char* opf = with_cover ? opf_with_cover : s_opf_no_cover;

  mz_zip_archive zip;
  (void)memset(&zip, 0, sizeof(zip));
  TEST_ASSERT(mz_zip_writer_init_heap(&zip, 0U, (size_t)k_epub_cap) == MZ_TRUE);
  internal_add_epub_base(&zip, opf);
  if (with_cover) {
    TEST_ASSERT(mz_zip_writer_add_mem(&zip,
                                      "OEBPS/cover.bin",
                                      garbage_cover,
                                      sizeof(garbage_cover),
  ...
  TEST_ASSERT((heap != nullptr) && (hsz > 0U) && (hsz <= sizeof(s_pipe.epub)));
  (void)memcpy(s_pipe.epub, heap, hsz);
  s_pipe.epub_len = hsz;
  mz_free(heap);
 ...
 * @pre The RAM backend descriptor @p s_pipe.backend is initialised.
 * @pre Any prior volume has been unmounted.
 * @post A formatted, mounted FAT16 volume backs @p s_pipe.disk.
 * @post @p s_pipe.disk.bytes references a fresh zeroed backing store.
 * @note Not thread-safe.
 */
RA8_INTERNAL static inline ra8_fs_mount_t* internal_fresh_volume(void)
{
  (void)memset(s_pipe.disk_bytes, 0, sizeof(s_pipe.disk_bytes));
  s_pipe.disk.block_count = (uint32_t)k_disk_blocks;
  s_pipe.disk.bytes       = s_pipe.disk_bytes;
  s_pipe.backend          = (ra8_fs_backend_t){.read_block   = internal_mem_read,
  ...
  opts.label                = "RABOOK";
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_format(&s_pipe.backend, &opts));

  ra8_fs_mount_t* mount = nullptr;
  TEST_ASSERT_EQ(k_ra8_ok, ra8_fs_mount(&s_pipe.backend, &mount));
  return mount;
}

/**
 * @brief Unmount @p mount and release its view of the RAM backing store.
 ...
 * @post @p mount is unmounted and the backing store remains fixture-owned.
 * @post @p s_pipe.disk.bytes is reset to NULL.
                                                    ...
                                                    ra8_img_arena_t*               arena)
{
  ...
  *scr   = (ra8_rabook_pipeline_scratch_t){
    ...
    .image_raw     = s_pipe.image_raw,
    .image_cap     = sizeof(s_pipe.image_raw),
    .img_arena     = arena,
    .gray          = s_pipe.gray,
    .gray_cap      = (uint32_t)k_graypix_cap,
    .css           = s_pipe.css,
    .css_cap       = sizeof(s_pipe.css),
    .xml_workspace = &s_pipe.xml_workspace,
  };
}
...
internal_make_bmp(uint8_t* out, uint16_t w, uint16_t h, uint8_t gray)
{
  ...
  (void)memset(out, 0, (size_t)total);
  out[0] = (uint8_t)'B';
  out[1] = (uint8_t)'M';
  ...
  for (uint32_t y = 0U; y < (uint32_t)h; y++) {
    uint8_t* px = &out[(size_t)hdr + ((size_t)y * (size_t)row)];
...
/**
 * @brief Assemble an in-memory `.epub` ZIP from @p entries into @p s_pipe.epub.
 ...
 * @pre The finalized archive fits in @p s_pipe.epub.
 * @post @p s_pipe.epub holds the ZIP and @p s_pipe.epub_len its length.
 * @post No filesystem state is touched.
...
static const struct {
  const char* opf_raster;     /**< Package with small and large raster images. */
  const char* opf_tall;       /**< Package with a tall image.                  */
  const char* opf_css;        /**< Package linking a stylesheet.               */
  const char* opf_img_absent; /**< Package referencing an absent image.        */
  const char* opf_toc;        /**< Package linking a navigation document.      */
  const char* nav_xhtml;      /**< XHTML navigation fixture.                   */
  const char* chapter2_xhtml; /**< Second chapter fixture.                     */
  const char* style_css;      /**< Linked stylesheet fixture.                  */
  const char* chapter_plain;  /**< Plain text-only chapter fixture.            */
} s_fixture_text = {
  .opf_raster =
    "<?xml version=\"1.0\"?><package xmlns=\"http://www.idpf.org/2007/opf\" version=\"3.0\" "
    "unique-identifier=\"id\"><metadata xmlns:dc=\"http://purl.org/dc/elements/1.1/\">"
 ...
 *           epub_toc_entry_to_chapter() fails on it -- driving the
 *           first-condition-false leg of internal_chapter_title's compound guard. */
  ...
  .chapter2_xhtml = "<html><body><p>Second chapter.</p></body></html>",
  /** @brief Stylesheet body interned by the CSS-present fixture. */
  .style_css = ".lead { color: #C00000; } p { margin: 0; }",
  /** @brief Chapter body with no XML root element (forces a parse miss). */
  .chapter_plain = "just plain text with no markup elements at all",
};
 ...
 * @pre @p s_pipe.epub and @p s_bmp are large enough for the fixtures.
 * @pre miniz is available (host build).
 * @post @p s_pipe.epub holds the ZIP; the small image is the declared cover.
 * @post No filesystem state is touched.
...
RA8_INTERNAL static inline void internal_build_epub_raster(void)
{
  ...
  const pipe_zip_entry_t entries[] = {
    {"mimetype", s_mimetype, strlen(s_mimetype), true},
    {"META-INF/container.xml", s_container, strlen(s_container), false},
    {"OEBPS/content.opf", s_fixture_text.opf_raster, strlen(s_fixture_text.opf_raster), false},
 ...
 * @pre @p s_pipe.epub and @p s_bmp are large enough for the fixtures.
 * @post @p s_pipe.epub holds the ZIP; the tall image is the declared cover.
 * @post No filesystem state is touched.
...
RA8_INTERNAL static inline void internal_build_epub_tall(void)
{
  ...
  const pipe_zip_entry_t entries[] = {
    {"mimetype", s_mimetype, strlen(s_mimetype), true},
    {"META-INF/container.xml", s_container, strlen(s_container), false},
    {"OEBPS/content.opf", s_fixture_text.opf_tall, strlen(s_fixture_text.opf_tall), false},
    {"OEBPS/c1.xhtml", s_chapter_xhtml, strlen(s_chapter_xhtml), false},
...
RA8_INTERNAL static inline void internal_build_epub_css(bool present)
{
  pipe_zip_entry_t entries[] = {
    {"mimetype", s_mimetype, strlen(s_mimetype), true},
    {"META-INF/container.xml", s_container, strlen(s_container), false},
    {"OEBPS/content.opf", s_fixture_text.opf_css, strlen(s_fixture_text.opf_css), false},
    {"OEBPS/c1.xhtml", s_chapter_xhtml, strlen(s_chapter_xhtml), false},
    {"OEBPS/style.css", s_fixture_text.style_css, strlen(s_fixture_text.style_css), false},
  };
...
RA8_INTERNAL static inline void internal_build_epub_image_absent(void)
{
  const pipe_zip_entry_t entries[] = {
    ...
    {"META-INF/container.xml", s_container, strlen(s_container), false},
    {"OEBPS/content.opf",
     s_fixture_text.opf_img_absent,
     strlen(s_fixture_text.opf_img_absent),
     false},
    {"OEBPS/c1.xhtml", s_chapter_xhtml, strlen(s_chapter_xhtml), false},
  };
  internal_build_zip(entries, sizeof(entries) / sizeof(entries[0]));
 ...
 * @pre @p s_pipe.epub is large enough for the finalized archive.
 * @post @p s_pipe.epub holds the ZIP; both spine chapters carry a TOC title.
 * @post No filesystem state is touched.
...
RA8_INTERNAL static inline void internal_build_epub_toc(void)
{
  const pipe_zip_entry_t entries[] = {
    {"mimetype", s_mimetype, strlen(s_mimetype), true},
    {"META-INF/container.xml", s_container, strlen(s_container), false},
    {"OEBPS/content.opf", s_fixture_text.opf_toc, strlen(s_fixture_text.opf_toc), false},
    {"OEBPS/nav.xhtml", s_fixture_text.nav_xhtml, strlen(s_fixture_text.nav_xhtml), false},
    {"OEBPS/c1.xhtml", s_chapter_xhtml, strlen(s_chapter_xhtml), false},
    {"OEBPS/c2.xhtml", s_fixture_text.chapter2_xhtml, strlen(s_fixture_text.chapter2_xhtml), false},
  };
  internal_build_zip(entries, sizeof(entries) / sizeof(entries[0]));
}
