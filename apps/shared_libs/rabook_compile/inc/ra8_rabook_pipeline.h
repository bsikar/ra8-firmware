 ...
 *     @ref epub_get_resource and interned, in OPF order.
 ...
 *  3. Spine chapters -- raw XHTML extracted by @ref epub_load_chapter,
 *     parsed and DOM-built by @ref ra8_rabook_xml_parse_chapter.
 *  4. Metadata -- Dublin Core from the open @ref epub_book_t, interned LAST
 ...
 * @ref epub_book_t and for providing correctly-sized arenas.
 *
 ...
 *
 *          @p pixel_format (default 0 == @ref k_book_pixfmt_gray4) is the
 *          device profile that selects the raster depth: 4-bpp packed grayscale
 *          for an even 16-level e-ink panel, or @ref k_book_pixfmt_gray8 for
...
typedef struct {
  ...
  uint8_t                     pixel_format;   /**< Device profile: raster depth to emit,
                                   *   @ref book_image_pixfmt_t. 0 (the
                                   *   zero-init default) is
                                   *   @ref k_book_pixfmt_gray4 -- the 4-bpp
                                   *   packing a grayscale panel wants;
                                   *   @ref k_book_pixfmt_gray8 keeps the
 ...
 *
 *  -# Initialise the builder context via @ref rabook_compile_init.
 ...
 *         (@ref epub_get_metadata / @ref epub_load_chapter / ...) or from
 ...
 *         @ref ra8_rabook_finalize (see @ref rabook_compile_from_epub).
 *
 * @pre @p epub->in_use == 1.
 * @pre All arenas and scratch buffers are non-NULL with valid capacities.
 ...
 *
 * @note Not thread-safe.
 * @see rabook_compile_from_epub  The filesystem-writing variant.
 * @since Version 0.1.0
 */
