...
RA8_INTERNAL static void internal_put_u64(uint8_t* out, uint64_t value)
{
  for (uint8_t i = 0U; i < (uint8_t)k_rbkc_u64_bytes; i++) {
    const uint32_t shift = (uint32_t)i * (uint32_t)k_rbkc_bits_per_byte;
                                                ...
                                                uint32_t*                               out_count)
{
  ...
  if (ws->compressor_cap < (uint32_t)k_ra8_compress_scratch_bytes) {
    return k_ra8_err_invalid_size;
  }
...
RA8_INTERNAL static ra8_err_t internal_write_prefix(ra8_rabook_write_at_fn write_at,
                                                    ...
                                                    uint32_t               count)
{
  static const uint8_t rbkc_magic[k_book_container_magic_len] = {'R', 'B', 'K', 'C'};
  uint8_t              header[k_book_container_header_len]    = {};
  for (uint8_t i = 0U; i < (uint8_t)k_book_container_magic_len; i++) {
    header[i] = rbkc_magic[i];
  }
  ...
  const uint8_t zero[k_book_container_entry_len] = {};
  for (uint32_t i = 0U; i <= count; i++) {
    const uint64_t at =
      (uint64_t)k_book_container_header_len + ((uint64_t)i * (uint64_t)k_book_container_entry_len);
                                                    ...
                                                    uint64_t                          payload_off)
{
  ...
  for (uint32_t i = 0U; i < count; i++) {
    ...
    uint32_t packed = 0U;
    err             = ra8_compress_zlib(ws->input,
                                        want,
                                        ws->compressed,
                                        ws->compressed_cap,
                                        ws->compressor,
...
RA8_INTERNAL static ra8_err_t internal_write_table(ra8_rabook_write_at_fn                  write_at,
                                                   ...
                                                   uint32_t                                count)
{
  uint8_t encoded[k_book_container_entry_len];
  for (uint32_t i = 0U; i <= count; i++) {
                                     ...
                                     uint64_t*                         out_len)
{
  ...
  const uint64_t table_bytes = ((uint64_t)count + 1U) * (uint64_t)k_book_container_entry_len;
