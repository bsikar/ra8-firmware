 ...
 *          @ref rabook_compile_init flat (one table-driven check instead of ten
 *          inline guards).
...
ra8_err_t rabook_compile_init(ra8_rabook_ctx_t* ctx, const ra8_rabook_buffers_t* buf)
{
  ...
  *ctx                   = (ra8_rabook_ctx_t){};
   ...
   * tools/epub_compile/src/epub_compile.py (offset 0 == ""). */
...
uint32_t ra8_rabook_intern(ra8_rabook_ctx_t* ctx, const char* str)
{
  ...
  while (scan < ctx->string_size) {
    ...
    if (cand_len == len) {
      uint32_t equal_bytes = 0U;
      while ((equal_bytes < len) && (cand[equal_bytes] == str[equal_bytes])) {
        equal_bytes++;
      }
      if (equal_bytes == len) {
        return scan;
 ...
 * @return The index of the first appended attribute, or @ref k_book_nil when
 *         @p attr_count is 0.
 * @retval k_book_nil @p attr_count is 0 (no attributes appended).
...
RA8_INTERNAL
static uint32_t
internal_append_attrs(ra8_rabook_ctx_t* ctx, const book_attr_t* attrs, uint16_t attr_count)
{
  if (attr_count == 0U) {
    return (uint32_t)k_book_nil;
  }
                                ...
                                uint16_t           attr_count)
{
  ...
  book_node_t*   node = &ctx->buf.nodes[idx];
  *node               = (book_node_t){};
  node->kind          = (uint8_t)k_book_node_element;
  node->attr_count    = attr_count;
  node->name_off      = name_off;
  node->text_off      = 0U;
  node->first_attr    = first_attr;
  node->first_child   = (uint32_t)k_book_nil;
  node->next_sibling  = (uint32_t)k_book_nil;
  ctx->node_count++;
  return idx;
}
...
uint32_t ra8_rabook_add_text(ra8_rabook_ctx_t* ctx, uint32_t text_off)
{
  ...
  book_node_t*   node = &ctx->buf.nodes[idx];
  *node               = (book_node_t){};
  node->kind          = (uint8_t)k_book_node_text;
  node->attr_count    = 0U;
  node->name_off      = 0U;
  node->text_off      = text_off;
  node->first_attr    = (uint32_t)k_book_nil;
  node->first_child   = (uint32_t)k_book_nil;
  node->next_sibling  = (uint32_t)k_book_nil;
  ctx->node_count++;
  return idx;
                                ...
                                uint32_t          root_node)
{
  ...
  book_chapter_t* ch  = &ctx->buf.chapters[idx];
                                                 ...
                                                 uint32_t          data_size)
{
  ...
  book_image_t*  img = &ctx->buf.images[idx];
  *img               = (book_image_t){};
  img->id_off        = id_off;
  img->width         = width;
  img->height        = height;
...
ra8_rabook_add_stylesheet(ra8_rabook_ctx_t* ctx, uint32_t source_off, uint32_t scope_chapter)
{
  ...
  book_stylesheet_t* ss  = &ctx->buf.stylesheets[idx];
