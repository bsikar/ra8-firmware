...
typedef enum : uint16_t {
  k_pipeline_max_chapters = 64U, /**< == k_epub_max_chapters (cross-check). */
  k_pipeline_max_toc      = 64U, /**< == k_epub_max_toc (cross-check).      */
} ra8_pipeline_limits_t;

static_assert((uint16_t)k_pipeline_max_chapters == (uint16_t)k_epub_max_chapters,
              "pipeline chapter cap must match epub cap");
static_assert((uint16_t)k_pipeline_max_toc == (uint16_t)k_epub_max_toc,
                                                   ...
                                                   uint16_t                             oh)
{
  ...
  if (((uint32_t)ow * (uint32_t)oh) > scr->gray_cap) {
 ...
 * @details Dispatches on @p scr->pixel_format: @ref k_book_pixfmt_gray8 copies
...
static ra8_err_t internal_encode_gray(const ra8_rabook_pipeline_scratch_t* scr,
                                      ...
                                      uint32_t*                            out_size)
{
  if (scr->pixel_format == (uint8_t)k_book_pixfmt_gray8) {
 ...
 * @return Image index on success, or @ref k_book_nil on any failure (decode,
 *         gray-scratch capacity, downscale, encode, or builder overflow).
 * @retval k_book_nil Decode / capacity / downscale / encode / append failed.
 ...
 */
RA8_INTERNAL
static const char* internal_chapter_title(const epub_book_t* epub,
                                          ...
                                          epub_toc_entry_t*  entry_buf)
{
  for (uint16_t ti = 0U; ti < toc_count; ti++) {
    uint16_t ch_idx = 0U;
    if ((epub_toc_entry_to_chapter(epub, ti, &ch_idx) == k_ra8_ok) && (ch_idx == chapter_idx)) {
      if (epub_get_toc_entry(epub, ti, entry_buf) == k_ra8_ok) {
 ...
 *        points (@ref rabook_compile_from_epub and the buffer variant).
 ...
 *          is exactly `text/css`, loads its bytes via @ref epub_get_resource
 *          into @p scr->css, NUL-terminates them, interns the source string, and
 *          appends a book-wide stylesheet (scope @ref k_book_nil). An item
...
static ra8_err_t internal_compile_stylesheets(ra8_rabook_ctx_t*                    ctx,
                                              ...
                                              epub_book_t*                         epub)
{
  const uint16_t count = epub_manifest_count(epub);
  for (uint16_t i = 0U; i < count; i++) {
    const epub_manifest_item_t* item = epub_manifest_item(epub, i);
    if (item == nullptr) {
      continue;
    }
    ...
    size_t    got = 0U;
    ra8_err_t err =
      epub_get_resource(epub, item->href, (uint8_t*)scr->css, scr->css_cap - 1U, &got);
    ...
    (void)ra8_rabook_add_stylesheet(ctx, source_off, (uint32_t)k_book_nil);
  }
  return k_ra8_ok;
}
 ...
 * @return Image-table index of the added image, or @ref k_book_nil when the
 *         item is not an image, is absent, or failed to decode.
 * @retval k_book_nil Not an image / absent / undecodable (skipped).
                                            ...
                                            const epub_manifest_item_t*          item)
{
  ...
  uint8_t           fmt        = (uint8_t)k_book_image_gray4;
  if (strcmp(item->media_type, "image/svg+xml") == 0) {
    fmt = (uint8_t)k_book_image_svg;
  } else if (strncmp(item->media_type, img_prefix, strlen(img_prefix)) != 0) {
    return (uint32_t)k_book_nil; /* not an image item */
  } else {
    /* All remaining image media types use the raster decode path below. */
  }

#ifdef RA8_RABOOK_NO_RASTER
  if (fmt != (uint8_t)k_book_image_svg) {
  ...
  size_t    got = 0U;
  ra8_err_t err = epub_get_resource(epub, item->href, scr->image_raw, scr->image_cap, &got);
  if (err != k_ra8_ok) {
    return (uint32_t)k_book_nil; /* absent / unreadable -> skip, like the desktop */
  }
  ...
  if (fmt == (uint8_t)k_book_image_svg) {
    return ra8_rabook_add_image(ctx,
                                ...
                                0U,
                                0U,
                                (uint8_t)k_book_image_svg,
                                (uint8_t)k_book_pixfmt_gray4, /* unused for SVG; store 0 */
                                scr->image_raw,
                                (uint32_t)got);
  }
#ifndef RA8_RABOOK_NO_RASTER
...
#else
  return (uint32_t)k_book_nil; /* unreachable: raster returned nil above */
#endif
}
 ...
 *          @c epub->cover_path (epub already resolves that from
 ...
 * @pre The manifest was populated by @ref epub_open.
...
{
  *cover_index_out = (uint32_t)k_book_nil;
  if (scr->skip_images) {
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_compile_chapters(epub_book_t*                         epub,
                                           const ra8_rabook_pipeline_scratch_t* scr,
                                           ra8_rabook_ctx_t*                    ctx)
{
  uint16_t  chapter_count = 0U;
  ra8_err_t err           = epub_get_chapter_count(epub, &chapter_count);
  if (err != k_ra8_ok) {
    return err;
  }

  uint16_t toc_count = 0U;
  (void)epub_get_toc_count(epub, &toc_count);

  epub_toc_entry_t toc_entry = {};

  for (uint16_t ci = 0U; ci < chapter_count; ci++) {
    size_t got_len = 0U;
    err            = epub_load_chapter(epub, ci, scr->xhtml, scr->xhtml_cap, &got_len);
    if (err != k_ra8_ok) {
      return err;
    }
 ...
 * @retval k_ra8_err_invalid_arg @ref epub_get_metadata rejected @p epub.
...
RA8_INTERNAL
static ra8_err_t
internal_compile_metadata(ra8_rabook_ctx_t* ctx, epub_book_t* epub, uint32_t cover_image_index)
{
  epub_metadata_t meta = {};
  ra8_err_t       err  = epub_get_metadata(epub, &meta);
  if (err != k_ra8_ok) {
    return err;
  }
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_compile_to_blob(epub_book_t*                         epub,
                                          const ra8_rabook_buffers_t*          bufs,
                                          const ra8_rabook_pipeline_scratch_t* scr,
                                          const void**                         out_blob,
                                          uint32_t*                            out_len)
{
  ra8_rabook_ctx_t ctx = {};
  ra8_err_t        err = rabook_compile_init(&ctx, bufs);
  if (err != k_ra8_ok) {
    return err;
  }
  ...
  uint32_t cover_image_index = (uint32_t)k_book_nil;
