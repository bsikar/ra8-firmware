 ...
 * size fields) and walks them through `unarch_tar_*`, proving:
 *
 ...
 *   4. the promoted field parsers (`unarch_tar_internal.h`) hit every
...
#include "unarch_tar_fixture.h"
#include "unarch_tar_internal.h"
#include "unity_minimal.h"

/** @brief Compare two character spans for exact equality. */
RA8_INTERNAL static bool internal_chars_equal(const char* left, const char* right, size_t length)
{
...
/** @brief Compare unsigned output bytes with expected character data. */
RA8_INTERNAL static bool
internal_bytes_equal_text(const uint8_t* bytes, const char* text, size_t length)
{
  for (size_t i = 0U; i < length; ++i) {
 ...
 * @par MC/DC:
 * Decision owner:
 * apps/shared_libs/unarch/src/unarch_tar_fields.c@internal_octal
 * The octal scan guard `(i < len) && (field[i] != 0) && (field[i] != ' ')`
 * receives the four N+1 evaluations for its three conditions: an ordinary
 * digit supplies `(T,T,T)`; `f_full` reaches `(F,-,-)` at the exact bound;
 * `f_ok` reaches `(T,F,-)` at its NUL; and `f_sp` reaches `(T,T,F)` at its
 * space terminator. Each false vector varies one condition from the control.
...
static void internal_test_tar_num_fields(void)
{
  ...
  TEST_ASSERT(v == 0644U);
  const uint8_t f_full[3] = {'6', '4', '4'}; /* exact bound, no terminator */
  TEST_ASSERT_EQ(k_ra8_ok, priv_unarch_tar_num(f_full, sizeof(f_full), &v));
  TEST_ASSERT(v == 0644U);
  ...
  (void)memset(f_over, (int)'7', sizeof(f_over));
...
static void internal_test_tar_block_primitives(void)
{
  ...
  (void)memset(&block[k_tt_off_chksum], 0, k_tt_len_chksum); /* undecodable chksum field */
  TEST_ASSERT(!priv_unarch_tar_checksum_ok(block));
...
/**
 * @test internal_test_tar_full_width_name_mcdc
 * @brief A full-width ustar name exits its bounded scan through capacity.
 *
 * @par MC/DC:
 * Decision owner: apps/shared_libs/unarch/src/unarch_tar.c@internal_field_length
 * For `(length < capacity) && (block[offset + length] != 0)`, the full-width
 * name supplies `(T,T)` for each byte and `(F,-)` at byte 100. Ordinary names
 * throughout this suite supply `(T,T)` followed by `(T,F)` at their NUL.
 * Together these are the N+1 executions for the two-condition scan guard.
 */
RA8_INTERNAL static void internal_test_tar_full_width_name_mcdc(void)
{
  TEST_BEGIN("tar: full-width name field MC/DC");
  char full_name[k_tt_len_name + 1U];
  (void)memset(full_name, (int)'n', (size_t)k_tt_len_name);
  full_name[k_tt_len_name] = '\0';
  const size_t member_end  = internal_tb_add(0U, full_name, nullptr, (uint8_t)'0', nullptr, 0U);
  s_arc_len                = internal_tb_end(member_end);
  unarch_tar_t t;
  unarch_mem_t mem;
  TEST_ASSERT_EQ(k_ra8_ok, internal_tt_open(&t, &mem, nullptr));
  unarch_tar_entry_t entry;
  TEST_ASSERT_EQ(k_ra8_ok, unarch_tar_next(&t, 0U, s_name, sizeof(s_name), &entry));
  TEST_ASSERT_EQ(k_tt_len_name, entry.name_len);
  TEST_END("tar: full-width name field MC/DC");
}
 ...
 * @par MC/DC:
 * Decision owner:
 * apps/shared_libs/unarch/src/unarch_tar_fields.c@internal_pax_apply
 * Each key comparison initializes `is_path` / `is_size` from its length and
 * scans with `(is_key && (i < sizeof(key)))`:
 * - Vector 1: key "path" (len 4, match)    -> true  (T,T)
 * - Vector 2: key "abcd" (len 4, no match) -> false (T,F)
 * - Vector 3: key "mtime" (len 5)          -> false (F,-)
 * V1 supplies loop `(T,T)` and its exact-bound `(T,F)` exit; V2 makes the next
 * guard `(F,T)` after a byte mismatch; V3 holds the initializer false. These
 * are the N+1 executions for both the path and size loops.
 * Additionally, the following decision is covered by the same input table:
 * Decision owner:
 * apps/shared_libs/unarch/src/unarch_tar_fields.c@priv_unarch_tar_pax_parse
 * The key scan `(key_len < kv_len) && (kv[key_len] != '=')` sees `(T,T)` on
 * honest path/size keys, `(T,F)` at their delimiter, and `(F,-)` for `d_noeq`
 * in `internal_tpx_malformed`; both conditions independently stop the scan.
...
static void internal_test_tar_pax_records(void)
{
  ...
  TEST_ASSERT(internal_chars_equal(nb, "x/y.png", 7U));
  TEST_ASSERT(hsize);
 ...
 */
RA8_INTERNAL
static void internal_tar_walk_tail(unarch_tar_t* t, unarch_tar_entry_t* e, const char* d3)
{
  size_t got = 0U;
  /* 4: the pax member (path + size overrides applied). */
  TEST_ASSERT_EQ(k_ra8_ok, unarch_tar_next(t, e->next_off, s_name, sizeof(s_name), e));
  TEST_ASSERT_EQ(1U, e->is_file);
  TEST_ASSERT(internal_chars_equal(s_name, "pax/page2.png", 13U));
  TEST_ASSERT(e->size == (uint64_t)(strlen(d3)));
  TEST_ASSERT_EQ(k_ra8_ok, unarch_tar_read(t, e, s_out, sizeof(s_out), &got));
  TEST_ASSERT(internal_bytes_equal_text(s_out, d3, got));
  /* 5: the symlink -- enumerated, neither file nor dir. */
  TEST_ASSERT_EQ(k_ra8_ok, unarch_tar_next(t, e->next_off, s_name, sizeof(s_name), e));
  TEST_ASSERT_EQ(0U, e->is_file);
  TEST_ASSERT_EQ(0U, e->is_dir);
  /* 6: the end marker. */
  TEST_ASSERT_EQ(k_ra8_err_not_found, unarch_tar_next(t, e->next_off, s_name, sizeof(s_name), e));
}
...
{
  static const char honest_pax[] = "22 path=pax/page2.png\n10 size=9\n";
  size_t            off          = 0U;
  ...
  off =
    internal_tb_add(off, "ignored2", nullptr, (uint8_t)'x', honest_pax, sizeof(honest_pax) - 1U);
  off =
 ...
 * in apps/shared_libs/unarch/src/unarch_tar.c@unarch_tar_next (2 conditions)
...
static void internal_test_tar_walk_honest_archive(void)
{
  ...
  TEST_ASSERT(internal_chars_equal(s_name, "dir/", 4U));
  /* 2: prefix-joined file. */
  TEST_ASSERT_EQ(k_ra8_ok, unarch_tar_next(&t, e.next_off, s_name, sizeof(s_name), &e));
  TEST_ASSERT_EQ(1U, e.is_file);
  TEST_ASSERT(internal_chars_equal(s_name, "vol1/ch1/b.png", 14U));
  TEST_ASSERT(e.size == (uint64_t)(sizeof(s_honest_d1) - 1U));
  size_t got = 0U;
  TEST_ASSERT_EQ(k_ra8_ok, unarch_tar_read(&t, &e, s_out, sizeof(s_out), &got));
  TEST_ASSERT_EQ(sizeof(s_honest_d1) - 1U, got);
  TEST_ASSERT(internal_bytes_equal_text(s_out, s_honest_d1, got));
  /* 3: the longname member (its 'L' prelude was consumed transparently). */
  TEST_ASSERT_EQ(k_ra8_ok, unarch_tar_next(&t, e.next_off, s_name, sizeof(s_name), &e));
  TEST_ASSERT_EQ(1U, e.is_file);
  TEST_ASSERT_EQ(sizeof(s_honest_lname) - 1U, e.name_len);
  TEST_ASSERT(internal_chars_equal(s_name, s_honest_lname, e.name_len));
  TEST_ASSERT_EQ(k_ra8_ok, unarch_tar_read(&t, &e, s_out, sizeof(s_out), &got));
  TEST_ASSERT(internal_bytes_equal_text(s_out, s_honest_d2, got));
...
static void internal_test_tar_open_probe_guards(void)
{
  ...
  TEST_ASSERT(!unarch_tar_probe(nullptr, 512U));
  TEST_ASSERT(!unarch_tar_probe(s_arc, 511U));
  TEST_ASSERT(!unarch_tar_probe(s_arc, 512U)); /* zero block: no magic */
  uint8_t good[k_tt_tar_block] = {};
  internal_tb_header(good, "x.png", nullptr, (uint8_t)'0', 0U);
  TEST_ASSERT(unarch_tar_probe(good, sizeof(good)));
  good[k_tt_off_magic_nul] = (uint8_t)'X'; /* magic passes "ustar", term fails */
  TEST_ASSERT(!unarch_tar_probe(good, sizeof(good)));

  unarch_tar_t t   = {};
  unarch_mem_t mem = {.base = s_arc, .len = s_arc_len};
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 unarch_tar_open(nullptr, unarch_mem_read, &mem, 1024U, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, unarch_tar_open(&t, nullptr, &mem, 1024U, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, unarch_tar_open(&t, unarch_mem_read, &mem, 511U, nullptr));
  ra8_decomp_limits_t bad = ra8_decomp_limits_default();
  bad.max_entries         = 0U;
  TEST_ASSERT_EQ(k_ra8_err_invalid_arg, unarch_tar_open(&t, unarch_mem_read, &mem, 1024U, &bad));
  /* Zero blocks are not a tar header: refused at open. */
  TEST_ASSERT_EQ(k_ra8_err_not_supported,
                 unarch_tar_open(&t, unarch_mem_read, &mem, (uint64_t)s_arc_len, nullptr));
  /* A backing that cannot serve one whole block. */
  mem.len = k_tt_len_name;
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 unarch_tar_open(&t, unarch_mem_read, &mem, 1024U, nullptr));

  /* next/read on a dead walker. */
  unarch_tar_entry_t e = {};
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, unarch_tar_next(&t, 0U, s_name, sizeof(s_name), &e));
  size_t got = 0U;
  TEST_ASSERT_EQ(k_ra8_err_invalid_state, unarch_tar_read(&t, &e, s_out, sizeof(s_out), &got));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, unarch_tar_next(nullptr, 0U, s_name, sizeof(s_name), &e));
 ...
 */
RA8_INTERNAL
static void internal_tar_hostile_pax_floods(unarch_tar_t*       t,
                                            unarch_mem_t*       mem,
                                            unarch_tar_entry_t* e,
                                            const char*         data)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, internal_tt_open(t, mem, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_validation_failed, unarch_tar_next(t, 0U, s_name, sizeof(s_name), e));

  /* Oversized pax data (over k_unarch_tar_pax_max). */
  uint8_t big[k_t_pax_oversize];
  (void)memset(big, (int)'a', sizeof(big));
...
static void internal_test_tar_hostile_headers(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, unarch_tar_next(&t, 0U, s_name, sizeof(s_name), &e));
  TEST_ASSERT_EQ(k_ra8_err_validation_failed,
                 unarch_tar_next(&t, e.next_off, s_name, sizeof(s_name), &e));
  ...
  (void)memset(big_member, (int)'m', sizeof(big_member));
...
static void internal_test_tar_policy_bounds(void)
{
  ...
  TEST_ASSERT_EQ(k_ra8_ok, internal_tt_open(&t, &mem, &lim));
  TEST_ASSERT_EQ(k_ra8_err_decomp_output_cap, unarch_tar_next(&t, 0U, s_name, sizeof(s_name), &e));
...
static void internal_test_tar_read_guards(void)
{
  ...
  unarch_mem_t mem = {};
  TEST_ASSERT_EQ(k_ra8_ok, internal_tt_open(&t, &mem, nullptr));
  unarch_tar_entry_t dir = {};
  TEST_ASSERT_EQ(k_ra8_ok, unarch_tar_next(&t, 0U, s_name, sizeof(s_name), &dir));
  unarch_tar_entry_t file = {};
  TEST_ASSERT_EQ(k_ra8_ok, unarch_tar_next(&t, dir.next_off, s_name, sizeof(s_name), &file));
  unarch_tar_entry_t empty = {};
  TEST_ASSERT_EQ(k_ra8_ok, unarch_tar_next(&t, file.next_off, s_name, sizeof(s_name), &empty));

  size_t got = 1U;
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, unarch_tar_read(nullptr, &file, s_out, sizeof(s_out), &got));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, unarch_tar_read(&t, nullptr, s_out, sizeof(s_out), &got));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, unarch_tar_read(&t, &file, nullptr, sizeof(s_out), &got));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr, unarch_tar_read(&t, &file, s_out, sizeof(s_out), nullptr));
  TEST_ASSERT_EQ(k_ra8_err_not_supported, unarch_tar_read(&t, &dir, s_out, sizeof(s_out), &got));
  TEST_ASSERT_EQ(0U, got);
  TEST_ASSERT_EQ(k_ra8_err_no_mem, unarch_tar_read(&t, &file, s_out, 4U, &got));

  /* Forged entries: data area outside / overrunning the archive. */
  unarch_tar_entry_t forged = file;
  forged.data_off           = (uint64_t)s_arc_len + k_tt_tar_block;
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, unarch_tar_read(&t, &forged, s_out, sizeof(s_out), &got));
  forged      = file;
  forged.size = (uint64_t)s_arc_len; /* overruns from a valid offset */
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 unarch_tar_read(&t, &forged, s_out, (size_t)forged.size + 1U, &got));

  /* Empty member: k_ra8_ok with zero bytes. */
  TEST_ASSERT_EQ(k_ra8_ok, unarch_tar_read(&t, &empty, s_out, sizeof(s_out), &got));
  TEST_ASSERT_EQ(0U, got);

  /* Honest read still works after the hostile probes. */
  TEST_ASSERT_EQ(k_ra8_ok, unarch_tar_read(&t, &file, s_out, sizeof(s_out), &got));
  TEST_ASSERT_EQ(sizeof(data) - 1U, got);
  TEST_ASSERT(internal_bytes_equal_text(s_out, data, got));

  /* A backing that shrinks under the reader (short read). */
  mem.len = (size_t)file.data_off + 2U;
  TEST_ASSERT_EQ(k_ra8_err_invalid_size, unarch_tar_read(&t, &file, s_out, sizeof(s_out), &got));
  TEST_ASSERT_EQ(0U, got);
...
static void internal_test_tar_base256_and_gnu_magic(void)
{
  ...
  (void)memset(&s_arc[k_tt_off_size], 0, k_tt_len_size);
...
int main(void)
{
  ...
  internal_test_tar_full_width_name_mcdc();
