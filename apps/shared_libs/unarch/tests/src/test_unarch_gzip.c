...
/**
 * @brief Append the optional gzip header fields selected by the flag byte.
 * @details Serializes the bounded FEXTRA, FNAME, FCOMMENT, and FHCRC test
 ...
 * @since Version 0.1.0
 */
RA8_INTERNAL static void internal_tg_append_optional_fields(uint8_t flg, size_t* offset)
{
  TEST_ASSERT(offset != nullptr);
  TEST_ASSERT(*offset < sizeof(s_member));
  size_t off = *offset;
  if ((flg & 0x04U) != 0U) { /* FEXTRA */
    ...
    s_member[off] = 0U;
    off += 1U;
    (void)memcpy(&s_member[off], "EXTR", 4U);
    off += 4U;
  }
  if ((flg & 0x08U) != 0U) { /* FNAME */
    (void)memcpy(&s_member[off], "name.tar", (size_t)k_tg_len_fname);
    off += k_t_method_wrong;
  }
  if ((flg & 0x10U) != 0U) { /* FCOMMENT */
    (void)memcpy(&s_member[off], "comment", 8U);
    off += 8U;
  }
...
static size_t internal_tg_build(const uint8_t* payload, size_t n, uint8_t flg)
{
  ...
  s_member[off] = flg;
  off += 1U;
  (void)memset(&s_member[off], 0, 6U); /* MTIME, XFL, OS */
  off += 6U;
  internal_tg_append_optional_fields(flg, &off);
...
internal_tg_unwrap(const uint8_t* buf, size_t len, const ra8_decomp_limits_t* lim, size_t* out_len)
{
  ...
  return unarch_gzip_unwrap(unarch_mem_read,
                            &mem,
                            (uint64_t)len,
                            s_arena,
                            sizeof(s_arena),
...
static void internal_test_gzip_magic_and_guards(void)
{
  ...
  TEST_ASSERT(!unarch_gzip_magic(nullptr, 2U));
  TEST_ASSERT(!unarch_gzip_magic(sig, 1U));
  TEST_ASSERT(unarch_gzip_magic(sig, 2U));
  const uint8_t bad1[2] = {0x1EU, 0x8BU};
  TEST_ASSERT(!unarch_gzip_magic(bad1, 2U));
  const uint8_t bad2[2] = {0x1FU, 0x8CU};
  TEST_ASSERT(!unarch_gzip_magic(bad2, 2U));

  size_t       got = 1U;
  unarch_mem_t mem = {.base = s_member, .len = 32U};
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 unarch_gzip_unwrap(nullptr, &mem, 32U, s_arena, 8U, nullptr, &got));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 unarch_gzip_unwrap(unarch_mem_read, &mem, 32U, nullptr, 8U, nullptr, &got));
  TEST_ASSERT_EQ(k_ra8_err_null_ptr,
                 unarch_gzip_unwrap(unarch_mem_read, &mem, 32U, s_arena, 8U, nullptr, nullptr));
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 unarch_gzip_unwrap(unarch_mem_read, &mem, 0U, s_arena, 8U, nullptr, &got));
  TEST_ASSERT_EQ(k_ra8_err_invalid_size,
                 unarch_gzip_unwrap(unarch_mem_read, &mem, 32U, s_arena, 0U, nullptr, &got));
...
/**
 * @brief Verify variable-length hostile gzip headers fail closed.
 * @param[out] got Receives the decoder's reported output length.
 */
RA8_INTERNAL
static void internal_assert_hostile_variable_headers(size_t* got)
{
  ...
  tiny[2]          = 8U;
  tiny[3]          = 0x08U;
  (void)memset(&tiny[k_tg_off_extra], (int)'n', (size_t)k_tg_len_unterm);
  TEST_ASSERT_EQ(k_ra8_err_validation_failed, internal_tg_unwrap(tiny, sizeof(tiny), nullptr, got));

  /* Over-long FNAME: longer than the fail-closed cap, NUL far away. */
  uint8_t* big = s_member;
  (void)memset(big, 0, sizeof(s_member));
  big[0] = k_t_magic_b0;
  big[1] = k_t_magic_b1;
  big[2] = 8U;
  big[3] = 0x08U;
  (void)memset(&big[k_tg_off_extra], (int)'n', (size_t)k_tg_longname);
...
static void internal_test_gzip_hostile_headers(void)
{
  ...
  internal_assert_hostile_variable_headers(&got);
...
static void internal_test_gzip_policy_bounds(void)
{
  ...
  (void)memset(s_payload, 0, (size_t)k_tg_zeros);
  ...
  unarch_mem_t mem = {.base = s_member, .len = len};
  TEST_ASSERT_EQ(
    k_ra8_err_no_mem,
    unarch_gzip_unwrap(unarch_mem_read, &mem, (uint64_t)len, s_arena, 1024U, nullptr, &got));
  TEST_ASSERT_EQ(0U, got);
