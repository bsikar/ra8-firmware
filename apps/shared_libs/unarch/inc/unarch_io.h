 ...
 * flat-memory view (::unarch_mem_t + ::unarch_mem_read) used to
 ...
 *
 * @see unarch_tar.h  The tar walker driven through this seam.
 * @see unarch_gzip.h The gzip member decoder driven through this seam.
 * @see unarch_xz.h   The XZ stream decoder driven through this seam.
 *
...
/**
 * @typedef unarch_read_fn
 * @brief Seek+read backing over an archive's bytes.
 * @details Identical in shape to `comic_read_fn` / `ra8_rar_read_fn`: an
...
/**
 * @struct unarch_mem_t
 * @brief Bounded flat-memory backing for ::unarch_read_fn.
 *
...
/**
 * @brief ::unarch_read_fn over a flat ::unarch_mem_t view.
 *
 ...
 *
 * @param[in]  ctx    The ::unarch_mem_t view (may be NULL: reads 0).
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] size_t unarch_mem_read(void* ctx, uint64_t offset, void* buf, size_t len);
