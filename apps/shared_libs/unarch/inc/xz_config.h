 ...
 *
 * @details
 * xz-embedded (SOUP, `apps/shared_libs/third_party/xz_embedded/`) is written against the
 ...
 * `unarch_xz_pool.h` (NASA P10 Rule 3 -- this firmware traps `_sbrk`).
 *
 ...
 *
 * @see unarch_xz_pool.h The bump arena `kmalloc` / `vmalloc` map onto.
 * @see unarch_xz.h      The bounded decode wrapper over the SOUP.
 ...
 *          and never grown. The only mode the `unarch_xz` wrapper
 ...
 * @since Version 0.1.0
 */
#define kmalloc(size, flags) unarch_xz_pool_alloc((uint32_t)(size))
 ...
 * @since Version 0.1.0
 */
#define vmalloc(size) unarch_xz_pool_alloc((uint32_t)(size))
