 ...
 * the shared seek+read seam (`unarch_io.h`) and exposes each member
...
typedef enum : uint16_t {
  k_unarch_tar_block    = 512U,  /**< On-disk block / header size.          */
  k_unarch_tar_meta_max = 4U,    /**< Max meta (x/g/L/K) blocks per member. */
  k_unarch_tar_pax_max  = 2048U, /**< Max pax / longname data bytes.        */
} unarch_tar_dims_t;
 ...
 *
 * @details Populated by ::unarch_tar_open; treat every field as
 *          read-only outside the API. The embedded budget accumulates
 *          across ::unarch_tar_next calls, so the policy's entry cap
 ...
 */
typedef struct {
  unarch_read_fn      read;   /**< Byte reader over the archive.          */
 ...
 *
 * @details Filled by ::unarch_tar_next. @p next_off always advances
 ...
 *
 * @details Pure probe over the first ::k_unarch_tar_block bytes: the
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] bool unarch_tar_probe(const uint8_t* block, size_t len);
 ...
 * @pre @p t is a writable ::unarch_tar_t.
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t unarch_tar_open(unarch_tar_t*              t,
                                        unarch_read_fn             read,
                                        void*                      ctx,
                                        uint64_t                   size,
 ...
 *          ::k_unarch_tar_meta_max blocks and
 *          ::k_unarch_tar_pax_max data bytes), decodes the real
 ...
 *
 * @param[in,out] t        Walker bound by ::unarch_tar_open (non-NULL).
 ...
 *
 * @pre @p t was populated by ::unarch_tar_open.
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t unarch_tar_next(unarch_tar_t*       t,
                                        uint64_t            off,
                                        char*               name_buf,
                                        uint16_t            name_cap,
 ...
 *
 * @param[in]  t   Walker bound by ::unarch_tar_open (non-NULL).
 * @param[in]  ent A file member from ::unarch_tar_next (non-NULL,
 *                 `is_file == 1`).
 ...
 *
 * @pre @p t was populated by ::unarch_tar_open.
 * @pre @p ent came from ::unarch_tar_next on the same @p t.
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t unarch_tar_read(const unarch_tar_t*       t,
