 ...
 *
 * @details
 * The vendored xz-embedded decoder (SOUP,
...
typedef enum : uint32_t {
  k_unarch_xz_pool_align = 8U, /**< Bump-allocation alignment, bytes. */
 ...
 *
 * @pre @p base is aligned to ::k_unarch_xz_pool_align.
 * @pre No other XZ decode is in flight (single-threaded reader loop).
 * @post On k_ra8_ok the pool serves ::unarch_xz_pool_alloc calls.
 * @post On any error the pool state is unchanged.
 *
 * @note Not thread-safe.
 * @see unarch_xz_pool_reset()
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t unarch_xz_pool_install(void* base, uint32_t len);
 ...
 * @post The pool is uninstalled; ::unarch_xz_pool_alloc returns NULL.
 * @post A fresh ::unarch_xz_pool_install may follow.
 *
 * @note Not thread-safe.
 * @see unarch_xz_pool_install()
 * @since Version 0.1.0
 */
 ...
 *
 * @pre ::unarch_xz_pool_install succeeded (else NULL is returned).
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] void* unarch_xz_pool_alloc(uint32_t size);
 ...
 *
 * @note Not thread-safe.
 * @see unarch_xz_pool_alloc()
 * @since Version 0.1.0
 */
[[nodiscard]] uint32_t unarch_xz_pool_used(void);
