 ...
 * decoder (`apps/shared_libs/third_party/xz_embedded/`, decode-only, 0BSD) behind two
 * bounded shapes:
 *
 * - ::unarch_xz_stream_begin / `_run` / `_end` -- a multi-call decode
 ...
 *   scratch buffer** through the zero-heap pool (`unarch_xz_pool.h`) and
 *   never grow: a stream whose declared dictionary exceeds the scratch is
 *   rejected fail-closed instead of allocating.
 * - ::unarch_xz_unwrap -- decode one whole `.xz` stream fetched through
 ...
 * @see unarch_io.h      The seek+read seam the unwrap consumes.
...
/**
 * @enum unarch_xz_dims_t
 * @brief Scratch sizing constants for the XZ decoder wrappers.
 * @details ::k_unarch_xz_state_reserve is the slice of a streaming
 ...
 *          dictionary a stream may declare. ::k_unarch_xz_sig_len is
 *          the length of the XZ stream header magic used for container
 *          detection.
 * @since Version 0.1.0
 */
typedef enum : uint32_t {
  k_unarch_xz_state_reserve = 64U * 1024U, /**< Decoder-state slice of the scratch. */
  k_unarch_xz_sig_len       = 6U,          /**< XZ stream header magic length.      */
} unarch_xz_dims_t;
 ...
 *
 * @details Bound by ::unarch_xz_stream_begin over a caller scratch;
 *          `dec` points into that scratch (via the XZ pool) and is owned by
 *          the session until ::unarch_xz_stream_end. Treat the fields
 ...
 * @see unarch_xz_unwrap()
 * @since Version 0.1.0
 */
[[nodiscard]] bool unarch_xz_magic(const uint8_t* sig, size_t sig_len);
 ...
 *          `scratch_len - k_unarch_xz_state_reserve`: the dictionary is
 ...
 * @param[in]  scratch_len Scratch length, > ::k_unarch_xz_state_reserve.
 *
 ...
 * @post On k_ra8_ok, `xs->live` is true until ::unarch_xz_stream_end.
 * @post On any error the pool is released and @p xs is dead.
 *
 * @note Not thread-safe.
 * @see unarch_xz_stream_run()
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t
unarch_xz_stream_begin(unarch_xz_stream_t* xs, void* scratch, uint32_t scratch_len);
 ...
 *
 * @pre ::unarch_xz_stream_begin succeeded on @p xs.
 ...
 *
 * @note Not thread-safe.
 * @see unarch_xz_stream_end()
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t unarch_xz_stream_run(unarch_xz_stream_t* xs,
                                             const uint8_t*      in,
                                             size_t              in_len,
                                             size_t*             in_used,
                                             uint8_t*            out,
 ...
 * @post A subsequent ::unarch_xz_stream_begin may reuse the scratch.
 *
 * @note Not thread-safe.
 * @see unarch_xz_stream_begin()
 * @since Version 0.1.0
 ...
 *                         > ::k_unarch_xz_state_reserve bytes).
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra8_err_t unarch_xz_unwrap(unarch_read_fn             read,
                                         void*                      ctx,
                                         uint64_t                   size,
                                         uint8_t*                   out,
                                         size_t                     out_cap,
