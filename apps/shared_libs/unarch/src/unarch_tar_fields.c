 ...
 * `unarch_tar_internal.h`) so the host tests drive each rejection
...
static ra8_err_t internal_octal(const uint8_t* field, size_t len, uint64_t* out)
{
  ...
  uint64_t v      = 0U;
  size_t   digits = 0U;
  while ((i < len) && (field[i] != 0U) && (field[i] != (uint8_t)' ')) { /* bound: len */
    ...
    const uint64_t d = (uint64_t)c - (uint64_t)(uint8_t)'0';
...
{
  for (uint32_t i = 0U; i < (uint32_t)k_unarch_tar_block; ++i) { /* bound: block size */
    if (block[i] != 0U) {
      return false;
    }
  }
...
{
  static const uint8_t magic[] = {'u', 's', 't', 'a', 'r'};
  for (size_t i = 0U; i < sizeof(magic); ++i) {
    if (block[(size_t)k_ra8_tar_off_magic + i] != magic[i]) {
      return false;
    }
  }
...
static ra8_err_t internal_pax_reclen(const uint8_t* data, size_t len, size_t* reclen, size_t* body)
{
  ...
  while (i < len) { /* bound: len */
    ...
    v = (v * (uint64_t)k_tar_decimal_base) + ((uint64_t)c - (uint64_t)(uint8_t)'0');
    digits += 1U;
    i += 1U;
  }
  if (digits == 0U) {
...
static ra8_err_t internal_pax_apply(const uint8_t* key,
                                    ...
                                    bool*          have_size)
{
  static const uint8_t key_path[] = {'p', 'a', 't', 'h'};
  static const uint8_t key_size[] = {'s', 'i', 'z', 'e'};
  bool                 is_path    = key_len == sizeof(key_path);
  for (size_t i = 0U; is_path && (i < sizeof(key_path)); ++i) {
    is_path = key[i] == key_path[i];
  }
  if (is_path) {
  ...
  bool is_size = key_len == sizeof(key_size);
  for (size_t i = 0U; is_size && (i < sizeof(key_size)); ++i) {
    is_size = key[i] == key_size[i];
  }
  if (is_size) {
...
// NOLINTNEXTLINE(readability-function-size,readability-function-cognitive-complexity) -- keyed pax record state machine: length, key and value share one bounded cursor, so splitting it would duplicate that cursor state across helpers.
                                             ...
                                             bool*          have_size)
{
  ...
  while (off < len) { /* bound: every record advances by reclen >= body + 1 */
    ...
    size_t         key_len = 0U;
    while ((key_len < kv_len) && (kv[key_len] != (uint8_t)'=')) {
      key_len += 1U;
    }
    if (key_len == kv_len) {
