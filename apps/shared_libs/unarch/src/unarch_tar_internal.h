 ...
 * (`unarch_tar_fields.c`) behind these TU-external declarations so the
 ...
 * callers are exclusively `unarch_tar.c`; nothing outside the module
 ...
 *
 * @param[in] block One ::k_unarch_tar_block byte block (non-NULL).
 *
 ...
 * @param[in]  len       pax data length (<= ::k_unarch_tar_pax_max).
