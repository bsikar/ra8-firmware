 ...
 *
 * @details
 * See `unarch_xz.h` for the contract. The session shape drives the SOUP
 * decoder in `XZ_PREALLOC` mode: begin installs the caller scratch as the
 * zero-heap allocation arena (`unarch_xz_pool.h`), reserves
 * `k_unarch_xz_state_reserve` of it for the decoder state, and hands the
...
bool unarch_xz_magic(const uint8_t* sig, size_t sig_len)
{
  ...
  if (sig_len < (size_t)k_unarch_xz_sig_len) {
    return false;
  }
  const uint8_t want[k_unarch_xz_sig_len] = {
...
ra8_err_t unarch_xz_stream_begin(unarch_xz_stream_t* xs, void* scratch, uint32_t scratch_len)
{
  ...
  *xs = (unarch_xz_stream_t){};
  RA8_CHECK_NULL_PTR(scratch, s_tag_xz, "begin: null scratch");
  if (scratch_len <= (uint32_t)k_unarch_xz_state_reserve) {
    return k_ra8_err_invalid_size;
  }
  const ra8_err_t perr = unarch_xz_pool_install(scratch, scratch_len);
  if (perr != k_ra8_ok) {
    return perr;
  }
  ...
  xz_crc32_init();
  xz_crc64_init();
  const uint32_t dict_max = scratch_len - (uint32_t)k_unarch_xz_state_reserve;
...
// NOLINTNEXTLINE(readability-function-size) -- atomic decoder cursor transaction.
...
/**
 * @brief Reject any NULL required argument to ::unarch_xz_unwrap.
 ...
 * @pre The caller forwards ::unarch_xz_unwrap's arguments unchanged.
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_unwrap_reject_null(unarch_read_fn read,
                                             const uint8_t* out,
                                             const void*    scratch,
                                             const size_t*  out_len)
{
  if (read == nullptr) {
    ra8_log_error(s_tag_xz, "unwrap: null read");
 ...
 */
typedef struct {
  unarch_read_fn      read;              /**< Byte reader over the stream.    */
  ...
  unarch_xz_stream_t  xs;                /**< Live decode session.            */
...
static ra8_err_t internal_unwrap_pass(xz_unwrap_state_t* st, bool* end)
{
  ...
  size_t          used     = 0U;
  size_t          produced = 0U;
  const ra8_err_t rerr     = unarch_xz_stream_run(&st->xs,
                                                  st->chunk,
                                                  got,
                                                  &used,
                                                  &st->out[st->total_out],
...
// NOLINTNEXTLINE(readability-function-size) -- contract macros expand the bounded decode driver.
                           ...
                           size_t*                    out_len)
{
  ...
  *out_len = 0U;
  ...
  const ra8_err_t serr = unarch_xz_stream_begin(&st.xs, scratch, scratch_len);
  if (serr != k_ra8_ok) {
    return serr;
  }
  bool      end  = false;
