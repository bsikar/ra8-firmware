 ...
 *
 * @details
 * See `unarch_gzip.h` for the contract. The TU is organised as a small
 ...
 */
typedef struct {
  unarch_read_fn read;      /**< Byte reader over the member.       */
 ...
 *          ::k_unarch_gzip_str_max or an unterminated one (EOF first)
 *          rejects the member.
...
{
  for (uint32_t i = 0U; i < (uint32_t)k_unarch_gzip_str_max; ++i) { /* bound: string cap */
    uint8_t         b    = 0U;
 ...
 */
RA8_INTERNAL
// NOLINTNEXTLINE(readability-function-size) -- wire-order guards keep cursor and header CRC mutations auditable.
 ...
 */
RA8_INTERNAL
// NOLINTNEXTLINE(readability-function-size) -- atomic miniz cursor and budget transaction.
...
bool unarch_gzip_magic(const uint8_t* sig, size_t sig_len)
{
  ...
  if (sig_len < (size_t)k_unarch_gzip_sig_len) {
    return false;
  }
...
// NOLINTNEXTLINE(readability-function-size) -- fail-closed member transaction.
ra8_err_t unarch_gzip_unwrap(unarch_read_fn             read,
                             ...
                             size_t*                    out_len)
{
  static const char* const tag = "unarch_gzip";
  if (read == nullptr) {
    ra8_log_error(tag, "unwrap: null read");
    return k_ra8_err_null_ptr;
  }
  RA8_CHECK_NULL_PTR(out, tag, "unwrap: null out");
  RA8_CHECK_NULL_PTR(out_len, tag, "unwrap: null out_len");
  *out_len = 0U;
  if (size == 0U) {
