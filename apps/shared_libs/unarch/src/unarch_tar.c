 ...
 *
 * @details
 * The block-chain half of the clean-room tar reader (`unarch_tar.h`);
 * the untrusted-byte field parsers live in `unarch_tar_fields.c`. One
 * ::unarch_tar_next call consumes a member's bounded pax / GNU meta
 ...
 * @details Reset at the start of every ::unarch_tar_next; the real
...
bool unarch_tar_probe(const uint8_t* block, size_t len)
{
  ...
  if (len < (size_t)k_unarch_tar_block) {
    return false;
  }
                          ...
                          const ra8_decomp_limits_t* limits)
{
  ...
  *t = (unarch_tar_t){};
  if (read == nullptr) {
    ra8_log_error(s_tag_tar, "open: null read");
    return k_ra8_err_null_ptr;
  }
  if (size < (uint64_t)k_unarch_tar_block) {
  ...
  uint8_t      block[k_unarch_tar_block] = {};
  const size_t got                       = read(ctx, 0U, block, sizeof(block));
  if (got != sizeof(block)) {
    return k_ra8_err_invalid_size;
  }
  if (!unarch_tar_probe(block, sizeof(block))) {
 ...
 * @param[out] block Destination (::k_unarch_tar_block bytes).
...
{
  const uint64_t need = (uint64_t)k_unarch_tar_block;
  if (off > (t->size - need)) {
...
{
  const uint64_t blk = (uint64_t)k_unarch_tar_block;
 ...
 * @details Bounds the data to ::k_unarch_tar_pax_max, reads it whole,
 *          and applies it: a pax header contributes `path` / `size`
 *          overrides, a longname contributes the (NUL-stripped) name.
 *          Skipped meta kinds ('g', 'K') consume nothing here. A function-local
 *          static buffer keeps the frame below the first-party stack budget; the
 *          single-threaded tar walker overwrites it for each metadata block.
 * @param[in]     t     Live walker.
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_meta_consume(const unarch_tar_t* t,
                                       ra8_tar_type_t      type,
                                       uint64_t            doff,
                                       uint64_t            dsize,
                                       tar_meta_t*         meta)
{
  static uint8_t s_tar_meta_scratch[k_unarch_tar_pax_max];
  if (dsize > (uint64_t)k_unarch_tar_pax_max) {
...
/**
 * @brief Measure one NUL-terminated-or-full ustar header field.
 * @details Scans no farther than @p capacity bytes from @p offset.
 * @param[in] block Complete ustar header block.
 * @param[in] offset Byte offset of the field within @p block.
 * @param[in] capacity Maximum field width in bytes.
 * @return Field length before the first NUL or the full capacity.
 * @retval 0 The first field byte is NUL or capacity is zero.
 * @pre @p block is readable through @p offset plus @p capacity bytes.
 * @pre The offset and capacity describe one field within the header block.
 * @post No header byte is modified.
 * @post The result does not exceed @p capacity.
...
{
  size_t length = 0U;
  while ((length < (size_t)capacity) && (block[offset + length] != 0U)) {
    length++;
  }
  return length;
}
 ...
 * @details Used when no pax path or longname override exists. A non-empty
 *          prefix contributes `prefix '/'` before the name field.
 * @param[in] block Complete member header block.
 * @param[in,out] meta Accumulator whose name buffer receives the result.
 * @pre @p block is a full validated header block.
 * @pre `meta->name_buf` holds `meta->name_cap` writable bytes when capacity is nonzero.
...
{
  const size_t n_len =
    internal_field_length(block, (uint32_t)k_ra8_tar_off_name, (uint32_t)k_ra8_tar_len_name);
  const size_t p_len =
    internal_field_length(block, (uint32_t)k_ra8_tar_off_prefix, (uint32_t)k_ra8_tar_len_prefix);
  size_t used = 0U;
  if (p_len > 0U) {
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_finish_member(unarch_tar_t*       t,
                                        const uint8_t*      block,
                                        uint64_t            off,
                                        ra8_tar_type_t      type,
                                        uint64_t            dsize,
                                        ...
                                        unarch_tar_entry_t* out)
{
  ...
  const uint64_t doff = off + (uint64_t)k_unarch_tar_block;
...
// NOLINTNEXTLINE(readability-function-size,readability-function-cognitive-complexity) -- one ustar header record is parsed field-by-field in declaration order; splitting it would scatter the bounds checks away from the fields they guard.
                          ...
                          unarch_tar_entry_t* out)
{
  ...
  *out = (unarch_tar_entry_t){};
  ...
  uint64_t   cur  = off;
  for (uint32_t m = 0U; m <= (uint32_t)k_unarch_tar_meta_max; ++m) { /* bound: meta cap */
    ...
    if ((type == k_ra8_tar_type_pax) || (type == k_ra8_tar_type_longname)) {
      const uint64_t doff = cur + (uint64_t)k_unarch_tar_block;
...
// NOLINTNEXTLINE(readability-function-size) -- contract macros expand the linear read body.
