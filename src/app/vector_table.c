 ...
 * after ra_isr_register armed the NVIC line halted the chip -- this NSC
 * demo routes USBFS_INT to NVIC slot 0 (ra_usb_device_attach), so it hit
...
void Reset_Handler(void)
{
   ...
   * full Secure bring-up (clock tree + SAU + `ra_trustzone_init`), and
   * ra_trustzone_init() BLXNS-es into the Non-Secure world and never returns on
