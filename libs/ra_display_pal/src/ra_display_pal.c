...
static const char* const s_tag = "ra_display_pal";
 ...
 * @retval k_ra_ok               ``d`` points at the live handle.
 * @retval k_ra_err_null_ptr     ``d`` was NULL.
 * @retval k_ra_err_invalid_arg  ``d`` did not match the live handle
 ...
 * @since 0.1.0
 */
static ra_err_t internal_validate_handle(const display_handle_t* d)
{
  if (d == nullptr) {
...
ra_err_t display_init(const display_cfg_t* cfg, display_handle_t** out_handle)
{
  ...
  RA_CHECK_NULL_PTR(out_handle, s_tag, "out_handle must not be nullptr");
  RA_CHECK_NULL_PTR(cfg->iface, s_tag, "cfg->iface must not be nullptr");
  RA_CHECK_NULL_PTR(cfg->iface->init, s_tag, "iface->init must not be nullptr");

  if (s_initialized) {
    ra_log_error(s_tag, "display_init: PAL already initialised");
    return k_ra_err_busy;
  }

  void*          backend_ctx = nullptr;
  const ra_err_t err         = cfg->iface->init(cfg, &backend_ctx);
  if (err != k_ra_ok) {
    return err;
  }
  ...
  s_initialized  = true;
  *out_handle    = &s_handle;
  ra_log_info(s_tag, "display_init: backend bound");
  return k_ra_ok;
}
...
ra_err_t display_deinit(display_handle_t* d)
{
  ...
  const ra_err_t err = s_handle.iface->deinit(s_handle.ctx);
...
display_rect_t display_full_rect(const display_handle_t* d)
{
  ...
  if (internal_validate_handle(d) != k_ra_ok) {
    return empty;
  }
  display_caps_t caps = {};
  const ra_err_t e    = s_handle.iface->get_caps(s_handle.ctx, &caps);
  if (e != k_ra_ok) {
    return empty;
  }
