 ...
 * @brief E-ink (IT8951) backend for the display PAL, wired to ra_epaper.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 ...
 * ``libs/ra_hal/src/ra_epaper.c``. The app keeps painting the same
 ...
 * simulator-backed ``ra_epaper`` (see tests/test_ra_display_pal.c).
 *
...
/**
 * @enum ra_display_pal_eink_rgb_t
...
uint8_t ra_display_pal_eink_luma_from_rgb565(uint16_t px)
{
 ...
 *
 * @return The IT8951 waveform to pass to ``ra_epaper_display_area``.
 * @retval k_ra_epaper_wf_a2   For ``k_display_refresh_fast``.
 * @retval k_ra_epaper_wf_gc16 For ``k_display_refresh_quality`` / default.
 *
 * @pre  None.
 * @pre  None.
 ...
 * @since 0.1.0
 */
static ra_epaper_waveform_t internal_eink_waveform(display_refresh_hint_t hint)
{
  if (hint == k_display_refresh_fast) {
    return k_ra_epaper_wf_a2;
  }
  if (hint == k_display_refresh_init) {
    return k_ra_epaper_wf_init;
  }
  return k_ra_epaper_wf_gc16;
}
 ...
 * @retval k_ra_err_invalid_arg   Dimensions / buffer / descriptor invalid.
 ...
 * @retval k_ra_err_invalid_arg Rectangle steps out of bounds.
 *
 ...
 * @since 0.1.0
 */
static ra_err_t internal_eink_check_rect(const display_caps_t* caps, display_rect_t r)
{
 ...
 * luma in the bounded ``s_eink_line`` buffer, then ``ra_epaper_load_image``
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok            All rows loaded into frame RAM.
 * @retval (from ra_epaper)   Forwarded from ``ra_epaper_load_image``.
 *
 * @pre  ``ra_epaper_init`` succeeded; ``rect`` lies in the framebuffer.
 ...
 * @since 0.1.0
 */
static ra_err_t internal_eink_load_rect(const eink_ctx_t* c, display_rect_t rect)
{
  ...
  for (uint16_t row = 0U; row < rect.h; ++row) {
    ...
    for (uint16_t col = 0U; col < rect.w; ++col) {
      s_eink_line[col] = ra_display_pal_eink_luma_from_rgb565(src[col]);
    }
    const ra_epaper_area_t area = {.x      = rect.x,
                                   .y      = (uint16_t)(rect.y + row),
                                   .width  = rect.w,
                                   .height = 1U};
    const ra_err_t         err =
      ra_epaper_load_image(&area, s_eink_line, (size_t)rect.w, k_ra_epaper_endian_little);
    if (err != k_ra_ok) {
      return err;
    }
  }
 ...
 * then brings up the panel via ``ra_epaper_init`` using the BSP-supplied
 * ``ra_epaper_cfg_t`` carried in ``cfg->panel_timing``, and snapshots
 ...
 * @retval k_ra_err_null_ptr      cfg / out_ctx / framebuffer NULL.
 * @retval k_ra_err_invalid_arg   Dimensions / descriptor invalid.
 * @retval k_ra_err_not_supported Pixel format not RGB565.
...
static ra_err_t eink_init(const display_cfg_t* cfg, void** out_ctx)
{
  ...
  if (s_eink_ctx.initialised) {
    ra_log_error(s_tag, "eink_init: already initialised");
    return k_ra_err_busy;
  }
  const ra_err_t err = ra_epaper_init((const ra_epaper_cfg_t*)cfg->panel_timing);
  if (err != k_ra_ok) {
    ra_log_error(s_tag, "eink_init: ra_epaper_init failed");
    return err;
  }
  internal_eink_snapshot(cfg);
  *out_ctx = &s_eink_ctx;
  ra_log_info(s_tag, "eink_init: IT8951 backend bound");
  return k_ra_ok;
}
 ...
 * ``ra_epaper_display_area`` with the waveform mapped from ``hint``.
 *
 * @param[in] ctx  Backend context.
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok              Region pushed and refreshed.
 * @retval k_ra_err_null_ptr    ctx was NULL.
 * @retval k_ra_err_invalid_arg rect leaves the framebuffer.
 * @retval (from ra_epaper)     Forwarded load / display failure.
 *
 * @pre  ``eink_init`` has succeeded.
...
static ra_err_t eink_flush(void* ctx, display_rect_t rect, display_refresh_hint_t hint)
{
  ...
  const ra_err_t    v = internal_eink_check_rect(&c->caps, rect);
  if (v != k_ra_ok) {
    return v;
  }
  const ra_err_t lerr = internal_eink_load_rect(c, rect);
  if (lerr != k_ra_ok) {
    return lerr;
  }
  const ra_epaper_area_t area = {.x = rect.x, .y = rect.y, .width = rect.w, .height = rect.h};
  return ra_epaper_display_area(&area, internal_eink_waveform(hint));
}
 ...
 *
 * @details
 * Issues ``ra_epaper_sleep`` (which returns the driver to its
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           Panel asleep; state dropped.
 * @retval k_ra_err_null_ptr ctx was NULL.
 * @retval (from ra_epaper)  Forwarded sleep failure.
 *
 * @pre  ``eink_init`` has succeeded.
...
static ra_err_t eink_deinit(void* ctx)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx");
  eink_ctx_t*    c   = (eink_ctx_t*)ctx;
  const ra_err_t err = ra_epaper_sleep();
  c->initialised     = false;
  c->fb.pixels       = nullptr;
  return err;
}
