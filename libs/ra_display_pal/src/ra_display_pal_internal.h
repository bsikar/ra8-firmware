/**
 * @file ra_display_pal_internal.h
 ...
 *     ``ra_display_pal.h`` forward-declares as opaque. Holds the
 ...
 * ``ra_display_pal_<name>.h`` header. The dispatcher in
 * ``ra_display_pal.c`` is the only consumer at run time.
 *
...
struct display_backend_iface {
   ...
   */
  ra_err_t (*init)(const display_cfg_t* cfg, void** out_ctx);

  /** @brief Fill in ``*out`` with the backend's capabilities. */
  ra_err_t (*get_caps)(const void* ctx, display_caps_t* out);

  /** @brief Hand out the framebuffer descriptor for paint loops. */
  ra_err_t (*get_framebuffer)(void* ctx, display_fb_t* out);

  /** @brief Commit pending FB changes to the panel. */
  ra_err_t (*flush)(void* ctx, display_rect_t rect, display_refresh_hint_t hint);

  /** @brief Clear the framebuffer to ``color``. */
  ra_err_t (*clear)(void* ctx, uint32_t color);
