 ...
 * controller is on the other end. Today there are two backends:
 *
 *   - ``k_display_backend_lcd_ra_glcdc`` -- thin wrapper over
 *     ``libs/ra_hal/src/ra_glcdc.c`` driving the EK-RA8D2 1024x600
 *     parallel-RGB TFT.
 ...
 *     | ra_display_pal (this file)  |<- dispatches via display_backend_iface_t
 *     +--------------+--------------+
 *                    |
 *           +--------+---------+
 *           |                  |
 ...
 * rejects them with ``k_ra_err_not_supported``.
 *
 * @since 0.1.0
 */
 ...
 * (e.g. ``k_display_backend_lcd_ra_glcdc``) and pass its address
 ...
 * lives in ``src/ra_display_pal_internal.h`` and is invisible to
 * applications.
 *
 * @since 0.1.0
 */
 ...
 * @retval k_ra_ok                Backend ready; ``out_handle`` populated.
 * @retval k_ra_err_null_ptr      ``cfg`` / ``cfg->iface`` /
 *                                ``cfg->framebuffer`` / ``out_handle`` NULL.
 * @retval k_ra_err_invalid_arg   Width / height / pixfmt rejected by backend.
 * @retval k_ra_err_busy          PAL already initialised; deinit first.
 * @retval k_ra_err_not_supported Pixel format unsupported by chosen backend.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t display_init(const display_cfg_t* cfg, display_handle_t** out_handle);
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           Capabilities written to ``*out``.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t display_get_caps(const display_handle_t* d, display_caps_t* out);
 ...
 * @retval k_ra_err_null_ptr      Either argument was NULL.
 * @retval k_ra_err_not_supported Backend does not expose a
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t display_get_framebuffer(display_handle_t* d, display_fb_t* out);
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                Flush completed (or queued for the
 *                                next scan on continuous-refresh panels).
 * @retval k_ra_err_null_ptr      ``d`` was NULL.
 * @retval k_ra_err_invalid_arg   ``rect`` extends outside the framebuffer.
 * @retval k_ra_err_not_supported Backend does not implement flush yet
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t display_clear(display_handle_t* d, uint32_t color);
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           Backend released; ``d`` is now stale.
 * @retval k_ra_err_null_ptr ``d`` was NULL.
 *
 * @pre ``display_init`` has succeeded.
 * @pre No paint loop is in flight.
 * @post Subsequent calls through ``d`` return ``k_ra_err_invalid_arg``.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t display_deinit(display_handle_t* d);
