 ...
 * IT8951-compatible e-ink panel through ``libs/ra_hal/src/ra_epaper.c``.
 * Apps select this backend by setting
 * ``display_cfg_t.iface = &k_display_backend_eink_it8951;`` and pass the
 * BSP's IT8951 descriptor (a ``const ra_epaper_cfg_t*``) through
 ...
 * simulator-backed ``ra_epaper`` driver.
 *
 ...
 *
 * @details
 * ``init`` brings up the IT8951 over SPI via ``ra_epaper_init`` (using
 * the ``ra_epaper_cfg_t`` supplied in ``display_cfg_t.panel_timing``);
