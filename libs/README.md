...
| [`ra_net_pal/`](ra_net_pal/) | 4 -- PAL | 1 c / 1 h | Ethernet Platform Abstraction Layer. Sits above `ra_hal/`'s ESWM / ETHA / RMAC drivers and below an external IP stack (NetX Duo today; FreeRTOS+TCP / TCPDirect possible). Ring-buffer-shaped frame I/O so the IP stack doesn't need to know about RA8D2 specifics. |
