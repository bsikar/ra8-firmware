/**
 * @file ra_widget_internal.h
 * @brief Module-private paint helpers shared by the concrete leaf-widget TUs.
 *
 * @details
 * The concrete leaf widgets (::ra_widget_label_t in `ra_widget_label.c` and
 * ::ra_widget_button_t in `ra_widget_button.c`) both place aligned text and
 * fill a (optionally bordered) box through the injected ::ra_widget_paint_t
 ...
 * `ra_widget_paint.c` and are declared here as `RA_PRIV` (module-private,
 * shared across TUs of this one library). They remain pure compute + callback
 * dispatch: they carry no `ra_gfx` dependency, so the library stays graphics
 ...
 * @since 0.1.0
 */
RA_PRIV void ra_widget_priv_text_pos(const ra_widget_paint_t* paint,
                                     const ra_ui_rect_t*      rect,
                                     const char*              text,
                                     int16_t                  pad,
                                     ra_widget_align_t        align,
                                     int32_t*                 out_x,
                                     int32_t*                 out_y);
 ...
 * @since 0.1.0
 */
RA_PRIV void ra_widget_priv_fill_box(const ra_widget_paint_t* paint,
                                     const ra_ui_rect_t*      rect,
                                     uint32_t                 fill,
                                     uint32_t                 border,
                                     int16_t                  border_w);
