/**
 * @file ra_widget.c
 * @brief Implementation of the ra_widget composable-UI layer (#145).
 *
...
static const char* s_tag = "ra_widget";
 ...
 * @since 0.1.0
 */
static bool internal_rect_empty(const ra_ui_rect_t* r)
{
  return (r->w <= 0) && (r->h <= 0);
}
 ...
 * @since 0.1.0
 */
static ra_ui_rect_t internal_rect_union(ra_ui_rect_t acc, ra_ui_rect_t r)
{
  ...
  const ra_ui_rect_t out = {.x = x0, .y = y0, .w = x1 - x0, .h = y1 - y0};
  return out;
}

/**
 * @brief Count the visible widgets in @p widgets[0..count).
 * @details Used to size the `ra_box` scratch (one leaf per visible widget plus
 ...
 * @since 0.1.0
 */
static uint16_t internal_visible_count(const ra_widget_t* widgets, uint16_t count)
{
  uint16_t vis = 0U;
...
/**
 * @brief Build a ra_box stack tree: one container + a leaf per visible widget.
 ...
 *          `fixed`/`flex`. The caller then runs `ra_box_layout` and copies the
 *          leaf rects back. Split out so ::ra_widget_layout_stack stays within
 ...
 * @param[in]  scratch  Caller ra_box scratch.
 ...
 * @return ra_err_t
 * @retval k_ra_ok              Tree built; @p out_root is the container.
 * @retval k_ra_err_invalid_arg Scratch too small or a box add failed.
 * @retval <ra_box_tree_init's> Forwarded from `ra_box_tree_init`.
 ...
 * @since 0.1.0
 */
static ra_err_t internal_build_stack_tree(const ra_widget_t* widgets,
                                          uint16_t           count,
                                          ...
                                          int16_t*           out_root)
{
  ...
  const ra_err_t ierr = ra_box_tree_init(out_tree, scratch, cap);
  if (ierr != k_ra_ok) {
    return ierr;
  }
  ra_box_t container = {};
  container.kind =
    (axis == k_ra_widget_axis_row) ? (uint8_t)k_ra_box_stack_h : (uint8_t)k_ra_box_stack_v;
  container.pad  = pad;
  container.gap  = gap;
  container.flex = 1U;
  container.tag  = (int16_t)k_ra_box_none;
  *out_root      = ra_box_add(out_tree, (int16_t)k_ra_box_none, &container);
  if (*out_root == (int16_t)k_ra_box_none) {
    return k_ra_err_invalid_arg;
  }
  for (uint16_t i = 0U; i < count; ++i) {
    if (!widgets[i].visible) {
    ...
    ra_box_t leaf = {};
    leaf.kind     = (uint8_t)k_ra_box_leaf;
    leaf.fixed    = widgets[i].fixed;
    leaf.flex     = widgets[i].flex;
    leaf.tag      = (int16_t)widgets[i].action_id;
    if (ra_box_add(out_tree, *out_root, &leaf) == (int16_t)k_ra_box_none) {
      return k_ra_err_invalid_arg;
    }
  }
  return k_ra_ok;
...
[[nodiscard]] ra_err_t ra_widget_layout_stack(ra_widget_t*        widgets,
                                              ...
                                              uint16_t            box_cap)
{
  RA_CHECK_NULL_PTR(widgets, s_tag, "widgets must not be nullptr");
  RA_CHECK_NULL_PTR(frame, s_tag, "frame must not be nullptr");
  RA_CHECK_NULL_PTR(box_scratch, s_tag, "box_scratch must not be nullptr");

  ra_box_tree_t  tree = {};
  int16_t        root = (int16_t)k_ra_box_none;
  const ra_err_t berr =
    internal_build_stack_tree(widgets, count, axis, gap, pad, box_scratch, box_cap, &tree, &root);
  if (berr != k_ra_ok) {
    return berr;
  }
  const ra_err_t lerr = ra_box_layout(&tree, root, frame);
  if (lerr != k_ra_ok) {
    return lerr;
  }
  ...
  *out_handled = false;
  ...
  for (uint16_t i = 0U; i < count; ++i) {
    ...
    if (ev->kind == k_ra_widget_ev_touch) {
      ...
      if (!ra_ui_rect_contains(&w->rect, ev->x, ev->y)) {
        continue;
      }
...
[[nodiscard]] ra_err_t ra_widget_invalidate(ra_widget_t* w, ra_widget_refresh_t refresh)
{
  ...
  if (refresh == k_ra_widget_refresh_none) {
    return k_ra_err_invalid_arg;
  }
  w->dirty = true;
                                        ...
                                        uint16_t*            out_count)
{
  ...
  ra_ui_rect_t acc   = {.x = 0, .y = 0, .w = 0, .h = 0};
  uint8_t      hint  = (uint8_t)k_ra_widget_refresh_none;
  uint16_t     dirty = 0U;
  for (uint16_t i = 0U; i < count; ++i) {
  ...
  *out_rect  = acc;
  *out_hint  = (ra_widget_refresh_t)hint;
  *out_count = dirty;
  return k_ra_ok;
}
...
[[nodiscard]] ra_err_t ra_widget_render_dirty(ra_widget_t* widgets, uint16_t count)
{
  ...
  for (uint16_t i = 0U; i < count; ++i) {
    ...
    w->dirty   = false;
    w->refresh = (uint8_t)k_ra_widget_refresh_none;
  }
  return k_ra_ok;
}
