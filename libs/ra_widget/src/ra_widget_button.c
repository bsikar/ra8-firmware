/**
 * @file ra_widget_button.c
 * @brief Push-button leaf widget for the ra_widget tree model (#145 Phase 2).
 *
 * @details
 ...
 * ::ra_widget_paint_t backend; `on_input` consumes a touch the dispatcher has
 ...
 * Like the rest of `ra_widget`, the file carries no `ra_gfx` dependency.
 *
...
static const char* s_tag = "ra_widget_button";
 ...
 * @since 0.1.0
 */
static uint32_t internal_button_face(const ra_widget_button_t* b)
{
  if (b->pressed) {
 ...
 * @details Reads the ::ra_widget_button_t from `w->ctx`, fills its (optionally
 *          bordered) face for the current pressed state via
 *          ::ra_widget_priv_fill_box, then -- when there is text and a
 *          `draw_text` backend -- places and draws the label over the face.
 * @param[in] w The button widget (its `ctx` is a ::ra_widget_button_t).
 * @return Nothing.
 ...
 * @since 0.1.0
 */
static void internal_button_render(ra_widget_t* w)
{
  ra_widget_button_t* b = (ra_widget_button_t*)w->ctx;
  if (b == nullptr) {
    return;
  }
  if (b->paint == nullptr) {
  ...
  ra_widget_priv_fill_box(b->paint, &w->rect, face, b->border, b->border_w);
  if (b->text == nullptr) {
    return;
  }
  ...
  int32_t tx = 0;
  int32_t ty = 0;
  ra_widget_priv_text_pos(b->paint, &w->rect, b->text, b->pad, b->align, &tx, &ty);
 ...
 * @since 0.1.0
 */
static bool internal_button_on_input(ra_widget_t* w, const ra_widget_event_t* ev)
{
  ra_widget_button_t* b = (ra_widget_button_t*)w->ctx;
  if (b == nullptr) {
    return false;
  ...
  b->pressed = !b->pressed;
  b->presses++;
  (void)ra_widget_invalidate(w, k_ra_widget_refresh_fast);
...
static const ra_widget_vtable_t s_button_vt = {
  .measure  = nullptr,
...
[[nodiscard]] ra_err_t ra_widget_button_init(ra_widget_t* w, ra_widget_button_t* button)
{
  ...
  RA_CHECK_NULL_PTR(button, s_tag, "button must not be nullptr");
  w->vt      = ra_widget_button_vtable();
  w->ctx     = button;
  w->visible = true;
  return k_ra_ok;
}
