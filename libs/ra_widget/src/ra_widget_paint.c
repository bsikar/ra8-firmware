/**
 * @file ra_widget_paint.c
 * @brief Shared geometry helpers for the concrete leaf widgets (#145 Phase 2).
 *
 * @details
 * Holds the two `RA_PRIV` helpers the text-label and push-button leaf widgets
 * both need: aligned text placement and a filled / bordered box. They compute
 * pixel geometry and dispatch through the injected ::ra_widget_paint_t backend,
 * so -- like the rest of `ra_widget` -- this file carries no `ra_gfx`
 * dependency. The full contracts live on the declarations in
 * `ra_widget_internal.h`; the definitions below are deliberately comment-free.
 *
                                     ...
                                     int32_t*                 out_y)
{
  ...
  *out_y = rect->y + (int32_t)pad;
  if (align == k_ra_widget_align_left) {
    return;
  }
  ...
  *out_y = rect->y + ((rect->h - th) / 2);
  if (align == k_ra_widget_align_right) {
