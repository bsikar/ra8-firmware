 ...
 * frames, and event emission -- lives in `ra8_xml.c`.
 *
 * [Ring 3 / LIB] {World: NS}
 *
...
bool priv_ra8_xml_span_valid(size_t source_len, ra8_xml_span_t span)
{
                         ...
                         size_t*        out_length)
{
  ...
  if (!priv_ra8_xml_span_valid(source_len, span)) {
    return k_ra8_err_validation_failed;
  }
...
bool ra8_xml_span_equal(const uint8_t* source,
                        ...
                        const char*    literal)
{
  if ((source == nullptr) || (literal == nullptr) || !priv_ra8_xml_span_valid(source_len, span)) {
    return false;
  }
...
bool ra8_xml_span_local_equal(const uint8_t* source,
                              ...
                              const char*    literal)
{
  if ((source == nullptr) || !priv_ra8_xml_span_valid(source_len, span)) {
    return false;
  }
