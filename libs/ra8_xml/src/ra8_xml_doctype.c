...
internal_literal(const uint8_t* source, size_t end, size_t* position, bool public_id)
{
  ...
  const ra8_err_t err = public_id ? k_ra8_ok : priv_ra8_xml_raw(source, start, *position);
  ++*position;
  return err;
}
...
{
  if (((*position + 6U) <= end) && (memcmp(&source[*position], "SYSTEM", 6U) == 0)) {
  ...
  if (((*position + 6U) <= end) && (memcmp(&source[*position], "PUBLIC", 6U) == 0)) {
...
ra8_err_t priv_ra8_xml_doctype(ra8_xml_reader_t* reader)
{
  ...
  if ((reader->root_count != 0U) || (reader->doctype_seen != 0U) ||
      ...
      (memcmp(&reader->source[position], prefix, sizeof(prefix) - 1U) != 0)) {
    return k_ra8_err_validation_failed;
  }
  ...
  size_t name_end = 0U;
  if (priv_ra8_xml_qname(reader->source, end, position, &name_end) != k_ra8_ok) {
    return k_ra8_err_validation_failed;
  }
  position             = name_end;
