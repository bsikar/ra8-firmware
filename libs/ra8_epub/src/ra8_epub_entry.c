 ...
 *
 * @details
 * The whole-entry accessors (`ra8_epub_load_chapter`, `ra8_epub_get_resource`,
 ...
 *
 *   - A **forward streaming cursor** (`ra8_epub_entry_open/read/close`) over
 ...
 *     of the entry's size.
 *   - A **positioned read** (`ra8_epub_entry_pread`) for *stored* (uncompressed)
 ...
 *
 * Both work for a resident book (`ra8_epub_open`) and a streamed book
...
static const char* const s_tag = "ra8_epub_entry";
 ...
 *          begins right after them. `ra8_epub_entry_pread()` reads the 30-byte
 *          header to learn those two lengths and thereby the data offset.
 */
typedef enum : uint8_t {
  k_ra8_epub_ldh_size          = 30U, /**< Fixed local-header length, bytes.       */
  k_ra8_epub_ldh_fname_len_ofs = 26U, /**< uint16 file-name length field offset.   */
  k_ra8_epub_ldh_extra_len_ofs = 28U, /**< uint16 extra-field length field offset. */
} ra8_epub_ldh_t;

/**
 * @enum ra8_epub_ldh_sig_t
 * @brief Local-file-header signature bytes (little-endian 0x04034b50).
 */
typedef enum : uint8_t {
  k_ra8_epub_ldh_sig_0 = 0x50U, /**< Signature byte 0 ('P'). */
  k_ra8_epub_ldh_sig_1 = 0x4BU, /**< Signature byte 1 ('K'). */
  k_ra8_epub_ldh_sig_2 = 0x03U, /**< Signature byte 2.       */
  k_ra8_epub_ldh_sig_3 = 0x04U, /**< Signature byte 3.       */
} ra8_epub_ldh_sig_t;
 ...
 */
typedef enum : uint8_t {
  k_ra8_epub_le_lo    = 0U, /**< Low-order byte index.  */
  k_ra8_epub_le_hi    = 1U, /**< High-order byte index. */
  k_ra8_epub_le_shift = 8U, /**< High-byte left shift.  */
} ra8_epub_le_t;
...
static bool priv_data_offset(const uint8_t* hdr, uint64_t local_header_ofs, uint64_t* out_data_ofs)
{
  const uint8_t sig[4] = {(uint8_t)k_ra8_epub_ldh_sig_0,
                          ...
                          (uint8_t)k_ra8_epub_ldh_sig_2,
                          (uint8_t)k_ra8_epub_ldh_sig_3};
  if (memcmp(hdr, sig, sizeof(sig)) != 0) {
    return false; /* GCOVR_EXCL_LINE -- a located, stat'd entry always has a valid local header */
  }
  const uint32_t fname = (uint32_t)hdr[(uint8_t)k_ra8_epub_ldh_fname_len_ofs + k_ra8_epub_le_lo] |
                         ((uint32_t)hdr[(uint8_t)k_ra8_epub_ldh_fname_len_ofs + k_ra8_epub_le_hi]
                          << k_ra8_epub_le_shift);
  const uint32_t extra = (uint32_t)hdr[(uint8_t)k_ra8_epub_ldh_extra_len_ofs + k_ra8_epub_le_lo] |
                         ((uint32_t)hdr[(uint8_t)k_ra8_epub_ldh_extra_len_ofs + k_ra8_epub_le_hi]
                          << k_ra8_epub_le_shift);
  *out_data_ofs =
    local_header_ofs + (uint64_t)k_ra8_epub_ldh_size + (uint64_t)fname + (uint64_t)extra;
  return true;
}

/**
 * @brief Locate + stat an entry and start a miniz extract-iterator over it.
 * @details See implementation. Split out so `ra8_epub_entry_open` stays under the
 ...
 * @details See implementation. Split out so `ra8_epub_entry_pread` stays under the
...
ra8_err_t ra8_epub_entry_open(ra8_epub_book_t*         book,
                              const char*              path,
...
ra8_err_t ra8_epub_entry_close(ra8_epub_entry_reader_t* reader)
{
  RA8_CHECK_NULL_PTR(reader, s_tag, "reader must not be nullptr");
  if (reader->iter == nullptr) {
...
/**
 * @brief Reject any NULL `ra8_epub_entry_pread` pointer argument.
 * @details See implementation. Split out so `ra8_epub_entry_pread` stays under the
 *          NASA-Rule-4 statement budget.
...
ra8_err_t ra8_epub_entry_pread(ra8_epub_book_t* book,
                               const char*      path,
                               uint64_t         offset,
                               uint8_t*         buf,
                               size_t           len,
