 ...
 * helpers are declared beside it, in `ra8_epub_xml_opf_internal.h` and
 * `ra8_epub_xml_toc_internal.h`, so neither unit sees a `static` prototype
 * it does not define.
 ...
 * @since 0.1.0
 */
RA8_PRIV void priv_ra8_epub_xml_copy(const uint8_t* source,
                                     size_t         source_len,
                                     ra8_xml_span_t span,
                                     char*          destination,
                                     size_t         capacity);
 ...
 * @since 0.1.0
 */
RA8_PRIV priv_attr_t priv_ra8_epub_xml_attr(const uint8_t*         source,
                                            size_t                 source_len,
 ...
 * @since 0.1.0
 */
RA8_PRIV bool priv_ra8_epub_xml_attr_contains(const uint8_t* source,
                                              size_t         source_len,
 ...
 * @since 0.1.0
 */
RA8_PRIV ra8_xml_span_t priv_ra8_epub_xml_frame_name(const ra8_xml_reader_t* reader,
                                                     uint16_t                frame);
 ...
 * @since 0.1.0
 */
RA8_PRIV bool priv_ra8_epub_xml_direct_child(uint16_t child, uint16_t parent);
 ...
 */
RA8_PRIV bool
priv_ra8_epub_xml_ancestor_frame(uint16_t depth, uint16_t levels, uint16_t* out_frame);
 ...
 * @since 0.1.0
 */
RA8_PRIV uint16_t priv_ra8_epub_xml_ancestor_marker(const ra8_xml_reader_t* reader,
                                                    uint16_t                depth,
                                                    uint16_t                levels);
 ...
 * @since 0.1.0
 */
RA8_PRIV ra8_err_t priv_ra8_epub_xml_reader(ra8_xml_reader_t*         reader,
 ...
 * @since 0.1.0
 */
RA8_PRIV ra8_err_t priv_ra8_epub_xml_find(const uint8_t*            source,
                                          size_t                    length,
                                          ra8_epub_xml_workspace_t* workspace,
                                          const char*               local,
                                          bool                      use_parent,
