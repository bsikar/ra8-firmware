 ...
 */
typedef struct {
  ra8_epub_entry_reader_t reader; /**< Open entry cursor. */
} ra8_epub_entry_pull_t;
 ...
 * @param[in]  ctx A ::ra8_epub_entry_pull_t.
...
static ra8_err_t priv_classify(ra8_epub_book_t* book, const char* href, bool* out_is_atlas)
{
  *out_is_atlas                                      = false;
  ...
  size_t          got                                = 0U;
  const ra8_err_t err = ra8_epub_entry_pread(book, href, 0U, magic, sizeof(magic), &got);
                                ...
                                ra8_tileatlas_info_t*              out_info)
{
  ...
  uint64_t              size = 0U;
  ra8_err_t             err  = ra8_epub_entry_open(book, href, &pull.reader, &size);
  if (err != k_ra8_ok) {
    return err;
  }
                                      ...
                                      const ra8_epub_atlas_import_cfg_t* cfg)
{
  ...
  return ra8_epub_tile_binder_add_ext(binder,
                                      cfg->store.pread,
                                      cfg->store.pread_ctx,
