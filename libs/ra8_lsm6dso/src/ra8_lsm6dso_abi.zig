...
//! driver never links against `ra8_i3c_i2c` or `ra8_spi`, so the host suite's
...
/// Component tag on this library's log lines, matching the C's `s_lsm6dso_tag`.
...
/// Single validation gate for the FIFO drain, in the C's guard order.
