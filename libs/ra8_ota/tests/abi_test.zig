...
test "the exported MC/DC predicates carry the C semantics across the membrane" {
    try std.testing.expect(abi.priv_ota_char_in_range('c', 'a', 'f') != 0);
    try std.testing.expect(abi.priv_ota_char_in_range('g', 'a', 'f') == 0);
    try std.testing.expect(abi.priv_ota_char_in_range('A', 'a', 'f') == 0);

    try std.testing.expect(abi.priv_ota_download_state_invalid(0, 1, 0) == 0);
    try std.testing.expect(abi.priv_ota_download_state_invalid(0, 1, 1) == 0);
    try std.testing.expect(abi.priv_ota_download_state_invalid(0, 1, 2) != 0);
}
