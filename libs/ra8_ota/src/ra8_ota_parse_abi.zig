...
pub export fn priv_ota_char_in_range(c: c_char, lo: c_char, hi: c_char) callconv(.c) u8 {
    return @intFromBool(impl.charInRange(c, lo, hi));
}
...
pub export fn priv_ota_download_state_invalid(
    ...
    return @intFromBool(impl.downloadStateInvalid(state_idle_val, state_downloading_val, state));
}
