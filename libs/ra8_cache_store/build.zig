...
//!
//! Build graph for the Zig implementation of the `ra8_cache_store` runtime
//! path. CMake consumes the installed static archive through the unchanged
//! `inc/ra8_cache_store.h` C ABI; the `test` step covers the pure logic and the
//! ABI membrane.
//!
//! No build options: the mount half still lives in C on this branch and the
//! runtime path has no compile-time switches.
