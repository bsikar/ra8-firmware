 ...
 * @brief Transcode producer: sniff, stripe-decode, tile, encode, emit (#231).
 *
 * @details
 * Implements `ra8_tileatlas_produce()` and `ra8_tileatlas_work_bytes()`.
 * The producer owns the band accumulator (one tile row of decoded pixels),
 * the tile cut + intra-encode + sink stage, and the trailing index/footer
 * emission; the pixel rows arrive from `ra8_jpeg_sw_decode_stripes()`
 * (JPEG) or `ra8_ta_priv_png_rows()` (PNG), both bounded-RAM by
 * construction. Every internal buffer is carved from the caller's work
 * arena through the bump allocator -- the producer allocates nothing.
 *
...
typedef enum : uint32_t {
  k_ra8_ta_sniff_bytes     = 8U,     /**< Magic bytes pulled up front.    */
...
static const uint8_t s_prod_png_sig[k_ra8_ta_sniff_bytes] =
...
/**
 * @brief Dispatch the sniffed source to the matching stripe decoder.
 * @details The compound sniff decision carries MC/DC vectors in
 *          `test_ra8_tileatlas_produce.c` (JPEG head, PNG head, 0xFF+junk).
 * @param[in,out] st   Producer state.
 ...
 * @retval k_ra8_err_not_supported The head is neither JPEG nor PNG.
