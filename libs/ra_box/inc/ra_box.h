 ...
 *
 * @details
 * `ra_box` is the minimal CSS-style box model the e-reader chrome needs
 * (issue #80, #76). The full document engine (`ra_reflow`) reflows
 ...
 * the RA8D2, and the renderer (`ra_gfx`) and hit-tester (`ra_ui`) are
 * layered on top.
 *
...
typedef enum : uint8_t {
  k_ra_box_stack_v = 0U, /**< Vertical stack: children stack top->bottom. */
  k_ra_box_stack_h = 1U, /**< Horizontal stack: children left->right.     */
  k_ra_box_grid    = 2U, /**< Fixed-column grid, row-major.               */
  k_ra_box_leaf    = 3U, /**< Terminal box (no children laid out).        */
} ra_box_kind_t;
 ...
 * are maintained by ::ra_box_add. Sizing along the parent's main axis is
 * `fixed` when `fixed > 0`, otherwise `flex` weight (0 flex = collapse).
 * `fill` / `border` are opaque 0xRRGGBB values the renderer reads;
 * `k_ra_box_no_colour` (0, i.e. black) is reserved as "none" -- chrome
 * uses the grayscale ramp where pure black is `ink`, so leaves that need
 * a true black fill should use `k_ra_box_ink` instead.
 */
typedef struct {
  uint8_t      kind;        /**< ra_box_kind_t.                            */
  uint8_t      grid_cols;   /**< Columns for k_ra_box_grid (>= 1).         */
  ...
  uint32_t     fill;        /**< Background colour, or k_ra_box_no_colour. */
 ...
 * lower index than its children -- which is what lets ::ra_box_layout run
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok             Initialised (empty tree).
 * @retval k_ra_err_null_ptr   `tree` or `storage` is NULL.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_box_tree_init(ra_box_tree_t* tree, ra_box_t* storage, uint16_t cap);
 ...
 * k_ra_box_none`). Returns the new node's index for use as a parent in
 * later calls.
 *
 * @param[in,out] tree   Initialised tree.
 * @param[in]     parent Parent index, or `k_ra_box_none` for a root.
 * @param[in]     node   Node template (kind / sizing / colours).
 *
 * @return New node index in `[0, cap)`, or `k_ra_box_none` on error.
 *
 * @pre `tree` and `node` non-NULL; `tree` initialised.
 * @pre `parent` is `k_ra_box_none` or a valid earlier index.
 ...
 * @since 0.1.0
 */
[[nodiscard]] int16_t ra_box_add(ra_box_tree_t* tree, int16_t parent, const ra_box_t* node);
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok               Laid out; every reachable `rect` set.
 * @retval k_ra_err_null_ptr     `tree` or `frame` is NULL.
 * @retval k_ra_err_invalid_arg  `root` out of range, or empty tree.
 *
 * @pre `tree` and `frame` non-NULL; `0 <= root < tree->count`.
 * @pre Children indices exceed their parent's (ensured by ra_box_add).
 * @post Every node reachable from `root` has its `rect` computed.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_box_layout(ra_box_tree_t* tree, int16_t root, const ra_ui_rect_t* frame);
