 ...
 *
 * @details
 * Single forward-pass layout (no recursion, P10 Rule 1). See ra_box.h
...
static const char* const s_tag = "ra_box";
 ...
 * @since 0.1.0
 */
static ra_ui_rect_t internal_inset(ra_ui_rect_t outer, int32_t pad)
{
  ra_ui_rect_t r;
  r.x = outer.x + pad;
  r.y = outer.y + pad;
 ...
 * defensive guard
 * against a malformed cyclic link; ra_box_add never builds a cycle, so it
 * only ever evaluates true on a well-formed tree.
 *
 * @param[in] link  Next node index, or k_ra_box_none.
 ...
 * @since 0.1.0
 */
static void internal_tally(const ra_box_tree_t* tree,
                           int16_t              parent,
                           uint16_t*            out_count,
                           int32_t*             out_fixed,
                           uint32_t*            out_flex)
{
  ...
  for (uint16_t guard = 0U; internal_iter_live(c, guard, tree->count); ++guard) {
    const ra_box_t* child = &tree->nodes[c];
    if (child->fixed > 0) {
 ...
 * @since 0.1.0
 */
static void internal_layout_stack(ra_box_tree_t* tree, int16_t parent, bool horizontal)
{
  const ra_box_t*    p    = &tree->nodes[parent];
  const ra_ui_rect_t cont = internal_inset(p->rect, (int32_t)p->pad);
 ...
 * @since 0.1.0
 */
static void internal_layout_grid(ra_box_tree_t* tree, int16_t parent)
{
  const ra_box_t*    p    = &tree->nodes[parent];
  const ra_ui_rect_t cont = internal_inset(p->rect, (int32_t)p->pad);
  const int32_t      gap  = (int32_t)p->gap;
...
ra_err_t ra_box_tree_init(ra_box_tree_t* tree, ra_box_t* storage, uint16_t cap)
{
  RA_CHECK_NULL_PTR(tree, s_tag, "tree must not be nullptr");
...
int16_t ra_box_add(ra_box_tree_t* tree, int16_t parent, const ra_box_t* node)
{
  if ((tree == nullptr) || (node == nullptr)) {
    return (int16_t)k_ra_box_none;
  }
  if (tree->count >= tree->cap) {
    return (int16_t)k_ra_box_none;
  }
  if ((parent != (int16_t)k_ra_box_none) && ((parent < 0) || (parent >= (int16_t)tree->count))) {
    return (int16_t)k_ra_box_none;
  }

  const int16_t idx            = (int16_t)tree->count;
  tree->nodes[idx]             = *node;
  tree->nodes[idx].first_child = (int32_t)k_ra_box_none;
  tree->nodes[idx].next        = (int32_t)k_ra_box_none;
  tree->count++;

  if (parent != (int16_t)k_ra_box_none) {
    ra_box_t* par = &tree->nodes[parent];
    if (par->first_child == (int32_t)k_ra_box_none) {
...
ra_err_t ra_box_layout(ra_box_tree_t* tree, int16_t root, const ra_ui_rect_t* frame)
{
  RA_CHECK_NULL_PTR(tree, s_tag, "tree must not be nullptr");
  ...
  for (uint16_t i = 0U; i < tree->count; ++i) {
    switch ((ra_box_kind_t)tree->nodes[i].kind) {
      case k_ra_box_stack_v:
        internal_layout_stack(tree, (int16_t)i, false);
        break;
      case k_ra_box_stack_h:
