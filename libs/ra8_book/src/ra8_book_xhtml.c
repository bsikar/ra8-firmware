 ...
 *
 * @details
 * `ra8_reflow_layout_chapter()` consumes XHTML. This bridge walks a chapter's
 * pre-parsed `ra8_book` DOM and re-emits well-formed XHTML so a compiled book
...
static const char* const s_tag_xhtml = "ra8_book_xhtml";

/**
 * @struct ra8_book_walk_entry_t
 ...
 *                      @c ra8_book_string and @c ra8_book_attrs.
 ...
 *
 * @pre  @p base points to a valid, fully initialised compiled book blob.
 * @pre  @p node is a non-null pointer to an element node; @p *sp < @c k_ra8_book_xhtml_stack.
                                  ...
                                  uint32_t*              sp)
{
  ...
  stack[(*sp)++] = (ra8_book_walk_entry_t){false, node->first_child}; /* children first       */
  return true;
}
 ...
 * without recursion.  A @c ra8_book_walk_entry_t stack (maximum depth
 * @c k_ra8_book_xhtml_stack) holds deferred work: each entry is either a
                                   ...
                                   size_t*     pos)
{
  ...
  while (sp > 0U && ok && guard < max_iter) {
    ...
    const ra8_book_node_t* node = &nodes[e.value];
