 ...
 * extractor) and ra8_book_paged.c (the #163 paged text extractor). The plain-text
 ...
 * without duplicating the logic. Also the seam between ra8_book.c (resident
 * container open) and ra8_book_chunked.c (demand-paged chunk reader): both parse
...
/**
 * @enum ra8_book_container_field_off_t
 * @brief Byte offsets of the fixed "RBKC" container-header fields.
 * @details Mirrors the layout documented on @ref ra8_book_container_t; shared by
 *          the resident open (ra8_book.c) and the demand-paged chunk reader
 *          (ra8_book_chunked.c) so both parse one definition of the format.
 * @since Version 0.1.0
 */
typedef enum : uint8_t {
  k_ra8_book_cont_off_chunk_bytes = 4U,  /**< uint32 LE: inflated bytes per chunk. */
  k_ra8_book_cont_off_total       = 8U,  /**< uint64 LE: flat-blob inflated total. */
  k_ra8_book_cont_off_count       = 16U, /**< uint32 LE: number of chunks.         */
  k_ra8_book_cont_off_reserved    = 20U, /**< uint32 LE: reserved, must be 0.      */
} ra8_book_container_field_off_t;
 ...
 *
 * @param[in]  hdr             First @ref k_ra8_book_container_header_len bytes of
 *                             the container file.
 ...
 *
 * @pre @p hdr holds at least @ref k_ra8_book_container_header_len readable bytes.
 ...
 *          `table + idx * k_ra8_book_container_entry_len`.
 *
 ...
 *
 * @pre @p table holds at least `(idx + 1) * k_ra8_book_container_entry_len` bytes.
...
typedef enum : uint32_t {
  k_ra8_book_xhtml_stack  = 512U, /**< Max pending open/close walk entries.        */
  k_ra8_book_xhtml_iter_x = 4U,   /**< Iteration-guard multiplier over node_count. */
} ra8_book_xhtml_bound_t;
