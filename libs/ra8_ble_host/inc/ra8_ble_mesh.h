...
/**
 * @enum ra8_ble_mesh_limits_t
 * @brief Static-allocation upper bounds.
 */
typedef enum : uint8_t {
  k_ra8_ble_mesh_max_elements      = 4U,  /**< Elements per node.  */
  k_ra8_ble_mesh_max_models_per_el = 8U,  /**< Models per element. */
  k_ra8_ble_mesh_uuid_bytes        = 16U, /**< Device UUID length. */
} ra8_ble_mesh_limits_t;

/**
 * @enum ra8_ble_mesh_event_kind_t
 * @brief Lifecycle event delivered to the user callback.
 */
typedef enum : uint8_t {
  k_ra8_ble_mesh_evt_provisioned   = 0U, /**< Node has been provisioned.      */
  k_ra8_ble_mesh_evt_unprovisioned = 1U, /**< Node has been factory-reset.    */
  k_ra8_ble_mesh_evt_key_refresh   = 2U, /**< Mesh key refresh phase changed. */
  k_ra8_ble_mesh_evt_iv_update     = 3U, /**< IV update procedure tick.       */
} ra8_ble_mesh_event_kind_t;
...
/**
 * @struct ra8_ble_mesh_model_id_t
...
} ra8_ble_mesh_model_id_t;

/**
 * @struct ra8_ble_mesh_element_t
...
typedef struct {
  ...
  ra8_ble_mesh_model_id_t models[k_ra8_ble_mesh_max_models_per_el]; /**< Models. */
} ra8_ble_mesh_element_t;

/**
 * @struct ra8_ble_mesh_config_t
 * @brief Configuration passed to ``ra8_ble_mesh_init``.
 */
typedef struct {
  uint16_t               cid;                             /**< Company ID for composition data. */
  uint16_t               pid;                             /**< Product ID.                      */
  uint16_t               vid;                             /**< Version ID.                      */
  uint8_t                uuid[k_ra8_ble_mesh_uuid_bytes]; /**< Device UUID.                     */
  uint8_t                element_count;                   /**< Number of valid elements.        */
  ra8_ble_mesh_element_t elements[k_ra8_ble_mesh_max_elements]; /**< Elements. */
} ra8_ble_mesh_config_t;

/**
 * @struct ra8_ble_mesh_event_t
 * @brief Event payload delivered to the user callback.
 */
typedef struct {
  ra8_ble_mesh_event_kind_t kind;    /**< Event kind.                 */
  uint16_t                  net_idx; /**< Active subnet (NetKey idx). */
  uint16_t                  addr;    /**< Primary unicast address.    */
} ra8_ble_mesh_event_t;

/**
 * @typedef ra8_ble_mesh_event_fn_t
 * @brief Mesh lifecycle event callback.
 */
typedef void (*ra8_ble_mesh_event_fn_t)(void* ctx, const ra8_ble_mesh_event_t* evt);
 ...
 * @retval k_ra8_ok                   Mesh stack up.
 * @retval k_ra8_err_null_ptr         ``cfg == NULL``.
 * @retval k_ra8_err_invalid_arg      Counts out of range.
 * @retval k_ra8_err_not_initialized  ra8_ble_host_init was not called.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ble_mesh_init(const ra8_ble_mesh_config_t* cfg);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ble_mesh_close(void);
 ...
 *
 * @return ra8_err_t Error code.
 * @retval k_ra8_ok                   Beacon started.
 * @retval k_ra8_err_not_initialized  Stack not initialized.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ble_mesh_prov_enable(void);

/**
 * @brief Stop the unprovisioned beacon broadcast.
 *
 * @return ra8_err_t Error code.
 * @retval k_ra8_ok                   Beacon stopped.
 * @retval k_ra8_err_not_initialized  Stack not initialized.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ble_mesh_prov_disable(void);

/**
 * @brief Reset the node back to unprovisioned, clearing all keys.
 *
 * @return ra8_err_t Error code.
 * @retval k_ra8_ok                   Reset complete.
 * @retval k_ra8_err_not_initialized  Stack not initialized.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ble_mesh_factory_reset(void);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ble_mesh_attach_event_handler(ra8_ble_mesh_event_fn_t fn, void* ctx);
