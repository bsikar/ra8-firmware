 ...
 *    to apps through ``ra8_ble_host_gatt_*`` calls.
 ...
 *  - HCI / Controller (Vol 4 Part E) -- ra8_ble.
 *
...
typedef enum : uint8_t {
  k_ra8_ble_host_max_services = 8U,  /**< Maximum registered Primary Services. */
  k_ra8_ble_host_max_chars    = 32U, /**< Maximum registered Characteristics.  */
  k_ra8_ble_host_uuid_bytes =
    16U, /**< 128-bit UUID byte length (Bluetooth Core Vol 3 Part B 2.5.1). */
} ra8_ble_host_limits_t;

/**
 * @enum ra8_ble_host_role_t
 * @brief GAP role selection for ``ra8_ble_host_init``.
 *
 * @details
 ...
 */
typedef enum : uint8_t {
  k_ra8_ble_host_role_peripheral  = 0U, /**< Connectable, advertises, accepts connections. */
  k_ra8_ble_host_role_central     = 1U, /**< Scans, initiates connections.                 */
  k_ra8_ble_host_role_observer    = 2U, /**< Scans only, never connects.                   */
  k_ra8_ble_host_role_broadcaster = 3U, /**< Non-connectable advertiser only.              */
} ra8_ble_host_role_t;

/**
 * @enum ra8_ble_host_char_props_t
 * @brief Characteristic Properties bitmask
 *        (Bluetooth Core 5.3 Vol 3 Part G 3.3.1.1 Table 3.5).
 *
 * @details
 * Used as the ``props`` argument to
 * ``ra8_ble_host_gatt_register_char``. The bits combine with bitwise
 * OR. Only the four properties this starter stack actually
 * implements are exposed.
 */
typedef enum : uint8_t {
  k_ra8_ble_host_char_prop_read     = 0x02U, /**< Read.                      */
  k_ra8_ble_host_char_prop_write_nr = 0x04U, /**< Write Without Response.    */
  k_ra8_ble_host_char_prop_write    = 0x08U, /**< Write Request.             */
  k_ra8_ble_host_char_prop_notify   = 0x10U, /**< Handle Value Notification. */
  k_ra8_ble_host_char_prop_indicate = 0x20U, /**< Handle Value Indication.   */
} ra8_ble_host_char_props_t;

/**
 * @enum ra8_ble_host_event_kind_t
 * @brief Event kind delivered to the user-supplied event handler.
 */
typedef enum : uint8_t {
  k_ra8_ble_host_event_connected    = 0U, /**< Link established (LE_Connection_Complete). */
  k_ra8_ble_host_event_disconnected = 1U, /**< Link torn down (Disconnection_Complete).   */
  k_ra8_ble_host_event_write        = 2U, /**< Peer wrote to a Characteristic.            */
  k_ra8_ble_host_event_subscribe    = 3U, /**< Peer wrote to a CCCD (notify/indicate on). */
} ra8_ble_host_event_kind_t;
...
/**
 * @struct ra8_ble_host_config_t
 * @brief Configuration passed to ``ra8_ble_host_init``.
 *
 * @details
 ...
 */
typedef struct {
  ra8_ble_host_role_t role;       /**< GAP role.                                            */
...
} ra8_ble_host_config_t;

/**
 * @struct ra8_ble_host_event_t
 * @brief Event payload delivered by the host to the user handler.
 */
typedef struct {
  ra8_ble_host_event_kind_t kind;        /**< What kind of event is this.                       */
...
} ra8_ble_host_event_t;

/**
 * @typedef ra8_ble_host_event_fn_t
 ...
 */
typedef void (*ra8_ble_host_event_fn_t)(void* ctx, const ra8_ble_host_event_t* evt);
 ...
 *
 * @details
 * Calls ``ra8_ble_open`` under the hood, attaches the host-private
 * HCI event and ACL data handlers, and zeroes the GATT attribute
 * table. After this returns ``k_ra8_ok`` the application may
 * register services / characteristics and start advertising.
 *
 * Algorithm:
 *  1. Validate ``cfg``.
 *  2. ``ra8_ble_open`` with controller defaults (external osc, deep sleep).
 *  3. Register internal HCI event + ACL handlers via ``ra8_ble_attach_*``.
 ...
 *                ``ra8_ble_host_role_t`` enumerators.
 *
 * @return ra8_err_t Error code.
 * @retval k_ra8_ok                  Stack is up.
 * @retval k_ra8_err_null_ptr        ``cfg == NULL``.
 * @retval k_ra8_err_invalid_arg     ``cfg->role`` out of range, or stack
 *                                  was already initialized.
 * @retval k_ra8_err_invalid_state   Underlying ``ra8_ble_open`` refused.
 *
 ...
 *
 * @see ra8_ble_host_close()
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ble_host_init(const ra8_ble_host_config_t* cfg);
 ...
 * @retval k_ra8_err_not_initialized Stack was not initialized.
 *
 * @pre Stack is initialized.
 * @post Underlying ``ra8_ble_close`` was invoked.
 * @post Subsequent host calls return ``k_ra8_err_not_initialized``.
 *
 * @note Not thread-safe.
 *
 * @see ra8_ble_host_init()
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ble_host_close(void);
 ...
 * @param[in] adv_data_len   Bytes (0..k_ra8_ble_adv_data_max).
 * @param[in] scan_resp      Scan-response payload. NULL iff ``scan_resp_len == 0``.
 * @param[in] scan_resp_len  Bytes (0..k_ra8_ble_adv_data_max).
 * @param[in] interval_ms    Advertising interval in milliseconds (20..10240).
 *
 * @return ra8_err_t Error code.
 * @retval k_ra8_ok                  Advertising started.
 * @retval k_ra8_err_not_initialized Stack not initialized.
 * @retval k_ra8_err_invalid_arg     Length / interval out of range, or role
 *                                  is observer/central.
 * @retval k_ra8_err_null_ptr        Length>0 but pointer NULL.
 *
 * @pre Stack initialized via ra8_ble_host_init().
 * @pre Role is peripheral or broadcaster.
 * @post Controller is advertising on the standard primary channels.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ble_host_advertise_start(const uint8_t* adv_data,
 ...
 *
 * @return ra8_err_t Error code.
 * @retval k_ra8_ok                   Advertising stopped.
 * @retval k_ra8_err_not_initialized  Stack not initialized.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ble_host_advertise_stop(void);
 ...
 * @retval k_ra8_err_null_ptr        Either argument NULL.
 * @retval k_ra8_err_not_initialized Stack not initialized.
 * @retval k_ra8_err_no_mem          Service table full.
 *
 * @pre Stack initialized via ra8_ble_host_init().
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ble_host_gatt_register_service(const uint8_t* uuid_128,
                                                           uint16_t*      out_handle);
 ...
 * which is what apps pass to ``ra8_ble_host_gatt_set_value`` and
 * ``ra8_ble_host_gatt_notify``.
 *
 * @param[in]  svc_handle  ATT handle of the parent service (as returned
 *                         from ``ra8_ble_host_gatt_register_service``).
 ...
 * @retval k_ra8_err_null_ptr        ``uuid_128`` or ``out_handle`` NULL,
 *                                  or value_buf NULL with value_max>0.
 * @retval k_ra8_err_invalid_arg     ``svc_handle`` is not a registered
 *                                  service handle, ``props`` is zero,
 *                                  or ``value_max`` exceeds 512.
 * @retval k_ra8_err_no_mem          Characteristic table full.
 * @retval k_ra8_err_not_initialized Stack not initialized.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ble_host_gatt_register_char(uint16_t       svc_handle,
 ...
 * call ``ra8_ble_host_gatt_notify`` afterwards if subscribers
 * should be informed.
 *
 ...
 * @retval k_ra8_err_null_ptr        value NULL with len>0.
 * @retval k_ra8_err_not_found       Handle does not match any registered
 *                                  characteristic.
 * @retval k_ra8_err_invalid_arg     ``len`` exceeds the buffer capacity.
 * @retval k_ra8_err_not_initialized Stack not initialized.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t
ra8_ble_host_gatt_set_value(uint16_t char_handle, const uint8_t* value, uint16_t len);
 ...
 * B-frame on CID 0x0004, and pushes it down ``ra8_ble_hci_send_acl_data``.
 *
 * If no peer is currently connected, or the peer has not enabled
 * notifications via the CCCD, the call returns ``k_ra8_ok`` and no
 ...
 *
 * @return ra8_err_t Error code.
 * @retval k_ra8_ok                  Notification sent (or skipped silently).
 * @retval k_ra8_err_not_found       Handle does not match.
 * @retval k_ra8_err_not_initialized Stack not initialized.
 * @retval k_ra8_err_invalid_arg     Characteristic does not advertise
 *                                  the notify property.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ble_host_gatt_notify(uint16_t char_handle);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ble_host_attach_event_handler(ra8_ble_host_event_fn_t fn, void* ctx);
 ...
 * @post A k_ra8_ble_host_event_connected event has been dispatched.
 *
 * @note Not thread-safe; intended for the unit-test harness.
 *
 * @since 0.1.0
