 ...
 * simpler, ra8_err_t-returning API and so the SMP cryptographic
 ...
 *   - **Host unit-test build** (``RA8_SIMULATOR_MODE`` defined by
...
/**
 * @struct ra8_ble_security_state_t
 * @brief Internal singleton state.
 */
typedef struct {
  uint8_t                     initialized; /**< 1 once init succeeded. */
  ra8_ble_security_config_t   config;      /**< Captured config.       */
  ra8_ble_security_event_fn_t event_fn;    /**< User callback or NULL. */
  void*                       event_ctx;   /**< User context.          */
  uint8_t                     bond_count;  /**< Active bonds in store. */
} ra8_ble_security_state_t;

/**
 * @var s_state
 * @brief File-scope security state (not thread-safe; init-time API).
 */
static ra8_ble_security_state_t s_state;
 ...
 *          ra8_ble_security_io_cap_t (mirroring Bluetooth Core 5.3
 ...
 * @retval k_ra8_err_invalid_arg  Out-of-range field.
 *
 * @pre Called from ra8_ble_security_init.
...
static ra8_err_t internal_validate_cfg(const ra8_ble_security_config_t* cfg)
{
  ...
  if ((uint8_t)cfg->io_cap > (uint8_t)k_ra8_ble_io_cap_keyboard_display) {
 ...
 * @retval BLE_HS_IO_DISPLAY_ONLY     when io_cap == k_ra8_ble_io_cap_display_only.
 * @retval BLE_HS_IO_DISPLAY_YESNO    when io_cap == k_ra8_ble_io_cap_display_yes_no.
 * @retval BLE_HS_IO_KEYBOARD_ONLY    when io_cap == k_ra8_ble_io_cap_keyboard_only.
 * @retval BLE_HS_IO_NO_INPUT_OUTPUT  when io_cap == k_ra8_ble_io_cap_no_input_no_out.
...
{
  switch (io_cap) {
    case k_ra8_ble_io_cap_display_only:
      return (uint8_t)BLE_HS_IO_DISPLAY_ONLY;
    case k_ra8_ble_io_cap_display_yes_no:
      return (uint8_t)BLE_HS_IO_DISPLAY_YESNO;
    case k_ra8_ble_io_cap_keyboard_only:
      return (uint8_t)BLE_HS_IO_KEYBOARD_ONLY;
    case k_ra8_ble_io_cap_no_input_no_out:
      return (uint8_t)BLE_HS_IO_NO_INPUT_OUTPUT;
    case k_ra8_ble_io_cap_keyboard_display:
    default:
 ...
 * @retval k_ra8_err_null_ptr      cfg == NULL.
 * @retval k_ra8_err_invalid_arg   Out-of-range io_cap.
 *
 ...
 * @since 0.1.0
 */
ra8_err_t ra8_ble_security_init(const ra8_ble_security_config_t* cfg)
{
  ra8_err_t err = internal_validate_cfg(cfg);
  if (err != k_ra8_ok) {
    return err;
  }
 ...
 *          bond store; use ra8_ble_security_clear_bonds for that.
 *
 * @return ra8_err_t Error code.
 * @retval k_ra8_ok                  Wrapper closed.
 * @retval k_ra8_err_not_initialized Wrapper was never initialized.
 *
 * @pre ra8_ble_security_init has succeeded.
 * @pre Caller is single-threaded.
 ...
 * @since 0.1.0
 */
ra8_err_t ra8_ble_security_close(void)
{
 ...
 *
 * @return ra8_err_t Error code.
 * @retval k_ra8_ok                  Pairing started.
 ...
 * @since 0.1.0
 */
ra8_err_t ra8_ble_security_pair(uint16_t conn_handle)
{
  if (s_state.initialized == 0U) {
    return k_ra8_err_not_initialized;
  }
 ...
 *
 * @return ra8_err_t Error code.
 * @retval k_ra8_ok                  Reply injected.
 * @retval k_ra8_err_not_initialized Wrapper not initialized.
 * @retval k_ra8_err_invalid_arg     passkey out of range or NimBLE rejected.
 *
 * @pre ra8_ble_security_init has succeeded.
 ...
 * @since 0.1.0
 */
ra8_err_t ra8_ble_security_passkey_reply(uint16_t conn_handle, uint32_t passkey, uint8_t accept)
{
  if (s_state.initialized == 0U) {
    return k_ra8_err_not_initialized;
  }
...
#ifdef RA8_TARGET_BUILD
  struct ble_sm_io io = {};
  if (s_state.config.io_cap == k_ra8_ble_io_cap_display_yes_no ||
      s_state.config.io_cap == k_ra8_ble_io_cap_keyboard_display) {
    io.action        = BLE_SM_IOACT_NUMCMP;
    io.numcmp_accept = (accept != 0U) ? 1U : 0U;
  } else if (s_state.config.io_cap == k_ra8_ble_io_cap_display_only) {
 ...
 *
 * @return ra8_err_t Error code.
 * @retval k_ra8_ok                  Bonds cleared.
 * @retval k_ra8_err_not_initialized Wrapper not initialized.
 * @retval k_ra8_err_hw_error        Underlying store rejected the wipe.
 *
 * @pre ra8_ble_security_init has succeeded.
 * @pre Caller is single-threaded.
 ...
 * @since 0.1.0
 */
ra8_err_t ra8_ble_security_clear_bonds(void)
{
  if (s_state.initialized == 0U) {
    return k_ra8_err_not_initialized;
  }
 ...
 * @since 0.1.0
 */
ra8_err_t ra8_ble_security_bond_count(uint8_t* out_count)
{
 ...
 * @since 0.1.0
 */
ra8_err_t ra8_ble_security_attach_event_handler(ra8_ble_security_event_fn_t fn, void* ctx)
{
  s_state.event_fn  = fn;
 ...
 *
 * @param[in] count New count (0..k_ra8_ble_security_max_bonds).
 *
