 ...
 * through `ra_fs`, and loads a named font file into a caller-owned buffer.
 *
 ...
 *
 * The helper is transport-thin: it owns the `ra_sdmmc_spi` <-> `ra_sci_spi`
 ...
 * if (ra_sdfont_load(&cfg, font_buf, sizeof(font_buf), &len, &src) == k_ra_ok) {
 *   ra_reflow_init(w, h, font_buf, len, px, ink, link, &engine);
 * }
 * @endcode
 *
...
/**
 * @enum ra_sdfont_source_t
 * @brief Provenance of the font bytes returned by ::ra_sdfont_load.
 *
...
typedef struct {
  ...
  ra_port_pin_t  sck;            /**< SPI clock pin.                                     */
  ra_port_pin_t  cipo;           /**< Controller-In / Peripheral-Out pin.                */
  ra_port_pin_t  copi;           /**< Controller-Out / Peripheral-In pin.                */
 ...
 * GPIO output, brings up the card through `ra_sdmmc_spi`, mounts the FAT volume
 * through `ra_fs`, and opens @ref ra_sdfont_cfg_t::filename for reading. If the
 * file is absent and @ref ra_sdfont_cfg_t::provision_blob is non-NULL, the blob
 * is written to the card with `ra_fs_write_file`, then re-opened and read back
 ...
 * @retval k_ra_ok              Font loaded; @p *out_len holds its length.
 * @retval k_ra_err_null_ptr    @p cfg, @p buf, or @p out_len is NULL.
 * @retval k_ra_err_invalid_arg @p cap is 0.
 * @retval k_ra_err_not_found   File absent and provisioning disabled (NULL blob).
 * @retval k_ra_err_no_data     File present but shorter than a usable font header.
 * @retval other                Propagated from `ra_sci_spi`, `ra_sdmmc_spi`, or
 *                              `ra_fs` (no card, mount failure, write failure...).
 *
 * @pre `ra_cgc_init` has run and @ref ra_sdfont_cfg_t::pclka_hz is the live PCLKA rate.
 ...
 *
 * @see ra_sdfont_source_t
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_sdfont_load(const ra_sdfont_cfg_t* cfg,
                                      uint8_t*               buf,
                                      uint32_t               cap,
                                      uint32_t*              out_len,
