 ...
 *
 * @details Validates ``len`` is in (0, k_ra_nsc_xspi_max_read] and that
 *   the NS destination buffer is in writable NS memory, then forwards
 *   to ``ra_xspi_flash_read`` for the single configured instance.
 *
 * @param[in]  flash_off Byte offset into XSPI flash.
 * @param[out] ns_dst    Non-Secure destination buffer.
 * @param[in]  len       Bytes to read; 1..k_ra_nsc_xspi_max_read.
 *
 * @return ra_err_t outcome.
 * @retval k_ra_ok               Bytes copied to ``ns_dst``.
 * @retval k_ra_err_null_ptr     ``ns_dst`` was NULL.
 * @retval k_ra_err_invalid_arg  ``len`` zero / too large or range outside NS.
 *
 ...
 *
 * @return ra_err_t outcome.
 * @retval k_ra_ok               Status copied to ``*out_mask``.
 * @retval k_ra_err_null_ptr     ``out_mask`` was NULL.
 * @retval k_ra_err_invalid_arg  Range outside NS region or bad instance.
 *
 * @pre TrustZone substrate up.
