 ...
 *
 * @details
 * The WDT (``ra_wdt``, HUM Ch 27) is a Secure-owned peripheral -- only the
 * Secure world programmes WDTCR / WDTRR. The Non-Secure e-reader runs the
 * ThreadX watchdog supervisor (``ra_wdt_supervisor``), which decides in NS
 ...
 *
 *  - ``ra_nsc_wdt_start`` -- arm the counter once, with a fixed Secure-side
 *    configuration, just before the supervisor thread begins refreshing it.
 *  - ``ra_nsc_wdt_refresh`` -- refresh the down-counter on the supervisor's
 *    cadence. It returns ``void`` so it installs directly as the supervisor's
 *    ``ra_wdt_sup_refresh_fn_t`` hook.
 *
...
/**
 * @var s_wdt_cfg
 * @brief Fixed Secure-side WDT configuration used by ::ra_nsc_wdt_start.
 *
 * @details
 ...
 * ``k_ra_wdt_on_expiry_reset`` is a separate product decision). Living in the
 ...
 * @since 0.1.0
 */
static const ra_wdt_cfg_t s_wdt_cfg = {
 ...
 *
 * @details
 * Forwards to ``ra_wdt_init`` with the fixed ::s_wdt_cfg. In register-start
 * mode ``ra_wdt_init`` programmes WDTCR/WDTRCR/WDTCSTPR and issues the
 ...
 *
 * @return ``ra_err_t`` outcome from ``ra_wdt_init``.
 ...
 * @since 0.1.0
 */
RA_NSC_VENEER ra_err_t ra_nsc_wdt_start(void)
{
  return ra_wdt_init(&s_wdt_cfg);
}
 ...
 * ThreadX supervisor's ``ra_wdt_sup_refresh_fn_t`` hook so the supervisor kicks
