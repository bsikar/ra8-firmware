 ...
 * ``ra_nsc_cgc.h``. Each entry validates pointer arguments (when
 * present) and forwards to the secure ``ra_cgc_*`` API. The CGC
 ...
 *
 * @details Forwards to ::ra_cgc_pll2_enable. No pointer arguments
 ...
 *
 * @return ::ra_err_t from ::ra_cgc_pll2_enable.
 * @retval k_ra_ok               PLL2 locked.
 * @retval k_ra_err_invalid_arg  Bad multiplier.
 * @retval k_ra_err_hw_timeout   PLL2SF handshake timed out.
 *
 ...
 * @since 0.1.0
 */
RA_NSC_VENEER ra_err_t ra_nsc_cgc_pll2_enable(uint8_t     mul_int,
                                              uint8_t     mul_quarters,
                                              ra_plodiv_t p_div_code)
{
  return ra_cgc_pll2_enable(mul_int, mul_quarters, p_div_code);
}

/**
 * @brief NSC veneer: bring up the USB-FS module clock.
 *
 * @details Forwards to ::ra_cgc_usbfs_clock_enable.
 *
 * @return ::ra_err_t from ::ra_cgc_usbfs_clock_enable.
 * @retval k_ra_ok               USB-FS clock running.
 * @retval k_ra_err_hw_timeout   USBCKSRDY never came up.
 *
 * @pre TrustZone substrate up.
 * @pre PLL2 has been locked via ::ra_nsc_cgc_pll2_enable.
 ...
 * @since 0.1.0
 */
RA_NSC_VENEER ra_err_t ra_nsc_cgc_usbfs_clock_enable(void)
{
  return ra_cgc_usbfs_clock_enable();
}
 ...
 *   forwards to ::ra_cgc_get_clock_hz.
 *
 * @param[in]  id     Clock identifier.
 * @param[out] hz_out NS destination for the frequency in Hz.
 *
 * @return ::ra_err_t outcome.
 * @retval k_ra_ok               Frequency stored.
 * @retval k_ra_err_null_ptr     ``hz_out`` was NULL.
 * @retval k_ra_err_invalid_arg  ``hz_out`` outside NS region or bad id.
 *
 * @pre TrustZone substrate up.
 ...
 * @since 0.1.0
 */
RA_NSC_VENEER ra_err_t ra_nsc_cgc_get_clock_hz(ra_clock_id_t id, uint32_t* hz_out)
{
  RA_CHECK_NULL_PTR(hz_out, s_tag, "cgc_get_clock_hz: hz_out");
  RA_NSC_CHECK_NS_RANGE_RW(hz_out, sizeof(*hz_out));
  return ra_cgc_get_clock_hz(id, hz_out);
}
