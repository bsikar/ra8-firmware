 ...
 * that simply forwards to the matching ``ra_cgc_*`` API, with pointer
 ...
 *
 * @details
 * Forwards to ::ra_cgc_pll2_enable. PLL2 is required for the USB-FS
 * 48 MHz reference clock; see ::ra_nsc_cgc_usbfs_clock_enable for the
 ...
 * @param[in] p_div_code   PLL2 P-divider code (::ra_plodiv_t).
 *
 ...
 *
 * @pre TrustZone substrate up and ::ra_cgc_init has been called.
 ...
 * @since 0.1.0
 */
[[nodiscard]] RA_NSC_VENEER ra_err_t ra_nsc_cgc_pll2_enable(uint8_t     mul_int,
 ...
 *
 * @details
 * Forwards to ::ra_cgc_usbfs_clock_enable. Must run before any NS
 * caller releases MSTPB11 (USBFS), otherwise the SREQ -> SRDY
 * handshake hangs silently. ::ra_nsc_cgc_pll2_enable must have been
 * called first to lock PLL2 at the canonical 240 MHz target.
 *
 * @return ::ra_err_t outcome.
 * @retval k_ra_ok               USB-FS module clock running at 48 MHz.
 * @retval k_ra_err_hw_timeout   USBCKSRDY handshake never completed.
 *
 * @pre ::ra_nsc_cgc_pll2_enable returned ::k_ra_ok.
 * @pre TrustZone substrate up and ::ra_cgc_init has been called.
 *
 * @par TrustZone:
 ...
 * @since 0.1.0
 */
[[nodiscard]] RA_NSC_VENEER ra_err_t ra_nsc_cgc_usbfs_clock_enable(void);

/**
 * @brief NSC veneer: query a clock-tree frequency.
 *
 * @details
 * Forwards to ::ra_cgc_get_clock_hz with NS range-validation of the
 * ``hz_out`` destination so the secure driver can never write outside
 * the Non-Secure data region.
 *
 * @param[in]  id     Clock identifier (::ra_clock_id_t).
 * @param[out] hz_out NS destination for the current frequency in Hz.
 *
 * @return ::ra_err_t outcome.
 * @retval k_ra_ok               Frequency stored at ``*hz_out``.
 ...
 * @since 0.1.0
 */
[[nodiscard]] RA_NSC_VENEER ra_err_t ra_nsc_cgc_get_clock_hz(ra_clock_id_t id, uint32_t* hz_out);
