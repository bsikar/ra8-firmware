...
pub fn priv_modem_str_len(s: [*:0]const u8) callconv(.c) u16 {
    return core.strLen(s);
}

pub fn priv_modem_starts_with(hay: [*:0]const u8, needle: [*:0]const u8) callconv(.c) u8 {
    return core.startsWith(hay, needle);
}

pub fn priv_modem_str_eq(a: [*:0]const u8, b: [*:0]const u8) callconv(.c) u8 {
    return core.strEq(a, b);
}

pub fn priv_modem_classify(
    line: [*:0]const u8,
    cmd_echo: ?[*:0]const u8,
    expected_response: ?[*:0]const u8,
) callconv(.c) u8 {
...
pub fn priv_modem_reset_line_should_clear(
...
pub fn priv_modem_payload_prefix_matches(
    line: [*:0]const u8,
    expected_response: ?[*:0]const u8,
) callconv(.c) u8 {
...
pub fn priv_modem_capture_should_clear(
...
comptime {
    if (build_options.test_helpers) {
        @export(&priv_modem_str_len, .{ .name = "priv_modem_str_len" });
        @export(&priv_modem_starts_with, .{ .name = "priv_modem_starts_with" });
        @export(&priv_modem_str_eq, .{ .name = "priv_modem_str_eq" });
        @export(&priv_modem_classify, .{ .name = "priv_modem_classify" });
        @export(&priv_modem_capture_line, .{ .name = "priv_modem_capture_line" });
        @export(&priv_modem_reset_line_should_clear, .{ .name = "priv_modem_reset_line_should_clear" });
        @export(&priv_modem_payload_prefix_matches, .{ .name = "priv_modem_payload_prefix_matches" });
        @export(&priv_modem_capture_should_clear, .{ .name = "priv_modem_capture_should_clear" });
    }
}
