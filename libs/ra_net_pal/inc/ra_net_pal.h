 ...
 * delivered through ``ra_net_pal_recv_frame`` polling -- the IRQ
...
/**
 * @enum ra_net_pal_event_t
 * @brief Bits passed to ``ra_net_pal_event_fn_t``.
 */
 ...
 *
 * @details
 * Powers on the underlying ``ra_eth`` driver via ``ra_eth_init``,
 ...
 * handler, and calls ``ra_eth_deinit`` to drop the ESWM MSTP
 * reference.
 *
 * @return ``ra_err_t`` error code.
 ...
 *
 * @post Link state reads as ``k_ra_net_pal_link_down``.
 ...
 *
 * @details
 * Updates the in-memory copy of the MAC and (when ra_eth gains
 * MAC-write support) the ESWM hardware filter. Called by the
 * stack at any time after ``ra_net_pal_init``.
 *
