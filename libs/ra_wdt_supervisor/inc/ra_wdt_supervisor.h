 ...
 *
 * @details
 * The hardware Watchdog Timer (``ra_wdt``) demands a refresh inside the
 ...
 *  2. The thread calls ``ra_wdt_supervisor_checkin`` at safe points in
 *     its loop (typically once per iteration).
 *  3. A dedicated supervisor thread wakes every ``refresh_period_ms``
 *     and only calls ``ra_wdt_refresh_deferred`` if **every** registered
...
/**
 * @enum ra_wdt_sup_handle_t
 * @brief Opaque handle returned by ``ra_wdt_supervisor_register_thread``.
 *
 * @details
 * Handles are small integers in [0, k_ra_wdt_sup_max_threads). The
 * sentinel ``k_ra_wdt_sup_handle_invalid`` is returned in ``out_handle``
 * if registration fails.
 */
typedef enum : uint8_t {
  k_ra_wdt_sup_handle_invalid = 0xFFU, /**< Invalid / unregistered handle. */
} ra_wdt_sup_handle_t;

/**
 * @struct ra_wdt_sup_cfg_t
 ...
 * const ra_wdt_sup_cfg_t cfg = {
 *   .stack          = s_sup_stack,
 *   .stack_size_bytes = sizeof s_sup_stack,
 *   .priority       = 4,
 *   .refresh_period_ms = 50,
 * };
 * (void)ra_wdt_supervisor_init(&cfg);
 * @endcode
 */
typedef struct {
 ...
 */
typedef uint32_t (*ra_wdt_sup_now_fn_t)(void);

/**
 * @typedef ra_wdt_sup_refresh_fn_t
 * @brief Hook used to call into ``ra_wdt_refresh_deferred``.
 *
 * @details
 * Default points at ``ra_wdt_refresh_deferred``. Tests override this so
 * the test body can count refresh calls without mmapping fake WDT
 * registers.
 */
typedef void (*ra_wdt_sup_refresh_fn_t)(void);
 ...
 * first, then call ``ra_wdt_supervisor_start``.
 *
 * Algorithm:
 ...
 * @endcode
 *
 * @see ra_wdt_supervisor_register_thread
 * @see ra_wdt_supervisor_start
 *
 * @since 0.1.0
 */
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_wdt_supervisor_deinit(void);

/**
 * @brief Register a worker thread with the supervisor.
 *
 * @details
 * Allocates one of the ``k_ra_wdt_sup_max_threads`` static slots, copies
 * up to ``k_ra_wdt_sup_name_max - 1`` bytes of ``name`` for diagnostics,
 ...
 *
 * @return ``ra_err_t``
 * @retval k_ra_ok Slot allocated, ``*out_handle`` populated.
 ...
 * @post On failure, ``*out_handle`` is set to ``k_ra_wdt_sup_handle_invalid``.
 *
 ...
 *
 * @param[in] handle Handle returned by ``ra_wdt_supervisor_register_thread``.
 *
 * @return ``ra_err_t``
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_wdt_supervisor_checkin(uint8_t handle);
 ...
 *  4. If every slot passes, call the refresh hook -> ``ra_wdt_refresh_deferred``.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_wdt_supervisor_start(void);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_wdt_supervisor_tick(bool* out_did_refresh);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_wdt_supervisor_set_now_hook(ra_wdt_sup_now_fn_t now);
 ...
 * ``ra_wdt_refresh_deferred``. NASA Rule 9 deviation: function pointer
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_wdt_supervisor_set_refresh_hook(ra_wdt_sup_refresh_fn_t refresh);
 ...
 *
 * @return Count in [0, k_ra_wdt_sup_max_threads].
 *
 * @pre None.
