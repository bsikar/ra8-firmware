 ...
 * `ra8_reflow_close`), loader binding (image / CSS), chapter dispatch
...
static ra8_err_t priv_layout_tokens(ra8_reflow_t* engine, const stbtt_fontinfo* font)
{
  priv_cursor_t cur = {
    ...
    .line_first_glyph = 0U,
    .line_has_content = 0U,
    .align            = (uint8_t)k_ra8_reflow_align_left,
    .reserved8        = {0U, 0U},
  };
...
ra8_err_t ra8_reflow_run_layout(ra8_reflow_t* engine)
{
  if (engine == nullptr) {
...
ra8_err_t ra8_reflow_init(uint16_t       viewport_w,
                          uint16_t       viewport_h,
                          ...
                          ra8_reflow_t*  out_engine)
{
  ...
  if (font_px < k_ra8_reflow_min_font_px || font_px > k_ra8_reflow_max_font_px) {
    return k_ra8_err_invalid_arg;
  }
  if (font_len < 16U) {
...
ra8_err_t ra8_reflow_close(ra8_reflow_t* engine)
{
  if (engine == nullptr) {
    return k_ra8_err_null_ptr;
  }
...
ra8_err_t ra8_reflow_set_image_loader(ra8_reflow_t*              engine,
...
ra8_err_t ra8_reflow_layout_chapter(ra8_reflow_t*  engine,
                                    ...
                                    uint32_t*      out_total_pages)
{
  ...
  *out_total_pages = 0U;
  ...
  ra8_err_t err = ra8_reflow_parse_xhtml(engine, xhtml_buf, xhtml_len);
  if (err != k_ra8_ok) {
    return err;
  }
  err = ra8_reflow_run_layout(engine);
  if (err != k_ra8_ok) {
    return err;
  }
...
ra8_err_t ra8_reflow_get_page_count(const ra8_reflow_t* engine, uint32_t* out_count)
{
...
ra8_err_t ra8_reflow_set_font_size(ra8_reflow_t* engine, uint16_t new_font_px)
{
  if (engine == nullptr) {
    return k_ra8_err_null_ptr;
  }
...
/** @brief Implementation of `ra8_reflow_bind_font()` -- validate then swap. */
ra8_err_t ra8_reflow_bind_font(ra8_reflow_t* engine, const uint8_t* font_data, size_t font_len)
{
  ...
  if (font_len < (size_t)k_ra8_reflow_min_font_bytes) {
    return k_ra8_err_invalid_size;
  }
...
/** @brief Implementation of `ra8_reflow_register_face()` -- validate then append. */
ra8_err_t ra8_reflow_register_face(ra8_reflow_t*  engine,
                                   ...
                                   size_t         len)
{
  ...
  if (len < (size_t)k_ra8_reflow_min_font_bytes) {
    return k_ra8_err_invalid_size;
  }
  if (engine->face_count >= (uint8_t)k_ra8_reflow_max_faces) {
    return k_ra8_err_no_mem;
  }
