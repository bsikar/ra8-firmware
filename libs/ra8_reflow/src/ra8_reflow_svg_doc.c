 ...
 *
 * @details Uses @c ra8_svgp_starts_ci to match @p name at @p at, then verifies that
...
static bool priv_elem_at(const uint8_t* s, size_t len, size_t at, const char* name)
{
  ...
  return ra8_svgp_ws(c) || (c == '>') || (c == '/');
}

/**
 * @brief Locate the tag span [*open, *close) of element @p name at or after @p from.
 *
 * @details Calls @c ra8_svgp_find_ci to find the first occurrence of @p name in
...
static bool priv_tag_span(const uint8_t* s,
                          ...
                          size_t*        close)
{
  const size_t at = ra8_svgp_find_ci(s, len, from, name);
  if (at >= len) {
    return false;
  }
  size_t j = at;
...
static void priv_read_viewbox(const uint8_t* s, size_t len, svg_xform_t* t)
{
  ...
  size_t         voff = 0U;
  size_t         vlen = 0U;
  if (ra8_svgp_attr(tag, tlen, "viewBox", &voff, &vlen)) {
    int32_t nums[k_svg_viewbox_n] = {};
    size_t  k                     = 0U;
    for (size_t n = 0U; n < (size_t)k_svg_viewbox_n; ++n) {
      nums[n] = ra8_svgp_num(&tag[voff], vlen, &k);
    }
    ...
    const int32_t wv = ra8_svgp_attr_num(tag, tlen, "width", t->bw);
    const int32_t hv = ra8_svgp_attr_num(tag, tlen, "height", t->bh);
    if ((wv > 0) && (hv > 0)) {
      t->vw = wv;
      t->vh = hv;
    }
 ...
 * element-level 'transform=' onto a local copy of @p t via @c ra8_svgp_apply_xform,
...
priv_dispatch_shape(const uint8_t* s, size_t len, size_t at, size_t close, const svg_xform_t* t)
{
  ...
  svg_xform_t lt = *t;
  ra8_svgp_apply_xform(&lt, tag, tlen);
  if (priv_elem_at(s, len, at, "<rect")) {
    ra8_svgp_draw_rect(tag, tlen, &lt);
  } else if (priv_elem_at(s, len, at, "<circle")) {
    ra8_svgp_draw_circle(tag, tlen, &lt);
  } else if (priv_elem_at(s, len, at, "<polygon")) {
    ra8_svgp_draw_polygon(tag, tlen, &lt);
  } else if (priv_elem_at(s, len, at, "<polyline")) {
    ra8_svgp_draw_polyline(tag, tlen, &lt);
  } else if (priv_elem_at(s, len, at, "<path")) {
    ra8_svgp_draw_path(tag, tlen, &lt);
  } else if (priv_elem_at(s, len, at, "<line")) {
    ra8_svgp_draw_line(tag, tlen, &lt);
  } else {
 ...
 * with any 'transform=' attribute on the 'g' element via @c ra8_svgp_apply_xform.
                               ...
                               int32_t            gsp)
{
  ...
  ra8_svgp_apply_xform(&gt, &s[i], (close > i) ? (close - i) : 0U);
  /*
 ...
 * that slice to @c ra8_svgp_numf starting at offset zero. Returns @p def when the
...
static float priv_attr_numf(const uint8_t* s, size_t len, const char* name, float def)
{
  ...
  size_t k = 0U;
  return ra8_svgp_numf(&s[off], vl, &k);
}

/**
 * @brief Copy the 'id' attribute value into @p dst as a NUL-terminated string.
 *
 * @details Looks up the "id" attribute via @c ra8_svgp_attr. If absent, writes a
...
static void priv_copy_attr_id(const uint8_t* s, size_t len, char* dst)
{
  ...
  size_t off = 0U;
  size_t vl  = 0U;
  if (!ra8_svgp_attr(s, len, "id", &off, &vl)) {
    return;
  }
 ...
 *
 * @details Reads the "offset" attribute from the stop tag via @c ra8_svgp_attr and
 * parses its value as a float via @c ra8_svgp_numf. If a '%' character is found
...
static float priv_stop_offset(const uint8_t* tag, size_t tlen)
{
  size_t ooff = 0U;
  size_t ovl  = 0U;
  if (!ra8_svgp_attr(tag, tlen, "offset", &ooff, &ovl)) {
    return 0.0F;
  }
  size_t      k   = 0U;
  const float off = ra8_svgp_numf(&tag[ooff], ovl, &k);
 ...
 *
 * @details Reads the "stop-color" attribute from the stop tag via @c ra8_svgp_attr
 * and parses the value as a paint via @c ra8_svgp_paint. If the attribute is absent
...
static uint32_t priv_stop_color(const uint8_t* tag, size_t tlen)
{
  size_t coff = 0U;
  size_t cvl  = 0U;
  if (!ra8_svgp_attr(tag, tlen, "stop-color", &coff, &cvl)) {
    return (uint32_t)k_svg_def_fill;
  }
  const uint32_t pc = ra8_svgp_paint(&tag[coff], cvl);
 ...
 *
 * @details Scans @p s[@p from .. @p end) for '<stop' tags using @c ra8_svgp_find_ci.
...
static void priv_parse_stops(const uint8_t* s, size_t from, size_t end, svg_grad_t* g)
{
  ...
  while (g->nstops < (uint8_t)k_svg_grad_stops) {
    const size_t sp = ra8_svgp_find_ci(s, end, at, "<stop");
    if (sp >= end) {
      break;
    }
    size_t close = sp;
...
static void priv_scan_grads(const uint8_t* s, size_t len, svg_grads_t* gs)
{
  ...
  while (gs->n < (int32_t)k_svg_grad_max) {
    const size_t lin    = ra8_svgp_find_ci(s, len, from, "<linearGradient");
    const size_t rad    = ra8_svgp_find_ci(s, len, from, "<radialGradient");
    ...
    *g                  = (svg_grad_t){};
    ...
    const size_t endpos  = ra8_svgp_find_ci(s, len, close, endtag);
...
bool ra8_svg_is_svg(const uint8_t* bytes, size_t len)
{
  ...
  while ((i < len) && ra8_svgp_ws((char)bytes[i])) {
    ++i;
  }
  return ra8_svgp_starts_ci(bytes, len, i, "<?xml") || ra8_svgp_starts_ci(bytes, len, i, "<svg");
}
...
ra8_err_t ra8_svg_image_href(const uint8_t* svg, size_t len, size_t* out_off, size_t* out_len)
{
  ...
  size_t         voff = 0U;
  size_t         vlen = 0U;
  if (!ra8_svgp_attr(tag, tlen, "xlink:href", &voff, &vlen) &&
      !ra8_svgp_attr(tag, tlen, "href", &voff, &vlen)) {
    return k_ra8_err_not_found;
  }
  *out_off = open + voff;
  *out_len = vlen;
...
ra8_err_t ra8_svg_size(const uint8_t* svg, size_t len, int32_t* out_w, int32_t* out_h)
{
  ...
  const uint8_t* tag  = &svg[open];
  const size_t   tlen = close - open;
  int32_t        w    = ra8_svgp_attr_num(tag, tlen, "width", 0);
  int32_t        h    = ra8_svgp_attr_num(tag, tlen, "height", 0);
  if ((w <= 0) || (h <= 0)) {
    size_t voff = 0U;
    size_t vlen = 0U;
    if (ra8_svgp_attr(tag, tlen, "viewBox", &voff, &vlen)) {
