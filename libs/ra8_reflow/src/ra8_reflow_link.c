...
ra8_err_t ra8_reflow_hit_test_link(const ra8_reflow_t* engine,
                                   uint32_t            page_idx,
                                   int32_t             x,
                                   int32_t             y,
                                   ...
                                   uint32_t*           out_href_len)
{
  ...
  for (uint32_t i = 0U; i < engine->link_rect_count; ++i) {
    const ra8_reflow_link_rect_t* rect = &engine->link_rects[i];
...
ra8_err_t ra8_reflow_find_anchor(const ra8_reflow_t* engine,
                                 const char*         id,
                                 uint32_t            id_len,
                                 uint32_t*           out_page)
{
  ...
  for (uint32_t i = 0U; i < engine->anchor_count; ++i) {
    const ra8_reflow_anchor_t* anchor = &engine->anchors[i];
 ...
 * one of ::ra8_reflow_href_kind_t. Factored out of ra8_reflow_href_split() so the
 ...
 * @post @p out_kind is set to a valid ::ra8_reflow_href_kind_t value.
                               ...
                               uint32_t*               out_frag_len)
{
  ...
  *out_path_len       = hash;
  ...
  if (hash == 0U) {
    *out_kind = has_frag ? k_ra8_reflow_href_fragment : k_ra8_reflow_href_empty;
  } else {
    *out_kind = has_frag ? k_ra8_reflow_href_chapter_fragment : k_ra8_reflow_href_chapter;
  }
}

ra8_err_t ra8_reflow_href_split(const char*             href,
                                uint32_t                len,
                                ra8_reflow_href_kind_t* out_kind,
                                uint32_t*               out_path_len,
                                uint32_t*               out_frag_off,
