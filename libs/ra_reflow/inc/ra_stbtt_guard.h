 ...
 * `@font-face` fonts embedded in an EPUB (via `ra_reflow_register_face()` /
 * `ra_reflow_bind_font()`) and the book font attached with
 * `ra_epub_set_font()`. An attacker-supplied EPUB can thus reach the unguarded
 * parse on-device.
 *
 * ::ra_stbtt_sfnt_dir_in_bounds validates the full offset table + table
 ...
 * if (off >= 0 && !ra_stbtt_sfnt_dir_in_bounds(blob, blob_len, (uint32_t)off)) {
 *   return k_ra_err_not_supported; // malformed directory -- reject
 * }
