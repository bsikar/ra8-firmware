/**
 * @file ra_reflow_svg.h
 * @brief Minimal SVG handling for the ra_reflow ereader (#112).
 *
 * @details
 ...
 *      (filled; M/L/H/V/Z exact, curves C/S/Q/T/A approximated by their endpoint
 *      chords), with solid `fill` / `stroke`. ::ra_svg_render maps them from the
 *      SVG `viewBox` (or width/height) into a caller box and draws them through
 *      the bound ra_gfx framebuffer.
 *
 * Out of scope (documented limitations, tracked as follow-ups): proper Bezier /
 * arc flattening for `<path>` curves, gradients, transforms, opacity, CSS
 * styling, stroke width, and fractional coordinates (truncated to integers).
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] bool ra_svg_is_svg(const uint8_t* bytes, size_t len);
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok            An `<image>` href was found.
 * @retval k_ra_err_null_ptr  A required pointer is NULL.
 * @retval k_ra_err_not_found No `<image>` with an href was present.
 *
 ...
 */
[[nodiscard]] ra_err_t
ra_svg_image_href(const uint8_t* svg, size_t len, size_t* out_off, size_t* out_len);
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok            A positive size was determined.
 * @retval k_ra_err_null_ptr  A required pointer is NULL.
 * @retval k_ra_err_not_found No `<svg>` or no positive size present.
 *
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              Rendered (possibly zero shapes).
 * @retval k_ra_err_null_ptr    @p svg is NULL.
 * @retval k_ra_err_invalid_arg @p w or @p h is non-positive.
 *
 * @pre @p svg non-NULL; ra_gfx_init() has bound a framebuffer.
 * @pre @p w > 0 and @p h > 0.
 * @post Shapes are drawn into the bound framebuffer within @p (x,y,w,h).
 * @post No allocation performed.
 *
 * @note Not thread-safe relative to the bound ra_gfx framebuffer.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t
ra_svg_render(const uint8_t* svg, size_t len, int32_t x, int32_t y, int32_t w, int32_t h);
