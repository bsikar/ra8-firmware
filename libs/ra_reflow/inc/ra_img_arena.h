 ...
 * bump-allocate out of a **caller-bound** arena (::ra_img_arena_t). Unlike the
 ...
 * the consumer via ra_img_arena_bind() rather than baked in here.
 *
 * The allocator is a bump arena with reference-counted auto-reset: each
 * ra_img_arena_malloc() bumps `offset` and increments `live`; each
 * ra_img_arena_free() decrements `live` and, when it reaches zero, rewinds
...
#include "ra_reflow_image.h" /* ra_img_arena_t */
 ...
 * (`offset = 0`, `live = 0`). Every ra_img_arena_malloc() / _free() / _realloc
 * until the next ra_img_arena_bind() or ra_img_arena_unbind() operates on it.
 *
 * @param[in,out] arena Arena to make active; reset to empty on entry. NULL
 *                      unbinds (equivalent to ra_img_arena_unbind()).
 * @return None.
 ...
 * @post A subsequent ra_img_arena_malloc() draws from @p arena.
 * @note Not thread-safe: image decoding is single-threaded.
 * @since 0.1.0
 */
void ra_img_arena_bind(ra_img_arena_t* arena);
 ...
 * @post A subsequent ra_img_arena_malloc() returns nullptr until the next bind.
 * @note Not thread-safe: image decoding is single-threaded.
 * @since 0.1.0
 */
void ra_img_arena_unbind(void);
 ...
 * @since 0.1.0
 */
void* ra_img_arena_malloc(size_t n);
 ...
 * @return None.
 * @pre @p p was returned by ra_img_arena_malloc()/_realloc_sized(), or is nullptr.
 * @pre Called single-threaded from the decoder.
 ...
 * @since 0.1.0
 */
void* ra_img_arena_realloc_sized(void* p, size_t oldsz, size_t newsz);
