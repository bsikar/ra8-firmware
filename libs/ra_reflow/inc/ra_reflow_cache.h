/**
 * @file ra_reflow_cache.h
 * @brief Import-time pagination cache for `ra_reflow` (#79).
 *
 * @details
 * Laying out a chapter (`ra_reflow_layout_chapter()`) is the expensive
 ...
 * `ra_fs_write_file()` / `ra_fs_open()` + `ra_fs_read()`), so this
 * module pulls in **no** filesystem dependency. The intended flow:
 *
 * @code
 *   // open path -- skip layout when the cache is valid:
 *   if (ra_reflow_cache_load(&eng, body, body_len, blob, blob_len) != k_ra_ok) {
 *     uint32_t pages = 0U;
 *     (void)ra_reflow_layout_chapter(&eng, body, body_len, &pages);
 *     size_t n = 0U;
 *     (void)ra_reflow_cache_serialize(&eng, body, body_len, blob, sizeof blob, &n);
 *     // ... persist blob[0..n) for next time ...
 *   }
 * @endcode
 *
 * `ra_reflow_cache_load()` validates the blob's key against the engine's
 * current viewport / font / colours and the supplied content; on any
 * mismatch it returns `k_ra_err_invalid_state` so the caller re-lays-out.
...
/**
 * @enum ra_reflow_cache_id_t
 * @brief On-wire format identity for a serialised pagination cache.
 *
 * @details
 * `k_ra_reflow_cache_magic` is the first 4 bytes of every blob (stored
 * little-endian). `k_ra_reflow_cache_version` is bumped whenever the
...
typedef enum : uint32_t {
  ...
  k_ra_reflow_cache_version = 1U,          /**< Serialised format version.  */
} ra_reflow_cache_id_t;

/**
 * @enum ra_reflow_cache_layout_t
 * @brief Byte sizes of the serialised header and per-record payloads.
 *
 * @details
 * The blob is `header + glyph_count * glyph_bytes + page_count *
 * page_bytes`. A glyph serialises its 6 meaningful fields (x, y, cp,
 * colour, font_px, style = 4+4+4+4+2+1); a page serialises its two
 * `uint32_t` index fields. The struct padding bytes are **not** stored.
 */
typedef enum : size_t {
  k_ra_reflow_cache_header_bytes = 52U, /**< Fixed header size, bytes.       */
  k_ra_reflow_cache_glyph_bytes  = 19U, /**< Serialised bytes per glyph.     */
 ...
 * calling `ra_reflow_cache_serialize()`. The value is
 * `k_ra_reflow_cache_header_bytes +
 *  engine->glyph_count * k_ra_reflow_cache_glyph_bytes +
 *  engine->page_count  * k_ra_reflow_cache_page_bytes`.
 *
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok                  Size reported.
 * @retval k_ra_err_null_ptr        `engine` or `out_bytes` is NULL.
 * @retval k_ra_err_not_initialized `engine->in_use == 0`.
 *
 * @pre  `engine` non-NULL and `engine->in_use == 1`.
 * @post `*out_bytes >= k_ra_reflow_cache_header_bytes`.
 *
 * @since 0.1.0
 */
 ...
 * later `ra_reflow_cache_load()` can detect any change that would
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok                  Serialised; `*out_len` bytes written.
 * @retval k_ra_err_null_ptr        `engine`, `out_buf` or `out_len` NULL.
 * @retval k_ra_err_not_initialized `engine->in_use == 0`.
 * @retval k_ra_err_invalid_arg     `content` NULL with `content_len > 0`.
 * @retval k_ra_err_invalid_size    `out_cap` smaller than the blob size.
 *
 * @pre  `engine->in_use == 1`; a chapter has been laid out.
 * @post On success the first `*out_len` bytes of `out_buf` are a valid
 *       blob accepted by `ra_reflow_cache_load()`.
 *
 * @see ra_reflow_cache_size()
 * @see ra_reflow_cache_load()
 *
 * @since 0.1.0
 */
 ...
 * later `ra_reflow_set_font_size()` can still re-flow). On any mismatch
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok                   Loaded; glyphs / pages restored.
 * @retval k_ra_err_null_ptr         `engine` or `buf` is NULL.
 * @retval k_ra_err_not_initialized  `engine->in_use == 0`.
 * @retval k_ra_err_invalid_arg      `content` NULL with `content_len>0`.
 * @retval k_ra_err_not_found        Magic mismatch (not a cache blob).
 * @retval k_ra_err_invalid_state    Version or key mismatch -- the layout
 *                                   is stale; re-lay-out and re-serialise.
 * @retval k_ra_err_invalid_size     Truncated blob or counts exceed pools.
 * @retval k_ra_err_checksum_mismatch Body checksum failed (corrupt blob).
 *
 * @pre  `engine->in_use == 1`.
 * @post On `k_ra_ok`, `engine->glyph_count` / `page_count` and the
 *       restored arrays match the serialised layout and rendering may
 *       proceed without `ra_reflow_layout_chapter()`.
 * @post On any non-`k_ra_ok` return the engine is unchanged.
 *
 * @see ra_reflow_cache_serialize()
 *
 * @since 0.1.0
 */
