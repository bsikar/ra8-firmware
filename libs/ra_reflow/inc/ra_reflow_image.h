/**
 * @file ra_reflow_image.h
 * @brief Zero-heap raster image decode + scale + blit for ra_reflow (#106).
 *
 * @details
 ...
 * layout box -- into the `ra_gfx` framebuffer. The cover path (#106 Phase 1)
 * and the in-chapter `<img>` path (Phase 2) both call ::ra_img_decode_blit.
 *
 ...
 * arena** (::ra_img_arena_t). The caller sizes the scratch for the largest
 ...
 * `(dst_x, dst_y)` via `ra_gfx`. The arena is fully reset on return (success or
 * failure). The caller must have bound a framebuffer with `ra_gfx_init()`.
 *
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok                 Image decoded, scaled, and blitted.
 * @retval k_ra_err_null_ptr       @p arena or @p bytes is NULL.
 * @retval k_ra_err_invalid_arg    @p len is 0, or @p box_w / @p box_h is < 1.
 * @retval k_ra_err_not_supported  stb_image could not decode the bytes.
 * @retval k_ra_err_no_mem         The arena is too small for this image.
 *
 * @pre `ra_gfx_init()` bound a framebuffer.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_img_decode_blit(ra_img_arena_t* arena,
                                          const uint8_t*  bytes,
                                          size_t          len,
                                          int32_t         dst_x,
                                          int32_t         dst_y,
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok                Dimensions read.
 * @retval k_ra_err_null_ptr      Any pointer argument is NULL.
 * @retval k_ra_err_not_supported stb_image could not parse the header.
 *
 ...
 */
[[nodiscard]] ra_err_t
ra_img_probe_size(const uint8_t* bytes, size_t len, int32_t* out_w, int32_t* out_h);
