/**
 * @file ra_reflow_layout_internal.h
 * @brief Cross-TU shared declarations for the ra_reflow layout engine.
 *
 * @details
 * The layout engine is split across three translation units so each stays
 * under the project file-size cap:
 *
 *   - `ra_reflow_layout.c`        -- core line/page break + inline flow.
 *   - `ra_reflow_layout_table.c`  -- `<table>` equal-column grid layout.
 *   - `ra_reflow_layout_driver.c` -- the token-stream driver + public API.
 *
...
/* ===========================================================================
 * Cross-TU layout helpers (defined in ra_reflow_layout.c unless noted).
 * ===========================================================================
 */
 ...
 * would exceed the bottom margin it calls @ref ra_reflow_layout_finish_page to
 ...
 *        placeholder.
 *
 * @details Defined in `ra_reflow_layout_image.c`. When the engine has a bound
 ...
 * @since 0.1.0
 */
bool ra_reflow_layout_apply_image(ra_reflow_t*             engine,
                                  priv_cursor_t*           cur,
                                  const ra_reflow_token_t* tok);
 ...
 *
 * @details Flushes the pending line (if any) via @ref ra_reflow_layout_newline,
 ...
 * @since 0.1.0
 */
ra_err_t ra_reflow_layout_table(ra_reflow_t*          engine,
                                priv_cursor_t*        cur,
                                const stbtt_fontinfo* font,
                                uint32_t              start,
                                uint32_t*             out_next);
