/**
 * @file ra_reflow_css_cascade.c
 * @brief Content-CSS stylesheet scanner, cascade, and face lookup (#111).
 *
 * @details
 * The top-level stylesheet scanner (::ra_css_parse, comment + block skipping),
 * the selector-matching and specificity cascade, and the `@font-face` lookup
 * API for the v1 CSS subset documented in `ra_reflow_css.h`. The scanner walks
 * `selector { ... }` blocks and hands each to ::ra_reflow_css_parse_one_block in
 * a sibling translation unit; the cascade resolves inherited / rule / inline
 * sources into a single ::ra_css_style_t. No MMIO, no heap; all state lives in
 * the caller-owned ::ra_css_sheet_t.
 *
...
#include "ra_reflow.h" /* k_ra_reflow_tag_unknown, style/align enums */
...
ra_err_t ra_css_parse(ra_css_sheet_t* sheet, const char* css, uint32_t len)
{
  ...
  while (i < (size_t)len) {
    ...
    ra_reflow_css_parse_one_block(sheet,
                                  &css[i],
                                  brace - i,
                                  &css[brace + 1U],
                                  close - (brace + 1U));
...
static bool priv_class_list_has(const char* list, size_t list_len, const char* name, size_t nlen)
{
  ...
  while (i < list_len) {
    while ((i < list_len) && ra_reflow_css_is_ws(list[i])) {
      ++i;
    }
    size_t start = i;
    while ((i < list_len) && !ra_reflow_css_is_ws(list[i])) {
 ...
 * ASCII case-folding via ra_reflow_css_lower(). Returns false immediately when either
...
static bool priv_ci_eq_span(const char* a, size_t alen, const char* b, size_t blen)
{
  ...
  for (size_t k = 0U; k < alen; ++k) {
    if (ra_reflow_css_lower(a[k]) != ra_reflow_css_lower(b[k])) {
      return false;
    }
  }
  return true;
