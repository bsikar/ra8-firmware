 ...
 * (::ra_svgp_mx / ::ra_svgp_my / ::ra_svgp_sx) and the general
 * ::ra_svgp_map_point used by the shape and path stages. No DOM, no heap. See
 ...
 * rotation or shear, use @c ra_svgp_map_point instead to apply the full affine.
 *
 ...
 * path via @c ra_svgp_map_point applied to each vertex.
 *
 * @param[in] t Active coordinate transform; must not be NULL.
 *
 ...
 * @c (ra_svgp_mx(ux), ra_svgp_my(uy)), so routing the polygon/path/line emitters
 ...
 * affine that can be applied per-point in @c ra_svgp_map_point.
 *
 ...
 * @c ra_svgp_numf for the next argument in a transform function's argument list.
 *
 ...
 * @c ra_svgp_numf for each number that begins at the current cursor. The loop
...
{
  int32_t na = 0;
  /* Bounded: <= k_svg_argc_cube args; each ra_svgp_numf advances *j. */
  while (na < (int32_t)k_svg_argc_cube) {
    while ((*j < vlen) && (ra_svgp_ws((char)v[*j]) || (v[*j] == ','))) {
      ++(*j);
    }
    if (!priv_is_num_start(v, vlen, *j)) {
      break;
    }
    args[na] = ra_svgp_numf(v, vlen, j);
    ++na;
  }
  return na;
}
 ...
 * @c ra_svgp_starts_ci. Returns the first matching @c svg_xf_kind_t value, or
...
{
  if (ra_svgp_starts_ci(v, vlen, i, "translate(")) {
    return k_svg_xf_translate;
  }
  if (ra_svgp_starts_ci(v, vlen, i, "scale(")) {
    return k_svg_xf_scale;
  }
  if (ra_svgp_starts_ci(v, vlen, i, "rotate(")) {
    return k_svg_xf_rotate;
  }
  if (ra_svgp_starts_ci(v, vlen, i, "skewx(")) {
    return k_svg_xf_skewx;
  }
  if (ra_svgp_starts_ci(v, vlen, i, "skewy(")) {
    return k_svg_xf_skewy;
  }
  if (ra_svgp_starts_ci(v, vlen, i, "matrix(")) {
...
static svg_utf_t priv_parse_xform(const uint8_t* v, size_t vlen)
{
  ...
  while (i < vlen) {
    while ((i < vlen) && (ra_svgp_ws((char)v[i]) || (v[i] == ','))) {
      ++i;
    }
 ...
 * @c ra_svgp_attr. If absent, returns without modification. If found, parses
...
void ra_svgp_apply_xform(svg_xform_t* t, const uint8_t* tag, size_t tlen)
{
  size_t off = 0U;
  size_t vl  = 0U;
  if (!ra_svgp_attr(tag, tlen, "transform", &off, &vl)) {
    return;
  }
