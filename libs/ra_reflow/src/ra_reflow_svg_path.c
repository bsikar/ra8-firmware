 ...
 * the shared arc flatten in ra_reflow_svg_shape.c. The resulting vertices are
 ...
 * via @c ra_svgp_map_point, and appends the framebuffer-space point to
                               ...
                               int32_t*           n)
{
  ...
  for (int32_t j = 1; (j <= (int32_t)k_svg_curve_seg) && (*n < (int32_t)k_svg_poly_max); ++j) {
    ...
    ra_svgp_map_point(t, (int32_t)bx, (int32_t)by, &xs[*n], &ys[*n]);
    ++(*n);
  }
}
 ...
 * @c ra_svgp_map_point, and appends the framebuffer-space point to
 * @p xs[@p *n] / @p ys[@p *n], incrementing @p *n. Stops early when @p *n
 * reaches @c k_svg_poly_max to avoid buffer overflow.
 *
 * @param[in]     t   Active coordinate transform; must not be NULL.
 ...
 * parametrised and sampled via @c ra_svgp_flatten_arc. Returns -1 for any other
                               ...
                               int32_t            n)
{
  ...
  if (u == 'a') {
    ...
    int32_t        m     = n;
    ra_svgp_flatten_arc(t, p0, args, p_end, xs, ys, &m);
...
priv_parse_path(const uint8_t* d, size_t dlen, const svg_xform_t* t, int32_t* xs, int32_t* ys)
{
  ...
  while ((i < dlen) && (n < (int32_t)k_svg_poly_max)) {
    ...
    if (cn >= 0) {
      ...
      ra_svgp_map_point(t, st.cx, st.cy, &xs[n], &ys[n]);
      ++n;
 ...
 *
 * @details Resolves the fill (solid or gradient via @c ra_svgp_resolve_fill),
 * reads the 'd' attribute, parses it into framebuffer-space vertices via
 * @c priv_parse_path, and fills the resulting polygon with @c ra_svgp_fill_poly
 * or @c ra_svgp_fill_poly_grad. Returns without drawing when fill is absent/none
...
void ra_svgp_draw_path(const uint8_t* s, size_t len, const svg_xform_t* t)
{
  ...
  size_t         off  = 0U;
  size_t         vl   = 0U;
  if (((gi < 0) && (fill == (uint32_t)k_svg_no_paint)) || !ra_svgp_attr(s, len, "d", &off, &vl)) {
    return;
  }
