/**
 * @file ra_reflow_cache.c
 * @brief Import-time pagination cache serialiser / loader for `ra_reflow`.
 *
 * @details
 * Implements `ra_reflow_cache.h`: turn a laid-out engine's `glyphs[]` +
 * `pages[]` into a keyed little-endian byte blob and back, so the
 * expensive `ra_reflow_layout_chapter()` pass can be skipped when the
 ...
 * against the engine's static pools (`k_ra_reflow_max_glyphs` /
 * `k_ra_reflow_max_pages`) and the blob length, so a malformed blob can
 ...
 * @since 0.1.0
 */
static void priv_put_glyph(uint8_t* buf, size_t* off, const ra_reflow_glyph_t* glyph)
{
 ...
 * @since 0.1.0
 */
static void priv_get_glyph(const uint8_t* buf, size_t* off, ra_reflow_glyph_t* glyph)
{
 ...
 * @since 0.1.0
 */
static void priv_put_page(uint8_t* buf, size_t* off, const ra_reflow_page_t* page)
{
 ...
 * @since 0.1.0
 */
static void priv_get_page(const uint8_t* buf, size_t* off, ra_reflow_page_t* page)
{
 ...
 * @since 0.1.0
 */
static uint32_t priv_font_hash(const ra_reflow_t* engine)
{
 ...
 * @pre  `buf` has at least `k_ra_reflow_cache_header_bytes` capacity.
 * @post Header bytes `[0, k_ra_reflow_cache_header_bytes)` are written.
 * @post The body-checksum field holds a zero placeholder.
 * @note Pairs with priv_get_header().
 * @since 0.1.0
 */
static void
priv_put_header(uint8_t* buf, const ra_reflow_t* engine, size_t content_len, uint32_t content_hash)
{
  size_t off = 0U;
  priv_put_u32(buf, &off, (uint32_t)k_ra_reflow_cache_magic);
  priv_put_u16(buf, &off, (uint16_t)k_ra_reflow_cache_version);
 ...
 * @pre  `buf` has at least `k_ra_reflow_cache_header_bytes` readable.
 ...
 * @retval value `>= k_ra_reflow_cache_header_bytes`.
 *
...
{
  return (size_t)k_ra_reflow_cache_header_bytes +
         ((size_t)glyph_count * (size_t)k_ra_reflow_cache_glyph_bytes) +
         ((size_t)page_count * (size_t)k_ra_reflow_cache_page_bytes);
}
 ...
 * @since 0.1.0
 */
static bool priv_key_matches(const ra_reflow_t*      engine,
 ...
 * engine's static pools (`k_ra_reflow_max_glyphs` /
 * `k_ra_reflow_max_pages`), then computes the expected total blob size
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok                Header valid; `*need` set.
 * @retval k_ra_err_not_found     Magic mismatch (not a cache blob).
 * @retval k_ra_err_invalid_state Format version mismatch.
 * @retval k_ra_err_invalid_size  Short header, record counts beyond the
 *                                engine pools, or `len` below the size.
 *
 * @pre  `buf`, `key` and `need` are non-NULL.
 * @pre  `len` is the readable length of `buf`.
 * @post On `k_ra_ok`, `*key` is fully populated and `*need <= len`.
 * @post On any failure the return code names the first failed check.
 *
 * @note Internal helper for `ra_reflow_cache_load()`; depends only on its
...
{
  if (len < (size_t)k_ra_reflow_cache_header_bytes) {
    return k_ra_err_invalid_size;
  }
  priv_get_header(buf, key);
  if (key->magic != (uint32_t)k_ra_reflow_cache_magic) {
    return k_ra_err_not_found;
  }
  if (key->version != (uint32_t)k_ra_reflow_cache_version) {
    return k_ra_err_invalid_state;
  }
  if (key->glyph_count > (uint32_t)k_ra_reflow_max_glyphs) {
    return k_ra_err_invalid_size;
  }
  if (key->page_count > (uint32_t)k_ra_reflow_max_pages) {
    return k_ra_err_invalid_size;
  }
...
[[nodiscard]] ra_err_t ra_reflow_cache_size(const ra_reflow_t* engine, size_t* out_bytes)
{
                                                 ...
                                                 size_t*            out_len)
{
  ...
  size_t off = (size_t)k_ra_reflow_cache_header_bytes;
  ...
  const uint32_t checksum = priv_fnv1a(&out_buf[(size_t)k_ra_reflow_cache_header_bytes],
                                       need - (size_t)k_ra_reflow_cache_header_bytes);
...
[[nodiscard]] ra_err_t ra_reflow_cache_load(ra_reflow_t*   engine,
                                            const uint8_t* content,
                                            size_t         content_len,
                                            const uint8_t* buf,
                                            size_t         len)
{
  ...
  size_t           need = 0U;
  const ra_err_t   perr = priv_parse_header(buf, len, &key, &need);
  if (perr != k_ra_ok) {
    return perr;
  }
  ...
  const uint32_t checksum = priv_fnv1a(&buf[(size_t)k_ra_reflow_cache_header_bytes],
                                       need - (size_t)k_ra_reflow_cache_header_bytes);
