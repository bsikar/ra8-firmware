...
static ra_err_t priv_handle_cdata(tok_ctx_t* ctx, const uint8_t* buf, size_t* pi, size_t len)
{
  ...
  if ((ctx->sp > 0U) && !priv_suppressed(ctx)) {
    ...
    for (size_t k = inner; k < close; ++k) {
      if (!ra_reflow_tok_feed(ctx->engine, (char)buf[k], &last_ws)) {
        return k_ra_err_no_mem;
      }
    }
    const uint32_t tlen = ctx->engine->text_pool_used - off;
    if ((tlen > 0U) && !ra_reflow_tok_emit(ctx->engine,
...
static ra_err_t priv_handle_end(tok_ctx_t* ctx, const uint8_t* buf, size_t* pi, size_t len)
{
  ...
  *pi = (i < len) ? (i + 1U) : len;
  ...
  if (ra_reflow_tok_is_block(tag) && !priv_suppressed(ctx) &&
      !ra_reflow_tok_emit(ctx->engine, k_ra_reflow_tok_block_end, tag, ctx->style, 0U, 0U)) {
    return k_ra_err_no_mem;
  }
                                ...
                                uint16_t              css_font_px)
{
  ...
  if (block) {
    uint32_t id_off = 0U;
    uint32_t id_len = 0U;
    ra_reflow_tok_capture_attr(ctx->engine, tag, span, "id", sizeof("id") - 1U, &id_off, &id_len);
    if (!ra_reflow_tok_emit(ctx->engine,
                            k_ra_reflow_tok_block_start,
                            kind,
                            ctx->style,
                            id_off,
                            id_len)) {
  ...
  if (kind == k_ra_reflow_tag_a) {
    ...
    ra_reflow_tok_capture_attr(ctx->engine,
                               tag,
                               span,
                               "href",
                               sizeof("href") - 1U,
                               &href_off,
                               &href_len);
    ctx->active_link = ra_reflow_tok_intern_link(ctx->engine, href_off, href_len);
  }
  return k_ra_ok;
}
...
static void priv_handle_link(tok_ctx_t* ctx, const uint8_t* buf, size_t tag_lt, size_t end)
{
  ...
  size_t         href_off = 0U;
  size_t         href_len = 0U;
  if (!ra_reflow_tok_find_attr(tagbuf, span, "rel", sizeof("rel") - 1U, &rel_off, &rel_len) ||
      !ra_reflow_tok_find_attr(tagbuf, span, "href", sizeof("href") - 1U, &href_off, &href_len) ||
      !ra_reflow_tok_rel_is_stylesheet(&tagbuf[rel_off], rel_len)) {
    return;
  }
                             ...
                             ra_err_t*            out_err)
{
  ...
  if (tag == k_ra_reflow_tag_br) {
    *out_err = ra_reflow_tok_emit(ctx->engine, k_ra_reflow_tok_break, tag, ctx->style, 0U, 0U)
                 ? k_ra_ok
                 : k_ra_err_no_mem;
    return true;
  }
  if (tag == k_ra_reflow_tag_hr) {
    *out_err = ra_reflow_tok_emit(ctx->engine, k_ra_reflow_tok_rule, tag, ctx->style, 0U, 0U)
                 ? k_ra_ok
                 : k_ra_err_no_mem;
    return true;
  }
  if (tag == k_ra_reflow_tag_img) {
    ...
    ra_reflow_tok_capture_attr(ctx->engine,
                               ...
                               &src_len);
    *out_err =
      ra_reflow_tok_emit(ctx->engine, k_ra_reflow_tok_image, tag, ctx->style, src_off, src_len)
        ? k_ra_ok
        : k_ra_err_no_mem;
    return true;
  }
...
static ra_err_t priv_open_styled(tok_ctx_t*           ctx,
                                 ...
                                 bool                 block)
{
  const uint8_t          base = (uint8_t)(ctx->style | ra_reflow_tok_style_for(tag));
  ...
  const ra_css_style_t inl = ra_reflow_tok_css_inline(tagbuf, span);
  ...
  ctx->face_slot = ra_reflow_tok_resolve_face_slot(ctx->engine, &comp);
  return k_ra_ok;
}
...
static ra_err_t priv_handle_start(tok_ctx_t* ctx, const uint8_t* buf, size_t* pi, size_t len)
{
  ...
  const bool block = ra_reflow_tok_is_block(tag);
  if (selfclose) {
    if (block && !priv_suppressed(ctx)) {
      if (!ra_reflow_tok_emit(ctx->engine, k_ra_reflow_tok_block_start, tag, ctx->style, 0U, 0U)) {
        return k_ra_err_no_mem;
      }
      if (!ra_reflow_tok_emit(ctx->engine, k_ra_reflow_tok_block_end, tag, ctx->style, 0U, 0U)) {
        return k_ra_err_no_mem;
      }
    }
    return k_ra_ok;
  ...
  ctx->stack_el[ctx->sp]      = ra_reflow_tok_css_element(tag, &buf[tag_lt], span);
...
{
  if (!ra_reflow_tok_starts_with(buf, i, len, name)) {
    return false;
  }
...
{
  ctx->saw_element      = true;
  const size_t open_end = ra_reflow_tok_skip_past(buf, *pi, len, ">");
  const size_t close_at = ra_reflow_tok_find_lit(buf, open_end, len, close_lit);
...
{
  if (ra_reflow_tok_starts_with(buf, *pi, len, "<!--")) {
    *pi = ra_reflow_tok_skip_past(buf, *pi + 4U, len, "-->");
    return k_ra_ok;
  }
  if (ra_reflow_tok_starts_with(buf, *pi, len, "<![CDATA[")) {
    return priv_handle_cdata(ctx, buf, pi, len);
  }
  if (ra_reflow_tok_starts_with(buf, *pi, len, "<?")) {
    *pi = ra_reflow_tok_skip_past(buf, *pi + 2U, len, "?>");
    return k_ra_ok;
  }
  if (ra_reflow_tok_starts_with(buf, *pi, len, "<!")) {
    *pi = ra_reflow_tok_skip_past(buf, *pi + 2U, len, ">");
...
static bool priv_emit_text_run(tok_ctx_t* ctx, const uint8_t* buf, size_t run, size_t end)
{
  ...
  uint32_t off = 0U;
  uint32_t tln = 0U;
  if (!ra_reflow_tok_stash_run(ctx->engine, buf, run, end, &off, &tln)) {
    return false;
  }
  if (tln == 0U) {
    return true;
