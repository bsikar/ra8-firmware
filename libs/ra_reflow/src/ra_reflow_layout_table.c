/**
 * @file ra_reflow_layout_table.c
 * @brief `<table>` equal-column grid layout for the ra_reflow engine (#107).
 *
 * @details
 * Splits the table-layout sub-responsibility out of `ra_reflow_layout.c` so
 ...
 * overflow).
 *
 * The driver enters this module through @ref ra_reflow_layout_table; the
 * core inline-flow helpers it reuses (line height, glyph advance/push, line
 * wrap, page flush) are shared via `ra_reflow_layout_internal.h`.
 *
 ...
 * `ra_reflow_layout_table` to locate row boundaries while scanning the flat
 ...
 * `(*lines)++`. Glyphs are pushed via @ref ra_reflow_layout_push_glyph with
...
static void priv_cell_text(ra_reflow_t*             engine,
                           ...
                           uint32_t*                lines)
{
  const int32_t  line_h = (int32_t)ra_reflow_layout_line_height(font_px);
  ...
  while (i < len) {
    ...
    while ((word_end < len) && (base[word_end] != ' ')) {
      word_w += ra_reflow_layout_glyph_advance(font, font_px, (int32_t)base[word_end]);
      ++word_end;
    }
    ...
    while (i < word_end) {
      const int32_t adv = ra_reflow_layout_glyph_advance(font, font_px, (int32_t)base[i]);
      (void)ra_reflow_layout_push_glyph(engine, *cx, *cy, (int32_t)base[i], font_px, 0U, color, 0U);
      *cx += adv;
      ++i;
    }
  }
 ...
 * flush is issued via @ref ra_reflow_layout_finish_page, and the row is re-laid
                                ...
                                int32_t               col_w)
{
  ...
  const int32_t  line_h       = (int32_t)ra_reflow_layout_line_height(engine->font_px);
  ...
  if (((cur->y + row_h) > bottom_limit) && (cur->y > (int32_t)k_ra_reflow_margin_px)) {
    ...
    (void)ra_reflow_layout_finish_page(engine, cur); /* ... flush the page (cur->y -> margin) ... */
