 ...
 * stages live in the sibling ra_reflow_svg_*.c files. No DOM, no heap. See
 * ra_reflow_svg.h for the public scope and ra_reflow_svg_internal.h for the
 ...
 * both sides through @c ra_svgp_lc. Returns false immediately when the remaining
...
bool ra_svgp_starts_ci(const uint8_t* s, size_t len, size_t at, const char* lit)
{
  ...
  for (size_t k = 0U; k < n; ++k) {
    if (ra_svgp_lc((char)s[at + k]) != ra_svgp_lc(lit[k])) {
      return false;
    }
  }
  return true;
 ...
 * @c ra_svgp_starts_ci returns true for @p lit. If @p lit is empty (zero-length),
...
size_t ra_svgp_find_ci(const uint8_t* s, size_t len, size_t from, const char* lit)
{
  ...
  while ((i + n) <= len) {
    if (ra_svgp_starts_ci(s, len, i, lit)) {
      return i;
    }
    ++i;
  }
 ...
 * performed after ASCII-folding via @c ra_svgp_lc so both cases are handled
...
{
  while ((*i < len) && (ra_svgp_ws((char)s[*i]) || (s[*i] == ','))) {
    ++(*i);
  }
  int32_t sign = 1;
  if ((*i < len) && ((s[*i] == '-') || (s[*i] == '+'))) {
 ...
 *
 * @details Uses @c ra_svgp_starts_ci to confirm the case-insensitive match of @p name
...
static bool priv_attr_at(const uint8_t* s, size_t len, size_t at, const char* name)
{
  ...
  if ((at > 0U) && !ra_svgp_ws((char)s[at - 1U])) {
  ...
  return (s[after] == '=') || ra_svgp_ws((char)s[after]);
}
...
bool ra_svgp_attr(const uint8_t* s, size_t len, const char* name, size_t* voff, size_t* vlen)
{
  ...
  for (size_t i = 0U; i < len; ++i) {
    ...
    ++j; /* past '=' */
    while ((j < len) && ra_svgp_ws((char)s[j])) {
      ++j;
    }
    if ((j >= len) || ((s[j] != '"') && (s[j] != '\''))) {
      return false;
 ...
 * that slice to @c ra_svgp_num starting at offset zero. If the attribute is absent,
 * returns @p def without modifying any output. Fractional parts of the attribute
 * value are truncated by @c ra_svgp_num. Useful for reading integer-valued SVG
 ...
 * comparing each pair of characters after folding through @c ra_svgp_lc. The loop
...
static bool priv_ci_eq(const uint8_t* s, size_t len, const char* lit)
{
  size_t k = 0U;
  for (; (k < len) && (lit[k] != '\0'); ++k) {
    if (ra_svgp_lc((char)s[k]) != ra_svgp_lc(lit[k])) {
      return false;
    }
  }
 ...
 *
 * @details Calls @c ra_svgp_attr to locate the attribute value slice, then
 * forwards that slice to @c ra_svgp_paint. If the attribute is absent, returns
...
ra_svgp_resolve_fill(const uint8_t* s, size_t len, const svg_xform_t* t, uint32_t def, int32_t* gi)
{
  *gi        = -1;
  size_t off = 0U;
  size_t vl  = 0U;
  if (!ra_svgp_attr(s, len, "fill", &off, &vl)) {
    return def;
  }
  if (ra_svgp_starts_ci(&s[off], vl, 0U, "url(#")) {
