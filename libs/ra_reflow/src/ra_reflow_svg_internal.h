 ...
 *   - ra_reflow_svg.c        -- scan / attribute / colour-parsing core.
 *   - ra_reflow_svg_xform.c  -- viewBox + 2x3 affine `transform=` coordinate map.
 *   - ra_reflow_svg_shape.c  -- rect / circle / line / polygon / polyline draw,
 *                               the scanline polygon fill, gradient evaluation,
 *                               and the elliptical-arc flatten.
 *   - ra_reflow_svg_path.c   -- the `<path>` `d` mini-language (Bezier curves).
 *   - ra_reflow_svg_doc.c    -- the document walk, gradient definitions, and the
 *                               public ::ra_svg_render / ::ra_svg_size API.
 ...
 * module is drawing through the bound ra_gfx framebuffer.
 *
 ...
 * carriage return (0x0D), or form feed (0x0C). Defined in ra_reflow_svg.c.
 *
 ...
 * @since 0.1.0
 */
bool ra_svgp_starts_ci(const uint8_t* s, size_t len, size_t at, const char* lit);

/**
 * @brief Find the first case-insensitive occurrence of @p lit in @p s[from..len).
 *
 * @details Scans for the first position where @c ra_svgp_starts_ci matches; an
 * empty @p lit returns @p len. Defined in ra_reflow_svg.c.
 *
 * @param[in] s    Byte buffer to search; must not be NULL.
 ...
 * @since 0.1.0
 */
size_t ra_svgp_find_ci(const uint8_t* s, size_t len, size_t from, const char* lit);
 ...
 * decimal digits, and discards any fractional part. Defined in ra_reflow_svg.c.
 *
 ...
 * @since 0.1.0
 */
int32_t ra_svgp_num(const uint8_t* s, size_t len, size_t* i);
 ...
 * sets @p *voff / @p *vlen to the value content. Defined in ra_reflow_svg.c.
 *
 ...
 * @since 0.1.0
 */
bool ra_svgp_attr(const uint8_t* s, size_t len, const char* name, size_t* voff, size_t* vlen);

/**
 * @brief Read an SVG attribute as a signed integer, returning @p def when absent.
 *
 * @details Locates the value via @c ra_svgp_attr and parses it via @c ra_svgp_num.
 * Defined in ra_reflow_svg.c.
 *
 ...
 * @since 0.1.0
 */
int32_t ra_svgp_attr_num(const uint8_t* s, size_t len, const char* name, int32_t def);
 ...
 * no-paint sentinel for empty/"none"/unknown. Defined in ra_reflow_svg.c.
 *
 ...
 * @since 0.1.0
 */
uint32_t ra_svgp_paint(const uint8_t* s, size_t len);

/**
 * @brief Read an SVG attribute as a paint colour, returning @p def when absent.
 *
 * @details Locates the value via @c ra_svgp_attr and parses it via @c ra_svgp_paint.
 * Defined in ra_reflow_svg.c.
 *
 * @param[in] s    Byte buffer containing the SVG tag text; must not be NULL.
 * @param[in] len  Total valid bytes in @p s.
 ...
 * @since 0.1.0
 */
uint32_t ra_svgp_attr_paint(const uint8_t* s, size_t len, const char* name, uint32_t def);
 ...
 * solid colour or @p def. Defined in ra_reflow_svg.c.
 *
 * @param[in]  s    Byte buffer containing the SVG tag text; must not be NULL.
 * @param[in]  len  Total valid bytes in @p s.
 ...
 */
uint32_t
ra_svgp_resolve_fill(const uint8_t* s, size_t len, const svg_xform_t* t, uint32_t def, int32_t* gi);
 ...
 * @since 0.1.0
 */
float ra_svgp_numf(const uint8_t* s, size_t len, size_t* i);
 ...
 * @since 0.1.0
 */
bool ra_svgp_has_rot(const svg_xform_t* t);
 ...
 * @since 0.1.0
 */
void ra_svgp_map_point(const svg_xform_t* t, int32_t ux, int32_t uy, int32_t* fx, int32_t* fy);
 ...
 * Valid only on the axis-aligned fast path. Defined in ra_reflow_svg_xform.c.
 *
 * @param[in] t   Active coordinate transform; must not be NULL.
 ...
 * @since 0.1.0
 */
int32_t ra_svgp_mx(const svg_xform_t* t, int32_t sx);
 ...
 * @since 0.1.0
 */
int32_t ra_svgp_my(const svg_xform_t* t, int32_t sy);
 ...
 * axis-aligned fast path. Defined in ra_reflow_svg_xform.c.
 *
 * @param[in] t   Active coordinate transform; must not be NULL.
 ...
 * @since 0.1.0
 */
int32_t ra_svgp_sx(const svg_xform_t* t, int32_t sw);
 ...
 * user affine in @p t. Defined in ra_reflow_svg_xform.c.
 *
 ...
 * @since 0.1.0
 */
void ra_svgp_apply_xform(svg_xform_t* t, const uint8_t* tag, size_t tlen);

/**
 * @brief Even-odd scanline fill of polygon (xs,ys)[0..n) with a solid colour.
 *
 * @details Computes the bounding Y range and fills spans via ra_gfx. Defined in
 * ra_reflow_svg_shape.c.
 *
 ...
 * @since 0.1.0
 */
void ra_svgp_fill_poly(const int32_t* xs, const int32_t* ys, int32_t n, uint32_t color);
 ...
 * writes it with ra_gfx_pixel. Defined in ra_reflow_svg_shape.c.
 *
 * @param[in] xs Array of @p n framebuffer X coordinates; must not be NULL.
 * @param[in] ys Array of @p n framebuffer Y coordinates; must not be NULL.
 * @param[in] n  Number of polygon vertices.
 ...
 * Defined in ra_reflow_svg_shape.c.
 *
 * @param[in]     t      Active coordinate transform; must not be NULL.
 ...
 * @since 0.1.0
 */
void ra_svgp_flatten_arc(const svg_xform_t* t,
                         svg_pt_t           p0,
                         const int32_t*     args,
                         svg_pt_t           p_end,
                         int32_t*           xs,
 ...
 *
 * @details Axis-aligned solid rects use the ra_gfx_rect fast-path; rotated rects
 * are scanline-filled. Defined in ra_reflow_svg_shape.c.
 *
 ...
 * @since 0.1.0
 */
void ra_svgp_draw_rect(const uint8_t* s, size_t len, const svg_xform_t* t);

/**
 * @brief Draw one SVG 'circle' element using its fill colour or gradient.
 *
 * @details Axis-aligned solid circles use the ra_gfx_circle fast-path; rotated
 ...
 * @since 0.1.0
 */
void ra_svgp_draw_circle(const uint8_t* s, size_t len, const svg_xform_t* t);

/**
 * @brief Draw one SVG 'line' element using its stroke colour.
 *
 * @details Maps both endpoints through the affine and draws with ra_gfx_line;
 * 'fill' is ignored for lines. Defined in ra_reflow_svg_shape.c.
 *
 * @param[in] s   Byte span of the element tag; must not be NULL.
 * @param[in] len Total valid bytes in @p s (the tag span length).
 * @param[in] t   Active coordinate transform; must not be NULL.
 ...
 * @since 0.1.0
 */
void ra_svgp_draw_line(const uint8_t* s, size_t len, const svg_xform_t* t);
 ...
 * @since 0.1.0
 */
void ra_svgp_draw_polygon(const uint8_t* s, size_t len, const svg_xform_t* t);

/**
 * @brief Draw one SVG 'polyline' element as connected stroke line segments.
 *
 * @details Parses 'points', maps vertices, and connects them with ra_gfx_line.
 * Defined in ra_reflow_svg_shape.c.
 *
 * @param[in] s   Byte span of the element tag; must not be NULL.
 * @param[in] len Total valid bytes in @p s.
 ...
 * @since 0.1.0
 */
void ra_svgp_draw_polyline(const uint8_t* s, size_t len, const svg_xform_t* t);
 ...
 * (solid or gradient). Defined in ra_reflow_svg_path.c.
 *
 * @param[in] s   Byte span of the element tag; must not be NULL.
 * @param[in] len Total valid bytes in @p s.
 * @param[in] t   Active coordinate transform; must not be NULL.
 ...
 * @since 0.1.0
 */
void ra_svgp_draw_path(const uint8_t* s, size_t len, const svg_xform_t* t);
