...
/**
 * @enum ra_stbtt_alloc_consts_t
 * @brief Arena sizing knobs (no magic numbers).
 */
typedef enum : uint32_t {
  k_ra_stbtt_align       = 16U,         /**< Allocation alignment, bytes. */
  k_ra_stbtt_align_mask  = 15U,         /**< k_ra_stbtt_align - 1.        */
  k_ra_stbtt_arena_bytes = 96U * 1024U, /**< 3x the 32 KiB worst case.    */
} ra_stbtt_alloc_consts_t;
...
alignas(k_ra_stbtt_align) static uint8_t s_arena[k_ra_stbtt_arena_bytes];
...
void* ra_stbtt_malloc(size_t n)
{
  /* Reject requests too large to ever fit; this also prevents the
   * alignment round-up below from overflowing. */
  if (n > (size_t)k_ra_stbtt_arena_bytes) {
    return nullptr;
  }
  const size_t aligned = (n + (size_t)k_ra_stbtt_align_mask) & ~(size_t)k_ra_stbtt_align_mask;
  if (aligned > (size_t)k_ra_stbtt_arena_bytes - s_offset) {
    return nullptr;
  }
...
size_t ra_stbtt_alloc_high_water(void)
{
  return s_high_water;
}
