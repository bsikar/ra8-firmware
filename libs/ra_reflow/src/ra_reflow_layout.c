 ...
 * @retval true Caller must call ra_reflow_layout_newline before emitting.
...
/** @brief Implementation of `ra_reflow_layout_byte_zero()` -- bounded byte-walk. */
void ra_reflow_layout_byte_zero(uint8_t* dst, size_t n)
{
 ...
 * units and live in `ra_reflow_layout_internal.h`.
 * ===========================================================================
 */

/** @brief Implementation of `ra_reflow_layout_init_font()` -- parse the TTF blob. */
ra_err_t ra_reflow_layout_init_font(const ra_reflow_t* engine, stbtt_fontinfo* out_font)
{
...
/** @brief Implementation of `ra_reflow_layout_line_height()` -- scaled integer ratio. */
uint16_t ra_reflow_layout_line_height(uint16_t font_px)
{
...
/** @brief Implementation of `ra_reflow_layout_glyph_advance()` -- scaled hmetrics. */
int32_t ra_reflow_layout_glyph_advance(const stbtt_fontinfo* font, uint16_t font_px, int32_t cp)
{
...
/** @brief Implementation of `ra_reflow_layout_push_glyph()` -- append + bounds check. */
bool ra_reflow_layout_push_glyph(ra_reflow_t* engine,
                                 int32_t      x,
                                 int32_t      y,
                                 int32_t      cp,
                                 uint16_t     font_px,
...
/** @brief Implementation of `ra_reflow_layout_finish_page()` -- flush + reset cursor. */
bool ra_reflow_layout_finish_page(ra_reflow_t* engine, priv_cursor_t* cur)
{
...
/** @brief Implementation of `ra_reflow_layout_newline()` -- align, advance, page-break. */
bool ra_reflow_layout_newline(ra_reflow_t* engine, priv_cursor_t* cur, bool allow_justify)
{
 ...
 *
 * @details Measures the advance width of @p cp via @ref ra_reflow_layout_glyph_advance,
 ...
 * and the line already has content, @ref ra_reflow_layout_newline is called before the
 * glyph is pushed. The glyph is then appended via @ref ra_reflow_layout_push_glyph and
...
static ra_err_t priv_emit_char(ra_reflow_t*          engine,
                               ...
                               uint8_t               link_id)
{
  const int32_t advance = ra_reflow_layout_glyph_advance(font, cur->active_font_px, cp);
  ...
  if (ra_reflow_internal_right_overflow_break(cur->x,
    ...
    if (!ra_reflow_layout_newline(engine, cur, true)) { /* wrap -> previous line may justify */
      return k_ra_err_no_mem;
    }
  }
  if (!ra_reflow_layout_push_glyph(engine,
                                   cur->x,
                                   cur->y,
                                   cp,
                                   cur->active_font_px,
                                 ...
                                 const ra_reflow_token_t* tok)
{
  ...
  while (i < len) {
    ...
    while (word_end < len && base[word_end] != ' ') {
      word_w += ra_reflow_layout_glyph_advance(font, cur->active_font_px, (int32_t)base[word_end]);
      ++word_end;
    }
    const int32_t right_limit = (int32_t)engine->viewport_w - (int32_t)k_ra_reflow_margin_px;
    if (ra_reflow_internal_right_overflow_break(cur->x,
...
static bool priv_open_block(ra_reflow_t* engine, priv_cursor_t* cur, const ra_reflow_token_t* tok)
{
  if (cur->line_has_content != 0U) {
    if (!ra_reflow_layout_newline(engine, cur, false)) {
      return false;
    }
  }
  ...
  cur->line_height_px = ra_reflow_layout_line_height(cur->active_font_px);
 ...
 * (measured by `ra_reflow_layout_glyph_advance`), with a generous vertical band
                                ...
                                uint32_t              page)
{
  ...
  const int32_t x1 = glast->x + ra_reflow_layout_glyph_advance(font, glast->font_px, glast->cp);
...
/** @brief Implementation of `ra_reflow_layout_build_link_rects()` -- per-page link runs. */
void ra_reflow_layout_build_link_rects(ra_reflow_t* engine, const stbtt_fontinfo* font)
{
...
/** @brief Implementation of `ra_reflow_layout_apply_token()` -- per-token switch. */
ra_err_t ra_reflow_layout_apply_token(ra_reflow_t*             engine,
                                      priv_cursor_t*           cur,
                                      const stbtt_fontinfo*    font,
                                      const ra_reflow_token_t* tok)
{
  ...
  switch (tok->kind) {
    ...
    case k_ra_reflow_tok_break:
      return ra_reflow_layout_newline(engine, cur, false) ? k_ra_ok : k_ra_err_no_mem;
    case k_ra_reflow_tok_rule:
      return priv_apply_rule(engine, cur) ? k_ra_ok : k_ra_err_no_mem;
    case k_ra_reflow_tok_image:
      return ra_reflow_layout_apply_image(engine, cur, tok) ? k_ra_ok : k_ra_err_no_mem;
    default:
      return k_ra_ok;
  }
}
