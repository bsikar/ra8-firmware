 ...
 *
 * @details
 * Companion translation unit to ra_reflow_tokenize.c. It collects the leaf
 ...
 * cascade engine live in ra_reflow_tokenize.c and call the cross-TU helpers
 * promoted into ra_reflow_tokenize_internal.h. Pure forward-pass scanning --
 ...
 *
 * @details Encodes `cp` to 1-4 bytes and feeds each via ra_reflow_tok_feed().
 *
 * @param[in,out] engine  Engine whose text pool is appended to.
...
static bool priv_feed_utf8(ra_reflow_t* engine, uint32_t cp, bool* last_ws)
{
  ...
  for (size_t b = 0U; b < n; ++b) {
    if (!ra_reflow_tok_feed(engine, (char)enc[b], last_ws)) {
      return false;
    }
  }
  return true;
                           ...
                           size_t*        consumed)
{
  ...
  *consumed = 1U;
  return ra_reflow_tok_feed(engine, (char)buf[i], last_ws);
}
 ...
 *
 * @details Like ::ra_reflow_tok_skip_past but returns the literal's START offset, so the
