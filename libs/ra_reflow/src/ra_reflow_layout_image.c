/**
 * @file ra_reflow_layout_image.c
 * @brief Block-level `<img>` layout for the ra_reflow engine (#106).
 *
 * @details
 * Splits the image-layout sub-responsibility out of `ra_reflow_layout.c` so
 ...
 *
 * The driver reaches this module via @ref ra_reflow_layout_apply_image; the
 * core inline-flow helpers it reuses (line wrap, page flush) are shared through
 * `ra_reflow_layout_internal.h`.
 *
