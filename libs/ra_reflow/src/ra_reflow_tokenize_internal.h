 ...
 * (ra_reflow_tokenize.c, ra_reflow_tokenize_lex.c): the lexical primitives use
...
/* ===========================================================================
 * Cross-TU helpers shared between ra_reflow_tokenize.c and its companions
 * (ra_reflow_tokenize_lex.c / ra_reflow_tokenize_attr.c). Promoted from
 ...
 * introduce a block box. Defined in ra_reflow_tokenize_lex.c.
 *
 ...
 * @since 0.1.0
 */
bool ra_reflow_tok_is_block(ra_reflow_html_tag_t tag);
 ...
 * tag contributes nothing. Defined in ra_reflow_tokenize_lex.c.
 *
 * @param[in] tag Classified tag.
 ...
 * @since 0.1.0
 */
uint8_t ra_reflow_tok_style_for(ra_reflow_html_tag_t tag);

/**
 * @brief Append one token to the engine's token pool.
 *
 * @details Fills the next ra_reflow_token_t and advances token_count, or fails
 * if the fixed pool is full. Defined in ra_reflow_tokenize_lex.c.
 *
 ...
 * @since 0.1.0
 */
bool ra_reflow_tok_emit(ra_reflow_t*           engine,
 ...
 * Defined in ra_reflow_tokenize_lex.c.
 *
 ...
 * @since 0.1.0
 */
bool ra_reflow_tok_feed(ra_reflow_t* engine, char ch, bool* last_ws);
 ...
 * @since 0.1.0
 */
bool ra_reflow_tok_stash_run(ra_reflow_t*   engine,
                             const uint8_t* buf,
                             size_t         start,
                             size_t         end,
                             uint32_t*      out_off,
 ...
 * @since 0.1.0
 */
bool ra_reflow_tok_starts_with(const uint8_t* buf, size_t i, size_t len, const char* lit);
 ...
 * @since 0.1.0
 */
size_t ra_reflow_tok_skip_past(const uint8_t* buf, size_t i, size_t len, const char* lit);

/**
 * @brief Return the index of the first occurrence of `lit` (its start), or len.
 *
 * @details Like ::ra_reflow_tok_skip_past but returns the literal's START
 * offset. Defined in ra_reflow_tokenize_lex.c.
 *
 * @param[in] buf Source buffer.
 * @param[in] i   Offset to search from.
 * @param[in] len Total buffer length.
 ...
 * @since 0.1.0
 */
size_t ra_reflow_tok_find_lit(const uint8_t* buf, size_t i, size_t len, const char* lit);
 ...
 * @since 0.1.0
 */
bool ra_reflow_tok_find_attr(const uint8_t* tag,
                             size_t         tag_len,
                             const char*    name,
                             size_t         name_len,
                             size_t*        out_voff,
 ...
 * @since 0.1.0
 */
void ra_reflow_tok_capture_attr(ra_reflow_t*   engine,
                                const uint8_t* tag,
                                size_t         tag_len,
                                const char*    name,
                                size_t         name_len,
 ...
 * pointers alias @p tag. Defined in ra_reflow_tokenize_attr.c.
 *
 * @param[in] kind Classified tag.
 * @param[in] tag  Raw tag span starting at '<'.
 * @param[in] span Length of @p tag, bytes.
 * @return The element identity for ra_css_cascade().
 * @retval ra_css_element_t Populated struct; absent attributes have NULL pointer and zero length.
 ...
 */
ra_css_element_t
ra_reflow_tok_css_element(ra_reflow_html_tag_t kind, const uint8_t* tag, size_t span);

/**
 * @brief Parse a just-opened tag's inline `style="..."` into a CSS declaration.
 *
 * @details Locates the `style` attribute and delegates to
 * `ra_css_parse_inline()`. The returned struct has `set == 0` when no inline
 * declarations override the cascade. Defined in ra_reflow_tokenize_attr.c.
 *
 * @param[in] tag  Raw tag span starting at '<'.
 * @param[in] span Length of @p tag, bytes.
 ...
 * @since 0.1.0
 */
ra_css_style_t ra_reflow_tok_css_inline(const uint8_t* tag, size_t span);
 ...
 * empty href or a full table returns 0. Defined in ra_reflow_tokenize_attr.c.
 *
 ...
 * @since 0.1.0
 */
uint8_t ra_reflow_tok_intern_link(ra_reflow_t* engine, uint32_t href_off, uint32_t href_len);
 ...
 * @since 0.1.0
 */
bool ra_reflow_tok_rel_is_stylesheet(const uint8_t* rel, size_t len);
 ...
 * @since 0.1.0
 */
uint8_t ra_reflow_tok_resolve_face_slot(const ra_reflow_t* engine, const ra_css_style_t* comp);
