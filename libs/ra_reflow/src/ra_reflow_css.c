 ...
 * for the v1 CSS subset documented in `ra_reflow_css.h`. This translation unit
 ...
 * public inline-declaration entry point ::ra_css_parse_inline. No MMIO, no heap;
 * every buffer is a fixed-size field of the caller-owned ::ra_css_sheet_t.
 *
...
#include "ra_reflow.h" /* style/align enums, k_ra_css_font_* units */
 ...
 * @since 0.1.0
 */
bool ra_reflow_css_is_ws(char c)
{
  return (c == ' ') || (c == '\t') || (c == '\n') || (c == '\r') || (c == '\f');
}
 ...
 * @since 0.1.0
 */
char ra_reflow_css_lower(char c)
{
 ...
 * string @p lit using ASCII case folding via ra_reflow_css_lower(). Returns true only
 ...
 * @since 0.1.0
 */
bool ra_reflow_css_ci_eq(const char* s, size_t len, const char* lit)
{
  ...
  size_t k = 0U;
  for (; (k < len) && (lit[k] != '\0'); ++k) {
    if (ra_reflow_css_lower(s[k]) != ra_reflow_css_lower(lit[k])) {
      return false;
    }
  }
 ...
 * checks for a case-insensitive match at each position via ra_reflow_css_lower(). The
...
static bool priv_ci_contains(const char* s, size_t len, const char* sub)
{
  ...
  for (size_t i = 0U; (i + sl) <= len; ++i) {
    ...
    for (size_t j = 0U; j < sl; ++j) {
      if (ra_reflow_css_lower(s[i + j]) != ra_reflow_css_lower(sub[j])) {
        hit = false;
        break;
      }
    }
...
const char* ra_reflow_css_trim(const char* s, size_t* len)
{
  size_t a = 0U;
  size_t b = *len;
  while ((a < b) && ra_reflow_css_is_ws(s[a])) {
    ++a;
  }
  while ((b > a) && ra_reflow_css_is_ws(s[b - 1U])) {
    --b;
  }
  *len = b - a;
  return &s[a];
 ...
 * its numeric value (0-15). The digit is first lower-cased via ra_reflow_css_lower() so
...
static uint8_t priv_hex_val(char c)
{
  ...
  const char l = ra_reflow_css_lower(c);
  if ((l >= 'a') && (l <= 'f')) {
...
static uint32_t priv_parse_color(const char* s, size_t len)
{
  ...
  if (ra_reflow_css_ci_eq(s, len, "black")) {
    return (uint32_t)k_priv_col_black;
  }
  if (ra_reflow_css_ci_eq(s, len, "white")) {
    return (uint32_t)k_priv_col_white;
  }
  if (ra_reflow_css_ci_eq(s, len, "red")) {
    return (uint32_t)k_priv_col_red;
  }
  if (ra_reflow_css_ci_eq(s, len, "green")) {
    return (uint32_t)k_priv_col_green;
  }
  if (ra_reflow_css_ci_eq(s, len, "blue")) {
    return (uint32_t)k_priv_col_blue;
  }
  if (ra_reflow_css_ci_eq(s, len, "gray") || ra_reflow_css_ci_eq(s, len, "grey")) {
    return (uint32_t)k_priv_col_gray;
  }
  if (ra_reflow_css_ci_eq(s, len, "silver")) {
    return (uint32_t)k_priv_col_silver;
  }
  if (ra_reflow_css_ci_eq(s, len, "maroon")) {
    return (uint32_t)k_priv_col_maroon;
  }
  if (ra_reflow_css_ci_eq(s, len, "navy")) {
...
static bool priv_parse_fontsize(const char* s, size_t len, uint16_t* out_val, uint8_t* out_unit)
{
  ...
  if ((rem == (size_t)k_priv_fs_ulen) && ra_reflow_css_ci_eq(&s[i], rem, "px")) {
  ...
  if ((rem == (size_t)k_priv_fs_ulen) && ra_reflow_css_ci_eq(&s[i], rem, "em")) {
...
static bool priv_apply_emphasis(const char*     prop,
                                ...
                                ra_css_style_t* out)
{
  if (ra_reflow_css_ci_eq(prop, plen, "font-weight")) {
    const bool on =
      ra_reflow_css_ci_eq(val, vlen, "bold") || ra_reflow_css_ci_eq(val, vlen, "bolder") ||
      ra_reflow_css_ci_eq(val, vlen, "600") || ra_reflow_css_ci_eq(val, vlen, "700") ||
      ra_reflow_css_ci_eq(val, vlen, "800") || ra_reflow_css_ci_eq(val, vlen, "900");
    priv_set_emphasis(out, (uint8_t)k_ra_css_set_bold, (uint8_t)k_ra_reflow_style_bold, on);
    return true;
  }
  if (ra_reflow_css_ci_eq(prop, plen, "font-style")) {
    const bool on =
  ...
  if (ra_reflow_css_ci_eq(prop, plen, "text-decoration") ||
      ra_reflow_css_ci_eq(prop, plen, "text-decoration-line")) {
 ...
 * Comparison is case-insensitive via ra_reflow_css_ci_eq().
 *
...
static void priv_apply_align(const char* val, size_t vlen, ra_css_style_t* out)
{
  ...
  if (ra_reflow_css_ci_eq(val, vlen, "right")) {
    out->align = (uint8_t)k_ra_reflow_align_right;
  } else if (ra_reflow_css_ci_eq(val, vlen, "center")) {
    out->align = (uint8_t)k_ra_reflow_align_center;
  } else if (ra_reflow_css_ci_eq(val, vlen, "justify")) {
...
priv_apply_decl(const char* prop, size_t plen, const char* val, size_t vlen, ra_css_style_t* out)
{
  ...
  if (ra_reflow_css_ci_eq(prop, plen, "text-align")) {
    priv_apply_align(val, vlen, out);
  } else if (ra_reflow_css_ci_eq(prop, plen, "color")) {
  ...
  } else if (ra_reflow_css_ci_eq(prop, plen, "font-size")) {
    uint16_t fv = 0U;
    uint8_t  fu = 0U;
  ...
  } else if (ra_reflow_css_ci_eq(prop, plen, "display")) {
    out->set     = (uint8_t)(out->set | (uint8_t)k_ra_css_set_display);
    out->display = ra_reflow_css_ci_eq(val, vlen, "none") ? 1U : 0U;
  } else {
 ...
 * @since 0.1.0
 */
void ra_reflow_css_parse_decls(const char* s, size_t len, ra_css_style_t* out)
{
  size_t i = 0U;
  ...
  while (i < len) {
    ...
    if (colon < semi) {
      size_t      plen = colon - i;
      const char* prop = ra_reflow_css_trim(&s[i], &plen);
      size_t      vlen = semi - (colon + 1U);
      const char* val  = ra_reflow_css_trim(&s[colon + 1U], &vlen);
      if ((plen > 0U) && (vlen > 0U)) {
...
ra_err_t ra_css_parse_inline(const char* decls, uint32_t len, ra_css_style_t* out)
{
  ...
  *out = (ra_css_style_t){};
  ra_reflow_css_parse_decls(decls, (size_t)len, out);
  return k_ra_ok;
}
