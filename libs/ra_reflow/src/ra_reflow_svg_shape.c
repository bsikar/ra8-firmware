 ...
 * (shared with the `<path>` `A` command). Axis-aligned shapes keep the ra_gfx
 * fast path; rotated / sheared shapes are mapped through the affine and
 * scanline-filled. No DOM, no heap. See ra_reflow_svg_internal.h for the shared
 ...
 * through the full affine via @c ra_svgp_map_point and the resulting quad is
 * scanline-filled by @c ra_svgp_fill_poly or @c ra_svgp_fill_poly_grad.
...
{
  int32_t        gi   = -1;
  const uint32_t fill = ra_svgp_resolve_fill(s, len, t, (uint32_t)k_svg_def_fill, &gi);
  if ((gi < 0) && (fill == (uint32_t)k_svg_no_paint)) {
    return;
  }
  const int32_t rx = ra_svgp_attr_num(s, len, "x", 0);
  const int32_t ry = ra_svgp_attr_num(s, len, "y", 0);
  const int32_t rw = ra_svgp_attr_num(s, len, "width", 0);
  const int32_t rh = ra_svgp_attr_num(s, len, "height", 0);
  if ((gi < 0) && !ra_svgp_has_rot(t)) {
    (void)ra_gfx_rect(ra_svgp_mx(t, rx),
                      ra_svgp_my(t, ry),
                      ra_svgp_sx(t, rw),
                      ra_svgp_sx(t, rh),
                      fill,
  ...
  ra_svgp_map_point(t, rx, ry, &xs[0], &ys[0]);
  ra_svgp_map_point(t, rx + rw, ry, &xs[1], &ys[1]);
  ra_svgp_map_point(t, rx + rw, ry + rh, &xs[2], &ys[2]);
  ra_svgp_map_point(t, rx, ry + rh, &xs[3], &ys[3]);
  if (gi >= 0) {
    ra_svgp_fill_poly_grad(xs, ys, (int32_t)k_svg_rect_pts, &t->grads->g[gi]);
  } else {
    ra_svgp_fill_poly(xs, ys, (int32_t)k_svg_rect_pts, fill);
  }
}
 ...
 * by @c ra_svgp_fill_poly or @c ra_svgp_fill_poly_grad. Returns without drawing
...
void ra_svgp_draw_circle(const uint8_t* s, size_t len, const svg_xform_t* t)
{
  ...
  const int32_t cx = ra_svgp_attr_num(s, len, "cx", 0);
  const int32_t cy = ra_svgp_attr_num(s, len, "cy", 0);
  const int32_t r  = ra_svgp_attr_num(s, len, "r", 0);
  if ((gi < 0) && !ra_svgp_has_rot(t)) {
    (void)ra_gfx_circle(ra_svgp_mx(t, cx), ra_svgp_my(t, cy), ra_svgp_sx(t, r), fill, true);
    return;
  }
  ...
  if (gi >= 0) {
    ra_svgp_fill_poly_grad(xs, ys, (int32_t)k_svg_circle_seg, &t->grads->g[gi]);
  } else {
    ra_svgp_fill_poly(xs, ys, (int32_t)k_svg_circle_seg, fill);
  }
}
 ...
 * via @c ra_svgp_map_point and the result is forwarded to @c ra_gfx_line.
...
{
  const uint32_t stroke = ra_svgp_attr_paint(s, len, "stroke", (uint32_t)k_svg_no_paint);
  if (stroke == (uint32_t)k_svg_no_paint) {
    return;
  }
  const int32_t x1  = ra_svgp_attr_num(s, len, "x1", 0);
  const int32_t y1  = ra_svgp_attr_num(s, len, "y1", 0);
  const int32_t x2  = ra_svgp_attr_num(s, len, "x2", 0);
  const int32_t y2  = ra_svgp_attr_num(s, len, "y2", 0);
  int32_t       fx1 = 0;
  int32_t       fy1 = 0;
  int32_t       fx2 = 0;
  int32_t       fy2 = 0;
  ra_svgp_map_point(t, x1, y1, &fx1, &fy1);
  ra_svgp_map_point(t, x2, y2, &fx2, &fy2);
 ...
 * pairs via @c ra_svgp_num, maps each pair through the full affine via
 * @c ra_svgp_map_point, and stores the result in @p xs[@p n] / @p ys[@p n].
...
priv_parse_points(const uint8_t* v, size_t vlen, const svg_xform_t* t, int32_t* xs, int32_t* ys)
{
  ...
  while (n < (int32_t)k_svg_poly_max) {
    ...
    const int32_t ux = ra_svgp_num(v, vlen, &k);
    const int32_t uy = ra_svgp_num(v, vlen, &k);
    ra_svgp_map_point(t, ux, uy, &xs[n], &ys[n]);
    ++n;
  }
  return n;
}
 ...
 * the resulting polygon with @c ra_svgp_fill_poly or @c ra_svgp_fill_poly_grad.
...
void ra_svgp_draw_polygon(const uint8_t* s, size_t len, const svg_xform_t* t)
{
  ...
  size_t         vl   = 0U;
  if (((gi < 0) && (fill == (uint32_t)k_svg_no_paint)) ||
      !ra_svgp_attr(s, len, "points", &off, &vl)) {
    return;
  }
  ...
  if (gi >= 0) {
    ra_svgp_fill_poly_grad(xs, ys, n, &t->grads->g[gi]);
  } else {
    ra_svgp_fill_poly(xs, ys, n, fill);
  }
}
...
void ra_svgp_draw_polyline(const uint8_t* s, size_t len, const svg_xform_t* t)
{
  ...
  size_t         off    = 0U;
  size_t         vl     = 0U;
  if ((stroke == (uint32_t)k_svg_no_paint) || !ra_svgp_attr(s, len, "points", &off, &vl)) {
    return;
  }
  int32_t       xs[k_svg_poly_max] = {};
  int32_t       ys[k_svg_poly_max] = {};
                         ...
                         int32_t*           n)
{
  ...
  if (!a.ok) {
    if (*n < (int32_t)k_svg_poly_max) {
      ra_svgp_map_point(t, p_end.x, p_end.y, &xs[*n], &ys[*n]);
      ++(*n);
    }
    return;
  }
  ...
  for (int32_t j = 1; (j <= segs) && (*n < (int32_t)k_svg_poly_max); ++j) {
    ...
    ra_svgp_map_point(t, (int32_t)px, (int32_t)py, &xs[*n], &ys[*n]);
    ++(*n);
  }
}
