/**
 * @file ra_reflow_tokenize_attr.c
 ...
 *
 * @details
 * Companion translation unit to ra_reflow_tokenize.c. It groups the routines
 * that read a raw `<...>` start-tag span: locating a named quoted attribute,
 * copying its value into the engine text pool, building the
 * `ra_css_element_t` identity (tag + id + class) the cascade matches against,
 ...
 * ra_reflow_tokenize.c call the cross-TU helpers promoted into
 * ra_reflow_tokenize_internal.h. Pure forward-pass scanning -- no recursion
 ...
 *
 * @details The non-copying core of ra_reflow_tok_capture_attr(): returns the value's
                                ...
                                uint32_t*      out_len)
{
  ...
  *out_len      = 0U;
  size_t vstart = 0U;
  size_t vlen   = 0U;
  if (!ra_reflow_tok_find_attr(tag, tag_len, name, name_len, &vstart, &vlen)) {
    return;
  }
  if ((vlen == 0U) ||
...
ra_reflow_tok_css_element(ra_reflow_html_tag_t kind, const uint8_t* tag, size_t span)
{
  ...
  size_t off          = 0U;
  size_t len          = 0U;
  if (ra_reflow_tok_find_attr(tag, span, "id", sizeof("id") - 1U, &off, &len)) {
    el.id     = (const char*)&tag[off];
    el.id_len = (uint16_t)len;
  }
  if (ra_reflow_tok_find_attr(tag, span, "class", sizeof("class") - 1U, &off, &len)) {
...
ra_css_style_t ra_reflow_tok_css_inline(const uint8_t* tag, size_t span)
{
  ...
  size_t         off = 0U;
  size_t         len = 0U;
  if (ra_reflow_tok_find_attr(tag, span, "style", sizeof("style") - 1U, &off, &len)) {
