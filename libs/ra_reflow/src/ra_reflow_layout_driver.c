/**
 * @file ra_reflow_layout_driver.c
 * @brief Token-stream driver + public lifecycle API for the ra_reflow engine.
 *
 * @details
 * Splits the outer layout driver and the public entry points out of
 * `ra_reflow_layout.c` so each translation unit stays under the project
 * file-size cap. The driver (`priv_layout_tokens`) walks the parsed token
 * stream once, dispatching `<table>` ranges to @ref ra_reflow_layout_table
 * and every other token to @ref ra_reflow_layout_apply_token, then flushes a
 * final page and builds the tappable link rectangles.
 *
 * The public API covers engine lifecycle (`ra_reflow_init`,
 * `ra_reflow_close`), loader binding (image / CSS), chapter dispatch
 * (`ra_reflow_layout_chapter`, `ra_reflow_set_font_size`), font (re)binding,
...
static ra_err_t priv_layout_tokens(ra_reflow_t* engine, const stbtt_fontinfo* font)
{
  priv_cursor_t cur = {
    ...
    .line_height_px   = ra_reflow_layout_line_height(engine->font_px),
  ...
  while (i < engine->token_count) {
    ...
    if ((tok->kind == (uint8_t)k_ra_reflow_tok_block_start) &&
      ...
      ra_err_t err = ra_reflow_layout_table(engine, &cur, font, i, &i);
      if (err != k_ra_ok) {
        return err;
      }
    } else {
      ra_err_t err = ra_reflow_layout_apply_token(engine, &cur, font, tok);
      if (err != k_ra_ok) {
        return err;
      }
      i++;
...
ra_err_t ra_reflow_run_layout(ra_reflow_t* engine)
{
  ...
  stbtt_fontinfo font;
  ra_err_t       err = ra_reflow_layout_init_font(engine, &font);
  if (err != k_ra_ok) {
    return err;
  }
  ...
  ra_reflow_layout_build_link_rects(engine, &font);
  return k_ra_ok;
}
                        ...
                        ra_reflow_t*   out_engine)
{
  ...
  ra_reflow_layout_byte_zero((uint8_t*)out_engine, sizeof(*out_engine));
