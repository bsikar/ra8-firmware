...
} ra_import_crc_const_t;

/**
 * @enum ra_import_misc_t
...
} ra_import_name_idx_t;

/** @brief Log tag for this module. @since Version 0.1.0 */
static const char* const s_tag = "ra_rabook_import";
 ...
 * @return Error code.
 * @retval k_ra_ok           Stream consumed to EOF.
 * @retval k_ra_err_null_ptr @p file or @p buf is NULL.
 * @retval k_ra_err_*        Read error.
...
RA_INTERNAL
static ra_err_t
s_crc_stream(ra_fs_file_t* file, uint8_t* buf, uint32_t cap, uint32_t* out_size, uint32_t* out_crc)
{
 ...
 *          @p mount, @p epub_path, and @p buf are delegated to `ra_fs_open` and
 ...
 * @param[out] out_crc   Receives the finalised CRC-32 cache key.
 * @return Error code.
 * @retval k_ra_ok            Key computed.
 * @retval k_ra_err_null_ptr  @p out_size or @p out_crc is NULL.
 * @retval k_ra_err_invalid_size `cap` is zero.
 * @retval k_ra_err_*         Open/read error (e.g. source missing).
 * @pre @p out_size and @p out_crc are non-NULL.
 * @pre @p cap > 0.
 ...
 */
RA_INTERNAL
static ra_err_t s_compute_source_key(ra_fs_mount_t* mount,
                                     ...
                                     uint32_t*      out_crc)
{
  ...
  RA_CHECK_NULL_PTR(out_crc, s_tag, "out_crc");
  if (cap == 0U) {
    return k_ra_err_invalid_size;
  }

  ra_fs_file_t* file = nullptr;
  ra_err_t      err  = ra_fs_open(mount, epub_path, k_ra_fs_mode_read, &file);
  if (err != k_ra_ok) {
    return err;
  }
 ...
 * @param[in]  cap  Capacity of @p out (>= @ref k_ra_rabook_import_name_cap).
 * @return Error code.
 * @retval k_ra_ok               Name written + NUL-terminated.
 * @retval k_ra_err_null_ptr     @p out is NULL.
 * @retval k_ra_err_invalid_size @p cap is too small.
 * @pre @p out is non-NULL.
 * @pre @p cap >= @ref k_ra_rabook_import_name_cap.
 ...
 */
RA_INTERNAL
static ra_err_t s_make_name(uint32_t crc, char ext0, char ext1, char ext2, char* out, uint32_t cap)
{
  RA_CHECK_NULL_PTR(out, s_tag, "out");
  if (cap < (uint32_t)k_ra_rabook_import_name_cap) {
    return k_ra_err_invalid_size;
  }
 ...
 * @param[out] cache_name Receives `XXXXXXXX.RBK` (cap @ref k_ra_rabook_import_name_cap).
 ...
 * @retval k_ra_ok           All three names written.
 * @retval k_ra_err_null_ptr A destination buffer is NULL.
 ...
 */
RA_INTERNAL
static ra_err_t s_derive_names(uint32_t crc, char* cache_name, char* tmp_name, char* mark_name)
{
  RA_CHECK_NULL_PTR(cache_name, s_tag, "cache_name");
  RA_CHECK_NULL_PTR(tmp_name, s_tag, "tmp_name");
  RA_CHECK_NULL_PTR(mark_name, s_tag, "mark_name");
  const uint32_t cap = (uint32_t)k_ra_rabook_import_name_cap;
  ra_err_t       err = s_make_name(crc, 'R', 'B', 'K', cache_name, cap);
  if (err != k_ra_ok) {
    return err;
  }
 ...
 */
RA_INTERNAL
static bool s_file_exists(ra_fs_mount_t* mount, const char* path)
{
  if (mount == nullptr) {
    return false;
  }
 ...
 * @return Error code.
 * @retval k_ra_ok               Stamp read in full.
 * @retval k_ra_err_null_ptr     A pointer argument is NULL.
 * @retval k_ra_err_invalid_size Marker shorter than a stamp.
 * @retval k_ra_err_*            Open/read error (e.g. marker absent).
 * @pre @p out is non-NULL.
 ...
 */
RA_INTERNAL
static ra_err_t s_read_stamp(ra_fs_mount_t* mount, const char* path, ra_rabook_import_stamp_t* out)
{
  ...
  uint32_t got  = 0U;
  err           = ra_fs_read(file, (uint8_t*)out, (uint32_t)sizeof(*out), &got);
  ra_err_t cerr = ra_fs_close(file);
  if (err != k_ra_ok) {
    return err;
  }
...
RA_INTERNAL
static bool
s_cache_is_fresh(ra_fs_mount_t* mount, const char* mark_path, const ra_rabook_import_stamp_t* want)
{
  if (mount == nullptr) {
    return false;
  }
  ...
  ra_rabook_import_stamp_t got = {};
  ra_err_t                 err = s_read_stamp(mount, mark_path, &got);
  if (err != k_ra_ok) {
    return false;
  }
 ...
 * @details Unlinks first because `ra_fs_write_file` will not overwrite.
 * @param[in] mount Mounted volume.
 * @param[in] path  Marker path (root-level 8.3).
 * @param[in] stamp Stamp to persist.
 * @return Error code from the write.
 * @retval k_ra_ok           Marker written.
 * @retval k_ra_err_null_ptr A pointer argument is NULL.
 * @retval k_ra_err_*        Filesystem write error.
...
RA_INTERNAL
static ra_err_t
s_write_stamp(ra_fs_mount_t* mount, const char* path, const ra_rabook_import_stamp_t* stamp)
{
  RA_CHECK_NULL_PTR(mount, s_tag, "mount");
  RA_CHECK_NULL_PTR(path, s_tag, "path");
  RA_CHECK_NULL_PTR(stamp, s_tag, "stamp");
  (void)ra_fs_unlink(mount, path);
  return ra_fs_write_file(mount, path, (const uint8_t*)stamp, (uint32_t)sizeof(*stamp));
}

/**
 * @brief Atomically replace @p dst_path with @p tmp_path (temp+rename).
 * @details Unlinks @p dst_path first because `ra_fs_rename` will not overwrite.
 * @param[in] mount    Mounted volume.
 * @param[in] tmp_path Existing temp file to promote.
 * @param[in] dst_path Final cache path.
 * @return Error code from the rename.
 * @retval k_ra_ok           Promoted.
 * @retval k_ra_err_null_ptr A pointer argument is NULL.
 * @retval k_ra_err_*        Filesystem rename error.
 ...
 */
RA_INTERNAL
static ra_err_t s_atomic_replace(ra_fs_mount_t* mount, const char* tmp_path, const char* dst_path)
{
  RA_CHECK_NULL_PTR(mount, s_tag, "mount");
  RA_CHECK_NULL_PTR(tmp_path, s_tag, "tmp_path");
  RA_CHECK_NULL_PTR(dst_path, s_tag, "dst_path");
  (void)ra_fs_unlink(mount, dst_path);
  return ra_fs_rename(mount, tmp_path, dst_path);
}
 ...
 * @return Error code.
 * @retval k_ra_ok               Copied incl. terminator.
 * @retval k_ra_err_null_ptr     A pointer argument is NULL.
 * @retval k_ra_err_invalid_size @p src did not terminate within @p cap.
 ...
 */
RA_INTERNAL
static ra_err_t s_copy_name(char* dst, uint32_t cap, const char* src)
{
 ...
 * @retval k_ra_ok    Cache + marker published.
 * @retval k_ra_err_* Propagated compiler or filesystem error.
 ...
 */
RA_INTERNAL
static ra_err_t s_compile_and_cache(const ra_rabook_import_cfg_t*   cfg,
                                    const char*                     epub_path,
                                    const char*                     cache_name,
                                    const char*                     tmp_name,
                                    const char*                     mark_name,
                                    const ra_rabook_import_stamp_t* want)
{
  RA_CHECK_NULL_PTR(cfg, s_tag, "cfg");
  RA_CHECK_NULL_PTR(cfg->compile, s_tag, "cfg->compile");
  RA_CHECK_NULL_PTR(want, s_tag, "want");
  (void)ra_fs_unlink(cfg->mount, tmp_name);
  ra_err_t err = cfg->compile(cfg->compile_ctx, cfg->mount, epub_path, tmp_name);
  if (err != k_ra_ok) {
    ra_log_error(s_tag, "compile seam failed");
    return err;
  }
 ...
 *          @ref k_ra_err_null_ptr without inflating this validator.
 ...
 * @return Error code.
 * @retval k_ra_ok               Arguments valid.
 * @retval k_ra_err_null_ptr     A required pointer is NULL.
 * @retval k_ra_err_invalid_size @p cache_path_cap is too small.
 * @pre Called first from @ref ra_rabook_import_open.
 * @pre @p cfg may be NULL (checked here before any dereference).
 * @post On `k_ra_ok` @p cfg and @p cfg->mount are safe to dereference.
 * @post On error the caller returns immediately without side effects.
 * @note Not thread-safe.
 * @since Version 0.1.0
 */
RA_INTERNAL
static ra_err_t s_validate_open_args(const ra_rabook_import_cfg_t*     cfg,
                                     const char*                       epub_path,
                                     const char*                       out_cache_path,
                                     uint32_t                          cache_path_cap,
                                     const ra_rabook_import_outcome_t* out_outcome)
{
  RA_CHECK_NULL_PTR(cfg, s_tag, "cfg");
  RA_CHECK_NULL_PTR(epub_path, s_tag, "epub_path");
  RA_CHECK_NULL_PTR(out_cache_path, s_tag, "out_cache_path");
  RA_CHECK_NULL_PTR(out_outcome, s_tag, "out_outcome");
  RA_CHECK_NULL_PTR(cfg->mount, s_tag, "cfg->mount");
  if (cache_path_cap < (uint32_t)k_ra_rabook_import_name_cap) {
    return k_ra_err_invalid_size;
  }
  return k_ra_ok;
}
...
ra_err_t ra_rabook_import_open(const ra_rabook_import_cfg_t* cfg,
                               const char*                   epub_path,
                               char*                         out_cache_path,
                               uint32_t                      cache_path_cap,
                               ra_rabook_import_outcome_t*   out_outcome)
{
  ra_err_t err = s_validate_open_args(cfg, epub_path, out_cache_path, cache_path_cap, out_outcome);
  if (err != k_ra_ok) {
    return err;
  }
  ...
  char cache_name[k_ra_rabook_import_name_cap];
  char tmp_name[k_ra_rabook_import_name_cap];
  char mark_name[k_ra_rabook_import_name_cap];
  ...
  const ra_rabook_import_stamp_t want = {
    .magic            = (uint32_t)k_ra_rabook_import_stamp_magic,
  ...
  if (s_cache_is_fresh(cfg->mount, mark_name, &want)) {
    if (s_file_exists(cfg->mount, cache_name)) {
      *out_outcome = k_ra_rabook_import_hit;
  ...
  *out_outcome = k_ra_rabook_import_compiled;
  return s_copy_name(out_cache_path, cache_path_cap, cache_name);
}
