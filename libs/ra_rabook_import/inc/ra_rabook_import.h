/**
 * @file ra_rabook_import.h
 * @brief On-import EPUB -> .rabook compile-and-cache manager (#151).
 *
 * @details
 * `ra_rabook_import` makes the e-reader a self-contained appliance: drop a raw
 ...
 * @ref ra_rabook_import_compile_fn function pointer, and the filesystem only
 * through @ref ra_fs_mount_t. The manager itself depends on nothing but
 * `ra_fs` and `ra_core`, so it is host-testable with a lightweight compile spy
 * (no EPUB parser, no image decoder, no 24 MiB arenas). Production wires the
 * real `ra_rabook_compile_from_epub` behind the seam -- see
 * @ref ra_rabook_import_compiler.h for the supplied adapter.
 *
 * @par Crash safety
 ...
 * `.rabook` body CRC-32 (checked by `ra_book_open`) is a second line of defence.
 * The source `.epub` is never modified or deleted.
 *
 * @note Not thread-safe: one import at a time on the single reader core.
 * @see ra_rabook_import_compiler.h  Production adapter onto the real compiler.
 * @see ra_fs.h                      Filesystem the cache lives on.
 * @since Version 0.1.0
 *
 ...
 *          reader's root-only `ra_fs_open()` can re-read them. The name is the
 *          8 uppercase-hex digits of the source CRC-32 plus a 3-char extension.
 * @since Version 0.1.0
 */
typedef enum : uint8_t {
  k_ra_rabook_import_key_hex_len = 8U,  /**< Hex digits of CRC-32 in the name.   */
  k_ra_rabook_import_name_cap    = 13U, /**< "XXXXXXXX.EXT" + NUL (8.3 + term).  */
  k_ra_rabook_import_stamp_bytes = 20U, /**< Pinned size of the freshness stamp. */
} ra_rabook_import_size_t;

/**
 * @enum ra_rabook_import_magic_t
...
typedef enum : uint32_t {
  k_ra_rabook_import_stamp_magic = 0x52424B31U, /**< Importer marker tag "RBK1". */
} ra_rabook_import_magic_t;

/**
 * @enum ra_rabook_import_outcome_t
 * @brief How @ref ra_rabook_import_open satisfied the request.
 * @since Version 0.1.0
 */
typedef enum : uint8_t {
  k_ra_rabook_import_hit      = 0U, /**< Fresh cache reused; the compiler was NOT invoked. */
  k_ra_rabook_import_compiled = 1U, /**< Miss/stale; the book was compiled and cached.     */
} ra_rabook_import_outcome_t;

/**
 * @struct ra_rabook_import_stamp_t
 ...
 *
 * @invariant `magic == k_ra_rabook_import_stamp_magic` for a valid marker.
 * @since Version 0.1.0
 */
typedef struct {
...
/**
 * @typedef ra_rabook_import_compile_fn
 ...
 *          `ra_rabook_compile_from_epub` (through the supplied adapter); host
 ...
 * @return `k_ra_ok` on success; any non-zero @ref ra_err_t aborts the import
 *         and the manager falls back without touching the cache.
 * @since Version 0.1.0
 */
typedef ra_err_t (*ra_rabook_import_compile_fn)(void*          compile_ctx,
                                                ra_fs_mount_t* mount,
                                                const char*    epub_path,
                                                const char*    out_path);

/**
 * @struct ra_rabook_import_cfg_t
 * @brief Injected dependencies + versioning for @ref ra_rabook_import_open.
 *
 ...
 *          stamps that gate reuse -- pass `k_ra_book_format_version` and the
 ...
 */
typedef struct {
  ra_fs_mount_t*              mount;            /**< Mounted FAT volume (cache + source). */
  ra_rabook_import_compile_fn compile;          /**< Compile-on-miss seam (non-NULL).     */
 ...
 *                            (8.3) the caller then opens with `ra_book_open`.
 * @param[in]  cache_path_cap Capacity of @p out_cache_path in bytes
 *                            (>= @ref k_ra_rabook_import_name_cap).
 * @param[out] out_outcome    Receives @ref k_ra_rabook_import_hit or
 *                            @ref k_ra_rabook_import_compiled on success.
 *
 * @return Error code.
 * @retval k_ra_ok            Cache is fresh (hit) or freshly written (compiled).
 * @retval k_ra_err_null_ptr  A required pointer (incl. an injected dependency)
 *                            is NULL.
 * @retval k_ra_err_invalid_size `cache_path_cap` is too small, or `scratch_cap`
 *                            is zero.
 * @retval k_ra_err_not_found The source `.epub` does not exist.
 * @retval k_ra_err_*         Propagated filesystem or compiler error (on a
 ...
 * @post On `k_ra_ok`, `out_cache_path` is a NUL-terminated 8.3 path to a present
 ...
 * @see ra_rabook_import_compiler.h
 * @since Version 0.1.0
 */
[[nodiscard]] ra_err_t ra_rabook_import_open(const ra_rabook_import_cfg_t* cfg,
                                             const char*                   epub_path,
                                             char*                         out_cache_path,
