...
//!
//! Build graph for the Zig implementation of `ra8_net_pal`. CMake consumes
//! the installed static library through the unchanged `inc/ra8_net_pal.h` C
//! ABI; the `test` step covers the frame ring and the ABI membrane.
//!
//! No build options: the Ring-3 `ra8_eth` driver is a link-time seam on both
...
pub fn build(b: *std.Build) void {
    ...
    const library_module = b.createModule(.{
        .root_source_file = b.path("src/ra8_net_pal_abi.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const test_step = b.step("test", "Run Zig ra8_net_pal tests");
