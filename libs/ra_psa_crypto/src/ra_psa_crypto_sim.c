/**
 * @file ra_psa_crypto_sim.c
 * @brief Simulator-mode crypto primitives for the ``ra_psa_crypto`` facade.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 * @details
 * Companion translation unit to ``ra_psa_crypto.c``. When the host
 * unit-test build defines ``RA_SIMULATOR_MODE`` the heavy TF-PSA-Crypto
 * object library is not linked; this file supplies tiny in-memory crypto
 * stand-ins so the public ``ra_psa_*`` surface still behaves:
 *
 ...
 * guarded by ``RA_SIMULATOR_MODE`` and never runs on the target.
 *
                                  ...
                                  uint8_t        out_tag[k_ra_psa_gcm_tag_len])
{
  ...
  ra_psa_sim_sha256_oneshot(buf, off, digest);
                                   ...
                                   size_t         len)
{
  ...
  while (produced < len) {
    ...
    ra_psa_sim_sha256_oneshot(seed, off + (size_t)k_ra_psa_bytes_per_word, block);
