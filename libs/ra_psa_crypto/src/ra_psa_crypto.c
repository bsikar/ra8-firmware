...
#include "ra_err.h"
#include "ra_log.h"
#include "ra_psa_crypto_internal.h"
 ...
 * ``struct ra_psa_key_handle`` definition live in
 * ``ra_psa_crypto_internal.h`` so the simulator TU can share them.
 */
...
/* Translate ``ra_psa_key_type_t`` to its ``PSA_KEY_TYPE_*`` peer -- see implementation for details. */
...
/* Translate ``ra_psa_alg_t`` to its ``PSA_ALG_*`` peer -- see implementation for details. */
...
/* Translate ``ra_psa_key_usage_t`` bitmask to ``PSA_KEY_USAGE_*`` flags -- see implementation for details. */
...
/* Run the PSA-side import + slot wire-up for ``ra_psa_key_import`` -- see implementation for details. */
                             ...
                             size_t*        out_len)
{
  ...
  ra_psa_sim_sha256_oneshot(input, input_len, out);
#else
  size_t             produced = 0U;
                          ...
                          size_t*        sig_len)
{
  ...
  uint8_t digest[32];
  ra_psa_sim_sha256_oneshot(buf, handle->key_len + hash_len, digest);
...
/* Shared precondition checks for ``ra_psa_aead_encrypt`` -- see implementation for details. */
                             ...
                             size_t*        out_len)
{
  ...
  const ra_err_t ser =
    ra_psa_sim_aead_encrypt(handle, nonce, nonce_len, aad, aad_len, plain, plain_len, out, out_len);
  if (ser != k_ra_ok) {
    return ser;
  }
#else
...
/* Shared precondition checks for ``ra_psa_aead_decrypt`` -- see implementation for details. */
...
#ifdef RA_SIMULATOR_MODE
  const ra_err_t sdr = ra_psa_sim_aead_decrypt(handle,
                                               nonce,
                                               nonce_len,
                                               aad,
                                               aad_len,
