/**
 * @file ra_psa_crypto_internal.h
 * @brief Module-private definitions shared across the ``ra_psa_crypto`` TUs.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 * @details
 * The ``ra_psa_crypto`` facade is split across two translation units:
 * ``ra_psa_crypto.c`` (public API + key-handle pool) and
 * ``ra_psa_crypto_sim.c`` (simulator-mode SHA-256 / AEAD stand-ins). This
 * private header holds the definitions both TUs need:
 *
 * - The concrete ``struct ra_psa_key_handle`` slot layout (the public
 ...
 *   ``ra_psa_crypto_sim.c`` but called from the public API in
 *   ``ra_psa_crypto.c``.
 *
 * This header is internal to the library; consumers include the public
 * ``ra_psa_crypto.h`` only.
 *
...
/* =============================================================================
 * Simulator-mode crypto helpers (defined in ra_psa_crypto_sim.c)
 * =============================================================================
 */
 ...
 * @param[out] out    Destination digest, ``k_ra_psa_sha256_len`` bytes.
 *
 * @pre ``out`` is non-NULL and at least ``k_ra_psa_sha256_len`` bytes.
 ...
 * @since 0.1.0
 */
void ra_psa_sim_sha256_oneshot(const uint8_t* in, size_t in_len, uint8_t out[k_ra_psa_sha256_len]);
 ...
 * @retval k_ra_err_invalid_size Plaintext exceeds the scratch budget.
 *
 ...
 * @since 0.1.0
 */
ra_err_t ra_psa_sim_aead_encrypt(const struct ra_psa_key_handle* slot,
                                 const uint8_t*                  nonce,
                                 size_t                          nonce_len,
                                 const uint8_t*                  aad,
                                 size_t                          aad_len,
 ...
 * @since 0.1.0
 */
ra_err_t ra_psa_sim_aead_decrypt(const struct ra_psa_key_handle* slot,
                                 const uint8_t*                  nonce,
                                 size_t                          nonce_len,
                                 const uint8_t*                  aad,
                                 size_t                          aad_len,
