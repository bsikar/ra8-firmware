/**
 * @file ra_io_stream_internal.h
 * @brief Internal vtable type for the ra_io byte-stream facade.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 ...
 *
 * Defines ::ra_io_stream_iface -- the function-pointer table each sink fills in
 * and binds into a caller-owned ::ra_io_stream_t. It is forward declared opaque
 * in `ra_io_stream.h`.
 *
...
struct ra_io_stream_iface {
   ...
   */
  ra_err_t (*write)(void* ctx, const uint8_t* buf, uint32_t len, uint32_t* out_written);

  /** @brief Commit any buffering. May be NULL (nothing buffered). */
  ra_err_t (*flush)(void* ctx);
};
