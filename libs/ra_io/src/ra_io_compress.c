...
/**
 * @brief Validate ::ra_io_compress arguments before driving the compressor.
 *
 * @details Performs the four non-NULL precondition checks and the scratch-size
 *          lower-bound check, returning the matching ::ra_err_t so the public
 ...
 *
 * @return ra_err_t Validation result.
 * @retval k_ra_ok               All arguments are valid.
 ...
 * @post On `k_ra_ok` every pointer argument is non-NULL.
 *
 * @note Pure precondition check; touches no compressor state.
 *
 * @since 0.1.0
 */
static ra_err_t ra_io_compress_validate(const uint8_t*  src,
                                        const uint8_t*  out,
                                        const void*     scratch,
 ...
 *          ::ra_err_t. On success the produced length is taken from the sink.
 *
 ...
 *
 * @return ra_err_t Compression result.
 * @retval k_ra_ok         Compressed; `*out_len` set from the sink.
 * @retval k_ra_err_no_mem `out` was too small for the result.
 * @retval k_ra_fail       The compressor reported an error.
 *
 * @pre `d` points to at least ::k_ra_io_compress_scratch_bytes of scratch.
 * @pre `ctx->out`/`ctx->cap` describe the destination, `ctx->len == 0`.
 * @post On `k_ra_ok` `*out_len` equals the bytes appended to the sink.
 ...
 * @since 0.1.0
 */
static ra_err_t ra_io_compress_drive(tdefl_compressor* d,
                                     compress_ctx_t*   ctx,
                                     const uint8_t*    src,
                                     uint32_t          src_len,
                                     uint32_t*         out_len)
...
ra_err_t ra_io_compress(const uint8_t* src,
                        ...
                        uint32_t*      out_len)
{
  const ra_err_t vr = ra_io_compress_validate(src, out, scratch, scratch_len, out_len);
  if (vr != k_ra_ok) {
    return vr;
  }
  tdefl_compressor* d   = (tdefl_compressor*)scratch;
  compress_ctx_t    ctx = {.out = out, .cap = out_cap, .len = 0, .overflow = 0};
  return ra_io_compress_drive(d, &ctx, src, src_len, out_len);
}
