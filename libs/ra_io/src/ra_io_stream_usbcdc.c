/**
 * @file ra_io_stream_usbcdc.c
 * @brief USB-CDC byte-stream sink -- bulk-IN writes through ra_usb_pal.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 * @details
 * Binds ::ra_io_stream_iface to `ra_usb_pal_ep_send`. The PAL takes a 16-bit
...
static const char* const s_tag = "ra_io_stream_usbcdc";

/**
 * @enum ra_io_usbcdc_const_t
 * @brief USB-CDC sink constants.
 *
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_usbcdc_max_chunk = 65535, /**< Largest single ra_usb_pal_ep_send length. */
} ra_io_usbcdc_const_t;
 ...
 * @retval k_ra_err_*        Propagated from `ra_usb_pal_ep_send`.
 *
 ...
 * @since 0.1.0
 */
static ra_err_t usbcdc_write(void* ctx, const uint8_t* buf, uint32_t len, uint32_t* out_written)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  RA_CHECK_NULL_PTR(buf, s_tag, "buf must not be nullptr");
  const ra_io_stream_usbcdc_state_t* st   = (const ra_io_stream_usbcdc_state_t*)ctx;
  uint32_t                           done = 0;
  while (done < len) {
    const uint32_t rem = len - done;
    const uint32_t chunk =
      (rem > (uint32_t)k_usbcdc_max_chunk) ? (uint32_t)k_usbcdc_max_chunk : rem;
    const ra_err_t e = ra_usb_pal_ep_send(st->ep_addr, &buf[done], (uint16_t)chunk);
    if (e != k_ra_ok) {
...
static const ra_io_stream_iface_t k_usbcdc_iface = {
  .write = usbcdc_write,
  .flush = nullptr,
};
