/**
 * @file ra_io_blockdev_sdspi.c
 * @brief Micro-SD-over-SPI block-device backend -- thin shim over ra_sdmmc_spi.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
...
static const char* const s_tag = "ra_io_blockdev_sdspi";

/**
 * @enum ra_io_sdspi_const_t
...
typedef enum : uint32_t {
  k_ra_io_sdspi_erase_unit_blocks = 1, /**< One logical block per erase unit. */
} ra_io_sdspi_const_t;
 ...
 * @retval k_ra_err_out_of_range Range past the card capacity.
 *
 ...
 * @since 0.1.0
 */
static ra_err_t sdspi_read(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf)
{
  (void)ctx;
 ...
 * @since 0.1.0
 */
static ra_err_t sdspi_write(void* ctx, uint32_t lba, uint32_t count, const uint8_t* buf)
{
  (void)ctx;
  RA_CHECK_NULL_PTR(buf, s_tag, "buf must not be nullptr");
 ...
 * reads back as `0x00` (returning ::k_ra_err_not_supported on a ones-erasing
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                Range erased and verified to read back zero.
 * @retval k_ra_err_invalid_arg   `count` was zero.
 * @retval k_ra_err_out_of_range  Range past the card capacity.
 * @retval k_ra_err_not_supported Card erases to a non-zero value.
 *
 * @pre The card is initialised via `ra_sdmmc_spi_init`.
 ...
 * @since 0.1.0
 */
static ra_err_t sdspi_erase(void* ctx, uint32_t lba, uint32_t count)
{
  (void)ctx;
 ...
 * @since 0.1.0
 */
static ra_err_t sdspi_get_caps(const void* ctx, ra_io_blockdev_caps_t* out)
{
  (void)ctx;
  RA_CHECK_NULL_PTR(out, s_tag, "out must not be nullptr");
  uint32_t blocks = 0;
  RA_RETURN_ON_ERROR(ra_sdmmc_spi_get_capacity(&blocks), s_tag, "sd capacity");
  out->block_count             = blocks;
  out->erase_unit_blocks       = (uint32_t)k_ra_io_sdspi_erase_unit_blocks;
...
static const ra_io_blockdev_iface_t k_sdspi_iface = {
  .read     = sdspi_read,
  .write    = sdspi_write,
  .erase    = sdspi_erase,
...
ra_err_t ra_io_blockdev_sdspi_init(ra_io_blockdev_t* bd)
{
