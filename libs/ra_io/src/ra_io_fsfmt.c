 ...
 *
 * @details
 * Holds pointers to registered ::ra_io_fsfmt_t descriptors and probes them in
 * order against a block device's boot sector. FAT and exFAT are built-ins whose
 * probes read block 0 directly (no `ra_fs` internals); foreign formats register
 * through ::ra_io_fsfmt_register. Every check is a single condition, so no MC/DC
 * vectors are due.
 *
...
static const char* const s_tag = "ra_io_fsfmt";

/**
 * @enum ra_io_fsfmt_const_t
...
static const ra_io_fsfmt_t* s_reg[(uint32_t)k_ra_io_fsfmt_max];
 ...
 * @param[out] blk Destination of `k_ra_io_block_size_bytes` bytes.
 *
...
static bool internal_read_boot0(const ra_fs_backend_t* be, uint8_t* blk)
{
  ...
  return be->read_block(be->ctx, 0, 1, blk) == k_ra_ok;
}
 ...
 * @since 0.1.0
 */
static bool fat_probe(const ra_fs_backend_t* be)
{
 ...
 * @since 0.1.0
 */
static bool exfat_probe(const ra_fs_backend_t* be)
{
  uint8_t blk[(size_t)k_ra_io_block_size_bytes];
  if (!internal_read_boot0(be, blk)) {
    return false;
...
static const ra_io_fsfmt_t k_fmt_fat = {
  .name = "fat",
  .caps =
    {
...
static const ra_io_fsfmt_t k_fmt_exfat = {
  .name = "exfat",
  .caps =
    {
...
ra_err_t ra_io_fsfmt_init(void)
{
  s_count = 0;
  RA_RETURN_ON_ERROR(ra_io_fsfmt_register(&k_fmt_exfat), s_tag, "register exfat");
  RA_RETURN_ON_ERROR(ra_io_fsfmt_register(&k_fmt_fat), s_tag, "register fat");
  return k_ra_ok;
}

ra_err_t ra_io_fsfmt_register(const ra_io_fsfmt_t* fmt)
{
  RA_CHECK_NULL_PTR(fmt, s_tag, "fmt must not be nullptr");
  RA_CHECK_NULL_PTR(fmt->name, s_tag, "fmt->name must not be nullptr");
  RA_CHECK_NULL_PTR(fmt->probe, s_tag, "fmt->probe must not be nullptr");
  if (s_count >= (uint32_t)k_ra_io_fsfmt_max) {
...
ra_err_t ra_io_fsfmt_probe(const ra_fs_backend_t* backend, const ra_io_fsfmt_t** out)
{
  RA_CHECK_NULL_PTR(backend, s_tag, "backend must not be nullptr");
