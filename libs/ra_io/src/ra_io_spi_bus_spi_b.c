/**
 * @file ra_io_spi_bus_spi_b.c
 * @brief SPI_B backend for the ra_io SPI-bus facade -- thin shim over ra_spi.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 * @details
 * Implements ::ra_io_spi_bus_iface by forwarding to the SPI_B driver
 * (`ra_spi.h`), unmodified. The bound channel index travels in the handle
...
static const char* const s_tag = "ra_io_spi_bus_spi_b";
 ...
 * `ra_spi_xfer8`, which validates the channel and owns the SPSR polling.
 *
 ...
 *
 * @pre The channel was initialised via `ra_spi_init`.
 * @pre `ctx` carries a channel index bound by `ra_io_spi_bus_bind_spi_b`.
 ...
 * @since 0.1.0
 */
static ra_err_t spi_b_xfer8(void* ctx, uint8_t tx, uint8_t* rx)
{
  return ra_spi_xfer8((uint8_t)(uintptr_t)ctx, tx, rx);
}
 ...
 *
 * @details
 * Forwards straight to `ra_spi_write_read`, which supports 8/16/32-bit
 ...
 * @retval k_ra_err_null_ptr    Both buffers NULL with `len` > 0.
 * @retval k_ra_err_invalid_arg Channel or `width` invalid.
 * @retval k_ra_err_hw_timeout  SPSR polling expired.
 *
 * @pre The channel was initialised via `ra_spi_init`.
 ...
 */
static ra_err_t
spi_b_write_read(void* ctx, const void* tx, void* rx, uint32_t len, ra_spi_bit_width_t width)
{
  return ra_spi_write_read((uint8_t)(uintptr_t)ctx, tx, rx, len, width);
}
 ...
 *
 * @details
 * Forwards to `ra_spi_set_clock`, which validates the channel and clock
 ...
 * @retval k_ra_err_invalid_arg Channel or clock arguments invalid.
 *
 * @pre The channel was initialised via `ra_spi_init`.
 * @pre `ctx` carries a channel index bound by `ra_io_spi_bus_bind_spi_b`.
 ...
 * @since 0.1.0
 */
static ra_err_t spi_b_set_clock(void* ctx, uint32_t baud_hz, uint32_t pclk_hz)
{
  return ra_spi_set_clock((uint8_t)(uintptr_t)ctx, baud_hz, pclk_hz);
}

/** @brief SPI_B backend vtable -- every row forwards to `ra_spi_*`. */
static const ra_io_spi_bus_iface_t k_spi_b_iface = {
  .xfer8      = spi_b_xfer8,
  .write_read = spi_b_write_read,
  .set_clock  = spi_b_set_clock,
};

ra_err_t ra_io_spi_bus_bind_spi_b(ra_io_spi_bus_t* bus, uint8_t channel)
{
