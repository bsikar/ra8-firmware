 ...
 *
 * @details
 * Implements ::ra_io_blockdev_iface by forwarding reads and writes to the
 * `ra_sdcard` HAL driver, which drives the RA8D2 4-bit SDHI controller. The
 ...
 * `erase` slot is NULL; capacity comes from ::ra_sdcard_get_capacity. This file
 * touches no raw MMIO -- the HUM citations live in the `ra_sdcard` driver.
 *
...
static const char* const s_tag = "ra_io_blockdev_sdhi";

/**
 * @enum ra_io_sdhi_const_t
 * @brief Native-SDHI backend layout constants.
 *
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_ra_io_sdhi_erase_unit_blocks = 1, /**< One logical block per erase unit. */
} ra_io_sdhi_const_t;
 ...
 *
 * @details
 * Validates `buf` then forwards straight to ::ra_sdcard_read_blocks; the card
 ...
 * @retval k_ra_err_invalid_arg   `count` was zero.
 * @retval k_ra_err_invalid_state The card was never initialised.
 * @retval k_ra_err_out_of_range  Range past card capacity.
 *
 ...
 * @since 0.1.0
 */
static ra_err_t sdhi_read(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf)
{
  (void)ctx;
 ...
 *
 * @details
 * Validates `buf` then forwards straight to ::ra_sdcard_write_blocks; the card
 * driver enforces capacity bounds and initialisation state. `ctx` is unused
 * (the card is a singleton).
 *
 * @param[in] ctx   Unused backend context (always NULL for this backend).
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                Blocks written to the card.
 * @retval k_ra_err_null_ptr      `buf` was NULL.
 * @retval k_ra_err_invalid_arg   `count` was zero.
 * @retval k_ra_err_invalid_state The card was never initialised.
 * @retval k_ra_err_out_of_range  Range past card capacity.
 ...
 * @since 0.1.0
 */
static ra_err_t sdhi_write(void* ctx, uint32_t lba, uint32_t count, const uint8_t* buf)
{
  (void)ctx;
  RA_CHECK_NULL_PTR(buf, s_tag, "buf must not be nullptr");
 ...
 *
 * @details
 * Queries the card block count via ::ra_sdcard_get_capacity and fills `out` for
 ...
 * @since 0.1.0
 */
static ra_err_t sdhi_get_caps(const void* ctx, ra_io_blockdev_caps_t* out)
{
  (void)ctx;
  RA_CHECK_NULL_PTR(out, s_tag, "out must not be nullptr");
  uint32_t       blocks = 0;
  const ra_err_t cap    = ra_sdcard_get_capacity(&blocks);
  if (cap != k_ra_ok) {
    return cap;
  }
  out->block_count             = blocks;
  out->erase_unit_blocks       = (uint32_t)k_ra_io_sdhi_erase_unit_blocks;
...
static const ra_io_blockdev_iface_t k_sdhi_iface = {
  .read     = sdhi_read,
  .write    = sdhi_write,
  .erase    = nullptr,
...
ra_err_t ra_io_blockdev_sdhi_init(ra_io_blockdev_t* bd)
{
