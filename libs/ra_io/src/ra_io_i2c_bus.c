/**
 * @file ra_io_i2c_bus.c
 * @brief I2C-bus dispatcher -- forwards each public call into the bound
 *        backend vtable, and bridges any backend to an `ra_i2c_bus_ops_t`.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 ...
 * bound ::ra_io_i2c_bus_iface. The bridge installs static trampolines
 * whose `ctx` is the ::ra_io_i2c_bus_t itself, mirroring the
 * `ra_io_blockdev_as_fs_backend()` idiom. No MMIO is touched here; the
...
static const char* const s_tag = "ra_io_i2c_bus";
 ...
 * @retval k_ra_ok                  `bus` is non-NULL with a bound backend.
 * @retval k_ra_err_null_ptr        `bus` was NULL.
 * @retval k_ra_err_not_initialized `bus->iface` was NULL (never bound).
 *
 * @pre None.
 * @pre None.
 ...
 * @since 0.1.0
 */
static ra_err_t internal_validate(const ra_io_i2c_bus_t* bus)
{
  if (bus == nullptr) {
...
ra_err_t ra_io_i2c_bus_write(const ra_io_i2c_bus_t* bus,
                             uint8_t                addr,
                             const uint8_t*         data,
                             uint32_t               len,
                             bool                   send_stop)
{
  const ra_err_t v = internal_validate(bus);
  if (v != k_ra_ok) {
    return v;
  }
  RA_CHECK_NULL_PTR(bus->iface->write, s_tag, "backend write op missing");
  return bus->iface->write(bus->ctx, addr, data, len, send_stop);
}

ra_err_t ra_io_i2c_bus_read(const ra_io_i2c_bus_t* bus, uint8_t addr, uint8_t* data, uint32_t len)
{
  const ra_err_t v = internal_validate(bus);
  if (v != k_ra_ok) {
    return v;
  }
  RA_CHECK_NULL_PTR(bus->iface->read, s_tag, "backend read op missing");
  return bus->iface->read(bus->ctx, addr, data, len);
}

ra_err_t ra_io_i2c_bus_transfer(const ra_io_i2c_bus_t* bus,
                                uint8_t                addr,
                                const uint8_t*         wr,
                                uint32_t               wr_len,
                                uint8_t*               rd,
                                uint32_t               rd_len)
{
  ...
  RA_CHECK_NULL_PTR(bus->iface->transfer, s_tag, "backend transfer op missing");
  return bus->iface->transfer(bus->ctx, addr, wr, wr_len, rd, rd_len);
}

/* =============================================================================
 * ra_i2c_bus_ops_t bridge (Ring-3 seam)
 * =============================================================================
 */
 ...
 * through ::ra_io_i2c_bus_write.
 *
 * @param[in] ctx       The ::ra_io_i2c_bus_t handle (as a void cookie).
 ...
 * @retval k_ra_err_*        Propagated from ::ra_io_i2c_bus_write.
 *
 * @pre `ctx` is a bound ::ra_io_i2c_bus_t.
...
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx (i2c bus) must not be nullptr");
  return ra_io_i2c_bus_write((const ra_io_i2c_bus_t*)ctx, addr, data, len, send_stop);
}
 ...
 * @retval k_ra_err_null_ptr `ctx` was NULL.
 * @retval k_ra_err_*        Propagated from ::ra_io_i2c_bus_read.
 *
 * @pre `ctx` is a bound ::ra_io_i2c_bus_t.
 * @pre The underlying channel was initialised by its own driver init.
 ...
 * @since 0.1.0
 */
static ra_err_t internal_ops_read(void* ctx, uint8_t addr, uint8_t* data, uint32_t len)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx (i2c bus) must not be nullptr");
  return ra_io_i2c_bus_read((const ra_io_i2c_bus_t*)ctx, addr, data, len);
}
 ...
 * write-then-RESTART-then-read through ::ra_io_i2c_bus_transfer.
 *
 * @param[in]  ctx    The ::ra_io_i2c_bus_t handle (as a void cookie).
 * @param[in]  addr   7-bit peripheral address.
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           Both phases completed.
 * @retval k_ra_err_null_ptr `ctx` was NULL.
 * @retval k_ra_err_*        Propagated from ::ra_io_i2c_bus_transfer.
 *
 * @pre `ctx` is a bound ::ra_io_i2c_bus_t.
 * @pre The underlying channel was initialised by its own driver init.
 ...
 * @since 0.1.0
 */
static ra_err_t internal_ops_transfer(void*          ctx,
                                      uint8_t        addr,
                                      const uint8_t* wr,
                                      uint32_t       wr_len,
                                      uint8_t*       rd,
...
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx (i2c bus) must not be nullptr");
  return ra_io_i2c_bus_transfer((const ra_io_i2c_bus_t*)ctx, addr, wr, wr_len, rd, rd_len);
}

ra_err_t ra_io_i2c_bus_as_ops(const ra_io_i2c_bus_t* bus, ra_i2c_bus_ops_t* out)
{
  const ra_err_t v = internal_validate(bus);
  if (v != k_ra_ok) {
    return v;
