/**
 * @file ra_io_blockdev_internal.h
 * @brief Internal vtable type for the ra_io block-device fabric.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 ...
 *
 * Defines ::ra_io_blockdev_iface -- the function-pointer table each backend
 * fills in and binds into a caller-owned ::ra_io_blockdev_t. It is forward
 * declared opaque in `ra_io_blockdev.h`.
 *
 ...
 * maps a NULL `erase` to ::k_ra_err_not_supported and a NULL `sync` to success.
 *
...
struct ra_io_blockdev_iface {
  ...
  ra_err_t (*read)(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf);

  /** @brief Write `count` blocks from `buf` at `lba`. */
  ra_err_t (*write)(void* ctx, uint32_t lba, uint32_t count, const uint8_t* buf);

  /** @brief Erase `count` blocks at `lba`. May be NULL (no erase concept). */
  ra_err_t (*erase)(void* ctx, uint32_t lba, uint32_t count);

  /** @brief Fill `*out` with the medium capabilities. */
  ra_err_t (*get_caps)(const void* ctx, ra_io_blockdev_caps_t* out);

  /** @brief Commit any write buffering. May be NULL (nothing buffered). */
  ra_err_t (*sync)(void* ctx);
};
