 ...
 *
 * @details
 * Reference sink implementation of ::ra_io_stream_iface. Writes are a
 * bounds-checked `memcpy` into the caller buffer; a full buffer reports
 * ::k_ra_err_no_mem. Pure memory access -- identical on host and target.
 *
...
static const char* const s_tag = "ra_io_stream_ram";
 ...
 * accepted count. Returns ::k_ra_err_no_mem when not all bytes fit.
 *
 ...
 * @retval k_ra_ok           All bytes appended.
 * @retval k_ra_err_null_ptr `ctx` or `buf` was NULL.
 * @retval k_ra_err_no_mem   The buffer could not accept all bytes.
 *
 ...
 * @since 0.1.0
 */
static ra_err_t ram_write(void* ctx, const uint8_t* buf, uint32_t len, uint32_t* out_written)
{
...
static const ra_io_stream_iface_t k_ram_iface = {
  .write = ram_write,
  .flush = nullptr,
};

ra_err_t ra_io_stream_ram_init(ra_io_stream_t*           s,
...
ra_err_t ra_io_stream_ram_used(const ra_io_stream_ram_state_t* state, uint32_t* out_used)
{
  RA_CHECK_NULL_PTR(state, s_tag, "state must not be nullptr");
  RA_CHECK_NULL_PTR(out_used, s_tag, "out_used must not be nullptr");
  *out_used = state->len;
  return k_ra_ok;
}
