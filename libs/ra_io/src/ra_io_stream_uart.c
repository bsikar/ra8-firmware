/**
 * @file ra_io_stream_uart.c
 * @brief UART/SCI byte-stream sink -- polled writes through ra_sci.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 * @details
 * Thin wrapper binding ::ra_io_stream_iface to `ra_sci_write_polling` /
 * `ra_sci_flush`. Touches no raw MMIO -- the `ra_sci` driver owns the HUM
 * citations.
 *
 * @copyright Copyright (c) 2026 Brighton Sikarskie
 * SPDX-License-Identifier: MIT
 */

#include "ra_io_stream_uart.h"

#include <stdint.h>

#include "ra_check.h"
#include "ra_err.h"
#include "ra_io_stream_internal.h"
#include "ra_sci.h"

/** @brief Module log tag. */
static const char* const s_tag = "ra_io_stream_uart";

/**
 * @brief UART sink: write all bytes out of the SCI channel by polling.
 *
 * @details
 * Forwards to `ra_sci_write_polling`. On success every byte was shifted out, so
 * the accepted count equals `len`.
 *
 * @param[in]  ctx         UART sink state (as a void cookie).
 * @param[in]  buf         Source bytes.
 * @param[in]  len         Number of bytes to send.
 * @param[out] out_written Accepted byte count, or NULL.
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           All bytes sent.
 * @retval k_ra_err_null_ptr `ctx` or `buf` was NULL.
 * @retval k_ra_err_*        Propagated from `ra_sci_write_polling`.
 *
 * @pre `ctx` is a populated UART sink state.
 * @pre The SCI channel is initialised.
 * @post On success all `len` bytes were transmitted.
 * @post `*out_written` (when provided) equals `len` on success.
 *
 * @note Not thread-safe with respect to the same channel.
 *
 * @since 0.1.0
 */
static ra_err_t uart_write(void* ctx, const uint8_t* buf, uint32_t len, uint32_t* out_written)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  RA_CHECK_NULL_PTR(buf, s_tag, "buf must not be nullptr");
  const ra_io_stream_uart_state_t* st = (const ra_io_stream_uart_state_t*)ctx;
  const ra_err_t                   e  = ra_sci_write_polling(st->channel, buf, len);
  if (e != k_ra_ok) {
    return e;
  }
  if (out_written != nullptr) {
    *out_written = len;
  }
  return k_ra_ok;
}

/**
 * @brief UART sink: drain the SCI transmit shift register.
 *
 * @details
 * Forwards to `ra_sci_flush` so a caller can guarantee the line is on the wire
 * before sleeping or asserting a panic.
 *
 * @param[in] ctx UART sink state (as a void cookie).
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           Transmit drained.
 * @retval k_ra_err_null_ptr `ctx` was NULL.
 * @retval k_ra_err_*        Propagated from `ra_sci_flush`.
 *
 * @pre `ctx` is a populated UART sink state.
 * @pre The SCI channel is initialised.
 * @post On success the shift register is empty.
 * @post No sink state is mutated.
 *
 * @note Not thread-safe with respect to the same channel.
 *
 * @since 0.1.0
 */
static ra_err_t uart_flush(void* ctx)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  const ra_io_stream_uart_state_t* st = (const ra_io_stream_uart_state_t*)ctx;
  return ra_sci_flush(st->channel);
}

/** @brief UART stream sink vtable. */
static const ra_io_stream_iface_t k_uart_iface = {
  .write = uart_write,
  .flush = uart_flush,
};

ra_err_t
ra_io_stream_uart_init(ra_io_stream_t* s, ra_io_stream_uart_state_t* state, uint8_t channel)
{
  RA_CHECK_NULL_PTR(s, s_tag, "s must not be nullptr");
  RA_CHECK_NULL_PTR(state, s_tag, "state must not be nullptr");
  state->channel = channel;
  s->iface       = &k_uart_iface;
  s->ctx         = state;
  return k_ra_ok;
}
