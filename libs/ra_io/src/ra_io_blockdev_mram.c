 ...
 *
 * @details
 * Implements ::ra_io_blockdev_iface over the on-chip extra/data MRAM region.
 * Reads `memcpy` from the memory-mapped MRAM array; writes route through
 * `ra_flash_extra_mram_write` chunked into the 32-byte MRAM program unit; erases
 * route through `ra_flash_extra_mram_erase`, one 32-byte block per call. The
 ...
 * touches no raw MMIO -- the `ra_flash` driver carries the HUM citations.
 *
...
static const char* const s_tag = "ra_io_blockdev_mram";

/**
 * @enum ra_io_mram_const_t
 ...
 * @since 0.1.0
 */
static ra_err_t mram_bounds(const ra_io_blockdev_mram_state_t* st, uint32_t lba, uint32_t count)
{
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok              Window is aligned and inside data MRAM.
 * @retval k_ra_err_invalid_arg Window misaligned, sized wrong, or out of region.
 *
 ...
 * @since 0.1.0
 */
static ra_err_t mram_window_ok(uintptr_t base, uint32_t block_count)
{
  ...
  if ((span % (uintptr_t)k_ra_io_mram_erase_block_bytes) != 0U) {
    return k_ra_err_invalid_arg;
  }
  if (base < extra_start) {
    return k_ra_err_invalid_arg;
 ...
 * device window. No `ra_flash` call is needed for reads.
 *
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Blocks copied into `buf`.
 ...
 * @since 0.1.0
 */
static ra_err_t mram_read(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  RA_CHECK_NULL_PTR(buf, s_tag, "buf must not be nullptr");
  const ra_io_blockdev_mram_state_t* st = (const ra_io_blockdev_mram_state_t*)ctx;
  const ra_err_t                     b  = mram_bounds(st, lba, count);
  if (b != k_ra_ok) {
    return b;
  }
  const size_t off = (size_t)lba * (size_t)k_ra_io_block_size_bytes;
  const size_t n   = (size_t)count * (size_t)k_ra_io_block_size_bytes;
 ...
 *
 * @details
 * Routes the write through `ra_flash_extra_mram_write` in 32-byte program-unit
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                Blocks programmed into the MRAM window.
 * @retval k_ra_err_null_ptr      `ctx` or `buf` was NULL.
 * @retval k_ra_err_not_supported The device is read-only.
 * @retval k_ra_err_out_of_range  Range past capacity.
 * @retval k_ra_err_invalid_arg   The driver rejected the program request.
 * @retval k_ra_err_hw_error      The controller reported a program error.
 *
 * @pre `ctx` is a populated MRAM state.
 ...
 * @since 0.1.0
 */
static ra_err_t mram_write(void* ctx, uint32_t lba, uint32_t count, const uint8_t* buf)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  RA_CHECK_NULL_PTR(buf, s_tag, "buf must not be nullptr");
  ra_io_blockdev_mram_state_t* st = (ra_io_blockdev_mram_state_t*)ctx;
  if (st->read_only) {
  ...
  const uintptr_t off  = (uintptr_t)lba * (uintptr_t)k_ra_io_block_size_bytes;
  const uintptr_t span = (uintptr_t)count * (uintptr_t)k_ra_io_block_size_bytes;
  const uint32_t  step = (uint32_t)k_ra_io_mram_program_bytes;
  for (uintptr_t done = (uintptr_t)k_ra_io_mram_zero_len; done < span; done += step) {
    const ra_err_t w =
      ra_flash_extra_mram_write((uint32_t)(st->base + off + done), &buf[done], step);
    if (w != k_ra_ok) {
      return w;
    }
  }
 ...
 * one block per `ra_flash_extra_mram_erase` call across the range. After success
 ...
 * @retval k_ra_err_not_supported The device is read-only.
 * @retval k_ra_err_out_of_range  Range past capacity.
 * @retval k_ra_err_invalid_arg   The driver rejected the erase request.
 * @retval k_ra_err_hw_error      The controller reported a program error.
 *
 * @pre `ctx` is a populated MRAM state.
 ...
 * @since 0.1.0
 */
static ra_err_t mram_erase(void* ctx, uint32_t lba, uint32_t count)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  ra_io_blockdev_mram_state_t* st = (ra_io_blockdev_mram_state_t*)ctx;
  if (st->read_only) {
  ...
  const uintptr_t off  = (uintptr_t)lba * (uintptr_t)k_ra_io_block_size_bytes;
  const uintptr_t span = (uintptr_t)count * (uintptr_t)k_ra_io_block_size_bytes;
  const uint32_t  step = (uint32_t)k_ra_io_mram_erase_block_bytes;
  for (uintptr_t done = (uintptr_t)k_ra_io_mram_zero_len; done < span; done += step) {
    const ra_err_t e = ra_flash_extra_mram_erase((uint32_t)(st->base + off + done));
    if (e != k_ra_ok) {
      return e;
    }
  }
 ...
 * @since 0.1.0
 */
static ra_err_t mram_get_caps(const void* ctx, ra_io_blockdev_caps_t* out)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  RA_CHECK_NULL_PTR(out, s_tag, "out must not be nullptr");
  const ra_io_blockdev_mram_state_t* st = (const ra_io_blockdev_mram_state_t*)ctx;
  out->block_count                      = st->block_count;
  out->erase_unit_blocks                = (uint32_t)k_ra_io_mram_erase_unit_blocks;
  out->program_size_bytes               = (uint32_t)k_ra_io_mram_program_bytes;
...
/** @brief MRAM backend vtable. `sync` NULL: `ra_flash_extra_mram_write` commits inline. */
static const ra_io_blockdev_iface_t k_mram_iface = {
  .read     = mram_read,
  .write    = mram_write,
  .erase    = mram_erase,
...
ra_err_t ra_io_blockdev_mram_init(ra_io_blockdev_t*            bd,
                                  ...
                                  bool                         read_only)
{
  ...
  if (block_count < (uint32_t)k_ra_io_mram_min_blocks) {
    return k_ra_err_invalid_arg;
  }
  const ra_err_t w = mram_window_ok(base_addr, block_count);
  if (w != k_ra_ok) {
    return w;
  }
