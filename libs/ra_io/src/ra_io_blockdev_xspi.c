/**
 * @file ra_io_blockdev_xspi.c
 * @brief OSPI NOR block-device backend -- 512-byte LBAs over ra_xspi flash.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 * @details
 * Implementation of ::ra_io_blockdev_iface over the `ra_xspi` HAL. Reads chunk
 * `ra_xspi_flash_read` to the controller's per-transfer limit. Arbitrary writes
...
static const char* const s_tag = "ra_io_blockdev_xspi";

/**
 * @enum ra_io_xspi_const_t
 ...
 * @retval k_ra_ok               Range lies within the device.
 * @retval k_ra_err_out_of_range Range extends past the device capacity.
 *
 * @pre `st` is non-NULL.
 ...
 * @since 0.1.0
 */
static ra_err_t xspi_bounds(const ra_io_blockdev_xspi_state_t* st, uint32_t lba, uint32_t count)
{
 ...
 *
 * @details
 * Splits the request into transfers of at most `k_ra_xspi_max_xfer` bytes so
 * every `ra_xspi_flash_read` call stays within the controller's per-transfer
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               All bytes read.
 * @retval k_ra_err_null_ptr     `buf` invalid or `instance` out of range.
 * @retval k_ra_err_invalid_arg  HAL rejected a chunk length.
 *
 * @pre `buf` is writable for `len` bytes.
 * @pre The controller was initialised via `ra_xspi_init`.
 ...
 * @since 0.1.0
 */
static ra_err_t xspi_read_chunked(uint8_t instance, uint32_t flash_addr, uint8_t* buf, uint32_t len)
{
  ...
  while (done < len) {
    ...
    const uint32_t chunk =
      (remaining > (uint32_t)k_ra_xspi_max_xfer) ? (uint32_t)k_ra_xspi_max_xfer : remaining;
    const ra_err_t e = ra_xspi_flash_read(instance, flash_addr + done, &buf[done], chunk);
    if (e != k_ra_ok) {
      return e;
    }
    done += chunk;
 ...
 * @details
 * Splits the request into transfers of at most `k_ra_xspi_max_xfer` bytes so
 * every `ra_xspi_flash_program` call stays within the controller window. The
 ...
 * @retval k_ra_err_null_ptr     `data` invalid or `instance` out of range.
 * @retval k_ra_err_invalid_arg  HAL rejected a chunk length.
 * @retval k_ra_err_timeout      Flash WIP bit never cleared.
 *
...
xspi_program_chunked(uint8_t instance, uint32_t flash_addr, const uint8_t* data, uint32_t len)
{
  ...
  while (done < len) {
    ...
    const uint32_t chunk =
      (remaining > (uint32_t)k_ra_xspi_max_xfer) ? (uint32_t)k_ra_xspi_max_xfer : remaining;
    const ra_err_t e = ra_xspi_flash_program(instance, flash_addr + done, &data[done], chunk);
    if (e != k_ra_ok) {
      return e;
    }
    done += chunk;
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Sector rewritten with the overlay applied.
 * @retval k_ra_err_null_ptr     `st`, `src`, or a HAL pointer was NULL.
 * @retval k_ra_err_invalid_arg  HAL rejected a transfer length.
 * @retval k_ra_err_timeout      Flash WIP bit never cleared.
 *
 ...
 * @since 0.1.0
 */
static ra_err_t write_one_sector(const ra_io_blockdev_xspi_state_t* st,
                                 uint32_t                           sector_lba,
                                 uint32_t                           first_blk,
                                 uint32_t                           n_blocks,
                                 const uint8_t*                     src)
{
  ...
  const uint32_t sec_addr = st->base_off + (sector_lba * (uint32_t)k_ra_io_block_size_bytes);
  const ra_err_t r =
    xspi_read_chunked(st->instance, sec_addr, s_sector, (uint32_t)k_xspi_sector_bytes);
  if (r != k_ra_ok) {
    return r;
  }
  const size_t dst_off = (size_t)first_blk * (size_t)k_ra_io_block_size_bytes;
  const size_t n_bytes = (size_t)n_blocks * (size_t)k_ra_io_block_size_bytes;
  (void)memcpy(&s_sector[dst_off], src, n_bytes);
  const ra_err_t e = ra_xspi_flash_erase_sector(st->instance, sec_addr);
  if (e != k_ra_ok) {
    return e;
  }
 ...
 * @retval k_ra_err_out_of_range Range past capacity.
 * @retval k_ra_err_invalid_arg  HAL rejected a chunk length.
 *
 ...
 * @since 0.1.0
 */
static ra_err_t xspi_read(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  RA_CHECK_NULL_PTR(buf, s_tag, "buf must not be nullptr");
  const ra_io_blockdev_xspi_state_t* st = (const ra_io_blockdev_xspi_state_t*)ctx;
  const ra_err_t                     b  = xspi_bounds(st, lba, count);
  if (b != k_ra_ok) {
    return b;
  }
  const uint32_t addr = st->base_off + (lba * (uint32_t)k_ra_io_block_size_bytes);
  const uint32_t len  = count * (uint32_t)k_ra_io_block_size_bytes;
  return xspi_read_chunked(st->instance, addr, buf, len);
}
 ...
 * each via ::write_one_sector (whole-sector read-modify-write). Rejects the
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                Blocks committed to flash.
 * @retval k_ra_err_null_ptr      `ctx` or `buf` was NULL.
 ...
 * @since 0.1.0
 */
static ra_err_t xspi_write(void* ctx, uint32_t lba, uint32_t count, const uint8_t* buf)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  RA_CHECK_NULL_PTR(buf, s_tag, "buf must not be nullptr");
  const ra_io_blockdev_xspi_state_t* st = (const ra_io_blockdev_xspi_state_t*)ctx;
  ...
  while (done < count) {
    ...
    const uint8_t* src        = &buf[(size_t)done * (size_t)k_ra_io_block_size_bytes];
    const ra_err_t e          = write_one_sector(st, sector_lba, first_blk, n_blocks, src);
    if (e != k_ra_ok) {
      return e;
    }
    done += n_blocks;
 ...
 * spanned sector is erased to all-ones via `ra_xspi_flash_erase_sector`.
 *
 * @param[in] ctx   xSPI backend state (as a void cookie).
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                Range erased to all-ones.
 * @retval k_ra_err_null_ptr      `ctx` was NULL.
 * @retval k_ra_err_not_supported The device is read-only.
 * @retval k_ra_err_invalid_arg   Range is not sector-aligned.
 * @retval k_ra_err_out_of_range  Range past capacity.
 * @retval k_ra_err_timeout       Flash WIP bit never cleared.
 *
 * @pre `ctx` is a populated xSPI state.
 ...
 * @since 0.1.0
 */
static ra_err_t xspi_erase(void* ctx, uint32_t lba, uint32_t count)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  const ra_io_blockdev_xspi_state_t* st = (const ra_io_blockdev_xspi_state_t*)ctx;
  if (st->read_only) {
  ...
  uint32_t blk = lba;
  while (blk < lba + count) {
    const uint32_t addr = st->base_off + (blk * (uint32_t)k_ra_io_block_size_bytes);
    const ra_err_t e    = ra_xspi_flash_erase_sector(st->instance, addr);
    if (e != k_ra_ok) {
      return e;
    }
 ...
 * @since 0.1.0
 */
static ra_err_t xspi_get_caps(const void* ctx, ra_io_blockdev_caps_t* out)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  ...
  out->erase_value                      = (uint8_t)k_ra_io_erase_value_ones;
...
static const ra_io_blockdev_iface_t k_xspi_iface = {
  .read     = xspi_read,
  .write    = xspi_write,
  .erase    = xspi_erase,
  .get_caps = xspi_get_caps,
  .sync     = nullptr,
};

/**
 * @brief Validate the window geometry for ::ra_io_blockdev_xspi_init.
 *
 * @details
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok              Geometry is valid.
 * @retval k_ra_err_invalid_arg Some constraint was violated.
 *
 ...
 * @since 0.1.0
 */
static ra_err_t xspi_check_geom(uint8_t instance, uint32_t base_off, uint32_t block_count)
{
...
ra_err_t ra_io_blockdev_xspi_init(ra_io_blockdev_t*            bd,
                                  ...
                                  bool                         read_only)
{
  ...
  const ra_err_t g = xspi_check_geom(instance, base_off, block_count);
  if (g != k_ra_ok) {
    return g;
  }
  ...
  bd->iface          = &k_xspi_iface;
  bd->ctx            = state;
  return k_ra_ok;
}

static_assert((uint32_t)k_xspi_sector_bytes ==
                ((uint32_t)k_xspi_blocks_per_sector * (uint32_t)k_ra_io_block_size_bytes),
