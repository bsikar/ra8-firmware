 ...
 *
 * @details
 * Reference implementation of ::ra_io_blockdev_iface. Reads and writes are
...
static const char* const s_tag = "ra_io_blockdev_ram";

/**
 * @enum ra_io_ram_const_t
 * @brief RAM-backend layout constants.
 *
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_ra_io_ram_erase_unit_blocks = 1, /**< One logical block per erase unit. */
  k_ra_io_ram_min_blocks        = 1, /**< Smallest legal device size.       */
} ra_io_ram_const_t;
 ...
 * @since 0.1.0
 */
static ra_err_t ram_bounds(const ra_io_blockdev_ram_state_t* st, uint32_t lba, uint32_t count)
{
 ...
 * @since 0.1.0
 */
static ra_err_t ram_read(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  RA_CHECK_NULL_PTR(buf, s_tag, "buf must not be nullptr");
  const ra_io_blockdev_ram_state_t* st = (const ra_io_blockdev_ram_state_t*)ctx;
  const ra_err_t                    b  = ram_bounds(st, lba, count);
  if (b != k_ra_ok) {
    return b;
  }
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                Blocks copied into the backing buffer.
 * @retval k_ra_err_null_ptr      `ctx` or `buf` was NULL.
 ...
 * @since 0.1.0
 */
static ra_err_t ram_write(void* ctx, uint32_t lba, uint32_t count, const uint8_t* buf)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  RA_CHECK_NULL_PTR(buf, s_tag, "buf must not be nullptr");
  ra_io_blockdev_ram_state_t* st = (ra_io_blockdev_ram_state_t*)ctx;
  if (st->read_only) {
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                Range zero-filled.
 ...
 * @since 0.1.0
 */
static ra_err_t ram_erase(void* ctx, uint32_t lba, uint32_t count)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  ra_io_blockdev_ram_state_t* st = (ra_io_blockdev_ram_state_t*)ctx;
  if (st->read_only) {
  ...
  const size_t off = (size_t)lba * (size_t)k_ra_io_block_size_bytes;
  const size_t n   = (size_t)count * (size_t)k_ra_io_block_size_bytes;
  (void)memset(&st->storage[off], (int)k_ra_io_erase_value_zero, n);
  return k_ra_ok;
}
 ...
 * @since 0.1.0
 */
static ra_err_t ram_get_caps(const void* ctx, ra_io_blockdev_caps_t* out)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  RA_CHECK_NULL_PTR(out, s_tag, "out must not be nullptr");
  const ra_io_blockdev_ram_state_t* st = (const ra_io_blockdev_ram_state_t*)ctx;
  out->block_count                     = st->block_count;
  out->erase_unit_blocks               = (uint32_t)k_ra_io_ram_erase_unit_blocks;
...
static const ra_io_blockdev_iface_t k_ram_iface = {
  .read     = ram_read,
  .write    = ram_write,
  .erase    = ram_erase,
...
ra_err_t ra_io_blockdev_ram_init(ra_io_blockdev_t*           bd,
                                 ...
                                 bool                        read_only)
{
  ...
  if (block_count < (uint32_t)k_ra_io_ram_min_blocks) {
