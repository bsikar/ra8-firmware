/**
 * @file ra_io_spi_bus_internal.h
 * @brief Internal vtable type for the ra_io SPI-bus facade.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 ...
 * should.
 *
 * Defines ::ra_io_spi_bus_iface -- the function-pointer table each backend
 * fills in and binds into a caller-owned ::ra_io_spi_bus_t. It is forward
 * declared opaque in `ra_io_spi_bus.h`.
 *
...
struct ra_io_spi_bus_iface {
  ...
  ra_err_t (
    *write_read)(void* ctx, const void* tx, void* rx, uint32_t len, ra_spi_bit_width_t width);

  /** @brief Re-program the bit-rate divider from `baud_hz` / `pclk_hz`. */
  ra_err_t (*set_clock)(void* ctx, uint32_t baud_hz, uint32_t pclk_hz);
};
