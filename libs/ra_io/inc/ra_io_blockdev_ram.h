/**
 * @file ra_io_blockdev_ram.h
 * @brief ra_io block-device backend over a caller-owned RAM buffer.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 ...
 * implementation of ::ra_io_blockdev_iface), the substrate for host unit tests,
...
/**
 * @struct ra_io_blockdev_ram_state_t
 * @brief Caller-owned private state for a RAM block device.
 *
 * @details
 * Zero-initialise and pass to ::ra_io_blockdev_ram_init, which fills it and
 * binds it into a ::ra_io_blockdev_t. Treat the fields as private. The `storage`
 * buffer and this state must out-live every call made through the device.
 *
 * @invariant `storage` covers `block_count * k_ra_io_block_size_bytes` bytes.
 *
 * @since 0.1.0
 */
 ...
 * @retval k_ra_err_null_ptr     `bd`, `state`, or `storage` was NULL.
 * @retval k_ra_err_invalid_size `block_count` was zero.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_blockdev_ram_init(ra_io_blockdev_t*           bd,
