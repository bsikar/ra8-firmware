/**
 * @file ra_io_stream_uart.h
 * @brief ra_io byte-stream sink over a UART/SCI channel.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 * @details
 * Streams bytes out of an `ra_sci` channel by polling -- the "print to the host
 * over the serial console" target. The application must have brought the channel
 * up with `ra_sci_init` first. The data path is exercised on board_sim / HIL;
 * on the host this sink binds and validates only.
 *
 * @copyright Copyright (c) 2026 Brighton Sikarskie
 * SPDX-License-Identifier: MIT
 *
 * @since 0.1.0
 */

#pragma once

#ifdef __cplusplus
extern "C" {
#endif

#include <stdint.h>

#include "ra_err.h"
#include "ra_io_stream.h"

/**
 * @struct ra_io_stream_uart_state_t
 * @brief Caller-owned private state for a UART stream sink.
 *
 * @details Zero-initialise and pass to ::ra_io_stream_uart_init. Treat the
 *          field as private.
 *
 * @invariant `channel` indexes an initialised SCI channel.
 *
 * @since 0.1.0
 */
/* cppcheck-suppress-begin [unusedStructMember] */
typedef struct {
  uint8_t channel; /**< SCI channel index (private). */
} ra_io_stream_uart_state_t;
/* cppcheck-suppress-end [unusedStructMember] */

/**
 * @brief Bind a UART stream sink into a caller-owned stream handle.
 *
 * @param[out] s       Stream handle to bind (zero-initialised by the caller).
 * @param[out] state   Caller-owned sink state to populate.
 * @param[in]  channel SCI channel index to write to.
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           Sink bound; `s` is usable.
 * @retval k_ra_err_null_ptr `s` or `state` was NULL.
 *
 * @pre The SCI channel has been initialised with `ra_sci_init`.
 * @pre `s` and `state` out-live every call made through the stream.
 * @post On success `s` writes bytes out of `channel`.
 * @post On any non-ok return `s` and `state` are left unbound/untouched.
 *
 * @note Not thread-safe with respect to the same stream.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t
ra_io_stream_uart_init(ra_io_stream_t* s, ra_io_stream_uart_state_t* state, uint8_t channel);

#ifdef __cplusplus
}
#endif
