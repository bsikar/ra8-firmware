/**
 * @file ra_io_blockdev_mram.h
 * @brief ra_io block-device backend over the on-chip extra/data MRAM region.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 ...
 * region as an array of 512-byte logical blocks on top of the ::ra_io_blockdev_iface
 * fabric. It wraps the `ra_flash` HAL driver: reads `memcpy` directly from the
 * memory-mapped MRAM array, writes route through `ra_flash_write` (chunked into
 * the 32-byte MRAM program unit), and erases route through `ra_flash_erase`
 ...
 * **dangerous**: ::ra_io_blockdev_mram_init refuses any window that is not
 * wholly contained in the extra/data MRAM region
 * (`[k_ra_flash_extra_start, k_ra_flash_extra_start + k_ra_flash_extra_size)`)
 * or that overlaps the code-MRAM region
 * (`[k_ra_flash_code_start, k_ra_flash_code_start + k_ra_flash_code_size)`).
...
/**
 * @struct ra_io_blockdev_mram_state_t
 * @brief Caller-owned private state for an MRAM block device.
 *
 * @details
 * Zero-initialise (`= {}`) and pass to ::ra_io_blockdev_mram_init, which
 * validates the window, fills this state, and binds it into a
 * ::ra_io_blockdev_t. Treat the fields as private. This state must out-live
 ...
 * @invariant `block_count * k_ra_io_block_size_bytes` is a multiple of the MRAM
 ...
 * leaves `bd` and `state` untouched and returns ::k_ra_err_invalid_arg.
 *
 ...
 * @retval k_ra_err_null_ptr    `bd` or `state` was NULL.
 * @retval k_ra_err_invalid_arg `block_count` was zero, the window was
 ...
 * @pre `ra_flash_init` (or `ra_flash_open`) has been called before any write.
 ...
 *
 * @see ra_io_blockdev_ram_init
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_blockdev_mram_init(ra_io_blockdev_t*            bd,
