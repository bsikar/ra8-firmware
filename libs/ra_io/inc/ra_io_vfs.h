/**
 * @file ra_io_vfs.h
 * @brief ra_io virtual filesystem -- mount many volumes, address them by name.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 * @details
 * The VFS is a small mount table over `ra_fs`: register a mounted volume under a
 ...
 * told apart by their mount name. Each registered mount is an `ra_fs_mount_t`
 * the caller obtained by mounting an `ra_fs_backend_t` (typically one produced
 * by ::ra_io_blockdev_as_fs_backend over any block device).
 *
 ...
 * `ra_fs_read` / `ra_fs_write` / `ra_fs_seek` / `ra_fs_close` file operations.
 *
 * @code
 * ra_fs_backend_t be = {};
 * (void)ra_io_blockdev_as_fs_backend(&sd_bd, &be);
 * ra_fs_mount_t* m = nullptr;
 * (void)ra_fs_mount(&be, &m);
 * (void)ra_io_vfs_mount("sd", m);
 * ra_fs_file_t* f = nullptr;
 * (void)ra_io_vfs_open("sd:/book.epb", k_ra_fs_mode_read, &f);
 * @endcode
 *
...
/**
 * @struct ra_io_vfs_stat_t
 * @brief Metadata returned by ::ra_io_vfs_stat.
 *
 * @details `exists` is false (with ::k_ra_ok) when the path resolves to a mount
...
typedef struct {
  ...
  uint8_t  attr;         /**< FAT attribute byte (::ra_fs_attr_t).    */
 ...
 * @retval k_ra_ok All mount slots released.
 *
 * @pre None.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_vfs_init(void);

/**
 * @brief Register a mounted volume under a name.
 *
 * @param[in] name  Mount name (1..15 chars, no `:` or `/`).
 * @param[in] mount Live `ra_fs_mount_t` (must out-live the registration).
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Volume registered.
 * @retval k_ra_err_null_ptr     `name` or `mount` was NULL.
 * @retval k_ra_err_invalid_arg  Name empty or too long.
 * @retval k_ra_err_exists       The name is already mounted.
 * @retval k_ra_err_no_mem       The mount table is full.
 *
 * @pre `mount` was returned by `ra_fs_mount` and stays alive while registered.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_vfs_mount(const char* name, ra_fs_mount_t* mount);
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok            Mount released.
 * @retval k_ra_err_null_ptr  `name` was NULL.
 * @retval k_ra_err_not_found The name was not mounted.
 *
 * @pre No file opened on this mount is still in use.
 * @pre `name` is non-NULL.
 * @post `"name:/..."` paths no longer resolve.
 * @post The underlying `ra_fs_mount_t` is left untouched (caller owns it).
 *
 * @note Not thread-safe.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_vfs_unmount(const char* name);
 ...
 * @retval k_ra_err_null_ptr    `path` or `out_file` was NULL.
 * @retval k_ra_err_invalid_arg `path` has no `name:` prefix.
 * @retval k_ra_err_not_found   The mount name or the file is absent.
 * @retval k_ra_err_*           Propagated from `ra_fs_open`.
 *
 * @pre The named volume is mounted.
 * @pre `out_file` is writable.
 * @post On success drive the handle with `ra_fs_read` / `ra_fs_write` / etc.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_vfs_open(const char* path, ra_fs_mode_t mode, ra_fs_file_t** out_file);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_vfs_unlink(const char* path);
 ...
 * @retval k_ra_err_invalid_arg  Missing prefix, or the two mounts differ.
 * @retval k_ra_err_not_found    The mount or `old_path` is absent.
 * @retval k_ra_err_exists       `new_path` already exists.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_vfs_rename(const char* old_path, const char* new_path);
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok              Metadata resolved (`exists` may be false).
 * @retval k_ra_err_null_ptr    `path` or `out` was NULL.
 * @retval k_ra_err_invalid_arg `path` has no `name:` prefix.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_vfs_stat(const char* path, ra_io_vfs_stat_t* out);
 ...
 * @retval k_ra_err_null_ptr    `path` or `cb` was NULL.
 * @retval k_ra_err_invalid_arg `path` has no `name:` prefix.
 * @retval k_ra_err_not_found   The mount name is absent.
 * @retval k_ra_err_*           Propagated from `ra_fs_listdir`.
 *
 * @pre The named volume is mounted.
 * @pre `cb` is non-NULL.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_vfs_listdir(const char* path, ra_fs_listdir_cb_t cb, void* ctx);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_vfs_mkdir(const char* path);
