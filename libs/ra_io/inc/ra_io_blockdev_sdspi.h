/**
 * @file ra_io_blockdev_sdspi.h
 * @brief ra_io block-device backend over the micro-SD-in-SPI-mode driver.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 * @details
 * This backend wraps the `ra_sdmmc_spi` singleton driver so a micro-SD card on
 ...
 * erase value is therefore ::k_ra_io_erase_value_zero. Because the underlying
 * driver keeps a single global instance, this backend is stateless: the bound
 * ::ra_io_blockdev_t carries a NULL context and the vtable dispatches to the
 * one driver singleton.
 *
 * The application must bring the card up with `ra_sdmmc_spi_init(transport)`
 ...
 *
 * @details
 * Points `bd` at the SD-SPI vtable with a NULL context (the `ra_sdmmc_spi`
 ...
 * must already be initialised with `ra_sdmmc_spi_init` before any I/O call is
 * made through the bound device.
 *
 * @param[out] bd Handle to bind (zero-initialised by the caller).
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           Backend bound; `bd` is usable once the card is up.
 * @retval k_ra_err_null_ptr `bd` was NULL.
 *
 * @pre `bd` is non-NULL and out-lives every call made through the device.
 * @pre The application calls `ra_sdmmc_spi_init` before issuing I/O.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_blockdev_sdspi_init(ra_io_blockdev_t* bd);
