/**
 * @file ra_io_blockdev_sdram.h
 * @brief ra_io block-device backend over the external 64 MiB SDRAM window.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 ...
 * up, the external 64 MiB window at `k_ra_sdram_base_addr` is plain memory, so
 * this backend reuses the RAM backend's vtable and state
 * (::ra_io_blockdev_ram_state_t) verbatim. It treats the SDRAM window as an
 ...
 * SDRAM controller via `ra_sdramc_init`, then delegates to
 * ::ra_io_blockdev_ram_init using the SDRAM base address as the backing buffer
 * (writable, never read-only). The reused ::ra_io_blockdev_ram_state_t is
 ...
 * @retval k_ra_ok               Controller up and backend bound; `bd` is usable.
 * @retval k_ra_err_null_ptr     `bd` or `state` was NULL.
 * @retval k_ra_err_invalid_size `block_count` was zero or exceeds the window.
 *
 * @pre `bd` and `state` out-live every call made through the device.
 * @pre The SDRAM pins/clocks are configured for `ra_sdramc_init` to succeed.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_blockdev_sdram_init(ra_io_blockdev_t*           bd,
