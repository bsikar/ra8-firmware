/**
 * @file ra_io_stream.h
 * @brief ra_io targetable byte-stream facade -- one writer, many destinations.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 ...
 * caller-owned ::ra_io_stream_t, then write to it. This is the "do an output
 ...
 * of a `fprintf`: integers go out through ::ra_io_stream_put_u32 /
 * ::ra_io_stream_put_hex, strings through ::ra_io_stream_puts.
 *
 * @code
 * static uint8_t s_buf[256];
 * static ra_io_stream_ram_state_t s_st;
 * ra_io_stream_t out = {};
 * (void)ra_io_stream_ram_init(&out, &s_st, s_buf, sizeof(s_buf));
 * (void)ra_io_stream_puts(&out, "temp=");
 * (void)ra_io_stream_put_u32(&out, 42U);
 * (void)ra_io_stream_putc(&out, '\n');
 * @endcode
 *
...
/**
 * @struct ra_io_stream_iface
 * @brief Opaque to applications -- backends implement this internally and
 *        export a binding helper through their own header.
 *
 * @details
 * The concrete vtable layout lives in `src/ra_io_stream_internal.h`. Apps never
 * construct one; they call a sink's `_init()` helper, which binds the sink's
 * const vtable into a caller-owned ::ra_io_stream_t.
 *
 * @since 0.1.0
 */
typedef struct ra_io_stream_iface ra_io_stream_iface_t;

/**
 * @struct ra_io_stream_t
 ...
 * the `ra_io_stream_*` functions. The handle and the sink's context must
...
typedef struct {
  const ra_io_stream_iface_t* iface; /**< Bound sink vtable (private).    */
 ...
 * @retval k_ra_ok                  All `len` bytes accepted.
 * @retval k_ra_err_null_ptr        `s` or `buf` was NULL.
 * @retval k_ra_err_not_initialized No sink is bound to `s`.
 * @retval k_ra_err_no_mem          A bounded sink could not accept all bytes.
 *
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                  Pending bytes committed (or none pending).
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_stream_flush(ra_io_stream_t* s);
 ...
 * @retval k_ra_err_null_ptr        `s` was NULL.
 * @retval k_ra_err_not_initialized No sink is bound to `s`.
 * @retval k_ra_err_no_mem          A bounded sink was full.
 *
 * @pre A sink has been bound into `s`.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_stream_putc(ra_io_stream_t* s, char c);
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                  String written.
 * @retval k_ra_err_null_ptr        `s` or `str` was NULL.
 * @retval k_ra_err_not_initialized No sink is bound to `s`.
 * @retval k_ra_err_no_mem          A bounded sink could not accept all bytes.
 *
 * @pre A sink has been bound into `s`.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_stream_puts(ra_io_stream_t* s, const char* str);
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                  Digits written.
 * @retval k_ra_err_null_ptr        `s` was NULL.
 * @retval k_ra_err_not_initialized No sink is bound to `s`.
 * @retval k_ra_err_no_mem          A bounded sink could not accept the digits.
 *
 * @pre A sink has been bound into `s`.
 * @pre The stream is idle.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_stream_put_u32(ra_io_stream_t* s, uint32_t value);
 ...
 * @retval k_ra_err_null_ptr        `s` was NULL.
 * @retval k_ra_err_not_initialized No sink is bound to `s`.
 * @retval k_ra_err_invalid_arg     `min_digits` is zero or greater than eight.
 * @retval k_ra_err_no_mem          A bounded sink could not accept the digits.
 *
 * @pre A sink has been bound into `s`.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_stream_put_hex(ra_io_stream_t* s, uint32_t value, uint8_t min_digits);
