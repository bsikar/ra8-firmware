/**
 * @file ra_io_blockdev_cache.h
 * @brief ra_io caching block device -- an LRU sector cache over any backend.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 * @details
 * A decorator block device: it wraps another ::ra_io_blockdev_t and keeps a
 ...
 * static ra_io_blockdev_cache_slot_t s_cache_slots[32U];
 * static ra_io_blockdev_cache_state_t s_cache_state;
 * ra_io_blockdev_t cached = {};
 * (void)ra_io_blockdev_cache_init(&cached, &s_cache_state, &sd_bd,
...
/**
 * @struct ra_io_blockdev_cache_slot_t
...
/**
 * @struct ra_io_blockdev_cache_state_t
 * @brief Caller-owned private state for a caching block device.
 *
 * @details Zero-initialise and pass to ::ra_io_blockdev_cache_init. The
 ...
 * @retval k_ra_ok               Cache bound; `bd` is usable.
 * @retval k_ra_err_null_ptr     `bd`, `state`, `under`, `data`, or `slots` NULL.
 * @retval k_ra_err_invalid_size `n_slots` was zero.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_blockdev_cache_init(ra_io_blockdev_t*             bd,
 ...
 *
 * @pre `state` was populated by ::ra_io_blockdev_cache_init.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_blockdev_cache_stats(const ra_io_blockdev_cache_state_t* state,
                                                  uint32_t*                           out_hits,
                                                  uint32_t*                           out_misses);
