/**
 * @file ra_io_compress.h
 * @brief ra_io DEFLATE compress / decompress (zero-heap, firmware-safe).
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 ...
 * ::k_ra_io_compress_scratch_bytes and 8-byte aligned.
 *
 * This header is opt-in: it pulls in miniz, so it is NOT part of the `ra_io.h`
...
typedef enum : uint32_t {
  ...
  k_ra_io_compress_scratch_bytes = (uint32_t)sizeof(tdefl_compressor),
} ra_io_compress_const_t;
 ...
 * @retval k_ra_ok               Compressed; `*out_len` set.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_compress(const uint8_t* src,
                                      uint32_t       src_len,
                                      uint8_t*       out,
                                      uint32_t       out_cap,
                                      void*          scratch,
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok            Decompressed; `*out_len` set.
 * @retval k_ra_err_null_ptr  Any pointer argument was NULL.
 * @retval k_ra_err_no_mem    `out` was too small, or the stream is corrupt.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_decompress(const uint8_t* src,
                                        uint32_t       src_len,
                                        uint8_t*       out,
                                        uint32_t       out_cap,
                                        uint32_t*      out_len);
