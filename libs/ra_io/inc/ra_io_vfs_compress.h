 ...
 * codec, ::ra_io_compress, or `ra_fs_write_file` directly. Reading reverses the
 ...
 *
 * This header is opt-in: it pulls in ::ra_io_compress (and through it miniz), so
 * it is NOT part of the `ra_io.h` umbrella -- exactly like `ra_io_compress.h`.
 * Apps that want transparent compression include it directly and add `miniz` to
 * their `ra_add_app(... LIBS ...)`.
 *
 * @code
 * static uint8_t scratch[(size_t)k_ra_io_compress_scratch_bytes];
 * static uint8_t blob[(size_t)k_demo_blob_cap];
 * uint32_t       blob_len = 0;
 * (void)ra_io_vfs_write_compressed("ram:/STORY.RBK", payload, payload_len,
 *                                  scratch, sizeof(scratch),
 *                                  blob, sizeof(blob), &blob_len);
 * uint32_t out_len = 0;
 * (void)ra_io_vfs_read_compressed("ram:/STORY.RBK", blob, sizeof(blob),
 ...
 * the caller-provided `blob_buf` via ::ra_io_compress (using the caller-provided
 ...
 * @param[in]  scratch      Caller buffer >= ::k_ra_io_compress_scratch_bytes,
 ...
 * @retval k_ra_ok               Compressed and stored; `*out_blob_len` set.
 * @retval k_ra_err_null_ptr     Any pointer argument was NULL.
 * @retval k_ra_err_invalid_size `scratch_len` is below the minimum.
 * @retval k_ra_err_no_mem       `blob_buf` was too small for the result.
 * @retval k_ra_err_invalid_arg  `path` has no `name:` prefix.
 * @retval k_ra_err_not_found    The mount name is absent.
 * @retval k_ra_err_*            Propagated from the compressor or the VFS write.
 *
 * @pre `scratch` is at least ::k_ra_io_compress_scratch_bytes and 8-aligned.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_vfs_write_compressed(const char*    path,
                                                  const uint8_t* src,
                                                  uint32_t       src_len,
                                                  void*          scratch,
 ...
 * caller-provided `blob_buf`, then inflates it with ::ra_io_decompress into
 * `out`. Heap-free: the staging blob buffer and the output buffer are both
 * caller-provided. Reverses ::ra_io_vfs_write_compressed exactly.
 *
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Read and decompressed; `*out_len` set.
 * @retval k_ra_err_null_ptr     Any pointer argument was NULL.
 * @retval k_ra_err_invalid_arg  `path` has no `name:` prefix.
 * @retval k_ra_err_not_found    The mount name or the file is absent.
 * @retval k_ra_err_invalid_size The blob exceeded `blob_cap`.
 * @retval k_ra_err_no_mem       `out` was too small, or the blob is corrupt.
 * @retval k_ra_err_*            Propagated from the VFS read.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_vfs_read_compressed(const char* path,
                                                 uint8_t*    blob_buf,
                                                 uint32_t    blob_cap,
                                                 uint8_t*    out,
                                                 uint32_t    out_cap,
