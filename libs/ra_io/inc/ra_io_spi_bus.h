/**
 * @file ra_io_spi_bus.h
 * @brief ra_io SPI-bus facade -- one controller-transfer vtable over the
 ...
 *
 * @details
 * The RA8D2 implements SPI twice: the dedicated SPI_B block (`ra_spi.h`)
 * and SCI in Simple-SPI mode (`ra_sci_spi.h` -- the transport behind the
 ...
 *
 * Modeled directly on the ::ra_io_blockdev_t pattern: the handle
 * (::ra_io_spi_bus_t) is caller-allocated; a backend `_bind_*()` helper
 * (see `ra_io_spi_bus_spi_b.h` and `ra_io_spi_bus_sci_spi.h`) fills the
 * opaque vtable with thin trampolines to the existing `ra_spi_*` /
 * `ra_sci_spi_*` drivers, unmodified. No dynamic allocation occurs and no
 ...
 * own facade (`ra_io_i2c_bus.h`) rather than a merged one.
 *
 * ## Boundary with Ring-3 device drivers
 *
 * A Ring-3 device driver (e.g. `ra_epaper`) must not depend on this
 * Ring-4 facade -- that would invert ring ordering (see
 * `docs/RING_AND_WORLD.md`). The sanctioned bridge is
 * ::ra_io_spi_bus_as_ops, which exposes a bound bus through the Ring-3
 * seam `ra_spi_bus_ops_t` (`ra_spi_bus_ops.h`), mirroring
 * `ra_io_blockdev_as_fs_backend()`.
 *
 * @par Example:
 * @code
 * ra_io_spi_bus_t bus = {};
 * (void)ra_io_spi_bus_bind_spi_b(&bus, 0U);      // today's board
 * // (void)ra_io_spi_bus_bind_sci_spi(&bus, 0U); // future board revision
 * uint8_t rx = 0U;
 * (void)ra_io_spi_bus_xfer8(&bus, 0xA5U, &rx);
 * @endcode
 *
...
/**
 * @struct ra_io_spi_bus_iface
 * @brief Opaque to applications -- backends implement this internally and
 *        export a binding helper through their own header.
 *
 * @details
 * The concrete vtable layout lives in `src/ra_io_spi_bus_internal.h`. Apps
 * never construct one; they call a backend's `_bind_*()` helper, which binds
 * the backend's const vtable into a caller-owned ::ra_io_spi_bus_t.
 *
 * @since 0.1.0
 */
typedef struct ra_io_spi_bus_iface ra_io_spi_bus_iface_t;

/**
 * @struct ra_io_spi_bus_t
 ...
 * only through the `ra_io_spi_bus_*` functions below. The handle must
 * out-live every call made through it (including calls made through an
 * `ra_spi_bus_ops_t` filled by ::ra_io_spi_bus_as_ops).
 *
...
typedef struct {
  const ra_io_spi_bus_iface_t* iface; /**< Bound backend vtable (private).    */
 ...
 *
 * @details
 * Forwards to the bound backend's `xfer8` primitive (`ra_spi_xfer8` or
 * `ra_sci_spi_xfer8`): shifts `tx` out on COPI and captures the CIPO byte
 * into `*rx`.
 *
 ...
 * @retval k_ra_err_not_initialized No backend is bound to `bus`.
 * @retval k_ra_err_hw_timeout      The transport's status poll expired.
 *
 ...
 *
 * @see ra_io_spi_bus_write_read  Multi-frame variant.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_spi_bus_xfer8(const ra_io_spi_bus_t* bus, uint8_t tx, uint8_t* rx);

/**
 * @brief Full-duplex multi-frame transfer on the bound bus.
 *
 * @details
 * Forwards to the backend's bulk primitive (`ra_spi_write_read` or
 * `ra_sci_spi_xfer`). Either buffer may be NULL: a NULL `tx` shifts idle
 ...
 * reports ::k_ra_err_not_supported for wider `width` values.
 *
 * @param[in]  bus   Bound bus handle.
 ...
 * @param[in]  width Per-frame bit width (::ra_spi_bit_width_t).
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                  Every frame exchanged.
 * @retval k_ra_err_null_ptr        `bus` was NULL, or both buffers NULL
 *                                  with `len` > 0.
 * @retval k_ra_err_not_initialized No backend is bound to `bus`.
 * @retval k_ra_err_not_supported   Backend cannot do `width` (SCI != 8).
 * @retval k_ra_err_hw_timeout      The transport's status poll expired.
 *
 * @pre A backend has been bound into `bus`.
 ...
 * @note Not thread-safe with respect to the same channel.
 *
 * @see ra_io_spi_bus_xfer8  Single-byte variant.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_spi_bus_write_read(const ra_io_spi_bus_t* bus,
                                                const void*            tx,
                                                void*                  rx,
                                                uint32_t               len,
                                                ra_spi_bit_width_t     width);

/**
 * @brief Re-program the bound bus bit-rate without reconfiguring.
 *
 * @details
 * Forwards to `ra_spi_set_clock` / `ra_sci_spi_set_clock`. Used by
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                  Bit-rate divider reprogrammed.
 * @retval k_ra_err_null_ptr        `bus` was NULL.
 * @retval k_ra_err_not_initialized No backend is bound to `bus`.
 * @retval k_ra_err_invalid_arg     `baud_hz` zero or divider out of range.
 *
 * @pre A backend has been bound into `bus`.
 * @pre The underlying channel was initialised by its own driver init.
 ...
 */
[[nodiscard]] ra_err_t
ra_io_spi_bus_set_clock(const ra_io_spi_bus_t* bus, uint32_t baud_hz, uint32_t pclk_hz);

/**
 * @brief Expose a bound bus through the Ring-3 seam `ra_spi_bus_ops_t`.
 *
 * @details
 ...
 * for handing a bound bus to a Ring-3 device driver (e.g. `ra_epaper`)
 * without the driver depending upward on `ra_io` -- the exact analogue of
 * `ra_io_blockdev_as_fs_backend()`.
 *
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                  `*out` wired to `bus`.
 * @retval k_ra_err_null_ptr        `bus` or `out` was NULL.
 * @retval k_ra_err_not_initialized No backend is bound to `bus`.
 *
 * @pre A backend has been bound into `bus`.
 ...
 *
 * @see ra_spi_bus_ops_t  The Ring-3 seam definition.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_spi_bus_as_ops(const ra_io_spi_bus_t* bus, ra_spi_bus_ops_t* out);
