/**
 * @file ra_io_blockdev_xspi.h
 * @brief ra_io block-device backend over OSPI NOR flash via ra_xspi.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 ...
 * of 512-byte logical blocks on top of the `ra_xspi` HAL driver. NOR semantics
 ...
 * goes through `ra_xspi_flash_read`/`_program`/`_erase_sector`, which carry the
...
/**
 * @struct ra_io_blockdev_xspi_state_t
 * @brief Caller-owned private state for an OSPI NOR block device.
 *
 * @details
 * Zero-initialise and pass to ::ra_io_blockdev_xspi_init, which fills it and
 * binds it into a ::ra_io_blockdev_t. Treat the fields as private. The state
 ...
 * whole-sector aligned (see ::ra_io_blockdev_xspi_init).
 *
 ...
 * have brought up the controller with `ra_xspi_init(instance, mode)` before any
 ...
 * @retval k_ra_err_invalid_arg  `instance` >= 2, `base_off` not sector-aligned,
 *                               or `block_count` not a non-zero multiple of 8.
 *
 * @pre `ra_xspi_init(instance, mode)` has already succeeded.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_blockdev_xspi_init(ra_io_blockdev_t*            bd,
