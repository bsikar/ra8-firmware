/**
 * @file ra_io_i2c_bus.h
 * @brief ra_io I2C-bus facade -- one controller-transfer vtable over the
 ...
 *
 * @details
 * The RA8D2 implements I2C twice: the classic RIIC block (`ra_i2c.h`,
 * three channels) and the I3C block's I2C-compatibility mode (`ra_i3c.h`,
 ...
 * (::ra_io_i2c_bus_t) is caller-allocated; a backend `_bind_*()` helper
 * (see `ra_io_i2c_bus_riic.h` and `ra_io_i2c_bus_i3c_compat.h`) fills the
 * opaque vtable with thin trampolines to the existing `ra_i2c_*` /
 * `ra_i3c_*` drivers, unmodified. No dynamic allocation occurs and no MMIO
 ...
 * facade is deliberately separate from `ra_io_spi_bus.h`: I2C carries an
 ...
 *
 * A Ring-3 device driver (e.g. `ra_touch`, `ra_smbus`) must not depend on
 * this Ring-4 facade -- that would invert ring ordering (see
 * `docs/RING_AND_WORLD.md`). The sanctioned bridge is
 * ::ra_io_i2c_bus_as_ops, which exposes a bound bus through the Ring-3
 * seam `ra_i2c_bus_ops_t` (`ra_i2c_bus_ops.h`), mirroring
 ...
 * uint8_t status     = 0U;
 * (void)ra_io_i2c_bus_transfer(&bus, 0x5DU, reg_ptr, 2U, &status, 1U);
 * @endcode
 *
...
/**
 * @struct ra_io_i2c_bus_iface
 * @brief Opaque to applications -- backends implement this internally and
 *        export a binding helper through their own header.
 *
 * @details
 * The concrete vtable layout lives in `src/ra_io_i2c_bus_internal.h`. Apps
 * never construct one; they call a backend's `_bind_*()` helper, which binds
 * the backend's const vtable into a caller-owned ::ra_io_i2c_bus_t.
 *
 * @since 0.1.0
 */
typedef struct ra_io_i2c_bus_iface ra_io_i2c_bus_iface_t;

/**
 * @struct ra_io_i2c_bus_t
 ...
 * only through the `ra_io_i2c_bus_*` functions below. The handle must
 * out-live every call made through it (including calls made through an
 * `ra_i2c_bus_ops_t` filled by ::ra_io_i2c_bus_as_ops).
 *
...
typedef struct {
  const ra_io_i2c_bus_iface_t* iface; /**< Bound backend vtable (private).    */
 ...
 *
 * @details
 * Forwards to the bound backend's write primitive (`ra_i2c_write` or
 * `ra_i3c_write`; the I3C driver's inverted `restart` flag is adapted by
 ...
 * @retval k_ra_err_null_ptr        `bus` was NULL, or `data` NULL with
 *                                  `len` > 0.
 * @retval k_ra_err_not_initialized No backend is bound to `bus`.
 * @retval k_ra_err_nack            Peripheral did not acknowledge.
 ...
 *
 * @see ra_io_i2c_bus_transfer  Write-then-read in one transaction.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_i2c_bus_write(const ra_io_i2c_bus_t* bus,
                                           uint8_t                addr,
                                           const uint8_t*         data,
                                           uint32_t               len,
                                           bool                   send_stop);
 ...
 *
 * @details
 * Forwards to the bound backend's read primitive (`ra_i2c_read` or
 * `ra_i3c_read`). Issues START, the address byte with R/W# = 1, drains
 ...
 * @retval k_ra_err_null_ptr        `bus` or `data` was NULL.
 * @retval k_ra_err_not_initialized No backend is bound to `bus`.
 ...
 * @note Not thread-safe with respect to the same channel.
 *
 * @see ra_io_i2c_bus_write  Controller-write counterpart.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t
ra_io_i2c_bus_read(const ra_io_i2c_bus_t* bus, uint8_t addr, uint8_t* data, uint32_t len);

/**
 * @brief Combined write-then-RESTART-then-read in one bus transaction.
 *
 * @details
 * Forwards to the bound backend's combined primitive (`ra_i2c_transfer`
 * or `ra_i3c_transfer`) -- the classic "address a register, read its
 ...
 * @retval k_ra_err_null_ptr        `bus` was NULL, or a required buffer
 *                                  was NULL.
 * @retval k_ra_err_not_initialized No backend is bound to `bus`.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_i2c_bus_transfer(const ra_io_i2c_bus_t* bus,
                                              uint8_t                addr,
                                              const uint8_t*         wr,
                                              uint32_t               wr_len,
                                              uint8_t*               rd,
...
/**
 * @brief Expose a bound bus through the Ring-3 seam `ra_i2c_bus_ops_t`.
 *
 * @details
 ...
 * (e.g. `ra_touch`, `ra_smbus`) without the driver depending upward on
 * `ra_io` -- the exact analogue of `ra_io_blockdev_as_fs_backend()`.
 *
 ...
 *
 * @see ra_i2c_bus_ops_t  The Ring-3 seam definition.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_i2c_bus_as_ops(const ra_io_i2c_bus_t* bus, ra_i2c_bus_ops_t* out);
