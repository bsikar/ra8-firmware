/**
 * @file ra_io_spi_bus_sci_spi.h
 * @brief SCI Simple-SPI backend binder for the ra_io SPI-bus facade.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 * @details
 * Binds a caller-owned ::ra_io_spi_bus_t to an SCI channel running in
 * Simple-SPI mode via thin trampolines to `ra_sci_spi_xfer8` /
 * `ra_sci_spi_xfer` / `ra_sci_spi_set_clock` (`ra_sci_spi.h`, unmodified).
 * This is the transport behind the EK-RA8D2 Pmod2 microSD path. The
 * channel must already be initialised through `ra_sci_spi_init` -- the
 * facade owns transfers only, never bring-up.
 *
 * The SCI block clocks 8-bit frames only, so
 * `ra_io_spi_bus_write_read` on this backend reports
 * ::k_ra_err_not_supported for 16/32-bit widths.
 *
 ...
 * with `ra_sci_spi_init(channel, ...)` (before or after binding) and own
 ...
 * @retval k_ra_ok              `bus` now dispatches to `ra_sci_spi_*`.
 ...
 *
 * @see ra_io_spi_bus_bind_spi_b  The dedicated SPI_B twin.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_spi_bus_bind_sci_spi(ra_io_spi_bus_t* bus, uint8_t channel);
