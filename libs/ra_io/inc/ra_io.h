/**
 * @file ra_io.h
 * @brief ra_io -- unified peripheral-agnostic I/O fabric (umbrella header).
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 * @details
 * `ra_io` is one central I/O subsystem with swappable backends, built so an
 ...
 * backend choice, exactly as selecting a display backend is in `ra_display_pal`.
 *
 * The subsystem is layered internally but ships as one coherent facade:
 *
 *       ra_io  (one public facade: block devices, streams, files, compression)
 ...
 *
 * This milestone ships the block-device fabric (`ra_io_blockdev.h`): a single
 * 512-byte LBA block-device vtable with one backend per storage medium, plus a
 * bridge that exposes any backend as an `ra_fs_backend_t` so the existing FAT
