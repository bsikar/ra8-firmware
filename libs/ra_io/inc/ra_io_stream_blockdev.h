/**
 * @file ra_io_stream_blockdev.h
 * @brief ra_io byte-stream sink that buffers into a raw block device.
 *
 * @par Tag
 * [Ring 4 / PAL] {World: NS}
 *
 ...
 * ::ra_io_blockdev_t starting at a chosen LBA: bytes fill an internal sector
 * buffer; a full sector is committed automatically; ::ra_io_stream_flush commits
...
/**
 * @struct ra_io_stream_blockdev_state_t
 * @brief Caller-owned private state for a block-device stream sink.
 *
 * @details
 * Zero-initialise and pass to ::ra_io_stream_blockdev_init. Treat the fields as
 * private. The bound block device and this state must out-live the stream.
 *
 * @invariant `fill < k_ra_io_block_size_bytes`.
 *
 * @since 0.1.0
 */
 ...
 * @retval k_ra_err_null_ptr `s`, `state`, or `bd` was NULL.
 *
 ...
 *       ::ra_io_stream_flush to commit a partial trailing sector.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_stream_blockdev_init(ra_io_stream_t*                s,
