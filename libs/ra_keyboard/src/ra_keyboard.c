...
/**
 * @brief Append one key descriptor to the layout, bounded by k_ra_kbd_max_keys.
 *
 * @details
 * Writes a single ra_kbd_key_t entry into kb->keys[kb->count] and increments
 * kb->count.  If the layout is already at capacity (kb->count >=
 * k_ra_kbd_max_keys) the call is silently discarded so callers do not need
 ...
 * @param[in]     aux  Auxiliary data; for k_ra_kbd_key_layer this is the target
 *                     layer index, otherwise 0.
 *
 * @return Nothing.
 *
 * @pre  kb is not nullptr.
 * @pre  kb->count <= k_ra_kbd_max_keys on entry.
 * @post If kb->count was less than k_ra_kbd_max_keys, kb->count is incremented
 *       by exactly one and the new key slot is fully initialised.
 * @post If kb->count was already equal to k_ra_kbd_max_keys, kb is unchanged.
 *
 * @note Not thread-safe; must be called from the same context as the owning
 *       ra_kbd_layout_t initialisation.
 *
 * @since 0.1.0
 */
 ...
 * @post Exactly min(n, k_ra_kbd_max_keys - kb->count) new char keys have been
 *       appended to kb->keys.
 ...
 * @param[in]     aux  Auxiliary data; for k_ra_kbd_key_layer this is the
 ...
 * 1. A k_ra_kbd_key_layer special key spanning [0, k_kbd_act_hu) whose aux
 ...
 * 3. A k_ra_kbd_key_backspace special key spanning
 ...
 *                        aux field (k_ra_kbd_layer_symbols or
 *                        k_ra_kbd_layer_numbers).
 *
 * @return Nothing.
 *
 * @pre  kb and f are not nullptr.
 * @pre  tog_aux is a valid ra_kbd_layer_t cast to uint8_t.
 * @post Seven keys (1 layer-toggle + 5 punctuation + 1 backspace) are appended
 *       to kb->keys, subject to the k_ra_kbd_max_keys capacity guard.
 ...
 * 2. A k_ra_kbd_key_space key spanning
 *    [k_kbd_act_hu, k_kbd_act_hu + k_kbd_space_hu).
 * 3. A k_ra_kbd_key_enter key spanning
 *    [k_kbd_hu_div - k_kbd_act_hu, k_kbd_hu_div).
 ...
 *
 * @pre  kb and f are not nullptr.
 * @pre  left_aux is a valid ra_kbd_layer_t cast to uint8_t.
 * @post Three keys (layer-toggle, space, enter) are appended to kb->keys,
 *       subject to the k_ra_kbd_max_keys capacity guard.
 ...
 * @param[in]     f   Bounding rectangle of the keyboard widget.
 * @param[in]     rh  Row height in pixels (f->h / k_ra_kbd_rows).
 *
 * @return Nothing.
 *
 * @pre  kb and f are not nullptr.
 * @pre  rh > 0 (non-zero row height, guaranteed by ra_kbd_layout_init).
 * @post All letter-layer keys are appended to kb->keys, up to the
 *       k_ra_kbd_max_keys capacity limit.
 ...
 *          priv_row_punct() with tog_aux = k_ra_kbd_layer_symbols.
 ...
 *          priv_row_punct() with tog_aux = k_ra_kbd_layer_numbers.
 * - Row 3: layer-toggle to letters, SPACE, RETURN via priv_row_bottom().
 *
 ...
 * kb->frame.h / k_ra_kbd_rows, then dispatches to the appropriate layer
 * builder:
 * - k_ra_kbd_layer_numbers -> priv_build_numbers()
 * - k_ra_kbd_layer_symbols -> priv_build_symbols()
 * - any other value (including k_ra_kbd_layer_letters) -> priv_build_letters()
 * Called once at init and again every time the active layer changes (via
 * ra_kbd_apply() handling k_ra_kbd_key_layer).
 *
 ...
 *
 * @pre  kb is not nullptr.
 * @pre  kb->frame.h > 0 and kb->frame.w > 0 (ensured by ra_kbd_layout_init).
 ...
 *
 * @details
 * Checks whether t->len is strictly less than (k_ra_kbd_text_max - 1) to
 ...
 * @pre  t->len <= k_ra_kbd_text_max - 1 on entry (invariant maintained by
 *       this function and ra_kbd_apply()).
 * @post If t->len was less than (k_ra_kbd_text_max - 1), t->len is incremented
 *       by one and t->buf[t->len] == '\0'.
 * @post t->buf is always NUL-terminated on exit.
 *
 * @note Not thread-safe; must be called from the same context as ra_kbd_apply().
 *
 * @since 0.1.0
 */
