 ...
 * index of the next free cell (::k_ra_slab_nil terminates). `memcpy` is used to
...
static const char* const s_tag = "ra_slab";
 ...
 *
 * @return The stored next-cell index, or ::k_ra_slab_nil at the list tail.
 * @retval k_ra_slab_nil Cell @p idx is the tail of the freelist.
 *
 * @pre `slab` and `slab->base` are non-NULL.
 * @pre `idx` addresses a cell currently on the freelist.
 * @post No state is modified.
 * @post The returned value is a valid index or ::k_ra_slab_nil.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
static uint32_t priv_slab_next(const ra_slab_t* slab, uint32_t idx)
{
  uint32_t next = 0U;
 ...
 * @param[in]     next Next-cell index to store (or ::k_ra_slab_nil).
 *
 * @return Nothing.
 *
 * @pre `slab` and `slab->base` are non-NULL.
 ...
 * @since 0.1.0
 */
static void priv_slab_set_next(ra_slab_t* slab, uint32_t idx, uint32_t next)
{
  (void)memcpy(&slab->base[(size_t)idx * (size_t)slab->cell_bytes], &next, sizeof(next));
}

ra_err_t ra_slab_init(ra_slab_t* slab, void* buffer, uint32_t buffer_bytes, uint32_t cell_bytes)
{
  RA_CHECK_NULL_PTR(slab, s_tag, "slab must not be nullptr");
  RA_CHECK_NULL_PTR(buffer, s_tag, "buffer must not be nullptr");
  if (cell_bytes < (uint32_t)k_ra_slab_min_cell_bytes) {
    return k_ra_err_invalid_size;
  }
  if ((cell_bytes % (uint32_t)k_ra_slab_align_bytes) != 0U) {
    return k_ra_err_invalid_size;
  }
  ...
  for (uint32_t i = 0U; i < count; ++i) {
    const uint32_t next = ((i + 1U) < count) ? (i + 1U) : (uint32_t)k_ra_slab_nil;
...
ra_err_t ra_slab_alloc(ra_slab_t* slab, void** out_cell)
{
  RA_CHECK_NULL_PTR(slab, s_tag, "slab must not be nullptr");
  RA_CHECK_NULL_PTR(out_cell, s_tag, "out_cell must not be nullptr");
  if (slab->free_head == (uint32_t)k_ra_slab_nil) {
...
ra_err_t ra_slab_free(ra_slab_t* slab, void* cell)
{
  RA_CHECK_NULL_PTR(slab, s_tag, "slab must not be nullptr");
  RA_CHECK_NULL_PTR(cell, s_tag, "cell must not be nullptr");
...
ra_err_t ra_slab_stats(const ra_slab_t* slab, uint32_t* out_free, uint32_t* out_total)
{
  RA_CHECK_NULL_PTR(slab, s_tag, "slab must not be nullptr");
