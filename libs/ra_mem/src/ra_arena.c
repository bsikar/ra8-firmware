...
static const char* const s_tag = "ra_arena";
...
ra_err_t ra_arena_init(ra_arena_t* arena, void* base, uint32_t size)
{
  RA_CHECK_NULL_PTR(arena, s_tag, "arena must not be nullptr");
  RA_CHECK_NULL_PTR(base, s_tag, "base must not be nullptr");
  if (size == 0U) {
...
ra_err_t ra_arena_carve(ra_arena_t* arena, uint32_t bytes, uint32_t align, void** out_ptr)
{
  RA_CHECK_NULL_PTR(arena, s_tag, "arena must not be nullptr");
...
ra_err_t ra_arena_remaining(const ra_arena_t* arena, uint32_t* out_remaining)
{
  RA_CHECK_NULL_PTR(arena, s_tag, "arena must not be nullptr");
  RA_CHECK_NULL_PTR(out_remaining, s_tag, "out_remaining must not be nullptr");
