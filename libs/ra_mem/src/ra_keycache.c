...
static const char* const s_tag = "ra_keycache";

/**
 * @enum ra_keycache_hash_const_t
 ...
 * @since 0.1.0
 */
static uint8_t* priv_cell_ptr(const ra_keycache_t* kc, uint32_t idx)
{
 ...
 * @since 0.1.0
 */
static uint8_t* priv_key_ptr(const ra_keycache_t* kc, uint32_t idx)
{
 ...
 * @since 0.1.0
 */
static void* priv_user_ptr(const ra_keycache_t* kc, uint32_t idx)
{
 ...
 * @since 0.1.0
 */
static bool priv_key_eq(const ra_keycache_t* kc, const void* a, const void* b)
{
 ...
 * @since 0.1.0
 */
static uint32_t priv_hash(const ra_keycache_t* kc, const void* key)
{
 ...
 * @since 0.1.0
 */
static void priv_unlink(ra_keycache_t* kc, int32_t f)
{
  ra_keycache_cell_t* m = kc->cfg.meta;
  if (m[f].prev != -1) {
 ...
 * @since 0.1.0
 */
static void priv_push_head(ra_keycache_t* kc, int32_t f)
{
  ra_keycache_cell_t* m = kc->cfg.meta;
  m[f].prev             = -1;
 ...
 * @since 0.1.0
 */
static void priv_hash_insert(ra_keycache_t* kc, int32_t f)
{
 ...
 * @since 0.1.0
 */
static void priv_hash_remove(ra_keycache_t* kc, int32_t f)
{
  const uint32_t b   = priv_hash(kc, priv_key_ptr(kc, (uint32_t)f));
 ...
 * @since 0.1.0
 */
static int32_t priv_hash_lookup(const ra_keycache_t* kc, const void* key)
{
  ...
  for (uint32_t guard = 0U; guard < kc->cfg.cell_count; ++guard) {
    ...
    const ra_keycache_cell_t* m = &kc->cfg.meta[cur];
    if (m->valid != 0U) {
 ...
 * @since 0.1.0
 */
static int32_t priv_pick_victim(const ra_keycache_t* kc)
{
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok         Cell rendered, inserted, and pinned.
 * @retval k_ra_err_no_mem Every cell is pinned.
 * @retval k_ra_err_*      The render callback's error (the victim is dropped).
 *
 ...
 * @since 0.1.0
 */
static ra_err_t priv_miss(ra_keycache_t* kc, const void* key, ra_keycache_view_t* out_view)
{
  const int32_t v = priv_pick_victim(kc);
  if (v < 0) {
    return k_ra_err_no_mem;
  }
  ra_keycache_cell_t* m = &kc->cfg.meta[v];
  if (m->valid != 0U) {
  ...
  const ra_err_t rerr = kc->cfg.render(kc->cfg.render_ctx, key, cell, kc->cfg.cell_bytes, user);
  if (rerr != k_ra_ok) {
    m->valid = 0U;
    priv_push_head(kc, v);
    return rerr;
  ...
  priv_push_head(kc, v);
  *out_view = (ra_keycache_view_t){.data = cell, .user = user};
  return k_ra_ok;
}

ra_err_t ra_keycache_get(ra_keycache_t* kc, const void* key, ra_keycache_view_t* out_view)
{
  RA_CHECK_NULL_PTR(kc, s_tag, "kc must not be nullptr");
  RA_CHECK_NULL_PTR(key, s_tag, "key must not be nullptr");
  RA_CHECK_NULL_PTR(out_view, s_tag, "out_view must not be nullptr");
  ...
  if (f >= 0) {
    ...
    priv_unlink(kc, f);
    priv_push_head(kc, f);
    *out_view = (ra_keycache_view_t){.data = priv_cell_ptr(kc, (uint32_t)f),
...
ra_err_t ra_keycache_put(ra_keycache_t* kc, const uint8_t* data)
{
  RA_CHECK_NULL_PTR(kc, s_tag, "kc must not be nullptr");
...
ra_err_t ra_keycache_stats(const ra_keycache_t* kc,
                           uint32_t*            out_hits,
                           uint32_t*            out_misses,
