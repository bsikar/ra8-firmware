...
static const char* const s_tag = "ra_vsource";

ra_err_t ra_vsource_init(ra_vsource_t* vs, ra_vsource_obj_t* objs, uint32_t cap)
{
  RA_CHECK_NULL_PTR(vs, s_tag, "vs must not be nullptr");
  RA_CHECK_NULL_PTR(objs, s_tag, "objs must not be nullptr");
  if (cap == 0U) {
...
ra_err_t ra_vsource_add_paged(ra_vsource_t*      vs,
...
{
  RA_CHECK_NULL_PTR(vs, s_tag, "vs must not be nullptr");
  RA_CHECK_NULL_PTR(read, s_tag, "read must not be nullptr");
...
ra_err_t ra_vsource_loader(void*    ctx,
                           uint32_t object_id,
                           uint64_t offset,
                           uint8_t* frame,
                           uint32_t frame_bytes)
{
  ...
  const ra_vsource_t* vs = (const ra_vsource_t*)ctx;
  if (object_id >= vs->count) {
    return k_ra_err_out_of_range;
  }
  const ra_vsource_obj_t* o = &vs->objs[object_id];
  if (offset >= o->size) {
    return k_ra_err_out_of_range;
  }
...
ra_err_t ra_vsource_xip_ptr(const ra_vsource_t* vs,
                            uint32_t            object_id,
                            uint64_t            offset,
                            uint32_t            len,
...
{
  RA_CHECK_NULL_PTR(vs, s_tag, "vs must not be nullptr");
  RA_CHECK_NULL_PTR(out_ptr, s_tag, "out_ptr must not be nullptr");
  if (object_id >= vs->count) {
    return k_ra_err_out_of_range;
  }
  const ra_vsource_obj_t* o = &vs->objs[object_id];
