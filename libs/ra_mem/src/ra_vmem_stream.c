 ...
 * one at a time: page in the frame that holds `cur` (`ra_vmem_get`), copy the
 * in-frame slice, release the pin (`ra_vmem_put`), and advance. At most one frame
 * is pinned at any instant, so the resident set is the caller's fixed `ra_vmem`
...
static const char* const s_tag = "ra_vmem_stream";

ra_err_t ra_vmem_stream_init(ra_vmem_stream_t* st, ra_vmem_t* vm, uint32_t object_id, uint64_t size)
{
  ...
  if (fb == 0U) {
    return k_ra_err_invalid_size; /* GCOVR_EXCL_LINE -- ra_vmem_init rejects a 0 frame size, so a live cache never has one */
  }
  st->vm          = vm;
...
{
  ra_vmem_stream_t* st = (ra_vmem_stream_t*)ctx;
  if (st == nullptr) {
    return 0U;
  }
  if (buf == nullptr) {
  ...
  while (done < want) {
    ...
    void* page = nullptr;
    if (ra_vmem_get(st->vm, st->object_id, frame_base, &page) != k_ra_ok) {
    ...
    if (ra_vmem_put(st->vm, page) != k_ra_ok) {
