 ...
 * with ::ra_keycache_put once it is done reading the cell.
 *
 ...
 * layer down in the ::ra_vmem page cache, where linear file floods happen; the
 ...
 * tile width/height) and ::ra_keycache_get hands the descriptor back alongside
 * the cell data. This is how a typed facade (::ra_glyph_atlas, ::ra_tile_cache)
 ...
 * link metadata, and the hash buckets -- carved once from a tier ::ra_arena /
 * ::ra_slab at init (hot tier = SRAM/DTCM).
 *
 ...
 * @since 0.1.0
 */
typedef ra_err_t (*ra_keycache_render_fn)(void*       ctx,
                                          const void* key,
                                          uint8_t*    cell,
                                          uint32_t    cell_bytes,
                                          void*       user);
...
/**
 * @struct ra_keycache_cfg_t
 * @brief Caller-supplied storage + renderer for ::ra_keycache_init.
 *
...
typedef struct {
  ...
  ra_keycache_render_fn render;       /**< Render-on-miss callback.                    */
  void*                 render_ctx;   /**< Opaque context passed to @c render.         */
} ra_keycache_cfg_t;

/**
 * @struct ra_keycache_t
...
/**
 * @struct ra_keycache_view_t
 * @brief A pinned view of a cached cell returned by ::ra_keycache_get.
 *
 * @details `data` points into the cell storage and stays valid until the
 *          matching ::ra_keycache_put. `user` points at this cell's user
 ...
 *          Copy out any descriptor fields before ::ra_keycache_put.
 *
 * @since 0.1.0
 */
typedef struct {
 ...
 * @param[in]  cfg Storage + renderer configuration (see ::ra_keycache_cfg_t).
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Cache ready; all cells cold.
 * @retval k_ra_err_null_ptr     `kc`, `cfg`, or a required `cfg` pointer is NULL.
 * @retval k_ra_err_invalid_size `cell_count`, `cell_bytes`, `key_bytes`, or
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_keycache_init(ra_keycache_t* kc, const ra_keycache_cfg_t* cfg);
 ...
 *          stays valid until ::ra_keycache_put.
 *
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           Cell resident and pinned; `*out_view` set.
 * @retval k_ra_err_null_ptr `kc`, `key`, or `out_view` was NULL.
 * @retval k_ra_err_no_mem   Every cell is pinned (cannot evict for the miss).
 * @retval k_ra_err_*        The render callback failed (returned verbatim).
 *
 * @pre `kc` was populated by ::ra_keycache_init.
 * @pre The caller will ::ra_keycache_put the returned cell.
 ...
 */
[[nodiscard]] ra_err_t
ra_keycache_get(ra_keycache_t* kc, const void* key, ra_keycache_view_t* out_view);

/**
 * @brief Release one pin on a cell previously returned by ::ra_keycache_get.
 *
 * @param[in] kc   Initialised cache.
 * @param[in] data The `data` pointer from a returned ::ra_keycache_view_t.
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok              Pin released.
 * @retval k_ra_err_null_ptr    `kc` or `data` was NULL.
 * @retval k_ra_err_invalid_arg `data` is not a cell of this cache, or the cell
 *                              was not pinned.
 *
 * @pre `data` came from ::ra_keycache_get on this cache and is still pinned.
 * @pre `kc` was populated by ::ra_keycache_init.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_keycache_put(ra_keycache_t* kc, const uint8_t* data);
 ...
 * @retval k_ra_err_null_ptr    `kc` was NULL.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_keycache_stats(const ra_keycache_t* kc,
                                         uint32_t*            out_hits,
                                         uint32_t*            out_misses,
