 ...
 *
 * @details
 * Layer 0 of the #147 memory hierarchy, paired with ::ra_slab. An arena owns one
 ...
 * ra_arena_t sdram = {};
 * (void)ra_arena_init(&sdram, __sdram_pool_start, 40U * 1024U * 1024U);
 * void* frame_backing = nullptr;
 * (void)ra_arena_carve(&sdram, 64U * 4096U, 8U, &frame_backing); // 64 x 4 KiB
 * ra_slab_t frames = {};
 * (void)ra_slab_init(&frames, frame_backing, 64U * 4096U, 4096U);
 * @endcode
 *
 ...
 *
 * @details Zero-initialise and pass to ::ra_arena_init. The backing region must
 ...
 * @retval k_ra_ok               Arena ready (empty).
 * @retval k_ra_err_null_ptr     `arena` or `base` was NULL.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_arena_init(ra_arena_t* arena, void* base, uint32_t size);
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Block carved; `*out_ptr` set.
 * @retval k_ra_err_null_ptr     `arena` or `out_ptr` was NULL.
 * @retval k_ra_err_invalid_size `bytes` was zero.
 * @retval k_ra_err_invalid_arg  `align` was zero or not a power of two.
 * @retval k_ra_err_no_mem       The aligned block does not fit the remainder.
 *
 * @pre `arena` was populated by ::ra_arena_init.
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           Remaining bytes reported.
 * @retval k_ra_err_null_ptr `arena` or `out_remaining` was NULL.
 *
 * @pre `arena` was populated by ::ra_arena_init.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_arena_remaining(const ra_arena_t* arena, uint32_t* out_remaining);
