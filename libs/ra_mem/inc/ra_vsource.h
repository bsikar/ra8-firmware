 ...
 *
 * @details
 * Layer 1 of the #147 memory hierarchy. ::ra_vmem (Layer 2) pages objects in
 ...
 *    `ra_io` block device or `ra_fs` file. Paged objects are demand-paged through
 *    the Layer-2 cache.
 *  - **XIP** -- backed by an already memory-mapped, execute-in-place region
 *    (OSPI flash). Resident assets (fonts!) cost zero SRAM: ::ra_vsource_xip_ptr
 *    hands back a real pointer with no copy and no cache frame.
 *
 * ::ra_vsource_loader is the ::ra_vmem_loader_fn the cache calls on a miss: it
...
/**
 * @typedef ra_vsource_read_fn
 * @brief Read `len` bytes at `offset` from a paged object's backing.
 *
 * @details The storage seam: the app binds this to an `ra_io` block device or an
 *          `ra_fs` file so ::ra_vsource stays storage-agnostic.
 *
 ...
 *
 * @return ra_err_t k_ra_ok on success; any error aborts the page-in.
 *
 * @since 0.1.0
 */
typedef ra_err_t (*ra_vsource_read_fn)(void* ctx, uint64_t offset, uint8_t* buf, uint32_t len);

/**
 * @struct ra_vsource_obj_t
 * @brief One registered object's backing (paged or XIP).
 *
 * @details Treat as private; populated by ::ra_vsource_add_paged /
 *          ::ra_vsource_add_xip. `xip != NULL` marks an XIP object.
 *
 * @since 0.1.0
 */
 ...
 */
typedef struct {
  ra_vsource_obj_t* objs;  /**< `cap` object slots (caller-owned).     */
 ...
 * @retval k_ra_ok               Registry ready (empty).
 * @retval k_ra_err_null_ptr     `vs` or `objs` was NULL.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_vsource_init(ra_vsource_t* vs, ra_vsource_obj_t* objs, uint32_t cap);
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Registered; `*out_id` set.
 * @retval k_ra_err_null_ptr     `vs`, `read`, or `out_id` was NULL.
 * @retval k_ra_err_invalid_size `size` was zero.
 * @retval k_ra_err_no_mem       The registry is full.
 *
 * @pre `read` reads the object's bytes for offsets in `[base, base + size)`.
 * @pre `vs` was populated by ::ra_vsource_init.
 * @post On success the object is addressable via ::ra_vsource_loader.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_vsource_add_paged(ra_vsource_t*      vs,
                                            ra_vsource_read_fn read,
                                            void*              ctx,
                                            uint64_t           base,
                                            uint64_t           size,
                                            uint32_t*          out_id);
 ...
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Registered; `*out_id` set.
 * @retval k_ra_err_null_ptr     `vs`, `xip_base`, or `out_id` was NULL.
 * @retval k_ra_err_invalid_size `size` was zero.
 * @retval k_ra_err_no_mem       The registry is full.
 *
 * @pre `xip_base` is a readable region of at least `size` bytes.
 * @pre `vs` was populated by ::ra_vsource_init.
 * @post On success ::ra_vsource_xip_ptr returns pointers into the region.
 * @post On any non-ok return the registry is unchanged.
 *
 * @note Not thread-safe.
 *
 ...
 *          The signature matches `ra_vmem_loader_fn`, so pass `vs` as the cache's
 *          `loader_ctx` and this function as its `loader`.
 *
 * @param[in]  ctx         The ::ra_vsource_t registry (as a void cookie).
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Frame filled (zero-padded past the object end).
 * @retval k_ra_err_null_ptr     `ctx` or `frame` was NULL.
 * @retval k_ra_err_out_of_range `object_id` is not registered, or `offset` is at
 *                               or beyond the object size.
 * @retval k_ra_err_*            The read callback's error (paged objects).
 *
 * @pre `ctx` is a populated ::ra_vsource_t.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_vsource_loader(void*    ctx,
                                         uint32_t object_id,
                                         uint64_t offset,
                                         uint8_t* frame,
 ...
 * @retval k_ra_err_null_ptr      `vs` or `out_ptr` was NULL.
 * @retval k_ra_err_out_of_range  Unknown id, or `offset + len` exceeds the size.
 * @retval k_ra_err_not_supported The object is paged, not XIP.
 *
 * @pre `vs` was populated by ::ra_vsource_init.
 * @pre `object_id` was registered via ::ra_vsource_add_xip.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_vsource_xip_ptr(const ra_vsource_t* vs,
                                          uint32_t            object_id,
                                          uint64_t            offset,
                                          uint32_t            len,
