 ...
 *
 * @details
 * A thin adapter that turns a ::ra_vmem-cached paged object into a random-access
 * byte-stream reader: ::ra_vmem_stream_read serves an arbitrary
 ...
 * is call-compatible with `ra_epub_open_streamed()`'s `ra_epub_stream_read_fn`,
 * which is how a large `.epub` on the SD card is opened without whole-file
 * residency (#151): register the file as a ::ra_vsource paged object, front it
 * with a fixed ::ra_vmem pool (the asserted RAM budget), and hand the resulting
 * ::ra_vmem_stream_read to the EPUB reader.
 *
 * Zero allocation (NASA P10 Rule 3): all storage is the caller's ::ra_vmem pool;
...
/**
 * @struct ra_vmem_stream_t
 * @brief Binds one page-cached object to the byte-stream read adapter.
 *
 * @details Caller-owned; populate with ::ra_vmem_stream_init and pass its address
 *          as the `ctx` of ::ra_vmem_stream_read. Treat the fields as private.
 *
 * @invariant `frame_bytes == vm->cfg.frame_bytes` and `frame_bytes > 0`.
 * @since 0.1.0
 */
typedef struct {
  ra_vmem_t* vm;          /**< Initialised page cache (fixed pool = RAM budget). */
 ...
 * @retval k_ra_ok               Bound; ::ra_vmem_stream_read may be used.
 * @retval k_ra_err_null_ptr     `st` or `vm` was NULL.
 * @retval k_ra_err_invalid_size `size` was 0, or `vm`'s frame size was 0.
 *
 * @pre `vm` was populated by ::ra_vmem_init and its loader serves @p object_id.
 * @pre `st` out-lives every ::ra_vmem_stream_read call that uses it.
 ...
 */
[[nodiscard]] ra_err_t
ra_vmem_stream_init(ra_vmem_stream_t* st, ra_vmem_t* vm, uint32_t object_id, uint64_t size);
 ...
 *          consumer like `ra_epub_open_streamed()` treats as end-of-file.
 *
 * @param[in]  ctx    The ::ra_vmem_stream_t binding (as a void cookie).
 ...
 * @since 0.1.0
 */
size_t ra_vmem_stream_read(void* ctx, uint64_t offset, void* buf, size_t len);
