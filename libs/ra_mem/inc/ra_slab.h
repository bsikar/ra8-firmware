 ...
 * first 4 bytes hold the index of the next free cell (::k_ra_slab_nil
 ...
 * void* f = nullptr;
 * if (ra_slab_alloc(&frames, &f) == k_ra_ok) { ... ra_slab_free(&frames, f); }
 * @endcode
 *
 ...
 *
 * @details Zero-initialise and pass to ::ra_slab_init. The backing buffer must
 *          out-live the slab. Treat the fields as private.
 *
 * @invariant `free_count <= cell_count`.
 * @invariant `free_head` is either ::k_ra_slab_nil or a valid cell index.
 *
 * @since 0.1.0
 */
typedef struct {
  ...
  uint32_t free_head;  /**< Index of the first free cell, or ::k_ra_slab_nil. */
 ...
 * @retval k_ra_ok               Slab ready; all cells free.
 * @retval k_ra_err_null_ptr     `slab` or `buffer` was NULL.
 * @retval k_ra_err_invalid_size `cell_bytes` < 4, not 4-aligned, or larger than
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           A cell was handed out; `*out_cell` set.
 * @retval k_ra_err_null_ptr `slab` or `out_cell` was NULL.
 * @retval k_ra_err_no_mem   The slab is exhausted (no free cell).
 *
 * @pre `slab` was populated by ::ra_slab_init.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_slab_alloc(ra_slab_t* slab, void** out_cell);

/**
 * @brief Return a previously-allocated cell to the slab (O(1)).
 *
 * @param[in] slab Initialised slab.
 * @param[in] cell A cell pointer previously returned by ::ra_slab_alloc.
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                 Cell returned to the freelist.
 * @retval k_ra_err_null_ptr       `slab` or `cell` was NULL.
 * @retval k_ra_err_invalid_arg    `cell` is outside the slab or not on a cell
 *                                 boundary.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_slab_free(ra_slab_t* slab, void* cell);
 ...
 * @retval k_ra_err_null_ptr `slab` was NULL.
 *
 * @pre `slab` was populated by ::ra_slab_init.
