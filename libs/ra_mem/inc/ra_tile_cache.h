 ...
 * see the bare-metal arena/TLS notes around ::ra_img_arena -- and a synthetic
 * generator in tests).
 *
 * Like ::ra_glyph_atlas, this is a thin typed facade over the reusable
 * ::ra_keycache (key bytes + cell bytes + render-on-miss + pin/unpin): the tile
 * key is the cache key, the decoded tile is the cell payload, and the decoded
 * width/height ride in the per-cell user descriptor (::ra_tile_dims_t). Tiles
...
/* The key is compared/hashed byte-wise over sizeof(ra_tile_key_t); guard the
...
static_assert(sizeof(ra_tile_key_t) == sizeof(uint32_t) + sizeof(uint16_t) + sizeof(uint16_t) +
                                         sizeof(uint16_t) + sizeof(uint16_t),
              "ra_tile_key_t must be padding-free for byte-wise key comparison");

/**
 * @struct ra_tile_dims_t
 ...
 *          ::ra_tile_cache_get reads them back into ::ra_tile_t. Treat as
...
/**
 * @struct ra_tile_t
 * @brief A pinned view of a cached tile returned by ::ra_tile_cache_get.
 *
 * @details `pixels` points into the cache cell and stays valid until the matching
 *          ::ra_tile_cache_put. The pixel format and stride are an agreement
 ...
 *
 * @return ra_err_t k_ra_ok on success (with the decoded pixels fitting
 *         `cell_bytes`); any error aborts the get with that code.
 *
 * @since 0.1.0
 */
typedef ra_err_t (*ra_tile_decode_fn)(void*                ctx,
...
/**
 * @struct ra_tile_cache_cfg_t
 * @brief Caller-supplied storage + decoder for ::ra_tile_cache_init.
 *
 ...
 * @retval k_ra_ok               Cache ready; all cells cold.
 * @retval k_ra_err_null_ptr     `tc`, `cfg`, or a required `cfg` pointer NULL.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_tile_cache_init(ra_tile_cache_t* tc, const ra_tile_cache_cfg_t* cfg);
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           Tile resident and pinned; `*out_tile` set.
 * @retval k_ra_err_null_ptr `tc`, `key`, or `out_tile` was NULL.
 * @retval k_ra_err_no_mem   Every cell is pinned (cannot evict for the miss).
 * @retval k_ra_err_*        The decoder failed (returned verbatim).
 *
 * @pre `tc` was populated by ::ra_tile_cache_init.
 * @pre The caller will ::ra_tile_cache_put the returned tile.
 ...
 */
[[nodiscard]] ra_err_t
ra_tile_cache_get(ra_tile_cache_t* tc, const ra_tile_key_t* key, ra_tile_t* out_tile);

/**
 * @brief Release one pin on a tile previously returned by ::ra_tile_cache_get.
 *
 * @param[in] tc     Initialised cache.
 * @param[in] pixels The `pixels` pointer from a returned ::ra_tile_t.
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok              Pin released.
 * @retval k_ra_err_null_ptr    `tc` or `pixels` was NULL.
 * @retval k_ra_err_invalid_arg `pixels` is not a cell of this cache, or the cell
 *                              was not pinned.
 *
 * @pre `pixels` came from ::ra_tile_cache_get on this cache and is still pinned.
 * @pre `tc` was populated by ::ra_tile_cache_init.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_tile_cache_put(ra_tile_cache_t* tc, const uint8_t* pixels);
 ...
 * @retval k_ra_err_null_ptr     `tc` was NULL.
 * @retval k_ra_err_invalid_state The cache was not initialised.
 *
 * @pre `tc` was populated by ::ra_tile_cache_init.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_tile_cache_stats(const ra_tile_cache_t* tc,
                                           uint32_t*              out_hits,
                                           uint32_t*              out_misses,
