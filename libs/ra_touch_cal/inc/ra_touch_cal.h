 ...
 *
 * @details
 * ``ra_touch_cal`` computes and applies the 2-D affine transform that
 * maps raw touch-controller coordinates (the values reported by
 * ``ra_touch_read`` straight from the GT911 / FT5x06 / Atmel maXTouch
 ...
 *   target points (here ``::k_ra_touch_cal_n_targets`` == 5). The same
 *   3x3 system with ``v`` substituted for ``u`` solves the second row
 *   of the matrix.
 *
 *   Three points (``::k_ra_touch_cal_n_targets`` reduced) would also
 ...
 *   three sample pairs to ``ra_touch_cal_compute()``.
 *
 ...
 *   the public ``ra_touch_cal_apply`` API still accepts and returns
 *   integer pixel coordinates.
 *
 * Storage layout for ``ra_touch_cal_save`` / ``ra_touch_cal_load``:
 *
 * @verbatim
 *   offset  size  field
 *   ------  ----  ------------------------------------------------
 *   0       4     magic   = 'T','C','A','L' (ASCII)
 *   4       1     version = ::k_ra_touch_cal_storage_version
 *   5       3     reserved (zeroed)
 *   8       24    coeffs  = a..f, six little-endian IEEE-754 floats
 *   32      4     crc32   = IEEE 802.3 polynomial over bytes 0..31
 *   ------  ----  ------------------------------------------------
 *   36 total bytes (::k_ra_touch_cal_blob_size)
 * @endverbatim
 *
...
/**
 * @enum ra_touch_cal_layout_t
 * @brief Byte-offsets inside the serialised storage blob.
 */
typedef enum : uint8_t {
  k_ra_touch_cal_off_magic    = 0U,  /**< Offset of 'TCAL' magic.            */
  k_ra_touch_cal_off_version  = 4U,  /**< Offset of version byte.            */
  k_ra_touch_cal_off_reserved = 5U,  /**< Offset of 3 reserved zero bytes.   */
  k_ra_touch_cal_off_coeffs   = 8U,  /**< Offset of the 6-float coeff array. */
  k_ra_touch_cal_off_crc32    = 32U, /**< Offset of the CRC32 trailer.       */
} ra_touch_cal_layout_t;

/**
 * @enum ra_touch_cal_magic_t
 * @brief Bytes of the storage magic 'TCAL'.
 */
typedef enum : uint8_t {
  k_ra_touch_cal_magic_b0 = 0x54U, /**< 'T' */
  k_ra_touch_cal_magic_b1 = 0x43U, /**< 'C' */
  k_ra_touch_cal_magic_b2 = 0x41U, /**< 'A' */
  k_ra_touch_cal_magic_b3 = 0x4CU, /**< 'L' */
} ra_touch_cal_magic_t;
 ...
 *
 * @invariant ``ra_touch_cal_compute`` populates all six members or
 *            returns an error and leaves the struct untouched.
 *
 * @see ra_touch_cal_compute
...
/**
 * @typedef ra_touch_cal_draw_target_fn_t
 ...
 * @brief Configuration for ``ra_touch_cal_run``.
 *
 * @details
 * The library does not include ra_lcd.h or ra_touch.h; the caller
 * provides function-pointer shims, satisfying SOLID-D (Dependency
 * Inversion) and keeping ``ra_touch_cal`` host-testable without the
 * register simulator.
 */
typedef struct {
 ...
 *      ``k_ra_err_invalid_arg`` if ``|det|`` is below
 *      ``::k_ra_touch_cal_min_det`` (panel-collinear targets).
 ...
 * @retval k_ra_ok               Matrix populated.
 * @retval k_ra_err_null_ptr     Any pointer NULL.
 * @retval k_ra_err_invalid_arg  ``n`` out of range or system is singular.
 *
 * @pre ``raw``, ``screen``, ``out_mtx`` non-NULL.
 * @pre ``n`` in [::k_ra_touch_cal_min_targets,
 *      ::k_ra_touch_cal_max_targets].
 ...
 * @par Example:
 * @code
 * ra_touch_cal_point_t raw[5] = { ... };
 * ra_touch_cal_point_t scr[5] = { ... };
 * ra_touch_cal_matrix_t m = {};
 * if (ra_touch_cal_compute(raw, scr, 5, &m) == k_ra_ok) { ... }
 * @endcode
 *
 * @see ra_touch_cal_run
 * @see ra_touch_cal_apply
 *
 * @since 0.1.0
 */
 ...
 * ``ra_touch_cal_compute`` and copies the result into ``out_matrix``.
 *
 * @param[in]  cfg         Configuration (non-NULL).
 * @param[out] out_matrix  Destination matrix (non-NULL).
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               Calibration matrix produced.
 * @retval k_ra_err_null_ptr     Any pointer or shim NULL.
 * @retval k_ra_err_invalid_arg  Panel size or inset implausible, or
 *                               solve failed.
 * @retval k_ra_err_hw_error     A draw or read shim returned non-OK.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               Sample mapped.
 * @retval k_ra_err_null_ptr     ``matrix`` or ``out_screen`` NULL.
 * @retval k_ra_err_invalid_arg  Panel size is zero.
 *
 ...
 *
 * @details
 * Writes ``::k_ra_touch_cal_blob_size`` bytes laid out as documented in
 ...
 * intended for ``ra_flash`` page storage but the function does not
 ...
 * @retval k_ra_err_null_ptr     Any pointer NULL.
 * @retval k_ra_err_invalid_size ``dst_size`` <
 *                               ::k_ra_touch_cal_blob_size.
 *
 * @pre ``dst_size`` >= ::k_ra_touch_cal_blob_size.
 * @post On success the first ::k_ra_touch_cal_blob_size bytes of
 *       ``dst`` form a valid blob.
 *
 * @note Round-trip stable with ::ra_touch_cal_load on any platform that
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                Blob accepted, matrix populated.
 * @retval k_ra_err_null_ptr      Any pointer NULL.
 * @retval k_ra_err_invalid_size  ``src_size`` is too small.
 * @retval k_ra_err_invalid_arg   Magic, version, or reserved bytes wrong.
 * @retval k_ra_err_crc_mismatch  CRC trailer does not match payload.
 *
 * @pre ``src``, ``out_matrix`` non-NULL.
 * @pre ``src_size`` >= ::k_ra_touch_cal_blob_size.
