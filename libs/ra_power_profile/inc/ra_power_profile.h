 ...
 * ``ra_gpio_*`` and ``ra_rtc_*``; unit tests inject mocks. There
 * are no calls into ra_hal/ from this translation unit.
 *
 * Layout in this firmware:
 *   - ``inc/ra_power_profile.h``  public API (this file)
 *   - ``src/ra_power_profile.c``  implementation
 *
 * Region IDs are a fixed-size C23 typed enum capped at
 * ``k_ra_power_profile_max_regions`` (16) to satisfy NASA Power-of-10
...
/**
 * @enum ra_power_profile_limits_t
 * @brief Static sizing constants for the profiler.
 *
 * @details
 * Used to bound the per-region accumulator array at compile time
 * (NASA Rule 3). Region IDs must be in ``[0, k_ra_power_profile_max_regions)``.
 *
 * @invariant ``k_ra_power_profile_max_regions`` is exactly 16.
 *
...
/**
 * @enum ra_power_profile_region_id_t
 * @brief Default named region IDs.
 *
 * @details
 * Applications are free to use raw integer values in
 * ``[0, k_ra_power_profile_max_regions)`` instead of these names.
...
/**
 * @typedef ra_power_profile_gpio_pulse_fn_t
 ...
 *       as ``ra_power_profile_mark_enter`` / ``_exit``.
 *
 * @since 0.1.0
 */
 ...
 * @brief Per-region accumulator returned by ``ra_power_profile_get_stats``.
 *
 * @details
...
/**
 * @struct ra_power_profile_stats_t
 * @brief Aggregate snapshot of all per-region accumulators.
 *
 * @details
 * Flat array indexed by ``ra_power_profile_region_id_t`` value.
 * Slots that were never used hold all-zero accumulators.
 *
 * @invariant ``regions`` is exactly ``k_ra_power_profile_max_regions`` long.
 *
 * @since 0.1.0
 */
typedef struct {
  /** Per-region accumulators. */
  ra_power_profile_region_stats_t regions[k_ra_power_profile_max_regions];
} ra_power_profile_stats_t;

/**
 * @struct ra_power_profile_config_t
 ...
 *
 * @code
 * static void board_pulse(void* c, ra_power_profile_region_id_t r, bool e) {
 *     (void)c; (void)r;
 *     ra_gpio_write(k_ra_port_4, 0, e ? k_ra_level_high : k_ra_level_low);
 * }
 * static uint64_t board_now_us(void* c) { (void)c; return ra_rtc_now_us(); }
 *
 * ra_power_profile_config_t cfg = {
 *     .pulse    = board_pulse,
 *     .now_us   = board_now_us,
 * };
 * ra_power_profile_init(&cfg);
 * @endcode
 *
 * @see ra_power_profile_init
 * @since 0.1.0
 */
typedef struct {
 ...
 *
 * @see ra_power_profile_mark_enter
 ...
 * @return ra_err_t Error code.
 * @retval k_ra_ok                       Success.
 * @retval k_ra_err_not_initialized      ``ra_power_profile_init`` was not
 * called.
 ...
 *
 * @see ra_power_profile_mark_exit
 *
 * @since 0.1.0
 */
 ...
 * ``exits`` and returns ``k_ra_err_invalid_state`` so the caller can
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                       Success: matching enter was open.
 * @retval k_ra_err_invalid_state        No matching open enter for this region.
 * @retval k_ra_err_not_initialized      ``ra_power_profile_init`` was not
 * called.
 * @retval k_ra_err_range_check_failed   ``region_id`` is out of range.
 *
