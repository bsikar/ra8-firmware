 ...
 * @param[in,out] opaque Bound ::ra8_mdl_rabook_vfs_t.
 ...
 */
RA8_INTERNAL
static void internal_rabook_clear_open(ra8_mdl_rabook_vfs_t* ctx)
{
  ctx->reader    = (ra8_book_chunked_t){};
  ctx->header    = (ra8_book_header_t){};
  ctx->file      = nullptr;
  ctx->file_size = 0U;
  ctx->open      = false;
}
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_rabook_close(ra8_mdl_rabook_vfs_t* ctx)
{
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_rabook_open_reader(ra8_mdl_rabook_vfs_t* ctx,
                                             const char*           path,
 ...
 */
RA8_INTERNAL
static ra8_err_t internal_rabook_validate_open(ra8_mdl_rabook_vfs_t* ctx,
                                               ra8_book_header_t*    out_header)
{
  return ra8_book_chunked_validate_strict(&ctx->reader,
                                          ctx->chunk,
                                          ctx->chunk_cap,
                                          ctx->scratch,
                                          ctx->scratch_cap,
...
ra8_err_t ra8_mdl_rabook_vfs_validate(void*         opaque,
...
ra8_err_t ra8_mdl_rabook_vfs_read_chunk(ra8_mdl_rabook_vfs_t* ctx,
                                        uint64_t              offset,
                                        uint8_t*              dst,
                                        uint32_t              len)
{
...
ra8_err_t ra8_mdl_rabook_vfs_info(const ra8_mdl_rabook_vfs_t* ctx,
                                  ra8_book_header_t*          out_header,
...
ra8_err_t ra8_mdl_rabook_vfs_close(ra8_mdl_rabook_vfs_t* ctx)
{
  if (ctx == nullptr) {
    return k_ra8_err_null_ptr;
  }
