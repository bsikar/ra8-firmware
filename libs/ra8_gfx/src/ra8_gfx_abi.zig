...
/// carry their own message, under this library's own log tag.
...
pub export fn ra8_gfx_dither_gray8_to_gray4(
    ...
    switch (guard) {
        .ok => {},
        .no_src => ra8_log_emit_error(dither_tag, "src must not be nullptr"),
        .no_out => ra8_log_emit_error(dither_tag, "out must not be nullptr"),
        .no_out_size => ra8_log_emit_error(dither_tag, "out_size must not be nullptr"),
        .bad_dims => ra8_log_emit_error(dither_tag, "w or h is non-positive"),
        .too_small => ra8_log_emit_error(dither_tag, "output buffer too small"),
    }
