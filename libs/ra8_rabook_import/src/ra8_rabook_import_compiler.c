                                            ...
                                            const char*     out_path)
{
  ...
  /* The cookie's epub/load-buf are validated by `ra8_epub_open_fs` and the
   * bufs/scr by `ra8_rabook_compile_from_epub`, so a NULL cookie field still
   * yields k_ra8_err_null_ptr without restating those guards here. */
  ra8_rabook_import_compiler_ctx_t* ctx = (ra8_rabook_import_compiler_ctx_t*)compile_ctx;

  ra8_err_t err =
    ra8_epub_open_fs(mount, epub_path, ctx->epub_load_buf, ctx->epub_load_cap, ctx->epub);
  if (err != k_ra8_ok) {
    return err;
  }
 ...
 *          single @ref ra8_fs_read (byte count returned in @p *out_len), then
 ...
 * @since Version 0.1.0
 */
static ra8_err_t s_offload_or_fallback(ra8_rabook_import_compiler_m33_ctx_t* ctx,
                                       uint32_t                              epub_len,
                                       ra8_fs_mount_t*                       mount,
                                       const char*                           epub_path,
                                       const char*                           out_path)
