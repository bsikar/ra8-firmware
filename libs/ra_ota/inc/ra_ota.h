 ...
 * is free to be erased and re-programmed. ``ra_ota`` orchestrates
 * the safe-update flow on top of the existing ``ra_flash`` driver:
 *
 ...
 * @endcode
 *
 * Each public entry point is one transition arrow. ``ra_ota_run_step``
 * drives the machine cooperatively from a caller's main loop;
 * ``cfg.run_as_thread = true`` instead spawns a single static ThreadX
 * thread that calls ``ra_ota_run_step`` until ``DONE`` / ``ERROR``.
 *
 * The HTTPS download and ECDSA verification are exposed through
 * function-pointer interfaces (``ra_ota_net_iface_t``,
 * ``ra_ota_crypto_iface_t``) so the module compiles independently of
 * ra_tls / mbedtls / tf-psa-crypto and can be unit-tested with
 * mocks. This is the Dependency-Inversion deviation called out in
 * CLAUDE.md "NASA Rule 9 -- INTENTIONAL DEVIATION".
 *
 * @par Memory budget:
 * - 1 download chunk buffer of ``k_ra_ota_chunk_bytes``
 *   (see ``ra_ota_constants_t``).
 * - 1 manifest scratch buffer of ``k_ra_ota_manifest_max_bytes``.
 * - 1 SHA-256 streaming context (caller-provided through
 *   ``ra_ota_crypto_iface_t.sha256_ctx_size`` bytes).
 * - 1 static ThreadX thread (only if ``cfg.run_as_thread``).
 *
 * Every buffer is a file-static in ``ra_ota.c``. There is zero
 ...
 *
 * @details
 * Returned by ``ra_ota_get_state``. Values are stable across calls so
...
/**
 * @struct ra_ota_manifest_t
 * @brief Decoded representation of the server manifest.
 *
 * @details
 * The manifest the server returns is JSON; ``ra_ota`` decodes the
 * relevant fields here. The struct is also used as the input to
 * ``ra_ota_download_to_inactive_bank`` and
 * ``ra_ota_verify_signature``.
 *
 * @invariant ``image_size_bytes`` <= ``k_ra_ota_max_image_bytes``.
 * @invariant ``signature_len`` <= ``k_ra_ota_signature_max_bytes``.
 */
 ...
 */
typedef void (*ra_ota_progress_cb_t)(const ra_ota_progress_t* p);

/**
 * @struct ra_ota_net_iface_t
 * @brief Injected HTTPS-download interface (Dependency Inversion).
 *
 * @details
 * The OTA module never links directly against ra_tls / mbedtls. The
 * caller registers a vtable with ``open``, ``read``, ``close``. The
 * default wiring -- when ra_tls eventually lands -- will be a thin
 * shim in ra_tls that adapts ra_tls handles to this struct.
 *
 * @invariant All three function pointers are non-NULL when this
 *            struct is passed to ``ra_ota_init``.
 */
/* cppcheck-suppress-begin [unusedStructMember] */
typedef struct {
  /**
   * @brief Begin a streaming GET against ``url``.
   * @return ``k_ra_ok`` if a download session has been opened and
   *         subsequent ``read`` calls are valid.
   */
  ra_err_t (*open)(void* ctx, const char* url, uint32_t* out_content_len);

  /**
   * @brief Read up to ``cap`` bytes from the open session.
   * @param[out] out_len Bytes actually read (0 == EOF).
   */
  ra_err_t (*read)(void* ctx, uint8_t* dst, uint32_t cap, uint32_t* out_len);

  /**
   * @brief Tear the streaming GET down.
   */
  ra_err_t (*close)(void* ctx);
...
/**
 * @struct ra_ota_crypto_iface_t
 * @brief Injected hash + ECDSA verification interface.
 *
 * @details
 * Same Dependency-Inversion pattern as ``ra_ota_net_iface_t``. A
...
typedef struct {
  ...
  /** @brief Opaque context passed back to every callback. */
  void* ctx;
} ra_ota_crypto_iface_t;
/* cppcheck-suppress-end [unusedStructMember] */

/**
 * @struct ra_ota_flash_iface_t
 ...
 * ``ra_flash_erase`` / ``ra_flash_write`` /
 * ``ra_flash_set_startup_area`` and lives outside this module so
 * libs/ra_ota does not transitively pull in libs/ra_hal at link
 * time. Unit tests register an in-RAM mock backend.
 */
/* cppcheck-suppress-begin [unusedStructMember] */
typedef struct {
  /** @brief Erase ``len`` bytes starting at ``addr`` in the inactive bank. */
  ra_err_t (*erase)(void* ctx, uint32_t addr, uint32_t len);
  /** @brief Program ``len`` bytes at ``addr`` (must be 32-byte aligned). */
  ra_err_t (*program)(void* ctx, uint32_t addr, const uint8_t* src, uint32_t len);
  /** @brief Pick the bank to boot from at the next reset. */
  ra_err_t (*set_startup)(void* ctx, uint8_t which_bank, bool persistent);
  /** @brief Read back ``len`` bytes (used by verification re-hash). */
  ra_err_t (*readback)(void* ctx, uint32_t addr, uint8_t* dst, uint32_t len);
  ...
  /** @brief Opaque context passed back to every callback. */
  void* ctx;
} ra_ota_flash_iface_t;
/* cppcheck-suppress-end [unusedStructMember] */

/**
 * @struct ra_ota_cfg_t
 * @brief Initialisation descriptor for ``ra_ota_init``.
 *
 * @details
 ...
 * state machine in ``k_ra_ota_state_idle``.
 *
 * @param[in] cfg Non-NULL configuration descriptor.
 *
 * @return ``ra_err_t`` outcome.
 * @retval k_ra_ok               Module ready.
 * @retval k_ra_err_null_ptr     ``cfg`` was NULL or any required
 ...
 * @post Module is in ``k_ra_ota_state_idle``.
 * @post Subsequent ``ra_ota_*`` calls are valid.
 *
 * @note Thread-safe: no, single-threaded init only.
 * @see ra_ota_deinit
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ota_init(const ra_ota_cfg_t* cfg);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ota_deinit(void);
 ...
 *
 * @return ``ra_err_t`` outcome.
 * @retval k_ra_ok                  Manifest decoded.
 * @retval k_ra_err_null_ptr        ``out_manifest`` was NULL.
 * @retval k_ra_err_not_initialized ``ra_ota_init`` not called.
 * @retval k_ra_err_invalid_state   Module not in ``idle``.
 * @retval k_ra_err_hw_error        Network backend reported an error.
 * @retval k_ra_err_invalid_size    Manifest exceeded
 *                                  ``k_ra_ota_manifest_max_bytes``.
 * @retval k_ra_err_invalid_arg     JSON malformed / missing field.
 *
 * @pre ``ra_ota_init`` has been called.
 * @pre ``out_manifest`` non-NULL.
 * @post On success, every field in ``*out_manifest`` is populated.
 * @post Module returns to ``k_ra_ota_state_idle`` on either path.
 *
 * @note Thread-safe: no.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ota_check_for_update(ra_ota_manifest_t* out_manifest);

/**
 * @brief Stream the firmware blob into the inactive MRAM bank.
 *
 * @details
 * Walks the image URL one ``k_ra_ota_chunk_bytes`` chunk at a
 * time. Each chunk is:
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ota_download_to_inactive_bank(const ra_ota_manifest_t* manifest);
 ...
 *       ``k_ra_ota_state_committing``.
 * @post On failure, the module is in ``k_ra_ota_state_error``.
 *
 * @note Thread-safe: no.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ota_verify_signature(const ra_ota_manifest_t* manifest);
 ...
 * state to ``k_ra_ota_state_done``, and -- in production -- triggers
 * ``NVIC_SystemReset``. In the host test build the system-reset
 * call is forwarded to a weak ``ra_ota_system_reset_hook`` symbol so
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ota_commit_and_reboot(void);
 ...
 * caller may inspect ``ra_ota_get_state`` between invocations.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ota_run_step(void);
 ...
 *
 * @return ``ra_err_t`` outcome (the success path on hardware
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ota_run_full_update(void);
