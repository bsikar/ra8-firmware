 ...
 *
 *   - ``s_ra_ota_state``  -- single-byte state machine value.
 *   - ``s_ra_ota_cfg``    -- copy of the caller's configuration (function
 *                     pointers + URLs + bank metadata).
 *   - ``s_ra_ota_buf``    -- 4 KiB streaming chunk buffer used both for
...
/** @brief Current state-machine value (shared; contract in ra_ota_internal.h). */
ra_ota_state_t s_ra_ota_state = k_ra_ota_state_idle;

/** @brief Configuration captured at init time (shared; contract in ra_ota_internal.h). */
ra_ota_cfg_t s_ra_ota_cfg;

/** @brief True once ``ra_ota_init`` succeeded (shared; contract in ra_ota_internal.h). */
bool s_ra_ota_initialized = false;
...
/** @brief Streaming buffer (shared; contract in ra_ota_internal.h). */
uint8_t s_ra_ota_buf[k_ra_ota_chunk_bytes];

/* =============================================================================
 * Internal helpers
 * ============================================================================= */

/** @brief Implementation of `ra_ota_internal_set_state()` -- latch state + progress fan-out. */
void ra_ota_internal_set_state(ra_ota_state_t new_state, ra_err_t err)
{
  s_ra_ota_state = new_state;
  s_last_err     = err;
  if (s_ra_ota_cfg.on_progress != nullptr) {
 ...
 *
 * @details
 * Loops calling the user-supplied ``s_ra_ota_cfg.net.read`` callback until
 ...
 *
 * @pre Module is initialized and ``s_ra_ota_cfg.net.read`` is set.
...
static ra_err_t priv_drain(uint8_t* dst, uint32_t cap, uint32_t* out_n)
{
  ...
  for (uint32_t guard = 0U; guard < cap + 1U; ++guard) {
    ...
    uint32_t       got = 0U;
    const ra_err_t e = s_ra_ota_cfg.net.read(s_ra_ota_cfg.net.ctx, dst + total, cap - total, &got);
    if (e != k_ra_ok) {
      return e;
    }
    if (got == 0U) {
 ...
 * by-value into ``s_ra_ota_cfg`` and resets the state machine to
 * ``k_ra_ota_state_idle``.
 *
...
ra_err_t ra_ota_init(const ra_ota_cfg_t* cfg)
{
  ...
  (void)memcpy(&s_ra_ota_cfg, cfg, sizeof s_ra_ota_cfg);
  s_ra_ota_state       = k_ra_ota_state_idle;
  s_manifest_valid     = false;
  s_bytes_done         = 0U;
  s_last_err           = k_ra_ok;
  s_ra_ota_initialized = true;
 ...
 * @post ``s_ra_ota_initialized`` is false.
 * @post ``s_ra_ota_cfg`` is zeroed.
 *
 * @see ra_ota_init()
 *
...
{
  s_ra_ota_initialized = false;
  s_ra_ota_state       = k_ra_ota_state_idle;
  s_manifest_valid     = false;
  s_bytes_done         = 0U;
  s_last_err           = k_ra_ok;
  (void)memset(&s_ra_ota_cfg, 0, sizeof s_ra_ota_cfg);
  return k_ra_ok;
}
 ...
 *
 * @details
 * Reads the latched ``s_ra_ota_state`` directly. ``s_ra_ota_state`` is a single byte,
...
{
  return s_ra_ota_state;
}

/**
 * @brief Open the manifest URL and drain its payload into ``s_ra_ota_buf``.
 *
 * @details
 * Calls ``s_ra_ota_cfg.net.open`` on the configured manifest URL, validates
 ...
 *
 * @param[out] out_got Bytes received (NUL terminator added at ``s_ra_ota_buf[got]``).
 *
 * @return ra_err_t outcome.
 * @retval k_ra_ok                Payload fetched into ``s_ra_ota_buf``.
 * @retval k_ra_err_invalid_size  Server advertised too large a body.
 * @retval other                  Whatever the network backend returned.
 *
 * @pre Module is initialized.
 * @pre ``out_got`` non-NULL.
 * @post On success ``s_ra_ota_buf[0..*out_got]`` holds the payload + trailing NUL.
...
static ra_err_t priv_fetch_manifest_payload(uint32_t* out_got)
{
  ...
  ra_err_t e = s_ra_ota_cfg.net.open(s_ra_ota_cfg.net.ctx, s_ra_ota_cfg.manifest_url, &content_len);
  if (e != k_ra_ok) {
    return e;
  }
  if (content_len > k_ra_ota_manifest_max_bytes) {
    (void)s_ra_ota_cfg.net.close(s_ra_ota_cfg.net.ctx);
    return k_ra_err_invalid_size;
  }
  uint32_t got = 0U;
  e            = priv_drain(s_ra_ota_buf, k_ra_ota_manifest_max_bytes - 1U, &got);
  (void)s_ra_ota_cfg.net.close(s_ra_ota_cfg.net.ctx);
  if (e != k_ra_ok) {
    return e;
  }
  s_ra_ota_buf[got] = 0U; /* NUL terminate so JSON helpers can use strstr. */
  *out_got          = got;
  return k_ra_ok;
}
...
ra_err_t ra_ota_check_for_update(ra_ota_manifest_t* out_manifest)
{
  ...
  if (s_ra_ota_state != k_ra_ota_state_idle) {
    return k_ra_err_invalid_state;
  }
  ra_ota_internal_set_state(k_ra_ota_state_checking, k_ra_ok);

  uint32_t got = 0U;
  ra_err_t e   = priv_fetch_manifest_payload(&got);
  if (e != k_ra_ok) {
    ra_ota_internal_set_state(k_ra_ota_state_error, e);
    return e;
  }

  e = ra_ota_internal_manifest_decode((const char*)s_ra_ota_buf, out_manifest);
  if (e != k_ra_ok) {
    ra_ota_internal_set_state(k_ra_ota_state_error, e);
    return e;
  }
  ...
  ra_ota_internal_set_state(k_ra_ota_state_idle, k_ra_ok);
  return k_ra_ok;
}
...
static ra_err_t priv_download_chunk(uint32_t addr_base, uint32_t* in_out_done, uint32_t total)
{
  ...
  uint32_t       got       = 0U;
  ra_err_t       e         = priv_drain(s_ra_ota_buf, want, &got);
  if (e != k_ra_ok) {
    return e;
  }
  if (got == 0U) {
  ...
  e = s_ra_ota_cfg.crypto.sha256_update(s_ra_ota_cfg.crypto.ctx, s_ra_ota_buf, got);
  if (e != k_ra_ok) {
    return e;
  }
  e =
    s_ra_ota_cfg.flash.program(s_ra_ota_cfg.flash.ctx, addr_base + *in_out_done, s_ra_ota_buf, got);
  if (e != k_ra_ok) {
    return e;
  }
  *in_out_done += got;
  ra_ota_internal_set_state(k_ra_ota_state_downloading, k_ra_ok);
  return k_ra_ok;
}
...
{
  ra_err_t e = s_ra_ota_cfg.flash.erase(s_ra_ota_cfg.flash.ctx,
                                        s_ra_ota_cfg.flash.inactive_bank_addr,
                                        manifest->image_size_bytes);
  if (e != k_ra_ok) {
    return e;
  }
  return s_ra_ota_cfg.crypto.sha256_init(s_ra_ota_cfg.crypto.ctx);
}
...
static ra_err_t priv_download_loop(const ra_ota_manifest_t* manifest)
{
  ...
  while (s_bytes_done < manifest->image_size_bytes) {
    ...
    e = priv_download_chunk(s_ra_ota_cfg.flash.inactive_bank_addr,
                            &s_bytes_done,
                            manifest->image_size_bytes);
    if (e != k_ra_ok) {
      break;
...
ra_err_t ra_ota_download_to_inactive_bank(const ra_ota_manifest_t* manifest)
{
  ...
  if (ra_ota_internal_download_state_invalid((uint32_t)k_ra_ota_state_idle,
                                             ...
                                             (uint32_t)s_ra_ota_state)) {
    return k_ra_err_invalid_state;
  }
  if (manifest->image_size_bytes > s_ra_ota_cfg.flash.bank_size_bytes) {
    ra_ota_internal_set_state(k_ra_ota_state_error, k_ra_err_invalid_size);
    return k_ra_err_invalid_size;
  }
  ...
  uint32_t content_len = 0U;
  ra_err_t e = s_ra_ota_cfg.net.open(s_ra_ota_cfg.net.ctx, manifest->image_url, &content_len);
  if (e != k_ra_ok) {
    ra_ota_internal_set_state(k_ra_ota_state_error, e);
    return e;
  }
  ...
  ra_ota_internal_set_state(k_ra_ota_state_verifying, k_ra_ok);
  return k_ra_ok;
}
 ...
 *
 * @details
 * Calls ``s_ra_ota_cfg.flash.set_startup`` to mark the inactive bank as the
...
ra_err_t ra_ota_commit_and_reboot(void)
{
  ...
  if (s_ra_ota_state != k_ra_ota_state_committing) {
    return k_ra_err_invalid_state;
  }
  const ra_err_t e = s_ra_ota_cfg.flash.set_startup(s_ra_ota_cfg.flash.ctx,
                                                    s_ra_ota_cfg.flash.inactive_bank_index,
                                                    true);
  if (e != k_ra_ok) {
    ra_ota_internal_set_state(k_ra_ota_state_error, e);
    return e;
  }
  ra_ota_internal_set_state(k_ra_ota_state_done, k_ra_ok);
 ...
 *
 * @details
 * Switches on ``s_ra_ota_state`` and dispatches to the matching public-API
 ...
 * ``s_ra_ota_initialized``. Intended for callers that opted out of running
...
ra_err_t ra_ota_run_full_update(void)
{
  ...
  for (uint32_t i = 0U; i < (uint32_t)k_ra_ota_state_count; ++i) {
    if ((s_ra_ota_state == k_ra_ota_state_done) || (s_ra_ota_state == k_ra_ota_state_error)) {
      break;
    }
