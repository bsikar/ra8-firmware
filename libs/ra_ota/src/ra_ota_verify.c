/**
 * @file ra_ota_verify.c
 * @brief Phase-5 OTA signature-verification cluster -- implementation.
 *
 * @details
 * Companion translation unit to ``ra_ota.c``. Holds the cohesive
 ...
 *   - ``ra_ota_verify_signature`` (public API) compares the re-derived
 *     digest under a constant-time equality (T5-12) and then runs the
 *     injected ECDSA verifier over that bound material.
 *
 * The verify path reads the module-static state defined in ``ra_ota.c``
 * (configuration, state byte, init flag, streaming buffer) through the
 * ``extern`` declarations in ``ra_ota_internal.h`` and drives state
 * transitions through ``ra_ota_internal_set_state``. The read-only log
 ...
 * ``k_ra_ota_chunk_bytes`` chunks via ``s_ra_ota_cfg.flash.readback``,
 * feeding each one to ``s_ra_ota_cfg.crypto.sha256_update``. Finalises
...
{
  ra_err_t e = s_ra_ota_cfg.crypto.sha256_init(s_ra_ota_cfg.crypto.ctx);
  if (e != k_ra_ok) {
    return e;
  }
  uint32_t       offset     = 0U;
  ...
  for (uint32_t i = 0U; i < max_chunks; ++i) {
    ...
    e = s_ra_ota_cfg.flash.readback(s_ra_ota_cfg.flash.ctx,
                                    s_ra_ota_cfg.flash.inactive_bank_addr + offset,
                                    s_ra_ota_buf,
                                    want);
    if (e != k_ra_ok) {
      return e;
    }
    e = s_ra_ota_cfg.crypto.sha256_update(s_ra_ota_cfg.crypto.ctx, s_ra_ota_buf, want);
    if (e != k_ra_ok) {
      return e;
    }
    offset += want;
  }
  return s_ra_ota_cfg.crypto.sha256_final(s_ra_ota_cfg.crypto.ctx, out_digest);
}
                                            ...
                                            uint8_t out_bound[k_ra_ota_sha256_bytes])
{
  ...
  for (uint8_t i = 0U; i < (uint8_t)(sizeof segments / sizeof segments[0]); ++i) {
    e = s_ra_ota_cfg.crypto.sha256_update(s_ra_ota_cfg.crypto.ctx, segments[i], lengths[i]);
    if (e != k_ra_ok) {
      return e;
    }
  }
  return s_ra_ota_cfg.crypto.sha256_final(s_ra_ota_cfg.crypto.ctx, out_bound);
}
...
ra_err_t ra_ota_verify_signature(const ra_ota_manifest_t* manifest)
{
  ...
  if (s_ra_ota_state != k_ra_ota_state_verifying) {
  ...
  if (!ra_ct_equal(digest, manifest->image_sha256, k_ra_ota_sha256_bytes)) {
    ra_ota_internal_set_state(k_ra_ota_state_error, k_ra_err_crc_mismatch);
  ...
  e = s_ra_ota_cfg.crypto.ecdsa_verify(s_ra_ota_cfg.crypto.ctx,
                                       s_ra_ota_cfg.pubkey_handle,
                                       bound,
                                       manifest->signature,
                                       manifest->signature_len);
  if (e != k_ra_ok) {
    ra_ota_internal_set_state(k_ra_ota_state_error, k_ra_err_hw_error);
    return k_ra_err_hw_error;
  }
  ra_ota_internal_set_state(k_ra_ota_state_committing, k_ra_ok);
  return k_ra_ok;
}
