 ...
 *
 * @details
 * Implements ::ra_ftl_init, ::ra_ftl_as_blockdev, and the ::ra_io_blockdev_iface
...
static const char* const s_tag = "ra_ftl";

/**
 * @enum ra_ftl_impl_const_t
...
typedef enum : uint32_t {
  k_ra_ftl_one_block      = 1,           /**< Count for a single-block raw transfer.  */
  k_ra_ftl_req_erase_unit = 1,           /**< Required raw `erase_unit_blocks` value. */
  k_ra_ftl_count_max      = 0xFFFFFFFFU, /**< Sentinel: highest possible erase count. */
} ra_ftl_impl_const_t;
 ...
 * @retval k_ra_ok        Reclamation pass completed (some blocks may stay live).
 * @retval k_ra_err_null_ptr `ftl` was NULL.
 ...
 * @since 0.1.0
 */
static ra_err_t ftl_reclaim_stale(ra_ftl_t* ftl)
{
  RA_CHECK_NULL_PTR(ftl, s_tag, "ftl must not be nullptr");
  RA_CHECK_NULL_PTR(ftl->raw, s_tag, "raw must not be nullptr");
  for (uint32_t p = 0; p < ftl->physical_blocks; ++p) {
    if (ftl->pblocks[p].state != (uint8_t)k_ra_ftl_pstate_stale) {
      continue;
    }
    const ra_err_t e = ra_io_blockdev_erase(ftl->raw, p, (uint32_t)k_ra_ftl_one_block);
    if (e != k_ra_ok) {
      return e;
    }
    ftl->pblocks[p].erase_count += 1U;
    ftl->pblocks[p].state = (uint8_t)k_ra_ftl_pstate_free;
  }
  return k_ra_ok;
}
 ...
 * the wear-levelling policy. Returns ::k_ra_err_no_data if no FREE block exists
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           A free block was chosen into `*out`.
 * @retval k_ra_err_null_ptr `ftl` or `out` was NULL.
 * @retval k_ra_err_no_data  No FREE physical block is available.
 *
 * @pre `ftl` is initialised.
 * @pre `out` is writable.
 ...
 * @since 0.1.0
 */
static ra_err_t ftl_pick_free(const ra_ftl_t* ftl, uint32_t* out)
{
  RA_CHECK_NULL_PTR(ftl, s_tag, "ftl must not be nullptr");
  ...
  uint32_t bestc = 0;
  for (uint32_t p = 0; p < ftl->physical_blocks; ++p) {
    if (ftl->pblocks[p].state != (uint8_t)k_ra_ftl_pstate_free) {
      continue;
    }
    if (found) {
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           A blank block was allocated into `*out`.
 * @retval k_ra_err_null_ptr `ftl` or `out` was NULL.
 * @retval k_ra_err_no_mem   No block available even after reclamation.
 ...
 * @since 0.1.0
 */
static ra_err_t ftl_alloc_blank(ra_ftl_t* ftl, uint32_t* out)
{
  RA_CHECK_NULL_PTR(ftl, s_tag, "ftl must not be nullptr");
  RA_CHECK_NULL_PTR(out, s_tag, "out must not be nullptr");
  uint32_t phys = 0;
  ra_err_t pick = ftl_pick_free(ftl, &phys);
  if (pick == k_ra_err_no_data) {
    const ra_err_t rec = ftl_reclaim_stale(ftl);
    if (rec != k_ra_ok) {
      return rec;
    }
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           Block resolved into `dst`.
 * @retval k_ra_err_null_ptr `ftl` or `dst` was NULL.
 ...
 * @since 0.1.0
 */
static ra_err_t ftl_read_one(const ra_ftl_t* ftl, uint32_t lbn, uint8_t* dst)
{
  RA_CHECK_NULL_PTR(ftl, s_tag, "ftl must not be nullptr");
  RA_CHECK_NULL_PTR(dst, s_tag, "dst must not be nullptr");
  const uint16_t phys = ftl->map[lbn];
  if (phys == (uint16_t)k_ra_ftl_unmapped) {
    (void)memset(dst, (int)ftl->erase_value, (size_t)k_ra_io_block_size_bytes);
    return k_ra_ok;
  }
  return ra_io_blockdev_read(ftl->raw, phys, (uint32_t)k_ra_ftl_one_block, dst);
}
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           Logical block now reflects `src`.
 * @retval k_ra_err_null_ptr `ftl` or `src` was NULL.
 * @retval k_ra_err_no_mem   No physical block available to relocate into.
 ...
 * @since 0.1.0
 */
static ra_err_t ftl_write_one(ra_ftl_t* ftl, uint32_t lbn, const uint8_t* src)
{
  RA_CHECK_NULL_PTR(ftl, s_tag, "ftl must not be nullptr");
  RA_CHECK_NULL_PTR(src, s_tag, "src must not be nullptr");
  uint32_t       phys  = 0;
  const ra_err_t alloc = ftl_alloc_blank(ftl, &phys);
  if (alloc != k_ra_ok) {
    return alloc;
  }
  const ra_err_t prog = ra_io_blockdev_write(ftl->raw, phys, (uint32_t)k_ra_ftl_one_block, src);
  if (prog != k_ra_ok) {
    return prog;
  }
  const uint16_t old = ftl->map[lbn];
  if (old != (uint16_t)k_ra_ftl_unmapped) {
    ftl->pblocks[old].state = (uint8_t)k_ra_ftl_pstate_stale;
  }
  ftl->pblocks[phys].state = (uint8_t)k_ra_ftl_pstate_live;
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Range is within the presented capacity.
 * @retval k_ra_err_out_of_range Range exceeds `logical_blocks`.
 *
 * @pre `ftl` is initialised.
 ...
 * @since 0.1.0
 */
static ra_err_t ftl_bounds(const ra_ftl_t* ftl, uint32_t lba, uint32_t count)
{
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Blocks resolved into `buf`.
 * @retval k_ra_err_null_ptr     `ctx` or `buf` was NULL.
 * @retval k_ra_err_out_of_range Range past presented capacity.
 * @retval other                 Propagated underlying read error.
 *
...
static ra_err_t ftl_dev_read(void* ctx, uint32_t lba, uint32_t count, uint8_t* buf)
{
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    const ra_err_t e   = ftl_read_one(ftl, lba + i, &buf[off]);
    if (e != k_ra_ok) {
      return e;
    }
  }
 ...
 * @since 0.1.0
 */
static ra_err_t ftl_dev_write(void* ctx, uint32_t lba, uint32_t count, const uint8_t* buf)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  RA_CHECK_NULL_PTR(buf, s_tag, "buf must not be nullptr");
  ...
  for (uint32_t i = 0; i < count; ++i) {
    const size_t   off = (size_t)i * (size_t)k_ra_io_block_size_bytes;
    const ra_err_t e   = ftl_write_one(ftl, lba + i, &buf[off]);
    if (e != k_ra_ok) {
      return e;
    }
  }
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Logical blocks unmapped.
 ...
 * @since 0.1.0
 */
static ra_err_t ftl_dev_erase(void* ctx, uint32_t lba, uint32_t count)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  ra_ftl_t*      ftl = (ra_ftl_t*)ctx;
  const ra_err_t b   = ftl_bounds(ftl, lba, count);
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    if (old != (uint16_t)k_ra_ftl_unmapped) {
      ftl->pblocks[old].state = (uint8_t)k_ra_ftl_pstate_stale;
      ftl->map[lba + i]       = (uint16_t)k_ra_ftl_unmapped;
    }
  }
  return k_ra_ok;
}
 ...
 * @since 0.1.0
 */
static ra_err_t ftl_dev_get_caps(const void* ctx, ra_io_blockdev_caps_t* out)
{
  RA_CHECK_NULL_PTR(ctx, s_tag, "ctx must not be nullptr");
  RA_CHECK_NULL_PTR(out, s_tag, "out must not be nullptr");
  const ra_ftl_t* ftl          = (const ra_ftl_t*)ctx;
  out->block_count             = ftl->logical_blocks;
  out->erase_unit_blocks       = (uint32_t)k_ra_ftl_one_block;
  out->program_size_bytes      = (uint32_t)k_ra_io_block_size_bytes;
  out->logical_block_bytes     = (uint16_t)k_ra_io_block_size_bytes;
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           Underlying device flushed (or nothing to flush).
 * @retval k_ra_err_null_ptr `ctx` was NULL.
...
static ra_err_t ftl_dev_sync(void* ctx)
{
  ...
  const ra_ftl_t* ftl = (const ra_ftl_t*)ctx;
  RA_CHECK_NULL_PTR(ftl->raw, s_tag, "raw must not be nullptr");
  return ra_io_blockdev_sync(ftl->raw);
}

/** @brief Presented FTL vtable -- a clean free-overwrite block device. */
static const ra_io_blockdev_iface_t k_ftl_iface = {
  .read     = ftl_dev_read,
  .write    = ftl_dev_write,
  .erase    = ftl_dev_erase,
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok              Underlying device is suitable.
 * @retval k_ra_err_null_ptr    `erase_out` was NULL.
 * @retval k_ra_err_invalid_arg Device unsuitable (read-only, wrong erase unit,
 *                              too small, or no spare block).
 *
 * @pre `caps` was filled by ::ra_io_blockdev_get_caps.
 ...
 * @since 0.1.0
 */
static ra_err_t ftl_check_caps(const ra_io_blockdev_caps_t* caps,
                               uint32_t                     logical_blocks,
                               uint32_t                     physical_blocks,
                               uint8_t*                     erase_out)
{
  RA_CHECK_NULL_PTR(caps, s_tag, "caps must not be nullptr");
  RA_CHECK_NULL_PTR(erase_out, s_tag, "erase_out must not be nullptr");
  if (caps->read_only) {
    return k_ra_err_invalid_arg;
  }
  if (caps->erase_unit_blocks != (uint32_t)k_ra_ftl_req_erase_unit) {
    return k_ra_err_invalid_arg;
  }
  if (caps->block_count < physical_blocks) {
    return k_ra_err_invalid_arg;
  }
  if (physical_blocks < logical_blocks + (uint32_t)k_ra_ftl_min_spare) {
    return k_ra_err_invalid_arg;
  }
 ...
 *
 * @details
 * Sets every logical map entry to ::k_ra_ftl_unmapped and every physical block
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           Tables initialised to the cold-start state.
 * @retval k_ra_err_null_ptr `ftl` was NULL.
 *
 ...
 * @since 0.1.0
 */
static ra_err_t ftl_reset_tables(ra_ftl_t* ftl)
{
  RA_CHECK_NULL_PTR(ftl, s_tag, "ftl must not be nullptr");
  RA_CHECK_NULL_PTR(ftl->map, s_tag, "map must not be nullptr");
  for (uint32_t l = 0; l < ftl->logical_blocks; ++l) {
    ftl->map[l] = (uint16_t)k_ra_ftl_unmapped;
  }
  for (uint32_t p = 0; p < ftl->physical_blocks; ++p) {
    ftl->pblocks[p].state       = (uint8_t)k_ra_ftl_pstate_free;
                                       ...
                                       uint32_t                physical_blocks)
{
  ...
  RA_CHECK_NULL_PTR(map, s_tag, "map must not be nullptr");
  RA_CHECK_NULL_PTR(pblocks, s_tag, "pblocks must not be nullptr");
                     ...
                     uint8_t*                scratch)
{
  ...
  const ra_err_t ck          = ftl_check_caps(&caps, logical_blocks, physical_blocks, &erase_value);
  if (ck != k_ra_ok) {
    return ck;
  }
  bd->raw             = raw;
...
ra_err_t ra_ftl_as_blockdev(ra_ftl_t* ftl, ra_io_blockdev_t* out)
{
  RA_CHECK_NULL_PTR(ftl, s_tag, "ftl must not be nullptr");
  RA_CHECK_NULL_PTR(out, s_tag, "out must not be nullptr");
  if (ftl->raw == nullptr) {
...
ra_err_t ra_ftl_wear_stats(const ra_ftl_t* ftl, uint32_t* max_out, uint32_t* min_out)
{
  RA_CHECK_NULL_PTR(ftl, s_tag, "ftl must not be nullptr");
  RA_CHECK_NULL_PTR(max_out, s_tag, "max_out must not be nullptr");
  RA_CHECK_NULL_PTR(min_out, s_tag, "min_out must not be nullptr");
  if (ftl->pblocks == nullptr) {
    return k_ra_err_not_initialized;
  }
  uint32_t hi = 0;
  uint32_t lo = (uint32_t)k_ra_ftl_count_max;
  for (uint32_t p = 0; p < ftl->physical_blocks; ++p) {
