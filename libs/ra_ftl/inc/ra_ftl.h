 ...
 * erase-before-write ::ra_io_blockdev_t (the underlying device) and **presents**
 * a clean free-overwrite ::ra_io_blockdev_t to the FAT/VFS layer above. The FAT
 ...
 *   data, or ::k_ra_ftl_unmapped if the logical block has never been written
 *   (a read of an unmapped block returns the erase value).
 * - **Per-physical-block metadata** (caller storage, one
 *   ::ra_ftl_pblock_t per physical block): the block's state (free / live /
 ...
 * ## Invariants
 *
 * - `map[lbn]` is either ::k_ra_ftl_unmapped or a valid physical index whose
 ...
 *
 * All state is caller-provided: the ::ra_ftl_t handle, the `map` array
 * (`logical_blocks` entries of `uint16_t`), the `pblocks` array
 * (`physical_blocks` entries of ::ra_ftl_pblock_t), and one 512-byte scratch
 * buffer for copy operations. Nothing is allocated.
 *
 * @code
 * extern ra_io_blockdev_t raw;          // bound MRAM (erase-before-write) device
 * static uint16_t        s_map[24];     // logical_blocks entries
 * static ra_ftl_pblock_t s_pb[24];      // physical_blocks entries
 * static uint8_t         s_scratch[512];
 * ra_ftl_t        ftl = {};
 * ra_io_blockdev_t bd = {};             // clean free-overwrite device for FAT
 * (void)ra_ftl_init(&ftl, &raw, s_map, 16U, s_pb, 24U, s_scratch);
 * (void)ra_ftl_as_blockdev(&ftl, &bd);  // FAT now mounts on bd
 * @endcode
 *
 ...
 * addresses at most ::k_ra_ftl_max_pblocks physical blocks. ::k_ra_ftl_unmapped
 * is the reserved sentinel meaning "this logical block has never been written".
 *
 * @since 0.1.0
 */
typedef enum : uint16_t {
  k_ra_ftl_unmapped    = 0xFFFF, /**< `map[]` sentinel: logical block unwritten. */
  k_ra_ftl_max_pblocks = 0xFFFE, /**< Max physical blocks an FTL may wrap.       */
  k_ra_ftl_min_spare   = 1,      /**< Minimum spare blocks (`P - L >= 1`).       */
} ra_ftl_const_t;

/**
 * @enum ra_ftl_pstate_t
...
typedef enum : uint8_t {
  k_ra_ftl_pstate_free  = 0, /**< Blank (reads as erase value); allocatable. */
  k_ra_ftl_pstate_live  = 1, /**< Holds current data for one logical block.  */
  k_ra_ftl_pstate_stale = 2, /**< Superseded; reclaimable by erase to FREE.  */
} ra_ftl_pstate_t;
 ...
 *
 * @invariant `state` is one of ::ra_ftl_pstate_t.
...
typedef struct {
  ...
  uint8_t  state;       /**< ::ra_ftl_pstate_t lifecycle state (private).   */
} ra_ftl_pblock_t;
 ...
 *
 * @details
 * Zero-initialise (`= {}`) and pass to ::ra_ftl_init, which validates the
 ...
 * `ra_ftl_*` functions and the block device produced by ::ra_ftl_as_blockdev.
...
/* cppcheck-suppress-begin [unusedStructMember] */
typedef struct {
  const ra_io_blockdev_t* raw;             /**< Underlying erase-before-write dev. */
  uint16_t*               map;             /**< logical->physical map (private).   */
  ra_ftl_pblock_t*        pblocks;         /**< Per-physical metadata (private).   */
 ...
 * `logical_blocks + k_ra_ftl_min_spare` physical blocks), records the
 * caller-provided storage, snapshots the medium erase value, and sets every
 * logical block to ::k_ra_ftl_unmapped and every physical block to FREE. No
 ...
 *                             (>= `logical_blocks + k_ra_ftl_min_spare`).
 * @param[out] scratch         Caller 512-byte copy scratch buffer.
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                  FTL initialised and ready.
 * @retval k_ra_err_null_ptr        Any pointer argument was NULL.
 * @retval k_ra_err_invalid_size    `logical_blocks` was zero or `physical_blocks`
 *                                  exceeds ::k_ra_ftl_max_pblocks.
 * @retval k_ra_err_invalid_arg     Underlying caps query failed, the device is
 *                                  read-only, `erase_unit_blocks != 1`, or there
 *                                  is no spare block.
 * @retval k_ra_err_not_initialized No backend is bound to `raw`.
 *
 ...
 *
 * @see ra_ftl_as_blockdev
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ftl_init(ra_ftl_t*               bd,
...
/**
 * @brief Expose an initialised FTL as a free-overwrite ::ra_io_blockdev_t.
 *
 * @details
 * Binds `out` to the FTL vtable with the ::ra_ftl_t as its context, so the
 ...
 *
 * @param[in]  ftl FTL handle previously initialised by ::ra_ftl_init.
 * @param[out] out Block-device handle to bind (zero-initialised by the caller).
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                  `out` bound to the FTL.
 * @retval k_ra_err_null_ptr        `ftl` or `out` was NULL.
 * @retval k_ra_err_not_initialized `ftl` was not initialised (no raw device).
 *
 * @pre `ftl` was initialised by ::ra_ftl_init.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ftl_as_blockdev(ra_ftl_t* ftl, ra_io_blockdev_t* out);
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                  `*max_out` and `*min_out` populated.
 * @retval k_ra_err_null_ptr        Any pointer argument was NULL.
 * @retval k_ra_err_not_initialized `ftl` was not initialised.
 *
 * @pre `ftl` was initialised by ::ra_ftl_init.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ftl_wear_stats(const ra_ftl_t* ftl, uint32_t* max_out, uint32_t* min_out);
