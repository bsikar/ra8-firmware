...
test "priv should_dispatch_event: all three C MC/DC vectors" {
    var sink: u8 = 0;
    try std.testing.expect(abi.priv_usb_pal_should_dispatch_event(null, 0x0001, 0x0000) == 0);
    try std.testing.expect(abi.priv_usb_pal_should_dispatch_event(&sink, 0x0001, 0x0000) != 0);
    try std.testing.expect(abi.priv_usb_pal_should_dispatch_event(&sink, 0x0000, 0x0000) == 0);
}

test "priv ep_out_of_range: all three C MC/DC vectors" {
    try std.testing.expect(abi.priv_usb_pal_ep_out_of_range(1, 10) == 0);
    try std.testing.expect(abi.priv_usb_pal_ep_out_of_range(0, 10) != 0);
    try std.testing.expect(abi.priv_usb_pal_ep_out_of_range(11, 10) != 0);
}
...
test "a failing driver init leaves the PAL uninitialized" {
    ...
    try std.testing.expectEqual(err_invalid_state, abi.ra8_usb_pal_attach(1));
    try std.testing.expect(usb_handler == null);
}
...
test "attach then detach tracks the cached state" {
    ...
    try std.testing.expectEqual(err_ok, abi.ra8_usb_pal_attach(0));
    try std.testing.expectEqual(err_ok, abi.ra8_usb_pal_get_state(&state));
    try std.testing.expectEqual(@as(u8, 0), state);
}
