...
// C23 bool is one byte; use u8 at the exported boundary and return only 0/1.
pub export fn priv_usb_pal_should_dispatch_event(
    event_fn: ?*const anyopaque,
    mask: u16,
    none_value: u16,
) callconv(.c) u8 {
    return @intFromBool(core.shouldDispatchEvent(event_fn, mask, none_value));
}

pub export fn priv_usb_pal_ep_out_of_range(ep_addr: u8, ep_max: u8) callconv(.c) u8 {
...
// Match the unchanged C23 bool parameter with its one-byte ABI representation.
pub export fn ra8_usb_pal_attach(attached: u8) callconv(.c) u16 {
    if (!s_state.initialized) return err_invalid_state;

    // `ra8_usb_device_attach` rejects only an invalid controller selector, and
    // a successful init stored a validated one.
    _ = ra8_usb_device_attach(s_state.speed, attached != 0);
    s_state.state = if (attached != 0) .attached else .detached;
    return err_ok;
}
