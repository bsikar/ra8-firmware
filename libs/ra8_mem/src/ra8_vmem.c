 ...
 * caller-owned ::ra8_vmem_frame_t array (prev/next, hash_next); nothing is
...
static int32_t priv_hash_lookup(const ra8_vmem_t* vm, uint32_t object_id, uint64_t aligned_off)
{
  ...
  int32_t        cur = vm->cfg.buckets[b];
  while (cur != -1) {
    const ra8_vmem_frame_t* m = &vm->cfg.meta[cur];
 ...
 *          Extracted from ::ra8_vmem_init to keep the entry point short.
 *
...
ra8_err_t ra8_vmem_init(ra8_vmem_t* vm, const ra8_vmem_cfg_t* cfg)
{
  ...
  const ra8_err_t verr = priv_vmem_validate_cfg(cfg);
  if (verr != k_ra8_ok) {
    return verr;
  }
 ...
 *
 * @return ra8_err_t Error code.
 * @retval k_ra8_ok         Page loaded, inserted, and pinned.
 * @retval k_ra8_err_no_mem Every frame is pinned.
 * @retval k_ra8_err_*      The loader's error (the victim is dropped, cold).
 *
...
priv_vmem_miss(ra8_vmem_t* vm, uint32_t object_id, uint64_t aligned_off, void** out_page)
{
  ...
  ra8_vmem_frame_t* m = &vm->cfg.meta[v];
  if (m->valid != 0U) {
  ...
  const ra8_err_t lerr = vm->cfg.loader(vm->cfg.loader_ctx,
                                        object_id,
                                        aligned_off,
