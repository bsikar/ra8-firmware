 ...
 * @pre Caller has already range-checked ``fs`` via ``RA_CHECK_RANGE_TAG``.
 ...
 *
 * @return Transport error code; ``k_ra_ok`` on success.
 * @retval k_ra_ok Register byte updated.
 * @retval other   Forwarded from the read or write transport call.
 *
 * @pre ``dev`` is non-NULL and previously bound by ``ra_lsm6dso_init``.
 ...
 * @since 0.1.0
 */
static ra_err_t internal_lsm6dso_rmw_odr(ra_lsm6dso_t* dev, uint8_t reg, uint8_t odr_bits)
{
  uint8_t        now = 0U;
  const ra_err_t r   = internal_lsm6dso_read_byte(dev, reg, &now);
  if (r != k_ra_ok) {
    return r;
  }
...
ra_err_t ra_lsm6dso_set_accel_range(ra_lsm6dso_t* dev, ra_lsm6dso_xl_fs_t fs)
{
  ...
  uint8_t        current = 0U;
  const ra_err_t r = internal_lsm6dso_read_byte(dev, (uint8_t)k_lsm6dso_reg_ctrl1_xl, &current);
  if (r != k_ra_ok) {
    return r;
  }
  ...
  const ra_err_t w = internal_lsm6dso_write_byte(dev, (uint8_t)k_lsm6dso_reg_ctrl1_xl, merged);
  if (w != k_ra_ok) {
    return w;
  }
...
ra_err_t ra_lsm6dso_set_gyro_range(ra_lsm6dso_t* dev, ra_lsm6dso_g_fs_t fs)
{
  ...
  uint8_t        current = 0U;
  const ra_err_t r = internal_lsm6dso_read_byte(dev, (uint8_t)k_lsm6dso_reg_ctrl2_g, &current);
  if (r != k_ra_ok) {
    return r;
  }
  ...
  const ra_err_t w       = internal_lsm6dso_write_byte(dev, (uint8_t)k_lsm6dso_reg_ctrl2_g, merged);
  if (w != k_ra_ok) {
    return w;
  }
  dev->gyro_fs_code = fs;
...
ra_err_t ra_lsm6dso_set_odr(ra_lsm6dso_t* dev, ra_lsm6dso_odr_t odr)
{
  ...
  const ra_err_t rxl = internal_lsm6dso_rmw_odr(dev, (uint8_t)k_lsm6dso_reg_ctrl1_xl, odr_bits);
  if (rxl != k_ra_ok) {
    return rxl;
  }
  const ra_err_t rg = internal_lsm6dso_rmw_odr(dev, (uint8_t)k_lsm6dso_reg_ctrl2_g, odr_bits);
  if (rg != k_ra_ok) {
    return rg;
  }
 ...
 * @retval k_ra_ok ``*out`` populated with the latest sample.
 * @retval other   Forwarded from ``dev->bus.read_regs``.
 *
 * @pre ``dev`` was bound by ``ra_lsm6dso_init`` and is initialized.
 * @pre ``out`` points at writable ``ra_lsm6dso_xyz_t`` storage.
 ...
 * @since 0.1.0
 */
static ra_err_t internal_lsm6dso_read_xyz(ra_lsm6dso_t* dev, uint8_t reg, ra_lsm6dso_xyz_t* out)
{
  uint8_t        bytes[6] = {};
  const ra_err_t r =
 ...
 *
 * @return Transport error code; ``k_ra_ok`` on success.
 * @retval k_ra_ok ``*out_n`` populated with the DIFF_FIFO count.
 ...
 * @since 0.1.0
 */
static ra_err_t internal_lsm6dso_read_fifo_depth(ra_lsm6dso_t* dev, uint32_t* out_n)
{
  uint8_t        status[2] = {};
 ...
 *
 * @return Transport error code; ``k_ra_ok`` on success.
 * @retval k_ra_ok ``out_buf`` populated with ``n_words * 7`` bytes.
 * @retval other   Forwarded from ``dev->bus.read_regs``.
 *
 * @pre ``dev`` was bound by ``ra_lsm6dso_init`` and is initialized.
 ...
 * @since 0.1.0
 */
static ra_err_t internal_lsm6dso_burst_fifo(ra_lsm6dso_t* dev, uint8_t* out_buf, uint32_t n_words)
{
...
/**
 * @brief Validate the inputs to ``ra_lsm6dso_read_xl_gyro_fifo``.
 *
 * @details
 ...
 * matches the public API one-to-one: NULL pointer -> ``k_ra_err_null_ptr``,
 * not initialized -> ``k_ra_err_not_initialized``, zero ``max_words``
 * -> ``k_ra_err_invalid_arg``.
 *
 ...
 *
 * @return ``k_ra_ok`` if all preconditions hold, otherwise the
 *         matching error code.
 * @retval k_ra_ok                All preconditions satisfied.
 * @retval k_ra_err_null_ptr      Any of ``dev`` / ``out_buf`` / ``out_words`` is NULL.
 * @retval k_ra_err_not_initialized ``dev->initialized`` is false.
 * @retval k_ra_err_invalid_arg   ``max_words`` is zero.
 *
 ...
 * @since 0.1.0
 */
static ra_err_t internal_lsm6dso_fifo_check_args(ra_lsm6dso_t*   dev,
                                      ...
                                      uint32_t*     out_words)
{
  ...
  const ra_err_t chk = internal_lsm6dso_fifo_check_args(dev, out_buf, max_words, out_words);
  if (chk != k_ra_ok) {
    return chk;
  }

  uint32_t       live = 0U;
  const ra_err_t rs   = internal_lsm6dso_read_fifo_depth(dev, &live);
  if (rs != k_ra_ok) {
    return rs;
  }
  ...
  const ra_err_t rd = internal_lsm6dso_burst_fifo(dev, out_buf, to_read);
  if (rd != k_ra_ok) {
    return rd;
  }
  *out_words = to_read;
