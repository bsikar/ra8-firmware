 ...
 * Principles for C"):
 *
 *   - The driver does not call ``ra_iic_b_*`` or ``ra_spi_*`` directly.
 *   - Instead the caller hands a ``ra_lsm6dso_bus_t`` interface whose
 *     ``read_regs`` / ``write_regs`` callbacks address the part. The
 *     production firmware wires these to ``ra_iic_b_transfer`` /
 *     ``ra_iic_b_write`` (I2C) or ``ra_spi_xfer8`` (SPI); unit tests
 *     wire them to a software-mock transport in
 *     ``tests/test_ra_lsm6dso.c``.
 *
 * Surface (mirrors the deliverables list in the task brief):
 *
 *   - ``ra_lsm6dso_init``               Bind a bus + cache its handle.
 *   - ``ra_lsm6dso_who_am_i``           Returns ``0x6C`` if the part is
 *                                       alive (DS12140 sec 9.11 WHO_AM_I).
 *   - ``ra_lsm6dso_set_accel_range``    Configure +-2 / +-4 / +-8 / +-16 g.
 *   - ``ra_lsm6dso_set_gyro_range``     Configure +-125 / +-250 / +-500 /
 *                                       +-1000 / +-2000 dps.
 *   - ``ra_lsm6dso_set_odr``            Output Data Rate (12.5 .. 6664 Hz).
 *   - ``ra_lsm6dso_read_accel``         Raw 16-bit two's complement counts.
 *   - ``ra_lsm6dso_read_gyro``          Raw 16-bit two's complement counts.
 *   - ``ra_lsm6dso_read_temp``          On-die temperature in centi-degrees
 *                                       Celsius (DS12140 sec 9.5).
 *   - ``ra_lsm6dso_read_xl_gyro_fifo``  Batch read of XL+G samples from
 *                                       the embedded 9 kbyte FIFO.
 *
 * The driver carries no global state outside the caller's
 * ``ra_lsm6dso_t`` -- multiple LSM6DSO parts on different buses can
...
} ra_lsm6dso_i2c_addr_t;

/**
 * @enum ra_lsm6dso_reg_t
 ...
 * are identical for both peripherals; ``ra_lsm6dso_set_odr`` writes
...
/**
 * @struct ra_lsm6dso_xyz_t
 * @brief 3-axis raw sample (X, Y, Z as 16-bit two's-complement counts).
 *
 * @details
 * Returned by ``ra_lsm6dso_read_accel`` and ``ra_lsm6dso_read_gyro``.
 ...
 *
 * @param[in]  ctx Transport context (e.g. ``ra_lsm6dso_i2c_ctx_t`` or
 *                 ``ra_lsm6dso_spi_ctx_t`` cast to ``void*``).
 ...
 * @retval k_ra_err_nack     Target NACKed (I2C) or returned a hard
 *                           error (SPI).
 * @retval k_ra_err_null_ptr ``buf`` is NULL with non-zero ``len``.
 */
typedef ra_err_t (*ra_lsm6dso_read_fn_t)(void* ctx, uint8_t reg, uint8_t* buf, uint32_t len);

/**
 * @typedef ra_lsm6dso_write_fn_t
 ...
 * @brief Transport interface bound at ``ra_lsm6dso_init`` time.
 *
 * @details
 * Production code wires ``read_regs`` and ``write_regs`` to thin
 * adapters over ``ra_iic_b_transfer`` / ``ra_iic_b_write`` or
 * ``ra_spi_xfer8``. Unit tests wire them to a canned-response mock.
 * The ``ctx`` field is passed through to both callbacks unchanged.
 */
/* cppcheck-suppress-begin [unusedStructMember] */
typedef struct {
  ra_lsm6dso_read_fn_t  read_regs;  /**< Read callback.  Non-NULL. */
  ra_lsm6dso_write_fn_t write_regs; /**< Write callback. Non-NULL. */
 ...
 * configuration codes so ``ra_lsm6dso_read_*`` can apply the right
 ...
 * expected to follow this with ``ra_lsm6dso_who_am_i`` (sanity check)
 * and then ``ra_lsm6dso_set_*`` calls to configure the part.
 *
 ...
 * @retval k_ra_ok              Driver state initialized.
 * @retval k_ra_err_null_ptr    ``out_dev`` or ``bus`` is NULL, or one
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_lsm6dso_init(ra_lsm6dso_t* out_dev, const ra_lsm6dso_bus_t* bus);
 ...
 * @retval k_ra_err_null_ptr    ``dev`` or ``out_id`` is NULL.
 * @retval k_ra_err_invalid_state ``dev`` has not been initialized.
 * @retval k_ra_err_nack        Transport reported a NACK.
 *
 * @pre ``ra_lsm6dso_init`` returned ``k_ra_ok`` for ``dev``.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_lsm6dso_who_am_i(ra_lsm6dso_t* dev, uint8_t* out_id);
 ...
 *
 * @return ``ra_err_t`` Error code.
 * @retval k_ra_ok                Range programmed.
 * @retval k_ra_err_null_ptr      ``dev`` is NULL.
 * @retval k_ra_err_invalid_state ``dev`` not initialized.
 * @retval k_ra_err_invalid_arg   ``fs`` out of range.
 * @retval k_ra_err_nack          Transport reported a NACK.
 *
 * @pre ``ra_lsm6dso_init`` returned ``k_ra_ok`` for ``dev``.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_lsm6dso_set_accel_range(ra_lsm6dso_t* dev, ra_lsm6dso_xl_fs_t fs);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_lsm6dso_set_gyro_range(ra_lsm6dso_t* dev, ra_lsm6dso_g_fs_t fs);
 ...
 *
 * @return ``ra_err_t`` Error code.
 * @retval k_ra_ok                ODR programmed for XL and G.
 * @retval k_ra_err_null_ptr      ``dev`` is NULL.
 * @retval k_ra_err_invalid_state ``dev`` not initialized.
 * @retval k_ra_err_invalid_arg   ``odr`` out of range.
 * @retval k_ra_err_nack          Transport reported a NACK.
 *
 * @pre ``ra_lsm6dso_init`` returned ``k_ra_ok`` for ``dev``.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_lsm6dso_set_odr(ra_lsm6dso_t* dev, ra_lsm6dso_odr_t odr);
 ...
 *
 * @return ``ra_err_t`` Error code.
 * @retval k_ra_ok                Sample read.
 * @retval k_ra_err_null_ptr      ``dev`` or ``out`` is NULL.
 * @retval k_ra_err_invalid_state ``dev`` not initialized.
 * @retval k_ra_err_nack          Transport reported a NACK.
 *
 * @pre ``ra_lsm6dso_init`` returned ``k_ra_ok`` for ``dev``.
 * @pre Caller has previously enabled the XL with ``ra_lsm6dso_set_odr``.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_lsm6dso_read_accel(ra_lsm6dso_t* dev, ra_lsm6dso_xyz_t* out);
 ...
 *
 * @pre ``ra_lsm6dso_init`` returned ``k_ra_ok`` for ``dev``.
 * @pre Caller has previously enabled the G with ``ra_lsm6dso_set_odr``.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_lsm6dso_read_gyro(ra_lsm6dso_t* dev, ra_lsm6dso_xyz_t* out);
 ...
 *
 * @return ``ra_err_t`` Error code.
 * @retval k_ra_ok                Temperature read.
 * @retval k_ra_err_null_ptr      ``dev`` or ``out_centi_c`` is NULL.
 * @retval k_ra_err_invalid_state ``dev`` not initialized.
 * @retval k_ra_err_nack          Transport reported a NACK.
 *
 * @pre ``ra_lsm6dso_init`` returned ``k_ra_ok`` for ``dev``.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_lsm6dso_read_temp(ra_lsm6dso_t* dev, int32_t* out_centi_c);
 ...
 *
 * @return ``ra_err_t`` Error code.
 * @retval k_ra_ok                FIFO drained (possibly to zero words).
 * @retval k_ra_err_null_ptr      Any of the pointers is NULL.
 * @retval k_ra_err_invalid_state ``dev`` not initialized.
 * @retval k_ra_err_invalid_arg   ``max_words == 0``.
 * @retval k_ra_err_nack          Transport reported a NACK.
 *
 * @pre ``ra_lsm6dso_init`` returned ``k_ra_ok`` for ``dev``.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_lsm6dso_read_xl_gyro_fifo(ra_lsm6dso_t* dev,
                                                    uint8_t*      out_buf,
                                                    uint32_t      max_words,
                                                    uint32_t*     out_words);
