 ...
 * fields documented in `ra8d2_mpu_regs.h`; this layer adds
 ...
 * rejects that combination with `k_ra_err_invalid_arg`.
 */
typedef enum : uint8_t {
  k_ra_mpu_perm_none = 0U, /**< No access at this privilege level.  */
  k_ra_mpu_perm_ro   = 1U, /**< Read-only at this privilege level.  */
  k_ra_mpu_perm_rw   = 2U, /**< Read-write at this privilege level. */
} ra_mpu_perm_t;
 ...
 */
typedef enum : uint8_t {
  k_ra_mpu_share_non   = 0U, /**< Non-shareable.   */
  k_ra_mpu_share_outer = 2U, /**< Outer shareable. */
  k_ra_mpu_share_inner = 3U, /**< Inner shareable. */
} ra_mpu_share_t;

/**
 * @enum ra_mpu_attr_idx_t
 ...
 */
typedef enum : uint8_t {
  k_ra_mpu_attr_idx_0 = 0U, /**< MAIR0 byte 0. */
  k_ra_mpu_attr_idx_1 = 1U, /**< MAIR0 byte 1. */
  k_ra_mpu_attr_idx_2 = 2U, /**< MAIR0 byte 2. */
  k_ra_mpu_attr_idx_3 = 3U, /**< MAIR0 byte 3. */
  k_ra_mpu_attr_idx_4 = 4U, /**< MAIR1 byte 0. */
  k_ra_mpu_attr_idx_5 = 5U, /**< MAIR1 byte 1. */
  k_ra_mpu_attr_idx_6 = 6U, /**< MAIR1 byte 2. */
  k_ra_mpu_attr_idx_7 = 7U, /**< MAIR1 byte 3. */
} ra_mpu_attr_idx_t;

/**
 * @enum ra_mpu_size_limits_t
 * @brief Architectural size limits for an MPU region.
 *
 * @details Armv8-M requires 32-byte alignment of base and limit.
 */
typedef enum : uint32_t {
  k_ra_mpu_min_region_size = 32U, /**< Minimum power-of-two size in bytes. */
} ra_mpu_size_limits_t;

/**
 * @struct ra_mpu_region_t
 ...
 *
 * @invariant size is a power of two >= k_ra_mpu_min_region_size.
...
typedef struct {
  ...
  ra_mpu_perm_t     priv;       /**< Privileged-mode permission.          */
  ra_mpu_perm_t     unpriv;     /**< Unprivileged-mode permission.        */
  bool              executable; /**< If true, instruction fetch allowed.  */
  ra_mpu_share_t    shareable;  /**< SH[1:0] field.                       */
  ra_mpu_attr_idx_t attr_idx;   /**< MAIR slot index.                     */
} ra_mpu_region_t;
 ...
 * @retval k_ra_ok              Region table programmed and MPU enabled.
 * @retval k_ra_err_null_ptr    cfg == NULL or cfg->regions == NULL.
 * @retval k_ra_err_invalid_arg region_count > DREGION, or a region has
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_mpu_enable(void);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_mpu_disable(void);
 ...
 *
 * @return ra_err_t error code.
 * @retval k_ra_ok              Region updated.
 * @retval k_ra_err_null_ptr    region_cfg == NULL.
 * @retval k_ra_err_invalid_arg region >= DREGION, or region_cfg has
 *                              non-power-of-two size, misaligned base,
 *                              or unrepresentable AP pair.
 *
 * @pre region_cfg != NULL.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_mpu_set_region(uint8_t region, const ra_mpu_region_t* region_cfg);
