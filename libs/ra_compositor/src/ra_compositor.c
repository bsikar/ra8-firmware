 ...
 * damage compositing described in ra_compositor.h. Pixel drawing is delegated
 * to `ra_gfx` (used read-only); the geometry helpers are pure value functions
...
static const char* const s_tag = "ra_compositor";

/**
 * @enum ra_compositor_priv_t
 * @brief Private layout magnitudes used by the tiling math.
 */
typedef enum : int32_t {
  k_ra_compositor_edges = 2, /**< Padding is inset on both edges of an axis. */
} ra_compositor_priv_t;
 ...
 * @since 0.1.0
 */
static bool s_rect_empty(ra_compositor_rect_t r)
{
  if (r.w <= 0) {
    return true;
  }
 ...
 * @since 0.1.0
 */
static ra_compositor_rect_t s_rect_intersect(ra_compositor_rect_t a, ra_compositor_rect_t b)
{
  int32_t x0 = a.x;
  if (b.x > x0) {
    x0 = b.x;
  ...
  ra_compositor_rect_t out = {x0, y0, x1 - x0, y1 - y0};
  if (out.w < 0) {
    out.w = 0;
  }
  if (out.h < 0) {
 ...
 * @since 0.1.0
 */
static ra_compositor_rect_t s_rect_union(ra_compositor_rect_t acc, ra_compositor_rect_t r)
{
  if (s_rect_empty(r)) {
    return acc;
  }
 ...
 * @since 0.1.0
 */
static void s_measure_children(const ra_compositor_t* scene,
                               uint16_t               pi,
                               int32_t*               out_n,
                               int32_t*               out_fixed,
                               int32_t*               out_flex)
 ...
 * @since 0.1.0
 */
static void s_place_child(ra_compositor_widget_t* c,
...
static void s_tile_children(ra_compositor_t* scene, uint16_t pi)
{
  ...
  int32_t axis_row = (p->axis == k_ra_compositor_axis_row) ? 1 : 0;
...
/**
 * @brief Paint one node into the ra_gfx framebuffer, clipped to its bounds.
 *
 ...
 *
 * @note Not thread-safe; uses the global ra_gfx clip state.
 * @since 0.1.0
 */
static void s_paint_node(const ra_compositor_widget_t* n, ra_compositor_rect_t clip)
{
  if (n->kind == k_ra_compositor_kind_container) {
    return;
  }
  ra_compositor_rect_t eff = s_rect_intersect(n->bounds, clip);
  if (s_rect_empty(eff)) {
    return;
  }
  (void)ra_gfx_set_clip(eff.x, eff.y, eff.w, eff.h);
  (void)ra_gfx_rect(n->bounds.x, n->bounds.y, n->bounds.w, n->bounds.h, n->color, true);
  if (n->kind != k_ra_compositor_kind_label) {
    return;
  }
  if (n->text == nullptr) {
    return;
  }
  (void)ra_gfx_text_out(n->bounds.x, n->bounds.y, n->text, &ra_gfx_font_8x16, n->fg, n->color);
}
...
ra_err_t ra_compositor_init(ra_compositor_t* scene, ra_compositor_widget_t* pool, uint16_t cap)
{
  RA_CHECK_NULL_PTR(scene, s_tag, "scene must not be nullptr");
  RA_CHECK_NULL_PTR(pool, s_tag, "pool must not be nullptr");
  if (cap == 0U) {
...
ra_err_t ra_compositor_add(ra_compositor_t*              scene,
                           const ra_compositor_widget_t* widget,
                           uint16_t                      parent,
                           uint16_t*                     out_index)
{
  RA_CHECK_NULL_PTR(scene, s_tag, "scene must not be nullptr");
  RA_CHECK_NULL_PTR(widget, s_tag, "widget must not be nullptr");
  RA_CHECK_NULL_PTR(out_index, s_tag, "out_index must not be nullptr");
  if (scene->count >= scene->cap) {
    return k_ra_err_no_mem;
  }
  if (scene->count == 0U) {
    if (parent != (uint16_t)k_ra_compositor_no_parent) {
      return k_ra_err_invalid_arg;
    }
  } else {
...
ra_err_t ra_compositor_layout(ra_compositor_t* scene, const ra_compositor_rect_t* frame)
{
  RA_CHECK_NULL_PTR(scene, s_tag, "scene must not be nullptr");
...
ra_err_t ra_compositor_render(const ra_compositor_t* scene, const ra_compositor_rect_t* clip)
{
  RA_CHECK_NULL_PTR(scene, s_tag, "scene must not be nullptr");
  if (scene->count == 0U) {
    return k_ra_err_invalid_arg;
  }
  ra_compositor_rect_t outer = scene->nodes[0].bounds;
  if (clip != nullptr) {
    outer = *clip;
  }
  for (uint16_t i = 0U; i < scene->count; ++i) {
...
ra_err_t ra_compositor_damage(const ra_compositor_t* scene,
                              ra_compositor_rect_t*  out_rect,
                              uint16_t*              out_count)
{
  RA_CHECK_NULL_PTR(scene, s_tag, "scene must not be nullptr");
  RA_CHECK_NULL_PTR(out_rect, s_tag, "out_rect must not be nullptr");
  RA_CHECK_NULL_PTR(out_count, s_tag, "out_count must not be nullptr");
  ra_compositor_rect_t acc = {};
  uint16_t             n   = 0U;
  for (uint16_t i = 0U; i < scene->count; ++i) {
    if (!scene->nodes[i].visible) {
      continue;
...
ra_err_t ra_compositor_invalidate(ra_compositor_t* scene, uint16_t index)
{
  RA_CHECK_NULL_PTR(scene, s_tag, "scene must not be nullptr");
...
ra_err_t ra_compositor_clear_dirty(ra_compositor_t* scene)
{
  RA_CHECK_NULL_PTR(scene, s_tag, "scene must not be nullptr");
  if (scene->count > scene->cap) {
...
ra_err_t ra_compositor_compose_full(ra_compositor_t* scene, const ra_compositor_rect_t* frame)
{
  RA_CHECK_NULL_PTR(scene, s_tag, "scene must not be nullptr");
  RA_CHECK_NULL_PTR(frame, s_tag, "frame must not be nullptr");
  ra_err_t err = ra_compositor_layout(scene, frame);
  if (err != k_ra_ok) {
    return err;
  }
  err = ra_compositor_render(scene, nullptr);
  if (err != k_ra_ok) {
    return err;
  }
  return ra_compositor_clear_dirty(scene);
}

ra_err_t ra_compositor_compose_dirty(ra_compositor_t*            scene,
                                     ...
                                     uint16_t*                   out_count)
{
  ...
  err = ra_compositor_damage(scene, out_damage, out_count);
  if (err != k_ra_ok) {
    return err;
  }
  if (*out_count > 0U) {
    err = ra_compositor_render(scene, out_damage);
    if (err != k_ra_ok) {
      return err;
    }
  }
