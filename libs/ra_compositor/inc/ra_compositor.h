/**
 * @file ra_compositor.h
 * @brief Zero-heap framebuffer compositor: a tiled widget tree painted into
 *        a caller-owned framebuffer through ra_gfx, with dirty-region updates.
 *
 * @details
 * `ra_compositor` is the *framebuffer half* of issue #145. The sibling
 * `ra_widget` library models the pure layout / route / damage **selection**
 ...
 * **composites** a widget tree into an in-memory framebuffer (via `ra_gfx`,
 ...
 * index is strictly smaller (enforced by ::ra_compositor_add), so the pool is
 ...
 * `ra_gfx_set_clip`), so a node can never spill outside its rectangle. The
 ...
 * (`ra_gfx` discards everything outside it), which touches exactly the changed
 ...
 * on the RA8D2 panel. The library depends only on `ra_core` (errors / checks)
 * and `ra_gfx` (the pixel backend); it modifies neither.
 *
...
/**
 * @enum ra_compositor_sentinel_t
 * @brief Sentinel values for the widget pool.
 *
 * @details
 * `k_ra_compositor_no_parent` marks the root node, which has no parent and is
 * pinned directly to the frame by ::ra_compositor_layout.
 */
typedef enum : uint16_t {
  k_ra_compositor_no_parent = 0xFFFFU, /**< Root sentinel: this node has no parent. */
} ra_compositor_sentinel_t;

/**
 * @enum ra_compositor_kind_t
 * @brief The concrete kind of one widget node.
 *
 * @details
 * Selects how ::ra_compositor_render paints the node: a container draws
...
typedef enum : uint8_t {
  k_ra_compositor_kind_container = 0U, /**< Layout-only group; paints nothing. */
  k_ra_compositor_kind_rect      = 1U, /**< Solid filled rectangle.            */
  k_ra_compositor_kind_label     = 2U, /**< Background fill plus ASCII text.   */
} ra_compositor_kind_t;

/**
 * @enum ra_compositor_axis_t
 ...
 * that ::ra_compositor_layout writes as output. Set the inputs before adding
 ...
 * currently in use. ::ra_compositor_init binds the pool; ::ra_compositor_add
 ...
 */
typedef struct {
  ra_compositor_widget_t* nodes; /**< Caller-owned fixed widget pool (no allocation). */
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              Bound; the scene is empty.
 * @retval k_ra_err_null_ptr    @p scene or @p pool is NULL.
 ...
 * @see ra_compositor_add
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t
ra_compositor_init(ra_compositor_t* scene, ra_compositor_widget_t* pool, uint16_t cap);
 ...
 * the root (`parent == k_ra_compositor_no_parent`); every later node must name
 ...
 * @param[in]     parent    Parent index, or k_ra_compositor_no_parent for the root.
 * @param[out]    out_index Receives the new node's index (non-NULL).
 *
 * @return ra_err_t
 * @retval k_ra_ok              Added; `*out_index` is the new node.
 * @retval k_ra_err_null_ptr    @p scene, @p widget, or @p out_index is NULL.
 * @retval k_ra_err_no_mem      The pool is full (`count == cap`).
 * @retval k_ra_err_invalid_arg Bad parent: a non-root references an index that
 ...
 * @pre The first node uses `k_ra_compositor_no_parent`; later nodes use an
 *      existing parent index.
 * @post On success `scene->count` grew by one and `*out_index == count - 1`.
 * @post On failure the scene is unchanged.
 *
 * @note Not thread-safe.
 * @see ra_compositor_init
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_compositor_add(ra_compositor_t*              scene,
                                         const ra_compositor_widget_t* widget,
                                         uint16_t                      parent,
                                         uint16_t*                     out_index);
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              Laid out; every visible node has bounds.
 * @retval k_ra_err_null_ptr    @p scene or @p frame is NULL.
 * @retval k_ra_err_invalid_arg The scene is empty (`count == 0`).
 *
 ...
 *
 * @note Not thread-safe.
 * @see ra_compositor_render
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_compositor_layout(ra_compositor_t*            scene,
                                            const ra_compositor_rect_t* frame);
...
/**
 * @brief Paint every visible node into the bound ra_gfx framebuffer.
 *
 * @details
 ...
 * their text. The ra_gfx clip is reset to the full framebuffer on return.
 * Pixel-level drawing is delegated to `ra_gfx`, which the caller must have
 * bound to a framebuffer with `ra_gfx_init`.
 *
 * @param[in] scene Laid-out scene (non-NULL).
 * @param[in] clip  Region to confine painting to, or NULL for the full frame.
 *
 * @return ra_err_t
 * @retval k_ra_ok              Painted (clipped as requested).
 * @retval k_ra_err_null_ptr    @p scene is NULL.
 * @retval k_ra_err_invalid_arg The scene is empty (`count == 0`).
 *
 * @pre @p scene is non-NULL and has been laid out.
 * @pre `ra_gfx_init` has bound a framebuffer.
 * @post Visible nodes intersecting @p clip are painted; others are untouched.
 * @post The ra_gfx clip rectangle is the full framebuffer on return.
 *
 * @note Not thread-safe; shares the single global ra_gfx bind + clip state.
 * @see ra_compositor_damage
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_compositor_render(const ra_compositor_t*      scene,
                                            const ra_compositor_rect_t* clip);
 ...
 * rectangle to ::ra_compositor_render re-paints exactly the changed pixels.
 ...
 * @retval k_ra_err_null_ptr @p scene, @p out_rect, or @p out_count is NULL.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_compositor_damage(const ra_compositor_t* scene,
                                            ra_compositor_rect_t*  out_rect,
                                            uint16_t*              out_count);
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              Node marked dirty.
 * @retval k_ra_err_null_ptr    @p scene is NULL.
 * @retval k_ra_err_invalid_arg @p index is out of range.
 *
 ...
 *
 * @note Not thread-safe.
 * @see ra_compositor_clear_dirty
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_compositor_invalidate(ra_compositor_t* scene, uint16_t index);
 ...
 * ::ra_compositor_damage starts from a clean slate.
 *
 * @param[in,out] scene Scene to clean (non-NULL).
 *
 * @return ra_err_t
 * @retval k_ra_ok           All nodes cleaned.
 * @retval k_ra_err_null_ptr @p scene is NULL.
 *
 * @pre @p scene is non-NULL.
 ...
 *
 * @note Not thread-safe.
 * @see ra_compositor_invalidate
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_compositor_clear_dirty(ra_compositor_t* scene);

/**
 * @brief Lay out and fully composite the whole scene, then clear damage.
 *
 * @details
 * The full-frame compose cycle: ::ra_compositor_layout against @p frame,
 * ::ra_compositor_render with no clip (every visible node painted), then
 * ::ra_compositor_clear_dirty. Use this for the first frame or any full
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              Laid out, painted, and cleaned.
 * @retval k_ra_err_null_ptr    @p scene or @p frame is NULL.
 * @retval k_ra_err_invalid_arg The scene is empty (forwarded from layout).
 *
 * @pre @p scene and @p frame are non-NULL; `ra_gfx_init` bound a framebuffer.
 ...
 *
 * @note Not thread-safe.
 * @see ra_compositor_compose_dirty
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_compositor_compose_full(ra_compositor_t*            scene,
                                                  const ra_compositor_rect_t* frame);
 ...
 *
 * @details
 * The partial-update compose cycle the issue asks for: ::ra_compositor_layout
 * against @p frame, ::ra_compositor_damage to find the union of the dirty
 * nodes, ::ra_compositor_render clipped to that damage (a no-op when nothing
 * is dirty), then ::ra_compositor_clear_dirty. The painted result inside the
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              Composited; see @p out_count / @p out_damage.
 * @retval k_ra_err_null_ptr    Any pointer argument is NULL.
 * @retval k_ra_err_invalid_arg The scene is empty (forwarded from layout).
 *
 * @pre All pointer arguments are non-NULL; `ra_gfx_init` bound a framebuffer.
 * @pre @p scene has at least one node.
 ...
 *
 * @note Not thread-safe.
 * @see ra_compositor_compose_full
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_compositor_compose_dirty(ra_compositor_t*            scene,
                                                   const ra_compositor_rect_t* frame,
                                                   ra_compositor_rect_t*       out_damage,
                                                   uint16_t*                   out_count);
