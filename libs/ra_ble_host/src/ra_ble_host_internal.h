 ...
 * L2CAP core (ra_ble_l2cap.c) shares with the GAP advertising TU
 * (ra_ble_l2cap_advertise.c):
 *
 ...
 * API and must not be included outside libs/ra_ble_host/src/.
 *
 ...
 *          advertising TU (ra_ble_l2cap_advertise.c) so the advertise
 *          path can read the initialized flag and the configured GAP
 *          role without an extra accessor call.
 *
 * @note Access restricted to the libs/ra_ble_host/src TUs.
 *
