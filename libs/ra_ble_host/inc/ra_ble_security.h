 ...
 *     security engine via libs/ra_hal/ra_rsip rather than the
...
/**
 * @enum ra_ble_security_event_kind_t
...
/**
 * @struct ra_ble_security_config_t
 * @brief Configuration passed to ``ra_ble_security_init``.
 *
 * @details
...
/**
 * @struct ra_ble_security_event_t
...
/**
 * @typedef ra_ble_security_event_fn_t
 ...
 * @retval k_ra_ok                   SMP layer is up.
 * @retval k_ra_err_null_ptr         ``cfg == NULL``.
 * @retval k_ra_err_invalid_arg      ``cfg->io_cap`` out of range.
 * @retval k_ra_err_not_initialized  ra_ble_host_init was never called.
 * @retval k_ra_err_hw_init_failed   RSIP self-test failed (offload only).
 *
 ...
 *
 * @see ra_ble_security_close()
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ble_security_init(const ra_ble_security_config_t* cfg);
 ...
 *
 * @pre ra_ble_security_init succeeded previously.
 * @post Subsequent SMP calls return k_ra_err_not_initialized.
 *
 * @since 0.1.0
 */
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                   Pairing request queued.
 * @retval k_ra_err_not_initialized  Security wrapper not initialized.
 * @retval k_ra_err_invalid_arg      ``conn_handle`` is not connected.
 *
 * @pre ra_ble_security_init() succeeded.
 * @pre ``conn_handle`` references an established LE connection.
 * @post Pairing event is posted to the host event queue.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ble_security_pair(uint16_t conn_handle);
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                   Reply delivered to NimBLE.
 * @retval k_ra_err_not_initialized  Security wrapper not initialized.
 * @retval k_ra_err_invalid_arg      ``passkey > 999999`` or unknown handle.
 *
 * @pre A k_ra_ble_sec_evt_passkey_request or _numeric_compare event
 ...
 */
[[nodiscard]] ra_err_t
ra_ble_security_passkey_reply(uint16_t conn_handle, uint32_t passkey, uint8_t accept);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ble_security_clear_bonds(void);

/**
 * @brief Return the count of currently stored bonds.
 *
 * @param[out] out_count Receives the count (0..k_ra_ble_security_max_bonds).
 *
 * @return ra_err_t Error code.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ble_security_attach_event_handler(ra_ble_security_event_fn_t fn,
                                                            void*                      ctx);
