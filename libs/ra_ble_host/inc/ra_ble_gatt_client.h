 ...
 * ra_err_t and hides the NimBLE event-callback signature behind a
...
/**
 * @enum ra_ble_gatt_client_limits_t
 ...
 */
typedef void (*ra_ble_gatt_disc_fn_t)(void* ctx, const ra_ble_gatt_service_t* svc, uint16_t status);

/**
 * @typedef ra_ble_gatt_read_fn_t
 ...
 * @param[in] status 0 on success, BLE_HS_E* on failure.
 */
typedef void (*ra_ble_gatt_read_fn_t)(void*          ctx,
                                      const uint8_t* data,
                                      uint16_t       len,
                                      uint16_t       status);

/**
 * @typedef ra_ble_gatt_write_fn_t
 * @brief Completion callback for a GATT write request.
 *
 * @param[in] ctx    User context.
 * @param[in] status 0 on success, BLE_HS_E* on failure.
 */
typedef void (*ra_ble_gatt_write_fn_t)(void* ctx, uint16_t status);

/**
 * @typedef ra_ble_gatt_notify_fn_t
 ...
 */
typedef void (*ra_ble_gatt_notify_fn_t)(void*          ctx,
 ...
 * @retval k_ra_ok                   Discovery started.
 * @retval k_ra_err_null_ptr         ``cb == NULL``.
 * @retval k_ra_err_not_initialized  Stack not initialized.
 * @retval k_ra_err_invalid_arg      ``conn_handle`` not connected.
 *
 * @pre ra_ble_host_init() succeeded with role==central.
 ...
 *
 * @see ra_ble_gatt_read()
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t
ra_ble_gatt_discover_services(uint16_t conn_handle, ra_ble_gatt_disc_fn_t cb, void* ctx);
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                   Read queued.
 * @retval k_ra_err_null_ptr         ``cb == NULL``.
 * @retval k_ra_err_not_initialized  Stack not initialized.
 * @retval k_ra_err_invalid_arg      Unknown ``conn_handle``.
 *
 ...
 * @param[in] len          Byte count (0..k_ra_ble_gatt_client_max_write_bytes).
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ble_gatt_write(uint16_t               conn_handle,
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                   CCCD write queued.
 * @retval k_ra_err_not_initialized  Stack not initialized.
 * @retval k_ra_err_invalid_arg      Unknown handle, or both enable
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ble_gatt_subscribe(uint16_t                conn_handle,
