 ...
 * - ``ra_xspi_set_xip_mode / set_dtr_mode / calibrate_dqs / suspend /
 ...
 * ``ra_xspi_flash.c``; the two manual-command primitives it exports
 * (``ra_xspi_kick_command`` / ``ra_xspi_issue_simple_opcode``) are
 * declared in ``ra_xspi_internal.h`` and reused here by the
...
ra_err_t ra_xspi_suspend(uint8_t instance)
{
  volatile r_xspi_regs_t* reg = ra_xspi(instance);
  RA_CHECK_NULL_PTR(reg, s_tag, "instance out of range");
  return ra_xspi_issue_simple_opcode(reg, k_ra_spi_flash_op_suspend);
}
...
ra_err_t ra_xspi_resume(uint8_t instance)
{
  volatile r_xspi_regs_t* reg = ra_xspi(instance);
  RA_CHECK_NULL_PTR(reg, s_tag, "instance out of range");
  return ra_xspi_issue_simple_opcode(reg, k_ra_spi_flash_op_resume);
}
