 ...
 * convenience surfaces live in the sibling `ra_mipi_dsi_dispatch.c`;
 * state shared across the two TUs lives in `ra_mipi_dsi_internal.h`.
 *
...
#include "ra_err.h"
#include "ra_log.h"
#include "ra_mipi_dsi_internal.h"
#include "ra_mstp.h"
 ...
 * ``ra_mipi_dsi_internal.h`` and read from IRQ context in
 * ``ra_mipi_dsi_dispatch.c``. Mutated only from the single-threaded
 ...
 * @since 0.1.0
 */
ra_mipi_dsi_event_fn_t s_mipi_dsi_event_fn;
 ...
 * @details
 * Declared `extern` (with the full contract) in
 * ``ra_mipi_dsi_internal.h``. Lifetime is owned by the caller of
 * `ra_mipi_dsi_attach_handler`.
 *
 ...
 * @details
 * Declared `extern` (with the full contract) in
 * ``ra_mipi_dsi_internal.h``. The receive ISR in
 * ``ra_mipi_dsi_dispatch.c`` copies long-packet payload bytes from
 ...
 * @details
 * Declared `extern` (with the full contract) in
 * ``ra_mipi_dsi_internal.h``. Cleared to zero once the response is
 ...
 * with ``ra_mipi_dsi_dispatch.c`` live in ``ra_mipi_dsi_internal.h``.
 */
...
/** @brief Implementation of `ra_mipi_dsi_internal_wait_eq()` -- bounded busy-poll. */
...
[[nodiscard]] ra_err_t ra_mipi_dsi_hs_clock_start(void)
{
  ...
  return ra_mipi_dsi_internal_wait_eq(&reg->PLSR,
...
[[nodiscard]] ra_err_t ra_mipi_dsi_send_command(const ra_mipi_dsi_command_t* cmd)
{
  ...
  if (cmd->p_rx_buffer != nullptr) {
    ...
    s_mipi_dsi_pending_rx_len    = k_ra_mipi_dsi_payload_max;
  }
