 ...
 *
 * @details
 * Two cohesive groups split out of ``ra_usb.c`` to keep every translation
 * unit under the 1000-line cap:
 *
 *  - The interrupt-delivery + power surface: the per-controller event
 *    callback table, ``ra_usb_dispatch`` (W0C-correct INTSTS0 ack +
 *    handler fan-out), ``ra_usb_intsts0_snapshot``, and the
 *    module-stop enter / exit helpers.
 *  - The host-mode lifecycle bring-up (``ra_usb_host_init`` /
 *    ``ra_usb_host_deinit`` / ``ra_usb_host_bus_reset`` /
 *    ``ra_usb_host_set_uact`` / ``ra_usb_host_setup_request``), peer of
 *    the device-mode lifecycle in ``ra_usb_device.c``.
 *
 * The shared register helpers and the USBHS PHY bring-up it calls are
 * declared in ``ra_usb_internal.h``. Modelled on FSP
