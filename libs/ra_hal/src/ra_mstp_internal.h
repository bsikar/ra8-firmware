/**
 * @file ra_mstp_internal.h
 * @brief Cross-TU surface for the ra_mstp module-stop driver (test access).
 *
 * @details
 ...
 * @retval k_ra_ok             The cared-for bits settled to @p expect.
 * @retval k_ra_err_hw_timeout They never settled within the spin budget.
 *
 * @pre ``reg`` < 5.
 * @pre The MSTPCR write being confirmed has already been issued.
 * @post On k_ra_ok the cared-for bits read back at @p expect.
 ...
 * loop-exit is routed through the ra_sim_mmio fault seam (which overrides the
 * decision to model timing), so the mask's tolerance of Non-secure-owned bits
 * is verified on silicon (the TrustZone e-reader boots through ra_mstp_init
 * with USBFS0 + USBHS delegated Non-secure), while the host tests cover the
 * mask value (ra_mstp_ns_mask_internal) and the init integration.
 *
 * @note Not thread-safe; init-time single-threaded context only.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t
ra_mstp_wait_reg_settle_internal(uint8_t reg, uint32_t expect, uint32_t care_mask);
 ...
 * @since 0.1.0
 */
[[nodiscard]] uint32_t ra_mstp_ns_mask_internal(uint8_t reg);
