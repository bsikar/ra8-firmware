 ...
 * ``ra_i2c.c`` to keep each translation unit under the file-size cap. Owns
 * the init / deinit / bit-rate sequence (HUM Ch 39.3.2 "Initial Settings"
 * p 2395), the runtime clock re-program (``ra_i2c_set_clock``) and the
 * error-status helpers (``ra_i2c_get_errors`` / ``ra_i2c_clear_errors``).
 *
 * The data-transfer plane (start / write / read / stop / scan) lives in
 * ``ra_i2c.c``. Both translation units share ``s_i2c_state`` and the log
 * tag via ``ra_i2c_internal.h``.
 *
...
static ra_err_t internal_i2c_bitrate(uint32_t bus_hz,
                                     ...
                                     uint8_t* out_brl)
{
  RA_CHECK_NULL_PTR(out_cks, s_i2c_tag, "bitrate: out_cks");
  RA_CHECK_NULL_PTR(out_brh, s_i2c_tag, "bitrate: out_brh");
...
{
  RA_CHECK_NULL_PTR(cfg, s_i2c_tag, "i2c_init: cfg");
  ...
  RA_RETURN_ON_ERROR(mst_err, s_i2c_tag, "i2c_init: mstp"); /* GCOVR_EXCL_BR_LINE */

  uint8_t        cks    = 0U;
  uint8_t        brh    = 0U;
  uint8_t        brl    = 0U;
  const ra_err_t br_err = internal_i2c_bitrate(cfg->bus_hz, cfg->pclkb_hz, &cks, &brh, &brl);
  RA_RETURN_ON_ERROR(br_err, s_i2c_tag, "i2c_init: bitrate"); /* GCOVR_EXCL_BR_LINE */
  ...
  ra_log_info_val(s_i2c_tag, "i2c_init channel", (uint32_t)channel);
  return k_ra_ok;
}
...
{
  RA_CHECK_NULL_PTR(out_mask, s_i2c_tag, "i2c_get_errors: out_mask");
