/**
 * @file ra_usb_internal.h
 * @brief Cross-TU surface for the ra_usb driver split.
 *
 * @par Tag
 * [Ring 3 / HAL] {World: NS}
 *
 ...
 *
 *  - ``ra_usb.c``           -- shared low-level register helpers
 *                              (the symbols promoted below) plus the
 *                              CFIFO byte-mover.
 *  - ``ra_usb_phy.c``       -- USBHS embedded-PHY bring-up sequence.
 *  - ``ra_usb_device.c``    -- device-mode lifecycle, status and the
 *                              endpoint/pipe configuration entry points.
 *  - ``ra_usb_xfer.c``      -- device-mode data path (queue IN/OUT, DCP
 *                              control data, SETUP drain, control resp).
 *  - ``ra_usb_irq.c``       -- IRQ dispatch / callbacks / power +
 *                              host-mode bring-up.
 *  - ``ra_usb_host_ctrl.c`` -- host-mode control-transfer engine.
 *  - ``ra_usb_host_bulk.c`` -- host-mode bulk-transfer engine.
 *
 ...
 * the public ``ra_usb_*`` API in ``ra_usb.h``; the symbols here are an
 ...
 * split can map a ::ra_usb_speed_t to its controller register block.
 *
 ...
 * @retval k_ra_mstp_usbhs ``speed == k_ra_usb_speed_hs``.
 * @retval k_ra_mstp_usbfs Any other speed value.
 ...
 * @since 0.1.0
 */
ra_mstp_t internal_mstp(ra_usb_speed_t speed);
 ...
 * @retval k_ra_ok FRDY asserted within the bound.
 ...
 * @since 0.1.0
 */
void internal_dcp_pid(volatile r_usb_regs_t* reg, ra_usb_pid_t pid);
 ...
 * @since 0.1.0
 */
void internal_pipe_pid(volatile r_usb_regs_t* reg, uint8_t pipe_num, ra_usb_pid_t pid);
 ...
 *
 * @details Promoted from a TU-private static in ``ra_usb_phy.c`` so both
 * the device-mode init (``ra_usb_device.c``) and the host-mode init
 * (``ra_usb_irq.c``) can run the UTMI PHY bring-up. Implements the
 ...
 * @since 0.1.0
 */
ra_err_t internal_usbhs_phy_bringup(volatile r_usb_regs_t* reg);

/**
 * @brief Programme the post-SYSCFG common registers (FIFO, DCP, INTENB).
 *
 * @details Promoted from a TU-private static in ``ra_usb_phy.c`` so the
 * device-mode init (``ra_usb_device.c``) can apply the shared FS/HS tail
 * of device bring-up.
 *
 ...
 *
 * @details Promoted from a TU-private static in ``ra_usb_phy.c`` so the
 * device-mode init (``ra_usb_device.c``) can run the FS SYSCFG sequence.
 *
 ...
 * @since 0.1.0
 */
ra_err_t internal_usbfs_module_bringup(volatile r_usb_regs_t* reg);
 ...
 * (``ra_usb_host_ctrl.c``) and the host bulk engine
 * (``ra_usb_host_bulk.c``) share one DEVADDn programmer. The DEVADDn
