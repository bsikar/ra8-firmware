 ...
 *
 * @details
 * Glues the host-mode bring-up paths in `ra_usb` to a CDC-ECM USB
 ...
 *
 *  - `R_USB_HCDC_ECM_Open`              -> `ra_usb_hcdc_ecm_init`
 *  - `R_USB_HCDC_ECM_Close`             -> `ra_usb_hcdc_ecm_close`
 *  - `R_USB_HCDC_ECM_Write`             -> `ra_usb_hcdc_ecm_send_frame`
 *  - `R_USB_HCDC_ECM_Read`              -> `ra_usb_hcdc_ecm_recv_frame`
 *  - `usb_hcdc_ecm_set_ethernet_packet_filter`
 *                                       -> `ra_usb_hcdc_ecm_set_packet_filter`
 *  - `R_USB_HCDC_ECM_LinkProcess`       -> `ra_usb_hcdc_ecm_get_link_status`
 *  - `usb_hcdc_ecm_check_config`        -> `internal_walk_config_descriptor`
 *  - `usb_hcdc_ecm_get_mac_address`     -> `ra_usb_hcdc_ecm_parse_mac`
 ...
 * (production) or directly via `ra_usb_hcdc_ecm_step` (tests). Each
...
/**
 * @enum ra_usb_hcdc_ecm_setup_field_t
...
typedef enum : uint8_t {
  ...
  k_ra_hcdc_ecm_desc_string        = 0x03U, /**< STRING descriptor.        */
...
/**
 * @enum ra_usb_hcdc_ecm_size_t
 * @brief Standard descriptor sizes and request payload sizes.
 */
typedef enum : uint16_t {
  k_ra_hcdc_ecm_dev_desc_len     = 18U,     /**< USB DEVICE descriptor.        */
  k_ra_hcdc_ecm_cfg_desc_len     = 9U,      /**< CONFIGURATION descriptor hdr. */
  k_ra_hcdc_ecm_iface_desc_len   = 9U,      /**< INTERFACE descriptor.         */
  k_ra_hcdc_ecm_ep_desc_len      = 7U,      /**< ENDPOINT descriptor.          */
  k_ra_hcdc_ecm_string_desc_len  = 0x00FFU, /**< wLength for string fetch.     */
  k_ra_hcdc_ecm_assigned_address = 1U,      /**< First assigned device addr.   */
  k_ra_hcdc_ecm_default_config   = 1U,      /**< bConfigurationValue = 1.      */
  k_ra_hcdc_ecm_data_alt         = 1U,      /**< Data IF alt setting = 1.      */
  k_ra_hcdc_ecm_lang_id_us       = 0x0409U, /**< wIndex for English (US).      */
} ra_usb_hcdc_ecm_size_t;

/**
 * @enum ra_usb_hcdc_ecm_byte_shift_t
 * @brief Per-byte shift constants for descriptor / wValue serialisation.
 */
typedef enum : uint8_t {
  k_ra_hcdc_ecm_shift_byte0  = 0U,
  k_ra_hcdc_ecm_shift_byte1  = 8U,
  k_ra_hcdc_ecm_shift_nibble = 4U,
  k_ra_hcdc_ecm_hex_alpha    = 0x0AU, /**< Offset for 'A'/'a' in hex digit. */
} ra_usb_hcdc_ecm_byte_shift_t;

/**
 * @enum ra_usb_hcdc_ecm_byte_mask_t
 * @brief Single-byte mask used during serialisation.
 */
typedef enum : uint32_t {
  k_ra_hcdc_ecm_byte_mask = 0xFFU,
} ra_usb_hcdc_ecm_byte_mask_t;
...
/**
 * @struct ra_usb_hcdc_ecm_state_t
...
typedef struct {
  ...
  ra_usb_hcdc_ecm_attach_fn_t attach_cb;   /**< Attach callback, or NULL. */
  void*                       attach_ctx;  /**< Attach callback ctx.      */
  ra_usb_hcdc_ecm_device_t    device;      /**< Snapshot of attached dev. */
  ra_usb_hcdc_ecm_link_t      link;        /**< Cached link state.        */
} ra_usb_hcdc_ecm_state_t;

static ra_usb_hcdc_ecm_state_t s_state = {};
...
{
  return (speed == k_ra_usb_speed_hs) ? k_ra_hcdc_ecm_bulk_max_packet_hs
                                      : k_ra_hcdc_ecm_bulk_max_packet_fs;
}
                                              ...
                                              uint16_t length)
{
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_hcdc_ecm_bm_std_dev_in,
    .b_request       = k_ra_hcdc_ecm_breq_get_descriptor,
    .w_value =
      (uint16_t)(((uint16_t)desc_type << k_ra_hcdc_ecm_shift_byte1) | (uint16_t)desc_index),
    .w_index  = lang_id,
    .w_length = length,
  };
...
{
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_hcdc_ecm_bm_std_dev_out,
    .b_request       = k_ra_hcdc_ecm_breq_set_address,
...
static ra_err_t internal_setup_set_config(uint8_t config_value)
{
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_hcdc_ecm_bm_std_dev_out,
    .b_request       = k_ra_hcdc_ecm_breq_set_config,
...
{
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_hcdc_ecm_bm_std_iface_out,
    .b_request       = k_ra_hcdc_ecm_breq_set_interface,
    .w_value         = k_ra_hcdc_ecm_data_alt,
    .w_index         = k_ra_hcdc_ecm_data_alt,
    .w_length        = 0U,
  };
  return ra_usb_host_setup_request(s_state.speed, &setup);
}
...
{
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_hcdc_ecm_bm_class_iface_out,
    .b_request       = k_ra_hcdc_ecm_req_set_packet_filter,
    .w_value         = filter_mask,
    .w_index         = k_ra_hcdc_ecm_data_alt, /* Communications IF index. */
    .w_length        = 0U,
  };
  return ra_usb_host_setup_request(s_state.speed, &setup);
}
...
static ra_err_t internal_hex_nibble(char c, uint8_t* out_nibble)
{
  ...
  if ((c >= 'a') && (c <= 'f')) {
    *out_nibble = (uint8_t)(k_ra_hcdc_ecm_hex_alpha + (c - 'a'));
    return k_ra_ok;
  }
  if ((c >= 'A') && (c <= 'F')) {
    *out_nibble = (uint8_t)(k_ra_hcdc_ecm_hex_alpha + (c - 'A'));
...
{
  s_state.device.device_address      = k_ra_hcdc_ecm_assigned_address;
...
{
  s_state.step = k_ra_hcdc_ecm_step_bus_reset;
...
{
  s_state.step = k_ra_hcdc_ecm_step_get_cfg_desc;
  return internal_setup_get_descriptor(k_ra_hcdc_ecm_desc_configuration,
                                       0U,
                                       0U,
                                       k_ra_hcdc_ecm_cfg_desc_len);
}
...
{
  s_state.step = k_ra_hcdc_ecm_step_set_config;
  return internal_setup_set_config(k_ra_hcdc_ecm_default_config);
}
...
static ra_err_t internal_do_set_config(void)
{
  ...
  s_state.step = k_ra_hcdc_ecm_step_walk_desc;
  return k_ra_ok;
}
...
{
  s_state.step = k_ra_hcdc_ecm_step_get_string_mac;
  return internal_setup_get_descriptor(k_ra_hcdc_ecm_desc_string,
                                       s_state.device.i_mac_address,
                                       k_ra_hcdc_ecm_lang_id_us,
                                       k_ra_hcdc_ecm_string_desc_len);
}
...
{
  s_state.step = k_ra_hcdc_ecm_step_set_filter;
  return internal_setup_set_packet_filter(k_ra_hcdc_ecm_filter_all);
}
...
{
  s_state.step = k_ra_hcdc_ecm_step_set_interface;
...
{
  switch (s_state.step) {
    case k_ra_hcdc_ecm_step_idle:
      return internal_do_idle();
    case k_ra_hcdc_ecm_step_bus_reset:
      return internal_do_bus_reset();
    case k_ra_hcdc_ecm_step_set_address:
      return internal_do_set_address();
    case k_ra_hcdc_ecm_step_get_dev_desc:
      return internal_do_get_dev_desc();
    case k_ra_hcdc_ecm_step_get_cfg_desc:
      return internal_do_get_cfg_desc();
    case k_ra_hcdc_ecm_step_set_config:
      return internal_do_set_config();
    case k_ra_hcdc_ecm_step_walk_desc:
      return internal_do_walk_desc();
    case k_ra_hcdc_ecm_step_get_string_mac:
      return internal_do_get_string_mac();
    case k_ra_hcdc_ecm_step_set_filter:
      return internal_do_set_filter();
    case k_ra_hcdc_ecm_step_set_interface:
...
ra_err_t ra_usb_hcdc_ecm_init(ra_usb_speed_t speed)
{
  ...
  s_state.speed       = speed;
  s_state.step        = k_ra_hcdc_ecm_step_idle;
...
ra_err_t ra_usb_hcdc_ecm_send_frame(const uint8_t* buf, uint16_t len)
{
  ...
  if (len > k_ra_hcdc_ecm_max_ethernet_frame) {
    return k_ra_err_invalid_arg;
  }
...
ra_err_t ra_usb_hcdc_ecm_recv_frame(uint8_t* buf, uint16_t max_len, uint16_t* got_len)
{
  ...
  const ra_err_t err = ra_usb_queue_out(s_state.speed, k_ra_hcdc_ecm_pipe_bulk_in, buf, &inout_len);
  if (err == k_ra_ok) {
    *got_len = inout_len;
  } else {
    *got_len = 0U;
...
ra_err_t ra_usb_hcdc_ecm_set_packet_filter(uint16_t filter_mask)
{
  ...
  if ((filter_mask & (uint16_t)~k_ra_hcdc_ecm_filter_all) != 0U) {
    return k_ra_err_invalid_arg;
  }
...
ra_err_t ra_usb_hcdc_ecm_get_link_status(ra_usb_hcdc_ecm_link_t* out_link)
{
  ...
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_hcdc_ecm_bm_class_iface_in,
    .b_request       = k_ra_hcdc_ecm_req_get_statistic,
    .w_value         = 0U,
    .w_index         = k_ra_hcdc_ecm_data_alt,
    .w_length        = 1U,
  };
  const ra_err_t err = ra_usb_host_setup_request(s_state.speed, &setup);
  if (err == k_ra_ok) {
...
ra_err_t ra_usb_hcdc_ecm_parse_mac(const char* chars, uint8_t* out_mac)
{
  ...
  RA_CHECK_NULL_PTR(out_mac, s_tag, "parse_mac: out_mac");
  ...
  for (size_t i = 0U; i < (size_t)k_ra_hcdc_ecm_mac_bytes; ++i) {
    uint8_t        hi     = 0U;
    uint8_t        lo     = 0U;
    const ra_err_t hi_err = internal_hex_nibble(chars[i * 2U], &hi);
    if (hi_err != k_ra_ok) {
      return hi_err;
    }
    const ra_err_t lo_err = internal_hex_nibble(chars[(i * 2U) + 1U], &lo);
    if (lo_err != k_ra_ok) {
      return lo_err;
    }
    out_mac[i] = (uint8_t)((hi << k_ra_hcdc_ecm_shift_nibble) | lo);
  }
  return k_ra_ok;
}
