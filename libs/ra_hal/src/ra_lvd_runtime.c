 ...
 *
 * @details
 * Companion translation unit to `ra_lvd.c`. Holds the single-bit CR0
 ...
 * calls live in `ra_lvd_internal.h`; every register access keeps its
...
{
  uint8_t        idx     = 0U;
  const ra_err_t map_err = ra_lvd_internal_channel_to_idx(channel, &idx);
...
ra_err_t ra_lvd_disable_irq(ra_lvd_channel_t channel)
{
  ...
  ra_lvd_internal_cr0_rmw(&map, k_ra_lvd_cr0_mask_rie, 0U);
  return k_ra_ok;
}
...
ra_err_t ra_lvd_disable_reset(ra_lvd_channel_t channel)
{
  ...
  ra_lvd_internal_cr0_rmw(&map, enable_bit, 0U);
  return k_ra_ok;
}
...
ra_err_t ra_lvd_disable_cmpe(ra_lvd_channel_t channel)
{
  ...
  const ra_lvd_channel_map_t map = s_lvd_map[idx];
  /* HUM Ch 8.2.4 "PVDmCR0" p 305 */
  ra_lvd_internal_cr0_rmw(&map, k_ra_lvd_cr0_mask_cmpe, 0U);
  return k_ra_ok;
}
...
ra_err_t ra_lvd_set_filter(ra_lvd_channel_t channel, ra_lvd_loco_div_t filter_div, bool filter_en)
{
  ...
  const ra_err_t div_err = ra_lvd_internal_validate_div(filter_div);
  ...
  ra_lvd_internal_cr0_rmw(&map, k_ra_lvd_cr0_mask_fsamp, fsamp_bits);

  if (filter_en) {
    /* Drop DFDIS only after FSAMP has landed. */
    ra_lvd_internal_cr0_rmw(&map, k_ra_lvd_cr0_mask_dfdis, 0U);
  }
  return k_ra_ok;
}
...
ra_err_t ra_lvd_set_hysteresis_mode(ra_lvd_channel_t channel, ra_lvd_hysteresis_t hyst)
{
  ...
  if (map.has_irq && (hyst == k_ra_lvd_hysteresis_hvd)) {
    if (ra_lvd_internal_read_ri(&map) == 0U) {
