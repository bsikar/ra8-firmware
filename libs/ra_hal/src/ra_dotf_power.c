 ...
 * ``ra_dotf.c`` purely to keep each translation unit under the per-file
 * size cap. Hosts the multi-step ``ra_dotf_open`` bring-up sequence and its
 * internal sub-steps, the symmetric ``ra_dotf_close``, the convenience
 * window helper ``ra_dotf_set_region_window``, and the module-stop
 * enter/exit pair. Every register / clock-gate access carries a HUM Ch 45
 * citation. See ``ra_dotf.h`` for the public surface.
 *
...
{
  if (!ra_dotf_internal_channel_in_range(cfg->channel)) {
...
{
  if (!ra_dotf_internal_channel_in_range(channel)) {
    return k_ra_err_invalid_arg;
  }
  if (len == 0U) {
    return k_ra_err_invalid_arg;
