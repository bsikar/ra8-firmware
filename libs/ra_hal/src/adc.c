 ...
 * ----------------------------------------------------------------------------
 * SIL3 / DO-178C Level B periodic proof that the SAR core + reference are
 * healthy (self-diagnosis modes 1/2/3, HUM Ch 53.3.11 p 3411-3414), plus the
 * close-the-loop read of the on-chip temperature sensor / internal reference
 * voltage from their ADEXDRn result registers (HUM Ch 53.2.13.2 p 3391).
 * =============================================================================
 */

/* The public ra_adc_internal_chan_t values must mirror the register-header
 * CNVCS codes so ra_adc_b_adexdr_index_for_chan() maps them correctly. */
static_assert((uint8_t)k_ra_adc_chan_temperature == (uint8_t)k_ra_adc_b_chan_temperature,
              "temperature CNVCS code drift between ra_adc.h and ra8d2_adc_b_regs.h");
static_assert((uint8_t)k_ra_adc_chan_int_ref_volt == (uint8_t)k_ra_adc_b_chan_int_ref_volt,
              "internal-Vref CNVCS code drift between ra_adc.h and ra8d2_adc_b_regs.h");

/**
 * @enum ra_adc_diag_slot_t
 ...
 */
typedef enum : uint8_t {
  k_ra_adc_diag_vchan = 23U, /**< Dedicated ADCHCR slot for diagnostic reads. */
  k_ra_adc_diag_group = 8U,  /**< Dedicated scan group for diagnostic reads.  */
} ra_adc_diag_slot_t;

/**
 * @enum ra_adc_selfdiag_expect_t
...
typedef enum : int32_t {
  k_ra_adc_selfdiag_expect_mode1 = 0,      /**< 0x0000. */
  k_ra_adc_selfdiag_expect_mode2 = -32768, /**< 0x8000. */
  k_ra_adc_selfdiag_expect_mode3 = 32767,  /**< 0x7FFF. */
} ra_adc_selfdiag_expect_t;

/**
 * @enum ra_adc_selfdiag_tol_t
 ...
 */
typedef enum : int32_t {
  k_ra_adc_selfdiag_tol_lsb = 256, /**< +/- LSB tolerance about the ideal. */
} ra_adc_selfdiag_tol_t;
 ...
 * @pre @p mode is sourced from ra_adc_selfdiag_mode_t.
 ...
 * @since 0.1.0
 */
static bool internal_diagval_for_mode(ra_adc_selfdiag_mode_t mode, uint32_t* out_diagval)
{
  switch (mode) {
    case k_ra_adc_selfdiag_mode_1:
      *out_diagval = (uint32_t)k_ra_adsgdcr_diag_mode1;
      return true;
    case k_ra_adc_selfdiag_mode_2:
      *out_diagval = (uint32_t)k_ra_adsgdcr_diag_mode2;
      return true;
    case k_ra_adc_selfdiag_mode_3:
      *out_diagval = (uint32_t)k_ra_adsgdcr_diag_mode3;
      return true;
    default:
      return false;
  }
 ...
 * @since 0.1.0
 */
static int32_t internal_selfdiag_expected(ra_adc_selfdiag_mode_t mode)
{
  switch (mode) {
    case k_ra_adc_selfdiag_mode_2:
      return (int32_t)k_ra_adc_selfdiag_expect_mode2;
    case k_ra_adc_selfdiag_mode_3:
      return (int32_t)k_ra_adc_selfdiag_expect_mode3;
    case k_ra_adc_selfdiag_mode_1:
    default:
      return (int32_t)k_ra_adc_selfdiag_expect_mode1;
  }
}
...
{
  return (diff <= (int32_t)k_ra_adc_selfdiag_tol_lsb) &&
         (diff >= -(int32_t)k_ra_adc_selfdiag_tol_lsb);
}
 ...
 * @pre @p chan is sourced from ra_adc_internal_chan_t.
 ...
 * @since 0.1.0
 */
static bool internal_is_supported_ext_chan(ra_adc_internal_chan_t chan)
{
  return (chan == k_ra_adc_chan_temperature) || (chan == k_ra_adc_chan_int_ref_volt);
}
...
{
  volatile uint32_t* reg = ra_adc_b_adchcr(vch);
  if (reg == nullptr) {
    return;
  }
  const uint32_t cnvcs =
  ...
  const uint32_t ainmd =
    differential ? (((uint32_t)1UL << (uint32_t)k_ra_adchcr_bit_ainmd) & k_ra_adchcr_mask_ainmd)
                 : 0UL;
...
{
  volatile uint32_t* reg = ra_adc_b_addopcrc(vch);
  if (reg == nullptr) {
    return;
  }
  const uint32_t adprc_field =
    ((uint32_t)adprc << (uint32_t)k_ra_addopcrc_bit_adprc) & k_ra_addopcrc_mask_adprc;
  const uint32_t sign_field =
    ((uint32_t)signsel << (uint32_t)k_ra_addopcrc_bit_signsel) & k_ra_addopcrc_mask_signsel;
  /* HUM Ch 53.2.3.4 "ADDOPCRCn : A/D Conversion Data Operation Control C Register n" p 3339 */
  const uint32_t cleared = *reg & ~(k_ra_addopcrc_mask_adprc | k_ra_addopcrc_mask_signsel);
 ...
 * @since 0.1.0
 */
static ra_err_t internal_start_and_wait(uint8_t group)
{
  volatile uint32_t* adstr = ra_adc_b_adstr(group);
  if (adstr == nullptr) {
    return k_ra_err_out_of_range;
...
ra_err_t ra_adc_self_diagnose(ra_adc_selfdiag_mode_t mode, uint16_t* out_code, bool* out_pass)
{
  ...
  *out_pass = false;
  ...
  internal_program_ext_channel((uint8_t)k_ra_adc_diag_vchan,
                               ...
                               true);
  internal_set_data_format((uint8_t)k_ra_adc_diag_vchan,
                           (uint8_t)k_ra_addopcrc_adprc_16bit,
                           (uint8_t)k_ra_addopcrc_signsel_signed);

  /* HUM Ch 53 "16-bit A/D Converter (ADC16H)" p 3308 */
  *ra_adc_b_adsger() |= (uint32_t)(1UL << (uint32_t)k_ra_adc_diag_group);
...
ra_err_t ra_adc_read_internal_channel(ra_adc_internal_chan_t chan, uint16_t* out_raw)
{
  RA_CHECK_NULL_PTR(out_raw, s_tag, "out_raw must not be nullptr");
  *out_raw = 0U;
  ...
  internal_program_ext_channel((uint8_t)k_ra_adc_diag_vchan,
                               ...
                               false);
  internal_set_data_format((uint8_t)k_ra_adc_diag_vchan,
                           (uint8_t)k_ra_addopcrc_adprc_12bit,
                           (uint8_t)k_ra_addopcrc_signsel_unsigned);

  /* HUM Ch 53 "16-bit A/D Converter (ADC16H)" p 3308 */
  *ra_adc_b_adsger() |= (uint32_t)(1UL << (uint32_t)k_ra_adc_diag_group);

  const ra_err_t run_err = internal_start_and_wait((uint8_t)k_ra_adc_diag_group);
  RA_RETURN_ON_ERROR(run_err, s_tag, "read_internal: conversion"); /* GCOVR_EXCL_BR_LINE */

  const uint8_t      idx = ra_adc_b_adexdr_index_for_chan((uint8_t)chan);
  volatile uint32_t* exd = ra_adc_b_adexdr(idx);
  if (exd == nullptr) {
    return k_ra_err_out_of_range;
  }
  /* HUM Ch 53.2.13.2 "ADEXDRn : A/D Extended Analog Data Register n" p 3391 */
  *out_raw = (uint16_t)(*exd & k_ra_adexdr_mask_data);
  return k_ra_ok;
}
