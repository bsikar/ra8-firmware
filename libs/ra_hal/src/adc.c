...
{
  return ((uint32_t)resolution << (uint32_t)k_ra_adchcr_bit_ressel0) & k_ra_adchcr_mask_ressel;
}
...
static void internal_apply_resolution_all(ra_adc_resolution_t resolution)
{
  ...
  for (uint8_t ch = 0U; ch < k_ra_adc_b_max_channels; ++ch) {
    ...
    *reg = ((*reg & ~k_ra_adchcr_mask_ressel) | field);
  }
}
...
ra_err_t ra_adc_init(void)
{
  ...
  *ra_adc_b_adclkcr()  = 0U;
  *ra_adc_b_admdr()    = k_ra_adc_admdr_default;
  *ra_adc_b_adintcr()  = 0U;
...
ra_err_t ra_adc_read_channel(uint8_t channel, uint16_t* out_raw)
{
  ...
  *chcr = (*chcr | k_ra_adchcr_mask_cven);

  for (uint32_t i = 0U; i < k_ra_adc_poll_limit; ++i) {
    if ((*chcr & k_ra_adchcr_mask_cven) == 0U) {
      *out_raw = (uint16_t)(*addr & k_ra_adc_result_mask);
      return k_ra_ok;
    }
  }
...
ra_err_t ra_adc_init_configured(const ra_adc_cfg_t* cfg)
{
  ...
  *ra_adc_b_adclkcr()  = 0U;
  ...
  uint32_t mdr = k_ra_adc_admdr_default;
  if (cfg->scan_mode) {
    mdr |= k_ra_adc_admdr_scan_bit;
  }
  if ((cfg->trigger == k_ra_adc_trig_external) || (cfg->trigger == k_ra_adc_trig_elc)) {
    mdr |= k_ra_adc_admdr_trigext_bit;
  }
  *ra_adc_b_admdr()   = mdr;
  *ra_adc_b_adintcr() = 0U;
...
ra_err_t ra_adc_get_status(uint16_t* out_mask)
{
  ...
  uint16_t       out = 0U;
  if ((mdr & k_ra_admdr_mask_adbusy) != 0U) {
...
ra_err_t ra_adc_clear_status(void)
{
  ...
  *ra_adc_b_admdr() = (*ra_adc_b_admdr() & ~k_ra_admdr_mask_adbusy);
  return k_ra_ok;
}
...
void ra_adc_dispatch_cnv_end(uint8_t channel)
{
  ...
  const uint16_t             result = (uint16_t)(*addr & k_ra_adc_result_mask);
