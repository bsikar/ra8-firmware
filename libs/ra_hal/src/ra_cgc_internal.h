 ...
 * in `libs/ra_hal/src/`:
 *
 *  - `ra_cgc.c`        -- core clock-tree bring-up (PLL1, SCKDIVCR, MRMS
 *                         wait states, SCICLK) plus runtime reconfigure and
 *                         oscillation-stop detection.
 *  - `ra_cgc_usb.c`    -- PLL2 bring-up and the USB-FS / USB-HS clock paths.
 *  - `ra_cgc_eswclk.c` -- the Ethernet-switch (ESWCLK / ESWPHYCLK) path.
 *
 ...
 * bring-up established. The shared `k_ra_cgc_quarters_per_unit` PLL-multiplier
 * scale factor is the only member of ::ra_cgc_byte_const_t referenced from
 ...
 * must not be included outside `libs/ra_hal/src/ra_cgc*.c`.
 *
 * @par Tag ring/world:
 ...
 *
 * @details
 * ::k_ra_cgc_quarters_per_unit is referenced by both the core PLL1 path
 * (`ra_cgc.c`) and the PLL2 path (`ra_cgc_usb.c`), so the whole block lives
 * in this shared seam. The two Hz-conversion members are used only by the
 * MRMS wait-state stage in `ra_cgc.c`, but they ride along so the multiplier
 ...
 *
 * @details
 * Shared between the core PLL1 bring-up (`ra_cgc.c`) and the PLL2 / USB
 * paths (`ra_cgc_usb.c`). Spins up to ::k_ra_cgc_osc_spin_limit iterations.
 *
 ...
 *
 * @details
 * Shared between the core PLL1 bring-up (`ra_cgc.c`) and the PLL2 path
 * (`ra_cgc_usb.c`). Spins up to ::k_ra_cgc_pll_spin_limit iterations.
 *
 ...
 * (HCSTP = 1) the handshake never completes. ::ra_cgc_init leaves HOCO
 ...
 * PRCR window (read-only). Defined in `ra_cgc_usb.c` and reused by
 * `ra_cgc_eswclk.c`.
 *
 * @return ::ra_err_t error code.
 ...
 * @since 0.1.0
 */
ra_err_t ra_cgc_ensure_hoco_running_for_usb_ck(void);
