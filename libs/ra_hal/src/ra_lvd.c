 ...
 *
 * @details
 * Single definition for the whole ra_lvd driver. The descriptor type,
 * its index enum, and the matching `extern` declaration live in
 * ``ra_lvd_internal.h`` so the runtime / events translation units can
 * read the same table.
 *
 ...
 * @since 0.1.0
 */
const ra_lvd_channel_map_t s_lvd_map[k_ra_lvd_map_idx_count] = {
...
/** @brief Implementation of `ra_lvd_internal_channel_to_idx()` -- switch on PVD id. */
...
/** @brief Implementation of `ra_lvd_internal_validate_div()` -- upper-bound check. */
...
static ra_err_t internal_validate_cfg(const ra_lvd_channel_map_t* map, const ra_lvd_cfg_t* cfg)
{
  ...
  err = ra_lvd_internal_validate_div(cfg->filter_div);
  if (err != k_ra_ok) {
    return err;
  }
  if (map->has_irq) {
