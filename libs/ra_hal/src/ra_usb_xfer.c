 ...
 *
 * @details
 * Device-mode transfer entry points: ``ra_usb_queue_in`` /
 * ``ra_usb_queue_out`` (pipe FIFO push / drain), the DCP control-IN data
 * stage (``ra_usb_dcp_in_data`` and its chunk / ZLP helpers), the OUT
 * pipe re-arm / park controls, the SETUP-packet drains
 * (``ra_usb_read_setup_if_valid`` / ``ra_usb_read_setup_unconditional``),
 * and ``ra_usb_control_response``. Split out of ``ra_usb.c`` so every
 * translation unit stays under the 1000-line cap; the shared register and
 * FIFO helpers it calls live in ``ra_usb.c`` (declared in
 * ``ra_usb_internal.h``). Modelled on FSP ``r_usb_preg_abs.c`` /
