/**
 * @file ra_usb_hmsc_internal.h
 * @brief Cross-TU surface shared between the host-MSC class layer
 *        (ra_usb_hmsc.c) and the polled enumeration ladder
 *        (ra_usb_hmsc_enum.c).
 *
 * @par Tag
 * [Ring 3 / HAL] {World: NS}
 ...
 * public contract lives in `ra_usb_hmsc.h`, and the shadow-state singleton
 * `s_usb_hmsc_state` is shared through `ra_hal_internal.h`.
 *
