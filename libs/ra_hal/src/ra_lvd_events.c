 ...
 *
 * @details
 * Companion translation unit to `ra_lvd.c`. Owns the PVDSAR security
 ...
 * register helpers it calls live in `ra_lvd_internal.h`; every register
...
ra_err_t ra_lvd_cancel_deep_standby_path(void)
{
  ...
  for (uint8_t i = 0U; i < k_ra_lvd_nmi_channel_count; ++i) {
    ra_lvd_internal_cr0_rmw(&s_lvd_map[i], k_ra_lvd_cr0_mask_ri, 0U);
  }
  return k_ra_ok;
}
