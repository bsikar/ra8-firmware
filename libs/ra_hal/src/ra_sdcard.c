 ...
 * @retval k_ra_ok                   Identification echo matched.
 * @retval k_ra_err_hw_init_failed   CMD8 pattern mismatch.
 *
 * @pre ra_sdhi_init() succeeded for ``instance``.
 ...
 * @since 0.1.0
 */
static ra_err_t internal_sdcard_identify(uint8_t instance)
{
  uint32_t rsp[4] = {0U, 0U, 0U, 0U};
  ...
  const ra_err_t e0 = ra_sdhi_send_command(instance, (uint32_t)k_ra_sdcard_cmd0_go_idle, 0U, rsp);
  ...
  const ra_err_t e8 = ra_sdhi_send_command(instance,
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                   Card parked in TRAN.
 * @retval k_ra_err_hw_init_failed   CSD decode rejected the layout.
 *
...
internal_sdcard_publish_and_select(uint8_t instance, uint16_t* out_rca, uint32_t* out_blocks)
{
  ...
  const ra_err_t e2 =
    ra_sdhi_send_command(instance, (uint32_t)k_ra_sdcard_cmd2_all_send_cid, 0U, rsp);
  ...
  const ra_err_t e9 =
    ra_sdhi_send_command(instance, (uint32_t)k_ra_sdcard_cmd9_send_csd, rca_arg, rsp);
  RA_RETURN_ON_ERROR(e9, s_tag, "cmd9"); /* GCOVR_EXCL_BR_LINE */

  const ra_err_t dec_err = internal_decode_csd(rsp, out_blocks);
  if (dec_err != k_ra_ok) {
    return dec_err;
  }
  ...
  const ra_err_t e7 =
    ra_sdhi_send_command(instance, (uint32_t)k_ra_sdcard_cmd7_select_card, rca_arg, rsp);
 ...
 *
 * @return ::ra_sdcard_card_type_t classification.
 * @retval k_ra_sdcard_type_sdsc  high_capacity == 0 (byte-addressed card).
 * @retval k_ra_sdcard_type_sdhc  high_capacity != 0 and blocks <= 32 GiB threshold.
 * @retval k_ra_sdcard_type_sdxc  high_capacity != 0 and blocks > 32 GiB threshold.
 *
 ...
 * @since 0.1.0
 */
static ra_sdcard_card_type_t internal_classify(uint8_t high_capacity, uint32_t blocks)
{
  if (high_capacity == 0U) {
    return k_ra_sdcard_type_sdsc;
  }
  if (blocks > (uint32_t)k_ra_sdcard_sdhc_threshold_blocks * 16U) {
    return k_ra_sdcard_type_sdxc;
  }
...
ra_err_t ra_sdcard_init(const ra_sdcard_cfg_t* cfg)
{
  ...
  const ra_err_t id_err = internal_sdcard_identify(cfg->instance);
  if (id_err != k_ra_ok) {
  ...
  uint16_t       rca     = 0U;
  uint32_t       blocks  = 0U;
  const ra_err_t pub_err = internal_sdcard_publish_and_select(cfg->instance, &rca, &blocks);
  if (pub_err != k_ra_ok) {
    (void)ra_sdhi_deinit(cfg->instance);
    return pub_err;
  }
