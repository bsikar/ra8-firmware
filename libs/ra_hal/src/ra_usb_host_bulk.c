 ...
 * detection. Provides ``ra_usb_host_set_target``,
 * ``ra_usb_host_pipe_setup``, ``ra_usb_host_bulk_out`` /
 * ``ra_usb_host_bulk_in``, and ``ra_usb_host_line_state``. Split out of
 * ``ra_usb.c`` so every translation unit stays under the 1000-line cap;
 * the shared register helpers and host addressing enums it uses are
 * declared in ``ra_usb_internal.h``. No FSP source ships in this tree.
 *
