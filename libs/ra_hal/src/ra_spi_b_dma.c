 ...
 * (``ra_spi_write_dma`` / ``ra_spi_read_dma``) that stream a byte buffer
 ...
 * L1 data cache:
 *
 *  - ``ra_spi_write_dma`` cleans (flushes) the dirty cache lines covering
 *    the caller's TX buffer before arming the engine so the DMA reads the
 *    CPU-written bytes from memory.
 *  - ``ra_spi_read_dma`` installs ::internal_spi_dma_rx_complete as the
 ...
 *
 * The per-channel register accessor ``ra_spi()`` and the channel-count
 * constant ``k_ra_spi_b_channel_count`` live in ``ra8d2_spi_regs.h``; the
 * public DMA API contract is in ``ra_spi.h``. No other ``ra_spi_b.c``
...
typedef struct {
  ...
  ra_dma_complete_fn_t user_on_complete; /**< Caller callback (may be NULL).  */
 ...
 *
 * @details Written by ::ra_spi_read_dma before arming the transfer and read by
 ...
 * @since 0.1.0
 */
static ra_spi_dma_rx_ctx_t s_spi_dma_rx_ctx[k_ra_spi_b_channel_count];
 ...
 * Cortex-M85, reported by ``ra_cache_dcache_line_bytes()``). Rounding the
 ...
 * @post The result is a whole multiple of ``ra_cache_dcache_line_bytes()``.
 ...
 *
 * @details
 * Installed as ``req.on_complete`` for every ::ra_spi_read_dma transfer. Runs
 * from the DMAC transfer-end ISR (or ``ra_sim_dma_complete`` under
 * ``RA_SIMULATOR_MODE``) once the engine has finished writing ``out_buf`` to
 ...
 *
 * @param[in] ctx Pointer to the driver's ::ra_spi_dma_rx_ctx_t for the channel.
 *
 * @pre ``ctx`` points at a ::ra_spi_dma_rx_ctx_t populated by ::ra_spi_read_dma.
...
{
  ra_spi_dma_rx_ctx_t* rxc = (ra_spi_dma_rx_ctx_t*)ctx;
  if (rxc == nullptr) { /* GCOVR_EXCL_BR_LINE */
    return;             /* GCOVR_EXCL_LINE    */
  }
  ra_cache_dcache_invalidate_by_addr(rxc->rx_buf, internal_round_up_to_cache_line(rxc->rx_len));
                          ...
                          uint8_t*             out_dma_channel)
{
   ...
   * No-op when the D-cache is disabled. */
  ra_cache_dcache_clean_by_addr(data, internal_round_up_to_cache_line(len));
                         ...
                         uint8_t*             out_dma_channel)
{
   ...
   * (ra_dma_request returns immediately; completion fires from the DMAC
   * transfer-end ISR). No-op when the D-cache is disabled. */
  ra_spi_dma_rx_ctx_t* rxc = &s_spi_dma_rx_ctx[channel];
  rxc->rx_buf              = out_buf;
  rxc->rx_len              = len;
