/**
 * @file ra_hal_internal.h
 * @brief Cross-translation-unit private helpers shared inside ra_hal.
 *
 * @par Tag
 * [Ring 3 / HAL] {World: S}
 *
 ...
 * extern declarations between sibling translation units of a single ra_hal
 ...
 * never be included from outside ``libs/ra_hal/src``.
 *
 ...
 *
 * @details
 * Shared between ``ra_dotf.c`` and ``ra_dotf_power.c``. Defined ``static
 ...
 * @since 0.1.0
 */
static inline bool ra_dotf_internal_channel_in_range(uint8_t channel)
{
  return (uint16_t)channel < (uint16_t)k_ra_dotf_channel_count;
}

/* =============================================================================
 * ra_rmac: split between ra_rmac.c (lifecycle / config / MDIO primitives /
 * status / dispatch) and ra_rmac_mgmt.c (statistic-counter snapshot + IEEE
 ...
 *
 * @details
 * Shared between ``ra_rmac.c`` and ``ra_rmac_mgmt.c``. Defined ``static
...
/* =============================================================================
 * ra_mipi_csi: split between ra_mipi_csi.c (lifecycle / register knobs) and
 * ra_mipi_csi_irq.c (interrupt path + per-VC framing). The interrupt TU owns
 * every callback function-pointer / context static, so lifecycle teardown in
 * ra_mipi_csi.c detaches them through this promoted helper rather than
 ...
 * ``ra_mipi_csi_irq.c`` back to ``nullptr``. Invoked by
 * ::ra_mipi_csi_deinit during teardown so that no stale callback can fire
 * after the receiver has been gated. The per-VC error callback is left
 * untouched, mirroring the pre-split ``ra_mipi_csi_deinit`` behaviour.
 *
 * @return Nothing.
 *
 * @pre The MIPI CSI-2 receiver is being torn down by ::ra_mipi_csi_deinit.
 ...
 * @since 0.1.0
 */
void ra_mipi_csi_detach_all_handlers(void);

/* =============================================================================
 * ra_ssie: split between ra_ssie.c (lifecycle / register knobs / polled FIFO /
 * status / IRQ) and ra_ssie_stream.c (DMA channel attach/detach + polled ISO
...
/**
 * @var s_ssie_runtime
 * @brief Per-channel SSIE runtime state, shared across the ra_ssie split.
 *
 * @details
 * Single definition lives in ``ra_ssie.c``; ``ra_ssie_stream.c`` reads and
 ...
 *
 * @note Module-private to ``libs/ra_hal/src``; not part of the public surface.
 * @warning Not thread-safe; callers must serialise concurrent access.
 * @since 0.1.0
 */
extern ra_ssie_runtime_t s_ssie_runtime[k_ra_ssie_channel_count];

/**
 * @brief Validate an SSIE channel index and return its register block.
 *
 * @details
 * Shared between ``ra_ssie.c`` and ``ra_ssie_stream.c``. Promoted from a
 ...
 * ``nullptr`` for an out-of-range channel exactly like ``ra_ssie()``.
 *
 ...
 * @since 0.1.0
 */
volatile r_ssie_regs_t* ra_ssie_internal_regs(uint8_t channel);

/* =============================================================================
 * ra_usb_hmsc: split between ra_usb_hmsc.c (CBW/CSW helpers, lifecycle, attach
 * callback, BOT/SCSI command path) and ra_usb_hmsc_enum.c (the polled
 ...
 * declaration live here, the one definition lives in ra_usb_hmsc.c.
 * =============================================================================
 */
 ...
 *
 * @details
 * Shared across the ra_usb_hmsc split. The lifecycle / command-path TU
 * (``ra_usb_hmsc.c``) and the enumeration-ladder TU (``ra_usb_hmsc_enum.c``)
 ...
 * @brief Singleton host-MSC shadow state, shared across the ra_usb_hmsc split.
 *
 * @details
 * Single definition lives in ``ra_usb_hmsc.c``; ``ra_usb_hmsc_enum.c`` reads
 ...
 * @since 0.1.0
 */
extern ra_usb_hmsc_state_t s_usb_hmsc_state;

/* =============================================================================
 * ra_canfd: split between ra_canfd.c (clock / MSTP / global+channel mode state
 * machine, AFL / RX-FIFO bring-up, lifecycle, status / IRQ / filter / test-mode
 * / power) and two siblings: ra_canfd_frame.c (TX/RX frame data path) and
 * ra_canfd_timing.c (bit-timing solver + bitrate / BRS configuration). The
 ...
 * ra_canfd.c next to the rest of the mode machinery -- is promoted to
 ...
 *
 * @details
 * Promoted from a file-scope ``static`` helper in ``ra_canfd.c`` to TU-external
 * linkage so the bit-timing TU (``ra_canfd_timing.c``) can transition the
 ...
 * stays in ``ra_canfd.c`` beside the rest of the mode-transition machinery
 ...
 * @retval k_ra_ok            The channel latched the requested mode.
 * @retval k_ra_err_hw_timeout The status bit never latched within the budget.
 *
