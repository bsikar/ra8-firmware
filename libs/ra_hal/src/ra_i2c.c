...
/**
 * @var s_i2c_tag
 * @brief Log tag for this driver, shared with ``ra_i2c_config.c``.
 *
 * @details
 * Owning definition for the ``s_i2c_tag`` symbol declared ``extern`` in
 * ``ra_i2c_internal.h``; both I2C translation units log under "I2C".
 *
 ...
 * ``ra_i2c_internal.h`` so the bring-up plane in ``ra_i2c_config.c`` and
 ...
 * @since 0.1.0
 */
ra_i2c_state_t s_i2c_state[k_ra_i2c_channel_count];
                      ...
                      bool           send_stop)
{
  ...
  RA_CHECK_NULL_PTR(reg, s_i2c_tag, "i2c_write: channel");
  RA_CHECK_NULL_PTR(data, s_i2c_tag, "i2c_write: data");
...
{
  volatile r_i2c_regs_t* reg = ra_i2c_regs(channel);
  RA_CHECK_NULL_PTR(reg, s_i2c_tag, "i2c_read: channel");
  RA_CHECK_NULL_PTR(data, s_i2c_tag, "i2c_read: data");
  if (len == 0U) {
...
{
  volatile r_i2c_regs_t* reg = ra_i2c_regs(channel);
  RA_CHECK_NULL_PTR(reg, s_i2c_tag, "i2c_scan: channel");
  RA_CHECK_NULL_PTR(out_acked, s_i2c_tag, "i2c_scan: out_acked");
