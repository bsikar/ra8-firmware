 ...
 * split out of ra_eth_gwca.c to stay under the per-file line-count
...
static ra_err_t internal_drain_rx_slot(ra_gwca_basic_descriptor_t* desc,
                                       ...
                                       uint32_t*                   out_len)
{
  const uint8_t* const buf      = ra_eth_gwca_decode_ptr(desc);
                                             ...
                                             uint32_t*                   cursor)
{
  ...
  ra_eth_gwca_set_linkfix_entry(term, &chain[0]);
  term->dt = (uint8_t)k_ra_gwdcc_dt_link;
  *cursor  = 0U;
  (void)ra_eth_gwca_reload_queue(queue_index);
}
