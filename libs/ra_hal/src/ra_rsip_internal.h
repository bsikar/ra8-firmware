/**
 * @file ra_rsip_internal.h
 * @brief Cross-TU surface for the ra_rsip driver split.
 *
 * @par Tag
 * [Ring 3 / HAL] {World: S}
 *
 ...
 *   declared in ``ra_rsip_asym_internal.h``).
 *
 ...
 * iteration cap (``k_ra_rsip_poll_budget``). Defined in ``ra_rsip.c``;
 * shared with the mailbox-completion driver in ``ra_rsip_cipher.c``.
 *
 ...
 * terminates. Defined in ``ra_rsip.c``; shared with the generic hash /
 * HMAC path in ``ra_rsip_cipher.c``.
 *
 * @return ``k_ra_ok`` on success; ``k_ra_err_hw_timeout`` otherwise.
 *
 ...
 * ``ra_rsip_cipher.c``; shared with ``ra_rsip.c`` and ``ra_rsip_asym.c``.
 *
 ...
 * word-addressed result registers. Defined in ``ra_rsip_cipher.c``;
 * shared with ``ra_rsip.c`` and ``ra_rsip_asym.c``.
 *
 ...
 * Defined in ``ra_rsip_cipher.c``; shared with ``ra_rsip.c`` and
 * ``ra_rsip_asym.c``.
 *
 ...
 * W1C-acks the completion bit. Defined in ``ra_rsip_cipher.c``; shared
 * with the asymmetric / key-management entry points in ``ra_rsip_asym.c``.
 *
 * @param[in] done_mask Completion bit mask to poll and acknowledge.
 *
 * @return ``k_ra_ok`` on success; an error otherwise.
 * @retval k_ra_ok Operation completed successfully.
 * @retval k_ra_err_hw_timeout Completion bit never observed.
 * @retval k_ra_err_hw_error ``MBOX_RET`` was non-zero.
 *
 * @pre A mailbox command has been issued.
 * @pre ``done_mask`` is non-zero.
 *
 * @post On ``k_ra_ok``, ``done_mask`` has been acknowledged.
 ...
 * ``ra_rsip_cipher.c``; shared with the unwrap path in ``ra_rsip_asym.c``.
 *
 ...
 *
 * @pre ``cmd`` is one of ``ra_rsip_oem_cmd_t``.
 ...
 *
 * @details
 * Defined in ``ra_rsip_cipher.c``; shared with the key wrap / unwrap
 * entry points in ``ra_rsip_asym.c`` (KEK validation).
 *
 * @param[in] alg Wrapped-key install opcode (``ra_rsip_oem_cmd_t`` value).
 *
 * @return AES algorithm selector byte, or 0 for a non-AES key.
 * @retval 0 Key is not an AES key.
 *
 * @pre ``alg`` is a ``ra_rsip_oem_cmd_t`` value.
 ...
 * ``ra_rsip_cipher.c``; shared with the wrap engine in ``ra_rsip_asym.c``.
 *
 ...
 * port. Defined in ``ra_rsip_cipher.c``; shared with the wrap and KDF
 * paths in ``ra_rsip_asym.c``.
 *
 ...
 * Defined in ``ra_rsip_cipher.c``; shared with every key-touching
 * asymmetric entry point in ``ra_rsip_asym.c``.
 *
