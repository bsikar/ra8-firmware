 ...
 *
 *  - ``ra_spi_b_target_init`` mirrors ``ra_spi_init`` from
 *    ``ra_spi_b.c`` but omits SPBR (irrelevant: clock is external) and
 ...
 *
 *  - ``ra_spi_b_target_xfer`` performs a polled full-duplex single-byte
 ...
 *
 * @details Matches the table in ``ra_spi_b.c``; this TU manages its own
...
/**
 * @enum ra_spi_b_target_poll_t
 * @brief Polling-loop budget for SPSR flag waits in target mode.
 *
 * @details Matches ``k_ra_spi_b_poll_limit`` in ``ra_spi_b.c`` so that
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              Flag asserted within the poll budget.
 * @retval k_ra_err_hw_timeout  Flag did not assert within the budget.
 *
 * @pre ``reg`` is a valid, mapped SPI_B register block pointer.
 * @pre ``flag_mask`` is one of the single-bit ``k_ra_spsr_mask_*`` constants.
 * @post On ``k_ra_ok`` the indicated SPSR flag is still asserted (caller
 *       must clear via SPSRC).
 * @post On ``k_ra_err_hw_timeout`` no SPSR flag has been cleared.
 *
...
static uint32_t internal_target_spcr(void)
{
  ...
  v |= (uint32_t)k_ra_spcr_mask_spe;    /* SPI Function Enable.        */
  v |= (uint32_t)k_ra_spcr_mask_modfen; /* Mode Fault Error Detection. */
  return v;
}
 ...
 * from the public ``ra_spi_cfg_t``. The bit-rate divider and SSL-select
 ...
 * @post Returned SPB[4:0] encodes an 8-bit frame (k_ra_spcmd_spb_8bit).
 *
 * @note Pure; no side effects; thread-safe.
 * @since 0.1.0
 */
static uint32_t internal_spcmd_target(const ra_spi_cfg_t* cfg)
{
  uint32_t v = 0U;
  switch (cfg->mode) {
    case k_ra_spi_mode_1:
      v |= (uint32_t)k_ra_spcmd_mask_cpha;
      break;
    case k_ra_spi_mode_2:
      v |= (uint32_t)k_ra_spcmd_mask_cpol;
      break;
    case k_ra_spi_mode_3:
      v |= (uint32_t)k_ra_spcmd_mask_cpha;
      v |= (uint32_t)k_ra_spcmd_mask_cpol;
      break;
    case k_ra_spi_mode_0:
    default:
      break;
  }
  if (cfg->lsb_first) {
    v |= (uint32_t)k_ra_spcmd_mask_lsbf;
  }
  /* 8-bit frame. */
 ...
 *
 * @details
 * Called from ``ra_spi_b_target_init`` after SPCR has been cleared to
 ...
 * ``ra_spi_b_target_xfer`` sees a clean status.
 *
 ...
 * @since 0.1.0
 */
static void internal_target_program_regs(volatile r_spi_regs_t* reg, const ra_spi_cfg_t* cfg)
{
  /* HUM Ch 43.2.13 "SPSRC : SPI Status Clear Register" p 2905 */
  reg->SPSRC = (uint32_t)k_ra_spsrc_mask_all;
  ...
  reg->SPFCR = (uint32_t)k_ra_spfcr_mask_spfrst;
...
/* =============================================================================
 * Public API -- definitions (declarations + Doxygen live in ra_spi.h)
 * =============================================================================
 */

ra_err_t ra_spi_b_target_init(uint8_t channel, const ra_spi_cfg_t* cfg)
{
  RA_CHECK_NULL_PTR(cfg, s_tag, "target_init: cfg");
  if (channel >= (uint8_t)k_ra_spi_b_channel_count) {
  ...
  RA_RETURN_ON_ERROR(mst_err, s_tag, "target_init: mstp"); /* GCOVR_EXCL_BR_LINE */
...
ra_err_t ra_spi_b_target_xfer(uint8_t channel, uint8_t tx, uint8_t* rx)
{
  volatile r_spi_regs_t* reg = ra_spi(channel);
  RA_CHECK_NULL_PTR(reg, s_tag, "target_xfer: channel out of range");

  /* Wait until the TX buffer is empty (safe to pre-load response). */
  /* HUM Ch 43.2.9 "SPSR : SPI Status Register" p 2898 */
  ra_err_t err = internal_target_wait_spsr(reg, (uint32_t)k_ra_spsr_mask_sptef);
  if (err != k_ra_ok) {
    return err;
  }
  ...
  /* HUM Ch 43.2.13 "SPSRC : SPI Status Clear Register" p 2905 */
  reg->SPSRC = (uint32_t)k_ra_spsrc_mask_sptefc;

  /* Wait for the external controller to complete a full frame (SPRF). */
  /* HUM Ch 43.2.9 "SPSR : SPI Status Register" p 2898 */
  err = internal_target_wait_spsr(reg, (uint32_t)k_ra_spsr_mask_sprf);
  if (err != k_ra_ok) {
    return err;
  }
  ...
  /* Clear RX-full flag. */
  /* HUM Ch 43.2.13 "SPSRC : SPI Status Clear Register" p 2905 */
  reg->SPSRC = (uint32_t)k_ra_spsrc_mask_sprfc;
