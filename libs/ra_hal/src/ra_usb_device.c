 ...
 * the shared register helpers it calls live in ``ra_usb.c`` and the PHY /
 * common bring-up it dispatches to lives in ``ra_usb_phy.c`` -- both are
 * declared in ``ra_usb_internal.h``. Modelled on FSP ``r_usb_pdriver.c``
