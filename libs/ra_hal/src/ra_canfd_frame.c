/**
 * @file ra_canfd_frame.c
 * @brief CANFD frame transmit / receive data path (split from ra_canfd.c)
 *
 * @par Tag
 * [Ring 3 / HAL] {World: NS}
 *
 ...
 *
 *  - Range-check a ``ra_canfd_frame_t`` against the protocol limits,
 ...
 *  - The host-only ``ra_canfd_test_inject_frame`` seam used by the
...
/**
 * @enum ra_canfd_frame_spin_t
 * @brief TX-completion poll budget owned by the frame data path.
 *
 * @details
 * Private copy of the matching constant in ``ra_canfd.c``; this TU only
 * needs the TX-completion budget. Bounds the back-to-back TMTRF poll in
 * ::ra_canfd_transmit. ``k_ra_canfd_tx_spin`` is a read-only compile-time
