 ...
 * ``ra_rsip_asym.c`` (issue #216) both to keep every translation unit under
 * the file-size budget and because this cluster is FAIL-CLOSED for a reason
 * distinct from the hash / key fiction in ``ra_rsip_asym.c``.
 *
 ...
 * production build: it returns ``k_ra_err_not_supported`` and writes no
 ...
 * so ``ra_rsip_devsec.c`` is enforced by ``check_stub_crypto_guarded.py``; a
 ...
 * returns a hard error (never k_ra_ok) and writes no fabricated state, so a
...
ra_err_t ra_rsip_life_get(ra_rsip_life_state_t* out)
{
  RA_CHECK_NULL_PTR(out, s_tag, "life_get: out must not be nullptr");
...
ra_err_t ra_rsip_debug_level_get(ra_rsip_debug_level_t* out)
{
  RA_CHECK_NULL_PTR(out, s_tag, "debug_level_get: out must not be nullptr");
  return k_ra_err_not_supported;
}
...
ra_err_t ra_rsip_tamper_status(uint32_t* out)
{
  RA_CHECK_NULL_PTR(out, s_tag, "tamper_status: out must not be nullptr");
  return k_ra_err_not_supported;
}
