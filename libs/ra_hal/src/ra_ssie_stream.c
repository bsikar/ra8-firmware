 ...
 *
 * @details
 * Sibling translation unit of ``ra_ssie.c`` carrying the bulk-transfer half of
 * the Serial Sound Interface Enhanced (SSIE / I2S) driver: the DMAC adapter
 * (``ra_ssie_attach_dma`` / ``ra_ssie_detach_dma`` / ``ra_ssie_attach_dma_pair``
 * and their private helpers) and the polled isochronous streaming primitives
 * (``ra_ssie_send_iso`` / ``ra_ssie_recv_iso``). Split out of ``ra_ssie.c``
 ...
 * (``ra_ssie_internal_regs``) are shared with ``ra_ssie.c`` through
 * ``ra_hal_internal.h``.
 *
 ...
 *       identical literal in ``ra_ssie.c`` so the split needs no promoted
...
ra_err_t ra_ssie_attach_dma(uint8_t channel, const ra_ssie_dma_cfg_t* dma)
{
  ...
  volatile r_ssie_regs_t* reg = ra_ssie_internal_regs(channel);
  if (reg == nullptr) {
    return k_ra_err_invalid_arg;
  }
