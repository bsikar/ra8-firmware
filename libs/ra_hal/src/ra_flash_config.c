 ...
 *
 * @details
 * Configuration / data-programming aspect of the ra_flash driver, split
 * out of ``ra_flash.c`` so every translation unit stays under the
 * file-size cap. Implements the slice of the HUM Ch 7 + Ch 59 surface
 * declared in ``ra_flash.h``:
 *
 ...
 * ``ra_flash_internal.h``. Every register access carries a HUM Ch 7 or
 * Ch 59 citation.
 *
...
{
  RA_CHECK_NULL_PTR(out_btflg, s_flash_tag, "out_btflg must not be nullptr");
  RA_CHECK_NULL_PTR(out_fspr, s_flash_tag, "out_fspr must not be nullptr");
...
static ra_err_t internal_arc_cmd(uint8_t mcntselr, uint8_t cmd)
{
  ...
  *ra_mram_reg8(k_ra_mram_off_mcntselr) = (uint8_t)(mcntselr & k_ra_mcntselr_mask);
  ra_flash_internal_maci_cmd8(cmd);
  ra_flash_internal_maci_cmd8(k_ra_maci_cmd_final);
  ra_err_t err = ra_flash_internal_wait_mrdy(k_ra_flash_maci_spin_limit);
  if (err != k_ra_ok) {
    return err;
...
ra_err_t ra_flash_arc_increment(ra_flash_arc_id_t counter)
{
  ...
  RA_VALIDATE_INIT(s_flash_rt.initialized, s_flash_tag, "arc_inc before init");
...
{
  RA_CHECK_NULL_PTR(out_count, s_flash_tag, "out_count must not be nullptr");
  if (counter >= k_ra_flash_arc_count) {
    return k_ra_err_invalid_arg;
  }
  RA_VALIDATE_INIT(s_flash_rt.initialized, s_flash_tag, "arc_read before init");
...
{
  RA_VALIDATE_INIT(s_flash_rt.initialized, s_flash_tag, "zeroize before init");
...
{
  RA_CHECK_NULL_PTR(out_code_ted, s_flash_tag, "out_code_ted null");
  RA_CHECK_NULL_PTR(out_code_dec, s_flash_tag, "out_code_dec null");
  RA_CHECK_NULL_PTR(out_extra_ted, s_flash_tag, "out_extra_ted null");
  RA_CHECK_NULL_PTR(out_extra_dec, s_flash_tag, "out_extra_dec null");
...
{
  RA_CHECK_NULL_PTR(out_addr, s_flash_tag, "out_addr must not be nullptr");
...
ra_err_t ra_flash_update_clock_freq(uint16_t mrcfreq_mhz, uint8_t mrefreq_mhz)
{
  ...
  *ra_mram_reg32(k_ra_mram_off_mrefreq) =
  ...
  ra_flash_internal_set_prefetch(prefetch_was);
  return k_ra_ok;
}
...
{
  RA_CHECK_NULL_PTR(out_busy, s_flash_tag, "out_busy null");
  RA_CHECK_NULL_PTR(out_done, s_flash_tag, "out_done null");
  RA_CHECK_NULL_PTR(out_err, s_flash_tag, "out_err null");
