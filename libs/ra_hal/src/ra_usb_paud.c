 ...
 *
 * @details
 * Glues the device-mode `ra_usb` driver to a USB Audio Class 1.0
 ...
 * Mapping vs FSP:
 *
 *  - `usb_paud_class_request_init` -> `ra_usb_paud_init`
 *  - `usb_paud_write_complete`     -> `ra_usb_paud_send_frame` queue
 *  - `usb_paud_read_complete`      -> `ra_usb_paud_recv_frame` drain
 *  - `usb_paud_setup`              -> `ra_usb_paud_handle_setup`
 *
...
/**
 * @enum ra_usb_paud_setup_field_t
...
{
  return (speed == k_ra_usb_speed_hs) ? k_ra_paud_iso_max_packet_hs
                                      : k_ra_paud_iso_max_packet_fs_default;
}
...
static ra_err_t internal_configure_pipes(ra_usb_speed_t speed)
{
  ...
  ra_err_t err = ra_usb_configure_endpoint(speed,
                                           k_ra_paud_pipe_iso_in,
                                           k_ra_paud_ep_iso_in_addr,
                                           k_ra_usb_ep_dir_in,
...
static void internal_reset_shadow(ra_usb_speed_t speed)
{
  ...
  s_state.desc_len                = 0U;
  s_state.format.sample_rate_hz   = (uint32_t)k_ra_paud_default_rate_hz;
  s_state.format.channels         = (uint8_t)k_ra_paud_default_channels;
  s_state.format.bytes_per_sample = (uint8_t)k_ra_paud_default_bps;
  s_state.volume_q8_8             = (int16_t)k_ra_paud_default_volume;
...
{
  return (b_request == k_ra_paud_req_set_cur) || (b_request == k_ra_paud_req_get_cur) ||
         (b_request == k_ra_paud_req_set_min) || (b_request == k_ra_paud_req_get_min) ||
         (b_request == k_ra_paud_req_set_max) || (b_request == k_ra_paud_req_get_max) ||
         (b_request == k_ra_paud_req_set_res) || (b_request == k_ra_paud_req_get_res) ||
         (b_request == k_ra_paud_req_get_stat);
}
...
{
  return (bm == k_ra_paud_bm_class_iface_in) || (bm == k_ra_paud_bm_class_iface_out) ||
         (bm == k_ra_paud_bm_class_ep_in) || (bm == k_ra_paud_bm_class_ep_out);
}
...
ra_err_t ra_usb_paud_recv_frame(uint8_t* buf, uint16_t max_len, uint16_t* got_len)
{
  ...
  const ra_err_t err = ra_usb_queue_out(s_state.speed, k_ra_paud_pipe_iso_out, buf, &inout_len);
  if (err == k_ra_ok) {
    *got_len = inout_len;
  } else {
    *got_len = 0U;
...
ra_err_t ra_usb_paud_set_format(ra_usb_paud_format_t format)
{
  ...
  if ((format.channels < (uint8_t)k_ra_paud_min_channels) ||
      (format.channels > (uint8_t)k_ra_paud_max_channels)) {
    return k_ra_err_invalid_arg;
  }
  if ((format.bytes_per_sample < (uint8_t)k_ra_paud_min_bps) ||
      (format.bytes_per_sample > (uint8_t)k_ra_paud_max_bps)) {
    return k_ra_err_invalid_arg;
  }
