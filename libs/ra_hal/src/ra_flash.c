 ...
 * ``ra_flash_internal.h``; the configuration and IRQ TUs reference it
...
{
  RA_CHECK_NULL_PTR(cfg, s_flash_tag, "cfg must not be nullptr");
  ...
  ra_flash_internal_set_prefetch(false);
  ...
  *ra_mram_reg16(k_ra_mram_off_mrcdecc) = (uint16_t)(k_ra_mrcdecc_key_shift | mrcdecc_bits);
  ...
  ra_flash_internal_set_prefetch(cfg->prefetch_en);
  ...
  *ra_mram_reg8(k_ra_mram_off_mreraes) = 0U;
  ...
  ra_log_info_val(s_flash_tag, "flash_init mrcfreq", (uint32_t)cfg->mrcfreq_mhz);
  return k_ra_ok;
}
...
ra_err_t ra_flash_deinit(void)
{
  ...
  *ra_mram_reg16(k_ra_mram_off_mentryr) = k_ra_mentryr_read_mode;
  /* HUM Ch 59 "MRCPS : Code MRAM Program Status Register" p 3601 */
  *ra_mram_reg8(k_ra_mram_off_mrcps) = k_ra_mrcps_mask_errors;
  ra_flash_internal_set_prefetch(true);
...
{
  RA_CHECK_NULL_PTR(out_status, s_flash_tag, "out_status must not be nullptr");
  /* HUM Ch 59 "MRCPS : Code MRAM Program Status Register" p 3601 */
...
{
  RA_CHECK_NULL_PTR(out, s_flash_tag, "out must not be nullptr");
  /* HUM Ch 59 "MRCPS : Code MRAM Program Status Register" p 3601 */
...
ra_err_t ra_flash_set_rww_disable(bool disable)
{
  ...
  ra_flash_internal_set_prefetch(prefetch);
  return k_ra_ok;
}
 ...
 *          wrapper @c ra_flash_internal_window_allows simply forwards.
 *
...
{
  return ra_flash_internal_window_allows_pure(addr, len, s_flash_rt.win_low, s_flash_rt.win_high);
}
...
static ra_err_t internal_validate_write_block(uint32_t mram_addr, uint32_t len)
{
  ...
  if (!ra_flash_internal_window_allows((uintptr_t)mram_addr, len)) {
...
{
  RA_CHECK_NULL_PTR(src, s_flash_tag, "src must not be nullptr");
  const ra_err_t v_err = internal_validate_write_block(mram_addr, len);
  RA_RETURN_ON_ERROR(v_err, s_flash_tag, "flash_write: validate"); /* GCOVR_EXCL_BR_LINE */

  /* Step 1: wait for the controller to be idle. */
  ra_err_t err = internal_wait_buffer_ready(k_ra_flash_busy_spin_limit);
  RA_RETURN_ON_ERROR(err, s_flash_tag, "flash_write: busy wait"); /* GCOVR_EXCL_BR_LINE */

  /* Steps 2-6: gate, write, flush, commit, teardown. */
  err = internal_flash_program_window(mram_addr, src, len, world);
  RA_RETURN_ON_ERROR(err, s_flash_tag, "flash_write: commit wait"); /* GCOVR_EXCL_BR_LINE */
  ...
  if ((status & k_ra_mrcps_mask_errors) != 0U) {
    *ra_mram_reg8(k_ra_mram_off_mrcps) = k_ra_mrcps_mask_errors;
    ra_log_error_val(s_flash_tag, "flash_write: hw error", (uint32_t)status);
...
ra_err_t ra_flash_block_protect_set(ra_flash_world_t world, bool lock, bool permanent)
{
  ...
  if (permanent) {
    ra_log_warn(s_flash_tag, "permanent block protect requires config-set");
  }
  return k_ra_ok;
}
...
ra_err_t ra_flash_exit_pe_mode(void)
{
  ...
  *ra_mram_reg16(k_ra_mram_off_mentryr) = k_ra_mentryr_read_mode;
  ...
  for (uint32_t i = 0U; i < k_ra_flash_pe_spin_limit; ++i) { /* GCOVR_EXCL_BR_LINE */
    ...
    const uint16_t v = *ra_mram_reg16(k_ra_mram_off_mentryr);
    if ((v & k_ra_mentryr_mask_pe_mode) == 0U) { /* GCOVR_EXCL_BR_LINE */
      ra_flash_internal_set_prefetch(s_flash_rt.prefetch_on);
      return k_ra_ok;
    }
  }
  return k_ra_err_hw_timeout;
...
{
  /* HUM Ch 59 "MACI Command-Issuing Area" p 3550 */
  ra_flash_internal_maci_cmd8(k_ra_maci_cmd_forced_stop);
...
{
  RA_VALIDATE_INIT(s_flash_rt.initialized, s_flash_tag, "flash_reset before init");
  ...
  /* HUM Ch 59 "MACI Command-Issuing Area" p 3550 */
  ra_flash_internal_maci_cmd8(k_ra_maci_cmd_status_clear);
  (void)ra_flash_internal_wait_mrdy(k_ra_flash_maci_spin_limit);
