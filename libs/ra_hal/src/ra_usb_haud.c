 ...
 *
 * @details
 * Glues the host-mode bring-up paths in `ra_usb` to a USB Audio class
 ...
 *  - `usb_haud_set_sampling_frequency`        -> `ra_usb_haud_set_format`
 *  - Feature-Unit SET_CUR(VOLUME)             -> `ra_usb_haud_set_volume`
 *  - Feature-Unit SET_CUR(MUTE)               -> `ra_usb_haud_set_mute`
 ...
 * (production) or directly via `ra_usb_haud_step` (tests). Each step
...
/**
 * @enum ra_usb_haud_setup_field_t
...
/**
 * @enum ra_usb_haud_byte_shift_t
 * @brief Per-byte left-shift constants for wValue / wIndex layout.
 */
typedef enum : uint8_t {
  k_ra_haud_shift_byte0 = 0U,
  k_ra_haud_shift_byte1 = 8U,
} ra_usb_haud_byte_shift_t;

/**
 * @enum ra_usb_haud_default_unit_t
...
/**
 * @struct ra_usb_haud_state_t
 * @brief Singleton shadow state for the host-Audio driver.
 */
typedef struct {
  bool                    initialized; /**< True after `ra_usb_haud_init`. */
  bool                    attached;    /**< True after enumeration done.   */
  ra_usb_speed_t          speed;       /**< Underlying controller.         */
  ra_usb_haud_step_t      step;        /**< Current enumeration step.      */
  ra_usb_haud_attach_fn_t attach_cb;   /**< Attach callback, or NULL.      */
  void*                   attach_ctx;  /**< Attach callback ctx.           */
  ra_usb_haud_device_t    device;      /**< Snapshot of attached device.   */
} ra_usb_haud_state_t;

static ra_usb_haud_state_t s_state = {};
...
{
  return (speed == k_ra_usb_speed_hs) ? k_ra_haud_iso_max_packet_hs : k_ra_haud_iso_max_packet_fs;
}
...
static ra_err_t internal_setup_get_descriptor(uint8_t desc_type, uint16_t length)
{
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_haud_bm_std_dev_in,
    .b_request       = k_ra_haud_breq_get_descriptor,
    .w_value         = (uint16_t)((uint16_t)desc_type << k_ra_haud_shift_byte1),
    .w_index         = 0U,
    .w_length        = length,
  };
...
{
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_haud_bm_std_dev_out,
    .b_request       = k_ra_haud_breq_set_address,
...
static ra_err_t internal_setup_set_config(uint8_t config_value)
{
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_haud_bm_std_dev_out,
    .b_request       = k_ra_haud_breq_set_config,
...
{
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_haud_bm_std_iface_out,
    .b_request       = k_ra_haud_breq_set_interface,
    .w_value         = k_ra_haud_default_alt,
...
{
  s_state.device.device_address     = (uint8_t)k_ra_haud_assigned_address;
  s_state.device.ac_interface       = k_ra_haud_default_ac_interface;
  s_state.device.as_interface       = k_ra_haud_default_as_interface;
  s_state.device.feature_unit_id    = k_ra_haud_default_feature_unit_id;
  s_state.device.iso_out_ep         = k_ra_haud_default_iso_out_ep;
  s_state.device.iso_in_ep          = k_ra_haud_default_iso_in_ep;
  s_state.device.channel_count      = (uint8_t)k_ra_haud_default_channels;
  s_state.device.bits_per_sample    = (uint8_t)k_ra_haud_default_bits;
  s_state.device.iso_out_max_packet = internal_iso_max_packet(s_state.speed);
  s_state.device.iso_in_max_packet  = 0U;
  s_state.device.sample_rate_min_hz = k_ra_haud_default_sample_rate_hz;
  s_state.device.sample_rate_max_hz = k_ra_haud_default_sample_rate_hz;
...
{
  s_state.step = k_ra_haud_step_bus_reset;
...
static ra_err_t internal_do_bus_reset(void)
{
  ...
  RA_RETURN_ON_ERROR(rel, s_tag, "haud: release bus reset"); /* GCOVR_EXCL_BR_LINE */
  s_state.step = k_ra_haud_step_set_address;
  return internal_setup_set_address((uint8_t)k_ra_haud_assigned_address);
}
...
{
  const ra_err_t addr_err = ra_usb_set_address(s_state.speed, (uint8_t)k_ra_haud_assigned_address);
  RA_RETURN_ON_ERROR(addr_err, s_tag, "haud: set USBADDR"); /* GCOVR_EXCL_BR_LINE */
  s_state.step = k_ra_haud_step_get_dev_desc;
  return internal_setup_get_descriptor(k_ra_haud_desc_device, k_ra_haud_dev_desc_len);
}
...
{
  s_state.step = k_ra_haud_step_get_cfg_desc;
  return internal_setup_get_descriptor(k_ra_haud_desc_configuration, k_ra_haud_cfg_desc_len);
}
...
static ra_err_t internal_do_get_cfg_desc(void)
{
  ...
  s_state.step = k_ra_haud_step_set_config;
  return internal_setup_set_config((uint8_t)k_ra_haud_default_config);
}
...
{
  s_state.step = k_ra_haud_step_set_interface;
...
{
  s_state.step = k_ra_haud_step_walk_desc;
  return k_ra_ok;
}
...
static ra_err_t internal_do_walk_desc(void)
{
  ...
  RA_RETURN_ON_ERROR(pipes_err, s_tag, "haud: configure pipes"); /* GCOVR_EXCL_BR_LINE */
  s_state.attached = true;
  s_state.step     = k_ra_haud_step_done;
...
{
  switch (s_state.step) {
    case k_ra_haud_step_idle:
      return internal_do_idle();
    case k_ra_haud_step_bus_reset:
      return internal_do_bus_reset();
    case k_ra_haud_step_set_address:
      return internal_do_set_address();
    case k_ra_haud_step_get_dev_desc:
      return internal_do_get_dev_desc();
    case k_ra_haud_step_get_cfg_desc:
      return internal_do_get_cfg_desc();
    case k_ra_haud_step_set_config:
      return internal_do_set_config();
    case k_ra_haud_step_set_interface:
      return internal_do_set_interface();
    case k_ra_haud_step_walk_desc:
...
{
  if ((channel_count < (uint8_t)k_ra_haud_min_channels) ||
      (channel_count > (uint8_t)k_ra_haud_max_channels)) {
    return false;
  }
  if ((bits_per_sample < (uint8_t)k_ra_haud_min_bits) ||
      (bits_per_sample > (uint8_t)k_ra_haud_max_bits)) {
    return false;
  }
  if ((sample_rate < k_ra_haud_min_sample_rate_hz) ||
      (sample_rate > k_ra_haud_max_sample_rate_hz)) {
    return false;
  }
  return true;
}
...
ra_err_t ra_usb_haud_init(ra_usb_speed_t speed)
{
  ...
  s_state.speed       = speed;
  s_state.step        = k_ra_haud_step_idle;
  s_state.attached    = false;
  s_state.attach_cb   = nullptr;
  s_state.attach_ctx  = nullptr;
  s_state.device      = (ra_usb_haud_device_t){};
  s_state.initialized = true;

  ra_log_info_val(s_tag, "host-Audio ready", (uint32_t)speed);
  return k_ra_ok;
}
...
ra_usb_haud_set_format(uint8_t channel_count, uint8_t bits_per_sample, uint32_t sample_rate_hz)
{
   ...
   * `ra_usb_haud_device_t` reflects the new format. */
  ...
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_haud_bm_class_ep_out,
    .b_request       = k_ra_haud_req_set_cur,
    .w_value  = (uint16_t)((uint16_t)k_ra_haud_ep_control_sampling_freq << k_ra_haud_shift_byte1),
    .w_index  = (uint16_t)target_ep,
    .w_length = k_ra_haud_sample_rate_payload,
  };
  return ra_usb_host_setup_request(s_state.speed, &setup);
}
...
ra_err_t ra_usb_haud_set_volume(uint8_t channel, int16_t volume)
{
  ...
  const uint16_t w_value =
    (uint16_t)(((uint16_t)k_ra_haud_fu_control_volume << k_ra_haud_shift_byte1) |
               (uint16_t)channel);
  const uint16_t w_index =
    (uint16_t)(((uint16_t)s_state.device.feature_unit_id << k_ra_haud_shift_byte1) |
               (uint16_t)s_state.device.ac_interface);
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_haud_bm_class_iface_out,
    .b_request       = k_ra_haud_req_set_cur,
    .w_value         = w_value,
    .w_index         = w_index,
    .w_length        = k_ra_haud_volume_payload,
  };
...
ra_err_t ra_usb_haud_set_mute(uint8_t channel, bool mute)
{
  ...
  const uint16_t w_value =
    (uint16_t)(((uint16_t)k_ra_haud_fu_control_mute << k_ra_haud_shift_byte1) | (uint16_t)channel);
  const uint16_t w_index =
    (uint16_t)(((uint16_t)s_state.device.feature_unit_id << k_ra_haud_shift_byte1) |
               (uint16_t)s_state.device.ac_interface);
  const ra_usb_setup_t setup = {
    ...
    .w_value         = w_value,
    .w_index         = w_index,
    .w_length        = k_ra_haud_mute_payload,
  };
  (void)mute;
  return ra_usb_host_setup_request(s_state.speed, &setup);
}
...
ra_err_t ra_usb_haud_recv_samples(uint8_t* buf, uint16_t max_len_bytes, uint16_t* got_len_bytes)
{
  ...
  RA_CHECK_NULL_PTR(got_len_bytes, s_tag, "recv_samples: got_len_bytes");
  const ra_err_t pre = internal_class_preflight();
  if (pre != k_ra_ok) {
    return pre;
  }
