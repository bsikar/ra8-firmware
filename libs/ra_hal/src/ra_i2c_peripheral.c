 ...
 * ``ra_i2c.c`` and the bring-up plane in ``ra_i2c_config.c`` are unchanged.
 * Both planes share ``s_i2c_state`` and the log tag via ``ra_i2c_internal.h``.
 *
...
{
  uint8_t mask = (uint8_t)((uint32_t)k_ra_i2c_msk_icser_sar0e << (uint32_t)slot);
  if (general_call) {
    mask |= (uint8_t)k_ra_i2c_msk_icser_gcae;
  }
  return mask;
}
...
ra_i2c_peripheral_receive(uint8_t channel, uint8_t* buf, uint32_t capacity, uint32_t* out_received)
{
  ...
  RA_CHECK_NULL_PTR(buf, s_i2c_tag, "i2c_target_receive: buf");
  RA_CHECK_NULL_PTR(out_received, s_i2c_tag, "i2c_target_receive: out_received");
  *out_received = 0U;
  if (capacity == 0U) {
    return k_ra_err_invalid_arg;
  }
...
ra_i2c_peripheral_transmit(uint8_t channel, const uint8_t* data, uint32_t len, uint32_t* out_sent)
{
  ...
  RA_CHECK_NULL_PTR(data, s_i2c_tag, "i2c_target_transmit: data");
  RA_CHECK_NULL_PTR(out_sent, s_i2c_tag, "i2c_target_transmit: out_sent");
  *out_sent = 0U;
  if (len == 0U) {
    return k_ra_err_invalid_arg;
  }
