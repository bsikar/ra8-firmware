 ...
 * control-OUT DCP arm / read helpers (``ra_usb_dcp_out_arm`` /
 * ``ra_usb_dcp_out_read``). Split out of ``ra_usb.c`` so every
 ...
 * ``ra_usb_internal.h``. Modelled on FSP ``r_usb_hreg_abs.c``; no FSP
