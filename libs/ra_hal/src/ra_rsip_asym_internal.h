 ...
 *
 * - ``ra_rsip_asym.c`` -- the OEM anti-rollback counter, wrapped-key
 ...
 * - ``ra_rsip_ecc.c`` -- ECDSA sign / verify, ECDH key agreement, and
 ...
 * ``ra_rsip_asym.c`` and consumed by those TUs; their full Doxygen
 ...
 * ``ra_rsip_asym.c``; shared with the key wrap / unwrap + KDF paths there and
 * with the ECDH shared-secret pull in ``ra_rsip_ecc.c``.
 *
