 ...
 *
 * @details
 * Companion translation unit to ``ra_sci.c`` for the RA8D2 SCI_B driver
 * (HUM Ch 38). Carries the DMA request-descriptor builders that wire a
 * byte stream to/from TDR/RDR (``ra_sci_write_dma`` /
 * ``ra_sci_read_dma``) and the interrupt dispatch entry points
 * (``ra_sci_dispatch_txi`` / ``ra_sci_dispatch_rxi`` /
 * ``ra_sci_dispatch_eri``) that advance the per-channel async byte
 * stream and fire the legacy attach callbacks.
 *
 * The shared per-channel dispatch table (``s_sci_state``) is defined in
 * ``ra_sci.c`` and reached here through ``ra_sci_internal.h``. See
...
void ra_sci_dispatch_txi(uint8_t channel)
{
  ...
  const ra_sci_tx_fn_t cb  = s_sci_state[channel].tx_fn;
...
void ra_sci_dispatch_rxi(uint8_t channel)
{
  ...
  const ra_sci_rx_fn_t cb  = s_sci_state[channel].rx_fn;
