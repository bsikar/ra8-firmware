 ...
 * lane / status / IRQ path lives in ``ra_mipi_phy.c``; the bulky HUM
 * Tables 64.2 (DSI) / 64.3 (CSI) timing matrix, its lookup walker, the
 * PLL-frequency arithmetic helper, and the ``ra_mipi_phy_select_timing``
 * surface live in ``ra_mipi_phy_timing.c``. Two helper functions are
 ...
 *  - ``internal_mipi_phy_write_timing``: defined in ``ra_mipi_phy.c`` and
 *    used by the table-driven ``ra_mipi_phy_select_timing`` in
 *    ``ra_mipi_phy_timing.c``;
 *  - ``internal_mipi_phy_compute_freq``: defined in
 *    ``ra_mipi_phy_timing.c`` and used by ``ra_mipi_phy_validate_pll_band``
 *    in ``ra_mipi_phy.c``.
 *
 ...
 *
 * @details
 * Defined in ``ra_mipi_phy.c`` and shared with the table-driven
 * ``ra_mipi_phy_select_timing`` in ``ra_mipi_phy_timing.c``. Programs
 ...
 *
 * @pre ``t`` is a valid, non-NULL ``ra_mipi_phy_timing_t``.
 ...
 * @since 0.1.0
 */
void internal_mipi_phy_write_timing(const ra_mipi_phy_timing_t* t);

/**
 * @brief Compute the PLL output frequency for a ``(mosc, pll)`` tuple.
 *
 * @details
 * Defined in ``ra_mipi_phy_timing.c`` and shared with
 * ``ra_mipi_phy_validate_pll_band`` in ``ra_mipi_phy.c``. Implements the
 ...
 *
 * @pre ``pll`` is a valid, non-NULL ``ra_mipi_phy_pll_t``.
 ...
 * @since 0.1.0
 */
uint32_t internal_mipi_phy_compute_freq(const ra_mipi_phy_pll_t* pll, uint8_t mosc_mhz);
