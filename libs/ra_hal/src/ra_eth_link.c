 ...
 *
 * @details
 * Split out of ra_eth.c to keep each translation unit under the
 * per-file line-count cap. Holds the read-mostly half of the NIC
 * API: ``ra_eth_link_status`` plus the one-shot MAC speed/duplex
 ...
 * resync latch (::s_eth_mac_speed_resynced) are defined in ra_eth.c
 * and shared via ra_eth_internal.h.
 *
 ...
 *
 * @pre ::ra_eth_open has succeeded (::s_eth_state.opened == 1).
...
ra_err_t ra_eth_link_status(ra_eth_link_t* out_status)
{
  ...
  const ra_rmac_port_t port = ra_eth_channel_to_port(s_eth_state.cfg.channel);
  uint16_t             bmcr = 0U;
