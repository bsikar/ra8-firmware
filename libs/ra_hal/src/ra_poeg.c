...
/**
 * @brief Translate a public ::ra_poeg_cfg_t into a POEGG bitfield.
 *
 * @details
 ...
 * @retval 0..k_ra_poeg_en_all Bitwise-OR of the four control enables.
 *
 ...
 * @retval k_ra_ok                 Group powered, POEGG written.
 * @retval k_ra_err_null_ptr       ``cfg`` was NULL.
 * @retval k_ra_err_invalid_arg    ``group`` >= 4.
 * @retval k_ra_err_invalid_state  MSTP enable refused.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                 Group disabled, MSTP released.
 * @retval k_ra_err_invalid_arg    ``group`` >= 4 (no register window).
 *
 * @pre IRQs masked or single-threaded shutdown context.
 * @pre ``ra_poeg_init`` was previously called for ``group``.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                 Snapshot written into ``*out_mask``.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                 Selected bits acknowledged.
 * @retval k_ra_err_invalid_arg    ``group`` >= 4.
 *
 * @pre ``ra_poeg_init`` was previously called for ``group``.
 ...
 * ``ra_poeg_dispatch``. Passing ``fn == NULL`` is the documented
 ...
 *
 * @pre IRQs masked or single-threaded init context.
 * @pre Lifetime of ``ctx`` outlives the next ``ra_poeg_dispatch`` call.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                 MSTP reference taken; group running.
 * @retval k_ra_err_invalid_arg    ``group`` >= 4.
 * @retval k_ra_err_invalid_state  ``ra_mstp_enable`` over-took limit.
 *
 * @pre ``ra_poeg_enter_stop`` was previously called for ``group``.
 * @pre Caller manages MSTP reference balance per-group.
 *
