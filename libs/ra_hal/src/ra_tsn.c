...
/**
 * @enum ra_tsn_internal_t
 * @brief Driver-internal magic values.
 *
 * @details
 * - ``k_ra_tsn_min_stab_us`` matches HUM Figure 55.2 (p 3502)
 *   tTSTBL = 30 us; ``ra_tsn_init`` rejects shorter waits.
 * - ``k_ra_tsn_busy_loops_per_us`` is a worst-case lower bound
 ...
 *
 * @details Populated from ``ra_tsn_config_t::high_ref_degc`` so
 ...
 * helper; ``ra_delay_ms()`` is the only timed primitive and its
 ...
 * on the host (RA_SIMULATOR_MODE) it collapses to a no-op which
 * is fine because the host has no timing requirement.
 *
 * @param[in] usec Microseconds to spin (>= ``k_ra_tsn_min_stab_us``).
 *
 * @pre ``usec`` >= 1.
 ...
 *
 * @return ``k_ra_ok`` if the trim selection is one of the
 *         RA8D2-supported pairs, else ``k_ra_err_invalid_arg``.
 *
 * @pre ``cfg`` is non-null.
...
ra_err_t ra_tsn_convert_to_milli_c(uint16_t raw_code, int32_t* out_milli_c)
{
  ...
  volatile const r_tsn_cal_regs_t* cal    = ra_tsn_cal();
  const uint32_t                   cal_hi = cal->TSCDR & (uint32_t)k_ra_tscdr_data_mask;
  const uint32_t                   cal_lo = cal->TSCDR2 & (uint32_t)k_ra_tscdr_data_mask;
  ...
  const int64_t avcc_uv      = (int64_t)k_ra_tsn_adc_avcc_uv;
  const int64_t full_scale   = (int64_t)k_ra_tsn_adc_full_scale;
  ...
  const int64_t t1_milli_c   = (int64_t)s_ra_tsn_high_ref_degc * (int64_t)k_ra_tsn_uv_per_mv;
  const int64_t t2_milli_c   = (int64_t)s_ra_tsn_low_ref_degc * (int64_t)k_ra_tsn_uv_per_mv;
...
{
  /* HUM Ch 11.2.9 "MSTPCRD : Module Stop Control Register D", p 449 */
  return ra_mstp_enable(k_ra_mstp_tsn);
}
