 ...
 * ``libs/ra_hal/inc/ra_rsip_key_injection.h``. Wrapped key blobs are
 * laid out as ``[type | mgmt-info | payload | mac]`` and the MAC is
 * computed with the same xorshift mixer ``ra_sce_key_injection`` uses
...
typedef enum : uint64_t {
  ...
  k_ra_rsip_ki_xor_c     = 27ULL,                 /**< xorshift step c. */
  k_ra_rsip_ki_byte_mask = 0xFFULL,               /**< Low-byte mask.   */
} ra_rsip_ki_internal_t;

/**
 * @enum ra_rsip_ki_layout_t
...
/**
 * @enum ra_rsip_ki_const_t
 * @brief Numeric constants used by the byte-shuffle helpers.
 *
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_ra_rsip_ki_byte_bits   = 8U,    /**< Bits per byte.                  */
  k_ra_rsip_ki_word_byte0  = 0U,    /**< LSB index in a 32-bit word.     */
  k_ra_rsip_ki_word_byte1  = 1U,    /**< Byte 1 of a 32-bit word.        */
  k_ra_rsip_ki_word_byte2  = 2U,    /**< Byte 2 of a 32-bit word.        */
  k_ra_rsip_ki_word_byte3  = 3U,    /**< MSB index in a 32-bit word.     */
  k_ra_rsip_ki_shift_8     = 8U,    /**< 8-bit shift.                    */
  k_ra_rsip_ki_shift_16    = 16U,   /**< 16-bit shift.                   */
  k_ra_rsip_ki_shift_24    = 24U,   /**< 24-bit shift.                   */
  k_ra_rsip_ki_byte_low    = 0xFFU, /**< Low-byte mask (32-bit form).    */
  k_ra_rsip_ki_rsa_e_bytes = 4U,    /**< RSA exponent byte count (stub). */
} ra_rsip_ki_const_t;
 ...
 *
 * @details
 * Mirrors the mixer used in ``ra_sce_key_injection.c`` so the secure
...
{
  uint64_t x = state;
  x ^= x >> k_ra_rsip_ki_xor_a;
  x ^= x << k_ra_rsip_ki_xor_b;
  x ^= x >> k_ra_rsip_ki_xor_c;
  return x * k_ra_rsip_ki_mul;
}
...
{
  buf[k_ra_rsip_ki_word_byte0] = (uint8_t)((tag >> 0U) & (uint32_t)k_ra_rsip_ki_byte_low);
  buf[k_ra_rsip_ki_word_byte1] =
    (uint8_t)((tag >> (uint32_t)k_ra_rsip_ki_shift_8) & (uint32_t)k_ra_rsip_ki_byte_low);
  buf[k_ra_rsip_ki_word_byte2] =
    (uint8_t)((tag >> (uint32_t)k_ra_rsip_ki_shift_16) & (uint32_t)k_ra_rsip_ki_byte_low);
  buf[k_ra_rsip_ki_word_byte3] =
    (uint8_t)((tag >> (uint32_t)k_ra_rsip_ki_shift_24) & (uint32_t)k_ra_rsip_ki_byte_low);
}
...
{
  uint32_t v = 0U;
  v |= (uint32_t)buf[k_ra_rsip_ki_word_byte0] << 0U;
  v |= (uint32_t)buf[k_ra_rsip_ki_word_byte1] << (uint32_t)k_ra_rsip_ki_shift_8;
  v |= (uint32_t)buf[k_ra_rsip_ki_word_byte2] << (uint32_t)k_ra_rsip_ki_shift_16;
  v |= (uint32_t)buf[k_ra_rsip_ki_word_byte3] << (uint32_t)k_ra_rsip_ki_shift_24;
  return v;
}
...
{
  uint64_t state = k_ra_rsip_ki_seed ^ (uint64_t)len;
  for (uint32_t i = 0U; i < len; ++i) {
    state = ki_mix_byte(state, raw[i]);
  }
  for (uint32_t i = 0U; i < (uint32_t)k_ra_rsip_wrapped_mgmt_info_bytes; ++i) {
    state   = ki_xorshift(state);
    mgmt[i] = (uint8_t)(state & k_ra_rsip_ki_byte_mask);
  }
}
...
static void ki_pack(uint8_t* dst, uint32_t type, const uint8_t* payload, uint32_t payload_len)
{
  ...
  for (uint32_t i = 0U; i < (uint32_t)k_ra_rsip_wrapped_max_total; ++i) {
    dst[i] = 0U;
  }
  ki_write_type(dst + (uint32_t)k_ra_rsip_ki_off_type, type);
  ki_fill_mgmt_info(dst + (uint32_t)k_ra_rsip_ki_off_mgmt_info, payload, payload_len);
  for (uint32_t i = 0U; i < payload_len; ++i) {
    dst[(uint32_t)k_ra_rsip_ki_off_payload + i] = payload[i];
  }
                                ...
                                ra_rsip_aes_key_bits_t key_bits)
{
  ...
  ki_pack(installed_key_buf, (uint32_t)k_ra_rsip_wrapped_type_aes, raw_key, key_len);
  return k_ra_ok;
}
                                ...
                                ra_rsip_rsa_size_t size)
{
  ...
  uint8_t        payload[k_ra_rsip_wrapped_max_payload] = {};
  const uint32_t exp_bytes                              = (uint32_t)k_ra_rsip_ki_rsa_e_bytes;
  if ((mod_bytes + exp_bytes) > (uint32_t)k_ra_rsip_wrapped_max_payload) {
    return k_ra_err_invalid_arg;
  }
  ...
  ki_pack(installed_key_buf,
          (uint32_t)k_ra_rsip_wrapped_type_rsa_pub,
          payload,
          mod_bytes + exp_bytes);
  return k_ra_ok;
}
                                ...
                                bool            is_private)
{
  ...
  const uint32_t tag = is_private ? (uint32_t)k_ra_rsip_wrapped_type_ecc_priv
                                  : (uint32_t)k_ra_rsip_wrapped_type_ecc_pub;
                              ...
                              ra_rsip_wrapped_key_type_t expected_type)
{
  ...
  uint8_t        mac[k_ra_rsip_wrapped_mac_bytes] = {};
  const uint32_t mac_off =
    (uint32_t)k_ra_rsip_wrapped_max_total - (uint32_t)k_ra_rsip_wrapped_mac_bytes;
