 ...
 *
 * @details
 * Glues the host-mode bring-up paths in `ra_usb` to a USB HID
 ...
 *
 *  - `usb_hhid_class_request_init` -> `ra_usb_hhid_init`
 *  - `usb_hhid_enumeration`        -> `internal_enum_*` step machine
 *                                     driven by `ra_usb_hhid_step`.
 *  - `usb_hhid_pipe_info`          -> `internal_walk_config_descriptor`
 *  - `usb_hhid_set_report`         -> `ra_usb_hhid_set_report`
 *  - `usb_hhid_get_report`         -> `ra_usb_hhid_get_report`
 *  - `usb_hhid_set_idle`           -> `ra_usb_hhid_set_idle`
 *  - `usb_hhid_set_protocol`       -> `ra_usb_hhid_set_protocol`
 ...
 * (production) or directly via `ra_usb_hhid_step` (tests). Each step
...
/**
 * @enum ra_usb_hhid_setup_field_t
...
/**
 * @enum ra_usb_hhid_byte_shift_t
 * @brief Per-byte left-shift constants for wValue layout.
 */
typedef enum : uint8_t {
  k_ra_hhid_shift_byte0 = 0U,
  k_ra_hhid_shift_byte1 = 8U,
} ra_usb_hhid_byte_shift_t;
...
typedef struct {
  ...
  ra_usb_hhid_attach_fn_t attach_cb;                        /**< Attach callback, or NULL.      */
  void*                   attach_ctx;                       /**< Attach callback ctx.           */
  ra_usb_hhid_device_t    device;                           /**< Snapshot of attached device.   */
  uint8_t                 hid_desc[k_ra_hhid_hid_desc_len]; /**< HID
                                            class descriptor cache.         */
  uint8_t                 report_desc[k_ra_hhid_report_desc_max]; /**< HID
                                            Report descriptor cache.        */
} ra_usb_hhid_state_t;

static ra_usb_hhid_state_t s_state = {};
...
{
  return (speed == k_ra_usb_speed_hs) ? k_ra_hhid_intr_max_packet_hs
                                      : k_ra_hhid_intr_max_packet_default;
}
...
static ra_err_t internal_setup_get_descriptor(uint8_t desc_type, uint16_t length)
{
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_hhid_bm_std_dev_in,
    .b_request       = k_ra_hhid_breq_get_descriptor,
    .w_value         = (uint16_t)((uint16_t)desc_type << k_ra_hhid_shift_byte1),
    .w_index         = 0U,
    .w_length        = length,
  };
...
{
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_hhid_bm_std_dev_out,
    .b_request       = k_ra_hhid_breq_set_address,
...
static ra_err_t internal_setup_set_config(uint8_t config_value)
{
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_hhid_bm_std_dev_out,
    .b_request       = k_ra_hhid_breq_set_config,
...
{
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_hhid_bm_std_iface_out,
    .b_request       = k_ra_hhid_breq_set_interface,
    .w_value         = 0U,
    .w_index         = 0U,
    .w_length        = 0U,
  };
 ...
 * @since 0.1.0
 */
static ra_err_t internal_setup_get_report_descriptor(uint16_t length)
{
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_hhid_bm_std_iface_in,
    .b_request       = k_ra_hhid_breq_get_descriptor,
    .w_value         = (uint16_t)((uint16_t)k_ra_hhid_desc_report << k_ra_hhid_shift_byte1),
...
{
  s_state.device.device_address        = k_ra_hhid_assigned_address;
  s_state.device.intr_in_ep            = 1U;
  s_state.device.intr_out_ep           = 0U;
  s_state.device.interface_number      = 0U;
  s_state.device.subclass              = k_ra_hhid_subclass_boot;
  s_state.device.protocol              = k_ra_hhid_protocol_keyboard;
...
{
  s_state.step = k_ra_hhid_step_bus_reset;
...
{
  s_state.step = k_ra_hhid_step_get_cfg_desc;
  return internal_setup_get_descriptor(k_ra_hhid_desc_configuration, k_ra_hhid_cfg_desc_len);
}
...
{
  s_state.step = k_ra_hhid_step_set_config;
  return internal_setup_set_config(k_ra_hhid_default_config);
}
...
{
  s_state.step = k_ra_hhid_step_set_interface;
...
static ra_err_t internal_do_set_interface(void)
{
  ...
  s_state.step = k_ra_hhid_step_walk_desc;
  return k_ra_ok;
}
 ...
 * @since 0.1.0
 */
static ra_err_t internal_do_get_report_desc(void)
{
  s_state.attached = true;
  s_state.step     = k_ra_hhid_step_done;
 ...
 *
 * @details Invoked from `ra_usb_hhid_step` (tests) or from the CTRT
...
{
  switch (s_state.step) {
    case k_ra_hhid_step_idle:
      return internal_do_idle();
    case k_ra_hhid_step_bus_reset:
      return internal_do_bus_reset();
    case k_ra_hhid_step_set_address:
      return internal_do_set_address();
    case k_ra_hhid_step_get_dev_desc:
      return internal_do_get_dev_desc();
    case k_ra_hhid_step_get_cfg_desc:
      return internal_do_get_cfg_desc();
    case k_ra_hhid_step_set_config:
      return internal_do_set_config();
    case k_ra_hhid_step_set_interface:
      return internal_do_set_interface();
    case k_ra_hhid_step_walk_desc:
      return internal_do_walk_desc();
    case k_ra_hhid_step_get_report_desc:
...
/**
 * @brief Validate that a `ra_usb_hhid_report_type_t` value is in range.
 *
 * @details See implementation.
 ...
 * @since 0.1.0
 */
static bool internal_report_type_ok(ra_usb_hhid_report_type_t t)
{
...
ra_err_t ra_usb_hhid_init(ra_usb_speed_t speed)
{
  ...
  s_state.speed       = speed;
  s_state.step        = k_ra_hhid_step_idle;
                                ...
                                uint16_t*                 got_len)
{
  ...
  const uint16_t       value = (uint16_t)(((uint16_t)target_report_type << k_ra_hhid_shift_byte1) |
                                          (uint16_t)target_report_id);
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_hhid_bm_class_iface_in,
    .b_request       = k_ra_hhid_req_get_report,
    .w_value         = value,
    .w_index         = (uint16_t)s_state.device.interface_number,
    .w_length        = max_len,
  };
                                ...
                                uint16_t                  len)
{
  ...
  const uint16_t       value = (uint16_t)(((uint16_t)target_report_type << k_ra_hhid_shift_byte1) |
                                          (uint16_t)target_report_id);
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_hhid_bm_class_iface_out,
    .b_request       = k_ra_hhid_req_set_report,
    .w_value         = value,
    .w_index         = (uint16_t)s_state.device.interface_number,
    .w_length        = len,
  };
...
ra_err_t ra_usb_hhid_set_idle(uint8_t duration, uint8_t report_id)
{
  ...
  const uint16_t value =
    (uint16_t)(((uint16_t)duration << k_ra_hhid_shift_byte1) | (uint16_t)report_id);
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_hhid_bm_class_iface_out,
    .b_request       = k_ra_hhid_req_set_idle,
    .w_value         = value,
    .w_index         = (uint16_t)s_state.device.interface_number,
    .w_length        = 0U,
  };
...
ra_err_t ra_usb_hhid_set_protocol(ra_usb_hhid_protocol_select_t boot_or_report)
{
  ...
  if ((boot_or_report != k_ra_hhid_proto_boot) && (boot_or_report != k_ra_hhid_proto_report)) {
    return k_ra_err_invalid_arg;
  }
  const ra_usb_setup_t setup = {
    .bm_request_type = k_ra_hhid_bm_class_iface_out,
    .b_request       = k_ra_hhid_req_set_protocol,
