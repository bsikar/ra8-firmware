/**
 * @file ra_eth_internal.h
 * @brief Cross-TU shared surface for the ra_eth driver split.
 *
 * @par Tag
 * [Ring 3 / HAL] {World: NS}
 *
 * @details
 * The ra_eth driver is split across two translation units to stay
 * under the per-file line-count cap: ra_eth.c holds the ESWM block,
 * the GWCA-backed NIC frame path (open / close / write / read /
 * get_stats), and the test-inject hook; ra_eth_link.c holds the PHY
...
/**
 * @enum ra_eth_phy_t
 * @brief PHY-side MIIM constants shared by both ra_eth TUs: the ra_eth.c
 ...
 *        ra_eth_link.c; referenced from ra_eth.c via this declaration.
 * @note Read externally by J-Link only; firmware never reads back.
 * @since 0.1.0
 */
extern volatile uint16_t g_ra_eth_phy_bmsr_after_wait;
 ...
 *
 * @details
 * Defined once in ra_eth.c and referenced from the split-out
 * link-status TU (ra_eth_link.c) via this declaration. Holds the
 * cached configuration, the cumulative software counters, and the
 * "open" flag.
 *
 * @warning Not safe to mutate outside the ra_eth driver path.
 * @since 0.1.0
 */
extern ra_eth_state_t s_eth_state;
 ...
 * ::ra_eth_link_status skip the MPIC write so they remain read-only
 * status pollers. Defined once in ra_eth.c and updated from
 * ra_eth_link.c via this declaration.
 *
 ...
 *
 * @details
 * Shared mapping used by both ra_eth translation units: the NIC
 * frame path (ra_eth.c) selects the RMAC port for teardown, and the
 * link-status TU (ra_eth_link.c) selects the port whose PHY it
 * polls. Pure; no side effects.
 *
 * @param[in] channel Channel id from ::ra_eth_cfg_t (0 or 1).
 * @return Matching ::ra_rmac_port_t.
 *
 * @retval k_ra_rmac_port_0 channel == 0.
 * @retval k_ra_rmac_port_1 channel != 0.
 ...
 * @since 0.1.0
 */
ra_rmac_port_t ra_eth_channel_to_port(uint8_t channel);
