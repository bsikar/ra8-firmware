/**
 * @file ra_usb_pmsc_internal.h
 * @brief Cross-TU surface shared between the device-MSC BOT state
 *        machine (ra_usb_pmsc.c) and the SCSI command handlers
 *        (ra_usb_pmsc_scsi.c).
 *
 * @par Tag
 * [Ring 3 / HAL] {World: NS}
 ...
 * the public contract lives in `ra_usb_pmsc.h`.
 *
...
/**
 * @enum ra_usb_pmsc_cdb_opcode_offset_t
 * @brief Offset of the SCSI operation code inside the cached CDB.
 *
 * @details Shared by the BOT dispatcher (`ra_usb_pmsc.c`) and the
 * SCSI handler TU (`ra_usb_pmsc_scsi.c`). Per SPC-4 the CDB
 * operation code is always byte 0.
 */
typedef enum : uint8_t {
  k_ra_pmsc_cdb_off_opcode = 0U, /**< Operation Code (CDB byte 0). */
} ra_usb_pmsc_cdb_opcode_offset_t;
 ...
 * gate stays clean. Defined in `ra_usb_pmsc.c`, shared with the SCSI
 * handler TU.
 *
 ...
 * @retval k_ra_err_invalid_size Buffer too small.
 * @pre Module state is consistent.
 ...
 * @since 0.1.0
 */
ra_err_t internal_handle_inquiry(uint8_t* data_buf, uint32_t capacity, uint32_t* out_len);
 ...
 * @since 0.1.0
 */
ra_err_t internal_handle_read_capacity(uint8_t* data_buf, uint32_t capacity, uint32_t* out_len);
 ...
 * @since 0.1.0
 */
ra_err_t internal_handle_request_sense(uint8_t* data_buf, uint32_t capacity, uint32_t* out_len);
 ...
 * @since 0.1.0
 */
ra_err_t internal_handle_mode_sense(uint8_t* data_buf, uint32_t capacity, uint32_t* out_len);
 ...
 * @return Result code.
 * @retval k_ra_ok Operation succeeded.
 * @retval k_ra_err_invalid_size Buffer too small for the read.
 * @pre Module state is consistent.
 * @pre Storage backend is attached.
 * @post Caller-visible state matches the documented contract.
 ...
 * @since 0.1.0
 */
ra_err_t internal_handle_read10(uint8_t* data_buf, uint32_t capacity, uint32_t* out_len);
 ...
 * @since 0.1.0
 */
ra_err_t internal_handle_write10(const uint8_t* data_buf, uint32_t* out_len);
