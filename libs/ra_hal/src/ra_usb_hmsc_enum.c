 ...
 *
 * @details
 * Sibling translation unit of `ra_usb_hmsc.c`, split out purely to satisfy
 * the per-file size cap. Holds the hardware-proven polled enumeration ladder
 * (`ra_usb_hmsc_enumerate`) and its `internal_enum_*` helpers: wait for the
 ...
 *
 * The singleton shadow state (`s_usb_hmsc_state`) lives in `ra_usb_hmsc.c`
 * and is shared with this TU through `ra_hal_internal.h`.
 *
...
#include "ra_usb_hmsc_internal.h"
...
static ra_err_t internal_enum_read_dev_desc(uint8_t* desc)
{
  ...
  uint16_t       rx  = 0U;
  const ra_err_t err = ra_usb_host_control_xfer(s_usb_hmsc_state.speed,
                                                &setup,
                                                desc,
                                                (uint16_t)k_ra_hmsc_dev_desc_len,
                                                &rx);
...
static ra_err_t internal_enum_hunt(uint8_t* desc, uint8_t* out_addr)
{
  ...
  for (uint32_t spin = 0U; spin < (uint32_t)k_ra_hmsc_attach_spin_limit; spin++) {
    if (ra_usb_host_line_state(s_usb_hmsc_state.speed) != 0U) {
      break;
    }
  ...
  for (uint8_t attempt = 0U; attempt < (uint8_t)k_ra_hmsc_enum_tries; attempt++) {
    if (attempt >= (uint8_t)k_ra_hmsc_no_reset_tries) {
      (void)ra_usb_host_bus_reset(s_usb_hmsc_state.speed, true);
      ra_delay_ms(k_ra_hmsc_reset_hold_ms);
      (void)ra_usb_host_bus_reset(s_usb_hmsc_state.speed, false);
    }
    (void)ra_usb_host_set_uact(s_usb_hmsc_state.speed, true);
    ra_delay_ms(k_ra_hmsc_recovery_ms);
    const uint8_t addr = (uint8_t)(attempt & (uint8_t)k_ra_hmsc_addr_alt_mask);
    (void)ra_usb_host_set_target(s_usb_hmsc_state.speed, addr);
...
static ra_err_t internal_enum_assign_addr(uint8_t* dev_addr)
{
  ...
  ra_err_t err = ra_usb_host_control_xfer(s_usb_hmsc_state.speed, &setup, nullptr, 0U, nullptr);
  RA_RETURN_ON_ERROR(err, s_tag, "enum: set address"); /* GCOVR_EXCL_BR_LINE */
  ra_delay_ms(k_ra_hmsc_addr_settle_ms);
  err = ra_usb_host_set_target(s_usb_hmsc_state.speed, (uint8_t)k_ra_hmsc_assigned_address);
...
static void internal_enum_note_endpoint(const uint8_t* d)
{
  ...
  if ((ea & (uint8_t)k_ra_hmsc_ep_dir_in_bit) != 0U) {
    if (s_usb_hmsc_state.device.bulk_in_ep == 0U) {
      s_usb_hmsc_state.device.bulk_in_ep         = (uint8_t)(ea & (uint8_t)k_ra_hmsc_ep_num_mask);
      s_usb_hmsc_state.device.bulk_in_max_packet = mps;
    }
  } else {
    if (s_usb_hmsc_state.device.bulk_out_ep == 0U) {
      s_usb_hmsc_state.device.bulk_out_ep         = (uint8_t)(ea & (uint8_t)k_ra_hmsc_ep_num_mask);
...
static ra_err_t internal_enum_walk_cfg(const uint8_t* cfg, uint16_t len)
{
  ...
  while (off < len) {
    ...
    if (dtype == (uint8_t)k_ra_hmsc_desc_interface) {
      ...
      if (in_msc) {
        s_usb_hmsc_state.device.interface_number = cfg[off + (uint16_t)k_ra_hmsc_off_iface_num];
      }
    }
...
static ra_err_t internal_enum_read_config(uint8_t* out_cfg_value)
{
  ...
  ra_err_t err = ra_usb_host_control_xfer(s_usb_hmsc_state.speed,
                                          &setup,
                                          cfg,
                                          (uint16_t)k_ra_hmsc_cfg_desc_len,
                                          &rx);
  ...
  *out_cfg_value = cfg[k_ra_hmsc_off_cfg_value];
  ...
  err            = ra_usb_host_control_xfer(s_usb_hmsc_state.speed, &setup, cfg, total, &rx);
...
static ra_err_t internal_enum_configure(uint8_t dev_addr, uint8_t cfg_value)
{
  ...
  ra_err_t err = ra_usb_host_control_xfer(s_usb_hmsc_state.speed, &set_cfg, nullptr, 0U, nullptr);
  ...
  uint8_t        lun              = 0U;
  uint16_t       lun_rx           = 0U;
  const ra_err_t lun_err          = ra_usb_host_control_xfer(s_usb_hmsc_state.speed,
                                                             &get_lun,
                                                             &lun,
  ...
  err = ra_usb_host_pipe_setup(s_usb_hmsc_state.speed,
  ...
  return ra_usb_host_pipe_setup(s_usb_hmsc_state.speed,
