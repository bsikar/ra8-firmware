 ...
 * line cap:
 *
 *   - `ra_jpeg_sw.c`        -- shared entropy/DSP primitives (bit
 *                             reader, Huffman tables, inverse DCT,
 *                             colour conversion) plus the lightweight
 *                             `ra_jpeg_sw_get_dimensions()` public API.
 *   - `ra_jpeg_sw_decode.c` -- the marker parser, MCU scan loop and
 *                             the `ra_jpeg_sw_decode()` public API.
 *   - `ra_jpeg_sw_encode.c` -- the forward DCT, quantization, Huffman
 *                             code emission and `ra_jpeg_sw_encode()`.
 *
 ...
 * MSBs as a non-negative integer. Defined in `ra_jpeg_sw.c`; the
 ...
 * used by the symbol decoder. Defined in `ra_jpeg_sw.c`; the parser
 ...
 * in `ra_jpeg_sw.c`; the parser unit calls it from `dec_block()`.
 *
 * @param[in,out] br Bit reader (state mutated in place).
 ...
 * Defined in `ra_jpeg_sw.c`; the parser unit calls it from
 * `dec_block()`.
 *
 ...
 * cosine basis. Defined in `ra_jpeg_sw.c`; the parser unit calls it
 * from `dec_idct_into()`.
 *
 ...
 *
 * @details
 * Defined in `ra_jpeg_sw.c`; the parser unit calls it from the MCU
