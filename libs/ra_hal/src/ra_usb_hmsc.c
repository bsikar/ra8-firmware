...
/* Singleton shadow state. Shared with ra_usb_hmsc_enum.c (the polled
 * enumeration ladder) through ra_hal_internal.h: the type and the extern
 * declaration live there, the single definition lives here. */
ra_usb_hmsc_state_t s_usb_hmsc_state = {};
...
ra_err_t ra_usb_hmsc_init(ra_usb_speed_t speed)
{
  ...
  s_usb_hmsc_state.next_cbw_tag = k_ra_hmsc_initial_tag;
...
ra_err_t ra_usb_hmsc_close(void)
{
  ...
  (void)ra_usb_host_set_uact(s_usb_hmsc_state.speed, false);
  const ra_err_t err           = ra_usb_host_deinit(s_usb_hmsc_state.speed);
...
{
  return ra_usb_host_bulk_out(s_usb_hmsc_state.speed,
                                      ...
                                      uint16_t       push_len)
{
  ...
  while (offset < push_len) {
    ...
    const ra_err_t werr =
      ra_usb_host_bulk_out(s_usb_hmsc_state.speed, k_ra_hmsc_pipe_bulk_out, &in_buf[offset], chunk);
