 ...
 *
 * @details
 * Sibling translation unit of ``ra_ipc.c`` carrying the cohesive
 ...
 * ``ra_ipc.h`` for the public API surface. Each register access carries
 * a HUM Ch 3 citation -- IPC is not power-gated by MSTPCR so the driver
 * does not call ``ra_mstp_*``.
 *
