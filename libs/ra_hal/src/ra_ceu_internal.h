/**
 * @file ra_ceu_internal.h
 * @brief Cross-TU surface for the ra_ceu driver split.
 *
 * @details
 ...
 * translation unit (``ra_ceu_init_regs.c``) from the runtime-control
 * entry points (``ra_ceu.c``). The two TUs share only the three
 * ``ra_ceu_program_*`` helpers declared below; everything else stays
 ...
 * (CAMCR). Pre-geometry phase of ``ra_ceu_init``. Promoted from a
 * TU-private static so ``ra_ceu_init`` (in ``ra_ceu.c``) can call it
 * while the implementation lives in ``ra_ceu_init_regs.c``.
 *
 ...
 * @since 0.1.0
 */
void ra_ceu_program_format(const ra_ceu_config_t* cfg);
 ...
 * Promoted from a TU-private static so ``ra_ceu_init`` (in
 * ``ra_ceu.c``) can call it while the implementation lives in
 * ``ra_ceu_init_regs.c``.
 *
 ...
 * @since 0.1.0
 */
void ra_ceu_program_geometry(const ra_ceu_config_t* cfg);
 ...
 * ``ra_ceu_init``. Promoted from a TU-private static so
 * ``ra_ceu_init`` (in ``ra_ceu.c``) can call it while the
 * implementation lives in ``ra_ceu_init_regs.c``.
 *
 * @param[in] cfg Validated config; must not be nullptr.
 *
 ...
 * @since 0.1.0
 */
void ra_ceu_program_destination(const ra_ceu_config_t* cfg);
