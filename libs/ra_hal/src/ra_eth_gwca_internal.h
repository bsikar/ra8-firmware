/**
 * @file ra_eth_gwca_internal.h
 * @brief Cross-TU shared surface for the ra_eth_gwca driver split.
 *
 * @par Tag
 * [Ring 3 / HAL] {World: NS}
 *
 * @details
 * The ra_eth_gwca driver is split across three translation units to
 * stay under the per-file line-count cap: ra_eth_gwca.c holds the
 * lifecycle / status / dispatch surface plus the GWCA state-machine
 * bring-up (set_operation_mode / axi_init / install_linkfix /
 * bring_up); ra_eth_gwca_queue.c holds the per-queue descriptor and
 * ring primitives (configure_queue / reload_queue / init_ring /
 * attach_buffers / kick_tx / find_slot / tx_frame); and
 * ra_eth_gwca_default.c holds the one-call default-state API
 ...
 * on real silicon. On host (RA_SIMULATOR_MODE) the loops run against
 * the mmap'd peri region and the bits flip immediately, so the
 * budget is mostly hit-once. Shared because ra_eth_gwca.c spins on
 * GWMS.OPS / GWARIRM.ARR, ra_eth_gwca_queue.c spins on GWDCCi.BALR,
 * and ra_eth_gwca_default.c spins on TX descriptor write-back.
 */
enum : uint32_t {
  k_ra_eth_gwca_mode_spin    = 2000000UL, /**< GWMS.OPS / GWARIRM.ARR poll budget.   */
 ...
 * because ra_eth_gwca.c bumps the bring-up trail while
 * ra_eth_gwca_default.c bumps the open / pre trails.
 */
 ...
 * Defined in ra_eth_gwca.c; bumped from ra_eth_gwca_default.c.
 *
 * @note Read externally by J-Link only; firmware never reads back.
 * @since 0.1.0
 */
extern volatile uint32_t g_ra_eth_gwca_open_step;
 ...
 * ra_eth_gwca.c; bumped from ra_eth_gwca_default.c.
 *
 * @note Read externally by J-Link only; firmware never reads back.
 * @since 0.1.0
 */
extern volatile uint32_t g_ra_eth_gwca_pre_step;
 ...
 * Error codes ``0x10|N`` for failures at step N. Defined in
 * ra_eth_gwca.c; bumped from ra_eth_gwca.c (bring-up phase).
 *
 * @note Read externally by J-Link only; firmware never reads back.
 * @since 0.1.0
 */
extern volatile uint32_t g_ra_eth_gwca_bring_up_step;
 ...
 * ra_eth_gwca_queue.c and shared with ra_eth_gwca_default.c
 ...
 * 40-bit format generically. Defined in ra_eth_gwca_queue.c and
 * shared with ra_eth_gwca_default.c (the RX drain path decodes the
