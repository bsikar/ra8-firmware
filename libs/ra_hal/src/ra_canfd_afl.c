/**
 * @file ra_canfd_afl.c
 * @brief CANFD Acceptance-Filter-List (AFL) configuration (split from ra_canfd.c)
 *
 * @par Tag
 * [Ring 3 / HAL] {World: NS}
 *
 ...
 * ``ra_canfd.c`` purely to satisfy the per-file size cap. This
 ...
 *
 *  - Assemble each ``ra_canfd_afl_rule_t`` into the paged GAFL register
 ...
 *
 * The single-rule ``ra_canfd_filter_set`` (which wraps the GL_RESET
 * transition itself) lives in ``ra_canfd.c``; this file provides the
...
/**
 * @enum ra_canfd_afl_local_t
 * @brief Local constants owned by the AFL list-config path.
 *
 * @details
 * ``k_ra_canfd_afl_page0`` is the AFLPN page index this API targets --
 * all rules land in the single 16-entry page 0, matching the
 * ``k_ra_canfd_afl_rule_capacity`` cap in the public header. It is a
...
typedef enum : uint32_t {
  k_ra_canfd_afl_page0 = 0U, /**< AFLPN page index used by this API. */
} ra_canfd_afl_local_t;

/**
 * @brief Range-check one ``ra_canfd_afl_rule_t`` against the protocol limits.
 *
 * @details
 ...
 * @retval k_ra_ok              Rule fields are all in range.
 * @retval k_ra_err_invalid_arg id / mask overflow or target_rx out of range.
 *
 ...
 * @post On k_ra_ok every field fits its documented range.
 *
 * @note Internal helper, not exported. Not thread-safe.
 * @since 0.1.0
 */
static ra_err_t internal_afl_validate_rule(const ra_canfd_afl_rule_t* rule)
{
  if (rule->target_rx >= (uint8_t)k_ra_canfd_rx_fifo_count) {
    return k_ra_err_invalid_arg;
  }
  if (rule->extended) {
    if ((rule->id & ~(uint32_t)k_ra_canfd_id_ext_mask) != 0U) {
      return k_ra_err_invalid_arg;
    }
    if ((rule->mask & ~(uint32_t)k_ra_canfd_id_ext_mask) != 0U) {
      return k_ra_err_invalid_arg;
    }
    return k_ra_ok;
  }
  if ((rule->id & ~(uint32_t)k_ra_canfd_id_std_mask) != 0U) {
    return k_ra_err_invalid_arg;
  }
  if ((rule->mask & ~(uint32_t)k_ra_canfd_id_std_mask) != 0U) {
    return k_ra_err_invalid_arg;
  }
  return k_ra_ok;
}
 ...
 * @since 0.1.0
 */
static uint32_t internal_afl_id_word(const ra_canfd_afl_rule_t* rule)
{
  const uint32_t masked = rule->extended ? (rule->id & (uint32_t)k_ra_canfd_id_ext_mask)
                                         : (rule->id & (uint32_t)k_ra_canfd_id_std_mask);
  uint32_t       word   = masked;
  if (rule->extended) {
    word |= (uint32_t)k_ra_canfd_id_ide;
  }
  if (rule->rtr) {
    word |= (uint32_t)k_ra_canfd_id_rtr;
  }
  return word;
}
 ...
 * @since 0.1.0
 */
static uint32_t internal_afl_mask_word(const ra_canfd_afl_rule_t* rule)
{
  const uint32_t masked = rule->extended ? (rule->mask & (uint32_t)k_ra_canfd_id_ext_mask)
                                         : (rule->mask & (uint32_t)k_ra_canfd_id_std_mask);
  return masked | (uint32_t)k_ra_gaflm_bit_gaflidem | (uint32_t)k_ra_gaflm_bit_gaflrtrm;
}
 ...
 *
 * @pre ``rule`` is non-NULL and already range-checked.
 * @pre ``rule->target_rx`` < ::k_ra_canfd_rx_fifo_count.
 * @post No register state is modified (pure assembly).
 ...
 * @since 0.1.0
 */
static uint32_t internal_afl_p1_word(const ra_canfd_afl_rule_t* rule)
{
  return (uint32_t)k_ra_gaflp1_bit_gaflfdp0 << rule->target_rx;
}
 ...
 */
static void
internal_afl_write_slot(volatile r_canfd_t* reg, uint8_t slot, const ra_canfd_afl_rule_t* rule)
{
 ...
 *
 * @param[in] reg   Non-NULL CANFD register block for the target channel.
 * @param[in] count Rule count, 1..::k_ra_canfd_afl_rule_capacity.
 *
 * @pre ``reg`` is non-NULL and the global block is in GL_RESET.
 * @pre ``count`` <= ::k_ra_canfd_afl_rule_capacity (fits the 5-bit field).
...
static void internal_afl_set_rule_count(volatile r_canfd_t* reg, uint8_t count)
{
  ...
  const uint32_t rnc  = ((uint32_t)count & k_ra_gaflcfg0_mask_rnc0) << k_ra_gaflcfg0_shift_rnc0;
  /* HUM Ch 41.2.18 "CFDGAFLCFG0 : Global AFL Configuration Register 0" p 2735 */
  reg->CFDGAFLCFG0 = (cfg0 & ~(k_ra_gaflcfg0_mask_rnc0 << k_ra_gaflcfg0_shift_rnc0)) | rnc;
}
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              All rules are in range.
 * @retval k_ra_err_invalid_arg The first failing rule's error.
 *
 * @pre ``rules`` is non-NULL with at least ``count`` elements.
 * @pre ``count`` <= ::k_ra_canfd_afl_rule_capacity (loop bound).
 * @post No register or global state is modified (pure predicate).
 * @post On k_ra_ok every rule passed ::internal_afl_validate_rule.
 *
 * @note Internal helper, not exported. Not thread-safe.
 * @since 0.1.0
 */
static ra_err_t internal_afl_validate_all(const ra_canfd_afl_rule_t* rules, uint8_t count)
{
  for (uint8_t i = 0U; i < count; i++) {
    const ra_err_t v = internal_afl_validate_rule(&rules[i]);
    if (v != k_ra_ok) {
      return v;
    }
  }
...
{
  RA_CHECK_NULL_PTR(rules, s_tag, "rules must not be nullptr");
  volatile r_canfd_t* reg = ra_canfd(channel);
  RA_CHECK_NULL_PTR(reg, s_tag, "channel out of range");
  if (count == 0U) {
    return k_ra_err_invalid_arg;
  }
  if (count > (uint8_t)k_ra_canfd_afl_rule_capacity) {
    return k_ra_err_invalid_arg;
  }

  const ra_err_t v = internal_afl_validate_all(rules, count);
  if (v != k_ra_ok) {
    return v;
  }
  ...
  reg->CFDGAFLECTR =
    ((uint32_t)k_ra_canfd_afl_page0 & k_ra_gaflectr_mask_aflpn) | k_ra_gaflectr_bit_afldae;
