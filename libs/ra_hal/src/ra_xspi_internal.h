/**
 * @file ra_xspi_internal.h
 * @brief Module-private seam shared between the ra_xspi translation units.
 *
 * @par Tag
 * [Ring 3 / HAL] {World: S}
 *
 ...
 *
 * @details
 * Defined in ``ra_xspi_flash.c``. FSP ``r_ospi_b_direct_transfer`` waits
 ...
 * ``ra_xspi.c`` (software-reset path) can reuse it. HUM Ch 44 p 2986.
 *
 * @param[in] reg xSPI register block (already gated open by the caller).
 *
 * @return ::ra_err_t outcome.
 * @retval k_ra_ok             CMDCMP observed; transfer retired.
 * @retval k_ra_err_hw_timeout CMDCMP never fired within the budget.
 *
 ...
 *
 * @details
 * Defined in ``ra_xspi_flash.c``. Populates CDBUF slot 0 per FSP
 * ``r_ospi_b_direct_transfer`` (CDT carries opcode + size encoding,
 * CDA/CDD0/CDD1 zeroed) and kicks the transfer. Promoted to
 * TU-external linkage so the lifecycle surface in ``ra_xspi.c``
 ...
 *
 * @return ::ra_err_t outcome.
 * @retval k_ra_ok             Command dispatched and CMDCMP cleared.
 * @retval k_ra_err_hw_timeout Underlying CMDCMP timeout.
 *
 * @pre ``reg != nullptr`` and the xSPI MSTP gate is open.
