 ...
 *
 * @details
 * Polling-mode peripheral companion to ``ra_i3c_i2c``. Mirrors FSP
 ...
 *    into ``k_ra_i3c_i2c_peripheral_status_*``.
 *
...
/**
 * @enum ra_i3c_i2c_peripheral_internal_t
 * @brief Internal tunables.
 */
typedef enum : uint32_t {
  k_ra_i3c_i2c_peripheral_spin_budget = 50000U, /**< Bounded poll iterations. */
  k_ra_i3c_i2c_peripheral_max_addr_7b = 0x7FU,  /**< 7-bit address ceiling.   */
} ra_i3c_i2c_peripheral_internal_t;

/**
 * @enum ra_i3c_i2c_peripheral_bits_t
 ...
 * @since 0.1.0
 */
static ra_err_t internal_wait_ntst(volatile r_i3c_i2c_regs_t* reg, uint32_t mask)
{
  for (uint32_t i = 0U; i < k_ra_i3c_i2c_peripheral_spin_budget; ++i) { /* GCOVR_EXCL_BR_LINE */
...
ra_err_t internal_i3c_i2c_peripheral_open(uint8_t channel, const ra_i3c_i2c_peripheral_cfg_t* cfg)
{
  ...
  if (reg == nullptr) {
    ra_log_error(s_tag, "internal_i3c_i2c_peripheral_open: channel out of range");
    return k_ra_err_invalid_arg;
  }
  if (cfg->peripheral_addr_7b > (uint8_t)k_ra_i3c_i2c_peripheral_max_addr_7b) {
    return k_ra_err_invalid_arg;
  }

  reg->RSTCTL = 0U;
  reg->MSDVAD = (uint32_t)cfg->peripheral_addr_7b << k_ra_i3c_i2c_peripheral_msdvad_shift_dvad;
  reg->SVCTL  = (cfg->general_call != 0U) ? k_ra_i3c_i2c_peripheral_msk_svctl_gcae : 0U;
  reg->BCTL   = k_ra_i3c_i2c_msk_bctl_buse;
  ra_log_info_val(s_tag, "internal_i3c_i2c_peripheral_open ch", (uint32_t)channel);
  return k_ra_ok;
}
...
ra_err_t internal_i3c_i2c_peripheral_send(uint8_t channel, const uint8_t* data, uint32_t len)
{
  ...
  if ((len > 0U) && (data == nullptr)) {
    ra_log_error(s_tag, "internal_i3c_i2c_peripheral_send: data null");
    return k_ra_err_null_ptr;
  }
  for (uint32_t i = 0U; i < len; ++i) {
    RA_RETURN_ON_ERROR(
      internal_wait_ntst(reg, k_ra_i3c_i2c_peripheral_msk_ntst_tdbef0), /* GCOVR_EXCL_BR_LINE */
      s_tag,
...
ra_err_t internal_i3c_i2c_peripheral_receive(uint8_t channel, uint8_t* buf, uint32_t len)
{
  ...
  if ((len > 0U) && (buf == nullptr)) {
    ra_log_error(s_tag, "internal_i3c_i2c_peripheral_receive: buf null");
    return k_ra_err_null_ptr;
  }
  for (uint32_t i = 0U; i < len; ++i) {
    RA_RETURN_ON_ERROR(
      internal_wait_ntst(reg, k_ra_i3c_i2c_peripheral_msk_ntst_rdbff0), /* GCOVR_EXCL_BR_LINE */
      s_tag,
      "internal_i3c_i2c_peripheral_receive: RDBFF0 wait");
    buf[i] = (uint8_t)(reg->NTDTBP0 & k_ra_i3c_i2c_peripheral_byte_mask);
  }
  return k_ra_ok;
}
...
ra_err_t internal_i3c_i2c_peripheral_status(uint8_t channel, uint8_t* out_mask)
{
  ...
  uint8_t        mask = 0U;
  if ((ntst & k_ra_i3c_i2c_peripheral_msk_ntst_rdbff0) != 0U) {
    mask |= k_ra_i3c_i2c_peripheral_status_rx_full;
  }
  if ((ntst & k_ra_i3c_i2c_peripheral_msk_ntst_tdbef0) != 0U) {
    mask |= k_ra_i3c_i2c_peripheral_status_tx_empty;
  }
  if ((bst & k_ra_i3c_i2c_msk_bst_spcnddf) != 0U) {
    mask |= k_ra_i3c_i2c_peripheral_status_stop;
  }
  if ((bst & k_ra_i3c_i2c_msk_bst_nackdf) != 0U) {
    mask |= k_ra_i3c_i2c_peripheral_status_nack;
  }
  if (reg->MSDVAD != 0U) {
    mask |= k_ra_i3c_i2c_peripheral_status_aas;
  }
  *out_mask = mask;
  return k_ra_ok;
}
