 ...
 * its clocks come up alongside MIPI CSI when `k_ra_mstp_mipi_csi`
 * is enabled. The driver therefore routes power transitions through
 * the existing `ra_mstp` enum rather than opening up MSTPCRC for
 * direct access.
 *
 ...
 * Matches the convention from ra_glcdc.c and ra_acmphs.c.
 *
...
/**
 * @var s_vin_fn
 * @brief Registered VIN event callback (set by `ra_vin_attach_handler`).
 *
 * @details
 * Single shared callback for both the status and error IRQ vectors.
 * NULL means "no callback installed" -- `ra_vin_dispatch` becomes
 * a status-clear-only no-op in that case.
 *
 * @warning Direct modification outside `ra_vin_attach_handler` is not
 ...
 * @since 0.1.0
 */
static ra_vin_event_fn_t s_vin_fn;
 ...
 *
 * @details
 * Opaque pointer supplied to `ra_vin_attach_handler` and handed back
 ...
 * @pre `cfg->interlace_mode <= k_ra_vin_max_im`.
 * @post Returned value has `k_ra_vin_mc_me` = 0.
 * @post Returned value has `k_ra_vin_mc_st` = 0 (start pulse only on demand).
 *
 * @note Pure function; safe from any context.
 * @since 0.1.0
 */
static uint32_t internal_compose_mc(const ra_vin_config_t* cfg)
{
 ...
 * VIN shares its module-stop bit with MIPI CSI on the RA8D2. */
  ...
  /* HUM Ch 67.2.1 "MC: Main Control Register" p 3975 */
  *ra_vin_reg32(k_ra_vin_off_mc) = mc_clean;
  ...
  *ra_vin_reg32(k_ra_vin_off_is) = (uint32_t)cfg->image_stride_px;
  /* HUM Ch 67.2.10 "MB1: Memory Base 1 Register" p 3985 */
  *ra_vin_reg32(k_ra_vin_off_mb1) = cfg->framebuffer_addr_1;
  /* HUM Ch 67.2.11 "MB2: Memory Base 2 Register" p 3986 */
  *ra_vin_reg32(k_ra_vin_off_mb2) = cfg->framebuffer_addr_2;
  /* HUM Ch 67.2.12 "MB3: Memory Base 3 Register" p 3987 */
  *ra_vin_reg32(k_ra_vin_off_mb3) = cfg->framebuffer_addr_3;

  /* HUM Ch 67.2.15 "IE: Interrupt Enable Register" p 3987 */
  *ra_vin_reg32(k_ra_vin_off_ie) = cfg->interrupt_enable;
...
[[nodiscard]] ra_err_t ra_vin_deinit(void)
{
  ...
  *ra_vin_reg32(k_ra_vin_off_fc) = 0UL;
  /* HUM Ch 67.2.1 "MC: Main Control Register" p 3975 */
  *ra_vin_reg32(k_ra_vin_off_mc) = 0UL;
  /* HUM Ch 67.2.15 "IE: Interrupt Enable Register" p 3987 */
 ...
 * recycle the frame buffer once OUTSTAND reaches 0. */
  ...
  *mtc_reg                   = k_ra_vin_mtcstop_req;
  ...
  ra_log_info(s_tag, "vin_capture_stop");
  return err;
}
...
[[nodiscard]] ra_err_t ra_vin_set_preclip(const ra_vin_preclip_t* window)
{
  ...
  *ra_vin_reg32(k_ra_vin_off_slprc) = (uint32_t)window->line_start;
  /* HUM Ch 67.2.5 "ELPRC: Pre-Clip End Line Register" p 3980 */
  *ra_vin_reg32(k_ra_vin_off_elprc) = (uint32_t)window->line_end;
  /* HUM Ch 67.2.6 "SPPRC: Pre-Clip Start Pixel Register" p 3981 */
  *ra_vin_reg32(k_ra_vin_off_spprc) = (uint32_t)window->pixel_start;
  /* HUM Ch 67.2.7 "EPPRC: Pre-Clip End Pixel Register" p 3981 */
  *ra_vin_reg32(k_ra_vin_off_epprc) = (uint32_t)window->pixel_end;
  return k_ra_ok;
}
...
{
  if ((v_bwidth > (uint8_t)k_ra_vin_uds_max_bw) || (h_bwidth > (uint8_t)k_ra_vin_uds_max_bw)) {
    return k_ra_err_invalid_arg;
  }
 ...
 */
static void
internal_yc_offsets(uint8_t channel, ra_vin_off_t* off1, ra_vin_off_t* off2, ra_vin_off_t* off3)
{
  /* HUM Ch 67.2.27 "YCCR1: RGB-to-Y Coefficient R" p 4002-4011 */
  if (channel == 0U) {
    *off1 = k_ra_vin_off_yccr1;
    *off2 = k_ra_vin_off_yccr2;
    *off3 = k_ra_vin_off_yccr3;
  } else if (channel == 1U) {
    *off1 = k_ra_vin_off_cbccr1;
    *off2 = k_ra_vin_off_cbccr2;
    *off3 = k_ra_vin_off_cbccr3;
  } else {
    *off1 = k_ra_vin_off_crccr1;
    *off2 = k_ra_vin_off_crccr2;
    *off3 = k_ra_vin_off_crccr3;
  }
}
...
[[nodiscard]] ra_err_t ra_vin_set_rgb_to_yc(uint8_t channel, const ra_vin_rgb_to_yc_chan_t* row)
{
  ...
  ra_vin_off_t off2 = k_ra_vin_off_yccr2;
  ra_vin_off_t off3 = k_ra_vin_off_yccr3;
...
[[nodiscard]] ra_err_t ra_vin_set_framebuffers(uint32_t mb1, uint32_t mb2, uint32_t mb3)
{
  ...
  if (mb1 != 0U) {
    /* HUM Ch 67.2.10 "MB1: Memory Base 1 Register" p 3985 */
    *ra_vin_reg32(k_ra_vin_off_mb1) = mb1;
  }
  if (mb2 != 0U) {
    /* HUM Ch 67.2.11 "MB2: Memory Base 2 Register" p 3986 */
    *ra_vin_reg32(k_ra_vin_off_mb2) = mb2;
  }
  if (mb3 != 0U) {
    /* HUM Ch 67.2.12 "MB3: Memory Base 3 Register" p 3987 */
    *ra_vin_reg32(k_ra_vin_off_mb3) = mb3;
  }
  return k_ra_ok;
}
...
[[nodiscard]] ra_err_t ra_vin_set_uv_offset(uint32_t uv_addr)
{
  ...
  *ra_vin_reg32(k_ra_vin_off_uvaof) = uv_addr;
  return k_ra_ok;
}
...
[[nodiscard]] ra_err_t ra_vin_get_status(uint32_t* out_mask)
{
  ...
  /* HUM Ch 67.2.16 "INTS: Interrupt Status Register" p 3989 */
  *out_mask = *ra_vin_reg32(k_ra_vin_off_ints);
  return k_ra_ok;
}
...
{
  /* HUM Ch 67.2.15 "IE: Interrupt Enable Register" p 3987 */
  *ra_vin_reg32(k_ra_vin_off_ie) = mask;
  return k_ra_ok;
}

[[nodiscard]] ra_err_t ra_vin_set_scanline_compare(uint16_t line)
{
  if (line > (uint16_t)k_ra_vin_si_mask) {
    return k_ra_err_invalid_arg;
  }
  /* HUM Ch 67.2.17 "SI: Scanline Interrupt Register" p 3990 */
  *ra_vin_reg32(k_ra_vin_off_si) = (uint32_t)line;
  return k_ra_ok;
}
...
{
  /* HUM Ch 67.2.16 "INTS: Interrupt Status Register" p 3989 */
  volatile uint32_t* reg  = ra_vin_reg32(k_ra_vin_off_ints);
  const uint32_t     mask = *reg;

  const ra_vin_event_fn_t fn  = s_vin_fn;
