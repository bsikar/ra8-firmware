 ...
 *
 * @details
 * Companion translation unit to ``ra_mipi_csi.c``. Owns the
 ...
 * lifecycle code in ``ra_mipi_csi.c`` detaches every callback through
 * the promoted ::ra_mipi_csi_detach_all_handlers helper so that the
...
/**
 * @enum ra_mipi_csi_irq_intern_t
 * @brief Internal limits used by this translation unit only.
 *
 * @details
 * ``k_ra_mipi_csi_vc_invalid_idx`` is the sentinel VC id forwarded to
