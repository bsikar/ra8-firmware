 ...
 *
 * @details
 * Glues the device-mode bring-up paths in `ra_usb` to a class-agnostic
 ...
 *    `ra_usb_composite_set_descriptors`.
...
/**
 * @enum ra_usb_composite_request_type_t
...
/**
 * @enum ra_usb_composite_recipient_t
...
/**
 * @enum ra_usb_composite_std_request_t
 * @brief Standard `bRequest` codes the composite layer answers
 *        (USB 2.0 sec 9.4 Table 9-4).
 */
typedef enum : uint8_t {
  k_ra_usb_composite_std_get_status        = 0x00U, /**< GET_STATUS.        */
  k_ra_usb_composite_std_clear_feature     = 0x01U, /**< CLEAR_FEATURE.     */
  k_ra_usb_composite_std_set_feature       = 0x03U, /**< SET_FEATURE.       */
  k_ra_usb_composite_std_set_address       = 0x05U, /**< SET_ADDRESS.       */
  k_ra_usb_composite_std_get_descriptor    = 0x06U, /**< GET_DESCRIPTOR.    */
  k_ra_usb_composite_std_set_descriptor    = 0x07U, /**< SET_DESCRIPTOR.    */
  k_ra_usb_composite_std_get_configuration = 0x08U, /**< GET_CONFIGURATION. */
  k_ra_usb_composite_std_set_configuration = 0x09U, /**< SET_CONFIGURATION. */
  k_ra_usb_composite_std_get_interface     = 0x0AU, /**< GET_INTERFACE.     */
  k_ra_usb_composite_std_set_interface     = 0x0BU, /**< SET_INTERFACE.     */
} ra_usb_composite_std_request_t;

/**
 * @enum ra_usb_composite_address_limit_t
...
/**
 * @enum ra_usb_composite_handler_sentinel_t
 ...
 * dispatched. Equals `k_ra_usb_composite_max_classes` so a direct
 * comparison with the registered-class index is unambiguous.
 */
typedef enum : uint8_t {
  k_ra_usb_composite_handler_self = (uint8_t)k_ra_usb_composite_max_classes,
} ra_usb_composite_handler_sentinel_t;
...
/**
 * @struct ra_usb_composite_state_data_t
...
{
  for (uint8_t i = 0U; i < (uint8_t)k_ra_usb_composite_max_ifs; ++i) {
...
static ra_err_t internal_validate_class(const ra_usb_composite_class_t* cl)
{
  ...
  if (end > (uint16_t)k_ra_usb_composite_max_ifs) {
    return k_ra_err_invalid_arg;
  }
  return k_ra_ok;
}
...
{
  if (if_num >= (uint8_t)k_ra_usb_composite_max_ifs) {
 ...
 * address into `USBADDR` via `ra_usb_set_address`; GET_DESCRIPTOR /
...
static ra_err_t internal_handle_standard(const ra_usb_setup_t* setup)
{
  ...
  if (setup->b_request == (uint8_t)k_ra_usb_composite_std_set_address) {
    const uint16_t addr16 = setup->w_value;
    if (addr16 > (uint16_t)k_ra_usb_composite_max_address) {
      return k_ra_err_invalid_arg;
    }
    return ra_usb_set_address(s_state.speed, (uint8_t)addr16);
  }
...
static ra_err_t internal_route_class(const ra_usb_setup_t* setup, uint8_t* out_idx)
{
  ...
  *out_idx                                 = idx;
  const ra_usb_composite_class_t* const cl = &s_state.classes[idx];
...
ra_err_t ra_usb_composite_init(ra_usb_speed_t speed)
{
  ...
  s_state.speed              = speed;
  s_state.state              = k_ra_usb_composite_state_idle;
  s_state.class_count        = 0U;
  s_state.device_desc        = nullptr;
  s_state.config_desc        = nullptr;
  s_state.last_handler_class = (uint8_t)k_ra_usb_composite_handler_self;
...
ra_err_t ra_usb_composite_close(void)
{
  ...
  if (deinit_err != k_ra_ok) {
    ra_log_error_val(s_tag, "ra_usb_device_deinit failed", (uint32_t)deinit_err);
    return deinit_err;
  }
  return k_ra_ok;
}
...
ra_err_t ra_usb_composite_register_class(const ra_usb_composite_class_t* class_layer)
{
  ...
  if (init_check != k_ra_ok) {
    return init_check;
  }
  const ra_err_t val = internal_validate_class(class_layer);
  if (val != k_ra_ok) {
    return val;
  }
  if (s_state.class_count >= (uint8_t)k_ra_usb_composite_max_classes) {
    return k_ra_err_no_mem;
  }
  const ra_err_t coll = internal_check_collision(class_layer);
  if (coll != k_ra_ok) {
    return coll;
  }
   ...
   * `ra_usb_composite_close` to tear everything down). */
  const ra_err_t init_err = class_layer->init(class_layer->ctx);
  if (init_err != k_ra_ok) {
    ra_log_error_val(s_tag, "class init failed", (uint32_t)init_err);
    return init_err;
  }
  ra_log_info_val(s_tag, "class registered", (uint32_t)slot);
  return k_ra_ok;
}
...
ra_err_t ra_usb_composite_set_descriptors(const uint8_t* device_desc, const uint8_t* config_desc)
{
  ...
  RA_CHECK_NULL_PTR(device_desc, s_tag, "set_descriptors: device");
  RA_CHECK_NULL_PTR(config_desc, s_tag, "set_descriptors: config");

  s_state.device_desc = device_desc;
  s_state.config_desc = config_desc;
  ra_log_info(s_tag, "descriptors cached");
  return k_ra_ok;
}
...
ra_err_t ra_usb_composite_step(void)
{
  ...
  switch (s_state.state) {
    case k_ra_usb_composite_state_idle:
      s_state.state = k_ra_usb_composite_state_setup_rx;
      break;
    case k_ra_usb_composite_state_setup_rx:
      s_state.state = k_ra_usb_composite_state_std_dispatch;
      break;
    case k_ra_usb_composite_state_std_dispatch:
      s_state.state = k_ra_usb_composite_state_class_dispatch;
      break;
    case k_ra_usb_composite_state_class_dispatch:
      s_state.state = k_ra_usb_composite_state_done;
      break;
    case k_ra_usb_composite_state_done:
    default:
      s_state.state = k_ra_usb_composite_state_idle;
      break;
  }
...
ra_err_t ra_usb_composite_dispatch_setup(const ra_usb_setup_t* setup, uint8_t* out_handler_class)
{
  ...
  RA_CHECK_NULL_PTR(setup, s_tag, "dispatch_setup: setup");
  RA_CHECK_NULL_PTR(out_handler_class, s_tag, "dispatch_setup: out_handler");
  ...
  const uint8_t type =
    (uint8_t)(setup->bm_request_type & (uint8_t)k_ra_usb_composite_req_type_mask);

  if (type == (uint8_t)k_ra_usb_composite_req_type_standard) {
    s_state.last_handler_class = (uint8_t)k_ra_usb_composite_handler_self;
    *out_handler_class         = (uint8_t)k_ra_usb_composite_handler_self;
    return internal_handle_standard(setup);
  }

  uint8_t        class_idx = 0U;
  const ra_err_t route_err = internal_route_class(setup, &class_idx);
  if (route_err == k_ra_err_not_found) {
    return k_ra_err_not_found;
  }
...
ra_err_t ra_usb_composite_get_class_count(uint8_t* out_count)
{
  ...
  RA_CHECK_NULL_PTR(out_count, s_tag, "get_class_count: out");
...
ra_err_t ra_usb_composite_get_device_descriptor(const uint8_t** out_desc)
{
  ...
  RA_CHECK_NULL_PTR(out_desc, s_tag, "get_device_descriptor: out");
...
ra_err_t ra_usb_composite_get_config_descriptor(const uint8_t** out_desc)
{
  ...
  RA_CHECK_NULL_PTR(out_desc, s_tag, "get_config_descriptor: out");
