 ...
 *
 * @details
 * Split out of `ra_cgc.c`. Brings up PLL2 (960 MHz VCO, PLL2P = 240 MHz on
 ...
 *
 * Every protected-register write is wrapped in `RA_PROTECTED_WRITE` so the
 ...
 * `ra_cgc_internal.h`; `ra_cgc_ensure_hoco_running_for_usb_ck` is defined
 * here and reused by `ra_cgc_eswclk.c`.
 *
 * @par Tag ring/world:
...
static ra_err_t internal_pll2_program_protected(uint32_t pll2ccr, uint16_t pll2ccr2)
{
  ...
  RA_PROTECTED_WRITE(k_ra_prcr_unlock_cgc)
  {
    ...
    *ra_sys_pll2cr()   = (uint8_t)k_ra_pll2cr_run;
    err                = ra_cgc_wait_oscsf_set(k_ra_oscsf_bit_pll2sf);
    if (err != k_ra_ok) {
  ...
  void) // NOLINT(readability-function-size,readability-function-cognitive-complexity)
{
  ...
  const ra_err_t osc_err = ra_cgc_wait_oscsf_set(k_ra_oscsf_bit_moscsf);
  if (osc_err != k_ra_ok) {
