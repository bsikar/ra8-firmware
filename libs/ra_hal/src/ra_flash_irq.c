 ...
 *
 * @details
 * Interrupt-handling and FSP-parity aspect of the ra_flash driver, split
 ...
 * live in ``ra_flash_internal.h``. Every register access carries a HUM
 * Ch 59 citation.
 *
...
static ra_err_t internal_validate_range(uintptr_t address, uint64_t total_len)
{
  ...
  if (!ra_flash_internal_window_allows(address, (uint32_t)total_len)) {
...
{
  RA_VALIDATE_INIT(s_flash_rt.initialized, s_flash_tag, "flash_erase before init");
  if (num_blocks == 0U) {
    return k_ra_err_invalid_arg;
  }
  ...
  RA_RETURN_ON_ERROR(v_err, s_flash_tag, "flash_erase: validate"); /* GCOVR_EXCL_BR_LINE */
...
ra_err_t ra_flash_write(uintptr_t address, const uint8_t* src, uint32_t len)
{
  ...
  RA_VALIDATE_INIT(s_flash_rt.initialized, s_flash_tag, "flash_write before init");
...
{
  RA_CHECK_NULL_PTR(out_blank, s_flash_tag, "out_blank must not be nullptr");
  if (len == 0U) {
    return k_ra_err_invalid_arg;
  }
