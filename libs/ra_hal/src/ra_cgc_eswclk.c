/**
 * @file ra_cgc_eswclk.c
 * @brief Clock Generation Circuit driver -- Ethernet-switch (ESWCLK) path.
 *
 * @details
 * Split out of `ra_cgc.c`. Powers on the ESWM peripheral domain and routes
 ...
 * HOCO at reset); this file reuses `ra_cgc_ensure_hoco_running_for_usb_ck`
 * defined in `ra_cgc_usb.c`. Every protected-register write is wrapped in
 * `RA_PROTECTED_WRITE` so the PRCR re-lock always happens.
 *
 * @par Tag ring/world:
 ...
 *
 * @pre HOCO is running (caller invoked ``ra_cgc_ensure_hoco_running_for_usb_ck``).
 * @pre Single-threaded init context.
