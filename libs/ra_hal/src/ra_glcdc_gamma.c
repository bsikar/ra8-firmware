 ...
 * shared OUT block at k_ra_glcdc_off_out_gamsw.
 *
 ...
 * k_ra_glcdc_gamma_lut_depth / 2 = 8 gives the register count.
 */
typedef enum : uint8_t {
  k_ra_glcdc_gam_reg_count = 8U, /**< LUT/AREA registers per channel (depth/2). */
  k_ra_glcdc_gam_pair_step = 2U, /**< Entries packed per register (2 per reg).  */
} ra_glcdc_gam_reg_count_t;
 ...
 * @since 0.1.0
 */
static void internal_write_lut(volatile ra_glcdc_gam_t* gam, const uint16_t* gain)
{
  for (uint8_t i = 0U; i < (uint8_t)k_ra_glcdc_gam_reg_count; i++) {
    const uint8_t  hi_idx = (uint8_t)(i * (uint8_t)k_ra_glcdc_gam_pair_step);
    const uint8_t  lo_idx = (uint8_t)(hi_idx + 1U);
    const uint32_t packed = ((uint32_t)gain[hi_idx] << k_ra_glcdc_gam_lut_gain_h_shift) |
                            ((uint32_t)gain[lo_idx] & (uint32_t)k_ra_glcdc_gam_lut_gain_l_mask);
 ...
 *
 * @param[in] gam       Pointer to the channel's gamma register block.
 * @param[in] threshold Array of k_ra_glcdc_gamma_lut_depth 10-bit threshold values.
 *
 * @pre gam != nullptr (caller guarantees).
 ...
 * @since 0.1.0
 */
static void internal_write_area(volatile ra_glcdc_gam_t* gam, const uint16_t* threshold)
{
  for (uint8_t i = 0U; i < (uint8_t)k_ra_glcdc_gam_reg_count; i++) {
    const uint8_t  hi_idx = (uint8_t)(i * (uint8_t)k_ra_glcdc_gam_pair_step);
    const uint8_t  lo_idx = (uint8_t)(hi_idx + 1U);
    const uint32_t packed = ((uint32_t)threshold[hi_idx] << k_ra_glcdc_gam_area_th_h_shift) |
                            ((uint32_t)threshold[lo_idx] & (uint32_t)k_ra_glcdc_gam_area_th_l_mask);
...
ra_err_t ra_glcdc_set_gamma(ra_glcdc_color_channel_t channel,
                            const uint16_t*          gain,
                            const uint16_t*          threshold,
                            uint8_t                  count)
{
  RA_CHECK_NULL_PTR(gain, s_tag, "gain must not be nullptr");
  RA_CHECK_NULL_PTR(threshold, s_tag, "threshold must not be nullptr");
  if ((uint8_t)channel >= (uint8_t)k_ra_glcdc_ch_count) {
    return k_ra_err_invalid_arg;
  }
  if (count != (uint8_t)k_ra_glcdc_gamma_lut_depth) {
    return k_ra_err_invalid_arg;
  }

  volatile ra_glcdc_gam_t* gam = ra_glcdc_gam_regs((uint8_t)channel);
  RA_CHECK_NULL_PTR(gam, s_tag, "gamma register block inaccessible");

  internal_write_lut(gam, gain);
  internal_write_area(gam, threshold);

  ra_log_info_val(s_tag, "set_gamma channel", (uint32_t)channel);
  return k_ra_ok;
}

ra_err_t ra_glcdc_gamma_enable(bool enable)
{
  volatile uint32_t* const reg = ra_glcdc_reg32(k_ra_glcdc_off_out_gamsw);
  RA_CHECK_NULL_PTR(reg, s_tag, "GAMSW register inaccessible");

  /* HUM Ch 63 "OUT_GAMSW Gamma Correction Block Switch Register" p 3789 */
  if (enable) {
    *reg = (uint32_t)k_ra_glcdc_gamsw_gamon;
  } else {
    *reg = 0U;
  }

  ra_log_info_val(s_tag, "gamma_enable", enable ? 1UL : 0UL);
  return k_ra_ok;
}
