 ...
 * that ``ra_mipi_phy.c`` delegates to so that neither file exceeds the
 ...
 *  - ``ra_mipi_phy_select_timing``, the public table-driven entry point
 *    that selects a row and programs DPHYTIM1..6.
 *  - ``internal_mipi_phy_compute_freq``, the HUM 64.2.2 PLL-frequency
 *    arithmetic helper consumed by ``ra_mipi_phy_validate_pll_band`` in
 *    ``ra_mipi_phy.c``.
 *
 * The register-write path used by ``ra_mipi_phy_select_timing`` lives in
 * ``ra_mipi_phy.c`` (``internal_mipi_phy_write_timing``) and is reached
 * through ``ra_mipi_phy_internal.h``.
 *
...
#include "ra_mipi_phy_internal.h"
