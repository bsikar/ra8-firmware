 ...
 * of ``ra_rsip_asym.c`` to keep every translation unit under the file-size
 * budget. Exposes:
 *
 ...
 * ``k_ra_err_not_supported``. The real ECDSA-P256 / ECDH / Ed25519 backend is
 ...
 * ``internal_zero_handle_tail`` are defined in ``ra_rsip_asym.c`` and shared
 * via ``ra_rsip_asym_internal.h``; the remaining cross-TU primitives
...
ra_err_t ra_rsip_ecdsa_sign(const ra_rsip_key_handle_t* key,
                            ...
                            uint8_t*                    signature)
{
  RA_CHECK_NULL_PTR(key, s_tag, "ecdsa_sign: key must not be nullptr");
  RA_CHECK_NULL_PTR(signature, s_tag, "ecdsa_sign: signature must not be nullptr");
  (void)curve;
  (void)digest;
  (void)digest_len;
...
ra_err_t ra_rsip_ecdsa_verify(const ra_rsip_key_handle_t* key,
                              ...
                              const uint8_t*              signature)
{
  RA_CHECK_NULL_PTR(key, s_tag, "ecdsa_verify: key must not be nullptr");
  RA_CHECK_NULL_PTR(signature, s_tag, "ecdsa_verify: signature must not be nullptr");
  (void)curve;
  (void)digest;
  (void)digest_len;
  return k_ra_err_not_supported;
...
ra_err_t ra_rsip_ecdh_compute(const ra_rsip_key_handle_t* key,
                              ...
                              ra_rsip_key_handle_t*       out)
{
  RA_CHECK_NULL_PTR(key, s_tag, "ecdh_compute: key must not be nullptr");
  RA_CHECK_NULL_PTR(out, s_tag, "ecdh_compute: out must not be nullptr");
  (void)curve;
  (void)peer_x;
  (void)peer_y;
  return k_ra_err_not_supported;
