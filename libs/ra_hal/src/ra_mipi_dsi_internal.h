 ...
 * sequence-channel path lives in ``ra_mipi_dsi.c``; the status, IRQ
 * dispatch, video-mode, and convenience surfaces live in
 * ``ra_mipi_dsi_dispatch.c``. A handful of file-scope mutable state
 ...
 * Shared between ``ra_mipi_dsi.c`` and ``ra_mipi_dsi_dispatch.c``.
 */
 ...
 *
 * @details
 * Shared between ``ra_mipi_dsi.c`` and ``ra_mipi_dsi_dispatch.c`` so
 ...
 *
 * @details
 * Single definition lives in ``ra_mipi_dsi.c``. Lifetime is owned by
 * the caller of ``ra_mipi_dsi_attach_handler``.
 *
 ...
 * @brief Buffer the caller passed to ``ra_mipi_dsi_read_packet()``.
 *
 * @details
 * Single definition lives in ``ra_mipi_dsi.c``. The receive ISR in
 * ``ra_mipi_dsi_dispatch.c`` copies long-packet payload bytes into this
 ...
 *
 * @details
 * Single definition lives in ``ra_mipi_dsi.c``. Cleared to zero once the
 ...
 *
 * @details
 * Defined in ``ra_mipi_dsi.c`` and shared with the video-mode path in
 * ``ra_mipi_dsi_dispatch.c``. Spins at most
 * ``k_ra_mipi_dsi_busy_loop_max`` iterations so the loop bound is
 ...
 *
 * @return `k_ra_ok` if the condition was met within the iteration cap,
 *         otherwise `k_ra_err_hw_timeout`.
 * @retval k_ra_ok Condition satisfied before the iteration cap.
 * @retval k_ra_err_hw_timeout Cap reached without the condition holding.
 *
 ...
 * @since 0.1.0
 */
ra_err_t ra_mipi_dsi_internal_wait_eq(volatile const uint32_t* reg, uint32_t mask, uint32_t expect);
