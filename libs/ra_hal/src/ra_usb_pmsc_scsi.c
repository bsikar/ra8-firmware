 ...
 * dispatcher and lives in `ra_usb_pmsc_internal.h` as
...
ra_err_t internal_handle_inquiry(uint8_t* data_buf, uint32_t capacity, uint32_t* out_len)
{
  ...
  const ra_err_t err = s_usb_pmsc_state.storage.get_inquiry(s_usb_pmsc_state.storage.ctx,
...
ra_err_t internal_handle_read10(uint8_t* data_buf, uint32_t capacity, uint32_t* out_len)
{
  ...
  uint32_t       block_size         = 0U;
  const ra_err_t cap_err = s_usb_pmsc_state.storage.get_capacity(s_usb_pmsc_state.storage.ctx,
                                                                 &device_block_count,
                                                                 &block_size);
  if (cap_err != k_ra_ok) {
    return cap_err;
