 ...
 * split out of ``ra_ceu.c`` to keep both translation units under the
 * file-size cap. This TU owns the ``ra_ceu_config_t``-to-register
 * packing (CAMCR / CAPCR / CAIFR / CFLCR / CFSZR / CDOCR) and the
 * three ``ra_ceu_program_*`` phases that ``ra_ceu_init`` drives. The
 * runtime-control entry points (capture start/stop, plane-B shadow,
 * status, dispatch) remain in ``ra_ceu.c``. Register sequences are
