 ...
 *
 * @details
 * Implements ``ra_sci_spi.h`` against the RA8D2 SCI_B peripheral in
 ...
 */
static const ra_mstp_t s_mstp_table[k_ra_sci_spi_channel_count] = {
  k_ra_mstp_sci0,
  k_ra_mstp_sci1,
  k_ra_mstp_sci2,
  k_ra_mstp_sci3,
...
{
  for (uint32_t cks = 0U; cks <= (uint32_t)k_ra_sci_spi_cks_max; cks++) {
    const uint32_t divisor = (uint32_t)k_ra_sci_spi_div_base << (2U * cks);
    ...
    if ((q - 1U) <= (uint32_t)k_ra_sci_spi_brr_max) {
      *out_cks = cks;
      *out_brr = q - 1U;
      return;
    }
  ...
  *out_cks = (uint32_t)k_ra_sci_spi_cks_max;
  *out_brr = (uint32_t)k_ra_sci_spi_brr_max;
}
...
static void internal_apply_rate(volatile r_sci_regs_t* reg, uint32_t baud_hz, uint32_t pclk_hz)
{
  ...
  uint32_t ccr2 =
    (brr << (uint32_t)k_ra_sci_ccr2_shift_brr) & (uint32_t)k_ra_sci_ccr2_mask_brr_field;
  ccr2 |= (cks << (uint32_t)k_ra_sci_ccr2_shift_cks) & (uint32_t)k_ra_sci_ccr2_mask_cks_field;
  ccr2 |= ((uint32_t)k_ra_sci_mddr_default << (uint32_t)k_ra_sci_ccr2_shift_mddr);
  reg->CCR2 = ccr2;
}
 ...
 * @pre ``cfg`` is non-NULL.
 * @pre ``cfg->mode`` is one of ::ra_spi_mode_t (0..3).
 * @post No register is written; the value is returned to the caller.
 * @post ``cfg`` is unmodified.
 * @note Not thread-safe.
 * @since 0.1.0
 */
static uint32_t internal_ccr3(const ra_sci_spi_cfg_t* cfg)
{
  uint32_t ccr3 = ((uint32_t)k_ra_sci_ccr3_mod_simple_spi << (uint32_t)k_ra_sci_ccr3_shift_mod);
  ccr3 |= ((uint32_t)k_ra_sci_ccr3_chr_8bit << (uint32_t)k_ra_sci_ccr3_shift_chr);
  /* ra_spi_mode_t is 0..3 where bit 1 = CPOL and bit 0 = CPHA. */
  if (((uint32_t)cfg->mode & (uint32_t)k_ra_spi_mode_2) != 0U) {
    ccr3 |= (1U << (uint32_t)k_ra_sci_ccr3_bit_cpol);
  }
  if (((uint32_t)cfg->mode & (uint32_t)k_ra_spi_mode_1) != 0U) {
    ccr3 |= (1U << (uint32_t)k_ra_sci_ccr3_bit_cpha);
  }
  if (cfg->lsb_first) {
    ccr3 |= (1U << (uint32_t)k_ra_sci_ccr3_bit_lsbf);
  }
  return ccr3;
}

ra_err_t ra_sci_spi_init(uint8_t channel, const ra_sci_spi_cfg_t* cfg)
{
  RA_CHECK_NULL_PTR(cfg, s_tag, "spi_init: cfg");
  if (channel >= (uint8_t)k_ra_sci_spi_channel_count) {
  ...
  reg->CFCLR = (uint32_t)k_ra_sci_cfclr_default;
  /* HUM Ch 38.2.5 "CCR0 : Common Control Register 0" p 2182 */
  reg->CCR0 = (1U << (uint32_t)k_ra_sci_ccr0_bit_te) | (1U << (uint32_t)k_ra_sci_ccr0_bit_re);
  return k_ra_ok;
}

ra_err_t ra_sci_spi_deinit(uint8_t channel)
{
  if (channel >= (uint8_t)k_ra_sci_spi_channel_count) {
    return k_ra_err_invalid_arg;
  }
...
ra_err_t ra_sci_spi_set_clock(uint8_t channel, uint32_t baud_hz, uint32_t pclk_hz)
{
  if (channel >= (uint8_t)k_ra_sci_spi_channel_count) {
    return k_ra_err_invalid_arg;
  }
...
ra_err_t ra_sci_spi_xfer8(uint8_t channel, uint8_t tx, uint8_t* rx)
{
  volatile r_sci_regs_t* reg = ra_sci(channel);
  RA_CHECK_NULL_PTR(reg, s_tag, "spi_xfer8: channel out of range");

  /* HUM Ch 38.2.17 "CSR : Common Status Register" p 2225 */
  const uint32_t tdre = (1U << (uint32_t)k_ra_sci_csr_bit_tdre);
  ra_err_t       err  = ra_hw_wait_flag_set32(&reg->CSR, tdre, k_ra_hw_budget_long);
  if (err != k_ra_ok) {
    return err;
  }
  ...
  /* HUM Ch 38.2.17 "CSR : Common Status Register" p 2225 */
  const uint32_t rdrf = (1U << (uint32_t)k_ra_sci_csr_bit_rdrf);
  err                 = ra_hw_wait_flag_set32(&reg->CSR, rdrf, k_ra_hw_budget_long);
  if (err != k_ra_ok) {
    return err;
  }
  /* HUM Ch 38.2.2 "RDR : Receive Data Register" p 2180 */
  const uint8_t received = (uint8_t)(reg->RDR & (uint32_t)k_ra_sci_rdr_mask_data8);
  /* HUM Ch 38.2.24 "CFCLR : Common Flag Clear Register" p 2238 */
  reg->CFCLR = (1U << (uint32_t)k_ra_sci_cfclr_bit_rdrfc);
...
ra_err_t ra_sci_spi_xfer(uint8_t channel, const uint8_t* tx, uint8_t* rx, uint32_t len)
{
  if (channel >= (uint8_t)k_ra_sci_spi_channel_count) {
    return k_ra_err_invalid_arg;
  }
  for (uint32_t i = 0U; i < len; i++) {
    const uint8_t  out = (tx != nullptr) ? tx[i] : (uint8_t)k_ra_sci_spi_idle_byte;
    uint8_t        in  = 0U;
    const ra_err_t err = ra_sci_spi_xfer8(channel, out, &in);
    if (err != k_ra_ok) {
      return err;
    }
    if (rx != nullptr) {
