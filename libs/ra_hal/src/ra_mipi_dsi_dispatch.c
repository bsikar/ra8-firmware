 ...
 * ``ra_mipi_dsi.c``; this file carries:
 *
 ...
 * as every other ra_hal driver (see `ra_glcdc.c`, `ra_doc.c`). The
 * sibling ``ra_mipi_dsi.c`` keeps its own identical copy -- read-only
...
[[nodiscard]] ra_err_t ra_mipi_dsi_video_start(const ra_mipi_dsi_video_cfg_t* vcfg)
{
  ...
  return ra_mipi_dsi_internal_wait_eq(&reg->VMSR,
...
[[nodiscard]] ra_err_t ra_mipi_dsi_video_stop(void)
{
  ...
  /* HUM Ch 65.2 "VMSR : Video Mode Status Register", p 3893 */
  const ra_err_t err =
    ra_mipi_dsi_internal_wait_eq(&reg->VMSR, k_ra_mipi_dsi_vmsr_stop, k_ra_mipi_dsi_vmsr_stop);
  if (err == k_ra_ok) {
...
{
  const ra_mipi_dsi_event_fn_t fn  = s_mipi_dsi_event_fn;
...
void ra_mipi_dsi_dispatch_receive(void)
{
  ...
  if ((bits & k_ra_mipi_dsi_rxsr_rxresp) != 0U) {
    // mcdc-deactivated: ra_mipi_dsi_dispatch_receive pending-RX gate; s_mipi_dsi_pending_rx_buffer and s_mipi_dsi_pending_rx_len are written together (atomic pair) by ra_mipi_dsi_rx_payload_register; the buffer is never set without a non-zero length and vice-versa, so the conditions are co-dependent on any reachable path.
    if ((s_mipi_dsi_pending_rx_buffer != nullptr) && (s_mipi_dsi_pending_rx_len > 0U)) {
      uint16_t got = 0U;
      (void)ra_mipi_dsi_rx_payload_read(s_mipi_dsi_pending_rx_buffer,
