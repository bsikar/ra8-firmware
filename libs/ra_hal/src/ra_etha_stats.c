 ...
 * sit alongside the lifecycle / IRQ / traffic-shaping path in ra_etha.c:
 *  - Descriptor-ring sizing (::ra_etha_descriptor_ring_init), which clamps
 *    every per-class TX queue depth to the host ring depth;
 *  - Per-port software traffic counters (::ra_etha_get_stats,
 *    ::ra_etha_account_traffic) with saturating accumulation;
 *  - PHY bring-up (::ra_etha_open), driving ETHA into OPERATION and then
 *    sequencing the RMAC PHY auto-negotiation handshake.
 *
 * The shared logger tag, per-port slot table, and port range-check
 * predicate live in ra_etha_internal.h and are defined in ra_etha.c.
 *
