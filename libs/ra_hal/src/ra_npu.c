 ...
 *
 * @details
 * Implementation of the `ra_npu.h` contract. The whole translation unit is
 * gated behind `RA_HAS_NPU` so it compiles to nothing on the RA8D2 (which has
 * no NPU) and is only built for the RA8P1 (`-DRA_DEVICE_RA8P1`). Every MMIO
 * access carries an Arm Ethos-U55 NPU TRM citation; the RA8P1 integration facts
 * (module-stop bit, clock, IRQ) are sourced in `ra_npu_regs.h`.
 *
...
/**
 * @enum ra_npu_mstp_enc_t
 * @brief Packing of the NPU module-stop identifier for `ra_mstp_enable()`.
 *
 * @details `ra_mstp_t` packs a peripheral as `(register_index << 8) | bit`. The
 *          RA8P1 NPU module-stop is MSTPCRA (`k_ra_mstp_reg_a`) bit 16 -- a
 *          confirmed RA8P1 constant (see `ra_npu_regs.h`). It is not a named
 *          member of the shared RA8D2 `ra_mstp_t` enum (the RA8D2 has no NPU), so
...
typedef enum : uint16_t {
  ...
  k_ra_npu_mstp_bit       = 16U, /**< NPU module-stop = MSTPCRA bit 16.    */
} ra_npu_mstp_enc_t;

/**
 * @enum ra_npu_budget_t
...
typedef enum : uint32_t {
  ...
  k_ra_npu_job_poll_iters   = 2000000U, /**< Job-completion poll budget. */
} ra_npu_budget_t;
...
typedef enum : uint32_t {
  k_ra_npu_status_fault_mask = ((uint32_t)1U << k_ra_npu_status_bus_error_bit) |
                               ((uint32_t)1U << k_ra_npu_status_cmd_parse_bit) |
                               ((uint32_t)1U << k_ra_npu_status_wd_fault_bit) |
                               ((uint32_t)1U << k_ra_npu_status_ecc_fault_bit),
} ra_npu_mask_t;
 ...
 * @retval k_ra_ok Reset completed (STATUS.reset read back 0).
 * @retval k_ra_err_hw_timeout Reset did not clear within the spin budget.
 *
...
static ra_err_t internal_npu_apply_reset(void)
{
  ...
  *ra_npu_reg(k_ra_npu_off_reset) = 0U;
  for (uint32_t i = 0U; i < k_ra_npu_reset_poll_iters; i++) { /* GCOVR_EXCL_BR_LINE */
    /* Ethos-U55 NPU TRM "NPU_STATUS" reg @ 0x04 -- reset bit clears when done. */
    const uint32_t status = *ra_npu_reg(k_ra_npu_off_status);
    if ((status & ((uint32_t)1U << k_ra_npu_status_reset_bit)) == 0U) { /* GCOVR_EXCL_BR_LINE */
      return k_ra_ok;
    }
  }
 ...
 *
 * @param[in] idx  Region index (0 .. `k_ra_npu_region_count` - 1).
...
static void internal_npu_write_region(ra_npu_region_idx_t idx, uint64_t base)
{
  ...
  *ra_npu_reg(lo_off) = (uint32_t)base;
  *ra_npu_reg(hi_off) = (uint32_t)(base >> k_ra_npu_addr_hi_shift);
}

ra_err_t ra_npu_init(void)
{
  const ra_err_t mst = ra_mstp_enable(s_npu_mstp_id);
  RA_RETURN_ON_ERROR(mst, s_tag, "npu_init: mstp enable"); /* GCOVR_EXCL_BR_LINE */
  const ra_err_t rst = internal_npu_apply_reset();
  RA_RETURN_ON_ERROR(rst, s_tag, "npu_init: reset"); /* GCOVR_EXCL_BR_LINE */
  s_npu_initialized   = true;
  s_npu_job_submitted = false;
  ra_log_info(s_tag, "npu_init");
  return k_ra_ok;
}

ra_err_t ra_npu_deinit(void)
{
  RA_VALIDATE_INIT(s_npu_initialized, s_tag, "npu_deinit: not initialized");
  const ra_err_t dis = ra_mstp_disable(s_npu_mstp_id);
  RA_RETURN_ON_ERROR(dis, s_tag, "npu_deinit: mstp disable"); /* GCOVR_EXCL_BR_LINE */
  s_npu_initialized   = false;
  s_npu_job_submitted = false;
  return k_ra_ok;
}

ra_err_t ra_npu_reset(void)
{
  RA_VALIDATE_INIT(s_npu_initialized, s_tag, "npu_reset: not initialized");
  const ra_err_t rst = internal_npu_apply_reset();
  RA_RETURN_ON_ERROR(rst, s_tag, "npu_reset"); /* GCOVR_EXCL_BR_LINE */
  s_npu_job_submitted = false;
  return k_ra_ok;
}

ra_err_t ra_npu_read_id(uint32_t* out_id)
{
  RA_CHECK_NULL_PTR(out_id, s_tag, "out_id must not be nullptr");
  RA_VALIDATE_INIT(s_npu_initialized, s_tag, "npu_read_id: not initialized");
  /* Ethos-U55 NPU TRM "NPU_ID" reg @ 0x00 -- architecture / product id. */
  *out_id = *ra_npu_reg(k_ra_npu_off_id);
  return k_ra_ok;
}

ra_err_t ra_npu_submit(const ra_npu_job_t* job)
{
  RA_CHECK_NULL_PTR(job, s_tag, "job must not be nullptr");
  RA_CHECK_NULL_PTR(job->cmd_stream, s_tag, "job->cmd_stream must not be nullptr");
  RA_VALIDATE_INIT(s_npu_initialized, s_tag, "npu_submit: not initialized");
  if (job->cmd_stream_bytes == 0U) {
    ra_log_error(s_tag, "npu_submit: empty command stream");
    return k_ra_err_invalid_size;
  }
  if (job->region_count > (uint8_t)k_ra_npu_region_count) {
    ra_log_error(s_tag, "npu_submit: too many regions");
    return k_ra_err_invalid_arg;
  }
  const uint64_t qbase = (uint64_t)(uintptr_t)job->cmd_stream;
  /* Ethos-U55 NPU TRM "QBASE" regs @ 0x10 / 0x14 -- command-stream address. */
  *ra_npu_reg(k_ra_npu_off_qbase_lo) = (uint32_t)qbase;
  *ra_npu_reg(k_ra_npu_off_qbase_hi) = (uint32_t)(qbase >> k_ra_npu_addr_hi_shift);
  /* Ethos-U55 NPU TRM "QSIZE" reg @ 0x20 -- command-stream length in bytes. */
  *ra_npu_reg(k_ra_npu_off_qsize) = job->cmd_stream_bytes;
  for (uint8_t r = 0U; r < job->region_count; r++) {
    internal_npu_write_region((ra_npu_region_idx_t)r, job->region_base[r]);
  }
  s_npu_job_submitted = true;
  return k_ra_ok;
}

ra_err_t ra_npu_run(void)
{
  RA_VALIDATE_INIT(s_npu_initialized, s_tag, "npu_run: not initialized");
  if (!s_npu_job_submitted) {
    ra_log_error(s_tag, "npu_run: no job submitted");
    return k_ra_err_invalid_state;
  }
  /* Ethos-U55 NPU TRM "NPU_CMD" reg @ 0x08 -- transition_to_running_state. */
  *ra_npu_reg(k_ra_npu_off_cmd) = ((uint32_t)1U << k_ra_npu_cmd_run_bit);
  return k_ra_ok;
}

ra_err_t ra_npu_read_status(ra_npu_status_t* out)
{
  RA_CHECK_NULL_PTR(out, s_tag, "out must not be nullptr");
  RA_VALIDATE_INIT(s_npu_initialized, s_tag, "npu_read_status: not initialized");
  /* Ethos-U55 NPU TRM "NPU_STATUS" reg @ 0x04 -- state + fault flags. */
  const uint32_t raw = *ra_npu_reg(k_ra_npu_off_status);
  out->raw           = raw;
  out->running       = (raw & ((uint32_t)1U << k_ra_npu_status_state_bit)) != 0U;
  out->irq_raised    = (raw & ((uint32_t)1U << k_ra_npu_status_irq_raised_bit)) != 0U;
  out->cmd_end       = (raw & ((uint32_t)1U << k_ra_npu_status_cmd_end_bit)) != 0U;
  out->fault         = (raw & (uint32_t)k_ra_npu_status_fault_mask) != 0U;
  return k_ra_ok;
}

ra_err_t ra_npu_poll(bool* out_done)
{
  RA_CHECK_NULL_PTR(out_done, s_tag, "out_done must not be nullptr");
  RA_VALIDATE_INIT(s_npu_initialized, s_tag, "npu_poll: not initialized");
  /* Ethos-U55 NPU TRM "NPU_STATUS" reg @ 0x04 -- fault + cmd-end flags. */
  const uint32_t raw = *ra_npu_reg(k_ra_npu_off_status);
  if ((raw & (uint32_t)k_ra_npu_status_fault_mask) != 0U) {
    ra_log_error(s_tag, "npu_poll: fault latched");
    return k_ra_err_hw_error;
  }
  *out_done = (raw & ((uint32_t)1U << k_ra_npu_status_cmd_end_bit)) != 0U;
  return k_ra_ok;
}

ra_err_t ra_npu_wait(void)
{
  RA_VALIDATE_INIT(s_npu_initialized, s_tag, "npu_wait: not initialized");
  for (uint32_t i = 0U; i < k_ra_npu_job_poll_iters; i++) { /* GCOVR_EXCL_BR_LINE */
    bool           done = false;
    const ra_err_t err  = ra_npu_poll(&done);
    RA_RETURN_ON_ERROR(err, s_tag, "npu_wait: poll"); /* GCOVR_EXCL_BR_LINE */
    if (done) {
      return k_ra_ok;
    }
  }
...
ra_err_t ra_npu_clear_irq(void)
{
  RA_VALIDATE_INIT(s_npu_initialized, s_tag, "npu_clear_irq: not initialized");
  /* Ethos-U55 NPU TRM "NPU_CMD" reg @ 0x08 -- write 1 to clear_irq. */
  *ra_npu_reg(k_ra_npu_off_cmd) = ((uint32_t)1U << k_ra_npu_cmd_clear_irq_bit);
  return k_ra_ok;
}

#else
/* RA8D2 (no NPU): this translation unit is intentionally empty. */
typedef int ra_npu_not_on_this_device_t;
#endif /* RA_HAS_NPU */
