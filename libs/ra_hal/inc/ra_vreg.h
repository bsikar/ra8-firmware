 ...
 * |--------|---------|---------------------------|---------------|
 * | Reset  | LDO     | ra_vreg_init              | always        |
 * | Reset  | DCDC    | ra_vreg_init              | OFS2.DCDCEN=1 |
 * | LDO    | DCDC    | ra_vreg_set_mode          | VCC >= 2.4 V  |
 * | DCDC   | LDO     | ra_vreg_set_mode          | always        |
 * | LDO    | Standby | ra_vreg_enter_standby     | always        |
 * | DCDC   | Standby | ra_vreg_enter_standby     | rejected      |
 * | DCDC   | DSBY    | ra_vreg_enter_standby     | rejected      |
 *
 * ## IRQ surface
 *
 ...
 *              `ra_vreg_set_mode(k_ra_vreg_mode_ldo)`.
 ...
 *              use `ra_vreg_set_lv_profile()` to enforce mutual
 *              exclusion.
 *
...
typedef enum : uint8_t {
  k_ra_vreg_mode_ldo  = 0U, /**< Low-dropout linear regulator (default at reset). */
  k_ra_vreg_mode_dcdc = 1U, /**< Switching regulator (DCDC).                      */
} ra_vreg_mode_t;

/**
 * @enum ra_vreg_lv_profile_t
 ...
 */
typedef enum : uint8_t {
  k_ra_vreg_lv_off = 0U, /**< Both LVO0E and LVO1E cleared (full-speed). */
  k_ra_vreg_lv_p0  = 1U, /**< LVO0E set, LVO1E cleared.                  */
  k_ra_vreg_lv_p1  = 2U, /**< LVO1E set, LVO0E cleared.                  */
} ra_vreg_lv_profile_t;

/**
 * @enum ra_vreg_standby_t
 * @brief Software Standby variant the caller intends to enter next.
 *
 * @details
 * Used by `ra_vreg_enter_standby()` to validate the regulator state.
 * HUM Ch 68.2.2 explicitly forbids DCDC mode for any of these
 * variants -- the driver returns `k_ra_err_invalid_state` if the
...
/**
 * @struct ra_vreg_cfg_t
 * @brief Configuration descriptor for `ra_vreg_init`.
 *
 * @details
 * cppcheck cannot see tests/ so it flags every field as unused; each
 * member is read in `ra_vreg_init` in `libs/ra_hal/src/ra_vreg.c`.
 */
...
/**
 * @struct ra_vreg_status_t
 * @brief Snapshot of the regulator state.
 *
 * @details
 * Decoded view returned by `ra_vreg_get_status()`. Both the raw
...
/**
 * @typedef ra_vreg_event_fn_t
 * @brief Voltage-regulator fault callback.
 *
 * @details
 * Invoked from `ra_vreg_dispatch()`. The driver does not own an IRQ
 * line on RA8D2 -- a higher layer (typically the LVD or a user fault
 * router) wires its ISR to call `ra_vreg_dispatch()` so this callback
 * can run.
 *
 * @param[in] ctx        Caller context recorded at attach time.
 * @param[in] status_word Snapshot of `DCDCCTL` at dispatch time.
 */
typedef void (*ra_vreg_event_fn_t)(void* ctx, uint8_t status_word);
 ...
 * `k_ra_vreg_mode_dcdc` runs the FSP-style four-step DCDC enable
 ...
 *  7. Cache the desired state for `ra_vreg_exit_standby()`.
 *
 ...
 * @retval k_ra_err_invalid_arg    `cfg->vccsel`, `cfg->ocp` or
 ...
 *
 * @see ra_vreg_set_mode
 * @see ra_vreg_set_vccsel
 *
 * @since 0.1.0
 */
 ...
 *
 * @param[in] sel One of `k_ra_vreg_vccsel_*`.
 *
 * @return ra_err_t error code.
 * @retval k_ra_ok Success.
 * @retval k_ra_err_invalid_arg `sel` is outside the 2-bit field.
 *
 * @pre Caller has unlocked PRCR group 1.
 ...
 * @return ra_err_t error code.
 * @retval k_ra_ok Success.
 * @retval k_ra_err_invalid_arg `level` outside the enum.
 *
 * @pre Caller has unlocked PRCR group 1.
 * @pre Regulator is in DCDC mode (or the call is staging for a
 *      subsequent LDO->DCDC switch).
 * @post DCDCCTL.OCPEN matches `level != k_ra_vreg_ocp_off`.
 *
 * @note Not thread-safe.
 *
 * @since 0.1.0
 ...
 * Callers that just want full-speed timing pass `k_ra_vreg_lv_off`.
 *
 * @param[in] profile One of `k_ra_vreg_lv_off / lv_p0 / lv_p1`.
 *
 * @return ra_err_t error code.
 * @retval k_ra_ok Success.
 * @retval k_ra_err_invalid_arg `profile` outside the enum.
 *
 * @pre Caller has unlocked PRCR group 1.
 ...
 * built from `k_ra_vreg_mask_*` values; the corresponding bits are
 * cleared in DCDCCTL while the rest are preserved. Any bits outside
 * `k_ra_vreg_mask_dcdcctl_all` are rejected with
 ...
 * @return ra_err_t error code.
 * @retval k_ra_ok               Success.
 * @retval k_ra_err_invalid_arg  `mask` includes reserved bits.
 *
 * @pre Caller has unlocked PRCR group 1.
 * @pre `mask` only contains bits from `k_ra_vreg_mask_dcdcctl_all`.
 ...
 *
 * @details
 * Equivalent to `ra_vreg_deinit()` followed by clearing the cached
 ...
 *
 * The actual WFI is the caller's responsibility (see `ra_lpm`).
 *
 * @param[in] variant Software Standby variant the caller intends to enter.
 *
 * @return ra_err_t error code.
 * @retval k_ra_ok                 Regulator parked.
 * @retval k_ra_err_invalid_arg    `variant` outside the enum.
 *
 * @pre Caller has unlocked PRCR group 1.
...
/**
 * @brief Legacy alias for `ra_vreg_enter_standby(k_ra_vreg_standby_software)`.
 *
 * @details
 ...
 * `ra_vreg_enter_standby`, this restores VCCSEL and re-enables DCDC
 ...
 *
 * @pre Caller has unlocked PRCR group 1.
 * @pre `ra_vreg_init` previously ran (so the desired state is cached).
...
/**
 * @brief Legacy alias for `ra_vreg_exit_standby()`.
 * @return ra_err_t -- forwards to `ra_vreg_exit_standby()`.
 * @since 0.1.0
 */
 ...
 *
 * @details
 * The callback is invoked synchronously from `ra_vreg_dispatch()`.
 ...
 * @post Subsequent `ra_vreg_dispatch` calls invoke `fn(ctx, ...)`.
 *
 * @note Not thread-safe.
 *
 * @since 0.1.0
