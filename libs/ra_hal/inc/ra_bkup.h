 ...
 *
 * - Lifecycle: ``ra_bkup_init`` / ``ra_bkup_deinit`` / cold + warm
 *   start helpers (``ra_bkup_cold_start_init``,
 *   ``ra_bkup_warm_start_check``, ``ra_bkup_no_switch_init``).
 * - Status decode: ``ra_bkup_get_status`` / ``ra_bkup_clear_status``.
 ...
 * - IRQ dispatch: ``ra_bkup_attach_handler`` + ``ra_bkup_dispatch``
...
/**
 * @enum ra_bkup_source_t
 * @brief Live power-source decoded from VBTBPSR.BPWSWM.
 */
typedef enum : uint8_t {
  k_ra_bkup_source_vbatt = 0U, /**< BPWSWM=0: VCC < VDETBATT, on VBATT. */
  k_ra_bkup_source_vcc   = 1U, /**< BPWSWM=1: VCC > VDETBATT, on VCC.   */
} ra_bkup_source_t;

/**
 * @enum ra_bkup_channel_t
 * @brief RTCICn tamper-detection channel index (HUM Ch 12.2.8 p 505).
 */
typedef enum : uint8_t {
  k_ra_bkup_chan_rtcic0 = 0U, /**< RTCIC0 -> VBTADF0 / VCH0*. */
  k_ra_bkup_chan_rtcic1 = 1U, /**< RTCIC1 -> VBTADF1 / VCH1*. */
  k_ra_bkup_chan_rtcic2 = 2U, /**< RTCIC2 -> VBTADF2 / VCH2*. */
} ra_bkup_channel_t;
 ...
 */
typedef enum : uint8_t {
  k_ra_bkup_nc_width_32768hz = 0U, /**< 000b: 32.768 kHz (sub-clock). */
  k_ra_bkup_nc_width_64hz    = 1U, /**< 001b: 64 Hz.                  */
  k_ra_bkup_nc_width_32hz    = 2U, /**< 010b: 32 Hz.                  */
  k_ra_bkup_nc_width_16hz    = 3U, /**< 011b: 16 Hz.                  */
  k_ra_bkup_nc_width_8hz     = 4U, /**< 100b:  8 Hz.                  */
  k_ra_bkup_nc_width_4hz     = 5U, /**< 101b:  4 Hz.                  */
  k_ra_bkup_nc_width_2hz     = 6U, /**< 110b:  2 Hz.                  */
  k_ra_bkup_nc_width_1hz     = 7U, /**< 111b:  1 Hz.                  */
} ra_bkup_nc_width_t;

/**
 * @enum ra_bkup_capture_src_t
 * @brief RTC time-capture event source select
 *        (HUM Ch 12.2.16 p 511, VBTADCR2.VBRTCESn).
 */
typedef enum : uint8_t {
  k_ra_bkup_capture_src_pin    = 0U, /**< 0 -> raw RTCICn pin. */
  k_ra_bkup_capture_src_vbtadf = 1U, /**< 1 -> VBTADFn flag.   */
} ra_bkup_capture_src_t;
...
/**
 * @struct ra_bkup_config_t
 * @brief Configuration descriptor passed to ``ra_bkup_init``.
 *
 * @details
 * cppcheck cannot see tests/ so it flags every field as unused; each
 * member is read in ``ra_bkup_init`` in
 * ``libs/ra_hal/src/ra_bkup.c``.
 */
/* cppcheck-suppress-begin [unusedStructMember] */
typedef struct {
  ra_bkup_vdet_level_t vdet_level;    /**< VDETBATT trip threshold (VBTBPCR2.VDETLVL).         */
...
typedef struct {
  ...
  ra_bkup_edge_t        edge;               /**< VCHnEG: falling vs rising trigger edge.    */
  ...
  ra_bkup_capture_src_t capture_src;        /**< VBRTCESn: RTC time-capture source select.  */
} ra_bkup_tamper_chan_cfg_t;
/* cppcheck-suppress-end [unusedStructMember] */

/**
 * @struct ra_bkup_tamper_config_t
 * @brief Whole-block tamper-detection configuration.
 *
 * @details
 * Aggregates the noise-canceller width and per-channel descriptors so
 * one call to ``ra_bkup_tamper_init`` covers HUM Ch 12.3.7.4
 * "Tamper Detection Function Initialization Setting Flow" steps 1-8.
 * cppcheck cannot see tests/ so it flags every field as unused.
 */
/* cppcheck-suppress-begin [unusedStructMember] */
typedef struct {
  ra_bkup_nc_width_t        nc_width; /**< VINCW noise-sampling clock. */
  ra_bkup_tamper_chan_cfg_t channels[(uint8_t)k_ra_bkup_chan_count];
} ra_bkup_tamper_config_t;
/* cppcheck-suppress-end [unusedStructMember] */

/**
 * @struct ra_bkup_security_config_t
 * @brief BBFSAR + backup-register boundary descriptor.
 *
 * @details
 * Drives ``ra_bkup_security_apply``. ``saba``, ``pabas`` and
 ...
 */
typedef void (*ra_bkup_event_fn_t)(void* ctx, uint8_t tamper_flags);
 ...
 * @startuml
 * [*] --> Off
 * Off    --> Armed     : ra_bkup_init(cfg)
 * Armed  --> Off       : ra_bkup_deinit()
 * @enduml
 *
 * @param[in] cfg Non-NULL configuration descriptor.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok             Hardware armed; backup window open.
 * @retval k_ra_err_null_ptr   ``cfg == nullptr``.
 * @retval k_ra_err_invalid_arg ``cfg->vdet_level`` is one of the two
 ...
 * ``ra_bkup_clear_status`` to also wipe them.
 *
 * @return ``ra_err_t`` error code.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Switch armed.
 * @retval k_ra_err_invalid_arg ``level`` out of range.
 * @retval k_ra_err_hw_timeout  VBPORM stayed 0 for the whole poll loop.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok            Status decoded into ``*needs_reinit``.
 * @retval k_ra_err_null_ptr  ``needs_reinit == nullptr``.
 * @retval k_ra_err_hw_timeout VBPORM stayed 0.
 *
 * @pre PRCR unlocked.
 * @pre ``timeout_iters > 0``.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              All registers reset, switch stopped.
 * @retval k_ra_err_hw_timeout  VBPORM never dropped.
 *
 * @pre PRCR unlocked, IRQs masked.
 * @pre ``timeout_iters > 0``.
 * @post VBTBPCR1.BPWSWSTP == 1.
 ...
 * bit mask they observed via ``ra_bkup_get_status``.
 *
 * @param[in] mask  OR of ``k_ra_bkup_vbtbpsr_mask_vbporf`` and
 *                  ``k_ra_bkup_vbtadsr_mask_*`` values.
 *
 * @return ``ra_err_t`` error code.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok             Word read into ``*out``.
 * @retval k_ra_err_null_ptr   ``out == nullptr``.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Hardware programmed.
 * @retval k_ra_err_null_ptr    ``cfg == nullptr``.
 * @retval k_ra_err_invalid_arg ``cfg->nc_width`` out of range.
 *
 * @pre PRCR unlocked, IRQs masked.
 ...
 *
 * @details
 * Counterpart to ``ra_bkup_tamper_init``: zeroes VBTICTLR /
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok Hardware disarmed.
 *
 * @pre PRCR unlocked, IRQs masked.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              VBTICTLR updated.
 * @retval k_ra_err_invalid_arg ``channel >= 3``.
 *
 * @pre PRCR unlocked.
 ...
 * @retval k_ra_err_null_ptr ``enabled_out == nullptr``.
 *
 ...
 * within ``k_ra_bkup_saba_max``, then writes the registers in the
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              All four registers updated.
 * @retval k_ra_err_null_ptr    ``cfg == nullptr``.
 * @retval k_ra_err_invalid_arg An address is unaligned or > saba_max,
 ...
 * call ``ra_bkup_dispatch`` with the live VBTADSR mask.
 *
