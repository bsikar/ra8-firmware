 ...
 * ::ra_tscr_bit_t. ``k_ra_tscr_mask_all`` covers every documented
...
typedef enum : uint16_t {
  k_ra_tscdr_data_mask = 0x0FFFU, /**< 12-bit data mask (HUM Ch 55.2.2 p 3498). */
} ra_tscdr_field_t;
 ...
 * the lower 12 bits (``k_ra_tscdr_data_mask``) carry the calibration
 ...
 */
static inline volatile const r_tsn_cal_regs_t* ra_tsn_cal(void)
{
  return (volatile const r_tsn_cal_regs_t*)k_ra_tsn_cal_base_addr;
}
 ...
 *
 *  - ``k_ra_tsn_adc_avcc_uv``    -- AVCC0 / VREFH0 in microvolts.
 ...
 *  - ``k_ra_tsn_adc_full_scale`` -- 12-bit ADC full-scale (4096).
 ...
 *  - ``k_ra_tsn_uv_per_mv``      -- 1000 scale factor that promotes
 *    a degC value into milli-degC. Naming reflects the equivalent
 *    microvolt-per-millivolt ratio so the conversion math stays
 *    self-documenting.
 */
typedef enum : int32_t {
  k_ra_tsn_adc_avcc_uv    = 3300000, /**< AVCC = 3.3 V expressed in uV. */
  k_ra_tsn_adc_full_scale = 4096,    /**< 12-bit ADC full-scale.        */
  k_ra_tsn_uv_per_mv      = 1000,    /**< 1000-x scale (degC -> mC).    */
} ra_tsn_adc_const_t;
