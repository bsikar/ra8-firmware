 ...
 * and SCI in Simple-SPI mode (`ra_sci_spi.h`). A device driver at
 * [Ring 3 / HAL] (e.g. the IT8951 e-paper driver in `ra_epaper.h`) must
 ...
 * seam in its config instead of naming `ra_spi_*` or `ra_sci_spi_*`
 * directly.
 *
 * This seam deliberately lives at Ring 3, NOT in the `ra_io` fabric: the
 * peripheral-agnostic bus facade `ra_io_spi_bus.h` sits at [Ring 4 / PAL],
 ...
 * `ra_io_spi_bus_as_ops()`, which fills this struct with trampolines into
 * a bound ::ra_io_spi_bus_t; the app owns the bus, binds a backend, and
 ...
 * (the Ring-4 `ra_io_spi_bus_as_ops()` points it at the bound
 * ::ra_io_spi_bus_t handle, which must therefore out-live the driver).
 ...
 * @code
 * ra_io_spi_bus_t bus = {};
 * (void)ra_io_spi_bus_bind_spi_b(&bus, 0U);
 * ra_spi_bus_ops_t ops = {};
 * (void)ra_io_spi_bus_as_ops(&bus, &ops);
 * ra_epaper_cfg_t cfg = { .bus = ops, ... };
 * @endcode
 *
 * @see ra_io_spi_bus_as_ops  Ring-4 binder over the `ra_io` SPI facade.
 * @see ra_epaper_cfg_t       First Ring-3 consumer of this seam.
 *
 * @since 0.1.0
 */
...
typedef struct {
   ...
   * `ra_spi_xfer8` / `ra_sci_spi_xfer8` contract byte for byte.
   *
   ...
   */
  ra_err_t (*xfer8)(void* ctx, uint8_t tx, uint8_t* rx);
