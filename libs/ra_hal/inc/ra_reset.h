 ...
 * | API                              | Purpose                            |
 * |----------------------------------|------------------------------------|
 * | ``ra_reset_init``                | Snapshot reset cause at boot        |
 * | ``ra_reset_get_cause``           | Decode RSTSR0/1/2/3 -> typed cause  |
 * | ``ra_reset_get_raw``             | Return the raw 8/32-bit register words |
 ...
 * no PRCR unlock is required for ``ra_reset_get_attribution``.
 *
 ...
 * wins; ``k_ra_reset_cause_unknown`` is returned when no flag is set
...
/**
 * @struct ra_reset_raw_t
 * @brief Raw register snapshot returned by ``ra_reset_get_raw``.
 *
 * @details
 * cppcheck cannot see tests/ so it flags every member as unused; each
 * field is read by the test suite in ``tests/test_ra_reset.c`` and by
 ...
 * a file-scope static. Subsequent calls to ``ra_reset_get_cause`` /
 * ``ra_reset_get_raw`` return the snapshot rather than re-reading the
 * registers. This is the only safe way to share the cause across the
 * firmware -- any other code path that calls
 * ``ra_reset_clear_cause`` would otherwise destroy the information
 * before later modules could see it.
 *
 * Idempotent: calling ``ra_reset_init`` more than once re-reads the
 ...
 * @retval k_ra_ok Snapshot captured.
 *
 * @pre None -- can be called as the very first init step.
 * @pre PRCR does not need to be unlocked for RSTSRn reads.
 * @post ``ra_reset_get_cause`` will return the cached value.
 * @post ``ra_reset_get_raw`` will return the cached raw register snapshot.
 *
 ...
 *
 * @details
 * If ``ra_reset_init`` was called this returns the cached snapshot;
 ...
 *
 * @details
 * If ``ra_reset_init`` has run this returns the cached snapshot;
 * otherwise the registers are read fresh.
 *
 * @param[out] out  Destination struct (non-NULL).
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok            Raw values written.
 * @retval k_ra_err_null_ptr  ``out`` was nullptr.
 *
 * @pre ``out != nullptr``.
 ...
 *
 * @pre None.
 * @pre Caller has already inspected the cause via ``ra_reset_get_cause``
 ...
 * ``k_ra_reset_rstsar_field_msk`` to drop reserved upper bits.
 *
 ...
 * @post On host:   ``*ra_reset_aircr() & SYSRESETREQ_msk`` reads 1.
 *
