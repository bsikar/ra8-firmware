 ...
 * bits so `ra_npu.c` can drive an inference job out of shared system SRAM.
 *
 ...
 *          `k_ra_npu_off_basep0_lo` and `ra_npu_region_idx_t`) supply the AXI
...
typedef enum : uint32_t {
  ...
  k_ra_npu_off_status    = 0x0004U, /**< NPU_STATUS (state + fault flags, RO).   */
  k_ra_npu_off_cmd       = 0x0008U, /**< NPU_CMD (run / clear-irq / q-enables).  */
  k_ra_npu_off_reset     = 0x000CU, /**< NPU_RESET (software reset request).     */
  k_ra_npu_off_qbase_lo  = 0x0010U, /**< QBASE bits [31:0] (cmd-stream address). */
  k_ra_npu_off_qbase_hi  = 0x0014U, /**< QBASE bits [63:32].                     */
  k_ra_npu_off_qread     = 0x0018U, /**< QREAD (cmd-stream read offset, RO).     */
  k_ra_npu_off_qconfig   = 0x001CU, /**< QCONFIG (cmd-stream AXI region config). */
  k_ra_npu_off_qsize     = 0x0020U, /**< QSIZE (cmd-stream length in bytes).     */
  k_ra_npu_off_prot      = 0x0024U, /**< NPU_PROT (privilege/security, RO).      */
  k_ra_npu_off_config    = 0x0028U, /**< NPU_CONFIG (build-time macs/rev, RO).   */
  k_ra_npu_off_regioncfg = 0x003CU, /**< REGIONCFG (per-region AXI attributes).  */
  k_ra_npu_off_basep0_lo = 0x0080U, /**< BASEP0 bits [31:0] (region 0 base).     */
} ra_npu_layout_t;

/**
 * @enum ra_npu_region_idx_t
 ...
 *          `k_ra_npu_off_basep0_lo + n * k_ra_npu_basep_stride_bytes`, and its
 *          high word `k_ra_npu_reg_hi_offset` bytes above that. Region 0 is the
...
typedef enum : uint8_t {
  ...
  k_ra_npu_region_1     = 1U, /**< AXI region 1.                                */
  k_ra_npu_region_2     = 2U, /**< AXI region 2.                                */
  k_ra_npu_region_3     = 3U, /**< AXI region 3.                                */
  k_ra_npu_region_4     = 4U, /**< AXI region 4.                                */
  k_ra_npu_region_5     = 5U, /**< AXI region 5.                                */
  k_ra_npu_region_6     = 6U, /**< AXI region 6.                                */
  k_ra_npu_region_7     = 7U, /**< AXI region 7.                                */
  k_ra_npu_region_count = 8U, /**< Number of BASEPn region base-pointer pairs.  */
} ra_npu_region_idx_t;
 ...
 *          32-bit words: a low word then a high word `k_ra_npu_reg_hi_offset`
 *          bytes above it. Consecutive BASEPn pairs are
 *          `k_ra_npu_basep_stride_bytes` apart.
 *
...
typedef enum : uint32_t {
  ...
  k_ra_npu_basep_stride_bytes = 0x0008U, /**< Bytes between BASEPn pairs (lo+hi). */
} ra_npu_geom_t;

/**
 * @enum ra_npu_cmd_bit_t
 * @brief Bit positions in the NPU_CMD register.
 *
 * @details Writing 1 to `k_ra_npu_cmd_run_bit` transitions the NPU into the
 *          running state and it begins parsing the command queue. Writing 1 to
 *          `k_ra_npu_cmd_clear_irq_bit` acknowledges (clears) a raised IRQ. The
...
typedef enum : uint8_t {
  ...
  k_ra_npu_cmd_clear_irq_bit = 1U, /**< clear_irq (write 1 to acknowledge IRQ).  */
  k_ra_npu_cmd_clock_q_bit   = 2U, /**< clock_q_enable.                          */
  k_ra_npu_cmd_power_q_bit   = 3U, /**< power_q_enable.                          */
} ra_npu_cmd_bit_t;

/**
 * @enum ra_npu_status_bit_t
 ...
 *          driver surfaces as `k_ra_err_hw_error`.
 *
 * @invariant Each value is a valid 0-based bit index in a 32-bit register.
...
typedef enum : uint8_t {
  ...
  k_ra_npu_status_irq_raised_bit = 1U, /**< IRQ asserted by the NPU.       */
  k_ra_npu_status_bus_error_bit  = 2U, /**< AXI bus error latched.         */
  k_ra_npu_status_reset_bit      = 3U, /**< Reset in progress.             */
  k_ra_npu_status_cmd_parse_bit  = 4U, /**< Command-stream parse error.    */
  k_ra_npu_status_cmd_end_bit    = 5U, /**< Command stream fully consumed. */
  k_ra_npu_status_pmu_irq_bit    = 6U, /**< Performance-counter IRQ.       */
  k_ra_npu_status_wd_fault_bit   = 7U, /**< Weight-decoder fault.          */
  k_ra_npu_status_ecc_fault_bit  = 8U, /**< Internal SRAM ECC fault.       */
} ra_npu_status_bit_t;

/**
 * @enum ra_npu_reset_bit_t
 ...
 *          `k_ra_npu_status_reset_bit` until the reset completes.
 *
 * @invariant Each value is a valid 0-based bit index in a 32-bit register.
 * @see ra_npu_status_bit_t
 * @since 0.1.0
...
typedef enum : uint8_t {
  ...
  k_ra_npu_reset_pending_csl_bit = 1U, /**< Security level adopted after reset.  */
} ra_npu_reset_bit_t;
 ...
 *          `ra8d2_icu_regs.h` / `ra_isr_register`). This number occupies a slot
 ...
 *
 * @details Returns `k_ra_npu_base_addr + byte_off` as a `volatile uint32_t*` so
 *          `ra_npu.c` can read/write a confirmed offset (e.g. `k_ra_npu_off_cmd`)
 ...
 *
 * @param[in] byte_off Register byte offset from `ra_npu_layout_t`
 *                     (0 .. `k_ra_npu_window_bytes` - 4, 4-byte aligned).
 *
...
{
  return (volatile uint32_t*)(k_ra_npu_base_addr + (uintptr_t)byte_off);
}
