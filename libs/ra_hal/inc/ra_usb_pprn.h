 ...
 *
 * @details
 * Glues the device-mode `ra_usb` controller driver to a USB Printer
...
typedef enum : uint8_t {
  k_ra_pprn_pipe_bulk_out = 3U, /**< PIPE3 -> EP1 OUT (print data). */
  k_ra_pprn_pipe_bulk_in  = 4U, /**< PIPE4 -> EP2 IN  (status).     */
} ra_usb_pprn_pipe_t;

/**
 * @enum ra_usb_pprn_ep_t
 * @brief USB endpoint addresses used by the device-Printer function.
 */
typedef enum : uint8_t {
  k_ra_pprn_ep_bulk_out_addr = 1U, /**< EP1 OUT address. */
  k_ra_pprn_ep_bulk_in_addr  = 2U, /**< EP2 IN  address. */
} ra_usb_pprn_ep_t;

/**
 * @enum ra_usb_pprn_packet_t
...
typedef enum : uint16_t {
  k_ra_pprn_bulk_max_packet_fs = 64U,  /**< Bulk size at FS. */
  k_ra_pprn_bulk_max_packet_hs = 512U, /**< Bulk size at HS. */
} ra_usb_pprn_packet_t;

/**
 * @enum ra_usb_pprn_class_t
 ...
 */
typedef enum : uint8_t {
  k_ra_pprn_class_printer    = 0x07U, /**< Printer interface class. */
  k_ra_pprn_subclass_default = 0x01U, /**< Printers subclass.       */
  k_ra_pprn_protocol_unidir  = 0x01U, /**< Uni-directional.         */
  k_ra_pprn_protocol_bidir   = 0x02U, /**< Bi-directional.          */
  k_ra_pprn_protocol_1284_4  = 0x03U, /**< IEEE 1284.4 bi-dir.      */
} ra_usb_pprn_class_t;

/**
 * @enum ra_usb_pprn_request_t
 ...
 */
typedef enum : uint8_t {
  k_ra_pprn_req_get_device_id   = 0x00U, /**< GET_DEVICE_ID.   */
  k_ra_pprn_req_get_port_status = 0x01U, /**< GET_PORT_STATUS. */
  k_ra_pprn_req_soft_reset      = 0x02U, /**< SOFT_RESET.      */
} ra_usb_pprn_request_t;

/**
 * @enum ra_usb_pprn_port_status_bit_t
 ...
 */
typedef enum : uint8_t {
  k_ra_pprn_status_bit_paper_empty = 5U, /**< 1 = paper empty.    */
  k_ra_pprn_status_bit_select      = 4U, /**< 1 = printer online. */
  k_ra_pprn_status_bit_not_error   = 3U, /**< 1 = no error.       */
} ra_usb_pprn_port_status_bit_t;

/**
 * @typedef ra_usb_pprn_setup_fn_t
 ...
 */
typedef ra_err_t (*ra_usb_pprn_setup_fn_t)(void* ctx, const ra_usb_setup_t* setup);
 ...
 * it via `ra_usb_device_attach` once descriptors are set.
 *
 * @param[in] speed Which USB controller (FS or HS).
 *
 * @return `ra_err_t` error code.
 * @retval k_ra_ok Device-Printer ready.
 ...
 * @see ra_usb_pprn_set_descriptors
 * @see ra_usb_pprn_close
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pprn_init(ra_usb_speed_t speed);
 ...
 *       device-Printer API calls return `k_ra_err_invalid_state`.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pprn_close(void);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pprn_set_descriptors(const uint8_t* desc,
                                                   uint16_t       desc_len,
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pprn_recv(uint8_t* buf, uint16_t max_len, uint16_t* got_len);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pprn_send(const uint8_t* data, uint16_t len);
 ...
 * `ra_usb_pprn_port_status_bit_t`. The application typically calls this
 ...
 * @pre `ra_usb_pprn_init` succeeded.
 *
 * @post `ra_usb_pprn_get_port_status` reflects the new value.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pprn_set_port_status(uint8_t status_byte);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pprn_get_port_status(uint8_t* out_status);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pprn_attach_setup_handler(ra_usb_pprn_setup_fn_t setup_fn, void* ctx);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pprn_handle_setup(const ra_usb_setup_t* setup);
