 ...
 * (``ra_iwdt.h``, HUM Ch 28) which has its own free-running clock
 ...
 * this driver's ``ra_wdt_init`` is a no-op in that build, but
 ...
 * to arm the down-counter. This is the mode ``ra_wdt_init``
 * expects.
 *
 * The driver does not (and cannot) flip OFS0 itself -- ``ra_ofs.c``
 * owns the writable side. ``ra_wdt_ofs_get`` exposes the read-only
 * view so the application can decode whichever mode the boot ROM
 * latched.
 *
 * ## API surface
 *
 * - ``ra_wdt_init(cfg)`` -- programme WDTCR/WDTRCR/WDTCSTPR and arm
 * the counter via the first refresh (register-start mode only).
 * - ``ra_wdt_deinit`` -- snapshot WDTCSTPR so the counter halts on
 * Sleep entry. Auto-started WDTs *cannot* truly be stopped.
 * - ``ra_wdt_refresh_deferred`` -- run-time heartbeat (WDT0).
 * - ``ra_wdt_refresh_for(which)`` -- per-instance heartbeat.
 * - ``ra_wdt_get_status`` / ``ra_wdt_clear_status`` /
 * ``ra_wdt_clear_status_blocking`` -- UNDFF / REFEF flag query
 * and clear (with W0C semantics + bounded busy-wait).
 * - ``ra_wdt_get_counter`` -- read live CNTVAL[13:0].
 * - ``ra_wdt_timeout_cycles_get`` / ``ra_wdt_pclkb_divisor`` --
 * decode TOPS + CKS into clock-cycle counts (FSP timeoutGet).
 * - ``ra_wdt_attach_handler`` (legacy single-callback API) and
 * ``ra_wdt_subscribe`` / ``ra_wdt_unsubscribe`` (multi-
 * subscriber API) plus ``ra_wdt_dispatch`` -- NMI / underflow
 * callback injection.
 * - ``ra_wdt_install_nmi`` / ``ra_wdt_uninstall_nmi`` -- enable
 * or disable the WDT bit in NMIER (full NMI vector wiring).
 * - ``ra_wdt_enter_stop`` / ``ra_wdt_exit_stop`` -- Sleep-mode
 * stop-control toggle.
 * - ``ra_wdt_ofs_get(which, *out)`` -- decode the OFSm option-
 * setting word for the targeted instance into ``ra_wdt_cfg_t``
 ...
 * @startuml
 * [*] --> Reset
 * Reset --> Configured: ra_wdt_init (register-start)
 * Reset --> Running: OFS0.WDT0STRT == 0 (auto-start)
 * Configured --> Running: ra_wdt_refresh_deferred (first refresh)
 * Running --> Running: ra_wdt_refresh_deferred inside window
 ...
 */
typedef enum : uint8_t {
  k_ra_wdt_max_subs = 6U, /**< Max simultaneous subscribers. */
} ra_wdt_subs_count_t;

/**
 * @enum ra_wdt_clear_timeout_t
 * @brief Bounded retry budget for ``ra_wdt_clear_status_blocking``.
 *
 * @details
 ...
 * margin) before returning ``k_ra_err_hw_timeout``.
 */
typedef enum : uint32_t {
  k_ra_wdt_clear_max_polls = 0x4000UL, /**< 16384 reads -- safely > 8193. */
} ra_wdt_clear_timeout_t;
 ...
 *
 * @details
 * Filled by the caller and passed to ``ra_wdt_init``. Maps 1:1 onto
 * the bit-fields of WDTCR + WDTRCR + WDTCSTPR.
 *
 * @code
 * const ra_wdt_cfg_t cfg = {
 *.timeout = k_ra_wdt_timeout_4096,
 *.clock_div = k_ra_wdt_clkdiv_128,
...
/**
 * @struct ra_wdt_ofs_decoded_t
 * @brief Decoded view of an ``OFSm`` option-setting word.
 *
 * @details
 * Returned by ``ra_wdt_ofs_get``. ``cfg`` mirrors the same five
 * fields as ``ra_wdt_cfg_t`` (so the same caller code can compare
 ...
 * ``ra_wdt_attach_handler``.
 * @param[in] status_mask Latched WDTSR top bits at dispatch time
 * (``k_ra_wdt_status_underflow`` and / or
 * ``k_ra_wdt_status_refresh``).
 */
 ...
 * @retval k_ra_ok Counter armed.
 * @retval k_ra_err_null_ptr ``cfg`` was null.
 * @retval k_ra_err_invalid_arg ``cfg->clock_div`` is not one of the
 * legal CKS encodings.
 *
 * @pre ``cfg`` is non-null.
 ...
 * Also clears every multi-subscriber slot so a follow-up ``ra_wdt_init``
 ...
 *
 * @param[in] which Instance to refresh (k_ra_wdt0 or k_ra_wdt1).
 *
 * @return ``ra_err_t``
 * @retval k_ra_ok Counter refreshed.
 * @retval k_ra_err_invalid_arg ``which`` not a known instance.
 *
 * @pre ``which`` < ``k_ra_wdt_instance_count``.
 ...
 *
 * @param[out] out_mask Receives ``k_ra_wdt_status_underflow`` and / or
 * ``k_ra_wdt_status_refresh`` (others = 0).
 *
 * @return ``ra_err_t``
 ...
 *
 * @post ``*out_mask`` is one of the ``k_ra_wdt_status_*`` masks.
 ...
 * value. Use ``ra_wdt_clear_status_blocking`` for the polled variant.
 *
 * @return ``ra_err_t``
 * @retval k_ra_ok Always succeeds.
 *
 ...
 * to ``k_ra_wdt_clear_max_polls`` times and returns
 * ``k_ra_err_hw_timeout`` if the bit is still set when the budget
 * runs out (the FSP analogue is the ``do/while`` loop in
 * ``R_WDT_StatusClear``).
 *
 * @param[in] mask Bit mask of flags to clear (any combination of
 * ``k_ra_wdt_status_underflow`` / ``..._refresh``).
 *
 * @return ``ra_err_t``
 * @retval k_ra_ok Targeted flags read 0 within budget.
 * @retval k_ra_err_invalid_arg ``mask`` outside the legal flag bits.
 * @retval k_ra_err_hw_timeout Flag still latched after ``k_ra_wdt_clear_max_polls``.
 *
 * @pre WDT block has been powered.
...
/**
 * @brief Decode a ``ra_wdt_timeout_sel_t`` into its cycle count.
 *
 * @param[in] sel TOPS field (``k_ra_wdt_timeout_*``).
 * @param[out] out_cycles Receives the cycle count (1024 / 4096 /
 * 8192 / 16384) per HUM Ch 27.2.2 Table 27.2
 * p 1259.
 *
 * @return ``ra_err_t``
 * @retval k_ra_ok Decoded.
 * @retval k_ra_err_null_ptr ``out_cycles`` was null.
 * @retval k_ra_err_invalid_arg ``sel`` outside 0..3.
 *
...
/**
 * @brief Decode a ``ra_wdt_clock_div_t`` into its numeric divisor.
 *
 * @param[in] div CKS field (``k_ra_wdt_clkdiv_*``).
 * @param[out] out_divisor Receives the divisor in {4, 64, 128, 512,
 * 2048, 8192} per HUM Ch 27.2.2 p 1258.
 *
 * @return ``ra_err_t``
 * @retval k_ra_ok Decoded.
 * @retval k_ra_err_null_ptr ``out_divisor`` was null.
 * @retval k_ra_err_invalid_arg ``div`` not a legal CKS encoding.
 *
 ...
 *
 * @return ``ra_err_t``
 * @retval k_ra_ok Computed.
 * @retval k_ra_err_null_ptr ``out_pclkb_cycles`` was null.
 * @retval k_ra_err_invalid_arg ``sel`` or ``div`` invalid.
 *
 ...
 * keeps compiling. Internally just calls ``ra_wdt_subscribe`` /
 * ``ra_wdt_unsubscribe`` with a private "legacy" slot, so it
 ...
 * task scheduler,...). Each ``ra_wdt_subscribe`` call grabs the
 * first free slot in the static dispatch table; ``ra_wdt_dispatch``
 ...
 * every dispatch.
 * @param[out] out_slot Receives the slot index (0..k_ra_wdt_max_subs-1)
 * assigned to this subscriber. May be ``nullptr``
 * if the caller doesn't need it.
 *
 * @return ``ra_err_t``
 * @retval k_ra_ok Subscriber installed.
 * @retval k_ra_err_null_ptr ``fn`` was null.
 * @retval k_ra_err_no_mem All ``k_ra_wdt_max_subs`` slots are taken.
 *
 * @pre ``fn`` is non-null.
 ...
 *
 * @param[in] slot Slot index returned by ``ra_wdt_subscribe``.
 *
 * @return ``ra_err_t``
 * @retval k_ra_ok Slot freed.
 * @retval k_ra_err_invalid_arg ``slot`` out of range.
 * @retval k_ra_err_not_found Slot was already empty.
 *
 * @pre ``slot`` < ``k_ra_wdt_max_subs``.
 * @pre Caller is in single-writer init context.
 *
 ...
 *
 * @return ``ra_err_t``
 * @retval k_ra_ok Always succeeds (the underlying ``ra_icu_nmi_enable``
 * only OR's into the register).
 *
 * @pre ``ra_icu_init`` has been called.
 * @pre Caller is in single-writer init context.
 *
...
/**
 * @brief Disable the WDT bit in NMIER (counterpart to ``ra_wdt_install_nmi``).
 *
 * @return ``ra_err_t``
 * @retval k_ra_ok Always succeeds.
 *
...
/**
 * @typedef ra_wdt_ofs_reader_fn_t
 ...
 *
 * @param[in] ofs_addr One of ``k_ra_wdt_ofs0_addr`` /
 * ``k_ra_wdt_ofs3_addr`` (HUM Ch 7).
 * @param[out] out_word Receives the 32-bit option-setting word.
 *
 * @return ``k_ra_ok`` on success; ``k_ra_err_*`` on hook-defined failure.
 */
typedef ra_err_t (*ra_wdt_ofs_reader_fn_t)(uintptr_t ofs_addr, uint32_t* out_word);

/**
 * @brief Override the OFSm reader hook used by ``ra_wdt_ofs_get``.
 *
 * @details
...
/**
 * @brief Decode an OFSm option-setting word into ``ra_wdt_cfg_t``.
 *
 * @details
 * Reads the 32-bit OFS0 (for ``k_ra_wdt0``) or OFS3 (for ``k_ra_wdt1``)
 ...
 *
 * The WDT driver is *read-only* with respect to OFSm; only ``ra_ofs.c``
 * may write the option-setting sections. The actual fetch goes through
 * the ``ra_wdt_ofs_reader_set`` hook so unit tests / bring-up code
 ...
 * @retval k_ra_err_invalid_arg ``which`` is not a known instance.
 * @retval k_ra_err_* Whatever the reader hook returned.
 *
 * @pre ``out`` is non-null.
 * @pre ``which`` < ``k_ra_wdt_instance_count``.
 *
 * @post ``out->cfg`` mirrors the latched OFSm fields.
 * @post ``out->auto_start`` matches ``out->start_mode == k_ra_wdt_ofs_strt_auto``.
 *
