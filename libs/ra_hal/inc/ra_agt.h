...
/**
 * @struct ra_agt_pulse_cfg_t
 * @brief Configuration for `ra_agt_start_pulse_output`.
 *
 * @details
...
typedef struct {
  ...
  ra_agt_pulse_mode_t      mode;     /**< continuous or one-shot.           */
  ra_agt_output_polarity_t polarity; /**< Start-level of AGTOn.             */
  ra_agt_pulse_compare_t   compare;  /**< Which compare-match pin is armed. */
} ra_agt_pulse_cfg_t;
 ...
 * @retval k_ra_err_null_ptr    ``cfg`` was NULL or ``channel`` >= 10.
 * @retval k_ra_err_invalid_arg ``cfg->compare`` enum out of range.
 *
 * @pre IRQs masked or single-threaded init context.
 * @pre Pin function-select has already routed AGTOn / AGTOAn / AGTOBn
 *      to the chosen package pin via ``ra_pfs_route_peripheral``.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_agt_start_pulse_output(uint8_t channel, const ra_agt_pulse_cfg_t* cfg);

/**
 * @struct ra_agt_cascade_cfg_t
 * @brief Configuration for `ra_agt_start_cascade`.
 *
 * @details
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_agt_start_cascade(const ra_agt_cascade_cfg_t* cfg);
