 ...
 * provides the `RA_PROTECTED_WRITE` helper macro which:
 *
 ...
 *
 * @param unlock_val One of the `k_ra_prcr_unlock_*` values from
 *                   `ra8d2_system_regs.h`.
 */
#define RA_PROTECTED_WRITE(unlock_val)                                                             \
  for (uint32_t ra_prot_once_ = ((*ra_sys_prcr() = (uint16_t)(unlock_val)), 1U);                   \
       ra_prot_once_ != 0U;                                                                        \
       (*ra_sys_prcr() = (uint16_t)k_ra_prcr_lock_all), ra_prot_once_ = 0U)
