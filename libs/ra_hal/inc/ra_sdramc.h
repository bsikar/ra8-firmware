 ...
 *    `k_ra_sdramc_spin_max` iterations (NASA P10 Rule 2).
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               SDRAM is now in self-refresh.
 * @retval k_ra_err_invalid_state SDSR status bits were non-zero on
 *                                entry, or SDSELF.SFEN was already 1.
 * @retval k_ra_err_hw_timeout   SDSR.SRFST did not clear within
 *                                `k_ra_sdramc_spin_max` polls.
 *
 * @pre SDSR.SRFST, INIST, and MRSST are all 0 (no operation in progress).
 * @pre SDSELF.SFEN is 0 (not already in self-refresh).
 * @post On k_ra_ok, SDSELF.SFEN is 1 (self-refresh active).
 * @post On k_ra_ok, SDSR.SRFST is 0 (transition complete).
 *
 ...
 *       self-refresh.
 *
 * @see ra_sdramc_exit_self_refresh() Paired exit function.
 * @see ra_sdramc_enter_stop() Deeper stop (loses refresh entirely).
 *
 * @since 0.1.0
 *
 * @par HUM:
 ...
 * - Rule 2: SDSR poll loop bounded by `k_ra_sdramc_spin_max`.
 * - Rule 5: 2 precondition checks (SDSR idle, SFEN clear).
 */
[[nodiscard]] ra_err_t ra_sdramc_enter_self_refresh(void);
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               SDRAM has returned to auto-refresh.
 * @retval k_ra_err_invalid_state SDSELF.SFEN was 0 (not in self-refresh)
 ...
 * @post On k_ra_ok, SDSELF.SFEN is 0 (self-refresh inactive).
 * @post On k_ra_ok, SDRFEN.RFEN is 1 (auto-refresh re-enabled).
 *
 ...
 *
 * @see ra_sdramc_enter_self_refresh() Paired entry function.
 *
 * @since 0.1.0
 *
 * @par HUM:
 ...
 */
[[nodiscard]] ra_err_t ra_sdramc_exit_self_refresh(void);
