 ...
 *
 * @details
 * Glues the device-mode `ra_usb` controller driver to a vendor-defined
...
typedef enum : uint8_t {
  k_ra_pvnd_pipe_bulk_in  = 5U, /**< PIPE5 -> EP1 IN  (vendor IN).  */
  k_ra_pvnd_pipe_bulk_out = 1U, /**< PIPE1 -> EP2 OUT (vendor OUT). */
} ra_usb_pvnd_pipe_t;

/**
 * @enum ra_usb_pvnd_ep_t
 * @brief USB endpoint addresses used by the device-Vendor function.
 */
typedef enum : uint8_t {
  k_ra_pvnd_ep_bulk_in_addr  = 1U, /**< EP1 IN  address. */
  k_ra_pvnd_ep_bulk_out_addr = 2U, /**< EP2 OUT address. */
} ra_usb_pvnd_ep_t;

/**
 * @enum ra_usb_pvnd_packet_t
...
typedef enum : uint16_t {
  k_ra_pvnd_bulk_max_packet_fs = 64U,  /**< Bulk size at FS. */
  k_ra_pvnd_bulk_max_packet_hs = 512U, /**< Bulk size at HS. */
} ra_usb_pvnd_packet_t;

/**
 * @enum ra_usb_pvnd_class_t
 ...
 */
typedef enum : uint8_t {
  k_ra_pvnd_class_vendor = 0xFFU, /**< Vendor-specific interface class. */
} ra_usb_pvnd_class_t;

/**
 * @enum ra_usb_pvnd_envelope_t
...
/**
 * @typedef ra_usb_pvnd_setup_fn_t
 ...
 *       vendor SETUP lands.
 */
typedef ra_err_t (*ra_usb_pvnd_setup_fn_t)(void* ctx, const ra_usb_setup_t* setup);
 ...
 * @retval k_ra_ok Device-Vendor ready.
 * @retval k_ra_err_invalid_arg `speed` out of range.
 * @retval k_ra_err_hw_init_failed Underlying `ra_usb_device_init` failed.
 *
 ...
 * @see ra_usb_pvnd_set_descriptors
 * @see ra_usb_pvnd_close
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pvnd_init(ra_usb_speed_t speed);
 ...
 *       device-Vendor API calls return `k_ra_err_invalid_state`.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pvnd_close(void);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pvnd_set_descriptors(const uint8_t* desc, uint16_t desc_len);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pvnd_send(const uint8_t* data, uint16_t len);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pvnd_recv(uint8_t* buf, uint16_t max_len, uint16_t* got_len);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pvnd_attach_setup_handler(ra_usb_pvnd_setup_fn_t setup_fn, void* ctx);
 ...
 * Standard / class SETUPs are rejected with `k_ra_err_not_supported`
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pvnd_handle_setup(const ra_usb_setup_t* setup);
