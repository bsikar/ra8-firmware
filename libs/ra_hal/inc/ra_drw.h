 ...
 *
 * - Lifecycle: ``ra_drw_init``, ``ra_drw_deinit``, power
 * transitions, MSTP gate.
 * - Diagnostics: ``ra_drw_get_status``, ``ra_drw_clear_status``,
 * ``ra_drw_get_hwrevision``, ``ra_drw_wait_idle``.
 * - IRQ: ``ra_drw_attach_handler``, ``ra_drw_dispatch``,
 * ``ra_drw_set_irq_enables``.
...
/**
 * @struct ra_drw_config_t
 * @brief Minimal DRW configuration captured by ``ra_drw_init``.
 *
 * @details
...
/**
 * @struct ra_drw_rect_t
 * @brief Rectangle parameters for ``ra_drw_fill_rect``.
 *
 * @details
...
/**
 * @struct ra_drw_texture_t
 * @brief Texture parameters for ``ra_drw_blit_textured_rect``.
 *
 * @details
 ...
 * @invariant ``pitch_px`` <= ``k_ra_drw_max_texpitch_tx`` (HUM 62.2.15).
 */
/* cppcheck-suppress-begin [unusedStructMember] */
typedef struct {
...
/**
 * @typedef ra_drw_event_fn_t
 * @brief DRW asynchronous event callback signature.
 *
 * @details
 * Fired from ``ra_drw_dispatch`` after the DRW IRQ snapshots and
 * acknowledges the STATUS register. The ``status_mask`` argument
 * carries the raw STATUS value the dispatcher observed before
 * clearing it; consumers test bits with the ``k_ra_drw_status_*``
 * masks from ``ra8d2_drw_regs.h``.
 */
typedef void (*ra_drw_event_fn_t)(void* ctx, uint32_t status_mask);
 ...
 * @details
 * Sequence performed:
 * 1. Ungate the DRW peripheral via ``ra_mstp_enable(k_ra_mstp_drw)``.
 ...
 * @retval k_ra_ok DRW powered up + configured.
 ...
 * non-null. Bits decode via ``k_ra_drw_status_*``.
 *
 * @return ``ra_err_t`` error code.
 ...
 * ``k_ra_drw_irqctl_*_clr`` enum values; arbitrary other bits in
 ...
 * ``k_ra_drw_irqctl_enumirqen``,
 * ``k_ra_drw_irqctl_dlistirqen``,
 * ``k_ra_drw_irqctl_busirqen``.
 *
 * @return ``ra_err_t`` error code.
 ...
 *
 * @post Subsequent calls to ``ra_drw_dispatch`` will invoke ``fn``.
 ...
 *
 * @details
 * Bounded wait (NASA Rule 2). Returns ``k_ra_err_hw_timeout`` if the
 ...
 * accessed without first calling ``ra_drw_exit_stop``.
 *
 * @note Not thread-safe.
 ...
 *
 * @return ``ra_err_t`` propagated from ``ra_mstp_enable``.
 * @retval k_ra_ok DRW running.
 *
 * @pre Caller holds single-threaded power transition lock.
 * @pre Previous ``ra_drw_enter_stop`` happened (or this is first
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok COLOR1/COLOR2 updated.
...
/**
 * @brief Configure the alpha-blend unit using ``ra_drw_blend_t`` fields.
 *
 * @param[in] blend Blend parameters. Must not be ``nullptr``.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok CONTROL2 + COLOR1.A updated.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok Texture programmed.
 * @retval k_ra_err_null_ptr ``tex`` was nullptr.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok CLUT loaded.
 * @retval k_ra_err_null_ptr ``entries`` was nullptr.
 * @retval k_ra_err_invalid_arg start_index + count > 256, or count = 0.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok Blit issued (engine may still be busy).
 * @retval k_ra_err_null_ptr ``rect`` was ``nullptr``.
 ...
 *
 * @details
 * The caller programmes the texture state via ``ra_drw_set_texture``
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok Blit issued.
 * @retval k_ra_err_null_ptr ``rect`` was nullptr.
 * @retval k_ra_err_invalid_arg width/height out of range.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok Line issued.
 * @retval k_ra_err_null_ptr ``line`` was nullptr.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok Triangle issued.
