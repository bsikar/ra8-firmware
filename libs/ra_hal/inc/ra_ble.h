...
/**
 * @struct ra_ble_config_t
 * @brief Configuration passed to ``ra_ble_open``.
 */
typedef struct {
 ...
 */
typedef void (*ra_ble_event_fn_t)(void*          ctx,
                                  uint8_t        evt_code,
                                  const uint8_t* params,
                                  uint8_t        params_len);

/**
 * @typedef ra_ble_acl_fn_t
 ...
 */
typedef void (*ra_ble_acl_fn_t)(void* ctx, uint16_t handle, const uint8_t* payload, uint16_t len);
 ...
 *
 * @return ``k_ra_err_null_ptr`` if ``cfg == NULL``.
 * @return ``k_ra_err_invalid_arg`` if controller is already open.
 ...
 * @post Subsequent ``ra_ble_hci_send_command`` calls are accepted.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ble_open(const ra_ble_config_t* cfg);

/**
 * @brief Close the HCI mailbox and put the controller back in reset.
 *
 * @return ``k_ra_err_invalid_arg`` if controller is not open.
 * @return ``k_ra_ok`` on success.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ble_close(void);
 ...
 *
 * @return ``k_ra_err_not_initialized`` if controller is not open.
 * @return ``k_ra_err_null_ptr`` if ``params == NULL && params_len > 0``.
 * @return ``k_ra_err_invalid_arg`` if ``params_len > k_ra_ble_max_cmd_params``.
 * @return ``k_ra_ok`` on success.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t
ra_ble_hci_send_command(uint16_t opcode, const uint8_t* params, uint8_t params_len);
 ...
 * @param[in] len     Payload byte count (0..k_ra_ble_max_acl_payload).
 *
 * @return ``k_ra_err_not_initialized`` if controller is not open.
 * @return ``k_ra_err_null_ptr`` if ``payload == NULL && len > 0``.
 * @return ``k_ra_err_invalid_arg`` if ``len > k_ra_ble_max_acl_payload``.
 * @return ``k_ra_ok`` on success.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t
ra_ble_hci_send_acl_data(uint16_t handle, const uint8_t* payload, uint16_t len);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ble_attach_event_handler(ra_ble_event_fn_t fn, void* ctx);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ble_attach_acl_handler(ra_ble_acl_fn_t fn, void* ctx);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ble_dispatch(void);
 ...
 *
 * @return ``k_ra_err_null_ptr`` if ``addr == NULL``.
 * @return ``k_ra_err_not_initialized`` if controller is not open.
 * @return ``k_ra_ok`` on success.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ble_set_random_address(const uint8_t addr[k_ra_ble_addr_bytes]);

/**
 * @brief Send LE_Set_Advertising_Data (opcode 0x2008).
 *
 * @param[in] data Advertising-data bytes.
 * @param[in] len  Byte count (0..k_ra_ble_adv_data_max).
 *
 * @return ``k_ra_err_invalid_arg`` if ``len > k_ra_ble_adv_data_max``.
 * @return ``k_ra_err_null_ptr`` if ``data == NULL && len > 0``.
 * @return ``k_ra_err_not_initialized`` if controller is not open.
 * @return ``k_ra_ok`` on success.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ble_set_advertising_data(const uint8_t* data, uint8_t len);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ble_set_advertising_enable(uint8_t enable);
 ...
 *
 * @return ``k_ra_err_invalid_arg`` if window > interval or values out of range.
 * @return ``k_ra_err_not_initialized`` if controller is not open.
 * @return ``k_ra_ok`` on success.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ble_scan_start(uint8_t active, uint16_t interval, uint16_t window);
