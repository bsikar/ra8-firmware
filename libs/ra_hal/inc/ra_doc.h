...
/**
 * @enum ra_doc_window_polarity_t
 * @brief Window-comparison polarity for ra_doc_set_window().
 *
 * @details
 * Selects between inside-band and outside-band detection. The silicon
 * encodes this via DOCR.DCSEL[2:0] (HUM Ch 57.2.1 p 3519):
 * - k_ra_doc_window_inside  -> DCSEL = 100b (4)
 * - k_ra_doc_window_outside -> DCSEL = 101b (5)
 *
 ...
 * ra_doc_set_window(0x0800U, 0x1800U, k_ra_doc_window_inside);
 * @endcode
 *
 * @see ra_doc_set_window() Configure window thresholds and polarity.
 * @see ra_doc_window_compare() Trigger a comparison and read DOPCF.
 *
 * @since 0.1.0
 */
typedef enum : uint8_t {
  k_ra_doc_window_inside  = 0U, /**< Flag when DODSR0 < DODIR < DODSR1 (strict inequalities). */
  k_ra_doc_window_outside = 1U, /**< Flag when DODIR < DODSR0 or DODSR1 < DODIR.              */
} ra_doc_window_polarity_t;
 ...
 * ra_doc_window_compare() triggers a hardware comparison; DOSR.DOPCF is
 * set when the input satisfies the selected condition:
 *
 * - k_ra_doc_window_inside:  DODSR0 < DODIR < DODSR1 (strict).
 * - k_ra_doc_window_outside: DODIR < DODSR0 or DODSR1 < DODIR (strict).
 *
 ...
 * before returning so the first ra_doc_window_compare() call reflects a
 ...
 * @param[in] polarity Detection polarity (k_ra_doc_window_inside or
 *                     k_ra_doc_window_outside).
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok              Window programmed and stale DOPCF cleared.
 * @retval k_ra_err_invalid_arg lower >= upper, or polarity is out of range.
 *
 * @pre ra_doc_init() has been called successfully.
 ...
 * @see ra_doc_window_compare() Trigger a comparison and read DOPCF.
 * @see ra_doc_init() Must be called before this function.
 *
 * @since 0.1.0
 *
 * @par HUM:
 ...
 * thresholds programmed by ra_doc_set_window(). DOSR.DOPCF is read back
 ...
 *
 * In RA_SIMULATOR_MODE the host model computes the flag in software
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Comparison complete; *out_flag updated.
 * @retval k_ra_err_null_ptr     out_flag is nullptr.
 * @retval k_ra_err_invalid_state DOCR.OMS != 0 (not in compare mode;
 *                                call ra_doc_set_window() first).
 *
 * @pre ra_doc_set_window() has been called to configure the window.
 ...
 * @see ra_doc_set_window() Configure the window thresholds first.
 *
 * @since 0.1.0
 *
 * @par HUM:
