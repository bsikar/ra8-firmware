 ...
 * Lifecycle:
 *
 *  1. `ra_usb_hcdc_ecm_init(speed)` flips the controller to host mode,
 ...
 *     and the parsed MAC.
 *  5. `ra_usb_hcdc_ecm_send_frame` / `_recv_frame` move Ethernet frames
 *     through the bulk pipes.
 *  6. `ra_usb_hcdc_ecm_set_packet_filter` issues the SET_ETHERNET_PACKET_FILTER
 *     class request (bRequest=0x43, bmRequestType=0x21) so the adapter
 *     accepts the desired class of frames.
 *  7. `ra_usb_hcdc_ecm_get_link_status` issues GET_ETHERNET_STATISTIC
 *     (bRequest=0x44).
 *  8. `ra_usb_hcdc_ecm_close()` drops bus power and releases the device.
 *
...
typedef enum : uint16_t {
  k_ra_hcdc_ecm_bulk_max_packet_fs = 64U,   /**< Bulk size at full speed. */
  k_ra_hcdc_ecm_bulk_max_packet_hs = 512U,  /**< Bulk size at high speed. */
  k_ra_hcdc_ecm_intr_max_packet    = 16U,   /**< Notification pipe size.  */
  k_ra_hcdc_ecm_max_ethernet_frame = 1514U, /**< Ethernet frame ceiling.  */
} ra_usb_hcdc_ecm_packet_t;

/**
 * @enum ra_usb_hcdc_ecm_class_t
...
/**
 * @enum ra_usb_hcdc_ecm_request_t
 * @brief CDC-ECM class-specific request codes the host issues to the
 *        attached adapter (USB CDC ECM 1.20 sec 6.2 "Management
 *        Element Requests").
 */
typedef enum : uint8_t {
  k_ra_hcdc_ecm_req_set_packet_filter = 0x43U, /**< SET_ETHERNET_PACKET_FILTER. */
  k_ra_hcdc_ecm_req_get_statistic     = 0x44U, /**< GET_ETHERNET_STATISTIC.     */
} ra_usb_hcdc_ecm_request_t;

/**
 * @enum ra_usb_hcdc_ecm_filter_t
...
/**
 * @enum ra_usb_hcdc_ecm_link_t
 * @brief Adapter link status reported by `_get_link_status`.
 */
typedef enum : uint8_t {
  k_ra_hcdc_ecm_link_down = 0U, /**< NetworkConnection notification 0. */
  k_ra_hcdc_ecm_link_up   = 1U, /**< NetworkConnection notification 1. */
} ra_usb_hcdc_ecm_link_t;

/**
 * @enum ra_usb_hcdc_ecm_mac_t
...
/**
 * @struct ra_usb_hcdc_ecm_device_t
 ...
 */
typedef void (*ra_usb_hcdc_ecm_attach_fn_t)(void* ctx, const ra_usb_hcdc_ecm_device_t* device);
 ...
 * by delegating to `ra_usb_host_init`, programs the DCP for 64-byte
 ...
 * @retval k_ra_ok Host-CDC-ECM ready, awaiting attach.
 ...
 * @see ra_usb_hcdc_ecm_attach_callback
 * @see ra_usb_hcdc_ecm_close
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hcdc_ecm_init(ra_usb_speed_t speed);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hcdc_ecm_close(void);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hcdc_ecm_attach_callback(ra_usb_hcdc_ecm_attach_fn_t on_attach,
                                                       void*                       ctx);
 ...
 * @param[in] len Byte count, 0..k_ra_hcdc_ecm_max_ethernet_frame.
 *
 * @return `ra_err_t` error code.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hcdc_ecm_send_frame(const uint8_t* buf, uint16_t len);
 ...
 */
[[nodiscard]] ra_err_t
ra_usb_hcdc_ecm_recv_frame(uint8_t* buf, uint16_t max_len, uint16_t* got_len);
 ...
 * `ra_usb_host_setup_request`. See USB CDC ECM 1.20 sec 6.2.4
 * "SetEthernetPacketFilter".
 *
 * @param[in] filter_mask OR of `k_ra_hcdc_ecm_filter_*` bits.
 *
 * @return `ra_err_t` error code.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hcdc_ecm_set_packet_filter(uint16_t filter_mask);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hcdc_ecm_get_link_status(ra_usb_hcdc_ecm_link_t* out_link);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hcdc_ecm_step(void);
 ...
 *
 * @return `ra_err_t` error code.
 * @retval k_ra_ok Parsed successfully.
 * @retval k_ra_err_null_ptr `chars` or `out_mac` was NULL.
 * @retval k_ra_err_invalid_arg One of the characters was not a valid
 *         ASCII hex digit.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hcdc_ecm_parse_mac(const char* chars, uint8_t* out_mac);
