...
typedef enum : uintptr_t {
  k_ra_poeg0_base_addr = 0x40212000UL,
  k_ra_poeg1_base_addr = 0x40212100UL,
  k_ra_poeg2_base_addr = 0x40212200UL,
  k_ra_poeg3_base_addr = 0x40212300UL,
} ra_poeg_addr_t;

typedef enum : uint16_t {
  k_ra_poeg_group_count  = 4U,
  k_ra_poeg_group_stride = 0x100U,
} ra_poeg_limits_t;
...
static inline volatile r_poeg_regs_t* ra_poeg(uint8_t group)
{
  if ((uint16_t)group >= k_ra_poeg_group_count) {
    return nullptr;
  }
  return (volatile r_poeg_regs_t*)(k_ra_poeg0_base_addr +
                                   ((uintptr_t)group * (uintptr_t)k_ra_poeg_group_stride));
}
