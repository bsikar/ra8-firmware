 ...
 * an MDIO bus -- instead, it borrows one through a `ra_ether_phy_io_t`
...
/**
 * @enum ra_ether_phy_speed_t
 * @brief Negotiated PHY link speed / duplex.
 */
typedef enum : uint8_t {
  k_ra_ether_phy_speed_no_link = 0U,
  k_ra_ether_phy_speed_10h     = 1U,
  k_ra_ether_phy_speed_10f     = 2U,
  k_ra_ether_phy_speed_100h    = 3U,
  k_ra_ether_phy_speed_100f    = 4U,
  k_ra_ether_phy_speed_1000h   = 5U,
  k_ra_ether_phy_speed_1000f   = 6U,
} ra_ether_phy_speed_t;

/**
 * @enum ra_ether_phy_mii_t
 * @brief Media-independent interface type seen on the MAC side.
 */
typedef enum : uint8_t {
  k_ra_ether_phy_mii   = 0U,
  k_ra_ether_phy_rmii  = 1U,
  k_ra_ether_phy_gmii  = 2U,
  k_ra_ether_phy_rgmii = 3U,
} ra_ether_phy_mii_t;

/**
 * @enum ra_ether_phy_clause22_reg_t
...
/**
 * @enum ra_ether_phy_addr_limit_t
 * @brief Five-bit MDIO PHY address (0..31).
 */
typedef enum : uint8_t {
  k_ra_ether_phy_addr_max = 31U,
} ra_ether_phy_addr_limit_t;

/**
 * @struct ra_ether_phy_io_t
 ...
 *
 * @return `ra_err_t`.
 * @retval k_ra_ok                  PHY responded to reset.
 * @retval k_ra_err_null_ptr        `cfg`, or any required IO callback NULL.
 * @retval k_ra_err_invalid_arg     `phy_address` > 31.
 * @retval k_ra_err_exists          Already opened.
 * @retval k_ra_err_hw_timeout      Reset bit never cleared.
 *
 * @pre Single-threaded init context.
 * @post Driver is in the open state.
 * @post `ra_ether_phy_close` will succeed.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ether_phy_open(const ra_ether_phy_cfg_t* cfg);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ether_phy_close(void);
 ...
 *
 * @pre `ra_ether_phy_open` succeeded.
 * @post `*out_data` is defined on success.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ether_phy_mdio_read(uint8_t reg_addr, uint16_t* out_data);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ether_phy_mdio_write(uint8_t reg_addr, uint16_t data);

/**
 * @brief Kick off auto-negotiation by re-arming BMCR.
 *
 * @return `ra_err_t`.
 * @retval k_ra_ok                   Auto-neg restarted.
 * @retval k_ra_err_not_initialized  Not opened.
 *
 * @pre `ra_ether_phy_open` succeeded.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ether_phy_auto_negotiate_start(void);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_ether_phy_link_status_get(ra_ether_phy_link_t* out);
