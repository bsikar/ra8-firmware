 ...
 *
 * @details
 * Glues the device-mode `ra_usb` controller driver to a USB CDC ACM
 ...
 *
 * The descriptor table built by `ra_usb_cdc_init` advertises:
 *
...
typedef enum : uint8_t {
  k_ra_cdc_pipe_bulk_in  = 1U, /**< PIPE1 -> EP1 IN  (bulk).      */
  k_ra_cdc_pipe_bulk_out = 2U, /**< PIPE2 -> EP2 OUT (bulk).      */
  k_ra_cdc_pipe_intr_in  = 6U, /**< PIPE6 -> EP3 IN  (interrupt). */
} ra_usb_cdc_pipe_t;

/**
 * @enum ra_usb_cdc_ep_t
 * @brief USB endpoint addresses used by the CDC ACM function.
 */
typedef enum : uint8_t {
  k_ra_cdc_ep_bulk_in_addr  = 1U, /**< EP1 IN address.  */
  k_ra_cdc_ep_bulk_out_addr = 2U, /**< EP2 OUT address. */
  k_ra_cdc_ep_intr_in_addr  = 3U, /**< EP3 IN address.  */
} ra_usb_cdc_ep_t;

/**
 * @enum ra_usb_cdc_packet_t
...
/**
 * @enum ra_usb_cdc_request_t
 * @brief CDC class-specific request codes carried in `bRequest`.
 *
 * @details Numbered from the USB CDC PSTN subclass spec rev 1.20.
 */
typedef enum : uint8_t {
  k_ra_cdc_req_set_line_coding        = 0x20U, /**< 7-byte payload. */
  k_ra_cdc_req_get_line_coding        = 0x21U, /**< 7-byte payload. */
  k_ra_cdc_req_set_control_line_state = 0x22U, /**< 0-byte payload. */
} ra_usb_cdc_request_t;

/**
 * @enum ra_usb_cdc_line_state_t
 * @brief Bits inside `wValue` for SET_CONTROL_LINE_STATE.
 */
typedef enum : uint16_t {
  k_ra_cdc_line_state_dtr = 0x0001U, /**< Data terminal ready. */
  k_ra_cdc_line_state_rts = 0x0002U, /**< Request to send.     */
} ra_usb_cdc_line_state_t;

/**
 * @struct ra_usb_cdc_line_coding_t
 ...
 *
 * @post `ra_usb_cdc_get_line_coding` returns 9600/8/N/1.
 * @post Bulk + interrupt pipes are configured but not attached.
 *
 * @note Not thread-safe.
 * @see ra_usb_cdc_attach
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_cdc_init(ra_usb_speed_t speed);
 ...
 *
 * @post Subsequent CDC API calls return `k_ra_err_invalid_state`.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_cdc_deinit(void);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_cdc_attach(bool attached);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_cdc_send(const uint8_t* data, uint16_t len);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_cdc_recv(uint8_t* out_buf, uint16_t* inout_len);
 ...
 * `k_ra_err_not_supported` so the caller can fall back to its own
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_cdc_handle_setup(const ra_usb_setup_t* setup);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_cdc_get_line_coding(ra_usb_cdc_line_coding_t* out);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_cdc_get_line_state(bool* out_dtr, bool* out_rts);
