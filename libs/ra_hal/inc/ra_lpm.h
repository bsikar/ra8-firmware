 ...
 *    plus per-source helpers (``ra_lpm_arm_wupen0_bits``,
 *    ``ra_lpm_arm_wupen1_bits``).
 *
 ...
 *    ``ra_lpm_snooze_set_request_sources`` /
 *    ``ra_lpm_snooze_set_end_sources`` writing to the WUPEN1
 *    ULPT bits and DPSIER3 ULPT bits respectively, mirroring the
 *    FSP "Standby + Snooze" pattern.
 *
 *  - **Deep-standby cancel sources**: ``ra_lpm_arm_dpsier`` /
 *    ``ra_lpm_clear_dpsifr`` / ``ra_lpm_set_dpsiegr`` give per-
 *    factor control over DPSIER0..3, DPSIFR0..3, and DPSIEGR0..2.
 *
 *  - **RAM retention**: ``ra_lpm_set_ram_retention`` writes
 *    PDRAMSCR0 (per-bank user-SRAM) and PDRAMSCR1 (per-CPU TCM).
 *
 *  - **LDO standby**: ``ra_lpm_set_ldo_standby`` writes the
 *    PLL1LDOCR / PLL2LDOCR / HOCOLDOCR SKEEP bits subject to the
 *    OPCCR.OPCM=0 precondition.
 *
 *  - **Clock shutdown matrix**: ``ra_lpm_set_clock_stop`` /
 *    ``ra_lpm_get_clock_stop`` toggle the per-oscillator stop bits
 *    so the application can stop MOCO/HOCO before sleep entry.
 *
 *  - **OPCCR**: ``ra_lpm_get_opccr`` exposes the optimal-power-
 *    control mode and its in-progress flag.
 *
 *  - **PRCR**: ``ra_lpm_prcr_unlock`` / ``ra_lpm_prcr_relock``
 *    encapsulate the 16-bit key-write dance; the rest of the API
 *    assumes the caller has unlocked PRC1 around protected writes.
 *
 *  - **Diagnostics**: ``ra_lpm_get_status`` packs SBYCR/DPSBYCR/
 *    LPSCR/SSCR1; ``ra_lpm_get_exit_cause`` snapshots WUPEN0/1.
 *
...
/**
 * @struct ra_lpm_config_t
 * @brief Configuration descriptor for ``ra_lpm_init``.
 *
 * @details
 ...
 * as unused; every member is consumed in ``ra_lpm_init`` in
 * ``libs/ra_hal/src/ra_lpm.c``.
 */
 ...
 * companion ``ra_lpm_prcr_relock`` writes ``0xA500`` to lock the
 * region again. Pairs with the standby/wake critical section.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok  PRC1 unlocked.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok  PRC1 locked.
 *
 * @pre Earlier ``ra_lpm_prcr_unlock`` succeeded.
 * @pre IRQs masked or single-threaded context.
 *
 * @post PRCR.PRC1 == 0.
 ...
 * verbatim. Use the ``k_ra_lpm_wupen0_*`` / ``k_ra_lpm_wupen1_*``
 ...
 *
 * @param[in] bits ``k_ra_lpm_wupen0_*`` mask to set.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok WUPEN0 updated.
 *
 * @pre IRQs masked or caller holds the LPM lock.
 ...
 *
 * @param[in] bits ``k_ra_lpm_wupen0_*`` mask to clear.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok WUPEN0 updated.
 *
 * @pre IRQs masked or caller holds the LPM lock.
 * @pre WUPEN0 was previously armed via ``ra_lpm_arm_wupen0_bits``.
 *
 ...
 *
 * @param[in] bits ``k_ra_lpm_wupen1_*`` mask to set.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok WUPEN1 updated.
 *
 * @pre IRQs masked or caller holds the LPM lock.
 * @pre Wake-up source has been configured in its owning peripheral.
 *
 ...
 *
 * @param[in] bits ``k_ra_lpm_wupen1_*`` mask to clear.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok WUPEN1 updated.
 *
 * @pre IRQs masked or caller holds the LPM lock.
 * @pre WUPEN1 was previously armed via ``ra_lpm_arm_wupen1_bits``.
 *
 ...
 *
 * @param[in] idx   DPSIER selector (``k_ra_lpm_dpsier_idx_*``).
 * @param[in] value 8-bit enable mask to write.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               Register updated.
 * @retval k_ra_err_invalid_arg  ``idx`` out of range.
 *
 * @pre IRQs masked or single-threaded context.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok DPSIER3 updated.
 *
 * @pre IRQs masked or single-threaded context.
 * @pre PRCR.PRC1 unlocked.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok           Both registers programmed.
 * @retval k_ra_err_null_ptr ``cfg`` was nullptr.
 *
 * @pre IRQs masked or single-threaded context.
 * @pre PRCR.PRC1 unlocked.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                  All three LDOCR bytes updated.
 * @retval k_ra_err_null_ptr        ``cfg`` was nullptr.
 * @retval k_ra_err_invalid_state   OPCCR.OPCM != 0 (writes prohibited).
 *
 ...
 *
 * @param[in] clock Oscillator selector (``k_ra_lpm_clock_*``).
 * @param[in] stop  true -- request stop; false -- request run.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               Stop bit toggled.
 * @retval k_ra_err_invalid_arg  ``clock`` out of range.
 *
 * @pre IRQs masked or single-threaded context.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               ``*stop`` populated.
 * @retval k_ra_err_invalid_arg  ``clock`` out of range.
 * @retval k_ra_err_null_ptr     ``stop`` was nullptr.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok           ``*opccr`` populated.
 * @retval k_ra_err_null_ptr ``opccr`` was nullptr.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok             OPCMTSF cleared within budget.
 * @retval k_ra_err_hw_timeout poll_limit exhausted with OPCMTSF=1.
 *
 * @pre poll_limit > 0.
 * @pre SYSC bus is clocked.
 *
 ...
 * modes, then executes WFI. For ``k_ra_sleep_mode_sleep`` the LPMD
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                 Mode programmed and WFI returned.
 * @retval k_ra_err_invalid_arg    ``mode`` was not a valid encoding.
 *
 * @pre Caller has armed at least one wake-up source via
 *      ``ra_lpm_set_wakeup_sources`` / ``ra_lpm_arm_dpsier``.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok WFI returned (cold path: never returns on hardware).
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok            All buffers populated.
 * @retval k_ra_err_null_ptr  Any of ``enables`` / ``flags`` / ``edges`` was nullptr.
 *
