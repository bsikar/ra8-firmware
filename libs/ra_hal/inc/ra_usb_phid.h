 ...
 *
 * @details
 * Glues the device-mode `ra_usb` controller driver to a USB HID
 ...
 *  - EP0 64-byte default control pipe (provided by `ra_usb`).
...
typedef enum : uint8_t {
  k_ra_phid_pipe_intr_in  = 6U, /**< PIPE6 -> EP1 IN  (intr). */
  k_ra_phid_pipe_intr_out = 7U, /**< PIPE7 -> EP2 OUT (intr). */
} ra_usb_phid_pipe_t;

/**
 * @enum ra_usb_phid_ep_t
 * @brief USB endpoint addresses used by the device-HID function.
 */
typedef enum : uint8_t {
  k_ra_phid_ep_intr_in_addr  = 1U, /**< EP1 IN address.  */
  k_ra_phid_ep_intr_out_addr = 2U, /**< EP2 OUT address. */
} ra_usb_phid_ep_t;

/**
 * @enum ra_usb_phid_packet_t
...
typedef enum : uint16_t {
  k_ra_phid_intr_max_packet_default = 8U,    /**< Boot-protocol default. */
  k_ra_phid_intr_max_packet_fs      = 64U,   /**< FS ceiling.            */
  k_ra_phid_intr_max_packet_hs      = 1024U, /**< HS ceiling.            */
} ra_usb_phid_packet_t;

/**
 * @enum ra_usb_phid_class_t
 ...
 */
typedef enum : uint8_t {
  k_ra_phid_req_get_report   = 0x01U, /**< GET_REPORT.   */
  k_ra_phid_req_get_idle     = 0x02U, /**< GET_IDLE.     */
  k_ra_phid_req_get_protocol = 0x03U, /**< GET_PROTOCOL. */
  k_ra_phid_req_set_report   = 0x09U, /**< SET_REPORT.   */
  k_ra_phid_req_set_idle     = 0x0AU, /**< SET_IDLE.     */
  k_ra_phid_req_set_protocol = 0x0BU, /**< SET_PROTOCOL. */
} ra_usb_phid_request_t;

/**
 * @enum ra_usb_phid_report_type_t
 ...
 */
typedef enum : uint8_t {
  k_ra_phid_report_type_input   = 0x01U, /**< Input report.   */
  k_ra_phid_report_type_output  = 0x02U, /**< Output report.  */
  k_ra_phid_report_type_feature = 0x03U, /**< Feature report. */
} ra_usb_phid_report_type_t;

/**
 * @enum ra_usb_phid_protocol_select_t
 ...
 */
typedef enum : uint8_t {
  k_ra_phid_proto_boot   = 0U, /**< Boot protocol.   */
  k_ra_phid_proto_report = 1U, /**< Report protocol. */
} ra_usb_phid_protocol_select_t;

/**
 * @enum ra_usb_phid_desc_t
 * @brief HID-specific descriptor types (USB HID 1.11 sec 7.1).
 */
typedef enum : uint8_t {
  k_ra_phid_desc_hid      = 0x21U, /**< HID class descriptor.    */
  k_ra_phid_desc_report   = 0x22U, /**< HID Report descriptor.   */
  k_ra_phid_desc_physical = 0x23U, /**< HID Physical descriptor. */
} ra_usb_phid_desc_t;

/**
 * @typedef ra_usb_phid_setup_fn_t
 * @brief Caller-supplied HID class-setup handler signature.
 *
 * @details The application registers one of these via
 * `ra_usb_phid_attach_setup_handler`. The class layer pre-decodes the
 ...
 */
typedef ra_err_t (*ra_usb_phid_setup_fn_t)(void* ctx, const ra_usb_setup_t* setup);
 ...
 * `ra_usb_device_attach` once descriptors are set.
 *
 * @param[in] speed Which USB controller (FS or HS).
 *
 * @return `ra_err_t` error code.
 * @retval k_ra_ok Device-HID ready.
 ...
 * @see ra_usb_phid_set_descriptors
 * @see ra_usb_phid_close
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_phid_init(ra_usb_speed_t speed);
 ...
 *       device-HID API calls return `k_ra_err_invalid_state`.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_phid_close(void);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_phid_set_descriptors(const uint8_t* report_desc,
 ...
 */
[[nodiscard]] ra_err_t
ra_usb_phid_send_report(uint8_t report_id, const uint8_t* payload, uint16_t len);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_phid_attach_setup_handler(ra_usb_phid_setup_fn_t setup_fn, void* ctx);
 ...
 *
 * @note Call from the `CTRT` ISR path of `ra_usb`.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_phid_handle_setup(const ra_usb_setup_t* setup);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_phid_get_idle(uint8_t* out_idle_rate);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_phid_get_protocol(ra_usb_phid_protocol_select_t* out_protocol);
