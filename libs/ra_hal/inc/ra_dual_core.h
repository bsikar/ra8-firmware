...
typedef enum : uintptr_t {
  k_ra_dual_core_ctrl_base_addr = 0x4000F000UL, /**< CPU_CTRL Secure base. */
} ra_dual_core_addr_t;
...
typedef enum : uint16_t {
  k_ra_dual_core_off_cpu1_initvtor = 0x044U, /**< CPU1INITVTOR (32-bit). */
  k_ra_dual_core_off_cpu1_waitcr   = 0x054U, /**< CPU1WAITCR   (8-bit).  */
  k_ra_dual_core_off_cpu1_actcsr   = 0x064U, /**< CPU1ACTCSR   (16-bit). */
} ra_dual_core_off_t;

/**
 * @enum ra_dual_core_field_t
 ...
 */
typedef enum : uint16_t {
  k_ra_dual_core_waitcr_cpuwait_mask = (1U << 0), /**< CPU1WAITCR.CPUWAIT. */
  k_ra_dual_core_actcsr_actreq_mask  = (1U << 0), /**< CPU1ACTCSR.ACTREQ.  */
  k_ra_dual_core_actcsr_act_mask     = (1U << 7), /**< CPU1ACTCSR.ACT.     */
  k_ra_dual_core_actcsr_key_shift    = 8U,        /**< Key byte position.  */
  k_ra_dual_core_actcsr_key_value    = 0xA5U,     /**< Required key code.  */
} ra_dual_core_field_t;
 ...
 * @retval k_ra_ok                Core released and confirmed active.
 ...
 * @retval k_ra_err_timeout       ACTCSR.ACT did not assert within the
 *                                bounded poll budget.
 ...
 *          ``ra_cpu1_is_running()`` will return false after this call.
 * @return ra_err_t Error code.
