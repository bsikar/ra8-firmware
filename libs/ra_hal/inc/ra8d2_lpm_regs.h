 ...
 * driver in ``libs/ra_hal/src/ra_lpm.c`` touches. Sources:
 *
 ...
 * ``k_ra_lpm_sysc_base_addr``; WUPEN offsets are relative to
 * ``k_ra_lpm_icu_base_addr``.
 */
typedef enum : uint16_t {
  ...
  k_ra_lpm_pdramscr1_off = 0x142U, /**< PDRAMSCR1 (8b)  -- HUM Ch 11.2.17 p 455. */

  /* --- SYSC: LDO standby --------------------------------------------- */
  k_ra_lpm_pll1ldocr_off = 0xB04U, /**< PLL1LDOCR  (8b) -- HUM Ch 11.2.40 p 475. */
  k_ra_lpm_pll2ldocr_off = 0xB08U, /**< PLL2LDOCR  (8b) -- HUM Ch 11.2.41 p 475. */
 ...
 * p 457 *plus* two pseudo-encodings (``k_ra_sleep_mode_sleep`` and
 * ``k_ra_sleep_mode_deep_sleep``) for which LPMD stays at 0
...
/**
 * @enum ra_lpm_sbycr_bits_t
 * @brief SBYCR bit positions and reset value (HUM Ch 11.2.18 p 456).
 */
typedef enum : uint8_t {
  k_ra_lpm_sbycr_ope_mask  = 0x40U, /**< OPE @ bit 6 -- bus-signal output keep. */
  k_ra_lpm_sbycr_reset_val = 0x40U, /**< Cold-reset value (OPE=1, all else 0).  */
} ra_lpm_sbycr_bits_t;

/**
 * @enum ra_lpm_dpsbycr_bits_t
 ...
 */
typedef enum : uint8_t {
  k_ra_lpm_dpsbycr_dcssmode_shift = 2U,    /**< DCSSMODE[1:0] starts at bit 2. */
  k_ra_lpm_dpsbycr_dcssmode_mask  = 0x0CU, /**< DCSSMODE[1:0] mask.            */
  k_ra_lpm_dpsbycr_iokeep_mask    = 0x40U, /**< IOKEEP @ bit 6.                */
  k_ra_lpm_dpsbycr_reset_val      = 0x14U, /**< Cold-reset value.              */
} ra_lpm_dpsbycr_bits_t;

/**
 * @enum ra_lpm_dcssmode_t
 ...
 */
typedef enum : uint8_t {
  k_ra_lpm_dcssmode_128us = 0x1U, /**< 128 us soft-start (cold-reset default). */
  k_ra_lpm_dcssmode_256us = 0x2U, /**< 256 us soft-start.                      */
  k_ra_lpm_dcssmode_512us = 0x3U, /**< 512 us soft-start.                      */
} ra_lpm_dcssmode_t;

/**
 * @enum ra_lpm_sscr1_bits_t
...
/**
 * @enum ra_lpm_lpscr_bits_t
 * @brief LPSCR bit fields (HUM Ch 11.2.20 p 457).
 */
typedef enum : uint8_t {
  k_ra_lpm_lpscr_lpmd_mask = 0x0FU, /**< LPMD[3:0] occupies bits 3..0. */
} ra_lpm_lpscr_bits_t;

/**
 * @enum ra_lpm_opccr_bits_t
...
/**
 * @enum ra_lpm_pdramscr0_groups_t
...
/**
 * @enum ra_lpm_pdramscr1_bits_t
 * @brief PDRAMSCR1 bit positions (HUM Ch 11.2.17 p 455).
 */
typedef enum : uint8_t {
  k_ra_lpm_pdramscr1_rkeep0_mask = 0x01U, /**< RKEEP0 @ bit 0 -- CPU0 TCM. */
  k_ra_lpm_pdramscr1_rkeep1_mask = 0x02U, /**< RKEEP1 @ bit 1 -- CPU1 TCM. */
} ra_lpm_pdramscr1_bits_t;

/**
 * @enum ra_lpm_ldocr_bits_t
 * @brief PLL[12]LDOCR / HOCOLDOCR shared bit fields
 *        (HUM Ch 11.2.40..42 p 475..476).
 */
typedef enum : uint8_t {
  k_ra_lpm_ldocr_ldostp_mask = 0x01U, /**< LDOSTP @ bit 0 -- stop in normal mode. */
  k_ra_lpm_ldocr_skeep_mask  = 0x02U, /**< SKEEP  @ bit 1 -- retain in standby.   */
  k_ra_lpm_ldocr_skeep_shift = 1U,    /**< SKEEP starts at bit 1.                 */
} ra_lpm_ldocr_bits_t;

/**
 * @enum ra_lpm_ldo_state_t
 * @brief LDOCR.SKEEP encoding for ``ra_lpm_set_ldo_standby``.
 *
 * @details
 ...
 */
typedef enum : uint8_t {
  k_ra_lpm_ldo_disabled = 0U, /**< LDO stops during standby (SKEEP=0). */
  k_ra_lpm_ldo_retained = 1U, /**< LDO retains state (SKEEP=1).        */
} ra_lpm_ldo_state_t;

/**
 * @enum ra_lpm_clock_stop_bits_t
 ...
 */
typedef enum : uint8_t {
  k_ra_lpm_clock_stop_mask = 0x01U, /**< STOP @ bit 0 in every OCR. */
} ra_lpm_clock_stop_bits_t;

/**
 * @enum ra_lpm_dpsier_idx_t
...
/**
 * @enum ra_lpm_wupen1_bits_t
 * @brief WUPEN1 bit masks for ULPT/I3C/SOSC/PDM/COMP sources
 *        (HUM Ch 14.2.20 p 552).
 */
typedef enum : uint32_t {
  k_ra_lpm_wupen1_comphs0 = 1UL << 3,  /**< COMPHS0WUPEN @ bit 3.  */
  k_ra_lpm_wupen1_sosc    = 1UL << 7,  /**< SOSCWUPEN    @ bit 7.  */
  k_ra_lpm_wupen1_ulpt0u  = 1UL << 8,  /**< ULP0UWUPEN   @ bit 8.  */
  k_ra_lpm_wupen1_ulpt0a  = 1UL << 9,  /**< ULP0AWUPEN   @ bit 9.  */
  k_ra_lpm_wupen1_ulpt0b  = 1UL << 10, /**< ULP0BWUPEN   @ bit 10. */
  k_ra_lpm_wupen1_i3c0    = 1UL << 11, /**< I3CWUPEN     @ bit 11. */
  k_ra_lpm_wupen1_ulpt1u  = 1UL << 12, /**< ULP1UWUPEN   @ bit 12. */
  k_ra_lpm_wupen1_ulpt1a  = 1UL << 13, /**< ULP1AWUPEN   @ bit 13. */
  k_ra_lpm_wupen1_ulpt1b  = 1UL << 14, /**< ULP1BWUPEN   @ bit 14. */
  k_ra_lpm_wupen1_pdm     = 1UL << 15, /**< PDMWUPEN     @ bit 15. */
} ra_lpm_wupen1_bits_t;
 ...
 *
 * @param[in] off Byte offset (one of ``k_ra_lpm_*_off``).
...
{
  return (volatile uint8_t*)(k_ra_lpm_sysc_base_addr + (uintptr_t)off);
}
...
{
  return (volatile uint16_t*)(k_ra_lpm_sysc_base_addr + (uintptr_t)off);
}

/**
 * @brief Return a typed pointer to a 32-bit ICU/WUPEN register.
 *
 * @param[in] off Byte offset (``k_ra_lpm_wupen0_off`` or
 *                ``k_ra_lpm_wupen1_off``).
 * @return Volatile pointer suitable for direct dereference.
 *
...
{
  return (volatile uint32_t*)(k_ra_lpm_icu_base_addr + (uintptr_t)off);
}
