 ...
 * argument at 4.
 *
 * Initialization sequence implemented by ``ra_pdg_init`` follows
 * Figure 23.2 (p 1160):
 *
 ...
 * - Multi-edge atomic batch updates: ``ra_pdg_set_delay_batch``.
 * - Runtime FRANGE switch: ``ra_pdg_set_frange`` re-runs the DLL
 * re-lock sequence with the new band, preserving channel state.
 * - Full GPT0/1/2/3 binding helpers: ``ra_pdg_bind_gpt_channel``
 * cracks open GTWP for the matching GPT channel before letting
 * delay-register writes through (HUM 23.2.3 p 1156).
 * - Per-pin enable/disable helpers ``ra_pdg_pin_bypass_set`` so a
 * single pin can stay bypassed while its sibling is delayed.
 * - Full status reporting: ``ra_pdg_get_status_full`` returns a
 * structured snapshot of every PDG bit.
 * - Compare-match constraint check: ``ra_pdg_check_constraints``
 * implements HUM Ch 23.4.2 Table 23.4 (p 1162).
 * - Register-write-interval helper ``ra_pdg_required_write_ns``
 * implements HUM Ch 23.4.3 (p 1163).
 *
 * @see ra_gpt.h GPT timer driver -- you must configure GPT320..323
 ...
 * GTDLYFn (falling) registers. Used by ``ra_pdg_set_delay`` /
 * ``ra_pdg_get_delay``.
 */
typedef enum : uint8_t {
  k_ra_pdg_edge_rising  = 0U, /**< GTDLYRnA / GTDLYRnB. */
  k_ra_pdg_edge_falling = 1U, /**< GTDLYFnA / GTDLYFnB. */
} ra_pdg_edge_t;
 ...
 */
typedef enum : uint8_t {
  k_ra_pdg_pin_a = 0U, /**< GTIOCnA -- "A" output of GPT32n. */
  k_ra_pdg_pin_b = 1U, /**< GTIOCnB -- "B" output of GPT32n. */
} ra_pdg_pin_t;

/**
 * @enum ra_pdg_status_t
 * @brief Bits surfaced by ``ra_pdg_get_status`` (legacy compact form).
 *
 * @details
 * The PDG has no dedicated status register -- "status" here is a
 * snapshot of GTDLYCR (DLLEN / DLYRST / FRANGE) packed into a
 * caller-friendly mask. ``ra_pdg_get_status`` reads GTDLYCR
...
/**
 * @enum ra_pdg_count_dir_t
 * @brief Counter direction passed to ``ra_pdg_check_constraints``.
 *
 * @details
 ...
 * here so that ``ra_pdg_check_constraints`` selects the right row.
 */
typedef enum : uint8_t {
...
/**
 * @struct ra_pdg_config_t
 * @brief Driver configuration descriptor for ``ra_pdg_init``.
 *
 * @details
 * cppcheck cannot see tests/ so it flags every field as unused;
 * each member is read in ``ra_pdg_init`` in
 * ``libs/ra_hal/src/ra_pdg.c``.
 *
 * @invariant ``frange`` must be one of the ``k_ra_pdg_frange_*``
...
/**
 * @struct ra_pdg_delay_entry_t
 * @brief Single-edge descriptor for the batch update API.
 *
 * @details
 * A flat array of these is handed to ``ra_pdg_set_delay_batch``
...
/**
 * @typedef ra_pdg_event_fn_t
 * @brief PDG event callback (DLL lock event, etc.).
 *
 * @param[in] ctx Caller context forwarded from ``ra_pdg_attach_handler``.
 *
 * @details
 ...
 */
typedef void (*ra_pdg_event_fn_t)(void* ctx);
 ...
 * @retval k_ra_ok Init complete.
 * @retval k_ra_err_null_ptr ``cfg`` was NULL.
 * @retval k_ra_err_invalid_arg ``cfg->frange`` out of range or
 * ``cfg->channel_mask`` selects bits
 * outside [0..3] or auto-tune saw
 * gptclk_hz == 0.
 * @retval k_ra_err_out_of_range Auto-tune saw gptclk_hz outside the
 * 80..300 MHz range allowed by HUM
 * Ch 23.2.1 p 1154.
 * @retval k_ra_err_hw_init_failed MSTP failed to release the block.
 *
 ...
 * @see ra_gpt.h
 * @see ra_pdg_set_frange Switch FRANGE at runtime without deinit.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_pdg_init(const ra_pdg_config_t* cfg);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_pdg_deinit(void);
 ...
 * @param[in] pin ``k_ra_pdg_pin_a`` (GTIOCnA) or
 * ``k_ra_pdg_pin_b`` (GTIOCnB).
 * @param[in] edge ``k_ra_pdg_edge_rising`` or
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok Code written.
 * @retval k_ra_err_invalid_arg Channel >= 4, pin/edge invalid, or
 * code > 0x7F.
 ...
 * 23.4) -- use ``ra_pdg_check_constraints`` first.
 ...
 */
[[nodiscard]] ra_err_t
ra_pdg_set_delay(uint8_t channel, ra_pdg_pin_t pin, ra_pdg_edge_t edge, uint8_t code);

/**
 * @brief Read back the last-programmed delay code.
 *
 * @param[in] channel PDG channel (0..3).
 * @param[in] pin ``k_ra_pdg_pin_a`` / ``k_ra_pdg_pin_b``.
 * @param[in] edge ``k_ra_pdg_edge_rising`` / ``k_ra_pdg_edge_falling``.
 ...
 */
[[nodiscard]] ra_err_t
ra_pdg_get_delay(uint8_t channel, ra_pdg_pin_t pin, ra_pdg_edge_t edge, uint8_t* out_code);
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok All entries written.
 * @retval k_ra_err_null_ptr ``entries`` is NULL.
 * @retval k_ra_err_invalid_arg Count out of range or any entry
 ...
 * 1/128 step at ``k_ra_pdg_frange_80_160_mhz``, 1/64 step at
 * ``k_ra_pdg_frange_155_300_mhz``. The result is rounded to the
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok Code computed.
 * @retval k_ra_err_null_ptr ``out_code`` is NULL.
 * @retval k_ra_err_invalid_arg ``gptclk_hz`` is zero or
 * ``frange`` out of range.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_pdg_exit_stop(uint8_t channel);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_pdg_enter_stop(uint8_t channel);
 ...
 * applied, 0 = bypass. ``ra_pdg_init`` already sets the bit for
 ...
 * cells (rising + falling). ``ra_pdg_pin_disable`` does that, and
 * ``ra_pdg_pin_enable`` restores caller-supplied codes.
 *
 * @param[in] channel 0..3.
 * @param[in] pin ``k_ra_pdg_pin_a`` or ``k_ra_pdg_pin_b``.
 *
 * @return ``ra_err_t`` error code.
 *
 * @pre PDG ``init`` has run.
 ...
 *
 * @param[out] out Receives a bitmask of ``ra_pdg_status_t`` bits.
 *
 * @return ``ra_err_t`` error code.
 *
 * @pre ``out`` is non-null.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_pdg_get_status(uint16_t* out);
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok Snapshot stored.
 * @retval k_ra_err_null_ptr ``out`` is NULL.
 *
 * @pre ``out`` non-null.
 ...
 * ``ra_pdg_enter_stop``. The "status mask" is in fact the GTDLYCR
 ...
 * ``k_ra_pdg_status_in_reset`` is meaningful today.
 *
 * @return ``ra_err_t`` error code.
 *
 * @pre PDG ``init`` has run.
 * @post GTDLYCR.DLYRST = 0 if ``mask`` includes
 * ``k_ra_pdg_status_in_reset``.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_pdg_clear_status(uint16_t mask);
 ...
 * registration point.
 *
 * @param[in] fn Callback invoked from ``ra_pdg_dispatch``.
 * @param[in] ctx Forwarded to ``fn``.
 *
 * @return ``k_ra_ok`` -- the registration cannot fail.
 *
 * @pre None.
 * @post Subsequent ``ra_pdg_dispatch`` calls invoke ``fn``.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_pdg_attach_handler(ra_pdg_event_fn_t fn, void* ctx);
 ...
 *
 * - 80..160 MHz -> ``k_ra_pdg_frange_80_160_mhz``
 * - 155..300 MHz -> ``k_ra_pdg_frange_155_300_mhz``
 *
 ...
 * @param[out] out Receives the chosen ``ra_pdg_frange_t``.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok Band selected.
 * @retval k_ra_err_null_ptr ``out`` is NULL.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok Switch complete.
 * @retval k_ra_err_invalid_arg ``new_frange`` not a legal band.
 * @retval k_ra_err_not_initialized PDG init has not run.
 *
 * @pre PDG ``init`` has run.
 ...
 * later via ``ra_pdg_unbind_gpt_channel``).
 *
 * @param[in] channel 0..3 (GPT320..GPT323).
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok GTWP cleared, binding active.
...
/**
 * @brief Reverse ``ra_pdg_bind_gpt_channel``, restoring write-protect.
 *
 * @param[in] channel 0..3.
 *
 * @return ``ra_err_t`` error code.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok Safe to write the delay register.
 * @retval k_ra_err_invalid_state Inside the forbidden window.
 *
 * @pre None.
