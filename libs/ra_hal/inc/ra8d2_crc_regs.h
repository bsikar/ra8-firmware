...
typedef enum : uintptr_t {
  k_ra_crc_base_addr = 0x40310000UL, /**< HUM Ch 48 p 3180 "CRC base". */
} ra_crc_addr_t;
...
/**
 * @enum ra_crc_bit_order_t
 * @brief Bit-order for CRC calculation, written to CRCCR0.LMS (bit 6).
 *
 * Mirrors FSP `crc_bit_order_t`. HUM Ch 48.2.1 p 3181.
 */
typedef enum : uint8_t {
  k_ra_crc_lsb_first = 0U, /**< LMS=0 LSB-first communication. */
  k_ra_crc_msb_first = 1U, /**< LMS=1 MSB-first communication. */
} ra_crc_bit_order_t;
 ...
 * @return Volatile pointer to the CRC registers at `k_ra_crc_base_addr`.
 */
