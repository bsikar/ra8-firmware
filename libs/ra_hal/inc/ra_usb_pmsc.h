 ...
 *
 * Lifecycle (mirrors `ra_usb_cdc.h` and `ra_usb_hmsc.h` shape):
 *
 *  1. `ra_usb_pmsc_init(speed)` flips the controller to device mode,
 ...
 *  2. `ra_usb_pmsc_attach_storage(storage)` registers a caller-owned
 ...
 *  3. `ra_usb_pmsc_step()` is invoked by the application from the
 *     bulk-OUT completion ISR. Each call pumps the BOT state
 *     machine: IDLE -> CBW_RX -> CDB_DECODE -> DATA_IN/OUT ->
 *     CSW_TX -> IDLE.
 *  4. `ra_usb_pmsc_close()` drops the bulk pipes and tears down the
...
typedef enum : uint8_t {
  k_ra_pmsc_pipe_bulk_in  = 3U, /**< PIPE3 -> local EP bulk IN.  */
  k_ra_pmsc_pipe_bulk_out = 4U, /**< PIPE4 -> local EP bulk OUT. */
} ra_usb_pmsc_pipe_t;

/**
 * @enum ra_usb_pmsc_endpoint_t
 ...
 */
typedef enum : uint8_t {
  k_ra_pmsc_ep_bulk_in  = 1U, /**< Bulk-IN endpoint number.  */
  k_ra_pmsc_ep_bulk_out = 2U, /**< Bulk-OUT endpoint number. */
} ra_usb_pmsc_endpoint_t;

/**
 * @enum ra_usb_pmsc_packet_t
 * @brief Packet sizing for the local bulk endpoints.
 */
typedef enum : uint16_t {
  k_ra_pmsc_bulk_max_packet_fs = 64U,  /**< Bulk size at full speed. */
  k_ra_pmsc_bulk_max_packet_hs = 512U, /**< Bulk size at high speed. */
} ra_usb_pmsc_packet_t;

/**
 * @enum ra_usb_pmsc_class_t
 ...
 */
typedef enum : uint8_t {
  k_ra_pmsc_class_msc     = 0x08U, /**< MSC interface class.       */
  k_ra_pmsc_subclass_scsi = 0x06U, /**< Transparent SCSI subclass. */
  k_ra_pmsc_protocol_bbb  = 0x50U, /**< Bulk-Only Transport.       */
} ra_usb_pmsc_class_t;

/**
 * @enum ra_usb_pmsc_request_t
 ...
 */
typedef enum : uint8_t {
  k_ra_pmsc_req_mass_storage_reset = 0xFFU, /**< 0-byte payload.  */
  k_ra_pmsc_req_get_max_lun        = 0xFEU, /**< 1-byte response. */
} ra_usb_pmsc_request_t;

/**
 * @enum ra_usb_pmsc_max_lun_t
 ...
 */
typedef enum : uint8_t {
  k_ra_pmsc_max_lun = 0U, /**< Highest valid LUN index. */
} ra_usb_pmsc_max_lun_t;

/**
 * @enum ra_usb_pmsc_csw_status_t
...
typedef enum : uint8_t {
  ...
  k_ra_pmsc_csw_status_phase_error = 0x02U, /**< BBB phase error -- the
                                                  host must run reset
                                                  recovery.            */
} ra_usb_pmsc_csw_status_t;

/**
 * @enum ra_usb_pmsc_scsi_t
 ...
 * `k_ra_pmsc_csw_status_failed` and a sense code of "Invalid Command
 * Operation Code".
 */
typedef enum : uint8_t {
  k_ra_pmsc_scsi_test_unit_ready  = 0x00U, /**< TEST UNIT READY.   */
  k_ra_pmsc_scsi_request_sense    = 0x03U, /**< REQUEST SENSE.     */
  k_ra_pmsc_scsi_inquiry          = 0x12U, /**< INQUIRY.           */
  k_ra_pmsc_scsi_mode_sense_6     = 0x1AU, /**< MODE SENSE(6).     */
  k_ra_pmsc_scsi_read_capacity_10 = 0x25U, /**< READ CAPACITY(10). */
  k_ra_pmsc_scsi_read_10          = 0x28U, /**< READ(10).          */
  k_ra_pmsc_scsi_write_10         = 0x2AU, /**< WRITE(10).         */
} ra_usb_pmsc_scsi_t;

/**
 * @enum ra_usb_pmsc_resp_size_t
 ...
 */
typedef enum : uint16_t {
  k_ra_pmsc_inquiry_resp_len       = 36U,  /**< INQUIRY response len.   */
  k_ra_pmsc_read_capacity_resp_len = 8U,   /**< READ_CAPACITY(10).      */
  k_ra_pmsc_request_sense_resp_len = 18U,  /**< REQUEST SENSE response. */
  k_ra_pmsc_mode_sense_resp_len    = 4U,   /**< MODE SENSE(6) response. */
  k_ra_pmsc_block_size_default     = 512U, /**< SCSI default block.     */
} ra_usb_pmsc_resp_size_t;

/**
 * @enum ra_usb_pmsc_inquiry_field_t
 ...
 *
 * @return `ra_err_t` error code. Non-zero failures land in the CSW
 *         as `k_ra_pmsc_csw_status_failed`.
 */
typedef ra_err_t (*ra_usb_pmsc_read_block_fn_t)(void*    ctx,
                                                uint32_t lba,
                                                uint32_t block_count,
                                                uint8_t* buf);

/**
 * @typedef ra_usb_pmsc_write_block_fn_t
 ...
 */
typedef ra_err_t (*ra_usb_pmsc_write_block_fn_t)(void*          ctx,
                                                 uint32_t       lba,
                                                 uint32_t       block_count,
                                                 const uint8_t* buf);

/**
 * @typedef ra_usb_pmsc_get_capacity_fn_t
 ...
 * @return `ra_err_t` error code.
 */
typedef ra_err_t (*ra_usb_pmsc_get_capacity_fn_t)(void*     ctx,
                                                  uint32_t* block_count,
                                                  uint32_t* block_size);

/**
 * @typedef ra_usb_pmsc_get_inquiry_fn_t
 ...
 * @return `ra_err_t` error code.
 */
typedef ra_err_t (*ra_usb_pmsc_get_inquiry_fn_t)(void*    ctx,
                                                 uint8_t* vendor8,
                                                 uint8_t* product16,
                                                 uint8_t* revision4);

/**
 * @struct ra_usb_pmsc_storage_t
 ...
 *
 * @code
 * static ra_err_t my_read(void* ctx, uint32_t lba, uint32_t n, uint8_t* b);
 * static ra_err_t my_write(void* ctx, uint32_t lba, uint32_t n, const uint8_t* b);
 * static ra_err_t my_cap(void* ctx, uint32_t* nb, uint32_t* bs);
 * static ra_err_t my_inq(void* ctx, uint8_t* v, uint8_t* p, uint8_t* r);
 *
 ...
 *
 * @details
 * Initialises the underlying `ra_usb` driver in DEVICE mode for
 ...
 *
 * @return `ra_err_t` error code.
 * @retval k_ra_ok Device-MSC ready, awaiting `attach_storage`.
 ...
 *
 * @post `ra_usb_device_init` succeeded for `speed`.
 * @post Internal BOT state machine in IDLE; storage backend NULL.
 *
 * @note Not thread-safe.
 * @see ra_usb_pmsc_attach_storage
 * @see ra_usb_pmsc_close
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pmsc_init(ra_usb_speed_t speed);
 ...
 *
 * @post `ra_usb_device_deinit` ran; D+ pull-up dropped; subsequent
 *       device-MSC API calls return `k_ra_err_invalid_state`.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pmsc_close(void);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pmsc_attach_storage(const ra_usb_pmsc_storage_t* storage);
 ...
 *
 * @pre `ra_usb_pmsc_init` has run.
 * @pre `ra_usb_pmsc_attach_storage` has run.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pmsc_step(void);
 ...
 * directly. Returns `k_ra_err_invalid_arg` on signature mismatch
 ...
 * @pre `cbw` non-NULL.
 *
 * @post On `k_ra_ok` the cached SCSI opcode and data-length are
 *       valid for `ra_usb_pmsc_dispatch_command`.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pmsc_feed_cbw(const uint8_t* cbw);

/**
 * @brief Run the SCSI command currently parked in the BOT state.
 *
 * @details
 * Test / debug entry point. After `ra_usb_pmsc_feed_cbw` accepts a
 ...
 *
 * @pre `ra_usb_pmsc_feed_cbw` returned `k_ra_ok` immediately prior.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_pmsc_dispatch_command(uint8_t*                  data_buf,
 ...
 * @pre A prior `ra_usb_pmsc_feed_cbw` set the dCBWTag we echo.
 *
