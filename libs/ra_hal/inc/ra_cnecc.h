 ...
 *
 * Lifecycle:
 *  - ``ra_cnecc_init``                 -- bring up both instances
 *  - ``ra_cnecc_deinit``               -- tear down both instances
 *  - ``ra_cnecc_enable_instance``      -- per-instance enable
 *  - ``ra_cnecc_disable_instance``     -- per-instance disable
 *  - ``ra_cnecc_enter_standby``        -- HUM 42.5.1 standby prep
 *  - ``ra_cnecc_exit_standby``         -- HUM 42.5.2 standby exit
 *
 * Configuration:
 *  - ``ra_cnecc_set_irq_enables``      -- per-instance IRQ mask
 *  - ``ra_cnecc_set_correction_permission`` -- per-instance EC1ECP
 *
 * Status / counters:
 *  - ``ra_cnecc_get_status``           -- snapshot live + counters
 *  - ``ra_cnecc_clear_status``         -- W0C the status flags
 *  - ``ra_cnecc_get_counters``         -- read-only counter bundle
 *  - ``ra_cnecc_reset_counters``       -- zero counters per instance
 *  - ``ra_cnecc_set_counter_mirror``   -- attach BBR-mirrored counters
 *
 * Fault injection (HUM 42.3.2 Figure 42.2):
 *  - ``ra_cnecc_inject_fault``         -- run the four-step procedure
 *  - ``ra_cnecc_test_mode_disable``    -- force EC710TMC = 0x8000
 *  - ``ra_cnecc_test_mode_active``     -- query ECTMCE bit
 *
 * IRQ wiring (HUM 42.4):
 *  - ``ra_cnecc_attach_handler``       -- callback registration
 *  - ``ra_cnecc_attach_isr``           -- auto-wire ICU vectors
 *  - ``ra_cnecc_detach_isr``           -- unwire ICU vectors
 *  - ``ra_cnecc_isr_handler``          -- generic ICU trampoline
 *  - ``ra_cnecc_dispatch``             -- direct dispatch (test/manual)
 *
...
/**
 * @struct ra_cnecc_instance_cfg_t
 ...
 * read in ``ra_cnecc.c`` and flags every member as unused -- the
...
} ra_cnecc_instance_cfg_t;
/* cppcheck-suppress-end [unusedStructMember] */

/**
 * @struct ra_cnecc_config_t
...
/* cppcheck-suppress-begin [unusedStructMember] */
typedef struct {
  ra_cnecc_instance_cfg_t instances[k_ra_cnecc_instance_count]; /**< Per-instance settings. */
} ra_cnecc_config_t;
/* cppcheck-suppress-end [unusedStructMember] */
...
/**
 * @struct ra_cnecc_status_t
 * @brief Snapshot of one CNECC instance, returned by ``ra_cnecc_get_status``.
 *
 * @details
 ...
 * ``ra_cnecc_dispatch`` -- the hardware itself only latches "was
...
/**
 * @struct ra_cnecc_counters_t
 * @brief Cumulative per-instance fault counters.
 *
 * @details
 * Returned by ``ra_cnecc_get_counters`` for callers that only care
 * about the soft counters (without the live register snapshot
 * provided by ``ra_cnecc_get_status``). When a BBR mirror is
 * attached via ``ra_cnecc_set_counter_mirror`` the values returned
 ...
 */
typedef void (*ra_cnecc_error_fn_t)(void* ctx, uint8_t instance, bool is_2bit, uint16_t err_addr);
 ...
 * @pre  ``ra_cnecc_init`` was previously called (deinit is idempotent
 *       even if not).
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Judgment enabled.
 * @retval k_ra_err_invalid_arg ``instance`` >= ``k_ra_cnecc_instance_count``.
 *
 * @pre  ``ra_cnecc_init`` has run.
 ...
 * caller can re-enable later via ``ra_cnecc_enable_instance``.
 *
 * @param[in] instance Instance index 0..1.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Judgment disabled.
 * @retval k_ra_err_invalid_arg ``instance`` >= ``k_ra_cnecc_instance_count``.
 *
 * @pre  ``ra_cnecc_init`` has run.
 * @pre  Caller has masked IRQs or runs single-threaded.
 ...
 * ``ra_cnecc_init`` call so per-instance ``correct_1bit`` /
 * ``irq_1bit`` / ``irq_2bit`` survive across the standby trip.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                  Both instances re-armed.
 * @retval k_ra_err_not_initialized ``ra_cnecc_init`` was never called.
 *
 * @pre  ``ra_cnecc_init`` has run at least once before standby.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Mask updated.
 * @retval k_ra_err_invalid_arg ``instance`` out of range.
 *
 * @pre  ``ra_cnecc_init`` has run.
 * @pre  Caller has masked IRQs or runs single-threaded.
 ...
 * ``ra_cnecc_dispatch``) -- the hardware only carries the "at least
 ...
 * ``ra_cnecc_get_status`` snapshot. Returns the same counters that
 * ``ra_cnecc_dispatch`` increments and that the optional BBR mirror
 * tracks.
 *
 * @param[in]  instance Instance index 0..1.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Counters copied.
 * @retval k_ra_err_null_ptr    ``out`` was NULL.
 * @retval k_ra_err_invalid_arg ``instance`` out of range.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Counters zeroed (in driver and BBR mirror).
 * @retval k_ra_err_invalid_arg ``instance`` out of range.
 *
 * @pre  ``ra_cnecc_init`` has run.
 ...
 *       ``ra_cnecc_clear_status`` for that).
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
 ...
 * 32 ``VBTBKRn`` words exposed by ``ra_bkup``) so the lifetime
 * counts persist across cold boots. Callers allocate a
 * ``ra_cnecc_counters_t`` in BBR memory, hand the pointer to this
 * function, and the driver mirrors every ``ra_cnecc_dispatch`` /
 * ``ra_cnecc_reset_counters`` write into it.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Mirror attached / detached.
 * @retval k_ra_err_invalid_arg ``instance`` out of range.
 *
 * @pre  ``ra_cnecc_init`` has run.
 * @pre  Caller manages the lifetime of ``*mirror``; the driver
 *       does not copy it.
 * @post Subsequent ``ra_cnecc_dispatch`` calls update both the
 *       driver-local counter and ``*mirror``.
 * @post Subsequent ``ra_cnecc_reset_counters`` calls zero both.
 *
 ...
 * cumulative counters are NOT zeroed (use ``ra_cnecc_reset_counters``
 * for that).
 *
 * @param[in] instance Instance index 0..1.
 *
 ...
 * ``ra_cnecc_dispatch`` -- this helper only programs the registers.
 *
 ...
 *
 * Call ``ra_cnecc_test_mode_disable`` after the test to restore
 * normal operation.
 *
 * @param[in] instance Instance index 0..1.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Test-mode programmed.
 * @retval k_ra_err_null_ptr    ``req`` was NULL.
 * @retval k_ra_err_invalid_arg ``instance`` out of range.
 *
 * @pre  ``ra_cnecc_init`` has run with the matching instance enabled.
 ...
 * @see ra_cnecc_test_mode_disable
 * @since 0.1.0
 */
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Test mode disabled.
 * @retval k_ra_err_invalid_arg ``instance`` out of range.
 *
 * @pre  ``ra_cnecc_init`` has run.
 * @pre  Caller has masked IRQs or runs single-threaded.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Query succeeded.
 * @retval k_ra_err_null_ptr    ``out`` was NULL.
 * @retval k_ra_err_invalid_arg ``instance`` out of range.
 *
 * @pre  ``out`` points to writable storage.
 ...
 * @post Subsequent ``ra_cnecc_dispatch`` calls fire ``fn``.
 ...
 * one shot to keep the API simple. Detach via ``ra_cnecc_detach_isr``.
 *
 * @param[in] priority NVIC priority shared by both vectors
 *                     (0..k_ra_isr_prio_max).
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                Both vectors wired.
 * @retval k_ra_err_invalid_arg   ``priority`` out of range.
 * @retval k_ra_err_no_mem        No free IELSR slot.
 * @retval k_ra_err_exists        One of the events was already routed.
 *
 * @pre  ``ra_isr_init`` and ``ra_cnecc_init`` have run.
 * @pre  Caller has registered a fault callback via
 *       ``ra_cnecc_attach_handler`` (the dispatcher is harmless
 ...
 *       ``ra_cnecc_dispatch`` to the registered callback.
 *
 * @note Not thread-safe.
 * @see ra_cnecc_detach_isr
 * @since 0.1.0
 */
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok            Both vectors released (or already gone).
 *
 * @pre  ``ra_cnecc_attach_isr`` was previously called.
 * @pre  Caller has masked IRQs or runs single-threaded.
 ...
 * ``ra_cnecc_attach_isr``; ``ctx`` carries the instance index
 * cast to a ``void*``. Reads the instance's ``EC710CTL`` /
 * ``EC710EAD0``, decides between 1-bit, 2-bit, or pure-overflow,
 * forwards to ``ra_cnecc_dispatch``, then W0Cs the latched flags
 ...
 *
 * @details
 * Drives the registered ``ra_cnecc_error_fn_t`` callback and bumps
 ...
 *
 * @pre  ``instance < k_ra_cnecc_instance_count`` OR the call is dropped.
 ...
 *
 * @details
 * Called by ``ra_cnecc_isr_handler`` when ``ECOVFF`` is set so the
