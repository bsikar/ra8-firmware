 ...
 * sub-mode:
 *
 * - **Commander** (``k_ra_sci_lin_role_commander``): hardware GENERATES the
 ...
 * - **Responder** (``k_ra_sci_lin_role_responder``): hardware DETECTS the
 ...
 * async-UART bring-up in ``ra_sci.h``: ``ra_sci_lin_init`` configures the
 ...
 *
 * Lifecycle:
 * - ``ra_sci_lin_init``          -- configure SCI_B for a LIN role
 *
 ...
 * - ``ra_sci_lin_break_detected``-- poll XSR0.BFDF once (non-blocking)
 * - ``ra_sci_lin_wait_break``    -- block until a break field is detected
 * - ``ra_sci_lin_clear_status``  -- clear the XSR0 LIN latches via XFCLR
 * - ``ra_sci_lin_check_header``  -- pure: validate a received SYNC + PID
 *
 * Response (data) phase, either role:
 * - ``ra_sci_lin_send_response`` -- publish data bytes + checksum
 * - ``ra_sci_lin_read_response`` -- subscribe: read data bytes + checksum
 * - ``ra_sci_lin_check_response``-- pure: verify a received response checksum
 *
 * Pure helpers:
 ...
 */
typedef enum : uint8_t {
  k_ra_sci_lin_role_commander = 0U, /**< Generate the header (bus controller). */
  k_ra_sci_lin_role_responder = 1U, /**< Detect the header (subordinate node). */
} ra_sci_lin_role_t;
...
typedef struct {
  ...
  ra_sci_lin_role_t      role;            /**< Commander (generate) or responder
 ...
 * per frame by ``ra_sci_lin_send_break``); a responder sets XCR1.SDST
 ...
 * @retval k_ra_ok               Channel configured for the requested role.
 * @retval k_ra_err_null_ptr     ``cfg`` was NULL or ``channel`` out of range.
 * @retval k_ra_err_invalid_arg  ``role`` undefined, ``timer_clk`` not a defined
 ...
 * ``ra_sci_lin_clear_status``. Reads no data and modifies no hardware.
 *
 * @param[in]  channel      SCI channel number (0..9).
 * @param[out] out_detected Receives true if XSR0.BFDF is set, else false.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Flag state written to ``*out_detected``.
 * @retval k_ra_err_null_ptr    ``out_detected`` is NULL or ``channel`` out of
 *                              range.
 *
 * @pre ``out_detected`` is non-NULL.
 * @pre Channel previously configured as a responder via ``ra_sci_lin_init``.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_sci_lin_break_detected(uint8_t channel, bool* out_detected);
 ...
 * field is detected. On the host (``RA_SIMULATOR_MODE``) the poll runs
 * against the programmable MMIO seam, so a test pre-stages BFDF or arms a
 * timeout. Does not clear the flag -- call ``ra_sci_lin_clear_status``
 * before waiting for the next frame.
 *
 * @param[in] channel SCI channel number (0..9).
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok             A break field was detected within the budget.
 * @retval k_ra_err_null_ptr   ``channel`` out of range.
 * @retval k_ra_err_hw_timeout No break field detected within the budget.
 *
 * @pre Channel previously configured as a responder via ``ra_sci_lin_init``.
 ...
 *
 * @note Thread safety: not thread-safe.
 * @see ra_sci_lin_break_detected
 * @see ra_sci_lin_clear_status
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_sci_lin_wait_break(uint8_t channel);
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok            LIN status latches cleared.
 * @retval k_ra_err_null_ptr  ``channel`` out of range.
 *
 * @pre Channel previously configured via ``ra_sci_lin_init``.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_sci_lin_clear_status(uint8_t channel);
 ...
 * in for the enhanced mode) via ``ra_sci_lin_checksum``, then clocks the
 * ``len`` data bytes and the checksum byte out as ordinary UART frames
 * with ``ra_sci_putc_polling``. Used by the transmitting node of a frame:
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Data + checksum clocked out.
 * @retval k_ra_err_null_ptr    ``channel`` out of range, or ``data`` NULL with
 *                              ``len`` > 0.
 * @retval k_ra_err_invalid_arg ``mode`` undefined or ``len`` is 0 or > 8.
 ...
 *
 * @note Thread safety: not thread-safe.
 * @see ra_sci_lin_read_response
 * @see ra_sci_lin_checksum
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_sci_lin_send_response(uint8_t                    channel,
 ...
 * byte into ``*out_checksum``, each via ``ra_sci_putc_polling``'s receive
 * peer ``ra_sci_getc_polling``. This is a thin I/O primitive: it does NOT
 * validate the checksum -- the caller passes ``out_data`` / ``*out_checksum``
 * to ``ra_sci_lin_check_response`` to verify. Used by the receiving node
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              ``len`` data bytes + checksum read.
 * @retval k_ra_err_null_ptr    ``channel`` out of range, or ``out_data`` /
 *                              ``out_checksum`` is NULL.
 * @retval k_ra_err_invalid_arg ``len`` is 0 or > 8.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok           Decision written to ``*out_valid`` / ``*out_id``.
 * @retval k_ra_err_null_ptr ``out_id`` or ``out_valid`` is NULL.
 *
 ...
 */
[[nodiscard]] ra_err_t
ra_sci_lin_check_header(uint8_t sync, uint8_t pid, uint8_t* out_id, bool* out_valid);
 ...
 * for the enhanced mode) with ``ra_sci_lin_checksum`` and compares it to
 * the ``received`` checksum byte, writing the match result to
 * ``*out_valid``. Pure: touches no hardware register. Pair with
 * ``ra_sci_lin_read_response`` to validate an inbound response.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Decision written to ``*out_valid``.
 * @retval k_ra_err_null_ptr    ``out_valid`` is NULL, or ``data`` NULL with
 *                              ``len`` > 0.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_sci_lin_check_response(ra_sci_lin_checksum_mode_t mode,
                                                 uint8_t                    pid,
                                                 const uint8_t*             data,
                                                 uint8_t                    len,
                                                 uint8_t                    received,
