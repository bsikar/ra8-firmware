 ...
 * `ra_ceu_config_t` open-time descriptor, the live-status snapshot
 * struct, the address-bundle struct, and the event-callback typedef.
 * The function prototypes that consume these types live in
 * `ra_ceu_api.h`; both are aggregated by the thin umbrella
 * `ra_ceu.h`. See that umbrella for the full driver overview and
 * state machine.
 *
