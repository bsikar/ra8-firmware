 ...
 * window (`0x4001E000`) so the driver in `libs/ra_hal/src/ra_vreg.c`
 * can stay self-contained without extending the shared
 * `ra8d2_system_regs.h`. The offsets are taken from the FSP CMSIS
 ...
 * have to pull in `ra8d2_system_regs.h`.
 */
typedef enum : uintptr_t {
  k_ra_vreg_sysc_base_addr = 0x4001E000UL, /**< R_SYSTEM (SYSC) base. */
} ra_vreg_addr_t;
...
/**
 * @enum ra_vreg_dcdcctl_mask_t
 * @brief DCDCCTL bit masks.
 *
 * @details
 * `k_ra_vreg_mask_dcdcctl_all` is the union of every writable bit
 * (0,1,4,5,6,7 = 0xF3) and is used by `ra_vreg_clear_status()` to
 ...
 * this via the `k_ra_vreg_lvo_*` enum.
 */
typedef enum : uint8_t {
  k_ra_vreg_mask_lvo0e   = 0x01U, /**< LVO0E -- low-voltage profile 0 enable. */
  k_ra_vreg_mask_lvo1e   = 0x02U, /**< LVO1E -- low-voltage profile 1 enable. */
  k_ra_vreg_mask_lvo_all = 0x03U, /**< Union of every LVOCR writable bit.     */
} ra_vreg_lvocr_mask_t;

/**
 * @enum ra_vreg_timing_us_t
 ...
 * memory window; on the host unit-test build the simulated mmap
 * (`tests/mocks/ra_sim_mmap.c`) maps anonymous RAM at the same
 ...
 */
static inline volatile uint8_t* ra_vreg_dcdcctl(void)
{
  return (volatile uint8_t*)(k_ra_vreg_sysc_base_addr + (uintptr_t)k_ra_vreg_off_dcdcctl);
}
 ...
 */
static inline volatile uint8_t* ra_vreg_vccsel(void)
{
  return (volatile uint8_t*)(k_ra_vreg_sysc_base_addr + (uintptr_t)k_ra_vreg_off_vccsel);
}
 ...
 */
static inline volatile uint8_t* ra_vreg_lvocr(void)
{
  return (volatile uint8_t*)(k_ra_vreg_sysc_base_addr + (uintptr_t)k_ra_vreg_off_lvocr);
}
