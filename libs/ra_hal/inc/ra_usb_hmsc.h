 ...
 *
 * Lifecycle (mirrors `ra_usb_hcdc.h` shape):
 *
 *  1. `ra_usb_hmsc_init(speed)` flips the controller to host mode,
 ...
 *  5. `ra_usb_hmsc_close()` drops bus power and releases the device.
 *
 * The starter only tracks a single attached MSC device with up to
 * `k_ra_hmsc_max_lun` logical units; hubs and CD-ROM (ATAPI) command
...
typedef enum : uint8_t {
  k_ra_hmsc_pipe_bulk_in  = 3U, /**< PIPE3 -> attached EP bulk IN.  */
  k_ra_hmsc_pipe_bulk_out = 4U, /**< PIPE4 -> attached EP bulk OUT. */
} ra_usb_hmsc_pipe_t;

/**
 * @enum ra_usb_hmsc_packet_t
 * @brief Packet sizing for the attached device's bulk endpoints.
 */
typedef enum : uint16_t {
  k_ra_hmsc_bulk_max_packet_fs = 64U,  /**< Bulk size at full speed. */
  k_ra_hmsc_bulk_max_packet_hs = 512U, /**< Bulk size at high speed. */
} ra_usb_hmsc_packet_t;

/**
 * @enum ra_usb_hmsc_class_t
 ...
 */
typedef enum : uint8_t {
  k_ra_hmsc_class_msc     = 0x08U, /**< MSC interface class.       */
  k_ra_hmsc_subclass_scsi = 0x06U, /**< Transparent SCSI subclass. */
  k_ra_hmsc_protocol_bbb  = 0x50U, /**< Bulk-Only Transport.       */
} ra_usb_hmsc_class_t;

/**
 * @enum ra_usb_hmsc_request_t
 ...
 */
typedef enum : uint8_t {
  k_ra_hmsc_req_mass_storage_reset = 0xFFU, /**< 0-byte payload.  */
  k_ra_hmsc_req_get_max_lun        = 0xFEU, /**< 1-byte response. */
} ra_usb_hmsc_request_t;

/**
 * @enum ra_usb_hmsc_max_lun_t
 ...
 */
typedef enum : uint8_t {
  k_ra_hmsc_max_lun = 4U, /**< Max LUNs the starter tracks. */
} ra_usb_hmsc_max_lun_t;

/**
 * @enum ra_usb_hmsc_csw_status_t
 ...
 */
typedef enum : uint8_t {
  k_ra_hmsc_csw_status_passed      = 0x00U, /**< Command succeeded.    */
  k_ra_hmsc_csw_status_failed      = 0x01U, /**< Command failed.       */
  k_ra_hmsc_csw_status_phase_error = 0x02U, /**< BBB phase error -- need
                                                  reset recovery.       */
} ra_usb_hmsc_csw_status_t;

/**
 * @enum ra_usb_hmsc_scsi_t
 ...
 */
typedef enum : uint8_t {
  k_ra_hmsc_scsi_test_unit_ready  = 0x00U, /**< TEST UNIT READY.   */
  k_ra_hmsc_scsi_request_sense    = 0x03U, /**< REQUEST SENSE.     */
  k_ra_hmsc_scsi_inquiry          = 0x12U, /**< INQUIRY.           */
  k_ra_hmsc_scsi_read_capacity_10 = 0x25U, /**< READ CAPACITY(10). */
  k_ra_hmsc_scsi_read_10          = 0x28U, /**< READ(10).          */
  k_ra_hmsc_scsi_write_10         = 0x2AU, /**< WRITE(10).         */
} ra_usb_hmsc_scsi_t;

/**
 * @enum ra_usb_hmsc_resp_size_t
 ...
 */
typedef enum : uint16_t {
  k_ra_hmsc_inquiry_resp_len       = 36U,  /**< INQUIRY response len. */
  k_ra_hmsc_read_capacity_resp_len = 8U,   /**< READ_CAPACITY(10).    */
  k_ra_hmsc_block_size_default     = 512U, /**< SCSI default block.  */
} ra_usb_hmsc_resp_size_t;

/**
 * @struct ra_usb_hmsc_device_t
...
/**
 * @struct ra_usb_hmsc_inquiry_response_t
 ...
 */
typedef void (*ra_usb_hmsc_attach_fn_t)(void* ctx, const ra_usb_hmsc_device_t* device);
 ...
 * @retval k_ra_ok Host-MSC ready, awaiting attach.
 ...
 * @see ra_usb_hmsc_attach_callback
 * @see ra_usb_hmsc_close
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hmsc_init(ra_usb_speed_t speed);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hmsc_attach_callback(ra_usb_hmsc_attach_fn_t on_attach, void* ctx);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hmsc_inquiry(uint8_t                         target_lun,
                                           ra_usb_hmsc_inquiry_response_t* response);
 ...
 */
[[nodiscard]] ra_err_t
ra_usb_hmsc_read_capacity(uint8_t target_lun, uint32_t* block_count, uint32_t* block_size);
 ...
 *
 * @return `ra_err_t` error code.
 * @retval k_ra_ok CBW constructed.
 * @retval k_ra_err_null_ptr `cdb` or `out_cbw` was NULL.
 * @retval k_ra_err_invalid_arg `cdb_len` out of range or
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hmsc_build_cbw(uint8_t        target_lun,
 ...
 *
 * @return `ra_err_t` error code.
 * @retval k_ra_ok Signature + tag match; `*out_status` valid.
 * @retval k_ra_err_null_ptr Either pointer was NULL.
 * @retval k_ra_err_invalid_arg Signature mismatch or tag mismatch.
 *
 * @pre `csw`, `out_status` non-NULL.
 *
 * @post On success `*out_status` is one of
 *       `k_ra_hmsc_csw_status_passed`, `..._failed`, `..._phase_error`.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hmsc_decode_csw(const uint8_t*            csw,
