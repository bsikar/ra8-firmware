 ...
 * the `ra_npu` driver's submit -> run -> poll -> read-output protocol can be
 ...
 *    the NPU command queue at it (`ra_npu_submit`), and kicks it (`ra_npu_run`).
 *  - `tools/board_sim/src/board_periph_npu.c` DECODES a stream in this layout and
 ...
 *  - `tests/test_ra_npu.c` MOCKS the same decode against the host MMIO backing
 ...
 * `RA_HAS_NPU`: the host-side board_sim model (which does not target the RA8P1)
 * must include it too.
 *
 ...
 * bits carry ::k_ra_npu_sim_magic (an "SE55" = Sim-Ethos-U55 marker) and the low
 * 16 bits carry a ::ra_npu_sim_op_t opcode. The marker lets a decoder tell a sim
 ...
 *
 * @invariant ::k_ra_npu_sim_magic has no bits in common with ::k_ra_npu_sim_op_mask.
 * @see ra_npu_sim_op_t
 * @since 0.1.0
 */
 ...
 * the destination region referenced by the command header. ::k_ra_npu_sim_op_copy
 * copies bytes verbatim; ::k_ra_npu_sim_op_addk adds a constant to every byte
 ...
 *
 * @invariant Every opcode fits in ::k_ra_npu_sim_op_mask (16 bits).
 * @see ra_npu_sim_word0_t
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_ra_npu_sim_op_copy = 0x0001U, /**< dst[i] = src[i].                     */
  k_ra_npu_sim_op_addk = 0x0002U, /**< dst[i] = (src[i] + constant) & 0xFF. */
} ra_npu_sim_op_t;

/**
 * @enum ra_npu_sim_word_idx_t
 * @brief 32-bit word indices of the fixed sim command-stream header.
 *
 * @details
 * The header is ::k_ra_npu_sim_word_num little-endian 32-bit words:
 *  - ::k_ra_npu_sim_word_op    -- `magic | opcode` (see ::ra_npu_sim_word0_t).
 *  - ::k_ra_npu_sim_word_src   -- source region index (BASEPn the op reads).
 *  - ::k_ra_npu_sim_word_dst   -- destination region index (BASEPn the op writes).
 *  - ::k_ra_npu_sim_word_count -- element count in bytes (1 .. ::k_ra_npu_sim_max_bytes).
 *  - ::k_ra_npu_sim_word_const -- constant addend (used by add-constant only).
 *
 * @invariant ::k_ra_npu_sim_word_num equals the number of populated header words.
 * @see ra_npu_sim_bound_t
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_ra_npu_sim_word_op    = 0U, /**< magic | opcode.                      */
  k_ra_npu_sim_word_src   = 1U, /**< Source region index (0 .. 7).        */
  k_ra_npu_sim_word_dst   = 2U, /**< Destination region index (0 .. 7).   */
  k_ra_npu_sim_word_count = 3U, /**< Byte count to process.               */
  k_ra_npu_sim_word_const = 4U, /**< Constant addend (add-constant op).   */
  k_ra_npu_sim_word_num   = 5U, /**< Header word count (populated words). */
} ra_npu_sim_word_idx_t;

/**
 * @enum ra_npu_sim_bound_t
 * @brief Size bounds and the 8-bit element mask of the sim convention.
 *
 * @details
 * ::k_ra_npu_sim_header_bytes is the fixed header byte length (word count times
 * four) a submitted stream must be at least as long as. ::k_ra_npu_sim_max_bytes
 * bounds a transfer so every decoder loop has a static upper bound (NASA Power of
 * 10 Rule 2). ::k_ra_npu_sim_byte_mask is the 8-bit wrap applied by the
 * add-constant opcode.
 *
 * @invariant ::k_ra_npu_sim_header_bytes == 4 * ::k_ra_npu_sim_word_num.
 * @see ra_npu_sim_word_idx_t
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_ra_npu_sim_header_bytes = 20U,   /**< Fixed header length in bytes (5 words). */
  k_ra_npu_sim_max_bytes    = 4096U, /**< Max processed bytes (bounds the loop).  */
  k_ra_npu_sim_byte_mask    = 0xFFU, /**< 8-bit element wrap (add-constant op).   */
} ra_npu_sim_bound_t;
