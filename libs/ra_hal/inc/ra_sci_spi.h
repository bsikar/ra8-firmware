...
/**
 * @brief Configuration descriptor for ::ra_sci_spi_init.
 *
 ...
 *
 * @retval k_ra_ok Channel is enabled (TE+RE) and ready to transfer.
 * @retval k_ra_err_null_ptr ``cfg`` is NULL.
 * @retval k_ra_err_invalid_arg ``channel`` out of range or baud 0.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_sci_spi_init(uint8_t channel, const ra_sci_spi_cfg_t* cfg);
 ...
 * @pre ::ra_sci_spi_init succeeded for this channel.
 * @post TE/RE are cleared and the MSTP gate is released.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_sci_spi_deinit(uint8_t channel);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_sci_spi_set_clock(uint8_t channel, uint32_t baud_hz, uint32_t pclk_hz);
 ...
 * @retval k_ra_ok Frame completed.
 * @retval k_ra_err_invalid_arg ``channel`` out of range.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_sci_spi_xfer8(uint8_t channel, uint8_t tx, uint8_t* rx);
 ...
 * @retval k_ra_ok All bytes transferred.
 * @retval k_ra_err_invalid_arg ``channel`` out of range.
 ...
 */
[[nodiscard]] ra_err_t
ra_sci_spi_xfer(uint8_t channel, const uint8_t* tx, uint8_t* rx, uint32_t len);
