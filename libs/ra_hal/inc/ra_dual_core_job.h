 ...
 * @ref ra_dual_core_compile_dispatch_fn with the source `.epub` bytes and an
 * output buffer; the production implementation stages the bytes into shared
 * memory, releases CPU1 (see `ra_cpu1_release`), posts the job to the shared
...
/**
 * @enum ra_dual_core_job_status_t
 ...
 *          maps any non-`ok` status to a non-zero @ref ra_err_t for its caller.
 *
 * @invariant Exactly one status is live per job; ::k_ra_dual_core_job_running is
 *            the initial value the primary core writes before posting the job.
 *
 * @code
 * if (mb->status == (uint32_t)k_ra_dual_core_job_ok) { use_blob(mb); }
 * @endcode
 *
...
typedef enum : uint32_t {
  k_ra_dual_core_job_running    = 0U, /**< Worker core is still building the blob.   */
  k_ra_dual_core_job_ok         = 1U, /**< Compile finalized a valid RABOOK1 blob.   */
  k_ra_dual_core_job_build_fail = 2U, /**< A compile stage overflowed or failed.     */
  k_ra_dual_core_job_open_fail  = 3U, /**< The source `.epub` could not be parsed.   */
  k_ra_dual_core_job_no_request = 4U, /**< No job was posted (request magic absent). */
} ra_dual_core_job_status_t;

/**
 * @typedef ra_dual_core_compile_dispatch_fn
 ...
 *          the returned blob with `ra_book_validate` before trusting it.
 *
 ...
 * @param[out] out_len  Receives the produced blob length on @c k_ra_ok.
 *
 * @return Error code.
 * @retval k_ra_ok           A valid-status blob of @p *out_len bytes is in @p out_buf.
 * @retval k_ra_err_null_ptr A required pointer argument is NULL.
 * @retval k_ra_err_no_mem   The produced blob did not fit @p out_cap.
 * @retval k_ra_err_hw_error The worker reported a non-ok status or never finished.
 *
 * @note Implementations run single-threaded from the primary core's request path.
 * @see ra_dual_core_job_status_t
 * @since 0.1.0
 */
typedef ra_err_t (*ra_dual_core_compile_dispatch_fn)(void*          ctx,
                                                     const uint8_t* epub,
                                                     uint32_t       epub_len,
                                                     uint8_t*       out_buf,
                                                     uint32_t       out_cap,
