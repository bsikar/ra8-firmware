 ...
 * use the ``ra_sram_regs()`` and ``ra_sram_cpscu_regs()`` accessors so
...
typedef enum : uint8_t {
  k_ra_sram_bank_count = 4U, /**< SRAM0, SRAM1, SRAM2, SRAM3. */
} ra_sram_bank_count_t;

/**
 * @enum ra_sram_ear_pair_t
 ...
 */
typedef enum : uint32_t {
  k_ra_sram_iclk_threshold_250mhz = 125000000UL, /**< 250 MHz max -> 125 MHz threshold. */
  k_ra_sram_iclk_threshold_200mhz = 100000000UL, /**< 200 MHz max -> 100 MHz threshold. */
  k_ra_sram_iclk_threshold_150mhz = 75000000UL,  /**< 150 MHz max ->  75 MHz threshold. */
  k_ra_sram_iclk_max_default      = 250000000UL, /**< Default RA8D2 max ICLK = 250 MHz. */
} ra_sram_iclk_threshold_t;
 ...
 */
typedef enum : uint32_t {
  k_ra_sram_esar_bit_esa  = 0x00000001UL, /**< SRAMESA -- ECC region Non-Secure. */
  k_ra_sram_esar_writable = 0x00000001UL, /**< Union of defined bits.            */
} ra_sram_esar_bit_t;

/**
 * @enum ra_sram_sabar_limit_t
 ...
 */
typedef enum : uint32_t {
  k_ra_sram_sabar_align_mask = 0x00001FFFUL, /**< b12..b0 must be 0.            */
  k_ra_sram_sabar_field_mask = 0x001FE000UL, /**< b20..b13 are the SA boundary. */
  k_ra_sram_sabar_bank0_full = 0x00080000UL, /**< Whole SRAM0 Secure.           */
  k_ra_sram_sabar_bank1_full = 0x00100000UL, /**< Whole SRAM1 Secure.           */
  k_ra_sram_sabar_bank2_full = 0x00180000UL, /**< Whole SRAM2 Secure.           */
  k_ra_sram_sabar_bank3_full = 0x001A0000UL, /**< Whole SRAM3 Secure.           */
} ra_sram_sabar_limit_t;
 ...
 * ``RA_SIMULATOR_MODE`` the same address is backed by host RAM via
 * ``tests/mocks/ra_sim_mmap.c``, so the same accessor works on host
 * tests without a separate mock header.
 */
static inline volatile r_sram_regs_t* ra_sram_regs(void)
{
  return (volatile r_sram_regs_t*)k_ra_sram_base_addr;
}
 ...
 * registers.
 */
static inline volatile r_sram_cpscu_regs_t* ra_sram_cpscu_regs(void)
{
  return (volatile r_sram_cpscu_regs_t*)k_ra_sram_cpscu_base_addr;
}

/**
 * @brief Indexed write helper: ``SRAMCRn`` slot.
 * @param[in] regs Result of ``ra_sram_regs()``.
