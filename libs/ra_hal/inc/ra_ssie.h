 ...
 * pages 3051-3121:
 *
 *  - Lifecycle: ``ra_ssie_init`` / ``ra_ssie_deinit`` follow the
 ...
 *  - DMA pump: TX + RX channels via ``ra_dmac`` with FIFO half-full
 ...
 * |-------------------------------|-----:|-----:|------------:|
 * | k_ra_ssie_format_i2s          |  00b |  00b | 2 (L,R)     |
 * | k_ra_ssie_format_left_just    |  00b |  00b | 2, SDTA=0   |
 * | k_ra_ssie_format_right_just   |  00b |  00b | 2, PDTA=1   |
 * | k_ra_ssie_format_monaural     |  10b |  00b | 1           |
 * | k_ra_ssie_format_tdm_4        |  01b |  01b | 4           |
 * | k_ra_ssie_format_tdm_6        |  01b |  10b | 6           |
 * | k_ra_ssie_format_tdm_8        |  01b |  11b | 8           |
 */
typedef enum : uint8_t {
  k_ra_ssie_format_i2s        = 0U, /**< I2S, 2 channels per frame.   */
  k_ra_ssie_format_left_just  = 1U, /**< Left-justified, 2 channels.  */
  k_ra_ssie_format_right_just = 2U, /**< Right-justified, 2 channels. */
  k_ra_ssie_format_monaural   = 3U, /**< Monaural single-channel.     */
  k_ra_ssie_format_tdm_4      = 4U, /**< TDM 4-slot.                  */
  k_ra_ssie_format_tdm_6      = 5U, /**< TDM 6-slot.                  */
  k_ra_ssie_format_tdm_8      = 6U, /**< TDM 8-slot.                  */
  /**
   * Backwards alias kept for unit tests that index OMOD directly.
   * Same encoding as ``k_ra_ssie_format_tdm_4`` (OMOD=01b, FRM=01b).
   */
  k_ra_ssie_format_tdm = 4U,
} ra_ssie_format_t;

/**
 * @enum ra_ssie_data_word_t
 ...
 */
typedef enum : uint8_t {
  k_ra_ssie_swl_8   = 0U, /**< 8-bit system word.   */
  k_ra_ssie_swl_16  = 1U, /**< 16-bit system word.  */
  k_ra_ssie_swl_24  = 2U, /**< 24-bit system word.  */
  k_ra_ssie_swl_32  = 3U, /**< 32-bit system word.  */
  k_ra_ssie_swl_48  = 4U, /**< 48-bit system word.  */
  k_ra_ssie_swl_64  = 5U, /**< 64-bit system word.  */
  k_ra_ssie_swl_128 = 6U, /**< 128-bit system word. */
  k_ra_ssie_swl_256 = 7U, /**< 256-bit system word. */
} ra_ssie_system_word_t;

/**
 * @enum ra_ssie_bclk_div_t
 ...
 */
typedef enum : uint8_t {
  k_ra_ssie_bclk_div_1   = 0x0U, /**< AUDIO_MCK / 1.   */
  k_ra_ssie_bclk_div_2   = 0x1U, /**< AUDIO_MCK / 2.   */
  k_ra_ssie_bclk_div_4   = 0x2U, /**< AUDIO_MCK / 4.   */
  k_ra_ssie_bclk_div_8   = 0x3U, /**< AUDIO_MCK / 8.   */
  k_ra_ssie_bclk_div_16  = 0x4U, /**< AUDIO_MCK / 16.  */
  k_ra_ssie_bclk_div_32  = 0x5U, /**< AUDIO_MCK / 32.  */
  k_ra_ssie_bclk_div_64  = 0x6U, /**< AUDIO_MCK / 64.  */
  k_ra_ssie_bclk_div_128 = 0x7U, /**< AUDIO_MCK / 128. */
  k_ra_ssie_bclk_div_6   = 0x8U, /**< AUDIO_MCK / 6.   */
  k_ra_ssie_bclk_div_12  = 0x9U, /**< AUDIO_MCK / 12.  */
  k_ra_ssie_bclk_div_24  = 0xAU, /**< AUDIO_MCK / 24.  */
  k_ra_ssie_bclk_div_48  = 0xBU, /**< AUDIO_MCK / 48.  */
  k_ra_ssie_bclk_div_96  = 0xCU, /**< AUDIO_MCK / 96.  */
} ra_ssie_bclk_div_t;
...
/**
 * @struct ra_ssie_cfg_t
 * @brief Configuration descriptor for ``ra_ssie_init``.
 *
 * @details
 * cppcheck cannot see tests/ so it flags every field as unused;
 * each member is read by ``ra_ssie_init`` in
 * ``libs/ra_hal/src/ra_ssie.c``.
 */
...
/**
 * @struct ra_ssie_dma_cfg_t
 * @brief DMA pump descriptor for ``ra_ssie_attach_dma``.
 */
/* cppcheck-suppress-begin [unusedStructMember] */
typedef struct {
...
/**
 * @typedef ra_ssie_event_fn_t
 * @brief SSIE event callback fired by ``ra_ssie_dispatch``.
 *
 * @param[in] ctx     Caller context registered via
 *                    ``ra_ssie_attach_handler``.
 * @param[in] channel Channel that fired (0 or 1).
 * @param[in] events  Decoded ``ra_ssie_event_t`` bitmap.
 * @param[in] ssisr   Snapshot of SSISR at IRQ entry.
 */
typedef void (*ra_ssie_event_fn_t)(void* ctx, uint8_t channel, uint8_t events, uint32_t ssisr);
 ...
 *  1. Ungate MSTPC8 (SSIE0) or MSTPC7 (SSIE1) via ``ra_mstp``.
 ...
 *     the FIFOs and call ``ra_ssie_start``.
 *
 ...
 *                               ``k_ra_ssie_reset_poll_max`` reads.
 *
 ...
 * @pre ``ra_ssie_init`` previously succeeded for ``channel``.
 *
 ...
 *
 * @param[in] channel Channel index.
 * @param[in] dir     ``k_ra_ssie_dir_tx``, ``..._rx`` or ``..._tx_rx``.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Direction enabled.
 * @retval k_ra_err_invalid_arg Channel or direction out of range.
 ...
 * @param[in] channel Channel index.
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Channel reset and flags cleared.
 * @retval k_ra_err_invalid_arg Channel out of range.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              At least zero samples written.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              At least zero samples consumed.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              DMA channels programmed.
 ...
 * @param[in] channel Channel index.
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              All attached DMA channels stopped.
 * @retval k_ra_err_invalid_arg Channel out of range.
 *
 * @pre ``ra_ssie_attach_dma`` previously succeeded for ``channel``.
 ...
 *                    ``k_ra_ssie_mask_err_all`` to clear them all).
 *
 * @return ``ra_err_t`` error code.
 ...
 *
 * @param[in] fn  Callback invoked from ``ra_ssie_dispatch``.
 * @param[in] ctx Opaque context forwarded to the callback.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok Always succeeds (handler may be nullptr to detach).
 *
 * @pre None.
 * @pre Storage for the function pointer pair is process-global.
 *
 * @post Subsequent ``ra_ssie_dispatch`` calls invoke ``fn``.
 ...
 * TUIEN}. Pass ``k_ra_ssie_mask_irq_all`` in ``mask`` to enable
 * every IRQ source.
 *
 * @param[in] channel Channel index.
 ...
 * @return ``ra_err_t`` error code.
 *
 * @pre Channel was previously gated via ``ra_ssie_enter_stop``.
 * @pre IRQs masked.
 *
