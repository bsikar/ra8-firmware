 ...
 * a wrapped-key buffer (produced by ``ra_rsip_key_inject_*``) instead
 ...
 * ``ra_rsip_key_validate``, unwraps the payload into a private
 * scratch buffer, drives the underlying ``ra_rsip_*_install_plain``
 * call to obtain a wrapped ``ra_rsip_key_handle_t``, dispatches the
 ...
 * @see ra_rsip_key_injection.h  Producer of wrapped-key blobs.
 *
 ...
 * @retval k_ra_err_hw_timeout RSIP install primitive timed out.
 *
 * @pre Wrapper was produced by ``ra_rsip_key_inject_aes``.
 * @pre ``ra_rsip_init`` returned ``k_ra_ok`` on the live engine
 *      (or ``ra_sim_mmap`` is wired up under test).
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_rsip_protected_aes_init(const uint8_t*         wrapped_key,
 ...
 *
 * @pre ``ra_rsip_protected_aes_init`` returned ``k_ra_ok``.
 ...
 */
[[nodiscard]] ra_err_t
ra_rsip_protected_aes_encrypt(const uint8_t* plaintext, uint8_t* ciphertext, uint32_t len);
 ...
 */
[[nodiscard]] ra_err_t
ra_rsip_protected_aes_decrypt(const uint8_t* ciphertext, uint8_t* plaintext, uint32_t len);
 ...
 * encrypt / decrypt and before another ``ra_rsip_protected_aes_init``
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_rsip_protected_aes_finish(void);
 ...
 * the wrapper is presented to ``ra_rsip_rsa_sign`` so the engine
 ...
 * @retval k_ra_err_hw_timeout Engine never signalled DONE.
 *
 * @pre Wrapper was produced by ``ra_rsip_key_inject_rsa``.
 * @pre ``ra_rsip_init`` returned ``k_ra_ok`` on the live engine.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_rsip_protected_rsa_decrypt(const uint8_t*     wrapped_priv,
 ...
 * @retval k_ra_err_hw_timeout Engine never signalled DONE.
 *
 * @pre Wrapper was produced by ``ra_rsip_key_inject_ecc(..., true)``.
 * @pre ``ra_rsip_init`` returned ``k_ra_ok`` on the live engine.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_rsip_protected_ecdsa_sign(const uint8_t*  wrapped_priv,
