 ...
 *
 * @details
 * Glues the host-mode bring-up paths in `ra_usb` to a USB HUB device
 ...
 * Lifecycle:
 *
 *  1. `ra_usb_hhub_init(speed)` flips the controller to host mode and
 ...
 *     `ra_usb_hhub_set_port_feature(port, PORT_POWER)` ->
 *     `ra_usb_hhub_set_port_feature(port, PORT_RESET)` ->
 *     `ra_usb_hhub_get_port_status(port, &status)` and waits for
 *     `C_PORT_RESET` to clear before initiating SET_ADDRESS on the
 *     newly attached child.
 *  5. `ra_usb_hhub_close()` drops bus power and releases the device.
 *
...
/**
 * @enum ra_usb_hhub_class_t
 * @brief HUB class code per USB 2.0 sec 11.24.1.
 */
typedef enum : uint8_t {
  k_ra_hhub_class_hub = 0x09U, /**< bDeviceClass / bInterfaceClass = HUB. */
} ra_usb_hhub_class_t;

/**
 * @enum ra_usb_hhub_desc_t
 * @brief HUB descriptor types (USB 2.0 sec 11.23.2).
 */
typedef enum : uint8_t {
  k_ra_hhub_desc_hub = 0x29U, /**< HUB class descriptor type. */
} ra_usb_hhub_desc_t;

/**
 * @enum ra_usb_hhub_request_t
...
/**
 * @enum ra_usb_hhub_port_status_bits_t
 ...
 */
typedef enum : uint8_t {
  k_ra_hhub_max_ports  = 16U, /**< FSP-aligned downstream port ceiling. */
  k_ra_hhub_first_port = 1U,  /**< Port numbering starts at 1.          */
} ra_usb_hhub_limits_t;

/**
 * @struct ra_usb_hhub_device_t
 ...
 */
typedef void (*ra_usb_hhub_attach_fn_t)(void* ctx, const ra_usb_hhub_device_t* device);
 ...
 * @retval k_ra_ok Host-HUB ready, awaiting attach.
 ...
 * @see ra_usb_hhub_attach_callback
 * @see ra_usb_hhub_close
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hhub_init(ra_usb_speed_t speed);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hhub_close(void);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hhub_attach_callback(ra_usb_hhub_attach_fn_t on_attach, void* ctx);
 ...
 *                   [`k_ra_hhub_first_port` .. `k_ra_hhub_max_ports`].
 *
 * @return `ra_err_t` error code.
 ...
 *
 * @post `*count <= k_ra_hhub_max_ports` on success.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hhub_get_port_count(uint8_t* count);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hhub_get_port_status(uint8_t port, uint32_t* status);
 ...
 * @param[in] feature Feature selector (see ::ra_usb_hhub_feature_t).
 *
 * @return `ra_err_t` error code.
 * @retval k_ra_ok Control transfer queued.
 * @retval k_ra_err_invalid_state Driver not initialized, or no HUB
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hhub_set_port_feature(uint8_t port, ra_usb_hhub_feature_t feature);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hhub_clear_port_feature(uint8_t port, ra_usb_hhub_feature_t feature);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_hhub_step(void);
