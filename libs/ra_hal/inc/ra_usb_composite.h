 ...
 * `ra_usb_cdc.h`, `ra_usb_phid.h`, or `ra_usb_pmsc.h`. Instead the
 * application registers each class layer by passing a small
 * function-pointer struct (`ra_usb_composite_class_t`) at boot time;
 ...
 *
 * Lifecycle (mirrors `ra_usb_pmsc.h` and `ra_usb_phid.h` shape):
 *
 *  1. `ra_usb_composite_init(speed)` flips the controller to device
 ...
 *     on the bus.
 *  2. `ra_usb_composite_register_class(class_layer)` is called once
 ...
 *  3. `ra_usb_composite_set_descriptors(device, configuration)` stores
 ...
 *  4. `ra_usb_composite_step()` drives the dispatch state machine.
 ...
 *  5. `ra_usb_composite_close()` invokes each registered class's
...
/**
 * @enum ra_usb_composite_limits_t
...
/**
 * @typedef ra_usb_composite_init_fn_t
 * @brief Class-layer initialise hook.
 *
 * @param[in] ctx Caller-supplied context registered with the class.
 *
 * @return `ra_err_t` error code propagated unchanged.
 */
typedef ra_err_t (*ra_usb_composite_init_fn_t)(void* ctx);

/**
 * @typedef ra_usb_composite_setup_fn_t
 * @brief Class-layer SETUP-handler hook.
 *
 * @param[in] ctx   Caller-supplied context registered with the class.
 * @param[in] setup Decoded 8-byte SETUP packet (USB 2.0 sec 9.3).
 *
 * @return `ra_err_t` error code. Non-zero stalls the control transfer.
 */
typedef ra_err_t (*ra_usb_composite_setup_fn_t)(void* ctx, const ra_usb_setup_t* setup);

/**
 * @typedef ra_usb_composite_close_fn_t
 ...
 */
typedef ra_err_t (*ra_usb_composite_close_fn_t)(void* ctx);

/**
 * @struct ra_usb_composite_class_t
 ...
 *            k_ra_usb_composite_max_ifs`.
 *
 * @code
 * static ra_err_t my_init(void* ctx);
 * static ra_err_t my_setup(void* ctx, const ra_usb_setup_t* s);
 * static ra_err_t my_close(void* ctx);
 *
 * static const ra_usb_composite_class_t s_cdc_layer = {
...
typedef struct {
  ...
  ra_usb_composite_init_fn_t  init;                   /**< init callback (non-NULL).   */
  ra_usb_composite_setup_fn_t handle_setup;           /**< setup callback (non-NULL).  */
  ra_usb_composite_close_fn_t close;                  /**< close callback (non-NULL).  */
  void*                       ctx;                    /**< Opaque context.             */
} ra_usb_composite_class_t;
 ...
 *
 * @return `ra_err_t` error code.
 * @retval k_ra_ok Composite ready, awaiting `register_class`.
 ...
 * @see ra_usb_composite_register_class
 * @see ra_usb_composite_set_descriptors
 * @see ra_usb_composite_close
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_composite_init(ra_usb_speed_t speed);
 ...
 * table and calls `ra_usb_device_deinit`.
 *
 * @return `ra_err_t` error code.
 ...
 *
 * @post `ra_usb_device_deinit` ran; subsequent composite API calls
 *       return `k_ra_err_invalid_state`.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_composite_close(void);
 ...
 *    exceed `k_ra_usb_composite_max_ifs`.
 ...
 * @retval k_ra_err_exists IF range overlaps a previously-registered
 *         class.
 * @retval k_ra_err_no_mem Class table is full (already at
 *         `k_ra_usb_composite_max_classes`).
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_composite_register_class(const ra_usb_composite_class_t* class_layer);
 ...
 *
 * @post `ra_usb_composite_get_device_descriptor` /
 *       `ra_usb_composite_get_config_descriptor` return the supplied
 *       pointers.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_composite_set_descriptors(const uint8_t* device_desc,
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_composite_step(void);
 ...
 *                               `k_ra_usb_composite_max_classes` if
 ...
 *
 * @post On `k_ra_ok` for a class request, the matching class's
 *       `handle_setup` callback was invoked exactly once.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_composite_dispatch_setup(const ra_usb_setup_t* setup,
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_composite_get_class_count(uint8_t* out_count);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_composite_get_device_descriptor(const uint8_t** out_desc);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_composite_get_config_descriptor(const uint8_t** out_desc);
