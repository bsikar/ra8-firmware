 ...
 * ::ra_pdm_channel_cfg_t (see the EK-RA8D2 ``pdm_mic_demo`` for the
 ...
 *   1. ::ra_pdm_init            -- module-stop release.
 *   2. ::ra_pdm_configure       -- mode + filter + coefficients.
 *   3. ::ra_pdm_start           -- activate channel filtering.
 *   4. wait mic wake-up + filter settling (caller delay).
 *   5. ::ra_pdm_read_enable     -- clear status, enable + prime FIFO.
 *   6. ::ra_pdm_read (repeated) -- drain PCM samples.
 *   7. ::ra_pdm_stop            -- halt the channel.
 *
...
typedef enum : uint8_t {
  k_ra_pdm_ch0      = 0U, /**< Channel 0.                     */
  k_ra_pdm_ch1      = 1U, /**< Channel 1.                     */
  k_ra_pdm_ch2      = 2U, /**< Channel 2 (EK-RA8D2 MEMS mic). */
  k_ra_pdm_ch_count = 3U, /**< Number of channels (bound).    */
} ra_pdm_channel_t;

/**
 * @enum ra_pdm_coeff_count_t
 ...
 * coefficient arrays in ::ra_pdm_channel_cfg_t.
 */
typedef enum : uint8_t {
  k_ra_pdm_hpf_h_count  = 2U,  /**< High-pass filter h(0..1) coefficients.       */
  k_ra_pdm_comp_h_count = 11U, /**< Compensation filter h(0..10) coefficients.   */
  k_ra_pdm_lpf_h1_count = 20U, /**< Low-pass (half-band) h1(0..19) coefficients. */
} ra_pdm_coeff_count_t;

/**
 * @struct ra_pdm_channel_cfg_t
...
typedef struct {
  ...
  uint16_t hpf_h[k_ra_pdm_hpf_h_count];   /**< High-pass filter h(0..1).                        */
  uint16_t comp_h[k_ra_pdm_comp_h_count]; /**< Compensation filter h(0..10).                    */
  uint16_t lpf_h0;                        /**< Low-pass filter coefficient h0.                  */
  uint16_t lpf_h1[k_ra_pdm_lpf_h1_count]; /**< Low-pass filter h1(0..19).                       */
} ra_pdm_channel_cfg_t;
 ...
 * @retval k_ra_ok                Module clocked and common bank reset.
 * @retval k_ra_err_hw_timeout    Module-stop release did not settle.
 *
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                Module stopped.
 * @retval k_ra_err_hw_timeout    Module-stop entry did not settle.
 *
 * @pre ::ra_pdm_init previously succeeded.
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Start trigger issued.
 * @retval k_ra_err_invalid_arg  ``ch`` out of range.
 *
 * @pre ::ra_pdm_configure succeeded for ``ch``.
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Data read enabled and FIFO primed.
 * @retval k_ra_err_invalid_arg  ``ch`` out of range.
 *
 * @pre ::ra_pdm_start succeeded and the settling delay elapsed.
 * @pre The channel filter reports running.
 * @post Subsequent ::ra_pdm_read calls return live PCM samples.
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Zero or more samples returned.
 * @retval k_ra_err_invalid_arg  ``ch`` out of range or ``max`` was 0.
 * @retval k_ra_err_null_ptr     ``out`` or ``out_count`` was NULL.
 *
 * @pre ::ra_pdm_read_enable succeeded for ``ch``.
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok               Channel stopped.
 * @retval k_ra_err_invalid_arg  ``ch`` out of range.
 * @retval k_ra_err_hw_timeout   Channel did not report stopped.
 *
 * @pre ::ra_pdm_start previously succeeded for ``ch``.
