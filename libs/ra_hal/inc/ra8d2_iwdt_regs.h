...
typedef enum : uintptr_t {
  k_ra_iwdt_base_addr = 0x40202200UL, /**< IWDT register block base. */
} ra_iwdt_addr_t;

/**
 * @enum ra_iwdt_ofs_addr_t
 ...
 * ``ra_wdt_ofs0_addr`` and the FSP ``BSP_CFG_OPTION_SETTING_OFS0``
 * macro. We mirror the address here for HAL self-containment.
 *
 * @see HUM Ch 7 "Option-Setting Memory" p 278.
 */
typedef enum : uintptr_t {
  k_ra_iwdt_ofs0_addr = 0x03001E04UL, /**< OFS0 word -- IWDT fields. */
} ra_iwdt_ofs_addr_t;

/**
 * @enum ra_iwdt_refresh_t
 ...
 * ``ra_iwdt_refresh`` both perform both writes.
 */
 ...
 */
typedef enum : uint8_t {
  k_ra_iwdt_rcr_rstirqs = 0x80U, /**< RSTIRQS = bit 7 (1 = reset, 0 = NMI / IRQ). */
} ra_iwdt_iwdtrcr_layout_t;

/**
 * @enum ra_iwdt_iwdtcstpr_layout_t
 ...
 */
typedef enum : uint8_t {
  k_ra_iwdt_cstpr_slcstp = 0x80U, /**< SLCSTP = bit 7 (1 = halt counter in Sleep). */
} ra_iwdt_iwdtcstpr_layout_t;

/**
 * @enum ra_iwdt_ofs0_layout_t
 ...
 * layout in ``ra8d2_wdt_regs.h`` because OFS0 packs them in the same
