 ...
 * `quality` is negative -- see `ra_jpeg_sw_encode()`).
 *
 * Out of scope:
...
/**
 * @enum ra_jpeg_sw_pixfmt_t
 * @brief Output pixel layout produced by `ra_jpeg_sw_decode()`.
 *
 * @details
...
typedef enum : uint8_t {
  k_ra_jpeg_sw_pixfmt_rgb888 = 0U, /**< 24-bit packed R,G,B per pixel. */
} ra_jpeg_sw_pixfmt_t;

/**
 * @enum ra_jpeg_sw_quality_t
 * @brief Convenience quality presets for `ra_jpeg_sw_encode()`.
 *
 * @details
 * The `quality` argument to `ra_jpeg_sw_encode()` is a plain
 ...
 * @retval k_ra_ok                Dimensions written to `*out_w`, `*out_h`.
 * @retval k_ra_err_null_ptr      Any pointer argument was NULL.
 * @retval k_ra_err_invalid_size  `jpeg_len` smaller than the SOI
 *                                marker (2 bytes).
 * @retval k_ra_err_protocol_error Stream lacks SOI, lacks SOF0, or a
 *                                 marker length field overflows the
 *                                 buffer.
 * @retval k_ra_err_not_supported SOF marker is non-baseline (e.g.
 ...
 *      this call.
 * @post On `k_ra_ok` both `*out_w` and `*out_h` are non-zero.
 ...
 * @code
 * uint16_t w, h;
 * if (ra_jpeg_sw_get_dimensions(buf, len, &w, &h) == k_ra_ok) {
 *   printf("%u x %u\n", w, h);
 * }
 * @endcode
 *
 * @see ra_jpeg_sw_decode  Full decode that also returns dimensions.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_jpeg_sw_get_dimensions(const uint8_t* jpeg_buf,
                                                 uint32_t       jpeg_len,
                                                 uint16_t*      out_w,
                                                 uint16_t*      out_h);
 ...
 *
 * @return `ra_err_t` error code.
 * @retval k_ra_ok                  Decoded successfully.
 * @retval k_ra_err_null_ptr        Any pointer argument was NULL.
 * @retval k_ra_err_protocol_error  Malformed JPEG stream.
 * @retval k_ra_err_not_supported   Progressive / arithmetic / 12-bit /
 *                                  unsupported chroma layout.
 * @retval k_ra_err_invalid_size    `out_buf_len` < `width*height*3`.
 *
 ...
 *
 * @see ra_jpeg_sw_get_dimensions  Lightweight dimension probe.
 * @see ra_jpeg_sw_encode          Inverse operation.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_jpeg_sw_decode(const uint8_t* jpeg_buf,
                                         uint32_t       jpeg_len,
                                         uint8_t*       out_buf,
 ...
 *                         [`k_ra_jpeg_sw_quality_min`,
 *                          `k_ra_jpeg_sw_quality_max`].
 ...
 *
 * @return `ra_err_t` error code.
 * @retval k_ra_ok                Encoded; `*out_len` <= `out_buf_len`.
 * @retval k_ra_err_null_ptr      Any pointer argument was NULL.
 * @retval k_ra_err_invalid_arg   `quality` out of range, or
 *                                `width == 0`, or `height == 0`.
 * @retval k_ra_err_invalid_size  `out_buf_len` too small to hold the
 ...
 * @code
 * uint32_t produced;
 * ra_err_t e = ra_jpeg_sw_encode(rgb, 640, 480,
 *                                k_ra_jpeg_sw_quality_default,
 *                                jpeg, sizeof jpeg, &produced);
 * @endcode
 *
 * @see ra_jpeg_sw_decode  Inverse operation.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_jpeg_sw_encode(const uint8_t* rgb_buf,
                                         uint16_t       width,
                                         uint16_t       height,
                                         uint8_t        quality,
                                         uint8_t*       out_buf,
