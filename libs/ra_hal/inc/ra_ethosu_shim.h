 ...
 * Arm Ethos-U55 NPU through this project's first-party `ra_npu` HAL, instead of
 ...
 * kernel calls -- three functions -- on top of `ra_npu_init()` /
 * `ra_npu_submit()` / `ra_npu_run()` / `ra_npu_wait()`:
 *
 ...
 *                                array) call into an ::ra_npu_job_t and run it.
 ...
 * NOT carry this project's `ra_` / `k_` prefixes -- they are an external ABI
 * contract owned by Arm, and renaming them would defeat the drop-in purpose.
 *
 * ## Status / gating
 *
 * RA8P1-only: the whole surface is guarded behind `RA_HAS_NPU` (defined only
 * for `-DRA_DEVICE_RA8P1`, see `ra_device.h`), exactly like `ra_npu.h`. There
 * is no RA8P1 board yet, so the adapter is host-tested for the exact NPU
 * register-programming contract only (see `tests/test_ra_ethosu_shim.c`); a real
 ...
 * (`RA_USE_TFLITE_MICRO`, OFF by default): it only PROVIDES the symbols that
 * stack would need.
 *
 * ## Threading
 *
 * Not thread-safe. The NPU is a single-job engine and the shim keeps one static
 * driver context; the caller owns serialisation, the same contract as `ra_npu`.
 *
 ...
 * `ra_ethosu_shim.c`. Keeping the type incomplete in this header means the
 ...
 * single static driver context: the first call lazily runs `ra_npu_init()` (out
 ...
 * @retval nullptr  `ra_npu_init()` failed; no usable NPU handle.
 *
 ...
 *       IRQ masked, matching the `ra_npu` ownership contract.
...
/**
 * @brief Run one Ethos-U55 command stream through the `ra_npu` submission path.
 *
 * @details
 * Translates the Arm invoke arguments into an ::ra_npu_job_t and drives the
 ...
 * so `ra_npu` programs only the region bases. @p user_arg (the Arm inference
 * begin/end callback cookie) is likewise unused: this shim has no callbacks.
 *
 * @p num_base_addr is bounded to `ra_npu`'s region capacity
 * (`k_ra_npu_region_count`); an out-of-range count is rejected before any
 * register is touched, so the fixed ::ra_npu_job_t region array can never
 * overflow.
 *
 ...
 * @param[in] num_base_addr    Region count, 0 .. `k_ra_npu_region_count`.
 ...
 * @retval -1 A bad argument, region-count overflow, or an `ra_npu` submit /
