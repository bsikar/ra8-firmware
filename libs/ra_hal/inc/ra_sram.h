 ...
 * Public surface:
 *
 *  - **Lifecycle**: ``ra_sram_init`` / ``ra_sram_deinit``,
 *    ``ra_sram_enter_stop`` / ``ra_sram_exit_stop`` per bank.
 *  - **Mode**: per-bank ECC mode + OAD via ``ra_sram_set_mode``,
 *    1-bit latch via ``cfg.enable_1bit_latch``, ECC region size via
 *    ``ra_sram_set_eccrgn``.
 *  - **Wait state**: ``ra_sram_set_wait_state`` /
 *    ``ra_sram_set_wait_state_for_clock`` per HUM Ch 58.3.7 thresholds.
 *  - **TrustZone**: ``ra_sram_set_security``,
 *    ``ra_sram_set_ecc_security``, ``ra_sram_set_boundary``.
 *  - **Status**: ``ra_sram_get_status``, ``ra_sram_clear_status``,
 *    ``ra_sram_clear_address``.
 *  - **Zero-init**: ``ra_sram_zero_init_bank`` per HUM Ch 58.3.2 to
 *    write deterministic ECC syndromes before enabling ``with-check``.
 *  - **ECC self-test**: ``ra_sram_self_test`` -- runs the
 *    HUM Ch 58.3.4 decoder-test flowchart.
 *  - **Error handler**: global ``ra_sram_attach_handler`` and per-bank
 *    ``ra_sram_attach_bank_handler``; both fan-outs invoked by
 *    ``ra_sram_dispatch`` (test path) and
 *    ``ra_sram_dispatch_from_esr`` (NMI path that snapshots SRAMESR
 ...
 * ``ra_sram_set_wait_state_for_clock`` after init.
 */
/* cppcheck-suppress-begin [unusedStructMember] */
typedef struct {
...
/**
 * @struct ra_sram_status_t
 * @brief Snapshot of ECC error state (filled by ``ra_sram_get_status``).
 *
 * @details
 * Bits in ``raw_esr`` follow the SRAMESR layout exactly (HUM Ch 58.2.12
 * p 3535) -- see ``ra_sram_esr_bit_t`` in ``ra8d2_sram_regs.h``. The
...
/**
 * @struct ra_sram_bank_info_t
 * @brief Static description of one bank.
 *
 * @details
 * Returned by ``ra_sram_get_bank_info`` so callers do not have to
 * carry the layout constants from ``ra8d2_sram_regs.h``.
 */
/* cppcheck-suppress-begin [unusedStructMember] */
typedef struct {
 ...
 */
typedef void (*ra_sram_error_fn_t)(void* ctx, uint8_t bank, bool is_2bit, uintptr_t err_addr);
 ...
 *   2. ``ra_mstp_enable`` per bank (HUM 58.3.1 p 3538).
 ...
 * @pre  ``ra_sram_init`` has been called previously (deinit is idempotent
 ...
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               Bank gated.
 * @retval k_ra_err_invalid_arg  ``bank`` >= ``k_ra_sram_bank_count``.
 *
 * @pre  ``ra_sram_init`` has run.
 ...
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                Bank reprogrammed.
 * @retval k_ra_err_null_ptr      ``cfg`` was NULL.
 * @retval k_ra_err_invalid_arg   ``bank`` >= ``k_ra_sram_bank_count``
 ...
 *
 * @param[in] bank   Bank index 0..3.
 * @param[in] region One of ``ra_sram_eccrgn_size_t``.
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               Region size applied.
 * @retval k_ra_err_invalid_arg  Bank or region out of range, or bank 3
 ...
 *
 * @note Not thread-safe.
 * @see ra_sram_set_wait_state_for_clock
 * @since 0.1.0
 */
 ...
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               WTEN written.
 * @retval k_ra_err_invalid_arg  Either argument is zero.
 *
 * @pre  ``ra_sram_init`` has run.
 ...
 *                     ``ra_sram_esr_bit_t``).
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               Mask written to SRAMESCLR.
 * @retval k_ra_err_invalid_arg  ``esr_mask`` includes reserved bits.
 *
 * @pre  ``ra_sram_init`` has run (the register is not PRCR-protected
 ...
 *
 * @param[in] bank Bank index 0..3.
 * @param[in] slot ``k_ra_sram_ear_slot_1bit`` or
 *                 ``k_ra_sram_ear_slot_2bit``.
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               EAR cleared via the matching SRAMESCLR bit.
 * @retval k_ra_err_invalid_arg  Bank or slot out of range.
 *
 * @pre  ``ra_sram_init`` has run.
 ...
 * can pick the final mode via ``ra_sram_set_mode``.
 *
 * The fill stride is 8 bytes (HUM 58.4.2 p 3541 -- "SRAM are read in
 * 8-byte (64-bit) units"), and the bank size comes from
 * ``ra_sram_bank_size_bytes`` so SRAM3's smaller 128 KB region is
 * handled correctly.
 *
 * @param[in] bank Bank index 0..3.
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               Bank zeroed.
 * @retval k_ra_err_invalid_arg  Bank out of range.
 *
 * @pre  ``ra_sram_init`` has run.
 ...
 *       reprograms via ``ra_sram_set_mode`` if a different mode is
 *       desired.
 *
 ...
 * under ``RA_SIMULATOR_MODE`` the barrier is a no-op.
 *
 * @param[in]  bank          Bank index 0..3.
 ...
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               Self-test ran to completion.
 * @retval k_ra_err_invalid_arg  Bank/offset/out_caught invalid.
 *
 * @pre  ``ra_sram_init`` has run.
 ...
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               Layout filled.
 * @retval k_ra_err_invalid_arg  Bank out of range.
 * @retval k_ra_err_null_ptr     ``out`` was NULL.
 *
 * @pre  ``out`` points to writable storage of size ``ra_sram_bank_info_t``.
 * @pre  ``bank`` < ``k_ra_sram_bank_count``.
 * @post ``out->bank == bank``.
 ...
 *
 * @param[in] sa_mask Bitmask of ``ra_sram_sar_bit_t`` values.
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               SAR register updated.
 * @retval k_ra_err_invalid_arg  ``sa_mask`` includes undefined bits.
 *
 ...
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               SABAR updated.
 * @retval k_ra_err_invalid_arg  Bank or offset rejected.
 *
 * @pre  Caller is in Secure World.
 * @pre  ``offset & k_ra_sram_sabar_align_mask == 0``.
 * @post SRAMSABARn reads back ``offset & ~k_ra_sram_sabar_align_mask``.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
 ...
 * @post Subsequent calls to ``ra_sram_dispatch`` will fire ``fn``.
 *
 ...
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               Handler installed for this bank only.
 * @retval k_ra_err_null_ptr     ``fn`` was NULL.
 * @retval k_ra_err_invalid_arg  Bank out of range.
 *
 * @pre  ``ra_sram_init`` has run.
 ...
 * @post Subsequent ``ra_sram_dispatch`` / ``ra_sram_dispatch_from_esr``
 *       call ``fn(ctx, bank, ...)``.
 *
 * @note Not thread-safe at install time.
 * @since 0.1.0
 ...
 * defined error bits, calls ``ra_sram_dispatch`` for every set bit
