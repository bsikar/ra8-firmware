 ...
 * writable side of those words is emitted by `ra_ofs.c` into the
 * `.option_setting_*` linker sections; the runtime-readable mirror of the
 * WDT-relevant words lives in `ra8d2_wdt_regs.h`.
 *
 ...
 *
 * The OFS3 presence is switched by the `RA_HAS_OFS3` build-config macro from
 * `ra_device.h` (defined for RA8D2, deliberately undefined for RA8P1). On the
 * RA8P1 build `k_ra_ofs_word_ofs3` is not even declared, so any firmware that
 ...
 *       builds unchanged under `RA_SIMULATOR_MODE` and in the host unit tests
 *       (which default to the RA8D2 device selection).
 *
 * @see ra_device.h  RA8D2/RA8P1 compile-time device switch and `RA_HAS_OFS3`.
 * @see ra_ofs.c     Emits the writable option-setting words into flash.
 *
 ...
 * live in `ra8d2_wdt_regs.h`.
 *
 * `k_ra_ofs_word_ofs3` is declared ONLY when `RA_HAS_OFS3` is defined (RA8D2).
 * On the RA8P1 the OFS3 word does not exist, so it is omitted and
 * `k_ra_ofs_word_count` drops from 4 to 3.
 *
 * @invariant `k_ra_ofs_word_count` equals the number of OFS words the device
 *            has (4 on RA8D2, 3 on RA8P1).
 *
 * @see ra_ofs_word_count()
 * @see ra_ofs_has_ofs3()
 * @since 0.1.0
 */
typedef enum : uint8_t {
  k_ra_ofs_word_ofs0 = 0U, /**< OFS0: IWDT/BOR/HOCO/security/TrustZone (both parts). */
  k_ra_ofs_word_ofs1 = 1U, /**< OFS1: LVD0 reset / VCC monitor (both parts).         */
  k_ra_ofs_word_ofs2 = 2U, /**< OFS2: extended oscillator settings (both parts).     */
#ifdef RA_HAS_OFS3
  k_ra_ofs_word_ofs3  = 3U, /**< OFS3: WDT1 + RA8 extended settings (RA8D2 only). */
  k_ra_ofs_word_count = 4U, /**< OFS word count on this device (RA8D2: 4).        */
#else
  k_ra_ofs_word_count = 3U, /**< OFS word count on this device (RA8P1: 3, no OFS3). */
#endif
} ra_ofs_word_t;
 ...
 * the `k_ra_feat_ofs3` device-feature mirror from `ra_device.h`. True on the
 ...
 *
 * @pre The device selection in `ra_device.h` resolved to exactly one part.
 * @pre `k_ra_feat_ofs3` is a valid 0/1 compile-time constant.
 * @post No hardware or global state is modified.
 * @post The return value matches `k_ra_ofs_word_count == 4`.
 *
 * @note Thread-safe: pure function of a compile-time constant, no shared state.
 *
 * @see ra_ofs_word_count()
 * @see ra_device_feature_t
 * @since 0.1.0
 */
 ...
 *
 * @details
 * Yields `k_ra_ofs_word_count` -- 4 on the RA8D2 (OFS0..OFS3) and 3 on the
 ...
 *
 * @pre The device selection in `ra_device.h` resolved to exactly one part.
 * @pre `k_ra_ofs_word_count` is defined by `ra_ofs_word_t`.
 * @post No hardware or global state is modified.
