 ...
 *
 * Drivers must call ``ra_mstp_enable()`` before they touch the
 * peripheral's registers and ``ra_mstp_disable()`` when they tear
 ...
 * was updated." ``ra_mstp_enable() / disable()`` therefore
 * read the register back after the modify-write and return
 * ``k_ra_err_hw_timeout`` if the read does not see the new value
 ...
 *
 * The full list of peripherals lives in ``ra8d2_mstp_regs.h`` as
 * the ``ra_mstp_t`` enum. Each entry packs the (register, bit)
...
/**
 * @brief Re-establish the ra_mstp ref-count table from current hardware state.
 *
 * @details
 ...
 */
[[nodiscard]] ra_err_t ra_mstp_enable(ra_mstp_t id);
 ...
 *
 * Honors the same read-back protocol as ``ra_mstp_enable()``.
 *
 * @param[in] id Peripheral identifier from ``ra_mstp_t``.
 *
 * @return ``ra_err_t`` error code.
 ...
 * ``ra_mstp_enable() / disable()`` pairs balance correctly. Should
