 ...
 * that ``#include "ra_rsip.h"`` continue to see the full surface
 * unchanged:
 *
 * - ``ra_rsip_core.h`` -- lifecycle, status / IRQ, TRNG, one-shot
 *   SHA-256, incremental SHA-256 / HMAC-SHA-256, power transition.
 * - ``ra_rsip_keys.h`` -- wrapped-key handle type, key install
 ...
 * - ``ra_rsip_mgmt.h`` -- OEM boot loader version, wrapped-key vault,
...
#include "ra_rsip_keys.h"
#include "ra_rsip_mgmt.h"
