 ...
 *  - ``ra_iwdt_refresh_deferred`` <-> FSP ``R_IWDT_Refresh``
 *  - ``ra_iwdt_get_status``       <-> FSP ``R_IWDT_StatusGet``
 *  - ``ra_iwdt_clear_status``     <-> FSP ``R_IWDT_StatusClear``
 *  - ``ra_iwdt_get_counter``      <-> FSP ``R_IWDT_CounterGet``
 *
 ...
 * ``ra_iwdt_get_counter`` instead.
 */
typedef enum : uint16_t {
  k_ra_iwdt_status_none      = 0x0000U,
  k_ra_iwdt_status_underflow = 0x4000U, /**< IWDTSR.UNDFF bit 14. */
  k_ra_iwdt_status_refresh   = 0x8000U, /**< IWDTSR.REFEF bit 15. */
 ...
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok ``*out_counter`` populated.
 * @retval k_ra_err_null_ptr ``out_counter == NULL``.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_iwdt_get_counter(uint16_t* out_counter);
