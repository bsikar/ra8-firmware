...
/**
 * @enum ra_gpt_pwm_pin_t
 * @brief Selector for the GTIOCnA / GTIOCnB PWM output pins.
 *
 * @details
 * Used by ``ra_gpt_duty_cycle_set`` and ``ra_gpt_pwm_pin_configure``.
 ...
 */
typedef enum : uint8_t {
  k_ra_gpt_pin_a = 0U, /**< GTIOCnA -- compare register A path. */
  k_ra_gpt_pin_b = 1U, /**< GTIOCnB -- compare register B path. */
} ra_gpt_pwm_pin_t;

/**
 * @enum ra_gpt_pwm_polarity_t
 * @brief Output polarity for ``ra_gpt_pwm_pin_configure``.
 *
 * @details
...
/**
 * @enum ra_gpt_pwm_stop_level_t
 * @brief Pin level when the timer is stopped.
 *
 * @details Maps to GTIOR.OADFLT / GTIOR.OBDFLT (HUM Ch 22.2.13).
 */
typedef enum : uint8_t {
  k_ra_gpt_stop_low  = 0U, /**< OnDFLT = 0 -> stop level low.  */
  k_ra_gpt_stop_high = 1U, /**< OnDFLT = 1 -> stop level high. */
} ra_gpt_pwm_stop_level_t;

/**
 * @enum ra_gpt_pwm_disable_t
...
/**
 * @struct ra_gpt_pwm_pin_cfg_t
 * @brief Configuration descriptor for ``ra_gpt_pwm_pin_configure``.
 *
 * @details
 * Holds the per-pin policy programmed into the GTIOR register
 * (HUM Ch 22.2.13). See ``ra_gpt_pwm_pin_configure`` for the
 * field-by-field semantics. The structure is consumed by
 * ``ra_gpt_pwm_pin_configure`` in
 * ``libs/ra_hal/src/ra_gpt.c``.
 */
/* cppcheck-suppress-begin [unusedStructMember] */
typedef struct {
...
/**
 * @enum ra_gpt_three_phase_idx_t
 * @brief Index into ``ra_gpt_three_phase_cfg_t::channels`` for U/V/W.
 */
typedef enum : uint8_t {
...
/**
 * @struct ra_gpt_three_phase_cfg_t
 * @brief Configuration descriptor for ``ra_gpt_three_phase_open``.
 *
 * @details
 ...
 *
 * Each member is read inside ``ra_gpt_three_phase_open`` in
 * ``libs/ra_hal/src/ra_gpt.c``.
 */
/* cppcheck-suppress-begin [unusedStructMember] */
typedef struct {
 ...
 * cleared so the next ``ra_gpt_set_period`` style call has the same
 * starting point as a fresh ``ra_gpt_init``. Mirrors FSP
 ...
 *
 * @note Thread safety: not thread-safe.
 * @see ra_gpt_set_period() Legacy single-shot variant (no buffer).
 * @since 0.1.0
 *
 * @par HUM:
 ...
 * p 985..986.
 */
[[nodiscard]] ra_err_t ra_gpt_period_set(uint8_t channel, uint32_t period_counts);

/**
 * @brief Update one PWM compare value via the shadow buffer.
 *
 * @details
 * Writes ``compare_counts`` into GTCCRC (for ``k_ra_gpt_pin_a``) or
 * GTCCRE (for ``k_ra_gpt_pin_b``). Per FSP ``R_GPT_DutyCycleSet``
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Compare buffer programmed.
 ...
 */
[[nodiscard]] ra_err_t
ra_gpt_duty_cycle_set(uint8_t channel, ra_gpt_pwm_pin_t pin, uint32_t compare_counts);
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               Counter updated.
 * @retval k_ra_err_null_ptr     ``channel`` out of range.
 * @retval k_ra_err_invalid_state Counter is currently running.
 *
 ...
 */
[[nodiscard]] ra_err_t ra_gpt_counter_set(uint8_t channel, uint32_t value);
 ...
 */
[[nodiscard]] ra_err_t
ra_gpt_dead_time_set(uint8_t channel, uint32_t rising_dt, uint32_t falling_dt);
 ...
 *
 * @details
 * Calls ``ra_gpt_init`` on each of the three channels in
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              All three channels opened and started.
 * @retval k_ra_err_null_ptr    ``cfg`` NULL.
 * @retval k_ra_err_invalid_arg Any channel id out of range or already open.
 *
 * @pre ``ra_mstp_init`` has been called.
 * @pre IRQs masked or single-threaded init context.
 ...
 */
[[nodiscard]] ra_err_t ra_gpt_three_phase_open(const ra_gpt_three_phase_cfg_t* cfg);
 ...
 * ``ra_gpt_three_phase_open``. The GTBER buffer-enable bit makes
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               Compare values queued for swap.
 * @retval k_ra_err_invalid_state ``ra_gpt_three_phase_open`` not called.
 * @retval k_ra_err_invalid_arg  Any duty value > GTPR.
 *
 * @pre ``ra_gpt_three_phase_open`` returned ``k_ra_ok``.
 ...
 */
[[nodiscard]] ra_err_t
ra_gpt_three_phase_set_duty(uint32_t u_duty, uint32_t v_duty, uint32_t w_duty);
 ...
 * ``ra_gpt_deinit`` on each. Mirrors FSP
 * ``R_GPT_THREE_PHASE_Stop`` + ``R_GPT_THREE_PHASE_Close``.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok               All three channels closed.
 * @retval k_ra_err_invalid_state ``ra_gpt_three_phase_open`` not called.
 *
 * @pre ``ra_gpt_three_phase_open`` returned ``k_ra_ok``.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_gpt_three_phase_close(void);
 ...
 * ``ra_gpt_capture_read``; the TCFA / TCFB flag in GTST
 * (``k_ra_gpt_status_ccra`` / ``k_ra_gpt_status_ccrb``) is raised on
 ...
 *
 * @invariant Only bits inside ``k_ra_gpt_cap_src_valid_mask`` are legal.
 * @see ra_gpt_capture_configure()
 * @see ra_gpt_capture_read()
 *
 ...
 */
typedef enum : uint32_t {
  k_ra_gpt_cap_src_none            = 0x00000000UL, /**< No capture source (disable). */
  k_ra_gpt_cap_src_gtetrga_rising  = 0x00000001UL, /**< bit 0  GTETRGA rising.       */
  k_ra_gpt_cap_src_gtetrga_falling = 0x00000002UL, /**< bit 1  GTETRGA falling.      */
  k_ra_gpt_cap_src_gtetrgb_rising  = 0x00000004UL, /**< bit 2  GTETRGB rising.       */
  k_ra_gpt_cap_src_gtetrgb_falling = 0x00000008UL, /**< bit 3  GTETRGB falling.      */
  k_ra_gpt_cap_src_gtetrgc_rising  = 0x00000010UL, /**< bit 4  GTETRGC rising.       */
  k_ra_gpt_cap_src_gtetrgc_falling = 0x00000020UL, /**< bit 5  GTETRGC falling.      */
  k_ra_gpt_cap_src_gtetrgd_rising  = 0x00000040UL, /**< bit 6  GTETRGD rising.       */
  k_ra_gpt_cap_src_gtetrgd_falling = 0x00000080UL, /**< bit 7  GTETRGD falling.      */
  k_ra_gpt_cap_src_ioca_rising     = 0x00000300UL, /**< bits 8,9   GTIOCnA rising.   */
  k_ra_gpt_cap_src_ioca_falling    = 0x00000C00UL, /**< bits 10,11 GTIOCnA falling.  */
  k_ra_gpt_cap_src_iocb_rising     = 0x00003000UL, /**< bits 12,13 GTIOCnB rising.   */
  k_ra_gpt_cap_src_iocb_falling    = 0x0000C000UL, /**< bits 14,15 GTIOCnB falling.  */
  k_ra_gpt_cap_src_elc_a           = 0x00010000UL, /**< bit 16 ELC_GPTA event.       */
  k_ra_gpt_cap_src_elc_b           = 0x00020000UL, /**< bit 17 ELC_GPTB event.       */
  k_ra_gpt_cap_src_elc_c           = 0x00040000UL, /**< bit 18 ELC_GPTC event.       */
  k_ra_gpt_cap_src_elc_d           = 0x00080000UL, /**< bit 19 ELC_GPTD event.       */
  k_ra_gpt_cap_src_elc_e           = 0x00100000UL, /**< bit 20 ELC_GPTE event.       */
  k_ra_gpt_cap_src_elc_f           = 0x00200000UL, /**< bit 21 ELC_GPTF event.       */
  k_ra_gpt_cap_src_elc_g           = 0x00400000UL, /**< bit 22 ELC_GPTG event.       */
  k_ra_gpt_cap_src_elc_h           = 0x00800000UL, /**< bit 23 ELC_GPTH event.       */
  k_ra_gpt_cap_src_other_channel   = 0x01000000UL, /**< bit 24 ASOC/BSOC (GPT4..9).  */
  k_ra_gpt_cap_src_valid_mask      = 0x01FFFFFFUL, /**< bits 0..24: all legal bits.  */
} ra_gpt_capture_source_t;

/**
 * @enum ra_gpt_count_source_t
 ...
 *
 * @invariant Only bits inside ``k_ra_gpt_cnt_src_valid_mask`` are legal.
 * @see ra_gpt_event_count_configure()
 *
 ...
 */
typedef enum : uint32_t {
  k_ra_gpt_cnt_src_none            = 0x00000000UL, /**< No count source (disable).  */
  k_ra_gpt_cnt_src_gtetrga_rising  = 0x00000001UL, /**< bit 0  GTETRGA rising.      */
  k_ra_gpt_cnt_src_gtetrga_falling = 0x00000002UL, /**< bit 1  GTETRGA falling.     */
  k_ra_gpt_cnt_src_gtetrgb_rising  = 0x00000004UL, /**< bit 2  GTETRGB rising.      */
  k_ra_gpt_cnt_src_gtetrgb_falling = 0x00000008UL, /**< bit 3  GTETRGB falling.     */
  k_ra_gpt_cnt_src_gtetrgc_rising  = 0x00000010UL, /**< bit 4  GTETRGC rising.      */
  k_ra_gpt_cnt_src_gtetrgc_falling = 0x00000020UL, /**< bit 5  GTETRGC falling.     */
  k_ra_gpt_cnt_src_gtetrgd_rising  = 0x00000040UL, /**< bit 6  GTETRGD rising.      */
  k_ra_gpt_cnt_src_gtetrgd_falling = 0x00000080UL, /**< bit 7  GTETRGD falling.     */
  k_ra_gpt_cnt_src_ioca_rising     = 0x00000300UL, /**< bits 8,9   GTIOCnA rising.  */
  k_ra_gpt_cnt_src_ioca_falling    = 0x00000C00UL, /**< bits 10,11 GTIOCnA falling. */
  k_ra_gpt_cnt_src_iocb_rising     = 0x00003000UL, /**< bits 12,13 GTIOCnB rising.  */
  k_ra_gpt_cnt_src_iocb_falling    = 0x0000C000UL, /**< bits 14,15 GTIOCnB falling. */
  k_ra_gpt_cnt_src_elc_a           = 0x00010000UL, /**< bit 16 ELC_GPTA event.      */
  k_ra_gpt_cnt_src_elc_b           = 0x00020000UL, /**< bit 17 ELC_GPTB event.      */
  k_ra_gpt_cnt_src_elc_c           = 0x00040000UL, /**< bit 18 ELC_GPTC event.      */
  k_ra_gpt_cnt_src_elc_d           = 0x00080000UL, /**< bit 19 ELC_GPTD event.      */
  k_ra_gpt_cnt_src_elc_e           = 0x00100000UL, /**< bit 20 ELC_GPTE event.      */
  k_ra_gpt_cnt_src_elc_f           = 0x00200000UL, /**< bit 21 ELC_GPTF event.      */
  k_ra_gpt_cnt_src_elc_g           = 0x00400000UL, /**< bit 22 ELC_GPTG event.      */
  k_ra_gpt_cnt_src_elc_h           = 0x00800000UL, /**< bit 23 ELC_GPTH event.      */
  k_ra_gpt_cnt_src_valid_mask      = 0x00FFFFFFUL, /**< bits 0..23: all legal bits. */
} ra_gpt_count_source_t;
 ...
 * ``k_ra_gpt_ccr_a``) or GTICBSR (when ``which`` is ``k_ra_gpt_ccr_b``)
 ...
 * GTST flag (read via ``ra_gpt_get_status``). Passing
 * ``k_ra_gpt_cap_src_none`` returns the register to output-compare use.
 *
 * The counter itself is unaffected -- start it on the internal clock
 * with ``ra_gpt_init`` (or ``ra_gpt_start_free_run``) so GTCNT advances
 ...
 * @param[in] source_mask OR of ``k_ra_gpt_cap_src_*`` bits; must lie
 *                         within ``k_ra_gpt_cap_src_valid_mask``.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Capture source programmed.
 * @retval k_ra_err_null_ptr    ``channel`` out of range.
 * @retval k_ra_err_invalid_arg ``which`` out of range or ``source_mask``
 *                              sets a reserved bit.
 *
 * @pre Channel previously initialized via ``ra_gpt_init`` /
 *      ``ra_gpt_start_free_run``.
 * @pre IRQs masked or single-threaded init context.
 * @post GTICASR (A) or GTICBSR (B) equals ``source_mask``.
 * @post GTWP is re-locked.
 *
 * @note Thread safety: not thread-safe.
 * @see ra_gpt_capture_read()  Read the latched GTCNT value.
 * @see ra_gpt_get_status()    Poll the TCFA / TCFB capture flag.
 * @since 0.1.0
 *
 * @par HUM:
 ...
 */
[[nodiscard]] ra_err_t
ra_gpt_capture_configure(uint8_t channel, ra_gpt_ccr_sel_t which, uint32_t source_mask);

/**
 * @brief Read the value latched by the last input capture.
 *
 * @details
 * Reads GTCCRA (``which`` == ``k_ra_gpt_ccr_a``) or GTCCRB (``which`` ==
 * ``k_ra_gpt_ccr_b``), which hold the GTCNT value latched on the most
 * recent capture edge while the matching source register is armed. The
 * caller typically polls ``ra_gpt_get_status`` for TCFA / TCFB, reads
 * the value here, then clears the flag with ``ra_gpt_clear_status``.
 *
 * @param[in]  channel   GPT channel 0..13.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Value read into ``out_value``.
 * @retval k_ra_err_null_ptr    ``out_value`` NULL or ``channel`` out of range.
 * @retval k_ra_err_invalid_arg ``which`` out of range.
 *
 * @pre ``ra_gpt_capture_configure`` armed the register being read.
 ...
 */
[[nodiscard]] ra_err_t
ra_gpt_capture_read(uint8_t channel, ra_gpt_ccr_sel_t which, uint32_t* out_value);
 ...
 * Passing ``k_ra_gpt_cnt_src_none`` for both arguments returns the
 * channel to internal-clock counting.
 *
 * Start the counter separately (``ra_gpt_init`` with ``auto_start`` or
 * ``ra_gpt_start_free_run``); GTCR.CST still gates whether edges are
 * counted. GTCR.TPCS and the GTCR.MD waveform mode have no effect while
 * an event-count source is active.
 *
 * @param[in] channel     GPT channel 0..13.
 * @param[in] up_source   OR of ``k_ra_gpt_cnt_src_*`` up-count bits;
 *                        must lie within ``k_ra_gpt_cnt_src_valid_mask``.
 * @param[in] down_source OR of ``k_ra_gpt_cnt_src_*`` down-count bits;
 *                        must lie within ``k_ra_gpt_cnt_src_valid_mask``.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok              Up / down sources programmed.
 * @retval k_ra_err_null_ptr    ``channel`` out of range.
 * @retval k_ra_err_invalid_arg ``up_source`` or ``down_source`` sets a
 *                              reserved bit.
 *
 * @pre Channel previously initialized via ``ra_gpt_init``.
 * @pre IRQs masked or single-threaded init context.
 ...
 *
 * @note Thread safety: not thread-safe.
 * @see ra_gpt_read()  Read the accumulated GTCNT edge count.
 * @since 0.1.0
 *
 * @par HUM:
 ...
 */
[[nodiscard]] ra_err_t
ra_gpt_event_count_configure(uint8_t channel, uint32_t up_source, uint32_t down_source);
