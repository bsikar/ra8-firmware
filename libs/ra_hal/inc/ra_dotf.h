 ...
 *    ``k_ra_dotf_max_regions`` pre-validated regions, only one of
 *    which can be live at a time per HUM 45.3 ("one contiguous
 *    area"); the active region is selected via
 *    ``ra_dotf_select_region`` for live key-region rotation.
 *  - Live key rotation: ``ra_dotf_rotate_key`` quiesces the channel,
 *    re-stages the wrapped key handle from ra_rsip, and re-arms the
 *    AES core in a single bounded sequence.
 *  - RSIP-key-injection wiring: ``ra_dotf_install_key`` consumes a
 *    ``ra_dotf_key_handle_t`` (an opaque cookie produced by
 *    ``ra_rsip``'s key-injection layer) and stages the wrapped-key
 *    bytes into REG03 in big-endian order. The HAL never touches
 *    raw key material -- it only forwards the handle to RSIP.
 *  - Region overlap validation: ``ra_dotf_set_region`` rejects any
 ...
 *    exposed via ``ra_dotf_set_sca_level``.
 *  - Self-test trigger: REG00 bit 20 is exposed via
 *    ``ra_dotf_run_self_test``; the result is observable through
 *    ``ra_dotf_get_status``.
 ...
 *          ``ra_dotf_init()`` -> ``ra_dotf_install_key(handle)`` ->
 *          ``ra_dotf_set_iv(iv)`` -> ``ra_dotf_set_region(region)``
 *          -> ``ra_dotf_enable()`` -> jump into XiP code.
 *
 ...
 * @startuml
 *  [*] --> RESET
 *  RESET --> IDLE             : ra_dotf_init
 *  IDLE  --> KEY_STAGED       : ra_dotf_install_key
 *  KEY_STAGED --> IV_STAGED   : ra_dotf_set_iv
 *  IV_STAGED  --> REGION_SET  : ra_dotf_set_region / select_region
 *  REGION_SET --> ARMED       : ra_dotf_enable
 *  ARMED      --> REGION_SET  : ra_dotf_disable
 *  ARMED      --> ARMED       : ra_dotf_rotate_key (re-stage + re-arm)
 *  ARMED      --> RESET       : ra_dotf_deinit
 * @enduml
 *
 ...
 *  - standard: bit16=1, bit17=0  (default after ra_dotf_init)
 *  - max:      bit16=1, bit17=1  (paranoid -- higher latency)
 */
typedef enum : uint8_t {
  k_ra_dotf_sca_off      = 0U, /**< Side-channel countermeasures disabled. */
  k_ra_dotf_sca_standard = 1U, /**< Default level, bit 16 set.             */
  k_ra_dotf_sca_max      = 2U, /**< Maximum level, bits 16 + 17 set.       */
} ra_dotf_sca_level_t;
...
/**
 * @struct ra_dotf_region_t
 * @brief Conversion-area descriptor for ``ra_dotf_set_region``.
 *
 * @details
 ...
 * being unused; ``ra_dotf_set_region`` reads them all.
 */
...
typedef struct {
  ...
  uint8_t  region_id;  /**< 0..k_ra_dotf_max_regions-1 staging slot.        */
} ra_dotf_region_t;
/* cppcheck-suppress-end [unusedStructMember] */

/* =============================================================================
 * Key handle (opaque to ra_rsip)
 * =============================================================================
 */

/**
 * @struct ra_dotf_key_handle_t
 * @brief Wrapped-key handle handed to DOTF by ra_rsip.
 *
 * @details
 * The HAL never sees raw key bits. ``ra_rsip``'s key-injection layer
...
} ra_dotf_key_handle_t;
/* cppcheck-suppress-end [unusedStructMember] */

/**
 * @typedef ra_dotf_event_fn_t
 ...
 */
typedef void (*ra_dotf_event_fn_t)(void* ctx, uint8_t channel);
 ...
 * via ``ra_mstp_enable``, then writes ``0`` to each channel's REG00
 ...
 * @retval k_ra_ok                 DOTF clocked and zeroed.
 * @retval k_ra_err_hw_init_failed MSTP enable failed for either id.
 *
 ...
 * ``ra_dotf_select_region(channel, region_id)`` is called.
 *
 * @param[in] channel Channel index 0..1.
 * @param[in] region  Non-NULL region descriptor.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                  Region staged.
 * @retval k_ra_err_null_ptr        ``region`` is NULL.
 * @retval k_ra_err_invalid_arg     ``channel`` or ``region_id`` out of
 *                                   range, addresses not 4 KB aligned,
 *                                   start > end, or region escapes the
 *                                   matching XSPI window.
 * @retval k_ra_err_conflict        Region overlaps the other channel's
 ...
 *       until ``ra_dotf_select_region`` runs.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                  Region promoted.
 * @retval k_ra_err_invalid_arg     Out-of-range channel or region_id.
 * @retval k_ra_err_invalid_state   The slot has not been staged.
 *
 * @pre ``ra_dotf_set_region(channel, ..., region_id=region_id)``
 *      previously returned ``k_ra_ok``.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                  Descriptor returned.
 * @retval k_ra_err_null_ptr        ``region`` is NULL.
 * @retval k_ra_err_invalid_arg     Channel out of range.
 * @retval k_ra_err_invalid_state   No region active on ``channel``.
 *
 ...
 * ``ra_dotf_enable`` (or ``ra_dotf_rotate_key``) is called. The HAL
 * does not unwrap the key -- that work belongs to ra_rsip and runs
 * before this call. The handle's ``size`` field is mirrored into the
 * driver's REG00 cache so subsequent ``ra_dotf_enable`` writes the
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                Key staged.
 * @retval k_ra_err_null_ptr      ``handle`` is NULL.
 * @retval k_ra_err_invalid_arg   Channel out of range or handle invalid.
 *
 ...
 * @param[in] iv_words   ``k_ra_dotf_iv_word_count`` words of new IV
 *                       (or NULL to reuse the current IV).
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                  Rotation complete.
 * @retval k_ra_err_null_ptr        ``new_handle`` is NULL.
 * @retval k_ra_err_invalid_arg     Channel out of range or handle invalid.
 * @retval k_ra_err_invalid_state   No region active on the channel.
 *
 * @pre ``ra_dotf_install_key`` was called at least once.
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                Words written.
 * @retval k_ra_err_null_ptr      ``iv_words`` is NULL.
 * @retval k_ra_err_invalid_arg   Channel out of range.
 *
 ...
 * @return ``ra_err_t`` error code.
 *
 * @pre ``ra_dotf_set_region`` + ``ra_dotf_select_region`` have been
 *      called for ``channel``.
 * @pre ``ra_dotf_install_key`` has been called for ``channel``.
 *
 ...
 *
 * @param[in] channel Channel index 0..1.
 * @param[in] level   ``k_ra_dotf_sca_off`` / ``standard`` / ``max``.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                Cached and (if armed) written through.
 * @retval k_ra_err_invalid_arg   Channel out of range or unknown level.
 *
 * @pre ``ra_dotf_init`` has run.
 *
 ...
 *
 * @param[in] channel Channel index 0..1.
 * @param[in] size    ``k_ra_dotf_key_size_*``.
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                Size cached; if armed, REG00 updated.
 * @retval k_ra_err_invalid_arg   Channel out of range or unknown size.
 *
 * @pre ``ra_dotf_init`` has run.
 *
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                Self-test triggered.
 ...
 * forwards the channel index to ``ra_dotf_dispatch`` which fires this
