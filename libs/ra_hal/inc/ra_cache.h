 ...
 *
 * @note On a host build (`RA_SIMULATOR_MODE`) the SCB window is backed by the
 ...
 * @since 0.1.0
 */
[[nodiscard]] uint32_t ra_cache_dcache_line_bytes(void);
 ...
 *
 * @return Error code.
 * @retval k_ra_ok           Lines cleaned (or `size == 0`).
 ...
 *
 * @return Error code.
 * @retval k_ra_ok           Lines invalidated (or `size == 0`).
 * @retval k_ra_err_null_ptr `addr` was NULL.
 *
 * @pre `addr` is non-NULL (unless `size == 0`).
 ...
 *
 * @return Error code.
 * @retval k_ra_ok           Lines cleaned and invalidated (or `size == 0`).
 * @retval k_ra_err_null_ptr `addr` was NULL.
 *
 * @pre `addr` is non-NULL (unless `size == 0`).
 ...
 *          those writes; use ::ra_cache_dcache_clean_invalidate_by_addr instead.
 * @since 0.1.0
 */
