 ...
 * in `ra_touch.h`, the SMBus 3.2 layer in `ra_smbus.h`) must not care
 ...
 * in their config instead of naming `ra_i2c_*` or `ra_i3c_*` directly.
 *
 * The seam deliberately lives at Ring 3, NOT in the `ra_io` fabric: the
 * peripheral-agnostic bus facade `ra_io_i2c_bus.h` sits at [Ring 4 / PAL],
 * and a Ring-3 driver depending on it would invert ring ordering (see
 * `docs/RING_AND_WORLD.md` and the identical `ra_vsource` /
 * `ra_io_blockdev` split). The sanctioned bridge is the Ring-4 adapter
 * `ra_io_i2c_bus_as_ops()`, which fills this struct with trampolines into
 * a bound ::ra_io_i2c_bus_t; the app owns the bus, binds a backend, and
 ...
 * points at (the Ring-4 `ra_io_i2c_bus_as_ops()` points it at the bound
 * ::ra_io_i2c_bus_t handle, which must therefore out-live the driver).
 ...
 * @code
 * ra_io_i2c_bus_t bus = {};
 * (void)ra_io_i2c_bus_bind_i3c_compat(&bus, 0U);
 * ra_i2c_bus_ops_t ops = {};
 * (void)ra_io_i2c_bus_as_ops(&bus, &ops);
 * ra_touch_cfg_t cfg = { .bus = ops, ... };
 * @endcode
 *
 * @see ra_io_i2c_bus_as_ops  Ring-4 binder over the `ra_io` I2C facade.
 * @see ra_touch_cfg_t        Ring-3 consumer (GT911 touch driver).
 * @see ra_smbus_cfg_t        Ring-3 consumer (SMBus 3.2 layer).
 *
 * @since 0.1.0
 */
...
typedef struct {
   ...
   * bus injects a repeated-START. Matches the `ra_i2c_write` contract
   * (`ra_i3c_write` uses the inverted `restart` flag; binders adapt).
   *
   ...
   *
   * @return `ra_err_t` forwarded from the bound transport.
   */
  ra_err_t (*write)(void* ctx, uint8_t addr, const uint8_t* data, uint32_t len, bool send_stop);
   ...
   * @return `ra_err_t` forwarded from the bound transport.
   */
  ra_err_t (*read)(void* ctx, uint8_t addr, uint8_t* data, uint32_t len);
   ...
   * issues STOP. Matches `ra_i2c_transfer` / `ra_i3c_transfer`.
   *
   * @param[in]  ctx    Binder-owned cookie (the `ctx` member below).
   * @param[in]  addr   7-bit peripheral address.
