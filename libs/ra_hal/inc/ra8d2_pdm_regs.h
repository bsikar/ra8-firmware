...
typedef enum : uintptr_t {
  k_ra_pdm_base_addr = 0x40256000UL, /**< PDM-IF Secure base (HUM Ch 49 p 3190). */
} ra_pdm_addr_t;
...
typedef enum : uint8_t {
  k_ra_pdm_fifo_depth = 32U, /**< PDDRR FIFO depth (samples). */
} ra_pdm_fifo_t;
 ...
 *
 * @details
 * One instance per channel; ``ra_pdm()->CH[n]`` selects channel n. Every
 ...
 *
 * @details
 * Accessed through ``ra_pdm()``. The common bank drives channel-wide
 ...
 */
typedef enum : uint32_t {
  k_ra_pdm_pdmdsr_inpsel   = 0x1U, /**< [0]     Input data edge select.   */
  k_ra_pdm_pdmdsr_sfmd_pos = 4U,   /**< [6:4]   Sinc filter mode shift.   */
  k_ra_pdm_pdmdsr_hfis_pos = 8U,   /**< [9:8]   High-pass input shift.    */
  k_ra_pdm_pdmdsr_cfis_pos = 12U,  /**< [13:12] Compensation input shift. */
  k_ra_pdm_pdmdsr_lfis_pos = 16U,  /**< [17:16] Low-pass input shift.     */
  k_ra_pdm_pdmdsr_sdma_pos = 24U,  /**< [25:24] Sound-detect avg mode.    */
  k_ra_pdm_pdmdsr_dbis_pos = 28U,  /**< [31:28] Data buffer input shift.  */
} ra_pdm_pdmdsr_bits_t;

/**
 * @enum ra_pdm_pdsfcr_bits_t
 * @brief PDSFCRCHn (Sinc Filter Control) field shifts (HUM Ch 49.2.20 p 3209).
 */
typedef enum : uint32_t {
  k_ra_pdm_pdsfcr_ckdiv_pos   = 0U,  /**< [3:0]   PDM_CLKn dividend ratio. */
  k_ra_pdm_pdsfcr_sincdec_pos = 16U, /**< [23:16] Sinc decimation ratio.   */
  k_ra_pdm_pdsfcr_sincrng_pos = 24U, /**< [28:24] Sinc output valid range. */
} ra_pdm_pdsfcr_bits_t;

/**
 * @enum ra_pdm_status_bits_t
 * @brief Trigger / status / data-read bit masks (HUM Ch 49.2).
 */
typedef enum : uint32_t {
  k_ra_pdm_pdsr_state    = 0x1U,        /**< PDSR[0]  channel running.      */
  k_ra_pdm_pdsr_drf      = 0x4U,        /**< PDSR[2]  data reception flag.  */
  k_ra_pdm_pdscr_clr_all = 0x08070002U, /**< PDSCR clear-all write pattern. */
  k_ra_pdm_pdstrtr_strt  = 0x1U,        /**< PDSTRTR[0] start trigger.      */
  k_ra_pdm_pdstptr_stp   = 0x1U,        /**< PDSTPTR[0] stop trigger.       */
  k_ra_pdm_pddrcr_datre  = 0x1U,        /**< PDDRCR[0] data read enable.    */
} ra_pdm_status_bits_t;

/**
 * @enum ra_pdm_data_bits_t
 * @brief PDDRRCHn / PDDSRCHn field masks (HUM Ch 49.2.36, 49.2.37).
 */
typedef enum : uint32_t {
  k_ra_pdm_pddrr_dat_mask = 0x000FFFFFU, /**< PDDRR[19:0] 20-bit PCM.      */
  k_ra_pdm_pddrr_sign_bit = 0x00080000U, /**< PDDRR[19]   sign bit.        */
  k_ra_pdm_pddrr_sign_ext = 0xFFF00000U, /**< High bits set on negatives.  */
  k_ra_pdm_pddsr_num_mask = 0x000000FFU, /**< PDDSR[7:0]  FIFO fill count. */
} ra_pdm_data_bits_t;
 ...
 */
static inline volatile r_pdm_ch_regs_t* ra_pdm_ch(uint8_t ch)
{
  return &ra_pdm()->CH[ch];
}
