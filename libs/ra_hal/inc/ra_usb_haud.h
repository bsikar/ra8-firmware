 ...
 *
 * Lifecycle (mirrors `ra_usb_hcdc.h` / `ra_usb_hhid.h` shape):
 *
 *  1. `ra_usb_haud_init(speed)` flips the controller to host mode,
 ...
 *  5. `ra_usb_haud_send_samples` / `ra_usb_haud_recv_samples` push and
 *     drain isochronous packets to / from a connected
 *     speaker / microphone.
 *  6. `ra_usb_haud_close()` drops bus power and releases the device.
 *
...
/**
 * @enum ra_usb_haud_ac_subtype_t
 * @brief Class-specific AC interface descriptor subtypes (USB Audio
 *        1.0 sec A.5 "Audio Class-Specific AC Interface Descriptor
 *        Subtypes").
 */
typedef enum : uint8_t {
  k_ra_haud_ac_header          = 0x01U, /**< HEADER.          */
  k_ra_haud_ac_input_terminal  = 0x02U, /**< INPUT_TERMINAL.  */
  k_ra_haud_ac_output_terminal = 0x03U, /**< OUTPUT_TERMINAL. */
  k_ra_haud_ac_mixer_unit      = 0x04U, /**< MIXER_UNIT.      */
  k_ra_haud_ac_selector_unit   = 0x05U, /**< SELECTOR_UNIT.   */
  k_ra_haud_ac_feature_unit    = 0x06U, /**< FEATURE_UNIT.    */
  k_ra_haud_ac_processing_unit = 0x07U, /**< PROCESSING_UNIT. */
  k_ra_haud_ac_extension_unit  = 0x08U, /**< EXTENSION_UNIT.  */
} ra_usb_haud_ac_subtype_t;

/**
 * @enum ra_usb_haud_as_subtype_t
...
/**
 * @enum ra_usb_haud_format_type_t
 * @brief Audio-Streaming format-type codes (USB Audio Data Formats 1.0
 *        sec 2 "Format Type Codes").
 */
typedef enum : uint8_t {
  k_ra_haud_format_type_undefined = 0x00U, /**< FORMAT_TYPE_UNDEFINED. */
  k_ra_haud_format_type_i         = 0x01U, /**< FORMAT_TYPE_I (PCM).   */
  k_ra_haud_format_type_ii        = 0x02U, /**< FORMAT_TYPE_II.        */
  k_ra_haud_format_type_iii       = 0x03U, /**< FORMAT_TYPE_III.       */
} ra_usb_haud_format_type_t;

/**
 * @enum ra_usb_haud_request_t
...
/**
 * @enum ra_usb_haud_packet_t
 * @brief Packet sizing for the attached device's isochronous endpoints.
 */
typedef enum : uint16_t {
  k_ra_haud_iso_max_packet_default = 192U,  /**< Common 48 kHz / 16-bit / stereo. */
  k_ra_haud_iso_max_packet_fs      = 1023U, /**< FS isochronous ceiling.          */
  k_ra_haud_iso_max_packet_hs      = 1024U, /**< HS isochronous ceiling.          */
} ra_usb_haud_packet_t;

/**
 * @enum ra_usb_haud_default_t
...
typedef enum : uint32_t {
  k_ra_haud_default_sample_rate_hz = 48000U, /**< 48 kHz default. */
  k_ra_haud_default_channels       = 2U,     /**< Stereo.         */
  k_ra_haud_default_bits           = 16U,    /**< 16-bit PCM.     */
} ra_usb_haud_default_t;

/**
 * @enum ra_usb_haud_format_limit_t
 * @brief Range checks for `ra_usb_haud_set_format`.
 */
typedef enum : uint32_t {
  k_ra_haud_min_channels       = 1U,      /**< Mono.                 */
  k_ra_haud_max_channels       = 8U,      /**< 7.1 surround ceiling. */
  k_ra_haud_min_bits           = 8U,      /**< 8-bit ceiling.        */
  k_ra_haud_max_bits           = 32U,     /**< 32-bit ceiling.       */
  k_ra_haud_min_sample_rate_hz = 8000U,   /**< 8 kHz min.            */
  k_ra_haud_max_sample_rate_hz = 192000U, /**< 192 kHz max.          */
} ra_usb_haud_format_limit_t;

/**
 * @struct ra_usb_haud_device_t
 ...
 */
typedef void (*ra_usb_haud_attach_fn_t)(void* ctx, const ra_usb_haud_device_t* device);
 ...
 *
 * @details
 * Initialises the underlying `ra_usb` driver in HOST mode for `speed`,
 ...
 * @retval k_ra_ok Host-Audio ready, awaiting attach.
 * @retval k_ra_err_invalid_arg `speed` out of range.
 * @retval k_ra_err_hw_init_failed Underlying `ra_usb_host_init` failed.
 *
 ...
 * @see ra_usb_haud_attach_callback
 * @see ra_usb_haud_close
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_haud_init(ra_usb_speed_t speed);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_haud_close(void);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_haud_attach_callback(ra_usb_haud_attach_fn_t on_attach, void* ctx);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_haud_set_volume(uint8_t channel, int16_t volume);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_haud_set_mute(uint8_t channel, bool mute);
 ...
 * micro-frame return `k_ra_err_busy` for the caller to retry.
 *
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_haud_send_samples(const uint8_t* buf, uint16_t len_bytes);
 ...
 * (isochronous IN) into `buf`. Returns `k_ra_err_no_data` if the pipe
 ...
 */
[[nodiscard]] ra_err_t
ra_usb_haud_recv_samples(uint8_t* buf, uint16_t max_len_bytes, uint16_t* got_len_bytes);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_usb_haud_step(void);
