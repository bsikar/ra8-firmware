 ...
 *    to fetch.
 *  - ``ra_bscan_get_status()`` -- report the recorded TAP instruction
 *    plus initialisation state.
 *  - ``ra_bscan_set_instruction()`` -- record (NOT actually load) the
...
/**
 * @struct ra_bscan_status_t
 * @brief Snapshot of the firmware-side TAP bookkeeping object.
 *
 * @details
 * Returned by ``ra_bscan_get_status``. Reflects only what firmware
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok          Success, ``*out`` is valid.
 * @retval k_ra_err_null_ptr ``out`` is NULL.
 * @retval k_ra_err_not_initialized ``ra_bscan_init`` has not been called.
 *
 ...
 * @retval k_ra_err_null_ptr ``out`` is NULL.
 *
 * @pre ``out`` is a writable ``ra_bscan_status_t`` object.
 * @pre Single-threaded read or caller serialises access.
 * @post Driver state unchanged.
 * @post ``*out`` reflects the most recent calls to ``ra_bscan_init``,
 *       ``ra_bscan_deinit``, and ``ra_bscan_set_instruction``.
 *
 ...
 *                 ``ra_<short>_clear_status(uint32_t)`` shape used by
 *                 sibling HAL drivers.
 *
 * @return ra_err_t
 * @retval k_ra_ok                Success.
 * @retval k_ra_err_invalid_arg   ``mask`` is non-zero.
 * @retval k_ra_err_not_initialized ``ra_bscan_init`` not called.
 *
 * @pre ``ra_bscan_init`` has been called.
 * @pre ``mask`` is 0.
 ...
 * external fixture.
 *
 * @param[in] instr One of the named opcodes in ``ra_bscan_instr_t``.
 *
 * @return ra_err_t
 * @retval k_ra_ok                Recorded.
 * @retval k_ra_err_invalid_arg   ``instr`` is a reserved opcode.
 * @retval k_ra_err_not_initialized ``ra_bscan_init`` not called.
 *
 * @pre ``ra_bscan_init`` has been called.
 ...
 *
 * @see ra_bscan_get_status
 * @see ra_bscan_clear_status
 * @since 0.1.0
 */
