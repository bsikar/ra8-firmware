...
typedef struct {
  ...
  volatile uint32_t _r2[k_ra_dma_pad0_words];    /**< +0x04..0x0F reserved.                   */
  volatile uint8_t  DMCTL;                       /**< +0x10 DMAC Control (PR / ERCH).         */
  volatile uint8_t  _r3;                         /**< +0x11 reserved.                         */
  volatile uint16_t _r4;                         /**< +0x12 reserved.                         */
  volatile uint32_t _r5[k_ra_dma_pad1_words];    /**< +0x14..0x3F reserved.                   */
  volatile uint32_t DMECHR;                      /**< +0x40 Error-channel status.             */
  volatile uint32_t _r6[k_ra_dma_pad2_words];    /**< +0x44..0x7F reserved.                   */
  volatile uint32_t DELSR[k_ra_dma_delsr_count]; /**< +0x80 ELC link sel.                     */
} r_dma_shared_regs_t;
...
typedef enum : uint16_t {
  k_ra_dmtmd_dctg_pos = 0U,  /**< DCTG[1:0] bit 0..1 -- request source. */
  k_ra_dmtmd_sz_pos   = 8U,  /**< SZ[1:0]   bit 8..9 -- transfer size.  */
  k_ra_dmtmd_tkp_pos  = 10U, /**< TKP       bit 10   -- transfer keep.  */
  k_ra_dmtmd_dts_pos  = 12U, /**< DTS[1:0]  bit 12..13 -- repeat area.  */
  k_ra_dmtmd_md_pos   = 14U, /**< MD[1:0]   bit 14..15 -- mode.         */
} ra_dmtmd_pos_t;

typedef enum : uint16_t {
  k_ra_dmtmd_dctg_mask = (uint16_t)(0x3U << k_ra_dmtmd_dctg_pos),
  k_ra_dmtmd_sz_mask   = (uint16_t)(0x3U << k_ra_dmtmd_sz_pos),
  k_ra_dmtmd_tkp_mask  = (uint16_t)(0x1U << k_ra_dmtmd_tkp_pos),
  k_ra_dmtmd_dts_mask  = (uint16_t)(0x3U << k_ra_dmtmd_dts_pos),
  k_ra_dmtmd_md_mask   = (uint16_t)(0x3U << k_ra_dmtmd_md_pos),
} ra_dmtmd_mask_t;

typedef enum : uint16_t {
  k_ra_dmtmd_dctg_software = 0x0U, /**< 00b: software request. */
  k_ra_dmtmd_dctg_hardware = 0x1U, /**< 01b: hardware request. */
} ra_dmtmd_dctg_t;

typedef enum : uint16_t {
  k_ra_dmtmd_sz_byte = 0x0U, /**< 00b: 8-bit.  */
  k_ra_dmtmd_sz_half = 0x1U, /**< 01b: 16-bit. */
  k_ra_dmtmd_sz_word = 0x2U, /**< 10b: 32-bit. */
  k_ra_dmtmd_sz_long = 0x3U, /**< 11b: 64-bit. */
} ra_dmtmd_sz_t;

typedef enum : uint16_t {
  k_ra_dmtmd_dts_dest_repeat = 0x0U, /**< 00b: dest is repeat/block area. */
  k_ra_dmtmd_dts_src_repeat  = 0x1U, /**< 01b: src is repeat/block area.  */
  k_ra_dmtmd_dts_none        = 0x2U, /**< 10b: no repeat/block area.      */
} ra_dmtmd_dts_t;
...
typedef enum : uint8_t {
  k_ra_dmint_darie_pos = 0U, /**< DARIE: dst extended-repeat overflow IRQ. */
  k_ra_dmint_sarie_pos = 1U, /**< SARIE: src extended-repeat overflow IRQ. */
  k_ra_dmint_rptie_pos = 2U, /**< RPTIE: repeat-size end IRQ.              */
  k_ra_dmint_esie_pos  = 3U, /**< ESIE:  transfer-escape end IRQ.          */
  k_ra_dmint_dtie_pos  = 4U, /**< DTIE:  transfer-end IRQ.                 */
} ra_dmint_pos_t;

typedef enum : uint8_t {
  k_ra_dmint_darie_mask = (uint8_t)(0x1U << k_ra_dmint_darie_pos),
  k_ra_dmint_sarie_mask = (uint8_t)(0x1U << k_ra_dmint_sarie_pos),
  k_ra_dmint_rptie_mask = (uint8_t)(0x1U << k_ra_dmint_rptie_pos),
  k_ra_dmint_esie_mask  = (uint8_t)(0x1U << k_ra_dmint_esie_pos),
  k_ra_dmint_dtie_mask  = (uint8_t)(0x1U << k_ra_dmint_dtie_pos),
} ra_dmint_mask_t;
...
typedef enum : uint16_t {
  k_ra_dmamd_dara_mask = (uint16_t)(0x1FU << k_ra_dmamd_dara_pos),
  k_ra_dmamd_dadr_mask = (uint16_t)(0x1U << k_ra_dmamd_dadr_pos),
  k_ra_dmamd_dm_mask   = (uint16_t)(0x3U << k_ra_dmamd_dm_pos),
  k_ra_dmamd_sara_mask = (uint16_t)(0x1FU << k_ra_dmamd_sara_pos),
  k_ra_dmamd_sadr_mask = (uint16_t)(0x1U << k_ra_dmamd_sadr_pos),
  k_ra_dmamd_sm_mask   = (uint16_t)(0x3U << k_ra_dmamd_sm_pos),
} ra_dmamd_mask_t;
...
typedef enum : uint8_t {
  k_ra_dmcnt_dte_pos  = 0U, /**< DMCNT.DTE bit 0 (HUM 17.2.14, p 743). */
  k_ra_dmcnt_dte_mask = 0x1U,
} ra_dmcnt_bits_t;

typedef enum : uint8_t {
  k_ra_dmreq_swreq_pos  = 0U, /**< DMREQ.SWREQ bit 0. */
  k_ra_dmreq_clrs_pos   = 4U, /**< DMREQ.CLRS  bit 4. */
  k_ra_dmreq_swreq_mask = (uint8_t)(0x1U << k_ra_dmreq_swreq_pos),
  k_ra_dmreq_clrs_mask  = (uint8_t)(0x1U << k_ra_dmreq_clrs_pos),
} ra_dmreq_bits_t;

typedef enum : uint8_t {
  k_ra_dmsts_esif_pos  = 0U, /**< DMSTS.ESIF bit 0. */
  k_ra_dmsts_dtif_pos  = 4U, /**< DMSTS.DTIF bit 4. */
  k_ra_dmsts_act_pos   = 7U, /**< DMSTS.ACT  bit 7. */
  k_ra_dmsts_esif_mask = (uint8_t)(0x1U << k_ra_dmsts_esif_pos),
  k_ra_dmsts_dtif_mask = (uint8_t)(0x1U << k_ra_dmsts_dtif_pos),
  k_ra_dmsts_act_mask  = (uint8_t)(0x1U << k_ra_dmsts_act_pos),
} ra_dmsts_bits_t;

typedef enum : uint8_t {
  k_ra_dmast_dmst_pos  = 0U, /**< DMAST.DMST bit 0 (HUM 17.2.20, p 749). */
  k_ra_dmast_dmst_mask = 0x1U,
} ra_dmast_bits_t;
...
typedef enum : uint32_t {
  k_ra_dmcra_low_pos   = 0U,
  k_ra_dmcra_low_mask  = 0x3FFU << k_ra_dmcra_low_pos,
  k_ra_dmcra_high_pos  = 16U,
  k_ra_dmcra_high_mask = 0x3FFU << k_ra_dmcra_high_pos,
} ra_dmcra_bits_t;

typedef enum : uint32_t {
  k_ra_dmbs_low_pos   = 0U,
  k_ra_dmbs_low_mask  = 0xFFFFU << k_ra_dmbs_low_pos,
  k_ra_dmbs_high_pos  = 16U,
  k_ra_dmbs_high_mask = (uint32_t)0xFFFFU << k_ra_dmbs_high_pos,
} ra_dmbs_bits_t;
...
static inline volatile r_dma_shared_regs_t* ra_dma_shared(void)
{
  return (volatile r_dma_shared_regs_t*)k_ra_dma_base_addr;
}
