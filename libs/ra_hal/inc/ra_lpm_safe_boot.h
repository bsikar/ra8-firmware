 ...
 *
 * ``ra_lpm_safe_boot()`` is the antidote: it unlocks PRCR.PRC1,
...
/**
 * @enum ra_lpm_safe_boot_prcr_t
 * @brief PRCR keyed values used by ra_lpm_safe_boot.
 *
 * @details
 ...
 *
 * @note No-op on host (``RA_SIMULATOR_MODE``).
 *
...
__attribute__((always_inline)) static inline void ra_lpm_safe_boot(void)
{
  ...
  *prcr    = (uint16_t)k_ra_lpm_safe_boot_prcr_unlock;
  *lpscr   = (uint8_t)k_ra_lpm_safe_boot_lpscr_sleep;
  *sbycr   = (uint8_t)k_ra_lpm_safe_boot_sbycr_reset;
  *dpsbycr = (uint8_t)k_ra_lpm_safe_boot_dpsbycr_reset;
  *prcr    = (uint16_t)k_ra_lpm_safe_boot_prcr_relock;
