 ...
 * @return String-pool offset, or ::k_ra8_book_nil on failure.
 * @retval k_ra8_book_nil Decode or string-pool capacity failed.
                                                  ...
                                                  ra8_xml_span_t    span)
{
  ...
  size_t decoded = 0U;
  if (ra8_xml_decoded_size(source, source_len, span, &decoded) != k_ra8_ok) {
    ctx->failed = true;
    return (uint32_t)k_ra8_book_nil;
  }
 ...
 * @param[in] node Live node to append.
 * @return Repository error code.
 ...
 * @pre @p node and its parent are live builder nodes.
 * @post Success records @p node as the parent's last child.
...
{
  size_t decoded = 0U;
  if (ra8_xml_decoded_size(source, source_len, event->markup, &decoded) != k_ra8_ok) {
    return k_ra8_err_validation_failed;
  }
  if (decoded == 0U) {
    return k_ra8_ok;
 ...
 * @pre @p source already passed ::ra8_xml_validate.
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_select(const uint8_t*       source,
                                              size_t               length,
                                              ra8_xml_workspace_t* xml,
                                              priv_selection_t*    out)
{
  ra8_xml_reader_t reader          = {};
  ra8_err_t        err             = ra8_xml_reader_init(&reader, source, length, xml);
...
{
  ra8_xml_reader_t reader = {};
  ra8_err_t        err    = ra8_xml_reader_init(&reader, source, length, &workspace->xml);
  bool             active = false;
  while (err == k_ra8_ok) {
    ra8_xml_event_t event = {};
    err                   = ra8_xml_reader_next(&reader, &event);
                                       ...
                                       ra8_rabook_xml_workspace_t* workspace)
{
  ...
  uint32_t root = (uint32_t)k_ra8_book_nil;
  if (err == k_ra8_ok) {
  ...
  if (chapter == (uint32_t)k_ra8_book_nil) {
    *ctx = checkpoint;
    return k_ra8_err_no_mem;
  }
  return k_ra8_ok;
