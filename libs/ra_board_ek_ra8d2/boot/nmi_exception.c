/**
 * @file libs/ra_board_ek_ra8d2/boot/nmi_exception.c
 * @brief RA8D2 NMI handler -- record the ICU cause instead of a bare trap
 *
 * @par Tag
 * [Ring 1 / Boot] {World: S}
 *
 ...
 * cause into `ra_exception_report_nmi()`, which snapshots both --
 * plus the SCB diagnostics -- into the fixed-SRAM
 * `g_ra_exception_last` record, best-effort logs them, and parks
 ...
 * into `ra_exception_report_nmi()`. The register is read here -- not
 * in libs/ra_core -- because NMISR is an RA8D2 ICU register, outside
 ...
 *
 * @post `g_ra_exception_last` carries exc_number 2 plus the NMISR cause.
 * @post Control never returns; the CPU halts at a named symbol.
 *
 * @note Not thread-safe (single NMI context by construction).
 *
 * @see ra_exception_report_nmi()  The recording + halt path.
 *
 * @since 0.1.0
 */
[[noreturn]] void ra_board_nmi_report(const ra_exception_frame_t* frame);
[[noreturn]] void ra_board_nmi_report(const ra_exception_frame_t* frame)
{
 ...
 * `ra_board_nmi_report()`, which appends the ICU cause. Naked asm is
 ...
 *
 * @post Control transfers to `ra_board_nmi_report()` and never returns.
