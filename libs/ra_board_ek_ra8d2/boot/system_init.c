/**
 * @file libs/ra_board_ek_ra8d2/boot/system_init.c
 ...
 * @pre g_ra_vector_table_start resolves to the .vectors base.
...
static void internal_set_vtor(void)
{
  ...
  ra_boot_write32(k_ra_scb_vtor_addr, (uint32_t)(uintptr_t)g_ra_vector_table_start);
}
...
static void internal_enable_fpu(void)
{
  ...
  uint32_t cpacr = ra_boot_read32(k_ra_scb_cpacr_addr);
  cpacr |= k_ra_cpacr_cp10_cp11_full_access;
  ra_boot_write32(k_ra_scb_cpacr_addr, cpacr);
  ra_boot_dsb();
  ra_boot_isb();
}
...
static void internal_enable_fpu_lazy_stack(void)
{
  ...
  uint32_t fpccr = ra_boot_read32(k_ra_fpu_fpccr_addr);
  fpccr |= k_ra_fpccr_lspen | k_ra_fpccr_aspen;
  ra_boot_write32(k_ra_fpu_fpccr_addr, fpccr);
}
...
[[maybe_unused]] static void internal_enable_icache(void)
{
  ...
  ra_boot_dsb();
  ra_boot_write32(k_ra_scb_iciallu, 0U);
  ra_boot_dsb();
  ra_boot_isb();
...
static void internal_set_priority_grouping(void)
{
  ...
  ra_boot_write32(k_ra_nvic_aircr, k_ra_aircr_vectkey | k_ra_aircr_prigroup_4);
}
...
[[maybe_unused]] static void internal_mpu_init(void)
{
  ...
  ra_boot_dsb();
  ra_boot_write32(k_ra_mpu_ctrl_addr, k_ra_mpu_ctrl_enable | k_ra_mpu_ctrl_privdefena);
  ra_boot_dsb();
  ra_boot_isb();
}
