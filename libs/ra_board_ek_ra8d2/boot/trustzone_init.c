/**
 * @file libs/ra_board_ek_ra8d2/boot/trustzone_init.c
...
{
  ra_boot_write32(k_ra_sau_rnr_addr, region);
  ra_boot_write32(k_ra_sau_rbar_addr, base);
  uint32_t rlar = limit | (uint32_t)k_ra_sau_rlar_enable;
  if (is_nsc) {
    rlar |= (uint32_t)k_ra_sau_rlar_nsc;
  }
  ra_boot_write32(k_ra_sau_rlar_addr, rlar);
}
...
void ra_trustzone_init(void)
{
  ...
  ra_boot_dsb();
  ra_boot_write32(k_ra_sau_ctrl_addr, (uint32_t)k_ra_sau_ctrl_enable);
  ra_boot_dsb();
  ra_boot_isb();
#endif
}
