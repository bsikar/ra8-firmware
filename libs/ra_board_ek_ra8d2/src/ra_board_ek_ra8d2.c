...
static ra_err_t internal_eth_eswm_select_rgmii(void)
{
   ...
   * TXCIDE = 1 (on-chip TX clock delay) plus the GPY111's MIICTRL.RXSKEW
   * = 1.0 ns at the PHY provides the full RGMII clock-to-data
   * alignment. Issue #21 iteration #7 Candidate C bench: TXCIDE = 0
   * killed even small-frame ARP/ping, so the chip-side delay is
   * required; TXCIDE = 1 stays. */
