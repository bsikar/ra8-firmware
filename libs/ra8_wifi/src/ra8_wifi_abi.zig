...
/// Component tag on this facade's log lines, matching the C's `RA8_WIFI_TAG`.
...
/// Caller-owned handle (`ra8_wifi_t`). `state` stays a raw byte: a caller's
...
pub export fn ra8_wifi_init(wifi: ?*Wifi, cfg: ?*const Config) u16 {
...
pub export fn ra8_wifi_deinit(wifi: ?*Wifi) u16 {
    const handle = wifi orelse return nullPtr("wifi");
...
pub export fn ra8_wifi_connect(wifi: ?*Wifi, ssid: ?[*:0]const u8, psk: ?[*:0]const u8) u16 {
    const handle = wifi orelse return nullPtr("wifi");
...
pub export fn ra8_wifi_disconnect(wifi: ?*Wifi) u16 {
    const handle = wifi orelse return nullPtr("wifi");
    if (!handle.open) return implementation.err_not_initialized;
...
pub export fn ra8_wifi_wait_ip(wifi: ?*Wifi, out: ?*Lease) u16 {
    const handle = wifi orelse return nullPtr("wifi");
...
pub export fn ra8_wifi_get_ip(wifi: ?*const Wifi, out: ?*Lease) u16 {
    const handle = wifi orelse return nullPtr("wifi");
    const sink = out orelse return nullPtr("out");
    if (!handle.open) return implementation.err_not_initialized;
    sink.* = handle.lease;
...
pub export fn ra8_wifi_status(wifi: ?*const Wifi, out: ?*Status) u16 {
    const handle = wifi orelse return nullPtr("wifi");
    const sink = out orelse return nullPtr("out");
    if (!handle.open) return implementation.err_not_initialized;
...
pub export fn ra8_wifi_poll(wifi: ?*Wifi, out: ?*u8) u16 {
    const handle = wifi orelse return nullPtr("wifi");
    const sink = out orelse return nullPtr("out");
...
pub export fn ra8_wifi_get_mac(wifi: ?*Wifi, out: ?*Mac) u16 {
    const handle = wifi orelse return nullPtr("wifi");
    const sink = out orelse return nullPtr("out");
    if (!handle.open) return implementation.err_not_initialized;
...
pub export fn ra8_wifi_get_ap(wifi: ?*Wifi, out: ?*Ap) u16 {
    const handle = wifi orelse return nullPtr("wifi");
    const sink = out orelse return nullPtr("out");
    if (!handle.open) return implementation.err_not_initialized;
    if (!implementation.isAssociated(handle.state)) return implementation.err_invalid_state;
