...
static const char* const s_tag_cbt = "ra8_comic_cbt";
 ...
 * @param[in]     ent  Decoded member from ::ra8_unarch_tar_next.
 ...
 * @pre @p ent came from ::ra8_unarch_tar_next; @p name holds @p nlen bytes.
...
ra8_err_t ra8_comic_cbt_open(ra8_comic_t* c)
{
  ...
  for (uint32_t n = 0U; n < (uint32_t)k_cbt_max_members; ++n) { /* bound: static member cap */
    ...
    const ra8_err_t        nerr = ra8_unarch_tar_next(&c->tar, off, nb, (uint16_t)sizeof(nb), &ent);
                                ...
                                size_t*                 got)
{
  ...
  *got = 0U;
  ...
  ent.size                   = p->raw_size;
  return ra8_unarch_tar_read(&c->tar, &ent, buf, cap, got);
}
