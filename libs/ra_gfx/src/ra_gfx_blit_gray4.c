/**
 * @file ra_gfx_blit_gray4.c
 * @brief Sub-rectangle nearest-neighbour integer-zoom blit for packed gray4 images.
 *
 * @details
 * Implements ra_gfx_blit_gray4_zoom(): the one primitive the reader loupe (#211)
 ...
 * pixels. It shares the single ra_gfx framebuffer binding and the clipped
 ...
 * index in the low nibble -- the same packing ra_gfx_blit_gray8() consumers unpack
...
/**
 * @enum ra_gfx_g4_const_t
 * @brief Nibble-unpack + gray-to-colour expansion constants for the gray4 blit.
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_ra_g4_nib_lo   = 0x0FU, /**< Low-nibble mask.                           */
  k_ra_g4_nib_sh   = 4U,    /**< Nibble shift (and the 4->8-bit replicate). */
  k_ra_g4_rgb_g_sh = 8U,    /**< Green shift of the 0x00RRGGBB gray expand. */
  k_ra_g4_rgb_r_sh = 16U,   /**< Red shift of the 0x00RRGGBB gray expand.   */
} ra_gfx_g4_const_t;
 ...
 * gray level ra_gfx_blit_gray8() would produce for the same nibble.
 *
...
static uint32_t internal_gray4_color(const uint8_t* src, int32_t src_w, int32_t x, int32_t y)
{
  ...
  const uint8_t nib =
    ((flat & 1U) != 0U) ? (uint8_t)(byte & k_ra_g4_nib_lo) : (uint8_t)(byte >> k_ra_g4_nib_sh);
  const uint8_t g8 = (uint8_t)((nib << k_ra_g4_nib_sh) | nib);
  return ((uint32_t)g8 << k_ra_g4_rgb_r_sh) | ((uint32_t)g8 << k_ra_g4_rgb_g_sh) | (uint32_t)g8;
}
...
ra_err_t ra_gfx_blit_gray4_zoom(const uint8_t* src,
                                int32_t        src_w,
                                int32_t        src_h,
                                int32_t        sx,
                                int32_t        sy,
