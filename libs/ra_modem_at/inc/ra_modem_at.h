 ...
 *
 * - ``k_ra_modem_at_state_idle``      -- waiting for command.
 * - ``k_ra_modem_at_state_await_echo``-- expecting the modem to
 *   echo the command we just transmitted (``ATE1`` default).
 * - ``k_ra_modem_at_state_await_resp``-- collecting payload + final
 *   result code.
 * - ``k_ra_modem_at_state_done``      -- result code matched,
 ...
 * via ``ra_modem_at_cfg_t``. Unsolicited-handler slots live in a
...
typedef enum : uint16_t {
  k_ra_modem_at_default_timeout_ms = 1000U,
} ra_modem_at_timeouts_t;

/**
 * @struct ra_modem_at_io_t
 ...
 * production wiring binds these pointers to an ``ra_uart``-backed
...
typedef struct {
   ...
   * @return ``k_ra_ok`` on success, ``ra_err_t`` otherwise.
   */
  ra_err_t (*tx_byte)(void* ctx, uint8_t byte);
   ...
   * @return ``k_ra_ok`` if a byte was popped,
   *         ``k_ra_err_no_data`` if the RX FIFO is empty.
   */
  ra_err_t (*rx_byte)(void* ctx, uint8_t* out_byte);
...
/**
 * @struct ra_modem_at_cfg_t
 * @brief Configuration handed to ``ra_modem_at_init``.
 *
 * @invariant ``line_buf != NULL``.
 * @invariant ``line_buf_len >= 16``.
 */
typedef struct {
  ra_modem_at_io_t io;                 /**< Byte transport (must be fully populated). */
 ...
 * @post Internal state reset to ``k_ra_modem_at_state_idle``.
 ...
 * };
 * (void)ra_modem_at_init(&cfg);
 * @endcode
 *
 * @see ra_modem_at_send_cmd()
 * @see ra_modem_at_register_unsolicited_handler()
 *
 * @since 0.1.0
 */
 ...
 *
 * - the final result code ``OK`` arrives -> ``k_ra_ok``,
 * - the final result code ``ERROR`` (or ``+CME ERROR:``,
 *   ``+CMS ERROR:``) arrives -> ``k_ra_err_hw_error``,
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                  Final ``OK`` (and expected, if set) seen.
 * @retval k_ra_err_null_ptr        ``cmd`` was NULL.
 * @retval k_ra_err_not_initialized ``ra_modem_at_init`` not yet called.
 * @retval k_ra_err_hw_error        Modem replied ``ERROR``/``+CME ERROR``.
 * @retval k_ra_err_hw_timeout      No final result code in time.
 *
 * @pre ``ra_modem_at_init`` returned ``k_ra_ok``.
 * @pre ``cmd`` NUL-terminated and shorter than ``line_buf_len``.
 * @post Driver returned to ``k_ra_modem_at_state_idle``.
 ...
 *
 * @details
 * Identical wait policy to ``ra_modem_at_send_cmd`` but every
 ...
 *
 * @return ``ra_err_t`` error code.
 * @retval k_ra_ok                  Command finished with ``OK``.
 * @retval k_ra_err_null_ptr        ``cmd`` or ``out_buf`` NULL.
 * @retval k_ra_err_invalid_size    ``buf_len`` is 0.
 * @retval k_ra_err_not_initialized ``ra_modem_at_init`` not called.
 * @retval k_ra_err_hw_error        Modem replied ``ERROR``.
 * @retval k_ra_err_hw_timeout      No final result code in time.
 *
 ...
 *
 * @note Not thread-safe with concurrent ``ra_modem_at_send_cmd``.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_modem_at_poll(void);
