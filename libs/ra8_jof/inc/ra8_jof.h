 ...
 * source image is transcoded once (`ra8_jof_produce()`) into a grid of
 ...
 * ## Validation model
 *
 * Atlases arrive from untrusted EPUB content. `ra8_jof_parse()` and
 * `ra8_jof_read_tile()` are fail-closed: every geometry field, both
 ...
 * @see ra8_jof_produce.h  Import-time transcode producer.
...
typedef enum : uint8_t {
  k_ra8_jof_hdr_bytes      = 32U, /**< Header length.                   */
  k_ra8_jof_footer_bytes   = 16U, /**< Footer length.                   */
  k_ra8_jof_index_entry    = 8U,  /**< Bytes per tile-index entry.      */
  k_ra8_jof_magic_len      = 4U,  /**< Magic string length (both ends). */
  k_ra8_jof_ofs_magic      = 0U,  /**< Header: magic "JOF1".            */
  k_ra8_jof_ofs_width      = 4U,  /**< Header: u16 image width.         */
  k_ra8_jof_ofs_height     = 6U,  /**< Header: u16 image height.        */
  k_ra8_jof_ofs_tile_w     = 8U,  /**< Header: u16 tile width.          */
  k_ra8_jof_ofs_tile_h     = 10U, /**< Header: u16 tile height.         */
  k_ra8_jof_ofs_bpp        = 12U, /**< Header: u8 bytes per pixel.      */
  k_ra8_jof_ofs_codec      = 13U, /**< Header: u8 tile codec.           */
  k_ra8_jof_ofs_reserved   = 14U, /**< Header: first reserved byte.     */
  k_ra8_jof_ofs_tile_count = 16U, /**< Header: u32 tile count.          */
  k_ra8_jof_ofs_reserved2  = 20U, /**< Header: second reserved run.     */
  k_ra8_jof_ftr_index_off  = 0U,  /**< Footer: u32 index offset.        */
  k_ra8_jof_ftr_tile_count = 4U,  /**< Footer: u32 tile count.          */
  k_ra8_jof_ftr_total_size = 8U,  /**< Footer: u32 total atlas size.    */
  k_ra8_jof_ftr_magic      = 12U, /**< Footer: magic "JOFE".            */
  k_ra8_jof_idx_ofs_offset = 0U,  /**< Index entry: u32 tile offset.    */
  k_ra8_jof_idx_ofs_length = 4U,  /**< Index entry: u32 tile length.    */
} ra8_jof_layout_t;

/**
 * @enum ra8_jof_limits_t
 ...
 *          `k_ra8_jof_bpp_max` is the largest legal bytes-per-pixel.
 *
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_ra8_jof_max_dim   = 32768U, /**< Max image width/height, pixels. */
  k_ra8_jof_max_tiles = 65536U, /**< Max tiles per atlas.            */
  k_ra8_jof_bpp_max   = 4U,     /**< Max bytes per pixel.            */
} ra8_jof_limits_t;

/**
 * @enum ra8_jof_codec_t
 ...
 */
typedef enum : uint8_t {
  k_ra8_jof_codec_raw     = 0U, /**< Tile stream is the packed pixels.   */
  k_ra8_jof_codec_deflate = 1U, /**< Tile stream is one raw-DEFLATE run. */
} ra8_jof_codec_t;

/**
 * @struct ra8_jof_info_t
 * @brief Parsed + validated geometry of one JOF atlas.
 *
 * @details Filled by `ra8_jof_parse()`; callers treat it as read-only
 ...
 */
typedef ra8_err_t (
  *ra8_jof_pread_fn)(void* ctx, uint64_t offset, uint8_t* buf, size_t len, size_t* got);

/**
 * @struct ra8_jof_memstore_t
 ...
 * @see ra8_jof_memstore_sink()
 * @see ra8_jof_memstore_pread()
 * @since 0.1.0
 */
typedef struct {
...
/**
 * @brief Append `len` bytes to a ::ra8_jof_memstore_t (producer sink).
 *
 * @details Matching `ra8_jof_sink_fn` shape (see the producer header):
 *          bind with `ctx = &store`. Fails closed when the store is full so a
 *          hostile source can never write past `cap`.
 *
 * @param[in] ctx Store to append to (a ::ra8_jof_memstore_t*).
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_jof_memstore_sink(void* ctx, const uint8_t* buf, size_t len);

/**
 * @brief Positioned read from a ::ra8_jof_memstore_t (reader seam).
 *
 * @details Matching ::ra8_jof_pread_fn: bind with `ctx = &store`.
 *          Reads clamp at `store->len`; a read at/after the end reports
 *          `*got == 0` without error, mirroring `ra8_epub_entry_pread`.
 *
 * @param[in]  ctx    Store to read from (a ::ra8_jof_memstore_t*).
 ...
 */
[[nodiscard]] ra8_err_t
ra8_jof_memstore_pread(void* ctx, uint64_t offset, uint8_t* buf, size_t len, size_t* got);
 ...
 * non-zero geometry within ::ra8_jof_limits_t, legal bpp/codec, zeroed
 ...
 * `ra8_jof_read_tile()` (the index itself may be larger than any
 ...
 * @see ra8_jof_read_tile()
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_jof_parse(ra8_jof_pread_fn pread,
                                      void*            pread_ctx,
                                      uint64_t         total_size,
 ...
 *
 * @pre @p info came from a successful `ra8_jof_parse()`.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_jof_tile_dims(const ra8_jof_info_t* info,
                                          uint16_t              tile_x,
                                          uint16_t              tile_y,
                                          uint16_t*             out_w,
                                          uint16_t*             out_h);
 ...
 *          worst case). Size `ra8_jof_read_tile()`'s scratch with this
 ...
 * @pre The atlas codec is one of ::ra8_jof_codec_t.
 * @post No state mutated.
 * @post Return >= @p raw_bytes + 256.
 * @note Thread-safe (pure).
 * @since 0.1.0
 */
[[nodiscard]] uint32_t ra8_jof_stored_bound(uint32_t raw_bytes);
 ...
 *                         `ra8_jof_stored_bound()` of the tile payload
 *                         for deflate atlases.
 ...
 * @retval other                       Propagated from @p pread.
 *
 * @pre @p info came from a successful `ra8_jof_parse()` over the same
 *      backing.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_jof_read_tile(ra8_jof_pread_fn      pread,
                                          void*                 pread_ctx,
