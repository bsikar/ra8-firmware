 ...
 * ``ra_sdmmc_spi.h``: init/deinit, single- and multi-block read/write,
 * erase, capacity/type queries, the ``ra_fs`` backend adapter, and the
 ...
 * ``ra_sdmmc_spi.c``; this unit reaches them through the module-private
 * ``ra_sdmmc_spi_internal.h`` surface. All data-token, data-response, and
...
{
  ra_err_t err = s_sdmmc_spi_state.transport.set_clock(s_sdmmc_spi_state.transport.ctx,
                                                       (uint32_t)k_ra_sdmmc_spi_clock_data_hz);
  if (err != k_ra_ok) {
    return err;
  }
...
{
  ra_err_t err = ra_sdmmc_spi_validate_transport(transport);
  ...
  err = ra_sdmmc_spi_run_init_sequence();
...
{
  if (s_sdmmc_spi_state.card_type == k_ra_sdmmc_spi_type_sdhc) {
    return lba;
  }
...
static ra_err_t internal_cmd_require_ready(sd_cmd_t cmd, uint32_t arg)
{
  ...
  uint8_t r1 = 0U;
  err        = ra_sdmmc_spi_send_command(cmd, arg, &r1);
...
static ra_err_t internal_erase_range(uint32_t lba, uint32_t count)
{
  ...
  err = ra_sdmmc_spi_cs_assert();
  if (err != k_ra_ok) {
    return err;
  }
  uint8_t r1 = 0U;
  err        = ra_sdmmc_spi_send_command(k_sd_cmd_erase, 0U, &r1);
  if (err != k_ra_ok) {
    (void)ra_sdmmc_spi_cs_release();
    return err;
  }
  ...
  err = ra_sdmmc_spi_wait_not_busy_bounded((uint32_t)k_sd_max_erase_poll_bytes);
  (void)ra_sdmmc_spi_cs_release();
  return err;
}
...
static ra_err_t internal_read_block_payload(uint8_t* buf)
{
  for (uint32_t i = 0U; i < (uint32_t)k_ra_sdmmc_spi_block_size; i++) {
    ra_err_t err = ra_sdmmc_spi_xfer_one((uint8_t)k_sd_token_idle, &buf[i]);
    if (err != k_ra_ok) {
      return err;
    }
  }
...
static ra_err_t internal_read_block_crc_check(const uint8_t* buf)
{
  uint8_t crc_hi = 0U;
  uint8_t crc_lo = 0U;
  (void)ra_sdmmc_spi_xfer_one((uint8_t)k_sd_token_idle, &crc_hi);
  (void)ra_sdmmc_spi_xfer_one((uint8_t)k_sd_token_idle, &crc_lo);
...
static ra_err_t internal_read_data_phase(uint32_t lba, uint8_t* buf)
{
  uint8_t  r1 = 0U;
  ra_err_t err =
    ra_sdmmc_spi_send_command(k_sd_cmd_read_single_block, internal_lba_to_arg(lba), &r1);
  if (err != k_ra_ok) {
    return err;
  }
  if (r1 != 0U) {
...
{
  ra_err_t err = ra_sdmmc_spi_send_idle(1U); /* N_WR pad (spec >= 1 byte). */
  if (err != k_ra_ok) {
    return err;
  }
  err = ra_sdmmc_spi_xfer_one(start_token, nullptr);
  if (err != k_ra_ok) {
    return err;
  }
  ...
  if (err != k_ra_ok) {
    ...
    for (uint32_t i = 0U; i < (uint32_t)k_ra_sdmmc_spi_block_size; i++) {
      uint8_t dummy = 0U;
      err           = ra_sdmmc_spi_xfer_one(buf[i], &dummy);
      if (err != k_ra_ok) {
        return err;
      }
    }
  ...
  (void)ra_sdmmc_spi_xfer_one(
    (uint8_t)((uint32_t)(crc >> k_sd_bit_byte) & (uint32_t)k_sd_mask_byte),
    nullptr);
  (void)ra_sdmmc_spi_xfer_one((uint8_t)((uint32_t)crc & (uint32_t)k_sd_mask_byte), nullptr);
  uint8_t response = 0U;
  err              = ra_sdmmc_spi_xfer_one((uint8_t)k_sd_token_idle, &response);
  if (err != k_ra_ok) {
    return err;
  }
...
ra_err_t ra_sdmmc_spi_write_block(uint32_t lba, const uint8_t* buf)
{
  ...
  RA_RETURN_ON_ERROR(err, s_tag, "cs assert");
  uint8_t r1 = 0U;
  err = ra_sdmmc_spi_send_command(k_sd_cmd_write_single_block, internal_lba_to_arg(lba), &r1);
  if (err != k_ra_ok) {
    (void)ra_sdmmc_spi_cs_release();
    return err;
  }
...
static ra_err_t internal_write_multi_stream(const uint8_t* buf, uint32_t count)
{
  ...
  (void)ra_sdmmc_spi_send_idle(1U);
...
ra_err_t ra_sdmmc_spi_write_blocks(uint32_t lba, const uint8_t* buf, uint32_t count)
{
  ...
  uint8_t acmd_r1 = 0U;
  (void)ra_sdmmc_spi_send_acmd(k_sd_acmd_set_wr_blk_erase_count, count, &acmd_r1);
  uint8_t r1 = 0U;
  err        = ra_sdmmc_spi_send_command(k_sd_cmd_write_multi_block, internal_lba_to_arg(lba), &r1);
