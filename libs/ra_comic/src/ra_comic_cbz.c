 ...
 * path `ra_epub_open_streamed` uses -- so only the central directory and one
 * entry at a time are ever fetched. ::ra_comic_cbz_open walks the central
 ...
 * ::ra_comic_cbz_extract inflates one page's encoded image (STORE or DEFLATE)
 * on demand.
 *
 * On firmware, miniz's central-directory and decompressor allocations route
 * through the zero-heap `ra_epub_miniz_alloc` static pool (NASA Rule 3), exactly
 * as `ra_epub` does; the host unit-test build (`RA_SIMULATOR_MODE`) leaves miniz
...
static const char* const s_tag_cbz = "ra_comic_cbz";
...
static_assert(k_ra_comic_zip_bytes >= sizeof(mz_zip_archive),
              "k_ra_comic_zip_bytes too small for mz_zip_archive");
 ...
 * @param[in]  opaque   The comic's ::ra_comic_stream_t (via `m_pIO_opaque`).
...
{
  const ra_comic_stream_t* sm = (const ra_comic_stream_t*)opaque;
  if (sm == nullptr) {
 ...
 *          and inflate allocations to come from the `ra_epub_miniz_alloc` pool;
 *          the host build (`RA_SIMULATOR_MODE`) leaves the default allocator.
 ...
 * @return ra_err_t status (k_ra_ok also for a skipped, non-page entry).
 * @retval k_ra_ok               Entry indexed or intentionally skipped.
 ...
 * @since Version 0.1.0
 */
static ra_err_t s_add_entry(ra_comic_t* c, mz_zip_archive* zip, mz_uint i)
{
  ...
  if (!ra_comic_is_page_name(nb, (uint16_t)nl)) {
    return k_ra_ok;
  }
  mz_zip_archive_file_stat st = {};
  if (mz_zip_reader_file_stat(zip, i, &st) == MZ_FALSE) {
    return k_ra_ok; /* GCOVR_EXCL_LINE -- stat cannot fail on an index locate already found */
  }
...
{
  RA_CHECK_NULL_PTR(c, s_tag_cbz, "cbz open: null c");
  ...
  for (mz_uint i = 0U; i < num; ++i) { /* bound: finite central-directory count */
    const ra_err_t e = s_add_entry(c, zip, i);
    if (e != k_ra_ok) {
...
ra_err_t
ra_comic_cbz_extract(ra_comic_t* c, const ra_comic_page_t* p, uint8_t* buf, size_t cap, size_t* got)
{
  RA_CHECK_NULL_PTR(c, s_tag_cbz, "cbz extract: null c");
  RA_CHECK_NULL_PTR(p, s_tag_cbz, "cbz extract: null p");
  RA_CHECK_NULL_PTR(buf, s_tag_cbz, "cbz extract: null buf");
  RA_CHECK_NULL_PTR(got, s_tag_cbz, "cbz extract: null got");
  *got = 0U;
  if (c->zip_active == 0U) {
...
{
  RA_CHECK_NULL_PTR(c, s_tag_cbz, "cbz close: null c");
  if (c->zip_active != 0U) {
