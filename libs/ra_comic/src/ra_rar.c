/**
 * @file ra_rar.c
 * @brief Implementation of the clean-room RAR4/RAR5 archive walker (ra_rar.h).
 *
 * @par Tag
 * [Ring 4 / Domain] {World: NS}
 *
 * @details
 * Two block grammars behind one iterator. ::ra_rar_open matches the signature and
 * records the first block offset; ::ra_rar_next reads one block header into a
 ...
 * ::ra_rar_extract_stored streams a STORE member's literal bytes out.
 *
 ...
 * truncated block is rejected (::k_ra_err_validation_failed) rather than
 * over-read.
 *
 * @since Version 0.1.0
 *
...
static const char* const s_tag_rar = "ra_rar";
 ...
 *          longer than ::k_ra_rar_vint_max bytes. The value is built in two 32-bit
 ...
 * @post On true, `*pos` advanced by 1..k_ra_rar_vint_max and `*val` set.
...
static bool s_vint(const uint8_t* buf, size_t len, size_t* pos, uint64_t* val)
{
  ...
  for (uint16_t i = 0U; i < (uint16_t)k_ra_rar_vint_max; ++i) { /* bound: <=10 bytes */
    if (*pos >= len) {
      return false;
    }
 ...
 * @since Version 0.1.0
 */
static uint16_t s_copy_name(const ra_rar_t* rar,
                            uint64_t        name_abs,
 ...
 * @since Version 0.1.0
 */
static bool s_rar4_file(const ra_rar_t* rar,
                        uint64_t        off,
                        const uint8_t*  scratch,
                        size_t          n,
                        uint16_t        flags,
                        ...
                        ra_rar_entry_t* out)
{
  ...
  out->method  = (method == (uint8_t)k_rar4_method_stor) ? (uint8_t)k_ra_rar_method_store
                                                         : (uint8_t)k_ra_rar_method_compressed;
  out->pack_size = pack;
  out->unp_size  = unp;
 ...
 * @retval k_ra_ok                    Block decoded.
 * @retval k_ra_err_validation_failed Truncated / non-advancing block.
 * @pre @p rar is bound; @p off < rar->size.
 * @pre @p out is writable.
 * @post On k_ra_ok, `out->next_off > off`.
 * @post On error @p out is left as the caller zeroed it.
 * @note Not thread-safe.
 * @since Version 0.1.0
 */
static ra_err_t
s_rar4_block(const ra_rar_t* rar, uint64_t off, char* nbuf, uint16_t ncap, ra_rar_entry_t* out)
{
  uint8_t      scratch[k_ra_rar_hdr_scratch] = {};
 ...
 * @since Version 0.1.0
 */
static bool s_rar5_file(const ra_rar_t* rar,
                        uint64_t        off,
                        const uint8_t*  scratch,
                        size_t          n,
                        size_t          pos,
                        ...
                        ra_rar_entry_t* out)
{
  ...
  out->method =
    (method == 0U) ? (uint8_t)k_ra_rar_method_store : (uint8_t)k_ra_rar_method_compressed;
  out->unp_size  = unp;
  out->pack_size = dsize;
 ...
 */
static ra_err_t
s_rar5_block(const ra_rar_t* rar, uint64_t off, char* nbuf, uint16_t ncap, ra_rar_entry_t* out)
{
  uint8_t      scratch[k_ra_rar_hdr_scratch] = {};
  const size_t n                             = rar->read(rar->ctx, off, scratch, sizeof(scratch));
...
ra_err_t ra_rar_open(ra_rar_t* rar, ra_rar_read_fn read, void* ctx, uint64_t size)
{
  ...
  RA_CHECK_NULL_PTR((const void*)read, s_tag_rar, "open: null read");
  *rar = (ra_rar_t){};
...
ra_err_t ra_rar_next(const ra_rar_t* rar,
                     uint64_t        off,
                     char*           name_buf,
                     uint16_t        name_cap,
                     ra_rar_entry_t* out)
{
  RA_CHECK_NULL_PTR(rar, s_tag_rar, "next: null rar");
  RA_CHECK_NULL_PTR(out, s_tag_rar, "next: null out");
  *out = (ra_rar_entry_t){};
  if (rar->version == k_ra_rar_ver_none) {
