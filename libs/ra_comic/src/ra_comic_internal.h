 ...
 *
 * @details
 * `ra_comic.c` owns the public API, the shared page-index storage, the entry-name
 * image filter, and the name-sort. Each container backend -- `ra_comic_cbz.c`
 * (miniz ZIP) and `ra_comic_cbr.c` (RAR via `ra_rar.h`) -- enumerates its image
 * members by calling ::ra_comic_page_add and extracts one page through its own
 * `*_extract`. This header is the internal contract between them; it is not part
 * of the public `ra_comic.h` surface.
 *
 ...
 *          ::ra_comic_page_t slot with the backend bookkeeping. Rejects an index
 ...
 * @retval k_ra_ok               Page appended.
 * @retval k_ra_err_invalid_size Page index or name arena is full.
 * @pre @p c has its `pages` / `names` buffers bound.
 * @pre @p name addresses @p name_len readable bytes.
 * @post On k_ra_ok, `c->page_count` and `c->names_len` advanced by one entry.
 ...
 *          ::ra_comic_page_add. On firmware, miniz allocations route through the
 *          `ra_epub_miniz_alloc` static pool (no heap).
 * @param[in,out] c Comic with backing + buffers bound, `kind == cbz`.
 * @return ra_err_t status.
 * @retval k_ra_ok                Central directory walked; index populated.
 * @retval k_ra_err_validation_failed miniz could not open the ZIP.
 * @retval k_ra_err_invalid_size  The page index / name arena was too small.
 * @pre `c->read` / `c->size` / `c->pages` / `c->names` are bound.
 * @pre `c->stream` is available for the miniz I/O opaque pointer.
 * @post On k_ra_ok, `c->zip_active == 1` and the index holds every image entry.
 * @post On any error the miniz reader is torn down.
 * @note Not thread-safe.
 * @since Version 0.1.0
 */
ra_err_t ra_comic_cbz_open(ra_comic_t* c);
 ...
 * @retval k_ra_err_no_mem       @p cap is smaller than `p->raw_size`.
 * @retval k_ra_err_validation_failed miniz could not extract the member.
 * @pre `c->zip_active == 1`; @p p came from this comic's index.
 * @pre @p buf holds @p cap writable bytes.
 * @post On k_ra_ok, `*got == p->raw_size`.
 * @post On any error `*got == 0`.
 * @note Not thread-safe.
 * @since Version 0.1.0
 */
ra_err_t ra_comic_cbz_extract(ra_comic_t*            c,
                              const ra_comic_page_t* p,
                              uint8_t*               buf,
                              size_t                 cap,
                              size_t*                got);
 ...
 * @return ra_err_t status.
 * @retval k_ra_ok Always; a non-active reader is a no-op.
 ...
 * @since Version 0.1.0
 */
ra_err_t ra_comic_cbz_close(ra_comic_t* c);

/**
 * @brief Open the CBR (RAR) backend: walk the archive + build the page index.
 * @details Iterates the RAR block chain (`ra_rar_next`) from `c->rar.first_off`,
 *          appending each image file member via ::ra_comic_page_add. A member
 *          packed with a RAR compressor is indexed with `extractable == 0`.
 * @param[in,out] c Comic with `c->rar` bound by `ra_rar_open`, `kind == cbr`.
 * @return ra_err_t status.
 * @retval k_ra_ok                Archive walked; index populated.
 * @retval k_ra_err_invalid_size  The page index / name arena was too small.
 * @retval k_ra_err_*             A RAR-walk error.
 * @pre `c->rar` was bound by `ra_rar_open`; buffers are bound.
 * @pre `c->size` is the archive length.
 * @post On k_ra_ok, the index holds every image member in archive order.
 ...
 * @param[out] got  Receives the bytes written.
 * @return ra_err_t status.
 * @retval k_ra_ok                Member copied into @p buf.
 * @retval k_ra_err_not_supported The page is a compressed RAR member.
 * @retval k_ra_err_no_mem        @p cap is smaller than `p->raw_size`.
 * @retval k_ra_err_invalid_size  The member overruns the archive / short read.
 ...
 * @since Version 0.1.0
 */
ra_err_t ra_comic_cbr_extract(ra_comic_t*            c,
                              const ra_comic_page_t* p,
                              uint8_t*               buf,
                              size_t                 cap,
                              size_t*                got);
