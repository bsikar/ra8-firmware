 ...
 *
 * @details
 * The container-agnostic half of `ra_comic.h`. ::ra_comic_open reads the leading
 ...
 * ::ra_comic_page_add; the index is then sorted by entry name so reading order is
 * the sorted names. ::ra_comic_page_read / ::ra_comic_page_info / ::ra_comic_close
 * dispatch on the detected kind.
 *
 * Every decision here is a single condition on purpose, so the facade carries no
 * MC/DC obligation of its own (mirroring ra_cbz_container.c).
 *
 * @since Version 0.1.0
 *
...
static const char* const s_tag_comic = "ra_comic";
 ...
 *          is delegated to ::ra_rar_open, which owns its own signature match.
 * @since Version 0.1.0
 */
 ...
 *          policy is applied separately by ::ra_comic_is_page_name.
 * @param[in] name Entry name bytes.
 * @param[in] len  Length of @p name.
...
bool ra_comic_is_page_name(const char* name, uint16_t len)
{
  if (name == nullptr) {
    return false;
  }
...
ra_err_t ra_comic_page_add(ra_comic_t* c,
                           const char* name,
                           uint16_t    name_len,
                           uint64_t    raw_size,
                           uint8_t     extractable,
                           ...
                           uint64_t    pack_size)
{
  RA_CHECK_NULL_PTR(c, s_tag_comic, "add: null c");
  RA_CHECK_NULL_PTR(name, s_tag_comic, "add: null name");
  ...
  ra_comic_page_t* p = &c->pages[c->page_count];
  *p                 = (ra_comic_page_t){};
 ...
 *          EOCD (0x05 0x06) pair; a RAR is detected separately via ::ra_rar_open.
...
/**
 * @brief Reject any NULL required argument to ::ra_comic_open.
 ...
 * @pre The caller forwards ::ra_comic_open's pointer arguments unchanged.
 * @pre No argument is dereferenced before this returns k_ra_ok.
 * @post On k_ra_ok each checked pointer is safe to dereference.
 * @post On any error the reason is logged against ::s_tag_comic.
 * @note Thread-safe: reads only its pointer arguments.
 * @since Version 0.1.0
 */
static ra_err_t s_open_reject_null(const ra_comic_t*      c,
                                   ra_comic_read_fn       read,
                                   const ra_comic_page_t* pages,
                                   const char*            names)
{
  RA_CHECK_NULL_PTR(c, s_tag_comic, "open: null c");
  RA_CHECK_NULL_PTR((const void*)read, s_tag_comic, "open: null read");
  RA_CHECK_NULL_PTR(pages, s_tag_comic, "open: null pages");
  RA_CHECK_NULL_PTR(names, s_tag_comic, "open: null names");
  return k_ra_ok;
}
 ...
 *          page index populated (unsorted). Split from ::ra_comic_open so the
 *          entry point stays within the function-size budget.
 * @param[in,out] c Comic with backing + buffers already bound.
 * @return ra_err_t status.
 * @retval k_ra_ok                A backend populated the index.
 * @retval k_ra_err_invalid_size  A short magic read.
 * @retval k_ra_err_not_supported Neither a ZIP nor a RAR archive.
 * @retval k_ra_err_*             A backend error.
 * @pre `c->read` / `c->size` / buffers are bound.
 * @pre @p c was zeroed then populated by the caller.
 * @post On k_ra_ok, `c->kind` is set and the index is filled (unsorted).
 * @post On any error `c->kind` reflects the detection outcome.
 * @note Not thread-safe.
 * @since Version 0.1.0
 */
static ra_err_t s_open_detect(ra_comic_t* c)
{
  uint8_t      sig[k_ra_comic_magic_len] = {};
  const size_t got                       = c->read(c->ctx, 0U, sig, sizeof(sig));
  if (got < (size_t)k_comic_sig_min) {
    return k_ra_err_invalid_size;
  }
  if (s_is_zip(sig)) {
    c->kind        = k_ra_comic_kind_cbz;
    c->stream.read = c->read;
    c->stream.ctx  = c->ctx;
    c->stream.size = c->size;
    return ra_comic_cbz_open(c);
  }
  const ra_err_t rerr = ra_rar_open(&c->rar, c->read, c->ctx, c->size);
  if (rerr != k_ra_ok) {
    return k_ra_err_not_supported;
  }
  c->kind = k_ra_comic_kind_cbr;
  return ra_comic_cbr_open(c);
}

ra_err_t ra_comic_open(ra_comic_t*      c,
                       ra_comic_read_fn read,
                       void*            ctx,
                       uint64_t         size,
                       ...
                       uint32_t         names_cap)
{
  const ra_err_t nerr = s_open_reject_null(c, read, pages, names);
  if (nerr != k_ra_ok) {
    return nerr;
  }
  *c = (ra_comic_t){};
  ...
  const ra_err_t err = s_open_detect(c);
  if (err != k_ra_ok) {
    (void)ra_comic_close(c);
    return err;
  }
  if (c->page_count == 0U) {
    (void)ra_comic_close(c);
...
ra_err_t ra_comic_page_info(const ra_comic_t* c,
                            uint32_t          page,
                            char*             name_buf,
                            uint16_t          name_cap,
...
{
  RA_CHECK_NULL_PTR(c, s_tag_comic, "info: null c");
  ...
  const ra_comic_page_t* p = &c->pages[page];
  if (name_buf != nullptr) {
...
ra_err_t ra_comic_page_read(ra_comic_t* c, uint32_t page, uint8_t* buf, size_t cap, size_t* got)
{
  RA_CHECK_NULL_PTR(c, s_tag_comic, "read: null c");
  RA_CHECK_NULL_PTR(buf, s_tag_comic, "read: null buf");
  RA_CHECK_NULL_PTR(got, s_tag_comic, "read: null got");
  *got = 0U;
  if (c->kind == k_ra_comic_kind_none) {
    return k_ra_err_invalid_state;
  }
  ...
  const ra_comic_page_t* p = &c->pages[page];
  if (c->kind == k_ra_comic_kind_cbz) {
    return ra_comic_cbz_extract(c, p, buf, cap, got);
  }
  return ra_comic_cbr_extract(c, p, buf, cap, got);
}

ra_err_t ra_comic_close(ra_comic_t* c)
{
  RA_CHECK_NULL_PTR(c, s_tag_comic, "close: null c");
  if (c->kind == k_ra_comic_kind_cbz) {
    (void)ra_comic_cbz_close(c);
  }
  c->kind       = k_ra_comic_kind_none;
  c->page_count = 0U;
  c->zip_active = 0U;
  return k_ra_ok;
}
