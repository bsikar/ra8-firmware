 ...
 * report ::k_ra_rar_method_compressed and are not decoded here -- see the module
 ...
 * The walker never materialises the archive. ::ra_rar_open reads only the
 * signature; ::ra_rar_next reads one block header at a time into a small internal
 * scratch (a fixed ::k_ra_rar_hdr_scratch bytes on the stack) plus the caller's
 * name buffer; ::ra_rar_extract_stored streams one member's data area through the
 ...
 *
 * @see ra_comic.h  The comic facade that turns this walk into a page list.
 ...
 * return), so the same `ra_fs` file / `ra_vmem` page-cache backing that streams a
 * `.epub` off storage drives a `.cbr` with no whole-file residency. A return
 * shorter than @p len is treated as end-of-file.
 *
 * @param[in]  ctx    Opaque backing context (::ra_rar_t::ctx).
 ...
 * @since Version 0.1.0
 */
typedef size_t (*ra_rar_read_fn)(void* ctx, uint64_t offset, void* buf, size_t len);

/**
 * @enum ra_rar_version_t
 * @brief Which RAR container generation an archive uses.
 * @details Set by ::ra_rar_open from the file signature; selects the block-header
 *          grammar ::ra_rar_next applies.
 * @since Version 0.1.0
 */
typedef enum : uint8_t {
  k_ra_rar_ver_none = 0U, /**< Not a recognised RAR archive.      */
  k_ra_rar_ver_4    = 4U, /**< RAR 1.5-4.x block format ("RAR4"). */
  k_ra_rar_ver_5    = 5U, /**< RAR 5.0 block format ("RAR5").     */
} ra_rar_version_t;

/**
 * @enum ra_rar_method_t
 ...
 */
typedef enum : uint8_t {
  k_ra_rar_method_store      = 0U, /**< Uncompressed: data area is the file bytes.  */
  k_ra_rar_method_compressed = 1U, /**< Packed with a RAR compressor (not decoded). */
} ra_rar_method_t;

/**
 * @enum ra_rar_limits_t
...
typedef enum : uint16_t {
  k_ra_rar_sig4_len    = 7U,   /**< "Rar!\x1A\x07\x00" marker length (RAR4).        */
  k_ra_rar_sig5_len    = 8U,   /**< "Rar!\x1A\x07\x01\x00" signature length (RAR5). */
  k_ra_rar_hdr_scratch = 128U, /**< Per-block header read window (fixed fields).    */
  k_ra_rar_vint_max    = 10U,  /**< Max bytes in one RAR5 vint (64-bit value).      */
} ra_rar_limits_t;
 ...
 *
 * @details Populated by ::ra_rar_open; treat every field as read-only afterwards.
 *          @p read addresses the container file bytes; @p first_off is the offset
 *          of the first block just past the signature, where ::ra_rar_next begins.
 *
 * @invariant `version != k_ra_rar_ver_none` and `first_off <= size`.
 ...
 */
typedef struct {
  ra_rar_read_fn   read;      /**< Byte reader over the container file.          */
  ...
  ra_rar_version_t version;   /**< Detected container generation.                */
} ra_rar_t;

/**
 * @struct ra_rar_entry_t
 * @brief One decoded RAR block: a file member, a directory, or a skipped block.
 *
 * @details Filled by ::ra_rar_next. @p next_off always advances strictly past the
...
typedef struct {
  ...
  uint8_t  method;    /**< @ref ra_rar_method_t (normalised; 0 = store).       */
 ...
 * @retval k_ra_ok                Recognised RAR archive; @p rar bound.
 * @retval k_ra_err_null_ptr      @p rar or @p read was NULL.
 * @retval k_ra_err_invalid_size  @p size is 0 or shorter than a signature.
 * @retval k_ra_err_not_supported The bytes are not a RAR4 or RAR5 signature.
 *
 * @pre @p read serves offsets `[0, size)` of the container file.
 * @pre @p rar is a writable ::ra_rar_t.
 * @post On k_ra_ok, `rar->version` is 4 or 5 and `rar->first_off <= size`.
 * @post On any error @p rar is left with `version == k_ra_rar_ver_none`.
 *
 * @note Not thread-safe.
 * @see ra_rar_next()
 * @since Version 0.1.0
 */
[[nodiscard]] ra_err_t ra_rar_open(ra_rar_t* rar, ra_rar_read_fn read, void* ctx, uint64_t size);
 ...
 *
 * @param[in]  rar      Archive bound by ::ra_rar_open (non-NULL).
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                Block decoded; @p out (and @p name_buf) filled.
 * @retval k_ra_err_null_ptr      @p rar or @p out was NULL.
 * @retval k_ra_err_invalid_state @p rar was never bound by ::ra_rar_open.
 * @retval k_ra_err_invalid_arg   @p off is at or past `rar->size`.
 * @retval k_ra_err_validation_failed A truncated / malformed / non-advancing block.
 *
 * @pre @p rar was populated by ::ra_rar_open.
 * @pre @p off is the start of a block header.
 * @post On k_ra_ok, `out->next_off > off` (the walk strictly advances).
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra_err_t ra_rar_next(const ra_rar_t* rar,
                                   uint64_t        off,
                                   char*           name_buf,
                                   uint16_t        name_cap,
                                   ra_rar_entry_t* out);
 ...
 *          with `ent->method != k_ra_rar_method_store` returns
 *          ::k_ra_err_not_supported (the RAR compressor is not implemented).
 *
 * @param[in]  rar Archive bound by ::ra_rar_open (non-NULL).
 * @param[in]  ent A file member from ::ra_rar_next (non-NULL, `is_file == 1`).
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                Member copied; `*got == ent->unp_size`.
 * @retval k_ra_err_null_ptr      A required pointer argument was NULL.
 * @retval k_ra_err_invalid_state @p rar was never bound by ::ra_rar_open.
 * @retval k_ra_err_not_supported @p ent is a directory or a compressed member.
 * @retval k_ra_err_no_mem        @p cap is smaller than `ent->unp_size`.
 * @retval k_ra_err_invalid_size  The member overruns the archive, or the reader
 *                                returned a short read.
 *
 * @pre @p rar was populated by ::ra_rar_open.
 * @pre @p ent came from ::ra_rar_next on the same @p rar.
 * @post On k_ra_ok, `buf[0..*got)` holds the member's literal bytes.
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra_err_t ra_rar_extract_stored(const ra_rar_t*       rar,
