...
/// Component tag on the library log lines, matching `s_tag` from C.
...
// inline. `dsb sy` / `isb sy` are the assembler spellings in the C
...
pub export fn ra8_mpu_is_enabled() callconv(.c) u8 {
    return if ((regs().CTRL & implementation.ctrl_enable) != 0) 1 else 0;
}
