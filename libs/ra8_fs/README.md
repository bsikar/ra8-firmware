...
| Verification | Held to the whole-tree per-file coverage ratchet with **no allowlist** (`scripts/checks/check_tree_coverage.py`; every `ra8_fs` unit is a MEASURED row in `.github/tree-coverage-baseline.txt`, and it has no row in `.github/mcdc-baseline.txt`), MC/DC vectors on its compound decisions, MISRA via `scripts/checks/misra_check.sh` (ratcheted in `.github/misra-baseline.txt`), clang-tidy, the ASCII / Doxygen / annotation gates |
