 ...
 * single case for its case-insensitive lookups. The Microsoft reference table
 * covers the whole Basic Multilingual Plane in run-length-compressed form; it
 * is the exact byte sequence `mkfs.exfat` and the Windows formatter emit, and
 * its rotate-add checksum is the well-known 0xE619D30D. Embedding it verbatim
 * (rather than the earlier a-z-only stub) makes a card the firmware formats
 * byte-for-byte compatible with a PC-formatted one and folds accented / Greek /
 * Cyrillic file names correctly -- so a book dragged onto the card from any
 * computer resolves regardless of case.
 *
 * This table is 5836 bytes (::k_exfat_fmt_upc_std_bytes), lives in its own
...
/**
 * @var s_exfat_upcase
 * @brief The canonical Microsoft exFAT up-case table (compressed, 5836 bytes).
 * @details Little-endian UTF-16 up-case mapping table in the exFAT run-length
 *          compressed encoding, identical to the one `mkfs.exfat` writes. Its
 *          rotate-add checksum is 0xE619D30D. Read-only; lives in flash.
 * @note Access only via ::priv_exfat_write_upcase.
 * @warning Do not edit by hand -- the bytes and their checksum are a matched
 *          pair the exFAT reader validates.
 * @since 0.1.0
 */
...
ra8_err_t priv_exfat_write_upcase(const ra8_fs_backend_t* backend,
                                  ...
                                  uint32_t*               out_csum)
{
  const uint32_t upc_secs = ((uint32_t)k_exfat_fmt_upc_std_bytes + bps - 1U) / bps;
  uint32_t       cs       = 0U;
  uint32_t       off      = 0U;
  for (uint32_t s = 0U; s < upc_secs; s++) {
    ...
    priv_byte_copy(g_fs_scratch, &s_exfat_upcase[off], n);
    cs                  = priv_exfat_csum32(cs, g_fs_scratch, n);
    const ra8_err_t err = backend->write_block(backend->ctx, abs_lba + s, 1U, g_fs_scratch);
    if (err != k_ra8_ok) {
      return err;
    }
    off += n;
