...
const Fake = struct {
    ...
    fn devRead(ctx: ?*anyopaque, lba: u32, count: u32, buf: ?[*]u8) callconv(.c) core.Err {
        ...
        const output = buf orelse return core.err_null_ptr;
        self.reads += 1;
        if (self.read_status != core.ok) return self.read_status;
        const off = @as(usize, lba) * block;
        @memcpy(output[0 .. count * block], self.medium[off .. off + count * block]);
        return core.ok;
    }

    fn devWrite(ctx: ?*anyopaque, lba: u32, count: u32, buf: ?[*]const u8) callconv(.c) core.Err {
        const self: *Fake = @ptrCast(@alignCast(ctx.?));
        const input = buf orelse return core.err_null_ptr;
        self.programs += 1;
        if (self.write_status != core.ok) return self.write_status;
        const off = @as(usize, lba) * block;
        @memcpy(self.medium[off .. off + count * block], input[0 .. count * block]);
        return core.ok;
    }
    ...
    fn devGetCaps(ctx: ?*const anyopaque, out: ?*core.Caps) callconv(.c) core.Err {
        ...
        const output = out orelse return core.err_null_ptr;
...
test "vtable: NULL buffers and caps output are rejected after ctx" {
    var rig: Rig = .{};
    try expectEqual(core.ok, rig.init());
    try expectEqual(core.ok, rig.bind());
    resetLog();

    // The old C callbacks check ctx, then the data pointer, before bounds.
    // Invalid ranges keep the NULL guard's precedence observable.
    try expectEqual(core.err_null_ptr, rig.bd.iface.?.read.?(rig.bd.ctx, 4, 1, null));
    try std.testing.expectEqualStrings("buf must not be nullptr", last_message);
    try expectEqual(core.err_null_ptr, rig.bd.iface.?.write.?(rig.bd.ctx, 4, 1, null));
    try std.testing.expectEqualStrings("buf must not be nullptr", last_message);
    try expectEqual(core.err_null_ptr, rig.bd.iface.?.get_caps.?(rig.bd.ctx, null));
    try std.testing.expectEqualStrings("out must not be nullptr", last_message);

    // When both inputs are NULL, the C guard order reports ctx first.
    try expectEqual(core.err_null_ptr, abi.ftl_iface.read.?(null, 4, 1, null));
    try std.testing.expectEqualStrings("ctx must not be nullptr", last_message);
    try expectEqual(core.err_null_ptr, abi.ftl_iface.write.?(null, 4, 1, null));
    try expectEqual(core.err_null_ptr, abi.ftl_iface.get_caps.?(null, null));
    try std.testing.expectEqualStrings("ctx must not be nullptr", last_message);
    try expectEqual(@as(usize, 6), log_count);
