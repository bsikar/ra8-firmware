...
fn devRead(ctx: ?*anyopaque, lba: u32, count: u32, buf: ?[*]u8) callconv(.c) Err {
    ...
    var i: u32 = 0;
    while (i < count) : (i += 1) {
        const e = readOne(ftl, lba + i, output + core.blockOffset(i));
        if (e != core.ok) {
            return e;
        }
    }
...
fn devWrite(ctx: ?*anyopaque, lba: u32, count: u32, buf: ?[*]const u8) callconv(.c) Err {
    ...
    var i: u32 = 0;
    while (i < count) : (i += 1) {
        const e = writeOne(ftl, lba + i, input + core.blockOffset(i));
        if (e != core.ok) {
            return e;
        }
    }
...
fn devGetCaps(ctx: ?*const anyopaque, out: ?*core.Caps) callconv(.c) Err {
    ...
    const ftl: *const Ftl = @ptrCast(@alignCast(raw_ctx));
    output.* = core.presentedCaps(ftl.logical_blocks, ftl.erase_value);
    return core.ok;
}
