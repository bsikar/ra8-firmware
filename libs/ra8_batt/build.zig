...
//!
//! Build graph for the Zig implementation of `ra8_batt`. CMake consumes the
//! installed static library through the unchanged `inc/ra8_batt.h` C ABI; the
//! `test` step covers the nag policy and the ABI membrane.
//!
...
pub fn build(b: *std.Build) void {
    ...
    const library_module = b.createModule(.{
        .root_source_file = b.path("src/ra8_batt_abi.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const test_step = b.step("test", "Run Zig ra8_batt tests");
