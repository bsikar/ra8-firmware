/**
 * @file ra_rabook_compile.h
 * @brief Zero-heap builder that emits a RABOOK1 blob (the #149 compiler back-end).
 *
 * @details
 * `ra_rabook_compile` is the serialization back-end of the on-device EPUB ->
 ...
 * emits and the on-device reader @ref ra_book parses. This file is ONLY the
 ...
 * image pools plus the output buffer) via @ref ra_rabook_buffers_t. Every
 * builder call appends into those arenas; an overflow latches a sticky failure
 * that @ref ra_rabook_finalize reports, so the caller checks once at the end.
 *
 * @par Byte layout (matches @ref ra_book.h and the desktop tool)
 * @code
 ...
 * @ref ra_book_validate expects.
 *
 * @note Not thread-safe: a builder context is single-owner, mutated in place.
 * @see ra_book.h  The format definition + the reader this emitter targets.
 *
 * @since Version 0.1.0
 *
...
/**
 * @struct ra_rabook_ctx_t
 * @brief Builder state: the arenas plus running counts and a sticky-fail flag.
 * @details Initialise with @ref ra_rabook_compile_init; treat the fields as
 *          private. `failed` latches on the first arena overflow so the caller
 *          can build optimistically and check once at @ref ra_rabook_finalize.
 * @invariant Each `*_count` / `*_size` never exceeds the matching `_cap`.
 * @since Version 0.1.0
 */
typedef struct {
  ra_rabook_buffers_t buf;               /**< Caller-provided arenas.             */
 ...
 * @return Error code.
 * @retval k_ra_ok           Context bound and ready.
 * @retval k_ra_err_null_ptr @p ctx or @p buf (or a member pointer) is NULL.
 ...
 * @details Writes a @ref k_ra_book_node_element node carrying @p name_off and
 ...
 *          nil; set them with @ref ra_rabook_link_child / @ref ra_rabook_link_sibling.
 * @param[in,out] ctx        Builder context (non-NULL, initialised).
 ...
 * @return The new node index, or @ref k_ra_book_nil on overflow / error.
 * @retval k_ra_book_nil @p ctx is NULL, the node/attr arena is full, or @p attrs
 ...
 * @details Writes a @ref k_ra_book_node_text node whose `text_off` is @p text_off;
 *          a text node has no attributes and no children. Sibling links default
 *          to nil; set them with @ref ra_rabook_link_sibling.
 * @param[in,out] ctx      Builder context (non-NULL, initialised).
 * @param[in]     text_off String-pool offset of the text run.
 * @return The new node index, or @ref k_ra_book_nil on overflow / error.
 * @retval k_ra_book_nil @p ctx is NULL or the node arena is full.
 * @pre @p ctx was initialised.
 ...
 *          parent with the first child; chain the rest via @ref ra_rabook_link_sibling.
 * @param[in,out] ctx    Builder context (non-NULL, initialised).
 * @param[in]     parent Node index of the parent element (valid, an element).
 * @param[in]     child  Node index of the first child (valid).
 * @return Error code.
 * @retval k_ra_ok              Link recorded.
 * @retval k_ra_err_null_ptr    @p ctx is NULL.
 * @retval k_ra_err_invalid_arg @p parent or @p child is out of range.
 * @pre @p ctx was initialised.
 ...
 * @retval k_ra_ok              Link recorded.
 * @retval k_ra_err_null_ptr    @p ctx is NULL.
 * @retval k_ra_err_invalid_arg @p node or @p sibling is out of range.
 * @pre @p ctx was initialised.
 ...
 * @return The new chapter index, or @ref k_ra_book_nil on overflow / error.
 * @retval k_ra_book_nil @p ctx is NULL or the chapter arena is full.
 * @pre @p ctx was initialised.
 ...
 * @param[in]     format    @ref ra_book_image_format_t (gray4 or svg).
 * @param[in]     data      Pixel / SVG bytes to copy (non-NULL iff @p data_size > 0).
 * @param[in]     data_size Byte length of @p data.
 * @return The new image index, or @ref k_ra_book_nil on overflow / error.
 * @retval k_ra_book_nil @p ctx is NULL, the image table or pool is full, or
 ...
 *          @p scope_chapter limits it to a single chapter, or @ref k_ra_book_nil
 ...
 * @param[in]     scope_chapter Chapter index it scopes to, or @ref k_ra_book_nil.
 * @return The new stylesheet index, or @ref k_ra_book_nil on overflow / error.
 * @retval k_ra_book_nil @p ctx is NULL or the stylesheet arena is full.
 * @pre @p ctx was initialised.
 ...
 */
uint32_t
ra_rabook_add_stylesheet(ra_rabook_ctx_t* ctx, uint32_t source_off, uint32_t scope_chapter);
 ...
 * @param[in]     cover_image_index Image index of the cover, or @ref k_ra_book_nil.
 * @return Error code.
 ...
 * header), fills the 100-byte @ref ra_book_header_t (magic, version, all offsets
 * / counts / sizes, metadata, `total_size`), and computes the body CRC-32 over
 * every byte after the header. The result passes @ref ra_book_validate.
 *
 ...
 * @retval k_ra_err_no_mem       A prior builder call overflowed an arena.
 * @retval k_ra_err_invalid_size The laid-out blob exceeds the output capacity.
 *
 ...
 * @post On k_ra_ok, `out_blob[0..*out_len)` is a valid RABOOK1 blob.
