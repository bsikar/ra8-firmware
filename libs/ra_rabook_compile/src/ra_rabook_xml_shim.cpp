 ...
 *          @ref ra_rabook_finalize surfaces the overflow.
 ...
 * @since Version 0.1.0
 */
uint16_t s_collect_attrs(const XMLElement* elem, ra_rabook_ctx_t* ctx, ra_book_attr_t* out)
{
  uint16_t            count = 0U;
  ...
  if (a != nullptr) {
    ...
    ra_log_error(s_tag, "element exceeds k_xhtml_max_attrs; refusing to truncate");
    ctx->failed = true;
  }
  return count;
}
...
/**
 * @brief Emit one XML node as a ra_rabook element / text node (or skip it).
 * @details Element nodes become @ref ra_rabook_add_element calls (tag + attrs);
 *          non-CDATA text nodes with non-empty content become
 *          @ref ra_rabook_add_text calls.  Comments, processing instructions,
 ...
 * @return The new ra_rabook node index, or @ref k_ra_book_nil when the node is
 *         skipped or the builder overflowed.
 * @retval k_ra_book_nil Node skipped (comment/PI/CDATA/empty) or builder full.
 ...
 * @since Version 0.1.0
 */
uint32_t s_emit_node(ra_rabook_ctx_t* ctx, const XMLNode* node)
{
  ...
  if (elem != nullptr) {
    ...
    return ra_rabook_add_element(ctx, ra_rabook_intern(ctx, elem->Name()), attrs, ac);
  }
 ...
 * @param[in]     parent_idx   ra_rabook index of the node's parent.
 * @param[in]     prev_sib_idx ra_rabook index of the preceding sibling, or nil.
 ...
 * @pre @p parent_idx / @p prev_sib_idx are valid ra_rabook indices or nil.
 * @post The returned count never exceeds @ref k_xhtml_max_stack.
 * @post At most one frame is written, at index @p top.
 * @note Not thread-safe.
 * @since Version 0.1.0
 */
uint16_t s_push_frame(ra_xhtml_frame_t* stack,
                      uint16_t          top,
                      const XMLNode*    node,
                      uint32_t          parent_idx,
 ...
 * @param[in]     chapter_root ra_rabook index of @p body's already-added node.
 ...
 * @since Version 0.1.0
 */
void s_walk_body_subtree(ra_rabook_ctx_t* ctx, const XMLElement* body, uint32_t chapter_root)
{
  ...
  while (top > 0U) {
    ...
    const uint32_t sib_prev = (new_idx != k_ra_book_nil) ? new_idx : frame.prev_sib_idx;
    ...
    if (elem != nullptr && new_idx != k_ra_book_nil) {
      top = s_push_frame(stack, top, elem->FirstChild(), new_idx, k_ra_book_nil);
    }
  }
}

/**
 * @brief Reject any NULL argument to @ref ra_rabook_xml_parse_chapter.
 ...
 * @post On k_ra_ok all four arguments are guaranteed non-NULL.
 * @note Not thread-safe.
 * @since Version 0.1.0
 */
ra_err_t s_check_parse_args(const uint8_t*         xhtml_bytes,
                            const ra_rabook_ctx_t* ctx,
...
ra_err_t ra_rabook_xml_parse_chapter(const uint8_t*   xhtml_bytes,
                                     ...
                                     const char*      chapter_title)
{
  const ra_err_t arg_err = s_check_parse_args(xhtml_bytes, ctx, chapter_href, chapter_title);
  if (arg_err != k_ra_ok) {
    return arg_err;
  }
  ...
  const uint32_t title_off   = ra_rabook_intern(ctx, chapter_title);
  const uint32_t href_off    = ra_rabook_intern(ctx, chapter_href);
  const uint32_t chapter_idx = ra_rabook_add_chapter(ctx, title_off, href_off, chapter_root);
