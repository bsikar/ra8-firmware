 ...
 *
 * @details
 * driver for the RA8D2 GPTP block. Every register access
...
static ra8_eth_gptp_event_fn_t s_gptp_fn;
static void*                   s_gptp_ctx;

ra8_err_t ra8_eth_gptp_init(void)
{
  /* HUM Ch 11.2.8 "MSTPCRC : Module Stop Control Register C" p 446 */
  const ra8_err_t mst_err = ra8_mstp_enable(k_ra8_mstp_eswm);
  RA8_RETURN_ON_ERROR(mst_err, s_tag, "gptp_init: mstp enable"); /* GCOVR_EXCL_BR_LINE */
...
ra8_err_t ra8_eth_gptp_get_status(uint32_t* out_mask)
{
  RA8_CHECK_NULL_PTR(out_mask, s_tag, "out_mask must not be nullptr");
  /* HUM Ch 35 "Ethernet Generic PTP Timer (GPTP)" p 1925 */
  *out_mask = ra8_gptp()->GPTP_STS;
  return k_ra8_ok;
}

ra8_err_t ra8_eth_gptp_clear_status(uint32_t mask)
{
  volatile r_gptp_regs_t* reg = ra8_gptp();
  /* HUM Ch 35 "Ethernet Generic PTP Timer (GPTP)" p 1925 */
  reg->GPTP_ICLR = mask;
...
ra8_err_t ra8_eth_gptp_attach_handler(ra8_eth_gptp_event_fn_t fn, void* ctx)
{
  s_gptp_fn  = fn;
  s_gptp_ctx = ctx;
  return k_ra8_ok;
...
void ra8_eth_gptp_dispatch(void)
{
  ...
  const ra8_eth_gptp_event_fn_t fn   = s_gptp_fn;
