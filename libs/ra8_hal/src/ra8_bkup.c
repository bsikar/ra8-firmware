...
[[nodiscard]] ra8_err_t ra8_bkup_clear_status(uint8_t mask)
{
  if ((mask & k_ra8_bkup_vbtbpsr_mask_vbporf) != 0U) {
    ...
    const uint8_t live  = *ra8_bkup_vbtbpsr();
    *ra8_bkup_vbtbpsr() = (uint8_t)(live & (uint8_t)~k_ra8_bkup_vbtbpsr_mask_vbporf);
  }
  const uint8_t adf_bits = (uint8_t)(mask & k_ra8_bkup_vbtadsr_mask_all);
  if (adf_bits != 0U) {
    /* HUM Ch 12.2.14 "VBTADSR : VBATT Tamper Detection Status Register", p 509 */
    const uint8_t live  = *ra8_bkup_vbtadsr();
    *ra8_bkup_vbtadsr() = (uint8_t)(live & (uint8_t)~adf_bits);
  }
  return k_ra8_ok;
}
...
[[nodiscard]] ra8_err_t ra8_bkup_set_input_enable(ra8_bkup_channel_t channel, bool enable)
{
  ...
  internal_rmw8(ra8_bkup_vbtictlr(), mask, enable);
  return k_ra8_ok;
}
...
[[nodiscard]] ra8_err_t ra8_bkup_set_voltage_monitor(bool enable)
{
  ...
  internal_rmw8(ra8_bkup_vbattmnselr(), k_ra8_bkup_vbattmnselr_mask_vbtmnsel, enable);
  return k_ra8_ok;
}
...
[[nodiscard]] ra8_err_t ra8_bkup_isr_handle(void)
{
  ...
  if (fired != 0U) {
    ...
    const uint8_t live  = *ra8_bkup_vbtadsr();
    *ra8_bkup_vbtadsr() = (uint8_t)(live & (uint8_t)~fired);
