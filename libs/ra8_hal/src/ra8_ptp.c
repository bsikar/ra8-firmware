 ...
 *
 * @details
 * Implementation peer to ``ra8_eth_gptp.c``. ``ra8_eth_gptp`` owns the
...
static const char* s_tag = "RA8_PTP";

/**
 * @enum ra8_ptp_state_t
 * @brief Internal lifecycle state.
 */
typedef enum : uint8_t {
  k_ra8_ptp_state_closed = 0U, /**< Driver not open. */
  k_ra8_ptp_state_open   = 1U, /**< Driver open.     */
} ra8_ptp_state_t;

/**
 * @enum ra8_ptp_mac_idx_t
 ...
 */
static ra8_ptp_state_t s_state = k_ra8_ptp_state_closed;

/**
 * @var s_role
 * @brief Currently selected port role.
 */
static ra8_ptp_role_t s_role = k_ra8_ptp_role_controller;

/**
 * @var s_msg_fn
 * @brief Registered message-arrival callback.
 */
static ra8_ptp_msg_fn_t s_msg_fn;
...
static inline uint32_t internal_pack_mac4(uint8_t b0, uint8_t b1, uint8_t b2, uint8_t b3)
{
  return ((uint32_t)b0 << k_ra8_ptp_shift_b0) | ((uint32_t)b1 << k_ra8_ptp_shift_b1) |
         ((uint32_t)b2 << k_ra8_ptp_shift_b2) | ((uint32_t)b3 << k_ra8_ptp_shift_b3);
}

ra8_err_t ra8_ptp_open(const ra8_ptp_cfg_t* cfg)
{
  RA8_CHECK_NULL_PTR(cfg, s_tag, "cfg must not be nullptr");
  if ((uint16_t)cfg->domain > (uint16_t)k_ra8_ptp_domain_user_max) {
    return k_ra8_err_invalid_arg;
  }

  /* HUM Ch 11.2.8 "MSTPCRC : Module Stop Control Register C" p 446 */
  const ra8_err_t mst_err = ra8_mstp_enable(k_ra8_mstp_eswm);
  RA8_RETURN_ON_ERROR(mst_err, s_tag, "ptp_open: mstp enable"); /* GCOVR_EXCL_BR_LINE */

  volatile r_ptp_regs_t* reg = ra8_ptp_regs();
  ...
  reg->PTP_MAC0 = internal_pack_mac4(cfg->mac_addr[k_ra8_ptp_mac_idx_0],
                                     cfg->mac_addr[k_ra8_ptp_mac_idx_1],
                                     cfg->mac_addr[k_ra8_ptp_mac_idx_2],
                                     cfg->mac_addr[k_ra8_ptp_mac_idx_3]);
  reg->PTP_MAC1 = internal_pack_mac4(cfg->mac_addr[k_ra8_ptp_mac_idx_4],
                                     cfg->mac_addr[k_ra8_ptp_mac_idx_5],
                                     0U,
                                     0U);

  /* Default to controller role (IEEE 1588-2019 sec 9.2.5). */
  reg->PTP_CTRL = k_ra8_ptp_mask_port_en;
  s_role        = k_ra8_ptp_role_controller;
  s_state       = k_ra8_ptp_state_open;
  ra8_log_info(s_tag, "ptp_open");
  return k_ra8_ok;
}

ra8_err_t ra8_ptp_close(void)
{
  ...
  /* HUM Ch 35 "Ethernet Generic PTP Timer (GPTP)" p 1925 */
  reg->PTP_CTRL = 0UL;
  s_state       = k_ra8_ptp_state_closed;
  s_msg_fn      = nullptr;
  s_msg_ctx     = nullptr;
  return ra8_mstp_disable(k_ra8_mstp_eswm);
}

ra8_err_t ra8_ptp_set_role(ra8_ptp_role_t role)
{
  if (s_state != k_ra8_ptp_state_open) {
    return k_ra8_err_invalid_state;
  }
  if (role > k_ra8_ptp_role_transparent_clock) {
    return k_ra8_err_invalid_arg;
  }
  volatile r_ptp_regs_t* reg = ra8_ptp_regs();
  /* IEEE 1588-2019 sec 9.2.5 "portState" -- ROLE[1:0] field of PTP_CTRL. */
  uint32_t ctrl = reg->PTP_CTRL;
  ctrl &= ~k_ra8_ptp_mask_role;
  ctrl |= (uint32_t)role << k_ra8_ptp_role_shift;
  reg->PTP_CTRL = ctrl;
  s_role        = role;
  return k_ra8_ok;
}
...
static ra8_err_t internal_send(uint32_t trig_mask)
{
  ...
  if ((s_role != k_ra8_ptp_role_controller) && (s_role != k_ra8_ptp_role_boundary_clock)) {
    return k_ra8_err_invalid_state;
  }
  volatile r_ptp_regs_t* reg = ra8_ptp_regs();
...
ra8_err_t ra8_ptp_send_sync(void)
{
  /* IEEE 1588-2019 sec 13.6 "Sync and Delay_Req message format". */
  return internal_send(k_ra8_ptp_mask_tx_sync);
}

ra8_err_t ra8_ptp_send_announce(void)
{
  /* IEEE 1588-2019 sec 13.5 "Announce message format". */
  return internal_send(k_ra8_ptp_mask_tx_annc);
}

ra8_err_t ra8_ptp_get_time(uint64_t* sec, uint32_t* nsec)
{
  RA8_CHECK_NULL_PTR(sec, s_tag, "sec must not be nullptr");
  RA8_CHECK_NULL_PTR(nsec, s_tag, "nsec must not be nullptr");
  if (s_state != k_ra8_ptp_state_open) {
    return k_ra8_err_invalid_state;
  }
  volatile r_ptp_regs_t* reg = ra8_ptp_regs();
  ...
  *sec              = (hi << (uint64_t)k_ra8_ptp_shift_b3) | lo;
  *nsec             = reg->PTP_TIME_NS;
  return k_ra8_ok;
}

ra8_err_t ra8_ptp_set_time(uint64_t sec, uint32_t nsec)
{
  if (s_state != k_ra8_ptp_state_open) {
    return k_ra8_err_invalid_state;
  }
  if (nsec >= k_ra8_ptp_ns_per_sec) {
    return k_ra8_err_invalid_arg;
  }
  volatile r_ptp_regs_t* reg = ra8_ptp_regs();
  /* HUM Ch 35 "Ethernet Generic PTP Timer (GPTP)" p 1925 */
  reg->PTP_TIME_SECH = (uint32_t)(sec >> (uint64_t)k_ra8_ptp_shift_b3);
  reg->PTP_TIME_SECL = (uint32_t)sec;
  reg->PTP_TIME_NS   = nsec;
  return k_ra8_ok;
}

ra8_err_t ra8_ptp_adjust_time(int32_t delta_ns)
{
  if (s_state != k_ra8_ptp_state_open) {
    return k_ra8_err_invalid_state;
  }
  ...
  if (cur_ns >= 0) {
    ...
    int64_t carry_s = 0;
    while (new_ns >= (int64_t)k_ra8_ptp_ns_per_sec) {
      new_ns -= (int64_t)k_ra8_ptp_ns_per_sec;
      carry_s += 1;
    }
    reg->PTP_TIME_NS   = (uint32_t)new_ns;
    reg->PTP_TIME_SECL = reg->PTP_TIME_SECL + (uint32_t)carry_s;
  } else {
    int64_t new_ns     = cur_ns + (int64_t)k_ra8_ptp_ns_per_sec;
    reg->PTP_TIME_NS   = (uint32_t)new_ns;
...
ra8_err_t ra8_ptp_adjust_rate(int32_t ppb)
{
  if (s_state != k_ra8_ptp_state_open) {
    return k_ra8_err_invalid_state;
  }
...
ra8_err_t ra8_ptp_attach_message_handler(ra8_ptp_msg_fn_t fn, void* ctx)
{
  s_msg_fn  = fn;
  s_msg_ctx = ctx;
  return k_ra8_ok;
...
ra8_err_t ra8_ptp_get_offset(int32_t* offset_ns)
{
  RA8_CHECK_NULL_PTR(offset_ns, s_tag, "offset_ns must not be nullptr");
  if (s_state != k_ra8_ptp_state_open) {
    return k_ra8_err_invalid_state;
  }
  /* HUM Ch 35 "Ethernet Generic PTP Timer (GPTP)" p 1925 */
  *offset_ns = ra8_ptp_regs()->PTP_OFFS_NS;
  return k_ra8_ok;
}
...
void ra8_ptp_dispatch_message(ra8_ptp_msg_type_t type, uint64_t sec, uint32_t nsec)
{
  ...
  ra8_ptp_regs()->PTP_RX_LATCH = (uint32_t)type;
  if (fn != nullptr) {
