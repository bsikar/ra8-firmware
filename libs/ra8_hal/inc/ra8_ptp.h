 ...
 * existing ``ra8_eth_gptp`` scaffold (which only owns the GPTP timer
 * lifecycle): ``ra8_ptp`` adds the SYNFP / STCA layer that turns the
...
/**
 * @enum ra8_ptp_msg_type_t
 * @brief IEEE 1588-2019 sec 13.3.2.3 messageType values.
 */
typedef enum : uint8_t {
  k_ra8_ptp_msg_sync                  = 0x00U, /**< Sync.                  */
  k_ra8_ptp_msg_delay_req             = 0x01U, /**< Delay_Req.             */
  k_ra8_ptp_msg_pdelay_req            = 0x02U, /**< Pdelay_Req.            */
  k_ra8_ptp_msg_pdelay_resp           = 0x03U, /**< Pdelay_Resp.           */
  k_ra8_ptp_msg_follow_up             = 0x08U, /**< Follow_Up.             */
  k_ra8_ptp_msg_delay_resp            = 0x09U, /**< Delay_Resp.            */
  k_ra8_ptp_msg_pdelay_resp_follow_up = 0x0AU, /**< Pdelay_Resp_Follow_Up. */
  k_ra8_ptp_msg_announce              = 0x0BU, /**< Announce.              */
  k_ra8_ptp_msg_signaling             = 0x0CU, /**< Signaling.             */
  k_ra8_ptp_msg_management            = 0x0DU, /**< Management.            */
} ra8_ptp_msg_type_t;
 ...
 */
typedef enum : uint8_t {
  k_ra8_ptp_role_controller        = 0U, /**< Ordinary or boundary controller.  */
  k_ra8_ptp_role_peripheral        = 1U, /**< Ordinary peripheral.              */
  k_ra8_ptp_role_boundary_clock    = 2U, /**< Boundary clock (multi-port).      */
  k_ra8_ptp_role_transparent_clock = 3U, /**< Transparent clock (forward only). */
} ra8_ptp_role_t;

/**
 * @enum ra8_ptp_clock_class_t
 * @brief IEEE 1588-2019 sec 7.6.2.5 clockClass values used in BMCA.
 */
typedef enum : uint8_t {
  k_ra8_ptp_clock_class_primary_ref     = 6U,   /**< Primary reference (GPS).              */
  k_ra8_ptp_clock_class_app_specific    = 13U,  /**< Application-specific controller.      */
  k_ra8_ptp_clock_class_holdover        = 52U,  /**< Holdover (degraded).                  */
  k_ra8_ptp_clock_class_default         = 248U, /**< Default for ordinary clocks.          */
  k_ra8_ptp_clock_class_peripheral_only = 255U, /**< Peripheral-only (never a controller). */
} ra8_ptp_clock_class_t;

/**
 * @enum ra8_ptp_sync_interval_t
 * @brief log2 of sync interval in seconds (IEEE 1588-2019 sec 7.7.2.3).
 */
typedef enum : int8_t {
  k_ra8_ptp_sync_int_1_128 = -7, /**< 1/128 s. */
  k_ra8_ptp_sync_int_1_64  = -6, /**< 1/64 s.  */
  k_ra8_ptp_sync_int_1_32  = -5, /**< 1/32 s.  */
  k_ra8_ptp_sync_int_1_16  = -4, /**< 1/16 s.  */
  k_ra8_ptp_sync_int_1_8   = -3, /**< 1/8 s.   */
  k_ra8_ptp_sync_int_1_4   = -2, /**< 1/4 s.   */
  k_ra8_ptp_sync_int_1_2   = -1, /**< 1/2 s.   */
  k_ra8_ptp_sync_int_1     = 0,  /**< 1 s.     */
  k_ra8_ptp_sync_int_2     = 1,  /**< 2 s.     */
  k_ra8_ptp_sync_int_4     = 2,  /**< 4 s.     */
  k_ra8_ptp_sync_int_8     = 3,  /**< 8 s.     */
} ra8_ptp_sync_interval_t;

/**
 * @enum ra8_ptp_domain_t
 * @brief IEEE 1588-2019 sec 7.1 named clock domains.
 */
typedef enum : uint8_t {
  k_ra8_ptp_domain_default  = 0U,   /**< Default PTP domain.        */
  k_ra8_ptp_domain_alt1     = 1U,   /**< Alternate domain 1.        */
  k_ra8_ptp_domain_alt2     = 2U,   /**< Alternate domain 2.        */
  k_ra8_ptp_domain_alt3     = 3U,   /**< Alternate domain 3.        */
  k_ra8_ptp_domain_user_min = 4U,   /**< First user-defined domain. */
  k_ra8_ptp_domain_user_max = 127U, /**< Last user-defined domain.  */
} ra8_ptp_domain_t;

/**
 * @enum ra8_ptp_limits_t
 * @brief Argument limits for PTP setters.
 */
typedef enum : uint32_t {
  k_ra8_ptp_mac_byte_count = 6UL,          /**< IEEE 802 MAC address length. */
  k_ra8_ptp_ns_per_sec     = 1000000000UL, /**< Nanoseconds in one second.   */
} ra8_ptp_limits_t;

/**
 * @struct ra8_ptp_cfg_t
 * @brief Static configuration for ``ra8_ptp_open``.
 *
 * @details
 ...
 *                    ``ra8_ptp_attach_message_handler``.
 ...
 */
typedef void (*ra8_ptp_msg_fn_t)(void* ctx, ra8_ptp_msg_type_t type, uint64_t sec, uint32_t nsec);
 ...
 * port role defaults to controller; call ``ra8_ptp_set_role`` to change it.
 *
 * @param[in] cfg Static configuration. Must not be ``nullptr``.
 *
 * @retval k_ra8_ok                 Driver opened.
 * @retval k_ra8_err_null_ptr       ``cfg == nullptr``.
 * @retval k_ra8_err_invalid_arg    Domain out of range.
 * @retval k_ra8_err_hw_init_failed MSTP enable failed.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ptp_open(const ra8_ptp_cfg_t* cfg);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ptp_close(void);
 ...
 *
 * @retval k_ra8_ok                Role applied.
 * @retval k_ra8_err_invalid_state Driver not open.
 * @retval k_ra8_err_invalid_arg   Role not in ``ra8_ptp_role_t``.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ptp_set_role(ra8_ptp_role_t role);

/**
 * @brief Emit a Sync message tagged with the current PTP time.
 *
 * @retval k_ra8_ok                Sync queued.
 * @retval k_ra8_err_invalid_state Driver not open or not a controller.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ptp_send_sync(void);

/**
 * @brief Emit an Announce message advertising the local clock.
 *
 * @retval k_ra8_ok                Announce queued.
 * @retval k_ra8_err_invalid_state Driver not open or not a controller.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ptp_send_announce(void);
 ...
 *
 * @retval k_ra8_ok            Time copied.
 * @retval k_ra8_err_null_ptr  Either output pointer is ``nullptr``.
 * @retval k_ra8_err_invalid_state Driver not open.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ptp_get_time(uint64_t* sec, uint32_t* nsec);
 ...
 *
 * @retval k_ra8_ok              Time loaded.
 * @retval k_ra8_err_invalid_arg ``nsec >= 1e9``.
 * @retval k_ra8_err_invalid_state Driver not open.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ptp_set_time(uint64_t sec, uint32_t nsec);
 ...
 *
 * @retval k_ra8_ok                Offset applied.
 * @retval k_ra8_err_invalid_state Driver not open.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ptp_adjust_time(int32_t delta_ns);
 ...
 *
 * @retval k_ra8_ok                Rate applied.
 * @retval k_ra8_err_invalid_state Driver not open.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ptp_adjust_rate(int32_t ppb);
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ptp_attach_message_handler(ra8_ptp_msg_fn_t fn, void* ctx);
 ...
 *
 * @retval k_ra8_ok            Offset copied.
 * @retval k_ra8_err_null_ptr  ``offset_ns == nullptr``.
 * @retval k_ra8_err_invalid_state Driver not open.
 *
 * @since 0.1.0
 */
[[nodiscard]] ra8_err_t ra8_ptp_get_offset(int32_t* offset_ns);
