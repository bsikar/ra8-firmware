 ...
 *
 * @details
 * `ra_cache_store` is the persistent-cache tier from the multi-GB book-streaming
 ...
 * `ra_fs` mount. It is the on-flash backing for a compiled `.rabook` so the
 ...
 * The bind to a physical flash is the injected @ref ra_cache_store_nor_init_fn
 * driver-initialise callback -- `lx_nor_driver_ra_xspi_initialize` on the
 ...
 * ::ra_cache_store_t handle, the ::ra_cache_store_entry_t index array, and a
 * one-sector (>= @ref k_ra_cache_store_sector_bytes) staging buffer. The module
 * allocates nothing.
 *
 * ## Streaming a hit (feeds ra_vsource)
 * ::ra_cache_store_get fills a ::ra_cache_store_reader_t and the caller wires
 * ::ra_cache_store_read (an `ra_vsource_read_fn`-shaped `read(ctx,off,buf,len)`)
 * into `ra_vsource_add_paged`, so a cached `.rabook` demand-pages through the
 * #204/#205 paged path -- full residency is never required.
 *
 * ## Write-once invariant
 * An entry is sealed by a single ::ra_cache_store_put; there is deliberately no
 ...
 * @see ra_cache_store_put   Seal a new entry.
 * @see ra_cache_store_get   Open an entry for streaming reads.
 * @see ra_cache_store_evict Drop an entry and reclaim its sectors.
 * @since 0.1.0
 *
 * @par Tag
...
/**
 * @typedef ra_cache_store_nor_init_fn
 ...
 *          Production binds `lx_nor_driver_ra_xspi_initialize`; host tests bind
 ...
 * @since 0.1.0
 */
typedef unsigned int (*ra_cache_store_nor_init_fn)(struct LX_NOR_FLASH_STRUCT* nor_flash);

/**
 * @enum ra_cache_store_geom_t
 * @brief Fixed geometry + tuning constants for the store.
 * @details `k_ra_cache_store_sector_bytes` is the LevelX logical-sector size the
 *          module reads/writes a sector at a time; the staging buffer and every
 *          media record are sized against it.
 * @since 0.1.0
 */
typedef enum : uint16_t {
  k_ra_cache_store_sector_bytes = 512U, /**< LevelX logical-sector size (bytes). */
  k_ra_cache_store_min_sectors  = 8U,   /**< Minimum usable logical-sector span. */
  k_ra_cache_store_overprov_pct = 20U,  /**< Default GC headroom margin (%).     */
  k_ra_cache_store_max_overprov = 90U,  /**< Reject nonsensical margins above.   */
} ra_cache_store_geom_t;

/**
 * @enum ra_cache_store_flag_t
 * @brief Per-entry index-slot flag bits.
 * @since 0.1.0
 */
typedef enum : uint8_t {
  k_ra_cache_store_flag_none   = 0U,      /**< Empty slot.                         */
  k_ra_cache_store_flag_in_use = 1U << 0, /**< Slot holds a live entry.            */
  k_ra_cache_store_flag_pinned = 1U << 1, /**< Never-evict (open book / metadata). */
} ra_cache_store_flag_t;

/**
 * @struct ra_cache_store_entry_t
 * @brief One RAM index slot: `key -> {header sector, run length, payload len}`.
 *
 * @details Caller-owned array element (see ::ra_cache_store_cfg_t::index).
 ...
 *          after it.
 *
 * @invariant `sector_count >= 1` when @ref k_ra_cache_store_flag_in_use is set.
 * @since 0.1.0
 */
...
typedef struct {
  ...
  uint8_t  flags;        /**< ::ra_cache_store_flag_t bit set.               */
  uint8_t  reserved;     /**< Padding; keeps the slot 16 bytes.              */
} ra_cache_store_entry_t;
/* cppcheck-suppress-end unusedStructMember */

/**
 * @struct ra_cache_store_t
 * @brief Caller-owned store handle. Treat every field as private.
 *
 * @details Zero-initialise (`= {}`) and pass to ::ra_cache_store_init. The
 *          handle out-lives every call and every ::ra_cache_store_reader_t
 *          derived from it.
 *
...
typedef struct {
  ...
  ra_cache_store_entry_t*     index;           /**< Caller-owned index slot array.          */
...
/**
 * @struct ra_cache_store_reader_t
 * @brief Handle to an open entry that streams through ::ra_cache_store_read.
 *
 * @details Filled by ::ra_cache_store_get. Pass its address as the `ctx` to
 *          `ra_vsource_add_paged` together with ::ra_cache_store_read and
...
/* cppcheck-suppress-begin unusedStructMember */
typedef struct {
  const ra_cache_store_t* store;        /**< Parent store (flash + staging).   */
...
} ra_cache_store_reader_t;
/* cppcheck-suppress-end unusedStructMember */

/**
 * @struct ra_cache_store_cfg_t
 * @brief Injected dependencies + geometry for ::ra_cache_store_init.
 *
...
typedef struct {
  ...
  ra_cache_store_nor_init_fn  nor_driver_init;   /**< Injected physical-flash bind.        */
  const char*                 name;              /**< LevelX partition name (may be NULL). */
  ra_cache_store_entry_t*     index;             /**< Caller-owned index array.            */
  uint8_t*                    staging;           /**< Caller-owned >=1-sector scratch.     */
 ...
 *
 * @return Error code.
 * @retval k_ra_ok                Store mounted (or formatted) and ready.
 * @retval k_ra_err_null_ptr      `store`, `cfg`, or a required `cfg` member NULL.
 * @retval k_ra_err_invalid_size  `staging_bytes` too small, `index_cap` zero, or
 *                                `logical_sectors` too small for the layout.
 * @retval k_ra_err_invalid_arg   `overprovision_pct` out of range.
 * @retval k_ra_err_hw_init_failed LevelX open/format failed.
 * @retval k_ra_err_invalid_state  On-flash checkpoint/superblock is corrupt.
 *
 * @pre `store`, `cfg`, and every non-optional `cfg` member are non-NULL.
 * @pre `cfg->nor_driver_init` brings a working LevelX NOR flash up.
 * @post On `k_ra_ok`, `store->inited` is true and the index reflects flash.
 * @post On any error the store is left uninitialised.
 *
 * @note Not thread-safe.
 * @see ra_cache_store_close
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_cache_store_init(ra_cache_store_t*           store,
                                           const ra_cache_store_cfg_t* cfg);
 ...
 *
 * @return Error code.
 * @retval k_ra_ok               Entry sealed and indexed.
 * @retval k_ra_err_null_ptr     `store` or `data` NULL.
 * @retval k_ra_err_invalid_size `len` is zero.
 * @retval k_ra_err_not_initialized Store not initialised.
 * @retval k_ra_err_exists       `key` already present (write-once).
 * @retval k_ra_err_no_mem       Index full or budget/free-run exhausted.
 * @retval k_ra_err_hw_init_failed LevelX sector write failed.
 *
 * @pre `store->inited` is true.
 * @pre `data` covers `len` bytes.
 * @post On `k_ra_ok` a subsequent ::ra_cache_store_get on `key` streams `data`.
 * @post On any error no partial entry is visible to ::ra_cache_store_get.
 *
 * @note Not thread-safe.
 * @see ra_cache_store_get
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t
ra_cache_store_put(ra_cache_store_t* store, uint32_t key, const uint8_t* data, uint32_t len);

/**
 * @brief Open a sealed entry for random reads through ::ra_cache_store_read.
 *
 * @details Looks `key` up in the index and fills @p out_reader so the caller can
 *          register it with `ra_vsource_add_paged(vs, ra_cache_store_read,
 ...
 *
 * @return Error code.
 * @retval k_ra_ok                 Entry found; `out_reader` populated.
 * @retval k_ra_err_null_ptr       `store` or `out_reader` NULL.
 * @retval k_ra_err_not_initialized Store not initialised.
 * @retval k_ra_err_not_found      `key` is not cached.
 *
 * @pre `store->inited` is true.
 * @pre `out_reader` is writable.
 * @post On `k_ra_ok`, `out_reader->byte_len` is the entry's payload length.
 * @post On any error `out_reader` is untouched.
 *
 * @note Not thread-safe.
 * @see ra_cache_store_read
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_cache_store_get(const ra_cache_store_t*  store,
                                          uint32_t                 key,
                                          ra_cache_store_reader_t* out_reader);

/**
 * @brief `ra_vsource_read_fn`-shaped random read over an open entry.
 *
 ...
 *          Signature matches `ra_vsource_read_fn` so it binds straight into
 *          `ra_vsource_add_paged`.
 *
 * @param[in]  ctx    A ::ra_cache_store_reader_t populated by ::ra_cache_store_get.
 ...
 * @retval k_ra_err_null_ptr      `ctx` or `buf` NULL.
 * @retval k_ra_err_out_of_range  `offset + len` exceeds `byte_len`.
 * @retval k_ra_err_hw_init_failed LevelX sector read failed.
 *
 * @pre `ctx` is a reader from ::ra_cache_store_get on a still-mounted store.
 * @pre `buf` covers `len` bytes.
 * @post On `k_ra_ok`, `buf[0..len)` holds the payload slice.
 * @post On any error `buf` content is unspecified.
 *
 * @note Not thread-safe; shares the store staging buffer.
 * @since 0.1.0
 */
ra_err_t ra_cache_store_read(void* ctx, uint64_t offset, uint8_t* buf, uint32_t len);
 ...
 *
 * @return Error code.
 * @retval k_ra_ok                 Entry dropped and reclaimed.
 * @retval k_ra_err_null_ptr       `store` NULL.
 * @retval k_ra_err_not_initialized Store not initialised.
 * @retval k_ra_err_not_found      `key` is not cached.
 * @retval k_ra_err_busy           Entry is pinned (unpin first).
 * @retval k_ra_err_hw_init_failed LevelX sector release failed.
 *
 * @pre `store->inited` is true.
 * @pre `key` is not pinned.
 * @post On `k_ra_ok` a later ::ra_cache_store_get on `key` returns not-found.
 * @post On `k_ra_ok` the freed sectors are available to a later put.
 *
 * @note Not thread-safe.
 * @see ra_cache_store_pin
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_cache_store_evict(ra_cache_store_t* store, uint32_t key);
 ...
 *          The pin state is persisted at the next ::ra_cache_store_sync /
 *          ::ra_cache_store_close.
 *
 * @param[in,out] store Initialised store.
 ...
 *
 * @return Error code.
 * @retval k_ra_ok                 Pin state updated.
 * @retval k_ra_err_null_ptr       `store` NULL.
 * @retval k_ra_err_not_initialized Store not initialised.
 * @retval k_ra_err_not_found      `key` is not cached.
 *
 * @pre `store->inited` is true.
 * @pre `key` is cached.
 * @post On `k_ra_ok` the slot's pinned flag matches @p pin.
 * @post ::ra_cache_store_evict on a pinned `key` returns `k_ra_err_busy`.
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_cache_store_pin(ra_cache_store_t* store, uint32_t key, bool pin);
 ...
 *          rewrites the superblock. Called internally by ::ra_cache_store_close;
 ...
 *
 * @return Error code.
 * @retval k_ra_ok                 Checkpoint written.
 * @retval k_ra_err_null_ptr       `store` NULL.
 * @retval k_ra_err_not_initialized Store not initialised.
 * @retval k_ra_err_hw_init_failed LevelX write failed.
 *
 * @pre `store->inited` is true.
 * @pre The index reflects the intended live set.
 * @post On `k_ra_ok` a clean remount can rebuild the index without a full scan.
 * @post The on-flash clean marker remains unset (dirty).
 *
 * @note Not thread-safe.
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_cache_store_sync(ra_cache_store_t* store);
 ...
 *          next ::ra_cache_store_init takes the fast (no-scan) mount path, then
 ...
 *
 * @return Error code.
 * @retval k_ra_ok                 Store checkpointed, marked clean, and closed.
 * @retval k_ra_err_null_ptr       `store` NULL.
 * @retval k_ra_err_not_initialized Store not initialised.
 * @retval k_ra_err_hw_init_failed LevelX write/close failed.
 *
 * @pre `store->inited` is true.
 * @pre No reader is mid-stream against this store.
 * @post On `k_ra_ok`, `store->inited` is false and the flash marker is clean.
 * @post A subsequent ::ra_cache_store_init loads the checkpoint directly.
 *
 * @note Not thread-safe.
 * @see ra_cache_store_init
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_cache_store_close(ra_cache_store_t* store);
