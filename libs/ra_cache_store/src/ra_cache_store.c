/**
 * @file ra_cache_store.c
 * @brief ra_cache_store runtime path: put / get / read / evict / pin / checkpoint
 *        (#201).
 *
 * @par Tag
...
/**
 * @enum ra_cs_rt_const_t
 * @brief Runtime sizing limits.
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_ra_cs_max_run = 0xFFFFU, /**< Max run length (fits ::ra_cs_entry_hdr_t sector_count). */
} ra_cs_rt_const_t;
 ...
 * @since 0.1.0
 */
static uint16_t cs_index_used(const ra_cache_store_t* store)
{
  if (store == nullptr) {
    return 0U;
  }
 ...
 * @since 0.1.0
 */
static bool cs_run_free(const ra_cache_store_t* store, uint32_t start, uint32_t count)
{
  if (store == nullptr) {
    return false;
  }
 ...
 * @return Error code.
 * @retval k_ra_ok               A free run was found.
 * @retval k_ra_err_null_ptr     `store` or `out_start` NULL.
 * @retval k_ra_err_invalid_size `count` is zero.
 * @retval k_ra_err_no_mem       No free run of that length exists.
 * @pre `store->log_start < store->logical_sectors`.
 * @pre @p out_start is writable.
 * @post On `k_ra_ok`, `[*out_start, *out_start+count)` is free.
 * @post On error @p out_start is untouched.
 * @note Not thread-safe; the store serialises access.
 * @since 0.1.0
 */
static ra_err_t cs_alloc_run(const ra_cache_store_t* store, uint32_t count, uint32_t* out_start)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(out_start, s_tag, "out_start");
  if (count == 0U) {
...
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(data, s_tag, "data");
  uint16_t       data_sectors = (uint16_t)(count - 1U);
  uint32_t       off          = 0U;
  const uint32_t sec_sz       = (uint32_t)k_ra_cache_store_sector_bytes;
  for (uint16_t i = 0U; i < data_sectors; i++) {
    ...
    RA_RETURN_ON_ERROR(ra_cs_sector_write(store, start + 1U + (uint32_t)i, store->staging),
                       s_tag,
                       "payload");
    off += chunk;
  }
  ra_cs_entry_hdr_t h = {.magic        = (uint32_t)k_ra_cs_entry_magic,
                         ...
                         .flags        = 0U,
                         .hdr_crc      = 0U};
  h.hdr_crc           = ra_cs_crc32((const uint8_t*)&h, (uint32_t)(sizeof(h) - sizeof(h.hdr_crc)));
  (void)memset(store->staging, 0, (size_t)sec_sz);
  (void)memcpy(store->staging, &h, sizeof(h));
  return ra_cs_sector_write(store, start, store->staging);
}
 ...
 * @return Error code.
 * @retval k_ra_ok                Sector 0 now reflects a dirty session.
 ...
 * @pre `store->staging` covers one sector.
 * @post On `k_ra_ok`, `store->flash_state == dirty`.
 * @post A later mount will replay the log rather than trust a stale checkpoint.
 * @note Not thread-safe; the store serialises access.
 * @since 0.1.0
 */
static ra_err_t cs_mark_dirty(ra_cache_store_t* store)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_VALIDATE_INIT(store->inited, s_tag, "store");
  if (store->flash_state == (uint8_t)k_ra_cs_dirty) {
    return k_ra_ok;
  }
  RA_RETURN_ON_ERROR(ra_cs_super_write(store, (uint32_t)k_ra_cs_dirty), s_tag, "mark dirty");
 ...
 * @return Error code.
 * @retval k_ra_ok                Checkpoint committed clean.
 * @retval k_ra_err_null_ptr      `store` NULL.
 * @retval k_ra_err_not_initialized Store not initialised.
 * @retval k_ra_err_hw_init_failed LevelX write failed.
 * @pre `store->inited` is true.
 * @pre The index reflects the intended live set.
 * @post On `k_ra_ok`, `store->flash_state == clean` and the directory is current.
 * @post The dirty marker precedes the directory rewrite (crash-safe ordering).
 * @note Not thread-safe; the store serialises access.
 * @since 0.1.0
 */
static ra_err_t cs_checkpoint(ra_cache_store_t* store)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_VALIDATE_INIT(store->inited, s_tag, "store");
  RA_RETURN_ON_ERROR(cs_mark_dirty(store), s_tag, "pre-dirty");
  uint32_t count = 0U;
  RA_RETURN_ON_ERROR(ra_cs_dir_save(store, &count), s_tag, "dir save");
  RA_RETURN_ON_ERROR(ra_cs_super_write(store, (uint32_t)k_ra_cs_clean), s_tag, "clean super");
 ...
 * @return Error code.
 * @retval k_ra_ok                 Request is valid; `*out_count` set.
 * @retval k_ra_err_null_ptr       `store` or `data` NULL.
 * @retval k_ra_err_not_initialized Store not initialised.
 * @retval k_ra_err_invalid_size   `len` zero or run longer than the header field.
 * @retval k_ra_err_exists         `key` already present (write-once).
 * @retval k_ra_err_no_mem         Index full or budget exhausted.
 * @pre `store->inited` is true.
 * @pre @p out_count is writable.
 * @post On `k_ra_ok`, `*out_count >= 2` (header + at least one payload sector).
 * @post No store state is modified.
 * @note Not thread-safe; the store serialises access.
 * @since 0.1.0
 */
static ra_err_t cs_put_check(ra_cache_store_t* store,
                             uint32_t          key,
                             const uint8_t*    data,
                             uint32_t          len,
                             uint32_t*         out_count)
{
  ...
  if (ra_cs_index_find(store, key) >= 0) {
  ...
  if (count > (uint32_t)k_ra_cs_max_run) {
    return k_ra_err_invalid_size;
  }
 ...
 * @retval k_ra_err_null_ptr       `store` or `dst` NULL.
 * @retval k_ra_err_hw_init_failed LevelX read failed.
 * @pre @p max >= 1 and @p dst covers @p max bytes.
 * @pre `store->staging` covers one sector.
 * @post On `k_ra_ok`, `1 <= *out_copied <= max`.
 * @post On error @p dst content is unspecified.
 * @note Not thread-safe; shares the store staging buffer.
 * @since 0.1.0
 */
static ra_err_t cs_read_at(const ra_cache_store_t* store,
                           uint32_t                data_start,
                           uint64_t                byte_pos,
                           uint8_t*                dst,
                           uint32_t                max,
                           uint32_t*               out_copied)
{
  ...
  RA_RETURN_ON_ERROR(ra_cs_sector_read(store, sector, store->staging), s_tag, "read");
 ...
 * @details Loops `[start, start+count)` through ::ra_cs_sector_release so eviction
 ...
 * @return Error code.
 * @retval k_ra_ok                 Run released.
 * @retval k_ra_err_null_ptr       `store` NULL.
 * @retval k_ra_err_not_initialized Store not initialised.
 * @retval k_ra_err_hw_init_failed LevelX release failed.
 * @pre `store->inited` is true.
 * @pre `[start, start+count)` is the entry's run.
 * @post On `k_ra_ok` every sector of the run is free.
 * @post On error some sectors may remain mapped (caller retries).
 * @note Not thread-safe; the store serialises access.
 * @since 0.1.0
 */
static ra_err_t cs_release_run(ra_cache_store_t* store, uint32_t start, uint16_t count)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_VALIDATE_INIT(store->inited, s_tag, "store");
  for (uint16_t i = 0U; i < count; i++) {
    RA_RETURN_ON_ERROR(ra_cs_sector_release(store, start + (uint32_t)i), s_tag, "release");
  }
  return k_ra_ok;
}
...
{
  uint32_t count = 0U;
  RA_RETURN_ON_ERROR(cs_put_check(store, key, data, len, &count), s_tag, "check");
  RA_RETURN_ON_ERROR(cs_mark_dirty(store), s_tag, "dirty");
  uint32_t start = 0U;
  RA_RETURN_ON_ERROR(cs_alloc_run(store, count, &start), s_tag, "alloc");
  RA_RETURN_ON_ERROR(cs_write_entry(store, start, store->next_seq, key, data, len, (uint16_t)count),
                     s_tag,
                     "write");
  (void)ra_cs_index_add(store, key, start, (uint16_t)count, len, false);
...
ra_err_t
ra_cache_store_get(const ra_cache_store_t* store, uint32_t key, ra_cache_store_reader_t* out_reader)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(out_reader, s_tag, "out_reader");
  RA_VALIDATE_INIT(store->inited, s_tag, "store");
  int32_t slot = ra_cs_index_find(store, key);
  if (slot < 0) {
    return k_ra_err_not_found;
  }
  const ra_cache_store_entry_t* e = &store->index[slot];
  *out_reader = (ra_cache_store_reader_t){.store        = store,
 ...
 * @return Error code.
 * @retval k_ra_ok                 All bytes copied.
 * @retval k_ra_err_null_ptr       `store` or `buf` NULL.
 * @retval k_ra_err_hw_init_failed LevelX read failed.
 ...
 * @since 0.1.0
 */
static ra_err_t cs_read_stream(const ra_cache_store_t* store,
                               uint32_t                data_start,
...
ra_err_t ra_cache_store_read(void* ctx, uint64_t offset, uint8_t* buf, uint32_t len)
{
  ...
  RA_CHECK_NULL_PTR(buf, s_tag, "buf");
  const ra_cache_store_reader_t* r     = (const ra_cache_store_reader_t*)ctx;
  const ra_cache_store_t*        store = r->store;
  RA_CHECK_NULL_PTR(store, s_tag, "reader store");
...
ra_err_t ra_cache_store_evict(ra_cache_store_t* store, uint32_t key)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_VALIDATE_INIT(store->inited, s_tag, "store");
  int32_t slot = ra_cs_index_find(store, key);
  ...
  ra_cache_store_entry_t* e = &store->index[slot];
  if ((e->flags & (uint8_t)k_ra_cache_store_flag_pinned) != 0U) {
    return k_ra_err_busy;
  }
  RA_RETURN_ON_ERROR(cs_mark_dirty(store), s_tag, "dirty");
  RA_RETURN_ON_ERROR(cs_release_run(store, e->start_sector, e->sector_count), s_tag, "release");
  store->live_sectors -= e->sector_count;
  *e = (ra_cache_store_entry_t){};
  return k_ra_ok;
}

ra_err_t ra_cache_store_pin(ra_cache_store_t* store, uint32_t key, bool pin)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_VALIDATE_INIT(store->inited, s_tag, "store");
  int32_t slot = ra_cs_index_find(store, key);
  ...
  ra_cache_store_entry_t* e = &store->index[slot];
  if (pin) {
    e->flags |= (uint8_t)k_ra_cache_store_flag_pinned;
  } else {
    e->flags &= (uint8_t)~(uint8_t)k_ra_cache_store_flag_pinned;
  }
  return k_ra_ok;
}

ra_err_t ra_cache_store_sync(ra_cache_store_t* store)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_VALIDATE_INIT(store->inited, s_tag, "store");
...
ra_err_t ra_cache_store_close(ra_cache_store_t* store)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_VALIDATE_INIT(store->inited, s_tag, "store");
  RA_RETURN_ON_ERROR(cs_checkpoint(store), s_tag, "checkpoint");
