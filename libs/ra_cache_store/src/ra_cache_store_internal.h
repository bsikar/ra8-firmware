/**
 * @file ra_cache_store_internal.h
 * @brief Internal on-media format + cross-TU helpers for ra_cache_store (#201).
 *
 * @details
 * Private to `libs/ra_cache_store/src/`. Declares the three fixed-width on-flash
 * records (superblock, entry header, directory entry) and the helpers shared
 * between the runtime TU (`ra_cache_store.c`) and the mount/recovery TU
 * (`ra_cache_store_mount.c`). Not a public interface -- consumers use
 * `ra_cache_store.h`.
 *
 ...
 *   |----------------------------|----------------------------------------------|
 *   | 0                          | ::ra_cs_super_t (superblock + clean marker)  |
 *   | [1, log_start)             | directory checkpoint (::ra_cs_dir_ent_t run) |
 *   | [log_start, logical)       | append log: ::ra_cs_entry_hdr_t + payload    |
 *
...
/**
 * @enum ra_cs_media_const_t
 * @brief Magics + serialization sizing for the on-flash records.
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_ra_cs_super_magic    = 0x52435331U, /**< Superblock tag 'R','C','S','1'.        */
  k_ra_cs_entry_magic    = 0x52435345U, /**< Entry-header tag 'R','C','S','E'.      */
  k_ra_cs_format_version = 1U,          /**< On-flash layout revision.              */
  k_ra_cs_dir_ent_bytes  = 16U,         /**< Serialized ::ra_cs_dir_ent_t size.     */
  k_ra_cs_dir_per_sector = 32U,         /**< Directory entries per 512-byte sector. */
  k_ra_cs_crc32_poly     = 0xEDB88320U, /**< Reflected CRC-32/ISO-HDLC polynomial.  */
  k_ra_cs_crc32_seed     = 0xFFFFFFFFU, /**< CRC-32 pre/post conditioning.          */
} ra_cs_media_const_t;

/**
 * @enum ra_cs_clean_t
 * @brief Values of ::ra_cs_super_t::clean (the shutdown marker).
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_ra_cs_dirty = 0U, /**< Session open or crashed -> mount must replay the log. */
  k_ra_cs_clean = 1U, /**< Clean shutdown -> the checkpoint directory is valid.  */
} ra_cs_clean_t;

/**
 * @struct ra_cs_super_t
...
typedef struct {
  uint32_t magic;           /**< ::k_ra_cs_super_magic.                */
  uint32_t version;         /**< ::k_ra_cs_format_version.             */
  uint32_t seq;             /**< Checkpoint sequence (monotonic).      */
  uint32_t clean;           /**< ::ra_cs_clean_t shutdown marker.      */
...
/* cppcheck-suppress-begin unusedStructMember */
typedef struct {
  uint32_t magic;        /**< ::k_ra_cs_entry_magic.                          */
...
static_assert(sizeof(ra_cs_dir_ent_t) == (uint32_t)k_ra_cs_dir_ent_bytes,
              "directory entry layout pinned at 16 bytes");
static_assert(sizeof(ra_cs_entry_hdr_t) <= (uint32_t)k_ra_cache_store_sector_bytes,
              "entry header must fit one sector");
static_assert(sizeof(ra_cs_super_t) <= (uint32_t)k_ra_cache_store_sector_bytes,
 ...
 *          XOR-finalises with ::k_ra_cs_crc32_seed internally, so the caller
 ...
 * @details Wraps `lx_nor_flash_sector_read`, mapping the LevelX status onto an `ra_err_t`.
 *
 ...
 * @return Error code.
 * @retval k_ra_ok               Sector read.
 * @retval k_ra_err_not_found    Sector is unmapped/released (LevelX miss).
 ...
 * @post On `k_ra_ok`, @p out512 holds the sector bytes.
 ...
 * @return Error code.
 * @retval k_ra_ok                Sector written.
 * @retval k_ra_err_hw_init_failed LevelX write error (e.g. no free sectors).
 *
 * @pre `store->flash` is an open LevelX NOR partition.
 * @pre @p in512 covers one sector.
 * @post On `k_ra_ok`, a later ::ra_cs_sector_read returns @p in512.
 ...
 * @param[in]     sector Logical sector index.
 * @return Error code.
 * @retval k_ra_ok                Sector released (or already free).
 * @retval k_ra_err_hw_init_failed LevelX release error.
 *
 * @pre `store->flash` is an open LevelX NOR partition.
 * @pre @p sector is within the partition.
 * @post On `k_ra_ok`, a later read of @p sector misses until rewritten.
 ...
 *          `next_seq`, ...) plus @p clean into ::ra_cs_super_t, seals it with a
 *          CRC, and writes sector 0 -- the commit point of a checkpoint.
 *
 * @param[in,out] store Store to snapshot.
 * @param[in]     clean ::ra_cs_clean_t marker to stamp.
 * @return Error code.
 * @retval k_ra_ok                Superblock written.
 * @retval k_ra_err_hw_init_failed LevelX write error.
 *
 * @pre `store->inited` is true (or mid-init with geometry set).
 * @pre `store->staging` covers one sector.
 * @post On `k_ra_ok`, sector 0 holds a valid superblock with @p clean.
 ...
 *
 * @details Packs every in-use slot as an ::ra_cs_dir_ent_t across the directory
