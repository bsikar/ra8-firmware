/**
 * @file ra_cache_store_mount.c
 * @brief ra_cache_store mount/recovery path + shared on-flash helpers (#201).
 *
 * @par Tag
 * [Ring 4 / Storage] {World: NS}
 *
 * @details
 * Owns ::ra_cache_store_init and the crash-recovery machinery: superblock
...
#include "lx_api.h"
#include "ra_cache_store.h"
#include "ra_cache_store_internal.h"
#include "ra_check.h"
#include "ra_err.h"

/** @brief Module log tag. */
static const char* const s_tag = "ra_cache_store";
 ...
 * @warning Written only by ::ra_cache_store_init; not for other modules.
 * @since 0.1.0
 */
...
typedef enum : uint32_t {
  k_ra_cs_crc_bits = 8U,   /**< Bits folded per input byte.               */
  k_ra_cs_pct_full = 100U, /**< Whole-percent denominator for the margin. */
} ra_cs_shift_t;

/* ------------------------------------------------------------------------- */
/* Shared low-level helpers (declared in ra_cache_store_internal.h) */
/* ------------------------------------------------------------------------- */

uint32_t ra_cs_crc32(const uint8_t* data, uint32_t len)
{
  if (data == nullptr) {
    return 0U;
  }
  ...
  uint32_t crc = (uint32_t)k_ra_cs_crc32_seed;
  for (uint32_t i = 0U; i < len; i++) {
    crc ^= data[i];
    for (uint8_t b = 0U; b < (uint8_t)k_ra_cs_crc_bits; b++) {
      bool lsb = (crc & 1U) != 0U;
      crc >>= 1U;
      if (lsb) {
        crc ^= (uint32_t)k_ra_cs_crc32_poly;
      }
    }
  }
  return crc ^ (uint32_t)k_ra_cs_crc32_seed;
}

ra_err_t ra_cs_sector_read(const ra_cache_store_t* store, uint32_t sector, uint8_t* out512)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(out512, s_tag, "out512");
...
ra_err_t ra_cs_sector_write(ra_cache_store_t* store, uint32_t sector, const uint8_t* in512)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(in512, s_tag, "in512");
...
ra_err_t ra_cs_sector_release(ra_cache_store_t* store, uint32_t sector)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(store->flash, s_tag, "flash");
...
int32_t ra_cs_index_find(const ra_cache_store_t* store, uint32_t key)
{
  if (store == nullptr) {
    return -1;
  }
  ...
  for (uint16_t i = 0U; i < store->index_cap; i++) {
    const ra_cache_store_entry_t* e = &store->index[i];
    if ((e->flags & (uint8_t)k_ra_cache_store_flag_in_use) == 0U) {
      continue;
    }
    if (e->key == key) {
      return (int32_t)i;
...
int32_t ra_cs_index_add(ra_cache_store_t* store,
                        uint32_t          key,
                        ...
                        bool              pinned)
{
  ...
  for (uint16_t i = 0U; i < store->index_cap; i++) {
    ra_cache_store_entry_t* e = &store->index[i];
    if ((e->flags & (uint8_t)k_ra_cache_store_flag_in_use) != 0U) {
      continue;
    }
    uint8_t flags = (uint8_t)k_ra_cache_store_flag_in_use;
    if (pinned) {
      flags |= (uint8_t)k_ra_cache_store_flag_pinned;
    }
    *e = (ra_cache_store_entry_t){.key          = key,
...
ra_err_t ra_cs_super_write(ra_cache_store_t* store, uint32_t clean)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(store->staging, s_tag, "staging");
  ra_cs_super_t sb = {.magic           = (uint32_t)k_ra_cs_super_magic,
                      .version         = (uint32_t)k_ra_cs_format_version,
                      .seq             = store->next_seq,
                      .clean           = clean,
                      .entry_count     = 0U,
                      ...
                      .crc             = 0U};
  for (uint16_t i = 0U; i < store->index_cap; i++) {
    if ((store->index[i].flags & (uint8_t)k_ra_cache_store_flag_in_use) != 0U) {
      sb.entry_count++;
    }
  }
  sb.crc = ra_cs_crc32((const uint8_t*)&sb, (uint32_t)(sizeof(sb) - sizeof(sb.crc)));
  (void)memset(store->staging, 0, (size_t)k_ra_cache_store_sector_bytes);
  (void)memcpy(store->staging, &sb, sizeof(sb));
  return ra_cs_sector_write(store, 0U, store->staging);
}
 ...
 * @return Error code.
 * @retval k_ra_ok Sector 0 parsed, or zeroed on an unreadable miss.
 ...
 * @since 0.1.0
 */
static ra_err_t cs_super_read(ra_cache_store_t* store, ra_cs_super_t* out_sb)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(out_sb, s_tag, "out_sb");
  if (ra_cs_sector_read(store, 0U, store->staging) != k_ra_ok) {
 ...
 * @since 0.1.0
 */
static bool cs_super_is_clean(const ra_cs_super_t* sb)
{
  if (sb == nullptr) {
    return false;
  }
  if (sb->magic != (uint32_t)k_ra_cs_super_magic) {
    return false;
  }
  uint32_t want = ra_cs_crc32((const uint8_t*)sb, (uint32_t)(sizeof(*sb) - sizeof(sb->crc)));
  if (sb->crc != want) {
    return false;
  }
  if (sb->clean != (uint32_t)k_ra_cs_clean) {
    return false;
  }
  return true;
}
 ...
 * @post @p out512 holds `<= k_ra_cs_dir_per_sector` serialized entries.
 * @note Not thread-safe; the store serialises access.
 * @since 0.1.0
 */
static uint32_t cs_dir_pack_sector(ra_cache_store_t* store, uint16_t* slot, uint8_t* out512)
{
  if (store == nullptr) {
    return 0U;
  }
  ...
  (void)memset(out512, 0, (size_t)k_ra_cache_store_sector_bytes);
  uint32_t packed = 0U;
  for (uint32_t e = 0U; e < (uint32_t)k_ra_cs_dir_per_sector; e++) {
    for (; *slot < store->index_cap; (*slot)++) {
      if ((store->index[*slot].flags & (uint8_t)k_ra_cache_store_flag_in_use) != 0U) {
        break;
      }
    }
    ...
    const ra_cache_store_entry_t* src = &store->index[*slot];
    ra_cs_dir_ent_t ent = {.key          = src->key,
                           .start_sector = src->start_sector,
                           .byte_len     = src->byte_len,
                           .sector_count = src->sector_count,
                           .flags = (uint16_t)(src->flags & (uint8_t)k_ra_cache_store_flag_pinned)};
    (void)memcpy(&out512[(size_t)e * (uint32_t)k_ra_cs_dir_ent_bytes], &ent, sizeof(ent));
    (*slot)++;
    packed++;
  }
  return packed;
...
ra_err_t ra_cs_dir_save(ra_cache_store_t* store, uint32_t* out_entry_count)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(out_entry_count, s_tag, "out_entry_count");
  uint32_t count = 0U;
  uint16_t slot  = 0U;
  for (uint16_t d = 0U; d < store->checkpoint_dirs; d++) {
    count += cs_dir_pack_sector(store, &slot, store->staging);
    RA_RETURN_ON_ERROR(ra_cs_sector_write(store, 1U + (uint32_t)d, store->staging), s_tag, "dir");
  }
 ...
 * @details Deserializes each ::ra_cs_dir_ent_t in @p in512, adds it to the index,
 ...
 * @since 0.1.0
 */
static void cs_dir_unpack_sector(ra_cache_store_t* store,
                                 const uint8_t*    in512,
                                 uint32_t*         loaded,
                                 uint32_t          entry_count)
{
  ...
  for (uint32_t e = 0U; e < (uint32_t)k_ra_cs_dir_per_sector; e++) {
    ...
    ra_cs_dir_ent_t ent = {};
    (void)memcpy(&ent, &in512[(size_t)e * (uint32_t)k_ra_cs_dir_ent_bytes], sizeof(ent));
    bool pinned = (ent.flags & (uint16_t)k_ra_cache_store_flag_pinned) != 0U;
    (void)ra_cs_index_add(store, ent.key, ent.start_sector, ent.sector_count, ent.byte_len, pinned);
 ...
 * @return Error code.
 * @retval k_ra_ok                Directory loaded; `live_sectors` accumulated.
 * @retval k_ra_err_invalid_state `entry_count` exceeds the index capacity.
 * @retval k_ra_err_hw_init_failed LevelX read error.
 * @pre `store->checkpoint_dirs` directory sectors exist on flash.
 * @pre `entry_count <= store->index_cap`.
 * @post On `k_ra_ok` the index holds every checkpoint entry.
 * @post `store->live_sectors` equals the loaded run total.
 * @note Not thread-safe; shares the store staging buffer.
 * @since 0.1.0
 */
static ra_err_t cs_dir_load(ra_cache_store_t* store, uint32_t entry_count)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(store->staging, s_tag, "staging");
  ...
  for (uint16_t d = 0U; d < store->checkpoint_dirs; d++) {
    ...
    RA_RETURN_ON_ERROR(ra_cs_sector_read(store, 1U + (uint32_t)d, store->staging), s_tag, "dir");
 ...
 * @since 0.1.0
 */
static bool cs_hdr_read(ra_cache_store_t* store, uint32_t s, ra_cs_entry_hdr_t* out_hdr)
{
  if (ra_cs_sector_read(store, s, store->staging) != k_ra_ok) {
    return false;
  }
  ra_cs_entry_hdr_t h = {};
  (void)memcpy(&h, store->staging, sizeof(h));
  if (h.magic != (uint32_t)k_ra_cs_entry_magic) {
    return false;
  }
  if (h.start_sector != s) {
    return false;
  ...
  uint32_t want = ra_cs_crc32((const uint8_t*)&h, (uint32_t)(sizeof(h) - sizeof(h.hdr_crc)));
  if (h.hdr_crc != want) {
    return false;
  }
  *out_hdr = h;
 ...
 * @since 0.1.0
 */
static void cs_scan_accept(ra_cache_store_t* store, const ra_cs_entry_hdr_t* h, uint32_t* max_seq)
{
  bool pinned = (h->flags & (uint16_t)k_ra_cache_store_flag_pinned) != 0U;
  if (ra_cs_index_find(store, h->key) >= 0) {
    return; /* write-once: a duplicate key on flash is ignored (keep the first). */
  }
  if (ra_cs_index_add(store, h->key, h->start_sector, h->sector_count, h->byte_len, pinned) < 0) {
 ...
 * @return Error code.
 * @retval k_ra_ok           Index rebuilt from surviving entries.
 * @retval k_ra_err_null_ptr `store` NULL.
 ...
 * @since 0.1.0
 */
static ra_err_t cs_scan_log(ra_cache_store_t* store)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(store->staging, s_tag, "staging");
  store->live_sectors = 0U;
 ...
 * @return Error code.
 * @retval k_ra_ok               Config is usable.
 * @retval k_ra_err_null_ptr     A required pointer member is NULL.
 * @retval k_ra_err_invalid_size `index_cap` zero or `staging_bytes` too small.
 * @retval k_ra_err_invalid_arg  `overprovision_pct` out of range.
 * @pre @p cfg is non-NULL.
 * @pre The caller wants the store bound to `cfg->nor_flash`.
 * @post No state is modified (pure validation).
 * @post On `k_ra_ok` every dependency needed by init is present.
 * @note Thread-safe: pure validation (no I/O).
 * @since 0.1.0
 */
static ra_err_t cs_validate_cfg(const ra_cache_store_cfg_t* cfg)
{
  RA_CHECK_NULL_PTR(cfg, s_tag, "cfg");
  RA_CHECK_NULL_PTR(cfg->nor_flash, s_tag, "nor_flash");
  if (cfg->nor_driver_init == nullptr) {
    ra_log_error(s_tag, "nor_driver_init");
    return k_ra_err_null_ptr;
  }
  RA_CHECK_NULL_PTR(cfg->index, s_tag, "index");
  RA_CHECK_NULL_PTR(cfg->staging, s_tag, "staging");
  if (cfg->index_cap == 0U) {
    return k_ra_err_invalid_size;
  }
  if (cfg->staging_bytes < (uint32_t)k_ra_cache_store_sector_bytes) {
    return k_ra_err_invalid_size;
  }
  if (cfg->overprovision_pct > (uint8_t)k_ra_cache_store_max_overprov) {
 ...
 * @return Error code.
 * @retval k_ra_ok               Geometry derived and stored.
 * @retval k_ra_err_invalid_size `logical_sectors` too small for the layout.
 * @pre @p store and @p cfg are non-NULL.
 * @pre @p cfg passed ::cs_validate_cfg.
 * @post On `k_ra_ok`, `log_start < logical_sectors` and `data_capacity > 0`.
 * @post On error @p store geometry is left partially set (init aborts).
 * @note Not thread-safe; the store serialises access.
 * @since 0.1.0
 */
static ra_err_t cs_geometry(ra_cache_store_t* store, const ra_cache_store_cfg_t* cfg)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(cfg, s_tag, "cfg");
  uint32_t dirs          = ((uint32_t)cfg->index_cap + (uint32_t)k_ra_cs_dir_per_sector - 1U) /
                           (uint32_t)k_ra_cs_dir_per_sector;
  store->checkpoint_dirs = (uint16_t)dirs;
  store->log_start       = 1U + dirs;
  store->logical_sectors = cfg->logical_sectors;
  if (cfg->logical_sectors < (store->log_start + (uint32_t)k_ra_cache_store_min_sectors)) {
    return k_ra_err_invalid_size;
  }
  uint32_t op          = (cfg->overprovision_pct != 0U) ? (uint32_t)cfg->overprovision_pct
                                                        : (uint32_t)k_ra_cache_store_overprov_pct;
  uint32_t usable      = cfg->logical_sectors - store->log_start;
  store->data_capacity = (usable * ((uint32_t)k_ra_cs_pct_full - op)) / (uint32_t)k_ra_cs_pct_full;
 ...
 * @return Error code.
 * @retval k_ra_ok                Partition open.
 * @retval k_ra_err_hw_init_failed LevelX format or open failed.
 * @pre `cfg->nor_driver_init` populates the LevelX control block.
 * @pre `store->flash` points at caller-owned LevelX storage.
 * @post On `k_ra_ok` the partition is open for sector I/O.
 * @post On error the partition is not usable.
 * @note Not thread-safe; single-threaded bring-up.
 * @since 0.1.0
 */
static ra_err_t cs_open_levelx(ra_cache_store_t* store, const ra_cache_store_cfg_t* cfg)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(cfg, s_tag, "cfg");
  ...
  CHAR* nm = (cfg->name != nullptr) ? (CHAR*)(uintptr_t)cfg->name : (CHAR*)(uintptr_t)"ra_cache";
  if (cfg->format) {
  ...
  if (cfg->format) {
    RA_RETURN_ON_ERROR(ra_cs_super_write(store, (uint32_t)k_ra_cs_clean), s_tag, "format super");
  }
  return k_ra_ok;
}
 ...
 * @return Error code.
 * @retval k_ra_ok                Index rebuilt.
 * @retval k_ra_err_invalid_state Checkpoint directory inconsistent.
 * @retval k_ra_err_hw_init_failed LevelX I/O error.
 * @pre @p sb was populated by ::cs_super_read.
 * @pre The LevelX partition is open.
 * @post On `k_ra_ok` the index reflects flash and `flash_state` is set.
 * @post No flash sector is written.
 * @note Not thread-safe; shares the store staging buffer.
 * @since 0.1.0
 */
static ra_err_t cs_recover(ra_cache_store_t* store, const ra_cs_super_t* sb)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(sb, s_tag, "sb");
  if (cs_super_is_clean(sb)) {
    store->next_seq    = sb->next_seq;
    store->flash_state = (uint8_t)k_ra_cs_clean;
    return cs_dir_load(store, sb->entry_count);
  }
  store->flash_state = (uint8_t)k_ra_cs_dirty;
 ...
 * @return Error code.
 * @retval k_ra_ok                Index recovered; `flash_state` set.
 * @retval k_ra_err_hw_init_failed LevelX I/O error.
 * @retval k_ra_err_invalid_state  Checkpoint directory inconsistent.
 * @pre The LevelX partition is open.
 * @pre `store` geometry and staging are set.
 * @post On `k_ra_ok` the index reflects flash; no flash sector was written.
 * @post On error the store is not usable.
 * @note Not thread-safe; shares the store staging buffer.
 * @since 0.1.0
 */
static ra_err_t cs_mount(ra_cache_store_t* store)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(store->staging, s_tag, "staging");
  ra_cs_super_t sb = {};
  RA_RETURN_ON_ERROR(cs_super_read(store, &sb), s_tag, "super");
 ...
 * @since 0.1.0
 */
static void cs_init_fields(ra_cache_store_t* store, const ra_cache_store_cfg_t* cfg)
{
  if (store == nullptr) {
    return;
  }
 ...
 * @param[in]     cfg   Validated config.
 * @return Error code.
 * @retval k_ra_ok                 Geometry set, partition open, index mounted.
 * @retval k_ra_err_null_ptr       `store` or `cfg` NULL.
 * @retval k_ra_err_invalid_size   `logical_sectors` too small for the layout.
 * @retval k_ra_err_hw_init_failed LevelX open/format/I-O error.
 * @retval k_ra_err_invalid_state  Checkpoint directory inconsistent.
 * @pre `store` buffers are bound (::cs_init_fields ran).
 * @pre @p cfg passed ::cs_validate_cfg.
 * @post On `k_ra_ok` the store is ready except for `inited`.
 * @post On error the store is not usable.
 * @note Not thread-safe; single-threaded bring-up.
 * @since 0.1.0
 */
static ra_err_t cs_bringup(ra_cache_store_t* store, const ra_cache_store_cfg_t* cfg)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_CHECK_NULL_PTR(cfg, s_tag, "cfg");
  RA_RETURN_ON_ERROR(cs_geometry(store, cfg), s_tag, "geometry");
  RA_RETURN_ON_ERROR(cs_open_levelx(store, cfg), s_tag, "levelx open");
  RA_RETURN_ON_ERROR(cs_mount(store), s_tag, "mount");
  return k_ra_ok;
}

ra_err_t ra_cache_store_init(ra_cache_store_t* store, const ra_cache_store_cfg_t* cfg)
{
  RA_CHECK_NULL_PTR(store, s_tag, "store");
  RA_RETURN_ON_ERROR(cs_validate_cfg(cfg), s_tag, "cfg");
  cs_init_fields(store, cfg);
  RA_RETURN_ON_ERROR(cs_bringup(store, cfg), s_tag, "bringup");
  store->inited = true;
  return k_ra_ok;
}
