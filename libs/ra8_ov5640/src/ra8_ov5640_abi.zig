...
comptime {
    ...
    // The JPEG status snapshot is part of the public C ABI; pin its exact
    // field layout here at the adapter boundary as well as in the core model.
    std.debug.assert(@sizeOf(JpegStatus) == 16);
    std.debug.assert(@alignOf(JpegStatus) == 4);
