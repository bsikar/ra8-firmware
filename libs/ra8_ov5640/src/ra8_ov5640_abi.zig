...
pub export fn ra8_ov5640_init(dev: ?*Device, bus: ?*const Bus) callconv(.c) u16 {
    if (rejectNull(dev, "init")) return @intFromEnum(Ov5640Error.null_ptr);
    if (rejectNull(bus, "init")) return @intFromEnum(Ov5640Error.null_ptr);
    const device = dev.?;
    const source = bus.?;
    if (rejectNull(@ptrCast(source.read_reg), "init")) return @intFromEnum(Ov5640Error.null_ptr);
    if (rejectNull(@ptrCast(source.write_reg), "init")) return @intFromEnum(Ov5640Error.null_ptr);
    if (rejectNull(@ptrCast(source.delay_ms), "init")) return @intFromEnum(Ov5640Error.null_ptr);
    device.* = .{
        .bus = source.*,
...
pub export fn ra8_ov5640_read_reg(dev: ?*Device, reg: u16, out_value: ?*u8) callconv(.c) u16 {
    if (rejectNull(dev, "read")) return @intFromEnum(Ov5640Error.null_ptr);
    if (rejectNull(out_value, "read")) return @intFromEnum(Ov5640Error.null_ptr);
    const device = dev.?;
...
pub export fn ra8_ov5640_write_reg(dev: ?*Device, reg: u16, value: u8) callconv(.c) u16 {
    if (rejectNull(dev, "write")) return @intFromEnum(Ov5640Error.null_ptr);
    const device = dev.?;
    if (!device.initialized) {
        return @intFromEnum(Ov5640Error.not_initialized);
    }
...
pub export fn ra8_ov5640_probe(dev: ?*Device, out_id: ?*u16) callconv(.c) u16 {
    if (rejectNull(dev, "probe")) return @intFromEnum(Ov5640Error.null_ptr);
    if (rejectNull(out_id, "probe")) return @intFromEnum(Ov5640Error.null_ptr);
    const device = dev.?;
    if (!device.initialized) {
        return @intFromEnum(Ov5640Error.not_initialized);
    }
...
pub export fn ra8_ov5640_configure(dev: ?*Device, mode: u8) callconv(.c) u16 {
    if (rejectNull(dev, "configure")) return @intFromEnum(Ov5640Error.null_ptr);
    const device = dev.?;
    if (!device.initialized) {
        return @intFromEnum(Ov5640Error.not_initialized);
    }
...
pub export fn ra8_ov5640_set_jpeg_quantization_scale(dev: ?*Device, quant_scale: u8) callconv(.c) u16 {
    if (rejectNull(dev, "jpeg_quality")) return @intFromEnum(Ov5640Error.null_ptr);
    const device = dev.?;
    if (!device.initialized) {
        return @intFromEnum(Ov5640Error.not_initialized);
    }
...
pub export fn ra8_ov5640_jpeg_status_get(dev: ?*Device, out_status: ?*JpegStatus) callconv(.c) u16 {
    if (rejectNull(dev, "jpeg_status")) return @intFromEnum(Ov5640Error.null_ptr);
    if (rejectNull(out_status, "jpeg_status")) return @intFromEnum(Ov5640Error.null_ptr);
    const device = dev.?;
