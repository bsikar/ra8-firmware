 ...
 * Every public API that can fail returns `ra_err_t`; every caller checks it
 * via the macros in `ra_check.h` (`RA_RETURN_ON_ERROR`, `RA_ERROR_CHECK`,
 ...
 * ## Design
 *
 * - **Single enum**: one `ra_err_codes_t` with `uint16_t` underlying type
 ...
 * - **0 is success**: `k_ra_ok == 0` so the idiom `if (err)` treats any
 *   non-zero value as a failure without requiring `!= k_ra_ok`.
 ...
 *
 * @code{.c}
 * ra_err_t ra_sci_init(const ra_sci_cfg_t* cfg)
 * {
 *     RA_CHECK_NULL_PTR(cfg, "SCI", "cfg must not be nullptr");
 *     if (cfg->channel >= k_ra_sci_channel_count) {
 *         return k_ra_err_gpio_invalid_port;  // reusing a generic "out-of-range" code
 *     }
 ...
 *   `ra_check.h` returns one of the values below.
 * - **Rule 7**: every API that can fail returns `ra_err_t`. `[[nodiscard]]`
 ...
 * `ra_err_t`. Values are grouped by category and use stable hexadecimal
 * numbering so logs, telemetry, and debugger watch expressions decode
 * without needing source.
 *
 * @invariant `k_ra_ok == 0` (success is the only zero value).
 ...
 *            aliases such as `k_ra_err_threadx`).
 *
...
typedef enum : uint16_t {
   ...
   *          `k_ra_ok` rather than the numeric 0 for clarity.
   */
  k_ra_ok = 0,
   ...
   * @par Value: 0x103
   * @see k_ra_err_null_ptr           More specific: pointer was NULL
   * @see k_ra_err_range_check_failed More specific: numeric out of range
   */
  k_ra_err_invalid_arg = 0x103,
   ...
   * @par Value: 0x104
   * @see k_ra_err_not_initialized More specific: init() never ran
   */
  k_ra_err_invalid_state = 0x104,

  /**
   * @brief Invalid size parameter (too large, too small, or misaligned).
   * @par Value: 0x105
   */
  k_ra_err_invalid_size = 0x105,

  /**
   * @brief Requested item not found (lookup / search missed).
   * @par Value: 0x106
   */
  k_ra_err_not_found = 0x106,
   ...
   * @par Value: 0x408
   */
  k_ra_err_conflict = 0x408,

  /**
   * @brief Retry budget exhausted -- operation still failing after all attempts.
   * @par Value: 0x409
   */
  k_ra_err_retry_limit = 0x409,

  /* =========================================================================
   * Validation errors (0x500 -- 0x5FF)
   * Raised by ra_check.h helpers and explicit pre/post-condition checks
   * =========================================================================
   */
   ...
   * @par Value: 0x501
   */
  k_ra_err_validation_failed = 0x501,

  /**
   * @brief Stored / transmitted checksum does not match computed value.
   * @par Value: 0x502
   */
  k_ra_err_checksum_mismatch = 0x502,

  /**
   * @brief Value outside range enforced by `RA_CHECK_RANGE` / `RA_CHECK_RANGE_TAG`.
   * @par Value: 0x503
   */
  k_ra_err_range_check_failed = 0x503,

  /**
   * @brief Pointer was NULL where a valid pointer was required.
   * @details This is the value returned by `RA_CHECK_NULL_PTR`.
   * @par Value: 0x504
   */
  k_ra_err_null_ptr = 0x504,
} ra_err_codes_t;
 ...
 * `ra_check.h` to propagate and check.
 *
 ...
 */
typedef ra_err_codes_t ra_err_t;
