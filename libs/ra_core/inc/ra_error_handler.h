/**
 * @file ra_error_handler.h
 * @brief Centralised fatal-error sink
 *
 * @details
 * `internal_ra_fatal_error()` is the single entry point for "stop
 * running and alert the developer". Every `RA_ASSERT` and every
 * `RA_ERROR_CHECK` that fails eventually lands here.
 *
 ...
 * Field deployments can override `internal_ra_fatal_error()` with a
