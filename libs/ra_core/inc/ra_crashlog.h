 ...
 *
 * @details
 * `ra_exception.c` decodes every CPU fault / NMI into the fixed-SRAM
 * ::ra_exception_last_t snapshot `g_ra_exception_last` and then halts.
 ...
 *
 *  1. A `.noinit` record (::ra_crashlog_record_t) that the reset handler
 ...
 *     `libs/ra_board_ek_ra8d2/ld/linker_script.ld` (`.noinit`).
 *
 ...
 *     fault. If it climbs past ::k_ra_crashlog_loop_threshold without the
 *     application reaching a known-good checkpoint and calling
 *     ra_crashlog_claim(), ra_crashlog_safe_mode_requested() latches true
 ...
 * ra_crashlog_install();                 // arm the fault-persist hook
 ...
 *     ra_crashlog_claim();               // reset the guard, boot safe mode
 *     enter_safe_mode();
 *   }
 * }
 * run_risky_bringup();                    // a fault here bumps boot_loops
 * ra_crashlog_claim();                    // reached known-good: reset guard
 * @endcode
 *
 ...
 * (VBATT `ra_bkup` is silicon-blocked -- see issue #131) and is the named
...
/**
 * @enum ra_crashlog_magic_t
 * @brief Sentinel marking a fully-written ::ra_crashlog_record_t.
 *
 * @details
 ...
 * from ::k_ra_exc_magic_valid so the two fixed-SRAM records are never
 * confused by a debugger.
 *
 * @see ra_crashlog_record_t
 * @since 0.1.0
 */
typedef enum : uint32_t {
  k_ra_crashlog_magic_valid = 0x5AFEB007UL, /**< "SAFE-BOOT": record is valid. */
} ra_crashlog_magic_t;

/**
 * @enum ra_crashlog_limits_t
 * @brief Reset-loop threshold and the linker reservation size.
 *
 * @details
 * ::k_ra_crashlog_loop_threshold is the number of recorded faults that
 * may accumulate before ra_crashlog_safe_mode_requested() latches; the
 * app resets the count by reaching a known-good state and calling
 * ra_crashlog_claim(). ::k_ra_crashlog_reserve_bytes MUST equal the
 * `LENGTH(NOINIT)` region size in
 * `libs/ra_board_ek_ra8d2/ld/linker_script.ld`; a compile-time assertion
 * in ra_crashlog.c proves the record fits it.
 *
 * @since 0.1.0
 */
typedef enum : uint32_t {
 ...
 * when `magic == k_ra_crashlog_magic_valid` AND the recomputed CRC equals
 * `crc`.
 *
 * @invariant `magic == k_ra_crashlog_magic_valid` only while `crc` matches
 ...
 * //   (gdb) print/x g_ra_ls_noinit_start
 * //   (gdb) print *(ra_crashlog_record_t*)<that address>
 * // magic 0x5AFEB007 => trust boot_loops + fault.
 * @endcode
 *
 * @see ra_crashlog_peek()
 * @see ra_exception_last_t
 * @since 0.1.0
 */
typedef struct {
  uint32_t            magic;      /**< ::k_ra_crashlog_magic_valid when valid (not CRC-covered). */
  uint32_t            crc;        /**< CRC-32 over `boot_loops`..end of record.                  */
  uint32_t            boot_loops; /**< Monotonic recorded-fault counter; cleared by a claim.     */
  ra_exception_last_t fault;      /**< Embedded copy of the decoded fault / NMI snapshot.        */
} ra_crashlog_record_t;

/**
 * @brief Arm the fault-persist hook so decoded faults are logged to `.noinit`.
 *
 * @details
 * Registers ra_crashlog_record_fault() as the exception module's
 * post-decode persist sink (see ra_exception_set_persist_hook()). After
 * this call, every fault/NMI that reaches ra_exception_report() copies its
 ...
 *
 * @details
 * The write path installed by ra_crashlog_install(): reads the prior
 ...
 *                    `nullptr`; typically `&g_ra_exception_last`.
 *
 * @return Nothing.
 *
 * @pre @p decoded points at a populated ::ra_exception_last_t.
 ...
 * @post `magic == k_ra_crashlog_magic_valid` once the write completes.
 *
 * @note Not thread-safe; single fault/boot context by construction.
 * @see ra_crashlog_install()
 * @see ra_crashlog_peek()
 * @since 0.1.0
 */
 ...
 * stored record is left intact (call ra_crashlog_claim() to consume it).
 ...
 * @see ra_crashlog_claim()
 * @see ra_crashlog_safe_mode_requested()
 * @since 0.1.0
 */
 ...
 * ra_crashlog_peek() reports empty) and zeroes `boot_loops` (so the
 ...
 * @post A subsequent ra_crashlog_peek() returns `false`.
 ...
 * exceeds ::k_ra_crashlog_loop_threshold -- i.e. the device has recorded
 * more than the allowed number of faults without a clean
 * ra_crashlog_claim() in between. The application reads this early in boot
 * to divert into a minimal safe mode instead of re-running the code that
 * keeps faulting.
 *
 * @return Whether safe mode is requested.
 * @retval true  Valid record and `boot_loops > k_ra_crashlog_loop_threshold`.
 ...
 * @post The record, if any, is left intact for ra_crashlog_peek().
 *
 * @note Not thread-safe.
 * @see ra_crashlog_claim()
 * @since 0.1.0
