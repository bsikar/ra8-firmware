 ...
 * by `cmake/ra_warnings.cmake`, and aggregates a project-wide report
 ...
 * with `RA_STACK_BUDGET(N)` so it is greppable and so the next reader
 ...
 * @code{.c}
 * ra_err_t ra_rsip_protected_rsa_decrypt(...)
 * {
 *     RA_STACK_BUDGET(1720); // approved frame ceiling per .su file
 ...
 * @since 0.1.0
 */
#define RA_STACK_BUDGET(bytes) ((void)0) /* approved frame budget = bytes */
