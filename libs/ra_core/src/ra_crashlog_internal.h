/**
 * @file ra_crashlog_internal.h
 * @brief Cross-TU surface for ra_crashlog (host-test white-box access only).
 *
 * @details
 * Not part of the public API. On the host unit-test build
 * (`RA_SIMULATOR_MODE`) the `.noinit` record is an ordinary zero-init
 ...
 * for `ra_crashlog_is_valid`. See CLAUDE.md "Test access to internal
 * symbols (MC/DC scope)".
 *
 ...
 * ra_crashlog_peek() rejects the record. Never linked into a firmware
 * build (guarded by `RA_SIMULATOR_MODE`).
 *
 * @par MC/DC:
 * Backs the vectors for `libs/ra_core/src/ra_crashlog.c@ra_crashlog_is_valid`:
 ...
 *
 * @pre Host unit-test build (`RA_SIMULATOR_MODE`).
 ...
 * `magic == 0`, so ra_crashlog_peek() reports empty.
 *
 * @return Nothing.
 *
 * @pre Host unit-test build (`RA_SIMULATOR_MODE`).
 * @pre The caller is a test under `tests/`.
 * @post The record reads as all-zero; ra_crashlog_peek() returns `false`.
