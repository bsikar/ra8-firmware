 ...
 * `.heap` region and `_sbrk()` (``ra_sbrk_trap.c``) traps any allocation, per
 * NASA Power of 10 Rule 3. But `ra_epub` opens `.epub` archives through the
 ...
 * arena (``ra_epub_xml_shim.cpp``), `ra_img_arena`, and `ra_stbtt_alloc`.
 *
 * The free list coalesces adjacent free blocks on every free, so the pool is
 * reusable across an arbitrary number of `ra_epub_load_chapter` calls (each
 ...
 *
 * @note Not thread-safe: one shared static pool, single-book usage. `ra_epub`
 *       drives a single archive at a time (file-scope OPF scratch), matching
 *       this constraint.
 * @note On the host unit-test build (``RA_SIMULATOR_MODE``) `ra_epub` leaves
...
/**
 * @enum ra_epub_miniz_alloc_limits_t
...
typedef enum : uint32_t {
  k_ra_epub_miniz_pool_bytes = 98304U, /**< Arena size, bytes (96 KiB). */
} ra_epub_miniz_alloc_limits_t;
 ...
 * @param[in] address Block previously returned by ::ra_epub_miniz_alloc /
 *                    ::ra_epub_miniz_realloc, or NULL.
 *
