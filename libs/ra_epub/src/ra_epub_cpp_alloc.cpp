/**
 * @file ra_epub_cpp_alloc.cpp
 * @brief Arena-backed global `operator new` / `operator delete` for firmware.
 *
 * @details
 * The only C++ in `libs/ra_epub` is the tinyxml2-backed XML shim
 * (`ra_epub_xml_shim.cpp`). tinyxml2 allocates its node pools
 ...
 * (::ra_epub_miniz_alloc / ::ra_epub_miniz_free). It is compiled into the
 * firmware only: under `RA_SIMULATOR_MODE` (the host unit-test build) the body
 ...
 * single `ra_epub_*` call. Only apps that link `ra_epub` pull this TU in (the
 * build wires the ra_epub C++ sources solely for them), so non-EPUB firmware
 * is unaffected.
 *
 ...
 * scratch is capped at `k_ra_epub_opf_xml_buf`, so tinyxml2's node footprint is
...
constexpr std::size_t k_ra_epub_new_one = 1U;

/**
 * @brief Allocate @p n bytes from the shared ra_epub static arena.
 *
 ...
 * @since 0.1.0
 */
void* ra_epub_cpp_new(std::size_t n)
{
  const std::size_t want = (n != 0U) ? n : (std::size_t)1U;
  return ra_epub_miniz_alloc(nullptr, k_ra_epub_new_one, want);
}
...
{
  ra_epub_miniz_free(nullptr, ptr);
}
