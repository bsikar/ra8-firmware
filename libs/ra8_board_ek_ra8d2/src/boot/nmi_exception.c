/**
 * @file libs/ra8_board_ek_ra8d2/src/boot/nmi_exception.c
...
internal_nmi_report(const ra8_exception_frame_t* frame);
RA8_INTERNAL [[noreturn, gnu::used]] static void
internal_nmi_report(const ra8_exception_frame_t* frame)
{
 ...
 * `internal_nmi_report()`, which appends the ICU cause. Naked asm is
 ...
 *
 * @post Control transfers to `internal_nmi_report()` and never returns.
 * @post The interrupted context's frame pointer is in `r0`.
 *
 * @note Runs only in NMI context and is not thread-callable.
 *
 * @since 0.1.0
 */
