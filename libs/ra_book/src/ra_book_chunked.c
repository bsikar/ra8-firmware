/**
 * @file ra_book_chunked.c
 * @brief Implementation of the demand-paged "RBKC" chunk reader.
 *
 * @details
 * Geometry parsing is shared with the resident open through
 * `ra_book_container_header_fields()` / `ra_book_container_table_entry()`
 * (ra_book_internal.h), so the container format has exactly one in-firmware
 ...
 * ::ra_book_chunked_read stages one compressed stream and inflates it into
 * the caller's frame.
 *
 * @since Version 0.1.0
 *
...
static const char* const s_tag_chunked = "ra_book_chunked";
 ...
 * @retval k_ra_err_invalid_size A compressed chunk exceeds the staging budget.
 *
 ...
 * @post On k_ra_ok every stream span is loadable through the staging buffer.
 *
 ...
 */
static ra_err_t
s_table_check(const ra_book_chunked_t* rd, const uint64_t* table_buf, uint64_t payload_len)
{
  if (table_buf[0] != 0U) {
 ...
 *
 * @return ra_err_t Status code.
 * @retval k_ra_ok               Table loaded and validated; @p rd bound.
 * @retval k_ra_err_invalid_arg  Table shape invalid (start, monotonicity, end).
 * @retval k_ra_err_invalid_size Table exceeds the caller buffer / file bounds /
 ...
 * @post On k_ra_ok, `rd->table == table_buf` and `rd->payload_off` is set.
 ...
 * @since Version 0.1.0
 */
static ra_err_t s_load_table(ra_book_chunked_t* rd,
                             uint64_t           file_len,
                             uint64_t*          table_buf,
                             uint32_t           table_cap_entries)
{
  ...
  if (file_len < ((uint64_t)k_ra_book_container_header_len + table_bytes)) {
    return k_ra_err_invalid_size;
  }
  const ra_err_t err = rd->file_read(rd->file_ctx,
                                     (uint64_t)k_ra_book_container_header_len,
                                     (uint8_t*)table_buf,
                                     (uint32_t)table_bytes);
  if (err != k_ra_ok) {
    return err;
  }
  const uint64_t payload_off = (uint64_t)k_ra_book_container_header_len + table_bytes;
  const ra_err_t shape       = s_table_check(rd, table_buf, file_len - payload_off);
  if (shape != k_ra_ok) {
    return shape;
  }
  rd->table       = table_buf;
...
/**
 * @brief The argument-checked body of ::ra_book_chunked_open.
 *
 * @details
 ...
 * @post On k_ra_ok, `rd->table` is bound and reads may be served.
 * @post On any error `rd->table` stays NULL (reader unbound).
 *
 * @note Not thread-safe.
 *
 * @since Version 0.1.0
 */
static ra_err_t s_chunked_open_body(ra_book_chunked_t* rd,
                                    uint64_t           file_len,
                                    uint64_t*          table_buf,
                                    uint32_t           table_cap_entries)
{
  if (file_len < (uint64_t)k_ra_book_container_header_len) {
    return k_ra_err_invalid_size;
  }
  uint8_t  hdr[k_ra_book_container_header_len] = {};
  ra_err_t err = rd->file_read(rd->file_ctx, 0U, hdr, (uint32_t)sizeof(hdr));
  if (err != k_ra_ok) {
    return err;
  }
  err =
    ra_book_container_header_fields(hdr, &rd->chunk_bytes, &rd->inflated_total, &rd->chunk_count);
  if (err != k_ra_ok) {
    return err;
  }
...
ra_err_t ra_book_chunked_open(ra_book_chunked_t* rd,
...
{
  RA_CHECK_NULL_PTR(rd, s_tag_chunked, "open: null rd");
  RA_CHECK_NULL_PTR((const void*)file_read, s_tag_chunked, "open: null file_read");
  RA_CHECK_NULL_PTR((const void*)inflate, s_tag_chunked, "open: null inflate");
  RA_CHECK_NULL_PTR(table_buf, s_tag_chunked, "open: null table_buf");
  RA_CHECK_NULL_PTR(staging, s_tag_chunked, "open: null staging");

  *rd             = (ra_book_chunked_t){};
 ...
 *
 * @details
 * The I/O tail of ::ra_book_chunked_read after the geometry checks: reads
 ...
 *
 * @return ra_err_t Status code.
 * @retval k_ra_ok               Chunk inflated into @p buf.
 ...
 * @post On k_ra_ok, `buf[0..len)` holds the chunk's inflated bytes.
 ...
 */
static ra_err_t
s_stage_and_inflate(const ra_book_chunked_t* rd, uint32_t idx, uint8_t* buf, uint32_t len)
{
...
ra_err_t ra_book_chunked_read(void* ctx, uint64_t offset, uint8_t* buf, uint32_t len)
{
  RA_CHECK_NULL_PTR(ctx, s_tag_chunked, "read: null ctx");
  RA_CHECK_NULL_PTR(buf, s_tag_chunked, "read: null buf");
  const ra_book_chunked_t* rd = (const ra_book_chunked_t*)ctx;
  if (rd->table == nullptr) {
