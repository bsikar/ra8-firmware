 ...
 * reader at open and validated once; each ::ra_cbz_page_read then stages one
 * page's compressed stream and inflates it into the caller's frame. The format
 * is page-indexed rather than uniform-chunked (see ra_book_chunked.c for the
 ...
 * MC/DC obligation of its own (mirroring ra_book_chunked.c).
 *
 * @since Version 0.1.0
 *
...
static const char* const s_tag_cbz = "ra_cbz";
 ...
 *
 * @details Reads the 12-byte record at `meta + page * k_ra_cbz_meta_len` and
 ...
 * @param[out] out_format   Receives the @ref ra_book_image_format_t.
 * @param[out] out_raw_size Receives the inflated raster length.
 *
 * @pre @p meta holds at least `(page + 1) * k_ra_cbz_meta_len` bytes.
...
{
  const uint8_t* rec = &meta[(size_t)page * (size_t)k_ra_cbz_meta_len];
  *out_raw_size      = s_le32(&rec[k_ra_cbz_meta_off_raw]);
  *out_width         = s_le16(&rec[k_ra_cbz_meta_off_width]);
  *out_height        = s_le16(&rec[k_ra_cbz_meta_off_height]);
  *out_format        = rec[k_ra_cbz_meta_off_format];
}
 ...
 *          ::k_ra_book_flag_mask_known. Field decoding is memcpy-based so the
 *          header buffer needs no alignment.
 *
 * @param[in]  hdr        First @ref k_ra_cbz_header_len bytes of the container.
 ...
 * @retval k_ra_err_invalid_arg Bad magic, zero page count, non-zero reserved
 *                              word, or an unknown feature-flag bit.
 *
 * @pre @p hdr holds at least @ref k_ra_cbz_header_len readable bytes.
 * @pre @p out_pages and @p out_flags are distinct, writable locations.
 * @post On k_ra_ok both outputs are populated and internally consistent.
 * @post On any error no output is modified.
 *
 * @note Thread-safe: reads only @p hdr, writes only the outputs.
 * @since Version 0.1.0
 */
static ra_err_t s_parse_header(const uint8_t* hdr, uint32_t* out_pages, uint32_t* out_flags)
{
  if (memcmp(hdr, "RCBZ", (size_t)k_ra_cbz_magic_len) != 0) {
    return k_ra_err_invalid_arg;
  }
  const uint32_t pages    = s_le32(&hdr[k_ra_cbz_hdr_off_page_count]);
  const uint32_t flags    = s_le32(&hdr[k_ra_cbz_hdr_off_flags]);
  const uint32_t reserved = s_le32(&hdr[k_ra_cbz_hdr_off_reserved]);
  if (pages == 0U) {
    return k_ra_err_invalid_arg;
  }
  if (reserved != 0U) {
  ...
  if ((flags & ~(uint32_t)k_ra_book_flag_mask_known) != 0U) {
    return k_ra_err_invalid_arg;
  }
  *out_pages = pages;
  *out_flags = flags;
 ...
 *
 * @return ra_err_t Status code.
 * @retval k_ra_ok               Table well-formed; every stream fits staging.
 * @retval k_ra_err_invalid_arg  Bad start / non-monotonic / bad end offset.
 * @retval k_ra_err_invalid_size A compressed page exceeds the staging budget.
 *
 ...
 * @post On k_ra_ok every page's stream is loadable through the staging buffer.
 *
 * @note Thread-safe: reads only its arguments.
 * @since Version 0.1.0
 */
static ra_err_t s_check_offsets(const ra_cbz_t* cbz, const uint64_t* offsets, uint64_t payload_len)
{
  if (offsets[0] != 0U) {
    return k_ra_err_invalid_arg;
  }
 ...
 *          be registrable (::ra_vsource_add_paged rejects a zero size); a
 ...
 *
 * @return ra_err_t Status code.
 * @retval k_ra_ok              Every page has a non-zero raster length.
 * @retval k_ra_err_invalid_arg Some page declares a zero raster length.
 *
 * @pre @p meta holds `page_count * k_ra_cbz_meta_len` readable bytes.
 * @pre @p page_count matches the header's validated page count.
 * @post No state is modified (pure validation).
 * @post On k_ra_ok every page's raster length is usable as an object size.
 *
 * @note Thread-safe: reads only its arguments.
 * @since Version 0.1.0
 */
static ra_err_t s_check_meta(const uint8_t* meta, uint32_t page_count)
{
  for (uint32_t i = 0U; i < page_count; ++i) { /* bound: validated page_count */
    const uint8_t* rec = &meta[(size_t)i * (size_t)k_ra_cbz_meta_len];
    if (s_le32(&rec[k_ra_cbz_meta_off_raw]) == 0U) {
      return k_ra_err_invalid_arg;
    }
  }
  return k_ra_ok;
 ...
 *
 * @return ra_err_t Status code.
 * @retval k_ra_ok               Tables loaded and validated; @p cbz bound.
 * @retval k_ra_err_invalid_arg  Offset shape or a raster length is invalid.
 * @retval k_ra_err_invalid_size A table exceeds a caller buffer / file bounds /
 *                               the one-call read limit, or a page exceeds
 *                               staging.
 * @retval k_ra_err_*            A file-read error, returned verbatim.
 *
 ...
 * @post On k_ra_ok, `cbz->offsets`, `cbz->meta` and `cbz->payload_off` are set.
 * @post On any error `cbz->offsets` stays NULL (reader unbound).
 *
 * @note Not thread-safe.
 * @since Version 0.1.0
 */
static ra_err_t s_load_tables(ra_cbz_t* cbz,
                              uint64_t  file_len,
                              ...
                              uint32_t  meta_cap)
{
  ...
  const uint64_t offset_bytes = entries * (uint64_t)k_ra_cbz_offset_len;
  const uint64_t meta_bytes   = (uint64_t)cbz->page_count * (uint64_t)k_ra_cbz_meta_len;
  ...
  const uint64_t tables_end = (uint64_t)k_ra_cbz_header_len + offset_bytes + meta_bytes;
  if (file_len < tables_end) {
    return k_ra_err_invalid_size;
  }
  ra_err_t err = cbz->file_read(cbz->file_ctx,
                                (uint64_t)k_ra_cbz_header_len,
                                (uint8_t*)offsets_buf,
  ...
  err = cbz->file_read(cbz->file_ctx,
                       (uint64_t)k_ra_cbz_header_len + offset_bytes,
                       meta_buf,
                       (uint32_t)meta_bytes);
  if (err != k_ra_ok) {
    return err;
  }
  const ra_err_t shape = s_check_offsets(cbz, offsets_buf, file_len - tables_end);
  if (shape != k_ra_ok) {
    return shape;
  }
  const ra_err_t mchk = s_check_meta(meta_buf, cbz->page_count);
  if (mchk != k_ra_ok) {
    return mchk;
  }
...
/**
 * @brief The argument-checked body of ::ra_cbz_open.
 *
 ...
 * @retval k_ra_err_invalid_arg  Bad magic / header fields / table shape.
 * @retval k_ra_err_invalid_size Short file or a caller buffer too small.
 * @retval k_ra_err_*            A file-read error, returned verbatim.
 *
 * @pre Every pointer argument was null-checked by the caller.
 * @pre `cbz->file_read` / `cbz->staging` / `cbz->staging_cap` are pre-bound.
 * @post On k_ra_ok, `cbz->offsets` is bound and reads may be served.
 * @post On any error `cbz->offsets` stays NULL (reader unbound).
 *
 * @note Not thread-safe.
 * @since Version 0.1.0
 */
static ra_err_t s_open_body(ra_cbz_t* cbz,
                            uint64_t  file_len,
                            uint64_t* offsets_buf,
                            uint32_t  offsets_cap,
                            uint8_t*  meta_buf,
                            uint32_t  meta_cap)
{
  if (file_len < (uint64_t)k_ra_cbz_header_len) {
    return k_ra_err_invalid_size;
  }
  uint8_t  hdr[k_ra_cbz_header_len] = {};
  ra_err_t err                      = cbz->file_read(cbz->file_ctx, 0U, hdr, (uint32_t)sizeof(hdr));
  if (err != k_ra_ok) {
    return err;
  }
...
/**
 * @brief Reject any NULL required argument to ::ra_cbz_open.
 *
 ...
 *
 * @return ra_err_t Status code.
 * @retval k_ra_ok           Every required argument is non-NULL.
 * @retval k_ra_err_null_ptr Some required argument was NULL.
 *
 * @pre The caller forwards ::ra_cbz_open's arguments unchanged.
 * @pre No argument is dereferenced before this check returns k_ra_ok.
 * @post On k_ra_ok every checked pointer is safe to dereference.
 * @post On any error the reason is logged against ::s_tag_cbz.
 *
 * @note Thread-safe: reads only its pointer arguments.
 * @since Version 0.1.0
 */
static ra_err_t s_open_reject_null(const ra_cbz_t*    cbz,
...
{
  RA_CHECK_NULL_PTR(cbz, s_tag_cbz, "open: null cbz");
  RA_CHECK_NULL_PTR((const void*)file_read, s_tag_cbz, "open: null file_read");
  RA_CHECK_NULL_PTR((const void*)inflate, s_tag_cbz, "open: null inflate");
  RA_CHECK_NULL_PTR(offsets_buf, s_tag_cbz, "open: null offsets_buf");
  RA_CHECK_NULL_PTR(meta_buf, s_tag_cbz, "open: null meta_buf");
  RA_CHECK_NULL_PTR(staging, s_tag_cbz, "open: null staging");
  return k_ra_ok;
}

ra_err_t ra_cbz_open(ra_cbz_t*          cbz,
                     ra_vsource_read_fn file_read,
                     void*              file_ctx,
                     uint64_t           file_len,
                     ra_book_inflate_fn inflate,
...
{
  const ra_err_t nerr = s_open_reject_null(cbz, file_read, inflate, offsets_buf, meta_buf, staging);
  if (nerr != k_ra_ok) {
    return nerr;
  }
  *cbz             = (ra_cbz_t){};
...
ra_err_t ra_cbz_page_info(const ra_cbz_t* cbz,
                          uint32_t        page,
                          uint16_t*       out_width,
                          uint16_t*       out_height,
                          uint8_t*        out_format,
                          uint32_t*       out_raw_size)
{
  RA_CHECK_NULL_PTR(cbz, s_tag_cbz, "info: null cbz");
  RA_CHECK_NULL_PTR(out_width, s_tag_cbz, "info: null out_width");
  RA_CHECK_NULL_PTR(out_height, s_tag_cbz, "info: null out_height");
  RA_CHECK_NULL_PTR(out_format, s_tag_cbz, "info: null out_format");
  RA_CHECK_NULL_PTR(out_raw_size, s_tag_cbz, "info: null out_raw_size");
  if (cbz->meta == nullptr) {
...
ra_err_t ra_cbz_page_bind(const ra_cbz_t* cbz, uint32_t page, ra_cbz_page_t* out_cur)
{
  RA_CHECK_NULL_PTR(cbz, s_tag_cbz, "bind: null cbz");
  RA_CHECK_NULL_PTR(out_cur, s_tag_cbz, "bind: null out_cur");
  *out_cur = (ra_cbz_page_t){};
  if (cbz->meta == nullptr) {
    return k_ra_err_invalid_state;
  }
  if (page >= cbz->page_count) {
 ...
 *
 * @details The I/O tail of ::ra_cbz_page_read after the geometry checks: reads
 ...
 * @post On k_ra_ok, `buf[0..raw_size)` holds the page's inflated raster.
 ...
 */
static ra_err_t
s_stage_and_inflate_page(const ra_cbz_t* cbz, uint32_t page, uint32_t raw_size, uint8_t* buf)
{
...
ra_err_t ra_cbz_page_read(void* ctx, uint64_t offset, uint8_t* buf, uint32_t len)
{
  RA_CHECK_NULL_PTR(ctx, s_tag_cbz, "read: null ctx");
  RA_CHECK_NULL_PTR(buf, s_tag_cbz, "read: null buf");
  const ra_cbz_page_t* cur = (const ra_cbz_page_t*)ctx;
  if (cur->cbz == nullptr) {
    return k_ra_err_invalid_state;
  }
