/**
 * @file ra_book_chunked.h
 * @brief Demand-paged chunk reader for the "RBKC" `.rabook` container.
 *
 * @details
 * The bridge between the chunked on-disk container (see @ref
 * ra_book_container_t in ra_book.h) and the #147 page-cache stack: where
 * ra_book_open() inflates *every* chunk into one resident SDRAM buffer, this
 * reader inflates *single* chunks on demand, so a book far larger than RAM is
 * read through an ::ra_vmem cache with a bounded resident working set.
 *
 * ::ra_book_chunked_read has exactly the ::ra_vsource_read_fn signature, so a
 * bound reader plugs straight into ::ra_vsource_add_paged as the backing of a
 * paged object whose byte space is the *inflated flat blob*:
 *
 * @code
 * ra_book_chunked_t rd = {};
 * err = ra_book_chunked_open(&rd, sd_file_read, &file, file_len, sh_inflate,
 *                            table_buf, k_table_entries, staging, sizeof staging);
 * err = ra_vsource_add_paged(&vs, ra_book_chunked_read, &rd,
 *                            0U, rd.inflated_total, &book_obj);
 * err = ra_book_src_paged(&src, &vm, book_obj,
 *                         rd.chunk_bytes, (uint32_t)rd.inflated_total);
 * @endcode
 *
 * One chunk equals one cache frame equals one inflate call: the container is
 * produced with `chunk_bytes` equal to the reader's ::ra_vmem frame size, each
 * chunk's zlib stream is staged through a caller-owned compressed-bytes buffer
 * and inflated directly into the frame ::ra_vsource_loader hands over -- no
 ...
 * what ::ra_vsource_loader passes when the cache's `frame_bytes` equals the
 ...
 *
 * @see ra_book.h        Container layout + the resident open.
 * @see ra_book_paged.h  Binds the paged object as a book source.
 * @see ra_vsource.h     The registry ::ra_book_chunked_read plugs into.
 *
 * @since Version 0.1.0
 *
...
/**
 * @struct ra_book_chunked_t
 * @brief One open chunked `.rabook` file: parsed geometry + caller storage.
 *
 * @details Populated by ::ra_book_chunked_open; treat every field as read-only
 *          afterwards. `file_read` addresses the *container file bytes* (the
 *          compressed on-disk form); ::ra_book_chunked_read then serves the
 ...
 *
 * @see ra_book_chunked_open()
 * @since Version 0.1.0
 */
/* cppcheck-suppress-begin [unusedStructMember] -- members are read in
 * ra_book_chunked.c through the bound pointer; cppcheck's per-TU view of
 * this header cannot see those uses (same waiver as ra_vsource_obj_t). */
typedef struct {
 ...
 * @p staging_cap. On success @p rd is bound and ::ra_book_chunked_read may
 ...
 * @retval k_ra_err_invalid_size @p file_len shorter than header + table, the
 ...
 * @post On k_ra_ok, @p rd is bound and ::ra_book_chunked_read serves
 *       `[0, rd->inflated_total)` of the flat blob.
 * @post On any error @p rd is left unbound (its `table` stays NULL).
 *
 * @note Not thread-safe.
 * @see ra_book_chunked_read()
 * @since Version 0.1.0
 */
[[nodiscard]] ra_err_t ra_book_chunked_open(ra_book_chunked_t* rd,
 ...
 *
 * @details
 * The ::ra_vsource_read_fn implementation: looks the chunk covering @p offset
 ...
 * ::ra_vsource_loader passes when the ::ra_vmem `frame_bytes` equals the
 * container's `chunk_bytes`. One call is one SD-read burst plus one inflate.
 *
 * @param[in]  ctx    The bound ::ra_book_chunked_t (as a void cookie).
 ...
 * @retval k_ra_err_invalid_state @p ctx was never bound by ::ra_book_chunked_open.
 * @retval k_ra_err_invalid_arg   @p offset is not chunk-aligned, or @p len is
 *                                not the chunk's exact inflated span.
 * @retval k_ra_err_out_of_range  @p offset is at or past the inflated total.
 ...
 *
 * @pre @p ctx was populated by ::ra_book_chunked_open.
 * @pre @p buf addresses at least @p len writable bytes.
 * @post On k_ra_ok, `buf[0..len)` holds the flat blob's bytes at @p offset.
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra_err_t ra_book_chunked_read(void* ctx, uint64_t offset, uint8_t* buf, uint32_t len);
