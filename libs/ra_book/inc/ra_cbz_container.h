 ...
 *
 * Where ::ra_book_chunked_t serves one *flat blob* split into uniform
 * `chunk_bytes` chunks, ::ra_cbz_t serves a *page-indexed* container: the
 ...
 * length). Each page is read a page at a time -- one ::ra_cbz_page_read is one
 ...
 *
 * ::ra_cbz_page_read has exactly the ::ra_vsource_read_fn signature, so a
 * per-page cursor bound by ::ra_cbz_page_bind plugs straight into
 * ::ra_vsource_add_paged as the backing of a paged object whose byte space is
 * *that one page's inflated raster*:
 *
 * @code
 * ra_cbz_t cbz = {};
 * err = ra_cbz_open(&cbz, sd_file_read, &file, file_len, sh_inflate,
 *                   offsets_buf, k_offset_entries, meta_buf, sizeof meta_buf,
 *                   staging, sizeof staging);
 * ra_cbz_page_t cur = {};
 * err = ra_cbz_page_bind(&cbz, page_idx, &cur);
 * uint32_t oid = 0U;
 * err = ra_vsource_add_paged(&vs, ra_cbz_page_read, &cur, 0U, cur.raw_size, &oid);
 * @endcode
 *
 ...
 *   [8]  uint32 flags            ra_book_flag_t bits (k_ra_book_flag_rtl, ...)
 ...
 *        uint16 height, uint8 format (ra_book_image_format_t), 3 reserved bytes
 ...
 * ::ra_cbz_page_read serves a *whole page* per call: `offset` must be 0 and
 * `len` must equal the page's `raw_size`. This is precisely what
 * ::ra_vsource_loader passes for a per-page object when the cache's
 ...
 * sub-page tiling for pan/zoom -- that is ::ra_tile_cache's job (issue #211),
 ...
 *
 * @see ra_book_chunked.h  The uniform-chunk flat-blob reader this mirrors.
 * @see ra_vsource.h       The registry ::ra_cbz_page_read plugs into.
 * @see ra_book.h          ::ra_book_flag_t / ::ra_book_image_format_t reused here.
 *
 * @since Version 0.1.0
 *
...
typedef enum : uint8_t {
  k_ra_cbz_magic_len  = 4U,  /**< Length of the "RCBZ" magic.                       */
  k_ra_cbz_header_len = 16U, /**< Fixed header bytes ahead of the offset table.     */
  k_ra_cbz_offset_len = 8U,  /**< One offset-table entry (uint64 LE stream offset). */
  k_ra_cbz_meta_len   = 12U, /**< One metadata-table entry (see @ref ra_cbz_t).     */
} ra_cbz_layout_t;

/**
 * @enum ra_cbz_header_field_off_t
 * @brief Byte offsets of the fields inside the fixed "RCBZ" header.
 * @details Mirrors the layout documented on @ref ra_cbz_container.h; the magic
 *          occupies `[0, k_ra_cbz_magic_len)` and is matched, not decoded.
 * @since Version 0.1.0
 */
typedef enum : uint8_t {
  k_ra_cbz_hdr_off_page_count = 4U,  /**< uint32 LE: number of pages.     */
  k_ra_cbz_hdr_off_flags      = 8U,  /**< uint32 LE: ra_book_flag_t bits. */
  k_ra_cbz_hdr_off_reserved   = 12U, /**< uint32 LE: reserved, must be 0. */
} ra_cbz_header_field_off_t;

/**
 * @enum ra_cbz_meta_field_off_t
 * @brief Byte offsets of the fields inside one metadata-table entry.
 * @details Entry `i` starts at `meta + i * k_ra_cbz_meta_len`; the three bytes
 *          after `format` are reserved padding (0).
 * @since Version 0.1.0
 */
typedef enum : uint8_t {
  k_ra_cbz_meta_off_raw    = 0U, /**< uint32 LE: inflated raster length in bytes. */
  k_ra_cbz_meta_off_width  = 4U, /**< uint16 LE: pixel width.                     */
  k_ra_cbz_meta_off_height = 6U, /**< uint16 LE: pixel height.                    */
  k_ra_cbz_meta_off_format = 8U, /**< uint8: @ref ra_book_image_format_t.         */
} ra_cbz_meta_field_off_t;

/**
 * @struct ra_cbz_t
 * @brief One open "RCBZ" container: parsed geometry + caller-owned storage.
 *
 * @details Populated by ::ra_cbz_open; treat every field as read-only
 *          afterwards. `file_read` addresses the *container file bytes*;
 *          ::ra_cbz_page_read then serves each page's *inflated raster* on top
 ...
 * ra_cbz_container.c through the bound pointer; cppcheck's per-TU view of this
 * header cannot see those uses (same waiver as ra_book_chunked_t). */
typedef struct {
  ra_vsource_read_fn file_read;   /**< Byte reader over the container file.      */
  void*              file_ctx;    /**< Context for @ref file_read.               */
  ra_book_inflate_fn inflate;     /**< zlib-stream decompressor.                 */
  ...
  uint32_t           flags;       /**< @ref ra_book_flag_t bits (RTL, ...).      */
} ra_cbz_t;
/* cppcheck-suppress-end [unusedStructMember] */

/**
 * @struct ra_cbz_page_t
 * @brief A cursor onto one page, used as the ::ra_vsource_read_fn context.
 *
 * @details Bound by ::ra_cbz_page_bind; pass its address as the `ctx` of
 *          ::ra_vsource_add_paged so the paged object's read callback resolves
 *          to exactly this page. `raw_size` is the page's inflated raster
 *          length -- the object size to register, and the only `len` value
 *          ::ra_cbz_page_read accepts for this page.
 *
 * @invariant `page < cbz->page_count` and `raw_size == meta[page].raw_size`.
 *
 * @see ra_cbz_page_bind()
 * @see ra_cbz_page_read()
 * @since Version 0.1.0
 */
/* cppcheck-suppress-begin [unusedStructMember] -- read in ra_cbz_container.c
 * through the bound pointer; cppcheck's per-TU header view cannot see it. */
typedef struct {
  const ra_cbz_t* cbz;      /**< The open container this page belongs to. */
 ...
 * compressed span is). On success @p cbz is bound and ::ra_cbz_page_read may
 ...
 * @param[in]  inflate            zlib decompressor (see @ref ra_book_inflate_fn).
 ...
 * @retval k_ra_ok               Reader bound; geometry fully validated.
 * @retval k_ra_err_null_ptr     A required pointer argument was NULL.
 * @retval k_ra_err_invalid_arg  Bad magic / header field / offset-table shape /
 *                               zero page raster length.
 * @retval k_ra_err_invalid_size @p file_len shorter than header + tables, a
 ...
 * @retval k_ra_err_*            A @p file_read error, returned verbatim.
 *
 ...
 * @post On k_ra_ok, @p cbz is bound and each page's raster is servable.
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra_err_t ra_cbz_open(ra_cbz_t*          cbz,
                                   ra_vsource_read_fn file_read,
                                   void*              file_ctx,
                                   uint64_t           file_len,
 ...
 * @details Reads the page count parsed by ::ra_cbz_open, guarding against a
 *          NULL or not-yet-bound reader so a shelf view can query it safely
 *          before a container is opened.
 * @param[in] cbz Container bound by ::ra_cbz_open (non-NULL).
 * @return The page count, or 0 for a NULL / unbound reader.
 * @retval 0 @p cbz is NULL or was never bound by ::ra_cbz_open.
 * @pre @p cbz was populated by ::ra_cbz_open.
 ...
 * @since Version 0.1.0
 */
static inline uint32_t ra_cbz_page_count(const ra_cbz_t* cbz)
{
  if (cbz == nullptr) {
    return 0U;
  }
 ...
 * @details Decodes ::k_ra_book_flag_rtl from the header `flags`; the reader
 ...
 * @since Version 0.1.0
 */
static inline bool ra_cbz_is_rtl(const ra_cbz_t* cbz)
{
  if (cbz == nullptr) {
    return false;
  }
  ...
  return (cbz->flags & (uint32_t)k_ra_book_flag_rtl) != 0U;
}
 ...
 *
 * @param[in]  cbz          Container bound by ::ra_cbz_open (non-NULL).
 * @param[in]  page         Page index (`< ra_cbz_page_count(cbz)`).
 * @param[out] out_width    Receives the page pixel width (non-NULL).
 * @param[out] out_height   Receives the page pixel height (non-NULL).
 * @param[out] out_format   Receives the @ref ra_book_image_format_t (non-NULL).
 * @param[out] out_raw_size Receives the inflated raster length (non-NULL).
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                Outputs populated from the manifest.
 * @retval k_ra_err_null_ptr      A required pointer argument was NULL.
 * @retval k_ra_err_invalid_state @p cbz was never bound by ::ra_cbz_open.
 ...
 * @post On k_ra_ok every output is populated from page @p page.
 ...
 * @since Version 0.1.0
 */
[[nodiscard]] ra_err_t ra_cbz_page_info(const ra_cbz_t* cbz,
                                        uint32_t        page,
                                        uint16_t*       out_width,
                                        uint16_t*       out_height,
                                        uint8_t*        out_format,
...
/**
 * @brief Bind a per-page cursor for use as a ::ra_vsource_read_fn context.
 *
 ...
 *          `ra_vsource_add_paged(&vs, ra_cbz_page_read, out_cur, 0,
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                Cursor bound to @p page.
 * @retval k_ra_err_null_ptr      @p cbz or @p out_cur was NULL.
 * @retval k_ra_err_invalid_state @p cbz was never bound by ::ra_cbz_open.
 * @retval k_ra_err_out_of_range  @p page is at or past the page count.
 *
 * @pre @p cbz was populated by ::ra_cbz_open.
 * @pre @p out_cur is a writable ::ra_cbz_page_t.
 * @post On k_ra_ok, @p out_cur addresses page @p page and out-lives its object.
 * @post On any error @p out_cur is left unbound (its `cbz` stays NULL).
 *
 * @note Not thread-safe with respect to a concurrent ::ra_cbz_open on @p cbz.
 * @see ra_cbz_page_read()
 * @since Version 0.1.0
 */
[[nodiscard]] ra_err_t ra_cbz_page_bind(const ra_cbz_t* cbz, uint32_t page, ra_cbz_page_t* out_cur);

/**
 * @brief Serve one whole page's inflated raster -- the ::ra_vsource_read_fn.
 *
 * @details
 * The ::ra_vsource_read_fn implementation for a page cursor: reads the page's
 ...
 * equals the page's `raw_size` -- exactly what ::ra_vsource_loader passes for a
 ...
 *
 * @param[in]  ctx    The bound ::ra_cbz_page_t (as a void cookie).
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                Page inflated into @p buf.
 * @retval k_ra_err_null_ptr      @p ctx or @p buf was NULL.
 * @retval k_ra_err_invalid_state @p ctx or its container was never bound.
 * @retval k_ra_err_invalid_arg   @p offset is non-zero, or @p len is not the
 *                                page's exact raster length.
 * @retval k_ra_err_invalid_size  The stream inflated to the wrong length.
 * @retval k_ra_err_*             A file-read or inflater error, verbatim.
 *
 * @pre @p ctx was populated by ::ra_cbz_page_bind.
 * @pre @p buf addresses at least @p len writable bytes.
 * @post On k_ra_ok, `buf[0..len)` holds the page's inflated raster.
 * @post On any error @p buf contents are unspecified.
 *
 * @note Not thread-safe: the staging buffer is reused across calls.
 * @see ra_vsource_add_paged()
 * @since Version 0.1.0
 */
[[nodiscard]] ra_err_t ra_cbz_page_read(void* ctx, uint64_t offset, uint8_t* buf, uint32_t len);
