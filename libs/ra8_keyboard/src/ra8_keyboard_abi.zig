...
extern fn ra8_ui_rect_contains(r: ?*const Rect, px: i32, py: i32) u8;

fn containsSeam(r: ?*const Rect, px: i32, py: i32) callconv(.c) bool {
    return ra8_ui_rect_contains(r, px, py) != 0;
}
...
pub export fn ra8_kbd_key_glyph(kb: ?*const Layout, key_idx: u8) callconv(.c) c_char {
    const layout = kb orelse return 0;
    return @bitCast(implementation.glyphOf(layout, key_idx));
}
