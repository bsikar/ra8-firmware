...
test "the exported symbols keep the header's signatures" {
    ...
    try std.testing.expectEqual(c_char, @typeInfo(@TypeOf(abi.ra8_kbd_key_glyph)).@"fn".return_type.?);
...
test "key-glyph guard MC/DC V1: a laid-out key returns its glyph" {
    ...
    try std.testing.expectEqual(@as(c_char, 'q'), abi.ra8_kbd_key_glyph(&kb, q));
}
...
test "key-glyph guard MC/DC V3: an index at the count returns nothing" {
    const kb = laidOut();
    try std.testing.expectEqual(@as(c_char, 0), abi.ra8_kbd_key_glyph(&kb, kb.count));
}
...
test "key_glyph reports nothing for a special key" {
    const kb = laidOut();
    try std.testing.expectEqual(@as(c_char, 0), abi.ra8_kbd_key_glyph(&kb, indexOfKind(&kb, .enter)));
    try std.testing.expectEqual(@as(c_char, 0), abi.ra8_kbd_key_glyph(&kb, indexOfKind(&kb, .shift)));
    try std.testing.expectEqual(@as(c_char, 0), abi.ra8_kbd_key_glyph(&kb, indexOfKind(&kb, .space)));
}
