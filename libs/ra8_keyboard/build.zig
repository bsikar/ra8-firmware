...
//!
//! Build graph for the Zig implementation of `ra8_keyboard`. CMake consumes the
//! installed static library through the unchanged `inc/ra8_keyboard.h` C ABI;
//! the `test` step covers the layout core and the ABI membrane.
//!
//! There is no build option here, deliberately: the C translation unit this
//! replaces took no `-D` of its own.
...
pub fn build(b: *std.Build) void {
    ...
    const library_module = b.createModule(.{
        .root_source_file = b.path("src/ra8_keyboard_abi.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    abi_tests.root_module.addIncludePath(b.path("../ra8_core/inc"));
    abi_tests.root_module.addIncludePath(b.path("../ra8_ui/inc"));
    abi_tests.root_module.addCSourceFile(.{
        .file = b.path("tests/abi_fixture.c"),
        .flags = &.{"-std=c23"},
    });

    const run_internal_tests = b.addRunArtifact(internal_tests);
    const run_abi_tests = b.addRunArtifact(abi_tests);
    const test_step = b.step("test", "Run Zig ra8_keyboard tests");
