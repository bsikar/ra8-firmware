...
pub export fn fw_fs_rename(
    ...
    if (valid != core.ok) return valid;
    const bound = names.?;
    const capability = core.renameCapabilityStatus(bound.caps.flags, replace);
    ...
    return bound.iface.?.rename.?(bound.ctx, old_path.?, new_path.?, replace);
}
...
pub export fn fw_fs_transaction_commit(
    ...
    published.* = false;
    const result = handle.iface.?.commit.?(handle.ctx, handle.state, published);
    if ((result == core.ok) and !published.*) return core.err_invalid_state;
    if (published.*) {
        handle.active = false;
        handle.validated = false;
    }
