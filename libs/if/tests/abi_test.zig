...
test "rename checks capability first, then both paths, then the root" {
    ...
    try std.testing.expectEqual(
        ...
        abi.fw_fs_rename(&names, "/a", "/b", true),
    );
    try std.testing.expectEqual(core.ok, abi.fw_fs_rename(&names, "/a", "/b", false));
    try std.testing.expect(!rig.fake.last_replace);
    try std.testing.expectEqual(
        core.err_access_denied,
        abi.fw_fs_rename(&names, "/", "/b", false),
    );
    try std.testing.expectEqual(
        core.err_access_denied,
        abi.fw_fs_rename(&names, "/a", "/", false),
    );
    try std.testing.expectEqual(
        core.err_invalid_arg,
        abi.fw_fs_rename(&names, "/a", "b", false),
    );
}
