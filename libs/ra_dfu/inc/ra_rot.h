 ...
 *
 *  - **copy-to-run** (``libs/ra_dfu/src/ra_dfu_launch.c``): the bootloader
 *    must authenticate a slot image before copying it to SRAM and branching
 *    into it.
 *  - **BLXNS** (``libs/ra_tz_secure_boot/src/ra_tz_secure_boot.c``): the
 ...
 * ## Signed-image format
 *
 * A signed image is ``[ body (body_len bytes) ] [ ra_rot_trailer_t ]``. The
 ...
 *     the ``ra_psa`` software hash in ``RA_SIMULATOR_MODE``);
 ...
 *
 * The whole root of trust is gated behind the ``RA_ENABLE_ROOT_OF_TRUST``
 * build flag (default OFF), mirroring ``RA_BOOT_ENABLE_CACHE_MPU``:
 *
 *  - **Flag OFF (default):** ``ra_rot.c`` compiles to nothing and pulls in no
 ...
 *    PRIVATE RA_ENABLE_ROOT_OF_TRUST)`` and MUST then also (a) link
 *    ``ra_psa_crypto`` + the RSIP HAL, and (b) sign its DFU / Non-Secure
 *    images with an ::ra_rot_trailer_t -- otherwise every launch
 *    **default-denies**.
 *
 ...
 * (``k_ra_dfu_img_max`` ~= 448 KiB) and the Non-Secure MRAM partition
 ...
 * (``ra_rsip_sha256`` on target, ``ra_psa_hash_compute`` under
 * ``RA_SIMULATOR_MODE``), cross-checks it against ``trailer->digest``, then
 * verifies ``trailer->sig`` over the *freshly-computed* digest against the
 * provisioned root public key via ``ra_psa_verify_hash`` (algorithm
 * ``k_ra_psa_alg_ecdsa_sha_256``). The signature is the authority; the
 * trailer digest is only a fast pre-check.
 *
 * Any failure returns a non-``k_ra_ok`` error -- the caller MUST treat that
 * as DENY and not launch / not branch.
 *
 * @param[in] body     Pointer to the image body's first byte; non-NULL.
 * @param[in] body_len Body length in bytes; ``(0, k_ra_rot_body_max]`` and
 *                     equal to ``trailer->body_len``.
 * @param[in] trailer  Authenticity trailer that follows the body; non-NULL.
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok                  Image is authentic -- launch is permitted.
 * @retval k_ra_err_null_ptr        ``body`` or ``trailer`` is NULL (missing
 *                                  trailer -> deny).
 * @retval k_ra_err_validation_failed Trailer magic / version wrong (malformed
 *                                  trailer -> deny).
 * @retval k_ra_err_invalid_size    ``body_len`` out of range, mismatched
 *                                  against the trailer, or ``sig_len`` invalid.
 * @retval k_ra_err_checksum_mismatch Re-computed digest differs from the
 *                                  trailer digest (tampered body -> deny).
 * @retval k_ra_err_crc_mismatch    Signature did not verify (forged /
 *                                  mis-signed -> deny).
 * @retval k_ra_err_hw_error        Underlying hash / verify engine fault.
 *
 * @pre ``body`` addresses at least ``body_len`` readable bytes.
 * @pre ``trailer`` addresses a readable ::ra_rot_trailer_t.
 ...
 * @param[in] body_len   Body length in bytes; ``(0, k_ra_rot_body_max]``.
 *
 ...
 *
 * @pre ``image_base`` addresses at least ``body_len + sizeof(ra_rot_trailer_t)``
 *      readable bytes.
 ...
 * logic can be driven end-to-end against the ``RA_SIMULATOR_MODE`` crypto
 ...
 *
 * @return ra_err_t Error code.
 * @retval k_ra_ok           ``*out_key`` / ``*out_len`` populated.
 * @retval k_ra_err_null_ptr ``out_key`` or ``out_len`` is NULL.
 *
 * @pre ``out_key`` and ``out_len`` are non-NULL.
 * @pre The caller is a host unit test (the symbol is a test helper).
 * @post ``*out_len == k_ra_rot_pubkey_bytes`` on success.
 ...
 * @since 0.1.0
 */
RA_TEST_HELPER ra_err_t ra_rot_root_public_key(const uint8_t** out_key, uint32_t* out_len);
