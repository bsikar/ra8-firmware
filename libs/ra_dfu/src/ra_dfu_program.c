...
ra_err_t ra_dfu_program_prepare(ra_dfu_slot_t inactive)
{
  ...
  /* Fence every subsequent write/erase to this one slot -- the bootloader
   * and the active slot are outside [base, high) and cannot be touched. */
  err = ra_flash_set_window(base, high);
  if (err != k_ra_ok) {
    return err;
  }
  const uint32_t blocks = (uint32_t)k_ra_dfu_slot_size / (uint32_t)k_ra_dfu_page_size;
  return ra_flash_erase(base, blocks);
}
...
ra_dfu_program_image(ra_dfu_slot_t inactive, uint32_t img_offset, const uint8_t* data, uint32_t len)
{
  ...
  return ra_flash_write(dst, data, len);
}
...
ra_err_t ra_dfu_program_commit(ra_dfu_slot_t inactive, uint32_t img_len, uint32_t seq)
{
   ...
   * (invalid), never a valid header over a partial image. */
  return ra_flash_write(base, (const uint8_t*)&hdr, (uint32_t)k_ra_dfu_hdr_size);
}
