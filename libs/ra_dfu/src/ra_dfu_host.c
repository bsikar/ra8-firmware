 ...
 * ra_usb_host_control_xfer.  On success the 18-byte descriptor is stored in
 ...
 * returns k_ra_err_hw_error so that the caller can retry.
 *
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              Descriptor received and fully length-checked.
 * @retval k_ra_err_hw_error    Transfer succeeded but byte count was short.
 * @retval k_ra_err_*           Underlying ra_usb_host_control_xfer failure.
 *
 * @pre The USB host controller named by @p speed has been initialized.
 * @pre The device is held at address 0 (before SET_ADDRESS).
 * @post On k_ra_ok, @p desc holds the complete 18-byte DEVICE descriptor.
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              Device attached and descriptor received.
 * @retval k_ra_err_hw_timeout  All k_rdh_enum_tries attempts failed.
 * @retval k_ra_err_*           Last internal_get_dev_desc failure code.
 *
 * @pre The USB host controller named by @p speed has been initialized.
 * @pre VBUS is powered on the host jack.
 * @post On k_ra_ok, the device is enumerated at address 0 and @p desc is valid.
 ...
 * calls ra_usb_host_set_target to point the host DCP at address 1 so
 * subsequent control transfers reach the addressed device.
 *
 * @param[in]  speed  Controller speed (FS or HS) selecting the USB port.
 *
 * @return ra_err_t
 * @retval k_ra_ok              SET_ADDRESS completed; DCP retargeted to addr 1.
 * @retval k_ra_err_*           ra_usb_host_control_xfer or set_target failure.
 *
 ...
 * @post On k_ra_ok, all subsequent DCP transactions use device address 1.
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              Device is now in the Configured state (dfuIDLE).
 * @retval k_ra_err_*           ra_usb_host_control_xfer failure.
 *
 ...
 * @post On k_ra_ok, the DFU interface is active and the device is in dfuIDLE.
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              Status received; @p out_state is valid.
 * @retval k_ra_err_hw_error    Transfer succeeded but byte count was not 6.
 * @retval k_ra_err_*           ra_usb_host_control_xfer failure.
 *
 ...
 * @post On k_ra_ok, @p out_state holds the current DFU bState value.
 ...
 * returns k_ra_ok immediately.  Between unsuccessful polls it inserts a
 * k_rdh_status_poll_ms delay.  If the retry cap is exhausted without seeing
 * the target state the function returns k_ra_err_hw_timeout.  Any
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              The device entered @p want_state within the limit.
 * @retval k_ra_err_hw_timeout  k_rdh_status_tries polls elapsed without match.
 * @retval k_ra_err_*           internal_getstatus failure propagated.
 *
 * @pre The device is in the Configured state and the DFU interface is active.
 * @pre @p want_state is a valid DFU bState value for the expected transition.
 * @post On k_ra_ok, the device bState at the last GETSTATUS poll equals @p want_state.
 * @post On k_ra_err_hw_timeout, at least k_rdh_status_tries GETSTATUS polls
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              Block delivered and device reached dfuDNLOAD-IDLE.
 * @retval k_ra_err_hw_timeout  Device did not reach dfuDNLOAD-IDLE in time.
 * @retval k_ra_err_*           ra_usb_host_control_xfer or wait_state failure.
 *
 ...
 * @post On k_ra_ok, the device has accepted block @p block and is in dfuDNLOAD-IDLE.
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              All blocks downloaded; device is in dfuIDLE.
 * @retval k_ra_err_hw_timeout  A block poll timed out or abort wait timed out.
 * @retval k_ra_err_*           internal_dnload_block or DFU_ABORT xfer failure.
 *
 ...
 * @post On k_ra_ok, all image blocks are buffered in the device and it is in dfuIDLE.
 ...
 * is responsible for confirming the commit via ra_dfu_device_committed because
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              All blocks and the zero-length manifest were accepted.
 * @retval k_ra_err_hw_timeout  A block poll timed out before the manifest was sent.
 * @retval k_ra_err_*           internal_dnload_block or manifest xfer failure.
 *
 * @pre The device is in dfuIDLE state (SET_CONFIGURATION completed).
 * @pre @p img_len is a non-zero multiple of k_rdh_xfer_size and @p img is valid.
 * @post On k_ra_ok, the device has received all data blocks and the manifest request.
 * @post The device is entering dfuMANIFEST; the caller must not re-enumerate it
 *       until the commit is confirmed via ra_dfu_device_committed.
 *
 * @note Not thread-safe; called only from the single-threaded polled sequence.
 * @since 0.1.0
 */
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok                All blocks uploaded and byte-matched @p img.
 * @retval k_ra_err_invalid_size  An upload block returned the wrong byte count.
 * @retval k_ra_err_invalid_state An upload block's data differed from @p img.
 * @retval k_ra_err_*             ra_usb_host_control_xfer failure.
 *
 ...
 * @post On k_ra_ok, out->blocks_ok equals img_len / k_rdh_xfer_size.
 ...
 * Any step failure propagates immediately; the caller (ra_dfu_host_run) tears
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              All five steps completed successfully.
 * @retval k_ra_err_hw_timeout  Attach or GETSTATUS wait exhausted.
 * @retval k_ra_err_*           Error from any of the five sub-functions.
 *
 * @pre The USB host controller named by @p speed has been initialized via ra_usb_host_init.
 * @pre @p img and @p out are non-NULL; @p img_len is a non-zero multiple of 64.
 * @post On k_ra_ok, out->pid is set and out->blocks_ok equals img_len / 64.
 ...
 * same chip) commits the slot and the caller confirms via ra_dfu_device_committed.
 * Any step failure propagates immediately; the caller (ra_dfu_host_program)
 ...
 *
 * @return ra_err_t
 * @retval k_ra_ok              All four steps completed; device is in dfuMANIFEST.
 * @retval k_ra_err_hw_timeout  Attach or GETSTATUS wait exhausted.
 * @retval k_ra_err_*           Error from any of the four sub-functions.
 *
 * @pre The USB host controller named by @p speed has been initialized via ra_usb_host_init.
 * @pre @p img and @p out are non-NULL; @p img_len is a non-zero multiple of 64.
 * @post On k_ra_ok, out->pid is set and the device has received the manifest request.
 * @post On failure, the host controller is NOT torn down here; the caller handles it.
 *
 * @note Not thread-safe; called only from the single-threaded polled sequence.
 * @since 0.1.0
