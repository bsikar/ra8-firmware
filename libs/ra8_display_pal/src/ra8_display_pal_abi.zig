...
comptime {
    // Keep every public value type tied directly to the C declarations at the
    // ABI membrane, as well as the backend/config/handle structs below.
...
fn validateHandle(d: ?*const Handle) u16 {
    if (d == null) return implementation.err_null_ptr;
    if (!s_initialized) return implementation.err_invalid_arg;
    if (d != @as(?*const Handle, &s_handle)) return implementation.err_invalid_arg;
