...
#
# Usage:
#   /bin/bash -p coprocessor/esp32c6/build.sh
#
# Requires:
...
if [[ "$-" == *p* ]]; then
  ...
  # ---- 1b. run both offline C6 contract gates before fetching ----
  ...
  echo "build.sh: OK -- flash these with /bin/bash -p coprocessor/esp32c6/flash.sh:"
  for artifact in \
