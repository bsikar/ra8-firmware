 ...
 * ``lx_nor_driver_ra_xspi_initialize`` that
 ...
 *     ``ra_xspi_flash_*`` byte-oriented HAL calls.
 *
 * Sector geometry:
 *
...
/**
 * @brief LevelX NOR driver-initialise entry point bridging onto ``ra_xspi``.
 *
 * @details
 ...
 * |-----------------------------------------|----------------------------------------------|
 * | ``lx_nor_flash_driver_read``            | ``ra_xspi_flash_read(0, off, buf, words*4)`` |
 * | ``lx_nor_flash_driver_write``           | ``ra_xspi_flash_program(0, off, buf, w*4)``  |
 * | ``lx_nor_flash_driver_block_erase``     | ``ra_xspi_flash_erase_sector(0, off)``       |
 ...
 * @pre ``ra_xspi_init(0, k_ra_xspi_lio_1s1s1s)`` has been called.
 *
