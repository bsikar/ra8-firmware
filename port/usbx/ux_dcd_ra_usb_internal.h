/**
 * @file port/usbx/ux_dcd_ra_usb_internal.h
 * @brief USBX device-controller-driver bridge to ra_usb -- per-module
 ...
 *
 * @details
 * The DCD bridge implementation is split across ux_dcd_ra_usb.c and
 * its per-aspect siblings (ux_dcd_ra_usb_{ep,xfer,isr,setup,dvst,
...
/**
 * @var s_dcd
 * @brief The single bridge instance, defined in ux_dcd_ra_usb.c.
 * @details RA8D2 has two USB controllers but the device stack only ever
 ...
 * @since 0.1.0
 */
extern ra_usb_dcd_t s_dcd;

/**
 * @var s_diag
 * @brief Bridge diagnostic counter block, defined in ux_dcd_ra_usb.c.
 * @note Single-writer per counter; JLink-readable.
 * @since 0.1.0
 */
extern ra_usb_dcd_diag_t s_diag;
 ...
 *        re-enable. Defined in ux_dcd_ra_usb.c.
 * @since 0.1.0
 */
 ...
 *        ux_dcd_ra_usb_xfer.c.
 * @since 0.1.0
 */
extern uint8_t s_orphan_buf[k_ra_usb_orphan_bytes];
 ...
 *        Defined in ux_dcd_ra_usb_setup.c.
 * @since 0.1.0
 */
 ...
 *        completed. Defined in ux_dcd_ra_usb_setup.c.
 * @since 0.1.0
 */
 ...
 * @details Defined in ``ux_dcd_ra_usb_xfer.c``; validates the incoming
 ...
 * @details Defined in ux_dcd_ra_usb.c; called from the ISR
 ...
 * @details Defined in ux_dcd_ra_usb_setup.c; packs
 ...
 * @since 0.1.0
 */
ra_elc_event_t internal_pick_event(ra_usb_speed_t speed);
 ...
 * @since 0.1.0
 */
ra_isr_handler_t internal_pick_isr(ra_usb_speed_t speed);

/**
 * @brief ra_usb_attach_handler trampoline into ::ux_dcd_ra_usb_irq.
 * @details Defined in ux_dcd_ra_usb_isr.c.
 * @param[in,out] ctx Unused ra_usb callback context.
 * @param[in] speed Which controller fired.
 * @param[in] status_mask INTSTS0 snapshot forwarded from ra_usb_dispatch.
 * @pre ::s_dcd is past ux_dcd_ra_usb_initialize.
 * @pre Caller is ra_usb_dispatch.
 * @post ::ux_dcd_ra_usb_irq has run for the snapshot.
 * @post No other state changes here.
 * @note Not thread-safe unless documented otherwise.
 * @since 0.1.0
 */
void internal_event_cb(void* ctx, ra_usb_speed_t speed, uint16_t status_mask);

/**
 * @brief Push a decoded SETUP packet into the USBX chapter-9 dispatcher.
 * @details Defined in ux_dcd_ra_usb_setup.c.
 ...
 * @since 0.1.0
 */
unsigned int internal_dispatch_setup(const ra_usb_setup_t* setup);
 ...
 * @since 0.1.0
 */
void internal_handle_ctrt(ra_usb_speed_t speed, uint16_t intsts0);
 ...
 * @since 0.1.0
 */
void internal_handle_ctrl_out_data(ra_usb_speed_t speed);

/**
 * @brief Process the DVSQ=Default branch of the DVST handler.
 * @details Defined in ux_dcd_ra_usb_dvst_default.c; drains the
 * persistent SETUP mirrors, dispatches a fresh SETUP if USBREQ changed,
 * then re-arms the DCP via ra_usb_device_busreset_rearm.
 * @param[in] speed Which controller fired.
 ...
 * @since 0.1.0
 */
void internal_dvst_default_state(ra_usb_speed_t speed);

/**
 * @brief Capture DVSTCTR0 / RHST history, mirror speed, dispatch DVST.
 * @details Defined in ux_dcd_ra_usb_dvst.c.
 * @param[in] speed Which controller fired (FS or HS).
 ...
 * @since 0.1.0
 */
void internal_irq_dvst_prelude(ra_usb_speed_t speed, uint16_t intsts0);
