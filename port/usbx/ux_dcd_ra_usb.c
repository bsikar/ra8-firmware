 ...
 * @brief USBX device-controller-driver bridge to ra_usb -- lifecycle + storm guard.
 *
 * @par Tag
 * [Ring 5 / PORT] {World: S}
 *
 ...
 * the lifecycle entry points (``ux_dcd_ra_usb_initialize`` /
 * ``_uninitialize`` / ``_state``), and the USBFS interrupt-storm guard.
 * The per-aspect siblings (``ux_dcd_ra_usb_{ep,xfer,isr,setup,dvst,dvst_default,irq}.c``)
 * share state and helpers through ``ux_dcd_ra_usb_internal.h``.
 *
