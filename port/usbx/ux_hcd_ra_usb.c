 ...
 *
 * @details Builds an ``ra_usb_setup_t`` from the USBX transfer request,
 * fires the SETUP token via ``ra_usb_host_setup_request``, then for the
 * optional data stage routes IN (D2H) through ``ra_usb_queue_out`` (chip
 * reads from device, fills caller buffer) and OUT (H2D) through
 * ``ra_usb_queue_in`` (chip writes caller buffer onto wire).
 *
 ...
 * @retval UX_TRANSFER_ERROR ``ra_usb_host_setup_request`` or queue failed.
 *
 * @pre ``tr`` is non-null and points to an EP0 transfer.
 * @pre HCD bridge is past ``ux_hcd_ra_usb_initialize``.
...
static unsigned int internal_control_xfer(struct UX_TRANSFER_STRUCT* tr)
{
  ...
  if (tr->ux_transfer_request_requested_length != 0U &&
    ...
    if ((setup.bm_request_type & 0x80U) != 0U) {
      ...
      if (ra_usb_queue_in(s_hcd.speed, 0U, tr->ux_transfer_request_data_pointer, len) != k_ra_ok) {
        return UX_TRANSFER_ERROR;
      }
      tr->ux_transfer_request_actual_length = len;
    }
 ...
 * fill the caller buffer via ``ra_usb_queue_out`` (the host-side OUT
 * direction is "data flowing out of the chip into caller memory"). OUT
 * endpoints write caller bytes to the device via ``ra_usb_queue_in``.
 *
 ...
 * trampoline both edges into ``ra_usb_host_bus_reset``.
 *
 ...
 *
 * @details Wraps ``ra_usb_host_set_uact`` so the dispatcher's
 ...
 * @retval UX_ERROR ``ra_usb_host_set_uact`` rejected the change.
 *
 * @pre HCD bridge is past ``ux_hcd_ra_usb_initialize``.
...
{
  if (ra_usb_host_set_uact(s_hcd.speed, enable) != k_ra_ok) {
    return UX_ERROR;
  }
  s_hcd.state = enable ? k_ux_hcd_ra_usb_state_active : k_ux_hcd_ra_usb_state_ready;
  return UX_SUCCESS;
}
 ...
 * ``ux_hcd_ra_usb_initialize``. The USBX host stack calls this with a
