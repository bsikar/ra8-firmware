/**
 * @file port/filex/fx_media_driver_ra_sdhi.h
 * @brief FileX media driver shim that bridges FileX onto the RA8D2 SDHI HAL
 *
 * @par Tag
 ...
 * ``fx_media_driver_ra_sdhi`` that ``fx_media_open`` /
 ...
 * request into the matching ``ra_sdhi_*`` block-driver call exported
 * from ``libs/ra_hal/inc/ra_sdhi.h``.
 *
 * Sector size is fixed at 512 bytes (the SD specification block size
 * the underlying ``ra_sdhi_read_block`` / ``ra_sdhi_write_block``
...
/**
 * @brief FileX media-driver entry point bridging onto ``ra_sdhi_*``.
 *
 * @details
 ...
 * |------------------------------|----------------------------------------|
 * | ``FX_DRIVER_INIT``           | ``ra_sdhi_init(0)``                    |
 * | ``FX_DRIVER_UNINIT``         | ``ra_sdhi_deinit(0)``                  |
 * | ``FX_DRIVER_READ``           | ``ra_sdhi_read_block(0, lba, buf, n)`` |
 * | ``FX_DRIVER_WRITE``          | ``ra_sdhi_write_block(0, lba, buf, n)``|
 * | ``FX_DRIVER_BOOT_READ``      | ``ra_sdhi_read_block(0, 0, buf, 1)``   |
 * | ``FX_DRIVER_BOOT_WRITE``     | ``ra_sdhi_write_block(0, 0, buf, 1)``  |
