/**
 * @file port/esp-hosted/tests/inc/ra8_esp_hosted_tx_shim_internal.h
 ...
 * host unit-test build (``RA8_OFF_TARGET``) links no ThreadX at all, so this
 * test-support header stands in for it. It is intentionally below ``tests/``
 * because target production builds include the real ``tx_api.h`` and must
 * never consume this model. Unlike the always-succeeding stub used by
...
// NOLINTBEGIN(readability-identifier-naming) -- ThreadX compatibility requires the vendor type spellings.
...
} TX_BYTE_POOL; // NOLINT(readability-identifier-naming) -- ThreadX compatibility requires this type spelling.
...
} TX_QUEUE; // NOLINT(readability-identifier-naming) -- ThreadX compatibility requires this type spelling.
...
} TX_SEMAPHORE; // NOLINT(readability-identifier-naming) -- ThreadX compatibility requires this type spelling.
...
} TX_MUTEX; // NOLINT(readability-identifier-naming) -- ThreadX compatibility requires this type spelling.
...
} TX_THREAD; // NOLINT(readability-identifier-naming) -- ThreadX compatibility requires this type spelling.
...
} TX_TIMER; // NOLINT(readability-identifier-naming) -- ThreadX compatibility requires this type spelling.
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static inline UINT internal_tx_byte_pool_create(
  TX_BYTE_POOL* pool_ptr,
  // NOLINTNEXTLINE(readability-non-const-parameter) -- ThreadX compatibility requires the vendor signature.
  CHAR* name_ptr,
  void* pool_start,
  ULONG pool_size)
{
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static inline UINT internal_tx_byte_pool_info_get(TX_BYTE_POOL*  pool_ptr,
                                                               CHAR**         name,
...
internal_tx_queue_send(TX_QUEUE* queue_ptr, void* source_ptr, ULONG wait_option)
{
  ...
  (void)memcpy(&queue_ptr->storage[(size_t)slot * queue_ptr->words],
               source_ptr,
...
internal_tx_queue_receive(TX_QUEUE* queue_ptr, void* destination_ptr, ULONG wait_option)
{
  ...
  (void)memcpy(destination_ptr,
               &queue_ptr->storage[(size_t)queue_ptr->head * queue_ptr->words],
               (size_t)queue_ptr->words * sizeof(ULONG));
 ...
 * @{
 */
// NOLINTBEGIN(readability-identifier-naming) -- ThreadX compatibility requires the vendor call spellings.
