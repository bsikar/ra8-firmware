/**
 * @file port/esp-hosted/inc/port_esp_hosted_host_uart.h
 * @brief esp-hosted port header: UART transport buffer size.
 *
 * @par Tag
 ...
 * chain evaluated inside shared vendored files, so the UART header must
 * exist with its upstream name and its one upstream macro.
 *
 ...
 * @brief Bytes framed in one UART transport packet.
 * @details Derived from the vendored ``ESP_TRANSPORT_UART_MAX_BUF_SIZE``
 * (1600). The UART transport carries the same esp-hosted payload header
 * as the SPI one, so the frame size matches; only the pacing differs.
 * @note Read-only build configuration.
 * @warning Only meaningful when ``H_TRANSPORT_IN_USE == H_TRANSPORT_UART``.
 * @par Example:
 * @code
