/**
 * @file examples/ra8p1_foundation/npu_infer/src/vector_table.c
 * @brief Cortex-M85 vector table + ra8_isr dispatching IRQ trampolines (RA8P1)
 *
 * @details
 ...
 * in blink/linker_script.ld defines them with this exact spelling.
 */
 ...
 * a driver that called ra8_isr_register (here: the NPU completion IRQ ->
 * ra8_npu_irq_handler) runs in NVIC handler-mode. */
 ...
 * (handler, ctx) registered through the substrate (ra8_isr_register) runs in
 * NVIC handler-mode. Slots that were never registered land in the dispatcher's
 * bounds-checked no-op path. The trampolines are weak so a driver can still
 * install a fully-custom IRQ handler by defining a non-weak ``IRQ<n>_Handler``.
...
/* On Apple host syntax-check we drop ra8_isr_dispatch (host build never branches
 * through these) and keep only a weak empty body. */
#ifndef __APPLE__
/** @brief RA8 IRQ STUB. */
