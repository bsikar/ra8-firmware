...
`ra_power_profile` (DTS-style enter/exit accumulator) across the
...
2. Enter SLEEP via `ra_lpm_enter_sleep(k_ra_sleep_mode_sleep)`;
...
Bare EK-RA8D2 only -- the profiler hooks into `ra_time_ms` for the
