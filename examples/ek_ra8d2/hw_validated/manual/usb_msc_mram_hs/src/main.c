/**
 * @file examples/ek_ra8d2/hw_validated/manual/usb_msc_mram_hs/src/main.c
 ...
 *
 * P4_08 = USBHS_VBUS sense (PSEL ``k_ra8_psel_usb_hs``; the only
...
{
  static CHAR s_thread_name[] = "usb_msc_mram_hs";
 ...
 * @retval k_ra8_ok VBUS routed and PD07 driven LOW (device role).
 *
...
{
  ra8_err_t err =
    ra8_pfs_route_peripheral(k_demo_pin_hs_vbus, k_ra8_psel_usb_hs, "usb_msc_mram_hs.vbus");
  if (err != k_ra8_ok) {
    return err;
  }
...
void main(void)
{
   ...
   * released inside ux_dcd_ra8_usb_initialize -- without it the UTMI
