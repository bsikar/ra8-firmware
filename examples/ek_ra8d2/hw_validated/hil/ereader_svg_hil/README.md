...
counterpart to `ereader_image_hil` (PNG) and `ereader_jpeg_hil` (JPEG), which
...
PNG/JPEG/BMP/GIF (`ereader_image_hil` #106, `ereader_jpeg_hil`), but SVG is a
...
(`ereader_cover_hil`), this completes the PNG/JPEG/SVG cover-art coverage of
#143.
...
```
make ereader_svg_hil
make -C examples/ek_ra8d2/hw_validated/hil/ereader_svg_hil flash
```
