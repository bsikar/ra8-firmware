/**
 * @file examples/ek_ra8d2/hw_validated/hil/pagecache_hil/main.c
 * @brief On-silicon HIL: ra_reflow pagination-cache round-trip on SD (#117).
 *
 * @par Tag
 * [Ring 6 / APP] {World: S}
 *
 * @details
 * #79 added the import-time pagination cache (`ra_reflow_cache`): lay a chapter
 ...
 * volume through `ra_fs` -- the exact storage the e-reader uses.
 *
 * Flow: bring up the Pmod2 microSD over `ra_sdmmc_spi`, mount an `ra_fs` volume
 * (formatting FAT32 if blank), self-provision `FONT.OTF` (a baked Latin-1 OTF),
 * then exercise the cache three ways:
 *   1. ROUND-TRIP: lay out a fixed chapter live, `ra_reflow_cache_serialize` it,
 *      write the blob to SD, read it back, `ra_reflow_cache_load` it into a fresh
 ...
 *      font size and assert `k_ra_err_invalid_state` (the key mismatch is caught;
...
static const ra_port_pin_t k_pc_pin_cipo = (ra_port_pin_t)k_ra_board_pmod2_spi_cipo;
static const ra_port_pin_t k_pc_pin_copi = (ra_port_pin_t)k_ra_board_pmod2_spi_copi;
static const ra_port_pin_t k_pc_pin_cs   = (ra_port_pin_t)k_ra_board_pmod2_spi_cs;

/** @brief Font + cache file paths on the SD volume (8.3, ra_fs is root-only). */
...
/* ---- SPI -> ra_sdmmc_spi transport adapter (mirror of epub_open_hil) ------ */
...
static ra_err_t pc_spi_set_clock(void* ctx, uint32_t hz)
{
  const uint32_t pclka_hz = *(const uint32_t*)ctx;
  return ra_sci_spi_set_clock((uint8_t)k_pc_spi_chan, hz, pclka_hz);
}

/** @brief ra_sdmmc_spi_transport_t::cs over ra_gpio (CS active-low). */
static ra_err_t pc_spi_cs(void* ctx, bool asserted)
{
  (void)ctx;
  return ra_gpio_write(k_pc_pin_cs, asserted ? k_ra_level_low : k_ra_level_high);
}

/** @brief ra_sdmmc_spi_transport_t::xfer over ra_sci_spi_xfer. */
static ra_err_t pc_spi_xfer(void* ctx, const uint8_t* tx, uint8_t* rx, uint32_t len)
{
  (void)ctx;
...
{
  ra_err_t err = ra_pfs_route_peripheral(k_pc_pin_sck, k_ra_psel_sci_async, "pc.sck");
  if (err != k_ra_ok) {
    return err;
  }
  err = ra_pfs_route_peripheral(k_pc_pin_cipo, k_ra_psel_sci_async, "pc.cipo");
  if (err != k_ra_ok) {
    return err;
  }
  err = ra_pfs_route_peripheral(k_pc_pin_copi, k_ra_psel_sci_async, "pc.copi");
  if (err != k_ra_ok) {
    return err;
  }
  return ra_gpio_output_init(k_pc_pin_cs, k_ra_level_high);
}
...
static void pc_setup_or_halt(uint32_t* out_pclka_hz)
{
  ...
  const ra_sci_spi_cfg_t spi_cfg = {.baud_hz   = (uint32_t)k_ra_sdmmc_spi_clock_init_hz,
                                    ...
                                    .lsb_first = false};
  if (ra_sci_spi_init((uint8_t)k_pc_spi_chan, &spi_cfg) != k_ra_ok) {
    pc_fail((uint32_t)k_pc_err_init);
  }
...
{
  const ra_sdmmc_spi_transport_t transport = {.set_clock = pc_spi_set_clock,
                                              .cs        = pc_spi_cs,
                                              .xfer      = pc_spi_xfer,
                                              .ctx       = pclka_hz};
...
static ra_fs_mount_t* pc_mount_or_halt(void)
{
 ...
 */
static void pc_font_or_halt(ra_fs_mount_t* mount, uint32_t* out_len)
{
  ...
  uint32_t got = 0U;
  if ((ra_fs_read(file, s_font_buf, (uint32_t)k_pc_font_cap, &got) != k_ra_ok) || (got == 0U)) {
...
static uint32_t pc_read_cache(ra_fs_mount_t* mount, uint8_t* buf, uint32_t cap)
{
  ra_fs_file_t* file = nullptr;
  if (ra_fs_open(mount, k_pc_cache_path, k_ra_fs_mode_read, &file) != k_ra_ok) {
    return 0U;
  }
  uint32_t got = 0U;
  if (ra_fs_read(file, buf, cap, &got) != k_ra_ok) {
    got = 0U;
  }
  (void)ra_fs_close(file);
  return got;
...
{
  return ra_reflow_init((uint16_t)k_pc_view_w,
...
static void pc_live_layout_or_halt(uint32_t font_len, size_t* out_n)
{
  ...
  g_pc_pages = pages;
  size_t n   = 0U;
  if (ra_reflow_cache_serialize(&s_engine,
                                (const uint8_t*)k_pc_body,
                                (size_t)(sizeof(k_pc_body) - 1U),
                                s_live_blob,
...
static void pc_persist_and_verify_or_halt(ra_fs_mount_t* mount, size_t n)
{
  if (ra_fs_write_file(mount, k_pc_cache_path, s_live_blob, (uint32_t)n) != k_ra_ok) {
...
static void pc_reload_check_or_halt(uint32_t font_len, size_t n)
{
  ...
  if (ra_reflow_cache_load(&s_engine,
                           (const uint8_t*)k_pc_body,
                           (size_t)(sizeof(k_pc_body) - 1U),
                           s_read_blob,
                           n) != k_ra_ok) {
...
int32_t main(void)
{
  ...
  ra_fs_mount_t* const mount = pc_mount_or_halt();
   ...
   * in WFI without bumping it -- so a memprobe gate proves the full SD -> ra_fs
   * -> ra_reflow_cache round-trip + invalidation ran clean. */
  while (1) {
    ra_delay_ms(k_pc_frame_ms);
    g_pc_heartbeat++;
  }
}
