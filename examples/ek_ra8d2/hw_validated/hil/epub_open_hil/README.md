...
it, on the target (issue #114). `epub_parse_hil` (#139) proved the parser runs
...
```sh
make epub_open_hil                         # cross-compile -> build/epub_open_hil.elf
make -C examples/epub_open_hil flash       # flash via on-board J-Link OB
```
...
```sh
./tools/board_sim/build/board_sim build/epub_open_hil.elf --sd-new 64:fat32 \
