# ra_io_cache_demo -- caching block device (Phase 5, #160)

A single self-contained app that drives the `ra_io` caching block device with no
external hardware, so it runs headlessly in `board_sim`.

The caching block device is a decorator: it wraps any other `ra_io_blockdev_t`
...
2. **Cache decorator (#160):** the backend is wrapped with
   `ra_io_blockdev_cache_init` over 32 cached sectors (16 KiB), all in
   caller-owned SRAM (zero allocation).
3. **Filesystem over the cache (Phase 3, #158):** the *cached* device is bridged
   to `ra_fs` (`ra_io_blockdev_as_fs_backend`), formatted + mounted as FAT12, and
...
4. **Re-read workload:** a file is written, then read back eight times through
   ...
   as hits.
5. **Hit/miss observability:** `ra_io_blockdev_cache_stats` reports the counters,
   and the run requires a non-zero hit count.

## Build

```
make            # -> build/ra_io_cache_demo.elf
```

## Run in the simulator

```
BOARD_SIM_WALL_S=12 tools/board_sim/build/board_sim build/ra_io_cache_demo.elf
```

Expected console output (exact hit/miss counts depend on FAT geometry):

```
[uart] SCI8: ra_io_cache_demo: boot
[uart] SCI8: ra_io_cache_demo: re-read x8 hits=H misses=M ram:/HELLO.TXT PASS
```
