...
`_GIF`, `_BMP` in `stb_image_impl.c`). PNG (`ereader_image_hil`, #106) and JPEG
(`ereader_jpeg_hil`) already had a gate; this app exercises the remaining two:
...
Together with the EPUB cover-extraction (`ereader_cover_hil`) and SVG vector
(`ereader_svg_hil`) gates, this rounds out the cover-art image-decode family of
#143.

## Validation

Deterministic, so the board_sim CRC gate is the regression net (the same way
`ereader_image_hil` / `ereader_jpeg_hil` gate). Run on `tools/board_sim` (the
...
```
cd examples/ek_ra8d2/hw_validated/hil/ereader_imgfmt_hil
...
```
make ereader_imgfmt_hil
make -C examples/ek_ra8d2/hw_validated/hil/ereader_imgfmt_hil flash
```
