 ...
 * hardware. With RA_ENABLE_ROOT_OF_TRUST, ::ra_dfu_launch verifies an image's
 ...
 * 7) and:
 *   1. Flips one body byte and hands the corrupted copy to ::ra_dfu_launch --
 ...
 *   2. Hands the genuine signed image to ::ra_dfu_launch -- signature +
 *      anti-rollback pass, so it copies to k_ra_dfu_run_base and BRANCHES there
...
static void sb_setup_or_halt(void)
{
  ...
  if (ra_board_uart_console_init((uint32_t)k_sb_baud) != k_ra_ok) {
    sb_panic_halt();
  }
 ...
 * @return true if ::ra_dfu_launch returned (default-deny) without launching.
 */
static bool sb_tampered_rejected(void)
{
  ...
  /* A rejected image makes ra_dfu_launch return without copying or branching.
  ...
  ra_dfu_launch((uintptr_t)s_sb_tampered,
...
int32_t main(void)
{
   ...
   * copied to the run base and branched to -- ra_dfu_launch does not return. */
  sb_write(k_sb_msg_enforcing, (size_t)(sizeof(k_sb_msg_enforcing) - 1U));
  ra_dfu_launch((uintptr_t)g_sb_signed_payload,
                (uint32_t)k_sb_payload_body_len,
                (uint32_t)k_ra_dfu_run_base);
