...
(`ns_main.c` + `ns_usb.c` + ThreadX + USBX + `ra_usb`), merged into one
...
- **USBFS (J11) = CDC-ACM DEVICE** -- USBX device stack + the `ux_dcd_ra_usb`
  bridge, driven by polling `ra_usb_dispatch` (compiled `RA_USB_POLLED_ONLY`, so
...
- **USBHS (J7) = polled HOST** -- the first-party `ra_usb_host_*` primitives.
...
| `g_tz_nsc_cgc_usb_init_step` | `4` | all three NSC CGC veneers returned `k_ra_ok` |
...
1. **The RA8 IDAU is fixed by address bit[28]** (HUM s51.3.3.1): real NS lives
   ...
   brick risk. `ns_usb.c`/`ra_usb`/`ra_mstp` reach USBFS/USBHS through the NS
   aliases `0x5025_0000` / `0x5035_0000` (`-DRA_PERIPH_NS_ALIAS`).
2. **The Secure side delegates both controllers before BLXNS**
   ...
   (`ra_cgc_usbhs_pll_enable` -- CGC is Secure-only; the 48 MHz USBFS clock comes
...
3. **No USB IRQ** (`-DRA_USB_POLLED_ONLY`) -- both stacks are polled, so the
   Secure-attributed ICU/NVIC is never touched from NS. `ra_time.c` is dropped
   from the NS link (it would fight ThreadX's SysTick); `ns_usb.c` supplies a
   ThreadX-backed `ra_delay_ms` / `ra_time_ms`.
...
RIIC1's BBSY can still be set and the write reports `k_ra_err_busy`. The external
