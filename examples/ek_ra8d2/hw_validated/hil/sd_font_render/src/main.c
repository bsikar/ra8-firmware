/**
 * @file examples/ek_ra8d2/hw_validated/hil/sd_font_render/src/main.c
 * @brief Load a TTF/OTF font off an SD card and render text with reflow.
 *
 * @par Tag
 * [Ring 6 / APP] {World: S}
 *
 ...
 * reads it over SPI, @ref ra8_fs serves it as a file, and @ref reflow
 * rasterises a paragraph of XHTML into the GLCDC framebuffer through
 * @ref ra8_gfx. The same pipeline is covered on host by
 * @c apps/shared_libs/reflow/tests/src/test_ra8_sdmmc_card_reflow.c; this app promotes it to a
...
typedef enum : uint32_t {
  ...
  k_sfr_stage_layout_ok   = 5U,    /**< reflow laid out the chapter.   */
 ...
 * exercising the full SD-font -> ra8_fs -> reflow -> framebuffer path. */
...
/** @brief Pages produced by reflow_layout_chapter. */
...
{
  if (reflow_init((uint16_t)k_sfr_fb_w,
                  (uint16_t)k_sfr_fb_h,
                  s_font_buf,
                  g_sfr_font_len,
  ...
  if (reflow_render_page(&s_engine, 0U, nullptr) != k_ra8_ok) {
    sfr_panic_halt(k_sfr_stage_reflow_fail);
  }
