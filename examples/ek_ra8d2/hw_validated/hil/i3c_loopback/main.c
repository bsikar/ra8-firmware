 ...
 * @brief I3C (ra_i3c) controller bring-up self-test for the EK-RA8D2
 *
 * @par Tag
 * [Ring 6 / APP] {World: S}
 *
 * @details
 * Standalone EVM-tier app that exercises the ra_i3c controller driver in
 * I2C-compatibility mode (``libs/ra_hal/inc/ra_i3c.h``) on channel 0.
 ...
 * So this is a controller self-test: it proves ra_i3c powers up and
 ...
 *         a completed scan (k_ra_ok) still proves the controller clocks. */
...
static void i3c_demo_pfs_or_halt(void)
{
  ...
  if (ra_pfs_route_peripheral(k_i3c_demo_pin_sda, k_ra_psel_iic, "i3c_loopback.sda0") != k_ra_ok) {
    i3c_demo_panic_halt();
  }
...
static void i3c_demo_setup_or_halt(void)
{
  ...
  if (ra_i3c_init((uint8_t)k_i3c_demo_iic_channel, &iic_cfg) != k_ra_ok) {
    i3c_demo_panic_halt();
  }
...
int32_t main(void)
{
  ...
  while (1) {
    bool           acked = false;
    const ra_err_t err =
      ra_i3c_scan((uint8_t)k_i3c_demo_iic_channel, (uint8_t)k_i3c_demo_probe_addr, &acked);
    ...
    ra_delay_ms(k_i3c_demo_period_ms);
  }
