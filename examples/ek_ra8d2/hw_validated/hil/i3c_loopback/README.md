...
I3C (`ra8_i3c`) controller bring-up self-test for the EK-RA8D2, run in
...
pull-ups), so `ra8_i3c_scan` cannot ACK -- `ack=0` / "bus idle" is the
EXPECTED result and is not a failure. Reaching the scan loop at all
proves `ra8_i3c_init` configured the controller (an init failure
...
`i2c_loopback` -- the RIIC (`ra8_i2c`) controller against U15.
