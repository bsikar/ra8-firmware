/**
 * @file examples/ek_ra8d2/hw_validated/hil/dfu_selftest_hs_host/vector_table.c
...
void Reset_Handler(void)
{
  ...
  /* Step 3b: copy the SRAM ram-functions (ra_flash + ra_dfu_program) from
   * their MRAM load image into SRAM, so the code-MRAM program loop never
   * executes from the MRAM it is programming (HUM Ch 59 / ra_flash.h). */
  src = &g_ra_ls_sram_text_load;
  dst = &g_ra_ls_ssram_text;
  while (dst < &g_ra_ls_esram_text) {
    *dst++ = *src++;
  }
