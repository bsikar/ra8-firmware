...
This app, building on `epub_open_hil` (#114), exercises both forms plus
...
```sh
make epub_toc_hil                          # cross-compile -> build/epub_toc_hil.elf
make -C examples/epub_toc_hil flash        # flash via on-board J-Link OB
```
...
./tools/board_sim/build/board_sim build/epub_toc_hil.elf --sd-new 64:fat32 \
...
reason `epub_open_hil` / `sd_font_render` / `fs_format_mount` gate on SWD
