/**
 * @file examples/ek_ra8d2/hw_validated/hil/epub_toc_hil/main.c
 ...
 *
 * @details
 * #74 added titled-TOC parsing to `ra_epub` -- both the EPUB2 NCX (`<navMap>`)
 * and the EPUB3 `nav.xhtml` (`<nav epub:type="toc">`) forms -- but it has only
 * ever run on the x86 host. This app, building on `epub_open_hil` (#114), runs
 ...
 * (the same reason the sibling SD HIL apps -- `epub_open_hil`, `sd_font_render`,
...
static const ra_port_pin_t k_etoc_pin_cipo = (ra_port_pin_t)k_ra_board_pmod2_spi_cipo;
...
static ra_err_t etoc_spi_set_clock(void* ctx, uint32_t hz)
{
  const uint32_t pclka_hz = *(const uint32_t*)ctx;
  return ra_sci_spi_set_clock((uint8_t)k_etoc_spi_chan, hz, pclka_hz);
}

/** @brief ra_sdmmc_spi_transport_t::cs over ra_gpio (CS active-low). */
static ra_err_t etoc_spi_cs(void* ctx, bool asserted)
{
  (void)ctx;
  return ra_gpio_write(k_etoc_pin_cs, asserted ? k_ra_level_low : k_ra_level_high);
}

/** @brief ra_sdmmc_spi_transport_t::xfer over ra_sci_spi_xfer. */
static ra_err_t etoc_spi_xfer(void* ctx, const uint8_t* tx, uint8_t* rx, uint32_t len)
{
  (void)ctx;
...
{
  ra_err_t err = ra_pfs_route_peripheral(k_etoc_pin_sck, k_ra_psel_sci_async, "etoc.sck");
  if (err != k_ra_ok) {
    return err;
  }
  err = ra_pfs_route_peripheral(k_etoc_pin_cipo, k_ra_psel_sci_async, "etoc.cipo");
  if (err != k_ra_ok) {
    return err;
  }
  err = ra_pfs_route_peripheral(k_etoc_pin_copi, k_ra_psel_sci_async, "etoc.copi");
  if (err != k_ra_ok) {
    return err;
  }
  return ra_gpio_output_init(k_etoc_pin_cs, k_ra_level_high);
}
...
static void etoc_setup_or_halt(uint32_t* out_pclka_hz)
{
  ...
  const ra_sci_spi_cfg_t spi_cfg = {.baud_hz   = (uint32_t)k_ra_sdmmc_spi_clock_init_hz,
                                    ...
                                    .lsb_first = false};
  if (ra_sci_spi_init((uint8_t)k_etoc_spi_chan, &spi_cfg) != k_ra_ok) {
    etoc_fail((uint32_t)k_etoc_err_init, k_msg_finit, (uint32_t)sizeof(k_msg_finit) - 1U);
  }
...
{
  const ra_sdmmc_spi_transport_t transport = {.set_clock = etoc_spi_set_clock,
                                              .cs        = etoc_spi_cs,
                                              .xfer      = etoc_spi_xfer,
                                              .ctx       = pclka_hz};
...
static ra_fs_mount_t* etoc_mount_or_halt(void)
{
...
static void
etoc_provision_one(ra_fs_mount_t* mount, const char* path, const uint8_t* data, uint32_t len)
{
  ra_fs_file_t* file = nullptr;
  if (ra_fs_open(mount, path, k_ra_fs_mode_read, &file) == k_ra_ok) {
    (void)ra_fs_close(file); /* already present -- nothing to provision. */
    return;
  }
  if (ra_fs_write_file(mount, path, data, len) != k_ra_ok) {
...
static void etoc_provision_or_halt(ra_fs_mount_t* mount)
{
 ...
 * @param[out] kind    Receives `ra_epub_get_toc_kind`.
 * @param[out] count   Receives `ra_epub_get_toc_count`.
 ...
 */
static void etoc_read_toc(ra_fs_mount_t*     mount,
                          const char*        path,
                          uint32_t           stage,
                          const uint8_t*     fmsg,
                          uint32_t           fmsglen,
...
{
  if (ra_epub_open_fs(mount, path, s_epub_buf, (size_t)k_etoc_epub_cap, &s_book) != k_ra_ok) {
    etoc_fail(stage, fmsg, fmsglen);
  }
  uint8_t  k = 0U;
  uint16_t n = 0U;
  if ((ra_epub_get_toc_kind(&s_book, &k) != k_ra_ok) ||
      (ra_epub_get_toc_count(&s_book, &n) != k_ra_ok)) {
    etoc_fail(stage, fmsg, fmsglen);
  }
  *kind  = (uint32_t)k;
  *count = (uint32_t)n;
  ...
  *ch0   = (uint32_t)k_etoc_no_chap;
  if (n > 0U) {
    ...
    if ((ra_epub_get_toc_entry(&s_book, 0U, &entry) != k_ra_ok) ||
        (ra_epub_toc_entry_to_chapter(&s_book, 0U, &chap) != k_ra_ok)) {
      etoc_fail(stage, fmsg, fmsglen);
    }
...
static void etoc_check_ncx(ra_fs_mount_t* mount)
{
  etoc_read_toc(mount,
                k_etoc_path_ncx,
...
static void etoc_check_nav(ra_fs_mount_t* mount)
{
  etoc_read_toc(mount,
                k_etoc_path_nav,
 ...
 * spine is still usable (`ra_epub_get_chapter_count` == 2) so a renderer degrades
 * to the spine list rather than faulting.
 */
static void etoc_check_bad(ra_fs_mount_t* mount)
{
  if (ra_epub_open_fs(mount, k_etoc_path_bad, s_epub_buf, (size_t)k_etoc_epub_cap, &s_book) !=
      k_ra_ok) {
    etoc_fail((uint32_t)k_etoc_err_bad, k_msg_fbad, (uint32_t)sizeof(k_msg_fbad) - 1U);
  }
  uint8_t  kind = (uint8_t)k_etoc_kind_unset;
  uint16_t chap = 0U;
  if ((ra_epub_get_toc_kind(&s_book, &kind) != k_ra_ok) ||
      (ra_epub_get_chapter_count(&s_book, &chap) != k_ra_ok)) {
    etoc_fail((uint32_t)k_etoc_err_bad, k_msg_fbad, (uint32_t)sizeof(k_msg_fbad) - 1U);
  }
...
int32_t main(void)
{
  ...
  ra_fs_mount_t* const mount = etoc_mount_or_halt();
