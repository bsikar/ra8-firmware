...
`ereader_image_hil` (#106) and `ereader_jpeg_hil` (#143) prove the bare
...
is the EPUB analogue of `ereader_image_hil` -- same deterministic CRC gate, with
...
`ereader_image_hil` / `ereader_jpeg_hil` / `epub_parse_hil` gate). Run on
...
(`epub_parse_hil` #139, `ereader_image_hil` #106); this app composes them, so
the board_sim CRC gate plus those on-silicon precedents cover it end to end.

## Regenerating the fixture

```
cd examples/ek_ra8d2/hw_validated/hil/ereader_cover_hil
...
```
make ereader_cover_hil
make -C examples/ek_ra8d2/hw_validated/hil/ereader_cover_hil flash
```
