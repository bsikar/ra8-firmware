/**
 * @file examples/ek_ra8d2/hw_validated/hil/ereader_rabook/src/main.c
 ...
 * compiled by tools/epub_compile into the `book` format and baked (inflated)
 * into rabook_fixture.h, is loaded the way the device loads any book:
 *
 *   1. `book_validate()` -- accept the flat blob (magic, bounds, CRC). A real
 *      SD-loaded book is `book_open()`-ed instead (mz_uncompress + validate);
 *      see README.md. The baked fixture is already inflated so the gate needs no
 *      decompressor.
 *   2. For each chapter, `book_chapter_to_xhtml()` walks the pre-parsed DOM
 *      and bridges it to the existing renderer.
 *   3. `reflow_layout_chapter()` paginates, then every page is rendered into
...
{
  size_t xlen = 0U;
  if (book_chapter_to_xhtml(book, chapter_idx, s_xhtml, (size_t)k_erb_xhtml_cap, &xlen) !=
      k_ra8_ok) {
    erb_panic_halt(k_msg_berr, (uint32_t)sizeof(k_msg_berr) - 1U);
  }
  uint32_t pages = 0U;
  if (reflow_layout_chapter(&s_engine, (const uint8_t*)s_xhtml, (uint32_t)xlen, &pages) !=
      k_ra8_ok) {
...
static void erb_render_image(void)
{
  ...
  if (book_validate(img_book, (size_t)k_rabook_gray8_fixture_len) != k_ra8_ok) {
    erb_panic_halt(k_msg_berr, (uint32_t)sizeof(k_msg_berr) - 1U);
  }
  const book_header_t* hdr = book_header(img_book);
  if (hdr->image_count == 0U) {
    erb_panic_halt(k_msg_berr, (uint32_t)sizeof(k_msg_berr) - 1U);
  }
  const book_image_t* im  = &book_images(img_book)[0];
  ...
  if ((im->format != (uint8_t)k_book_image_gray4) ||
      (book_image_pixfmt(im) != k_book_pixfmt_gray8) || (im->raw_size != npx) || (npx == 0U) ||
      (npx > (uint32_t)k_erb_img_max_px)) {
    erb_panic_halt(k_msg_berr, (uint32_t)sizeof(k_msg_berr) - 1U);
  }
  ...
  const uint8_t* px = book_image_data(img_book, im);
...
void main(void)
{
  ...
  if (book_validate(book, (size_t)k_rabook_fixture_len) != k_ra8_ok) {
    erb_panic_halt(k_msg_berr, (uint32_t)sizeof(k_msg_berr) - 1U);
  }
  const uint32_t chapters = book_header(book)->chapter_count;
  ...
  if (reflow_init((uint16_t)k_erb_fb_w,
                  (uint16_t)k_erb_fb_h,
                  k_ahem_ttf,
