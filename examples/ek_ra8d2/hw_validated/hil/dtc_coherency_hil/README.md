...
boots, drives the real `ra_dtc` + ELC path, and reports PASS with the cache + MPU
...
where the engine cannot see them. `ra_dtc_enable()` was wired (commit `b80c247b`)
to `ra_cache_dcache_clean_by_addr()` the caller-populated vector table back to RAM
...
with `RA_BOOT_ENABLE_CACHE_MPU` defined for this app, so it programmes the MPU and
...
1. `ra_cgc_init` + `ra_mstp_init` + `ra_isr_init` + `ra_elc_init` + SCI8 console + LEDs.
2. `ra_dtc_init(s_dtc_vt)` programmes `DTCVBR`; `ra_isr_register` allocates an
...
5. Clean `s_src` + `s_dtc_ti`; `ra_dtc_enable()` (HAL cleans the vector table,
...
`ra_isr_globals_enable`): the registered slot + `DTCE` still activate the DTC, the
...
`ra_dtc` + ELC path runs in sim and the copy verifies. board_sim's memory is
