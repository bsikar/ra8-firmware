 ...
 *   6. ``ra_lpm_enter_sleep(k_ra_sleep_mode_deep_standby_2)``.
 *
...
typedef enum : uint32_t {
  k_lpm_dpsby2_baud        = 115200U,
  k_lpm_dpsby2_sci_channel = 8U,
} lpm_dpsby2_const_t;
...
typedef enum : uint8_t {
  ...
  k_lpm_dpsby2_seed_month     = 1U,
  k_lpm_dpsby2_seed_day       = 1U,
  k_lpm_dpsby2_secs_per_min   = 60U,
  k_lpm_dpsby2_mins_per_hour  = 60U,
  k_lpm_dpsby2_hours_per_day  = 24U,
} lpm_dpsby2_byte_t;

/** @brief SCI8 pinout on the EK-RA8D2 J-Link CDC channel. */
static const ra_port_pin_t k_lpm_dpsby2_pin_txd =
  (ra_port_pin_t)(((uint16_t)k_ra_port_13 << 8) | (uint16_t)k_ra_pin_2);
static const ra_port_pin_t k_lpm_dpsby2_pin_rxd =
...
[[nodiscard]] static ra_err_t lpm_dpsby2_pins_init(void)
{
  ra_err_t err =
    ra_pfs_route_peripheral(k_lpm_dpsby2_pin_txd, k_ra_psel_sci_async, "lpm_dpsby2.txd8");
  if (err != k_ra_ok) {
    return err;
  }
  return ra_pfs_route_peripheral(k_lpm_dpsby2_pin_rxd, k_ra_psel_sci_async, "lpm_dpsby2.rxd8");
}
 ...
 * @since 0.1.0
 */
static void lpm_dpsby2_setup_or_halt(uint32_t* out_pclka_hz)
{
  ...
  if (lpm_dpsby2_pins_init() != k_ra_ok) {
    lpm_dpsby2_panic_halt();
  }
  const ra_sci_cfg_t sci_cfg = {
    .baud      = k_lpm_dpsby2_baud,
    .data_bits = k_ra_sci_data_8,
    .parity    = k_ra_sci_parity_none,
    .stop_bits = k_ra_sci_stop_1,
    .pclk_hz   = *out_pclka_hz,
  };
  if (ra_sci_init((uint8_t)k_lpm_dpsby2_sci_channel, &sci_cfg) != k_ra_ok) {
    lpm_dpsby2_panic_halt();
  }
  ...
  const ra_rtc_datetime_t seed = {
    .year    = (uint16_t)(2000U + (uint16_t)k_lpm_dpsby2_seed_year_lo),
 ...
 * @since 0.1.0
 */
static void lpm_dpsby2_add_offset(const ra_rtc_datetime_t* now, ra_rtc_datetime_t* out)
{
  *out                = *now;
 ...
 * @since 0.1.0
 */
[[nodiscard]] static ra_err_t lpm_dpsby2_arm_wake(const ra_rtc_datetime_t* now)
{
  ra_rtc_datetime_t a = {};
...
int32_t main(void)
{
  ...
  lpm_dpsby2_setup_or_halt(&pclka_hz);
  (void)pclka_hz;
  ...
  (void)ra_sci_write_polling((uint8_t)k_lpm_dpsby2_sci_channel,
                             k_lpm_dpsby2_boot_msg,
                             (uint32_t)(sizeof(k_lpm_dpsby2_boot_msg) - 1U));
  ...
  while (1) {
    ...
    if (lpm_dpsby2_arm_wake(&now) != k_ra_ok) {
      break;
    }
     ...
     * iterates. */
    if (ra_lpm_enter_sleep(k_ra_sleep_mode_deep_standby_2) != k_ra_ok) {
      break;
    }
  }
  lpm_dpsby2_panic_halt();
