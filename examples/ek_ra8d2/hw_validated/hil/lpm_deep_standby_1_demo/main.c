/**
 * @file examples/ek_ra8d2/lpm_deep_standby_1_demo/main.c
 * @brief Deep Software Standby 1 (LPSCR.LPMD = 0x8) entry demo
 *
 * @par Tag
 * [Ring 6 / APP] {World: S}
 *
 * @details
 * Demonstrates Deep Software Standby variant 1 (LPSCR.LPMD = 0x8 per
 ...
 * running (HUM Ch 11.1 Table 11.3 p 431..432). Variants 2 and 3 (in
 * sibling demos) progressively turn more domains off.
 *
 * Boot flow:
 *   1. CGC + SysTick + UART (SCI8) bring-up.
 *   2. Emit boot banner ``"lpm_dpsby1: boot\r\n"`` -- HIL gate.
...
static const ra_port_pin_t k_lpm_dpsby1_pin_txd =
  (ra_port_pin_t)(((uint16_t)k_ra_port_13 << 8) | (uint16_t)k_ra_pin_2);
static const ra_port_pin_t k_lpm_dpsby1_pin_rxd =
...
[[nodiscard]] static ra_err_t lpm_dpsby1_pins_init(void)
{
  ra_err_t err =
    ra_pfs_route_peripheral(k_lpm_dpsby1_pin_txd, k_ra_psel_sci_async, "lpm_dpsby1.txd8");
  if (err != k_ra_ok) {
    return err;
  }
  return ra_pfs_route_peripheral(k_lpm_dpsby1_pin_rxd, k_ra_psel_sci_async, "lpm_dpsby1.rxd8");
}
 ...
 * @since 0.1.0
 */
static void lpm_dpsby1_setup_or_halt(uint32_t* out_pclka_hz)
{
  ...
  if (lpm_dpsby1_pins_init() != k_ra_ok) {
    lpm_dpsby1_panic_halt();
  }
  const ra_sci_cfg_t sci_cfg = {
    .baud      = k_lpm_dpsby1_baud,
    .data_bits = k_ra_sci_data_8,
    .parity    = k_ra_sci_parity_none,
    .stop_bits = k_ra_sci_stop_1,
    .pclk_hz   = *out_pclka_hz,
  };
  if (ra_sci_init((uint8_t)k_lpm_dpsby1_sci_channel, &sci_cfg) != k_ra_ok) {
    lpm_dpsby1_panic_halt();
  }
  ...
  const ra_rtc_datetime_t seed = {
    .year    = (uint16_t)(2000U + (uint16_t)k_lpm_dpsby1_seed_year_lo),
...
static void lpm_dpsby1_add_offset(const ra_rtc_datetime_t* now, ra_rtc_datetime_t* out)
{
  *out                = *now;
  ...
  out->second         = (uint8_t)(s % (uint16_t)k_lpm_dpsby1_secs_per_min);
  const uint16_t carm = (uint16_t)(s / (uint16_t)k_lpm_dpsby1_secs_per_min);
  const uint16_t m    = (uint16_t)now->minute + carm;
  out->minute         = (uint8_t)(m % (uint16_t)k_lpm_dpsby1_mins_per_hour);
  const uint16_t carh = (uint16_t)(m / (uint16_t)k_lpm_dpsby1_mins_per_hour);
  const uint16_t h    = (uint16_t)now->hour + carh;
  out->hour           = (uint8_t)(h % (uint16_t)k_lpm_dpsby1_hours_per_day);
}
...
int32_t main(void)
{
  ...
  lpm_dpsby1_setup_or_halt(&pclka_hz);
  (void)pclka_hz;
  ...
  (void)ra_sci_write_polling((uint8_t)k_lpm_dpsby1_sci_channel,
                             k_lpm_dpsby1_boot_msg,
                             (uint32_t)(sizeof(k_lpm_dpsby1_boot_msg) - 1U));
