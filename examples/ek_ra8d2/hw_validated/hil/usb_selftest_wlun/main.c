/**
 * @file examples/ek_ra8d2/hw_validated/hil/usb_selftest_wlun/main.c
...
{
  if (ra_pfs_route_peripheral(k_wlun_pin_fs_vbus, k_ra_psel_usb_fs, "wlun.fs_vbus") != k_ra_ok) {
    wlun_panic_halt();
  }
  if (ra_gpio_output_init(k_wlun_pin_fs_vbusen, k_ra_level_low) != k_ra_ok) {
    wlun_panic_halt();
  }
  if (ra_pfs_route_peripheral(k_wlun_pin_fs_dp, k_ra_psel_usb_fs, "wlun.fs_dp") != k_ra_ok) {
    wlun_panic_halt();
  }
  if (ra_pfs_route_peripheral(k_wlun_pin_fs_dm, k_ra_psel_usb_fs, "wlun.fs_dm") != k_ra_ok) {
    wlun_panic_halt();
  }
  if (ra_board_io_expander_set_usbhs_host_mode() != k_ra_ok) {
    wlun_panic_halt();
  }
  if (ra_gpio_output_init(k_wlun_pin_hs_pwr, k_ra_level_high) != k_ra_ok) {
    wlun_panic_halt();
  }
  if (ra_pfs_route_peripheral(k_wlun_pin_hs_vbus, k_ra_psel_usb_hs, "wlun.hs_vbus") != k_ra_ok) {
    wlun_panic_halt();
  }
}
...
static void wlun_setup_or_halt(void)
{
  ...
  if (ra_pfs_route_peripheral(k_wlun_pin_sci_tx, k_ra_psel_sci_async, "wlun.txd8") != k_ra_ok) {
    wlun_panic_halt();
  }
  if (ra_pfs_route_peripheral(k_wlun_pin_sci_rx, k_ra_psel_sci_async, "wlun.rxd8") != k_ra_ok) {
    wlun_panic_halt();
  }
  ...
  if (ra_sci_init((uint8_t)k_wlun_sci_channel, &sci_cfg) != k_ra_ok) {
    wlun_panic_halt();
  }
