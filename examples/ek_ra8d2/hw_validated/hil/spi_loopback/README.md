...
SPI_B internal-loopback HIL test for the EK-RA8D2. Brings up
`ra8_spi` on channel 0 at 1 MHz mode-0 MSB-first, then stamps
`SPCR2.SPLP = 1` (HUM Ch 43.2.5 p 2889 -- `k_ra8_spcr2_mask_splp`)
...
`ra8_spi_xfer8` and verifies RX matches TX byte-for-byte. SCI8 prints
