/**
 * @file examples/ek_ra8d2/hw_validated/hil/agt_pulse_demo/src/main.c
 ...
 * covered in `tests/misc/src/test_ra8_agt.c`.
 *
...
RA8_INTERNAL static void internal_format_tick(uint32_t tick, uint8_t* digits)
{
  for (uint32_t i = 0U; i < (uint32_t)k_agt_pulse_hex_digits; ++i) {
    ...
    digits[i]          = (uint8_t)s_agt_pulse_hex_chars[nib];
  }
}
...
[[nodiscard]] RA8_INTERNAL static ra8_err_t internal_emit_tick(uint32_t tick)
{
  ...
  return ra8_board_uart_console_write(s_agt_pulse_msg_tail,
                                      (size_t)(sizeof(s_agt_pulse_msg_tail) - 1U));
}
