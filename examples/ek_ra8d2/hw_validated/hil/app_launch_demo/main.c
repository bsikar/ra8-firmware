 ...
 * `ra_app` registry + launcher + the navigation back-stack (`ra_app_nav_t`)
 ...
 *   2. Launches `reader` from the chrome through `ra_app_nav_go` (first focus,
 *      nothing pushed) and renders it.
 *   3. Switches to `settings` through `ra_app_nav_go` -- the focus lifecycle
 *      fires (`reader.on_leave` -> `settings.on_enter`) and `reader` is pushed
 *      onto the back-stack.
 *   4. Presses "back" through `ra_app_nav_back` -- `settings` leaves, `reader`
...
static bool app_launch_register(void)
{
  ...
  s_reader_app = (ra_app_t){.vt        = &k_app_stub_vt,
                            .ctx       = &s_reader_ctx,
                            .id        = (uint16_t)k_app_id_reader,
                            .name      = "reader",
                            .removable = false};
  bool ok      = (ra_app_register(&s_reg, &s_reader_app) == k_ra_ok);
#if APP_LAUNCH_SETTINGS
  s_settings_app = (ra_app_t){.vt        = &k_app_stub_vt,
                              .ctx       = &s_settings_ctx,
                              .id        = (uint16_t)k_app_id_settings,
                              .name      = "settings",
                              .removable = true};
  if (ok) {
    ok = (ra_app_register(&s_reg, &s_settings_app) == k_ra_ok);
  }
#endif
  return ok;
}
 ...
 *
 * @details
 * The first `ra_app_nav_go` has no prior focus, so it pushes nothing: the
 ...
 *
 * @details
 * Proves the "event" leg of the vtable: `ra_app_route_input` delivers the event
 ...
 *
 * @details
 * `ra_app_nav_go(settings)` fires `reader.on_leave` then `settings.on_enter`
 * and pushes `reader` (depth 1). `ra_app_nav_back` pops it: `settings.on_leave`
...
int32_t main(void)
{
  ...
  if (!ok) {
    ra_log_info("app_launch", "FAIL register");
  }
  if (ok) {
    ok = app_launch_selfcheck();
    if (!ok) {
      ra_log_info("app_launch", "FAIL selfcheck");
    }
  }
  if (ok) {
    app_launch_banner();
