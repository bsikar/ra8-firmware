 ...
 * host-side ladder: the ra8_io USB-MSC block device the ra8_fs mount runs on,
 * the FAT16 mount + directory browse, the raw multi-block READ(10) verify
...
/**
 * @brief Browse the FAT root directory: READ(10) it and parse the file entry.
 *
 * @details Reads the synthesized volume's root-directory sector
 * (::k_fat_root_lba) over the loop and parses the first file entry (the volume
...
[[nodiscard]] static ra8_err_t selftest_browse_root(void)
{
  ...
  const uint8_t* entry = &s_dir[k_dir_entry_bytes]; /* entry 0 = volume label */
  char           name[(uint32_t)k_dir_name_bytes + 1U];
  for (uint32_t i = 0U; i < (uint32_t)k_dir_name_bytes; i++) {
    name[i] = (char)entry[i];
  }
  name[(uint32_t)k_dir_name_bytes] = '\0';
  uint32_t size                    = 0U;
