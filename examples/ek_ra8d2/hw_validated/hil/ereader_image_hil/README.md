...
```
make ereader_image_hil
scripts/hil_run_local.sh ereader_image_hil      # flash + scrape the banner
```
...
`scripts/board_sim_smoke.sh ereader_image_hil` runs the firmware ELF on the
emulated RA8D2 and scrapes the banner:

```
ereader_image_hil   OK (uart: 'ereader-img-hil: img 160x120 crc=BDC56EC5')
```
