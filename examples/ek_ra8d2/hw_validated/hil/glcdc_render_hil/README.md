...
`ereader_chrome_hil` gates the *software* rasteriser (`ra_box` + `ra_gfx` into
...
```
make glcdc_render_hil
scripts/hil_run_local.sh glcdc_render_hil      # flash + scrape the banner
```
...
tools/board_sim/build/board_sim \
  examples/ek_ra8d2/hw_validated/hil/glcdc_render_hil/build/glcdc_render_hil.elf \
