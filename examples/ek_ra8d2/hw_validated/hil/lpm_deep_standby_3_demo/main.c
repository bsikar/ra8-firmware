/**
 * @file examples/ek_ra8d2/hw_validated/hil/lpm_deep_standby_3_demo/main.c
 ...
 * @since 0.1.0
 */
static void lpm_dpsby3_add_offset(const ra8_rtc_datetime_t* now, ra8_rtc_datetime_t* out)
{
  *out                = *now;
 ...
 * @since 0.1.0
 */
[[nodiscard]] static ra8_err_t lpm_dpsby3_arm_wake(const ra8_rtc_datetime_t* now)
{
...
int32_t main(void)
{
  ...
  (void)ra8_board_uart_console_write(k_lpm_dpsby3_boot_msg,
  ...
  while (1) {
    ...
    if (lpm_dpsby3_arm_wake(&now) != k_ra8_ok) {
      break;
    }
