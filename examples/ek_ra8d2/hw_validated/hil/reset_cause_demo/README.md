...
HIL gate for `libs/ra_hal/ra_reset` (#52). The app boots, triggers a
software reset via `ra_reset_software_reset()`, and on the post-reset
boot proves that `ra_reset_get_cause()` decodes `RSTSR1.SWRF` correctly
by entering a 20 Hz loop that advances `g_reset_cause_loop` only when
the observed cause is `k_ra_reset_cause_software`.
...
the `ra_reset_cause_t` value observed on this boot.
