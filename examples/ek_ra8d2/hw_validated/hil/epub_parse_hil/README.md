...
```sh
make epub_parse_hil                          # cross-compile -> build/epub_parse_hil.elf
make -C examples/epub_parse_hil flash        # flash via on-board J-Link OB
```

Or standalone, from inside the app directory:

```sh
cd examples/ek_ra8d2/hw_validated/hil/epub_parse_hil/
make
make flash
make clean
```
...
./tools/board_sim/build/board_sim build/epub_parse_hil.elf
```
