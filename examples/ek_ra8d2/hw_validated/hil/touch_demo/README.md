...
gate** (`ra_touch`, #122). Until now the GT911 driver was only exercised
...
2. `ra_touch_open()` -- the real driver: configures IIC_B channel 0, wakes the
...
3. Polls `ra_touch_read()` (statically bounded) for one touch frame and reports
...
of the gate -- it proves the real `ra_touch_open -> IIC_B -> GT911` path reached
...
`board_sim` models the GT911 on the modelled I2C bus and injects a tap with
`--click X Y`; the tap returns through the genuine `ra_touch_read` decode (there
is no function-level stub), so the banner carries a **real decoded coordinate**.

```
$ board_sim touch_demo.elf --click 250 250
[uart] SCI8: touch-demo: boot
[uart] SCI8: touch: open=OK pts=1 x=250 y=250
  I3C/I2C GT911 : 1 touch frame(s) drained via ra_touch -> I3C
```
...
(`ra_touch -> ra_i3c -> GT911` decode). The driver is the same one
`ereader_ui` uses and `tests/test_ra_touch.c` covers on the host.
