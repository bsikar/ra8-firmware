...
End-to-end demo of the **ra_io** I/O fabric (epic #155) with the opt-in
...
2. Bridges it to `ra_fs`, formats a FAT12 volume, mounts it, and registers it in
   the VFS as `"ram"` (Phase 3, #158).
3. Generates a 4096-byte, highly-compressible payload and DEFLATEs it with
   `ra_io_compress()` into a staging buffer, using a caller-provided scratch
...
5. Reads the blob back, inflates it with `ra_io_decompress()`, and verifies the
...
`ra_io_compress.h` is **not** part of the `ra_io.h` umbrella -- it pulls in the
vendored miniz. An app opts in by including it directly and adding `miniz` to its
`ra_add_app(... LIBS ...)`, as this app's `CMakeLists.txt` does. Plain ra_io
consumers (e.g. `ra_io_demo`) carry no compressor and no miniz dependency.

## Expected output (board_sim or J-Link RTT/UART)

```
ra_io_compress_demo: boot
...
```
make            # -> build/ra_io_compress_demo.elf / .hex / .bin
```
