/**
 * @file examples/ek_ra8d2/hw_validated/hil/ra_io_fsfmt_demo/main.c
 * @brief ra_io pluggable filesystem-format registry demo (Phase 4, #159).
 *
 * @details
 ...
 *   1. Build a RAM block device, bridge it to ra_fs, and format a FAT12 volume
 *      on it (Phase 1/3, #156/#158).
 *   2. Register the built-in formats (`ra_io_fsfmt_init`) and probe the FAT
 ...
 *   4. Report progress on the SCI8 console through a ra_io UART stream sink.
 *
 * The board_sim emulator captures the SCI8 console, so the PASS line is
 * observable headlessly: a successful run prints
 * `ra_io_fsfmt_demo: probed fat maxname=12 + foreign stub seam PASS`.
 *
...
static uint8_t s_stub_disk[(size_t)k_demo_stub_blocks * (size_t)k_ra_io_block_size_bytes];
...
/** @brief ra_fs backend bridged onto the FAT block device. */
...
static const char* const s_tag = "ra_io_fsfmt_demo";
...
static const ra_io_fsfmt_t k_demo_stub = {
  .name = "stub",
  .caps =
    {
...
static ra_err_t demo_probe_fat(const ra_io_fsfmt_t** out_fmt)
{
  RA_CHECK_NULL_PTR(out_fmt, s_tag, "out_fmt");
  RA_RETURN_ON_ERROR(
  ...
  RA_RETURN_ON_ERROR(ra_io_fsfmt_init(), s_tag, "fsfmt init");
  RA_RETURN_ON_ERROR(ra_io_fsfmt_probe(&s_be, out_fmt), s_tag, "probe fat");
...
static ra_err_t demo_probe_foreign(void)
{
  RA_RETURN_ON_ERROR(ra_io_fsfmt_register(&k_demo_stub), s_tag, "register stub");

  ra_io_blockdev_t           bd    = {};
  ra_io_blockdev_ram_state_t state = {};
  RA_RETURN_ON_ERROR(
    ra_io_blockdev_ram_init(&bd, &state, s_stub_disk, (uint32_t)k_demo_stub_blocks, false),
    s_tag,
    "stub blockdev");
  uint8_t blk[(size_t)k_ra_io_block_size_bytes] = {};
  blk[0]                                        = (uint8_t)k_demo_stub_magic;
  RA_RETURN_ON_ERROR(ra_io_blockdev_write(&bd, 0, 1, blk), s_tag, "stub mark");

  ra_fs_backend_t be = {};
  RA_RETURN_ON_ERROR(ra_io_blockdev_as_fs_backend(&bd, &be), s_tag, "stub bridge");
  const ra_io_fsfmt_t* fmt = nullptr;
  RA_RETURN_ON_ERROR(ra_io_fsfmt_probe(&be, &fmt), s_tag, "probe stub");
...
int main(void)
{
  ...
  demo_print("ra_io_fsfmt_demo: boot\r\n");

  const ra_io_fsfmt_t* fat = nullptr;
  if ((demo_probe_fat(&fat) == k_ra_ok) && (demo_probe_foreign() == k_ra_ok)) {
    demo_print("ra_io_fsfmt_demo: probed ");
    demo_print(fat->name);
    demo_print(" maxname=");
    (void)ra_io_stream_put_u32(&s_uart, (uint32_t)fat->caps.max_name_len);
    demo_print(" + foreign stub seam PASS\r\n");
  } else {
    demo_print("ra_io_fsfmt_demo: FAIL\r\n");
  }
