...
```sh
make blink_hal                       # cross-compile -> examples/ek_ra8d2/hw_validated/hil/blink_hal/build/blink_hal.elf
make -C examples/blink_hal flash     # flash via on-board J-Link OB
```

Or standalone, from inside `examples/ek_ra8d2/hw_validated/hil/blink_hal/`:

```sh
cd examples/ek_ra8d2/hw_validated/hil/blink_hal/
