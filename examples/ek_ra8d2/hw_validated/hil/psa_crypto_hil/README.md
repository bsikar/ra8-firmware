...
`ra_rot` needs SHA-256 + ECDSA-P256 verify to authenticate images, and the RSIP
crypto *hardware* is non-functional on this silicon (see `libs/ra_hal/src/ra_rsip.c`).
...
`ra_add_app` builds the app; the per-app `CMakeLists.txt` compiles the vendored
