/**
 * @file examples/ek_ra8d2/hw_validated/hil/usb_selftest_hs_host/src/usb_selftest_hs_host_steps.h
 ...
 *
 * @details
 * This header is the seam between `main.c` (board bring-up, ThreadX
 * workers, USBX descriptors, MSC class registration) and the two helper
 * translation units that carry the bulk of the self-test logic:
 *
 *  - `usb_selftest_hs_host_msc.c` -- device-side FAT16 synthesis over the
 *    1 MiB MRAM window and the USBX storage media callbacks
 *    (read / write / status). `main.c`'s `selftest_msc_class_register`
 ...
 *    is declared here.
 *
 * All app-specific compile-time constants (tunables, FAT16 geometry,
 * boot-sector field values, byte offsets, SCSI sense triples) live in
 * this header as C23 typed enums so every translation unit shares one
 * definition.
 *
...
/* -------------------------------------------------------------------------- */
/* Device-side storage media callbacks (usb_selftest_hs_host_msc.c) */
/* -------------------------------------------------------------------------- */
 ...
 * self-loop traffic is visible on the board. Registered with the USBX
 * storage class by `main.c`'s `selftest_msc_class_register`.
 *
...
/* -------------------------------------------------------------------------- */
/* Host-side pass ladder (usb_selftest_hs_host_host.c) */
/* -------------------------------------------------------------------------- */
 ...
 * host-ladder phase probe for J-Link readout. On any failure the host
