/**
 * @file examples/ek_ra8d2/hw_validated/hil/dfu_bootloader/src/vector_table.c
 ...
 * after ra8_isr_register armed the NVIC line halted the chip. The
...
void Reset_Handler(void)
{
  ...
  /* Step 3b: copy the SRAM ram-functions (ra8_flash + ra8_dfu_program) from
   * their MRAM load image into SRAM, so the code-MRAM program loop never
   * executes from the MRAM it is programming (HUM Ch 59 / ra8_flash.h). */
 ...
 *          RA8_ENABLE_ROOT_OF_TRUST is on, ra8_rot_antirollback_prepare()
 ...
 *          other fault falls through to the noreturn ra8_exception_report.
 * @param[in,out] frame      Exception stack frame captured at handler entry.
 * @param[in]     exc_number Vector number (::ra8_vector_exc_t).
...
{
#ifdef RA8_ENABLE_ROOT_OF_TRUST
  if (ra8_rot_antirollback_on_probe_fault(frame)) {
    return; /* recovered the anti-rollback counter probe -> exception return */
  }
#endif
  ra8_exception_report(frame, exc_number);
}
