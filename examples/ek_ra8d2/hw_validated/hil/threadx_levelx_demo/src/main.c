/**
 * @file examples/ek_ra8d2/hw_validated/hil/threadx_levelx_demo/src/main.c
...
{
  static CHAR s_flash_name[] = "ra8_xspi_nor";

  UINT status =
    lx_nor_flash_format(&s_nor_flash, s_flash_name, lx_nor_driver_ra8_xspi_initialize, LX_NULL);
  if (status != LX_SUCCESS) {
    demo_print("[lx] format failed\r\n");
    demo_panic_halt();
  }
  status = lx_nor_flash_open(&s_nor_flash, s_flash_name, lx_nor_driver_ra8_xspi_initialize);
  if (status != LX_SUCCESS) {
...
{
  static CHAR s_thread_name[] = "lx_demo";
