 ...
 *      anchor) through `reflow` with the bundled Ahem face.
 *   2. Synthesises a "tap" at the centre of each laid-out link rectangle and
 *      resolves it with reflow_hit_test_link() + reflow_href_split():
...
{
  const reflow_link_rect_t* rect = &s_engine.link_rects[idx];
  const int32_t             cx   = rect->x + (rect->w / 2);
  const int32_t             cy   = rect->y + (rect->h / 2);
  uint32_t                  off  = 0U;
  uint32_t                  len  = 0U;
  if (reflow_hit_test_link(&s_engine, rect->page_index, cx, cy, &off, &len) != k_ra8_ok) {
    return;
  }
  ...
  uint32_t           fo   = 0U;
  uint32_t           fl   = 0U;
  if (reflow_href_split((const char*)&s_engine.text_pool[off], len, &kind, &pl, &fo, &fl) !=
      k_ra8_ok) {
    return;
  }
...
void main(void)
{
  ...
  if (reflow_init((uint16_t)k_lk_view_w,
  ...
  uint32_t apage = 0U;
  if (reflow_find_anchor(&s_engine, "foot", (uint32_t)(sizeof("foot") - 1U), &apage) != k_ra8_ok) {
