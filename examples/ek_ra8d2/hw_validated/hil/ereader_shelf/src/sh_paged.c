/**
 * @file examples/ek_ra8d2/hw_validated/hil/ereader_shelf/src/sh_paged.c
 ...
 * is opened through the chunked "RBKC" reader (ra_book_chunked) and bound as an
 * ::ra_book_src_paged source over an ::ra_vmem page cache. There is deliberately
 ...
 *   - the single-slot ::ra_vsource registry and the ::ra_vmem cache.
 *
 ...
 * ra_book_chunked_read relies on).
 *
 ...
 *          entries; a bigger book fails ra_book_chunked_open honestly).
...
typedef enum : uint32_t {
  k_shp_frame_bytes   = 64U * 1024U, /**< ra_vmem frame size == container chunk_bytes. */
...
static ra_vmem_frame_t s_shp_meta[k_shp_frame_count];
...
static ra_book_chunked_t s_shp_rd;

/** @brief Single-slot object registry: object 0 is the open book. */
static ra_vsource_obj_t s_shp_obj;

/** @brief Source registry fronting ::s_shp_obj. */
static ra_vsource_t s_shp_vs;

/** @brief The page cache the bound ::ra_book_src_t reads through. */
static ra_vmem_t s_shp_vm;
...
/** @brief ra_vsource_read_fn over a memory-mapped (MRAM) container blob. */
static ra_err_t sh_paged_mram_read(void* ctx, uint64_t offset, uint8_t* buf, uint32_t len)
{
  RA_CHECK_NULL_PTR(ctx, s_shp_tag, "mram: null ctx");
  RA_CHECK_NULL_PTR(buf, s_shp_tag, "mram: null buf");
 ...
 *          ra_book_validate() runs, minus its body CRC-32. Verifying that CRC
 ...
 *          ra_book_chunked_read.
 */
static bool sh_paged_hdr_ok(const ra_book_header_t* hdr, uint64_t inflated_total)
{
  ...
  if (hdr->total_size < (uint32_t)sizeof(ra_book_header_t)) {
    return false;
  }
  const struct {
    ...
    {hdr->chapter_off, hdr->chapter_count, (uint32_t)sizeof(ra_book_chapter_t)},
    {hdr->node_off, hdr->node_count, (uint32_t)sizeof(ra_book_node_t)},
    {hdr->attr_off, hdr->attr_count, (uint32_t)sizeof(ra_book_attr_t)},
    {hdr->stylesheet_off, hdr->stylesheet_count, (uint32_t)sizeof(ra_book_stylesheet_t)},
    {hdr->image_off, hdr->image_count, (uint32_t)sizeof(ra_book_image_t)},
...
{
  if (ra_vsource_init(&s_shp_vs, &s_shp_obj, 1U) != k_ra_ok) {
    return false;
  }
  uint32_t oid = 0U;
  if (ra_vsource_add_paged(&s_shp_vs,
                           ra_book_chunked_read,
                           &s_shp_rd,
                           0U,
  ...
  s_shp_vm = (ra_vmem_t){};
  if (ra_vmem_init(&s_shp_vm, &cfg) != k_ra_ok) {
    return false;
  }
  /* Binding reads the 100-byte header through the cache (faults chunk 0 in). */
  if (ra_book_src_paged(&g_sh.book_src,
                        &s_shp_vm,
                        oid,
                        (uint32_t)k_shp_frame_bytes,
                        (uint32_t)s_shp_rd.inflated_total) != k_ra_ok) {
    return false;
  }
...
bool sh_paged_open(ra_vsource_read_fn file_read, void* file_ctx, uint64_t file_len)
{
  ...
  g_sh.book_src = (ra_book_src_t){};
  s_shp_rd      = (ra_book_chunked_t){};
  if ((file_read == nullptr) || (file_len == 0U)) {
    sh_paged_close();
    return false;
  }
  if (ra_book_chunked_open(&s_shp_rd,
                           file_read,
                           file_ctx,
                           file_len,
                           sh_inflate,
                           ...
                           s_shp_staging,
                           (uint32_t)k_shp_staging_bytes) != k_ra_ok) {
    sh_paged_close();
    return false;
  }
  ...
  /* ra_book_src_t carries a uint32 size; the table budget already caps books
