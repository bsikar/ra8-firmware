/**
 * @file examples/ek_ra8d2/hw_validated/hil/ereader_shelf/src/sh_toc.c
 * @brief Table-of-contents screen: scrollable chapter list for the open book.
 *
 * @details
 * Lists the open book's spine chapters using their `ra_book` TOC labels (falling
...
void sh_toc_render(void)
{
  ...
  for (int32_t r = 0; r < rows; ++r) {
    ...
    (void)ra_gfx_rect(0, ry, (int32_t)k_sh_fb_w, (int32_t)k_sh_toc_row_h, bg, true);
    (void)ra_gfx_rect(0,
    ...
    (void)
      ra_gfx_text_out(k_sh_toc_text_pad, ty, num, &ra_gfx_font_8x16, (uint32_t)k_sh_col_sub, bg);
    char label[k_sh_linebuf];
    sh_toc_label(ci, label, sizeof label);
    (void)ra_gfx_text_out(k_sh_toc_text_pad + k_sh_toc_num_w,
                          ty,
                          label,
                          &ra_gfx_font_8x16,
