...
TAP bookkeeping driver `ra_bscan` (#138). No panel / SD / touch / external JTAG
fixture needed.
...
`ra_bscan` bookkeeping object. This app runs that contract end-to-end:

1. `ra_bscan_init()` seeds the expected device ID code.
2. `ra_bscan_get_idcode()` returns the chip's hardwired JTIDR
...
3. `ra_bscan_set_instruction()` accepts the named JTIR opcodes (EXTEST /
   SAMPLE_PRELOAD / IDCODE / CLAMP / HIGHZ / BYPASS) and **rejects** reserved
   4-bit codes (HUM Ch 50.2.1 p 3258).
4. `ra_bscan_clear_status()` accepts only the legal zero mask and reports
   BYPASS; `ra_bscan_get_status()` reflects the bookkeeping snapshot.
...
host unit tests in `tests/test_ra_bscan.c`, so the same contract is covered
...
This app validates the firmware-side `ra_bscan` contract only. Exercising the
...
external-tool path for `ra_bscan` (see the driver header `libs/ra_hal/inc/ra_bscan.h`).
