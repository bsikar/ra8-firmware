/**
 * @file examples/ek_ra8d2/hw_validated/hil/dualcore_mailbox/src/main.c
 ...
 *       log level of INFO or finer (a Debug build). `just apps::emulator::run dualcore_mailbox`
 ...
 * @since 0.1.0
 */
static bool wait_for_reply(const volatile dualcore_mailbox_t* mb, uint32_t target)
{
 ...
 * @since 0.1.0
 */
static bool wait_for_m33_boot(const volatile dualcore_mailbox_t* mb)
{
  for (uint32_t i = 0U; i < (uint32_t)k_dualcore_poll_budget; i++) {
