/**
 * @file examples/ek_ra8d2/hw_validated/hil/ereader_image/src/main.c
 * @brief Headless on-silicon HIL gate for the raster image decode pipeline (#106).
 *
 * @details
 * Closes the *real-hardware* gap for the zero-heap `<img>` / cover-image
 * pipeline (reflow_image): decode -> nearest-neighbour scale-to-fit -> blit,
 * with no panel / SDRAM / touch / SD dependency. The app:
 *
 *   1. Decodes a baked 120x90 PNG cover (cover_fixture.h) through
...
/** @brief Fixed bump arena backing the heap-free PNG decode. */
