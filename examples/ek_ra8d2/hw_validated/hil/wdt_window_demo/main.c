...
typedef enum : uint32_t {
  ...
  k_wdt_window_demo_sci_channel = 8U,
...
static const ra_port_pin_t k_wdt_window_demo_pin_txd =
  (ra_port_pin_t)(((uint16_t)k_ra_port_13 << 8) | (uint16_t)k_ra_pin_2);
static const ra_port_pin_t k_wdt_window_demo_pin_rxd =
...
[[nodiscard]] static ra_err_t wdt_window_demo_pins_init(void)
{
  ra_err_t err =
    ra_pfs_route_peripheral(k_wdt_window_demo_pin_txd, k_ra_psel_sci_async, "wdt_window.txd8");
  if (err != k_ra_ok) {
    return err;
  }
...
static void wdt_window_demo_setup_or_halt(void)
{
  ...
  if (wdt_window_demo_pins_init() != k_ra_ok) {
    wdt_window_demo_panic_halt();
  }
  const ra_sci_cfg_t sci_cfg = {
    .baud      = k_wdt_window_demo_baud,
    .data_bits = k_ra_sci_data_8,
    .parity    = k_ra_sci_parity_none,
    .stop_bits = k_ra_sci_stop_1,
    .pclk_hz   = pclka_hz,
  };
  if (ra_sci_init((uint8_t)k_wdt_window_demo_sci_channel, &sci_cfg) != k_ra_ok) {
    wdt_window_demo_panic_halt();
  }
...
{
  (void)ra_sci_write_polling((uint8_t)k_wdt_window_demo_sci_channel, msg, len);
}
...
int32_t main(void)
{
  ...
  (void)ra_sci_write_polling((uint8_t)k_wdt_window_demo_sci_channel,
                             k_wdt_window_demo_msg_boot,
                             (uint32_t)(sizeof(k_wdt_window_demo_msg_boot) - 1U));
