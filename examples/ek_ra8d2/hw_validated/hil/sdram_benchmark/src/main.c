/**
 * @file examples/ek_ra8d2/hw_validated/hil/sdram_benchmark/src/main.c
 ...
 * controller driver (``libs/ra8_hal/inc/ra8_sdramc.h``). The EK-RA8D2
...
{
  const volatile uint32_t* mem      = (const volatile uint32_t*)k_ra8_sdram_base_addr;
  const uint32_t           start_ms = ra8_time_ms();
  diag->count                       = 0U;
  for (uint32_t i = 0U; i < (uint32_t)k_sdram_demo_block_words; i++) {
