 ...
 * @post On any failing step ::c6_wifi_panic_halt parks the CPU, so this call never
 ...
 * @post Control never returns to ThreadX: a refused port, a failed readiness phase and
 *       a completed run all end in ::c6_wifi_heartbeat, which loops forever.
