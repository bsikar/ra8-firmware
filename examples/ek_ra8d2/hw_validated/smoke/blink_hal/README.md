...
example app: `examples/ek_ra8d2/hw_validated/smoke/blink_hal/main.c` plus its own `vector_table.c`,
...
```sh
make blink_hal                       # cross-compile -> examples/ek_ra8d2/hw_validated/smoke/blink_hal/build/blink_hal.elf
make -C examples/blink_hal flash     # flash via on-board J-Link OB
```

Or standalone, from inside `examples/ek_ra8d2/hw_validated/smoke/blink_hal/`:

```sh
cd examples/ek_ra8d2/hw_validated/smoke/blink_hal/
