/**
 * @file examples/ek_ra8d2/hw_pending/common/ra_io_roundtrip.h
 * @brief Shared, backend-agnostic ra_io VFS round-trip for the FAT demos (#155).
 *
 * @details
 * The four `ra_io_*_demo` apps (RAM, SDRAM, OSPI/xSPI NOR, SD-over-SPI) all run
 * the IDENTICAL fabric round-trip above the block-device seam: bridge the bound
 * block device to `ra_fs`, format + mount a FAT volume, register it in the VFS
 ...
 * thing that differs per demo is the backend bind line (`ra_io_blockdev_*_init`)
 ...
 * ::ra_io_blockdev_t to the helpers below, and print its own PASS banner. The
 ...
 *   - ::ra_io_roundtrip_mount       -- bridge + format + mount + VFS register.
 *   - ::ra_io_roundtrip_root_file   -- whole-file write at the volume root +
 *                                      VFS read-back + byte-compare.
 *   - ::ra_io_roundtrip_subdir_file -- mkdir + nested open/write/read/compare.
 *
...
/**
 * @struct ra_io_roundtrip_params_t
 * @brief Per-demo knobs for the shared ra_io round-trip.
 *
 ...
 * @invariant `fat_type` is one of `k_ra_fs_type_fat12`, `_fat16`, or `_fat32`.
 *
 * @par Example:
 * @code
 ...
 * @endcode
 *
 * @see ra_io_roundtrip_mount
 * @see ra_io_roundtrip_root_file
...
/**
 * @brief Bridge a bound block device to ra_fs, format + mount FAT, VFS-register.
 *
 * @details The backend-agnostic mount core shared by every FAT demo. The caller
 *          has already bound @p bd with the appropriate `ra_io_blockdev_*_init`;
 *          this function bridges it to an `ra_fs_backend_t`, formats it to
 ...
 *
 * @param[in]  bd  Bound block device (its `ra_io_blockdev_*_init` already ran).
 ...
 * @retval k_ra_ok            The volume is formatted, mounted, and registered.
 * @retval k_ra_err_null_ptr  @p bd, @p p, @p be, or @p out_mount was NULL.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_roundtrip_mount(ra_io_blockdev_t*               bd,
                                             const ra_io_roundtrip_params_t* p,
                                             ra_fs_backend_t*                be,
 ...
 * @pre `p->root_bytes <= k_ra_io_roundtrip_max_payload`.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_roundtrip_root_file(ra_fs_mount_t*                  mnt,
                                                 const ra_io_roundtrip_params_t* p);
 ...
 *          open(write) + `ra_fs_write` + close, reads it back via VFS
 *          open(read) + `ra_fs_read` + close, and byte-compares -- exercising
 ...
 * @retval k_ra_err_null_ptr          @p p was NULL.
 * @retval k_ra_err_invalid_size      The read-back length differed.
 * @retval k_ra_err_checksum_mismatch The read-back bytes differed.
 * @retval (other)                    The first failing VFS / ra_fs step's code.
 *
 * @pre @p p is non-NULL and its volume is mounted (see ::ra_io_roundtrip_mount).
 * @pre `p->subdir_bytes <= k_ra_io_roundtrip_max_payload`.
 ...
 * @since 0.1.0
 */
[[nodiscard]] ra_err_t ra_io_roundtrip_subdir_file(const ra_io_roundtrip_params_t* p);
