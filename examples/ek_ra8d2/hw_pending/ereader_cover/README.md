...
reader, zero-heap through a static arena); `ra8_epub_get_cover_image` resolves
...
the box. No panel, SDRAM, touch or SD dependency.

## It does not pass on silicon

On the bench `ra8_epub_open` fails outright on the real part, against the same
...
Regenerate the fixture with `make_cover_fixture.py` beside `main.c` (it needs
Pillow), then re-read the CRC the board prints and update `hil.conf`.
