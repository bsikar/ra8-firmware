...
instead of the project's hand-rolled `ra_usb_cdc` layer.

## What it does

A single ThreadX worker brings the USBX device stack up against the
project's `ra_usb` DCD bridge, registers the CDC ACM class, attaches
...
| USB_FS_DP     | P8_14  | k_ra_psel_usb_fs (0x13) | D+ data line.                  |
| USB_FS_DM     | P8_15  | k_ra_psel_usb_fs (0x13) | D- data line.                  |

## Sequence

1. `ra_cgc_init()` -- standard FSP-quickstart clock tree.
2. `ra_cgc_get_clock_hz(k_ra_clock_id_cpuclk0, ...)` + `ra_time_init`.
3. `ra_board_led_init(k_ra_board_led1)`.
4. `ra_pfs_route_peripheral` for the four USB-FS pins.
5. `ra_isr_globals_enable` then `tx_kernel_enter`.
6. Worker (4 KiB stack, prio 8): `ux_system_initialize` (16 KiB pool)
   ...
   -> `ux_dcd_ra_usb_initialize(k_ra_usb_speed_fs)` ->
   `ra_usb_device_attach(true)` -> echo loop.
...
directly via `ra_pfs_route_peripheral`.
