...
bare EK-RA8D2 EVM. Exercises the safe, key-free half of the `ra_dotf` driver
(#127).
...
1. `ra_dotf_init` -- clock-ungate both DOTF channels (`MSTPB16` / `MSTPB17`,
...
2. `ra_dotf_run_self_test` on channels 0 and 1 -- set `REG00` bit 20 (the
...
- installs a key (that needs an `ra_rsip`-wrapped key handle), stages an IV,
  or programs a conversion region;
- calls `ra_dotf_enable` -- arming the AES core over a live XiP window would
...
core -- the bring-up + self-test calls return `k_ra_ok` and the demo reports
...
- `REG00` (AES core / control, +0x080 in the channel window) bit 20 is the
  self-test trigger; decoded by `ra_dotf_run_self_test` / `ra_dotf_get_status`
