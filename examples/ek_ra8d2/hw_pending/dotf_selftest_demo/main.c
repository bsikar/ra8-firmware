 ...
 *
 *  1. ``ra_dotf_init`` -- clock-ungate both channels (MSTPB16/17, shared with
 *     OSPI0/1) and reset REG00 to its bypass value. Neither channel is armed.
 *  2. ``ra_dotf_run_self_test`` on channels 0 and 1 -- set REG00 bit 20, poll
 ...
 *     snapshot value.
 *  3. ``ra_dotf_get_status`` -- read REG00 back for the bench probe.
 *
 * **What this demo deliberately does NOT do:** install a key, stage an IV,
 * program a conversion region, or ``ra_dotf_enable`` a channel. Arming the
 * AES core over a live XiP window would fault the next instruction fetch
 * (HUM Ch 45 warning), and key handling needs an ra_rsip-wrapped key. None
 ...
 * a real AES core -- the bring-up + self-test calls return ``k_ra_ok`` and
...
/** @brief Sentinel for "ra_dotf_init has not run yet" in g_dotf_init_err. */
typedef enum : uint32_t {
  k_dotf_demo_err_unset = 0xFFFFFFFFU, /**< Distinct from any ra_err_t code. */
} dotf_demo_sentinel_t;

/** @brief Pinout for SCI8 on the J-Link OB CDC channel (PD02 / PD03). */
static const ra_port_pin_t k_dotf_demo_pin_txd =
  (ra_port_pin_t)(((uint16_t)k_ra_port_13 << 8) | (uint16_t)k_ra_pin_2);
static const ra_port_pin_t k_dotf_demo_pin_rxd =
...
/**
 * @var g_dotf_ok
 * @brief 1 when both channels' self-test + status calls returned k_ra_ok.
...
/**
 * @var g_dotf_init_err
 * @brief ra_err_t from ra_dotf_init (0 == k_ra_ok).
...
/**
 * @var g_dotf_reg00
 * @brief Last REG00 snapshot from ra_dotf_get_status(ch0).
 * @note Read externally only.
 * @since 0.1.0
 */
 ...
 * @since 0.1.0
 */
[[nodiscard]] static ra_err_t dotf_demo_pins_init(void)
{
  ra_err_t err =
    ra_pfs_route_peripheral(k_dotf_demo_pin_txd, k_ra_psel_sci_async, "dotf_demo.txd8");
  if (err != k_ra_ok) {
    return err;
  }
  return ra_pfs_route_peripheral(k_dotf_demo_pin_rxd, k_ra_psel_sci_async, "dotf_demo.rxd8");
}
...
static void dotf_demo_setup_or_halt(void)
{
  ...
  if (dotf_demo_pins_init() != k_ra_ok) {
    dotf_demo_panic_halt();
  }
  const ra_sci_cfg_t sci_cfg = {
    .baud      = k_dotf_demo_baud,
    .data_bits = k_ra_sci_data_8,
    .parity    = k_ra_sci_parity_none,
    .stop_bits = k_ra_sci_stop_1,
    .pclk_hz   = pclka_hz,
  };
  if (ra_sci_init((uint8_t)k_dotf_demo_sci_channel, &sci_cfg) != k_ra_ok) {
    dotf_demo_panic_halt();
  }
 ...
 *
 * @param[in] st0_err     ra_err_t from ra_dotf_run_self_test(ch0).
 * @param[in] st1_err     ra_err_t from ra_dotf_run_self_test(ch1).
 * @param[in] status_err  ra_err_t from ra_dotf_get_status(ch0).
 *
 * @return 1 when every call returned k_ra_ok, else 0.
 *
 * @par MC/DC:
 ...
 *
 * @pre All three arguments are ra_err_t-domain values.
 * @post Returns a 0/1 verdict (no side effects).
 * @since 0.1.0
 */
[[nodiscard]] static uint8_t
dotf_demo_verdict(ra_err_t st0_err, ra_err_t st1_err, ra_err_t status_err)
{
  const bool ok = (st0_err == k_ra_ok) && (st1_err == k_ra_ok) && (status_err == k_ra_ok);
  return ok ? 1U : 0U;
}
 ...
 *                    k_ra_ok. The opaque REG00 snapshots are reported via
 *                    globals, not gated (board_sim does not model the AES core).
 *
 * @return ``ra_err_t`` -- the first non-ok status from the three sub-calls, or
 *         k_ra_ok when all succeeded.
 ...
 *
 * @pre ::ra_dotf_init succeeded.
 * @post ``g_dotf_st0_snap`` / ``g_dotf_st1_snap`` / ``g_dotf_reg00`` updated.
 * @since 0.1.0
 */
[[nodiscard]] static ra_err_t dotf_demo_sample(uint8_t* out_ok)
{
  RA_CHECK_NULL_PTR(out_ok, s_tag, "out_ok must not be nullptr");

  uint32_t       st0     = 0U;
  const ra_err_t st0_err = ra_dotf_run_self_test((uint8_t)k_dotf_demo_ch0, &st0);
  g_dotf_st0_snap        = st0;

  uint32_t       st1     = 0U;
  const ra_err_t st1_err = ra_dotf_run_self_test((uint8_t)k_dotf_demo_ch1, &st1);
  g_dotf_st1_snap        = st1;

  uint32_t       reg00    = 0U;
  const ra_err_t stat_err = ra_dotf_get_status((uint8_t)k_dotf_demo_ch0, &reg00);
  g_dotf_reg00            = reg00;
...
int32_t main(void)
{
  ...
  while (1) {
    uint8_t        ok   = 0U;
    const ra_err_t err  = dotf_demo_sample(&ok);
    const uint8_t  good = (err == k_ra_ok && ok != 0U) ? 1U : 0U;
    g_dotf_ok           = (uint32_t)good;
    if (good != 0U) {
      (void)ra_sci_write_polling((uint8_t)k_dotf_demo_sci_channel,
                                 k_dotf_demo_ok_msg,
                                 (uint32_t)(sizeof(k_dotf_demo_ok_msg) - 1U));
      (void)ra_board_led_toggle(k_ra_board_led1);
    } else {
      (void)ra_sci_write_polling((uint8_t)k_dotf_demo_sci_channel,
                                 k_dotf_demo_bad_msg,
                                 (uint32_t)(sizeof(k_dotf_demo_bad_msg) - 1U));
      (void)ra_board_led_toggle(k_ra_board_led2);
    }
    ++g_dotf_heartbeat;
    ra_delay_ms(k_dotf_demo_period_ms);
  }
  dotf_demo_panic_halt();
  return 0;
}
