...
- Cross-compile builds cleanly with `RA_TRUSTZONE_ENABLE=ON`.
...
- Whether the ping/pong loop round-trips ping=0x1234 / pong=0x4321
  through `ra_ipc_send_message` / `ra_ipc_recv_message` on
  channels 0 and 2.
...
5. JTAG read `g_cpu1_pingpong_ipc_tz_step` -- must be 6
   (`k_ra_tz_secure_boot_step_branched`).
...
- `libs/ra_tz_secure_boot/` -- the shared secure-boot implementation.
