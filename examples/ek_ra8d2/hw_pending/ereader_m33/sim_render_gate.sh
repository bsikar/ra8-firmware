#!/usr/bin/env bash
#
# sim_render_gate.sh -- board_sim CRC gate for the ereader_m33 dual-core demo.
#
...
#
# Usage:
#   examples/ek_ra8d2/hw_pending/ereader_m33/sim_render_gate.sh
#   ERM33_GOLDEN_CRC=C0BA74D3 .../sim_render_gate.sh   # override the golden
#
...
# banner reaches stdout over ITM, so the run is captured and grepped (board_sim's
# BOARD_SIM_STOP_ON only watches the SCI UART line, not ITM).
out="$(BOARD_SIM_MAX_CHUNKS=1000 BOARD_SIM_WALL_S=300 "${sim}" "${elf}" 2>&1 || true)"
