/**
 * @file examples/ek_ra8d2/hw_pending/ereader_m33/vector_table.c
 ...
 * to ra8_isr_dispatch(slot) so any (handler, ctx) registered through the
 * substrate (ra8_isr_register) runs in NVIC handler-mode -- this app
 * arms one such line for the IPC0 receive event so the M85 can WFI-idle
 * until the M33 pokes IPC0ISET0 (HUM Ch 3.2.11 p 215) on a page turn to
 * wake it, instead of busy-polling the mailbox. Slots that were never
 * registered land in the dispatcher's bounds-checked no-op path. The
 * trampolines are weak so a driver can still install a fully custom IRQ
 * handler by defining a non-weak ``IRQ<n>_Handler``.
 *
 ...
 * working uart_irq_echo / compile_on_m33 apps use.
 */
