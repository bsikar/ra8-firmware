#!/usr/bin/env bash
#
# sim_handoff_gate.sh -- board_sim gate for the #150 ereader_m33 MODE-SWITCH.
#
# Cross-builds the app (Debug, so the M85's INFO banners are compiled in), builds
# the board_sim emulator, boots the real M85 ELF (which auto-boots the embedded
...
#
# What this proves, using ONLY existing board_sim mechanisms (dual-core + IPC +
...
# the whole exchange (board_sim echoes only the primary core's ITM), so the gate
...
#
# Usage:
#   examples/ek_ra8d2/hw_pending/ereader_m33/sim_handoff_gate.sh
#   ERM33_GOLDEN_CRC=C0BA74D3 .../sim_handoff_gate.sh   # override the golden
#
...
out="$(BOARD_SIM_MAX_CHUNKS=4000 BOARD_SIM_WALL_S=300 "${sim}" "${elf}" 2>&1 || true)"

fail() {
  echo "ereader_m33 handoff gate: FAIL -- $1" >&2
  echo "----- board_sim output (tail) -----" >&2
  tail -n 40 <<<"${out}" >&2
  exit 1
}
