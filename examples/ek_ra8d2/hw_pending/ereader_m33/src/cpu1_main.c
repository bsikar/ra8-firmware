/**
 * @file examples/ek_ra8d2/hw_pending/ereader_m33/src/cpu1_main.c
 ...
 *      no recursion) with the pure inline `book.h` accessors, collecting the
 ...
 *
 * @note Only `book.h` (header-only inline accessors), `ra8_gfx` (three
 ...
 * @param[in]  root       Chapter root node index, or ::k_book_nil.
...
collect_chapter_text(const void* base, uint32_t root, uint32_t node_count, char* out, uint32_t cap)
{
  ...
  for (uint32_t it = 0U; it < (uint32_t)k_walk_iter_max; it++) {
    ...
    if (node->kind == (uint8_t)k_book_node_text) {
      append_run(out, &len, cap, book_node_text(base, node));
    } else {
...
static bool book_is_valid(const void* base, uint32_t size)
{
  ...
  if (size < (uint32_t)k_book_sizeof_header) {
    return false;
  }
...
static bool render_held_page(const void* base, uint32_t* out_crc, uint32_t* out_glyphs)
{
  ...
  const book_header_t*  hdr      = book_header(base);
  const book_chapter_t* chapters = book_chapters(base);
