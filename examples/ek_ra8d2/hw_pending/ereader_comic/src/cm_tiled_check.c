 ...
 * JOF atlas through ::comic_tiles_import, and decodes every tile of the
...
[[gnu::section(".sdram_data")]] static comic_page_t s_ct_pages[k_ct_page_cap];
/** @brief SDRAM: comic name arena. */
[[gnu::section(".sdram_data")]] static char s_ct_names[k_ct_name_cap];
/** @brief SDRAM: the open comic reader. */
[[gnu::section(".sdram_data")]] static comic_t s_ct_comic;
/** @brief SDRAM: the comic tile reader. */
[[gnu::section(".sdram_data")]] static comic_tile_reader_t s_ct_reader;
...
/** @brief ::comic_read_fn over the resident CBZ blob (bounds-clamped). */
// cppcheck-suppress constParameterCallback -- ctx is pinned by the comic_read_fn vtable signature
...
static bool cm_ct_open(size_t* out_got)
{
  ...
  if (comic_page_count(&s_ct_comic) == 0U) {
    return false;
  }
  *out_got = 0U;
  return comic_page_read(&s_ct_comic, 0U, s_ct_pagebuf, sizeof s_ct_pagebuf, out_got) == k_ra8_ok;
}
...
static bool cm_ct_hash_tiles(const jof_info_t* info, uint32_t* out_crc)
{
  ...
  for (uint16_t ty = 0U; ty < info->tile_rows; ++ty) {
    for (uint16_t tx = 0U; tx < info->tile_cols; ++tx) {
      ra8_tile_t tile = {};
      if (comic_tiles_tile(&s_ct_reader, tx, ty, &tile) != k_ra8_ok) {
        return false;
      }
      const size_t n = (size_t)tile.width * (size_t)tile.height * (size_t)info->bpp;
      crc            = cm_ct_fnv(crc, tile.pixels, n);
      if (comic_tiles_release(&s_ct_reader, tile.pixels) != k_ra8_ok) {
        return false;
      }
    }
  }
...
cm_tiled_result_t cm_comic_tiled_selfcheck(void)
{
  ...
  uint16_t h   = 0U;
  uint64_t dec = 0U;
  if (comic_tiles_footprint(s_ct_pagebuf, got, &w, &h, &dec) != k_ra8_ok) {
    (void)comic_close(&s_ct_comic);
    return r;
  }
  if (!comic_tiles_over_budget(dec, (uint64_t)k_ct_budget)) {
    (void)comic_close(&s_ct_comic);
    return r; /* fixture must be oversized; a fit page is not the case under test */
  }
  ra8_tile_cache_cfg_t storage = cm_ct_cache_cfg();
  if (comic_tiles_init(&s_ct_reader, &storage, s_ct_scratch, (uint32_t)k_ct_scratch) != k_ra8_ok) {
    (void)comic_close(&s_ct_comic);
    return r;
  }
  ...
  if (comic_tiles_import(&s_ct_reader, s_ct_pagebuf, got, &cfg) != k_ra8_ok) {
    (void)comic_close(&s_ct_comic);
    return r;
  }
  jof_info_t info = {};
  if (comic_tiles_info(&s_ct_reader, &info) != k_ra8_ok) {
    (void)comic_close(&s_ct_comic);
    return r;
  }
  uint32_t crc = 0U;
