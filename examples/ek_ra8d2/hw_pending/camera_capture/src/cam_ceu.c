 ...
 * bounded-poll capture wrapper. The raw `ra_ceu_*` driver is wrapped so the
 * app never touches the ::ra_ceu_config_t descriptor.
 *
...
{
  ra_ceu_config_t cfg = {};
  ra_err_t        err = cam_fill_ceu_config(&cfg);
  if (err != k_ra_ok) {
    return err;
  }
  return ra_ceu_init(&cfg);
}
...
{
  RA_CHECK_NULL_PTR(out_evt, "cam", "ceu_status");
  return ra_ceu_get_status(out_evt);
}
