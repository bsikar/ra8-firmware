...
{
  const epub_mem_media_t media = {.data = k_epub_stress_fixture,
                                  .size = (size_t)k_epub_stress_fixture_len};
  if (epub_open(&media, "stress.epub", &s_book) != k_ra8_ok) {
  ...
  uint16_t toc = 0U;
  if ((epub_get_toc_count(&s_book, &toc) != k_ra8_ok) || (toc != (uint16_t)k_est_expect_toc)) {
