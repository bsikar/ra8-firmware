 ...
 * build a tall colourful strip as a JOF band-tile atlas (`jof`, one
 ...
 *
 *
 * @see longstrip.h  The continuous vertical-scroll engine this drives.
 ...
 *
 * @details `jof_parse` reads only the header + footer and validates
...
static jof_memstore_t   s_store; /**< Atlas backing-read seam.        */
static ra8_tile_cache_t s_cache; /**< Band cache (procedural decode). */
static longstrip_t      s_strip; /**< Opened longstrip scroll state.  */
 ...
 *          so there is no pixel region) and the footer. `jof_parse`
...
static uint32_t ls_build_atlas(void)
{
  ...
  s_atlas[13] = (uint8_t)k_jof_codec_raw;
...
{
  s_store = (jof_memstore_t){.buf = s_atlas, .cap = (size_t)k_ls_atlas_total, .len = total};
  ...
  const longstrip_cfg_t lcfg = {.pread      = jof_memstore_pread,
                                .pread_ctx  = &s_store,
                                .atlas_size = total,
                                .cache      = &s_cache,
                                ...
                                .blit       = ls_blit,
                                .blit_ctx   = nullptr};
  if (longstrip_open(&s_strip, &lcfg) != k_ra8_ok) {
    app_panic_halt();
  }
}
...
static void ls_redraw(void)
{
  ...
  (void)longstrip_render(&s_strip, &st);
...
static void ls_apply_nav(int32_t delta, bool toggle)
{
  if (toggle) {
    ...
    (void)longstrip_scroll_by(&s_strip, delta);
