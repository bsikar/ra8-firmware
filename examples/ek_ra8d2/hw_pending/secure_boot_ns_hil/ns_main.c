 ...
 * programmes the SAU, and -- only if ``ra_rot_verify_image`` authenticates the
 ...
 * ``ra_tz_secure_boot_jump_ns`` returns an error, and the counter stays 0.
 *
 * ## Memory layout
 *
 ...
 *      ``g_ra_ls_ns_stack_top`` (the Secure RoT gate must have passed first).
 ...
 * (see ``ns_image.ld`` + ::k_ra_tz_ns_rot_header_offset). 8-byte aligned per
