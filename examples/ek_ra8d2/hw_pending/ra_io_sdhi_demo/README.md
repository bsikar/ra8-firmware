...
Proves the `ra_io` fabric's "swappable backend" promise (epic #155, phase #156)
on the **native 4-bit SDHI** controller (#123): the **same** `ra_io` VFS API that
`ra_io_demo` runs over a RAM disk and `ra_io_sd_demo` runs over SD-over-SPI, now
running over a **micro-SD card reached through the dedicated SDHI host
controller** by swapping only the block-device backend.

## What it proves

`ra_io_sd_demo` binds the SD-over-SPI block device; this app binds the
native-SDHI block device (`ra_io_blockdev_sdhi_init`) on top of the `ra_sdcard` +
`ra_sdhi` HAL drivers. The SD bring-up routes the eight SDHI pins (port 4, pins
0..7) and runs the full SD Physical Layer identification through
`ra_sdcard_init`. Everything above the block device is the identical fabric:

1. `ra_io_blockdev_sdhi_init` -- native-SDHI block-device vtable over the card.
2. `ra_io_blockdev_as_fs_backend` -- bridge the block device to `ra_fs`.
3. `ra_fs_format` (FAT16) + `ra_fs_mount`, then `ra_io_vfs_mount("sd", ...)`.
4. `ra_io_vfs_mkdir("sd:/LOGS")` -- exercise the VFS mkdir path.
5. `ra_io_vfs_open("sd:/LOGS/A.TXT", write)` + `ra_fs_write` -- write 512 bytes.
...
```
ra_io_sdhi_demo: boot
ra_io_sdhi_demo: card ready
ra_io_sdhi_demo: sd:/LOGS/A.TXT 512 bytes PASS
```
...
All eight route to `PSEL = k_ra_psel_sdhi`.
...
Outputs `build/ra_io_sdhi_demo.elf` / `.hex` / `.bin`.
...
tools/board_sim/build/board_sim \
    examples/ek_ra8d2/hw_pending/ra_io_sdhi_demo/build/ra_io_sdhi_demo.elf \
    --sd-new 64:fat16
```
