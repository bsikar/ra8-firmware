/**
 * @file examples/ek_ra8d2/hw_pending/blink_m33_hal/src/trustzone_init.c
...
/* SAU (RNR / RBAR / RLAR / TYPE / CTRL) is an Arm v8-M architectural block in
 * the PPB window, described by the Arm v8-M Architecture Reference Manual and
 * not by the RA8D2 Hardware User's Manual -- a HUM citation here would name a
 * chapter that does not describe these registers. Route every access through
 * one named accessor rather than casting at each use, exactly as ra8_scb_reg
 * does for the System Control Block. */
static inline volatile uint32_t* internal_sau_reg(uintptr_t addr)
{
...
{
  *internal_sau_reg(addr) = value;
}

static uint32_t internal_read32(uintptr_t addr)
{
  return *internal_sau_reg(addr);
}
