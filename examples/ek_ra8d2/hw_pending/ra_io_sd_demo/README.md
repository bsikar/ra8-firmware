...
the **same** `ra_io` VFS API that `ra_io_demo` runs over a RAM disk, now running
over a **micro-SD card** by swapping only the block-device backend.

## What it proves

`ra_io_demo` binds the RAM block device; this app binds the SD-over-SPI block
device (`ra_io_blockdev_sdspi_init`) on top of the `ra_sdmmc_spi` driver. The SD
...
1. `ra_io_blockdev_sdspi_init` -- SD-SPI block-device vtable over the card.
...
```
ra_io_sd_demo: boot
ra_io_sd_demo: card ready
ra_io_sd_demo: sd:/LOGS/A.TXT 512 bytes PASS
```
...
Outputs `build/ra_io_sd_demo.elf` / `.hex` / `.bin`.

## Run in board_sim (no hardware)

board_sim models the microSD over `ra_sdmmc_spi`; attach a blank card with
`--sd-new <MiB>[:fat16|fat32]`. From the repo root:

```
tools/board_sim/build/board_sim \
    examples/ek_ra8d2/hw_pending/ra_io_sd_demo/build/ra_io_sd_demo.elf \
    --sd-new 64:fat16
```
