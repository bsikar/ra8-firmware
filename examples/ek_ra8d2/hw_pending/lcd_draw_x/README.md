...
1. `ra_pfs_route_peripheral` does not set PDR; LCD pins need a
...
2. The GLCDC output stage composes `BG x GR2 x GR1`.  BOTH GR1 and
   ...
   `libs/ra_hal/src/ra_glcdc.c::internal_panel_program`.
...
4. `BG_BGC` is shadow-registered.  Writes only take effect at the
   ...
   `ra_glcdc_set_background_color` pulses VEN internally so callers
   don't need to.
...
- `libs/ra_hal/src/ra_glcdc.c` -- HAL implementation; see comments
