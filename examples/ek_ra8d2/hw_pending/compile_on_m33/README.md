...
# compile_on_m33 -- compile `.epub` to `.rabook` on the M33 (#149b)

Demonstrates the complete bounded EPUB-to-RABOOK1 path on the Cortex-M33:
miniz reads the archive, `ra8_xml` and the EPUB consumers parse metadata and
chapters, and `libs/ra8_rabook_compile` emits the result. On a real
...
- **Compiler on the slow core.** The M33 drives the zero-heap front end and emitter
  API (`ra8_rabook_compile_init` / `intern` / `add_element` / `add_text` /
  `link_child` / `link_sibling` / `add_chapter` / `set_metadata` / `finalize`)
  over the M85-staged EPUB and lays a complete RABOOK1 blob into shared SRAM.
...
The M33 image now proves unzip, bounded XML parsing, chapter compilation, and
serialization. Raster transcoding remains compiled out with
`RA8_RABOOK_NO_RASTER`; SVG payloads are stored verbatim and the M85 still owns
the filesystem and staging policy.
...
| `cpu1_main.c`           | M33: parse EPUB, drive the compiler, publish result  |
