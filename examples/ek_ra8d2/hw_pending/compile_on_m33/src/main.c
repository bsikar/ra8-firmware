/**
 * @file examples/ek_ra8d2/hw_pending/compile_on_m33/src/main.c
 ...
 *      compile (`epub_open` -> the pipeline) into the shared output blob.
 *   3. Once the M33 signals `done`, the M85 runs `book_validate` over the blob,
 ...
 *       logging (a Debug build). `just apps::emulator::run compile_on_m33`
 *       builds Debug so the
 ...
 * @ref book_validate over the shared blob (magic, version, table extents, and
...
static bool verify_blob(volatile com33_mailbox_t* mb)
{
  ...
  if (book_validate(base, (size_t)len) != k_ra8_ok) {
    return false;
  }
  const book_header_t* hdr = book_header(base);
  if (hdr->crc32_val != mb->blob_crc) {
    return false;
  }
...
void main(void)
{
  ...
  if (verify_blob(mb)) {
    ra8_log_info_val("M85", "book_validate OK; chapters in blob", mb->chapter_count);
