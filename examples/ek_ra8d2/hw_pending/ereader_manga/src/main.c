/**
 * @file examples/ek_ra8d2/hw_pending/ereader_manga/src/main.c
 ...
 *      ``jof_produce`` -- one bounded band at a time, the decoded
 ...
 * a tile still on screen. `apps/board/stand_alone/ereader/tests/src/test_app_ereader_manga.c`
 ...
 * `jof_work_bytes(k_mg_cap_edge, k_mg_cap_edge, k_mg_tile_edge,
 ...
 *     fails the build;
 *   - `apps/board/stand_alone/ereader/tests/src/test_app_ereader_manga.c` asserts the arena
 *     against the real
 *     `jof_work_bytes()` return, which is the exact and
...
static_assert((uint64_t)k_mg_work_bytes >=
                ((uint64_t)k_mg_cap_edge * (uint64_t)k_mg_tile_edge * (uint64_t)k_jof_bpp_max),
              "work arena is below the producer band term for the advertised cap: re-derive "
              "k_mg_work_bytes from jof_work_bytes() for k_mg_cap_edge");
...
static bool mg_build_atlas(void)
{
  ...
  const uint32_t need = jof_work_bytes((uint16_t)k_mg_cap_edge,
  ...
  if (jof_produce(&cfg, &s_info) != k_ra8_ok) {
    return false;
  }
  return jof_parse(jof_memstore_pread, &s_store, s_store.len, &s_info) == k_ra8_ok;
}
