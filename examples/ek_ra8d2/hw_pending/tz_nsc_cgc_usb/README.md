...
- PC parked in ra_exception_halt_loop, LR in ra_exception_report.
- g_ra_exception_last is all zero (the report path apparently
  silently aborted before writing the snapshot).
- No UART output -- the SCI8 console never came up either.

The firmware crashes very early -- before `ra_cgc_init` finishes
or even reaches `ra_log_error_val`. Without further per-step
instrumentation it's not possible to tell whether the crash is in
SystemInit, in ra_cgc_init's PLL1 bring-up, or in the USBX memory
...
2. Stamp it 1 right after entering `main`, 2 after `ra_cgc_init`,
   3 after `ra_pfs_route_peripheral` for USB pins, 4 after
