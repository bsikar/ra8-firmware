/**
 * @file examples/ek_ra8d2/hw_pending/lpm_periodic_idle/vector_table.c
...
#ifndef __APPLE__
/* On the firmware target every peripheral IRQ vector slot forwards to
 * ra8_isr_dispatch(slot) so the ra8_isr substrate can deliver the ULPT0
 * underflow (and any other registered event) to its handler. */
 ...
 * event registered through ra8_isr_register lands on its handler. On the
 * Apple host syntax-check we drop ra8_isr_dispatch (the host build never
 * branches through these vectors) and keep an empty weak stub. */
#ifndef __APPLE__
/** @brief RA8 IRQ STUB. */
