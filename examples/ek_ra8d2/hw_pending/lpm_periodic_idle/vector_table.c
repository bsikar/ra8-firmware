 ...
 * ra_isr_dispatch(slot) so the ra_isr substrate can deliver the ULPT0
 ...
 * event registered through ra_isr_register lands on its handler. On the
 * Apple host syntax-check we drop ra_isr_dispatch (the host build never
