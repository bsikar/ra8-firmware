/**
 * @file examples/ek_ra8d2/drw_blend_demo/main.c
 ...
 * blit AND alpha-blend path (``ra_drw_blend_t``), not just the solid fill the
 ...
 *
 *  1. Background  -- an opaque dark-blue ``ra_drw_fill_rect`` over the whole FB.
 *  2. Sprite blit -- an opaque green ``ra_drw_fill_rect`` sub-rectangle (the
 *     "sprite" laid down over the background).
 *  3. Alpha blend -- a half-transparent red foreground composited over the
 *     background + sprite via ``ra_drw_set_blend`` (source-over, global alpha
 *     0x80) followed by a final ``ra_drw_fill_rect``. With the blend unit armed
 ...
 * (``board_periph_drw.c``): a ``ra_drw_fill_rect`` issued while CONTROL2.USEACB
 ...
 *
 * @details
 * Baud, loop period, the bounded ``ra_drw_wait_idle`` poll budget, the
...
static void drw_blend_setup_or_halt(void)
{
  ...
  if (ra_board_uart_console_init((uint32_t)k_drw_blend_baud) != k_ra_ok) {
    drw_blend_panic_halt();
  }
 ...
 *
 * @par MC/DC:
 * Single decision ``err != k_ra_ok`` in the caller -- no compound condition.
 *
 * @return ``ra_err_t`` from ``ra_drw_init``.
 * @retval k_ra_ok DRW configured for ARGB8888 over the demo framebuffer.
 * @retval k_ra_err_null_ptr Internal config pointer was NULL (cannot happen).
 ...
 * @since 0.1.0
 */
[[nodiscard]] static ra_err_t drw_blend_configure(void)
{
...
/**
 * @brief Issue one opaque ``ra_drw_fill_rect`` and wait for the engine.
 *
 ...
 *
 * @par MC/DC:
 * Single decision ``ferr != k_ra_ok`` -- no compound condition.
 *
 * @return ``ra_err_t`` -- ``k_ra_ok`` once the fill is issued and the engine
 *         goes idle, else the fill / wait error.
 * @retval k_ra_ok Fill rasterized.
 * @retval k_ra_err_invalid_arg Geometry rejected by ``ra_drw_fill_rect``.
 * @retval k_ra_err_hw_timeout Engine stayed busy past the poll budget.
...
drw_blend_fill(int16_t x, int16_t y, uint16_t w, uint16_t h, uint32_t argb)
{
  ...
  return ra_drw_wait_idle((uint32_t)k_drw_blend_idle_budget);
}
 ...
 *
 * @details
 * Sets COLOR1 to the foreground source via ``ra_drw_set_gradient`` (COLOR2 is
 * unused here but written for determinism), then programs ``ra_drw_blend_t``
 * for the standard source-over combination at the demo's global alpha. The
 * subsequent ``ra_drw_fill_rect`` composites COLOR1 over each destination
 * pixel instead of overwriting it.
 *
 * @par MC/DC:
 * Single decision ``err != k_ra_ok`` per call -- no compound condition.
 *
 * @return ``ra_err_t`` -- ``k_ra_ok`` once COLOR1/COLOR2 and CONTROL2 are set.
 * @retval k_ra_ok Blend unit armed.
 * @retval k_ra_err_null_ptr Internal descriptor pointer was NULL (cannot
 ...
 * @since 0.1.0
 */
[[nodiscard]] static ra_err_t drw_blend_arm(void)
{
  const ra_drw_gradient_t grad = {
    .color1_argb8888 = (uint32_t)k_drw_blend_fg_argb,
    .color2_argb8888 = (uint32_t)k_drw_blend_bg_argb,
  };
  const ra_err_t gerr = ra_drw_set_gradient(&grad);
  if (gerr != k_ra_ok) {
    return gerr;
  }
 ...
 *
 * @par MC/DC:
 * Each step is a single ``err != k_ra_ok`` early-return decision; no compound
 * conditions. The host test drives the success path plus one failure-injection
 * vector per step.
 *
 * @return ``ra_err_t`` -- ``k_ra_ok`` once all three layers are composited.
 * @retval k_ra_ok Framebuffer rendered and hashed.
 * @retval k_ra_err_null_ptr ``out_crc`` was NULL.
 * @retval k_ra_err_invalid_arg A layer rectangle was rejected.
 * @retval k_ra_err_hw_timeout The engine stayed busy past the budget.
 * @pre ::drw_blend_configure succeeded.
 ...
 * @since 0.1.0
 */
[[nodiscard]] static ra_err_t drw_blend_render(uint32_t* out_crc)
{
  RA_CHECK_NULL_PTR(out_crc, s_tag, "out_crc must not be nullptr");

  const ra_err_t bg = drw_blend_fill((int16_t)k_drw_blend_full_org,
  ...
  const ra_err_t sp = drw_blend_fill((int16_t)k_drw_blend_sp_xy,
  ...
  const ra_err_t armed = drw_blend_arm();
  if (armed != k_ra_ok) {
...
static void drw_blend_print_hex(uint32_t value)
{
  ...
  (void)ra_board_uart_console_write(out, (size_t)sizeof(out));
}

/** @brief Emit the success banner: prefix + 8 hex CRC + " PASS\r\n". */
static void drw_blend_print_pass(uint32_t crc)
{
  (void)ra_board_uart_console_write(k_drw_blend_pass_pfx,
                                    (size_t)(sizeof(k_drw_blend_pass_pfx) - 1U));
  drw_blend_print_hex(crc);
  (void)ra_board_uart_console_write(k_drw_blend_pass_sfx,
...
int32_t main(void)
{
  ...
  if (drw_blend_configure() != k_ra_ok) {
    drw_blend_panic_halt();
  }

  while (1) {
    uint32_t       crc = 0U;
    const ra_err_t err = drw_blend_render(&crc);
    if (err == k_ra_ok) {
      g_drw_blend_crc = crc;
      drw_blend_print_pass(crc);
      (void)ra_board_led_toggle(k_ra_board_led1);
    } else {
      (void)ra_board_uart_console_write(k_drw_blend_fail_msg,
                                        (size_t)(sizeof(k_drw_blend_fail_msg) - 1U));
      (void)ra_board_led_toggle(k_ra_board_led2);
    }
    ++g_drw_blend_heartbeat;
    ra_delay_ms(k_drw_blend_period_ms);
  }
  drw_blend_panic_halt();
  return 0;
}
