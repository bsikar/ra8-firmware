...
| drw_fill_demo | `tools/board_sim` shadows the DRW (2D Dave engine) control registers and `ra_drw_wait_idle` returns, but it models no rasterizer, so `ra_drw_fill_rect` never touches the framebuffer and the centre pixel stays clear (`match=N`). The actual fill needs silicon. The pixel-index math + the fill verdict (MC/DC, against a software fill model) are host-tested. Tracked in #120. |
...
| dotf_selftest_demo | Brings up the DOTF (Decryption On The Fly) block and runs its AES built-in self-test (`REG00` bit 20) on both channels -- key-free, no channel armed, no persistent writes. `tools/board_sim` does **not** model the DOTF APB window (`0x4026_8800`/`0x4026_8900`), so the self-test register reads are not a real AES core: the bring-up + self-test calls return `k_ra_ok` and the demo reports `ok=Y`, but the genuine AES BIST result + the in-place decryption path only exist on silicon (the opaque `REG00` snapshots go to `g_dotf_st0_snap`/`g_dotf_st1_snap`). The verdict (MC/DC) is host-tested. The on-silicon self-test result is the real acceptance. Tracked in #127. |
| sdhi_card_demo | Native **4-bit SDHI** microSD bring-up + FAT round-trip (the SDHI analogue of the SPI-mode `tz_secure_only_sd`): routes the eight SDHI pins, runs the full SD ident (CMD0..CMD7) via `ra_sdcard`/`ra_sdhi`, binds an `ra_fs` block backend, mounts + read/write-verifies a file. `tools/board_sim` does **not** model the SDHI controller or a card, so the ident times out (`SD_INFO1.RSPEND` never asserts) and the demo stops at `g_sdhi_stage = cardinit` (`sdhi: card init failed`) -- it runs without faulting on the unmodeled `0x4025_20xx` window but cannot reach `sdhi: roundtrip ok` without a real card. The payload + round-trip verdict (MC/DC) are host-tested. Tracked in #123. |
