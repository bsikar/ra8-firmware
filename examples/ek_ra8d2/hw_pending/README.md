...
| threadx_filex_levelx_demo | Local HIL (2026-06-10): the LevelX format failure (#87) is fixed, but FileX `fx_media_open` fails one layer up (FAT mount on the LevelX media) -- tracked in #88. `threadx_levelx_demo` (same LevelX stack) is fully validated and promoted to `hw_validated/hil/`, so the bus + LevelX are proven; only the FileX/adapter layer remains. |
