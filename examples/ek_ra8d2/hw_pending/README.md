...
| ra_sdhi_card_demo | **Raw-HAL SDHI block smoke test -- no ra_io, no ra_fs.** Drives the on-board microSD through the native SDHI 4-bit controller directly via `ra_sdcard` / `ra_sdhi`, then writes + reads back one raw 512-byte block (LBA 64) and byte-compares. `tools/board_sim` models the SDHI host controller (`board_periph_sdhi.c`) so the app reaches its PASS banner headlessly (`ra_sdhi_card_demo: native SDHI block round-trip PASS`). The bench run on real silicon confirms the HAL without any file-system layer. Tracked in #123. |
