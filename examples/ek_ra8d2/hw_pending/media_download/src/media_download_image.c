...
typedef struct {
  ...
  alignas(max_align_t) uint8_t bytes[k_ra8_compress_scratch_bytes];
} image_compressor_t;
 ...
 * @details Maps each bounded application array to the builder field that owns
 *          it without initializing or copying any payload bytes.
 ...
 * @details Resets both decoder arenas and assembles the comic and container
 *          workspaces over the application's statically reserved SDRAM.
 ...
 * @details Opens the committed RAM source, binds the bounded image pipeline,
 *          and emits one canonical RABOOK1 page wrapped in an RBKC container.
...
{
  if (s_output_format != k_mdl_format_rabook) {
 ...
 * @details Writes the same ordered byte span to the active storage stage and
 *          SHA-256 stream, rejecting a successful short storage write.
 ...
 * @details Streams the generated bytes through hashing and staged storage,
 *          validates the exact digest and length, then commits atomically.
 ...
 * @details Checks the complete storage and digest callback surface before any
 *          source fetch, conversion, or publication transaction begins.
                                                 ...
                                                 const ra8_mdl_transfer_config_t* transfer)
{
  ...
  if (err == k_ra8_ok) {
    ...
    source_transfer.format = k_mdl_format_loose;
