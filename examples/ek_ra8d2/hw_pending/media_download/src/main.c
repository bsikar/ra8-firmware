 ...
 *
 * The image is credential-free. A bounded versioned record supplies Wi-Fi and
 * URL configuration over the board console after boot. The app stays in
 * `hw_pending` until the exact mixed RA8/C6 image and removable-media path pass
 * physical fault-injection HIL.
 *
...
/** @brief Board-console binding for the shared runtime provisioner. */
 ...
 * handshake, starts Wi-Fi, and applies the supplied runtime station credentials.
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t internal_open_and_join(const ra8_net_credentials_t* credentials)
{
  if ((credentials == nullptr) || (credentials->ssid[0] == '\0')) {
 ...
 * @note The callback satisfies ::book_inflate_fn.
 * @since 0.1.0
 */
...
{
  const mdl_rabook_vfs_config_t reader_config = {
  ...
  ra8_err_t               err               = mdl_rabook_vfs_init(&s_rabook, &reader_config);
  ...
  if (err == k_ra8_ok) {
    ...
    err = mdl_storage_vfs_init(&s_storage, &storage_config, &storage_interface);
  }
  if (err == k_ra8_ok) {
...
{
  ra8_err_t     err       = mdl_rabook_vfs_open(&s_rabook, s_destination);
  book_header_t header    = {};
  uint64_t      flat_size = 0U;
  if (err == k_ra8_ok) {
    err = mdl_rabook_vfs_info(&s_rabook, &header, &flat_size);
  }
  ...
  if (err == k_ra8_ok) {
    err = mdl_rabook_vfs_read_chunk(&s_rabook, 0U, s_chunk, first_bytes);
  }
  const ra8_err_t close_err = mdl_rabook_vfs_close(&s_rabook);
  return ((err == k_ra8_ok) ? close_err : err);
}

/**
 * @brief Bind, execute, validate, and consume one configured media transfer.
 * @param[in] configuration Validated runtime configuration whose URL is nonempty.
 * @return Transfer or post-publication consumption status.
 * @retval k_ra8_ok The artifact was transferred and consumed successfully.
 * @pre @p configuration is non-NULL and remains live for the synchronous call.
 * @pre Storage, RSIP, and the C6 station are initialized.
 * @post Success leaves a validated artifact at ::s_destination.
 * @post The C6 link remains open for the caller to close.
 * @note Uses only bounded, caller-owned static workspaces.
 * @since 0.1.0
 */
RA8_INTERNAL static ra8_err_t
internal_transfer_and_consume(const ra8_net_credentials_t* configuration)
{
  ra8_mdl_transfer_config_t transfer = {};
  ra8_err_t                 err      = internal_bind_transfer(&transfer);
  if (err == k_ra8_ok) {
    if (s_source_is_image) {
      err = priv_media_download_image_run(&s_link, configuration->url, s_destination, &transfer);
    } else {
      ra8_mdl_transfer_result_t result = {};
      err = ra8_c6link_mdl_transfer(&s_link, configuration->url, s_destination, &transfer, &result);
    }
  }
  if (err == k_ra8_ok) {
 ...
 * @param[in,out] configuration Validated runtime network and URL configuration.
 * @pre Runtime URL and credentials are non-empty and bounded.
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static bool internal_run(ra8_net_credentials_t* configuration)
{
  if (s_port_err != k_ra8_ok) {
    return internal_fail("c6 port", s_port_err);
  }
  if ((configuration == nullptr) || (configuration->url[0] == '\0')) {
    return internal_fail("runtime configuration", k_ra8_err_invalid_arg);
  }
  ...
  err = internal_open_and_join(configuration);
  if (err != k_ra8_ok) {
    return internal_fail("c6 join", err);
  }
  ra8_secure_memzero(configuration->ssid, sizeof(configuration->ssid));
  ra8_secure_memzero(configuration->psk, sizeof(configuration->psk));
  configuration->ssid_len = 0U;
  configuration->psk_len  = 0U;
  err                     = internal_transfer_and_consume(configuration);
...
{
  (void)input;
  ra8_net_credentials_t configuration = {};
  ...
  if (provisioned == k_ra8_ok) {
    passed = internal_run(&configuration);
  } else {
    (void)internal_fail("runtime provisioning", provisioned);
  }
  ra8_net_provision_clear(&configuration);
  internal_heartbeat(passed);
}
