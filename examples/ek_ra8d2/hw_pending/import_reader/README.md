...
1. Brings up a micro-SD over `ra_sdmmc_spi` (Pmod2 / SCI0 Simple-SPI) and
   **mounts** the card's existing FAT volume with `ra_fs` (it does **not**
   reformat -- the source `.epub` is preserved).
2. Drives the `ra_rabook_import` cache manager wired to its **production** compile
   adapter `ra_rabook_import_compile_adapter` (which runs `ra_epub_open_fs` +
...
3. Reads the cached `.rabook` back, `ra_book_validate`s the bare (uncompressed)
...
- The cache uses the library's current **v1 root-level 8.3** name layout. The
  ...
  subdirectory write (`ra_fs_mkdir`, tracked by #151/#165) and is the next
  increment.
