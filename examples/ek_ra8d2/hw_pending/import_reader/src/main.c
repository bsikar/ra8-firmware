 ...
 *      it: `book_validate` accepts the bare (uncompressed) blob, and the app
...
[[gnu::section(".sdram_data"), gnu::aligned(8)]] static epub_book_t s_imp_epub;

/** @brief RABOOK1 builder arenas (one per table + the two pools + output). */
[[gnu::section(".sdram_data"),
  gnu::aligned(8)]] static book_chapter_t s_imp_chapters[k_imp_chapter_cap];
[[gnu::section(".sdram_data"), gnu::aligned(8)]] static book_node_t  s_imp_nodes[k_imp_node_cap];
[[gnu::section(".sdram_data"), gnu::aligned(8)]] static book_attr_t  s_imp_attrs[k_imp_attr_cap];
[[gnu::section(".sdram_data"),
  gnu::aligned(8)]] static book_stylesheet_t                         s_imp_styles[k_imp_style_cap];
[[gnu::section(".sdram_data"), gnu::aligned(8)]] static book_image_t s_imp_images[k_imp_image_cap];
...
static void imp_build_cookie(void)
{
  ...
  s_imp_cookie = (rabook_import_compiler_ctx_t){
    .epub               = &s_imp_epub,
    .cache              = &s_imp_cache,
...
static rabook_import_cfg_t imp_make_cfg(ra8_fs_mount_t* mount)
{
  ...
  cfg.mount               = mount;
  cfg.compile             = rabook_import_compile_adapter;
 ...
 * @retval k_ra8_err_*        `book_validate` / per-chapter walk error.
...
[[nodiscard]] static ra8_err_t imp_walk_book(uint32_t len)
{
  ...
  ra8_err_t   err  = book_validate(base, (size_t)len);
  if (err != k_ra8_ok) {
    return err;
  }
  const book_header_t* hdr = book_header(base);
  IMP_PUTS(k_msg_title);
  imp_print_cstr(book_string(base, hdr->title_off));
  ...
  for (uint32_t ci = 0U; ci < hdr->chapter_count; ci++) {
    size_t    tlen = 0U;
    ra8_err_t cerr = book_chapter_text(base, ci, (char*)s_imp_xhtml, sizeof(s_imp_xhtml), &tlen);
    if (cerr != k_ra8_ok) {
      return cerr;
    }
    IMP_PUTS(k_msg_ch);
...
{
  rabook_import_cfg_t     cfg                   = imp_make_cfg(mount);
  ...
  if (rabook_import_open(&cfg, k_imp_epub_path, path1, (uint32_t)sizeof(path1), &out) != k_ra8_ok) {
    imp_panic("import compile");
  }
  if (out != k_rabook_import_compiled) {
  ...
  if (rabook_import_open(&cfg, k_imp_epub_path, path2, (uint32_t)sizeof(path2), &out) != k_ra8_ok) {
    imp_panic("import reopen");
  }
  if (out != k_rabook_import_hit) {
