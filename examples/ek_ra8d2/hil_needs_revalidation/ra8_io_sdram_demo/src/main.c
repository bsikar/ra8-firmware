/**
 * @file examples/ek_ra8d2/hil_needs_revalidation/ra8_io_sdram_demo/src/main.c
 ...
 * backend differs, the ra8_fs + VFS layers above are identical and implemented in
 * this app's `ra8_io_roundtrip.{h,c}`. This app differs only by the ONE backend bind
 * line (`ra8_io_blockdev_sdram_init`, which brings the controller up with
 ...
 * @retval k_ra8_ok    The SDRAM volume mounted and the root file round-tripped.
...
{
  RA8_RETURN_ON_ERROR(ra8_io_blockdev_sdram_init(&s_bd, &s_bstate, (uint32_t)k_demo_disk_blocks),
                      s_tag,
                      "sdram blockdev init");
...
void main(void)
{
  ...
  demo_print("ra8_io_sdram_demo: boot\r\n");

  ra8_fs_mount_t* mnt = nullptr;
  const ra8_err_t e   = demo_run(&mnt);
  if (e == k_ra8_ok) {
    demo_print("ra8_io_sdram_demo: wrote/read 128 bytes dr:/HELLO.TXT PASS\r\n");
    if (ra8_io_roundtrip_subdir_file(&s_params) == k_ra8_ok) {
      demo_print("ra8_io_sdram_demo: mkdir+nested dr:/SUB/NOTE.TXT PASS\r\n");
    } else {
      demo_print("ra8_io_sdram_demo: mkdir FAIL\r\n");
    }
  } else {
    demo_print("ra8_io_sdram_demo: FAIL\r\n");
  }
