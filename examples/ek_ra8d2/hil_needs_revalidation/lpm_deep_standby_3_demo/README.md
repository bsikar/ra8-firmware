...
Deep Software Standby variant 3 (LPSCR.LPMD = 0xA) entry demo for
the bare EK-RA8D2 EVM.

Variant 3 is the deepest standby state the RA8D2 exposes. On top of
the variant 2 cuts (voltage-monitor and sub-clock-detection off),
variant 3 also stops the LOCO oscillator (HUM Ch 11.1 Table 11.3
p 431..432). Only the always-on wake-up detectors driven by the
sub-clock survive.

## What it does

1. CGC + SysTick + SCI8 + RTC + LPM bring-up.
2. Emit boot banner `lpm_dpsby3: boot` over SCI8.
...
5. `ra8_lpm_enter_sleep(k_ra8_sleep_mode_deep_standby_3)`.
6. On wake the chip RESETS -- control lands back in
   `Reset_Handler` and step 1 repeats.

## HIL gate

`HIL_MODE=uart_scrape` on `lpm_dpsby3: boot`. The deep-standby wake
...
2. Flash via the Pi-bound HIL flasher: `bash scripts/hil/flash.sh
   examples/ek_ra8d2/hil_needs_revalidation/lpm_deep_standby_3_demo/build/lpm_deep_standby_3_demo.hex`
