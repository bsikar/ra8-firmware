 ...
 * @details Sized for a ::k_sh_img_max_w-wide source row: ::book_src_image_rect
 ...
 *          bounded gray8 window row per frame via ::book_src_image_rect and
...
{
  return book_src_image_rect(src,
                             img,
                             0U,
                             sy,
                             (uint32_t)img->width,
...
{
  if (book_src_image(src, img_idx, out_img) != k_ra8_ok) {
    return false;
  }
  if (out_img->format != (uint8_t)k_book_image_gray4) {
    return false;
  }
 ...
 *         Each row is read as unpacked gray8 through ::book_src_image_rect,
 *         then repacked to the gray4 lens buffer. */
static bool sh_loupe_stage(const book_src_t*   src,
