...
typedef struct {
  ...
  book_src_t    book_src;      /**< Paged source over the open rabook.       */
 ...
 * @pre @p src was bound by book_src_paged() / book_src_resident().
...
/**
 * @brief ::comic_read_fn over the held-open SD book file (seek + clamped read).
 ...
 *          serves a short read at EOF instead of rejecting it -- comic's
 ...
 * @param[out] out_book epub_book_t* (void* to keep epub out of this header).
 ...
 * @param[in,out] book The epub_book_t* passed to ::sh_sd_open_epub.
 * @since 0.1.0
 */
...
/**
 * @brief Heap-free zlib inflater matching book_inflate_fn (miniz; main.c).
 ...
 *          sh_paged.c; `.epub` entries open through epub.
 ...
 *          book_src_prefetch_chapter() so that crossing finds them resident.
 ...
 * @details Binds ::comic over the entry's backing -- a baked MRAM blob, or
 ...
 * @details Opens each baked fixture through the SAME ::comic path the live
