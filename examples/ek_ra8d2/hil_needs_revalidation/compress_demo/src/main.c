 ...
 * the app hands a plain payload to ::ra8_vfs_compress_write and reads it
 * back with ::ra8_vfs_compress_read -- it NEVER touches the codec
 * (`ra8_compress` / `ra8_decompress`) or `ra8_fs_write_file` directly. The
 ...
 *   b. ::ra8_vfs_compress_write -- compresses + stores `STORY.RBK`.
 *   c. ::ra8_vfs_compress_read -- reads + inflates the blob.
 ...
 * `compress_demo: ram+dr 4096 -> N bytes -> 4096 round-trip PASS`.
 *
 ...
 *          ::ra8_vfs_compress_write (compress-on-write), reads it back with
 *          ::ra8_vfs_compress_read (decompress-on-read), and byte-compares
...
RA8_INTERNAL static ra8_err_t internal_demo_roundtrip(const char* prefix, uint32_t* out_blob_len)
{
  ...
  RA8_RETURN_ON_ERROR(ra8_vfs_compress_write(path,
                                             ...
                                             s_scratch,
                                             (uint32_t)k_ra8_compress_scratch_bytes,
                                             s_blob,
  ...
  uint32_t out_len = 0;
  RA8_RETURN_ON_ERROR(ra8_vfs_compress_read(path,
                                            s_blob,
                                            (uint32_t)k_demo_blob_cap,
                                            s_restored,
                                            (uint32_t)k_demo_payload,
