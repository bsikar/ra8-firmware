 ...
 * multi-paragraph chapter through `reflow` (fixed-metric Ahem face) into a
 * 160x192 RGB565 framebuffer, renders each page and folds an FNV-1a-32 over the
 * framebuffer, then calls reflow_set_font_size() to re-flow at a larger size
...
void main(void)
{
  ...
  if (reflow_init((uint16_t)k_rc_fb_w,
                  (uint16_t)k_rc_fb_h,
                  s_ahem_ttf,
  ...
  if (reflow_set_font_size(&s_engine, (uint16_t)k_rc_reflow_px) != k_ra8_ok) {
    internal_rc_panic_halt(s_msg_lerr, (uint32_t)sizeof(s_msg_lerr) - 1U);
  }
  uint32_t       crc2   = 0U;
