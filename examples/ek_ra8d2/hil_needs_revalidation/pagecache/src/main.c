 ...
 *   1. ROUND-TRIP: lay out a fixed chapter live, `reflow_cache_serialize` it,
 *      write the blob to SD, read it back, `reflow_cache_load` it into a fresh
...
typedef enum : uint32_t {
  ...
  k_pc_err_layout   = 5U,  /**< reflow_init / layout_chapter.        */
  k_pc_err_ser      = 6U,  /**< reflow_cache_serialize.              */
  k_pc_err_persist  = 7U,  /**< Write / read-back the blob on SD.    */
  k_pc_err_readback = 8U,  /**< Read-back blob != written blob.      */
  k_pc_err_load     = 9U,  /**< reflow_cache_load (expected hit).    */
 ...
 * @since 0.1.0
 */
// cppcheck-suppress constParameterCallback -- The set_clock vtable fixes void* ctx.
...
{
  return reflow_init((uint16_t)k_pc_view_w,
...
RA8_INTERNAL static void internal_pc_live_layout_or_halt(uint32_t font_len, size_t* out_n)
{
  ...
  g_pc_pages = pages;
  size_t n   = 0U;
  if (reflow_cache_serialize(&s_engine,
                             (const uint8_t*)s_pc_body,
                             (size_t)(sizeof(s_pc_body) - 1U),
                             s_live_blob,
...
void main(void)
{
   ...
   * -> reflow_cache round-trip + invalidation ran clean. */
  while (1) {
