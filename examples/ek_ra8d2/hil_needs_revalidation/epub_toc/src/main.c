 ...
 * @param[out] kind    Receives `epub_get_toc_kind`.
 * @param[out] count   Receives `epub_get_toc_count`.
...
{
  if (epub_open_streamed_fs(mount, path, &s_epub_io, &s_book) != k_ra8_ok) {
    etoc_fail(stage, fmsg, fmsglen);
  }
  uint8_t  k = 0U;
  uint16_t n = 0U;
  if ((epub_get_toc_kind(&s_book, &k) != k_ra8_ok) ||
      (epub_get_toc_count(&s_book, &n) != k_ra8_ok)) {
    etoc_fail(stage, fmsg, fmsglen);
  }
  *kind  = (uint32_t)k;
  *count = (uint32_t)n;
  ...
  *ch0   = (uint32_t)k_etoc_no_chap;
  if (n > 0U) {
    ...
    if ((epub_get_toc_entry(&s_book, 0U, &entry) != k_ra8_ok) ||
        (epub_toc_entry_to_chapter(&s_book, 0U, &chap) != k_ra8_ok)) {
      etoc_fail(stage, fmsg, fmsglen);
    }
    *e0crc = etoc_title_crc(entry.title);
    *ch0   = (uint32_t)chap;
  }
  (void)epub_close_streamed_fs(&s_epub_io, &s_book);
}
 ...
 * spine is still usable (`epub_get_chapter_count` == 2) so a renderer degrades
 * to the spine list rather than faulting.
 */
static void etoc_check_bad(ra8_fs_mount_t* mount)
{
  if (epub_open_streamed_fs(mount, k_etoc_path_bad, &s_epub_io, &s_book) != k_ra8_ok) {
    etoc_fail((uint32_t)k_etoc_err_bad, k_msg_fbad, (uint32_t)sizeof(k_msg_fbad) - 1U);
  }
  uint8_t  kind = (uint8_t)k_etoc_kind_unset;
  uint16_t chap = 0U;
  if ((epub_get_toc_kind(&s_book, &kind) != k_ra8_ok) ||
      (epub_get_chapter_count(&s_book, &chap) != k_ra8_ok)) {
    etoc_fail((uint32_t)k_etoc_err_bad, k_msg_fbad, (uint32_t)sizeof(k_msg_fbad) - 1U);
  }
