 ...
 * `epub_miniz_alloc` static arena -- `epub_open` (ZIP central directory)
 * and `epub_load_chapter` (DEFLATE decompressor) -- then CRC-32s chapter 0's
...
{
  const epub_mem_media_t media = {.data = k_epub_fixture, .size = (size_t)k_epub_fixture_len};
  ...
  size_t got = 0U;
  if ((epub_load_chapter(&s_book, 0U, s_chapter, (size_t)k_ep_chap_cap, &got) != k_ra8_ok) ||
      (got == 0U)) {
    ep_panic_halt(k_msg_load, (uint32_t)sizeof(k_msg_load) - 1U);
  }
  epub_metadata_t meta = {};
  if (epub_get_metadata(&s_book, &meta) != k_ra8_ok) {
