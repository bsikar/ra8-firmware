/**
 * @file examples/ek_ra8d2/hil_needs_revalidation/lpm_deep_standby_2_demo/src/main.c
...
static const uint8_t s_lpm_dpsby2_boot_msg[] = "lpm_dpsby2: boot\r\n";
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_lpm_dpsby2_panic_halt(void)
{
  while (1) {
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_lpm_dpsby2_setup_or_halt(void)
{
  uint32_t cpuclk0_hz = 0U;
  if (ra8_cgc_init() != k_ra8_ok) {
    internal_lpm_dpsby2_panic_halt();
  }
 ...
 * @since 0.1.0
 */
RA8_INTERNAL static void internal_lpm_dpsby2_add_offset(const ra8_rtc_datetime_t* now,
...
internal_lpm_dpsby2_arm_wake(const ra8_rtc_datetime_t* now)
{
  ra8_rtc_datetime_t a = {};
  internal_lpm_dpsby2_add_offset(now, &a);
...
void main(void)
{
  internal_lpm_dpsby2_setup_or_halt();
  ...
  (void)ra8_board_uart_console_write(s_lpm_dpsby2_boot_msg,
                                     (size_t)(sizeof(s_lpm_dpsby2_boot_msg) - 1U));
  ...
  while (1) {
    ...
    if (internal_lpm_dpsby2_arm_wake(&now) != k_ra8_ok) {
      break;
    }
