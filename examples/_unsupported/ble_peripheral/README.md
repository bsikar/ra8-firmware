...
the BLE controller via `ra8_ble_open`, layers the `ra8_ble_host` stack on
...
via `ra8_ble_host_gatt_notify`. Connection / disconnection / subscribe
...
3. SCI8 logging on PD_02 / PD_03 via `ra8_pfs_route_peripheral` +
   `ra8_sci_init` at 115200 8N1.
4. `ra8_ble_host_init({ role = peripheral, name = "EK-RA8D2" })`.
5. `ra8_ble_host_attach_event_handler` to log connect / subscribe /
...
7. Build a 31-byte legacy AD payload (Flags + Complete UUID16 List +
   Complete Local Name) and `ra8_ble_host_advertise_start` at 100 ms
   interval.
8. Loop: every 10 s decrement the battery value, push it into the
   value cache via `ra8_ble_host_gatt_set_value`, and call
   `ra8_ble_host_gatt_notify` to fan out a Handle Value Notification to
   subscribed peers.
...
Uses `ra8_board_ek_ra8d2` BSP for LED1 init/toggle (LED1 = P600 per
EK-RA8D2 v1 UM Table 24 p 31). The SCI8 / J-Link OB CDC pins (PD02 /
PD03) are routed directly via `ra8_pfs_route_peripheral`; PD02 / PD03
...
SCI3-based `ra8_board_uart_console_init` veneer.
