...
if(NOT RA8_USE_THREADX)
    message(FATAL_ERROR
        "RA8_USE_USBX=ON requires RA8_USE_THREADX=ON. USBX's port glue "
        "calls into ThreadX semaphores / threads. Enable both options.")
endif()

if(NOT DEFINED RA8_REPO_ROOT)
    get_filename_component(RA8_REPO_ROOT
...
if(NOT EXISTS "${_RA8_USBX_COMMON_INC}/ux_api.h")
    message(FATAL_ERROR
        "RA8_USE_USBX=ON but USBX vendor tree is missing at "
...
file(GLOB _RA8_USBX_CORE_SOURCES CONFIGURE_DEPENDS
    "${_RA8_USBX_COMMON_SRC}/*.c")
list(FILTER _RA8_USBX_CORE_SOURCES
...
list(FILTER _RA8_USBX_MSC_SOURCES
...
target_include_directories(usbx_objs PUBLIC
    ${_RA8_USBX_COMMON_INC}
    ${_RA8_USBX_DEV_CLS_INC}
    ${_RA8_USBX_PORT_INC})
...
set(RA8_USBX_REQUEST_DATA_MAX_LENGTH 4096 CACHE STRING
    "USBX device transfer-request buffer / MSC bulk chunk size (bytes)")
target_compile_definitions(usbx_objs PUBLIC
    UX_SLAVE_REQUEST_DATA_MAX_LENGTH=${RA8_USBX_REQUEST_DATA_MAX_LENGTH})
target_compile_definitions(usbx INTERFACE
    UX_SLAVE_REQUEST_DATA_MAX_LENGTH=${RA8_USBX_REQUEST_DATA_MAX_LENGTH})

# Pull in the project's ra8_usb <-> USBX bridge.
add_subdirectory(${RA8_REPO_ROOT}/port/usbx
