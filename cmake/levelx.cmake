...
# Exposes the `RA_USE_LEVELX` option; when ON, this file:
#
...
# levelx levelx_port_ra_xspi)`. Everything else (include dirs,
...
if(DEFINED _RA_LEVELX_INCLUDED)
    return()
endif()
set(_RA_LEVELX_INCLUDED TRUE)

option(RA_USE_LEVELX "Enable the vendored LevelX NOR wear-levelling library" OFF)

if(NOT RA_USE_LEVELX)
    return()
endif()
...
if(NOT RA_USE_THREADX)
    message(FATAL_ERROR
        "RA_USE_LEVELX=ON requires RA_USE_THREADX=ON. LevelX's protection "
...
get_filename_component(_RA_LEVELX_REPO_ROOT
    "${CMAKE_CURRENT_LIST_DIR}/.." ABSOLUTE)

set(_RA_LEVELX_VENDOR_DIR "${_RA_LEVELX_REPO_ROOT}/libs/third_party/levelx")
set(_RA_LEVELX_COMMON_INC "${_RA_LEVELX_VENDOR_DIR}/common/inc")
set(_RA_LEVELX_COMMON_SRC "${_RA_LEVELX_VENDOR_DIR}/common/src")

if(NOT EXISTS "${_RA_LEVELX_COMMON_INC}/lx_api.h")
    message(FATAL_ERROR
        "RA_USE_LEVELX=ON but LevelX vendor tree is missing at "
        "${_RA_LEVELX_VENDOR_DIR}.")
endif()
...
# `port/levelx/lx_nor_driver_ra_xspi.c` shim, so dragging the NAND
# code in only inflates the build and the warning surface.
file(GLOB _RA_LEVELX_NOR_SOURCES CONFIGURE_DEPENDS
    "${_RA_LEVELX_COMMON_SRC}/lx_nor_*.c")
...
list(FILTER _RA_LEVELX_NOR_SOURCES
...
add_library(levelx_objs OBJECT ${_RA_LEVELX_NOR_SOURCES})

target_include_directories(levelx_objs PUBLIC
    ${_RA_LEVELX_COMMON_INC})
...
target_compile_options(levelx_objs PRIVATE
    -w)

add_library(levelx INTERFACE)
target_sources(levelx INTERFACE $<TARGET_OBJECTS:levelx_objs>)
target_include_directories(levelx INTERFACE
    ${_RA_LEVELX_COMMON_INC})
target_link_libraries(levelx INTERFACE threadx)

# Now pull in our ra_xspi <-> LevelX shim.
add_subdirectory(${_RA_LEVELX_REPO_ROOT}/port/levelx
                 ${CMAKE_BINARY_DIR}/port_levelx)

message(STATUS "LevelX enabled: ${_RA_LEVELX_VENDOR_DIR}")
