...
get_filename_component(_RA8_MBEDTLS_REPO_ROOT
...
if(NOT EXISTS "${_RA8_MBEDTLS_INC_DIR}/mbedtls/build_info.h")
    message(FATAL_ERROR
        "RA8_USE_MBEDTLS=ON but Mbed TLS vendor tree is missing at "
        "${_RA8_MBEDTLS_VENDOR_DIR}.")
endif()

if(NOT EXISTS "${_RA8_MBEDTLS_CONFIG_FILE}")
    message(FATAL_ERROR
        "RA8_USE_MBEDTLS=ON but the project Mbed TLS config "
...
# include path.
file(GLOB _RA8_MBEDTLS_LIB_SOURCES CONFIGURE_DEPENDS
    "${_RA8_MBEDTLS_LIB_DIR}/*.c")

file(GLOB _RA8_TFPSA_CORE_SOURCES CONFIGURE_DEPENDS
    "${_RA8_TFPSA_CORE_DIR}/*.c")

file(GLOB _RA8_TFPSA_DRIVER_SOURCES CONFIGURE_DEPENDS
    "${_RA8_TFPSA_DRIVERS_SRC_DIR}/*.c")

file(GLOB _RA8_TFPSA_PLATFORM_SOURCES CONFIGURE_DEPENDS
    "${_RA8_TFPSA_PLATFORM_DIR}/*.c")

file(GLOB _RA8_TFPSA_UTIL_SOURCES CONFIGURE_DEPENDS
    "${_RA8_TFPSA_UTILITIES_DIR}/*.c")

file(GLOB _RA8_TFPSA_EXTRAS_SOURCES CONFIGURE_DEPENDS
    "${_RA8_TFPSA_EXTRAS_DIR}/*.c")
...
target_compile_definitions(mbedtls_objs PUBLIC
    MBEDTLS_CONFIG_FILE="${_RA8_MBEDTLS_CONFIG_FILE}")
...
if(EXISTS "${_RA8_TFPSA_CRYPTO_CONFIG}")
    target_compile_definitions(mbedtls_objs PUBLIC
        TF_PSA_CRYPTO_CONFIG_FILE="${_RA8_TFPSA_CRYPTO_CONFIG}")
endif()
