...
#
# Exposes the `RA_USE_NETXDUO` option; when ON, this file:
#
...
#      Ethernet driver entry point (`nx_ether_driver_ra_eth`) and the
#      `_nx_crypto_method_aes_*` / `_nx_crypto_method_sha256_*` ALT
#      replacements that route to `ra_rsip_aes_cipher` and
#      `ra_rsip_sha256` on real silicon.
#
...
# through ``ra_rsip_aes_cipher`` / ``ra_rsip_sha256`` on AES-128 ECB
...
# netxduo netxduo_port_ra_eth)` -- everything else (include dirs,
...
# Idempotency guard.
if(DEFINED _RA_NETXDUO_INCLUDED)
    return()
endif()
set(_RA_NETXDUO_INCLUDED TRUE)

option(RA_USE_NETXDUO "Enable the vendored NetX Duo TCP/IP stack" OFF)
...
if(NOT RA_USE_THREADX)
    message(FATAL_ERROR
        "RA_USE_NETXDUO=ON requires RA_USE_THREADX=ON. NetX Duo's "
...
get_filename_component(_RA_NETXDUO_REPO_ROOT
    "${CMAKE_CURRENT_LIST_DIR}/.." ABSOLUTE)

set(_RA_NETXDUO_VENDOR_DIR  "${_RA_NETXDUO_REPO_ROOT}/libs/third_party/netxduo")
set(_RA_NETXDUO_COMMON_INC  "${_RA_NETXDUO_VENDOR_DIR}/common/inc")
set(_RA_NETXDUO_COMMON_SRC  "${_RA_NETXDUO_VENDOR_DIR}/common/src")
set(_RA_NETXDUO_CRYPTO_INC  "${_RA_NETXDUO_VENDOR_DIR}/crypto_libraries/inc")
set(_RA_NETXDUO_CRYPTO_SRC  "${_RA_NETXDUO_VENDOR_DIR}/crypto_libraries/src")
set(_RA_NETXDUO_PORT_INC    "${_RA_NETXDUO_VENDOR_DIR}/ports/cortex_m85/gnu/inc")

if(NOT EXISTS "${_RA_NETXDUO_COMMON_INC}/nx_api.h")
    message(FATAL_ERROR
        "RA_USE_NETXDUO=ON but NetX Duo vendor tree is missing at "
        "${_RA_NETXDUO_VENDOR_DIR}.")
endif()

# ---------------------------------------------------------------------------
# Common NetX Duo + NetX Crypto sources.
# ---------------------------------------------------------------------------
file(GLOB _RA_NETXDUO_COMMON_SOURCES CONFIGURE_DEPENDS
    "${_RA_NETXDUO_COMMON_SRC}/*.c")
file(GLOB _RA_NETXDUO_CRYPTO_SOURCES CONFIGURE_DEPENDS
    "${_RA_NETXDUO_CRYPTO_SRC}/*.c")
...
list(FILTER _RA_NETXDUO_CRYPTO_SOURCES
    EXCLUDE REGEX ".*/nx_crypto_generic_ciphersuites\\.c$")

set(_RA_NETXDUO_SECURE_INC "${_RA_NETXDUO_VENDOR_DIR}/nx_secure/inc")

add_library(netxduo_objs OBJECT
    ${_RA_NETXDUO_COMMON_SOURCES}
    ${_RA_NETXDUO_CRYPTO_SOURCES}
)

target_include_directories(netxduo_objs PUBLIC
    ${_RA_NETXDUO_COMMON_INC}
    ${_RA_NETXDUO_CRYPTO_INC}
    ${_RA_NETXDUO_SECURE_INC}
    ${_RA_NETXDUO_PORT_INC})
...
set(_RA_NETXDUO_WRAP_FLAGS "")
foreach(_sym IN LISTS _RA_NETXDUO_WRAP_SYMS)
    list(APPEND _RA_NETXDUO_WRAP_FLAGS "-Wl,--wrap=${_sym}")
endforeach()
...
target_link_options(netxduo INTERFACE ${_RA_NETXDUO_WRAP_FLAGS})

# Now pull in our ra_eth + ra_rsip <-> NetX Duo shim.
add_subdirectory(${_RA_NETXDUO_REPO_ROOT}/port/netxduo
                 ${CMAKE_BINARY_DIR}/port_netxduo)

message(STATUS "NetX Duo enabled: ${_RA_NETXDUO_VENDOR_DIR}")
