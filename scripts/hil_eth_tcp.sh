...
#
# Usage:
#   scripts/hil_eth_tcp.sh --hex <path> --board-ip <a.b.c.d> \
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --boot-timeout)   BOOT_TIMEOUT_S="$2"; shift 2 ;;
...
case "$PROTO" in tcp|udp|http) ;; *) echo "bad --proto $PROTO"; usage ;; esac
...
if ! ip -4 addr show dev "$USB_ETH_IFACE" | grep -q "${PI_IP}/${PI_PREFIX}"; then
    sudo -n ip addr add "${PI_IP}/${PI_PREFIX}" dev "$USB_ETH_IFACE" 2>/dev/null \
        || { echo -e "${RED}[HIL]${NC} could not assign ${PI_IP} to ${USB_ETH_IFACE} (need sudo NOPASSWD on ip)"; exit 1; }
fi
...
if [[ -f "$ELF_PATH" ]] && command -v arm-none-eabi-nm >/dev/null 2>&1; then
    ...
    if [[ -n "$DIAG_ADDR" ]]; then
        ...
        # 21 uint32_t fields = 84 bytes = mem32 21 words.
        cat > "$DIAG_TMP" <<DIAG
device ${JLINK_DEVICE}
si SWD
speed 1000
