...
CANONICAL_PROJECT_FIELDS = ("Status", "Track", "Horizon", "Priority")
REQUIRED_PROJECT_FIELDS = ("Status", "Track", "Priority")
PROJECT_LIMIT = "2500"
...
def _field_command(schema: TrackerSchema) -> list[str]:
    return [
        "gh",
        "project",
        "field-list",
...
def _label_command(schema: TrackerSchema) -> list[str]:
    return [
        "gh",
        "api",
        "--paginate",
        "--slurp",
        f"repos/{schema.repository}/issues?state=all&per_page=100",
    ]
...
def _run_gh_readonly(
    ...
    allowed = {
        _tuple(_explorer_command(schema)),
        _tuple(_field_command(schema)),
        _tuple(_label_command(schema)),
    }
...
def _read_board_data(schema: TrackerSchema, command: list[str], prefix: str) -> str:
    """Run one allowed board read and raise one prefixed transport failure."""
    try:
        proc = _run_gh_readonly(schema, command)
    except ToolMissingError:
        raise
    except WorkError as exc:
        error = WorkError(f"{prefix}: {exc}")
        raise error from exc
    if not proc.ok:
        message = proc.stderr.strip() or proc.stdout.strip() or f"gh exited {proc.returncode}"
        error = WorkError(f"{prefix}: {message}")
        raise error
    return proc.stdout
...
def _parse_project_fields(text: str) -> None:
    """Reject lossy field-name mappings before item-list serializes values."""
    root = _strict_object(_strict_json(text), "GitHub project fields response")
    raw_fields = root.get("fields")
    total = root.get("totalCount")
    if not isinstance(raw_fields, list):
        _fail_data("GitHub project fields must be a list")
    if not isinstance(total, int) or isinstance(total, bool) or total != len(raw_fields):
        _fail_data("GitHub project fields totalCount must equal fields length")
    seen: dict[str, str] = {}
    for raw in raw_fields:
        field = _strict_object(raw, "GitHub project field")
        raw_name = field.get("name")
        name = _strict_string(raw_name, "GitHub project field name")
        if name != raw_name:
            _fail_data(f"GitHub project field name must not have surrounding whitespace: {name}")
        folded = name.casefold()
        if folded in seen:
            _fail_data(
                f"GitHub project fields collide case-insensitively: {seen[folded]} and {name}"
            )
        seen[folded] = name
    missing = [name for name in REQUIRED_PROJECT_FIELDS if name.casefold() not in seen]
    if missing:
        _fail_data(f"GitHub project is missing required fields: {', '.join(missing)}")
    for expected in CANONICAL_PROJECT_FIELDS:
        actual = seen.get(expected.casefold())
        if actual is not None and actual != expected:
            _fail_data(f"GitHub project field {actual} must use canonical name {expected}")


def _parse_repository_issue_labels(text: str) -> dict[int, tuple[str, ...]]:
    """Read labels from every page of the repository Issues endpoint."""
    pages = _strict_json(text)
    if not isinstance(pages, list):
        _fail_data("GitHub repository issues response must be a list of pages")
    result: dict[int, tuple[str, ...]] = {}
    for page in pages:
        if not isinstance(page, list):
            _fail_data("GitHub repository issues page must be a list")
        for raw in page:
            issue = _strict_object(raw, "GitHub open issue")
            if "pull_request" in issue:
                continue
            number = _strict_positive_int(issue.get("number"), "GitHub repository issue number")
            if number in result:
                _fail_data(f"GitHub repository issues response contains duplicate issue #{number}")
            result[number] = _detail_labels(issue.get("labels"))
    return result


def _classify_labels(
    labels: tuple[str, ...], number: int
    ...
    if any(not value for value in (*effort, *epic)):
        _fail_data(f"project item #{number} has an empty derived label")
    return priority, effort, epic
...
def _parse_item(raw: object, schema: TrackerSchema) -> BoardItem:
    ...
    priority_labels, effort_labels, epic_labels = _classify_labels(labels, number)
    return BoardItem(
        number=number,
        title=title,
        url=url,
...
def _fetch_snapshot(schema: TrackerSchema) -> tuple[tuple[BoardItem, ...] | None, int]:
    try:
        _parse_project_fields(
            _read_board_data(schema, _field_command(schema), "error fetching project fields")
        )
        items = _parse_snapshot(
            _read_board_data(schema, _explorer_command(schema), "error fetching board snapshot"),
            schema,
        )
        labels_by_number = _parse_repository_issue_labels(
            _read_board_data(schema, _label_command(schema), "error fetching issue labels")
        )
        hydrated: list[BoardItem] = []
        for item in items:
            labels = labels_by_number.get(item.number)
            if labels is None:
                _fail_data(
                    f"open project issue #{item.number} is absent from repository issue data"
                )
            priority, effort, epic = _classify_labels(labels, item.number)
            hydrated.append(
                replace(
                    item,
                    labels=labels,
        ...
        _error(f"error parsing GitHub board data: {exc}")
        return None, 1
...
def _parse_connection(value: object, context: str) -> RelationConnection:
    ...
    urls = [relation.url for relation in parsed]
    if len(urls) != len(set(urls)):
        _fail_data(f"{context} contains duplicate issue URLs")
