...
field_command = vars(board)["_field_command"]
issue_command = vars(board)["_issue_command"]
label_command = vars(board)["_label_command"]
parse_detail = vars(board)["_parse_detail"]
parse_repository_issue_labels = vars(board)["_parse_repository_issue_labels"]
parse_project_fields = vars(board)["_parse_project_fields"]
...
class DetailParser(unittest.TestCase):
    ...
    def test_duplicate_connection_url_is_rejected(self) -> None:
        """Verify one relationship cannot be repeated under the same URL."""
        payload = json.loads(fixture("issue_detail.json"))
        payload["subIssues"]["nodes"].append(payload["subIssues"]["nodes"][0])
        payload["subIssues"]["totalCount"] = 2
        with self.assertRaisesRegex(board.BoardDataError, "duplicate issue URLs"):
            parse_detail(json.dumps(payload))

    def test_same_issue_number_in_two_repositories_is_not_a_duplicate(self) -> None:
        """Verify cross-repository relations use their canonical URLs as identity."""
        payload = json.loads(fixture("issue_detail.json"))
        second = dict(payload["subIssues"]["nodes"][0])
        second["url"] = "https://github.com/example/other/issues/31"
        payload["subIssues"]["nodes"].append(second)
        payload["subIssues"]["totalCount"] = 2
        detail = parse_detail(json.dumps(payload))
        self.assertEqual(len(detail.sub_issues.nodes), 2)
...
class CommandAndCliContract(unittest.TestCase):
    ...
    def test_field_and_label_commands_are_fixed_read_shapes(self) -> None:
        """Verify completeness reads pin the project and repository."""
        self.assertEqual(field_command(schema())[1:3], ["project", "field-list"])
        self.assertEqual(
            label_command(schema()),
            [
                "gh",
                "api",
                "--paginate",
                "--slurp",
                "repos/bsikar/ra8-firmware/issues?state=all&per_page=100",
            ],
        )

    def test_project_field_case_collision_is_rejected_before_item_loading(self) -> None:
        """Verify CLI key normalization cannot silently overwrite a field."""
        fields = {
            "totalCount": 5,
            "fields": [
                {"name": "Status"},
        ...
        with self.assertRaisesRegex(board.BoardDataError, "collide case-insensitively"):
            parse_project_fields(json.dumps(fields))

    def test_project_field_casing_must_match_serialized_keys(self) -> None:
        """Verify lower-first CLI serialization cannot hide a required value."""
        fields = {
            "totalCount": 3,
            "fields": [{"name": "STATUS"}, {"name": "Track"}, {"name": "Priority"}],
        }
        with self.assertRaisesRegex(board.BoardDataError, "must use canonical name Status"):
            parse_project_fields(json.dumps(fields))

    def test_optional_horizon_casing_must_match_serialized_key(self) -> None:
        """Verify a present optional Horizon field cannot masquerade as absent."""
        fields = {
            "totalCount": 4,
            "fields": [
                {"name": "Status"},
        ...
        with self.assertRaisesRegex(board.BoardDataError, "must use canonical name Horizon"):
            parse_project_fields(json.dumps(fields))

    def test_canonical_field_names_reject_surrounding_whitespace(self) -> None:
        """Verify trimming cannot conceal the key emitted by the project CLI."""
        for index, padded in enumerate((" Status", "Track ", " Horizon ", "Priority\t")):
            with self.subTest(padded=padded):
                ...
                fields["fields"][index]["name"] = padded
                with self.assertRaisesRegex(board.BoardDataError, "surrounding whitespace"):
                    parse_project_fields(json.dumps(fields))

    def test_missing_optional_horizon_reaches_focus_fallback(self) -> None:
        """Verify field preflight preserves the documented no-Horizon behavior."""
        payload = json.loads(fixture("board_snapshot.json"))
        ...
        responses = (
            ...
            Completed(argv=(), returncode=0, stdout=json.dumps(payload), stderr=""),
            Completed(argv=(), returncode=0, stdout=json.dumps([issue_rows]), stderr=""),
        )
        with patch.object(board, "_run_gh_readonly", side_effect=responses):
            items, code = fetch_snapshot(schema())
        self.assertEqual(code, 0)
        self.assertIsNotNone(items)
        self.assertEqual(tuple(item.horizon for item in items or ()), (None,) * 6)
        output = io.StringIO()
        with patch("sys.stdout", output):
            emit(render_focus(schema(), items or ()))
        self.assertIn("Horizon is unavailable", output.getvalue())

    def test_paginated_issue_labels_preserve_more_than_ten(self) -> None:
        """Verify classification receives the complete REST label collection."""
        labels = [{"name": f"ordinary-{index}"} for index in range(11)]
        labels.append({"name": "needs-bench"})
        parsed = parse_repository_issue_labels(json.dumps([[{"number": 13, "labels": labels}]]))
        self.assertEqual(len(parsed[13]), 12)
        self.assertIn("needs-bench", parsed[13])

    def test_issue_closed_after_snapshot_still_hydrates_labels(self) -> None:
        """Verify state changes between reads do not invalidate the board snapshot."""
        payload = json.loads(fixture("board_snapshot.json"))
        ...
        closed_issue = {
            "number": 11,
            "state": "closed",
            "labels": [{"name": "effort:L"}],
        }
        responses = (
            Completed(argv=(), returncode=0, stdout=json.dumps(fields), stderr=""),
            Completed(argv=(), returncode=0, stdout=json.dumps(payload), stderr=""),
            Completed(argv=(), returncode=0, stdout=json.dumps([[closed_issue]]), stderr=""),
        )
        with patch.object(board, "_run_gh_readonly", side_effect=responses):
            items, code = fetch_snapshot(schema())
        self.assertEqual(code, 0)
        self.assertEqual(tuple(item.effort_labels for item in items or ()), (("L",),))
