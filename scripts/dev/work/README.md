...
views: they allow only exact project-field, project-item, repository-issue, and
issue-detail read argument vectors, pin `GH_HOST=github.com`, remove `GH_REPO`,
and use the repository/project target from `tracker.json`. The separate field
and repository-issue reads prevent GitHub CLI serialization limits from hiding
case-colliding fields or labels. `work plan
