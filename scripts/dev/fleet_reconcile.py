...
@dataclass(frozen=True)
class ApplyRecovery:
    """How many parked applies to spend on one host, and what to do once spent."""
...
def apply_host(
    ...
    *,
    ...
    recovery: ApplyRecovery | None = None,
) -> tuple[bool, int]:
    """Apply one host and prove the resulting declaration is idempotent."""
    policy = recovery if recovery is not None else ApplyRecovery()
    applied = False
    for attempt in range(1, policy.attempts + 1):
        ...
        if attempt < policy.attempts:
            print(
                ...
                f"(attempt {attempt}/{policy.attempts}); retrying while capacity stays zero",
                file=sys.stderr,
            )
    if not applied:
        quarantine(host, run)
        if policy.on_mutation_exhausted is not None:
            policy.on_mutation_exhausted()
        return False, 0
...
def reconcile_host(
    ...
    applied, _ = apply_host(
        ...
        recovery=ApplyRecovery(attempts=attempts, on_mutation_exhausted=reopen_last_known_good),
    )
    if not applied:
