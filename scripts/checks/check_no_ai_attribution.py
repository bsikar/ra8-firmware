...
# ra8_viewer, ra8_fmt), .github/, .devcontainer/, cmake/, esp32/ and the whole
...
def _in_scan_scope(rel: str) -> bool:
    """Whether repo-relative ``rel`` is a text file this ban inspects.

    Pure and total, so the selftest can assert scope on synthetic paths without
    touching the tree: a re-narrowing that drops ``tools/`` fails the selftest
    instead of passing silently.

    ``first_party_paths`` has already removed vendored SOUP and build output;
    this adds the extension filter and the generated-doc-tree skip that git
    itself may not exclude.
    """
    if Path(rel).suffix.lower() not in TEXT_EXTS:
        return False
    wrapped = "/" + rel + "/"
    return not any(frag in wrapped for frag in SKIP_FRAGMENTS)


def _iter_files() -> Iterable[Path]:
    """Every tracked first-party text file, derived from git ls-files.
    ...
    for rel in first_party_paths(text_suffixes, respect_language_excludes=False):
...
_BAD_FIXTURE = (
    ...
    "/* This driver was generated by ChatGPT. */\n"
    "// see the github.copilot extension\n"
    "# authored with Cursor Composer\n"
)
_GOOD_FIXTURE = (
    "// The house rules live in CLAUDE.md, config under .claude/.\n"
    "// Uses the ra8_gpt3 channel and the GPT13 PWM timer.\n"
    "// Co-Authored-By: Brighton Sikarskie <bsikar@tuta.io>\n"
    "// Names a banned token but carries a reason. AI-OK: quoting policy\n"
)


def _scan_text(text: str) -> list[str]:
    """Every offending token in ``text``, honouring the per-line AI-OK opt-out."""
    hits: list[str] = []
    for line in text.splitlines():
        if _line_has_opt_out(line):
            continue
        hits.extend(_scan_line(line))
    return hits


def selftest() -> int:
    """Prove the ban fires, stays quiet on legal prose, and now enumerates tools/."""
    print("check_no_ai_attribution.py --selftest")
    failures: list[str] = []
    expect(bool(_scan_text(_BAD_FIXTURE)), "bad fixture reports AI attribution", failures)
    ...
    scanned = {str(p.relative_to(REPO_ROOT)) for p in _iter_files()}
    expect(
        any(s.startswith("tools/") for s in scanned),
        "the live enumeration actually reaches tools/",
        failures,
    )
