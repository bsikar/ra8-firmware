...
selftest_scope() {
  ...
  # path (an empty registry silently lints generated bytes again), every path
  # it names must be OUT of the collection, and a hand-authored neighbour in
  # the same directory must still be IN it -- so a carve-out that widened from
  # the exact path to its whole directory fails here rather than reporting a
  # smaller, cleaner run.
  local generated_list rel
