...
Waivers
-------
A generated file may waive the cap with a standalone comment in its head that
names a tracked generator and a recipe/reason. A hand-authored exception uses
the equally strict ``FILE-SIZE-OK: <reason>`` standalone-comment grammar.
...
from suppression_generated_markers import path_has_effective_waiver

# Maintainability cap: a single source file should stay reviewable.
THRESHOLD_LINES = 1000


def _is_waived(path: Path, tracked_paths: frozenset[str]) -> bool:
    """Return whether the shared canonical head-waiver grammar accepts `path`."""
    return path_has_effective_waiver(path, REPO_ROOT, tracked_paths)
...
def over_cap(targets: list[Path]) -> list[tuple[int, str]]:
    ...
    over = []
    grouped = files_for()
    tracked_paths = frozenset(rel for paths in grouped.values() for rel in paths)
    for path in targets:
        ...
        if language_of(_rel(path)) is None or _is_waived(path, tracked_paths):
            continue
...
_SELFTEST_CASES = (
    ...
    (
        ...
        0,
        "a canonical generated marker waives when its generator resolves",
    ),
    (
        "marker_only.py",
        "# @generated by tools/generator.py -- fixture recipe\n" + "\n" * _OVER,
        1,
        "a marker followed only by blank lines does not waive",
    ),
    (
        "comment_only.py",
        "# @generated by tools/generator.py -- fixture recipe\n" + "# fixture\n" * _OVER,
        1,
        "a marker followed only by comments does not waive",
    ),
    (
        "waived.py",
    ...
    (
        "prose.py",
        'text = "@generated by tools/generator.py -- string data"\n' + "x\n" * _OVER,
        1,
        "generated prose/string data does not waive",
    ),
    (
        "bare.py",
        "# @generated\n" + "x\n" * _OVER,
        1,
        "a generator-less marker does not waive",
    ),
)
...
def _selftest_cap(cases: tuple) -> list[str]:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for name, body, expected, description in cases:
            ...
            # The production helper requires generated provenance to resolve
            # beneath REPO_ROOT. Use a real tracked generator in these local
            # fixtures by substituting its path rather than weakening that rule.
            if "tools/generator.py" in body:
                tracked = REPO_ROOT / "scripts" / "gen" / "rabook_parity_gen.py"
                rewritten = body.replace("tools/generator.py", str(tracked.relative_to(REPO_ROOT)))
