...
def _selftest() -> int:
    ...
    A size gate whose enumeration has gone stale reports a clean tree, which is
    the failure this checker was rewritten to end. The scope assertions below
    are therefore as load-bearing as the threshold ones.
    """
    failures: list[str] = []
    over = THRESHOLD_LINES + 1
    cases = (
        ("over.c", "x\n" * over, 1, "a C file over the cap fires"),
        ("under.c", "x\n" * THRESHOLD_LINES, 0, "a C file exactly at the cap is clean"),
        ("over.py", "x\n" * over, 1, "a Python file over the cap fires"),
        ("over.sh", "x\n" * over, 1, "a shell file over the cap fires"),
        ("over.cmake", "x\n" * over, 1, "a CMake file over the cap fires"),
        ("over.md", "x\n" * over, 0, "Markdown is prose and out of scope"),
        ("over.json", "x\n" * over, 0, "a data file is out of scope"),
        (
            "gen.py",
            "# @generated by a tool\n" + "x\n" * over,
            0,
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # 670 lines of exactly it.
        hook = root / "pre-commit-like"
        hook.write_text("#!/usr/bin/env bash\n" + "x\n" * over)
        if len(over_cap([hook])) != 1:
            failures.append("  FAIL an extensionless shebang script is in scope")

    # The live scope must reach every language, or the gate has silently
    # narrowed again. Zero files for a language is always a broken enumeration.
    grouped = files_for()
    ...
    if failures:
        ...
        return 1
    print(
        f"check_file_size.py: --selftest OK ({len(cases) + 1} cases both directions; "
        f"live scope covers {len(grouped)} language(s), "
