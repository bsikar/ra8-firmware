...
#
#   dev     | star@star.local  | ssh -o BatchMode=yes dev bash -s
#   truenas | bench-star       | ssh -o BatchMode=yes truenas bash -s
#   wsl     | star@10.0.40.101 | ssh -o BatchMode=yes -J star user@host wsl -e bash -s
#   local   | star@star.local  | bash -s
#
...
bc_on() {
  local i="$1"
  # shellcheck disable=SC2086  # the transport is a word list by contract; see the roster format above.
  ${BC_TRANSPORTS[$i]}
}
...
bc_app_dir() {
  ...
  [ -n "$d" ] || {
    bc_say "app '$1' not found under examples/ -- build it first: make $1"
    return 1
  }
  printf '%s' "$d"
}
...
bc_prestrip() {
  ...
  for app in "${BC_APPS[@]}"; do
    ...
    [ -f "$dir/build/$app.hex" ] || {
      bc_say "missing $dir/build/$app.hex -- run: make $app"
      return 1
    }
...
bc_phase_negative_control() {
  ...
  for i in "${!BC_NAMES[@]}"; do
    ...
    bc_payload "$name" "$barrier" 0 \
      "RA8_BENCH_NEGATIVE_CONTROL=1 bash scripts/hil/bench_unguarded_probe.sh 15" |
...
bc_phase_death() {
  ...
  bc_payload "$victim" "$barrier" "$(bc_seconds "$BC_WAIT")" \
    "bash scripts/hil/bench_hold_and_work.sh 90" |
...
bc_phase_fairness() {
  ...
  for i in "${!BC_NAMES[@]}"; do
    name="${BC_NAMES[$i]}"
    round="bash scripts/hil/bench_hold_and_work.sh 8"
    bc_payload "$name" "$barrier" "$(bc_seconds "$BC_WAIT")" \
