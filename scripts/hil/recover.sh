...
#   /bin/bash -p scripts/hil/recover.sh uart_hello
#   /bin/bash -p scripts/hil/recover.sh uart_hello --expect "hello, ra8d2!"
#   /bin/bash -p scripts/hil/recover.sh uart_hello --uart /dev/cu.usbmodem001... --expect "hello"
#
# Exit codes:
...
if [[ "$-" == *p* ]]; then
  ...
  ra8_bench_require_recovery "recovery flash of ${APP_ID}" 30m || exit $?
  ...
  internal_try_flash() {
    ...
    ssh "$PI_HOST" /bin/bash -p <<REMOTE >"$log" 2>&1 || true
set -uo pipefail
TMP=\$(mktemp)
trap 'rm -f "\$TMP"' EXIT
cat > "\$TMP" <<JLINK
...
timeout ${TIMEOUT_S} /bin/bash -p -c '
