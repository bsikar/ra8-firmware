...
#   /bin/bash -p scripts/hil/usb_test.sh                  # both FS and HS
#   /bin/bash -p scripts/hil/usb_test.sh --only fs
#   /bin/bash -p scripts/hil/usb_test.sh --only hs
#   /bin/bash -p scripts/hil/usb_test.sh --quick          # short correctness only
#
# Exit codes:
...
if [[ "$-" == *p* ]]; then
  ...
  ra8_bench_require "hil usb test $*" 30m || exit $?
  HUB_PORT_USBHS=1
  HUB_PORT_USBFS=4
  ENUM_WAIT_S=35
  ...
  _usb_flash_and_enumerate() {
    ...
    /bin/bash -p scripts/hil/flash.sh "${app}" >/dev/null

    echo -e "${YELLOW}[USB-${name}]${NC} step 2/3 -- power-cycle + wait for enumeration"
    for attempt in 1 2 3 4 5; do
      /bin/bash -p scripts/hil/tapo.sh board cycle >/dev/null
      ...
      # state machine.
      /bin/bash -p scripts/hil/ppps.sh cycle "${hub_port}" >/dev/null 2>&1 || true
    done
  ...
  _usb_reenumerate() {
    ...
    for attempt in $(seq 1 "${PPPS_REENUM_RETRIES}"); do
      /bin/bash -p scripts/hil/ppps.sh ${soft_arg[@]+"${soft_arg[@]}"} cycle "${hub_port}" \
        >/dev/null 2>&1 || true
      if wait_for_enum "${vidpid}" "${hub_port}" "${ENUM_WAIT_S}"; then
