...
#
# The bench-side helpers (hil_run_direct.sh / hil_jlink_memprobe.sh /
# hil_check_alive.sh) flash + verify on the Pi instrument host (`$PI_HOST`) and read
# /dev/ttyACM0 with Linux-only tools (udevadm, setsid, stdbuf, stty -F,
# timeout). This local helper mirrors their exact J-Link command sequences and
...
if [[ "$-" == *p* ]]; then
  ...
  [[ -n "$APP_DIR" && -n "$APP" ]] || {
    echo -e "${RED}[local]${NC} app '${APP_ID}' not found"
    exit 2
  }
  ...
  if [[ ! -f "$HEX" || ! -f "$ELF" ]]; then
    echo -e "${YELLOW}[local]${NC} building ${APP_ID}..."
  ...
  # Resolve, build, and validate configuration before taking the physical bench.
  # The live hold then spans every probe, flash, and UART operation.
  # shellcheck source=scripts/hil/lib/bench_lock.sh
  source "$_hil_dir/lib/bench_lock.sh"
  ra8_bench_require "hil local gate ${APP_ID}" 30m || exit $?
...
EOF
  ...
  # Mode: uart_scrape
  # =============================================================================
  _run_uart_provision_wifi() { # _run_uart_provision_wifi <log> <timeout-seconds>
    local log="$1" timeout_s="$2" provision_ready=0
    local provision_deadline=$((SECONDS + timeout_s))
    local provision_provider_timeout="$timeout_s"
    if ((provision_provider_timeout > 600)); then
      provision_provider_timeout=600
    fi
    while ((SECONDS < provision_deadline)); do
      if grep -qaF "ra8_net_provision: READY v1" "$log" 2>/dev/null; then
        provision_ready=1
        break
      fi
      sleep 0.1
    ...
    if ((provision_ready == 0)); then
      echo -e "${RED}[local]${NC} runtime Wi-Fi provisioning prompt was not observed" >&2
      return 1
    fi
    if ! python3 "$ROOT/scripts/secrets/wifi_provision.py" \
      --timeout "$provision_provider_timeout" emit |
      python3 "$ROOT/scripts/hil/uart_write.py" --baud 115200 "$UART"; then
      echo -e "${RED}[local]${NC} runtime Wi-Fi provisioning failed" >&2
      return 1
    fi
    echo -e "${YELLOW}[local]${NC} runtime Wi-Fi configuration sent"
  }
  ...
  run_uart_scrape() {
    ...
    if [[ "${HIL_PROVISION_WIFI:-0}" == "1" ]] &&
      ! _run_uart_provision_wifi "$log" "$timeout_s"; then
      stop_reader
      return 1
    fi
