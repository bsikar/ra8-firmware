...
#
# Usage: bash scripts/hil/hid_test.sh --app <app-name> [--vidpid VVVV:PPPP]
...
if ! bash "${SCRIPT_DIR}/flash.sh" "${APP}" >/dev/null 2>&1; then
...
pi_run "sudo -n dmesg -C" >/dev/null 2>&1 || true
bash "${SCRIPT_DIR}/ppps.sh" --soft cycle 4 >/dev/null 2>&1 || true
sleep "${ENUM_WAIT_S}"
DM="$(pi_run "sudo -n dmesg -T 2>/dev/null" || true)"
