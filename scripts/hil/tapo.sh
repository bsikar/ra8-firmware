...
#   /bin/bash -p scripts/hil/tapo.sh <board|pi|relay> [status|on|off|cycle]
#
# Exit codes:
#   0  -- command succeeded
...
if [[ "$-" == *p* ]]; then
  ...
  source "$_hil_dir/lib/python_env.sh"
  ...
  run_local() {
    ...
    local py
    py="$(ra8_hil_python "$ROOT" 2>/dev/null)" || py=""
    [[ -x "$py" ]] ||
      {
        echo -e "${RED}[ERROR]${NC} locked HIL Python is absent; run just setup locally or apply the hil_bench role"
        exit 2
      }
  ...
  if [[ "$TARGET" == "pi" || "$TARGET" == "relay" ]]; then
    ...
    ssh -o ConnectTimeout=5 -o BatchMode=yes "$PI_HOST" "test -x /opt/ra8-hil-python/bin/python3" 2>/dev/null ||
      {
        echo -e "${RED}[ERROR]${NC} locked HIL Python is absent on ${PI_HOST}; apply the hil_bench role"
        exit 2
      }
    ...
    ssh "$PI_HOST" "cd $REMOTE_DIR/scripts/hil && /opt/ra8-hil-python/bin/python3 tapo_control.py board $CMD; rc=\$?; rm -rf $REMOTE_DIR; exit \$rc" ||
      {
