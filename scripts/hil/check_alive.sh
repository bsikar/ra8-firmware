...
#
# Usage:
#   /bin/bash -p scripts/hil/check_alive.sh --hex /path/to/<app>.hex \
#                                   [--boot-seconds 2] \
#                                   [--app-name <name>]
#
...
if [[ "$-" == *p* ]]; then
  ...
  ra8_bench_require "hil alive-check $*" || exit $?

  # Keep direct bench-side developer/operator invocation working. In that case
  # we must not `ssh $PI_HOST`: the local process already has access to the same
  # files and devices. CI itself runs on the dev-box listener and takes the
  # remote branch below.
  LOCAL_PI=0
