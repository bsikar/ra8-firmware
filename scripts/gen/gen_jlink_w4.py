...
"""Emit a J-Link script that programs MRAM with w4 and starts without a reset.
...
def main() -> None:
    """Emit the J-Link commander script for one binary at one base address.

    Writes the script to stdout; the caller pipes it to JLinkExe. Exits 1 on
    a usage error and returns None otherwise -- there is no partial-success
    state, since the script is either fully emitted or not at all.
    """
