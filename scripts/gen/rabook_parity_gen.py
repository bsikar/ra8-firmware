...
"""Generate the rabook_compile byte-identity parity fixture headers.

The on-device compiler (rabook_compile_from_epub) must emit a RABOOK1 flat
blob byte-identical to the desktop reference tools/epub_compile/src/epub_compile.py.
...
bakes both sides of that comparison into a C header the host test embeds:
  ...
  * s_parity_epub[]    -- the fixture .epub bytes (written to a RAM FAT volume
                          and opened with epub, the test's compiler input).
...
Run it via `just tools::rabook_golden` after any change to the format, the
...
Usage:
    ...
    rabook_parity_gen.py --color OUT_HEADER
...
The `--color` form bakes rabook_color_parity_fixture.h: a synthetic RGB PNG plus
the golden 4-bpp blob the desktop tool emits for it on the DEFAULT no-downscale
...
# RBKC chunked container (keep in sync with book_container_t in
# apps/shared_libs/book/inc/book.h): b"RBKC" + <I chunk_bytes + <Q total + <I count
...
def _build_epub(src_dir: Path) -> bytes:
    ...
    and epub read the members by name, so STORED is read identically.
    """
...
def _compile_desktop(epub_bytes: bytes, *, no_images: bool = False) -> bytes:
    ...
    tool = repo / "tools" / "epub_compile" / "src" / "epub_compile.py"
...
def _render(epub_bytes: bytes, golden: bytes, golden_noimg: bytes) -> str:
    return (
        ...
        "enum : uint32_t {\n"
        f"  k_parity_epub_len = {len(epub_bytes)}U, /**< Length of s_parity_epub in bytes. */\n"
        f"  k_parity_golden_len = {len(golden)}U, "
        "/**< Length of s_parity_golden in bytes. */\n"
        f"  k_parity_golden_noimg_len = {len(golden_noimg)}U, "
        "/**< Length of s_parity_golden_noimg in bytes. */\n"
        "};\n"
    )
...
def _render_realbook(epub_bytes: bytes, golden_noimg: bytes) -> str:
    return (
        ...
        "/**< Length of s_realbook_epub in bytes. */\n"
        f"  k_realbook_golden_noimg_len = {len(golden_noimg)}U, "
        "/**< Length of s_realbook_golden_noimg in bytes. */\n"
        "};\n"
    )
...
def _render_downscale(src: bytes, out_w: int, out_h: int, golden: bytes, sha_hex: str) -> str:
    return (
        ...
        "enum : uint32_t {\n"
        f"  k_ds_src_w = {_DS_SRC_W}U, /**< Source width in pixels. */\n"
        f"  k_ds_src_h = {_DS_SRC_H}U, /**< Source height in pixels. */\n"
        f"  k_ds_max_edge = {_DS_MAX_EDGE}U, /**< Maximum output edge in pixels. */\n"
        f"  k_ds_out_w = {out_w}U, /**< Downscaled width in pixels. */\n"
        f"  k_ds_out_h = {out_h}U, /**< Downscaled height in pixels. */\n"
        f"  k_ds_golden_len = {len(golden)}U, /**< Length of s_ds_golden in bytes. */\n"
        "};\n"
    )
...
def _main_downscale(argv: list[str]) -> int:
    ...
    out = Path(argv[2])
    repo = Path(__file__).resolve().parents[2]
    sys.path.insert(0, str(repo / "tools" / "epub_compile" / "src"))
...
# device share one luma and one quantiser, decode included.
_COLOR_W = 16
_COLOR_H = 16
_COLOR_R_STEP = 1
_COLOR_G_STEP = 5
_COLOR_B_STEP = 11
_COLOR_BYTE_MASK = 0xFF


def _color_source_rgb() -> bytes:
    """Deterministic 16x16 RGB source (row-major, 3 bytes/pixel) for the fixture."""
    out = bytearray()
    for i in range(_COLOR_W * _COLOR_H):
        out.append((i * _COLOR_R_STEP) & _COLOR_BYTE_MASK)
        out.append((i * _COLOR_G_STEP) & _COLOR_BYTE_MASK)
        out.append((i * _COLOR_B_STEP) & _COLOR_BYTE_MASK)
    return bytes(out)


def _render_color(png: bytes, golden: bytes, sha_hex: str, crc: int) -> str:
    return (
        "/**\n"
        ...
        f"{_emit_array('s_color_src_png', png)}"
        "\n"
        "/** @brief Golden 4-bpp packed output (host stb_luma8 + gray4_kernel). */\n"
        f"{_emit_array('s_color_golden', golden)}"
        "\n"
        "/** @brief SHA-256 (hex) of s_color_golden -- the host digest. */\n"
        f'static const char s_color_golden_sha256[] = "{sha_hex}";\n'
        "\n"
        "/** @brief Source dimensions, PNG length, golden length, and golden CRC-32. */\n"
        "enum : uint32_t {\n"
        f"  k_color_img_w = {_COLOR_W}U, /**< Source image width in pixels. */\n"
        f"  k_color_img_h = {_COLOR_H}U, /**< Source image height in pixels. */\n"
        f"  k_color_src_png_len = {len(png)}U, /**< Length of s_color_src_png in bytes. */\n"
        f"  k_color_golden_len = {len(golden)}U, /**< Length of s_color_golden in bytes. */\n"
        f"  k_color_golden_crc32 = 0x{crc:08X}U, /**< CRC-32/ISO-HDLC of s_color_golden. */\n"
        "};\n"
    )


def _main_color(argv: list[str]) -> int:
    if len(argv) != 3:  # noqa: PLR2004 -- --color OUT_HEADER
        sys.stderr.write("usage: rabook_parity_gen.py --color OUT_HEADER\n")
        return 2
    out = Path(argv[2])
    repo = Path(__file__).resolve().parents[2]
    sys.path.insert(0, str(repo / "tools" / "epub_compile" / "src"))
    ...
    rgb = _color_source_rgb()
    png_buf = io.BytesIO()
    Image.frombytes("RGB", (_COLOR_W, _COLOR_H), rgb).save(png_buf, "PNG")
    png = png_buf.getvalue()

    builder = BlobBuilder()
    idx = builder.add_raster_image("cover.png", png)
    golden = builder.images[idx][4]  # packed 4-bpp bytes (default no-downscale path)
    sha_hex = hashlib.sha256(golden).hexdigest()
    crc = zlib.crc32(golden) & 0xFFFFFFFF
    out.write_text(_render_color(png, golden, sha_hex, crc), encoding="ascii")
    sys.stdout.write(
        f"{out}: {_COLOR_W}x{_COLOR_H} RGB -> {len(golden)} B gray4 golden "
        f"(crc32 {crc:08X}, sha256 {sha_hex})\n"
    )
    return 0
...
def main(argv: list[str]) -> int:
    """Generate one of the .rabook byte-identity parity fixtures.

    Four modes -- default, ``--realbook``, ``--downscale`` and ``--color`` -- each
    baking a different input through the compiler and emitting the expected output
    as a C header. The firmware then asserts it reproduces those bytes exactly,
    ...
    if len(argv) >= 2 and argv[1] == "--color":  # noqa: PLR2004 -- mode flag
        return _main_color(argv)
