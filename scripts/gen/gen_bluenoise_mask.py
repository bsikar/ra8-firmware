...
levels; source figures carry 256. ``libs/ra8_gfx/src/internal/root.zig`` breaks
...
a committed Zig table, following the ``rabook_parity_gen.py`` /
...
Usage:
    python3 scripts/gen/gen_bluenoise_mask.py            # rewrite the table
    python3 scripts/gen/gen_bluenoise_mask.py --check    # fail if it drifted
    just tools::bluenoise_update                         # regenerate + format
"""
...
_ROW_BYTES = 16  # Byte literals per emitted source row (zig fmt grid fit).

_OUT = Path(__file__).resolve().parents[2] / "libs" / "ra8_gfx" / "src" / "internal/dither_mask.zig"
...
def _format_rows(texture: list[int]) -> str:
    """Render the texture as zig-fmt-clean 16-per-row hex byte literals."""
    lines: list[str] = []
...
def _render_table() -> str:
    """Produce the full committed Zig text for the blue-noise mask table."""
...
//! do not hand-edit. This file is the single committed copy of the texture:
//! the generator computes it once and emits it here, so there is no second
//! table anywhere that could drift out of step with it.
//!
//! Each byte is a threshold in [0, 255], blue-noise distributed so that the
//! level set {{{{mask < t}}}} is a high-frequency (void-and-cluster) dot pattern
//! for every t -- the property that gives smooth, grain-only dithering with no
//! Bayer cross-hatch. The table is indexed toroidally by
//! `(y & {_DIM - 1}) * {_DIM} + (x & {_DIM - 1})` at absolute panel coordinates, so
//! abutting tiles share one continuous mask phase and never seam.

/// {_DIM}x{_DIM} row-major blue-noise threshold texture ({_N} bytes).
///
/// Entry `[(y & {_DIM - 1}) * {_DIM} + (x & {_DIM - 1})]` is the [0, 255] dither
/// threshold for panel pixel `(x, y)`. Generated, read-only; regenerate via the
/// script above rather than editing.
pub const mask = [_]u8{{
{rows}
}};
"""
...
def _write() -> int:
    """Rewrite the committed mask table in place; return a process exit code."""
    _OUT.write_text(_render_table(), encoding="ascii")
    print(f"gen_bluenoise_mask.py: wrote {_OUT}")
    return 0


def _check() -> int:
    """Fail (exit 1) when the committed table differs from a fresh regenerate."""
    fresh = _render_table()
    try:
    ...
    if current != fresh:
        sys.stderr.write(
            ...
            "Run `just tools::bluenoise_update` and commit the result.\n"
        )
        return 1
...
def main() -> int:
    """Parse arguments and either rewrite or verify the committed mask table."""
    parser = argparse.ArgumentParser(description="Generate the blue-noise dither mask table.")
    parser.add_argument(
        "--check", action="store_true", help="verify the committed table is up to date"
    )
