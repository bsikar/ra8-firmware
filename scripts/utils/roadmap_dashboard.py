...
scripts/utils/roadmap_stats.py:
...
Usage:
    ...
    python3 scripts/utils/roadmap_dashboard.py --check
...
def render_dashboard(drivers: list[Driver]) -> str:
    ...
    out: list[str] = []
    out.append("<!--")
    out.append("  ROADMAP_DASHBOARD.md -- generated by")
    out.append("  scripts/utils/roadmap_dashboard.py.  Do not edit by hand;")
    out.append("  re-run `make dashboard` to refresh.")
    out.append("-->")
    out.append("")
    out.append("# Roadmap Dashboard")
    out.append("")
    out.append(
    ...
    out.append("| --- | --- |")
    out.append(f"| Drivers tracked | {total_drivers} |")
    out.append(f"| DONE | {counts['DONE']} |")
    ...
    out.append("## Per-phase progress")
    out.append("")
    out.append("| Phase | Drivers | DONE | WIP | BLOCKED | TODO | Boxes | Bar |")
    out.append("| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |")
    ...
    # Per-driver detail, grouped by phase.
    out.append("## Per-driver progress")
    out.append("")
    for phase in phase_order:
        out.append(f"### {phase}")
        out.append("")
        out.append("| Driver | Status | Boxes | Bar |")
        out.append("| --- | --- | ---: | --- |")
...
def main(argv: list[str]) -> int:
    ...
    if args.check:
        ...
        if not dashboard_path.exists() or (
            dashboard_path.read_text(encoding="utf-8") != dashboard_md
        ):
        ...
        if changed:
            print(
                "roadmap_dashboard.py: the following outputs are stale:",
                file=sys.stderr,
            )
            for c in changed:
                print(f"  {c}", file=sys.stderr)
            print(
                "  (run `make dashboard` to refresh)",
                file=sys.stderr,
            )
            return 1
        print(
