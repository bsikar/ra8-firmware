...
``../../scripts/flash.sh``        relative -- resolved against the citing file
``scripts/utils/``                trailing slash -- must be a directory
``scripts/hil_*.sh``              glob -- must match at least one path
...
"``scripts/hil_*.sh``" honest after the HIL scripts move, instead of leaving a
...
def _literal_prefix(token: str) -> str:
    ...
    ``scripts/utils/{a,b}.py`` -> ``scripts/utils``.  Used to salvage a
