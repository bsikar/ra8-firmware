...
`scripts/utils/misra_check.sh` (make misra) runs cppcheck + the bundled
...
Usage:
    python3 scripts/utils/misra_ratchet.py --check    # gate (default)
    python3 scripts/utils/misra_ratchet.py --update   # rewrite the baseline
...
def write_baseline(path: Path, counts: Counter[tuple[str, str]]) -> None:
    ...
    lines = [
        ...
        "# Consumed by scripts/utils/misra_ratchet.py --check (CI job: misra).",
...
def main() -> int:
    ...
    if not RESULTS_TSV.is_file():
        print(
            ...
            f"run `bash scripts/utils/misra_check.sh` first."
        )
        return 1
