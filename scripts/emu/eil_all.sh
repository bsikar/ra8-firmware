...
#
# Usage:
#   bash scripts/emu/eil_all.sh                  # every app, -j = ra8_max_jobs
#   bash scripts/emu/eil_all.sh -j 8             # cap parallelism at 8
#   bash scripts/emu/eil_all.sh --only blink     # one app
#   bash scripts/emu/eil_all.sh --mode uart_scrape   # one mode
#   bash scripts/emu/eil_all.sh --serial         # -j 1 (deterministic debug)
#   bash scripts/emu/eil_all.sh --skip-build     # assume .elf files already built
#   bash scripts/emu/eil_all.sh --list           # enumerate apps + modes, no run
#
...
SELF="${REPO_ROOT}/scripts/emu/eil_all.sh"
...
build_shelf_library() {
  ...
  if ! bash "${REPO_ROOT}/scripts/builders/books.sh" --shelf-only >/tmp/eil_books.log 2>&1; then
...
run_one() {
  ...
  if [ "${EIL_SKIP_BUILD:-0}" != "1" ]; then
    if ! make -C "$appdir" build >"${EIL_RUN_DIR}/${app}.build.log" 2>&1; then
