...
#     binary) before any app is judged -- a stale or missing ra8_emulator aborts
...
#     the normal path -- ra8_emulator auto-boots the embedded cpu1 image -- so they
...
#   * ra8_emulator's exit code is captured and factored in: a fault exit can never
...
#
#   1. ra8_emulator's own CPU-time guard (RA8_EMU_WALL_S, a clock() budget).
...
#   scripts/emu/matrix.sh blink dtc_transfer_demo   # explicit subset
#   scripts/emu/matrix.sh --selftest      # assert the classifier, build nothing
#   MATRIX_JOBS=1 scripts/emu/matrix.sh   # serial (identical verdicts, slower)
#   BUILD_TIMEOUT=240 RUN_TIMEOUT=600 MAX_CHUNKS=4000 scripts/emu/matrix.sh
#
...
# ra8_emulator jobs -- usb_printer_vendor and i2c_peripheral_responder exceeded it
...
# an independent ra8_emulator process, and -- because verdicts are bounded by the
...
log() { printf 'ra8_emulator matrix: %s\n' "$*" >&2; }

# Which apps need a ra8_emulator device (a card) attached. Shared with eil_all.sh
...
# The ra8_emulator exit codes this classifier keys on, named so the tests below
# read as intent rather than digits. The full space is in emu_run.h: 0 clean,
...
readonly k_rc_wall=3      # ra8_emulator's own CPU-time guard fired
readonly k_rc_timeout=124 # the outer `timeout` fired

# Classify one run from ra8_emulator's stdout AND its exit code.
classify_run() { # out rc -> OK|FAULT|TRUNCATED|HALT|UNKNOWN
  ...
  #
  # With RA8_EMU_WALL_S=0 the emulator's own guard cannot fire, so in practice only
  ...
  # Belt-and-suspenders: ra8_emulator prints this whenever it cut a run short on
...
app_src_dir() { # name -> source dir on stdout (empty if not found)
  find examples -type d -name "$1" -not -path '*/build/*' \
    -not -path '*/build-emu/*' 2>/dev/null | head -1
}
...
# single-image path" -- and that stopped being true: ra8_emulator gained the
# two-image path (NS-elf load across the BLXNS / VTOR_NS seam), eil_all.sh has
# been driving it through the same `<app>_ns.elf` convention for a while, and
# tz_nsc_cgc_usb passes in the EIL suite on every run.
#
...
# single ELF (ra8_emulator auto-boots it). Detected by a `cpu1_reset_handler`
...
# fix, and the dependency runs the other way too: ra8_emulator's CPU-time guard
...
# instruction stream alone -- identical serially and at any -j. eil_all.sh
# reached the same conclusion for the same reason.
# Build one app and resolve what ra8_emulator needs to boot it. Writes the failing
...
run_one() {
  ...
  local -a dev_args=()
  read -r -a dev_args <<<"$(emu_extra_args "$app")"
  local out rc verdict
  # RA8_EMU_WALL_S=0 DISABLES ra8_emulator's CPU-time guard, leaving the run
  ...
  out="$(RA8_EMU_MAX_CHUNKS="$max_chunks" RA8_EMU_WALL_S=0 \
    timeout "$run_timeout" "$emu" "$elf" ${ns_args[@]+"${ns_args[@]}"} \
  ...
  # number and no plan. scripts/emu/matrix_triage.sh groups these.
  ...
  case "$verdict" in
    ...
    FAULT) printf 'FAULT\tFAULT (rc=%s -- ra8_emulator model gap or firmware bug)%s\n' \
      "$rc" "$note" >"$rf" ;;
...
if [ "${1:-}" = "--build-one" ] || [ "${1:-}" = "--run-one" ]; then
  ...
  emu="${MATRIX_EMU:?worker needs MATRIX_EMU}"
  if [ "$1" = "--build-one" ]; then
    build_one "$2"
  else
    run_one "$2"
...
TRUNCATED	0	  => ra8_emulator TRUNCATED by the wall-clock guard at chunk 2473 of 4000
...
EOF
  ...
  if [ "$sel_fail" -eq 0 ]; then
    echo "scripts/emu/matrix.sh --selftest: OK (truncation is its own state, both directions)"
  fi
  exit "$sel_fail"
fi

# -- Build the emulator, and HARD-CHECK it. -----------------------------------
# If ra8_emulator does not configure + build into a runnable binary, $emu would be
...
emu_cmake_log="$ROOT/build/ra8_emulator_matrix_emu_build.log"
...
if ! cmake --build "$emu_dir/build" -j "$(ra8_max_jobs)" >>"$emu_cmake_log" 2>&1; then
  echo "FATAL: ra8_emulator build failed (see $emu_cmake_log)" >&2
  exit 2
fi
emu="$emu_dir/build/ra8_emulator"
if [ ! -x "$emu" ]; then
  echo "FATAL: ra8_emulator binary missing or not executable: $emu" >&2
...
printf '%s\n' "${apps[@]}" | grep -v '^$' |
  xargs -P "$jobs" -I{} bash "$ROOT/scripts/emu/matrix.sh" --build-one {} || true
log "booting $(find "$run_dir" -name '*.built' | wc -l | tr -d ' ') built example(s), -j $jobs ..."
printf '%s\n' "${apps[@]}" | grep -v '^$' |
  xargs -P "$jobs" -I{} bash "$ROOT/scripts/emu/matrix.sh" --run-one {} || true
...
# emulator-matrix gate), because a bare "any fault fails" rule would have to
