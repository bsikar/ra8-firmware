...
#
# shellcheck disable=SC2154  # the app-class lists come from smoke_apps.sh and ROOT/emu/elf/fail from smoke.sh, both sourced before this file runs
...
# (its Cortex-M33 image rides inside as a .cpu1_image PT_LOAD, so ra8_emulator
# spins up the second engine automatically). RA8_EMU_STOP_ON ends the run at
...
smoke_verdict_dualcore_itm() {
  ...
  dout="$(RA8_EMU_STOP_ON="$dc_stop" RA8_EMU_WALL_S=120 \
    "$emu" "$elf" 2>&1 || true)"
...
# assert the app's full PASS banner. RA8_EMU_STOP_ON ends the run the moment
...
smoke_verdict_import_reader() {
  ...
  iout="$(RA8_EMU_STOP_ON="$want" RA8_EMU_MAX_CHUNKS=4000000 RA8_EMU_WALL_S=300 \
    "$emu" "$elf" --sd "$book_sd_image" 2>&1 || true)"
...
# addressed peripheral and ra8_emulator is the external controller. Bound the run
# (the demo idles forever) and assert ra8_emulator's target summary -- a non-zero
...
smoke_verdict_i3c_target() {
  local app="$1"
  tout="$(RA8_EMU_MAX_CHUNKS=20000 RA8_EMU_WALL_S=60 "$emu" "$elf" 2>&1 || true)"
...
# device reaches CONFIGURED. RA8_EMU_USB_STOP stops the run a short settle
...
smoke_verdict_usb_enum() {
  local app="$1"
  uout="$(RA8_EMU_MAX_CHUNKS=8000 RA8_EMU_USB_STOP=300 RA8_EMU_WALL_S=300 \
    "$emu" "$elf" 2>&1 || true)"
...
# RA8_EMU_USB_STOP still ends the run shortly after enumeration. The SD
...
smoke_verdict_usb_msc_sd() {
  ...
  mout="$({ RA8_EMU_MAX_CHUNKS=60000 RA8_EMU_USB_STOP=300 RA8_EMU_WALL_S=300 \
    "$emu" "$elf" --sd "$msc_sd_image" 2>&1 || true; } | LC_ALL=C tr -cd '[:print:]\n')"
...
# USB host-mode apps: the firmware's host stack enumerates ra8_emulator's virtual
...
# RA8_EMU_STOP_ON stops the run as soon as the success line prints (these
# apps loop forever otherwise). Assert the app's own PASS banner.
smoke_verdict_usb_host() {
  local app="$1"
  hout="$(RA8_EMU_MAX_CHUNKS=20000 RA8_EMU_STOP_ON='PASS' RA8_EMU_WALL_S=300 \
    "$emu" "$elf" 2>&1 || true)"
...
# banner, so RA8_EMU_STOP_ON ends the run as soon as ALL PASS prints.
smoke_verdict_sd_format() {
  ...
  # The SCI8 "TX <n> bytes" total printed by ra8_emulator is the independent
  # completeness signal; this grep is the readable assertion.
  fclean="$({ RA8_EMU_MAX_CHUNKS=40000 RA8_EMU_STOP_ON='ALL PASS' RA8_EMU_WALL_S=300 \
    "$emu" "$elf" --sd-new 64:fat32 2>&1 || true; } | LC_ALL=C tr -cd '[:print:]\n')"
...
smoke_verdict_sd_io() {
  local app="$1"
  want="$(uart_expect "$app")"
  ioclean="$({ RA8_EMU_MAX_CHUNKS=200000 RA8_EMU_WALL_S=300 \
    "$emu" "$elf" --sd-new 64:fat16 2>&1 || true; } | LC_ALL=C tr -cd '[:print:]\n')"
...
# the app reports "modem: sync FAIL", so the model is load-bearing (EIL==HIL).
smoke_verdict_modem() {
  local app="$1"
  want="$(uart_expect "$app")"
  mdclean="$({ RA8_EMU_MAX_CHUNKS=4000000 RA8_EMU_STOP_ON="$want" RA8_EMU_WALL_S=300 \
    "$emu" "$elf" --modem 2>&1 || true; } | LC_ALL=C tr -cd '[:print:]\n')"
...
smoke_verdict_selfpark_banner() {
  local app="$1"
  want="$(uart_expect "$app")"
  spout="$(RA8_EMU_STOP_ON="$want" RA8_EMU_WALL_S=300 "$emu" "$elf" ${extra[@]+"${extra[@]}"} 2>&1 || true)"
...
smoke_verdict_button() {
  local app="$1"
  # The app ran with --button held (see emu_extra_args). gpio_input_demo
...
smoke_verdict_render_assert() {
  local app="$1"
  ppm="$(mktemp)"
  "$emu" "$elf" --ppm "$ppm" ${extra[@]+"${extra[@]}"} >/dev/null 2>&1 || true
  ...
  case " $golden_apps " in
    *" $app "*)
      if python3 "$ROOT/scripts/gen/ereader_golden.py" check \
        --elf "$elf" --emulator "$emu" --golden-dir "$golden_dir" \
...
smoke_build_app() {
  ...
  # emu_extra_args prints zero or more whitespace-separated flags; read them
  ...
  extra=()
  read -r -a extra <<<"$(emu_extra_args "$app")"
  return 0
}
...
# Run an unclassified app on ra8_emulator and capture its output into $out.
#
# Every app with a known banner runs with RA8_EMU_STOP_ON set and the
...
smoke_capture_run() {
  local app="$1"
  # Run the app on ra8_emulator. EVERY app with a known banner runs with the
  # CPU-time wall-clock guard DISABLED (RA8_EMU_WALL_S=0), so the run is
  # bounded only by the deterministic instruction-counted chunk budget and never
  # by host CPU-time. RA8_EMU_STOP_ON ends the run the instant the banner
  ...
  for _t in $(seq 1 "$tick_tries"); do
    if [ -n "$tick_stop" ]; then
      out="$(RA8_EMU_STOP_ON="$tick_stop" RA8_EMU_WALL_S=0 "$emu" "$elf" ${extra[@]+"${extra[@]}"} 2>&1 || true)"
    else
      out="$(RA8_EMU_WALL_S=300 "$emu" "$elf" ${extra[@]+"${extra[@]}"} 2>&1 || true)"
    fi
...
smoke_assert_run() {
  ...
  # is a halt regardless of which symbol it sits in -- ra8_emulator stops on it
