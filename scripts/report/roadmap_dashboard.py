...
"""Generate a dashboard for the closed historical HAL completion record.

This consumer renders archived completion evidence from docs/ROADMAP.md using
the same conventions as scripts/report/roadmap_stats.py. It is not a current
work tracker; live work belongs in GitHub issues and the project board.

    - "## <phase>"   -- phase heading (groups drivers)
    - "### <name>"   -- driver heading
    - "`[<m>]` Status:" line where <m> is one of " x~!"
    - a fenced ``` ... ``` checklist block per driver

The script emits two artifacts:

    1. docs/ROADMAP_DASHBOARD.md
       A markdown historical report with a per-phase completion table
       and a per-driver evidence bar.
...
def _render_header() -> list[str]:
    """Render the do-not-edit banner and historical-record notice."""
    return [
        "<!--",
        "  ROADMAP_DASHBOARD.md -- generated by",
        "  scripts/report/roadmap_dashboard.py.  Do not edit by hand;",
        "  re-run `just docs::dashboard` to refresh.",
        "-->",
        "",
        "# HAL Completion Dashboard (Historical)",
        "",
        "This is a generated view of the closed completion evidence in",
        "`docs/ROADMAP.md`, not a live roadmap or work tracker. Current work",
        "belongs in GitHub issues and the repository project board.",
        "",
        "Bars show `ticked / total` for each archived driver checklist.",
        "",
    ]
...
def _check_up_to_date(
    ...
    if changed:
        ...
        print("  (run `just docs::dashboard` to refresh)", file=sys.stderr)
        return 1
    print(
