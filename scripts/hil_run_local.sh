...
#
# Usage:
#   scripts/hil_run_local.sh <app> [--uart /dev/cu.usbmodemXXXX]
#
# Exit codes:
#   0  PASS
...
usage() { echo "Usage: $0 <app> [--uart /dev/cu.usbmodemXXXX]"; exit 2; }
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        *)  [[ -z "$APP" ]] && APP="$1" || usage; shift ;;
    esac
done
[[ -n "$APP" ]] || usage

command -v JLinkExe          >/dev/null 2>&1 || { echo -e "${RED}[local]${NC} JLinkExe not in PATH"; exit 2; }
command -v arm-none-eabi-objcopy >/dev/null 2>&1 || { echo -e "${RED}[local]${NC} arm-none-eabi-objcopy not in PATH"; exit 2; }
command -v python3           >/dev/null 2>&1 || { echo -e "${RED}[local]${NC} python3 not in PATH"; exit 2; }
...
while IFS= read -r d; do
    if [[ -f "$d/main.c" && "$(basename "$d")" == "$APP" ]]; then APP_DIR="$d"; break; fi
done < <(find "$ROOT/examples" -type d -name "$APP")
[[ -n "$APP_DIR" ]] || { echo -e "${RED}[local]${NC} app '${APP}' not found under examples/"; exit 2; }
...
[[ -f "$HEX" && -f "$ELF" ]] || { echo -e "${RED}[local]${NC} build did not produce ${APP}.{hex,elf}"; exit 1; }

# ---- Load the per-app hil.conf ----------------------------------------------
CONF="${APP_DIR}/hil.conf"
[[ -f "$CONF" ]] || { echo -e "${RED}[local]${NC} no hil.conf in ${APP_DIR}"; exit 2; }
...
need_uart() {
    if [[ -z "$UART" ]]; then
        for d in /dev/cu.usbmodem*; do [[ -e "$d" ]] && { UART="$d"; break; }; done
    fi
    [[ -n "$UART" && -e "$UART" ]] || { echo -e "${RED}[local]${NC} no /dev/cu.usbmodem* VCOM found"; exit 2; }
}
...
run_uart_scrape() {
    ...
    if ! flash_local; then stop_reader; return 1; fi
    local result="TIMEOUT" deadline=$(( SECONDS + timeout_s ))
    while (( SECONDS < deadline )); do
        if grep -qaF "${HIL_EXPECT}" "$log" 2>/dev/null; then result="MATCH"; break; fi
        sleep 0.1
    done
    stop_reader
    echo "--- captured UART ---"
    ...
    if [[ -n "${HIL_EXPECT_NEGATIVE:-}" ]]; then
        local neg; neg="$(grep -aiE -- "${HIL_EXPECT_NEGATIVE}" "$log" | head -3 || true)"
        if [[ -n "$neg" ]]; then
    ...
    if [[ "$result" == "MATCH" ]]; then
        echo -e "${GREEN}[local PASS]${NC} ${APP}: saw '${HIL_EXPECT}'"; return 0
    fi
    echo -e "${RED}[local FAIL]${NC} ${APP}: '${HIL_EXPECT}' not seen within ${timeout_s}s"; return 1
}
...
mem32 0x${saddr} 1"
    ...
    if (( ok == 1 )); then echo -e "${GREEN}[local PASS]${NC} ${APP} (${msg})"; return 0; fi
    echo -e "${RED}[local FAIL]${NC} ${APP} (${msg})"; echo "    (J-Link log: ${log})"; return 1
}
...
q"
    ...
    local cfsr hfsr; cfsr="$(echo "$txt" | grep -iE '^E000ED28' | head -1 | awk '{print $3}' | tr 'a-f' 'A-F')"
    ...
    if [[ -n "$pc1" ]] && command -v arm-none-eabi-addr2line >/dev/null 2>&1; then
        local s1 s2; s1="$(arm-none-eabi-addr2line -e "$ELF" -f "0x${pc1}" 2>/dev/null | head -1)"
        s2="$(arm-none-eabi-addr2line -e "$ELF" -f "0x${pc2}" 2>/dev/null | head -1)"
        if echo "$s1" | grep -qE "$spinner" || echo "$s2" | grep -qE "$spinner"; then
            issues+=("PC in fault spinner: PC1=${s1:-?} PC2=${s2:-?}"); ok=0
        fi
    fi
    if [[ -n "$cfsr" ]] && (( 16#$cfsr != 0 )); then issues+=("CFSR=0x${cfsr} non-zero"); ok=0; fi
    if [[ -n "$hfsr" ]] && (( (16#$hfsr & ~(1<<31)) != 0 )); then issues+=("HFSR=0x${hfsr} real HardFault"); ok=0; fi
    if [[ -s "$uart" ]]; then
        local neg; neg="$(grep -aiE '\b(FAIL|FAILED|panic|NAK|ERROR|HardFault|hardfault|MemFault|BusFault|UsageFault|stack[ _-]?overflow)\b' "$uart" | head -3 || true)"
        [[ -n "$neg" ]] && { issues+=("UART failure banner during boot:"); while IFS= read -r l; do issues+=("      $l"); done <<< "$neg"; ok=0; }
    fi
    if (( ok == 1 )); then
        echo -e "${GREEN}[local PASS]${NC} ${APP} (PC=0x${pc1}->0x${pc2}, CycleCnt 0x${cyc1}->0x${cyc2})"; return 0
    fi
    echo -e "${RED}[local FAIL]${NC} ${APP}"; for i in "${issues[@]}"; do echo "    - $i"; done
    echo "    (J-Link log: ${log})"; return 1
}
...
case "$MODE" in
    ...
    alive)          run_alive ;;
    *) echo -e "${RED}[local]${NC} unsupported HIL_MODE='${MODE}'"; exit 2 ;;
esac
