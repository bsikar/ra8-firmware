...
Run::
    ...
    check_gate_bodies.py --selftest # prove the checker fires on both defects

Exit 0 when every gate body can fail, 1 (listing each offender) otherwise, and
2 when no gate body could be found at all -- a parser that has stopped seeing
its subject must not report a clean tree.
"""
...
def check_bodies(bodies: list[GateBody]) -> list[str]:
    """Apply both structural rules and return one message per violation."""
    errors: list[str] = []
    for gate in bodies:
...
def scan(gate_dir: Path) -> tuple[list[str], int]:
    ...
    for fragment in sorted(gate_dir.glob("*.sh")):
        bodies = parse_gate_bodies(fragment.read_text(encoding="utf-8"), fragment)
        seen += len(bodies)
...
def main() -> int:
    ...
    if errors:
        sys.stderr.write("check_gate_bodies.py: gate bodies that cannot fail:\n\n")
        for error in errors:
            sys.stderr.write(f"  {error}\n\n")
        sys.stderr.write(f"{len(errors)} gate body/bodies swallow a failing command.\n")
        return 1

    print(f"check_gate_bodies.py: clean -- all {seen} gate bodies can fail.")
    return 0
