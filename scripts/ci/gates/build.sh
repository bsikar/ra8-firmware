...
# prevent.
#
# Gates in this file: tools-build, build-cross, docs, docs-site, sbom, roadmap-stats
...
# --- docs-site ------------------------------------------------------------
# #900 / ADR-0005: the ASSEMBLED documentation site, every generator at once.
# The docs gate above owns the legacy wide Doxygen build and its diagram
# fidelity; this one owns the site described by ADR-0005 -- one slot per
# generator (Markdown hub, C ABI Doxygen, Zig autodoc, rustdoc) mounted into a
# single tree by scripts/builders/docs_site.sh.
#
# It exists so a generator that stops working fails CI instead of quietly
# dropping its half of the site. Until this gate landed, the only documentation
# job was docs-publish -- push to main, legacy wide site only -- so a broken
# hub, autodoc or C ABI build went red nowhere.
#
# The builder already fails closed on a failing generator. This gate
# re-asserts the result independently afterwards, so a builder that starts
# reporting success over an empty tree is still caught: a gate that only
# forwards another script's exit status inherits that script's blind spots.
#
# The slot-manifest checker runs FIRST, and --selftest before --check: a
# manifest checker that stopped detecting would otherwise wave through a site
# whose slots no longer match the tree.
#
# graphviz is deliberately NOT required here. Without `dot` the C ABI
# reference degrades to text-only diagrams, which is a fidelity question the
# docs gate already owns and hard-requires; duplicating it here would fail two
# gates with two different reasons for one missing package.
gate_docs_site() (
  set -e
  python3 scripts/checks/check_docs_site_slots.py --selftest
  python3 scripts/checks/check_docs_site_slots.py --check

  # Its own output directory, never the developer-facing build/docs/site: a
  # gate that overwrites the tree someone is serving locally is a surprise,
  # and a stale site/ from an earlier run must not be able to make a broken
  # generator look healthy.
  local out="build/docs-site-gate"
  rm -rf "$out"

  # Generated output is never committed (ADR-0005 keeps every generated byte
  # under build/), so a generator writing into the source tree is a defect in
  # its own right. Compare the working tree before and after rather than after
  # alone, so pre-existing local debris is not read as this build's doing.
  local before after
  before="$(git ls-files --modified --others --exclude-standard | sort)"

  bash scripts/builders/docs_site.sh --out "$out"

  after="$(git ls-files --modified --others --exclude-standard | sort)"
  if [[ "$before" != "$after" ]]; then
    echo "FAIL: the documentation build wrote into the source tree:" >&2
    diff <(printf '%s\n' "$before") <(printf '%s\n' "$after") >&2 || true
    return 1
  fi

  if [[ ! -f "$out/index.html" ]]; then
    echo "FAIL: assembled site has no landing page at $out/index.html" >&2
    return 1
  fi

  # Re-assert every wired slot from the manifest itself. The hub mounts at the
  # site root, so its index.html is the landing page checked just above; the
  # rest must each carry their own entry point or the site has a dead link
  # where a generator's output should be.
  local wired rc=0
  wired="$(python3 -c "import json;print(chr(10).join(s['mount'] for s in json.load(open('config/docs_site_slots.json'))['slots'] if s['state'] == 'wired' and s['mount']))")"
  local mount
  for mount in $wired; do
    if [[ ! -f "$out/$mount/index.html" ]]; then
      echo "FAIL: wired slot mounted at '$mount' has no index.html under $out" >&2
      rc=1
    fi
  done
  return "$rc"
