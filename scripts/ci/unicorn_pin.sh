...
# CPU-emulator version that every board_sim environment must link.
#
...
#
# Pinning makes the decode identical everywhere board_sim runs. 2.1.4 is the
# pin because it is what CI -- the authority (docs/TOOLCHAIN.md) -- already
# links, it decodes the MVE stores real M85 silicon executes, and the board_sim
# smoke / matrix / SIL suite is green on it.
#
...
# board_sim gates when the runtime Unicorn is not the pin). The devcontainer
...
# distro package and board_sim's find_library(unicorn) picks it with no
