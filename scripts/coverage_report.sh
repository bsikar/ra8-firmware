#!/usr/bin/env bash
#
# scripts/coverage_report.sh -- generate an HTML coverage report for the
# host unit-test suite via lcov + genhtml.
#
# This is purely an additional reporting tool layered alongside the
# existing coverage gate (scripts/coverage.sh, which uses gcovr). It
...
# scripts/test-docker.sh. On Linux it runs natively.
#
# Usage:
...
if [[ "$(uname -s)" == "Darwin" ]]; then
  # Run inside the same image scripts/test-docker.sh uses.
  ...
  docker run --rm \
    ...
    bash -lc '
            ...
            bash scripts/coverage_report.sh --in-container
        '
  exit $?
fi
