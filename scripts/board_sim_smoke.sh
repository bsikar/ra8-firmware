...
if [ "${#apps[@]}" -eq 0 ]; then
    ...
    # framebuffer, GPIO LED, SCI UART, GPT+ICU IRQ, and SSIE (I2S). The
    # ThreadX/USBX apps (usb_cdc_echo, threadx_usbx_cdc_demo, ...) drive the
    # hand-rolled exception path on the first context switch, which Unicorn 2.0.1
    # mis-delivers (UC_ERR_EXCEPTION); they run on a newer Unicorn (macOS / a
    # source build) -- pass them explicitly there (e.g.
    # `board_sim_smoke.sh usb_cdc_echo`).
    apps=(blink lcd_color_cycle display_pal_animation bedroom_ui_panel \
        uart_hello gpt_irq_demo ssie_audio_loop)
fi
