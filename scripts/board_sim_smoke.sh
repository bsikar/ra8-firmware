...
if [ "${#apps[@]}" -eq 0 ]; then
    ...
    # framebuffer, GPIO LED, SCI UART, GPT+ICU IRQ, SSIE (I2S), and the CRC unit
    # (crc_demo self-checks hw==sw). The ThreadX/USBX apps (usb_cdc_echo,
    ...
    apps=(blink lcd_color_cycle display_pal_animation bedroom_ui_panel \
        uart_hello gpt_irq_demo ssie_audio_loop crc_demo)
fi
