...
#
# scripts/builders/check_c6_integration.sh -- gate: the offline-checkable
# ESP32-C6 staging and link contract holds.
#
# This is a minimal trusted launcher, not an implementation: the detector is
# the Zig host tool tools/check_c6_integration (#858), which replaced the
# Python scripts/checks/check_c6_integration.py  PATHREF-OK: the predecessor
...
#
# Invoked by the C6 section of scripts/ci/gates/checks.sh.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
TOOL_DIR="${ROOT}/tools/check_c6_integration"
...
if [ -z "${ZIG}" ]; then
  echo "check_c6_integration.sh: FATAL -- zig not found. Set ZIG, or put zig on PATH." >&2
  exit 2
fi
...
RA8_REPO_ROOT="${ROOT}" exec "${TOOL_DIR}/build/bin/check_c6_integration" "$@"
