...
#
# scripts/builders/check_no_driver_asm_guard.sh -- gate: a HAL peripheral
...
#
# Invoked by the source-form gate in scripts/ci/gates/checks.sh.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
TOOL_DIR="${ROOT}/tools/check_no_driver_asm_guard"
...
if [ -z "${ZIG}" ]; then
  echo "check_no_driver_asm_guard.sh: FATAL -- zig not found. Set ZIG, or put zig on PATH." >&2
  exit 2
fi
...
RA8_REPO_ROOT="${ROOT}" exec "${TOOL_DIR}/build/bin/check_no_driver_asm_guard" "$@"
