...
#
# scripts/builders/check_assert_casts.sh -- gate: reject redundant integer
...
#     bash scripts/builders/check_assert_casts.sh tests/foo/test_bar.c
#
...
TOOL_DIR="${ROOT}/tools/check_assert_casts"
...
if [ -z "${ZIG}" ]; then
  echo "check_assert_casts.sh: FATAL -- zig not found. Set ZIG, or put zig on PATH." >&2
  exit 2
fi
...
RA8_REPO_ROOT="${ROOT}" exec "${TOOL_DIR}/build/bin/check_assert_casts" "$@"
