...
#
# scripts/builders/check_no_wave_references.sh -- gate: no session-bookkeeping
# references in first-party files.
#
# This is a minimal trusted launcher, not an implementation: the gate is the
# Zig host tool tools/check_no_wave_references (#858), which replaced the
# Python scripts/checks/check_no_wave_references.py  PATHREF-OK: the
...
TOOL_DIR="${ROOT}/tools/check_no_wave_references"
...
if [ -z "${ZIG}" ]; then
  echo "check_no_wave_references.sh: FATAL -- zig not found. Set ZIG, or put zig on PATH." >&2
  exit 2
fi
...
RA8_REPO_ROOT="${ROOT}" exec "${TOOL_DIR}/build/bin/check_no_wave_references" "$@"
