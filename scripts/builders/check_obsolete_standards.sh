...
#
# scripts/builders/check_obsolete_standards.sh -- gate: no file cites a
# safety standard that has been superseded.
#
# This is a minimal trusted launcher, not an implementation: the gate is the
# Zig host tool tools/check_obsolete_standards (#858), which replaced the
# Python scripts/checks/check_obsolete_standards.py  PATHREF-OK: the
...
#     bash scripts/builders/check_obsolete_standards.sh --staged
#
# Invoked by the checks gate in scripts/ci/gates/checks.sh and by the
# pre-commit recipe in just/hooks.just.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
TOOL_DIR="${ROOT}/tools/check_obsolete_standards"
...
if [ -z "${ZIG}" ]; then
  echo "check_obsolete_standards.sh: FATAL -- zig not found. Set ZIG, or put zig on PATH." >&2
  exit 2
fi
...
RA8_REPO_ROOT="${ROOT}" exec "${TOOL_DIR}/build/bin/check_obsolete_standards" "$@"
